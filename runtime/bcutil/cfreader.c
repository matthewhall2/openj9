...
j9bcutil_readSuperClassFromClassFileBytes(J9PortLibrary *portLib, U_8 *classData, UDATA classDataLength, UDATA *superClassNameLength, U_8 **nameBuffer, UDATA bufferLength)
{
...
#endif
	flags |= (BCT_JavaMaxMajorVersionShifted | BCT_AnyPreviewVersion | BCT_SuperClassOnly);
	...
	if (BCT_ERR_NO_ERROR != readResult) {
		...
		J9CfrClassFile *classFileBuffer = (J9CfrClassFile *)segment;
		J9CfrConstantPoolInfo *constantPool = classFileBuffer->constantPool;
		J9CfrConstantPoolInfo *superClassSlot = &(constantPool[classFileBuffer->superClass]);
		J9CfrConstantPoolInfo *superClassNameSlot = &(constantPool[superClassSlot->slot1]);
		U_8 *nameBytes = superClassNameSlot->bytes;
		UDATA nameLength = superClassNameSlot->slot1;

		if (NULL != nameBytes) {
			if (nameLength > bufferLength) {
				*nameBuffer = j9mem_allocate_memory(nameLength, J9MEM_CATEGORY_CLASSES);
				if (NULL == *nameBuffer) {
					readResult = BCT_ERR_OUT_OF_MEMORY;
					goto free;
				}
			}

			memcpy(*nameBuffer, nameBytes, nameLength);
			*superClassNameLength = nameLength;
		}
	}
