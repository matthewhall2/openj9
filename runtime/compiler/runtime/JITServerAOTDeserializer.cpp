...
JITServerAOTDeserializer::JITServerAOTDeserializer(TR_PersistentClassLoaderTable *loaderTable, J9JITConfig *jitConfig)
    ...
    , _segmentProvider(64 * 1024, classLoadScratchMemoryLimit, classLoadScratchMemoryLimit, _segmentAllocator,
          _rawAllocator)
    ...
    , _numClassSizeMismatches(0)
    , _numClassHashMismatches(0)
    , _threadsToNotifyOnReset(decltype(_threadsToNotifyOnReset)::allocator_type(TR::Compiler->persistentAllocator()))
{
    bool allMonitors = _classLoaderMonitor && _classMonitor && _methodMonitor && _classChainMonitor
        && _wellKnownClassesMonitor && _resetMonitor;
    if (!allMonitors)
        throw std::bad_alloc();
}
...
void JITServerAOTDeserializer::registerThreadToNotifyOnReset(J9VMThread *vmThread)
{
...
void JITServerAOTDeserializer::unregisterThreadToNotifyOnReset(J9VMThread *vmThread)
{
    OMR::CriticalSection rcs(_resetMonitor);
...
TR_Memory &JITServerAOTDeserializer::classLoadTRMemory()
{
    TR_ASSERT(TR::MonitorTable::get()->getClassTableMutex()->owned_by_self(), "Must hold classTableMutex");
    return _classLoadTRMemory;
}

bool JITServerAOTDeserializer::deserializerWasReset(TR_J9VMBase *vm, bool &wasReset)
{
    return vm->getDeserializerWasReset() ? (wasReset = true) : false;
}

bool JITServerAOTDeserializer::deserializationFailure(const SerializedAOTMethod *method,
    const DeserializerContext &context, bool wasReset)
{
    ++_numDeserializationFailures;

    if (TR::Options::getVerboseOption(TR_VerboseJITServer))
        TR_VerboseLog::writeLineLocked(TR_Vlog_JITServer, "ERROR: Failed to deserialize AOT method %s%s",
            method->signature(), wasReset ? " due to concurrent deserializer reset" : "");
    return false;
}

void JITServerAOTDeserializer::reset(TR::CompilationInfoPerThread *compInfoPT)
{
 ...
 */
void JITServerAOTDeserializer::addNewKnownIds(const Vector<uintptr_t> &newIds, const DeserializerContext &context)
{
...
std::vector<uintptr_t> JITServerAOTDeserializer::getNewKnownIds(const DeserializerContext &context)
{
    OMR::CriticalSection cs(_newKnownIdsMonitor);
    bool wasReset = false;
    if (deserializerWasReset(context._fej9, wasReset))
...
static void assertExclusiveVmAccess(J9VMThread *vmThread)
{
...
static void assertSharedVmAccess(J9VMThread *vmThread)
{
    TR_ASSERT((vmThread->publicFlags & J9_PUBLIC_FLAGS_VM_ACCESS) && !vmThread->omrVMThread->exclusiveCount,
 ...
 */
bool JITServerAOTDeserializer::cacheRecords(const uint8_t *records, size_t recordsSize,
    const DeserializerContext &context, bool ignoreFailures, bool &wasReset)
{
...
bool JITServerAOTDeserializer::deserialize(SerializedAOTMethod *method, const std::vector<std::string> &records,
...
void JITServerAOTDeserializer::onClassLoad(J9Class *ramClass, J9VMThread *vmThread)
{
    ...
    try {
        if (auto prefixLength = JITServerHelpers::getGeneratedClassNamePrefixLength(name)) {
            ...
            try {
                ...
                JITServerROMClassHash hash(ramClass->romClass, classLoadTRMemory(),
                    TR_J9VMBase::get(_jitConfig, vmThread), true);
                ...
                if (h_r.second) {
                    ...
                    char buffer[ROMCLASS_HASH_BYTES * 2 + 1];
                    TR_VerboseLog::writeLineLocked(TR_Vlog_JITServer,
                        "ERROR: Duplicate generated class %.*s %p ROMClass %p hash %s",
                        ROMCLASS_NAME((ramClass)->romClass), ramClass, ramClass->romClass,
                        hash.toString(buffer, sizeof(buffer)));
                }
            } catch (...) {
        ...
        if (TR::Options::getVerboseOption(TR_VerboseJITServer))
            TR_VerboseLog::writeLineLocked(TR_Vlog_JITServer,
                "ERROR: AOT deserializer failed to handle class load for %.*s %p: %s", nameLen, nameStr, ramClass,
                e.what());
    }
}

bool JITServerAOTDeserializer::cacheRecord(const AOTSerializationRecord *record, const DeserializerContext &context,
...
J9Class *JITServerAOTDeserializer::findGeneratedClass(J9ClassLoader *loader, const uint8_t *namePrefix,
    size_t namePrefixLength, const JITServerROMClassHash &hash, J9VMThread *vmThread)
{
    assertSharedVmAccess(vmThread);
    OMR::CriticalSection cs(_generatedClassesMonitor);
...
std::string JITServerAOTDeserializer::findGeneratedClassHash(J9ClassLoader *loader, J9Class *ramClass, TR_J9VM *fe,
    J9VMThread *vmThread)
{
    assertSharedVmAccess(vmThread);
...
void JITServerAOTDeserializer::printStats(FILE *f) const
{
    fprintf(f,
        ...
        _numCacheBypasses, _numCacheHits, _numCacheMisses, _numDeserializedMethods, _numDeserializationFailures,
        _numClassSizeMismatches, _numClassHashMismatches);
...
JITServerAOTDeserializer::GeneratedClassMap::~GeneratedClassMap() { jitPersistentFree(_namePrefix); }

bool JITServerAOTDeserializer::isClassMatching(const ClassSerializationRecord *record, J9Class *ramClass,
    const DeserializerContext &context)
{
    int32_t numDimensions = 0;
    // Use base (non-SCC) front-end method to avoid needless validations
    auto baseComponent
        = (J9Class *)TR_J9VMBase::staticGetBaseComponentClass((TR_OpaqueClassBlock *)ramClass, numDimensions);
    ...
    J9ROMClass *packedROMClass = JITServerHelpers::packROMClass((numDimensions ? baseComponent : ramClass)->romClass,
        context._trMemory, context._fej9, packedSize, numDimensions ? 0 : record->romClassSize());
    if (!packedROMClass) {
        ...
        if (TR::Options::getVerboseOption(TR_VerboseJITServer))
            TR_VerboseLog::writeLineLocked(TR_Vlog_JITServer,
                "ERROR: ROMClass size mismatch for class %.*s ID %zu: %zu != %u", RECORD_NAME(record), record->id(),
    ...
    if (numDimensions) {
        auto &arrayHash
            = JITServerROMClassHash::getObjectArrayHash(ramClass->romClass, *context._trMemory, context._fej9);
    ...
    if (hash != record->hash()) {
        if (TR::Options::getVerboseOption(TR_VerboseJITServer)) {
            ...
            TR_VerboseLog::writeLineLocked(TR_Vlog_JITServer,
                "ERROR: ROMClass hash mismatch for class %.*s ID %zu: %s != %s", RECORD_NAME(record), record->id(),
                hash.toString(actual, sizeof(actual)), record->hash().toString(expected, sizeof(expected)));
        }
...
template<typename V0, typename K1>
static void addToMaps(PersistentUnorderedMap<uintptr_t, V0> &map0, PersistentUnorderedMap<K1, uintptr_t> &map1,
    typename PersistentUnorderedMap<uintptr_t, V0>::const_iterator it, uintptr_t id, const V0 &value0, const K1 &key1)
{
...
template<typename V>
V JITServerAOTDeserializer::findInMap(const PersistentUnorderedMap<uintptr_t, V> &map, uintptr_t id,
    TR::Monitor *monitor, TR_J9VMBase *vm, bool &wasReset)
{
...
bool JITServerAOTDeserializer::invalidateGeneratedClass(J9Class *ramClass)
{
...
void JITServerLocalSCCAOTDeserializer::clearCachedData()
{
...
void JITServerLocalSCCAOTDeserializer::invalidateClassLoader(J9VMThread *vmThread, J9ClassLoader *loader)
{
...
void JITServerLocalSCCAOTDeserializer::invalidateClass(J9VMThread *vmThread, J9Class *oldClass, J9Class *newClass)
{
...
J9Class *JITServerLocalSCCAOTDeserializer::getGeneratedClass(J9ClassLoader *loader, uintptr_t romClassSccOffset,
    const DeserializerContext &context)
{
    assertSharedVmAccess(context._vmThread);
...
bool JITServerLocalSCCAOTDeserializer::cacheRecord(const ClassLoaderSerializationRecord *record,
    const DeserializerContext &context, bool &isNew, bool &wasReset)
{
    ...
    auto result
        = getLoaderTable()->lookupClassLoaderAndChainAssociatedWithClassName(record->name(), record->nameLength());
    auto loader = (J9ClassLoader *)result.first;
    void *chain = result.second;
    if (!loader) {
        if (TR::Options::getVerboseOption(TR_VerboseJITServer))
            TR_VerboseLog::writeLineLocked(TR_Vlog_JITServer,
                "ERROR: Failed to find class loader ID %" OMR_PRIuPTR " for first loaded class %.*s", record->id(),
                RECORD_NAME(record));
        return false;
    } else if (!chain) {
        if (TR::Options::getVerboseOption(TR_VerboseJITServer))
    ...
    if (TR::Options::getVerboseOption(TR_VerboseJITServer))
        TR_VerboseLog::writeLineLocked(TR_Vlog_JITServer,
            "Cached class loader record ID %zu -> { %p, %zu } for first loaded class %.*s", record->id(), loader,
            offset, RECORD_NAME(record));
    return true;
}

bool JITServerLocalSCCAOTDeserializer::cacheRecord(const ClassSerializationRecord *record,
    const DeserializerContext &context, bool &isNew, bool &wasReset)
{
    OMR::CriticalSection cs(getClassMonitor());
    if (deserializerWasReset(context._fej9, wasReset))
    ...
    if (!loader) {
        if (TR::Options::getVerboseOption(TR_VerboseJITServer))
            TR_VerboseLog::writeLineLocked(TR_Vlog_JITServer,
                "ERROR: Class loader for class %.*s ID %zu was marked invalid", RECORD_NAME(record), record->id());
        return false;
    }

    // Lookup the RAMClass by name in the class loader, or in the generated classes map if the class is
    // runtime-generated
    ...
    if (!ramClass) {
        if (TR::Options::getVerboseOption(TR_VerboseJITServer))
            TR_VerboseLog::writeLineLocked(TR_Vlog_JITServer,
                "ERROR: Failed to find class %.*s ID %zu in class loader %p", RECORD_NAME(record), record->id(),
                loader);
        return false;
    }
    ...
    if (record->isGenerated())
        _generatedClassesSccMap.insert({
            { loader, offset },
            ramClass
        });
    ...
    if (TR::Options::getVerboseOption(TR_VerboseJITServer))
        TR_VerboseLog::writeLineLocked(TR_Vlog_JITServer,
            "Cached class record ID %zu -> { %p, %zu, %zu } for class %.*s", record->id(), ramClass, offset,
            loaderOffset, RECORD_NAME(record));
    return true;
}

bool JITServerLocalSCCAOTDeserializer::cacheRecord(const MethodSerializationRecord *record,
    const DeserializerContext &context, bool &isNew, bool &wasReset)
{
    ...
    if (TR::Options::getVerboseOption(TR_VerboseJITServer))
        TR_VerboseLog::writeLineLocked(TR_Vlog_JITServer,
            "Cached method record ID %zu -> { %p, %zu } for method %.*s.%.*s%.*s", record->id(), ramMethod, offset,
            ROMCLASS_NAME(ramClass->romClass), ROMMETHOD_NAS(romMethod));
    return true;
}

bool JITServerLocalSCCAOTDeserializer::cacheRecord(const ClassChainSerializationRecord *record,
    const DeserializerContext &context, bool &isNew, bool &wasReset)
{
    ...
    if (TR_SharedCache::INVALID_CLASS_CHAIN_OFFSET == offset) {
        if (TR::Options::getVerboseOption(TR_VerboseJITServer))
            TR_VerboseLog::writeLineLocked(TR_Vlog_JITServer,
                "ERROR: Failed to get class chain ID %zu for class %.*s ID %zu", record->id(),
        ...
        if (romClass != ramClasses[i]->romClass) {
            if (TR::Options::getVerboseOption(TR_VerboseJITServer))
                TR_VerboseLog::writeLineLocked(TR_Vlog_JITServer,
                    ...
                    ROMCLASS_NAME(ramClasses[i]->romClass), record->id(), ROMCLASS_NAME(ramClasses[0]->romClass),
                    record->list().ids()[0]);
            return false;
        }
    }
    ...
    if (TR::Options::getVerboseOption(TR_VerboseJITServer))
        TR_VerboseLog::writeLineLocked(TR_Vlog_JITServer,
            "Cached class chain record ID %zu -> { %p, %zu } for class %.*s ID %zu", record->id(), ramClasses[0],
            offset, ROMCLASS_NAME(ramClasses[0]->romClass), record->list().ids()[0]);
    return true;
}

bool JITServerLocalSCCAOTDeserializer::cacheRecord(const WellKnownClassesSerializationRecord *record,
    const DeserializerContext &context, bool &isNew, bool &wasReset)
{
    ...
    for (size_t i = 0; i < record->list().length(); ++i) {
        chainOffsets[1 + i]
            = getSCCOffset(AOTSerializationRecordType::ClassChain, record->list().ids()[i], context, wasReset);
        if (chainOffsets[1 + i] == (uintptr_t)-1)
            return false;
    }

    // Store the "well-known classes" object in the local SCC or find the existing one
    const void *wkcOffsets = _sharedCache->storeWellKnownClasses(context._vmThread, chainOffsets,
        1 + record->list().length(), record->includedClasses());
    if (!wkcOffsets) {
        if (TR::Options::getVerboseOption(TR_VerboseJITServer))
...
bool JITServerLocalSCCAOTDeserializer::cacheRecord(const ThunkSerializationRecord *record,
    const DeserializerContext &context, bool &isNew, bool &wasReset)
{
    ...
    // the internal synchronization of getJ2IThunk and setJ2IThunk. We use a read barrier here for
    // deserializerWasReset().
    ...
    if (TR::Options::getVerboseOption(TR_VerboseJITServer))
        TR_VerboseLog::writeLineLocked(TR_Vlog_JITServer, "Cached thunk record ID %zu -> for thunk %.*s", record->id(),
            record->signatureSize(), record->signature());
    return true;
}

J9ClassLoader *JITServerLocalSCCAOTDeserializer::getClassLoader(uintptr_t id, uintptr_t &loaderSCCOffset,
    const DeserializerContext &context, bool &wasReset)
{
    OMR::CriticalSection cs(getClassLoaderMonitor());
    if (deserializerWasReset(context._fej9, wasReset))
    ...
    if (TR::Options::getVerboseOption(TR_VerboseJITServer))
        TR_VerboseLog::writeLineLocked(TR_Vlog_JITServer, "Re-cached class loader ID %zu -> { %p, %zu }", id, loader,
            it->second._loaderChainSCCOffset);
    loaderSCCOffset = it->second._loaderChainSCCOffset;
    return loader;
}

J9Class *JITServerLocalSCCAOTDeserializer::getRAMClass(uintptr_t id, const DeserializerContext &context, bool &wasReset)
{
    OMR::CriticalSection cs(getClassMonitor());
    if (deserializerWasReset(context._fej9, wasReset))
        return NULL;
    ...
    J9Class *ramClass
        = jitGetClassInClassloaderFromUTF8(context._vmThread, loader, (char *)J9UTF8_DATA(name), J9UTF8_LENGTH(name));
    if (!ramClass) {
    ...
    if (TR::Options::getVerboseOption(TR_VerboseJITServer))
        TR_VerboseLog::writeLineLocked(TR_Vlog_JITServer, "Re-cached class ID %zu -> { %p, %zu, %zu }", id, ramClass,
            it->second._romClassSCCOffset, it->second._loaderChainSCCOffset);
    return ramClass;
}

uintptr_t JITServerLocalSCCAOTDeserializer::getSCCOffset(AOTSerializationRecordType type, uintptr_t id,
    const DeserializerContext &context, bool &wasReset)
{
    switch (type) {
        case ClassLoader: {
            uintptr_t offset = findInMap(_classLoaderIdMap, id, getClassLoaderMonitor(), context._fej9, wasReset)
                                   ._loaderChainSCCOffset;
            return wasReset ? (uintptr_t)-1 : offset;
        }
        case Class: {
            uintptr_t offset
                = findInMap(_classIdMap, id, getClassMonitor(), context._fej9, wasReset)._romClassSCCOffset;
        ...
        case WellKnownClasses: {
            uintptr_t offset
                = findInMap(_wellKnownClassesMap, id, getWellKnownClassesMonitor(), context._fej9, wasReset);
            return wasReset ? (uintptr_t)-1 : offset;
        }
        default:
...
bool JITServerLocalSCCAOTDeserializer::updateSCCOffsets(SerializedAOTMethod *method, const DeserializerContext &context,
    bool &wasReset, bool &usesSVM)
{
    ...
    TR_ASSERT_FATAL((header->offsetToRelocationDataItems != 0) || (method->numRecords() == 0),
        "Unexpected %zu serialization records in serialized method %s with no relocation data", method->numRecords(),
        method->signature());
    ...
    for (size_t i = 0; i < method->numRecords(); ++i) {
        ...
        uintptr_t sccOffset
            = getSCCOffset(serializedOffset.recordType(), serializedOffset.recordId(), context, wasReset);
        ...
        TR_ASSERT_FATAL((ptr >= start + sizeof(uintptr_t) /*skip the size word*/) && (ptr < end),
            "Out-of-bounds relocation data offset %zu in serialized method %s", serializedOffset.reloDataOffset(),
            method->signature());
#if defined(DEBUG)
        if (TR::Options::getVerboseOption(TR_VerboseJITServer))
            TR_VerboseLog::writeLineLocked(TR_Vlog_JITServer,
                "Updating SCC offset %zu -> %zu for record type %u ID %zu at relo data offset %zu in serialized method "
                "%s",
...
void JITServerNoSCCAOTDeserializer::invalidateClassLoader(J9VMThread *vmThread, J9ClassLoader *loader)
{
    assertExclusiveVmAccess(vmThread);
    ...
    if (TR::Options::getVerboseOption(TR_VerboseJITServer))
        TR_VerboseLog::writeLineLocked(TR_Vlog_JITServer,
            "Invalidated class loader %p ID %zu in the deserializer cache", loader, id);
}

void JITServerNoSCCAOTDeserializer::invalidateClass(J9VMThread *vmThread, J9Class *oldClass, J9Class *newClass)
{
    assertExclusiveVmAccess(vmThread);
    ...
    if (TR::Options::getVerboseOption(TR_VerboseJITServer))
        TR_VerboseLog::writeLineLocked(TR_Vlog_JITServer, "Invalidated RAMClass %p ID %zu in the deserializer cache",
            oldClass, id);
}

void JITServerNoSCCAOTDeserializer::invalidateMethod(J9Method *method)
{
    ...
    if (TR::Options::getVerboseOption(TR_VerboseJITServer))
        TR_VerboseLog::writeLineLocked(TR_Vlog_JITServer, "Invalidated RAMMethod %p ID %zu in the deserializer cache",
            method, id);
}

J9Class *JITServerNoSCCAOTDeserializer::getGeneratedClass(J9ClassLoader *loader, uintptr_t romClassSccOffset,
    const DeserializerContext &context)
{
    ...
    if (wasReset) {
        context._comp->failCompilation<J9::AOTDeserializerReset>("Deserializer reset during relocation of method %s",
            context._comp->signature());
    }
    return ramClass;
}

void JITServerNoSCCAOTDeserializer::clearCachedData()
{
    _classLoaderIdMap.clear();
    _classLoaderPtrMap.clear();
...
bool JITServerNoSCCAOTDeserializer::cacheRecord(const ClassLoaderSerializationRecord *record,
    const DeserializerContext &context, bool &isNew, bool &wasReset)
{
    OMR::CriticalSection cs(getClassLoaderMonitor());
    if (deserializerWasReset(context._fej9, wasReset))
    ...
    // Lookup the class loader using the name of the first class that it loaded
    auto loader = (J9ClassLoader *)getLoaderTable()->lookupClassLoaderAssociatedWithClassName(record->name(),
        record->nameLength());
    if (!loader) {
        if (TR::Options::getVerboseOption(TR_VerboseJITServer))
            TR_VerboseLog::writeLineLocked(TR_Vlog_JITServer,
    ...
    if (TR::Options::getVerboseOption(TR_VerboseJITServer))
        TR_VerboseLog::writeLineLocked(TR_Vlog_JITServer,
            "Cached class loader record ID %zu -> { %p } for first loaded class %.*s", record->id(), loader,
            RECORD_NAME(record));
    return true;
}

bool JITServerNoSCCAOTDeserializer::cacheRecord(const ClassSerializationRecord *record,
    const DeserializerContext &context, bool &isNew, bool &wasReset)
{
    OMR::CriticalSection cs(getClassMonitor());
    if (deserializerWasReset(context._fej9, wasReset))
    ...
    auto loader
        = findInMap(_classLoaderIdMap, record->classLoaderId(), getClassLoaderMonitor(), context._fej9, wasReset);
    if (!loader) {
        if (TR::Options::getVerboseOption(TR_VerboseJITServer))
            TR_VerboseLog::writeLineLocked(TR_Vlog_JITServer,
                "ERROR: Class loader for class %.*s ID %zu was marked invalid", RECORD_NAME(record), record->id());
    ...
    if (!record->isGenerated() && !isClassMatching(record, ramClass, context)) {
        ...
        //    same RAM class in future deserializations of this class record. (We don't report this class record ID as
        //    being new to the server if we fail to cache it, so the server will keep sending us this exact class
        //    record).
        // 2. If ramClass ever gets unloaded, we want invalidateClass to be able to remove both entries from the maps,
        // so that
        //    in subsequent deserializations we can attempt to find a new candidate ramClass that might end up matching
        //    after all.
    ...
    if (TR::Options::getVerboseOption(TR_VerboseJITServer))
        TR_VerboseLog::writeLineLocked(TR_Vlog_JITServer, "Cached class record ID %zu -> { %p, %zu } for class %.*s",
            record->id(), ramClass, record->classLoaderId(), RECORD_NAME(record));
    return true;
}

bool JITServerNoSCCAOTDeserializer::cacheRecord(const MethodSerializationRecord *record,
    const DeserializerContext &context, bool &isNew, bool &wasReset)
{
    OMR::CriticalSection cs(getMethodMonitor());
    if (deserializerWasReset(context._fej9, wasReset))
    ...
    if (it != _methodIdMap.end()) {
        if (it->second) {
            ...
            if (TR::Options::getVerboseOption(TR_VerboseJITServer))
                TR_VerboseLog::writeLineLocked(TR_Vlog_JITServer, "ERROR: Mismatching or unloaded method ID %zu",
                    record->id());
            return false;
        }
    }
    ...
    // that case.
    auto ramClass
        = findInMap(_classIdMap, record->definingClassId(), getClassMonitor(), context._fej9, wasReset)._ramClass;
    if (!ramClass)
        return false;
    ...
    if (TR::Options::getVerboseOption(TR_VerboseJITServer))
        TR_VerboseLog::writeLineLocked(TR_Vlog_JITServer,
            "Cached method record ID %zu -> { %p, %zu } for method %.*s.%.*s%.*s", record->id(), ramMethod,
            record->definingClassId(), ROMCLASS_NAME(ramClass->romClass), ROMMETHOD_NAS(romMethod));
    return true;
}

void JITServerNoSCCAOTDeserializer::getRAMClassChain(const DeserializerContext &context, J9Class *clazz,
    J9Class **chainBuffer, size_t &chainLength)
{
    ...
    *cursor++ = clazz;
    ...
    TR_ASSERT((cursor - chainBuffer) == chainLength, "Invalid RAM class chain length: %zu != %zu", cursor - chainBuffer,
        chainLength);
}

// Add a value (which must have been allocated with TR::Compiler->persistentGlobalMemory()->allocatePersistentMemory) to
// the given map
template<typename K, typename V, typename H>
static void addToChainMap(PersistentUnorderedMap<K, V *, H> &map,
...
J9Class *JITServerNoSCCAOTDeserializer::getRAMClass(uintptr_t id, const DeserializerContext &context, bool &wasReset)
{
    OMR::CriticalSection cs(getClassMonitor());
    if (deserializerWasReset(context._fej9, wasReset))
        return NULL;
...
bool JITServerNoSCCAOTDeserializer::cacheRecord(const ClassChainSerializationRecord *record,
    const DeserializerContext &context, bool &isNew, bool &wasReset)
{
    OMR::CriticalSection cs(getClassChainMonitor());
    if (deserializerWasReset(context._fej9, wasReset))
    ...
    auto firstClass
        = findInMap(_classIdMap, record->list().ids()[0], getClassMonitor(), context._fej9, wasReset)._ramClass;
    if (!firstClass)
        return false;
    ...
    if (record->list().length() != ramClassChainLength) {
        if (TR::Options::getVerboseOption(TR_VerboseJITServer))
            TR_VerboseLog::writeLineLocked(TR_Vlog_JITServer,
                "ERROR: Class chain length mismatch for class %.*s ID %zu: %zu != %zu",
                ROMCLASS_NAME(firstClass->romClass), record->list().ids()[0], ramClassChainLength,
                record->list().length());
        return false;
    }
    ...
    for (size_t i = 0; i < ramClassChainLength; ++i) {
        auto ramClass
            = findInMap(_classIdMap, record->list().ids()[i], getClassMonitor(), context._fej9, wasReset)._ramClass;
        if (!ramClass) {
            if (TR::Options::getVerboseOption(TR_VerboseJITServer))
                TR_VerboseLog::writeLineLocked(TR_Vlog_JITServer,
        ...
        if (ramClass != ramClassChain[i]) {
            if (TR::Options::getVerboseOption(TR_VerboseJITServer))
                TR_VerboseLog::writeLineLocked(TR_Vlog_JITServer,
                    "ERROR: Class %.*s mismatch in class chain ID %zu for class %.*s ID %zu",
                    ROMCLASS_NAME(ramClassChain[i]->romClass), record->id(), ROMCLASS_NAME(firstClass->romClass),
                    record->list().ids()[0]);
            return false;
        }
    }
    ...
    size_t chainSize = (record->list().length() + 1)
        * sizeof(uintptr_t); // offset chains are expected to store their size in bytes in their first entry
    ...
    if (TR::Options::getVerboseOption(TR_VerboseJITServer))
        TR_VerboseLog::writeLineLocked(TR_Vlog_JITServer,
            "Cached class chain record ID %zu -> { %p } for class %.*s ID %zu", record->id(), firstClass,
            ROMCLASS_NAME(firstClass->romClass), record->list().ids()[0]);
    return true;
}

bool JITServerNoSCCAOTDeserializer::cacheRecord(const WellKnownClassesSerializationRecord *record,
    const DeserializerContext &context, bool &isNew, bool &wasReset)
{
    OMR::CriticalSection cs(getWellKnownClassesMonitor());
    if (deserializerWasReset(context._fej9, wasReset))
    ...
    // Initialize the "well-known classes" object (see SymbolValidationManager::populateWellKnownClasses()).
    TR_ASSERT(record->list().length() <= WELL_KNOWN_CLASS_COUNT, "Too many well-known classes");
    // The first entry of the chain offsets contains the number of entries (and not the size in bytes as with class
    // chains)
    auto chainOffsets = (uintptr_t *)TR::Compiler->persistentGlobalMemory()->allocatePersistentMemory(
        (1 + record->list().length()) * sizeof(uintptr_t));
...
bool JITServerNoSCCAOTDeserializer::cacheRecord(const ThunkSerializationRecord *record,
    const DeserializerContext &context, bool &isNew, bool &wasReset)
{
    TR_ASSERT_FATAL(context._comp, "Should not be trying to cache a ThunkSerializationRecord outside a compilation\n");
    ...
    uint8_t *thunkStart = TR_JITServerRelocationRuntime::copyDataToCodeCache(record->thunkStart(), record->thunkSize(),
        fej9vm, comp->codeCacheKind());
    if (!thunkStart)
    ...
    void *vmHelper
        = j9ThunkVMHelperFromSignature(fej9vm->_jitConfig, record->signatureSize(), (char *)record->signature());
    compInfoPT->reloRuntime()->reloTarget()->performThunkRelocation(reinterpret_cast<uint8_t *>(thunkAddress),
        (uintptr_t)vmHelper);
...
bool JITServerNoSCCAOTDeserializer::updateSCCOffsets(SerializedAOTMethod *method, const DeserializerContext &context,
    bool &wasReset, bool &usesSVM)
{
    // NOTE: Defining class chain record is validated by now; there is no corresponding SCC offset to be updated
...
bool JITServerNoSCCAOTDeserializer::revalidateRecord(AOTSerializationRecordType type, uintptr_t id, TR_J9VMBase *vm,
    bool &wasReset)
{
    switch (type) {
        case ClassLoader: {
        ...
        case ClassChain: {
            ...
            for (size_t i = 0; i < chainLength; ++i) {
                auto ramClass
                    = findInMap(_classIdMap, offsetId(chainData[i]), getClassMonitor(), vm, wasReset)._ramClass;
                if (!ramClass) {
        ...
        case WellKnownClasses: {
            ...
            for (size_t i = 0; i < chainLength; ++i) {
                auto classChain
                    = findInMap(_classChainMap, offsetId(chainData[i]), getClassChainMonitor(), vm, wasReset);
                if (!classChain) {
                    TR::Compiler->persistentGlobalMemory()->freePersistentMemory(it->second);
                    it->second = NULL;
                    if (TR::Options::getVerboseOption(TR_VerboseJITServer))
...
J9ROMClass *JITServerNoSCCAOTDeserializer::romClassFromOffsetInSharedCache(uintptr_t offset,
    const DeserializerContext &context, bool &wasReset)
{
...
J9Class *JITServerNoSCCAOTDeserializer::classFromOffset(uintptr_t offset, const DeserializerContext &context,
    bool &wasReset)
{
...
void *JITServerNoSCCAOTDeserializer::pointerFromOffsetInSharedCache(uintptr_t offset,
    const DeserializerContext &context, bool &wasReset)
{
...
J9ROMMethod *JITServerNoSCCAOTDeserializer::romMethodFromOffsetInSharedCache(uintptr_t offset,
    const DeserializerContext &context, bool &wasReset)
{
