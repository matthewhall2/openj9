...
struct TR_IPHashedCallSite {
    ...
    TR_IPHashedCallSite()
        : _method(NULL)
        , _offset(0) {};
    TR_IPHashedCallSite(J9Method *method, uint32_t offset)
        : _method(method)
        , _offset(offset) {};
...
#define TR_IPBC_PERSISTENT_ENTRY_READ \
    0x1 // used to check if the persistent entry has been read, if so we want to avoid the overhead introduced by
...
public:
    ...
    TR_IPBytecodeHashTableEntry(uintptr_t pc)
        : _next(NULL)
        , _pc(pc)
        , _lastSeenClassUnloadID(-1)
        , _entryFlags(0)
        , _persistFlags(IPBC_ENTRY_CAN_PERSIST_FLAG)
        , _overflowFlags(0)
    {}
    ...
    virtual TR_IPBytecodeHashTableEntry *newEntry(TR_PersistentMemory *persistentMemory,
        TR_Memory::ObjectType tag = TR_Memory::UnknownType) const
        = 0;
    ...
    virtual void createPersistentCopy(TR_J9SharedCache *sharedCache, TR_IPBCDataStorageHeader *storage,
    ...
    bool countersWereReset() const
    {
        return (_overflowFlags & TR_IPBC_COUNTERS_RESET) != 0;
    } // only for CallGraph entries
...
public:
    TR_IPBCDataFourBytes(uintptr_t pc)
        : TR_IPBytecodeHashTableEntry(pc)
        , data(0)
    {}
    ...
    virtual uint32_t *getDataReference()
    {
        static uint32_t data_copy = data;
        return &data_copy;
    }

    virtual int32_t setData(uintptr_t value, uint32_t freq = 1)
    {
        data = (uint32_t)value;
        return 0;
    }
...
public:
    TR_IPBCDataAllocation(uintptr_t pc)
        : TR_IPBytecodeHashTableEntry(pc)
        , clazz(0)
        , method(0)
        , data(0)
...
public:
    TR_IPBCDataEightWords(uintptr_t pc)
        : TR_IPBytecodeHashTableEntry(pc)
    {
    ...
    {
        return getSumSwitchCount() > 1;
    } // Note: getSumSwitchCount() artificially adds one to the count

    virtual uintptr_t getData(TR::Compilation *comp = NULL) { /*TR_ASSERT(0, "Don't call me, I'm empty"); */ return 0; }

    virtual int32_t setData(uintptr_t value, uint32_t freq = 1) { /*TR_ASSERT(0, "Don't call me, I'm empty");*/
        return 0; }
...
public:
    TR_IPBCDataCallGraph(uintptr_t pc)
        : TR_IPBytecodeHashTableEntry(pc)
    {
        _csInfo.initialize();
    }
...
public:
    TR_IPBCDataDirectCall(uintptr_t pc)
        : TR_IPBytecodeHashTableEntry(pc)
    {
        _callCount = 0;
        _isInvalid = false;
...
#define SAMPLE_CUTOFF \
    120 // this must be correlated with HISTORY_BUFFER_SIZE
...
public:
    ...
    public:
        TR_IPChainedEntry(TR_IPBytecodeHashTableEntry *entry)
            : _next(NULL)
            , _IPentry(entry)
        {}
...
public:
    ...
    */
    virtual void persistIprofileInfo(TR::ResolvedMethodSymbol *methodSymbol, TR_ResolvedMethod *method,
        TR::Compilation *comp); // JITServer: mark virtual
    bool elgibleForPersistIprofileInfo(TR::Compilation *comp) const;

    void persistAllEntries(); // Persists all entries from IProfiler table into the SCC; TODO: check that JITServer does
                              // not execute this
    void traverseIProfilerTableAndCollectEntries(TR_AggregationHT *aggregationHT, J9VMThread *vmThread,
        bool collectOnlyCallGraphEntries = false);

    void checkMethodHashTable();

    // j9method.cpp
    void getFaninInfo(TR_OpaqueMethodBlock *calleeMethod, uint32_t *count, uint32_t *weight,
...
public:
    ...
    virtual TR_IPBytecodeHashTableEntry *profilingSample(uintptr_t pc, uintptr_t data, bool addIt,
        bool isRIData = false, uint32_t freq = 1);
...
private:
    ...
    virtual bool hasSameBytecodeInfo(TR_ByteCodeInfo &persistentByteCodeInfo, TR_ByteCodeInfo &currentByteCodeInfo,
        TR::Compilation *comp);
    virtual void getBranchCounters(TR::Node *node, TR::TreeTop *fallThroughTree, int32_t *taken, int32_t *notTaken,
        TR::Compilation *comp);
    ...
    TR_IPBCDataStorageHeader *getJ9SharedDataDescriptorForMethod(J9SharedDataDescriptor *descriptor,
    ...
    TR_IPBytecodeHashTableEntry *persistentProfilingSample(TR_OpaqueMethodBlock *method, uint32_t byteCodeIndex,
        TR::Compilation *comp, bool *methodProfileExistsInSCC);
    TR_IPBCDataStorageHeader *persistentProfilingSample(TR_OpaqueMethodBlock *method, uint32_t byteCodeIndex,
    ...
    TR_IPBytecodeHashTableEntry *getProfilingEntry(TR_OpaqueMethodBlock *method, uint32_t byteCodeIndex,
        TR::Compilation *);

    TR_IPBCDataCallGraph *getCGProfilingData(TR_ByteCodeInfo &bcInfo, TR::Compilation *comp);
    TR_IPBCDataCallGraph *getCGProfilingData(TR_OpaqueMethodBlock *method, uint32_t byteCodeIndex,
        TR::Compilation *comp);

    J9ROMMethod *findROMMethodFromPC(J9VMThread *vmThread, uintptr_t methodPC, J9ROMClass *&romClass);
    uintptr_t createBalancedBST(TR_IPBytecodeHashTableEntry **ipEntries, int32_t low, int32_t high, uintptr_t memChunk,
        TR_J9SharedCache *sharedCache);
    uintptr_t createBalancedBST(uintptr_t *pcEntries, int32_t low, int32_t high, uintptr_t memChunk,
        TR_J9SharedCache *sharedCache);
    uint32_t walkILTreeForEntries(uintptr_t *pcEntries, uint32_t &numEntries, TR_J9ByteCodeIterator *bcIterator,
        TR_OpaqueMethodBlock *method, TR::Compilation *comp, vcount_t visitCount, int32_t callerIndex,
        TR_BitVector *BCvisit, bool &abort);
