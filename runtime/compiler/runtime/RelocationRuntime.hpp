...
extern J9_CFUNC void printAOTHeaderProcessorFeatures(TR_AOTHeader *aotHeaderAddress, char *buff,
...
struct TR_RelocationError {
    ...
    enum TR_RelocationErrorCode {
        ...
        arrayClassFromComponentClassValidationFailure
            = (22 << RELO_ERRORCODE_SHIFT) | TR_RelocationErrorCodeType::VALIDATION,
        ...
        declaringClassFromFieldOrStaticValidationFailure
            = (27 << RELO_ERRORCODE_SHIFT) | TR_RelocationErrorCodeType::VALIDATION,
        concreteSubclassFromClassValidationFailure
            = (28 << RELO_ERRORCODE_SHIFT) | TR_RelocationErrorCodeType::VALIDATION,
        ...
        virtualMethodFromOffsetValidationFailure
            = (34 << RELO_ERRORCODE_SHIFT) | TR_RelocationErrorCodeType::VALIDATION,
        interfaceMethodFromCPValidationFailure = (35 << RELO_ERRORCODE_SHIFT) | TR_RelocationErrorCodeType::VALIDATION,
        improperInterfaceMethodFromCPValidationFailure
            = (36 << RELO_ERRORCODE_SHIFT) | TR_RelocationErrorCodeType::VALIDATION,
        methodFromClassAndSigValidationFailure = (37 << RELO_ERRORCODE_SHIFT) | TR_RelocationErrorCodeType::VALIDATION,
        stackWalkerMaySkipFramesValidationFailure
            = (38 << RELO_ERRORCODE_SHIFT) | TR_RelocationErrorCodeType::VALIDATION,
        ...
        methodFromSingleInterfaceImplValidationFailure
            = (41 << RELO_ERRORCODE_SHIFT) | TR_RelocationErrorCodeType::VALIDATION,
        methodFromSingleAbstractImplValidationFailure
            = (42 << RELO_ERRORCODE_SHIFT) | TR_RelocationErrorCodeType::VALIDATION,
        ...
        dynamicMethodFromCallsiteIndexValidationFailure
            = (48 << RELO_ERRORCODE_SHIFT) | TR_RelocationErrorCodeType::VALIDATION,
        handleMethodFromCPIndexValidationFailure
            = (49 << RELO_ERRORCODE_SHIFT) | TR_RelocationErrorCodeType::VALIDATION,
        ...
        maxRelocationError = (63 << RELO_ERRORCODE_SHIFT) | TR_RelocationErrorCodeType::NO_RELO_ERROR
    };

    static uint32_t decode(TR_RelocationErrorCode errorCode)
    {
        return static_cast<uint32_t>(errorCode >> RELO_ERRORCODE_SHIFT);
    }
};
...
public:
    ...
    class IsRelocating {
    public:
        IsRelocating(TR_RelocationRuntime *reloRuntime, TR_J9SharedCache *cacheOverride)
    ...
    J9JITExceptionTable *prepareRelocateAOTCodeAndData(J9VMThread *vmThread, TR_FrontEnd *fe,
        TR::CodeCache *aotMCCRuntimeCodeCache, const J9JITDataCacheHeader *cacheEntry, J9Method *theMethod,
        bool shouldUseCompiledCopy, TR::Options *options, TR::Compilation *compilation,
        TR_ResolvedMethod *resolvedMethod, uint8_t *existingCode = NULL, TR_J9SharedCache *cacheOverride = NULL);
    ...
    {
        return (errorCode & TR_RelocationErrorCodeType::VALIDATION);
    }

    const bool isRelocationError(TR_RelocationErrorCode errorCode)
    {
        return (errorCode & TR_RelocationErrorCodeType::RELOCATION);
    }

    static const char *getReloErrorCodeName(TR_RelocationErrorCode errorCode)
    {
        return _reloErrorCodeNames[TR_RelocationError::decode(errorCode)];
    }

private:
    virtual uint8_t *allocateSpaceInCodeCache(UDATA codeSize) { return NULL; }

    virtual uint8_t *allocateSpaceInDataCache(UDATA metaDataSize, UDATA type) { return NULL; }

    virtual void initializeAotRuntimeInfo() {}

    virtual void initializeCacheDeltas() {}

    void relocateAOTCodeAndData(U_8 *tempDataStart, U_8 *oldDataStart, U_8 *codeStart, U_8 *oldCodeStart);
...
public:
    ...
    TR_SharedCacheRelocationRuntime(J9JITConfig *jitCfg)
        ...
        , TR_RelocationRuntime(jitCfg)
    {}

    virtual bool storeAOTHeader(TR_FrontEnd *fe, J9VMThread *curThread);
    static const TR_AOTHeader *getStoredAOTHeaderWithConfig(J9SharedClassConfig *sharedClassConfig,
        J9VMThread *curThread);
    virtual const TR_AOTHeader *getStoredAOTHeader(J9VMThread *curThread);
    virtual TR_AOTHeader *createAOTHeader(TR_FrontEnd *fe);
    virtual bool validateAOTHeader(TR_FrontEnd *fe, J9VMThread *curThread);
    virtual OMRProcessorDesc getProcessorDescriptionFromSCC(J9VMThread *curThread);
...
public:
    ...
    TR_JITServerRelocationRuntime(J9JITConfig *jitCfg)
        : TR_RelocationRuntime(jitCfg)
    {}
    ...
    {
        TR_ASSERT_FATAL(false, "Should not be called in this RelocationRuntime!");
        return false;
    }
