...
UDATA jitX86Handler(J9VMThread *vmThread, U_32 sigType, void *sigInfo)
{
    ...
    if (jitConfig) {
        ...
        if (exceptionTable == NULL) {
             ...
             */
            BOOLEAN isCFGEnabled
                = J9_ARE_ANY_BITS_SET(vmThread->javaVM->sigFlags, J9_SIG_WINDOWS_MITIGATION_POLICY_CFG_ENABLED);
            ...
            if (!isCFGEnabled) {
                switch (sigType) {
                    ...
                    case J9PORT_SIG_FLAG_SIGFPE_INT_DIV_BY_ZERO:
                        ...
                        if (((J9JITExceptionTable *)1) == exceptionTable) {
                            ...
                            *espPtr += 8 * sizeof(UDATA); /* 7 pushes + ret addr */
                            ...
                            /* add 1 so as to move past the instruction so that the exception thrower will -1 like it
                             * would in all other cases. */
                    ...
                    case J9PORT_SIG_FLAG_SIGBUS:
                        ...
                        infoType = j9sig_info(sigInfo, J9PORT_SIG_SIGNAL, J9PORT_SIG_SIGNAL_INACCESSIBLE_ADDRESS,
                            &infoName, &infoValue);
                        if (sigType == J9PORT_SIG_FLAG_SIGSEGV && infoType == J9PORT_SIG_VALUE_ADDRESS) {
                            if (*(UDATA *)infoValue > 0xFFFF) {
                                /* we know where the fault occurred, and it wasn't within the first page. This is an
                                 * unexpected error */
                                break;
                            }
                        }

                        stackMap = jitConfig->jitGetStackMapFromPC(vmThread, vmThread->javaVM, exceptionTable,
                            (UDATA)(eip + 1));
                        if (stackMap) {
                        ...
                        vmThread->jitException = (J9Object *)((UDATA)eip + 1);
                        *eipPtr = (UDATA)(void *)(sigType == J9PORT_SIG_FLAG_SIGSEGV ? jitHandleNullPointerExceptionTrap
                                                                                     : jitHandleInternalErrorTrap);
                        ((J9VMJITRegisterState *)vmThread->entryLocalStorage->jitGlobalStorageBase)->jit_ebp
                            = (UDATA)*ebpPtr;
                        *ebpPtr = (UDATA)vmThread;
             ...
             * to continue
             * CMVC 104332 : do not update the java stack pointer as it is misleading. We just need the stack to be
             * walkable by our diagnostic tools.
             */
...
UDATA jitPPCHandler(J9VMThread *vmThread, U_32 sigType, void *sigInfo)
{
    ...
    if (jitConfig) {
        ...
        if (!exceptionTable && J9PORT_SIG_FLAG_SIGBUS == sigType) {
            ...
            if (exceptionTable) {
                vmThread->jitException
                    = (J9Object *)(*lrPtr); /* the lr points at the instruction after the helper call */
        ...
        if (exceptionTable) {
            if (J9PORT_SIG_FLAG_SIGBUS == sigType) {
                vmThread->jitException
                    = (J9Object *)(*iarPtr + 1); /* add one for symmetry with IA32, handler check subs one */
#if (defined(LINUXPPC64) && !defined(__LITTLE_ENDIAN__)) || defined(AIXPPC)
                *iarPtr = *(UDATA *)((void *)&jitHandleInternalErrorTrap); /* get routine address from TOC entry */
#else
                *iarPtr = (UDATA)((void *)&jitHandleInternalErrorTrap);
                ...
                switch (trapType) {
...
#if (defined(LINUXPPC64) && !defined(__LITTLE_ENDIAN__)) || defined(AIXPPC)
                        *iarPtr = *(UDATA *)((
                            void *)&jitHandleNullPointerExceptionTrap); /* get routine address from TOC entry */
#else
...
#if (defined(LINUXPPC64) && !defined(__LITTLE_ENDIAN__)) || defined(AIXPPC)
                        *iarPtr = *(UDATA *)((
                            void *)&jitHandleArrayIndexOutOfBoundsTrap); /* get routine address from TOC entry */
#else
...
#if (defined(LINUXPPC64) && !defined(__LITTLE_ENDIAN__)) || defined(AIXPPC)
                        *iarPtr = *(UDATA *)((
                            void *)&jitHandleIntegerDivideByZeroTrap); /* get routine address from TOC entry */
#else
                    ...
                    case TRAP_TYPE_ASYNC_CHECK: {
                        UDATA *lrPtr;
                        infoType = j9sig_info(sigInfo, J9PORT_SIG_CONTROL, J9PORT_SIG_CONTROL_POWERPC_LR, &infoName,
                            &infoValue);
                        lrPtr = (UDATA *)infoValue;
...
UDATA jit390Handler(J9VMThread *vmThread, U_32 sigType, void *sigInfo)
{
    ...
    if (jitConfig) {
        ...
        // if it's a SEGFAULT, could be a null pointer exception
            ...
            infoType
                = j9sig_info(sigInfo, J9PORT_SIG_SIGNAL, J9PORT_SIG_SIGNAL_INACCESSIBLE_ADDRESS, &infoName, &infoValue);
            ...
            if (infoType == J9PORT_SIG_VALUE_ADDRESS && *(UDATA *)infoValue <= 0xFFFF) {
                ...
                /* Defect 120395 : Save original value of GPR7 in vm->tempSlot, the trapHandlers will restore it from
                 * there */
                vmThread->tempSlot = (UDATA)(*entryPointRegister);
            } else {
                return J9PORT_SIG_EXCEPTION_CONTINUE_SEARCH;
...
#if defined(LINUX)
            /* Linux handles PD signals differently so we include illegal instruction exceptions (e.g. ZAP exceptions
             * will be SIGILL) */
...
#endif
            {
                U_32 isCEEHDLR = jitConfig->javaVM != NULL
                    && (jitConfig->javaVM->sigFlags & J9_SIG_ZOS_CEEHDLR) == J9_SIG_ZOS_CEEHDLR;
                 ...
                 */
                if (isCEEHDLR) {
                    infoType = j9sig_info(sigInfo, J9PORT_SIG_SIGNAL, J9PORT_SIG_SIGNAL_ZOS_CONDITION_MESSAGE_NUMBER,
                        &infoName, &infoValue);
                 ...
                 * every DAA generated instruction that might throw HW exception is followed by a
                 * TR::InstOpCode::COND_NOP branch (BRC/BRCL) the BRC/BRCL inst contains the OOL code offset to the
                 * instruction. It has to be computed and replace the entryPointRegister.
                 * */
                if (FPE_DECOVF == si_code) {
                     ...
                     * node and create a function call in the OOL path of a JIT-compiled method should exceptions be
                     * thrown. However, pdModifyPrecision under pdChk DAA pattern is a bit different because they were
                     ...
                     * is not installed.
                     *
                     * With vector support, the same tree structure evaluates to VPSOP+VTP, the former throws Decimal
                     * Exception. Since we don't have an OOL path to fall back to, the normal NOP BRC (which contains a
                     * snippet that indirectly points to OOL section start) is not generated. As a result,
                     * isDAACodeException() is not able to find the OOL snippet address and return an ILC of 0.
                     *
...
static UDATA jitAMD64isREXPrefix(U_8 byte) { return (0x40 <= byte && byte <= 0x4f); }
...
static I_64 jitAMD64maskValue(I_64 value, UDATA size)
{
    switch (size) {
        case 64:
            return value;
...
static UDATA jitAMD64isDivInstruction(U_8 *rip)
{
    ...
    if (jitAMD64isREXPrefix(*rip))
        rip++; /* REX Prefix must precede opcode */
                ...
                switch (sigType) {
                    ...
                    case J9PORT_SIG_FLAG_SIGFPE_INT_OVERFLOW:
                        ...
                        *ripPtr = (U_64)((void *)&jitHandleIntegerDivideByZeroTrap);
                        ((J9VMJITRegisterState *)vmThread->entryLocalStorage->jitGlobalStorageBase)->jit_rbp
                            = (UDATA)*rbpPtr;
                    ...
                    case J9PORT_SIG_FLAG_SIGBUS:
                        ...
                        *ripPtr = (U_64)(void *)(sigType == J9PORT_SIG_FLAG_SIGSEGV ? jitHandleNullPointerExceptionTrap
                                                                                    : jitHandleInternalErrorTrap);
                        ((J9VMJITRegisterState *)vmThread->entryLocalStorage->jitGlobalStorageBase)->jit_rbp
                            = (UDATA)*rbpPtr;
                        *rbpPtr = (U_64)vmThread;
    ...
    if (jitConfig) {
        ...
        if (exceptionTable) {
            switch (sigType) {
                case J9PORT_SIG_FLAG_SIGBUS:
                case J9PORT_SIG_FLAG_SIGSEGV:
                    infoType = j9sig_info(sigInfo, J9PORT_SIG_SIGNAL, J9PORT_SIG_SIGNAL_INACCESSIBLE_ADDRESS, &infoName,
                        &infoValue);
                    if (sigType == J9PORT_SIG_FLAG_SIGSEGV && infoType == J9PORT_SIG_VALUE_ADDRESS) {
                    ...
                    vmThread->jitException
                        = (J9Object *)(*pcPtr + 1); /* add one for symmetry with IA32, handler check subs one */
                    *pcPtr = (UDATA)(void *)(sigType == J9PORT_SIG_FLAG_SIGSEGV ? jitHandleNullPointerExceptionTrap
                                                                                : jitHandleInternalErrorTrap);
                    return J9PORT_SIG_EXCEPTION_CONTINUE_EXECUTION;
