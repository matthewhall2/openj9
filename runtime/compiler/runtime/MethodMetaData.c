...
static VMINLINE J9JIT32BitExceptionTableEntry *get32BitNextExceptionTableEntry(
    J9JIT32BitExceptionTableEntry *handlerCursor);
static VMINLINE J9JIT32BitExceptionTableEntry *get32BitNextExceptionTableEntryFSD(
    J9JIT32BitExceptionTableEntry *handlerCursor, UDATA fullSpeedDebug);
static VMINLINE J9JIT16BitExceptionTableEntry *get16BitNextExceptionTableEntry(
    J9JIT16BitExceptionTableEntry *handlerCursor);
static VMINLINE J9JIT16BitExceptionTableEntry *get16BitNextExceptionTableEntryFSD(
...
static VMINLINE void initializeIteratorWithSpecifiedMap(TR_MapIterator *i, J9TR_MethodMetaData *methodMetaData,
    U_8 *map, U_32 mapCount)
{
...
static VMINLINE TR_StackMapTable *initializeMapTable(J9JavaVM *javaVM, J9TR_MethodMetaData *metaData,
    UDATA fourByteOffsets)
{
    ...
    if (i._stackAtlas->numberOfMaps > threshold) {
        ...
        mapTable = (TR_StackMapTable *)j9mem_allocate_memory(
            concreteMapCount * sizeof(TR_MapTableEntry) + sizeof(TR_StackMapTable), J9MEM_CATEGORY_JIT);
...
static VMINLINE TR_StackMapTable *findOrCreateMapTable(J9VMThread *currentThread, J9JavaVM *javaVM,
    J9TR_MethodMetaData *metaData, UDATA fourByteOffsets)
{
    ...
    if ((NULL != currentThread)
        && J9_ARE_NO_BITS_SET(currentThread->privateFlags2, J9_PRIVATE_FLAGS2_ASYNC_GET_CALL_TRACE)) {
        if (metaData->bodyInfo
            && (javaVM->phase == J9VM_PHASE_NOT_STARTUP
                || 0
                    == (javaVM->jitConfig->runtimeFlags
                        & J9JIT_QUICKSTART))) // save footprint during startup in Xquickstart mode
        {
            ...
            if (((U_32)((UDATA)mapTablePtr) == (U_32)-1)
                || ((U_32)((UDATA)mapTablePtr >> 32) == (U_32)-1)) /* check upper and lower word */
            {
                mapTablePtr = 0;
...
static VMINLINE void *currentInlineMap(TR_MapIterator *i) { return i->_currentInlineMap; }

static VMINLINE void *currentStackMap(TR_MapIterator *i) { return i->_currentStackMap; }
...
static VMINLINE void findMapsAtPC(TR_MapIterator *i, UDATA offsetPC, void **stackMap, void **inlineMap,
    UDATA fourByteOffsets)
{
...
static VMINLINE int compareMapTableEntries(UDATA key, TR_MapTableEntry *ent)
{
     ...
     */
    if (key < ent->_lowCodeOffset)
        return -1;
    if (key >= (ent + 1)->_lowCodeOffset)
        return 1;
    return 0;
}
...
static void printMetaData(J9TR_MethodMetaData *methodMetaData)
{
    ...
    for (index = 0; index < iter._stackAtlas->numberOfMaps; ++index) {
        ...
        printf("index: %d map address: %p rangeStartOffset: %" OMR_PRIxPTR " rangeEndOffset: %" OMR_PRIxPTR
               " is inlineMap? %s\n",
...
static void printMapTable(TR_StackMapTable *stackMapTable, U_8 *addressOfFirstMap)
{
    ...
    for (index = 0; index <= stackMapTable->_tableSize; ++index) {
        printf("index: %d map address: %p lowCodeOffset: %" OMR_PRIxPTR " stackMapOffset: %" OMR_PRIxPTR
               " mapCount: %u\n",
...
static void fastwalkDebug(J9TR_MethodMetaData *methodMetaData, UDATA offsetPC, TR_StackMapTable *stackMapTable,
    TR_MapTableEntry *mapTableEntry)
{
    if (methodMetaData && (methodMetaData->flags & JIT_METADATA_IS_STUB))
        ...
        initializeIteratorWithSpecifiedMap(&iter2, methodMetaData,
            (U_8 *)(getFirstStackMap(stackAtlas) + mapTableEntry->_stackMapOffset), mapTableEntry->_mapCount);
...
void jitGetMapsFromPC(J9VMThread *currentThread, J9JavaVM *vm, J9TR_MethodMetaData *methodMetaData, UDATA jitPC,
    void **stackMap, void **inlineMap)
{
    TR_MapIterator i;
    ...
    if (stackMapTable) {
        ...
        for (; index < FASTWALK_CACHESIZE; ++index) {
            ...
            if (!compareMapTableEntries(offsetPC, cachedEntry)) {
                initializeIteratorWithSpecifiedMap(&i, methodMetaData,
                    (U_8 *)(getFirstStackMap(stackAtlas) + cachedEntry->_stackMapOffset), cachedEntry->_mapCount);
...
void *getStackAllocMapFromJitPC(J9VMThread *currentThread, J9TR_MethodMetaData *methodMetaData, UDATA jitPC,
    void *curStackMap)
{
...
void markClassesInInlineRanges(void *methodMetaData, J9StackWalkState *walkState)
{
    ...
    for (i = 0; i < numCallSites; i++) {
        ...
        if (!isPatchedValue(inlinedMethod)) {
            ...
            /*
            walkState->bytecodePCOffset = (IDATA) getCurrentByteCodeIndexAndIsSameReceiver(walkState->jitInfo,
   inlineMap, inlinedCallSite, NULL); #ifdef J9VM_INTERP_STACKWALK_TRACING jitPrintFrameType(walkState, "JIT inline");
   #endif
            */
...
static void setInlineRangeEndOffset(TR_MapIterator *i, I_32 callerIndex, UDATA *endOffset)
{
    ...
    while (getNextMap(i, fourByteOffsets)) {
        *endOffset = i->_rangeEndOffset;
        ...
        if (!i->_nextMap
            || ((TR_ByteCodeInfo *)getByteCodeInfoFromStackMap(i->_methodMetaData, i->_nextMap))->_callerIndex
                != callerIndex)
            break;
    }
}
...
static VMINLINE J9JIT32BitExceptionTableEntry *getNext32BitExceptionDataField(
    J9JIT32BitExceptionTableEntry *handlerCursor, UDATA bytecodePCBytes)
{
    return (J9JIT32BitExceptionTableEntry *)(((U_8 *)(handlerCursor + 1)) + bytecodePCBytes);
}

static VMINLINE J9JIT16BitExceptionTableEntry *getNext16BitExceptionDataField(
    J9JIT16BitExceptionTableEntry *handlerCursor, UDATA bytecodePCBytes)
{
...
void static VMINLINE setJitExceptionHandlerCache(TR_jitExceptionHandlerCache *jitExceptionHandlerCache, UDATA pc,
    J9Class *thrownClass)
{
...
static VMINLINE TR_jitExceptionHandlerCache getJitExceptionHandlerCache(
    TR_jitExceptionHandlerCache *jitExceptionHandlerCache, UDATA pc)
{
...
UDATA jitExceptionHandlerSearch(J9VMThread *currentThread, J9StackWalkState *walkState)
{
    ...
    if (searchCache) {
        ...
        currentThread->jitExceptionHandlerCache = j9mem_allocate_memory(
            JIT_EXCEPTION_HANDLER_CACHE_SIZE * sizeof(TR_jitExceptionHandlerCache), J9MEM_CATEGORY_VM);
    ...
    if (numberOfRanges) {
        ...
        UDATA(*isExceptionTypeCaughtByHandler)
        (J9VMThread *, J9Class *, J9ConstantPool *, UDATA, J9StackWalkState *)
            = walkState->walkThread->javaVM->internalVMFunctions->isExceptionTypeCaughtByHandler;

        /* we subtract one from the jitPC as jitPCs that come in are always pointing at the beginning of the next
         * instruction (ie: the return address from the child call).  In the case of trap handlers, the GP handler has
         * already bumped the PC forward by one, expecting this -1 to happen.
         */
        ...
        if (hasWideExceptions(walkState->jitInfo)) {
            ...
            while (numberOfRanges) {
                if ((deltaPC >= getJit32BitTableEntryStartPC(handlerCursor))
                    && (deltaPC < getJit32BitTableEntryEndPC(handlerCursor))) {
                    ...
                    if (isExceptionTypeCaughtByHandler(walkState->walkThread, (J9Class *)walkState->userData4,
                            UNTAGGED_METHOD_CP(handlerCursor->ramMethod), catchType, walkState)) {
                        if (bytecodePCBytes)
                            walkState->userData1
                                = (void *)(intptr_t)*get32BitByteCodeIndexFromExceptionTable(walkState->jitInfo);
                        walkState->userData2 = (void *)(getJittedMethodStartPC(walkState->jitInfo)
                            + getJit32BitTableEntryHandlerPC(handlerCursor));
            ...
            numberOfRanges &= ~(J9_JIT_METADATA_WIDE_EXCEPTIONS | J9_JIT_METADATA_HAS_BYTECODE_PC);
            while (numberOfRanges) {
                if ((deltaPC >= getJit16BitTableEntryStartPC(handlerCursor))
                    && (deltaPC < getJit16BitTableEntryEndPC(handlerCursor))) {
                    uintptr_t synthetic = 0;
                    ...
                    if (isExceptionTypeCaughtByHandler(walkState->walkThread, (J9Class *)walkState->userData4,
                            walkState->constantPool, catchType, walkState)) {
                        if (bytecodePCBytes)
                            walkState->userData1
                                = (void *)(intptr_t)*get16BitByteCodeIndexFromExceptionTable(walkState->jitInfo);
                        walkState->userData2 = (void *)(getJittedMethodStartPC(walkState->jitInfo)
                            + getJit16BitTableEntryHandlerPC(handlerCursor));
                        walkState->restartPoint = walkState->walkThread->javaVM->jitConfig->runJITHandler;
                        walkState->userData3 = (void *)J9_EXCEPT_SEARCH_JIT_HANDLER;
                        walkState->userData4 = (void *)synthetic;
                        return J9_STACKWALK_STOP_ITERATING;
...
#define J9_AOT_FIX_ENDIAN_INDIRECT(j9dst_priv)            \
    ...
    UDATA numExcptionRanges = ((UDATA)methodMetaData->numExcptionRanges)
...
#define J9_AOT_FIX_ENDIAN_INDIRECT(j9dst_priv)            \
    ...
    UDATA numExcptionRanges = ((UDATA)methodMetaData->numExcptionRanges)
        & ~(J9_JIT_METADATA_WIDE_EXCEPTIONS | J9_JIT_METADATA_HAS_BYTECODE_PC);
    J9JIT16BitExceptionTableEntry *excptEntry16
        = get16BitFirstExceptionDataField(methodMetaData); /*(J9JIT16BitExceptionTableEntry *)(methodMetaData + 1);*/
...
#define J9_AOT_FIX_ENDIAN_HALF(j9dst_priv)                 \
    ...
    J9_AOT_FIX_ENDIAN(methodMetaData -> constantPool) J9_AOT_FIX_ENDIAN(methodMetaData->ramMethod) J9_AOT_FIX_ENDIAN(
        methodMetaData->startPC) J9_AOT_FIX_ENDIAN(methodMetaData->endPC) J9_AOT_FIX_ENDIAN(methodMetaData->endWarmPC)
        J9_AOT_FIX_ENDIAN(methodMetaData->startColdPC)

            J9_AOT_FIX_ENDIAN(methodMetaData->hotness) J9_AOT_FIX_ENDIAN(methodMetaData->totalFrameSize)
                J9_AOT_FIX_ENDIAN_HALF(methodMetaData->slots) J9_AOT_FIX_ENDIAN_HALF(methodMetaData->scalarTempSlots)
                    J9_AOT_FIX_ENDIAN_HALF(methodMetaData->objectTempSlots) J9_AOT_FIX_ENDIAN_HALF(
                        methodMetaData->prologuePushes) J9_AOT_FIX_ENDIAN_HALF(methodMetaData->tempOffset)
                        J9_AOT_FIX_ENDIAN_HALF(methodMetaData->numExcptionRanges) J9_AOT_FIX_ENDIAN(
                            methodMetaData->size) J9_AOT_FIX_ENDIAN(methodMetaData->registerSaveDescription)
...
U_8 getNextDescriptionBit(U_8 **jitDescriptionCursor) { return *((*jitDescriptionCursor)++); }
    ...
    void *stackMap, J9JITStackAtlas *gcStackAtlas)
{
    ...
    */
    ...
    while (i < numDistinctPinningArrays) {
        ...
        J9Object **currPinningArrayCursor = (J9Object **)(((U_8 *)walkState->bp)
            + (offsetOfFirstInternalPtr + (((U_16)currPinningArrayIndex * sizeof(UDATA)))));
        J9Object *oldPinningArrayAddress = *((J9Object **)currPinningArrayCursor);
        J9Object *newPinningArrayAddress;

#ifdef J9VM_INTERP_STACKWALK_TRACING
        swPrintf(walkState, 6,
            "Before object slot walk &address : %p address : %p bp %p offset of first internal ptr %d\n",
            currPinningArrayCursor, oldPinningArrayAddress, walkState->bp, offsetOfFirstInternalPtr);
#endif
        walkState->objectSlotWalkFunction(walkState->walkThread, walkState, currPinningArrayCursor,
            currPinningArrayCursor);
        ...
        if (oldPinningArrayAddress != newPinningArrayAddress)
            displacement
                = walkState->walkThread->javaVM->memoryManagerFunctions->j9gc_objaccess_indexableDataDisplacement(
                    walkState, (J9IndexableObject *)oldPinningArrayAddress,
                    (J9IndexableObject *)newPinningArrayAddress);

        ++(walkState->slotIndex);

#ifdef J9VM_INTERP_STACKWALK_TRACING
        swPrintf(walkState, 6,
            "After object slot walk for pinning array with &address : %p old address %p new address %p displacement "
            "%p\n",
...
#endif
        {
            ...
            while (j < numInternalPtrsForArray) {
                ...
                J9Object **currInternalPtrCursor = (J9Object **)(((U_8 *)walkState->bp)
                    + (offsetOfFirstInternalPtr + (((U_16)internalPtrAuto) * sizeof(UDATA))));
            ...
            if (internalPointersInRegisters) {
                ...
                while (k < numDistinctPinningArrays) {
                    ...
                    if (currPinningArrayIndexForRegister == currPinningArrayIndex) {
                        ...
                        tempJitDescriptionCursorForRegs
                            += numInternalPtrRegsForArray; /* skip over requisite number of bytes */
...
U_8 *getJitDescriptionCursor(void *stackMap, J9StackWalkState *walkState) { return 0; /* deprecated */ }
...
U_8 getVariableLengthSizeOfInternalPtrRegMap(U_8 *internalPtrMapLocation) { return *internalPtrMapLocation; }

U_8 getNumberOfPinningArrays(U_8 *internalPtrMapLocation) { return *(internalPtrMapLocation + 1); }

U_8 *getFirstPinningArray(U_8 *internalPtrMapLocation) { return (internalPtrMapLocation + 2); }
...
U_8 getNumberOfInternalPtrs(U_8 *pinningArrayCursor) { return *(pinningArrayCursor + 1); }

U_8 *getFirstInternalPtr(U_8 *pinningArrayCursor) { return (pinningArrayCursor + 2); }

U_8 *getNextInternalPtr(U_8 *internalPtrCursor) { return (internalPtrCursor + 1); }

static U_32 getStackMapRegisterMap(U_8 *mapBits) { return *(U_32 *)mapBits; }
...
U_8 *getJitStackSlots(J9TR_MethodMetaData *metaData, void *stackMap)
{
    ...
    if ((*(U_32 *)cursor & INTERNAL_PTR_REG_MASK)
        && getJitInternalPointerMap((J9JITStackAtlas *)getJitGCStackAtlas(metaData))) {
        cursor += (*(cursor + 4) + 1);
    }
    cursor += 4;
    return cursor;
...
void jitAddSpilledRegisters(J9StackWalkState *walkState, void *stackMap)
{
    ...
    if (prologuePushes) {
        UDATA *saveCursor = walkState->bp
            - (((UDATA)getJitScalarTempSlots(walkState->jitInfo)) + ((UDATA)getJitObjectTempSlots(walkState->jitInfo))
                + prologuePushes);
...
static VMINLINE J9ConstantPool *getJitConstantPool(J9TR_MethodMetaData *md) { return md->constantPool; }

static VMINLINE J9Method *getJitRamMethod(J9TR_MethodMetaData *md) { return md->ramMethod; }

static VMINLINE UDATA getJittedMethodStartPC(J9TR_MethodMetaData *md) { return md->startPC; }

static VMINLINE UDATA getJittedMethodEndPC(J9TR_MethodMetaData *md) { return md->endPC; }

I_16 getJitTotalFrameSize(J9TR_MethodMetaData *md) { return (I_16)md->totalFrameSize; }

I_16 getJitSlots(J9TR_MethodMetaData *md) { return md->slots; }

I_16 getJitScalarTempSlots(J9TR_MethodMetaData *md) { return md->scalarTempSlots; }

I_16 getJitObjectTempSlots(J9TR_MethodMetaData *md) { return md->objectTempSlots; }

static VMINLINE U_16 getJitProloguePushes(J9TR_MethodMetaData *md) { return md->prologuePushes; }

static VMINLINE I_16 getJitTempOffset(J9TR_MethodMetaData *md) { return md->tempOffset; }

static VMINLINE U_16 getJitNumberOfExceptionRanges(J9TR_MethodMetaData *md) { return md->numExcptionRanges; }

static VMINLINE I_32 getJitExceptionTableSize(J9TR_MethodMetaData *md) { return md->size; }

void *getJitGCStackAtlas(J9TR_MethodMetaData *md) { return md->gcStackAtlas; }

void *getJitInlinedCallInfo(J9TR_MethodMetaData *md) { return md->inlinedCalls; }

U_8 *getJitInternalPointerMap(J9TR_StackAtlas *sa) { return sa->internalPointerMap; }

U_16 getJitNumberOfMaps(J9TR_StackAtlas *sa) { return sa->numberOfMaps; }

U_16 getJitNumberOfMapBytes(J9TR_StackAtlas *sa) { return sa->numberOfMapBytes; }

I_16 getJitParmBaseOffset(J9TR_StackAtlas *sa) { return sa->parmBaseOffset; }

U_16 getJitNumberOfParmSlots(J9TR_StackAtlas *sa) { return sa->numberOfParmSlots; }

static VMINLINE I_16 getJitLocalBaseOffset(J9TR_StackAtlas *sa) { return sa->localBaseOffset; }

static VMINLINE U_32 getJit32BitTableEntryStartPC(J9JIT32BitExceptionTableEntry *te) { return te->startPC; }

static VMINLINE U_32 getJit32BitTableEntryEndPC(J9JIT32BitExceptionTableEntry *te) { return te->endPC; }

static VMINLINE U_32 getJit32BitTableEntryHandlerPC(J9JIT32BitExceptionTableEntry *te) { return te->handlerPC; }

static VMINLINE U_32 getJit32BitTableEntryCatchType(J9JIT32BitExceptionTableEntry *te) { return te->catchType; }

static VMINLINE J9Method *getJit32BitTableEntryRamMethod(J9JIT32BitExceptionTableEntry *te) { return te->ramMethod; }

static VMINLINE U_16 getJit16BitTableEntryStartPC(J9JIT16BitExceptionTableEntry *te) { return te->startPC; }

static VMINLINE U_16 getJit16BitTableEntryEndPC(J9JIT16BitExceptionTableEntry *te) { return te->endPC; }

static VMINLINE U_16 getJit16BitTableEntryHandlerPC(J9JIT16BitExceptionTableEntry *te) { return te->handlerPC; }

static VMINLINE U_16 getJit16BitTableEntryCatchType(J9JIT16BitExceptionTableEntry *te) { return te->catchType; }
...
UDATA hasFourByteOffset(J9TR_MethodMetaData *md) { return md->flags & JIT_METADATA_GC_MAP_32_BIT_OFFSETS; }

UDATA *getTempBase(UDATA *bp, J9TR_MethodMetaData *methodMetaData)
{
    return ((UDATA *)(((U_8 *)bp) + getJitLocalBaseOffset((J9JITStackAtlas *)getJitGCStackAtlas(methodMetaData))))
        + getJitTempOffset(methodMetaData);
}
...
void *getInlinedMethod(void *inlinedCallSite) { return ((TR_InlinedCallSite *)inlinedCallSite)->_methodInfo; }
...
void *getNextInlinedCallSite(J9TR_MethodMetaData *methodMetaData, void *inlinedCallSite)
{
    if (hasMoreInlinedMethods(inlinedCallSite)) {
        return getNotUnloadedInlinedCallSiteArrayElement(methodMetaData,
            ((TR_ByteCodeInfo *)getByteCodeInfo(inlinedCallSite))->_callerIndex);
    }
    return NULL;
}
...
UDATA getCurrentByteCodeIndexAndIsSameReceiver(J9TR_MethodMetaData *methodMetaData, void *stackMap,
    void *currentInlinedCallSite, UDATA *isSameReceiver)
{
...
/* Number of slots pushed between the unwindSP of the JIT frame and the pushed register arguments for recompilation
   resolves.
...
/* Number of slots pushed between the unwindSP of the JIT frame and the pushed resolve arguments for static/special
   method resolves.
...
/* Number of slots pushed between the unwindSP of the JIT frame and the pushed register arguments for virtual/interface
   method resolves.
...
void *preOSR(J9VMThread *currentThread, J9JITExceptionTable *metaData, void *pc)
{
    ...
    */
    ...
    numberOfMapping = *(uint32_t *)callerIndex2OSRCatchBlock;
    callerIndex2OSRCatchBlock += sizeof(uint32_t);
...
J9JITInvokeBasicCallSite *jitGetInvokeBasicCallSiteFromPC(J9VMThread *vmThread, UDATA jitPC)
{
    J9JITExceptionTable *metadata = vmThread->javaVM->jitConfig->jitGetExceptionTableFromPC(vmThread, jitPC);
...
/* New hook for getting a JIT exception table.  This will be used as a pivot to help assimilate the implementation into
 * the JIT. */
