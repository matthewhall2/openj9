...
void *AOTCacheRecord::allocate(size_t size)
{
...
void AOTCacheRecord::free(void *ptr) { TR::Compiler->persistentGlobalMemory()->freePersistentMemory(ptr); }

// Read a single AOT cache record R from a cache file
template<class R> R *AOTCacheRecord::readRecord(FILE *f, const JITServerAOTCacheReadContext &context)
{
    typename R::SerializationRecord header;
    if (1 != fread(&header, sizeof(header), 1, f)) {
        if (TR::Options::getVerboseOption(TR_VerboseJITServer))
            TR_VerboseLog::writeLineLocked(TR_Vlog_JITServer, "AOT cache: Could not read %s record header",
                R::getRecordName());
        return NULL;
    }
    ...
    if (!header.isValidHeader(context)) {
        if (TR::Options::getVerboseOption(TR_VerboseJITServer))
            TR_VerboseLog::writeLineLocked(TR_Vlog_JITServer, "AOT cache: Header for %s record is invalid",
                R::getRecordName());
        return NULL;
    }
        ...
        if (1 != fread((uint8_t *)record->dataAddr() + sizeof(header), variableDataBytes, 1, f)) {
            if (TR::Options::getVerboseOption(TR_VerboseJITServer))
                TR_VerboseLog::writeLineLocked(TR_Vlog_JITServer,
                    "AOT cache: Unable to read variable part of %s record", R::getRecordName());
...
bool AOTSerializationRecord::isValidHeader(AOTSerializationRecordType type) const
{
    return (type == this->type()) && (0 != this->id());
}
...
AOTCacheClassLoaderRecord *AOTCacheClassLoaderRecord::create(uintptr_t id, const uint8_t *name, size_t nameLength)
{
...
ClassSerializationRecord::ClassSerializationRecord(uintptr_t id, uintptr_t classLoaderId,
    const JITServerROMClassHash &hash, uint32_t romClassSize, bool generated, const J9ROMClass *romClass,
    ...
    , _hash(hash)
    , _romClassSize(romClassSize | (generated ? AOTCACHE_CLASS_RECORD_GENERATED : 0))
    , _nameLength(nameLength)
{
...
bool ClassSerializationRecord::isValidHeader(const JITServerAOTCacheReadContext &context) const
{
    return AOTSerializationRecord::isValidHeader(AOTSerializationRecordType::Class)
        && (classLoaderId() < context._classLoaderRecords.size()) && context._classLoaderRecords[classLoaderId()];
}
...
AOTCacheClassRecord::AOTCacheClassRecord(uintptr_t id, const AOTCacheClassLoaderRecord *classLoaderRecord,
    ...
    , _data(id, classLoaderRecord->data().id(), hash, romClassSize, generated, romClass, baseComponent, numDimensions,
          nameLength)
{}

AOTCacheClassRecord::AOTCacheClassRecord(const JITServerAOTCacheReadContext &context,
    const ClassSerializationRecord &header)
    : _classLoaderRecord(context._classLoaderRecords[header.classLoaderId()])
{}

AOTCacheClassRecord *AOTCacheClassRecord::create(uintptr_t id, const AOTCacheClassLoaderRecord *classLoaderRecord,
    const JITServerROMClassHash &hash, uint32_t romClassSize, bool generated, const J9ROMClass *romClass,
    const J9ROMClass *baseComponent, uint32_t numDimensions)
{
    ...
    void *ptr = AOTCacheRecord::allocate(size(nameLength));
    return new (ptr) AOTCacheClassRecord(id, classLoaderRecord, hash, romClassSize, generated, romClass, baseComponent,
        numDimensions, nameLength);
}

void AOTCacheClassRecord::subRecordsDo(const std::function<void(const AOTCacheRecord *)> &f) const
{
...
bool MethodSerializationRecord::isValidHeader(const JITServerAOTCacheReadContext &context) const
{
    return AOTSerializationRecord::isValidHeader(AOTSerializationRecordType::Method)
        && (definingClassId() < context._classRecords.size()) && context._classRecords[definingClassId()];
}

AOTCacheMethodRecord::AOTCacheMethodRecord(uintptr_t id, const AOTCacheClassRecord *definingClassRecord, uint32_t index)
    : _definingClassRecord(definingClassRecord)
    , _data(id, definingClassRecord->data().id(), index)
{}

AOTCacheMethodRecord::AOTCacheMethodRecord(const JITServerAOTCacheReadContext &context,
    const MethodSerializationRecord &header)
    : _definingClassRecord(context._classRecords[header.definingClassId()])
{}

AOTCacheMethodRecord *AOTCacheMethodRecord::create(uintptr_t id, const AOTCacheClassRecord *definingClassRecord,
    uint32_t index)
{
...
void AOTCacheMethodRecord::subRecordsDo(const std::function<void(const AOTCacheRecord *)> &f) const
{
...
template<class R>
static void listClassCopyRecords(uintptr_t *ids, void *subrecordBuffer, uintptr_t id, const R * const *subRecords,
    size_t length)
{
...
// For a list-like AOT cache record with serialization record type D and subrecord type R, given the serialization
// record data and subRecords of such a record, apply the given function to each of the entries of subRecords.
template<class D, class R>
static void listClassSubRecordsDo(const D &data, const R * const *subRecords,
    const std::function<void(const AOTCacheRecord *)> &f)
{
    for (size_t i = 0; i < data.list().length(); ++i)
        f(subRecords[i]);
}

// For a list-like AOT cache record with serialization record type D and subrecord type R, initialize the subRecords
// array of an already-allocated record of that type by copying the matching R pointers from cacheRecords into
// subRecords using the given serialization record data.
template<class D, class R>
static bool listClassSetSubrecordPointers(const D &data, R **subRecords, const Vector<R *> &cacheRecords,
    const char *recordName, const char *subrecordName)
{
    for (size_t i = 0; i < data.list().length(); ++i) {
        uintptr_t id = data.list().ids()[i];
        if ((id >= cacheRecords.size()) || !cacheRecords[id]) {
            if (TR::Options::getVerboseOption(TR_VerboseJITServer))
                TR_VerboseLog::writeLineLocked(TR_Vlog_JITServer, "AOT cache: Invalid %s subrecord: type %s, ID %zu",
                    recordName, subrecordName, id);
            return false;
        }
...
AOTCacheClassChainRecord::AOTCacheClassChainRecord(uintptr_t id, const AOTCacheClassRecord * const *records,
    size_t length)
    : _data(id, length)
{
...
void AOTCacheClassChainRecord::subRecordsDo(const std::function<void(const AOTCacheRecord *)> &f) const
{
    return listClassSubRecordsDo(data(), records(), f);
}

AOTCacheClassChainRecord *AOTCacheClassChainRecord::create(uintptr_t id, const AOTCacheClassRecord * const *records,
    size_t length)
{
...
bool AOTCacheClassChainRecord::setSubrecordPointers(const JITServerAOTCacheReadContext &context)
{
...
AOTCacheWellKnownClassesRecord::AOTCacheWellKnownClassesRecord(uintptr_t id,
    const AOTCacheClassChainRecord * const *records, size_t length, uintptr_t includedClasses)
    : _data(id, length, includedClasses)
{
    listClassCopyRecords(_data.list().ids(), (void *)this->records(), id, records, length);
}

void AOTCacheWellKnownClassesRecord::subRecordsDo(const std::function<void(const AOTCacheRecord *)> &f) const
{
    return listClassSubRecordsDo(data(), records(), f);
}

AOTCacheWellKnownClassesRecord *AOTCacheWellKnownClassesRecord::create(uintptr_t id,
    const AOTCacheClassChainRecord * const *records, size_t length, uintptr_t includedClasses)
{
    void *ptr = AOTCacheRecord::allocate(size(length));
...
bool AOTCacheWellKnownClassesRecord::setSubrecordPointers(const JITServerAOTCacheReadContext &context)
{
    return listClassSetSubrecordPointers(data(), records(), context._classChainRecords, "well-known classes",
        "class chain");
}
...
AOTCacheAOTHeaderRecord *AOTCacheAOTHeaderRecord::create(uintptr_t id, const TR_AOTHeader *header)
{
...
AOTCacheThunkRecord *AOTCacheThunkRecord::create(uintptr_t id, const uint8_t *signature, uint32_t signatureSize,
    const uint8_t *thunkStart, uint32_t thunkSize)
{
...
SerializedAOTMethod::SerializedAOTMethod(uintptr_t definingClassChainId, uint32_t index, TR_Hotness optLevel,
    uintptr_t aotHeaderId, size_t numRecords, size_t numDependencies, const void *code, size_t codeSize,
    const void *data, size_t dataSize, const char *signature, size_t signatureSize)
    : _size(size(numRecords, numDependencies, codeSize, dataSize, signatureSize))
    , _definingClassChainId(definingClassChainId)
    , _index(index)
    , _optLevel(optLevel)
...
SerializedAOTMethod::SerializedAOTMethod()
    : _size(0)
    , _definingClassChainId(0)
    , _index(0)
    , _optLevel(TR_Hotness::numHotnessLevels)
    , _aotHeaderId(0)
    , _numRecords(0)
    , _numDependencies(0)
    , _codeSize(0)
...
bool SerializedAOTMethod::isValidHeader(const JITServerAOTCacheReadContext &context) const
{
    return _optLevel < TR_Hotness::numHotnessLevels && (definingClassChainId() < context._classChainRecords.size())
        && (aotHeaderId() < context._aotHeaderRecords.size()) && context._classChainRecords[definingClassChainId()]
...
CachedAOTMethod::CachedAOTMethod(const AOTCacheClassChainRecord *definingClassChainRecord, uint32_t index,
    ...
    const UnorderedMap<uintptr_t, bool> &dependencies, const void *code, size_t codeSize, const void *data,
    size_t dataSize, const char *signature, size_t signatureSize)
    : _nextRecord(NULL)
    , _data(definingClassChainRecord->data().id(), index, optLevel, aotHeaderRecord->data().id(), records.size(),
          dependencies.size(), code, codeSize, data, dataSize, signature, signatureSize)
...
CachedAOTMethod *CachedAOTMethod::create(const AOTCacheClassChainRecord *definingClassChainRecord, uint32_t index,
    TR_Hotness optLevel, const AOTCacheAOTHeaderRecord *aotHeaderRecord,
    const Vector<std::pair<const AOTCacheRecord *, uintptr_t> > &records,
    const UnorderedMap<uintptr_t, bool> &dependencies, const void *code, size_t codeSize, const void *data,
    size_t dataSize, const char *signature)
{
    ...
    return new (ptr) CachedAOTMethod(definingClassChainRecord, index, optLevel, aotHeaderRecord, records, dependencies,
        code, codeSize, data, dataSize, signature, signatureSize);
}

bool CachedAOTMethod::setSubrecordPointers(const JITServerAOTCacheReadContext &context)
{
    const char *invalidSubrecordName = "";
    size_t subrecordId = 0;

    // The defining class chain record pointer for the record will have been set by the CachedAOTMethod constructor at
    // this point
    for (size_t i = 0; i < data().numRecords(); ++i) {
        ...
        switch (sccOffset.recordType()) {
            ...
            case AOTSerializationRecordType::WellKnownClasses:
                if ((subrecordId >= context._wellKnownClassesRecords.size())
                    || !context._wellKnownClassesRecords[subrecordId]) {
...
error:
    if (TR::Options::getVerboseOption(TR_VerboseJITServer))
        TR_VerboseLog::writeLineLocked(TR_Vlog_JITServer,
            "AOT cache: Invalid cached AOT method subrecord: type %s, ID %zu", invalidSubrecordName, subrecordId);
    return false;
}

bool JITServerAOTCache::ClassKey::operator==(const ClassKey &k) const
{
    return (_classLoaderRecord == k._classLoaderRecord) && (*_hash == *k._hash);
}

size_t JITServerAOTCache::ClassKey::Hash::operator()(const ClassKey &k) const noexcept
{
...
bool JITServerAOTCache::ClassChainKey::operator==(const ClassChainKey &k) const
{
    return (_length == k._length) && (memcmp(_records, k._records, _length * sizeof(_records[0])) == 0);
}

size_t JITServerAOTCache::ClassChainKey::Hash::operator()(const ClassChainKey &k) const noexcept
{
    return recordListHash((const AOTCacheRecord * const *)k._records, k._length);
}

bool JITServerAOTCache::WellKnownClassesKey::operator==(const WellKnownClassesKey &k) const
{
    return (_length == k._length) && (_includedClasses == k._includedClasses)
        && (memcmp(_records, k._records, _length * sizeof(_records[0])) == 0);
}

size_t JITServerAOTCache::WellKnownClassesKey::Hash::operator()(const WellKnownClassesKey &k) const noexcept
{
    return recordListHash((const AOTCacheRecord * const *)k._records, k._length) ^ k._includedClasses;
}

bool JITServerAOTCache::AOTHeaderKey::operator==(const AOTHeaderKey &k) const
{
    return memcmp(_header, k._header, sizeof(*_header)) == 0;
}

size_t JITServerAOTCache::AOTHeaderKey::Hash::operator()(const AOTHeaderKey &k) const noexcept
{
...
static void addToMap(PersistentUnorderedMap<K, V *, H> &map, V *&traversalHead, V *&traversalTail,
    const typename PersistentUnorderedMap<K, V *, H>::const_iterator &it, const K &key, V *value)
{
    try {
...
template<typename K, typename V, typename H>
static bool addToMap(PersistentUnorderedMap<K, V *, H> &map, V *&traversalHead, V *&traversalTail, const K &key,
    V *value)
{
...
template<typename K, typename V, typename H> static void freeMapValues(const PersistentUnorderedMap<K, V *, H> &map)
{
    for (auto &kv : map)
...
JITServerAOTCache::JITServerAOTCache(const std::string &name, J9JavaVM *javaVM)
    ...
    , // protected by the _cachedMethodMonitor
    _excludedFromSavingToFile(false)
    , _numCacheBypasses(0)
    , _numCacheHits(0)
    , _numCacheMisses(0)
    , _numDeserializedMethods(0)
    , _numDeserializationFailures(0)
    , _numGeneratedClasses(0)
{
    bool allMonitors = _classLoaderMonitor && _classMonitor && _methodMonitor && _classChainMonitor
        && _wellKnownClassesMonitor && _aotHeaderMonitor && _cachedMethodMonitor;
    if (!allMonitors)
        throw std::bad_alloc();
}
...
JITServerAOTCacheReadContext::JITServerAOTCacheReadContext(const JITServerAOTCacheHeader &header,
    TR::StackMemoryRegion &stackMemoryRegion)
...
const AOTCacheClassLoaderRecord *JITServerAOTCache::getClassLoaderRecord(const uint8_t *name, size_t nameLength)
{
    ...
    if (TR::Options::getVerboseOption(TR_VerboseJITServer))
        TR_VerboseLog::writeLineLocked(TR_Vlog_JITServer, "AOT cache %s: created class loader ID %zu -> %.*s",
            _name.c_str(), record->data().id(), (int)nameLength, (const char *)name);

    return record;
}

const AOTCacheClassRecord *JITServerAOTCache::getClassRecord(const AOTCacheClassLoaderRecord *classLoaderRecord,
    ...
    if (cache) {
        ...
        if (prefixLength) {
            ...
            if (scratchSegmentProvider) {
                // Called from outside of a compilation. Use supplied scratchSegmentProvider for scratch memory
                // allocations.
                ...
                J9::SystemSegmentProvider segmentProvider(1 << 16 /*64 KB*/, segmentSize,
...
const AOTCacheMethodRecord *JITServerAOTCache::getMethodRecord(const AOTCacheClassRecord *definingClassRecord,
    uint32_t index, const J9ROMMethod *romMethod)
{
    ...
    if (TR::Options::getVerboseOption(TR_VerboseJITServer)) {
        ...
        TR_VerboseLog::writeLineLocked(TR_Vlog_JITServer,
            "AOT cache %s: created method ID %zu -> %.*s.%.*s%.*s index %u class ID %zu", _name.c_str(),
            record->data().id(), RECORD_NAME(c), ROMMETHOD_NAS(romMethod), index, c->id());
    }

    return record;
}

const AOTCacheClassChainRecord *JITServerAOTCache::getClassChainRecord(const AOTCacheClassRecord * const *classRecords,
    size_t length)
{
    ...
    if (TR::Options::getVerboseOption(TR_VerboseJITServer)) {
        ...
        TR_VerboseLog::writeLineLocked(TR_Vlog_JITServer,
            "AOT cache %s: created class chain ID %zu -> %.*s ID %zu length %zu", _name.c_str(), record->data().id(),
            RECORD_NAME(c), c->id(), length);
    }

    return record;
}

const AOTCacheWellKnownClassesRecord *JITServerAOTCache::getWellKnownClassesRecord(
    const AOTCacheClassChainRecord * const *chainRecords, size_t length, uintptr_t includedClasses)
{
    ...
    auto record
        = AOTCacheWellKnownClassesRecord::create(_nextWellKnownClassesId, chainRecords, length, includedClasses);
    ...
    if (TR::Options::getVerboseOption(TR_VerboseJITServer))
        TR_VerboseLog::writeLineLocked(TR_Vlog_JITServer,
            "AOT cache %s: created well-known classes ID %zu -> length %zu includedClasses %zx", _name.c_str(),
            record->data().id(), includedClasses, length);

    return record;
}

const AOTCacheAOTHeaderRecord *JITServerAOTCache::getAOTHeaderRecord(const TR_AOTHeader *header, uint64_t clientUID)
{
    ...
    if (it != _aotHeaderMap.end()) {
        if (TR::Options::getVerboseOption(TR_VerboseJITServer))
            TR_VerboseLog::writeLineLocked(TR_Vlog_JITServer,
                "AOT cache %s: using existing AOT header ID %zu for clientUID %llu", _name.c_str(),
                it->second->data().id(), (unsigned long long)clientUID);
        return it->second;
    }
    ...
    if (TR::Options::getVerboseOption(TR_VerboseJITServer))
        TR_VerboseLog::writeLineLocked(TR_Vlog_JITServer, "AOT cache %s: created AOT header ID %zu for clientUID %llu",
            _name.c_str(), record->data().id(), (unsigned long long)clientUID);

    return record;
}

const AOTCacheThunkRecord *JITServerAOTCache::getThunkRecord(const uint8_t *signature, uint32_t signatureSize)
{
...
const AOTCacheThunkRecord *JITServerAOTCache::createAndStoreThunk(const uint8_t *signature, uint32_t signatureSize,
    const uint8_t *thunkStart, uint32_t thunkSize)
{
    OMR::CriticalSection cs(_thunkMonitor);
    ...
    if (TR::Options::getVerboseOption(TR_VerboseJITServer))
        TR_VerboseLog::writeLineLocked(TR_Vlog_JITServer, "AOT cache %s: created thunk ID %zu -> %.*s thunkSize %u",
            _name.c_str(), record->data().id(), signatureSize, signature, thunkSize);

    return record;
}

bool JITServerAOTCache::storeMethod(const AOTCacheClassChainRecord *definingClassChainRecord, uint32_t index,
    TR_Hotness optLevel, const AOTCacheAOTHeaderRecord *aotHeaderRecord,
    const Vector<std::pair<const AOTCacheRecord *, uintptr_t /*reloDataOffset*/> > &records,
    const UnorderedMap<uintptr_t, bool> &dependencies, const void *code, size_t codeSize, const void *data,
    size_t dataSize, const char *signature, uint64_t clientUID, const CachedAOTMethod *&methodRecord)
{
    ...
    if (!JITServerAOTCacheMap::cacheHasSpace()) {
        if (TR::Options::getVerboseOption(TR_VerboseJITServer))
            TR_VerboseLog::writeLineLocked(TR_Vlog_JITServer,
                "AOT cache %s: method %s @ %s index %u class ID %zu AOT header ID %zu compiled fully but failed to "
                "store due to AOT cache size limit",
    ...
    if (it != _cachedMethodMap.end()) {
        ...
        if (TR::Options::getVerboseOption(TR_VerboseJITServer))
            TR_VerboseLog::writeLineLocked(TR_Vlog_JITServer,
                "AOT cache %s: method %s @ %s index %u class ID %zu AOT header ID %zu already exists", _name.c_str(),
                signature, levelName, index, definingClassId, aotHeaderRecord->data().id());
        return false;
    }

    auto method = CachedAOTMethod::create(definingClassChainRecord, index, optLevel, aotHeaderRecord, records,
        dependencies, code, codeSize, data, dataSize, signature);
    methodRecord = method;
    ...
    if (TR::Options::getVerboseOption(TR_VerboseJITServer))
        TR_VerboseLog::writeLineLocked(TR_Vlog_JITServer,
            ...
            _name.c_str(), signature, levelName, index, definingClassId, aotHeaderRecord->data().id(), records.size(),
            (unsigned long long)clientUID);

    return true;
}

const CachedAOTMethod *JITServerAOTCache::findMethod(const AOTCacheClassChainRecord *definingClassChainRecord,
    uint32_t index, TR_Hotness optLevel, const AOTCacheAOTHeaderRecord *aotHeaderRecord)
{
    CachedMethodKey key(definingClassChainRecord, index, optLevel, aotHeaderRecord);
    OMR::CriticalSection cs(_cachedMethodMonitor);
...
Vector<const AOTSerializationRecord *> JITServerAOTCache::getSerializationRecords(const CachedAOTMethod *method,
    const KnownIdSet &knownIds, TR_Memory &trMemory) const
{
 ...
 */
void JITServerAOTCache::packSerializationRecords(const Vector<const AOTSerializationRecord *> &records, uint8_t *buffer,
    size_t bufferSize)
{
...
void JITServerAOTCache::addRecord(const AOTCacheRecord *record, Vector<const AOTSerializationRecord *> &result,
    UnorderedSet<const AOTCacheRecord *> &newRecords, const KnownIdSet &knownIds) const
{
    ...
    record->subRecordsDo([&](const AOTCacheRecord *r) { addRecord(r, result, newRecords, knownIds); });

    newRecords.insert(record);
    result.push_back(data);
}

void JITServerAOTCache::printStats(FILE *f) const
{
    fprintf(f,
        ...
        _name.c_str(), _cachedMethodMap.size(), _classLoaderMap.size(), _classMap.size(), _numGeneratedClasses,
        _methodMap.size(), _classChainMap.size(), _wellKnownClassesMap.size(), _aotHeaderMap.size(), _numCacheBypasses,
        _numCacheHits, _numCacheMisses, _numDeserializedMethods, _numDeserializationFailures);
}

// Write at most numRecordsToWrite to the given stream from the linked list starting at head.
static bool writeRecordList(FILE *f, const AOTCacheRecord *head, size_t numRecordsToWrite)
{
    ...
    TR_ASSERT(recordsWritten == numRecordsToWrite, "Expected to write %zu records, wrote %zu", numRecordsToWrite,
        recordsWritten);

    return true;
}

static bool writeCachedMethodList(FILE *f, const CachedAOTMethod *head, size_t numRecordsToWrite)
{
...
size_t JITServerAOTCache::writeCache(FILE *f) const
{
...
static bool isCompatibleSnapshotVersion(const JITServerAOTCacheVersion &version)
{
...
JITServerAOTCache *JITServerAOTCache::readCache(FILE *f, const std::string &name, TR_Memory &trMemory)
{
    if (!JITServerAOTCacheMap::cacheHasSpace())
        return NULL;
    ...
    if (!isCompatibleSnapshotVersion(header._version)) {
        if (TR::Options::getVerboseOption(TR_VerboseJITServer))
            TR_VerboseLog::writeLineLocked(TR_Vlog_JITServer,
                "AOT cache: Cache file header incompatible with running server");
        return NULL;
    }
    ...
    try {
        ...
        if (TR::Options::getVerboseOption(TR_VerboseJITServer)) {
            TR_VerboseLog::writeLineLocked(TR_Vlog_JITServer, "AOT cache allocation failed with exception: %s",
                e.what());
        }
    }
...
template<typename K, typename V, typename H>
bool JITServerAOTCache::readRecords(FILE *f, JITServerAOTCacheReadContext &context, size_t numRecordsToRead,
    PersistentUnorderedMap<K, V *, H> &map, V *&traversalHead, V *&traversalTail, Vector<V *> &records)
{
    for (size_t i = 0; i < numRecordsToRead; ++i) {
        ...
        if ((record->data().id() >= records.size() || records[record->data().id()])
            || !addToMap(map, traversalHead, traversalTail, getRecordKey(record), record)) {
            if (TR::Options::getVerboseOption(TR_VerboseJITServer))
                TR_VerboseLog::writeLineLocked(TR_Vlog_JITServer,
                    "AOT cache: Record of type %s has invalid or overlapping ID %zu", V::getRecordName(),
                    record->data().id());
            AOTCacheRecord::free(record);
            return false;
        }
...
bool JITServerAOTCache::readCache(FILE *f, const JITServerAOTCacheHeader &header, TR_Memory &trMemory)
{
    ...
    if (!readRecords(f, context, header._numClassLoaderRecords, _classLoaderMap, _classLoaderHead, _classLoaderTail,
            context._classLoaderRecords))
        return false;
    if (!readRecords(f, context, header._numClassRecords, _classMap, _classHead, _classTail, context._classRecords))
        return false;
    if (!readRecords(f, context, header._numMethodRecords, _methodMap, _methodHead, _methodTail,
            context._methodRecords))
        return false;
    if (!readRecords(f, context, header._numClassChainRecords, _classChainMap, _classChainHead, _classChainTail,
            context._classChainRecords))
        return false;
    if (!readRecords(f, context, header._numWellKnownClassesRecords, _wellKnownClassesMap, _wellKnownClassesHead,
        ...
        return false;
    if (!readRecords(f, context, header._numAOTHeaderRecords, _aotHeaderMap, _aotHeaderHead, _aotHeaderTail,
            context._aotHeaderRecords))
        return false;
    ...
    for (size_t i = 0; i < header._numCachedAOTMethods; ++i) {
        ...
        CachedMethodKey key(record->definingClassChainRecord(), record->data().index(), record->data().optLevel(),
...
size_t JITServerAOTCache::getNumCachedMethods() const
{
    OMR::CriticalSection cs(_cachedMethodMonitor);
    return _cachedMethodMap.size();
}

bool JITServerAOTCache::triggerAOTCacheStoreToFileIfNeeded()
{
    TR::CompilationInfo *compInfo = TR::CompilationInfo::get();
...

    {
        ...
        if (compInfo->getPersistentInfo()->getElapsedTime()
            < _timePrevSaveOperation + TR::Options::_aotCachePersistenceMinPeriodMs)
            return false;
    ...
    bool queuedRequest = false;
    {
        ...
        if (!compInfo->getPersistentInfo()->getDisableFurtherCompilation()) {
            ...
            if (compInfo->addOutOfProcessMethodToBeCompiled(SAVE_AOTCACHE_REQUEST /*stream*/)) {
                ...
                if (TR::Options::getVerboseOption(TR_VerboseJITServer))
                    TR_VerboseLog::writeLineLocked(TR_Vlog_JITServer,
                        "AOT cache: t=%llu Queued comp request to save cache '%s' to file in the background",
...
void JITServerAOTCache::finalizeSaveOperation(bool success, size_t numMethodsSavedToFile)
{
    OMR::CriticalSection cs(_cachedMethodMonitor);
    if (success) {
...
bool JITServerAOTCache::isAOTCacheBetterThanSnapshot(const std::string &cacheFileName, size_t numExtraMethods)
{
    bool doSave = false;
    ...
    if (cacheFile) {
        ...
        if (1 == fread(&header, sizeof(JITServerAOTCacheHeader), 1, cacheFile)) {
            ...
            if (!isCompatibleSnapshotVersion(header._version)) {
                if (TR::Options::getVerboseOption(TR_VerboseJITServer))
                    TR_VerboseLog::writeLineLocked(TR_Vlog_JITServer,
                        "AOT cache: Found incompatible AOT cache file %s. Will overwrite.", cacheFileName.c_str());
                doSave = true;
            } else // Header is compatible, check the number of methods
            {
                ...
                {
                    setMinNumAOTMethodsToSave(
                        header._numCachedAOTMethods + TR::Options::_aotCachePersistenceMinDeltaMethods);
                    if (TR::Options::getVerboseOption(TR_VerboseJITServer))
                        TR_VerboseLog::writeLineLocked(TR_Vlog_JITServer,
                            "AOT cache: Save operation aborted for cache '%s' because we don't have %zu more methods "
                            "than existing snapshot: %zu vs %zu.",
        ...
        } else // Read error
        {
            if (TR::Options::getVerboseOption(TR_VerboseJITServer))
                TR_VerboseLog::writeLineLocked(TR_Vlog_JITServer,
                    "AOT cache: Cannot read header from exiting cache file %s. Will overwrite existing file.",
                    cacheFileName.c_str());
    ...
    } else // Cannot open file
    {
        ...
        if (TR::Options::getVerboseOption(TR_VerboseJITServer))
            TR_VerboseLog::writeLineLocked(TR_Vlog_JITServer, "AOT cache: Cannot open cache file %s for reading: %s",
                cacheFileName.c_str(), strerror(errno));
        doSave = true;
    }
    return doSave;
}

bool JITServerAOTCacheMap::cacheHasSpace()
{
    if (_cacheIsFull) {
        return false;
    }
    ...
    size_t aotTotalRecordAllocations
    ...
    if (aotTotalRecordAllocations >= _cacheMaxBytes) {
        ...
        if (TR::Options::getVerboseOption(TR_VerboseJITServer)) {
            TR_VerboseLog::writeLineLocked(TR_Vlog_JITServer,
                "AOT cache allocations exceeded maximum of %zu bytes, disabling future allocations", _cacheMaxBytes);
        }
        return false;
    } else {
        return true;
...
JITServerAOTCacheMap::JITServerAOTCacheMap()
    ...
    , _cachesExcludedFromLoading(
          decltype(_cachesExcludedFromLoading)::allocator_type(TR::Compiler->persistentGlobalAllocator()))
...
std::string JITServerAOTCacheMap::buildCacheFileName(const std::string &cacheDir, const std::string &cacheName)
{
...
void JITServerAOTCacheMap::queueAOTCacheForSavingToFile(const std::string &cacheName)
{
    OMR::CriticalSection cs(_monitor);
    _cachesToSaveQueue.push_back(cacheName);
}

bool JITServerAOTCacheMap::saveNextQueuedAOTCacheToFile()
{
    ...
    try {
        std::string cacheFileName
            = buildCacheFileName(compInfo->getPersistentInfo()->getJITServerAOTCacheDir(), cacheName);
        ...
        if (cache->isAOTCacheBetterThanSnapshot(cacheFileName, TR::Options::_aotCachePersistenceMinDeltaMethods)) {
            ...
            if (newCacheFile) {
                if ((numAOTMethodsWritten = cache->writeCache(newCacheFile)) != 0) {
                    ...
                    // Before the rename operation, check again if our in-memory cache is still better than the existing
                    // snapshot
                    if (cache->isAOTCacheBetterThanSnapshot(cacheFileName, 1)) {
                        ...
                        if (0 == rename(tempFileName.c_str(), cacheFileName.c_str())) {
                            ...
                            if (TR::Options::getVerboseOption(TR_VerboseJITServer)) {
                                char timestamp[32];
                                uint64_t durationUsec = j9time_hires_delta(startTime, j9time_hires_clock(),
                                    J9PORT_TIME_DELTA_IN_MICROSECONDS);
                                omrstr_ftime_ex(timestamp, sizeof(timestamp), "%b-%d-%Y_%H:%M:%S ",
                                    j9time_current_time_millis(), OMRSTR_FTIME_FLAG_LOCAL);

                                TR_VerboseLog::CriticalSection vlogLock;
                                TR_VerboseLog::write(TR_Vlog_JITServer,
                                    "AOT cache: t=%llu Saved cache '%s' to file %s. %zu methods saved in %llu usec. "
                                    "Current time:",
                                    compInfo->getPersistentInfo()->getElapsedTime(), cacheName.c_str(),
                                    cacheFileName.c_str(), numAOTMethodsWritten, durationUsec);
                        ...
                        } else // Renaming failed
                        {
                            if (TR::Options::getVerboseOption(TR_VerboseJITServer))
                                TR_VerboseLog::writeLineLocked(TR_Vlog_JITServer,
                                    "AOT cache: Failed to rename temporary cache file %s to %s: %s",
                                    tempFileName.c_str(), cacheFileName.c_str(), strerror(errno));
                            remove(tempFileName.c_str());
                            cache->excludeCacheFromSavingToFile();
                        }
                    } else // We changed our mind because some other JITServer instance has just saved a better snapshot
                           // than ours
                    {
                        if (TR::Options::getVerboseOption(TR_VerboseJITServer))
                            TR_VerboseLog::writeLineLocked(TR_Vlog_JITServer,
                                "AOT cache: Gave up renaming cache file %s to %s", tempFileName.c_str(),
                                cacheFileName.c_str());
                        remove(tempFileName.c_str());
                    }
                } else // Saving the AOT cache to file failed
                {
                    ...
                    if (TR::Options::getVerboseOption(TR_VerboseJITServer))
                        TR_VerboseLog::writeLineLocked(TR_Vlog_JITServer,
                            "AOT cache: Failed to serialize cache '%s' to file", cacheName.c_str());
                    remove(tempFileName.c_str());
                    cache->excludeCacheFromSavingToFile();
                }
            } else // Cannot open file for writing; permissions?
            {
                if (TR::Options::getVerboseOption(TR_VerboseJITServer))
                    TR_VerboseLog::writeLineLocked(TR_Vlog_JITServer,
                        "AOT cache: Cannot open new file %s for writing: %s", tempFileName.c_str(), strerror(errno));
                cache->excludeCacheFromSavingToFile();
            }
        }
    } // end try
        ...
        if (TR::Options::getVerboseOption(TR_VerboseJITServer)) {
            TR_VerboseLog::writeLineLocked(TR_Vlog_JITServer,
                "AOT cache: exception caught when trying to save cache '%s': %s", cacheName.c_str(), e.what());
        }
    }
...
void JITServerAOTCacheMap::loadNextQueuedAOTCacheFromFile(J9::J9SegmentProvider &scratchSegmentProvider)
{
    std::string cacheName;
    {
        OMR::CriticalSection cs(_monitor);
        TR_ASSERT(_cachesBeingLoaded.size() != 0,
            "There must be at least one AOT cache waiting to be loaded from file");
    ...
    try {
        ...
        if (cacheFile) {
            if (TR::Options::getVerboseOption(TR_VerboseJITServer))
                TR_VerboseLog::writeLineLocked(TR_Vlog_JITServer,
                    "AOT cache: t=%llu Opened file %s to load cache '%s' from file",
            ...
            if (cache) {
                ...
                cache->setMinNumAOTMethodsToSave(
                    cache->getNumCachedMethods() + TR::Options::_aotCachePersistenceMinDeltaMethods);
            ...
            } else // Failed to create the AOT cache from file
            {
                if (TR::Options::getVerboseOption(TR_VerboseJITServer))
                    TR_VerboseLog::writeLineLocked(TR_Vlog_JITServer,
                        "AOT cache: Failed to create cache '%s' from file", cacheName.c_str());
            }
        } else // Cannot open the AOT cache file
        {
            if (TR::Options::getVerboseOption(TR_VerboseJITServer))
                TR_VerboseLog::writeLineLocked(TR_Vlog_JITServer, "AOT cache: Failed to open cache file %s: %s",
                    cacheFileName.c_str(), strerror(errno));
        }
    } catch (const std::exception &e) {
        if (TR::Options::getVerboseOption(TR_VerboseJITServer)) {
            TR_VerboseLog::writeLineLocked(TR_Vlog_JITServer,
                "AOT cache: exception caught when trying to read-in cache '%s': %s", cacheName.c_str(), e.what());
        }
        if (cacheFile) {
            fclose(cacheFile);
            cacheFile = NULL;
...
JITServerAOTCache *JITServerAOTCacheMap::get(const std::string &name, uint64_t clientUID, bool &pending)
{
    OMR::CriticalSection cs(_monitor);
    ...
    if (compInfo->getPersistentInfo()->getJITServerUseAOTCachePersistence() && // Persistence feature is enabled
        _cachesExcludedFromLoading.find(name)
            == _cachesExcludedFromLoading.end()) // I am allowed to load this cache from file
    {
        ...
        if (_cachesBeingLoaded.find(name) != _cachesBeingLoaded.end()) {
            ...
            if (TR::Options::getVerboseOption(TR_VerboseJITServer))
                TR_VerboseLog::writeLineLocked(TR_Vlog_JITServer,
                    "AOT cache: Another thread is already loading cache '%s' from file", name.c_str());
            pending = true;
            return NULL;
        }
        ...
        if (!compInfo->getPersistentInfo()->getDisableFurtherCompilation()) {
            ...
            if (compInfo->addOutOfProcessMethodToBeCompiled(LOAD_AOTCACHE_REQUEST /*stream*/)) {
                ...
                if (TR::Options::getVerboseOption(TR_VerboseJITServer))
                    TR_VerboseLog::writeLineLocked(TR_Vlog_JITServer,
                        "AOT cache: t=%llu Queued comp request to load cache '%s' from file in the background",
...
size_t JITServerAOTCacheMap::getNumDeserializedMethods() const
{
    size_t result = 0;
    OMR::CriticalSection cs(_monitor);
    for (auto &it : _map)
...
void JITServerAOTCacheMap::printStats(FILE *f) const
{
    OMR::CriticalSection cs(_monitor);
    for (auto &it : _map)
