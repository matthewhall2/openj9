...
bool TR_TranslationArtifactManager::initializeGlobalArtifactManager(J9AVLTree *translationArtifacts, J9JavaVM *vm)
{
...
TR_TranslationArtifactManager::TR_TranslationArtifactManager(J9AVLTree *translationArtifacts, J9JavaVM *vm,
    TR::Monitor *monitor)
...
bool TR_TranslationArtifactManager::addCodeCache(TR::CodeCache *codeCache)
{
    ...
    J9JITHashTable *newTable = hash_jit_allocate(_portLibrary, reinterpret_cast<UDATA>(codeCache->getCodeBase()),
        reinterpret_cast<UDATA>(codeCache->getCodeTop()));
    if (newTable) {
...
bool TR_TranslationArtifactManager::removeCodeCache(TR::CodeCache *codeCache)
{
    TR_ASSERT(codeCache, "codeCache must not be null");
    TR_VMExclusiveAccess exclusiveAccess(_vm);
    _cachedPC = 0;
...
bool TR_TranslationArtifactManager::insertArtifact(J9JITExceptionTable *artifact)
{
...
bool TR_TranslationArtifactManager::containsArtifact(J9JITExceptionTable *artifact) const
{
    return (artifact && artifact == retrieveArtifact(artifact->startPC));
}

bool TR_TranslationArtifactManager::removeArtifact(J9JITExceptionTable *artifact)
{
    TR_ASSERT(artifact, "artifact must not be null");
...
const J9JITExceptionTable *TR_TranslationArtifactManager::retrieveArtifact(UDATA pc) const
{
...
bool TR_TranslationArtifactManager::insertRange(J9JITExceptionTable *artifact, UDATA startPC, UDATA endPC)
{
    bool insertSuccess = false;
...
bool TR_TranslationArtifactManager::removeRange(J9JITExceptionTable *artifact, UDATA startPC, UDATA endPC)
{
    bool removeSuccess = false;
    updateCache(artifact->startPC);
    if (_cachedHashTable) {
...
void TR_TranslationArtifactManager::updateCache(UDATA currentPC) const
{
    ...
    if (currentPC != _cachedPC) {
        ...
        _cachedPC = currentPC;
        _cachedHashTable
            = static_cast<J9JITHashTable *>(static_cast<void *>(avl_search(_translationArtifacts, currentPC)));
        TR_ASSERT(_cachedHashTable,
            "Either we lost a code cache or We attempted to find a hash table for a non-code cache startPC");
    }
}
