...
public:
     ...
     */
    virtual void processBufferRecords(J9VMThread *vmThread, uint8_t *dataStart, uintptr_t size,
        uintptr_t bufferFilledSize, uint32_t dataTag = 0)
        = 0;
     ...
     */
    U_8 *swapBufferToWorkingQueue(const uint8_t *dataStart, UDATA size, UDATA bufferFilledSize, uint32_t dataTag = 0,
     ...
     */
    bool recompilationLogic(TR_PersistentJittedBodyInfo *bodyInfo, void *startPC, uint64_t startCount, uint64_t count,
        uint64_t totalCount, TR_FrontEnd *fe, J9VMThread *vmThread);
     ...
     * @return the Bytecode PC
     */
    uintptr_t getPCFromMethodAndBCIndex(TR_OpaqueMethodBlock *method, uint32_t byteCodeIndex, TR::Compilation *comp);
     ...
     */
    TR_HWPBytecodePCToIAMap createBCMap(uint8_t *ia, uint32_t bcIndex, TR_OpaqueMethodBlock *method,
        TR::Compilation *comp);
