...
template<typename T>
TR_DataCacheManager *TR_DataCacheManager::constructManager(J9JITConfig *jitConfig, TR::Monitor *monitor,
    uint32_t quantumSize, uint32_t minQuanta, bool newImplementation)
{
    PORT_ACCESS_FROM_JITCONFIG(jitConfig);
    T *newManager = static_cast<T *>(j9mem_allocate_memory(sizeof(T), J9MEM_CATEGORY_JIT));
    if (newManager) {
        newManager = new (newManager) T(jitConfig, monitor, quantumSize, minQuanta, newImplementation);
    }
    return newManager;
}
...
TR_DataCacheManager *TR_DataCacheManager::initialize(J9JITConfig *jitConfig)
{
    if (!_dataCacheManager) {
        ...
        _dataCacheManager
            = managerConstructor(jitConfig, monitor, TR::Options::getCmdLineOptions()->getDataCacheQuantumSize(),
...
void TR_DataCacheManager::destroyManager()
{
    if (_dataCacheManager) {
...
TR_DataCacheManager::TR_DataCacheManager(J9JITConfig *jitConfig, TR::Monitor *monitor, uint32_t quantumSize,
    uint32_t minQuanta, bool newImplementation, bool worstFit)
    ...
    , _minQuanta(
          ...
          std::max(minQuanta,
              (roundToMultiple<uint32_t>(sizeof(Allocation), roundToMultiple<uint32_t>(quantumSize, sizeof(uintptr_t)))
                  / roundToMultiple<uint32_t>(quantumSize, sizeof(uintptr_t)))))
    ...
    , _mutex(monitor)
{
    ...
    fprintf(stderr,
        "Initialized the data cache manager with a quantum size of %d with a minimum of %d quanta per allocation.\n",
...
void TR_DataCacheManager::freeDataCacheList(TR_DataCache *&head)
{
...
TR_DataCache *TR_DataCacheManager::allocateNewDataCache(uint32_t minimumSize)
{
    ...
    if (((_jitConfig->runtimeFlags & J9JIT_GROW_CACHES) || _numAllocatedCaches == 0)
        && // make sure we are allowed to allocate
        ...
        if (_jitConfig->dataCacheList->totalSegmentSize < _jitConfig->dataCacheTotalKB * 1024) {
            ...
            if (dataCache) {
                ...
                if (_disclaimEnabled) {
                    ...
                    segSize = OMR::align((size_t)segSize,
                        (size_t)defaultPageSize); // Round up to a multiple of default page size if needed
                    memoryType |= MEMORY_TYPE_VIRTUAL; // Use mmap for allocation
                    // If swap is enabled, we can allocate memory with mmap(MAP_ANOYNMOUS|MAP_PRIVATE) and disclaim to
                    // swap If swap is not enabled we can disclaim to a backing file
...
#endif
                {
                    ...
                    dataCacheSeg
                        = _jitConfig->javaVM->internalVMFunctions->allocateMemorySegmentInList(_jitConfig->javaVM,
                            _jitConfig->dataCacheList, segSize, memoryType, J9MEM_CATEGORY_JIT_DATA_CACHE);
                    if (dataCacheSeg)
                ...
                if (dataCacheSeg) {
                    ...
                    if (OMR::RSSReport::instance()) {
                        ...
                        dataCache->_rssRegion
                            = new (PERSISTENT_NEW) OMR::RSSRegion("data cache", dataCacheSeg->heapBase, segSize,
                                OMR::RSSRegion::lowToHigh, j9vmem_supported_page_sizes()[0]);
                    ...
                    if (TR::Options::getCmdLineOptions()->getVerboseOption(TR_VerbosePerformance))
                        TR_VerboseLog::writeLineLocked(TR_Vlog_INFO,
                            "Allocated new data cache segment starting at address %p", dataCacheSeg->heapBase);

#ifdef LINUX
                    if (_disclaimEnabled) {
                        // The start address must be page aligned because we obtained it from mmap
                        TR_ASSERT_FATAL(
                            OMR::alignedNoCheck((uintptr_t)dataCacheSeg->heapBase, j9vmem_supported_page_sizes()[0]),
                            "Start address of the segment is not page aligned");
                        ...
                        if (madvise(dataCacheSeg->heapBase, segLength, MADV_NOHUGEPAGE) != 0) {
                            if (TR::Options::getCmdLineOptions()->getVerboseOption(TR_VerbosePerformance))
                                TR_VerboseLog::writeLineLocked(TR_Vlog_INFO,
                                    "Failed to set MADV_NOHUGEPAGE for data cache");
                        }
                        ...
                        if (dataCacheSeg->vmemIdentifier.allocator == OMRPORT_VMEM_RESERVE_USED_MMAP_SHM) {
                            if (madvise(dataCacheSeg->heapBase, segLength, MADV_RANDOM) != 0) {
                                if (TR::Options::getCmdLineOptions()->getVerboseOption(TR_VerbosePerformance))
                                    TR_VerboseLog::writeLineLocked(TR_Vlog_INFO,
                                        "Failed to set MADV_RANDOM for data cache");
                            }
                        }
                    }
#endif // LINUX
                    ...
                    if (TR::Options::getCmdLineOptions()->getVerboseOption(TR_VerbosePerformance))
                        TR_VerboseLog::writeLineLocked(TR_Vlog_INFO, "Failed to allocate %d Kb data cache",
            ...
            {
                TR_VerboseLog::writeLineLocked(TR_Vlog_INFO, "Failed to allocate %d bytes for data cache",
                    sizeof(TR_DataCache));
                _jitConfig->runtimeFlags |= J9JIT_DATA_CACHE_FULL; // prevent future allocations
#ifdef DATA_CACHE_DEBUG
    ...
    uint32_t *allocatedSizePtr)
{
    ...
    if (_newImplementation) {
        ...
        TR_ASSERT(size == TR_DataCacheManager::alignToMachineWord(size),
            "Byte multiples should always align to machine words");
        ...
        if (alloc) {
            ...
            alloc->prepareForUse();
            fillDataCacheHeader(static_cast<J9JITDataCacheHeader *>(static_cast<void *>(alloc)), allocationType,
                alloc->size());
            retValue = static_cast<uint8_t *>(alloc->getBuffer());
            if (allocatedSizePtr) {
                *allocatedSizePtr = alloc->size() - sizeof(J9JITDataCacheHeader);
            }
            TR_ASSERT(retValue == static_cast<uint8_t *>(static_cast<void *>(alloc)) + sizeof(J9JITDataCacheHeader),
                "Pointer to the buffer should point to address immediately following the data cache header.");
        }
#if defined(DATA_CACHE_DEBUG)
        printStatistics();
#endif
...
{
    if (_newImplementation) {
        J9JITDataCacheHeader *hdr = static_cast<J9JITDataCacheHeader *>(
            static_cast<void *>(static_cast<uint8_t *>(record) - sizeof(J9JITDataCacheHeader)));
...
double TR_DataCacheManager::computeDataCacheEfficiency()
{
    ...
    return 100.0 * (_totalSegmentMemoryAllocated - availableSpaceActive - availableSpaceRetired)
...
int TR_DataCacheManager::disclaimSegment(J9MemorySegment *segment, bool canDisclaimOnSwap, bool canDisclaimOnFile)
{
    ...
    if ((segment->vmemIdentifier.allocator == OMRPORT_VMEM_RESERVE_USED_MMAP_SHM && canDisclaimOnFile)
        ...
        if (TR::Options::getCmdLineOptions()->getVerboseOption(TR_VerbosePerformance))
            TR_VerboseLog::writeLineLocked(TR_Vlog_PERF,
                "Will disclaim data cache segment %p of size %zu. Present pages =%d swapped=%d fileMapped=%d", segment,
                segLength, numPresentPages, swappedCount, filePageCount);
        ...
        if (ret != 0) {
            ...
            if (TR::Options::getCmdLineOptions()->getVerboseOption(TR_VerbosePerformance))
                TR_VerboseLog::writeLineLocked(TR_Vlog_PERF,
                    "WARNING: Failed to use madvise to disclaim memory for data cache");
            if (ret == EINVAL) {
                _disclaimEnabled = false; // Don't try to disclaim again, since support seems to be missing
                if (TR::Options::getCmdLineOptions()->getVerboseOption(TR_VerbosePerformance))
                    TR_VerboseLog::writeLineLocked(TR_Vlog_PERF,
                        "WARNING: Disabling data cache disclaiming from now on");
            }
        } else {
            disclaimDone = 1;
#ifdef DEBUG_DISCLAIM
            ...
            if (TR::Options::getCmdLineOptions()->getVerboseOption(TR_VerbosePerformance))
                TR_VerboseLog::writeLineLocked(TR_Vlog_PERF,
                    "Disclaimed data cache segment %p of size %zu. Present pages =%d swapped=%d fileMapped=%d", segment,
                    segLength, numPresentPages, swappedCount, filePageCount);
#endif // DEBUG_DISCLAIM
        }
    ...
    {
        if (TR::Options::getCmdLineOptions()->getVerboseOption(TR_VerbosePerformance))
            TR_VerboseLog::writeLineLocked(TR_Vlog_PERF, "WARNING: Data cache segment %p is not backed by a file/swap",
                segment);
    }
#endif // LINUX
    return disclaimDone;
...
int TR_DataCacheManager::disclaimAllDataCaches()
{
    ...
    for (J9MemorySegment *dataCacheSeg = _jitConfig->dataCacheList->nextSegment; dataCacheSeg;
...
void *TR_DataCacheManager::allocateMemoryFromVM(size_t size)
{
    PORT_ACCESS_FROM_JITCONFIG(_jitConfig);
...
void TR_DataCacheManager::freeMemoryToVM(void *ptr)
{
    PORT_ACCESS_FROM_JITCONFIG(_jitConfig);
    j9mem_free_memory(ptr);
}

void TR_DataCacheManager::addToPool(TR_DataCacheManager::Allocation *alloc)
{
...
TR_DataCacheManager::Allocation *TR_DataCacheManager::getFromPool(uint32_t size)
{
#if defined(DATA_CACHE_DEBUG) && (DATA_CACHE_VERBOSITY_LEVEL >= 3)
...
void TR_DataCacheManager::convertDataCachesToAllocations()
{
    if (_newImplementation) {
        OMR::CriticalSection criticalSection(_mutex);
...
TR_DataCacheManager::Allocation *TR_DataCacheManager::convertDataCacheToAllocation(TR_DataCache *dataCache)
{
...
TR_DataCacheManager::Allocation *TR_DataCacheManager::Allocation::split(uint32_t requiredSize)
{
    TR_ASSERT(this->_header.size >= requiredSize + sizeof(Allocation),
        "Should only split allocations when actual size is bigger than required size plus enough space to fit another "
        "allocation.");
...
void TR_DataCacheManager::SizeBucket::push(Allocation *alloc) { _allocations.push_front(*alloc); }
...
void TR_DataCacheManager::growHook(UDATA allocationSize) {}

void TR_DataCacheManager::allocationHook(UDATA allocationSize, UDATA requestedSize) {}

void TR_DataCacheManager::freeHook(UDATA allocationSize) {}

void TR_DataCacheManager::insertHook(UDATA allocationSize) {}

void TR_DataCacheManager::removeHook(UDATA allocationSize) {}

void TR_DataCacheManager::printStatistics() {}

void TR_DataCacheManager::SizeBucket::print()
{
...
void TR_DataCacheManager::Allocation::print()
{
    fprintf(stderr, "\t\tAllocation at %p of size %u\n", this, _header.size);
}

TR_InstrumentedDataCacheManager::TR_InstrumentedDataCacheManager(J9JITConfig *jitConfig, TR::Monitor *monitor,
    uint32_t quantumSize, uint32_t minQuanta, bool newImplementation, bool worstFit)
    ...
    , _squares(0)
    , _bytesInPool(0)
    , _allocationStatistics("Bytes requested per allocation", roundToMultiple<uint32_t>(quantumSize, sizeof(uintptr_t)),
          1 << 13)
    , _wasteStatistics("Waste per allocation",
          (roundToMultiple<uint32_t>(quantumSize, sizeof(uintptr_t)) / static_cast<double>(sizeof(uintptr_t))),
          ((roundToMultiple<uint32_t>(quantumSize, sizeof(uintptr_t)) / static_cast<double>(sizeof(uintptr_t)))
              * (static_cast<double>(sizeof(uintptr_t)) - 1)))
{}

TR_InstrumentedDataCacheManager::~TR_InstrumentedDataCacheManager() { printStatistics(); }

void TR_InstrumentedDataCacheManager::growHook(UDATA bytesAdded)
{
    _jitSpace += bytesAdded;
    _freeSpace += bytesAdded;
}

void TR_InstrumentedDataCacheManager::allocationHook(UDATA allocationSize, UDATA requestedSize)
{
...
void TR_InstrumentedDataCacheManager::freeHook(UDATA allocationSize)
{
    _freeSpace += allocationSize;
    _usedSpace -= allocationSize;
    --_numberOfCurrentAllocations;
...
void TR_InstrumentedDataCacheManager::insertHook(UDATA allocationSize) { _bytesInPool += allocationSize; }

void TR_InstrumentedDataCacheManager::removeHook(UDATA allocationSize) { _bytesInPool -= allocationSize; }

void TR_InstrumentedDataCacheManager::printStatistics()
{
    OMR::CriticalSection criticalSection(_mutex);
...
void TR_InstrumentedDataCacheManager::printPoolContents()
{
