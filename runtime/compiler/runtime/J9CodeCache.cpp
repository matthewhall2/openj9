...
TR::CodeCache *J9::CodeCache::self() { return static_cast<TR::CodeCache *>(this); }

TR::CodeCacheMemorySegment *J9::CodeCache::trj9segment() { return self()->segment(); }

J9MemorySegment *J9::CodeCache::j9segment() { return self()->trj9segment()->j9segment(); }

TR::CodeCache *J9::CodeCache::allocate(TR::CodeCacheManager *cacheManager, size_t segmentSize,
    int32_t reservingCompThreadID, TR::CodeCacheKind kind)
{
...
bool J9::CodeCache::initialize(TR::CodeCacheManager *manager, TR::CodeCacheMemorySegment *codeCacheSegment,
    size_t allocatedCodeCacheSizeInBytes, TR::CodeCacheKind kind)
{
    ...
    if (OMR::RSSReport::instance()) {
      ...
      OMR::RSSReport::instance()->addRegion(_coldCodeRSSRegion);
        ...
        OMR::RSSRegion *codeCacheRSSRegion = new (PERSISTENT_NEW) OMR::RSSRegion("Code Cache", _warmCodeAlloc,
            _coldCodeAlloc - _warmCodeAlloc, OMR::RSSRegion::lowToHigh, j9vmem_supported_page_sizes()[0]);
    ...
    if (manager->isDisclaimEnabled()) {
        ...
        if (TR::Options::getCmdLineOptions()->getVerboseOption(TR_VerbosePerformance))
            TR_VerboseLog::writeLineLocked(TR_Vlog_INFO,
                "Code cache warm area %p - %p (size=%zu); cold area %p - %p (size=%zu)", (uint8_t *)warmSectionStart,
                _smallPageAreaStart, _smallPageAreaStart - (uint8_t *)warmSectionStart, _smallPageAreaStart,
                _smallPageAreaEnd, coldCacheSize);

        if (madvise(_smallPageAreaStart, coldCacheSize, MADV_NOHUGEPAGE) != 0) {
            const char *error = strerror(errno);
            if (TR::Options::getCmdLineOptions()->getVerboseOption(TR_VerbosePerformance))
                TR_VerboseLog::writeLineLocked(TR_Vlog_INFO, "Failed to set MADV_NOHUGEPAGE for code cache: %s: %p %zu",
...
void J9::CodeCache::onClassUnloading(J9ClassLoader *loaderPtr)
{
...
void J9::CodeCache::onClassRedefinition(TR_OpaqueMethodBlock *oldMethod, TR_OpaqueMethodBlock *newMethod)
{
...
OMR::CodeCacheHashEntry *J9::CodeCache::findUnresolvedMethod(void *constPool, int32_t constPoolIndex)
{
    return _unresolvedMethodHT->findUnresolvedMethod(constPool, constPoolIndex);
}

bool J9::CodeCache::addUnresolvedMethod(void *constPool, int32_t constPoolIndex)
{
...
bool J9::CodeCache::addResolvedMethod(TR_OpaqueMethodBlock *method)
{
...
//
void J9::CodeCache::resolveHashEntry(OMR::CodeCacheHashEntry *entry, TR_OpaqueMethodBlock *method)
{
...
void J9::CodeCache::addFreeBlock(OMR::FaintCacheBlock *block)
{
    ...
    if (metaData->startColdPC) {
        ...
        OMR::CodeCacheMethodHeader *coldBlock
            = (OMR::CodeCacheMethodHeader *)(metaData->startColdPC - sizeof(OMR::CodeCacheMethodHeader));
...
// Caller should expect that block may sometimes not be added.
OMR::CodeCacheMethodHeader *J9::CodeCache::addFreeBlock(void *voidMetaData)
{
    ...
    if (warmBlock) {
        if (config.verboseReclamation()) {
            if (metaData->ramMethod) {
                TR_VerboseLog::writeLineLocked(TR_Vlog_CODECACHE,
                    "CC=%p unloading j9method=%p metaData=%p warmBlock=%p size=%d: %.*s.%.*s%.*s", this,
                    metaData->ramMethod, metaData, warmBlock, (int)warmBlock->_size, J9UTF8_LENGTH(metaData->className),
                    J9UTF8_DATA(metaData->className), J9UTF8_LENGTH(metaData->methodName),
                    J9UTF8_DATA(metaData->methodName), J9UTF8_LENGTH(metaData->methodSignature),
                    J9UTF8_DATA(metaData->methodSignature));
            } else {
                TR_ASSERT_FATAL(metaData->flags & JIT_METADATA_NOT_INITIALIZED,
                    "metaData->ramMethod is NULL but metaData (%p) does not have the JIT_METADATA_NOT_INITIALIZED flag "
                    "set",
                    metaData);
        ...
        if (metaData->bodyInfo && !TR::Options::getCmdLineOptions()->getOption(TR_EnableHCR)
            ...
            if (!bi->getIsAotedBody()) // AOTed methods allocate the bodyInfo in the dataCache with allocateException
            {
                ...
                if (!pmi || !pmi->isInDataCache()) {
                    // If compiled remotely, the body info is currently also in the DataCache so don't free it but still
                    // consider freeing the MethodInfo below since it is independent
                    if (!bi->getIsRemoteCompileBody()) {
                        TR_Memory::jitPersistentFree(bi);
                        // If we free bodyInfo, we need to also free metaData->bodyInfo->mapTable by calling
                        // freeFastWalkCache()
                        J9VMThread *currentVMThread
                            = _manager->javaVM()->internalVMFunctions->currentVMThread(_manager->javaVM());
                ...
                if (pmi && !pmi->isInDataCache()) {
                    ...
                    if ((ramMethodStartPC != 0) && (ramMethodStartPC == (uintptr_t)metaData->startPC)) {
                        ...
                        if (TR::Options::getVerboseOption(TR_VerboseReclamation)) {
                            TR_VerboseLog::writeLineLocked(TR_Vlog_RECLAMATION, "Reclaiming PersistentMethodInfo 0x%p.",
                                pmi);
                        }
    ...
    if (metaData->startColdPC) {
        // startPC of the cold code is preceded by OMR::CodeCacheMethodHeader
        OMR::CodeCacheMethodHeader *coldBlock
            = (OMR::CodeCacheMethodHeader *)((UDATA)metaData->startColdPC - sizeof(OMR::CodeCacheMethodHeader));
        if (self()->addFreeBlock2((uint8_t *)coldBlock, (uint8_t *)coldBlock + coldBlock->_size)) {
        }
    }
...
void J9::CodeCache::dumpCodeCache()
{
...
void J9::CodeCache::reportCodeLoadEvents()
{
    ...
    if (helperTrampolinesSize > 0)
        ALWAYS_TRIGGER_J9HOOK_VM_DYNAMIC_CODE_LOAD(javaVM->hookInterface, currentThread, NULL, (void *)_helperBase,
            helperTrampolinesSize, HELPER_TRAMPOLINE_AREA_NAME, NULL);

    UDATA methodTrampolinesSize = (UDATA)_helperBase - (UDATA)_trampolineBase;
    if (methodTrampolinesSize > 0)
        ALWAYS_TRIGGER_J9HOOK_VM_DYNAMIC_CODE_LOAD(javaVM->hookInterface, currentThread, NULL, (void *)_trampolineBase,
            methodTrampolinesSize, METHOD_TRAMPOLINE_AREA_NAME, NULL);

    UDATA preLoadedCodeSize = (UDATA)_trampolineBase - (UDATA)_CCPreLoadedCodeBase;
    if (preLoadedCodeSize > 0)
        ALWAYS_TRIGGER_J9HOOK_VM_DYNAMIC_CODE_LOAD(javaVM->hookInterface, currentThread, NULL,
            (void *)_CCPreLoadedCodeBase, preLoadedCodeSize, PRELOADED_CODE_AREA_NAME, NULL);
}

void J9::CodeCache::generatePerfToolEntries(TR::FILE *file)
{
#ifdef LINUX
    if (!file)
        return;
    ...
    UDATA helperTrampolinesSize = (UDATA)_helperTop - (UDATA)_helperBase;
    if (helperTrampolinesSize > 0)
        j9jit_fprintf(file, "%p %lX %s\n", _helperBase, helperTrampolinesSize, HELPER_TRAMPOLINE_AREA_NAME);

    UDATA methodTrampolinesSize = (UDATA)_helperBase - (UDATA)_trampolineBase;
    if (methodTrampolinesSize > 0)
        j9jit_fprintf(file, "%p %lX %s\n", _trampolineBase, methodTrampolinesSize, METHOD_TRAMPOLINE_AREA_NAME);

    UDATA preLoadedCodeSize = (UDATA)_trampolineBase - (UDATA)_CCPreLoadedCodeBase;
    if (preLoadedCodeSize > 0)
        j9jit_fprintf(file, "%p %lX %s\n", _CCPreLoadedCodeBase, preLoadedCodeSize, PRELOADED_CODE_AREA_NAME);
#endif
}

// Remove over-booked trampoline reservations
//
void J9::CodeCache::adjustTrampolineReservation(TR_OpaqueMethodBlock *method, void *cp, int32_t cpIndex)
{
...
void J9::CodeCache::onFSDDecompile() { self()->resetTrampolines(); }

void J9::CodeCache::resetTrampolines()
{
    TR_ASSERT(_manager->codeCacheConfig().needsMethodTrampolines(),
        "Attempting to purge trampolines when they do not exist");
...
//---------------------------------------------------------------------------
int32_t J9::CodeCache::reserveUnresolvedTrampoline(void *cp, int32_t cpIndex)
{
...
void J9::CodeCache::setInitialAllocationPointers()
{
    _warmCodeAllocBase = self()->getWarmCodeAlloc();
    _coldCodeAllocBase = self()->getColdCodeAlloc();
}

void J9::CodeCache::resetAllocationPointers()
{
...
void J9::CodeCache::resetCodeCache()
{
...
void mcc_callPointPatching_unwrapper(void **argsPtr, void **resPtr)
{
    ...
    if (codeCache)
        codeCache->patchCallPoint(reinterpret_cast<TR_OpaqueMethodBlock *>(argsPtr[0]), argsPtr[1], argsPtr[2],
            argsPtr[3]);
}
...
void mcc_reservationAdjustment_unwrapper(void **argsPtr, void **resPtr)
{
    ...
    if (codeCache)
        codeCache->adjustTrampolineReservation(reinterpret_cast<TR_OpaqueMethodBlock *>(argsPtr[1]), argsPtr[2],
            static_cast<int32_t>((UDATA)argsPtr[3]));
}

void mcc_lookupHelperTrampoline_unwrapper(void **argsPtr, void **resPtr)
{
    intptr_t trampoline
        = TR::CodeCacheManager::instance()->findHelperTrampoline(static_cast<int32_t>((UDATA)argsPtr[1]), argsPtr[0]);
    *resPtr = reinterpret_cast<void *>(trampoline);
}

void mcc_reservationInterfaceCache_unwrapper(void **args, void **res)
{
    TR::CodeCacheManager::instance()->reservationInterfaceCache(args[0],
        reinterpret_cast<TR_OpaqueMethodBlock *>(args[1]));
}

OMR::CodeCacheTrampolineCode *mcc_replaceTrampoline(TR_OpaqueMethodBlock *method, void *callSite, void *oldTrampoline,
    void *oldTargetPC, void *newTargetPC, bool needSync)
{
    return TR::CodeCacheManager::instance()->replaceTrampoline(method, callSite, oldTrampoline, oldTargetPC,
        newTargetPC, needSync);
}
}

int32_t J9::CodeCache::disclaim(TR::CodeCacheManager *manager, bool canDisclaimOnSwap, bool canDisclaimOnFile)
{
    ...
    TR_ASSERT_FATAL((uintptr_t)_smallPageAreaEnd >= (uintptr_t)disclaimStart,
        "disclaimStart is past the cold area end");
    ...
    if (trace) {
        ...
        TR_VerboseLog::writeLineLocked(TR_Vlog_PERF,
            "Disclaim code cache %p between Start=%p End=%p. coldStart=%p coldBase=%p warm_size=%zuB cold_size=%zuB "
            "cold_size/(cold_size + warm_size)=%5.2f%%",
            this, disclaimStart, _smallPageAreaEnd, _coldCodeAlloc, _coldCodeAllocBase, warm_size, cold_size,
            cold_size * 100.0 / (cold_size + warm_size));
    }
    ...
    if (ret != 0) {
        if (trace)
            TR_VerboseLog::writeLineLocked(TR_Vlog_PERF,
                "WARNING: Failed to use madvise to disclaim memory for code cache");
