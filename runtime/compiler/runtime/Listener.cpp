    ...
    TR::CompilationInfo *compInfo)
{
    ...
    if (TR::Options::getVerboseOption(TR_VerboseJITServer))
        TR_VerboseLog::writeLineLocked(TR_Vlog_JITServer, "t=%lu %s: errno=%d sslError=%d errorString=%s",
            (unsigned long)compInfo->getPersistentInfo()->getElapsedTime(), errMsg, errnoCopy, sslError, errorString);
...
static bool acceptOpenSSLConnection(SSL_CTX *sslCtx, int connfd, BIO *&bio, TR::CompilationInfo *compInfo)
{
    if (TR::Options::getVerboseOption(TR_VerboseJITServer))
    ...
    if (TR::Options::getVerboseOption(TR_VerboseJITServer))
        TR_VerboseLog::writeLineLocked(TR_Vlog_JITServer,
            "t=%lu SSL connection on socket 0x%x, Version: %s, Cipher: %s",
            (unsigned long)compInfo->getPersistentInfo()->getElapsedTime(), connfd, (*OSSL_get_version)(ssl),
            (*OSSL_get_cipher)(ssl));
    return true;
}

static int openCommunicationSocket(uint32_t port, uint32_t &boundPort)
{
...
TR_Listener::TR_Listener()
    ...
    , _listenerOSThread(NULL)
    , _listenerThreadAttachAttempted(false)
    , _listenerThreadExitFlag(false)
{}

void TR_Listener::serveRemoteCompilationRequests(BaseCompileDispatcher *compiler)
{
    ...
    if (JITServer::CommunicationStream::useSSL()) {
        ...
        if (!JITServer::ServerStream::createSSLContext(sslCtx, sessionContextId, sizeof(sessionContextId), sslKeys,
    ...
    struct pollfd pfd[2] = {
        { .fd = healthSockfd, .events = POLLIN, .revents = 0 },
    ...
    while (!getListenerThreadExitFlag()) {
        ...
        for (size_t fdIndex = 0; fdIndex < numFds; fdIndex++) {
            ...
            TR_ASSERT_FATAL(pfd[fdIndex].revents == POLLIN,
                "Unexpected event occurred during poll for new connection: socketIndex=%zu revents=%d\n", fdIndex,
                pfd[fdIndex].revents);
            ...
            do {
                ...
                if (connfd < 0) {
                    if ((EAGAIN != errno) && (EWOULDBLOCK != errno)) {
                        if (TR::Options::getVerboseOption(TR_VerboseJITServer)) {
                            TR_VerboseLog::writeLineLocked(TR_Vlog_JITServer,
                                "Error accepting connection: errno=%d: %s", errno, strerror(errno));
                        }
                    }
                } else // accept() succeeded
                {
                    ...
                    } else // compilation request socket
                    {
                        ...
                        JITServer::ServerStream *stream
                            = new (TR::Compiler->persistentGlobalAllocator()) JITServer::ServerStream(connfd, bio);
...
static int32_t J9THREAD_PROC listenerThreadProc(void *entryarg)
{
    ...
    if (TR::Options::isAnyVerboseOptionSet()) {
        ...
        omrstr_ftime_ex(timestamp, sizeof(timestamp), "%b %d %H:%M:%S %Y", j9time_current_time_millis(),
...
void TR_Listener::startListenerThread(J9JavaVM *javaVM)
{
    ...
    if (_listenerMonitor) {
        ...
        if (J9THREAD_SUCCESS
            != javaVM->internalVMFunctions->createJoinableThreadWithCategory(&_listenerOSThread, defaultOSStackSize,
                priority, 0, &listenerThreadProc, javaVM->jitConfig,
                J9THREAD_CATEGORY_SYSTEM_JIT_THREAD)) { // cannot create the listener thread
