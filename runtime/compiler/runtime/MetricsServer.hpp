...
public:
    CPUUtilMetric()
        : PrometheusMetric("jitserver_cpu_utilization", "Cpu utilization of the JITServer")
    {}
...
public:
    AvailableMemoryMetric()
        : PrometheusMetric("jitserver_available_memory", "Available memory for JITServer")
    {}
...
public:
    ConnectedClientsMetric()
        : PrometheusMetric("jitserver_connected_clients", "Number of connected clients")
    {}
...
/**
   @brief Class used to serialize the number of active compilation threads in OpenJ9, as a metric understood by
   Prometheus
 */
class ActiveThreadsMetric : public PrometheusMetric {
public:
    ActiveThreadsMetric()
        : PrometheusMetric("jitserver_active_threads", "Number of active compilation threads")
    {}
...
public:
    ...
    */
    ...
    /**
       ...
       @return An error code: READ_ERROR, WANT_READ, WANT_WRITE, REQUEST_TOO_LARGE, NO_GET_REQUEST,
       FULL_REQUEST_RECEIVED
    */
...
private:
    ...
    struct pollfd _pfd[1 + MAX_CONCURRENT_REQUESTS]
        = { { 0 } }; // poll file descriptors; first entry is for connection requests
