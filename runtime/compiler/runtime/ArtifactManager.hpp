...
/**
   ...
   The main use of this class is expected to be instantiation on the stack at the beginning of the scope requiring VM
   access. VM access will then be freed when the instances goes out of scope through auto destruction.
*/
class TR_VMExclusiveAccess {
public:
    /**
...
/**
   ...
   This class is intended to be use as the centralized manager of the JIT artifact AVL tree and individual hash tables
   for each code cache. This class has uses a TR::Monitor to synchronise access to the individual artifacts and uses
   exclusive VM access to add a hash table for each code cache.
*/
...
public:
    ...
    public:
        CriticalSection()
            : OMR::CriticalSection(getGlobalArtifactManager()->_monitor)
        {}

        CriticalSection(TR_TranslationArtifactManager *mgr)
            : OMR::CriticalSection(mgr->_monitor)
        {}
    };
    ...
    */
    ...
    /**
    @brief For a given method's J9JITExceptionTable, finds the appropriate hashtable in the AVL tree and inserts the
    exception table.

    Note, insertArtifact does not check to verify that an artifact's given range is not already occupied by an existing
    artifact.  This is because artifact ranges represent the virtual memory actually occupied by compiled code. Thus, if
    two artifact ranges overlap, then two pieces of compiled code co-exist on top of each other, which is inherently
    incorrect.  If the possibility occurs that two artifacts may legitimately have range values that overlap, checks
    ...
    */
    ...
    Note: Just because the JIT artifacts return an artifact for a particular startPC does not mean that it is the same
    artifact being searched for. The method could have been recompiled, and then subsequently unloaded and another
    method compiled to the same location before the recompilation-based reclamation could occur. This will result in the
    JIT artifacts containing a different J9JITExceptionTable for a given startPC.
    ...
    */
    ...
    @return Returns true if the artifact is found within, and successfully removed from, the JIT artifacts and false
    otherwise.
    */
...
protected:
    ...
    The non-cache members are pointers to types instantiated externally, the constructor simply copies the pointers into
    the objects private members.
    */
    ...
    Note this method expects to be called via another method in the artifact manager and thus does not acquire the
    artifact manager's monitor.
...
private:
    /**
    @brief Determines if the current artifactManager query is using the same artifact as the previous query, and if not,
    searches for and retrieves the new artifact's code cache's hash table.
