...
void printUnloadingInformation(J9JITExceptionTable *metaData, const char *message)
{
    ...
    fprintf(stderr, "%s causing reclamation of  %.*s.%.*s%.*s @ %p\n", message, J9UTF8_LENGTH(className),
        J9UTF8_DATA(className), J9UTF8_LENGTH(name), J9UTF8_DATA(name), J9UTF8_LENGTH(signature),
        J9UTF8_DATA(signature), reinterpret_cast<U_8 *>(metaData->startPC));
}
#endif
...
void dispatchUnloadHooks(J9JITConfig *jitConfig, J9VMThread *vmThread, J9JITExceptionTable *metaData)
{
    ...
    if (J9_EVENT_IS_HOOKED(vm->hookInterface, J9HOOK_VM_DYNAMIC_CODE_UNLOAD)) {
        ALWAYS_TRIGGER_J9HOOK_VM_DYNAMIC_CODE_UNLOAD(vm->hookInterface, vmThread, metaData->ramMethod,
...
inline void markMetaDataAsUnloaded(J9JITExceptionTable *metaData) { metaData->constantPool = 0; }

inline void reclaimAssumptions(J9JITConfig *jitConfig, J9JITExceptionTable *metaData,
    bool reclaimPrePrologueAssumptions)
{
    // expunge the runtime assumptions that exist on this method body
    pointer_cast<TR_PersistentMemory *>(jitConfig->scratchSegment)
        ->getPersistentInfo()
        ->getRuntimeAssumptionTable()
        ->reclaimAssumptions(metaData, reclaimPrePrologueAssumptions);
}

inline void markAssumptionsAndDetach(J9JITConfig *jitConfig, J9JITExceptionTable *metaData,
    bool reclaimPrePrologueAssumptions)
{
    ...
    pointer_cast<TR_PersistentMemory *>(jitConfig->scratchSegment)
        ->getPersistentInfo()
        ->getRuntimeAssumptionTable()
        ->markAssumptionsAndDetach(metaData, reclaimPrePrologueAssumptions);
}
...
inline void reclaimMarkedAssumptionsFromRAT(int cleanUpCount)
{
    ...
    pointer_cast<TR_PersistentMemory *>(jitConfig->scratchSegment)
        ->getPersistentInfo()
        ->getRuntimeAssumptionTable()
        ->reclaimMarkedAssumptionsFromRAT(cleanUpCount);
}
...
inline void updateExceptionTableLinkedList(J9VMThread *vmThread, J9JITExceptionTable *oldMetadata,
    J9JITExceptionTable *newMetadata)
{
...
inline J9JITExceptionTable *createStubExceptionTable(J9VMThread *vmThread, J9JITExceptionTable *metaData)
{
    ...
    if (!TR::Options::getCmdLineOptions()->getOption(TR_DisableMetadataReclamation)) {
        ...
        uint32_t size = 0;
        stubMetadata = (J9JITExceptionTable *)TR_DataCacheManager::getManager()->allocateDataCacheRecord(numBytes,
...
void cleanUpJitExceptionHandlerCache(J9VMThread *vmThread, J9JITExceptionTable *metaData)
{
    ...
    do {
        if (currentThread->jitExceptionHandlerCache) {
            TR_jitExceptionHandlerCache *searchCache
                = (TR_jitExceptionHandlerCache *)(currentThread->jitExceptionHandlerCache);
            for (int counter = 0; counter < JIT_EXCEPTION_HANDLER_CACHE_SIZE; counter++) {
                if ((searchCache[counter].pc >= metaData->startPC && searchCache[counter].pc <= metaData->endWarmPC)
                    || (metaData->startColdPC && searchCache[counter].pc >= metaData->startColdPC
                        && searchCache[counter].pc <= metaData->endPC)) {
...
void cleanUpJitArtifactSearchCache(J9VMThread *vmThread, J9JITExceptionTable *metaData)
{
    ...
    do {
        if (currentThread->jitArtifactSearchCache) {
            TR_jit_artifact_search_cache *searchCache
                = (TR_jit_artifact_search_cache *)(currentThread->jitArtifactSearchCache);
            for (int counter = 0; counter < JIT_ARTIFACT_SEARCH_CACHE_SIZE; counter++) {
                if ((searchCache[counter].searchValue >= metaData->startPC
                        && searchCache[counter].searchValue <= metaData->endWarmPC)
                    || (metaData->startColdPC && searchCache[counter].searchValue >= metaData->startColdPC
                        && searchCache[counter].searchValue <= metaData->endPC)) {
...
void jitRemoveConstRefsFromBody(J9VMThread *vmThread, J9JITExceptionTable *metaData)
{
    ...
    TR_ASSERT_FATAL(vm->exclusiveAccessState == J9_XACCESS_EXCLUSIVE,
    ...
    do {
        ...
        TR_ASSERT_FATAL(node->jitBodyMetaData == metaData,
            "const ref array %p: expected JIT body metadata %p but found %p", node, metaData, node->jitBodyMetaData);
...
void jitReleaseCodeCollectMetaData(J9JITConfig *jitConfig, J9VMThread *vmThread, J9JITExceptionTable *metaData,
    OMR::FaintCacheBlock *faintCacheBlock)
{
    ...
    if (!TR::Options::getCmdLineOptions()->getOption(TR_DisableCodeCacheReclamation)
        ...
        TR_PersistentJittedBodyInfo *bodyInfo
            = metaData->bodyInfo ? (TR_PersistentJittedBodyInfo *)metaData->bodyInfo : NULL;
        ...
        if (faintCacheBlock) {
            ...
            TR::CodeCacheManager::instance()->freeFaintCacheBlock(faintCacheBlock,
                (uint8_t *)faintCacheBlock->_metaData->startPC);
            ...
            if (stubMetadata) {
                ...
                if (TR::Options::getVerboseOption(TR_VerboseReclamation)) {
                    TR_VerboseLog::writeLineLocked(TR_Vlog_RECLAMATION,
                        "Reclaiming old metadata 0x%p, new metadata = 0x%p", metaData, stubMetadata);
                }
            } else {
                if (TR::Options::getVerboseOption(TR_VerboseReclamation)) {
                    TR_VerboseLog::writeLineLocked(TR_Vlog_RECLAMATION,
                        "Did not allocate stub metadata, reusing existing metadata 0x%p", metaData);
                }
            ...
            TR::CodeCacheManager::instance()->addFreeBlock(reinterpret_cast<void *>(metaData),
...
void vlogReclamation(const char *prefix, J9JITExceptionTable *metaData, size_t bytesToSaveAtStart)
{
    if (TR::Options::getVerboseOption(TR_VerboseReclamation)) {
        ...
        TR_VerboseLog::write(TR_Vlog_RECLAMATION, "%s %.*s.%.*s%.*s @ %s [" POINTER_PRINTF_FORMAT "-", prefix,
            J9UTF8_LENGTH(metaData->className), J9UTF8_DATA(metaData->className), J9UTF8_LENGTH(metaData->methodName),
            J9UTF8_DATA(metaData->methodName), J9UTF8_LENGTH(metaData->methodSignature),
            J9UTF8_DATA(metaData->methodSignature), TR::Compilation::getHotnessName(TR_Hotness(metaData->hotness)),
            metaData->startPC + bytesToSaveAtStart);

        if (metaData->startColdPC) {
            TR_VerboseLog::write(POINTER_PRINTF_FORMAT "] & [" POINTER_PRINTF_FORMAT "-", metaData->endWarmPC,
                metaData->startColdPC);
        }

        TR_VerboseLog::writeLine(POINTER_PRINTF_FORMAT "]", metaData->endPC);
    }
}
