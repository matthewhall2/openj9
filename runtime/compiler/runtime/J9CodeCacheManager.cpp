...
TR_FrontEnd *J9::CodeCacheManager::fe() { return _fe; }

TR_J9VMBase *J9::CodeCacheManager::fej9() { return (TR_J9VMBase *)(self()->fe()); }

TR::CodeCache *J9::CodeCacheManager::initialize(bool useConsolidatedCache, uint32_t numberOfCodeCachesToCreateAtStartup)
{
...
bool J9::CodeCacheManager::isSufficientPhysicalMemoryAvailableForAllocation(size_t requestedCodeCacheSize)
{
    ...
    if (!incomplete && freePhysicalMemoryB < desiredMemory) {
        if (config.verboseCodeCache() || config.verbosePerformance())
            TR_VerboseLog::writeLineLocked(TR_Vlog_CODECACHE,
                "Warning: low physical memory detected during code cache allocation, requestedCodeCacheSize=%zu, "
                "freePhysicalMemory=%zu, safeMemReserve=%zu",
...
void J9::CodeCacheManager::addCodeCache(TR::CodeCache *codeCache)
{
    ...
    if (j9segment) {
        ...
        J9VMThread *vmThread
            = TR::CodeCacheManager::javaVM()->internalVMFunctions->currentVMThread(TR::CodeCacheManager::javaVM());
        ...
        jit_artifact_protected_add_code_cache(TR::CodeCacheManager::javaVM(),
            TR::CodeCacheManager::jitConfig()->translationArtifacts, j9segment, NULL);
...
void J9::CodeCacheManager::addFaintCacheBlock(OMR::MethodExceptionData *metaData, uint8_t bytesToSaveAtStart)
{
    PORT_ACCESS_FROM_JITCONFIG(_jitConfig);

    OMR::FaintCacheBlock *block
        = (OMR::FaintCacheBlock *)j9mem_allocate_memory(sizeof(OMR::FaintCacheBlock), J9MEM_CATEGORY_JIT);
    if (!block)
        return;
...
void J9::CodeCacheManager::freeFaintCacheBlock(OMR::FaintCacheBlock *block, uint8_t *startPC)
{
    TR::CodeCache *owningCodeCache = self()->findCodeCacheFromPC(startPC);
    owningCodeCache->addFreeBlock(block);
}

void J9::CodeCacheManager::setCodeCacheFull()
{
    self()->OMR::CodeCacheManager::setCodeCacheFull();
    _jitConfig->runtimeFlags |= J9JIT_CODE_CACHE_FULL;
}

void J9::CodeCacheManager::purgeClassLoaderFromFaintBlocks(J9ClassLoader *classLoader)
{
...
//
void J9::CodeCacheManager::onClassUnloading(J9ClassLoader *loaderPtr)
{
    TR::CodeCacheConfig &config = self()->codeCacheConfig();
...
TR::CodeCache *J9::CodeCacheManager::reserveCodeCache(bool compilationCodeAllocationsMustBeContiguous,
    size_t sizeEstimate, int32_t compThreadID, int32_t *numReserved, TR::CodeCacheKind kind)
{
    TR::CodeCache *codeCache = self()->OMR::CodeCacheManager::reserveCodeCache(
        compilationCodeAllocationsMustBeContiguous, sizeEstimate, compThreadID, numReserved, kind);
...
void J9::CodeCacheManager::reportCodeLoadEvents()
{
...
TR::CodeCacheMemorySegment *J9::CodeCacheManager::allocateCodeCacheSegment(size_t segmentSize,
    size_t &codeCacheSizeToAllocate, void *preferredStartAddress)
{
...
#endif
    {
        ...
        if (vmemParams.pageSize > ccTotalSizeB) {
            ...
            if (config.verboseCodeCache() || config.verbosePerformance())
                TR_VerboseLog::writeLineLocked(TR_Vlog_CODECACHE,
                    "Warning: Using page size %zu instead of large page size %zu", (size_t)vmemParams.pageSize,
                    largeCodePageSize);
        }
    }

    UDATA mode = J9PORT_VMEM_MEMORY_MODE_READ | J9PORT_VMEM_MEMORY_MODE_WRITE | J9PORT_VMEM_MEMORY_MODE_EXECUTE;
    ...
    if (preferredStartAddress) {
        vmemParams.options
            |= J9PORT_VMEM_STRICT_ADDRESS; // if we cannot allocate a block in the desired range, return NULL
        ...
        } else // Allocate after JIT dll
        {
            vmemParams.endAddress
                = (void *)(someJitLibraryAddress + MAX_DISTANCE_NEAR_JITLIBRARY_TO_AVOID_TRAMPOLINE - segmentSize);
        }
        ...
        vmemParams.options |= J9PORT_VMEM_STRICT_ADDRESS; // if we cannot allocate a block whose start is between
                                                          // preferredStartAddress and endAddress, return NULL
    ...
    J9MemorySegment *codeCacheSegment = javaVM->internalVMFunctions->allocateVirtualMemorySegmentInList(javaVM,
        jitConfig->codeCacheList, codeCacheSizeToAllocate, segmentType, &vmemParams);
    ...
    if (!codeCacheSegment && preferredStartAddress) {
        ...
        codeCacheSegment = javaVM->internalVMFunctions->allocateVirtualMemorySegmentInList(javaVM,
            jitConfig->codeCacheList, codeCacheSizeToAllocate, segmentType, &vmemParams);
    }
    ...
    if (codeCacheSegment) {
        ...
        if (config.verboseCodeCache()) {
            const char *verboseLogString = "The code cache repository was allocated between addresses %p and %p "
                                           "alignment=%zu largeCodePageSize=%zu";
            if (preferredStartAddress
                && self()->isInRange((uintptr_t)(codeCacheSegment->baseAddress), someJitLibraryAddress,
                    MAX_DISTANCE_NEAR_JITLIBRARY_TO_AVOID_TRAMPOLINE)) {
                verboseLogString = "The code cache repository was allocated between addresses %p and %p to avoid "
                                   "helper trampolines. alignment=%zu largeCodePageSize=%zu";
            }
            TR_VerboseLog::writeLineLocked(TR_Vlog_CODECACHE, verboseLogString, codeCacheSegment->baseAddress,
                codeCacheSegment->heapTop, alignment, largeCodePageSize);
        }
#ifdef LINUX
        if (0
            != madvise((void *)codeCacheSegment->baseAddress,
                (codeCacheSegment->heapTop - codeCacheSegment->baseAddress), MADV_HUGEPAGE)) {
            if (config.verboseCodeCache()
                || TR::Options::getCmdLineOptions()->getVerboseOption(TR_VerbosePerformance)) {
                TR_VerboseLog::writeLineLocked(TR_Vlog_PERF,
                    "Warning: madvise failed while providing hint to use large pages for code cache");
            }
        }
#endif // LINUX
        ...
        if (TR::Options::getCmdLineOptions()->getVerboseOption(TR_VerbosePerformance))
            TR_VerboseLog::writeLineLocked(TR_Vlog_INFO, "Allocated new code cache segment %p starting at address %p",
                codeCacheSegment, codeCacheSegment->heapBase);
    } else {
        // TODO: we should generate a trace point
        mcc_printf("TR::CodeCache::allocate : codeCacheSegment is NULL, %p\n", codeCacheSegment);

        if (TR::Options::getCmdLineOptions()->getVerboseOption(TR_VerbosePerformance))
            TR_VerboseLog::writeLineLocked(TR_Vlog_INFO, "Failed to allocate new code cache segment of %d Kb",
    ...
    if (config.codeCachePadKB()) {
        void *rc = j9vmem_commit_memory(codeCacheSegment->vmemIdentifier.address, segmentSize,
    ...
    if (config.verboseCodeCache())
        TR_VerboseLog::writeLineLocked(TR_Vlog_CODECACHE, "allocated code cache segment of size %u",
            codeCacheSizeToAllocate);

    TR::CodeCacheMemorySegment *memSegment
        = (TR::CodeCacheMemorySegment *)self()->getMemory(sizeof(TR::CodeCacheMemorySegment));
    new (memSegment) TR::CodeCacheMemorySegment(codeCacheSegment);

    return memSegment;
}

TR::CodeCacheMemorySegment *J9::CodeCacheManager::setupMemorySegmentFromRepository(uint8_t *start, uint8_t *end,
    size_t &codeCacheSizeToAllocate)
{
    ...
    TR::CodeCacheMemorySegment *memorySegment
        = (TR::CodeCacheMemorySegment *)self()->getMemory(sizeof(TR::CodeCacheMemorySegment));
...
void J9::CodeCacheManager::freeMemorySegment(TR::CodeCacheMemorySegment *segment) { segment->free(self()); }

void *J9::CodeCacheManager::chooseCacheStartAddress(size_t repositorySize)
{
    ...
    if (!TR::Options::getCmdLineOptions()->getOption(TR_DisableSmartPlacementOfCodeCaches)) {
        ...
        if (someFunctionPointer > MAX_DISTANCE_NEAR_JITLIBRARY_TO_AVOID_TRAMPOLINE) {
            ...
            // For Linux we can afford to search a larger space (~2 GB) because we use smaps to accelerate the search
            // operation. Also, if we use very large pages (1 GB), then we can afford to search a larger space because
            // there won't be too many attempts to find the right spot.
            if (TR::Compiler->target.isLinux() || alignment > 2 * 1024 * 1024) {
                startAddress = (void *)OMR::align(
                    (size_t)(someFunctionPointer - MAX_DISTANCE_NEAR_JITLIBRARY_TO_AVOID_TRAMPOLINE), alignment);
            } else {
        ...
        {
            startAddress
                = (void *)OMR::align((size_t)(someFunctionPointer + SAFE_DISTANCE_REPOSITORY_JITLIBRARY), alignment);
        }
    }
#endif
    return startAddress;
...
//
void J9::CodeCacheManager::onFSDDecompile()
{
    TR::CodeCacheConfig &config = self()->codeCacheConfig();
    if (!config.needsMethodTrampolines())
        return;
...
//
void J9::CodeCacheManager::onClassRedefinition(TR_OpaqueMethodBlock *oldMethod, TR_OpaqueMethodBlock *newMethod)
{
    TR::CodeCacheConfig &config = self()->codeCacheConfig();
    if (!config.needsMethodTrampolines())
        return;
...
//
uintptr_t J9::CodeCacheManager::getSomeJitLibraryAddress()
{
...
//
bool J9::CodeCacheManager::isInRange(uintptr_t address1, uintptr_t address2, uintptr_t range)
{
...
void J9::CodeCacheManager::reservationInterfaceCache(void *callSite, TR_OpaqueMethodBlock *method)
{
    TR::CodeCacheConfig &config = self()->codeCacheConfig();
    if (!config.needsMethodTrampolines())
        return;
...
void J9::CodeCacheManager::lateInitialization()
{
    TR::CodeCacheConfig &config = self()->codeCacheConfig();
    ...
    for (TR::CodeCache *codeCache = self()->getFirstCodeCache(); codeCache; codeCache = codeCache->next()) {
        config.mccCallbacks().createHelperTrampolines((uint8_t *)codeCache->getHelperBase(),
            config.numRuntimeHelpers());
    }
}

void J9::CodeCacheManager::addFreeBlock(void *metaData, uint8_t *startPC)
{
    TR::CodeCache *owningCodeCache = self()->findCodeCacheFromPC(startPC);
    owningCodeCache->addFreeBlock(metaData);
}

bool J9::CodeCacheManager::almostOutOfCodeCache()
{
    ...
    if (self()->canAddNewCodeCache()) {
        ...
        if (!foundSpace) {
            ...
            if (config.verbosePerformance()) {
                TR_VerboseLog::writeLineLocked(TR_Vlog_CODECACHE,
                    "Reached code cache space threshold. Disabling JIT profiling.");
            }
...
void J9::CodeCacheManager::printMccStats()
{
    self()->printRemainingSpaceInCodeCaches();
    self()->printOccupancyStats();
}

void J9::CodeCacheManager::printRemainingSpaceInCodeCaches()
{
    CacheListCriticalSection scanCacheList(self());
    for (TR::CodeCache *codeCache = self()->getFirstCodeCache(); codeCache; codeCache = codeCache->next()) {
...
void J9::CodeCacheManager::printOccupancyStats()
{
    CacheListCriticalSection scanCacheList(self());
    for (TR::CodeCache *codeCache = self()->getFirstCodeCache(); codeCache; codeCache = codeCache->next()) {
...
int32_t J9::CodeCacheManager::disclaimAllCodeCaches()
{
    if (!_disclaimEnabled)
        return 0;
