...
public:
    ...
    static uintptr_t getId(uintptr_t idAndType) { return idAndType >> idShift; }
...
private:
    ...
    ClassSerializationRecord(uintptr_t id, uintptr_t classLoaderId, const JITServerROMClassHash &hash,
        uint32_t romClassSize, bool generated, const J9ROMClass *romClass, const J9ROMClass *baseComponent,
...
private:
    ...
    static size_t size(size_t length) { return offsetof(ClassChainSerializationRecord, _list) + IdList::size(length); }
...
private:
    ...
    ThunkSerializationRecord(uintptr_t id, const uint8_t *signature, uint32_t signatureSize, const uint8_t *thunkStart,
        uint32_t thunkSize);
...
public:
    SerializedSCCOffset(uintptr_t recordId, AOTSerializationRecordType recordType, uintptr_t reloDataOffset)
        : _recordIdAndType(AOTSerializationRecord::idAndType(recordId, recordType))
        , _data(reloDataOffset)
    {
...
public:
    ...
    {
        TR_ASSERT_FATAL(recordType == AOTSerializationRecordType::Class,
            "Cannot create AOT depenency using recordType=%d\n", recordType);
    }

    uintptr_t reloDataOffset() const
    {
...
private:
    ...
    SerializedAOTMethod(uintptr_t definingClassChainId, uint32_t index, TR_Hotness optLevel, uintptr_t aotHeaderId,
        size_t numRecords, size_t numDependencies, const void *code, size_t codeSize, const void *data, size_t dataSize,
