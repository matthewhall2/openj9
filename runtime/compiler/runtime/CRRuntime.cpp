...
void TR::CRRuntime::cacheEventsStatus()
{
    ...
    bool exceptionCatchEventHooked = J9_EVENT_IS_HOOKED(jitConfig->javaVM->hookInterface, J9HOOK_VM_EXCEPTION_CATCH)
        || J9_EVENT_IS_RESERVED(jitConfig->javaVM->hookInterface, J9HOOK_VM_EXCEPTION_CATCH);
    bool exceptionThrowEventHooked = J9_EVENT_IS_HOOKED(jitConfig->javaVM->hookInterface, J9HOOK_VM_EXCEPTION_THROW)
...
void TR::CRRuntime::acquireCRMonitor() { _crMonitor->enter(); }

void TR::CRRuntime::releaseCRMonitor() { _crMonitor->exit(); }

void TR::CRRuntime::waitOnCRMonitor() { _crMonitor->wait(); }

void TR::CRRuntime::acquireCompMonitor() { _compMonitor->enter(); }

void TR::CRRuntime::releaseCompMonitor() { _compMonitor->exit(); }

void TR::CRRuntime::acquireCRRuntimeMonitor() { _crRuntimeMonitor->enter(); }

void TR::CRRuntime::releaseCRRuntimeMonitor() { _crRuntimeMonitor->exit(); }

void TR::CRRuntime::waitOnCRRuntimeMonitor() { _crRuntimeMonitor->wait(); }

template<typename T>
void TR::CRRuntime::pushMemoizedCompilation(TR_MemoizedCompilations &list, J9Method *method, void *data)
{
...
J9Method *TR::CRRuntime::popMemoizedCompilation(TR_MemoizedCompilations &list, void **data)
{
...
template<typename T> void TR::CRRuntime::removeMemoizedCompilation(TR_MemoizedCompilations &list, T *entryToRemove)
{
    if (!list.isEmpty()) {
...
template<typename T> void TR::CRRuntime::removeMethodsFromMemoizedCompilations(T *entryToRemove)
{
...
void TR::CRRuntime::purgeMemoizedCompilation(TR_MemoizedCompilations &list)
{
    while (!list.isEmpty()) {
        jitPersistentFree(list.pop());
    }
...
void TR::CRRuntime::purgeMemoizedCompilations()
{
    OMR::CriticalSection removeMemoizedCompilations(getCRRuntimeMonitor());
...
void TR::CRRuntime::pushFailedCompilation(J9Method *method)
{
    pushMemoizedCompilation<TR_MemoizedComp>(_failedComps, method);
}

void TR::CRRuntime::pushForcedRecompilation(J9Method *method)
{
    pushMemoizedCompilation<TR_MemoizedComp>(_forcedRecomps, method);
}

void TR::CRRuntime::pushImportantMethodForCR(J9Method *method)
{
    pushMemoizedCompilation<TR_MemoizedComp>(_impMethodForCR, method);
}

void TR::CRRuntime::pushJNIAddr(J9Method *method, void *addr)
{
    pushMemoizedCompilation<TR_JNIMethodAddr>(_jniMethodAddr, method, addr);
}

void TR::CRRuntime::resetJNIAddr()
{
...
void TR::CRRuntime::triggerRecompilationForPreCheckpointGeneratedFSDBodies(J9VMThread *vmThread)
{
        ...
        if (_compInfo->isCompiled(method)) {
            if (TR::Options::getCmdLineOptions()->getVerboseOption(TR_VerboseCheckpointRestoreDetails))
                TR_VerboseLog::writeLineLocked(TR_Vlog_CHECKPOINT_RESTORE,
                    "%p Attempting to force %p for recompilation", vmThread, method);
...
void TR::CRRuntime::setupEnvForProactiveCompilation(J9JavaVM *javaVM, J9VMThread *vmThread, TR_J9VMBase *fej9)
{
...
void TR::CRRuntime::teardownEnvForProactiveCompilation(J9JavaVM *javaVM, J9VMThread *vmThread, TR_J9VMBase *fej9)
{
    /* Proactive compilation should not be FSD compiles */
    if (javaVM->internalVMFunctions->isDebugOnRestoreEnabled(javaVM)) {
...
bool TR::CRRuntime::compileMethodsForCheckpoint(J9VMThread *vmThread)
{
...

    {
        ...
        try {
            ...
            if (TR::Options::getCmdLineOptions()->getVerboseOption(TR_VerboseCheckpointRestore))
                TR_VerboseLog::writeLineLocked(TR_Vlog_CHECKPOINT_RESTORE,
                    "Failed to collect methods for compilation before checkpoint");
        }
...
bool TR::CRRuntime::suspendJITThreadsForCheckpoint(J9VMThread *vmThread)
{
...
void TR::CRRuntime::resumeJITThreadsForRestore(J9VMThread *vmThread)
{
 ...
 */
void TR::CRRuntime::resetStartTime()
{
    ...
    if (TR::Options::isAnyVerboseOptionSet())
        TR_VerboseLog::writeLineLocked(TR_Vlog_CHECKPOINT_RESTORE,
            "Start and elapsed time: startTime=%6u, elapsedTime=%6u", (uint32_t)persistentInfo->getStartTime(),
            (uint32_t)persistentInfo->getElapsedTime());
    ...
    if (TR::Options::isAnyVerboseOptionSet())
        TR_VerboseLog::writeLineLocked(TR_Vlog_CHECKPOINT_RESTORE,
            "Reset start and elapsed time: startTime=%6u, elapsedTime=%6u", (uint32_t)persistentInfo->getStartTime(),
            (uint32_t)persistentInfo->getElapsedTime());

    _restoreTime = persistentInfo->getElapsedTime();
}

void TR::CRRuntime::closeLogFiles()
{
...
void TR::CRRuntime::reopenLogFiles()
{
    TR_JitPrivateConfig *privateConfig = (TR_JitPrivateConfig *)getJITConfig()->privateConfig;

    if (privateConfig->vLogFileName) {
        TR_VerboseLog::vlogAcquire();
        privateConfig->vLogFile
            = fileOpen(TR::Options::getCmdLineOptions(), getJITConfig(), privateConfig->vLogFileName, "ab", false);
        TR_VerboseLog::vlogRelease();
    }

    if (privateConfig->rtLogFileName) {
        JITRT_LOCK_LOG(getJITConfig());
        privateConfig->rtLogFile
            = fileOpen(TR::Options::getCmdLineOptions(), getJITConfig(), privateConfig->rtLogFileName, "ab", false);
        JITRT_UNLOCK_LOG(getJITConfig());
...
void TR::CRRuntime::prepareForCheckpoint()
{
    ...
    {
        ReleaseVMAccessAndAcquireMonitor suspendCompThreadsForCheckpointCriticalSection(vmThread,
            getCompilationMonitor());

        if (TR::Options::_sleepMsBeforeCheckpoint) {
            if (TR::Options::getCmdLineOptions()->getVerboseOption(TR_VerboseCheckpointRestore))
                TR_VerboseLog::writeLineLocked(TR_Vlog_CHECKPOINT_RESTORE, "Sleeping for %d ms",
                    TR::Options::_sleepMsBeforeCheckpoint);
        ...
        if (!TR::Options::getCmdLineOptions()->getOption(TR_DisableDataCacheDisclaiming)
            ...
            U_32 ghostFileLimit
                = std::max(vm->jitConfig->dataCacheKB, vm->jitConfig->codeCacheTotalKB) * 1024; // convert to bytes
        ...
        printIprofilerStats(TR::Options::getCmdLineOptions(), _jitConfig,
            TR_J9VMBase::get(_jitConfig, NULL)->getIProfiler(), "Checkpoint");
    }
...
void TR::CRRuntime::prepareForRestore()
{
    ...
    if (TR::Options::getCmdLineOptions()->getVerboseOption(TR_VerboseCodeCache)) {
        TR_VerboseLog::writeLineLocked(TR_Vlog_CODECACHE, "At Checkpoint Restore:: Swap Memory is %s",
...
void TR::CRRuntime::recompileMethodsCompiledPreCheckpoint()
{
...
bool TR::CRRuntime::allowStateChange()
{
    TR::PersistentInfo *persistentInfo = getCompInfo()->getPersistentInfo();
    return (_restoreTime != 0)
        && ((persistentInfo->getElapsedTime() - _restoreTime) > TR::Options::_delayBeforeStateChange);
}

void TR::CRRuntime::process()
{
    acquireCRRuntimeMonitor();
    do {
...
void TR::CRRuntime::startCRRuntimeThread(J9JavaVM *javaVM)
{
    ...
    if (javaVM->internalVMFunctions->createThreadWithCategory(&_crRuntimeOSThread, defaultOSStackSize, priority, 0,
            &crRuntimeThreadProc, javaVM->jitConfig, J9THREAD_CATEGORY_SYSTEM_JIT_THREAD)) {
...
void TR::CRRuntime::stopCRRuntimeThread()
{
