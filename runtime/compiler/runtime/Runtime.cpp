    ...
    OMR::RuntimeAssumption **sentinel)
{
    ...
    return TR_UnloadedClassPicSite::make(fe, trPersistentMemory, (uintptr_t)classPointer, (uint8_t *)addressToBePatched,
        size, RuntimeAssumptionOnClassRedefinitionPIC, sentinel);
}
    ...
    OMR::RuntimeAssumption **sentinel)
{
    TR_FrontEnd *fe = (TR_FrontEnd *)jitConfig->compilationInfo; // ugly, but codert cannot include VMJ9.h
    if (unresolved) {
        TR_RedefinedClassUPicSite::make(fe, trPersistentMemory, (uintptr_t)classPointer, (uint8_t *)addressToBePatched,
            size, sentinel);
    } else {
        TR_RedefinedClassRPicSite::make(fe, trPersistentMemory, (uintptr_t)classPointer, (uint8_t *)addressToBePatched,
            size, sentinel);
    }
}
...
static void initS390ArrayCopyTable(uint8_t *code)
{
    ...
    static const uint8_t templat3[] = "\xD2\x00\x10\x00\x20\x00" // MVC 0(x,R1),0(R2)
                                      "\x07\xFE\x00\x00" // BR R14
                                      "\x00\x00\x00\x00"
                                      "\x00\x00";
    static const uint8_t zeroTemplate[] = "\x07\xFE\x00\x00"
                                          "\x00\x00\x00\x00"
                                          "\x00\x00\x00\x00"
                                          "\x00\x00\x00\x00";
...
static void initS390ArraySetZeroTable(uint8_t *code)
{
    ...
    static const uint8_t templat3[] = "\xD7\x00\x10\x00\x10\x00" // XC 0(x,R1),0(R1)
                                      "\x07\xFE" // BR R14
                                      "\x00\x00\x00\x00"
                                      "\x00\x00\x00\x00";
    static const uint8_t zeroTemplate[] = "\x07\xFE\x00\x00" // BR R14
                                          "\x00\x00\x00\x00"
                                          "\x00\x00\x00\x00"
                                          "\x00\x00\x00\x00";
...
static void initS390ArraySetGeneralTable(uint8_t *code)
{
    ...
    static const uint8_t templat3[] = "\x42\x20\x10\x00" // STC R2,0(,R1)
    ...
    static const uint8_t zeroTemplate[] = "\x07\xFE\x00\x00" // BR R14
                                          ...
                                          "\x00\x00\x00\x00"
                                          "\x00\x00\x00\x00";
    static const uint8_t oneTemplate[] = "\x42\x20\x10\x00" // STC R2,0(,R1)
                                         "\x07\xFE\x00\x00" // BR R14
                                         "\x00\x00\x00\x00"
                                         "\x00\x00\x00\x00";
...
static void initS390ArrayCmpTable(uint8_t *code)
{
    ...
    static const uint8_t templat3[] = "\xD5\x00\x10\x00\x20\x00" // CLC 0(x,R1),0(R2)
                                      "\x07\xFE\x00\x00" // BR R14
                                      "\x00\x00\x00\x00"
                                      "\x00\x00";
    static const uint8_t zeroTemplate[] = "\x17\x00" // XR R0,R0 (set CC to 0)
                                          "\x07\xFE" // BR R14
                                          "\x00\x00\x00\x00"
                                          "\x00\x00\x00\x00"
                                          "\x00\x00\x00\x00";
...
static void initS390ArrayTranslateAndTestTable(uint8_t *code)
{
    ...
    //           R0 may be clobbered as part of the call
    //           CC is set based on CLC operation for perusal from mainline code
    static const uint8_t templat3[] = "\xDD\x00\x10\x00\x30\x00" // TRT 0(x,R1),0(R3)
                                      "\x07\xFE\x00\x00" // BR R14
                                      "\x00\x00\x00\x00"
                                      "\x00\x00";
    static const uint8_t zeroTemplate[] = "\x19\x00" // CR R0,R0 (set zero flag)
                                          "\x07\xFE" // BR R14
                                          "\x00\x00\x00\x00"
                                          "\x00\x00\x00\x00"
                                          "\x00\x00\x00\x00";
...
static void initS390Long2StringTable(uint8_t *code)
{
    ...
    static const uint8_t templat3[] = "\xE2\x00\x20\x00\x10\x00" // UNPKU 0(x,R2),0(R1)
                                      "\x07\xFE\x00\x00" // BR R14
                                      "\x00\x00\x00\x00"
                                      "\x00\x00";
    static const uint8_t zeroTemplate[] = "\x07\xFE" // BR R14
                                          "\x00\x00"
                                          "\x00\x00\x00\x00"
                                          "\x00\x00\x00\x00"
                                          "\x00\x00\x00\x00";
...
static void initS390ArrayBitOpMemTable(uint8_t *code, uint8_t opcode)
{
    ...
    //           R0 may be clobbered as part of the call
    //           R1,R2 are the dest/src arrays
    static uint8_t templat3[] = "\x00\x00\x10\x00\x20\x00" // SSInstruction 0(x,R1),0(R2)
                                "\x07\xFE\x00\x00" // BR R14
                                "\x00\x00\x00\x00"
                                "\x00\x00";
    static const uint8_t zeroTemplate[] = "\x07\xFE\x00\x00" // BR R14
...
static void initS390WriteOnceHelpers(J9JITConfig *jitConfig, enum TR_RuntimeHelper arrayCopyHelper,
    enum TR_RuntimeHelper arraySetZeroHelper, enum TR_RuntimeHelper arraySetGeneralHelper,
    enum TR_RuntimeHelper arrayCmpHelper)
{
    J9JavaVM *javaVM = jitConfig->javaVM;
    PORT_ACCESS_FROM_JAVAVM(javaVM);
    UDATA codeSize = (9) * 4096; // 9 tables of 4K each

    /* Memory Leak: The base address of this allocated memory is never stored, so it can't possibly ever be
     * de-allocated. If ever this memory is to be de-allocated then a reference to the J9PortVmenIdentifier must be
     * maintained and passed to j9vmem_free_memory.
     */
    J9PortVmemParams vmemParams;
    j9vmem_vmem_params_init(&vmemParams);

    UDATA allocMode = J9PORT_VMEM_MEMORY_MODE_READ | J9PORT_VMEM_MEMORY_MODE_WRITE | J9PORT_VMEM_MEMORY_MODE_EXECUTE
...
void initializeCodeRuntimeHelperTable(J9JITConfig *jitConfig, char isSMP)
{
    ...
    SET(TR_writeBarrierStoreGenerationalAndConcurrentMark, (void *)jitWriteBarrierStoreGenerationalAndConcurrentMark,
        TR_Helper);
    ...
    SET(TR_throwUnreportedException, (void *)jitThrowUnreportedException,
        TR_Helper); // used for throwing exceptions from synthetic handlers
    ...
    SET(TR_X86interpreterUnresolvedClassFromStaticFieldGlue, (void *)interpreterUnresolvedClassFromStaticFieldGlue,
        TR_Helper);
    ...
    SET(TR_interpreterUnresolvedMethodTypeTableEntryGlue, (void *)interpreterUnresolvedMethodTypeTableEntryGlue,
        TR_Helper);
    SET(TR_X86interpreterUnresolvedStaticFieldGlue, (void *)interpreterUnresolvedStaticFieldGlue, TR_Helper);
    SET(TR_X86interpreterUnresolvedStaticFieldSetterGlue, (void *)interpreterUnresolvedStaticFieldSetterGlue,
        TR_Helper);
    ...
    SET(TR_AMD64JitMethodMonitorEnterPreservingReservation, (void *)jitMethodMonitorEnterPreservingReservation,
        TR_CHelper);
    SET(TR_AMD64JitMethodMonitorExitPreservingReservation, (void *)jitMethodMonitorExitPreservingReservation,
        TR_CHelper);
    ...
    SET(TR_IA32JitMethodMonitorEnterPreservingReservation, (void *)jitMethodMonitorEnterPreservingReservation,
        TR_CHelper);
    SET(TR_IA32JitMethodMonitorExitPreservingReservation, (void *)jitMethodMonitorExitPreservingReservation,
        TR_CHelper);
    ...
    SET(TR_interpreterUnresolvedCallSiteTableEntryGlue, (void *)_interpreterUnresolvedCallSiteTableEntryGlue,
        TR_Helper);
    SET(TR_interpreterUnresolvedMethodTypeTableEntryGlue, (void *)_interpreterUnresolvedMethodTypeTableEntryGlue,
        TR_Helper);
    ...
    SET(TR_PPCinterpreterUnresolvedInstanceDataStoreGlue, (void *)_interpreterUnresolvedInstanceDataStoreGlue,
        TR_Helper);
    ...
    SET(TR_ARMinterpreterUnresolvedInstanceDataStoreGlue, (void *)_interpreterUnresolvedInstanceDataStoreGlue,
        TR_Helper);
    ...
    SET(TR_ARM64interpreterUnresolvedInstanceDataStoreGlue, (void *)_interpreterUnresolvedInstanceDataStoreGlue,
        TR_Helper);
    ...
    SET(TR_S390interpreterUnresolvedInstanceDataStoreGlue, (void *)_interpreterUnresolvedInstanceDataStoreGlue,
        TR_Helper);
    ...
    initS390WriteOnceHelpers(jitConfig, TR_S390arrayCopyHelper, TR_S390arraySetZeroHelper, TR_S390arraySetGeneralHelper,
        TR_S390arrayCmpHelper);
...
int32_t adjustPatchingOffset(void *startPC)
{
    ...
    return (linkageInfo->isSamplingMethodBody() ? -SAMPLING_CALL_SIZE : 0)
        + (linkageInfo->isRecompMethodBody() ? -JITTED_BODY_INFO_SIZE : 0);
}
...
void _fsdSwitchToInterpPatchEntry(void *intEP)
{
    ...
    uint32_t brcEncoding = 0xA7F40000
        | (0x0000FFFF & ((OFFSET_INTEP_VM_CALL_HELPER_TRAMPOLINE - (int16_t)linkageInfo->getJitEntryOffset()) / 2));
...
extern "C" OMR::CodeCacheMethodHeader *getCodeCacheMethodHeader_C_callable(char *p, int searchLimit,
            ...
            while (current < end) {
                ...
                if (hdr->type == J9DataTypeExceptionInfo) {
                    ...
                    if (metaData->constantPool != NULL) {
                        ...
                        ALWAYS_TRIGGER_J9HOOK_VM_DYNAMIC_CODE_LOAD(vm->hookInterface, currentThread,
                            metaData->ramMethod, (void *)metaData->startPC, metaData->endWarmPC - metaData->startPC,
                            "JIT warm body", metaData);
                        if (metaData->startColdPC != 0) {
                            ALWAYS_TRIGGER_J9HOOK_VM_DYNAMIC_CODE_LOAD(vm->hookInterface, currentThread,
                                metaData->ramMethod, (void *)metaData->startColdPC,
                                metaData->endPC - metaData->startColdPC, "JIT cold body", metaData);
                        }
                        ...
                        if (ccMethodHeader && (metaData->bodyInfo != NULL)) {
                            ...
                            if (linkageInfo->isRecompMethodBody()) {
                                ALWAYS_TRIGGER_J9HOOK_VM_DYNAMIC_CODE_LOAD(vm->hookInterface, currentThread,
                                    metaData->ramMethod, (void *)((char *)ccMethodHeader->_eyeCatcher + 4),
                                    metaData->startPC - (UDATA)((char *)ccMethodHeader->_eyeCatcher + 4),
                                    "JIT method header", metaData);
                            }
                        }
                    }
                }
                ...
                if (hdr->type == J9_JIT_DCE_THUNK_MAPPING) {
                    ...
                    ALWAYS_TRIGGER_J9HOOK_VM_DYNAMIC_CODE_LOAD(vm->hookInterface, currentThread, NULL,
                        (void *)thunk->thunkAddress, *((uint32_t *)thunk->thunkAddress - 2), "JIT virtual thunk", NULL);
                }
...
OMR::CodeCacheMethodHeader *getCodeCacheMethodHeader(char *p, int searchLimit, J9JITExceptionTable *metaData)
{
    ...
    int32_t size = sizeof(
        localCodeCacheMethodHeader->_eyeCatcher); // size of eyecatcher and therefore the search interval as well
    ...
    if (metaData) {
        ...
        localCodeCacheMethodHeader
            = (OMR::CodeCacheMethodHeader *)(metaData->codeCacheAlloc - sizeof(OMR::CodeCacheMethodHeader));
