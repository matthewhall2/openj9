...
bool TR_PersistentClassInfo::isInitialized(bool validate)
{
    ...
    if (comp && comp->compileRelocatableCode() && comp->getOption(TR_UseSymbolValidationManager) && validate
        && initialized) {
...
void TR_PersistentClassInfo::setInitialized(TR_PersistentMemory *persistentMemory)
{
    _classId = getClassId();
...
TR_SubClass *TR_PersistentClassInfo::addSubClass(TR_PersistentClassInfo *subClass)
{
...
void TR_PersistentClassInfo::removeASubClass(TR_PersistentClassInfo *subClassToRemove)
{
...
void TR_PersistentClassInfo::removeSubClasses(TR_PersistentMemory *persistentMemory)
{
    TR_SubClass *scl = _subClasses.getFirst();
...
void TR_PersistentClassInfo::removeUnloadedSubClasses()
{
...
void OMR::RuntimeAssumption::dumpInfo(const char *subclassName)
{
    TR_VerboseLog::write("%s@%p: key=%p", subclassName, this, _key);
}

bool OMR::RuntimeAssumption::isAssumingRange(uintptr_t rangeStartPC, uintptr_t rangeEndPC, uintptr_t rangeColdStartPC,
    uintptr_t rangeColdEndPC, uintptr_t rangeStartMD, uintptr_t rangeEndMD)
{
...
bool OMR::RuntimeAssumption::isAssumingMethod(void *md, bool reclaimPrePrologueAssumptions)
{
    ...
    if (assumptionInRange((uintptr_t)metaData, ((uintptr_t)metaData) + ((uintptr_t)metaData->size))) {
        ...
        if (bodyInfo && bodyInfo->getMethodInfo() && bodyInfo->getMethodInfo()->isInDataCache()) {
            ...
            if (!assumptionInRange((uintptr_t)metaData->bodyInfo,
                    ((uintptr_t)metaData->bodyInfo) + (uintptr_t)sizeof(TR_PersistentJittedBodyInfo)
                        + (uintptr_t)sizeof(TR_PersistentMethodInfo)))
                return true;
        } else {
            return true;
        }
...
void TR_RuntimeAssumptionTable::addAssumption(OMR::RuntimeAssumption *a, TR_RuntimeAssumptionKind kind, TR_FrontEnd *fe,
    ...
    for (int kind = 0; _marked > 0 && cleanupCount != 0 && kind < LastAssumptionKind; kind++) // for each table
    {
        if (_detachPending[kind] == true) // Is there anything to remove from this table?
        {
            ...
            for (size_t i = 0; cleanupCount != 0 && i < hashTable->_spineArraySize; ++i) // for each bucket in the table
            {
                ...
                for (cursor = hashTable->_htSpineArray[i], prev = NULL;
                     cursor && cleanupCount != 0 && hashTable->_markedforDetachCount[i] > 0; cursor = next) {
...
void TR_RuntimeAssumptionTable::reclaimAssumptions(OMR::RuntimeAssumption **sentinel, void *metaData,
    bool reclaimPrePrologueAssumptions)
{
    ...
    if (*sentinel != NULL) // list is not empty
    {
        ...
        if (numAssumptionsNotReclaimed > 0) {
            ...
            while (cursor != sentry) {
                TR_RuntimeAssumptionKind kind = cursor->getAssumptionKind();
                if (kind != RuntimeAssumptionOnClassRedefinitionPIC && kind != RuntimeAssumptionOnClassRedefinitionUPIC
                    && kind != RuntimeAssumptionOnClassRedefinitionNOP) {
                    fprintf(stderr, "%d assumptions were left after metadata %p assumption reclaiming\n",
                        numAssumptionsNotReclaimed, metaData);
                    fprintf(stderr, "RA=%p kind=%d key=%p assumingPC=%p\n", cursor, cursor->getAssumptionKind(),
                        (void *)cursor->getKey(), cursor->getFirstAssumingPC());
...
int32_t TR_RuntimeAssumptionTable::countRatAssumptions()
{
    ...
    for (int k = 0; k < LastAssumptionKind; k++) // for each table
    {
        ...
        {
            for (OMR::RuntimeAssumption *cursor = hashTable->_htSpineArray[i]; cursor;
                 cursor = cursor->getNext()) // for each entry
                count++;
        }
    }
    return count;
...
{
    J9JITExceptionTable *metaData = (J9JITExceptionTable *)md;
    reclaimAssumptions((OMR::RuntimeAssumption **)(&metaData->runtimeAssumptionList), metaData,
        reclaimPrePrologueAssumptions);
 ...
 */
void TR_RuntimeAssumptionTable::notifyIllegalStaticFinalFieldModificationEvent(TR_FrontEnd *vm, void *key)
{
    OMR::CriticalSection notifyIllegalStaticFinalFieldModificationEvent(assumptionTableMutex);
    bool reportDetails = TR::Options::getCmdLineOptions()->getVerboseOption(TR_VerboseRuntimeAssumptions);

    OMR::RuntimeAssumption **headPtr
        = getBucketPtr(RuntimeAssumptionOnStaticFinalFieldModification, hashCode((uintptr_t)key));
...
void TR_RuntimeAssumptionTable::notifyClassUnloadEvent(TR_FrontEnd *vm, bool isSMP, TR_OpaqueClassBlock *assumingClass,
    TR_OpaqueClassBlock *unloadedClass)
{
    ...
    for (TR_UnloadedClassPicSite *cursor = (TR_UnloadedClassPicSite *)(*headPtr); cursor;
         cursor = (TR_UnloadedClassPicSite *)cursor->getNext()) {
        if (cursor->matches((uintptr_t)assumingClass)
            && ((unloadedClass == assumingClass) || (cursor->getPicEntry() == (uintptr_t)unloadedClass))) {
            cursor->compensate(vm, 0, 0);
...
void TR_UnloadedClassPicSite::dumpInfo()
{
...
void TR_RuntimeAssumptionTable::notifyClassRedefinitionEvent(TR_FrontEnd *vm, bool isSMP, void *oldKey, void *newKey)
{
    OMR::CriticalSection notifyClassRedefinitionEvent(assumptionTableMutex);
    bool reportDetails = TR::Options::getCmdLineOptions()->getVerboseOption(TR_VerboseRuntimeAssumptions);

    OMR::RuntimeAssumption **oldHeadPtr
        = getBucketPtr(RuntimeAssumptionOnClassRedefinitionPIC, hashCode((uintptr_t)oldKey));
    OMR::RuntimeAssumption **newHeadPtr
        = getBucketPtr(RuntimeAssumptionOnClassRedefinitionPIC, hashCode((uintptr_t)newKey));
    ...
    if (reportDetails) {
        TR_VerboseLog::CriticalSection vlogLock;
        TR_VerboseLog::writeLine(TR_Vlog_RA, "Scanning for PIC assumptions for %p in array %p bucket %p", oldKey,
            raArray, oldHeadPtr);
        if (!pic_cursor)
    ...
    if (reportDetails) {
        TR_VerboseLog::CriticalSection vlogLock;
        TR_VerboseLog::writeLine(TR_Vlog_RA, "Scanning for NOP assumptions for %p in array %p bucket %p", oldKey,
            raArray, oldHeadPtr);
        if (!nop_cursor)
        ...
        while (pic_cursor) {
            ...
            if (!pic_cursor->isForAddressMaterializationSequence()) {
                ...
                if ((uintptr_t)oldKey == resolvedKey) {
                    if (reportDetails)
                        TR_VerboseLog::writeLineLocked(TR_Vlog_RA, "old=%p resolved=%p @ %p patching new=%p",
                            initialKey, resolvedKey, pic_cursor->getPicLocation(),
                            *(uintptr_t *)(pic_cursor->getPicLocation()));
    ...
    for (size_t index = 0; index < classRedefinitionUPICAssumptionTable->_spineArraySize; index++) {
        ...
        while (pic_cursor) {
            ...
            if (pic_cursor->isForAddressMaterializationSequence()) {
                ...
                bool isAddressMaterialization = (location[0] & 0xf0) == 0x40 // REX
                    && (location[1] & 0xf8) == 0xb8; // MOV
                uintptr_t *immLocation = (uintptr_t *)(location + 2);
#else
                bool isAddressMaterialization = (location[0] & 0xf8) == 0xb8; // MOV
                ...
                if (isAddressMaterialization) {
                    ...
                    if ((uintptr_t)oldKey == resolvedKey) {
                        if (reportDetails)
                            TR_VerboseLog::writeLineLocked(TR_Vlog_RA, "old=%p resolved=%p @ %p+2 patching new=%p",
                                initialKey, resolvedKey, location, newKey);
        ...
        while (pic_cursor) {
            ...
            if (pic_cursor->isForAddressMaterializationSequence()) {
                ...
                if (oldKey4 == (resolvedKey4 & 0xffff) && oldKey3 == (resolvedKey3 & 0xffff)
                    ...
                    bool isAddressMaterialization = ((resolvedKey1 >> 26) == 0x0f) // lis
                    ...
                    if (isAddressMaterialization) {
                        TR_VerboseLog::CriticalSection vlogLock(reportDetails);
                        if (reportDetails) {
                            TR_VerboseLog::write(TR_Vlog_RA, "o=%p @ %p  r=%p %p %p %p", initialKey,
                                pic_cursor->getPicLocation(), resolvedKey1, resolvedKey2, resolvedKey3, resolvedKey4);
                        }

                        *(uint32_t *)(pic_cursor->getPicLocation())
                            = resolvedKey1 & 0xffff0000 | (uint32_t)((uintptr_t)newKey >> 48);
                        *(uint32_t *)(pic_cursor->getPicLocation() + 4)
                            = resolvedKey2 & 0xffff0000 | (uint32_t)((uintptr_t)newKey >> 32 & 0xffff);
                        *(uint32_t *)(pic_cursor->getPicLocation() + 12)
                            = resolvedKey3 & 0xffff0000 | (uint32_t)((uintptr_t)newKey >> 16 & 0xffff);
                        *(uint32_t *)(pic_cursor->getPicLocation() + 16)
                            = resolvedKey4 & 0xffff0000 | (uint32_t)((uintptr_t)newKey & 0xffff);
                ...
                if (oldKey2 == (resolvedKey2 & 0xffff) && oldKey1 == (resolvedKey1 & 0xffff)) {
                    ...
                    if (isAddressMaterialization) {
                        TR_VerboseLog::CriticalSection vlogLock(reportDetails);
                        if (reportDetails)
                            TR_VerboseLog::write(TR_Vlog_RA, "o=%p r=%p %p @ %p", initialKey, resolvedKey1,
                                resolvedKey2, pic_cursor->getPicLocation());

                        *(uint32_t *)(pic_cursor->getPicLocation())
                            = resolvedKey1 & 0xffff0000 | HI_VALUE((uint32_t)newKey);
                        *(uint32_t *)(pic_cursor->getPicLocation() + 4)
                            = resolvedKey2 & 0xffff0000 | LO_VALUE((uint32_t)newKey);

                        if (reportDetails)
                            TR_VerboseLog::writeLine(" patched n=%p %p", *(uint32_t *)(pic_cursor->getPicLocation()),
                                *(uint32_t *)(pic_cursor->getPicLocation() + 4));
                    }
                }
#endif
...
void TR_RuntimeAssumptionTable::notifyMethodBreakpointed(TR_FrontEnd *fe, TR_OpaqueMethodBlock *method)
{
    ...
    while (cursor) {
        ...
        if (cursor->matches((uintptr_t)method)) {
            if (reportDetails) {
                TR_VerboseLog::CriticalSection vlogLock;
                TR_VerboseLog::write(TR_Vlog_RA, "compensating key (breakpointed method) " UINT64_PRINTF_FORMAT_HEX " ",
                    method);
                cursor->dumpInfo();
                TR_VerboseLog::writeLine("");
            }
...
void TR_RuntimeAssumptionTable::notifyMutableCallSiteChangeEvent(TR_FrontEnd *fe, uintptr_t cookie)
{
    OMR::CriticalSection notifyMutableCallSiteChangeEvent(assumptionTableMutex);
    ...
    while (cursor) {
        TR_PatchNOPedGuardSiteOnMutableCallSiteChange *next
            = (TR_PatchNOPedGuardSiteOnMutableCallSiteChange *)cursor->getNext();
...
void TR_RedefinedClassPicSite::dumpInfo()
{
...
TR_PseudoRandomNumbersListElement *J9::PersistentInfo::advanceCurPseudoRandomNumbersListElem()
{
    if (_curPseudoRandomNumbersListElem == NULL) {
        _curIndex = 0;
        _curPseudoRandomNumbersListElem = _pseudoRandomNumbersListHead;
    } else {
        if ((_curIndex == PSEUDO_RANDOM_NUMBERS_SIZE) || (_curIndex == _curPseudoRandomNumbersListElem->_curIndex)) {
...
bool J9::PersistentInfo::ensureUnloadedAddressSetsAreInitialized()
{
...
bool J9::PersistentInfo::isUnloadedClass(void *v, bool yesIReallyDontCareAboutHCR)
{
#if defined(J9VM_OPT_JITSERVER)
...
bool J9::PersistentInfo::isObsoleteClass(void *v, TR_FrontEnd *fe)
{
    TR::Compilation *comp = TR::comp();
...
bool J9::PersistentInfo::isInUnloadedMethod(uintptr_t address)
{
#if defined(J9VM_OPT_JITSERVER)
    TR_ASSERT(getRemoteCompilationMode() != JITServer::SERVER,
        "JITServer does not maintain unloaded method ranges, this method should not be called");
#endif
...
void J9::PersistentInfo::addUnloadedClass(TR_OpaqueClassBlock *clazz, uintptr_t startAddress, uint32_t size)
{
    OMR::CriticalSection addUnloadedClass(assumptionTableMutex);
    TR_ASSERT(_unloadedClassAddresses && _unloadedMethodAddresses,
        "Address sets should be initialized before TR::PersistentInfo::addUnloadedClass");
...
void TR_AddressSet::add(uintptr_t start, uintptr_t end)
{
    ...
    if (firstHigherThanStartIndex == firstHigherThanEndIndex) {
        ...
        if ((firstHigherIndex < _numAddressRanges) && (firstHigher.covers(start) || firstHigher.covers(end))) {
            trace("1) Intersects existing range %d [%p - %p] to %p\n", firstHigherIndex, firstHigher.getStart(),
                firstHigher.getEnd(), this);
            ...
            if (extendUpCost > 0 && extendDownCost > 0) {
                ...
                if (bestMergeCost < MAX_UINTPTRJ)
                    traceDetails("   Cheapest ranges to merge: %d [%p - %p] and %d [%p - %p]\n", bestMergeIndex,
                        _addressRanges[bestMergeIndex].getStart(), _addressRanges[bestMergeIndex].getEnd(),
                        1 + bestMergeIndex, _addressRanges[1 + bestMergeIndex].getStart(),
                        _addressRanges[1 + bestMergeIndex].getEnd());
                else
            ...
            if (bestMergeCost < std::min(extendDownCost, extendUpCost)) {
                ...
                trace("3) Merging range %d [%p - %p] into range %d [%p - %p] to make room in %p; cost=%p\n",
                    bestMergeIndex, mergeRange1.getStart(), mergeRange1.getEnd(), bestMergeIndex + 1,
                    mergeRange2.getStart(), mergeRange2.getEnd(), this, bestMergeCost);
                ...
                trace("4) Extending range %d [%p - %p] down in %p; cost=%p\n", firstHigherIndex, firstHigher.getStart(),
                    firstHigher.getEnd(), this, extendDownCost);
                firstHigher.add(start, end);
            } else {
                trace("5) Extending range %d [%p - %p] up in %p; cost=%p\n", lastLowerIndex, lastLower.getStart(),
                    lastLower.getEnd(), this, extendUpCost);
                lastLower.add(start, end);
            }
        }
    } else {
        // It's a bit complicated -- the given range overlaps some existing ranges
        TR_ASSERT(firstHigherThanStartIndex < firstHigherThanEndIndex,
            "If indices are not equal, start must be less than end");
        ...
        if (firstHigherThanEndIndex < _numAddressRanges && firstHigherThanEnd.covers(end)) {
            trace("6) Collapsing ranges from %d [%p - %p] to %d [%p - %p] in %p and setting start = %p\n",
                ...
                firstHigherThanEndIndex, firstHigherThanEnd.getStart(), firstHigherThanEnd.getEnd(), this, newStart);
            ...
            if (firstHigherThanEndIndex - firstHigherThanStartIndex == 1) {
                ...
                trace("7) Intersects existing range %d [%p - %p] in %p\n", firstHigherThanStartIndex,
                    firstHigherThanStart.getStart(), firstHigherThanStart.getEnd(), this);
                firstHigherThanStart.add(start, end);
            } else {
                trace("8) Collapsing ranges from %d [%p - %p] to %d [%p - %p] in %p and setting end = %p\n",
                    firstHigherThanStartIndex, firstHigherThanStart.getStart(), firstHigherThanStart.getEnd(),
                    firstHigherThanEndIndex - 1, _addressRanges[firstHigherThanEndIndex - 1].getStart(),
                    _addressRanges[firstHigherThanEndIndex - 1].getEnd(), this, end);
    ...
    if (enableTraceDetails()) {
        ...
        for (int i = 0; i < _numAddressRanges; i += 4) {
            ...
            for (int j = 0; (j < 4) && (i + j < _numAddressRanges); j++)
                fprintf(stderr, " %4d [%#" OMR_PRIxPTR " - %#" OMR_PRIxPTR "]", i + j, _addressRanges[i + j].getStart(),
                    _addressRanges[i + j].getEnd());
            fprintf(stderr, "\n");
        }
    }
