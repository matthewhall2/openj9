...
#endif
{
    ...
    if ((trace && (numSymsThatShareSlot > 0)) || details) {
        ...
        TR_VerboseLog::writeLine(TR_Vlog_OSR,
            "%x prepareForOSR at %p (startPC %p +%d) at %d:%x numSharingSyms:%d totalSlots:%d vmThread=%p",
            (int)vmThreadArg, metaData->startPC + jitPCOffset, metaData->startPC, (int)jitPCOffset,
        ...
        if (details) {
            ...
            if (currentInlinedSiteIndex != -1) {
                inlinedMethod = (TR_OpaqueMethodBlock *)getInlinedMethod(
                    getInlinedCallSiteArrayElement(metaData, currentInlinedSiteIndex));
                fe->printTruncatedSignature(inlinedMethodSignature, sizeof(inlinedMethodSignature), inlinedMethod);
            }

            if (inlinedMethod)
                TR_VerboseLog::writeLine(TR_Vlog_OSRD, "%X   Inlined method: %s", (int)vmThreadArg,
                    inlinedMethodSignature);
            TR_VerboseLog::writeLine(TR_Vlog_OSRD,
                "%X   osrBuffer=%p osrFrame=%p, osrReturnAddress=%p osrScratchBuffer=%p osrJittedFrameCopy=%p",
                (int)vmThreadArg, vmThread->osrBuffer, osrFrame, vmThread->osrReturnAddress, vmThread->osrScratchBuffer,
                vmThread->osrJittedFrameCopy);
            ...
            TR_VerboseLog::writeLine(TR_Vlog_OSRD,
                "%X     OSRFrame: j9method=%p bytecodePC=%x numberOfLocals=%d maxStack=%d pendingStackHeight=%d "
                "monitorEnterRecords=%p",
                (int)vmThreadArg, osrFrame->method, osrFrame->bytecodePCOffset + osrFrame->method->bytecodes,
                (int)osrFrame->numberOfLocals, (int)osrFrame->maxStack, (int)osrFrame->pendingStackHeight,
                osrFrame->monitorEnterRecords);
            void **arg0EA = (void **)(osrFrame + 1) + osrFrame->numberOfLocals + osrFrame->maxStack - 1;
            int32_t i;
            for (i = osrFrame->pendingStackHeight - 1; i >= 0; --i)
                TR_VerboseLog::writeLine(TR_Vlog_OSRD, "%X       stack %2d: %p", (int)vmThreadArg, i,
                    arg0EA[-i - osrFrame->numberOfLocals]);
    ...
    if (numSymsThatShareSlot > 0) // this is a condition that depends on the current inlined method
    {
        ...
        int32_t numberOfMappings = *osrMetaData;
        osrMetaData++;
        if (details)
        ...
        if (numberOfMappings > 0) // this is a condition that depends on the whole compilation
        {
            ...
            for (i = 0; i < numberOfMappings; i++) {
                int32_t instructionPC = *osrMetaData;
                osrMetaData++;
                if (jitPCOffset < instructionPC) {
                    if (details)
                        TR_VerboseLog::writeLineLocked(TR_Vlog_OSRD, "%X   Found mapping @%d > %d", (int)vmThreadArg,
                            instructionPC, jitPCOffset);
                    break;
                } else {
                    previousMapping = osrMetaData;
                    // skipping through the info of all symbols to get to beginning of the next mapping
                    int32_t numSymbols = *osrMetaData;
                    osrMetaData++;
                    osrMetaData += numSymbols * 4;
                    if (details)
                        TR_VerboseLog::writeLineLocked(TR_Vlog_OSRD, "%X     Skip mapping @%d <= %d with %d symbols",
                            (int)vmThreadArg, instructionPC, jitPCOffset, numSymbols);
                }
            }
            ...
            // In debugging mode, meta data keeps both slots sharing and dead slots info. There are cases where dead
            // slots don't exist in certain bci but do exist in another bci. This is not possible under other OSR modes
            // where only slots sharing info are kept.
            if (!osrMetaData) {
                TR_ASSERT(TR::Options::getCmdLineOptions()->getOption(TR_FullSpeedDebug),
                    "it's only possible to not to find");
                return;
            }
            ...
            for (i = 0; i < numSymbols; i++) {
                int32_t inlinedSiteIndex = *osrMetaData;
                osrMetaData++;
                int32_t osrFrameDataOffset = *osrMetaData;
                osrMetaData++;
                int32_t scratchBufferOffset = *osrMetaData;
                osrMetaData++;
                int32_t symSize = *osrMetaData;
                osrMetaData++;

                if (inlinedSiteIndex != currentInlinedSiteIndex)
                    continue;
                ...
                if (scratchBufferOffset != -1) {
                    ...
                    if (details) {
                        TR_VerboseLog::CriticalSection vlogLock;
                        TR_VerboseLog::write(TR_Vlog_OSRD,
                            "%X     Symbol #%d osrFrameDataOffset=%d scratchBufferOffset=%d size=%d data:",
                            (int)vmThreadArg, i, osrFrameDataOffset, scratchBufferOffset, symSize);
                        switch (symSize) {
                            case 4:
                    ...
                    memcpy((uint8_t *)(osrFrame) + osrFrameDataOffset, dataAtScrBuffer, symSize);
                } else {
                    if (details) {
                        TR_VerboseLog::writeLineLocked(TR_Vlog_OSRD,
                            "%X     Symbol #%d osrFrameDataOffset=%d size=%d data:Zeros", (int)vmThreadArg, i,
                            osrFrameDataOffset, symSize);
                    }
                    memset((uint8_t *)(osrFrame) + osrFrameDataOffset, 0, symSize);
                }
            }
            TR_ASSERT(numSymbolsForThisCallerIndex <= numSymsThatShareSlot,
                "the number of live symbols we are going to write (%d) is more than the number of symbols that share "
                "slots (%d)\n",
