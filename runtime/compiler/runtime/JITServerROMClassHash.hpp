...
static_assert(ROMCLASS_HASH_BITS % (CHAR_BIT * sizeof(size_t)) == 0, "Invalid number of ROMClass hash bits");
...
public:
    ...
    JITServerROMClassHash(const JITServerROMClassHash &objectArrayHash, const JITServerROMClassHash &baseComponentHash,
    ...
    bool operator==(const JITServerROMClassHash &h) const { return memcmp(_data, h._data, sizeof(_data)) == 0; }
    ...
    // Returns the hash of the "[L" ROMClass. Caches the result for reuse in future invocations. Only used on the
    // client.
    static const JITServerROMClassHash &getObjectArrayHash(const J9ROMClass *objectArrayROMClass, TR_Memory &trMemory,
        TR_J9VMBase *fej9);

private:
    void init(const void *data, size_t size);
    void init(const J9ROMClass *romClass, TR_Memory &trMemory, TR_J9VMBase *fej9, bool checkGenerated,
        size_t prefixLength);
    void init(const JITServerROMClassHash &objectArrayHash, const JITServerROMClassHash &baseComponentHash,
        size_t numDimensions);
