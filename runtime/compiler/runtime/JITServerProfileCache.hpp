...
struct BytecodeProfileSummary {
    BytecodeProfileSummary(uint64_t samples, size_t numBC, bool stable)
        : _numSamples(samples)
        , _numProfiledBytecodes(numBC)
        , _stable(stable)
    {}

    BytecodeProfileSummary()
        : _numSamples(0)
        , _numProfiledBytecodes(0)
        , _stable(false)
    {}
...
struct FaninProfileSummary {
    FaninProfileSummary(uint64_t samples)
        : _numSamples(samples)
    {}
...
public:
    ...
    private:
        ...
        PersistentVector<TR_IPBytecodeHashTableEntry *>
            _data; // Allocated with persistentMemory using the global allocator
    ...
    void cloneBytecodeData(TR_Memory &trMemory, bool stable, Vector<TR_IPBytecodeHashTableEntry *> &newEntries,
...
private:
    ...
    const AOTCacheMethodRecord
        * const _methodRecord; // Link back to the AOTCacheMethodRecord associated with this entry
    const J9ROMMethod * const _romMethod;
    J9ROMClass
        * const _romClass; // Counting references to shared ROMClass cache entry prevents deletion of profiling entries
    ...
    // FaninProfile is allocated inline since it has a small size. 0 samples is like the fanin profile has never been
    // cached
...
public:
    ...
    TR_IPBytecodeHashTableEntry *getBytecodeData(const AOTCacheMethodRecord *methodRecord, uint32_t bci, bool &present,
        TR_Memory &trMemory, TR::Region &cgEntryRegion);
