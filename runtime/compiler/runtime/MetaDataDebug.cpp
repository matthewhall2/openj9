    ...
    J9JITExceptionTable *dbgextRemotePtr)
{
    ...
    log->printf("totalFrameSize=%d { Real Frame Size: %d }\n", data->totalFrameSize,
        (data->totalFrameSize + 1) * TR::Compiler->om.sizeofReferenceAddress());
    log->printf("bodyInfo= [%p]\n", data->bodyInfo);
}

void TR_Debug::print(OMR::Logger *log, J9JITExceptionTable *data, TR_ResolvedMethod *feMethod, bool fourByteOffsets)
{
    uintptr_t startPC = (uintptr_t)data->startPC;
    ...
    if (sa)
        offsetInfo = printStackAtlas(log, startPC, sa->getAtlasBits(), sa->getNumberOfSlotsMapped(), fourByteOffsets,
            &sizeOfStackAtlas, data->totalFrameSize);
...
int32_t *TR_Debug::printStackAtlas(OMR::Logger *log, uintptr_t startPC, uint8_t *mapBits, int32_t numberOfSlotsMapped,
    bool fourByteOffsets, int32_t *sizeOfStackAtlas, int32_t frameSize)
{
    ...
    uint16_t indexOfFirstInternalPtr = printStackAtlasDetails(log, startPC, mapBits, numberOfSlotsMapped,
        fourByteOffsets, sizeOfStackAtlas, frameSize, offsetInfo);
    ...
    for (U_32 j = 0; j < stackAtlas->numberOfMaps; j++) {
        ...
        mapBits = printMapInfo(log, startPC, mapBits, numberOfSlotsMapped, fourByteOffsets, sizeOfStackAtlas,
            byteCodeInfo, indexOfFirstInternalPtr, offsetInfo);
    }

    return offsetInfo;
}

uint16_t TR_Debug::printStackAtlasDetails(OMR::Logger *log, uintptr_t startPC, uint8_t *mapBits,
    int32_t numberOfSlotsMapped, bool fourByteOffsets, int32_t *sizeOfStackAtlas, int32_t frameSize,
    int32_t *offsetInfo)
{
    J9JITStackAtlas *stackAtlas = (J9JITStackAtlas *)mapBits;
...
uint8_t *TR_Debug::printMapInfo(OMR::Logger *log, uintptr_t startPC, uint8_t *mapBits, int32_t numberOfSlotsMapped,
    bool fourByteOffsets, int32_t *sizeOfStackAtlas, TR_ByteCodeInfo *byteCodeInfo, uint16_t indexOfFirstInternalPtr,
    int32_t offsetInfo[], bool nummaps)
{
    ...
    if (!nummaps) {
        ...
        log->printf("      byteCodeInfo: <_callerIndex=%d, byteCodeIndex=%d>, _isSameReceiver=%d, _doNotProfile=%d\n",
            byteCodeInfo->getCallerIndex(), byteCodeInfo->getByteCodeIndex(), byteCodeInfo->isSameReceiver(),
            byteCodeInfo->doNotProfile());
    }
...
void TR_Debug::printStackMapInfo(OMR::Logger *log, uint8_t *&mapBits, int32_t numberOfSlotsMapped,
    int32_t *sizeOfStackAtlas, int32_t *offsetInfo, bool nummaps)
{
...
void TR_Debug::printByteCodeStack(OMR::Logger *log, int32_t parentStackIndex, uint16_t byteCodeIndex, size_t *indentLen)
{
    ...
    if (!_comp->fej9()->isAOT_DEPRECATED_DO_NOT_USE()) {
        ...
        if (parentStackIndex == -1) {
            ...
            printByteCodeStack(log, site._byteCodeInfo.getCallerIndex(), site._byteCodeInfo.getByteCodeIndex(),
                indentLen);
        ...
        log->prints(" \\\\");
        j9bcutil_dumpBytecodes(((TR_J9VMBase *)_comp->fej9())->_portLibrary, J9_CLASS_FROM_METHOD(ramMethod)->romClass,
            J9_BYTECODE_START_FROM_RAM_METHOD(ramMethod), byteCodeIndex, byteCodeIndex, flags, bcPrintFunc, log,
            *indentLen);
        *indentLen += 3;
    }
}
