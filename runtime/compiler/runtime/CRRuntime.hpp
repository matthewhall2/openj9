...
public:
    ...
    bool shouldCompileMethodsForCheckpoint()
    {
        return _checkpointStatus == TR_CheckpointStatus::COMPILE_METHODS_FOR_CHECKPOINT;
    }

    void setCompileMethodsForCheckpoint() { _checkpointStatus = TR_CheckpointStatus::COMPILE_METHODS_FOR_CHECKPOINT; }

    bool shouldSuspendThreadsForCheckpoint()
    {
        return _checkpointStatus == TR_CheckpointStatus::SUSPEND_THREADS_FOR_CHECKPOINT;
    }
    ...
    void setCanPerformRemoteCompilationInCRIUMode(bool remoteComp)
    {
        _canPerformRemoteCompilationInCRIUMode = remoteComp;
    }

    bool remoteCompilationRequestedAtBootstrap() { return _remoteCompilationRequestedAtBootstrap; }

    void setRemoteCompilationRequestedAtBootstrap(bool remoteComp)
    {
        _remoteCompilationRequestedAtBootstrap = remoteComp;
    }

    bool remoteCompilationExplicitlyDisabledAtBootstrap() { return _remoteCompilationExplicitlyDisabledAtBootstrap; }

    void setRemoteCompilationExplicitlyDisabledAtBootstrap(bool remoteComp)
    {
        _remoteCompilationExplicitlyDisabledAtBootstrap = remoteComp;
    }
#endif

    TR_CRRuntimeThreadLifetimeStates getCRRuntimeThreadLifetimeState() { return _crRuntimeThreadLifetimeState; }

    void setCRRuntimeThreadLifetimeState(TR_CRRuntimeThreadLifetimeStates state)
    {
        _crRuntimeThreadLifetimeState = state;
    }
     ...
     */
    template<typename T> void removeMethodsFromMemoizedCompilations(T *entryToRemove);
...
private:
    ...
    public:
        ...
        ~SetupEnvForProactiveComp() { _crRuntime->teardownEnvForProactiveCompilation(_javaVM, _vmThread, _fej9); }
     ...
     */
    template<typename T> void removeMemoizedCompilation(TR_MemoizedCompilations &list, T *entryToRemove);
