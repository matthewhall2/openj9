...
struct TR_ByteInfo {
    ...
    TR_ByteInfo(char *source, size_t len)
        : length(len)
        , chars(source)
    {}
    ...
    bool operator<(const TR_ByteInfo &orig) const { return length < orig.length; }
};
...
template<typename T> struct TR_ProfiledValue {
    ...
    bool operator>(const TR_ProfiledValue<T> &b) const { return (_frequency > b._frequency); }
};
...
public:
    TR_AbstractProfilerInfo(TR_ByteCodeInfo &bci)
        : _byteCodeInfo(bci)
    {}
 ...
 */
template<typename T> class TR_HashTableProfilerInfo : public TR_AbstractHashTableProfilerInfo {
public:
 ...
 */
template<typename T, size_t bits> class TR_EmbeddedHashTable : public TR_HashTableProfilerInfo<T> {
public:
 ...
 */
template<typename T> uint32_t TR_HashTableProfilerInfo<T>::getTotalFrequency()
{
 ...
 */
template<typename T> uint32_t TR_HashTableProfilerInfo<T>::getNumProfiledValues()
{
    uint32_t *freqs = getFrequencies();
 ...
 */
template<typename T> uint32_t TR_HashTableProfilerInfo<T>::getTopValue(T &value)
{
 ...
 * \return The frequency for the value.
 */
template<typename T> uint32_t TR_HashTableProfilerInfo<T>::getMaxValue(T &value)
{
    uint32_t topFrequency = 0;
    uint32_t *freqs = getFrequencies();
    T *values = getKeys();
 ...
 */
template<typename T> void TR_HashTableProfilerInfo<T>::getList(TR::vector<TR_ProfiledValue<T>, TR::Region &> &vec)
{
    uint32_t numProfiled = 0;
    uint32_t *freqs = getFrequencies();
    T *values = getKeys();
 ...
 */
template<typename T> void TR_HashTableProfilerInfo<T>::dumpInfo(OMR::Logger *log)
{
    uint32_t *freqs = getFrequencies();
    T *values = getKeys();
    ...
    if (_metaData.hash == BitShiftHash || _metaData.hash == BitIndexHash) {
        ...
        for (uint8_t i = 0; i < getBits(); ++i)
            log->printf("    %01d : %03d - 0x%0*llX\n", i, _hashConfig.shifts[i], padding,
                1 << (_hashConfig.shifts[i] + (_metaData.hash == BitShiftHash ? i : 0)));
    } else
 ...
 */
template<typename T, size_t bits> bool TR_EmbeddedHashTable<T, bits>::resetLowFreqKeys()
{
    uint32_t matched = 0;
 ...
 */
template<typename T, size_t bits> void TR_EmbeddedHashTable<T, bits>::reset()
{
 ...
 */
template<typename T, size_t bits> void TR_EmbeddedHashTable<T, bits>::addKey(T value)
{
 ...
 */
template<typename T> size_t TR_HashTableProfilerInfo<T>::initialHashConfig(HashFunction &hash, T value)
{
 ...
 */
template<typename T> void TR_HashTableProfilerInfo<T>::updateHashConfig(HashFunction &hash, T mask)
{
 ...
 */
template<typename T, size_t bits> T TR_EmbeddedHashTable<T, bits>::recursivelySplit(T mask, T choices)
{
    T key0, key1;
 ...
 */
template<typename T> size_t TR_HashTableProfilerInfo<T>::applyHash(HashFunction &hash, T value)
{
    size_t result = 0;
    if (getHashType() == BitMaskHash) {
        size_t i = 1;
 ...
 */
template<typename T, size_t bits> void TR_EmbeddedHashTable<T, bits>::rearrange(HashFunction &hash)
{
    static bool dumpInfo = feGetEnv("TR_JProfilingValueDumpInfo") != NULL;
    ...
    if (dumpInfo) {
        for (size_t i = 0; i < length; ++i) {
            printf("%" OMR_PRIu64 " -> %" OMR_PRIu64 "\n", static_cast<uint64_t>(i),
                static_cast<uint64_t>(plannedMoves[i]));
        }
    }
    ...
    if (plannedMoves[i] != i) {
        size_t dest = plannedMoves[i];
        TR_ASSERT_FATAL(plannedMoves[dest] == i, "Moves should have simplified to a single swap, %d %d %d %d", i, dest,
            plannedMoves[i], plannedMoves[dest]);
 ...
 */
template<typename T> class TR_LinkedListProfilerInfo : public TR_AbstractProfilerInfo {
public:
    TR_ALLOC(TR_Memory::PersistentProfileInfo)

    TR_LinkedListProfilerInfo(TR_ByteCodeInfo &bci, TR_ValueInfoKind kind, T initialValue, uint32_t initialFreq,
        bool external = false)
        : TR_AbstractProfilerInfo(bci)
        , _kind(kind)
    ...
    void incrementOrCreate(T &value, uintptr_t **addrOfTotalFrequency, uint32_t maxNumValuesProfiled, uint32_t inc = 1,
        TR::Region *region = NULL);
...
template<typename T> TR_LinkedListProfilerInfo<T>::~TR_LinkedListProfilerInfo()
{
    if (_external)
        return;
 ...
 * Get the number of profiled values with non-zero frequencies.
 */
template<typename T> uint32_t TR_LinkedListProfilerInfo<T>::getNumProfiledValues()
{
    OMR::CriticalSection lock(vpMonitor);
 ...
 * \return The frequency for the value.
 */
template<typename T> uint32_t TR_LinkedListProfilerInfo<T>::getTopValue(T &value)
{
    OMR::CriticalSection lock(vpMonitor);
 ...
 * \return The frequency for the value.
 */
template<typename T> uint32_t TR_LinkedListProfilerInfo<T>::getMaxValue(T &value)
{
    OMR::CriticalSection lock(vpMonitor);
 ...
 * \return Reference to vector.
 */
template<typename T> void TR_LinkedListProfilerInfo<T>::getList(TR::vector<TR_ProfiledValue<T>, TR::Region &> &vec)
{
    OMR::CriticalSection lock(vpMonitor);
 ...
 */
template<> void TR_LinkedListProfilerInfo<TR_ByteInfo>::dumpInfo(OMR::Logger *log);

template<typename T> void TR_LinkedListProfilerInfo<T>::dumpInfo(OMR::Logger *log)
{
    OMR::CriticalSection lock(vpMonitor);
 ...
 */
template<typename T> uint32_t TR_LinkedListProfilerInfo<T>::getTotalFrequency(uintptr_t **addrOfTotalFrequency)
{
    OMR::CriticalSection lock(vpMonitor);
 ...
 */
template<typename T>
void TR_LinkedListProfilerInfo<T>::incrementOrCreate(T &value, uintptr_t **addrOfTotalFrequency,
    uint32_t maxNumValuesProfiled, uint32_t inc, TR::Region *region)
{
 ...
 */
template<typename T> class TR_ArrayProfilerInfo : public TR_AbstractProfilerInfo {
public:
    TR_PERSISTENT_ALLOC(TR_Memory::PersistentProfileInfo)
 ...
 * Get the number of profiled values with non-zero frequencies.
 */
template<typename T> uint32_t TR_ArrayProfilerInfo<T>::getNumProfiledValues()
{
    OMR::CriticalSection lock(vpMonitor);
 ...
 * \return The frequency for the value.
 */
template<typename T> uint32_t TR_ArrayProfilerInfo<T>::getTopValue(T &value)
{
    OMR::CriticalSection lock(vpMonitor);
 ...
 * \return The frequency for the value.
 */
template<typename T> uint32_t TR_ArrayProfilerInfo<T>::getMaxValue(T &value)
{
    OMR::CriticalSection lock(vpMonitor);
 ...
 * \return Reference to vector.
 */
template<typename T> void TR_ArrayProfilerInfo<T>::getList(TR::vector<TR_ProfiledValue<T>, TR::Region &> &vec)
{
    OMR::CriticalSection lock(vpMonitor);
 ...
 * Trace properties and content.
 */
template<typename T> void TR_ArrayProfilerInfo<T>::dumpInfo(OMR::Logger *log)
{
    OMR::CriticalSection lock(vpMonitor);
 ...
 */
template<typename T> void TR_ArrayProfilerInfo<T>::incrementOrCreate(const T &value)
{
 ...
 */
template<typename T> class TR_GenericValueInfo : public TR_AbstractInfo {
public:
    TR_GenericValueInfo(TR_AbstractProfilerInfo *profiler)
        : _profiler(profiler)
    {}
...
protected:
    struct DescendingSort {
        bool operator()(const ProfiledValue &a, const ProfiledValue &b) const { return a > b; }
    };
...
public:
    ...
    void getMethodsList(TR::Compilation *, TR_ResolvedMethod *callerMethod, TR_OpaqueClassBlock *calleeClass,
        int32_t vftSlot, MethodVector &sortedList);

    void getMethodsList(TR::Compilation *, TR_ResolvedMethod *callerMethod, TR_OpaqueClassBlock *calleeClass,
        int32_t vftSlot, List<ProfiledMethod> *sortedList);
};
 ...
 */
template<typename T> float TR_GenericValueInfo<T>::getTopProbability()
{
    T tmp;
 ...
 */
template<typename T> T TR_GenericValueInfo<T>::getTopValue()
{
    T temp;
 ...
 * Replicates behaviour of prior API, defaults to 0 if the table is empty.
 */
template<typename T> T TR_GenericValueInfo<T>::getMaxValue()
{
    T temp;
 ...
 */
template<typename T> void TR_GenericValueInfo<T>::getSortedList(Vector &vec)
{
 ...
 */
template<typename T> void TR_GenericValueInfo<T>::getSortedList(TR::Compilation *comp, List<ProfiledValue> *sortedList)
{
