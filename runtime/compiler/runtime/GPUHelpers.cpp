...
static double currentTime()
{
    ...
    if (rc != 0)
        TR_VerboseLog::writeLine(TR_Vlog_GPU, "Error start time");
...
#define checkCudaError(result, error, tracing)                                         \
    {                                                                                  \
        if (result != cudaSuccess) {                                                   \
            if (tracing > 0)                                                           \
                TR_VerboseLog::writeLine(TR_Vlog_GPU, "[time.ms=%lld]: CUDA error %d", \
                    (int64_t)(currentTime() / 1000), result);                          \
            return error;                                                              \
        }                                                                              \
    }
...
#define checkNVVMError(result, tracing)                                                \
    {                                                                                  \
        if (result != NVVM_SUCCESS) {                                                  \
            if (tracing > 0)                                                           \
                TR_VerboseLog::writeLine(TR_Vlog_GPU, "[time.ms=%lld]: NVVM error %d", \
                    (int64_t)(currentTime() / 1000), result);                          \
            return NULL;                                                               \
        }                                                                              \
    }
...
CUresult (*jitCuLinkAddData)(CUlinkState, CUjitInputType, void *, size_t, const char *, unsigned int, CUjit_option *,
    void **);
...
CUresult (*jitCuLaunchKernel)(CUfunction, unsigned int, unsigned int, unsigned int, unsigned int, unsigned int,
...
static void saveSignalHandlers(struct sigaction sigactionArray[], bool trace)
{
    for (int i = 1; i <= 31; i++) {
        ...
        {
            if (trace)
                TR_VerboseLog::writeLine(TR_Vlog_GPU, "\tsigaction error: Can't read sigaction for signal %d", i);
        }
    }
}
...
static void restoreSignalHandlers(struct sigaction sigactionArray[], bool trace)
{
    for (int i = 1; i <= 31; i++) {
        ...
        {
            if (trace)
                TR_VerboseLog::writeLine(TR_Vlog_GPU, "\tsigaction error: Can't set sigaction for signal %d", i);
        }
    }
}
#endif
...
static bool checkDlError(int tracing, bool isNull)
{
    ...
    if (errorMessage) {
        if (tracing > 0)
            TR_VerboseLog::writeLine(TR_Vlog_GPU, "\tDynamic linking error: %s", errorMessage);
        return true;
    }
#endif
    ...
    if (isNull) {
        if (tracing > 0)
            TR_VerboseLog::writeLine(TR_Vlog_GPU, "\tDynamic linking error: null pointer while loading shared library");
        return true;
    }
...
static bool loadNVVMlibrary(int tracing)
{
    ...
    if (tracing > 0 && !libNvvmPointer)
        TR_VerboseLog::writeLine(TR_Vlog_GPU,
            "\tDynamic linking error: Unable to locate NVVM library nvvm64_30_0.dll or nvvm64_20_0.dll");
#endif

    if (checkDlError(tracing, !libNvvmPointer))
        return false;

    jitNvvmCreateProgram = (nvvmResult(*)(nvvmProgram *))GET_FUNCTION(libNvvmPointer, stringMacro(nvvmCreateProgram));
    if (checkDlError(tracing, !jitNvvmCreateProgram))
        return false;

    jitNvvmAddModuleToProgram = (nvvmResult(*)(nvvmProgram, const char *, size_t, const char *))GET_FUNCTION(
        libNvvmPointer, stringMacro(nvvmAddModuleToProgram));
    if (checkDlError(tracing, !jitNvvmAddModuleToProgram))
        return false;

    jitNvvmCompileProgram
        = (nvvmResult(*)(nvvmProgram, int, const char **))GET_FUNCTION(libNvvmPointer, stringMacro(nvvmCompileProgram));
    if (checkDlError(tracing, !jitNvvmCompileProgram))
        return false;

    jitNvvmGetProgramLogSize
        = (nvvmResult(*)(nvvmProgram, size_t *))GET_FUNCTION(libNvvmPointer, stringMacro(nvvmGetProgramLogSize));
    if (checkDlError(tracing, !jitNvvmGetProgramLogSize))
        return false;

    jitNvvmGetProgramLog
        = (nvvmResult(*)(nvvmProgram, char *))GET_FUNCTION(libNvvmPointer, stringMacro(nvvmGetProgramLog));
    if (checkDlError(tracing, !jitNvvmGetProgramLog))
        return false;

    jitNvvmGetCompiledResultSize
        = (nvvmResult(*)(nvvmProgram, size_t *))GET_FUNCTION(libNvvmPointer, stringMacro(nvvmGetCompiledResultSize));
    if (checkDlError(tracing, !jitNvvmGetCompiledResultSize))
        return false;

    jitNvvmGetCompiledResult
        = (nvvmResult(*)(nvvmProgram, char *))GET_FUNCTION(libNvvmPointer, stringMacro(nvvmGetCompiledResult));
    if (checkDlError(tracing, !jitNvvmGetCompiledResult))
        return false;

    jitNvvmDestroyProgram = (nvvmResult(*)(nvvmProgram *))GET_FUNCTION(libNvvmPointer, stringMacro(nvvmDestroyProgram));
    if (checkDlError(tracing, !jitNvvmDestroyProgram))
        return false;

    jitNvvmVersion = (nvvmResult(*)(int *, int *))GET_FUNCTION(libNvvmPointer, stringMacro(nvvmVersion));
    if (checkDlError(tracing, !jitNvvmVersion))
        return false;
...
static bool loadCudaRuntimeLibrary(int tracing)
{
...
#else
    HINSTANCE libCudartPointer
        = LoadLibrary("cudart64_75.dll"); // TODO: change this to support future versions of the runtime library
    ...
    if (tracing > 0 && !libCudartPointer)
        TR_VerboseLog::writeLine(TR_Vlog_GPU,
            "\tDynamic linking error: Unable to locate Cuda Runtime library cudart64_75.dll");
#endif
    if (checkDlError(tracing, !libCudartPointer))
        return false;

    jitCudaGetDeviceCount = (cudaError_t(*)(int *))GET_FUNCTION(libCudartPointer, stringMacro(cudaGetDeviceCount));
    if (checkDlError(tracing, !jitCudaGetDeviceCount))
        return false;

    jitCudaSetDevice = (cudaError_t(*)(int))GET_FUNCTION(libCudartPointer, stringMacro(cudaSetDevice));
    if (checkDlError(tracing, !jitCudaSetDevice))
        return false;

    jitCudaFree = (cudaError_t(*)(void *))GET_FUNCTION(libCudartPointer, stringMacro(cudaFree));
    if (checkDlError(tracing, !jitCudaFree))
        return false;

    jitCudaGetDeviceProperties
        = (cudaError_t(*)(cudaDeviceProp *, int))GET_FUNCTION(libCudartPointer, stringMacro(cudaGetDeviceProperties));
    if (checkDlError(tracing, !jitCudaGetDeviceProperties))
        return false;

    jitCudaDeviceSynchronize = (cudaError_t(*)())GET_FUNCTION(libCudartPointer, stringMacro(cudaDeviceSynchronize));
    if (checkDlError(tracing, !jitCudaDeviceSynchronize))
        return false;

    jitCudaStreamCreate = (cudaError_t(*)(cudaStream_t *))GET_FUNCTION(libCudartPointer, stringMacro(cudaStreamCreate));
    if (checkDlError(tracing, !jitCudaStreamCreate))
        return false;

    jitCudaStreamSynchronize
        = (cudaError_t(*)(cudaStream_t))GET_FUNCTION(libCudartPointer, stringMacro(cudaStreamSynchronize));
    if (checkDlError(tracing, !jitCudaStreamSynchronize))
        return false;

    jitCudaGetErrorString
        = (const char *(*)(cudaError_t))GET_FUNCTION(libCudartPointer, stringMacro(cudaGetErrorString));
    if (checkDlError(tracing, !jitCudaGetErrorString))
        return false;
...
static bool loadCudaLibrary(int tracing)
{
    ...
    if (tracing > 0 && !libCudaPointer)
        TR_VerboseLog::writeLine(TR_Vlog_GPU,
            "\tDynamic linking error: Unable to locate Cuda Driver library nvcuda.dll");
#endif
    if (checkDlError(tracing, !libCudaPointer))
        return false;

    jitCuInit = (CUresult(*)(unsigned int))GET_FUNCTION(libCudaPointer, stringMacro(cuInit));
    if (checkDlError(tracing, !jitCuInit))
        return false;

    jitCuLinkCreate = (CUresult(*)(unsigned int, CUjit_option *, void **, CUlinkState *))GET_FUNCTION(libCudaPointer,
        stringMacro(cuLinkCreate));
    if (checkDlError(tracing, !jitCuLinkCreate))
        return false;

    jitCuLinkAddData = (CUresult(*)(CUlinkState, CUjitInputType, void *, size_t, const char *, unsigned int,
        CUjit_option *, void **))GET_FUNCTION(libCudaPointer, stringMacro(cuLinkAddData));
    if (checkDlError(tracing, !jitCuLinkAddData))
        return false;

    jitCuLinkComplete
        = (CUresult(*)(CUlinkState, void **, size_t *))GET_FUNCTION(libCudaPointer, stringMacro(cuLinkComplete));
    if (checkDlError(tracing, !jitCuLinkComplete))
        return false;

    jitCuModuleLoadData
        = (CUresult(*)(CUmodule *, const void *))GET_FUNCTION(libCudaPointer, stringMacro(cuModuleLoadData));
    if (checkDlError(tracing, !jitCuModuleLoadData))
        return false;

    jitCuDeviceGet = (CUresult(*)(CUdevice *, int))GET_FUNCTION(libCudaPointer, stringMacro(cuDeviceGet));
    if (checkDlError(tracing, !jitCuDeviceGet))
        return false;

    jitCuModuleLoadDataEx = (CUresult(*)(CUmodule *, const void *, unsigned int, CUjit_option *, void **))GET_FUNCTION(
        libCudaPointer, stringMacro(cuModuleLoadDataEx));
    if (checkDlError(tracing, !jitCuModuleLoadDataEx))
        return false;

    jitCuModuleGetFunction = (CUresult(*)(CUfunction *, CUmodule, const char *))GET_FUNCTION(libCudaPointer,
        stringMacro(cuModuleGetFunction));
    if (checkDlError(tracing, !jitCuModuleGetFunction))
        return false;

    jitCuLinkDestroy = (CUresult(*)(CUlinkState))GET_FUNCTION(libCudaPointer, stringMacro(cuLinkDestroy));
    if (checkDlError(tracing, !jitCuLinkDestroy))
        return false;

    jitCuMemAlloc = (CUresult(*)(CUdeviceptr *, size_t))GET_FUNCTION(libCudaPointer, stringMacro(cuMemAlloc));
    if (checkDlError(tracing, !jitCuMemAlloc))
        return false;

    jitCuMemcpyHtoD
        = (CUresult(*)(CUdeviceptr, const void *, size_t))GET_FUNCTION(libCudaPointer, stringMacro(cuMemcpyHtoD));
    if (checkDlError(tracing, !jitCuMemcpyHtoD))
        return false;

    jitCuMemcpyHtoDAsync = (CUresult(*)(CUdeviceptr, const void *, size_t, CUstream))GET_FUNCTION(libCudaPointer,
        stringMacro(cuMemcpyHtoDAsync));
    if (checkDlError(tracing, !jitCuMemcpyHtoDAsync))
        return false;

    jitCuLaunchKernel
        = (CUresult(*)(CUfunction, unsigned int, unsigned int, unsigned int, unsigned int, unsigned int, unsigned int,
            unsigned int, CUstream, void **, void **))GET_FUNCTION(libCudaPointer, stringMacro(cuLaunchKernel));
    if (checkDlError(tracing, !jitCuLaunchKernel))
        return false;

    jitCuMemcpyDtoH = (CUresult(*)(void *, CUdeviceptr, size_t))GET_FUNCTION(libCudaPointer, stringMacro(cuMemcpyDtoH));
    if (checkDlError(tracing, !jitCuMemcpyDtoH))
        return false;

    jitCuMemcpyDtoHAsync = (CUresult(*)(void *, CUdeviceptr, size_t, CUstream))GET_FUNCTION(libCudaPointer,
        stringMacro(cuMemcpyDtoHAsync));
    if (checkDlError(tracing, !jitCuMemcpyDtoHAsync))
        return false;

    jitCuMemFree = (CUresult(*)(CUdeviceptr))GET_FUNCTION(libCudaPointer, stringMacro(cuMemFree));
    if (checkDlError(tracing, !jitCuMemFree))
        return false;

    jitCuModuleUnload = (CUresult(*)(CUmodule))GET_FUNCTION(libCudaPointer, stringMacro(cuModuleUnload));
    if (checkDlError(tracing, !jitCuModuleUnload))
        return false;

    jitCuMemHostRegister
        = (CUresult(*)(void *, size_t, unsigned int))GET_FUNCTION(libCudaPointer, stringMacro(cuMemHostRegister));
    if (checkDlError(tracing, !jitCuMemHostRegister))
        return false;

    jitCuMemHostUnregister = (CUresult(*)(void *))GET_FUNCTION(libCudaPointer, stringMacro(cuMemHostUnregister));
    if (checkDlError(tracing, !jitCuMemHostUnregister))
        return false;

    jitCuGetErrorString
        = (CUresult(*)(CUresult, const char **))GET_FUNCTION(libCudaPointer, stringMacro(cuGetErrorString));
    if (checkDlError(tracing, !jitCuGetErrorString))
        return false;
...
static bool loadNVMLLibrary(int tracing)
{
    ...
    void *libNvmlPointer;
    bool found = false;
    char *dirnames[DIRNAMES_LEN] = { "", "/usr/lib64/", "/usr/lib64/nvidia/", "/usr/lib/", "/usr/lib/nvidia/" };
    for (int i = 0; i < DIRNAMES_LEN; i++) {
        ...
        if (!checkDlError(tracing, !libNvmlPointer)) {
            found = true;
            if (tracing > 1)
                TR_VerboseLog::writeLine(TR_Vlog_GPU, "NVML path to load %s", path);
            break;
        }
    }
    ...
    if (checkDlError(tracing, !jitNvmlInit))
        return false;

    jitNvmlDeviceGetHandleByIndex = (nvmlReturn_t(*)(unsigned int, nvmlDevice_t *))GET_FUNCTION(libNvmlPointer,
        stringMacro(nvmlDeviceGetHandleByIndex));
    if (checkDlError(tracing, !jitNvmlDeviceGetHandleByIndex))
        return false;

    jitNvmlDeviceGetUtilizationRates = (nvmlReturn_t(*)(nvmlDevice_t, nvmlUtilization_t *))GET_FUNCTION(libNvmlPointer,
        stringMacro(nvmlDeviceGetUtilizationRates));
    if (checkDlError(tracing, !jitNvmlDeviceGetUtilizationRates))
        return false;

    jitNvmlDeviceGetMemoryInfo = (nvmlReturn_t(*)(nvmlDevice_t, nvmlMemory_t *))GET_FUNCTION(libNvmlPointer,
        stringMacro(nvmlDeviceGetMemoryInfo));
    if (checkDlError(tracing, !jitNvmlDeviceGetMemoryInfo))
        return false;

    jitNvmlErrorString = (char *(*)(nvmlReturn_t))GET_FUNCTION(libNvmlPointer, stringMacro(nvmlErrorString));
    if (checkDlError(tracing, !jitNvmlErrorString))
        return false;
    ...
    if (result == NVML_SUCCESS) {
        ...
        if (tracing > 0)
            TR_VerboseLog::writeLine(TR_Vlog_GPU, "Failed to initialize NVML: %s", jitNvmlErrorString(result));
        return false;
    }
}
...
static int deviceCCs[MAX_DEVICE_NUM] = { UNKNOWN_CC_MAJORMINOR, UNKNOWN_CC_MAJORMINOR, UNKNOWN_CC_MAJORMINOR,
    UNKNOWN_CC_MAJORMINOR, UNKNOWN_CC_MAJORMINOR, UNKNOWN_CC_MAJORMINOR, UNKNOWN_CC_MAJORMINOR, UNKNOWN_CC_MAJORMINOR };
...
int getGpuDeviceCount(TR::PersistentInfo *persistentInfo, int tracing)
{
    ...
    // If everything has been initialized, we should already have the device count (or 0 for the failing case) so we
    // just return it
    ...
    if (result != CUDA_SUCCESS) {
        ...
        persistentInfo->getGpuInitializationMonitor()->exit(); // release the lock
        if (tracing > 0)
            TR_VerboseLog::writeLine(TR_Vlog_GPU, "FAILED with CUDA error : CUresult = %d", result);
        return 0; // failed
    }
    ...
    if (cudaError != cudaSuccess) {
        ...
        persistentInfo->getGpuInitializationMonitor()->exit(); // release the lock
        if (tracing > 0)
            TR_VerboseLog::writeLine(TR_Vlog_GPU, "FAILED with CUDA error : cudaError_t = %d", cudaError);
        return 0; // failed
    }

    int deviceLoopLimit = nDevices > MAX_DEVICE_NUM ? MAX_DEVICE_NUM : nDevices;
    for (int deviceId = 0; deviceId < deviceLoopLimit; deviceId++) {
        cudaDeviceProp prop[3]; // workaround to avoid memory collapsing since recent CUDA version requires large size
                                // for cudeDeviceProp (by Keith)
        ...
        if (tracing) {
            TR_VerboseLog::writeLine(TR_Vlog_GPU, "\tDevice Number %2d: name=%s, ComputeCapability=%d.%d", deviceId,
                prop[0].name, major, minor);
            fflush(NULL);
        }
        ...
        if (deviceCCs[deviceId] != deviceCCs[0]) {
            if (tracing > 1)
                TR_VerboseLog::writeLine(TR_Vlog_GPU,
                    "GPU compute capability mismatch detected. Disabling multi GPU use");
            enableMultiGPU
                = false; // TODO: multiple devices not currently supported if devices have varying compute capabilities
        }
    }
...
int getCUmoduleSize() { return sizeof(CUmodule); }
...
bool captureCudaError(cudaError_t result, CudaInfo *cudaInfo, char *functionName)
{
    ...
    if (!cudaInfo->sessionFailed && result != cudaSuccess) { // new error, capture it
...
bool captureCuError(CUresult result, CudaInfo *cudaInfo, char *functionName, bool print = true)
{
    ...
    if (!cudaInfo->sessionFailed && result != CUDA_SUCCESS) { // new error, capture it
...
bool captureJITError(char *errorText, CudaInfo *cudaInfo, bool print = true)
{
    ...
    if (!cudaInfo->sessionFailed) { // new error, capture it
        cudaInfo->sessionFailed = true;
...
static HashEntry *cudaInfoHashPut(CudaInfo *cudaInfo, void **hostref, int32_t elementSize, CUdeviceptr deviceArray,
    uint32_t accessMode)
{
    ...
    if (i == cudaInfo->hashSize) {
        ...
        HashEntry *hashTable
            = (HashEntry *)TR_Memory::jitPersistentAlloc(hashSize * sizeof(HashEntry), TR_Memory::GPUHelpers);

        if (!hashTable) {
            if (detailsTrace)
                TR_VerboseLog::writeLine(TR_Vlog_GPU,
                    "FAILED to extend persistent memory for cudaInfo hashTable to size %d bytes.",
                    hashSize * sizeof(HashEntry));
            returnOnTrue(captureJITError("cudaInfoHashPut - out of JIT persistent memory", cudaInfo, detailsTrace),
                NULL);
        }
...
static CUdeviceptr cudaInfoHashRemove(CudaInfo *cudaInfo, void **hostref, int32_t elementSize)
{
    for (int32_t i = 0; i < cudaInfo->hashSize; i++) {
        if (cudaInfo->hashTable[i].hostRef == 0)
            break;
...
static int cudaModuleLoadData(CUlinkState *lState, CudaInfo *cudaInfo, const char *ptxSource, bool writeCUBIN,
    bool writeLINKER)
{
    ...
    returnOnTrue(captureCuError(jitCuLinkCreate(OPTIONSNUM, optionCmds, optionVals, lState), cudaInfo,
                     "cudaModuleLoadData - jitCuLinkCreate"),
        ERROR_CODE);
    returnOnTrue(captureCuError(
                     jitCuLinkAddData(*lState, CU_JIT_INPUT_PTX, (void *)ptxSource, strlen(ptxSource) + 1, 0, 0, 0, 0),
                     cudaInfo, "cudaModuleLoadData - jitCuLinkAddData"),
        ERROR_CODE);
    returnOnTrue(captureCuError(jitCuLinkComplete(*lState, &cubin, &cubinSize), cudaInfo,
                     "cudaModuleLoadData - jitCuLinkComplete"),
        ERROR_CODE);
    ...
    returnOnTrue(captureCuError(jitCuModuleLoadData(&(cudaInfo->module), cubin), cudaInfo,
                     "cudaModuleLoadData - jitCuModuleLoadData"),
        ERROR_CODE);
...
void *initGPUThread(J9VMThread *vmThread, TR::PersistentInfo *persistentInfo, bool isForce)
{
    ...
    int tracing = 0;
    if (TR::Options::getCmdLineOptions()->getEnableGPU(TR_EnableGPUVerbose))
        tracing = 1;
    if (TR::Options::getCmdLineOptions()->getEnableGPU(TR_EnableGPUDetails))
        tracing = 2;
    ...
    if (!threadGPUInfo) {
        if (tracing)
            TR_VerboseLog::writeLine(TR_Vlog_GPU, "FAILED to allocate persistent memory for threadGPUInfo object.");
...
static CudaInfo *initCUDA(J9VMThread *vmThread, int tracing, const char *ptxSource, int deviceId, int ptxSourceID,
    GpuMetaData *gpuMetaData)
{
    ...
    if (!cudaInfo) {
        if (trace)
            TR_VerboseLog::writeLine(TR_Vlog_GPU, "\tFAILED to allocate persistent memory for CudaInfo object.");
        return 0;
    }
    ...
    cudaInfo->hashTable
        = (HashEntry *)TR_Memory::jitPersistentAlloc(cudaInfo->hashSize * sizeof(HashEntry), TR_Memory::GPUHelpers);
    if (!cudaInfo->hashTable) {
        TR_Memory::jitPersistentFree(cudaInfo);
        if (trace)
            TR_VerboseLog::writeLine(TR_Vlog_GPU, "\tFAILED to allocate persistent memory for cudaInfo hashTable.");
        return 0;
    }
    ...
    if (trace)
        TR_VerboseLog::writeLine(TR_Vlog_GPU, "\tcudaSetDevice Time: %.3f msec.", (deviceSetTime - startTime) / 1000);
    if (trace)
        TR_VerboseLog::writeLine(TR_Vlog_GPU, "\tcontext setup Time: %.3f msec.",
            (contextSetupTime - deviceSetTime) / 1000);
...
static bool loadKernel(CudaInfo *cudaInfo, GpuMetaData *gpuMetaData, int kernelId, int deviceId)
{
    ...
    if (!validStoredModule || disableModuleCaching) {
        if (writeCUBIN || writeLINKER)
            ...
            returnOnTrue(captureCuError(jitCuModuleLoadDataEx(&cudaInfo->module, ptxSource, 0, 0, 0), cudaInfo,
                             "loadKernel - jitCuModuleLoadDataEx"),
                false);
    ...
    returnOnTrue(captureCuError(jitCuModuleGetFunction(&cudaInfo->kernel, cudaInfo->module, functionName), cudaInfo,
                     "loadKernel - jitCuModuleGetFunction"),
        false);

    if ((!validStoredModule || disableModuleCaching) && (writeCUBIN || writeLINKER)) {
        // Destroy the linker invocation
        if (lState)
            returnOnTrue(captureCuError(jitCuLinkDestroy(lState), cudaInfo, "loadKernel - jitCuLinkDestroy"),
                false); // TODO: need to free cudaInfo on error
    }
...
static bool addlibDeviceToProgram(nvvmProgram program, int computeArch, bool trace, TR_Memory *m)
{
    ...
    if (!m) {
        if (trace)
            TR_VerboseLog::writeLine(TR_Vlog_GPU, "\tTR_Memory is null. Can't allocate memory for libDevice");
        return false;
    }
    ...
    if (libNVVMHOME == NULL) {
        if (trace)
            TR_VerboseLog::writeLine(TR_Vlog_GPU, "\tPath to libNVVM is UNKNOWN.");
        return false;
    }
    ...
    if (libDevicePath == NULL) {
        if (trace)
            TR_VerboseLog::writeLine(TR_Vlog_GPU, "\tFAILED to allocate memory for libDevice path.");
        return false;
    }
    libDevicePath = strcat(strcpy(libDevicePath, libNVVMHOME), libdeviceByteCode);

    if (trace)
        TR_VerboseLog::writeLine(TR_Vlog_GPU, "\tAdding libDevice to NVVM program : %s", libDevicePath);
    ...
    if (f == NULL) {
        if (trace)
            TR_VerboseLog::writeLine(TR_Vlog_GPU, "\tFAILED to open libDevice file.");
        return false;
    }
    ...
    if (buffer == NULL) {
        if (trace)
            TR_VerboseLog::writeLine(TR_Vlog_GPU, "\tFAILED to allocate memory for libDevice file.");
        fclose(f);
        return false;
    }
    ...
    if (ferror(f)) {
        if (trace)
            TR_VerboseLog::writeLine(TR_Vlog_GPU, "\tFAILED to allocate memory for a buffer for libDevice.");
        fclose(f);
        return false;
    }
    fclose(f);
    ...
    if (jitNvvmAddModuleToProgram(program, buffer, size, libDevicePath) != NVVM_SUCCESS) {
        if (trace)
            TR_VerboseLog::writeLine(TR_Vlog_GPU, "\tFAILED to add the module %s to the compilation unit.",
                libDevicePath);
        return false;
    }
...
bool calculateComputeCapability(int tracing, short *computeMajor, short *computeMinor, int deviceId)
{
    ...
    if ((major != UNKNOWN_CC) && (minor != UNKNOWN_CC)) {
        ...
        } else // Compute Capability range: 0.0-1.9
        {
            if (detailsTrace)
                TR_VerboseLog::writeLine(TR_Vlog_GPU,
                    "\tCompute Capability %d.%d is unsupported. Compute Capability must be 2.0 or higher", major,
                    minor);
            return false;
        }
    } else // Compute Capability unknown
    {
        if (detailsTrace)
            TR_VerboseLog::writeLine(TR_Vlog_GPU,
                "\tUnknown Compute Capability. Compute Capability must be 2.0 or higher", major, minor);
        return false;
    }
    ...
    TR_Memory *m, bool enableMath)
{
    ...
    // Currently only device ids in the range of [0,MAX_DEVICE_NUM-1] are supported. Compilation fails for ids above
    // MAX_DEVICE_NUM-1.
    if (deviceId >= MAX_DEVICE_NUM) {
        if (detailsTrace)
            TR_VerboseLog::writeLine(TR_Vlog_GPU,
                "\tgeneratePTX failed. Device ID is %d. Device IDs greater than or equal to %d are not supported.",
                deviceId, MAX_DEVICE_NUM);
        return NULL;
    }
    ...
    if (deviceId >= threadGPUInfo->nDevices) {
        if (detailsTrace)
            TR_VerboseLog::writeLine(TR_Vlog_GPU,
                "\tDevice ID is %d which is greater than or equal to the total number of devices: %d", deviceId,
                threadGPUInfo->nDevices);
        return NULL;
    }

    if (!calculateComputeCapability(tracing, &computeMajor, &computeMinor, deviceId)) {
        if (detailsTrace)
            TR_VerboseLog::writeLine(TR_Vlog_GPU, "\tgeneratePTX failed. calculateComputeCapability was unsuccessful.");
        return NULL;
    }

    if (detailsTrace)
        TR_VerboseLog::writeLine(TR_Vlog_GPU, "\tCreating NVVM program");
    checkNVVMError(jitNvvmCreateProgram(&program), tracing);

    if (detailsTrace)
        TR_VerboseLog::writeLine(TR_Vlog_GPU, "\tAdding NVVM module size=%d", strlen(programSource));
    checkNVVMError(jitNvvmAddModuleToProgram(program, programSource, strlen(programSource), "jittedcode"), tracing);
    if (detailsTrace)
        TR_VerboseLog::writeLine(TR_Vlog_GPU, "\tAdded NVVM module size=%d", strlen(programSource));

#define OPTIONLENGTH 6
#define OPTIONMAXSIZE 28

    char optionStr[OPTIONLENGTH][OPTIONMAXSIZE]
        = { "-opt=0", "-ftz=1", "-prec-sqrt=1", "-prec-div=1", "-fma=0", "-arch=compute_MMMMmmmm" };
    ...
    if (detailsTrace)
        TR_VerboseLog::writeLine(TR_Vlog_GPU, "\tNVVM Version: %d.%d", nvvmVersionMajor, nvvmVersionMinor);
    ...
    if (enableMath) {
        ...
        if (!addlibDeviceToProgram(program, computeMajor * 10 + computeMinor, detailsTrace, m)) {
            if (detailsTrace)
                TR_VerboseLog::writeLine(TR_Vlog_GPU, "\tFAILED to add libDevice.");
            return 0;
        }
        end = clock();
        if (detailsTrace)
            TR_VerboseLog::writeLine(TR_Vlog_GPU, "\tAdding libdevice : %6.3f msec",
    ...
    // nvvmCompileProgram changes the signal handler for multiple signals which can cause errors when the wrong signal
    // handler is called. As a temporary solution, the current signal handlers are saved before the call to
    // nvvmCompileProgram. After the call returns, the signal handlers are restored from the saved versions. This is a
    // temporary solution since errors can still occur if a trap occurs between the call to nvvmCompileProgram and
    // restoring the signal handlers. Incorrect signal handler restoration is also possible if the signal handlers are
    // saved during this point. In the future the real problem of nvvmCompileProgram messing with the signal handlers
    // will need to be fixed.
    // TODO: remove signal handler saving and restoring after we get a version of nvvmCompileProgram that doesn't change
    // the signal handlers.
#if (defined(LINUX) || !defined(TR_TARGET_X86))
    ...
    if (result != NVVM_SUCCESS) {
        ...
        if (log) {
            ...
            if (detailsTrace)
                TR_VerboseLog::writeLine(TR_Vlog_GPU, "%s", log);
        }
        return 0;
    }
...
static TR::CodeGenerator::GPUResult launchKernel(int tracing, CudaInfo *cudaInfo, int gridDimX, int gridDimY,
    int gridDimZ, int blockDimX, int blockDimY, int blockDimZ, int argCount, bool needExtraArg, void **args,
    int kernelID, int hasExceptionChecks)
{
    ...
    if (hasExceptionChecks) {
        if (cudaInfo->exceptionPointer == NULL_DEVICE_PTR) {
            returnOnTrue(captureCuError(jitCuMemAlloc(&(cudaInfo->exceptionPointer), sizeof(int32_t)), cudaInfo,
                             "launchKernel - jitCuMemAlloc"),
                TR::CodeGenerator::GPUHelperError);

            if (tracing > 1)
                TR_VerboseLog::writeLine(TR_Vlog_GPU, "\tallocated %d bytes at %p for GPU exceptions (%p)",
                    sizeof(int32_t), cudaInfo->exceptionPointer, cudaInfo);
        }

        returnOnTrue(captureCuError(!disableAsyncOps
                             ? jitCuMemcpyHtoDAsync(cudaInfo->exceptionPointer, &ExceptionKind, sizeof(int32_t), stream)
                             : jitCuMemcpyHtoD(cudaInfo->exceptionPointer, &ExceptionKind, sizeof(int32_t)),
                         cudaInfo, "launchKernel - jitCuMemcpyHtoD"),
            TR::CodeGenerator::GPUHelperError);

        if (tracing > 1)
            TR_VerboseLog::writeLine(TR_Vlog_GPU,
                "\tperforming a copy to GPU %d bytes for GPU exceptions. Device: %p (%p)", sizeof(int32_t),
                cudaInfo->exceptionPointer, cudaInfo);
    }
    ...
    if (tracing > 1) {
        ...
        TR_VerboseLog::writeLine(TR_Vlog_GPU, "\tgrid: Dim(%d %d %d), Block(%d %d %d)", gridDimX, gridDimY, gridDimZ,
    ...
    returnOnTrue(captureCuError(jitCuLaunchKernel(cudaInfo->kernel, gridDimX, gridDimY, gridDimZ, blockDimX, blockDimY,
                                    blockDimZ, 0, stream, _args, NULL),
    ...
    if (hasExceptionChecks) {
        returnOnTrue(captureCuError(!disableAsyncOps
                             ? jitCuMemcpyDtoHAsync(&ExceptionKind, cudaInfo->exceptionPointer, sizeof(int32_t), stream)
                             : jitCuMemcpyDtoH(&ExceptionKind, cudaInfo->exceptionPointer, sizeof(int32_t)),
                         cudaInfo, "launchKernel - jitCuMemcpyDtoH"),
            TR::CodeGenerator::GPUHelperError);

        if (tracing > 1)
            TR_VerboseLog::writeLine(TR_Vlog_GPU, "\tCopy out %d bytes for exceptions. Device: %p (%p)",
                sizeof(int32_t), cudaInfo->exceptionPointer, cudaInfo);

        if (disableAsyncOps) {
            returnOnTrue(
                captureCudaError(jitCudaDeviceSynchronize(), cudaInfo, "launchKernel - jitCudaDeviceSynchronize"),
                TR::CodeGenerator::GPUHelperError);
        } else {
            returnOnTrue(
                captureCudaError(jitCudaStreamSynchronize(stream), cudaInfo, "launchKernel - jitCudaStreamSynchronize"),
                TR::CodeGenerator::GPUHelperError);
        }
    }
...
extern "C" int estimateGPU(CudaInfo *cudaInfo, int ptxSourceID, uint8_t *startPC, int lambdaCost, int dataSize,
    int startInclusive, int endExclusive)
{
    ...
    if (trace > 1) {
        TR_VerboseLog::writeLine(TR_Vlog_GPU,
            "\tforEach in %s at line %d has lambdaCost %d dataCost %d range %lld ratio %d", methodSignature, lineNumber,
            lambdaCost, dataCost, range, dataCost / range);
    }

    bool directToCPU = ((cudaInfo->availableMemory < (unsigned long long)dataSize) || lambdaCost == 0
        || dataCost / range > 80 || range < 1024);

    if (directToCPU) {
        if (trace > 0)
            TR_VerboseLog::writeLine(TR_Vlog_GPU,
                "[time.ms=%lld]: Sent forEach in %s at line %d to CPU based on performance heuristics",
                (int64_t)(currentTime() / 1000), methodSignature, lineNumber);
...
static int selectGPUDevice(J9VMThread *vmThread, int tracing, unsigned long long *availableMemory, bool enforceGPU)
{
    ...
    if (threadGPUInfo == NULL) {
        ...
        if (threadGPUInfo == NULL) {
            if (tracing > 1)
                TR_VerboseLog::writeLine(TR_Vlog_GPU, "[%16p] GPUdevice is not selected due to null threadGPUInfo",
                    vmThread);
            return 0;
        }
    }
    ...
    if (tracing > 1)
        TR_VerboseLog::writeLine(TR_Vlog_GPU, "[%16p] #ofDevices=%d", vmThread, deviceCount);
    ...
    for (uint32_t i = 0; i < deviceCount; i++) {
        ...
        if (NVML_SUCCESS != result) {
            if (tracing > 1)
                TR_VerboseLog::writeLine(TR_Vlog_GPU, "Failed to get handle for device %d: %s", devId,
                    jitNvmlErrorString(result));
            return 0;
        }
        result = jitNvmlDeviceGetUtilizationRates(device, &utilization);
        if (NVML_SUCCESS != result) {
            if (tracing > 1)
                TR_VerboseLog::writeLine(TR_Vlog_GPU, "Failed to get GPU utilization for device %d: %s", devId,
                    jitNvmlErrorString(result));
            return 0;
        }
        ...
        if (NVML_SUCCESS != result) {
            if (tracing > 1)
                TR_VerboseLog::writeLine(TR_Vlog_GPU, "Failed to get memory information for device %d: %s", devId,
                    jitNvmlErrorString(result));
            return 0;
        }
        if (tracing > 1)
            TR_VerboseLog::writeLine(TR_Vlog_GPU, "[%16p]   device%3d: GPUutilization=%4d, freemem=%lld", vmThread,
                devId, utilization.gpu, memory.free);
    ...
    if (tracing > 1) {
        if (foundDevice) {
            TR_VerboseLog::writeLine(TR_Vlog_GPU, "[%16p] Selected GPUdevice=%3d, GPUutilization=%4d, freemem=%lld",
                vmThread, deviceId, gpuUtilization, *availableMemory);
        } else {
...
static CudaInfo *initGPUData(int trace, int ptxSourceID, uint8_t *startPC, bool enforceGPU)
{
    J9VMThread *vmThread = jitConfig->javaVM->internalVMFunctions->currentVMThread(jitConfig->javaVM);
    GpuMetaData *gpuMetaData = getGPUMetaData(startPC);
    ...
    if (!disableMultipleGPU) {
        ...
        if (deviceId < 0) {
            if (trace > 0) {
                TR_VerboseLog::writeLine(TR_Vlog_GPU,
                    "[time.ms=%lld]: Redirected parallel forEach in %s at line %d to CPU due to high GPU utilization",
                    (int64_t)(currentTime() / 1000), methodSignature, lineNumber);
            }
            return NULL;
        }
    ...
    if (trace > 0 && info == NULL) {
        TR_VerboseLog::writeLine(TR_Vlog_GPU, "[time.ms=%lld]: Redirected parallel forEach in %s at line %d to CPU",
            (int64_t)(currentTime() / 1000), methodSignature, lineNumber);
    }
...
extern "C" CudaInfo *regionEntryGPU(int trace, int ptxSourceID, uint8_t *startPC,
    TR::CodeGenerator::GPUScopeType regionType, bool enforceGPU)
{
    if (trace > 1)
...
static int copyGPUtoHost(CudaInfo *cudaInfo, void **hostRef, CUdeviceptr deviceArray, int32_t elementSize,
    int32_t isNoCopyDtoH, void *startAddress, void *endAddress, bool forceCopy);
...
int flushGPUDatatoCPU(CudaInfo *cudaInfo)
{
    ...
    for (int32_t i = 0; i < cudaInfo->hashSize; i++) {
        if (cudaInfo->hashTable[i].hostRef != 0 && cudaInfo->hashTable[i].hostRef != (void **)DELETED_HOSTREF
            && cudaInfo->hashTable[i].deviceArray != NULL_DEVICE_PTR
            && cudaInfo->hashTable[i].deviceArray != BAD_DEVICE_PTR && cudaInfo->hashTable[i].valid) {
            cudaInfo->hashTable[i].valid = false; // invalidate GPU data

            void *startAddress
                = ((char *)*(cudaInfo->hashTable[i].hostRef)) + cudaInfo->hashTable[i].cpOutStartAddressOffset;
            void *endAddress
                = ((char *)*(cudaInfo->hashTable[i].hostRef)) + cudaInfo->hashTable[i].cpOutEndAddressOffset;

            copyGPUtoHost(cudaInfo, cudaInfo->hashTable[i].hostRef, cudaInfo->hashTable[i].cpOutdevArray,
                cudaInfo->hashTable[i].cpOutElementSize, cudaInfo->hashTable[i].cpOutIsNoCopyDtoH, startAddress,
                endAddress,
                true); // force copy
        }
    }
...
bool freeGPUScope(CudaInfo *cudaInfo)
{
    ...
    for (int32_t i = 0; i < cudaInfo->hashSize; i++) {
        if (cudaInfo->hashTable[i].hostRef != 0 && cudaInfo->hashTable[i].hostRef != (void **)DELETED_HOSTREF
            ...
            && cudaInfo->hashTable[i].deviceArray != BAD_DEVICE_PTR) {
            if (tracing > 1) {
                TR_VerboseLog::writeLine(TR_Vlog_GPU, "\tFree GPU memory. Host: %p, Device: %p (%p)",
                    *(cudaInfo->hashTable[i].hostRef), cudaInfo->hashTable[i].cpOutdevArray, cudaInfo);
            }
            ...
            if (res != CUDA_SUCCESS && tracing > 1) {
                TR_VerboseLog::writeLine(TR_Vlog_GPU, "\terror de-allocating GPU memory. Host: %p, Device: %p (%p)",
                    *(cudaInfo->hashTable[i].hostRef), cudaInfo->hashTable[i].cpOutdevArray, cudaInfo);
            }
        }
    }
    ...
    if (cudaInfo->exceptionPointer != NULL_DEVICE_PTR) {
        if (tracing > 1) {
            TR_VerboseLog::writeLine(TR_Vlog_GPU, "\tFree GPU Exception memory %p (%p)", cudaInfo->exceptionPointer,
                cudaInfo);
        }
        ...
        if (res != CUDA_SUCCESS && tracing > 1) {
            TR_VerboseLog::writeLine(TR_Vlog_GPU, "\terror de-allocating GPU Exception memory %p (%p)",
                cudaInfo->exceptionPointer, cudaInfo);
        }
    }
...
extern "C" int regionExitGPU(CudaInfo *cudaInfo, uint8_t *startPC, int ptxSourceID, void **liveRef)
{
    returnOnTrue(!cudaInfo, 1);
...
extern "C" CUdeviceptr copyToGPU(CudaInfo *cudaInfo, void **hostRef, int32_t elementSize, int32_t accessMode,
    int32_t isNoCopyHtoD, int32_t isNoCopyDtoH, void *startAddressHtoD, void *endAddressHtoD, void *startAddressDtoH,
    void *endAddressDtoH)
{
    ...
    if (startAddressHtoD == 0) {
        ...
        numPinnedBytes
            = (size_t)arrayLength * elementSize + OBJECT_HEADER_SIZE; // TODO: change to partial array in future
    ...
    if (doAllocate) {
        ...
        returnOnTrue(captureCuError(jitCuMemAlloc(&deviceArray, allocNumBytes), cudaInfo, "copyToGPU - jitCuMemAlloc"),
            BAD_DEVICE_PTR);

        if (tracing > 1)
            TR_VerboseLog::writeLine(TR_Vlog_GPU, "\tallocated %d bytes at %p (%p)", allocNumBytes, deviceArray,
                cudaInfo);
        ...
        if (isPinned) {
            ...
            if (result == CUDA_SUCCESS)
                ...
                if (tracing > 0)
                    TR_VerboseLog::writeLine(TR_Vlog_GPU, "FAILED with CUDA error : CUresult = %d (%p)", result,
                        cudaInfo);
        ...
        parm_entry = cudaInfoHashPut(cudaInfo, hostRef, elementSize, deviceArray,
            accessMode); // TODO: hostRef might be invalid here without VM access
        if (parm_entry == NULL) {
            if (!disableAsyncOps)
                jitCudaStreamSynchronize(stream);
            jitCuMemFree(deviceArray);
    ...
    if (doCopy) {
        if (tracing > 1)
            TR_VerboseLog::writeLine(TR_Vlog_GPU, "\tperforming a copy to GPU %d bytes: %p -> %p (%p)", transNumBytes,
                *hostRef, deviceArray, cudaInfo);

        CUresult result;
        if (offset == 0) {
            // copy header + array body
            result = !disableAsyncOps
                ? jitCuMemcpyHtoDAsync(deviceArray + (isalign ? HEADER_PADDING : 0), (char *)*hostRef, transNumBytes,
                      stream)
                : jitCuMemcpyHtoD(deviceArray + (isalign ? HEADER_PADDING : 0), (char *)*hostRef, transNumBytes);
        } else {
            // copy header
            result = !disableAsyncOps
                ? jitCuMemcpyHtoDAsync(deviceArray + (isalign ? HEADER_PADDING : 0), (char *)*hostRef,
                      OBJECT_HEADER_SIZE, stream)
                : jitCuMemcpyHtoD(deviceArray + (isalign ? HEADER_PADDING : 0), (char *)*hostRef, OBJECT_HEADER_SIZE);
            // copy array body
            if (result == CUDA_SUCCESS)
                result = !disableAsyncOps
                    ? jitCuMemcpyHtoDAsync(deviceArray + (isalign ? HEADER_PADDING : 0) + OBJECT_HEADER_SIZE + offset,
        ...
        if (tracing > 1)
            TR_VerboseLog::writeLine(TR_Vlog_GPU, "\tNo copy to GPU (%d bytes: %p -> %p (%p))", transNumBytes, *hostRef,
                deviceArray, cudaInfo);
        ...
        if (((accessMode & TR::CodeGenerator::WriteAccess) && (hArrayAccessMode == TR::CodeGenerator::ReadAccess))
            ...
            returnOnTrue(captureJITError("copyToGPU - non-trivial array access", cudaInfo, tracing > 1),
                BAD_DEVICE_PTR); // TODO: better handling helper error conditions
        }
    }
    ...
    int32_t isNoCopyDtoH, void *startAddress, void *endAddress, bool forceCopy)
{
    ...
    if (hostRef == NULL) {
        ...
        returnOnTrue(captureJITError("copyGPUtoHost - host array pointer is NULL", cudaInfo, tracing > 1),
            0); // TODO: better handling helper error conditions
    }
    ...
    if ((*hostRef) == NULL) {
        ...
        returnOnTrue(captureJITError("copyGPUtoHost - host array is NULL", cudaInfo, tracing > 1),
            0); // TODO: better handling helper error conditions
    }
    ...
    bool doCopy = ((!parm_entry->valid || forceCopy) && isNoCopyDtoH == 0) && (deviceArrayInHash != NULL_DEVICE_PTR)
        && (deviceArrayInHash != BAD_DEVICE_PTR); // only when pointers are valid

    if (doCopy) {
        if (tracing > 1)
            TR_VerboseLog::writeLine(TR_Vlog_GPU, "\tCopy out %d bytes: %p -> %p", numBytes,
                deviceArray + offset + (isalign ? HEADER_PADDING : 0), (char *)*hostRef + offset);

        CUresult result = !disableAsyncOps
            ? jitCuMemcpyDtoHAsync((char *)*hostRef + offset, deviceArray + offset + (isalign ? HEADER_PADDING : 0),
                  numBytes, stream)
            : jitCuMemcpyDtoH((char *)*hostRef + offset, deviceArray + offset + (isalign ? HEADER_PADDING : 0),
                  numBytes);
        returnOnTrue(captureCuError(result, cudaInfo, "copyGPUtoHost - jitCuMemcpyDtoH"),
            TR::CodeGenerator::GPUHelperError);

        parm_entry->cpOutIsNoCopyDtoH = 1; // resets flag. array was copied back and does not need to be copied back
                                           // again until this flag is cleared.

        if (isPinned) {
            CUresult result = jitCuMemHostUnregister(*hostRef);
            if (result != CUDA_SUCCESS) {
                if (tracing > 1)
                    TR_VerboseLog::writeLine(TR_Vlog_GPU, "\tFailed to unpin memory : CUresult = %d", result);
            }
        }
    }
    ...
    if (tracing > 1) {
        ...
        if (doCopy && !forceCopy)
            TR_VerboseLog::writeLine(TR_Vlog_GPU,
                "\tcopied %lld bytes from GPU to Java %sarray %p, device ptr %p : %.3f msec.", (int64_t)numBytes,
                isPinned ? "(Pinned) " : "", *hostRef, deviceArray, (currentTime() - time) / 1000);

        if (doCopy && forceCopy)
            TR_VerboseLog::writeLine(TR_Vlog_GPU,
                "\tflushed %lld bytes from GPU to Java %sarray %p, device ptr %p : %.3f msec.", (int64_t)numBytes,
                isPinned ? "(Pinned) " : "", *hostRef, deviceArray, (currentTime() - time) / 1000);

        if (!doCopy && isNoCopyDtoH != 0)
            TR_VerboseLog::writeLine(TR_Vlog_GPU,
                "\tno copy from GPU since Java array %p is not updated, device ptr %p: %.3f msec.", *hostRef,
                deviceArray, (currentTime() - time) / 1000);

        if (!doCopy && parm_entry->valid)
            TR_VerboseLog::writeLine(TR_Vlog_GPU,
                "\tno copy from GPU since host Java array %p is valid, device ptr %p : %.3f msec.", *hostRef,
                deviceArray, (currentTime() - time) / 1000);
    }
...
extern "C" int copyFromGPU(CudaInfo *cudaInfo, void **hostRef, CUdeviceptr deviceArray, int32_t elementSize,
    int32_t isNoCopyDtoH, void *startAddress, void *endAddress)
{
    returnOnTrue(!cudaInfo, 0);
    returnOnTrue(isGPUFailed(cudaInfo), 0);
...
extern "C" CudaInfo *invalidateGPU(CudaInfo *cudaInfo, void **hostRef)
{
    ...
    if (isGPUFailed(cudaInfo)) {
        ...
        if (tracing > 0) {
            ...
            if (cudaInfo->ffdcCudaError != cudaSuccess) {
                ...
                TR_VerboseLog::writeLine(TR_Vlog_GPU,
                    "[time.ms=%lld]: Redirected parallel forEach in %s to CPU due to CUDA Runtime error %d: %s",
                    (int64_t)(currentTime() / 1000), methodSignature, cudaInfo->ffdcCudaError, cudaRuntimeErrorMsg);
                ...
                if (jitCuGetErrorString(cudaInfo->ffdcCuError, &cudaDriverErrorMsg) == CUDA_SUCCESS)
                    TR_VerboseLog::writeLine(TR_Vlog_GPU,
                        "[time.ms=%lld]: Redirected parallel forEach in %s to CPU due to CUDA Driver error %d: %s",
                        (int64_t)(currentTime() / 1000), methodSignature, cudaInfo->ffdcCuError, cudaDriverErrorMsg);
                else
                    TR_VerboseLog::writeLine(TR_Vlog_GPU,
                        "[time.ms=%lld]: Redirected parallel forEach in %s to CPU due to CUDA Driver error %d: "
                        "unrecognized error code",
                        (int64_t)(currentTime() / 1000), methodSignature, cudaInfo->ffdcCuError);
            } else // non-nvidia library failures like running out of persistent memory
            {
                TR_VerboseLog::writeLine(TR_Vlog_GPU,
                    "[time.ms=%lld]: Redirected parallel forEach in %s to CPU due to JIT GPU error",
                    (int64_t)(currentTime() / 1000), methodSignature);
            }
        }
...
extern "C" void **allocateGPUKernelParms(int trace, int argCount)
{
    ...
    void **args
        = (void **)TR_Memory::jitPersistentAlloc(sizeof(void *) * (argCount + NUMEXTRAPARMS), TR_Memory::GPUHelpers);

    if (!args) {
        if (trace > 1)
            TR_VerboseLog::writeLine(TR_Vlog_GPU, "\tFAILED to allocate persistent memory for GPU args array.");
        return 0;
    } else {
        if (trace > 1)
            TR_VerboseLog::writeLine(TR_Vlog_GPU, "\tAllocated %d parms + %d parms for GPU args array", argCount,
                NUMEXTRAPARMS);
    }
...
extern "C" int launchGPUKernel(CudaInfo *cudaInfo, int startInclusive, int endExclusive, int argCount, void **args,
    int kernelId, uint8_t *startPC, int hasExceptionChecks)
{
    if (!cudaInfo) {
        if (args)
    ...
    if (!args) {
        if (tracing > 1)
            TR_VerboseLog::writeLine(TR_Vlog_GPU, "\tFAILED to launch GPU kernel. GPU args array was not initialized.");
        returnOnTrue(captureJITError("launchGPUKernel - GPU args not initialized", cudaInfo, tracing > 1),
            TR::CodeGenerator::GPUHelperError);
    }

    if (tracing > 1) {
        TR_VerboseLog::writeLine(TR_Vlog_GPU, "\tlaunchGPUKernel: start=%d end=%d range=%lld argCount=%d",
            startInclusive, endExclusive, range, argCount);
    ...
    if (!loadKernel(cudaInfo, getGPUMetaData(startPC), kernelId, cudaInfo->device)) {
        returnOnTrue(captureJITError("launchGPUKernel - loadKernel failed", cudaInfo, tracing > 1),
            TR::CodeGenerator::GPUHelperError);
    }
    ...
    if (tracing > 0) {
        ...
        if (tracing > 1)
            TR_VerboseLog::writeLine(TR_Vlog_GPU,
                "[time.ms=%lld]: Launching parallel forEach in %s at line %d on GPU %d",
                (int64_t)(currentTime() / 1000), methodSignature, lineNumber, cudaInfo->device);
        else
            TR_VerboseLog::writeLine(TR_Vlog_GPU, "[time.ms=%lld]: Launching parallel forEach in %s at line %d on GPU",
                (int64_t)(currentTime() / 1000), methodSignature, lineNumber);
    }
    ...
    time = currentTime();
    result = launchKernel(tracing, cudaInfo, gridDimX, gridDimY, gridDimZ, blockDimX, blockDimY, blockDimZ, argCount,
        false, args, kernelId, hasExceptionChecks);
    ...
    if (result != TR::CodeGenerator::GPUSuccess) {
        captureJITError("launchGPUKernel - launchKernel did not return GPUSuccess", cudaInfo,
            tracing > 1); // TODO: better handling helper error conditions
    }
    ...
    if (tracing > 0) {
        ...
        if (tracing > 1)
            TR_VerboseLog::writeLine(TR_Vlog_GPU,
                "[time.ms=%lld]: Finished parallel forEach in %s at line %d on GPU %d", (int64_t)(currentTime() / 1000),
                methodSignature, lineNumber, cudaInfo->device);
        else
            TR_VerboseLog::writeLine(TR_Vlog_GPU, "[time.ms=%lld]: Finished parallel forEach in %s at line %d on GPU",
                (int64_t)(currentTime() / 1000), methodSignature, lineNumber);
    }
...
I_32 jitCallGPU(J9VMThread *vmThread, J9Method *method, char *programSource, jobject invokeObject, int deviceId,
    I_32 gridDimX, I_32 gridDimY, I_32 gridDimZ, I_32 blockDimX, I_32 blockDimY, I_32 blockDimZ, I_32 argCount,
    void **args)
{
    return ERROR_CODE;
}

extern "C" int callGPU(void *vmThread, void *method, char *programSource, void *invokeObject, int deviceId,
    int gridDimX, int gridDimY, int gridDimZ, int blockDimX, int blockDimY, int blockDimZ, int argCount, void **args)
{
    return 0;
}
