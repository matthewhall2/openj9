...
const char *TR_RecompilationModifier::optDetailString() const throw() { return "O^O RECOMPILATION COUNTERS: "; }
...
void TR_LocalRecompilationCounters::removeTrees()
{
    ...
    for (TR::TreeTop *treeTop = comp()->getStartTree(); treeTop; treeTop = treeTop->getNextTreeTop()) {
        ...
        if (node->getOpCodeValue() == TR::istore && node->getSymbolReference() == symRef) {
    ...
    for (TR::TreeTop *treeTop = comp()->getStartTree(); treeTop; treeTop = treeTop->getNextTreeTop()) {
        ...
        if (node->getOpCodeValue() == TR::asynccheck) {
            if (performTransformation(comp(),
                    "%s LOCAL RECOMPILATION COUNTERS: Add recomp counter decrement at async check %p\n", OPT_DETAILS,
                    node)) {
...
void TR_GlobalRecompilationCounters::examineStructure(TR_Structure *s, TR_BitVector &headerBlocks)
{
    ...
    if (blockStructure) {
        ...
        if (headerBlocks.get(block->getNumber())) {
            if (performTransformation(comp(),
                    "%s GLOBAL RECOMPILATION COUNTERS: Add recomp counter decrement at loop header block_%d\n",
                    OPT_DETAILS, block->getNumber())) {
                TR::TreeTop::createIncTree(comp(), block->getEntry()->getNode(), _recompilation->getCounterSymRef(), -1,
                    block->getEntry());
                setHasModifiedTrees(true);
            }
        }
    ...
    for (TR_StructureSubGraphNode *subNode = subNodes.getCurrent(); subNode; subNode = subNodes.getNext()) {
        examineStructure(subNode->getStructure(), headerBlocks);
    }
}

TR_CatchBlockProfiler::TR_CatchBlockProfiler(TR::Compilation *c, TR::Recompilation *r, bool forInitialCompilation)
...
void TR_CatchBlockProfiler::modifyTrees()
{
    ...
    TR_CatchBlockProfileInfo *profileInfo
        = _recompilation->findOrCreateProfileInfo()->findOrCreateCatchBlockProfileInfo(comp());
    ...
    for (TR::TreeTop *tt = firstTree; tt; tt = tt->getNextTreeTop()) {
        ...
        if (((node->getOpCodeValue() == TR::athrow && !node->throwInsertedByOSR())
                || (node->getNumChildren() > 0 && (node->getFirstChild()->getOpCodeValue() == TR::athrow)
                    && (!node->getFirstChild()->throwInsertedByOSR())))) {
            if (performTransformation(comp(),
                    "%s CATCH BLOCK PROFILER: Add profiling trees to track the execution frequency of throw %p\n",
                    OPT_DETAILS, node)) {
                if (!_throwCounterSymRef)
                    _throwCounterSymRef
                        = symRefTab->createKnownStaticDataSymbolRef(&profileInfo->getThrowCounter(), TR::Int32);
                TR::TreeTop *profilingTree
                    = TR::TreeTop::createIncTree(comp(), node, _throwCounterSymRef, 1, tt->getPrevTreeTop());
    ...
    for (TR::Block *b = firstTree->getNode()->getBlock(); b; b = b->getNextBlock())
        if (!b->getExceptionPredecessors().empty() && !b->isOSRCatchBlock()) {
            if (performTransformation(comp(),
                    "%s CATCH BLOCK PROFILER: Add profiling trees to track the execution frequency of catch block_%d\n",
                    OPT_DETAILS, b->getNumber())) {
                if (!_catchCounterSymRef)
                    _catchCounterSymRef
                        = symRefTab->createKnownStaticDataSymbolRef(&profileInfo->getCatchCounter(), TR::Int32);
                TR::TreeTop *profilingTree = TR::TreeTop::createIncTree(comp(), b->getEntry()->getNode(),
                    _catchCounterSymRef, 1, b->getEntry());
                profilingTree->getNode()->setIsProfilingCode();
                setHasModifiedTrees(true);
            }
        }
...
{
    TR_BlockFrequencyInfo *blockFrequencyInfo
        = _recompilation->findOrCreateProfileInfo()->findOrCreateBlockFrequencyInfo(comp());
    ...
    for (TR::TreeTop *treeTop = comp()->getStartTree(); treeTop; treeTop = treeTop->getNextTreeTop()) {
        ...
        if (node->getOpCodeValue() == TR::BBStart) {
            ...
            // counted.
            //
            if ((node->getByteCodeInfo().getCallerIndex() == prevByteCodeInfo.getCallerIndex()
                    && node->getByteCodeInfo().getByteCodeIndex() == prevByteCodeInfo.getByteCodeIndex())) {
            ...
            if (!performTransformation(comp(),
                    "%s BLOCK FREQUENCY PROFILER: Add profiling trees to track the execution frequency of block_%d\n",
            ...
            TR::SymbolReference *symRef = comp()->getSymRefTab()->createKnownStaticDataSymbolRef(
                blockFrequencyInfo->getFrequencyForBlock(node->getBlock()->getNumber()), TR::Int32);
...
void TR_ValueProfiler::modifyTrees()
{
    ...
    for (TR::TreeTop *tt = comp()->getStartTree(); tt; tt = tt->getNextTreeTop()) {
        ...
        if (firstChild && firstChild->getOpCodeValue() == TR::arraycopy && !getInitialCompilation()
            && !debug("disableProfileArrayCopy")) {
            TR::Node *arrayCopyLen = firstChild->getChild(firstChild->getNumChildren() - 1);
            if (!arrayCopyLen->getOpCode().isLoadConst() && !arrayCopyLen->getByteCodeInfo().doNotProfile()
                && !(arrayCopyLen->getOpCode().isCallIndirect()
                    && !arrayCopyLen->isTheVirtualCallNodeForAGuardedInlinedCall())) {
                addProfilingTrees(arrayCopyLen, tt, firstChild->getByteCodeInfo(), 0, LastValueInfo, LastProfiler,
                    true);
            }
        } else if (firstChild && firstChild->getOpCode().isCall() && firstChild->getVisitCount() != visitCount
            ...
            if (firstChild->isTheVirtualCallNodeForAGuardedInlinedCall()) {
                ...
                if (!guard
                    || (guard->getLastRealTreeTop()->getNode()->isProfiledGuard()
                        || !guard->getLastRealTreeTop()->getNode()->isNopableInlineGuard()))
                    doit = true;
            }

            if (doit
                && firstChild->getOpCode().isCallIndirect()
                //&& !firstChild->getFirstChild()->getByteCodeInfo().doNotProfile() // We should profile this situation
                // even if the bytecode has doNotProfile on it since we could have a node
                // that was cloned by loop versioner (or any other opt that clones node) that we still want to profile.
                // The usual risk with profiling a value that is different than the intended one because of arbitrary
                // sharing of bytecode info between a node created by IL gen originally and newly created
                //  nodes that have nothing to do with any value seen in the original problem is not an issue here I
                //  feel
                // mostly because I do not see how we can have such a value appear as the vft child of a virtual call
                // anyway The vft child should share the bc info with the virtual call and no opt should be able to
                // stick in a different expression at that precise spot in the IL trees

                && !firstChild->getSymbol()
                        ->castToMethodSymbol()
                        ->isComputed()) // 182550: We currently don't support profiling computed call targets because we
                                        // have no way to map a target address to a MethodHandle object or to inline a
                                        // thunk body
            {
                ...
                if (methodSymbol->isInterface()) {
                    ...
                    TR_OpaqueClassBlock *thisClass
                        = comp()->fe()->getClassFromSignature(s, len, methodSymRef->getOwningMethod(comp()));
                    if (thisClass) {
                        TR_PersistentCHTable *chTable = comp()->getPersistentInfo()->getPersistentCHTable();
                        picMiss = chTable->isKnownToHaveMoreThanTwoInterfaceImplementers(thisClass, cpIndex,
                ...
                if (firstChild->isTheVirtualCallNodeForAGuardedInlinedCall()) {
                    ...
                    if (guard && (guard->getSuccessors().size() == 2) && guard->getExit()
                        ...
                        if ((callChild->getOpCode().isLoadVarDirect()
                                && callChild->getSymbolReference()->getSymbol()->isAutoOrParm())
                            || (callChild->getOpCode().hasSymbolReference()
                                && (callChild->getSymbolReference() == comp()->getSymRefTab()->findVftSymbolRef()))) {
                            ...
                            while (cursorTree != tt) {
                                ...
                                if (cursorStoreNode && (cursorStoreNode->getSymbolReference() == callChildSymRef)) {
                            ...
                            if (correctToClone && !callChild->isUnsafeToDuplicateAndExecuteAgain(&visitBudget)
                                && performTransformation(comp(),
                                    "%s CALL PROFILE VERIFICATION: Duplicating node %p so it can be profiled near %p\n",
            ...
            if (comp()->cg()->getSupportsBigDecimalLongLookasideVersioning() && !methodSymRef->isUnresolved()
                && !methodSymbol->isHelper() /* && !firstChild->getByteCodeInfo().doNotProfile() */) {
        ...
        } else if ((node->getOpCodeValue() == TR::ificmpne) || (node->getOpCodeValue() == TR::ificmpeq)) {
            ...
            if ((firstChild->getOpCodeValue() == TR::iloadi) && (secondChild->getOpCodeValue() == TR::iconst)
...
bool TR_ValueProfiler::validConfiguration(TR::DataType dataType, TR_ValueInfoKind kind)
{
...
void TR_ValueProfiler::addProfilingTrees(TR::Node *node, TR::TreeTop *cursorTree, size_t numExpandedValues,
    TR_ValueInfoKind kind, TR_ValueInfoSource source, bool commonNode, bool decrementRecompilationCounter)
{
    addProfilingTrees(node, cursorTree, node->getByteCodeInfo(), numExpandedValues, kind, source, commonNode,
        decrementRecompilationCounter);
}
 ...
 * \param source Optional implementation to use, should be List, Array or HashTable. Defaults to ValueProfiler's
 * configured default. \param commonNode Optional argument to prevent the profiling trees from referencing the value
 * node directly. \param decrementRecompilationCounter Decrement the recompilation counter when a value is profiled. Not
 * supported by all implementations.
 */
void TR_ValueProfiler::addProfilingTrees(TR::Node *node, TR::TreeTop *cursorTree, TR_ByteCodeInfo &bci,
    size_t numExpandedValues, TR_ValueInfoKind kind, TR_ValueInfoSource source, bool commonNode,
    bool decrementRecompilationCounter)
{
    ...
    if (source == LinkedListProfiler || source == ArrayProfiler)
        addListOrArrayProfilingTrees(node, cursorTree, bci, numExpandedValues, kind, source, commonNode,
            decrementRecompilationCounter);
    else if (source == HashTableProfiler)
        addHashTableProfilingTrees(node, cursorTree, bci, kind, source, commonNode);
}

void TR_ValueProfiler::addHashTableProfilingTrees(TR::Node *node, TR::TreeTop *cursorTree, TR_ByteCodeInfo &bci,
    TR_ValueInfoKind kind, TR_ValueInfoSource source, bool commonNode)
{
    if (comp()->getOption(TR_DisableValueProfiling)
        || !performTransformation(comp(),
            "%s VALUE PROFILER: Add JProfiling trees to track the value of node %p near tree %p, commonNode %d\n",
            OPT_DETAILS, node, cursorTree->getNode(), commonNode))
        return;

    TR_ValueProfileInfo *valueProfileInfo
        = _recompilation->findOrCreateProfileInfo()->findOrCreateValueProfileInfo(comp());
...
void TR_ValueProfiler::addListOrArrayProfilingTrees(TR::Node *node, TR::TreeTop *cursorTree, TR_ByteCodeInfo &bci,
    size_t numExpandedValues, TR_ValueInfoKind kind, TR_ValueInfoSource source, bool commonNode,
    bool decrementRecompilationCounter)
{
    ...
    if (kind == BigDecimalInfo) {
        ...
        if (scaleOffset == -1) {
            ...
            fieldNameLen = 11;
            scaleOffset
                = comp()->fej9()->getInstanceFieldOffset(bdClass, fieldName, fieldNameLen, fieldSig, fieldSigLen);
        }
        ...
        scaleOffset
            += (int32_t)comp()->fe()->getObjectHeaderSizeInBytes(); // size of a J9 object header to move past it
    }
    ...
    if (kind == StringInfo) {
        ...
        lengthOffset
            = comp()->fej9()->getInstanceFieldOffset(stringClass, fieldName, fieldNameLen, fieldSig, fieldSigLen);
        fieldName = "value";
        fieldSig = "[C";
        fieldSigLen = 2;
        charsOffset
    ...
    if (!validBigDecimalFieldOffset || !validStringFieldOffset || comp()->getOption(TR_DisableValueProfiling)
        || !performTransformation(comp(),
            "%s VALUE PROFILER: Add profiling trees to track the value of node %p near tree %p, commonNode %d, "
            "decrementRecompilationCounter %d, up to %d distinct values will be tracked \n",
...
TR_AbstractInfo *TR_ValueProfileInfoManager::getProfiledValueInfo(TR::Node *node, TR::Compilation *comp,
...
TR_AbstractInfo *TR_ValueProfileInfoManager::getProfiledValueInfo(TR_ByteCodeInfo &bci, TR::Compilation *comp,
    TR_ValueInfoKind kind, uint32_t source)
{
    TR_ValueProfileInfoManager *manager = TR_ValueProfileInfoManager::get(comp);
...
TR_AbstractInfo *TR_ValueProfileInfoManager::getValueInfo(TR::Node *node, TR::Compilation *comp, TR_ValueInfoKind kind,
    uint32_t source)
{
...
float TR_ValueProfileInfoManager::getAdjustedInliningWeight(TR::Node *callNode, int32_t weight, TR::Compilation *comp)
{
    return (float)weight;
}

bool TR_ValueProfileInfoManager::isCallGraphProfilingEnabled(TR::Compilation *comp)
{
...
float TR_BranchProfileInfoManager::getCallFactor(int32_t callSiteIndex, TR::Compilation *comp)
{
...
void TR_BranchProfileInfoManager::getBranchCounters(TR::Node *node, TR::TreeTop *treeTop, int32_t *branchToCount,
    int32_t *fallThroughCount, TR::Compilation *comp)
{
    if (_iProfiler == NULL) {
        *branchToCount = 0;
        *fallThroughCount = 0;
    ...
    TR_MethodBranchProfileInfo *mbpInfo
        = TR_MethodBranchProfileInfo::getMethodBranchProfileInfo(node->getInlinedSiteIndex(), comp);
    ...
    if (mbpInfo && node->getInlinedSiteIndex() >= 0) {
        ...
        *fallThroughCount = (int32_t)((float)(*fallThroughCount) * callFactor);
        ...
        if (*branchToCount >= comp->getFlowGraph()->_max_edge_freq
        ...
        if (((*branchToCount + breakEven) >= 0) && (*branchToCount == *fallThroughCount))
            *branchToCount += breakEven;
    } else {
        _iProfiler->getBranchCounters(node, treeTop, branchToCount, fallThroughCount, comp);
    }
}

TR_MethodBranchProfileInfo *TR_MethodBranchProfileInfo::addMethodBranchProfileInfo(uint32_t callSiteIndex,
    TR::Compilation *comp)
{
    ...
    TR_MethodBranchProfileInfo *methodBPInfo
        = new (comp->trHeapMemory()) TR_MethodBranchProfileInfo(callSiteIndex, comp);
    comp->getMethodBranchInfos().push_front(methodBPInfo);

    return methodBPInfo;
}

TR_MethodBranchProfileInfo *TR_MethodBranchProfileInfo::getMethodBranchProfileInfo(uint32_t callSiteIndex,
    TR::Compilation *comp)
{
    TR::list<TR_MethodBranchProfileInfo *> &infos = comp->getMethodBranchInfos();
...
void TR_MethodBranchProfileInfo::resetMethodBranchProfileInfos(int32_t oldMaxFrequency, int32_t oldMaxEdgeFrequency,
    TR::Compilation *comp)
{
    TR::list<TR_MethodBranchProfileInfo *> &infos = comp->getMethodBranchInfos();
 ...
 */
TR_ExternalValueProfileInfo *TR_ExternalValueProfileInfo::addInfo(TR_OpaqueMethodBlock *method,
    TR_ExternalProfiler *profiler, TR::Compilation *comp)
{
    TR_ExternalValueProfileInfo *methodVPInfo
        = new (comp->trHeapMemory()) TR_ExternalValueProfileInfo(method, profiler);
 ...
 */
TR_ExternalValueProfileInfo *TR_ExternalValueProfileInfo::getInfo(TR_OpaqueMethodBlock *method, TR::Compilation *comp)
{
 ...
 */
TR_AbstractInfo *TR_ExternalValueProfileInfo::getValueInfo(TR_ByteCodeInfo &bcInfo, TR::Compilation *comp)
{
 ...
 */
TR_AbstractInfo *TR_ExternalValueProfileInfo::createAddressInfo(TR_ByteCodeInfo &bcInfo, TR::Compilation *comp,
    uintptr_t initialValue, uint32_t initialFreq, TR_LinkedListProfilerInfo<ProfileAddressType> **list)
{
    TR::Region &region = comp->trMemory()->heapMemoryRegion();

    // Create the linked list profiler
    auto profiler = new (region)
        TR_LinkedListProfilerInfo<ProfileAddressType>(bcInfo, AddressInfo, initialValue, initialFreq, true);
    if (list)
        *list = profiler;
 ...
 */
TR_ValueProfileInfo::TR_ValueProfileInfo(TR_CallSiteInfo *info)
    : _callSiteInfo(info)
{
...
/**
 * Extract persistent value profiling information and convert it to the common profiling information API. A region can
 * be specified, otherwise, it will be allocated in the current stack region.
 *
...
/**
 * Extract persistent value profiling information. It will attempt to exactly match kind and source, falling back on a
 * fuzzy match for BCI if an exact cannot be found.
 *
 * \param bcInfo Bytecode info to find profiling information for.
 * \param comp Current compilation.
 * \param kind Kind of profiling information to find.
 ...
 * \return The matched profiling information or NULL if nothing was present.
 */
TR_AbstractProfilerInfo *TR_ValueProfileInfo::getProfilerInfo(TR_ByteCodeInfo &bcInfo, TR::Compilation *comp,
    TR_ValueInfoKind kind, TR_ValueInfoSource source, bool fuzz)
{
    ...
    if (fuzz) {
        ...
        for (TR_AbstractProfilerInfo *valueInfo = _values[source]; valueInfo; valueInfo = valueInfo->getNext()) {
            if (valueInfo->getKind() == kind
                && (matchCount = _callSiteInfo->hasSamePartialBytecodeInfo(valueInfo->getByteCodeInfo(), bcInfo, comp))
                    > maxMatchedCount) {
...
/**
 * Generate new persistent value profiling information for the provided arguments. Will be allocated in persistent
 * memory and added to the corresponding list.
 *
 ...
 */
TR_AbstractProfilerInfo *TR_ValueProfileInfo::createAndInitializeProfilerInfo(TR_ByteCodeInfo &bcInfo,
    TR::Compilation *comp, TR_ValueInfoKind kind, TR_ValueInfoSource source, uint64_t initialValue)
{
    ...
    switch (kind) {
        case ValueInfo:
            if (source == HashTableProfiler)
                ...
                valueInfo = new (comp->trPersistentMemory())
                    TR_LinkedListProfilerInfo<uint32_t>(bcInfo, kind, initialValue, initialFreq);
            break;

        case AddressInfo:
            if (source == HashTableProfiler)
                valueInfo
                    = new (comp->trPersistentMemory()) TR_EmbeddedHashTable<ProfileAddressType, 2>(bcInfo, hash, kind);
            else if (source == LinkedListProfiler)
                valueInfo = new (comp->trPersistentMemory())
                    TR_LinkedListProfilerInfo<ProfileAddressType>(bcInfo, kind, initialValue, initialFreq);
        ...
        case BigDecimalInfo:
            if (source == HashTableProfiler)
                ...
                valueInfo = new (comp->trPersistentMemory())
                    TR_LinkedListProfilerInfo<uint64_t>(bcInfo, kind, initialValue, initialFreq);
            break;

        case StringInfo:
            if (source == LinkedListProfiler)
                valueInfo = new (comp->trPersistentMemory())
                    TR_LinkedListProfilerInfo<TR_ByteInfo>(bcInfo, kind, bytes, initialFreq);
            break;
 ...
 */
TR_AbstractProfilerInfo *TR_ValueProfileInfo::getOrCreateProfilerInfo(TR_ByteCodeInfo &bcInfo, TR::Compilation *comp,
    TR_ValueInfoKind kind, TR_ValueInfoSource source, uint64_t initialValue)
{
 ...
 *
 * \param log Dump all the tables that have been inspected, as well as a log of any that were cleared. NULL will disable
 * tracing.
 */
void TR_ValueProfileInfo::resetLowFreqValues(OMR::Logger *log)
{
...
TR_BlockFrequencyInfo::TR_BlockFrequencyInfo(TR_CallSiteInfo *callSiteInfo, int32_t numBlocks, TR_ByteCodeInfo *blocks,
    int32_t *frequencies)
...
TR_BlockFrequencyInfo::TR_BlockFrequencyInfo(TR::Compilation *comp, TR_AllocationKind allocKind)
    : _callSiteInfo(TR_CallSiteInfo::getCurrent(comp))
    , _numBlocks(comp->getFlowGraph()->getNextNodeNumber())
    , _blocks(
          _numBlocks ? new(comp->trMemory(), allocKind, TR_Memory::BlockFrequencyInfo) TR_ByteCodeInfo[_numBlocks] : 0)
    , _frequencies(_numBlocks ?
                              /*
    ...
    , _isQueuedForRecompilation(0)
{
    ...
    for (TR::CFGNode *node = comp->getFlowGraph()->getFirstNode(); node; node = node->getNext()) {
        ...
        if (startTree) {
            ...
            TR_ASSERT(callSite
                    < static_cast<int32_t>(TR_CallSiteInfo::getCurrent(comp)->getNumCallSites() & 0x7FFFFFFF),
                "Block call site number is unexpected");

            _blocks[node->getNumber()] = byteCodeInfo;
        } else {
            TR_ASSERT(node->getNumber() == 0 || node->getNumber() == 1,
...
TR_BlockFrequencyInfo::~TR_BlockFrequencyInfo()
{
    ...
    if (_counterDerivationInfo) {
        for (int i = 0; i < 2 * _numBlocks; ++i) {
            if (_counterDerivationInfo[i] && ((int64_t)_counterDerivationInfo[i] & 0x1) != 1) {
    ...
    if (callerIndex > -1) {
        ...
        while (!callStack.empty()) {
            ...
            if (callerFrequency < 0) {
                ...
                if (!resolvedMethod->isInterpretedForHeuristics() && !resolvedMethod->isNative()
                    ...
                    if (info && info->getBlockFrequencyInfo()
 ...
 */
int32_t TR_BlockFrequencyInfo::getOriginalBlockNumberToGetRawCount(TR_ByteCodeInfo &bci, TR::Compilation *comp,
    bool trace)
{
    int32_t callerIndex = bci.getCallerIndex();
    TR::ResolvedMethodSymbol *resolvedMethod
        = callerIndex < 0 ? comp->getMethodSymbol() : comp->getInlinedResolvedMethodSymbol(callerIndex);
    int32_t byteCodeToSearch = resolvedMethod->getProfilingByteCodeIndex(bci.getByteCodeIndex());
    TR_ByteCodeInfo searchBCI = bci;
    searchBCI.setByteCodeIndex(byteCodeToSearch);
    ...
    for (auto i = 0; i < _numBlocks; ++i) {
        if ((currentCallSiteInfo && _callSiteInfo->hasSameBytecodeInfo(_blocks[i], searchBCI, comp))
            || (!currentCallSiteInfo && _blocks[i].getCallerIndex() == searchBCI.getCallerIndex()
                && _blocks[i].getByteCodeIndex() == searchBCI.getByteCodeIndex())) {
...
/**   \brief Using stored static blocl frequency counters creates a node that calculates the raw count of block in which
 * passed node belongs to \param comp Current compilation object \return root A node that loads/adds/subtracts the
 * static block counter to calculate raw frequency of corresponding block
 */
TR::Node *TR_BlockFrequencyInfo::generateBlockRawCountCalculationSubTree(TR::Compilation *comp, TR::Node *node,
    bool trace)
{
    return generateBlockRawCountCalculationSubTree(comp,
        getOriginalBlockNumberToGetRawCount(node->getByteCodeInfo(), comp, trace), node);
}

/**   \brief Creates a node that calculates the raw count of passed block from the stored static block frequency
 * counters \param comp Current compilation object \param blockNumber Number of a block for which we need to generate
 * frequency calculation node \return root A node that loads/adds/subtracts the static block counter to calculate raw
 * frequency of corresponding block
 */
TR::Node *TR_BlockFrequencyInfo::generateBlockRawCountCalculationSubTree(TR::Compilation *comp, int32_t blockNumber,
    TR::Node *node)
{
    TR::Node *root = NULL;
    if (blockNumber > -1
        && (_counterDerivationInfo[blockNumber * 2]
        ...
        if ((((uintptr_t)_counterDerivationInfo[blockNumber * 2]) & 0x1) == 1) {
            TR::SymbolReference *symRef = comp->getSymRefTab()->createKnownStaticDataSymbolRef(
                getFrequencyForBlock(((uintptr_t)_counterDerivationInfo[blockNumber * 2]) >> 1), TR::Int32);
            symRef->getSymbol()->setIsBlockFrequency();
            symRef->getSymbol()->setNotDataAddress();
            ...
            while (addBVI.hasMoreElements()) {
                TR::SymbolReference *symRef = comp->getSymRefTab()->createKnownStaticDataSymbolRef(
                    getFrequencyForBlock(addBVI.getNextElement()), TR::Int32);
                symRef->getSymbol()->setIsBlockFrequency();
                symRef->getSymbol()->setNotDataAddress();
        ...
        if (_counterDerivationInfo[blockNumber * 2 + 1] != NULL) {
            if ((((uintptr_t)_counterDerivationInfo[blockNumber * 2 + 1]) & 0x1) == 1) {
                TR::SymbolReference *symRef = comp->getSymRefTab()->createKnownStaticDataSymbolRef(
                    getFrequencyForBlock(((uintptr_t)_counterDerivationInfo[blockNumber * 2 + 1]) >> 1), TR::Int32);
                symRef->getSymbol()->setIsBlockFrequency();
                symRef->getSymbol()->setNotDataAddress();
                ...
                while (subBVI.hasMoreElements()) {
                    TR::SymbolReference *symRef = comp->getSymRefTab()->createKnownStaticDataSymbolRef(
                        getFrequencyForBlock(subBVI.getNextElement()), TR::Int32);
                    symRef->getSymbol()->setIsBlockFrequency();
                    symRef->getSymbol()->setNotDataAddress();
                    TR::Node *counterLoad = TR::Node::createWithSymRef(node, TR::iload, 0, symRef);
                    if (subRoot) {
    ...
    TR::Compilation *comp)
{
    ...
    for (uint32_t i = 0; i < _numBlocks; ++i) {
        if ((currentCallSiteInfo && callSiteInfo->hasSameBytecodeInfo(_blocks[i], bci, comp))
            || (!currentCallSiteInfo && _blocks[i].getCallerIndex() == bci.getCallerIndex()
                && _blocks[i].getByteCodeIndex() == bci.getByteCodeIndex())) {
            int64_t rawCount = 0;
...
TR_BlockFrequencyInfo::TR_BlockFrequencyInfo(const SerializedBFI *serializedData, uint8_t *&buffer,
    TR_PersistentProfileInfo *currentProfile)
    : _callSiteInfo(currentProfile->getCallSiteInfo())
    , _numBlocks(serializedData->numBlocks)
    , _blocks(_numBlocks ? new(PERSISTENT_NEW) TR_ByteCodeInfo[_numBlocks] : 0)
    , _frequencies(_numBlocks ?
                              /*
                               * The explicit parens value initializes the array,
                               * which in turn value initializes each array member,
              ...
              new(PERSISTENT_NEW) int32_t[_numBlocks]()
                              : NULL)
    , _counterDerivationInfo(_numBlocks ? (TR_BitVector **)new(PERSISTENT_NEW) void **[_numBlocks * 2]() : NULL)
    , _entryBlockNumber(-1)
    , _isQueuedForRecompilation(0)
{
    if (_numBlocks > 0) {
...
TR_BlockFrequencyInfo *TR_BlockFrequencyInfo::deserialize(uint8_t *&buffer,
    TR_PersistentProfileInfo *currentProfileInfo)
{
    SerializedBFI *serializedData = reinterpret_cast<SerializedBFI *>(buffer);
    buffer += sizeof(SerializedBFI);
...
TR_CallSiteInfo::TR_CallSiteInfo(TR::Compilation *comp, TR_AllocationKind allocKind)
    ...
    , _callSites(_numCallSites ? new(comp->trMemory(), allocKind, TR_Memory::CallSiteInfo)
                                     TR_InlinedCallSite[_numCallSites]
                               : NULL)
...
bool TR_CallSiteInfo::computeEffectiveCallerIndex(TR::Compilation *comp,
        ...
        if (itr != end) {
            ...
            while (cursor && next != end) {
                if (comp->fe()->getInlinedCallSiteMethod(cursor) != itr->first
                    || (cursor->_byteCodeInfo.getByteCodeIndex() != next->second.getByteCodeIndex())) {
                    break;
                }
...
bool TR_CallSiteInfo::hasSameBytecodeInfo(TR_ByteCodeInfo &persistentBytecodeInfo, TR_ByteCodeInfo &currentBytecodeInfo,
    TR::Compilation *comp)
{
    TR_ASSERT(currentBytecodeInfo.getByteCodeIndex() != TR_ByteCodeInfo::invalidByteCodeIndex,
    ...
    TR_ASSERT(persistentCallSite < static_cast<int32_t>(_numCallSites & 0x7FFFFFFF),
        "Unexpected call site will overflow. persistentCallSite = %d, _numCallSites = %d", persistentCallSite,
        _numCallSites);
...
int32_t TR_CallSiteInfo::hasSamePartialBytecodeInfo(TR_ByteCodeInfo &persistentBytecodeInfo,
    TR_ByteCodeInfo &currentBytecodeInfo, TR::Compilation *comp)
{
    if (persistentBytecodeInfo.getByteCodeIndex() != currentBytecodeInfo.getByteCodeIndex())
        return 0;
 ...
 * \param comp Current compilation.
 */
void TR_PersistentProfileInfo::prepareForProfiling(TR::Compilation *comp)
{
    if (!comp->haveCommittedCallSiteInfo()) {
        ...
        TR_CallSiteInfo * const originalCallSiteInfo = getCallSiteInfo();
        TR_ASSERT_FATAL(originalCallSiteInfo != NULL,
            "Existing CallSiteInfo should not be NULL for persistent profile info %p.", this);

        // Destroy the prior call site info and placement new the updated version
        originalCallSiteInfo->~TR_CallSiteInfo();
        TR_CallSiteInfo * const updatedCallSiteInfo
            = new ((void *)originalCallSiteInfo) TR_CallSiteInfo(comp, persistentAlloc);
    }
}
 ...
 * \param comp The current compilation.
 */
TR_PersistentProfileInfo *TR_PersistentProfileInfo::get(TR::Compilation *comp)
{
 ...
 */
TR_PersistentProfileInfo *TR_PersistentProfileInfo::get(TR::Compilation *comp, TR_ResolvedMethod *feMethod)
{
 ...
 * \param comp The current compilation.
 */
TR_PersistentProfileInfo *TR_PersistentProfileInfo::getCurrent(TR::Compilation *comp)
{
 ...
 */
void TR_PersistentProfileInfo::incRefCount(TR_PersistentProfileInfo *info)
{
 ...
 * \param info Persistent profile info to manipulate and potentially free.
 */
void TR_PersistentProfileInfo::decRefCount(TR_PersistentProfileInfo *info)
{
    ...
    if (!TR::Options::getCmdLineOptions()->getOption(TR_DisableJProfilerThread)) {
        if (info->_refCount == 0 && TR::Options::isAnyVerboseOptionSet(TR_VerboseProfiling, TR_VerboseReclamation))
            TR_VerboseLog::writeLineLocked(TR_Vlog_RECLAMATION, "PersistentProfileInfo 0x%p queued for reclamation.",
                info);
    } else if (info->_refCount == 0
        && !TR::Options::getCmdLineOptions()->getOption(TR_DisableProfilingDataReclamation)) {
        if (TR::Options::getVerboseOption(TR_VerboseReclamation))
            TR_VerboseLog::writeLineLocked(TR_Vlog_RECLAMATION, "Reclaiming PersistentProfileInfo immediately 0x%p.",
                info);
 ...
 */
TR_CatchBlockProfileInfo *TR_PersistentProfileInfo::findOrCreateCatchBlockProfileInfo(TR::Compilation *comp)
{
...
TR_BlockFrequencyInfo *TR_PersistentProfileInfo::findOrCreateBlockFrequencyInfo(TR::Compilation *comp)
{
    prepareForProfiling(comp);
    if (!_blockFrequencyInfo)
...
TR_ValueProfileInfo *TR_PersistentProfileInfo::findOrCreateValueProfileInfo(TR::Compilation *comp)
{
    prepareForProfiling(comp);
    if (!_valueProfileInfo)
...
void TR_CallSiteInfo::dumpInfo(OMR::Logger *log)
{
    ...
    for (int32_t i = 0; i < _numCallSites; i++)
        log->printf("   Call site index = %d, method = %p, parent = %d\n",
            _callSites[i]._byteCodeInfo.getByteCodeIndex(), _callSites[i]._methodInfo,
            _callSites[i]._byteCodeInfo.getCallerIndex());
}
...
TR_CallSiteInfo::TR_CallSiteInfo(const SerializedCSI *data, uint8_t *&buffer)
    ...
    , _callSites(_numCallSites ? new(PERSISTENT_NEW) TR_InlinedCallSite[_numCallSites] : NULL)
    ...
    , _refCount(1)
{
    ...
    _blockFrequencyInfo
        = serializedData->hasBlockFrequencyInfo ? TR_BlockFrequencyInfo::deserialize(buffer, this) : NULL;
 ...
 */
static int32_t J9THREAD_PROC jProfilerThreadProc(void *entryarg)
{
...
TR_JProfilerThread *TR_JProfilerThread::allocate()
{
 ...
 */
void TR_JProfilerThread::start(J9JavaVM *javaVM)
{
    ...
    if (_jProfilerMonitor) {
        ...
        if (javaVM->internalVMFunctions->createThreadWithCategory(&_jProfilerOSThread,
                TR::Options::_profilerStackSize << 10, priority, 0, &jProfilerThreadProc, javaVM->jitConfig,
 ...
 */
void TR_JProfilerThread::stop(J9JavaVM *javaVM)
{
    PORT_ACCESS_FROM_JAVAVM(javaVM);
    if (!_jProfilerMonitor)
        return;
 ...
 */
void TR_JProfilerThread::processWorkingQueue()
{
    while (_state == Run) {
        _jProfilerMonitor->enter();
         ...
         *
         * _listHead has the type "TR_PersistentProfileInfo * volatile" which is a volatile pointer to a non-volatile
         * TR_PersistentProfileInfo. It is the pointer value itself that is volatile not the data it is pointing to.
         *
         * prevPtr is a pointer to _listHead. prevPtr itself is non-volatile.
         * When put together, prevPtr is a non-volatile pointer to a volatile pointer to a non-volatile
 ...
 */
void TR_JProfilerThread::addProfileInfo(TR_PersistentProfileInfo *newHead)
{
    ...
    do {
        ...
        != VM_AtomicSupport::lockCompareExchange(reinterpret_cast<volatile uintptr_t *>(&_listHead), oldPtr,
            reinterpret_cast<uintptr_t>(newHead)));
 ...
 *
 * \param prevNext Pointer to previous info's next pointer. This is a non-volatile pointer to a volatile pointer to a
 * non-volatile TR_PersistentProfileInfo. \param info Info to remove. \return The next TR_PersistentProfileInfo after
 * the removed info.
 */
TR_PersistentProfileInfo *TR_JProfilerThread::deleteProfileInfo(TR_PersistentProfileInfo * volatile *prevNext,
    TR_PersistentProfileInfo *info)
{
    ...
    if (oldPtr
        != VM_AtomicSupport::lockCompareExchange(reinterpret_cast<volatile uintptr_t *>(prevNext), oldPtr,
