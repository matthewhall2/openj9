...
uintptr_t TR_PatchNOPedGuardSiteOnClassPreInitialize::hashCode(char *sig, uint32_t sigLen)
{
    uintptr_t sum = 0;
    uintptr_t scale = 1;
...
void TR_PatchNOPedGuardSiteOnClassPreInitialize::reclaim()
{
...
bool TR_PatchNOPedGuardSiteOnClassPreInitialize::matches(char *sig, uint32_t sigLen)
{
    if (sigLen + 2 != _sigLen) {
        return false;
    }
...
TR_PatchNOPedGuardSiteOnMethodBreakPoint *TR_PatchNOPedGuardSiteOnMethodBreakPoint::make(TR_FrontEnd *fe,
...
void TR_PreXRecompile::compensate(TR_FrontEnd *fe, bool, void *)
{
    TR_J9VMBase *fej9 = (TR_J9VMBase *)fe;

#if (defined(TR_HOST_X86) || defined(TR_HOST_POWER) || defined(TR_HOST_S390) || defined(TR_HOST_ARM) \
    || defined(TR_HOST_ARM64))
    TR::Recompilation::invalidateMethodBody(_startPC, fe, TR_JitBodyInvalidations::Preexistence);
...
void TR_ClassUnloadRecompile::compensate(TR_FrontEnd *fe, bool, void *)
{
    TR_J9VMBase *fej9 = (TR_J9VMBase *)fe;
    TR::Recompilation::invalidateMethodBody(_startPC, fe, TR_JitBodyInvalidations::Unloading);

    fej9->reportUnloadInvalidation(_startPC);
}

void TR_PatchJNICallSite::compensate(TR_FrontEnd *fe, bool isSMP, void *newAddress)
{
#if (defined(TR_HOST_X86) || defined(TR_HOST_POWER) || defined(TR_HOST_S390) || defined(TR_HOST_ARM) \
    || defined(TR_HOST_ARM64))
...
void TR_PersistentCHTable::classGotUnloadedPost(TR_FrontEnd *fe, TR_OpaqueClassBlock *classId)
{
    ...
    if ((classDepth >= 0) && (cl->isInitialized() || fej9->isInterfaceClass(classId))) {
...
bool TR_PersistentCHTable::classGotExtended(TR_FrontEnd *fe, TR_PersistentMemory *persistentMemory,
    TR_OpaqueClassBlock *superClassId, TR_OpaqueClassBlock *subClassId)
{
    ...
    TR_PersistentClassInfo *subClass
        = findClassInfo(subClassId); // This is actually the class that got loaded extending the superclass
...
void TR_PersistentCHTable::removeClass(TR_FrontEnd *fe, TR_OpaqueClassBlock *classId, TR_PersistentClassInfo *info,
    bool removeInfo)
{
    TR_ASSERT_FATAL(isAccessible(), "Should not be called if table is not accessible!");
    if (!info)
...
bool TR_PersistentCHTable::classGotInitialized(TR_FrontEnd *fe, TR_PersistentMemory *persistentMemory,
    TR_OpaqueClassBlock *classId, TR_PersistentClassInfo *clazz)
{
    TR_ASSERT_FATAL(isAccessible(), "Should not be called if table is not accessible!");
    if (!clazz)
        clazz = findClassInfo(classId);
...

    {
        ...
        TR_RuntimeAssumptionTable *table = persistentMemory->getPersistentInfo()->getRuntimeAssumptionTable();
        OMR::RuntimeAssumption **headPtr = table->getBucketPtr(RuntimeAssumptionOnClassPreInitialize,
            TR_PatchNOPedGuardSiteOnClassPreInitialize::hashCode(sig, sigLen));
...
void TR_PersistentCHTable::methodGotOverridden(TR_FrontEnd *fe, TR_PersistentMemory *persistentMemory,
    TR_OpaqueMethodBlock *overriddingMethod, TR_OpaqueMethodBlock *overriddenMethod, int32_t smpFlag)
{
...
void TR_PersistentCHTable::classGotRedefined(TR_FrontEnd *fe, TR_OpaqueClassBlock *oldClassId,
...
void TR_PersistentCHTable::removeAssumptionFromRAT(OMR::RuntimeAssumption *assumption)
{
...
void TR_PersistentClassInfo::setShouldNotBeNewlyExtended(int32_t ID)
{
#if defined(J9VM_OPT_JITSERVER)
