...
TR_RelocationRuntime::TR_RelocationRuntime(J9JITConfig *jitCfg)
{
    ...
    if (!_globalValuesInitialized) {
        ...
        setGlobalValue(TR_HeapBaseForBarrierRange0,
            (uintptr_t) * (uintptr_t *)((uint8_t *)vmThread + offsetof(J9VMThread, heapBaseForBarrierRange0)));
        setGlobalValue(TR_ActiveCardTableBase,
            (uintptr_t) * (uintptr_t *)((uint8_t *)vmThread + offsetof(J9VMThread, activeCardTableBase)));
        setGlobalValue(TR_HeapSizeForBarrierRange0,
            (uintptr_t) * (uintptr_t *)((uint8_t *)vmThread + offsetof(J9VMThread, heapSizeForBarrierRange0)));
        setGlobalValue(TR_MethodEnterHookEnabledAddress,
            (uintptr_t) & (javaVM()->hookInterface.flags[J9HOOK_VM_METHOD_ENTER]));
        setGlobalValue(TR_MethodExitHookEnabledAddress,
...
J9JITExceptionTable *TR_RelocationRuntime::prepareRelocateAOTCodeAndData(J9VMThread *vmThread, TR_FrontEnd *theFE,
    TR::CodeCache *aotMCCRuntimeCodeCache, const J9JITDataCacheHeader *cacheEntry, J9Method *theMethod,
    bool shouldUseCompiledCopy, TR::Options *options, TR::Compilation *comp, TR_ResolvedMethod *resolvedMethod,
    uint8_t *existingCode, TR_J9SharedCache *cacheOverride)
{
    ...
    if (_aotMethodHeaderEntry->flags & TR_AOTMethodHeader_UsesOSR) {
        if (!comp->getOption(TR_EnableOSR)
            || !fej9->ensureOSRBufferSize(comp, _aotMethodHeaderEntry->_osrBufferInfo._frameSizeInBytes,
    ...
    _exceptionTableCacheEntry
        = (J9JITDataCacheHeader *)((uint8_t *)cacheEntry + _aotMethodHeaderEntry->offsetToExceptionTable);
    ...
    if (_exceptionTableCacheEntry->type == J9_JIT_DCE_EXCEPTION_INFO) {
        ...
        TR_ASSERT(codeSize > sizeof(OMR::CodeCacheMethodHeader),
            "codeSize for AOT loads should include the CodeCacheHeader");
        ...
        if (useCompiledCopy()) {
            ...
            _exceptionTable
                = (J9JITExceptionTable *)(_exceptionTableCacheEntry + 1); // skip the header J9JITDataCacheHeader
        } else {
            _newExceptionTableStart
                = allocateSpaceInDataCache(_exceptionTableCacheEntry->size, _exceptionTableCacheEntry->type);
            if (existingCode) {
            ...
            if (_newExceptionTableStart) {
                TR_DataCacheManager::copyDataCacheAllocation(
                    reinterpret_cast<J9JITDataCacheHeader *>(_newExceptionTableStart), _exceptionTableCacheEntry);
                _exceptionTable = reinterpret_cast<J9JITExceptionTable *>(
                    _newExceptionTableStart + sizeof(J9JITDataCacheHeader)); // Get new exceptionTable location
                ...
                static_cast<TR::SentinelRuntimeAssumption *>(*comp->getMetadataAssumptionList())
                    ->setOwningMetadata(_exceptionTable);
                ...
                if (_exceptionTable->bodyInfo) {
                    J9JITDataCacheHeader *persistentInfoCacheEntry
                        = (J9JITDataCacheHeader *)((U_8 *)cacheEntry + _aotMethodHeaderEntry->offsetToPersistentInfo);
                    TR_ASSERT(persistentInfoCacheEntry->type == J9_JIT_DCE_AOT_PERSISTENT_INFO,
                        "Incorrect data cache type read from disk.");
                    _newPersistentInfo
                        = allocateSpaceInDataCache(persistentInfoCacheEntry->size, persistentInfoCacheEntry->type);
                    if (_newPersistentInfo) {
                        TR_DataCacheManager::copyDataCacheAllocation(
                            reinterpret_cast<J9JITDataCacheHeader *>(_newPersistentInfo), persistentInfoCacheEntry);
                    } else {
                        reloLogger()->maxCodeOrDataSizeWarning();
                        _relocationStatus = RelocationPersistentCreateError;
                    }
                }
                // newCodeStart points after a OMR::CodeCacheMethodHeader, but tempCodeStart points at a
                // OMR::CodeCacheMethodHeader to keep alignment consistent, back newCodeStart over the
    ...
    TR_ASSERT_FATAL(_exceptionTable == NULL || _exceptionTable->constRefArrays == NULL,
        "unexpected const ref array %p for J9JITExceptionTable %p", _exceptionTable->constRefArrays, _exceptionTable);
#endif
    ...
    switch (_relocationStatus) {
        ...
        case RelocationCodeCreateError: {
            ...
            if (!useCompiledCopy() && _exceptionTable->bodyInfo) {
                TR_DataCacheManager::getManager()->freeDataCacheRecord(
                    _newPersistentInfo + sizeof(J9JITDataCacheHeader));
            }
        }
...
//
void TR_RelocationRuntime::relocateAOTCodeAndData(U_8 *tempDataStart, U_8 *oldDataStart, U_8 *codeStart,
    U_8 *oldCodeStart)
{
    J9JITDataCacheHeader *cacheEntry = (J9JITDataCacheHeader *)(tempDataStart);
    UDATA startPC = 0;

    RELO_LOG(_reloLogger, 2,
        "relocateAOTCodeAndData jitConfig=%p aotDataCache=%p aotMccCodeCache=%p method=%p tempDataStart=%p "
        "exceptionTable=%p oldDataStart=%p codeStart=%p oldCodeStart=%p classReloAmount=%p cacheEntry=%p\n",
        jitConfig(), dataCache(), codeCache(), _method, tempDataStart, _exceptionTable, oldDataStart, codeStart,
        oldCodeStart, classReloAmount(), cacheEntry);
    ...
    if (_exceptionTableCacheEntry->type == J9_JIT_DCE_EXCEPTION_INFO) {
        ...
        getClassNameSignatureFromMethod(_method, _exceptionTable->className, _exceptionTable->methodName,
            _exceptionTable->methodSignature);
        RELO_LOG(reloLogger(), 1, "relocateAOTCodeAndData: method %.*s.%.*s%.*s\n",
            J9UTF8_LENGTH(_exceptionTable->className), J9UTF8_DATA(_exceptionTable->className),
            J9UTF8_LENGTH(_exceptionTable->methodName), J9UTF8_DATA(_exceptionTable->methodName),
            J9UTF8_LENGTH(_exceptionTable->methodSignature), J9UTF8_DATA(_exceptionTable->methodSignature));

        /* Now it is safe to perform the JITExceptionTable structure relocations */
        relocateMethodMetaData((UDATA)codeStart - (UDATA)oldCodeStart,
            (UDATA)_exceptionTable
                - (UDATA)((U_8 *)oldDataStart + _aotMethodHeaderEntry->offsetToExceptionTable
                    + sizeof(J9JITDataCacheHeader)));

        reloTarget()->preRelocationsAppliedEvent();

        /* Perform code relocations */
        if (_aotMethodHeaderEntry->offsetToRelocationDataItems != 0) {
            TR_RelocationRecordBinaryTemplate *binaryReloRecords
                = (TR_RelocationRecordBinaryTemplate *)((U_8 *)_aotMethodHeaderEntry - sizeof(J9JITDataCacheHeader)
                    + _aotMethodHeaderEntry->offsetToRelocationDataItems);
            TR_RelocationRecordGroup reloGroup(binaryReloRecords);

            RELO_LOG(reloLogger(), 2,
                "relocateAOTCodeAndData: jitConfig=%x aotDataCache=%x aotMccCodeCache=%x method=%x tempDataStart=%x "
                "exceptionTable=%x\n",
                jitConfig(), dataCache(), codeCache(), _method, tempDataStart, _exceptionTable);
            RELO_LOG(reloLogger(), 2,
                "                        oldDataStart=%x codeStart=%x oldCodeStart=%x classReloAmount=%x "
                "cacheEntry=%x\n",
                oldDataStart, codeStart, oldCodeStart, classReloAmount(), cacheEntry);
            RELO_LOG(reloLogger(), 2,
                "                        tempDataStart: %p, _aotMethodHeaderEntry: %p, header offset: %x, "
                "binaryReloRecords: %p\n",
                tempDataStart, _aotMethodHeaderEntry, (UDATA)_aotMethodHeaderEntry - (UDATA)tempDataStart,
                binaryReloRecords);

            try {
                TR_RelocationErrorCode errorCode
                    = reloGroup.applyRelocations(this, reloTarget(), newMethodCodeStart() + codeCacheDelta());
            ...
            if ((getNumInlinedMethodRelos() > 10)
                && (getNumFailedInlinedMethodRelos() > getNumInlinedMethodRelos() / 2)) {
                if (aotStats())
        ...
        if (((UDATA)_exceptionTable->numExcptionRanges) & J9_JIT_METADATA_WIDE_EXCEPTIONS) {
            ...
            uint16_t numExcptionRanges = _exceptionTable->numExcptionRanges
            ...
            while (numExcptionRanges > 0) {
                ...
                if (inlinedSiteIndex != (UDATA)-1) {
                    TR_InlinedCallSite *inlinedCallSite
                        = (TR_InlinedCallSite *)getInlinedCallSiteArrayElement(_exceptionTable, (int)inlinedSiteIndex);
        ...
        if (comp()->getOptions()->getVerboseOption(TR_VerboseInlining)) {
            ...
            if (_exceptionTable->inlinedCalls) {
                ...
                TR_VerboseLog::writeLine(TR_Vlog_INL, "%d methods inlined into %x %s @ %p", numInlinedCallSites,
                    jittedBodyHash, comp()->signature(), _exceptionTable->startPC);
                for (auto siteIndex = 0; siteIndex < numInlinedCallSites; siteIndex++) {
                    TR_InlinedCallSite *site
                        = (TR_InlinedCallSite *)getInlinedCallSiteArrayElement(_exceptionTable, siteIndex);
                    int32_t calleeLen
                        = fej9()->printTruncatedSignature(calleeBuf, sizeof(calleeBuf) - 1, site->_methodInfo);
                    ...
                    if (callerIndex != -1) {
                        TR_InlinedCallSite *callerCallsite
                            = (TR_InlinedCallSite *)getInlinedCallSiteArrayElement(_exceptionTable, callerIndex);
                        int32_t callerLen = fej9()->printTruncatedSignature(callerBuf, sizeof(callerBuf) - 1,
                            callerCallsite->_methodInfo);
                        callerBuf[callerLen] = 0; // null terminate
                        callerSig = callerBuf;
                    }
                    TR_VerboseLog::writeLine(TR_Vlog_INL, "#%d: %x #%d inlined %x@%d -> %x bcsz=%d %s", siteIndex,
                ...
                TR_VerboseLog::writeLineLocked(TR_Vlog_INL, "0 methods inlined into %x %s @ %p", jittedBodyHash,
                    comp()->signature(), _exceptionTable->startPC);
            }
        }
...
#if defined(J9VM_OPT_JITSERVER)
    TR_ASSERT_FATAL(!_comp->getStream(),
        "TR_RelocationRuntime::relocateAOTCodeAndData should not be called at the JITSERVER");
#endif /* defined(J9VM_OPT_JITSERVER) */
    if (startPC) {
...
void TR_RelocationRuntime::relocateMethodMetaData(UDATA codeRelocationAmount, UDATA dataRelocationAmount)
{
#if 0
      J9UTF8 * name;
    ...
    if (_exceptionTable->bodyInfo && !useCompiledCopy()) {
        TR_PersistentJittedBodyInfo *persistentBodyInfo
            = reinterpret_cast<TR_PersistentJittedBodyInfo *>((_newPersistentInfo + sizeof(J9JITDataCacheHeader)));
        TR_PersistentMethodInfo *persistentMethodInfo = reinterpret_cast<TR_PersistentMethodInfo *>(
            (_newPersistentInfo + sizeof(J9JITDataCacheHeader)) + sizeof(TR_PersistentJittedBodyInfo));

#if defined(J9VM_OPT_JITSERVER)
        if (_comp->isRemoteCompilation() && !_comp->getCurrentMethod()->isInterpreted()) {
            TR_PersistentMethodInfo *existingPersistentMethodInfo
                = _comp->getRecompilationInfo()->getExistingMethodInfo(_comp->getCurrentMethod());
    ...
    if (getPersistentInfo()->isRuntimeInstrumentationEnabled()
        ...
        && TR::Options::getCmdLineOptions()->getOption(TR_EnableMetadataBytecodePCToIAMap) && _exceptionTable->riData) {
...
bool TR_RelocationRuntime::aotMethodHeaderVersionsMatch()
{
...
void TR_RelocationRuntime::fillAOTHeader(J9JavaVM *vm, TR_FrontEnd *fe, TR_AOTHeader *aotHeader)
{
    ...
    aotHeader->compressedPointerShift = vm->memoryManagerFunctions->j9gc_objaccess_compressedPointersShift(
...
uint32_t TR_RelocationRuntime::getCurrentLockwordOptionHashValue(J9JavaVM *vm)
{
    ...
    if (currentLockwordArgIndex >= 0) {
        ...
        currentLockwordOptionHashValue
            = CS2::Hash_FNV((unsigned char *)currentLockwordOption, strlen(currentLockwordOption));
    }
    return currentLockwordOptionHashValue;
}

uintptr_t TR_RelocationRuntime::generateFeatureFlags(TR_FrontEnd *fe)
{
...
bool TR_RelocationRuntime::storeAOTHeader(TR_FrontEnd *fe, J9VMThread *curThread)
{
    TR_ASSERT_FATAL(false, "Error: storeAOTHeader not supported in this relocation runtime");
    return false;
}

const TR_AOTHeader *TR_RelocationRuntime::getStoredAOTHeader(J9VMThread *curThread)
{
    TR_ASSERT_FATAL(false, "Error: getStoredAOTHeader not supported in this relocation runtime");
    return NULL;
}

TR_AOTHeader *TR_RelocationRuntime::createAOTHeader(TR_FrontEnd *fe)
{
    TR_ASSERT_FATAL(false, "Error: createAOTHeader not supported in this relocation runtime");
    return NULL;
}

bool TR_RelocationRuntime::validateAOTHeader(TR_FrontEnd *fe, J9VMThread *curThread)
{
    TR_ASSERT_FATAL(false, "Error: validateAOTHeader not supported in this relocation runtime");
    return false;
}

OMRProcessorDesc TR_RelocationRuntime::getProcessorDescriptionFromSCC(J9VMThread *curThread)
{
...
#if defined(J9VM_OPT_JITSERVER)
J9JITExceptionTable *TR_RelocationRuntime::copyMethodMetaData(J9JITDataCacheHeader *dataCacheHeader)
{
    TR_AOTMethodHeader *aotMethodHeaderEntry = (TR_AOTMethodHeader *)(dataCacheHeader + 1);
    J9JITDataCacheHeader *exceptionTableCacheEntry
        = (J9JITDataCacheHeader *)((uint8_t *)dataCacheHeader + aotMethodHeaderEntry->offsetToExceptionTable);
    uint8_t *newExceptionTableStart
        = allocateSpaceInDataCache(exceptionTableCacheEntry->size, exceptionTableCacheEntry->type);
    J9JITExceptionTable *metaData = NULL;
    if (newExceptionTableStart) {
        TR_DataCacheManager::copyDataCacheAllocation(reinterpret_cast<J9JITDataCacheHeader *>(newExceptionTableStart),
            exceptionTableCacheEntry);
        metaData = reinterpret_cast<J9JITExceptionTable *>(
            newExceptionTableStart + sizeof(J9JITDataCacheHeader)); // Get new exceptionTable location
    }
    return metaData;
}
#endif /* defined(J9VM_OPT_JITSERVER) */
...
const char *TR_RelocationRuntime::_globalValueNames[TR_NumGlobalValueItems]
    = { "not used (0)", "TR_CountForRecompile (1)", "TR_HeapBase (2)", "TR_HeapTop (3)",
          "TR_HeapBaseForBarrierRange0 (4)", "TR_ActiveCardTableBase (5)", "TR_HeapSizeForBarrierRange0 (6)" };
...
const UDATA TR_SharedCacheRelocationRuntime::aotHeaderKeyLength
    = sizeof(TR_SharedCacheRelocationRuntime::aotHeaderKey) - 1;

U_8 *TR_SharedCacheRelocationRuntime::allocateSpaceInCodeCache(UDATA codeSize)
{
    TR_J9VMBase *fej9 = (TR_J9VMBase *)_fe;
    ...
    if (!codeCache()) {
        ...
        _codeCache = manager->reserveCodeCache(false, codeSize, compThreadID, &numReserved,
            _comp->codeCacheKind()); // Acquire a cold/warm code cache.
        if (!codeCache()) {
...
uint8_t *TR_SharedCacheRelocationRuntime::allocateSpaceInDataCache(uintptr_t metaDataSize, uintptr_t type)
{
...
void TR_SharedCacheRelocationRuntime::initializeAotRuntimeInfo()
{
    if (!useCompiledCopy())
        _classReloAmount = 1;
}

void TR_SharedCacheRelocationRuntime::initializeCacheDeltas()
{
    _dataCacheDelta = 0;
    _codeCacheDelta = 0;
}

void TR_SharedCacheRelocationRuntime::incompatibleCache(U_32 module_name, U_32 reason, const char *assumeMessage)
{
...
bool TR_SharedCacheRelocationRuntime::generateError(U_32 module_name, U_32 reason, const char *assumeMessage)
{
    incompatibleCache(module_name, reason, assumeMessage);
    return false;
}

void TR_SharedCacheRelocationRuntime::checkAOTHeaderFlags(const TR_AOTHeader *hdrInCache, intptr_t featureFlags)
{
    bool defaultMessage = true;

    if (!TR::Compiler->relocatableTarget.cpu.isCompatible(hdrInCache->processorDescription))
        defaultMessage = generateError(J9NLS_RELOCATABLE_CODE_WRONG_HARDWARE,
            "AOT header validation failed: Processor incompatible.");
    if ((featureFlags & TR_FeatureFlag_sanityCheckBegin)
        != (hdrInCache->featureFlags & TR_FeatureFlag_sanityCheckBegin))
        defaultMessage = generateError(J9NLS_RELOCATABLE_CODE_HEADER_START_SANITY_BIT_MANGLED,
            "AOT header validation failed: Processor feature sanity bit mangled.");
    if ((featureFlags & TR_FeatureFlag_IsSMP) != (hdrInCache->featureFlags & TR_FeatureFlag_IsSMP))
        defaultMessage
            = generateError(J9NLS_RELOCATABLE_CODE_SMP_MISMATCH, "AOT header validation failed: SMP feature mismatch.");
    if ((featureFlags & TR_FeatureFlag_UsesCompressedPointers)
        != (hdrInCache->featureFlags & TR_FeatureFlag_UsesCompressedPointers))
        defaultMessage = generateError(J9NLS_RELOCATABLE_CODE_CMPRS_PTR_MISMATCH,
            "AOT header validation failed: Compressed references feature mismatch.");
    if ((featureFlags & TR_FeatureFlag_DisableTraps) != (hdrInCache->featureFlags & TR_FeatureFlag_DisableTraps))
        defaultMessage = generateError(J9NLS_RELOCATABLE_CODE_DISABLE_TRAPS_MISMATCH,
            "AOT header validation failed: Use of trap instruction feature mismatch.");
    if ((featureFlags & TR_FeatureFlag_TLHPrefetch) != (hdrInCache->featureFlags & TR_FeatureFlag_TLHPrefetch))
        defaultMessage = generateError(J9NLS_RELOCATABLE_CODE_TLH_PREFETCH_MISMATCH,
            "AOT header validation failed: TLH prefetch feature mismatch.");
    if ((featureFlags & TR_FeatureFlag_MethodTrampolines)
        != (hdrInCache->featureFlags & TR_FeatureFlag_MethodTrampolines))
        defaultMessage = generateError(J9NLS_RELOCATABLE_CODE_METHOD_TRAMPOLINE_MISMATCH,
            "AOT header validation failed: MethodTrampolines feature mismatch.");
    if ((featureFlags & TR_FeatureFlag_FSDEnabled) != (hdrInCache->featureFlags & TR_FeatureFlag_FSDEnabled))
        defaultMessage
            = generateError(J9NLS_RELOCATABLE_CODE_FSD_MISMATCH, "AOT header validation failed: FSD feature mismatch.");
    if ((featureFlags & TR_FeatureFlag_HCREnabled) != (hdrInCache->featureFlags & TR_FeatureFlag_HCREnabled))
        defaultMessage
            = generateError(J9NLS_RELOCATABLE_CODE_HCR_MISMATCH, "AOT header validation failed: HCR feature mismatch.");
    if (((featureFlags & TR_FeatureFlag_SIMDEnabled) == 0)
        && ((hdrInCache->featureFlags & TR_FeatureFlag_SIMDEnabled) != 0))
        defaultMessage = generateError(J9NLS_RELOCATABLE_CODE_SIMD_MISMATCH,
            "AOT header validation failed: SIMD feature mismatch.");
    if ((featureFlags & TR_FeatureFlag_AsyncCompilation)
        != (hdrInCache->featureFlags & TR_FeatureFlag_AsyncCompilation))
        defaultMessage = generateError(J9NLS_RELOCATABLE_CODE_ASYNC_COMP_MISMATCH,
            "AOT header validation failed: AsyncCompilation feature mismatch.");
    if ((featureFlags & TR_FeatureFlag_ConcurrentScavenge)
        != (hdrInCache->featureFlags & TR_FeatureFlag_ConcurrentScavenge))
        defaultMessage = generateError(J9NLS_RELOCATABLE_CODE_CS_MISMATCH,
            "AOT header validation failed: Concurrent Scavenge feature mismatch.");
    if ((featureFlags & TR_FeatureFlag_SoftwareReadBarrier)
        != (hdrInCache->featureFlags & TR_FeatureFlag_SoftwareReadBarrier))
        defaultMessage = generateError(J9NLS_RELOCATABLE_CODE_SW_READBAR_MISMATCH,
            "AOT header validation failed: Software Read Barrier feature mismatch.");
    if ((featureFlags & TR_FeatureFlag_UsesTM) != (hdrInCache->featureFlags & TR_FeatureFlag_UsesTM))
        defaultMessage
            = generateError(J9NLS_RELOCATABLE_CODE_TM_MISMATCH, "AOT header validation failed: TM feature mismatch.");
    if ((featureFlags & TR_FeatureFlag_IsVariableHeapBaseForBarrierRange0)
        != (hdrInCache->featureFlags & TR_FeatureFlag_IsVariableHeapBaseForBarrierRange0))
        defaultMessage = generateError(J9NLS_RELOCATABLE_CODE_HEAP_BASE_FOR_BARRIER_RANGE_MISMATCH,
            "AOT header validation failed: Heap Base for Barrier Range feature mismatch.");
    if ((featureFlags & TR_FeatureFlag_IsVariableHeapSizeForBarrierRange0)
        != (hdrInCache->featureFlags & TR_FeatureFlag_IsVariableHeapSizeForBarrierRange0))
        defaultMessage = generateError(J9NLS_RELOCATABLE_CODE_HEAP_SIZE_FOR_BARRIER_RANGE_MISMATCH,
            "AOT header validation failed: Heap Size for Barrier Range feature mismatch.");
    if ((featureFlags & TR_FeatureFlag_IsVariableActiveCardTableBase)
        != (hdrInCache->featureFlags & TR_FeatureFlag_IsVariableActiveCardTableBase))
        defaultMessage = generateError(J9NLS_RELOCATABLE_CODE_ACTIVE_CARD_TABLE_BASE_MISMATCH,
            "AOT header validation failed: Active Card Table Base feature mismatch.");
    if ((featureFlags & TR_FeatureFlag_ArrayHeaderShape)
        != (hdrInCache->featureFlags & TR_FeatureFlag_ArrayHeaderShape))
        defaultMessage = generateError(J9NLS_RELOCATABLE_CODE_ARRAY_HEADER_SHAPE_MISMATCH,
            "AOT header validation failed: Array header shape mismatch.");
    if ((featureFlags & TR_FeatureFlag_CHTableEnabled) != (hdrInCache->featureFlags & TR_FeatureFlag_CHTableEnabled))
        defaultMessage = generateError(J9NLS_RELOCATABLE_CODE_CH_TABLE_MISMATCH,
            "AOT header validation failed: CH Table mismatch.");
...
OMRProcessorDesc TR_SharedCacheRelocationRuntime::getProcessorDescriptionFromSCC(J9VMThread *curThread)
{
...
// This function currently does not rely on the object beyond the v-table override (compiled as static without any
// problems). If this changes, we will need to look further into whether its users risk concurrent access.
bool TR_SharedCacheRelocationRuntime::validateAOTHeader(TR_FrontEnd *fe, J9VMThread *curThread)
{
    bool cacheTooBig;
    ...
    if (hdrInCache) {
        ...
        } else if (hdrInCache->featureFlags != featureFlags
        ...
        } else if (!((hdrInCache->gcPolicyFlag
                         == javaVM()->memoryManagerFunctions->j9gc_modron_getWriteBarrierType(javaVM()))
                       ||
                       // it's safe to run AOT code with inline cardmark barrier even if runtime will not need it since
                       // the barrier is only executed for active Concurrent Mark cycles (which won't ever happen with
                       // Concurrent Mark disabled in runtime)
...
const TR_AOTHeader *TR_SharedCacheRelocationRuntime::getStoredAOTHeaderWithConfig(
    J9SharedClassConfig *sharedClassConfig, J9VMThread *curThread)
{
    J9SharedDataDescriptor firstDescriptor;
    firstDescriptor.address = NULL;
    sharedClassConfig->findSharedData(curThread, aotHeaderKey, aotHeaderKeyLength, J9SHR_DATA_TYPE_AOTHEADER, FALSE,
        &firstDescriptor, NULL);
    return (const TR_AOTHeader *)firstDescriptor.address;
}

const TR_AOTHeader *TR_SharedCacheRelocationRuntime::getStoredAOTHeader(J9VMThread *curThread)
{
    return getStoredAOTHeaderWithConfig(javaVM()->sharedClassConfig, curThread);
}

TR_AOTHeader *TR_SharedCacheRelocationRuntime::createAOTHeader(TR_FrontEnd *fe)
{
    PORT_ACCESS_FROM_JAVAVM(javaVM());
...
bool TR_SharedCacheRelocationRuntime::storeAOTHeader(TR_FrontEnd *fe, J9VMThread *curThread)
{
    ...
    const void *store
        = javaVM()->sharedClassConfig->storeSharedData(curThread, aotHeaderKey, aotHeaderKeyLength, &dataDescriptor);
    if (store) {
...
void TR_RelocationRuntime::addClazzRecord(uint8_t *ia, uint32_t bcIndex, TR_OpaqueMethodBlock *method)
{
    if (getPersistentInfo()->isRuntimeInstrumentationEnabled() && _isLoading) {
        comp()->addHWPBCMap(_compInfo->getHWProfiler()->createBCMap(ia, bcIndex, method, comp()));
    }
}
...
#if defined(J9VM_OPT_JITSERVER)
void TR_JITServerRelocationRuntime::initializeCacheDeltas()
{
    _dataCacheDelta = 0;
    _codeCacheDelta = 0;
}

U_8 *TR_JITServerRelocationRuntime::allocateSpaceInCodeCache(UDATA codeSize)
{
    TR_J9VMBase *fej9 = (TR_J9VMBase *)_fe;
    TR::CodeCacheManager *manager = TR::CodeCacheManager::instance();
...
uint8_t *TR_JITServerRelocationRuntime::allocateSpaceInDataCache(uintptr_t metaDataSize, uintptr_t type)
{
    _metaDataAllocSize = TR_DataCacheManager::alignToMachineWord(metaDataSize);
    U_8 *newDataStart = TR_DataCacheManager::getManager()->allocateDataCacheRecord(_metaDataAllocSize, type, 0);
    if (newDataStart)
...
uint8_t *TR_JITServerRelocationRuntime::copyDataToCodeCache(const void *startAddress, size_t totalSize, TR_J9VMBase *fe,
    TR::CodeCacheKind kind)
{
    ...
    TR::CodeCacheManager *manager = TR::CodeCacheManager::instance();
    TR_ASSERT(!compInfoPT->getCompilation()->cg()->getCodeCache(),
        "No code caches should be reserved when copying a thunk");
 ...
 * @return void
 */
void printAOTHeaderProcessorFeatures(TR_AOTHeader *hdrInCache, char *buff, const size_t BUFF_SIZE)
{
