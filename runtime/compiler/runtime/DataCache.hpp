...
protected:
    template<class T> class InPlaceList {
    public:
        struct ListElement {
        public:
            ListElement *_prev;
    ...
    public:
        ...
        public:
            ...
            ListElement *currentElement() { return _currentElement; }
        };
        ...
        bool empty() { return _sentinel._next == &_sentinel; }
    ...
    public:
        ...
        bool isEmpty() { return _allocations.empty(); }
...
private:
    ...
    int disclaimSegment(J9MemorySegment *seg, bool canDisclaimOnSwap,
        bool canDisclaimOnFile); // disclaim memory used for the indicated segment
...
protected:
    TR_DataCacheManager(J9JITConfig *jitConfig, TR::Monitor *monitor, uint32_t quantumSize, uint32_t minQuanta,
        bool newImplementation = true, bool worstFit = false);
    ...
    template<typename T> static T roundToMultiple(T value, T multiple)
    {
...
#endif
    static uint32_t alignToMachineWord(uint32_t size)
    {
        return (size + (uint32_t)ALIGNMENT_DCM) & (~((uint32_t)ALIGNMENT_DCM));
    }

    template<typename T>
    static TR_DataCacheManager *constructManager(J9JITConfig *jitConfig, TR::Monitor *monitor, uint32_t quantumSize,
        uint32_t minQuanta, bool newImplementation);
};
...
public:
    TR_InstrumentedDataCacheManager(J9JITConfig *jitConfig, TR::Monitor *monitor, uint32_t quantumSize,
        uint32_t minQuanta, bool newImplementation = true, bool worstFit = false);
    virtual void printStatistics();
