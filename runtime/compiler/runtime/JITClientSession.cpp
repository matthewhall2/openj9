...
ClientSessionData::ClientSessionData(uint64_t clientUID, uint32_t seqNo, TR_PersistentMemory *persistentMemory,
    bool usesPerClientMemory)
    : _clientUID(clientUID)
    , _maxReceivedSeqNo(seqNo)
    , _lastProcessedCriticalSeqNo(seqNo)
    , _persistentMemory(persistentMemory)
    , _usesPerClientMemory(usesPerClientMemory)
    , _OOSequenceEntryList(NULL)
    , _chTable(NULL)
    ...
    , _constantPoolToClassMap(
          decltype(_constantPoolToClassMap)::allocator_type(persistentMemory->_persistentAllocator.get()))
    ...
    , _rtResolve(false)
    , _registeredJ2IThunksMap(
          decltype(_registeredJ2IThunksMap)::allocator_type(persistentMemory->_persistentAllocator.get()))
    , _registeredInvokeExactJ2IThunksSet(
          decltype(_registeredInvokeExactJ2IThunksSet)::allocator_type(persistentMemory->_persistentAllocator.get()))
...
void ClientSessionData::destroyMonitors()
{
...
void ClientSessionData::updateTimeOfLastAccess()
{
...
void ClientSessionData::initializeUnloadedClassAddrRanges(const std::vector<TR_AddressRange> &unloadedClassRanges,
    int32_t maxRanges)
{
...
void ClientSessionData::processUnloadedClasses(const std::vector<TR_OpaqueClassBlock *> &classes,
    bool updateUnloadedClasses)
{
    ...
    if (TR::Options::getVerboseOption(TR_VerboseJITServer))
        TR_VerboseLog::writeLineLocked(TR_Vlog_JITServer,
            "compThreadID=%d will process a list of %zu unloaded classes for clientUID %llu", compThreadID,
            numOfUnloadedClasses, (unsigned long long)_clientUID);

    Trc_JITServerUnloadClasses(TR::compInfoPT->getCompilationThread(), compThreadID, this,
        (unsigned long long)_clientUID, (unsigned long long)numOfUnloadedClasses);
    ...
    ClientSessionData::VMInfo *vminfo
        = compInfoPT->getClientData()->getOrCacheVMInfo(compInfoPT->getMethodBeingCompiled()->_stream);
...
void ClientSessionData::processIllegalFinalFieldModificationList(const std::vector<TR_OpaqueClassBlock *> &classes)
{
    ...
    if (TR::Options::getVerboseOption(TR_VerboseJITServer))
        TR_VerboseLog::writeLineLocked(TR_Vlog_JITServer,
            "compThreadID=%d will process a list of %zu classes with illegal final field modification for clientUID "
            "%llu",
            compThreadID, numOfClasses, (unsigned long long)_clientUID);
    {
        ...
        for (auto clazz : classes) {
            ...
            if (it != _romClassMap.end()) {
                ...
                if (TR::Options::getVerboseOption(TR_VerboseJITServer))
                    TR_VerboseLog::writeLineLocked(TR_Vlog_JITServer,
                        "compThreadID=%d found clazz %p in the cache and updated bit "
                        "J9ClassHasIllegalFinalFieldModifications to 1",
                        compThreadID, clazz);
            }
        }
    }
...
void ClientSessionData::dumpAllBytecodeProfilingData()
{
        ...
        for (const auto &entry : getJ9MethodMap()) {
            ...
            if (iProfilerMap) {
                ...
                fprintf(logFile, "Method: %.*s.%.*s%.*s\n", J9UTF8_LENGTH(className), (char *)J9UTF8_DATA(className),
                    J9UTF8_LENGTH(name), (char *)J9UTF8_DATA(name), J9UTF8_LENGTH(signature),
                ...
                for (const auto &bcEntry : *iProfilerMap) {
                    ...
                    if (branchData) {
                        fprintf(logFile,
                            "\tOffset %" OMR_PRIu32 "\tbranch samples=%" OMR_PRIu32 "\t taken=%" OMR_PRIu32 "\n",
                    ...
                    for (int j = 0; j < NUM_CS_SLOTS; j++) {
                        if (cgData->getClazz(j)) {
                            J9ROMClass *romClass
                                = JITServerHelpers::getRemoteROMClassIfCached(this, (J9Class *)cgData->getClazz(j));
                            int32_t len = 0;
                            const char *s = NULL;
                            if (romClass) {
 ...
 */
void ClientSessionData::printIProfilerCacheStats()
{
    ...
    TR_VerboseLog::writeLineLocked(TR_Vlog_JITServer,
        "Client %" OMR_PRIu64 " ProfileCache stats: numMethodsCached=%zu, bytecodesProfiled=%zu samples=%zu",
 ...
 */
TR_IPBytecodeHashTableEntry *ClientSessionData::getCachedIProfilerInfo(TR_OpaqueMethodBlock *method,
    uint32_t byteCodeIndex, bool *methodInfoPresent)
{
    *methodInfoPresent = false;
    ...
    if (it != j9methodMap.end()) {
        ...
        TR_ASSERT(false, "profilingSample: asking about j9method=%p but this is not present in the J9MethodMap",
            method);
    }
    return ipEntry;
}
 ...
 */
bool ClientSessionData::cacheIProfilerInfo(TR_OpaqueMethodBlock *method, uint32_t byteCodeIndex,
    TR_IPBytecodeHashTableEntry *entry, bool isCompiled)
{
    OMR::CriticalSection getRemoteROMClass(getROMMapMonitor());
 ...
 */
bool ClientSessionData::cacheIProfilerInfo(TR_OpaqueMethodBlock *method,
    const Vector<TR_IPBytecodeHashTableEntry *> &entries, bool isCompiled)
{
    OMR::CriticalSection getRemoteROMClass(getROMMapMonitor());
    // check whether info about j9method exists
    auto &j9methodMap = getJ9MethodMap();
        ...
        if (!iProfilerMap) {
            ...
            if (iProfilerMap) {
                ...
                for (auto &entry : entries) {
                    ...
                    // Note: It's possible that we run out of persistent memory and cache only part of the entries for
                    // this method. This is unlikely and only cause a small performance regression if anything (this
...
void ClientSessionData::checkProfileDataMatching(J9Method *method, const std::string &ipdata)
{
    ...
    TR_ASSERT_FATAL(iProfilerMap,
        "There must be some IP data cached because we just loaded from shared profile repository");
 ...
 */
ProfiledMethodEntry *ClientSessionData::getSharedProfileCacheForMethod(J9Method *method)
{
 ...
 */
BytecodeProfileSummary ClientSessionData::getSharedBytecodeProfileSummary(J9Method *method)
{
    // Convert from J9method to AOTCacheMethodRecord.
    const AOTCacheMethodRecord *methodRecord = NULL;
    {
 ...
 */
bool ClientSessionData::loadBytecodeDataFromSharedProfileCache(J9Method *method, bool stable, TR::Compilation *comp,
    const std::string &ipdata)
{
    ...
    // Note: 'newEntries' was populated with pointers to newly allocated entries (using persistent memory or heap
    // memory). Thus, if something prevents us from storing this data in the per-client profile cache, we must free it.
    try {
        if (TR::Options::getVerboseOption(TR_VerboseJITServerSharedProfileDetails))
            TR_VerboseLog::writeLineLocked(TR_Vlog_JITServer,
                "\tHave cloned entries. Fix ramClasses for numCgEntries=%zu", cgEntries.size());
            ...
            for (uint32_t i = 0; i < cgEntries.size(); ++i) {
                ...
                for (uint32_t j = 0; j < NUM_CS_SLOTS; ++j) {
                    if (auto record = (const AOTCacheClassRecord *)csInfo->getClazz(j)) {
                        if (TR::Options::getVerboseOption(TR_VerboseJITServerSharedProfileDetails))
                            TR_VerboseLog::writeLineLocked(TR_Vlog_JITServer,
                                "\tLooking at entry %p entryIndex %" OMR_PRIu32 " slot %" OMR_PRIu32
                                " classRecord %p classLoaderRecord %p",
                        ...
                        if (c_it != _classRecordMap.end()) {
                            ...
                            if (TR::Options::getVerboseOption(TR_VerboseJITServerSharedProfileDetails))
                                TR_VerboseLog::writeLineLocked(TR_Vlog_JITServer,
                                    "\t\tFound classRecord in _classRecordMap. j9class=%p", c_it->second);
                            continue;
                        }

                        // We could not find the <AOTCacheClassRecord, j9class> association
                        // Memorize this fact to ask the client later.
                        // TODO: maybe we should limit ourselves to just the dominant entry, or maybe the client should
                        // only send us the dominant entry (based on some option)
                        if (TR::Options::getVerboseOption(TR_VerboseJITServerSharedProfileDetails))
                            TR_VerboseLog::writeLineLocked(TR_Vlog_JITServer,
                                "\tCould not find the j9clazz so we have to ask the client later");

                        // Update the vector of unique uncached class records.
                        // Do a linear search since the size of the vector is typically 1-4 entries (most common case is
                        // 1).
                        bool found = false;
                        ...
                        if (!found) // Unseen record so far
                        {
                            ...
                            TR_ASSERT_FATAL(uniqueUncachedClassRecords.size() == uncachedIndexes.size(),
                                "The size of the uniqueUncachedRecords and uncachedIndexes vectors must be equal");
                        }
                    }
        ...
        if (uniqueUncachedClassRecords.size()) {
            ...
            recordsToSend.reserve(uniqueUncachedClassRecords.size()
                * 2); // *2 because we may send both the class loader records and the class records
            ...
            // Pack the serialization records that we gathered in 'recordsToSend' into a format that can be sent over
            // the network.
            ...
            if (TR::Options::getVerboseOption(TR_VerboseJITServerSharedProfileDetails))
                TR_VerboseLog::writeLineLocked(TR_Vlog_JITServer,
                    "\tcomp=%p Will send message to client with %zu serialized records asking for %zu classes", comp,
                    recordsToSend.size(), classIds.size());
            ...
            if (TR::Options::getVerboseOption(TR_VerboseJITServerSharedProfileDetails))
                TR_VerboseLog::writeLineLocked(TR_Vlog_JITServer,
                    "\tcomp=%p Have received response from client for %zu ramClasses", comp, ramClasses.size());
            ...
            if (ramClasses.empty()) // Possible if the deserializer was reset
            {
                if (TR::Options::getVerboseOption(TR_VerboseJITServerSharedProfile))
                    TR_VerboseLog::writeLineLocked(TR_Vlog_JITServer,
                        ...
                        "while compiling %s for clientUID %llu",
                        method, methodRecord->data().id(), comp->signature(), (unsigned long long)_clientUID);
            ...
            TR_ASSERT_FATAL(ramClasses.size() == uniqueUncachedClassRecords.size(), "Invalid vector length %zu != %zu",
                ramClasses.size(), uniqueUncachedClassRecords.size());
            // Update the set of IDs that the client knows about, but consider only the classes that were successfully
            // identified.
            {
                ...
                for (size_t i = 0; i < uniqueUncachedClassRecords.size(); ++i) {
                    if (ramClasses[i]) {
                        ...
                        _aotCacheKnownIds.insert(
                            uniqueUncachedClassRecords[i]->classLoaderRecord()->data().idAndType());
                    }
                }
            } // end critical section
            ...
            for (size_t i = 0; i < uniqueUncachedClassRecords.size(); ++i) {
                ...
                for (const auto &placeToPatch : placesToPatch) {
                    ...
                    if (TR::Options::getVerboseOption(TR_VerboseJITServerSharedProfileDetails))
                        TR_VerboseLog::writeLineLocked(TR_Vlog_JITServer,
                            "\t\tWill patch entry %p index %" OMR_PRIu32 " slot %" OMR_PRIu32 " with %p",
                    ...
                    if (!ramClasses[i]) {
                        ...
                        if (TR::Options::getVerboseOption(TR_VerboseJITServerSharedProfile)) {
                            TR_VerboseLog::writeLineLocked(TR_Vlog_JITServer,
                                "Shared Profile ERROR: Failed to get RAMClass for class %.*s ID %zu for profiled "
                                "method %p ID %" OMR_PRIuPTR " CG slot index %" OMR_PRIu32
                                " weight %u relWeight %.0f%% dominant=%d residue=%u while compiling %s",
                                RECORD_NAME(&classRecord->data()), classRecord->data().id(), method,
                                methodRecord->data().id(), slotIndex, (unsigned)csInfo->_weight[slotIndex],
                                csInfo->_weight[slotIndex] * 100.0 / cgEntries[cgEntryIndex]->getSumCount(),
                        ...
                        if (isDominantSlot) {
                            ...
                            // need to do some changes: move its samples to the other category, though I don't know if
                            // we support entries with just one class and the rest being into the other category. There
                            // might be some implicit assumptions that the 3 slots must be filled before having samples
                            // in the "other" category. Must check all the IProfiler code. If this dominant slot uses
                            // 100% of the samples, then we should use profiling info from the client. Note: Most of the
                            // methods that fall into this category have "lambda" or "proxy" in their name.
                        }
                    }
                }
            }
        ...
        if (!ipdata.empty() && !cgEntries.empty()
            && TR::Options::getVerboseOption(TR_VerboseJITServerSharedProfileDetails)) {
            Vector<TR_IPBytecodeHashTableEntry *> clientCgEntries(region);
            JITServerIProfiler::deserializeIProfilerData(method, ipdata, clientCgEntries, comp->trMemory(),
                /*cgEntriesOnly=*/true);

            TR_VerboseLog::writeLineLocked(TR_Vlog_JITServer,
                "\tWill validate %zu cgEntries aganist %zu client entries", cgEntries.size(), clientCgEntries.size());
                    ...
                    if (cgEntry->getPC() == clientCgEntry->getPC()) {
                        ...
                        for (int slot = 0; slot < NUM_CS_SLOTS; ++slot) {
                            ...
                            if (serverClazz && clientClazz && serverClazz != clientClazz) {
                                ...
                                TR_VerboseLog::writeLineLocked(TR_Vlog_JITServer,
                                    "Discrepancy for jmethod %p slot %d serverClazz=%p clientClazz=%p samples=%u "
                                    "residue=%u",
                                    method, slot, serverClazz, clientClazz, (unsigned)csInfoClient->_weight[slot],
                                    (unsigned)csInfoClient->_residueWeight);
                            }
                        }
                        break;
                    }
    ...
    if (stable) {
        ...
        if (!success) {
            ...
            if (TR::Options::getVerboseOption(TR_VerboseJITServerSharedProfileDetails))
                TR_VerboseLog::writeLineLocked(TR_Vlog_JITServer,
                    "\tWill delete the newEntries allocated with persistent memory because another thread was faster "
                    "than us");
            for (auto &profilingEntry : newEntries) {
                trMemory.freeMemory(profilingEntry, persistentAlloc);
            }
    ...
    if (success) {
        ...
        if (TR::Options::getVerboseOption(TR_VerboseJITServerSharedProfile)) {
            ...
            TR_VerboseLog::write(TR_Vlog_JITServer,
                "Loaded %zu bytecode entries from shared profile repository for method %p ", newEntries.size(), method);
            TR::CompilationInfo::printMethodNameToVlog(methodInfo->definingROMClass(), methodInfo->_romMethod);
            TR_VerboseLog::writeLine(" ROMMethod %p ROMClass=%p", methodInfo->_romMethod,
                methodInfo->definingROMClass());
        }
    } else {
        _numSharedProfileCacheMethodLoadsFailed++;
    }
...
/**
 * @brief Take the serialized IP data sent by the client for the indicated method and store it in the shared profile
 * cache.
 *
 ...
 */
bool ClientSessionData::storeBytecodeProfileInSharedRepository(TR_OpaqueMethodBlock *method, const std::string &ipData,
    uint64_t numSamples, bool isStable, TR::Compilation *comp)
{
    ...
    if (TR::Options::getVerboseOption(TR_VerboseJITServerSharedProfileDetails))
        TR_VerboseLog::writeLineLocked(TR_Vlog_JITServer,
            "Storing profile to shared repository for method %p. %zu non-cgEntries and %zu cgEntries", method,
            entries.size(), cgEntries.size());

    // If there are CallGraph entries, we must replace the J9class pointers in these entries with AOT cache class
    // records.
    if (!cgEntries.empty()) {
        struct CGEntryMetadata {
            J9Class *_ramClass;
            Vector<std::pair<uint32_t, uint32_t> > _entriesToPatch; // Fields: (1) index of the entry in cgEntries, (2)
                                                                    // index of the slot in the CGData structure
        ...
        if (!uniqueRAMClasses.empty() && TR::Options::getVerboseOption(TR_VerboseJITServerSharedProfileDetails))
            TR_VerboseLog::writeLineLocked(TR_Vlog_JITServer,
                "\tStoring profile: detected %zu unique classes that needs to be converted", uniqueRAMClasses.size());

        // Convert RAM classes from uniqueRAMClasses into class records and put those class records in 'classRecords'
        // vector
                ...
                if (classRecord) {
                    ...
                    if (TR::Options::getVerboseOption(TR_VerboseJITServerSharedProfile))
                        TR_VerboseLog::writeLineLocked(TR_Vlog_JITServer,
                            ...
                            "ID %zu while compiling %s for clientUID %llu",
                            ramClass, method, methodRecord->data().id(), comp->signature(),
        ...
        if (!uncachedRAMClasses.empty()) {
            if (TR::Options::getVerboseOption(TR_VerboseJITServerSharedProfileDetails))
                TR_VerboseLog::writeLineLocked(TR_Vlog_JITServer,
                    "\tSending AOTCache_getROMClassBatch message for %zu classes", uncachedRAMClasses.size());
            ...
            // Get class records for newly cached classes
            {
                ...
                for (auto &idx : uncachedClassIndexes) {
                    ...
                    J9Class *uncachedBaseComponent = NULL;
                    auto classRecord
                        = getClassRecord(ramClass, missingLoaderInfo, uncachedClass, uncachedBaseComponent);
                    if (classRecord) {
                        uniqueRAMClasses[idx]._classRecord = classRecord;
                    } else {
                        TR_ASSERT_FATAL(!uncachedClass && !uncachedBaseComponent, "Class %p must be already cached",
                            ramClass);
    ...
    if (_sharedProfileCache->addBytecodeData(entries, methodRecord, methodInfo->_romMethod,
            methodInfo->definingROMClass(), numSamples, isStable)) {
        _numSharedProfileCacheMethodStores++;
        if (TR::Options::getVerboseOption(TR_VerboseJITServerSharedProfile)) {
            TR_VerboseLog::CriticalSection vlogLock;
            TR_VerboseLog::write(TR_Vlog_JITServer,
                "Stored %zu bytecode entries (%" OMR_PRIu64 " samples) into shared profiling repo for method %p ",
        ...
        if (TR::Options::getVerboseOption(TR_VerboseJITServerSharedProfile)) {
            TR_VerboseLog::CriticalSection vlogLock;
            TR_VerboseLog::write(TR_Vlog_JITServer,
                "WARNING: Failed to store profiling info into the shared repo for method %p ", method);
            TR::CompilationInfo::printMethodNameToVlog(methodInfo->definingROMClass(), methodInfo->_romMethod);
            TR_VerboseLog::writeLine(" ROMMethod %p ROMClass=%p", methodInfo->_romMethod,
                methodInfo->definingROMClass());
        }
 ...
 * @note Acquires/releases ROMMapMonitor and shared profile repository monitor
 */
TR_FaninSummaryInfo *ClientSessionData::loadFaninDataFromSharedProfileCache(TR_OpaqueMethodBlock *method,
    TR_Memory *trMemory)
{
    ...
    TR_ASSERT_FATAL(methodInfo,
        "We cannot obtain a methodRecord without a methodInfo"); // TODO change this to not fatal
    ...
    if (!faninMethodEntry) {
        ...
        if (TR::Options::getVerboseOption(TR_VerboseJITServerSharedProfile)) {
            TR_VerboseLog::CriticalSection vlogLock;
            TR_VerboseLog::write(TR_Vlog_JITServer,
                "WARNING: Failed to load fanin info from shared profile repository for method %p ", method);
            TR::CompilationInfo::printMethodNameToVlog(methodInfo->definingROMClass(), methodInfo->_romMethod);
            TR_VerboseLog::writeLine(" ROMMethod %p ROMClass=%p", methodInfo->_romMethod,
                methodInfo->definingROMClass());
        }
        ...
        if (TR::Options::getVerboseOption(TR_VerboseJITServerSharedProfile)) {
            TR_VerboseLog::CriticalSection vlogLock;
            TR_VerboseLog::write(TR_Vlog_JITServer,
                "Loaded fanin info (%" OMR_PRIu64 " samples, %" OMR_PRIu32
                "  callers) from shared profile repository for method %p ",
...
/**
 * @brief Take the serialized fanin data sent by the client for the indicated method and store it in the shared profile
 * cache.
 *
 * @param method j9method for which profiling information is stored
 ...
 * @note Acquires/releases ROMMapMonitor and shared profile repository monitor
 */
bool ClientSessionData::storeFaninDataInSharedProfileCache(TR_OpaqueMethodBlock *method,
    const TR_ContiguousIPMethodHashTableEntry *serialEntry)
{
    ...
    TR_ASSERT_FATAL(methodInfo,
        "We cannot obtain a methodRecord without a methodInfo"); // TODO change this to not fatal
    // Note: methodInfo should still be valid because classUnloding cannot happen without us being informed.
    bool success = _sharedProfileCache->addFaninData(serialEntry, methodRecord, methodInfo->_romMethod,
        methodInfo->definingROMClass());
    if (success) {
        ...
        if (TR::Options::getVerboseOption(TR_VerboseJITServerSharedProfile)) {
            TR_VerboseLog::CriticalSection vlogLock;
            TR_VerboseLog::write(TR_Vlog_JITServer,
                "Stored fanin info (%" OMR_PRIu64 " samples, %" OMR_PRIu32
                " callers) to shared profile repository for method %p ",
        ...
        if (TR::Options::getVerboseOption(TR_VerboseJITServerSharedProfile)) {
            TR_VerboseLog::CriticalSection vlogLock;
            TR_VerboseLog::write(TR_Vlog_JITServer,
                "WARNING: Failed to store fanin info to shared profile repository for method %p ", method);
            TR::CompilationInfo::printMethodNameToVlog(methodInfo->definingROMClass(), methodInfo->_romMethod);
            TR_VerboseLog::writeLine(" ROMMethod %p ROMClass=%p", methodInfo->_romMethod,
                methodInfo->definingROMClass());
        }
...
void ClientSessionData::printStats()
{
    PORT_ACCESS_FROM_PORT(TR::Compiler->portLib);
...
ClientSessionData::ClassInfo::ClassInfo(TR_PersistentMemory *persistentMemory)
    ...
    , _constantClassPoolCache(
          decltype(_constantClassPoolCache)::allocator_type(persistentMemory->_persistentAllocator.get()))
    , _fieldAttributesCache(
          decltype(_fieldAttributesCache)::allocator_type(persistentMemory->_persistentAllocator.get()))
    , _staticAttributesCache(
          decltype(_staticAttributesCache)::allocator_type(persistentMemory->_persistentAllocator.get()))
    , _fieldAttributesCacheAOT(
          decltype(_fieldAttributesCacheAOT)::allocator_type(persistentMemory->_persistentAllocator.get()))
    , _staticAttributesCacheAOT(
          decltype(_fieldAttributesCacheAOT)::allocator_type(persistentMemory->_persistentAllocator.get()))
    ...
    , _fieldOrStaticDeclaringClassCache(
          decltype(_fieldOrStaticDeclaringClassCache)::allocator_type(persistentMemory->_persistentAllocator.get()))
    , _fieldOrStaticDefiningClassCache(
    ...
    , _referenceSlotsInClass(
          decltype(_referenceSlotsInClass)::allocator_type(persistentMemory->_persistentAllocator.get()))
{}

void ClientSessionData::ClassInfo::freeClassInfo(TR_PersistentMemory *persistentMemory)
{
...
ClientSessionData::VMInfo *ClientSessionData::getOrCacheVMInfo(JITServer::ServerStream *stream)
{
    if (!_vmInfo) {
...
J9SharedClassCacheDescriptor *ClientSessionData::reconstructJ9SharedClassCacheDescriptorList(
    const std::vector<ClientSessionData::CacheDescriptor> &listOfCacheDescriptors)
{
...
void ClientSessionData::destroyJ9SharedClassCacheDescriptorList()
{
    ...
    _vmInfo->_j9SharedClassCacheDescriptorList->previous->next
        = NULL; // break the circular links by setting tail's next pointer to be NULL
    while (cur) {
        next = cur->next;
...
void ClientSessionData::clearCaches(bool locked)
{
...
void ClientSessionData::clearCachesLocked(TR_J9VMBase *fe)
{
...
void ClientSessionData::destroy(ClientSessionData *clientSession)
{
...
TR_MethodToBeCompiled *ClientSessionData::notifyAndDetachFirstWaitingThread()
{
...
TR_PersistentCHTable *ClientSessionData::getCHTable()
{
    if (!_chTable) {
...
void ClientSessionData::purgeCache(const std::vector<ClassUnloadedData> &unloadedClasses, map &m,
    const key ClassUnloadedData::*k)
{
    for (auto &data : unloadedClasses) {
        if (data._cached) {
            m.erase((data.*k));
        } else {
            // If the class is not cached, this is the place to iterate the cache(Map) for deleting the entry by value
            // rather than key.
...
void ClientSessionData::readAcquireClassUnloadRWMutex(TR::CompilationInfoPerThreadBase *compInfoPT)
{
...
void ClientSessionData::readReleaseClassUnloadRWMutex(TR::CompilationInfoPerThreadBase *compInfoPT)
{
    // compInfoPT must be associated with the compilation thread that calls this
...
void ClientSessionData::writeAcquireClassUnloadRWMutex()
{
    _bClassUnloadingAttempt = true;
    omrthread_rwmutex_enter_write(_classUnloadRWMutex);
}

void ClientSessionData::writeReleaseClassUnloadRWMutex()
{
    _bClassUnloadingAttempt = false;
    omrthread_rwmutex_exit_write(_classUnloadRWMutex);
}

void ClientSessionData::getPermanentLoaders(TR::vector<J9ClassLoader *, TR::Region &> &dest, size_t n,
    JITServer::ServerStream *stream)
{
    ...
    stream->write(JITServer::MessageType::ClientSessionData_getPermanentLoaders, serverCount);
    ...
    for (size_t i = 0; i < newPermanentLoaders.size(); i++) {
        if (serverCount + i < _permanentLoaders.size()) {
            TR_ASSERT_FATAL(newPermanentLoaders[i] == _permanentLoaders[serverCount + i],
                "client permanent loader mismatch at index %zu: %p vs. %p", serverCount + i, newPermanentLoaders[i],
                _permanentLoaders[serverCount + i]);
        } else {
            TR_ASSERT_FATAL(serverCount + i == _permanentLoaders.size(), "skipped past the end of _permanentLoaders");

            _permanentLoaders.push_back(newPermanentLoaders[i]);
        }
    }

    TR_ASSERT_FATAL(_permanentLoaders.size() >= n, "too few permanent loaders: have %zu but need %zu",
...
const void *ClientSessionData::getCachedWellKnownClassChainOffsets(unsigned int includedClasses, size_t numClasses,
    const uintptr_t *classChainOffsets, const AOTCacheWellKnownClassesRecord *&wellKnownClassesRecord)
{
    ...
    if (_wellKnownClasses._includedClasses == includedClasses
        && memcmp(_wellKnownClasses._classChainOffsets, classChainOffsets, numClasses * sizeof(classChainOffsets[0]))
            == 0) {
        TR_ASSERT(_wellKnownClasses._wellKnownClassChainOffsets,
            "Cached well-known class chain offsets pointer is NULL");
...
void ClientSessionData::cacheWellKnownClassChainOffsets(unsigned int includedClasses, size_t numClasses,
    ...
    const AOTCacheWellKnownClassesRecord *&wellKnownClassesRecord)
{
    ...
    memcpy(_wellKnownClasses._classChainOffsets, classChainOffsets, numClasses * sizeof(classChainOffsets[0]));
    ...
    wellKnownClassesRecord = classChainRecords
        ? _aotCache->getWellKnownClassesRecord(classChainRecords, numClasses, includedClasses)
        : NULL;
    _wellKnownClasses._aotCacheWellKnownClassesRecord = wellKnownClassesRecord;
}

JITServerAOTCache *ClientSessionData::getOrCreateAOTCache(JITServer::ServerStream *stream)
{
    if (!_vmInfo)
        ...
        if (auto aotCacheMap = TR::CompilationInfo::get()->getJITServerAOTCacheMap()) {
            ...
            if (!aotCache) {
                if (cacheIsBeingLoadedFromDisk) {
                    ...
                    if (TR::Options::getVerboseOption(TR_VerboseJITServer))
                        TR_VerboseLog::writeLineLocked(TR_Vlog_JITServer,
                            "clientUID=%llu requested AOT cache but currently that cache is being loaded from disk",
                            (unsigned long long)_clientUID);
                } else // We cannot create the cache because memory limit is reached
                {
                    ...
                    if (TR::Options::getVerboseOption(TR_VerboseJITServer))
                        TR_VerboseLog::writeLineLocked(TR_Vlog_JITServer,
                            "clientUID=%llu requested AOT cache but the AOT cache size limit has been reached, "
                            "disabling AOT cache",
                            (unsigned long long)_clientUID);
                }
                return NULL;
            ...
            if (record) {
                ...
                if (TR::Options::getVerboseOption(TR_VerboseJITServer))
                    TR_VerboseLog::writeLineLocked(TR_Vlog_JITServer,
                        "clientUID=%llu failed to create AOT header record due to AOT cache size limit, disabling AOT "
                        "cache",
                        (unsigned long long)_clientUID);
            }
        } else {
            ...
            if (TR::Options::getVerboseOption(TR_VerboseJITServer))
                TR_VerboseLog::writeLineLocked(TR_Vlog_JITServer,
                    "clientUID=%llu requested AOT cache while it is disabled at the server",
                    (unsigned long long)_clientUID);
        }
    }
...
const AOTCacheClassRecord *ClientSessionData::getClassRecord(ClassInfo &classInfo, bool &missingLoaderInfo,
...
const AOTCacheClassRecord *ClientSessionData::getClassRecord(J9Class *clazz, bool &missingLoaderInfo,
    bool &uncachedClass, J9Class *&uncachedBaseComponent, J9::J9SegmentProvider *scratchSegmentProvider)
{
    TR_ASSERT(getROMMapMonitor()->owned_by_self(), "Must hold ROMMapMonitor");
...
const AOTCacheClassRecord *ClientSessionData::getClassRecord(J9Class *clazz, JITServer::ServerStream *stream,
    bool &missingLoaderInfo, J9::J9SegmentProvider *scratchSegmentProvider)
{
    ...
    if (uncachedBaseComponent) {
        ...
        JITServerHelpers::ClassInfoTuple classInfoTuple;
        auto romClass
            = JITServerHelpers::getRemoteROMClass(uncachedBaseComponent, stream, _persistentMemory, classInfoTuple);
        JITServerHelpers::cacheRemoteROMClassOrFreeIt(this, uncachedBaseComponent, romClass, classInfoTuple);

        OMR::CriticalSection cs(getROMMapMonitor());
        record = getClassRecord(clazz, missingLoaderInfo, uncachedClass, uncachedBaseComponent, scratchSegmentProvider);
        TR_ASSERT(!uncachedClass && !uncachedBaseComponent, "Class %p and base component must be already cached",
            clazz);
    }
    ...
    if (missingLoaderInfo) {
        ...
        if (!name.empty()) {
            ...
            TR_ASSERT(!missingLoaderInfo,
                "Class %p cannot have missing loader info as we've just received it from the client", clazz);
...
const AOTCacheMethodRecord *ClientSessionData::getMethodRecord(J9MethodInfo &methodInfo, J9Method *ramMethod)
{
    ...
    if (!methodInfo._aotCacheMethodRecord) {
        ...
        TR_ASSERT_FATAL(!uncachedBaseComponent, "Method %p defined by array class %p", ramMethod,
            methodInfo.definingClass());
        if (!classRecord)
            return NULL;
        // Then create a method record and cache it in J9MethodInfo
        methodInfo._aotCacheMethodRecord
            = _aotCache->getMethodRecord(classRecord, methodInfo._index, methodInfo._romMethod);
    }

    return methodInfo._aotCacheMethodRecord;
}

const AOTCacheMethodRecord *ClientSessionData::getMethodRecord(J9Method *ramMethod, const J9MethodInfo **methodInfo)
{
    OMR::CriticalSection cs(getROMMapMonitor());
...
const AOTCacheMethodRecord *ClientSessionData::getMethodRecord(J9Method *method, J9Class *definingClass,
    JITServer::ServerStream *stream)
{
    {
        OMR::CriticalSection cs(getROMMapMonitor());
    ...
    it->second._aotCacheMethodRecord
        = _aotCache->getMethodRecord(classRecord, it->second._index, it->second._romMethod);
    return it->second._aotCacheMethodRecord;
}

const AOTCacheClassChainRecord *ClientSessionData::getClassChainRecord(J9Class *clazz, uintptr_t classChainOffset,
    const std::vector<J9Class *> &ramClassChain, JITServer::ServerStream *stream, bool &missingLoaderInfo,
    J9::J9SegmentProvider *scratchSegmentProvider)
{
        ...
        for (size_t i = 0; i < ramClassChain.size(); ++i) {
            ...
            if (!classRecords[i]) {
                if (uncachedClass) {
                    ...
                    // The corner case of missing both the base component class and the class loader info is still
                    // handled correctly by requesting the class loader info in the getClassRecord(ramClassChain[0],
                    // ...) call below.
    ...
    if (!uncachedRAMClasses.empty()) {
        ...
        auto &classInfoTuples = std::get<0>(recv);
        auto &uncachedBases = std::get<1>(recv);
        TR_ASSERT_FATAL(uncachedBases.empty(),
            "Client should not have sent any bases because the server asked for them explicitely");
        ...
        OMR::CriticalSection cs(getROMMapMonitor());
        for (size_t i = 0; i < numUncachedClasses; ++i) // This possibly skips the last entry which could be the base
                                                        // component class which was processed separately
        {
    ...
    OMR::CriticalSection cs(getClassChainDataMapMonitor());
    auto result = getClassChainDataMap().insert({
        clazz, { classChainOffset, record }
    });
    if (!result.second)
...
const AOTCacheWellKnownClassesRecord *ClientSessionData::getWellKnownClassesRecord(
    const AOTCacheClassChainRecord * const *chainRecords, size_t length, uintptr_t includedClasses)
{
    return _aotCache->getWellKnownClassesRecord(chainRecords, length, includedClasses);
}

bool ClientSessionData::useServerOffsets(JITServer::ServerStream *stream)
{
 ...
 */
void ClientSessionData::printSharedProfileCacheStats() const
{
    if (useSharedProfileCache() && TR::Options::getVerboseOption(TR_VerboseJITServerSharedProfile)) {
        TR_VerboseLog::writeLineLocked(TR_Vlog_JITServer,
            "Client=%" OMR_PRIu64 " numSharedProfileCacheMethodLoads=%" OMR_PRIu32
            " numSharedProfileCacheMethodLoadsFailed=%" OMR_PRIu32 " numSharedProfileCacheMethodStores=%" OMR_PRIu32
            " numSharedProfileCacheMethodStoresFailed=%" OMR_PRIu32 "",
            getClientUID(), _numSharedProfileCacheMethodLoads, _numSharedProfileCacheMethodLoadsFailed,
            _numSharedProfileCacheMethodStores, _numSharedProfileCacheMethodStoresFailed);
        TR_VerboseLog::writeLineLocked(TR_Vlog_JITServer,
            "Client=%" OMR_PRIu64 " numSharedFaninCacheMethodLoads=%" OMR_PRIu32
            " numSharedFaninCacheMethodLoadsFailed=%" OMR_PRIu32 " numSharedFaninCacheMethodStores=%" OMR_PRIu32
            " numSharedFaninCacheMethodStoresFailed=%" OMR_PRIu32 "",
            getClientUID(), _numSharedFaninCacheMethodLoads, _numSharedFaninCacheMethodLoadsFailed,
            _numSharedFaninCacheMethodStores, _numSharedFaninCacheMethodStoresFailed);
    }
}

ClientSessionHT *ClientSessionHT::allocate()
{
...
ClientSessionHT::ClientSessionHT()
    ...
    , // 90 minutes
    OLD_AGE_UNDER_LOW_MEMORY(TR::Options::_oldAgeUnderLowMemory)
    , // 5 minutes
...
ClientSessionData *ClientSessionHT::findOrCreateClientSession(uint64_t clientUID, uint32_t seqNo,
    bool *newSessionWasCreated, J9JITConfig *jitConfig)
{
            ...
            *newSessionWasCreated = true;
            if (TR::Options::getVerboseOption(TR_VerboseJITServer)
                ...
                TR_VerboseLog::writeLineLocked(TR_Vlog_JITServer,
                    ...
                    (uint32_t)_compInfo->getPersistentInfo()->getElapsedTime(), (unsigned long long)clientUID);
            }
        } else {
            if (TR::Options::getVerboseOption(TR_VerboseJITServer))
                TR_VerboseLog::writeLineLocked(TR_Vlog_JITServer,
                    "ERROR: Server could not allocate client session data");
        }
    }
    return clientData;
}
...
// Must have compilation monitor in hand when calling this function.
bool ClientSessionHT::deleteClientSession(uint64_t clientUID, bool forDeletion)
{
...
ClientSessionData *ClientSessionHT::findClientSession(uint64_t clientUID)
{
    ClientSessionData *clientData = NULL;
    auto clientDataIt = _clientSessionMap.find(clientUID);
    if (clientDataIt != _clientSessionMap.end()) {
...
void ClientSessionHT::purgeOldDataIfNeeded()
{
    PORT_ACCESS_FROM_PORT(TR::Compiler->portLib);
    ...
    if (crtTime - _timeOfLastPurge > TIME_BETWEEN_PURGES) {
        uint64_t freePhysicalMemory
            = _compInfo->computeAndCacheFreePhysicalMemory(incomplete); // check if memory is free
        if (freePhysicalMemory != OMRPORT_MEMINFO_NOT_AVAILABLE && !incomplete) {
            if (freePhysicalMemory
                < TR::Options::getSafeReservePhysicalMemoryValue() + 4 * TR::Options::getScratchSpaceLowerBound()) {
        ...
        for (auto iter = _clientSessionMap.begin(); iter != _clientSessionMap.end(); ++iter) {
            ...
            if (iter->second->getInUse() == 0 && crtTime - iter->second->getTimeOflastAccess() > oldAge) {
                if (TR::Options::getVerboseOption(TR_VerboseJITServer))
                    TR_VerboseLog::writeLineLocked(TR_Vlog_JITServer,
                        "t=%u Server will purge session data for clientUID %llu of age %lld. Number of clients before "
                        "purge: %u",
                        (uint32_t)_compInfo->getPersistentInfo()->getElapsedTime(), (unsigned long long)iter->first,
                        (long long)oldAge, size());
                ClientSessionData::destroy(iter->second); // delete the client data
                _clientSessionMap.erase(iter); // delete the mapping from the hashtable
            }
        }
        // If the purge operation was responsible for deleting the last client sessions, shut down the shared ROM class
        // cache as well
...
void ClientSessionHT::printStats()
{
    PORT_ACCESS_FROM_PORT(TR::Compiler->portLib);
