...
TR_HWProfiler::TR_HWProfiler(J9JITConfig *jitConfig)
    : _hwProfilerOSThread(NULL)
    , _hwProfilerThread(NULL)
    , _hwProfilerThreadAttachAttempted(false)
    , _hwProfilerThreadExitFlag(false)
    , _workingBufferTail(NULL)
    , _currentBufferBeingProcessed(NULL)
    , _numOutstandingBuffers(0)
    , _numRecompilationsInduced(0)
    , _recompilationInterval(TR::Options::_hwprofilerRecompilationInterval)
    , _hwProfilerProcessBufferState(TR::Options::_hwProfilerRIBufferProcessingFrequency)
    , _bufferFilledSum(0)
    , _numBuffersCompletelyFilled(0)
    , _bufferSizeSum(1)
    , _recompDecisionsTotal(0)
    , _recompDecisionsYes(0)
    , _recompDecisionsTotalStart(0)
    , _recompDecisionsYesStart(0)
    , _numDowngradesSinceTurnedOff(0)
    , _qszThresholdToDowngrade(TR::Options::_qszMaxThresholdToRIDowngrade)
    , _expired(false)
    , _numRequests(0)
    , _numRequestsSkipped(0)
    , _totalMemoryUsedByMetadataMapping(0)
    , _jitConfig(jitConfig)
    , _numReducedWarmRecompilationsInduced(0)
...
bool TR_HWProfiler::isHWProfilingAvailable(J9VMThread *vmThread)
{
...
void TR_HWProfiler::setHWProfilingAvailable(bool supported) { _isHWProfilingAvailable = supported; }

int32_t TR_HWProfiler::IAHash(uintptr_t pc) { return (int32_t)(((pc >> 1) & 0x7FFFFFFF) % HASH_TABLE_SIZE); }

uintptr_t TR_HWProfiler::getPCFromMethodAndBCIndex(TR_OpaqueMethodBlock *method, uint32_t byteCodeIndex,
    TR::Compilation *comp)
{
 ...
 */
static int32_t J9THREAD_PROC hwProfilerThreadProc(void *entryarg)
{
    ...
    if (vm->javaOffloadSwitchOffNoEnvWithReasonFunc != 0)
        (*vm->javaOffloadSwitchOffNoEnvWithReasonFunc)(vm, j9thread_self(),
            J9_JNI_OFFLOAD_SWITCH_JIT_HARDWARE_PROFILER_THREAD);
#endif
...
void TR_HWProfiler::startHWProfilerThread(J9JavaVM *javaVM)
{
    ...
    if (_hwProfilerMonitor) {
        ...
        if (javaVM->internalVMFunctions->createThreadWithCategory(&_hwProfilerOSThread,
                TR::Options::_profilerStackSize << 10, priority, 0, &hwProfilerThreadProc, javaVM->jitConfig,
...
void TR_HWProfiler::stopHWProfilerThread(J9JavaVM *javaVM)
{
    PORT_ACCESS_FROM_JAVAVM(javaVM);
    if (!_hwProfilerMonitor)
        return;
...
void TR_HWProfiler::processWorkingQueue()
{
    _hwProfilerMonitor->enter();
    while (true) {
        while (_workingBufferList.isEmpty()) {
        ...
        if (_currentBufferBeingProcessed->getSize() > 0) {
            ...
            if (_currentBufferBeingProcessed->isValid()) {
                processBufferRecords(_hwProfilerThread, _currentBufferBeingProcessed->getBuffer(),
                    _currentBufferBeingProcessed->getSize(), _currentBufferBeingProcessed->getBufferFilledSize(),
...
void TR_HWProfiler::invalidateProfilingBuffers() // called for class unloading
{
...
U_8 *TR_HWProfiler::swapBufferToWorkingQueue(const U_8 *dataStart, UDATA size, UDATA bufferFilledSize, uint32_t dataTag,
    bool allocateNewBuffer)
{
    if (!_hwProfilerMonitor)
        return NULL;
...
bool TR_HWProfiler::checkAndTurnBufferProcessingOff()
{
    if (TR::Options::getCmdLineOptions()->getOption(TR_UseRIOnlyForLargeQSZ)
        ...
        if (TR::Options::isAnyVerboseOptionSet(TR_VerboseHWProfiler)) {
            TR_VerboseLog::writeLineLocked(TR_Vlog_HWPROFILER, "t=%6u RI continue because QSZ is large: %d",
                (uint32_t)_compInfo->getPersistentInfo()->getElapsedTime(), _compInfo->getMethodQueueSize());
        }
    ...
    if (newRecompDecisionsTotal >= TR::Options::_hwProfilerRecompDecisionWindow) {
        ...
        if (TR::Options::_hwProfilerRecompFrequencyThreshold * newRecompDecisionsYes
            < newRecompDecisionsTotal) // avoid float arithmetic
        {
            ...
            if (TR::Options::isAnyVerboseOptionSet(TR_VerboseHWProfiler, TR_VerbosePerformance)) {
                ...
                TR_VerboseLog::writeLineLocked(TR_Vlog_HWPROFILER,
                    "t=%6u RI buffer processing disabled because recomp frequency is %.4f newRecompDecisionsTotal=%llu",
                    (uint32_t)_compInfo->getPersistentInfo()->getElapsedTime(), recompFrequency,
                    newRecompDecisionsTotal);
            }
            return true;
        } else // continue to have RI on; TODO: delete this code
        {
            if (TR::Options::isAnyVerboseOptionSet(TR_VerboseHWProfiler)) {
                float recompFrequency = (float)newRecompDecisionsYes / (float)newRecompDecisionsTotal;
                TR_VerboseLog::writeLineLocked(TR_Vlog_HWPROFILER,
                    "t=%6u RI continue. recomp frequency is %.4f newRecompDecisionsTotal=%llu",
                    (uint32_t)_compInfo->getPersistentInfo()->getElapsedTime(), recompFrequency,
                    newRecompDecisionsTotal);
            }
        }
...
bool TR_HWProfiler::recompilationLogic(TR_PersistentJittedBodyInfo *bodyInfo, void *startPC, uint64_t startCount,
    uint64_t count, uint64_t totalCount, TR_FrontEnd *fe, J9VMThread *vmThread)
{
    ...
    if (bodyInfo->_hwpInducedRecompilation && bodyInfo->_hwpReducedWarmCompileRequested) {
        if (!TR::Options::getCmdLineOptions()->getOption(TR_DisableHardwareProfilerReducedWarmUpgrades)
            && !doHWPReducedWarmCompile && bodyInfo->_hwpReducedWarmCompileInQueue) {
...
uintptr_t TR_HWProfiler::getPCFromBCInfo(TR::Node *node, TR::Compilation *comp)
{
...
TR_OpaqueMethodBlock *TR_HWProfiler::getMethodFromBCInfo(TR_ByteCodeInfo &bcInfo, TR::Compilation *comp)
{
...
TR_HWPBytecodePCToIAMap TR_HWProfiler::createBCMap(uint8_t *ia, uint32_t bcIndex, TR_OpaqueMethodBlock *method,
    TR::Compilation *comp)
{
...
uintptr_t TR_HWProfiler::getBytecodePCFromIA(J9VMThread *vmThread, uint8_t *IA)
{
    if (vmThread) {
        J9JITExceptionTable *metaData = _jitConfig->jitGetExceptionTableFromPC(vmThread, (UDATA)IA);
        if (metaData && metaData->riData
            && ((TR_HWPBytecodePCToIAMap *)metaData->riData)->_bytecodePC
                == reinterpret_cast<void *>(static_cast<intptr_t>(METADATA_MAPPING_EYECATCHER))) {
            ...
            for (uint32_t i = 0; i < arraySize; i++, cursor++) {
                if ((uint8_t *)cursor->_instructionAddr == IA) {
                    ...
                    if (TR::Options::isAnyVerboseOptionSet(TR_VerboseHWProfiler) && metaData->bodyInfo) {
                        ...
                        if (bodyInfo->getMethodInfo() && bodyInfo->getMethodInfo()->getMethodInfo()) {
                            TR_VerboseLog::writeLineLocked(TR_Vlog_HWPROFILER,
                                "Found bytecodePC=0x%p from metadata=0x%p of j9method=0x%p", cursor->_bytecodePC,
                                metaData, bodyInfo->getMethodInfo()->getMethodInfo());
                        }
                    }
#endif
            ...
            if (TR::Options::isAnyVerboseOptionSet(TR_VerboseHWProfiler)) {
                TR_VerboseLog::writeLineLocked(TR_Vlog_HWPROFILER,
                    "metaData=0x%p, metaData->raData=0x%p, or eyecatcher invalid", metaData,
...
void TR_HWProfiler::registerRecords(J9JITExceptionTable *metaData, TR::Compilation *comp)
{
    if (TR::Options::getCmdLineOptions()->getOption(TR_EnableHardwareProfileIndirectDispatch)
        && TR::Options::getCmdLineOptions()->getOption(TR_EnableMetadataBytecodePCToIAMap) && metaData->riData) {
...
void TR_HWProfiler::createRecords(TR::Compilation *comp)
{
    if (!comp->getPersistentInfo()->isRuntimeInstrumentationEnabled() || comp->isProfilingCompilation()
    ...
    for (int32_t i = 0; i < hwpInstructionInfos->size(); i++) {
        ...
        switch (hwpInstructionType) {
            case TR_HWPInstructionInfo::valueProfileInstructions: {
                ...
                TR_HWPBytecodePCToIAMap map = { (void *)getPCFromMethodAndBCIndex(method, bcIndex, comp), (void *)ia };
        ...
        if (!TR::Options::getCmdLineOptions()->getOption(TR_HWProfilerDisableAOT)
            ...
            cg->addExternalRelocation(TR::ExternalRelocation::create(ia, target, target2, relocationTargetKind, cg),
                __FILE__, __LINE__, node);
        }
    }
}

void TR_HWProfiler::printStats()
{
