...
void TR_RelocationRuntimeLogger::setupOptions(TR::Options *options)
{
    _logEnabled = false;
    _logLevel = 0;
    if (options) {
...
void TR_RelocationRuntimeLogger::debug_printf(const char *format, ...)
{
    va_list args;
...
void TR_RelocationRuntimeLogger::printf(const char *format, ...)
{
    va_list args;
    char outputBuffer[512];
...
void TR_RelocationRuntimeLogger::relocatableDataHeader()
{
    if (logEnabled()) {
...
void TR_RelocationRuntimeLogger::relocatableDataFooter()
{
    if (logEnabled()) {
        endTag(headerTag);
...
void TR_RelocationRuntimeLogger::method(bool newLine)
{
    ...
    rtlogPrintf(jitConfig(), reloRuntime()->fej9()->_compInfoPT, patternString, classNameLen, className, methodNameLen,
        methodName, signatureLen, signature);
    unlockLog(wasLocked);
}

void TR_RelocationRuntimeLogger::exceptionTable()
{
...
void TR_RelocationRuntimeLogger::metaData()
{
    if (logEnabled()) {
        J9JavaVM *javaVM = jitConfig()->javaVM;
...
void TR_RelocationRuntimeLogger::relocationDump()
{
    J9JavaVM *javaVM = jitConfig()->javaVM;
...
void TR_RelocationRuntimeLogger::relocationTime()
{
    if (verbose()) {
        J9JavaVM *javaVM = jitConfig()->javaVM;
        PORT_ACCESS_FROM_JAVAVM(javaVM);
        ...
        rtlogPrintf(jitConfig(), reloRuntime()->fej9()->_compInfoPT, " <%p-%p> ",
            reinterpret_cast<void *>(reloRuntime()->exceptionTable()->startPC),
            reinterpret_cast<void *>(reloRuntime()->exceptionTable()->endPC));

        rtlogPrintf(jitConfig(), reloRuntime()->fej9()->_compInfoPT, " Time: %d usec\n",
...
void TR_RelocationRuntimeLogger::versionMismatchWarning()
{
    const char *warningMessageFormat = "AOT major/minor versions don't match the ones of running JVM: aotMajorVersion "
                                       "%d jvmMajorVersion %d aotMinorVersion %d jvmMinorVersion %d   ";
    if (verbose()) {
        bool wasLocked = lockLog();
        rtlogPrintf(jitConfig(), reloRuntime()->fej9()->_compInfoPT, warningMessageFormat,
            reloRuntime()->aotMethodHeaderEntry()->majorVersion, TR_AOTMethodHeader_MajorVersion,
            reloRuntime()->aotMethodHeaderEntry()->minorVersion, TR_AOTMethodHeader_MinorVersion);
        unlockLog(wasLocked);
...
void TR_RelocationRuntimeLogger::maxCodeOrDataSizeWarning()
{
    if (verbose()) {
...
bool TR_RelocationRuntimeLogger::lockLog()
{
    TR::CompilationInfoPerThread *compInfoPT = reloRuntime()->fej9()->_compInfoPT;
...
void TR_RelocationRuntimeLogger::unlockLog(bool shouldUnlock)
{
    if (shouldUnlock) {
...
void TR_RelocationRuntimeLogger::startTag(const char *tag)
{
    rtlogPrintf(jitConfig(), reloRuntime()->fej9()->_compInfoPT, "<%s>\n", tag);
}

void TR_RelocationRuntimeLogger::endTag(const char *tag)
{
    rtlogPrintf(jitConfig(), reloRuntime()->fej9()->_compInfoPT, "</%s>\n", tag);
}
