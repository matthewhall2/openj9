...
static void printHashedCallSite(OMR::Logger *log, TR_IPHashedCallSite *hcs, void *tag = NULL)
{
    J9UTF8 *nameUTF8;
    ...
    log->printf("%p %.*s.%.*s%.*s %p %d\n", tag, J9UTF8_LENGTH(methodClazzUTRF8), J9UTF8_DATA(methodClazzUTRF8),
        J9UTF8_LENGTH(nameUTF8), J9UTF8_DATA(nameUTF8), J9UTF8_LENGTH(signatureUTF8), J9UTF8_DATA(signatureUTF8),
        hcs->_method, hcs->_offset);
    log->flush();
}
...
bool TR_ReadSampleRequestsHistory::init(int32_t historyBufferSize)
{
    ...
    _history = (TR_ReadSampleRequestsStats *)TR_IProfiler::allocator()->allocate(
        historyBufferSize * sizeof(TR_ReadSampleRequestsStats), std::nothrow);
    if (_history) {
...
uint32_t TR_ReadSampleRequestsHistory::getReadSampleFailureRate() const
{
    ...
    uint32_t readSamplesDiff
        = _history[_crtIndex]._totalReadSampleRequests - _history[oldestIndex]._totalReadSampleRequests;
    if (readSamplesDiff
        > SAMPLE_CUTOFF) // prevent division by 0 and also false alarms when the number of samples is too small
    {
        uint32_t failedReadSamplesDiff
            = _history[_crtIndex]._failedReadSampleRequests - _history[oldestIndex]._failedReadSampleRequests;
...
uintptr_t TR_IProfiler::createBalancedBST(uintptr_t *pcEntries, int32_t low, int32_t high, uintptr_t memChunk,
    TR_J9SharedCache *sharedCache)
{
    ...
    uintptr_t leftChild = createBalancedBST(pcEntries, low, middle - 1, memChunk + bytes, sharedCache);
    ...
    uintptr_t rightChild = createBalancedBST(pcEntries, middle + 1, high, memChunk + bytes + leftChild, sharedCache);
    if (rightChild) {
        TR_ASSERT(bytes + leftChild < 1 << 16,
            "Error storing iprofile information: right child too far away"); // current size of right child
...
uint32_t TR_IProfiler::walkILTreeForEntries(uintptr_t *pcEntries, uint32_t &numEntries,
    TR_J9ByteCodeIterator *bcIterator, TR_OpaqueMethodBlock *method, TR::Compilation *comp, vcount_t visitCount,
    int32_t callerIndex, TR_BitVector *BCvisit, bool &abort)
{
    abort = false;
    ...
    for (; bc != J9BCunknown; bc = bcIterator->next()) {
        ...
        if (bci < TR::Compiler->mtd.bytecodeSize(method) && !BCvisit->isSet(bci)) {
            ...
            if (entry && !invalidateEntryIfInconsistent(entry)) {
                ...
                uint32_t canPersist
                    = entry->canBePersisted(comp->fej9()->sharedCache(), _compInfo->getPersistentInfo());
...
bool TR_IProfiler::elgibleForPersistIprofileInfo(TR::Compilation *comp) const
{
    return (TR::Options::sharedClassCache() && !comp->getOption(TR_DisablePersistIProfile) && isIProfilingEnabled());
}

void TR_IProfiler::persistIprofileInfo(TR::ResolvedMethodSymbol *resolvedMethodSymbol,
    TR_ResolvedMethod *resolvedMethod, TR::Compilation *comp)
{
    ...
    if (elgibleForPersistIprofileInfo(comp) && (!SCfull || !comp->getOption(TR_DisableUpdateJITBytesSize))) {
        ...
        if (comp->fej9()->sharedCache()->isROMMethodInSharedCache(romMethod)) {
            TR_ASSERT(comp->fej9()->sharedCache()->isPtrToROMClassesSectionInSharedCache((void *)methodStart),
                "bytecodes not in shared cache");
            ...
            if (!found) {
                ...
                TR_BitVector *BCvisit = new (comp->trStackMemory())
                    TR_BitVector(TR::Compiler->mtd.bytecodeSize(method), comp->trMemory(), stackAlloc);
                bool abort = false;
                TR_J9ByteCodeIterator bci(0, static_cast<TR_ResolvedJ9Method *>(resolvedMethod),
                    static_cast<TR_J9VMBase *>(comp->fej9()), comp);
                uintptr_t *pcEntries = (uintptr_t *)comp->trMemory()->allocateMemory(
                    sizeof(uintptr_t) * bci.maxByteCodeIndex(), stackAlloc);
                bytesFootprint
                ...
                if (numEntries && !abort) {
                    ...
                    if (SCfull && bytesToPersist && !comp->getOption(TR_DisableUpdateJITBytesSize)) {
...
uint32_t TR_IProfiler::getProfilerMemoryFootprint() { return memoryConsumed; }

void *TR_IProfiler::operator new(size_t size) throw()
{
    memoryConsumed += (int32_t)size;
    return _allocator->allocate(size, std::nothrow);
}

TR::PersistentAllocator *TR_IProfiler::createPersistentAllocator(J9JITConfig *jitConfig)
{
...
TR_IProfiler *TR_IProfiler::allocate(J9JITConfig *jitConfig)
{
...
TR_IProfiler::TR_IProfiler(J9JITConfig *jitConfig)
    ...
    , _valueProfileMethod(NULL)
    , _lightHashTableMonitor(0)
    , _allowedToGiveInlinedInformation(true)
    , _globalAllocationCount(0)
    , _maxCallFrequency(0)
    , _iprofilerThread(0)
    , _iprofilerOSThread(NULL)
    , _workingBufferTail(NULL)
    , _numOutstandingBuffers(0)
    , _numRequests(1)
    , _numRequestsDropped(0)
    , _numRequestsSkipped(0)
    , _numRequestsHandedToIProfilerThread(0)
    , _iprofilerMonitor(NULL)
    , _crtProfilingBuffer(NULL)
    , _iprofilerNumRecords(0)
    , _numMethodHashEntries(0)
    , _iprofilerThreadLifetimeState(TR_IprofilerThreadLifetimeStates::IPROF_THR_NOT_CREATED)
{
...
#endif
    {
        ...
        // bytecode hashtable
        _bcHashTable = (TR_IPBytecodeHashTableEntry **)_allocator->allocate(
            TR::Options::_iProfilerBcHashTableSize * sizeof(TR_IPBytecodeHashTableEntry *), std::nothrow);
...
#if defined(EXPERIMENTAL_IPROFILER)
        _allocHashTable = (TR_IPBCDataAllocation **)_allocator->allocate(
            ALLOC_HASH_TABLE_SIZE * sizeof(TR_IPBCDataAllocation *), std::nothrow);
        if (_allocHashTable != NULL)
            memset(_allocHashTable, 0, ALLOC_HASH_TABLE_SIZE * sizeof(TR_IPBCDataAllocation *));
#endif
        _methodHashTable = (TR_IPMethodHashTableEntry **)_allocator->allocate(
            TR::Options::_iProfilerMethodHashTableSize * sizeof(TR_IPMethodHashTableEntry *), std::nothrow);
        if (_methodHashTable != NULL)
            memset(_methodHashTable, 0,
                TR::Options::_iProfilerMethodHashTableSize * sizeof(TR_IPMethodHashTableEntry *));
        _readSampleRequestsHistory
            = (TR_ReadSampleRequestsHistory *)_allocator->allocate(sizeof(TR_ReadSampleRequestsHistory), std::nothrow);
...
bool TR_IProfiler::isCallGraphProfilingEnabled()
{
...
inline int32_t TR_IProfiler::bcHash(uintptr_t pc)
{
    return (int32_t)((pc & 0x7FFFFFFF) % TR::Options::_iProfilerBcHashTableSize);
}

inline int32_t TR_IProfiler::allocHash(uintptr_t pc) { return (int32_t)((pc & 0x7FFFFFFF) % ALLOC_HASH_TABLE_SIZE); }

inline int32_t TR_IProfiler::methodHash(uintptr_t data)
{
    return (int32_t)((data & 0x7FFFFFFF) % TR::Options::_iProfilerMethodHashTableSize);
}

bool TR_IProfiler::isCompact(U_8 byteCode)
{
    switch (byteCode) {
        case JBifacmpeq:
        case JBifacmpne:
...
bool TR_IProfiler::isSwitch(U_8 byteCode) { return ((byteCode == JBlookupswitch) || (byteCode == JBtableswitch)); }

bool isInterfaceBytecode(U_8 byteCode) { return (byteCode == JBinvokeinterface); }

bool isInterface2Bytecode(U_8 byteCode) { return (byteCode == JBinvokeinterface2); }

bool isCheckCastOrInstanceOf(U_8 byteCode) { return ((byteCode == JBinstanceof) || (byteCode == JBcheckcast)); }

bool isSpecialOrStatic(U_8 byteCode)
{
    return ((byteCode == JBinvokestatic) || (byteCode == JBinvokespecial) || (byteCode == JBinvokestaticsplit)
        || (byteCode == JBinvokespecialsplit));
}

static void getSwitchSegmentDataAndCount(uint64_t segment, uint32_t *segmentData, uint32_t *segmentCount)
{
...
static void setSwitchSegmentData(uint64_t *segment, uint32_t data)
{
    *segment &= CONSTANT64(0x00000000FFFFFFFF);
    *segment |= (((uint64_t)data) << 32);
}

static void setSwitchSegmentCount(uint64_t *segment, uint32_t count)
{
...
int32_t TR_IProfiler::getOrSetSwitchData(TR_IPBCDataEightWords *entry, uint32_t value, bool isSet, bool isLookup)
{
...
bool TR_IProfiler::addSampleData(TR_IPBytecodeHashTableEntry *entry, uintptr_t data, bool isRIData, uint32_t freq)
{
    if (entry->isInvalid())
        return false;
    ...
    switch (byteCodeType) {
        ...
        case JBcheckcast:
        case JBinstanceof: {
            bool isVirtualCall = (byteCodeType == JBinvokeinterface || byteCodeType == JBinvokeinterface2
                || byteCodeType == JBinvokevirtual);

            if (isRIData) {
                if (TR::Options::getCmdLineOptions()->getOption(TR_DontAddHWPDataToIProfiler))
                    return true;
            } else if (isVirtualCall && TR::Options::getCmdLineOptions()->getOption(TR_DisableVMCSProfiling)) {
                return true;
            }

#if defined(RI_VP_Verbose)
            if (isVirtualCall && TR::Options::isAnyVerboseOptionSet(TR_VerboseHWProfiler)) {
                ...
                TR_VerboseLog::writeLineLocked(TR_Vlog_HWPROFILER,
                    "Adding class=0x%p for bytecode=%s (bytecodePC=0x%p) from %s", data, byteCodeName, entry->pc,
        ...
        case JBlookupswitch:
            TR_ASSERT(entry->asIPBCDataEightWords(),
                "The new switch profiling code uses 8 bytes to store switch history");
...
TR_IPBytecodeHashTableEntry *TR_IProfiler::searchForSample(uintptr_t pc, int32_t bucket)
{
...
TR_IPBCDataAllocation *TR_IProfiler::searchForAllocSample(uintptr_t pc, int32_t bucket)
{
#if defined(EXPERIMENTAL_IPROFILER)
...
TR_IPBytecodeHashTableEntry *TR_IProfiler::findOrCreateEntry(int32_t bucket, uintptr_t pc, bool addIt)
{
...
TR_IPBCDataAllocation *TR_IProfiler::findOrCreateAllocEntry(int32_t bucket, uintptr_t pc, bool addIt)
{
#if defined(EXPERIMENTAL_IPROFILER)
...
TR_IPMethodHashTableEntry *TR_IProfiler::findOrCreateMethodEntry(J9Method *callerMethod, J9Method *calleeMethod,
    bool addIt, uint32_t pcIndex)
{
...
void TR_IPMethodHashTableEntry::add(TR_OpaqueMethodBlock *caller, TR_OpaqueMethodBlock *callee, uint32_t pcIndex)
{
    ...
    } else // caller is not in the list. Let's add it
    {
        ...
        if (size >= MAX_IPMETHOD_CALLERS) {
            ...
            TR_IPMethodData *newCaller
                = (TR_IPMethodData *)TR_IProfiler::allocator()->allocate(sizeof(TR_IPMethodData), std::nothrow);
            if (newCaller) {
                ...
                newCaller->incWeight();
                newCaller->next
                    = _caller.next; // add the existing list of callers (except the embedded one) to this new caller
                FLUSH_MEMORY(TR::Compiler->target.isSMP());
...
bool TR_IProfiler::invalidateEntryIfInconsistent(TR_IPBytecodeHashTableEntry *entry)
{
...
TR_IPBCDataStorageHeader *TR_IProfiler::searchForPersistentSample(TR_IPBCDataStorageHeader *root, uintptr_t pc)
{
    if (root->pc == pc)
        return root;
    else if (pc < root->pc) {
...
TR_IPBytecodeHashTableEntry *TR_IProfiler::persistentProfilingSample(TR_OpaqueMethodBlock *method,
    uint32_t byteCodeIndex, TR::Compilation *comp, bool *methodProfileExistsInSCC)
{
    if (TR::Options::sharedClassCache()) // shared classes must be enabled
    {
        ...
        *methodProfileExistsInSCC = true;
        ...
        store = searchForPersistentSample(store,
            (uintptr_t)comp->fej9()->sharedCache()->offsetInSharedCacheFromPtrToROMClassesSection((void *)pc));

        if (TR::Options::getAOTCmdLineOptions()->getOption(TR_EnableIprofilerChanges)
            || TR::Options::getJITCmdLineOptions()->getOption(TR_EnableIprofilerChanges)) {
            if (store) {
...
TR_IPBCDataStorageHeader *TR_IProfiler::persistentProfilingSample(TR_OpaqueMethodBlock *method, uint32_t byteCodeIndex,
    TR::Compilation *comp, bool *methodProfileExistsInSCC, TR_IPBCDataStorageHeader *store)
{
    if (TR::Options::sharedClassCache()) // shared classes must be enabled
    {
        ...
        *methodProfileExistsInSCC = true;
        ...
        store = searchForPersistentSample(store,
            (uintptr_t)comp->fej9()->sharedCache()->offsetInSharedCacheFromPtrToROMClassesSection(pc));
        return store;
    }
    return NULL;
}

TR_IPBCDataStorageHeader *TR_IProfiler::getJ9SharedDataDescriptorForMethod(J9SharedDataDescriptor *descriptor,
    unsigned char *buffer, uint32_t length, TR_OpaqueMethodBlock *method, TR::Compilation *comp)
{
...
TR_IPMethodHashTableEntry *TR_IProfiler::searchForMethodSample(TR_OpaqueMethodBlock *omb, int32_t bucket)
{
...
TR_IPBytecodeHashTableEntry *TR_IProfiler::profilingSample(TR_OpaqueMethodBlock *method, uint32_t byteCodeIndex,
    ...
    if (!addIt) // read request
    {
        ...
        */
        class EntryPlaceHolderClass {
            char buffer[MAX_THREE(sizeof(TR_IPBCDataFourBytes), sizeof(TR_IPBCDataEightWords),
                            sizeof(TR_IPBCDataCallGraph))
                + sizeof(void *)];
        };
        ...
        if (!comp->getOption(TR_DoNotUsePersistentIprofiler)) {
            ...
            if (preferHashtableData) {
                ...
                persistentEntryStore
                    = getJ9SharedDataDescriptorForMethod(&descriptor, storeBuffer, bufferLength, method, comp);
                if (persistentEntryStore) {
                    persistentEntryStore = persistentProfilingSample(method, byteCodeIndex, comp,
                        &methodProfileExistsInSCC, persistentEntryStore);
                }
                if (!persistentEntryStore) {
...
int32_t TR_IPBCDataEightWords::getSumSwitchCount() const
{
    int32_t sum = 1;
...
TR_IPBytecodeHashTableEntry *TR_IProfiler::profilingSample(uintptr_t pc, uintptr_t data, bool addIt, bool isRIData,
    uint32_t freq)
{
...
TR_IPBytecodeHashTableEntry *TR_IProfiler::profilingSampleRI(uintptr_t pc, uintptr_t data, bool addIt, uint32_t freq)
{
    return profilingSample(pc, data, addIt, true, freq);
}

TR_IPBCDataAllocation *TR_IProfiler::profilingAllocSample(uintptr_t pc, uintptr_t data, bool addIt)
{
#if defined(EXPERIMENTAL_IPROFILER)
    TR_IPBCDataAllocation *entry = NULL;
...
TR_ExternalProfiler *TR_IProfiler::canProduceBlockFrequencyInfo(TR::Compilation &comp)
{
...
static TR_OpaqueMethodBlock *getMethodFromBCInfo(TR_ByteCodeInfo &bcInfo, TR::Compilation *comp)
{
...
uint8_t TR_IProfiler::getBytecodeOpCode(TR::Node *node, TR::Compilation *comp)
{
    TR_OpaqueMethodBlock *method = NULL;
...
bool TR_IProfiler::branchHasSameDirection(TR::ILOpCodes nodeOpCode, TR::Node *node, TR::Compilation *comp)
{
    ...
    if (byteCodeOpCode != TR::BadILOp) {
        if (TR::ILOpCode::isStrictlyLessThanCmp(byteCodeOpCode)
            && ((!node->childrenWereSwapped() && TR::ILOpCode::isStrictlyLessThanCmp(nodeOpCode))
                || (node->childrenWereSwapped() && TR::ILOpCode::isStrictlyGreaterThanCmp(nodeOpCode))))
            return true;
        ...
        if (TR::ILOpCode::isLessCmp(byteCodeOpCode)
            && ((!node->childrenWereSwapped() && TR::ILOpCode::isLessCmp(nodeOpCode))
                || (node->childrenWereSwapped() && TR::ILOpCode::isGreaterCmp(nodeOpCode))))
            return true;
        ...
        if (TR::ILOpCode::isEqualCmp(byteCodeOpCode) && TR::ILOpCode::isEqualCmp(nodeOpCode))
            return true;

        if (TR::ILOpCode::isNotEqualCmp(byteCodeOpCode) && TR::ILOpCode::isNotEqualCmp(nodeOpCode))
            return true;
    }
...
bool TR_IProfiler::branchHasOppositeDirection(TR::ILOpCodes nodeOpCode, TR::Node *node, TR::Compilation *comp)
{
    TR::ILOpCodes byteCodeOpCode = opCodeForBranchFromBytecode(getBytecodeOpCode(node, comp));
    if (byteCodeOpCode != TR::BadILOp) {
        if (TR::ILOpCode::isStrictlyLessThanCmp(byteCodeOpCode)
            && (TR::ILOpCode::isGreaterCmp(nodeOpCode) || TR::ILOpCode::isLessCmp(nodeOpCode)))
            return true;
        ...
        if (TR::ILOpCode::isLessCmp(byteCodeOpCode)
            && (TR::ILOpCode::isStrictlyLessThanCmp(nodeOpCode) || TR::ILOpCode::isStrictlyGreaterThanCmp(nodeOpCode)))
            return true;
        ...
        if (TR::ILOpCode::isEqualCmp(byteCodeOpCode) && TR::ILOpCode::isNotEqualCmp(nodeOpCode))
            return true;

        if (TR::ILOpCode::isNotEqualCmp(byteCodeOpCode) && TR::ILOpCode::isEqualCmp(nodeOpCode))
            return true;
    }
    return false;
}

uintptr_t TR_IProfiler::getProfilingData(TR::Node *node, TR::Compilation *comp)
{
    if (!isIProfilingEnabled())
        return 0;
...
uintptr_t TR_IProfiler::getProfilingData(TR_ByteCodeInfo &bcInfo, TR::Compilation *comp)
{
    if (!isIProfilingEnabled())
        return 0;
...
TR_OpaqueMethodBlock *TR_IProfiler::getMethodFromNode(TR::Node *node, TR::Compilation *comp)
{
    if (!isIProfilingEnabled())
        return 0;
...
uintptr_t TR_IProfiler::getSearchPCFromMethodAndBCIndex(TR_OpaqueMethodBlock *method, uint32_t byteCodeIndex)

{
    ...
    if (byteCodeIndex < methodSize) {
        ...
        if (isInterfaceBytecode(*(U_8 *)searchedPC) && byteCodeIndex >= 2
            && isInterface2Bytecode(*(U_8 *)(searchedPC - 2))) {
            searchedPC -= 2;
        }
    } else {
        TR_ASSERT(false, "Bytecode index can't be higher than the methodSize: bci=%u methdSize=%u", byteCodeIndex,
            methodSize);
    }
    ...
    TR::Compilation *comp)
{
    ...
    if (pc == 0 && traceIProfiling) {
        ...
        logprints(traceIProfiling, log, "CSI : INLINER STACK :\n");
        for (int i = 0; i < len; i++) {
...
uintptr_t TR_IProfiler::getSearchPC(TR_OpaqueMethodBlock *method, uint32_t byteCodeIndex, TR::Compilation *comp)
{
    return getSearchPCFromMethodAndBCIndex(method, byteCodeIndex, comp);
}

uint32_t *TR_IProfiler::getGlobalAllocationDataPointer(bool isAOT)
{
    if (!isIProfilingEnabled())
        return NULL;
...
bool TR_IProfiler::isNewOpCode(U_8 byteCode)
{
    return (byteCode == JBnew) || (byteCode == JBnewarray) || (byteCode == JBanewarray);
}

uint32_t *TR_IProfiler::getAllocationProfilingDataPointer(TR_ByteCodeInfo &bcInfo, TR_OpaqueClassBlock *clazz,
...
TR_IPBytecodeHashTableEntry *TR_IProfiler::getProfilingEntry(TR_OpaqueMethodBlock *method, uint32_t byteCodeIndex,
    TR::Compilation *comp)
{
    ...
    logprintf(traceIProfiling, log, "Asked for profiling data on PC=%p, ", getSearchPC(method, byteCodeIndex, comp));

    if (entry) {
        if (invalidateEntryIfInconsistent(entry)) {
            logprints(traceIProfiling, log, "got nothing because it was invalidated\n");
            return 0;
        }
    ...
    logprints(traceIProfiling, log, "got nothing\n");

    return 0;
}

uintptr_t TR_IProfiler::getProfilingData(TR_OpaqueMethodBlock *method, uint32_t byteCodeIndex, TR::Compilation *comp)
{
...
int32_t TR_IProfiler::getSwitchCountForValue(TR::Node *node, int32_t value, TR::Compilation *comp)
{
    TR_ByteCodeInfo bcInfo = node->getByteCodeInfo();
    if (bcInfo.doNotProfile())
        return 0;
    TR_IPBytecodeHashTableEntry *entry
        = getProfilingEntry(getMethodFromNode(node, comp), bcInfo.getByteCodeIndex(), comp);
    ...
    if (entry && entry->asIPBCDataEightWords()) {
        ...
        return getOrSetSwitchData((TR_IPBCDataEightWords *)entry, (uint32_t)data, false,
            (node->getOpCodeValue() == TR::lookup));
    }
    return 0;
}

int32_t TR_IProfiler::getSumSwitchCount(TR::Node *node, TR::Compilation *comp)
{
    int32_t sum = 1;
    TR_ByteCodeInfo bcInfo = node->getByteCodeInfo();
    if (bcInfo.doNotProfile())
...
int32_t TR_IProfiler::getFlatSwitchProfileCounts(TR::Node *node, TR::Compilation *comp)
{
...
bool TR_IProfiler::isSwitchProfileFlat(TR::Node *node, TR::Compilation *comp)
{
    TR_ByteCodeInfo bcInfo = node->getByteCodeInfo();
    if (bcInfo.doNotProfile())
        return true;
...
void TR_IProfiler::getBranchCounters(TR::Node *node, TR::TreeTop *fallThroughTree, int32_t *taken, int32_t *notTaken,
    TR::Compilation *comp)
{
    static bool traceIProfiling = ((debug("traceIProfiling") != NULL));
    ...
    if (data) {
        ...
        if (nodeOpCode == TR::BadILOp)
            nodeOpCode = node->getOpCodeValue();
        ...
        if (branchHasSameDirection(nodeOpCode, node, comp)) {
            ...
            *taken = (int32_t)fallThroughCount;
            ...
            logprintf(traceIProfiling, comp->log(),
                "I couldn't figure out the branch direction after change for node [%p], so I gave default direction \n",
                node);
            *taken = (int32_t)branchToCount;
            *notTaken = (int32_t)fallThroughCount;
        }
...
void TR_IProfiler::setBlockAndEdgeFrequencies(TR::CFG *cfg, TR::Compilation *comp)
{
    if (!isIProfilingEnabled())
        return;
        ...
        for (TR::TreeTop *treeTop = comp->getStartTree(); treeTop; treeTop = treeTop->getNextTreeTop()) {
            ...
            if (node->getOpCodeValue() == TR::BBStart) {
                log->printf("\nBlock[%d] frequency = %d\n", node->getBlock()->getNumber(),
                    node->getBlock()->getFrequency());
            }
        }
    }
}

TR_AbstractInfo *TR_IProfiler::createIProfilingValueInfo(TR_ByteCodeInfo &bcInfo, TR::Compilation *comp)
{
    if (!isIProfilingEnabled())
        return NULL;
    ...
    if (_allowedToGiveInlinedInformation &&
       ...
       !bcInfo.doNotProfile()*/)
      {
        ...
        if (comp->getOption(TR_IProfilerPerformTimestampCheck) && method
            && !_compInfo->isCompiled((J9Method *)method)) // method is interpreted
        {
            bool allowForAOT = comp->getOption(TR_UseSymbolValidationManager);
            TR_PersistentClassInfo *currentPersistentClassInfo
                = _compInfo->getPersistentInfo()->getPersistentCHTable()->findClassInfoAfterLocking(
                    comp->getCurrentMethod()->containingClass(), comp, allowForAOT);
            TR_PersistentClassInfo *calleePersistentClassInfo
                = _compInfo->getPersistentInfo()->getPersistentCHTable()->findClassInfoAfterLocking(
                    (TR_OpaqueClassBlock *)J9_CLASS_FROM_METHOD(((J9Method *)method)), comp, allowForAOT);

            if (!currentPersistentClassInfo || !calleePersistentClassInfo) {
                logprints(traceIProfiling, log, "\nMissing persistent class or method info returning NULL\n");
            ...
            if ((currentPersistentClassInfo->getTimeStamp() == (uint16_t)0x0FFFF)
                ...
                logprints(traceIProfiling, log,
                    "\nThe time stamp for callee or caller class has expired, I refuse to give profiling information "
                    "back\n");
            ...
            if ((currentPersistentClassInfo->getTimeStamp() > calleePersistentClassInfo->getTimeStamp())
                && ((int32_t)(currentPersistentClassInfo->getTimeStamp() - calleePersistentClassInfo->getTimeStamp())
                    > _classLoadTimeStampGap)) {
                logprintf(traceIProfiling, log,
                    "\nCallee method %s (callerIndex=%d) is interpreted but class time stamps are too far apart, I "
                    "refuse to give profiling info for this callee method (ownerClass time stamp %d, callee class time "
                    "stamp %d).\n",
                    _vm->sampleSignature(method, 0, 0, comp->trMemory()), bcInfo.getCallerIndex(),
                    currentPersistentClassInfo->getTimeStamp(), calleePersistentClassInfo->getTimeStamp());
            ...
            logprintf(traceIProfiling, log,
                "\nCallee method %s (callerIndex=%d) is interpreted I'll give profiling information for it, ownerClass "
                "time stamp %d, callee class time stamp %d.\n",
                _vm->sampleSignature(method, 0, 0, comp->trMemory()), bcInfo.getCallerIndex(),
                currentPersistentClassInfo->getTimeStamp(), calleePersistentClassInfo->getTimeStamp());
        }
    ...
    if (!bcInfo.doNotProfile()) {
        ...
        if (!isNewOpCode(thisByteCode)) {
            ...
            if (csInfo) {
                ...
                if (!data) {
                    logprintf(traceIProfiling, log, "Call-graph 1 No profiling data for bcIndex=%d, callerIndex=%d\n",
                ...
                for (int32_t i = 1; i < NUM_CS_SLOTS; i++) {
                    ...
                    if (data) {
                        ...
                        list->incrementOrCreate(address, &addrOfTotalFrequency, i, weight,
                ...
                *addrOfTotalFrequency = (*addrOfTotalFrequency) + csInfo->_residueWeight;
                ...
                logprintf(traceIProfiling, log, "Call-graph 2 Set not to profile bcIndex=%d, callerIndex=%d\n",
                    bcInfo.getByteCodeIndex(), bcInfo.getCallerIndex());
                return NULL;
            }
        } else {
            ...
            if (!data) {
                logprintf(traceIProfiling, log, "No profiling data for bcIndex=%d, callerIndex=%d\n",
                    bcInfo.getByteCodeIndex(), bcInfo.getCallerIndex());
                return NULL;
            }
...
bool TR_IProfiler::hasSameBytecodeInfo(TR_ByteCodeInfo &persistentByteCodeInfo, TR_ByteCodeInfo &currentByteCodeInfo,
    TR::Compilation *comp)
{
...
TR_AbstractInfo *TR_IProfiler::createIProfilingValueInfo(TR::Node *node, TR::Compilation *comp)
{
    static bool traceIProfiling = ((debug("traceIProfiling") != NULL));
...
TR_ExternalValueProfileInfo *TR_IProfiler::getValueProfileInfo(TR_ByteCodeInfo &bcInfo, TR::Compilation *comp)
{
    if (!isIProfilingEnabled())
        return NULL;
    ...
    // TR_OpaqueMethodBlock *originatorMethod = (TR_OpaqueMethodBlock
    // *)(comp->getCurrentMethod()->getPersistentIdentifier());
...
bool TR_IProfiler::acquireHashTableWriteLock(bool forceFullLock) { return true; }

void TR_IProfiler::releaseHashTableWriteLock() { return; }

void TR_IProfiler::outputStats()
{
...
void *TR_IPBytecodeHashTableEntry::operator new(size_t size) throw()
{
    memoryConsumed += (int32_t)size;
    return TR_IProfiler::allocator()->allocate(size, std::nothrow);
}

void TR_IPBytecodeHashTableEntry::operator delete(void *p) throw() { TR_IProfiler::allocator()->deallocate(p); }

#if defined(J9VM_OPT_JITSERVER)
void TR_IPBCDataFourBytes::serialize(uintptr_t methodStartAddress, TR_IPBCDataStorageHeader *storage,
    TR::PersistentInfo *info)
{
...
void TR_IPBCDataFourBytes::loadFromPersistentCopy(TR_IPBCDataStorageHeader *storage, TR::Compilation *comp)
{
    TR_IPBCDataFourBytesStorage *store = (TR_IPBCDataFourBytesStorage *)storage;
    TR_ASSERT(storage->ID == TR_IPBCD_FOUR_BYTES,
        "Incompatible types between storage and loading of iprofile persistent data");
    data = store->data;
}

void TR_IPBCDataFourBytes::copyFromEntry(TR_IPBytecodeHashTableEntry *originalEntry)
{
    TR_IPBCDataFourBytes *entry = (TR_IPBCDataFourBytes *)originalEntry;
    TR_ASSERT(originalEntry->asIPBCDataFourBytes(),
        "Incompatible types between storage and loading of iprofile persistent data");
    data = entry->data;
}

int32_t TR_IPBCDataFourBytes::getSumBranchCount() const
{
...
void TR_IPBCDataEightWords::createPersistentCopy(TR_J9SharedCache *sharedCache, TR_IPBCDataStorageHeader *storage,
    TR::PersistentInfo *info)
{
...
void TR_IPBCDataEightWords::loadFromPersistentCopy(TR_IPBCDataStorageHeader *storage, TR::Compilation *comp)
{
    TR_IPBCDataEightWordsStorage *store = (TR_IPBCDataEightWordsStorage *)storage;
    TR_ASSERT(storage->ID == TR_IPBCD_EIGHT_WORDS,
        "Incompatible types between storage and loading of iprofile persistent data");
    for (int i = 0; i < SWITCH_DATA_COUNT; i++)
        data[i] = store->data[i];
}

void TR_IPBCDataEightWords::copyFromEntry(TR_IPBytecodeHashTableEntry *originalEntry)
{
    TR_IPBCDataEightWords *entry = (TR_IPBCDataEightWords *)originalEntry;
    TR_ASSERT(originalEntry->asIPBCDataEightWords(),
        "Incompatible types between storage and loading of iprofile persistent data");
    for (int i = 0; i < SWITCH_DATA_COUNT; i++)
        data[i] = entry->data[i];
}
...
#if defined(J9VM_OPT_JITSERVER)
void TR_IPBCDataEightWords::serialize(uintptr_t methodStartAddress, TR_IPBCDataStorageHeader *storage,
    TR::PersistentInfo *info)
{
    TR_IPBCDataEightWordsStorage *store = (TR_IPBCDataEightWordsStorage *)storage;
    storage->pc = _pc - methodStartAddress;
...
int32_t TR_IPBCDataCallGraph::getSumCount() const
{
...
int32_t TR_IPBCDataCallGraph::getSumCount(TR::Compilation *comp)
{
    ...
    for (int32_t i = 0; i < NUM_CS_SLOTS; i++) {
        if (debug) {
            int32_t len;
            const char *s = _csInfo.getClazz(i)
                ? comp->fej9()->getClassNameChars((TR_OpaqueClassBlock *)_csInfo.getClazz(i), len)
                : "0";
            fprintf(stderr, "[%p] slot %" OMR_PRId32 ", class %#" OMR_PRIxPTR " %s, weight %" OMR_PRId32 " : ", this, i,
...
uintptr_t TR_IPBCDataCallGraph::getData(TR::Compilation *comp)
{
    int32_t sumWeight;
    int32_t maxWeight;
    ...
    static bool traceIProfiling = ((debug("traceIProfiling") != NULL));
    logprintf(traceIProfiling && comp, comp->log(), "\nMax weight %d, current sum weight %d\n", maxWeight, sumWeight);
...
void *TR_IPMethodHashTableEntry::operator new(size_t size) throw()
{
    memoryConsumed += (int32_t)size;
    return TR_IProfiler::allocator()->allocate(size, std::nothrow);
}

int32_t TR_IPBCDataCallGraph::getEdgeWeight(TR_OpaqueClassBlock *clazz, TR::Compilation *comp)
{
    for (int32_t i = 0; i < NUM_CS_SLOTS; i++) {
...
void TR_IPBCDataCallGraph::printWeights(TR::Compilation *comp)
{
    for (int32_t i = 0; i < NUM_CS_SLOTS; i++) {
        int32_t len;
        const char *s = _csInfo.getClazz(i)
...
void TR_IPBCDataCallGraph::updateEdgeWeight(TR_OpaqueClassBlock *clazz, int32_t weight)
{
    for (int32_t i = 0; i < NUM_CS_SLOTS; i++) {
        if (_csInfo.getClazz(i) == (uintptr_t)clazz) {
...
bool TR_IPBCDataCallGraph::lockEntry()
{
...
void TR_IPBCDataCallGraph::releaseEntry()
{
    TR::Monitor *persistenceMonitor = TR::MonitorTable::get()->getIProfilerPersistenceMonitor();
    persistenceMonitor->enter();
    resetLockedEntry();
...
bool TR_IPBCDataCallGraph::isLocked()
{
    TR::Monitor *persistenceMonitor = TR::MonitorTable::get()->getIProfilerPersistenceMonitor();
    persistenceMonitor->enter();
    bool locked = false;
 ...
 */
uint32_t TR_IPBCDataCallGraph::canBeSerialized(TR::PersistentInfo *info)
{
 ...
 * @return void
 */
void TR_IPBCDataCallGraph::serialize(uintptr_t methodStartAddress, TR_IPBCDataStorageHeader *storage,
    TR::PersistentInfo *info)
{
...
void TR_IPBCDataCallGraph::createPersistentCopy(TR_J9SharedCache *sharedCache, TR_IPBCDataStorageHeader *storage,
    TR::PersistentInfo *info)
{
    TR_IPBCDataCallGraphStorage *store = (TR_IPBCDataCallGraphStorage *)storage;
    uintptr_t offset = (uintptr_t)sharedCache->offsetInSharedCacheFromPtrToROMClassesSection((void *)_pc);
        ...
        if (!info->isUnloadedClass(clazz, true)) {
            if (sharedCache->isClassInSharedCache(clazz)) {
                ...
                if (TR_SharedCache::INVALID_CLASS_CHAIN_OFFSET != classChainOffset) {
                    ...
                    // getClassChainOffsetIdentifyingLoader() fails the compilation if not found, hence we use a special
                    // implementation
                    uintptr_t classChainOffsetOfCLInSharedCache
                        = sharedCache->getClassChainOffsetIdentifyingLoaderNoThrow(clazz);
                    ...
                    if (TR_SharedCache::INVALID_CLASS_CHAIN_OFFSET == classChainOffsetOfCLInSharedCache)
                        if (TR::Options::getCmdLineOptions()->getVerboseOption(TR_VerboseIProfilerPersistence))
                            TR_VerboseLog::writeLineLocked(TR_Vlog_PERF,
                                "createPersistentCopy: Cannot store CallGraphEntry because classChain identifying "
                                "classloader is 0");
                } else {
                    if (TR::Options::getCmdLineOptions()->getVerboseOption(TR_VerboseIProfilerPersistence))
                        TR_VerboseLog::writeLineLocked(TR_Vlog_PERF,
                            "createPersistentCopy: Cannot store CallGraphEntry because cannot remember class");
                }
            } else // Most frequent class is not in SCC, so I cannot store IProfiler info about this in SCC
            {
                if (TR::Options::getCmdLineOptions()->getVerboseOption(TR_VerboseIProfilerPersistence))
                    TR_VerboseLog::writeLineLocked(TR_Vlog_PERF,
                        "createPersistentCopy: Cannot store CallGraphEntry because ROMClass is not in SCC");
            }
        } else {
            if (TR::Options::getCmdLineOptions()->getVerboseOption(TR_VerboseIProfilerPersistence))
                TR_VerboseLog::writeLineLocked(TR_Vlog_PERF,
                    "createPersistentCopy: Cannot store CallGraphEntry because RAMClass is unloaded");
        }
    } else {
        if (TR::Options::getCmdLineOptions()->getVerboseOption(TR_VerboseIProfilerPersistence))
            TR_VerboseLog::writeLineLocked(TR_Vlog_PERF,
                "createPersistentCopy: Cannot store CallGraphEntry because there is no data");
    }
}

void TR_IPBCDataCallGraph::loadFromPersistentCopy(TR_IPBCDataStorageHeader *storage, TR::Compilation *comp)
{
    TR_IPBCDataCallGraphStorage *store = (TR_IPBCDataCallGraphStorage *)storage;
    TR_ASSERT(storage->ID == TR_IPBCD_CALL_GRAPH,
        "Incompatible types between storage and loading of iprofile persistent data");
    ...
    if (csInfoChainOffset && classChainIdentifyingLoaderOffsetInSCC) {
        ...
        if (classChain) {
            ...
            void *classChainIdentifyingLoader
                = sharedCache->pointerFromOffsetInSharedCache(classChainIdentifyingLoaderOffsetInSCC);
            if (classChainIdentifyingLoader) {
                TR::VMAccessCriticalSection criticalSection(comp->fej9());
                J9ClassLoader *classLoader = (J9ClassLoader *)sharedCache->lookupClassLoaderAssociatedWithClassChain(
                    classChainIdentifyingLoader);
                if (classLoader) {
                    TR_OpaqueClassBlock *j9class
                        = sharedCache->lookupClassFromChainAndLoader(classChain, classLoader, comp);
                    if (j9class) {
                        // Optimizer and the codegen assume receiver classes of a call from profiling data are
                        // initialized, otherwise they shouldn't show up in the profile. But classes from iprofiling
                        // data from last run may be uninitialized in load time, as the program behavior may change in
                        // the second run. Thus we need to verify that a class is initialized, otherwise optimizer or
                        // codegen will make wrong transformation based on invalid assumption.
                        //
                        if (comp->fej9()->isClassInitialized(j9class)) {
                            ...
                            if (TR::Options::getCmdLineOptions()->getVerboseOption(TR_VerboseIProfilerPersistence)) {
                                J9UTF8 *classNameUTF8
                                    = J9ROMCLASS_CLASSNAME(sharedCache->startingROMClassOfClassChain(classChain));
                                TR_VerboseLog::writeLineLocked(TR_Vlog_PERF,
                                    "loadFromPersistentCopy: Cannot covert ROMClass to RAMClass because RAMClass is "
                                    "not initialized. Class: %.*s",
                        ...
                        if (TR::Options::getCmdLineOptions()->getVerboseOption(TR_VerboseIProfilerPersistence)) {
                            J9UTF8 *classNameUTF8
                                = J9ROMCLASS_CLASSNAME(sharedCache->startingROMClassOfClassChain(classChain));
                            TR_VerboseLog::writeLineLocked(TR_Vlog_PERF,
                                "loadFromPersistentCopy: Cannot convert ROMClass to RAMClass because "
                                "lookupClassFromChainAndLoader failed. Class: %.*s",
                                J9UTF8_LENGTH(classNameUTF8), J9UTF8_DATA(classNameUTF8));
                        }
                    }
                } else {
                    ...
                    if (TR::Options::getCmdLineOptions()->getVerboseOption(TR_VerboseIProfilerPersistence)) {
                        J9UTF8 *classNameUTF8
                            = J9ROMCLASS_CLASSNAME(sharedCache->startingROMClassOfClassChain(classChain));
                        TR_VerboseLog::writeLineLocked(TR_Vlog_PERF,
                            "loadFromPersistentCopy: Cannot convert ROMClass to RAMClass. Cannot find classloader. "
                            "Class: %.*s",
                            J9UTF8_LENGTH(classNameUTF8), J9UTF8_DATA(classNameUTF8));
                    }
                }
                ...
                if (TR::Options::getCmdLineOptions()->getVerboseOption(TR_VerboseIProfilerPersistence)) {
                    ...
                    TR_VerboseLog::writeLineLocked(TR_Vlog_PERF,
                        "loadFromPersistentCopy: Cannot convert ROMClass to RAMClass. Cannot find chain identifying "
                        "classloader. Class: %.*s",
                        J9UTF8_LENGTH(classNameUTF8), J9UTF8_DATA(classNameUTF8));
                }
            }
            ...
            if (TR::Options::getCmdLineOptions()->getVerboseOption(TR_VerboseIProfilerPersistence)) {
                TR_VerboseLog::writeLineLocked(TR_Vlog_PERF,
                    "loadFromPersistentCopy: Cannot convert ROMClass to RAMClass. Cannot get the class chain of "
                    "ROMClass");
            }
        }
    } else {
        if (TR::Options::getCmdLineOptions()->getVerboseOption(TR_VerboseIProfilerPersistence)) {
            TR_VerboseLog::writeLineLocked(TR_Vlog_PERF,
                "loadFromPersistentCopy: Cannot convert ROMClass to RAMClass. Don't have required information in the "
                "entry");
        }
    }
...
void TR_IPBCDataCallGraph::copyFromEntry(TR_IPBytecodeHashTableEntry *originalEntry)
{
    TR_IPBCDataCallGraph *entry = (TR_IPBCDataCallGraph *)originalEntry;
    TR_ASSERT(originalEntry->asIPBCDataCallGraph(),
        "Incompatible types between storage and loading of iprofile persistent data");
    for (int32_t i = 0; i < NUM_CS_SLOTS; i++) {
 ...
 */
int32_t TR_IPBCDataDirectCall::setData(uintptr_t v, uint32_t freq)
{
    if (v != 0) {
        if (v > 0xffff) {
            _callCount = 0xffff;
...
void TR_IPBCDataDirectCall::copyFromEntry(TR_IPBytecodeHashTableEntry *originalEntry)
{
    TR_IPBCDataDirectCall *entry = (TR_IPBCDataDirectCall *)originalEntry;
    TR_ASSERT(originalEntry->asIPBCDataDirectCall(),
        "Incompatible types between storage and loading of iprofile persistent data");
...
#if defined(J9VM_OPT_JITSERVER)
void TR_IPBCDataDirectCall::serialize(uintptr_t methodStartAddress, TR_IPBCDataStorageHeader *storage,
    TR::PersistentInfo *info)
{
...
TR_IPBCDataCallGraph *TR_IProfiler::getCGProfilingData(TR_ByteCodeInfo &bcInfo, TR::Compilation *comp)
{
    if (!isIProfilingEnabled())
        return NULL;
...
TR_IPBCDataCallGraph *TR_IProfiler::getCGProfilingData(TR_OpaqueMethodBlock *method, uint32_t byteCodeIndex,
    TR::Compilation *comp)
{
    TR_IPBytecodeHashTableEntry *entry = profilingSample(method, byteCodeIndex, comp);
    ...
    if (entry) {
        if (invalidateEntryIfInconsistent(entry)) {
            logprints(traceIProfiling, comp->log(), "got nothing because it was invalidated\n");
            return NULL;
        }
...
void TR_IProfiler::setWarmCallGraphTooBig(TR_OpaqueMethodBlock *method, int32_t bcIndex, TR::Compilation *comp,
    bool set)
{
    ...
    if (entry) {
        ...
        // special/static invokes are not tracked by the IProfiler, yet we still want to set the warmCallGraphTooBig
        // flag. Create an artificial entry for the sole purpose of setting the warmCallGraphTooBig flag.
...
bool TR_IProfiler::isWarmCallGraphTooBig(TR_OpaqueMethodBlock *method, int32_t bcIndex, TR::Compilation *comp)
{
    if (comp->getOption(TR_DisableWarmCallGraphTooBigHeuristic))
        return false;
    TR_IPBytecodeHashTableEntry *entry = profilingSample(method, bcIndex, comp);
...
void TR_IProfiler::printAllocationReport()
{
    printf("Total allocation count %d\n", _globalAllocationCount);

    if (!_globalAllocationCount)
        return;

    for (int32_t bucket = 0; bucket < ALLOC_HASH_TABLE_SIZE; bucket++) {
#if defined(EXPERIMENTAL_IPROFILER)
        for (TR_IPBCDataAllocation *entry = _allocHashTable[bucket]; entry;
             entry = (TR_IPBCDataAllocation *)entry->next) {
            if (entry->asIPBCDataAllocation()) {
                ...
                if (method && clazz && !_compInfo->getPersistentInfo()->isUnloadedClass((void *)clazz, true)
                    && entry->getData() > 0) {
                    ...
                    printf("%d\t%5.2lf\t%.*s\t%.*s.%.*s%.*s\n", entry->getData(),
                        (double)((double)(entry->getData()) / ((double)_globalAllocationCount)) * 100.0,
                        J9UTF8_LENGTH(clazzUTRF8), J9UTF8_DATA(clazzUTRF8), J9UTF8_LENGTH(methodClazzUTRF8),
...
uint32_t TR_IProfiler::releaseAllEntries(uint32_t &unexpectedLockedEntries)
{
    uint32_t count = 0;
...
uint32_t TR_IProfiler::countEntries()
{
    uint32_t count = 0;
    for (int32_t bucket = 0; bucket < TR::Options::_iProfilerBcHashTableSize; bucket++)
        for (TR_IPBytecodeHashTableEntry *entry = _bcHashTable[bucket]; entry; entry = entry->getNext())
...
void TR_IProfiler::checkMethodHashTable()
{
    ...
    TR_StatsHisto<TR_IPMethodHashTableEntry::MAX_IPMETHOD_CALLERS - 1> faninHisto("Fanin caller list length histo", 1,
        TR_IPMethodHashTableEntry::MAX_IPMETHOD_CALLERS);
...
void TR_IProfiler::getFaninInfo(TR_OpaqueMethodBlock *calleeMethod, uint32_t *count, uint32_t *weight,
    uint32_t *otherBucketWeight)
{
    uint32_t i = 0;
    uint32_t w = 0;
    uint32_t other = 0;
 ...
 */
bool TR_IProfiler::getCallerWeight(TR_OpaqueMethodBlock *calleeMethod, TR_OpaqueMethodBlock *callerMethod,
    uint32_t *weight, uint32_t pcIndex, TR::Compilation *comp)
{
    ...
    for (TR_IPMethodData *it = &entry->_caller; it; it = it->next) {
        if (it->getMethod() == callerMethod
            && (!useTuples
                || ((((uintptr_t)it->getPCIndex()) + TR::Compiler->mtd.bytecodeStart(callerMethod)) == pcAddress))) {
            *weight = it->getWeight();
            return true;
        }
    }
...
void TR_IProfiler::startIProfilerThread(J9JavaVM *javaVM)
{
    ...
    if (_iprofilerMonitor) {
        ...
        if (javaVM->internalVMFunctions->createThreadWithCategory(&_iprofilerOSThread,
                TR::Options::_profilerStackSize << 10, priority, 0, &iprofilerThreadProc, javaVM->jitConfig,
...
// can process it
//
bool TR_IProfiler::postIprofilingBufferToWorkingQueue(J9VMThread *vmThread, const U_8 *dataStart, UDATA size)
{
    PORT_ACCESS_FROM_PORT(_portLib);
...
void TR_IProfiler::discardFilledIProfilerBuffers()
{
    while (!_workingBufferList.isEmpty()) {
...
void TR_IProfiler::processWorkingQueue()
{
    ...
    do {
        ...
        while (
            getIProfilerThreadLifetimeState() == TR_IProfiler::IPROF_THR_INITIALIZED && _workingBufferList.isEmpty()) {
        ...
        if (getIProfilerThreadLifetimeState() == TR_IProfiler::IPROF_THR_STOPPING) {
            ...
            TR_ASSERT_FATAL(_crtProfilingBuffer->getSize() > 0, "size of _crtProfilingBuffer (%p) <= 0",
                _crtProfilingBuffer);
...
#endif // defined(J9VM_OPT_CRIU_SUPPORT)
            {
                TR_ASSERT_FATAL(false,
                    "Iprofiler cannot be in state IPROF_THR_SUSPENDING if checkpoint is not allowed.\n");
            }
        } else {
...
static inline uint16_t readU16(U_8 *_offset)
{
    ...
    return hostIsLittleEndian ? (((U_16)(*_offset)) | (((U_16)(*(_offset + 1))) << 8))
...
UDATA TR_IProfiler::parseBuffer(J9VMThread *vmThread, const U_8 *dataStart, UDATA size, bool verboseReparse)
{
    ...
    if (javaVM->internalVMFunctions->isDebugOnRestoreEnabled(javaVM)
        ...
        int32_t dropRate
            = (int32_t)((((float)(_numRequestsDropped + _numRequestsSkipped)) / ((float)_numRequests)) * 1000);
        if (TR::Options::_IprofilerPreCheckpointDropRate >= 1000
            || dropRate <= TR::Options::_IprofilerPreCheckpointDropRate) {
            _numRequestsDropped++;
            return 0;
        }
    }
            ...
            case JBinstanceof:
                ...
                if (TEST_verbose || verboseReparse) {
                    ...
                    j9tty_printf(PORTLIB, "pc=%p (cast bc=%d) operand=%.*s(%p) cursor=%p\n", pc, *pc,
                        (UDATA)J9UTF8_LENGTH(name), J9UTF8_DATA(name), receiverClass, cursorCopy);
                }
                break;
            case JBinvokestatic:
            case JBinvokespecial:
...
void TR_IProfiler::jitProfileParseBuffer(J9VMThread *currentThread)
{
    ...
    if (!TR::Options::getCmdLineOptions()->getOption(TR_DisableIProfilerThread)
        && processProfilingBuffer(currentThread, dataStart, size)) {
...
static void printCsInfo(CallSiteProfileInfo &csInfo, TR::Compilation *comp, void *tag = NULL, ::FILE *fout = stderr)
{
    for (int32_t i = 0; i < NUM_CS_SLOTS; i++) {
        ...
        if (comp) {
            char *clazzSig
                = TR::Compiler->cls.classSignature(comp, (TR_OpaqueClassBlock *)csInfo.getClazz(i), comp->trMemory());
            fprintf(fout, "%p CLASS %d %#" OMR_PRIxPTR " %s WEIGHT %d\n", tag, i, csInfo.getClazz(i), clazzSig,
                csInfo._weight[i]);
        } else {
...
//
void *TR_IPHashedCallSite::operator new(size_t size) throw()
{
    memoryConsumed += (int32_t)size;
    return TR_IProfiler::allocator()->allocate(size, std::nothrow);
}

uintptr_t CallSiteProfileInfo::getDominantClass(int32_t &sumW, int32_t &maxW)
{
...
uint32_t CallSiteProfileInfo::getDominantSlot() const
{
...
TR_AggregationHT::TR_AggregationHTNode::TR_AggregationHTNode(J9ROMMethod *romMethod, J9ROMClass *romClass,
    TR_IPBytecodeHashTableEntry *entry)
    : _next(NULL)
...
TR_AggregationHT::TR_AggregationHT(size_t sz)
    : _sz(sz)
    , _numTrackedMethods(0)
{
...
void TR_AggregationHT::add(J9ROMMethod *romMethod, J9ROMClass *romClass, TR_IPBytecodeHashTableEntry *cgEntry)
{
    ...
    if (!crtMethodNode) {
        ...
        TR_AggregationHTNode *newMethodNode
            = new (*TR_IProfiler::allocator()) TR_AggregationHTNode(romMethod, romClass, cgEntry);
...
void TR_AggregationHT::sortByNameAndPrint(const char *filename)
{
    // Scan the aggregationTable and convert from romMethod to methodName so that
    // we can sort and print the information
    SortingPair *sortingArray
        = (SortingPair *)TR_IProfiler::allocator()->allocate(sizeof(SortingPair) * numTrackedMethods(), std::nothrow);
    if (!sortingArray) {
    ...
    for (int32_t bucket = 0; bucket < getSize(); bucket++) {
        ...
        for (; node; node = node->getNext()) {
            ...
            if (!wholeName) {
                fprintf(stderr,
                    "Warning: Cannot allocate memory. Incomplete data will be printed during IProfiler data "
                    "dumping.\n");
                break;
            }
            snprintf(wholeName, len, "%.*s.%.*s%.*s", J9UTF8_LENGTH(className), utf8Data(className),
        ...
        for (; ipEntry; ipEntry = ipEntry->getNext()) {
            ...
            if (ipbcDirectCallData) {
                fprintf(logFile, "\tOffset %" OMR_PRIuSIZE "\tDirectCall samples=%" OMR_PRIu32 " TooBig=%d\n", bcOffset,
                    ipbcDirectCallData->getNumSamples(), ipbcDirectCallData->isWarmCallGraphTooBig());
                continue;
            }
            ...
            switch (*pc) {
                case JBinvokeinterface:
                    fprintf(logFile, "JBinvokeinterface");
                    break;
                case JBinvokeinterface2:
                    fprintf(logFile, "JBinvokeinterface2");
                    break;
                case JBinvokevirtual:
                    fprintf(logFile, "JBinvokevirtual");
                    break;
                case JBinstanceof:
                    fprintf(logFile, "JBinstanceof");
                    break;
                case JBcheckcast:
                    fprintf(logFile, "JBcheckcast");
                    break;
                default:
                    fprintf(logFile, "JBunknown");
            }
            fprintf(logFile, " samples=%" OMR_PRIu32 " TooBig=%d\n", ipbcCGData->getNumSamples(),
                ipbcCGData->isWarmCallGraphTooBig());

            CallSiteProfileInfo *cgData = ipbcCGData->getCGData();
            for (int j = 0; j < NUM_CS_SLOTS; j++) {
                if (cgData->getClazz(j)) {
                    int32_t len;
                    const char *s = utf8Data(
                        J9ROMCLASS_CLASSNAME(TR::Compiler->cls.romClassOf((TR_OpaqueClassBlock *)cgData->getClazz(j))),
                        len);
                    fprintf(logFile, "\t\tW:%4u\tM:%#" OMR_PRIxPTR "\t%.*s\n", cgData->_weight[j], cgData->getClazz(j),
                        len, s);
                }
            }
...
J9ROMMethod *TR_IProfiler::findROMMethodFromPC(J9VMThread *vmThread, uintptr_t methodPC, J9ROMClass *&romClass)
{
...
void TR_IProfiler::traverseIProfilerTableAndCollectEntries(TR_AggregationHT *aggregationHT, J9VMThread *vmThread,
    bool collectOnlyCallGraphEntries)
{
    J9JavaVM *javaVM = vmThread->javaVM;
    J9InternalVMFunctions *vmFunctions = javaVM->internalVMFunctions;
...
void TR_IProfiler::dumpAllBytecodeProfilingData(J9VMThread *vmThread)
{
...
uintptr_t TR_IProfiler::createBalancedBST(TR_IPBytecodeHashTableEntry **ipEntries, int32_t low, int32_t high,
    uintptr_t memChunk, TR_J9SharedCache *sharedCache)
{
    if (high < low)
        return 0;
...
void TR_IProfiler::persistAllEntries()
{
    J9JavaVM *javaVM = _compInfo->getJITConfig()->javaVM;
    ...
    try {
        ...
        if (aggregationHT.getSize() == 0) // OOM
        {
            if (TR::Options::getCmdLineOptions()->getVerboseOption(TR_VerboseIProfilerPersistence))
                TR_VerboseLog::writeLineLocked(TR_Vlog_PERF,
                    "IProfiler: Cannot allocate memory. Bailing out persisting all entries to SCC");
            return;
        }
        ...
        for (int32_t bucket = 0; bucket < aggregationHT.getSize(); bucket++) {
            for (TR_AggregationHT::TR_AggregationHTNode *node = aggregationHT.getBucket(bucket); node;
                 node = node->getNext()) {
                _STATS_methodPersistenceAttempts++;
                ...
                for (TR_AggregationHT::TR_IPChainedEntry *ipEntry = node->getFirstIPEntry(); ipEntry;
                     ipEntry = ipEntry->getNext()) {
        ...
        if (TR::Options::getCmdLineOptions()->getVerboseOption(TR_VerboseIProfilerPersistence))
            TR_VerboseLog::writeLineLocked(TR_Vlog_PERF,
                "IProfiler: persisted a total of %d entries, of which %d were persisted at shutdown",
...
void TR_IProfiler::traverseIProfilerTableAndGenerateHistograms(J9JITConfig *jitConfig)
{
...
#if defined(J9VM_OPT_CRIU_SUPPORT)
void TR_IProfiler::suspendIProfilerThreadForCheckpoint()
{
    ...
    if (_compInfo->getCRRuntime()->shouldSuspendThreadsForCheckpoint()) {
        ...
        TR_ASSERT_FATAL(getIProfilerThreadLifetimeState() == TR_IProfiler::IPROF_THR_SUSPENDING,
            "IProfiler Lifetime State is %d!", getIProfilerThreadLifetimeState());
        ...
        if (getIProfilerThreadLifetimeState() == TR_IProfiler::IPROF_THR_RESUMING) {
            ...
            if (TR::Options::isAnyVerboseOptionSet())
                TR_VerboseLog::writeLineLocked(TR_Vlog_CHECKPOINT_RESTORE,
                    "IProfiler Thread Lifetime State is %p which is not %p!", getIProfilerThreadLifetimeState(),
