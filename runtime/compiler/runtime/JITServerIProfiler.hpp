...
struct TR_ContiguousIPMethodHashTableEntry {
    ...
    TR_ContiguousIPMethodData
        _callers[TR_IPMethodHashTableEntry::MAX_IPMETHOD_CALLERS]; // array of callers and their weights. null _method
                                                                   // means EOL
...
public:
    ...
    virtual TR_IPBytecodeHashTableEntry *profilingSample(uintptr_t pc, uintptr_t data, bool addIt,
        bool isRIData = false, uint32_t freq = 1) override;
    ...
    static TR_IPBytecodeHashTableEntry *ipBytecodeHashTableEntryFactory(TR_IPBCDataStorageHeader *storage, uintptr_t pc,
        TR_Memory *mem, TR_AllocationKind allocKind);
    // This is used for fanin data
    TR_FaninSummaryInfo *cacheFaninDataForMethod(TR_OpaqueMethodBlock *method, const std::string &clientFaninData,
        TR_FrontEnd *fe, TR_Memory *trMemory);
    TR_FaninSummaryInfo *deserializeFaninMethodEntry(const TR_ContiguousIPMethodHashTableEntry *serialEntry,
        TR_Memory *trMemory);
    // This is used for bytecode profile data
    static void deserializeIProfilerData(J9Method *method, const std::string &ipdata,
        Vector<TR_IPBytecodeHashTableEntry *> &ipEntries, TR_Memory *trMemory, bool cgEntriesOnly = false);
    void printStats();
...
private:
    void validateCachedIPEntry(TR_IPBytecodeHashTableEntry *entry, TR_IPBCDataStorageHeader *clientData,
    ...
    bool cacheProfilingDataForMethod(TR_OpaqueMethodBlock *method, const std::string &ipdata, bool usePersistentCache,
        ClientSessionData *clientSessionData, TR::CompilationInfoPerThreadRemote *compInfoPT, bool isCompiled,
        TR::Compilation *comp);
    bool _useCaching;
    // Statistics
...
public:
    ...
    bool serializeAndSendIProfileInfoForMethod(TR_OpaqueMethodBlock *method, TR::Compilation *comp,
...
private:
    uint32_t walkILTreeForIProfilingEntries(uintptr_t *pcEntries, uint32_t &numEntries,
    ...
    uintptr_t serializeIProfilerMethodEntries(const uintptr_t *pcEntries, uint32_t numEntries, uintptr_t memChunk,
        uintptr_t methodStartAddress, TR::Compilation *comp, bool sharedProfile, uint64_t &totalSamples,
        std::vector<J9Class *> &uncachedClasses, std::vector<JITServerHelpers::ClassInfoTuple> &classInfos);
};
