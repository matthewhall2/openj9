...
MetricsDatabase::MetricsDatabase(TR::CompilationInfo *compInfo)
    : _compInfo(compInfo)
{
...
std::string MetricsDatabase::serializeMetrics()
{
    std::string output;
    for (int i = 0; i < MAX_METRICS; i++) {
...
HttpGetRequest::ReturnCodes HttpGetRequest::readHttpGetRequest()
{
        ...
        if (bytesRead < 4) {
            if (TR::Options::getVerboseOption(TR_VerboseJITServer))
                TR_VerboseLog::writeLineLocked(TR_Vlog_JITServer,
                    "MetricsServer: Too few bytes received when reading from socket  %d", socket);
            return READ_ERROR;
        }
...
HttpGetRequest::ReturnCodes HttpGetRequest::parseHttpGetRequest()
{
    ...
    if (strncmp(_buf + pos, HTTP_VERSION_HEADER_STRING, HTTP_VERSION_HEADER_STRING_SIZE))
        return INVALID_HTTP_PROTOCOL;
    memcpy(_httpVersion, _buf + pos + HTTP_VERSION_HEADER_STRING_SIZE,
        protocolLength - HTTP_VERSION_HEADER_STRING_SIZE);
...
HttpGetRequest::ReturnCodes HttpGetRequest::sendHttpResponse()
{
    int bytesWritten = 0;
...
MetricsServer::MetricsServer()
    ...
    , _metricsOSThread(NULL)
    , _metricsThreadAttachAttempted(false)
    , _metricsThreadExitFlag(false)
    , _jitConfig(NULL)
    , _sslCtx(NULL)
{
...
void MetricsServer::startMetricsThread(J9JavaVM *javaVM)
{
    ...
    if (_metricsMonitor) {
        ...
        if (J9THREAD_SUCCESS
            != javaVM->internalVMFunctions->createJoinableThreadWithCategory(&_metricsOSThread, defaultOSStackSize,
                priority, 0, &metricsThreadProc, javaVM->jitConfig,
                J9THREAD_CATEGORY_SYSTEM_JIT_THREAD)) { // cannot create the MetricsServer thread
...
int32_t MetricsServer::waitForMetricsThreadExit(J9JavaVM *javaVM)
{
...
int MetricsServer::openSocketForListening(uint32_t port)
{
...
void MetricsServer::reArmSocketForReading(int sockIndex)
{
    _pfd[sockIndex].events = POLLIN;
    _pfd[sockIndex].revents = 0;
}

void MetricsServer::reArmSocketForWriting(int sockIndex)
{
    _pfd[sockIndex].events = POLLOUT;
    _pfd[sockIndex].revents = 0;
}

void MetricsServer::closeSocket(int sockIndex)
{
...
void MetricsServer::handleConnectionRequest()
{
    static const int LISTEN_SOCKET = 0;
    TR_ASSERT_FATAL(_pfd[LISTEN_SOCKET].revents == POLLIN,
        "MetricsServer: Unexpected revent occurred during poll for new connection: revents=%d\n",
    ...
    if (sockfd >= 0) // success
    {
        ...
        if (k >= 1 + MAX_CONCURRENT_REQUESTS) {
            ...
            if (TR::Options::getVerboseOption(TR_VerboseJITServer))
                TR_VerboseLog::writeLineLocked(TR_Vlog_JITServer,
                    "MetricsServer error: could not find an available socket to process a request");
        }
    } else // accept() failed
    {
        if ((EAGAIN != errno) && (EWOULDBLOCK != errno)) {
            if (TR::Options::getVerboseOption(TR_VerboseJITServer))
                TR_VerboseLog::writeLineLocked(TR_Vlog_JITServer,
                    "MetricsServer error: cannot accept connection: errno=%d", errno);
        }
    }
...
void MetricsServer::handleDataForConnectedSocket(nfds_t sockIndex, MetricsDatabase &metricsDatabase)
{
    ...
    if (_pfd[sockIndex].revents & (POLLRDHUP | POLLERR | POLLHUP | POLLNVAL)) {
        ...
        switch (_requests[sockIndex].acceptSSLConnection()) {
            ...
            default: // An error occurred
                if (TR::Options::getVerboseOption(TR_VerboseJITServer)) {
                    TR_VerboseLog::writeLineLocked(TR_Vlog_JITServer,
                        "MetricsServer error on socket %d: Unable to establish SSL Connection", _pfd[sockIndex].fd);
                }
                closeSocket(sockIndex);
                break;
        }
        ...
        switch (rc) {
            ...
            default: // An error occurred
                if (TR::Options::getVerboseOption(TR_VerboseJITServer)) {
                    TR_VerboseLog::writeLineLocked(TR_Vlog_JITServer,
                        "MetricsServer experienced error code %d on socket index %u", rc, sockIndex);
                }
...
void MetricsServer::serveMetricsRequests()
{
    ...
    if (useSSL(compInfo)) {
        ...
        if (!JITServer::ServerStream::createSSLContext(_sslCtx, sessionContextId, sizeof(sessionContextId), sslKeys,
                sslCerts, sslRootCerts)) {
            if (TR::Options::getVerboseOption(TR_VerboseJITServer)) {
                TR_VerboseLog::writeLineLocked(TR_Vlog_JITServer,
                    "Cannot create MetricsServer SSL context. Will continue without metrics.");
            }
            return;
        }
    }
    ...
    if (TR::Options::getVerboseOption(TR_VerboseJITServer)) {
        const char *logMessage = _sslCtx ? "MetricsServer waiting for https requests on port %u"
                                         : "MetricsServer waiting for http requests on port %u";
...
std::string MetricsServer::messageForErrorCode(int err)
{
    std::string response;
    switch (err) {
