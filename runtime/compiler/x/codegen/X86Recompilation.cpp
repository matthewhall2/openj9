...
TR::Instruction *TR_X86Recompilation::generatePrePrologue()
{
    ...
    if (cg()->comp()->target().is64Bit())
        alignmentMargin += SAVE_AREA_SIZE + JITTED_BODY_INFO_SIZE
            + LINKAGE_INFO_SIZE; // save area for the first two bytes of the method + jitted body info pointer +
                                 // linkageinfo
    else
...
void TR_X86Recompilation::postCompilation() { setMethodReturnInfoBits(); }
