...
// On X86-64 Windows, Microsoft x64 calling convention is an extension of fastcall; and hence this code goes through
// USE_FASTCALL path On X86-64 Linux, the linkage is System V AMD64 ABI.
...
#endif
const size_t J9::X86::HelperCallSite::NumberOfIntParamRegisters
    = sizeof(IntParamRegisters) / sizeof(IntParamRegisters[0]);
const size_t J9::X86::HelperCallSite::StackIndexAdjustment
    = RegisterParameterShadowOnStack ? 0 : NumberOfIntParamRegisters;
...
{
    uint32_t ret = 0;
    for (size_t i = 0; i < sizeof(J9::X86::HelperCallSite::CalleeSavedRegisters)
             / sizeof(J9::X86::HelperCallSite::CalleeSavedRegisters[0]);
         i++) {
...
const uint32_t J9::X86::HelperCallSite::PreservedRegisterMapForGC
    = X86HelperCallSiteCalculatePreservedRegisterMapForGC();
...
TR::Register *J9::X86::HelperCallSite::BuildCall()
{
    ...
    for (size_t i = 0; i < sizeof(CallerSavedRegisters) / sizeof(CallerSavedRegisters[0]); i++) {
    ...
    // Stack adjustment
    {
        int SizeOfParamOnStack
            = StackSlotSize * (NumberOfParamOnStack + NumberOfIntParamRegisters - StackIndexAdjustment);
        if (CalleeCleanup) {
