...
bool TR::X86PicDataSnippet::shouldEmitJ2IThunkPointer()
{
    ...
    TR_OpaqueClassBlock *interfaceClass = owningMethod->getResolvedInterfaceMethod(cpIndex, &itableIndex);
    ...
    *(uintptr_t *)cursor = cpAddr;
    ...
    if (_hasJ2IThunkInPicData) {
        ...
        cg()->addExternalRelocation(
            TR::ExternalRelocation::create(cursor, (uint8_t *)info, NULL, TR_J2IVirtualThunkPointer, cg()), __FILE__,
            __LINE__, _startOfPicInstruction->getNode());
    } else if (_thunkAddress) {
        TR_ASSERT(comp->target().is64Bit(), "expecting a 64-bit target for thunk relocations");
        cg()->addExternalRelocation(
            TR::ExternalRelocation::create(cursor, *(uint8_t **)cursor, (uint8_t *)inlinedSiteIndex, TR_Thunks, cg()),
            __FILE__, __LINE__, _startOfPicInstruction->getNode());
    } else {
        cg()->addExternalRelocation(TR::ExternalRelocation::create(cursor, (uint8_t *)cpAddr,
                                        (uint8_t *)inlinedSiteIndex, TR_ConstantPool, cg()),
            __FILE__, __LINE__, _startOfPicInstruction->getNode());
    }
...
uint8_t *TR::X86PicDataSnippet::emitSnippetBody()
{
    ...
    if (isInterface()) {
        ...
        uintptr_t alignedIpicDataStart = (unalignedIpicDataStart + alignMask) & ~alignMask;
        ...
        *(int32_t *)(++cursor) = disp32;
        ...
        cg()->addExternalRelocation(
            TR::ExternalRelocation::create(cursor, (uint8_t *)_dispatchSymRef, TR_HelperAddress, cg()), __FILE__,
            __LINE__, _startOfPicInstruction->getNode());
        cursor += 4;
        ...
        *(int32_t *)cursor = disp32;
        ...
        // to them are atomic.
        TR_ASSERT_FATAL(((uintptr_t)cursor & (sizeof(uintptr_t) - 1)) == 0,
        ...
        *(uintptr_t *)cursor = 0;
        ...
        if (unresolvedDispatch()) {
            ...
            intptr_t entryPoint = ((intptr_t)cursor
                + ((3 * sizeof(uintptr_t)) + (hasJ2IThunkInPicData() ? sizeof(uintptr_t) : 0)
                    + (comp->target().is64Bit() ? 4 : 1)));

            intptr_t requiredEntryPoint = (entryPoint + (cg()->getLowestCommonCodePatchingAlignmentBoundary() - 1)
    ...
    if (_numberOfSlots >= 1) {
        ...
        TR::SymbolReference *resolveSlotHelperSymRef = cg()->symRefTab()->findOrCreateRuntimeHelper(resolveSlotHelper);
        ...
        *(int32_t *)(++picSlotCursor) = disp32;
        ...
        cg()->addExternalRelocation(
            TR::ExternalRelocation::create(picSlotCursor, (uint8_t *)resolveSlotHelperSymRef, TR_HelperAddress, cg()),
            __FILE__, __LINE__, _startOfPicInstruction->getNode());
        ...
        while (--numPicSlots) {
            ...
            *(int32_t *)(++picSlotCursor) = disp32;
            ...
            cg()->addExternalRelocation(TR::ExternalRelocation::create(picSlotCursor,
                                            (uint8_t *)populateSlotHelperSymRef, TR_HelperAddress, cg()),
                __FILE__, __LINE__, _startOfPicInstruction->getNode());
            picSlotCursor = (uint8_t *)(picSlotCursor - 1 + sizeofPicSlot);
        }
    }
...
void TR_Debug::print(OMR::Logger *log, TR::X86PicDataSnippet *snippet)
{
    ...
    if (snippet->isInterface()) {
        ...
        log->printf("call\t%s \t\t%s " POINTER_PRINTF_FORMAT, getName(dispatchSymRef), commentString(),
        ...
        if (methodSymRef->isUnresolved()) {
            ...
            log->printf("%s\t" POINTER_PRINTF_FORMAT "\t\t%s owning method cpAddr", op, (void *)*(uintptr_t *)bufferPos,
                commentString());
            bufferPos += sizeof(uintptr_t);

            printPrefix(log, NULL, bufferPos, sizeof(uintptr_t));
            log->printf("%s\t" POINTER_PRINTF_FORMAT "\t\t%s cpIndex", op, (void *)*(uintptr_t *)bufferPos,
                commentString());
            bufferPos += sizeof(uintptr_t);

            printPrefix(log, NULL, bufferPos, sizeof(uintptr_t));
            log->printf("%s\t" POINTER_PRINTF_FORMAT "\t\t%s interface class (initially null)", op,
                (void *)*(uintptr_t *)bufferPos, commentString());
            bufferPos += sizeof(uintptr_t);

            printPrefix(log, NULL, bufferPos, sizeof(uintptr_t));
            log->printf("%s\t" POINTER_PRINTF_FORMAT "\t\t%s itable offset%s (initially zero)", op,
                (void *)*(uintptr_t *)bufferPos, commentString(),
            ...
            if (_comp->target().is64Bit()) {
                ...
                log->printf("%s\t%s%02x%s\t\t\t\t\t\t\t\t%s REX of MOVRegImm64", dbString(), hexPrefixString(),
                    *bufferPos, hexSuffixString(), commentString());
                bufferPos += 1;

                printPrefix(log, NULL, bufferPos, 1);
                log->printf("%s\t%02x\t\t\t\t\t\t\t\t%s MOV opcode of MOVRegImm64", dbString(), *bufferPos,
                    commentString());
                bufferPos += 1;

                if (snippet->hasJ2IThunkInPicData()) {
                    printPrefix(log, NULL, bufferPos, sizeof(uintptr_t));
                    log->printf("%s\t" POINTER_PRINTF_FORMAT "\t\t%s j2i virtual thunk", op,
                        (void *)*(uintptr_t *)bufferPos, commentString());
                    bufferPos += sizeof(uintptr_t);
                }
            } else {
        ...
        if (snippet->unresolvedDispatch()) {
            ...
            if (_comp->target().is64Bit()) {
                printPrefix(log, NULL, bufferPos, 1);
                log->printf("%s\t%02x\t\t\t\t\t\t\t\t%s REX of MOVRegImm64", dbString(), *bufferPos, commentString());
                bufferPos += 1;
                ...
                printPrefix(log, NULL, bufferPos, 1);
                log->printf("%s\t%02x\t\t\t\t\t\t\t\t%s REX of CallMem", dbString(), *bufferPos, commentString());
                bufferPos += 1;
            ...
            printPrefix(log, NULL, bufferPos, sizeof(uintptr_t));
            log->printf("%s\t" POINTER_PRINTF_FORMAT "\t\t%s direct J9Method (initially null)", op,
                (void *)*(uintptr_t *)bufferPos, commentString());
            bufferPos += sizeof(uintptr_t);
        ...
        log->printf("call\t%s \t\t%s " POINTER_PRINTF_FORMAT "\tpatched with vtable call", getName(dispatchSymRef),
            commentString(), dispatchSymRef->getMethodAddress());
        bufferPos += length;
...
uint32_t TR::X86PicDataSnippet::getLength(int32_t estimatedSnippetStart)
{
    ...
    if (isInterface()) {
        ...
        return 6 // CALL [Mem] (pessimistically assume a SIB is needed)
            ...
            //  1 (ModRM for CMP)
            //
            + (comp->target().is64Bit() ? 4 : 1) + cg()->getLowestCommonCodePatchingAlignmentBoundary() - 1;
    }
}

uint8_t *TR::X86CallSnippet::alignCursorForCodePatching(uint8_t *cursor, bool alignWithNOPs)
{
    intptr_t alignedCursor = ((intptr_t)cursor + (cg()->getLowestCommonCodePatchingAlignmentBoundary() - 1)
        & (intptr_t)(~(cg()->getLowestCommonCodePatchingAlignmentBoundary() - 1)));
...
uint8_t *TR::X86CallSnippet::emitSnippetBody()
{
    ...
    if (comp->target().is64Bit()) {
        ...
        if (cg()->hasCodeCacheSwitched() && methodSymRef->getReferenceNumber() >= TR_AMD64numRuntimeHelpers
    ...
    if (methodSymRef->isUnresolved() || forceUnresolvedDispatch) {
        ...
        TR_ASSERT((methodSymbol->isStatic() || methodSymbol->isSpecial() || forceUnresolvedDispatch),
            "Unexpected unresolved dispatch");

        // CALL interpreterUnresolved{Static|Special}Glue
        //
        TR_RuntimeHelper resolutionHelper
            = methodSymbol->isStatic() ? TR_X86interpreterUnresolvedStaticGlue : TR_X86interpreterUnresolvedSpecialGlue;
        ...
        *(int32_t *)(++cursor) = disp32;
        ...
        cg()->addExternalRelocation(
            TR::ExternalRelocation::create(cursor, (uint8_t *)helperSymRef, TR_HelperAddress, cg()), __FILE__, __LINE__,
            getNode());
        cursor += 4;
        ...
        *(uint32_t *)cursor = methodSymRef->getCPIndexForVM();
        ...
        if (!isJitInduceOSRCall && !isJitDispatchJ9Method) {
            ...
            intptr_t ramMethod = comp->isOutOfProcessCompilation() && !methodSymbol->isInterpreted()
                ? (intptr_t)methodSymRef->getSymbol()
        ...
        *cursor = 0xe9;
        ...
        if (isJitInduceOSRCall) {
            ...
            // wasn't one.
            TR_RuntimeHelper helper = isJitDispatchJ9Method ? TR_j2iTransition : TR_X86interpreterStaticAndSpecialGlue;
        ...
        *(int32_t *)(++cursor) = disp32;
        ...
        cg()->addExternalRelocation(
            TR::ExternalRelocation::create(cursor, (uint8_t *)dispatchSymRef, TR_HelperAddress, cg()), __FILE__,
            __LINE__, getNode());
        cursor += 4;
    }
