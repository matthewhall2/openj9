...
public:
    ...
    X86PICSlot(uintptr_t classAddress, TR_ResolvedMethod *method, bool jumpToDone = true,
        TR_OpaqueMethodBlock *m = NULL, int32_t slot = -1)
        : _classAddress(classAddress)
        , _method(method)
        , _helperMethodSymbolRef(NULL)
        , _branchType(BranchJNE)
        , _methodAddress(m)
        , _slot(slot)
    {
        if (jumpToDone)
            setNeedsJumpToDone(); // TODO: Remove this oddball.  We can tell whether we need a dump to done based on
                                  // whether a doneLabel is passed to buildPICSlot
    }
    ...
    void setHelperMethodSymbolRef(TR::SymbolReference *symRef) { _helperMethodSymbolRef = symRef; }
...
public:
    ...
    void addPreCondition(TR::Register *virtualReg, TR::RealRegister::RegNum realReg)
    {
        _preConditionsUnderConstruction->unionPreCondition(virtualReg, realReg, cg());
    }

    void addPostCondition(TR::Register *virtualReg, TR::RealRegister::RegNum realReg)
    {
        _postConditionsUnderConstruction->unionPostCondition(virtualReg, realReg, cg());
    }
    ...
    {
        return getResolvedMethodSymbol() ? getResolvedMethodSymbol()->getResolvedMethod() : NULL;
    }
    ...
    TR_ResolvedMethod *getDevirtualizedMethod()
    {
        return _devirtualizedMethod;
    } // The method to be dispatched statically
...
public:
    ...
    void setArgSize(int32_t s)
    {
        TR_ASSERT(!argsHaveBeenBuilt(), "assertion failure");
        _argSize = s;
    }
...
public:
    ...
    //
    virtual void buildDirectCall(TR::SymbolReference *methodSymRef, // NOTE: the methodSymRef being called is not
                                                                    // necessarily site.getSymbolReference()
        TR::X86CallSite &site,
    ...
    virtual void buildVirtualOrComputedCall(TR::X86CallSite &site, TR::LabelSymbol *entryLabel,
        ...
        = 0;
    virtual void buildInterfaceCall(TR::X86CallSite &site, TR::LabelSymbol *entryLabel, TR::LabelSymbol *doneLabel,
        uint8_t *thunk);
    ...
    virtual bool buildVirtualGuard(TR::X86CallSite &site,
        TR::LabelSymbol *revirtualizeLabel); // returns false if it can't build the guard
    virtual void buildRevirtualizedCall(TR::X86CallSite &site, TR::LabelSymbol *revirtualizeLabel,
    ...
    virtual void buildIPIC(TR::X86CallSite &site, TR::LabelSymbol *entryLabel, TR::LabelSymbol *doneLabel,
        uint8_t *thunk)
        = 0;
    virtual TR::Instruction *buildVFTCall(TR::X86CallSite &site, TR::InstOpCode dispatchOp,
        TR::Register *targetAddressReg, TR::MemoryReference *targetAddressMemref);
    virtual void buildInterfaceDispatchUsingLastITable(TR::X86CallSite &site, int32_t numIPicSlots,
    ...
    {
        TR_ASSERT(0, "Thunks not implemented in X86Linkage");
        return NULL;
    }
