...
void J9::X86::Linkage::alignLocalObjectWithCollectedFields(uint32_t &stackIndex)
{
    ...
    // Offset of a collected local object determined by (stackIndex +
    // pointerSize*(localCursor->getGCMapIndex()-firstLocalGCIndex)) In createStackAtlas, we align
    // pointerSize*(localCursor->getGCMapIndex()-firstLocalGCIndex) by modifying local objects' gc indice Here we align
    // the stackIndex
    //
