    ...
    if (helperAddress && IS_32BIT_RIP(helperAddress, (buffer + 4))) {
        ...
        if (fej9->needRelocationsForHelpers()) {
            cg()->addExternalRelocation(
                TR::ExternalRelocation::create(buffer, (uint8_t *)helperSymRef, TR_HelperAddress, cg()), __FILE__,
                __LINE__, getNode());
        }
    }
...
TR_X86AllocPrefetchGeometry TR::X86AllocPrefetchSnippet::generatePrefetchGeometry()
{
    ...
    return TR_X86AllocPrefetchGeometry(TR::Options::_TLHPrefetchLineSize, TR::Options::_TLHPrefetchLineCount,
        TR::Options::_TLHPrefetchStaggeredLineCount, TR::Options::_TLHPrefetchBoundaryLineCount,
...
template<TR::HeapTypes::Type HEAP_TYPE> class HeapProperties {
private:
...
public:
    ...
    static bool needWideDisplacementForTLHPrefetchCount()
    {
        return (offsetOfTLHPrefetchCount() > 127 || offsetOfTLHPrefetchCount() < -128);
    }
};
...
uint8_t *TR::X86AllocPrefetchSnippet::emitSharedBody(uint8_t *prefetchSnippetBuffer, TR::Compilation *comp)
{
    ...
    if (printCodeCacheSnippetAddress) {
        fprintf(stdout, "%s Allocation snippet is at address %p, size=%d\n", TR::HeapTypes::getPrefix(HEAP_TYPE),
            prefetchSnippetBuffer, sizeOfSharedBody<HEAP_TYPE, is64Bit>());
        fflush(stdout);
    }
...
template<TR::HeapTypes::Type HEAP_TYPE, bool is64Bit> int32_t TR::X86AllocPrefetchSnippet::sizeOfSharedBody()
{
    typedef HeapProperties<HEAP_TYPE> HeapTraits;
    ...
    /*
     * TODO: Refactor the alignment value to use a common definition either from the code cache or from some form of
     * target query.
     */
...
uint32_t TR::getCCPreLoadedCodeSize()
{
    ...
    uint32_t sizeOfZeroedPrefetchBody
        = TR::X86AllocPrefetchSnippet::sizeOfSharedBody<TR::HeapTypes::ZeroedHeap, true>();
    uint32_t sizeOfNonZeroedPrefetchBody
        = TR::X86AllocPrefetchSnippet::sizeOfSharedBody<TR::HeapTypes::NonZeroedHeap, true>();
#else
    uint32_t sizeOfZeroedPrefetchBody
        = TR::X86AllocPrefetchSnippet::sizeOfSharedBody<TR::HeapTypes::ZeroedHeap, false>();
    uint32_t sizeOfNonZeroedPrefetchBody
        = TR::X86AllocPrefetchSnippet::sizeOfSharedBody<TR::HeapTypes::NonZeroedHeap, false>();
#endif
    ...
    TR::CodeGenerator *cg)
{
    ...
    TR_ASSERT(cursor == CCPreLoadedCodeTop,
        "The expected and actual sizes of the emitted code differ. cursor = %p, CCPreLoadedCodeTop = %p", cursor,
        CCPreLoadedCodeTop);
}
