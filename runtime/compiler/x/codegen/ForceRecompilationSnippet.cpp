...
uint8_t *TR::X86ForceRecompilationSnippet::emitSnippetBody()
{
    ...
    TR::SymbolReference *helper = cg()->symRefTab()->findOrCreateRuntimeHelper(
        cg()->comp()->target().is64Bit() ? TR_AMD64induceRecompilation : TR_IA32induceRecompilation);
    ...
    *buffer = 0xe8; // CallImm4
    if (cg()->directCallRequiresTrampoline(helperAddress, reinterpret_cast<intptr_t>(buffer++))) {
        helperAddress
            = TR::CodeCacheManager::instance()->findHelperTrampoline(helper->getReferenceNumber(), (void *)buffer);
    ...
    *(int32_t *)buffer = ((uint8_t *)helperAddress - buffer) - 4;
    ...
    cg()->addExternalRelocation(TR::ExternalRelocation::create(buffer, (uint8_t *)helper, TR_HelperAddress, cg()),
...
void TR_Debug::print(OMR::Logger *log, TR::X86ForceRecompilationSnippet *snippet)
{
    ...
    log->printf("call\t%s \t%s Helper Address = " POINTER_PRINTF_FORMAT, getName(helper), commentString(),
        helperAddress);
    bufferPos += 5;
    ...
    log->printf("%s  \t%s%08x%s\t\t%s Offset to startPC", ddString(), hexPrefixString(),
        _cg->getCodeStart() - offsetBase, hexSuffixString(), commentString());
    bufferPos += 4;
}

uint32_t TR::X86ForceRecompilationSnippet::getLength(int32_t estimatedSnippetStart) { return 14; }
