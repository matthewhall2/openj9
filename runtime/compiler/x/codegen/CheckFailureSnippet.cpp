...
void TR_Debug::print(OMR::Logger *log, TR::X86CheckFailureSnippet *snippet)
{
    ...
    if (snippet->getRequiredFPstackPop()) {
        ...
        log->printf("fstp\tst(0)\t\t%s Discard top of FP stack", commentString());
        bufferPos += 2;
    }

    printPrefix(log, NULL, bufferPos, 5);
    log->printf("call\t%s \t\t%s Helper Address = " POINTER_PRINTF_FORMAT, getName(symRef), commentString(),
        sym->getMethodAddress());
    bufferPos += 5;

    printPrefix(log, NULL, bufferPos, 4);
    log->printf("%s \t%s%08x%s", ddString(), hexPrefixString(),
        bufferPos - snippet->getCheckInstruction()->getBinaryEncoding(), hexSuffixString());
}
...
uint8_t *TR::X86CheckFailureSnippet::emitCheckFailureSnippetBody(uint8_t *buffer)
{
    ...
    *buffer = 0xe8; // CallImm4
    ...
    if (cg()->directCallRequiresTrampoline(destinationAddress, reinterpret_cast<intptr_t>(buffer++))) {
        destinationAddress = TR::CodeCacheManager::instance()->findHelperTrampoline(
            getDestination()->getReferenceNumber(), (void *)buffer);
        TR_ASSERT(IS_32BIT_RIP(destinationAddress, buffer + 4),
...
uint32_t TR::X86BoundCheckWithSpineCheckSnippet::getLength(int32_t estimatedSnippetStart) { return 0; }
...
void TR_Debug::print(OMR::Logger *log, TR::X86BoundCheckWithSpineCheckSnippet *snippet)
{
    TR::SymbolReference *symRef = snippet->getDestination();
    TR::MethodSymbol *sym = symRef->getSymbol()->castToMethodSymbol();
...
uint32_t TR::X86SpineCheckSnippet::getLength(int32_t estimatedSnippetStart) { return 0; }
...
void TR_Debug::print(OMR::Logger *log, TR::X86SpineCheckSnippet *snippet)
{
    TR::SymbolReference *symRef = snippet->getDestination();
    TR::MethodSymbol *sym = symRef->getSymbol()->castToMethodSymbol();
...
uint8_t *TR::X86CheckFailureSnippetWithResolve::emitSnippetBody()
{
    ...
    *(int32_t *)buffer = (int32_t)(intptr_t)getDataSymbolReference()->getOwningMethod(cg()->comp())->constantPool();
    cg()->addExternalRelocation(TR::ExternalRelocation::create(buffer, *(uint8_t **)buffer,
                                    getCheckInstruction()->getNode()
                                        ? (uint8_t *)(uintptr_t)getCheckInstruction()->getNode()->getInlinedSiteIndex()
                                        : (uint8_t *)-1,
                                    TR_ConstantPool, cg()),
    ...
    *buffer = 0xe8; // call  Imm4 glue routine
    ...
    if (cg()->directCallRequiresTrampoline(glueAddress, reinterpret_cast<intptr_t>(buffer++))) {
        glueAddress
            = TR::CodeCacheManager::instance()->findHelperTrampoline(glueSymRef->getReferenceNumber(), (void *)buffer);
    ...
    *(int32_t *)buffer = (int32_t)(glueAddress - (intptr_t)(buffer + 4));
    cg()->addExternalRelocation(TR::ExternalRelocation::create(buffer, (uint8_t *)glueSymRef, TR_HelperAddress, cg()),
        __FILE__, __LINE__, getCheckInstruction()->getNode());
    ...
    *(uint32_t *)buffer = (uint32_t)(buffer - checkSite);
    ...
    // and associate it with the jump instruction.  The flags on the instruction and snippet are set back to reflect
    // this. The real fix is to change the helper and snippet to not do the return code adjust,  have the return address
    // be the snippet and associate the gc map with the snippet.
    //
    gcMap().registerStackMap(checkSite, cg());
    if (gcMap().getStackMap()) {
        getCheckInstruction()->setNeedsGCMap();
...
void TR_Debug::print(OMR::Logger *log, TR::X86CheckFailureSnippetWithResolve *snippet)
{
    enum {
        longPushTag = 0x00800000,
        dontPatchTag = 0x00400000
    ...
    printPrefix(log, NULL, bufferPos, size);
    log->printf("push\t" POINTER_PRINTF_FORMAT "\t\t%s push cpIndex", cpIndexValue, commentString());
    bufferPos += size;

    printPrefix(log, NULL, bufferPos, 5);
    log->printf("push\t" POINTER_PRINTF_FORMAT "\t\t%s push address of constant pool",
        getOwningMethod(methodSymRef)->constantPool(), commentString());
    bufferPos += 5;
