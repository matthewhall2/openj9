...
inline void generateInlineSuperclassTest(TR::Node* node, TR::CodeGenerator *cg, TR::Register *toClassReg, TR::Register* fromClassReg, TR_ScratchRegisterManager *srm, TR::LabelSymbol* failLabel, bool use64BitClasses, int32_t toClassDepth = -1)
   ...
   if (toClassDepth == -1)
      {
      // if (use64BitClasses && cg->comp()->target().is64Bit())
      //    {
      //    generateRegImmInstruction(TR::InstOpCode::SHL8RegImm1, node, toClassDepthReg, 3, cg);
      //    }
      // else if (!use64BitClasses && cg->comp()->target().is64Bit())
      //    {
      //    generateRegImmInstruction(TR::InstOpCode::SHL8RegImm1, node, toClassDepthReg, 2, cg);
      //    }
      // else
      //    {
...
static TR::SymbolReference *getClassSymRefAndDepth(TR::Node *classNode, TR::Compilation *comp, int32_t &classDepth, TR_OpaqueClassBlock *&clazz, const char * test = NULL)
   {
   classDepth = -1;
   if (test != NULL) {
    printf("%s\n", test);
...
printf("opcodeValue: %d\n", classNode->getOpCodeValue());
 ...
 logprintf(trace, log, "node is loadaddr or aload\n");
   ...
   if (classSymRef != NULL && !classSymRef->isUnresolved())
      {
    printf("resolved\n");
  //   (TR_OpaqueClassBlock *)symbol->castToStaticSymbol()->getStaticAddress();
    //  TR::StaticSymbol *classSym = classSymRef->getSymbol()->getStaticSymbol();
      clazz = (TR_OpaqueClassBlock *)classSymRef->getSymbol()->castToStaticSymbol()->getStaticAddress();
      if (clazz != NULL) {
        printf("clazz non null\n");
         classDepth = classSymRef->classDepth(comp);
         printf("depth is %d\n", classDepth);
      }
         return classSymRef;
      }
