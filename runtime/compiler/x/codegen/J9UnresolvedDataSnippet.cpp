...
J9::X86::UnresolvedDataSnippet::UnresolvedDataSnippet(TR::CodeGenerator *cg, TR::Node *node,
    TR::SymbolReference *symRef, bool isStore, bool isGCSafePoint)
...
TR_RuntimeHelper J9::X86::UnresolvedDataSnippet::getHelper()
{
    if (getDataSymbol()->isShadow())
        return isUnresolvedStore() ? TR_X86interpreterUnresolvedFieldSetterGlue : TR_X86interpreterUnresolvedFieldGlue;
...
uint8_t *J9::X86::UnresolvedDataSnippet::emitSnippetBody()
{
...
uint8_t *J9::X86::UnresolvedDataSnippet::emitResolveHelperCall(uint8_t *cursor)
{
    // Get the address for the glue routine (or its trampoline)
    //
    intptr_t glueAddress = (intptr_t)_glueSymRef->getMethodAddress();

    cg()->addProjectSpecializedRelocation(cursor + 1, (uint8_t *)_glueSymRef, NULL, TR_HelperAddress, __FILE__,
        __LINE__, getNode());
    ...
    if ((cg()->needRelocationsForHelpers() && cg()->comp()->target().is64Bit())
        ...
        glueAddress
            = TR::CodeCacheManager::instance()->findHelperTrampoline(_glueSymRef->getReferenceNumber(), (void *)cursor);
...
uint32_t J9::X86::UnresolvedDataSnippet::getUnresolvedStaticStoreDeltaWithMemBarrier()
{
    ...
    // Find the delta between the start of the memory barrier that should be guarding the store to the static field, and
    // the start of the store instruction
    //
    ...
    if (cg()->comp()->getOption(TR_X86UseMFENCE)) {
        ...
        if (delta == 20 && instIter->getOpCode().getOpCodeValue() == TR::InstOpCode::MFENCE) {
            ...
            TR_ASSERT(0,
                "Invalid computed offset between start of store to static field and start of memory barrier: %d. "
                "Should be 16 or 20.",
                delta);
            return 0;
        }
...
uint8_t *J9::X86::UnresolvedDataSnippet::emitUnresolvedInstructionDescriptor(uint8_t *cursor)
{
...
uint8_t *J9::X86::UnresolvedDataSnippet::emitConstantPoolAddress(uint8_t *cursor)
{
    ...
    *(intptr_t *)cursor = (intptr_t)getDataSymbolReference()->getOwningMethod(comp)->constantPool();
    ...
    if (getDataReferenceInstruction()) {
        ...
        cg()->addProjectSpecializedRelocation(cursor,
            (uint8_t *)getDataSymbolReference()->getOwningMethod(comp)->constantPool(),
            node ? (uint8_t *)(uintptr_t)node->getInlinedSiteIndex() : (uint8_t *)-1, TR_ConstantPool, __FILE__,
            __LINE__, node);
    }
...
uint8_t *J9::X86::UnresolvedDataSnippet::emitConstantPoolIndex(uint8_t *cursor)
{
    ...
    if (getNumLiveX87Registers() > 0)
        cpIndex |= (getNumLiveX87Registers() << 24);
    if (hasLiveXMMRegisters() && comp->target().is32Bit())
        cpIndex |= cpIndex_hasLiveXMMRegisters;
    ...
    if (getDataSymbol()->isConstObjectRef())
        cpIndex |= cpIndex_doNotPatchSnippet;
    ...
    if (comp->target().is32Bit()) {
        if (getDataSymbolReference()->getOffset() == 4) {
            TR_ASSERT(((!getDataSymbol()->isShadow() && !getDataSymbol()->isClassObject()
                           && !getDataSymbol()->isConstObjectRef())
                          || getDataSymbol()->isShadow()),
        ...
        if (comp->target().isSMP()) {
            if (!getDataSymbol()->isClassObject() && !getDataSymbol()->isConstObjectRef() && isUnresolvedStore()
                ...
                if (comp->target().is64Bit() && !getDataSymbol()->isShadow()) {
                    // If this is a store into a static field, we need to know the offset between the start of the store
                    // instruction and the memory barrier.
                    //
            ...
            if (comp->target().is32Bit()) {
                ...
                if (dataRefInst->getKind() == TR::Instruction::IsRegMem) {
                    ...
                    } else if (((TR::X86RegMemInstruction *)dataRefInst)
                                   ->getMemoryReference()
                                   ->processAsLongVolatileHigh()) {
...
uint8_t *J9::X86::UnresolvedDataSnippet::fixupDataReferenceInstruction(uint8_t *cursor)
{
    uint8_t *instructionStart = getDataReferenceInstruction()->getBinaryEncoding();
    uint8_t bytesToCopy;
    TR::Compilation *comp = cg()->comp();
    ...
    if (!getDataSymbol()->isShadow() && !getDataSymbol()->isClassObject() && !getDataSymbol()->isConstObjectRef()) {
        ...
        if (comp->target().is32Bit() && getDataSymbol()->isConstObjectRef()) {
            uint8_t *stringConstantPtr = (cursor - bytesToCopy) + getDataReferenceInstruction()->getBinaryLength()
                - TR::Compiler->om.sizeofReferenceAddress();

            cg()->addProjectSpecializedRelocation(stringConstantPtr,
                (uint8_t *)getDataSymbolReference()->getOwningMethod(comp)->constantPool(),
                getDataReferenceInstruction()->getNode()
                    ? (uint8_t *)(uintptr_t)getDataReferenceInstruction()->getNode()->getInlinedSiteIndex()
                    : (uint8_t *)-1,
                TR_ConstantPool, __FILE__, __LINE__, getDataReferenceInstruction()->getNode());
        }
    }
...
uint32_t J9::X86::UnresolvedDataSnippet::getLength(int32_t estimatedSnippetStart)
{
    uint32_t length;
    ...
    if (!getDataSymbol()->isShadow() && !getDataSymbol()->isClassObject() && !getDataSymbol()->isConstObjectRef()) {
        ...
        if (cg()->comp()->target().is32Bit()) {
            ...
            if (getDataSymbol()->isShadow()) {
                length += (8 // cached instruction bytes
                    + 1 // descriptor byte
                );
            } else {
                length += 2; // REX+opcode
...
void TR_Debug::print(OMR::Logger *log, TR::UnresolvedDataSnippet *snippet)
{
    ...
    if (_comp->target().is64Bit()) {
        ...
        log->printf("%s\t" POINTER_PRINTF_FORMAT "\t%s address of constant pool for this method", dqString(),
            getOwningMethod(snippet->getDataSymbolReference())->constantPool(), commentString());
        bufferPos += 8;

        printPrefix(log, NULL, bufferPos, 4);
        log->printf("%s\t0x%08x\t\t\t\t%s constant pool index", ddString(),
            snippet->getDataSymbolReference()->getCPIndex(), commentString());
        bufferPos += 4;
        ...
        if (snippet->getDataSymbol()->isShadow()) {
            ...
            log->printf("%s\t%02x\t\t\t\t\t\t\t%s instruction descriptor: length=%d, disp32 offset=%d", dbString(),
                descriptor, commentString(), length, offset);
        ...
        if (!snippet->getDataReferenceInstruction()) {
            ...
            printPrefix(log, NULL, bufferPos, 1);
            log->printf("int \t3\t\t\t%s (No data reference instruction; NEVER CALLED)", commentString());
            return;
        }
        ...
        log->printf("push\t" POINTER_PRINTF_FORMAT "\t\t%s constant pool index",
            snippet->getDataSymbolReference()->getCPIndex(), commentString());
        bufferPos += size;

        printPrefix(log, NULL, bufferPos, 5);
        log->printf("push\t" POINTER_PRINTF_FORMAT "\t\t%s address of constant pool for this method",
            getOwningMethod(snippet->getDataSymbolReference())->constantPool(), commentString());
        bufferPos += 5;
    ...
    if (!snippet->getDataSymbol()->isShadow() && !snippet->getDataSymbol()->isClassObject()
        && !snippet->getDataSymbol()->isConstObjectRef() && !snippet->getDataSymbol()->isMethodTypeTableEntry()) {
        ...
        if (_comp->target().is64Bit()) {
            if (snippet->getDataSymbol()->isShadow()) {
                ...
                printPrefix(log, NULL, bufferPos, bytesToCopy);
                bufferPos += bytesToCopy;
                log->printf("%s\t(%d)\t\t\t\t\t\t%s patch instruction bytes", dbString(), bytesToCopy, commentString());
            } else {
                bytesToCopy = 2;
                printPrefix(log, NULL, bufferPos, bytesToCopy);
                bufferPos += bytesToCopy;
            ...
            if (snippet->getDataSymbol()->isConstString()) {
                ...
                printPrefix(log, NULL, bufferPos, bytesToCopy);
                bufferPos += bytesToCopy;
                log->printf("%s\t(%d)\t\t\t\t\t\t%s patched string instruction bytes", dbString(), bytesToCopy,
                    commentString());
            } else {
                bytesToCopy = 8;
                printPrefix(log, NULL, bufferPos, bytesToCopy);
