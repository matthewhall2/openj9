...
inline uint32_t lcm(uint32_t a, uint32_t b) { return a * b / gcd(a, b); }

J9::X86::PrivateLinkage::PrivateLinkage(TR::CodeGenerator *cg)
    : J9::PrivateLinkage(cg)
{
    ...
    _properties.setOutgoingArgAlignment(
        lcm(cg->comp()->target().is32Bit() ? 4 : 16, cg->fej9()->getLocalObjectAlignmentInBytes()));
}

const TR::X86LinkageProperties &J9::X86::PrivateLinkage::getProperties() { return _properties; }
...
//
TR::Instruction *J9::X86::PrivateLinkage::copyParametersToHomeLocation(TR::Instruction *cursor,
    bool parmsHaveBeenStored)
{
    TR::Machine *machine = cg()->machine();
...
static TR::Instruction *initializeLocals(TR::Instruction *cursor, int32_t lowOffset, uint32_t count,
...
void J9::X86::PrivateLinkage::createPrologue(TR::Instruction *cursor)
{
    ...
    {
        int32_t frameSize
            = localSize + preservedRegsSize + (_properties.getReservesOutgoingArgsInPrologue() ? outgoingArgSize : 0);
        ...
        logprintf(trace, log, "Stack size was %d, and is adjusted by +%d (alignment %d, return address width %d)\n",
            stackSize, cg()->getStackFramePaddingSizeInBytes(), _properties.getOutgoingArgAlignment(),
    ...
    logprintf(trace, log, "\nFrame size: %c%c locals=%d frame=%d peak=%d\n", frameIsSmall ? 'S' : '-',
        frameIsMedium ? 'M' : '-', localSize, cg()->getFrameSizeInBytes(), peakSize);
    ...
    if (metaDataReg) {
        ...
        if (atlas) {
            ...
            if (_properties.getNumIntegerArgumentRegisters() == 0) {
                ...
                for (paramCursor = paramIterator.getFirst(); paramCursor != NULL;
                     paramCursor = paramIterator.getNext()) {
                    int32_t intRegArgIndex = paramCursor->getLinkageRegisterIndex();
                    if (intRegArgIndex >= 0 && paramCursor->isReferencedParameter()
                        ...
                        map->setRegisterBits(
                            TR::RealRegister::gprMask((getProperties().getIntegerArgumentRegister(intRegArgIndex))));
                    }
                }
            }
    ...
    //
    if ((comp()->getOption(TR_PaintAllocatedFrameSlotsDead) || comp()->getOption(TR_PaintAllocatedFrameSlotsFauxObject))
        && allocSize != 0) {
    ...
    //
    cursor = cg()->generateDebugCounter(cursor, "cg.prologues:#preserved",
        preservedRegsSize >> getProperties().getParmSlotShift(), TR::DebugCounter::Expensive);
    ...
    TR::RealRegister *framePointer = machine()->getRealRegister(TR::RealRegister::vfp);
    if (atlas) {
        TR_ASSERT(_properties.getNumScratchRegisters() >= 2,
            "Need two scratch registers to initialize reference locals");
        ...
        if (atlas->getInternalPointerMap()) {
            numInternalPointerSlotsToInitialize
                = atlas->getNumberOfDistinctPinningArrays() + atlas->getInternalPointerMap()->getNumInternalPointers();
        }
        ...
        if (numReferenceLocalSlotsToInitialize > 0 || numInternalPointerSlotsToInitialize > 0) {
            ...
            if (numReferenceLocalSlotsToInitialize > 0) {
                cursor = initializeLocals(cursor, atlas->getLocalBaseOffset(), numReferenceLocalSlotsToInitialize,
                    properties.getPointerSize(), framePointer, scratchReg, loopReg, cg());

#if defined(DEBUG)
                debugFrameSlotInfo = new (trHeapMemory()) TR_DebugFrameSegmentInfo(comp(), atlas->getLocalBaseOffset(),
            ...
            if (numInternalPointerSlotsToInitialize > 0) {
                cursor = initializeLocals(cursor, atlas->getOffsetOfFirstInternalPointer(),
                    numInternalPointerSlotsToInitialize, properties.getPointerSize(), framePointer, scratchReg, loopReg,
                    cg());

#if defined(DEBUG)
                debugFrameSlotInfo
                    = new (trHeapMemory()) TR_DebugFrameSegmentInfo(comp(), atlas->getOffsetOfFirstInternalPointer(),
                        numInternalPointerSlotsToInitialize * properties.getPointerSize(),
                        "Initialized internal pointers", debugFrameSlotInfo);
#endif
            }
        }
    }
    ...
    debugFrameSlotInfo = new (trHeapMemory()) TR_DebugFrameSegmentInfo(comp(),
        -localSize - preservedRegsSize - outgoingArgSize, outgoingArgSize, "Outgoing args", debugFrameSlotInfo);
#endif
    ...
    cursor = cg()->generateDebugCounter(cursor, "cg.prologues:#frameBytes", cg()->getFrameSizeInBytes(),
        TR::DebugCounter::Expensive);
...
TR::Register *J9::X86::PrivateLinkage::buildDirectDispatch(TR::Node *callNode, bool spillFPRegs)
{
    ...
    if (r && callNode && callNode->getOpCode().hasSymbolReference()
        && comp()->getSymRefTab()->findOrCreateAThrowSymbolRef(comp()->getMethodSymbol())
    ...
    , _useLastITableCache(false)
{
    ...
    //
    uint32_t numPreconditions = calleeLinkage->getProperties().getNumIntegerArgumentRegisters()
    ...
    if (getMethodSymbol()->preservesAllRegisters()) {
        ...
        if (callNode->getDataType() != TR::NoType) {
            ...
            _preservedRegisterMask
                &= ~TR::RealRegister::gprMask(getLinkage()->getProperties().getIntegerReturnRegister());
        }
    }
}
...
void TR::X86CallSite::setupVirtualGuardInfo()
{
    ...
    if (getMethodSymbol()->isVirtual() && _callNode->getOpCode().isIndirect()) {
        ...
        if (resolvedMethod && (!getMethodSymbol()->isVMInternalNative() || !comp()->getOption(TR_FullSpeedDebug))
    ...
    TR_ASSERT((_virtualGuardKind == TR_NoGuard) == (_devirtualizedMethod == NULL),
        "Virtual guard requires _devirtualizedMethod");
    TR_ASSERT((_devirtualizedMethod == NULL) == (_devirtualizedMethodSymRef == NULL),
        "_devirtualizedMethod requires _devirtualizedMethodSymRef");
}
...
void TR::X86CallSite::computeProfiledTargets()
{
    ...
    if (getMethodSymbol()->isVirtual() && !callNode->getSymbolReference()->isUnresolved()
        ...
        if (!comp()->getOption(TR_DisableInterpreterProfiling) && TR_ValueProfileInfoManager::get(comp())) {
            TR::Node *callNode = getCallNode();
            TR_AddressInfo *valueInfo = static_cast<TR_AddressInfo *>(
            ...
            if (topValue) {
                TR_ResolvedMethod *profiledVirtualMethod
                    = callNode->getSymbolReference()->getOwningMethod(comp())->getResolvedVirtualMethod(comp(),
                        (TR_OpaqueClassBlock *)topValue, methodSymRef->getOffset());
                if (profiledVirtualMethod
                    && (!profiledVirtualMethod->isInterpreted() || profiledVirtualMethod->isJITInternalNative())) {
                    // if (!getMethodSymbol()->isInterface() && profiledVirtualMethod->isJITInternalNative())
                    // TR_ASSERT(profiledVirtualMethod->classOfMethod() == (TR_OpaqueClassBlock *)topValue, "assertion
                    // failure");
                    ...
                    _profiledTargets->add(new (comp()->trStackMemory())
                            TR::X86PICSlot((uintptr_t)topValue, profiledVirtualMethod, true, methodToBeCompared, slot));
                }
            }
        }
        ...
        TR_AddressInfo *addressInfo = static_cast<TR_AddressInfo *>(
            TR_ValueProfileInfoManager::getProfiledValueInfo(callNode, comp(), AddressInfo));
#if defined(OSX)
        uint64_t topValue;
#else
...
#endif /* OSX */
        float missRatio = 0.0;
        if (addressInfo && addressInfo->getTopValue(topValue) > 0 && topValue
            && !comp()->getPersistentInfo()->isObsoleteClass((void *)topValue, fej9)
            ...
            for (TR_ExtraAddressInfo *profiledInfo = sortedValuesIt.getFirst(); profiledInfo != NULL;
                ...
                logprintf(trace, log, "  Profiled target frequency %f", frequency);
                ...
                if (profiledInterfaceMethod
                    && (!profiledInterfaceMethod->isInterpreted() || profiledInterfaceMethod->isJITInternalNative())) {
                    ...
                    if (frequency < minProfiledCallFrequency) {
                        logprints(trace, log, " - Too infrequent");
                        totalPICMissFrequency += profiledInfo->_frequency;
                    } else if (numStaticPICSlots
                        >= comp()->getOptions()->getMaxStaticPICSlots(comp()->getMethodHotness())) {
                        logprintf(trace, log, " - Already reached limit of %d static PIC slots", numStaticPICSlots);
                        totalPICMissFrequency += profiledInfo->_frequency;
                    } else {
                        _profiledTargets->add(
                            new (comp()->trStackMemory()) TR::X86PICSlot((uintptr_t)thisType, profiledInterfaceMethod));
                        logprints(trace, log, " + Added static PIC slot");
                        numStaticPICSlots++;
                        totalPICHitFrequency += profiledInfo->_frequency;
                    }
                    logprintf(trace, log, " for %s\n",
                        profiledInterfaceMethod->signature(comp()->trMemory(), stackAlloc));
                } else {
                    logprints(trace, log, " * Can't find suitable method from profile info\n");
                }
            }
        ...
        // Since instruction cache pressure is not very significant at higher opt levels, and new classes could be
        // loaded in the future. So, keep this opt enabled at higher opt levels to save path length.
        //
        if (_useLastITableCache && comp()->target().is64Bit() && (comp()->getMethodHotness() <= warm)
            && _interfaceClassOfMethod && comp()->getPersistentInfo()->getJitState() != STARTUP_STATE) {
            ...
            int32_t numImplementers = chTable->findnInterfaceImplementers(_interfaceClassOfMethod, numPICSlots + 1,
                implArray, cpIndex, getSymbolReference()->getOwningMethod(comp()), comp());
            if (numImplementers <= numPICSlots) {
                _useLastITableCache = false;
                logprintf(trace, log,
                    "Found %d implementers for call to %s, can be fit into %d pic slots, disabling lastITable cache\n",
                    numImplementers, getMethodSymbol()->getMethod()->signature(comp()->trMemory()), numPICSlots);
            }
        } else if (_useLastITableCache
            && comp()->target().is32Bit()) // Use the original heuristic for ia32 due to defect 111651
        {
...
{
    if (getMethodSymbol()->isVirtual() && !getSymbolReference()->isUnresolved()
        && getMethodSymbol()->isVMInternalNative() && !getResolvedMethod()->virtualMethodIsOverridden()
...
bool TR::X86CallSite::resolvedVirtualShouldUseVFTCall()
{
    ...
    return fej9->isResolvedVirtualDispatchGuaranteed(comp())
        && (!comp()->getOption(TR_EnableVPICForResolvedVirtualCalls) || getProfiledTargets()
            || getCallNode()->isTheVirtualCallNodeForAGuardedInlinedCall()
            || (comp()->getSymRefTab()->findObjectNewInstanceImplSymbol()
                && comp()->getSymRefTab()->findObjectNewInstanceImplSymbol()->getSymbol()
...
void TR::X86CallSite::stopAddingConditions()
{
    ...
    if (COPY_PRECONDITIONS_TO_POSTCONDITIONS) {
        ...
        for (uint8_t i = 0; i < getPreConditionsUnderConstruction()->getAddCursorForPre(); i++) {
            ...
            getPostConditionsUnderConstruction()->unionPreCondition(pre->getRegister(), pre->getRealRegister(), cg(),
                pre->getFlags());
            TR::RegisterDependency *post = postconditions->findDependency(pre->getRealRegister(),
                getPostConditionsUnderConstruction()->getAddCursorForPost());
            if (!post)
                getPostConditionsUnderConstruction()->addPostCondition(pre->getRegister(), pre->getRealRegister(), cg(),
                    pre->getFlags());
        }
    }
...
static void evaluateCommonedNodes(TR::Node *node, TR::CodeGenerator *cg)
{
    ...
    if (node->getRegister() == NULL) {
        if (node->getReferenceCount() >= 2) {
            logprintf(comp->getOption(TR_TraceCG), comp->log(), "Promptly evaluating commoned node %s\n",
...
static bool indirectDispatchWillBuildVirtualGuard(TR::Compilation *comp, TR::X86CallSite *site)
{
    ...
    if (site->getVirtualGuardKind() != TR_NoGuard && fej9->canDevirtualizeDispatch()) {
        if (comp->performVirtualGuardNOPing()) {
            return true;
        } else if (site->getVirtualGuardKind() == TR_NonoverriddenGuard && !comp->getOption(TR_EnableHCR)
...
TR::Register *J9::X86::PrivateLinkage::buildIndirectDispatch(TR::Node *callNode)
{
    ...
    if (callNode->getSymbol()->castToMethodSymbol()->firstArgumentIsReceiver()) {
        ...
        if (enableX86VFTLoadOpt && loadVFTForNullCheck && willGenerateDirectCall && vftChild->getReferenceCount() == 1
            ...
            */
            ...
            TR::Instruction *instr
                = TR::TreeEvaluator::insertLoadMemory(vftChild, reg, sourceMR, TR_RematerializableAddress, cg());
            reg->setMemRef(sourceMR);
            cg()->setImplicitExceptionPoint(instr);
            site.setImplicitExceptionPoint(instr);
            cg()->stopUsingRegister(reg);
            skipVFTmaskInstruction = true;
        } else if (enableX86VFTLoadOpt && loadVFTForNullCheck == false && willGenerateDirectCall &&
        ...
        if (methodSymbol->isComputed()) {
            switch (method->getMandatoryRecognizedMethod()) {
                ...
                case TR::com_ibm_jit_JITHelpers_dispatchVirtual: {
                    ...
                    char *j2iSignature
                        = fej9->getJ2IThunkSignatureForDispatchVirtual(methodSymbol->getMethod()->signatureChars(),
                            methodSymbol->getMethod()->signatureLength(), comp());
                ...
                default:
                    if (fej9->needsInvokeExactJ2IThunk(callNode, comp())) {
                        TR_MHJ2IThunk *thunk
                            = generateInvokeExactJ2IThunk(callNode, methodSymbol->getMethod()->signatureChars());
            ...
            if (!virtualThunk)
                virtualThunk
                    = fej9->setJ2IThunk(methodSymbol->getMethod(), generateVirtualIndirectThunk(callNode), comp());
        }
    ...
    if (site.getVirtualGuardKind() != TR_NoGuard && fej9->canDevirtualizeDispatch()
        && buildVirtualGuard(site, revirtualizeLabel)) {
        ...
        if (profiledTargets) {
            ...
            while (picSlot) {
                ...
                if (fej9->isUnloadAssumptionRequired((TR_OpaqueClassBlock *)picSlot->getClassAddress(),
...
#if defined(J9VM_OPT_JITSERVER)
    // JITServer Workaround: Further transmute dispatchJ9Method symbols to appear as a runtime helper, this will cause
    // OMR to generate a TR_HelperAddress relocation instead of a TR_RelativeMethodAddress Relocation.
...
void J9::X86::PrivateLinkage::buildInterfaceCall(TR::X86CallSite &site, TR::LabelSymbol *entryLabel,
...
void J9::X86::PrivateLinkage::buildRevirtualizedCall(TR::X86CallSite &site, TR::LabelSymbol *revirtualizeLabel,
    TR::LabelSymbol *doneLabel)
{
    TR_J9VMBase *fej9 = (TR_J9VMBase *)(fe());
    TR::Register *vftRegister
        = site.getCallNode()->getFirstChild()->getRegister(); // may be NULL; we don't need to evaluate it here
    int32_t vftOffset = site.getSymbolReference()->getOffset();

    TR::Snippet *snippet;
    if (comp()->target().is64Bit()) {
#ifdef TR_TARGET_64BIT
        snippet = new (trHeapMemory())
            TR::AMD64GuardedDevirtualSnippet(cg(), site.getCallNode(), site.getDevirtualizedMethodSymRef(), doneLabel,
                revirtualizeLabel, vftOffset, cg()->getCurrentEvaluationBlock(), vftRegister, site.getArgSize());
#endif
    } else {
        snippet = new (trHeapMemory()) TR::X86GuardedDevirtualSnippet(cg(), site.getCallNode(), doneLabel,
            revirtualizeLabel, vftOffset, cg()->getCurrentEvaluationBlock(), vftRegister);
    }
...
bool J9::X86::PrivateLinkage::buildVirtualGuard(TR::X86CallSite &site, TR::LabelSymbol *revirtualizeLabel)
{
    ...
    if (comp()->performVirtualGuardNOPing()) {
        ...
        && !comp()->getOption(
            TR_MimicInterpreterFrameShape)) // Explicitly-guarded devirtualization is pretty pointless without inlining
    {
...
TR::Instruction *J9::X86::PrivateLinkage::buildVFTCall(TR::X86CallSite &site, TR::InstOpCode dispatchOp,
    TR::Register *targetAddressReg, TR::MemoryReference *targetAddressMemref)
{
    TR::Node *callNode = site.getCallNode();
    ...
    if (dispatchOp.sourceIsMemRef()) {
        ...
        if (resolvedMethodSymbol
            && (resolvedMethodSymbol->getRecognizedMethod() == TR::java_lang_invoke_ComputedCalls_dispatchDirect
                || resolvedMethodSymbol->getRecognizedMethod()
                    == TR::com_ibm_jit_JITHelpers_dispatchComputedStaticCall))
        ...
        if (mayReachJ2IThunk && dispatchOp.isCallOp()) {
            ...
            if (targetAddressReg && targetAddressReg->getRegisterPair()) {
                dependencies->unionPreCondition(targetAddressReg->getRegisterPair()->getHighOrder(),
                    TR::RealRegister::NoReg, cg());
                dependencies->unionPreCondition(targetAddressReg->getRegisterPair()->getLowOrder(),
                    TR::RealRegister::NoReg, cg());
            } else
    ...
    TR_ASSERT_FATAL(!site.getSymbolReference()->isUnresolved() || site.getMethodSymbol()->isInterface(),
...
TR::Register *J9::X86::PrivateLinkage::buildCallPostconditions(TR::X86CallSite &site)
{
    ...
    if (calleePreservesRegisters) {
        ...
        // Note: this assumes the postcondition regdeps have preconditions too; see
        // COPY_PRECONDITIONS_TO_POSTCONDITIONS.
        //
        ...
        for (int i = 0; i < dependencies->getAddCursorForPre(); i++) {
            ...
            if (regIndex != returnRegIndex && regIndex != highReturnRegIndex
        ...
        for (regIndex = TR::RealRegister::FirstGPR; regIndex <= TR::RealRegister::LastAssignableGPR;
            ...
            if ((regIndex != returnRegIndex) && (regIndex != highReturnRegIndex)
                && !properties.isPreservedRegister(regIndex)) {
    ...
    //
    dependencies->addPostCondition(cg()->getMethodMetaDataRegister(), getProperties().getMethodMetaDataRegister(),
        cg());
...
void J9::X86::PrivateLinkage::buildVPIC(TR::X86CallSite &site, TR::LabelSymbol *entryLabel, TR::LabelSymbol *doneLabel)
{
    ...
    TR::X86PicDataSnippet *snippet = new (trHeapMemory())
        TR::X86PicDataSnippet(VPicParameters.defaultNumberOfSlots, startOfPicInstruction, snippetLabel, doneLabel,
            site.getSymbolReference(), slotPatchInstruction, site.getThunkAddress(), false, cg());
 ...
 *
 */
static void generateITableEntryCompareLogic(TR::Node *callNode, TR::Register *scratchReg,
    TR::Register *interfaceClassReg, TR_OpaqueClassBlock *declaringClass, bool use32BitInterfaceClassPointers,
    bool isLastIteration, TR::LabelSymbol *lookupDispatchSnippetLabel, TR::LabelSymbol *gotoLastITableDispatchLabel,
    TR::CodeGenerator *cg)
{
...
/**
 * \brief
 *    Generate a sequence of instructions that compares the `declaringClass` with `iterations` number of entries in the
 * iTable.
 *
 * \parm iterations
 ...
 *    The VFT register.
 *
 * \parm interfaceClassReg
 *    The register that will be used to load the interface class.
 *
 * \parm declaringClass
 *    The class that is to be compared.
 *
 * \parm use32BitInterfaceClassPointers
 *    Whether or not using 32 bit interface class pointer.
 *
 * \parm lookupDispatchSnippetLabel
 *    The label that dispatches to interface look up snippet if the comparison fails.
 *
 * \parm gotoLastITableDispatchLabel
 *    The label that dispatches to invoke the interface call if the comparison succeeds.
 *
 * \parm comp
 ...
 *
 */
static void generateITableEntryLoop(uint32_t iterations, TR::Node *callNode, TR::Register *scratchReg,
    TR::Register *vftReg, TR::Register *interfaceClassReg, TR_OpaqueClassBlock *declaringClass,
    bool use32BitInterfaceClassPointers, TR::LabelSymbol *lookupDispatchSnippetLabel,
    TR::LabelSymbol *gotoLastITableDispatchLabel, TR::CodeGenerator *cg)
{
    TR_ASSERT_FATAL(iterations > 0, "Expected positive number of iTable entry compare iterations");
    ...
    if (iterations <= maxUnrolledIterations) {
        ...
        for (uint32_t i = 0; i < iterations; i++) {
            bool isLastIteration = (i + 1) == iterations;
            generateITableEntryCompareLogic(callNode, scratchReg, interfaceClassReg, declaringClass,
                use32BitInterfaceClassPointers, isLastIteration, lookupDispatchSnippetLabel,
...
{
    static char *breakBeforeInterfaceDispatchUsingLastITable
        = feGetEnv("TR_breakBeforeInterfaceDispatchUsingLastITable");
    ...
    //    lastITableDispatch:       <------------------------------+   |    |
    //          mov r8, sizeof(J9Class)                            |   |    |       ; r8 = jit vtable offset
    //          sub r8, [rdi + ITableSlotOffset]                   |   |    |       ; vtable dispatch
    //          jmp [vft + r8]                                     |   |    |
    //    lastITableTest:           <------------------------------|---+    |
    ...
    //          call lastITableDispatch    ------------------------+   |  | |       ; if so, call the dispatch sequence
    //          with return address pointing here
    //     done:    <--------------------------------------------------|--|-+
    //          ...                                                    |  |
    //                                                                 |  |
    //                                                                 |  |
    //    Outlined Label iterateITableLabel:     <---------------------+  |
    //          mov  rdi, qword ptr [vft + iTableOffset]                  |
    //          test rdi, rdi                                             |
    //          je   lookupDispatchSnippet       -------------------------|------>  ; If iTable[0] is NULL, jump to the
    //          slow path cmp  dword ptr [rdi], interfaceClass                      |         ; Check if it is our
    //          interface class je   gotoLastITableDispatchLabel -------------------------+         ; If yes, jmp to the
    //          call of the dispatch sequence mov  rdi, qword ptr [rdi + nextITableOffset]              | test rdi, rdi
    //          | je   lookupDispatchSnippet       -------------------------|------>  ; If iTable[1] is NULL, jump to
    //          the slow path cmp  dword ptr [rdi], interfaceClass                      |         ; Check if it is our
    //          interface class je   gotoLastITableDispatchLabel -------------------------+         ; If yes, jmp to the
    //          call of the dispatch sequence jmp  lookupDispatchSnippet       -------------------------------->  ; Done
    //          iterating the iTable, jump to the slow path
    ...
    TR::RealRegister::RegNum otherScratchRegister
        = getProperties().getJ9MethodArgumentRegister(); // scratch reg other than the vtable index reg
