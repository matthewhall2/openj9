...
void J9::X86::CodeGenerator::initialize()
{
    ...
    if (comp->target().cpu.supportsFeature(OMR_FEATURE_X86_SSSE3) && !comp->getOption(TR_DisableFastStringIndexOf)
        && !TR::Compiler->om.canGenerateArraylets() && !TR::Compiler->om.isOffHeapAllocationEnabled()) {
    ...
    if (comp->target().cpu.supportsFeature(OMR_FEATURE_X86_SSE4_1) && !comp->getOption(TR_DisableSIMDStringHashCode)
        && !TR::Compiler->om.canGenerateArraylets() && !TR::Compiler->om.isOffHeapAllocationEnabled()) {
...
TR::SymbolReference *J9::X86::CodeGenerator::getNanoTimeTemp()
{
...
void J9::X86::CodeGenerator::beginInstructionSelection()
{
    ...
    if (recompilation && recompilation->generatePrePrologue() != NULL) {
        ...
        int32_t alignmentMargin = comp->target().is64Bit()
            ? 2
            : 0; // # bytes between the alignment instruction and the address that needs to be aligned
        if (methodSymbol->getLinkageConvention() == TR_Private)
        ...
        TR::Instruction *cursor
            = self()->generateSwitchToInterpreterPrePrologue(NULL, alignmentBoundary, alignmentMargin);
        if (comp->target().is64Bit()) {
...
void J9::X86::CodeGenerator::endInstructionSelection()
{
    TR::Compilation *comp = self()->comp();
...
void J9::X86::CodeGenerator::preBinaryEncodingHook()
{
...
TR::VectorLength J9::X86::CodeGenerator::getMaxPreferredVectorLength()
{
...
TR::Instruction *J9::X86::CodeGenerator::generateSwitchToInterpreterPrePrologue(TR::Instruction *prev,
    uint8_t alignment, uint8_t alignmentMargin)
{
    TR::Compilation *comp = self()->comp();
...
bool J9::X86::CodeGenerator::nopsAlsoProcessedByRelocations()
{
    return self()->fej9()->nopsAlsoProcessedByRelocations();
}

bool J9::X86::CodeGenerator::enableAESInHardwareTransformations()
{
    if (self()->comp()->target().cpu.supportsFeature(OMR_FEATURE_X86_AESNI)
        && !self()->comp()->getOption(TR_DisableAESInHardware) && !self()->comp()->getCurrentMethod()->isJNINative())
        return true;
    else
        return false;
}

bool J9::X86::CodeGenerator::suppressInliningOfRecognizedMethod(TR::RecognizedMethod method)
{
    switch (method) {
...
bool J9::X86::CodeGenerator::supportsInliningOfIsAssignableFrom()
{
...
void J9::X86::CodeGenerator::reserveNTrampolines(int32_t numTrampolines)
{
 ...
 */
static bool willNotInlineCompareAndSwapNative(TR::Node *node, int8_t size, TR::Compilation *comp, bool isExchange)
{
 ...
 */
bool J9::X86::CodeGenerator::willBeEvaluatedAsCallByCodeGen(TR::Node *node, TR::Compilation *comp)
{
    TR::SymbolReference *callSymRef = node->getSymbolReference();
    TR::MethodSymbol *methodSymbol = callSymRef->getSymbol()->castToMethodSymbol();
