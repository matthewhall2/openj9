...
// Note AOT relocations assumes that this value is 0.  Shall this value change, the AOT relocations also need to reflect
// this change.
    ...
    : J9::X86::PrivateLinkage(cg)
{
    ...
    _properties._properties = EightBytePointers | EightByteParmSlots | IntegersInRegisters | LongsInRegisters
        | FloatsInRegisters | NeedsThunksForIndirectCalls | UsesRegsForHelperArgs;
...
uint8_t *J9::X86::AMD64::PrivateLinkage::flushArguments(TR::Node *callNode, uint8_t *cursor, bool calculateSizeOnly,
...
TR::Instruction *J9::X86::AMD64::PrivateLinkage::generateFlushInstruction(TR::Instruction *prev,
    TR_MovOperandTypes operandType, TR::DataType dataType, TR::RealRegister::RegNum regIndex, TR::Register *espReg,
    int32_t offset, TR::CodeGenerator *cg)
{
...
TR::Instruction *J9::X86::AMD64::PrivateLinkage::flushArguments(TR::Instruction *prev,
    TR::ResolvedMethodSymbol *methodSymbol, bool isReturnAddressOnStack, bool isLoad)
{
    // TODO:AMD64: Share code with the other flushArguments
    ...
    for (parmCursor = parameterIterator.getFirst(); parmCursor; parmCursor = parameterIterator.getNext()) {
        ...
        switch (dt) {
            ...
            case TR::Address:
                offset -= slotSize;
                if (numGPArgs < getProperties().getNumIntegerArgumentRegisters()) {
                    prev = generateFlushInstruction(prev, ot, dt, getProperties().getIntegerArgumentRegister(numGPArgs),
                        espReg, offset, cg());
                }
            ...
            case TR::Float:
                ...
                if (numFPArgs < getProperties().getNumFloatArgumentRegisters()) {
                    prev = generateFlushInstruction(prev, ot, dt, getProperties().getFloatArgumentRegister(numFPArgs),
                        espReg, offset, cg());
                }
...
TR_MHJ2IThunk *J9::X86::AMD64::PrivateLinkage::generateInvokeExactJ2IThunk(TR::Node *callNode, char *signature)
{
    ...
    *(uint16_t *)cursor = 0xbf48;
    cursor += 2;
    *(uint64_t *)cursor
        = (uintptr_t)cg()->fej9()->getInvokeExactThunkHelperAddress(comp, glueSymRef, callNode->getDataType());
    cursor += 8;
    ...
    logprintf(comp->getOption(TR_TraceCG), comp->log(),
        "\n-- ( Created invokeExact J2I thunk " POINTER_PRINTF_FORMAT " for node " POINTER_PRINTF_FORMAT " )", thunk,
        callNode);
...
TR::Instruction *J9::X86::AMD64::PrivateLinkage::savePreservedRegisters(TR::Instruction *cursor)
{
    ...
    cursor = cg()->generateDebugCounter(cursor, "cg.prologues:no-preservedRegStoreBytesSaved", 1,
...
TR::Instruction *J9::X86::AMD64::PrivateLinkage::restorePreservedRegisters(TR::Instruction *cursor)
{
    ...
    for (pindex = _properties.getMaxRegistersPreservedInPrologue() - 1; pindex >= 0; pindex--) {
        TR::RealRegister::RegNum idx = _properties.getPreservedRegister((uint32_t)pindex);
        TR::RealRegister *reg = machine()->getRealRegister(idx);
    ...
    cursor = cg()->generateDebugCounter(cursor, "cg.epilogues:no-preservedRegStoreBytesSaved", 1,
        TR::DebugCounter::Expensive);
    return cursor;
}
...
int32_t J9::X86::AMD64::PrivateLinkage::argAreaSize(TR::ResolvedMethodSymbol *methodSymbol)
{
    ...
    for (paramCursor = paramIterator.getFirst(); paramCursor; paramCursor = paramIterator.getNext()) {
        result += paramCursor->getRoundedSize()
            * ((DOUBLE_SIZED_ARGS && paramCursor->getDataType() != TR::Address) ? 2 : 1);
    }
    return result;
}
...
int32_t J9::X86::AMD64::PrivateLinkage::buildArgs(TR::Node *callNode, TR::RegisterDependencyConditions *dependencies)
{
    ...
    bool rightToLeft = methodSymbol && methodSymbol->isHelper()
        && !methodSymRef->isOSRInductionHelper() // we want the arguments for induceOSR to be passed from left to right
                                                 // as in any other non-helper call
    ...
    if (callNode->getOpCode().isIndirect()) {
        if (methodSymbol->isVirtual() && !methodSymRef->isUnresolved() && !comp()->getOption(TR_FullSpeedDebug)
            && // On FSD we need to call these through the vtable (because the methodIsOverridden flag is unreliable) so
               // we need args to be in regs
            ...
            passArgsOnStack = (!resolvedMethod->virtualMethodIsOverridden() && !resolvedMethod->isAbstract());
        } else {
...
int32_t J9::X86::AMD64::PrivateLinkage::buildPrivateLinkageArgs(TR::Node *callNode,
    TR::RegisterDependencyConditions *dependencies, bool rightToLeft, bool passArgsOnStack)
{
    ...
    uint16_t numIntArgs = 0, numFloatArgs = 0, numSpecialArgs = 0;
    ...
    if (!getProperties().getReservesOutgoingArgsInPrologue()
        && !callNode->getSymbol()->castToMethodSymbol()->isHelper()) {
        // Align the stack for alignment larger than 16 bytes(stack is aligned to the multiple of sizeOfJavaPointer by
        // default, which is 4 bytes on 32-bit, and 8 bytes on 64-bit) Basically, we're aligning the start of next frame
        // (after the return address) to a multiple of staceFrameAlignmentInBytes Stack frame alignment is needed for
        // stack allocated object alignment
    ...
    for (i = start; i != stop; i += step) {
        ...
        switch (dt) {
            case TR::Float:
            case TR::Double:
                rregIndex = (numFloatArgs < NUM_FLOAT_LINKAGE_REGS) && (!passArgsOnStack || createDummyLinkageRegisters)
        ...
        if ((rregIndex != noReg) || (child->getOpCodeValue() != TR::iconst)
    ...
    // Sanity check
    //
    TR_ASSERT(numIntArgs + numFloatArgs + numSpecialArgs
            == callNode->getNumChildren() - callNode->getFirstArgumentIndex(),
        "assertion failure");
...
TR::Instruction *J9::X86::AMD64::PrivateLinkage::buildPICSlot(TR::X86PICSlot picSlot, TR::LabelSymbol *mismatchLabel,
...
void J9::X86::AMD64::PrivateLinkage::buildIPIC(TR::X86CallSite &site, TR::LabelSymbol *entryLabel,
    TR::LabelSymbol *doneLabel, uint8_t *thunk)
{
    ...
    if (useLastITableCache
        && (declaringClass = site.getSymbolReference()->getOwningMethod(comp())->getResolvedInterfaceMethod(
                site.getSymbolReference()->getCPIndex(), &itableIndex))
        && performTransformation(comp(), "O^O useLastITableCache for n%dn itableIndex=%d: %.*s.%.*s%.*s\n",
            site.getCallNode()->getGlobalIndex(), (int)itableIndex, method->classNameLength(), method->classNameChars(),
            method->nameLength(), method->nameChars(), method->signatureLength(), method->signatureChars())) {
        buildInterfaceDispatchUsingLastITable(site, numIPicSlots, lastPicSlot, slotPatchInstruction, doneLabel,
            lookupDispatchSnippetLabel, declaringClass, itableIndex);
    } else {
    ...
    TR::X86PicDataSnippet *snippet
        = new (trHeapMemory()) TR::X86PicDataSnippet(numIPicSlots, startOfPicInstruction, lookupDispatchSnippetLabel,
            doneLabel, site.getSymbolReference(), slotPatchInstruction, site.getThunkAddress(), true, cg());
...
void J9::X86::AMD64::PrivateLinkage::buildVirtualOrComputedCall(TR::X86CallSite &site, TR::LabelSymbol *entryLabel,
    TR::LabelSymbol *doneLabel, uint8_t *thunk)
{
    ...
    if (methodSymRef->getSymbol()->castToMethodSymbol()->isComputed()) {
        ...
        if (comp()->compileRelocatableCode()) {
            ...
            TR_OpaqueMethodBlock *method = methodSymRef->getSymbol()
