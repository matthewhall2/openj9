...
int32_t J9::X86::AMD64::JNILinkage::computeMemoryArgSize(TR::Node *callNode, int32_t first, int32_t last,
    bool passThread)
{
    int32_t size = 0;
    ...
    //
    if (passThread) {
        TR_ASSERT(!_systemLinkage->getProperties().passArgsRightToLeft(),
            "JNI argument evaluation expected to be left-to-right");
        TR_ASSERT(_systemLinkage->getProperties().getNumIntegerArgumentRegisters() > 0,
            "JNI env pointer expected to be passed in a register.");
        numIntArgs++;
    ...
    if (_systemLinkage->getProperties().getCallerFrameAllocatesSpaceForLinkageRegisters()) {
        ...
        size += std::max(_systemLinkage->getProperties().getNumIntegerArgumentRegisters(),
                    _systemLinkage->getProperties().getNumFloatArgumentRegisters())
...
//
int32_t J9::X86::AMD64::JNILinkage::buildArgs(TR::Node *callNode, TR::RegisterDependencyConditions *deps,
    bool passThread, bool passReceiver)
{
    ...
    for (i = first; i != last; i++) {
        ...
        switch (type) {
            case TR::Float:
            case TR::Double:
                rregIndex = (numFloatArgs < _systemLinkage->getProperties().getNumFloatArgumentRegisters())
                    ? _systemLinkage->getProperties().getFloatArgumentRegister(numFloatArgs)
                    : noReg;
                numFloatArgs++;
            ...
            case TR::Address:
                ...
                //
            default:
                rregIndex = (numIntArgs < _systemLinkage->getProperties().getNumIntegerArgumentRegisters())
                    ? _systemLinkage->getProperties().getIntegerArgumentRegister(numIntArgs)
                    : noReg;
                numIntArgs++;
        ...
        if (rregIndex != noReg) {
            ...
            //
            if ((child->getReferenceCount() > 1) && (vreg == child->getRegister())) {
                TR::Register *argReg = cg()->allocateRegister();
...
TR::Register *J9::X86::AMD64::JNILinkage::buildVolatileAndReturnDependencies(TR::Node *callNode,
    TR::RegisterDependencyConditions *deps, bool omitDedicatedFrameRegister)
{
    ...
    for (i = 0; i < _systemLinkage->getProperties().getNumVolatileRegisters(); i++) {
        ...
        if (regIndex != returnRegIndex) {
            if (!omitDedicatedFrameRegister || (regIndex != _JNIDispatchInfo.dedicatedFrameRegisterIndex)) {
                TR_RegisterKinds rk
                    = (i < _systemLinkage->getProperties()._numberOfVolatileGPRegisters) ? TR_GPR : TR_FPR;
...
void J9::X86::AMD64::JNILinkage::buildJNICallOutFrame(TR::Node *callNode, TR::LabelSymbol *returnAddrLabel)
{
    ...
    if (IS_32BIT_SIGNED(methodAddr) && !TR::Compiler->om.nativeAddressesCanChangeSize()) {
        ...
        static const TR_ExternalRelocationTargetKind reloTypes[] = { TR_VirtualRamMethodConst,
...
void J9::X86::AMD64::JNILinkage::buildJNIMergeLabelDependencies(TR::Node *callNode, bool killNonVolatileGPRs)
{
    ...
    if (killNonVolatileGPRs) {
        for (i = 0; i < _systemLinkage->getProperties().getNumPreservedRegisters(); i++) {
            ...
            if (regIndex != returnRegIndex) {
                TR_RegisterKinds rk
                    = (i < _systemLinkage->getProperties()._numberOfPreservedGPRegisters) ? TR_GPR : TR_FPR;
                TR::Register *dummy = cg()->allocateRegister(rk);
...
void J9::X86::AMD64::JNILinkage::buildOutgoingJNIArgsAndDependencies(TR::Node *callNode, bool passThread,
    bool passReceiver, bool killNonVolatileGPRs)
{
    ...
    //
    uint32_t callPre = _systemLinkage->getProperties().getNumIntegerArgumentRegisters()
        + _systemLinkage->getProperties().getNumFloatArgumentRegisters();
    uint32_t callPost = _systemLinkage->getProperties().getNumVolatileRegisters() + 1
    ...
    //
    _JNIDispatchInfo.linkageReturnRegister
        = buildVolatileAndReturnDependencies(callNode, _JNIDispatchInfo.callPostDeps, true);
    ...
    uintptr_t targetAddress)
{
    ...
    //
    TR_ASSERT(_JNIDispatchInfo.dispatchTrampolineRegister,
        "No trampoline scratch register available for directJNI dispatch.");
    ...
    static const TR_ExternalRelocationTargetKind reloTypes[] = { TR_JNIVirtualTargetAddress,
        TR_NoRelocation /*Interfaces*/, TR_JNIStaticTargetAddress, TR_JNISpecialTargetAddress };
    int reloType = callSymbol->getMethodKind() - 1; // method kinds are 1-based!!
...
void J9::X86::AMD64::JNILinkage::cleanupReturnValue(TR::Node *callNode, TR::Register *linkageReturnReg,
...
void J9::X86::AMD64::JNILinkage::checkForJNIExceptions(TR::Node *callNode)
{
    ...
    TR::Snippet *snippet = new (trHeapMemory())
        TR::X86CheckFailureSnippet(cg(), cg()->symRefTab()->findOrCreateRuntimeHelper(TR_throwCurrentException),
            snippetLabel, instr, _JNIDispatchInfo.requiresFPstackPop);
...
void J9::X86::AMD64::JNILinkage::cleanupJNIRefPool(TR::Node *callNode)
{
    ...
    //
    TR_J9VMBase *fej9 = (TR_J9VMBase *)(fe());
    static_assert(IS_32BIT_SIGNED(J9_SSF_JIT_JNI_FRAME_COLLAPSE_BITS),
        "J9_SSF_JIT_JNI_FRAME_COLLAPSE_BITS must fit in immediate");
...
TR::Register *J9::X86::AMD64::JNILinkage::buildDirectDispatch(TR::Node *callNode, bool spillFPRegs)
{
    TR::SymbolReference *callSymRef = callNode->getSymbolReference();
    ...
    bool isGPUHelper = callSymbol->isHelper()
        && (callSymRef->getReferenceNumber() == TR_estimateGPU || callSymRef->getReferenceNumber() == TR_getStateGPU
            ...
            || callSymRef->getReferenceNumber() == TR_invalidateGPU || callSymRef->getReferenceNumber() == TR_flushGPU
...
void J9::X86::AMD64::JNILinkage::populateJNIDispatchInfo()
{
    ...
    if (!isGPUHelper) {
        ...
        if (keepVMDuringGPUHelper
            || (callSymRef->getReferenceNumber() == TR_copyToGPU || callSymRef->getReferenceNumber() == TR_copyFromGPU
                ...
                || callSymRef->getReferenceNumber() == TR_regionExitGPU
                || callSymRef->getReferenceNumber() == TR_estimateGPU))
            dropVMAccess = false; // TR_copyToGPU, TR_copyFromGPU, TR_regionExitGPU (and all others if
