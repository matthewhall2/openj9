    ...
    if (expectedDisp32 != actualDisp32) {
        ...
        if (!trampoline) {
            if (traceAMD64CallPointPatching) {
                fprintf(stderr,
                    "AMD64 NOT PATCHING: Call %p does not target method %p (startPC %p) and there is no trampoline\n",
        ...
        if (trampolineDisp32 != actualDisp32) {
            if (traceAMD64CallPointPatching) {
                fprintf(stderr,
                    "AMD64 NOT PATCHING: Call %p does not target method %p (startPC %p) nor its trampoline %p\n",
...
void J9::Recompilation::methodHasBeenRecompiled(void *oldStartPC, void *newStartPC, TR_FrontEnd *fe)
{
    ...
    if (linkageInfo->isCountingMethodBody()) {
        ...
        *((int16_t *)startByte) = ((offset & 0xFF) << 8) | CALL_INSTRUCTION;
        ...
        bytesToSaveAtStart
            = 7 + jitEntryOffset(oldStartPC); // the new call instruction + 2-byte offset; TODO: this could be 5 on IA32
    } else {
