...
uint8_t *TR_X86RelocationTarget::eipBaseForCallOffset(uint8_t *reloLocation)
{
    // reloLocation points at the start of the call offset, return address is 4
    return (uint8_t *)(reloLocation + 4);
}

void TR_X86RelocationTarget::storeCallTarget(uintptr_t callTarget, uint8_t *reloLocation)
{
...
void TR_X86RelocationTarget::storeRelativeTarget(uintptr_t callTarget, uint8_t *reloLocation)
{
    storeCallTarget(callTarget, reloLocation);
}

uint8_t *TR_X86RelocationTarget::loadAddressSequence(uint8_t *reloLocation)
{
    // reloLocation points at the start of the address, so just need to dereference as uint8_t *
    return loadPointer(reloLocation);
}

void TR_X86RelocationTarget::storeAddressSequence(uint8_t *address, uint8_t *reloLocation, uint32_t seqNumber)
{
...
uint32_t TR_X86RelocationTarget::loadCPIndex(uint8_t *reloLocation)
{
...
uintptr_t TR_X86RelocationTarget::loadThunkCPIndex(uint8_t *reloLocation)
{
    return (uintptr_t)loadCPIndex(reloLocation);
}

void TR_X86RelocationTarget::performThunkRelocation(uint8_t *thunkAddress, uintptr_t vmHelper)
{
...
#if defined(J9VM_OPT_JITSERVER)
static void *j9ThunkInvokeExactHelperFromTerseSignature(UDATA signatureLength, char *signatureChars,
...
void TR_X86RelocationTarget::performInvokeExactJ2IThunkRelocation(TR_MHJ2IThunk *thunk)
{
...
uint8_t *TR_X86RelocationTarget::arrayCopyHelperAddress(J9JavaVM *javaVM)
{
    uintptr_t *funcdescrptr = (UDATA *)javaVM->memoryManagerFunctions->referenceArrayCopy;
    return (uint8_t *)funcdescrptr[0];
}

void TR_X86RelocationTarget::patchNonVolatileFieldMemoryFence(J9ROMFieldShape *resolvedField, UDATA cpAddr,
    ...
    */
    ...
    // Is this a LCMPXCHG of a long store? ( f00f are the first two bytes of a LCMPXCHG instruction)
        ...
        if (TR::Options::getAOTCmdLineOptions() && TR::Options::getAOTCmdLineOptions()->getOption(TR_X86UseMFENCE)) {
            ...
            TR_ASSERT(*((U_16 *)(instructionAddress + barrierOffset)) == MFenceWordCheck,
                "Expecting mfence, not Lock or [esp]");
...
#endif // defined(TR_HOST_X86) && !defined(J9HAMMER)
        {
            ...
            TR_ASSERT(*((U_16 *)(instructionAddress + barrierOffset)) != MFenceWordCheck,
                "Expecting Lock or [esp], not Mfence");
