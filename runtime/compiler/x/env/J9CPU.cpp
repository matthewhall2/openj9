...
TR::CPU J9::X86::CPU::detectRelocatable(OMRPortLibrary * const omrPortLib)
{
    ...
    const uint32_t customFeatures[] = { OMR_FEATURE_X86_FPU, OMR_FEATURE_X86_CX8, OMR_FEATURE_X86_CMOV,
        OMR_FEATURE_X86_MMX, OMR_FEATURE_X86_SSE, OMR_FEATURE_X86_SSE2, OMR_FEATURE_X86_SSSE3, OMR_FEATURE_X86_SSE4_1,
        OMR_FEATURE_X86_POPCNT, OMR_FEATURE_X86_SSE3, OMR_FEATURE_X86_AESNI, OMR_FEATURE_X86_AVX };
    ...
    for (size_t i = 0; i < OMRPORT_SYSINFO_FEATURES_SIZE; i++) {
        portableProcessorDescription.features[i]
            = hostProcessorDescription.features[i] & customProcessorDescription.features[i];
    }

    return TR::CPU::customize(portableProcessorDescription);
}

TR::CPU J9::X86::CPU::detect(OMRPortLibrary * const omrPortLib)
{
    if (omrPortLib == NULL)
        return TR::CPU();
...
void J9::X86::CPU::enableFeatureMasks()
{
    ...
    const uint32_t utilizedFeatures[] = { OMR_FEATURE_X86_FPU, OMR_FEATURE_X86_CX8, OMR_FEATURE_X86_CMOV,
        OMR_FEATURE_X86_MMX, OMR_FEATURE_X86_SSE, OMR_FEATURE_X86_SSE2, OMR_FEATURE_X86_SSSE3, OMR_FEATURE_X86_SSE4_1,
        OMR_FEATURE_X86_POPCNT, OMR_FEATURE_X86_AESNI, OMR_FEATURE_X86_OSXSAVE, OMR_FEATURE_X86_AVX,
        OMR_FEATURE_X86_FMA, OMR_FEATURE_X86_HLE, OMR_FEATURE_X86_RTM, OMR_FEATURE_X86_SSE3, OMR_FEATURE_X86_AVX2,
        OMR_FEATURE_X86_AVX512F, OMR_FEATURE_X86_AVX512VL, OMR_FEATURE_X86_AVX512BW, OMR_FEATURE_X86_AVX512DQ,
        OMR_FEATURE_X86_AVX512CD, OMR_FEATURE_X86_SSE4_2, OMR_FEATURE_X86_BMI2, OMR_FEATURE_X86_AVX_VNNI };
...
TR_X86CPUIDBuffer *J9::X86::CPU::queryX86TargetCPUID()
{
...
const char *J9::X86::CPU::getProcessorVendorId() { return self()->getX86ProcessorVendorId(); }

uint32_t J9::X86::CPU::getProcessorSignature() { return self()->getX86ProcessorSignature(); }

bool J9::X86::CPU::isCompatible(const OMRProcessorDesc &processorDescription)
{
...
bool J9::X86::CPU::is(OMRProcessorArchitecture p)
{
...
bool J9::X86::CPU::supportsFeature(uint32_t feature)
{
    OMRPORT_ACCESS_FROM_OMRPORT(TR::Compiler->omrPortLib);
    ...
    static bool disableCPUDetectionTest = feGetEnv("TR_DisableCPUDetectionTest");
    if (!disableCPUDetectionTest) {
        TR_ASSERT_FATAL(self()->supports_feature_test(feature),
            "Old API and new API did not match: processor feature %d\n", feature);
        TR_ASSERT_FATAL(TRUE == omrsysinfo_processor_has_feature(&_supportedFeatureMasks, feature),
            "New processor feature usage detected, please add feature %d to _supportedFeatureMasks via "
            "TR::CPU::enableFeatureMasks()\n",
            feature);
    }
...
uint32_t J9::X86::CPU::getX86ProcessorFeatureFlags()
{
#if defined(J9VM_OPT_JITSERVER)
...
uint32_t J9::X86::CPU::getX86ProcessorFeatureFlags2()
{
#if defined(J9VM_OPT_JITSERVER)
    if (auto stream = TR::CompilationInfo::getStream()) {
        auto *vmInfo = TR::compInfoPT->getClientData()->getOrCacheVMInfo(stream);
...
uint32_t J9::X86::CPU::getX86ProcessorFeatureFlags8()
{
#if defined(J9VM_OPT_JITSERVER)
    if (auto stream = TR::CompilationInfo::getStream()) {
        auto *vmInfo = TR::compInfoPT->getClientData()->getOrCacheVMInfo(stream);
...
bool J9::X86::CPU::is_test(OMRProcessorArchitecture p)
{
#if defined(J9VM_OPT_JITSERVER)
    ...
    switch (p) {
        ...
        case OMR_PROCESSOR_X86_INTEL_SANDYBRIDGE:
            return TR::CodeGenerator::getX86ProcessorInfo().isIntelSandyBridge()
                == (_processorDescription.processor == p);
        case OMR_PROCESSOR_X86_INTEL_IVYBRIDGE:
            return TR::CodeGenerator::getX86ProcessorInfo().isIntelIvyBridge()
                == (_processorDescription.processor == p);
        ...
        case OMR_PROCESSOR_X86_INTEL_BROADWELL:
            return TR::CodeGenerator::getX86ProcessorInfo().isIntelBroadwell()
                == (_processorDescription.processor == p);
        ...
        case OMR_PROCESSOR_X86_INTEL_CASCADELAKE:
            return TR::CodeGenerator::getX86ProcessorInfo().isIntelCascadeLake()
                == (_processorDescription.processor == p);
        case OMR_PROCESSOR_X86_INTEL_COOPERLAKE:
            return TR::CodeGenerator::getX86ProcessorInfo().isIntelCooperLake()
                == (_processorDescription.processor == p);
        ...
        case OMR_PROCESSOR_X86_INTEL_SAPPHIRERAPIDS:
            return TR::CodeGenerator::getX86ProcessorInfo().isIntelSapphireRapids()
                == (_processorDescription.processor == p);
        case OMR_PROCESSOR_X86_INTEL_EMERALDRAPIDS:
            return TR::CodeGenerator::getX86ProcessorInfo().isIntelEmeraldRapids()
                == (_processorDescription.processor == p);
        case OMR_PROCESSOR_X86_AMD_ATHLONDURON:
            return TR::CodeGenerator::getX86ProcessorInfo().isAMDAthlonDuron()
                == (_processorDescription.processor == p);
...
bool J9::X86::CPU::supports_feature_test(uint32_t feature)
{
#if defined(J9VM_OPT_JITSERVER)
    if (TR::CompilationInfo::getStream())
        return true;
