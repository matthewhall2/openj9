...
TR::IA32J9SystemLinkage::IA32J9SystemLinkage(TR::CodeGenerator *cg)
    : TR::IA32SystemLinkage(cg)
{
    _properties._framePointerRegister = TR::RealRegister::ebx;
    _properties._methodMetaDataRegister = TR::RealRegister::ebp;
}

TR::Register *TR::IA32J9SystemLinkage::buildVolatileAndReturnDependencies(TR::Node *callNode,
...
TR::Register *TR::IA32J9SystemLinkage::buildDirectDispatch(TR::Node *callNode, bool spillFPRegs)
{
...
int32_t TR::IA32J9SystemLinkage::buildParametersOnCStack(TR::Node *callNode, int firstParamIndex, bool passVMThread,
    bool wrapAddress)
{
    ...
    if (argSize % 16 != 0) {
        ...
        logprintf(comp()->getOption(TR_TraceCG), comp()->log(),
            "adjust arguments size by %d to make arguments 16 byte aligned \n", adjustedArgSize);
    }
    for (int offset = adjustedArgSize; !paramsSlotsOnStack.isEmpty();
