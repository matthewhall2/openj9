...
TR::Register *J9::X86::I386::JNILinkage::buildJNIDispatch(TR::Node *callNode)
{
    ...
    cg()->getJNICallSites().push_front(new (trHeapMemory())
            TR_Pair<TR_ResolvedMethod, TR::Instruction>(resolvedMethodSymbol->getResolvedMethod(), instr));
    ...
    if (checkExceptions) {
        ...
        TR::Snippet *snippet = new (trHeapMemory())
            TR::X86CheckFailureSnippet(cg(), cg()->symRefTab()->findOrCreateRuntimeHelper(TR_throwCurrentException),
                snippetLabel, instr, callNode->getDataType() == TR::Float || callNode->getDataType() == TR::Double);
