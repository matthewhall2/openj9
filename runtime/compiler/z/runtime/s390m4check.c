...
int main(int argc, char **argv)
{
    ...
    while (!feof(in)) {
        ...
        {
            if ((!((data[i] >= 64 && data[i] <= 249) || data[i] == 13 || data[i] == 21 || data[i] == 0) && ascii == 0)
                || /* EBCDIC */
                (!((data[i] >= 31 && data[i] <= 127) || data[i] == 13 || data[i] == 10 || data[i] == 0)
                    && ascii == 1)) /* ASCII  */
            {
        ...
        if ((data[0] == 'Z' && data[1] == 'Z' && m4 == 1)
            || (m4 == 0
                && ((data[0] == '*' && data[1] == '*' && ascii == 0)
                    || (data[0] == '#' && data[1] == '#' && ascii == 1)))) /* filter out the comments */
        {
            linecnt++;
            continue;
        }
        ...
        {
            if (data[j] != ' ') {
                if ((data[j] == '#' && data[j + 1] != '#' && j == 0) || (data[j] == 'Z' && data[j + 1] == 'Z' && j != 0)
        ...
        for (i = 0; i < datalength; i++) {
            if (data[i] == '#' || (data[i] == 'Z' && data[i - 1] == 'Z') || data[i] == '\n'
                || (data[i] == '*' && data[i - 1] == '*' && ascii == 0)) { /* do not evaluate comments */
                break;
            } else { /* check all other requirements  */
                if (data[i] == ',' && (data[i - 1] == ' ' || data[i + 1] == ' ' || i == 0)) { /* comma errors */
                ...
                if (data[i] == '(' && data[i - 1] == ' '
                    && data[i - 2] == ',') /* check for whitespace before brackets */
                {
