    ...
    TR::RegisterDependencyConditions *dependencies, int32_t argSize)
{
    ...
    if (isJitDispatchJ9Method) {
        ...
        // TR::RegisterDependencyConditions(dependencies, 1, 0, cg()); preDeps->addPreCondition(scratchReg,
        // getVTableIndexArgumentRegister()); preDeps->setNumPostConditions(0, trMemory());
        // preDeps->setAddCursorForPost(0);

        // Make a copy of input deps, but add on 2 new slots.
        //       TR::RegisterDependencyConditions * postDeps = new (trHeapMemory())
        //       TR::RegisterDependencyConditions(dependencies, 0, 1, cg());
        //        postDeps->setAddCursorForPre(0);                     // Ignore all pre-deps that were copied.
        //        postDeps->setNumPreConditions(0, trMemory());        // Ignore all pre-deps that were copied.
        ...
        TR::Snippet *snippet = new (trHeapMemory())
            TR::S390J9HelperCallSnippet(cg(), callNode, snippetLabel, helperRef, doneLabel, argSize);
        ...
        //  cg()->addInvokeBasicCallSite(callNode, gcPoint);
        gcPoint->setNeedsGCMap(getPreservedRegisterMapForGC());
        // TR::Instruction *restartInstruction = generateS390LabelInstruction(cg(), TR::InstOpCode::label, callNode,
        // OOLReturnLabel);
