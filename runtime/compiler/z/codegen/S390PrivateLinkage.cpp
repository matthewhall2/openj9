...
#define MAX_IMPLEMENTERS_TO_EVALUATE 30
#define MIN_ITABLE_ITERATIONS 1
#define MAX_ITABLE_ITERATIONS 4
#define iComment(str, cursor) \
    if (debugObj)             \
        debugObj->addInstructionComment(cursor, (const_cast<char *>(str)));
...
////////////////////////////////////////////////////////////////////////////////
void J9::Z::PrivateLinkage::initS390RealRegisterLinkage()
{
...
void J9::Z::PrivateLinkage::alignLocalsOffset(uint32_t &stackIndex, uint32_t localObjectAlignment)
{
    if (stackIndex % localObjectAlignment != 0) {
        ...
        atlas->setNumberOfSlotsMapped(atlas->getNumberOfSlotsMapped()
            + ((stackIndexBeforeAlignment - stackIndex) / TR::Compiler->om.sizeofReferenceAddress()));

        logprintf(comp()->getOption(TR_TraceRA), comp()->log(),
            "\nAlign stack offset before alignment = %d and after alignment = %d\n", stackIndexBeforeAlignment,
            stackIndex);
    }
}
...
////////////////////////////////////////////////////////////////////////////////
void J9::Z::PrivateLinkage::mapCompactedStack(TR::ResolvedMethodSymbol *method)
{
...

    {
        ...
        int32_t *colourToOffsetMap = (int32_t *)trMemory()->allocateStackMemory(
            cg()->getLocalsIG()->getNumberOfColoursUsedToColour() * sizeof(int32_t));

        uint32_t *colourToSizeMap = (uint32_t *)trMemory()->allocateStackMemory(
            cg()->getLocalsIG()->getNumberOfColoursUsedToColour() * sizeof(uint32_t));
        ...
        for (localCursor = variableSizeSymIterator.getFirst(); localCursor;
             localCursor = variableSizeSymIterator.getNext()) {
        ...
        // because the offset of a local object is (stackIndex +
        // pointerSize*(localCursor->getGCMapIndex()-firstLocalGCIndex)) In createStackAtlas, we align
        // pointerSize*(localCursor->getGCMapIndex()-firstLocalGCIndex) by modifying local objects' gc indices Here we
        // align the stackIndex
        // *************************************how we align local objects********************************
        //

        logprintf(trace, log, "stackIndex after compaction = %d\n", stackIndex);

        // stackIndex in mapCompactedStack is calculated using only local reference sizes and does not include the
        // padding
        ...
        if (localObjectAlignment >= 16) {
            // we don't want to fail gc when it tries to uncompress the reference of a stack allocated object, so we
            // aligned the local objects based on the shift amount this is different to the alignment of heap objects,
            // which is controlled separately and could be larger than 2<<shiftamount
        ...
        for (localCursor = automaticIterator.getFirst(); localCursor; localCursor = automaticIterator.getNext())
            if (localCursor->getGCMapIndex() >= 0) {
                ...
                TR_ASSERT((localCursor->getOffset() <= 0),
        ...
        while (variableSizeSymCursor != NULL) {
            if (variableSizeSymCursor->isReferenced()) {
                logprintf(traceBCD, log, "map variableSize sym %p (size %d) because isReferenced=true ",
        ...
        TR_ASSERT((stackIndex % pointerSize) == 0, "size of scalar temp area not a multiple of Java pointer size");
        ...
        // it is possible that the stack might not be double-word aligned, due to mapping for long displacement if the
        // pointer size is 4
#ifdef DEBUG
        origSize += (origSize & 0x4) ? 4 : 0;
#endif
    ...
    if (debug("reportCL")) {
        ...
        diagnostic("\n**** Mapped locals size: %d (orig map size=%d, shared size=%d)  %s\n", (mappedSize), origSize,
            origSize - mappedSize, comp()->signature());
    }
#endif
}

void J9::Z::PrivateLinkage::mapStack(TR::ResolvedMethodSymbol *method)
{
    ...
    if (localObjectAlignment >= 16) {
        // we don't want to fail gc when it tries to uncompress the reference of a stack allocated object, so we aligned
        // the local objects based on the shift amount this is different to the alignment of heap objects, which is
        // controlled separately and could be larger than 2<<shiftamount
        alignLocalsOffset(stackIndex, localObjectAlignment);
    }
    ...
    while (variableSizeSymCursor != NULL) {
        ...
        if (variableSizeSymCursor->isReferenced()) {
            logprintf(trace, log, "map variableSize sym %p (size %d) because isReferenced=true ", variableSizeSymCursor,
                variableSizeSymCursor->getSize());
    ...
    if (comp()->getOption(TR_Randomize) && comp()->getOptions()->get390StackBufferSize() == 0) {
        if (cg()->randomizer.randomBoolean(300)
            && performTransformation(comp(),
                "O^O Random Codegen  - Added 5000 dummy slots to Java Stack frame to test large displacement.\n")) {
            stackIndex -= 5000;
        } else {
            stackIndex -= 0;
        }
...
////////////////////////////////////////////////////////////////////////////////
void J9::Z::PrivateLinkage::mapSingleAutomatic(TR::AutomaticSymbol *p, uint32_t &stackIndex)
{
...
// J9::Z::PrivateLinkage::mapSingleAutomatic - maps an automatic onto the stack
////////////////////////////////////////////////////////////////////////////////
void J9::Z::PrivateLinkage::mapSingleAutomatic(TR::AutomaticSymbol *p, uint32_t size, uint32_t &stackIndex)
{
    p->setOffset(stackIndex -= size);
}

bool J9::Z::PrivateLinkage::hasToBeOnStack(TR::ParameterSymbol *parm)
{
    TR_J9VMBase *fej9 = (TR_J9VMBase *)(fe());
        ...
        ((parm->getLinkageRegisterIndex() == 0 && // is first parameter (this pointer)
             ...
             ((bodySymbol->isSynchronised())
                 || (!strncmp(bodySymbol->getResolvedMethod()->nameChars(), "<init>", 6)
                     && ((throwableClass = fej9->getClassFromSignature("Ljava/lang/Throwable;", 21,
                              bodySymbol->getResolvedMethod()))
                             == 0
                         || fej9->isInstanceOf(bodySymbol->getResolvedMethod()->containingClass(), throwableClass, true)
                             != TR_no))))
...
void J9::Z::PrivateLinkage::setParameterLinkageRegisterIndex(TR::ResolvedMethodSymbol *method)
{
    ...
    while ((paramCursor != NULL)
        ...
        if (self()->isFastLinkLinkageType()) {
            if ((numFloatArgs == 1) || (numIntArgs >= self()->getNumIntegerArgumentRegisters())) {
                // force fastlink ABI condition of only one float parameter for fastlink parameter and it must be within
                // first slots
...
static TR::Instruction *initStg(TR::CodeGenerator *codeGen, TR::Node *node, TR::RealRegister *tmpReg,
    TR::RealRegister *srcReg, TR::RealRegister *itersReg, int32_t baseOffset, int32_t numBytes, TR::Instruction *cursor)
{
    ...
    if ((numBytes < 4096) && (numIters * 256 + baseOffset < 4096)) {
        ...
        if (baseOffset >= MIN_IMMEDIATE_VAL && baseOffset <= MAX_IMMEDIATE_VAL) {
            ...
            cursor = generateRIInstruction(codeGen, TR::InstOpCode::getAddHalfWordImmOpCode(), node, baseReg,
                baseOffset, cursor);
...
int32_t J9::Z::PrivateLinkage::calculateRegisterSaveSize(TR::RealRegister::RegNum firstUsedReg,
    TR::RealRegister::RegNum lastUsedReg, int32_t &registerSaveDescription, int32_t &numIntSaved,
    int32_t &numFloatSaved)
{
    ...
    regSaveSize += numIntSaved * cg()->machine()->getGPRSize() + numFloatSaved * cg()->machine()->getFPRSize();
...
int32_t J9::Z::PrivateLinkage::setupLiteralPoolRegister(TR::Snippet *firstSnippet)
{
...
////////////////////////////////////////////////////////////////////////////////
void J9::Z::PrivateLinkage::createPrologue(TR::Instruction *cursor)
{
    OMR::Logger *log = comp()->log();
    bool trace = comp()->getOption(TR_TraceCG);
    ...
    // Look between R6-R11
    //
    TR::RealRegister::RegNum firstUsedReg = getFirstSavedRegister(TR::RealRegister::GPR6, TR::RealRegister::GPR12);
    TR::RealRegister::RegNum lastUsedReg = getLastSavedRegister(TR::RealRegister::GPR6, TR::RealRegister::GPR12);

    // compute the register save area
    regSaveSize
        = calculateRegisterSaveSize(firstUsedReg, lastUsedReg, registerSaveDescription, numIntSaved, numFloatSaved);
    ...
    if (prologTuning) {
        ...
        cursor = generateRXInstruction(cg(), TR::InstOpCode::getExtendedStoreOpCode(), firstNode,
            getRealRegister(getReturnAddressRegister()), retAddrMemRef, cursor);
    }
    ...
    if (largeStack) {
        ...
        cursor = generateRXInstruction(cg(), TR::InstOpCode::LAY, firstNode, spReg,
            generateS390MemoryReference(spReg, (size) * -1, cg()), cursor);
    }
    ...
    if (!comp()->isDLT()) {
        ...
        cursor = generateRXInstruction(cg(), TR::InstOpCode::getCmpLogicalOpCode(), firstNode, spReg, stackLimitMR,
            cursor);
        ...
        cursor = generateS390BranchInstruction(cg(), TR::InstOpCode::BRC, TR::InstOpCode::COND_BL, firstNode,
            stackOverflowSnippetLabel, cursor);

        TR::SymbolReference *stackOverflowRef
            = comp()->getSymRefTab()->findOrCreateStackOverflowSymbolRef(comp()->getJittedMethodSymbol());

        TR::Snippet *snippet = new (trHeapMemory()) TR::S390StackCheckFailureSnippet(cg(), firstNode, reStartLabel,
            stackOverflowSnippetLabel, stackOverflowRef, size - cg()->machine()->getGPRSize());
    ...
    if (bppoutline && cg()->_outlineCall._frequency != -1) {
        cursor = new (cg()->trHeapMemory()) TR::S390RILInstruction(TR::InstOpCode::LARL, firstNode, epReg,
            (cg()->_outlineCall._callSymRef)->getSymbol(), (cg()->_outlineCall._callSymRef), cursor, cg());

        TR::MemoryReference *tempMR = generateS390MemoryReference(epReg, 0, cg());
        cursor = generateS390BranchPredictionPreloadInstruction(cg(), TR::InstOpCode::BPP, firstNode,
            cg()->_outlineCall._callLabel, (int8_t)0xD, tempMR, cursor);
    }
    if (bppoutline && cg()->_outlineArrayCall._frequency != -1) {
        cursor = new (cg()->trHeapMemory()) TR::S390RILInstruction(TR::InstOpCode::LARL, firstNode, epReg,
            (cg()->_outlineArrayCall._callSymRef)->getSymbol(), (cg()->_outlineArrayCall._callSymRef), cursor, cg());

        TR::MemoryReference *tempMR = generateS390MemoryReference(epReg, 0, cg());
        cursor = generateS390BranchPredictionPreloadInstruction(cg(), TR::InstOpCode::BPP, firstNode,
            cg()->_outlineArrayCall._callLabel, (int8_t)0xD, tempMR, cursor);
    }

    if (cg()->getSupportsRuntimeInstrumentation())
        cursor = TR::TreeEvaluator::generateRuntimeInstrumentationOnOffSequence(cg(), TR::InstOpCode::RION, firstNode,
            cursor, true);
    ...
    if (lastUsedReg != TR::RealRegister::NoReg) {
        ...
        if (firstUsedReg != lastUsedReg) {
            cursor = generateRSInstruction(cg(), TR::InstOpCode::getStoreMultipleOpCode(), firstNode,
                getRealRegister(firstUsedReg), getRealRegister(lastUsedReg), rsa, cursor);
        } else {
            cursor = generateRXInstruction(cg(), TR::InstOpCode::getStoreOpCode(), firstNode,
                getRealRegister(firstUsedReg), rsa, cursor);
        }
    }
    ...
    for (i = TR::RealRegister::FPR8; i <= TR::RealRegister::FPR15; ++i) {
        if ((getRealRegister(i))->getHasBeenAssignedInMethod()) {
            cursor = generateRXInstruction(cg(), TR::InstOpCode::STD, firstNode, getRealRegister(i),
    ...
    if (setupLiteralPoolRegister(firstSnippet) > 0) {
        cursor = new (trHeapMemory())
            TR::S390RILInstruction(TR::InstOpCode::LARL, firstNode, lpReg, firstSnippet, cursor, cg());
    }
    ...
    if (cg()->isPrefetchNextStackCacheLine() && prefetchStack) {
        cursor = generateRXInstruction(cg(), TR::InstOpCode::PFD, firstNode, 2,
            generateS390MemoryReference(spReg, -256, cg()), cursor);
    }
...
////////////////////////////////////////////////////////////////////////////////
void J9::Z::PrivateLinkage::createEpilogue(TR::Instruction *cursor)
{
    ...
    bool restoreRA = disableRARestoreOpt || !(performTransformation(comp(), "O^O No need to restore RAREG in epilog\n"))
        || getRealRegister(getReturnAddressRegister())->getHasBeenAssignedInMethod() || cg()->canExceptByTrap()
        || cg()->getExitPointsInMethod() || bodySymbol->isEHAware()
    ...
    if (getRaContextRestoreNeeded()) {
        cursor = generateRXInstruction(cg(), TR::InstOpCode::getExtendedLoadOpCode(), nextNode,
            getRealRegister(getReturnAddressRegister()), generateS390MemoryReference(spReg, frameSize, cg()), cursor);
    } else {
    ...
    if (enableBranchPreload && (cursor->getNext() == cg()->_hottestReturn._returnInstr)) {
        if (cg()->_hottestReturn._frequency > 6 && cg()->_hottestReturn._insertBPPInEpilogue) {
            ...
            TR::MemoryReference *tempMR
                = generateS390MemoryReference(getRealRegister(getReturnAddressRegister()), 0, cg());
            cursor = generateS390BranchPredictionPreloadInstruction(cg(), TR::InstOpCode::BPP, nextNode,
                cg()->_hottestReturn._returnLabel, (int8_t)0x6, tempMR, cursor);
    ...
    if (lastUsedReg != TR::RealRegister::NoReg) {
        if (firstUsedReg != lastUsedReg) {
            cursor = restorePreservedRegs(firstUsedReg, lastUsedReg, blockNumber, cursor, nextNode, spReg, rsa,
                getStackPointerRegister());
        } else {
            cursor = generateRXInstruction(cg(), TR::InstOpCode::getLoadOpCode(), nextNode,
                getRealRegister(firstUsedReg), rsa, cursor);
        }
    ...
    if (adjustSize < MAXDISP) {
        cursor = generateRXInstruction(cg(), TR::InstOpCode::LA, nextNode, spReg,
            generateS390MemoryReference(spReg, adjustSize, cg()), cursor);
    } else if (adjustSize < MAXLONGDISP) {
        cursor = generateRXInstruction(cg(), TR::InstOpCode::LAY, nextNode, spReg,
            generateS390MemoryReference(spReg, adjustSize, cg()), cursor);
    } else {
    ...
    if (cg()->getSupportsRuntimeInstrumentation())
        cursor = TR::TreeEvaluator::generateRuntimeInstrumentationOnOffSequence(cg(), TR::InstOpCode::RIOFF,
            currentNode, cursor, true);

    if (enableBranchPreload) {
        if (cursor->getNext() == cg()->_hottestReturn._returnInstr) {
            if (cg()->_hottestReturn._frequency > 6) {
                cursor = generateS390LabelInstruction(cg(), TR::InstOpCode::label, currentNode,
                    cg()->_hottestReturn._returnLabel, cursor);
            }
        }
    }

    cursor = generateS390RegInstruction(cg(), TR::InstOpCode::BCR, currentNode,
        getRealRegister(getReturnAddressRegister()), cursor);
    ((TR::S390RegInstruction *)cursor)->setBranchCondition(TR::InstOpCode::COND_BCR);
}

static int32_t getITableIterationsNumber(TR::Compilation *comp, TR::SymbolReference *methodSymRef,
    TR_OpaqueClassBlock *declaringClass)
{
    ...
    // Set a fix number between 1 and 32767 (int16_MAX) of iterations. Check all iTableEntries if a number larger than
    // 32767 is provided.
    ...
    if (numITableIterationsAfterLastITableCacheCheck) {
        ...
        logprintf(trace, log,
            "ITable iteration is %d because TR_NumITableIterationsAfterLastITableCacheCheck was set.\n", iterations);
        return iterations;
    }
    ...
    if (!comp->getOption(TR_DisableCHOpts)) {
        ...
        int32_t numImplementers = chTable->findnInterfaceImplementers(declaringClass, MAX_IMPLEMENTERS_TO_EVALUATE,
...
TR::Instruction *generateLastITableAndITableInstructions(TR::CodeGenerator *cg, TR::Node *callNode,
    TR::Register *vftReg, TR::Register *entryPointRegister, TR::Register *vTableIndexRegister,
    TR::Register *lastIpicMethodRegister, TR::RegisterDependencyConditions *postDeps, TR::Instruction *cursor)
{
    ...
    // Add last iTable cache instructions if possible/
    if (!cg->profiledPointersRequireRelocation() && !comp->getOption(TR_DisableLastITableCache)
        && (declaringClass
            = methodSymRef->getOwningMethod(comp)->getResolvedInterfaceMethod(methodSymRef->getCPIndex(), &itableIndex))
        ...
        // If iTableIterations is larger than ITABLE_DYNAMIC_LOOP_THRESHOLD but less than INT16_MAX, check iTable
        // entries in a loop but exit after iTableIterations times.
        bool checkLimitedNumberOfITableEntries
            = !checkAllITableEnrties && iTableIterations > ITABLE_DYNAMIC_LOOP_THRESHOLD;
        ...
        if (lastIpicMethodRegister == NULL) {
            cursor = generateS390BranchInstruction(cg, TR::InstOpCode::BRC, TR::InstOpCode::COND_B, callNode,
                entryLabel, cursor);
        } else {
            // If lastIpicMethodRegister exist, it should have a non NULL value. Otherwise the PIC slots are not fully
            // populated.
            cursor = generateS390CompareAndBranchInstruction(cg, TR::InstOpCode::getCmpOpCode(), callNode,
                lastIpicMethodRegister, 0, TR::InstOpCode::COND_BNE, entryLabel, false /* needsCC */,
                true /* targetIsFarAndCold */, cursor);
        }
        TR_S390OutOfLineCodeSection *outlinedITableCheckSequence
            = new (cg->trHeapMemory()) TR_S390OutOfLineCodeSection(entryLabel, exitLabel, cg);
        ...
        TR::Instruction *oolCursor = generateS390LabelInstruction(cg, TR::InstOpCode::label, callNode, entryLabel);

        static bool breakBeforeIPICUsingLastITable = feGetEnv("TR_breakBeforeIPICUsingLastITable") != NULL;
        if (breakBeforeIPICUsingLastITable)
            oolCursor = generateS390EInstruction(cg, TR::InstOpCode::BREAK, callNode, oolCursor);

        /********* Step 1: Check if LastITable is the target interface. *********/
        // Load the declaringClass in the vTableIndexRegister.
        oolCursor = genLoadAddressConstant(cg, callNode, (int64_t)declaringClass, vTableIndexRegister, oolCursor);
        iComment("Loading resolved interface method.", oolCursor)
            // Loading LastITable.
            oolCursor
            = generateRXInstruction(cg, TR::InstOpCode::getLoadOpCode(), callNode, entryPointRegister,
                generateS390MemoryReference(vftReg, static_cast<int32_t>(fej9->getOffsetOfLastITableFromClassField()),
                    cg),
                oolCursor);
        iComment("Loading lastITable.", oolCursor)
            // Compare the declaringClass with the LastItable->J9Class.
            oolCursor
            = generateRXInstruction(cg, TR::InstOpCode::getCmpLogicalOpCode(), callNode, vTableIndexRegister,
                generateS390MemoryReference(entryPointRegister, fej9->getOffsetOfInterfaceClassFromITableField(), cg),
                oolCursor);
        // If not matching, jump over dispatch instaructions.
        oolCursor = generateS390BranchInstruction(cg, TR::InstOpCode::BRC, TR::InstOpCode::COND_BNE, callNode,
            noLastITableMatchLabel, oolCursor);
        iComment("No Match, jump to next test.", oolCursor)

            /********* Step 2: Dispatch to the method implementation. *********/
            if (checkITableEntries) oolCursor
            = generateS390LabelInstruction(cg, TR::InstOpCode::label, callNode, matchLabel, oolCursor);
        // Calculating vTableIndex value. vTableIndexRegister must have the vTableIndex value when dispatching.
        oolCursor = generateRIInstruction(cg, TR::InstOpCode::getLoadHalfWordImmOpCode(), callNode, vTableIndexRegister,
            fej9->getITableEntryJitVTableOffset(), oolCursor);
        iComment("Start calculating entry point.", oolCursor) oolCursor
            = generateRXInstruction(cg, TR::InstOpCode::getSubstractOpCode(), callNode, vTableIndexRegister,
                generateS390MemoryReference(entryPointRegister, fej9->convertITableIndexToOffset(itableIndex), cg),
                oolCursor);
        // vTableIndexRegister is R0 so we can not use it in a memory reference. Copy the value to entryPointRegister,
        // to use in memory reference.
        oolCursor = generateRRInstruction(cg, TR::InstOpCode::getLoadRegOpCode(), callNode, entryPointRegister,
            vTableIndexRegister, oolCursor);

        // Jump to the entry point. Return address register already has the correct value.
        oolCursor = generateRXInstruction(cg, TR::InstOpCode::getLoadOpCode(), callNode, entryPointRegister,
            generateS390MemoryReference(entryPointRegister, vftReg, 0, cg), oolCursor);
        oolCursor = generateS390BranchInstruction(cg, TR::InstOpCode::BCR, callNode, TR::InstOpCode::COND_B,
            entryPointRegister, oolCursor);
        iComment("Branch to the target entry point.", oolCursor)
            ...
            if (checkITableEntries)
        {
            ...
            oolCursor
                = generateS390LabelInstruction(cg, TR::InstOpCode::label, callNode, noLastITableMatchLabel, oolCursor);
            iComment("Start comparing interface with iTable entries.", oolCursor) if (checkLimitedNumberOfITableEntries)
            {
                ...
                if (loopCountRegister == NULL) {
                    ...
                    stopUsingLoopCountRegister = true;
                }
                oolCursor = generateRIInstruction(cg, TR::InstOpCode::getLoadHalfWordImmOpCode(), callNode,
                    loopCountRegister, iTableIterations, oolCursor);
            }

            if (checkAllITableEnrties || checkLimitedNumberOfITableEntries) {
                // Use a loop to check all or first few iTable entries.
                const int16_t initialOffsetFromVft
                    = static_cast<int16_t>(offsetof(J9Class, iTable) - offsetof(J9ITable, next));
                TR::LabelSymbol *loopTopLabel = generateLabelSymbol(cg);
                if (comp->target().cpu.isAtLeast(OMR_PROCESSOR_S390_Z196)) {
                    oolCursor = generateRIEInstruction(cg, TR::InstOpCode::getAddHalfWordImmDistinctOperandOpCode(),
                        callNode, entryPointRegister, vftReg, initialOffsetFromVft, oolCursor);
                } else {
                    oolCursor = generateRRInstruction(cg, TR::InstOpCode::getLoadRegOpCode(), callNode,
                        entryPointRegister, vftReg, oolCursor);
                    oolCursor = generateRIInstruction(cg, TR::InstOpCode::getAddHalfWordImmOpCode(), callNode,
                        entryPointRegister, initialOffsetFromVft, oolCursor);
                }
                // Start of the loop.
                oolCursor = generateS390LabelInstruction(cg, TR::InstOpCode::label, callNode, loopTopLabel, oolCursor);
                oolCursor = generateRXInstruction(cg, TR::InstOpCode::getLoadTestOpCode(), callNode, entryPointRegister,
                    generateS390MemoryReference(entryPointRegister, offsetof(J9ITable, next), cg), oolCursor);
                // Exit if the value is NULL.
                oolCursor = generateS390BranchInstruction(cg, TR::InstOpCode::BRC, TR::InstOpCode::COND_BZ, callNode,
                    oolMergeLabel, oolCursor);
                iComment("Exit if null.", oolCursor)
                    ...
                    oolCursor
                    = generateRXInstruction(cg, TR::InstOpCode::getCmpLogicalOpCode(), callNode, vTableIndexRegister,
                        generateS390MemoryReference(entryPointRegister,
                            fej9->getOffsetOfInterfaceClassFromITableField(), cg),
                        oolCursor);
                oolCursor = generateS390BranchInstruction(cg, TR::InstOpCode::BRC, TR::InstOpCode::COND_BE, callNode,
                    matchLabel, oolCursor);
                iComment("Branch to call dispatch sequence.", oolCursor) if (checkAllITableEnrties)
                {
                    // Check the next entry. It will exit the loop at the last entry because the "next"  pointer will be
                    // null.
                    oolCursor = generateS390BranchInstruction(cg, TR::InstOpCode::BRC, TR::InstOpCode::COND_B, callNode,
                        loopTopLabel, oolCursor);
                    iComment("Check next entry.", oolCursor)
                }
                else
                {
                    // Check the next entry for "iTableIterations" time or until the last entry.
                    oolCursor = generateS390BranchInstruction(cg, TR::InstOpCode::BRCT, callNode, loopCountRegister,
                        loopTopLabel, oolCursor);
                    iComment("Check next entry.", oolCursor)
                }
            } else {
                for (int32_t i = 0; i < iTableIterations; i++) {
                    ...
                    if (i == 0)
                        ...
                        iTablePointerReference
                            = generateS390MemoryReference(entryPointRegister, offsetof(J9ITable, next), cg);

                    oolCursor = generateRXInstruction(cg, TR::InstOpCode::getLoadTestOpCode(), callNode,
                        entryPointRegister, iTablePointerReference, oolCursor);
                    // Exit if the value is NULL.
                    oolCursor = generateS390BranchInstruction(cg, TR::InstOpCode::BRC, TR::InstOpCode::COND_BZ,
                        callNode, oolMergeLabel, oolCursor);
                    iComment("Exit loop if null.", oolCursor) oolCursor = generateRXInstruction(cg,
                        TR::InstOpCode::getCmpLogicalOpCode(), callNode, vTableIndexRegister,
                        generateS390MemoryReference(entryPointRegister,
                            fej9->getOffsetOfInterfaceClassFromITableField(), cg),
                        oolCursor);
                    // Compare the class with the target interface and jump to the dispatch sequence if match.
                    // vTableIndexRegister contains the value of the target interface.
                    oolCursor = generateS390BranchInstruction(cg, TR::InstOpCode::BRC, TR::InstOpCode::COND_BE,
                        callNode, matchLabel, oolCursor);
        ...
        oolCursor = generateS390BranchInstruction(cg, TR::InstOpCode::BRC, TR::InstOpCode::COND_B, callNode, exitLabel,
            oolCursor);
        iComment("Back to main line.", oolCursor) outlinedITableCheckSequence->swapInstructionListsWithCompilation();
...
////////////////////////////////////////////////////////////////////////////////
void J9::Z::PrivateLinkage::buildVirtualDispatch(TR::Node *callNode, TR::RegisterDependencyConditions *dependencies,
    TR::Register *vftReg, uint32_t sizeOfArguments)
{
    ...
    if ((methodSymbol && !methodSymbol->isComputed())
        && (comp()->getPersistentInfo()->isRuntimeInstrumentationEnabled()) && (comp()->getOption(TR_EnableRIEMIT))) {
    ...
    if (methodSymbol && methodSymbol->isComputed()) {
        switch (methodSymbol->getMandatoryRecognizedMethod()) {
            ...
            case TR::com_ibm_jit_JITHelpers_dispatchVirtual: {
                char *j2iSignature = fej9->getJ2IThunkSignatureForDispatchVirtual(
                    methodSymbol->getMethod()->signatureChars(), methodSymbol->getMethod()->signatureLength(), comp());
                ...
                if (!virtualThunk) {
                    virtualThunk = fej9->setJ2IThunk(j2iSignature, signatureLen,
                        TR::S390J9CallSnippet::generateVIThunk(
                            fej9->getEquivalentVirtualCallNodeForDispatchVirtual(callNode, comp()), sizeOfArguments,
                            cg()),
                        comp()); // TODO:JSR292: Is this the right sizeOfArguments?
                }
            } break;
            default:
                if (fej9->needsInvokeExactJ2IThunk(callNode, comp())) {
                    TR_MHJ2IThunk *thunk = TR::S390J9CallSnippet::generateInvokeExactJ2IThunk(callNode, sizeOfArguments,
                        methodSymbol->getMethod()->signatureChars(), cg());
        ...
        if (!virtualThunk)
            virtualThunk = fej9->setJ2IThunk(methodSymbol->getMethod(),
                TR::S390J9CallSnippet::generateVIThunk(callNode, sizeOfArguments, cg()), comp());
    }
    ...
    if (methodSymbol->isVirtual()) {
        ...
        TR::RegisterDependencyConditions *postDeps
            = new (trHeapMemory()) TR::RegisterDependencyConditions(postDepsTemp, 0, 4, cg());
        ...
        if (methodSymRef->isUnresolved() || comp()->compileRelocatableCode()) {
            ...
            unresolvedSnippet = new (trHeapMemory())
                TR::S390VirtualUnresolvedSnippet(cg(), callNode, snippetLabel, sizeOfArguments, virtualThunk);
            ...
            if ((comp()->performVirtualGuardNOPing() && comp()->isVirtualGuardNOPingRequired())) {
                ...
                if (resolvedMethod && !resolvedMethod->isInterpreted()
                    ...
                    if (!resolvedMethod->virtualMethodIsOverridden() && !resolvedMethod->isAbstract()) {
                        ...
                        if (thisClass && TR::Compiler->cls.isAbstractClass(comp(), thisClass)
                            ...
                            TR_ResolvedMethod *method = chTable->findSingleAbstractImplementer(thisClass,
                                methodSymRef->getOffset(), methodSymRef->getOwningMethod(comp()), comp());
                            if (method
                                && (comp()->isRecursiveMethodTarget(method) || !method->isInterpreted()
                            ...
                            && !chTable->isOverriddenInThisHierarchy(resolvedMethod, refinedThisClass,
                            ...
                            if (resolvedMethod->virtualMethodIsOverridden()) {
                                ...
                                if (calleeMethod
                                    && (comp()->isRecursiveMethodTarget(calleeMethod) || !calleeMethod->isInterpreted()
                                        || calleeMethod->isJITInternalNative())) {
                                    performGuardedDevirtualization = true;
                                    resolvedMethod = calleeMethod;
                                    virtualGuard = TR_VirtualGuard::createGuardedDevirtualizationGuard(
                                        TR_HierarchyGuard, comp(), callNode);
                    ...
                    if (performGuardedDevirtualization && virtualGuard) {
                        ...
                        generateVirtualGuardNOPInstruction(cg(), callNode, virtualGuard->addNOPSite(), NULL,
                            virtualLabel);
                        if (comp()->getOption(TR_EnableHCR)) {
                            if (cg()->supportsMergingGuards()) {
                                virtualGuard->setMergedWithHCRGuard();
                            } else {
                                TR_VirtualGuard *HCRGuard = TR_VirtualGuard::createGuardedDevirtualizationGuard(
                                    TR_HCRGuard, comp(), callNode);
                                generateVirtualGuardNOPInstruction(cg(), callNode, HCRGuard->addNOPSite(), NULL,
                                    virtualLabel);
                            }
                        }
                    }
            ...
            if (!performGuardedDevirtualization && !comp()->getOption(TR_DisableInterpreterProfiling)
                ...
                if (!comp()->compileRelocatableCode())
                    valueInfo = static_cast<TR_AddressInfo *>(
                        TR_ValueProfileInfoManager::getProfiledValueInfo(callNode, comp(), AddressInfo));
                ...
                if (topValue) {
                    if (valueInfo->getTopProbability() < MIN_PROFILED_CALL_FREQUENCY
                        ...
                        TR_OpaqueClassBlock *callSiteMethodClass = methodSymRef->getSymbol()
                                                                       ->getResolvedMethodSymbol()
                                                                       ->getResolvedMethod()
                                                                       ->classOfMethod();
                        if (!cg()->isProfiledClassAndCallSiteCompatible((TR_OpaqueClassBlock *)topValue,
                                callSiteMethodClass)) {
                            topValue = 0;
                        }
                    }
                ...
                if (topValue) {
                    ...
                    if (profiledVirtualMethod && !profiledVirtualMethod->isInterpreted()) {
                        logprintf(trace, log, "Profiled method {%s}\n",
                            fej9->sampleSignature(
                                (TR_OpaqueMethodBlock *)(profiledVirtualMethod->getPersistentIdentifier()), 0, 0,
                                comp()->trMemory()));
            ...
            if (performGuardedDevirtualization || useProfiledValues) {
                ...
                if (useProfiledValues || resolvedMethod != resolvedSymbol->getResolvedMethod()) {
                    realMethodSymRef
                        = comp()->getSymRefTab()->findOrCreateMethodSymbol(methodSymRef->getOwningMethodIndex(), -1,
                            (useProfiledValues) ? profiledMethod : resolvedMethod, TR::MethodSymbol::Virtual);
                }

                if (useProfiledValues) {
                    genLoadProfiledClassAddressConstant(cg(), callNode, profiledClass, RegZero, NULL, dependencies,
                        NULL);
                    generateS390CompareAndBranchInstruction(cg(), TR::InstOpCode::getCmpLogicalRegOpCode(), callNode,
                        vftReg, RegZero, TR::InstOpCode::COND_BNE, virtualLabel);
                }

                buildDirectCall(callNode, realMethodSymRef, dependencies, sizeOfArguments);

                if (!virtualLabel)
                    generateS390BranchInstruction(cg(), TR::InstOpCode::BRC, TR::InstOpCode::COND_BRC, callNode,
                        doneVirtualLabel);
            }
        }
        ...
        if (virtualLabel) {
            ...
            outlinedSlowPath
                = new (cg()->trHeapMemory()) TR_S390OutOfLineCodeSection(vcallLabel, doneVirtualLabel, cg());
        ...
        // the VM to find VFT table entries can't handle negative displacements. For unresolved/AOT targets we must
        // assume the worse case (offset can't fit in 16bits). VFT offset 0 means unresolved/AOT, otherwise offset is
        // negative. Some special cases have positive offsets i.e. java/lang/Object.newInstancePrototype()
        ...
        } else // if unresolved || offset can't fit in 16bits
        {
            // If unresolved/AOT, this instruction will be patched by _virtualUnresolvedHelper() with the correct offset
            cursor
                = generateRILInstruction(cg(), TR::InstOpCode::LGFI, callNode, RegZero, static_cast<int32_t>(offset));
        }
        ...
        if (outlinedSlowPath) {
            TR::Instruction *temp = generateS390BranchInstruction(cg(), TR::InstOpCode::BRC, TR::InstOpCode::COND_BRC,
                callNode, doneVirtualLabel);
            if (debugObj) {
        ...
        TR::RegisterPair *classMethodEPPairRegister = NULL;
        ...
        if (valueProfileInfo)
            info = static_cast<TR_AddressInfo *>(
                valueProfileInfo->getValueInfo(callNode->getByteCodeInfo(), comp(), AddressInfo));
        ...
        if (!isAOT && callIsSafe && isAddressInfo
            ...
            profiledClassesList = new (trHeapMemory())
                TR::list<TR_OpaqueClassBlock *>(getTypedAllocator<TR_OpaqueClassBlock *>(comp()->allocator()));
            for (profiledInfo = valuesIt.getFirst(); numStaticPICs < maxStaticPICs && profiledInfo != NULL;
                 profiledInfo = valuesIt.getNext()) {
                ...
                TR::SymbolReference *methodSymRef = callNode->getSymbolReference();
                TR_ResolvedMethod *profiledMethod = methodSymRef->getOwningMethod(comp())->getResolvedInterfaceMethod(
                    comp(), (TR_OpaqueClassBlock *)clazz, methodSymRef->getCPIndex());
        ...
        if (numInterfaceCallCacheSlots == 0) {
            ...
            TR::RegisterDependencyConditions *postDeps
                = new (trHeapMemory()) TR::RegisterDependencyConditions(dependencies, 0, 2, cg());
            ...
            cursor = generateS390LabelInstruction(cg(), TR::InstOpCode::label, callNode, paramSetupDummyLabel, preDeps,
                cursor);

            cursor = generateLastITableAndITableInstructions(cg(), callNode, vftReg, regEP, vTableIndexRegister,
                methodRegister, postDeps, cursor);

            cursor = new (trHeapMemory())
                TR::S390RILInstruction(TR::InstOpCode::LARL, callNode, regEP, ifcSnippet, cursor, cg());
            ...
            TR::RegisterDependencyConditions *postDeps
                = new (trHeapMemory()) TR::RegisterDependencyConditions(dependencies, 0, 5, cg());
            postDeps->setAddCursorForPre(0); // Ignore all pre-deps that were copied.
            postDeps->setNumPreConditions(0, trMemory()); // Ignore all pre-deps that were copied.

            // Check the thisChild to see if anyone uses this object after the call (if not, we won't add it to post
            // Deps)
            if (callNode->getChild(callNode->getFirstArgumentIndex())->getReferenceCount() > 0)
                postDeps->addPostCondition(regThis, TR::RealRegister::AssignAny);
            ...
            if (comp()->getOption(TR_enableInterfaceCallCachingSingleDynamicSlot)) {
                cursor = new (trHeapMemory()) TR::S390RILInstruction(TR::InstOpCode::LARL, callNode, snippetReg,
                    ifcSnippet->getDataConstantSnippet(), cg());
                ...
                cursor = new (trHeapMemory())
                    TR::S390RILInstruction(TR::InstOpCode::LARL, callNode, regRA, returnLocationLabel, cursor, cg());

                if (comp()->target().is64Bit())
                    cursor = generateRXInstruction(cg(), TR::InstOpCode::LPQ, callNode, classMethodEPPairRegister,
                        generateS390MemoryReference(snippetReg,
                            ifcSnippet->getDataConstantSnippet()->getSingleDynamicSlotOffset(), cg()),
                        cursor);
                else
                ...
                cursor = generateS390LabelInstruction(cg(), TR::InstOpCode::label, callNode, paramSetupDummyLabel,
                    preDeps, cursor);
                ...
                cursor = generateLastITableAndITableInstructions(cg(), callNode, vftReg, snippetReg,
                    vTableIndexRegister, methodRegister, postDeps, cursor);
                ...
                useCLFIandBRCL = false
                    && (comp()->target().is64Bit() && // Support for 64-bit
                ...
                cursor
                    = generateRILInstruction(cg(), TR::InstOpCode::LARL, callNode, regRA, returnLocationLabel, cursor);
                ...
                if (useCLFIandBRCL) {
                    ...
                    // First CLFI/BRCL
                    cursor = generateRILInstruction(cg(), TR::InstOpCode::CLFI, callNode, vftReg, 0x0,
                        cursor); // compare against 0

                    ifcSnippet->getDataConstantSnippet()->setFirstCLFI(cursor);

                    // BRCL
                    cursor = generateRILInstruction(cg(), TR::InstOpCode::BRCL, callNode, static_cast<uint32_t>(0x0),
                        reinterpret_cast<void *>(0x0), cursor);

                    for (int i = 1; i < numInterfaceCallCacheSlots; i++) {
                        // We will generate CLFI / BRCL sequence to dispatch to target branches.
                        cursor = generateRILInstruction(cg(), TR::InstOpCode::CLFI, callNode, vftReg, 0x0,
                            cursor); // compare against 0

                        // BRCL
                        cursor = generateRILInstruction(cg(), TR::InstOpCode::BRCL, callNode,
                            static_cast<uint32_t>(0x0), reinterpret_cast<void *>(0x0), cursor);
                    }
                } else {
                    int32_t slotOffset = ifcSnippet->getDataConstantSnippet()->getFirstSlotOffset();
                    for (int i = 0; i < numInterfaceCallCacheSlots; i++) {
                        ...
                        cursor = generateRXInstruction(cg(), TR::InstOpCode::getLoadOpCode(), callNode, methodRegister,
                            generateS390MemoryReference(snippetReg,
                                slotOffset + TR::Compiler->om.sizeofReferenceAddress(), cg()),
                            cursor);

                        cursor
                            = generateS390RegInstruction(cg(), TR::InstOpCode::BCR, callNode, methodRegister, cursor);
                        ((TR::S390RegInstruction *)cursor)->setBranchCondition(TR::InstOpCode::COND_BER);
                ...
                cursor = new (trHeapMemory())
                    TR::S390RILInstruction(TR::InstOpCode::LARL, callNode, snippetReg, ifcSnippet, cursor, cg());
        ...
        if (targetAddress->getRegisterPair())
            targetAddress = targetAddress->getRegisterPair()
                                ->getLowOrder(); // on 31-bit, the top half doesn't matter, so discard it
        TR::Register *regRA = dependencies->searchPostConditionRegister(getReturnAddressRegister());
...
TR::Instruction *J9::Z::PrivateLinkage::buildDirectCall(TR::Node *callNode, TR::SymbolReference *callSymRef,
    TR::RegisterDependencyConditions *dependencies, int32_t argSize)
{
    ...
    if (isJitDispatchJ9Method) {
        ...
        // TR::RegisterDependencyConditions *preDeps = new (trHeapMemory()) TR::RegisterDependencyConditions(
        //     dependencies->getPreConditions(), NULL, dependencies->getAddCursorForPre(), 0, cg());
        TR::RegisterDependencyConditions *postDeps = dependencies;
        TR::RegisterDependencyConditions *postDepsTemp = new (trHeapMemory()) TR::RegisterDependencyConditions(NULL,
            dependencies->getPostConditions(), 0, dependencies->getAddCursorForPost(), cg());

        if (feGetEnv("onlyJ9MethodPre") != NULL) {
            postDeps = new (trHeapMemory()) TR::RegisterDependencyConditions(postDepsTemp, 1, 0, cg());
            postDeps->addPreCondition(j9MethodReg, getJ9MethodArgumentRegister());
        } else if (feGetEnv("onlyArgsPre") != NULL) {
            postDepsTemp = new (trHeapMemory()) TR::RegisterDependencyConditions(dependencies->getPreConditions(),
            dependencies->getPostConditions(), dependencies->getAddCursorForPre(), dependencies->getAddCursorForPost(), cg());

            postDeps = new (trHeapMemory()) TR::RegisterDependencyConditions(postDepsTemp, 0, 1, cg());
            postDeps->addPostCondition(j9MethodReg, getJ9MethodArgumentRegister());
        } else if (feGetEnv("noPre") != NULL) {
            postDeps = new (trHeapMemory()) TR::RegisterDependencyConditions(postDepsTemp, 0, 1, cg());
            postDeps->addPostCondition(j9MethodReg, getJ9MethodArgumentRegister());
        }
    ...
    if (!callSymRef->isUnresolved() && !callSymbol->isInterpreted()
        && ((comp()->compileRelocatableCode() && callSymbol->isHelper()) || !comp()->compileRelocatableCode())) {
...
void J9::Z::PrivateLinkage::callPreJNICallOffloadCheck(TR::Node *callNode)
{
    ...
    TR::SymbolReference *offloadOffSymRef
        = codeGen->symRefTab()->findOrCreateRuntimeHelper(TR_S390jitPreJNICallOffloadCheck);

    TR::Instruction *gcPoint = generateS390BranchInstruction(codeGen, TR::InstOpCode::BRC, TR::InstOpCode::COND_BRC,
        callNode, offloadOffSnippetLabel);
    gcPoint->setNeedsGCMap(0);

    codeGen->addSnippet(new (trHeapMemory()) TR::S390HelperCallSnippet(codeGen, callNode, offloadOffSnippetLabel,
        offloadOffSymRef, offloadOffRestartLabel));
    generateS390LabelInstruction(codeGen, TR::InstOpCode::label, callNode, offloadOffRestartLabel);
}

void J9::Z::PrivateLinkage::callPostJNICallOffloadCheck(TR::Node *callNode)
{
    TR::CodeGenerator *codeGen = cg();
    ...
    TR::Instruction *gcPoint = generateS390BranchInstruction(codeGen, TR::InstOpCode::BRC, TR::InstOpCode::COND_BRC,
        callNode, offloadOnSnippetLabel);
    gcPoint->setNeedsGCMap(0);
    TR::SymbolReference *offloadOnSymRef
        = codeGen->symRefTab()->findOrCreateRuntimeHelper(TR_S390jitPostJNICallOffloadCheck);
    codeGen->addSnippet(new (trHeapMemory()) TR::S390HelperCallSnippet(codeGen, callNode, offloadOnSnippetLabel,
        offloadOnSymRef, offloadOnRestartLabel));
...
void J9::Z::PrivateLinkage::collapseJNIReferenceFrame(TR::Node *callNode,
    TR::RealRegister *javaStackPointerRealRegister, TR::Register *javaLitPoolVirtualRegister, TR::Register *tempReg)
{
    ...
    generateRXInstruction(codeGen, TR::InstOpCode::getAndOpCode(), callNode, tempReg,
        new (trHeapMemory()) TR::MemoryReference(javaStackPointerRealRegister,
            (int32_t)fej9->constJNICallOutFrameFlagsOffset(), codeGen));
    TR::Instruction *gcPoint = generateS390BranchInstruction(codeGen, TR::InstOpCode::BRC, TR::InstOpCode::COND_BNE,
        callNode, refPoolSnippetLabel);
    gcPoint->setNeedsGCMap(0);

    TR::SymbolReference *collapseSymRef
        = cg()->symRefTab()->findOrCreateRuntimeHelper(TR_S390collapseJNIReferenceFrame);
    codeGen->addSnippet(new (trHeapMemory())
            TR::S390HelperCallSnippet(codeGen, callNode, refPoolSnippetLabel, collapseSymRef, refPoolRestartLabel));
...
void J9::Z::PrivateLinkage::setupJNICallOutFrame(TR::Node *callNode, TR::RealRegister *javaStackPointerRealRegister,
    ...
    TR::S390JNICallDataSnippet *jniCallDataSnippet)
{
    ...
    cursor = generateRXInstruction(codeGen, TR::InstOpCode::LAY, callNode, javaStackPointerRealRegister,
        generateS390MemoryReference(javaStackPointerRealRegister, stackAdjust, codeGen), cursor);
    ...
    TR_ASSERT(constJNICallOutFrameType < MAX_IMMEDIATE_VAL,
        "OMR::Z::Linkage::setupJNICallOutFrame constJNICallOutFrameType is too big for MVHI");

    TR_ASSERT((fej9->thisThreadGetJavaFrameFlagsOffset()
                  == fej9->thisThreadGetJavaLiteralsOffset() + TR::Compiler->om.sizeofReferenceAddress())
            && fej9->thisThreadGetJavaLiteralsOffset()
                == fej9->thisThreadGetJavaPCOffset() + TR::Compiler->om.sizeofReferenceAddress(),
    ...
    generateSS1Instruction(cg(), TR::InstOpCode::MVC, callNode, 3 * (TR::Compiler->om.sizeofReferenceAddress()) - 1,
        new (trHeapMemory())
            TR::MemoryReference(methodMetaDataVirtualRegister, fej9->thisThreadGetJavaPCOffset(), codeGen),
        new (trHeapMemory())
            TR::MemoryReference(jniCallDataSnippet->getBaseRegister(), jniCallDataSnippet->getPCOffset(), codeGen));

    // store out jsp
    generateRXInstruction(codeGen, TR::InstOpCode::getStoreOpCode(), callNode, javaStackPointerRealRegister,
        new (trHeapMemory())
            TR::MemoryReference(methodMetaDataVirtualRegister, fej9->thisThreadGetJavaSPOffset(), codeGen));
    ...
    generateSS1Instruction(cg(), TR::InstOpCode::MVC, callNode, -stackAdjust - 1,
        ...
        new (trHeapMemory()) TR::MemoryReference(jniCallDataSnippet->getBaseRegister(),
            jniCallDataSnippet->getJNICallOutFrameDataOffset(), codeGen));
}
 ...
 *                but the helper call saves and restores all regs
 */
void J9::Z::JNILinkage::releaseVMAccessMask(TR::Node *callNode, TR::Register *methodMetaDataVirtualRegister,
    TR::Register *methodAddressReg, TR::Register *javaLitOffsetReg, TR::S390JNICallDataSnippet *jniCallDataSnippet,
    TR::RegisterDependencyConditions *deps)
{
    ...
    generateRXInstruction(self()->cg(), TR::InstOpCode::getLoadOpCode(), callNode, methodAddressReg,
        generateS390MemoryReference(methodMetaDataVirtualRegister, fej9->thisThreadGetPublicFlagsOffset(),
            self()->cg()));
    ...
    generateRRInstruction(self()->cg(), TR::InstOpCode::getLoadRegOpCode(), callNode, javaLitOffsetReg,
        methodAddressReg);
    generateRXInstruction(self()->cg(), TR::InstOpCode::getAndOpCode(), callNode, javaLitOffsetReg,
        generateS390MemoryReference(jniCallDataSnippet->getBaseRegister(),
            jniCallDataSnippet->getConstReleaseVMAccessOutOfLineMaskOffset(), self()->cg()));

    TR::Instruction *gcPoint = (TR::Instruction *)generateS390BranchInstruction(self()->cg(), TR::InstOpCode::BRC,
        TR::InstOpCode::COND_BNE, callNode, longReleaseSnippetLabel);
    gcPoint->setNeedsGCMap(0);
    ...
    generateRXInstruction(self()->cg(), TR::InstOpCode::getAndOpCode(), callNode, javaLitOffsetReg,
        generateS390MemoryReference(jniCallDataSnippet->getBaseRegister(),
            jniCallDataSnippet->getConstReleaseVMAccessMaskOffset(), self()->cg()));
    generateRSInstruction(self()->cg(), TR::InstOpCode::getCmpAndSwapOpCode(), callNode, methodAddressReg,
        javaLitOffsetReg,
        generateS390MemoryReference(methodMetaDataVirtualRegister, fej9->thisThreadGetPublicFlagsOffset(),
            self()->cg()));
    ...
    TR::RegisterDependencyConditions *postDeps
        = new (self()->trHeapMemory()) TR::RegisterDependencyConditions(0, 3, self()->cg());
    ...
    self()->cg()->addSnippet(new (self()->trHeapMemory()) TR::S390HelperCallSnippet(self()->cg(), callNode,
    ...
    TR::Register *methodMetaDataVirtualRegister, TR::Register *methodAddressReg, TR::Register *javaLitOffsetReg)
{
    ...
    // should use stack.
    // If instruction uses literal pool, it must only be to do load, and such instruction's memory reference should be
    // marked MemRefMustNotSpill so that in case of long disp, we will reuse the target reg as a scratch reg

    TR_J9VMBase *fej9 = (TR_J9VMBase *)(self()->fe());
    intptr_t aValue = fej9->constAcquireVMAccessOutOfLineMask();

    TR::Instruction *loadInstr = (TR::Instruction *)genLoadAddressConstant(self()->cg(), callNode, aValue,
        methodAddressReg, NULL, NULL, javaLitPoolVirtualRegister);
    ...
    generateRRInstruction(self()->cg(), TR::InstOpCode::getXORRegOpCode(), callNode, javaLitOffsetReg,
        javaLitOffsetReg);
    ...
    generateRSInstruction(cg(), TR::InstOpCode::getCmpAndSwapOpCode(), callNode, javaLitOffsetReg, methodAddressReg,
        generateS390MemoryReference(methodMetaDataVirtualRegister, (int32_t)fej9->thisThreadGetPublicFlagsOffset(),
            self()->cg()));
    TR::Instruction *gcPoint = (TR::Instruction *)generateS390BranchInstruction(self()->cg(), TR::InstOpCode::BRC,
        TR::InstOpCode::COND_BNE, callNode, longAcquireSnippetLabel);
    gcPoint->setNeedsGCMap(0);

    self()->cg()->addSnippet(
        new (self()->trHeapMemory()) TR::S390HelperCallSnippet(self()->cg(), callNode, longAcquireSnippetLabel,
 ...
 *
 * In the JNI dispatch sequence, a release-vm-access action is performed before the branch to native code; and an
 * acquire-vm-access is done after the thread execution returns from the native call. Both of the actions require
 * synchronization between the application thread and the GC thread. This was previously implemented with the atomic
 * compare-and-swap (CS) instruction, which is slow in nature.
 *
 * To speed up the JNI acquire and release access actions (the fast path), a store-load sequence is generated by this
 * evaluator to replace the CS instruction. Normally, the fast path ST-LD are not serialized and can be done
 * out-of-order for higher performance. Synchronization burden is offloaded to the slow path.
 *
 * The slow path is where a thread tries to acquire exclusive vm access. The slow path should be taken proportionally
 * less often than the fast path. Should the slow path be taken, that thread will be penalized by calling a slow
 * flushProcessWriteBuffer() routine so that all threads can momentarily synchronize memory writes. Having fast and slow
 * paths makes the atomic-free JNI design asymmetric.
 *
 * Note that the z/OS currently does not support the asymmetric algorithm. Hence, a serialization instruction is
 * required between the store and the load.
 *
 */
void J9::Z::JNILinkage::releaseVMAccessMaskAtomicFree(TR::Node *callNode, TR::Register *methodMetaDataVirtualRegister,
    TR::Register *tempReg1)
{
    ...
    generateSILInstruction(cg, TR::InstOpCode::getMoveHalfWordImmOpCode(), callNode,
        generateS390MemoryReference(methodMetaDataVirtualRegister, offsetof(J9VMThread, inNative), cg), 1);
    ...
    TR_ASSERT_FATAL(J9_PUBLIC_FLAGS_VM_ACCESS >= MIN_IMMEDIATE_BYTE_VAL
            && J9_PUBLIC_FLAGS_VM_ACCESS <= MAX_IMMEDIATE_BYTE_VAL,
        "VM access bit must be immediate");

    generateS390CompareAndBranchInstruction(cg, TR::InstOpCode::getCmpOpCode(), callNode, tempReg1,
        J9_PUBLIC_FLAGS_VM_ACCESS, TR::InstOpCode::COND_BNE, longReleaseSnippetLabel, false);

    cg->addSnippet(new (self()->trHeapMemory()) TR::S390HelperCallSnippet(cg, callNode, longReleaseSnippetLabel,
 ...
 *
 * */
void J9::Z::JNILinkage::acquireVMAccessMaskAtomicFree(TR::Node *callNode, TR::Register *methodMetaDataVirtualRegister,
    TR::Register *tempReg1)
{
    TR_J9VMBase *fej9 = (TR_J9VMBase *)fe();
    TR::CodeGenerator *cg = self()->cg();
    ...
    generateS390CompareAndBranchInstruction(cg, TR::InstOpCode::getCmpOpCode(), callNode, tempReg1,
        J9_PUBLIC_FLAGS_VM_ACCESS, TR::InstOpCode::COND_BNE, longAcquireSnippetLabel, false);

    cg->addSnippet(new (self()->trHeapMemory()) TR::S390HelperCallSnippet(cg, callNode, longAcquireSnippetLabel,
        comp()->getSymRefTab()->findOrCreateAcquireVMAccessSymbolRef(comp()->getJittedMethodSymbol()),
...
void J9::Z::JNILinkage::checkException(TR::Node *callNode, TR::Register *methodMetaDataVirtualRegister,
    TR::Register *tempReg)
{
    TR_J9VMBase *fej9 = (TR_J9VMBase *)(fe());
    // check exception
    ...
    generateRXInstruction(self()->cg(), TR::InstOpCode::getLoadOpCode(), callNode, tempReg,
        new (self()->trHeapMemory()) TR::MemoryReference(methodMetaDataVirtualRegister,
            fej9->thisThreadGetCurrentExceptionOffset(), self()->cg()));

    TR::Instruction *gcPoint = generateS390CompareAndBranchInstruction(self()->cg(), TR::InstOpCode::getCmpOpCode(),
        callNode, tempReg, 0, TR::InstOpCode::COND_BNE, exceptionSnippetLabel, false, true);
    gcPoint->setNeedsGCMap(0);

    self()->cg()->addSnippet(
        new (self()->trHeapMemory()) TR::S390HelperCallSnippet(self()->cg(), callNode, exceptionSnippetLabel,
            comp()->getSymRefTab()->findOrCreateThrowCurrentExceptionSymbolRef(comp()->getJittedMethodSymbol()),
            exceptionRestartLabel));
    generateS390LabelInstruction(self()->cg(), TR::InstOpCode::label, callNode, exceptionRestartLabel);
}

void J9::Z::JNILinkage::processJNIReturnValue(TR::Node *callNode, TR::CodeGenerator *cg,
    TR::Register *javaReturnRegister)
{
    ...
    const bool isUnwrapAddressReturnValue
        = !((TR_J9VMBase *)fe())->jniDoNotWrapObjects(resolvedMethod) && (returnType == TR::Address);
    ...
    if (isUnwrapAddressReturnValue) {
        ...
        generateS390CompareAndBranchInstruction(cg, TR::InstOpCode::getCmpOpCode(), callNode, javaReturnRegister, 0,
...
TR::Register *J9::Z::JNILinkage::buildDirectDispatch(TR::Node *callNode)
{
    ...
    if ((fej9->isJavaOffloadEnabled() && static_cast<TR_ResolvedJ9Method *>(resolvedMethod)->methodIsNotzAAPEligible())
        || (fej9->CEEHDLREnabled() && isJNICallOutFrame))
        isJavaOffLoadCheck = true;

    logprintf(trace, log,
        "isPassReceiver: %d, isPassJNIThread: %d, isJNIGCPoint: %d, isJNICallOutFrame:%d, isReleaseVMAccess: %d, "
        "isCollapseJNIReferenceFrame: %d, isJNIGCPoint: %d\n",
    ...
    if (isPassJNIThread) {
        ...
        generateRRInstruction(codeGen, TR::InstOpCode::getLoadRegOpCode(), callNode, jniEnvRegister,
    ...
    setupRegisterDepForLinkage(callNode, TR_JNIDispatch, deps, killMask, systemLinkage, GlobalRegDeps, hasGlRegDeps,
        &methodAddressReg, javaLitOffsetReg);

    setupBuildArgForLinkage(callNode, TR_JNIDispatch, deps, isFastJNI, isPassReceiver, killMask, GlobalRegDeps,
        hasGlRegDeps, systemLinkage);
    ...
    if (isJNICallOutFrame) {
         ...
         */
        generateSIInstruction(cg(), TR::InstOpCode::AGSI, callNode,
            generateS390MemoryReference(methodMetaDataVirtualRegister, fej9->thisThreadGetCallOutCountOffset(), cg()),
            1);
        ...
        generateRXInstruction(codeGen, TR::InstOpCode::getStoreOpCode(), callNode, javaStackPointerRealRegister,
            new (trHeapMemory()) TR::MemoryReference(methodMetaDataVirtualRegister,
                (int32_t)fej9->thisThreadGetJavaSPOffset(), codeGen));

        auto *literalOffsetMemoryReference = new (trHeapMemory()) TR::MemoryReference(methodMetaDataVirtualRegister,
            (int32_t)fej9->thisThreadGetJavaLiteralsOffset(), codeGen);

        // Set up literal offset slot to zero
        generateSILInstruction(codeGen, TR::InstOpCode::getMoveHalfWordImmOpCode(), callNode,
            literalOffsetMemoryReference, 0);
    }
...
#else
        releaseVMAccessMask(callNode, methodMetaDataVirtualRegister, methodAddressReg, javaLitOffsetReg,
            jniCallDataSnippet, deps);
#endif
    }
    ...
    TR::Register *javaReturnRegister = systemLinkage->callNativeFunction(callNode, deps, targetAddress,
        methodAddressReg, javaLitOffsetReg, returnFromJNICallLabel, jniCallDataSnippet, isJNIGCPoint);
    ...
    generateRXInstruction(codeGen, TR::InstOpCode::getLoadOpCode(), callNode, javaStackPointerRealRegister,
        new (trHeapMemory())
            TR::MemoryReference(methodMetaDataVirtualRegister, (int32_t)fej9->thisThreadGetJavaSPOffset(), codeGen));
...
#else
        acquireVMAccessMask(callNode, javaLitPoolVirtualRegister, methodMetaDataVirtualRegister, methodAddressReg,
            javaLitOffsetReg);
#endif
    }
    ...
    OMR::Z::Linkage::generateDispatchReturnLable(callNode, codeGen, deps, javaReturnRegister, hasGlRegDeps,
        GlobalRegDeps);
...
////////////////////////////////////////////////////////////////////////////////
void J9::Z::PrivateLinkage::doNotKillSpecialRegsForBuildArgs(TR::Linkage *linkage, bool isFastJNI, int64_t &killMask)
{
    TR::SystemLinkage *systemLinkage = (TR::SystemLinkage *)cg()->getLinkage(TR_System);
...
////////////////////////////////////////////////////////////////////////////////
void J9::Z::PrivateLinkage::addSpecialRegDepsForBuildArgs(TR::Node *callNode,
    TR::RegisterDependencyConditions *dependencies, int32_t &from, int32_t step)
{
    TR::Node *child;
    ...
    if (specialArgReg != TR::RealRegister::NoReg) {
        ...
        logprintf(comp()->getOption(TR_TraceCG), comp()->log(), "Special arg %s %s reg %s in %s\n",
            callNode->getOpCode().getName(), comp()->getDebug()->getName(callNode->getChild(from)),
...
////////////////////////////////////////////////////////////////////////////////
int32_t J9::Z::PrivateLinkage::storeExtraEnvRegForBuildArgs(TR::Node *callNode, TR::Linkage *linkage,
    ...
    uint32_t &numIntegerArgs)
{
    ...
    if (isFastJNI) // Account for extra parameter env
    {
        ...
        if (linkage->isAllParmsOnStack()) {
            TR::Register *stackRegister = linkage->getStackRegisterForOutgoingArguments(callNode,
                dependencies); // delay (possibly) creating this till needed
            storeArgumentOnStack(callNode, TR::InstOpCode::getStoreOpCode(), jniEnvRegister, &stackOffset,
                stackRegister);
        }
...
////////////////////////////////////////////////////////////////////////////////
int64_t J9::Z::PrivateLinkage::addFECustomizedReturnRegDependency(int64_t killMask, TR::Linkage *linkage,
    TR::DataType resType, TR::RegisterDependencyConditions *dependencies)
{
...
////////////////////////////////////////////////////////////////////////////////
TR::Register *J9::Z::PrivateLinkage::buildDirectDispatch(TR::Node *callNode)
{
    ...
    switch (callNode->getOpCodeValue()) {
        ...
        case TR::lcall: {
            if (comp()->target().is64Bit()) {
                ...
                cursor = generateRRInstruction(cg(), TR::InstOpCode::LR, callNode, highReg, lowReg);
...
////////////////////////////////////////////////////////////////////////////////
TR::Register *J9::Z::PrivateLinkage::buildIndirectDispatch(TR::Node *callNode)
{
...
void J9::Z::PrivateLinkage::setupBuildArgForLinkage(TR::Node *callNode, TR_DispatchType dispatchType,
    TR::RegisterDependencyConditions *deps, bool isFastJNI, bool isPassReceiver, int64_t &killMask,
    TR::Node *GlobalRegDeps, bool hasGlRegDeps, TR::SystemLinkage *systemLinkage)
{
    TR::CodeGenerator *codeGen = cg();
    TR_J9VMBase *fej9 = (TR_J9VMBase *)(fe());
    // call base class
    OMR::Z::Linkage::setupBuildArgForLinkage(callNode, dispatchType, deps, isFastJNI, isPassReceiver, killMask,
        GlobalRegDeps, hasGlRegDeps, systemLinkage);
    ...
    if (dispatchType == TR_JNIDispatch)
        return;
...
void J9::Z::PrivateLinkage::setupRegisterDepForLinkage(TR::Node *callNode, TR_DispatchType dispatchType,
...
{
    // call base class
    OMR::Z::Linkage::setupRegisterDepForLinkage(callNode, dispatchType, deps, killMask, systemLinkage, GlobalRegDeps,
        hasGlRegDeps, methodAddressReg, javaLitOffsetReg);

    TR::CodeGenerator *codeGen = cg();

    if (dispatchType == TR_SystemDispatch) {
        killMask = killAndAssignRegister(killMask, deps, methodAddressReg,
            (comp()->target().isLinux()) ? TR::RealRegister::GPR14 : TR::RealRegister::GPR8, codeGen, true);
        killMask = killAndAssignRegister(killMask, deps, &javaLitOffsetReg,
            (comp()->target().isLinux()) ? TR::RealRegister::GPR8 : TR::RealRegister::GPR14, codeGen, true);
    }
    ...
    if (comp()->target().isZOS()) {
        ...
        TR::RealRegister::RegNum systemCAAPointerRegister
            = ((TR::S390zOSSystemLinkage *)systemLinkage)->getCAAPointerRegister();
        TR::Register *systemCAAVirtualRegister = NULL;

        killMask
            = killAndAssignRegister(killMask, deps, &systemCAAVirtualRegister, systemCAAPointerRegister, codeGen, true);
        ...
        if (systemStackRealRegister->getState() != TR::RealRegister::Locked) {
            ...
            killMask = killAndAssignRegister(killMask, deps, &systemStackVirtualRegister, systemStackPointerRegister,
                codeGen, true);
    ...
    if (codeGen->isLiteralPoolOnDemandOn()) {
        ...
        killMask = killAndAssignRegister(killMask, deps, &javaLitPoolVirtualRegister, javaLitPoolRealRegister, codeGen,
            true);
    ...
    // This logic was originally in OMR::Z::Linkage::buildNativeDispatch and the condition is
    // cg()->supportsJITFreeSystemStackPointer(). The original condition is only true for J9 and only on zos, so
    // replacing it with comp()->target().isZOS().
    if (comp()->target().isZOS()) {
        TR_J9VMBase *fej9 = (TR_J9VMBase *)(fe());
        generateRXInstruction(codeGen, TR::InstOpCode::getLoadOpCode(), callNode, systemStackVirtualRegister,
            new (trHeapMemory()) TR::MemoryReference(methodMetaDataVirtualRegister,
                (int32_t)fej9->thisThreadGetSystemSPOffset(), codeGen));
    }
}

TR::RealRegister::RegNum J9::Z::PrivateLinkage::getSystemStackPointerRegister()
{
