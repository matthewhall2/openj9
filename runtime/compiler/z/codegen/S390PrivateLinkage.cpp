   ...
   //   interpreterdDeps->addPostConditionIfNotAlreadyInserted(j9MethodReg, TR::RealRegister::AssignAny);
     ...
     //dependencies->addPostConditionIfNotAlreadyInserted(scratchReg, getVTableIndexArgumentRegister());
      ...
      if (feGetEnv("helperSnippet") != NULL) {
         snippet = new (trHeapMemory()) TR::S390J9HelperCallSnippet(cg(), callNode, snippetLabel, helperRef, doneLabel, argSize);
...
//0x000003ffd9014d2c:	00 00 00 00	.long	0x00000000
      ...
      if (feGetEnv("helperSnippet") != NULL) {
         if (feGetEnv("generateCall")) {
            gcPoint = generateSnippetCall(cg(), callNode, snippet, dependencies, snippetSymRef);
         } else {
            gcPoint = generateS390BranchInstruction(cg(), TR::InstOpCode::BRC, TR::InstOpCode::COND_BRC, callNode, snippetLabel, dependencies);
         }
      } else {
...
//0x000003ffd8e87b2c:	00 00 00 00	.long	0x00000000
