...
void UncommonBCDCHKAddressNode::perform()
{
    ...
    for (TR::TreeTop *tt = comp()->getStartTree(); tt; tt = tt->getNextTreeTop()) {
        ...
        if (node && node->getOpCodeValue() == TR::BCDCHK && node->getSymbol()
            && node->getSymbol()->getResolvedMethodSymbol() && node->getNumChildren() >= 2 && node->getFirstChild()
            && node->getFirstChild()->getType().isBCD() && node->getSecondChild()
            && node->getSecondChild()->getDataType().isAddress() && node->getSecondChild()->getReferenceCount() > 1) {
            ...
            if (method == TR::com_ibm_dataaccess_DecimalData_convertIntegerToPackedDecimal_
                ...
                TR_ASSERT_FATAL_WITH_NODE(node,
                    (oldAddressNode->isDataAddrPointer() && oldAddressNode->getNumChildren() == 1)
                        || ((addressOp == TR::aladd || addressOp == TR::aiadd)
                            && oldAddressNode->getNumChildren() == 2),
                    "Expecting either a dataAddrPointer node with 1 child or a aladd/aiadd with 2 children under "
                    "address node of BCDCHK."
                    "But the address node %s had %d children and dataAddrPointer flag set to %s\n",
                    oldAddressNode->getOpCode().getName(), oldAddressNode->getNumChildren(),
                    oldAddressNode->isDataAddrPointer() ? "true" : "false");
                TR::Node *newAddressNode = NULL;
                if (oldAddressNode->getOpCodeValue() == TR::aloadi) {
                    // handles expected tree structure 2
                    newAddressNode
                        = TR::TransformUtil::generateArrayElementAddressTrees(comp(), oldAddressNode->getFirstChild());
                } else {
                    // handles expected tree structure 1 and 3
                    newAddressNode = TR::Node::create(node, addressOp, 2, oldAddressNode->getFirstChild(),
