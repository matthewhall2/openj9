        ...
        if (node->getOpCodeValue() == TR::BBStart) {
            ...
            if (!block->isExtensionOfPreviousBlock()) {
                logprintf(cg->traceBCDCodeGen(), comp()->log(),
                    "block_%d is a new extension so clear all load and store lists\n", block->getNumber());
                clearAllLists();
            }
        }
...
void InMemoryLoadStoreMarking::refineConditionalCleanLoadList(TR::Node *child, TR::Node *parent)
{
    ...
    if (_BCDConditionalCleanLoadList.find(child)) {
        logprintf(trace, log,
            "found %s (%p) in condCleanLoadList : examine parent %s (%p) to see if it relies on clean sign\n",
            child->getOpCode().getName(), child, parent->getOpCode().getName(), parent);
        if (cg->reliesOnAParticularSignEncoding(parent)) {
            logprintf(trace, log,
                "\tz^z : parent %s (%p) does rely on a clean sign so remove child %s (%p) from condCleanLoadList\n",
                parent->getOpCode().getName(), parent, child->getOpCode().getName(), child);
            _BCDConditionalCleanLoadList.remove(child);
        } else {
            logprintf(trace, log,
                "\tparent %s (%p) does not rely on a clean sign so do not remove child %s (%p) from "
                "condCleanLoadList\n",
                parent->getOpCode().getName(), parent, child->getOpCode().getName(), child);
        }
    }
...
void InMemoryLoadStoreMarking::examineFirstReference(TR::Node *node,
    TR::TreeTop *tt) // called the first time 'node' is encountered
{
    OMR::Logger *log = comp()->log();
    bool trace = cg->traceBCDCodeGen();

    if ((node->getType().isAggregate() || node->getType().isBCD()) && node->getReferenceCount() > 1) {
        if (tt->getNode()->getOpCode().isBCDStore() && tt->getNode()->getValueChild() == node) {
            logprintf(trace, log,
                "store first encounter: %s (%p) under %s (%p), add store to list and set node futureUseCount %d\n",
                node->getOpCode().getName(), node, tt->getNode()->getOpCode().getName(), tt->getNode(),
        ...
        // pdstore     <- tt->getNode()
        //    pdload   <- node
        // the pdstore is added the _BCDAggrStoreList and the node is added to the _BCDAggrLoadList and the node
        // futureUseCount is set twice but to the same value
        if (node->getOpCode().isBCDLoadVar()) {
            logprintf(trace, log,
                "load var first encounter: %s (%p), add load to list and set node futureUseCount %d\n",
...
void InMemoryLoadStoreMarking::handleLoadLastReference(TR::Node *node, List<TR::Node> &nodeList,
    TR_NodeListTypes listType)
{
    ...
    if (isLastReferenceInList
        && performTransformation(comp(), "%ssetSkipCopyOnLoad to true on %s (0x%p) and remove from %s\n", OPT_DETAILS,
            node->getOpCode().getName(), node, getName(listType))) {
        node->setSkipCopyOnLoad(true);
        nodeList.remove(node);
    }

    if (isLastReferenceInList && listType == ConditionalCleanLoadList && node->hasSignStateOnLoad()
        && performTransformation(comp(), "%ssetHasSignStateOnLoad to false on %s (0x%p) for node in %s\n", OPT_DETAILS,
            node->getOpCode().getName(), node, getName(listType))) {
        // if every reference does not rely on the sign state then it is safe to consider that the load has no sign
        // state on the load (as all parent will not care about a particular sign)
...
void InMemoryLoadStoreMarking::examineCommonedReference(TR::Node *child, TR::Node *parent, TR::TreeTop *tt)
{
    ...
    logprintf(cg->traceBCDCodeGen(), comp()->log(),
        "looking at commoned reference to %s (%p) with parent %s (%p) and tt %s (%p) (isLastReference %s)\n",
        child->getOpCode().getName(), child, parent ? parent->getOpCode().getName() : "NULL", parent,
        tt ? tt->getNode()->getOpCode().getName() : "NULL", tt ? tt->getNode() : 0, isLastReference ? "yes" : "no");
    ...
    if (child->getType().isBCD() && !_BCDAggrStoreList.isEmpty()) {
        ...
        for (TR::Node *store = listIt.getFirst(); store; store = listIt.getNext()) {
            if (isLastReference && store->getValueChild() == child
                && performTransformation(comp(),
                    "%ssetSkipCopyOnStore to true on %s (0x%p) and remove from list for last commoned ref to "
                    "valueChild %s (0x%p)\n",
...
{
    if (_BCDAggrLoadList.isEmpty() && _BCDAggrStoreList.isEmpty() && _BCDConditionalCleanLoadList.isEmpty()) {
        return true;
    } else {
        return false;
    }
...
void InMemoryLoadStoreMarking::handleDef(TR::Node *node, TR::TreeTop *tt)
{
    if (!allListsAreEmpty()) {
        if (tt->isPossibleDef()) {
            ...
            TR::Node *defNode
                = tt->getNode()->getOpCodeValue() == TR::treetop ? tt->getNode()->getFirstChild() : tt->getNode();
...
{
    if (defNode->getOpCode().isBCDStore() && defNode->chkCleanSignInPDStoreEvaluator()
        && listNode->getOpCode().isBCDLoadVar() && defNode->getValueChild() == listNode
        ...
        //    =>pdload "a"
        //
        // In this case, if all the parents of pdload "a" do not rely on the sign being clean then this exact store from
        // "a" to "a" does not have to be considered a kill and the skipCopyOnLoad flag can still be set. The listNode
        // will be added to the _BCDConditionalCleanLoadList so all subsequent references have their parents checked
        // (for not relying on the clean sign).
        //
        // The _BCDConditionalCleanLoadList also must be checked for other possible kills as with the regular
        // _BCDAggrLoadList The caller is removing listNode from _BCDAggrLoadList so a particular listNode will only be
        // in one of (or neither of) _BCDConditionalCleanLoadList and _BCDAggrLoadList
        logprintf(cg->traceBCDCodeGen(), comp()->log(),
            "\t\tfound conditional clean listNode %s (%p) under %s (%p) : do not consider a def but add to "
            "condCleanLoadList if not already present\n",
...
void InMemoryLoadStoreMarking::addToConditionalCleanLoadList(TR::Node *listNode)
{
    ...
    if (_BCDConditionalCleanLoadList.find(listNode)) {
        // This condition may be hit if the commoned load is present under two or more different stores to the same
        // location that also clean: pdstore "a" clean
        //    pdload "a"
        //
        // pdstore "a" clean
...
void InMemoryLoadStoreMarking::processBCDAggrNodeList(List<TR::Node> &nodeList, TR::Node *defNode,
    TR_NodeListTypes listType)
{
    if (!nodeList.isEmpty()) {
        OMR::Logger *log = comp()->log();
        ...
        for (TR::Node *listNode = listIt.getFirst(); listNode; listNode = listIt.getNext()) {
            TR_ASSERT((isAnyTypeOfLoadList && listNode->getOpCode().isBCDLoadVar())
                    || listNode->getOpCode().isBCDStore(),
                "only bcd/aggr loadVars or stores should be in the list (%s (%p))\n", listNode->getOpCode().getName(),
                listNode);
            if (isAnyTypeOfLoadList || listNode != defNode) // skip the current store treetop node
            {
                ...
                logprintf(trace, log, "\t\tintersect defNode %s (%p) #%d aliases with listNode %s (%p) #%d\n",
                    defNode->getOpCode().getName(), defNode,
                    defNodeHasSymRef ? defNode->getSymbolReference()->getReferenceNumber() : -1,
                    listNode->getOpCode().getName(), listNode, listNode->getSymbolReference()->getReferenceNumber());

                if (!defNodeHasSymRef
                    || defNode->getSymbolReference()->getUseDefAliases().contains(listNode->getSymbolReference(),
                        comp())) {
                    if (isAnyTypeOfLoadList) {
                        // There are two possible list types (condClean and regular load list) that can reach here and
                        // two conditions (clean or not clean) so expressing 4 states as a table
                        //
                        //                           |   _BCDConditionalCleanLoadList (cleanList)  |   _BCDAggrLoadList
                        //                           (regList)
                        //---------------------------|---------------------------------------------|---------------------------------
                        //  isConditionalCleanLoad   |  1) add listNode to cleanList               |  1) add listNode to
                        //  cleanList
                        //                           |                                             |  2) remove listNode
                        //                           from regList
                        // --------------------------|---------------------------------------------|----------------------------------
                        // !isConditionalCleanLoad   |  1) remove listNode from cleanList          |  1) remove listNode
                        // from regList
                        //

                        // first check if we are able to place listNode in the conditional load list vs more
                        // conservatively just marking setSkipCopyOnLoad to false
                        if (isConditionalCleanLoad(listNode, defNode)) {
                            ...
                            // this path may either be removing a load from the regular or conditional load list (in the
                            // latter case when some other type of def is possibly seen)
                            logprintf(trace, log,
                                "\t\t\tfound an intersection so remove listNode %s (%p) from %s and set "
                                "setSkipCopyOnLoad flag to false (defNodeHasSymRef=%s)\n",
                                listNode->getOpCode().getName(), listNode, getName(listType),
                                defNodeHasSymRef ? "yes" : "no");
                            listNode->setSkipCopyOnLoad(false);
                            nodeList.remove(listNode);
                        }
                    } else {
                        logprintf(trace, log,
                            "\t\t\tfound an intersection so remove listNode %s (%p) from %s and set setSkipCopyOnStore "
...
const char *InMemoryLoadStoreMarking::_TR_NodeListTypeNames[NodeList_NumTypes]
    = { "LoadList", "ConditionalCleanLoadList", "StoreList" };
