...
void ReduceSynchronizedFieldLoad::inlineSynchronizedFieldLoad(TR::Node *node, TR::CodeGenerator *cg)
{
    ...
    cg->generateDebugCounter(TR::DebugCounter::debugCounterName(comp,
        "codegen/z/ReduceSynchronizedFieldLoad/success/OOL/%s", comp->signature()));
    ...
    TR_ASSERT_SAFE_FATAL(loadInstruction->isLoad()
            && (loadInstruction->getKind() == OMR::Instruction::Kind::IsRX
                || loadInstruction->getKind() == OMR::Instruction::Kind::IsRXY),
        "Expecting the append instruction to be a load of kind RX or RXY\n");

    TR::MemoryReference *loadMemoryReference
        = static_cast<TR::S390RXInstruction *>(loadInstruction)->getMemoryReference();

    TR_ASSERT_SAFE_FATAL(loadMemoryReference->getIndexRegister() == NULL,
        "Load memory reference must not have an index register\n");
    ...
    if (is32BitLock && is32BitLoad) {
        ...
        if (is32BitLoad) {
            if ((loadMemoryReference->getOffset() & 7) == 0) {
                ...
                // This is because we must use LPDG to load a 32-bit value using displacement -4
                TR_ASSERT_SAFE_FATAL((loadMemoryReference->getOffset() & 3) == 0,
                    "Field must be aligned on a word boundary\n");
            ...
            TR_ASSERT_SAFE_FATAL((loadMemoryReference->getOffset() & 7) == 0,
                "Field must be aligned on a double-word boundary\n");
        }

        generateSSFInstruction(cg, TR::InstOpCode::LPDG, node, registerPair, alignedLoadMemoryReference,
            alignedLockMemoryReference);
    ...
    generateRIInstruction(cg, TR::InstOpCode::NILL, node, lockRegister,
        ~(OBJECT_HEADER_LOCK_RECURSION_MASK | OBJECT_HEADER_LOCK_RESERVED));

    TR::Register *metaDataReg = cg->getMethodMetaDataRealRegister();
    if (is32BitLock && is32BitLoad) {
        // Now check if we loaded the lock of the current thread
        generateS390CompareAndBranchInstruction(cg, TR::InstOpCode::CR, node, lockRegister, metaDataReg,
            TR::InstOpCode::COND_BE, mergeLabel, false);

        // Lock could be free as well
        generateS390CompareAndBranchInstruction(cg, TR::InstOpCode::C, node, lockRegister, 0, TR::InstOpCode::COND_BE,
            mergeLabel, false);
    } else {
        // Now check if we loaded the lock of the current thread
        generateS390CompareAndBranchInstruction(cg, TR::InstOpCode::CGR, node, lockRegister, metaDataReg,
            TR::InstOpCode::COND_BE, mergeLabel, false);

        // Lock could be free as well
        generateS390CompareAndBranchInstruction(cg, TR::InstOpCode::CG, node, lockRegister, 0, TR::InstOpCode::COND_BE,
            mergeLabel, false);
    }
...
bool ReduceSynchronizedFieldLoad::perform()
{
    TR::Compilation *comp = cg->comp();
...
bool ReduceSynchronizedFieldLoad::performOnTreeTops(TR::TreeTop *startTreeTop, TR::TreeTop *endTreeTop)
{
    TR::Compilation *comp = cg->comp();
    ...
    for (TR::TreeTopIterator iter(startTreeTop, comp); iter != endTreeTop; ++iter) {
        if (iter.currentNode()->getOpCodeValue() == TR::monent
            || iter.currentNode()->getOpCodeValue() == TR::treetop
                && iter.currentNode()->getFirstChild()->getOpCodeValue() == TR::monent) {
            ...
            for (++iter; iter != endTreeTop; ++iter) {
                if (iter.currentNode()->getOpCodeValue() == TR::monexit
                    || iter.currentNode()->getOpCodeValue() == TR::treetop
                        && iter.currentNode()->getFirstChild()->getOpCodeValue() == TR::monexit) {
                    ...
                    if (synchronizedObjectNode == monexitNode->getFirstChild()) {
                        ...
                        TR::Node *loadNode = findLoadInSynchornizedRegion(startTreeTop, endTreeTop, monentTreeTop,
                            monexitTreeTop, synchronizedObjectNode);
                        ...
                        if (loadNode != NULL) {
                            ...
                            if (comp->target().is32Bit() && J9::DataType::getSize(loadNode->getDataType()) == 8) {
                                TR::DebugCounter::incStaticDebugCounter(comp,
                                    TR::DebugCounter::debugCounterName(comp,
                                        "codegen/z/ReduceSynchronizedFieldLoad/failure/31-bit-register-pairs/%s",
                                        comp->signature()));

                                break;
                            }

                            // When concurrent scavenge is enabled we need to load the object reference using a read
                            // barrier however there is no guarded load alternative for the LPD instruction. As such
                            // this optimization cannot be carried out for object reference loads under concurrent
                            // scavenge.
                            if (TR::Compiler->om.readBarrierType() != gc_modron_readbar_none
                                && loadNode->getDataType().isAddress()) {
                                TR::DebugCounter::incStaticDebugCounter(comp,
                                    TR::DebugCounter::debugCounterName(comp,
                                        "codegen/z/ReduceSynchronizedFieldLoad/failure/read-barrier/%s",
                                        comp->signature()));

                                break;
                            }

                            int32_t lockWordOffset = static_cast<TR_J9VMBase *>(comp->fe())
                                                         ->getByteOffsetToLockword(static_cast<TR_OpaqueClassBlock *>(
                                                             cg->getMonClass(monentNode)));
                            ...
                            if (lockWordOffset > 0 && lockWordOffset < 4096) {
                                if (performTransformation(comp,
                                        "%sReplacing monent [%p] - monexit [%p] synchronized region on load [%p] with "
                                        "fabricated call\n",
                                    ...
                                    TR::SymbolReference *methodSymRef
                                        = comp->getSymRefTab()->findOrCreateCodeGenInlinedHelper(
                                            TR::SymbolReferenceTable::synchronizedFieldLoadSymbol);

                                    TR::Node *callNode
                                        = TR::Node::createWithSymRef(loadNode, TR::call, 5, methodSymRef);
                                    ...
                                    TR::Node *monentSymbolReferenceNode = TR::Node::createWithSymRef(loadNode, TR::call,
                                        1, synchronizedObjectNode, monentNode->getSymbolReference());
                                    TR::Node *monexitSymbolReferenceNode = TR::Node::createWithSymRef(loadNode,
                                        TR::call, 1, synchronizedObjectNode, monexitNode->getSymbolReference());
                                    ...
                                    TR::DebugCounter::incStaticDebugCounter(comp,
                                        TR::DebugCounter::debugCounterName(comp,
                                            "codegen/z/ReduceSynchronizedFieldLoad/success/%s", comp->signature()));
                                }
                            } else {
                                TR::DebugCounter::incStaticDebugCounter(comp,
                                    TR::DebugCounter::debugCounterName(comp,
                                        "codegen/z/ReduceSynchronizedFieldLoad/failure/lockword-out-of-bounds/%s",
                                        comp->signature()));
                            }
                        } else {
                            TR::DebugCounter::incStaticDebugCounter(comp,
                                TR::DebugCounter::debugCounterName(comp,
                                    "codegen/z/ReduceSynchronizedFieldLoad/failure/load-not-found/%s",
                                    comp->signature()));
                        }
                    } else {
                        TR::DebugCounter::incStaticDebugCounter(comp,
                            TR::DebugCounter::debugCounterName(comp,
                                "codegen/z/ReduceSynchronizedFieldLoad/failure/monexit-synchronized-object-mismatch/%s",
                                comp->signature()));
                    }
...
TR::Node *ReduceSynchronizedFieldLoad::findLoadInSynchornizedRegion(TR::TreeTop *startTreeTop, TR::TreeTop *endTreeTop,
    TR::TreeTop *monentTreeTop, TR::TreeTop *monexitTreeTop, TR::Node *synchronizedObjectNode)
{
    TR::Compilation *comp = cg->comp();
    OMR::Logger *log = comp->log();
    bool trace = comp->getOption(TR_TraceCG);
    ...
    for (; iter != monexitTreeTop; ++iter) {
        ...
        if (opcode.hasSymbolReference() || opcode.isBranch()) {
            if (loadNode == NULL && opcode.isLoadIndirect() && (opcode.isRef() || opcode.isInt() || opcode.isLong())
