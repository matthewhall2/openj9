    ...
    : TR::Linkage(codeGen, elc)
{
    ...
    if (comp->target().isLinux()) {
        ...
        // Hardcoding register map for GC can lead to subtle bugs if linkage is changed and we did not change register
        // map for GC.
        // TODO We should write a function that goes through the list of preserved registers and prepare register map
        // for GC.
    ...
    if (comp->target().isZOS()) {
        ...
        if (comp->target().is64Bit()) {
            ...
            setRegisterFlag(TR::RealRegister::GPR15, Preserved);
            // 31-Bit zOS will need GPR12 for CAA register so it won't be preserved. For all other variant it is
            // preserved
...
public:
    ...
    // For the post dependency conditions we need return address register to be assigned to proper virtual register
    // instead of the dummy virtual register
    TR::RegisterDependencyConditions *buildRegisterDependencyConditions(TR::RealRegister::RegNum returnAddressReg)
    {
        ...
        // Hence asking one less post deps so this won't create a problem when combined with other dependency condition
        // for ICF.
    ...
    inline uint8_t numberOfRegistersInUse() const { return _numberOfRegistersInUse; }
 ...
 */
TR::Register *J9::Z::CHelperLinkage::buildDirectDispatch(TR::Node *callNode, TR::RegisterDependencyConditions **deps,
    TR::Register *returnReg)
{
    ...
    // TODO: Currently only jitInstanceOf is fast path helper. Need to modify following condition if we add support for
    // other fast path only helpers
    ...
    logprintf(trace, log, "%s: Internal Control Flow in OOL : %s\n", callNode->getOpCode().getName(),
        isHelperCallWithinICF ? "true" : "false");
    for (int i = TR::RealRegister::FirstGPR; i < TR::RealRegister::NumRegisters; i++) {
        if (!self()->getPreserved(REGNUM(i))
            && cg()->machine()->getRealRegister(i)->getState() != TR::RealRegister::Locked) {
            RealRegisters.use((TR::RealRegister::RegNum)i);
        }
    }
    TR::RegisterDependencyConditions *childNodeRegDeps
        = generateRegisterDependencyConditions(0, callNode->getNumChildren(), cg());
    TR::RegisterDependencyConditions *preDeps
        = generateRegisterDependencyConditions(callNode->getNumChildren() + 1, 0, cg());
    ...
    // TODO For Time Being, it is expected that param number won't increase beyond 3 need to fix this when support for
    // stack is there
    for (int i = 0; i < callNode->getNumChildren(); i++) {
        if (i < self()->getNumIntegerArgumentRegisters() - 1)
            preDeps->addPreCondition(cg()->gprClobberEvaluate(callNode->getChild(i)),
                getIntegerArgumentRegister(i + 1));
        else
            TR_ASSERT(false, "Parameters on Stack not supported yet");
        childNodeRegDeps->addPostConditionIfNotAlreadyInserted(callNode->getChild(i)->getRegister(),
     ...
     */
    TR::RealRegister::RegNum regRANum
        = isFastPathOnly ? self()->getReturnAddressRegister() : cg()->getReturnAddressRegister();
    ...
    // If buildDirectDispatch is called within ICF we need to pass the dependencies which will be used there, else we
    // need single dependencylist to be attached to the BRASL We return postdependency conditions back to evaluator to
    // merge with ICF condition and attach to merge label
    ...
    if (isFastPathOnly) {
        ...
        cursor = generateRXInstruction(cg(), TR::InstOpCode::getStoreOpCode(), callNode, javaStackPointerRegister,
            generateS390MemoryReference(vmThreadRegister, offsetJ9SP, cg()));
#if defined(J9ZOS390)
        padding += 2;
        // Loading DSAPointer Register
        DSAPointerReg = RealRegisters.use(getDSAPointerRegister());
        generateRXInstruction(cg(), TR::InstOpCode::getLoadOpCode(), callNode, DSAPointerReg,
        ...
        generateRXInstruction(cg(), TR::InstOpCode::getLoadOpCode(), callNode, CAAPointerReg,
            generateS390MemoryReference(DSAPointerReg, J9TR_CAA_save_offset, cg()));
#endif
#endif
    }
    ...
    if (isFastPathOnly) {
         ...
         *
         * As a result of this, JIT'ed code that does XPLINK calls needs 2 or 4-byte NOP paddings to ensure entry to
         * valid instruction.
         *
        ...
        cursor
            = new (cg()->trHeapMemory()) TR::S390NOPInstruction(TR::InstOpCode::NOP, padding, callNode, cursor, cg());
        // Storing DSA Register back
        cursor = generateRXInstruction(cg(), TR::InstOpCode::getStoreOpCode(), callNode, DSAPointerReg,
            generateS390MemoryReference(vmThreadRegister, offsetOfSSP, cg()), cursor);
#endif
        // Reloading Java Stack pointer
        cursor = generateRXInstruction(cg(), TR::InstOpCode::getLoadOpCode(), callNode, javaStackPointerRegister,
            generateS390MemoryReference(vmThreadRegister, offsetJ9SP, cg()), cursor);
    } else {
        // Fastpath helper do not expects GC call in-between so only attaching them for normal dual mode helpers
        // As GC map is attached to instruction after RA is done, it is guaranteed that all the non-preserved register
        // by system linkage are either stored in preserved register Or spilled to stack. We only need to mark preserved
        // register in GC map. Only possibility of non-preserved register containing a live object is in argument to
        // helper which should be a clobberable copy of actual object.
    ...
    if (returnReg != NULL)
        generateRRInstruction(cg(), TR::InstOpCode::getLoadRegOpCode(), callNode, returnReg,
            RealRegisters.use(isFastPathOnly ? getIntegerReturnRegister() : getLongHighReturnRegister()), cursor);
