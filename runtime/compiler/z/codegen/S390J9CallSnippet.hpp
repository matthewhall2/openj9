...
public:
    S390J9CallSnippet(TR::CodeGenerator *cg, TR::Node *n, TR::LabelSymbol *lab, int32_t s)
        : TR::S390CallSnippet(cg, n, lab, s)
    {}

    S390J9CallSnippet(TR::CodeGenerator *cg, TR::Node *n, TR::LabelSymbol *lab, TR::SymbolReference *symRef, int32_t s)
        : TR::S390CallSnippet(cg, n, lab, symRef, s)
    {}

    static uint8_t *generateVIThunk(TR::Node *callNode, int32_t argSize, TR::CodeGenerator *cg);
    static TR_MHJ2IThunk *generateInvokeExactJ2IThunk(TR::Node *callNode, int32_t argSize, char *signature,
        TR::CodeGenerator *cg);
...
public:
    J9S390InterfaceCallDataSnippet(TR::CodeGenerator *, TR::Node *, uint8_t, void *, bool useCLFIandBRCL = false);
    ...
    uint8_t *setCodeRA(uint8_t *codeRA) { return _codeRA = codeRA; }
...
public:
    S390JNICallDataSnippet(TR::CodeGenerator *, TR::Node *);
...
public:
    S390InterfaceCallSnippet(TR::CodeGenerator *cg, TR::Node *c, TR::LabelSymbol *lab, int32_t s, int8_t n,
        void *thunkPtr, bool useCLFIandBRCL = false);
 ...
 * 
 * For the case of jitDispatchJ9Method, moves the J9Method pointer in GPR4/GPR15 to GPR1 after flushing the args to the stack
 * so it is where the interpreter expects it to be.
 */
...
public:
    S390J9HelperCallSnippet(TR::CodeGenerator *cg, TR::Node *node, TR::LabelSymbol *snippetlab,
        ...
        : TR::S390HelperCallSnippet(cg, node, snippetlab, helper, restartlab, s), _needsArgFlush(needsArgFlush)
    {}
