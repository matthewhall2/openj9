...
////////////////////////////////////////////////////////////////////////////////
void J9::Z::zOSSystemLinkage::generateInstructionsForCall(TR::Node *callNode, TR::RegisterDependencyConditions *deps,
    ...
    TR::LabelSymbol *returnFromJNICallLabel, TR::Snippet *callDataSnippet, bool isJNIGCPoint)
{
     ...
     * _targetAddress is used in different relocation types, where sometimes it's an actual address and sometimes (eg.
     * TR_HelperAddress) it's a symref for cases when _targetAddress is actually a symref we cast it back to symref in
     * TR::AheadOfTimeCompile::initializeAOTRelocationHeader
     *
     * In this case
     * generateRegLitRefInstruction creates a ConstantDataSnippet with reloType: TR_HelperAddress, which creates a
     * 32Bit/64Bit ExternalRelocation of type TR_AbsoluteHelperAddress with _targetAddress set to TR::SymbolReference
     * from the call
     *
     */
    // get the address of the function descriptor
    if (fej9->needRelocationsForHelpers() && !(callNode->getSymbol()->isResolvedMethod() || jniCallDataSnippet)) {
        generateRegLitRefInstruction(cg(), TR::InstOpCode::getLoadOpCode(), callNode, methodAddressReg,
            (uintptr_t)callNode->getSymbolReference(), TR_HelperAddress, NULL, NULL, NULL);
    } else if (!callNode->getSymbol()->isResolvedMethod()
        || !jniCallDataSnippet) // An unresolved method means a helper being called  using system linkage
    {
        if (codeGen->needClassAndMethodPointerRelocations() && callNode->isPreparedForDirectJNI()) {
            ...
            generateRegLitRefInstruction(cg(), TR::InstOpCode::getLoadOpCode(), callNode, methodAddressReg,
                (uintptr_t)targetAddress, reloType, NULL, NULL, NULL);
        } else
    ...
    generateRRInstruction(codeGen, TR::InstOpCode::getLoadRegOpCode(), callNode, javaLitOffsetReg,
        systemEntryPointRegister);
    ...
    if (cg()->supportsJITFreeSystemStackPointer()) {
        auto *systemSPOffsetMR = generateS390MemoryReference(methodMetaDataVirtualRegister,
            static_cast<int32_t>(fej9->thisThreadGetSystemSPOffset()), codeGen);
     ...
     *
     * As a result of this, JIT'ed code that does XPLINK calls needs 2 or 4-byte NOP paddings to ensure entry to valid
     * instruction.
     *
     ...
     */
    TR::RegisterDependencyConditions *callPreDeps = new (self()->trHeapMemory())
        TR::RegisterDependencyConditions(deps->getPreConditions(), NULL, deps->getAddCursorForPre(), 0, codeGen);
    TR::RegisterDependencyConditions *callPostDeps = new (self()->trHeapMemory())
        TR::RegisterDependencyConditions(NULL, deps->getPostConditions(), 0, deps->getAddCursorForPost(), codeGen);

    gcPoint = generateRRInstruction(codeGen, TR::InstOpCode::BASR, callNode, systemReturnAddressRegister,
        systemEntryPointRegister, callPreDeps);
    if (isJNIGCPoint)
    ...
    generateRRInstruction(codeGen, TR::InstOpCode::getLoadRegOpCode(), callNode, systemEntryPointRegister,
        javaLitOffsetReg);
}

void J9::Z::zOSSystemLinkage::setupRegisterDepForLinkage(TR::Node *callNode, TR_DispatchType dispatchType,
    ...
    TR::Node *&GlobalRegDeps, bool &hasGlRegDeps, TR::Register **methodAddressReg, TR::Register *&javaLitOffsetReg)
{
    ...
    J9::Z::PrivateLinkage *privateLinkage = static_cast<J9::Z::PrivateLinkage *>(cg()->getLinkage(TR_Private));
    privateLinkage->setupRegisterDepForLinkage(callNode, dispatchType, deps, killMask, systemLinkage, GlobalRegDeps,
        hasGlRegDeps, methodAddressReg, javaLitOffsetReg);
}

void J9::Z::zOSSystemLinkage::setupBuildArgForLinkage(TR::Node *callNode, TR_DispatchType dispatchType,
...
{
    J9::Z::PrivateLinkage *privateLinkage = static_cast<J9::Z::PrivateLinkage *>(cg()->getLinkage(TR_Private));
    privateLinkage->setupBuildArgForLinkage(callNode, dispatchType, deps, isFastJNI, isPassReceiver, killMask,
        GlobalRegDeps, hasGlRegDeps, systemLinkage);
}

void J9::Z::zOSSystemLinkage::performCallNativeFunctionForLinkage(TR::Node *callNode, TR_DispatchType dispatchType,
    TR::Register *&javaReturnRegister, TR::SystemLinkage *systemLinkage, TR::RegisterDependencyConditions *&deps,
    TR::Register *javaLitOffsetReg, TR::Register *methodAddressReg, bool isJNIGCPoint)
{
    // call base class implementation first
    OMR::Z::Linkage::performCallNativeFunctionForLinkage(callNode, dispatchType, javaReturnRegister, systemLinkage,
        deps, javaLitOffsetReg, methodAddressReg, isJNIGCPoint);
...
void J9::Z::zOSSystemLinkage::doNotKillSpecialRegsForBuildArgs(TR::Linkage *linkage, bool isFastJNI, int64_t &killMask)
{
    J9::Z::PrivateLinkage *privateLinkage = static_cast<J9::Z::PrivateLinkage *>(cg()->getLinkage(TR_Private));
    privateLinkage->doNotKillSpecialRegsForBuildArgs(linkage, isFastJNI, killMask);
}

void J9::Z::zOSSystemLinkage::addSpecialRegDepsForBuildArgs(TR::Node *callNode,
...
int32_t J9::Z::zOSSystemLinkage::storeExtraEnvRegForBuildArgs(TR::Node *callNode, TR::Linkage *linkage,
    TR::RegisterDependencyConditions *dependencies, bool isFastJNI, int32_t stackOffset, int8_t gprSize,
    uint32_t &numIntegerArgs)
{
    J9::Z::PrivateLinkage *privateLinkage = static_cast<J9::Z::PrivateLinkage *>(cg()->getLinkage(TR_Private));
    stackOffset = privateLinkage->storeExtraEnvRegForBuildArgs(callNode, linkage, dependencies, isFastJNI, stackOffset,
        gprSize, numIntegerArgs);
    return stackOffset;
}

int64_t J9::Z::zOSSystemLinkage::addFECustomizedReturnRegDependency(int64_t killMask, TR::Linkage *linkage,
