...
public:
     ...
     *
     * For Java 8:
     * com.ibm.jit.JITHelpers.intrinsicIndexOfStringLatin1(Object s1Value, int s1len, Object s2Value, int s2len, int
     * start); com.ibm.jit.JITHelpers.intrinsicIndexOfStringUTF16(Object s1Value, int s1len, Object s2Value, int s2len,
     * int start);
     *
     * Assumptions:
     *
    ...
    static TR::Register *generateHelperCallForVMNewEvaluators(TR::Node *node, TR::CodeGenerator *cg,
        bool doInlineAllocation = false, TR::Register *resReg = NULL);
     ...
     */
    static TR::Register *generateSoftwareReadBarrier(TR::Node *node, TR::CodeGenerator *cg, TR::Register *resultReg,
        TR::MemoryReference *memRef, TR::RegisterDependencyConditions *deps = NULL, bool produceUnshiftedValue = false);
     ...
     *
     *     When software concurrent scavenge is enabled, it generates a internal control flow that first checks if the
     * current execution is in the GC cycle and performs either a loop-based array copy or a helper call.
     *
     *  \param node
    ...
    static void generateLoadAndStoreForArrayCopy(TR::Node *node, TR::CodeGenerator *cg, TR::MemoryReference *srcMemRef,
        TR::MemoryReference *dstMemRef, TR_S390ScratchRegisterManager *srm, TR::DataType elenmentType,
        bool needsGuardedLoad, TR::RegisterDependencyConditions *deps = NULL);

    static void forwardArrayCopySequenceGenerator(TR::Node *node, TR::CodeGenerator *cg, TR::Register *byteSrcReg,
        TR::Register *byteDstReg, TR::Register *byteLenReg, TR::Node *byteLenNode, TR_S390ScratchRegisterManager *srm,
    ...
    static TR::Register *generatePackedToBinaryConversion(TR::Node *node, TR::InstOpCode::Mnemonic op,
        TR::CodeGenerator *cg);
    static TR::Register *generateVectorPackedToBinaryConversion(TR::Node *node, TR::InstOpCode::Mnemonic op,
        TR::CodeGenerator *cg);
    static TR::Register *generateBinaryToPackedConversion(TR::Node *node, TR::InstOpCode::Mnemonic op,
        TR::CodeGenerator *cg);
    static TR::Register *generateVectorBinaryToPackedConversion(TR::Node *node, TR::InstOpCode::Mnemonic op,
        TR::CodeGenerator *cg);
    ...
    static TR::Register *pdArithmeticVectorEvaluatorHelper(TR::Node *node, TR::InstOpCode::Mnemonic op,
        TR::CodeGenerator *cg);
    ...
    static TR_PseudoRegister *simpleWideningOrTruncation(TR::Node *node, TR_PseudoRegister *srcReg, bool isSetSign,
        int32_t sign, TR::CodeGenerator *cg);
    ...
    static TR::Register *vectorPerformSignOperationHelper(TR::Node *node, TR::CodeGenerator *cg, bool setPrecision,
        ...
        bool digitValidityCheck = true, int32_t sign = 0, bool setConditionCode = true,
    ...
    /*
     * Generate instructions to fill in the J9JITWatchedStaticFieldData.fieldAddress,
