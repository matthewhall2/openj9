...
public:
    ...
    int32_t decOwningRegisterCount()
    {
        TR_ASSERT(_owningRegisterCount > 0, "_owningRegisterCount %d should be > 0");
        return --_owningRegisterCount;
    }
    ...
    enum // _flags
    {
        ...
        HintHasBeenUsed = 0x02, // only allow a storageRefHint to be used once to avoid possible overlapping code to be
                                // generated when a parent/child share a hint
        IsReadOnlyTemporary = 0x04, // for passThroughs and operations that do a conservative clobber (like some
                                    // pdcleans) to skip/delay the clobberEvaluate
    };
...
private:
    ...
    int32_t
        _owningRegisterCount; // how many unique registers point to this storageRef -- needed during ssrClobberEvaluate
    flags8_t _flags;
...
// Instead, it is used to hold a TR_StorageReference that encapsulates and abstracts the result memory location as well
// as other evaluated node (result) specific state.
//
// TR_PseudoRegisters come in two flavours: a base TR_OpaquePseudoRegister that is basically just a simple container for
// a TR_StorageReference and a TR_BCDPseudoRegister that contains the TR_StorageReference as well as BCD specific state
//

class TR_OpaquePseudoRegister : public TR::Register {
public:
    TR_OpaquePseudoRegister(TR::DataType dt, TR::Compilation *c)
        : TR::Register(TR_SSR)
        , _comp(c)
    ...
    TR_OpaquePseudoRegister(TR_OpaquePseudoRegister *reg, TR::Compilation *c)
        : TR::Register(TR_SSR)
        , _comp(c)
        , _storageReference(NULL)
        , _size(reg->getSize())
    ...
    enum PseudoRegisterFlags // _flags
    {
        ...
        SignStateKnown = 0x04, // Known   - if the preferred,clean or signCode state is from generated code that
                               // guarantees a particular state
        HasPreferredSign = 0x08, //
        HasCleanSign = 0x10, //
        HasSignCode = 0x20, //

        HasKnownBadSignCode
            = 0x40, // Bad - code has been generated that guarantees the sign is bad (0-9).
        ...
        SignStateInitialized = 0x200, // Set to true when preferred,clean or _signCode is set but also manually for
                                      // operations that have no particular sign state (loads for example)
                                      // SignStateInitialized=true does not imply that any valid or known sign state has
                                      // to be set so HasKnownBadSignCode=true *does* imply SignStateInitialized=true
        HasTemporaryKnownSignCode = 0x400, // to improve code generation for zoned leading separate sign to x
                                           // conversions a zone value (0xf) can be temporarily set on the register
    ...
    virtual int32_t addToRightAlignedDeadBytes(int32_t bytes)
    {
        return setRightAlignedDeadBytes(_rightAlignedDeadBytes + bytes);
    }
    ...
    virtual int32_t addToRightAlignedIgnoredBytes(int32_t bytes)
    {
        return setRightAlignedIgnoredBytes(_rightAlignedIgnoredBytes + bytes);
    }
    ...
    virtual void clearRightAlignedState() { setRightAlignedDeadAndIgnoredBytes(0); }
...
private:
    ...
    int32_t _rightAlignedIgnoredBytes; // these bytes are still valid (not clobbered/invalidated) but not used by this
                                       // register
};

class TR_PseudoRegister : public TR_OpaquePseudoRegister {
public:
    TR_PseudoRegister(TR::DataType dt, TR::Compilation *c)
        : TR_OpaquePseudoRegister(dt, c)
    ...
    TR_PseudoRegister(TR_PseudoRegister *reg, TR::Compilation *c)
        : TR_OpaquePseudoRegister(reg, c)
        , _leftAlignedZeroDigits(0)
        , // do not transfer storageReference dependent values
        _decimalPrecision(reg->getDecimalPrecision())
        , _signCode(
              reg->hasKnownOrAssumedSignCode() ? reg->getKnownOrAssumedSignCode() : TR::DataType::getInvalidSignCode())
        , _temporaryKnownSignCode(TR::DataType::getInvalidSignCode())
    {}
    ...
    virtual void clearLeftAlignedState() { setLeftAlignedZeroDigits(0); }

    virtual bool trackZeroDigits() { return getDataType() == TR::PackedDecimal || getDataType() == TR::ZonedDecimal; }
    ...
    virtual int32_t getByteOffsetFromLeftForClear(int32_t startDigit, int32_t endDigit, int32_t &digitsToClear,
        int32_t resultSize);

    // Sometimes a register value has some of its right most bytes invalidated (pdshr/pddiv for example). This value
    // must be tracked so a codegen TR_*MemoryReference created from this register can have its displacement adjust
    // accordingly.
    ...
    {
        if (signStateAssumed()) {
            // TR_ASSERT(false,"should not promote sign state from assumed to known\n");  // a new TR_PseudoRegister and
            // a transferSignState should be used instead
    ...
    int32_t setSignCode(int32_t sign)
    {
        ...
        if (preferredPlusSign == TR::DataType::getInvalidSignCode()
            || preferredMinusSign == TR::DataType::getInvalidSignCode()) {
    ...
    // There is no guaranteed invalid signCode value so an explicit HasAssumedSignCode/HasKnownSignCode is required to
    // indicate the _signCode value has been set. The default, unset, value of signCode is 0
    // (TR::DataType::getInvalidSignCode()) and even though this will likely not match any valid sign code the assume is
    // here so retrieving the knownSignCode without first checking hasKnownSignCode() is discouraged.
    ...
    {
        TR_ASSERT(hasAssumedSignCode(),
            "getAssumedSignCode() should not be called unless hasAssumedSignCode() is true\n");
        return _signCode;
    }

    int32_t getKnownOrAssumedSignCode()
    {
        TR_ASSERT(hasKnownOrAssumedSignCode(),
            "getKnownOrAssumedSignCode() should not be called unless hasKnownOrAssumedSignCode() is true\n");
        return _signCode;
    }
    ...
    void resetCleanSign() { _flags.set(HasCleanSign, false); }
    ...
    // Only provide a getter and resetter to not encourage setting the flag without also providing a known sign code
    // value.
    ...
    void setHasKnownValidData() { _flags.set(HasKnownValidData, true); }
    ...
    void setSignStateInitialized() { _flags.set(SignStateInitialized, true); }

    bool hasTemporaryKnownSignCode() { return _flags.testAny(HasTemporaryKnownSignCode); }

    int32_t getTemporaryKnownSignCode()
    {
        TR_ASSERT(hasTemporaryKnownSignCode(),
            "getTemporaryKnownSignCode() should not be called unless hasTemporaryKnownSignCode() is true\n");
    ...
    bool isLegalToCleanSign()
    {
        ...
        //
        return signStateInitialized() && !knownOrAssumedSignCodeIs(TR::DataType::getUnsignedCode());
    }
    ...
    void transferSignState(TR_PseudoRegister *srcReg, bool digitsLost)
    {
        ...
        // Before setting any of the optional sign state (clean,preferred,signCode) make sure the srcReg has
        // non-conflicting known/assumed settings
        ...
        if (srcReg->hasKnownOrAssumedSignCode()) {
            ...
            if (_signCode != TR::DataType::getInvalidSignCode()
                && _signCode == TR::DataType::getPreferredPlusSignCode(getDataType()))
                _flags.set(HasCleanSign, true);
        }
...
private:
    // The setHasCleanSign and setHasPreferredSign routines are private so the user is forced, based on the calling
    // context, to decide if either a known or assumed clean/preferred sign should be set.
    ...
    int32_t _decimalPrecision; // _decimalPrecision is the valid number of digits in a binary coded decimal memory slot
                               // result. The default value (set by node->setRegister()) is the corresponding
                               // node->getDecimalPrecision() but can be set to another value.
};
