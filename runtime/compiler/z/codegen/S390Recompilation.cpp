...
//
TR::Recompilation *TR_S390Recompilation::allocate(TR::Compilation *comp)
{
...
TR_PersistentMethodInfo *TR_S390Recompilation::getExistingMethodInfo(TR_ResolvedMethod *method)
{
...
TR::Instruction *TR_S390Recompilation::generatePrePrologue()
{
    ...
    if (useSampling()) {
        ...
        cursor = generateS390LabelInstruction(cg, TR::InstOpCode::label, node, samplingRecompileMethodSnippetLabel,
            cursor);
        ...
        cursor = generateRIInstruction(cg, TR::InstOpCode::getAddHalfWordImmOpCode(), node,
            cg->getStackPointerRealRegister(), -2 * cg->machine()->getGPRSize(), cursor);

        // Save the return address
        cursor = generateRRInstruction(cg, TR::InstOpCode::getLoadRegOpCode(), node,
            cg->machine()->getRealRegister(TR::RealRegister::GPR0), cg->getReturnAddressRealRegister(), cursor);

        TR::MemoryReference *epSaveAreaMemRef = generateS390MemoryReference(cg->getStackPointerRealRegister(), 0, cg);

        // Save the EP register
        cursor = generateRXInstruction(cg, TR::InstOpCode::getStoreOpCode(), node, cg->getEntryPointRealRegister(),
            epSaveAreaMemRef, cursor);

        // Load the EP register with the address of the next instruction
        cursor = generateRRInstruction(cg, TR::InstOpCode::BASR, node, cg->getEntryPointRealRegister(),
            cg->machine()->getRealRegister(TR::RealRegister::GPR0), cursor);

        basrInstruction = cursor;

        cursor = generateRRInstruction(cg, TR::InstOpCode::getLoadRegOpCode(), node, cg->getReturnAddressRealRegister(),
            cg->getEntryPointRealRegister(), cursor);

        // Displacement will be updated later once we know the offset
        TR::MemoryReference *samplingRecompileMethodAddressMemRef
            = generateS390MemoryReference(cg->getEntryPointRealRegister(), 0, cg);

        // Load the address of samplingRecompileMethod
        cursor = generateRXInstruction(cg, TR::InstOpCode::getLoadOpCode(), node, cg->getEntryPointRealRegister(),
            samplingRecompileMethodAddressMemRef, cursor);

        // Adjust the return address to point to the JIT entry point. Displacement will be updated later once we know
        // the offset.
        cursor = generateRIInstruction(cg, TR::InstOpCode::getAddHalfWordImmOpCode(), node,
            cg->getReturnAddressRealRegister(), 0, cursor);

        addImmediateToJITEPInstruction = static_cast<TR::S390RIInstruction *>(cursor);

        // Call samplingRecompileMethod
        cursor = generateS390BranchInstruction(cg, TR::InstOpCode::BCR, node, TR::InstOpCode::COND_BCR,
            cg->getEntryPointRealRegister(), cursor);

        int32_t padSize = CalcCodeSize(samplingRecompileMethodSnippetLabelInstruction, cursor)
        ...
        const int32_t offsetFromEPRegisterValueToSamplingRecompileMethod
            = CalcCodeSize(basrInstruction->getNext(), cursor);
        ...
        TR::S390EncodingRelocation *encodingRelocation
            = new (cg->trHeapMemory()) TR::S390EncodingRelocation(TR_AbsoluteHelperAddress, helperSymRef);
        ...
        if (comp->target().is64Bit()) {
            cursor = generateDataConstantInstruction(cg, TR::InstOpCode::dd, node,
                UPPER_4_BYTES(samplingRecompileMethodAddress), cursor);
            cursor->setEncodingRelocation(encodingRelocation);

            cursor = generateDataConstantInstruction(cg, TR::InstOpCode::dd, node,
                LOWER_4_BYTES(samplingRecompileMethodAddress), cursor);
        } else {
            cursor
                = generateDataConstantInstruction(cg, TR::InstOpCode::dd, node, samplingRecompileMethodAddress, cursor);
            cursor->setEncodingRelocation(encodingRelocation);
        }
        ...
        cursor = generateS390BranchInstruction(cg, TR::InstOpCode::BRC, TR::InstOpCode::COND_BRC, node,
            samplingRecompileMethodSnippetLabel, cursor);
    }
    ...
    TR::S390EncodingRelocation *encodingRelocation
        = new (cg->trHeapMemory()) TR::S390EncodingRelocation(TR_BodyInfoAddress, NULL);
 ...
 */
//-----------------------------------------------------------------------
TR::Instruction *TR_S390Recompilation::generatePrologue(TR::Instruction *cursor)
{
    ...
    cursor = generateRIInstruction(cg, TR::InstOpCode::getAddHalfWordImmOpCode(), node,
        cg->getStackPointerRealRegister(), -4 * cg->machine()->getGPRSize(), cursor);

    TR::MemoryReference *spillSlotMemRef
        = generateS390MemoryReference(cg->getStackPointerRealRegister(), cg->machine()->getGPRSize(), cg);

    // Spill GPR1 and GPR2
    cursor = generateRSInstruction(cg, TR::InstOpCode::getStoreMultipleOpCode(), node,
        cg->machine()->getRealRegister(TR::RealRegister::GPR1), cg->machine()->getRealRegister(TR::RealRegister::GPR2),
        spillSlotMemRef, cursor);
    ...
    cursor = generateRRInstruction(cg, TR::InstOpCode::getLoadRegOpCode(), node,
        cg->machine()->getRealRegister(TR::RealRegister::GPR1), cg->getEntryPointRealRegister(), cursor);
    ...
    cursor = generateRIInstruction(cg, TR::InstOpCode::getAddHalfWordImmOpCode(), node, cg->getEntryPointRealRegister(),
        offsetToIntEP, cursor);

    // Use a negative offset since pre-prologue is generated before the prologue
    int32_t offsetToBodyInfo = -CalcCodeSize(bodyInfoDataConstant, basrInstruction);

    TR::MemoryReference *bodyInfoAddressMemRef
        = generateS390MemoryReference(cg->machine()->getRealRegister(TR::RealRegister::GPR1), offsetToBodyInfo, cg);
    ...
    cursor = generateRXInstruction(cg, TR::InstOpCode::getExtendedLoadOpCode(), node,
        cg->machine()->getRealRegister(TR::RealRegister::GPR1), bodyInfoAddressMemRef, cursor);

    TR::MemoryReference *counterFieldMemRef = generateS390MemoryReference(
        cg->machine()->getRealRegister(TR::RealRegister::GPR1), offsetof(TR_PersistentJittedBodyInfo, _counter), cg);

    // Load the counter field value from the body info pointer
    cursor = generateRXInstruction(cg, TR::InstOpCode::L, node, cg->machine()->getRealRegister(TR::RealRegister::GPR2),
        counterFieldMemRef, cursor);

    // Compare the counter value to zero
    cursor
        = generateRRInstruction(cg, TR::InstOpCode::LTR, node, cg->machine()->getRealRegister(TR::RealRegister::GPR2),
            cg->machine()->getRealRegister(TR::RealRegister::GPR2), cursor);
    ...
    cursor = generateS390BranchInstruction(cg, TR::InstOpCode::BRC, TR::InstOpCode::COND_BNL, node,
        resumeMethodExecutionLabel, cursor);

    // Load GPR2 with the address of the next instruction
    cursor
        = generateRRInstruction(cg, TR::InstOpCode::BASR, node, cg->machine()->getRealRegister(TR::RealRegister::GPR2),
            cg->machine()->getRealRegister(TR::RealRegister::GPR0), cursor);
    ...
    cursor = generateRIInstruction(cg, TR::InstOpCode::getAddHalfWordImmOpCode(), node,
        cg->machine()->getRealRegister(TR::RealRegister::GPR2), 0, cursor);

    TR::S390RIInstruction *addImmediateToEndOfPrologueInstruction = static_cast<TR::S390RIInstruction *>(cursor);

    TR::MemoryReference *endOfPrologueSlotMemRef
        = generateS390MemoryReference(cg->getStackPointerRealRegister(), 0, cg);

    // Store GPR2 off to the stack so countingRecompileMethod can access it
    cursor = generateRXInstruction(cg, TR::InstOpCode::getStoreOpCode(), node,
        cg->machine()->getRealRegister(TR::RealRegister::GPR2), endOfPrologueSlotMemRef, cursor);

    // Adjust GPR2 to point to the countingRecompileMethod address
    cursor = generateRIInstruction(cg, TR::InstOpCode::getAddHalfWordImmOpCode(), node,
        cg->machine()->getRealRegister(TR::RealRegister::GPR2), 0, cursor);

    TR::S390RIInstruction *addImmediateToCountingRecompileMethodAddressInstruction
        = static_cast<TR::S390RIInstruction *>(cursor);

    TR::MemoryReference *countingRecompileMethodAddressMemRef
        = generateS390MemoryReference(cg->machine()->getRealRegister(TR::RealRegister::GPR2), 0, cg);

    // Load the countingRecompileMethod address
    cursor = generateRXInstruction(cg, TR::InstOpCode::getLoadOpCode(), node,
        cg->machine()->getRealRegister(TR::RealRegister::GPR2), countingRecompileMethodAddressMemRef, cursor);
    ...
    TR::S390EncodingRelocation *encodingRelocation
        = new (cg->trHeapMemory()) TR::S390EncodingRelocation(TR_AbsoluteHelperAddress, NULL);

    const intptr_t countingRecompileMethodAddress = reinterpret_cast<intptr_t>(helperSymRef->getMethodAddress());

    if (comp->target().is64Bit()) {
        cursor = generateDataConstantInstruction(cg, TR::InstOpCode::dd, node,
            UPPER_4_BYTES(countingRecompileMethodAddress), cursor);
        cursor->setEncodingRelocation(encodingRelocation);

        countingRecompileMethodAddressDataConstant = cursor;

        cursor = generateDataConstantInstruction(cg, TR::InstOpCode::dd, node,
            LOWER_4_BYTES(countingRecompileMethodAddress), cursor);
    } else {
    ...
    cursor = generateS390LabelInstruction(cg, TR::InstOpCode::label, node,
        patchCallerBranchToCountingRecompiledMethodLabel, cursor);
    ...
    cursor = generateRRInstruction(cg, TR::InstOpCode::BASR, node, cg->getReturnAddressRealRegister(),
        cg->machine()->getRealRegister(TR::RealRegister::GPR0), cursor);

    basrInstruction = cursor;

    TR::MemoryReference *countingPatchCallSiteAddressMemRef
        = generateS390MemoryReference(cg->getReturnAddressRealRegister(), 0, cg);

    // Load the countingPatchCallSite address
    cursor = generateRXInstruction(cg, TR::InstOpCode::getLoadOpCode(), node, cg->getEntryPointRealRegister(),
        countingPatchCallSiteAddressMemRef, cursor);
    ...
    cursor = generateRIInstruction(cg, TR::InstOpCode::getAddHalfWordImmOpCode(), node,
        cg->getReturnAddressRealRegister(), offsetToIntEP, cursor);

    TR::MemoryReference *intEPAddressSlotMemRef
        = generateS390MemoryReference(cg->getStackPointerRealRegister(), cg->machine()->getGPRSize(), cg);
    ...
    cursor = generateRXInstruction(cg, TR::InstOpCode::getStoreOpCode(), node, cg->getReturnAddressRealRegister(),
        intEPAddressSlotMemRef, cursor);
    ...
    cursor = generateRIInstruction(cg, TR::InstOpCode::getAddHalfWordImmOpCode(), node,
        cg->getReturnAddressRealRegister(), offsetToBodyInfo, cursor);

    bodyInfoAddressMemRef = generateS390MemoryReference(cg->getReturnAddressRealRegister(), 0, cg);

    // Load the body info address
    cursor = generateRXInstruction(cg, TR::InstOpCode::getLoadOpCode(), node, cg->getReturnAddressRealRegister(),
        bodyInfoAddressMemRef, cursor);
    ...
    if (comp->target().is64Bit()) {
        cursor = generateDataConstantInstruction(cg, TR::InstOpCode::dd, node,
            UPPER_4_BYTES(countingPatchCallSiteAddress), cursor);
        cursor->setEncodingRelocation(encodingRelocation);

        countingPatchCallSiteAddressDataConstant = cursor;

        cursor = generateDataConstantInstruction(cg, TR::InstOpCode::dd, node,
            LOWER_4_BYTES(countingPatchCallSiteAddress), cursor);
    } else {
    ...
    // Fill GPR1 and GPR2
    cursor = generateRSInstruction(cg, TR::InstOpCode::getLoadMultipleOpCode(), node,
        cg->machine()->getRealRegister(TR::RealRegister::GPR1), cg->machine()->getRealRegister(TR::RealRegister::GPR2),
        spillSlotMemRef, cursor);
    ...
    int32_t offsetToEndOfPrologue
        = CalcCodeSize(basrForCountingRecompileInstruction->getNext(), endOfPrologueLabel->getInstruction());

    addImmediateToEndOfPrologueInstruction->setSourceImmediate(offsetToEndOfPrologue);

    int32_t offsetToCountingRecompileMethodAddress
        = -CalcCodeSize(countingRecompileMethodAddressDataConstant, endOfPrologueLabel->getInstruction());

    addImmediateToCountingRecompileMethodAddressInstruction->setSourceImmediate(offsetToCountingRecompileMethodAddress);

    int32_t offsetToCountingPatchCallSiteAddress
        = CalcCodeSize(basrInstruction->getNext(), countingPatchCallSiteAddressDataConstant->getPrev());
