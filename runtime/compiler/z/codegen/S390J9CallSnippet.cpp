...
uint8_t *TR::S390J9CallSnippet::generateVIThunk(TR::Node *callNode, int32_t argSize, TR::CodeGenerator *cg)
{
...
TR_MHJ2IThunk *TR::S390J9CallSnippet::generateInvokeExactJ2IThunk(TR::Node *callNode, int32_t argSize, char *signature,
    TR::CodeGenerator *cg)
{
    uint32_t rEP = (uint32_t)cg->getEntryPointRegister() - 1;
    TR::Compilation *comp = cg->comp();
        ...
        *(int16_t *)cursor = 0xC0F4; // BRCL   <Helper Addr>
...
#endif
        {
            if (cg->directCallRequiresTrampoline(destAddr, reinterpret_cast<intptr_t>(cursor)))
                destAddr = TR::CodeCacheManager::instance()->findHelperTrampoline(helper->getReferenceNumber(),
                    (void *)cursor);
        }
#endif
    ...
    *(uintptr_t *)cursor
        = (uintptr_t)cg->fej9()->getInvokeExactThunkHelperAddress(comp, dispatcherSymbol, callNode->getDataType());
    cursor += sizeof(uintptr_t);

    diagnostic("\n-- ( Created invokeExact J2I thunk " POINTER_PRINTF_FORMAT " for node " POINTER_PRINTF_FORMAT " )",
        thunk, callNode);

    TR_ASSERT(cursor == thunk->entryPoint() + codeSize,
        "Must allocate correct amount of memory for invokeExact J2I thunk %p.  Allocated %d bytes but consumed %d = %d "
        "plus argument spills",
        thunk->entryPoint(), codeSize, cursor - thunk->entryPoint(), lengthOfIEThunk);

    if (verbose)
        TR_VerboseLog::writeLineLocked(TR_Vlog_J2I, "Created J2I thunk %s @ %p while compiling %s",
            thunk->terseSignature(), thunk->entryPoint(), comp->signature());
...
uint8_t *TR::S390J9CallSnippet::emitSnippetBody()
{
    ...
    int32_t pad_bytes
        = (dataStartAddr + (sizeof(uintptr_t) - 1)) / sizeof(uintptr_t) * sizeof(uintptr_t) - dataStartAddr;
    ...
    pad_bytes
        = (((uintptr_t)cursor + (sizeof(uintptr_t) - 1)) / sizeof(uintptr_t) * sizeof(uintptr_t) - (uintptr_t)cursor);
    ...
    *(uintptr_t *)cursor = (uintptr_t)glueRef->getMethodAddress();
    cg()->addExternalRelocation(
        TR::ExternalRelocation::create(cursor, (uint8_t *)glueRef, TR_AbsoluteHelperAddress, cg()), __FILE__, __LINE__,
        callNode);
    cursor += sizeof(uintptr_t);

    // Store the code cache RA
    *(uintptr_t *)cursor = (uintptr_t)getCallRA();
    cg()->addExternalRelocation(TR::ExternalRelocation::create(cursor, NULL, TR_AbsoluteMethodAddress, cg()), __FILE__,
        __LINE__, callNode);
    cursor += sizeof(uintptr_t);
        ...
        if (methodSymRef->isUnresolved()
            ...
            *(uintptr_t *)cursor = ramMethod;
            ...
            if (comp->getOption(TR_UseSymbolValidationManager)) {
                ...
                cg()->addExternalRelocation(TR::ExternalRelocation::create(cursor, (uint8_t *)ramMethod,
                                                (uint8_t *)TR::SymbolType::typeMethod, TR_SymbolFromManager, cg()),
                    __FILE__, __LINE__, callNode);
            }
#if defined(J9VM_OPT_JITSERVER)
            else if (!comp->isOutOfProcessCompilation()) // Since we query this information from the client, remote
                                                         // compilations don't need to add relocation records for
                                                         // TR_MethodObject
#else
            else
            ...
            {
                cg()->addExternalRelocation(
                    TR::ExternalRelocation::create(cursor, (uint8_t *)callNode->getSymbolReference(),
...
TR_RuntimeHelper TR::S390J9CallSnippet::getInterpretedDispatchHelper(TR::SymbolReference *methodSymRef,
    TR::DataType type)
{
    TR::Compilation *comp = cg()->comp();
    TR::MethodSymbol *methodSymbol = methodSymRef->getSymbol()->castToMethodSymbol();
...
uint32_t TR::S390J9CallSnippet::getPICBinaryLength()
{
...
uint8_t *TR::S390J9CallSnippet::generatePICBinary(uint8_t *cursor, TR::SymbolReference *glueRef)
{
    // Branch to the dispatcher.
        ...
        *(int16_t *)cursor = 0xC0E5;
...
#endif
        {
            if (cg()->directCallRequiresTrampoline(destAddr, instructionStartAddress)) {
                ...
                // trampoline.
                destAddr = TR::CodeCacheManager::instance()->findHelperTrampoline(glueRef->getReferenceNumber(),
                    (void *)cursor);
        ...
        TR_ASSERT_FATAL(
            cg()->comp()->target().cpu.isTargetWithinBranchRelativeRILRange(destAddr, instructionStartAddress),
...
void TR::S390J9CallSnippet::print(OMR::Logger *log, TR_Debug *debug)
{
    ...
    if (getKind() == TR::Snippet::IsUnresolvedCall) {
        ...
        log->printf("LARL \tGPR14, *+%d <%p>\t# Start of Data Const.", 8 + 6 + padbytes, bufferPos + 8 + 6 + padbytes);
        bufferPos += 6;
        debug->printPrefix(log, NULL, bufferPos, 6);
...
uint8_t *TR::S390UnresolvedCallSnippet::emitSnippetBody()
{
    TR::Compilation *comp = cg()->comp();
    TR_J9VMBase *fej9 = (TR_J9VMBase *)(comp->fe());
    ...
    *(uintptr_t *)cursor = (uintptr_t)methodSymRef->getOwningMethod(comp)->constantPool();
    ...
    {
        cg()->addExternalRelocation(
            TR::ExternalRelocation::create(cursor, *(uint8_t **)cursor,
                getNode() ? (uint8_t *)(intptr_t)getNode()->getInlinedSiteIndex() : (uint8_t *)-1, TR_Trampolines,
                cg()),
            __FILE__, __LINE__, getNode());
    }
#if defined(J9ZOS390)
    else {
        cg()->addExternalRelocation(TR::ExternalRelocation::create(cursor, *(uint8_t **)cursor,
    ...
    *(uint32_t *)cursor = (helperLookupOffset << 24) | methodSymRef->getCPIndexForVM();
...
#endif
    {
        if ((comp->compileRelocatableCode() || comp->isOutOfProcessCompilation())
            && comp->getOption(TR_TraceRelocatableDataDetailsCG)) {
...
uint32_t TR::S390UnresolvedCallSnippet::getLength(int32_t estimatedSnippetStart)
{
    return TR::S390J9CallSnippet::getLength(estimatedSnippetStart) + sizeof(uintptr_t) + sizeof(int32_t);
}

uint8_t *TR::S390VirtualSnippet::emitSnippetBody() { return NULL; }

uint32_t TR::S390VirtualSnippet::getLength(int32_t estimatedSnippetStart) { return 0; }

uint8_t *TR::S390VirtualUnresolvedSnippet::emitSnippetBody()
{
    uint8_t *cursor = cg()->getBinaryBufferCursor();
    TR::Node *callNode = getNode();
    TR::Compilation *comp = cg()->comp();
    ...
    *(uintptr_t *)cursor = (uintptr_t)callNode->getSymbolReference()->getOwningMethod(comp)->constantPool();
    ...
    auto j2iRelocInfo = reinterpret_cast<TR_RelocationRecordInformation *>(
        comp->trMemory()->allocateMemory(sizeof(TR_RelocationRecordInformation), heapAlloc));
    ...
    *(uintptr_t *)cursor = 0;
    ...
    // Field used by nestmate private calls
    // J2I thunk address
    // No explicit call to `addExternalRelocation` because its relocation info is passed to CP_addr
    // `addExternalRelocation` call.
    *(uintptr_t *)cursor = (uintptr_t)thunkAddress;
    ...
    cg()->addExternalRelocation(TR::ExternalRelocation::create((uint8_t *)cpAddrPosition, (uint8_t *)j2iRelocInfo, NULL,
    ...
    TR_ASSERT_FATAL(getIndirectCallInstruction()
            && getIndirectCallInstruction()->getOpCodeValue() == TR::InstOpCode::BASR,
        "Unexpected branch instruction in VirtualUnresolvedSnippet.\n");

    *(uintptr_t *)cursor = (uintptr_t)(getIndirectCallInstruction()->getBinaryEncoding()
        + getIndirectCallInstruction()->getBinaryLength());
    cg()->addExternalRelocation(TR::ExternalRelocation::create(cursor, NULL, TR_AbsoluteMethodAddress, cg()), __FILE__,
        __LINE__, callNode);
...
uint32_t TR::S390VirtualUnresolvedSnippet::getLength(int32_t estimatedSnippetStart)
{
    TR::Compilation *comp = cg()->comp();
...
TR::S390InterfaceCallSnippet::S390InterfaceCallSnippet(TR::CodeGenerator *cg, TR::Node *c, TR::LabelSymbol *lab,
    int32_t s, int8_t n, void *thunkPtr, bool useCLFIandBRCL)
...
uint8_t *TR::S390InterfaceCallSnippet::emitSnippetBody()
{
    uint8_t *cursor = cg()->getBinaryBufferCursor();
    getSnippetLabel()->setCodeLocation(cursor);
    TR::Compilation *comp = cg()->comp();
    ...
    cg()->addRelocation(new (cg()->trHeapMemory())
            TR::LabelRelative32BitRelocation(cursor, getDataConstantSnippet()->getSnippetLabel()));
    ...
    *(int16_t *)cursor = 0xC0F4;
...
#endif
    {
        if (cg()->directCallRequiresTrampoline(destAddr, reinterpret_cast<intptr_t>(cursor))) {
            ...
            // trampoline.
            destAddr
                = TR::CodeCacheManager::instance()->findHelperTrampoline(glueRef->getReferenceNumber(), (void *)cursor);
    ...
    *(int32_t *)cursor = (int32_t)((destAddr - (intptr_t)(cursor - 2)) / 2);
    cg()->addExternalRelocation(TR::ExternalRelocation::create(cursor, (uint8_t *)glueRef, TR_HelperAddress, cg()),
        __FILE__, __LINE__, getNode());
    cursor += sizeof(int32_t);
...
uint32_t TR::S390InterfaceCallSnippet::getLength(int32_t estimatedSnippetStart)
{
    return 12; // LARL + BRCL
}

void TR_Debug::print(OMR::Logger *log, TR::S390UnresolvedCallSnippet *snippet)
{
    uint8_t *bufferPos = snippet->getSnippetLabel()->getCodeLocation() + snippet->getLength(0) - sizeof(intptr_t)
        - sizeof(int32_t) // 2 DC's at end of this snippet.
...
void TR_Debug::print(OMR::Logger *log, TR::S390VirtualSnippet *snippet)
{
    uint8_t *bufferPos = snippet->getSnippetLabel()->getCodeLocation();
    printSnippetLabel(log, snippet->getSnippetLabel(), bufferPos, "Virtual Call Snippet");
}

void TR_Debug::print(OMR::Logger *log, TR::S390VirtualUnresolvedSnippet *snippet)
{
    uint8_t *bufferPos = snippet->getSnippetLabel()->getCodeLocation();
    TR::SymbolReference *methodSymRef = snippet->getNode()->getSymbolReference();
    ...
    printPrefix(log, NULL, bufferPos, sizeof(intptr_t));
    log->printf("DC   \t%p \t\t# Instruction to be patched with vft offset",
        snippet->getPatchVftInstruction()->getBinaryEncoding());
    bufferPos += sizeof(intptr_t);
    ...
    printPrefix(log, NULL, bufferPos, sizeof(intptr_t));
    log->printf("DC   \t%p \t\t# RA for private",
        snippet->getBranchInstruction()->getBinaryEncoding()
            + snippet->getIndirectCallInstruction()->getBinaryLength());
}

void TR_Debug::print(OMR::Logger *log, TR::S390InterfaceCallSnippet *snippet)
{
    uint8_t *bufferPos = snippet->getSnippetLabel()->getCodeLocation();
    TR::SymbolReference *methodSymRef = snippet->getNode()->getSymbolReference();
    ...
    printPrefix(log, NULL, bufferPos, 6);
    log->printf("BRCL \t<%p>\t\t# Branch to Helper %s", snippet->getSnippetDestAddr(),
        snippet->usedTrampoline() ? "- Trampoline Used." : "");
    bufferPos += 6;
}

void TR_Debug::print(OMR::Logger *log, TR::J9S390InterfaceCallDataSnippet *snippet)
{
    uint8_t *bufferPos = snippet->getSnippetLabel()->getCodeLocation();
...
///////////////////////////////////////////////////////////////////////////////
TR::J9S390InterfaceCallDataSnippet::J9S390InterfaceCallDataSnippet(TR::CodeGenerator *cg, TR::Node *node, uint8_t n,
    void *thunkPtr, bool useCLFIandBRCL)
 ...
 */
uint8_t *TR::J9S390InterfaceCallDataSnippet::emitSnippetBody()
{
    /*
    ...
    *(intptr_t *)cursor = 0;
    ...
    // J2I thunk address.
    // Note that J2I thunk relocation is associated with CP addr and the call to `addExternalRelocation` uses CP-Addr
    // cursor.
    *(intptr_t *)cursor = (intptr_t)_thunkAddress;
    ...
    if (!isSingleDynamic) {
        ...
        *(intptr_t *)(cursor) = snippetStart + getFirstSlotOffset() - (2 * TR::Compiler->om.sizeofReferenceAddress());
        cg()->addExternalRelocation(TR::ExternalRelocation::create(cursor, NULL, TR_AbsoluteMethodAddress, cg()),
            __FILE__, __LINE__, callNode);
    ...
    // Skip the top cache slots that are filled with IProfiler data by setting the cursorlastCachedSlot to point to the
    // fist dynamic cache slot
    if (updateField) {
        *(intptr_t *)(cursorlastCachedSlot) = snippetStart + getFirstSlotOffset()
            + ((getNumInterfaceCallCacheSlots() - numInterfaceCallCacheSlots - 1) * 2
...
uint32_t TR::J9S390InterfaceCallDataSnippet::getCallReturnAddressOffset() { return 0; }

uint32_t TR::J9S390InterfaceCallDataSnippet::getSingleDynamicSlotOffset()
{
    return getCallReturnAddressOffset() + 6 * TR::Compiler->om.sizeofReferenceAddress();
}

uint32_t TR::J9S390InterfaceCallDataSnippet::getLastCachedSlotFieldOffset()
{
    return getCallReturnAddressOffset() + 7 * TR::Compiler->om.sizeofReferenceAddress();
}

uint32_t TR::J9S390InterfaceCallDataSnippet::getFirstSlotFieldOffset()
{
    return getLastCachedSlotFieldOffset() + TR::Compiler->om.sizeofReferenceAddress();
}

uint32_t TR::J9S390InterfaceCallDataSnippet::getLastSlotFieldOffset()
{
    return getFirstSlotFieldOffset() + TR::Compiler->om.sizeofReferenceAddress();
}

uint32_t TR::J9S390InterfaceCallDataSnippet::getFirstSlotOffset()
{
    return getLastSlotFieldOffset() + TR::Compiler->om.sizeofReferenceAddress();
}

uint32_t TR::J9S390InterfaceCallDataSnippet::getLastSlotOffset()
{
...
uint32_t TR::J9S390InterfaceCallDataSnippet::getLength(int32_t)
{
    TR::Compilation *comp = cg()->comp();
    ...
    if (comp->getOption(TR_enableInterfaceCallCachingSingleDynamicSlot))
        return 3 * TR::Compiler->om.sizeofReferenceAddress() + getSingleDynamicSlotOffset()
            + 4 * TR::Compiler->om.sizeofReferenceAddress();

    if (getNumInterfaceCallCacheSlots() > 0)
        return 3 * TR::Compiler->om.sizeofReferenceAddress() + getLastSlotOffset()
            + TR::Compiler->om.sizeofReferenceAddress();
...
uint32_t TR::S390JNICallDataSnippet::getJNICallOutFrameFlagsOffset()
{
    return TR::Compiler->om.sizeofReferenceAddress();
}

uint32_t TR::S390JNICallDataSnippet::getReturnFromJNICallOffset()
{
    return 2 * TR::Compiler->om.sizeofReferenceAddress();
}

uint32_t TR::S390JNICallDataSnippet::getSavedPCOffset() { return 3 * TR::Compiler->om.sizeofReferenceAddress(); }

uint32_t TR::S390JNICallDataSnippet::getTagBitsOffset() { return 4 * TR::Compiler->om.sizeofReferenceAddress(); }

uint32_t TR::S390JNICallDataSnippet::getPCOffset() { return 5 * TR::Compiler->om.sizeofReferenceAddress(); }

uint32_t TR::S390JNICallDataSnippet::getLiteralsOffset() { return 6 * TR::Compiler->om.sizeofReferenceAddress(); }

uint32_t TR::S390JNICallDataSnippet::getJitStackFrameFlagsOffset()
{
    return 7 * TR::Compiler->om.sizeofReferenceAddress();
}

uint32_t TR::S390JNICallDataSnippet::getConstReleaseVMAccessMaskOffset()
{
    return 8 * TR::Compiler->om.sizeofReferenceAddress();
}

uint32_t TR::S390JNICallDataSnippet::getConstReleaseVMAccessOutOfLineMaskOffset()
{
    return 9 * TR::Compiler->om.sizeofReferenceAddress();
}

uint32_t TR::S390JNICallDataSnippet::getTargetAddressOffset() { return 10 * TR::Compiler->om.sizeofReferenceAddress(); }

uint32_t TR::S390JNICallDataSnippet::getLength(int32_t estimatedSnippetStart)
{
    return 12 * TR::Compiler->om.sizeofReferenceAddress(); /*one ptr more for possible padding */
}

TR::S390JNICallDataSnippet::S390JNICallDataSnippet(TR::CodeGenerator *cg, TR::Node *node)
    : TR::S390ConstantDataSnippet(cg, node, generateLabelSymbol(cg), 0)
    , _baseRegister(0)
    , _ramMethod(0)
...
uint8_t *TR::S390JNICallDataSnippet::emitSnippetBody()
{
    uint8_t *cursor = cg()->getBinaryBufferCursor();
    ...
    *(intptr_t *)cursor = (intptr_t)_ramMethod;
    ...
    if (cg()->needClassAndMethodPointerRelocations()) {
        cg()->addExternalRelocation(TR::ExternalRelocation::create(cursor, (uint8_t *)callNode->getSymbolReference(),
    ...
    *(intptr_t *)cursor = (intptr_t)_targetAddress;
    TR_OpaqueMethodBlock *method
        = getNode()->getSymbol()->castToResolvedMethodSymbol()->getResolvedMethod()->getPersistentIdentifier();
    ...
    if (comp->isOutOfProcessCompilation()) {
        ...
        SerializedRuntimeAssumption *sar = new (cg()->trHeapMemory())
    ...
    {
        TR_PatchJNICallSite::make(cg()->fe(), cg()->trPersistentMemory(), (uintptr_t)method, cursor,
    ...
    if (cg()->needClassAndMethodPointerRelocations()) {
        ...
        cg()->addExternalRelocation(
            TR::ExternalRelocation::create(cursor, reinterpret_cast<uint8_t *>(info), reloType, cg()), __FILE__,
            __LINE__, callNode);
    }
...
void TR::S390JNICallDataSnippet::print(OMR::Logger *log, TR_Debug *debug)
{
    /*
           ramMethod
           JNICallOutFrameFlags
...
uint8_t *TR::S390J9HelperCallSnippet::emitSnippetBody()
{
    ...
    /* The J9Method pointer to be passed to be interpreter is in the entry point register, but the interpreter expects it to be in R1.
     * Since the first integer argument register is also R1, we only load it after we have flushed the args to the
     * stack.
     */
    uint32_t rEP = (uint32_t)cg()->getEntryPointRegister() - 1;
