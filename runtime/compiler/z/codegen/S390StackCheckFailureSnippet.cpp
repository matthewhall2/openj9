...
TR::S390StackCheckFailureSnippet::S390StackCheckFailureSnippet(TR::CodeGenerator *cg, TR::Node *node,
    TR::LabelSymbol *restartlab, TR::LabelSymbol *snippetlab, TR::SymbolReference *helper, uint32_t frameSize)
...
uint8_t *TR::S390StackCheckFailureSnippet::emitSnippetBody()
{
    ...
    {
        *(int32_t *)cursor
            = 0xa7050000 + (rEP << 20) + (2 + TR::Compiler->om.sizeofReferenceAddress() / 2); // BRAS     rEP, 4/6
        cursor += sizeof(int32_t);
    ...
    *(int32_t *)cursor = (int32_t)((destAddr - (intptr_t)(cursor - 2)) / 2);
    cg()->addProjectSpecializedRelocation(cursor, (uint8_t *)getDestination(), NULL, TR_HelperAddress, __FILE__,
        __LINE__, getNode());
...
void TR::S390StackCheckFailureSnippet::setStackAtlasHelper()
{
    TR::Linkage *linkage = cg()->getS390Linkage();
    TR::Compilation *comp = cg()->comp();
    ...
    if (atlas) {
        ...
        while (paramCursor != NULL) {
            ...
            if (intRegArgIndex >= 0 && (paramCursor->isReferencedParameter() || comp->getOption(TR_FullSpeedDebug))
                ...
                map->setRegisterBits(
                    cg()->registerBitMask((int32_t)linkage->getIntegerArgumentRegister(intRegArgIndex)));
            }
...
uint32_t TR::S390StackCheckFailureSnippet::getLength(int32_t)
{
    int32_t size = 0;
...
void TR_Debug::print(OMR::Logger *log, TR::S390StackCheckFailureSnippet *snippet)
{
    ...
    printPrefix(log, NULL, bufferPos, 6);
    log->printf("BRCL \t<%p>\t# Return to Main Code", snippet->getReStartLabel()->getCodeLocation());
}
