...
J9::Z::UnresolvedDataSnippet::UnresolvedDataSnippet(TR::CodeGenerator *cg, TR::Node *node, TR::SymbolReference *symRef,
...
uint8_t *J9::Z::UnresolvedDataSnippet::getAddressOfDataReference()
{
    return (uint8_t *)(_branchInstruction->getNext())->getBinaryEncoding();
}

TR::Instruction *J9::Z::UnresolvedDataSnippet::setDataReferenceInstruction(TR::Instruction *i)
{
...
uint8_t *J9::Z::UnresolvedDataSnippet::emitSnippetBody()
{
    ...
    if (getDataSymbol()->getShadowSymbol() != NULL) {
        if (resolveForStore()) {
            ...
            TR_ASSERT_FATAL_WITH_INSTRUCTION(getDataReferenceInstruction(), _fenceNOPInst != NULL,
                "Unresolved store must have a fence NOP instruction");
        } else {
    ...
    *(uintptr_t *)cursor = (uintptr_t)glueRef->getMethodAddress();
    cg()->addProjectSpecializedRelocation(cursor, (uint8_t *)glueRef, NULL, TR_AbsoluteHelperAddress, __FILE__,
        __LINE__, getNode());
    ...
    *(uintptr_t *)cursor = (uintptr_t)(getBranchInstruction()->getNext())->getBinaryEncoding();
    ...
    if (getDataSymbolReference()->getSymbol()->isCallSiteTableEntry()) {
        *(int32_t *)cursor
            = getDataSymbolReference()->getSymbol()->castToCallSiteTableEntrySymbol()->getCallSiteIndex();
    } else if (getDataSymbol()->isMethodTypeTableEntry()) {
        *(int32_t *)cursor
            = getDataSymbolReference()->getSymbol()->castToMethodTypeTableEntrySymbol()->getMethodTypeIndex();
    ...
    *(uintptr_t *)cursor = 0x0;
    ...
    if (isInstanceData()) {
        ...
        TR::RealRegister::RegNum base
            = (mr->getBaseRegister() ? toRealRegister(mr->getBaseRegister())->getRegisterNumber()
        ...
        *(int32_t *)cursor
            = (int32_t)((cursor + sizeof(int32_t) - getBranchInstruction()->getNext()->getBinaryEncoding() + 6) / 2);
        ...
        *(uintptr_t *)offsetMarker = (uintptr_t)cursor;
        cg()->addProjectSpecializedRelocation(offsetMarker, NULL, NULL, TR_AbsoluteMethodAddress, __FILE__, __LINE__,
            getNode());
...
uint32_t J9::Z::UnresolvedDataSnippet::getLength(int32_t estimatedSnippetStart)
{
    TR::Compilation *comp = cg()->comp();
...
void TR_Debug::print(OMR::Logger *log, TR::UnresolvedDataSnippet *snippet)
{
    ...
    printPrefix(log, NULL, bufferPos, sizeof(intptr_t));
    log->printf("DC   \t0x%p \t# Address Of Literal Pool Slot", (intptr_t)(snippet->getLiteralPoolSlot()));
    bufferPos += sizeof(intptr_t);

    // Snippet has out-of-line sequence for large offsets for instance data
    if (snippet->isInstanceData()) {
        printPrefix(log, NULL, bufferPos, sizeof(intptr_t));
        log->printf("DC   \t0x%p \t# Address to large offset slot", (intptr_t)((intptr_t *)bufferPos));
        bufferPos += sizeof(intptr_t);

        printPrefix(log, NULL, bufferPos, sizeof(int32_t));
        log->printf("DC   \t0x%p \t# Displacement from helper branch to out-of-line sequence",
            (int32_t)((intptr_t)bufferPos + sizeof(int32_t)
                - (intptr_t)snippet->getBranchInstruction()->getBinaryEncoding() + 6));
        ...
        printPrefix(log, NULL, bufferPos, sizeof(int32_t));
        log->printf("DC   \t0x%p \t# Offset slot", (int32_t)(*(int32_t *)bufferPos));
        bufferPos += sizeof(int32_t);
    }
}

bool J9::Z::UnresolvedDataSnippet::isInstanceData() { return getDataSymbol()->getShadowSymbol() != NULL; }

TR::S390WritableDataSnippet *J9::Z::UnresolvedDataSnippet::createUnresolvedData(TR::CodeGenerator *cg, TR::Node *n)
{
