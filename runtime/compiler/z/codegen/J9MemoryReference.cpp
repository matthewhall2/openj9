...
void recursivelyIncrementReferenceCount(TR::Node *node, rcount_t increment, List<TR::Node> &incrementedNodesList,
    List<TR::Node> &nodesAlreadyEvaluatedBeforeFoldingList, TR::CodeGenerator *cg)
{
    ...
    if (node->getRegister() && node->getRegister()->getOpaquePseudoRegister()) {
        ...
        TR_ASSERT(node->getOpCodeValue() == TR::loadaddr,
            "temporary symbol references should only be attached to loadaddr nodes\n");
        ...
        logprintf(trace, log, "\tincrement temporary #%d (sym %p -- from loadaddr node %p) reference count %d->%d\n",
            node->getSymbolReference()->getReferenceNumber(), sym, node, sym->getReferenceCount(),
            sym->getReferenceCount() + 1);
    ...
    if (node->getRegister() == NULL) {
        ...
        for (int32_t childCount = node->getNumChildren() - 1; childCount >= 0; childCount--)
            recursivelyIncrementReferenceCount(node->getChild(childCount), increment, incrementedNodesList,
                nodesAlreadyEvaluatedBeforeFoldingList, cg);
    } else {
...
void J9::Z::MemoryReference::addInstrSpecificRelocation(TR::CodeGenerator *cg, TR::Instruction *instr, int32_t disp,
    uint8_t *cursor)
{}

bool J9::Z::MemoryReference::typeNeedsAlignment(TR::Node *node)
{
...
void J9::Z::MemoryReference::tryForceFolding(TR::Node *&rootLoadOrStore, TR::CodeGenerator *&cg,
    TR_StorageReference *&storageReference, TR::SymbolReference *&symRef, TR::Symbol *&symbol,
    List<TR::Node> &nodesAlreadyEvaluatedBeforeFoldingList)
{
    ...
    if (storageReference) {
        ...
        TR_ASSERT(!storageRefNode->getOpCode().isLoadConst(),
            "storageRefNode %s (%p) const should be BCD or Aggr type\n", storageRefNode->getOpCode().getName(),
            storageRefNode);

        TR_ASSERT(storageRefNode->getOpCode().isLoadVar() || storageRefNode->getOpCode().isStore(),
            "expecting storageRef node %p to be a loadVar or store\n", storageRefNode);
        ...
        logprintf(trace, log,
            "\t\tmr storageRef case: setting rootLoadOrStore from %s (%p) to storageRef->node %s (%p) "
            "(ref->nodeRefCount %d, symRef #%d (sym=%p), isIndirect %s, isConst %s)\n",
            rootLoadOrStore ? rootLoadOrStore->getOpCode().getName() : "NULL", rootLoadOrStore,
            storageRefNode ? storageRefNode->getOpCode().getName() : "NULL", storageRefNode,
            storageReference->getNodeReferenceCount(), _symbolReference->getReferenceNumber(), symbol,
            isIndirect ? "yes" : "no", "no");
        ...
        if (isIndirect) {
            if (storageReference->isNodeBasedHint()) {
                ...
                TR_ASSERT(storageReference->getNodeReferenceCount() >= 1,
                    "storageReference->getNodeReferenceCount() should be >=1 and not %d storageRefNode:[%p]\n",
        ...
        if (!storageReference->isNodeBasedHint()) {
            logprintf(trace, log, "\t\tdec nodeRefCount %d->%d on storageRef #%d (storageRefNode %s (%p))\n",
                ...
                storageReference->getReferenceNumber(), storageReference->getNode()->getOpCode().getName(),
                storageReference->getNode());
            storageReference->decrementNodeReferenceCount();
        }
        TR_ASSERT(rootLoadOrStore->getOpCode().isLoad() || rootLoadOrStore->getOpCode().isStore(),
            "rootLoadOrStore should be a load or store opcode\n");

        if (isImpliedMemoryReference) {
            // An addressHint is an implied reference to an address tree. The nodes in this tree must have their
            // references counts incremented to account for this implied use. Failing to reflect these implied uses
            // could allow an evaluator to incorrectly clobber a result register that is needed in a subsequent implied
            // or explicit use (i.e. an evaluator may perform a refCount==1 codegen optimization when it really should
            // not given the implied use). A drawback of incrementing the reference counts is that
            // populateMemoryReference will never try to fold an address tree into a 390 memory reference when the
            // refCount of the subTree is > 1. Depending on the simplicity of the tree (for example, if it is just a
            // simple add of a base+const) then it is probably still worthwhile attempting to fold the tree. The flag
            // forceFolding is set in these cases to force populateMemoryReference to attempt folding even when the
            // (now) higher refCounts would usually disallow it.
            TR::Node *addressChild = rootLoadOrStore->getFirstChild();

            self()->setForceFoldingIfAdvantageous(cg, addressChild);

            if (self()->forceFolding() || self()->forceFirstTimeFolding()
                || addressChild->getOpCodeValue() == TR::loadaddr) {
                logprintf(trace, log,
                    "\t\tisImpliedMemoryReference=true and %s so recInc refCounts for rootLoadOrStore %s (%p) and "
                    "addressChild %s (%p)\n",
                    self()->forceFirstTimeFolding()
                        ? "forceFirstTimeFolding"
                        : (self()->forceFolding() ? "forceFolding=true" : "addressChild is a loadaddr"),
                    rootLoadOrStore->getOpCode().getName(), rootLoadOrStore, addressChild->getOpCode().getName(),
                    addressChild);
                ...
                //    1. Any evaluators called for this tree will see accurate reference counts that include this
                //    implicit use so no
                //       incorrect codegen optimizations are performed.
                //    2. So any folding of the addressChild's children can be performed for this and future implicit
                //    uses and for the original use without
                //       underflowing the addressChild's children's referenceCounts (as each implied and actual use
                //       would reference the children each time when folding).
                recursivelyIncrementReferenceCount(addressChild, 1, _incrementedNodesList,
                    nodesAlreadyEvaluatedBeforeFoldingList, cg);
            } else {
                self()->setForceEvaluation();
                logprintf(trace, log,
                    "\t\tisImpliedMemoryReference=true and forceFolding=false so increment addressChild %s (%p) "
                    "refCount %d->%d and setForceEvaluation to true\n",
                    addressChild->getOpCode().getName(), addressChild, addressChild->getReferenceCount(),
                    addressChild->getReferenceCount() + 1);
...
TR::UnresolvedDataSnippet *J9::Z::MemoryReference::createUnresolvedDataSnippet(TR::Node *node, TR::CodeGenerator *cg,
    TR::SymbolReference *symRef, TR::Register *tempReg, bool isStore)
{
    TR::UnresolvedDataSnippet *uds;
    TR::Instruction *cursor;

    self()->setUnresolvedSnippet(
        uds = new (cg->trHeapMemory()) TR::UnresolvedDataSnippet(cg, node, symRef, isStore, false));
...
TR::UnresolvedDataSnippet *J9::Z::MemoryReference::createUnresolvedDataSnippetForAloadi(TR::Node *node,
    TR::CodeGenerator *cg, TR::SymbolReference *symRef, TR::Register *tempReg, bool &isStore)
{
    ...
    if (rootNode->getOpCode().isResolveCheck() && rootNode->getFirstChild()->getOpCode().isStoreIndirect()
...
void J9::Z::MemoryReference::createUnresolvedSnippetWithNodeRegister(TR::Node *node, TR::CodeGenerator *cg,
    TR::SymbolReference *symRef, TR::Register *&writableLiteralPoolRegister)
{
...
void J9::Z::MemoryReference::createUnresolvedDataSnippetForBaseNode(TR::CodeGenerator *cg,
    TR::Register *writableLiteralPoolRegister)
{
    self()->getUnresolvedSnippet()->createUnresolvedData(cg, _baseNode);
    self()->getUnresolvedSnippet()->getUnresolvedData()->setUnresolvedDataSnippet(self()->getUnresolvedSnippet());
...
void J9::Z::MemoryReference::createPatchableDataInLitpool(TR::Node *node, TR::CodeGenerator *cg, TR::Register *tempReg,
    TR::UnresolvedDataSnippet *uds)
{
    ...
    LRLinst = (TR::S390RILInstruction *)generateRILInstruction(cg, TR::InstOpCode::getLoadRelativeLongOpCode(), node,
        tempReg, reinterpret_cast<uintptr_t *>(0xBABE), 0);
...
bool J9::Z::MemoryReference::symRefHasTemporaryNegativeOffset()
{
    return self()->getStorageReference() && self()->getStorageReference()->getSymbolReference()
        && self()->getStorageReference()->getSymbolReference()->hasTemporaryNegativeOffset();
}

void J9::Z::MemoryReference::setMemRefAndGetUnresolvedData(TR::Snippet *&snippet)
{
...
TR::MemoryReference *reuseS390MemRefFromStorageRef(TR::MemoryReference *baseMR, int32_t offset, TR::Node *node,
    TR_StorageReference *storageReference, TR::CodeGenerator *cg, bool enforceSSLimits)
{
    if (baseMR == NULL) {
...
/**
 * When isNewTemp=true then do not transfer any deadBytes from the node/reg as this is a memref for brand new
 * tempStorageRef -- node is still needed in this case to attach on any instructions for line number lookup
 */
TR::MemoryReference *generateS390MemRefFromStorageRef(TR::Node *node, TR_StorageReference *storageReference,
    TR::CodeGenerator *cg, bool enforceSSLimits, bool isNewTemp)
{
    TR::Compilation *comp = cg->comp();
    OMR::Logger *log = comp->log();
    bool trace = cg->traceBCDCodeGen();
    ...
    // A memRef created for an indirect load may have its symRef replaced with its child's symRef (when the child is a
    // loadaddr for example) When it comes to right aligning the memRef the symbol size of the indirect load itself is
    // required and *not* the symbol size of the child So in these cases cache the indirect load symbol size on the
    // memRef for later use when right aligning. One example of this in a indirect load off the loadaddr of
    // comp()->getWCodeMainLitSymRef(). In this case the symbol size reflects all the constants present. Another example
    // is an indirect load off of a loadaddr auto. The auto symbol size may be larger than the indirect load size being
    // performed.
    if (storageReference->isTemporaryBased()) {
        memRef = new (cg->trHeapMemory())
            TR::MemoryReference(node, storageReference->getTemporarySymbolReference(), cg, storageReference);
    } else {
    ...
    if (!isNewTemp && !storageReference->isSingleUseTemporary() && node && node->getOpaquePseudoRegister()
        ...
        logprintf(trace, log,
            "\tgenerateS390AlignedMemoryReference: adjust memRef->_offset for regDeadAndIgnoredBytes (%d->%d) from "
            "node %s (%p) and reg %s\n",
            memRef->getOffset(), memRef->getOffset() - regDeadAndIgnoredBytes, node->getOpCode().getName(), node,
            cg->getDebug()->getName(node->getOpaquePseudoRegister()));
    ...
    if (storageReference->isTemporaryBased() && storageReference->getSymbolReference()->hasTemporaryNegativeOffset()) {
        logprintf(trace, log,
            "\tgenerateS390AlignedMemoryReference mr %p: call addToTemporaryNegativeOffset flag for storageRef->symRef "
            "#%d (offset = %d) and node %s (%p) to memRef\n",
    ...
    TR_ASSERT(!storageReference->isTemporaryBased() || storageReference->isSingleUseTemporary()
            || // refCount is always zero for these
    ...
    TR_ASSERT(!storageReference->isTemporaryBased() || storageReference->getTemporarySymbol()->isReferenced(),
        "using a temporary symbol that is marked as unreferenced\n");

    // enforcing the ss limits early means that each consuming instruction does not have to consolidate an index
    // register or a large offset to a new base register
    if (enforceSSLimits)
        memRef->enforceSSFormatLimits(node, cg, NULL);
...
TR::MemoryReference *generateS390LeftAlignedMemoryReference(TR::Node *node, TR_StorageReference *storageReference,
    TR::CodeGenerator *cg, int32_t leftMostByte, bool enforceSSLimits, bool isNewTemp)
{
    TR_ASSERT(!node->getType().isAggregate(), "do not use aligned memrefs for aggrs on node %p\n", node);
    TR::MemoryReference *mr = generateS390MemRefFromStorageRef(node, storageReference, cg, enforceSSLimits, isNewTemp);
...
TR::MemoryReference *reuseS390LeftAlignedMemoryReference(TR::MemoryReference *baseMR, TR::Node *node,
    TR_StorageReference *storageReference, TR::CodeGenerator *cg, int32_t leftMostByte, bool enforceSSLimits)
{
    TR_ASSERT(!node->getType().isAggregate(), "do not use aligned memrefs for aggrs on node %p\n", node);
    if (baseMR == NULL)
