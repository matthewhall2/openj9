...
uint8_t *TR_PPCRelocationTarget::eipBaseForCallOffset(uint8_t *reloLocation)
{
    // reloLocation points at the start of the call offset, return address is 4
    return (uint8_t *)(reloLocation);
}

void TR_PPCRelocationTarget::storeCallTarget(uintptr_t callTarget, uint8_t *reloLocation)
{
...
uint32_t TR_PPCRelocationTarget::loadCPIndex(uint8_t *reloLocation)
{
...
uintptr_t TR_PPCRelocationTarget::loadThunkCPIndex(uint8_t *reloLocation)
{
    // reloLocation points at the cpAddress in snippet, need to find cpIndex location // KEN
...
void TR_PPCRelocationTarget::performThunkRelocation(uint8_t *thunkBase, uintptr_t vmHelper)
{
    ...
    *(int32_t *)(thunkBase + *thunkRelocationData + 16) = 0x60840000 | (sendTarget & 0x0000FFFF);
    ...
    if (!(sendTarget & 0x00008000)) {
        ...
        *(int32_t *)(cursor) = 0x64840000 | ((sendTarget >> 16) & 0x0000FFFF);
        ...
        // lis gr4, upper
        *(int32_t *)(cursor) = 0x3c800000 | (((sendTarget >> 16) + ((sendTarget & (1 << 15)) ? 1 : 0)) & 0x0000FFFF);
        cursor += 4;
        // addi gr4, gr4, lower
...
uint8_t *TR_PPCRelocationTarget::loadAddress(uint8_t *reloLocationHigh, uint8_t *reloLocationLow)
{
    uint16_t highValue = loadUnsigned16b(reloLocationHigh);
    uint16_t lowValue = loadUnsigned16b(reloLocationLow);
    uintptr_t value
        = ((uintptr_t)highValue << 16) + (uintptr_t)lowValue; // Only 32-bit should use this so pointer is 32-bit
    return (uint8_t *)value;
}

void TR_PPCRelocationTarget::storeAddress(uint8_t *address, uint8_t *reloLocationHigh, uint8_t *reloLocationLow,
    uint32_t seqNumber)
{
...
uint8_t *TR_PPCRelocationTarget::loadAddressSequence(uint8_t *reloLocation)
{
...
void TR_PPCRelocationTarget::storeAddressSequence(uint8_t *address, uint8_t *reloLocation, uint32_t seqNumber)
{
    // TR_ASSERT(0, "Error: storeAddressSequence not implemented in relocation target base class");
    storePointer(address, reloLocation);
}

void TR_PPC32RelocationTarget::storeRelativeAddressSequence(uint8_t *address, uint8_t *reloLocation, uint32_t seqNumber)
{
    ...
    uintptr_t value = (uintptr_t)address;
    uint32_t *patchAddr
        = (uint32_t *)((U_8 *)reloLocation + (reloRuntime()->comp()->target().cpu.isBigEndian() ? 2 : 0));
...
void TR_PPC64RelocationTarget::storeAddressSequence(uint8_t *address, uint8_t *reloLocation, uint32_t seqNumber)
{
...
void TR_PPC32RelocationTarget::storeAddressSequence(uint8_t *address, uint8_t *reloLocation, uint32_t seqNumber)
{
    storePointer(address, reloLocation);
}

void TR_PPCRelocationTarget::storeRelativeTarget(uintptr_t callTarget, uint8_t *reloLocation)
{
    // reloLocation points at the start of the call offset, so just store the uint8_t * at reloLocation
...
bool TR_PPCRelocationTarget::isOrderedPairRelocation(TR_RelocationRecord *reloRecord, TR_RelocationTarget *reloTarget)
{
...
bool TR_PPC32RelocationTarget::isOrderedPairRelocation(TR_RelocationRecord *reloRecord, TR_RelocationTarget *reloTarget)
{
    switch (reloRecord->type(reloTarget)) {
        case TR_AbsoluteMethodAddressOrderedPair:
...
{
    return !reloRuntime()->comp()->target().cpu.isTargetWithinIFormBranchRange((intptr_t)helperAddress,
               (intptr_t)baseLocation)
        || TR::Options::getCmdLineOptions()->getOption(TR_StressTrampolines);
}

uint8_t *TR_PPCRelocationTarget::arrayCopyHelperAddress(J9JavaVM *javaVM)
{
...
void TR_PPCRelocationTarget::flushCache(uint8_t *codeStart, unsigned long size)
{
    ppcCodeSync((unsigned char *)codeStart, (unsigned int)size);
}

void TR_PPC64RelocationTarget::platformAddPICtoPatchPtrOnClassUnload(TR_OpaqueClassBlock *classKey, void *ptr)
{
    /*
     * On POWER, PICs always treat the value guarded as 4 bytes wide.  Taking the 4 bytes, they set the least
     * significant bit to 1. Because the platform is (currently) big-endian, if we send them the full pointer, it will
     * set 33rd bit one, which is not what we want. Instead, we will create the PIC site on the low bits directly so
     * that it flags the right bit.
     */
    uint8_t *sitePointer = static_cast<uint8_t *>(ptr) + (reloRuntime()->comp()->target().cpu.isBigEndian() ? 4 : 0);
    createClassUnloadPicSite(classKey, sitePointer, sizeof(int32_t),
