...
enum tr_pm_bhrb_ifm_t {
    ...
    BHRB_IFM3 // Same as IFM2, except exclude conditional branches with static hints and branches that don't depend on
              // CR
};
...
#ifdef AIXPPC
#define MFBHRBE(target, entry) asm(".long 0x7C00025C | %0 < 21 | %1 < 11    \n\t" : "=r"(target) : "i"(entry))
#elif defined(LINUXPPC)
#define MFBHRBE(target, entry) asm(".long 0x7C00025C | %0 << 21 | %1 << 11  \n\t" : "=r"(target) : "i"(entry))
#endif /* LINUXPPC */
...
static inline __attribute__((always_inline)) void readBHRB(uintptr_t *bhrb)
{
    GET_BHRBE(0, bhrb);
    GET_BHRBE(1, bhrb);
    GET_BHRBE(2, bhrb);
...
static inline __attribute__((always_inline)) int tr_pmc_read_1to4(uint32_t buf[4])
{
...
static inline __attribute__((always_inline)) int tr_pmc_read_5to6(uint32_t buf[2])
{
    register uintptr_t retval asm("r3") = (uintptr_t)buf;
...
static inline __attribute__((always_inline)) int tr_mmcr_read(uint64_t buf[3])
{
    register uintptr_t retval asm("r3") = (uintptr_t)buf;
...
static inline __attribute__((always_inline)) int tr_sampled_regs_read(uint32_t buf[3])
{
    register uintptr_t retval asm("r3") = (uintptr_t)buf;
...
static inline __attribute__((always_inline)) int tr_pmc_write(uint32_t pmc, const uint32_t *value)
{
...
static inline __attribute__((always_inline)) int tr_mmcr_write(uint32_t mmcr, const uint64_t *value)
{
...
static inline __attribute__((always_inline)) int tr_sampled_regs_read(uint64_t buf[3])
{
    MFSPR64(buf[0], SIER);
    MFSPR64(buf[1], SIAR);
    MFSPR64(buf[2], SDAR);
...
extern "C" UDATA updateJITRuntimeInstrumentationFlags(J9VMThread *targetThread, UDATA newFlags);
