...
static inline __attribute__((always_inline)) void *getTCBContext()
{
    void *ret;
    asm(laddr " %0, %1(" TCB ")" : "=r"(ret) : "i"(TCB_EBB_CONTEXT));
    return ret;
}

static inline __attribute__((always_inline)) void setTCBContext(void *context)
{
    asm(staddr " %0, %1(" TCB ")" : : "r"(context), "i"(TCB_EBB_CONTEXT));
}
...
TR_PPCHWProfiler *TR_PPCHWProfiler::allocate(J9JITConfig *jitConfig)
{
    ...
    if (major < requiredMajor || major == requiredMajor && minor < requiredMinor) {
        ...
        if (!isRHEL() || major < requiredMajorRHEL || major == requiredMajorRHEL && minor < requiredMinorRHEL) {
            VERBOSE("glibc version %s does not support EBB context in TCB, need %u.%u or later.", glibcVersion,
                requiredMajor, requiredMinor);
            return NULL;
        } else
            VERBOSE("OS appears to be RHEL with glibc %d.%d, assuming glibc supports EBB context in TCB.", major,
                minor);
    }
...
struct tr_perf_event_attr {
    ...
    __u64 disabled: 1, inherit: 1, pinned: 1, exclusive: 1, exclude_user: 1, exclude_kernel: 1, exclude_hv: 1,
        exclude_idle: 1, mmap: 1, comm: 1, freq: 1, inherit_stat: 1, enable_on_exec: 1, task: 1, watermark: 1,
        precise_ip: 2, mmap_data: 1, sample_id_all: 1,

        exclude_host: 1, exclude_guest: 1,

        exclude_callchain_kernel: 1, exclude_callchain_user: 1,
...
bool TR_PPCHWProfiler::initializeThread(J9VMThread *vmThread)
{
    ...
    for (int i = 0; i < MAX_PMCS; ++i) {
        ...
        if (fds[i] < 0) {
            VERBOSE("Failed to open perf interface for counter %u for J9VMThread=%p, errno: %d, perf_event_open : %s.",
                i + 1, vmThread, errno, strerror(errno));
            goto closefds;
        }
    ...
    for (int i = 0; i < MAX_PMCS; ++i) {
        ...
        context->sampleRates[i]
            = (configs[currentConfig].sampleRates[i] == INVALID_SAMPLERATE ? INVALID_SAMPLERATE
                                                                           : TR::Options::_hwprofilerPRISamplingRate);
        context->fds[i] = fds[i];
...
closefds:
    for (int i = 0; i < MAX_PMCS; ++i) {
        if (fds[i] >= 0 && close(fds[i]))
            VERBOSE("Failed to close perf interface on J9VMThread=%p, errno: %d, close : %s.", vmThread, errno,
                strerror(errno));
    }
fail:
...
bool TR_PPCHWProfiler::deinitializeThread(J9VMThread *vmThread)
{
    if (!IS_THREAD_RI_INITIALIZED(vmThread))
        return true;
    ...
    for (int i = 0; i < MAX_PMCS; ++i) {
        ...
        if (context->buffers[i].start)
            freeBuffer(context->buffers[i].start,
                configs[context->currentConfig].bufferElementSize[i] * configs[context->currentConfig].bufferSize[i]);
        if (context->fds[i] >= 0 && close(context->fds[i]))
            VERBOSE("Failed to close perf interface for counter %u (fd=%d) on J9VMThread=%p, errno: %d, close : %s.",
                i + 1, context->fds[i], vmThread, errno, strerror(errno));
    }
#if defined(DEBUG) || defined(PPCRI_VERBOSE)
