...
static bool pmapiInitPM()
{
    ...
    for (int i = 0; i < NumProfilingConfigs; ++i) {
        ...
        for (int c = 0; c < MAX_PMCS; ++c) {
            ...
            VERBOSE("PMC[%u]: %s (%s)", c + 1, event->short_name,
                event->status.b.verified         ? "verified"
                    : event->status.b.unverified ? "unverified"
                    : event->status.b.caveat     ? "caveat"
                                                 : "unknown status");
        }
    }
...
bool TR_PPCHWProfiler::initializeThread(J9VMThread *vmThread)
{
    ...
    // goto bypasses decl of freeze
    {
        ...
        if (tr_mmcr_write(MMCR0, &freeze))
            goto stopcounting;
    }

    for (int i = 0; i < MAX_PMCS; ++i)
        sampleRates[i]
            = configs[currentConfig].sampleRates[i] != INVALID_SAMPLERATE ? 0x80000000 - context->sampleRates[i] : 0;
    if (tr_pmc_write(PMC1, &sampleRates[0]))
        goto stopcounting;
    if (tr_pmc_write(PMC2, &sampleRates[1]))
        goto stopcounting;
    if (tr_pmc_write(PMC3, &sampleRates[2]))
        goto stopcounting;
    if (tr_pmc_write(PMC4, &sampleRates[3]))
        goto stopcounting;
    if (tr_pmc_write(PMC5, &sampleRates[4]))
        goto stopcounting;
    if (tr_pmc_write(PMC6, &sampleRates[5]))
        goto stopcounting;
    // Freeze the counters that we're not using
    uint64_t mmcr2;
    TR_PPCHWProfilerPMUConfig::calcMMCR2ForConfig(mmcr2, currentConfig);
    if (tr_mmcr_write(MMCR2, &mmcr2))
        goto stopcounting;
    ...
    if (configs[currentConfig].vmFlags) {
        ...
        if (tr_mmcr_write(MMCR0, &unfreeze))
            goto stopcounting;
