...
#else
uint32_t getPPCCacheLineSize() { return 32; }
#endif

TR::PPCReadMonitorSnippet::PPCReadMonitorSnippet(TR::CodeGenerator *codeGen, TR::Node *monitorEnterNode,
    TR::Node *monitorExitNode, TR::LabelSymbol *recurCheckLabel, TR::LabelSymbol *monExitCallLabel,
    TR::LabelSymbol *restartLabel, TR::InstOpCode::Mnemonic loadOpCode, int32_t loadOffset,
    ...
    , TR::PPCHelperCallSnippet(codeGen, monitorExitNode, monExitCallLabel, monitorExitNode->getSymbolReference(),
          restartLabel)
{
...
uint8_t *TR::PPCReadMonitorSnippet::emitSnippetBody()
{
    ...
    TR::RealRegister *cndReg
        = cg()->machine()->getRealRegister(deps->getPostConditions()->getRegisterDependency(2)->getRealRegister());
    TR::RealRegister *loadResultReg
        = cg()->machine()->getRealRegister(deps->getPostConditions()->getRegisterDependency(3)->getRealRegister());
    bool isResultCollectable
        = deps->getPostConditions()->getRegisterDependency(3)->getRegister()->containsCollectedReference();
    TR::RealRegister *loadBaseReg
        = cg()->machine()->getRealRegister(deps->getPostConditions()->getRegisterDependency(4)->getRealRegister());
    ...
    *(int32_t *)buffer |= (getRestartLabel()->getCodeLocation() - buffer) & 0x03FFFFFC;
    ...
    if (cg()->directCallRequiresTrampoline(helperAddress, (intptr_t)buffer)) {
        helperAddress = TR::CodeCacheManager::instance()->findHelperTrampoline(
            getMonitorEnterHelper()->getReferenceNumber(), (void *)buffer);
        TR_ASSERT_FATAL(comp->target().cpu.isTargetWithinIFormBranchRange(helperAddress, (intptr_t)buffer),
    ...
    if (comp->compileRelocatableCode()) {
        cg()->addExternalRelocation(
            TR::ExternalRelocation::create(buffer, (uint8_t *)getMonitorEnterHelper(), TR_HelperAddress, cg()),
    ...
    *(int32_t *)buffer |= _loadOffset & 0xFFFF;
    ...
    if (isResultCollectable)
        exitMap->setRegisterBits(
            cg()->registerBitMask((int)deps->getPostConditions()->getRegisterDependency(3)->getRealRegister()));
...
void TR::PPCReadMonitorSnippet::print(OMR::Logger *log, TR_Debug *debug)
{
    TR::Compilation *comp = cg()->comp();
    TR_J9VMBase *fej9 = (TR_J9VMBase *)(cg()->fe());
    ...
    if (comp->target().is64Bit())
        log->printf("rldicr \t%s, %s, 0, " INT64_PRINTF_FORMAT_HEX "\t; Get owner thread value",
            debug->getName(monitorReg), debug->getName(monitorReg), (int64_t)LOCK_THREAD_PTR_MASK);
    else
        log->printf("rlwinm \t%s, %s, 0, 0x%x\t; Get owner thread value", debug->getName(monitorReg),
            debug->getName(monitorReg), LOCK_THREAD_PTR_MASK);
    cursor += 4;

    debug->printPrefix(log, NULL, cursor, 4);
    if (comp->target().is64Bit())
        log->printf("cmp8 \t%s, %s, %s\t; Compare VMThread to owner thread", debug->getName(condReg),
            debug->getName(metaReg), debug->getName(monitorReg));
    else
        log->printf("cmp4 \t%s, %s, %s\t; Compare VMThread to owner thread", debug->getName(condReg),
            debug->getName(metaReg), debug->getName(monitorReg));
    cursor += 4;
    ...
    log->printf("bne %s, " POINTER_PRINTF_FORMAT "\t; Use Helpers", debug->getName(condReg),
        (intptr_t)cursor + distance);
    cursor += 4;
    ...
    log->printf("bl \t" POINTER_PRINTF_FORMAT "\t\t; %s", (intptr_t)cursor + distance,
        debug->getName(getMonitorEnterHelper()));
...
uint32_t TR::PPCJ9HelperCallSnippet::getLength(int32_t estimatedSnippetStart) {
...
void TR::PPCJ9HelperCallSnippet::print(OMR::Logger *log, TR_Debug* debug) {
...
uint32_t TR::getCCPreLoadedCodeSize()
{
    ...
    if (false) {
        const uint32_t linesToPrefetch
            = TR::Options::_TLHPrefetchLineCount > 0 ? TR::Options::_TLHPrefetchLineCount : 8;
        size += (linesToPrefetch + 1) / 2 * 4;
    } else {
        static bool l3SkipLines = feGetEnv("TR_l3SkipLines") != NULL;
        const uint32_t linesToLdPrefetch
            = TR::Options::_TLHPrefetchLineCount > 0 ? TR::Options::_TLHPrefetchLineCount : 4;
...
static uint8_t *initializeCCPreLoadedPrefetch(uint8_t *buffer, void **CCPreLoadedCodeTable, TR::CodeGenerator *cg)
{
    ...
    if (!TR::CodeGenerator::supportsTransientPrefetch() && !doL1Pref) {
        const uint32_t linesToPrefetch
            = TR::Options::_TLHPrefetchLineCount > 0 ? TR::Options::_TLHPrefetchLineCount : 8;
        const uint32_t restartAfterLines
            = TR::Options::_TLHPrefetchBoundaryLineCount > 0 ? TR::Options::_TLHPrefetchBoundaryLineCount : 8;
        const uint32_t skipLines
            = TR::Options::_TLHPrefetchStaggeredLineCount > 0 ? TR::Options::_TLHPrefetchStaggeredLineCount : 4;
        helperSize = (linesToPrefetch + 1) / 2 * 4;

        TR_ASSERT_FATAL((skipLines + 1) * ppcCacheLineSize <= UPPER_IMMED,
            "tlhPrefetchStaggeredLineCount (%u) is too high. Will cause imm field to overflow.", skipLines);
        TR_ASSERT_FATAL(restartAfterLines * ppcCacheLineSize <= UPPER_IMMED,
            "tlhPrefetchBoundaryLineCount (%u) is too high. Will cause imm field to overflow.", restartAfterLines);

        cursor = generateTrg1Src1ImmInstruction(cg, TR::InstOpCode::addi, n, r10, r8, skipLines * ppcCacheLineSize,
            cursor);
        cursor = generateTrg1Src1ImmInstruction(cg, TR::InstOpCode::addi, n, r11, r8,
            (skipLines + 1) * ppcCacheLineSize, cursor);
        cursor = generateMemInstruction(cg, TR::InstOpCode::dcbtst, n,
            TR::MemoryReference::createWithIndexReg(cg, NULL, r10, 4), cursor);
        cursor = generateMemInstruction(cg, TR::InstOpCode::dcbtst, n,
            TR::MemoryReference::createWithIndexReg(cg, NULL, r11, 4), cursor);

        for (uint32_t i = 2; i < linesToPrefetch; i += 2) {
            cursor
                = generateTrg1Src1ImmInstruction(cg, TR::InstOpCode::addi, n, r10, r10, ppcCacheLineSize * 2, cursor);
            cursor
                = generateTrg1Src1ImmInstruction(cg, TR::InstOpCode::addi, n, r11, r11, ppcCacheLineSize * 2, cursor);
            cursor = generateMemInstruction(cg, TR::InstOpCode::dcbtst, n,
                TR::MemoryReference::createWithIndexReg(cg, NULL, r10, 4), cursor);
            cursor = generateMemInstruction(cg, TR::InstOpCode::dcbtst, n,
                TR::MemoryReference::createWithIndexReg(cg, NULL, r11, 4), cursor);
        ...
        cursor
            = generateTrg1ImmInstruction(cg, TR::InstOpCode::li, n, r11, restartAfterLines * ppcCacheLineSize, cursor);
        ...
        const uint32_t linesToStPrefetch = linesToLdPrefetch * 2;
        const uint32_t restartAfterLines
            = TR::Options::_TLHPrefetchBoundaryLineCount > 0 ? TR::Options::_TLHPrefetchBoundaryLineCount : 4;
        const uint32_t skipLines
            = TR::Options::_TLHPrefetchStaggeredLineCount > 0 ? TR::Options::_TLHPrefetchStaggeredLineCount : 4;
        ...
        cursor = generateTrg1MemInstruction(cg, TR::InstOpCode::Op_load, n, r10,
            TR::MemoryReference::createWithDisplacement(cg, metaReg, offsetof(J9VMThread, debugEventData3),
                TR::Compiler->om.sizeofReferenceAddress()),
            cursor);
        ...
        cursor = generateTrg1Src1ImmInstruction(cg, TR::InstOpCode::addi, n, r11, r8,
            (skipLines + 1) * ppcCacheLineSize, cursor);
        cursor = generateMemInstruction(cg, TR::InstOpCode::dcbtt, n,
            TR::MemoryReference::createWithIndexReg(cg, NULL, r10, 4), cursor);
        cursor = generateMemInstruction(cg, TR::InstOpCode::dcbtt, n,
            TR::MemoryReference::createWithIndexReg(cg, NULL, r11, 4), cursor);
        ...
        if (l3SkipLines > 0) {
            TR_ASSERT_FATAL(ppcCacheLineSize * l3SkipLines <= UPPER_IMMED,
                "TR_l3SkipLines (%u) is too high. Will cause imm field to overflow.", l3SkipLines);
            cursor = generateTrg1Src1ImmInstruction(cg, TR::InstOpCode::addi, n, r10, r10,
                ppcCacheLineSize * l3SkipLines, cursor);
            cursor = generateTrg1Src1ImmInstruction(cg, TR::InstOpCode::addi, n, r11, r11,
                ppcCacheLineSize * l3SkipLines, cursor);
        }
        ...
        for (uint32_t i = 0; i < linesToStPrefetch; i += 2) {
            ...
            cursor
                = generateTrg1Src1ImmInstruction(cg, TR::InstOpCode::addi, n, r11, r11, ppcCacheLineSize * 2, cursor);
            cursor = generateMemInstruction(cg, TR::InstOpCode::dcbtstt, n,
                TR::MemoryReference::createWithIndexReg(cg, NULL, r10, 4), cursor);
            cursor = generateMemInstruction(cg, TR::InstOpCode::dcbtstt, n,
                TR::MemoryReference::createWithIndexReg(cg, NULL, r11, 4), cursor);
        }
...
static uint8_t *initializeCCPreLoadedNonZeroPrefetch(uint8_t *buffer, void **CCPreLoadedCodeTable,
    TR::CodeGenerator *cg)
{
    TR::Compilation *comp = cg->comp();
    TR::Node *n = cg->getFirstInstruction()->getNode();
    ...
    if (!TR::CodeGenerator::supportsTransientPrefetch() && !doL1Pref) {
        ...
        cursor
            = generateTrg1ImmInstruction(cg, TR::InstOpCode::li, n, r11, restartAfterLines * ppcCacheLineSize, cursor);
        cursor = generateMemSrc1Instruction(cg, TR::InstOpCode::Op_st, n,
            TR::MemoryReference::createWithDisplacement(cg, metaReg, offsetof(J9VMThread, nonZeroTlhPrefetchFTA),
                TR::Compiler->om.sizeofReferenceAddress()),
            r11, cursor);
    } else {
        // Transient version
...
static TR::Instruction *genZeroInit(TR::CodeGenerator *cg, TR::Node *n, TR::Register *objStartReg,
    TR::Register *objEndReg, TR::Register *needsZeroInitCondReg, TR::Register *iterReg, TR::Register *zeroReg,
    TR::Register *condReg, uint32_t initOffset, TR::Instruction *cursor)
{
    TR_ASSERT_FATAL_WITH_NODE(n, initOffset <= UPPER_IMMED,
        "initOffset (%u) is too big to fit in a signed immediate field.", initOffset);
    ...
    // Fill the object with junk to make sure zero-init is working
    {
        ...
        cursor = generateMemSrc1Instruction(cg, TR::InstOpCode::stw, n,
            TR::MemoryReference::createWithDisplacement(cg, iterReg, 0, 4), zeroReg, cursor);
    ...
    for (int i = 0; i < 32; i += 4)
        cursor = generateMemSrc1Instruction(cg, TR::InstOpCode::stw, n,
            TR::MemoryReference::createWithDisplacement(cg, iterReg, i, 4), zeroReg, cursor);
    cursor = generateTrg1Src1ImmInstruction(cg, TR::InstOpCode::addi, n, iterReg, iterReg, 32, cursor);
    cursor = generateConditionalBranchInstruction(cg, TR::InstOpCode::bdnz, n, unrolledLoopStartLabel,
        /* Not used */ condReg, cursor);
...
static uint8_t *initializeCCPreLoadedWriteBarrier(uint8_t *buffer, void **CCPreLoadedCodeTable, TR::CodeGenerator *cg)
{
    ...
    helperTrampolineLabel->setCodeLocation(
        (uint8_t *)TR::CodeCacheManager::instance()->findHelperTrampoline(TR_writeBarrierStoreGenerational, buffer));
    TR::Instruction *entry = generateLabelInstruction(cg, TR::InstOpCode::label, n, entryLabel);
    TR::Instruction *cursor = entry;
    ...
    const TR::InstOpCode::Mnemonic rememberedClassMaskOp = J9_OBJECT_HEADER_REMEMBERED_MASK_FOR_TEST > UPPER_IMMED
            || J9_OBJECT_HEADER_REMEMBERED_MASK_FOR_TEST < LOWER_IMMED
        ? TR::InstOpCode::andis_r
        : TR::InstOpCode::andi_r;
    const uint32_t rememberedClassMask = rememberedClassMaskOp == TR::InstOpCode::andis_r
        ? J9_OBJECT_HEADER_REMEMBERED_MASK_FOR_TEST >> 16
    ...
    TR_ASSERT_FATAL((J9_OBJECT_HEADER_REMEMBERED_MASK_FOR_TEST <= UPPER_IMMED
                        && J9_OBJECT_HEADER_REMEMBERED_MASK_FOR_TEST >= LOWER_IMMED)
            || (J9_OBJECT_HEADER_REMEMBERED_MASK_FOR_TEST & 0xffff) == 0,
        "Expecting J9_OBJECT_HEADER_REMEMBERED_MASK_FOR_TEST to fit in immediate field");
    TR_ASSERT_FATAL(rememberedClassMask <= 0xFFFF,
        "Expecting rememberedClassMask (%u) to fit in an unsigned immediate field.", rememberedClassMask);
    ...
    if (comp->target().is32Bit() && !comp->compileRelocatableCode() && constHeapBase) {
        ...
        cursor = generateTrg1MemInstruction(cg, TR::InstOpCode::Op_load, n, r5,
            TR::MemoryReference::createWithDisplacement(cg, metaReg,
                offsetof(struct J9VMThread, heapBaseForBarrierRange0), TR::Compiler->om.sizeofReferenceAddress()),
            cursor);
    if (comp->target().is32Bit() && !comp->compileRelocatableCode() && constHeapSize) {
        ...
        cursor = generateTrg1MemInstruction(cg, TR::InstOpCode::Op_load, n, r6,
            TR::MemoryReference::createWithDisplacement(cg, metaReg,
                offsetof(struct J9VMThread, heapSizeForBarrierRange0), TR::Compiler->om.sizeofReferenceAddress()),
            cursor);
    ...
    cursor = generateTrg1MemInstruction(cg, Op_lclass, n, r11,
        TR::MemoryReference::createWithDisplacement(cg, r3, TR::Compiler->om.offsetOfObjectVftField(),
    ...
    const uint32_t helperSize = 12
        + (comp->target().is32Bit() && !comp->compileRelocatableCode() && constHeapBase && heapBase > UPPER_IMMED
                    && heapBase < LOWER_IMMED
                ? 1
                : 0)
        + (comp->target().is32Bit() && !comp->compileRelocatableCode() && constHeapSize && heapSize > UPPER_IMMED
                    && heapSize < LOWER_IMMED
                ? 1
                : 0);
    TR_ASSERT(cg->getBinaryBufferCursor() - entryLabel->getCodeLocation() == helperSize * PPC_INSTRUCTION_LENGTH,
...
static uint8_t *initializeCCPreLoadedWriteBarrierAndCardMark(uint8_t *buffer, void **CCPreLoadedCodeTable,
    TR::CodeGenerator *cg)
{
    TR::Compilation *comp = cg->comp();
    TR_J9VMBase *fej9 = (TR_J9VMBase *)(cg->fe());
    ...
    TR::LabelSymbol *helperTrampolineLabel = generateLabelSymbol(cg);
    helperTrampolineLabel->setCodeLocation((uint8_t *)TR::CodeCacheManager::instance()->findHelperTrampoline(
        TR_writeBarrierStoreGenerationalAndConcurrentMark, buffer));
    TR::Instruction *entry = generateLabelInstruction(cg, TR::InstOpCode::label, n, entryLabel);
    TR::Instruction *cursor = entry;
    TR::InstOpCode::Mnemonic Op_lclass = TR::InstOpCode::Op_load;
    if (TR::Compiler->om.compressObjectReferences())
        Op_lclass = TR::InstOpCode::lwz;
    const TR::InstOpCode::Mnemonic cmActiveMaskOp
        = J9_PRIVATE_FLAGS_CONCURRENT_MARK_ACTIVE > UPPER_IMMED || J9_PRIVATE_FLAGS_CONCURRENT_MARK_ACTIVE < LOWER_IMMED
        ? TR::InstOpCode::andis_r
        : TR::InstOpCode::andi_r;
    ...
    TR_ASSERT_FATAL((J9_PRIVATE_FLAGS_CONCURRENT_MARK_ACTIVE <= UPPER_IMMED
                        && J9_PRIVATE_FLAGS_CONCURRENT_MARK_ACTIVE >= LOWER_IMMED)
            || (J9_PRIVATE_FLAGS_CONCURRENT_MARK_ACTIVE & 0xffff) == 0,
        "Expecting J9_PRIVATE_FLAGS_CONCURRENT_MARK_ACTIVE to fit in immediate field");
    TR_ASSERT_FATAL((J9_OBJECT_HEADER_REMEMBERED_MASK_FOR_TEST <= UPPER_IMMED
                        && J9_OBJECT_HEADER_REMEMBERED_MASK_FOR_TEST >= LOWER_IMMED)
            || (J9_OBJECT_HEADER_REMEMBERED_MASK_FOR_TEST & 0xffff) == 0,
        "Expecting J9_OBJECT_HEADER_REMEMBERED_MASK_FOR_TEST to fit in immediate field");
    TR_ASSERT_FATAL(cmActiveMask <= 0xFFFF, "Expecting cmActiveMask (%u) to fit in an unsigned immediate field.",
        cmActiveMask);
    TR_ASSERT_FATAL(rememberedClassMask <= 0xFFFF,
        "Expecting rememberedClassMask (%u) to fit in an unsigned immediate field.", rememberedClassMask);
    ...
    cursor = generateTrg1MemInstruction(cg, TR::InstOpCode::lwz, n, r6,
        TR::MemoryReference::createWithDisplacement(cg, metaReg, offsetof(struct J9VMThread, privateFlags), 4), cursor);
    ...
    cursor = generateTrg1MemInstruction(cg, TR::InstOpCode::Op_load, n, r6,
        TR::MemoryReference::createWithDisplacement(cg, metaReg, offsetof(struct J9VMThread, activeCardTableBase),
            TR::Compiler->om.sizeofReferenceAddress()),
        cursor);
    if (comp->target().is64Bit())
    ...
    cursor = generateMemSrc1Instruction(cg, TR::InstOpCode::stbx, n,
        TR::MemoryReference::createWithIndexReg(cg, r6, r11, 1), r5, cursor);
...
static uint8_t *initializeCCPreLoadedCardMark(uint8_t *buffer, void **CCPreLoadedCodeTable, TR::CodeGenerator *cg)
{
    ...
    cursor = generateTrg1MemInstruction(cg, TR::InstOpCode::Op_load, n, r5,
        TR::MemoryReference::createWithDisplacement(cg, metaReg, offsetof(struct J9VMThread, heapBaseForBarrierRange0),
            TR::Compiler->om.sizeofReferenceAddress()),
        cursor);
    cursor = generateTrg1MemInstruction(cg, TR::InstOpCode::Op_load, n, r4,
        TR::MemoryReference::createWithDisplacement(cg, metaReg, offsetof(struct J9VMThread, heapSizeForBarrierRange0),
            TR::Compiler->om.sizeofReferenceAddress()),
        cursor);
    ...
    cursor = generateMemSrc1Instruction(cg, TR::InstOpCode::stbx, n,
        TR::MemoryReference::createWithIndexReg(cg, r4, r5, 1), r11, cursor);
    cursor = generateInstruction(cg, TR::InstOpCode::blr, n, cursor);
...
static uint8_t *initializeCCPreLoadedArrayStoreCHK(uint8_t *buffer, void **CCPreLoadedCodeTable, TR::CodeGenerator *cg)
{
    ...
    TR::LabelSymbol *helperTrampolineLabel = generateLabelSymbol(cg);
    helperTrampolineLabel->setCodeLocation(
        (uint8_t *)TR::CodeCacheManager::instance()->findHelperTrampoline(TR_typeCheckArrayStore, buffer));
    TR::Instruction *entry = generateLabelInstruction(cg, TR::InstOpCode::label, n, entryLabel);
    TR::Instruction *cursor = entry;
    TR::InstOpCode::Mnemonic Op_lclass = TR::InstOpCode::Op_load;
    if (TR::Compiler->om.compressObjectReferences())
    ...
    cursor = generateTrg1MemInstruction(cg, Op_lclass, n, r6,
        TR::MemoryReference::createWithDisplacement(cg, r4, TR::Compiler->om.offsetOfObjectVftField(),
            TR::Compiler->om.sizeofReferenceField()),
        cursor);
    ...
    cursor = generateTrg1MemInstruction(cg, TR::InstOpCode::Op_load, n, r5,
        TR::MemoryReference::createWithDisplacement(cg, r5, offsetof(J9ArrayClass, componentType),
            TR::Compiler->om.sizeofReferenceAddress()),
        cursor);
    ...
    cursor = generateTrg1MemInstruction(cg, TR::InstOpCode::Op_load, n, r7,
        TR::MemoryReference::createWithDisplacement(cg, r6, offsetof(J9Class, castClassCache),
            TR::Compiler->om.sizeofReferenceAddress()),
        cursor);
    ...
    cursor = generateTrg1MemInstruction(cg, TR::InstOpCode::Op_load, n, r7,
        TR::MemoryReference::createWithDisplacement(cg, r5, offsetof(J9Class, romClass),
            TR::Compiler->om.sizeofReferenceAddress()),
        cursor);
    cursor = generateTrg1MemInstruction(cg, TR::InstOpCode::lwz, n, r7,
        TR::MemoryReference::createWithDisplacement(cg, r7, offsetof(J9ROMClass, modifiers), 4), cursor);
    ...
    cursor = generateTrg1Src1ImmInstruction(cg, TR::InstOpCode::andi_r, n, r7, r7, cr0,
        (J9AccClassArray | J9AccInterface) >> 1, cursor);
    cursor = generateConditionalBranchInstruction(cg, TR::InstOpCode::bne, n, skipSuperclassTestLabel, cr0, cursor);

    cursor = generateTrg1MemInstruction(cg, TR::InstOpCode::Op_load, n, r7,
        TR::MemoryReference::createWithDisplacement(cg, r6, offsetof(J9Class, classDepthAndFlags),
            TR::Compiler->om.sizeofReferenceAddress()),
        cursor);
    TR_ASSERT_FATAL(!(J9AccClassDepthMask & ~0xffff), "Expecting class depth mask to fit in immediate");
    ...
    cursor = generateTrg1MemInstruction(cg, Op_lclass, n, r7,
        TR::MemoryReference::createWithDisplacement(cg, r6, offsetof(J9Class, superclasses),
            TR::Compiler->om.sizeofReferenceField()),
        cursor);
    cursor = generateTrg1MemInstruction(cg, TR::InstOpCode::Op_load, n, r7,
        TR::MemoryReference::createWithDisplacement(cg, r7, 0, TR::Compiler->om.sizeofReferenceAddress()), cursor);
    cursor = generateTrg1Src2Instruction(cg, TR::InstOpCode::Op_cmpl, n, cr0, r5, r7, cursor);
    cursor = generateConditionalBranchInstruction(cg, TR::InstOpCode::beqlr, n, NULL, cr0, cursor);
    ...
    TR::CodeGenerator *cg)
{
    ...
    // We temporarily clobber the first and append instructions so we can use high level codegen to generate pre-loaded
    // code So save the original values here and restore them when done
    TR::Compilation *comp = cg->comp();
    ...
    if (comp->getOptions()->getGcCardSize() > 0) {
        buffer
            = initializeCCPreLoadedWriteBarrierAndCardMark(buffer + PPC_INSTRUCTION_LENGTH, CCPreLoadedCodeTable, cg);
...
uint8_t *TR::PPCAllocPrefetchSnippet::emitSnippetBody()
{
    ...
    intptr_t distance
        = (intptr_t)(cg()->getCodeCache())->getCCPreLoadedCodeAddress(TR_AllocPrefetch, cg()) - (intptr_t)buffer;
    opcode.setOpCodeValue(TR::InstOpCode::b);
    buffer = opcode.copyBinaryToBuffer(buffer);
...
void TR::PPCAllocPrefetchSnippet::print(OMR::Logger *log, TR_Debug *debug)
{
    TR_J9VMBase *fej9 = (TR_J9VMBase *)(cg()->fe());
    uint8_t *cursor = getSnippetLabel()->getCodeLocation();
...
TR::PPCNonZeroAllocPrefetchSnippet::PPCNonZeroAllocPrefetchSnippet(TR::CodeGenerator *codeGen, TR::Node *node,
    TR::LabelSymbol *callLabel)
    : TR::Snippet(codeGen, node, callLabel, false)
{}
...
uint8_t *TR::PPCNonZeroAllocPrefetchSnippet::emitSnippetBody()
{
    ...
    TR_ASSERT((uintptr_t)((cg()->getCodeCache())->getCCPreLoadedCodeAddress(TR_NonZeroAllocPrefetch, cg()))
            != 0xDEADBEEF,
        "Invalid addr for code cache helper");
    intptr_t distance
        = (intptr_t)(cg()->getCodeCache())->getCCPreLoadedCodeAddress(TR_NonZeroAllocPrefetch, cg()) - (intptr_t)buffer;
    opcode.setOpCodeValue(TR::InstOpCode::b);
    buffer = opcode.copyBinaryToBuffer(buffer);
    *(int32_t *)buffer |= distance & 0x03FFFFFC;
    return buffer + PPC_INSTRUCTION_LENGTH;
...
void TR::PPCNonZeroAllocPrefetchSnippet::print(OMR::Logger *log, TR_Debug *debug)
{
    TR_J9VMBase *fej9 = (TR_J9VMBase *)(cg()->fe());
    uint8_t *cursor = getSnippetLabel()->getCodeLocation();
