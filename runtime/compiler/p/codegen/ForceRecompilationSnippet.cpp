...
uint8_t *TR::PPCForceRecompilationSnippet::emitSnippetBody()
{
    ...
    TR::SymbolReference *induceRecompilationSymRef
        = cg()->symRefTab()->findOrCreateRuntimeHelper(TR_PPCinduceRecompilation);
    ...
    intptr_t startPCRegValue
        = cg()->needRelocationsForCurrentMethodStartPC() && !cg()->canEmitDataForExternallyRelocatableInstructions()
        ? 0
        : startPC;
    ...
    if (cg()->comp()->target().is64Bit()) {
        ...
        *(uint32_t *)buffer |= llval;
        ...
        if (cg()->needRelocationsForCurrentMethodStartPC()) {
            TR::Relocation *startPCRelo = new (cg()->trHeapMemory())
                TR::ExternalRelocation(firstInstruction, NULL, (uint8_t *)fixedSequence1, TR_StartPC, cg());
            cg()->addExternalRelocation(startPCRelo, __FILE__, __LINE__, getNode());
        }
    } else {
        uint8_t *firstInstruction = buffer;
        ...
        *(uint32_t *)buffer |= (uint32_t)(startPCRegValue & 0xffff);
        ...
        if (cg()->needRelocationsForCurrentMethodStartPC()) {
            ...
            TR::Relocation *startPCRelo = new (cg()->trHeapMemory()) TR::ExternalOrderedPair32BitRelocation(
                firstInstruction, secondInstruction, (uint8_t *)recordInfo, TR_StartPC, cg());
            cg()->addExternalRelocation(startPCRelo, __FILE__, __LINE__, getNode());
        }
    }
    ...
    if (cg()->directCallRequiresTrampoline(helperAddress, (intptr_t)buffer)) {
        helperAddress = TR::CodeCacheManager::instance()->findHelperTrampoline(
            induceRecompilationSymRef->getReferenceNumber(), (void *)buffer);
    ...
    *(int32_t *)buffer = 0x48000000 | ((helperAddress - (intptr_t)buffer) & 0x03ffffff);
    ...
    cg()->addExternalRelocation(
        TR::ExternalRelocation::create(buffer, (uint8_t *)induceRecompilationSymRef, TR_HelperAddress, cg()), __FILE__,
        __LINE__, getNode());
...
void TR_Debug::print(OMR::Logger *log, TR::PPCForceRecompilationSnippet *snippet)
{
    ...
    log->printf("b \t" POINTER_PRINTF_FORMAT "\t; %s%s", (intptr_t)cursor + value,
