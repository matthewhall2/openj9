...
static int32_t calculateFrameSize(TR::RealRegister::RegNum &intSavedFirst,
   ...
   if (!saveLR && size == -firstLocalOffset)   // TODO: have a emptyFrame bool to be used throughout
      ...
      logprintf(comp->getOption(TR_TraceCG), comp->log(), "PPCLinkage calculateFrameSize registerSaveDescription: 0x%x regSaveOffset: %x\n", registerSaveDescription, regSaveOffset);
...
void J9::Power::PrivateLinkage::buildDirectCall(TR::Node *callNode,
   ...
   if (!isJitDispatchJ9Method && (callSymbol->isJITInternalNative() ||
      ...
      interpCallSnippet->gcMap().setGCRegisterMask(flags ^ 1); // do not gc scratch reg 2
      ...
      // then or with 2
      generateTrg1ImmInstruction(cg(), TR::InstOpCode::li, callNode, scratchReg2, 45);
