    ...
    : J9::PrivateLinkage(cg)
{
    ...
    _properties._registerFlags[TR::RealRegister::gr13]
        = Preserved | PPC_Reserved; // meta data for 32bit, system for 64bit;
...
void J9::Power::PrivateLinkage::initPPCRealRegisterLinkage()
{
    ...
    for (icount = TR::RealRegister::FirstGPR; icount <= TR::RealRegister::gr12; icount++) {
        if (linkage.getReserved((TR::RealRegister::RegNum)icount)) {
            ...
            ++lockedGPRs;
            machine->getRealRegister((TR::RealRegister::RegNum)icount)
                ->setAssignedRegister(machine->getRealRegister((TR::RealRegister::RegNum)icount));
        } else {
            int weight;
    ...
    for (icount = TR::RealRegister::FirstVRF; icount <= TR::RealRegister::LastVRF; icount++) {
        if (linkage.getPreserved((TR::RealRegister::RegNum)icount)) {
            machine->getRealRegister((TR::RealRegister::RegNum)icount)->setWeight(0xf000 - icount);
        } else {
    ...
    for (icount = TR::RealRegister::FirstCCR; icount <= TR::RealRegister::LastCCR; icount++) {
        if (linkage.getPreserved((TR::RealRegister::RegNum)icount)) {
            machine->getRealRegister((TR::RealRegister::RegNum)icount)->setWeight(0xf000 - icount);
        } else {
            machine->getRealRegister((TR::RealRegister::RegNum)icount)->setWeight(icount);
...
uint32_t J9::Power::PrivateLinkage::getRightToLeft() { return getProperties().getRightToLeft(); }
...
{
    int32_t roundup
        = (comp()->useCompressedPointers() && p->isLocalObject() ? TR::Compiler->om.getObjectAlignmentInBytes()
...
void J9::Power::PrivateLinkage::setParameterLinkageRegisterIndex(TR::ResolvedMethodSymbol *method)
{
    ...
    while ((paramCursor != NULL)
        && ((numIntArgs < properties.getNumIntArgRegs()) || (numFloatArgs < properties.getNumFloatArgRegs()))) {
        int32_t index = -1;
...
bool J9::Power::PrivateLinkage::hasToBeOnStack(TR::ParameterSymbol *parm)
{
    ...
    bool result = (parm->getAssignedGlobalRegisterIndex() >= 0
        && ((parm->getLinkageRegisterIndex() == 0 && parm->isCollectedReference() && !bodySymbol->isStatic()
                && ((bodySymbol->isSynchronised())
                    || (!strncmp(bodySymbol->getResolvedMethod()->nameChars(), "<init>", 6)
                        && ((throwableClass = fej9->getClassFromSignature("Ljava/lang/Throwable;", 21,
                                ...
                                == 0
                            || fej9->isInstanceOf(bodySymbol->getResolvedMethod()->containingClass(), throwableClass,
                                   true)
                                != TR_no))))
...
static TR::Instruction *unrollPrologueInitLoop(TR::CodeGenerator *cg, TR::Node *node, int32_t num,
    int32_t initSlotOffset, TR::Register *nullReg, TR::RealRegister *gr11, TR::RealRegister *baseInitReg,
    TR::RealRegister *gr12, TR::RealRegister *cr0, TR::LabelSymbol *loopLabel, TR::Instruction *cursor)
{
    TR_HeapMemory trHeapMemory = cg->trMemory();
    int32_t wordsToUnroll = num;
    TR::Compilation *comp = cg->comp();

    static bool disableVSXMemInit
        = (feGetEnv("TR_disableVSXMemInit") != NULL); // Disable toggle incase we break in production.
    bool use8Bytes = ((cg->is64BitProcessor() && TR::Compiler->om.sizeofReferenceAddress() == 4)
        || TR::Compiler->om.sizeofReferenceAddress() == 8);
    bool useVectorStores = (comp->target().cpu.isAtLeast(OMR_PROCESSOR_PPC_P8)
        && comp->target().cpu.supportsFeature(OMR_FEATURE_PPC_HAS_VSX) && !disableVSXMemInit);

    if (useVectorStores) {
        int32_t wordsUnrolledPerIteration = (128 * 4)
            / (TR::Compiler->om.sizeofReferenceAddress()
                * 8); // (number of bits cleared by one iteration using 4 stxvw4x) / (number of bits per word i.e. bits
                      // in a java pointer)
        if (wordsToUnroll >= wordsUnrolledPerIteration) {
            TR::RealRegister *vectorNullReg = cg->machine()->getRealRegister(TR::RealRegister::vr0);
            cursor = generateTrg1Src2Instruction(cg, TR::InstOpCode::xxlxor, node, vectorNullReg, vectorNullReg,
                vectorNullReg, cursor);

            int32_t loopIterations = wordsToUnroll / wordsUnrolledPerIteration;
            wordsToUnroll = wordsToUnroll % wordsUnrolledPerIteration;

            TR_ASSERT_FATAL(initSlotOffset <= UPPER_IMMED,
                "initSlotOffset (%d) is too big to fit in a signed immediate field", initSlotOffset);
            cursor = generateTrg1Src1ImmInstruction(cg, TR::InstOpCode::addi, node, gr12, baseInitReg, initSlotOffset,
                cursor);
            baseInitReg = gr12;
            initSlotOffset = 0;
            ...
            if (loopIterations > 1) {
                ...
                cursor
                    = loadConstant(cg, node, 16, gr11, cursor); // r11 is now free so we can use it for const 16 offset
                cursor = generateLabelInstruction(cg, TR::InstOpCode::label, node, loopLabel, cursor);
            } else {
                cursor = loadConstant(cg, node, 16, gr11, cursor);
            }

            cursor = generateMemSrc1Instruction(cg, TR::InstOpCode::stxvw4x, node,
                TR::MemoryReference::createWithIndexReg(cg, nullReg, baseInitReg, 16), vectorNullReg, cursor);
            cursor = generateMemSrc1Instruction(cg, TR::InstOpCode::stxvw4x, node,
                TR::MemoryReference::createWithIndexReg(cg, gr11, baseInitReg, 16), vectorNullReg, cursor);
            cursor
                = generateTrg1Src1ImmInstruction(cg, TR::InstOpCode::addi, node, baseInitReg, baseInitReg, 32, cursor);
            cursor = generateMemSrc1Instruction(cg, TR::InstOpCode::stxvw4x, node,
                TR::MemoryReference::createWithIndexReg(cg, nullReg, baseInitReg, 16), vectorNullReg, cursor);
            cursor = generateMemSrc1Instruction(cg, TR::InstOpCode::stxvw4x, node,
                TR::MemoryReference::createWithIndexReg(cg, gr11, baseInitReg, 16), vectorNullReg, cursor);
            ...
            if (loopIterations > 1) {
                cursor = generateTrg1Src1ImmInstruction(cg, TR::InstOpCode::addi, node, baseInitReg, baseInitReg, 32,
                    cursor);
                cursor = generateConditionalBranchInstruction(cg, TR::InstOpCode::bdnz, node, loopLabel, cr0, cursor);
            } else {
                initSlotOffset
                    += 32; // if no loops needed, then no need to generate the addi when we can just increase the offset
            }
        }
        ...
        while (wordsToUnroll > 0) {
            cursor = generateMemSrc1Instruction(cg, instruction, node,
                TR::MemoryReference::createWithDisplacement(cg, baseInitReg, initSlotOffset, storeSize), nullReg,
                cursor);
        ...
        if ((use8Bytes && TR::Compiler->om.sizeofReferenceAddress() == 4) && (num % 2 == 1)) {
            cursor = generateMemSrc1Instruction(cg, TR::InstOpCode::stw, node,
                TR::MemoryReference::createWithDisplacement(cg, baseInitReg, initSlotOffset, 4), nullReg, cursor);
        }
    } else {
        if (use8Bytes && TR::Compiler->om.sizeofReferenceAddress() == 4)
            wordsToUnroll /= 2;
        ...
        if (wordsToUnroll >= 4) {
            if (wordsToUnroll >= 8) {
                if (baseInitReg->getRegisterNumber() == TR::RealRegister::gr14) {
                    cursor
                        = generateTrg1Src1ImmInstruction(cg, TR::InstOpCode::addi, node, gr12, baseInitReg, 0, cursor);
                    baseInitReg = gr12;
                }
            ...
            cursor = generateMemSrc1Instruction(cg, use8Bytes ? TR::InstOpCode::std : TR::InstOpCode::stw, node,
                TR::MemoryReference::createWithDisplacement(cg, baseInitReg, initSlotOffset,
                    TR::Compiler->om.sizeofReferenceAddress()),
                nullReg, cursor);
            cursor = generateMemSrc1Instruction(cg, use8Bytes ? TR::InstOpCode::std : TR::InstOpCode::stw, node,
                TR::MemoryReference::createWithDisplacement(cg, baseInitReg, initSlotOffset + (use8Bytes ? 8 : 4),
                    TR::Compiler->om.sizeofReferenceAddress()),
                nullReg, cursor);
            cursor = generateMemSrc1Instruction(cg, use8Bytes ? TR::InstOpCode::std : TR::InstOpCode::stw, node,
                TR::MemoryReference::createWithDisplacement(cg, baseInitReg, initSlotOffset + (use8Bytes ? 16 : 8),
                    TR::Compiler->om.sizeofReferenceAddress()),
                nullReg, cursor);
            cursor = generateMemSrc1Instruction(cg, use8Bytes ? TR::InstOpCode::std : TR::InstOpCode::stw, node,
                TR::MemoryReference::createWithDisplacement(cg, baseInitReg, initSlotOffset + (use8Bytes ? 24 : 12),
                    TR::Compiler->om.sizeofReferenceAddress()),
                nullReg, cursor);
            if (wordsToUnroll >= 8) {
                cursor = generateTrg1Src1ImmInstruction(cg, TR::InstOpCode::addi, node, gr12, gr12,
                    (use8Bytes ? 32 : 16), cursor);
                cursor = generateConditionalBranchInstruction(cg, TR::InstOpCode::bdnz, node, loopLabel, cr0, cursor);
            } else {
...
static int32_t calculateFrameSize(TR::RealRegister::RegNum &intSavedFirst, int32_t &argSize, int32_t &size,
    int32_t &regSaveOffset, int32_t &saveSize, TR::CodeGenerator *cg, bool &saveLR)
{
    TR::Compilation *comp = cg->comp();
    TR::Machine *machine = cg->machine();
    ...
    //
    saveLR = (!cg->getSnippetList().empty() || comp->getJittedMethodSymbol()->isEHAware() || cg->canExceptByTrap()
    ...
    while (intSavedFirst <= TR::RealRegister::LastGPR
        && !machine->getRealRegister(intSavedFirst)->getHasBeenAssignedInMethod())
        intSavedFirst = (TR::RealRegister::RegNum)((uint32_t)intSavedFirst + 1);

    if (comp->target().is64Bit() && comp->compilePortableCode() && !comp->target().cpu.isAtLeast(OMR_PROCESSOR_PPC_P10)
        && cg->getSavesNonVolatileGPRsForGC()) {
    ...
    if (!saveLR && size == -firstLocalOffset) // TODO: have a emptyFrame bool to be used throughout
    {
        ...
        if (regSaveOffset > 0x7FFF || regSaveOffset < 0) {
            ...
            comp->failCompilation<TR::CompilationInterrupted>(
                "Overflowed or underflowed bounds of regSaveOffset in calculateFrameSize.");
        }

        logprintf(comp->getOption(TR_TraceCG), comp->log(),
            "PPCLinkage calculateFrameSize registerSaveDescription: 0x%x regSaveOffset: %x\n", registerSaveDescription,
            regSaveOffset);
...
void J9::Power::PrivateLinkage::createPrologue(TR::Instruction *cursor)
{
    ...
    if (cg()->getFrameSizeInBytes() || saveLR) {
        if ((cg()->getFrameSizeInBytes() + TR::Compiler->om.sizeofReferenceAddress()) > (-LOWER_IMMED)) {
            ...
            cursor = loadConstant(cg(), firstNode,
                (int32_t)(cg()->getFrameSizeInBytes() + TR::Compiler->om.sizeofReferenceAddress()), gr12, cursor);

            // javaSP <- javaSP - (totalFrameSize + 1 slot)
            cursor
                = generateTrg1Src2Instruction(cg(), TR::InstOpCode::subf, firstNode, stackPtr, gr12, stackPtr, cursor);

            // gr12 == totalFrameSize
            if (!comp()->isDLT())
                cursor = generateTrg1Src1ImmInstruction(cg(), TR::InstOpCode::addi2, firstNode, gr12, gr12,
            ...
            if (!comp()->isDLT())
                cursor = generateTrg1Src2Instruction(cg(), TR::InstOpCode::Op_cmpl, firstNode, cr0, stackPtr, gr11,
                    cursor);

            if (saveLR) {
                // javaSP[totalFrameSize] <- linkRegister
                cursor = generateMemSrc1Instruction(cg(), TR::InstOpCode::Op_stx, firstNode,
                    TR::MemoryReference::createWithIndexReg(cg(), stackPtr, gr12,
                        TR::Compiler->om.sizeofReferenceAddress()),
                    gr0, cursor);
            }
        } else {
            ...
            // javaSP <- javaSP - (totalFrameSize + 1 slot)
            cursor = generateTrg1Src1ImmInstruction(cg(), TR::InstOpCode::addi2, firstNode, stackPtr, stackPtr,
                -(cg()->getFrameSizeInBytes() + TR::Compiler->om.sizeofReferenceAddress()), cursor);
            if (saveLR) {
                cursor = generateTrg1Instruction(cg(), TR::InstOpCode::mflr, firstNode, gr0, cursor);
            }
            ...
            if (saveLR) {
                // javaSP[totalFrameSize] <- linkRegister
                cursor = generateMemSrc1Instruction(cg(), TR::InstOpCode::Op_st, firstNode,
                    TR::MemoryReference::createWithDisplacement(cg(), stackPtr, cg()->getFrameSizeInBytes(),
                        TR::Compiler->om.sizeofReferenceAddress()),
                    gr0, cursor);
            }
        }
    ...
    if (!comp()->isDLT()) {
        ...
        if (comp()->target().cpu.isAtLeast(OMR_PROCESSOR_PPC_GP))
            ...
            cursor = generateConditionalBranchInstruction(cg(), TR::InstOpCode::ble, PPCOpProp_BranchUnlikely,
                firstNode, snippetLabel, cr0, cursor);
        else
            cursor
                = generateConditionalBranchInstruction(cg(), TR::InstOpCode::ble, firstNode, snippetLabel, cr0, cursor);
        TR::Snippet *snippet
            = new (trHeapMemory()) TR::PPCStackCheckFailureSnippet(cg(), firstNode, reStartLabel, snippetLabel);
    ...
    if (intSavedFirst <= TR::RealRegister::LastGPR) {
        if (comp()->target().is64Bit() || (TR::RealRegister::LastGPR - intSavedFirst <= 3)) {
            for (regIndex = intSavedFirst; regIndex <= TR::RealRegister::LastGPR;
                 regIndex = (TR::RealRegister::RegNum)((uint32_t)regIndex + 1)) {
                cursor = generateMemSrc1Instruction(cg(), TR::InstOpCode::Op_st, firstNode,
                    TR::MemoryReference::createWithDisplacement(cg(), stackPtr, argSize,
                        TR::Compiler->om.sizeofReferenceAddress()),
                    machine->getRealRegister(regIndex), cursor);

                argSize = argSize + TR::Compiler->om.sizeofReferenceAddress();
            }
        } else {
            cursor = generateMemSrc1Instruction(cg(),
                (intSavedFirst == TR::RealRegister::LastGPR) ? TR::InstOpCode::stw : TR::InstOpCode::stmw, firstNode,
                TR::MemoryReference::createWithDisplacement(cg(), stackPtr, argSize,
                    4 * (TR::RealRegister::LastGPR - intSavedFirst + 1)),
                machine->getRealRegister(intSavedFirst), cursor);
        ...
        if ((numLocalsToBeInitialized > 0) || atlas->getInternalPointerMap()) {
            ...
            if (numLocalsToBeInitialized > 5) {
                ...
                if (residualSize >= -LOWER_IMMED) {
                    cursor = generateTrg1Src2Instruction(cg(), TR::InstOpCode::add, firstNode, gr12, gr12, stackPtr,
                        cursor);
                    ...
                    if (initBottomOffset < LOWER_IMMED) {
                        if (0x00008000 == HI_VALUE(initBottomOffset)) {
                            cursor = generateTrg1Src1ImmInstruction(cg(), TR::InstOpCode::addis, firstNode, gr12, gr12,
                                0x7FFF, cursor);
                            cursor = generateTrg1Src1ImmInstruction(cg(), TR::InstOpCode::addis, firstNode, gr12, gr12,
                                0x1, cursor);
                        } else {
                            cursor = generateTrg1Src1ImmInstruction(cg(), TR::InstOpCode::addis, firstNode, gr12, gr12,
                                HI_VALUE(initBottomOffset), cursor);
                        }
                    }

                    if (initBottomOffset & 0xFFFF)
                        cursor = generateTrg1Src1ImmInstruction(cg(), TR::InstOpCode::addi2, firstNode, gr12, gr12,
                            LO_VALUE(initBottomOffset), cursor);
                } else {
                    adjustedFrameOnce = true;
                    if (size == residualSize) // only happens when LR is not saved
                        residualSize -= TR::Compiler->om.sizeofReferenceAddress();
                ...
                if (!disableUnrollPrologueInitLoop) {
                    ...
                    if (residualSize >= -LOWER_IMMED)
                        cursor = unrollPrologueInitLoop(cg(), firstNode, numLocalsToBeInitialized, loopDistance,
                            nullValueRegister, gr11, gr12, gr12, cr0, loopLabel, cursor);
                    else
                        cursor = unrollPrologueInitLoop(cg(), firstNode, numLocalsToBeInitialized, initSlotOffset,
                            nullValueRegister, gr11, stackPtr, gr12, cr0, loopLabel, cursor);
                } else {
                    ...
                    if (residualSize >= -LOWER_IMMED)
                        cursor = generateMemSrc1Instruction(cg(), TR::InstOpCode::Op_stx, firstNode,
                            TR::MemoryReference::createWithIndexReg(cg(), gr12, gr11,
                                TR::Compiler->om.sizeofReferenceAddress()),
                            nullValueRegister, cursor);
                    else
                        cursor = generateMemSrc1Instruction(cg(), TR::InstOpCode::Op_stx, firstNode,
                            TR::MemoryReference::createWithIndexReg(cg(), stackPtr, gr11,
                                initSlotOffset + TR::Compiler->om.sizeofReferenceAddress()),
                            nullValueRegister, cursor);
                    cursor = generateTrg1Src1ImmInstruction(cg(), TR::InstOpCode::addic_r, firstNode, gr11, gr11, cr0,
                        TR::Compiler->om.sizeofReferenceAddress(), cursor);
                    cursor = generateConditionalBranchInstruction(cg(), TR::InstOpCode::bne, firstNode, loopLabel, cr0,
                        cursor);
                }
            } else {
                adjustedFrameOnce = true;
                ...
                if (!disableUnrollPrologueInitLoop && use8BytesOn32Bit) {
                    ...
                    if (numLocalsToBeInitialized >= 2) {
                        cursor = generateMemSrc1Instruction(cg(), TR::InstOpCode::std, firstNode,
                            TR::MemoryReference::createWithDisplacement(cg(), stackPtr, offset + residualSize,
                                TR::Compiler->om.sizeofReferenceAddress()),
                            nullValueRegister, cursor);
                        count++;
                        if (numLocalsToBeInitialized >= 4) {
                            cursor = generateMemSrc1Instruction(cg(), TR::InstOpCode::std, firstNode,
                                TR::MemoryReference::createWithDisplacement(cg(), stackPtr, offset + residualSize + 8,
                                    TR::Compiler->om.sizeofReferenceAddress()),
                                nullValueRegister, cursor);
                            count++;
                        }
                    ...
                    if (numLocalsToBeInitialized % 2 == 1)
                        cursor = generateMemSrc1Instruction(cg(), TR::InstOpCode::Op_st, firstNode,
                            TR::MemoryReference::createWithDisplacement(cg(), stackPtr,
                                offset + residualSize + (count * 8), TR::Compiler->om.sizeofReferenceAddress()),
                            nullValueRegister, cursor);
                } else {
                    for (i = 0; i < numLocalsToBeInitialized;
                         i++, offset += TR::Compiler->om.sizeofReferenceAddress()) {
                        cursor = generateMemSrc1Instruction(cg(), TR::InstOpCode::Op_st, firstNode,
                            TR::MemoryReference::createWithDisplacement(cg(), stackPtr, offset + residualSize,
                                TR::Compiler->om.sizeofReferenceAddress()),
                            nullValueRegister, cursor);
            ...
            if (atlas->getInternalPointerMap()) {
                ...
                ListIterator<TR_InternalPointerPair> internalPtrIt(
                    &atlas->getInternalPointerMap()->getInternalPointerPairs());
                for (TR_InternalPointerPair *internalPtrPair = internalPtrIt.getFirst(); internalPtrPair;
                    ...
                    for (TR_InternalPointerPair *seenInternalPtrPair = seenInternalPtrIt.getFirst();
                        ...
                        if (internalPtrPair->getPinningArrayPointer()
                ...
                //
                int32_t numSlotsToBeInitialized
                    = numDistinctPinningArrays + atlas->getInternalPointerMap()->getNumInternalPointers();
                ...
                if (!disableUnrollPrologueInitLoop) {
                    TR::LabelSymbol *loopLabel = generateLabelSymbol(cg());
                    cursor = unrollPrologueInitLoop(cg(), firstNode, numSlotsToBeInitialized, offset + residualSize,
                        nullValueRegister, gr11, stackPtr, gr12, cr0, loopLabel, cursor);
                } else {
    ...
    while (localCursor != NULL) {
        TR_ASSERT(!comp()->useCompressedPointers() || !localCursor->isLocalObject()
...
TR::MemoryReference *J9::Power::PrivateLinkage::getOutgoingArgumentMemRef(int32_t argSize, TR::Register *argReg,
    TR::InstOpCode::Mnemonic opCode, TR::PPCMemoryArgument &memArg, uint32_t length)
{
    TR::MemoryReference *result
        = TR::MemoryReference::createWithDisplacement(cg(), cg()->getStackPointerRegister(), argSize, length);
...
void J9::Power::PrivateLinkage::createEpilogue(TR::Instruction *cursor)
{
    ...
    TR::RealRegister::RegNum regIndex;
    bool restoreLR = (cg()->getSnippetList().size() > 1 || (comp()->isDLT() && !cg()->getSnippetList().empty())
        || bodySymbol->isEHAware() || cg()->canExceptByTrap() || machine->getLinkRegisterKilled());

    bool saveLR = restoreLR || machine->getLinkRegisterKilled();

    if (restoreLR && frameSize <= UPPER_IMMED) {
        cursor = generateTrg1MemInstruction(cg(), TR::InstOpCode::Op_load, currentNode, gr0,
            TR::MemoryReference::createWithDisplacement(cg(), stackPtr, frameSize,
                TR::Compiler->om.sizeofReferenceAddress()),
            cursor);
    ...
    while (
        savedFirst <= TR::RealRegister::LastGPR && !machine->getRealRegister(savedFirst)->getHasBeenAssignedInMethod())
    ...
    if (savedFirst <= TR::RealRegister::LastGPR) {
        if (comp()->target().is64Bit() || (TR::RealRegister::LastGPR - savedFirst <= 3)) {
            for (regIndex = savedFirst; regIndex <= TR::RealRegister::LastGPR;
                 regIndex = (TR::RealRegister::RegNum)((uint32_t)regIndex + 1)) {
                cursor = generateTrg1MemInstruction(cg(), TR::InstOpCode::Op_load, currentNode,
                    machine->getRealRegister(regIndex),
                    TR::MemoryReference::createWithDisplacement(cg(), stackPtr, saveSize,
                        TR::Compiler->om.sizeofReferenceAddress()),
                    cursor);
            ...
            cursor = generateTrg1MemInstruction(cg(),
                (savedFirst == TR::RealRegister::LastGPR) ? TR::InstOpCode::lwz : TR::InstOpCode::lmw, currentNode,
                machine->getRealRegister(savedFirst),
                TR::MemoryReference::createWithDisplacement(cg(), stackPtr, saveSize,
                    4 * (TR::RealRegister::LastGPR - savedFirst + 1)),
                cursor);
    ...
    if (frameSize || saveLR) {
        ...
        if (saveSize > UPPER_IMMED) {
            ...
            cursor
                = generateTrg1Src2Instruction(cg(), TR::InstOpCode::add, currentNode, stackPtr, stackPtr, gr12, cursor);
        } else {
            cursor = generateTrg1Src1ImmInstruction(cg(), TR::InstOpCode::addi2, currentNode, stackPtr, stackPtr,
                saveSize, cursor);
        }

        if (restoreLR && frameSize > UPPER_IMMED) {
            cursor = generateTrg1MemInstruction(cg(), TR::InstOpCode::Op_load, currentNode, gr0,
                TR::MemoryReference::createWithDisplacement(cg(), stackPtr, -TR::Compiler->om.sizeofReferenceAddress(),
                    TR::Compiler->om.sizeofReferenceAddress()),
                cursor);
            cursor = generateSrc1Instruction(cg(), TR::InstOpCode::mtlr, currentNode, gr0, 0, cursor);
        }
...
int32_t J9::Power::PrivateLinkage::buildArgs(TR::Node *callNode, TR::RegisterDependencyConditions *dependencies)
{
...
int32_t J9::Power::PrivateLinkage::buildPrivateLinkageArgs(TR::Node *callNode,
    TR::RegisterDependencyConditions *dependencies, TR_LinkageConventions linkage)
{
    ...
    if (rightToLeft) {
        ...
        lastArgIndex = firstArgumentChild;
        step = -1;
    } else {
        firstRealArgIndex = firstArgumentChild;
    ...
    // if there is a special child, it will not be processed in this loop, due to the from value being incremented above.
    // special children are not arguments, but may be used for calling the J9Method
    for (int32_t i = firstRealArgIndex; (rightToLeft && i >= lastArgIndex) || (!rightToLeft && i <= lastArgIndex); i += step) {
    ...
    if (linkage == TR_CHelper) {
        ...
        generateTrg1Src1Instruction(cg(), TR::InstOpCode::mr, callNode, vmThreadArgRegister,
            cg()->getMethodMetaDataRegister());
        ...
        switch (child->getDataType()) {
                    ...
                    if (numIntegerArgs < numIntArgRegs) {
                        ...
                        if (numIntegerArgs == firstExplicitArg
                            ...
                            if (resType.isAddress())
                                ...
                                resultReg = cg()->allocateRegister();
                            dependencies->addPreCondition(argRegister,
                            ...
                            if (firstExplicitArg == 1)
                                dependencies->addPostCondition(argRegister,
                                    properties.getIntegerArgumentRegister(numIntegerArgs));
                        } else if (comp->target().is32Bit() && numIntegerArgs == (firstExplicitArg + 1)
                            && resType.isInt64()) {
                            ...
                            TR::addDependency(dependencies, argRegister,
                                properties.getIntegerArgumentRegister(numIntegerArgs), TR_GPR, cg());
                        }
                    } else // numIntegerArgs >= numIntArgRegs
                    {
                        if (child->getDataType() == TR::Address)
                            mref = getOutgoingArgumentMemRef(totalSize - argSize, argRegister, TR::InstOpCode::Op_st,
                                pushToMemory[argIndex++], TR::Compiler->om.sizeofReferenceAddress());
                        else
                            mref = getOutgoingArgumentMemRef(totalSize - argSize, argRegister, TR::InstOpCode::stw,
                                pushToMemory[argIndex++], 4);
                    }
                    numIntegerArgs++;
                }
                break;
            case TR::Int64:
                ...
                if (isSpecialArg) {
                    ...
                    TR::Register *specialArgRegister
                        = argRegister->getRegisterPair() ? argRegister->getLowOrder() : argRegister;
                    if (specialArgReg == properties.getIntegerReturnRegister(0)) {
                        TR::Register *resultReg;
                        if (resType.isAddress())
                            resultReg = cg()->allocateCollectedReferenceRegister();
                    ...
                    if (numIntegerArgs < numIntArgRegs) {
                        if (!cg()->canClobberNodesRegister(child, 0)) {
                            if (comp->target().is64Bit()) {
                                if (argRegister->containsCollectedReference())
                                    ...
                                    tempRegister = cg()->allocateRegister();
                                generateTrg1Src1Instruction(cg(), TR::InstOpCode::mr, callNode, tempRegister,
                                    argRegister);
                                argRegister = tempRegister;
                            } else {
                                tempRegister = cg()->allocateRegister();
                                generateTrg1Src1Instruction(cg(), TR::InstOpCode::mr, callNode, tempRegister,
                                    argRegister->getRegisterPair()->getHighOrder());
                                argRegister = cg()->allocateRegisterPair(argRegister->getRegisterPair()->getLowOrder(),
                                    tempRegister);
                        ...
                        if (numIntegerArgs == firstExplicitArg
                            ...
                            if (comp->target().is64Bit())
                                ...
                                dependencies->addPreCondition(argRegister->getRegisterPair()->getHighOrder(),
                                    properties.getIntegerArgumentRegister(numIntegerArgs));
                            dependencies->addPostCondition(resultReg, TR::RealRegister::gr3);
                            if (firstExplicitArg == 1)
                                dependencies->addPostCondition(argRegister,
                            ...
                            if (comp->target().is64Bit())
                                ...
                                TR::addDependency(dependencies, argRegister->getRegisterPair()->getHighOrder(),
                                    properties.getIntegerArgumentRegister(numIntegerArgs), TR_GPR, cg());
                        }
                        if (comp->target().is32Bit()) {
                            if (numIntegerArgs + 1 < numIntArgRegs) {
                                ...
                                if (numIntegerArgs == firstExplicitArg && resType.isInt64()) {
                                    TR::Register *resultReg = cg()->allocateRegister();
                                    dependencies->addPreCondition(argRegister->getRegisterPair()->getLowOrder(),
                                    ...
                                    TR::addDependency(dependencies, argRegister->getRegisterPair()->getLowOrder(),
                                        properties.getIntegerArgumentRegister(numIntegerArgs + 1), TR_GPR, cg());
                            } else // numIntegerArgs+1 == numIntArgRegs
                                mref = getOutgoingArgumentMemRef(totalSize - argSize + 4,
                                    argRegister->getRegisterPair()->getLowOrder(), TR::InstOpCode::stw,
                                    pushToMemory[argIndex++], 4);
                            numIntegerArgs++;
                        }
                    } else // numIntegerArgs >= numIntArgRegs
                    {
                        if (comp->target().is64Bit())
                            mref = getOutgoingArgumentMemRef(totalSize - argSize, argRegister, TR::InstOpCode::std,
                                pushToMemory[argIndex++], 8);
                        else {
                            mref = getOutgoingArgumentMemRef(totalSize - argSize,
                                argRegister->getRegisterPair()->getHighOrder(), TR::InstOpCode::stw,
                                pushToMemory[argIndex++], 4);
                            mref = getOutgoingArgumentMemRef(totalSize - argSize + 4,
                                argRegister->getRegisterPair()->getLowOrder(), TR::InstOpCode::stw,
                                pushToMemory[argIndex++], 4);
            ...
            case TR::Float:
                ...
                {
                    mref = getOutgoingArgumentMemRef(totalSize - argSize, argRegister, TR::InstOpCode::stfs,
                        pushToMemory[argIndex++], 4);
                }
                numFloatArgs++;
                break;
            case TR::Double:
                ...
                {
                    mref = getOutgoingArgumentMemRef(totalSize - argSize, argRegister, TR::InstOpCode::stfd,
                        pushToMemory[argIndex++], 8);
                }
                numFloatArgs++;
                break;
    ...
    for (int32_t i = TR::RealRegister::FirstGPR; i <= TR::RealRegister::LastGPR; ++i) {
        ...
        if (!dependencies->searchPreConditionRegister(realReg)) {
            if (realReg == properties.getIntegerArgumentRegister(0) && callNode->getDataType() == TR::Address) {
                ...
                // Helper linkage preserves all registers that are not argument registers, so we don't need to spill
                // them.
                if (linkage != TR_Helper)
    ...
    if (liveVSX || liveFPR) {
        while (numFloatArgs < 8) {
            TR::addDependency(dependencies, NULL,
                (TR::RealRegister::RegNum)((uint32_t)TR::RealRegister::fp0 + numFloatArgs), TR_FPR, cg());
            numFloatArgs++;
        }
        ...
        for (int32_t i = TR::RealRegister::fp8; i <= theLastOne; i++)
            if (!properties.getPreserved((TR::RealRegister::RegNum)i)) {
                TR::addDependency(dependencies, NULL, (TR::RealRegister::RegNum)i,
                    (i > TR::RealRegister::LastFPR) ? TR_VSX_SCALAR : TR_FPR, cg());
            }
    } else if (callNode->getType().isFloatingPoint()) {
        // add return floating-point register dependency
        TR::addDependency(dependencies, NULL, (TR::RealRegister::RegNum)getProperties().getFloatReturnRegister(),
            TR_FPR, cg());
    }
    ...
    If the callee function is in libj9jitxx.so module, (as in the case of CHelper functions are) then the module's
    TOC-base can be found in the J9VMThread jitTOC field. The TOC can be loaded from the jitTOC field of J9VMThread
    (pointed to by metaDataRegister).
    ...
    */
    if (linkage == TR_CHelper) {
        if (comp->target().isLinux() && comp->target().is64Bit() && comp->target().cpu.isLittleEndian()) {
            if (!comp->getOption(TR_DisableTOC) && !comp->compilePortableCode()) {
                ...
                generateTrg1MemInstruction(cg(), TR::InstOpCode::Op_load, callNode,
                    dependencies->searchPreConditionRegister(TR::RealRegister::gr12),
                    TR::MemoryReference::createWithDisplacement(cg(), cg()->getTOCBaseRegister(), helperOffset,
                        TR::Compiler->om.sizeofReferenceAddress()));
            } else {
                loadAddressConstant(cg(), callNode,
                    (int64_t)runtimeHelperValue((TR_RuntimeHelper)callNode->getSymbolReference()->getReferenceNumber()),
                    dependencies->searchPreConditionRegister(TR::RealRegister::gr12), NULL, false,
                    TR_AbsoluteHelperAddress);
            }

        } else if (comp->target().isAIX() || (comp->target().isLinux() && comp->target().is64Bit())) {
            generateTrg1MemInstruction(cg(), TR::InstOpCode::Op_load, callNode,
                dependencies->searchPreConditionRegister(TR::RealRegister::gr2),
    ...
    if (linkage == TR_Private || forcePreserveCCRs) {
        ...
        logprints(trace, log, "Omitting CCR save/restore for helper calls\n");
...
static bool getProfiledCallSiteInfo(TR::CodeGenerator *cg, TR::Node *callNode, uint32_t maxStaticPICs,
    TR_ScratchList<J9::PPCPICItem> &values)
{
    TR::Compilation *comp = cg->comp();
    ...
    if (methodSymbol->isVirtual())
        callSiteMethodClass
            = methodSymRef->getSymbol()->getResolvedMethodSymbol()->getResolvedMethod()->classOfMethod();
...
static TR::Instruction *buildStaticPICCall(TR::CodeGenerator *cg, TR::Node *callNode, uintptr_t profiledClass,
    TR_ResolvedMethod *profiledMethod, TR::Register *vftReg, TR::Register *tempReg, TR::Register *condReg,
    TR::LabelSymbol *missLabel, uint32_t regMapForGC)
{
    TR::Compilation *comp = cg->comp();
    TR::SymbolReference *methodSymRef = callNode->getSymbolReference();
    TR::SymbolReference *profiledMethodSymRef = comp->getSymRefTab()->findOrCreateMethodSymbol(
        methodSymRef->getOwningMethodIndex(), -1, profiledMethod, TR::MethodSymbol::Virtual);

    TR_J9VMBase *fej9 = (TR_J9VMBase *)(cg->fe());
    loadAddressConstant(cg, comp->compileRelocatableCode(), callNode, profiledClass, tempReg, NULL,
        fej9->isUnloadAssumptionRequired((TR_OpaqueClassBlock *)profiledClass, comp->getCurrentMethod()));
    ...
    TR::Instruction *gcPoint = generateDepImmSymInstruction(cg, TR::InstOpCode::bl, callNode,
        (uintptr_t)profiledMethod->startAddressForJittedMethod(),
...
static void buildVirtualCall(TR::CodeGenerator *cg, TR::Node *callNode, TR::Register *vftReg, TR::Register *gr12,
    uint32_t regMapForGC)
{
    ...
    if (offset < LOWER_IMMED || offset > UPPER_IMMED) {
        TR_ASSERT_FATAL_WITH_NODE(callNode, 0x00008000 != HI_VALUE(offset),
            "offset (0x%x) is unexpectedly high. Can not encode upper 16 bits into an addis instruction.", offset);
        ...
        generateTrg1MemInstruction(cg, TR::InstOpCode::Op_load, callNode, gr12,
            TR::MemoryReference::createWithDisplacement(cg, gr12, LO_VALUE(offset),
                TR::Compiler->om.sizeofReferenceAddress()));
    } else {
        generateTrg1MemInstruction(cg, TR::InstOpCode::Op_load, callNode, gr12,
            TR::MemoryReference::createWithDisplacement(cg, vftReg, offset, TR::Compiler->om.sizeofReferenceAddress()));
    }
...
static void buildInterfaceCall(TR::CodeGenerator *cg, TR::Node *callNode, TR::Register *vftReg, TR::Register *gr0,
    TR::Register *gr11, TR::Register *gr12, TR::Register *cr0, TR::PPCInterfaceCallSnippet *ifcSnippet,
    uint32_t regMapForGC)
{
    TR::Compilation *comp = cg->comp();
    TR_J9VMBase *fej9 = (TR_J9VMBase *)(cg->fe());
    ...
    if (comp->target().is64Bit()) {
        if (comp->target().cpu.isAtLeast(OMR_PROCESSOR_PPC_P10)) {
            ...
            generateTrg1MemInstruction(cg, TR::InstOpCode::paddi, callNode, gr12,
                TR::MemoryReference::createWithLabel(cg, ifcSnippet->getSnippetLabel(),
                    12 + 4 * TR::Compiler->om.sizeofReferenceAddress(), TR::Compiler->om.sizeofReferenceAddress()));
            generateTrg1MemInstruction(cg, TR::InstOpCode::Op_load, callNode, gr11,
                TR::MemoryReference::createWithDisplacement(cg, gr12, 0, TR::Compiler->om.sizeofReferenceAddress()));
        } else {
            ...
            if (beginIndex != PTOC_FULL_INDEX) {
                ...
                if (beginIndex < LOWER_IMMED || beginIndex > UPPER_IMMED) {
                    TR_ASSERT_FATAL_WITH_NODE(callNode, 0x00008000 != HI_VALUE(beginIndex),
                        "TOC offset (0x%x) is unexpectedly high. Can not encode upper 16 bits into an addis "
                        "instruction.",
                        beginIndex);
                    generateTrg1Src1ImmInstruction(cg, TR::InstOpCode::addis, callNode, gr12, cg->getTOCBaseRegister(),
                        HI_VALUE(beginIndex));
                    generateTrg1MemInstruction(cg, TR::InstOpCode::Op_load, callNode, gr12,
                        TR::MemoryReference::createWithDisplacement(cg, gr12, LO_VALUE(beginIndex),
                            TR::Compiler->om.sizeofReferenceAddress()));
                } else {
                    generateTrg1MemInstruction(cg, TR::InstOpCode::Op_load, callNode, gr12,
                        TR::MemoryReference::createWithDisplacement(cg, cg->getTOCBaseRegister(), beginIndex,
                            TR::Compiler->om.sizeofReferenceAddress()));
                }
                generateTrg1MemInstruction(cg, TR::InstOpCode::Op_load, callNode, gr11,
                    TR::MemoryReference::createWithDisplacement(cg, gr12, 0,
                        TR::Compiler->om.sizeofReferenceAddress()));
            } else {
                TR::Instruction *q[4];
                fixedSeqMemAccess(cg, callNode, 0, q, gr11, gr12, TR::InstOpCode::Op_loadu,
                    TR::Compiler->om.sizeofReferenceAddress(), NULL, gr11);
        ...
        ifcSnippet->setLowerInstruction(generateTrg1MemInstruction(cg, TR::InstOpCode::lwzu, callNode, gr11,
            TR::MemoryReference::createWithDisplacement(cg, gr12, 0, 4)));
    }
    ...
    generateTrg1MemInstruction(cg, TR::InstOpCode::Op_loadu, callNode, gr11,
        TR::MemoryReference::createWithDisplacement(cg, gr12, 2 * TR::Compiler->om.sizeofReferenceAddress(),
            TR::Compiler->om.sizeofReferenceAddress()));
    generateTrg1Src2Instruction(cg, TR::InstOpCode::Op_cmpl, callNode, cr0, vftReg, gr11);

#ifdef INLINE_LASTITABLE_CHECK
    // Doing this check inline doesn't perform too well because it prevents the PIC cache slots from being populated
    // with the best candidates. This check is done in the _interfaceSlotsUnavailable helper instead.
    ...
    TR_ASSERT(fej9->getITableEntryJitVTableOffset() <= UPPER_IMMED
        ...
        "ITable offset for JIT is too large, prevents lastITable dispatch from being generated");
    ...
    if (!comp->getOption(TR_DisableLastITableCache) && interfaceClassOfMethod &&
        ...
        fej9->getITableEntryJitVTableOffset() <= UPPER_IMMED && fej9->getITableEntryJitVTableOffset() >= LOWER_IMMED) {
        generateConditionalBranchInstruction(cg, TR::InstOpCode::beq, callNode, hitLabel, cr0);

        // Check if the lastITable belongs to the interface class we're using at this call site
        //
        generateTrg1MemInstruction(cg, TR::InstOpCode::Op_load, callNode, gr11,
            TR::MemoryReference::createWithDisplacement(cg, vftReg, fej9->getOffsetOfLastITableFromClassField(),
                TR::Compiler->om.sizeofReferenceAddress()));
        // Load the interface class from the snippet rather than materializing it, again because we want to do this in a
        // single instruction
        generateTrg1MemInstruction(cg, TR::InstOpCode::Op_load, callNode, gr12,
            TR::MemoryReference::createWithDisplacement(cg, gr12, -4 * TR::Compiler->om.sizeofReferenceAddress(),
                TR::Compiler->om.sizeofReferenceAddress()));
        generateTrg1MemInstruction(cg, TR::InstOpCode::Op_load, callNode, gr0,
            TR::MemoryReference::createWithDisplacement(cg, gr11, fej9->getOffsetOfInterfaceClassFromITableField(),
                TR::Compiler->om.sizeofReferenceAddress()));
        ...
        //
        generateTrg1MemInstruction(cg, TR::InstOpCode::Op_load, callNode, gr12,
            TR::MemoryReference::createWithDisplacement(cg, gr11, fej9->convertITableIndexToOffset(itableIndex),
                TR::Compiler->om.sizeofReferenceAddress()));
    ...
    generateTrg1MemInstruction(cg, TR::InstOpCode::Op_load, callNode, gr11,
        TR::MemoryReference::createWithDisplacement(cg, gr12, TR::Compiler->om.sizeofReferenceAddress(),
            TR::Compiler->om.sizeofReferenceAddress()));
#ifdef INLINE_LASTITABLE_CHECK
...
void J9::Power::PrivateLinkage::buildVirtualDispatch(TR::Node *callNode, TR::RegisterDependencyConditions *dependencies,
    uint32_t sizeOfArguments)
{
        ...
        switch (methodSymbol->getMandatoryRecognizedMethod()) {
            ...
            case TR::com_ibm_jit_JITHelpers_dispatchVirtual: {
                ...
                if (!thunk) {
                    thunk = fej9->setJ2IThunk(j2iSignature, signatureLen,
                        TR::PPCCallSnippet::generateVIThunk(
            ...
            default:
                if (fej9->needsInvokeExactJ2IThunk(callNode, comp())) {
                    TR_MHJ2IThunk *thunk = TR::PPCCallSnippet::generateInvokeExactJ2IThunk(callNode, sizeOfArguments,
                        cg(), methodSymbol->getMethod()->signatureChars());
    ...
    if (!thunk)
        thunk = fej9->setJ2IThunk(methodSymbol->getMethod(),
            TR::PPCCallSnippet::generateVIThunk(callNode, sizeOfArguments, cg()), comp());
    ...
    if (methodSymbol->isVirtual()) {
        ...
        if (methodSymRef->isUnresolved() || comp()->compileRelocatableCode()) {
            ...
            TR::Snippet *snippet = new (trHeapMemory())
                TR::PPCVirtualUnresolvedSnippet(cg(), callNode, snippetLabel, sizeOfArguments, doneLabel);
            cg()->addSnippet(snippet);
        ...
        if (callIsSafe) {
            ...
            if (comp()->performVirtualGuardNOPing() && comp()->isVirtualGuardNOPingRequired()
                && !resolvedMethod->isInterpreted() && !callNode->isTheVirtualCallNodeForAGuardedInlinedCall()) {
                TR_VirtualGuard *virtualGuard = NULL;

                if (!resolvedMethod->virtualMethodIsOverridden() && !resolvedMethod->isAbstract()) {
                    virtualGuard
                        = TR_VirtualGuard::createGuardedDevirtualizationGuard(TR_NonoverriddenGuard, comp(), callNode);
                } else {
                    TR_DevirtualizedCallInfo *devirtualizedCallInfo = comp()->findDevirtualizedCall(callNode);
                    TR_OpaqueClassBlock *refinedThisClass
                        = devirtualizedCallInfo ? devirtualizedCallInfo->_thisType : NULL;
                    TR_OpaqueClassBlock *thisClass
                        = refinedThisClass ? refinedThisClass : resolvedMethod->containingClass();
                    ...
                    if (thisClass && TR::Compiler->cls.isAbstractClass(comp(), thisClass)
                        ...
                        TR_ResolvedMethod *calleeMethod = chTable->findSingleAbstractImplementer(thisClass,
                        ...
                        if (calleeMethod
                            ...
                            virtualGuard = TR_VirtualGuard::createGuardedDevirtualizationGuard(TR_AbstractGuard, comp(),
                                callNode);
                        }
                    } else if (refinedThisClass && resolvedMethod->virtualMethodIsOverridden()
                        ...
                        TR_ResolvedMethod *calleeMethod
                            = methodSymRef->getOwningMethod(comp())->getResolvedVirtualMethod(comp(), refinedThisClass,
                ...
                if (virtualGuard) {
                    ...
                    // We prefer to evaluate the VFT child in the out of line path if possible, but if we can't do it
                    // then, do it now instead
                    if (vftNode->getReferenceCount() > 1 || vftNode->getRegister()) {
                        vftReg = evaluateUpToVftChild(callNode, cg());
                        TR::addDependency(dependencies, vftReg, TR::RealRegister::NoReg, TR_GPR, cg());
                    } else {
                        // If we push evaluation of the VFT node off to the outlined code section we have to make sure
                        // that any of it's children that are needed later are still evaluated. We could, in theory skip
                        // evaluating children with refcount == 1 for the same reason as above, but would have to then
                        // look at the child's children, and so on, so just evaluate the VFT's children here and now.
                    ...
                    generateVirtualGuardNOPInstruction(cg(), callNode, virtualGuard->addNOPSite(),
                        new (trHeapMemory()) TR::RegisterDependencyConditions(0, 0, trMemory()), virtualCallLabel);
                    if (comp()->getOption(TR_EnableHCR)) {
                        TR_VirtualGuard *HCRGuard
                            = TR_VirtualGuard::createGuardedDevirtualizationGuard(TR_HCRGuard, comp(), callNode);
                        generateVirtualGuardNOPInstruction(cg(), callNode, HCRGuard->addNOPSite(),
                            new (trHeapMemory()) TR::RegisterDependencyConditions(0, 0, trMemory()), virtualCallLabel);
                    }
                    if (resolvedMethod != resolvedMethodSymbol->getResolvedMethod()) {
                        methodSymRef = comp()->getSymRefTab()->findOrCreateMethodSymbol(
                            methodSymRef->getOwningMethodIndex(), -1, resolvedMethod, TR::MethodSymbol::Virtual);
                    ...
                    TR::Instruction *gcPoint
                        = generateDepImmSymInstruction(cg(), TR::InstOpCode::bl, callNode, methodAddress,
                    ...
                    //
                    TR_PPCOutOfLineCodeSection *virtualCallOOL
                        = new (trHeapMemory()) TR_PPCOutOfLineCodeSection(virtualCallLabel, doneLabel, cg());
                    ...
                    if (!vftReg) {
                        ...
                        // Normally we don't care which reg the VFT gets, but in this case we don't want to undo the
                        // assignments done by the main deps so we choose a reg that we know will not be needed
    ...
    if (callIsSafe && !callNode->isTheVirtualCallNodeForAGuardedInlinedCall()
        ...
        if (getProfiledCallSiteInfo(cg(), callNode, maxStaticPICs, values)) {
            ...
            if (pic->_frequency > MAX_PROFILED_CALL_FREQUENCY) {
                ...
                TR_PPCOutOfLineCodeSection *slowCallOOL
                    = new (trHeapMemory()) TR_PPCOutOfLineCodeSection(slowCallLabel, doneLabel, cg());
                ...
                if (methodSymbol->isInterface()) {
                    ...
                    TR::PPCInterfaceCallSnippet *ifcSnippet = new (trHeapMemory()) TR::PPCInterfaceCallSnippet(cg(),
                        callNode, ifcSnippetLabel, sizeOfArguments, doneOOLLabel, (uint8_t *)thunk);
                ...
                generateDepLabelInstruction(cg(), TR::InstOpCode::label, callNode, doneOOLLabel,
                    new (trHeapMemory()) TR::RegisterDependencyConditions(0, 0, trMemory()));
                generateLabelInstruction(cg(), TR::InstOpCode::b, callNode, doneLabel);
                slowCallOOL->swapInstructionListsWithCompilation();
                ...
                while (pic) {
                    ...
                    buildStaticPICCall(cg(), callNode, (uintptr_t)pic->_clazz, pic->_method, vftReg, gr11, cr0,
    ...
    if (methodSymbol->isInterface()) {
        TR::LabelSymbol *ifcSnippetLabel = generateLabelSymbol(cg());
        TR::PPCInterfaceCallSnippet *ifcSnippet = new (trHeapMemory())
            TR::PPCInterfaceCallSnippet(cg(), callNode, ifcSnippetLabel, sizeOfArguments, doneLabel, (uint8_t *)thunk);
...
void inlineCharacterIsMethod(TR::Node *node, TR::MethodSymbol *methodSymbol, TR::CodeGenerator *cg,
    TR::LabelSymbol *&doneLabel)
{
    ...
    int32_t imm
        = (TR::RealRegister::CRCC_GT << TR::RealRegister::pos_RT | TR::RealRegister::CRCC_GT << TR::RealRegister::pos_RA
            | TR::RealRegister::CRCC_GT << TR::RealRegister::pos_RB);
...
void buildCRC32CCall(TR::Node *callNode, TR::RegisterDependencyConditions *deps, TR::MethodSymbol *methodSymbol,
    TR::CodeGenerator *cg, TR::LabelSymbol *&returnLabel, bool crc32m2, bool crc32m3)
{
    TR::Compilation *comp = cg->comp();
    TR::Register *gr2Reg;
    ...
    if (crc32m2) {
        ...
        generateTrg1Src1ImmInstruction(cg, TR::InstOpCode::addi2, callNode, addrArg, addrArg,
    ...
    deps->getPreConditions()->setDependencyInfo(map.getTargetIndex(TR::RealRegister::gr4), addrArg,
        TR::RealRegister::gr4, UsesDependentRegister);
    deps->getPostConditions()->setDependencyInfo(map.getTargetIndex(TR::RealRegister::gr4), addrArg,
        TR::RealRegister::gr4, UsesDependentRegister);

    deps->getPreConditions()->setDependencyInfo(map.getTargetIndex(TR::RealRegister::gr5), lenArg,
        TR::RealRegister::gr5, UsesDependentRegister);
    deps->getPostConditions()->setDependencyInfo(map.getTargetIndex(TR::RealRegister::gr5), lenArg,
        TR::RealRegister::gr5, UsesDependentRegister);

    deps->getPreConditions()->setDependencyInfo(map.getTargetIndex(TR::RealRegister::gr6), posArg,
        TR::RealRegister::gr6, UsesDependentRegister);
    deps->getPostConditions()->setDependencyInfo(map.getTargetIndex(TR::RealRegister::gr6), posArg,
        TR::RealRegister::gr6, UsesDependentRegister);

    if (crc32m3 && !comp->target().is64Bit()) {
        deps->getPreConditions()->setDependencyInfo(map.getTargetIndex(TR::RealRegister::gr7), wasteArg,
            TR::RealRegister::gr7, UsesDependentRegister);
        deps->getPostConditions()->setDependencyInfo(map.getTargetIndex(TR::RealRegister::gr7), wasteArg,
            TR::RealRegister::gr7, UsesDependentRegister);
    }

    if (comp->compileRelocatableCode()) {
        TR::SymbolReference *vpmsumSymRef
            = (comp)->getSymRefTab()->findOrCreateRuntimeHelper(TR_PPCcrc32_vpmsum, false, false, false);
        ...
        if (vpmsumSnippetLabel == NULL) {
            ...
            cg->addSnippet(
                new (cg->trHeapMemory()) TR::PPCHelperCallSnippet(cg, callNode, vpmsumSnippetLabel, vpmsumSymRef));
        }
        ...
        if (comp->target().isLinux() && comp->target().is64Bit() && comp->target().cpu.isLittleEndian()) {
            if (!comp->getOption(TR_DisableTOC) && !comp->compilePortableCode()) {
                ...
                loadAddressConstant(cg, callNode,
                    (int64_t)runtimeHelperValue((TR_RuntimeHelper)vpmsumSymRef->getReferenceNumber()), gr12Reg, NULL,
                    false, TR_AbsoluteHelperAddress);
            }
        } else if (comp->target().isAIX() || (comp->target().isLinux() && comp->target().is64Bit())) {
        ...
        if (aix_style_linkage
            ...
            generateTrg1MemInstruction(cg, TR::InstOpCode::Op_load, callNode, gr0Reg,
                TR::MemoryReference::createWithDisplacement(cg, gr12Reg, 0, TR::Compiler->om.sizeofReferenceAddress()));
            ...
            generateTrg1MemInstruction(cg, TR::InstOpCode::Op_load, callNode, gr11Reg,
                TR::MemoryReference::createWithDisplacement(cg, gr12Reg, 2 * TR::Compiler->om.sizeofReferenceAddress(),
                    TR::Compiler->om.sizeofReferenceAddress()));
        } else {
            // put the target address into the count register
...
void J9::Power::PrivateLinkage::buildDirectCall(TR::Node *callNode, TR::SymbolReference *callSymRef,
    TR::RegisterDependencyConditions *dependencies, const TR::PPCLinkageProperties &pp, int32_t argSize)
{
    ...
    if (!isJitDispatchJ9Method
        ...
        cg()->addSnippet(snippet);
        gcPoint = generateDepImmSymInstruction(cg(), TR::InstOpCode::bl, callNode, 0, dependencies,
    ...
    gcPoint->PPCNeedsGCMap(
        callSymbol->getLinkageConvention() == TR_Helper ? 0xffffffff : pp.getPreservedRegisterMapForGC());
}
...
TR::Register *J9::Power::PrivateLinkage::buildDirectDispatch(TR::Node *callNode)
{
    ...
    TR::RegisterDependencyConditions *dependencies = new (trHeapMemory()) TR::RegisterDependencyConditions(
        pp.getNumberOfDependencyGPRegisters(), pp.getNumberOfDependencyGPRegisters(), trMemory());
    ...
    if (!TR::Compiler->om.canGenerateArraylets() && !TR::Compiler->om.isOffHeapAllocationEnabled()
        ...
        && (callNode->getSymbol()->castToMethodSymbol()->getRecognizedMethod() == TR::java_util_zip_CRC32C_updateBytes
            || callNode->getSymbol()->castToMethodSymbol()->getRecognizedMethod()
                == TR::java_util_zip_CRC32C_updateDirectByteBuffer)) {
    ...
    switch (callNode->getOpCodeValue()) {
        ...
        case TR::lcall: {
            if (comp()->target().is64Bit())
                returnRegister = dependencies->searchPostConditionRegister(pp.getLongReturnRegister());
            else {
...
TR::Register *J9::Power::PrivateLinkage::buildIndirectDispatch(TR::Node *callNode)
{
    const TR::PPCLinkageProperties &pp = getProperties();
    TR::RegisterDependencyConditions *dependencies = new (trHeapMemory()) TR::RegisterDependencyConditions(
        pp.getNumberOfDependencyGPRegisters() + 1, pp.getNumberOfDependencyGPRegisters() + 1, trMemory());
...
int32_t J9::Power::HelperLinkage::buildArgs(TR::Node *callNode, TR::RegisterDependencyConditions *dependencies)
{
    ...
    const TR::PPCLinkageProperties &properties)
{
    ...
    TR::MemoryReference *result = TR::MemoryReference::createWithDisplacement(cg(),
        machine->getRealRegister(properties.getNormalStackPointerRegister()), argSize + getOffsetToFirstParm(), length);
    memArg.argRegister = argReg;
    memArg.argMemory = result;
    memArg.opCode = opCode;
    return (result);
