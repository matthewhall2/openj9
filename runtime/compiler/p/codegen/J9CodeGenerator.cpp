...
void J9::Power::CodeGenerator::initialize()
{
    ...
    if (comp->target().cpu.isAtLeast(OMR_PROCESSOR_PPC_P8)
        && comp->target().cpu.supportsFeature(OMR_FEATURE_PPC_HAS_VSX) && comp->target().is64Bit()
        && !comp->getOption(TR_DisableFastStringIndexOf) && !TR::Compiler->om.canGenerateArraylets())
        cg->setSupportsInlineStringIndexOf();

    static bool disableIndexOfInlining = feGetEnv("TR_DisableStringIndexOfStringInlining") != NULL;
    if (comp->target().cpu.isAtLeast(OMR_PROCESSOR_PPC_P8)
        && comp->target().cpu.supportsFeature(OMR_FEATURE_PPC_HAS_VSX) && comp->target().is64Bit()
        && !comp->getOption(TR_DisableFastStringIndexOf) && !TR::Compiler->om.canGenerateArraylets()
        && !disableIndexOfInlining)
    ...
    if (comp->target().cpu.isAtLeast(OMR_PROCESSOR_PPC_P8)
        && comp->target().cpu.supportsFeature(OMR_FEATURE_PPC_HAS_VSX) && comp->target().is64Bit()
        && !TR::Compiler->om.canGenerateArraylets() && !disableStringInflateIntrinsic)
    ...
    if (!TR::Compiler->om.canGenerateArraylets() && comp->target().cpu.isAtLeast(OMR_PROCESSOR_PPC_P8)
    ...
    if (comp->target().cpu.isAtLeast(OMR_PROCESSOR_PPC_P8)
        && comp->target().cpu.supportsFeature(OMR_FEATURE_PPC_HAS_VSX) && !TR::Compiler->om.canGenerateArraylets()
        && comp->target().is64Bit()) {
    ...
    //
    if (!disableDualTLH && !comp->getOption(TR_DisableDualTLH) && !comp->compileRelocatableCode()
...
bool J9::Power::CodeGenerator::canEmitDataForExternallyRelocatableInstructions()
{
    ...
    // for OR'ing is that sometimes usefule information such as flags and hints can be stored during compilation in
    // these data fields). Hence, for the relocation to be applied correctly, we must ensure that the data fields inside
    // the instruction initially are zero.
#ifdef J9VM_OPT_JITSERVER
...
//
TR::Linkage *J9::Power::CodeGenerator::createLinkage(TR_LinkageConventions lc)
{
    TR::Linkage *linkage;
    switch (lc) {
        case TR_Private:
...
TR::Recompilation *J9::Power::CodeGenerator::allocateRecompilationInfo()
{
    return TR_PPCRecompilation::allocate(self()->comp());
}

void J9::Power::CodeGenerator::generateBinaryEncodingPrologue(TR_PPCBinaryEncodingData *data)
{
    ...
    if (data->recomp != NULL) {
        ...
        if (comp->getOption(TR_FullSpeedDebug) || comp->getOption(TR_SupportSwitchToInterpreter)) {
            ...
            if (methodSymbol->isJNI()) {
                uintptr_t JNIMethodAddress
                    = (uintptr_t)methodSymbol->getResolvedMethod()->startAddressForJNIMethod(comp);
                TR::Node *firstNode = comp->getStartTree()->getNode();
                if (comp->target().is64Bit()) {
                    int32_t highBits = (int32_t)(JNIMethodAddress >> 32), lowBits = (int32_t)JNIMethodAddress;
                    TR::Instruction *cursor = new (self()->trHeapMemory()) TR::PPCImmInstruction(TR::InstOpCode::dd,
                        firstNode, comp->target().cpu.isBigEndian() ? highBits : lowBits, NULL, self());
                    ...
                    new (self()->trHeapMemory())
                        TR::PPCImmInstruction(TR::InstOpCode::dd, firstNode, (int32_t)JNIMethodAddress, NULL, self());
                }
            }
        }
    }
...
void J9::Power::CodeGenerator::lowerTreeIfNeeded(TR::Node *node, int32_t childNumberOfNode, TR::Node *parent,
    TR::TreeTop *tt)
{
    J9::CodeGenerator::lowerTreeIfNeeded(node, childNumberOfNode, parent, tt);

    if ((node->getOpCode().isLeftShift() || node->getOpCode().isRightShift() || node->getOpCode().isRotate())
        && self()->needsNormalizationBeforeShifts() && !node->isNormalizedShift()) {
        ...
        if ((second->getOpCodeValue() != TR::iconst)
            && ((second->getOpCodeValue() != TR::iand) || (second->getSecondChild()->getOpCodeValue() != TR::iconst)
                || (second->getSecondChild()->getInt() != normalizationAmount))) {
            // Not normalized yet
            //
            TR::Node *normalizedNode
                = TR::Node::create(TR::iand, 2, second, TR::Node::create(second, TR::iconst, 0, normalizationAmount));
...
bool J9::Power::CodeGenerator::suppressInliningOfRecognizedMethod(TR::RecognizedMethod method)
{
    ...
    if (method == TR::java_lang_Math_fma_D || method == TR::java_lang_Math_fma_F
        || method == TR::java_lang_StrictMath_fma_D || method == TR::java_lang_StrictMath_fma_F) {
        return true;
    }
     ...
     */
    if (method == TR::java_util_zip_CRC32C_updateBytes || method == TR::java_util_zip_CRC32C_updateDirectByteBuffer) {
        return true;
    }
...
bool J9::Power::CodeGenerator::enableAESInHardwareTransformations()
{
    TR::Compilation *comp = self()->comp();
    if ((comp->target().cpu.getPPCSupportsAES()
            || (comp->target().cpu.supportsFeature(OMR_FEATURE_PPC_HAS_ALTIVEC)
...
void J9::Power::CodeGenerator::insertPrefetchIfNecessary(TR::Node *node, TR::Register *targetRegister)
{
    TR::Compilation *comp = self()->comp();
    ...
    if ((node->getOpCodeValue() == TR::aloadi && !comp->target().is64Bit())
        || (comp->target().is64Bit() && comp->useCompressedPointers() && node->getOpCodeValue() == TR::l2a
            && comp->getMethodHotness() >= scorching && TR::Compiler->om.compressedReferenceShiftOffset() == 0
        ...
        if (!disableHotFieldPrefetch && prefetchOffset >= 0) // Prefetch based on hot field information
        {
            ...
            while (!disableDelayPrefetch && tt && (tt->getNode()->getOpCodeValue() != TR::BBEnd)) {
                ...
                if (checkNode->getOpCodeValue() == TR::ifacmpeq && checkNode->getFirstChild() == node
            ...
            if (!canSkipNullChk) {
                ...
                // check for null
                generateTrg1Src1ImmInstruction(self(), TR::InstOpCode::cmpli4, node, condReg, targetRegister,
                    NULLVALUE);
                ...
                TR::RegisterDependencyConditions *deps
                    = new (self()->trHeapMemory()) TR::RegisterDependencyConditions(1, 2, self()->trMemory());
                ...
                if (comp->target().is64Bit() && !comp->useCompressedPointers()) {
                    TR::MemoryReference *tempMR
                        = TR::MemoryReference::createWithDisplacement(self(), targetRegister, prefetchOffset, 8);
                    generateTrg1MemInstruction(self(), TR::InstOpCode::ld, node, tempReg, tempMR);
                } else {
                    TR::MemoryReference *tempMR
                        = TR::MemoryReference::createWithDisplacement(self(), targetRegister, prefetchOffset, 4);
                    generateTrg1MemInstruction(self(), TR::InstOpCode::lwz, node, tempReg, tempMR);
                }

                TR::MemoryReference *targetMR
                    = TR::MemoryReference::createWithDisplacement(self(), tempReg, (int32_t)0, 4);
                ...
                TR_ASSERT(useTree->getNode()->getOpCodeValue() == TR::BBStart,
                    "Expecting a BBStart on the fall through\n");
                TR::Node *useNode = useTree->getNode();
                TR::Block *bb = useNode->getBlock();
                if (bb->isExtensionOfPreviousBlock()) // Survived the null check
                {
                    TR_PrefetchInfo *pf = new (self()->trHeapMemory()) TR_PrefetchInfo(
                        self()->getCurrentEvaluationTreeTop(), useTree, node, useNode, prefetchOffset, false);
            ...
            if (!disableHotFieldNextElementPrefetch) {
                ...
                if (comp->target().is32Bit()) {
                    if (!(firstChild->getOpCodeValue() == TR::aiadd && firstChild->getFirstChild()
                        ...
                        if (!fromRegLoad) {
                            ...
                            if (indexObject->getOpCode().isLoadConst()) {
                                ...
                                if (len >= LOWER_IMMED && len <= UPPER_IMMED)
                                    generateTrg1Src1ImmInstruction(self(), TR::InstOpCode::addi, node, pointerReg,
                                        baseReg, len);
                                else {
                                    indexReg = self()->allocateRegister();
                                    loadConstant(self(), node, len, indexReg);
                                    generateTrg1Src2Instruction(self(), TR::InstOpCode::add, node, pointerReg, baseReg,
                                        indexReg);
                ...
                // 64bit CR
                    ...
                    if (!(firstChild->getOpCodeValue() == TR::iu2l && firstChild->getFirstChild()
            ...
            if (!optDisabled) {
                ...
                if (pointerReg && (heapTop > comp->getOptions()->getHeapBase())) // Check for gencon
                {
                    ...
                    if (comp->target().is64Bit() && !comp->useCompressedPointers()) {
                        if (!prefetch2Ahead)
                            generateTrg1MemInstruction(self(), TR::InstOpCode::ld, node, temp3Reg,
                                TR::MemoryReference::createWithDisplacement(self(), pointerReg,
                                    (int32_t)TR::Compiler->om.sizeofReferenceField(), 8));
                        else
                            generateTrg1MemInstruction(self(), TR::InstOpCode::ld, node, temp3Reg,
                                TR::MemoryReference::createWithDisplacement(self(), pointerReg,
                                    (int32_t)(TR::Compiler->om.sizeofReferenceField() * 2), 8));
                    } else {
                        if (!prefetch2Ahead)
                            generateTrg1MemInstruction(self(), TR::InstOpCode::lwz, node, temp3Reg,
                                TR::MemoryReference::createWithDisplacement(self(), pointerReg,
                                    (int32_t)TR::Compiler->om.sizeofReferenceField(), 4));
                        else
                            generateTrg1MemInstruction(self(), TR::InstOpCode::lwz, node, temp3Reg,
                                TR::MemoryReference::createWithDisplacement(self(), pointerReg,
                                    (int32_t)(TR::Compiler->om.sizeofReferenceField() * 2), 4));
                    }

                    if (comp->getOptions()->getHeapBase() != 0) {
                        loadAddressConstant(self(), comp->compileRelocatableCode(), node,
                            (intptr_t)(comp->getOptions()->getHeapBase()), tempReg);
                        generateTrg1Src2Instruction(self(), TR::InstOpCode::cmpl4, node, condReg, temp3Reg, tempReg);
                        generateConditionalBranchInstruction(self(), TR::InstOpCode::blt, node, endCtrlFlowLabel,
                            condReg);
                    }
                    if (heapTop != 0xFFFFFFFF) {
                        loadAddressConstant(self(), comp->compileRelocatableCode(), node,
                            (intptr_t)(heapTop - prefetchOffset), tempReg);
                        generateTrg1Src2Instruction(self(), TR::InstOpCode::cmpl4, node, condReg, temp3Reg, tempReg);
                        generateConditionalBranchInstruction(self(), TR::InstOpCode::bgt, node, endCtrlFlowLabel,
                            condReg);
                    }
                    TR::MemoryReference *targetMR
                        = TR::MemoryReference::createWithDisplacement(self(), temp3Reg, (int32_t)0, 4);
                    targetMR->forceIndexedForm(node, self());
                    generateMemInstruction(self(), TR::InstOpCode::dcbt, node,
                        targetMR); // use dcbt for prefetch next element
        ...
        if (!disableStringObjPrefetch && 0 && node->getSymbolReference() && !node->getSymbolReference()->isUnresolved()
            ...
            if (fieldName && strstr(fieldName, "Ljava/lang/String;")) {
                TR::MemoryReference *targetMR
                    = TR::MemoryReference::createWithDisplacement(self(), targetRegister, (int32_t)0, 4);
                targetMR->forceIndexedForm(node, self());
                generateMemInstruction(self(), TR::InstOpCode::dcbt, node, targetMR);
            }
        }
    ...
    if ((node->getOpCodeValue() == TR::aloadi && !comp->target().is64Bit())
        ...
        if (!disableIteratorPrefetch) {
            // 32bit
            if (comp->target().is32Bit()) {
                if (!(firstChild && firstChild->getOpCodeValue() == TR::aiadd && firstChild->isInternalPointer()
                        && strstr(comp->fe()->sampleSignature(node->getOwningMethod(), 0, 0, self()->trMemory()),
            ...
            // 64bit cr
                ...
                if (!(firstChild && firstChild->getOpCodeValue() == TR::aladd && firstChild->isInternalPointer()
                        && strstr(comp->fe()->sampleSignature(node->getOwningMethod(), 0, 0, self()->trMemory()),
                            "java/util/ArrayList$Itr.next()"))) {
                    optDisabled = true;
                }
        ...
        // The GC will protected against the SEGV by adding a "tail-padding" page, but only when -XAggressive is
        // enabled!
        if (!optDisabled && comp->getOption(TR_AggressiveOpts)) {
            ...
            if (comp->useCompressedPointers()) {
                ...
                if (loopSize < 200) // comp->useCompressedPointers() is false && loopSize < 200.
                {
                    ...
                    < 300) // comp->useCompressedPointers() is false && loopsize >=200 && loopsize < 300.
                {
                    prefetchElementStride = 2;
                }
                // If comp->useCompressedPointers() is false and loopsize >= 300, prefetchElementStride does not get
                // changed.
            }
            ...
            if (baseReg && indexReg && loopSize > 0) {
                TR::Register *tempReg = self()->allocateRegister();
                generateTrg1Src1ImmInstruction(self(), TR::InstOpCode::addi, node, tempReg, indexReg,
                    prefetchElementStride * TR::Compiler->om.sizeofReferenceField());
                if (comp->target().is64Bit() && !comp->useCompressedPointers()) {
                    TR::MemoryReference *targetMR
                        = TR::MemoryReference::createWithIndexReg(self(), baseReg, tempReg, 8);
                    generateTrg1MemInstruction(self(), TR::InstOpCode::ld, node, tempReg, targetMR);
                } else {
                    TR::MemoryReference *targetMR
                        = TR::MemoryReference::createWithIndexReg(self(), baseReg, tempReg, 4);
                    generateTrg1MemInstruction(self(), TR::InstOpCode::lwz, node, tempReg, targetMR);
                }

                if (comp->useCompressedPointers()) {
                    generateShiftLeftImmediateLong(self(), node, tempReg, tempReg,
    ...
    } else if (node->getOpCodeValue() == TR::awrtbari && comp->getMethodHotness() >= scorching
        && comp->target().cpu.isAtLeast(OMR_PROCESSOR_PPC_P6)
        && (comp->target().is32Bit()
            || (comp->target().is64Bit() && comp->useCompressedPointers()
        ...
        int32_t prefetchOffset = comp->findPrefetchInfo(node);
        if (prefetchOffset >= 0) {
            TR::MemoryReference *targetMR = TR::MemoryReference::createWithDisplacement(self(), targetRegister,
                prefetchOffset, TR::Compiler->om.sizeofReferenceAddress());
            targetMR->forceIndexedForm(node, self());
            generateMemInstruction(self(), TR::InstOpCode::dcbt, node, targetMR);
        }
    }
...
bool J9::Power::CodeGenerator::supportsNonHelper(TR::SymbolReferenceTable::CommonNonhelperSymbol symbol)
{
    if (symbol == TR::SymbolReferenceTable::jitDispatchJ9MethodSymbol) {
        return feGetEnv("enableNewDispatchOnP") != NULL;
    }
