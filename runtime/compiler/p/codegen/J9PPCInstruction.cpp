    ...
    if (getOpCodeValue() == TR::InstOpCode::bl || getOpCodeValue() == TR::InstOpCode::b) {
        ...
        if (cg()->hasCodeCacheSwitched()) {
            ...
            if (calleeSymRef != NULL) {
                if (calleeSymRef->getReferenceNumber() >= TR_PPCnumRuntimeHelpers
                    && (calleeSymRef->getReferenceNumber() < cg()->symRefTab()->getNonhelperIndex(
                            TR::SymbolReferenceTable::firstPerCodeCacheHelperSymbol)
                        || calleeSymRef->getReferenceNumber() > cg()->symRefTab()->getNonhelperIndex(
                               TR::SymbolReferenceTable::lastPerCodeCacheHelperSymbol))) {
                ...
                *(int32_t *)cursor |= (imm - (intptr_t)cursor) & 0x03fffffc;
                ...
                if (refNum < TR_PPCnumRuntimeHelpers) {
                    targetAddress = TR::CodeCacheManager::instance()->findHelperTrampoline(refNum, (void *)cursor);
                        ...
                        >= cg()->symRefTab()->getNonhelperIndex(TR::SymbolReferenceTable::firstPerCodeCacheHelperSymbol)
                    && refNum <= cg()->symRefTab()->getNonhelperIndex(
                           TR::SymbolReferenceTable::lastPerCodeCacheHelperSymbol)) {
                    TR_ASSERT(cg()->hasCodeCacheSwitched(),
                        "Expecting per-codecache helper to be unreachable only when codecache was switched");
                    TR_CCPreLoadedCode helper = (TR_CCPreLoadedCode)(refNum
                        - cg()->symRefTab()->getNonhelperIndex(
                            TR::SymbolReferenceTable::firstPerCodeCacheHelperSymbol));
        ...
        if ((comp->compileRelocatableCode()
            ...
            if (sym && !sym->isHelper() && resolvedMethod && !callIsJ2ITransition) {
                cg()->addProjectSpecializedRelocation(cursor, (uint8_t *)getSymbolReference()->getMethodAddress(), NULL,
                    TR_MethodCallAddress, __FILE__, __LINE__, getNode());
            } else {
                cg()->addExternalRelocation(
                    TR::ExternalRelocation::create(cursor, (uint8_t *)getSymbolReference(), TR_HelperAddress, cg()),
