                ...
                if (intArgNum < linkageProperties.getNumIntArgRegs()) {
                    ...
                    if (comp->target().is32Bit()) {
                        if (intArgNum < linkageProperties.getNumIntArgRegs() - 1) {
                            buffer = storeArgumentItem(TR::InstOpCode::stw, buffer,
                                machine->getRealRegister(linkageProperties.getIntegerArgumentRegister(intArgNum + 1)),
                                offset + 4, cg);
                        }
                    }
                }
...
uint8_t *TR::PPCCallSnippet::setUpArgumentsInRegister(uint8_t *buffer, TR::Node *callNode, int32_t argSize,
    TR::CodeGenerator *cg)
{
    int32_t intArgNum = 0, floatArgNum = 0, offset;
    TR::Machine *machine = cg->machine();
        ...
        switch (child->getDataType()) {
            ...
            case TR::Int32:
                if (!linkageProperties.getRightToLeft())
                    offset -= TR::Compiler->om.sizeofReferenceAddress();
                if (intArgNum < linkageProperties.getNumIntArgRegs()) {
                    buffer = loadArgumentItem(TR::InstOpCode::lwz, buffer,
                        machine->getRealRegister(linkageProperties.getIntegerArgumentRegister(intArgNum)), offset, cg);
                }
                intArgNum++;
                if (linkageProperties.getRightToLeft())
            ...
            case TR::Address:
                if (!linkageProperties.getRightToLeft())
                    offset -= TR::Compiler->om.sizeofReferenceAddress();
                if (intArgNum < linkageProperties.getNumIntArgRegs()) {
                    buffer = loadArgumentItem(loadGPROp, buffer,
                        machine->getRealRegister(linkageProperties.getIntegerArgumentRegister(intArgNum)), offset, cg);
                }
                intArgNum++;
                if (linkageProperties.getRightToLeft())
            ...
            case TR::Float:
                if (!linkageProperties.getRightToLeft())
                    offset -= TR::Compiler->om.sizeofReferenceAddress();
                if (floatArgNum < linkageProperties.getNumFloatArgRegs()) {
                    buffer = loadArgumentItem(TR::InstOpCode::lfs, buffer,
                        machine->getRealRegister(linkageProperties.getFloatArgumentRegister(floatArgNum)), offset, cg);
                }
                floatArgNum++;
                if (linkageProperties.getRightToLeft())
            ...
            case TR::Double:
                if (!linkageProperties.getRightToLeft())
                    offset -= 2 * TR::Compiler->om.sizeofReferenceAddress();
                if (floatArgNum < linkageProperties.getNumFloatArgRegs()) {
                    buffer = loadArgumentItem(TR::InstOpCode::lfd, buffer,
                        machine->getRealRegister(linkageProperties.getFloatArgumentRegister(floatArgNum)), offset, cg);
                }
                floatArgNum++;
                if (linkageProperties.getRightToLeft())
...
TR_RuntimeHelper TR::PPCCallSnippet::getInterpretedDispatchHelper(TR::SymbolReference *methodSymRef, TR::DataType type,
    bool isSynchronised, bool &isNativeStatic, TR::CodeGenerator *cg)
{
    TR::Compilation *comp = cg->comp();
    ...
    if (methodSymRef->isUnresolved() || forceUnresolvedDispatch) {
        TR_ASSERT(!isJitInduceOSRCall || !forceUnresolvedDispatch,
        ...
        switch (type) {
            ...
            case TR::Address:
                if (comp->target().is64Bit())
                    return isSynchronised ? TR_PPCinterpreterSyncGPR3GPR4StaticGlue
                                          : TR_PPCinterpreterGPR3GPR4StaticGlue;
                else
                    return isSynchronised ? TR_PPCinterpreterSyncGPR3StaticGlue : TR_PPCinterpreterGPR3StaticGlue;
            case TR::Int64:
...
uint8_t *TR::PPCCallSnippet::emitSnippetBody()
{
    ...
    TR::SymbolReference *methodSymRef
        = (_realMethodSymbolReference) ? _realMethodSymbolReference : callNode->getSymbolReference();
    TR::MethodSymbol *methodSymbol = methodSymRef->getSymbol()->castToMethodSymbol();
    ...
    // we use "b" for induceOSR because we want the helper to think that it's been called from the mainline code and not
    // from the snippet.
    ...
    *(int32_t *)cursor = branchInstruction | ((helperAddress - (intptr_t)cursor) & 0x03fffffc);
    ...
    if (isNativeStatic) {
        ...
        *(int32_t *)cursor = 0x48000000 | ((intptr_t)(getCallRA() - (intptr_t)cursor) & 0x03fffffc);
        TR_ASSERT(gcMap().isGCSafePoint() && gcMap().getStackMap(),
            "Native static call snippets must have GC maps when preserving the link stack");
        ...
        if (methodSymRef->isUnresolved() || forceUnresolvedDispatch) {
            *(intptr_t *)cursor = 0;
            if (comp->getOption(TR_EnableHCR)) {
                ...
                cg()->addExternalRelocation(TR::ExternalRelocation::create((uint8_t *)cursor, NULL,
                                                (uint8_t *)needsFullSizeRuntimeAssumption, TR_HCR, cg()),
            ...
            *(intptr_t *)cursor = (intptr_t)methodSymbol->getMethodAddress();
            ...
            if (comp->compileRelocatableCode()) {
                cg()->addExternalRelocation(TR::ExternalRelocation::create(cursor,
                    ...
                    __FILE__, __LINE__, getNode());
                cg()->addExternalRelocation(TR::ExternalRelocation::create(cursor,
                                                (uint8_t *)methodSymbol->getMethodAddress(), TR_ResolvedTrampolines,
                                                cg()),
                    __FILE__, __LINE__, getNode());
            }
        }
...
uint8_t *TR::PPCUnresolvedCallSnippet::emitSnippetBody()
{
    ...
    TR::SymbolReference *methodSymRef
        = (_realMethodSymbolReference) ? _realMethodSymbolReference : getNode()->getSymbolReference();
    TR::MethodSymbol *methodSymbol = methodSymRef->getSymbol()->castToMethodSymbol();
...
uint8_t *TR::PPCVirtualSnippet::emitSnippetBody() { return (NULL); }

uint32_t TR::PPCVirtualSnippet::getLength(int32_t estimatedSnippetStart) { return (0); }
...
uint8_t *TR::PPCVirtualUnresolvedSnippet::emitSnippetBody()
{
    ...
    void *thunk
        = fej9->getJ2IThunk(callNode->getSymbolReference()->getSymbol()->castToMethodSymbol()->getMethod(), comp);
    ...
    *(int32_t *)cursor = 0x48000000 | (distance & 0x03fffffc);
    ...
    TR_ASSERT(gcMap().isGCSafePoint() && gcMap().getStackMap(),
        "Virtual call snippets must have GC maps when preserving the link stack");
    ...
    *(intptr_t *)cursor = (intptr_t)thunk;
    ...
    auto info = (TR_RelocationRecordInformation *)comp->trMemory()->allocateMemory(
    ...
    cg()->addExternalRelocation(
        TR::ExternalRelocation::create(j2iThunkRelocationPoint, (uint8_t *)info, NULL, TR_J2IVirtualThunkPointer, cg()),
        __FILE__, __LINE__, callNode);
...
uint8_t *TR::PPCInterfaceCallSnippet::emitSnippetBody()
{
    ...
    if (comp->target().is64Bit() && (((uint64_t)cursor % TR::Compiler->om.sizeofReferenceAddress()) == 0)) {
        // icallVMprJavaSendPatchupVirtual needs to determine if it was called for virtual dispatch as opposed to
        // interface dispatch To do that it checks for 'mtctr r12' at -8(LR), which points here when it's called for
        // interface dispatch in 64 bit mode We make sure it doesn't contain that particular bit pattern by accident
    ...
    *(int32_t *)cursor = 0x48000000 | (distance & 0x03fffffc);
    ...
    TR_ASSERT(gcMap().isGCSafePoint() && gcMap().getStackMap(),
        "Interface call snippets must have GC maps when preserving the link stack");
    gcMap().registerStackMap(cursor - PPC_INSTRUCTION_LENGTH, cg());
    cursor += PPC_INSTRUCTION_LENGTH;
    ...
    *(uintptr_t *)cursor = callNode->getSymbolReference()->getCPIndexForVM();
    ...
    if (comp->target().is64Bit()) {
        if (!comp->target().cpu.isAtLeast(OMR_PROCESSOR_PPC_P10)) {
            if (getTOCOffset() != PTOC_FULL_INDEX) {
                ...
                if (!comp->compileRelocatableCode()
                    ...
                    // If the high nibble is 0 and the next nibble's high bit is clear, change the first instruction to
                    // a nop and the third to a li Next nibble's high bit needs to be clear in order to use li (because
                    // li will sign extend the immediate)
                    ...
                    // We must take this path for all compiles that need to generate relocatable code (ex. AOT,
                    // outOfProcess). The immediate fields of relocatable instructions must be clear. This is because
                    // when performing the relocation, we OR the new address into the fields. So if the fields are not
                    // already clear, then OR'ing the new address can result in garbage data.
                    cg()->addExternalRelocation(
                        new (cg()->trHeapMemory()) TR::BeforeBinaryEncodingExternalRelocation(getUpperInstruction(),
                            (uint8_t *)(addrValue), (uint8_t *)fixedSequence4, TR_FixedSequenceAddress2, cg()),
                        __FILE__, __LINE__, callNode);
                }
            }
        }
        ...
        *patchAddress2 |= (int32_t)(intptr_t)cursor & 0x0000ffff;
        ...
        cg()->addExternalRelocation(
            new (cg()->trHeapMemory()) TR::ExternalOrderedPair32BitRelocation((uint8_t *)patchAddress1,
                (uint8_t *)patchAddress2, (uint8_t *)recordInfo, TR_AbsoluteMethodAddressOrderedPair, cg()),
            __FILE__, __LINE__, callNode);
    }
    ...
    *(intptr_t *)(cursor + 3 * TR::Compiler->om.sizeofReferenceAddress()) = (intptr_t)blAddress;
    ...
    cg()->addExternalRelocation(TR::ExternalRelocation::create(cursor + TR::Compiler->om.sizeofReferenceAddress(), NULL,
                                    TR_AbsoluteMethodAddress, cg()),
        __FILE__, __LINE__, callNode);

    // Register for relocation of the 2nd target address
    cg()->addExternalRelocation(TR::ExternalRelocation::create(cursor + 3 * TR::Compiler->om.sizeofReferenceAddress(),
                                    NULL, TR_AbsoluteMethodAddress, cg()),
        __FILE__, __LINE__, callNode);
    ...
    *(intptr_t *)cursor = (intptr_t)thunk;
    ...
    if (comp->compileRelocatableCode()) {
        ...
        cg()->addExternalRelocation(TR::ExternalRelocation::create(j2iThunkRelocationPoint, (uint8_t *)info, NULL,
...
uint8_t *TR::PPCCallSnippet::generateVIThunk(TR::Node *callNode, int32_t argSize, TR::CodeGenerator *cg)
{
    TR::Compilation *comp = cg->comp();
    TR_J9VMBase *fej9 = (TR_J9VMBase *)(comp->fe());
    int32_t codeSize = 4 * (instructionCountForArguments(callNode, cg) + (comp->target().is64Bit() ? 7 : 4))
        + 8; // Additional 4 bytes to hold size of thunk
    ...
    switch (callNode->getDataType()) {
        case TR::NoType:
            dispatcher = (intptr_t)cg->symRefTab()
                             ->findOrCreateRuntimeHelper(TR_PPCicallVMprJavaSendVirtual0)
                             ->getMethodAddress();
            break;
        case TR::Int32:
            dispatcher = (intptr_t)cg->symRefTab()
                             ->findOrCreateRuntimeHelper(TR_PPCicallVMprJavaSendVirtual1)
                             ->getMethodAddress();
            break;
        case TR::Address:
            if (comp->target().is64Bit())
                dispatcher = (intptr_t)cg->symRefTab()
                                 ->findOrCreateRuntimeHelper(TR_PPCicallVMprJavaSendVirtualJ)
                                 ->getMethodAddress();
            else
                dispatcher = (intptr_t)cg->symRefTab()
                                 ->findOrCreateRuntimeHelper(TR_PPCicallVMprJavaSendVirtual1)
        ...
        case TR::Float:
            dispatcher = (intptr_t)cg->symRefTab()
                             ->findOrCreateRuntimeHelper(TR_PPCicallVMprJavaSendVirtualF)
                             ->getMethodAddress();
            break;
        case TR::Double:
            dispatcher = (intptr_t)cg->symRefTab()
                             ->findOrCreateRuntimeHelper(TR_PPCicallVMprJavaSendVirtualD)
                             ->getMethodAddress();
            break;
        default:
            TR_ASSERT(0, "Bad return data type for a call node.  DataType was %s\n",
    ...
    *((int32_t *)thunk + 1) = buffer - returnValue; // patch offset for AOT relocation
    ...
    // NOTE: modification of the layout of the following will require a corresponding change in AOT relocation code
    // (codert/ppc/AOTRelocations.cpp)
    if (comp->target().is64Bit()) {
        ...
        *(int32_t *)buffer = 0x60840000 | (dispatcher & 0x0000ffff);
        ...
        if (!(dispatcher & 0x00008000)) {
            ...
            *(int32_t *)buffer = 0x64840000 | ((dispatcher >> 16) & 0x0000ffff);
            ...
            // lis gr4, upper
            *(int32_t *)buffer = 0x3c800000 | (((dispatcher >> 16) + (dispatcher & (1 << 15) ? 1 : 0)) & 0x0000ffff);
            buffer += 4;
    ...
    *(int32_t *)buffer = 0x4e800420;
    ...
    if (comp->target().is32Bit() && comp->compileRelocatableCode()
        && !((dispatcher & 0x80008000)
            == 0x80008000)) // Make size of thunk larger for AOT even if extra instruction is not generated at compile
                            // time.  The extra instruction could be needed for the runtime address.
        sizeThunk += 4;
...
TR_MHJ2IThunk *TR::PPCCallSnippet::generateInvokeExactJ2IThunk(TR::Node *callNode, int32_t argSize,
    TR::CodeGenerator *cg, char *signature)
{
    TR::Compilation *comp = cg->comp();
    TR_J9VMBase *fej9 = (TR_J9VMBase *)(comp->fe());
    int32_t codeSize = 4 * (instructionCountForArguments(callNode, cg) + (comp->target().is64Bit() ? 7 : 4))
    ...
    switch (callNode->getDataType()) {
        ...
        default:
            TR_ASSERT(0, "Bad return data type '%s' for call node [" POINTER_PRINTF_FORMAT "]\n",
                comp->getDebug()->getName(callNode->getDataType()), callNode);
    }

    dispatcher
        = (intptr_t)(cg->fej9()->getInvokeExactThunkHelperAddress(comp, dispatcherSymbol, callNode->getDataType()));
...
uint8_t *TR_Debug::printPPCArgumentsFlush(OMR::Logger *log, TR::Node *node, uint8_t *cursor, int32_t argSize)
{
        ...
        switch (child->getDataType()) {
            ...
            case TR::Int32:
                ...
                if (intArgNum < linkageProperties.getNumIntArgRegs()) {
                    ...
                    log->printf(", %d], ", offset);
                    print(log, machine->getRealRegister(linkageProperties.getIntegerArgumentRegister(intArgNum)),
                        TR_WordReg);
                    cursor += 4;
                }
                intArgNum++;
            ...
            case TR::Float:
                ...
                if (floatArgNum < linkageProperties.getNumFloatArgRegs()) {
                    ...
                    print(log, stackPtr, TR_WordReg);
                    log->printf(", %d], ", offset);
                    print(log, machine->getRealRegister(linkageProperties.getFloatArgumentRegister(floatArgNum)),
                        TR_WordReg);
                    cursor += 4;
                }
                floatArgNum++;
...
void TR_Debug::print(OMR::Logger *log, TR::PPCCallSnippet *snippet)
{
    ...
    TR::SymbolReference *methodSymRef = snippet->getRealMethodSymbolReference()
        ? snippet->getRealMethodSymbolReference()
    ...
    if (methodSymbol->isHelper() && methodSymRef->isOSRInductionHelper()) {
        ...
        if (methodSymbol->isVMInternalNative() || methodSymbol->isJITInternalNative()) {
            ...
            labelString
                = methodSymbol->isJNI() ? "Interpreted JNI Direct Call Snippet" : "Interpreted Direct Call Snippet";
...
void TR_Debug::print(OMR::Logger *log, TR::PPCUnresolvedCallSnippet *snippet)
{
    ...
    printPrefix(log, NULL, cursor, 4);
    log->printf(".long \t0x%08x\t\t; Offset | Flag | CP Index",
...
void TR_Debug::print(OMR::Logger *log, TR::PPCVirtualSnippet *snippet) {}

void TR_Debug::print(OMR::Logger *log, TR::PPCVirtualUnresolvedSnippet *snippet)
{
    uint8_t *cursor = snippet->getSnippetLabel()->getCodeLocation();
    TR::Node *callNode = snippet->getNode();
    ...
    printPrefix(log, NULL, cursor, sizeof(intptr_t));
    log->printf(".long \t" POINTER_PRINTF_FORMAT "\t\t; Call Site RA",
        (intptr_t)snippet->getReturnLabel()->getCodeLocation());
    cursor += sizeof(intptr_t);

    printPrefix(log, NULL, cursor, sizeof(intptr_t));
    log->printf(".long \t" POINTER_PRINTF_FORMAT "\t\t; Constant Pool Pointer",
        (intptr_t)getOwningMethod(callNode->getSymbolReference())->constantPool());
    cursor += sizeof(intptr_t);

    printPrefix(log, NULL, cursor, sizeof(intptr_t));
    log->printf(".long \t" POINTER_PRINTF_FORMAT "\t\t; Constant Pool Index",
        callNode->getSymbolReference()->getCPIndexForVM());
    cursor += sizeof(intptr_t);
...
void TR_Debug::print(OMR::Logger *log, TR::PPCInterfaceCallSnippet *snippet)
{
    uint8_t *cursor = snippet->getSnippetLabel()->getCodeLocation();
    TR::Node *callNode = snippet->getNode();
