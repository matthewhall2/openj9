...
TR::PPCInterfaceCastSnippet::PPCInterfaceCastSnippet(TR::CodeGenerator *cg, TR::Node *n, TR::LabelSymbol *restartLabel,
    TR::LabelSymbol *snippetLabel, TR::LabelSymbol *trueLabel, TR::LabelSymbol *falseLabel, TR::LabelSymbol *doneLabel,
    TR::LabelSymbol *callLabel, bool testCastClassIsSuper, bool checkCast, int32_t offsetClazz,
    int32_t offsetCastClassCache, bool needsResult)
    : TR::Snippet(cg, n, snippetLabel, false)
    , _restartLabel(restartLabel)
    , _trueLabel(trueLabel)
    , _falseLabel(falseLabel)
    , _doneLabel(doneLabel)
    , _callLabel(callLabel)
    , _testCastClassIsSuper(testCastClassIsSuper)
    , _checkCast(checkCast)
    , _offsetClazz(offsetClazz)
    , _offsetCastClassCache(offsetCastClassCache)
    , _needsResult(needsResult)
{}

uint8_t *TR::PPCInterfaceCastSnippet::emitSnippetBody()
{
        ...
        *(int32_t *)cursor |= _offsetCastClassCache & 0xffff;
        ...
        if (_testCastClassIsSuper) {
            ...
            *(int32_t *)cursor |= (branchDistance & 0x0fffc);
            ...
            TR_ASSERT(comp->target().cpu.isTargetWithinIFormBranchRange((intptr_t)getReStartLabel()->getCodeLocation(),
                          (intptr_t)cursor),
            ...
            *(int32_t *)cursor |= (branchDistance & 0x03fffffc);
            ...
            TR_ASSERT(comp->target().cpu.isTargetWithinIFormBranchRange((intptr_t)_doneLabel->getCodeLocation(),
                          (intptr_t)cursor),
                "backward jump in Interface Cache is too long\n");
            *(int32_t *)cursor |= (branchDistance & 0x03fffffc);
            cursor += PPC_INSTRUCTION_LENGTH;
            ...
            *(int32_t *)cursor |= (branchDistance & 0x0fffc);
            ...
            TR_ASSERT(comp->target().cpu.isTargetWithinIFormBranchRange((intptr_t)getDoneLabel()->getCodeLocation(),
                          (intptr_t)cursor),
                "backward jump in Interface Cache is too long\n");
            *(int32_t *)cursor |= (branchDistance & 0x03fffffc);
            cursor += PPC_INSTRUCTION_LENGTH;
            ...
            TR_ASSERT(comp->target().cpu.isTargetWithinIFormBranchRange((intptr_t)_callLabel->getCodeLocation(),
                          (intptr_t)cursor),
                "backward jump in Interface Cache is too long\n");
            *(int32_t *)cursor |= (branchDistance & 0x03fffffc);
            cursor += PPC_INSTRUCTION_LENGTH;
    ...
    {
        auto firstArgReg = getNode()->getSymbol()->castToMethodSymbol()->getLinkageConvention() == TR_CHelper
            ? TR::RealRegister::gr4
            : TR::RealRegister::gr3;
        TR::RegisterDependencyConditions *deps = _doneLabel->getInstruction()->getDependencyConditions();
        TR::RealRegister *castClassReg
            = cg()->machine()->getRealRegister(static_cast<TR::RealRegister::RegNum>(firstArgReg));
        TR::RealRegister *objClassReg
            = cg()->machine()->getRealRegister(static_cast<TR::RealRegister::RegNum>(firstArgReg + 3));
        TR::RealRegister *resultReg
            = cg()->machine()->getRealRegister(static_cast<TR::RealRegister::RegNum>(firstArgReg + 5));
        TR::RealRegister *cndReg = cg()->machine()->getRealRegister(TR::RealRegister::cr0);
        TR::RealRegister *scratch1Reg
            = cg()->machine()->getRealRegister(static_cast<TR::RealRegister::RegNum>(firstArgReg + 2));
        TR::RealRegister *scratch2Reg
            = cg()->machine()->getRealRegister(static_cast<TR::RealRegister::RegNum>(firstArgReg + 4));
            ...
            *(int32_t *)cursor |= 0x1; // Rc = 1
            ...
            if (_falseLabel != _trueLabel) {
                ...
                *(int32_t *)cursor |= (branchDistance & 0x0fffc);
                ...
                TR_ASSERT(comp->target().cpu.isTargetWithinIFormBranchRange((intptr_t)_trueLabel->getCodeLocation(),
                              (intptr_t)cursor),
                    "backward jump in Interface Cache is too long\n");
                *(int32_t *)cursor |= (branchDistance & 0x03fffffc);
                cursor += PPC_INSTRUCTION_LENGTH;
                ...
                TR_ASSERT(comp->target().cpu.isTargetWithinIFormBranchRange((intptr_t)_falseLabel->getCodeLocation(),
                              (intptr_t)cursor),
                    "backward jump in Interface Cache is too long\n");
                *(int32_t *)cursor |= (branchDistance & 0x03fffffc);
                cursor += PPC_INSTRUCTION_LENGTH;
...
void TR_Debug::print(OMR::Logger *log, TR::PPCInterfaceCastSnippet *snippet)
{
    ...
    if (checkcast) {
        ...
        TR::RealRegister *castClassReg
            = _cg->machine()->getRealRegister(deps->getPostConditions()->getRegisterDependency(1)->getRealRegister());
        TR::RealRegister *objClassReg
            = _cg->machine()->getRealRegister(deps->getPostConditions()->getRegisterDependency(2)->getRealRegister());
        TR::RealRegister *cndReg
            = _cg->machine()->getRealRegister(deps->getPostConditions()->getRegisterDependency(3)->getRealRegister());
        TR::RealRegister *scratch1Reg
            = _cg->machine()->getRealRegister(deps->getPostConditions()->getRegisterDependency(4)->getRealRegister());
        ...
        printPrefix(log, NULL, cursor, 4);
        log->printf("cmpl \t%s, %s, %s\t; Compare with type to cast", getName(cndReg), getName(castClassReg),
            getName(scratch1Reg));
        cursor += 4;
        ...
        printPrefix(log, NULL, cursor, 4);
        if (_comp->target().is64Bit())
            log->printf("rldicr \t%s, %s, 0, 0x3D; Clean last bit (cached result)", getName(scratch1Reg),
                getName(scratch2Reg));
        else
            log->printf("rlwinm \t%s, %s, 0, 0xFFFFFFFE; Clean last bit (cached result)", getName(scratch1Reg),
                getName(scratch2Reg));
        cursor += 4;

        printPrefix(log, NULL, cursor, 4);
        log->printf("cmpl \t%s, %s, %s\t; Compare with type to cast", getName(cndReg), getName(scratch1Reg),
            getName(castClassReg));
        cursor += 4;
        ...
        if (snippet->getNeedsResult()) {
            ...
            printPrefix(log, NULL, cursor, 4);
            log->printf("or \t%s, %s, %s; Set the last bit", getName(scratch1Reg), getName(scratch1Reg),
                getName(scratch2Reg));
            cursor += 4;

            printPrefix(log, NULL, cursor, 4);
            log->printf("xor. \t%s, %s, %s; Check if last bit is set in the cache", getName(resultReg),
                getName(scratch1Reg), getName(scratch2Reg));
            cursor += 4;
            ...
            if (snippet->getFalseLabel() != snippet->getTrueLabel()) {
                ...
                printPrefix(log, NULL, cursor, 4);
                log->printf("and. \t%s, %s, %s; Check if last bit is set in the cache", getName(scratch1Reg),
                    getName(scratch1Reg), getName(scratch2Reg));
                cursor += 4;
...
uint32_t TR::PPCInterfaceCastSnippet::getLength(int32_t estimatedSnippetStart)
{
    uint32_t size;
