...
#define LOCK_NON_PRIMITIVE_ENTER_IGNORE_MASK                                                                    \
    ((OBJECT_HEADER_LOCK_RECURSION_MASK & ~OBJECT_HEADER_LOCK_LAST_RECURSION_BIT) | OBJECT_HEADER_LOCK_RESERVED \
        | OBJECT_HEADER_LOCK_FLC)
#define LOCK_NON_PRIMITIVE_EXIT_IGNORE_MASK \
    (OBJECT_HEADER_LOCK_RECURSION_MASK | OBJECT_HEADER_LOCK_RESERVED | OBJECT_HEADER_LOCK_FLC)

extern TR::Register *addConstantToLong(TR::Node *node, TR::Register *srcReg, int64_t value, TR::Register *trgReg,
    TR::CodeGenerator *cg);
extern TR::Register *addConstantToInteger(TR::Node *node, TR::Register *trgReg, TR::Register *srcReg, int32_t value,
    TR::CodeGenerator *cg);

static TR::InstOpCode::Mnemonic getLoadOrStoreFromDataType(TR::CodeGenerator *cg, TR::DataType dt, int32_t elementSize,
    bool isUnsigned, bool returnLoad);
void loadFieldWatchSnippet(TR::CodeGenerator *cg, TR::Node *node, TR::Snippet *dataSnippet, TR::Register *snippetReg,
    TR::Register *scratchReg, bool isInstance);
static const char *ppcSupportsReadMonitors = feGetEnv("TR_ppcSupportReadMonitors");

extern uint32_t getPPCCacheLineSize();

static TR::RegisterDependencyConditions *createConditionsAndPopulateVSXDeps(TR::CodeGenerator *cg,
    int32_t nonVSXDepsCount)
{
    ...
    if (liveVSXVectorReg) {
        ...
        if (!lrScalar || (lrScalar->getNumberOfLiveRegisters() > 0)) {
            ...
            conditions
                = new (cg->trHeapMemory()) TR::RegisterDependencyConditions(depsCount, depsCount, cg->trMemory());
            for (int32_t i = TR::RealRegister::FirstVRF; i <= TR::RealRegister::LastVRF; i++) {
                if (!properties.getPreserved((TR::RealRegister::RegNum)i)) {
                    TR::addDependency(conditions, NULL, (TR::RealRegister::RegNum)i, TR_VRF, cg);
                }
...
static TR::Instruction *genNullTest(TR::Node *node, TR::Register *objectReg, TR::Register *crReg, TR::CodeGenerator *cg,
...
static inline TR::Instruction *genNullTest(TR::Node *node, TR::Register *objectReg, TR::Register *crReg,
    TR::Register *scratchReg, TR::Instruction *cursor, TR::CodeGenerator *cg)
{
    TR::Instruction *iRet;
...
static void genInlineTest(TR::Node *node, TR_OpaqueClassBlock *castClassAddr, TR_OpaqueClassBlock **guessClassArray,
    uint8_t num_PICS, TR::Register *objClassReg, TR::Register *resultReg, TR::Register *cndReg,
    TR::Register *scratch1Reg, TR::Register *scratch2Reg, bool checkCast, bool needsResult, TR::LabelSymbol *falseLabel,
    TR::LabelSymbol *trueLabel, TR::LabelSymbol *doneLabel, TR::LabelSymbol *callLabel, bool testCastClassIsSuper,
    TR::Instruction *iCursor, TR::CodeGenerator *cg)
{
    ...
    // the restart point on TR::PPCInterfaceCastSnippet, and the second as the reference to the call to the helper
    // function this lets us have a more generic Snippet. This can be simplified later on, should it be needed.
    //
    TR::PPCInterfaceCastSnippet *guessCacheSnippet = new (cg->trHeapMemory()) TR::PPCInterfaceCastSnippet(cg, node,
        callLabel, snippetLabel, trueLabel, falseLabel, doneLabel, callLabel, testCastClassIsSuper, checkCast,
        TR::Compiler->om.offsetOfObjectVftField(), offsetof(J9Class, castClassCache), needsResult);
    ...
    for (i = 0; i < num_PICS - 1; i++) {
        ...
        if (comp->compileRelocatableCode() && comp->getOption(TR_UseSymbolValidationManager)) {
            ...
            iCursor = loadAddressConstant(cg, true, node,
                (intptr_t) new (comp->trHeapMemory()) TR::SymbolReference(comp->getSymRefTab(), sym), scratch1Reg,
                iCursor, false, TR_ClassAddress);
        } else {
            iCursor = loadAddressConstant(cg, comp->compileRelocatableCode(), node, (intptr_t)(guessClassArray[i]),
                scratch1Reg, iCursor);
        }
        ...
        iCursor
            = generateTrg1Src2Instruction(cg, TR::InstOpCode::Op_cmpl, node, cndReg, objClassReg, scratch1Reg, iCursor);
        if (needsResult)
    ...
    if (comp->compileRelocatableCode() && comp->getOption(TR_UseSymbolValidationManager)) {
        ...
        iCursor = loadAddressConstant(cg, true, node,
            (intptr_t) new (comp->trHeapMemory()) TR::SymbolReference(comp->getSymRefTab(), sym), scratch1Reg, iCursor,
            false, TR_ClassAddress);
    } else {
        iCursor = loadAddressConstant(cg, comp->compileRelocatableCode(), node,
            (intptr_t)(guessClassArray[num_PICS - 1]), scratch1Reg, iCursor);
    }
...
static TR::Register *nonFixedDependency(TR::RegisterDependencyConditions *conditions, TR::Register *nonFixedReg,
    int32_t *depIndex, TR_RegisterKinds kind, bool excludeGR0, TR::CodeGenerator *cg)
{
...
static void VMoutlinedHelperWrtbarEvaluator(TR::Node *node, TR::Register *srcObjectReg, TR::Register *dstObjectReg,
    bool srcIsNonNull, TR::CodeGenerator *cg)
{
    TR::Compilation *comp = cg->comp();
    ...
    const bool doWrtbar = gcMode == gc_modron_wrtbar_satb || gcMode == gc_modron_wrtbar_oldcheck
        || gcMode == gc_modron_wrtbar_cardmark_and_oldcheck || gcMode == gc_modron_wrtbar_always
        || comp->getOptions()->realTimeGC();

    const bool doCardMark = !node->isNonHeapObjectWrtBar()
    ...
    if (doWrtbar) {
        ...
        TR::Instruction *gcPoint = generateDepImmSymInstruction(cg, TR::InstOpCode::bl, node, helperAddr,
            new (cg->trHeapMemory()) TR::RegisterDependencyConditions(0, 0, cg->trMemory()), symRef);
        ...
        TR::SymbolReference *symRef
            = comp->getSymRefTab()->findOrCreatePerCodeCacheHelperSymbolRef(TR_cardMark, helperAddr);
        generateDepImmSymInstruction(cg, TR::InstOpCode::bl, node, helperAddr,
            new (cg->trHeapMemory()) TR::RegisterDependencyConditions(0, 0, cg->trMemory()), symRef);
    }

    generateDepLabelInstruction(cg, TR::InstOpCode::label, node, doneWrtbarLabel,
        TR_PPCScratchRegisterDependencyConditions::createDependencyConditions(cg, NULL, &deps));
...
TR::Register *outlinedHelperWrtbarEvaluator(TR::Node *node, TR::CodeGenerator *cg)
{
    ...
    if (comp->getOptions()->realTimeGC()) {
        ...
        TR::LoadStoreHandler::generateStoreNodeSequence(cg, srcObjectReg, node, TR::InstOpCode::Op_st,
...
static void VMnonNullSrcWrtBarCardCheckEvaluator(TR::Node *node, TR::Register *srcReg, TR::Register *dstReg,
    TR::Register *condReg, TR::Register *temp1Reg, TR::Register *temp2Reg, TR::Register *temp3Reg,
    TR::Register *temp4Reg, TR::LabelSymbol *doneLabel, TR::RegisterDependencyConditions *deps, bool isCompressedRef,
    TR::CodeGenerator *cg, bool flagsInTemp1 = false)
{
    ...
    bool doWrtBar = (gcMode == gc_modron_wrtbar_satb || gcMode == gc_modron_wrtbar_oldcheck
        || gcMode == gc_modron_wrtbar_cardmark_and_oldcheck || gcMode == gc_modron_wrtbar_always
        || comp->getOptions()->realTimeGC());
    ...
    if (gcMode == gc_modron_wrtbar_cardmark_and_oldcheck)
        wbRef = comp->getSymRefTab()->findOrCreateWriteBarrierStoreGenerationalAndConcurrentMarkSymbolRef(
        ...
        if (wrtbarNode->getOpCodeValue() == TR::awrtbar || wrtbarNode->isUnsafeStaticWrtBar())
            wbRef
                = comp->getSymRefTab()->findOrCreateWriteBarrierClassStoreRealTimeGCSymbolRef(comp->getMethodSymbol());
        else
    ...
    if (gcMode != gc_modron_wrtbar_always && !comp->getOptions()->realTimeGC()) {
        ...
        if (!flagsInTemp1)
            generateTrg1MemInstruction(cg, TR::InstOpCode::lwz, node, temp1Reg,
                TR::MemoryReference::createWithDisplacement(cg, dstReg, getOffsetOfJ9ObjectFlags(), 4));
        ...
        if (inlineCrdmrk) {
            ...
            if (gcMode != gc_modron_wrtbar_cardmark_incremental) {
                ...
                generateTrg1MemInstruction(cg, TR::InstOpCode::Op_load, node, temp2Reg,
                    TR::MemoryReference::createWithDisplacement(cg, metaReg, offsetof(J9VMThread, privateFlags),
                        TR::Compiler->om.sizeofReferenceAddress()));
                ...
                generateTrg1Src1ImmInstruction(cg, TR::InstOpCode::andis_r, node, temp2Reg, temp2Reg, condReg,
                    J9_PRIVATE_FLAGS_CONCURRENT_MARK_ACTIVE >> 16);
            ...
            generateTrg1MemInstruction(cg, TR::InstOpCode::Op_load, node, temp2Reg,
                TR::MemoryReference::createWithDisplacement(cg, metaReg, offsetof(J9VMThread, activeCardTableBase),
                    TR::Compiler->om.sizeofReferenceAddress()));
#if defined(TR_TARGET_64BIT)
            generateTrg1Src1Imm2Instruction(cg, TR::InstOpCode::rldicl, node, temp3Reg, temp3Reg, 64 - card_size_shift,
                (CONSTANT64(1) << (64 - card_size_shift)) - CONSTANT64(1));
#else
            generateTrg1Src1Imm2Instruction(cg, TR::InstOpCode::rlwinm, node, temp3Reg, temp3Reg, 32 - card_size_shift,
                ((uint32_t)0xFFFFFFFF) >> card_size_shift);
#endif
            ...
            generateMemSrc1Instruction(cg, TR::InstOpCode::stb, node,
                TR::MemoryReference::createWithIndexReg(cg, temp2Reg, temp3Reg, 1), temp4Reg);
        ...
        TR::Instruction *i = generateTrg1Src1ImmInstruction(cg, TR::InstOpCode::andi_r, node, temp2Reg, temp1Reg,
            condReg, J9_OBJECT_HEADER_REMEMBERED_MASK_FOR_TEST);
        ...
        if (!comp->getOption(TR_DisableInlineWriteBarriersRT)) {
            ...
            TR::MemoryReference *fragmentParentMR
                = TR::MemoryReference::createWithDisplacement(cg, cg->getMethodMetaDataRegister(),
                    fej9->thisThreadRememberedSetFragmentOffset() + fej9->getFragmentParentOffset(),
                    TR::Compiler->om.sizeofReferenceAddress());
            generateTrg1MemInstruction(cg, TR::InstOpCode::Op_load, node, temp2Reg, fragmentParentMR);

            TR::MemoryReference *globalFragmentIDMR = TR::MemoryReference::createWithDisplacement(cg, temp2Reg,
                fej9->getRememberedSetGlobalFragmentOffset(), TR::Compiler->om.sizeofReferenceAddress());
            ...
            TR::MemoryReference *localFragmentIndexMR
                = TR::MemoryReference::createWithDisplacement(cg, cg->getMethodMetaDataRegister(),
                    fej9->thisThreadRememberedSetFragmentOffset() + fej9->getLocalFragmentOffset(),
                    TR::Compiler->om.sizeofReferenceAddress());
            ...
            generateTrg1MemInstruction(cg, opCode, node, temp1Reg,
                TR::MemoryReference::createWithDisplacement(cg, temp3Reg, 0, size));
            generateTrg1Src1ImmInstruction(cg, TR::InstOpCode::Op_cmpi, node, condReg, temp1Reg, NULLVALUE);
            generateConditionalBranchInstruction(cg, TR::InstOpCode::beq, node, doneLabel, condReg);
    ...
    generateDepImmSymInstruction(cg, TR::InstOpCode::bl, node,
        (uintptr_t)wbRef->getSymbol()->castToMethodSymbol()->getMethodAddress(),
...
static void VMCardCheckEvaluator(TR::Node *node, TR::Register *dstReg, TR::Register *condReg, TR::Register *temp1Reg,
    TR::Register *temp2Reg, TR::Register *temp3Reg, TR::RegisterDependencyConditions *deps, TR::CodeGenerator *cg)
{
    TR::Compilation *comp = cg->comp();
    // non-heap objects cannot be marked
    // Make sure we really should be here
    ...
    TR_ASSERT((gcMode == gc_modron_wrtbar_cardmark || gcMode == gc_modron_wrtbar_cardmark_and_oldcheck
                  || gcMode == gc_modron_wrtbar_cardmark_incremental)
    ...
    if (!definitelyNonHeapObject) {
        ...
        if (gcMode != gc_modron_wrtbar_cardmark_incremental) {
            ...
            TR_ASSERT(J9_PRIVATE_FLAGS_CONCURRENT_MARK_ACTIVE >= 0x00010000
                    && J9_PRIVATE_FLAGS_CONCURRENT_MARK_ACTIVE <= 0x80000000,
                "Concurrent mark active Value assumption broken.");
            generateTrg1Src1ImmInstruction(cg, TR::InstOpCode::andis_r, node, temp1Reg, temp1Reg, condReg,
                J9_PRIVATE_FLAGS_CONCURRENT_MARK_ACTIVE >> 16);
            generateConditionalBranchInstruction(cg, TR::InstOpCode::beq, node, noChkLabel, condReg);
        }
        ...
        generateMemSrc1Instruction(cg, TR::InstOpCode::stb, node,
            TR::MemoryReference::createWithIndexReg(cg, temp1Reg, temp3Reg, 1), temp2Reg);
...
static void VMwrtbarEvaluator(TR::Node *node, TR::Register *srcReg, TR::Register *dstReg, TR::Register *dstAddrReg,
    TR::RegisterDependencyConditions *conditions, bool srcNonNull, bool needDeps, bool isCompressedRef,
    TR::CodeGenerator *cg, TR::Register *flagsReg = NULL)
{
    TR::Compilation *comp = cg->comp();
    auto gcMode = TR::Compiler->om.writeBarrierType();
    bool doWrtBar = (gcMode == gc_modron_wrtbar_satb || gcMode == gc_modron_wrtbar_oldcheck
    ...
    if (doWrtBar) {
        ...
        VMnonNullSrcWrtBarCardCheckEvaluator(node, srcReg, dstReg, cr0, temp1Reg, temp2Reg, temp3Reg, temp4Reg, label,
            conditions, isCompressedRef, cg, flagsReg != NULL);
...
inline void generateLoadJ9Class(TR::Node *node, TR::Register *j9classReg, TR::Register *objReg, TR::CodeGenerator *cg)
{
    if (TR::Compiler->om.compressObjectReferences())
        generateTrg1MemInstruction(cg, TR::InstOpCode::lwz, node, j9classReg,
            TR::MemoryReference::createWithDisplacement(cg, objReg,
                static_cast<int32_t>(TR::Compiler->om.offsetOfObjectVftField()), 4));
    else
        generateTrg1MemInstruction(cg, TR::InstOpCode::Op_load, node, j9classReg,
            TR::MemoryReference::createWithDisplacement(cg, objReg,
                static_cast<int32_t>(TR::Compiler->om.offsetOfObjectVftField()),
                TR::Compiler->om.sizeofReferenceAddress()));
    TR::TreeEvaluator::generateVFTMaskInstruction(cg, node, j9classReg);
}

void J9::Power::TreeEvaluator::generateFillInDataBlockSequenceForUnresolvedField(TR::CodeGenerator *cg, TR::Node *node,
    TR::Snippet *dataSnippet, bool isWrite, TR::Register *sideEffectRegister, TR::Register *dataSnippetRegister)
{
    ...
    TR::RegisterDependencyConditions *deps
        = new (cg->trHeapMemory()) TR::RegisterDependencyConditions(numOfConditions, numOfConditions, cg->trMemory());
    ...
    TR_PPCOutOfLineCodeSection *generateReportOOL
        = new (cg->trHeapMemory()) TR_PPCOutOfLineCodeSection(unresolvedLabel, endLabel, cg);
    ...
    // of Static) Load value from dataSnippetRegister + offsetInDataBlock then compare and branch
    TR::Register *cndReg = cg->allocateRegister(TR_CCR);
    generateTrg1MemInstruction(cg, TR::InstOpCode::Op_load, node, scratchReg,
        TR::MemoryReference::createWithDisplacement(cg, dataSnippetRegister, offsetInDataBlock,
            TR::Compiler->om.sizeofReferenceAddress()));
    ...
    if (isStatic) {
        ...
        if (isWrite) {
            ...
            generateTrg1MemInstruction(cg, TR::InstOpCode::Op_load, node, fieldClassReg,
                TR::MemoryReference::createWithDisplacement(cg, sideEffectRegister,
                    comp->fej9()->getOffsetOfClassFromJavaLangClassField(), TR::Compiler->om.sizeofReferenceAddress()));
        } else {
            isSideEffectReg = true;
            fieldClassReg = sideEffectRegister;
        }
        TR::MemoryReference *memRef = TR::MemoryReference::createWithDisplacement(cg, dataSnippetRegister,
            offsetof(J9JITWatchedStaticFieldData, fieldClass), TR::Compiler->om.sizeofReferenceAddress());
    ...
    // loadAddressConstant(cg, node, (uintptr_t)methodSymbol->getResolvedMethod()->constantPool(), resultReg, NULL,
    // false, TR_ConstantPool);
    loadAddressConstant(cg, comp->compileRelocatableCode(), node,
        reinterpret_cast<uintptr_t>(methodSymbol->getResolvedMethod()->constantPool()), resultReg);
    ...
    TR::Instruction *call = generateDepImmSymInstruction(cg, TR::InstOpCode::bl, node,
        reinterpret_cast<uintptr_t>(helperSym->getMethodAddress()), deps, helperSym);
    ...
    TR::MemoryReference *dataRef = TR::MemoryReference::createWithDisplacement(cg, dataSnippetRegister,
        offsetInDataBlock, TR::Compiler->om.sizeofReferenceAddress());
 ...
 *
 */
void generateReportFieldAccessOutlinedInstructions(TR::Node *node, TR::LabelSymbol *endLabel,
    TR::Register *dataBlockReg, bool isWrite, TR::CodeGenerator *cg, TR::Register *sideEffectRegister,
    TR::Register *valueReg)
{
    ...
    if (isWrite) {
        ...
        TR::InstOpCode::Mnemonic storeOp
            = getLoadOrStoreFromDataType(cg, dt, elementSize, node->getOpCode().isUnsigned(), false);
...
void loadFieldWatchSnippet(TR::CodeGenerator *cg, TR::Node *node, TR::Snippet *dataSnippet, TR::Register *snippetReg,
    TR::Register *scratchReg, bool isInstanceField)
{
    TR::Compilation *comp = cg->comp();
...
TR::Snippet *J9::Power::TreeEvaluator::getFieldWatchInstanceSnippet(TR::CodeGenerator *cg, TR::Node *node, J9Method *m,
    UDATA loc, UDATA os)
{
    return new (cg->trHeapMemory()) TR::J9PPCWatchedInstanceFieldSnippet(cg, node, m, loc, os);
}

TR::Snippet *J9::Power::TreeEvaluator::getFieldWatchStaticSnippet(TR::CodeGenerator *cg, TR::Node *node, J9Method *m,
    UDATA loc, void *fieldAddress, J9Class *fieldClass)
{
    return new (cg->trHeapMemory()) TR::J9PPCWatchedStaticFieldSnippet(cg, node, m, loc, fieldAddress, fieldClass);
}

void J9::Power::TreeEvaluator::generateTestAndReportFieldWatchInstructions(TR::CodeGenerator *cg, TR::Node *node,
    ...
    TR::Register *dataSnippetRegister)
{
    ...
    TR_PPCOutOfLineCodeSection *generateReportOOL
        = new (cg->trHeapMemory()) TR_PPCOutOfLineCodeSection(fieldReportLabel, endLabel, cg);
    cg->getPPCOutOfLineCodeSectionList().push_front(generateReportOOL);
    ...
    if (isInstanceField) {
        ...
        fieldClassReg = cg->allocateRegister();
        // During Non-AOT (JIT and JITServer) compilation the fieldClass has been populated inside the dataSnippet
        // during compilation. During AOT compilation the fieldClass must be loaded from the snippet. The fieldClass in
        // an AOT body is invalid.
        if (cg->needClassAndMethodPointerRelocations()) {
            // Load FieldClass from snippet
            generateTrg1MemInstruction(cg, TR::InstOpCode::Op_load, node, fieldClassReg,
                TR::MemoryReference::createWithDisplacement(cg, dataSnippetRegister,
                    offsetof(J9JITWatchedStaticFieldData, fieldClass), TR::Compiler->om.sizeofReferenceAddress()));
        } else {
        ...
        if (isWrite) {
            ...
            generateTrg1MemInstruction(cg, TR::InstOpCode::Op_load, node, fieldClassReg,
                TR::MemoryReference::createWithDisplacement(cg, sideEffectRegister,
                    fej9->getOffsetOfClassFromJavaLangClassField(), TR::Compiler->om.sizeofReferenceAddress()));
        } else {
            isSideEffectReg = true;
            fieldClassReg = sideEffectRegister;
        }
    ...
    TR::MemoryReference *classFlagsMemRef = TR::MemoryReference::createWithDisplacement(cg, fieldClassReg,
        static_cast<uintptr_t>(fej9->getOffsetOfClassFlags()), 4);

    TR::Register *cndReg = cg->allocateRegister(TR_CCR);
    generateTrg1MemInstruction(cg, TR::InstOpCode::lwz, node, scratchReg, classFlagsMemRef);
    generateTrg1Src1ImmInstruction(cg, TR::InstOpCode::andi_r, node, scratchReg, scratchReg, cndReg,
        J9ClassHasWatchedFields);
...
TR::Register *J9::Power::TreeEvaluator::awrtbarEvaluator(TR::Node *node, TR::CodeGenerator *cg)
{
    ...
    if (!node->skipWrtBar() && !node->hasUnresolvedSymbolReference()
        ...
        generateTrg1MemInstruction(cg, TR::InstOpCode::lwz, node, flagsReg,
            TR::MemoryReference::createWithDisplacement(cg, destinationRegister, getOffsetOfJ9ObjectFlags(), 4));
    }
    ...
    if (comp->getOptions()->realTimeGC()) {
        ...
        VMwrtbarEvaluator(node, sourceRegister, destinationRegister, destinationAddressRegister, NULL,
            firstChild->isNonNull(), true, false, cg);
        TR::LoadStoreHandler::generateStoreAddressSequence(cg, sourceRegister, node, destinationAddressRegister,
            TR::InstOpCode::Op_st, TR::Compiler->om.sizeofReferenceAddress());

        cg->stopUsingRegister(destinationAddressRegister);
    } else {
        TR::LoadStoreHandler::generateStoreNodeSequence(cg, sourceRegister, node, TR::InstOpCode::Op_st,
            TR::Compiler->om.sizeofReferenceAddress());
        VMwrtbarEvaluator(node, sourceRegister, destinationRegister, NULL, NULL, firstChild->isNonNull(), true, false,
            cg, flagsReg);
    }
...
TR::Register *J9::Power::TreeEvaluator::awrtbariEvaluator(TR::Node *node, TR::CodeGenerator *cg)
{
    ...
    if (comp->getOptions()->realTimeGC()) {
        ...
        TR::LoadStoreHandler::generateComputeAddressSequence(cg, destinationAddressRegister, node);
        VMwrtbarEvaluator(node, sourceRegister, destinationRegister, destinationAddressRegister, NULL,
            secondChild->isNonNull(), true, usingCompressedPointers, cg);
        TR::LoadStoreHandler::generateStoreAddressSequence(cg, storeRegister, node, destinationAddressRegister, storeOp,
            sizeofMR);

        cg->stopUsingRegister(destinationAddressRegister);
    } else {
        TR::LoadStoreHandler::generateStoreNodeSequence(cg, storeRegister, node, storeOp, sizeofMR);
        VMwrtbarEvaluator(node, sourceRegister, destinationRegister, NULL, NULL, secondChild->isNonNull(), true,
            usingCompressedPointers, cg, flagsReg);
    }
...
TR::Register *iGenerateSoftwareReadBarrier(TR::Node *node, TR::CodeGenerator *cg)
{
    ...
    generateTrg1MemInstruction(cg, TR::InstOpCode::lwz, node, objReg,
        TR::MemoryReference::createWithDisplacement(cg, locationReg, 0, 4));

    generateLabelInstruction(cg, TR::InstOpCode::label, node, startLabel);
    generateTrg1MemInstruction(cg, TR::InstOpCode::lwz, node, evacuateReg,
        TR::MemoryReference::createWithDisplacement(cg, metaReg, comp->fej9()->thisThreadGetEvacuateBaseAddressOffset(),
            4));
    ...
    generateTrg1MemInstruction(cg, TR::InstOpCode::lwz, node, evacuateReg,
        TR::MemoryReference::createWithDisplacement(cg, metaReg, comp->fej9()->thisThreadGetEvacuateTopAddressOffset(),
            4));
    generateTrg1Src2Instruction(cg, TR::InstOpCode::cmpl4, node, condReg, objReg, evacuateReg);
    ...
    generateDepImmSymInstruction(cg, TR::InstOpCode::bl, node, (uintptr_t)helperSym->getMethodAddress(), deps,
        helperSym);
    ...
    if (node->getSymbol()->isAtLeastOrStrongerThanAcquireRelease() && comp->target().isSMP()) {
        generateInstruction(cg,
            comp->target().cpu.isAtLeast(OMR_PROCESSOR_PPC_P7) ? TR::InstOpCode::lwsync : TR::InstOpCode::isync, node);
    }
...
TR::Register *aGenerateSoftwareReadBarrier(TR::Node *node, TR::CodeGenerator *cg)
{
    ...
    generateTrg1MemInstruction(cg, TR::InstOpCode::Op_load, node, tempReg,
        TR::MemoryReference::createWithDisplacement(cg, locationReg, 0, TR::Compiler->om.sizeofReferenceAddress()));
...
TR::Register *J9::Power::TreeEvaluator::irdbariEvaluator(TR::Node *node, TR::CodeGenerator *cg)
{
    ...
    if (TR::Compiler->om.readBarrierType() != gc_modron_readbar_none && cg->comp()->useCompressedPointers()
...
TR::Register *J9::Power::TreeEvaluator::asynccheckEvaluator(TR::Node *node, TR::CodeGenerator *cg)
{
...
#endif
    {
        ...
        if (comp->target().is64Bit()) {
            TR_ASSERT(secondChild->getLongInt() == -1L, "asynccheck bad format");
            generateTrg1Src1ImmInstruction(cg, TR::InstOpCode::cmpi8, node, condReg, src1Reg,
                secondChild->getLongInt());
        } else {
            TR_ASSERT(secondChild->getInt() == -1, "asynccheck bad format");
        ...
        if (snippetLabel == NULL) {
            ...
            cg->addSnippet(
                new (cg->trHeapMemory()) TR::PPCHelperCallSnippet(cg, node, snippetLabel, node->getSymbolReference()));
        }
        ...
        gcPoint = generateDepConditionalBranchInstruction(cg, TR::InstOpCode::beql, PPCOpProp_BranchUnlikely, node,
            snippetLabel, condReg, dependencies);
    ...
    int32_t leafArrayElementSize)
{
    ...
    TR_ASSERT_FATAL(referenceFieldSize <= 8, "multianewArrayEvaluator - referenceFieldSize cannot be greater than 8!");
    ...
    TR_PPCOutOfLineCodeSection *outlinedHelperCall
        = new (cg->trHeapMemory()) TR_PPCOutOfLineCodeSection(node, TR::acall, targetReg, oolFailLabel, endLabel, cg);
    ...
    generateTrg1Src1ImmInstruction(cg, TR::InstOpCode::addi, node, temp1Reg, targetReg, zeroArraySizeAligned);
    generateTrg1MemInstruction(cg, TR::InstOpCode::Op_load, node, temp2Reg,
        TR::MemoryReference::createWithDisplacement(cg, vmThreadReg, offsetof(J9VMThread, heapTop), addrSize));
    ...
    generateMemSrc1Instruction(cg, storeClassOp, node,
        TR::MemoryReference::createWithDisplacement(cg, targetReg, TR::Compiler->om.offsetOfObjectVftField(),
            classPtrLen),
        classReg);
    ...
    {
        generateTrg1Src1Imm2Instruction(cg, TR::InstOpCode::rldicr, node, temp1Reg, temp1Reg, 0, -alignmentInBytes);
    }
    ...
    generateMemSrc1Instruction(cg, TR::InstOpCode::stw, node,
        TR::MemoryReference::createWithDisplacement(cg, targetReg, fej9->getOffsetOfContiguousArraySizeField(), 4),
        firstDimLenReg);
    ...
    generateTrg1MemInstruction(cg, TR::InstOpCode::Op_load, node, componentClassReg,
        TR::MemoryReference::createWithDisplacement(cg, classReg, offsetof(J9ArrayClass, componentType), addrSize));
    ...
    if (isOffHeapAllocationEnabled) {
        ...
        generateMemSrc1Instruction(cg, TR::InstOpCode::Op_st, node,
            TR::MemoryReference::createWithDisplacement(cg, targetReg, fej9->getOffsetOfContiguousDataAddrField(),
                addrSize),
            temp1Reg);
    }
    ...
    generateMemSrc1Instruction(cg, storeClassOp, node,
        TR::MemoryReference::createWithDisplacement(cg, temp2Reg, TR::Compiler->om.offsetOfObjectVftField(),
            classPtrLen),
        componentClassReg);
    ...
    {
        generateTrg1Src1Imm2Instruction(cg, TR::InstOpCode::rldicr, node, subArraySizeReg, subArraySizeReg, 0,
            -alignmentInBytes);
    }
    ...
    generateMemSrc1Instruction(cg, TR::InstOpCode::stw, node,
        TR::MemoryReference::createWithDisplacement(cg, temp2Reg, fej9->getOffsetOfContiguousArraySizeField(), 4),
        secondDimLenReg);
    ...
    int callUsesFirstChild = (callNode->getFirstChild() == node->getFirstChild())
        && (callNode->getFirstChild()->getRegister()) && (callNode->getFirstChild()->getRegister() != dimsPtrReg);
    int callUsesSecondChild
        = (callNode->getSecondChild() == node->getSecondChild()) && (callNode->getSecondChild()->getRegister());
    int callUsesThirdChild = (callNode->getThirdChild() == node->getThirdChild())
        && (callNode->getThirdChild()->getRegister()) && (callNode->getThirdChild()->getRegister() != classReg);
    ...
    if (callUsesFirstChild) {
        dependencies->addPostCondition(callNode->getFirstChild()->getRegister(), TR::RealRegister::NoReg);
    }
    if (callUsesSecondChild) {
        dependencies->addPostCondition(callNode->getSecondChild()->getRegister(), TR::RealRegister::NoReg);
    }
...
TR::Register *J9::Power::TreeEvaluator::multianewArrayEvaluator(TR::Node *node, TR::CodeGenerator *cg)
{
    ...
    if (nDims > 1 && !disableInlineMultianewArray && fej9->tlhHasBeenCleared() && !comp->getOptions()->realTimeGC()
...
{
    TR_ASSERT(cg->comp()->requiresSpineChecks(),
        "TR::arraylength should be lowered when hybrid arraylets are not in use");
    ...
    TR_PPCOutOfLineCodeSection *discontiguousArrayOOL
        = new (cg->trHeapMemory()) TR_PPCOutOfLineCodeSection(discontiguousArrayLabel, doneLabel, cg);
    ...
    generateConditionalBranchInstruction(cg, TR::InstOpCode::beq, PPCOpProp_BranchUnlikely, node,
        discontiguousArrayLabel, condReg);
...
TR::Register *J9::Power::TreeEvaluator::DIVCHKEvaluator(TR::Node *node, TR::CodeGenerator *cg)
{
    ...
    if (!constDivisor || (type.isInt32() && secondChild->getInt() == 0)
        || (type.isInt64() && secondChild->getLongInt() == 0)) {
        if (!constDivisor || cg->getHasResumableTrapHandler()) {
            srcReg = cg->evaluate(secondChild);
            if (type.isInt64() && comp->target().is32Bit()) {
                TR::Register *trgReg = cg->allocateRegister();
                generateTrg1Src2Instruction(cg, TR::InstOpCode::OR, node, trgReg, srcReg->getHighOrder(),
                    srcReg->getLowOrder());
                srcReg = trgReg;
                killSrc = true;
            }
        ...
        if (cg->getHasResumableTrapHandler()) {
            ...
            if (snippetLabel == NULL) {
                ...
                cg->addSnippet(new (cg->trHeapMemory())
                        TR::PPCHelperCallSnippet(cg, node, snippetLabel, node->getSymbolReference()));
            }
            ...
            if (constDivisor) {
                ...
                gcPoint = generateDepConditionalBranchInstruction(cg, TR::InstOpCode::beql, PPCOpProp_BranchUnlikely,
                    node, snippetLabel, condReg, conditions);
                cg->stopUsingRegister(condReg);
            }
            cg->stopUsingRegister(jumpReg);
        }
...
static void genBoundCheck(TR::CodeGenerator *cg, TR::Node *node, TR::Register *indexReg, int32_t indexVal,
    TR::Register *arrayLengthReg, int32_t arrayLengthVal, TR::Register *condReg, bool noTrap)
{
    TR::Instruction *gcPoint;
    if (noTrap) {
        TR::LabelSymbol *boundCheckFailSnippetLabel
            = cg->lookUpSnippet(TR::Snippet::IsHelperCall, node->getSymbolReference());
        if (!boundCheckFailSnippetLabel) {
            boundCheckFailSnippetLabel = generateLabelSymbol(cg);
            cg->addSnippet(new (cg->trHeapMemory())
                    TR::PPCHelperCallSnippet(cg, node, boundCheckFailSnippetLabel, node->getSymbolReference()));
        }
        ...
        gcPoint = generateConditionalBranchInstruction(cg, TR::InstOpCode::blel, PPCOpProp_BranchUnlikely, node,
            boundCheckFailSnippetLabel, condReg);
    } else {
        if (indexReg) {
            if (arrayLengthReg)
...
TR::Register *J9::Power::TreeEvaluator::BNDCHKEvaluator(TR::Node *node, TR::CodeGenerator *cg)
{
    ...
    if (firstChild->getOpCode().isLoadConst() && firstChild->getInt() >= 0 && firstChild->getInt() <= UPPER_IMMED
        && firstChild->getRegister() == NULL && !noReversedTrap) {
    ...
    if (!noTrap) {
        ...
        gcPoint = generateDepConditionalBranchInstruction(cg, reversed ? TR::InstOpCode::bgel : TR::InstOpCode::blel,
            PPCOpProp_BranchUnlikely, node, snippetLabel, cndReg, conditions);
...
TR::Register *J9::Power::TreeEvaluator::ArrayCopyBNDCHKEvaluator(TR::Node *node, TR::CodeGenerator *cg)
{
    ...
    TR::Instruction *gcPoint;
    bool noTrap = !cg->getHasResumableTrapHandler();
    bool directCase = firstChild->getOpCode().isLoadConst() && secondChild->getOpCode().isLoadConst()
        && firstChild->getInt() < secondChild->getInt();
    ...
    if (directCase) {
    ...
    } else if (firstChild->getOpCode().isLoadConst() && firstChild->getInt() >= LOWER_IMMED
        && firstChild->getInt() <= UPPER_IMMED && firstChild->getRegister() == NULL) {
        ...
        if (noTrap) {
            ...
            gcPoint = generateConditionalBranchInstruction(cg, TR::InstOpCode::bgtl, PPCOpProp_BranchUnlikely, node,
                snippetLabel, cndReg);
            cg->stopUsingRegister(cndReg);
        } else {
        ...
        if (secondChild->getOpCode().isLoadConst() && secondChild->getInt() >= LOWER_IMMED
            && secondChild->getInt() <= UPPER_IMMED) {
            if (noTrap) {
                TR::Register *cndReg = cg->allocateRegister(TR_CCR);
                generateTrg1Src1ImmInstruction(cg, TR::InstOpCode::cmpi4, node, cndReg, boundReg,
                    secondChild->getInt());
                gcPoint = generateConditionalBranchInstruction(cg, TR::InstOpCode::bltl, PPCOpProp_BranchUnlikely, node,
                    snippetLabel, cndReg);
                cg->stopUsingRegister(cndReg);
            } else {
...
static TR::Instruction *genSpineCheck(TR::CodeGenerator *cg, TR::Node *node, TR::Register *arrayLengthReg,
    TR::Register *condReg, TR::LabelSymbol *discontiguousArrayLabel)
{
...
static TR::Instruction *genSpineCheck(TR::CodeGenerator *cg, TR::Node *node, TR::Register *baseArrayReg,
    TR::Register *arrayLengthReg, TR::Register *condReg, TR::LabelSymbol *discontiguousArrayLabel)
{
    TR_J9VMBase *fej9 = (TR_J9VMBase *)(cg->fe());
    TR::MemoryReference *contiguousArraySizeMR
        = TR::MemoryReference::createWithDisplacement(cg, baseArrayReg, fej9->getOffsetOfContiguousArraySizeField(), 4);
    ...
    TR::Register *arrayletReg, TR::Register *offsetReg, int32_t &offsetVal)
{
    ...
    if (indexReg) {
        ...
        generateTrg1Src1Imm2Instruction(cg, TR::InstOpCode::rlwinm, node, arrayletReg, arrayletReg, 0,
            ~(spinePointerSize - 1));
    ...
    //
    generateTrg1MemInstruction(cg, spinePointerSize == 8 ? TR::InstOpCode::ld : TR::InstOpCode::lwz, node, arrayletReg,
        spineMR);
    ...
    if (indexReg) {
        ...
        generateTrg1Src1Imm2Instruction(cg, TR::InstOpCode::rlwinm, node, offsetReg, indexReg, elementSizeShift,
            arrayletMask << elementSizeShift);
    } else
...
static void genDecompressPointer(TR::CodeGenerator *cg, TR::Node *node, TR::Register *ptrReg,
    TR::Register *condReg = NULL, bool nullCheck = true)
{
...
static TR::Register *addConstantToLongWithTempReg(TR::Node *node, TR::Register *srcReg, int64_t value,
    TR::Register *trgReg, TR::Register *tempReg, TR::CodeGenerator *cg)
{
    if (!trgReg)
...
static void genDecompressPointerWithTempReg(TR::CodeGenerator *cg, TR::Node *node, TR::Register *ptrReg,
    TR::Register *tempReg, TR::Register *condReg = NULL, bool nullCheck = true)
{
    TR_J9VMBase *fej9 = (TR_J9VMBase *)(cg->comp()->fe());
    int32_t shiftAmount = TR::Compiler->om.compressedReferenceShift();
...
static TR::Register *genDecompressPointerNonNull2Regs(TR::CodeGenerator *cg, TR::Node *node, TR::Register *ptrReg,
...
static TR::Register *genDecompressPointerNonNull2RegsWithTempReg(TR::CodeGenerator *cg, TR::Node *node,
    TR::Register *ptrReg, TR::Register *targetReg, TR::Register *tempReg)
{
    TR_J9VMBase *fej9 = (TR_J9VMBase *)(cg->comp()->fe());
    int32_t shiftAmount = TR::Compiler->om.compressedReferenceShift();
...
static void genCompressPointerWithTempReg(TR::CodeGenerator *cg, TR::Node *node, TR::Register *tempReg,
    TR::Register *ptrReg, TR::Register *condReg = NULL, bool nullCheck = true)
{
    TR_J9VMBase *fej9 = (TR_J9VMBase *)(cg->comp()->fe());
    int32_t shiftAmount = TR::Compiler->om.compressedReferenceShift();
...
static void genCompressPointer(TR::CodeGenerator *cg, TR::Node *node, TR::Register *ptrReg,
    TR::Register *condReg = NULL, bool nullCheck = true)
{
    TR_J9VMBase *fej9 = (TR_J9VMBase *)(cg->comp()->fe());
    int32_t shiftAmount = TR::Compiler->om.compressedReferenceShift();
...
static TR::Register *genCompressPointerNonNull2RegsWithTempReg(TR::CodeGenerator *cg, TR::Node *node,
    TR::Register *ptrReg, TR::Register *targetReg, TR::Register *tempReg)
{
    TR_J9VMBase *fej9 = (TR_J9VMBase *)(cg->comp()->fe());
    int32_t shiftAmount = TR::Compiler->om.compressedReferenceShift();
...
static TR::Register *genCompressPointerNonNull2Regs(TR::CodeGenerator *cg, TR::Node *node, TR::Register *ptrReg,
    TR::Register *targetReg)
{
    TR_J9VMBase *fej9 = (TR_J9VMBase *)(cg->comp()->fe());
    int32_t shiftAmount = TR::Compiler->om.compressedReferenceShift();
...
TR::Register *J9::Power::TreeEvaluator::BNDCHKwithSpineCHKEvaluator(TR::Node *node, TR::CodeGenerator *cg)
{
    ...
    // Label to the OOL code that will perform the load/store/agen for discontiguous arrays (and the bound check if
    // needed).
    TR::LabelSymbol *discontiguousArrayLabel = generateLabelSymbol(cg);
    ...
    if (needsBoundCheck) {
        ...
        TR_ASSERT(arrayLengthChild->getOpCode().isConversion() || arrayLengthChild->getOpCodeValue() == TR::iloadi
                || arrayLengthChild->getOpCodeValue() == TR::iload || arrayLengthChild->getOpCodeValue() == TR::iRegLoad
                || arrayLengthChild->getOpCode().isLoadConst(),
            "Expecting array length child under BNDCHKwithSpineCHK to be a conversion, iloadi, iload, iRegLoad or "
            "iconst");
        ...
        if (arrayLengthReg) {
            ...
            genBoundCheck(cg, node, indexReg, indexChild->getInt(), arrayLengthReg, arrayLengthChild->getInt(), condReg,
                noTrap);
            ...
            if (arrayLengthChild->getInt() != 0) {
                ...
                OOLBranchInstr
                    = genSpineCheck(cg, node, baseArrayReg, scratchArrayLengthReg, condReg, discontiguousArrayLabel);
    ...
    if (loadOrStoreChild->getOpCode().isStore() && loadOrStoreChild->getReferenceCount() > 1) {
        ...
        if ((loadOrStoreChild->getOpCodeValue() == TR::aload || loadOrStoreChild->getOpCodeValue() == TR::aRegLoad)
            && node->isSpineCheckWithArrayElementChild() && comp->target().is64Bit() && comp->useCompressedPointers()) {
        ...
        // If the 1st child is not a load/store/aladd/aiadd it's probably a nop (e.g. const) at this point due to
        // commoning
        //
        doAddressComputation = !doLoadOrStore && actualLoadOrStoreChild->getOpCode().isArrayRef()
            && !node->isSpineCheckWithArrayElementChild();
    ...
    if (noTrap) {
        TR::RegisterDependencyConditions *mainlineDeps
    ...
    discontiguousArrayOOL->swapInstructionListsWithCompilation();
    {
        ...
        if (needsBoundCheckOOL) {
            ...
            TR::MemoryReference *discontiguousArraySizeMR = TR::MemoryReference::createWithDisplacement(cg,
                baseArrayReg, fej9->getOffsetOfDiscontiguousArraySizeField(), 4);
        ...
        if (doLoadOrStore || doAddressComputation) {
            arrayletReg
                = doAddressComputation ? loadOrStoreReg : /* Needs to be in !gr0 in this case */ cg->allocateRegister();
            ...
            genArrayletAccessAddr(cg, node, elementSize, baseArrayReg, indexReg, indexChild->getInt(), arrayletReg,
                arrayletOffsetReg, arrayletOffsetVal);
            ...
            if (doLoadOrStore) {
                ...
                if (loadOrStoreChild->getOpCode().isStore()) {
                    ...
                    TR::InstOpCode::Mnemonic storeOp
                        = getLoadOrStoreFromDataType(cg, dt, elementSize, isUnsigned, false);

                    if (storeOp == TR::InstOpCode::std && comp->target().is32Bit()) {
                        // Store using consecutive stws.
                        TR_ASSERT(elementSize == 8
                                && loadOrStoreChild->getSecondChild()->getRegister()->getRegisterPair(),
                            "Expecting 64-bit store value to be in a register pair");
                        if (indexReg) {
                            TR::MemoryReference *arrayletMR
                                = TR::MemoryReference::createWithIndexReg(cg, arrayletReg, arrayletOffsetReg, 4);
                            generateMemSrc1Instruction(cg, TR::InstOpCode::stw, node, arrayletMR,
                                loadOrStoreChild->getSecondChild()->getRegister()->getHighOrder());

                            generateTrg1Src1ImmInstruction(cg, TR::InstOpCode::addi, node, arrayletOffsetReg,
                                arrayletOffsetReg, 4);

                            TR::MemoryReference *arrayletMR2
                                = TR::MemoryReference::createWithIndexReg(cg, arrayletReg, arrayletOffsetReg, 4);
                            generateMemSrc1Instruction(cg, TR::InstOpCode::stw, node, arrayletMR2,
                                loadOrStoreChild->getSecondChild()->getRegister()->getLowOrder());
                        } else {
                            TR::MemoryReference *arrayletMR
                                = TR::MemoryReference::createWithDisplacement(cg, arrayletReg, arrayletOffsetVal, 4);
                            TR::MemoryReference *arrayletMR2 = TR::MemoryReference::createWithDisplacement(cg,
                                arrayletReg, arrayletOffsetVal + 4, 4);
                            generateMemSrc1Instruction(cg, TR::InstOpCode::stw, node, arrayletMR,
                                loadOrStoreChild->getSecondChild()->getRegister()->getHighOrder());
                            generateMemSrc1Instruction(cg, TR::InstOpCode::stw, node, arrayletMR2,
                                loadOrStoreChild->getSecondChild()->getRegister()->getLowOrder());
                        ...
                        TR::MemoryReference *arrayletMR = indexReg
                            ? TR::MemoryReference::createWithIndexReg(cg, arrayletReg, arrayletOffsetReg, elementSize)
                    ...
                    if (loadOp == TR::InstOpCode::ld && comp->target().is32Bit()) {
                        ...
                        TR_ASSERT(elementSize == 8 && loadOrStoreReg->getRegisterPair(),
                            "Expecting 64-bit load value to be in a register pair");
                        if (indexReg) {
                            TR::MemoryReference *arrayletMR
                                = TR::MemoryReference::createWithIndexReg(cg, arrayletReg, arrayletOffsetReg, 4);
                            generateTrg1MemInstruction(cg, TR::InstOpCode::lwz, node, loadOrStoreReg->getHighOrder(),
                                arrayletMR);
                            ...
                            TR::MemoryReference *arrayletMR2
                                = TR::MemoryReference::createWithIndexReg(cg, arrayletReg, arrayletOffsetReg, 4);
                            generateTrg1MemInstruction(cg, TR::InstOpCode::lwz, node, loadOrStoreReg->getLowOrder(),
                                arrayletMR2);
                        } else {
                            TR::MemoryReference *arrayletMR
                                = TR::MemoryReference::createWithDisplacement(cg, arrayletReg, arrayletOffsetVal, 4);
                    ...
                    if (doLoadDecompress) {
                        TR_ASSERT(dt == TR::Address,
                            "Expecting loads with decompression trees to have data type TR::Address");
                ...
                if (indexReg)
                    generateTrg1Src2Instruction(cg, TR::InstOpCode::add, node, loadOrStoreReg, loadOrStoreReg,
                        arrayletOffsetReg);
                else
        ...
        const uint32_t numOOLDeps = 1 + (doLoadOrStore ? 1 : 0) + (needsBoundCheck && arrayLengthReg ? 1 : 0)
            + (loadOrStoreReg ? (loadOrStoreReg->getRegisterPair() ? 2 : 1) : 0) + (indexReg ? 1 : 0)
            + srm->numAvailableRegisters();
        TR::RegisterDependencyConditions *OOLDeps
            = new (cg->trHeapMemory()) TR::RegisterDependencyConditions(0, numOOLDeps, cg->trMemory());
        OOLDeps->addPostCondition(baseArrayReg, TR::RealRegister::NoReg);
        TR_ASSERT(OOLDeps->getPostConditions()->getRegisterDependency(0)->getRegister() == baseArrayReg,
            "Unexpected register");
        ...
        if (doLoadOrStore) {
            ...
            TR_ASSERT(OOLDeps->getPostConditions()->getRegisterDependency(1)->getRegister() == arrayletReg,
                "Unexpected register");
...
static void VMoutlinedHelperArrayStoreCHKEvaluator(TR::Node *node, TR::Register *srcReg, TR::Register *dstReg,
    bool srcIsNonNull, TR::CodeGenerator *cg)
{
    TR::Compilation *comp = cg->comp();
    TR_J9VMBase *fej9 = (TR_J9VMBase *)(cg->fe());
    ...
    generateDepLabelInstruction(cg, TR::InstOpCode::label, node, doneArrayStoreCHKLabel,
        TR_PPCScratchRegisterDependencyConditions::createDependencyConditions(cg, NULL, &deps));
...
TR::Instruction *J9::Power::TreeEvaluator::generateVFTMaskInstruction(TR::CodeGenerator *cg, TR::Node *node,
    TR::Register *dstReg, TR::Register *srcReg, TR::Instruction *preced)
{
    ...
    if (~mask == 0) {
        ...
        TR::InstOpCode::Mnemonic op
            = (cg->comp()->target().is64Bit() && !isCompressed) ? TR::InstOpCode::rldicr : TR::InstOpCode::rlwinm;
...
static TR::Instruction *genTestIsSuper(TR::Node *node, TR::Register *objClassReg, TR::Register *castClassReg,
    TR::Register *crReg, TR::Register *scratch1Reg, TR::Register *scratch2Reg, int32_t castClassDepth,
    TR::LabelSymbol *failLabel, TR::Instruction *cursor, TR::CodeGenerator *cg, bool depthInReg2 = false)
{
    TR::Compilation *comp = cg->comp();
    int32_t superClassOffset = castClassDepth * TR::Compiler->om.sizeofReferenceAddress();
    bool outOfBound
        = (!depthInReg2 && (superClassOffset > UPPER_IMMED || superClassOffset < LOWER_IMMED)) ? true : false;
    ...
    cursor = generateTrg1MemInstruction(cg, TR::InstOpCode::Op_load, node, scratch1Reg,
        TR::MemoryReference::createWithDisplacement(cg, objClassReg, offsetof(J9Class, classDepthAndFlags),
            TR::Compiler->om.sizeofReferenceAddress()),
        cursor);
    if (outOfBound) {
        cursor = loadConstant(cg, node, castClassDepth, scratch2Reg, cursor);
    }
    cursor = generateTrg1Src1Imm2Instruction(cg, TR::InstOpCode::rlwinm, node, scratch1Reg, scratch1Reg, 0,
        J9AccClassDepthMask, cursor);
    ...
    if (outOfBound || depthInReg2) {
        ...
        cursor = generateTrg1Src1ImmInstruction(cg, TR::InstOpCode::cmpi4, node, crReg, scratch1Reg, castClassDepth,
            cursor);
    }
...
#endif
    cursor = generateTrg1MemInstruction(cg, TR::InstOpCode::Op_load, node, scratch1Reg,
        TR::MemoryReference::createWithDisplacement(cg, objClassReg, offsetof(J9Class, superclasses),
            TR::Compiler->om.sizeofReferenceAddress()),
        cursor);

    if (outOfBound || depthInReg2) {
        cursor = generateTrg1MemInstruction(cg, TR::InstOpCode::Op_loadx, node, scratch1Reg,
            TR::MemoryReference::createWithIndexReg(cg, scratch1Reg, scratch2Reg,
                TR::Compiler->om.sizeofReferenceAddress()),
            cursor);
    } else {
        cursor = generateTrg1MemInstruction(cg, TR::InstOpCode::Op_load, node, scratch1Reg,
            TR::MemoryReference::createWithDisplacement(cg, scratch1Reg, superClassOffset,
                TR::Compiler->om.sizeofReferenceAddress()),
            cursor);
    }
#ifdef OMR_GC_COMPRESSED_POINTERS
...
static void VMarrayStoreCHKEvaluator(TR::Node *node, TR::Register *src, TR::Register *dst, TR::Register *t1Reg,
    TR::Register *t2Reg, TR::Register *t3Reg, TR::Register *t4Reg, TR::Register *cndReg, TR::LabelSymbol *toWB,
    TR::LabelSymbol *fLabel, TR::CodeGenerator *cg)
{
    TR::Compilation *comp = cg->comp();
    TR_J9VMBase *fej9 = (TR_J9VMBase *)(cg->fe());
    ...
    generateTrg1MemInstruction(cg, TR::InstOpCode::Op_load, node, t1Reg,
        TR::MemoryReference::createWithDisplacement(cg, t1Reg, (int32_t)offsetof(J9ArrayClass, componentType),
            TR::Compiler->om.sizeofReferenceAddress()));
    ...
    if (!comp->compileRelocatableCode() || comp->getOption(TR_UseSymbolValidationManager)) {
        ...
        if (comp->compileRelocatableCode()) {
            ...
            loadAddressConstant(cg, true, node,
                (intptr_t) new (comp->trHeapMemory()) TR::SymbolReference(comp->getSymRefTab(), sym), t3Reg, NULL,
                false, TR_ClassAddress);
        } else {
    ...
    generateTrg1MemInstruction(cg, TR::InstOpCode::Op_load, node, t4Reg,
        TR::MemoryReference::createWithDisplacement(cg, t2Reg, (int32_t)offsetof(J9Class, castClassCache),
            TR::Compiler->om.sizeofReferenceAddress()));
...
#endif
    generateTrg1MemInstruction(cg, TR::InstOpCode::Op_load, node, t3Reg,
        TR::MemoryReference::createWithDisplacement(cg, t1Reg, (int32_t)offsetof(J9Class, romClass),
            TR::Compiler->om.sizeofReferenceAddress()));
    generateTrg1MemInstruction(cg, TR::InstOpCode::Op_load, node, t4Reg,
        TR::MemoryReference::createWithDisplacement(cg, t1Reg, (int32_t)offsetof(J9Class, classDepthAndFlags),
            TR::Compiler->om.sizeofReferenceAddress()));
    generateTrg1MemInstruction(cg, TR::InstOpCode::lwz, node, t3Reg,
        TR::MemoryReference::createWithDisplacement(cg, t3Reg, (int32_t)offsetof(J9ROMClass, modifiers), 4));
    ...
    generateTrg1Src1ImmInstruction(cg, TR::InstOpCode::andi_r, node, t3Reg, t3Reg, cndReg,
        (J9AccClassArray | J9AccInterface) >> 1);
...
TR::Register *J9::Power::TreeEvaluator::ArrayStoreCHKEvaluator(TR::Node *node, TR::CodeGenerator *cg)
{
    ...
    if (!comp->compileRelocatableCode() && !comp->getOptions()->realTimeGC() && !comp->useCompressedPointers()
        && !oldArrayStoreCHK)
    ...
    if (comp->getOption(TR_TraceCG)) {
        comp->log()->printf(
            "%s: Creating and evaluating the following tree to generate the necessary helper call for this node\n",
    ...
    VMarrayStoreCHKEvaluator(node, srcReg, dstReg, temp1Reg, temp2Reg, temp3Reg, temp4Reg, condReg, wbLabel, helperCall,
        cg);

    TR_PPCOutOfLineCodeSection *outlinedHelperCall
        = new (cg->trHeapMemory()) TR_PPCOutOfLineCodeSection(helperCallNode, TR::call, NULL, helperCall, wbLabel, cg);
    cg->getPPCOutOfLineCodeSectionList().push_front(outlinedHelperCall);
...
TR::Register *J9::Power::TreeEvaluator::conditionalHelperEvaluator(TR::Node *node, TR::CodeGenerator *cg)
{
    ...
    for (i = numArgs - 1; i >= 0; i--) {
        ...
        TR::addDependency(conditions, argReg,
            (argReg->getKind() == TR_GPR) ? // Didn't consider Long here
                linkage->getProperties().getIntegerArgumentRegister(iArgIndex++)
                                          : linkage->getProperties().getFloatArgumentRegister(fArgIndex++),
            argReg->getKind(), cg);
    }
    ...
    if (secondChild->getOpCode().isLoadConst() && (value = secondChild->getInt()) <= UPPER_IMMED
        && value >= LOWER_IMMED) {
    ...
    gcPoint = generateDepConditionalBranchInstruction(cg,
        testNode->getOpCodeValue() == TR::icmpeq ? TR::InstOpCode::beql : TR::InstOpCode::bnel, node, snippetLabel,
        cndReg, conditions);
    gcPoint->PPCNeedsGCMap(0xFFFFFFFF);
...
TR::Register *J9::Power::TreeEvaluator::flushEvaluator(TR::Node *node, TR::CodeGenerator *cg)
{
    ...
    if (opCode == TR::allocationFence) {
        if (!node->canOmitSync()) {
            // If the following node will issue an lwsync then skip emitting one now. EscapeAnalysis can purposefully
            // place an AllocationFence before a monexit or volatile access so that the codegen can optimize the memory
            // flush
            ...
            if (!child || (node->getOpCodeValue() != TR::monexit && child->getOpCodeValue() != TR::monexit)) {
                // Iterate the next tree to see if there is a resolved acquire/release load/store node that has yet to
                // be evaluated An unresolved acquire/release might not actually be an acquire/release access, and
                // therefore can not replace the AllocationFence A node that is already evaluated will not actually emit
                // an 'lwsync' so it can not replace the AllocationFence NOTE: As things currently stand, we won't ever
                // encounter an unresolved symref that is strictly acquire/release, but the following code will handle
                // unresolved volatile symrefs as well
...
static void genCheckCastTransitionToNextSequence(TR::Node *node,
    J9::TreeEvaluator::InstanceOfOrCheckCastSequences *iter, TR::LabelSymbol *nextSequenceLabel,
    TR::LabelSymbol *trueLabel, TR::Register *condReg, TR::CodeGenerator *cg)
{
...
static void genInstanceOfTransitionToNextSequence(TR::Node *node,
    J9::TreeEvaluator::InstanceOfOrCheckCastSequences *iter, TR::LabelSymbol *nextSequenceLabel,
    TR::LabelSymbol *doneLabel, TR::Register *condReg, TR::Register *resultReg, bool resultRegInitialValue,
    TR::LabelSymbol *oppositeResultLabel, bool profiledClassIsInstanceOf, TR::CodeGenerator *cg)
{
    J9::TreeEvaluator::InstanceOfOrCheckCastSequences current = *iter++;
    ...
    if (current == J9::TreeEvaluator::NullTest) {
        generateConditionalBranchInstruction(cg, TR::InstOpCode::beq, node,
            resultRegInitialValue == false ? doneLabel : oppositeResultLabel, condReg);
        ...
        if (nextSequenceIsOutOfLine)
            ...
            generateConditionalBranchInstruction(cg, TR::InstOpCode::beq, node,
                resultRegInitialValue == true ? doneLabel : oppositeResultLabel, condReg);
    }
}

static void genInstanceOfOrCheckCastClassEqualityTest(TR::Node *node, TR::Register *condReg,
    TR::Register *instanceClassReg, TR::Register *castClassReg, TR::CodeGenerator *cg)
{
    generateTrg1Src2Instruction(cg, TR::InstOpCode::Op_cmpl, node, condReg, instanceClassReg, castClassReg);

    // At this point condReg[eq] will be set if the cast class matches the instance class. Caller is responsible for
    // acting on the result.
}

static void genInstanceOfOrCheckCastSuperClassTest(TR::Node *node, TR::Register *condReg,
    TR::Register *instanceClassReg, TR::Register *castClassReg, int32_t castClassDepth, TR::LabelSymbol *falseLabel,
    TR_PPCScratchRegisterManager *srm, TR::CodeGenerator *cg)
{
    TR::Compilation *comp = cg->comp();
    ...
    if (dynamicCastClass) {
        ...
        generateTrg1MemInstruction(cg, TR::InstOpCode::Op_load, node, scratchRegister1,
            TR::MemoryReference::createWithDisplacement(cg, castClassReg, offsetof(J9Class, romClass),
                TR::Compiler->om.sizeofReferenceAddress()));
        generateTrg1MemInstruction(cg, TR::InstOpCode::lwz, node, scratchRegister1,
            TR::MemoryReference::createWithDisplacement(cg, scratchRegister1, offsetof(J9ROMClass, modifiers), 4));

        // Array and interface check
        loadConstant(cg, node, (J9AccClassArray | J9AccInterface), scratchRegister2);
        generateTrg1Src2Instruction(cg, TR::InstOpCode::and_r, node, scratchRegister2, scratchRegister1,
            scratchRegister2);
    ...
    generateTrg1Src1Imm2Instruction(cg, TR::InstOpCode::rlwinm, node, instanceClassDepthReg, instanceClassDepthReg, 0,
        J9AccClassDepthMask);
    if (dynamicCastClass) {
        ...
        generateTrg1MemInstruction(cg, TR::InstOpCode::Op_load, node, castClassDepthReg,
            TR::MemoryReference::createWithDisplacement(cg, castClassReg, offsetof(J9Class, classDepthAndFlags),
                TR::Compiler->om.sizeofReferenceAddress()));
        generateTrg1Src1Imm2Instruction(cg, TR::InstOpCode::rlwinm, node, castClassDepthReg, castClassDepthReg, 0,
            J9AccClassDepthMask);
        ...
        if (castClassDepth <= UPPER_IMMED && castClassDepth >= LOWER_IMMED)
            generateTrg1Src1ImmInstruction(cg, TR::InstOpCode::cmpi4, node, condReg, instanceClassDepthReg,
                castClassDepth);
        else {
            castClassDepthReg = srm->findOrCreateScratchRegister();
            loadConstant(cg, node, castClassDepth, castClassDepthReg);
            generateTrg1Src2Instruction(cg, TR::InstOpCode::cmp4, node, condReg, instanceClassDepthReg,
                castClassDepthReg);
        }
    }
    ...
    if (dynamicCastClass) {
        ...
        generateTrg1MemInstruction(cg, TR::InstOpCode::Op_loadx, node, instanceClassSuperClassReg,
        ...
        if (castClassDepthOffset <= UPPER_IMMED && castClassDepthOffset >= LOWER_IMMED) {
            ...
            if (!castClassDepthReg) {
                ...
                if (0x00008000 == HI_VALUE(castClassDepthOffset)) {
                    generateTrg1Src1ImmInstruction(cg, TR::InstOpCode::addis, node, castClassDepthReg,
                        instanceClassSuperClassesArrayReg, 0x7FFF);
                    generateTrg1Src1ImmInstruction(cg, TR::InstOpCode::addis, node, castClassDepthReg,
                        castClassDepthReg, 0x1);
                } else {
                    generateTrg1Src1ImmInstruction(cg, TR::InstOpCode::addis, node, castClassDepthReg,
                        instanceClassSuperClassesArrayReg, HI_VALUE(castClassDepthOffset));
                }
                generateTrg1MemInstruction(cg, TR::InstOpCode::Op_load, node, instanceClassSuperClassReg,
                    TR::MemoryReference::createWithDisplacement(cg, castClassDepthReg, LO_VALUE(castClassDepthOffset),
                        TR::Compiler->om.sizeofReferenceAddress()));
            } else {
                if (comp->target().is64Bit())
                    generateShiftLeftImmediateLong(cg, node, castClassDepthReg, castClassDepthReg, 3);
    ...
    // At this point condReg[eq] will be set if the cast class is a superclass of the instance class. Caller is
    // responsible for acting on the result.
}

static void genInstanceOfOrCheckCastArbitraryClassTest(TR::Node *node, TR::Register *condReg,
    TR::Register *instanceClassReg, TR_OpaqueClassBlock *arbitraryClass, TR_PPCScratchRegisterManager *srm,
    TR::CodeGenerator *cg)
{
    TR::Compilation *comp = cg->comp();
    ...
    if (comp->compileRelocatableCode() && comp->getOption(TR_UseSymbolValidationManager)) {
        ...
        loadAddressConstant(cg, true, node,
            (intptr_t) new (comp->trHeapMemory()) TR::SymbolReference(comp->getSymRefTab(), sym), arbitraryClassReg,
            NULL, false, TR_ClassAddress);
    } else {
    ...
    // At this point condReg[eq] will be set if the cast class matches the arbitrary class. Caller is responsible for
    // acting on the result.
}

static void genInstanceOfCastClassCacheTest(TR::Node *node, TR::Register *condReg, TR::Register *instanceClassReg,
    TR::Register *castClassReg, TR_PPCScratchRegisterManager *srm, TR::CodeGenerator *cg)
{
    TR::Compilation *comp = cg->comp();

    // Compare the cast class against the cache on the instance class.
    // If they are the same the cast is successful.
    // If not it's either because the cache class does not match the cast class, or it does match except the cache class
    // has the low bit set, which means the cast is not successful.
    //
    ...
    generateTrg1MemInstruction(cg, TR::InstOpCode::Op_load, node, castClassCacheReg,
        TR::MemoryReference::createWithDisplacement(cg, instanceClassReg, offsetof(J9Class, castClassCache),
            TR::Compiler->om.sizeofReferenceAddress()));
    ...
    // At this point condReg[lt] will be set if the cast is successful, condReg[eq] will be set if the cast is
    // unsuccessful, and condReg[gt] will be set if the cache class did not match the cast class. Caller is responsible
    // for acting on the result.
}

static void genCheckCastCastClassCacheTest(TR::Node *node, TR::Register *condReg, TR::Register *instanceClassReg,
    TR::Register *castClassReg, TR_PPCScratchRegisterManager *srm, TR::CodeGenerator *cg)
{
    TR::Compilation *comp = cg->comp();
    ...
    // If they are the same the cast is successful.
    // If not it's either because the cache class does not match the cast class, or it does match except the cache class
    // has the low bit set, which means the cast is invalid.
    //
    TR::Register *castClassCacheReg = srm->findOrCreateScratchRegister();
    generateTrg1MemInstruction(cg, TR::InstOpCode::Op_load, node, castClassCacheReg,
        TR::MemoryReference::createWithDisplacement(cg, instanceClassReg, offsetof(J9Class, castClassCache),
...
static void genInstanceOfOrCheckCastObjectArrayTest(TR::Node *node, TR::Register *cr0Reg,
    TR::Register *instanceClassReg, TR::LabelSymbol *falseLabel, TR_PPCScratchRegisterManager *srm,
    TR::CodeGenerator *cg)
{
    TR::Compilation *comp = cg->comp();
    ...
    generateTrg1MemInstruction(cg, TR::InstOpCode::Op_load, node, scratchReg,
        TR::MemoryReference::createWithDisplacement(cg, instanceClassReg, offsetof(J9Class, romClass),
            TR::Compiler->om.sizeofReferenceAddress()));
    generateTrg1MemInstruction(cg, TR::InstOpCode::lwz, node, scratchReg,
        TR::MemoryReference::createWithDisplacement(cg, scratchReg, offsetof(J9ROMClass, modifiers), 4));
    if (J9AccClassArray <= UPPER_IMMED && J9AccClassArray >= LOWER_IMMED)
        generateTrg1Src1ImmInstruction(cg, TR::InstOpCode::andi_r, node, scratchReg, scratchReg, cr0Reg,
            J9AccClassArray);
    else
        generateTrg1Src1ImmInstruction(cg, TR::InstOpCode::andis_r, node, scratchReg, scratchReg, cr0Reg,
            J9AccClassArray >> 16);
    ...
    //
    generateTrg1MemInstruction(cg, TR::InstOpCode::Op_load, node, scratchReg,
        TR::MemoryReference::createWithDisplacement(cg, instanceClassReg, offsetof(J9ArrayClass, componentType),
            TR::Compiler->om.sizeofReferenceAddress()));
    generateTrg1MemInstruction(cg, TR::InstOpCode::Op_load, node, scratchReg,
        TR::MemoryReference::createWithDisplacement(cg, scratchReg, offsetof(J9Class, romClass),
            TR::Compiler->om.sizeofReferenceAddress()));
    generateTrg1MemInstruction(cg, TR::InstOpCode::lwz, node, scratchReg,
        TR::MemoryReference::createWithDisplacement(cg, scratchReg, offsetof(J9ROMClass, modifiers), 4));
    if (J9AccClassInternalPrimitiveType <= UPPER_IMMED && J9AccClassInternalPrimitiveType >= LOWER_IMMED)
        generateTrg1Src1ImmInstruction(cg, TR::InstOpCode::andi_r, node, scratchReg, scratchReg, cr0Reg,
            J9AccClassInternalPrimitiveType);
    else
        generateTrg1Src1ImmInstruction(cg, TR::InstOpCode::andis_r, node, scratchReg, scratchReg, cr0Reg,
            J9AccClassInternalPrimitiveType >> 16);
...
static void genInstanceOfOrCheckCastHelperCall(TR::Node *node, TR::Register *objectReg, TR::Register *castClassReg,
    TR::Register *resultReg, TR::CodeGenerator *cg)
{
    ...
    TR::RegisterDependencyConditions *deps
        = new (cg->trHeapMemory()) TR::RegisterDependencyConditions(1, 4, cg->trMemory());
    ...
    TR::Instruction *gcPoint = generateDepImmSymInstruction(cg, TR::InstOpCode::bl, node,
        (uintptr_t)node->getSymbolReference()->getMethodAddress(), deps, node->getSymbolReference());
    gcPoint->PPCNeedsGCMap(0xFFFFFFFF);
...
TR::Register *J9::Power::TreeEvaluator::VMcheckcastEvaluator(TR::Node *node, TR::CodeGenerator *cg)
{
    ...
    uint32_t numSequencesRemaining = calculateInstanceOfOrCheckCastSequences(node, sequences, &compileTimeGuessClass,
        cg, profiledClassesList, &numberOfProfiledClass, 1, &topClassProbability, &topClassWasCastClass);
        ...
        switch (*iter) {
            ...
            case NullTest:
                ...
                if (node->getOpCodeValue() == TR::checkcastAndNULLCHK) {
                    ...
                    gcPoint
                        = generateNullTestInstructions(cg, objectReg, nullChkInfo, !cg->getHasResumableTrapHandler());
            ...
            case SuperClassTest: {
                ...
                genInstanceOfOrCheckCastSuperClassTest(node, cr0Reg, objectClassReg, castClassReg, castClassDepth,
                    nextSequenceLabel, srm, cg);
                break;
            }
            case ProfiledClassTest:
                logprintf(trace, log, "%s: Emitting ProfiledClassTest\n", node->getOpCode().getName());
                genInstanceOfOrCheckCastArbitraryClassTest(node, cr0Reg, objectClassReg,
                    profiledClassesList[0].profiledClass, srm, cg);
                break;
            case CompileTimeGuessClassTest:
                logprintf(trace, log, "%s: Emitting CompileTimeGuessClassTest\n", node->getOpCode().getName());
                genInstanceOfOrCheckCastArbitraryClassTest(node, cr0Reg, objectClassReg, compileTimeGuessClass, srm,
                    cg);
                break;
    ...
    cg->generateDebugCounter(TR::DebugCounter::debugCounterName(comp,
                                 "instanceOfOrCheckCast.perMethod/%s/(%s)/%d/%d/fastPath", node->getOpCode().getName(),
    ...
    if (numSequencesRemaining > 0 && *iter != GoToTrue) {
        ...
        logprintf(trace, log, "%s: Emitting HelperCall%s\n", node->getOpCode().getName(),
            helperCallForFailure ? " for failure" : "");
        TR_PPCOutOfLineCodeSection *outlinedHelperCall = new (cg->trHeapMemory())
            TR_PPCOutOfLineCodeSection(node, TR::call, NULL, nextSequenceLabel, helperReturnLabel, cg);
        cg->generateDebugCounter(nextSequenceLabel->getInstruction(),
            TR::DebugCounter::debugCounterName(comp, "instanceOfOrCheckCast/%s/%s", node->getOpCode().getName(),
                helperCallForFailure ? "fail" : "helperCall"));
        cg->generateDebugCounter(nextSequenceLabel->getInstruction(),
            TR::DebugCounter::debugCounterName(comp, "instanceOfOrCheckCast.perMethod/%s/(%s)/%d/%d/%s",
                node->getOpCode().getName(), comp->signature(), node->getByteCodeInfo().getCallerIndex(),
                node->getByteCodeInfo().getByteCodeIndex(), helperCallForFailure ? "fail" : "helperCall"));
...
TR::Register *J9::Power::TreeEvaluator::VMinstanceOfEvaluator(TR::Node *node, TR::CodeGenerator *cg)
{
    ...
    while (numSequencesRemaining > 1 || (numSequencesRemaining == 1 && *iter != HelperCall)) {
        ...
        if (--numSequencesRemaining > 0) {
            genInstanceOfTransitionToNextSequence(node, iter, nextSequenceLabel, doneLabel, cr0Reg, resultReg,
                initialResult, oppositeResultLabel, profiledClassIsInstanceOf, cg);
            if (*++iter != HelperCall) {
                generateLabelInstruction(cg, TR::InstOpCode::label, node, nextSequenceLabel);
                nextSequenceLabel = generateLabelSymbol(cg);
            }
    ...
    if (numSequencesRemaining > 0) {
        if (*iter == HelperCall) {
            ...
            TR_PPCOutOfLineCodeSection *outlinedHelperCall = new (cg->trHeapMemory())
                TR_PPCOutOfLineCodeSection(node, TR::icall, resultReg, nextSequenceLabel, helperReturnLabel, cg);
            cg->generateDebugCounter(nextSequenceLabel->getInstruction(),
...
static bool needTestCache(bool cachingEnabled, bool needsHelperCall, bool superClassTest,
    TR_OpaqueClassBlock *castClassAddr, uint8_t num_PICS)
{
...
TR::Register *J9::Power::TreeEvaluator::VMifInstanceOfEvaluator(TR::Node *node, TR::CodeGenerator *cg)
{
    ...
    uint8_t num_PICS
        = TR::TreeEvaluator::interpreterProfilingInstanceOfOrCheckCastInfo(cg, instanceOfNode, guessClassArray);
    ...
    // When class is unknown, testCastClassIsSuper is false, and needsHelperCall is true to handle non-typical class
    // types (e.g. interfaces)
    ...
    bool testCache = needTestCache(!comp->getOption(TR_DisableInlineCheckCast), needsHelperCall, testCastClassIsSuper,
        castClassAddr, num_PICS);
    ...
    if (depNode != NULL) {
        ...
        if (!needsHelperCall
            && numberOfRegisterCandidate(cg, depNode, TR_GPR) + 7 > cg->getMaximumNumberOfGPRsAllowedAcrossEdge(node))
        ...
        depIndex = numberOfRegisterCandidate(cg, depNode, TR_GPR) + numberOfRegisterCandidate(cg, depNode, TR_FPR)
            ...
            + numberOfRegisterCandidate(cg, depNode, TR_VSX_SCALAR)
            + numberOfRegisterCandidate(cg, depNode, TR_VSX_VECTOR);
    }
    ...
    resultReg = TR::TreeEvaluator::VMgenCoreInstanceofEvaluator(instanceOfNode, cg, true, depIndex, numDep, depNode,
        needResult, needsHelperCall, testEqualClass, testCache, testCastClassIsSuper, doneLabel, res0Label, res1Label,
        branchOn1);
...
TR::Register *J9::Power::TreeEvaluator::VMgenCoreInstanceofEvaluator(TR::Node *node, TR::CodeGenerator *cg,
    bool isVMifInstanceOf, int32_t depIndex, int32_t numDep, TR::Node *depNode, bool needResult, bool needHelperCall,
    bool testEqualClass, bool testCache, bool testCastClassIsSuper, TR::LabelSymbol *doneLabel,
    TR::LabelSymbol *res0Label, TR::LabelSymbol *res1Label, bool branchOn1)
{
    ...
    if (needHelperCall) {
        ...
        auto firstArgReg = node->getSymbol()->castToMethodSymbol()->getLinkageConvention() == TR_CHelper
            ? TR::RealRegister::gr4
            : TR::RealRegister::gr3;
        ...
        if (!crReg) {
            ...
            // Since we are not preserving CCRs and this code previously expected to find a CCR in the call
            // instruction's dependency conditions we allocate a new virtual here and stick it in the dependencies,
            // making sure to repeat everything that is normally done for virtual regs when they are referenced by a
            // generated instruction, which just means we record two uses to correspond to the fact that we added them
            // to the pre/post deps of the call.
        ...
        if (testEqualClass || testCache || testCastClassIsSuper) {
            if (needResult) {
                resultReg
                    = conditions->searchPreConditionRegister(static_cast<TR::RealRegister::RegNum>(firstArgReg + 5));
            ...
            if (!objectNode->isNonNull()) {
                ...
                iCursor
                    = generateConditionalBranchInstruction(cg, TR::InstOpCode::beq, node, res0Label, crReg, iCursor);
            }
        }
    } else {
        bool earlyEval = (castClassNode->getOpCodeValue() != TR::loadaddr || castClassNode->getReferenceCount() > 1)
            ? true
            : false;
    ...
    if (testEqualClass || testCache || testCastClassIsSuper) {
        if (TR::Compiler->om.compressObjectReferences())
            ...
            iCursor = generateTrg1MemInstruction(cg, TR::InstOpCode::Op_load, node, objClassReg,
                TR::MemoryReference::createWithDisplacement(cg, objectReg,
                    (int32_t)TR::Compiler->om.offsetOfObjectVftField(), TR::Compiler->om.sizeofReferenceAddress()),
                iCursor);
    ...
    if (testEqualClass) {
        iCursor
            = generateTrg1Src2Instruction(cg, TR::InstOpCode::Op_cmpl, node, crReg, objClassReg, castClassReg, iCursor);

        if (testCache || testCastClassIsSuper) {
            if (needResult) {
                trueLabel = generateLabelSymbol(cg);
                iCursor
                    = generateConditionalBranchInstruction(cg, TR::InstOpCode::beq, node, trueLabel, crReg, iCursor);
            } else {
                iCursor
                    = generateConditionalBranchInstruction(cg, TR::InstOpCode::beq, node, res1Label, crReg, iCursor);
            }
            ...
            bool castClassIsReferenceArray = (comp->isOptServer())
                && (!castClassNode->getSymbolReference()->isUnresolved())
                && (castClassNode->getSymbolReference() == comp->getSymRefTab()->findVftSymbolRef())
                && (castClassNode->getNumChildren() == 1)
            ...
            if (castClassIsReferenceArray) {
                ...
                J9Class *jlobjectarrayclassBlock = jlobjectclassBlock
                    ? (J9Class *)fej9->getArrayClassFromComponentClass((TR_OpaqueClassBlock *)jlobjectclassBlock)
                    : NULL;
                if (jlobjectarrayclassBlock != NULL) {
                    iCursor = loadAddressConstant(cg, comp->compileRelocatableCode(), node,
                        (intptr_t)(jlobjectarrayclassBlock), scratch1Reg, iCursor);
                    iCursor = generateTrg1Src2Instruction(cg, TR::InstOpCode::Op_cmpl, node, crReg, objClassReg,
                        scratch1Reg, iCursor);
                    if (needResult) {
                        iCursor = generateTrg1ImmInstruction(cg, TR::InstOpCode::li, node, resultReg, 0, iCursor);
                    }
                    iCursor = generateConditionalBranchInstruction(cg, TR::InstOpCode::beq, node, res0Label, crReg,
                        iCursor);
                }
            }
        } else {
    ...
    if (testCache && (!comp->compileRelocatableCode() || comp->getOption(TR_UseSymbolValidationManager))) {
        ...
        // An answer of 1 in the lower order bit indicates 'failure' (the cast would have thrown an exception,
        // instanceof would have been unsuccessful) Because of this, we can just do a simple load and compare of the 2
        // class pointers. If it succeeds, the low order bit must be off (success) from a previous checkcast or
        // instanceof. If the low order bit is on, it is guaranteed not to compare and we will take the slow path.
        // Generate inline test for instanceOf
        // Don't inline test when AOT && non-SVM compilation
        genInlineTest(node, castClassAddr, guessClassArray, num_PICS, objClassReg, resultReg, crReg, scratch1Reg,
            scratch2Reg, false, needResult, res0Label, res1Label, doneLabel, callLabel, testCastClassIsSuper, iCursor,
            cg);
    }
        ...
        if (castClassDepth == -1) {
            iCursor = generateTrg1MemInstruction(cg, TR::InstOpCode::Op_load, node, scratch2Reg,
                TR::MemoryReference::createWithDisplacement(cg, castClassReg, offsetof(J9Class, classDepthAndFlags),
                    TR::Compiler->om.sizeofReferenceAddress()),
                iCursor);
            iCursor = generateTrg1Src1Imm2Instruction(cg, TR::InstOpCode::rlwinm, node, scratch2Reg, scratch2Reg, 0,
                J9AccClassDepthMask, iCursor);
            depthInReg2 = true;
        }

        iCursor = genTestIsSuper(node, objClassReg, castClassReg, crReg, scratch1Reg, scratch2Reg, castClassDepth,
            res0Label, iCursor, cg, depthInReg2);
        iCursor = generateConditionalBranchInstruction(cg, TR::InstOpCode::bne, node, res0Label, crReg, iCursor);
...
static TR::Register *reservationLockEnter(TR::Node *node, int32_t lwOffset, TR::CodeGenerator *cg,
    TR::RegisterDependencyConditions *conditions, TR::Register *objReg, TR::Register *monitorReg, TR::Register *valReg,
    TR::Register *tempReg, TR::Register *cndReg, TR::LabelSymbol *callLabel)
{
    TR_J9VMBase *fej9 = (TR_J9VMBase *)(cg->fe());
    ...
    generateTrg1MemInstruction(cg, lockSize == 8 ? TR::InstOpCode::ld : TR::InstOpCode::lwz, node, monitorReg,
        TR::MemoryReference::createWithDisplacement(cg, objReg, lwOffset, lockSize));
    ...
    if (!isPrimitive) {
        ...
        generateMemSrc1Instruction(cg, lockSize == 8 ? TR::InstOpCode::std : TR::InstOpCode::stw, node,
            TR::MemoryReference::createWithDisplacement(cg, objReg, lwOffset, lockSize), tempReg);
        generateLabelInstruction(cg, TR::InstOpCode::b, node, doneLabel);
    } else
    ...
    if (!isPrimitive)
        generateTrg1Src1ImmInstruction(cg, TR::InstOpCode::addi, node, valReg, metaReg,
            LOCK_RESERVATION_BIT | LOCK_INC_DEC_VALUE);
    generateLabelInstruction(cg, TR::InstOpCode::label, node, loopLabel);
    generateTrg1MemInstruction(cg, lockSize == 8 ? TR::InstOpCode::ldarx : TR::InstOpCode::lwarx,
        PPCOpProp_LoadReserveExclusiveAccess, node, monitorReg,
        TR::MemoryReference::createWithIndexReg(cg, objReg, tempReg, lockSize));
    generateTrg1Src1ImmInstruction(cg, TR::InstOpCode::Op_cmpli, node, cndReg, monitorReg, 0);
    generateConditionalBranchInstruction(cg, TR::InstOpCode::bne, node, callLabel, cndReg);
    generateMemSrc1Instruction(cg, lockSize == 8 ? TR::InstOpCode::stdcx_r : TR::InstOpCode::stwcx_r, node,
        TR::MemoryReference::createWithIndexReg(cg, objReg, tempReg, lockSize), valReg);
    ...
    generateTrg1ImmInstruction(cg, TR::InstOpCode::li, node, tempReg,
    ...
    if (!isPrimitive) {
        ...
        generateMemSrc1Instruction(cg, lockSize == 8 ? TR::InstOpCode::std : TR::InstOpCode::stw, node,
            TR::MemoryReference::createWithDisplacement(cg, objReg, lwOffset & 0x0000FFFF, lockSize), monitorReg);
    }
     ...
     */
    generateTrg1MemInstruction(cg, TR::InstOpCode::Op_load, node, tempReg,
        TR::MemoryReference::createWithDisplacement(cg, metaReg, fej9->thisThreadGetOwnedMonitorCountOffset(),
            TR::Compiler->om.sizeofReferenceAddress()));
    ...
    TR_PPCOutOfLineCodeSection *outlinedHelperCall
        = new (cg->trHeapMemory()) TR_PPCOutOfLineCodeSection(node, TR::call, NULL, callLabel, callReturnLabel, cg);
    cg->getPPCOutOfLineCodeSectionList().push_front(outlinedHelperCall);
...
static TR::Register *reservationLockExit(TR::Node *node, int32_t lwOffset, TR::CodeGenerator *cg,
    TR::RegisterDependencyConditions *conditions, TR::Register *objReg, TR::Register *monitorReg, TR::Register *valReg,
    TR::Register *tempReg, TR::Register *cndReg, TR::LabelSymbol *callLabel)
{
    TR_J9VMBase *fej9 = (TR_J9VMBase *)(cg->fe());
    ...
    if (!node->isPrimitiveLockedRegion()) {
        generateTrg1Src1ImmInstruction(cg, TR::InstOpCode::ori, node, tempReg, metaReg,
            LOCK_RESERVATION_BIT + LOCK_INC_DEC_VALUE);
        generateTrg1Src2Instruction(cg, TR::InstOpCode::Op_cmpl, node, cndReg, monitorReg, tempReg);
    } else {
        generateTrg1Src1ImmInstruction(cg, TR::InstOpCode::andi_r, node, tempReg, monitorReg,
            LOCK_RES_PRIMITIVE_EXIT_MASK);
    ...
    if (!isPrimitive) {
        ...
        generateTrg1Src1ImmInstruction(cg, TR::InstOpCode::addi, node, valReg, metaReg, LOCK_RESERVATION_BIT);
        generateMemSrc1Instruction(cg, lockSize == 8 ? TR::InstOpCode::std : TR::InstOpCode::stw, node,
            TR::MemoryReference::createWithDisplacement(cg, objReg, lwOffset, lockSize), valReg);
        generateLabelInstruction(cg, TR::InstOpCode::b, node, doneLabel);
    } else
        generateConditionalBranchInstruction(cg, TR::InstOpCode::bne, node, resLabel, cndReg);
    ...
    generateTrg1Src1ImmInstruction(cg, TR::InstOpCode::addi, node, monitorReg, monitorReg,
        isPrimitive ? LOCK_INC_DEC_VALUE : -LOCK_INC_DEC_VALUE);
    generateMemSrc1Instruction(cg, lockSize == 8 ? TR::InstOpCode::std : TR::InstOpCode::stw, node,
        TR::MemoryReference::createWithDisplacement(cg, objReg, lwOffset, lockSize), monitorReg);
    if (!isPrimitive)
        generateLabelInstruction(cg, TR::InstOpCode::b, node, doneLabel);
...
TR::Register *J9::Power::TreeEvaluator::VMmonexitEvaluator(TR::Node *node, TR::CodeGenerator *cg)
{
    ...
    if (comp->getOption(TR_FullSpeedDebug) || (isMonitorValueBasedOrValueType == TR_yes)
    ...
    if (isMonitorValueBasedOrValueType == TR_maybe) {
        generateCheckForValueMonitorEnterOrExit(node, callLabel, objReg, objectClassReg, tempReg, threadReg, condReg,
            cg, J9_CLASS_DISALLOWS_LOCKING_FLAGS);
    }
    ...
    if (lwOffset <= 0) {
        ...
        TR::MemoryReference *tempMR = TR::MemoryReference::createWithDisplacement(cg, objectClassReg,
            offsetOfLockOffset, TR::Compiler->om.sizeofReferenceAddress());
        generateTrg1MemInstruction(cg, TR::InstOpCode::Op_load, node, objectClassReg, tempMR);

        generateTrg1Src1ImmInstruction(cg, comp->target().is64Bit() ? TR::InstOpCode::cmpi8 : TR::InstOpCode::cmpi4,
            node, condReg, objectClassReg, 0);
        ...
        if (comp->getOption(TR_EnableMonitorCacheLookup)) {
            ...
            if (comp->target().is64Bit())
                simplifyANDRegImm(node, lookupOffsetReg, lookupOffsetReg,
                    (int64_t)J9VMTHREAD_OBJECT_MONITOR_CACHE_SIZE - 1, cg, objNode);
            else
                simplifyANDRegImm(node, lookupOffsetReg, lookupOffsetReg, J9VMTHREAD_OBJECT_MONITOR_CACHE_SIZE - 1, cg,
                    objNode);

            if (comp->target().is64Bit())
                generateShiftLeftImmediateLong(cg, node, lookupOffsetReg, lookupOffsetReg,
                    trailingZeroes((int32_t)TR::Compiler->om.sizeofReferenceField()));
            else
                generateShiftLeftImmediate(cg, node, lookupOffsetReg, lookupOffsetReg,
                    trailingZeroes((int32_t)TR::Compiler->om.sizeofReferenceField()));

            generateTrg1Src1ImmInstruction(cg, TR::InstOpCode::addi, node, lookupOffsetReg, lookupOffsetReg,
                offsetOfMonitorLookupCache);

            generateTrg1MemInstruction(cg,
                (comp->target().is64Bit() && fej9->generateCompressedLockWord()) ? TR::InstOpCode::lwz
                                                                                 : TR::InstOpCode::Op_load,
                node, objectClassReg,
                TR::MemoryReference::createWithIndexReg(cg, metaReg, lookupOffsetReg,
            ...
            generateTrg1MemInstruction(cg, TR::InstOpCode::Op_load, node, lookupOffsetReg,
                TR::MemoryReference::createWithDisplacement(cg, objectClassReg, offsetOfMonitor,
                    TR::Compiler->om.sizeofReferenceAddress()));

            int32_t offsetOfUserData = offsetof(J9ThreadAbstractMonitor, userData);
            generateTrg1MemInstruction(cg, TR::InstOpCode::Op_load, node, lookupOffsetReg,
                TR::MemoryReference::createWithDisplacement(cg, lookupOffsetReg, offsetOfUserData,
                    TR::Compiler->om.sizeofReferenceAddress()));

            generateTrg1Src2Instruction(cg, comp->target().is64Bit() ? TR::InstOpCode::cmp8 : TR::InstOpCode::cmp4,
                node, condReg, lookupOffsetReg, objReg);
            ...
            generateTrg1Src1ImmInstruction(cg, TR::InstOpCode::addi, node, objectClassReg, objectClassReg,
    ...
    if (!fej9->isEnableGlobalLockReservationSet()) {
        ...
        if (!simpleLocking && comp->getOption(TR_ReservingLocks))
            TR::TreeEvaluator::evaluateLockForReservation(node, &reserveLocking, &normalLockWithReservationPreserving,
                cg);
    ...
    int32_t lockSize;
    TR::InstOpCode::Mnemonic loadOpCode, storeOpCode, reservedLoadOpCode, conditionalStoreOpCode, compareLogicalOpCode,
        compareLogicalImmOpCode;
    ...
    if (true || !ppcSupportsReadMonitors || !node->isReadMonitor()) {
         ...
         * Check 2 catches New-PreLearning, New-AutoReserve, Learning-Unlocked, Learning-Locked, Flat-Unlocked and
         * Inflated and sends them to the VM. Check 3 catches Flat-Locked (nested case) and Reserved-Locked. Check 4
         * catches Reserved-Unlocked and Flat-Locked (non-nested case, FLC bit set) and sends it to the VM.
         *
         *
        ...
        generateTrg1MemInstruction(cg, loadOpCode, node, monitorReg,
            TR::MemoryReference::createWithDisplacement(cg, baseReg, lwOffset, lockSize));
        ...
        generateMemSrc1Instruction(cg, storeOpCode, node,
            TR::MemoryReference::createWithDisplacement(cg, baseReg, lwOffset, lockSize), tempReg);
        generateLabelInstruction(cg, TR::InstOpCode::b, node, doneLabel);
        ...
        generateTrg1Src1ImmInstruction(cg, TR::InstOpCode::addi, node, monitorReg, monitorReg, -LOCK_INC_DEC_VALUE);
        generateMemSrc1Instruction(cg, storeOpCode, node,
            TR::MemoryReference::createWithDisplacement(cg, baseReg, lwOffset, lockSize), monitorReg);
        ...
        generateTrg1MemInstruction(cg, reservedLoadOpCode, node, monitorReg,
            TR::MemoryReference::createWithIndexReg(cg, baseReg, tempReg, lockSize));
        ...
        generateMemSrc1Instruction(cg, conditionalStoreOpCode, node,
            TR::MemoryReference::createWithIndexReg(cg, baseReg, tempReg, lockSize), threadReg);

        generateConditionalBranchInstruction(cg, TR::InstOpCode::bne, PPCOpProp_BranchUnlikely, node, loopLabel,
            condReg);
        generateLabelInstruction(cg, TR::InstOpCode::b, node, doneLabel);
...
static void genInitObjectHeader(TR::Node *node, TR::Instruction *&iCursor, TR_OpaqueClassBlock *clazz,
    TR::Register *classReg, TR::Register *resReg, TR::Register *zeroReg, TR::Register *condReg, TR::Register *temp1Reg,
    TR::Register *temp2Reg, TR::Register *packedClobberedOffset, TR::RegisterDependencyConditions *conditions,
    bool needZeroInit, TR::CodeGenerator *cg);

static void genHeapAlloc(TR::Node *node, TR::Instruction *&iCursor, TR_OpaqueClassBlock *clazz, bool isVariableLen,
    TR::Register *enumReg, TR::Register *classReg, TR::Register *resReg, TR::Register *zeroReg, TR::Register *condReg,
    TR::Register *dataSizeReg, TR::Register *heapTopReg, TR::Register *sizeReg, TR::Register *temp1Reg,
    TR::Register *temp2Reg, TR::Register *temp3Reg, TR::LabelSymbol *callLabel, TR::LabelSymbol *doneLabel,
    int32_t allocSize, int32_t elementSize, bool usingTLH, bool needZeroInit,
    TR::RegisterDependencyConditions *dependencies, TR::CodeGenerator *cg)
{
    ...
    if (comp->getOptions()->realTimeGC()) {
        ...
        iCursor = generateTrg1MemInstruction(cg, TR::InstOpCode::Op_load, node, temp3Reg,
            TR::MemoryReference::createWithDisplacement(cg, metaReg, fej9->thisThreadJavaVMOffset(),
                TR::Compiler->om.sizeofReferenceAddress()),
            iCursor);

        // Realtime size classes are now a pointer in J9JavaVM rather than an inlined struct of arrays, so we can't use
        // J9JavaVM as a base pointer anymore Use temp3Reg to hold J9JavaVM->realTimeSizeClasses
        iCursor = generateTrg1MemInstruction(cg, TR::InstOpCode::Op_load, node, temp3Reg,
            TR::MemoryReference::createWithDisplacement(cg, temp3Reg, fej9->getRealtimeSizeClassesOffset(),
                TR::Compiler->om.sizeofReferenceAddress()),
            iCursor);
        ...
        if (isVariableLen) {
            ...
            TR_ASSERT(((int32_t)(maxSize)-allocSize) / elementSize <= USHRT_MAX,
                "MaxObjectSizeForSizeClass won't fit into 16-bits.");

            iCursor = generateTrg1Src1ImmInstruction(cg, TR::InstOpCode::Op_cmpli, node, condReg, enumReg,
                ((int32_t)(maxSize)-allocSize) / elementSize, iCursor);
            ...
            if (TR::Compiler->om.useHybridArraylets()) {
                // Zero length arrays are discontiguous (i.e. they also need the discontiguous length field to be 0)
                // because they are indistinguishable from non-zero length discontiguous arrays
                iCursor
                    = generateTrg1Src1ImmInstruction(cg, TR::InstOpCode::cmpli4, node, condReg, enumReg, 0, iCursor);
                iCursor
                    = generateConditionalBranchInstruction(cg, TR::InstOpCode::beq, node, callLabel, condReg, iCursor);
            }

            // now compute size of object in bytes
            TR_ASSERT(elementSize == 1 || elementSize == 2 || elementSize == 4 || elementSize == 8 || elementSize == 16,
                "Illegal element size for genHeapAlloc()");

            TR::Register *scaledSizeReg = enumReg;

            if (elementSize > 1) {
                iCursor
                    = generateShiftLeftImmediate(cg, node, dataSizeReg, enumReg, trailingZeroes(elementSize), iCursor);
            ...
            if (elementSize < sizeof(UDATA))
                iCursor = generateTrg1Src1ImmInstruction(cg, TR::InstOpCode::addi, node, dataSizeReg, scaledSizeReg,
                    allocSize + sizeof(UDATA) - 1, iCursor);
            else
                iCursor = generateTrg1Src1ImmInstruction(cg, TR::InstOpCode::addi, node, dataSizeReg, scaledSizeReg,
                    allocSize, iCursor);
            ...
            if (elementSize < sizeof(UDATA)) {
                if (comp->target().is64Bit())
                    iCursor = generateTrg1Src1Imm2Instruction(cg, TR::InstOpCode::rldicr, node, dataSizeReg,
                        dataSizeReg, 0, int64_t(-sizeof(UDATA)), iCursor);
                else
                    iCursor = generateTrg1Src1Imm2Instruction(cg, TR::InstOpCode::rlwinm, node, dataSizeReg,
                        dataSizeReg, 0, -sizeof(UDATA), iCursor);
            }

#ifdef J9VM_INTERP_FLAGS_IN_CLASS_SLOT
            iCursor = generateTrg1Src1ImmInstruction(cg, TR::InstOpCode::Op_cmpli, node, condReg, dataSizeReg,
                J9_GC_MINIMUM_OBJECT_SIZE, iCursor);
            TR::LabelSymbol *doneLabel = generateLabelSymbol(cg);
            iCursor = generateConditionalBranchInstruction(cg, TR::InstOpCode::bge, PPCOpProp_BranchLikely, node,
                doneLabel, condReg, iCursor);
            iCursor = generateTrg1ImmInstruction(cg, TR::InstOpCode::li, node, dataSizeReg, J9_GC_MINIMUM_OBJECT_SIZE,
                iCursor);
            iCursor = generateLabelInstruction(cg, TR::InstOpCode::label, node, doneLabel, iCursor);
#endif

            // J9JavaVM (or realTimeSizeClasses when J9_CHANGES_PR95193 is defined) + rounded data size +
            // SizeClassesIndexOffset is pointer to the size class
            iCursor
                = generateTrg1Src2Instruction(cg, TR::InstOpCode::add, node, temp1Reg, dataSizeReg, temp3Reg, iCursor);

            // Now adjust dataSizeReg to only include the size of the array data in bytes, since this will be used in
            // genInitArrayHeader();
            iCursor = generateTrg1Src1ImmInstruction(cg, TR::InstOpCode::addi, node, dataSizeReg, dataSizeReg,
                -allocSize, iCursor);
            iCursor = generateTrg1MemInstruction(cg, TR::InstOpCode::Op_load, node, heapTopReg,
                TR::MemoryReference::createWithDisplacement(cg, temp1Reg, fej9->getSizeClassesIndexOffset(),
                    TR::Compiler->om.sizeofReferenceAddress()),
                iCursor);
            // heapTopReg now holds size class

            // get next cell for this size class at vmThread + thisThreadAllocationCacheCurrentOffset(0) + (size
            // class)*sizeof(J9VMGCSegregatedAllocationCacheEntry)
            ...
            if (comp->target().is64Bit())
                iCursor = generateShiftLeftImmediateLong(cg, node, temp1Reg, heapTopReg,
                    trailingZeroes((int32_t)sizeof(J9VMGCSegregatedAllocationCacheEntry)), iCursor);
            else
                iCursor = generateShiftLeftImmediate(cg, node, temp1Reg, heapTopReg,
                    trailingZeroes((int32_t)sizeof(J9VMGCSegregatedAllocationCacheEntry)), iCursor);
            ...
            iCursor = generateTrg1MemInstruction(cg, TR::InstOpCode::Op_load, node, resReg,
                TR::MemoryReference::createWithDisplacement(cg, sizeReg,
                    fej9->thisThreadAllocationCacheCurrentOffset(0), TR::Compiler->om.sizeofReferenceAddress()),
                iCursor);
            iCursor = generateTrg1MemInstruction(cg, TR::InstOpCode::Op_load, node, temp1Reg,
                TR::MemoryReference::createWithDisplacement(cg, sizeReg, fej9->thisThreadAllocationCacheTopOffset(0),
                    TR::Compiler->om.sizeofReferenceAddress()),
                iCursor);

            // if we've reached the top, then no cell available, use slow path
            iCursor
                = generateTrg1Src2Instruction(cg, TR::InstOpCode::Op_cmpl, node, condReg, resReg, temp1Reg, iCursor);
            ...
            if (comp->target().is64Bit())
                iCursor = generateShiftLeftImmediateLong(cg, node, temp1Reg, heapTopReg,
                    trailingZeroes((int32_t)sizeof(UDATA)), iCursor);
            else
                iCursor = generateShiftLeftImmediate(cg, node, temp1Reg, heapTopReg,
                    trailingZeroes((int32_t)sizeof(UDATA)), iCursor);
            ...
            iCursor = generateTrg1MemInstruction(cg, TR::InstOpCode::Op_load, node, temp1Reg,
                TR::MemoryReference::createWithDisplacement(cg, temp1Reg, fej9->getSmallCellSizesOffset(),
                    TR::Compiler->om.sizeofReferenceAddress()),
                iCursor);
            ...
            // sizeClass will be bogus for variable length allocations because it only includes the header size (+
            // arraylet ptr for arrays)
            UDATA sizeClass = fej9->getObjectSizeClass(allocSize);

            iCursor = generateTrg1MemInstruction(cg, TR::InstOpCode::Op_load, node, resReg,
                TR::MemoryReference::createWithDisplacement(cg, metaReg,
                    fej9->thisThreadAllocationCacheCurrentOffset(sizeClass), TR::Compiler->om.sizeofReferenceAddress()),
                iCursor);
            iCursor = generateTrg1MemInstruction(cg, TR::InstOpCode::Op_load, node, temp2Reg,
                TR::MemoryReference::createWithDisplacement(cg, metaReg,
                    fej9->thisThreadAllocationCacheTopOffset(sizeClass), TR::Compiler->om.sizeofReferenceAddress()),
                iCursor);
            iCursor
                = generateTrg1Src2Instruction(cg, TR::InstOpCode::Op_cmpl, node, condReg, resReg, temp2Reg, iCursor);
            iCursor = generateConditionalBranchInstruction(cg, TR::InstOpCode::bge, node, callLabel, condReg, iCursor);

            // now bump the current updatepointer
            if (fej9->getCellSizeForSizeClass(sizeClass) <= UPPER_IMMED)
                iCursor = generateTrg1Src1ImmInstruction(cg, TR::InstOpCode::addi, node, temp1Reg, resReg,
                    fej9->getCellSizeForSizeClass(sizeClass), iCursor);
            else {
                iCursor = loadConstant(cg, node, (int32_t)fej9->getCellSizeForSizeClass(sizeClass), temp1Reg, iCursor);
                iCursor
                    = generateTrg1Src2Instruction(cg, TR::InstOpCode::add, node, temp1Reg, temp1Reg, resReg, iCursor);
            }
            iCursor = generateMemSrc1Instruction(cg, TR::InstOpCode::Op_st, node,
                TR::MemoryReference::createWithDisplacement(cg, metaReg,
                    fej9->thisThreadAllocationCacheCurrentOffset(sizeClass), TR::Compiler->om.sizeofReferenceAddress()),
        ...
        if (isArrayAlloc) {
            if (TR::Compiler->om.useHybridArraylets()) {
                ...
                if (maxContiguousArraySize > UPPER_IMMED) {
                    ...
                    iCursor = generateTrg1Src2Instruction(cg, TR::InstOpCode::cmpl4, node, condReg, enumReg, temp2Reg,
                        iCursor);
                } else
                    iCursor = generateTrg1Src1ImmInstruction(cg, TR::InstOpCode::cmpli4, node, condReg, enumReg,
                        maxContiguousArraySize, iCursor);

                iCursor
                    = generateConditionalBranchInstruction(cg, TR::InstOpCode::bgt, node, callLabel, condReg, iCursor);
            }

            if (isVariableLen) { // Inline runtime zero length non-packed array allocation
                // Zero length arrays are discontiguous (i.e. they also need the discontiguous length field to be 0)
                // because they are indistinguishable from non-zero length discontiguous arrays
                iCursor
                    = generateTrg1Src1ImmInstruction(cg, TR::InstOpCode::cmpli4, node, condReg, enumReg, 0, iCursor);
                TR::LabelSymbol *nonZeroLengthLabel = generateLabelSymbol(cg);
                iCursor = generateConditionalBranchInstruction(cg, TR::InstOpCode::bne, node, nonZeroLengthLabel,
                    condReg, iCursor);

                int32_t zeroLenArraySize = (TR::Compiler->om.discontiguousArrayHeaderSizeInBytes()
                                               + TR::Compiler->om.getObjectAlignmentInBytes() - 1)
                    & (-TR::Compiler->om.getObjectAlignmentInBytes());
                TR_ASSERT(zeroLenArraySize >= J9_GC_MINIMUM_OBJECT_SIZE,
                    "Zero-length array size must be bigger than MIN_OBJECT_SIZE");
                ...
                iCursor = generateTrg1Src1ImmInstruction(cg, TR::InstOpCode::addi, node, temp2Reg, resReg,
                    zeroLenArraySize, iCursor);
                iCursor = generateTrg1Src2Instruction(cg, TR::InstOpCode::Op_cmpl, node, condReg, temp2Reg, heapTopReg,
                    iCursor);
                iCursor
                    = generateConditionalBranchInstruction(cg, TR::InstOpCode::bgt, node, callLabel, condReg, iCursor);

                if (comp->compileRelocatableCode())
                    genInitObjectHeader(node, iCursor, clazz, classReg, resReg, zeroReg, condReg, heapTopReg, sizeReg,
                        NULL, dependencies, needZeroInit, cg);
                else
                    genInitObjectHeader(node, iCursor, clazz, NULL, resReg, zeroReg, condReg, heapTopReg, sizeReg, NULL,
                        dependencies, needZeroInit, cg);
        ...
        if (usingTLH) {
            ...
            if (isVariableLen) {
                ...
                if (maxSafeSize < 0x00100000) {
                    ...
                    iCursor = generateTrg1Src1ImmInstruction(cg, TR::InstOpCode::Op_cmpli, node, condReg, enumReg,
                        (maxSafeSize >> 6) << 2, iCursor);
                    iCursor = generateConditionalBranchInstruction(cg, TR::InstOpCode::bge, node, callLabel, condReg,
                        iCursor);
                } else {
                    uintptr_t mask;
                    if (comp->target().is64Bit()) {
                        mask = 0xFFFFFFFF << (27 - leadingZeroes(maxSafeSize));
                        iCursor = generateTrg1Src1Imm2Instruction(cg, TR::InstOpCode::rlwinm_r, node, temp2Reg, enumReg,
                            condReg, 0, mask, iCursor);
                    } else {
                        mask = (0xFFFFFFFF << (11 - leadingZeroes(maxSafeSize))) & 0x0000FFFF;
                        iCursor = generateTrg1Src1ImmInstruction(cg, TR::InstOpCode::andis_r, node, temp2Reg, enumReg,
                            condReg, mask, iCursor);
                    }
                    iCursor = generateConditionalBranchInstruction(cg, TR::InstOpCode::bne, node, callLabel, condReg,
                        iCursor);
                }
            }
            ...
            if (!cg->isDualTLH()) {
                ...
                if (node->canSkipZeroInitialization()) {
                    if (verboseDualTLH)
                        fprintf(stderr,
                            "DELETEME genHeapAlloc useNonZeroTLH [%d] isVariableLen [%d] node: [%p]  %s [@%s]\n", 1,
                            isVariableLen, node, comp->signature(), comp->getHotnessName(comp->getMethodHotness()));

                    // Load non-zeroed TLH heapAlloc and heapTop values.
                    iCursor = generateTrg1MemInstruction(cg, TR::InstOpCode::Op_load, node, resReg,
                        TR::MemoryReference::createWithDisplacement(cg, metaReg, offsetof(J9VMThread, nonZeroHeapAlloc),
                            TR::Compiler->om.sizeofReferenceAddress()),
                        iCursor);

                    iCursor = generateTrg1MemInstruction(cg, TR::InstOpCode::Op_load, node, heapTopReg,
                        TR::MemoryReference::createWithDisplacement(cg, metaReg, offsetof(J9VMThread, nonZeroHeapTop),
                            TR::Compiler->om.sizeofReferenceAddress()),
                        iCursor);
            ...
            if (sizeInReg) {
                ...
                if (isVariableLen) {
                    ...
                    if (generateArraylets
                        ...
                        int32_t maxContiguousArrayletLeafSizeInBytes
                            = arrayletLeafSize - TR::Compiler->om.sizeofReferenceAddress(); // need to add definition
                        ...
                        if (maxArrayletSizeInElements <= UPPER_IMMED) {
                            iCursor = generateTrg1Src1ImmInstruction(cg, TR::InstOpCode::cmpi4, node, condReg, enumReg,
                                maxArrayletSizeInElements, iCursor);
                        } else {
                            iCursor = loadConstant(cg, node, maxArrayletSizeInElements, temp1Reg, iCursor);
                            iCursor = generateTrg1Src2Instruction(cg, TR::InstOpCode::cmp4, node, condReg, enumReg,
                                temp1Reg, iCursor);
                        }
                        ...
                        iCursor = generateConditionalBranchInstruction(cg, TR::InstOpCode::bge, node, callLabel,
                            condReg, iCursor);
                    }
                    ...
                    if (elementSize >= 2) {
                        if (comp->target().is64Bit())
                            iCursor = generateTrg1Src1Imm2Instruction(cg, TR::InstOpCode::rldic, node, dataSizeReg,
                                enumReg, trailingZeroes(elementSize),
                                CONSTANT64(0x00000000FFFFFFFF) << trailingZeroes(elementSize));
                        else
                            iCursor = generateShiftLeftImmediate(cg, node, dataSizeReg, enumReg,
                    ...
                    if (needZeroInit && elementSize <= 2) {
                        ...
                        if (elementSize == 2)
                            iCursor = generateTrg1Src1ImmInstruction(cg, TR::InstOpCode::addi, node, dataSizeReg,
                                dataSizeReg, 3, iCursor);
                        else // elementSize == 1
                            iCursor = generateTrg1Src1ImmInstruction(cg, TR::InstOpCode::addi, node, dataSizeReg,
                                enumReg, 3, iCursor);
                        ...
                        if (comp->target().is64Bit() && elementSize != 1)
                            iCursor = generateTrg1Src1Imm2Instruction(cg, TR::InstOpCode::rldicr, node, dataSizeReg,
                                dataSizeReg, 0, int64_t(-4), iCursor);
                        else
                            iCursor = generateTrg1Src1Imm2Instruction(cg, TR::InstOpCode::rlwinm, node, dataSizeReg,
                                dataSizeReg, 0, -4, iCursor);
                    }
                    ...
                    if ((round != 0 && (!needZeroInit || round != 4)) || !headerAligned) {
                        ...
                        if (elementSize == 1 && !needZeroInit)
                            iCursor = generateTrg1Src1ImmInstruction(cg, TR::InstOpCode::addi, node, sizeReg, enumReg,
                                allocSize + round - 1, iCursor);
                        else
                            iCursor = generateTrg1Src1ImmInstruction(cg, TR::InstOpCode::addi, node, sizeReg,
                                dataSizeReg, allocSize + round - 1, iCursor);
                        if (comp->target().is64Bit() && elementSize != 1)
                            iCursor = generateTrg1Src1Imm2Instruction(cg, TR::InstOpCode::rldicr, node, sizeReg,
                                sizeReg, 0, int64_t(-round), iCursor);
                        else
                            iCursor = generateTrg1Src1Imm2Instruction(cg, TR::InstOpCode::rlwinm, node, sizeReg,
                                sizeReg, 0, -round, iCursor);
                    } else {
                        if (elementSize == 1 && !needZeroInit)
                            iCursor = generateTrg1Src1ImmInstruction(cg, TR::InstOpCode::addi, node, sizeReg, enumReg,
                                allocSize, iCursor);
                            ...
                            iCursor = generateTrg1Src1ImmInstruction(cg, TR::InstOpCode::addi, node, sizeReg,
                                dataSizeReg, allocSize, iCursor);
                    }

#ifdef J9VM_INTERP_FLAGS_IN_CLASS_SLOT
                    if (!((node->getOpCodeValue() == TR::New) || (node->getOpCodeValue() == TR::anewarray)
                            || (node->getOpCodeValue() == TR::newarray))) {
                        iCursor = generateTrg1Src1ImmInstruction(cg, TR::InstOpCode::Op_cmpli, node, condReg, sizeReg,
                            J9_GC_MINIMUM_OBJECT_SIZE, iCursor);
                        TR::LabelSymbol *doneLabel = generateLabelSymbol(cg);
                        iCursor = generateConditionalBranchInstruction(cg, TR::InstOpCode::bge, PPCOpProp_BranchLikely,
                            node, doneLabel, condReg, iCursor);

                        iCursor = generateTrg1ImmInstruction(cg, TR::InstOpCode::li, node, sizeReg,
                            J9_GC_MINIMUM_OBJECT_SIZE, iCursor);
                        iCursor = generateLabelInstruction(cg, TR::InstOpCode::label, node, doneLabel, iCursor);
                    }
#endif
            ...
            if (sizeInReg)
                iCursor
                    = generateTrg1Src2Instruction(cg, TR::InstOpCode::add, node, temp2Reg, resReg, sizeReg, iCursor);
            else
                iCursor = generateTrg1Src1ImmInstruction(cg, TR::InstOpCode::addi, node, temp2Reg, resReg, allocSize,
                    iCursor);

            if (!isVariableLen && ((uint32_t)allocSize) > maxSafeSize) {
                iCursor = generateTrg1Src2Instruction(cg, TR::InstOpCode::Op_cmpl, node, condReg, temp2Reg, resReg,
                    iCursor);
                iCursor
                    = generateConditionalBranchInstruction(cg, TR::InstOpCode::blt, node, callLabel, condReg, iCursor);
            }
            ...
            if (generateArraylets
                && (node->getOpCodeValue() == TR::anewarray || node->getOpCodeValue() == TR::newarray)) {
                iCursor = generateTrg1Src1ImmInstruction(cg, TR::InstOpCode::addi, node, temp2Reg, temp2Reg,
                    TR::Compiler->om.getObjectAlignmentInBytes() - 1, iCursor);
                iCursor = generateTrg1Src1Imm2Instruction(cg,
                    comp->target().is64Bit() ? TR::InstOpCode::rldicr : TR::InstOpCode::rlwinm, node, temp2Reg,
                    temp2Reg, 0, int64_t(-TR::Compiler->om.getObjectAlignmentInBytes()), iCursor);
            ...
            if (cg->enableTLHPrefetching()) {
                ...
                if (cg->isDualTLH() && node->canSkipZeroInitialization()) {
                    if (verboseDualTLH)
                        fprintf(stderr,
                            "DELETEME genHeapAlloc PREFETCH useNonZeroTLH [%d] isVariableLen [%d] node: [%p]  %s "
                            "[@%s]\n",
                            1, isVariableLen, node, comp->signature(), comp->getHotnessName(comp->getMethodHotness()));

                    iCursor = generateTrg1MemInstruction(cg, TR::InstOpCode::Op_load, node, temp1Reg,
                        TR::MemoryReference::createWithDisplacement(cg, metaReg,
                            offsetof(J9VMThread, nonZeroTlhPrefetchFTA), TR::Compiler->om.sizeofReferenceAddress()),
                        iCursor);
                } else {
                    if (verboseDualTLH)
                        fprintf(stderr,
                            "DELETEME genHeapAlloc PREFETCH useNonZeroTLH [%d] isVariableLen [%d] node: [%p]  %s "
                            "[@%s]\n",
                            0, isVariableLen, node, comp->signature(), comp->getHotnessName(comp->getMethodHotness()));

                    iCursor = generateTrg1MemInstruction(cg, TR::InstOpCode::Op_load, node, temp1Reg,
                        TR::MemoryReference::createWithDisplacement(cg, metaReg, offsetof(J9VMThread, tlhPrefetchFTA),
                            TR::Compiler->om.sizeofReferenceAddress()),
                        iCursor);
                }

                if (sizeInReg)
                    iCursor = generateTrg1Src2Instruction(cg, TR::InstOpCode::subf_r, node, temp1Reg, sizeReg, temp1Reg,
                        condReg, iCursor);
                else
                    iCursor = generateTrg1Src1ImmInstruction(cg, TR::InstOpCode::addic_r, node, temp1Reg, temp1Reg,
                        condReg, -allocSize, iCursor);
                ...
                if (cg->isDualTLH() && node->canSkipZeroInitialization()) {
                    ...
                    if (callLabel == NULL) {
                        callLabel = generateLabelSymbol(cg);
                        TR::Snippet *snippet
                            = new (cg->trHeapMemory()) TR::PPCNonZeroAllocPrefetchSnippet(cg, node, callLabel);
                    ...
                    if (callLabel == NULL) {
                        callLabel = generateLabelSymbol(cg);
                        TR::Snippet *snippet
                            = new (cg->trHeapMemory()) TR::PPCAllocPrefetchSnippet(cg, node, callLabel);
                        cg->addSnippet(snippet);
                    }
                }

                // kills temp1Reg and sizeReg
                iCursor = generateDepConditionalBranchInstruction(cg, TR::InstOpCode::blel, PPCOpProp_BranchUnlikely,
                    node, callLabel, condReg, dependencies, iCursor);
            ...
            iCursor = generateTrg1MemInstruction(cg, TR::InstOpCode::Op_load, node, heapTopReg,
                TR::MemoryReference::createWithDisplacement(cg, temp1Reg, offsetof(J9MemorySegment, heapTop),
                    TR::Compiler->om.sizeofReferenceAddress()),
                iCursor);
            iCursor = generateTrg1Src1ImmInstruction(cg, TR::InstOpCode::addi, node, dataSizeReg, temp1Reg,
                offsetof(J9MemorySegment, heapAlloc), iCursor);
            iCursor = generateTrg1ImmInstruction(cg, TR::InstOpCode::li, node, zeroReg, 0, iCursor);
            if (sizeInReg) {
                if (0x00008000 == HI_VALUE(allocSize)) {
                    iCursor = generateTrg1ImmInstruction(cg, TR::InstOpCode::lis, node, sizeReg, 0x7FFF, iCursor);
                    iCursor = generateTrg1Src1ImmInstruction(cg, TR::InstOpCode::addis, node, sizeReg, sizeReg, 0x1,
                        iCursor);
                } else {
                    iCursor = generateTrg1ImmInstruction(cg, TR::InstOpCode::lis, node, sizeReg, HI_VALUE(allocSize),
                        iCursor);
                }
                iCursor = generateTrg1Src1ImmInstruction(cg, TR::InstOpCode::addi, node, sizeReg, sizeReg,
                    LO_VALUE(allocSize), iCursor);
            }

            // Try to allocate
            iCursor = generateLabelInstruction(cg, TR::InstOpCode::label, node, tryLabel, iCursor);
            iCursor = generateTrg1MemInstruction(cg, TR::InstOpCode::Op_load, node, resReg,
                TR::MemoryReference::createWithDisplacement(cg, dataSizeReg, 0,
                    TR::Compiler->om.sizeofReferenceAddress()),
                iCursor);
            ...
            if ((uint32_t)allocSize > maxSafeSize) {
                if (sizeInReg) {
                    iCursor = generateTrg1Src2Instruction(cg, TR::InstOpCode::add, node, temp1Reg, resReg, sizeReg,
                        iCursor);
                } else {
                    iCursor = generateTrg1Src1ImmInstruction(cg, TR::InstOpCode::addi, node, temp1Reg, resReg,
                        allocSize, iCursor);
                ...
                iCursor = generateTrg1Src2Instruction(cg, TR::InstOpCode::Op_cmpl, node, condReg, temp1Reg, resReg,
                    iCursor);
            ...
            iCursor = generateTrg1Src2Instruction(cg, TR::InstOpCode::Op_cmpl, node, condReg, temp1Reg, heapTopReg,
                iCursor);
            iCursor = generateConditionalBranchInstruction(cg, TR::InstOpCode::bgt, node, callLabel, condReg, iCursor);

            // todo64: I think these need to be ldarx/stdcx. in 64-bit mode
            iCursor = generateTrg1MemInstruction(cg, TR::InstOpCode::lwarx, node, temp2Reg,
                TR::MemoryReference::createWithIndexReg(cg, dataSizeReg, zeroReg, 4), iCursor);
            ...
            iCursor = generateMemSrc1Instruction(cg, TR::InstOpCode::stwcx_r, node,
                TR::MemoryReference::createWithIndexReg(cg, dataSizeReg, zeroReg, 4), temp1Reg, iCursor);

            iCursor = generateConditionalBranchInstruction(cg, TR::InstOpCode::bne, PPCOpProp_BranchUnlikely, node,
                tryLabel, condReg, iCursor);
        }
    }
}
    ...
    bool needZeroInit, TR::CodeGenerator *cg)
{
    ...
    if (comp->compileRelocatableCode() && !comp->getOption(TR_UseSymbolValidationManager)) {
        if (node->getOpCodeValue() == TR::newarray) {
            iCursor = generateTrg1MemInstruction(cg, TR::InstOpCode::Op_load, node, temp1Reg,
                ...
                iCursor);
            iCursor = generateTrg1MemInstruction(cg, TR::InstOpCode::Op_load, node, temp1Reg,
                TR::MemoryReference::createWithDisplacement(cg, temp1Reg,
                    ((TR_J9VM *)fej9)->getPrimitiveArrayOffsetInJavaVM(node->getSecondChild()->getInt()),
                    TR::Compiler->om.sizeofReferenceAddress()),
                iCursor);
            clzReg = temp1Reg;
    ...
    if (clzReg == NULL) {
        ...
        if (offset != PTOC_FULL_INDEX) {
            ...
            if (offset < LOWER_IMMED || offset > UPPER_IMMED) {
                TR_ASSERT_FATAL_WITH_NODE(node, 0x00008000 != HI_VALUE(offset),
                    "TOC offset (0x%x) is unexpectedly high. Can not encode upper 16 bits into an addis instruction.",
                    offset);
                iCursor = generateTrg1Src1ImmInstruction(cg, TR::InstOpCode::addis, node, temp1Reg,
                    cg->getTOCBaseRegister(), HI_VALUE(offset), iCursor);
                iCursor = generateTrg1MemInstruction(cg, TR::InstOpCode::Op_load, node, temp1Reg,
                    TR::MemoryReference::createWithDisplacement(cg, temp1Reg, LO_VALUE(offset),
                        TR::Compiler->om.sizeofReferenceAddress()),
                    iCursor);
            } else {
                iCursor = generateTrg1MemInstruction(cg, TR::InstOpCode::Op_load, node, temp1Reg,
                    TR::MemoryReference::createWithDisplacement(cg, cg->getTOCBaseRegister(), offset,
                        TR::Compiler->om.sizeofReferenceAddress()),
                    iCursor);
            }
            if (orFlag != 0) {
                iCursor = generateTrg1Src1ImmInstruction(cg, TR::InstOpCode::ori, node, temp1Reg, temp1Reg, orFlag,
                    iCursor);
            }
        } else {
        ...
        if (TR::Compiler->om.compressObjectReferences())
            ...
            iCursor = generateMemSrc1Instruction(cg, TR::InstOpCode::stw, node,
                TR::MemoryReference::createWithDisplacement(cg, resReg,
                    (int32_t)TR::Compiler->om.offsetOfObjectVftField(), 4),
                temp1Reg, iCursor);
        else
            iCursor = generateMemSrc1Instruction(cg, TR::InstOpCode::Op_st, node,
    ...
    if (isStaticFlag) {
        ...
        if (staticFlag != 0 || needZeroInit) {
            ...
            // Store the flags
            iCursor = generateMemSrc1Instruction(cg, TR::InstOpCode::stw, node,
                new (cg->trHeapMemory()) TR::MemoryReference(resReg, (int32_t)TMP_OFFSETOF_J9OBJECT_FLAGS, 4, cg),
                temp2Reg, iCursor);
        }
    } else if (!comp->getOptions()->realTimeGC()) {
        ...
        iCursor = generateTrg1MemInstruction(cg, TR::InstOpCode::Op_load, node, temp2Reg,
            TR::MemoryReference::createWithDisplacement(cg, metaReg,
                offsetof(J9VMThread, allocateThreadLocalHeap.objectFlags), TR::Compiler->om.sizeofReferenceAddress()),
            iCursor);
        ...
        if (staticFlag != 0) {
            if ((staticFlag & 0xFFFF0000) != 0) {
                iCursor = generateTrg1Src1ImmInstruction(cg, TR::InstOpCode::oris, node, temp2Reg, temp2Reg,
                    (staticFlag >> 16) & 0x0000FFFF, iCursor);
            }
            if ((staticFlag & 0x0000FFFF) != 0) {
                iCursor = generateTrg1Src1ImmInstruction(cg, TR::InstOpCode::ori, node, temp2Reg, temp2Reg,
                    staticFlag & 0x0000FFFF, iCursor);
            }
        }
        iCursor = generateMemSrc1Instruction(cg, TR::InstOpCode::stw, node,
            TR::MemoryReference::createWithDisplacement(cg, resReg, (int32_t)TMP_OFFSETOF_J9OBJECT_FLAGS, 4), temp2Reg,
            iCursor);
    }
    ...
    if (clazz && (lwOffset > 0)) {
        ...
        if (0 != lwInitialValue) {
            ...
            iCursor = generateMemSrc1Instruction(cg, storeOpCode, node,
                TR::MemoryReference::createWithDisplacement(cg, resReg, lwOffset, lockSize), temp1Reg, iCursor);
        }
    }
}

static void genAlignArray(TR::Node *node, TR::Instruction *&iCursor, bool isVariableLen, TR::Register *resReg,
    int32_t objectSize, int32_t dataBegin, TR::Register *dataSizeReg, TR::Register *condReg, TR::Register *temp1Reg,
    TR::Register *temp2Reg, TR::CodeGenerator *cg)
{
    ...
    if (isVariableLen) {
        ...
        iCursor = generateMemSrc1Instruction(cg, TR::InstOpCode::stw, node,
            TR::MemoryReference::createWithDisplacement(cg, temp1Reg, dataBegin, 4), temp2Reg, iCursor);
    } else if (objectSize > UPPER_IMMED) {
        iCursor = loadConstant(cg, node, objectSize, temp1Reg, iCursor);
        iCursor = generateMemSrc1Instruction(cg, TR::InstOpCode::stw, node,
            TR::MemoryReference::createWithIndexReg(cg, resReg, temp1Reg, 4), temp2Reg, iCursor);
    } else {
        iCursor = generateMemSrc1Instruction(cg, TR::InstOpCode::stw, node,
            TR::MemoryReference::createWithDisplacement(cg, resReg, objectSize, 4), temp2Reg, iCursor);
    }
    ...
    iCursor = generateMemSrc1Instruction(cg, TR::InstOpCode::stw, node,
        TR::MemoryReference::createWithDisplacement(cg, resReg, (int32_t)0, 4), temp2Reg, iCursor);
...
static void genInitArrayHeader(TR::Node *node, TR::Instruction *&iCursor, bool isVariableLen,
    TR_OpaqueClassBlock *clazz, TR::Register *classReg, TR::Register *resReg, TR::Register *zeroReg,
    TR::Register *condReg, TR::Register *eNumReg, TR::Register *dataSizeReg, TR::Register *temp1Reg,
    TR::Register *temp2Reg, TR::RegisterDependencyConditions *conditions, bool needZeroInit, TR::CodeGenerator *cg)
{
    TR_J9VMBase *fej9 = (TR_J9VMBase *)(cg->fe());
    TR::Register *instanceSizeReg;

    genInitObjectHeader(node, iCursor, clazz, classReg, resReg, zeroReg, condReg, temp1Reg, temp2Reg, NULL, conditions,
        needZeroInit, cg);
    ...
    if ((node->getOpCodeValue() == TR::newarray || node->getOpCodeValue() == TR::anewarray)
        ...
        && (node->getFirstChild()->getInt()
            == 0)) { // constant zero length non-packed array
                     // Zero length arrays are discontiguous (i.e. they also need the discontiguous length field to be
                     // 0) because they are indistinguishable from non-zero length discontiguous arrays
        iCursor = generateMemSrc1Instruction(cg, TR::InstOpCode::stw, node,
...
static void genZeroInit(TR::CodeGenerator *cg, TR::Node *node, TR::Register *objectReg, int32_t headerSize,
    int32_t totalSize, bool useInitInfo)
{
    TR_ASSERT((totalSize - headerSize > 0) && ((totalSize - headerSize) % 4 == 0),
        "Expecting non-zero word-aligned data size");
    ...
    if (useInitInfo) {
        ...
        if (comp->target().is64Bit()) {
            ...
            while (bvi.hasMoreElements()) {
                ...
                if (lastNotInit == -1) {
                    // currently looking at only one slot (either just starting, or just emitted double-word store in
                    // last iter)
                    ...
                    generateMemSrc1Instruction(cg, TR::InstOpCode::std, node,
                        TR::MemoryReference::createWithDisplacement(cg, objectReg, headerSize + lastNotInit * 4, 8),
                        zeroReg);
                    lastNotInit = -1;
                } else {
                    ...
                    generateMemSrc1Instruction(cg, TR::InstOpCode::stw, node,
                        TR::MemoryReference::createWithDisplacement(cg, objectReg, headerSize + lastNotInit * 4, 4),
                        zeroReg);
                    lastNotInit = currElem;
                }
            }
            ...
            while (bvi.hasMoreElements()) {
                generateMemSrc1Instruction(cg, TR::InstOpCode::stw, node,
                    TR::MemoryReference::createWithDisplacement(cg, objectReg,
                        headerSize + bvi.getNextElement() * TR::Compiler->om.sizeofReferenceAddress(),
                        TR::Compiler->om.sizeofReferenceAddress()),
                    zeroReg);
            }
        }
        ...
        int32_t ofs;
        for (ofs = headerSize; ofs < totalSize - TR::Compiler->om.sizeofReferenceAddress();
             ofs += TR::Compiler->om.sizeofReferenceAddress()) {
            generateMemSrc1Instruction(cg, TR::InstOpCode::Op_st, node,
                TR::MemoryReference::createWithDisplacement(cg, objectReg, ofs,
                    TR::Compiler->om.sizeofReferenceAddress()),
                zeroReg);
        }
        if (ofs + 4 == totalSize)
            generateMemSrc1Instruction(cg, TR::InstOpCode::stw, node,
                TR::MemoryReference::createWithDisplacement(cg, objectReg, ofs, 4), zeroReg);
        else if (ofs + 8 == totalSize)
            generateMemSrc1Instruction(cg, TR::InstOpCode::std, node,
                TR::MemoryReference::createWithDisplacement(cg, objectReg, ofs, 8), zeroReg);
    }
...
TR::Register *J9::Power::TreeEvaluator::VMnewEvaluator(TR::Node *node, TR::CodeGenerator *cg)
{
    ...
    TR_ASSERT(objectSize <= 0 || (objectSize & (TR::Compiler->om.sizeofReferenceAddress() - 1)) == 0,
        "object size causes an alignment problem");
    ...
    if (!isVariableLen) {
        allocateSize = (allocateSize + TR::Compiler->om.getObjectAlignmentInBytes() - 1)
            & (-TR::Compiler->om.getObjectAlignmentInBytes());
    }
    ...
    if (doInline) {
        ...
        if (opCode == TR::New) {
            // classReg is passed to the VM helper on the slow path and subsequently clobbered; copy it for later nodes
            // if necessary
            ...
            insertType = (opCode == TR::newarray && secondChild->getDataType() == TR::Int32
                && secondChild->getReferenceCount() == 1 && secondChild->getOpCode().isLoadConst());
            ...
            if (comp->compileRelocatableCode() && comp->getOption(TR_UseSymbolValidationManager)) {
                ...
                // classReg is passed to the VM helper on the slow path and subsequently clobbered; copy it for later
                // nodes if necessary
        ...
        TR::addDependency(conditions, tmp4Reg, TR::RealRegister::gr11, TR_GPR,
            cg); // used by TR::PPCAllocPrefetchSnippet
        ...
        TR::addDependency(conditions, tmp3Reg, TR::RealRegister::gr10, TR_GPR,
            cg); // used by TR::PPCAllocPrefetchSnippet
        ...
        genHeapAlloc(node, iCursor, clazz, isVariableLen, enumReg, classReg, resReg, zeroReg, condReg, dataSizeReg,
            tmp5Reg, tmp4Reg, tmp3Reg, tmp6Reg, tmp7Reg, callLabel, doneLabel, allocateSize, elementSize, usingTLH,
            needZeroInit, conditions, cg);

        // Call out to VM runtime helper if needed.
        TR::Register *objReg = cg->allocateCollectedReferenceRegister();
        TR_PPCOutOfLineCodeSection *outlinedHelperCall = new (cg->trHeapMemory())
            TR_PPCOutOfLineCodeSection(node, TR::acall, objReg, callLabel, callReturnLabel, cg);
        cg->getPPCOutOfLineCodeSectionList().push_front(outlinedHelperCall);

        if (isArray) {
            // Align the array if necessary.
            if (doublewordAlign && !comp->getOptions()->realTimeGC())
                genAlignArray(node, iCursor, isVariableLen, resReg, objectSize, dataBegin, dataSizeReg, condReg,
                    tmp5Reg, tmp4Reg, cg);
            if (comp->compileRelocatableCode()
                && (opCode == TR::anewarray || comp->getOption(TR_UseSymbolValidationManager)))
                genInitArrayHeader(node, iCursor, isVariableLen, clazz, classReg, resReg, zeroReg, condReg, enumReg,
                    dataSizeReg, tmp5Reg, tmp4Reg, conditions, needZeroInit, cg);
            else
                genInitArrayHeader(node, iCursor, isVariableLen, clazz, NULL, resReg, zeroReg, condReg, enumReg,
                    dataSizeReg, tmp5Reg, tmp4Reg, conditions, needZeroInit, cg);
            ...
            if (TR::Compiler->om.isOffHeapAllocationEnabled()) {
                ...
                if (TR::Compiler->om.compressObjectReferences()) {
                    TR_ASSERT_FATAL_WITH_NODE(node,
                        (fej9->getOffsetOfDiscontiguousDataAddrField() - fej9->getOffsetOfContiguousDataAddrField())
                            == 8,
                        "Offset of dataAddr field in discontiguous array is expected to be 8 bytes more than "
                        "contiguous array. "
                ...
                if (isVariableLen) {
                    ...
                    logprintf(trace, log, "Node (%p): Dealing with full/compressed refs variable length array.\n",
                        node);
                    ...
                    iCursor = generateTrg1Src1ImmInstruction(cg, TR::InstOpCode::cmpli4, node, condReg, enumReg, 0,
                        iCursor);
                    iCursor = generateConditionalBranchInstruction(cg, TR::InstOpCode::beq, node, zeroArrayLabel,
                        condReg, iCursor);

                    // Load first data element address
                    iCursor = generateTrg1Src1ImmInstruction(cg, TR::InstOpCode::addi, node, firstDataElementReg,
                        resReg, TR::Compiler->om.contiguousArrayHeaderSizeInBytes(), iCursor);
                    dataAddrSlotMR = TR::MemoryReference::createWithDisplacement(cg, resReg,
                        fej9->getOffsetOfContiguousDataAddrField(), 8);
                    // Store the first data element address to dataAddr slot
                    iCursor = generateMemSrc1Instruction(cg, TR::InstOpCode::std, node, dataAddrSlotMR,
                        firstDataElementReg, iCursor);
                    ...
                    if (needZeroInit) {
                        ...
                        iCursor = generateMemSrc1Instruction(cg, TR::InstOpCode::std, node,
                            TR::MemoryReference::createWithDisplacement(cg, resReg,
                                fej9->getOffsetOfDiscontiguousDataAddrField(), 8),
                            zeroReg, iCursor);
                    }

                    iCursor = generateLabelInstruction(cg, TR::InstOpCode::label, node, dataAddrInitDoneLabel, iCursor);
                } else if (!isVariableLen && node->getFirstChild()->getOpCode().isLoadConst()
                    && node->getFirstChild()->getInt() == 0) {
                    logprintf(trace, log,
                        "Node (%p): Dealing with full/compressed refs fixed length zero size array.\n", node);
                    ...
                    if (needZeroInit) {
                        ...
                        iCursor = generateMemSrc1Instruction(cg, TR::InstOpCode::std, node, dataAddrSlotMR, zeroReg,
                            iCursor);
                    }
                } else {
                    logprintf(trace, log,
                        "Node (%p): Dealing with either full/compressed refs fixed length non-zero size array or full "
            ...
            if (generateArraylets) {
                ...
                iCursor = generateTrg1Src1ImmInstruction(cg, TR::InstOpCode::addi, node, tmp4Reg, resReg, dataBegin,
                    iCursor);
                if (TR::Compiler->om.compressedReferenceShiftOffset() > 0)
                    iCursor = generateShiftRightLogicalImmediateLong(cg, node, tmp4Reg, tmp4Reg,
                        TR::Compiler->om.compressedReferenceShiftOffset(), iCursor);
                iCursor = generateMemSrc1Instruction(cg,
                    (comp->target().is64Bit() && !comp->useCompressedPointers()) ? TR::InstOpCode::std
                                                                                 : TR::InstOpCode::stw,
                    node,
                    TR::MemoryReference::createWithDisplacement(cg, resReg, fej9->getFirstArrayletPointerOffset(comp),
                        comp->useCompressedPointers() ? 4 : TR::Compiler->om.sizeofReferenceAddress()),
                    tmp4Reg, iCursor);
            ...
            genInitObjectHeader(node, iCursor, clazz, classReg, resReg, zeroReg, condReg, tmp5Reg, tmp4Reg,
                packedRegOffs, conditions, needZeroInit, cg);
        }
        ...
        if (needZeroInit && (!isConstantZeroLenArrayAlloc)) {
            ...
            if (!node->canSkipZeroInitialization() && (initInfo == NULL || initInfo->numZeroInitSlots > 0)) {
                if (!isVariableLen) {
                    if (initInfo != NULL && initInfo->zeroInitSlots != NULL && initInfo->numZeroInitSlots <= 9
                        && objectSize <= UPPER_IMMED) {
                        TR_BitVectorIterator bvi(*initInfo->zeroInitSlots);
                        ...
                        if (comp->target().is64Bit()) {
                            ...
                            // Try to group set of words into smallest groupings using double-word stores (where
                            // possible)
                            while (bvi.hasMoreElements()) {
                                currElem = bvi.getNextElement();

                                if (lastNotInit == -1) {
                                    // currently looking at only one slot (either just starting, or just emitted
                                    // double-word store in last iter)
                                    lastNotInit = currElem;
                                } else if (currElem == lastNotInit + 1) {
                                    // consecutive slots, can use std
                                    iCursor = generateMemSrc1Instruction(cg, TR::InstOpCode::std, node,
                                        TR::MemoryReference::createWithDisplacement(cg, resReg,
                                            lastNotInit * 4 + dataBegin, 8),
                                        zeroReg, iCursor);
                            ...
                            if (lastNotInit != -1) {
                                // Last slot is not consecutive with other slots, hasn't been initialized yet
                                iCursor = generateMemSrc1Instruction(cg, TR::InstOpCode::stw, node,
                                    TR::MemoryReference::createWithDisplacement(cg, resReg, lastNotInit * 4 + dataBegin,
                                        4),
                                    zeroReg, iCursor);
                            }
                        } else {
                            while (bvi.hasMoreElements()) {
                                iCursor = generateMemSrc1Instruction(cg, TR::InstOpCode::stw, node,
                                    TR::MemoryReference::createWithDisplacement(cg, resReg,
                                        bvi.getNextElement() * 4 + dataBegin, 4),
                                    zeroReg, iCursor);
                            }
                        }
                    } else if (objectSize <= (TR::Compiler->om.sizeofReferenceAddress() * 12)) {
                        for (idx = dataBegin; idx < (objectSize - TR::Compiler->om.sizeofReferenceAddress());
                             idx += TR::Compiler->om.sizeofReferenceAddress()) {
                            iCursor = generateMemSrc1Instruction(cg, TR::InstOpCode::Op_st, node,
                                TR::MemoryReference::createWithDisplacement(cg, resReg, idx,
                                    TR::Compiler->om.sizeofReferenceAddress()),
                                zeroReg, iCursor);
                        }
                        if (idx + 4 == objectSize)
                            iCursor = generateMemSrc1Instruction(cg, TR::InstOpCode::stw, node,
                                TR::MemoryReference::createWithDisplacement(cg, resReg, idx, 4), zeroReg, iCursor);
                        else if (idx + 8 == objectSize)
                            iCursor = generateMemSrc1Instruction(cg, TR::InstOpCode::std, node,
                                TR::MemoryReference::createWithDisplacement(cg, resReg, idx, 8), zeroReg, iCursor);
                    } else {
                        ...
                        if (!disableUnrollNewInitLoop1) {
                            ...
                            iCursor = generateTrg1Src1ImmInstruction(cg, TR::InstOpCode::addi, node, tmp4Reg, resReg,
                                dataBegin, iCursor);
                            if (loopCount > 0) {
                                if (loopCount > 1) {
                                    iCursor = loadConstant(cg, node, loopCount, tmp5Reg, iCursor);
                                    iCursor
                                        = generateSrc1Instruction(cg, TR::InstOpCode::mtctr, node, tmp5Reg, 0, iCursor);
                                    iCursor
                                        = generateLabelInstruction(cg, TR::InstOpCode::label, node, loopStart, iCursor);
                                }

                                for (int32_t i = 0; i < unrollFactor; i++)
                                    iCursor = generateMemSrc1Instruction(cg, TR::InstOpCode::Op_st, node,
                                        TR::MemoryReference::createWithDisplacement(cg, tmp4Reg, i * width, width),
                                        zeroReg, iCursor);

                                iCursor = generateTrg1Src1ImmInstruction(cg, TR::InstOpCode::addi2, node, tmp4Reg,
                                    tmp4Reg, (unrollFactor * width), iCursor);
                                if (loopCount > 1)
                                    iCursor = generateConditionalBranchInstruction(cg, TR::InstOpCode::bdnz, node,
                                        loopStart, condReg, iCursor);
                            }

                            for (int32_t i = 0; i < residueCount; i++)
                                iCursor = generateMemSrc1Instruction(cg, TR::InstOpCode::Op_st, node,
                                    TR::MemoryReference::createWithDisplacement(cg, tmp4Reg, i * width, width), zeroReg,
                                    iCursor);

                            if (res2 && residueCount != 0)
                                iCursor = generateTrg1Src1ImmInstruction(cg, TR::InstOpCode::addi2, node, tmp4Reg,
                                    tmp4Reg, residueCount * width, iCursor);

                            if (comp->target().is64Bit() && res2 >= 4) // Should only be 0 or 4 here
                                iCursor = generateMemSrc1Instruction(cg, TR::InstOpCode::stw, node,
                                    TR::MemoryReference::createWithDisplacement(cg, tmp4Reg, 0, 4), zeroReg, iCursor);
                        } else {
                            ...
                            iCursor = generateMemSrc1Instruction(cg, TR::InstOpCode::Op_st, node,
                                TR::MemoryReference::createWithDisplacement(cg, tmp4Reg, 0,
                                    TR::Compiler->om.sizeofReferenceAddress()),
                                zeroReg, iCursor);
                            iCursor = generateMemSrc1Instruction(cg, TR::InstOpCode::Op_st, node,
                                TR::MemoryReference::createWithDisplacement(cg, tmp4Reg,
                                    TR::Compiler->om.sizeofReferenceAddress(),
                                    TR::Compiler->om.sizeofReferenceAddress()),
                                zeroReg, iCursor);
                            if (comp->target().is32Bit()) {
                                iCursor = generateMemSrc1Instruction(cg, TR::InstOpCode::stw, node,
                                    TR::MemoryReference::createWithDisplacement(cg, tmp4Reg, 8, 4), zeroReg, iCursor);
                                iCursor = generateMemSrc1Instruction(cg, TR::InstOpCode::stw, node,
                                    TR::MemoryReference::createWithDisplacement(cg, tmp4Reg, 12, 4), zeroReg, iCursor);
                            }
                            iCursor = generateTrg1Src1ImmInstruction(cg, TR::InstOpCode::addi, node, tmp4Reg, tmp4Reg,
                                16, iCursor);
                            iCursor = generateConditionalBranchInstruction(cg, TR::InstOpCode::bdnz, node, initLoop,
                                condReg, iCursor);
                            ...
                            if (residue & 0x2) {
                                ...
                                if (comp->target().is32Bit())
                                    iCursor = generateMemSrc1Instruction(cg, TR::InstOpCode::stw, node,
                                        TR::MemoryReference::createWithDisplacement(cg, tmp4Reg, 4, 4), zeroReg,
                                        iCursor);
                            ...
                            if (residue & 0x1) {
                                iCursor = generateMemSrc1Instruction(cg, TR::InstOpCode::stw, node,
                                    TR::MemoryReference::createWithDisplacement(cg, tmp4Reg, idx, 4), zeroReg, iCursor);
                            }
                        }
                    }
                } else // Init variable length array
                {
                    ...
                    if (!disableUnrollNewInitLoop2) {
                        ...
                        iCursor = generateTrg1Src1ImmInstruction(cg, TR::InstOpCode::addi, node, tmp5Reg, resReg,
                            (dataBegin - width), iCursor);
                        iCursor = generateTrg1Src2Instruction(cg, TR::InstOpCode::add, node, tmp4Reg, tmp5Reg,
                            dataSizeReg, iCursor);
                        iCursor = generateTrg1Src2Instruction(cg, TR::InstOpCode::Op_cmpl, node, condReg, tmp4Reg,
                            tmp5Reg, iCursor);
                        iCursor = generateConditionalBranchInstruction(cg, TR::InstOpCode::beq, node, doneLabel,
                            condReg, iCursor);

                        iCursor = generateTrg1Src1ImmInstruction(cg, TR::InstOpCode::addi, node, tmp6Reg, tmp4Reg,
                            (int32_t)(-unrollFactor * width), iCursor);

                        // Switch order of compare to avoid FXU reject in P6 for later store instruction
                        iCursor = generateTrg1Src2Instruction(cg, TR::InstOpCode::Op_cmpl, node, condReg, tmp6Reg,
                            tmp5Reg, iCursor);
                        iCursor = generateConditionalBranchInstruction(cg, TR::InstOpCode::blt, node, resLabel, condReg,
                            iCursor);

                        iCursor = generateLabelInstruction(cg, TR::InstOpCode::label, node, loopLabel, iCursor);
                        for (; i <= unrollFactor; i++)
                            iCursor = generateMemSrc1Instruction(cg,
                                (width == 8) ? TR::InstOpCode::std : TR::InstOpCode::stw, node,
                                TR::MemoryReference::createWithDisplacement(cg, tmp5Reg, i * width, width), zeroReg,
                                iCursor);
                        iCursor = generateTrg1Src1ImmInstruction(cg, TR::InstOpCode::addi, node, tmp5Reg, tmp5Reg,
                            (int32_t)(unrollFactor * width), iCursor);

                        iCursor = generateTrg1Src2Instruction(cg, TR::InstOpCode::Op_cmpl, node, condReg, tmp6Reg,
                            tmp5Reg, iCursor);
                        iCursor = generateConditionalBranchInstruction(cg, TR::InstOpCode::bge, node, loopLabel,
                            condReg, iCursor);
                        ...
                        if (!disableNewZeroInit) {
                            ...
                            iCursor = generateTrg1Src2Instruction(cg, TR::InstOpCode::Op_cmpl, node, condReg, tmp6Reg,
                                tmp5Reg, iCursor);
                            iCursor = generateConditionalBranchInstruction(cg, TR::InstOpCode::beq, node, left4Label,
                                condReg, iCursor);
                            iCursor = generateConditionalBranchInstruction(cg, TR::InstOpCode::blt, node, doneLabel,
                                condReg, iCursor);

                            // Do 8-byte zero init and restart loop
                            iCursor = generateMemSrc1Instruction(cg, TR::InstOpCode::Op_st, node,
                                TR::MemoryReference::createWithDisplacement(cg, tmp5Reg, width, width), zeroReg,
                                iCursor);

                            if (comp->target().is32Bit()) {
                                iCursor = generateMemSrc1Instruction(cg, TR::InstOpCode::stw, node,
                                    TR::MemoryReference::createWithDisplacement(cg, tmp5Reg, 8, 4), zeroReg, iCursor);
                            }
                            ...
                            iCursor = generateMemSrc1Instruction(cg, TR::InstOpCode::stw, node,
                                TR::MemoryReference::createWithDisplacement(cg, tmp5Reg, width, 4), zeroReg, iCursor);
                        } else {
                            /* Need this in 64-bit as well */
                            iCursor = generateTrg1Src2Instruction(cg, TR::InstOpCode::subf, node, tmp6Reg, tmp5Reg,
                                tmp4Reg, iCursor);
                            iCursor = generateTrg1Src1ImmInstruction(cg, TR::InstOpCode::Op_cmpli, node, condReg,
                                tmp6Reg, 4, iCursor);
                            iCursor = generateConditionalBranchInstruction(cg, TR::InstOpCode::beq, node, left4Label,
                                condReg, iCursor);

                            iCursor = generateMemSrc1Instruction(cg, TR::InstOpCode::Op_st, node,
                                TR::MemoryReference::createWithDisplacement(cg, tmp5Reg, 8, width), zeroReg, iCursor);

                            if (comp->target().is32Bit()) {
                                iCursor = generateMemSrc1Instruction(cg, TR::InstOpCode::stw, node,
                                    TR::MemoryReference::createWithDisplacement(cg, tmp5Reg, 12, 4), zeroReg, iCursor);
                            }
                        ...
                        iCursor = generateTrg1Src1Imm2Instruction(cg, TR::InstOpCode::rlwinm, node, tmp5Reg,
                            dataSizeReg, 30, 0x3FFFFFFF, iCursor);
                        iCursor = generateTrg1Src1ImmInstruction(cg, TR::InstOpCode::cmpi4, node, condReg, tmp5Reg, 0,
                            iCursor);
                        iCursor = generateConditionalBranchInstruction(cg, TR::InstOpCode::beq, node, doneLabel,
                            condReg, iCursor);
                        iCursor = generateSrc1Instruction(cg, TR::InstOpCode::mtctr, node, tmp5Reg, 0, iCursor);
                        ...
                        iCursor = generateLabelInstruction(cg, TR::InstOpCode::label, node, initLoop, iCursor);
                        iCursor = generateMemSrc1Instruction(cg, TR::InstOpCode::stw, node,
                            TR::MemoryReference::createWithDisplacement(cg, tmp4Reg, 4, 4), zeroReg, iCursor);
                        iCursor = generateTrg1Src1ImmInstruction(cg, TR::InstOpCode::addi, node, tmp4Reg, tmp4Reg, 4,
                            iCursor);
                        iCursor = generateConditionalBranchInstruction(cg, TR::InstOpCode::bdnz, node, initLoop,
                            condReg, iCursor);
                    }
...
static bool simpleReadMonitor(TR::Node *node, TR::CodeGenerator *cg, TR::Node *objNode, TR::Register *objReg,
    TR::Register *objectClassReg, TR::Register *condReg, TR::Register *lookupOffsetReg)
{
    ...
    if (comp->getOption(TR_TraceCG) || debug("traceGRA")) {
        OMR::Logger *log = comp->log();
        log->println();
        comp->getDebug()->dumpSingleTreeWithInstrs(log, nextTreeTop, NULL, true, false, true,
            comp->getOption(TR_TraceRegisterPressureDetails));
        log->println();
        comp->getDebug()->dumpSingleTreeWithInstrs(log, secondNextTreeTop, NULL, true, false, true,
            comp->getOption(TR_TraceRegisterPressureDetails));
        log->flush();
    }
    ...
    if (nextTopNode->getSymbolReference()->getSymbol()->isInternalPointer()) {
        loadResultReg->setPinningArrayPointer(nextTopNode->getSymbolReference()
                                                  ->getSymbol()
    ...
    if (tempMR.getMemoryReference()->getBaseRegister() != objReg) {
        TR::addDependency(conditions, tempMR.getMemoryReference()->getBaseRegister(), TR::RealRegister::NoReg, TR_GPR,
            cg);
    ...
    bool altsequence = (comp->target().cpu.is(OMR_PROCESSOR_PPC_GP) || comp->target().cpu.is(OMR_PROCESSOR_PPC_GR))
        && tempMR.getMemoryReference()->getBaseRegister() == objReg;
    ...
    if (objectClassReg)
        ...
        generateTrg1ImmInstruction(cg, TR::InstOpCode::li, node, offsetReg,
            fej9->getByteOffsetToLockword((TR_OpaqueClassBlock *)cg->getMonClass(node)));
    generateLabelInstruction(cg, TR::InstOpCode::label, node, loopLabel);
    generateTrg1MemInstruction(cg, opCode, node, monitorReg,
        TR::MemoryReference::createWithIndexReg(cg, baseReg, offsetReg, lockSize));
    generateTrg1Src1ImmInstruction(cg, TR::InstOpCode::Op_cmpli, node, cndReg, monitorReg, 0);
    TR::Instruction *gcPoint
        = generateConditionalBranchInstruction(cg, TR::InstOpCode::bne, node, recurCheckLabel, cndReg);
    gcPoint->PPCNeedsGCMap(0xFFFFFFFF);
    ...
    if (altsequence) {
        ...
        generateMemSrc1Instruction(cg, opCode, node,
            TR::MemoryReference::createWithIndexReg(cg, baseReg, offsetReg, lockSize), metaReg);
        generateConditionalBranchInstruction(cg, TR::InstOpCode::bne, PPCOpProp_BranchUnlikely, node, loopLabel,
            cndReg);
        generateTrg1Src2Instruction(cg, TR::InstOpCode::OR, node, tempMR.getMemoryReference()->getBaseRegister(),
            tempMR.getMemoryReference()->getBaseRegister(), monitorReg);
        ...
        generateMemSrc1Instruction(cg, opCode, node,
            TR::MemoryReference::createWithDisplacement(cg, baseReg,
                objectClassReg ? 0 : fej9->getByteOffsetToLockword((TR_OpaqueClassBlock *)cg->getMonClass(node)),
                lockSize),
            monitorReg);
    } else {
        generateTrg1Src2Instruction(cg, TR::InstOpCode::OR, node, tempMR.getMemoryReference()->getBaseRegister(),
        ...
        if (comp->target().is64Bit() && !fej9->generateCompressedLockWord())
            ...
            opCode = TR::InstOpCode::stwcx_r;
        generateMemSrc1Instruction(cg, opCode, node,
            TR::MemoryReference::createWithIndexReg(cg, baseReg, offsetReg, lockSize), monitorReg);
        generateConditionalBranchInstruction(cg, TR::InstOpCode::bne, PPCOpProp_BranchUnlikely, node, loopLabel,
            cndReg);
    }
    ...
    TR::Snippet *snippet
        = new (cg->trHeapMemory()) TR::PPCReadMonitorSnippet(cg, node, secondNextTopNode, recurCheckLabel,
            monExitCallLabel, restartLabel, loadOpCode, tempMR.getMemoryReference()->getOffset(), objectClassReg);
    cg->addSnippet(snippet);
...
void J9::Power::TreeEvaluator::generateCheckForValueMonitorEnterOrExit(TR::Node *node, TR::LabelSymbol *helperCallLabel,
    TR::Register *objReg, TR::Register *objectClassReg, TR::Register *temp1Reg, TR::Register *temp2Reg,
    TR::Register *condReg, TR::CodeGenerator *cg, int32_t classFlag)
{
    // get class of object
    generateLoadJ9Class(node, objectClassReg, objReg, cg);
    ...
    TR_J9VMBase *fej9 = (TR_J9VMBase *)(cg->fe());
    TR::MemoryReference *classFlagsMemRef = TR::MemoryReference::createWithDisplacement(cg, objectClassReg,
        static_cast<uintptr_t>(fej9->getOffsetOfClassFlags()), 4);
...
TR::Register *J9::Power::TreeEvaluator::VMmonentEvaluator(TR::Node *node, TR::CodeGenerator *cg)
{
    ...
    // If object is not known to be value type or value based class at compile time, check at run time
    if (isMonitorValueBasedOrValueType == TR_maybe) {
        generateCheckForValueMonitorEnterOrExit(node, callLabel, objReg, objectClassReg, tempReg, offsetReg, condReg,
            cg, J9_CLASS_DISALLOWS_LOCKING_FLAGS);
    }
    ...
    if (!fej9->isEnableGlobalLockReservationSet()) {
        ...
        if (reserveLocking)
            return reservationLockEnter(node, lwOffset, cg, conditions, baseReg, monitorReg, offsetReg, tempReg,
                condReg, callLabel);

        if (!simpleLocking && !reserveLocking && !normalLockWithReservationPreserving
            && simpleReadMonitor(node, cg, objNode, objReg, objectClassReg, condReg, lookupOffsetReg))
            return NULL;
    }
    ...
    if (true || !ppcSupportsReadMonitors || !node->isReadMonitor()) {
         ...
         * 2. Check if TID matches, highest RC bit is 0, Learning bit is clear and Inflated bit is clear. If so,
         * increment RC.
         ...
         * Check 3 catches New-PreLearning, New-AutoReserve, Learning-Unlocked, Learning-Locked and Inflated and sends
         * them to the VM.
         *
         *
         *    ld/lwz monitorReg, lwOffset(baseReg)
...
TR::Register *J9::Power::TreeEvaluator::VMarrayCheckEvaluator(TR::Node *node, TR::CodeGenerator *cg)
{
    ...
    if (!node->isArrayChkPrimitiveArray1() && !node->isArrayChkReferenceArray1() && !node->isArrayChkPrimitiveArray2()
        ...
        generateTrg1MemInstruction(cg, TR::InstOpCode::Op_load, node, tmp1Reg,
            TR::MemoryReference::createWithDisplacement(cg, tmp1Reg, (int32_t)offsetof(J9Class, classDepthAndFlags),
                4));
        ...
        snippet = new (cg->trHeapMemory())
            TR::PPCHelperCallSnippet(cg, node, snippetLabel, node->getSymbolReference(), doneLabel);
        cg->addSnippet(snippet);
    }
    ...
    // We have to take care of the un-equal class situation: both of them must be of reference array
        ...
        if (!node->isArrayChkReferenceArray1()) {
            ...
            generateTrg1Src1Imm2Instruction(cg, TR::InstOpCode::rlwinm, node, tmp2Reg, tmp1Reg, 0,
                OBJECT_HEADER_SHAPE_MASK);
            generateTrg1Src1ImmInstruction(cg, TR::InstOpCode::cmpi4, node, cndReg, tmp2Reg,
...
void J9::Power::TreeEvaluator::genArrayCopyWithArrayStoreCHK(TR::Node *node, TR::CodeGenerator *cg)
{
    ...
    TR::RegisterDependencyConditions *conditions = new (cg->trHeapMemory()) TR::RegisterDependencyConditions(
        pp.getNumberOfDependencyGPRegisters(), pp.getNumberOfDependencyGPRegisters(), cg->trMemory());
    ...
    if (aix_style_linkage
        ...
        iCursor = loadAddressConstant(cg, comp->compileRelocatableCode(), node, target_ip, temp1Reg, NULL, false,
            TR_ArrayCopyHelper);
        iCursor = loadAddressConstant(cg, comp->compileRelocatableCode(), node, target_toc, gr2Reg, iCursor, false,
            TR_ArrayCopyToc);
    } else {
...
#endif
        iCursor = loadAddressConstant(cg, doRelocation, node, (intptr_t)funcdescrptr, temp1Reg, NULL, false,
            TR_ArrayCopyHelper);
    }
    ...
    gcPoint = generateDepConditionalBranchInstruction(cg, TR::InstOpCode::bnel, node, exceptionSnippetLabel, cr0Reg,
        conditions->cloneAndFix(cg));
...
void J9::Power::TreeEvaluator::genWrtbarForArrayCopy(TR::Node *node, TR::Register *srcObjReg, TR::Register *dstObjReg,
    TR::CodeGenerator *cg)
{
    TR::Compilation *comp = cg->comp();
    ...
    ageCheckIsNeeded = (gcMode == gc_modron_wrtbar_oldcheck || gcMode == gc_modron_wrtbar_cardmark_and_oldcheck
    ...
    if (ageCheckIsNeeded) {
        ...
        gcPoint = generateDepImmSymInstruction(cg, TR::InstOpCode::bl, node,
            (uintptr_t)wbRef->getSymbol()->castToMethodSymbol()->getMethodAddress(),
            new (cg->trHeapMemory()) TR::RegisterDependencyConditions((uint8_t)0, 0, cg->trMemory()), wbRef, NULL);
...
static TR::Register *genCAS(TR::Node *node, TR::CodeGenerator *cg, TR::Register *objReg, TR::Register *offsetReg,
    TR::Register *oldVReg, TR::Register *newVReg, TR::Register *cndReg, TR::LabelSymbol *doneLabel, TR::Node *objNode,
    int32_t oldValue, bool oldValueInReg, int32_t dataSize, bool isReference, bool isExchange, bool casWithoutSync)
{
    TR::Compilation *comp = cg->comp();
    TR::Register *resultReg;
    ...
    generateTrg1MemInstruction(cg, reservedLoadOpCode, node, resultReg,
        TR::MemoryReference::createWithIndexReg(cg, objReg, offsetReg, dataSize));
    if (oldValueInReg)
    ...
    generateMemSrc1Instruction(cg, conditionalStoreOpCode, node,
        TR::MemoryReference::createWithIndexReg(cg, objReg, offsetReg, dataSize), newVReg);
...
static TR::Register *VMinlineCompareAndSetOrExchange(TR::Node *node, TR::CodeGenerator *cg, int32_t dataSize,
    bool isExchange)
{
    TR::Compilation *comp = cg->comp();
    ...
    if (caller) {
        TR_ResolvedMethod *m
            = fej9->createResolvedMethod(cg->trMemory(), caller, node->getSymbolReference()->getOwningMethod(comp));
    ...
    resultReg = genCAS(node, cg, objReg, offsetReg, oldVReg, newVReg, cndReg, doneLabel, objNode, oldValue,
        oldValueInReg, dataSize, false, isExchange, casWithoutSync);
...
static TR::Register *VMinlineCompareAndSetOrExchangeReference(TR::Node *node, TR::CodeGenerator *cg, bool isExchange)
{
...
#if defined(J9VM_GC_SPARSE_HEAP_ALLOCATION)
    bool isObjOffHeapDataAddr = (gcMode == gc_modron_wrtbar_cardmark_incremental
    ...
    if (!useOldCompareAndSwapObject && comp->target().is64Bit()
    ...
    if (!useOldCompareAndSwapObject) {
        ...
        if (comp->target().is64Bit() && (TR::Compiler->om.compressedReferenceShiftOffset() != 0)
    ...
    if (TR::Compiler->om.readBarrierType() != gc_modron_readbar_none) {
        ...
        generateTrg1MemInstruction(cg, loadOpCode, node, tmpReg,
            TR::MemoryReference::createWithIndexReg(cg, objReg, offsetReg, loadWidth));
        ...
        generateTrg1MemInstruction(cg, loadOpCode, node, evacuateReg,
            TR::MemoryReference::createWithDisplacement(cg, metaReg,
                comp->fej9()->thisThreadGetEvacuateBaseAddressOffset(), loadWidth));
        ...
        generateTrg1MemInstruction(cg, loadOpCode, node, evacuateReg,
            TR::MemoryReference::createWithDisplacement(cg, metaReg,
                comp->fej9()->thisThreadGetEvacuateTopAddressOffset(), loadWidth));
        generateTrg1Src2Instruction(cg, cmpOpCode, node, cndReg, tmpReg, evacuateReg);
    ...
    if (!comp->getOptions()->realTimeGC())
        resultReg = genCAS(node, cg, objReg, offsetReg, oldVReg, newVReg, cndReg, doneLabel, objNode, 0, true,
            TR::Compiler->om.sizeofReferenceField(), true, isExchange, false);
    ...
    if (doWrtBar && doCrdMrk) {
        ...
        VMnonNullSrcWrtBarCardCheckEvaluator(node, comp->useCompressedPointers() ? wrtbarSrcReg : newVReg, objReg,
            cndReg, temp1Reg, temp2Reg, temp3Reg, temp4Reg, doneLabel, conditions, comp->useCompressedPointers(), cg);
        ...
        VMnonNullSrcWrtBarCardCheckEvaluator(node, comp->useCompressedPointers() ? wrtbarSrcReg : newVReg, objReg,
            cndReg, temp1Reg, temp2Reg, dstAddrReg, NULL, wrtBarEndLabel, conditions, comp->useCompressedPointers(),
            cg);
...
void J9::Power::TreeEvaluator::restoreTOCRegister(TR::Node *node, TR::CodeGenerator *cg,
    TR::RegisterDependencyConditions *dependencies)
{
    ...
    generateTrg1MemInstruction(cg, TR::InstOpCode::Op_load, node, tocReg,
        TR::MemoryReference::createWithDisplacement(cg, cg->getMethodMetaDataRegister(), offsetof(J9VMThread, jitTOC),
            TR::Compiler->om.sizeofReferenceAddress()));
#else
...
void J9::Power::TreeEvaluator::buildArgsProcessFEDependencies(TR::Node *node, TR::CodeGenerator *cg,
    TR::RegisterDependencyConditions *dependencies)
{
    TR::Compilation *comp = cg->comp();
...
TR::Register *J9::Power::TreeEvaluator::retrieveTOCRegister(TR::Node *node, TR::CodeGenerator *cg,
    TR::RegisterDependencyConditions *dependencies)
{
#if defined(DEBUG)
    TR::Compilation *comp = cg->comp();
    ...
    bool isArray)
{
    ...
    if (deltaChild && deltaChild->getOpCode().isLoadConst() && !deltaChild->getRegister()
    ...
    if (!isArray) {
        ...
        fieldOffset = fej9->getInstanceFieldOffset(containingClass, "value", 5, fieldSig, fieldSigLen, true)
        ...
        if (isArray) {
            ...
            TR_OpaqueClassBlock *containingClass
                = fej9->getClassFromSignature(className, classNameLen, owningMethod, true);
            int32_t arrayFieldOffset = fej9->getInstanceFieldOffset(containingClass, "array", 5, fieldSig, fieldSigLen)
                + fej9->getObjectHeaderSizeInBytes(); // size of a J9 object header

            TR::MemoryReference *tempMR
                = TR::MemoryReference::createWithDisplacement(cg, valueReg, arrayFieldOffset, isLong ? 8 : 4);
            ...
            if (TR::Compiler->om.compressObjectReferences())
                ...
                generateTrg1MemInstruction(cg, TR::InstOpCode::Op_load, node, memRefRegister,
            ...
            generateTrg1Src1ImmInstruction(cg, TR::InstOpCode::addi2, node, fieldOffsetReg, fieldOffsetReg,
                fieldOffset);
        }
    }
    ...
    generateMemSrc1Instruction(cg, isLong ? TR::InstOpCode::stdcx_r : TR::InstOpCode::stwcx_r, node,
        TR::MemoryReference::createWithIndexReg(cg, valueReg, fieldOffsetReg, isLong ? 8 : 4), deltaReg);
...
static TR::Register *inlineFPTrg1Src3(TR::Node *node, TR::InstOpCode::Mnemonic op, TR::CodeGenerator *cg)
{
    TR_ASSERT_FATAL(node->getNumChildren() == 3,
        "In function inlineFPTrg1Src3, the node at address %p should have exactly 3 children, but got %u instead", node,
        node->getNumChildren());

    TR::DataType type = node->getDataType();
    TR_ASSERT_FATAL(type == TR::Float || type == TR::Double,
        "In function inlineFPTrg1Src3, the node at address %p should be either TR::Float or TR::Double", node);
...
static TR::Register *inlineAtomicOperation(TR::Node *node, TR::CodeGenerator *cg, TR::MethodSymbol *method)
{
    ...
    if (isUnsafe) {
        ...
        fieldOffset = fej9->getObjectHeaderSizeInBytes()
            + fej9->getInstanceFieldOffset(classBlock, "value", 5, fieldSig, fieldSigLen);
    } else // isArray
    {
        ...
        classBlock = fej9->getClassFromSignature(className, classNameLen, comp->getCurrentMethod(), true);
        int32_t arrayFieldOffset = fej9->getObjectHeaderSizeInBytes()
            + fej9->getInstanceFieldOffset(classBlock, "array", 5, fieldSig, fieldSigLen);
    ...
    if (isCAS) {
        ...
        generateTrg1MemInstruction(cg, size == 8 ? TR::InstOpCode::ldarx : TR::InstOpCode::lwarx, node, currentReg,
            TR::MemoryReference::createWithIndexReg(cg, valueReg,
                (fieldOffsetReg->getRegisterPair() ? fieldOffsetReg->getLowOrder() : fieldOffsetReg), size));

        if (size == 8 && comp->target().is32Bit()) {
            if (!isArgImm && expValReg->getRegisterPair()) {
                generateTrg1Src1Imm2Instruction(cg, TR::InstOpCode::rldimi, node, expValReg->getLowOrder(),
                    expValReg->getHighOrder(), 32, CONSTANT64(0xFFFFFFFF00000000));
                expPair = true;
                numDeps++;
            }
            if (newValReg->getRegisterPair()) {
                if (newValReg != expValReg) {
                    generateTrg1Src1Imm2Instruction(cg, TR::InstOpCode::rldimi, node, newValReg->getLowOrder(),
                        newValReg->getHighOrder(), 32, CONSTANT64(0xFFFFFFFF00000000));
                }
                numDeps++;
                newPair = true;
            }
        ...
        if (isArgImm) {
            generateTrg1Src1ImmInstruction(cg, size == 8 ? TR::InstOpCode::cmpi8 : TR::InstOpCode::cmpi4, node, cndReg,
                currentReg, expValImm);
        } else {
            generateTrg1Src2Instruction(cg, size == 8 ? TR::InstOpCode::cmp8 : TR::InstOpCode::cmp4, node, cndReg,
                currentReg, expComposedReg);
        }
        ...
        generateConditionalBranchInstruction(cg, TR::InstOpCode::bne, PPCOpProp_BranchUnlikely, node, rsvFailLabel,
            cndReg);
        ...
        if (!isWeak && size == 8 && comp->target().is32Bit()) {
            ...
            generateMemSrc1Instruction(cg, size == 8 ? TR::InstOpCode::stdcx_r : TR::InstOpCode::stwcx_r, node,
                ...
                currentReg);
            /* Expect store to be successful, so this branch is usually not taken */
            generateConditionalBranchInstruction(cg, TR::InstOpCode::bne, PPCOpProp_BranchUnlikely, node, startLabel,
                cndReg);
        }
...
static TR::Register *inlineConcurrentLinkedQueueTMOffer(TR::Node *node, TR::CodeGenerator *cg)
{
    ...
    bool doCrdMrk = ((gcMode == gc_modron_wrtbar_cardmark || gcMode == gc_modron_wrtbar_cardmark_and_oldcheck
                         || gcMode == gc_modron_wrtbar_cardmark_incremental)
        && (!node->getOpCode().isWrtBar() || !node->isNonHeapObjectWrtBar()));

    TR_OpaqueClassBlock *classBlock = NULL;
    classBlock = fej9->getClassFromSignature("Ljava/util/concurrent/ConcurrentLinkedQueue$Node;", 49,
        comp->getCurrentMethod(), true);
    int32_t offsetNext = fej9->getObjectHeaderSizeInBytes()
        + fej9->getInstanceFieldOffset(classBlock, "next", 4, "Ljava/util/concurrent/ConcurrentLinkedQueue$Node;", 49);
    classBlock = fej9->getClassFromSignature("Ljava/util/concurrent/ConcurrentLinkedQueue;", 44,
        comp->getCurrentMethod(), true);
    int32_t offsetTail = fej9->getObjectHeaderSizeInBytes()
        + fej9->getInstanceFieldOffset(classBlock, "tail", 4, "Ljava/util/concurrent/ConcurrentLinkedQueue$Node;", 49);
    ...
    // ALG: p := this.tail
    generateTrg1MemInstruction(cg, loadOpCode, node, pReg,
        TR::MemoryReference::createWithDisplacement(cg, objReg, offsetTail, addressFieldSize));
    if (usesCompressedrefs) {
        genDecompressPointerWithTempReg(cg, node, pReg, resultReg, NULL, false); // p is not null
    }

    // ALG: q := p.next
    generateTrg1MemInstruction(cg, loadOpCode, node, qReg,
        TR::MemoryReference::createWithDisplacement(cg, pReg, offsetNext, addressFieldSize));
    ...
    if (usesCompressedrefs) {
        compressedReg = genCompressPointerNonNull2RegsWithTempReg(cg, node, nReg, qReg,
            resultReg); // In Java code checkNotNull() already ensures n != null
    }
    // ALG: p.next := n
    generateMemSrc1Instruction(cg, storeOpCode, node,
        TR::MemoryReference::createWithDisplacement(cg, pReg, offsetNext, addressFieldSize), compressedReg);

    // ALG: this.tail = n
    generateMemSrc1Instruction(cg, storeOpCode, node,
        TR::MemoryReference::createWithDisplacement(cg, objReg, offsetTail, addressFieldSize), compressedReg);
    ...
    if (doWrtBar) {
        ...
        VMnonNullSrcWrtBarCardCheckEvaluator(node, nReg, pReg, cndReg, qReg, retryCountReg, temp3Reg, temp4Reg,
            wrtbar1Donelabel, conditions, false, cg, false);
        ...
        VMnonNullSrcWrtBarCardCheckEvaluator(node, nReg, pReg, cndReg, qReg, retryCountReg, temp3Reg, temp4Reg,
            returnLabel, conditions, false, cg, false);
...
static TR::Register *inlineConcurrentLinkedQueueTMPoll(TR::Node *node, TR::CodeGenerator *cg)
{
    ...
    TR_OpaqueClassBlock *classBlock = fej9->getClassFromSignature("Ljava/util/concurrent/ConcurrentLinkedQueue;", 44,
        comp->getCurrentMethod(), true);
    int32_t offsetHead = fej9->getObjectHeaderSizeInBytes()
        + fej9->getInstanceFieldOffset(classBlock, "head", 4, "Ljava/util/concurrent/ConcurrentLinkedQueue$Node;", 49);
    classBlock = fej9->getClassFromSignature("Ljava/util/concurrent/ConcurrentLinkedQueue$Node;", 49,
        comp->getCurrentMethod(), true);
    int32_t offsetNext = fej9->getObjectHeaderSizeInBytes()
        + fej9->getInstanceFieldOffset(classBlock, "next", 4, "Ljava/util/concurrent/ConcurrentLinkedQueue$Node;", 49);
    int32_t offsetItem = fej9->getObjectHeaderSizeInBytes()
        + fej9->getInstanceFieldOffset(classBlock, "item", 4, "Ljava/lang/Object;", 18);
    ...
    // ALG: p := this.head
    generateTrg1MemInstruction(cg, loadOpCode, node, temp3Reg,
        TR::MemoryReference::createWithDisplacement(cg, objReg, offsetHead, addressFieldSize));
    ...
    // ALG: res := p.item
    generateTrg1MemInstruction(cg, loadOpCode, node, resultReg,
        TR::MemoryReference::createWithDisplacement(cg, decompressedP, offsetItem, addressFieldSize));
    if (usesCompressedrefs) {
    ...
    // ALG: q := p.next
    generateTrg1MemInstruction(cg, loadOpCode, node, qReg,
        TR::MemoryReference::createWithDisplacement(cg, decompressedP, offsetNext, addressFieldSize));

    // ALG: p.item := null
    generateTrg1ImmInstruction(cg, TR::InstOpCode::li, node, nullReg, 0);

    generateMemSrc1Instruction(cg, storeOpCode, node,
        TR::MemoryReference::createWithDisplacement(cg, decompressedP, offsetItem, addressFieldSize), nullReg);
    ...
    generateMemSrc1Instruction(cg, storeOpCode, node,
        TR::MemoryReference::createWithDisplacement(cg, objReg, offsetHead, addressFieldSize), qReg);
    ...
    // ALG: p.next := p
    generateMemSrc1Instruction(cg, storeOpCode, node,
        TR::MemoryReference::createWithDisplacement(cg, decompressedP, offsetNext, addressFieldSize), temp3Reg);
    ...
    if (doWrtBar) {
        ...
        VMnonNullSrcWrtBarCardCheckEvaluator(node, qReg, objReg, cndReg, nullReg, pReg, temp3Reg, temp4Reg, returnLabel,
            conditions, false, cg, false);
    } else if (doCrdMrk) {
...
static TR::Register *dangerousGetCPU(TR::Node *node, TR::CodeGenerator *cg)
{
    ...
    generateMemSrc1Instruction(cg, TR::InstOpCode::Op_st, node,
        TR::MemoryReference::createWithDisplacement(cg, gr1, 0x14, TR::Compiler->om.sizeofReferenceAddress()), gr2);
    intptr_t xx[3];
    ...
    generateTrg1Src1ImmInstruction(cg, TR::InstOpCode::addi, node, gr1, gr1, 256);
    generateDepLabelInstruction(cg, TR::InstOpCode::label, node, TR::LabelSymbol::create(cg->trHeapMemory(), cg),
        dependencies);
...
#else
static TR::Register *dangerousGetCPU(TR::Node *node, TR::CodeGenerator *cg) { return NULL; }
#endif
...
static TR::Register *inlineIsAssignableFrom(TR::Node *node, TR::CodeGenerator *cg)
{
    ...
    generateTrg1MemInstruction(cg, TR::InstOpCode::Op_load, node, receiverJ9ClassReg,
        TR::MemoryReference::createWithDisplacement(cg, receiverReg, fej9->getOffsetOfClassFromJavaLangClassField(),
            TR::Compiler->om.sizeofReferenceAddress()));
    generateTrg1MemInstruction(cg, TR::InstOpCode::Op_load, node, parmJ9ClassReg,
        TR::MemoryReference::createWithDisplacement(cg, parmReg, fej9->getOffsetOfClassFromJavaLangClassField(),
            TR::Compiler->om.sizeofReferenceAddress()));
    ...
    if (receiverClassDepth >= 0) {
        ...
        genTestIsSuper(node, parmJ9ClassReg, receiverJ9ClassReg, condReg, scratch1Reg, scratch2Reg, receiverClassDepth,
            outlinedCallLabel, NULL, cg);
    ...
    TR_PPCOutOfLineCodeSection *outlinedHelperCall = new (cg->trHeapMemory())
        TR_PPCOutOfLineCodeSection(node, TR::icall, resultReg, outlinedCallLabel, doneLabel, cg);
    cg->getPPCOutOfLineCodeSectionList().push_front(outlinedHelperCall);
...
static TR::Register *hashCodeHelper(TR::Node *node, TR::CodeGenerator *cg, TR::DataType elementType, TR::Node *hashNode,
    bool isSigned)
{
    TR::Compilation *comp = cg->comp();
    TR_J9VMBase *fej9 = (TR_J9VMBase *)(comp->fe());
...
#if defined(J9VM_GC_SPARSE_HEAP_ALLOCATION)
    if (TR::Compiler->om.isOffHeapAllocationEnabled()) {
        generateTrg1MemInstruction(cg, TR::InstOpCode::Op_load, node, valueReg,
            TR::MemoryReference::createWithDisplacement(cg, valueReg,
                TR::Compiler->om.offsetOfContiguousDataAddrField(), comp->target().is64Bit() ? 8 : 4));
    } else
#endif /* J9VM_GC_SPARSE_HEAP_ALLOCATION */
    {
    ...
    switch (elementType) {
        ...
        case TR::Int16:
            generateTrg1Src1Imm2Instruction(cg,
                comp->target().is64Bit() ? TR::InstOpCode::rldic : TR::InstOpCode::rlwinm, node, endReg, endReg, 1,
                comp->target().is64Bit() ? CONSTANT64(0x00000001FFFFFFFE) : 0xFFFFFFFE);
            generateTrg1Src1Imm2Instruction(cg,
                comp->target().is64Bit() ? TR::InstOpCode::rldic : TR::InstOpCode::rlwinm, node, tempReg, tempReg, 1,
                comp->target().is64Bit() ? CONSTANT64(0x00000001FFFFFFFE) : 0xFFFFFFFE);
            break;
        case TR::Int32:
            generateTrg1Src1Imm2Instruction(cg,
                comp->target().is64Bit() ? TR::InstOpCode::rldic : TR::InstOpCode::rlwinm, node, endReg, endReg, 2,
                comp->target().is64Bit() ? CONSTANT64(0x00000003FFFFFFFC) : 0xFFFFFFFC);
            generateTrg1Src1Imm2Instruction(cg,
                comp->target().is64Bit() ? TR::InstOpCode::rldic : TR::InstOpCode::rlwinm, node, tempReg, tempReg, 2,
                comp->target().is64Bit() ? CONSTANT64(0x00000003FFFFFFFC) : 0xFFFFFFFC);
            break;
        default:
            TR_ASSERT_FATAL(false, "Unsupported hashCodeHelper elementType");
    ...
    generateTrg1Src1Imm2Instruction(cg, TR::InstOpCode::rlwinm, node, multiplierAddrReg, tempReg, 5,
        0xFFFFFFFFFFFFFFE0);
    ...
    static uint32_t multiplierVectors8[20]
        = { 0x50A9DE01, 0x50A9DE01, 0x50A9DE01, 0x50A9DE01, 0xE191DDDF, 0x59DB6A41, 0xE1DDC99F, 0xEE830681, 0x07B1A55F,
              0x94E4B2C1, 0xF449711F, 0x94446F01, 0x67E12CDF, 887503681, 28629151, 923521, 29791, 961, 31, 1 };
    static uint32_t multiplierVectors16[12] = { 0x94446F01, 0x94446F01, 0x94446F01, 0x94446F01, 0x67E12CDF, 887503681,
        28629151, 923521, 29791, 961, 31, 1 };
    static uint32_t multiplierVectors32[8] = { 923521, 923521, 923521, 923521, 29791, 961, 31, 1 };
    ...
    if (elementType != TR::Int32 && !isSigned) {
        generateTrg1Src2ImmInstruction(cg, TR::InstOpCode::vsldoi, node, vunpackMaskReg, vconstant0Reg, vtmp1Reg,
            elementSize);
    ...
    generateTrg1Src1ImmInstruction(cg, TR::InstOpCode::addi, node, tempReg, endReg, elementSize - 16);
    ...
    // End of this method
    TR::RegisterDependencyConditions *dependencies = new (cg->trHeapMemory()) TR::RegisterDependencyConditions(0,
...
static TR::Register *inlineEncodeUTF16(TR::Node *node, TR::CodeGenerator *cg)
{
    ...
    TR::RegisterDependencyConditions *deps
        = new (cg->trHeapMemory()) TR::RegisterDependencyConditions(1, totalDeps, cg->trMemory());
    ...
    TR::Register *gprClobbers[gprClobberCount], *fprClobbers[fprClobberCount], *vrClobbers[vrClobberCount],
        *crClobbers[crClobberCount];
    for (int i = 0; i < gprClobberCount; ++i)
        gprClobbers[i] = cg->allocateRegister(TR_GPR);
    for (int i = 0; i < fprClobberCount; ++i)
        fprClobbers[i] = cg->allocateRegister(TR_FPR);
    for (int i = 0; i < vrClobberCount; ++i)
        vrClobbers[i] = cg->allocateRegister(TR_VRF);
    for (int i = 0; i < crClobberCount; ++i)
        crClobbers[i] = cg->allocateRegister(TR_CCR);
    ...
    if (inputLenReg != node->getChild(2)->getRegister())
        cg->stopUsingRegister(inputLenReg);
    for (int i = 0; i < gprClobberCount; ++i)
        cg->stopUsingRegister(gprClobbers[i]);
    for (int i = 0; i < vrClobberCount; ++i)
        cg->stopUsingRegister(vrClobbers[i]);
    for (int i = 0; i < fprClobberCount; ++i)
        cg->stopUsingRegister(fprClobbers[i]);
    for (int i = 0; i < crClobberCount; ++i)
        cg->stopUsingRegister(crClobbers[i]);
...
static TR::Register *inlineIntrinsicIndexOf_P10(TR::Node *node, TR::CodeGenerator *cg, bool isLatin1)
{
...
#if defined(J9VM_GC_SPARSE_HEAP_ALLOCATION)
    if (TR::Compiler->om.isOffHeapAllocationEnabled()) {
        generateTrg1MemInstruction(cg, TR::InstOpCode::ld, node, arrAddress,
            TR::MemoryReference::createWithDisplacement(cg, array, TR::Compiler->om.offsetOfContiguousDataAddrField(),
                8));
    } else
#endif /* J9VM_GC_SPARSE_HEAP_ALLOCATION */
    {
    ...
    // match first byte
    generateTrg1MemInstruction(cg, scalarLoadOp, node, temp,
        TR::MemoryReference::createWithIndexReg(cg, position, arrAddress, isLatin1 ? 1 : 2));
...
static TR::Register *inlineIntrinsicIndexOf(TR::Node *node, TR::CodeGenerator *cg, bool isLatin1)
{
    ...
    generateTrg1Src1Imm2Instruction(cg, TR::InstOpCode::rldicr, node, endVectorAddress, endAddress, 0,
        CONSTANT64(0xfffffffffffffff0));
    ...
    generateTrg1Src1Imm2Instruction(cg, TR::InstOpCode::rldicr, node, startVectorAddress, currentAddress, 0,
        CONSTANT64(0xfffffffffffffff0));
    ...
    // the string.
    generateTrg1MemInstruction(cg, TR::InstOpCode::lvx, node, searchVector,
        TR::MemoryReference::createWithIndexReg(cg, zeroRegister, currentAddress, 16));

    {
        ...
        generateTrg1Src2Instruction(cg,
            comp->target().cpu.isLittleEndian() ? TR::InstOpCode::lvsl : TR::InstOpCode::lvsr, node, permuteVector,
            zeroRegister, scratchRegister);
        generateTrg1Src3Instruction(cg, TR::InstOpCode::vperm, node, searchVector, searchVector, searchVector,
            permuteVector);
        ...
        if (comp->target().cpu.isLittleEndian()) {
            ...
            generateTrg1Src3Instruction(cg, TR::InstOpCode::vperm, node, searchVector, targetVectorNot, searchVector,
                permuteVector);
        } else {
            generateTrg1Src2Instruction(cg, TR::InstOpCode::lvsl, node, permuteVector, zeroRegister, scratchRegister);
            generateTrg1Src3Instruction(cg, TR::InstOpCode::vperm, node, searchVector, searchVector, targetVectorNot,
                permuteVector);
        }
...
/**
 * This function generates a sequence of instructions to perform an array compare specialized for the String indexOf
 intrinsic.
 * The specialization means it skips checks that are expected to have already been done earlier in the String indexOf
 code.
 *
 * @param node                   TR::Node associated with the code being generated.
 * @param cg                     Code Generator.
 * @param currentAddressReg      Register that holds an input value for the code being generated. Value in the register
 is not modified. This is the address of the first character in the section of the main string being compared to see if
 it matches the full substring.
 * @param subStringAddressReg    Register that holds an input value for the code being generated. Value in the register
 is not modified. This is the address of the first character of the substring.
 * @param subStringLenReg        Register that holds an input value for the code being generated. Value in the register
 is not modified. This is the length of the substring in bytes.
 * @param tempReg                Register to hold temporary GPR values so it gets clobbered. Starting value is not read.
 * @param temp2Reg               Register to hold temporary GPR values so it gets clobbered. Starting value is not read.
 * @param subSearchOffsetReg     Register to hold the offset into the substring. This is used to keep track of how far
 the substring match has progresses. The register gets clobbered and its starting value is not read.
 * @param vec1Reg                Register to hold temporary vector values so it gets clobbered. Starting value is not
 read.
 * @param vec2Reg                Register to hold temporary vector values so it gets clobbered. Starting value is not
 read.
 * @param cr0Reg                 One of the condition registers used.
 * @param cr6Reg                 One of the condition registers used.
 * @param subStringMatchLabel    If a full match is found on the substring, the code will jump to this label.
 * @param subStringMismatchLabel If a mismatch between the substring and main string is found, jump to this label. If
 NULL, jump to the end of the code segment generated by performArrayCmp instead.
 */
static void performArrayCmp(TR::Node *node, TR::CodeGenerator *cg, TR::Register *currentAddressReg,
    TR::Register *subStringAddressReg, TR::Register *subStringLenReg, TR::Register *tempReg, TR::Register *temp2Reg,
    TR::Register *subSearchOffsetReg, TR::Register *vec1Reg, TR::Register *vec2Reg, TR::Register *cr0Reg,
    TR::Register *cr6Reg, TR::LabelSymbol *subStringMatchLabel, TR::LabelSymbol *subStringMismatchLabel)
{
    /*
     * This is used by inlineIntrinsicIndexOfString to check if the remainder for a substring matches the next
     * characters in the main string. This process is very similar to an arraycmp but this version has special
     * optimizations for the inlineIntrinsicIndexOfString case. The most notable one is this arraycmp may read one fewer
     * byte of data to avoid smaller loads and to avoid byte by byte processsing. This works since this assembly is only
     * run after it is confirmed that the first character of the substring matches a character in the main string. This
     * occurs whenever the substring has an odd number of bytes.
     *
     * | Substring Length | 2 byte load | 4 byte load | 8 byte load | 16 byte load |
     * | ---------------- | ----------- | ----------- | ----------- | ------------ |
     * |                1 |           0 |           0 |           0 |            0 | The one character in the substring
     * is a known match so no further loads are needed. |                2 |           1 |             |             | |
     * |                3 |           1 |             |             |              | Recomparing the first byte isn't
     * needed so a single 2 byte load handles the rest of the substring. |                4 |             |           1
     * |             |              | |                5 |             |           1 |             |              | | 6
     * |           1 |           1 |             |              | |                7 |           1 |           1 | | |
     * |                8 |             |             |           1 |              |
     * |                9 |             |             |           1 |              |
    ...
    /*
     * subStringMismatchLabel is the location to jump to if the substring does not match the next characters in the main
     * string. This label can be provided by the caller of performArrayCmp if there is a specific location to jump to.
     ...
     */
    generateTrg1Src1Imm2Instruction(cg, TR::InstOpCode::rldicr, node, tempReg, subStringLenReg, 0,
        CONSTANT64(0xfffffffffffffffe));
     ...
     * So it doesn't matter if the byte order gets scrambled as long as it is scrambled the same way for both loaded
     * values.
     */
    ...
    /* If a difference is found in the 16 loaded bytes, jump to subStringMismatchLabel to indicate that the substring
     * was not found. */
    ...
    /* If there are no more bytes, the arrays match each other. Jump to subStringMatchLabel to handle the substring
     * match. */
    ...
    /* Use 8 byte loads to search for a mismatch between the two arrays. No loop is needed since there are less than 16
     * bytes left. */
    ...
    /* A fallthrough means the loaded values did not match. Continue on to handle the case where the substring was not
     * found. */
...
static TR::Register *inlineIntrinsicIndexOfString(TR::Node *node, TR::CodeGenerator *cg, bool isLatin1)
{
    ...
    if (disableStringIndexOfStringIntrinsic)
        return nullptr;

    /*
     * This intrinsic searches for a string inside of a string.
     * For clarity, the string being searched for is called the substring.
     * The string being search in is called the main string.
     */

    // TODO: Support for StringUTF16.indexOf is a WIP and currently unused. (isLatin1 is always true)

    TR::Compilation *comp = cg->comp();
    auto vectorCompareOp = isLatin1 ? TR::InstOpCode::vcmpequb_r : TR::InstOpCode::vcmpequh_r;
    auto scalarLoadOp = isLatin1 ? TR::InstOpCode::lbzx : TR::InstOpCode::lhzx;

    TR::Register *arrayObjReg = cg->evaluate(node->getChild(0));
    TR::Register *arrayLenReg = cg->evaluate(node->getChild(1));
    TR::Register *subStringObjReg = cg->evaluate(node->getChild(2));
    ...
    TR::Register *tempReg = cg->allocateRegister();
    TR::Register *temp2Reg = cg->allocateRegister();
    TR::Register *subSearchOffsetReg = cg->allocateRegister();

    TR::Register *vec1Reg = cg->allocateRegister(TR_VRF);
    TR::Register *vec2Reg = cg->allocateRegister(TR_VRF);
    TR::Register *targetVectorReg = cg->allocateRegister(TR_VRF);
    TR::Register *targetVectorNotReg = cg->allocateRegister(TR_VRF);
    ...
    TR::LabelSymbol *foundEndVectorRetryLabel = generateLabelSymbol(cg);
    TR::LabelSymbol *notSmallLabel = generateLabelSymbol(cg);
...

    {
         ...
         * it can't be found. In this case jump to notFoundLabel to return -1. */
    ...
    if (!isLatin1) {
        generateTrg1Src2Instruction(cg, TR::InstOpCode::add, node, resultReg, resultReg, resultReg);
        generateTrg1Src2Instruction(cg, TR::InstOpCode::add, node, subStringLenReg, subStringLenReg, subStringLenReg);
    }
...
#if defined(J9VM_GC_SPARSE_HEAP_ALLOCATION)
    if (TR::Compiler->om.isOffHeapAllocationEnabled()) {
        generateTrg1MemInstruction(cg, TR::InstOpCode::ld, node, arrayAddressReg,
            TR::MemoryReference::createWithDisplacement(cg, arrayObjReg,
                TR::Compiler->om.offsetOfContiguousDataAddrField(), 8));
        generateTrg1MemInstruction(cg, TR::InstOpCode::ld, node, subStringAddressReg,
            TR::MemoryReference::createWithDisplacement(cg, subStringObjReg,
                TR::Compiler->om.offsetOfContiguousDataAddrField(), 8));
    } else
#endif /* J9VM_GC_SPARSE_HEAP_ALLOCATION */
    {
        generateTrg1Src1ImmInstruction(cg, TR::InstOpCode::addi, node, arrayAddressReg, arrayObjReg,
            TR::Compiler->om.contiguousArrayHeaderSizeInBytes());
        generateTrg1Src1ImmInstruction(cg, TR::InstOpCode::addi, node, subStringAddressReg, subStringObjReg,
            TR::Compiler->om.contiguousArrayHeaderSizeInBytes());
    }

    /* resultReg currently holds the starting offset. currentAddressReg will hold the address of the first byte of the
     * main string to look at. */
     ...
     * The first character of the substring is compared against the first (after adjusted for offset) character of the
     * main string. If it matches, jump to foundEarlyLabel to try and match the rest of the substring. If it doesn't
     * match, jump back to notFoundEarlyLabel.
     */
    generateTrg1MemInstruction(cg, isLatin1 ? TR::InstOpCode::lbz : TR::InstOpCode::lhz, node, tempReg,
        TR::MemoryReference::createWithDisplacement(cg, currentAddressReg, 0, isLatin1 ? 1 : 2));
    generateTrg1MemInstruction(cg, isLatin1 ? TR::InstOpCode::lbz : TR::InstOpCode::lhz, node, zeroReg,
        TR::MemoryReference::createWithDisplacement(cg, subStringAddressReg, 0, isLatin1 ? 1 : 2));
    ...
    /* Under UTF16, the indices are doubled since offset and size in bytes are needed later. */
    if (!isLatin1) {
        generateTrg1Src2Instruction(cg, TR::InstOpCode::add, node, endAddressReg, endAddressReg, endAddressReg);
    }

    /*
     * It sometimes isn't necessary to match the first character of the substring against the last characters in the
     * main string because there aren't enough characters left in the main string to hold the rest of the substring. A
     * check for this case is performed here and goes to notFoundLabel to return -1 if there aren't enough characters
     * left.
     */
    generateTrg1Src2Instruction(cg, TR::InstOpCode::subf, node, endAddressReg, subStringLenReg, endAddressReg);
    generateTrg1Src2Instruction(cg, TR::InstOpCode::cmp8, node, cr0Reg, resultReg, endAddressReg);
    generateConditionalBranchInstruction(cg, TR::InstOpCode::bge, node, notFoundLabel, cr0Reg);

    /*
     * Calculate the actual addresses of the start and end points.
     * currentAddressReg is the address of the next character to load in the main string to compare against the
     * substring. endAddressReg is the address of the first character in the main string that does NOT need to be
     * compared against the first character of the substring. The reason for this is it is known there can not be a
     * match at that point because there aren't enough characters left in the main string.
     *
     ...
     *          abcd
     *
     * endAddressReg will contain the address of the character "7". This is the point where a match for "abcd" is
     * impossible since there are 3 characters left and the substring is 4 characters long. When endAddressReg is
     * reached, the search ends and -1 is returned.
     */
    generateTrg1Src1ImmInstruction(cg, TR::InstOpCode::addi, node, endAddressReg, endAddressReg, isLatin1 ? 1 : 2);
    generateTrg1Src1ImmInstruction(cg, TR::InstOpCode::addi, node, currentAddressReg, currentAddressReg,
        isLatin1 ? 1 : 2);
     ...
     * zeroReg currently holds the value of the first character of the substring.
     * targetVectorReg is a vector register with the first character of the substring splat across it. This is used to
     * quickly search for first character matches later. zeroReg is then set to 0.
     */
     ...
     * maskVectorReg is set to 0xFF for Latin1 and 0xFFFF for UTF16. This is later used to mask values in a vector
     * register to prevent matching a second time.
     */
    {
        generateTrg1ImmInstruction(cg, TR::InstOpCode::li, node, tempReg, isLatin1 ? 1 : 2);
        generateTrg1Src2Instruction(cg, TR::InstOpCode::vxor, node, targetVectorNotReg, targetVectorReg,
            targetVectorReg);
        generateTrg1Src2Instruction(cg, TR::InstOpCode::vnor, node, vec1Reg, targetVectorNotReg, targetVectorNotReg);

        if (comp->target().cpu.isLittleEndian()) {
            generateTrg1Src2Instruction(cg, TR::InstOpCode::lvsl, node, permuteVectorReg, zeroReg, tempReg);
            generateTrg1Src3Instruction(cg, TR::InstOpCode::vperm, node, maskVectorReg, targetVectorNotReg, vec1Reg,
                permuteVectorReg);
        } else {
            generateTrg1Src2Instruction(cg, TR::InstOpCode::lvsr, node, permuteVectorReg, zeroReg, tempReg);
            generateTrg1Src3Instruction(cg, TR::InstOpCode::vperm, node, maskVectorReg, vec1Reg, targetVectorNotReg,
                permuteVectorReg);
        }
    }

    /* targetVectorNotReg is set to the bitwise complement of targetVectorReg. These are values that will never match
     ...
     * A vector load from tempReg will contain 1-16 bytes that need to be compared against the first character of the
     * substring. A vector load from temp2Reg will contain 0-15 bytes that need to be compared against the first
     * character of the substring.
     */
    generateTrg1Src1Imm2Instruction(cg, TR::InstOpCode::rldicr, node, tempReg, currentAddressReg, 0,
        CONSTANT64(0xfffffffffffffff0));
    generateTrg1Src1Imm2Instruction(cg, TR::InstOpCode::rldicr, node, temp2Reg, endAddressReg, 0,
        CONSTANT64(0xfffffffffffffff0));

    /*
     * Check if the entire string is contained within a single 16-byte aligned section. If this
     * happens, we need to handle that case specially.
     * If the value in tempReg is equal to temp2Reg, then there are 1-15 bytes that need to be looked at and they are
     * all within a 16 byte aligned section.
     */
    generateTrg1Src2Instruction(cg, TR::InstOpCode::cmp8, node, cr0Reg, tempReg, temp2Reg);
    generateConditionalBranchInstruction(cg, TR::InstOpCode::bne, node, notSmallLabel, cr0Reg);
     ...
     *   mm mm mm 09 08 07 06 05 04 03 02 01 nn nn nn nn
     *
     * Big Endian example:
     *   nn nn nn nn 01 02 03 04 05 06 07 08 09 mm mm mm
     */
    generateTrg1MemInstruction(cg, TR::InstOpCode::lvx, node, searchVectorReg,
        TR::MemoryReference::createWithIndexReg(cg, zeroReg, currentAddressReg, 16));
     ...
     *
     * Little Endian example:
     *   mm mm mm 09 08 07 06 05 04 03 02 01 nn nn nn nn -> 09 08 07 06 05 04 03 02 01 nn nn nn nn mm mm mm
     *
     * Big Endian example:
     *   nn nn nn nn 01 02 03 04 05 06 07 08 09 mm mm mm -> mm mm mm nn nn nn nn 01 02 03 04 05 06 07 08 09
     */
    generateTrg1Src1Instruction(cg, TR::InstOpCode::neg, node, tempReg,
        endAddressReg); // Only the low 4 bits in tempReg are used so it acts as a value between 0-15.
    generateTrg1Src2Instruction(cg, comp->target().cpu.isLittleEndian() ? TR::InstOpCode::lvsl : TR::InstOpCode::lvsr,
        node, permuteVectorReg, zeroReg, tempReg);
    generateTrg1Src3Instruction(cg, TR::InstOpCode::vperm, node, searchVectorReg, searchVectorReg, searchVectorReg,
        permuteVectorReg);
     ...
     *
     * Little Endian example:
     *   09 08 07 06 05 04 03 02 01 nn nn nn nn mm mm mm -> XX XX XX XX XX XX XX 09 08 07 06 05 04 03 02 01
     *
     * Big Endian example:
     *   mm mm mm nn nn nn nn 01 02 03 04 05 06 07 08 09 -> 01 02 03 04 05 06 07 08 09 XX XX XX XX XX XX XX
     */
    generateTrg1Src2Instruction(cg, TR::InstOpCode::add, node, tempReg, tempReg, currentAddressReg);
    if (comp->target().cpu.isLittleEndian()) {
        generateTrg1Src2Instruction(cg, TR::InstOpCode::lvsr, node, permuteVectorReg, zeroReg, tempReg);
        generateTrg1Src3Instruction(cg, TR::InstOpCode::vperm, node, searchVectorReg, targetVectorNotReg,
            searchVectorReg, permuteVectorReg);
    } else {
        generateTrg1Src2Instruction(cg, TR::InstOpCode::lvsl, node, permuteVectorReg, zeroReg, tempReg);
        generateTrg1Src3Instruction(cg, TR::InstOpCode::vperm, node, searchVectorReg, searchVectorReg,
            targetVectorNotReg, permuteVectorReg);
    }

    /*
     * Now the search vector is ready for comparison: none of the garbage can compare as equal to
     * our target value and the start of the vector is now aligned to the start of the string. So
     * we can now perform the comparison as normal. If it isn't found, jump to notFoundLabel to return -1.
     */
    generateTrg1Src2Instruction(cg, vectorCompareOp, node, vec2Reg, searchVectorReg, targetVectorReg);
    generateConditionalBranchInstruction(cg, TR::InstOpCode::beq, node, notFoundLabel, cr6Reg);

    generateLabelInstruction(cg, TR::InstOpCode::label, node, foundEndVectorLabel);
    ...
    generateLabelInstruction(cg, TR::InstOpCode::label, node, foundEndVectorRetryLabel);
     ...
     * we're interested in is in the least significant half-word.
     */
    generateTrg1Src1Instruction(cg, TR::InstOpCode::cntlzw, node, resultReg, resultReg);
    generateTrg1Src1ImmInstruction(cg, TR::InstOpCode::addi, node, resultReg, resultReg, -16);

    /*
     * Finally, combine this with the address of the last vector load to find the address of the
     * first matching element in the string.
     */
    generateTrg1Src2Instruction(cg, TR::InstOpCode::add, node, currentAddressReg, resultReg, currentAddressReg);

    /*
     * The character at currentAddressReg is now known to match the first character of the substring.
     * Now code to perform an arraycmp is inserted here to verify that the next characters match the rest of the
     * substring as well. On a full match, it jumps to foundLabel to calculate the final result to return. If it isn't a
     * full match, the code path continues on here.
     */
    performArrayCmp(node, cg, currentAddressReg, subStringAddressReg, subStringLenReg, tempReg, temp2Reg,
        subSearchOffsetReg, vec1Reg, vec2Reg, cr0Reg, cr6Reg, foundLabel, nullptr);
    ...
    generateTrg1Src2Instruction(cg, comp->target().cpu.isLittleEndian() ? TR::InstOpCode::lvsl : TR::InstOpCode::lvsr,
        node, vec2Reg, zeroReg, resultReg);
    generateTrg1Src3Instruction(cg, TR::InstOpCode::vperm, node, vec1Reg, maskVectorReg, maskVectorReg, vec2Reg);
    generateTrg1Src3Instruction(cg, TR::InstOpCode::vsel, node, searchVectorReg, searchVectorReg, targetVectorNotReg,
        vec1Reg);

    /*
     * This checks for a second match. If one isn't found, jump to notFoundLabel to return -1.
     * If a second match is found, jump back to foundEndVectorRetryLabel to try the process again.
     * The value in permuteVectorReg has not changed since it was generated so it doesn't need to be generated again.
     */
    generateTrg1Src2Instruction(cg, vectorCompareOp, node, vec2Reg, searchVectorReg, targetVectorReg);
    generateConditionalBranchInstruction(cg, TR::InstOpCode::beq, node, notFoundLabel, cr6Reg);
    generateLabelInstruction(cg, TR::InstOpCode::b, node, foundEndVectorRetryLabel);
     ...
     * Check if we already have 16-byte alignment. Vector loads require 16-byte alignment, so if we
     * aren't properly aligned, we'll need to handle comparisons specially until we achieve 16-byte
     * alignment. At this point, tempReg is equal to currentAddressReg with the lowest 4 bits clear.
     */
    generateTrg1Src2Instruction(cg, TR::InstOpCode::cmp8, node, cr0Reg, currentAddressReg, tempReg);
     ...
     */
    if (comp->target().cpu.isLittleEndian()) {
        generateTrg1Src2Instruction(cg, TR::InstOpCode::lvsr, node, permuteVectorReg, zeroReg,
            currentAddressReg); // Only the low 4 bits in currentAddressReg are used so it acts as a value between 0-15.
        generateTrg1Src3Instruction(cg, TR::InstOpCode::vperm, node, searchVectorReg, targetVectorNotReg,
            searchVectorReg, permuteVectorReg);
    } else {
        generateTrg1Src2Instruction(cg, TR::InstOpCode::lvsl, node, permuteVectorReg, zeroReg,
            currentAddressReg); // Only the low 4 bits in currentAddressReg are used so it acts as a value between 0-15.
        generateTrg1Src3Instruction(cg, TR::InstOpCode::vperm, node, searchVectorReg, searchVectorReg,
            targetVectorNotReg, permuteVectorReg);
    }
     ...
     * Now our vector is ready for comparison: no garbage can match the target value and the start of
     * the vector is now aligned to the start of the string.
     * The value loaded into searchVectorReg is compared with targetVectorReg to see if any of the characters matches
     * the first character of the substring. If so, this jumps to foundFirstVectorLabel to check the rest of the
     * substring. If it doesn't match, it falls through to firstContinueLabel. If this branch is taken but the rest of
     * the substring does not match, it will jump back to firstContinueLabel.
     */
    generateTrg1Src2Instruction(cg, vectorCompareOp, node, vec2Reg, searchVectorReg, targetVectorReg);
    generateConditionalBranchInstruction(cg, TR::InstOpCode::bne, node, foundFirstVectorLabel, cr6Reg);

    generateLabelInstruction(cg, TR::InstOpCode::label, node, firstContinueLabel);

    generateTrg1Src1Imm2Instruction(cg, TR::InstOpCode::rldicr, node, currentAddressReg, currentAddressReg, 0,
        CONSTANT64(0xfffffffffffffff0));
    /* If the first vector didn't match, then we can slide right into the standard vectorized loop. */
    ...
    /*
     * The value loaded into searchVectorReg is again compared with targetVectorReg to see if any of the characters
     * matches the first character of the substring. If so, this jumps to foundVectorLabel to check the rest of the
     * substring. If it doesn't match, it falls through to vectorContinueLabel. If this branch is taken but the rest of
     * the substring does not match, it will jump back to vectorContinueLabel.
     */
    generateTrg1Src2Instruction(cg, vectorCompareOp, node, vec2Reg, searchVectorReg, targetVectorReg);
    ...
    /* Now we're done with the part of the loop which can be handled as a normal vectorized loop. From here we handle
     * the residue. */
    generateLabelInstruction(cg, TR::InstOpCode::label, node, residueLabel);
     ...
     */
    generateTrg1Src1Instruction(cg, TR::InstOpCode::neg, node, tempReg,
        endAddressReg); // Only the low 4 bits in endAddressReg are used so it acts as a value between 0-15.
    if (comp->target().cpu.isLittleEndian()) {
        generateTrg1Src2Instruction(cg, TR::InstOpCode::lvsl, node, permuteVectorReg, zeroReg, tempReg);
        generateTrg1Src3Instruction(cg, TR::InstOpCode::vperm, node, searchVectorReg, searchVectorReg, searchVectorReg,
            permuteVectorReg);
     ...
     * If this branch is taken but the rest of the substring does not match, it will jump back to notFoundLabel to
     * return -1.
     */
    ...
    /* We've looked through the entire string and didn't find the substring, so return the sentinel value -1. */
    generateTrg1ImmInstruction(cg, TR::InstOpCode::li, node, resultReg, -1);
    generateLabelInstruction(cg, TR::InstOpCode::b, node, endLabel);

    /*
     * This section handles the case where the first character of the substring matches the first character of the main
     * string (adjusted for offset). performArrayCmp generates arraycmp code to handle checking the rest of the
     * substring. On a match, jump to foundExactLabel since returnReg already has the offset to return. If it doesn't
     * match, jump to notFoundEarlyLabel to continue searching the rest of the main string. The end of the assembly
     * generated by performArrayCmp is a fall through so a branch to notFoundEarlyLabel is also needed here.
     */
    generateLabelInstruction(cg, TR::InstOpCode::label, node, foundEarlyLabel);
    performArrayCmp(node, cg, currentAddressReg, subStringAddressReg, subStringLenReg, tempReg, temp2Reg,
        subSearchOffsetReg, vec1Reg, vec2Reg, cr0Reg, cr6Reg, foundExactLabel, notFoundEarlyLabel);
    generateLabelInstruction(cg, TR::InstOpCode::b, node, notFoundEarlyLabel);

    /*
     * This section handles the case where the characters to search for in the main string do not fit in a single vector
     * load (not small case). More specifically, this is during the search before the vector loop.
     */
    ...
    /*
     * This checks for a second match. If one isn't found, jump to firstContinueLabel to keep searching.
     * If a second match is found, jump back to foundFirstVectorRetryLabel to try the process again.
     * The value in permuteVectorReg has not changed since it was generated so it doesn't need to be generated again.
     */
    generateTrg1Src2Instruction(cg, vectorCompareOp, node, vec2Reg, searchVectorReg, targetVectorReg);
    ...
    /*
     * This section handles the case where the characters to search for in the main string do not fit in a single vector
     * load (not small case). More specifically, this is during the vector loop.
     */
    ...
    /*
     * This checks for a second match. If one isn't found, jump to vectorContinueLabel to keep searching.
     * If a second match is found, jump back to foundVectorRetryLabel to try the process again.
     * The value in permuteVectorReg has not changed since it was generated so it doesn't need to be generated again.
     */
    generateTrg1Src2Instruction(cg, vectorCompareOp, node, vec2Reg, searchVectorReg, targetVectorReg);
    ...
    /* A full match of the substring inside the main string has been found. The index is calculated and returned in
     * resultReg. */
    generateLabelInstruction(cg, TR::InstOpCode::label, node, foundLabel);
    ...
    /* Under UTF16, resultReg is doubled to properly calculate index for 2 byte long characters. */
    if (!isLatin1)
        generateTrg1Src1ImmInstruction(cg, TR::InstOpCode::srawi, node, resultReg, resultReg, 1);

    TR::RegisterDependencyConditions *deps
        = new (cg->trHeapMemory()) TR::RegisterDependencyConditions(0, numDependencies, cg->trMemory());
    ...
    deps->getPostConditions()->getRegisterDependency(deps->getAddCursorForPost() - 1)->setExcludeGPR0();
    deps->addPostCondition(zeroReg, TR::RealRegister::NoReg);
    deps->getPostConditions()->getRegisterDependency(deps->getAddCursorForPost() - 1)->setExcludeGPR0();
    deps->addPostCondition(tempReg, TR::RealRegister::NoReg);
    deps->addPostCondition(temp2Reg, TR::RealRegister::NoReg);
    deps->addPostCondition(subSearchOffsetReg, TR::RealRegister::NoReg);
    deps->getPostConditions()->getRegisterDependency(deps->getAddCursorForPost() - 1)->setExcludeGPR0();
    ...
    deps->addPostCondition(targetVectorNotReg, TR::RealRegister::NoReg);
...
static bool inlineIntrinsicInflate(TR::Node *node, TR::CodeGenerator *cg)
{
    ...
    cg->generateDebugCounter(
        TR::DebugCounter::debugCounterName(comp, "inlineIntrinsicInflate/(%s)", comp->signature()));
     ...
     *   vmrghb   vec2Reg, constZeroVecReg, vec1Reg   //Interlaces 0x00 to convert the bytes to char, vec2Reg =
     * 0x00010002_00030004_00050006_00070008 stxvb16x vec2Reg, r0, outputAddressReg       //Stores vec2Reg to memory. 00
     * 01 00 02 00 03 00 04 00 05 00 06 00 07 00 08 vmrglb   vec1Reg, constZeroVecReg, vec1Reg   //Interlaces 0x00 to
     * convert the bytes to char, vec1Reg = 0x0009000a_000b000c_000d000e_000f0010 stxvb16x vec1Reg, outputAddressReg,
     * tempReg  //Stores vec1Reg to memory. 00 09 00 0a 00 0b 00 0c 00 0d 00 0e 00 0f 00 10 Power9 LE case: lxvb16x
     * vec1Reg, r0, inputAddressReg        //eg. vec1Reg = 01020304_05060708_090a0b0c_0d0e0f10 vmrghb   vec2Reg,
     * vec1Reg, constZeroVecReg   //Interlaces 0x00 to convert the bytes to char, vec2Reg =
     * 0x01000200_03000400_05000600_07000800 stxvb16x vec2Reg, r0, outputAddressReg       //Stores vec2Reg to memory. 01
     * 00 02 00 03 00 04 00 05 00 06 00 07 00 08 00 vmrglb   vec1Reg, vec1Reg, constZeroVecReg   //Interlaces 0x00 to
     * convert the bytes to char, vec2Reg = 0x09000a00_0b000c00_0d000e00_0f001000 stxvb16x vec1Reg, outputAddressReg,
     * tempReg  //Stores vec1Reg to memory. 09 00 0a 00 0b 00 0c 00 0d 00 0e 00 0f 00 10 00 Power8 BE case: lxvw4x
     * vec1Reg, r0, inputAddressReg        //eg. vec1Reg = 01020304_05060708_090a0b0c_0d0e0f10 vmrghb   vec2Reg,
     * constZeroVecReg, vec1Reg   //Interlaces 0x00 to convert the bytes to char, vec2Reg =
     * 0x00010002_00030004_00050006_00070008 stxvw4x  vec2Reg, r0, outputAddressReg       //Stores vec2Reg to memory. 00
     * 01 00 02 00 03 00 04 00 05 00 06 00 07 00 08 vmrglb   vec1Reg, constZeroVecReg, vec1Reg   //Interlaces 0x00 to
     * convert the bytes to char, vec1Reg = 0x0009000a_000b000c_000d000e_000f0010 stxvw4x  vec1Reg, outputAddressReg,
     * tempReg  //Stores vec1Reg to memory. 00 09 00 0a 00 0b 00 0c 00 0d 00 0e 00 0f 00 10 Power8 LE case: lxvw4x
     * vec1Reg, r0, inputAddressReg        //eg. vec1Reg = 0x04030201_08070605_0c0b0a09_100f0e0d, LE load mixes up the
     * byte order. vrlw     vec1Reg, vec1Reg, constNeg16VecReg  //Rotate by -16, vec1Reg =
     * 0x02010403_06050807_0a090c0b_0e0d100f vmrghb   vec2Reg, constZeroVecReg, vec1Reg   //Interlaces 0x00 to convert
     * the bytes to char, vec2Reg = 0x00020001_00040003_00060005_00080007 stxvw4x  vec2Reg, r0, outputAddressReg //This
     * puts the chars in the right order in memory. 01 00 02 00 03 00 04 00 05 00 06 00 07 00 08 00 vmrglb   vec1Reg,
     * constZeroVecReg, vec1Reg   //Interlaces 0x00 to convert the bytes to char, vec1Reg =
     * 0x000a0009_000c000b_000e000d_0010000f stxvw4x  vec1Reg, outputAddressReg, tempReg  //This puts the chars in the
     * right order in memory. 09 00 0a 00 0b 00 0c 00 0d 00 0e 00 0f 00 10 00
     */
    if (isAtLeastP9) {
        generateTrg1MemInstruction(cg, TR::InstOpCode::lxvb16x, node, vec1Reg,
            TR::MemoryReference::createWithIndexReg(cg, NULL, inputAddressReg, 16));
    } else {
        generateTrg1MemInstruction(cg, TR::InstOpCode::lxvw4x, node, vec1Reg,
            TR::MemoryReference::createWithIndexReg(cg, NULL, inputAddressReg, 16));
    ...
    if (isAtLeastP9) {
        generateMemSrc1Instruction(cg, TR::InstOpCode::stxvb16x, node,
            TR::MemoryReference::createWithIndexReg(cg, NULL, outputAddressReg, 16), vec2Reg);
    } else {
    ...
    if (isAtLeastP9) {
        generateMemSrc1Instruction(cg, TR::InstOpCode::stxvb16x, node,
            TR::MemoryReference::createWithIndexReg(cg, outputAddressReg, tempReg, 16), vec1Reg);
    } else {
        generateMemSrc1Instruction(cg, TR::InstOpCode::stxvw4x, node,
            TR::MemoryReference::createWithIndexReg(cg, outputAddressReg, tempReg, 16), vec1Reg);
    }
     ...
     *   mtvsrd   vec1Reg, tempReg                    // Move data into vector reg. vec1Reg =
     * 0x01020304_05060708_XXXXXXXX_XXXXXXXX vmrghb   vec1Reg, constZeroVecReg, vec1Reg   // Interlaces 0x00 to convert
     * the bytes to char, vec1Reg = 0x00010002_00030004_00050006_00070008 stxvb16x vec1Reg, NULL, outputAddressReg //
     * Stores vec1Reg to memory. 00 01 00 02 00 03 00 04 00 05 00 06 00 07 00 08 Power9 LE case: ldbrx    tempReg, NULL,
     * inputAddressReg      // eg. tempReg = 0x01020304_05060708 mtvsrd   vec1Reg, tempReg                    // Move
     * data into vector reg. vec1Reg = 0x01020304_05060708_XXXXXXXX_XXXXXXXX vmrghb   vec1Reg, vec1Reg, constZeroVecReg
     * // Interlaces 0x00 to convert the bytes to char, vec1Reg = 0x01000200_03000400_05000600_07000800 stxvb16x
     * vec1Reg, NULL, outputAddressReg     // Stores vec1Reg to memory. 01 00 02 00 03 00 04 00 05 00 06 00 07 00 08 00
     * Power8 BE case:
     *   ldx      tempReg, NULL, inputAddressReg      // eg. tempReg = 0x01020304_05060708
     *   mtvsrd   vec1Reg, tempReg                    // Move data into vector reg. vec1Reg =
     * 0x01020304_05060708_XXXXXXXX_XXXXXXXX vmrghb   vec1Reg, constZeroVecReg, vec1Reg   // Interlaces 0x00 to convert
     * the bytes to char, vec1Reg = 0x00010002_00030004_00050006_00070008 stxvw4x  vec1Reg, NULL, outputAddressReg //
     * Stores vec1Reg to memory. 00 01 00 02 00 03 00 04 00 05 00 06 00 07 00 08 Power8 LE case: ldbrx    tempReg, NULL,
     * inputAddressReg      // eg. tempReg = 0x01020304_05060708 mtvsrd   vec1Reg, tempReg                    // Move
     * data into vector reg. vec1Reg = 0x01020304_05060708_XXXXXXXX_XXXXXXXX vmrghb   vec1Reg, constZeroVecReg, vec1Reg
     * // Interlaces 0x00 to convert the bytes to char, vec1Reg = 0x00010002_00030004_00050006_00070008 vrlw vec1Reg,
     * vec1Reg, constNeg16VecReg  // Rotate by -16 to adjust byte order for stxvw4x. vec1Reg =
     * 0x00020001_00040003_00060005_00080007 stxvw4x  vec1Reg, NULL, outputAddressReg     // Stores vec1Reg to memory.
     * 01 00 02 00 03 00 04 00 05 00 06 00 07 00 08 00
     */
    if (isLE) {
        generateTrg1MemInstruction(cg, TR::InstOpCode::ldbrx, node, tempReg,
            TR::MemoryReference::createWithIndexReg(cg, NULL, inputAddressReg, 8));
    } else {
        generateTrg1MemInstruction(cg, TR::InstOpCode::ldx, node, tempReg,
            TR::MemoryReference::createWithIndexReg(cg, NULL, inputAddressReg, 8));
    }
    ...
    if (isAtLeastP9) {
        generateMemSrc1Instruction(cg, TR::InstOpCode::stxvb16x, node,
            TR::MemoryReference::createWithIndexReg(cg, NULL, outputAddressReg, 16), vec1Reg);
    } else {
        generateMemSrc1Instruction(cg, TR::InstOpCode::stxvw4x, node,
            TR::MemoryReference::createWithIndexReg(cg, NULL, outputAddressReg, 16), vec1Reg);
    }
     ...
     */
    generateTrg1MemInstruction(cg, TR::InstOpCode::lwzx, node, tempReg,
        TR::MemoryReference::createWithIndexReg(cg, NULL, inputAddressReg, 4));
    ...
    generateTrg1Src1Imm2Instruction(cg, TR::InstOpCode::rldimi, node, tempReg, tempReg, 24,
        CONSTANT64(0x00FFFFFFFF000000));
    generateTrg1Src1Imm2Instruction(cg, TR::InstOpCode::rldimi, node, tempReg, tempReg, 16,
        CONSTANT64(0x000000FFFFFF0000));
    generateTrg1Src1Imm2Instruction(cg, TR::InstOpCode::rldimi, node, tempReg, tempReg, 8,
        CONSTANT64(0x0000000000FFFF00));
    generateTrg1Src2Instruction(cg, TR::InstOpCode::AND, node, tempReg, tempReg, maskReg);

    generateMemSrc1Instruction(cg, TR::InstOpCode::stdx, node,
        TR::MemoryReference::createWithIndexReg(cg, NULL, outputAddressReg, 8), tempReg);
     ...
     */
    generateTrg1MemInstruction(cg, TR::InstOpCode::lbz, node, tempReg,
        TR::MemoryReference::createWithDisplacement(cg, inputAddressReg, 0, 1));
    generateMemSrc1Instruction(cg, TR::InstOpCode::sth, node,
        TR::MemoryReference::createWithDisplacement(cg, outputAddressReg, 0, 2), tempReg);
    generateTrg1Src1ImmInstruction(cg, TR::InstOpCode::addi, node, remainingReg, remainingReg, -1);
    generateTrg1Src1ImmInstruction(cg, TR::InstOpCode::cmpi4, node, condReg, remainingReg, 0);
    generateConditionalBranchInstruction(cg, TR::InstOpCode::beq, node, doneLabel, condReg);

    generateTrg1MemInstruction(cg, TR::InstOpCode::lbz, node, tempReg,
        TR::MemoryReference::createWithDisplacement(cg, inputAddressReg, 1, 1));
    generateMemSrc1Instruction(cg, TR::InstOpCode::sth, node,
        TR::MemoryReference::createWithDisplacement(cg, outputAddressReg, 2, 2), tempReg);
    generateTrg1Src1ImmInstruction(cg, TR::InstOpCode::addi, node, remainingReg, remainingReg, -1);
    generateTrg1Src1ImmInstruction(cg, TR::InstOpCode::cmpi4, node, condReg, remainingReg, 0);
    generateConditionalBranchInstruction(cg, TR::InstOpCode::beq, node, doneLabel, condReg);

    generateTrg1MemInstruction(cg, TR::InstOpCode::lbz, node, tempReg,
        TR::MemoryReference::createWithDisplacement(cg, inputAddressReg, 2, 1));
    generateMemSrc1Instruction(cg, TR::InstOpCode::sth, node,
        TR::MemoryReference::createWithDisplacement(cg, outputAddressReg, 4, 2), tempReg);
    ...
    bool isCountPositives)
{
    ...
    TR::LabelSymbol *serialHWordLabel = generateLabelSymbol(cg);
    ...
    // check empty
    generateTrg1Src1ImmInstruction(cg, TR::InstOpCode::cmpi4, node, cr0, lengthReg, 0);
    generateConditionalBranchInstruction(cg, TR::InstOpCode::ble, node, noMatchLabel, cr0);

    // skip over or load the header
#if defined(J9VM_GC_SPARSE_HEAP_ALLOCATION)
    if (TR::Compiler->om.isOffHeapAllocationEnabled()) {
        generateTrg1MemInstruction(cg, TR::InstOpCode::ld, node, startReg,
            TR::MemoryReference::createWithDisplacement(cg, startReg,
                TR::Compiler->om.offsetOfContiguousDataAddrField(), 8));
    } else
#endif /* J9VM_GC_SPARSE_HEAP_ALLOCATION */
    {
    ...
    // --- check first byte
    generateTrg1Src1ImmInstruction(cg, TR::InstOpCode::cmpi4, node, cr0, lengthReg, 1);
    generateConditionalBranchInstruction(cg, TR::InstOpCode::bne, node, serialCheckLabel, cr0);
    ...
    // --- a very quick check with anything longer being checked at the serialWordCheck
    ...
    generateTrg1ImmInstruction(cg, TR::InstOpCode::li, node, indexReg, 0);
    // Can we load at least a word?
    generateTrg1Src1ImmInstruction(cg, TR::InstOpCode::cmpi4, node, cr0, lengthReg, 4);
    generateConditionalBranchInstruction(cg, TR::InstOpCode::bge, node, serialWordCheckLabel, cr0);
    // fallthrough to load half word, since the length can't be 1
    ...
    } else // for hasNegatives we don't care about the byte ordering
    {
        ...
        generateTrg1Src1ImmInstruction(cg, TR::InstOpCode::andi_r, node, storeReg, storeReg, 0x8080);
        generateConditionalBranchInstruction(cg, TR::InstOpCode::bne, node, endLabel, cr0);
        generateTrg1Src1ImmInstruction(cg, TR::InstOpCode::addi, node, indexReg, indexReg, 2);
    }
    // 0-1 item remaining to be checked
    ...
    if (p10Plus) {
        if (isCountPositives) // indexReg = storeReg < 0 ? indexReg : indexReg + 1
        {
            ...
            generateTrg1Src2Instruction(cg, TR::InstOpCode::add, node, indexReg, indexReg, tempReg);
        } else // tempReg = (storeReg < 0)
        {
            generateTrg1Src1ImmInstruction(cg, TR::InstOpCode::setbcr, node, tempReg, cr0, 2);
        }
        generateLabelInstruction(cg, TR::InstOpCode::b, node, endLabel);
    ...
    // If we didn't jump away, we must have 0-3 items remaining to be checked.
    // We kinda have to tuck the code checking which residual handler to use here:
    //    no item - end; 1 item - byte; 2-3 items - hword
    generateTrg1Src2Instruction(cg, TR::InstOpCode::subf_r, node, tempReg, indexReg, lengthReg);
    if (isCountPositives)
        generateConditionalBranchInstruction(cg, TR::InstOpCode::beq, node, endLabel, cr0);
    else
        generateConditionalBranchInstruction(cg, TR::InstOpCode::beq, node, noMatchLabel, cr0);
    generateTrg1Src1ImmInstruction(cg, TR::InstOpCode::cmpi4, node, cr0, tempReg, 2);
    generateConditionalBranchInstruction(cg, TR::InstOpCode::blt, node, serial1Label, cr0);
    generateLabelInstruction(cg, TR::InstOpCode::b, node, serialHWordLabel);

    // --- size 8-15: load double word
    generateLabelInstruction(cg, TR::InstOpCode::label, node, serialDWordLabel);
    // No length check is needed, since we have checked it before coming here

    // maskReg is currently 0x80808080, we want it to have 8 bytes of 0x80 instead
    generateTrg1Src1Imm2Instruction(cg, TR::InstOpCode::rldimi, node, maskReg, maskReg, 32,
        CONSTANT64(0xFFFFFFFF00000000));
    ...
    if (isCountPositives) // when counting positives, we must consider every byte separately
    {
        ...
        generateTrg1Src2Instruction(cg, TR::InstOpCode::and_r, node, storeReg, storeReg, maskReg);
        generateConditionalBranchInstruction(cg, TR::InstOpCode::bne, node, matchLabel, cr0);
        generateTrg1Src1ImmInstruction(cg, TR::InstOpCode::addi, node, indexReg, indexReg, 8);
    }
    // If we didn't jump away, we have 0-7 items to deal with:
    // We kinda have to tuck the code checking which residual handler to use here:
    //    no item - end; 1 item - byte; 2-3 items - hword; 4-7 items - word.
    generateTrg1Src2Instruction(cg, TR::InstOpCode::subf_r, node, tempReg, indexReg, lengthReg);
    if (isCountPositives)
        generateConditionalBranchInstruction(cg, TR::InstOpCode::beq, node, endLabel, cr0);
    else
    ...
    generateConditionalBranchInstruction(cg, TR::InstOpCode::bge, node, serialWordLabel, cr0);
    generateTrg1Src1ImmInstruction(cg, TR::InstOpCode::cmpi4, node, cr0, tempReg, 2);
    ...
    generateLabelInstruction(cg, TR::InstOpCode::label, node, vectorLoopLabel);
    // Load 16 items; for p9+ we can count zeroes in a vector register, so loading in order matters;
    // before p9, we can't load bytes in the correct order anyways, so we ignore the ordering here.
    if (p9Plus)
    ...
    generateTrg1Src2Instruction(cg, TR::InstOpCode::cmp4, node, cr0, indexReg, tempReg);
    generateConditionalBranchInstruction(cg, TR::InstOpCode::blt, node, vectorLoopLabel, cr0);

    // go to residue if we don't have enough items to do one load
    // We kinda have to tuck the code checking which residual handler to use here:
    //    no item - end; 1 item - byte; 2-3 items - hword; 4-15 items - word/dword.
    generateTrg1Src2Instruction(cg, TR::InstOpCode::subf_r, node, tempReg, indexReg, lengthReg);
    if (isCountPositives)
        generateConditionalBranchInstruction(cg, TR::InstOpCode::beq, node, endLabel, cr0);
    else
        generateConditionalBranchInstruction(cg, TR::InstOpCode::beq, node, noMatchLabel, cr0);
    generateTrg1Src1ImmInstruction(cg, TR::InstOpCode::cmpi4, node, cr0, tempReg, 4);
    generateConditionalBranchInstruction(cg, TR::InstOpCode::bge, node, serialDWordCheckLabel, cr0);
    generateTrg1Src1ImmInstruction(cg, TR::InstOpCode::cmpi4, node, cr0, tempReg, 2);
    generateConditionalBranchInstruction(cg, TR::InstOpCode::bge, node, serialHWordLabel, cr0);
    generateLabelInstruction(cg, TR::InstOpCode::b, node, serial1Label);
    ...
    generateLabelInstruction(cg, TR::InstOpCode::label, node, noMatchLabel);
    if (isCountPositives)
...
/*
 * Arraycopy evaluator needs a version of inlineArrayCopy that can be used inside internal control flow. For this
 * version of inlineArrayCopy, registers must be allocated outside of this function so the dependency at the end of the
 * control flow knows about them.
 */
static void inlineArrayCopy_ICF(TR::Node *node, int64_t byteLen, TR::Register *src, TR::Register *dst,
    TR::CodeGenerator *cg, TR::Register *cndReg, TR::Register *tmp1Reg, TR::Register *tmp2Reg, TR::Register *tmp3Reg,
    TR::Register *tmp4Reg, TR::Register *fp1Reg, TR::Register *fp2Reg, TR::Register *fp3Reg, TR::Register *fp4Reg)
{
    if (byteLen == 0)
        return;
    ...
    if (postP10CopyInline) {
        ...
        if (iteration64 > 0) {
            ...
            generateTrg1MemInstruction(cg, TR::InstOpCode::lxv, node, fp1Reg,
                TR::MemoryReference::createWithDisplacement(cg, src, 0, 16));
            generateMemSrc1Instruction(cg, TR::InstOpCode::stxv, node,
                TR::MemoryReference::createWithDisplacement(cg, dst, 0, 16), fp1Reg);
            generateTrg1MemInstruction(cg, TR::InstOpCode::lxv, node, fp1Reg,
                TR::MemoryReference::createWithDisplacement(cg, src, 16, 16));
            generateMemSrc1Instruction(cg, TR::InstOpCode::stxv, node,
                TR::MemoryReference::createWithDisplacement(cg, dst, 16, 16), fp1Reg);
            generateTrg1MemInstruction(cg, TR::InstOpCode::lxv, node, fp1Reg,
                TR::MemoryReference::createWithDisplacement(cg, src, 32, 16));
            generateMemSrc1Instruction(cg, TR::InstOpCode::stxv, node,
                TR::MemoryReference::createWithDisplacement(cg, dst, 32, 16), fp1Reg);
            generateTrg1MemInstruction(cg, TR::InstOpCode::lxv, node, fp1Reg,
                TR::MemoryReference::createWithDisplacement(cg, src, 48, 16));
            generateMemSrc1Instruction(cg, TR::InstOpCode::stxv, node,
                TR::MemoryReference::createWithDisplacement(cg, dst, 48, 16), fp1Reg);
        ...
        for (int32_t i = 0; i < (residue64 >> 4); i++) {
            ...
            generateMemSrc1Instruction(cg, TR::InstOpCode::stxv, node,
                TR::MemoryReference::createWithDisplacement(cg, dst, standingOffset + i * 16, 16), fp1Reg);
        }
        ...
        if ((residue64 & 0xF) != 0) {
            ...
            switch (residue64 & 0xF) {
                case 1:
                    generateTrg1MemInstruction(cg, TR::InstOpCode::lbz, node, tmp1Reg,
                        TR::MemoryReference::createWithDisplacement(cg, src, standingOffset, 1));
                    generateMemSrc1Instruction(cg, TR::InstOpCode::stb, node,
                        TR::MemoryReference::createWithDisplacement(cg, dst, standingOffset, 1), tmp1Reg);
                    break;
                case 2:
                    generateTrg1MemInstruction(cg, TR::InstOpCode::lhz, node, tmp1Reg,
                        TR::MemoryReference::createWithDisplacement(cg, src, standingOffset, 2));
                    generateMemSrc1Instruction(cg, TR::InstOpCode::sth, node,
                        TR::MemoryReference::createWithDisplacement(cg, dst, standingOffset, 2), tmp1Reg);
                    break;
                case 4:
                    ...
                    generateMemSrc1Instruction(cg, TR::InstOpCode::stw, node,
                        TR::MemoryReference::createWithDisplacement(cg, dst, standingOffset, 4), tmp1Reg);
                    break;
                case 8:
                    generateTrg1MemInstruction(cg, TR::InstOpCode::ld, node, tmp1Reg,
                        TR::MemoryReference::createWithDisplacement(cg, src, standingOffset, 8));
                    generateMemSrc1Instruction(cg, TR::InstOpCode::std, node,
                        TR::MemoryReference::createWithDisplacement(cg, dst, standingOffset, 8), tmp1Reg);
                    break;
                default:
        ...
        bool supportsLEArrayCopyInline = cg->comp()->target().cpu.isAtLeast(OMR_PROCESSOR_PPC_P8)
            && !disableLEArrayCopyInline && cg->comp()->target().cpu.isLittleEndian()
            && cg->comp()->target().cpu.hasFPU() && cg->comp()->target().is64Bit();
        ...
        /* Some machines have issues with 64bit loads/stores in 32bit mode, ie. sicily, do not check for
         * is64BitProcessor() */
...
bool J9::Power::CodeGenerator::inlineDirectCall(TR::Node *node, TR::Register *&resultReg)
{
    ...
    if (comp->getSymRefTab()->isNonHelper(node->getSymbolReference(),
            TR::SymbolReferenceTable::singlePrecisionSQRTSymbol)) {
        ...
        switch (methodSymbol->getRecognizedMethod()) {
            case TR::java_lang_Thread_onSpinWait: {
                if (!disableOSW) {
                    logprintf(comp->getOption(TR_TraceCG), comp->log(),
                        "Inlining Thread.onSpinWait call node %p as yield instruction.\n", node);

                    /*
                     * onSpinWait() method calls VM_AtomicSupport::yieldCPU() which is a yield instruction encoded as
                     * "or r27, r27, r27". Check omr/include_core/AtomicSupport.hpp for the JNI implementation.
                     */
            ...
            case TR::sun_misc_Unsafe_setMemory:
                if (comp->canTransformUnsafeSetMemory()) {
                    ...
                    //
                    // 1.) The object at dest is known to be a non-array object at compile time. In this scenario, the
                    // final destination address
                    //     can be calculated right away as dest += destOffset. The IL node passed in to
                    //     setmemoryEvaluator will have three children: dest, len, and byteValue.
                    // 2.) The object at dest is known to be an array at compile time. In this scenario, we must load
                    // dataAddr from dest
                    //     to account for the way arrays are allocated when offheap is enabled. Once this adjustment is
                    //     made, the final destination address can be calculated dest += destOffset. Similar to case
                    //     (1), the IL node passed in to setmemoryEvaluator() will have three children: dest, len, and
                    //     byteValue.
                    // 3.) The type of the object at dest is unknown at compile time. In this scenario, a runtime array
                    // check must be generated to
                    //     determine whether dataAddr needs to be loaded before calculating the final destination
                    //     address. This means that we cannot determine the final destination address right away, and
                    //     thus need to keep dest and destOffset separate. In this case, the node passed in to
                    //     setmemoryEvaluator() will have four children: dest, destOffset, len, and byteValue.
                    //
                    // Note that if the default (gencon) GC policy is being used OR if the object is a NULL reference,
                    // dataAddr will never need to be loaded no matter the type of the object at dest, so all of the
                    // above cases will be treated like case (1).

                    // if offheap is not enabled OR if dest is known to be a NULL reference at compile time, default to
                    // case (1):
                    //  - do NOT to load dataAddr to calculate final destination address
                    //  - calculate the final destination address right away and only pass three children into
                    //  setmemoryEvaluator()
                    ...
                    if (TR::Compiler->om.isOffHeapAllocationEnabled() && comp->target().is64Bit()
                        ...
                        if (dest->getSymbolReference() != NULL) {
                            ...
                            // 3.) The signature names some interface type, wich is a problem because interface types
                            // are
                            //     not checked by the verifier
                            // 4.) The signature belongs to a parameter of the calling method, which is a problem
                            // because
                            ...
                            if (objTypeSig == NULL) {
                                ...
                                objClass = fej9->getClassFromSignature(objTypeSig, length,
                            ...
                            // case (2): want to load dataAddr and calculate final dest address right away if both of
                            ...
                            // case (3): want to keep dest and destOffset separate so that we can perform runtime
                            // NULL/array tests if at least one of
                            //           the following conditions are met:
                    ...
                    {
                        // four child setup
                        copyMemNode = TR::Node::createWithSymRef(TR::arrayset, 4, 4, dest, destOffset, len, byteValue,
                            node->getSymbolReference());
                    } else // CASE (1) and (2): dest += destoffset, then pass in to evaluator
                    {
                        ...
                        // three child setup
                        dest = TR::Node::create(comp->target().is32Bit() ? TR::aiadd : TR::aladd, 2, dest,
                            destOffset); // dest += destOffset
                        copyMemNode = TR::Node::createWithSymRef(TR::arrayset, 3, 3, dest, len, byteValue,
                            node->getSymbolReference());
                    }
            ...
            case TR::com_ibm_jit_JITHelpers_intrinsicIndexOfUTF16:
                if (cg->getSupportsInlineStringIndexOf()) {
                    bool isLatin1
                        = (methodSymbol->getRecognizedMethod() == TR::com_ibm_jit_JITHelpers_intrinsicIndexOfLatin1)
            ...
            case TR::java_lang_Class_isAssignableFrom: {
                ...
                if (receiverNode->getOpCodeValue() == TR::aloadi
                    ...
                    if (receiverClassSymRef->isClassInterface(comp) || receiverClassSymRef->isClassAbstract(comp))
                        break;
                }
                ...
                if (!disable
                    && performTransformation(comp,
                        "O^O PPC Evaluator: Specialize call to java/lang/Class.isAssignableFrom [%p].\n", node)) {
...
void VMgenerateCatchBlockBBStartPrologue(TR::Node *node, TR::Instruction *fenceInstruction, TR::CodeGenerator *cg)
{
    ...
    if (fej9->shouldPerformEDO(block, comp)) {
        ...
        TR::Instruction *cursor = loadAddressConstant(cg, cg->needRelocationsForBodyInfoData(), node, addr, biAddrReg,
            NULL, false, TR_BodyInfoAddressLoad);
        TR::MemoryReference *loadbiMR
            = TR::MemoryReference::createWithDisplacement(cg, biAddrReg, 0, TR::Compiler->om.sizeofReferenceAddress());
        TR::MemoryReference *storebiMR
            = TR::MemoryReference::createWithDisplacement(cg, biAddrReg, 0, TR::Compiler->om.sizeofReferenceAddress());
        ...
        cursor = generateTrg1Src1ImmInstruction(cg, TR::InstOpCode::addi2_r, node, recompCounterReg, recompCounterReg,
            cndRegister, -1, cursor);
    ...
    if (value != 0x0) {
        ...
        if (isAOT)
            cg->addExternalRelocation(
                new (cg->trHeapMemory())
                    TR::BeforeBinaryEncodingExternalRelocation(cursor, (uint8_t *)node->getSymbolReference(),
                        node ? (uint8_t *)(intptr_t)node->getInlinedSiteIndex() : (uint8_t *)-1, reloType, cg),
                __FILE__, __LINE__, node);
    }

    cursor = generateTrg1Src1ImmInstruction(cg, TR::InstOpCode::ori, node, trgReg, trgReg,
        isAOT ? 0 : value & 0x0000ffff, cursor);
...
TR::Instruction *loadAddressRAM(TR::CodeGenerator *cg, TR::Node *node, intptr_t value, TR::Register *trgReg)
{
    ...
    cursor = generateTrg1Src1ImmInstruction(cg, TR::InstOpCode::ori, node, trgReg, trgReg,
        isAOT ? 0 : ((value >> 32) & 0x0000ffff), cursor);
    ...
    cursor = generateTrg1Src1ImmInstruction(cg, TR::InstOpCode::oris, node, trgReg, trgReg,
        isAOT ? 0 : ((value >> 16) & 0x0000ffff), cursor);
...
TR::Instruction *loadAddressJNI32(TR::CodeGenerator *cg, TR::Node *node, int32_t value, TR::Register *trgReg)
{
    ...
    if (isAOT && value != 0x0) {
        ...
        cg->addExternalRelocation(new (cg->trHeapMemory()) TR::BeforeBinaryEncodingExternalRelocation(cursor,
                                      reinterpret_cast<uint8_t *>(info), reloType, cg),
            __FILE__, __LINE__, node);
    }
...
TR::Register *J9::Power::TreeEvaluator::directCallEvaluator(TR::Node *node, TR::CodeGenerator *cg)
{
    ...
    if (callee->getRecognizedMethod() >= TR::FirstVectorMethod && callee->getRecognizedMethod() <= TR::LastVectorMethod
...
TR::Register *J9::Power::TreeEvaluator::setmemoryEvaluator(TR::Node *node, TR::CodeGenerator *cg)
{
    ...
    // if node three children (i.e.: object base address and offset have already been added together), don't need array
    // check
    ...
    bool stopUsingCopyRegBase
        = dstBaseAddrNode ? TR::TreeEvaluator::stopUsingCopyReg(dstBaseAddrNode, dstBaseAddrReg, cg) : false;
    ...
    if (dstOffsetNode && !useOffsetAsImmVal) // only want to allocate a register for dstoffset if we're using it for the
                                             // array check AND it isn't a constant
    {
        if (!cg->canClobberNodesRegister(
                lengthNode)) // only need to copy dstOffset into another register if the current one isn't clobberable
        {
            if (cg->comp()
                    ->target()
                    .is32Bit()) // on 32-bit systems, need to grab the lower 32 bits of offset from the register pair
            {
                dstOffsetReg = cg->evaluate(dstOffsetNode);
                TR::Register *offsetCopyReg = cg->allocateRegister();
                generateTrg1Src1Instruction(cg, TR::InstOpCode::mr, dstOffsetNode, offsetCopyReg,
                    dstOffsetReg->getLowOrder());
    ...
    if (!cg->canClobberNodesRegister(lengthNode)) {
        ...
        if (cg->comp()
                ->target()
                .is32Bit()) // on 32-bit systems, need to grab the lower 32 bits of length from the register pair
        ...
        if (cg->comp()
                ->target()
                .is32Bit()) // on 32-bit system, need to grab lower 32 bits of length from the register pair
    ...
    if (cg->comp()->target().cpu.isAtLeast(OMR_PROCESSOR_PPC_P8)) {
        // on P8 or higher, we can use vector instructions to cut down on loop iterations and residual tests -> need to
        // copy valueReg into a VSX register
    ...
    // need extra register for offset only if it isn't already included in the destination address AND it isn't a
    // constant
    ...
    if (arrayCheckNeeded) // CASE (3)
    {
        ...
        // We only want to generate a runtime NULL check if the status of the object (i.e.: whether it is NULL or
        // non-NULL) is NOT known. Note that if the object is known to be NULL, arrayCheckNeeded will be false, so there
        // is no need to check that condition here.
        ...
        if (TR::Compiler->om.compressObjectReferences())
            generateTrg1MemInstruction(cg, TR::InstOpCode::lwz, node, dstClassInfoReg,
                TR::MemoryReference::createWithDisplacement(cg, dstBaseAddrReg,
                    static_cast<int32_t>(TR::Compiler->om.offsetOfObjectVftField()), 4));
        else
        ...
        TR::MemoryReference *dstClassMR = TR::MemoryReference::createWithDisplacement(cg, dstClassInfoReg,
            offsetof(J9Class, classDepthAndFlags), TR::Compiler->om.sizeofReferenceAddress());
        ...
        generateTrg1Src1ImmInstruction(cg, TR::InstOpCode::andis_r, node, arrayFlagReg, dstClassInfoReg,
            arrayFlagValue >> 16);
        ...
        TR::MemoryReference *dataAddrSlotMR = TR::MemoryReference::createWithDisplacement(cg, dstBaseAddrReg,
            comp->fej9()->getOffsetOfContiguousDataAddrField(), TR::Compiler->om.sizeofReferenceAddress());
    ...
    if (cg->comp()->target().cpu.isAtLeast(OMR_PROCESSOR_PPC_P8)) {
        ...
        generateTrg1Src1Imm2Instruction(cg, TR::InstOpCode::rldimi, node, valueReg, valueReg, 8,
            CONSTANT64(0x000000000000FF00));
        generateTrg1Src1Imm2Instruction(cg, TR::InstOpCode::rldimi, node, valueReg, valueReg, 16,
            CONSTANT64(0x00000000FFFF0000));
        generateTrg1Src1Imm2Instruction(cg, TR::InstOpCode::rldimi, node, valueReg, valueReg, 32,
            CONSTANT64(0xFFFFFFFF00000000));
    }
    ...
    generateTrg1Src1ImmInstruction(cg, lengthNode->getType().isInt32() ? TR::InstOpCode::srawi : TR::InstOpCode::sradi,
        node, temp1Reg, lengthReg, 5);
    ...
    if (cg->comp()->target().cpu.isAtLeast(OMR_PROCESSOR_PPC_P8)) {
        ...
        generateMemSrc1Instruction(cg, TR::InstOpCode::stxvd2x, node,
            TR::MemoryReference::createWithIndexReg(cg, NULL, dstAddrReg, 16), valueReg);
        ...
        generateMemSrc1Instruction(cg, TR::InstOpCode::std, node,
            TR::MemoryReference::createWithDisplacement(cg, dstAddrReg, 0, 8), valueReg);
        generateMemSrc1Instruction(cg, TR::InstOpCode::std, node,
            TR::MemoryReference::createWithDisplacement(cg, dstAddrReg, 8, 8), valueReg);
        generateMemSrc1Instruction(cg, TR::InstOpCode::std, node,
            TR::MemoryReference::createWithDisplacement(cg, dstAddrReg, 16, 8), valueReg);
        generateMemSrc1Instruction(cg, TR::InstOpCode::std, node,
            TR::MemoryReference::createWithDisplacement(cg, dstAddrReg, 24, 8), valueReg);
            ...
            OMR_PROCESSOR_PPC_P10)) // on P10, we can use stxvl to store all residual bytes efficiently
    {
        ...
        generateMemSrc1Instruction(cg, TR::InstOpCode::std, node,
            TR::MemoryReference::createWithDisplacement(cg, dstAddrReg, 0, 8), valueResidueReg);
        generateMemSrc1Instruction(cg, TR::InstOpCode::std, node,
            TR::MemoryReference::createWithDisplacement(cg, dstAddrReg, 8, 8), valueResidueReg);
        generateTrg1Src1ImmInstruction(cg, TR::InstOpCode::addi, node, dstAddrReg, dstAddrReg, 16);
        ...
        generateMemSrc1Instruction(cg, TR::InstOpCode::stw, node,
            TR::MemoryReference::createWithDisplacement(cg, dstAddrReg, 0, 4), valueResidueReg);
        ...
        generateMemSrc1Instruction(cg, TR::InstOpCode::sth, node,
            TR::MemoryReference::createWithDisplacement(cg, dstAddrReg, 0, 2), valueResidueReg);
        ...
        generateConditionalBranchInstruction(cg, TR::InstOpCode::beq, node, doneLabel, cndReg);
        generateMemSrc1Instruction(cg, TR::InstOpCode::stb, node,
            TR::MemoryReference::createWithDisplacement(cg, dstAddrReg, 0, 1), valueResidueReg);
    }
...
TR::Register *J9::Power::TreeEvaluator::tstartEvaluator(TR::Node *node, TR::CodeGenerator *cg)
{
    ...
    if (debugTMTLE)
        printf("\nTM: use TM TLE in %s (%s) %p", comp->signature(), comp->getHotnessName(comp->getMethodHotness()),
            node);
    ...
    generateConditionalBranchInstruction(cg, TR::InstOpCode::bne, node, lockwordLabel, cndReg);
    ...
    if (transientConditions) {
        generateDepConditionalBranchInstruction(cg, TR::InstOpCode::beq, node, labelTransientFailure, cndReg,
            transientConditions);
    } else {
    ...
    if (comp->target().is64Bit() && cg->fej9()->generateCompressedLockWord()) {
        ...
        generateTrg1MemInstruction(cg, TR::InstOpCode::Op_load, node, monReg,
            TR::MemoryReference::createWithDisplacement(cg, objReg, lwOffset,
                TR::Compiler->om.sizeofReferenceAddress()));
    }
...
TR::Register *J9::Power::TreeEvaluator::arraycopyEvaluator(TR::Node *node, TR::CodeGenerator *cg)
{
     ...
     */
    if (TR::Compiler->om.readBarrierType() == gc_modron_readbar_none || !node->chkNoArrayStoreCheckArrayCopy()
        || !node->isReferenceArrayCopy() || debug("noArrayCopy")) {
    ...
    if (node->isForwardArrayCopy() && lengthNode->getOpCode().isLoadConst()) {
        ...
        // inlineArrayCopy_ICF is not currently capable of handling very long lengths correctly. Under some
        // circumstances, it will generate an li instruction with an out-of-bounds immediate, which triggers an assert
        // in the binary encoder.
        if (len >= 0 && len < MAX_PPC_ARRAYCOPY_INLINE) {
            /*
             * This path generates code to perform a runtime check on whether concurrent GC is done moving objects or
             * not. If it isn't, a call to referenceArrayCopy helper should be made. If it is, using the inlined array
             * copy code path is okay.
             */
            int32_t groups;
            ...
            static bool disableLEArrayCopyInline = (feGetEnv("TR_disableLEArrayCopyInline") != NULL);
            bool supportsLEArrayCopyInline = cg->comp()->target().cpu.isAtLeast(OMR_PROCESSOR_PPC_P8)
                && !disableLEArrayCopyInline && comp->target().cpu.isLittleEndian() && comp->target().cpu.hasFPU()
                && comp->target().is64Bit();
             ...
             * r11 is used for the tmp1Reg since r11 gets killed by the trampoline and values put into tmp1Reg are not
             * needed after the trampoline.
             */
            TR::RegisterDependencyConditions *deps
                = new (cg->trHeapMemory()) TR::RegisterDependencyConditions(0, numDeps, cg->trMemory());
            ...
            generateTrg1MemInstruction(cg, TR::InstOpCode::Op_load, node, tmp1Reg,
                TR::MemoryReference::createWithDisplacement(cg, metaReg, offsetof(J9VMThread, readBarrierRangeCheckTop),
                    TR::Compiler->om.sizeofReferenceAddress()));
            ...
            inlineArrayCopy_ICF(node, len, srcAddrReg, dstAddrReg, cg, condReg, tmp1Reg, tmp2Reg, tmp3Reg, tmp4Reg,
            ...
            gcPoint = generateDepImmSymInstruction(cg, TR::InstOpCode::bl, node,
                (uintptr_t)helperSym->getMethodAddress(), helperDeps, helperSym);
            gcPoint->PPCNeedsGCMap(pp.getPreservedRegisterMapForGC());
    ...
    bool useVSXForCopy = cg->comp()->target().cpu.isAtLeast(OMR_PROCESSOR_PPC_P8) && !disableVSXArrayCopy
        && cg->comp()->target().cpu.supportsFeature(OMR_FEATURE_PPC_HAS_VSX);
     ...
     * Unfortunately, the two different paths have a conflict regarding which real register they want srcAddrReg,
     * dstAddrReg and lengthReg in. Dependencies are set up to favour the fast assembly path. Register moves are used in
     * the slow helper path to move the values to the real registers they are expected to be in.
     */
     ...
     * r8 = length
     *
     * CAUTION: Virtual register names are based on their use during the non-helper path so they are misleading after
     * this point. Due to register reuse, pay attention to copying order so that a register is not clobbered too early.
     */
    generateTrg1Src1Instruction(cg, TR::InstOpCode::mr, node, r3Reg, metaReg);
    ...
    generateTrg1Src1Instruction(cg, TR::InstOpCode::mr, node, srcAddrReg,
        lengthReg); // srcAddrReg is tied to r8. Need to copy srcAddrReg first.
    generateTrg1Src1Instruction(cg, TR::InstOpCode::mr, node, lengthReg,
        dstAddrReg); // lengthReg is tied to r7.  Need to copy lengthReg first.

    TR::RegisterDependencyConditions *helperDeps
        = new (cg->trHeapMemory()) TR::RegisterDependencyConditions(0, 0, cg->trMemory());
    TR::SymbolReference *helperSym = comp->getSymRefTab()->findOrCreateRuntimeHelper(TR_referenceArrayCopy);

    gcPoint = generateDepImmSymInstruction(cg, TR::InstOpCode::bl, node, (uintptr_t)helperSym->getMethodAddress(),
        helperDeps, helperSym);
    gcPoint->PPCNeedsGCMap(pp.getPreservedRegisterMapForGC());
...
TR::Register *J9::Power::TreeEvaluator::NULLCHKEvaluator(TR::Node *node, TR::CodeGenerator *cg)
{
    return TR::TreeEvaluator::evaluateNULLCHKWithPossibleResolve(node, false, cg);
}

TR::Register *J9::Power::TreeEvaluator::resolveAndNULLCHKEvaluator(TR::Node *node, TR::CodeGenerator *cg)
{
    return TR::TreeEvaluator::evaluateNULLCHKWithPossibleResolve(node, true, cg);
}

TR::Register *J9::Power::TreeEvaluator::evaluateNULLCHKWithPossibleResolve(TR::Node *node, bool needsResolve,
    TR::CodeGenerator *cg)
{
    // NOTE:
