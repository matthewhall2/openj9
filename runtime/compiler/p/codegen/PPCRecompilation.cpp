...
TR::Instruction *TR_PPCRecompilation::generatePrePrologue()
{
    ...
    TR::SymbolReference *recompileMethodSymRef
        = cg()->symRefTab()->findOrCreateRuntimeHelper(TR_PPCsamplingRecompileMethod);
    ...
    if (useSampling() && couldBeCompiledAgain()) {
        ...
        cursor = generateDepImmSymInstruction(cg(), TR::InstOpCode::bl, firstNode,
        ...
        if (comp->target().is64Bit()) {
            ...
            cursor = generateImmInstruction(cg(), TR::InstOpCode::dd, firstNode, (int32_t)(intptr_t)info,
    ...
    if (couldBeCompiledAgain()) {
        ...
        if (comp->target().is64Bit()) {
            ...
            cursor = generateTrg1ImmInstruction(cg(), TR::InstOpCode::lis, firstNode, gr11,
                (int16_t)(adjustedAddr >> 32), cursor);
            ...
            cursor = generateTrg1Src1Imm2Instruction(cg(), TR::InstOpCode::rldicr, firstNode, gr11, gr11, 32,
            ...
            cursor = generateTrg1Src1ImmInstruction(cg(), TR::InstOpCode::oris, firstNode, gr11, gr11,
                (adjustedAddr & 0x0000FFFF), cursor);
        } else {
            cursor = generateTrg1ImmInstruction(cg(), TR::InstOpCode::lis, firstNode, gr11, (int16_t)HI_VALUE(addr),
                cursor);
        }

        // lwzu  gr0, last 16-bits(gr11)
        cursor
            = generateTrg1MemInstruction(cg(), (!isProfilingCompilation()) ? TR::InstOpCode::lwzu : TR::InstOpCode::lwz,
                firstNode, gr0, TR::MemoryReference::createWithDisplacement(cg(), gr11, LO_VALUE(addr), 4), cursor);
        ...
        if (!isProfilingCompilation()) {
            ...
            TR_ASSERT(comp->getProfilingMode() == JitProfiling,
                "JProfiling should not use counting mechanism to trip recompilation");
        ...
        TR::Snippet *snippet = new (cg()->trHeapMemory())
            TR::PPCRecompilationSnippet(snippetLabel, cursor->getPPCConditionalBranchInstruction(), cg());
