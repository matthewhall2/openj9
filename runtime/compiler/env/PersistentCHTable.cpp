...
void TR_PersistentCHTable::commitSideEffectGuards(TR::Compilation *comp)
{
    if (!isActive()) {
        TR_ASSERT_FATAL(comp->getClassesThatShouldNotBeLoaded()->isEmpty(),
            "Classes that should not be loaded is not empty!");
        TR_ASSERT_FATAL(comp->getClassesThatShouldNotBeNewlyExtended()->isEmpty(),
            "Classes that should not be newly extended is not empty!");
    ...
    for (TR_ClassLoadCheck *clc = comp->getClassesThatShouldNotBeLoaded()->getFirst(); clc; clc = clc->getNext()) {
        TR_OpaqueClassBlock *clazz
            = comp->fej9()->getClassFromSignature(clc->_name, clc->_length, comp->getCurrentMethod());
        if (clazz) {
        ...
        for (TR_ClassExtendCheck *cec = comp->getClassesThatShouldNotBeNewlyExtended()->getFirst(); cec;
            ...
            if (comp->fe()->classHasBeenExtended(clazz)) {
                ...
                for (info = it.getFirst(); info; info = it.getNext()) {
                    ...
                    for (TR_ClassExtendCheck *currCec = comp->getClassesThatShouldNotBeNewlyExtended()->getFirst();
                         currCec; currCec = currCec->getNext()) {
    ...
    if (nopAssumptionIsValid) {
        for (TR_ClassLoadCheck *clc = comp->getClassesThatShouldNotBeLoaded()->getFirst(); clc; clc = clc->getNext()) {
            for (auto site = sideEffectPatchSites->begin(); site != sideEffectPatchSites->end(); ++site) {
                ...
                TR_PatchNOPedGuardSiteOnClassPreInitialize::make(comp->fe(), comp->trPersistentMemory(), clc->_name,
                    clc->_length, (*site)->getLocation(), (*site)->getDestination(), comp->getMetadataAssumptionList());
...
public:
    FindSingleJittedImplementer(TR::Compilation *comp, TR_OpaqueClassBlock *topClassId, TR_ResolvedMethod *callerMethod,
        int32_t slotOrIndex)
...
bool FindSingleJittedImplementer::visitSubclass(TR_PersistentClassInfo *cl)
{
...
TR_ResolvedMethod *TR_PersistentCHTable::findSingleJittedImplementer(TR_OpaqueClassBlock *thisClass, int32_t vftSlot,
    TR_ResolvedMethod *callerMethod, TR::Compilation *comp, TR::ResolvedMethodSymbol *calleeSymbol, bool locked)
{
 ...
 */
TR_PersistentClassInfo *TR_PersistentCHTable::findClassInfo(TR_OpaqueClassBlock *classId)
{
    if (!isAccessible())
        return NULL;

    TR_PersistentClassInfo *cl
        = _classes[TR_RuntimeAssumptionTable::hashCode((uintptr_t)classId) % CLASSHASHTABLE_SIZE].getFirst();
    while (cl && cl->getClassId() != classId)
        cl = cl->getNext();
    return cl;
}
 ...
 */
TR_PersistentClassInfo *TR_PersistentCHTable::findClassInfoAfterLocking(TR_OpaqueClassBlock *classId,
    TR::Compilation *comp, bool returnClassInfoForAOT)
{
    if (!isActive())
        return NULL;
 ...
 * The class table lock is used to synchronize use of this method
 */
TR_PersistentClassInfo *TR_PersistentCHTable::findClassInfoAfterLocking(TR_OpaqueClassBlock *classId, TR_FrontEnd *fe,
...
bool TR_PersistentCHTable::isOverriddenInThisHierarchy(TR_ResolvedMethod *method, TR_OpaqueClassBlock *thisClass,
    int32_t vftSlot, TR::Compilation *comp, bool locked)
{
    if (comp->getOption(TR_DisableCHOpts))
    ...
    if (debug("traceOverriddenInHierarchy")) {
        ...
        printf("offset %d\n", vftSlot);
        int32_t len;
        char *s = TR::Compiler->cls.classNameChars(comp, thisClass, len);
        printf("thisClass %.*s\n", len, s);
    }
    ...
    for (classInfo = i.getFirst(); classInfo; classInfo = i.getNext()) {
        if (debug("traceOverriddenInHierarchy")) {
            int32_t len;
            char *s = TR::Compiler->cls.classNameChars(comp, classInfo->getClassId(), len);
            printf("leaf %.*s\n", len, s);
        }
...
TR_ResolvedMethod *TR_PersistentCHTable::findSingleImplementer(TR_OpaqueClassBlock *thisClass, int32_t cpIndexOrVftSlot,
    TR_ResolvedMethod *callerMethod, TR::Compilation *comp, bool locked, TR_YesNoMaybe useGetResolvedInterfaceMethod,
    bool validate)
{
    if (comp->getOption(TR_DisableCHOpts))
        return 0;
    ...
    TR_PersistentClassInfo *classInfo
        = comp->getPersistentInfo()->getPersistentCHTable()->findClassInfoAfterLocking(thisClass, comp, true);
    if (!classInfo) {
        return 0;
    }
    ...
    int32_t implCount = TR_ClassQueries::collectImplementorsCapped(classInfo, implArray, 2, cpIndexOrVftSlot,
        callerMethod, comp, locked, useGetResolvedInterfaceMethod);
    ...
    if (implementer && comp->getOption(TR_UseSymbolValidationManager) && validate) {
        ...
        bool validated = svm->addMethodFromSingleImplementerRecord(implementer->getPersistentIdentifier(), thisClass,
            cpIndexOrVftSlot, callerMethod->getPersistentIdentifier(), useGetResolvedInterfaceMethod);
        if (validated)
...
TR_ResolvedMethod *TR_PersistentCHTable::findSingleInterfaceImplementer(TR_OpaqueClassBlock *thisClass, int32_t cpIndex,
    TR_ResolvedMethod *callerMethod, TR::Compilation *comp, bool locked, bool validate)
{
    if (comp->getOption(TR_DisableCHOpts))
        return 0;
    ...
    comp->enterHeuristicRegion();
    int32_t implCount
        = TR_ClassQueries::collectImplementorsCapped(classInfo, implArray, 2, cpIndex, callerMethod, comp, locked);
    comp->exitHeuristicRegion();
    ...
    if (implementer && comp->getOption(TR_UseSymbolValidationManager) && validate) {
        ...
        bool validated = svm->addMethodFromSingleInterfaceImplementerRecord(implementer->getPersistentIdentifier(),
            thisClass, cpIndex, callerMethod->getPersistentIdentifier());
...
bool TR_PersistentCHTable::hasThreeOrMoreCompiledImplementors(TR_OpaqueClassBlock *thisClass, int32_t cpIndex,
    TR_ResolvedMethod *callerMethod, TR::Compilation *comp, TR_Hotness hotness, bool locked)
{
    if (comp->getOption(TR_DisableCHOpts))
        return false;
    ...
    return TR_ClassQueries::collectCompiledImplementorsCapped(classInfo, implArray, 3, cpIndex, callerMethod, comp,
               hotness, locked)
        == 3;
}

int32_t TR_PersistentCHTable::findnInterfaceImplementers(TR_OpaqueClassBlock *thisClass, int32_t n,
    TR_ResolvedMethod *implArray[], int32_t cpIndex, TR_ResolvedMethod *callerMethod, TR::Compilation *comp,
    bool locked)
{
    if (comp->getOption(TR_DisableCHOpts))
        return 0;
    ...
    int32_t implCount
        = TR_ClassQueries::collectImplementorsCapped(classInfo, implArray, n, cpIndex, callerMethod, comp, locked);
    return implCount;
}

bool TR_PersistentCHTable::isKnownToHaveMoreThanTwoInterfaceImplementers(TR_OpaqueClassBlock *thisClass,
    int32_t cpIndex, TR_ResolvedMethod *callerMethod, TR::Compilation *comp, bool locked)
{
    if (comp->getOption(TR_DisableCHOpts))
    ...
    int32_t implCount
        = TR_ClassQueries::collectImplementorsCapped(classInfo, implArray, 3, cpIndex, callerMethod, comp, locked);
    return (implCount == 3);
}

TR_ResolvedMethod *TR_PersistentCHTable::findSingleAbstractImplementer(TR_OpaqueClassBlock *thisClass, int32_t vftSlot,
    TR_ResolvedMethod *callerMethod, TR::Compilation *comp, bool locked, bool validate)
{
    if (comp->getOption(TR_DisableCHOpts))
        return 0;
    ...
    comp->enterHeuristicRegion();
    int32_t implCount
        = TR_ClassQueries::collectImplementorsCapped(classInfo, implArray, 2, vftSlot, callerMethod, comp, locked);
    comp->exitHeuristicRegion();
    ...
    if (implementer && comp->getOption(TR_UseSymbolValidationManager) && validate) {
        ...
        bool validated = svm->addMethodFromSingleAbstractImplementerRecord(implementer->getPersistentIdentifier(),
            thisClass, vftSlot, callerMethod->getPersistentIdentifier());
...
TR_OpaqueClassBlock *TR_PersistentCHTable::findSingleConcreteSubClass(TR_OpaqueClassBlock *opaqueClass,
    TR::Compilation *comp, bool validate)
{
    ...
    TR_PersistentClassInfo *classInfo
        = comp->getPersistentInfo()->getPersistentCHTable()->findClassInfoAfterLocking(opaqueClass, comp, allowForAOT);
    if (classInfo) {
        TR_ScratchList<TR_PersistentClassInfo> subClasses(comp->trMemory());
...
#ifdef DEBUG
void TR_PersistentCHTable::dumpStats(TR_FrontEnd *fe) {}
#endif

void TR_PersistentCHTable::dumpMethodCounts(TR_FrontEnd *fe, TR_Memory &trMemory)
{
...
void TR_PersistentCHTable::resetVisitedClasses() // highly time consuming
{
...
void TR_PersistentCHTable::classGotUnloaded(TR_FrontEnd *fe, TR_OpaqueClassBlock *classId)
{
    TR_ASSERT_FATAL(isActive(), "Should not be called if table is not active!");
...
TR_PersistentClassInfo *TR_PersistentCHTable::classGotLoaded(TR_FrontEnd *fe, TR_OpaqueClassBlock *classId)
{
    TR_ASSERT_FATAL(isAccessible(), "Should not be called if table is not accessible!");
...
void TR_PersistentCHTable::collectAllSubClasses(TR_PersistentClassInfo *clazz, ClassList &classList, TR_J9VMBase *fej9,
    bool locked)
{
    TR_ASSERT_FATAL(isActive(), "Should not be called if table is not active!");
...
void TR_PersistentCHTable::collectAllSubClassesLocked(TR_PersistentClassInfo *clazz, ClassList &classList,
    VisitTracker<> &marked)
{
    TR_ASSERT_FATAL(isActive(), "Should not be called if table is not active!");
...
void TR_PersistentCHTable::resetCachedCCVResult(TR_J9VMBase *fej9, TR_OpaqueClassBlock *clazz)
{
    TR_ASSERT_FATAL(isActive(), "Should not be called if table is not active!");
...
bool TR_PersistentCHTable::addClassToTable(J9VMThread *vmThread, J9JITConfig *jitConfig, J9Class *j9clazz,
...
bool TR_PersistentCHTable::activate(J9VMThread *vmThread, TR_J9VMBase *fej9, TR::CompilationInfo *compInfo)
{
