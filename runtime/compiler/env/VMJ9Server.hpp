...
public:
    ...
    virtual TR_ResolvedMethod *createResolvedMethod(TR_Memory *trMemory, TR_OpaqueMethodBlock *aMethod,
        TR_ResolvedMethod *owningMethod, TR_OpaqueClassBlock *classForNewInstance) override;
    TR_ResolvedMethod *createResolvedMethod(TR_Memory *trMemory, TR_OpaqueMethodBlock *aMethod,
        ...
        TR_OpaqueClassBlock *classForNewInstance = NULL);
    virtual TR_ResolvedMethod *createResolvedMethodWithSignature(TR_Memory *trMemory, TR_OpaqueMethodBlock *aMethod,
        TR_OpaqueClassBlock *classForNewInstance, char *signature, int32_t signatureLength,
        TR_ResolvedMethod *owningMethod, uint32_t = 0) override;
    TR_ResolvedMethod *createResolvedMethodWithSignature(TR_Memory *trMemory, TR_OpaqueMethodBlock *aMethod,
        TR_OpaqueClassBlock *classForNewInstance, char *signature, int32_t signatureLength,
        TR_ResolvedMethod *owningMethod, const TR_ResolvedJ9JITServerMethodInfo &methodInfo);
    virtual TR_YesNoMaybe isInstanceOf(TR_OpaqueClassBlock *a, TR_OpaqueClassBlock *b, bool objectTypeIsFixed,
        bool castTypeIsFixed = true, bool optimizeForAOT = false) override;
    virtual TR_OpaqueClassBlock *getSystemClassFromClassName(const char *name, int32_t length,
        bool isVettedForAOT = false) override;
    ...
    virtual bool jitFieldsAreSame(TR_ResolvedMethod *method1, I_32 cpIndex1, TR_ResolvedMethod *method2, I_32 cpIndex2,
        int32_t isStatic) override;
    ...
    virtual bool ensureOSRBufferSize(TR::Compilation *comp, uintptr_t osrFrameSizeInBytes,
        uintptr_t osrScratchBufferSizeInBytes, uintptr_t osrStackFrameSizeInBytes) override;
     ...
     */
    virtual TR_OpaqueClassBlock *getNullRestrictedArrayClassFromComponentClass(
        TR_OpaqueClassBlock *componentClass) override;
    ...
    virtual bool scanReferenceSlotsInClassForOffset(TR::Compilation *comp, TR_OpaqueClassBlock *clazz,
    ...
    virtual TR_OpaqueMethodBlock *getResolvedVirtualMethod(TR_OpaqueClassBlock *classObject, int32_t cpIndex,
        bool ignoreReResolve = true) override;
    ...
    virtual uint8_t *allocateCodeMemory(TR::Compilation *comp, uint32_t warmCodeSize, uint32_t coldCodeSize,
        uint8_t **coldCode, bool isMethodHeaderNeeded) override;
    ...
    virtual void *setJ2IThunk(char *signatureChars, uint32_t signatureLength, void *thunkptr,
        TR::Compilation *comp) override;
    virtual uint32_t getInstanceFieldOffset(TR_OpaqueClassBlock *clazz, const char *fieldName, uint32_t fieldLen,
        const char *sig, uint32_t sigLen, UDATA options) override;
    virtual int32_t getJavaLangClassHashCode(TR::Compilation *comp, TR_OpaqueClassBlock *clazz,
        bool &hashCodeComputed) override;
    ...
    virtual TR_OpaqueMethodBlock *getMethodFromClass(TR_OpaqueClassBlock *methodClass, const char *methodName,
        const char *signature, TR_OpaqueClassBlock *callingClass) override;
    ...
    virtual void markClassForTenuredAlignment(TR::Compilation *comp, TR_OpaqueClassBlock *clazz,
        uint32_t alignFromStart) override;
    virtual void reportHotField(int32_t reducedCpuUtil, J9Class *clazz, uint8_t fieldOffset,
        uint32_t reducedFrequency) override;
    ...
    virtual void *getStaticFieldAddress(TR_OpaqueClassBlock *clazz, unsigned char *fieldName, uint32_t fieldLen,
        unsigned char *sig, uint32_t sigLen) override;
    ...
    virtual TR_ResolvedMethod *createMethodHandleArchetypeSpecimen(TR_Memory *trMemory, uintptr_t *methodHandleLocation,
        TR_ResolvedMethod *owningMethod = 0) override;
    virtual TR_ResolvedMethod *createMethodHandleArchetypeSpecimen(TR_Memory *trMemory, TR_OpaqueMethodBlock *archetype,
        uintptr_t *methodHandleLocation, TR_ResolvedMethod *owningMethod = 0) override;
    virtual TR::KnownObjectTable::Index mutableCallSiteEpoch(TR::Compilation *comp,
        TR::KnownObjectTable::Index mcs) override;
    ...
    //   virtual bool canDereferenceAtCompileTime(TR::SymbolReference *fieldRef,  TR::Compilation *comp) override; //
    ...
    virtual void reserveTrampolineIfNecessary(TR::Compilation *, TR::SymbolReference *symRef,
        bool inBinaryEncoding) override;
    ...
    virtual TR_OpaqueMethodBlock *targetMethodFromMemberName(TR::Compilation *comp,
        TR::KnownObjectTable::Index objIndex) override;
    ...
    virtual TR_ResolvedMethod *targetMethodFromInvokeCacheArrayMemberNameObj(TR::Compilation *comp,
        TR_ResolvedMethod *owningMethod, uintptr_t *invokeCacheArray) override;
    virtual TR::KnownObjectTable::Index getKnotIndexOfInvokeCacheArrayAppendixElement(TR::Compilation *comp,
        uintptr_t *invokeCacheArray) override;
    ...
    virtual bool getMemberNameMethodInfo(TR::Compilation *comp, TR::KnownObjectTable::Index objIndex,
        MemberNameMethodInfo *out) override;
    virtual TR::KnownObjectTable::Index delegatingMethodHandleTargetHelper(TR::Compilation *comp,
        TR::KnownObjectTable::Index dmhIndex, TR_OpaqueClassBlock *cwClass) override;
    virtual UDATA getVMTargetOffset() override;
    virtual UDATA getVMIndexOffset() override;
    virtual int32_t getVarHandleAccessDescriptorMode(TR::Compilation *comp,
        TR::KnownObjectTable::Index adIndex) override;
    virtual TR::KnownObjectTable::Index getMethodHandleTableEntryIndex(TR::Compilation *comp,
        TR::KnownObjectTable::Index vhIndex, TR::KnownObjectTable::Index adIndex) override;
    virtual TR::KnownObjectTable::Index getLayoutVarHandle(TR::Compilation *comp,
        TR::KnownObjectTable::Index layoutIndex) override;
    ...
    virtual OMR::KnownObjectTable::Index getConvertedMethodhandle(TR::Compilation *comp, TR::KnownObjectTable::Index mhIndex,
        TR::KnownObjectTable::Index desiredTypeIndex) override;
    ...
    void addBondMethodsFromClient(TR::Compilation *comp, const std::vector<TR_ClientBondMethod> &methods);
...
private:
    ...
    void *sendJ2IThunkToClient(const std::string &signature, const uint8_t *thunkStart, const uint32_t thunkSize,
        TR::Compilation *comp);
...
public:
    ...
    virtual TR_OpaqueMethodBlock *getMethodFromClass(TR_OpaqueClassBlock *, const char *, const char *,
        TR_OpaqueClassBlock * = NULL) override;
    ...
    virtual TR_YesNoMaybe isInstanceOf(TR_OpaqueClassBlock *instanceClass, TR_OpaqueClassBlock *castClass,
        bool instanceIsFixed, bool castIsFixed = true, bool optimizeForAOT = false) override;
    virtual TR_OpaqueClassBlock *getClassFromSignature(const char *sig, int32_t length, J9ConstantPool *constantPool,
        bool isVettedForAOT = false) override;
    virtual TR_OpaqueClassBlock *getSystemClassFromClassName(const char *name, int32_t length,
        bool isVettedForAOT = false) override;
    ...
    virtual intptr_t methodTrampolineLookup(TR::Compilation *, TR::SymbolReference *symRef, void *callSite) override
    {
        TR_ASSERT_FATAL(0, "methodTrampolineLookup not implemented for AOT");
        return 0;
    }

    virtual TR::CodeCache *getDesignatedCodeCache(TR::Compilation *comp) override;

    virtual void *persistMHJ2IThunk(void *thunk) override
    {
        TR_ASSERT_FATAL(0, "persistMHJ2IThunk should not be called on the server");
        return NULL;
    }

    virtual void *findPersistentMHJ2IThunk(char *signatureChars) override
    {
        TR_ASSERT_FATAL(0, "findPersistentMHJ2IThunk should not be called on the server");
        return NULL;
    }
    ...
    virtual TR_OpaqueMethodBlock *getResolvedInterfaceMethod(TR_OpaqueMethodBlock *ownerMethod,
        TR_OpaqueClassBlock *classObject, int32_t cpIndex) override;
};
