...
void J9::ObjectModel::initialize()
{
    ...
    // Compressed refs
    uintptr_t result
        = mmf->j9gc_modron_getConfigurationValueForKey(vm, j9gc_modron_configuration_compressObjectReferences, &value);
    if (result == 1 && value == 1)
    ...
    //
    result = mmf->j9gc_modron_getConfigurationValueForKey(vm, j9gc_modron_configuration_discontiguousArraylets, &value);
...
bool J9::ObjectModel::areValueTypesEnabled()
{
#if defined(J9VM_OPT_JITSERVER)
...
bool J9::ObjectModel::areFlattenableValueTypesEnabled()
{
#if defined(J9VM_OPT_JITSERVER)
    if (auto stream = TR::CompilationInfo::getStream()) {
...
bool J9::ObjectModel::areValueBasedMonitorChecksEnabled()
{
    ...
    if (auto stream = TR::CompilationInfo::getStream()) {
        ...
        return J9_ARE_ANY_BITS_SET(vmInfo->_extendedRuntimeFlags2,
            J9_EXTENDED_RUNTIME2_VALUE_BASED_EXCEPTION | J9_EXTENDED_RUNTIME2_VALUE_BASED_WARNING);
    }
#endif /* defined(J9VM_OPT_JITSERVER) */
...
bool J9::ObjectModel::isValueTypeArrayFlatteningEnabled()
{
#if defined(J9VM_OPT_JITSERVER)
    if (auto stream = TR::CompilationInfo::getStream()) {
#if defined(J9VM_OPT_VALHALLA_FLATTENABLE_VALUE_TYPES)
...
bool J9::ObjectModel::isHotReferenceFieldRequired()
{
#if defined(J9VM_OPT_JITSERVER)
    if (auto stream = TR::CompilationInfo::getStream()) {
        auto *vmInfo = TR::compInfoPT->getClientData()->getOrCacheVMInfo(stream);
...
bool J9::ObjectModel::isOffHeapAllocationEnabled()
{
#if defined(J9VM_OPT_JITSERVER)
    if (auto stream = TR::CompilationInfo::getStream()) {
        auto *vmInfo = TR::compInfoPT->getClientData()->getOrCacheVMInfo(stream);
...
UDATA
J9::ObjectModel::elementSizeOfBooleanArray() { return 1; }

uint32_t J9::ObjectModel::getSizeOfArrayElement(TR::Node *node)
{
    TR_ASSERT(node->getOpCodeValue() == TR::newarray || node->getOpCodeValue() == TR::anewarray,
        "getSizeOfArrayElement expects either newarray or anewarray at [%p]", node);
...
int64_t J9::ObjectModel::maxArraySizeInElementsForAllocation(TR::Node *newArray, TR::Compilation *comp)
{
...
int64_t J9::ObjectModel::maxArraySizeInElements(int32_t knownMinElementSize, TR::Compilation *comp)
{
...
bool J9::ObjectModel::nativeAddressesCanChangeSize()
{
...
bool J9::ObjectModel::generateCompressedObjectHeaders() { return compressObjectReferences(); }

uintptr_t J9::ObjectModel::contiguousArrayHeaderSizeInBytes()
{
#if defined(J9VM_OPT_JITSERVER)
    if (auto stream = TR::CompilationInfo::getStream()) {
        auto *vmInfo = TR::compInfoPT->getClientData()->getOrCacheVMInfo(stream);
...
uintptr_t J9::ObjectModel::discontiguousArrayHeaderSizeInBytes()
{
#if defined(J9VM_OPT_JITSERVER)
    if (auto stream = TR::CompilationInfo::getStream()) {
        auto *vmInfo = TR::compInfoPT->getClientData()->getOrCacheVMInfo(stream);
...
//
int32_t J9::ObjectModel::maxContiguousArraySizeInBytes()
{
...
//
bool J9::ObjectModel::isDiscontiguousArray(int32_t sizeInBytes)
{
...
//
bool J9::ObjectModel::isDiscontiguousArray(int32_t sizeInElements, int32_t elementSize)
{
...
int32_t J9::ObjectModel::compressedReferenceShiftOffset()
{
...
int32_t J9::ObjectModel::compressedReferenceShift()
{
#if defined(J9VM_OPT_JITSERVER)
    if (auto stream = TR::CompilationInfo::getStream()) {
        auto *vmInfo = TR::compInfoPT->getClientData()->getOrCacheVMInfo(stream);
...
uintptr_t J9::ObjectModel::offsetOfObjectVftField() { return TMP_OFFSETOF_J9OBJECT_CLAZZ; }

uintptr_t J9::ObjectModel::offsetOfHeaderFlags()
{
...
uintptr_t J9::ObjectModel::maskOfObjectVftField()
{
...
//
bool J9::ObjectModel::mayRequireSpineChecks() { return TR::Compiler->om.useHybridArraylets(); }

int32_t J9::ObjectModel::arraySpineShift(int32_t width)
{
...
int32_t J9::ObjectModel::arrayletMask(int32_t width)
{
    TR_ASSERT(TR::Compiler->om.canGenerateArraylets(), "not supposed to be generating arraylets!");
    TR_ASSERT(width >= 0, "unexpected arraylet datatype width");
...
int32_t J9::ObjectModel::arrayletLeafIndex(int32_t index, int32_t elementSize)
{
    TR_ASSERT(TR::Compiler->om.canGenerateArraylets(), "not supposed to be generating arraylets!");
...
int32_t J9::ObjectModel::objectAlignmentInBytes()
{
    J9JavaVM *javaVM = TR::Compiler->javaVM;
    if (!javaVM)
        return 0;
    ...
    J9MemoryManagerFunctions *mmf = javaVM->memoryManagerFunctions;
    uintptr_t result = 0;
    result = mmf->j9gc_modron_getConfigurationValueForKey(javaVM, j9gc_modron_configuration_objectAlignment, &result)
        ? result
        : 0;
    return (int32_t)result;
}

uintptr_t J9::ObjectModel::offsetOfContiguousArraySizeField()
{
    return compressObjectReferences() ? offsetof(J9IndexableObjectContiguousCompressed, size)
                                      : offsetof(J9IndexableObjectContiguousFull, size);
}

uintptr_t J9::ObjectModel::offsetOfDiscontiguousArraySizeField()
{
    return compressObjectReferences() ? offsetof(J9IndexableObjectDiscontiguousCompressed, size)
                                      : offsetof(J9IndexableObjectDiscontiguousFull, size);
}

#if defined(J9VM_ENV_DATA64)
uintptr_t J9::ObjectModel::offsetOfContiguousDataAddrField()
{
    return compressObjectReferences() ? offsetof(J9IndexableObjectWithDataAddressContiguousCompressed, dataAddr)
                                      : offsetof(J9IndexableObjectWithDataAddressContiguousFull, dataAddr);
}

uintptr_t J9::ObjectModel::offsetOfDiscontiguousDataAddrField()
{
    return compressObjectReferences() ? offsetof(J9IndexableObjectWithDataAddressDiscontiguousCompressed, dataAddr)
                                      : offsetof(J9IndexableObjectWithDataAddressDiscontiguousFull, dataAddr);
}
#endif /* defined(J9VM_ENV_DATA64) */

uintptr_t J9::ObjectModel::objectHeaderSizeInBytes()
{
    return compressObjectReferences() ? sizeof(J9ObjectCompressed) : sizeof(J9ObjectFull);
}

uintptr_t J9::ObjectModel::offsetOfIndexableSizeField() { return offsetof(J9ROMArrayClass, arrayShape); }

bool J9::ObjectModel::isDiscontiguousArray(TR::Compilation *comp, uintptr_t objectPointer)
{
    TR_ASSERT(TR::Compiler->vm.hasAccess(comp), "isDicontiguousArray requires VM access");
    TR_ASSERT(TR::Compiler->cls.isClassArray(comp, TR::Compiler->cls.objectClass(comp, (objectPointer))),
        "Object is not an array");

    int32_t length = *(int32_t *)(objectPointer + TR::Compiler->om.offsetOfContiguousArraySizeField());

    if (TR::Compiler->om.canGenerateArraylets() && length == 0)
        return true;
 ...
 */
uintptr_t J9::ObjectModel::getAddressOfElement(TR::Compilation *comp, uintptr_t objectPointer, int64_t offset)
{
    ...
    TR_ASSERT(offset >= TR::Compiler->om.contiguousArrayHeaderSizeInBytes()
            && offset < TR::Compiler->om.getArrayLengthInBytes(comp, objectPointer)
...
uintptr_t J9::ObjectModel::getArrayElementWidthInBytes(TR::DataType type)
{
    if (type == TR::Address)
...
uintptr_t J9::ObjectModel::getArrayElementWidthInBytes(TR::Compilation *comp, uintptr_t objectPointer)
{
...
uintptr_t J9::ObjectModel::getArrayLengthInBytes(TR::Compilation *comp, uintptr_t objectPointer)
{
    TR_ASSERT(TR::Compiler->vm.hasAccess(comp), "Must haveAccess in getArrayLengthInBytes");
    return (uintptr_t)TR::Compiler->om.getArrayLengthInElements(comp, objectPointer)
        * TR::Compiler->om.getArrayElementWidthInBytes(comp, objectPointer);
}

intptr_t J9::ObjectModel::getArrayLengthInElements(TR::Compilation *comp, uintptr_t objectPointer)
{
    return comp->fej9()->getArrayLengthInElements(objectPointer);
}

uintptr_t J9::ObjectModel::decompressReference(TR::Compilation *comp, uintptr_t compressedReference)
{
    return (compressedReference << TR::Compiler->om.compressedReferenceShift());
}

bool J9::ObjectModel::usesDiscontiguousArraylets()
{
#if defined(J9VM_OPT_JITSERVER)
    if (auto stream = TR::CompilationInfo::getStream()) {
        auto *vmInfo = TR::compInfoPT->getClientData()->getOrCacheVMInfo(stream);
...
int32_t J9::ObjectModel::arrayletLeafSize()
{
#if defined(J9VM_OPT_JITSERVER)
    if (auto stream = TR::CompilationInfo::getStream()) {
        auto *vmInfo = TR::compInfoPT->getClientData()->getOrCacheVMInfo(stream);
...
int32_t J9::ObjectModel::arrayletLeafLogSize()
{
#if defined(J9VM_OPT_JITSERVER)
    if (auto stream = TR::CompilationInfo::getStream()) {
        auto *vmInfo = TR::compInfoPT->getClientData()->getOrCacheVMInfo(stream);
 ...
 * @return true if isIndexableDualHeaderShapeEnabled is false OR if option -Xgcpolicy:balanced is specified at runtime,
 * false otherwise
 */
bool J9::ObjectModel::isIndexableDataAddrPresent()
{
#if defined(J9VM_OPT_JITSERVER)
    if (auto stream = TR::CompilationInfo::getStream()) {
        auto *vmInfo = TR::compInfoPT->getClientData()->getOrCacheVMInfo(stream);
...
MM_GCReadBarrierType J9::ObjectModel::readBarrierType()
{
#if defined(J9VM_OPT_JITSERVER)
    if (auto stream = TR::CompilationInfo::getStream()) {
        auto *vmInfo = TR::compInfoPT->getClientData()->getOrCacheVMInfo(stream);
...
MM_GCWriteBarrierType J9::ObjectModel::writeBarrierType()
{
#if defined(J9VM_OPT_JITSERVER)
    if (auto stream = TR::CompilationInfo::getStream()) {
        auto *vmInfo = TR::compInfoPT->getClientData()->getOrCacheVMInfo(stream);
...
bool J9::ObjectModel::compressObjectReferences()
{
#if defined(J9VM_OPT_JITSERVER)
    if (auto stream = TR::CompilationInfo::getStream()) {
        auto *vmInfo = TR::compInfoPT->getClientData()->getOrCacheVMInfo(stream);
...
int32_t J9::ObjectModel::getObjectAlignmentInBytes()
{
#if defined(J9VM_OPT_JITSERVER)
    if (auto stream = TR::CompilationInfo::getStream()) {
        auto *vmInfo = TR::compInfoPT->getClientData()->getOrCacheVMInfo(stream);
...
int32_t J9::ObjectModel::getTwoDimensionalArrayComponentSize(TR::Node *classNode)
{
