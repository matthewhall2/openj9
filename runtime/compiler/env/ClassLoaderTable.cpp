...
struct TR_ClassLoaderInfo {
    ...
    TR_ClassLoaderInfo(void *loader, void *chain, J9UTF8 *nameStr)
        : _loader(loader)
        , _loaderTableNext(NULL)
        , _chain(chain)
        , _chainTableNext(NULL)
...
#if defined(J9VM_OPT_JITSERVER)
    const J9UTF8 *name() const { return _nameStr; }
#endif /* defined(J9VM_OPT_JITSERVER) */
...
static J9UTF8 *copyJ9UTF8(const J9UTF8 *nameStr, TR_PersistentMemory *persistentMemory)
{
...
template<TableKind T>
static TR_ClassLoaderInfo *lookup(TR_ClassLoaderInfo * const *table, size_t &index, TR_ClassLoaderInfo *&prev,
    const void *key)
{
...
template<TableKind T> static void insert(TR_ClassLoaderInfo *info, TR_ClassLoaderInfo **table, size_t index)
{
...
template<TableKind T>
static void remove(TR_ClassLoaderInfo *info, TR_ClassLoaderInfo *prev, TR_ClassLoaderInfo **table, size_t index)
{
    if (prev)
...
template<> bool TR_ClassLoaderInfo::equals<Name>(const void *keyPtr) const
{
...
template<> size_t hash<Name>(const void *keyPtr)
{
    auto key = (const NameKey *)keyPtr;
    size_t h = 0;
...
static bool hasSharedVMAccess(J9VMThread *vmThread)
{
    return (vmThread->publicFlags & J9_PUBLIC_FLAGS_VM_ACCESS) && !vmThread->omrVMThread->exclusiveCount;
}

void TR_PersistentClassLoaderTable::associateClassLoaderWithClass(J9VMThread *vmThread, void *loader,
    TR_OpaqueClassBlock *clazz)
{
        ...
        if (TR_SharedCache::INVALID_CLASS_CHAIN_OFFSET == chainOffset) {
            ...
            if (useAOTCache && TR::Options::getVerboseOption(TR_VerboseJITServer))
                TR_VerboseLog::writeLineLocked(TR_Vlog_JITServer,
                    "ERROR: Failed to get class chain for %.*s loaded by %p", nameLength, (const char *)name, loader);
#endif /* defined(J9VM_OPT_JITSERVER) */
        } else {
...
#if defined(J9VM_OPT_JITSERVER)
    if (useAOTCache) {
        nameStr = (_sharedCache && _sharedCache->isROMClassInSharedCache(romClass))
            ? romName
            : copyJ9UTF8(romName, _persistentMemory);
    }
#endif /* defined(J9VM_OPT_JITSERVER) */
    ...
    if (!info) {
        ...
        if (useAOTCache && TR::Options::getVerboseOption(TR_VerboseJITServer))
            TR_VerboseLog::writeLineLocked(TR_Vlog_JITServer,
                "ERROR: Failed to associate class %.*s chain %p with loader %p", nameLength, (const char *)name, chain,
                loader);
#endif /* defined(J9VM_OPT_JITSERVER) */
        return;
    }
    ...
    if (chain) {
        ...
        if (otherInfo) {
            ...
            if (useAOTCache && TR::Options::getVerboseOption(TR_VerboseJITServer))
                TR_VerboseLog::writeLineLocked(TR_Vlog_JITServer,
                    "ERROR: Class %.*s chain %p already associated with loader %p != %p", nameLength,
                    (const char *)name, chain, otherInfo->_loader, loader);
#endif /* defined(J9VM_OPT_JITSERVER) */
            return;
        }
    ...
    if (nameStr) {
        ...
        if (otherInfo) {
            ...
            if (TR::Options::getVerboseOption(TR_VerboseJITServer))
                TR_VerboseLog::writeLineLocked(TR_Vlog_JITServer,
                    "ERROR: Class name %.*s already associated with loader %p != %p", nameLength, (const char *)name,
                    otherInfo->_loader, loader);
            return;
        }
        insert<Name>(info, _nameTable, index);

        if (TR::Options::getVerboseOption(TR_VerboseJITServer))
            TR_VerboseLog::writeLineLocked(TR_Vlog_JITServer, "Associated class loader %p with class %.*s chain %p",
                loader, nameLength, (const char *)name, chain);
    }
#endif /* defined(J9VM_OPT_JITSERVER) */
}

static void assertCurrentThreadCanRead()
{
...
void *TR_PersistentClassLoaderTable::lookupClassChainAssociatedWithClassLoader(void *loader) const
{
...
void *TR_PersistentClassLoaderTable::lookupClassLoaderAssociatedWithClassChain(void *chain) const
{
    assertCurrentThreadCanRead();
    if (!_sharedCache)
        return NULL;
...
std::pair<void *, void *> TR_PersistentClassLoaderTable::lookupClassLoaderAndChainAssociatedWithClassName(
...
void *TR_PersistentClassLoaderTable::lookupClassLoaderAssociatedWithClassName(const uint8_t *data, size_t length) const
{
    assertCurrentThreadCanRead();
...
const J9UTF8 *TR_PersistentClassLoaderTable::lookupClassNameAssociatedWithClassLoader(void *loader) const
{
    assertCurrentThreadCanRead();
...
void TR_PersistentClassLoaderTable::removeClassLoader(J9VMThread *vmThread, void *loader)
{
    ...
    if (useAOTCache) {
        ...
        if (TR::Options::getVerboseOption(TR_VerboseJITServer))
            TR_VerboseLog::writeLineLocked(TR_Vlog_JITServer,
                "Removed class loader %p associated with class %.*s chain %p", loader, nameLength, (const char *)name,
                info->_chain);
...
void TR_PersistentClassLoaderTable::addPermanentLoader(J9VMThread *vmThread, J9ClassLoader *loader)
{
    TR_ASSERT_FATAL(loader->outlivingLoaders == J9CLASSLOADER_OUTLIVING_LOADERS_PERMANENT, "loader %p is not permanent",
        loader);
...
void TR_PersistentClassLoaderTable::getPermanentLoaders(J9VMThread *vmThread,
