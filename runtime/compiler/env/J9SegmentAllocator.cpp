...
SegmentAllocator::SegmentAllocator(int32_t segmentType, J9JavaVM &javaVM) throw()
    : _segmentType(segmentType)
    , _javaVM(javaVM)
{
    TR_ASSERT(((pageSize() & (pageSize() - 1)) == 0), "Page size is not a power of 2, %llu",
        static_cast<unsigned long long>(pageSize()));
}

SegmentAllocator::~SegmentAllocator() throw() {}

J9MemorySegment *SegmentAllocator::allocate(const size_t segmentSize, const std::nothrow_t &tag) throw()
{
    ...
    J9MemorySegment *newSegment
        = _javaVM.internalVMFunctions->allocateMemorySegment(&_javaVM, alignedSize, _segmentType, J9MEM_CATEGORY_JIT);
    TR_ASSERT(!newSegment || (newSegment->heapAlloc == newSegment->heapBase),
...
J9MemorySegment &SegmentAllocator::allocate(size_t const segmentSize)
{
...
void SegmentAllocator::deallocate(J9MemorySegment &unusedSegment) throw()
{
    _javaVM.internalVMFunctions->freeMemorySegment(&_javaVM, &unusedSegment, TRUE);
}

J9MemorySegment &SegmentAllocator::request(size_t segmentSize) { return allocate(segmentSize); }

void SegmentAllocator::release(J9MemorySegment &unusedSegment) throw() { deallocate(unusedSegment); }

size_t SegmentAllocator::pageSize() throw()
{
...
size_t SegmentAllocator::pageAlign(const size_t requestedSize) throw()
{
...
void SegmentAllocator::preventAllocationOfBTLMemory(J9MemorySegment *&segment, J9JavaVM *javaVM, int32_t segmentType)
{
#if defined(J9ZOS390)
    ...
    if (TR::Options::getCmdLineOptions()->getOption(TR_DontAllocateScratchBTL) && segment
        && ((uintptr_t)(segment->heapBase) < (uintptr_t)(1 << 24))) {
