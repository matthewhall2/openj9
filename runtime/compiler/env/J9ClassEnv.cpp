...
TR::ClassEnv *J9::ClassEnv::self() { return static_cast<TR::ClassEnv *>(this); }

J9Class *J9::ClassEnv::convertClassOffsetToClassPtr(TR_OpaqueClassBlock *clazzOffset)
{
...
TR_OpaqueClassBlock *J9::ClassEnv::convertClassPtrToClassOffset(J9Class *clazzPtr)
{
    // NOTE : We could pass down vmThread() in the call below if the conversion
    // required the VM thread. Currently it does not. If we did change that
    // such that the VM thread was reqd, we would need to handle AOT where the
...
bool J9::ClassEnv::isClassSpecialForStackAllocation(TR_OpaqueClassBlock *clazz)
{
    const UDATA mask = (J9AccClassReferenceWeak | J9AccClassReferenceSoft | J9AccClassFinalizeNeeded
        | J9AccClassOwnableSynchronizer | J9AccClassContinuation);

#if defined(J9VM_OPT_JITSERVER)
    if (auto stream = TR::CompilationInfo::getStream()) {
        uintptr_t classDepthAndFlags = 0;
        JITServerHelpers::getAndCacheRAMClassInfo((J9Class *)clazz, TR::compInfoPT->getClientData(), stream,
...
uintptr_t J9::ClassEnv::classFlagsValue(TR_OpaqueClassBlock *classPointer)
{
#if defined(J9VM_OPT_JITSERVER)
    if (auto stream = TR::CompilationInfo::getStream()) {
...
uintptr_t J9::ClassEnv::classFlagReservableWordInitValue(TR_OpaqueClassBlock *classPointer)
{
#if defined(J9VM_OPT_JITSERVER)
    if (auto stream = TR::CompilationInfo::getStream()) {
        uintptr_t classFlags = 0;
        JITServerHelpers::getAndCacheRAMClassInfo((J9Class *)classPointer, TR::compInfoPT->getClientData(), stream,
            JITServerHelpers::CLASSINFO_CLASS_FLAGS, (void *)&classFlags);
#ifdef DEBUG
        stream->write(JITServer::MessageType::ClassEnv_classFlagsValue, classPointer);
...
uintptr_t J9::ClassEnv::classDepthOf(TR_OpaqueClassBlock *clazzPointer)
{
#if defined(J9VM_OPT_JITSERVER)
    if (auto stream = TR::CompilationInfo::getStream()) {
        uintptr_t classDepthAndFlags = 0;
        JITServerHelpers::getAndCacheRAMClassInfo((J9Class *)clazzPointer, TR::compInfoPT->getClientData(), stream,
            JITServerHelpers::CLASSINFO_CLASS_DEPTH_AND_FLAGS, (void *)&classDepthAndFlags);
...
uintptr_t J9::ClassEnv::classInstanceSize(TR_OpaqueClassBlock *clazzPointer)
{
#if defined(J9VM_OPT_JITSERVER)
    if (auto stream = TR::CompilationInfo::getStream()) {
...
J9ROMClass *J9::ClassEnv::romClassOf(TR_OpaqueClassBlock *clazz)
{
...
J9Class **J9::ClassEnv::superClassesOf(TR_OpaqueClassBlock *clazz)
{
#if defined(J9VM_OPT_JITSERVER)
    if (auto stream = TR::CompilationInfo::getStream()) {
...
J9ROMClass *J9::ClassEnv::romClassOfSuperClass(TR_OpaqueClassBlock *clazz, size_t index)
{
#if defined(J9VM_OPT_JITSERVER)
    if (auto stream = TR::CompilationInfo::getStream()) {
...
J9ITable *J9::ClassEnv::iTableOf(TR_OpaqueClassBlock *clazz)
{
#if defined(J9VM_OPT_JITSERVER)
    if (auto stream = TR::CompilationInfo::getStream()) {
...
J9ITable *J9::ClassEnv::iTableNext(J9ITable *current)
{
#if defined(J9VM_OPT_JITSERVER)
    if (auto stream = TR::CompilationInfo::getStream()) {
...
J9ROMClass *J9::ClassEnv::iTableRomClass(J9ITable *current)
{
#if defined(J9VM_OPT_JITSERVER)
    if (auto stream = TR::CompilationInfo::getStream()) {
...
#if defined(J9VM_OPT_JITSERVER)
std::vector<TR_OpaqueClassBlock *> J9::ClassEnv::getITable(TR_OpaqueClassBlock *clazz)
{
    if (auto stream = TR::CompilationInfo::getStream()) {
...
bool J9::ClassEnv::isStringClass(TR_OpaqueClassBlock *clazz)
{
    // return (J9Class*)clazz == J9VMJAVALANGSTRING(jitConfig->javaVM);
    return false;
}

bool J9::ClassEnv::isAbstractClass(TR::Compilation *comp, TR_OpaqueClassBlock *clazzPointer)
{
    return comp->fej9()->isAbstractClass(clazzPointer);
}

bool J9::ClassEnv::isInterfaceClass(TR::Compilation *comp, TR_OpaqueClassBlock *clazzPointer)
{
    return comp->fej9()->isInterfaceClass(clazzPointer);
}

bool J9::ClassEnv::isConcreteClass(TR::Compilation *comp, TR_OpaqueClassBlock *clazzPointer)
{
    return comp->fej9()->isConcreteClass(clazzPointer);
}

bool J9::ClassEnv::isPrimitiveClass(TR::Compilation *comp, TR_OpaqueClassBlock *clazz)
{
    return comp->fej9()->isPrimitiveClass(clazz);
}

bool J9::ClassEnv::isPrimitiveArray(TR::Compilation *comp, TR_OpaqueClassBlock *clazz)
{
    return comp->fej9()->isPrimitiveArray(clazz);
}

TR::DataTypes J9::ClassEnv::primitiveArrayComponentType(TR::Compilation *comp, TR_OpaqueClassBlock *clazz)
{
    TR_ASSERT_FATAL(self()->isPrimitiveArray(comp, clazz), "not a primitive array: %p", clazz);
    ...
    for (int32_t i = 0; i < sizeof(newArrayTypes) / sizeof(newArrayTypes[0]); i++) {
        TR_OpaqueClassBlock *primitiveArrayClass = fej9->getClassFromNewArrayTypeNonNull(firstNewArrayTypeCode + i);
...
bool J9::ClassEnv::isReferenceArray(TR::Compilation *comp, TR_OpaqueClassBlock *clazz)
{
    return comp->fej9()->isReferenceArray(clazz);
}

bool J9::ClassEnv::isClassArray(TR::Compilation *comp, TR_OpaqueClassBlock *clazz)
{
    return comp->fej9()->isClassArray(clazz);
}

bool J9::ClassEnv::isClassFinal(TR::Compilation *comp, TR_OpaqueClassBlock *clazz)
{
    return comp->fej9()->isClassFinal(clazz);
}

bool J9::ClassEnv::hasFinalizer(TR::Compilation *comp, TR_OpaqueClassBlock *clazz)
{
    return comp->fej9()->hasFinalizer(clazz);
}

bool J9::ClassEnv::isClassInitialized(TR::Compilation *comp, TR_OpaqueClassBlock *clazz)
{
    return comp->fej9()->isClassInitialized(clazz);
}

bool J9::ClassEnv::isClassVisible(TR::Compilation *comp, TR_OpaqueClassBlock *sourceClass,
    TR_OpaqueClassBlock *destClass)
{
    return comp->fej9()->isClassVisible(sourceClass, destClass);
}

bool J9::ClassEnv::classHasIllegalStaticFinalFieldModification(TR_OpaqueClassBlock *clazzPointer)
{
    J9Class *j9clazz = TR::Compiler->cls.convertClassOffsetToClassPtr(clazzPointer);
#if defined(J9VM_OPT_JITSERVER)
    if (auto stream = TR::CompilationInfo::getStream()) {
        // J9ClassHasIllegalFinalFieldModifications bit is cached by
        // ClientSessionData::processIllegalFinalFieldModificationList() before the compilation takes place.
        uintptr_t classFlags = 0;
        JITServerHelpers::getAndCacheRAMClassInfo(j9clazz, TR::compInfoPT->getClientData(), stream,
            JITServerHelpers::CLASSINFO_CLASS_FLAGS, (void *)&classFlags);
...
void J9::ClassEnv::setClassHasIllegalStaticFinalFieldModification(TR_OpaqueClassBlock *clazz, TR::Compilation *comp)
{
    ...
    if (comp->isOutOfProcessCompilation()) {
        ...
        stream->write(JITServer::MessageType::ClassEnv_setClassHasIllegalStaticFinalFieldModification, clazz);
    ...
    VM_AtomicSupport::bitOrU32(&j9c->classFlags, J9ClassHasIllegalFinalFieldModifications);
}

bool J9::ClassEnv::hasFinalFieldsInClass(TR::Compilation *comp, TR_OpaqueClassBlock *clazz)
{
    return comp->fej9()->hasFinalFieldsInClass(clazz);
}

bool J9::ClassEnv::sameClassLoaders(TR::Compilation *comp, TR_OpaqueClassBlock *class1, TR_OpaqueClassBlock *class2)
{
    return comp->fej9()->sameClassLoaders(class1, class2);
}

bool J9::ClassEnv::isString(TR::Compilation *comp, TR_OpaqueClassBlock *clazz) { return comp->fej9()->isString(clazz); }

bool J9::ClassEnv::jitStaticsAreSame(TR::Compilation *comp, TR_ResolvedMethod *method1, int32_t cpIndex1,
    TR_ResolvedMethod *method2, int32_t cpIndex2)
{
    return comp->fej9()->jitStaticsAreSame(method1, cpIndex1, method2, cpIndex2);
}

bool J9::ClassEnv::jitFieldsAreSame(TR::Compilation *comp, TR_ResolvedMethod *method1, int32_t cpIndex1,
...
bool J9::ClassEnv::isAnonymousClass(TR::Compilation *comp, TR_OpaqueClassBlock *clazz)
{
 ...
 */
static char *mergeFieldNames(char *prefix, uint32_t prefixLength, J9ROMFieldShape *field, TR::Region &region,
    uint32_t &mergedLength)
{
 ...
 *    The merged length of the concatenated field name that is returned to the caller.
 */
static char *buildTransitiveFieldNames(char *prefix, uint32_t prefixLength, J9ROMFieldShape *field, TR::Region &region,
    uint32_t &mergedLength)
{
    J9UTF8 *fieldNameUTF = J9ROMFIELDSHAPE_NAME(field);
    char *fieldName = reinterpret_cast<char *>(J9UTF8_DATA(fieldNameUTF));
...
static void addEntryForFieldImpl(TR_VMField *field, TR::TypeLayoutBuilder &tlb, TR::Region &region,
    J9Class *definingClass, char *prefix, uint32_t prefixLength, IDATA offsetBase, TR::Compilation *comp)
{
    ...
    if (TR::Compiler->om.areFlattenableValueTypesEnabled()
        ...
        char *prefixForChild = buildTransitiveFieldNames(prefix, prefixLength, field->shape,
            comp->trMemory()->currentStackRegion(), mergedLength);
        ...
        for (TR_VMField *childField = iter.getFirst(); childField; childField = iter.getNext()) {
            ...
            addEntryForFieldImpl(childField, tlb, region, fieldClass, prefixForChild, prefixLengthForChild,
                offsetBaseForChild, comp);
        }
    } else {
...
static void addEntryForField(TR_VMField *field, TR::TypeLayoutBuilder &tlb, TR::Region &region,
    TR_OpaqueClassBlock *opaqueClazz, TR::Compilation *comp)
{
    char *prefix = NULL;
...
const TR::TypeLayout *J9::ClassEnv::enumerateFields(TR::Region &region, TR_OpaqueClassBlock *opaqueClazz,
    TR::Compilation *comp)
{
    ...
    if (comp->isOutOfProcessCompilation()) {
        ...
        auto recv
            = stream->read<std::vector<TR::TypeLayoutEntry>, std::vector<std::string>, std::vector<std::string> >();
...
int32_t J9::ClassEnv::vTableSlot(TR::Compilation *comp, TR_OpaqueMethodBlock *method, TR_OpaqueClassBlock *clazz)
{
    return comp->fej9()->getVTableSlot(method, clazz);
}

int32_t J9::ClassEnv::flagValueForPrimitiveTypeCheck(TR::Compilation *comp)
{
    return comp->fej9()->getFlagValueForPrimitiveTypeCheck();
}

int32_t J9::ClassEnv::flagValueForArrayCheck(TR::Compilation *comp)
{
    return comp->fej9()->getFlagValueForArrayCheck();
}

int32_t J9::ClassEnv::flagValueForFinalizerCheck(TR::Compilation *comp)
{
    return comp->fej9()->getFlagValueForFinalizerCheck();
}

// this should be a method of TR_SymbolReference
char *J9::ClassEnv::classNameChars(TR::Compilation *comp, TR::SymbolReference *symRef, int32_t &length)
{
    return comp->fej9()->classNameChars(comp, symRef, length);
}

char *J9::ClassEnv::classNameChars(TR::Compilation *comp, TR_OpaqueClassBlock *clazz, int32_t &length)
{
    return comp->fej9()->getClassNameChars(clazz, length);
}

char *J9::ClassEnv::classSignature_DEPRECATED(TR::Compilation *comp, TR_OpaqueClassBlock *clazz, int32_t &length,
    TR_Memory *m)
{
    return comp->fej9()->getClassSignature_DEPRECATED(clazz, length, m);
}

char *J9::ClassEnv::classSignature(TR::Compilation *comp, TR_OpaqueClassBlock *clazz, TR_Memory *m)
{
    return comp->fej9()->getClassSignature(clazz, m);
}

uintptr_t J9::ClassEnv::persistentClassPointerFromClassPointer(TR::Compilation *comp, TR_OpaqueClassBlock *clazz)
{
    return comp->fej9()->getPersistentClassPointerFromClassPointer(clazz);
}

TR_OpaqueClassBlock *J9::ClassEnv::objectClass(TR::Compilation *comp, uintptr_t objectPointer)
{
    return comp->fej9()->getObjectClass(objectPointer);
}

TR_OpaqueClassBlock *J9::ClassEnv::classFromJavaLangClass(TR::Compilation *comp, uintptr_t objectPointer)
{
    return comp->fej9()->getClassFromJavaLangClass(objectPointer);
}

uint16_t J9::ClassEnv::getStringCharacter(TR::Compilation *comp, uintptr_t objectPointer, int32_t index)
{
    return comp->fej9()->getStringCharacter(objectPointer, index);
}

bool J9::ClassEnv::getStringFieldByName(TR::Compilation *comp, TR::SymbolReference *stringRef,
    TR::SymbolReference *fieldRef, void *&pResult)
{
    return comp->fej9()->getStringFieldByName(comp, stringRef, fieldRef, pResult);
}

uintptr_t J9::ClassEnv::getArrayElementWidthInBytes(TR::Compilation *comp, TR_OpaqueClassBlock *arrayClass)
{
...
intptr_t J9::ClassEnv::getVFTEntry(TR::Compilation *comp, TR_OpaqueClassBlock *clazz, int32_t offset)
{
    return comp->fej9()->getVFTEntry(clazz, offset);
}

uint8_t *J9::ClassEnv::getROMClassRefName(TR::Compilation *comp, TR_OpaqueClassBlock *clazz, uint32_t cpIndex,
    int &classRefLen)
{
...
J9ROMConstantPoolItem *J9::ClassEnv::getROMConstantPool(TR::Compilation *comp, TR_OpaqueClassBlock *clazz)
{
#if defined(J9VM_OPT_JITSERVER)
    if (comp->isOutOfProcessCompilation()) {
...
bool J9::ClassEnv::isValueTypeClass(TR_OpaqueClassBlock *clazz)
{
#if defined(J9VM_OPT_JITSERVER)
    if (auto stream = TR::CompilationInfo::getStream()) {
        uintptr_t classFlags = 0;
...
bool J9::ClassEnv::isValueTypeClassFlattened(TR_OpaqueClassBlock *clazz)
{
#if defined(J9VM_OPT_JITSERVER)
    if (auto stream = TR::CompilationInfo::getStream()) {
        uintptr_t classFlags = 0;
...
bool J9::ClassEnv::isValueBasedOrValueTypeClass(TR_OpaqueClassBlock *clazz)
{
#if defined(J9VM_OPT_JITSERVER)
    if (auto stream = TR::CompilationInfo::getStream()) {
        uintptr_t classFlags = 0;
...
bool J9::ClassEnv::isArrayNullRestricted(TR::Compilation *comp, TR_OpaqueClassBlock *arrayClass)
{
#if defined(J9VM_OPT_JITSERVER)
    if (auto stream = comp->getStream()) {
        uintptr_t classFlags = 0;
        JITServerHelpers::getAndCacheRAMClassInfo((J9Class *)arrayClass, TR::compInfoPT->getClientData(), stream,
            JITServerHelpers::CLASSINFO_CLASS_FLAGS, (void *)&classFlags);
#ifdef DEBUG
...
bool J9::ClassEnv::classHasIdentity(TR_OpaqueClassBlock *clazz)
{
#if defined(J9VM_OPT_JITSERVER)
    if (auto stream = TR::CompilationInfo::getStream()) {
        uintptr_t classFlags = 0;
...
bool J9::ClassEnv::classSupportsDirectMemoryComparison(TR_OpaqueClassBlock *clazz)
{
    uintptr_t classFlags = 0;
#if defined(J9VM_OPT_JITSERVER)
    if (auto stream = TR::CompilationInfo::getStream()) {
...
bool J9::ClassEnv::containsZeroOrOneConcreteClass(TR::Compilation *comp, List<TR_PersistentClassInfo> *subClasses)
{
    int count = 0;
#if defined(J9VM_OPT_JITSERVER)
    if (comp->isOutOfProcessCompilation()) {
...
char *J9::ClassEnv::classNameToSignature(const char *name, int32_t &len, TR::Compilation *comp,
...
int32_t J9::ClassEnv::flattenedArrayElementSize(TR::Compilation *comp, TR_OpaqueClassBlock *arrayClass)
{
#if defined(J9VM_OPT_JITSERVER)
    if (auto stream = comp->getStream()) {
    ...
    {
        J9JavaVM *vm = comp->fej9()->getJ9JITConfig()->javaVM;
        return vm->internalVMFunctions->arrayElementSize(
            (J9ArrayClass *)self()->convertClassOffsetToClassPtr(arrayClass));
    }
}
