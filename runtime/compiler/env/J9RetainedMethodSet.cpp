...
static J9Class *definingJ9Class(TR_ResolvedMethod *method)
{
...
public:
    InliningTable(TR::Compilation *comp)
        : _comp(comp)
    {}
...
class J9::RetainedMethodSet::CompInliningTable : public J9::RetainedMethodSet::InliningTable {
public:
    CompInliningTable(TR::Compilation *comp)
        : InliningTable(comp)
    {}

    virtual TR_ResolvedMethod *inlinedResolvedMethod(uint32_t i)
    {
        TR_ASSERT_FATAL(i < numInlinedSites(), "out of bounds index %u >= %u", i, numInlinedSites());
...
class J9::RetainedMethodSet::VectorInliningTable : public J9::RetainedMethodSet::InliningTable {
public:
    VectorInliningTable(TR::Compilation *comp, const TR::vector<J9::ResolvedInlinedCallSite, TR::Region &> &vec)
        : InliningTable(comp)
        , _vec(vec)
    {}
    ...
    virtual TR_ResolvedMethod *inlinedResolvedMethod(uint32_t i)
    {
        TR_ASSERT_FATAL(i < _vec.size(), "out of bounds index %u >= %u\n", i, (uint32_t)_vec.size());
...
static void traceMethod(TR::Compilation *comp, TR_ResolvedMethod *method)
{
    comp->log()->printf("%p %.*s.%.*s%.*s in class %p", method->getPersistentIdentifier(), method->classNameLength(),
        method->classNameChars(), method->nameLength(), method->nameChars(), method->signatureLength(),
        method->signatureChars(), method->containingClass());
}

J9::RetainedMethodSet::RetainedMethodSet(TR::Compilation *comp, TR_ResolvedMethod *method,
    J9::RetainedMethodSet *parent, InliningTable *inliningTable)
...
J9::RetainedMethodSet *J9::RetainedMethodSet::create(TR::Compilation *comp, TR_ResolvedMethod *method)
{
...
const TR::vector<J9::ResolvedInlinedCallSite, TR::Region &> &J9::RetainedMethodSet::copyInliningTable(
    TR::Compilation *comp, J9JITExceptionTable *metadata)
{
    auto &inliningTable = *new (comp->region()) TR::vector<J9::ResolvedInlinedCallSite, TR::Region &>(comp->region());
    ...
    for (uint32_t i = 0; i < n; i++) {
        ...
        TR_ResolvedMethod *method
            = new (comp->trHeapMemory()) TR_ResolvedJ9Method(site->_methodInfo, comp->fe(), comp->trMemory());
...
J9::RetainedMethodSet *J9::RetainedMethodSet::createChild(TR_ResolvedMethod *method)
{
    TR::Region &region = comp()->trMemory()->heapMemoryRegion();
    auto *result = new (region) J9::RetainedMethodSet(comp(), method, this, _inliningTable);
    result->init();
    return result;
}

static void traceNamedLoader(TR::Compilation *comp, J9::RetainedMethodSet *s, const char *name, J9ClassLoader *loader)
{
    comp->log()->printf("RetainedMethodSet %p: %s=%p\n", s, name, loader);
}

void J9::RetainedMethodSet::init()
{
    if (parent() == NULL) {
        ...
        for (auto it = permanentLoaders.begin(); it != end; it++) {
            ...
            logprintf(trace, comp()->log(), "RetainedMethodSet %p: add permanent loader %p\n", this, loader);
        }
    }
    ...
    switch (bcOp) {
        ...
        case J9BCinvokehandle: {
            ...
            if (bcOp == J9BCinvokehandle) {
                uint16_t cpIndex = loadBc<uint16_t>(caller, bcStart, bcSize, bcIndex, comp(), 1, "invokehandle");
                ...
                uint16_t callSiteIndex = loadBc<uint16_t>(caller, bcStart, bcSize, bcIndex, comp(), 1, "invokedynamic");

                if (caller->isUnresolvedCallSiteTableEntry(callSiteIndex)) {
                    break;
                }

                invokeCacheArray = (uintptr_t *)caller->callSiteTableEntryAddress(callSiteIndex);
            ...
            linkedCallee
                = comp()->fej9()->targetMethodFromInvokeCacheArrayMemberNameObj(comp(), caller, invokeCacheArray);
        ...
        default:
            TR_ASSERT_FATAL(false, "unexpected invoke bytecode %d at %d in %.*s.%.*s%.*s", bcOp, bcIndex,
...
void J9::RetainedMethodSet::attestWillRemainLoaded(TR_ResolvedMethod *method)
{
    if (comp()->getOption(TR_TraceRetainedMethods)) {
...
void J9::RetainedMethodSet::attestWillRemainLoaded(TR_OpaqueClassBlock *clazz)
{
    if (comp()->getOption(TR_TraceRetainedMethods)) {
        int32_t len;
...
void J9::RetainedMethodSet::scan(J9Class *clazz)
{
    if (isAnonymousClass(clazz) && !willAnonymousClassRemainLoaded(clazz)) {
        logprintf(comp()->getOption(TR_TraceRetainedMethods), comp()->log(),
    ...
    logprintf(comp()->getOption(TR_TraceRetainedMethods), comp()->log(), "RetainedMethodSet %p: add loader %p\n", this,
        loader);
    ...
    while (!queue.empty()) {
        ...
        if (outlivingLoaders == NULL || outlivingLoaders == J9CLASSLOADER_OUTLIVING_LOADERS_PERMANENT) {
            // Empty set.
            ...
            TR_ASSERT_FATAL(repeatLoadedOutlivingLoaders == outlivingLoaders,
                "unexpected change to outlivingLoaders of J9ClassLoader %p: old %p, new %p", loader, outlivingLoaders,
...
bool J9::RetainedMethodSet::willRemainLoaded(TR_ResolvedMethod *method)
{
    return willRemainLoaded(definingJ9Class(method));
}

bool J9::RetainedMethodSet::willRemainLoaded(TR_OpaqueClassBlock *clazz)
{
    return willRemainLoaded(TR::Compiler->cls.convertClassOffsetToClassPtr(clazz));
}

bool J9::RetainedMethodSet::willRemainLoaded(J9Class *clazz)
{
...
bool J9::RetainedMethodSet::willRemainLoaded(J9ClassLoader *loader)
{
...
bool J9::RetainedMethodSet::willAnonymousClassRemainLoaded(J9Class *clazz)
{
...
void J9::RetainedMethodSet::keepalive()
{
    OMR::RetainedMethodSet::keepalive();
    keepalivesAndBonds()->_keepaliveLoaders.insert(_loaders.begin(), _loaders.end());
    keepalivesAndBonds()->_keepaliveAnonClasses.insert(_anonClasses.begin(), _anonClasses.end());
}

void J9::RetainedMethodSet::bond()
{
    OMR::RetainedMethodSet::bond();
    keepalivesAndBonds()->_bondLoaders.insert(_loaders.begin(), _loaders.end());
    keepalivesAndBonds()->_bondAnonClasses.insert(_anonClasses.begin(), _anonClasses.end());
}

J9ClassLoader *J9::RetainedMethodSet::getLoader(J9Class *clazz)
{
    auto *c = TR::Compiler->cls.convertClassPtrToClassOffset(clazz);
    return (J9ClassLoader *)comp()->fej9()->getClassLoader(c);
}

bool J9::RetainedMethodSet::isAnonymousClass(J9Class *clazz)
{
    auto *c = TR::Compiler->cls.convertClassPtrToClassOffset(clazz);
    return comp()->fej9()->isAnonymousClass(c);
}

void *J9::RetainedMethodSet::unloadingKey(TR_ResolvedMethod *method)
{
    J9Class *c = definingJ9Class(method);
    return isAnonymousClass(c) ? (void *)c : (void *)getLoader(c);
}

J9::RetainedMethodSet::KeepalivesAndBonds *J9::RetainedMethodSet::createKeepalivesAndBonds()
{
...
J9::RetainedMethodSet *J9::RetainedMethodSet::withKeepalivesAttested()
{
...
J9::RetainedMethodSet *J9::RetainedMethodSet::withBondsAttested()
{
    J9::RetainedMethodSet *result = createChild(method());
    KeepalivesAndBonds *kb = keepalivesAndBonds();
...
void J9::RepeatRetainedMethodsAnalysis::getDataForClient(TR::Compilation *comp,
...
static void assertSubset(const char *what, TR::vector<void *, TR::Region &> clientKeys,
    TR::vector<void *, TR::Region &> serverKeys)
{
    ...
    for (auto cIt = clientKeys.begin(); cIt != clientKeys.end(); cIt++) {
        ...
        TR_ASSERT_FATAL(match, "client %s method key %p has no corresponding server key", what, *cIt);
    }
}
...
OMR::RetainedMethodSet *J9::RepeatRetainedMethodsAnalysis::analyzeOnClient(TR::Compilation *comp,
    const TR::vector<J9::ResolvedInlinedCallSite, TR::Region &> &inliningTable,
    ...
    const std::vector<TR_ResolvedMethod *> &serverBondMethods)
{
    ...
    if (!comp->mustTrackRetainedMethods()) {
        ...
        OMR::RetainedMethodSet *result
            = new (comp->region()) OMR::RetainedMethodSet(comp, comp->getMethodBeingCompiled(), NULL);

        comp->setClientRetainedMethods(result);
        return result;
    }

    J9::RetainedMethodSet *root = J9::RetainedMethodSet::create(comp, comp->getMethodBeingCompiled(), inliningTable);
    ...
    for (uint32_t siteIndex = 0; siteIndex < n; siteIndex++) {
        ...
        J9::RetainedMethodSet *surroundingSet = callerIndex == -1 ? root : retainedMethods[callerIndex];

        surroundingSet = surroundingSet->withLinkedCalleeAttested(bci);

        J9::RetainedMethodSet *callSiteSet = surroundingSet;
        if (info != NULL && info->_refinedMethod != NULL && !surroundingSet->willRemainLoaded(info->_refinedMethod)) {
            TR_ASSERT_FATAL(info->_generatedKeepalive,
        ...
        if (!callSiteSet->willRemainLoaded(targetMethod)) {
            TR_ASSERT_FATAL(info != NULL && info->_generatedBond,
    ...
    TR_ASSERT_FATAL(infoIt == infoItEnd, "failed to reach the end of inlinedSiteInfo");
    ...
    for (auto it = serverKeepaliveMethods.begin(); it != serverKeepaliveMethods.end(); it++) {
...
OMR::RetainedMethodSet *J9::RepeatRetainedMethodsAnalysis::analyzeOnClient(TR::Compilation *comp,
    J9JITExceptionTable *metadata, const std::vector<InlinedSiteInfo> &inlinedSiteInfo,
    ...
    const std::vector<TR_ResolvedMethod *> &serverBondMethods)
{
    ...
    return analyzeOnClient(comp, inliningTable, inlinedSiteInfo, serverKeepaliveMethods, serverBondMethods);
}
