...
bool TR_AOTDependencyTable::trackMethod(J9VMThread *vmThread, J9Method *method, J9ROMMethod *romMethod,
    bool &dependenciesSatisfied)
{
    ...
    if (!methodDependencies) {
        ...
        if (TR::Options::getVerboseOption(TR_VerboseDependencyTracking))
            TR_VerboseLog::writeLineLocked(TR_Vlog_INFO,
                "Dependency table: method %p with 0 dependencies will start at count 0", method);
    ...
    try {
        ...
        auto m_it = _methodMap.insert({
            method, { 0, methodDependencies }
        });
        ...
        for (size_t i = 1; i <= totalDependencies; ++i) {
            ...
            uintptr_t offset = _sharedCache->startingROMClassOffsetOfClassChain(
                _sharedCache->pointerFromOffsetInSharedCache(chainOffset));
        ...
        if (numberRemainingDependencies == 0) {
            ...
            if (TR::Options::getVerboseOption(TR_VerboseDependencyTracking))
                TR_VerboseLog::writeLineLocked(TR_Vlog_INFO,
                    "Dependency table: method %p with %lu dependencies will start at count 0", method,
                    totalDependencies);
        } else {
            methodEntry->second._remainingDependencies = numberRemainingDependencies;
            if (TR::Options::getVerboseOption(TR_VerboseDependencyTracking))
                TR_VerboseLog::writeLineLocked(TR_Vlog_INFO,
                    "Dependency table: method %p with %lu dependencies will be tracked with %lu remaining", method,
                    totalDependencies, numberRemainingDependencies);
        }
    } catch (std::exception &) {
        deactivateTable();
        return false;
...
void TR_AOTDependencyTable::methodWillBeCompiled(J9Method *method)
{
    OMR::CriticalSection cs(_tableMonitor);
    if (!isActive())
        return;
...
void TR_AOTDependencyTable::stopTracking(MethodEntryRef entry, bool isEarlyStop)
{
    ...
    if (verboseUnsatisfied)
        TR_VerboseLog::writeLineLocked(TR_Vlog_INFO,
            "Dependency table: early tracking stop for method %p with %lu remaining dependencies", method,
            methodEntry._remainingDependencies);
    ...
    for (size_t i = 1; i <= dependencyChainLength; ++i) {
        ...
        if (verboseUnsatisfiedDetails && !findCandidateForDependency(o_it->second._loadedClasses, needsInitialization))
            TR_VerboseLog::writeLineLocked(TR_Vlog_INFO,
                "Dependency table: stopped tracking method %p with unsatisfied dependency chainOffset=%lu "
                "romClassOffset=%lu needsInit=%d",
...
void TR_AOTDependencyTable::stopTracking(J9Method *method, bool isEarlyStop)
{
...
void TR_AOTDependencyTable::eraseOffsetEntryIfEmpty(const OffsetEntry &entry, uintptr_t offset)
{
    if (entry._loadedClasses.empty() && entry._waitingInitMethods.empty() && entry._waitingLoadMethods.empty())
        _offsetMap.erase(offset);
}

void TR_AOTDependencyTable::classLoadEvent(TR_OpaqueClassBlock *clazz, bool isClassLoad, bool isClassInitialization)
{
...
void TR_AOTDependencyTable::registerSatisfaction(PersistentUnorderedSet<MethodEntryRef> waitingMethods)
{
...
void TR_AOTDependencyTable::classLoadEventAtOffset(J9Class *ramClass, uintptr_t offset, bool isClassLoad,
    bool isClassInitialization)
{
    ...
    if (TR::Options::getVerboseOption(TR_VerboseDependencyTracking)) {
        ...
        TR_VerboseLog::writeLineLocked(TR_Vlog_INFO,
            "Dependency table: class load event %.*s ramClass=%p romClassOffset=%lu isLoad=%d isInit=%d",
...
OffsetEntry *TR_AOTDependencyTable::getOffsetEntry(uintptr_t offset, bool create)
{
    ...
    if (create) {
        PersistentUnorderedSet<J9Class *> loadedClasses(
            PersistentUnorderedSet<J9Class *>::allocator_type(TR::Compiler->persistentAllocator()));
        PersistentUnorderedSet<MethodEntryRef> waitingLoadMethods(
            PersistentUnorderedSet<MethodEntryRef>::allocator_type(TR::Compiler->persistentAllocator()));
        PersistentUnorderedSet<MethodEntryRef> waitingInitMethods(
            PersistentUnorderedSet<MethodEntryRef>::allocator_type(TR::Compiler->persistentAllocator()));
        return &(*_offsetMap.insert(it,
                     {
                         offset, { loadedClasses, waitingLoadMethods, waitingInitMethods }
        }))
                    .second;
    }
...
void TR_AOTDependencyTable::invalidateUnloadedClass(TR_OpaqueClassBlock *clazz)
{
    uintptr_t classOffset = TR_SharedCache::INVALID_ROM_CLASS_OFFSET;
    if (!_sharedCache->isClassInSharedCache(clazz, &classOffset))
        return;
...
void TR_AOTDependencyTable::registerDissatisfaction(PersistentUnorderedSet<MethodEntryRef> waitingMethods)
{
    for (auto &entry : waitingMethods) {
        ++entry->second._remainingDependencies;
...
bool TR_AOTDependencyTable::invalidateClassAtOffset(J9Class *ramClass, uintptr_t romClassOffset)
{
    auto entry = getOffsetEntry(romClassOffset, false);
    if (entry) {
        if (TR::Options::getVerboseOption(TR_VerboseDependencyTracking))
            TR_VerboseLog::writeLineLocked(TR_Vlog_INFO, "Dependency table: invalidating class %p romClassOffset=%lu",
                ramClass, romClassOffset);
...
void TR_AOTDependencyTable::invalidateMethodsOfClass(J9Class *ramClass)
{
...
void TR_AOTDependencyTable::recheckSubclass(J9Class *ramClass, uintptr_t offset, bool shouldRevalidate)
{
...
void TR_AOTDependencyTable::invalidateRedefinedClass(TR_PersistentCHTable *table, TR_J9VMBase *fej9,
    TR_OpaqueClassBlock *oldClass, TR_OpaqueClassBlock *freshClass)
{
    ...
    if (!_sharedCache->isClassInSharedCache(freshClass, &freshClassOffset)
        && !_sharedCache->isClassInSharedCache(oldClass, &oldClassOffset))
        return;
...
void TR_AOTDependencyTable::resolvePendingLoads()
{
    for (auto &entry : _pendingLoads) {
        ...
        if (TR::Options::isAnyVerboseOptionSet(TR_VerboseDependencyTracking, TR_VerboseCounts))
            TR_VerboseLog::writeLineLocked(TR_Vlog_INFO,
                "Dependency table: pending load %s - method %p count %lu --> %lu", (count == 0) ? "success" : "failure",
                method, initCount, count);
...
J9Class *TR_AOTDependencyTable::findCandidateWithChainAndLoader(TR::Compilation *comp, uintptr_t classChainOffset,
...
J9Class *TR_AOTDependencyTable::findCandidateWithChainAndLoader(TR::Compilation *comp, uintptr_t *classChain,
    void *classLoaderChain)
{
    if (comp->isDeserializedAOTMethod() || comp->ignoringLocalSCC())
        return NULL;
...
J9Class *TR_AOTDependencyTable::findChainLoaderCandidate(TR::Compilation *comp, uintptr_t *classChain,
    void *classLoaderChain)
{
    ...
    for (const auto &clazz : it->second._loadedClasses) {
        ...
        if ((J9ClassInitFailed != clazz->initializeStatus)
            && (_sharedCache->persistentClassLoaderTable()->lookupClassChainAssociatedWithClassLoader(
                    clazz->classLoader)
                == classLoaderChain))
            return clazz;
    }
...
void TR_AOTDependencyTable::printStats()
{
...
J9Class *TR_AOTDependencyTable::findCandidateForDependency(const PersistentUnorderedSet<J9Class *> &loadedClasses,
...
void TR_AOTDependencyTable::deactivateTable()
{
    _offsetMap.clear();
    _methodMap.clear();
    _pendingLoads.clear();
