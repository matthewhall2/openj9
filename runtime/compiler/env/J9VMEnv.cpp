...
int64_t J9::VMEnv::maxHeapSizeInBytes()
{
#if defined(J9VM_OPT_JITSERVER)
...
bool J9::VMEnv::hasAccess(OMR_VMThread *omrVMThread)
{
    return TR::Compiler->vm.hasAccess(self()->J9VMThreadFromOMRVMThread(omrVMThread));
}

bool J9::VMEnv::hasAccess(J9VMThread *j9VMThread)
{
    return (j9VMThread->publicFlags & J9_PUBLIC_FLAGS_VM_ACCESS) ? true : false;
}

bool J9::VMEnv::hasAccess(TR::Compilation *comp) { return comp->fej9()->haveAccess(comp); }
...
//
static bool acquireVMaccessIfNeededInner(J9VMThread *vmThread, TR_YesNoMaybe isCompThread)
{
    ...
    if (isCompThread == TR_no) {
        TR_ASSERT((vmThread->publicFlags & J9_PUBLIC_FLAGS_VM_ACCESS),
            "Must have vm access if this is not a compilation thread");
        return false;
    }
...
bool J9::VMEnv::acquireVMAccessIfNeeded(OMR_VMThread *omrVMThread)
{
    TR_ASSERT(0, "not implemented");
    return false;
}

bool J9::VMEnv::acquireVMAccessIfNeeded(TR_J9VMBase *fej9)
{
...
bool J9::VMEnv::acquireVMAccessIfNeeded(TR::Compilation *comp)
{
    return TR::Compiler->vm.acquireVMAccessIfNeeded(comp->fej9());
}

bool J9::VMEnv::tryToAcquireAccess(TR::Compilation *comp, bool *haveAcquiredVMAccess)
{
    return comp->fej9()->tryToAcquireAccess(comp, haveAcquiredVMAccess);
}

bool J9::VMEnv::tryToAcquireAccess(OMR_VMThread *omrVMThread, bool *haveAcquiredVMAccess)
{
    TR_ASSERT(0, "not implemented");
    return false;
}

static void releaseVMaccessIfNeededInner(J9VMThread *vmThread, bool haveAcquiredVMAccess)
{
...
void J9::VMEnv::releaseVMAccessIfNeeded(TR::Compilation *comp, bool haveAcquiredVMAccess)
{
    comp->fej9()->releaseVMAccessIfNeeded(haveAcquiredVMAccess);
}

void J9::VMEnv::releaseVMAccessIfNeeded(OMR_VMThread *omrVMThread, bool haveAcquiredVMAccess)
{
    releaseVMaccessIfNeededInner(self()->J9VMThreadFromOMRVMThread(omrVMThread), haveAcquiredVMAccess);
}

void J9::VMEnv::releaseVMAccessIfNeeded(TR_J9VMBase *fej9, bool haveAcquiredVMAccess)
{
    fej9->releaseVMAccessIfNeeded(haveAcquiredVMAccess);
}

void J9::VMEnv::releaseAccess(TR::Compilation *comp) { comp->fej9()->releaseAccess(comp); }

void J9::VMEnv::releaseAccess(OMR_VMThread *omrVMThread) { TR_ASSERT(0, "not implemented"); }

void J9::VMEnv::releaseAccess(TR_J9VMBase *fej9)
{
...
uintptr_t J9::VMEnv::thisThreadGetPendingExceptionOffset() { return offsetof(J9VMThread, jitException); }

bool J9::VMEnv::hasResumableTrapHandler(TR::Compilation *comp) { return comp->fej9()->hasResumableTrapHandler(); }

bool J9::VMEnv::hasResumableTrapHandler(OMR_VMThread *omrVMThread)
{
    TR_ASSERT(0, "not implemented");
    return false;
}

uint64_t J9::VMEnv::getUSecClock(TR::Compilation *comp) { return comp->fej9()->getUSecClock(); }

uint64_t J9::VMEnv::getUSecClock(OMR_VMThread *omrVMThread)
{
    TR_ASSERT(0, "not implemented");
    return 0;
}

uint64_t J9::VMEnv::getHighResClock(TR::Compilation *comp) { return comp->fej9()->getHighResClock(); }

uint64_t J9::VMEnv::getHighResClock(OMR_VMThread *omrVMThread)
{
    TR_ASSERT(0, "not implemented");
    return 0;
}

uint64_t J9::VMEnv::getHighResClockResolution(TR::Compilation *comp)
{
    return comp->fej9()->getHighResClockResolution();
}

uint64_t J9::VMEnv::getHighResClockResolution(OMR_VMThread *omrVMThread)
{
    TR_ASSERT(0, "not implemented");
    return 0;
}

uint64_t J9::VMEnv::getHighResClockResolution(TR_FrontEnd *fej9)
{
    return ((TR_J9VMBase *)fej9)->getHighResClockResolution();
}

uint64_t J9::VMEnv::getHighResClockResolution()
{
    PORT_ACCESS_FROM_PORT(TR::Compiler->portLib);
    return j9time_hires_frequency();
}

bool J9::VMEnv::canMethodEnterEventBeHooked(TR::Compilation *comp)
{
    return comp->fej9()->canMethodEnterEventBeHooked();
}

bool J9::VMEnv::canMethodExitEventBeHooked(TR::Compilation *comp) { return comp->fej9()->canMethodExitEventBeHooked(); }

uintptr_t J9::VMEnv::getOverflowSafeAllocSize(TR::Compilation *comp)
{
    return comp->fej9()->getOverflowSafeAllocSize();
}

int64_t J9::VMEnv::cpuTimeSpentInCompilationThread(TR::Compilation *comp)
{
    return comp->fej9()->getCpuTimeSpentInCompThread(comp);
}

uintptr_t J9::VMEnv::OSRFrameHeaderSizeInBytes(TR::Compilation *comp)
{
    return comp->fej9()->getOSRFrameHeaderSizeInBytes();
}

uintptr_t J9::VMEnv::OSRFrameSizeInBytes(TR::Compilation *comp, TR_OpaqueMethodBlock *method)
{
    return comp->fej9()->getOSRFrameSizeInBytes(method);
}

bool J9::VMEnv::ensureOSRBufferSize(TR::Compilation *comp, uintptr_t osrFrameSizeInBytes,
    uintptr_t osrScratchBufferSizeInBytes, uintptr_t osrStackFrameSizeInBytes)
{
    return comp->fej9()->ensureOSRBufferSize(comp, osrFrameSizeInBytes, osrScratchBufferSizeInBytes,
        osrStackFrameSizeInBytes);
}

uintptr_t J9::VMEnv::thisThreadGetOSRReturnAddressOffset(TR::Compilation *comp)
{
    return comp->fej9()->thisThreadGetOSRReturnAddressOffset();
}

uintptr_t J9::VMEnv::thisThreadGetGSIntermediateResultOffset(TR::Compilation *comp)
{
    return comp->fej9()->thisThreadGetGSIntermediateResultOffset();
}

uintptr_t J9::VMEnv::thisThreadGetConcurrentScavengeActiveByteAddressOffset(TR::Compilation *comp)
{
    return comp->fej9()->thisThreadGetConcurrentScavengeActiveByteAddressOffset();
}

uintptr_t J9::VMEnv::thisThreadGetEvacuateBaseAddressOffset(TR::Compilation *comp)
{
    return comp->fej9()->thisThreadGetEvacuateBaseAddressOffset();
}

uintptr_t J9::VMEnv::thisThreadGetEvacuateTopAddressOffset(TR::Compilation *comp)
{
    return comp->fej9()->thisThreadGetEvacuateTopAddressOffset();
}

uintptr_t J9::VMEnv::thisThreadGetGSOperandAddressOffset(TR::Compilation *comp)
{
    return comp->fej9()->thisThreadGetGSOperandAddressOffset();
}

uintptr_t J9::VMEnv::thisThreadGetGSHandlerAddressOffset(TR::Compilation *comp)
{
 ...
 */
bool J9::VMEnv::isSelectiveMethodEnterExitEnabled(TR::Compilation *comp)
{
    return comp->fej9()->isSelectiveMethodEnterExitEnabled();
}

size_t J9::VMEnv::getInterpreterVTableOffset()
{
#if defined(J9VM_OPT_JITSERVER)
    if (auto stream = TR::CompilationInfo::getStream()) {
        auto *vmInfo = TR::compInfoPT->getClientData()->getOrCacheVMInfo(stream);
...
bool J9::VMEnv::isVMInStartupPhase(J9JITConfig *jitConfig)
{
#if defined(J9VM_OPT_JITSERVER)
    if (auto stream = TR::CompilationInfo::getStream()) {
...
bool J9::VMEnv::isVMInStartupPhase(TR::Compilation *comp)
{
