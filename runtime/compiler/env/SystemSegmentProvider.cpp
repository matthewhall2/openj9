...
J9::SystemSegmentProvider::SystemSegmentProvider(size_t defaultSegmentSize, size_t systemSegmentSize,
    size_t allocationLimit, J9::J9SegmentProvider &segmentAllocator, TR::RawAllocator rawAllocator)
...
{
    TR_ASSERT_FATAL(defaultSegmentSize <= systemSegmentSize,
        "defaultSegmentSize should be smaller than or equal to systemSegmentSize");

    // We cannot simply assign systemSegmentSize to _systemSegmentSize because:
    // We want to make sure that _currentSystemSegment is always a small system segment, i.e. its size <=
    // _systemSegmentSize. When size alignment happens in _systemSegmentAllocator.request, this will be violated, make
    // it _currentSystemSegment a large system segment capable of allocating large segments and small segments. A large
    // segment and its containing system segment is allocated/released differently, we don't want to have a system
    // segment whose memory is used by a mix of small segments and large segments.
    //
...
bool J9::SystemSegmentProvider::isLargeSegment(size_t segmentSize) { return segmentSize > _systemSegmentSize; }

TR::MemorySegment &J9::SystemSegmentProvider::request(size_t requiredSize)
{
    size_t const roundedSize = round(requiredSize);
    if (_freeSegments != &_freeSegmentsEmpty && !(defaultSegmentSize() < roundedSize)) {
    ...
    if (remaining(_currentSystemSegment) >= roundedSize) {
        ...
        TR_ASSERT_FATAL(!isLargeSegment(remaining(_currentSystemSegment)),
            "_currentSystemSegment must be a small segment");
    ...
    TR_ASSERT_FATAL(newSegment.heapAlloc == newSegment.heapBase,
        "Segment @ %p { heapBase: %p, heapAlloc: %p, heapTop: %p } is stale", &newSegment, newSegment.heapBase,
    ...
    if (!isLargeSegment(roundedSize)) {
        ...
        // Carve its remaining space into small segments and add them to the free segment list so that we can use it
        // later
        //
        while (remaining(_currentSystemSegment) >= defaultSegmentSize()) {
            TR::MemorySegment &remainingSegment = allocateNewSegment(defaultSegmentSize(), _currentSystemSegment);
...
void J9::SystemSegmentProvider::release(TR::MemorySegment &segment) throw()
{
...
size_t J9::SystemSegmentProvider::systemBytesAllocated() const throw() { return _systemBytesAllocated; }

size_t J9::SystemSegmentProvider::regionBytesAllocated() const throw() { return _regionBytesAllocated; }

size_t J9::SystemSegmentProvider::bytesAllocated() const throw() { return _regionBytesAllocated; }

TR::MemorySegment &J9::SystemSegmentProvider::allocateNewSegment(size_t size,
    TR::reference_wrapper<J9MemorySegment> systemSegment)
{
...
TR::MemorySegment &J9::SystemSegmentProvider::createSegmentFromArea(size_t size, void *newSegmentArea)
{
...
size_t J9::SystemSegmentProvider::allocationLimit() const throw() { return _allocationLimit; }

void J9::SystemSegmentProvider::setAllocationLimit(size_t allocationLimit) { _allocationLimit = allocationLimit; }

size_t J9::SystemSegmentProvider::round(size_t requestedSize)
{
    return (((requestedSize + (defaultSegmentSize() - 1)) / defaultSegmentSize()) * defaultSegmentSize());
}

ptrdiff_t J9::SystemSegmentProvider::remaining(const J9MemorySegment &memorySegment)
{
