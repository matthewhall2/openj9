...
static int32_t computeSignatureLength(char *signature)
{
    char *currentArgument;
    for (currentArgument = signature + 1; currentArgument[0] != ')';
...
TR_MHJ2IThunk *TR_MHJ2IThunk::allocate(int16_t codeSize, char *signature, TR::CodeGenerator *cg,
...
int16_t TR_MHJ2IThunkTable::terseSignatureLength(char *signature)
{
    int16_t numArgs = 0;
    for (char *currentArgument = signature + 1; currentArgument[0] != ')';
         currentArgument = nextSignatureArgument(currentArgument))
        numArgs++;
...
TR_MHJ2IThunkTable::Node *TR_MHJ2IThunkTable::Node::get(char *terseSignature, TR_PersistentArray<Node> &nodeArray,
...
TR_MHJ2IThunk *TR_MHJ2IThunkTable::findThunk(char *signature, TR_FrontEnd *fe, bool isForCurrentRun)
{
...
TR_MHJ2IThunk *TR_MHJ2IThunkTable::getThunk(char *signature, TR_FrontEnd *fe, bool isForCurrentRun)
{
    ...
    if (!result) {
        ...
        log->printf("\nERROR: Failed to find J2I thunk for %s signature %.*s\n", terseSignature,
            computeSignatureLength(signature), signature);
        TR_ASSERT(result != NULL, "Expected a J2I thunk for %s signature %.*s", terseSignature,
            computeSignatureLength(signature), signature);
    }
    return result;
}

TR_MHJ2IThunk *TR_MHJ2IThunkTable::findThunkFromTerseSignature(char *terseSignature, TR_FrontEnd *fe,
...
void TR_MHJ2IThunkTable::addThunk(TR_MHJ2IThunk *thunk, TR_FrontEnd *fe, bool isForCurrentRun)
{
    TR_J9VMBase *fej9 = (TR_J9VMBase *)(fe);
    ...
    if (fej9->isAOT_DEPRECATED_DO_NOT_USE() && !isForCurrentRun) {
        ...
        TR_ASSERT(findThunkFromTerseSignature(thunk->terseSignature(), fe, isForCurrentRun) == thunk,
            "TR_MHJ2IThunkTable: setThunk must cause findThunk(%s) to return %p", thunk->terseSignature(), thunk);
        if (TR::Options::getCmdLineOptions()->getVerboseOption(TR_VerboseJ2IThunks))
            TR_VerboseLog::writeLineLocked(TR_Vlog_J2I, "add %s @%p", thunk->terseSignature(), thunk);
    }
}

void TR_MHJ2IThunkTable::Node::dumpTo(OMR::Logger *log, TR_FrontEnd *fe, TR_PersistentArray<Node> &nodeArray,
    int indent)
{
...
void TR_MHJ2IThunkTable::dumpTo(OMR::Logger *log, TR_FrontEnd *fe)
{
    OMR::CriticalSection criticalSection(_monitor);
