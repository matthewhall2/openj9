...
private:
    ...
    CpuUsageCircularBuffer *_cpuUsageCircularBuffer; // Circular buffer containing timestamp, system cpu usage sample,
                                                     // and jvm cpu use sample
    int32_t _cpuUsageCircularBufferIndex; // Current index of the buffer; contains the oldest data. Subtract 1 to get
...
public:
    TR_PERSISTENT_ALLOC(TR_Memory::PersistentInfo);
    CpuSelfThreadUtilization(TR::PersistentInfo *persistentInfo, J9JITConfig *jitConfig, int64_t minPeriodNs,
        int32_t id);
    bool update();

    int64_t getCpuTime() const { return _cpuTimeAtLastUpdate; }

    int64_t getCpuTimeNow() const
    {
        PORT_ACCESS_FROM_JITCONFIG(_jitConfig);
        return j9thread_get_self_cpu_time(j9thread_self());
    } // expensive
    ...
    int32_t computeThreadCpuUtilOverLastNns(
        int64_t validInterval) const; // this one looks at the last two intervals, but not too distant in the past

    int64_t getCrtTimeNs() const
    {
        PORT_ACCESS_FROM_JITCONFIG(_jitConfig);
        return j9time_current_time_millis() * 1000000; /* j9time_nano_time() */
    }
...
private:
    ...
    uint64_t _lowResolutionClockAtLastUpdate; // this is based on the fast but inaccurate time maintained in
                                              // persistentinfo (ms)
    int64_t
        _cpuTimeAtLastUpdate; // cpu time spent in thread at the time of last update (since the start of the process)
    ...
    int64_t _cpuTimeDuringSecondLastInterval; // cpu time spent in thread during the second last measurement interval
                                              // (precedes last interval)
...
public:
    ...
    double getGuestCpuEntitlement() const
    {
        return _guestCpuEntitlement;
    } // as given by the hypervisor; 0 if error or no hypervisor

    double getJvmCpuEntitlement() const
    {
        return _jvmCpuEntitlement;
    } // smallest of _numTargetCpu and _guestCpuEntitlement
