...
TR_PatchNOPedGuardSiteOnMethodOverride *TR_PatchNOPedGuardSiteOnMethodOverride::make(TR_FrontEnd *fe,
...
bool TR_CHTable::recompileOnNewClassExtend(TR::Compilation *c, TR_OpaqueClassBlock *classId)
{
    ...
    if (!_classesThatShouldNotBeNewlyExtended)
        _classesThatShouldNotBeNewlyExtended
            = new (c->trHeapMemory()) TR_Array<TR_OpaqueClassBlock *>(c->trMemory(), 8);
...
#endif
        (comp->getVirtualGuards().empty() && comp->getSideEffectGuardPatchSites()->empty() && !_preXMethods && !_classes
            && !_classesThatShouldNotBeNewlyExtended && !comp->retainedMethods()->bondMethods().next(&bondMethod));
}
...
bool TR_CHTable::commit(TR::Compilation *comp)
{
    ...
    if (_preXMethods) {
        ...
        for (i = 0; i <= last; ++i) {
            ...
            TR_PreXRecompileOnMethodOverride::make(comp->fe(), comp->trPersistentMemory(), method, startPC,
    ...
    if (_classes) {
        ...
        for (int32_t i = 0; i <= last; ++i) {
            ...
            if (!alreadyAdded) {
                ...
                TR_PreXRecompileOnClassExtend::make(comp->fe(), comp->trPersistentMemory(), classId, startPC,
                    comp->getMetadataAssumptionList());
    ...
    if (_classesThatShouldNotBeNewlyExtended) {
        ...
        for (auto cl = it.getFirst(); cl; cl = it.getNext()) {
            ...
            TR_PreXRecompileOnClassExtend::make(comp->fe(), comp->trPersistentMemory(), cl->getClassId(), startPC,
                comp->getMetadataAssumptionList());
            comp->setHasClassExtendAssumptions();
        }
    ...
    while (bondIter.next(&bondMethod)) {
        TR_ClassUnloadRecompile::make(comp->fe(), comp->trPersistentMemory(), bondMethod->containingClass(), startPC,
            comp->getMetadataAssumptionList());
    ...
    TR_Array<TR_OpaqueClassBlock *> *clazzesOnStaticFinalFieldModification
    ...
    for (int i = 0; i < clazzesOnStaticFinalFieldModification->size(); ++i) {
        ...
        if (TR::Compiler->cls.classHasIllegalStaticFinalFieldModification(clazz)) {
            if (TR::Options::isAnyVerboseOptionSet(TR_VerboseRuntimeAssumptions, TR_VerboseCompileEnd,
                    TR_VerbosePerformance, TR_VerboseCompFailure)) {
                TR_VerboseLog::writeLineLocked(TR_Vlog_FAILURE,
                    "Failure while commiting static final field assumption for class %p for %s", clazz,
                    comp->signature());
            }
            comp->failCompilation<TR::CompilationInterrupted>(
                "Compilation interrupted: Static final field of a class has been modified");
        }
    }
...
void TR_CHTable::commitOSRVirtualGuards(TR::Compilation *comp, const TR::Compilation::GuardSet &vguards)
{
    ...
    TR_Array<TR_OpaqueClassBlock *> *clazzesForStaticFinalFieldModification
        = comp->getClassesForStaticFinalFieldModification();
    if (osrSites == 0
        || (clazzesForOSRRedefinition->size() == 0 && clazzesForStaticFinalFieldModification->size() == 0)) {
        return;
    } else if (osrSites == 1) {
        // Only one patch point, create an assumption for each class
        for (int i = 0; i < clazzesForOSRRedefinition->size(); ++i)
            TR_PatchNOPedGuardSiteOnClassRedefinition ::make(comp->fe(), comp->trPersistentMemory(),
                (*clazzesForOSRRedefinition)[i], onlySite->getLocation(), onlySite->getDestination(),
                comp->getMetadataAssumptionList());

        for (int i = 0; i < clazzesForStaticFinalFieldModification->size(); ++i)
            TR_PatchNOPedGuardSiteOnStaticFinalFieldModification ::make(comp->fe(), comp->trPersistentMemory(),
                (*clazzesForStaticFinalFieldModification)[i], onlySite->getLocation(), onlySite->getDestination(),
                comp->getMetadataAssumptionList());
    } else if (osrSites > 1) {
        ...
        for (int i = 0; i < clazzesForOSRRedefinition->size(); ++i)
            TR_PatchMultipleNOPedGuardSitesOnClassRedefinition ::make(comp->fe(), comp->trPersistentMemory(),
                (*clazzesForOSRRedefinition)[i], points, comp->getMetadataAssumptionList());

        for (int i = 0; i < clazzesForStaticFinalFieldModification->size(); ++i)
            TR_PatchMultipleNOPedGuardSitesOnStaticFinalFieldModification ::make(comp->fe(), comp->trPersistentMemory(),
                (*clazzesForStaticFinalFieldModification)[i], points, comp->getMetadataAssumptionList());
    }
...
void TR_CHTable::commitVirtualGuard(TR_VirtualGuard *info, List<TR_VirtualGuardSite> &sites,
    TR_PersistentCHTable *table, TR::Compilation *comp)
{
            ...
            if (clazzesForRedefinition || clazzesForStaticFinalFieldModification) {
                ...
                for (TR_VirtualGuardSite *site = it.getFirst(); site; site = it.getNext()) {
                    for (uint32_t i = 0; i < clazzesForRedefinition->size(); ++i)
                        TR_PatchNOPedGuardSiteOnClassRedefinition ::make(comp->fe(), comp->trPersistentMemory(),
                            (*clazzesForRedefinition)[i], site->getLocation(), site->getDestination(),
                            comp->getMetadataAssumptionList());
                    ...
                    for (uint32_t i = 0; i < clazzesForStaticFinalFieldModification->size(); ++i)
                        TR_PatchNOPedGuardSiteOnStaticFinalFieldModification ::make(comp->fe(),
                            comp->trPersistentMemory(), (*clazzesForStaticFinalFieldModification)[i],
    ...
    if ((info->getKind() == TR_HCRGuard) || info->mergedWithHCRGuard()) {
        ...
        for (TR_VirtualGuardSite *site = it.getFirst(); site; site = it.getNext()) {
            ...
            TR_PatchNOPedGuardSiteOnClassRedefinition ::make(comp->fe(), comp->trPersistentMemory(), hcrGuardedClass,
                site->getLocation(), site->getDestination(), comp->getMetadataAssumptionList());
            comp->setHasClassRedefinitionAssumptions();
        }
    ...
    if (info->getKind() == TR_DummyGuard) {
        ...
        if (!dontInvalidateMCSTargetGuards) {
            ...
            TR_ASSERT(!comp->isOutOfProcessCompilation(),
                "TR_CHTable::commitVirtualGuard() should not be called at the server\n");
#endif /* defined(J9VM_OPT_JITSERVER) */
            ...
            if (nopAssumptionIsValid) {
                ...
                for (TR_VirtualGuardSite *site = it.getFirst(); site; site = it.getNext()) {
                    TR_ASSERT(site->getLocation(), "assertion failure");
                    TR_PatchNOPedGuardSiteOnMutableCallSiteChange ::make(comp->fe(), comp->trPersistentMemory(), cookie,
                        site->getLocation(), site->getDestination(), comp->getMetadataAssumptionList());
                }
            } else
                logprintf(trace, log,
        ...
        if (comp->fej9()->isMethodBreakpointed(method)) {
            ...
            ListIterator<TR_VirtualGuardSite> it(&sites);
            for (TR_VirtualGuardSite *site = it.getFirst(); site; site = it.getNext()) {
                TR_PatchNOPedGuardSiteOnMethodBreakPoint ::make(comp->fe(), comp->trPersistentMemory(), method,
                    site->getLocation(), site->getDestination(), comp->getMetadataAssumptionList());
            }
        }
        ...
        TR_ResolvedMethod *implementer
            = table->findSingleImplementer(thisClass, cpIndex, owningMethod, comp, true, TR_yes);
        if (!implementer
        ...
        if (guardedMethod->virtualMethodIsOverridden())
            nopAssumptionIsValid = false;
                   ...
                   && !TR::Compiler->cls.isAbstractClass(comp,
        ...
        TR_OpaqueClassBlock *thisClass = info->isInlineGuard()
            ? info->getThisClass()
        ...
        if (table->isOverriddenInThisHierarchy(guardedMethod, thisClass, symRef->getOffset(), comp))
            nopAssumptionIsValid = false;
    } else if (info->isInlineGuard() && info->getTestType() == TR_VftTest
    ...
    if (nopAssumptionIsValid) {
        ...
        for (TR_VirtualGuardSite *site = it.getFirst(); site; site = it.getNext()) {
            TR_ASSERT(site->getLocation(), "assertion failure");
            if (guardedClass) {
                TR_PatchNOPedGuardSiteOnClassExtend ::make(comp->fe(), comp->trPersistentMemory(), guardedClass,
                    site->getLocation(), site->getDestination(), comp->getMetadataAssumptionList());
                comp->setHasClassExtendAssumptions();
            }
            if (guardedMethod) {
                TR_PatchNOPedGuardSiteOnMethodOverride ::make(comp->fe(), comp->trPersistentMemory(),
                    guardedMethod->getPersistentIdentifier(), site->getLocation(), site->getDestination(),
                    comp->getMetadataAssumptionList());
                comp->setHasMethodOverrideAssumptions();
            }
        }
...
VirtualGuardInfoForCHTable getImportantVGuardInfo(TR::Compilation *comp, TR_VirtualGuard *vguard)
{
    ...
    if (symRef) {
        ...
        info._owningMethod
            = static_cast<TR_ResolvedJ9JITServerMethod *>(symRef->getOwningMethod(comp))->getRemoteMirror();
        info._isInterface = methodSymbol->isInterface();
        info._guardedMethod = resolvedMethodSymbol
            ? static_cast<TR_ResolvedJ9JITServerMethod *>(resolvedMethodSymbol->getResolvedMethod())->getRemoteMirror()
            : NULL;
    ...
    info._inlinedResolvedMethod = info._kind == TR_BreakpointGuard
        ? static_cast<TR_ResolvedJ9JITServerMethod *>(comp->getInlinedResolvedMethod(info._calleeIndex))
              ->getRemoteMirror()
        : NULL;
...
CHTableCommitData TR_CHTable::computeDataForCHTableCommit(TR::Compilation *comp)
{
    ...
    return std::make_tuple(classes, classesThatShouldNotBeNewlyExtended, preXMethods, sideEffectPatchSitesVec,
        ...
        classesForOSRRedefinition, classesForStaticFinalFieldModification, inlinedSiteInfo, keepaliveMethods,
        bondMethods, startPC);
}
#endif /* defined(J9VM_OPT_JITSERVER) */
...
public:
    CollectImplementors(TR::Compilation *comp, TR_OpaqueClassBlock *topClassId, TR_ResolvedMethod **implArray,
        int32_t maxCount, TR_ResolvedMethod *callerMethod, int32_t slotOrIndex,
...
bool CollectImplementors::isInterface()
{
...
bool CollectImplementors::addImplementor(TR_ResolvedMethod *implementor)
{
    TR_ASSERT_FATAL(_count < _maxCount, "Max implementor count exceeded: _maxCount = %d, _count = %d", _maxCount,
        _count);
...
public:
    CollectCompiledImplementors(TR::Compilation *comp, TR_OpaqueClassBlock *topClassId, TR_ResolvedMethod **implArray,
        int32_t maxCount, TR_ResolvedMethod *callerMethod, int32_t slotOrIndex, TR_Hotness hotness,
        TR_YesNoMaybe useGetResolvedInterfaceMethod = TR_maybe)
        : CollectImplementors(comp, topClassId, implArray, maxCount + 1, callerMethod, slotOrIndex,
        ...
        if (currentCount < _count) {
            if (!TR::Compiler->mtd.isCompiledMethod((_implArray[_count - 1])->getPersistentIdentifier())) {
                ...
                TR_PersistentJittedBodyInfo *bodyInfo
                    = ((TR_ResolvedJ9Method *)_implArray[_count - 1])->getExistingJittedBodyInfo();
...
void TR_ClassQueries::getSubClasses(TR_PersistentClassInfo *clazz, TR_ScratchList<TR_PersistentClassInfo> &list,
    TR_FrontEnd *fe, bool locked)
{
...
int32_t TR_ClassQueries::collectImplementorsCapped(TR_PersistentClassInfo *clazz, TR_ResolvedMethod **implArray,
    int32_t maxCount, int32_t slotOrIndex, TR_ResolvedMethod *callerMethod, TR::Compilation *comp, bool locked,
...
#if defined(J9VM_OPT_JITSERVER)
    if (comp->isOutOfProcessCompilation()) {
        return static_cast<TR_ResolvedJ9JITServerMethod *>(callerMethod)
            ->collectImplementorsCapped(clazz->getClassId(), maxCount, slotOrIndex, useGetResolvedInterfaceMethod,
                implArray);
    }
#endif
    CollectImplementors collector(comp, clazz->getClassId(), implArray, maxCount, callerMethod, slotOrIndex,
        useGetResolvedInterfaceMethod);
...
int32_t TR_ClassQueries::collectCompiledImplementorsCapped(TR_PersistentClassInfo *clazz, TR_ResolvedMethod **implArray,
    int32_t maxCount, int32_t slotOrIndex, TR_ResolvedMethod *callerMethod, TR::Compilation *comp, TR_Hotness hotness,
    bool locked, TR_YesNoMaybe useGetResolvedInterfaceMethod)
{
    if (comp->getOption(TR_DisableCHOpts))
        return maxCount + 1; // fail the collection
    CollectCompiledImplementors collector(comp, clazz->getClassId(), implArray, maxCount, callerMethod, slotOrIndex,
        hotness, useGetResolvedInterfaceMethod);
    collector.visitSubclass(clazz);
    collector.visit(clazz->getClassId(), locked);
    return collector.getCount();
}

void TR_ClassQueries::collectLeafs(TR_PersistentClassInfo *clazz, TR_ScratchList<TR_PersistentClassInfo> &leafs,
...
void TR_ClassQueries::collectLeafsLocked(TR_PersistentClassInfo *clazz, TR_ScratchList<TR_PersistentClassInfo> &leafs,
...
void TR_ClassQueries::collectAllSubClasses(TR_PersistentClassInfo *clazz, TR_ScratchList<TR_PersistentClassInfo> *leafs,
    TR::Compilation *comp, bool locked)
{
 ...
 */
void TR_ClassQueries::collectAllSubClassesLocked(TR_PersistentClassInfo *clazz,
    TR_ScratchList<TR_PersistentClassInfo> *leafs, TR_CHTable::VisitTracker &marked)
{
    TR_SubClass *subClass = clazz->_subClasses.getFirst();
...
void TR_ClassQueries::collectAllNonIFSubClasses(TR_PersistentClassInfo *clazz,
...
void TR_ClassQueries::addAnAssumptionForEachSubClass(TR_PersistentCHTable *table, TR_PersistentClassInfo *clazz,
    List<TR_VirtualGuardSite> &list, TR::Compilation *comp)
{
...
int32_t TR_ClassQueries::countAllNonIFSubClassesWithDepth(TR_PersistentClassInfo *clazz, TR::Compilation *comp,
    int32_t maxcount, bool locked)
{
...
void TR_SubclassVisitor::visit(TR_OpaqueClassBlock *clazz, bool locked)
{
    ...
    //
    _mightVisitAClassMoreThanOnce
        = TR::Compiler->cls.isInterfaceClass(comp(), clazz) || TR::Compiler->cls.classDepthOf(clazz) == 0;

    if (_trace && classInfo->getFirstSubclass()) {
        int32_t len;
        char *s = TR::Compiler->cls.classNameChars(comp(), clazz, len);
...
void TR_SubclassVisitor::visitSubclasses(TR_PersistentClassInfo *classInfo, TR_CHTable::VisitTracker &visited)
{
    ++_depth;
    for (TR_SubClass *subclass = classInfo->getFirstSubclass(); subclass; subclass = subclass->getNext()) {
        ...
        if (!subclassInfo->hasBeenVisited()) {
            if (_trace) {
                int32_t len;
                char *s = TR::Compiler->cls.classNameChars(comp(), subclassInfo->getClassId(), len);
