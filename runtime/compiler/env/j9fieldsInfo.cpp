...
static int isReferenceSignature(char *signature) { return ((signature[0] == 'L') || (signature[0] == '[')); }

int TR_VMField::isReference() { return (isReferenceSignature(signature)); }
...
TR_VMField::TR_VMField(TR::Compilation *comp, J9Class *aClazz, J9ROMFieldShape *fieldShape, TR_AllocationKind allocKind)
{
    ...
    if (modifiers & J9AccStatic)
        ...
        offset = comp->fej9()->getInstanceFieldOffset((TR_OpaqueClassBlock *)aClazz, name, (uint32_t)(nameLength - 1),
            signature, (uint32_t)(sigLength - 1));
}

void TR_VMField::print(OMR::Logger *log, TR_FrontEnd *fe)
{
...
TR_VMFieldsInfo::TR_VMFieldsInfo(TR::Compilation *comp, J9Class *aClazz, int buildFields, TR_AllocationKind allocKind)
    ...
    , _comp(comp)
    , _gcDescriptor(
          allocKind == stackAlloc ? comp->trMemory()->currentStackRegion() : comp->trMemory()->heapMemoryRegion())
    , _fields(NULL)
    , _statics(NULL)
    , _allocKind(allocKind)
{
    ...
    // self
    TR_ASSERT(!(aClazz->romClass->modifiers & J9AccClassArray), "Cannot construct TR_VMFieldsInfo for array class %p",
        aClazz);
    ...
    while (1) {
        ...
        if ((countSlots >= (slotsInHeader + numSlotsInObject)) || (useImmediate && bitIndex == (numBitsInWord - 1))) {
            break;
        }
...
void TR_VMFieldsInfo::collectFieldInfo(J9Class *aClazz)
{
...
void TR_VMFieldsInfo::buildField(J9Class *aClazz, J9ROMFieldShape *fieldShape)
{
