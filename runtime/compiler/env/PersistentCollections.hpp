...
template<typename T> using PersistentVectorAllocator = TR::typed_allocator<T, TR::PersistentAllocator &>;
template<typename T> using PersistentVector = std::vector<T, PersistentVectorAllocator<T> >;

template<typename T> using PersistentListAllocator = TR::typed_allocator<T, TR::PersistentAllocator &>;
template<typename T> using PersistentList = std::list<T, PersistentListAllocator<T> >;

template<typename T> using PersistentUnorderedSetAllocator = TR::typed_allocator<T, TR::PersistentAllocator &>;
...
template<typename T> using VectorAllocator = TR::typed_allocator<T, TR::Region &>;
template<typename T> using Vector = std::vector<T, VectorAllocator<T> >;

template<typename T> using UnorderedSetAllocator = TR::typed_allocator<T, TR::Region &>;
template<typename T, typename H = std::hash<T>, typename E = std::equal_to<T> >
using UnorderedSet = std::unordered_set<T, H, E, UnorderedSetAllocator<T> >;

template<typename K, typename V> using UnorderedMapAllocator = TR::typed_allocator<std::pair<const K, V>, TR::Region &>;
template<typename K, typename V, typename H = std::hash<K>, typename E = std::equal_to<K> >
...
struct StringKey {
    StringKey()
        : _string(NULL)
        , _length(0)
    {} // Safely represents an empty string

    StringKey(const uint8_t *string, size_t length)
        : _string(string)
        , _length(length)
    {}

    StringKey(const J9UTF8 *str)
        : _string(str ? J9UTF8_DATA(str) : NULL)
        , _length(str ? J9UTF8_LENGTH(str) : 0)
    {}
