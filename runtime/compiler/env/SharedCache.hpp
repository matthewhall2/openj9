...
public:
    ...
    virtual uintptr_t rememberClass(TR_OpaqueClassBlock *clazz,
    ...
    // A uintptr_t value that can never represent a class chain in an SCC. Used to check the return value of
    // rememberClass, and also useful to represent "class chain not present" in places where a class chain offset is
    // optional.
    static const uintptr_t INVALID_CLASS_CHAIN_OFFSET = 0;

    // A uintptr_t value that can never represent a ROM method in an SCC. Used to check the return value of
    // isMethodInSharedCache and related methods.
