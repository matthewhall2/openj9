...
#pragma warning(suppress: 4351)
        ...
        , _lateSCCDisclaimTime(300000000000)
        , // 300s
    ...
    void setGCwillBlockOnClassUnloadMonitor()
    {
        _GCwillBlockOnClassUnloadMonitor = true;
        _timeGCwillBlockOnClassUnloadMonitorWasSet = _elapsedTime;
    }
    ...
    TR_MHJ2IThunkTable *getInvokeExactJ2IThunkTable()
    {
        return _invokeExactJ2IThunkTable;
    } // NULL if the platform needs no thunks, so J2I helpers can be called directly
    ...
    bool isRuntimeInstrumentationRecompilationEnabled()
    {
        return _runtimeInstrumentationEnabled && _runtimeInstrumentationRecompilationEnabled;
    }
...
#if defined(J9VM_OPT_JITSERVER)
    static JITServer::RemoteCompilationModes
        _remoteCompilationMode; // JITServer::NONE, JITServer::CLIENT, JITServer::SERVER
...
private:
    ...
    bool _hasEverConnectedToServer; // At the client, true if the client has connected to a server at some point in the
                                    // past
    ...
    bool _JITServerAOTCacheDelayMethodRelocation; // At the client, whether to delay deserialized method relocation or
                                                  // not
    // At the client, whether or not to use the new AOT cache implementation (with serialization record IDs as SCC
    // offsets)
