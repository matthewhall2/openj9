...
public:
    ...
    TR_AOTGuardSite()
        : TR_VirtualGuardSite()
        , _guardType(TR_NoGuard)
        , _virtualGuard(NULL)
        , _node(NULL)
        , _aconstNode(NULL)
    {}

    TR_AOTGuardSite(TR_VirtualGuardKind type, TR_VirtualGuard *virtualGuard, TR::Node *node,
        TR::Node *aconstNode = NULL)
        : TR_VirtualGuardSite()
        , _guardType(type)
        , _virtualGuard(virtualGuard)
        , _node(node)
        , _aconstNode(node)
    {}
...
protected:
    TR_PatchNOPedGuardSiteOnClassExtend(TR_PersistentMemory *pm, TR_OpaqueClassBlock *clazz, uint8_t *loc,
        uint8_t *dest)
        : TR::PatchNOPedGuardSite(pm, (uintptr_t)clazz, RuntimeAssumptionOnClassExtend, loc, dest)
    {}

public:
    static TR_PatchNOPedGuardSiteOnClassExtend *make(TR_FrontEnd *fe, TR_PersistentMemory *pm,
...
protected:
    TR_PatchNOPedGuardSiteOnMethodOverride(TR_PersistentMemory *pm, TR_OpaqueMethodBlock *method, uint8_t *loc,
        uint8_t *dest)
        : TR::PatchNOPedGuardSite(pm, (uintptr_t)method, RuntimeAssumptionOnMethodOverride, loc, dest)
    {}

public:
    static TR_PatchNOPedGuardSiteOnMethodOverride *make(TR_FrontEnd *fe, TR_PersistentMemory *pm,
        TR_OpaqueMethodBlock *method, uint8_t *loc, uint8_t *dest, OMR::RuntimeAssumption **sentinel);
...
public:
    static TR_PatchNOPedGuardSiteOnClassPreInitialize *make(TR_FrontEnd *fe, TR_PersistentMemory *, char *sig,
        uint32_t sigLen, uint8_t *loc, uint8_t *dest, OMR::RuntimeAssumption **sentinel);
...
private:
    TR_PatchNOPedGuardSiteOnClassPreInitialize(TR_PersistentMemory *pm, char *sig, uint32_t sigLen, uint8_t *loc,
        uint8_t *dest)
        : TR::PatchNOPedGuardSite(pm, (uintptr_t)sig, RuntimeAssumptionOnClassPreInitialize, loc, dest)
        , _sigLen(sigLen)
    {}
...
protected:
    TR_PatchNOPedGuardSiteOnClassRedefinition(TR_PersistentMemory *pm, TR_OpaqueClassBlock *clazz, uint8_t *loc,
        uint8_t *dest)
        : TR::PatchNOPedGuardSite(pm, (uintptr_t)clazz, RuntimeAssumptionOnClassRedefinitionNOP, loc, dest)
    {}

public:
    static TR_PatchNOPedGuardSiteOnClassRedefinition *make(TR_FrontEnd *fe, TR_PersistentMemory *pm,
        TR_OpaqueClassBlock *clazz, uint8_t *loc, uint8_t *dest, OMR::RuntimeAssumption **sentinel);
...
protected:
    TR_PatchMultipleNOPedGuardSitesOnClassRedefinition(TR_PersistentMemory *pm, TR_OpaqueClassBlock *clazz,
        TR::PatchSites *sites)
        : TR::PatchMultipleNOPedGuardSites(pm, (uintptr_t)clazz, RuntimeAssumptionOnClassRedefinitionNOP, sites)
    {}

public:
    static TR_PatchMultipleNOPedGuardSitesOnClassRedefinition *make(TR_FrontEnd *fe, TR_PersistentMemory *pm,
...
public:
    static TR_PatchNOPedGuardSiteOnStaticFinalFieldModification *make(TR_FrontEnd *fe, TR_PersistentMemory *pm,
        TR_OpaqueClassBlock *clazz, uint8_t *loc, uint8_t *dest, OMR::RuntimeAssumption **sentinel);
...
protected:
    TR_PatchMultipleNOPedGuardSitesOnStaticFinalFieldModification(TR_PersistentMemory *pm, TR_OpaqueClassBlock *clazz,
        TR::PatchSites *sites)
        : TR::PatchMultipleNOPedGuardSites(pm, (uintptr_t)clazz, RuntimeAssumptionOnStaticFinalFieldModification, sites)
    {}

public:
    static TR_PatchMultipleNOPedGuardSitesOnStaticFinalFieldModification *make(TR_FrontEnd *fe, TR_PersistentMemory *pm,
        TR_OpaqueClassBlock *clazz, TR::PatchSites *sites, OMR::RuntimeAssumption **sentinel);
...
protected:
    TR_PatchNOPedGuardSiteOnMutableCallSiteChange(TR_PersistentMemory *pm, uintptr_t key, uint8_t *loc, uint8_t *dest)
        : TR::PatchNOPedGuardSite(pm, key, RuntimeAssumptionOnMutableCallSiteChange, loc, dest)
    {}

public:
    static TR_PatchNOPedGuardSiteOnMutableCallSiteChange *make(TR_FrontEnd *fe, TR_PersistentMemory *pm,
        uintptr_t KEY_WILL_GO_HERE, uint8_t *loc, uint8_t *dest, OMR::RuntimeAssumption **sentinel);
...
protected:
    TR_PatchNOPedGuardSiteOnMethodBreakPoint(TR_PersistentMemory *pm, TR_OpaqueMethodBlock *j9method, uint8_t *location,
        uint8_t *destination)
        : TR::PatchNOPedGuardSite(pm, (uintptr_t)j9method, RuntimeAssumptionOnMethodBreakPoint, location, destination)
    {}

public:
    static TR_PatchNOPedGuardSiteOnMethodBreakPoint *make(TR_FrontEnd *fe, TR_PersistentMemory *pm,
        TR_OpaqueMethodBlock *j9method, uint8_t *location, uint8_t *destination, OMR::RuntimeAssumption **sentinel);
...
public:
    ...
    static TR_PatchJNICallSite *make(TR_FrontEnd *fe, TR_PersistentMemory *pm, uintptr_t key, uint8_t *pc,
...
protected:
    TR_PreXRecompile(TR_PersistentMemory *pm, uintptr_t key, TR_RuntimeAssumptionKind kind, uint8_t *startPC)
        : OMR::LocationRedirectRuntimeAssumption(pm, key)
        , _startPC(startPC)
    {}
...
public:
    ...
    static TR_PreXRecompileOnClassExtend *make(TR_FrontEnd *fe, TR_PersistentMemory *pm, TR_OpaqueClassBlock *clazz,
        uint8_t *startPC, OMR::RuntimeAssumption **sentinel);
};
...
public:
    ...
    static TR_PreXRecompileOnMethodOverride *make(TR_FrontEnd *fe, TR_PersistentMemory *pm,
...
protected:
    TR_ClassUnloadRecompile(TR_PersistentMemory *pm, TR_OpaqueClassBlock *clazz, uint8_t *startPC)
        : OMR::LocationRedirectRuntimeAssumption(pm, (uintptr_t)clazz)
        , _startPC(startPC)
    {}

public:
    static TR_ClassUnloadRecompile *make(TR_FrontEnd *fe, TR_PersistentMemory *pm, TR_OpaqueClassBlock *clazz,
        uint8_t *startPC, OMR::RuntimeAssumption **sentinel);
...
public:
    ...
    public:
        ...
        {
            _classes.add(info); // this could throw std::bad_alloc and take us to the exception handler, before which
                                // the destructor is called
            info->setVisited(); // it's important to set the visited flag *after* adding to marked list
        }

        ListIterator<TR_PersistentClassInfo> iterator() { return ListIterator<TR_PersistentClassInfo>(&_classes); }
    ...
    void commitVirtualGuard(TR_VirtualGuard *info, List<TR_VirtualGuardSite> &sites, TR_PersistentCHTable *table,
        TR::Compilation *comp);
    void commitOSRVirtualGuards(TR::Compilation *comp, const TR::Compilation::GuardSet &vguards);
#if defined(J9VM_OPT_JITSERVER)
    ...
    TR_Array<TR_OpaqueClassBlock *> *getClassesThatShouldNotBeNewlyExtended()
    {
...
public:
    TR_PersistentFieldInfo(char *sig, int32_t sigLength = -1, TR_PersistentFieldInfo *next = NULL,
        uint8_t b = VALID_BUT_NOT_ALWAYS_INITIALIZED, const char *c = 0, int32_t numChars = -1)
...
public:
    TR_PersistentArrayFieldInfo(char *sig, int32_t sigLength = -1, TR_PersistentArrayFieldInfo *next = NULL,
        int32_t numDimensions = -1, int32_t *dimensionInfo = NULL, uint8_t b1 = VALID_BUT_NOT_ALWAYS_INITIALIZED,
        uint8_t b2 = VALID_BUT_NOT_ALWAYS_INITIALIZED, const char *c = "", int32_t numChars = -1)
...
public:
    static void getSubClasses(TR_PersistentClassInfo *clazz, TR_ScratchList<TR_PersistentClassInfo> &list,
        TR_FrontEnd *vm, bool locked = false);
    static int32_t collectImplementorsCapped(TR_PersistentClassInfo *clazz, TR_ResolvedMethod **implArray,
        int32_t maxCount, int32_t slotOrIndex, TR_ResolvedMethod *callerMethod, TR::Compilation *comp,
        bool locked = false, TR_YesNoMaybe useGetResolvedInterfaceMethod = TR_maybe);
    static int32_t collectCompiledImplementorsCapped(TR_PersistentClassInfo *clazz, TR_ResolvedMethod **implArray,
        int32_t maxCount, int32_t slotOrIndex, TR_ResolvedMethod *callerMethod, TR::Compilation *comp,
        TR_Hotness hotness, bool locked = false, TR_YesNoMaybe useGetResolvedInterfaceMethod = TR_maybe);

    static void collectLeafs(TR_PersistentClassInfo *clazz, TR_ScratchList<TR_PersistentClassInfo> &list,
        TR::Compilation *comp, bool locked = false);

    static void collectAllNonIFSubClasses(TR_PersistentClassInfo *clazz, TR_ScratchList<TR_PersistentClassInfo> &leafs,
        TR::Compilation *comp, bool locked = false);

    static void collectAllSubClasses(TR_PersistentClassInfo *clazz, TR_ScratchList<TR_PersistentClassInfo> *leafs,
        TR::Compilation *comp, bool locked = false);

    static int32_t countAllNonIFSubClassesWithDepth(TR_PersistentClassInfo *clazz, TR::Compilation *comp,
        int32_t maxCount, bool locked = false);

    static void addAnAssumptionForEachSubClass(TR_PersistentCHTable *table, TR_PersistentClassInfo *clazz,
        List<TR_VirtualGuardSite> &, TR::Compilation *comp);

private:
    static void collectLeafsLocked(TR_PersistentClassInfo *clazz, TR_ScratchList<TR_PersistentClassInfo> &leafs,
        TR_CHTable::VisitTracker &marked);
    static void collectAllSubClassesLocked(TR_PersistentClassInfo *clazz, TR_ScratchList<TR_PersistentClassInfo> *leafs,
        TR_CHTable::VisitTracker &marked);
};
