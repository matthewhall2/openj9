...
#define x(a, b, c) a, sizeof(b) - 1, b, (int16_t)strlen(c), c
   ...
   static X StringMethods[] =
      ...
      /* Matching is done top to bottom so the wildcard match for "<init>" must be last or it will override the more specialized matches. */
   ...
   static X AbstractStringBuilderMethods[] =
      {
      {x(TR::java_lang_AbstractStringBuilder_append_char,            "append",                 "(C)Ljava/lang/AbstractStringBuilder;")},
...
*/
   ...
   static X EncodeMethods[] =
      ...
      {x(TR::sun_nio_cs_ISO_8859_1_Decoder_decodeISO8859_1,       "decodeISO8859_1",      "([BII[CI)I")}, // Used in IBM SDK Java 8
      ...
      {x(TR::sun_nio_cs_UTF16_Encoder_encodeUTF16Little,          "encodeUTF16Little",    "([CII[BI)I")}, // Used in IBM SDK Java 8
      {  TR::unknownMethod}
      };
         ...
         for (
            ...
            if (targetType[0] == 'L')
               ...
               if (fej9->isInterfaceClass(targetParmClass) && isExplicit)
                  ...
                  {
                  logprintf(trace(), log, "source type and target type are compatible, omit checkcast for arg %d\n", argIndex);
                  }
               else
                  {
                  push(argValue);
