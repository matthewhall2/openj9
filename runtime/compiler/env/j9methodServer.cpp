...
static J9ROMMethod *romMethodAtClassIndex(J9ROMClass *romClass, uint64_t methodIndex)
{
...
TR_ResolvedJ9JITServerMethod::TR_ResolvedJ9JITServerMethod(TR_OpaqueMethodBlock *aMethod, TR_FrontEnd *fe,
    TR_Memory *trMemory, TR_ResolvedMethod *owningMethod, uint32_t vTableSlot)
    : TR_ResolvedJ9Method(fe, owningMethod)
{
    ...
    TR_ResolvedJ9Method *owningMethodMirror
        = owningMethod ? ((TR_ResolvedJ9JITServerMethod *)owningMethod)->_remoteMirror : NULL;

    // If in AOT mode, will actually create relocatable version of resolved method on the client
    _stream->write(JITServer::MessageType::mirrorResolvedJ9Method, aMethod, owningMethodMirror, vTableSlot,
        fej9->isAOT_DEPRECATED_DO_NOT_USE());
...
TR_ResolvedJ9JITServerMethod::TR_ResolvedJ9JITServerMethod(TR_OpaqueMethodBlock *aMethod, TR_FrontEnd *fe,
    TR_Memory *trMemory, const TR_ResolvedJ9JITServerMethodInfo &methodInfo, TR_ResolvedMethod *owningMethod,
    uint32_t vTableSlot)
    : TR_ResolvedJ9Method(fe, owningMethod)
{
...
J9ROMClass *TR_ResolvedJ9JITServerMethod::romClassPtr() { return _romClass; }

J9RAMConstantPoolItem *TR_ResolvedJ9JITServerMethod::literals() { return _literals; }
...
J9ClassLoader *TR_ResolvedJ9JITServerMethod::getClassLoader()
{
...
bool TR_ResolvedJ9JITServerMethod::staticAttributes(TR::Compilation *comp, I_32 cpIndex, void **address,
    ...
    bool needAOTValidation)
{
    ...
    if (!getCachedFieldAttributes(cpIndex, attributes, isStatic)) {
        ...
        _stream->write(JITServer::MessageType::ResolvedMethod_staticAttributes, _remoteMirror, cpIndex, isStore,
            needAOTValidation);
        ...
        TR_ASSERT(validateMethodFieldAttributes(attributes, isStatic, cpIndex, isStore, needAOTValidation),
            "static attributes from client and cache do not match");
    }
...
TR_OpaqueClassBlock *TR_ResolvedJ9JITServerMethod::definingClassFromCPFieldRef(TR::Compilation *comp, int32_t cpIndex,
    bool isStatic, TR_OpaqueClassBlock **fromResolvedJ9Method)
{
    ...
    _stream->write(JITServer::MessageType::ResolvedMethod_definingClassFromCPFieldRef, _remoteMirror, cpIndex,
        isStatic);
...
TR_OpaqueClassBlock *TR_ResolvedJ9JITServerMethod::getClassFromConstantPool(TR::Compilation *comp, uint32_t cpIndex,
    bool returnClassForAOT)
{
    ...
    bool doRuntimeResolve
        = compInfoPT->getClientData()->getRtResolve() && !comp->ilGenRequest().details().isMethodHandleThunk();
    if (doRuntimeResolve && performTransformation(comp, "Setting as unresolved class from CP cpIndex=%d\n", cpIndex)) {
        return NULL;
    }
    ...
    _stream->write(JITServer::MessageType::ResolvedMethod_getClassFromConstantPool, _remoteMirror, cpIndex,
        returnClassForAOT);
    TR_OpaqueClassBlock *resolvedClass = std::get<0>(_stream->read<TR_OpaqueClassBlock *>());
...
TR_OpaqueClassBlock *TR_ResolvedJ9JITServerMethod::getDeclaringClassFromFieldOrStatic(TR::Compilation *comp,
    int32_t cpIndex)
{
    TR::CompilationInfoPerThread *compInfoPT = _fe->_compInfoPT;
    {
...
TR_OpaqueClassBlock *TR_ResolvedJ9JITServerMethod::classOfStatic(I_32 cpIndex, bool returnClassForAOT)
{
...
void *TR_ResolvedJ9JITServerMethod::getConstantDynamicTypeFromCP(I_32 cpIndex)
{
    ...
    J9UTF8 *constantDynamicType
        = str2utf8(retConstantDynamicTypeStr.data(), retConstantDynamicTypeStr.length(), trMemory, heapAlloc);
    return constantDynamicType;
}

bool TR_ResolvedJ9JITServerMethod::isConstantDynamic(I_32 cpIndex)
{
    TR_ASSERT_FATAL(cpIndex != -1, "ConstantDynamic cpIndex shouldn't be -1");
...
bool TR_ResolvedJ9JITServerMethod::isUnresolvedConstantDynamic(I_32 cpIndex)
{
...
void *TR_ResolvedJ9JITServerMethod::dynamicConstant(I_32 cpIndex, uintptr_t *obj)
{
    TR_ASSERT_FATAL(cpIndex != -1, "ConstantDynamic cpIndex shouldn't be -1");
...
TR_ResolvedMethod *TR_ResolvedJ9JITServerMethod::getResolvedPossiblyPrivateVirtualMethod(TR::Compilation *comp,
    I_32 cpIndex, bool ignoreRtResolve, bool *unresolvedInCP)
{
    ...
    if (!((compInfoPT->getClientData()->getRtResolve()) && !comp->ilGenRequest().details().isMethodHandleThunk()
            && // cmvc 195373
            performTransformation(comp, "Setting as unresolved virtual call cpIndex=%d\n", cpIndex))
        || ignoreRtResolve || shouldCompileTimeResolve) {
        if (compInfoPT->getCachedResolvedMethod(compInfoPT->getResolvedMethodKey(TR_ResolvedMethodType::VirtualFromCP,
                                                    (TR_OpaqueClassBlock *)_ramClass, cpIndex),
                this, &resolvedMethod, unresolvedInCP)) {
        ...
        _stream->write(JITServer::MessageType::ResolvedMethod_getResolvedPossiblyPrivateVirtualMethodAndMirror,
            (TR_ResolvedMethod *)_remoteMirror, literals(), cpIndex);
        ...
        if (vTableIndex) {
            ...
            if (createResolvedMethod) {
                resolvedMethod
                    = createResolvedMethodFromJ9Method(comp, cpIndex, vTableIndex, ramMethod, aotStats, methodInfo);
                compInfoPT->cacheResolvedMethod(compInfoPT->getResolvedMethodKey(TR_ResolvedMethodType::VirtualFromCP,
                                                    (TR_OpaqueClassBlock *)_ramClass, cpIndex),
                    (TR_OpaqueMethodBlock *)ramMethod, (uint32_t)vTableIndex, methodInfo, *unresolvedInCP);
            }
        }
    }
...
TR_ResolvedMethod *TR_ResolvedJ9JITServerMethod::getResolvedVirtualMethod(TR::Compilation *comp, I_32 cpIndex,
...
bool TR_ResolvedJ9JITServerMethod::fieldsAreSame(int32_t cpIndex1, TR_ResolvedMethod *m2, int32_t cpIndex2,
    bool &sigSame)
{
    ...
    if (sig1Len == sig2Len && !memcmp(signature1, signature2, sig1Len) && name1Len == name2Len
        && !memcmp(name1, name2, name1Len)) {
...
bool TR_ResolvedJ9JITServerMethod::staticsAreSame(int32_t cpIndex1, TR_ResolvedMethod *m2, int32_t cpIndex2,
    bool &sigSame)
{
    auto comp = TR::comp();
...
TR_FieldAttributesCache &TR_ResolvedJ9JITServerMethod::getAttributesCache(bool isStatic, bool unresolvedInCP)
{
    // Return a persistent attributes cache for regular JIT compilations
    TR::CompilationInfoPerThread *compInfoPT = _fe->_compInfoPT;
    auto &attributesCache = isStatic ? JITServerHelpers::getJ9ClassInfo(compInfoPT, _ramClass)._staticAttributesCache
                                     : JITServerHelpers::getJ9ClassInfo(compInfoPT, _ramClass)._fieldAttributesCache;
    return attributesCache;
}

bool TR_ResolvedJ9JITServerMethod::getCachedFieldAttributes(int32_t cpIndex, TR_J9MethodFieldAttributes &attributes,
    bool isStatic)
{
    auto compInfoPT = static_cast<TR::CompilationInfoPerThreadRemote *>(_fe->_compInfoPT);
    {
    ...
    return compInfoPT->getCachedFieldOrStaticAttributes((TR_OpaqueClassBlock *)_ramClass, cpIndex, attributes,
        isStatic);
}

void TR_ResolvedJ9JITServerMethod::cacheFieldAttributes(int32_t cpIndex, const TR_J9MethodFieldAttributes &attributes,
    bool isStatic)
{
    ...
    if (attributes.isUnresolvedInCP()) {
        ...
        TR_ASSERT(canCacheFieldAttributes(cpIndex, attributes, isStatic),
            "new and cached field attributes are not equal");
#endif
        attributesCache.insert({ cpIndex, attributes });
    }
}

bool TR_ResolvedJ9JITServerMethod::canCacheFieldAttributes(int32_t cpIndex,
    const TR_J9MethodFieldAttributes &attributes, bool isStatic)
{
    auto &attributesCache = getAttributesCache(isStatic);
    auto it = attributesCache.find(cpIndex);
    if (it != attributesCache.end()) {
...
bool TR_ResolvedJ9JITServerMethod::fieldAttributes(TR::Compilation *comp, I_32 cpIndex, U_32 *fieldOffset,
    TR::DataType *type, bool *volatileP, bool *isFinal, bool *isPrivate, bool isStore, bool *unresolvedInCP,
    bool needAOTValidation)
{
    bool isStatic = false;
    ...
    if (!getCachedFieldAttributes(cpIndex, attributes, isStatic)) {
        // make a remote call, if attributes are not cached
        _stream->write(JITServer::MessageType::ResolvedMethod_fieldAttributes, _remoteMirror, cpIndex, isStore,
            needAOTValidation);
        auto recv = _stream->read<TR_J9MethodFieldAttributes>();
        attributes = std::get<0>(recv);
        ...
        TR_ASSERT(validateMethodFieldAttributes(attributes, isStatic, cpIndex, isStore, needAOTValidation),
            "field attributes from client and cache do not match");
    }

    bool result;
    attributes.setMethodFieldAttributesResult(fieldOffset, type, volatileP, isFinal, isPrivate, unresolvedInCP,
        &result);
    return result;
}
...
TR_ResolvedMethod *TR_ResolvedJ9JITServerMethod::getResolvedStaticMethod(TR::Compilation *comp, I_32 cpIndex,
    bool *unresolvedInCP)
{
    ...
    if (compInfoPT->getCachedResolvedMethod(
            compInfoPT->getResolvedMethodKey(TR_ResolvedMethodType::Static, (TR_OpaqueClassBlock *)_ramClass, cpIndex),
            this, &resolvedMethod, unresolvedInCP)) {
    ...
    if (resolvedMethod == NULL) {
        ...
        compInfoPT->cacheResolvedMethod(
            compInfoPT->getResolvedMethodKey(TR_ResolvedMethodType::Static, (TR_OpaqueClassBlock *)_ramClass, cpIndex),
            (TR_OpaqueMethodBlock *)ramMethod, 0, methodInfo, *unresolvedInCP);
    }
...
TR_ResolvedMethod *TR_ResolvedJ9JITServerMethod::getResolvedSpecialMethod(TR::Compilation *comp, I_32 cpIndex,
    bool *unresolvedInCP)
{
    // JITServer TODO: Decide whether the counters should be updated on the server or the client
    TR_ASSERT(cpIndex != -1, "cpIndex shouldn't be -1");
    ...
    if (compInfoPT->getCachedResolvedMethod(
            compInfoPT->getResolvedMethodKey(TR_ResolvedMethodType::Special, clazz, cpIndex), this, &resolvedMethod,
            unresolvedInCP)) {
        if ((resolvedMethod == NULL) && unresolvedInCP)
...
TR_ResolvedMethod *TR_ResolvedJ9JITServerMethod::createResolvedMethodFromJ9Method(TR::Compilation *comp,
    int32_t cpIndex, uint32_t vTableSlot, J9Method *j9Method, TR_AOTInliningStats *aotStats)
{
    TR_ResolvedMethod *m = new (comp->trHeapMemory())
        TR_ResolvedJ9JITServerMethod((TR_OpaqueMethodBlock *)j9Method, _fe, comp->trMemory(), this, vTableSlot);
...
{
    TR_ResolvedMethod *m = new (comp->trHeapMemory()) TR_ResolvedJ9JITServerMethod((TR_OpaqueMethodBlock *)j9Method,
        _fe, comp->trMemory(), methodInfo, this, vTableSlot);
    if (((TR_ResolvedJ9Method *)m)->isSignaturePolymorphicMethod()) {
        // Signature polymorphic method's signature varies at different call sites and will be different than its
        // declared signature
        int32_t signatureLength;
...
void *TR_ResolvedJ9JITServerMethod::startAddressForJittedMethod()
{
...
char *TR_ResolvedJ9JITServerMethod::localName(U_32 slotNumber, U_32 bcIndex, I_32 &len, TR_Memory *trMemory)
{
...
TR_OpaqueClassBlock *TR_ResolvedJ9JITServerMethod::getResolvedInterfaceMethod(I_32 cpIndex, UDATA *pITableIndex)
{
    // client will add the const provenance edge
...
TR_ResolvedMethod *TR_ResolvedJ9JITServerMethod::getResolvedInterfaceMethod(TR::Compilation *comp,
    TR_OpaqueClassBlock *classObject, I_32 cpIndex)
{
    ...
    TR_OpaqueClassBlock *clazz = (TR_OpaqueClassBlock *)_ramClass;
    if (compInfoPT->getCachedResolvedMethod(
            compInfoPT->getResolvedMethodKey(TR_ResolvedMethodType::Interface, clazz, cpIndex, classObject), this,
            &resolvedMethod)) {
        // No need to add a const provenance edge here. The cache is per-compilation.
        return resolvedMethod;
    }
    ...
    // client will add the const provenance edge
    _stream->write(JITServer::MessageType::ResolvedMethod_getResolvedInterfaceMethodAndMirror_3,
        getPersistentIdentifier(), classObject, cpIndex, _remoteMirror);
    ...
    if (resolvedMethod) {
        compInfoPT->cacheResolvedMethod(
            compInfoPT->getResolvedMethodKey(TR_ResolvedMethodType::Interface, clazz, cpIndex, classObject),
            (TR_OpaqueMethodBlock *)ramMethod, 0, methodInfo, true);
        return resolvedMethod;
    }
...
U_32 TR_ResolvedJ9JITServerMethod::getResolvedInterfaceMethodOffset(TR_OpaqueClassBlock *classObject, I_32 cpIndex)
{
    _stream->write(JITServer::MessageType::ResolvedMethod_getResolvedInterfaceMethodOffset, _remoteMirror, classObject,
        cpIndex);
 ...
 */
TR_ResolvedMethod *TR_ResolvedJ9JITServerMethod::getResolvedImproperInterfaceMethod(TR::Compilation *comp, I_32 cpIndex)
{
    TR_ASSERT(cpIndex != -1, "cpIndex shouldn't be -1");
#if TURN_OFF_INLINING
    return 0;
    ...
    if (!(_fe->_compInfoPT->getClientData()->getRtResolve())) {
        ...
        _stream->write(JITServer::MessageType::ResolvedMethod_getResolvedImproperInterfaceMethodAndMirror,
            _remoteMirror, cpIndex);
        ...
        compInfoPT->cacheResolvedMethod(compInfoPT->getResolvedMethodKey(TR_ResolvedMethodType::ImproperInterface,
                                            (TR_OpaqueClassBlock *)_ramClass, cpIndex),
            (TR_OpaqueMethodBlock *)j9method, vtableOffset, methodInfo, true);
        if (j9method == NULL)
            return NULL;
        else
...
void *TR_ResolvedJ9JITServerMethod::startAddressForInterpreterOfJittedMethod()
{
    _stream->write(JITServer::MessageType::ResolvedMethod_startAddressForInterpreterOfJittedMethod, _remoteMirror);
    return std::get<0>(_stream->read<void *>());
}

TR_ResolvedMethod *TR_ResolvedJ9JITServerMethod::getResolvedVirtualMethod(TR::Compilation *comp,
    TR_OpaqueClassBlock *classObject, I_32 virtualCallOffset, bool ignoreRtResolve)
{
    ...
    if (compInfoPT->getCachedResolvedMethod(compInfoPT->getResolvedMethodKey(TR_ResolvedMethodType::VirtualFromOffset,
    ...
    _stream->write(JITServer::MessageType::ResolvedMethod_getResolvedVirtualMethod, classObject, virtualCallOffset,
        ignoreRtResolve, (TR_ResolvedJ9Method *)getRemoteMirror());
    ...
    if (_fe->isAOT_DEPRECATED_DO_NOT_USE()) {
        ...
        resolvedMethod = ramMethod ? new (comp->trHeapMemory()) TR_ResolvedRelocatableJ9JITServerMethod(
                                         (TR_OpaqueMethodBlock *)ramMethod, _fe, comp->trMemory(), methodInfo, this)
                                   : 0;
    } else {
        resolvedMethod = ramMethod ? new (comp->trHeapMemory()) TR_ResolvedJ9JITServerMethod(
                                         (TR_OpaqueMethodBlock *)ramMethod, _fe, comp->trMemory(), methodInfo, this)
                                   : 0;
    }
    if (resolvedMethod)
        compInfoPT->cacheResolvedMethod(compInfoPT->getResolvedMethodKey(TR_ResolvedMethodType::VirtualFromOffset,
                                            clazz, virtualCallOffset, classObject),
            (TR_OpaqueMethodBlock *)ramMethod, 0, methodInfo, true);
    return resolvedMethod;
}
...
TR_OpaqueMethodBlock *TR_ResolvedJ9JITServerMethod::getTargetMethodFromMemberName(uintptr_t *invokeCacheArray,
    bool *isInvokeCacheAppendixNull)
{
    _stream->write(JITServer::MessageType::ResolvedMethod_getTargetMethodFromMemberName, _remoteMirror,
        invokeCacheArray);
...
TR_ResolvedMethod *TR_ResolvedJ9JITServerMethod::getResolvedHandleMethod(TR::Compilation *comp, I_32 cpIndex,
    bool *unresolvedInCP, bool *isInvokeCacheAppendixNull)
{
    ...
    if (comp->compileRelocatableCode()) {
        if (!unresolved) {
            bool valid = comp->getSymbolValidationManager()->addHandleMethodFromCPIndex(ramMethod,
                getNonPersistentIdentifier(), cpIndex, invokeCacheAppendixNull);

            if (!valid)
                comp->failCompilation<J9::AOTHasInvokeHandle>(
                    "Failed to add validation record for resolved handle method %p", ramMethod);
        } else {
    ...
    return static_cast<TR_J9ServerVM *>(_fe)->createResolvedMethodWithSignature(comp->trMemory(), ramMethod, NULL,
        (char *)signature.data(), signature.length(), this, methodInfo);
#endif
}

void *TR_ResolvedJ9JITServerMethod::methodTypeTableEntryAddress(int32_t cpIndex)
{
    _stream->write(JITServer::MessageType::ResolvedMethod_methodTypeTableEntryAddress, _remoteMirror, cpIndex);
    return std::get<0>(_stream->read<void *>());
}

bool TR_ResolvedJ9JITServerMethod::isUnresolvedMethodTypeTableEntry(int32_t cpIndex)
{
    _stream->write(JITServer::MessageType::ResolvedMethod_isUnresolvedMethodTypeTableEntry, _remoteMirror, cpIndex);
    return std::get<0>(_stream->read<bool>());
}

bool TR_ResolvedJ9JITServerMethod::isUnresolvedCallSiteTableEntry(int32_t callSiteIndex)
{
    _stream->write(JITServer::MessageType::ResolvedMethod_isUnresolvedCallSiteTableEntry, _remoteMirror, callSiteIndex);
    return std::get<0>(_stream->read<bool>());
}

void *TR_ResolvedJ9JITServerMethod::callSiteTableEntryAddress(int32_t callSiteIndex)
{
...
bool TR_ResolvedJ9JITServerMethod::isUnresolvedVarHandleMethodTypeTableEntry(int32_t cpIndex)
{
    _stream->write(JITServer::MessageType::ResolvedMethod_isUnresolvedVarHandleMethodTypeTableEntry, _remoteMirror,
        cpIndex);
    return std::get<0>(_stream->read<bool>());
}

void *TR_ResolvedJ9JITServerMethod::varHandleMethodTypeTableEntryAddress(int32_t cpIndex)
{
...
TR_ResolvedMethod *TR_ResolvedJ9JITServerMethod::getResolvedDynamicMethod(TR::Compilation *comp, I_32 callSiteIndex,
    bool *unresolvedInCP, bool *isInvokeCacheAppendixNull)
{
    ...
    if (comp->compileRelocatableCode()) {
        if (!unresolved) {
            bool valid = comp->getSymbolValidationManager()->addDynamicMethodFromCallsiteIndex(ramMethod,
                getNonPersistentIdentifier(), callSiteIndex, invokeCacheAppendixNull);

            if (!valid)
                comp->failCompilation<J9::AOTHasInvokeHandle>(
                    "Failed to add validation record for resolved dynamic method %p", ramMethod);
        } else {
            const char *className = "java/lang/invoke/MethodHandle";
            const char *methodName = "linkToStatic";
            const char *methodSignature = "([Ljava/lang/Object;)Ljava/lang/Object;";
    ...
    return static_cast<TR_J9ServerVM *>(_fe)->createResolvedMethodWithSignature(comp->trMemory(), ramMethod, NULL,
        (char *)signature.data(), signature.size(), this, methodInfo);
#endif
}

bool TR_ResolvedJ9JITServerMethod::shouldFailSetRecognizedMethodInfoBecauseOfHCR()
{
    _stream->write(JITServer::MessageType::ResolvedMethod_shouldFailSetRecognizedMethodInfoBecauseOfHCR, _remoteMirror);
    return std::get<0>(_stream->read<bool>());
}

bool TR_ResolvedJ9JITServerMethod::isSameMethod(TR_ResolvedMethod *m2)
{
...
bool TR_ResolvedJ9JITServerMethod::isInlineable(TR::Compilation *comp)
{
...
void TR_ResolvedJ9JITServerMethod::setWarmCallGraphTooBig(uint32_t bcIndex, TR::Compilation *comp)
{
...
void TR_ResolvedJ9JITServerMethod::setVirtualMethodIsOverridden()
{
    _stream->write(JITServer::MessageType::ResolvedMethod_setVirtualMethodIsOverridden, _remoteMirror);
    _stream->read<JITServer::Void>();
}

bool TR_ResolvedJ9JITServerMethod::methodIsNotzAAPEligible()
{
    _stream->write(JITServer::MessageType::ResolvedMethod_methodIsNotzAAPEligible, _remoteMirror);
    return std::get<0>(_stream->read<bool>());
}

void TR_ResolvedJ9JITServerMethod::setClassForNewInstance(J9Class *c)
{
...
TR_OpaqueClassBlock *TR_ResolvedJ9JITServerMethod::classOfMethod()
{
...
TR_PersistentJittedBodyInfo *TR_ResolvedJ9JITServerMethod::getExistingJittedBodyInfo()
{
    return _bodyInfo; // return cached value
}

void TR_ResolvedJ9JITServerMethod::getFaninInfo(uint32_t *count, uint32_t *weight, uint32_t *otherBucketWeight)
{
...
bool TR_ResolvedJ9JITServerMethod::getCallerWeight(TR_ResolvedJ9Method *caller, uint32_t *weight, uint32_t pcIndex)
{
    if (!_fe->getIProfiler())
        return false;
...
void TR_ResolvedJ9JITServerMethod::createResolvedMethodMirror(TR_ResolvedJ9JITServerMethodInfo &methodInfo,
    TR_OpaqueMethodBlock *method, uint32_t vTableSlot, TR_ResolvedMethod *owningMethod, TR_FrontEnd *fe,
    TR_Memory *trMemory)
{
    ...
    if (!((TR_J9VMBase *)fe)->isAOT_DEPRECATED_DO_NOT_USE())
        resolvedMethod
            = new (trMemory->trHeapMemory()) TR_ResolvedJ9Method(method, fe, trMemory, owningMethod, vTableSlot);
    else
        resolvedMethod = new (trMemory->trHeapMemory())
            TR_ResolvedRelocatableJ9Method(method, fe, trMemory, owningMethod, vTableSlot);
    if (!resolvedMethod)
        throw std::bad_alloc();
...
void TR_ResolvedJ9JITServerMethod::createResolvedMethodFromJ9MethodMirror(TR_ResolvedJ9JITServerMethodInfo &methodInfo,
    TR_OpaqueMethodBlock *method, uint32_t vTableSlot, TR_ResolvedMethod *owningMethod, TR_FrontEnd *fe,
    TR_Memory *trMemory)
{
    // Create resolved method mirror on the client.
    // Should be called to mirror a call to
    // TR_Resolved(Relocatable)MethodFromJ9Method::createResolvedMethodFromJ9Method. Requires owning method to exist to
    // work properly. Resolved method can be NULL.
    ...
    // Thanks to polymorphism, the correct version of the resolved method would be initialized and everything would be
    // simple. However, createResolvedMethodFromJ9Method is declared protected in TR_ResolvedJ9Method, so we can't call
    // it here. Maybe we should make it public, but that would require changing baseline. For now, I'll have to do with
    // mostly copy-pasting code from that method.
    TR_J9VMBase *fej9 = (TR_J9VMBase *)fe;
    ...
    && (defined(TR_HOST_X86) || defined(TR_HOST_POWER) || defined(TR_HOST_S390) || defined(TR_HOST_ARM) \
...

        {
            ...
            TR_OpaqueClassBlock *clazzOfCompiledMethod = fej9->convertClassPtrToClassOffset(
                J9_CLASS_FROM_METHOD(((TR_ResolvedJ9Method *)owningMethod)->ramMethod()));

            if (enableAggressive) {
                isSystemClassLoader = ((void *)fej9->vmThread()->javaVM->systemClassLoader->classLoaderObject
                    == (void *)fej9->getClassLoader(clazzOfInlinedMethod));
            }
            ...
            if (comp->ignoringLocalSCC() || fej9->sharedCache()->isClassInSharedCache(J9_CLASS_FROM_METHOD(j9method))) {
                ...
                if (resolveAOTMethods
                    ...
                    resolvedMethod = new (comp->trHeapMemory()) TR_ResolvedRelocatableJ9Method(
                        (TR_OpaqueMethodBlock *)j9method, fe, comp->trMemory(), owningMethod, vTableSlot);
                    if (!resolvedMethod)
                        throw std::bad_alloc();
                }
            }
...
void TR_ResolvedJ9JITServerMethod::packMethodInfo(TR_ResolvedJ9JITServerMethodInfo &methodInfo,
    TR_ResolvedJ9Method *resolvedMethod, TR_FrontEnd *fe)
{
    ...
    methodInfoStruct.startAddressForJittedMethod = TR::CompilationInfo::isCompiled(resolvedMethod->ramMethod())
        ? resolvedMethod->startAddressForJittedMethod()
        : NULL;
    ...
    methodInfoStruct.isLambdaFormGeneratedMethod
        = static_cast<TR_J9VMBase *>(fe)->isLambdaFormGeneratedMethod(resolvedMethod);
    ...
    std::string methodInfoStr
        = bodyInfo ? std::string((char *)bodyInfo->getMethodInfo(), sizeof(TR_PersistentMethodInfo)) : std::string();
    ...
    std::get<3>(methodInfo) = (comp && comp->getOptLevel() >= warm && iProfiler)
        ? iProfiler->serializeFaninMethodEntry(resolvedMethod->getPersistentIdentifier())
        : std::string();
}

void TR_ResolvedJ9JITServerMethod::unpackMethodInfo(TR_OpaqueMethodBlock *aMethod, TR_FrontEnd *fe, TR_Memory *trMemory,
    uint32_t vTableSlot, TR::CompilationInfoPerThread *threadCompInfo,
    const TR_ResolvedJ9JITServerMethodInfo &methodInfo)
{
    auto &methodInfoStruct = std::get<0>(methodInfo);
    ...
    _faninSummaryInfo = iProfiler
        ? iProfiler->cacheFaninDataForMethod((TR_OpaqueMethodBlock *)_ramMethod, entryStr, fe, trMemory)
        : NULL;
}

void TR_ResolvedJ9JITServerMethod::getClientBondMethodsToSendToServer(TR::Compilation *comp,
    std::vector<TR_ClientBondMethod> &dest)
{
...
bool TR_ResolvedJ9JITServerMethod::addValidationRecordForCachedResolvedMethod(const TR_ResolvedMethodKey &key,
    TR_OpaqueMethodBlock *method)
{
    ...
    switch (key.type) {
        ...
        case VirtualFromOffset:
            ...
            break;
        case Interface:
            added = svm->addInterfaceMethodFromCPRecord(method,
                (TR_OpaqueClassBlock *)((TR_J9VM *)_fe)->getClassFromMethodBlock(getPersistentIdentifier()),
                classObject, cpIndex);
            break;
        case Static:
...
void TR_ResolvedJ9JITServerMethod::cacheResolvedMethodsCallees(int32_t ttlForUnresolved)
{
    ...
    for (TR_J9ByteCode bc = bci.first(); bc != J9BCunknown; bc = bci.next()) {
        ...
        if (type != TR_ResolvedMethodType::NoType
            && !compInfoPT->getCachedResolvedMethod(
                compInfoPT->getResolvedMethodKey(type, (TR_OpaqueClassBlock *)_ramClass, cpIndex), this,
                &resolvedMethod)) {
    ...
    _stream->write(JITServer::MessageType::ResolvedMethod_getMultipleResolvedMethods,
        (TR_ResolvedJ9Method *)_remoteMirror, methodTypes, cpIndices);
    auto recv = _stream->read<std::vector<TR_OpaqueMethodBlock *>, std::vector<uint32_t>,
        std::vector<TR_ResolvedJ9JITServerMethodInfo>, std::vector<char> >();
    ...
    TR_ASSERT(numMethods == ramMethods.size(),
        "Number of received methods does not match the number of requested methods");
    for (int32_t i = 0; i < numMethods; ++i) {
        TR_ResolvedMethodType type = methodTypes[i];
        TR_ResolvedMethod *resolvedMethod;
        TR_ResolvedMethodKey key
            = compInfoPT->getResolvedMethodKey(type, (TR_OpaqueClassBlock *)_ramClass, cpIndices[i]);
        if (!compInfoPT->getCachedResolvedMethod(key, this, &resolvedMethod)) {
            compInfoPT->cacheResolvedMethod(key, ramMethods[i], vTableOffsets[i], methodInfos[i],
                (bool)unresolvedInCPs[i], ttlForUnresolved);
        }
    }
}

void TR_ResolvedJ9JITServerMethod::cacheFields()
{
    ...
    stream->write(JITServer::MessageType::VM_getFields, getRemoteMirror(), cpIndices, isStaticField);
...
int32_t TR_ResolvedJ9JITServerMethod::collectImplementorsCapped(TR_OpaqueClassBlock *topClass, int32_t maxCount,
    int32_t cpIndexOrOffset, TR_YesNoMaybe useGetResolvedInterfaceMethod, TR_ResolvedMethod **implArray)
{
    auto compInfoPT = static_cast<TR::CompilationInfoPerThreadRemote *>(_fe->_compInfoPT);
    JITServer::ServerStream *stream = compInfoPT->getMethodBeingCompiled()->_stream;
    stream->write(JITServer::MessageType::ResolvedMethod_getResolvedImplementorMethods, topClass, maxCount,
        cpIndexOrOffset, _remoteMirror, useGetResolvedInterfaceMethod);
    ...
    for (int32_t i = 0; i < methodInfos.size(); ++i) {
        TR_ResolvedMethodType type
            = isInterface ? TR_ResolvedMethodType::Interface : TR_ResolvedMethodType::VirtualFromOffset;

        TR_ResolvedMethod *resolvedMethod;
        TR_ResolvedMethodKey key
            = compInfoPT->getResolvedMethodKey(type, reinterpret_cast<TR_OpaqueClassBlock *>(_ramClass),
                cpIndexOrOffset, reinterpret_cast<TR_OpaqueClassBlock *>(std::get<0>(methodInfos[i]).ramClass));

        bool success;
        if (!(success = compInfoPT->getCachedResolvedMethod(key, this, &resolvedMethod))) {
            compInfoPT->cacheResolvedMethod(key, (TR_OpaqueMethodBlock *)ramMethods[i], cpIndexOrOffset, methodInfos[i],
                true,
...
bool TR_ResolvedJ9JITServerMethod::validateMethodFieldAttributes(const TR_J9MethodFieldAttributes &attributes,
    bool isStatic, int32_t cpIndex, bool isStore, bool needAOTValidation)
{
...
TR_ResolvedRelocatableJ9JITServerMethod::TR_ResolvedRelocatableJ9JITServerMethod(TR_OpaqueMethodBlock *aMethod,
    TR_FrontEnd *fe, TR_Memory *trMemory, TR_ResolvedMethod *owner, uint32_t vTableSlot)
    ...
    if (comp && this->TR_ResolvedMethod::getRecognizedMethod() != TR::unknownMethod) {
        if (fej9->canRememberClass(containingClass())) {
            if (comp->getOption(TR_UseSymbolValidationManager)) {
                ...
                ((TR_ResolvedRelocatableJ9JITServerMethod *)owner)
...
TR_ResolvedRelocatableJ9JITServerMethod::TR_ResolvedRelocatableJ9JITServerMethod(TR_OpaqueMethodBlock *aMethod,
    TR_FrontEnd *fe, TR_Memory *trMemory, const TR_ResolvedJ9JITServerMethodInfo &methodInfo, TR_ResolvedMethod *owner,
    uint32_t vTableSlot)
...
void *TR_ResolvedRelocatableJ9JITServerMethod::constantPool() { return romLiterals(); }

bool TR_ResolvedRelocatableJ9JITServerMethod::isInterpreted()
{
    ...
    */
    ...
    if (isJNINative() && !comp->getOption(TR_DisableDirectToJNI) && !comp->getOption(TR_DisableDirectToJNIInline)
        && !comp->getOption(TR_DisableDirectToJNI) && !comp->getOption(TR_DisableDirectToJNIInline)) {
        alwaysTreatAsInterpreted = false;
    }
#endif
...
bool TR_ResolvedRelocatableJ9JITServerMethod::isInterpretedForHeuristics()
{
    return TR_ResolvedJ9JITServerMethod::isInterpreted();
}

TR_OpaqueMethodBlock *TR_ResolvedRelocatableJ9JITServerMethod::getNonPersistentIdentifier()
{
    return (TR_OpaqueMethodBlock *)ramMethod();
}

bool TR_ResolvedRelocatableJ9JITServerMethod::validateArbitraryClass(TR::Compilation *comp, J9Class *clazz)
{
    return storeValidationRecordIfNecessary(comp, cp(), 0, TR_ValidateArbitraryClass, ramMethod(), clazz);
}

bool TR_ResolvedRelocatableJ9JITServerMethod::storeValidationRecordIfNecessary(TR::Compilation *comp,
    ...
    J9Class *definingClass)
{
    ...
    if (comp->getDebug() && trace) {
        ...
        J9UTF8 *methodClassName = J9ROMCLASS_CLASSNAME(TR::Compiler->cls.romClassOf(
    ...
    TR::AOTClassInfo *classInfo
        = new (comp->trHeapMemory()) TR::AOTClassInfo(fej9, (TR_OpaqueClassBlock *)definingClass, classChainOffset,
...
U_8 *TR_ResolvedRelocatableJ9JITServerMethod::allocateException(uint32_t numBytes, TR::Compilation *comp)
{
...
TR_ResolvedMethod *TR_ResolvedRelocatableJ9JITServerMethod::createResolvedMethodFromJ9Method(TR::Compilation *comp,
    I_32 cpIndex, uint32_t vTableSlot, J9Method *j9method, TR_AOTInliningStats *aotStats)
{
    ...
    _stream->write(JITServer::MessageType::ResolvedRelocatableMethod_createResolvedRelocatableJ9Method,
    ...
    if (std::get<0>(methodInfo).remoteMirror) {
        ...
        resolvedMethod = new (comp->trHeapMemory()) TR_ResolvedRelocatableJ9JITServerMethod(
            (TR_OpaqueMethodBlock *)j9method, _fe, comp->trMemory(), methodInfo, this, vTableSlot);
        if (aotStats) {
    ...
    const TR_ResolvedJ9JITServerMethodInfo &methodInfo)
{
    ...
    // In either case, methodInfo contains all parameters required to construct a resolved method or check that it's
    // NULL. The only problem is that we can't update AOT stats from this method.
    TR_ResolvedMethod *resolvedMethod = NULL;
    if (std::get<0>(methodInfo).remoteMirror)
        resolvedMethod = new (comp->trHeapMemory()) TR_ResolvedRelocatableJ9JITServerMethod(
            (TR_OpaqueMethodBlock *)j9method, _fe, comp->trMemory(), methodInfo, this, vTableSlot);
...
TR_ResolvedMethod *TR_ResolvedRelocatableJ9JITServerMethod::getResolvedImproperInterfaceMethod(TR::Compilation *comp,
    I_32 cpIndex)
{
...
void *TR_ResolvedRelocatableJ9JITServerMethod::startAddressForJittedMethod() { return NULL; }

void *TR_ResolvedRelocatableJ9JITServerMethod::startAddressForJNIMethod(TR::Compilation *comp)
{
...
void *TR_ResolvedRelocatableJ9JITServerMethod::startAddressForJITInternalNativeMethod() { return NULL; }

void *TR_ResolvedRelocatableJ9JITServerMethod::startAddressForInterpreterOfJittedMethod()
{
    return ((J9Method *)getNonPersistentIdentifier())->extra;
}

TR_OpaqueClassBlock *TR_ResolvedRelocatableJ9JITServerMethod::definingClassFromCPFieldRef(TR::Compilation *comp,
    int32_t cpIndex, bool isStatic, TR_OpaqueClassBlock **fromResolvedJ9Method)
{
    TR_OpaqueClassBlock *resolvedClass
        = TR_ResolvedJ9JITServerMethod::definingClassFromCPFieldRef(comp, cpIndex, isStatic);
    if (fromResolvedJ9Method != NULL)
        *fromResolvedJ9Method = resolvedClass;
    if (resolvedClass) {
        bool valid = false;
        if (comp->getOption(TR_UseSymbolValidationManager))
            valid = comp->getSymbolValidationManager()->addDefiningClassFromCPRecord(resolvedClass, cp(), cpIndex,
                isStatic);
        else
...
TR_OpaqueClassBlock *TR_ResolvedRelocatableJ9JITServerMethod::getClassFromConstantPool(TR::Compilation *comp,
    uint32_t cpIndex, bool returnClassForAOT)
{
    if (returnClassForAOT || comp->getOption(TR_UseSymbolValidationManager)) {
        TR_OpaqueClassBlock *resolvedClass
            = TR_ResolvedJ9JITServerMethod::getClassFromConstantPool(comp, cpIndex, returnClassForAOT);
        if (resolvedClass && validateClassFromConstantPool(comp, (J9Class *)resolvedClass, cpIndex)) {
...
bool TR_ResolvedRelocatableJ9JITServerMethod::validateClassFromConstantPool(TR::Compilation *comp, J9Class *clazz,
...
bool TR_ResolvedRelocatableJ9JITServerMethod::isUnresolvedString(I_32 cpIndex, bool optimizeForAOT)
{
    TR_ASSERT(cpIndex != -1, "cpIndex shouldn't be -1");
...
void *TR_ResolvedRelocatableJ9JITServerMethod::methodTypeConstant(I_32 cpIndex)
{
    TR_ASSERT(false, "should be unreachable");
    return NULL;
}

bool TR_ResolvedRelocatableJ9JITServerMethod::isUnresolvedMethodType(I_32 cpIndex)
{
    TR_ASSERT(false, "should be unreachable");
    return true;
}

void *TR_ResolvedRelocatableJ9JITServerMethod::methodHandleConstant(I_32 cpIndex)
{
    TR_ASSERT(false, "should be unreachable");
    return NULL;
}

bool TR_ResolvedRelocatableJ9JITServerMethod::isUnresolvedMethodHandle(I_32 cpIndex)
{
    TR_ASSERT(false, "should be unreachable");
    return true;
}

bool TR_ResolvedRelocatableJ9JITServerMethod::isUnresolvedMethodTypeTableEntry(int32_t cpIndex)
{
...
bool TR_ResolvedRelocatableJ9JITServerMethod::isUnresolvedCallSiteTableEntry(int32_t callSiteIndex)
{
    bool unresolved = true;
#if defined(J9VM_OPT_OPENJDK_METHODHANDLE)
    auto *vmInfo = _fe->_compInfoPT->getClientData()->getOrCacheVMInfo(_stream);
...
TR_OpaqueClassBlock *TR_ResolvedRelocatableJ9JITServerMethod::getDeclaringClassFromFieldOrStatic(TR::Compilation *comp,
    int32_t cpIndex)
{
    TR_OpaqueClassBlock *definingClass
        = TR_ResolvedJ9JITServerMethod::getDeclaringClassFromFieldOrStatic(comp, cpIndex);
    if (comp->getOption(TR_UseSymbolValidationManager)) {
...
TR_OpaqueClassBlock *TR_ResolvedRelocatableJ9JITServerMethod::classOfStatic(int32_t cpIndex, bool returnClassForAOT)
{
...
TR_ResolvedMethod *TR_ResolvedRelocatableJ9JITServerMethod::getResolvedPossiblyPrivateVirtualMethod(
    TR::Compilation *comp, int32_t cpIndex, bool ignoreRtResolve, bool *unresolvedInCP)
{
    TR_ResolvedMethod *method = TR_ResolvedJ9JITServerMethod::getResolvedPossiblyPrivateVirtualMethod(comp, cpIndex,
        ignoreRtResolve, unresolvedInCP);

    return aotMaskResolvedPossiblyPrivateVirtualMethod(comp, method);
}

bool TR_ResolvedRelocatableJ9JITServerMethod::getUnresolvedFieldInCP(I_32 cpIndex)
{
    TR_ASSERT(cpIndex != -1, "cpIndex shouldn't be -1");
...
bool TR_ResolvedRelocatableJ9JITServerMethod::getUnresolvedStaticMethodInCP(int32_t cpIndex)
{
    _stream->write(JITServer::MessageType::ResolvedMethod_getUnresolvedStaticMethodInCP, getRemoteMirror(), cpIndex);
    return std::get<0>(_stream->read<bool>());
}

bool TR_ResolvedRelocatableJ9JITServerMethod::getUnresolvedSpecialMethodInCP(I_32 cpIndex)
{
    _stream->write(JITServer::MessageType::ResolvedMethod_getUnresolvedSpecialMethodInCP, getRemoteMirror(), cpIndex);
    return std::get<0>(_stream->read<bool>());
}

void TR_ResolvedRelocatableJ9JITServerMethod::handleUnresolvedStaticMethodInCP(int32_t cpIndex, bool *unresolvedInCP)
{
    *unresolvedInCP = getUnresolvedStaticMethodInCP(cpIndex);
}

void TR_ResolvedRelocatableJ9JITServerMethod::handleUnresolvedSpecialMethodInCP(int32_t cpIndex, bool *unresolvedInCP)
{
    *unresolvedInCP = getUnresolvedSpecialMethodInCP(cpIndex);
}

void TR_ResolvedRelocatableJ9JITServerMethod::handleUnresolvedVirtualMethodInCP(int32_t cpIndex, bool *unresolvedInCP)
{
    *unresolvedInCP = getUnresolvedVirtualMethodInCP(cpIndex);
}

bool TR_ResolvedRelocatableJ9JITServerMethod::getUnresolvedVirtualMethodInCP(int32_t cpIndex) { return false; }
...
bool TR_ResolvedRelocatableJ9JITServerMethod::fieldAttributes(TR::Compilation *comp, int32_t cpIndex,
    uint32_t *fieldOffset, TR::DataType *type, bool *volatileP, bool *isFinal, bool *isPrivate, bool isStore,
    bool *unresolvedInCP, bool needAOTValidation)
{
    TR_ASSERT(cpIndex != -1, "cpIndex shouldn't be -1");

    // Will search per-resolved method cache and make a remote call if needed, client will call AOT version of the
    // method. We still need to create a validation record, and if it can't be added, change return values
    // appropriately.
    bool isStatic = false;
    ...
    TR_J9MethodFieldAttributes attributes;
    if (!getCachedFieldAttributes(cpIndex, attributes, isStatic)) {
        _stream->write(JITServer::MessageType::ResolvedRelocatableMethod_fieldAttributes, getRemoteMirror(), cpIndex,
            isStore, needAOTValidation);
        auto recv = _stream->read<TR_J9MethodFieldAttributes>();
        attributes = std::get<0>(recv);

        cacheFieldAttributes(cpIndex, attributes, isStatic);
    } else {
        TR_ASSERT(validateMethodFieldAttributes(attributes, isStatic, cpIndex, isStore, constantPool),
            "static attributes from client and cache do not match");
    }
    attributes.setMethodFieldAttributesResult(fieldOffset, type, volatileP, isFinal, isPrivate, unresolvedInCP,
        &theFieldIsFromLocalClass, &definingClass);
    ...
    if (comp->getOption(TR_DisableAOTInstanceFieldResolution)) {
        ...
        if (needAOTValidation) {
            if (comp->getOption(TR_UseSymbolValidationManager)) {
                fieldInfoCanBeUsed = comp->getSymbolValidationManager()->addDefiningClassFromCPRecord(
                    reinterpret_cast<TR_OpaqueClassBlock *>(definingClass), constantPool, cpIndex);
            } else {
    ...
    if (!fieldInfoCanBeUsed) {
        ...
        if (isPrivate)
            *isPrivate = false;
        if (fieldOffset)
            *(U_32 *)fieldOffset = TR::Compiler->om.objectHeaderSizeInBytes();
    }

    return theFieldIsFromLocalClass;
}

bool TR_ResolvedRelocatableJ9JITServerMethod::staticAttributes(TR::Compilation *comp, int32_t cpIndex, void **address,
    TR::DataType *type, bool *volatileP, bool *isFinal, bool *isPrivate, bool isStore, bool *unresolvedInCP,
    bool needAOTValidation)
{
    TR_ASSERT(cpIndex != -1, "cpIndex shouldn't be -1");
    ...
    TR_J9MethodFieldAttributes attributes;
    if (!getCachedFieldAttributes(cpIndex, attributes, isStatic)) {
        _stream->write(JITServer::MessageType::ResolvedRelocatableMethod_staticAttributes, getRemoteMirror(), cpIndex,
            isStore, needAOTValidation);
        auto recv = _stream->read<TR_J9MethodFieldAttributes>();
        attributes = std::get<0>(recv);
    ...
    attributes.setMethodFieldAttributesResult(address, type, volatileP, isFinal, isPrivate, unresolvedInCP,
        &theFieldIsFromLocalClass, &definingClass);
    ...
    if (comp->getOption(TR_DisableAOTInstanceFieldResolution)) {
        ...
        if (needAOTValidation) {
            if (comp->getOption(TR_UseSymbolValidationManager)) {
                fieldInfoCanBeUsed = comp->getSymbolValidationManager()->addDefiningClassFromCPRecord(
                    reinterpret_cast<TR_OpaqueClassBlock *>(definingClass), constantPool, cpIndex, true);
            } else {
                fieldInfoCanBeUsed = storeValidationRecordIfNecessary(comp, constantPool, cpIndex,
                    TR_ValidateInstanceField, ramMethod());
            }
...
TR_FieldAttributesCache &TR_ResolvedRelocatableJ9JITServerMethod::getAttributesCache(bool isStatic, bool unresolvedInCP)
{
    // Return persistent attributes cache for AOT compilations
    TR::CompilationInfoPerThread *compInfoPT = _fe->_compInfoPT;
    auto &attributesCache = isStatic ? JITServerHelpers::getJ9ClassInfo(compInfoPT, _ramClass)._staticAttributesCacheAOT
                                     : JITServerHelpers::getJ9ClassInfo(compInfoPT, _ramClass)._fieldAttributesCacheAOT;
    return attributesCache;
}

bool TR_ResolvedRelocatableJ9JITServerMethod::validateMethodFieldAttributes(
    const TR_J9MethodFieldAttributes &attributes, bool isStatic, int32_t cpIndex, bool isStore, bool needAOTValidation)
{
    // Validation should not be done against attributes cached per-resolved method,
    // because unresolved attribute might have become resolved during the current compilation
    // so validation would fail, but since it does not affect functional correctness,
    ...
    : TR_J9Method()
{
    ...
    if (!cached) {
        ...
        auto &cache = JITServerHelpers::getJ9ClassInfo(compInfoPT, aClazz)._J9MethodNameCache;
        cache.insert({
            cpIndex, { classNameStr, methodNameStr, methodSignatureStr }
        });
    }
