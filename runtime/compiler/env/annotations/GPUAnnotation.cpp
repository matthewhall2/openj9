...
void TR_SharedMemoryAnnotations::loadAnnotations(TR::Compilation *comp)
{
    ...
    logprints(trace, log, "looking for annotations\n");
    ...
    for (int i = 0; i <= numSupClasses; i++) {
        ...
        for (; currentField; currentField = romFieldsNextDo(&state)) {
            ...
            for (int i = 0; i < numAnnotations; i++) {
                ...
                logprintf(trace, log, "      annotation %.*s\n", len, str);

                if (len != 33 || strncmp(str, "Lcom/ibm/gpu/Kernel$SharedMemory;", len) != 0)
                    continue;
            ...
            logprintf(trace, log, "       size = %d\n", size);
            _sharedMemoryFields.push_back(
                TR_SharedMemoryField(fieldName, fieldNameLength, fieldSig, fieldSigLength, size));
        }
    }
}

void extractFieldName(TR::Compilation *comp, TR::SymbolReference *symRef, int32_t &classNameLength, char *&className,
    int32_t &fieldNameLength, char *&fieldName, int32_t &fieldSigLength, char *&fieldSig)
{
...
TR_SharedMemoryField TR_SharedMemoryAnnotations::find(TR::Compilation *comp, TR::SymbolReference *symRef)
{
    ...
    extractFieldName(comp, symRef, classNameLength, className, fieldNameLength, fieldName, fieldSigLength, fieldSig);
    ...
    for (auto lit = _sharedMemoryFields.begin(); lit != _sharedMemoryFields.end(); ++lit) {
        if ((*lit)._fieldNameLength == fieldNameLength && // TODO: check class !!!!
            strncmp((*lit)._fieldName, fieldName, fieldNameLength) == 0 && (*lit)._fieldSigLength == fieldSigLength
...
bool currentMethodHasFpreductionAnnotation(TR::Compilation *comp, bool trace)
{
    ...
    J9ROMMethod *romMethod
        = static_cast<TR_J9VM *>(comp->fej9())
              ->getROMMethodFromRAMMethod((J9Method *)comp->getCurrentMethod()->getPersistentIdentifier());
    ...
    logprintf(trace, log, "current method has %d annotations %p\n", numAnnotations, annotationsData);
    ...
    for (int i = 0; i < numAnnotations; i++) {
        ...
        logprintf(trace, log, "found annotation %.*s\n", len, str);

        if (len == 44 && strncmp(str, "Lorg/apache/spark/sql/execution/fpreduction;", len) == 0) {
            logprints(trace, log, "current method has @fpreduction annotation\n");
            return true;
        }
    }
