...
void TR_Debug::printAnnotationInfoEntry(OMR::Logger *log, J9AnnotationInfo *annotationInfo,
...
#define ANNO_NAMEBUF_LEN 30
    ...
    switch (flag) {
        ...
        default: {
            ...
            if ((flag & ~ANNOTATION_PARM_MASK) == ANNOTATION_TYPE_PARAMETER) {
                int len = snprintf(annNameBuffer, sizeof(annNameBuffer), "parm(%d)",
                    (flag & ANNOTATION_PARM_MASK) >> ANNOTATION_PARM_SHIFT);
    ...
    while (namePtr) {
        ...
        for (int32_t j = 0; j < indentationLevel; ++j)
            log->printc('\t');
        int32_t len;
