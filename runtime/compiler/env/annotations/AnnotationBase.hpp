...
public:
    ...
    TR_ScratchList<const char *>
    getAnnotationNames(); // get list of names of all the annotations associated with this object.

    AnnotationType getAnnotationType(const char *annotationName); // get the annotation type of this annotation

    // returns true if successful. Returns false and leaves ptr unset if annotation of the expected type cannot be
    // found. ptr must be matched with the AnnotationType.  The datatypes are as follows:
    bool getValue(TR::SymbolReference *symRef, const char *annotationName, AnnotationType type, void *ptr);

    bool getEnumeration(TR::SymbolReference *symRef, const char *annotationName, char **enumerationName,
        int32_t *nameLen, char **enumerationValue, int32_t *valueLen);
