...
void TR_AnnotationBase::loadExpectedAnnotationClasses(J9VMThread *vmThread)
{
    ...
    */
    ...
    for (i = 0; i < kLastAnnotationSignature; ++i) {
        ...
        J9Class *theClass
            = intFunc->internalFindClassUTF8(vmThread, (U_8 *)(name + 1), nameLen - 2, javaVM->systemClassLoader, 0);
 ...
 */
bool TR_AnnotationBase::scanForKnownAnnotationsAndRecord(TR::CompilationInfo *compInfo, J9Method *method,
    J9JavaVM *javaVM, TR_FrontEnd *fe)
{
        ...
        for (int32_t j = 0; j < kLastAnnotationSignature; ++j) {
            ...
            if (nameLength == recognizedAnnotations[j].nameLen
                ...
                if (false)
                    printf("* * *  found match * * *\n");
                if (classInfo)
                    classInfo->setHasRecognizedAnnotations(true);
...
J9AnnotationInfoEntry *TR_AnnotationBase::getAnnotationInfo(J9AnnotationInfo *annotationInfo, int32_t annotationType,
    const char *memberName, const char *memberSignature, const char *annotationName, bool isTag)
{
    ...
    if (memberName)
        nameLen = strlen(memberName);
    if (memberSignature)
        signatureLen = strlen(memberSignature);

    if (isTag) {
        annotationInfoEntryPtr
            = intFunc->getAnnotationFromAnnotationInfo(annotationInfo, annotationType, (char *)memberName, nameLen,
                (char *)memberSignature, signatureLen, (char *)annotationName, strlen(annotationName));
        if (false && annotationInfoEntryPtr)
            printf("found tag annotation %s for %s(%s)\n", annotationName, memberName, memberSignature);
...
J9AnnotationInfoEntry *TR_AnnotationBase::getDefaultAnnotationInfo(const char *annotationName)
{
    J9JavaVM *javaVM = ((TR_J9VMBase *)_comp->fej9())->_jitConfig->javaVM;
    J9InternalVMFunctions *intFunc = javaVM->internalVMFunctions;
    J9VMThread *vmThread = intFunc->currentVMThread(javaVM);
    if (NULL == _comp->getClassClassPointer())
        return NULL;
    int32_t i;
    J9Class *clazz = NULL;
    for (i = 0; i < kLastAnnotationSignature; ++i) {
    ...
    J9AnnotationInfoEntry *defaultEntry = intFunc->getAnnotationDefaultsForNamedAnnotation(vmThread, clazz,
        (char *)className, classNameLength, J9_FINDCLASS_FLAG_EXISTING_ONLY);
    return defaultEntry;
}

bool TR_AnnotationBase::isValid() { return _isValid; }
...
TR_AnnotationBase::AnnotationType TR_AnnotationBase::getAnnotationType(const char *annotationName) { return kUnknown; }

/* for now must do a search though all the annotation entries--later when Charlie has fixed it use
 * more efficient query */
J9AnnotationInfoEntry *TR_AnnotationBase::getAnnotationInfoEntry(TR::SymbolReference *symRef,
    AnnotationSignature annotationEnum)
{
...
J9AnnotationInfoEntry *TR_AnnotationBase::getTaggedAnnotationInfoEntry(TR::SymbolReference *symRef,
    AnnotationSignature annotationEnum)
{
...

{
    ...
    if (trace)
        printf("getAnnotationInfoEntry: searching for >%s<\n", annotationName);
    ...
    } else if (sym->isShadow()) // field
    {
        ...
        if (!resolvedSym->isStatic())
            --parmNo; // accommodate 'this'
        ...
        memberSignature = &(nameBuf[nameLen + 1]);
        if (trace)
            printf("Parameter:(%.*s) slot is %d in method %s%s\n", len, s, parmNo, memberName, memberSignature);
    ...
    J9AnnotationInfoEntry *annotationInfoEntryPtr;
    annotationInfoEntryPtr
        = getAnnotationInfo(_annotationInfo, annotationj9Type, memberName, memberSignature, annotationName, isTag);
...
/* for now must do a search though all the annotation entries--later when Charlie has fixed it use
 * more efficient query */
bool TR_AnnotationBase::getValue(TR::SymbolReference *symRef, const char *annotationName, AnnotationType type,
    void *ptr)
{
    const bool trace = false;
    J9AnnotationInfoEntry *annotationInfoEntryPtr = getAnnotationInfoEntry(symRef, annotationName);
    if ((NULL == annotationInfoEntryPtr) || !extractValue(annotationInfoEntryPtr, annotationName, type, ptr)) {
        if (trace)
            printf("Searching in default annotations...\n");
...
bool TR_AnnotationBase::extractValue(J9AnnotationInfoEntry *annotationInfoEntryPtr, const char *annotationName,
    AnnotationType type, void *ptr)
{
    ...
    do {
        ...
        if (trace)
            printf("Comparing fieldName %.*s(%c) to %s(%d)\n", len, fieldName, tag, annotationName, len);

        if (strncmp(fieldName, annotationName, len))
            continue;
...
bool TR_AnnotationBase::getEnumeration(TR::SymbolReference *symRef, const char *annotationName, char **enumerationName,
    int32_t *nameLength, char **enumerationValue, int32_t *valueLength)
{
    J9SRP *j9ptr;
    bool trace = false;
    if (trace)
    ...
    *enumerationValue = utf8Data(valueName, *valueLength);
    ...
    if (trace)
        printf("Enumeration returns %.*s %.*s\n", *nameLength, *enumerationName, *valueLength, *enumerationValue);
    return true;
}
