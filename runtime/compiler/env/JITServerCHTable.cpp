...
void JITClientCommitVirtualGuard(const VirtualGuardInfoForCHTable *info, std::vector<TR_VirtualGuardSite> &sites,
    TR_PersistentCHTable *table, TR::Compilation *comp);

void JITClientCommitOSRVirtualGuards(TR::Compilation *comp, std::vector<VirtualGuardForCHTable> &vguards);

void JITClientAddAnAssumptionForEachSubClass(TR_PersistentCHTable *table, TR_PersistentClassInfo *clazz,
    std::vector<TR_VirtualGuardSite> &list, TR::Compilation *comp)
{
...
bool JITClientCHTableCommit(TR::Compilation *comp, TR_MethodMetaData *metaData, CHTableCommitData &data)
{
    ...
    OMR::RetainedMethodSet *retainedMethods = J9::RepeatRetainedMethodsAnalysis::analyzeOnClient(comp, metaData,
        inlinedSiteInfo, serverKeepaliveMethods, serverBondMethods);

    TR_ResolvedMethod *bondMethod = NULL;
    if (vguards.empty() && sideEffectPatchSites.empty() && preXMethods.empty() && classes.empty()
        && classesThatShouldNotBeNewlyExtended.empty() && !retainedMethods->bondMethods().next(&bondMethod)) {
        return true;
    }
        ...
        if (TR::Compiler->cls.classHasIllegalStaticFinalFieldModification(clazz)) {
            if (TR::Options::isAnyVerboseOptionSet(TR_VerboseRuntimeAssumptions, TR_VerboseCompileEnd,
                ...
                TR_VerboseLog::writeLineLocked(TR_Vlog_FAILURE,
                    "JITClientCHTableCommit failure while commiting static final field assumption for class %p for %s",
                    clazz, comp->signature());
            }
            comp->failCompilation<TR::CompilationInterrupted>(
                "JITClientCHTableCommit compilation interrupted: Static final field of a class has been modified");
        }
    ...
    TR_PersistentCHTable *table, TR::Compilation *comp)
{
    ...
    if ((info->_kind == TR_HCRGuard) || info->_mergedWithHCRGuard) {
        ...
        for (TR_VirtualGuardSite &site : sites) {
            ...
            TR_PatchNOPedGuardSiteOnClassRedefinition ::make(comp->fe(), comp->trPersistentMemory(), guardedClass,
                site.getLocation(), site.getDestination(), comp->getMetadataAssumptionList());
            comp->setHasClassRedefinitionAssumptions();
        }
    ...
    if (info->_kind == TR_DummyGuard) {
        ...
        if (table->isOverriddenInThisHierarchy(guardedMethod, thisClass, info->_offset, comp))
            nopAssumptionIsValid = false;
    } else if (info->_isInlineGuard && info->_testType == TR_VftTest
        ...
        TR_OpaqueClassBlock *thisClass
            = info->_isInlineGuard ? thisClass = info->_thisClass : info->_guardedMethod->containingClass();
