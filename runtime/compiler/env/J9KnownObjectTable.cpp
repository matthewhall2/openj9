...
TR::KnownObjectTable *J9::KnownObjectTable::self() { return static_cast<TR::KnownObjectTable *>(this); }

TR::KnownObjectTable::Index J9::KnownObjectTable::getEndIndex() { return _objectInfos.size(); }

bool J9::KnownObjectTable::isNull(Index index) { return index == 0; }

TR::KnownObjectTable::Index J9::KnownObjectTable::getOrCreateIndex(uintptr_t objectPointer)
{
    if (objectPointer == 0)
...
#if defined(J9VM_OPT_JITSERVER)
    TR_ASSERT_FATAL(!self()->comp()->isOutOfProcessCompilation(),
        "It is not safe to call getOrCreateIndex() at the server. The object pointer could have become stale at the "
        "client.");
#endif /* defined(J9VM_OPT_JITSERVER) */
    ...
    _objectInfos[nextIndex]._jniReference = (uintptr_t *)thread->javaVM->internalVMFunctions->j9jni_createLocalRef(
        (JNIEnv *)thread, (j9object_t)objectPointer);
...
TR::KnownObjectTable::Index J9::KnownObjectTable::getOrCreateIndexAt(uintptr_t *objectReferenceLocation)
{
...
#endif /* defined(J9VM_OPT_JITSERVER) */
    {
        ...
        uintptr_t objectPointer
            = *objectReferenceLocation; // Note: object references held as uintptr_t must never be compressed refs
...
TR::KnownObjectTable::Index J9::KnownObjectTable::getExistingIndexAt(uintptr_t *objectReferenceLocation)
{
    TR::KnownObjectTable::Index result = UNKNOWN;
#if defined(J9VM_OPT_JITSERVER)
    if (self()->comp()->isOutOfProcessCompilation()) {
...
uintptr_t J9::KnownObjectTable::getPointer(Index index)
{
...
#if defined(J9VM_OPT_JITSERVER)
        TR_ASSERT_FATAL(!self()->comp()->isOutOfProcessCompilation(),
            "It is not safe to call getPointer() at the server. The object pointer could have become stale at the "
            "client.");
#endif /* defined(J9VM_OPT_JITSERVER) */
        TR_J9VMBase *fej9 = (TR_J9VMBase *)(self()->fe());
...
uintptr_t *J9::KnownObjectTable::getPointerLocation(Index index)
{
    TR_ASSERT(index != UNKNOWN && 0 <= index && index < _objectInfos.size(),
        "getPointerLocation(%d): index must be in range 0..%d", (int)index, _objectInfos.size());
    return _objectInfos[index]._jniReference;
}

void J9::KnownObjectTable::setObjectInfoFields(Index index, const ObjectInfo &objInfoSource)
{
    ...
    TR_ASSERT_FATAL(objInfo._jniReference == objInfoSource._jniReference,
        "The _jniReference for source and destination do not match. Index=%d %p != %p", index, objInfo._jniReference,
        objInfoSource._jniReference);
    objInfo = objInfoSource;
}

// Delete all the JNI references to the known objects tracked by the table
void J9::KnownObjectTable::freeKnownObjectTable()
{
#if defined(J9VM_OPT_JITSERVER)
    ...
    uint32_t nextIndex = self()->getEndIndex();
    for (uint32_t i = 1; i < nextIndex; i++)
        thread->javaVM->internalVMFunctions->j9jni_deleteLocalRef((JNIEnv *)thread,
            (jobject)_objectInfos.element(i)._jniReference);
}

#if defined(J9VM_OPT_JITSERVER)
void J9::KnownObjectTable::updateKnownObjectTableAtServer(Index index, uintptr_t *objectReferenceLocationClient,
    bool isArrayWithConstantElements)
{
    TR_ASSERT_FATAL(self()->comp()->isOutOfProcessCompilation(),
        "updateKnownObjectTableAtServer should only be called at the server");
    if (index == TR::KnownObjectTable::UNKNOWN)
        return;

    TR_ASSERT(objectReferenceLocationClient || (index == 0),
        "objectReferenceLocationClient should not be NULL (index=%d)", index);
    ...
    if (index == nextIndex) {
        ...
        TR_ASSERT_FATAL(false, "index %d from the client is greater than the KOT nextIndex %d at the server", index,
            nextIndex);
    }
...
void J9::KnownObjectTable::updateKnownObjectTableAtServer(Index index, const struct ObjectInfo &objInfo,
    bool isArrayWithConstantElements)
{
    TR_ASSERT_FATAL(self()->comp()->isOutOfProcessCompilation(),
        "updateKnownObjectTableAtServer should only be called at the server");
...
void J9::KnownObjectTable::dumpObjectTo(OMR::Logger *log, Index i, const char *fieldName, const char *sep,
    TR::Compilation *comp, TR_BitVector &visited, TR_VMFieldsInfo **fieldsInfoByIndex, int32_t depth)
{
    ...
    if (comp->getKnownObjectTable()->isNull(i)) {
        ...
        int32_t len;
        char *className = TR::Compiler->cls.classNameChars(comp, j9fe->getObjectClass(*ref), len);
        ...
        log->printf("%*s%s%sobj%d @ %p hash %8x %.*s", indent, "", fieldName, sep, i, *ref, hashCode, len - offs,
            className + offs);

#if defined(J9VM_OPT_METHOD_HANDLE)
        if (len == 29 && !strncmp("java/lang/invoke/DirectHandle", className, 29)) {
            J9Method *j9method
                = (J9Method *)J9VMJAVALANGINVOKEPRIMITIVEHANDLE_VMSLOT(j9fe->vmThread(), (J9Object *)(*ref));
            ...
            log->printf("  vmSlot: %.*s.%.*s", J9UTF8_LENGTH(className) - offs, utf8Data(className) + offs,
            ...
            for (TR_VMField *field = refIter.getFirst(); field; field = refIter.getNext()) {
                if (field->isReference()) {
                    ...
                    if (targetIndex != UNKNOWN)
                        self()->dumpObjectTo(log, targetIndex, field->name,
                            (field->modifiers & J9AccFinal) ? " is " : " = ", comp, visited, fieldsInfoByIndex,
                            depth + 1);
                }
            }
        } else {
...
#if defined(J9VM_OPT_JITSERVER)
void J9::KnownObjectTable::getKnownObjectTableDumpInfo(std::vector<TR_KnownObjectTableDumpInfo> &knotDumpInfoList)
{
    TR_ASSERT_FATAL(!self()->comp()->isOutOfProcessCompilation(),
        "getKnownObjectTableDumpInfo() can not be executed at the server.");
    ...
    TR::VMAccessCriticalSection j9KnownObjectTableDumpToCriticalSection(j9fe,
        TR::VMAccessCriticalSection::tryToAcquireVMAccess, self()->comp());
    if (j9KnownObjectTableDumpToCriticalSection.hasVMAccess()) {
        ...
        for (uint32_t i = 0; i < endIndex; i++) {
            if (self()->isNull(i)) {
                ...
                knotDumpInfoList.push_back(
                    make_tuple(TR_KnownObjectTableDumpInfoStruct(ref, *ref, hashCode), std::string(className, len)));
            }
        }
    }
}
...
void J9::KnownObjectTable::dumpTo(OMR::Logger *log, TR::Compilation *comp)
{
    TR::KnownObjectTable::Index endIndex = self()->getEndIndex();
#if defined(J9VM_OPT_JITSERVER)
    if (comp->isOutOfProcessCompilation()) {
        ...
        TR_ASSERT_FATAL((numOfEntries == endIndex),
            "The client table size %u is different from the server table size %u", numOfEntries, endIndex);

        log->printf("<knownObjectTable size=\"%u\"> // ", numOfEntries);
        int32_t pointerLen = log->printf("%p", this);
        log->printf("\n  %-6s   %-*s   %-*s %-8s   Class\n", "id", pointerLen, "JNI Ref", pointerLen, "Address",
            "Hash");
        ...
        for (uint32_t i = 0; i < numOfEntries; i++) {
            ...
            if (!std::get<0>(knotDumpInfoList[i]).ref)
                ...
                log->printf("   %p   %p %8x   %.*s\n", std::get<0>(knotDumpInfoList[i]).ref,
                    std::get<0>(knotDumpInfoList[i]).objectPointer, std::get<0>(knotDumpInfoList[i]).hashCode,
                    std::get<1>(knotDumpInfoList[i]).size(), std::get<1>(knotDumpInfoList[i]).c_str());
            }
        }
            ...
            if (comp->getOption(TR_TraceKnownObjectGraph)) {
...

                {
                    ...
                    TR_VMFieldsInfo **fieldsInfoByIndex
                        = (TR_VMFieldsInfo **)alloca(endIndex * sizeof(TR_VMFieldsInfo *));
...
void J9::KnownObjectTable::addStableArray(Index index, int32_t stableArrayRank)
{
    TR_J9VMBase *fej9 = static_cast<TR_J9VMBase *>(fe());
    TR_OpaqueClassBlock *clazz = fej9->getObjectClassFromKnownObjectIndex(comp(), index);

    TR_ASSERT_FATAL(fej9->isClassArray(clazz), "addStableArray can only be called for arrays");
...
bool J9::KnownObjectTable::isArrayWithStableElements(Index index)
{
    TR_ASSERT_FATAL(index != UNKNOWN && 0 <= index && index < self()->getEndIndex(),
        "isArrayWithStableElements(%d): index must be in range 0..%d", index, self()->getEndIndex());

    return (index < _stableArrayRanks.size() && _stableArrayRanks[index] > 0);
}

int32_t J9::KnownObjectTable::getArrayWithStableElementsRank(Index index)
{
    TR_ASSERT_FATAL(index != UNKNOWN && 0 <= index && index < self()->getEndIndex(),
        "getArrayWithStableElementsRank(%d): index must be in range 0..%d", index, self()->getEndIndex());
