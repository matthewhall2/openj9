...
PersistentAllocator::PersistentAllocator(const PersistentAllocatorKit &creationKit)
    ...
    , _segmentAllocator(
          ...
          creationKit.javaVM.internalVMFunctions->isJITServerEnabled(&creationKit.javaVM)
              ? MEMORY_TYPE_VIRTUAL | MEMORY_TYPE_JIT_PERSISTENT
              : // use virtual memory for JITServer because we need to release memory
...
void *PersistentAllocator::allocate(size_t size, const std::nothrow_t tag, void *hint) throw()
{
    return allocateInternal(size);
}

void *PersistentAllocator::allocate(size_t size, void *hint)
{
...
PersistentAllocator::Block *PersistentAllocator::allocateFromVariableSizeListLocked(size_t allocSize)
{
    Block *block = NULL;
    Block *prev = NULL;
    for (block = _freeBlocks[LARGE_BLOCK_LIST_INDEX]; block && block->size() < allocSize;
...
#if defined(J9VM_OPT_JITSERVER)
PersistentAllocator::Block *PersistentAllocator::allocateFromIndexedListLocked(size_t allocSize)
{
...
void *PersistentAllocator::allocateInternal(size_t requestedSize)
{
    // Workaround for when user wants to allocate 0 bytes
    if (requestedSize == 0)
        requestedSize = sizeof(void *);
    TR_ASSERT(sizeof(Block) == mem_round(sizeof(Block)),
        "Persistent block size will prevent us from properly aligning allocations.");
...
void *PersistentAllocator::allocateFromSegmentLocked(size_t allocSize)
{
    ...
    if (!segment) {
        ...
        if (_disclaimEnabled) {
            ...
            segmentSize = OMR::align(segmentSize,
                ...
                if (madvise(segment->heapBase, segLength, MADV_RANDOM) != 0) {
                    if (TR::Options::getCmdLineOptions()->getVerboseOption(TR_VerbosePerformance))
                        TR_VerboseLog::writeLine(TR_Vlog_INFO,
                            "Failed to set MADV_RANDOM for persistent memory segment");
                }
            }
        }
#endif // LINUX
...
J9MemorySegment *PersistentAllocator::findUsableSegment(size_t requiredSize)
{
...
size_t PersistentAllocator::remainingSpace(J9MemorySegment &segment) throw()
{
    return (segment.heapTop - segment.heapAlloc);
}

void PersistentAllocator::freeFixedSizeBlock(Block *block)
{
    ...
    TR_ASSERT(index != LARGE_BLOCK_LIST_INDEX,
        "freeFixedSizeBlock should be used for small blocks, so index cannot be LARGE_BLOCK_LIST_INDEX");
    block->setNext(_freeBlocks[index]);
    _freeBlocks[index] = block;
}

void PersistentAllocator::freeVariableSizeBlock(Block *block)
{
    // Appropriate lock should have been obtained
    TR_ASSERT(block->size() > 0, "Block size is non-positive");
    block->setNext(NULL);
    ...
    //
    TR_ASSERT(freeBlocksIndex(block->size()) == LARGE_BLOCK_LIST_INDEX,
        "freeVariableSizeBlock should be used for large blocks, so index should be LARGE_BLOCK_LIST_INDEX");
...
#if defined(J9VM_OPT_JITSERVER)
size_t PersistentAllocator::getInterval(size_t blockSize)
{
...
void PersistentAllocator::freeBlockToIndexedList(Block *blockToBeFreed)
{
    ...
    TR_ASSERT(freeBlocksIndex(blockToBeFreed->size()) == LARGE_BLOCK_LIST_INDEX,
        "We must be working on the variable size block list");
        ...
        if (_startInterval[index]) {
            ...
            if (blockIterator) {
                ...
                } else // Insert before blockIterator
                {
                    ...
                    TR_ASSERT(blockIterator->previous(),
                        "blockIterator->previous() must exist because we already treated the case where we insert at "
                    ...
                    if (_startInterval[index]->size() > block->size()) {
                        ...
                        TR_ASSERT(_startInterval[index] == blockIterator,
                            "blockInterator must be first block in this interval");
                        _startInterval[index] = block;
                    } else // Insert in the middle or end of interval
                    {
                        ...
                        if (_endInterval[index]->size() < block->size()) {
                            TR_ASSERT(getInterval(blockIterator->size()) > index,
                                "blockInterator must be first block in the next interval: index=%zu b->sz=%zu "
                                "bi->sz=%zu",
                                index, block->size(), blockIterator->size());
                            _endInterval[index] = block;
                        }
                    }
                }
        ...
        } else // There are no other blocks in this interval
        {
            ...
            for (int i = (int)index - 1; i >= 0; i--) {
                if (_endInterval[i]) {
                    ...
                    if (block->next()) {
                        block->next()->setPrevious(block);
                        TR_ASSERT(block->size() < block->next()->size(),
                            "wrong sizes after block->size()=%zu next->size()=%zu", block->size(),
                            block->next()->size());
                    }
...
void PersistentAllocator::checkIntegrity(const char msg[])
{
    ...
    for (size_t i = 0; i < NUM_INTERVALS; i++) {
        if (_startInterval[i]) {
            ...
            if (i == 0)
                TR_ASSERT_FATAL(_startInterval[i]
                        == reinterpret_cast<ExtendedBlock *>(_freeBlocks[LARGE_BLOCK_LIST_INDEX]),
                    "Error with first block");
        ...
        if (_endInterval[i]) {
            ...
            TR_ASSERT_FATAL(i == index,
                "endInterval block is not in the correct interval i=%zu index=%zu start->size()=%zu end->size()=%zu\n",
            ...
            if (nextBlock) {
                ...
                TR_ASSERT_FATAL(nextBlock == _startInterval[index],
                    "Error with interval %zu end->size()=%zu index=%zu next->size()=%zu\n", i, _endInterval[i]->size(),
                    index, nextBlock->size());
            }
        }
        if (!_startInterval[i]) {
...
void PersistentAllocator::freeBlock(Block *block)
{
    TR_ASSERT(block->size() > 0, "Block size is non-positive");
...
//---------------------------------------------------------------------------
int PersistentAllocator::disclaimAllSegments()
{
    ...
    for (auto segmentIterator = _segments.begin(); segmentIterator != _segments.end(); ++segmentIterator) {
        ...
        if ((segment.vmemIdentifier.allocator == OMRPORT_VMEM_RESERVE_USED_MMAP_SHM && compInfo->canDisclaimOnFile())
            ...
            fprintf(stderr,
                "Will disclaim persistent memory segment %p of size %zu. Present pages =%d swapped=%d fileMapped=%d\n",
            ...
            if (ret != 0) {
                ...
                if (verbose)
                    TR_VerboseLog::writeLine(TR_Vlog_PERF,
                        "WARNING: Failed to use madvise to disclaim memory for persistent memory");
                if (ret == EINVAL) {
                    _disclaimEnabled = false; // Don't try to disclaim again, since support seems to be missing
                    if (verbose)
                        TR_VerboseLog::writeLine(TR_Vlog_PERF,
                            "WARNING: Disabling persistent memory disclaiming for this allocator from now on");
                }
            } else {
#ifdef DEBUG_DISCLAIM
                swappedCount = 0;
                ...
                fprintf(stderr,
                    "Disclaimed persistent memory segment %p of size %zu. Present pages =%d swapped=%d fileMapped=%d\n",
                    &segment, segLength, numPresentPages, swappedCount, filePageCount);
#endif
                numSegDisclaimed++;
            }
            ...
            if (verbose)
                TR_VerboseLog::writeLine(TR_Vlog_PERF, "WARNING: Persistent memory segment %p is not backed by a file",
                    &segment);
        }
    }
    j9thread_monitor_exit(_segmentMonitor);
...
void TR::PersistentAllocator::adviseDontNeedSegments()
{
#ifdef LINUX
    j9thread_monitor_enter(_segmentMonitor);
    for (auto segmentIterator = _segments.begin(); segmentIterator != _segments.end(); ++segmentIterator) {
