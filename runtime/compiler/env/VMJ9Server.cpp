...
bool TR_J9ServerVM::isClassLibraryClass(TR_OpaqueClassBlock *clazz)
{
    ...
    JITServerHelpers::getAndCacheRAMClassInfo((J9Class *)clazz, _compInfoPT->getClientData(), stream,
        JITServerHelpers::CLASSINFO_CLASS_LOADER, &classLoader);
...
TR_OpaqueClassBlock *TR_J9ServerVM::getSuperClass(TR_OpaqueClassBlock *clazz)
{
    TR_OpaqueClassBlock *parentClass = NULL;

    JITServer::ServerStream *stream = _compInfoPT->getMethodBeingCompiled()->_stream;
    JITServerHelpers::getAndCacheRAMClassInfo((J9Class *)clazz, _compInfoPT->getClientData(), stream,
        JITServerHelpers::CLASSINFO_PARENT_CLASS, (void *)&parentClass);
...
bool TR_J9ServerVM::isSameOrSuperClass(J9Class *superClass, J9Class *subClass)
{
...
TR::Method *TR_J9ServerVM::createMethod(TR_Memory *trMemory, TR_OpaqueClassBlock *clazz, int32_t refOffset)
{
    return new (trMemory->trHeapMemory())
        TR_J9ServerMethod(this, trMemory, TR::Compiler->cls.convertClassOffsetToClassPtr(clazz), refOffset);
}

TR_ResolvedMethod *TR_J9ServerVM::createResolvedMethod(TR_Memory *trMemory, TR_OpaqueMethodBlock *aMethod,
...
TR_ResolvedMethod *TR_J9ServerVM::createResolvedMethod(TR_Memory *trMemory, TR_OpaqueMethodBlock *aMethod,
    TR_ResolvedMethod *owningMethod, const TR_ResolvedJ9JITServerMethodInfo &methodInfo,
...
TR_ResolvedMethod *TR_J9ServerVM::createResolvedMethodWithSignature(TR_Memory *trMemory, TR_OpaqueMethodBlock *aMethod,
    ...
    uint32_t vTableSlot)
{
    ...
    if (isAOT_DEPRECATED_DO_NOT_USE()) {
        ...
        result = new (trMemory->trHeapMemory())
            TR_ResolvedRelocatableJ9JITServerMethod(aMethod, this, trMemory, owningMethod, vTableSlot);
        TR::Compilation *comp = _compInfoPT->getCompilation();
        ...
        result = new (trMemory->trHeapMemory())
            TR_ResolvedJ9JITServerMethod(aMethod, this, trMemory, owningMethod, vTableSlot);
...
TR_ResolvedMethod *TR_J9ServerVM::createResolvedMethodWithSignature(TR_Memory *trMemory, TR_OpaqueMethodBlock *aMethod,
    TR_OpaqueClassBlock *classForNewInstance, char *signature, int32_t signatureLength, TR_ResolvedMethod *owningMethod,
    const TR_ResolvedJ9JITServerMethodInfo &methodInfo)
{
    TR_ResolvedJ9Method *result = NULL;
    if (isAOT_DEPRECATED_DO_NOT_USE()) {
#if defined(J9VM_INTERP_AOT_COMPILE_SUPPORT)
        result = new (trMemory->trHeapMemory())
            TR_ResolvedRelocatableJ9JITServerMethod(aMethod, this, trMemory, methodInfo, owningMethod);
        TR::Compilation *comp = _compInfoPT->getCompilation();
        if (comp && comp->getOption(TR_UseSymbolValidationManager)) {
            TR::SymbolValidationManager *svm = comp->getSymbolValidationManager();
            if (!svm->isAlreadyValidated(result->containingClass()))
        ...
        result = new (trMemory->trHeapMemory())
            TR_ResolvedJ9JITServerMethod(aMethod, this, trMemory, methodInfo, owningMethod);
        if (classForNewInstance) {
            result->setClassForNewInstance((J9Class *)classForNewInstance);
            TR_ASSERT(result->isNewInstanceImplThunk(),
                "createResolvedMethodWithSignature: if classForNewInstance is given this must be a thunk");
...
TR_YesNoMaybe TR_J9ServerVM::isInstanceOf(TR_OpaqueClassBlock *a, TR_OpaqueClassBlock *b, bool objectTypeIsFixed,
...
TR_OpaqueClassBlock *TR_J9ServerVM::getSystemClassFromClassName(const char *name, int32_t length, bool isVettedForAOT)
{
    ...
    PersistentUnorderedMap<ClassLoaderStringPair, TR_OpaqueClassBlock *> &classBySignatureMap
        = _compInfoPT->getClientData()->getClassBySignatureMap();
...
TR_OpaqueClassBlock *TR_J9ServerVM::getByteArrayClass()
{
    JITServer::ServerStream *stream = _compInfoPT->getMethodBeingCompiled()->_stream;
    auto *vmInfo = _compInfoPT->getClientData()->getOrCacheVMInfo(stream);
...
bool TR_J9ServerVM::isMethodTracingEnabled(TR_OpaqueMethodBlock *method)
{
    {
...
bool TR_J9ServerVM::canMethodEnterEventBeHooked()
{
    JITServer::ServerStream *stream = _compInfoPT->getMethodBeingCompiled()->_stream;
    auto *vmInfo = _compInfoPT->getClientData()->getOrCacheVMInfo(stream);
...
bool TR_J9ServerVM::canMethodExitEventBeHooked()
{
    JITServer::ServerStream *stream = _compInfoPT->getMethodBeingCompiled()->_stream;
    auto *vmInfo = _compInfoPT->getClientData()->getOrCacheVMInfo(stream);
...
bool TR_J9ServerVM::canExceptionEventBeHooked()
{
    JITServer::ServerStream *stream = _compInfoPT->getMethodBeingCompiled()->_stream;
    auto *vmInfo = _compInfoPT->getClientData()->getOrCacheVMInfo(stream);
...
TR_OpaqueClassBlock *TR_J9ServerVM::getClassClassPointer(TR_OpaqueClassBlock *objectClassPointer)
{
...
void *TR_J9ServerVM::getClassLoader(TR_OpaqueClassBlock *clazz)
{
    void *loader = NULL;
    JITServer::ServerStream *stream = _compInfoPT->getMethodBeingCompiled()->_stream;
    JITServerHelpers::getAndCacheRAMClassInfo((J9Class *)clazz, _compInfoPT->getClientData(), stream,
...
TR_OpaqueClassBlock *TR_J9ServerVM::getClassOfMethod(TR_OpaqueMethodBlock *method)
{
    return TR_J9ServerVM::getClassFromMethodBlock(method);
}

TR_OpaqueClassBlock *TR_J9ServerVM::getBaseComponentClass(TR_OpaqueClassBlock *clazz, int32_t &numDims)
{
    TR_OpaqueClassBlock *baseComponentClass = NULL;
    JITServer::ServerStream *stream = _compInfoPT->getMethodBeingCompiled()->_stream;
    JITServerHelpers::getAndCacheRAMClassInfo((J9Class *)clazz, _compInfoPT->getClientData(), stream,
        JITServerHelpers::CLASSINFO_NUMBER_DIMENSIONS, &numDims, JITServerHelpers::CLASSINFO_BASE_COMPONENT_CLASS,
        (void *)&baseComponentClass);

    return baseComponentClass;
}

TR_OpaqueClassBlock *TR_J9ServerVM::getLeafComponentClassFromArrayClass(TR_OpaqueClassBlock *arrayClass)
{
    ...
    JITServerHelpers::getAndCacheRAMClassInfo((J9Class *)arrayClass, _compInfoPT->getClientData(), stream,
        JITServerHelpers::CLASSINFO_LEAF_COMPONENT_CLASS, (void *)&leafComponentClass);
    return leafComponentClass;
}

TR_OpaqueClassBlock *TR_J9ServerVM::getClassFromSignature(const char *sig, int32_t sigLength,
...
void *TR_J9ServerVM::getSystemClassLoader()
{
    JITServer::ServerStream *stream = _compInfoPT->getMethodBeingCompiled()->_stream;
    auto *vmInfo = _compInfoPT->getClientData()->getOrCacheVMInfo(stream);
...
bool TR_J9ServerVM::jitFieldsOrStaticsAreIdentical(TR_ResolvedMethod *method1, I_32 cpIndex1,
    TR_ResolvedMethod *method2, I_32 cpIndex2, int32_t isStatic)
{
    ...
    if (needRemoteCall) {
        ...
        stream->write(JITServer::MessageType::VM_jitFieldsOrStaticsAreSame, clientMethod1, cpIndex1, clientMethod2,
            cpIndex2, isStatic);
...
bool TR_J9ServerVM::jitFieldsAreSame(TR_ResolvedMethod *method1, I_32 cpIndex1, TR_ResolvedMethod *method2,
    I_32 cpIndex2, int32_t isStatic)
{
    ...
    if (classOfMethod1 && classOfMethod2 && (isHiddenClass(classOfMethod1) || isHiddenClass(classOfMethod2))
...
bool TR_J9ServerVM::jitStaticsAreSame(TR_ResolvedMethod *method1, I_32 cpIndex1, TR_ResolvedMethod *method2,
    I_32 cpIndex2)
{
    bool result = false;
    bool sigSame = true;
...
bool TR_J9ServerVM::getCachedField(J9Class *ramClass, int32_t cpIndex, J9Class **declaringClass, UDATA *field)
{
    OMR::CriticalSection getRemoteROMClass(_compInfoPT->getClientData()->getROMMapMonitor());
...
void TR_J9ServerVM::cacheField(J9Class *ramClass, int32_t cpIndex, J9Class *declaringClass, UDATA field)
{
...
TR_OpaqueClassBlock *TR_J9ServerVM::getComponentClassFromArrayClass(TR_OpaqueClassBlock *arrayClass)
{
    TR_OpaqueClassBlock *componentClass = NULL;
    JITServer::ServerStream *stream = _compInfoPT->getMethodBeingCompiled()->_stream;
    JITServerHelpers::getAndCacheRAMClassInfo((J9Class *)arrayClass, _compInfoPT->getClientData(), stream,
        JITServerHelpers::CLASSINFO_COMPONENT_CLASS, (void *)&componentClass);
...
bool TR_J9ServerVM::classHasBeenReplaced(TR_OpaqueClassBlock *clazz)
{
    ...
    JITServer::ServerStream *stream = _compInfoPT->getMethodBeingCompiled()->_stream;
    JITServerHelpers::getAndCacheRAMClassInfo((J9Class *)clazz, _compInfoPT->getClientData(), stream,
        JITServerHelpers::CLASSINFO_CLASS_DEPTH_AND_FLAGS, (void *)&classDepthAndFlags);
    return ((classDepthAndFlags & J9AccClassHotSwappedOut) != 0);
}

bool TR_J9ServerVM::checkCHTableIfClassInfoExistsAndHasBeenExtended(TR_OpaqueClassBlock *clazz,
    bool &bClassHasBeenExtended)
{
    JITServerPersistentCHTable *table
        = (JITServerPersistentCHTable *)(_compInfo->getPersistentInfo()->getPersistentCHTable());
...
bool TR_J9ServerVM::classHasBeenExtended(TR_OpaqueClassBlock *clazz)
{
    if (!clazz)
        return false;
    ...
    uintptr_t classDepthAndFlags = JITServerHelpers::getRemoteClassDepthAndFlagsWhenROMClassNotCached((J9Class *)clazz,
        clientSessionData, stream);
    return (classDepthAndFlags & J9AccClassHasBeenOverridden) != 0;
}

bool TR_J9ServerVM::isGetImplInliningSupported() { return isGetImplAndRefersToInliningSupported(); }

bool TR_J9ServerVM::isGetImplAndRefersToInliningSupported()
{
    JITServer::ServerStream *stream = _compInfoPT->getMethodBeingCompiled()->_stream;
    auto *vmInfo = _compInfoPT->getClientData()->getOrCacheVMInfo(stream);
...
bool TR_J9ServerVM::compiledAsDLTBefore(TR_ResolvedMethod *method)
{
...
uintptr_t TR_J9ServerVM::getOverflowSafeAllocSize()
{
    JITServer::ServerStream *stream = _compInfoPT->getMethodBeingCompiled()->_stream;
    auto *vmInfo = _compInfoPT->getClientData()->getOrCacheVMInfo(stream);
...
void *TR_J9ServerVM::getReferenceArrayCopyHelperAddress()
{
    JITServer::ServerStream *stream = _compInfoPT->getMethodBeingCompiled()->_stream;
    auto *vmInfo = _compInfoPT->getClientData()->getOrCacheVMInfo(stream);
...
bool TR_J9ServerVM::tlhHasBeenCleared()
{
...
uint32_t TR_J9ServerVM::getStaticObjectFlags()
{
    JITServer::ServerStream *stream = _compInfoPT->getMethodBeingCompiled()->_stream;
    auto *vmInfo = _compInfoPT->getClientData()->getOrCacheVMInfo(stream);
...
bool TR_J9ServerVM::isThunkArchetype(J9Method *method)
{
    JITServer::ServerStream *stream = _compInfoPT->getMethodBeingCompiled()->_stream;
...
int32_t TR_J9ServerVM::printTruncatedSignature(char *sigBuf, int32_t bufLen, TR_OpaqueMethodBlock *method)
{
    JITServer::ServerStream *stream = _compInfoPT->getMethodBeingCompiled()->_stream;
...
bool TR_J9ServerVM::isClassInitialized(TR_OpaqueClassBlock *clazz)
{
    bool isClassInitialized = false;
    JITServer::ServerStream *stream = _compInfoPT->getMethodBeingCompiled()->_stream;
    JITServerHelpers::getAndCacheRAMClassInfo((J9Class *)clazz, _compInfoPT->getClientData(), stream,
        JITServerHelpers::CLASSINFO_CLASS_INITIALIZED, (void *)&isClassInitialized);
...
bool TR_J9ServerVM::ensureOSRBufferSize(TR::Compilation *comp, uintptr_t osrFrameSizeInBytes,
    uintptr_t osrScratchBufferSizeInBytes, uintptr_t osrStackFrameSizeInBytes)
{
    ...
    if (osrGlobalBufferSize > vmInfo->_osrGlobalBufferSize) {
        stream->write(JITServer::MessageType::VM_increaseOSRGlobalBufferSize, osrFrameSizeInBytes,
            osrScratchBufferSizeInBytes, osrStackFrameSizeInBytes);
...
int32_t TR_J9ServerVM::getByteOffsetToLockword(TR_OpaqueClassBlock *clazz)
{
    uint32_t byteOffsetToLockword = 0;
    if (clazz) {
        JITServer::ServerStream *stream = _compInfoPT->getMethodBeingCompiled()->_stream;
        JITServerHelpers::getAndCacheRAMClassInfo((J9Class *)clazz, _compInfoPT->getClientData(), stream,
            JITServerHelpers::CLASSINFO_BYTE_OFFSET_TO_LOCKWORD, (void *)&byteOffsetToLockword);
    }
    return byteOffsetToLockword;
}

int32_t TR_J9ServerVM::getInitialLockword(TR_OpaqueClassBlock *ramClass)
{
...
bool TR_J9ServerVM::isEnableGlobalLockReservationSet()
{
    JITServer::ServerStream *stream = _compInfoPT->getMethodBeingCompiled()->_stream;
    auto *vmInfo = _compInfoPT->getClientData()->getOrCacheVMInfo(stream);
...
bool TR_J9ServerVM::isString(TR_OpaqueClassBlock *clazz)
{
    JITServer::ServerStream *stream = _compInfoPT->getMethodBeingCompiled()->_stream;
    auto *vmInfo = _compInfoPT->getClientData()->getOrCacheVMInfo(stream);
...
bool TR_J9ServerVM::isJavaLangObject(TR_OpaqueClassBlock *clazz)
{
    JITServer::ServerStream *stream = _compInfoPT->getMethodBeingCompiled()->_stream;
    auto *vmInfo = _compInfoPT->getClientData()->getOrCacheVMInfo(stream);
...
void *TR_J9ServerVM::getMethods(TR_OpaqueClassBlock *clazz)
{
    J9Method *methodsOfClass = NULL;
    JITServer::ServerStream *stream = _compInfoPT->getMethodBeingCompiled()->_stream;
    JITServerHelpers::getAndCacheRAMClassInfo((J9Class *)clazz, _compInfoPT->getClientData(), stream,
        JITServerHelpers::CLASSINFO_METHODS_OF_CLASS, (void *)&methodsOfClass);
...
bool TR_J9ServerVM::isPrimitiveArray(TR_OpaqueClassBlock *clazz)
{
    ...
    JITServer::ServerStream *stream = _compInfoPT->getMethodBeingCompiled()->_stream;
    JITServerHelpers::getAndCacheRAMClassInfo((J9Class *)clazz, _compInfoPT->getClientData(), stream,
        JITServerHelpers::CLASSINFO_ROMCLASS_MODIFIERS, (void *)&modifiers, JITServerHelpers::CLASSINFO_COMPONENT_CLASS,
        (void *)&componentClass);

    if (!J9_ARE_ALL_BITS_SET(modifiers, J9AccClassArray))
        return false;

    JITServerHelpers::getAndCacheRAMClassInfo((J9Class *)componentClass, _compInfoPT->getClientData(), stream,
        JITServerHelpers::CLASSINFO_ROMCLASS_MODIFIERS, (void *)&modifiers);
    return J9_ARE_ALL_BITS_SET(modifiers, J9AccClassInternalPrimitiveType) ? true : false;
}

uint32_t TR_J9ServerVM::getAllocationSize(TR::StaticSymbol *classSym, TR_OpaqueClassBlock *clazz)
{
    uintptr_t totalInstanceSize = 0;
    JITServer::ServerStream *stream = _compInfoPT->getMethodBeingCompiled()->_stream;
    JITServerHelpers::getAndCacheRAMClassInfo((J9Class *)clazz, _compInfoPT->getClientData(), stream,
        JITServerHelpers::CLASSINFO_TOTAL_INSTANCE_SIZE, (void *)&totalInstanceSize);
...
TR_OpaqueClassBlock *TR_J9ServerVM::getObjectClass(uintptr_t objectPointer)
{
    JITServer::ServerStream *stream = _compInfoPT->getMethodBeingCompiled()->_stream;
...
TR_OpaqueClassBlock *TR_J9ServerVM::getObjectClassAt(uintptr_t objectAddress)
{
    JITServer::ServerStream *stream = _compInfoPT->getMethodBeingCompiled()->_stream;
...
TR::KnownObjectTable::ObjectInfo TR_J9ServerVM::getObjClassInfoFromKnotIndexNoCaching(TR::Compilation *comp,
    TR::KnownObjectTable::Index knotIndex)
{
    ...
    TR_ASSERT_FATAL(retrievedObjInfo._jniReference
            == comp->getKnownObjectTable()->getObjectInfo(knotIndex)._jniReference,
        "JNI ref mismatch");
...
bool TR_J9ServerVM::hasFinalFieldsInClass(TR_OpaqueClassBlock *clazz)
{
    ...
    JITServer::ServerStream *stream = _compInfoPT->getMethodBeingCompiled()->_stream;
    JITServerHelpers::getAndCacheRAMClassInfo((J9Class *)clazz, _compInfoPT->getClientData(), stream,
        JITServerHelpers::CLASSINFO_CLASS_HAS_FINAL_FIELDS, (void *)&classHasFinalFields);

    return classHasFinalFields;
}

const char *TR_J9ServerVM::sampleSignature(TR_OpaqueMethodBlock *aMethod, char *buf, int32_t bufLen,
    TR_Memory *trMemory_unused)
{
    ...
    if (s)
        snprintf(s, len, "%.*s.%.*s%.*s", J9UTF8_LENGTH(className), utf8Data(className), J9UTF8_LENGTH(name),
            utf8Data(name), J9UTF8_LENGTH(signature), utf8Data(signature));
    return s;
}

TR_OpaqueClassBlock *TR_J9ServerVM::getHostClass(TR_OpaqueClassBlock *clazz)
{
    TR_OpaqueClassBlock *hostClass = NULL;
    JITServer::ServerStream *stream = _compInfoPT->getMethodBeingCompiled()->_stream;
    JITServerHelpers::getAndCacheRAMClassInfo((J9Class *)clazz, _compInfoPT->getClientData(), stream,
        JITServerHelpers::CLASSINFO_HOST_CLASS, (void *)&hostClass);
...
int32_t TR_J9ServerVM::getStringUTF8Length(uintptr_t objectPointer)
{
    TR_ASSERT_FATAL(false, "getStringUTF8Length(uintptr_t) should not be called by JITServer");
}

uint64_t TR_J9ServerVM::getStringUTF8UnabbreviatedLength(uintptr_t objectPointer)
{
    TR_ASSERT_FATAL(false, "getStringUTF8UnabbreviatedLength(uintptr_t) should not be called by JITServer");
}

bool TR_J9ServerVM::classInitIsFinished(TR_OpaqueClassBlock *clazz)
{
    JITServer::ServerStream *stream = _compInfoPT->getMethodBeingCompiled()->_stream;
...
TR_OpaqueClassBlock *TR_J9ServerVM::getClassFromNewArrayType(int32_t arrayType)
{
    JITServer::ServerStream *stream = _compInfoPT->getMethodBeingCompiled()->_stream;
...
bool TR_J9ServerVM::isCloneable(TR_OpaqueClassBlock *clazzPointer)
{
    uintptr_t classDepthAndFlags = 0;
    JITServer::ServerStream *stream = _compInfoPT->getMethodBeingCompiled()->_stream;
    JITServerHelpers::getAndCacheRAMClassInfo((J9Class *)clazzPointer, _compInfoPT->getClientData(), stream,
        JITServerHelpers::CLASSINFO_CLASS_DEPTH_AND_FLAGS, (void *)&classDepthAndFlags);
    return ((classDepthAndFlags & J9AccClassCloneable) != 0);
}

bool TR_J9ServerVM::canAllocateInlineClass(TR_OpaqueClassBlock *clazz)
{
    uint32_t modifiers = 0;
...
TR_OpaqueClassBlock *TR_J9ServerVM::getArrayClassFromComponentClass(TR_OpaqueClassBlock *componentClass)
{
    JITServer::ServerStream *stream = _compInfoPT->getMethodBeingCompiled()->_stream;
    TR_OpaqueClassBlock *arrayClass = NULL;
    JITServerHelpers::getAndCacheRAMClassInfo((J9Class *)componentClass, _compInfoPT->getClientData(), stream,
        JITServerHelpers::CLASSINFO_ARRAY_CLASS, (void *)&arrayClass);
    if (!arrayClass) {
...
TR_OpaqueClassBlock *TR_J9ServerVM::getNullRestrictedArrayClassFromComponentClass(TR_OpaqueClassBlock *componentClass)
{
    JITServer::ServerStream *stream = _compInfoPT->getMethodBeingCompiled()->_stream;
    TR_OpaqueClassBlock *nullRestrictedArrayClass = NULL;
    JITServerHelpers::getAndCacheRAMClassInfo((J9Class *)componentClass, _compInfoPT->getClientData(), stream,
        JITServerHelpers::CLASSINFO_NULLRESTRICTED_ARRAY_CLASS, (void *)&nullRestrictedArrayClass);
...
J9Class *TR_J9ServerVM::matchRAMclassFromROMclass(J9ROMClass *clazz, TR::Compilation *comp)
{
    JITServer::ServerStream *stream = _compInfoPT->getMethodBeingCompiled()->_stream;
...
int32_t *TR_J9ServerVM::getCurrentLocalsMapForDLT(TR::Compilation *comp)
{
    ...
    jitConfig->javaVM->localMapFunction(_portLibrary, currentMethod->romClassPtr(), currentMethod->romMethod(),
        comp->getDltBcIndex(), (U_32 *)currentBundles, NULL, NULL, NULL);
#endif
    return currentBundles;
}

uintptr_t TR_J9ServerVM::getReferenceFieldAtAddress(uintptr_t fieldAddress)
{
    TR_ASSERT_FATAL(false, "getReferenceFieldAtAddress() should not be called by JITServer");
}

uintptr_t TR_J9ServerVM::getReferenceFieldAt(uintptr_t objectPointer, uintptr_t fieldOffset)
{
    TR_ASSERT_FATAL(false, "getReferenceFieldAt() should not be called by JITServer");
}

uintptr_t TR_J9ServerVM::getVolatileReferenceFieldAt(uintptr_t objectPointer, uintptr_t fieldOffset)
{
    TR_ASSERT_FATAL(false, "getVolatileReferenceFieldAt() should not be called by JITServer");
}

int32_t TR_J9ServerVM::getInt32FieldAt(uintptr_t objectPointer, uintptr_t fieldOffset)
{
    JITServer::ServerStream *stream = _compInfoPT->getMethodBeingCompiled()->_stream;
...
int64_t TR_J9ServerVM::getInt64FieldAt(uintptr_t objectPointer, uintptr_t fieldOffset)
{
    JITServer::ServerStream *stream = _compInfoPT->getMethodBeingCompiled()->_stream;
...
void TR_J9ServerVM::setInt64FieldAt(uintptr_t objectPointer, uintptr_t fieldOffset, int64_t newValue)
{
    JITServer::ServerStream *stream = _compInfoPT->getMethodBeingCompiled()->_stream;
...
bool TR_J9ServerVM::compareAndSwapInt64FieldAt(uintptr_t objectPointer, uintptr_t fieldOffset, int64_t oldValue,
    int64_t newValue)
{
    JITServer::ServerStream *stream = _compInfoPT->getMethodBeingCompiled()->_stream;
    stream->write(JITServer::MessageType::VM_compareAndSwapInt64FieldAt, objectPointer, fieldOffset, oldValue,
        newValue);
    return std::get<0>(stream->read<bool>());
}

intptr_t TR_J9ServerVM::getArrayLengthInElements(uintptr_t objectPointer)
{
    JITServer::ServerStream *stream = _compInfoPT->getMethodBeingCompiled()->_stream;
...
TR_OpaqueClassBlock *TR_J9ServerVM::getClassFromJavaLangClass(uintptr_t objectPointer)
{
    JITServer::ServerStream *stream = _compInfoPT->getMethodBeingCompiled()->_stream;
...
uintptr_t TR_J9ServerVM::getConstantPoolFromMethod(TR_OpaqueMethodBlock *method)
{
    TR_OpaqueClassBlock *owningClass = getClassFromMethodBlock(method);
    return getConstantPoolFromClass(owningClass);
}

uintptr_t TR_J9ServerVM::getConstantPoolFromClass(TR_OpaqueClassBlock *clazz)
{
    J9ConstantPool *cp;
    JITServer::ServerStream *stream = _compInfoPT->getMethodBeingCompiled()->_stream;
    JITServerHelpers::getAndCacheRAMClassInfo((J9Class *)clazz, _compInfoPT->getClientData(), stream,
        JITServerHelpers::CLASSINFO_CONSTANT_POOL, (void *)&cp);
    return reinterpret_cast<uintptr_t>(cp);
}

uintptr_t TR_J9ServerVM::getProcessID()
{
    JITServer::ServerStream *stream = _compInfoPT->getMethodBeingCompiled()->_stream;
    auto *vmInfo = _compInfoPT->getClientData()->getOrCacheVMInfo(stream);
...
bool TR_J9ServerVM::scanReferenceSlotsInClassForOffset(TR::Compilation *comp, TR_OpaqueClassBlock *clazz,
    int32_t offset)
{
    JITServer::ServerStream *stream = _compInfoPT->getMethodBeingCompiled()->_stream;
...
int32_t TR_J9ServerVM::findFirstHotFieldTenuredClassOffset(TR::Compilation *comp, TR_OpaqueClassBlock *clazz)
{
    JITServer::ServerStream *stream = _compInfoPT->getMethodBeingCompiled()->_stream;
...
TR_OpaqueMethodBlock *TR_J9ServerVM::getResolvedVirtualMethod(TR_OpaqueClassBlock *classObject, I_32 virtualCallOffset,
...
TR::CodeCache *TR_J9ServerVM::getDesignatedCodeCache(TR::Compilation *comp)
{
...
uint8_t *TR_J9ServerVM::allocateCodeMemory(TR::Compilation *comp, uint32_t warmCodeSize, uint32_t coldCodeSize,
    uint8_t **coldCode, bool isMethodHeaderNeeded)
{
    ...
    TR_ASSERT_FATAL(comp->getRelocatableMethodCodeStart(),
        "Should have set relocatable method code start by this point");
    return warmCode;
}

bool TR_J9ServerVM::sameClassLoaders(TR_OpaqueClassBlock *class1, TR_OpaqueClassBlock *class2)
{
    void *class1Loader = NULL;
    void *class2Loader = NULL;
    JITServer::ServerStream *stream = _compInfoPT->getMethodBeingCompiled()->_stream;
    JITServerHelpers::getAndCacheRAMClassInfo((J9Class *)class1, _compInfoPT->getClientData(), stream,
        JITServerHelpers::CLASSINFO_CLASS_LOADER, (void *)&class1Loader);
    JITServerHelpers::getAndCacheRAMClassInfo((J9Class *)class2, _compInfoPT->getClientData(), stream,
        JITServerHelpers::CLASSINFO_CLASS_LOADER, (void *)&class2Loader);

    return (class1Loader == class2Loader);
}

bool TR_J9ServerVM::isUnloadAssumptionRequired(TR_OpaqueClassBlock *clazz, TR_ResolvedMethod *methodBeingCompiled)
{
    TR_OpaqueClassBlock *classOfMethod
        = static_cast<TR_ResolvedJ9JITServerMethod *>(methodBeingCompiled)->classOfMethod();
    ...
    JITServerHelpers::getAndCacheRAMClassInfo((J9Class *)clazz, _compInfoPT->getClientData(), stream,
        JITServerHelpers::CLASSINFO_CLASS_LOADER, (void *)&classLoader,
        JITServerHelpers::CLASSINFO_ROMCLASS_EXTRAMODIFIERS, (void *)&extraModifiers);

    if (!J9_ARE_ALL_BITS_SET(extraModifiers, J9AccClassAnonClass)) {
        if (classLoader == getSystemClassLoader()) {
            return false;
        } else {
            JITServerHelpers::getAndCacheRAMClassInfo((J9Class *)classOfMethod, _compInfoPT->getClientData(), stream,
                JITServerHelpers::CLASSINFO_CLASS_LOADER, (void *)&classOfMethodLoader);
...
uint32_t TR_J9ServerVM::getInstanceFieldOffset(TR_OpaqueClassBlock *clazz, const char *fieldName, uint32_t fieldLen,
    const char *sig, uint32_t sigLen, UDATA options)
{
    JITServer::ServerStream *stream = _compInfoPT->getMethodBeingCompiled()->_stream;
    stream->write(JITServer::MessageType::VM_getInstanceFieldOffset, clazz, std::string(fieldName, fieldLen),
        std::string(sig, sigLen), options);
    return std::get<0>(stream->read<uint32_t>());
}

int32_t TR_J9ServerVM::getJavaLangClassHashCode(TR::Compilation *comp, TR_OpaqueClassBlock *clazz,
...
bool TR_J9ServerVM::hasFinalizer(TR_OpaqueClassBlock *clazz)
{
    uintptr_t classDepthAndFlags = 0;
    JITServer::ServerStream *stream = _compInfoPT->getMethodBeingCompiled()->_stream;
    JITServerHelpers::getAndCacheRAMClassInfo((J9Class *)clazz, _compInfoPT->getClientData(), stream,
    ...
    return ((classDepthAndFlags & (J9AccClassFinalizeNeeded | J9AccClassOwnableSynchronizer | J9AccClassContinuation))
        != 0);
}

uintptr_t TR_J9ServerVM::getClassDepthAndFlagsValue(TR_OpaqueClassBlock *clazz)
{
    JITServer::ServerStream *stream = _compInfoPT->getMethodBeingCompiled()->_stream;
...
uintptr_t TR_J9ServerVM::getClassFlagsValue(TR_OpaqueClassBlock *clazz)
{
    JITServer::ServerStream *stream = _compInfoPT->getMethodBeingCompiled()->_stream;
...
TR_OpaqueMethodBlock *TR_J9ServerVM::getMethodFromClass(TR_OpaqueClassBlock *methodClass, const char *methodName,
    const char *signature, TR_OpaqueClassBlock *callingClass)
{
    ...
    PersistentUnorderedMap<ClassMethodNamePair, TR_OpaqueMethodBlock *> &methodMap
        = _compInfoPT->getClientData()->getMethodByNameMap();
    ...
    JITServer::ServerStream *stream = _compInfoPT->getMethodBeingCompiled()->_stream;
    stream->write(JITServer::MessageType::VM_getMethodFromClass, methodClass,
        std::string(methodName, strlen(methodName)), std::string(signature, strlen(signature)), callingClass);
        ...
        bool cache = false;
        {
            ...
            if (it != romClassMap.end()) {
                ...
                ClientSessionData::VMInfo *vminfo
                    = _compInfoPT->getClientData()->getOrCacheVMInfo(_compInfoPT->getMethodBeingCompiled()->_stream);
...
bool TR_J9ServerVM::isClassVisible(TR_OpaqueClassBlock *sourceClass, TR_OpaqueClassBlock *destClass)
{
    JITServer::ServerStream *stream = _compInfoPT->getMethodBeingCompiled()->_stream;
...
void *TR_J9ServerVM::getClientJ2IThunk(const std::string &signature, TR::Compilation *comp)
{
    {
...
void *TR_J9ServerVM::getJ2IThunk(char *signatureChars, uint32_t signatureLength, TR::Compilation *comp)
{
...
void *TR_J9ServerVM::sendJ2IThunkToClient(const std::string &signature, const uint8_t *thunkStart,
    const uint32_t thunkSize, TR::Compilation *comp)
{
...
void *TR_J9ServerVM::setJ2IThunk(char *signatureChars, uint32_t signatureLength, void *thunkptr, TR::Compilation *comp)
{
    std::string signature(signatureChars, signatureLength);
    ...
    void *clientThunkPtr = NULL;
    if (comp->isAOTCacheStore()) {
        auto record = comp->getClientData()->getAOTCache()->createAndStoreThunk((uint8_t *)signatureChars,
            signatureLength, thunkStart, thunkSize);
        comp->addThunkRecord(record);
...
void TR_J9ServerVM::markClassForTenuredAlignment(TR::Compilation *comp, TR_OpaqueClassBlock *clazz,
...
void TR_J9ServerVM::reportHotField(int32_t reducedCpuUtil, J9Class *clazz, uint8_t fieldOffset,
    uint32_t reducedFrequency)
{
    JITServer::ServerStream *stream = _compInfoPT->getMethodBeingCompiled()->_stream;
...
int32_t *TR_J9ServerVM::getReferenceSlotsInClass(TR::Compilation *comp, TR_OpaqueClassBlock *clazz)
{
...
uint32_t TR_J9ServerVM::getMethodSize(TR_OpaqueMethodBlock *method)
{
    JITServer::ServerStream *stream = _compInfoPT->getMethodBeingCompiled()->_stream;
...
void *TR_J9ServerVM::getStaticFieldAddress(TR_OpaqueClassBlock *clazz, unsigned char *fieldName, uint32_t fieldLen,
    unsigned char *sig, uint32_t sigLen)
{
    JITServer::ServerStream *stream = _compInfoPT->getMethodBeingCompiled()->_stream;
    stream->write(JITServer::MessageType::VM_getStaticFieldAddress, clazz,
        std::string(reinterpret_cast<char *>(fieldName), fieldLen), std::string(reinterpret_cast<char *>(sig), sigLen));
    return std::get<0>(stream->read<void *>());
}

int32_t TR_J9ServerVM::getInterpreterVTableSlot(TR_OpaqueMethodBlock *method, TR_OpaqueClassBlock *clazz)
{
    JITServer::ServerStream *stream = _compInfoPT->getMethodBeingCompiled()->_stream;
...
void TR_J9ServerVM::revertToInterpreted(TR_OpaqueMethodBlock *method)
{
    JITServer::ServerStream *stream = _compInfoPT->getMethodBeingCompiled()->_stream;
...
void *TR_J9ServerVM::getLocationOfClassLoaderObjectPointer(TR_OpaqueClassBlock *clazz)
{
    JITServer::ServerStream *stream = _compInfoPT->getMethodBeingCompiled()->_stream;
...
bool TR_J9ServerVM::isOwnableSyncClass(TR_OpaqueClassBlock *clazz)
{
    uintptr_t classDepthAndFlags = 0;
    JITServer::ServerStream *stream = _compInfoPT->getMethodBeingCompiled()->_stream;
    JITServerHelpers::getAndCacheRAMClassInfo((J9Class *)clazz, _compInfoPT->getClientData(), stream,
...
bool TR_J9ServerVM::isContinuationClass(TR_OpaqueClassBlock *clazz)
{
    uintptr_t classDepthAndFlags = 0;
    JITServer::ServerStream *stream = _compInfoPT->getMethodBeingCompiled()->_stream;
    JITServerHelpers::getAndCacheRAMClassInfo((J9Class *)clazz, _compInfoPT->getClientData(), stream,
...
TR_OpaqueClassBlock *TR_J9ServerVM::getClassFromMethodBlock(TR_OpaqueMethodBlock *method)
{
    {
        OMR::CriticalSection getRemoteROMClass(_compInfoPT->getClientData()->getROMMapMonitor());
        auto it = _compInfoPT->getClientData()->getJ9MethodMap().find((J9Method *)method);
...
U_8 *TR_J9ServerVM::fetchMethodExtendedFlagsPointer(J9Method *method)
{
    JITServer::ServerStream *stream = _compInfoPT->getMethodBeingCompiled()->_stream;
...
void *TR_J9ServerVM::getStaticHookAddress(int32_t event)
{
    JITServer::ServerStream *stream = _compInfoPT->getMethodBeingCompiled()->_stream;
...
bool TR_J9ServerVM::stringEquals(TR::Compilation *comp, uintptr_t *stringLocation1, uintptr_t *stringLocation2,
    int32_t &result)
{
    JITServer::ServerStream *stream = _compInfoPT->getMethodBeingCompiled()->_stream;
...
bool TR_J9ServerVM::getStringHashCode(TR::Compilation *comp, uintptr_t *stringLocation, int32_t &result)
{
    JITServer::ServerStream *stream = _compInfoPT->getMethodBeingCompiled()->_stream;
...
int32_t TR_J9ServerVM::getLineNumberForMethodAndByteCodeIndex(TR_OpaqueMethodBlock *method, int32_t bcIndex)
{
...
TR_OpaqueMethodBlock *TR_J9ServerVM::getObjectNewInstanceImplMethod()
{
    JITServer::ServerStream *stream = _compInfoPT->getMethodBeingCompiled()->_stream;
...
uintptr_t TR_J9ServerVM::getBytecodePC(TR_OpaqueMethodBlock *method, TR_ByteCodeInfo &bcInfo)
{
    JITServer::ServerStream *stream = _compInfoPT->getMethodBeingCompiled()->_stream;
...
bool TR_J9ServerVM::isClassLoadedBySystemClassLoader(TR_OpaqueClassBlock *clazz)
{
    void *classLoader = NULL;
    JITServer::ServerStream *stream = _compInfoPT->getMethodBeingCompiled()->_stream;
    JITServerHelpers::getAndCacheRAMClassInfo((J9Class *)clazz, _compInfoPT->getClientData(), stream,
...
void TR_J9ServerVM::setInvokeExactJ2IThunk(void *thunkptr, TR::Compilation *comp)
{
...
bool TR_J9ServerVM::needsInvokeExactJ2IThunk(TR::Node *callNode, TR::Compilation *comp)
{
    ...
    if (methodSymbol->isComputed()
        ...
        thunkTable->getTerseSignature(terseSignature, sizeof(terseSignature),
            methodSymbol->getMethod()->signatureChars());
        ...
        JITServer::ServerStream *stream = _compInfoPT->getMethodBeingCompiled()->_stream;
        std::string signature(methodSymbol->getMethod()->signatureChars(),
            methodSymbol->getMethod()->signatureLength());
...
TR_ResolvedMethod *TR_J9ServerVM::createMethodHandleArchetypeSpecimen(TR_Memory *trMemory,
    ...
    uintptr_t *methodHandleLocation, TR_ResolvedMethod *owningMethod)
{
    ...
    TR_ResolvedMethod *result = createResolvedMethodWithSignature(trMemory, archetype, NULL,
        (char *)thunkableSignature.data(), thunkableSignature.length(), owningMethod, methodInfo);
    result->convertToMethod()->setArchetypeSpecimen();
    result->setMethodHandleLocation(methodHandleLocation);
    return result;
}

TR::KnownObjectTable::Index TR_J9ServerVM::mutableCallSiteEpoch(TR::Compilation *comp, TR::KnownObjectTable::Index mcs)
{
    JITServer::ServerStream *stream = _compInfoPT->getMethodBeingCompiled()->_stream;
...
intptr_t TR_J9ServerVM::getVFTEntry(TR_OpaqueClassBlock *clazz, int32_t offset)
{
    JITServer::ServerStream *stream = _compInfoPT->getMethodBeingCompiled()->_stream;
...
bool TR_J9ServerVM::isClassArray(TR_OpaqueClassBlock *klass)
{
...
bool TR_J9ServerVM::instanceOfOrCheckCastHelper(J9Class *instanceClass, J9Class *castClass, bool cacheUpdate)
{
    ...
    // since superclasses and interfaces are cached on the server
    {
        ...
        if (it != _compInfoPT->getClientData()->getROMClassMap().end()) {
            ...
            if (isInterfaceClass(castClassOffset)) {
                ...
                // We reach the end of this block when instanceNumDims > castNumDims, or when one of the arrays contains
                // primitive values. To correctly deal with these cases on the server, we need to call
                // getComponentFromArrayClass multiple times, or getLeafComponentTypeFromArrayClass.
                // Each of these calls requires a remote message. It's faster and easier to just call
                // instanceOfOrCheckCast on the client. So we exit out of the critical section here and do the remote
                // call.
            } else {
    ...
    // instance class is not cached or isClassArray(instanceClassOffset) && isClassArray(castClassOffset). So make a
    // remote call.
    JITServer::ServerStream *stream = _compInfoPT->getMethodBeingCompiled()->_stream;
    if (cacheUpdate)
...
bool TR_J9ServerVM::instanceOfOrCheckCast(J9Class *instanceClass, J9Class *castClass)
{
    return instanceOfOrCheckCastHelper(instanceClass, castClass, true);
}

bool TR_J9ServerVM::instanceOfOrCheckCastNoCacheUpdate(J9Class *instanceClass, J9Class *castClass)
{
...
bool TR_J9ServerVM::isAnonymousClass(TR_OpaqueClassBlock *j9clazz)
{
    uint32_t extraModifiers = 0;
    JITServer::ServerStream *stream = _compInfoPT->getMethodBeingCompiled()->_stream;
    JITServerHelpers::getAndCacheRAMClassInfo((J9Class *)j9clazz, _compInfoPT->getClientData(), stream,
        JITServerHelpers::CLASSINFO_ROMCLASS_EXTRAMODIFIERS, (void *)&extraModifiers);

    return (J9_ARE_ALL_BITS_SET(extraModifiers, J9AccClassAnonClass));
}

bool TR_J9ServerVM::isHiddenClass(TR_OpaqueClassBlock *j9clazz)
{
    uint32_t extraModifiers = 0;
    JITServer::ServerStream *stream = _compInfoPT->getMethodBeingCompiled()->_stream;
    JITServerHelpers::getAndCacheRAMClassInfo((J9Class *)j9clazz, _compInfoPT->getClientData(), stream,
...
TR_IProfiler *TR_J9ServerVM::getIProfiler()
{
    JITServer::ServerStream *stream = _compInfoPT->getMethodBeingCompiled()->_stream;
    auto *vmInfo = _compInfoPT->getClientData()->getOrCacheVMInfo(stream);
...
TR_StaticFinalData TR_J9ServerVM::dereferenceStaticFinalAddress(void *staticAddress, TR::DataType addressType)
{
    if (!staticAddress)
...
TR_OpaqueClassBlock *TR_J9ServerVM::getClassFromCP(J9ConstantPool *cp)
{
...
void TR_J9ServerVM::reserveTrampolineIfNecessary(TR::Compilation *comp, TR::SymbolReference *symRef,
    bool inBinaryEncoding)
{
    ...
    if (!comp->compileRelocatableCode()
        && _compInfoPT->getClientData()->getOrCacheVMInfo(stream)->_needsMethodTrampolines && !symRef->isUnresolved()) {
...
intptr_t TR_J9ServerVM::methodTrampolineLookup(TR::Compilation *comp, TR::SymbolReference *symRef, void *callSite)
{
    // Not necessary in JITServer server mode, return the call's PC so that the call will not appear to require a
    // trampoline
    return (intptr_t)callSite;
}

uintptr_t TR_J9ServerVM::getPersistentClassPointerFromClassPointer(TR_OpaqueClassBlock *clazz)
{
    J9ROMClass *remoteRomClass = NULL;
    JITServer::ServerStream *stream = _compInfoPT->getMethodBeingCompiled()->_stream;
    JITServerHelpers::getAndCacheRAMClassInfo((J9Class *)clazz, _compInfoPT->getClientData(), stream,
        JITServerHelpers::CLASSINFO_REMOTE_ROM_CLASS, (void *)&remoteRomClass);
    return (uintptr_t)remoteRomClass;
}

TR_OpaqueClassBlock *TR_J9ServerVM::getClassFromNewArrayTypeNonNull(int32_t arrayType)
{
    JITServer::ServerStream *stream = _compInfoPT->getMethodBeingCompiled()->_stream;
    auto *vmInfo = _compInfoPT->getClientData()->getOrCacheVMInfo(stream);
...
bool TR_J9ServerVM::isStringCompressionEnabledVM()
{
    JITServer::ServerStream *stream = _compInfoPT->getMethodBeingCompiled()->_stream;
    auto *vmInfo = _compInfoPT->getClientData()->getOrCacheVMInfo(stream);
...
J9ROMMethod *TR_J9ServerVM::getROMMethodFromRAMMethod(J9Method *ramMethod)
{
    {
...
bool TR_J9ServerVM::getReportByteCodeInfoAtCatchBlock()
{
    JITServer::ServerStream *stream = _compInfoPT->getMethodBeingCompiled()->_stream;
    return _compInfoPT->getClientData()->getOrCacheVMInfo(stream)->_reportByteCodeInfoAtCatchBlock;
}

void *TR_J9ServerVM::getInvokeExactThunkHelperAddress(TR::Compilation *comp, TR::SymbolReference *glueSymRef,
...
bool TR_J9ServerVM::acquireClassTableMutex()
{
    // Acquire per-client class table lock
    TR::Monitor *classTableMonitor
        = static_cast<JITServerPersistentCHTable *>(_compInfoPT->getClientData()->getCHTable())->getCHTableMonitor();
...
void TR_J9ServerVM::releaseClassTableMutex(bool releaseVMAccess)
{
...
bool TR_J9ServerVM::getNurserySpaceBounds(uintptr_t *base, uintptr_t *top)
{
    JITServer::ServerStream *stream = _compInfoPT->getMethodBeingCompiled()->_stream;
...
TR_J9VMBase::MethodOfHandle TR_J9ServerVM::methodOfDirectOrVirtualHandle(uintptr_t *mh, bool isVirtual)
{
...
TR_OpaqueMethodBlock *TR_J9ServerVM::targetMethodFromMemberName(TR::Compilation *comp,
...
TR_OpaqueMethodBlock *TR_J9ServerVM::targetMethodFromMemberName(uintptr_t memberName)
{
    TR_ASSERT_FATAL(false, "targetMethodFromMemberName must not be called on JITServer");
    return NULL;
}

TR_OpaqueMethodBlock *TR_J9ServerVM::targetMethodFromMethodHandle(TR::Compilation *comp,
    TR::KnownObjectTable::Index objIndex)
{
    auto knot = comp->getKnownObjectTable();
    if (objIndex != TR::KnownObjectTable::UNKNOWN && knot && !knot->isNull(objIndex)) {
...
bool TR_J9ServerVM::isInvokeCacheEntryAnArray(uintptr_t *invokeCacheArray)
{
    JITServer::ServerStream *stream = _compInfoPT->getMethodBeingCompiled()->_stream;
    stream->write(JITServer::MessageType::VM_isInvokeCacheEntryAnArray, invokeCacheArray);
    return std::get<0>(stream->read<bool>());
}

TR::KnownObjectTable::Index TR_J9ServerVM::getKnotIndexOfInvokeCacheArrayAppendixElement(TR::Compilation *comp,
    uintptr_t *invokeCacheArray)
{
    ...
    JITServer::ServerStream *stream = _compInfoPT->getMethodBeingCompiled()->_stream;
    stream->write(JITServer::MessageType::VM_getKnotIndexOfInvokeCacheArrayAppendixElement, invokeCacheArray);
    auto recv = stream->read<TR::KnownObjectTable::Index, uintptr_t *>();
...
TR_ResolvedMethod *TR_J9ServerVM::targetMethodFromInvokeCacheArrayMemberNameObj(TR::Compilation *comp,
    TR_ResolvedMethod *owningMethod, uintptr_t *invokeCacheArray)
{
    TR_OpaqueMethodBlock *targetMethodObj = 0;
    JITServer::ServerStream *stream = _compInfoPT->getMethodBeingCompiled()->_stream;
    stream->write(JITServer::MessageType::VM_targetMethodFromInvokeCacheArrayMemberNameObj,
        static_cast<TR_ResolvedJ9JITServerMethod *>(owningMethod)->getRemoteMirror(), invokeCacheArray);
...
bool TR_J9ServerVM::getMemberNameMethodInfo(TR::Compilation *comp, TR::KnownObjectTable::Index objIndex,
    MemberNameMethodInfo *out)
{
    *out = {};

    auto knot = comp->getKnownObjectTable();
    if (objIndex == TR::KnownObjectTable::UNKNOWN || knot == NULL || knot->isNull(objIndex)) {
        return false;
    }
...
TR::KnownObjectTable::Index TR_J9ServerVM::delegatingMethodHandleTargetHelper(TR::Compilation *comp,
...
int32_t TR_J9ServerVM::getVarHandleAccessDescriptorMode(TR::Compilation *comp, TR::KnownObjectTable::Index adIndex)
{
    TR::KnownObjectTable *knot = comp->getKnownObjectTable();
    if (!knot)
        return -1;
...
TR::KnownObjectTable::Index TR_J9ServerVM::getMethodHandleTableEntryIndex(TR::Compilation *comp,
...
TR::KnownObjectTable::Index TR_J9ServerVM::getLayoutVarHandle(TR::Compilation *comp,
...
TR::KnownObjectTable::Index TR_J9ServerVM::getMethodAccessorIndex(TR::Compilation *comp,
...
{
    JITServer::ServerStream *stream = _compInfoPT->getMethodBeingCompiled()->_stream;
    stream->write(JITServer::MessageType::VM_getMemberNameFieldKnotIndexFromMethodHandleKnotIndex, mhIndex,
...
bool TR_J9ServerVM::isMethodHandleExpectedType(TR::Compilation *comp, TR::KnownObjectTable::Index mhIndex,
...
bool TR_J9ServerVM::isLambdaFormGeneratedMethod(TR_ResolvedMethod *method)
{
    return static_cast<TR_ResolvedJ9JITServerMethod *>(method)->isLambdaFormGeneratedMethod();
}

bool TR_J9ServerVM::isStable(J9Class *fieldClass, int32_t cpIndex)
{
...
bool TR_J9ServerVM::inSnapshotMode()
{
    JITServer::ServerStream *stream = _compInfoPT->getMethodBeingCompiled()->_stream;
    auto *vmInfo = _compInfoPT->getClientData()->getOrCacheVMInfo(stream);
...
bool TR_J9ServerVM::isSnapshotModeEnabled()
{
    JITServer::ServerStream *stream = _compInfoPT->getMethodBeingCompiled()->_stream;
    auto *vmInfo = _compInfoPT->getClientData()->getOrCacheVMInfo(stream);
...
bool TR_J9ServerVM::isPortableRestoreModeEnabled()
{
    JITServer::ServerStream *stream = _compInfoPT->getMethodBeingCompiled()->_stream;
    auto *vmInfo = _compInfoPT->getClientData()->getOrCacheVMInfo(stream);
...
bool TR_J9ServerVM::isIndexableDataAddrPresent()
{
 ...
 */
bool TR_J9ServerVM::isOffHeapAllocationEnabled()
{
    JITServer::ServerStream *stream = _compInfoPT->getMethodBeingCompiled()->_stream;
    auto *vmInfo = _compInfoPT->getClientData()->getOrCacheVMInfo(stream);
...
TR_arrayTypeCode TR_J9ServerVM::getPrimitiveArrayTypeCode(TR_OpaqueClassBlock *clazz)
{
...
TR::DataType TR_J9ServerVM::getClassPrimitiveDataType(TR_OpaqueClassBlock *clazz)
{
...
TR_OpaqueClassBlock *TR_J9ServerVM::getArrayClassFromDataType(TR::DataType type, bool booleanClass)
{
    J9Class *j9class;
    auto vmInfo = _compInfoPT->getClientData()->getOrCacheVMInfo(_compInfoPT->getStream());
...
uint32_t TR_J9ServerVM::numInterfacesImplemented(J9Class *clazz)
{
    JITServer::ServerStream *stream = _compInfoPT->getMethodBeingCompiled()->_stream;
...
bool TR_J9SharedCacheServerVM::isClassLibraryMethod(TR_OpaqueMethodBlock *method, bool vettedForAOT)
{
    TR_ASSERT(vettedForAOT,
        "The TR_J9SharedCacheServerVM version of this method is expected to be called only from isClassLibraryMethod.\n"
...
TR_OpaqueMethodBlock *TR_J9SharedCacheServerVM::getMethodFromClass(TR_OpaqueClassBlock *methodClass,
    const char *methodName, const char *signature, TR_OpaqueClassBlock *callingClass)
{
    ...
    if (omb) {
        ...
        if (comp->getOption(TR_UseSymbolValidationManager)) {
            ...
            if (!((TR_ResolvedRelocatableJ9JITServerMethod *)comp->getCurrentMethod())
                     ->validateArbitraryClass(comp, (J9Class *)methodClass))
                omb = NULL;
            if (callingClass
                && !((TR_ResolvedRelocatableJ9JITServerMethod *)comp->getCurrentMethod())
...
bool TR_J9SharedCacheServerVM::isPublicClass(TR_OpaqueClassBlock *classPointer)
{
    TR::Compilation *comp = _compInfoPT->getCompilation();
    TR_ASSERT(comp, "Should be called only within a compilation");
...
TR_OpaqueClassBlock *TR_J9SharedCacheServerVM::getProfiledClassFromProfiledInfo(TR_ExtraAddressInfo *profiledInfo)
{
...
TR_YesNoMaybe TR_J9SharedCacheServerVM::isInstanceOf(TR_OpaqueClassBlock *a, TR_OpaqueClassBlock *b,
    bool objectTypeIsFixed, bool castTypeIsFixed, bool optimizeForAOT)
{
    TR::Compilation *comp = _compInfoPT->getCompilation();
...
TR_OpaqueClassBlock *TR_J9SharedCacheServerVM::getSystemClassFromClassName(const char *name, int32_t length,
    bool isVettedForAOT)
{
    TR::Compilation *comp = _compInfoPT->getCompilation();
...
bool TR_J9SharedCacheServerVM::supportAllocationInlining(TR::Compilation *comp, TR::Node *node)
{
...
TR_OpaqueClassBlock *TR_J9SharedCacheServerVM::getClassFromSignature(const char *sig, int32_t sigLength,
    J9ConstantPool *constantPool, bool isVettedForAOT)
{
    ...
    if (j9class) {
        if (comp->getOption(TR_UseSymbolValidationManager))
            ...
            validated = ((TR_ResolvedRelocatableJ9JITServerMethod *)comp->getCurrentMethod())
...
bool TR_J9SharedCacheServerVM::hasFinalizer(TR_OpaqueClassBlock *classPointer)
{
    TR::Compilation *comp = _compInfoPT->getCompilation();
    TR_ASSERT(comp, "Should be called only within a compilation");
...
bool TR_J9SharedCacheServerVM::isPrimitiveClass(TR_OpaqueClassBlock *classPointer)
{
    TR::Compilation *comp = _compInfoPT->getCompilation();
    TR_ASSERT(comp, "Should be called only within a compilation");
    ...
    bool validated = false;
    bool isPrimClass
        = TR_J9VMBase::isPrimitiveClass(classPointer); // TR_J9ServerVM also uses TR_J9VMBase::isPrimitiveClass
...
bool TR_J9SharedCacheServerVM::isMethodTracingEnabled(TR_OpaqueMethodBlock *method)
{
    TR::Compilation *comp = _compInfoPT->getCompilation();

    // We want to return the same answer as TR_J9VMBase unless we want to force it to allow tracing
    return TR_J9ServerVM::isMethodTracingEnabled(method) || comp->getOptions()->getOption(TR_EnableAOTMethodEnter)
        || comp->getOptions()->getOption(TR_EnableAOTMethodExit);
}

bool TR_J9SharedCacheServerVM::traceableMethodsCanBeInlined() { return true; }

bool TR_J9SharedCacheServerVM::canMethodEnterEventBeHooked()
{
    TR::Compilation *comp = _compInfoPT->getCompilation();
...
bool TR_J9SharedCacheServerVM::canMethodExitEventBeHooked()
{
    TR::Compilation *comp = _compInfoPT->getCompilation();
    bool option = comp->getOptions()->getOption(TR_EnableAOTMethodExit);
    // We want to return the same answer as TR_J9ServerVM unless we want to force it to allow tracing
...
bool TR_J9SharedCacheServerVM::methodsCanBeInlinedEvenIfEventHooksEnabled(TR::Compilation *comp)
{
    return !comp->getOption(TR_FullSpeedDebug);
}

int32_t TR_J9SharedCacheServerVM::getJavaLangClassHashCode(TR::Compilation *comp, TR_OpaqueClassBlock *clazzPointer,
    bool &hashCodeComputed)
{
...
bool TR_J9SharedCacheServerVM::javaLangClassGetModifiersImpl(TR_OpaqueClassBlock *clazzPointer, int32_t &result)
{
    return false;
}

uint32_t TR_J9SharedCacheServerVM::getInstanceFieldOffset(TR_OpaqueClassBlock *classPointer, const char *fieldName,
...
TR_OpaqueClassBlock *TR_J9SharedCacheServerVM::getSuperClass(TR_OpaqueClassBlock *classPointer)
{
    TR::Compilation *comp = _compInfoPT->getCompilation();
    TR_ASSERT(comp, "Should be called only within a compilation");
...
TR_ResolvedMethod *TR_J9SharedCacheServerVM::getResolvedMethodForNameAndSignature(TR_Memory *trMemory,
    TR_OpaqueClassBlock *classPointer, const char *methodName, const char *signature)
{
    ...
    TR_ResolvedMethod *resolvedMethod
        = TR_J9ServerVM::getResolvedMethodForNameAndSignature(trMemory, classPointer, methodName, signature);
...
bool TR_J9SharedCacheServerVM::isPrimitiveArray(TR_OpaqueClassBlock *classPointer)
{
    TR::Compilation *comp = _compInfoPT->getCompilation();
    TR_ASSERT(comp, "Should be called only within a compilation");
...
bool TR_J9SharedCacheServerVM::isReferenceArray(TR_OpaqueClassBlock *classPointer)
{
    TR::Compilation *comp = _compInfoPT->getCompilation();
    TR_ASSERT(comp, "Should be called only within a compilation");
...
TR_OpaqueMethodBlock *TR_J9SharedCacheServerVM::getInlinedCallSiteMethod(TR_InlinedCallSite *ics)
{
    return ics->_methodInfo;
}

uintptr_t TR_J9SharedCacheServerVM::getClassDepthAndFlagsValue(TR_OpaqueClassBlock *classPointer)
{
    TR::Compilation *comp = _compInfoPT->getCompilation();
    TR_ASSERT(comp, "Should be called only within a compilation");
...
uintptr_t TR_J9SharedCacheServerVM::getClassFlagsValue(TR_OpaqueClassBlock *classPointer)
{
    TR::Compilation *comp = _compInfoPT->getCompilation();
    TR_ASSERT(comp, "Should be called only within a compilation");
...
bool TR_J9SharedCacheServerVM::isClassVisible(TR_OpaqueClassBlock *sourceClass, TR_OpaqueClassBlock *destClass)
{
    TR::Compilation *comp = _compInfoPT->getCompilation();
    TR_ASSERT(comp, "Should be called only within a compilation");
...
TR_OpaqueClassBlock *TR_J9SharedCacheServerVM::getClassOfMethod(TR_OpaqueMethodBlock *method)
{
    TR::Compilation *comp = _compInfoPT->getCompilation();
    TR_ASSERT(comp, "Should be called only within a compilation");
...
J9Class *TR_J9SharedCacheServerVM::getClassForAllocationInlining(TR::Compilation *comp,
...
bool TR_J9SharedCacheServerVM::ensureOSRBufferSize(TR::Compilation *comp, uintptr_t osrFrameSizeInBytes,
    uintptr_t osrScratchBufferSizeInBytes, uintptr_t osrStackFrameSizeInBytes)
{
    bool valid = TR_J9ServerVM::ensureOSRBufferSize(comp, osrFrameSizeInBytes, osrScratchBufferSizeInBytes,
...
TR::CodeCache *TR_J9SharedCacheServerVM::getDesignatedCodeCache(TR::Compilation *comp)
{
    ...
    if (codeCache) {
        ...
        if (!(jitConfig->runtimeFlags & J9JIT_CODE_CACHE_FULL) && (numReserved > 0) && comp) {
...
TR_OpaqueClassBlock *TR_J9SharedCacheServerVM::getComponentClassFromArrayClass(TR_OpaqueClassBlock *arrayClass)
{
    TR::Compilation *comp = _compInfoPT->getCompilation();
    TR_ASSERT(comp, "Should be called only within a compilation");
...
TR_OpaqueClassBlock *TR_J9SharedCacheServerVM::getArrayClassFromComponentClass(TR_OpaqueClassBlock *componentClass)
{
    TR::Compilation *comp = _compInfoPT->getCompilation();
    TR_ASSERT(comp, "Should be called only within a compilation");
...
TR_OpaqueClassBlock *TR_J9SharedCacheServerVM::getNullRestrictedArrayClassFromComponentClass(
    TR_OpaqueClassBlock *componentClass)
{
    ...
    TR_OpaqueClassBlock *nullRestrictedArrayClass
        = TR_J9ServerVM::getNullRestrictedArrayClassFromComponentClass(componentClass);
...
TR_OpaqueClassBlock *TR_J9SharedCacheServerVM::getLeafComponentClassFromArrayClass(TR_OpaqueClassBlock *arrayClass)
{
    TR::Compilation *comp = _compInfoPT->getCompilation();
    TR_ASSERT(comp, "Should be called only within a compilation");
...
TR_OpaqueClassBlock *TR_J9SharedCacheServerVM::getBaseComponentClass(TR_OpaqueClassBlock *classPointer,
    int32_t &numDims)
{
    TR::Compilation *comp = _compInfoPT->getCompilation();
    TR_ASSERT(comp, "Should be called only within a compilation");
    ...
    return validated
        ? baseComponent
        : classPointer; // not sure about this return value, but it's what we used to return before we got "smart"
}

TR_OpaqueClassBlock *TR_J9SharedCacheServerVM::getClassFromNewArrayType(int32_t arrayType)
{
    TR::Compilation *comp = _compInfoPT->getCompilation();
    if (comp && comp->getOption(TR_UseSymbolValidationManager))
...
TR_OpaqueMethodBlock *TR_J9SharedCacheServerVM::getResolvedVirtualMethod(TR_OpaqueClassBlock *classObject,
    I_32 virtualCallOffset, bool ignoreRtResolve)
{
    TR_OpaqueMethodBlock *ramMethod
        = TR_J9ServerVM::getResolvedVirtualMethod(classObject, virtualCallOffset, ignoreRtResolve);
...
TR_OpaqueMethodBlock *TR_J9SharedCacheServerVM::getResolvedInterfaceMethod(TR_OpaqueMethodBlock *interfaceMethod,
    TR_OpaqueClassBlock *classObject, I_32 cpIndex)
{
    ...
    if (comp && comp->getOption(TR_UseSymbolValidationManager)) {
        if (!comp->getSymbolValidationManager()->addInterfaceMethodFromCPRecord(ramMethod,
                (TR_OpaqueClassBlock *)TR_J9VM::getClassFromMethodBlock(interfaceMethod), classObject, cpIndex)) {
            return NULL;
        }
    }
    return ramMethod;
...
bool TR_J9SharedCacheServerVM::isResolvedDirectDispatchGuaranteed(TR::Compilation *comp)
{
    return isAotResolvedDirectDispatchGuaranteed(comp);
}

bool TR_J9SharedCacheServerVM::isResolvedVirtualDispatchGuaranteed(TR::Compilation *comp)
{
    return isAotResolvedVirtualDispatchGuaranteed(comp);
}

void TR_J9ServerVM::addBondMethodsFromClient(TR::Compilation *comp, const std::vector<TR_ClientBondMethod> &methods)
{
    ...
    for (auto it = methods.begin(), end = methods.end(); it != end; it++) {
        ...
        TR_ResolvedMethod *rm = createResolvedMethod(comp->trMemory(), m, owningMethod, info);

        logprintf(trace, log, "bond method from client: %p %.*s.%.*s%.*s\n", rm->getNonPersistentIdentifier(),
            rm->classNameLength(), rm->classNameChars(), rm->nameLength(), rm->nameChars(), rm->signatureLength(),
            rm->signatureChars());
