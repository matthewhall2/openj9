...
void TR_J9SharedCache::validateAOTHeader(J9JITConfig *jitConfig, J9VMThread *vmThread, TR::CompilationInfo *compInfo)
{
    ...
    if (!compInfo->reloRuntime()->validateAOTHeader(fe, vmThread)) {
        ...
        TR::Compiler->relocatableTarget.cpu
            = TR::CPU::customize(compInfo->reloRuntime()->getProcessorDescriptionFromSCC(vmThread));
    ...
    if (nextPage < end) {
        ...
        if (trace)
            TR_VerboseLog::writeLineLocked(TR_Vlog_PERF,
                "WARNING: Failed to use madvise to disclaim memory for shared class cache; errno: %d", errno);
...
const CCVResult TR_J9SharedCache::getCachedCCVResult(TR_OpaqueClassBlock *clazz)
{
...
bool TR_J9SharedCache::cacheCCVResult(TR_OpaqueClassBlock *clazz, CCVResult result)
{
    if (!TR::Options::getCmdLineOptions()->getOption(TR_DisableCHOpts)) {
        TR::ClassTableCriticalSection cacheResult(_fe);
        TR_PersistentCHTable *table = _compInfo->getPersistentInfo()->getPersistentCHTable();
...
TR_J9SharedCache::TR_J9SharedCache(TR_J9VMBase *fe)
{
...
#if defined(J9VM_OPT_JITSERVER)
    TR_ASSERT_FATAL(_sharedCacheConfig
            || _compInfo->getPersistentInfo()->getRemoteCompilationMode() == JITServer::SERVER
...
#endif
    {
        ...
        _initialHintSCount = std::min(TR::Options::getCmdLineOptions()->getInitialSCount(),
            TR::Options::getAOTCmdLineOptions()->getInitialSCount());
        if (_initialHintSCount == 0)
            _initialHintSCount = 1;

        _logLevel = std::max(TR::Options::getAOTCmdLineOptions()->getAotrtDebugLevel(),
            TR::Options::getCmdLineOptions()->getAotrtDebugLevel());
...
J9SharedClassCacheDescriptor *TR_J9SharedCache::getCacheDescriptorList()
{
#if defined(J9VM_OPT_SHARED_CLASSES)                                                                    \
    && (defined(TR_HOST_X86) || defined(TR_HOST_POWER) || defined(TR_HOST_S390) || defined(TR_HOST_ARM) \
        || defined(TR_HOST_ARM64))
...
void TR_J9SharedCache::log(const char *format, ...)
{
...
TR_J9SharedCache::SCCHint TR_J9SharedCache::getHint(J9VMThread *vmThread, J9Method *method)
{
    SCCHint result;
    ...
    if ((find != (SCCHint *)descriptor.address) || (dataIsCorrupt != -1)) {
       ...
       // || defined(TR_HOST_ARM) || defined(TR_HOST_ARM64))
    return result;
}

uint16_t TR_J9SharedCache::getAllEnabledHints(J9Method *method)
{
...
bool TR_J9SharedCache::isHint(J9Method *method, TR_SharedCacheHint theHint, uint16_t *dataField)
{
    bool isHint = false;
...
bool TR_J9SharedCache::isHint(TR_ResolvedMethod *method, TR_SharedCacheHint hint, uint16_t *dataField)
{
    return isHint(((TR_ResolvedJ9Method *)method)->ramMethod(), hint, dataField);
}

void TR_J9SharedCache::addHint(J9Method *method, TR_SharedCacheHint theHint)
{
#if defined(J9VM_OPT_SHARED_CLASSES)                                                                    \
    && (defined(TR_HOST_X86) || defined(TR_HOST_POWER) || defined(TR_HOST_S390) || defined(TR_HOST_ARM) \
        || defined(TR_HOST_ARM64))
    ...
    if (newHint) {
        ...
        if (scHints.flags == 0) // If no prior hints exist, we can perform a "storeAttachedData" operation
        {
            ...
            if (!SCfull) {
                ...
                descriptor.length = scHintDataLength; // Size includes the 2nd data field, currently only used for
                                                      // TR_HintFailedValidation
                descriptor.type = J9SHR_ATTACHED_DATA_TYPE_JITHINT;
                descriptor.flags = J9SHR_ATTACHED_DATA_NO_FLAGS;
                ...
                if (store == 0) {
                    if (_verboseHints)
                        TR_VerboseLog::writeLineLocked(TR_Vlog_SCHINTS, "hint added 0x%x, key = %s, scount: %d",
                            scHints.flags, methodSignature, scHints.data);
        ...
        } else // Some hints already exist for this method. We must perform an "updateAttachedData"
        {
            ...
            if (updateHint) {
                ...
                if (_verboseHints) {
                    if (update == 0) {
                        TR_VerboseLog::writeLineLocked(TR_Vlog_SCHINTS, "hint updated 0x%x, key = %s, scount: %d",
                            scHints.flags, methodSignature, scHints.data);
                    } else {
...
void TR_J9SharedCache::addHint(TR_ResolvedMethod *method, TR_SharedCacheHint hint)
{
    addHint(((TR_ResolvedJ9Method *)method)->ramMethod(), hint);
}

void TR_J9SharedCache::persistIprofileInfo(TR::ResolvedMethodSymbol *methodSymbol, TR::Compilation *comp)
{
    persistIprofileInfo(methodSymbol, methodSymbol->getResolvedMethod(), comp);
}

void TR_J9SharedCache::persistIprofileInfo(TR::ResolvedMethodSymbol *methodSymbol, TR_ResolvedMethod *method,
    TR::Compilation *comp)
{
    TR_J9VMBase *fej9 = (TR_J9VMBase *)(fe());
...
bool TR_J9SharedCache::isMostlyFull()
{
    return (double)sharedCacheConfig()->getFreeSpaceBytes(_javaVM) / sharedCacheConfig()->getCacheSizeBytes(_javaVM)
        < 0.8;
}

bool TR_J9SharedCache::isPointerInCache(const J9SharedClassCacheDescriptor *cacheDesc, void *ptr)
{
...
bool TR_J9SharedCache::isOffsetInCache(const J9SharedClassCacheDescriptor *cacheDesc, uintptr_t offset)
{
...
bool TR_J9SharedCache::isPointerInMetadataSectionInCache(const J9SharedClassCacheDescriptor *cacheDesc, void *ptr)
{
    ...
    if (isPointerInCache(cacheDesc, ptr)) {
        uintptr_t ptrValue = reinterpret_cast<uintptr_t>(ptr);
        uintptr_t metadataEndAddress
            = reinterpret_cast<uintptr_t>(UPDATEPTR(cacheDesc->cacheStartAddress)); // Inclusive
...
bool TR_J9SharedCache::isOffsetInMetadataSectionInCache(const J9SharedClassCacheDescriptor *cacheDesc, uintptr_t offset)
{
    ...
    if (isOffsetFromEnd(offset) && isOffsetInCache(cacheDesc, offset)) {
        ...
        isOffsetInMetadataSection
            = ((decodeOffsetFromEnd(offset) > 0) && (decodeOffsetFromEnd(offset) <= metadataEndOffset));
    }
#endif
    return isOffsetInMetadataSection;
}

bool TR_J9SharedCache::isPointerInROMClassesSectionInCache(const J9SharedClassCacheDescriptor *cacheDesc, void *ptr)
{
    ...
    if (isPointerInCache(cacheDesc, ptr)) {
        ...
        uintptr_t romclassEndAddress
            = reinterpret_cast<uintptr_t>(SEGUPDATEPTR(cacheDesc->cacheStartAddress)); // Exclusive
...
bool TR_J9SharedCache::isOffsetinROMClassesSectionInCache(const J9SharedClassCacheDescriptor *cacheDesc,
    uintptr_t offset)
{
...
void *TR_J9SharedCache::pointerFromOffsetInSharedCache(uintptr_t offset)
{
    uintptr_t ptr = 0;
...
void *TR_J9SharedCache::lookupClassLoaderAssociatedWithClassChain(void *chainData)
{
    return persistentClassLoaderTable()->lookupClassLoaderAssociatedWithClassChain(chainData);
}

void *TR_J9SharedCache::romStructureFromOffsetInSharedCache(uintptr_t offset)
{
...
J9ROMClass *TR_J9SharedCache::romClassFromOffsetInSharedCache(uintptr_t offset)
{
    return reinterpret_cast<J9ROMClass *>(romStructureFromOffsetInSharedCache(offset));
}

J9ROMMethod *TR_J9SharedCache::romMethodFromOffsetInSharedCache(uintptr_t offset)
{
    return reinterpret_cast<J9ROMMethod *>(romStructureFromOffsetInSharedCache(offset));
}

void *TR_J9SharedCache::ptrToROMClassesSectionFromOffsetInSharedCache(uintptr_t offset)
{
    return romStructureFromOffsetInSharedCache(offset);
}

bool TR_J9SharedCache::isOffsetInSharedCache(uintptr_t encoded_offset, void *ptr)
{
#if defined(J9VM_OPT_SHARED_CLASSES)                                                                    \
    && (defined(TR_HOST_X86) || defined(TR_HOST_POWER) || defined(TR_HOST_S390) || defined(TR_HOST_ARM) \
        || defined(TR_HOST_ARM64))
...
bool TR_J9SharedCache::isROMStructureOffsetInSharedCache(uintptr_t encoded_offset, void **romStructurePtr)
{
#if defined(J9VM_OPT_SHARED_CLASSES)                                                                    \
    && (defined(TR_HOST_X86) || defined(TR_HOST_POWER) || defined(TR_HOST_S390) || defined(TR_HOST_ARM) \
        || defined(TR_HOST_ARM64))
    ...
    do {
        ...
        if (isOffsetinROMClassesSectionInCache(curCache, encoded_offset)) {
            if (romStructurePtr) {
                ...
                *romStructurePtr
                    = reinterpret_cast<void *>(romclassStartAddress + decodeOffsetFromStart(encoded_offset));
            }
            return true;
        }
...
bool TR_J9SharedCache::isROMClassOffsetInSharedCache(uintptr_t offset, J9ROMClass **romClassPtr)
{
    return isROMStructureOffsetInSharedCache(offset, reinterpret_cast<void **>(romClassPtr));
}

bool TR_J9SharedCache::isROMMethodOffsetInSharedCache(uintptr_t offset, J9ROMMethod **romMethodPtr)
{
    return isROMStructureOffsetInSharedCache(offset, reinterpret_cast<void **>(romMethodPtr));
}

bool TR_J9SharedCache::isOffsetOfPtrToROMClassesSectionInSharedCache(uintptr_t offset, void **ptr)
{
    return isROMStructureOffsetInSharedCache(offset, ptr);
}

uintptr_t TR_J9SharedCache::offsetInSharedCacheFromPointer(void *ptr)
{
    uintptr_t offset = 0;
...
uintptr_t TR_J9SharedCache::offsetInSharedcacheFromROMStructure(void *romStructure)
{
    uintptr_t offset = 0;
...
uintptr_t TR_J9SharedCache::offsetInSharedCacheFromROMClass(J9ROMClass *romClass)
{
    return offsetInSharedcacheFromROMStructure(romClass);
}

uintptr_t TR_J9SharedCache::offsetInSharedCacheFromROMMethod(J9ROMMethod *romMethod)
{
...
uintptr_t TR_J9SharedCache::offsetInSharedCacheFromPtrToROMClassesSection(void *ptr)
{
    return offsetInSharedcacheFromROMStructure(ptr);
}

bool TR_J9SharedCache::isPointerInSharedCache(void *ptr, uintptr_t *cacheOffset)
{
#if defined(J9VM_OPT_SHARED_CLASSES)                                                                    \
    && (defined(TR_HOST_X86) || defined(TR_HOST_POWER) || defined(TR_HOST_S390) || defined(TR_HOST_ARM) \
        || defined(TR_HOST_ARM64))
...
bool TR_J9SharedCache::isROMStructureInSharedCache(void *romStructure, uintptr_t *cacheOffset)
{
#if defined(J9VM_OPT_SHARED_CLASSES)                                                                    \
    && (defined(TR_HOST_X86) || defined(TR_HOST_POWER) || defined(TR_HOST_S390) || defined(TR_HOST_ARM) \
        || defined(TR_HOST_ARM64))
    ...
    do {
        if (isPointerInROMClassesSectionInCache(curCache, romStructure)) {
            if (cacheOffset) {
                uintptr_t romclassStartAddress = reinterpret_cast<uintptr_t>(curCache->romclassStartAddress);
                *cacheOffset
                    = encodeOffsetFromStart(reinterpret_cast<uintptr_t>(romStructure) - romclassStartAddress + offset);
            }
            return true;
        }
        offset += curCache->cacheSizeBytes;
...
bool TR_J9SharedCache::isClassInSharedCache(TR_OpaqueClassBlock *clazz, uintptr_t *cacheOffset)
{
...
bool TR_J9SharedCache::isROMClassInSharedCache(J9ROMClass *romClass, uintptr_t *cacheOffset)
{
    return isROMStructureInSharedCache(romClass, cacheOffset);
}

bool TR_J9SharedCache::isROMMethodInSharedCache(J9ROMMethod *romMethod, uintptr_t *cacheOffset)
{
    return isROMStructureInSharedCache(romMethod, cacheOffset);
}

bool TR_J9SharedCache::isMethodInSharedCache(TR_OpaqueMethodBlock *method, TR_OpaqueClassBlock *definingClass,
...
bool TR_J9SharedCache::isPtrToROMClassesSectionInSharedCache(void *ptr, uintptr_t *cacheOffset)
{
    return isROMStructureInSharedCache(ptr, cacheOffset);
}

J9ROMClass *TR_J9SharedCache::startingROMClassOfClassChain(UDATA *classChain)
{
    return romClassFromOffsetInSharedCache(startingROMClassOffsetOfClassChain(classChain));
}

uintptr_t TR_J9SharedCache::startingROMClassOffsetOfClassChain(void *chain)
{
...
void TR_J9SharedCache::convertUnsignedOffsetToASCII(UDATA offset, char *buffer)
{
...
void TR_J9SharedCache::createClassKey(UDATA classOffsetInCache, char *key, uint32_t &keyLength)
{
    keyLength = _numDigitsForCacheOffsets;
    convertUnsignedOffsetToASCII(classOffsetInCache, key);
}

uintptr_t TR_J9SharedCache::rememberClass(J9Class *clazz, const AOTCacheClassChainRecord **classChainRecord,
    bool create)
{
    ...
    if (chainData != NULL) {
        ...
        if (classMatchesCachedVersion(clazz, chainData)) {
            if (isPointerInSharedCache(chainData, &chainOffset)) {
                ...
                LOG(1,
                    "\tcurrent class and class chain found (%p) are identical but its offset isn't available yet; "
                    "returning INVALID_CLASS_CHAIN_OFFSET\n",
                    chainData);
            }
        } else {
            LOG(1,
                "\tcurrent class and class chain found (%p) do not match, so cannot use class chain; returning "
...
TR_J9SharedCache::rememberDebugCounterName(const char *name)
{
    ...
    const U_8 *data = sharedCacheConfig()->storeSharedData(vmThread, (const char *)NULL, 0, &dataDescriptor);
...
const char *TR_J9SharedCache::getDebugCounterName(UDATA offset)
{
    const char *name = (offset != (UDATA)-1) ? (const char *)pointerFromOffsetInSharedCache(offset) : NULL;
    return name;
}

bool TR_J9SharedCache::writeClassToChain(J9ROMClass *romClass, UDATA *&chainPtr)
{
    uintptr_t classOffsetInCache;
    if (!isROMClassInSharedCache(romClass, &classOffsetInCache)) {
    ...
    J9UTF8 *className = J9ROMCLASS_CLASSNAME(romClass);
    LOG(3, "\t\tChain %p storing romclass %p (%.*s) offset %d\n", chainPtr, romClass, J9UTF8_LENGTH(className),
        J9UTF8_DATA(className), classOffsetInCache);
...
bool TR_J9SharedCache::writeInterfacesToChain(J9Class *clazz, UDATA *&chainPtr)
{
...
bool TR_J9SharedCache::romclassMatchesCachedVersion(J9ROMClass *romClass, UDATA *&chainPtr, UDATA *chainEnd)
{
    J9UTF8 *className = J9ROMCLASS_CLASSNAME(romClass);
...
UDATA *TR_J9SharedCache::findChainForClass(J9Class *clazz, const char *key, uint32_t keyLength)
{
    ...
    J9VMThread *vmThread = fej9->getCurrentVMThread();
    sharedCacheConfig()->findSharedData(vmThread, key, keyLength, J9SHR_DATA_TYPE_AOTCLASSCHAIN, FALSE, &dataDescriptor,
        NULL);
...
bool TR_J9SharedCache::validateInterfacesInClassChain(TR_OpaqueClassBlock *clazz, UDATA *&chainPtr, UDATA *chainEnd)
{
...
bool TR_J9SharedCache::classMatchesCachedVersion(J9Class *clazz, UDATA *chainData)
{
    J9ROMClass *romClass = TR::Compiler->cls.romClassOf(fe()->convertClassPtrToClassOffset(clazz));
    J9UTF8 *className = J9ROMCLASS_CLASSNAME(romClass);
...
TR_OpaqueClassBlock *TR_J9SharedCache::lookupClassFromChainAndLoader(uintptr_t *chainData, void *classLoader,
    TR::Compilation *comp)
{
...
uintptr_t TR_J9SharedCache::getClassChainOffsetIdentifyingLoader(TR_OpaqueClassBlock *clazz, uintptr_t **classChain)
{
    void *loaderForClazz = _fe->getClassLoader(clazz);
    void *classChainIdentifyingLoaderForClazz
        = persistentClassLoaderTable()->lookupClassChainAssociatedWithClassLoader(loaderForClazz);
    ...
    if (comp) {
        ...
        if (!isPointerInSharedCache(classChainIdentifyingLoaderForClazz, &classChainOffsetInSharedCache))
            comp->failCompilation<J9::ClassChainPersistenceFailure>("Failed to find pointer %p in SCC",
...
#if defined(J9VM_OPT_JITSERVER)
uintptr_t TR_J9SharedCache::getClassChainOffsetIdentifyingLoaderNoFail(TR_OpaqueClassBlock *clazz,
...
uintptr_t TR_J9SharedCache::getClassChainOffsetIdentifyingLoaderNoThrow(TR_OpaqueClassBlock *clazz)
{
    void *loaderForClazz = _fe->getClassLoader(clazz);
    void *classChainIdentifyingLoaderForClazz
        = persistentClassLoaderTable()->lookupClassChainAssociatedWithClassLoader(loaderForClazz);
...
const void *TR_J9SharedCache::storeSharedData(J9VMThread *vmThread, const char *key,
    const J9SharedDataDescriptor *descriptor)
{
#if defined(J9VM_OPT_SHARED_CLASSES)                                                                    \
    && (defined(TR_HOST_X86) || defined(TR_HOST_POWER) || defined(TR_HOST_S390) || defined(TR_HOST_ARM) \
        || defined(TR_HOST_ARM64))
    return _sharedCacheConfig->storeSharedData(vmThread, key, strlen(key), descriptor);
#else
    return NULL;
#endif
}

void TR_J9SharedCache::buildWellKnownClassesSCCKey(char *buffer, size_t size, unsigned int includedClasses)
{
    TR::snprintfNoTrunc(buffer, size, "AOTWellKnownClasses:%x", includedClasses);
}

const void *TR_J9SharedCache::storeWellKnownClasses(J9VMThread *vmThread, uintptr_t *classChainOffsets,
    size_t classChainOffsetsSize, unsigned int includedClasses)
{
    char key[128];
...
void TR_J9SharedCache::buildAOTMethodDependenciesKey(uintptr_t offset, char *buffer, size_t &keyLength)
{
    auto cursor = buffer;
...
const void *TR_J9SharedCache::storeAOTMethodDependencies(J9VMThread *vmThread, TR_OpaqueMethodBlock *method,
    TR_OpaqueClassBlock *definingClass, uintptr_t *methodDependencies, size_t methodDependenciesSize)
{
...
bool TR_J9SharedCache::methodHasAOTBodyWithDependencies(J9VMThread *vmThread, J9ROMMethod *method,
    const uintptr_t *&methodDependencies)
{
    ...
    auto aotMethodHeader
        = (TR_AOTMethodHeader *)(dataCacheHeader + 1); // skip the data cache header to get to the AOT method header
    ...
    sharedCacheConfig()->findSharedData(vmThread, key, keyLength, J9SHR_DATA_TYPE_JITHINT, FALSE, &dataDescriptor,
        NULL);
...
#else
    return false;
#endif /*  defined(J9VM_OPT_SHARED_CLASSES) && (defined(TR_HOST_X86) || defined(TR_HOST_POWER) || \
          defined(TR_HOST_S390) || defined(TR_HOST_ARM) || defined(TR_HOST_ARM64)) */
}
...
uintptr_t TR_J9JITServerSharedCache::rememberClass(J9Class *clazz, const AOTCacheClassChainRecord **classChainRecord,
    bool create)
{
    TR::Compilation *comp = _compInfoPT->getCompilation();
    TR_ASSERT_FATAL(classChainRecord || !create || !comp->isAOTCacheStore(),
        "Must pass classChainRecord if creating class chain at JITServer");
    ...
    _stream->write(JITServer::MessageType::SharedCache_rememberClass, clazz, create, !useServerOffsets,
        needClassChainRecord);
    auto recv = _stream->read<uintptr_t, std::vector<J9Class *>, std::vector<J9Class *>,
        std::vector<JITServerHelpers::ClassInfoTuple> >();
    if (!useServerOffsets && (TR_SharedCache::INVALID_CLASS_CHAIN_OFFSET != clientClassChainOffset))
        TR_ASSERT_FATAL(std::get<0>(recv) == clientClassChainOffset,
            "Received mismatching class chain offset: %" OMR_PRIuPTR " != %" OMR_PRIuPTR, std::get<0>(recv),
            clientClassChainOffset);
    ...
    if (create) {
        if (needClassChainRecord) {
            ...
            record = clientData->getClassChainRecord(clazz, clientClassChainOffset, ramClassChain, _stream,
                missingLoaderInfo);
            if (classChainRecord)
                *classChainRecord = record;
            ...
            OMR::CriticalSection classChainDataMapMonitor(clientData->getClassChainDataMapMonitor());
            cache.insert({
                clazz, { clientClassChainOffset, NULL }
            });
        }
    }
...
J9SharedClassCacheDescriptor *TR_J9JITServerSharedCache::getCacheDescriptorList()
{
    TR_ASSERT(_stream, "stream must be initialized by now");
    TR::Compilation *comp = _compInfoPT->getCompilation();
    ClientSessionData *clientData = comp->getClientData();
...
bool TR_J9JITServerSharedCache::isClassInSharedCache(TR_OpaqueClassBlock *clazz, uintptr_t *cacheOffset)
{
    TR_ASSERT(_stream, "stream must be initialized by now");
    TR::Compilation *comp = _compInfoPT->getCompilation();
    ClientSessionData *clientData = comp->getClientData();
...
bool TR_J9JITServerSharedCache::isMethodInSharedCache(TR_OpaqueMethodBlock *method, TR_OpaqueClassBlock *definingClass,
    uintptr_t *cacheOffset)
{
    TR::Compilation *comp = _compInfoPT->getCompilation();
    ClientSessionData *clientData = comp->getClientData();
    ...
    if (useServerOffsets) {
        auto record = clientData->getMethodRecord(reinterpret_cast<J9Method *>(method),
            reinterpret_cast<J9Class *>(definingClass), _stream);
        if (record) {
            if (cacheOffset)
...
uintptr_t TR_J9JITServerSharedCache::getClassChainOffsetIdentifyingLoader(TR_OpaqueClassBlock *clazz,
    uintptr_t **classChain)
{
    ...
    if (useServerOffsets) {
        ...
        auto classRecord = clientData->getClassRecord(reinterpret_cast<J9Class *>(clazz), _stream, missingLoaderInfo);
        if (classRecord)
            return AOTSerializationRecord::idAndType(classRecord->data().classLoaderId(),
                AOTSerializationRecordType::ClassLoader);
        return TR_SharedCache::INVALID_CLASS_CHAIN_OFFSET;
    }
    ...
    JITServerHelpers::getAndCacheRAMClassInfo((J9Class *)clazz, clientData, _stream,
        JITServerHelpers::CLASSINFO_CLASS_CHAIN_OFFSET_IDENTIFYING_LOADER, (void *)&classChainOffset);
...
void TR_J9JITServerSharedCache::addHint(J9Method *method, TR_SharedCacheHint theHint)
{
    TR_ASSERT(_stream, "stream must be initialized by now");
...
const void *TR_J9JITServerSharedCache::storeSharedData(J9VMThread *vmThread, const char *key,
    const J9SharedDataDescriptor *descriptor)
{
    TR_ASSERT(_stream, "stream must be initialized by now");
    TR::Compilation *comp = _compInfoPT->getCompilation();
    ...
    _stream->write(JITServer::MessageType::SharedCache_storeSharedData, std::string(key, strlen(key)), *descriptor,
        dataStr);
...
J9ROMClass *TR_J9DeserializerSharedCache::romClassFromOffsetInSharedCache(uintptr_t offset)
{
    TR::Compilation *comp = _compInfoPT->getCompilation();
    bool wasReset = false;
    auto context = DeserializerContext(comp);
    ...
    if (wasReset)
        comp->failCompilation<J9::AOTDeserializerReset>("Deserializer reset during relocation of method %s",
            comp->signature());
    TR_ASSERT_FATAL(romClass, "ROM class for offset %zu could not be found", offset,
        JITServerNoSCCAOTDeserializer::offsetId(offset), JITServerNoSCCAOTDeserializer::offsetType(offset));
    return romClass;
}

void *TR_J9DeserializerSharedCache::pointerFromOffsetInSharedCache(uintptr_t offset)
{
    TR::Compilation *comp = _compInfoPT->getCompilation();
    bool wasReset = false;
    auto context = DeserializerContext(comp);
    ...
    if (wasReset)
        comp->failCompilation<J9::AOTDeserializerReset>("Deserializer reset during relocation of method %s",
            comp->signature());
    TR_ASSERT_FATAL(ptr, "Pointer for offset %zu ID %zu type %u could not be found", offset,
        JITServerNoSCCAOTDeserializer::offsetId(offset), JITServerNoSCCAOTDeserializer::offsetType(offset));
    return ptr;
}

void *TR_J9DeserializerSharedCache::lookupClassLoaderAssociatedWithClassChain(void *chainData)
{
...
bool TR_J9DeserializerSharedCache::classMatchesCachedVersion(J9Class *clazz, UDATA *chainData)
{
    ...
    if (wasReset)
        comp->failCompilation<J9::AOTDeserializerReset>("Deserializer reset during relocation of method %s",
            comp->signature());
    TR_ASSERT_FATAL(ramClass, "RAM class for offset %zu ID %zu type %zu could not be found", chainData[1],
        JITServerNoSCCAOTDeserializer::offsetId(chainData[1]), JITServerNoSCCAOTDeserializer::offsetType(chainData[1]));
    return ramClass == clazz;
}

TR_OpaqueClassBlock *TR_J9DeserializerSharedCache::lookupClassFromChainAndLoader(uintptr_t *chainData,
    void *classLoader, TR::Compilation *comp)
{
    // The base TR_J9SharedCache::lookupClassFromChainAndLoader(), when given a class chain and class loader, will look
    // in the loader a J9Class correspoding to the first class in the chain. If one could be found, it then verifies
    // that the class matches the cached version. We do not need to perform that checking here, because during
    // deserialization we will have already resolved the first class in the chain to a J9Class and verified that it
    // matches. Thus we can simply return that cached first J9Class.
    bool wasReset = false;
    auto context = DeserializerContext(comp);
    ...
    if (wasReset)
        comp->failCompilation<J9::AOTDeserializerReset>("Deserializer reset during relocation of method %s",
            comp->signature());
    TR_ASSERT_FATAL(clazz, "Class for offset %zu could not be found", chainData[1],
        JITServerNoSCCAOTDeserializer::offsetId(chainData[1]), JITServerNoSCCAOTDeserializer::offsetType(chainData[1]));
    return reinterpret_cast<TR_OpaqueClassBlock *>(clazz);
}

J9ROMMethod *TR_J9DeserializerSharedCache::romMethodFromOffsetInSharedCache(uintptr_t offset)
{
    TR::Compilation *comp = _compInfoPT->getCompilation();
    bool wasReset = false;
    auto context = DeserializerContext(comp);
    ...
    if (wasReset)
        comp->failCompilation<J9::AOTDeserializerReset>("Deserializer reset during relocation of method %s",
            comp->signature());
    TR_ASSERT_FATAL(romMethod, "ROM method for offset %zu ID %zu type %zu could not be found", offset,
        JITServerNoSCCAOTDeserializer::offsetId(offset), JITServerNoSCCAOTDeserializer::offsetType(offset));
    return romMethod;
}
