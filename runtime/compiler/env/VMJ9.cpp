...
int32_t TR_J9VMBase::getCompThreadIDForVMThread(void *vmThread)
{
...
bool TR_J9VMBase::instanceOfOrCheckCast(J9Class *instanceClass, J9Class *castClass)
{
    return ::instanceOfOrCheckCast(instanceClass, castClass);
}

bool TR_J9VMBase::instanceOfOrCheckCastNoCacheUpdate(J9Class *instanceClass, J9Class *castClass)
{
...
bool acquireVMaccessIfNeeded(J9VMThread *vmThread, TR_YesNoMaybe isCompThread)
{
    ...
    TR::CompilationInfoPerThread *compInfoPT = isCompThread != TR_no ? compInfo->getCompInfoForThread(vmThread) : NULL;
    ...
    if (!(vmThread->publicFlags & J9_PUBLIC_FLAGS_VM_ACCESS)) {
        ...
        } else // the GC is having exclusive VM access
        {
...
#endif
                hadClassUnloadMonitor
                    = TR::MonitorTable::get()->readReleaseClassUnloadMonitor(compInfoPT->getCompThreadId()) >= 0;
#if defined(J9VM_GC_DYNAMIC_CLASS_UNLOADING)
            // We must have had classUnloadMonitor by the way we architected the application
            TR_ASSERT(hadClassUnloadMonitor,
                "Comp thread must hold classUnloadMonitor when compiling without VMaccess");
#endif
            ...
            TR_ASSERT(!heldMonitor,
                "Current thread must not hold TR::Monitor %p called %s when trying to acquire VM access\n", heldMonitor,
                TR_J9VMBase::get(jitConfig, NULL)->getJ9MonitorName((J9ThreadMonitor *)heldMonitor->getVMMonitor()));
#endif // #if defined(DEBUG) || defined(PROD_WITH_ASSUMES)

            TR_ASSERT_FATAL(!compInfo->getCompilationMonitor()->owned_by_self(),
                "Current VM thread [%p] holds the comp monitor [%p] while attempting to acquire VM access", vmThread,
                compInfo->getCompilationMonitor());

            TR::Compilation *comp = compInfoPT->getCompilation();
            if ((comp && comp->getOptions()->realTimeGC()) || TR::Options::getCmdLineOptions()->realTimeGC())
     ...
     */
    if (compInfoPT->compilationShouldBeInterrupted()) {
        TR_ASSERT(compInfoPT->compilationShouldBeInterrupted() != GC_COMP_INTERRUPT,
            "GC should not have cut in _compInfoPT=%p\n", compInfoPT);
...
void releaseVMaccessIfNeeded(J9VMThread *vmThread, bool haveAcquiredVMAccess)
{
    if (haveAcquiredVMAccess) {
        TR_ASSERT((vmThread->publicFlags & J9_PUBLIC_FLAGS_VM_ACCESS),
            "Must have VM access"); // I don't already have VM access
        releaseVMAccess(vmThread);
    }
}

bool TR_J9VMBase::isAnyMethodTracingEnabled(TR_OpaqueMethodBlock *method) { return isMethodTracingEnabled(method); }

int32_t *TR_J9VMBase::staticStringEnableCompressionFieldAddr = NULL;

bool TR_J9VMBase::releaseClassUnloadMonitorAndAcquireVMaccessIfNeeded(TR::Compilation *comp,
    bool *hadClassUnloadMonitor)
{
    ...
    *hadClassUnloadMonitor = false;
    ...
    if (!TR::Options::getCmdLineOptions()->getOption(TR_DisableNoVMAccess)) {
        TR_ASSERT(_vmThreadIsCompilationThread != TR_no,
            "releaseClassUnloadMonitorAndAcquireVMaccessIfNeeded should only be called for compilation threads\n");
...
#if defined(J9VM_GC_DYNAMIC_CLASS_UNLOADING)
        *hadClassUnloadMonitor
            = TR::MonitorTable::get()->readReleaseClassUnloadMonitor(_compInfoPT->getCompThreadId()) >= 0;
        TR_ASSERT(*hadClassUnloadMonitor,
            "releaseClassUnloadMonitorAndAcquireVMaccessIfNeeded without classUnloadMonitor\n");
#else
        ...
        if (!(vmThread()->publicFlags & J9_PUBLIC_FLAGS_VM_ACCESS)) // I don't already have VM access
        {
            ...
            TR_ASSERT(!heldMonitor,
                "Current thread must not hold TR::Monitor %p called %s when trying to acquire VM access\n", heldMonitor,
                getJ9MonitorName((J9ThreadMonitor *)heldMonitor->getVMMonitor()));
#endif // #if defined(DEBUG) || defined(PROD_WITH_ASSUMES)
...
void TR_J9VMBase::acquireClassUnloadMonitorAndReleaseVMAccessIfNeeded(TR::Compilation *comp, bool hadVMAccess,
    bool hadClassUnloadMonitor)
{
...
bool TR_J9SharedCacheVM::shouldDelayAotLoad() { return isAOT_DEPRECATED_DO_NOT_USE(); }

bool TR_J9SharedCacheVM::isResolvedDirectDispatchGuaranteed(TR::Compilation *comp)
{
    return isAotResolvedDirectDispatchGuaranteed(comp);
}

bool TR_J9VMBase::isAotResolvedDirectDispatchGuaranteed(TR::Compilation *comp)
{
    return comp->getOption(TR_UseSymbolValidationManager) && comp->cg()->guaranteesResolvedDirectDispatchForSVM();
}

bool TR_J9SharedCacheVM::isResolvedVirtualDispatchGuaranteed(TR::Compilation *comp)
{
    return isAotResolvedVirtualDispatchGuaranteed(comp);
}

bool TR_J9VMBase::isAotResolvedVirtualDispatchGuaranteed(TR::Compilation *comp)
{
    return comp->getOption(TR_UseSymbolValidationManager) && comp->cg()->guaranteesResolvedVirtualDispatchForSVM();
}

J9Class *TR_J9VMBase::matchRAMclassFromROMclass(J9ROMClass *clazz, TR::Compilation *comp)
{
    ...
    J9Class *ramClass
        = jitGetClassInClassloaderFromUTF8(vmThread(), classLoader, (char *)J9UTF8_DATA(utf8), J9UTF8_LENGTH(utf8));
    if (!ramClass) {
...
J9VMThread *TR_J9VMBase::getCurrentVMThread()
{
    return _vmThread ? _vmThread : _jitConfig->javaVM->internalVMFunctions->currentVMThread(_jitConfig->javaVM);
}

bool TR_J9VMBase::createGlobalFrontEnd(J9JITConfig *jitConfig, TR::CompilationInfo *compInfo)
{
...
TR_J9VMBase *TR_J9VMBase::get(J9JITConfig *jitConfig, J9VMThread *vmThread, VM_TYPE vmType)
{
...
#error "Unsupported AOT configuration"
    ...
    if (vmThread) {
        ...
        if (vmType == J9_SERVER_VM || vmType == J9_SHARED_CACHE_SERVER_VM) {
            TR_ASSERT(vmWithoutThreadInfo->_compInfo->getPersistentInfo()->getRemoteCompilationMode()
                    == JITServer::SERVER,
                "J9_SERVER_VM and J9_SHARED_CACHE_SERVER_VM should only be instantiated in JITServer::SERVER mode");
            ...
            if (vmType == J9_SHARED_CACHE_SERVER_VM) {
                ...
                if (!sharedCacheServerVM) {
                    ...
                    if (alloc)
                        sharedCacheServerVM
                            = new (alloc) TR_J9SharedCacheServerVM(jitConfig, vmWithoutThreadInfo->_compInfo, vmThread);
                    if (sharedCacheServerVM) {
            ...
            {
#if !defined(J9VM_INTERP_AOT_COMPILE_SUPPORT)
                TR_ASSERT((vmType != J9_SHARED_CACHE_SERVER_VM),
                    "vmType cannot be J9_SHARED_CACHE_SERVER_VM when J9VM_INTERP_AOT_COMPILE_SUPPORT is disabled");
#endif /* !defined(J9VM_INTERP_AOT_COMPILE_SUPPORT) */
        ...
        if (vmType == AOT_VM) {
            ...
            if (!aotVMWithThreadInfo) {
                ...
                if (alloc) {
                    aotVMWithThreadInfo
                        = new (alloc) TR_J9SharedCacheVM(jitConfig, vmWithoutThreadInfo->_compInfo, vmThread);
                }
                if (aotVMWithThreadInfo) {
                    vmThread->aotVMwithThreadInfo = aotVMWithThreadInfo;
                    if (vmWithoutThreadInfo->_compInfo) {
                        TR::CompilationInfoPerThread *compInfoPT
                            = vmWithoutThreadInfo->_compInfo->getCompInfoForThread(vmThread);
...
#endif /* defined(J9VM_INTERP_AOT_COMPILE_SUPPORT) */
        {
            ...
            if (!vmWithThreadInfo) // This thread has not cached a frontend
            {
                ...
                if (alloc) {
                    vmWithThreadInfo = new (alloc)
                        TR_J9VM(jitConfig, vmWithoutThreadInfo->_compInfo, vmThread); // allocate the frontend
                }
                if (vmWithThreadInfo) {
                    ...
                    if (vmWithoutThreadInfo->_compInfo) {
                        ...
                        if (compInfoPT) {
                            ...
                            if (deserializer
                                && vmWithoutThreadInfo->_compInfo->getPersistentInfo()
                                       ->getJITServerAOTCacheIgnoreLocalSCC()) {
                                vmWithThreadInfo->_deserializerSharedCache
                                    = new (vmWithoutThreadInfo->_compInfo->persistentMemory())
                                        TR_J9DeserializerSharedCache(vmWithThreadInfo,
                                            (JITServerNoSCCAOTDeserializer *)deserializer, compInfoPT);
                            }
#endif /* defined(J9VM_OPT_JITSERVER) */
                        } else {
...
#endif /* defined(J9VM_OPT_JITSERVER) */
{
    ...
    if (TR::Options::sharedClassCache()
        ...
        || (vmThread && jitConfig->javaVM->sharedClassConfig
    ...
    // shared classes and AOT must be enabled, or we should be on the JITServer with remote AOT enabled
    {
        ...
        if (!_sharedCache) {
            ...
            TR_PersistentClassLoaderTable *loaderTable
                = persistentMemory->getPersistentInfo()->getPersistentClassLoaderTable();
            _sharedCache->setPersistentClassLoaderTable(loaderTable);
        }
    }
}

void TR_J9VMBase::freeSharedCache()
{
...
TR::CompilationInfo *getCompilationInfo(J9JITConfig *jitConfig) { return TR::CompilationInfo::get(jitConfig); }

J9VMThread *TR_J9VMBase::vmThread()
{
    TR_ASSERT(_vmThread, "TR_J9VMBase::vmThread() TR_J9VMBase was created without thread information");
    return _vmThread;
}

void TR_J9VMBase::acquireCompilationLock()
{
    if (_compInfo)
        _compInfo->acquireCompilationLock();
}

void TR_J9VMBase::releaseCompilationLock()
{
    if (_compInfo)
        _compInfo->releaseCompilationLock();
}

void TR_J9VMBase::acquireLogMonitor()
{
    if (_compInfo)
        _compInfo->acquireLogMonitor();
}

void TR_J9VMBase::releaseLogMonitor()
{
    if (_compInfo)
        _compInfo->releaseLogMonitor();
}

bool TR_J9VMBase::isAsyncCompilation() { return _compInfo ? _compInfo->asynchronousCompilation() : false; }

uintptr_t TR_J9VMBase::getProcessID()
{
...
void TR_J9VM::initializeHasResumableTrapHandler()
{
...
bool TR_J9VMBase::hasResumableTrapHandler() { return _compInfo ? _compInfo->getHasResumableTrapHandler() : false; }

void TR_J9VM::initializeHasFixedFrameC_CallingConvention()
{
    TR_ASSERT(_compInfo, "compInfo not defined");
...
bool TR_J9VMBase::hasFixedFrameC_CallingConvention()
{
    return _compInfo ? _compInfo->getHasFixedFrameC_CallingConvention() : false;
}

bool TR_J9VMBase::pushesOutgoingArgsAtCallSite(TR::Compilation *comp)
{
...
bool TR_J9VMBase::assumeLeftMostNibbleIsZero() { return false; }
...
#else
    notImplemented("thisThreadGetSystemSPOffset");
    return 0;
#endif
}
...
UDATA TR_J9VMBase::getThreadDebugEventDataOffset(int32_t index)
{
    J9VMThread *dummy = 0;
    return offsetof(J9VMThread, debugEventData1) + (index - 1) * sizeof(dummy->debugEventData1);
} // index counts from 1
...
UDATA TR_J9VMBase::getOffsetOfDiscontiguousArraySizeField()
{
    return TR::Compiler->om.offsetOfDiscontiguousArraySizeField();
}
#if defined(TR_TARGET_64BIT)
UDATA TR_J9VMBase::getOffsetOfContiguousDataAddrField() { return TR::Compiler->om.offsetOfContiguousDataAddrField(); }

UDATA TR_J9VMBase::getOffsetOfDiscontiguousDataAddrField()
{
    return TR::Compiler->om.offsetOfDiscontiguousDataAddrField();
}
...
bool TR_J9VMBase::ensureOSRBufferSize(TR::Compilation *comp, uintptr_t osrFrameSizeInBytes,
...
bool TR_J9VMBase::compiledAsDLTBefore(TR_ResolvedMethod *method)
{
#if defined(J9VM_JIT_DYNAMIC_LOOP_TRANSFER)
...
TR_OpaqueClassBlock *TR_J9VMBase::getObjectClass(uintptr_t objectPointer)
{
...
TR_OpaqueClassBlock *TR_J9VMBase::getObjectClassAt(uintptr_t objectAddress)
{
...
TR_OpaqueClassBlock *TR_J9VMBase::getObjectClassFromKnownObjectIndex(TR::Compilation *comp,
    TR::KnownObjectTable::Index idx, bool *isJavaLangClass)
{
...
uintptr_t TR_J9VMBase::getStaticReferenceFieldAtAddress(uintptr_t fieldAddress)
{
...
TR::KnownObjectTable::ObjectInfo TR_J9VMBase::getObjClassInfoFromKnotIndex(TR::Compilation *comp,
    TR::KnownObjectTable::Index knotIndex)
{
    TR::KnownObjectTable *knot = comp->getKnownObjectTable();
    ...
    } else // Caching is enabled
    {
        ...
        } else // Reply with information from the cache
        {
            if (comp->getOption(TR_EnableKnownObjectTableCachingVerification)) {
                TR::KnownObjectTable::ObjectInfo retrievedObjInfo
                    = getObjClassInfoFromKnotIndexNoCaching(comp, knotIndex);
                ...
                TR_ASSERT_FATAL(existingObjInfo._isFixedJavaLangClass == retrievedObjInfo._isFixedJavaLangClass,
                    "_isFixedJavaLangClass discrepancy");
                TR_ASSERT_FATAL(existingObjInfo._jniReference == retrievedObjInfo._jniReference,
                    "_jniReference discrepancy");
            }
...
uintptr_t TR_J9VMBase::getReferenceFieldAtAddress(uintptr_t fieldAddress)
{
...
uintptr_t TR_J9VMBase::getReferenceFieldAt(uintptr_t objectPointer, uintptr_t fieldOffset)
{
    TR_ASSERT(haveAccess(), "Must haveAccess in getReferenceFieldAt");
    return (uintptr_t)J9OBJECT_OBJECT_LOAD(vmThread(), objectPointer,
        TR::Compiler->om.objectHeaderSizeInBytes() + fieldOffset);
}

uintptr_t TR_J9VMBase::getVolatileReferenceFieldAt(uintptr_t objectPointer, uintptr_t fieldOffset)
{
    TR_ASSERT(haveAccess(), "Must haveAccess in getVolatileReferenceFieldAt");
    return (uintptr_t)vmThread()->javaVM->javaVM->memoryManagerFunctions->j9gc_objaccess_mixedObjectReadObject(
        vmThread(), (J9Object *)objectPointer, TR::Compiler->om.objectHeaderSizeInBytes() + fieldOffset, IS_VOLATILE);
}

int32_t TR_J9VMBase::getInt32FieldAt(uintptr_t objectPointer, uintptr_t fieldOffset)
{
    TR_ASSERT(haveAccess(), "Must haveAccess in getInt32Field");
    return *(int32_t *)(objectPointer + getObjectHeaderSizeInBytes() + fieldOffset);
}

int64_t TR_J9VMBase::getInt64FieldAt(uintptr_t objectPointer, uintptr_t fieldOffset)
{
    TR_ASSERT(haveAccess(), "Must haveAccess in getInt64Field");
    return *(int64_t *)(objectPointer + getObjectHeaderSizeInBytes() + fieldOffset);
}

void TR_J9VMBase::setInt64FieldAt(uintptr_t objectPointer, uintptr_t fieldOffset, int64_t newValue)
{
...
bool TR_J9VMBase::compareAndSwapInt64FieldAt(uintptr_t objectPointer, uintptr_t fieldOffset, int64_t oldValue,
    int64_t newValue)
{
    TR_ASSERT(haveAccess(), "Must haveAccess in compareAndSwapInt64FieldAt");
    UDATA success = vmThread()->javaVM->javaVM->memoryManagerFunctions->j9gc_objaccess_mixedObjectCompareAndSwapLong(
        vmThread(), (J9Object *)objectPointer, fieldOffset + getObjectHeaderSizeInBytes(), oldValue, newValue);
    return success != 0;
}

intptr_t TR_J9VMBase::getArrayLengthInElements(uintptr_t objectPointer)
{
...
int32_t TR_J9VMBase::getInt32Element(uintptr_t objectPointer, int32_t elementIndex)
{
    TR_ASSERT(haveAccess(), "getInt32Element requires VM access");
    return J9JAVAARRAYOFINT_LOAD(vmThread(), objectPointer, elementIndex);
}

uintptr_t TR_J9VMBase::getReferenceElement(uintptr_t objectPointer, intptr_t elementIndex)
{
...
TR::DataType TR_J9VMBase::getClassPrimitiveDataType(TR_OpaqueClassBlock *clazz)
{
...
TR_OpaqueClassBlock *TR_J9VMBase::getArrayClassFromDataType(TR::DataType type, bool booleanClass)
{
    J9Class *j9class;
    J9JavaVM *vm = getJ9JITConfig()->javaVM;
...
TR_OpaqueClassBlock *TR_J9VMBase::getClassFromJavaLangClass(uintptr_t objectPointer)
{
    return (TR_OpaqueClassBlock *)J9VM_J9CLASS_FROM_HEAPCLASS(_vmThread, objectPointer);
}

uintptr_t TR_J9VMBase::getConstantPoolFromMethod(TR_OpaqueMethodBlock *method)
{
    return (uintptr_t)J9_CP_FROM_METHOD((J9Method *)method);
}

uintptr_t TR_J9VMBase::getConstantPoolFromClass(TR_OpaqueClassBlock *clazz)
{
...
void TR_J9VMBase::printVerboseLogHeader(TR::Options *cmdLineOptions)
{
    ...
    TR_VerboseLog::writeLine(TR_Vlog_INFO, "Processor Information:");
    TR_VerboseLog::writeLine(TR_Vlog_INFO, "     Platform Info:%s", processorName);
    TR_VerboseLog::writeLine(TR_Vlog_INFO, "     Supports HardwareSQRT:%d",
        TR::Compiler->target.cpu.isAtLeast(OMR_PROCESSOR_PPC_HW_SQRT_FIRST));
    TR_VerboseLog::writeLine(TR_Vlog_INFO, "     Supports HardwareRound:%d",
        TR::Compiler->target.cpu.isAtLeast(OMR_PROCESSOR_PPC_HW_ROUND_FIRST));
    TR_VerboseLog::writeLine(TR_Vlog_INFO, "     Supports HardwareCopySign:%d",
        TR::Compiler->target.cpu.isAtLeast(OMR_PROCESSOR_PPC_HW_COPY_SIGN_FIRST));
    TR_VerboseLog::writeLine(TR_Vlog_INFO, "     Supports FPU:%d", TR::Compiler->target.cpu.hasFPU());
    TR_VerboseLog::writeLine(TR_Vlog_INFO, "     Supports DFP:%d",
        TR::Compiler->target.cpu.supportsDecimalFloatingPoint());
    TR_VerboseLog::writeLine(TR_Vlog_INFO, "     Supports VMX:%d",
        TR::Compiler->target.cpu.supportsFeature(OMR_FEATURE_PPC_HAS_ALTIVEC));
    TR_VerboseLog::writeLine(TR_Vlog_INFO, "     Supports VSX:%d",
        TR::Compiler->target.cpu.supportsFeature(OMR_FEATURE_PPC_HAS_VSX));
    TR_VerboseLog::writeLine(TR_Vlog_INFO, "     Supports AES:%d", TR::Compiler->target.cpu.getPPCSupportsAES());
    TR_VerboseLog::writeLine(TR_Vlog_INFO, "     Supports  TM:%d",
        TR::Compiler->target.cpu.supportsFeature(OMR_FEATURE_PPC_HTM));
    TR_VerboseLog::writeLine(TR_Vlog_INFO, "     Vendor:%s", vendorId);
    TR_VerboseLog::writeLine(TR_Vlog_INFO, "     numProc=%u", TR::Compiler->target.numberOfProcessors());
    TR_VerboseLog::writeLine(TR_Vlog_INFO, "");
#endif
    ...
    TR_VerboseLog::writeLine(TR_Vlog_INFO, "      Vendor: %s", vendorId);
    TR_VerboseLog::writeLine(TR_Vlog_INFO, "     numProc: %u", TR::Compiler->target.numberOfProcessors());
    TR_VerboseLog::writeLine(TR_Vlog_INFO, "         DFP: %d",
        TR::Compiler->target.cpu.supportsFeature(OMR_FEATURE_S390_DFP));
    TR_VerboseLog::writeLine(TR_Vlog_INFO, "         FPE: %d",
        TR::Compiler->target.cpu.supportsFeature(OMR_FEATURE_S390_FPE));
    TR_VerboseLog::writeLine(TR_Vlog_INFO, "         HPR: %d",
        TR::Compiler->target.cpu.supportsFeature(OMR_FEATURE_S390_HIGH_WORD));
    TR_VerboseLog::writeLine(TR_Vlog_INFO, "          RI: %d",
        TR::Compiler->target.cpu.supportsFeature(OMR_FEATURE_S390_RI));
    TR_VerboseLog::writeLine(TR_Vlog_INFO, "          TX: %d",
        TR::Compiler->target.cpu.supportsFeature(OMR_FEATURE_S390_TRANSACTIONAL_EXECUTION_FACILITY));
    TR_VerboseLog::writeLine(TR_Vlog_INFO, "         TXC: %d",
        TR::Compiler->target.cpu.supportsFeature(OMR_FEATURE_S390_CONSTRAINED_TRANSACTIONAL_EXECUTION_FACILITY));
    TR_VerboseLog::writeLine(TR_Vlog_INFO, "          VF: %d",
        TR::Compiler->target.cpu.supportsFeature(OMR_FEATURE_S390_VECTOR_FACILITY));
...
UDATA TR_J9VMBase::getLowTenureAddress() { return (UDATA)_vmThread->lowTenureAddress; }

UDATA TR_J9VMBase::getHighTenureAddress() { return (UDATA)_vmThread->highTenureAddress; }

bool TR_J9VMBase::generateCompressedPointers() { return TR::Compiler->om.compressObjectReferences(); }

bool TR_J9VMBase::generateCompressedLockWord() { return TR::Compiler->om.compressObjectReferences(); }
...
int32_t *TR_J9VMBase::getCurrentLocalsMapForDLT(TR::Compilation *comp)
{
    ...
    jitConfig->javaVM->localMapFunction(_portLibrary, J9_CLASS_FROM_METHOD(j9method)->romClass,
        getOriginalROMMethod(j9method), comp->getDltBcIndex(), (U_32 *)currentBundles, NULL, NULL, NULL);
#endif

    return currentBundles;
}

UDATA TR_J9VMBase::getOffsetOfInitializeStatusFromClassField() { return offsetof(J9Class, initializeStatus); }

UDATA TR_J9VMBase::getOffsetOfJavaLangClassFromClassField() { return offsetof(J9Class, classObject); }
...
UDATA TR_J9VMBase::getOffsetOfInterfaceClassFromITableField() { return offsetof(J9ITable, interfaceClass); }

int32_t TR_J9VMBase::getITableEntryJitVTableOffset() { return sizeof(J9Class); }
...
UDATA TR_J9VMBase::getOffsetOfInstanceShapeFromClassField() { return offsetof(J9Class, totalInstanceSize); }

UDATA TR_J9VMBase::getOffsetOfInstanceDescriptionFromClassField() { return offsetof(J9Class, instanceDescription); }

UDATA TR_J9VMBase::getOffsetOfDescriptionWordFromPtrField() { return 0; }
...
int32_t TR_J9VMBase::getFlagValueForFinalizerCheck()
{
    return J9AccClassFinalizeNeeded | J9AccClassOwnableSynchronizer | J9AccClassContinuation;
}

UDATA TR_J9VMBase::getGCForwardingPointerOffset() { return 0; }
...
{
    return (((TR_ResolvedJ9Method *)method)->getJNIProperties() & J9_FAST_JNI_RETAIN_VM_ACCESS) != 0;
}

bool TR_J9VMBase::jniNoGCPoint(TR_ResolvedMethod *method)
{
    return (((TR_ResolvedJ9Method *)method)->getJNIProperties() & J9_FAST_JNI_NOT_GC_POINT) != 0;
}

bool TR_J9VMBase::jniNoNativeMethodFrame(TR_ResolvedMethod *method)
{
    return (((TR_ResolvedJ9Method *)method)->getJNIProperties() & J9_FAST_NO_NATIVE_METHOD_FRAME) != 0;
}

bool TR_J9VMBase::jniNoExceptionsThrown(TR_ResolvedMethod *method)
{
    return (((TR_ResolvedJ9Method *)method)->getJNIProperties() & J9_FAST_JNI_NO_EXCEPTION_THROW) != 0;
}

bool TR_J9VMBase::jniNoSpecialTeardown(TR_ResolvedMethod *method)
{
    return (((TR_ResolvedJ9Method *)method)->getJNIProperties() & J9_FAST_JNI_NO_SPECIAL_TEAR_DOWN) != 0;
}

bool TR_J9VMBase::jniDoNotWrapObjects(TR_ResolvedMethod *method)
{
    return (((TR_ResolvedJ9Method *)method)->getJNIProperties() & J9_FAST_JNI_DO_NOT_WRAP_OBJECTS) != 0;
}

bool TR_J9VMBase::jniDoNotPassReceiver(TR_ResolvedMethod *method)
{
    return (((TR_ResolvedJ9Method *)method)->getJNIProperties() & J9_FAST_JNI_DO_NOT_PASS_RECEIVER) != 0;
}

bool TR_J9VMBase::jniDoNotPassThread(TR_ResolvedMethod *method)
{
    return (((TR_ResolvedJ9Method *)method)->getJNIProperties() & J9_FAST_JNI_DO_NOT_PASS_THREAD) != 0;
}
...
UDATA
TR_J9VMBase::thisThreadJavaVMOffset() { return offsetof(J9VMThread, javaVM); }
...
{
#if defined(J9VM_GC_REALTIME)
    return J9_VMTHREAD_SEGREGATED_ALLOCATION_CACHE_OFFSET + sizeClass * sizeof(J9VMGCSegregatedAllocationCacheEntry)
...
UDATA
TR_J9VMBase::thisThreadOSThreadOffset() { return offsetof(J9VMThread, osThread); }
...
UDATA TR_J9VMBase::thisThreadGetProfilingBufferCursorOffset() { return offsetof(J9VMThread, profilingBufferCursor); }

UDATA TR_J9VMBase::thisThreadGetProfilingBufferEndOffset() { return offsetof(J9VMThread, profilingBufferEnd); }

UDATA TR_J9VMBase::thisThreadGetOSRBufferOffset() { return offsetof(J9VMThread, osrBuffer); }

UDATA TR_J9VMBase::thisThreadGetOSRScratchBufferOffset() { return offsetof(J9VMThread, osrScratchBuffer); }

UDATA TR_J9VMBase::thisThreadGetOSRFrameIndexOffset() { return offsetof(J9VMThread, osrFrameIndex); }

UDATA TR_J9VMBase::thisThreadGetOSRReturnAddressOffset() { return offsetof(J9VMThread, osrReturnAddress); }
...
{
#if !defined(J9ZTPF)
    // This assume is valid to confirm the TDB is on an 8 byte boundary because we know the VMThread is already aligned
    // on a 256 byte boundary If this assume fails, then that means someone modified the J9VMThread structure which has
    // thrown off the alignment.
    TR_ASSERT(offsetof(J9VMThread, transactionDiagBlock) % 8 == 0,
        "The Transaction Diagnostic Block must be aligned on a doubleword i.e. 8 byte boundary");
...
int32_t TR_J9VMBase::getArraySpineShift(int32_t width)
{
    ...
    switch (width) {
        ...
        case 8:
            ...
            break;
        default:
            TR_ASSERT(0, "unexpected element width");
    }
    return shift;
}
...
int32_t TR_J9VMBase::getFirstArrayletPointerOffset(TR::Compilation *comp)
{
    ...
    int32_t headerSize = TR::Compiler->om.useHybridArraylets() ? TR::Compiler->om.discontiguousArrayHeaderSizeInBytes()
                                                               : TR::Compiler->om.contiguousArrayHeaderSizeInBytes();

    return (headerSize + TR::Compiler->om.sizeofReferenceField() - 1)
        & (-1) * (intptr_t)(TR::Compiler->om.sizeofReferenceField());
}
...
int32_t TR_J9VMBase::getArrayletFirstElementOffset(int8_t elementSize, TR::Compilation *comp)
{
    ...
    int32_t offset;
    if (TR::Compiler->om.compressObjectReferences()) {
        offset = (getFirstArrayletPointerOffset(comp) + TR::Compiler->om.sizeofReferenceField() + sizeof(UDATA) - 1)
            & (-(int32_t)sizeof(UDATA));
...
uint32_t TR_J9VMBase::getWordOffsetToGCFlags()
{
...
uint32_t TR_J9VMBase::getWriteBarrierGCFlagMaskAsByte()
{
    return (OBJECT_HEADER_OLD) >> 8; // shift right 8 bits
}

int32_t TR_J9VMBase::getByteOffsetToLockword(TR_OpaqueClassBlock *clazzPointer)
{
...
int32_t TR_J9VMBase::getInitialLockword(TR_OpaqueClassBlock *ramClass)
{
    ...
    return VM_ObjectMonitor::getInitialLockword(_jitConfig->javaVM,
        TR::Compiler->cls.convertClassOffsetToClassPtr(ramClass));
}
...
bool TR_J9VMBase::isLogSamplingSet()
{
    return TR::Options::getJITCmdLineOptions()
        && TR::Options::getJITCmdLineOptions()->getVerboseOption(TR_VerboseSampling);
}

uintptr_t TR_J9VMBase::getOffsetOfIndexableSizeField() { return offsetof(J9ROMArrayClass, arrayShape); }

// This method can be called by an app thread in onLoadInternal
TR_Debug *TR_J9VMBase::createDebug(TR::Compilation *comp)
{
...
uint32_t TR_J9VMBase::getNewInstanceImplVirtualCallOffset()
{
    return offsetof(J9Class, romableAotITable); // should be something like offsetof(J9Class, newInstanceImpl)
}

uint8_t *TR_J9VMBase::allocateDataCacheRecord(uint32_t numBytes, TR::Compilation *comp, bool contiguous,
    bool *shouldRetryAllocation, uint32_t allocationType, uint32_t *allocatedSizePtr)
{
    U_8 *retValue = NULL;
        ...
        *shouldRetryAllocation = false;
        ...
        if (dataCache) {
            ...
            if (!ptr) // designated data cache is not big enough
            {
                ...
                dataCache = TR_DataCacheManager::getManager()->reserveAvailableDataCache(_vmThread,
                    comp->getTotalNeededDataCacheSpace());
    ...
    {
        retValue
            = TR_DataCacheManager::getManager()->allocateDataCacheRecord(numBytes, allocationType, allocatedSizePtr);
    }

    return retValue;
}

uint8_t *TR_J9VMBase::allocateRelocationData(TR::Compilation *comp, uint32_t numBytes)
{
    uint8_t *relocationData = NULL;
    uint32_t size = 0;
    bool shouldRetryAllocation;
    relocationData = allocateDataCacheRecord(numBytes, comp, needsContiguousCodeAndDataCacheAllocation(),
        &shouldRetryAllocation, J9_JIT_DCE_RELOCATION_DATA, &size);
    if (!relocationData) {
...
uint8_t *TR_J9VMBase::allocateCodeMemory(TR::Compilation *comp, uint32_t warmCodeSize, uint32_t coldCodeSize,
...
bool TR_J9VMBase::supportsCodeCacheSnippets()
{
#if defined(TR_TARGET_X86)
...
#if defined(TR_TARGET_X86)
void *TR_J9VMBase::getAllocationPrefetchCodeSnippetAddress(TR::Compilation *comp)
{
    TR::CodeCache *codeCache = comp->cg()->getCodeCache();
    return codeCache->getCCPreLoadedCodeAddress(TR_AllocPrefetch, comp->cg());
}

void *TR_J9VMBase::getAllocationNoZeroPrefetchCodeSnippetAddress(TR::Compilation *comp)
{
    TR::CodeCache *codeCache = comp->cg()->getCodeCache();
...
//
void TR_J9VMBase::releaseCodeMemory(void *startPC, uint8_t bytesToSaveAtStart)
{
...
bool TR_J9VMBase::vmRequiresSelector(uint32_t mask) { return true; }

bool TR_J9VMBase::callTheJitsArrayCopyHelper() { return true; }

void *TR_J9VMBase::getReferenceArrayCopyHelperAddress()
{
...
bool TR_J9VMBase::isClassFinal(TR_OpaqueClassBlock *clazz)
{
    return (TR::Compiler->cls.romClassOf(clazz)->modifiers & J9AccFinal) ? true : false;
}

bool TR_J9VMBase::hasFinalFieldsInClass(TR_OpaqueClassBlock *clazz)
{
...
void TR_MarkHotField::mark(J9Class *clazz, bool isFixedClass)
{
    ...
    _slotIndex = ((_symRef->getOffset() - fej9->getObjectHeaderSizeInBytes()) / TR::Compiler->om.sizeofReferenceField())
        + 1; // +1 because low order bit means tenured alignment
...
bool TR_MarkHotField::visitSubclass(TR_PersistentClassInfo *subclassInfo)
{
    return markHotField(TR::Compiler->cls.convertClassOffsetToClassPtr(subclassInfo->getClassId()), false);
}

bool TR_MarkHotField::markHotField(J9Class *clazz, bool rootClass)
{
    TR_J9VMBase *fej9 = (TR_J9VMBase *)(_comp->fe());
    if (fej9->isAOT_DEPRECATED_DO_NOT_USE())
        return false;
...
void TR_J9VMBase::markHotField(TR::Compilation *comp, TR::SymbolReference *symRef, TR_OpaqueClassBlock *clazz,
    bool isFixedClass)
{
...
/**
 * Report a hot field if the JIT has determined that the field has met appropriate thresholds to be determined a hot
 * field. Valid if dynamicBreadthFirstScanOrdering is enabled.
 *
 ...
 */
void TR_J9VMBase::reportHotField(int32_t reducedCpuUtil, J9Class *clazz, uint8_t fieldOffset, uint32_t reducedFrequency)
{
    J9JavaVM *javaVM = _jitConfig->javaVM;
 ...
 */
bool TR_J9VMBase::isHotReferenceFieldRequired() { return TR::Compiler->om.isHotReferenceFieldRequired(); }

bool TR_J9VMBase::isIndexableDataAddrPresent()
{
 ...
 */
bool TR_J9VMBase::isOffHeapAllocationEnabled() { return TR::Compiler->om.isOffHeapAllocationEnabled(); }

bool TR_J9VMBase::scanReferenceSlotsInClassForOffset(TR::Compilation *comp, TR_OpaqueClassBlock *classPointer,
    int32_t offset)
{
    ...
    for (TR_VMField *field = iter.getFirst(); field != NULL; field = iter.getNext()) {
        ...
        if (field->isReference()) {
            ...
            int32_t fieldOffset = getInstanceFieldOffset(classPointer, fieldName, (uint32_t)strlen(fieldName),
                fieldSignature, (uint32_t)strlen(fieldSignature));
            if (fieldOffset == offset) {
                J9Class *fieldClass = TR::Compiler->cls.convertClassOffsetToClassPtr(
                    getClassFromSignature(fieldSignature, (int32_t)strlen(fieldSignature), comp->getCurrentMethod()));
...
int32_t TR_J9VMBase::findFirstHotFieldTenuredClassOffset(TR::Compilation *comp, TR_OpaqueClassBlock *opclazz)
{
    if (!isAOT_DEPRECATED_DO_NOT_USE()) {
...
void TR_J9VMBase::markClassForTenuredAlignment(TR::Compilation *comp, TR_OpaqueClassBlock *opclazz,
    uint32_t alignFromStart)
{
    if (!jitConfig->javaVM->memoryManagerFunctions->j9gc_hot_reference_field_required(jitConfig->javaVM)
        ...
        TR_ASSERT(0 == (alignFromStart % TR::Compiler->om.getObjectAlignmentInBytes()),
            "alignment undershot should be multiple of %d bytes", TR::Compiler->om.getObjectAlignmentInBytes());
...
char *TR_J9VMBase::getClassSignature_DEPRECATED(TR_OpaqueClassBlock *clazz, int32_t &length, TR_Memory *trMemory)
{
    int32_t numDims = 0;
...
char *TR_J9VMBase::getClassSignature(TR_OpaqueClassBlock *clazz, TR_Memory *trMemory)
{
    int32_t numDims = 0;
...
int32_t TR_J9VMBase::printTruncatedSignature(char *sigBuf, int32_t bufLen, J9UTF8 *className, J9UTF8 *name,
    J9UTF8 *signature)
{
    ...
    } else // truncate some parts of the signature
    {
        ...
        {
            sigLen = snprintf(sigBuf, (size_t)bufLen, "%.*s.%.*s%.*s", J9UTF8_LENGTH(className), utf8Data(className),
                J9UTF8_LENGTH(name), utf8Data(name), (J9UTF8_LENGTH(signature) - (sigLen - bufLen)),
                utf8Data(signature));
        } else {
            ...
            if (nameLen == bufLen - 3) // not even the method name can be printed entirely
                ...
                sigLen = snprintf(sigBuf, (size_t)bufLen, "%.*s.%.*s",
                    std::min<int32_t>(bufLen - 2 - nameLen, J9UTF8_LENGTH(className)), utf8Data(className), nameLen,
                    utf8Data(name));
        }
    }
    return sigLen;
...
int32_t TR_J9VMBase::printTruncatedSignature(char *sigBuf, int32_t bufLen, TR_OpaqueMethodBlock *method)
{
...
int32_t *TR_J9VMBase::getReferenceSlotsInClass(TR::Compilation *comp, TR_OpaqueClassBlock *classPointer)
{
...
bool TR_J9VMBase::shouldPerformEDO(TR::Block *catchBlock, TR::Compilation *comp)
{
    TR_ASSERT(catchBlock->isCatchBlock(), "shouldPerformEDO expected block_%d to be a catch block",
        catchBlock->getNumber());
    ...
    if (recomp && comp->getOptions()->allowRecompilation() && recomp->useSampling()
        && recomp->shouldBeCompiledAgain()) {
        int32_t threshold = TR::Compiler->vm.isVMInStartupPhase(_jitConfig)
            ? comp->getOptions()->getEdoRecompSizeThresholdInStartupMode()
            : comp->getOptions()->getEdoRecompSizeThreshold();
        if (comp->getOption(TR_EnableOldEDO)) {
            return comp->getMethodHotness() < hot && comp->getNodeCount() < threshold;
        } else {
            ncount_t nodeCount
                = TR::Compiler->vm.isVMInStartupPhase(_jitConfig) ? comp->getNodeCount() : comp->getAccurateNodeCount();
...
bool TR_J9VMBase::isClassArray(TR_OpaqueClassBlock *klass)
{
...
bool TR_J9VMBase::isInlineableNativeMethod(TR::Compilation *comp, TR::ResolvedMethodSymbol *methodSymbol)
{
...
bool TR_J9VMBase::isQueuedForVeryHotOrScorching(TR_ResolvedMethod *calleeMethod, TR::Compilation *comp)
{
    ...
    for (TR_MethodToBeCompiled *cur = TR::CompilationController::getCompilationInfo()->getMethodQueue(); cur;
         cur = cur->_next) {
        if (cur->getMethodDetails().getMethod() == (J9Method *)calleeMethod->getPersistentIdentifier()
            && cur->getMethodDetails().isOrdinaryMethod()) {
...
bool TR_J9VMBase::maybeHighlyPolymorphic(TR::Compilation *comp, TR_ResolvedMethod *caller, int32_t cpIndex,
    TR::Method *callee, TR_OpaqueClassBlock *receiverClass)
{
    // if (method->isInterface())
    {
        ...
        if (classOfMethod) {
            ...
            logprintf(comp->getOption(TR_TraceOptDetails), comp->log(),
                "maybeHighlyPolymorphic classOfMethod: %s yizhang\n", getClassNameChars(classOfMethod, len));
...
TR::Node *TR_J9VMBase::testAreSomeClassFlagsSet(TR::Node *j9ClassRefNode, uint32_t flagsToTest)
{
    ...
    TR::Node *maskedFlags
        = TR::Node::create(TR::iand, 2, loadClassFlags, TR::Node::iconst(j9ClassRefNode, flagsToTest));

    return maskedFlags;
}

TR::Node *TR_J9VMBase::testIsClassValueType(TR::Node *j9ClassRefNode)
{
    return testAreSomeClassFlagsSet(j9ClassRefNode, J9ClassIsValueType);
}

TR::Node *TR_J9VMBase::testIsClassIdentityType(TR::Node *j9ClassRefNode)
{
    return testAreSomeClassFlagsSet(j9ClassRefNode, J9ClassHasIdentity);
}

TR::Node *TR_J9VMBase::loadClassDepthAndFlags(TR::Node *j9ClassRefNode)
{
    TR::SymbolReference *classDepthAndFlagsSymRef
        = TR::comp()->getSymRefTab()->findOrCreateClassDepthAndFlagsSymbolRef();
...
TR::Node *TR_J9VMBase::testAreSomeClassDepthAndFlagsSet(TR::Node *j9ClassRefNode, uint32_t flagsToTest)
{
...
TR::Node *TR_J9VMBase::testIsClassArrayType(TR::Node *j9ClassRefNode)
{
    return testAreSomeClassDepthAndFlagsSet(j9ClassRefNode, getFlagValueForArrayCheck());
}

TR::Node *TR_J9VMBase::testIsArrayClassNullRestrictedType(TR::Node *j9ClassRefNode)
{
    return testAreSomeClassFlagsSet(j9ClassRefNode, J9ClassArrayIsNullRestricted);
}

TR::Node *TR_J9VMBase::loadArrayClassComponentType(TR::Node *j9ClassRefNode)
{
...
TR::Node *TR_J9VMBase::checkSomeArrayCompClassFlags(TR::Node *arrayBaseAddressNode, TR::ILOpCodes ifCmpOp,
...
TR::Node *TR_J9VMBase::checkArrayCompClassValueType(TR::Node *arrayBaseAddressNode, TR::ILOpCodes ifCmpOp)
{
    return checkSomeArrayCompClassFlags(arrayBaseAddressNode, ifCmpOp, J9ClassIsValueType);
}

TR::TreeTop *TR_J9VMBase::lowerAsyncCheck(TR::Compilation *comp, TR::Node *root, TR::TreeTop *treeTop)
{
    // Generate the inline test as a child of the asynccheck node
    //
    TR::SymbolReference *stackOverflowSymRef = new (comp->trHeapMemory()) TR::SymbolReference(comp->getSymRefTab(),
        TR::RegisterMappedSymbol::createMethodMetaDataSymbol(comp->trHeapMemory(), "stackOverflowMark"));
...
bool TR_J9VMBase::isMethodTracingEnabled(TR_OpaqueMethodBlock *method)
{
    return VM_VMHelpers::methodBeingTraced(_jitConfig->javaVM, (J9Method *)method);
}

bool TR_J9VMBase::isLambdaFormGeneratedMethod(TR_OpaqueMethodBlock *method)
{
    return VM_VMHelpers::isLambdaFormGeneratedMethod(vmThread(), (J9Method *)method);
}

bool TR_J9VMBase::isLambdaFormGeneratedMethod(TR_ResolvedMethod *method)
{
    return isLambdaFormGeneratedMethod(method->getPersistentIdentifier());
}

bool TR_J9VMBase::isSelectiveMethodEnterExitEnabled() { return false; }

bool TR_J9VMBase::canMethodEnterEventBeHooked()
{
    J9JavaVM *javaVM = _jitConfig->javaVM;
...
bool TR_J9VMBase::canMethodExitEventBeHooked()
{
    J9JavaVM *javaVM = _jitConfig->javaVM;
    J9HookInterface **vmHooks = javaVM->internalVMFunctions->getVMHookInterface(javaVM);
...
bool TR_J9VMBase::methodsCanBeInlinedEvenIfEventHooksEnabled(TR::Compilation *comp) { return false; }

bool TR_J9VMBase::canExceptionEventBeHooked()
{
    J9JavaVM *javaVM = _jitConfig->javaVM;
    J9HookInterface **vmHooks = javaVM->internalVMFunctions->getVMHookInterface(javaVM);
...
TR::TreeTop *TR_J9VMBase::lowerMethodHook(TR::Compilation *comp, TR::Node *root, TR::TreeTop *treeTop)
{
            ...
            TR::Node::createif(TR::ificmpne,
                TR::Node::create(TR::iand, 2,
                    TR::Node::create(TR::bu2i, 1,
                        TR::Node::createWithSymRef(root, TR::bload, 0,
                            new (comp->trHeapMemory()) TR::SymbolReference(comp->getSymRefTab(), addressSym))),
        ...
        if (comp->getOption(TR_EnableSelectiveEnterExitHooks) && !comp->compileRelocatableCode()) {
            ...
            TR::TreeTop *selectedTest = TR::TreeTop::create(comp,
                TR::Node::createif(TR::ificmpne,
                    TR::Node::create(TR::bu2i, 1,
                        TR::Node::createWithSymRef(root, TR::bload, 0,
                            new (comp->trHeapMemory()) TR::SymbolReference(comp->getSymRefTab(), extendedFlagsSym))),
                    TR::Node::create(root, TR::iconst, 0, 0)));
...
U_8 *TR_J9VMBase::fetchMethodExtendedFlagsPointer(J9Method *method)
{
    return ::fetchMethodExtendedFlagsPointer(method);
}

void *TR_J9VMBase::getStaticHookAddress(int32_t event) { return &vmThread()->javaVM->hookInterface.flags[event]; }
...
TR::TreeTop *TR_J9VMBase::lowerArrayLength(TR::Compilation *comp, TR::Node *root, TR::TreeTop *treeTop)
{
...
TR::TreeTop *TR_J9VMBase::lowerContigArrayLength(TR::Compilation *comp, TR::Node *root, TR::TreeTop *treeTop)
{
    lowerContiguousArrayLength(comp, root);
    return treeTop;
}

TR::TreeTop *TR_J9VMBase::lowerMultiANewArray(TR::Compilation *comp, TR::Node *root, TR::TreeTop *treeTop)
{
    ...
    else if ((root->getFirstChild()->getSymbolReference() != NULL)
        && root->getFirstChild()->getSymbolReference()->isLiteralPoolAddress()) {
    ...
    for (int32_t i = dims; i > 0; i--) {
        TR::SymbolReference *symRef
            = new (comp->trHeapMemory()) TR::SymbolReference(comp->getSymRefTab(), temp, offset);
        TR::TreeTop::create(comp, treeTop->getPrevTreeTop(),
            TR::Node::createWithSymRef(TR::istore, 1, 1, root->getChild(i), symRef));
    ...
    TR::Node *tempRef = TR::Node::createWithSymRef(root, TR::loadaddr, 0,
        new (comp->trHeapMemory()) TR::SymbolReference(comp->getSymRefTab(), temp));
...
TR::TreeTop *TR_J9VMBase::lowerToVcall(TR::Compilation *comp, TR::Node *root, TR::TreeTop *treeTop)
{
...
TR::TreeTop *TR_J9VMBase::lowerTree(TR::Compilation *comp, TR::Node *root, TR::TreeTop *treeTop)
{
    switch (root->getOpCodeValue()) {
        case TR::asynccheck:
            return lowerAsyncCheck(comp, root, treeTop);
        case TR::arraylength:
            return lowerArrayLength(comp, root, treeTop);
        case TR::contigarraylength:
            return lowerContigArrayLength(comp, root, treeTop);
        case TR::discontigarraylength: {
            lowerDiscontiguousArrayLength(comp, root);
            return treeTop;
        }
        case TR::multianewarray:
            return lowerMultiANewArray(comp, root, treeTop);
        case TR::athrow:
            return lowerToVcall(comp, root, treeTop);
        case TR::MethodEnterHook:
            return lowerMethodHook(comp, root, treeTop);
        case TR::MethodExitHook:
            return lowerMethodHook(comp, root, treeTop);
        default:
            return treeTop;
    }
...
uint8_t TR_J9VMBase::getCompilationShouldBeInterruptedFlag()
{
...
int64_t TR_J9VMBase::getCpuTimeSpentInCompThread(TR::Compilation *comp)
{
    _compInfoPT->getCompThreadCPU().update();
    return _compInfoPT->getCompThreadCPU().getCpuTime();
}

bool TR_J9VMBase::compilationShouldBeInterrupted(TR::Compilation *comp, TR_CallingContext callingContext)
{
    ...
    if (_compInfoPT->getCompThreadCPU().update()) // returns true if an update happened and metric looks good
    {
        if (TR::Options::getCmdLineOptions()->getVerboseOption(TR_VerboseCompilationThreads)) {
            ...
            TR_VerboseLog::writeLineLocked(TR_Vlog_PERF, "t=%6llu CPU time spent so far in compThread:%d = %d ms",
                static_cast<unsigned long long>(_compInfo->getPersistentInfo()->getElapsedTime()),
                _compInfoPT->getCompThreadId(), CPUmillis);
        }
    }

    if (comp->getOption(TR_EnableYieldVMAccess) && comp->getOption(TR_DisableNoVMAccess)
    ...
    if (!comp->getOption(TR_DisableNoVMAccess)) {
        bool exitClassUnloadMonitor
            = persistentMemory(_jitConfig)->getPersistentInfo()->GCwillBlockOnClassUnloadMonitor();
        if (comp->getOptions()->realTimeGC()) {
#if defined(J9VM_GC_REALTIME)
            J9JavaVM *vm = _jitConfig->javaVM;
...
#if defined(J9VM_GC_DYNAMIC_CLASS_UNLOADING)
            bool hadClassUnloadMonitor
                = TR::MonitorTable::get()->readReleaseClassUnloadMonitor(compInfoPTB->getCompThreadId()) >= 0;
            TR_ASSERT(hadClassUnloadMonitor,
                "Comp thread must hold classUnloadMonitor when compiling without VMaccess");
...
#endif
            //--- GC CAN INTERVENE HERE ---
            TR_ASSERT((vmThread()->publicFlags & J9_PUBLIC_FLAGS_VM_ACCESS) == 0,
                "comp thread must not have vm access");
            if (comp->getOptions()->realTimeGC()) {
                // no compilation on application thread
                TR_ASSERT(_compInfoPT, "Missing compilation info per thread.");
                _compInfoPT->waitForGCCycleMonitor(false);
...
bool TR_J9VMBase::checkForExclusiveAcquireAccessRequest(TR::Compilation *)
{
...
bool TR_J9VMBase::haveAccess(TR::Compilation *comp)
{
    if (!comp->getOption(TR_DisableNoVMAccess)) {
        if (vmThread()->publicFlags & J9_PUBLIC_FLAGS_VM_ACCESS)
            return true;
...
void TR_J9VMBase::releaseAccess(TR::Compilation *comp)
{
    if (!comp->getOption(TR_DisableNoVMAccess)) {
        if (vmThread()->publicFlags & J9_PUBLIC_FLAGS_VM_ACCESS)
            releaseVMAccess(vmThread());
...
bool TR_J9VMBase::tryToAcquireAccess(TR::Compilation *comp, bool *haveAcquiredVMAccess)
{
    bool hasVMAccess;
    *haveAcquiredVMAccess = false;
    ...
    if (!comp->getOption(TR_DisableNoVMAccess)) {
        if (!(vmThread()->publicFlags & J9_PUBLIC_FLAGS_VM_ACCESS)) {
            if (!vmThread()->javaVM->internalVMFunctions->internalTryAcquireVMAccessWithMask(vmThread(),
                    J9_PUBLIC_FLAGS_HALT_THREAD_ANY_NO_JAVA_SUSPEND)) {
                hasVMAccess = true;
                *haveAcquiredVMAccess = true;
            } else {
    ...
    if (!hasVMAccess) {
        logprintf(comp->getOption(TR_TraceOptDetails), comp->log(), "tryToAcquireAccess couldn't acquire vm access");
    }
    return hasVMAccess;
}

// The following is called from TR::Options::jitLatePostProcess
bool TR_J9VMBase::compileMethods(TR::OptionSet *optionSet, void *config)
{
    ...
    while (clazz) {
        if (!J9ROMCLASS_IS_PRIMITIVE_OR_ARRAY(clazz->romClass)) {
            ...
            for (uint32_t m = 0; m < clazz->romClass->romMethodCount; m++) {
                ...
                if (!(romMethod->modifiers & (J9AccNative | J9AccAbstract)) && method != newInstanceThunk
                    && !TR::CompilationInfo::isCompiled(method)) {
                    J9UTF8 *className;
                    J9UTF8 *name;
                    J9UTF8 *signature;
                    getClassNameSignatureFromMethod(method, className, name, signature);
                    if (J9UTF8_LENGTH(className) + J9UTF8_LENGTH(name) + J9UTF8_LENGTH(signature) + 1
                        > maxMethodNameLen) {
                        maxMethodNameLen
                            = J9UTF8_LENGTH(className) + J9UTF8_LENGTH(name) + J9UTF8_LENGTH(signature) + 1;
                    ...
                    snprintf(fullMethodName, maxMethodNameLen, "%.*s.%.*s%.*s", J9UTF8_LENGTH(className),
                    ...
                    if (TR::SimpleRegex::match(regex, fullMethodName)) {
                        ...
                        bool newPlanCreated;
                        TR_OptimizationPlan *plan = TR::CompilationController::getCompilationStrategy()->processEvent(
                        ...
                        if (plan) {
                            ...
                            IDATA result
                                = (IDATA)compInfo->compileMethod(vmThread, details, 0, TR_yes, NULL, &queued, plan);
...
// The following is called from TR::Options::jitLatePostProcess
void TR_J9VMBase::waitOnCompiler(void *config)
{
    J9JITConfig *jitConfig = (J9JITConfig *)config;
...
bool TR_J9VMBase::tossingCode() { return (_jitConfig->runtimeFlags & J9JIT_TOSS_CODE); }

TR::KnownObjectTable::Index TR_J9VMBase::getCompiledMethodReceiverKnownObjectIndex(TR::Compilation *comp)
{
...
bool TR_J9VMBase::methodMayHaveBeenInterpreted(TR::Compilation *comp)
{
    if (comp->ilGenRequest().details().isMethodHandleThunk())
        return false;
    else {
        int32_t initialCount
            = comp->mayHaveLoops() ? comp->getOptions()->getInitialBCount() : comp->getOptions()->getInitialCount();
        if (initialCount == 0)
            return false;
    }
...
bool TR_J9VMBase::canRecompileMethodWithMatchingPersistentMethodInfo(TR::Compilation *comp)
{
    return (comp->ilGenRequest().details().isJitDumpMethod()
        || // for a log recompilation, it's okay to compile at the same level
        comp->getOption(TR_EnableHCR)); // TODO: Why does this assume sometimes fail in HCR mode?
}
...
bool TR_J9VMBase::canDereferenceAtCompileTimeWithFieldSymbol(TR::Symbol *fieldSymbol, int32_t cpIndex,
    TR_ResolvedMethod *owningMethod)
{
    ...
    switch (fieldSymbol->getRecognizedField()) {
        ...
        default: {
            ...
            if (cpIndex < 0 && fieldSymbol->getRecognizedField() != TR::Symbol::UnknownField) {
...
bool TR_J9VMBase::canDereferenceAtCompileTime(TR::SymbolReference *fieldRef, TR::Compilation *comp)
{
    ...
    if (fieldRef->getSymbol()->isShadow()) {
        if (fieldRef->getReferenceNumber() < comp->getSymRefTab()->getNumPredefinedSymbols()) {
            ...
            return canDereferenceAtCompileTimeWithFieldSymbol(fieldRef->getSymbol(), fieldRef->getCPIndex(),
                fieldRef->getOwningMethodSymbol(comp)->getResolvedMethod());
    } else
        return false;
}

bool TR_J9VMBase::isStable(J9Class *fieldClass, int32_t cpIndex)
{
    TR_ASSERT_FATAL(fieldClass, "fieldClass must not be NULL");
    return jitIsFieldStable(vmThread(), fieldClass, cpIndex);
}

bool TR_J9VMBase::isForceInline(TR_ResolvedMethod *method)
{
    return jitIsMethodTaggedWithForceInline(vmThread(), (J9Method *)method->getPersistentIdentifier());
}

bool TR_J9VMBase::isDontInline(TR_ResolvedMethod *method)
{
    return jitIsMethodTaggedWithDontInline(vmThread(), (J9Method *)method->getPersistentIdentifier());
}

bool TR_J9VMBase::isChangesCurrentThread(TR_ResolvedMethod *method)
{
...
bool TR_J9VMBase::isIntrinsicCandidate(TR_ResolvedMethod *method)
{
    return jitIsMethodTaggedWithIntrinsicCandidate(vmThread(), (J9Method *)method->getPersistentIdentifier());
}

// Creates a node to initialize the local object flags field
//
TR::Node *TR_J9VMBase::initializeLocalObjectFlags(TR::Compilation *comp, TR::Node *allocationNode,
...
bool TR_J9VMBase::hasTwoWordObjectHeader() { return true; }

// Create trees to initialize the header of an object that is being created
// on the stack.
//
void TR_J9VMBase::initializeLocalObjectHeader(TR::Compilation *comp, TR::Node *allocationNode,
    TR::TreeTop *allocationTreeTop)
{
...
//
void TR_J9VMBase::initializeLocalArrayHeader(TR::Compilation *comp, TR::Node *allocationNode,
    TR::TreeTop *allocationTreeTop)
{
    /*
    TR_ASSERT(!comp->compileRelocatableCode(), "We create a class symbol with a CPI of -1!\n"
                     ...
                     "We need to figure out how to validate and relocate this symbol safely before removing this
    assertion!\n");
    */
    TR::TreeTop *prevTree = allocationTreeTop;
    ...
    TR::Node *classNode = TR::Node::createWithSymRef(allocationNode, TR::loadaddr, 0,
        comp->getSymRefTab()->findOrCreateClassSymbol(comp->getMethodSymbol(), -1, ramClass));
    ...
    if (TR::Compiler->om.canGenerateArraylets() && TR::Compiler->om.useHybridArraylets()
        && TR::Compiler->om.isDiscontiguousArray(instanceSize)) {
    ...
    //  clean up canGenerateArraylets() && TR::Compiler->om.useHybridArraylets() &&
    //  TR::Compiler->om.isDiscontiguousArray(instanceSize) queries?
    ...
    if (TR::Compiler->om.isOffHeapAllocationEnabled()) {
        ...
        // -----------------------------------------------------------------------------------
        TR::SymbolReference *dataAddrFieldOffsetSymRef
            = comp->getSymRefTab()->findOrCreateContiguousArrayDataAddrFieldShadowSymRef();
        TR::Node *headerSizeNode
            = TR::Node::lconst(allocationNode, TR::Compiler->om.contiguousArrayHeaderSizeInBytes());
        TR::Node *startOfDataNode = TR::Node::create(TR::aladd, 2, allocationNode, headerSizeNode);

        TR::Node *storeDataAddrPointerNode
            = TR::Node::createWithSymRef(TR::astorei, 2, allocationNode, startOfDataNode, 0, dataAddrFieldOffsetSymRef);
...
bool TR_J9VMBase::tlhHasBeenCleared()
{
...
bool TR_J9VMBase::isStaticObjectFlags()
{
...
uint32_t TR_J9VMBase::getStaticObjectFlags()
{
...
uintptr_t TR_J9VMBase::getOverflowSafeAllocSize()
{
...
void TR_J9VMBase::unsupportedByteCode(TR::Compilation *comp, U_8 opcode)
{
    char errMsg[64];
...
void TR_J9VMBase::unknownByteCode(TR::Compilation *comp, U_8 opcode)
{
    TR_ASSERT_FATAL(0, "Unknown bytecode to JIT %d \n", opcode);
}

char *TR_J9VMBase::printAdditionalInfoOnAssertionFailure(TR::Compilation *comp)
{
    size_t cSize = 20;
...
J9VMThread *getJ9VMThreadFromTR_VM(void *fe) { return ((TR_J9VMBase *)fe)->_vmThread; }

J9JITConfig *getJ9JitConfigFromFE(void *fe) { return ((TR_J9VMBase *)fe)->getJ9JITConfig(); }

void *TR_J9VMBase::setJ2IThunk(TR::Method *method, void *thunkptr, TR::Compilation *comp)
{
    return setJ2IThunk(method->signatureChars(), method->signatureLength(), thunkptr, comp);
}

void *TR_J9VMBase::getJ2IThunk(TR::Method *method, TR::Compilation *comp)
{
    return getJ2IThunk(method->signatureChars(), method->signatureLength(), comp);
}

void *TR_J9VMBase::getJ2IThunk(char *signatureChars, uint32_t signatureLength, TR::Compilation *comp)
{
...
void *TR_J9VMBase::setJ2IThunk(char *signatureChars, uint32_t signatureLength, void *thunkptr, TR::Compilation *comp)
{
...
#define THUNK_NAME "JIT virtual thunk"
    ...
    if (J9_EVENT_IS_HOOKED(jitConfig->javaVM->hookInterface, J9HOOK_VM_DYNAMIC_CODE_LOAD)
        ...
        ALWAYS_TRIGGER_J9HOOK_VM_DYNAMIC_CODE_LOAD(jitConfig->javaVM->hookInterface,
            jitConfig->javaVM->internalVMFunctions->currentVMThread(jitConfig->javaVM), NULL, (void *)thunkptr,
            *((uint32_t *)thunkptr - 2), THUNK_NAME, NULL);
#ifdef LINUX
    if (TR::CompilationInfoPerThreadBase::getPerfFile())
        j9jit_fprintf(TR::CompilationInfoPerThreadBase::getPerfFile(), "%p %lX %s\n", thunkptr,
            *((uint32_t *)thunkptr - 2), THUNK_NAME);
#endif

    return thunkptr;
}

void TR_J9VMBase::setInvokeExactJ2IThunk(void *thunkptr, TR::Compilation *comp)
{
    comp->getPersistentInfo()->getInvokeExactJ2IThunkTable()->addThunk((TR_MHJ2IThunk *)thunkptr, this);
}

void *TR_J9VMBase::findPersistentMHJ2IThunk(char *signatureChars)
{
    void *thunk = NULL;
...
void *TR_J9VMBase::persistMHJ2IThunk(void *thunk)
{
    return NULL; // only needed for AOT compilations
}

static char *getJ2IThunkSignature(char *invokeHandleSignature, uint32_t signatureLength, int argsToSkip,
    const char *description, TR::Compilation *comp)
{
    char *argsToCopy;
    ...
    logprintf(comp->getOption(TR_TraceCG), comp->log(), "JSR292: j2i-thunk signature for %s of '%.*s' is '%s'\n",
        description, signatureLength, invokeHandleSignature, resultBuf);
    return resultBuf;
}

static TR::Node *getEquivalentVirtualCallNode(TR::Node *callNode, int argsToSkip, const char *description,
    TR::Compilation *comp)
{
    TR::Node *j2iThunkCall = TR::Node::createWithSymRef(callNode, callNode->getOpCodeValue(),
        callNode->getNumChildren() - argsToSkip + 1, callNode->getSymbolReference());
...
char *TR_J9VMBase::getJ2IThunkSignatureForDispatchVirtual(char *invokeHandleSignature, uint32_t signatureLength,
    TR::Compilation *comp)
{
...
TR::Node *TR_J9VMBase::getEquivalentVirtualCallNodeForDispatchVirtual(TR::Node *callNode, TR::Compilation *comp)
{
...
bool TR_J9VMBase::needsInvokeExactJ2IThunk(TR::Node *callNode, TR::Compilation *comp)
{
    TR_ASSERT(callNode->getOpCode().isCall(), "needsInvokeExactJ2IThunk expects call node; found %s",
...
uintptr_t TR_J9VMBase::methodHandle_thunkableSignature(uintptr_t methodHandle)
{
    TR_ASSERT(haveAccess(), "methodHandle_thunkableSignature requires VM access");
    return getReferenceField(getReferenceField(methodHandle, "thunks", "Ljava/lang/invoke/ThunkTuple;"),
        "thunkableSignature", "Ljava/lang/String;");
}

uintptr_t TR_J9VMBase::methodHandle_type(uintptr_t methodHandle)
{
    TR_ASSERT(haveAccess(), "methodHandle_type requires VM access");
    return getReferenceField(methodHandle, "type", "Ljava/lang/invoke/MethodType;");
}

uintptr_t TR_J9VMBase::methodType_descriptor(uintptr_t methodType)
{
    TR_ASSERT(haveAccess(), "methodType_descriptor requires VM access");
    return getReferenceField(methodType, "methodDescriptor", "Ljava/lang/String;");
}
...
static uint8_t *bypassBaseAddress(uintptr_t mutableCallSite, TR_J9VMBase *fej9)
{
    ...
    */
    uintptr_t *fieldAddress = (uintptr_t *)fej9->getStaticFieldAddress(fej9->getObjectClass(mutableCallSite),
...
uintptr_t *TR_J9VMBase::mutableCallSite_bypassLocation(uintptr_t mutableCallSite)
{
    TR_ASSERT(haveAccess(), "mutableCallSite_bypassLocation requires VM access");

    int64_t bypassOffset = getInt64Field(
        getReferenceField(mutableCallSite, "globalRefCleaner", "Ljava/lang/invoke/GlobalRefCleaner;"), "bypassOffset");
    if (bypassOffset == 0)
        return NULL;
...
uintptr_t *TR_J9VMBase::mutableCallSite_findOrCreateBypassLocation(uintptr_t mutableCallSite)
{
    TR_ASSERT(haveAccess(), "mutableCallSite_bypassLocation requires VM access");
...
static TR_OpaqueMethodBlock *findClosestArchetype(TR_OpaqueClassBlock *clazz, const char *name, const char *signature,
    char *currentArgument, TR_FrontEnd *fe, J9VMThread *vmThread)
{
    ...
    if (currentArgument[1] == ')') {
        ...
        TR_OpaqueMethodBlock *result
            = findClosestArchetype(clazz, name, signature, nextSignatureArgument(currentArgument), fej9, vmThread);
        if (result)
            return result;
    ...
    if (result) {
        ...
        if (methodClass == clazz) {
            if (details)
                TR_VerboseLog::writeLineLocked(TR_Vlog_MHD, "%p   - Found matching archetype %.*s.%s%s", vmThread,
                    methodClassNameLength, methodClassName, name, signature);
        } else {
            ...
            result = NULL;
            if (details)
                TR_VerboseLog::writeLineLocked(TR_Vlog_MHD, "%p   - Ignoring inherited archetype %.*s.%s%s", vmThread,
                    methodClassNameLength, methodClassName, name, signature);
        }
    }
    return result;
...
TR_OpaqueMethodBlock *TR_J9VMBase::lookupArchetype(TR_OpaqueClassBlock *clazz, const char *name, const char *signature)
{
...
TR_OpaqueMethodBlock *TR_J9VMBase::lookupMethodHandleThunkArchetype(uintptr_t methodHandle)
{
    ...
    TR_OpaqueMethodBlock *result
        = lookupArchetype(getObjectClass((uintptr_t)methodHandle), methodName, archetypeSpecimenSignature);
    if (!result) {
        strcpy(returnType, "I");
        result = lookupArchetype(getObjectClass((uintptr_t)methodHandle), "invokeExact_thunkArchetype_X",
            archetypeSpecimenSignature);
    }
    return result;
}

TR_ResolvedMethod *TR_J9VMBase::createMethodHandleArchetypeSpecimen(TR_Memory *trMemory,
    TR_OpaqueMethodBlock *archetype, uintptr_t *methodHandleLocation, TR_ResolvedMethod *owningMethod)
{
    intptr_t length;
...

    {
        ...
        TR_ASSERT(archetype == lookupMethodHandleThunkArchetype(*methodHandleLocation),
            "Explicitly provided archetype must be the right one");
        uintptr_t signatureString
            = getReferenceField(getReferenceField(*methodHandleLocation, "thunks", "Ljava/lang/invoke/ThunkTuple;"),
                "thunkableSignature", "Ljava/lang/String;");
    ...
    TR_ResolvedMethod *result
        = createResolvedMethodWithSignature(trMemory, archetype, NULL, thunkableSignature, length, owningMethod);
...
TR_ResolvedMethod *TR_J9VMBase::createMethodHandleArchetypeSpecimen(TR_Memory *trMemory,
    uintptr_t *methodHandleLocation, TR_ResolvedMethod *owningMethod)
{
...
#else
    uint32_t offset = getInstanceFieldOffset(getObjectClass(mutableCallSite), "invalidationCookie", "J");
#endif
...
TR::KnownObjectTable::Index TR_J9VMBase::mutableCallSiteEpoch(TR::Compilation *comp, TR::KnownObjectTable::Index mcs)
{
    TR::KnownObjectTable *knot = comp->getKnownObjectTable();
    if (knot == NULL) {
        return TR::KnownObjectTable::UNKNOWN;
    ...
    uintptr_t mh = getVolatileReferenceField(mutableCallSite, "target", "Ljava/lang/invoke/MethodHandle;");
#else
    uintptr_t mh = getVolatileReferenceField(mutableCallSite, "epoch", "Ljava/lang/invoke/MethodHandle;");
#endif
...
TR_J9VMBase::MethodOfHandle TR_J9VMBase::methodOfDirectOrVirtualHandle(uintptr_t *mh, bool isVirtual)
{
    ...
    uintptr_t jlClass = getReferenceField(methodHandle, "referenceClass", "Ljava/lang/Class;");
    ...
    if (isVirtual) {
        ...
        TR_OpaqueMethodBlock **vtable = (TR_OpaqueMethodBlock **)((uintptr_t)clazz + vftStartOffset);
...
void *TR_J9VMBase::methodHandle_jitInvokeExactThunk(uintptr_t methodHandle)
{
    TR_ASSERT(haveAccess(), "methodHandle_jitInvokeExactThunk requires VM access");
    return (void *)(intptr_t)getInt64Field(getReferenceField(methodHandle, "thunks", "Ljava/lang/invoke/ThunkTuple;"),
        "invokeExactThunk");
}

#if defined(J9VM_OPT_OPENJDK_METHODHANDLE)
TR_OpaqueMethodBlock *TR_J9VMBase::targetMethodFromMemberName(TR::Compilation *comp,
    TR::KnownObjectTable::Index objIndex)
{
    auto knot = comp->getKnownObjectTable();
    if (objIndex != TR::KnownObjectTable::UNKNOWN && knot && !knot->isNull(objIndex)) {
...
TR_OpaqueMethodBlock *TR_J9VMBase::targetMethodFromMemberName(uintptr_t memberName)
{
...
TR_OpaqueMethodBlock *TR_J9VMBase::targetMethodFromMethodHandle(TR::Compilation *comp,
    TR::KnownObjectTable::Index objIndex)
{
    OMR::Logger *log = comp->log();
    ...
    if (objIndex != TR::KnownObjectTable::UNKNOWN && knot && !knot->isNull(objIndex)) {
        ...
        TR_OpaqueClassBlock *mhClass = getSystemClassFromClassName(mhClassName, mhClassNameLen);

        if (mhClass == NULL) {
            logprints(trace, log, "targetMethodFromMethodHandle: MethodHandle is not loaded\n");
        ...
        if (isInstanceOf(objClass, mhClass, true, true) != TR_yes) {
            logprintf(trace, log,
                "targetMethodFromMethodHandle: Cannot load ((MethodHandle)obj%d).form because obj%d is not a "
                "MethodHandle\n",
                objIndex, objIndex);
        ...
        if (vmentry == 0) {
            uintptr_t form = getReferenceField(handle, "form", "Ljava/lang/invoke/LambdaForm;");

            if (form == 0) {
                logprintf(trace, log, "targetMethodFromMethodHandle: null ((MethodHandle)obj%d).form\n", objIndex);

                return NULL;
            }

            vmentry = getReferenceField(form, "vmentry", "Ljava/lang/invoke/MemberName;");

            if (vmentry == 0) {
                logprintf(trace, log, "targetMethodFromMethodHandle: null ((MethodHandle)obj%d).form.vmentry\n",
                    objIndex);
            ...
            UDATA success = javaVM->memoryManagerFunctions->j9gc_objaccess_compareAndSwapObject(vmThread(),
                (J9Object *)handle, keepAliveAddr, NULL, (J9Object *)vmentry);

            if (success == 0) {
                vmentry = getVolatileReferenceFieldAt(handle, keepAliveOffsetNoHeader);
                TR_ASSERT_FATAL(vmentry != 0,
...
bool TR_J9VMBase::getMemberNameMethodInfo(TR::Compilation *comp, TR::KnownObjectTable::Index objIndex,
    MemberNameMethodInfo *out)
{
    ...
    *out = zero;
    ...
    TR_OpaqueClassBlock *mnClass = getSystemClassFromClassName(mnClassName, mnClassNameLen);
...
bool TR_J9VMBase::isInvokeCacheEntryAnArray(uintptr_t *invokeCacheArray)
{
    TR::VMAccessCriticalSection vmAccess(this);
    return VM_VMHelpers::objectIsArray(getCurrentVMThread(), (j9object_t)*invokeCacheArray);
}

TR::KnownObjectTable::Index TR_J9VMBase::getKnotIndexOfInvokeCacheArrayAppendixElement(TR::Compilation *comp,
    uintptr_t *invokeCacheArray)
{
    TR::KnownObjectTable *knot = comp->getOrCreateKnownObjectTable();
    if (!knot)
        return TR::KnownObjectTable::UNKNOWN;
    ...
    TR::VMAccessCriticalSection vmAccess(this);
    uintptr_t appendixElementRef
        = (uintptr_t)getReferenceElement(*invokeCacheArray, JSR292_invokeCacheArrayAppendixIndex);
    return knot->getOrCreateIndex(appendixElementRef);
}

TR_ResolvedMethod *TR_J9VMBase::targetMethodFromInvokeCacheArrayMemberNameObj(TR::Compilation *comp,
    TR_ResolvedMethod *owningMethod, uintptr_t *invokeCacheArray)
{
    TR_OpaqueMethodBlock *targetMethodObj = 0;
    {
        TR::VMAccessCriticalSection vmAccess(this);
        uintptr_t memberNameElementRef
            = (uintptr_t)getReferenceElement(*invokeCacheArray, JSR292_invokeCacheArrayMemberNameIndex);
...
static char *getSignatureForLinkToStatic(const char * const extraParamsBefore, const char * const extraParamsAfter,
    TR::Compilation *comp, J9UTF8 *romMethodSignature, int32_t &signatureLength)
{
    ...
    char *linkToStaticSignature = (char *)comp->trMemory()->allocateMemory(signatureAllocSize, heapAlloc);

    TR_ASSERT_FATAL(origSignature[0] == '(', "method signature must begin with '(': `%.*s'", origSignatureLength,
        origSignature);
    ...
    TR_ASSERT_FATAL(0 <= parsedLength && parsedLength <= INT_MAX,
        "bad parsedLength %lld for romMethodSignature (J9UTF8*)%p", romMethodSignature);

    TR_ASSERT_FATAL((int)parsedLength == origSignatureLength,
        "parsed method signature length %d differs from claimed length %d "
        "(origSignature `%.*s')",
        (int)parsedLength, origSignatureLength,
        parsedLength > origSignatureLength ? (int)parsedLength : origSignatureLength, origSignature);

    // Put together the new signature.
    signatureLength = TR::snprintfNoTrunc(linkToStaticSignature, signatureAllocSize, "(%s%.*s%s)%.*s",
        extraParamsBefore, (int)(paramsEnd - paramsStart), paramsStart, extraParamsAfter,
        (int)(returnTypeEnd - returnType), returnType);

    return linkToStaticSignature;
}

char *TR_J9VMBase::getSignatureForLinkToStaticForInvokeHandle(TR::Compilation *comp, J9UTF8 *romMethodSignature,
    int32_t &signatureLength)
{
    return getSignatureForLinkToStatic("Ljava/lang/Object;", "Ljava/lang/Object;Ljava/lang/Object;", comp,
        romMethodSignature, signatureLength);
}

char *TR_J9VMBase::getSignatureForLinkToStaticForInvokeDynamic(TR::Compilation *comp, J9UTF8 *romMethodSignature,
    int32_t &signatureLength)
{
    return getSignatureForLinkToStatic("", "Ljava/lang/Object;Ljava/lang/Object;", comp, romMethodSignature,
        signatureLength);
}

TR::KnownObjectTable::Index TR_J9VMBase::delegatingMethodHandleTarget(TR::Compilation *comp,
    TR::KnownObjectTable::Index dmhIndex, bool trace)
{
    OMR::Logger *log = comp->log();
    TR::KnownObjectTable *knot = comp->getOrCreateKnownObjectTable();
    if (knot == NULL)
    ...
    TR_OpaqueClassBlock *cwClass = getSystemClassFromClassName(cwClassName, cwClassNameLen);

    logprintf(trace, log, "delegating method handle target: delegating mh obj%d(*%p) CountingWrapper %p\n", dmhIndex,
        knot->getPointerLocation(dmhIndex), cwClass);

    if (cwClass == NULL) {
        logprints(trace, log, "failed to find CountingWrapper\n");

        return TR::KnownObjectTable::UNKNOWN;
    }

    TR_OpaqueClassBlock *dmhType = getObjectClassFromKnownObjectIndex(comp, dmhIndex);

    if (isInstanceOf(dmhType, cwClass, true) != TR_yes) {
        logprints(trace, log, "object is not a CountingWrapper\n");
    ...
    logprintf(trace, log, "target is obj%d\n", targetIndex);
...
TR::KnownObjectTable::Index TR_J9VMBase::delegatingMethodHandleTargetHelper(TR::Compilation *comp,
    TR::KnownObjectTable::Index dmhIndex, TR_OpaqueClassBlock *cwClass)
{
    ...
    TR::KnownObjectTable *knot = comp->getKnownObjectTable();
    int32_t targetFieldOffset = getInstanceFieldOffset(cwClass, "target", "Ljava/lang/invoke/MethodHandle;");
...
UDATA
TR_J9VMBase::getVMTargetOffset() { return vmThread()->javaVM->vmtargetOffset; }

UDATA
TR_J9VMBase::getVMIndexOffset() { return vmThread()->javaVM->vmindexOffset; }

#endif /* defined(J9VM_OPT_OPENJDK_METHODHANDLE) */

TR::KnownObjectTable::Index TR_J9VMBase::getMemberNameFieldKnotIndexFromMethodHandleKnotIndex(TR::Compilation *comp,
...
TR::KnownObjectTable::Index TR_J9VMBase::getLayoutVarHandle(TR::Compilation *comp,
    TR::KnownObjectTable::Index layoutIndex)
{
    ...
    TR_OpaqueClassBlock *layoutClass = getSystemClassFromClassName(layoutClassName, layoutClassNameLen);

    TR_OpaqueClassBlock *layoutObjClass = getObjectClassFromKnownObjectIndex(comp, layoutIndex);

    if (layoutClass == NULL || isInstanceOf(layoutObjClass, layoutClass, true, true) != TR_yes) {
        logprintf(comp->getOption(TR_TraceOptDetails), comp->log(),
            "getLayoutVarHandle: failed ValueLayouts$AbstractValueLayout type check.\n");
        return result;
    }

    uintptr_t layoutObj = knot->getPointer(layoutIndex);
    uintptr_t vhObject = getReferenceField(layoutObj, "handle", "Ljava/lang/invoke/VarHandle;");
    if (!vhObject)
        return result;
    ...
    TR::KnownObjectTable::Index methodIndex)
{
    ...
    TR_OpaqueClassBlock *methodClass = getSystemClassFromClassName(methodClassName, methodClassNameLen);

    TR_OpaqueClassBlock *methodObjClass = getObjectClassFromKnownObjectIndex(comp, methodIndex);

    if (methodClass == NULL || isInstanceOf(methodObjClass, methodClass, true, true) != TR_yes) {
        logprintf(comp->getOption(TR_TraceOptDetails), comp->log(),
            "getMethodAccessorIndex: failed java/lang/reflect/Method type check.\n");
        return result;
    }

    uintptr_t methodObj = knot->getPointer(methodIndex);
    uintptr_t maObject = getReferenceField(methodObj, "methodAccessor", "Ljdk/internal/reflect/MethodAccessor;");
    if (!maObject)
        return result;
...
int32_t TR_J9VMBase::getVarHandleAccessDescriptorMode(TR::Compilation *comp, TR::KnownObjectTable::Index adIndex)
{
    TR::VMAccessCriticalSection getAccessDescriptorMode(this);
    TR::KnownObjectTable *knot = comp->getKnownObjectTable();

    const char * const adClassName = "java/lang/invoke/VarHandle$AccessDescriptor";
    const int adClassNameLen = (int)strlen(adClassName);
    TR_OpaqueClassBlock *adClass = getSystemClassFromClassName(adClassName, adClassNameLen);

    TR_OpaqueClassBlock *adObjClass = getObjectClassFromKnownObjectIndex(comp, adIndex);

    if (adClass == NULL || isInstanceOf(adObjClass, adClass, true, true) != TR_yes) {
        logprints(comp->getOption(TR_TraceOptDetails), comp->log(),
...
TR::KnownObjectTable::Index TR_J9VMBase::getMethodHandleTableEntryIndex(TR::Compilation *comp,
    TR::KnownObjectTable::Index vhIndex, TR::KnownObjectTable::Index adIndex)
{
    ...
    uintptr_t typesAndInvokersObj
        = getReferenceField(varHandleObj, "typesAndInvokers", "Ljava/lang/invoke/VarHandle$TypesAndInvokers;");
    if (!typesAndInvokersObj)
        return result;

    mhTable = getReferenceField(typesAndInvokersObj, "methodHandle_table", "[Ljava/lang/invoke/MethodHandle;");

    mtTable = getReferenceField(typesAndInvokersObj, "methodType_table", "[Ljava/lang/invoke/MethodType;");
#else
    mhTable = getReferenceField(varHandleObj, "methodHandleTable", "[Ljava/lang/invoke/MethodHandle;");

    mtTable = getReferenceField(varHandleObj, "methodTypeTable", "[Ljava/lang/invoke/MethodType;");
    ...
    int32_t varHandleExactFieldOffset = getInstanceFieldOffset(getObjectClass(varHandleObj), "exact", "Z");
    ...
    if (varHandleHasInvokeExactBehaviour) {
        ...
        if (!methodTypeTableEntryObj)
            return result;
        uintptr_t symbolicMTExactObj
            = getReferenceField(accessDescriptorObj, "symbolicMethodTypeExact", "Ljava/lang/invoke/MethodType;");
...
bool TR_J9VMBase::isMethodHandleExpectedType(TR::Compilation *comp, TR::KnownObjectTable::Index mhIndex,
...
TR_J9VMBase::isSubtypeOf(TR_OpaqueClassBlock *fromClass, TR_OpaqueClassBlock *toClass, TR::Compilation *comp)
   ...
   if (!sameClassLoaders(fromClass, toClass))
    ...
    printf("enum of Integer: %d\n", TR::DataType::getSize(getClassPrimitiveDataType(getClassFromSignature("java/lang/Integer", 17, comp->getCurrentMethod()))));
    printf("enum of Boolean: %d\n", TR::DataType::getSize(getClassPrimitiveDataType(getClassFromSignature("java/lang/Boolean", 17, comp->getCurrentMethod()))));
 ...
 */
bool TR_J9VMBase::stringEquals(TR::Compilation *comp, uintptr_t *stringLocation1, uintptr_t *stringLocation2,
    int32_t &result)
{
    TR::VMAccessCriticalSection stringEquals(this, TR::VMAccessCriticalSection::tryToAcquireVMAccess, comp);
 ...
 */
bool TR_J9VMBase::getStringHashCode(TR::Compilation *comp, uintptr_t *stringLocation, int32_t &result)
{
    TR::VMAccessCriticalSection getStringHashCodeCriticalSection(this,
        TR::VMAccessCriticalSection::tryToAcquireVMAccess, comp);
...
bool TR_J9VMBase::getStringFieldByName(TR::Compilation *comp, TR::SymbolReference *stringRef,
    TR::SymbolReference *fieldRef, void *&pResult)
{
    TR::VMAccessCriticalSection getStringFieldCriticalSection(this, TR::VMAccessCriticalSection::tryToAcquireVMAccess,
        comp);
...
bool TR_J9VMBase::isJavaLangObject(TR_OpaqueClassBlock *clazz)
{
 ...
 */
TR_YesNoMaybe TR_J9VMBase::typeReferenceStringObject(TR_OpaqueClassBlock *clazz)
{
...
bool TR_J9VMBase::isString(TR_OpaqueClassBlock *clazz)
{
    return (J9Class *)clazz == J9VMJAVALANGSTRING(jitConfig->javaVM);
}

int32_t TR_J9VMBase::getStringLength(uintptr_t objectPointer)
{
...
uint16_t TR_J9VMBase::getStringCharacter(uintptr_t objectPointer, int32_t index)
{
...
int32_t TR_J9VMBase::getStringUTF8Length(uintptr_t objectPointer)
{
    TR_ASSERT(haveAccess(), "Must have VM access to call getStringUTF8Length");
    TR_ASSERT(objectPointer, "assertion failure");
    uint64_t actualLength = vmThread()->javaVM->internalVMFunctions->getStringUTF8LengthTruncated(vmThread(),
        (j9object_t)objectPointer, INT64_MAX);
    ...
    TR_ASSERT_FATAL(actualLength + 1 <= std::numeric_limits<int32_t>::max(),
        "UTF8-encoded String length of " UINT64_PRINTF_FORMAT
        " must be in the range permitted for type int32_t, also allowing for a NUL terminator.\n",
        actualLength);

    return (int32_t)actualLength;
}

uint64_t TR_J9VMBase::getStringUTF8UnabbreviatedLength(uintptr_t objectPointer)
{
    TR_ASSERT(haveAccess(), "Must have VM access to call getStringUTF8Length");
    TR_ASSERT(objectPointer, "assertion failure");
    return vmThread()->javaVM->internalVMFunctions->getStringUTF8LengthTruncated(vmThread(), (j9object_t)objectPointer,
        INT64_MAX);
}

char *TR_J9VMBase::getStringUTF8(uintptr_t objectPointer, char *buffer, uintptr_t bufferSize)
{
    TR_ASSERT(haveAccess(), "Must have VM access to call getStringUTF8");

    vmThread()->javaVM->internalVMFunctions->copyStringToUTF8Helper(vmThread(), (j9object_t)objectPointer,
        J9_STR_NULL_TERMINATE_RESULT, 0, J9VMJAVALANGSTRING_LENGTH(vmThread(), objectPointer), (U_8 *)buffer,
        (UDATA)bufferSize);

    return buffer;
}

uint32_t TR_J9VMBase::getVarHandleHandleTableOffset(TR::Compilation *comp)
{
...
void TR_J9VMBase::reportILGeneratorPhase()
{
    if (!_vmThread)
        return;

    enum {
        DEFAULT_LOW_BYTE = 0x80
    };

    vmThread()->omrVMThread->vmState = J9VMSTATE_JIT | (DEFAULT_LOW_BYTE & 0xFF);
}

void TR_J9VMBase::reportOptimizationPhase(OMR::Optimizations opts)
{
    if (!_vmThread)
        return;
...
void TR_J9VMBase::reportAnalysisPhase(uint8_t id)
{
    if (!_vmThread)
        return;
...
void TR_J9VMBase::reportOptimizationPhaseForSnap(OMR::Optimizations opts, TR::Compilation *comp)
{
    if (!_vmThread)
        return;
...
void TR_J9VMBase::reportCodeGeneratorPhase(TR::CodeGenPhase::PhaseValue phase)
{
    if (!_vmThread)
        return;
...
int32_t TR_J9VMBase::saveCompilationPhase()
{
    if (vmThread())
...
void TR_J9VMBase::restoreCompilationPhase(int32_t phase)
{
    if (vmThread())
        vmThread()->omrVMThread->vmState = phase;
}

void TR_J9VMBase::reportPrexInvalidation(void *startPC)
{
    if (!_vmThread)
        return;
...
void TR_J9VMBase::reportUnloadInvalidation(void *startPC)
{
    if (!_vmThread) {
        return;
    }
...
void TR_J9VMBase::setHasFailedCodeCacheAllocation()
{
    if (!_compInfo->getRampDownMCT()) {
        _compInfo->setRampDownMCT();
        if (TR::Options::getCmdLineOptions()->getVerboseOption(TR_VerboseCompilationThreads)) {
            TR_VerboseLog::writeLineLocked(TR_Vlog_INFO, "t=%u setRampDownMCT",
                (uint32_t)_compInfo->getPersistentInfo()->getElapsedTime());
        }
    }
}

TR::CodeCache *TR_J9VMBase::getDesignatedCodeCache(TR::Compilation *comp) // MCT
{
    int32_t numReserved;
...
void *TR_J9VMBase::getCCPreLoadedCodeAddress(TR::CodeCache *codeCache, TR_CCPreLoadedCode h, TR::CodeGenerator *cg)
{
    return codeCache->getCCPreLoadedCodeAddress(h, cg);
}

void TR_J9VMBase::reserveTrampolineIfNecessary(TR::Compilation *comp, TR::SymbolReference *symRef,
    bool inBinaryEncoding)
{
    ...
    TR::CodeCache *newCache
        = curCache; // optimistically assume that we will manage to allocate trampoline from current code cache
    if (isAOT_DEPRECATED_DO_NOT_USE() && isRecursive) {
        TR_AOTMethodHeader *aotMethodHeaderEntry = comp->getAotMethodHeaderEntry();
        aotMethodHeaderEntry->flags
            |= TR_AOTMethodHeader_NeedsRecursiveMethodTrampolineReservation; // Set flag in TR_AOTMethodHeader
      ...
      if (isAOT_DEPRECATED_DO_NOT_USE() && (comp->getOption(TR_TraceRelocatableDataCG) || comp->getOption(TR_TraceRelocatableDataDetailsCG)) )
        ...
        if (!isAOT_DEPRECATED_DO_NOT_USE()) {
            ...
            if (retValue != OMR::CodeCacheErrorCode::ERRORCODE_SUCCESS) {
                ...
                if (retValue == OMR::CodeCacheErrorCode::ERRORCODE_INSUFFICIENTSPACE
                    && !inBinaryEncoding) // code cache full, allocate a new one
                {
                    // Allocate a new code cache and try again
                    newCache = TR::CodeCacheManager::instance()->getNewCodeCache(comp->getCompThreadID(),
                        curCache->_kind); // class unloading may happen here
                    if (newCache) {
                        ...
                        if (compInfoPTB->compilationShouldBeInterrupted()) {
                            ...
                            newCache = NULL;
                            comp->failCompilation<TR::CompilationInterrupted>(
                                "Compilation Interrupted when reserving trampoline if necessary");
                        } else {
                    ...
                    newCache = 0;
                    if (inBinaryEncoding) {
                        comp->failCompilation<TR::RecoverableTrampolineError>(
                            "Failed to delete the old reservation"); // RAS only
                    } else {
    ...
    {
        newCache = getResolvedTrampoline(comp, curCache,
            (J9Method *)symRef->getSymbol()
...
TR_IProfiler *TR_J9VMBase::getIProfiler() { return NULL; }

bool TR_J9VMBase::isClassLibraryMethod(TR_OpaqueMethodBlock *method, bool vettedForAOT)
{
    if (!_vmThread || !_vmThread->javaVM)
        return false;
    J9ClassLoader *classLoader
        = TR::Compiler->cls
              .convertClassOffsetToClassPtr((TR_OpaqueClassBlock *)J9_CLASS_FROM_METHOD(((J9Method *)method)))
              ->classLoader;
...
bool TR_J9VMBase::isClassLibraryClass(TR_OpaqueClassBlock *clazz)
{
    if (!_vmThread || !_vmThread->javaVM)
        return false;
...
bool TR_J9VMBase::getSupportsRecognizedMethods()
{
    TR_ASSERT(TR::Compiler->target.cpu.isZ() || TR::Compiler->target.cpu.isX86() || TR::Compiler->target.cpu.isPower()
            || TR::Compiler->target.cpu.isARM() || TR::Compiler->target.cpu.isARM64() || !isAOT_DEPRECATED_DO_NOT_USE(),
        "getSupportsRecognizedMethods must be called only on X,P,Z or only for non-AOT");
    return true;
}

bool TR_J9VMBase::isCallGraphProfilingEnabled()
{
    if (getIProfiler()) {
...
TR_AbstractInfo *TR_J9VMBase::createIProfilingValueInfo(TR_ByteCodeInfo &bcInfo, TR::Compilation *comp)
{
...
TR_ExternalValueProfileInfo *TR_J9VMBase::getValueProfileInfoFromIProfiler(TR_ByteCodeInfo &bcInfo,
    TR::Compilation *comp)
{
    TR_IProfiler *iProfiler = getIProfiler();
    if (iProfiler == NULL)
...
uint32_t *TR_J9VMBase::getAllocationProfilingDataPointer(TR_ByteCodeInfo &bcInfo, TR_OpaqueClassBlock *clazz,
    TR_OpaqueMethodBlock *method, TR::Compilation *comp)
{
    TR_IProfiler *iProfiler = getIProfiler();
    if (iProfiler == NULL)
        return NULL;
...
uint32_t *TR_J9VMBase::getGlobalAllocationDataPointer()
{
    TR_IProfiler *iProfiler = getIProfiler();
    if (iProfiler == NULL)
        return NULL;
...
TR_ExternalProfiler *TR_J9VMBase::hasIProfilerBlockFrequencyInfo(TR::Compilation &comp)
{
    TR_IProfiler *iProfiler = getIProfiler();
    if (iProfiler == NULL)
        return NULL;
...
void TR_J9VMBase::createHWProfilerRecords(TR::Compilation *comp) { return; }

uint32_t TR_J9VMBase::getMethodSize(TR_OpaqueMethodBlock *method)
{
...
int32_t TR_J9VMBase::getLineNumberForMethodAndByteCodeIndex(TR_OpaqueMethodBlock *method, int32_t bcIndex)
{
    return isAOT_DEPRECATED_DO_NOT_USE()
        ? -1
        : (int32_t)getLineNumberForROMClass(_jitConfig->javaVM, (J9Method *)method, bcIndex);
}

bool TR_J9VMBase::isJavaOffloadEnabled()
{
...
int32_t TR_J9VMBase::getInvocationCount(TR_OpaqueMethodBlock *methodInfo)
{
    J9Method *method = (J9Method *)methodInfo;
    return TR::CompilationInfo::getInvocationCount(method);
}

bool TR_J9VMBase::setInvocationCount(TR_OpaqueMethodBlock *methodInfo, int32_t oldCount, int32_t newCount)
{
    J9Method *method = (J9Method *)methodInfo;
    return TR::CompilationInfo::setInvocationCount(method, oldCount, newCount);
}

bool TR_J9VMBase::startAsyncCompile(TR_OpaqueMethodBlock *method, void *oldStartPC, bool *queued,
...
bool TR_J9VMBase::isBeingCompiled(TR_OpaqueMethodBlock *method, void *startPC)
{
    return _compInfo->isQueuedForCompilation((J9Method *)method, startPC);
}

I_32 TR_J9VMBase::vTableSlotToVirtualCallOffset(U_32 vTableSlot)
{
...
U_32 TR_J9VMBase::offsetOfIsOverriddenBit() { return J9_STARTPC_METHOD_IS_OVERRIDDEN; }

void *TR_J9VMBase::getStaticFieldAddress(TR_OpaqueClassBlock *clazz, unsigned char *fieldName, uint32_t fieldLen,
    unsigned char *sig, uint32_t sigLen)
{
    TR::VMAccessCriticalSection getStaticFieldAddress(this);
    void *result = vmThread()->javaVM->internalVMFunctions->staticFieldAddress(vmThread(),
        TR::Compiler->cls.convertClassOffsetToClassPtr(clazz), fieldName, fieldLen, sig, sigLen, NULL, NULL,
        J9_LOOK_NO_JAVA, NULL);
    return result;
}

int32_t TR_J9VMBase::getInterpreterVTableSlot(TR_OpaqueMethodBlock *mBlock, TR_OpaqueClassBlock *clazz)
{
    TR::VMAccessCriticalSection getInterpreterVTableSlot(this);
    int32_t result = vmThread()->javaVM->internalVMFunctions->getVTableOffsetForMethod((J9Method *)mBlock,
        (J9Class *)clazz, vmThread());
    return result;
}
...
uint64_t TR_J9VMBase::getUSecClock()
{
    PORT_ACCESS_FROM_JITCONFIG(_jitConfig);
    return j9time_usec_clock();
}

uint64_t TR_J9VMBase::getHighResClock()
{
    PORT_ACCESS_FROM_JITCONFIG(_jitConfig);
    return j9time_hires_clock();
}

uint64_t TR_J9VMBase::getHighResClockResolution()
{
    PORT_ACCESS_FROM_JITCONFIG(_jitConfig);
    return j9time_hires_frequency();
}

TR_JitPrivateConfig *TR_J9VMBase::getPrivateConfig() { return (TR_JitPrivateConfig *)_jitConfig->privateConfig; }

TR_JitPrivateConfig *TR_J9VMBase::getPrivateConfig(void *jitConfig)
{
    return (TR_JitPrivateConfig *)((J9JITConfig *)jitConfig)->privateConfig;
}

void TR_J9VMBase::revertToInterpreted(TR_OpaqueMethodBlock *method) { revertMethodToInterpreted((J9Method *)method); }

int32_t *TR_J9VMBase::getStringClassEnableCompressionFieldAddr(TR::Compilation *comp, bool isVettedForAOT)
{
    TR_ASSERT_FATAL(!comp->compileRelocatableCode() || comp->reloRuntime()->isRelocating(),
        "Function cannot be called during AOT method compilation");
    if (!TR_J9VMBase::staticStringEnableCompressionFieldAddr) // Not yet cached
    {
        ...
        if (stringClass) {
            TR_PersistentClassInfo *classInfo = (comp->getPersistentInfo()->getPersistentCHTable() == NULL)
                ? NULL
                : comp->getPersistentInfo()->getPersistentCHTable()->findClassInfoAfterLocking(stringClass, comp,
                      isVettedForAOT);
            // Since this method should only be called during relocation (and not compilation), we pass false to
            // isInitialized here so that a relo record is not created for this query.
...
static TrustedMethod trustedMethods[] = {
    TR::java_util_Vector_contains, 1, TR::unknownMethod, -1 // Mark end of trusted methods
};

static TrustedMethod untrustedMethods[] = {
    TR::java_util_Vector_subList, -1, TR::unknownMethod, -1 // Mark end of untrusted methods
};

bool TR_J9VMBase::argumentCanEscapeMethodCall(TR::MethodSymbol *method, int32_t argIndex)
{
    ...
    for (i = 0; trustedClasses[i].name && i < numberOfTrustedClasses; ++i) {
        if (nameLength == trustedClasses[i].length && !strncmp(className, trustedClasses[i].name, nameLength)) {
            if (trustedClasses[i].argNum < 0 || trustedClasses[i].argNum == argIndex) {
                if (methodId == TR::unknownMethod)
                    return false;

                for (i = 0; untrustedMethods[i].method != TR::unknownMethod; ++i) {
                    if (untrustedMethods[i].method == methodId) {
                        if (untrustedMethods[i].argNum < 0 || untrustedMethods[i].argNum == argIndex) {
                            return true;
                        }
                    }
                }
    ...
    for (i = 0; trustedMethods[i].method != TR::unknownMethod; ++i) {
        if (trustedMethods[i].method == methodId) {
            if (trustedMethods[i].argNum < 0 || trustedMethods[i].argNum == argIndex) {
                return false;
            }
        }
    }
...
bool TR_J9VMBase::isThunkArchetype(J9Method *method)
{
#if defined(J9VM_OPT_METHOD_HANDLE)
    ...
    if (_J9ROMMETHOD_J9MODIFIER_IS_SET(romMethod, J9AccMethodFrameIteratorSkip)) {
        ...
        bool isInJLI = J9UTF8_LENGTH(classUTF8) >= strlen(jliPrefix)
        ...
        bool isThunkArchetype = J9UTF8_LENGTH(nameUTF8) >= strlen(thunkArchetypePrefix)
...
TR_OpaqueClassBlock *TR_J9VMBase::getHostClass(TR_OpaqueClassBlock *clazzOffset)
{
...
bool TR_J9VMBase::canAllocateInlineClass(TR_OpaqueClassBlock *clazzOffset)
{
    ...
    // Cannot inline the allocation if the class is an interface or abstract.
    if (clazz->romClass->modifiers & (J9AccAbstract | J9AccInterface))
        return false;
    return true;
}

bool TR_J9VMBase::isClassLoadedBySystemClassLoader(TR_OpaqueClassBlock *clazz)
{
    return getSystemClassLoader() == getClassLoader(clazz);
}

intptr_t TR_J9VMBase::getVFTEntry(TR_OpaqueClassBlock *clazz, int32_t offset)
{
    if (isInterfaceClass(clazz))
        return 0; // no VFT
    ...
    uint32_t index = static_cast<uint32_t>(offset - fixedPartOfOffset) / sizeof(uintptr_t);
...
TR_OpaqueClassBlock *TR_J9VMBase::convertClassPtrToClassOffset(J9Class *clazzPtr)
{
...
J9Method *TR_J9VMBase::convertMethodOffsetToMethodPtr(TR_OpaqueMethodBlock *methodOffset)
{
    return J9JitMemory::convertMethodOffsetToMethodPtr(methodOffset);
}

TR_OpaqueMethodBlock *TR_J9VMBase::convertMethodPtrToMethodOffset(J9Method *methodPtr)
{
    return J9JitMemory::convertMethodPtrToMethodOffset(methodPtr);
}

const char *TR_J9VMBase::getJ9MonitorName(J9ThreadMonitor *monitor) { return monitor->name; }

TR_OpaqueClassBlock *TR_J9VMBase::getClassFromNewArrayType(int32_t arrayType)
{
...
TR_OpaqueClassBlock *TR_J9VMBase::getClassFromNewArrayTypeNonNull(int32_t arrayType)
{
...
bool TR_J9VMBase::getReportByteCodeInfoAtCatchBlock()
{
    return _compInfoPT->getCompilation()->getOptions()->getReportByteCodeInfoAtCatchBlock();
}

TR_OpaqueClassBlock *TR_J9VMBase::getClassFromCP(J9ConstantPool *cp)
{
    return reinterpret_cast<TR_OpaqueClassBlock *>(J9_CLASS_FROM_CP(cp));
}

J9ROMMethod *TR_J9VMBase::getROMMethodFromRAMMethod(J9Method *ramMethod)
{
...
// d169771 [2177]
uintptr_t TR_J9VMBase::getPersistentClassPointerFromClassPointer(TR_OpaqueClassBlock *clazz)
{
    return (uintptr_t)TR::Compiler->cls.romClassOf(clazz);
}

void *TR_J9VMBase::getClassLoader(TR_OpaqueClassBlock *classPointer)
{
...
void *TR_J9VMBase::getLocationOfClassLoaderObjectPointer(TR_OpaqueClassBlock *classPointer)
{
    return (void *)&(
        TMP_J9CLASSLOADER_CLASSLOADEROBJECT(TR::Compiler->cls.convertClassOffsetToClassPtr(classPointer)->classLoader));
}

int32_t TR_J9VMBase::maxInternalPlusPinningArrayPointers(TR::Compilation *comp)
{
...
TR_OpaqueClassBlock *TR_J9VM::getClassOfMethod(TR_OpaqueMethodBlock *method)
{
    return (TR_OpaqueClassBlock *)J9_CLASS_FROM_METHOD(((J9Method *)method));
}

I_32 TR_J9VMBase::getLocalObjectAlignmentInBytes() { return 1 << TR::Compiler->om.compressedReferenceShift(); }

I_32 TR_J9VM::getObjectAlignmentInBytes()
{
    J9JavaVM *jvm = _jitConfig->javaVM;
    if (!jvm)
        return 0;
    ...
    uintptr_t result = 0;
    result = mmf->j9gc_modron_getConfigurationValueForKey(jvm, j9gc_modron_configuration_objectAlignment, &result)
        ? result
        : 0;
    return (I_32)result;
}

TR_ResolvedMethod *TR_J9VM::getObjectNewInstanceImplMethod(TR_Memory *trMemory)
{
...
TR_OpaqueMethodBlock *TR_J9VM::getObjectNewInstanceImplMethod()
{
    ...
    J9Class *jlObject = intFunc->internalFindKnownClass(vmThread(), J9VMCONSTANTPOOL_JAVALANGOBJECT,
        J9_FINDKNOWNCLASS_FLAG_EXISTING_ONLY);
    protoMethod = (J9Method *)intFunc->javaLookupMethod(vmThread(), jlObject,
        (J9ROMNameAndSignature *)&newInstancePrototypeNameAndSig, NULL,
...
uintptr_t TR_J9VMBase::getBytecodePC(TR_OpaqueMethodBlock *method, TR_ByteCodeInfo &bcInfo)
{
...
int TR_J9VMBase::findOrCreateMethodSymRef(TR::Compilation *comp, TR::ResolvedMethodSymbol *owningMethodSym,
    const char *classSig, const char **methodSig, TR::SymbolReference **symRefs, int methodCount)
{
...
// This function handles static and virtual functions only.
TR::SymbolReference *TR_J9VMBase::findOrCreateMethodSymRef(TR::Compilation *comp,
    TR::ResolvedMethodSymbol *owningMethodSym, const char *methodSig)
{
...
// this function handles static and virtual functions only
int TR_J9VMBase::findOrCreateMethodSymRef(TR::Compilation *comp, TR::ResolvedMethodSymbol *owningMethodSym,
    const char **methodSig, TR::SymbolReference **symRefs, int methodCount)
{
    int numMethodsFound = 0;
    for (int i = 0; i < methodCount; i++) {
        if (!methodSig[i])
...
bool TR_J9VMBase::isOwnableSyncClass(TR_OpaqueClassBlock *clazz)
{
    J9Class *j9class = TR::Compiler->cls.convertClassOffsetToClassPtr(clazz);
    return ((J9CLASS_FLAGS(j9class) & J9AccClassOwnableSynchronizer) != 0);
}

bool TR_J9VMBase::isContinuationClass(TR_OpaqueClassBlock *clazz)
{
    J9Class *j9class = TR::Compiler->cls.convertClassOffsetToClassPtr(clazz);
...
TR_ResolvedMethod *TR_J9VMBase::getResolvedMethodForNameAndSignature(TR_Memory *trMemory,
...
uint32_t TR_J9VMBase::getNumMethods(TR_OpaqueClassBlock *classPointer)
{
 ...
 */
uintptr_t TR_J9VMBase::getMethodIndexInClass(TR_OpaqueClassBlock *classPointer, TR_OpaqueMethodBlock *methodPointer)
{
    ...
    TR_ASSERT_FATAL(methodIndex < numMethods,
        "methodIndex %llx greater than numMethods %llx for method %p in class %p\n", static_cast<uint64_t>(methodIndex),
        static_cast<uint64_t>(numMethods), methodPointer, classPointer);

    return methodIndex;
}

uint32_t TR_J9VMBase::getNumInnerClasses(TR_OpaqueClassBlock *classPointer)
{
    return TR::Compiler->cls.romClassOf(classPointer)->innerClassCount;
}

uint32_t TR_J9VMBase::getInstanceFieldOffsetIncludingHeader(const char *classSignature, const char *fieldName,
    const char *fieldSig, TR_ResolvedMethod *method)
{
...
TR_OpaqueClassBlock *TR_J9VMBase::getProfiledClassFromProfiledInfo(TR_ExtraAddressInfo *profiledInfo)
{
    return (TR_OpaqueClassBlock *)(profiledInfo->_value);
}

uint32_t TR_J9VMBase::getInstanceFieldOffset(TR_OpaqueClassBlock *clazz, const char *fieldName, uint32_t fieldLen,
    const char *sig, uint32_t sigLen, UDATA options)
{
    ...
    uint32_t result = (uint32_t)vmThread()->javaVM->internalVMFunctions->instanceFieldOffset(vmThread(),
        (J9Class *)clazz, (unsigned char *)fieldName, fieldLen, (unsigned char *)sig, sigLen, (J9Class **)&clazz,
        (UDATA *)NULL, options);
...
TR_OpaqueClassBlock *TR_J9VM::getSuperClass(TR_OpaqueClassBlock *classPointer)
{
...
bool TR_J9VM::isSameOrSuperClass(J9Class *superClass, J9Class *subClass)
{
    return VM_VMHelpers::isSameOrSuperclass(superClass, subClass);
}

bool TR_J9VMBase::sameClassLoaders(TR_OpaqueClassBlock *class1, TR_OpaqueClassBlock *class2)
{
    return (getClassLoader(class1) == getClassLoader(class2));
}

bool TR_J9VM::isUnloadAssumptionRequired(TR_OpaqueClassBlock *clazzPointer, TR_ResolvedMethod *methodBeingCompiled)
{
    TR_OpaqueClassBlock *classOfMethod = methodBeingCompiled->classOfMethod();

    if (clazzPointer == classOfMethod
        || ((getClassLoader(clazzPointer) == getSystemClassLoader() || sameClassLoaders(clazzPointer, classOfMethod))
            && !isAnonymousClass(clazzPointer)))
        return false;

    return true;
}

bool TR_J9VM::classHasBeenExtended(TR_OpaqueClassBlock *clazzPointer)
{
    return (J9CLASS_FLAGS(TR::Compiler->cls.convertClassOffsetToClassPtr(clazzPointer)) & J9AccClassHasBeenOverridden)
        != 0;
}

bool TR_J9VM::classHasBeenReplaced(TR_OpaqueClassBlock *clazzPointer)
{
    return (J9CLASS_FLAGS(TR::Compiler->cls.convertClassOffsetToClassPtr(clazzPointer)) & J9AccClassHotSwappedOut) != 0;
}

bool TR_J9VMBase::isGetImplInliningSupported() { return isGetImplAndRefersToInliningSupported(); }

bool TR_J9VMBase::isGetImplAndRefersToInliningSupported()
{
    J9JavaVM *jvm = _jitConfig->javaVM;
    return jvm->memoryManagerFunctions->j9gc_modron_isFeatureSupported(jvm, j9gc_modron_feature_inline_reference_get)
        != 0;
}
 ...
 *
 *  \note
 *     Value should not be inspected by the JIT but simply returned, capability queries should be used to extract bits
 * from the field.
 */
bool TR_J9VMBase::javaLangClassGetModifiersImpl(TR_OpaqueClassBlock *clazzPointer, int32_t &result)
{
 ...
 *  \return
 *     The hash code of clazzPointer's java/lang/Class object if the query is successful (indicated by
 hashCodeComputed), otherwise return 0.
 */
int32_t TR_J9VMBase::getJavaLangClassHashCode(TR::Compilation *comp, TR_OpaqueClassBlock *clazzPointer,
...
bool TR_J9VMBase::isInterfaceClass(TR_OpaqueClassBlock *clazzPointer)
{
    return (TR::Compiler->cls.romClassOf(clazzPointer)->modifiers & J9AccInterface) ? true : false;
}

bool TR_J9VMBase::isAbstractClass(TR_OpaqueClassBlock *clazzPointer)
{
    return ((TR::Compiler->cls.romClassOf(clazzPointer)->modifiers & (J9AccInterface | J9AccAbstract)) == J9AccAbstract)
        ? true
        : false;
}

bool TR_J9VMBase::isConcreteClass(TR_OpaqueClassBlock *clazzPointer)
{
    return (TR::Compiler->cls.romClassOf(clazzPointer)->modifiers & (J9AccAbstract | J9AccInterface)) ? false : true;
}

bool TR_J9VM::isPublicClass(TR_OpaqueClassBlock *clazz)
{
    return (TR::Compiler->cls.romClassOf(clazz)->modifiers & J9AccPublic) ? true : false;
}

bool TR_J9VMBase::hasFinalizer(TR_OpaqueClassBlock *classPointer)
{
    return (J9CLASS_FLAGS(TR::Compiler->cls.convertClassOffsetToClassPtr(classPointer))
               & (J9AccClassFinalizeNeeded | J9AccClassOwnableSynchronizer | J9AccClassContinuation))
        != 0;
}

uintptr_t TR_J9VMBase::getClassDepthAndFlagsValue(TR_OpaqueClassBlock *classPointer)
{
    return (TR::Compiler->cls.convertClassOffsetToClassPtr(classPointer)->classDepthAndFlags);
}

uintptr_t TR_J9VMBase::getClassFlagsValue(TR_OpaqueClassBlock *classPointer)
{
...
TR_OpaqueMethodBlock *TR_J9VM::getMethodFromName(const char *className, const char *methodName, const char *signature)
{
 ...
 */
TR_OpaqueMethodBlock *TR_J9VMBase::getMethodFromClass(TR_OpaqueClassBlock *methodClass, const char *methodName,
    const char *signature, TR_OpaqueClassBlock *callingClass)
{
...

    {
        ...
        result = (TR_OpaqueMethodBlock *)vmThread()->javaVM->internalVMFunctions->javaLookupMethod(vmThread(),
            (J9Class *)methodClass, (J9ROMNameAndSignature *)&nameAndSig, (J9Class *)callingClass,
            J9_LOOK_JNI | J9_LOOK_NO_JAVA);
    }

    return result;
}

const char *TR_J9VM::sampleSignature(TR_OpaqueMethodBlock *aMethod, char *buf, int32_t bufLen, TR_Memory *memory)
{
    J9UTF8 *className;
    J9UTF8 *name;
    J9UTF8 *signature;
    ...
    if (s)
        snprintf(s, (size_t)len, "%.*s.%.*s%.*s", J9UTF8_LENGTH(className), utf8Data(className), J9UTF8_LENGTH(name),
            utf8Data(name), J9UTF8_LENGTH(signature), utf8Data(signature));
    return s;
}

bool TR_J9VMBase::isPrimitiveClass(TR_OpaqueClassBlock *clazz)
{
    return J9ROMCLASS_IS_PRIMITIVE_TYPE(TR::Compiler->cls.romClassOf(clazz)) ? true : false;
}

bool TR_J9VMBase::isClassInitialized(TR_OpaqueClassBlock *clazz)
{
    if (!clazz)
        return false;
 ...
 */
bool TR_J9VMBase::isClassVisible(TR_OpaqueClassBlock *sourceClass, TR_OpaqueClassBlock *destClass)
{
    ...
    IDATA result = intFunc->checkVisibility(vmThread(), sourceJ9Class, destJ9Class, destJ9Class->romClass->modifiers,
        J9_LOOK_REFLECT_CALL | J9_LOOK_NO_JAVA);
...
TR_OpaqueClassBlock *TR_J9VM::getComponentClassFromArrayClass(TR_OpaqueClassBlock *arrayClass)
{
    return convertClassPtrToClassOffset(
        ((J9ArrayClass *)TR::Compiler->cls.convertClassOffsetToClassPtr(arrayClass))->componentType);
}
...
TR_OpaqueClassBlock *TR_J9VM::getLeafComponentClassFromArrayClass(TR_OpaqueClassBlock *arrayClass)
{
    return convertClassPtrToClassOffset(
        ((J9ArrayClass *)TR::Compiler->cls.convertClassOffsetToClassPtr(arrayClass))->leafComponentType);
}

int32_t TR_J9VM::getNewArrayTypeFromClass(TR_OpaqueClassBlock *clazz)
{
    struct J9Class **arrayClasses = &_jitConfig->javaVM->booleanArrayClass;
...
uint32_t TR_J9VM::getPrimitiveArrayOffsetInJavaVM(uint32_t arrayType)
{
...
bool TR_J9VMBase::isCloneable(TR_OpaqueClassBlock *clazzPointer)
{
    return (J9CLASS_FLAGS(TR::Compiler->cls.convertClassOffsetToClassPtr(clazzPointer)) & J9AccClassCloneable) != 0;
}

bool TR_J9VMBase::isReferenceArray(TR_OpaqueClassBlock *klass)
{
    return isClassArray(klass) && !isPrimitiveArray(klass);
}

TR_OpaqueClassBlock *TR_J9VM::getSystemClassFromClassName(const char *name, int32_t length, bool isVettedForAOT)
{
    ...
    TR_OpaqueClassBlock *result = convertClassPtrToClassOffset(jitGetClassInClassloaderFromUTF8(vmThread(),
        (J9ClassLoader *)vmThread()->javaVM->systemClassLoader, (void *)name, length));
    return result;
}

TR_OpaqueClassBlock *TR_J9VMBase::getByteArrayClass()
{
    return convertClassPtrToClassOffset(_jitConfig->javaVM->byteArrayClass);
}

void *TR_J9VMBase::getSystemClassLoader() { return vmThread()->javaVM->systemClassLoader; }

bool TR_J9VMBase::acquireClassTableMutex()
{
...
void TR_J9VMBase::releaseClassTableMutex(bool releaseVMAccess)
{
    jitReleaseClassTableMutex(vmThread());
    releaseVMAccessIfNeeded(releaseVMAccess);
}

bool TR_J9VMBase::jitFieldsAreSame(TR_ResolvedMethod *method1, I_32 cpIndex1, TR_ResolvedMethod *method2, I_32 cpIndex2,
    int32_t isStatic)
{
    ...
    if (method1->classOfMethod() && method2->classOfMethod()
        && (isHiddenClass(method1->classOfMethod()) || isHiddenClass(method2->classOfMethod()))
    ...
    if (method1->fieldsAreSame(cpIndex1, method2, cpIndex2, sigSame))
        ...
        if (sigSame)
            result = jitFieldsAreIdentical(vmThread(), (J9ConstantPool *)method1->ramConstantPool(), cpIndex1,
                         (J9ConstantPool *)method2->ramConstantPool(), cpIndex2, isStatic)
                != 0;
    }
...
bool TR_J9VMBase::jitStaticsAreSame(TR_ResolvedMethod *method1, I_32 cpIndex1, TR_ResolvedMethod *method2,
    I_32 cpIndex2)
{
    ...
    if (method1->staticsAreSame(cpIndex1, method2, cpIndex2, sigSame))
        ...
        if (sigSame)
            result = jitFieldsAreIdentical(vmThread(), (J9ConstantPool *)method1->ramConstantPool(), cpIndex1,
                         (J9ConstantPool *)method2->ramConstantPool(), cpIndex2, true)
                != 0;
    }
...
TR::CodeCache *TR_J9VM::getResolvedTrampoline(TR::Compilation *comp, TR::CodeCache *curCache, J9Method *method,
    bool inBinaryEncoding)
{
    bool hadClassUnloadMonitor;
    bool hadVMAccess = releaseClassUnloadMonitorAndAcquireVMaccessIfNeeded(comp, &hadClassUnloadMonitor);
            ...
            if (!isAOT_DEPRECATED_DO_NOT_USE()) {
                ...
                if (newCache) {
                    ...
                    if (compInfoPTB->compilationShouldBeInterrupted()) {
                        ...
                        comp->failCompilation<TR::CompilationInterrupted>(
                            "Compilation interrupted in getResolvedTrampoline");
                    } else {
                        int32_t retValue
                            = newCache->reserveResolvedTrampoline((TR_OpaqueMethodBlock *)method, inBinaryEncoding);
                        if (retValue != OMR::CodeCacheErrorCode::ERRORCODE_SUCCESS) {
                            newCache->unreserve(); // delete the reservation
                            newCache = NULL;
...
intptr_t TR_J9VM::methodTrampolineLookup(TR::Compilation *comp, TR::SymbolReference *symRef, void *callSite)
{
...
TR_OpaqueClassBlock *TR_J9VMBase::staticGetBaseComponentClass(TR_OpaqueClassBlock *clazz, int32_t &numDims)
{
...
TR_YesNoMaybe TR_J9VM::isInstanceOf(TR_OpaqueClassBlock *a, TR_OpaqueClassBlock *b, bool objectTypeIsFixed,
    bool castTypeIsFixed, bool optimizeForAOT)
{
    ...
    if (castTypeIsFixed && objectClassIsInstanceOfCastTypeClass)
        ...
        result = TR_no;
    else if (!isInterfaceClass(b) && !isInterfaceClass(a) && !objectClassIsInstanceOfCastTypeClass
...
bool TR_J9VMBase::isPrimitiveArray(TR_OpaqueClassBlock *klass)
{
...
TR_OpaqueClassBlock *TR_J9VM::getClassFromSignature(const char *sig, int32_t sigLength, TR_ResolvedMethod *method,
    bool isVettedForAOT)
{
    return getClassFromSignature(sig, sigLength, (TR_OpaqueMethodBlock *)method->getPersistentIdentifier(),
        isVettedForAOT);
}

TR_OpaqueClassBlock *TR_J9VM::getClassFromSignature(const char *sig, int32_t sigLength, TR_OpaqueMethodBlock *method,
    bool isVettedForAOT)
{
...
TR_OpaqueClassBlock *TR_J9VM::getClassFromSignature(const char *sig, int32_t sigLength, TR_OpaqueClassBlock *clazz,
    bool isVettedForAOT)
{
...
TR_OpaqueClassBlock *TR_J9VM::getClassFromSignature(const char *sig, int32_t sigLength, J9ConstantPool *constantPool,
    bool isVettedForAOT)
{
...
uint32_t TR_J9VMBase::numInterfacesImplemented(J9Class *clazz)
{
    uint32_t count = 0;
...
TR_EstimateCodeSize *TR_J9VMBase::getCodeEstimator(TR::Compilation *comp)
{
    return new (comp->allocator()) TR_J9EstimateCodeSize();
}

void TR_J9VMBase::releaseCodeEstimator(TR::Compilation *comp, TR_EstimateCodeSize *estimator)
{
    comp->allocator().deallocate(estimator, sizeof(TR_J9EstimateCodeSize));
}

void TR_J9VM::transformJavaLangClassIsArray(TR::Compilation *comp, TR::Node *callNode, TR::TreeTop *treeTop)
{
    ...
    TR::Node *vftLoad = TR::Node::createWithSymRef(callNode, TR::aloadi, 1, jlClass,
        comp->getSymRefTab()->findOrCreateClassFromJavaLangClassSymbolRef());

    classFlagNode = testIsClassArrayType(vftLoad);

    // Decrement the ref count of jlClass since the call is going to be transmuted and its first child is not needed
    // anymore
...
void TR_J9VM::transformJavaLangClassIsArrayOrIsPrimitive(TR::Compilation *comp, TR::Node *callNode,
    TR::TreeTop *treeTop, int32_t andMask)
{
    ...
    vftFieldInd = TR::Node::createWithSymRef(TR::aloadi, 1, 1, vftField,
        comp->getSymRefTab()->findOrCreateClassFromJavaLangClassSymbolRef());
    isArrayField
        = TR::Node::createWithSymRef(TR::aloadi, 1, 1, vftFieldInd, symRefTab->findOrCreateClassRomPtrSymbolRef());
    ...
    if (treeTop->getNode()->getOpCode().isNullCheck()) {
        ...
        TR::ResolvedMethodSymbol *owningMethodSymbol
            = treeTop->getNode()->getSymbolReference()->getOwningMethodSymbol(comp);
        TR::TreeTop::create(comp, treeTop->getPrevTreeTop(),
            TR::Node::createWithSymRef(TR::NULLCHK, 1, 1, vftFieldInd,
                symRefTab->findOrCreateNullCheckSymbolRef(owningMethodSymbol)));
    }

    TR::Node::recreate(callNode, TR::idiv);
    callNode->setNumChildren(2);

    isArrayField = TR::Node::createWithSymRef(TR::iloadi, 1, 1, isArrayField,
...
TR::Node *TR_J9VM::inlineNativeCall(TR::Compilation *comp, TR::TreeTop *callNodeTreeTop, TR::Node *callNode)
{
    OMR::Logger *log = comp->log();
    ...
    //
    TR::RecognizedMethod mandatoryMethodID
        = callNode->getSymbol()->castToResolvedMethodSymbol()->getMandatoryRecognizedMethod();
    switch (mandatoryMethodID) {
        ...
        case TR::java_lang_invoke_ComputedCalls_dispatchJ9Method: {
            ...
            TR::SymbolReference *helperSymRef
                = comp->getSymRefTab()->findOrCreateRuntimeHelper(TR_j2iTransition, true, true, false);
    ...
    switch (methodID) {
        ...
        case TR::java_lang_Class_isPrimitive:
            transformJavaLangClassIsArrayOrIsPrimitive(comp, callNode, callNodeTreeTop,
                J9AccClassInternalPrimitiveType);
            return callNode;
        case TR::java_lang_Class_isInstance:
            if (comp->cg()->supportsInliningOfIsInstance()
                && performTransformation(comp,
                    "O^O inlineNativeCall: generate instanceof [%p] for Class.isInstance at bytecode %d\n", callNode,
                ...
                if (comp->getOption(TR_TraceILGen)) {
                    ...
                    log->prints("   /--- Class.isInstance call tree --\n");

                    comp->getDebug()->saveNodeChecklist(nodeChecklistBeforeDump);
                    comp->getDebug()->dumpSingleTreeWithInstrs(log, callNodeTreeTop, NULL, true, false, true, false);
        ...
        case TR::java_lang_Object_getClass:
            ...
            callNode = TR::Node::createWithSymRef(TR::aloadi, 1, 1, callNode,
                comp->getSymRefTab()->findOrCreateJavaLangClassFromClassSymbolRef());
        ...
        case TR::java_lang_Class_getStackClass: {
            ...
            if (stackIterator == stackIndex) {
                J9Class *clazz = callerIndex == -1
                    ? reinterpret_cast<J9Class *>(comp->getJittedMethodSymbol()->getResolvedMethod()->classOfMethod())
                ...
                if (performTransformation(comp,
                        "O^O inlineNativeCall: Optimize getStackClass call [%p] with loading Class object of %.*s at "
                        "bytecode %d\n",
                        callNode, classNameLength, className, callNode->getByteCodeInfo().getByteCodeIndex())) {
                    ...
                    TR::SymbolReference *callerClassSymRef = comp->getSymRefTab()->findOrCreateClassSymbol(
                        comp->getMethodSymbol(), -1, convertClassPtrToClassOffset(clazz));
        ...
        case TR::sun_reflect_Reflection_getCallerClass: {
            ...
            while (true) {
                if (callerIndex == -1) {
                    ...
                    callerMethod = reinterpret_cast<J9Method *>(
                        callerMethodSymbol->getResolvedMethod()->getPersistentIdentifier());
            ...
            if (!skipFrameAtDepth0 && targetInlineDepth == 0) {
                ...
                if (performTransformation(comp,
                    ...
                    callNode->removeAllChildren();
                    TR::SymbolReference *callerClassSymRef = comp->getSymRefTab()->findOrCreateClassSymbol(
                        comp->getMethodSymbol(), -1, convertClassPtrToClassOffset(callerClass));
                    callNode->setSymbolReference(callerClassSymRef);
                    callNode = TR::Node::createWithSymRef(TR::aloadi, 1, 1, callNode,
                        comp->getSymRefTab()->findOrCreateJavaLangClassFromClassSymbolRef());
                }
        ...
        case TR::java_lang_Thread_currentThread: {
            ...
            if (comp->cg()->getGRACompleted()) {
                ...
                if (comp->getOption(TR_TraceOptDetails) || comp->getOption(TR_TraceILGen))
                    log->printf("Inline Thread.currentThread() callNode n%dn 0x%p\n", callNode->getGlobalIndex(),
                        callNode);
        ...
        case TR::java_lang_ref_Reference_refersTo: {
            if (comp->getGetImplAndRefersToInlineable()) {
                ...
                int32_t offset = getInstanceFieldOffset(ReferenceClass, REFERENCEFIELD, REFERENCEFIELDLEN,
                    REFERENCERETURNTYPE, REFERENCERETURNTYPELEN, J9_RESOLVE_FLAG_INIT_CLASS);
                ...
                TR::SymbolReference *symRefField
                    = comp->getSymRefTab()->findOrFabricateShadowSymbol(ReferenceClass, TR::Address, offset,
                        /* isVolatile */ false,
                        /* isPrivate */ true,
                        /* isFinal */ false, REFERENCEFIELD, REFERENCERETURNTYPE);
                ...
                if (methodID == TR::java_lang_ref_Reference_getImpl) {
                    ...
                    TR::Node *referentLoadNode = TR::Node::createWithSymRef(comp->il.opCodeForIndirectLoad(TR::Address),
                        1, 1, thisNode, symRefField);
        ...
        case TR::java_lang_J9VMInternals_rawNewInstance: {
            ...
            callNode->setSymbolReference(comp->getSymRefTab()->findOrCreateNewObjectSymbolRef(
                callNode->getSymbol()->castToResolvedMethodSymbol()));
            TR::Node *newNode = TR::Node::createWithSymRef(callNode, TR::aloadi, 1,
                comp->getSymRefTab()->findOrCreateClassFromJavaLangClassAsPrimitiveSymbolRef());
        ...
        case TR::com_ibm_jit_JITHelpers_is32Bit: {
            // instance method so receiver object is first child, and other nodes may hold onto it so need to decrement
            // its ref count following code hammers this call node into an iconst which disconnects the child
        ...
        case TR::com_ibm_oti_vm_ORBVMHelpers_is32Bit: {
            ...
            if (callNodeTreeTop && callNodeTreeTop->getNode()->getOpCode().isResolveOrNullCheck())
            ...
            if (jitHelpersClass && isClassInitialized(jitHelpersClass)) {
                ...
                if (getSuperclassSymRef && getHelpersSymRef) {
                    ...
                    TR::Node *helpersCallNode
                        = TR::Node::createWithSymRef(callNode, method->directCallOpCode(), 0, getHelpersSymRef);
                    TR::TreeTop *helpersCallTT
                        = TR::TreeTop::create(comp, TR::Node::create(TR::treetop, 1, helpersCallNode));
                    callNodeTreeTop->insertBefore(helpersCallTT);

                    // TR::Node *vftLoad = TR::Node::create(TR::aloadi, 1, helpersCallNode,
                    // comp->getSymRefTab()->findOrCreateVftSymbolRef());
...
int32_t TR_J9VMBase::adjustedInliningWeightBasedOnArgument(int32_t origWeight, TR::Node *argNode,
    TR::ParameterSymbol *parmSymbol, TR::Compilation *comp)
{
...
bool TR_J9VMBase::canAllowDifferingNumberOrTypesOfArgsAndParmsInInliner() { return false; }

void TR_J9VMBase::setInlineThresholds(TR::Compilation *comp, int32_t &callerWeightLimit,
    int32_t &maxRecursiveCallByteCodeSizeEstimate, int32_t &methodByteCodeSizeThreshold,
    int32_t &methodInWarmBlockByteCodeSizeThreshold, int32_t &methodInColdBlockByteCodeSizeThreshold,
    int32_t &nodeCountThreshold, int32_t size)
{
    ...
    if (comp->isServerInlining()) {
        maxRecursiveCallByteCodeSizeEstimate = (int)((float)maxRecursiveCallByteCodeSizeEstimate
            * (float)((float)(_adjustSizeBoundary) / (float)(size)));
        if (maxRecursiveCallByteCodeSizeEstimate > _adjustMaxCGCutOff)
            maxRecursiveCallByteCodeSizeEstimate = _adjustMaxCGCutOff;
    }
    ...
    if (comp->isServerInlining()) {
        methodInWarmBlockByteCodeSizeThreshold
            = (int)((float)150 * (float)((float)(_adjustSizeBoundary) / (float)(size)));
...
bool TR_J9VMBase::isStringCompressionEnabledVM() { return IS_STRING_COMPRESSION_ENABLED_VM(getJ9JITConfig()->javaVM); }

bool TR_J9VMBase::isPortableSCCEnabled()
{
    return (J9_ARE_ANY_BITS_SET(getJ9JITConfig()->javaVM->extendedRuntimeFlags2,
        J9_EXTENDED_RUNTIME2_ENABLE_PORTABLE_SHARED_CACHE));
}

void *TR_J9VMBase::getInvokeExactThunkHelperAddress(TR::Compilation *comp, TR::SymbolReference *glueSymRef,
    TR::DataType dataType)
{
    return glueSymRef->getMethodAddress();
}

TR_OpaqueClassBlock *TR_J9VM::getClassClassPointer(TR_OpaqueClassBlock *objectClassPointer)
{
    ...
    if (TR::Compiler->om.generateCompressedObjectHeaders())
        j9class = (J9Class *)(uintptr_t)
            * ((uint32_t *)((uintptr_t)javaLangClass + (uintptr_t)TR::Compiler->om.offsetOfObjectVftField()));
    else
        j9class = (J9Class *)(*(
            (J9Class **)((uintptr_t)javaLangClass + (uintptr_t)TR::Compiler->om.offsetOfObjectVftField())));
...
TR_OpaqueClassBlock *TR_J9VM::getClassFromMethodBlock(TR_OpaqueMethodBlock *mb)
{
...
// interpreter profiling support
TR_IProfiler *TR_J9VM::getIProfiler()
{
    if (!_iProfiler) {
        // This used to use a global variable called 'jitConfig' instead of the local object's '_jitConfig'.  In early
        // out of memory scenarios, the global jitConfig may be NULL.
...
// HWProfiler support
void TR_J9VM::createHWProfilerRecords(TR::Compilation *comp)
{
    if (comp->getPersistentInfo()->isRuntimeInstrumentationEnabled())
        _compInfo->getHWProfiler()->createRecords(comp);
}

TR_OpaqueClassBlock *TR_J9VM::getClassOffsetForAllocationInlining(J9Class *clazzPtr)
{
    return convertClassPtrToClassOffset(clazzPtr);
}

J9Class *TR_J9VM::getClassForAllocationInlining(TR::Compilation *comp, TR::SymbolReference *classSymRef)
{
    if (!classSymRef->isUnresolved())
        return TR::Compiler->cls.convertClassOffsetToClassPtr(
            (TR_OpaqueClassBlock *)classSymRef->getSymbol()->getStaticSymbol()->getStaticAddress());

    return NULL;
}

uint32_t TR_J9VMBase::getAllocationSize(TR::StaticSymbol *classSym, TR_OpaqueClassBlock *opaqueClazz)
{
...
bool TR_J9VM::supportAllocationInlining(TR::Compilation *comp, TR::Node *node) { return true; }

TR_OpaqueClassBlock *TR_J9VM::getPrimitiveArrayAllocationClass(J9Class *clazz) { return (TR_OpaqueClassBlock *)clazz; }

TR_StaticFinalData TR_J9VM::dereferenceStaticFinalAddress(void *staticAddress, TR::DataType addressType)
{
...
bool TR_J9SharedCacheVM::isClassVisible(TR_OpaqueClassBlock *sourceClass, TR_OpaqueClassBlock *destClass)
{
    TR::Compilation *comp = _compInfoPT->getCompilation();
    ...
    if (comp->getOption(TR_UseSymbolValidationManager)) {
        ...
        validated = ((TR_ResolvedRelocatableJ9Method *)comp->getCurrentMethod())
                        ->validateArbitraryClass(comp, (J9Class *)sourceClass)
            && ((TR_ResolvedRelocatableJ9Method *)comp->getCurrentMethod())
                   ->validateArbitraryClass(comp, (J9Class *)destClass);
    }
...
bool TR_J9SharedCacheVM::isMethodTracingEnabled(TR_OpaqueMethodBlock *method)
{
    // We want to return the same answer as TR_J9VMBase unless we want to force it to allow tracing
    return TR_J9VMBase::isMethodTracingEnabled(method)
        || TR::Options::getAOTCmdLineOptions()->getOption(TR_EnableAOTMethodEnter)
        || TR::Options::getAOTCmdLineOptions()->getOption(TR_EnableAOTMethodExit);
}

bool TR_J9SharedCacheVM::traceableMethodsCanBeInlined() { return true; }

bool TR_J9SharedCacheVM::canMethodEnterEventBeHooked()
{
    // We want to return the same answer as TR_J9VMBase unless we want to force it to allow tracing
...
bool TR_J9SharedCacheVM::canMethodExitEventBeHooked()
{
    // We want to return the same answer as TR_J9VMBase unless we want to force it to allow tracing
...
bool TR_J9SharedCacheVM::methodsCanBeInlinedEvenIfEventHooksEnabled(TR::Compilation *comp)
{
    return !comp->getOption(TR_FullSpeedDebug);
}

int32_t TR_J9SharedCacheVM::getJavaLangClassHashCode(TR::Compilation *comp, TR_OpaqueClassBlock *clazzPointer,
    bool &hashCodeComputed)
{
    hashCodeComputed = false;
    return 0;
...
bool TR_J9SharedCacheVM::javaLangClassGetModifiersImpl(TR_OpaqueClassBlock *clazzPointer, int32_t &result)
{
    return false;
}

uint32_t TR_J9SharedCacheVM::getInstanceFieldOffset(TR_OpaqueClassBlock *classPointer, const char *fieldName,
    uint32_t fieldLen, const char *sig, uint32_t sigLen, UDATA options)
{
    ...
    if (comp->getOption(TR_UseSymbolValidationManager)) {
        ...
        validated = ((TR_ResolvedRelocatableJ9Method *)comp->getCurrentMethod())
                        ->validateArbitraryClass(comp, (J9Class *)classPointer);
    }
...
TR_OpaqueClassBlock *TR_J9SharedCacheVM::getClassOfMethod(TR_OpaqueMethodBlock *method)
{
...
TR_OpaqueClassBlock *TR_J9SharedCacheVM::getSuperClass(TR_OpaqueClassBlock *classPointer)
{
    TR::Compilation *comp = _compInfoPT->getCompilation();
    TR_ASSERT(comp, "Should be called only within a compilation");
...
TR_ResolvedMethod *TR_J9SharedCacheVM::getResolvedMethodForNameAndSignature(TR_Memory *trMemory,
    TR_OpaqueClassBlock *classPointer, const char *methodName, const char *signature)
{
    TR::Compilation *comp = _compInfoPT->getCompilation();
    TR_ASSERT(comp, "Should be called only within a compilation");
    ...
    TR_ResolvedMethod *resolvedMethod
        = TR_J9VM::getResolvedMethodForNameAndSignature(trMemory, classPointer, methodName, signature);
...
bool TR_J9SharedCacheVM::isClassLibraryMethod(TR_OpaqueMethodBlock *method, bool vettedForAOT)
{
    TR_ASSERT(vettedForAOT,
...
TR_OpaqueMethodBlock *TR_J9SharedCacheVM::getMethodFromClass(TR_OpaqueClassBlock *methodClass, const char *methodName,
    const char *signature, TR_OpaqueClassBlock *callingClass)
{
    ...
    if (omb) {
        ...
        if (comp->getOption(TR_UseSymbolValidationManager)) {
            bool validated = comp->getSymbolValidationManager()->addMethodFromClassAndSignatureRecord(omb, methodClass,
                callingClass);
            if (!validated)
                omb = NULL;
        } else {
            if (!((TR_ResolvedRelocatableJ9Method *)comp->getCurrentMethod())
                     ->validateArbitraryClass(comp, (J9Class *)methodClass))
                omb = NULL;
            if (callingClass
                && !((TR_ResolvedRelocatableJ9Method *)comp->getCurrentMethod())
                        ->validateArbitraryClass(comp, (J9Class *)callingClass))
                omb = NULL;
        }
    }
...
bool TR_J9SharedCacheVM::supportAllocationInlining(TR::Compilation *comp, TR::Node *node)
{
    if (comp->getOptions()->realTimeGC())
        return false;

    if ((comp->target().cpu.isX86() || comp->target().cpu.isPower() || comp->target().cpu.isZ()
...
TR_YesNoMaybe TR_J9SharedCacheVM::isInstanceOf(TR_OpaqueClassBlock *a, TR_OpaqueClassBlock *b, bool objectTypeIsFixed,
    bool castTypeIsFixed, bool optimizeForAOT)
{
    TR::Compilation *comp = TR::comp();
    ...
    if (comp && comp->getOption(TR_UseSymbolValidationManager)) {
        if (isAnInstanceOf != TR_maybe)
            validated = comp->getSymbolValidationManager()->addClassInstanceOfClassRecord(a, b, objectTypeIsFixed,
                castTypeIsFixed, (isAnInstanceOf == TR_yes));
    } else {
...
TR_OpaqueClassBlock *TR_J9SharedCacheVM::getClassFromSignature(const char *sig, int32_t sigLength,
    J9ConstantPool *constantPool, bool isVettedForAOT)
{
    ...
    if (j9class) {
        if (comp->getOption(TR_UseSymbolValidationManager))
            ...
            validated = ((TR_ResolvedRelocatableJ9Method *)comp->getCurrentMethod())
                            ->validateArbitraryClass(comp, (J9Class *)j9class);
    }
...
TR_OpaqueClassBlock *TR_J9SharedCacheVM::getSystemClassFromClassName(const char *name, int32_t length,
    bool isVettedForAOT)
{
    TR::Compilation *comp = TR::comp();
    ...
    if (comp && comp->getOption(TR_UseSymbolValidationManager)) {
        ...
        if (isVettedForAOT) {
            if (((TR_ResolvedRelocatableJ9Method *)TR::comp()->getCurrentMethod())
                    ->validateArbitraryClass(comp, (J9Class *)classPointer))
                validated = true;
        }
    }
...
TR_OpaqueClassBlock *TR_J9SharedCacheVM::getProfiledClassFromProfiledInfo(TR_ExtraAddressInfo *profiledInfo)
{
...
bool TR_J9SharedCacheVM::isPublicClass(TR_OpaqueClassBlock *classPointer)
{
    TR::Compilation *comp = _compInfoPT->getCompilation();
    TR_ASSERT(comp, "Should be called only within a compilation");
...
bool TR_J9SharedCacheVM::hasFinalizer(TR_OpaqueClassBlock *classPointer)
{
    TR::Compilation *comp = _compInfoPT->getCompilation();
    TR_ASSERT(comp, "Should be called only within a compilation");
...
uintptr_t TR_J9SharedCacheVM::getClassDepthAndFlagsValue(TR_OpaqueClassBlock *classPointer)
{
    TR::Compilation *comp = _compInfoPT->getCompilation();
    TR_ASSERT(comp, "Should be called only within a compilation");
...
uintptr_t TR_J9SharedCacheVM::getClassFlagsValue(TR_OpaqueClassBlock *classPointer)
{
    TR::Compilation *comp = _compInfoPT->getCompilation();
    TR_ASSERT(comp, "Should be called only within a compilation");
...
bool TR_J9SharedCacheVM::isPrimitiveClass(TR_OpaqueClassBlock *classPointer)
{
    TR::Compilation *comp = _compInfoPT->getCompilation();
    TR_ASSERT(comp, "Should be called only within a compilation");
...
TR_OpaqueClassBlock *TR_J9SharedCacheVM::getComponentClassFromArrayClass(TR_OpaqueClassBlock *arrayClass)
{
    TR::Compilation *comp = _compInfoPT->getCompilation();
    TR_ASSERT(comp, "Should be called only within a compilation");
    ...
    if (comp->getOption(TR_UseSymbolValidationManager)) {
        ...
        if (((TR_ResolvedRelocatableJ9Method *)comp->getCurrentMethod())
                ->validateArbitraryClass(comp, (J9Class *)arrayClass))
            validated = true;
    }
...
TR_OpaqueClassBlock *TR_J9SharedCacheVM::getArrayClassFromComponentClass(TR_OpaqueClassBlock *componentClass)
{
    TR::Compilation *comp = _compInfoPT->getCompilation();
    TR_ASSERT(comp, "Should be called only within a compilation");
    ...
    if (comp->getOption(TR_UseSymbolValidationManager)) {
        validated
            = comp->getSymbolValidationManager()->addArrayClassFromComponentClassRecord(arrayClass, componentClass);
    } else {
        if (((TR_ResolvedRelocatableJ9Method *)comp->getCurrentMethod())
                ->validateArbitraryClass(comp, (J9Class *)componentClass))
            validated = true;
    }
...
TR_OpaqueClassBlock *TR_J9SharedCacheVM::getNullRestrictedArrayClassFromComponentClass(
    TR_OpaqueClassBlock *componentClass)
{
    ...
    TR_OpaqueClassBlock *nullRestrictedArrayClass
        = TR_J9VM::getNullRestrictedArrayClassFromComponentClass(componentClass);

    if (comp->getOption(TR_UseSymbolValidationManager)) {
        validated = comp->getSymbolValidationManager()->addArrayClassFromComponentClassRecord(nullRestrictedArrayClass,
            componentClass);
    } else {
        validated = ((TR_ResolvedRelocatableJ9Method *)comp->getCurrentMethod())
                        ->validateArbitraryClass(comp, (J9Class *)componentClass);
...
TR_OpaqueClassBlock *TR_J9SharedCacheVM::getLeafComponentClassFromArrayClass(TR_OpaqueClassBlock *arrayClass)
{
    TR::Compilation *comp = _compInfoPT->getCompilation();
    TR_ASSERT(comp, "Should be called only within a compilation");
...
TR_OpaqueClassBlock *TR_J9SharedCacheVM::getBaseComponentClass(TR_OpaqueClassBlock *classPointer, int32_t &numDims)
{
    TR::Compilation *comp = _compInfoPT->getCompilation();
    TR_ASSERT(comp, "Should be called only within a compilation");
...
TR_OpaqueClassBlock *TR_J9SharedCacheVM::getClassFromNewArrayType(int32_t arrayType)
{
    TR::Compilation *comp = TR::comp();
    if (comp && comp->getOption(TR_UseSymbolValidationManager))
...
bool TR_J9SharedCacheVM::isPrimitiveArray(TR_OpaqueClassBlock *classPointer)
{
    TR::Compilation *comp = _compInfoPT->getCompilation();
    TR_ASSERT(comp, "Should be called only within a compilation");
...
bool TR_J9SharedCacheVM::isReferenceArray(TR_OpaqueClassBlock *classPointer)
{
    TR::Compilation *comp = _compInfoPT->getCompilation();
    TR_ASSERT(comp, "Should be called only within a compilation");
...
TR_OpaqueMethodBlock *TR_J9SharedCacheVM::getInlinedCallSiteMethod(TR_InlinedCallSite *ics) { return ics->_methodInfo; }

// Answer the following conservatively until we know how to do better
bool TR_J9SharedCacheVM::sameClassLoaders(TR_OpaqueClassBlock *class1, TR_OpaqueClassBlock *class2)
{
    // conservative answer: need a relocation to validate
    return false;
}

bool TR_J9SharedCacheVM::isUnloadAssumptionRequired(TR_OpaqueClassBlock *, TR_ResolvedMethod *) { return true; }

bool TR_J9SharedCacheVM::classHasBeenExtended(TR_OpaqueClassBlock *classPointer) { return true; }

TR_ResolvedMethod *TR_J9SharedCacheVM::getObjectNewInstanceImplMethod(TR_Memory *) { return NULL; }

////////////////// Under evaluation

TR::CodeCache *TR_J9SharedCacheVM::getResolvedTrampoline(TR::Compilation *comp, TR::CodeCache *, J9Method *method,
    bool inBinaryEncoding)
{
    return 0;
}

intptr_t TR_J9SharedCacheVM::methodTrampolineLookup(TR::Compilation *comp, TR::SymbolReference *symRef, void *callSite)
{
...
// Multiple codeCache support
TR::CodeCache *TR_J9SharedCacheVM::getDesignatedCodeCache(TR::Compilation *comp)
{
    int32_t numReserved;
    int32_t compThreadID = comp ? comp->getCompThreadID() : -1;
    bool hadClassUnloadMonitor;
...
void *TR_J9SharedCacheVM::getJ2IThunk(char *signatureChars, uint32_t signatureLength, TR::Compilation *comp)
{
    void *thunk = NULL;
#if defined(J9VM_OPT_JITSERVER)
...
void *TR_J9SharedCacheVM::setJ2IThunk(char *signatureChars, uint32_t signatureLength, void *thunkptr,
    TR::Compilation *comp)
{
...
void *TR_J9SharedCacheVM::persistMHJ2IThunk(void *thunk)
{
    ...
    const void *store = _jitConfig->javaVM->sharedClassConfig->storeSharedData(curThread, signatureChars,
        signatureLength, &dataDescriptor);
...
bool TR_J9SharedCacheVM::canRememberClass(TR_OpaqueClassBlock *classPtr)
{
    if (_sharedCache)
        return (_sharedCache->rememberClass((J9Class *)classPtr, NULL, false)
            != TR_SharedCache::INVALID_CLASS_CHAIN_OFFSET);
    return false;
}

bool TR_J9SharedCacheVM::ensureOSRBufferSize(TR::Compilation *comp, uintptr_t osrFrameSizeInBytes,
    uintptr_t osrScratchBufferSizeInBytes, uintptr_t osrStackFrameSizeInBytes)
{
    bool valid = TR_J9VMBase::ensureOSRBufferSize(comp, osrFrameSizeInBytes, osrScratchBufferSizeInBytes,
...
// d169771 [2177]
J9Class *TR_J9SharedCacheVM::getClassForAllocationInlining(TR::Compilation *comp, TR::SymbolReference *classSymRef)
{
    bool returnClassForAOT = true;
    if (!classSymRef->isUnresolved())
        return TR_J9VM::getClassForAllocationInlining(comp, classSymRef);
    else
        return (J9Class *)classSymRef->getOwningMethod(comp)->getClassFromConstantPool(comp, classSymRef->getCPIndex(),
            returnClassForAOT);
}
...
char *TR_J9VMBase::classNameChars(TR::Compilation *comp, TR::SymbolReference *symRef, int32_t &length)
{
...
bool TR_J9VMBase::inSnapshotMode()
{
#if defined(J9VM_OPT_CRIU_SUPPORT)
...
bool TR_J9VMBase::isPortableRestoreModeEnabled()
{
#if defined(J9VM_OPT_CRIU_SUPPORT)
...
bool TR_J9VMBase::isSnapshotModeEnabled()
{
#if defined(J9VM_OPT_CRIU_SUPPORT)
    J9JavaVM *javaVM = getJ9JITConfig()->javaVM;
...
bool TR_J9VMBase::isJ9VMThreadCurrentThreadImmutable()
{
#if JAVA_SPEC_VERSION >= 19
    return false;
#else
...
bool TR_J9VMBase::isLiveMonitorMapCorrectnessRequired()
{
...
jlong JNICALL Java_java_lang_invoke_ThunkTuple_initialInvokeExactThunk(JNIEnv *env, jclass clazz)
{
#if defined(J9ZOS390)
 ...
 */
jint JNICALL Java_java_lang_invoke_InterfaceHandle_convertITableIndexToVTableIndex(JNIEnv *env,
    jclass InterfaceMethodHandle, jlong interfaceArg, jint itableIndex, jlong receiverClassArg)
{
    ...
    for (itableEntry = (J9ITable *)receiverClass->iTable; itableEntry; itableEntry = itableEntry->next)
        if (itableEntry->interfaceClass == interfaceClass)
            break;
    TR_ASSERT(itableEntry,
        "Shouldn't call convertITableIndexToVTableIndex without first ensuring the receiver implements the interface");
...
void JNICALL Java_java_lang_invoke_MutableCallSite_invalidate(JNIEnv *env, jclass MutableCallSite,
    jlongArray cookieArrayObject)
{
    J9VMThread *vmThread = (J9VMThread *)env;
    J9JITConfig *jitConfig = vmThread->javaVM->jitConfig;
    TR_FrontEnd *fe = TR_J9VMBase::get(jitConfig, vmThread);
    TR_RuntimeAssumptionTable *rat
        = TR::CompilationInfo::get(jitConfig)->getPersistentInfo()->getRuntimeAssumptionTable();
    ...
    if (verbose)
        TR_VerboseLog::writeLineLocked(TR_Vlog_HK, "%x hook %s vmThread=%p ", threadID, "MutableCallSite.invalidate",
            vmThread);
    ...
    if (!env->ExceptionCheck()) {
        ...
        for (int32_t i = 0; i < numSites; i++) {
            ...
            if (cookie) {
                if (details)
                    TR_VerboseLog::writeLineLocked(TR_Vlog_HD,
                        "%x     notifying cookies[%3d] " UINT64_PRINTF_FORMAT_HEX, threadID, i, cookie);
        ...
        if (verbose)
            TR_VerboseLog::writeLineLocked(TR_Vlog_HK, "%x hook %s vmThread=%p failed exception check", threadID,
                "MutableCallSite.invalidate", vmThread);
    }
}
...
void TR_VerboseLog::vlogAcquire() { j9jit_lock_vlog(_config); }

void TR_VerboseLog::vlogRelease() { j9jit_unlock_vlog(_config); }
...
{
    if (TR::Options::getCmdLineOptions() && TR::Options::getCmdLineOptions()->isFullyInitialized()
        ...
        omrstr_ftime_ex(timestamp, sizeof(timestamp), "%b-%d-%Y_%H:%M:%S ", j9time_current_time_millis(),
