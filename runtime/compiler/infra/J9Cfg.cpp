...
static TR_PersistentProfileInfo *getProfilingInfoForCFG(TR::Compilation *comp, TR::CFG *cfg)
{
    ...
    if (cfg == comp->getFlowGraph() && comp->getRecompilationInfo())
        return info;

    if ((*(TR_BlockFrequencyInfo::getEnableJProfilingRecompilation())) == -1 && cfg->getMethodSymbol()
        && cfg->getMethodSymbol()->getResolvedMethod() && info && info->getBlockFrequencyInfo()
...
bool J9::CFG::setFrequencies()
{
    ...
    if (externFreq && comp()->hasBlockFrequencyInfo()
        ...
        if (!self()->consumePseudoRandomFrequencies()) {
            ...
            if (this == comp()->getFlowGraph() && comp()->getInlinedCalls() > 0) {
                for (TR::Block *block = comp()->getStartBlock(); block; block = block->getNextBlock()) {
                    ...
                    TR::ResolvedMethodSymbol *inlinedMethod = bci.getCallerIndex() == -1
        ...
        if (!self()->consumePseudoRandomFrequencies()) {
            ...
            if (comp()->getOption(TR_TraceBFGeneration))
                comp()->dumpMethodTrees(comp()->log(), "Trees after setting frequencies from structures",
                    comp()->getMethodSymbol());
        }
...
TR_BitVector *J9::CFG::setBlockAndEdgeFrequenciesBasedOnJITProfiler()
{
    ...
    for (int32_t traversalIndex = 0; traversalIndex < getForwardTraversalLength(); traversalIndex++) {
        ...
        if (frequency < 0) {
            ...
            for (edge = pit.getFirst(); edge; edge = pit.getNext()) {
                ...
                if (lastTree && lastTree->getNode()->getOpCode().isIf() && isVirtualGuard(lastTree->getNode())) {
            ...
            for (edge = pit.getFirst(); edge; edge = pit.getNext()) {
                ...
                if (lastTree && lastTree->getNode()->getOpCode().isIf()) {
                    ...
                    if (fallThrough && branchTarget) {
                        logprintf(trace, log,
                            "      checking effective singleton on block_%d with fallthrough block_%d and branchTarget "
                            "block_%d\n",
                ...
                if (trace && pred)
                    log->printf("      block_%d's pred block_%d has normalized frequency %d and %s\n",
                        node->getNumber(), pred->getNumber(), predRawFrequency,
                        (pred->getSuccessors().size() == 1)
                            ? "one successor"
                            : (effectiveSingleton ? "one effective successor" : "multiple successors"));
                ...
                if (((pred->getSuccessors().size() == 1) && pred->hasSuccessor(node)) || effectiveSingleton) {
                ...
                } else if (pred && pred->hasSuccessor(node)
                    && (frequency < 0
                        || (!blockFrequencyInfo->isJProfilingData() && pred->getFrequency() <= GUESS_THRESHOLD))) {
                    int32_t succCount = 0;
                    for (auto edge = pred->getSuccessors().begin(); edge != pred->getSuccessors().end(); ++edge) {
                        if ((*edge)->getTo() && (*edge)->getTo()->asBlock() && node->asBlock()
                            && (*edge)->getTo()->asBlock() != node->asBlock() && (*edge)->getTo()->getFrequency() > -1)
                    ...
                    if (predRawFrequency > 0) {
                        ...
                        //
                        combinedPredRawFrequency
                            += origMaxFrequency / 4; // probably intended something like denormalizedFrequency(25,100)
            ...
            logprintf(trace, log,
                "Raw frequency for block_%d is %d (maxCount %d origMax %d combinedPredRawFrequency %d)\n",
            ...
            // if (!isGuardedBlock)
            {
                logprintf(trace, log, "   Setting block_%d frequency %d (origMaxFrequency %d)\n", node->getNumber(),
                    frequency, origMaxFrequency);
                if ((frequency > origMaxFrequency) && (origMaxFrequency > -1)) {
                    if (!isGuardedBlock) {
                        if (!nodesToBeNormalized)
                            nodesToBeNormalized
                                = new (trStackMemory()) TR_BitVector(getNextNodeNumber(), trMemory(), stackAlloc);
        ...
        if (frequency >= 0 && (node->getPredecessors().size() == 1)
            && (node->getPredecessors().front()->getFrom()->getSuccessors().size() == 1)) {
    ...
    if (nodesToBeNormalized) {
        for (node = getFirstNode(); node; node = node->getNext()) {
            int32_t frequency = node->getFrequency();
            if (!nodesToBeNormalized->get(node->getNumber()) && !node->asBlock()->isCold()) {
                nodesToBeNormalized->set(node->getNumber());
    ...
    for (node = getFirstNode(); node; node = node->getNext()) {
        ...
        if (frequency >= 0) {
            ...
            for (auto e = node->getSuccessors().begin(); e != node->getSuccessors().end(); ++e) {
                ...
                if ((*e)->getVisitCount() > 1) {
                    ...
                    if (succ->getPredecessors().size() == 2) // succ has only one other predecessor
                    {
                        ...
                        if ((succ->getFrequency() >= 0) && (succ->getFrequency() >= (*e)->getFrequency())) {
                            ...
                            logprintf(trace, log,
                                "\t\tedge(%d->%d) frequency can be set to succ_%d(%d) - e_freq(%d->%d)(%d) = "
                                "ee_freq(%d)\n",
                                ee->getFrom()->getNumber(), ee->getTo()->getNumber(), succ->getNumber(),
                                succ->getFrequency(), (*e)->getFrom()->getNumber(), (*e)->getTo()->getNumber(),
                                (*e)->getFrequency(), ff);
                ...
                // successorFrequency, node->getNumber(), e->getTo()->getNumber());
            }
            ...
            // if (successorFrequency > 0)
            {
                for (auto e = node->getSuccessors().begin(); e != node->getSuccessors().end(); ++e) {
                    // if (e->getFrequency() <= 0)
                    {
                        ...
                        // e->getFrom()->getNumber(), e->getTo()->getNumber(), e->getFrequency(), _maxEdgeFrequency);
                    }
                }
            }
            ...
            if (_externalProfiler && (node->getSuccessors().size() == 2)) {
                ...
                if (frequencyIsIdentical) {
                    ...
                    if (!block->isCold() && block->getEntry() != NULL
                        ...
                        _externalProfiler->getBranchCounters(ifNode, block->getNextBlock()->getEntry(), &branchToCount,
                            &fallThroughCount, comp());
                        ...
                        if (branchToCount > fallThroughCount) {
                            ...
                            for (auto e = node->getSuccessors().begin(); e != node->getSuccessors().end(); ++e) {
                                if ((*e)->getTo()->asBlock() == branchToBlock) {
                                    // dumpOptDetails(comp(), "\tadjusting branch to frequency to %d in order to break
                                    // the tie\n", deFrq+1);
            ...
            for (auto e = node->getPredecessors().begin(); e != node->getPredecessors().end(); ++e) {
                ...
                if ((*e)->getVisitCount() > 1) {
                    ...
                    if (pred->getSuccessors().size() == 2) // pred has only one other successor
                    {
                        ...
                        if ((pred->getFrequency() >= 0) && (pred->getFrequency() >= (*e)->getFrequency())) {
                            int32_t ff = pred->getFrequency() - (*e)->getFrequency();
                            logprintf(trace, log,
                                "\t\tedge(%d->%d) frequency can be set to pred_%d(%d) - e_freq(%d->%d)(%d) = "
                                "ee_freq(%d)\n",
                                ee->getFrom()->getNumber(), ee->getTo()->getNumber(), pred->getNumber(),
                                pred->getFrequency(), (*e)->getFrom()->getNumber(), (*e)->getTo()->getNumber(),
                                (*e)->getFrequency(), ff);
...
void J9::CFG::setBlockFrequenciesBasedOnInterpreterProfiler()
{
    OMR::Logger *log = comp()->log();
    bool trace = comp()->getOption(TR_TraceBFGeneration);
    ...
    TR_BitVector *_seenNodesInCycle
    ...
    //_maxFrequency = 0;
    while ((temp->getSuccessors().size() == 1) || !temp->asBlock()->getEntry()
        ...
        if (!backEdgeExists && !temp->getPredecessors().empty() && !(temp->getPredecessors().size() == 1)) {
            if (comp()->getHCRMode() != TR::osr)
                ...
                for (auto edge = temp->getPredecessors().begin(); edge != temp->getPredecessors().end(); ++edge) {
                    if ((*edge)->getFrom()->asBlock()->isOSRCodeBlock()
                        || (*edge)->getFrom()->asBlock()->isOSRCatchBlock())
                        continue;
                    if (nonOSREdges > 0) {
                        backEdgeExists = true;
                        break;
    ...
    if ((temp->getSuccessors().size() == 2) && temp->asBlock()->getEntry()
        && temp->asBlock()->getLastRealTreeTop()->getNode()->getOpCode().isBranch()
        && !temp->asBlock()->getLastRealTreeTop()->getNode()->getByteCodeInfo().doNotProfile()) {
        ...
        if ((temp->getSuccessors().size() == 2) && (startFrequency > 0) && temp->asBlock()->getEntry()
            && temp->asBlock()->getLastRealTreeTop()->getNode()->getOpCode().isBranch())
    ...
    while (!stack.isEmpty()) {
        ...
        if (node->asBlock()->isCold()) {
            ...
            if ((node->getSuccessors().size() == 2) && (freq > 0) && node->asBlock()->getEntry()
        ...
        if (!node->asBlock()->isCold() && (node != start)) {
            if ((node->getSuccessors().size() == 2)
                ...
                if (!isVirtualGuard(node->asBlock()->getLastRealTreeTop()->getNode())
                    ...
                    if (comp()->getOption(TR_TraceBFGeneration))
                        dumpOptDetails(comp(),
                            "If on node %p is not guard I'm using the taken and nottaken counts for producing block "
                            "frequency\n",
                    ...
                    if (predNotSet && !_seenNodesInCycle->get(node->getNumber())) {
                ...
                _seenNodesInCycle->empty();
                int32_t sumFreq
                    = _externalProfiler->getSumSwitchCount(node->asBlock()->getLastRealTreeTop()->getNode(), comp());
                ...
                if (predNotSet && !_seenNodesInCycle->get(node->getNumber())) {
                    ...
                    if ((node->getSuccessors().size() == 2) && (sumFreq > 0) && node->asBlock()->getEntry()
                        && node->asBlock()->getLastRealTreeTop()->getNode()->getOpCode().isBranch())
                        self()->setEdgeFrequenciesOnNode(node, 0, sumFreq, comp());
                    else
            ...
            if (!_seenNodes->isSet(succ->getNumber()))
                ...
                if (!(((succ->getSuccessors().size() == 2)
                    ...
                    if (succ->getSuccessors().size() == 1) {
                        ...
                        TR_BitVector *_seenGotoNodes
                            = new (trStackMemory()) TR_BitVector(numBlocks, trMemory(), stackAlloc, notGrowable);
                        _seenGotoNodes->set(succ->getNumber());
                        while ((tempNode->getSuccessors().size() == 1) && (tempNode != succ) && (tempNode != getEnd())
    ...
    for (node = getFirstNode(); node; node = node->getNext()) {
        ...
        if (node->asBlock()->getEntry() && (node->asBlock()->getFrequency() < 0))
            node->asBlock()->setFrequency(0);
    }

    // scaleEdgeFrequencies();

    TR_BitVector *nodesToBeNormalized
        = new (trStackMemory()) TR_BitVector(numBlocks, trMemory(), stackAlloc, notGrowable);
...
void J9::CFG::computeInitialBlockFrequencyBasedOnExternalProfiler(TR::Compilation *comp)
{
...
void J9::CFG::getInterpreterProfilerBranchCountersOnDoubleton(TR::CFGNode *cfgNode, int32_t *taken, int32_t *nottaken)
{
    TR::Node *node = cfgNode->asBlock()->getLastRealTreeTop()->getNode();

    if (this != comp()->getFlowGraph()) {
        TR::TreeTop *entry
            = (cfgNode->asBlock()->getNextBlock()) ? cfgNode->asBlock()->getNextBlock()->getEntry() : NULL;
    ...
    if (*taken || *nottaken) {
        ...
        *nottaken = AVG_FREQ;
        ...
        if (cfgNode->asBlock()->getNextBlock() && cfgNode->asBlock()->getNextBlock()->isCold())
            *nottaken = 0;
        else
            *nottaken = LOW_FREQ;
        ...
        if (comp()->getOption(TR_TraceBFGeneration))
            dumpOptDetails(comp(),
                "If with no profiling information on node %p has low branch counts: taken=%d, not taken=%d\n", node,
                *taken, *nottaken);
    }
}
...
void J9::CFG::setSwitchEdgeFrequenciesOnNode(TR::CFGNode *node, TR::Compilation *comp)
{
    TR::Block *block = node->asBlock();
    ...
    if (treeNode->getInlinedSiteIndex() < -1) {
        if (comp->getOption(TR_TraceBFGeneration))
            dumpOptDetails(comp,
                "Dummy switch generated in estimate code size I'll set frequencies using uniform edge distribution\n");
    ...
    if (_externalProfiler->isSwitchProfileFlat(treeNode, comp)) {
        ...
        self()->setUniformEdgeFrequenciesOnNode(node, _externalProfiler->getFlatSwitchProfileCounts(treeNode, comp),
            false, comp);
        return;
    }
    ...
    for (int32_t count = 1; count < treeNode->getNumChildren(); count++) {
        ...
        if (comp->getOption(TR_TraceBFGeneration))
            dumpOptDetails(comp, "Edge %p between %d and %d has freq %d (Switch)\n", e, e->getFrom()->getNumber(),
                e->getTo()->getNumber(), e->getFrequency());
    }
}

void J9::CFG::setBlockFrequency(TR::CFGNode *node, int32_t frequency, bool addFrequency)
{
    OMR::Logger *log = comp()->log();
    bool trace = comp()->getOption(TR_TraceBFGeneration);
    TR::Block *block = node->asBlock();
    ...
    if (addFrequency) {
        ...
        logprintf(trace, log, "11 Setting freq %d on block_%d added freq %d\n", block->getFrequency(),
            block->getNumber(), addedFrequency);
    } else {
        // if (frequency > _maxFrequency)
        //    _maxFrequency = frequency;
...
void J9::CFG::getBranchCountersFromProfilingData(TR::Node *node, TR::Block *block, int32_t *taken, int32_t *notTaken)
{
    ...
    if (self() != comp->getFlowGraph())
        ...
        if (Confirmation
            && ((comp->hasBlockFrequencyInfo()
                && (self() == comp->getFlowGraph())) /*|| hasJProfilingInfo(comp, self())*/)) {
            TR_PersistentProfileInfo *profileInfo = getProfilingInfoForCFG(comp, self());
            if (profileInfo) {
                TR_BlockFrequencyInfo *blockFrequencyInfo = profileInfo->getBlockFrequencyInfo();
                // This check ensures that the JIT profiling block frequency can be properly correlated to interpreter
                // profiling edge frequency This can be only done when no other edge may come in to a block and
                // unnaturally inflate one of the block's counts.
                //
                if ((fallThroughBlock->getPredecessors().size() == 1)
                    && (branchToBlock->getPredecessors().size() == 1)) {
                    ...
                    if (currentBlockFreq > 0 && fallthruBlockFreq > 0 && branchBlockFreq > 0) {
                        if ((*taken > *notTaken && fallthruBlockFreq > branchBlockFreq)
                            || (*notTaken > *taken && branchBlockFreq > fallthruBlockFreq)) {
                            logprintf(trace, log,
                                "For block %d fallthru block %d and branch block %d  iprofiler says taken = %d "
                                "notTaken = %d jitprofiler says currentBlockfreq = %d "
                                "taken = %d notTaken = %d. Scaling iprofiler info.\n",
                                block->getNumber(), fallThroughBlock->getNumber(), branchToBlock->getNumber(), *taken,
                                *notTaken, currentBlockFreq, branchBlockFreq, fallthruBlockFreq);
...
//
void J9::CFG::setBlockAndEdgeFrequenciesBasedOnStructure() { self()->propagateFrequencyInfoFromExternalProfiler(NULL); }

void J9::CFG::propagateFrequencyInfoFromExternalProfiler(TR_ExternalProfiler *profiler)
{
    _externalProfiler = profiler;
...
bool J9::CFG::emitVerbosePseudoRandomFrequencies()
{
    TR::CFGNode *nextNode = getFirstNode();
    int32_t count = 1;
    int32_t edgeIndex = 0;
