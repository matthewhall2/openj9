...
TR::MonitorTable *J9::MonitorTable::init(J9PortLibrary *portLib, J9JavaVM *javaVM)
{
    if (_instance) {
        return _instance;
    }
    ...
    if (!table->_tableMonitor.init("JIT-MonitorTableMonitor"))
        return 0;
    if (!table->_j9ScratchMemoryPoolMonitor.init("JIT-ScratchMemoryPoolMonitor"))
        return 0;
#if defined(J9VM_GC_DYNAMIC_CLASS_UNLOADING)
    if (!table->_classUnloadMonitor.initFromVMMutex(javaVM->classUnloadMutex))
        return 0;
#else
    if (!table->_classUnloadMonitor.init("JIT-ClassUnloadMonitor"))
        return 0;
#endif
    if (!table->_iprofilerPersistenceMonitor.init("JIT-IProfilerPersistenceMonitor"))
        return 0;

    // Setup a wrapper for VM's monitors that the JIT can acquire
    if (!table->_classTableMutex.initFromVMMutex(javaVM->classTableMutex))
        return 0;
...
bool J9::MonitorTable::allocInitClassUnloadMonitorHolders(uint32_t allowedTotalCompThreads)
{
    PORT_ACCESS_FROM_PORT(_portLib);

    _numCompThreads = allowedTotalCompThreads;
    // Allocate and initialize the array of classUnloadMonitorHolders
    _classUnloadMonitorHolders
        = (int32_t *)j9mem_allocate_memory(sizeof(*(_classUnloadMonitorHolders)) * _numCompThreads, J9MEM_CATEGORY_JIT);
...
TR::Monitor *J9::MonitorTable::create(const char *name)
{
    PORT_ACCESS_FROM_PORT(_portLib);
...
void J9::MonitorTable::insert(TR::Monitor *monitor)
{
...
void J9::MonitorTable::removeAndDestroy(TR::Monitor *monitor)
{
...
bool J9::MonitorTable::isThreadInSafeMonitorState(J9VMThread *vmThread)
{
    // If we hold any of the following locks, return failure.
    if (_tableMonitor.owned_by_self() || _j9ScratchMemoryPoolMonitor.owned_by_self() || _classTableMutex.owned_by_self()
...
TR::Monitor *J9::MonitorTable::monitorHeldByCurrentThread()
{
    // If we hold any of the following locks, return failure.
...
int32_t J9::MonitorTable::readAcquireClassUnloadMonitor(int32_t compThreadIndex)
{
...
int32_t J9::MonitorTable::readReleaseClassUnloadMonitor(int32_t compThreadIndex)
{
    TR_ASSERT(compThreadIndex < _numCompThreads, "compThreadIndex is too high");
