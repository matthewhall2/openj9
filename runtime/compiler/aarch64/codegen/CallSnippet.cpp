...
static int32_t getBLDistance(uint8_t *cursor)
{
        ...
        if (methodSymRef->isUnresolved() || forceUnresolvedDispatch) {
            ...
            *(intptr_t *)cursor = (intptr_t)methodSymbol->getMethodAddress();
            ...
            if (comp->compileRelocatableCode()) {
                /*
                 * If we reach here, SymbolValidationManager is enabled because isResolvedDirectDispatchGuaranteed()
                 * returns false if it is AOT-compilation and SymbolValidationManger is disabled.
                 */
...
uint8_t *TR_Debug::printARM64ArgumentsFlush(OMR::Logger *log, TR::Node *callNode, uint8_t *cursor, int32_t argSize)
{
    uint32_t intArgNum = 0, floatArgNum = 0, offset;
...
void TR_Debug::print(OMR::Logger *log, TR::ARM64CallSnippet *snippet)
{
    ...
    log->printf("%s \t" POINTER_PRINTF_FORMAT "\t\t; %s%s", (glueRef->isOSRInductionHelper() ? "b" : "bl"), target,
        getName(glueRef), info);
    bufferPos += 4;
    ...
    if (!glueRef->isOSRInductionHelper()) {
        printPrefix(log, NULL, bufferPos, sizeof(intptr_t));
        log->printf(".dword \t" POINTER_PRINTF_FORMAT "\t\t; Method Pointer",
...
uint8_t *TR::ARM64VirtualSnippet::emitSnippetBody() { return NULL; }

uint32_t TR::ARM64VirtualSnippet::getLength(int32_t estimatedSnippetStart) { return 0; }
...
void TR_Debug::print(OMR::Logger *log, TR::ARM64UnresolvedCallSnippet *snippet)
{
    ...
    log->printf(".dword \t" POINTER_PRINTF_FORMAT "\t\t; Pointer To Constant Pool",
        *(reinterpret_cast<intptr_t *>(cursor)));
    cursor += sizeof(intptr_t);

    printPrefix(log, NULL, cursor, 8);
    log->printf(".dword \t%016llx\t\t; Offset | Flag | CP Index",
...
void TR_Debug::print(OMR::Logger *log, TR::ARM64VirtualUnresolvedSnippet *snippet)
{
    ...
    log->printf("bl \t" POINTER_PRINTF_FORMAT "\t\t; %s", (intptr_t)cursor + distance,
        getRuntimeHelperName(TR_ARM64virtualUnresolvedHelper));
    cursor += ARM64_INSTRUCTION_LENGTH;
...
uint8_t *TR::ARM64InterfaceCallSnippet::emitSnippetBody()
{
    ...
    *reinterpret_cast<intptr_t *>(cursor + 3 * sizeof(intptr_t)) = reinterpret_cast<intptr_t>(blAddress);
    ...
    cg()->addExternalRelocation(
        TR::ExternalRelocation::create(cursor + sizeof(intptr_t), NULL, TR_AbsoluteMethodAddress, cg()), __FILE__,
        __LINE__, callNode);

    // Register for relocation of the 2nd target address
    cg()->addExternalRelocation(
        TR::ExternalRelocation::create(cursor + 3 * sizeof(intptr_t), NULL, TR_AbsoluteMethodAddress, cg()), __FILE__,
        __LINE__, callNode);
    ...
    *reinterpret_cast<intptr_t *>(cursor) = reinterpret_cast<intptr_t>(thunk);
    if (comp->compileRelocatableCode()) {
        ...
        cg()->addExternalRelocation(TR::ExternalRelocation::create(j2iThunkRelocationPoint,
                                        reinterpret_cast<uint8_t *>(info), NULL, TR_J2IVirtualThunkPointer, cg()),
            __FILE__, __LINE__, callNode);
    }
    cursor += sizeof(intptr_t);
...
void TR_Debug::print(OMR::Logger *log, TR::ARM64InterfaceCallSnippet *snippet)
{
    TR::SymbolReference *callSymRef = snippet->getNode()->getSymbolReference();
    uint8_t *cursor = snippet->getSnippetLabel()->getCodeLocation();
    ...
    printPrefix(log, NULL, cursor, ARM64_INSTRUCTION_LENGTH);
    log->printf("bl \t" POINTER_PRINTF_FORMAT "\t\t; %s", (intptr_t)cursor + distance,
        getRuntimeHelperName(TR_ARM64interfaceCallHelper));
    cursor += ARM64_INSTRUCTION_LENGTH;
...
{
    int32_t codeSize = 4 * (instructionCountForArguments(callNode, cg) + 5)
        + 8; // 5 instructions for branch, Additional 8 bytes to hold size of thunk
...
TR_MHJ2IThunk *TR::ARM64CallSnippet::generateInvokeExactJ2IThunk(TR::Node *callNode, int32_t argSize,
    TR::CodeGenerator *cg, char *signature)
{
    ...
    dispatcher = reinterpret_cast<intptr_t>(
        cg->fej9()->getInvokeExactThunkHelperAddress(comp, dispatcherSymbol, callNode->getDataType()));
