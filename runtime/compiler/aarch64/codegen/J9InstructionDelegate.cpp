...
void J9::ARM64::InstructionDelegate::encodeBranchToLabel(TR::CodeGenerator *cg, TR::ARM64ImmSymInstruction *ins,
    uint8_t *cursor)
{
    ...
    TR::MemoryReference *mr)
{
    ...
    if (cg->getHasResumableTrapHandler()) {
        ...
        if (node != NULL & mr != NULL && mr->getCausesImplicitNullPointerException()
            && treeTopNode->getOpCode().isNullCheck() && isNPEThrowingInstruction(instr->getOpCodeValue())
...
void J9::ARM64::InstructionDelegate::setupImplicitNullPointerException(TR::CodeGenerator *cg,
    TR::ARM64Trg1MemInstruction *instr)
{
    setupImplicitNullPointerExceptionImpl(cg, instr, instr->getNode(), instr->getMemoryReference());
}

void J9::ARM64::InstructionDelegate::setupImplicitNullPointerException(TR::CodeGenerator *cg,
    TR::ARM64MemInstruction *instr)
{
    setupImplicitNullPointerExceptionImpl(cg, instr, instr->getNode(), instr->getMemoryReference());
}
