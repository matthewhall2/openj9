...
void J9::ARM64::CodeGenerator::initialize()
{
...
TR::Recompilation *J9::ARM64::CodeGenerator::allocateRecompilationInfo()
{
    return TR_ARM64Recompilation::allocate(self()->comp());
}

uint32_t J9::ARM64::CodeGenerator::encodeHelperBranchAndLink(TR::SymbolReference *symRef, uint8_t *cursor,
    TR::Node *node, bool omitLink)
{
    TR::CodeGenerator *cg = self();
    ...
    if (cg->directCallRequiresTrampoline(target, (intptr_t)cursor)) {
        ...
        TR_ASSERT_FATAL(
            cg->comp()->target().cpu.isTargetWithinUnconditionalBranchImmediateRange(target, (intptr_t)cursor),
...
TR::Instruction *J9::ARM64::CodeGenerator::generateSwitchToInterpreterPrePrologue(TR::Instruction *cursor,
    TR::Node *node)
{
    TR::Compilation *comp = self()->comp();
    ...
    TR::SymbolReference *revertToInterpreterSymRef
        = self()->symRefTab()->findOrCreateRuntimeHelper(TR_ARM64revertToInterpreterGlue);
...
bool J9::ARM64::CodeGenerator::supportsInliningOfIsInstance()
{
    return !self()->comp()->getOption(TR_DisableInlineIsInstance);
}

bool J9::ARM64::CodeGenerator::supportsInliningOfIsAssignableFrom()
{
...
bool J9::ARM64::CodeGenerator::suppressInliningOfRecognizedMethod(TR::RecognizedMethod method)
{
    TR::Compilation *comp = self()->comp();
    static const bool disableCRC32 = feGetEnv("TR_aarch64DisableCRC32") != NULL;

    if (method == TR::java_lang_Math_min_F || method == TR::java_lang_Math_max_F || method == TR::java_lang_Math_fma_D
        || method == TR::java_lang_Math_fma_F || method == TR::java_lang_StrictMath_fma_D
    ...
    if (method == TR::java_util_zip_CRC32C_updateBytes || method == TR::java_util_zip_CRC32C_updateDirectByteBuffer) {
        return (!TR::Compiler->om.canGenerateArraylets() && !TR::Compiler->om.isOffHeapAllocationEnabled())
            && comp->target().cpu.supportsFeature(OMR_FEATURE_ARM64_CRC32) && (!disableCRC32);
    }
    return false;
}

bool J9::ARM64::CodeGenerator::callUsesHelperImplementation(TR::Symbol *sym)
{
    return sym
