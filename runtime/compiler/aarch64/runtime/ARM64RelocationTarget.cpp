...
uint8_t *TR_ARM64RelocationTarget::eipBaseForCallOffset(uint8_t *reloLocation)
{
    // reloLocation points at the start of the call offset
    return reloLocation;
}

void TR_ARM64RelocationTarget::storeCallTarget(uintptr_t callTarget, uint8_t *reloLocation)
{
...
uint32_t TR_ARM64RelocationTarget::loadCPIndex(uint8_t *reloLocation)
{
...
uintptr_t TR_ARM64RelocationTarget::loadThunkCPIndex(uint8_t *reloLocation)
{
    // reloLocation points at the cpAddress in snippet, need to find cpIndex location
    reloLocation += sizeof(uintptr_t);
...
uint8_t *TR_ARM64RelocationTarget::loadAddressSequence(uint8_t *reloLocation)
{
...
void TR_ARM64RelocationTarget::storeAddressSequence(uint8_t *address, uint8_t *reloLocation, uint32_t seqNumber)
{
...
void TR_ARM64RelocationTarget::storeRelativeTarget(uintptr_t callTarget, uint8_t *reloLocation)
{
...
bool TR_ARM64RelocationTarget::useTrampoline(uint8_t *helperAddress, uint8_t *baseLocation)
{
    return !reloRuntime()->comp()->target().cpu.isTargetWithinUnconditionalBranchImmediateRange((intptr_t)helperAddress,
               (intptr_t)baseLocation)
        || TR::Options::getCmdLineOptions()->getOption(TR_StressTrampolines);
}

uint8_t *TR_ARM64RelocationTarget::arrayCopyHelperAddress(J9JavaVM *javaVM)
{
    return reinterpret_cast<uint8_t *>(javaVM->memoryManagerFunctions->referenceArrayCopy);
}

void TR_ARM64RelocationTarget::flushCache(uint8_t *codeStart, unsigned long size)
{
