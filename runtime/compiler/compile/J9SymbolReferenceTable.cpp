...
J9::SymbolReferenceTable::SymbolReferenceTable(size_t sizeHint, TR::Compilation *c)
    ...
    , _resolvedFieldShadows(std::less<ResolvedFieldShadowKey>(),
          getTypedAllocator<ResolvedFieldShadowsEntry>(c->allocator()))
    , _flattenedArrayElementFieldShadows(std::less<ResolvedFieldShadowKey>(),
...
TR::SymbolReference *J9::SymbolReferenceTable::findOrCreateCountForRecompileSymbolRef()
{
    if (!element(countForRecompileSymbol)) {
        ...
        element(countForRecompileSymbol)
            = new (trHeapMemory()) TR::SymbolReference(self(), countForRecompileSymbol, sym);
    }
    return element(countForRecompileSymbol);
}

TR::SymbolReference *J9::SymbolReferenceTable::findOrCreateOSRBufferSymbolRef()
{
    if (!element(osrBufferSymbol)) {
        ...
        aliasBuilder.addressStaticSymRefs().set(
            getNonhelperIndex(osrBufferSymbol)); // add the symRef to the statics list to get correct aliasing info
    }
    return element(osrBufferSymbol);
}

TR::SymbolReference *J9::SymbolReferenceTable::findOrCreateOSRScratchBufferSymbolRef()
{
    if (!element(osrScratchBufferSymbol)) {
        ...
        // We can't let the load/store of the exception symbol swing down
        aliasBuilder.addressStaticSymRefs().set(getNonhelperIndex(
            osrScratchBufferSymbol)); // add the symRef to the statics list to get correct aliasing info
    }
    return element(osrScratchBufferSymbol);
}

TR::SymbolReference *J9::SymbolReferenceTable::findOrCreateOSRFrameIndexSymbolRef()
{
    if (!element(osrFrameIndexSymbol)) {
        ...
        // SD: do we need this?
        aliasBuilder.addressStaticSymRefs().set(
            getNonhelperIndex(osrFrameIndexSymbol)); // add the symRef to the statics list to get correct aliasing info
    }
    return element(osrFrameIndexSymbol);
}

TR::SymbolReference *J9::SymbolReferenceTable::findOrCreateAcquireVMAccessSymbolRef(TR::ResolvedMethodSymbol *)
{
    return findOrCreateRuntimeHelper(TR_acquireVMAccess, true, false, true);
}

TR::SymbolReference *J9::SymbolReferenceTable::findOrCreateThrowCurrentExceptionSymbolRef(TR::ResolvedMethodSymbol *)
{
    return findOrCreateRuntimeHelper(TR_throwCurrentException, true, false, true);
}

TR::SymbolReference *J9::SymbolReferenceTable::findOrCreateThrowUnreportedExceptionSymbolRef(TR::ResolvedMethodSymbol *)
{
    return findOrCreateRuntimeHelper(TR_throwUnreportedException, true /* canGCandReturn */, false /* canGCandExcept */,
        true /* preservesAllRegisters */);
}

TR::SymbolReference *J9::SymbolReferenceTable::findOrCreateReleaseVMAccessSymbolRef(TR::ResolvedMethodSymbol *)
{
    return findOrCreateRuntimeHelper(TR_releaseVMAccess, true, false, true);
}

TR::SymbolReference *J9::SymbolReferenceTable::findOrCreateStackOverflowSymbolRef(TR::ResolvedMethodSymbol *)
{
    return findOrCreateRuntimeHelper(TR_stackOverflow, true, true, true);
}

TR::SymbolReference *J9::SymbolReferenceTable::findOrCreateWriteBarrierStoreSymbolRef(TR::ResolvedMethodSymbol *)
{
    return findOrCreateRuntimeHelper(TR_writeBarrierStore, false, false, true);
}

TR::SymbolReference *J9::SymbolReferenceTable::findOrCreateWriteBarrierStoreGenerationalSymbolRef(
    TR::ResolvedMethodSymbol *)
{
    return findOrCreateRuntimeHelper(TR_writeBarrierStoreGenerational, false, false, true);
}

TR::SymbolReference *J9::SymbolReferenceTable::findOrCreateWriteBarrierStoreGenerationalAndConcurrentMarkSymbolRef(
    TR::ResolvedMethodSymbol *)
{
    return findOrCreateRuntimeHelper(TR_writeBarrierStoreGenerationalAndConcurrentMark, false, false, true);
}

TR::SymbolReference *J9::SymbolReferenceTable::findOrCreateWriteBarrierStoreRealTimeGCSymbolRef(
    TR::ResolvedMethodSymbol *)
{
    return findOrCreateRuntimeHelper(TR_writeBarrierStoreRealTimeGC, true, true, true);
}

TR::SymbolReference *J9::SymbolReferenceTable::findOrCreateWriteBarrierClassStoreRealTimeGCSymbolRef(
    TR::ResolvedMethodSymbol *)
{
    return findOrCreateRuntimeHelper(TR_writeBarrierClassStoreRealTimeGC, true, true, true);
}

TR::SymbolReference *J9::SymbolReferenceTable::findOrCreateWriteBarrierBatchStoreSymbolRef(TR::ResolvedMethodSymbol *)
{
    return findOrCreateRuntimeHelper(TR_writeBarrierBatchStore, false, false, true);
}

TR::SymbolReference *J9::SymbolReferenceTable::findOrCreateAcmpeqHelperSymbolRef(
    TR::ResolvedMethodSymbol *owningMethodSymbol)
{
    return findOrCreateRuntimeHelper(TR_acmpeqHelper, true, false, true);
}

TR::SymbolReference *J9::SymbolReferenceTable::findOrCreateAcmpneHelperSymbolRef(
    TR::ResolvedMethodSymbol *owningMethodSymbol)
{
    return findOrCreateRuntimeHelper(TR_acmpneHelper, true, false, true);
}

TR::SymbolReference *J9::SymbolReferenceTable::findOrCreateGetFlattenableFieldSymbolRef(
    TR::ResolvedMethodSymbol *owningMethodSymbol)
{
...
TR::SymbolReference *J9::SymbolReferenceTable::findOrCreatePutFlattenableStaticFieldSymbolRef(
    TR::ResolvedMethodSymbol *owningMethodSymbol)
{
    return findOrCreateRuntimeHelper(TR_putFlattenableStaticField, true, true, true);
}

TR::SymbolReference *J9::SymbolReferenceTable::findOrCreateLoadFlattenableArrayElementSymbolRef(
    TR::ResolvedMethodSymbol *owningMethodSymbol)
{
    return findOrCreateRuntimeHelper(TR_ldFlattenableArrayElement, true, true, true);
}

TR::SymbolReference *J9::SymbolReferenceTable::findOrCreateStoreFlattenableArrayElementSymbolRef(
    TR::ResolvedMethodSymbol *owningMethodSymbol)
{
    return findOrCreateRuntimeHelper(TR_strFlattenableArrayElement, true, true, true);
}

TR::SymbolReference *J9::SymbolReferenceTable::findOrCreateLookupDynamicPublicInterfaceMethodSymbolRef()
{
    return findOrCreateRuntimeHelper(TR_jitLookupDynamicPublicInterfaceMethod, false, true, true);
}

TR::SymbolReference *J9::SymbolReferenceTable::findOrCreateDispatchJ9MethodSymbolRef()
{
    ...
    if (symRef == NULL) {
        ...
        TR::SymbolReference *symRef = new (trHeapMemory()) TR::SymbolReference(self(), jitDispatchJ9MethodSymbol, sym);
...
TR::SymbolReference *J9::SymbolReferenceTable::findOrCreateFloatSymbol(TR::ResolvedMethodSymbol *owningMethodSymbol,
    int32_t cpIndex)
{
...
TR::SymbolReference *J9::SymbolReferenceTable::findOrCreateDoubleSymbol(TR::ResolvedMethodSymbol *owningMethodSymbol,
    int32_t cpIndex)
{
...
TR::SymbolReference *J9::SymbolReferenceTable::createSystemRuntimeHelper(TR_RuntimeHelper index, bool canGCandReturn,
    bool canGCandExcept, bool preservesAllRegisters)
{
...
TR::SymbolReference *J9::SymbolReferenceTable::findOrCreateStaticMethodSymbol(
    TR::ResolvedMethodSymbol *owningMethodSymbol, int32_t cpIndex)
{
    bool isUnresolvedInCP;
    TR_ResolvedMethod *method
        = owningMethodSymbol->getResolvedMethod()->getResolvedStaticMethod(comp(), cpIndex, &isUnresolvedInCP);
    if (method)
        owningMethodSymbol->setMayHaveInlineableCall(true);

    return findOrCreateMethodSymbol(owningMethodSymbol->getResolvedMethodIndex(), cpIndex, method,
        TR::MethodSymbol::Static, isUnresolvedInCP);
}

TR::SymbolReference *J9::SymbolReferenceTable::findOrCreateSpecialMethodSymbol(
    TR::ResolvedMethodSymbol *owningMethodSymbol, int32_t cpIndex)
{
    bool isUnresolvedInCP;
    TR_ResolvedMethod *method
        = owningMethodSymbol->getResolvedMethod()->getResolvedSpecialMethod(comp(), cpIndex, &isUnresolvedInCP);
    if (method)
        owningMethodSymbol->setMayHaveInlineableCall(true);

    return findOrCreateMethodSymbol(owningMethodSymbol->getResolvedMethodIndex(), cpIndex, method,
        TR::MethodSymbol::Special, isUnresolvedInCP);
}

TR::SymbolReference *J9::SymbolReferenceTable::findOrCreateVirtualMethodSymbol(
    TR::ResolvedMethodSymbol *owningMethodSymbol, int32_t cpIndex)
{
    bool isUnresolvedInCP;
    TR_ResolvedMethod *method
        = owningMethodSymbol->getResolvedMethod()->getResolvedVirtualMethod(comp(), cpIndex, false, &isUnresolvedInCP);
    if (method)
        owningMethodSymbol->setMayHaveInlineableCall(true);

    return findOrCreateMethodSymbol(owningMethodSymbol->getResolvedMethodIndex(), cpIndex, method,
        TR::MethodSymbol::Virtual, false); // isUnresolvedInCP);
}

TR::SymbolReference *J9::SymbolReferenceTable::findOrCreateInterfaceMethodSymbol(
    TR::ResolvedMethodSymbol *owningMethodSymbol, int32_t cpIndex)
{
    owningMethodSymbol->setMayHaveInlineableCall(true);
    return findOrCreateMethodSymbol(owningMethodSymbol->getResolvedMethodIndex(), cpIndex, 0,
        TR::MethodSymbol::Interface);
}

TR::SymbolReference *J9::SymbolReferenceTable::findOrCreateDynamicMethodSymbol(
    TR::ResolvedMethodSymbol *owningMethodSymbol, int32_t callSiteIndex, bool *unresolvedInCP,
    bool *isInvokeCacheAppendixNull)
{
    ...
    TR_ResolvedMethod *method = owningMethodSymbol->getResolvedMethod()->getResolvedDynamicMethod(comp(), callSiteIndex,
        unresolvedInCP, isInvokeCacheAppendixNull);
    if (method)
        owningMethodSymbol->setMayHaveInlineableCall(true);

    TR::SymbolReference *symRef
        = findOrCreateMethodSymbol(owningMethodSymbol->getResolvedMethodIndex(), -1, method, TR::MethodSymbol::Static);
#else
    ...
    TR_ResolvedMethod *method
        = owningMethodSymbol->getResolvedMethod()->getResolvedDynamicMethod(comp(), callSiteIndex, unresolvedInCP);
    TR::SymbolReference *symRef = findOrCreateMethodSymbol(owningMethodSymbol->getResolvedMethodIndex(), -1, method,
...
TR::SymbolReference *J9::SymbolReferenceTable::findOrCreateHandleMethodSymbol(
    TR::ResolvedMethodSymbol *owningMethodSymbol, int32_t cpIndex, char *signature)
{
    TR_ResolvedMethod *method
        = owningMethodSymbol->getResolvedMethod()->getResolvedHandleMethodWithSignature(comp(), cpIndex, signature);
    if (method)
        owningMethodSymbol->setMayHaveInlineableCall(true);

    TR::SymbolReference *symRef = findOrCreateMethodSymbol(owningMethodSymbol->getResolvedMethodIndex(), cpIndex,
        method, TR::MethodSymbol::ComputedVirtual);
    return symRef;
}

TR::SymbolReference *J9::SymbolReferenceTable::findOrCreateHandleMethodSymbol(
    TR::ResolvedMethodSymbol *owningMethodSymbol, int32_t cpIndex, bool *unresolvedInCP,
    bool *isInvokeCacheAppendixNull)
{
#if defined(J9VM_OPT_OPENJDK_METHODHANDLE)
    TR_ResolvedMethod *method = owningMethodSymbol->getResolvedMethod()->getResolvedHandleMethod(comp(), cpIndex,
        unresolvedInCP, isInvokeCacheAppendixNull);
    TR::SymbolReference *symRef = findOrCreateMethodSymbol(owningMethodSymbol->getResolvedMethodIndex(), cpIndex,
        method, TR::MethodSymbol::Static);
    if (*unresolvedInCP)
        symRef->getSymbol()->setDummyResolvedMethod(); // linkToStatic is a dummy TR_ResolvedMethod
#else
    TR_ResolvedMethod *method
        = owningMethodSymbol->getResolvedMethod()->getResolvedHandleMethod(comp(), cpIndex, unresolvedInCP);
    TR::SymbolReference *symRef = findOrCreateMethodSymbol(owningMethodSymbol->getResolvedMethodIndex(), cpIndex,
        method, TR::MethodSymbol::ComputedVirtual);
#endif /* J9VM_OPT_OPENJDK_METHODHANDLE */
    if (method)
...
TR::SymbolReference *J9::SymbolReferenceTable::findOrCreateCallSiteTableEntrySymbol(
    TR::ResolvedMethodSymbol *owningMethodSymbol, int32_t callSiteIndex)
{
    TR::SymbolReference *symRef;
...
TR::SymbolReference *J9::SymbolReferenceTable::findOrCreateMethodTypeTableEntrySymbol(
    TR::ResolvedMethodSymbol *owningMethodSymbol, int32_t cpIndex)
{
    TR::SymbolReference *symRef;
...
TR::SymbolReference *J9::SymbolReferenceTable::findOrCreateVarHandleMethodTypeTableEntrySymbol(
    TR::ResolvedMethodSymbol *owningMethodSymbol, int32_t cpIndex)
{
    // This function does the same thing as what findOrCreateMethodTypeTableEntrySymbol does except the way it gets the
    // method type table entry address
    TR::SymbolReference *symRef;
    TR_SymRefIterator i(aliasBuilder.methodTypeTableEntrySymRefs(), self());
...
TR::SymbolReference *J9::SymbolReferenceTable::methodSymRefWithSignature(TR::SymbolReference *originalSymRef,
    char *effectiveSignature, int32_t effectiveSignatureLength)
{
    ...
    TR_ASSERT(!originalSymbol->isVirtual(),
        "methodSymRefFromName doesn't support virtual methods"); // Until we're able to look up vtable index
    ...
    int32_t fullSignatureLength
        = originalMethod->classNameLength() + 1 + originalMethod->nameLength() + effectiveSignatureLength;
    char *fullSignature = (char *)trMemory()->allocateMemory(1 + fullSignatureLength, stackAlloc);
    snprintf(fullSignature, 1 + fullSignatureLength, "%.*s.%.*s%.*s", originalMethod->classNameLength(),
        originalMethod->classNameChars(), originalMethod->nameLength(), originalMethod->nameChars(),
        effectiveSignatureLength, effectiveSignature);
    ...
    TR_J9VMBase *fej9 = (TR_J9VMBase *)(fe());
    // Note: we use cpIndex=-1 here so we don't end up getting back originalSymRef (which will not have the signature we
    // want)
    TR::SymbolReference *result = findOrCreateMethodSymbol(originalSymRef->getOwningMethodIndex(), -1,
        fej9->createResolvedMethodWithSignature(comp()->trMemory(), method, NULL, effectiveSignature,
            effectiveSignatureLength, originalSymbol->getResolvedMethod()->owningMethod()),
        originalSymbol->getMethodKind());

    result->setCPIndex(cpIndex);
    _methodsBySignature.Add(key, result);
    TR_ASSERT(_methodsBySignature.Locate(key),
        "After _methodsBySignature.Add, _methodsBySignature.Locate must be true");

    return result;
}

TR::SymbolReference *J9::SymbolReferenceTable::findOrCreateTypeCheckArrayStoreSymbolRef(TR::ResolvedMethodSymbol *)
{
    return findOrCreateRuntimeHelper(TR_typeCheckArrayStore, false, true, true);
}

TR::SymbolReference *J9::SymbolReferenceTable::findOrCreateArrayClassRomPtrSymbolRef()
{
...
// because searchRecognizedField only does a name comparison, t does not work if the field is accessed from a subclass
// of the expected one
TR::SymbolReference *J9::SymbolReferenceTable::findOrFabricateShadowSymbol(TR::ResolvedMethodSymbol *owningMethodSymbol,
    TR::Symbol::RecognizedField recognizedField, TR::DataType type, uint32_t offset, bool isVolatile, bool isPrivate,
    bool isFinal, const char *name)
{
    TR_ResolvedMethod *owningMethod = owningMethodSymbol->getResolvedMethod();
    ...
    sym = createShadowSymbol(type, isVolatile, isPrivate, isFinal, name, recognizedField);
...
TR::SymbolReference *J9::SymbolReferenceTable::findResolvedFieldShadow(ResolvedFieldShadowKey key, bool isVolatile,
    bool isPrivate, bool isFinal)
{
...
TR::SymbolReference *J9::SymbolReferenceTable::findOrFabricateShadowSymbol(TR_OpaqueClassBlock *containingClass,
    TR::DataType type, uint32_t offset, bool isVolatile, bool isPrivate, bool isFinal, const char *name,
    const char *signature)
{
    ...
    TR::snprintfNoTrunc(qualifiedFieldName, qualifiedFieldNameSize, "%.*s.%s %s", classNameLen, className, name,
        signature);

    TR::Symbol *sym
        = createShadowSymbol(type, isVolatile, isPrivate, isFinal, qualifiedFieldName, TR::Symbol::UnknownField);

    TR_J9VM *fej9 = reinterpret_cast<TR_J9VM *>(fe());
    TR_OpaqueClassBlock *declaredClass
        = fej9->getClassFromSignature(signature, (int32_t)strlen(signature), containingClass);
    if (declaredClass != NULL)
        sym->setDeclaredClass(declaredClass);

    mcount_t methodIndex = mcount_t::valueOf(0);
    int32_t cpIndex = -1;
    symRef = new (trHeapMemory()) TR::SymbolReference(self(), sym, methodIndex, cpIndex);
...
#if defined(J9VM_OPT_OPENJDK_METHODHANDLE)
TR::SymbolReference *J9::SymbolReferenceTable::findOrFabricateMemberNameVmTargetShadow()
{
    bool isVolatile = false;
    bool isPrivate = false;
    bool isFinal = true;
    TR::SymbolReference *symRef = self()->findOrFabricateShadowSymbol(comp()->getMethodSymbol(),
        TR::Symbol::Java_lang_invoke_MemberName_vmtarget, TR::Address, comp()->fej9()->getVMTargetOffset(), isVolatile,
        isPrivate, isFinal, "java/lang/invoke/MemberName.vmtarget J");
...
TR::SymbolReference *J9::SymbolReferenceTable::findFlattenedArrayElementFieldShadow(ResolvedFieldShadowKey key,
    bool isPrivate)
{
...
TR::SymbolReference *J9::SymbolReferenceTable::findOrFabricateFlattenedArrayElementFieldShadowSymbol(
    TR_OpaqueClassBlock *arrayComponentClass, TR::DataType type, int32_t fieldOffset, bool isPrivate,
    const char *fieldName, const char *fieldSignature)
{
    TR_ASSERT_FATAL(fieldOffset >= 0,
        "fieldOffset %d is invalid: fieldOffset %u objectHeaderSizeInBytes %" OMR_PRIuPTR " \n", fieldOffset,
    ...
    TR::Symbol *sym = createShadowSymbol(type, false /*isVolatile*/, isPrivate, false /*isFinal*/, qualifiedFieldName,
...
TR::Symbol *J9::SymbolReferenceTable::createShadowSymbol(TR::DataType type, bool isVolatile, bool isPrivate,
    bool isFinal, const char *name, TR::Symbol::RecognizedField recognizedField)
{
    TR::Symbol *sym = TR::Symbol::createPossiblyRecognizedShadowWithFlags(trHeapMemory(), type, isVolatile, isFinal,
        isPrivate, recognizedField);
...
TR::SymbolReference *J9::SymbolReferenceTable::findOrCreateShadowSymbol(TR::ResolvedMethodSymbol *owningMethodSymbol,
    int32_t cpIndex, bool isStore)
{
    ...
    uint32_t offset = 0;
    bool resolved = owningMethod->fieldAttributes(comp(), cpIndex, &offset, &type, &isVolatile, &isFinal, &isPrivate,
        isStore, &isUnresolvedInCP, true);
    bool sharesSymbol = false;

    TR_OpaqueClassBlock *containingClass = NULL;
    TR::Symbol::RecognizedField recognizedField
        = TR::Symbol::searchRecognizedField(comp(), owningMethod, cpIndex, false);
    ...
    if (resolved) {
        ...
        containingClass = owningMethod->definingClassFromCPFieldRef(comp(), cpIndex, isStatic, &fromResolvedJ9Method);
        ...
        if (containingClass == NULL)
            comp()->failCompilation<TR::CompilationException>(
                "failed to get defining class of resolved field ref cpIndex=%d in owning method J9Method=%p", cpIndex,
    ...
    if (symRef) {
        ...
        sym = createShadowSymbol(type, isVolatile, isPrivate, isFinal, NULL, recognizedField);
    }

    if (resolved) {
        int32_t len = 0;
        const char *sig = owningMethod->fieldSignatureChars(cpIndex, len);
        TR_OpaqueClassBlock *declaredClass = fe()->getClassFromSignature(sig, len, owningMethod);
        ...
        if (declaredClass != NULL) {
            ...
            if (prevDeclaredClass == NULL) {
                ...
                TR_ASSERT_FATAL(prevDeclaredClass == declaredClass, "declared class mismatch: %p vs. prev %p",
                    declaredClass, prevDeclaredClass);
            }
        }
    }
...
TR::SymbolReference *J9::SymbolReferenceTable::findOrCreateObjectNewInstanceImplSymbol(
    TR::ResolvedMethodSymbol *owningMethodSymbol)
{
...
TR::SymbolReference *J9::SymbolReferenceTable::findOrCreateDLTBlockSymbolRef()
{
    if (!element(dltBlockSymbol)) {
        ...
        //
        aliasBuilder.addressStaticSymRefs().set(
            getNonhelperIndex(dltBlockSymbol)); // add the symRef to the statics list to get correct aliasing info
        aliasBuilder.gcSafePointSymRefNumbers().set(getNonhelperIndex(dltBlockSymbol));
    }
    return element(dltBlockSymbol);
}

TR::SymbolReference *J9::SymbolReferenceTable::findDLTBlockSymbolRef() { return element(dltBlockSymbol); }

TR::SymbolReference *J9::SymbolReferenceTable::findOrCreateMultiANewArraySymbolRef(TR::ResolvedMethodSymbol *)
{
    return findOrCreateRuntimeHelper(TR_multiANewArray, true, true, true);
}

TR::SymbolReference *J9::SymbolReferenceTable::findOrCreateInstanceShapeSymbolRef()
{
...
TR::SymbolReference *J9::SymbolReferenceTable::findOrCreateInstanceDescriptionSymbolRef()
{
    if (!element(instanceDescriptionSymbol)) {
        ...
        if (self()->comp()->target().is64Bit())
            ...
            sym = TR::Symbol::createShadow(trHeapMemory(), TR::Int32);
        element(instanceDescriptionSymbol)
            = new (trHeapMemory()) TR::SymbolReference(self(), instanceDescriptionSymbol, sym);
...
TR::SymbolReference *J9::SymbolReferenceTable::findOrCreateDescriptionWordFromPtrSymbolRef()
{
    if (!element(descriptionWordFromPtrSymbol)) {
        ...
        if (self()->comp()->target().is64Bit())
            ...
            sym = TR::Symbol::createShadow(trHeapMemory(), TR::Int32);
        element(descriptionWordFromPtrSymbol)
            = new (trHeapMemory()) TR::SymbolReference(self(), descriptionWordFromPtrSymbol, sym);
...
TR::SymbolReference *J9::SymbolReferenceTable::findOrCreateClassFlagsSymbolRef()
{
...
TR::SymbolReference *J9::SymbolReferenceTable::findOrCreateClassDepthAndFlagsSymbolRef()
{
    if (!element(isClassDepthAndFlagsSymbol)) {
        TR_J9VMBase *fej9 = (TR_J9VMBase *)(fe());
        TR::Symbol *sym = TR::Symbol::createShadow(trHeapMemory(), TR::Int32);
        element(isClassDepthAndFlagsSymbol)
            = new (trHeapMemory()) TR::SymbolReference(self(), isClassDepthAndFlagsSymbol, sym);
        element(isClassDepthAndFlagsSymbol)->setOffset(fej9->getOffsetOfClassDepthAndFlags());
    }
    return element(isClassDepthAndFlagsSymbol);
}

TR::SymbolReference *J9::SymbolReferenceTable::findOrCreateArrayComponentTypeAsPrimitiveSymbolRef()
{
    if (!element(componentClassAsPrimitiveSymbol)) {
        ...
        element(componentClassAsPrimitiveSymbol)
            = new (trHeapMemory()) TR::SymbolReference(self(), componentClassAsPrimitiveSymbol, sym);
...
TR::SymbolReference *J9::SymbolReferenceTable::findOrCreateMethodTypeCheckSymbolRef(TR::ResolvedMethodSymbol *)
{
    return findOrCreateRuntimeHelper(TR_methodTypeCheck, false, true, true);
}

TR::SymbolReference *J9::SymbolReferenceTable::findOrCreateIncompatibleReceiverSymbolRef(TR::ResolvedMethodSymbol *)
{
    return findOrCreateRuntimeHelper(TR_incompatibleReceiver, false, true, true);
}

TR::SymbolReference *J9::SymbolReferenceTable::findOrCreateIdentityExceptionSymbolRef(TR::ResolvedMethodSymbol *)
{
    return findOrCreateRuntimeHelper(TR_identityException, false, true, true);
}

TR::SymbolReference *J9::SymbolReferenceTable::findOrCreateIncompatibleClassChangeErrorSymbolRef(
    TR::ResolvedMethodSymbol *)
{
    return findOrCreateRuntimeHelper(TR_IncompatibleClassChangeError, false, true, true);
}

TR::SymbolReference *J9::SymbolReferenceTable::findOrCreateReportStaticMethodEnterSymbolRef(TR::ResolvedMethodSymbol *)
{
    return findOrCreateRuntimeHelper(TR_reportStaticMethodEnter, true, false, true);
}

TR::SymbolReference *J9::SymbolReferenceTable::findOrCreateReportMethodExitSymbolRef(TR::ResolvedMethodSymbol *)
{
...
TR::SymbolReference *J9::SymbolReferenceTable::findOrCreateHeaderFlagsSymbolRef()
{
    if (!element(headerFlagsSymbol)) {
        ...
        symRef = new (trHeapMemory())
            TR::SymbolReference(self(), headerFlagsSymbol, TR::Symbol::createShadow(trHeapMemory(), TR::Int32));
...
TR::SymbolReference *J9::SymbolReferenceTable::findOrCreateDiscontiguousArraySizeSymbolRef()
{
    if (!element(discontiguousArraySizeSymbol)) {
        ...
        //
        sym = TR::Symbol::createShadow(trHeapMemory(), TR::Int32);
        element(discontiguousArraySizeSymbol)
            = new (trHeapMemory()) TR::SymbolReference(self(), discontiguousArraySizeSymbol, sym);
...
TR::SymbolReference *J9::SymbolReferenceTable::findOrCreateClassLoaderSymbolRef(TR_ResolvedMethod *method)
{
    ...
    aliasBuilder.addressStaticSymRefs().set(
        symRef->getReferenceNumber()); // add the symRef to the statics list to get correct aliasing info
...
TR::SymbolReference *J9::SymbolReferenceTable::findOrCreateCurrentThreadSymbolRef()
{
...
TR::SymbolReference *J9::SymbolReferenceTable::findOrCreateJ9MethodConstantPoolFieldSymbolRef(intptr_t offset)
{
    if (!element(j9methodConstantPoolSymbol)) {
        ...
        element(j9methodConstantPoolSymbol)
            = new (trHeapMemory()) TR::SymbolReference(self(), j9methodConstantPoolSymbol, sym);
        element(j9methodConstantPoolSymbol)->setOffset(offset);
    }
    TR::SymbolReference *result = element(j9methodConstantPoolSymbol);
    TR_ASSERT(result->getOffset() == offset,
        "findOrCreateJ9MethodConstantPoolFieldSymbolRef must use the right offset!  %d != %d", result->getOffset(),
        offset);
    return result;
}

TR::SymbolReference *J9::SymbolReferenceTable::findOrCreateJ9MethodExtraFieldSymbolRef(intptr_t offset)
{
    if (!element(j9methodExtraFieldSymbol)) {
        ...
        element(j9methodExtraFieldSymbol)
            = new (trHeapMemory()) TR::SymbolReference(self(), j9methodExtraFieldSymbol, sym);
        element(j9methodExtraFieldSymbol)->setOffset(offset);
    }
    TR::SymbolReference *result = element(j9methodExtraFieldSymbol);
    TR_ASSERT(result->getOffset() == offset,
        "findOrCreateJ9MethodExtraFieldSymbolRef must use the right offset!  %d != %d", result->getOffset(), offset);
    return result;
}

TR::SymbolReference *J9::SymbolReferenceTable::findOrCreateJ9JNIMethodIDvTableIndexFieldSymbol(intptr_t offset)
{
    if (!element(J9JNIMethodIDvTableIndexFieldSymbol)) {
        ...
        element(J9JNIMethodIDvTableIndexFieldSymbol)
            = new (trHeapMemory()) TR::SymbolReference(self(), J9JNIMethodIDvTableIndexFieldSymbol, sym);
        element(J9JNIMethodIDvTableIndexFieldSymbol)->setOffset(offset);
    }
    return element(J9JNIMethodIDvTableIndexFieldSymbol);
}

TR::SymbolReference *J9::SymbolReferenceTable::findOrCreateStartPCLinkageInfoSymbolRef(intptr_t offset)
{
    if (!element(startPCLinkageInfoSymbol)) {
        TR::Symbol *sym = TR::Symbol::createShadow(trHeapMemory(), TR::Int32);
        element(startPCLinkageInfoSymbol)
            = new (trHeapMemory()) TR::SymbolReference(self(), startPCLinkageInfoSymbol, sym);
        element(startPCLinkageInfoSymbol)->setOffset(offset);
    }
    TR::SymbolReference *result = element(startPCLinkageInfoSymbol);
    TR_ASSERT(result->getOffset() == offset,
        "findOrCreateStartPCLinkageInfoSymbolRef must use the right offset!  %d != %d", result->getOffset(), offset);
    return result;
}

TR::SymbolReference *J9::SymbolReferenceTable::findOrCreatePerCodeCacheHelperSymbolRef(TR_CCPreLoadedCode helper,
    uintptr_t helperAddr)
{
    ...
    if (!element(index)) {
        ...
        // The address of the helper depends on the current codecache, which can change during compilation, so we don't
        // have a single method address
...
TR::SymbolReference *J9::SymbolReferenceTable::findOrCreateANewArraySymbolRef(TR::ResolvedMethodSymbol *)
{
    return findOrCreateRuntimeHelper(TR_aNewArray, true, true, true);
}

TR::SymbolReference *J9::SymbolReferenceTable::findOrCreateStringSymbol(TR::ResolvedMethodSymbol *owningMethodSymbol,
    int32_t cpIndex)
{
    TR_ResolvedMethod *owningMethod = owningMethodSymbol->getResolvedMethod();
    ...
    else if (!sym->isConstString() && !sym->isNonSpecificConstObject()) {
...
TR::SymbolReference *J9::SymbolReferenceTable::findOrCreateConstantDynamicSymbol(
    TR::ResolvedMethodSymbol *owningMethodSymbol, int32_t cpIndex, char *symbolTypeSig, int32_t symbolTypeSigLength,
    bool isCondyPrimitive)
{
    TR_ResolvedMethod *owningMethod = owningMethodSymbol->getResolvedMethod();
...
TR::SymbolReference *J9::SymbolReferenceTable::findOrCreateMethodMonitorEntrySymbolRef(TR::ResolvedMethodSymbol *)
{
    return findOrCreateRuntimeHelper(TR_methodMonitorEntry, true, false, true);
}

TR::SymbolReference *J9::SymbolReferenceTable::findOrCreateMethodMonitorExitSymbolRef(TR::ResolvedMethodSymbol *)
{
    return findOrCreateRuntimeHelper(TR_methodMonitorExit, true /* canGCandReturn */, true /* canGCandExcept */,
        true /* preservesAllRegisters */);
}

TR::SymbolReference *J9::SymbolReferenceTable::findOrCreateCompiledMethodSymbolRef()
{
...
TR::SymbolReference *J9::SymbolReferenceTable::findOrCreateMethodTypeSymbol(
    TR::ResolvedMethodSymbol *owningMethodSymbol, int32_t cpIndex)
{
    TR_ResolvedMethod *owningMethod = owningMethodSymbol->getResolvedMethod();
...
TR::SymbolReference *J9::SymbolReferenceTable::findOrCreateMethodHandleSymbol(
    TR::ResolvedMethodSymbol *owningMethodSymbol, int32_t cpIndex)
{
    TR_ResolvedMethod *owningMethod = owningMethodSymbol->getResolvedMethod();
...
TR::SymbolReference *J9::SymbolReferenceTable::findOrCreateClassFromJavaLangClassAsPrimitiveSymbolRef()
{
    if (!element(classFromJavaLangClassAsPrimitiveSymbol)) {
        ...
        element(classFromJavaLangClassAsPrimitiveSymbol)
            = new (trHeapMemory()) TR::SymbolReference(self(), classFromJavaLangClassAsPrimitiveSymbol, sym);
...
TR::SymbolReference *J9::SymbolReferenceTable::findClassFromJavaLangClassAsPrimitiveSymbolRef()
{
    return element(classFromJavaLangClassAsPrimitiveSymbol);
}

TR::SymbolReference *J9::SymbolReferenceTable::createShadowSymbolWithoutCpIndex(
    TR::ResolvedMethodSymbol *owningMethodSymbol, bool isResolved, TR::DataType type, uint32_t offset,
    bool isUnresolvedInCP)
{
    TR_ResolvedMethod *owningMethod = owningMethodSymbol->getResolvedMethod();
...
TR::SymbolReference *J9::SymbolReferenceTable::findJavaLangReferenceReferentShadowSymbol(
    TR_ResolvedMethod *owningMethod, TR::DataType type, uint32_t offset)
{
    TR::SymbolReference *symRef;
    TR_SymRefIterator i(type == TR::Address
            ? aliasBuilder.addressShadowSymRefs()
            : (type == TR::Int32 ? aliasBuilder.intShadowSymRefs() : aliasBuilder.nonIntPrimitiveShadowSymRefs()),
        self());
    while ((symRef = i.getNext()) != NULL)
        if (symRef->getSymbol()->getDataType() == type && symRef->getOffset() == offset
            && symRef->getOwningMethod(comp()) == owningMethod)
            return symRef;
    return 0;
}

void J9::SymbolReferenceTable::initShadowSymbol(TR_ResolvedMethod *owningMethod, TR::SymbolReference *symRef,
    bool isResolved, TR::DataType type, uint32_t offset, bool isUnresolvedInCP)
{
    if (isResolved) {
...
List<TR::SymbolReference> *J9::SymbolReferenceTable::dynamicMethodSymrefsByCallSiteIndex(int32_t index)
{
    if (!_dynamicMethodSymrefsByCallSiteIndex[index])
        _dynamicMethodSymrefsByCallSiteIndex[index]
            = new (trHeapMemory()) List<TR::SymbolReference>(comp()->trMemory());
    return _dynamicMethodSymrefsByCallSiteIndex[index];
}

bool J9::SymbolReferenceTable::isFieldClassObject(TR::SymbolReference *symRef)
{
    int32_t len;
...
bool J9::SymbolReferenceTable::isFieldTypeBool(TR::SymbolReference *symRef)
{
    int32_t len;
...
bool J9::SymbolReferenceTable::isStaticTypeBool(TR::SymbolReference *symRef)
{
    int32_t len;
...
bool J9::SymbolReferenceTable::isFieldTypeChar(TR::SymbolReference *symRef)
{
    int32_t len;
    const char *fieldSignature = symRef->getOwningMethod(comp())->fieldSignatureChars(symRef->getCPIndex(), len);
    dumpOptDetails(comp(), "got field signature as %.*s\n", len, fieldSignature);
...
bool J9::SymbolReferenceTable::isStaticTypeChar(TR::SymbolReference *symRef)
{
    int32_t len;
    const char *fieldSignature = symRef->getOwningMethod(comp())->staticSignatureChars(symRef->getCPIndex(), len);
    dumpOptDetails(comp(), "got static signature as %.*s\n", len, fieldSignature);
...
bool J9::SymbolReferenceTable::isReturnTypeBool(TR::SymbolReference *symRef)
{
...
void J9::SymbolReferenceTable::addParameters(TR::ResolvedMethodSymbol *methodSymbol)
{
...
TR::SymbolReference *J9::SymbolReferenceTable::findOrCreateStaticSymbol(TR::ResolvedMethodSymbol *owningMethodSymbol,
    int32_t cpIndex, bool isStore)
{
    TR_ResolvedMethod *owningMethod = owningMethodSymbol->getResolvedMethod();
    void *dataAddress;
    ...
    bool resolved = owningMethod->staticAttributes(comp(), cpIndex, &dataAddress, &type, &isVolatile, &isFinal,
        &isPrivate, isStore, &isUnresolvedInCP);
    ...
    if (symRef) {
        ...
        TR::Symbol::RecognizedField recognizedField
            = TR::Symbol::searchRecognizedField(comp(), owningMethod, cpIndex, true);
...
static struct N {
    const char *name;
} pnames[] = { "java/", "javax/", "com/ibm/", "com/sun/" };

void J9::SymbolReferenceTable::checkUserField(TR::SymbolReference *symRef)
{
    ...
    if ((!symRef->getSymbol()->isShadow()
            && (!symRef->getSymbol()->isStatic() || symRef->getSymbol()->isMethod()
                || symRef->getSymbol()->isConstObjectRef()))
        || symRef->getCPIndex() <= 0)
        return;
    ...
    TR_ASSERT(sizeof(pnames) / sizeof(char *) == _numNonUserFieldClasses,
        "Size of package names array is not correct\n");
    int32_t i;
    ...
    if (isNonUserField) {
        ...
        // a user field in any of the other packages. However it is possible to have fields from one package (or class)
        // that we may want to consider as a user field in another package (class) in which case having the different
        // array elements below would then be the way to acomplish this. The population of the array would need to be
        // more selective.
        //
    } else {
        setHasUserField(true);
...
TR::SymbolReference *J9::SymbolReferenceTable::findOrCreateThreadLowTenureAddressSymbolRef()
{
...
TR::SymbolReference *J9::SymbolReferenceTable::findOrCreateThreadHighTenureAddressSymbolRef()
{
    if (!element(highTenureAddressSymbol)) {
        ...
        sym->setDataType(TR::Address);
        sym->setNotCollected();
        element(highTenureAddressSymbol)
            = new (trHeapMemory()) TR::SymbolReference(self(), highTenureAddressSymbol, sym);
...
TR::SymbolReference *J9::SymbolReferenceTable::findOrCreateRamStaticsFromClassSymbolRef()
{
    if (!element(ramStaticsFromClassSymbol)) {
        TR_J9VMBase *fej9 = (TR_J9VMBase *)(fe());
        TR::Symbol *sym = TR::Symbol::createShadow(trHeapMemory(), TR::Address);
        element(ramStaticsFromClassSymbol)
            = new (trHeapMemory()) TR::SymbolReference(self(), ramStaticsFromClassSymbol, sym);
...
TR::SymbolReference *J9::SymbolReferenceTable::findOrCreateJavaLangClassFromClassSymbolRef()
{
    if (!element(javaLangClassFromClassSymbol)) {
        TR_J9VMBase *fej9 = (TR_J9VMBase *)(fe());
        TR::Symbol *sym = TR::Symbol::createShadow(trHeapMemory(), TR::Address);
        element(javaLangClassFromClassSymbol)
            = new (trHeapMemory()) TR::SymbolReference(self(), javaLangClassFromClassSymbol, sym);
...
TR::SymbolReference *J9::SymbolReferenceTable::findOrCreateClassFromJavaLangClassSymbolRef()
{
    if (!element(classFromJavaLangClassSymbol)) {
        TR_J9VMBase *fej9 = (TR_J9VMBase *)(fe());
        TR::Symbol *sym = TR::Symbol::createShadow(trHeapMemory(), TR::Address);
        element(classFromJavaLangClassSymbol)
            = new (trHeapMemory()) TR::SymbolReference(self(), classFromJavaLangClassSymbol, sym);
...
TR::SymbolReference *J9::SymbolReferenceTable::findInitializeStatusFromClassSymbolRef()
{
    return element(initializeStatusFromClassSymbol);
}

TR::SymbolReference *J9::SymbolReferenceTable::findOrCreateInitializeStatusFromClassSymbolRef()
{
    if (!element(initializeStatusFromClassSymbol)) {
        ...
        element(initializeStatusFromClassSymbol)
            = new (trHeapMemory()) TR::SymbolReference(self(), initializeStatusFromClassSymbol, sym);
        element(initializeStatusFromClassSymbol)->setOffset(fej9->getOffsetOfInitializeStatusFromClassField());
    }
    return element(initializeStatusFromClassSymbol);
}

TR::SymbolReference *J9::SymbolReferenceTable::findOrCreateClassRomPtrSymbolRef()
{
...
TR::SymbolReference *J9::SymbolReferenceTable::findOrCreateGlobalFragmentSymbolRef()
{
...
TR::SymbolReference *J9::SymbolReferenceTable::findOrCreateFragmentParentSymbolRef()
{
    if (!element(fragmentParentSymbol)) {
        ...
        element(fragmentParentSymbol)
            ->setOffset(fej9->thisThreadRememberedSetFragmentOffset() + fej9->getFragmentParentOffset());
    }
    return element(fragmentParentSymbol);
}

TR::SymbolReference *J9::SymbolReferenceTable::findOrCreateThreadDebugEventData(int32_t index)
{
    TR_J9VMBase *fej9 = (TR_J9VMBase *)(fe());
    ...
    if (!_currentThreadDebugEventDataSymbol) {
        _currentThreadDebugEventDataSymbol
            = TR::RegisterMappedSymbol::createMethodMetaDataSymbol(trHeapMemory(), "debugEventData");
    ...
    TR::SymbolReference *result
        = new (trHeapMemory()) TR::SymbolReference(self(), _currentThreadDebugEventDataSymbol, offset);
    _currentThreadDebugEventDataSymbolRefs.add(result);
    return result;
}

TR::SymbolReference *J9::SymbolReferenceTable::findUnsafeSymbolRef(TR::DataType type, bool javaObjectReference,
    bool javaStaticReference, TR::Symbol::MemoryOrdering ordering)
{
    TR_Array<TR::SymbolReference *> *unsafeSymRefs = NULL;

    unsafeSymRefs = javaStaticReference ? _unsafeJavaStaticSymRefs[static_cast<int>(ordering)]
...
TR::SymbolReference *J9::SymbolReferenceTable::findOrCreateUnsafeSymbolRef(TR::DataType type, bool javaObjectReference,
    bool javaStaticReference, TR::Symbol::MemoryOrdering ordering)
{
    TR_Array<TR::SymbolReference *> *unsafeSymRefs = NULL;
    ...
    if (javaStaticReference) {
        if (_unsafeJavaStaticSymRefs[static_cast<int>(ordering)] == NULL)
            _unsafeJavaStaticSymRefs[static_cast<int>(ordering)]
                = new (trHeapMemory()) TR_Array<TR::SymbolReference *>(comp()->trMemory(), TR::NumAllTypes);
    ...
    if (symRef == NULL) {
        ...
        (*unsafeSymRefs)[type] = symRef = new (trHeapMemory())
            TR::SymbolReference(self(), sym, comp()->getMethodSymbol()->getResolvedMethodIndex(), -1);
...
void J9::SymbolReferenceTable::performClassLookahead(TR_PersistentClassInfo *classInfo, TR_ResolvedMethod *method)
{
...
int32_t J9::SymbolReferenceTable::userFieldMethodId(TR::MethodSymbol *methodSymbol)
{
    static const char *userField = feGetEnv("TR_UserField");
    if (userField) {
...
int32_t J9::SymbolReferenceTable::immutableConstructorId(TR::MethodSymbol *methodSymbol)
{
    TR::RecognizedMethod method = methodSymbol->getRecognizedMethod();
    switch (method) {
...
bool J9::SymbolReferenceTable::isImmutable(TR::SymbolReference *symRef)
{
    if (!_hasImmutable)
        return false;
...
TR_ImmutableInfo *J9::SymbolReferenceTable::findOrCreateImmutableInfo(TR_OpaqueClassBlock *clazz)
{
    ListElement<TR_ImmutableInfo> *immutableClassInfoElem = _immutableInfo.getListHead();
    while (immutableClassInfoElem) {
        TR_ImmutableInfo *immutableClassInfo = immutableClassInfoElem->getData();
    ...
    TR_ImmutableInfo *newImmutableInfo = new (trHeapMemory()) TR_ImmutableInfo(clazz,
        new (trHeapMemory()) TR_BitVector(_size_hint, comp()->trMemory(), heapAlloc, growable), NULL);
    _immutableInfo.add(newImmutableInfo);
    return newImmutableInfo;
}

void J9::SymbolReferenceTable::checkImmutable(TR::SymbolReference *symRef)
{
    ...
    if (true) {
        if (!symRef->getSymbol()->isArrayShadowSymbol()) {
            ...
            if (sym->isPrivate() || sym->isFinal()) {
                int32_t len;
                char *classNameOfFieldOrStatic
                    = symRef->getOwningMethod(comp())->classNameOfFieldOrStatic(symRef->getCPIndex(), len);
                TR_OpaqueClassBlock *classOfStatic = comp()->fe()->getClassFromSignature(classNameOfFieldOrStatic, len,
                    symRef->getOwningMethod(comp()));

                bool isClassInitialized = false;
                TR_PersistentClassInfo *classInfo
                    = comp()->getPersistentInfo()->getPersistentCHTable()->findClassInfoAfterLocking(classOfStatic,
                        comp());
                if (classInfo && classInfo->isInitialized())
                    isClassInitialized = true;

                if ((classOfStatic != comp()->getSystemClassPointer()) && isClassInitialized
                    && TR::Compiler->cls.isClassFinal(comp(), classOfStatic)) {
                    if (!classInfo->getFieldInfo() && (comp()->getMethodHotness() >= hot)) {
...
TR::SymbolReference *J9::SymbolReferenceTable::findOrCreateImmutableGenericIntShadowSymbolReference(intptr_t offset)
{
    ...
    TR::SymbolReference *symRef = new (trHeapMemory()) TR::SymbolReference(self(), findOrCreateGenericIntShadowSymbol(),
        comp()->getMethodSymbol()->getResolvedMethodIndex(), -1);
    symRef->setOffset(offset);
    return symRef;
}

TR::SymbolReference *J9::SymbolReferenceTable::findOrCreateCheckCastForArrayStoreSymbolRef(TR::ResolvedMethodSymbol *)
{
    return findOrCreateRuntimeHelper(TR_checkCastForArrayStore, false, true, true);
}

TR::SymbolReference *J9::SymbolReferenceTable::findOrCreateProfilingBufferCursorSymbolRef()
{
    if (!element(profilingBufferCursorSymbol)) {
        ...
        sym->setDataType(TR::Address);
        element(profilingBufferCursorSymbol)
            = new (trHeapMemory()) TR::SymbolReference(self(), profilingBufferCursorSymbol, sym);
        element(profilingBufferCursorSymbol)->setOffset(fej9->thisThreadGetProfilingBufferCursorOffset());

        // We can't let the load/store of the exception symbol swing down
        aliasBuilder.addressStaticSymRefs().set(getNonhelperIndex(
            profilingBufferCursorSymbol)); // add the symRef to the statics list to get correct aliasing info
    }
    return element(profilingBufferCursorSymbol);
}

TR::SymbolReference *J9::SymbolReferenceTable::findOrCreateProfilingBufferEndSymbolRef()
{
    if (!element(profilingBufferEndSymbol)) {
        ...
        sym->setDataType(TR::Address);
        element(profilingBufferEndSymbol)
            = new (trHeapMemory()) TR::SymbolReference(self(), profilingBufferEndSymbol, sym);
        element(profilingBufferEndSymbol)->setOffset(fej9->thisThreadGetProfilingBufferEndOffset());

        // We can't let the load/store of the exception symbol swing down
        aliasBuilder.addressStaticSymRefs().set(getNonhelperIndex(
            profilingBufferEndSymbol)); // add the symRef to the statics list to get correct aliasing info
    }
    return element(profilingBufferEndSymbol);
}

TR::SymbolReference *J9::SymbolReferenceTable::findOrCreateVMThreadTempSlotFieldSymbolRef()
{
    if (!element(j9VMThreadTempSlotFieldSymbol)) {
        TR_J9VMBase *fej9 = (TR_J9VMBase *)(fe());
        TR::Symbol *sym
            = TR::RegisterMappedSymbol::createMethodMetaDataSymbol(trHeapMemory(), "j9VMThreadTempSlotField");
        sym->setDataType(TR::Address);
        element(j9VMThreadTempSlotFieldSymbol)
            = new (trHeapMemory()) TR::SymbolReference(self(), j9VMThreadTempSlotFieldSymbol, sym);
...
TR::SymbolReference *J9::SymbolReferenceTable::findOrCreateProfilingBufferSymbolRef(intptr_t offset)
{
...
TR::SymbolReference *J9::SymbolReferenceTable::findShadowSymbol(TR_ResolvedMethod *owningMethod, int32_t cpIndex,
    TR::DataType type, TR::Symbol::RecognizedField *recognizedField)
{
    TR::SymbolReference *symRef;
    TR_SymRefIterator i(type == TR::Address
            ? aliasBuilder.addressShadowSymRefs()
        ...
        self());
    while ((symRef = i.getNext()) != NULL) {
        if ((recognizedField && *recognizedField != TR::Symbol::UnknownField
                && *recognizedField == symRef->getSymbol()->getRecognizedField())
            || (symRef->getSymbol()->getDataType() == type && symRef->getCPIndex() != -1 && cpIndex != -1
                && TR::Compiler->cls.jitFieldsAreSame(comp(), owningMethod, cpIndex, symRef->getOwningMethod(comp()),
                    symRef->getCPIndex(), symRef->getSymbol()->isStatic()))) {
            if (cpIndex != -1 && owningMethod->classOfMethod() != symRef->getOwningMethod(comp())->classOfMethod()) {
                ...
                bool resolved = owningMethod->fieldAttributes(comp(), cpIndex, &offset, &type, &isVolatile, &isFinal,
                    &isPrivate, false, &isUnresolvedInCP, true);
                symRef->setHasBeenAccessedAtRuntime(isUnresolvedInCP ? TR_no : TR_maybe);
            }
            return symRef;
        }
...
TR::SymbolReference *J9::SymbolReferenceTable::findOrCreateClassIsArraySymbolRef()
{
...
TR::SymbolReference *J9::SymbolReferenceTable::findOrCreateArrayComponentTypeSymbolRef()
{
...
TR::SymbolReference *J9::SymbolReferenceTable::findOrCreateVMThreadFloatTemp1SymbolRef()
{
    if (!element(j9VMThreadFloatTemp1Symbol)) {
        ...
        sym->setDataType(TR::Address);
        element(j9VMThreadFloatTemp1Symbol)
            = new (trHeapMemory()) TR::SymbolReference(self(), j9VMThreadFloatTemp1Symbol, sym);
...
TR::SymbolReference *J9::SymbolReferenceTable::findOrCreateObjectInequalityComparisonSymbolRef()
{
...
TR::SymbolReference *J9::SymbolReferenceTable::findOrCreateObjectEqualityComparisonSymbolRef()
{
...
TR::SymbolReference *J9::SymbolReferenceTable::findOrCreateEncodeASCIISymbolRef()
{
...
TR::SymbolReference *J9::SymbolReferenceTable::findOrCreateNonNullableArrayNullStoreCheckSymbolRef()
{
...
TR::SymbolReference *J9::SymbolReferenceTable::findOrCreateLoadFlattenableArrayElementNonHelperSymbolRef()
{
...
TR::SymbolReference *J9::SymbolReferenceTable::findOrCreateStoreFlattenableArrayElementNonHelperSymbolRef()
{
...
TR::SymbolReference *J9::SymbolReferenceTable::findOrCreateIsIdentityObjectNonHelperSymbolRef()
{
...
TR::ParameterSymbol *J9::SymbolReferenceTable::createParameterSymbol(TR::ResolvedMethodSymbol *owningMethodSymbol,
    int32_t slot, TR::DataType type, TR::KnownObjectTable::Index knownObjectIndex)
{
    ...
    if (knownObjectIndex == TR::KnownObjectTable::UNKNOWN)
        symRef
            = new (trHeapMemory()) TR::SymbolReference(self(), sym, owningMethodSymbol->getResolvedMethodIndex(), slot);
    else
        symRef = createTempSymRefWithKnownObject(sym, owningMethodSymbol->getResolvedMethodIndex(), slot,
            knownObjectIndex);
...
TR::SymbolReference *J9::SymbolReferenceTable::findArrayClassRomPtrSymbolRef()
{
    return element(arrayClassRomPtrSymbol);
}

TR::SymbolReference *J9::SymbolReferenceTable::findClassRomPtrSymbolRef() { return element(classRomPtrSymbol); }

const char *J9::SymbolReferenceTable::_commonNonHelperSymbolNames[] = { "<classRomPtr>", "<ramStaticsFromClass>",
    "<componentClassAsPrimitive>", "<initializeStatusFromClass>", "<threadPrivateFlags>", "<arrayletSpineFirstElement>",
    "<dltBlock>", "<classFromJavaLangClassAsPrimitive>", "<javaVM>", "<j9methodExtraField>",
    "<j9methodConstantPoolField>", "<startPCLinkageInfo>", "<instanceShapeFromROMClass>", "<j9VMThreadTempSlotField>" };

const char *J9::SymbolReferenceTable::getNonHelperSymbolName(CommonNonhelperSymbol nonHelper)
{
#ifdef OMR_ENABLE_NONHELPER_EXTENSIBLE_ENUM
    TR_ASSERT_FATAL(nonHelper <= J9lastNonhelperSymbol, "unknown nonhelper %" OMR_PRId32,
        static_cast<int32_t>(nonHelper));
...
void J9::SymbolReferenceTable::flagConstRefSymbol(void *addr, TR::Symbol *sym)
{
