...
public:
    ...
    static void printJITServerVersion()
    {
        ...
        TR_VerboseLog::writeLineLocked(TR_Vlog_JITServer, "JITServer version: %u.%u.%u", MAJOR_NUMBER, MINOR_NUMBER,
            PATCH_NUMBER);
    }

    static bool shouldReadRetry() { return (_numConsecutiveReadErrorsOfSameType < MAX_READ_RETRY); }
...
protected:
    ...
    static const uint16_t MINOR_NUMBER = 103; // ID: /HgmjAVrlw2DV8qPrQ3o
                ...
                if (bytesRead <= 0) {
                    if (EINTR != errno) {
                        if ((errno == _lastReadError) && (EAGAIN != errno))
                            ...
                            _numConsecutiveReadErrorsOfSameType
                                = 0; // If its a new error or errno is EAGAIN then reset the counter
                ...
                if (bytesWritten <= 0) {
                    if (EINTR != errno) {
                        throw JITServer::StreamFailure(
                            "JITServer I/O error: write error: " + std::string(strerror(errno)));
                    }
                } else {
                    totalBytesWritten += bytesWritten;
                }
