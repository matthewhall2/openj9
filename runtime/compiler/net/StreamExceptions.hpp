...
public:
    StreamFailure()
        : _message("Generic stream failure")
        , _retryConnectionImmediately(false)
    {}

    StreamFailure(const std::string &message)
        : _message(message)
        , _retryConnectionImmediately(false)
    {}

    StreamFailure(const std::string &message, bool shouldRetry)
        : _message(message)
        , _retryConnectionImmediately(shouldRetry)
    {}
...
public:
    ...
    virtual const char *what() const throw() { return "Compilation interrupted at JITClient's request"; }
};
...
public:
    ...
    virtual const char *what() const throw() { return "Connection terminated at JITClient's request"; }
};

class StreamClientSessionTerminate : public virtual std::exception {
public:
    StreamClientSessionTerminate(uint64_t clientId)
        : _clientId(clientId)
    {
...
public:
    StreamTypeMismatch(const std::string &message)
        : StreamFailure(message)
    {
        TR_ASSERT(false, "Type mismatch: %s", message.c_str());
    }
};
...
public:
    StreamMessageTypeMismatch()
        : _message("JITServer/JITClient message type mismatch detected")
    {}
    ...
    StreamMessageTypeMismatch(MessageType serverType, MessageType clientType)
    {
        ...
        _message = "JITServer expected message type " + std::to_string(serverType) + " " + expectedName + " received "
            + std::to_string(clientType) + " " + receivedName;
    }
...
public:
    StreamVersionIncompatible()
        : _message("JITServer/JITClient incompatibility detected")
    {}

    StreamVersionIncompatible(const std::string &message)
        : _message(message)
    {}
...
public:
    StreamArityMismatch(const std::string &message)
        : StreamFailure(message)
    {
        TR_ASSERT(false, "Arity mismatch: %s", message.c_str());
    }
};

class ServerCompilationFailure : public virtual std::exception {
public:
    ServerCompilationFailure()
        : _message("Generic JITServer compilation failure")
    {}

    ServerCompilationFailure(const std::string &message)
        : _message(message)
    {}
