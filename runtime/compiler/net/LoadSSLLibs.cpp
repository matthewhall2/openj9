...
int OSSL110_library_init(void) { return (*OOPENSSL_init_ssl)(0, NULL); }
...
long OSSL110_CTX_set_ecdh_auto(SSL_CTX *ctx, int onoff) { return ((onoff) != 0); }

const char *handle_SSL_get_cipher(const SSL *ssl) { return (*OSSL_CIPHER_get_name)((*OSSL_get_current_cipher)(ssl)); }
...
#define OPENSSL102_CHECKED_STACK_OF(type, p) ((_STACK *)(1 ? p : (STACK_OF(type) *)0))

#define OPENSSL102_CHECKED_SK_FREE_FUNC(type, p) ((void (*)(void *))((1 ? p : (void (*)(type *))0)))
...
X509_INFO *Osk102_X509_INFO_value(const STACK_OF(X509_INFO) * st, int i)
{
    ...
    // # def ine SKM_sk_value(type, st,i) \
...
void Osk102_X509_INFO_pop_free(STACK_OF(X509_INFO) * st, OX509_INFO_free_t *X509InfoFreeFunc)
{
    ...
    // # def ine SKM_sk_pop_free(type, st, free_func) \
   //     sk_pop_free(CHECKED_STACK_OF(type, st), CHECKED_SK_FREE_FUNC(type, free_func))
    (*Osk_pop_free)(OPENSSL102_CHECKED_STACK_OF(X509_INFO, st),
        OPENSSL102_CHECKED_SK_FREE_FUNC(X509_INFO, X509InfoFreeFunc));
}
...
void unloadLibssl(void *handle) { (void)dlclose(handle); }

void *findLibsslSymbol(void *handle, const char *symName) { return dlsym(handle, symName); }
...
bool loadLibsslAndFindSymbols()
{
    ...
    if (3 == ossl_ver) {
        OSSL_get_peer_certificate
            = (OSSL_get_peer_certificate_t *)findLibsslSymbol(handle, "SSL_get1_peer_certificate");
    } else {
    ...
    OSSL_CTX_set_session_id_context
        = (OSSL_CTX_set_session_id_context_t *)findLibsslSymbol(handle, "SSL_CTX_set_session_id_context");
    ...
    OEVP_MD_CTX_new
        = (OEVP_MD_CTX_new_t *)findLibsslSymbol(handle, (ossl_ver == 0) ? "EVP_MD_CTX_create" : "EVP_MD_CTX_new");
    OEVP_MD_CTX_free
        = (OEVP_MD_CTX_free_t *)findLibsslSymbol(handle, (ossl_ver == 0) ? "EVP_MD_CTX_destroy" : "EVP_MD_CTX_free");
    ...
    if ((OOpenSSL_version == NULL) ||

        (OSSL_load_error_strings == NULL) || (OSSL_library_init == NULL) || (OOPENSSL_init_ssl == NULL) ||

        (OSSLv23_server_method == NULL) || (OSSLv23_client_method == NULL) ||

        (OEVP_cleanup == NULL) ||

        (OSSL_CTX_ctrl == NULL) || (OBIO_ctrl == NULL) ||

        (Osk_num == NULL) || (Osk_value == NULL) || (Osk_pop_free == NULL) ||

        (OSSL_CIPHER_get_name == NULL) || (OSSL_get_current_cipher == NULL) ||

        (OSSL_new == NULL) || (OSSL_free == NULL) || (OSSL_set_connect_state == NULL) || (OSSL_set_accept_state == NULL)
        || (OSSL_set_fd == NULL) || (OSSL_get_version == NULL) || (OSSL_accept == NULL) || (OSSL_connect == NULL)
        || (OSSL_get_peer_certificate == NULL) || (OSSL_get_verify_result == NULL) || (OSSL_get_error == NULL) ||

        (OSSL_CTX_new == NULL) || (OSSL_CTX_set_session_id_context == NULL) || (OSSL_CTX_use_PrivateKey == NULL)
        || (OSSL_CTX_use_certificate == NULL) || (OSSL_CTX_check_private_key == NULL) || (OSSL_CTX_set_verify == NULL)
        || (OSSL_CTX_free == NULL) || (OSSL_CTX_get_cert_store == NULL) ||

        (OBIO_new_mem_buf == NULL) || (OBIO_free_all == NULL) || (OBIO_new_ssl == NULL) || (OBIO_write == NULL)
        || (OBIO_read == NULL) || (OBIO_test_flags == NULL) ||

        (OPEM_read_bio_PrivateKey == NULL) || (OPEM_read_bio_X509 == NULL) || (OPEM_X509_INFO_read_bio == NULL) ||

        (OX509_INFO_free == NULL) || (OX509_STORE_add_cert == NULL) || (OX509_STORE_add_crl == NULL)
        || (OX509_free == NULL) ||

        (OEVP_MD_CTX_new == NULL) || (OEVP_MD_CTX_free == NULL) || (OEVP_DigestInit_ex == NULL)
        || (OEVP_DigestUpdate == NULL) || (OEVP_DigestFinal_ex == NULL) || (OEVP_sha256 == NULL) ||

        (OERR_print_errors_fp == NULL) || (OERR_peek_error == NULL) || (OERR_get_error == NULL)
    ...
    if (TR::Options::getVerboseOption(TR_VerboseJITServer))
        TR_VerboseLog::writeLineLocked(TR_Vlog_JITServer, "Built against (%s); Loaded with (%s)", OPENSSL_VERSION_TEXT,
            (*OOpenSSL_version)(0));
