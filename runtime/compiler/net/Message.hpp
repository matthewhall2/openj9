...
public:
    ...
    */
    static uint64_t buildFullVersion(uint32_t version, uint32_t config) { return (((uint64_t)config) << 32) | version; }
    ...
    */
    struct DataDescriptor {
        ...
        */
        DataDescriptor(DataType type, uint32_t payloadSize)
            : _type(type)
            , _dataOffset(0)
        ...
        */
        void addInitialPadding(uint8_t initialPadding)
        {
            ...
            TR_ASSERT(initialPadding == 4,
                "Current implementation assumes only 4 bytes of padding"); // because we add 4 bytes to align on a
                                                                           // 8-byte boundary
    ...
    {
        return _buffer.getValueAtOffset<MetaData>(
            sizeof(uint32_t)); // sizeof(uint32_t) represents the space for message size
    }
    ...
    */
    void setSerializedSize(uint32_t serializedSize) { _buffer.writeValue(serializedSize); }
    ...
    */
    void expandBufferIfNeeded(uint32_t requiredSize) { _buffer.expandIfNeeded(requiredSize); }

    /**
       @brief Expand the internal buffer to requiredSize and copy numBytesToCopy
       from the old buffer into the new one
    */
    void expandBuffer(uint32_t requiredSize, uint32_t numBytesToCopy) { _buffer.expand(requiredSize, numBytesToCopy); }
