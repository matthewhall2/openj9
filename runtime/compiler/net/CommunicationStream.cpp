...
void CommunicationStream::initConfigurationFlags()
{
...
bool CommunicationStream::useSSL()
{
    ...
    return (compInfo->getJITServerSslKeys().size() || compInfo->getJITServerSslCerts().size()
...
void CommunicationStream::readMessage(Message &msg)
{
    msg.clearForRead();
...
std::string CommunicationStream::showFullVersionIncompatibility(uint64_t serverFullVersion, uint64_t clientFullVersion)
{
    ...
    if (serverJDKVersion != clientJDKVersion)
        return "JDK version: server " + std::to_string(serverJDKVersion) + ", client "
            + std::to_string(clientJDKVersion);
