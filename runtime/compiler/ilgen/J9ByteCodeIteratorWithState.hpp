...
protected:
    ...
    void patchPartialInliningCallBack(int32_t slot, TR::SymbolReference *symRef, TR::SymbolReference *tempRef,
        TR::TreeTop *tt2) // TR::Block *restartBlock)
    {
    ...
    void addTempForPartialInliningCallBack(int32_t slot, TR::SymbolReference *tempRef, int32_t numParms)
    {
        if (_replacedSymRefs == 0)
            _replacedSymRefs = new (_compilation->trStackMemory())
                TR_Array<TR::SymbolReference *>(_compilation->trMemory(), numParms, true, stackAlloc);
    ...
    TR::TreeTop *genPartialInliningCallBack(int32_t index, TR::TreeTop *callTreeTop)
    {
        ...
        for (si = li.getFirst(), i = 0; si != NULL; si = li.getNext(), i++) {
            TR::SymbolReference *ref
                = _compilation->getSymRefTab()->findOrCreateAutoSymbol(_methodSymbol, si->getSlot(), si->getDataType());
            ...
            {
                TR::Node *thisPointer
                    = TR::Node::createWithSymRef(_compilation->il.opCodeForDirectLoad(si->getDataType()), 0, ref);
                callNode->setAndIncChild(0,
                    TR::Node::createWithSymRef(TR::aloadi, 1, 1, thisPointer,
                        _compilation->getSymRefTab()->findOrCreateVftSymbolRef()));
                i++;
            }
            TR::Node *child
                = TR::Node::createWithSymRef(_compilation->il.opCodeForDirectLoad(si->getDataType()), 0, ref);
            callNode->setAndIncChild(i,
                child); // setChild doesn't modify the ref count.  setAndIncChild() will properly inc the ref count.
        }
    ...
    bool hasExceptionHandlers() { return method()->numberOfExceptionHandlers() > 0; }

    // how many bytes is the given operand stack element's result?
    int32_t getSize(TR::Node *n) { return n->getSize(); }

    // what is the data type of the operand stack element's result?
    TR::DataType getDataType(TR::Node *n) { return n->getDataType(); }
