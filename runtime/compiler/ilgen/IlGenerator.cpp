...
TR_J9ByteCodeIlGenerator::TR_J9ByteCodeIlGenerator(TR::IlGeneratorMethodDetails &methodDetails,
    TR::ResolvedMethodSymbol *methodSymbol, TR_J9VMBase *fe, TR::Compilation *comp, TR::SymbolReferenceTable *symRefTab,
    bool forceClassLookahead, TR_InlineBlocks *blocksToInline,
    int32_t argPlaceholderSlot) // TR_ScratchList<TR_InlineBlock> *blocksToInline)
    ...
    , _requiredConsts(
          (comp->currentILGenCallTarget() == NULL || comp->currentILGenCallTarget()->_requiredConsts.empty())
              ? NULL
    ...
    , _foldedRequiredConsts(NULL)
{
    ...
    if ((forceClassLookahead
        ...
        _classInfo = comp->getPersistentInfo()->getPersistentCHTable()->findClassInfoAfterLocking(
            method()->containingClass(), comp, allowForAOT);
    } else {
    ...
    if (comp->getOption(TR_EnableOSR) && !comp->isPeekingMethod() && comp->isOSRTransitionTarget(TR::postExecutionOSR)
        && !_cannotAttemptOSR)
        _processedOSRNodes = new (trStackMemory()) TR::NodeChecklist(comp);
}

bool TR_J9ByteCodeIlGenerator::genIL()
{
    ...
    if (_requiredConsts != NULL) {
        _foldedRequiredConsts = new (stackMemoryRegion) TR::set<int32_t>(stackMemoryRegion);
    }
    ...
    if (success) {
        ...
        if (!strcmp(methodName,
                "com/ibm/jit/DecimalFormatHelper.formatAsDouble(Ljava/text/DecimalFormat;Ljava/math/BigDecimal;)Ljava/"
                "lang/String;")
            || !strcmp(methodName,
                "com/ibm/jit/DecimalFormatHelper.formatAsFloat(Ljava/text/DecimalFormat;Ljava/math/BigDecimal;)Ljava/"
                "lang/String;"))
    ...
    if (success && !comp()->isPeekingMethod()) {
        ...
        for (TR::Block *block = _methodSymbol->getFirstTreeTop()->getEnclosingBlock(); block;
             block = block->getNextBlock())
            _methodSymbol->addProfilingOffsetInfo(block->getEntry()->getNode()->getByteCodeIndex(),
                block->getExit()->getNode()->getByteCodeIndex());
    }
...
void TR_J9ByteCodeIlGenerator::assertFoldedAllRequiredConsts()
{
    ...
    while (siteIndex >= 0) {
        ...
        const char *sig = fe()->sampleSignature(ics._methodInfo, sigBuf, sizeof(sigBuf), comp()->trMemory());
...
bool TR_J9ByteCodeIlGenerator::internalGenIL()
{
    ...
    if (recognizedMethod != TR::unknownMethod) {
        if (recognizedMethod == TR::com_ibm_jit_JITHelpers_supportsIntrinsicCaseConversion
            && !TR::Compiler->om.canGenerateArraylets()) {
            if (performTransformation(comp(),
                    "O^O IlGenerator: Generate com/ibm/jit/JITHelpers.supportsIntrinsicCaseConversion\n")) {
        ...
        if (recognizedMethod == TR::com_ibm_dataaccess_DecimalData_JITIntrinsicsEnabled) {
            if (performTransformation(comp(),
                    "O^O IlGenerator: Generate com/ibm/dataaccess/DecimalData.JITIntrinsicsEnabled\n")) {
        ...
        if (recognizedMethod == TR::com_ibm_rmi_io_FastPathForCollocated_isVMDeepCopySupported) {
            if (performTransformation(comp(),
                    "O^O IlGenerator: Generate com/ibm/rmi/io/FastPathForCollocated/isVMDeepCopySupported\n")) {
...
bool TR_J9ByteCodeIlGenerator::genILFromByteCodes()
{
    ...
    // need to do some more work for static methods, since the class sync object in static sync frames must always be
    // the "current" class.
    //
    if (_methodSymbol->isSynchronised()) {
        if (comp()->getOption(TR_FullSpeedDebug) || !comp()->getOption(TR_DisableLiveMonitorMetadata)) {
            ...
            if (comp()->getOption(TR_FullSpeedDebug))
                symRef = symRefTab()->findOrCreateAutoSymbol(_methodSymbol, _methodSymbol->getSyncObjectTempIndex(),
                    TR::Address);
            else
    ...
    if (_methodSymbol->getResolvedMethod()->isNonEmptyObjectConstructor()) {
        if (comp()->getOption(TR_FullSpeedDebug)) {
            TR::SymbolReference *symRef = symRefTab()->findOrCreateAutoSymbol(_methodSymbol,
                _methodSymbol->getThisTempForObjectCtorIndex(), TR::Address);
    ...
    if (!comp()->getOption(TR_DisableGuardedCountingRecompilations) && comp()->getRecompilationInfo()
        && comp()->getRecompilationInfo()->shouldBeCompiledAgain() && !comp()->getRecompilationInfo()->isRecompilation()
        && // only do it for first time compilations
        (!comp()->getPersistentInfo()->_countForRecompile || comp()->getOption(TR_EnableMultipleGCRPeriods))
        && comp()->isOutermostMethod() && comp()->getOptions()->getInsertGCRTrees() && !comp()->isDLT()
        && !method()->isJNINative()) {
        ...
        if (_methodSymbol->mayHaveInlineableCall() ||
            ...
            comp()
                ->getOptimizationPlan()
                ->resetAddToUpgradeQueue(); // do not add to upgrade queue methods for which we used GCR
            // stats
            comp()->getPersistentInfo()->incNumGCRBodies();
        } else {
            // TR_VerboseLog::writeLineLocked(TR_Vlog_PERF,"Saved a GCR gen body nmayHaveInlineableCall=%d
            // mayHaveLoops=%d _maxByteCodeIndex=%d", _methodSymbol->mayHaveInlineableCall(),
            // _methodSymbol->mayHaveLoops(), _maxByteCodeIndex);
            //  stats
    ...
    if (comp()->isOutermostMethod() && // Only do it once for the method to be compiled
        !comp()->getOptions()->isDisabled(
            OMR::samplingJProfiling)) // If heuristic enabled samplingJProfiling for this body
    {
        ...
        if ((comp()->isDLT()
            ...
            && (_methodSymbol->mayHaveInlineableCall() || _methodSymbol->hasCheckcastsOrInstanceOfs()
    ...
    if (fpIndex != -1)
        _unimplementedOpcode = _code[fpIndex];
    ...
    TR_BoolArrayStoreTransformer::NodeSet bstoreiUnknownArrayTypeNodes(std::less<TR::Node *>(),
        comp()->trMemory()->currentStackRegion());
    TR_BoolArrayStoreTransformer::NodeSet bstoreiBoolArrayTypeNodes(std::less<TR::Node *>(),
        comp()->trMemory()->currentStackRegion());
    ...
    for (; currTree != NULL; currTree = currTree->getNextTreeTop()) {
        ...
        if (currNode->getNumChildren() >= 1 && currNode->getFirstChild()->getOpCode().isCall()
        ...
        if ((opcode.isStoreDirect() && opcode.hasSymbolReference()
                && currNode->getSymbolReference()->getSymbol()->isAutoOrParm())
            || opcode.isCheckCast()) {
            ...
            if (opcode.isStoreDirect())
                ...
                typeNode = currNode->getSecondChild(); // checkcast
            ...
            if (arrayBase->getOpCode().hasSymbolReference()) {
                if (boolArrayStoreTransformer.isBoolArrayNode(arrayBase)) {
                    logprintf(comp()->getOption(TR_TraceILGen), comp()->log(),
                        "bstorei node n%dn is bool array store\n", currNode->getGlobalIndex());
        ...
        if (currNode->getOpCodeValue() == TR::checkcast && currNode->getSecondChild()->getOpCodeValue() == TR::loadaddr
            && currNode->getSecondChild()->getSymbolReference()->isUnresolved()) {
            unresolvedCheckcastTopsNeedingNullGuard.add(currTree);
        } else if (currNode->getOpCodeValue() == TR::treetop && currNode->getFirstChild()->getOpCodeValue() == TR::
                       instanceof &&currNode->getFirstChild()->getSecondChild()->getOpCodeValue() == TR::loadaddr
                       && currNode->getFirstChild()->getSecondChild()->getSymbolReference()->isUnresolved()) {
            unresolvedInstanceofTops.add(currTree);
        } else if (_invokeSpecialInterfaceCalls != NULL && currNode->getNumChildren() >= 1
            && currNode->getFirstChild()->getOpCode().isCallDirect()
            && !currNode->getFirstChild()->isPotentialOSRPointHelperCall()
            && _invokeSpecialInterfaceCalls->isSet(currNode->getFirstChild()->getByteCodeIndex())
        ...
        if (opcode.isResolveOrNullCheck()) {
            ...
            if (opcode.isCallIndirect() && !firstChild->getSymbol()->castToMethodSymbol()->isComputed()) {
                TR::Node *receiver;
                ...
                bool canCopyReceiver
                    = symRef && symRef->getSymbol()->isAutoOrParm() && !autoOrParmSymRefList.find(symRef);
                if (canCopyReceiver) {
        ...
        if (needMonitor && primitive) {
            if ((currNode->getOpCode().isStore() && currNode->getSymbol()->holdsMonitoredObject()
                    && !currNode->isLiveMonitorInitStore())
                || currNode->getOpCode().getOpCodeValue() == TR::monexitfence) {
            ...
            } else if (currNode->exceptionsRaised() != 0 || currNode->canCauseGC()) {
                primitive = false;
                continue;
            }
        }
    ...
    if (needMonitor) {
        if (primitive && monentTree && monexitTree && monentStore && monexitStore) {
...
TR::Block *TR_J9ByteCodeIlGenerator::cloneHandler(TryCatchInfo *handlerInfo, TR::Block *firstBlock,
    TR::Block *lastBlock, TR::Block *lastBlockInMethod, List<TR::Block> *clonedCatchBlocks)
{
...
TR::Block *TR_J9ByteCodeIlGenerator::genExceptionHandlers(TR::Block *lastBlock)
{
    ...
    for (auto handlerInfoIter = _tryCatchInfo.begin(); handlerInfoIter != _tryCatchInfo.end(); ++handlerInfoIter) {
        ...
        //
        if (_blocksToInline
            && !_blocksToInline->isInExceptionList(
                firstIndex)) // Case 1: item is not in exception list, therefore no ilgen to be done on it
        {
            continue; // nothing to be done for this handler!
        } else if (_blocksToInline && _blocksToInline->isInExceptionList(firstIndex)
            ...
            && !isGenerated(firstIndex)) // Case 2: item is in exception list, but not in list of blocks to be ilgen'd
        {
            _blocksToInline->hasGeneratedRestartTree()
                ? genGotoPartialInliningCallBack(firstIndex, _blocksToInline->getGeneratedRestartTree())
                : _blocksToInline->setGeneratedRestartTree(
                      genPartialInliningCallBack(firstIndex, _blocksToInline->getCallNodeTreeTop()));
            ...
            // ok what I'm trying here is saying that my first block, last block and catchblock in my catcher are all
            // the same (the one block)

            handlerInfo._catchBlock->setHandlerInfo(handlerInfo._catchType, (uint8_t)comp()->getInlineDepth(),
                handlerInfoIter - _tryCatchInfo.begin(), method(), comp());
            continue;
        }
        ...
        if (isGenerated(firstIndex)) {
            ...
            if (!dupHandler) {
                ...
                // comp()->failCompilation<TR::CompilationException>("can't figure out why the handler is already
                // generated");
            }

            if (!generateNewBlock) {
                if (handlerInfo._catchType != dupHandler->_catchType) {
                    lastBlock = cloneHandler(&handlerInfo, dupHandler->_firstBlock, dupHandler->_lastBlock, lastBlock,
                        &clonedCatchBlocks);
                    /*
                    TR_BlockCloner cloner(cfg());
        ...
        if (generateNewBlock) {
            ...
            if (handlerBlockFromNonExceptionControlFlow && _stack->topIndex() == 0) {
            ...
            exceptionLoad->setIsNonNull(true);
            ...
            if (handlerBlockFromNonExceptionControlFlow) {
                ...
                if (lastNode->getOpCode().isResolveOrNullCheck() || (lastNode->getOpCodeValue() == TR::treetop))
                    lastNode = lastNode->getFirstChild();

                TR::ILOpCode &lastOpCode = lastNode->getOpCode();
                if (!lastOpCode.isGoto() && !lastOpCode.isReturn() && (lastOpCode.getOpCodeValue() != TR::athrow)) {
                    comp()->failCompilation<TR::ILGenFailure>("Aborting: not goto, not return and not a throw.");
                }
            }

            if (handlerBlockFromNonExceptionControlFlow) {
                lastBlock = cloneHandler(&handlerInfo, handlerBlockFromNonExceptionControlFlow,
                    handlerBlockFromNonExceptionControlFlow, lastBlock, &clonedCatchBlocks);
    ...
    for (auto handlerInfoIter = _tryCatchInfo.begin(); handlerInfoIter != _tryCatchInfo.end(); ++handlerInfoIter) {
        TryCatchInfo &handlerInfo = *handlerInfoIter;
        if (_blocksToInline
            && !_blocksToInline->isInExceptionList(handlerInfo._handlerIndex)) // Case 1: item is not in exception list,
                                                                               // therefore no ilgen to be done on it
        {
            continue; // nothing to be done for this handler!
        }
        TR::Block *catchBlock = handlerInfo._catchBlock;
        TR::Block *restartBlock
            = _blocksToInline ? _blocksToInline->getGeneratedRestartTree()->getEnclosingBlock() : NULL;
        // Checking for a preceding J9BCmonitorenter only make sense when the try region starts at a bytecode index
        // above 0
        if (handlerInfo._startIndex > 0) {
            ...
            if (precedingBytecode == J9BCmonitorenter && lastBytecode == J9BCmonitorexit)
                catchBlock->setIsSynchronizedHandler(); // monitorenter preceding try region that ends in monitorexit
                                                        // means synchronized
        }
...
TR::Node *TR_J9ByteCodeIlGenerator::genMethodEnterHook()
{
    if (method()->isStatic())
        return TR::Node::createWithSymRef(TR::MethodEnterHook, 0,
            symRefTab()->findOrCreateReportStaticMethodEnterSymbolRef(_methodSymbol));

    loadAuto(TR::Address, 0);
    return TR::Node::createWithSymRef(TR::MethodEnterHook, 1, 1, pop(),
        symRefTab()->findOrCreateReportMethodEnterSymbolRef(_methodSymbol));
}

void TR_J9ByteCodeIlGenerator::prependEntryCode(TR::Block *firstBlock)
{
    bool trace = comp()->getOption(TR_TraceILGen);
    ...
    if (_methodSymbol->isSynchronised()) {
        ...
        if (firstChild->getOpCodeValue() == TR::loadaddr && firstChild->getSymbol()->isClassObject()) {
            monitorEnter = TR::Node::createWithSymRef(TR::aloadi, 1, 1, firstChild,
        ...
        if (_methodSymbol->getSyncObjectTemp()) {
            if (_methodSymbol->isStatic())
                loadSymbol(TR::loadaddr,
                    symRefTab()->findOrCreateClassSymbol(_methodSymbol, 0, method()->containingClass()));
            else
                loadAuto(TR::Address, 0);

            lockObject = pop();
            if (monitorEnter->getFirstChild()->getOpCodeValue() == TR::aloadi
                && monitorEnter->getFirstChild()->getSymbolReference()
                    == symRefTab()->findJavaLangClassFromClassSymbolRef())
    ...
    if ((fej9()->isMethodTracingEnabled(_methodSymbol->getResolvedMethod()->getPersistentIdentifier())
            || (!comp()->getOption(TR_FullSpeedDebug) && TR::Compiler->vm.canMethodEnterEventBeHooked(comp())))
...
void TR_J9ByteCodeIlGenerator::prependGuardedCountForRecompilation(TR::Block *originalFirstBlock)
{
    //
    ...
    if (comp()->getOption(TR_ImmediateCountingRecompilation)) {
        ...
        TR::Node *loadFlagNode = TR::Node::createWithSymRef(node, TR::iload, 0,
            comp()->getSymRefTab()->findOrCreateCountForRecompileSymbolRef());

        if (comp()->getOption(TR_EnableGCRPatching))
            cmpFlagNode = TR::Node::createif(TR::ificmpne, loadFlagNode, TR::Node::create(node, TR::iconst, 0, 1),
                originalFirstBlock->getEntry());
        else
            cmpFlagNode = TR::Node::createif(TR::ificmpeq, loadFlagNode, TR::Node::create(node, TR::iconst, 0, 0),
                originalFirstBlock->getEntry());
    }
    TR::TreeTop *cmpFlag = TR::TreeTop::create(comp(), cmpFlagNode);
    guardBlock->append(cmpFlag);
    TR::DebugCounter::prependDebugCounter(comp(),
        TR::DebugCounter::debugCounterName(comp(), "gcrMethods/byJittedBody/(%s)", comp()->signature()), cmpFlag, 1,
    ...
    TR::DebugCounter::prependDebugCounter(comp(),
        TR::DebugCounter::debugCounterName(comp(), "gcrCounterBumps/byJittedBody/(%s)", comp()->signature()), treeTop,
        1, TR::DebugCounter::Free);
    TR::Node *bumpNode = treeTop->getNode()->getNumChildren() > 1 ? treeTop->getNode()->getSecondChild()
    ...
    callRecompileBlock->append(TR::TreeTop::createResetTree(comp(), node,
    ...
    if (comp()->getOption(TR_EnableGCRPatching)) {
        ...
        callRecompileBlock->append(TR::TreeTop::create(comp(),
            TR::Node::createWithSymRef(TR::bstore, 1, 1, constNode,
...
void TR_J9ByteCodeIlGenerator::createGeneratedFirstBlock()
{
...
bool TR_J9ByteCodeIlGenerator::hasFPU()
{
    bool result = !comp()->getOption(TR_DisableFPCodeGen) ? comp()->target().cpu.hasFPU() : false;
    return result;
}

bool TR_J9ByteCodeIlGenerator::genJNIIL()
{
    if (!cg()->getSupportsDirectJNICalls() || comp()->getOption(TR_DisableDirectToJNI)
        || (comp()->compileRelocatableCode() && !cg()->supportsDirectJNICallsForAOT()))
        return false;

    // A JNI thunk method cannot call back to the slow path, as would occur in the following case
    if (method()->numberOfParameterSlots() > J9_INLINE_JNI_MAX_ARG_COUNT
        && comp()->cg()->hasFixedFrameC_CallingConvention())
        return false;
...
void TR_J9ByteCodeIlGenerator::genJITIntrinsicsEnabled()
{
...
void TR_J9ByteCodeIlGenerator::genJavaLangSystemIdentityHashCode()
{
    ...
    TR::Node *objectRef
        = TR::Node::createLoad(symRefTab()->findOrCreateAutoSymbol(_methodSymbol, p->getSlot(), p->getDataType()));
    ...
    //
    objHeaderFlagsField
        = TR::Node::createWithSymRef(TR::iloadi, 1, 1, objectRef, symRefTab->findOrCreateHeaderFlagsSymbolRef());
    opNode
        = TR::Node::create(TR::ishr, 2, objHeaderFlagsField, TR::Node::create(objHeaderFlagsField, TR::iconst, 0, 16));
...
bool TR_J9ByteCodeIlGenerator::genNewInstanceImplThunk()
{
    ...
    TR_OpaqueClassBlock *classId
        = method()->classOfMethod(); // will never be java.lang.Class (unless we are in a hacky world, e.g. JarTester?)

    TR_ResolvedMethod *ctor = fej9()->getDefaultConstructor(trMemory(), classId);
    if (!ctor || TR::Compiler->cls.isAbstractClass(comp(), classId))
        return false;
    ...
    if (!((TR_J9VM *)fej9())->isPublicClass(classId) || !ctor->isPublic()) {
        ...
        // the call to findOrCreateClassSymbol is safe even though we pass CPI of -1 since it is guarded by !isAOT check
        // in createResolvedMethodWithSignature
    ...
    // the call to findOrCreateClassSymbol is safe even though we pass CPI of -1 since it is guarded by !isAOT check in
    // createResolvedMethodWithSignature
...
TR::Node *TR_J9ByteCodeIlGenerator::genNewInstanceImplCall(TR::Node *classNode)
{
    ...
    TR::Node *classNodeAsClass = TR::Node::createWithSymRef(TR::aloadi, 1, 1, classNode,
        symRefTab()->findOrCreateClassFromJavaLangClassSymbolRef());
    // the call to findOrCreateClassSymbol is safe even though we pass CPI of -1 since we check for
    // !compileRelocatableCode() in the caller
    TR::Node *node = TR::Node::createWithSymRef(TR::loadaddr, 0,
        symRefTab()->findOrCreateClassSymbol(_methodSymbol, -1, caller->classOfMethod()));
    TR::Node *nodeAsObject = TR::Node::createWithSymRef(TR::aloadi, 1, 1, node,
        symRefTab()->findOrCreateJavaLangClassFromClassSymbolRef());

    TR::Node *callNode = TR::Node::createWithSymRef(TR::acalli, 3, 3, classNodeAsClass, classNode, nodeAsObject,
        symRefTab()->findOrCreateObjectNewInstanceImplSymbol(_methodSymbol));

    return callNode;
}

void TR_J9ByteCodeIlGenerator::genDLTransfer(TR::Block *firstBlock)
{
    ...
    if (nestedCnt > 1 || (innermostPair != NULL && dltBCIndex != innermostPair->_toIndex)) {
        ...
        TR::Node *storeNode
            = TR::Node::createWithSymRef(TR::istore, 1, 1, TR::Node::create(TR::iconst, 0, 0), dltSteerSymRef);
    ...
    if (_blocks[dltBCIndex] != NULL) {
        ...
        for (steerTarget = myBlock->getEntry();
             steerTarget != myBlock->getExit() && steerTarget->getNode()->getByteCodeIndex() != dltBCIndex;
             steerTarget = steerTarget->getNextTreeTop())
            ;
        TR_ASSERT(steerTarget->getNode()->getByteCodeIndex() == dltBCIndex,
            "Cannot find the treeTop DLT point belongs to");
    ...
    TR::TreeTop **haveSeenTargets
        = (TR::TreeTop **)comp()->trMemory()->allocateMemory(sizeof(void *) * (nestedCnt + 1), heapAlloc);
    ...
    if (fej9()->isLiveMonitorMapCorrectnessRequired() && _methodSymbol->mayContainMonitors()) {
        TR_ASSERT_FATAL(!comp()->getOption(TR_DisableLiveMonitorMetadata),
            "Live monitor metadata must not be disabled if the JVM requires it to be correct.\n");
        ...
        for (TR::Block *block = firstBlock; block; block = block->getNextBlock()) {
            ...
            for (; tt != block->getExit(); tt = tt->getNextTreeTop()) {
                ...
                if (node->getOpCodeValue() == TR::monent
                    || (node->getNumChildren() == 1 && node->getFirstChild()->getOpCodeValue() == TR::monent)) {
        ...
        for (int32_t targetIdx = 0; targetIdx < haveSeenCount; targetIdx++) {
            if (hasMonentOnPredecessorPath(haveSeenTargets[targetIdx]->getNode()->getBlock(), hasMonent, visited)) {
                comp()->failCompilation<TR::ILGenFailure>(
                    "DLT into region where live monitor map correctness required, but cannot be guaranteed");
            }
        }
    }
    ...
    if (dltSteerSymRef != NULL) {
        TR::Node *storeNode
            = TR::Node::createWithSymRef(TR::istore, 1, 1, TR::Node::create(TR::iconst, 0, 1), dltSteerSymRef);
        storeDltSteer = TR::TreeTop::create(comp(), storeNode);
    ...
    for (; parmIteratorIdx < parmSlots; parmIteratorIdx++) {
        ...
        for (; slotSymRef; slotSymRef = si.getNext()) {
            if (slotSymRef != parmSymRef) {
                ...
                bool zeroIt = ((OSlotParm && dataType != TR::Address) || (!OSlotParm && dataType == TR::Address));

                TR_ASSERT(slotSymRef->getSymbol()->getParmSymbol() == NULL, "Multiple parms on the same slot");
                if (!zeroIt) {
                    int32_t realOffset
                        = (tempSlots + parmSlots - parmIteratorIdx - 1) * TR::Compiler->om.sizeofReferenceAddress();
                    ...
                    if (addrNode == NULL || nodeRealOffset != realOffset) {
                        ...
                        if (comp()->target().is64Bit()) {
                            ...
                            addrNode = TR::Node::create(TR::aiadd, 2, dltBufChild,
                                TR::Node::create(TR::iconst, 0, realOffset));
                    }
                    loadNode = TR::Node::createWithSymRef(comp()->il.opCodeForIndirectLoad(dataType), 1, 1, addrNode,
                        shadowSymRef);
                } else {
                    switch (dataType) {
                        case TR::Int32:
                ...
                storeNode
                    = TR::Node::createWithSymRef(comp()->il.opCodeForDirectStore(dataType), 1, 1, loadNode, slotSymRef);
    ...
    if (tempSlots != 0) {
        ...
        for (int32_t i = 0; i < tempSlots; i++) {
            ...
            for (; symRef; symRef = iterator.getNext()) {
                ...
                bool zeroIt = ((isOSlot && dataType != TR::Address) || (!isOSlot && dataType == TR::Address));
                if (!zeroIt) {
                ...
                storeNode
                    = TR::Node::createWithSymRef(comp()->il.opCodeForDirectStore(dataType), 1, 1, loadNode, symRef);
                newFirstBlock->append(TR::TreeTop::create(comp(), storeNode));
            }
        }
    }

    for (auto nextSymRef = comp()->getMonitorAutoSymRefsInCompiledMethod()->begin();
         nextSymRef != comp()->getMonitorAutoSymRefsInCompiledMethod()->end(); ++nextSymRef) {
        TR_ASSERT(!comp()->getOption(TR_MimicInterpreterFrameShape),
            "cannot initialize sync temp symRefs in DLT block when autos can alias each other\n");
...
bool TR_J9ByteCodeIlGenerator::hasMonentOnPredecessorPath(TR::CFGNode *currBlock, TR_BitVector &hasMonent,
...
void TR_J9ByteCodeIlGenerator::inlineJitCheckIfFinalizeObject(TR::Block *firstBlock)
{
    ...
    for (TR::Block *block = firstBlock; block; block = block->getNextBlock()) {
        ...
        for (; tt != block->getExit(); tt = tt->getNextTreeTop()) {
            ...
            //
            if (node->getOpCode().isCall() && (node->getSymbolReference() == finalizeSymRef)) {
                TR::Node *vftLoad = TR::Node::createWithSymRef(TR::aloadi, 1, 1, node->getFirstChild(),
                    comp()->getSymRefTab()->findOrCreateVftSymbolRef());

                TR::ILOpCodes loadOp = is64bit ? TR::lloadi : TR::iloadi;

                TR::Node *classDepthAndFlagsNode = TR::Node::createWithSymRef(loadOp, 1, 1, vftLoad,
                ...
                TR::Node *andNode
                    = TR::Node::create(is64bit ? TR::land : TR::iand, 2, classDepthAndFlagsNode, andConstNode);
                ...
                TR::Node *cmp
                    = TR::Node::createif(is64bit ? TR::iflcmpne : TR::ificmpne, andNode, constCheckNode, NULL);
...
struct TR_J9ByteCodeIlGenerator::methodRenamePair TR_J9ByteCodeIlGenerator::_decFormatRenames[_numDecFormatRenames] = {
    ...
    { "com/ibm/jit/DecimalFormatHelper$FieldPosition.getFieldDelegate()Lcom/ibm/jit/DecimalFormatHelper$FieldDelegate;",
     "java/text/FieldPosition.getFieldDelegate()Ljava/text/Format$FieldDelegate;"                                                                                   },
    {                       "com/ibm/jit/DecimalFormatHelper$FieldPosition.getFieldAttribute()Ljava/text/Format$Field;",
    ...
    {                                       "com/ibm/jit/DecimalFormatHelper.isGroupingUsed(Ljava/text/NumberFormat;)Z",
     "java/text/NumberFormat.isGroupingUsed()Z"                                                                                                                     },
    {        "com/ibm/jit/DecimalFormatHelper.getBigDecimalMultiplier(Ljava/text/DecimalFormat;)Ljava/math/BigDecimal;",
     "java/text/DecimalFormat.getBigDecimalMultiplier()Ljava/math/BigDecimal;"                                                                                      },
    {                                          "com/ibm/jit/DecimalFormatHelper.subformat(Ljava/text/DecimalFormat;Ljava/lang/StringBuffer;Lcom/ibm/jit/"
                                          "DecimalFormatHelper$FieldDelegate;ZZIIII)Ljava/lang/StringBuffer;",                      "java/text/DecimalFormat.subformat(Ljava/lang/StringBuffer;Ljava/text/Format$FieldDelegate;ZZIIII)Ljava/lang/"
                      "StringBuffer;"            },
};
...
{
    for (int i = 0; i < _numDecFormatRenames; i++) {
        _decFormatRenamesDstSymRef[i]
            = fej9()->findOrCreateMethodSymRef(comp(), _methodSymbol, _decFormatRenames[i].dstMethodSignature);
    }
...
{
    if (!node->getOpCode().isCall() || !node->getOpCode().hasSymbolReference())
        return true;
    if (node->getSymbolReference()->getSymbol()->castToMethodSymbol()->isHelper())
        return true;
    ...
    for (int i = 0; i < _numDecFormatRenames; i++) {
        if (!strcmp(nodeName, _decFormatRenames[i].srcMethodSignature)) {
            if (performTransformation(comp(), "%sreplaced %s by %s in [%p]\n", OPT_DETAILS,
                    _decFormatRenames[i].srcMethodSignature, _decFormatRenames[i].dstMethodSignature, node)) {
...
{
    if ((!node->getOpCode().isLoad() && !node->getOpCode().isStore()) || !node->getOpCode().hasSymbolReference())
        return false;
    TR::SymbolReference *symRef = node->getSymbolReference();
    TR::Symbol *sym = symRef->getSymbol();
    if (sym == NULL || symRef->getCPIndex() < 0)
        return false;
    ...
    switch (sym->getKind()) {
        case TR::Symbol::IsStatic: {
            ...
            int32_t nonhelperIndex
                = comp->getSymRefTab()->getNonhelperIndex(comp->getSymRefTab()->getLastCommonNonhelperSymbol());
            int32_t numHelperSymbols = comp->getSymRefTab()->getNumHelperSymbols();
            if ((index < numHelperSymbols) || (index < nonhelperIndex) || !sym->isStaticField())
                return false;
...
bool TR_J9ByteCodeIlGenerator::replaceField(TR::Node *node, const char *destClass, const char *destFieldName,
    const char *destFieldSignature, int parmIndex)
{
    ...
    if (performTransformation(comp(), "%ssymref replaced by %s.%s %s in [%p]\n", OPT_DETAILS, destClass, destFieldName,
        ...
        TR::SymbolReference *symRef = new (trHeapMemory()) TR::SymbolReference(comp()->getSymRefTab(), sym,
            comp()->getMethodSymbol()->getResolvedMethodIndex(), -1, 0);
        ...
        if (!node->getOpCode().isIndirect()) {
            ...
            if (parmIndex == 1)
                p = parms.getNext();
            TR::Node *decFormObjLoad = TR::Node::createLoad(node,
                symRefTab()->findOrCreateAutoSymbol(_methodSymbol, p->getSlot(), p->getDataType()));
...
bool TR_J9ByteCodeIlGenerator::replaceStatic(TR::Node *node, const char *dstClassName, const char *staticName,
    const char *type)
{
    TR_OpaqueClassBlock *c
        = fej9()->getClassFromSignature(dstClassName, strlen(dstClassName), comp()->getCurrentMethod());
    ...
    if (dataAddress != NULL && !node->getSymbolReference()->isUnresolved()
...
{
    bool result = true;
    if (matchFieldOrStaticName(comp(), node,
            "com/ibm/jit/DecimalFormatHelper.INSTANCE Lcom/ibm/jit/DecimalFormatHelper$FieldPosition;")) {
    ...
    } else if (matchFieldOrStaticName(comp(), node,
                   "com/ibm/jit/DecimalFormatHelper.digitList Lcom/ibm/jit/DecimalFormatHelper$DigitList;")) {
        ...
        result = replaceField(node, "java/math/BigDecimal", "cachedScale", "I", 1);
                   ...
                   "com/ibm/jit/DecimalFormatHelper.symbols Ljava/text/DecimalFormatSymbols;")) {
        ...
        result = replaceField(node, "java/text/DecimalFormat", "useExponentialNotation", "Z", 0);
                   ...
                   "com/ibm/jit/DecimalFormatHelper.negativePrefix Ljava/lang/String;")) {
        result = replaceField(node, "java/text/DecimalFormat", "negativePrefix", "Ljava/lang/String;", 0);
    } else if (matchFieldOrStaticName(comp(), node,
                   "com/ibm/jit/DecimalFormatHelper.negativeSuffix Ljava/lang/String;")) {
        result = replaceField(node, "java/text/DecimalFormat", "negativeSuffix", "Ljava/lang/String;", 0);
    } else if (matchFieldOrStaticName(comp(), node,
                   "com/ibm/jit/DecimalFormatHelper.positivePrefix Ljava/lang/String;")) {
        result = replaceField(node, "java/text/DecimalFormat", "positivePrefix", "Ljava/lang/String;", 0);
    } else if (matchFieldOrStaticName(comp(), node,
                   "com/ibm/jit/DecimalFormatHelper.positiveSuffix Ljava/lang/String;")) {
...
void TR_J9ByteCodeIlGenerator::expandUnresolvedClassCheckcast(TR::TreeTop *tree)
{
    ...
    TR_ASSERT(checkcastNode->getOpCodeValue() == TR::checkcast,
        "unresolved class checkcast: expected treetop %p node n%un to be checkcast but was %s\n", tree,
        checkcastNode->getGlobalIndex(), checkcastNode->getOpCode().getName());

    TR::Node *objNode = checkcastNode->getFirstChild();
    TR::Node *classNode = checkcastNode->getSecondChild();
    TR_ASSERT(classNode->getOpCodeValue() == TR::loadaddr,
        "unresolved class checkcast n%un: expected class child n%un to be loadaddr but was %s\n",
        checkcastNode->getGlobalIndex(), classNode->getGlobalIndex(), classNode->getOpCode().getName());
    TR_ASSERT(classNode->getSymbolReference()->isUnresolved(),
        "unresolved class checkcast n%un: expected symref of class child n%un to be unresolved\n",
        checkcastNode->getGlobalIndex(), classNode->getGlobalIndex());
    TR_ASSERT(classNode->getReferenceCount() == 1,
        "unresolved class checkcast n%un: expected class child n%un to have reference count 1\n",
        checkcastNode->getGlobalIndex(), classNode->getGlobalIndex());

    bool trace = comp()->getOption(TR_TraceILGen);
    logprintf(trace, log, "expanding unresolved class checkcast n%un in block_%d\n", checkcastNode->getGlobalIndex(),
        tree->getEnclosingBlock()->getNumber());
    ...
    TR_ASSERT(checkcastNode->getSecondChild() == classNode,
        "unresolved class checkcast n%un: split block should not have replaced class child n%un with n%un\n",
        checkcastNode->getGlobalIndex(), classNode->getGlobalIndex(),
        checkcastNode->getSecondChild()->getGlobalIndex());
...
void TR_J9ByteCodeIlGenerator::expandUnresolvedClassInstanceof(TR::TreeTop *tree)
{
    ...
    TR_ASSERT(treetopNode->getOpCodeValue() == TR::treetop,
        "unresolved class instanceof: expected treetop %p node n%un to be treetop but was %s\n", tree,
        treetopNode->getGlobalIndex(), treetopNode->getOpCode().getName());

    TR::Node *instanceofNode = treetopNode->getFirstChild();
    TR_ASSERT(instanceofNode->getOpCodeValue() == TR:: instanceof,
              "unresolved class instanceof: expected treetop %p node n%un child n%un to be instanceof but was %s\n",
              tree, treetopNode->getGlobalIndex(), instanceofNode->getGlobalIndex(),
              instanceofNode->getOpCode().getName());

    TR::Node *objNode = instanceofNode->getFirstChild();
    TR::Node *origClassNode = instanceofNode->getSecondChild();
    TR_ASSERT(origClassNode->getOpCodeValue() == TR::loadaddr,
        "unresolved class instanceof n%un: expected class child n%un to be loadaddr but was %s\n",
        instanceofNode->getGlobalIndex(), origClassNode->getGlobalIndex(), origClassNode->getOpCode().getName());
    TR_ASSERT(origClassNode->getSymbolReference()->isUnresolved(),
        "unresolved class instanceof n%un: expected symref of class child n%un to be unresolved\n",
    ...
    if (instanceofNode->isReferenceNonNull() || objNode->isNonNull()) {
        ...
        logprintf(trace, log, "%s: emit ResolveCHK n%dn before the unresolved class instanceof n%un in block_%d\n",
            __FUNCTION__, resolveCheckNode->getGlobalIndex(), instanceofNode->getGlobalIndex(),
            tree->getEnclosingBlock()->getNumber());
        return;
    }

    logprintf(trace, log, "expanding unresolved class instanceof n%un in block_%d\n", instanceofNode->getGlobalIndex(),
        tree->getEnclosingBlock()->getNumber());
...
{
    static const bool disable = feGetEnv("TR_skipInvokeSpecialInterfaceTypeChecks") != NULL;
    return disable;
}
...
{
    TR_ASSERT(!comp()->compileRelocatableCode(), "in expandInvokeSpecialInterface under AOT\n");

    TR_ASSERT(!skipInvokeSpecialInterfaceTypeChecks(), "in expandInvokeSpecialInterface, but it is disabled\n");

    OMR::Logger *log = comp()->log();
    const bool trace = comp()->getOption(TR_TraceILGen);
    static const bool verbose = feGetEnv("TR_verboseInvokeSpecialInterface") != NULL;
    ...
    TR::SymbolReference * const instofSR = symRefTab()->findOrCreateInstanceOfSymbolRef(_methodSymbol);
    TR::Node * const instof = TR::Node::createWithSymRef(TR:: instanceof, 2, 2, receiver, iface, instofSR);

    TR::Node * const typeTest = TR::Node::createif(TR::ificmpne, instof, TR::Node::iconst(0));
    ...
    logprintf(trace, log,
        ...
        headBlock->getNumber(), headBlock->getNumber(), failBlock->getNumber(), okBlock->getNumber());
    ...
    TR::Node * const rejectedVFT = TR::Node::createWithSymRef(TR::aloadi, 1, 1,
        callNode->getArgument(0)->duplicateTree(), // load split temp
        symRefTab()->findOrCreateVftSymbolRef());

    TR::Node * const helperCall = TR::Node::createWithSymRef(TR::call, 2, 2,
        iface->duplicateTree(), // avoid commoning across blocks
        rejectedVFT, symRefTab()->findOrCreateIncompatibleReceiverSymbolRef(_methodSymbol));

    failBlock->append(TR::TreeTop::create(comp(), TR::Node::create(TR::treetop, 1, helperCall)));
    ...
    if (!voidReturn) {
        TR::Node *retValNode
            = TR::Node::create(comp()->il.opCodeForConst(_methodSymbol->getResolvedMethod()->returnType()), 0);
...
TR::Node *TR_J9ByteCodeIlGenerator::loadFromMethodTypeTable(TR::Node *methodHandleInvokeCall)
{
...
void TR_J9ByteCodeIlGenerator::insertCustomizationLogicTreeIfEnabled(TR::TreeTop *tree, TR::Node *methodHandle)
{
    if (comp()->getOption(TR_EnableMHCustomizationLogicCalls)) {
        TR::SymbolReference *doCustomizationLogic = comp()->getSymRefTab()->methodSymRefFromName(_methodSymbol,
            JSR292_MethodHandle, "doCustomizationLogic", "()V", TR::MethodSymbol::Special);
...
void TR_J9ByteCodeIlGenerator::expandInvokeHandle(TR::TreeTop *tree)
{
    ...
    if (callSiteMethodType->getSymbolReference()->isUnresolved()) {
        TR::Node *resolveChkOnMethodType = TR::Node::createWithSymRef(callNode, TR::ResolveCHK, 1, callSiteMethodType,
            comp()->getSymRefTab()->findOrCreateResolveCheckSymbolRef(_methodSymbol));
...
void TR_J9ByteCodeIlGenerator::expandInvokeHandleGeneric(TR::TreeTop *tree)
{
    ...
    TR::SymbolReference *typeConversionSymRef = comp()->getSymRefTab()->methodSymRefFromName(_methodSymbol,
        JSR292_MethodHandle, JSR292_asType, JSR292_asTypeSig, TR::MethodSymbol::Static);
...
void TR_J9ByteCodeIlGenerator::expandInvokeExact(TR::TreeTop *tree)
{
    ...
    uint32_t offset = fej9()->getInstanceFieldOffsetIncludingHeader("Ljava/lang/invoke/MethodHandle;", "thunks",
        "Ljava/lang/invoke/ThunkTuple;", method());
    TR::SymbolReference *thunksSymRef = comp()->getSymRefTab()->findOrFabricateShadowSymbol(_methodSymbol,
        TR::Symbol::Java_lang_invoke_MethodHandle_thunks, TR::Address, offset, false, false, false,
        "java/lang/invoke/MethodHandle.thunks Ljava/lang/invoke/ThunkTuple;");
    TR::Node *thunksNode = TR::Node::createWithSymRef(callNode, comp()->il.opCodeForIndirectLoad(TR::Address), 1,
        receiverHandle, thunksSymRef);
    thunksNode->setIsNonNull(true);

    offset = fej9()->getInstanceFieldOffsetIncludingHeader("Ljava/lang/invoke/ThunkTuple;", "invokeExactThunk", "J",
        method());
    TR::SymbolReference *invokeExactTargetAddrSymRef = comp()->getSymRefTab()->findOrFabricateShadowSymbol(
        _methodSymbol, TR::Symbol::Java_lang_invoke_ThunkTuple_invokeExactThunk, TR::Int64, offset, false, false, true,
        "java/lang/invoke/ThunkTuple.invokeExactThunk J");
    TR::Node *invokeExactTargetAddr = TR::Node::createWithSymRef(callNode, comp()->il.opCodeForIndirectLoad(TR::Int64),
        1, thunksNode, invokeExactTargetAddrSymRef);
...
void TR_J9ByteCodeIlGenerator::expandMethodHandleInvokeCall(TR::TreeTop *tree)
{
    ...
    for (int i = callNode->getFirstArgumentIndex(); i < callNode->getNumChildren(); i++) {
        ...
        logprintf(trace, log, "TreeTop n%dn is created to anchor node n%dn\n", anchorTT->getNode()->getGlobalIndex(),
            child->getGlobalIndex());
        tree->insertBefore(anchorTT);
    }

    if (_invokeHandleCalls && _invokeHandleCalls->isSet(_bcIndex)) {
        expandInvokeHandle(tree);
    } else if (_invokeHandleGenericCalls && _invokeHandleGenericCalls->isSet(_bcIndex)) {
        expandInvokeHandleGeneric(tree);
    } else if (_invokeDynamicCalls && _invokeDynamicCalls->isSet(_bcIndex)) {
        expandInvokeDynamic(tree);
    } else if (_ilGenMacroInvokeExactCalls && _ilGenMacroInvokeExactCalls->isSet(_bcIndex)) {
        expandInvokeExact(tree);
    } else {
    ...
    if (methodHandle->getOpCode().hasSymbolReference() && methodHandle->getSymbolReference()->hasKnownObjectIndex()) {
