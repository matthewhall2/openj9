...
void TR_J9ByteCodeIterator::initialize(TR_ResolvedJ9Method *method, TR_J9VMBase *fe)
{
    _fe = fe;
...
int32_t TR_J9ByteCodeIterator::nextSwitchValue(int32_t &bcIndex)
{
...
void TR_J9ByteCodeIterator::stepOverVariableSizeBC()
{
    if (_bc == J9BCwide)
...
bool TR_J9ByteCodeIterator::isThisChanged()
{
...
int32_t TR_J9ByteCodeIterator::findFloatingPointInstruction()
{
    ...
    for (TR_J9ByteCode bc = first(); bc != J9BCunknown; bc = next())
        switch (bc) {
            ...
            case J9BCgetfield:
            case J9BCputfield: {
                method()->fieldAttributes(_compilation, next2Bytes(), &offset, &type, &isVolatile, NULL, &isPrivate,
                    (bc == J9BCputfield), NULL, false);
            ...
            case J9BCgetstatic:
            case J9BCputstatic:
                method()->staticAttributes(_compilation, next2Bytes(), &staticAddress, &type, &isVolatile, NULL,
                    &isPrivate, (bc == J9BCputstatic), NULL, false);
                if (type == TR::Float || type == TR::Double)
                    return bcIndex();
                break;
...
void TR_J9ByteCodeIterator::printByteCodePrologue(OMR::Logger *log)
{
    log->prints("\n"
                "        +------------- Byte Code Index\n"
                "        |  +-------------------- OpCode\n"
...
void TR_J9ByteCodeIterator::printByteCodeEpilogue(OMR::Logger *log)
{
    log->prints("\n\n");
    comp()->getDebug()->printByteCodeAnnotations(log);
}

void TR_J9ByteCodeIterator::printFirst(OMR::Logger *log, int32_t i) { log->printf("%5i", i); }

void TR_J9ByteCodeIterator::printCPIndex(OMR::Logger *log, int32_t i) { log->printf("%13s%5i", "", i); }

void TR_J9ByteCodeIterator::printConstant(OMR::Logger *log, int32_t i) { log->printf("%11s%12i  ", "", i); }

void TR_J9ByteCodeIterator::printConstant(OMR::Logger *log, double d) { log->printf("%11s%12e  ", "", d); }

void TR_J9ByteCodeIterator::printFirstAndConstant(OMR::Logger *log, int32_t i, int32_t j)
{
    log->printf("%5i%6s%12i  ", i, "", j);
}

void TR_J9ByteCodeIterator::printJumpIndex(OMR::Logger *log, int32_t offset)
{
    log->printf("%5i,%5d,%11s ", offset, offset + bcIndex(), "");
}

void TR_J9ByteCodeIterator::printByteCode(OMR::Logger *log)
{
...
const TR_J9ByteCode TR_J9ByteCodeIterator::_opCodeToByteCodeEnum[] = {
    ...
    /* 159 */ J9BCificmpeq, J9BCificmpne, J9BCificmplt, J9BCificmpge, J9BCificmpgt, J9BCificmple, J9BCifacmpeq,
    J9BCifacmpne,
...
//                                              ==           !=            <             >=            >             <=
TR::ILOpCodes TR_J9ByteCodeIterator::_lcmpOps[]
    = { TR::iflcmpeq, TR::iflcmpne, TR::iflcmplt, TR::iflcmpge, TR::iflcmpgt, TR::iflcmple };
TR::ILOpCodes TR_J9ByteCodeIterator::_fcmplOps[]
    = { TR::iffcmpeq, TR::iffcmpneu, TR::iffcmpltu, TR::iffcmpge, TR::iffcmpgt, TR::iffcmpleu };
TR::ILOpCodes TR_J9ByteCodeIterator::_fcmpgOps[]
    = { TR::iffcmpeq, TR::iffcmpneu, TR::iffcmplt, TR::iffcmpgeu, TR::iffcmpgtu, TR::iffcmple };
TR::ILOpCodes TR_J9ByteCodeIterator::_dcmplOps[]
    = { TR::ifdcmpeq, TR::ifdcmpneu, TR::ifdcmpltu, TR::ifdcmpge, TR::ifdcmpgt, TR::ifdcmpleu };
TR::ILOpCodes TR_J9ByteCodeIterator::_dcmpgOps[]
    = { TR::ifdcmpeq, TR::ifdcmpneu, TR::ifdcmplt, TR::ifdcmpgeu, TR::ifdcmpgtu, TR::ifdcmple };
