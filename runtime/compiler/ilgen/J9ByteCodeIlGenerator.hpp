...
class TR_J9ByteCodeIlGenerator
    : public TR_IlGenerator
    , public TR_J9ByteCodeIteratorWithState {
public:
    TR_J9ByteCodeIlGenerator(TR::IlGeneratorMethodDetails &details, TR::ResolvedMethodSymbol *, TR_J9VMBase *fe,
        TR::Compilation *, TR::SymbolReferenceTable *, bool forceClassLookahead = false,
        TR_InlineBlocks *blocksToInline = 0, int32_t argPlaceholderSlot = -1);
...
private:
     ...
     * \param numExpectedArgs the number of arguments the call expects. Defaults to -1 since this can usually be
     * calculated from the symbol reference
     */
    TR::Node *genInvoke(TR::SymbolReference *symRef, TR::Node *indirectCallFirstChild,
        TR::Node *invokedynamicReceiver = NULL, int32_t numExpectedArgs = -1);
     ...
     * calculated from the symbol reference
     */
    TR::Node *genInvokeInner(TR::SymbolReference *symRef, TR::Node *indirectCallFirstChild,
        TR::Node *invokedynamicReceiver, TR::KnownObjectTable::Index *requiredKoi, int32_t numExpectedArgs = -1);
     ...
     * calculated from the symbol reference
     */
    TR::Node *genInvokeDirect(TR::SymbolReference *symRef, int32_t numExpectedArgs = -1)
    {
        return genInvoke(symRef, NULL, NULL, numExpectedArgs);
    }
    ...
    TR::SymbolReference *placeholderWithSignature(const char *prefix, int prefixLength, const char *middle,
        int middleLength, const char *suffix, int suffixLength);
    ...
    {
        // bool sigSame = true;
        // return s1->getOwningMethod(_compilation)->fieldsAreSame(s1->getCPIndex(), s2->getOwningMethod(_compilation),
        // s2->getCPIndex(), sigSame);
        return TR::Compiler->cls.jitFieldsAreSame(_compilation, s1->getOwningMethod(_compilation), s1->getCPIndex(),
            s2->getOwningMethod(_compilation), s2->getCPIndex(), false);
    }

    bool staticsAreSame(TR::SymbolReference *s1, TR::SymbolReference *s2)
    {
        // bool sigSame = true;
        // return s1->getOwningMethod(_compilation)->staticsAreSame(s1->getCPIndex(), s2->getOwningMethod(_compilation),
        // s2->getCPIndex(), sigSame);
        return TR::Compiler->cls.jitStaticsAreSame(_compilation, s1->getOwningMethod(_compilation), s1->getCPIndex(),
            s2->getOwningMethod(_compilation), s2->getCPIndex());
    }

    TR::Node *genNodeAndPopChildren(TR::ILOpCodes, int32_t, TR::SymbolReference *, int32_t = 0);
    TR::Node *genNodeAndPopChildren(TR::ILOpCodes, int32_t, TR::SymbolReference *, int32_t firstIndex,
        int32_t lastIndex);
    ...
    bool replaceField(TR::Node *node, const char *destClass, const char *destFieldName, const char *destFieldSignature,
        int ParmIndex);
