...
TR_ClassLookahead::TR_ClassLookahead(TR_PersistentClassInfo *classInfo, TR_FrontEnd *fe, TR::Compilation *comp,
...
int32_t TR_ClassLookahead::perform()
{
    ...
    if ((fej9->getNumInnerClasses(_classPointer) > 0) || _classInfo->cannotTrustStaticFinal())
        return 0;

    bool isClassInitialized = false;
    bool seenFirstInitializerMethod = false;
    bool allowForAOT = comp()->getOption(TR_UseSymbolValidationManager);
    TR_PersistentClassInfo *classInfo = comp()->getPersistentInfo()->getPersistentCHTable()->findClassInfoAfterLocking(
        _classPointer, comp(), allowForAOT);
    ...
    int32_t len;
    char *name = fej9->getClassNameChars(_classPointer, len);
    ...
    findInitializerMethods(&resolvedMethodsInClass, &initializerMethodsInClass, &resolvedMethodSyms, &classInitializer,
        &peekFailedForAnyMethod);
    ...
    resolvedMethod = NULL;
    for (resolvedMethodSymbol = resolvedIt.getFirst(); resolvedMethodSymbol;
         resolvedMethodSymbol = resolvedIt.getNext()) {
...
static bool isCalledByNonConstructorMethodsInClass(TR_ResolvedMethod *calleeMethod,
    List<TR::ResolvedMethodSymbol> *resolvedMethodSyms)
{
    ...
    for (resolvedMethodSymbol = resolvedIt.getFirst(); resolvedMethodSymbol;
        ...
        if (!resolvedMethod->isConstructor()) {
            for (TR::TreeTop *tt = resolvedMethodSymbol->getFirstTreeTop(); tt; tt = tt->getNextTreeTop()) {
                TR::Node *node = tt->getNode();
                if (node->getNumChildren() > 0 && node->getFirstChild()->getOpCode().isCall()
                    && !node->getFirstChild()->getOpCode().isIndirect()) {
                    TR::Method *calleeMethod1 = node->getFirstChild()->getSymbol()->getMethodSymbol()->getMethod();
                    if (calleeMethod1 && calleeMethod1->nameLength() == calleeMethod->nameLength()
                        ...
                        && !strncmp(calleeMethod1->signatureChars(), calleeMethod->signatureChars(),
                            calleeMethod->signatureLength())
                        && !strncmp(calleeMethod1->classNameChars(), calleeMethod->classNameChars(),
                            calleeMethod->classNameLength()))
                        return true;
                }
            }
        }
...
void TR_ClassLookahead::findInitializerMethods(List<TR_ResolvedMethod> *resolvedMethodsInClass,
    List<TR::ResolvedMethodSymbol> *initializerMethodsInClass, List<TR::ResolvedMethodSymbol> *resolvedMethodSyms,
    TR::ResolvedMethodSymbol **classInitializer, bool *peekFailedForAnyMethod)
{
    ...
    for (resolvedMethod = resolvedIt.getFirst(); resolvedMethod; resolvedMethod = resolvedIt.getNext()) {
        ...
        if (resolvedMethod->isStatic())
            methodSymRef = _symRefTab->findOrCreateMethodSymbol(JITTED_METHOD_INDEX, -1, resolvedMethod,
        ...
        if (resolvedMethod->isCompilable(comp()->trMemory()) && !resolvedMethod->isNewInstanceImplThunk()
            ...
            *peekFailedForAnyMethod = (NULL
                == resolvedMethodSymbol->getResolvedMethod()->genMethodILForPeeking(resolvedMethodSymbol, comp(),
                    true));
        }
    }
    ...
    TR::ResolvedMethodSymbol *resolvedMethodSymbol;
    for (resolvedMethodSymbol = resolvedMethodSymsIt.getFirst(); resolvedMethodSymbol;
         resolvedMethodSymbol = resolvedMethodSymsIt.getNext()) {
        resolvedMethod = resolvedMethodSymbol->getResolvedMethod();
        if ((resolvedMethod->isConstructor() || (!strncmp(resolvedMethod->nameChars(), "<clinit>", 8)))
            && resolvedMethodSymbol->getFirstTreeTop()) {
            if (!strncmp(resolvedMethod->nameChars(), "<clinit>", 8))
                *classInitializer = resolvedMethodSymbol;
                ...
                if (firstRealNode->getOpCodeValue() == TR::treetop) {
                    ...
                    if ((callChild->getOpCodeValue() == TR::call) && !callChild->getSymbolReference()->isUnresolved()) {
                        TR_ResolvedMethod *calleeMethod
                            = callChild->getSymbol()->castToResolvedMethodSymbol()->getResolvedMethod();
                ...
                TR::Node *node = tt->getNode();
                if (node->getOpCode().isTreeTop() && (node->getNumChildren() > 0))
    ...
    while (initMethodSymElem) {
        ...
        if (!resolvedMethodSyms->find(initMethodSym)) {
            ...
            for (resolvedMethodSym = resolvedIt.getFirst(); resolvedMethodSym;
                 resolvedMethodSym = resolvedIt.getNext()) {
...
bool TR_ClassLookahead::examineNode(TR::TreeTop *nextTree, TR::Node *grandParent, TR::Node *parent, int32_t childNum,
    TR::Node *node, vcount_t visitCount)
{
    ...
    if (node->getOpCode().isStore()) {
        ...
        if (isProperFieldAccess(node)) {
            ...
            //
            bool canMorph = false;
            if (rhsOfStoreNode->getOpCodeValue() == TR::aconst && rhsOfStoreNode->getInt() == 0)
                canMorph = true;
            ...
            //
            if (fieldInfo && !storedSymRef->isUnresolved() && (sym->isPrivate() || sym->isFinal())) {
                if (fieldInfo->isTypeInfoValid()) {
                    bool isClassPtrInitialized = false;
                    bool classPtrSame = true;
                    if (_inFirstBlock && _inInitializerMethod) {
                    ...
                    if (classPtrSame || isClassPtrInitialized) {
                        ...
                        if (_inClassInitializerMethod) {
                            ...
                            if (fieldInfo->isTypeInfoValid() == VALID_AND_INITIALIZED_STATICALLY) {
                                if (!_inFirstInitializerMethod && !sym->isStatic()) {
                                    logprintf(_traceIt, log,
                                        "C0Not always initializing type info for symbol %x at node %x\n", sym, node);
                                    fieldInfo->setIsTypeInfoValid(VALID_BUT_NOT_ALWAYS_INITIALIZED);
                                } else {
                                    if (!sym->isStatic()) {
                                        logprintf(_traceIt, log,
                                            "C0Always initializing type info for symbol %x at node %x\n", sym, node);
                                ...
                                if (!sym->isStatic()) {
                                    logprintf(_traceIt, log,
                                        "C1Always initializing type info for symbol %x at node %x\n", sym, node);
                                    fieldInfo->setIsTypeInfoValid(VALID_AND_ALWAYS_INITIALIZED);
                                } else {
                ...
                if (isNewArray && arrayFieldInfo && isConstantLengthArrayAllocation && (numberOfDimensions <= 2)) {
                    if (_inFirstBlock && _inInitializerMethod) {
                    ...
                    if (arrayFieldInfo->isDimensionInfoValid()) {
                        ...
                        for (; k < (rhsNumChildren - 1 - firstDimensionChild); k++) {
                            ...
                            if (_inFirstBlock && _inInitializerMethod && (rhsChild->getOpCodeValue() == TR::iconst)
                                && ((rhsChild->getInt() == arrayFieldInfo->getDimensionInfo(k)) || isInitialized)) {
                                arrayFieldInfo->setDimensionInfo(k, rhsChild->getInt());
                                if (_inClassInitializerMethod) {
                                    logprintf(_traceIt, log,
                                        "Statically initializing dimension info for symbol %x at node %x\n", sym, node);
                                    ...
                                    if (arrayFieldInfo->isDimensionInfoValid() == VALID_AND_INITIALIZED_STATICALLY) {
                                        if (!_inFirstInitializerMethod && !sym->isStatic()) {
                                            logprintf(_traceIt, log,
                                                "0Not always initializing dimension info for symbol %x at node %x\n",
                                                sym, node);
                                            arrayFieldInfo->setIsDimensionInfoValid(VALID_BUT_NOT_ALWAYS_INITIALIZED);
                                        } else {
                                            if (!sym->isStatic()) {
                                                logprintf(_traceIt, log,
                                                    "0Always initializing dimension info for symbol %x at node %x\n",
                                                    sym, node);
                                        ...
                                        if (!sym->isStatic()) {
                                            logprintf(_traceIt, log,
                                                "1Always initializing dimension info for symbol %x at node %x\n", sym,
                                                node);
                                            arrayFieldInfo->setIsDimensionInfoValid(VALID_AND_ALWAYS_INITIALIZED);
                                        } else {
                                            logprintf(_traceIt, log,
                                                "C0Invalidating dimension info for symbol %x at node %x\n", sym, node);
                                            arrayFieldInfo->setIsDimensionInfoValid(INVALID);
                                        }
                                    }
                                }
        ...
        } else if ((sym->isShadow() || sym->isStatic()) && !storedSymRef->isUnresolved()
            && (sym->isPrivate() || sym->isFinal())) {
            ...
            // if (isProperFieldAccess(node))
            {
                if (fieldInfo) {
                    ...
                    if (rhsOfStoreNode->getOpCode().isCall()
                        ...
                        if (rhsOfStoreNode->getSymbol()->getResolvedMethodSymbol()->getRecognizedMethod()
                            == TR::java_math_BigDecimal_valueOf) {
                            fieldInfo->setBigIntegerType(false);
                        } else if ((rhsOfStoreNode->getSymbol()->getResolvedMethodSymbol()->getRecognizedMethod()
                                       == TR::java_math_BigDecimal_add)
                            || (rhsOfStoreNode->getSymbol()->getResolvedMethodSymbol()->getRecognizedMethod()
                                == TR::java_math_BigDecimal_subtract)
                            || (rhsOfStoreNode->getSymbol()->getResolvedMethodSymbol()->getRecognizedMethod()
                                == TR::java_math_BigDecimal_multiply)) {
                            fieldInfo->setBigIntegerType(false);
                        } else if ((rhsOfStoreNode->getSymbol()->getResolvedMethodSymbol()->getRecognizedMethod()
                                       == TR::java_math_BigInteger_add)
                            || (rhsOfStoreNode->getSymbol()->getResolvedMethodSymbol()->getRecognizedMethod()
                        ...
                        if (rhsOfStoreNode->getOpCode().isLoadVar()
                            ...
                            TR::Symbol::RecognizedField field
                                = rhsOfStoreNode->getSymbolReference()->getSymbol()->getRecognizedField();
...
{
    if ((node->getOpCodeValue() != TR::iloadi) && (node->getOpCodeValue() != TR::lloadi)
        && (node->getOpCodeValue() != TR::iload) && (node->getOpCodeValue() != TR::lload))
        return false;
    ...
    if (!grandParent->getOpCode().isStore()) {
        if (grandParent->getOpCode().isConversion()) {
            if (treeNode->getOpCodeValue() == TR::treetop || treeNode->getOpCode().isNullCheck())
            ...
            if (treeNode->getOpCode().isStore()) {
                ...
                if (valueChild->getOpCode().isConversion() && (valueChild->getReferenceCount() == 1)
        ...
        if (grandParent->getOpCode().isIndirect() && (grandParent->getSecondChild() != parent))
            return false;

        storeNode = grandParent;
    }

    if ((parent->getOpCodeValue() == TR::iadd) || (parent->getOpCodeValue() == TR::isub)
                ...
                if (storeNode->getSymbolReference() == node->getSymbolReference()) {
                    if (storeNode->getFirstChild()->getOpCodeValue() == node->getFirstChild()->getOpCodeValue()) {
                        if ((storeNode->getFirstChild() == node->getFirstChild())
                            || (storeNode->getFirstChild()->getSymbolReference()
                                == node->getFirstChild()->getSymbolReference()))
                            return true;
                    }
                }
            }
        } else if ((node->getOpCodeValue() == TR::iload) && (storeNode->getOpCodeValue() == TR::istore)
            && (storeNode->getSymbolReference() == node->getSymbolReference()))
            return true;
    } else if ((parent->getOpCodeValue() == TR::ladd) || (parent->getOpCodeValue() == TR::lsub)
        ...
        } else if ((node->getOpCodeValue() == TR::lload) && (storeNode->getOpCodeValue() == TR::lstore)
            && (storeNode->getSymbolReference() == node->getSymbolReference()))
            return true;
    }
...
static bool isPureBigDecimalMethod(TR::Node *node, TR::Compilation *comp, TR_PersistentFieldInfo *fieldInfo,
    bool &isBigDecimal, bool &isBigInteger)
{
    if (!node)
        return false;
...
static bool isStoreToSameField(TR::Node *callNode, TR::Node *nextNode, TR::Node *loadNode)
{
    if (callNode->getReferenceCount() == 2) {
        if (nextNode->getOpCodeValue() == TR::treetop || nextNode->getOpCode().isNullCheck())
            nextNode = nextNode->getFirstChild();

        if ((nextNode->getOpCodeValue() == TR::awrtbari) || (nextNode->getOpCodeValue() == TR::astorei)) {
            if (nextNode->getSymbolReference() == loadNode->getSymbolReference()) {
                if (nextNode->getFirstChild()->getOpCodeValue() == loadNode->getFirstChild()->getOpCodeValue()) {
                    if ((nextNode->getFirstChild() == loadNode->getFirstChild())
                        || (nextNode->getFirstChild()->getSymbolReference()
                            == loadNode->getFirstChild()->getSymbolReference())) {
                        if (nextNode->getSecondChild() == callNode)
                            return true;
                    }
                }
            }
        } else if ((nextNode->getOpCodeValue() == TR::awrtbar) || (nextNode->getOpCodeValue() == TR::astore)) {
            if (nextNode->getSymbolReference() == loadNode->getSymbolReference()) {
...
void TR_ClassLookahead::invalidateIfEscapingLoad(TR::TreeTop *nextTree, TR::Node *grandParent, TR::Node *parent,
    int32_t childNum, TR::Node *node)
{
    TR::SymbolReference *storedSymRef = node->getSymbolReference();
    TR::Symbol *sym = storedSymRef->getSymbol();

    if ((sym->isShadow() || sym->isStatic())
        && (storedSymRef->isUnresolved() || (sym->isPrivate() || sym->isFinal()))) {
        TR_PersistentFieldInfo *previousFieldInfo = _classFieldInfo->find(comp(), sym, storedSymRef);
        TR_PersistentFieldInfo *fieldInfo = previousFieldInfo;
        if (!fieldInfo) {
            fieldInfo = getExistingFieldInfo(sym, storedSymRef);
        ...
        if (fieldInfo) {
            ...
            if (sym->isStatic() || (sym->isShadow() && node->getFirstChild()->isThisPointer())) {
                ...
                if (refCountMatches) {
                    if (parent && ((parent->getOpCodeValue() == TR::treetop) || parent->getOpCode().isAnchor()))
                        knownRead = true;
                    ...
                    if (isPureBigDecimalMethod(parent, comp(), fieldInfo, isBigDecimal, isBigInteger) && (childNum == 1)
                        ...
                        if ((parent->getReferenceCount() == 1) || isStoreToSameField(parent, nextTree->getNode(), node))
                            knownRead = true;
                    }

                    if (parent && (parent->getOpCodeValue() == TR::aloadi) && comp()->getSymRefTab()->findVftSymbolRef()
                        && (parent->getSymbolReference()->getOffset()
                            == comp()->getSymRefTab()->findVftSymbolRef()->getOffset())
            ...
            if (storedSymRef->isUnresolved() || (sym->isPrivate() && !sym->isFinal()))
                arrayFieldInfo = fieldInfo->asPersistentArrayFieldInfo();
        }

        if (arrayFieldInfo) {
            if (!parent
                || ((!parent->isInternalPointer()) && (parent->getOpCodeValue() != TR::treetop)
                    && (!parent->getOpCode().isArrayLength()) && (!parent->getOpCode().isAnchor())
...
bool TR_ClassLookahead::isProperFieldAccess(TR::Node *node)
{
    TR::SymbolReference *storedSymRef = node->getSymbolReference();
    TR::Symbol *sym = storedSymRef->getSymbol();
    if (((sym->isShadow() && node->getFirstChild()->isThisPointer()) || (sym->isStatic() && sym->isFinal()))
        && !storedSymRef->isUnresolved() && (sym->isPrivate() || sym->isFinal()))
        return true;
...
bool TR_ClassLookahead::isPrivateFieldAccess(TR::Node *node)
{
    TR::SymbolReference *storedSymRef = node->getSymbolReference();
    TR::Symbol *sym = storedSymRef->getSymbol();
    if ((sym->isShadow() || (sym->isStatic() && sym->isFinal())) && !storedSymRef->isUnresolved() && sym->isPrivate())
        return true;
...
char *TR_ClassLookahead::getFieldSignature(TR::Compilation *comp, TR::Symbol *sym, TR::SymbolReference *symRef,
    int32_t &length)
{
    if (!symRef->isUnresolved() || !sym->isStatic() || !sym->isConstObjectRef()) {
        char *sig = NULL;
        if (sym->isStatic())
...
TR_PersistentArrayFieldInfo *TR_ClassLookahead::getExistingArrayFieldInfo(TR::Symbol *fieldSym,
        ...
        if (length > -1) {
            ...
            if (newFieldInfo) {
                if (newFieldInfo->canChangeToArray()) {
                    ...
                    logprintf(_traceIt, comp()->log(),
                        "fieldInfo %p exists already for array field %s, so cannot morph\n", newFieldInfo, sig);
...
TR_PersistentFieldInfo *TR_ClassLookahead::getExistingFieldInfo(TR::Symbol *fieldSym, TR::SymbolReference *fieldSymRef,
    bool canMorph)
{
...
void TR_ClassLookahead::makeInfoPersistent()
{
    ...
    while (fieldInfo) {
        ...
        if (isTypeInfoValid || isDimensionInfoValid || isImmutable
            || (isNotRead
                && ((!hasBigDecimalAssumption || isBigDecimalType)
                    && (!hasBigIntegerAssumption || isBigIntegerType)))) {
...
TR_PersistentFieldInfo *TR_PersistentClassInfoForFields::find(TR::Compilation *comp, TR::Symbol *fieldSym,
    TR::SymbolReference *fieldSymRef)
{
    int32_t length = 0;
    char *sig = TR_ClassLookahead::getFieldSignature(comp, fieldSym, fieldSymRef, length);
    ...
    for (TR_PersistentFieldInfo *fieldInfo = getFirst(); fieldInfo; fieldInfo = fieldInfo->getNext()) {
        ...
        /*
           ...
           {
           if (fieldsAreSame(fieldInfo->getOwningMethodSymbol()->getResolvedMethod(),
        fieldInfo->getFieldSymRef()->getCPIndex(), fieldSymRef->getOwningMethod(), fieldSymRef->getCPIndex())) return
        fieldInfo->asPersistentArrayFieldInfo();
           }
        else if (fieldSym->isStatic())
           {
           if (staticsAreSame(fieldInfo->getOwningMethodSymbol()->getResolvedMethod(),
        fieldInfo->getFieldSymRef()->getCPIndex(), fieldSymRef->getOwningMethod(), fieldSymRef->getCPIndex())) return
        fieldInfo->asPersistentArrayFieldInfo();
           }
        */
...
TR_PersistentFieldInfo *TR_PersistentClassInfoForFields::findFieldInfo(TR::Compilation *comp, TR::Node *&node,
    bool canBeArrayShadow)
{
    ...
    if (sym->isStatic() || (sym->isShadow() && node->getNumChildren() > 0 && node->getFirstChild()->isThisPointer()))
...
#ifdef DEBUG
void TR_PersistentFieldInfo::dumpInfo(OMR::Logger *log, TR_FrontEnd *fe)
{
...
void TR_PersistentArrayFieldInfo::dumpInfo(OMR::Logger *log, TR_FrontEnd *fe)
{
