...
public:
    ...
    bool isBasicPackedArithmetic() const
    {
        return (isAdd() || isSub() || isMul() || isDiv() || isRem()) && getType().isAnyPacked();
    }
    ...
    bool isAnyBCDCompareOp() const
    {
        return isBinaryCodedDecimalOp() && isBooleanCompare();
    } // isAnyBCDCompareOp is true for all BCD nodes that do some type of compare e.g. pdcmpxx

    bool isBCDToNonBCDConversion() const { return isConversion() && isBinaryCodedDecimalOp() && !getType().isBCD(); }

    bool isPackedArithmeticOverflowMessage() const
    {
        return properties4().testAny(ILProp4::PackedArithmeticOverflowMessage);
    }

    bool isBasicOrSpecialPackedArithmetic() const
    {
        return isBasicPackedArithmetic() || isPackedArithmeticOverflowMessage();
    }
    ...
    bool isSqrt() { return OMR::ILOpCode::isSqrt(); }
    ...
    static TR::ILOpCodes setSignOpCode(TR::DataType type)
    {
        switch (type) {
            case TR::PackedDecimal:
                return TR::pdSetSign;
            default:
                TR_ASSERT(0, "no setSign opcode for this datatype");
        }
        return TR::BadILOp;
    }
    ...
    static TR::ILOpCodes setSignVersionOfOpCode(TR::ILOpCodes op)
    {
        switch (op) {
            case TR::pdshr:
                return TR::pdshrSetSign;
            case TR::pdshl:
                return TR::pdshlSetSign;
            case TR::pd2zdsls:
                return TR::pd2zdslsSetSign;
            case TR::pd2zdsts:
                return TR::pd2zdstsSetSign;
            default:
                return TR::BadILOp;
        }
        return TR::BadILOp;
    ...
    static TR::ILOpCodes getProperConversion(TR::DataType sourceDataType, TR::DataType targetDataType,
        bool needUnsignedConversion)
    {
        TR::ILOpCodes op = getDataTypeConversion(sourceDataType, targetDataType);
        if (!needUnsignedConversion)
            return op;
        ...
        switch (op) {
            ...
            default:
                return OMR::ILOpCode::getProperConversion(sourceDataType, targetDataType, needUnsignedConversion);
        }
    }

    static bool isStrictlyLessThanCmp(TR::ILOpCodes op) { return OMR::ILOpCode::isStrictlyLessThanCmp(op); }

    static bool isStrictlyGreaterThanCmp(TR::ILOpCodes op) { return OMR::ILOpCode::isStrictlyGreaterThanCmp(op); }

    static bool isLessCmp(TR::ILOpCodes op) { return OMR::ILOpCode::isLessCmp(op); }

    static bool isGreaterCmp(TR::ILOpCodes op) { return OMR::ILOpCode::isGreaterCmp(op); }

    static bool isEqualCmp(TR::ILOpCodes op) { return OMR::ILOpCode::isEqualCmp(op); }

    static bool isNotEqualCmp(TR::ILOpCodes op) { return OMR::ILOpCode::isNotEqualCmp(op); }
    ...
    static TR::ILOpCodes cleanOpCode(TR::DataType type)
    {
        switch (type) {
            case TR::PackedDecimal:
                return TR::pdclean;
            default:
                TR_ASSERT(0, "no clean opcode for this datatype");
        }
        return TR::BadILOp;
    }
    ...
    static TR::ILOpCodes absOpCode(TR::DataType type)
    {
        switch (type) {
            case TR::PackedDecimal:
                return TR::pdabs;
            default:
                return OMR::ILOpCode::absOpCode(type);
        }
        return TR::BadILOp;
    }
    ...
    static TR::ILOpCodes addOpCode(TR::DataType type, bool is64Bit)
    {
        switch (type) {
            case TR::PackedDecimal:
                return TR::pdadd;
            default:
                return OMR::ILOpCode::addOpCode(type, is64Bit);
        }
        return TR::BadILOp;
    }
    ...
    static TR::ILOpCodes subtractOpCode(TR::DataType type)
    {
        switch (type) {
            case TR::PackedDecimal:
                return TR::pdsub;
            default:
                return OMR::ILOpCode::subtractOpCode(type);
        }
        return TR::BadILOp;
    }
    ...
    static TR::ILOpCodes multiplyOpCode(TR::DataType type)
    {
        switch (type) {
            case TR::PackedDecimal:
                return TR::pdmul;
            default:
                return OMR::ILOpCode::multiplyOpCode(type);
        }
        return TR::BadILOp;
    }
    ...
    static TR::ILOpCodes divideOpCode(TR::DataType type)
    {
        switch (type) {
            case TR::PackedDecimal:
                return TR::pddiv;
            default:
                return OMR::ILOpCode::divideOpCode(type);
        }
        return TR::BadILOp;
    }
    ...
    static TR::ILOpCodes remainderOpCode(TR::DataType type)
    {
        switch (type) {
            case TR::PackedDecimal:
                return TR::pdrem;
            default:
                return OMR::ILOpCode::remainderOpCode(type);
        }
        return TR::BadILOp;
    }
    ...
    static TR::ILOpCodes negateOpCode(TR::DataType type)
    {
        switch (type) {
            case TR::PackedDecimal:
                return TR::pdneg;
            default:
                return OMR::ILOpCode::negateOpCode(type);
        }
        return TR::BadILOp;
    }
    ...
    static TR::ILOpCodes modifyPrecisionOpCode(TR::DataType type)
    {
        switch (type) {
            case TR::PackedDecimal:
                return TR::pdModifyPrecision;
            default:
                TR_ASSERT(0, "no modifyPrecision opcode for this datatype");
        }
        return TR::BadILOp;
    }

    static TR::ILOpCodes shiftLeftOpCode(TR::DataType type) { return OMR::ILOpCode::shiftLeftOpCode(type); }

    static TR::ILOpCodes shiftRightOpCode(TR::DataType type) { return OMR::ILOpCode::shiftRightOpCode(type); }
    ...
    static TR::ILOpCodes ifcmpeqOpCode(TR::DataType type) { return OMR::ILOpCode::ifcmpeqOpCode(type); }

    static TR::ILOpCodes ifcmpneOpCode(TR::DataType type) { return OMR::ILOpCode::ifcmpneOpCode(type); }

    static TR::ILOpCodes cmpeqOpCode(TR::DataType type) { return OMR::ILOpCode::cmpeqOpCode(type); }

    static TR::ILOpCodes constOpCode(TR::DataType type) { return OMR::ILOpCode::constOpCode(type); }

    static TR::ILOpCodes returnOpCode(TR::DataType type) { return OMR::ILOpCode::returnOpCode(type); }
};
