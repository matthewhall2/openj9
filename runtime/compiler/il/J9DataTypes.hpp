...
public:
    ...
    bool isZonedLeadingSign()
    {
        return (_type == TR::ZonedDecimalSignLeadingSeparate) || (_type == TR::ZonedDecimalSignLeadingEmbedded);
    }

    bool isZonedTrailingSign()
    {
        return (_type == TR::ZonedDecimalSignTrailingSeparate) || (_type == TR::ZonedDecimal);
    }

    bool isZonedSeparateSign()
    {
        return (_type == TR::ZonedDecimalSignLeadingSeparate) || (_type == TR::ZonedDecimalSignTrailingSeparate);
    }
    ...
    bool isUnicodeSeparateSign()
    {
        return (_type == TR::UnicodeDecimalSignLeading) || (_type == TR::UnicodeDecimalSignTrailing);
    }
    ...
    static inline int32_t getMaxDecimalFraction()
    {
        return J9::DataType::getMaxDecimalAdjust();
    } // fraction and adjust share the variable

    static inline int32_t getMinDecimalFraction()
    {
        return J9::DataType::getMinDecimalAdjust();
    } // fraction and adjust share the variable
    ...
    static inline int32_t getIgnoredSignCode()
    {
        return -1;
    } // a special marker value for when a setSign operation can set any or no sign code value (as it will be ignored by
      // consumer)
    ...
    static inline int32_t getUnicodeZeroCode()
    {
        return ((J9::DataType::getUnicodeZeroCodeHigh() << 8) | J9::DataType::getUnicodeZeroCodeLow()) & 0xFFFF;
    }
    ...
    static inline int32_t getUnicodePlusCode()
    {
        return ((J9::DataType::getUnicodePlusCodeHigh() << 8) | J9::DataType::getUnicodePlusCodeLow()) & 0xFFFF;
    }
    ...
    static inline int32_t getUnicodeMinusCode()
    {
        return ((J9::DataType::getUnicodeMinusCodeHigh() << 8) | J9::DataType::getUnicodeMinusCodeLow()) & 0xFFFF;
    }
    ...
    {
        if (sign == J9::DataType::getBCDPlusChar() || sign == J9::DataType::getBCDMinusChar()
    ...
    static int32_t byteLengthToPackedDecimalPrecisionCeiling(int32_t byteLength) { return (byteLength * 2) - 1; }

    static int32_t byteLengthToPackedDecimalPrecisionFloor(int32_t byteLength) { return (byteLength * 2) - 2; }
