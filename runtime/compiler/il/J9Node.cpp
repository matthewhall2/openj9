...
// COMPILER_MSVC outputs 'error C2057: expected constant expression' when the static functions are used to determine the
// array's size
#endif
    ...
    _unionPropertyB()
{
    ...
    TR_ASSERT(self()->hasDecimalInfo() + self()->hasBCDFlags() <= 1,
        ...
        self()->getOpCode().getName(), this, self()->hasDecimalInfo(), self()->hasBCDFlags());
    ...
    if (self()->hasDecimalInfo()) {
        ...
        _unionPropertyB._decimalInfo._decimalPrecision
            = TR::DataType::getInvalidDecimalPrecision(); // valid range is 0->TR_MAX_DECIMAL_PRECISION (defined in
                                                          // OMRDataTypes.hpp)
        _unionPropertyB._decimalInfo._decimalAdjustOrFractionOrDivisor
            = 0; // valid range is -31->31 for adjust/fraction and 0->62 for divisor precision
        _unionPropertyB._decimalInfo._decimalSourcePrecisionOrDividend
            = 0; // valid range is 0->63 for source precision and 0->62 for dividend precision
        _unionPropertyB._decimalInfo._setSign
            = raw_bcd_sign_unknown; // for isSetSignOnNode() opcodes such as TR::pdclearSetSign
        _unionPropertyB._decimalInfo._hasNoSignStateOnLoad
            = 0; // 0 (false) is the conservative setting -- meaning do not clobber the sign code unless FE explicitly
                 // instructs
        _unionPropertyB._decimalInfo._castedToBCD
            = 0; // 0 (false) originally was a BCD type, 1 (true) casted from a non-BCD type (e.g. aggr, int) to a BCD
...
J9::Node::~Node() { _unionPropertyB = UnionPropertyB(); }

bool J9::Node::dontEliminateStores(bool isForLocalDeadStore)
{
    // Disallow store sinking of BCD operations in Java because such operations may be under a BCDCHK node, implying
    // that an exception path may be taken at runtime to evaluate the tree. Sinking such potentially exceptional
    // operations violates the assumption that all such operations are to be anchored on a BCDCHK treetop, which the
    // codegen expects to be able to generate the exception handling fallback code.
...
void J9::Node::copyValidProperties(TR::Node *fromNode, TR::Node *toNode)
{
    ...
    // in order to determine the UnionPropertyB_type correctly, require children and symRef (from UnionPropertyA) to
    // have been defined already as depends on getDataType which in term depends on these
        ...
        switch (fromUnionPropertyB_Type) {
            case HasDecimalInfo:
                ...
                if (toNode->getType().isBCD() && fromNode->getType().isBCD()) {
                    ...
                    toNode->_unionPropertyB._decimalInfo._hasCleanSign
                        = fromNode->hasKnownOrAssumedCleanSign(); //  '10 0d' might be truncated to negative zero '0
                                                                  //  00d' -- so no longer clean
            ...
            case HasBcdFlags:
                ...
                TR_ASSERT(fromNode->getOpCodeValue() == toNode->getOpCodeValue(),
                    "Unexpected comparison. Nodes that have _bcdFlags property should always be BCDCHK nodes. These "
                    "are %s %p and %s %p nodes",
                    fromNode->getOpCode().getName(), fromNode, toNode->getOpCode().getName(), toNode);
                TR_ASSERT(false,
                    "This makes no sense as recreate does not change the opcodevalue when copying forward properties "
                    "from node %s %p to node %s %p",
                    fromNode->getOpCode().getName(), fromNode, toNode->getOpCode().getName(), toNode);
                break;
            default:
                /* HasNoUnionPropertyB */
...
uint32_t J9::Node::getSize()
{
    if (self()->getType().isBCD()) {
        // this is a temporary workaround for a compiler bug where too many bits are getting pulled out of the bit
        // container
        /*
              TR_ASSERT(_unionPropertyB._decimalInfo._decimalPrecision >=0 &&
           _unionPropertyB._decimalInfo._decimalPrecision <=TR_MAX_DECIMAL_PRECISION, "unexpected decimal precision %d
           on node %p\n", _unionPropertyB._decimalInfo._decimalPrecision, this); return
           TR::DataType::getSizeFromBCDPrecision(getDataType(), _unionPropertyB._decimalInfo._decimalPrecision);
        */
        TR_ASSERT(self()->hasDecimalInfo(),
            "attempting to access _decimalPrecision field for node %s %p that does not have it",
 ...
 */
TR_OpaqueClassBlock *J9::Node::getCloneClassInNode()
{
    TR_ASSERT(!self()->hasNodeExtension(), "Node %p should not have node extension");
    return (TR_OpaqueClassBlock *)_unionBase._children[1];
}

TR::Node *J9::Node::processJNICall(TR::TreeTop *callNodeTreeTop, TR::ResolvedMethodSymbol *owningSymbol)
{
...
TR::Node *J9::Node::processJNICall(TR::TreeTop *callNodeTreeTop, TR::ResolvedMethodSymbol *owningSymbol,
    TR::Compilation *comp)
{
    if (!comp->cg()->getSupportsDirectJNICalls() || comp->getOption(TR_DisableDirectToJNI)
        || (comp->compileRelocatableCode() && !comp->cg()->supportsDirectJNICallsForAOT()))
        return self();
    ...
    if (((methodSymbol->getRecognizedMethod() == TR::java_util_zip_CRC32_update) ||
        ...
        &&
#endif
        !comp->requiresSpineChecks() && !TR::Compiler->om.isOffHeapAllocationEnabled()
    ...
    if (callNodeTreeTop->getNode()->getOpCode().isNullCheck()) {
        // TR_ASSERT(!callNodeTreeTop->getNode()->getOpCode().isResolveCheck(), "Node::processJNICall, expected NULLCHK
        // node");
            ...
            if (n->getDataType() == TR::Address) {
                if (n->getOpCode().hasSymbolReference() && n->getSymbol()->isAutoOrParm()
                    ...
                    TR::TreeTop::create(comp, callNodeTreeTop->getPrevTreeTop(),
                        TR::Node::createWithSymRef(TR::astore, 1, 1, n, symRef));
    ...
    if (methodSymbol->isStatic()) {
        ...
        // passing cpIndex = -1 to findOrCreateClassSymbol shouldn't be an issue as it will figure out the address from
        // resolvedMethod->containingClass() we only need cpIndex for relocations in AOT and compiling JNI thunks is
        // disable in AOT
        int32_t classCP = (callerCP != -1) ? callerSymbol->getResolvedMethod()->classCPIndexOfMethod(callerCP) : -1;

        TR_ASSERT((callerCP != -1 || callerSymbol->isNative()),
            "Cannot have cp index -1 for JNI calls other than JNI thunks.\n");
        ...
        if (comp->target().is64Bit()) {
            addressOfJavaLangClassReference = TR::Node::create(TR::aladd, 2, addressOfJ9Class,
                TR::Node::lconst(newNode, fej9->getOffsetOfJavaLangClassFromClassField()));
        } else {
            addressOfJavaLangClassReference = TR::Node::create(TR::aiadd, 2, addressOfJ9Class,
        ...
        if (callNodeTreeTop->getNode() == self()) {
            ...
            TR_ASSERT(callNodeTreeTop->getNode()->getChild(0) == self(),
                "call node " POINTER_PRINTF_FORMAT " is not the child of a treetop", self());
...
void J9::Node::devirtualizeCall(TR::TreeTop *treeTop) { return self()->devirtualizeCall(treeTop, TR::comp()); }

void J9::Node::devirtualizeCall(TR::TreeTop *treeTop, TR::Compilation *comp)
{
...
bool J9::Node::isEvenPrecision() { return ((self()->getDecimalPrecision() & 0x1) == 0); }

bool J9::Node::isOddPrecision() { return ((self()->getDecimalPrecision() & 0x1) != 0); }

/**
 * A 'simple' widening or truncation is a pdshl node that changes the precision of its child but has a shift amount of 0
 */
bool J9::Node::isSimpleTruncation()
{
    if (self()->getOpCode().isModifyPrecision()
        && self()->getDecimalPrecision() < self()->getFirstChild()->getDecimalPrecision()) {
        return true;
    } else if (self()->getOpCodeValue() == TR::pdshl && self()->getSecondChild()->getOpCode().isLoadConst()
 ...
 */
bool J9::Node::hasIntermediateTruncation()
{
    TR::Node *valueChild = self()->getValueChild();
    TR_ASSERT(self()->getType().isBCD() && valueChild->getType().isBCD(),
        "hasIntermediateTruncation only valid for BCD parent (dt=%s) and child (dt=%s)\n",
        self()->getDataType().toString(), valueChild->getDataType().toString());
    if (valueChild->isTruncating() && self()->survivingDigits() > valueChild->getDecimalPrecision()) {
        return true;
    } else {
        return false;
    }
 ...
 *
 * Left shift example:
 * For a 3v0 conversion to a 2v2 result (left shift by 2) there are 2 surviving digits -- 123. -> 123.00 but result
 * precision is 4 so only 23.00 survive. The shiftedPrecision is 3+2 = 5 and this is > than the nodePrec of 4. There is
 * 1 truncated digit so reduce the shiftedPrecision by 1 to get the surviving digits value of 2.
 *
 * Right shift example:
 * For a 3v2 conversion to 2v1 result (right shift by 1) there are 3 surviving digits -- 123.45 -> 123.4 but result
 * precision is 3 so only 23.4 survive. The shiftedPrecision is 5-1 = 4 and this > than the nodePrec of 3. There is 1
 * truncated digit so reduce the shiftedPrecision by 1 to get the surviving digits value of 3.
 *
 * @note that the shift formula for surviving digits degenerates to just getDecimalPrecision() when adjust == 0
 */
int32_t J9::Node::survivingDigits()
{
    TR_ASSERT(self()->getType().isBCD(), "node %p (op %d) is not a BCD type (dt=%s)\n", self(),
        self()->getOpCodeValue(), self()->getDataType().toString());
...
bool J9::Node::isTruncating()
{
    if (self()->getType().isBCD() && self()->getNumChildren() >= 1 && self()->getValueChild()->getType().isBCD()) {
        if (self()->getOpCode().isShift())
            ...
            return false;
    } else if (self()->getType().isBCD() && self()->getOpCode().isConversion() && self()->getNumChildren() >= 1
        && !self()->getValueChild()->getType().isBCD()) {
        if (self()->hasSourcePrecision() && self()->getDecimalPrecision() < self()->getSourcePrecision())
            return true;
        else if (!self()->hasSourcePrecision()) // Conservative, but necessary for correctness in some opts; best to
                                                // always set sourcePrecision
            return true;
        else
            return false;
    } else {
...
bool J9::Node::isTruncatingBCDShift()
{
    TR_ASSERT(self()->getType().isBCD() && (self()->getOpCode().isModifyPrecision() || self()->getOpCode().isShift()),
        "node %p (op %d) is not a BCD shift\n", this, self()->getOpCodeValue());

    if (self()->getOpCode().isModifyPrecision()
        && self()->getDecimalPrecision() < self()->getFirstChild()->getDecimalPrecision()) {
        return true;
    } else if (self()->getOpCode().isShift()
        && self()->getDecimalPrecision()
            < (self()->getFirstChild()->getDecimalPrecision() + self()->getDecimalAdjust())) {
        return true;
    } else {
        return false;
    }
...
bool J9::Node::isWideningBCDShift()
{
    TR_ASSERT(self()->getType().isBCD() && (self()->getOpCode().isModifyPrecision() || self()->getOpCode().isShift()),
...
bool J9::Node::mustClean()
{
    if (self()->getType().isAnyPacked()
        && (self()->getOpCodeValue() == TR::pdclean
            || (self()->getOpCode().isStore() && self()->mustCleanSignInPDStoreEvaluator()))) {
        return true;
    }
    return false;
}

void J9::Node::setKnownSignCodeFromRawSign(int32_t rawSignCode)
{
 ...
 */
bool J9::Node::typeSupportedForSignCodeTracking(TR::DataType dt)
{
    return TR::DataType::getSignCodeSize(dt) == EmbeddedHalfByte;
}

bool J9::Node::typeSupportedForTruncateOrWiden(TR::DataType dt)
{
    return dt == TR::PackedDecimal || dt == TR::ZonedDecimalSignLeadingEmbedded || dt == TR::ZonedDecimal;
}

void J9::Node::truncateOrWidenBCDLiteral(TR::DataType dt, char *newLit, int32_t newPrecision, char *oldLit,
    int32_t oldPrecision)
{
    TR_ASSERT(TR::Node::typeSupportedForTruncateOrWiden(dt), "datatype %s not supported in truncateOrWidenBCDLiteral\n",
        dt.toString());
    ...
    if (isLeadingSign) {
        TR_ASSERT(dt == TR::ZonedDecimalSignLeadingEmbedded,
            "only zdsle leading sign type supported in truncateOrWidenBCDLiteral (dt=%s)\n", dt.toString());
        uint8_t sign = oldLit[0] & 0xf0;
        newLit[0] = (newLit[0] & 0x0f) | sign;
    }
}

void J9::Node::setNewBCDSignOnLiteral(uint32_t newSignCode, TR::DataType dt, char *lit, int32_t litSize)
{
    switch (dt) {
...
TR::Node *J9::Node::getSetSignValueNode()
{
...
bool J9::Node::alwaysGeneratesAKnownCleanSign()
{
    TR::Compilation *comp = TR::comp();
    return comp->cg()->alwaysGeneratesAKnownCleanSign(self());
}

bool J9::Node::alwaysGeneratesAKnownPositiveCleanSign()
{
    TR::Compilation *comp = TR::comp();
...
#ifdef TR_TARGET_S390
int32_t J9::Node::getStorageReferenceSize(TR::Compilation *comp)
{
    if (self()->getType().isAggregate()) {
        ...
        TR_ASSERT(self()->getType().isBCD(),
            "node result type should be a BCD type in getStorageReferenceSize and not type %s\n",
        ...
        if ((self()->getOpCode().isPackedRightShift() && self()->getDecimalRound() != 0)
            || self()->getOpCodeValue() == TR::pdshlOverflow)
...
int32_t J9::Node::getStorageReferenceSourceSize(TR::Compilation *comp)
{
    int32_t size = 0;
    switch (self()->getOpCodeValue()) {
        case TR::pd2l:
...
TR_PseudoRegister *J9::Node::getPseudoRegister()
{
...
TR_OpaquePseudoRegister *J9::Node::getOpaquePseudoRegister()
{
...
bool J9::Node::pdshrRoundIsConstantZero()
{
 ...
 */
const char *J9::Node::getTypeSignature(int32_t &len, TR_AllocationKind allocKind, bool parmAsAuto)
{
    ...
    TR_PersistentClassInfo *classInfo = c->getPersistentInfo()->getPersistentCHTable()->findClassInfoAfterLocking(
        c->getCurrentMethod()->containingClass(), c, allowForAOT);
    TR::Node *node = self();
    TR_PersistentFieldInfo *fieldInfo
        = classInfo && classInfo->getFieldInfo() ? classInfo->getFieldInfo()->findFieldInfo(c, node, false) : 0;
...
bool J9::Node::canRemoveArithmeticOperand()
{
    TR::Compilation *comp = TR::comp();
    if (!comp->getOption(TR_KeepBCDWidening) && self()->getOpCodeValue() == TR::pdclean) {
        return true;
    } else if (self()->getOpCodeValue() == TR::pdSetSign) {
        // could also do for pdshxSetSign if pdshx is left in place but shift with setSign is usually cheaper than
        // without
...
uint32_t J9::Node::hashOnBCDOrAggrLiteral(char *lit, size_t litSize)
{
    uint32_t hash = 0;
...
bool J9::Node::referencesSymbolInSubTree(TR::SymbolReference *symRef, vcount_t visitCount)
{
    ...
    for (int32_t i = self()->getNumChildren() - 1; i >= 0; i--) {
        ...
        if (child->referencesSymbolInSubTree(symRef, visitCount))
            return true;
    }
...
bool J9::Node::referencesMayKillAliasInSubTree(TR::Node *rootNode, vcount_t visitCount, TR::Compilation *comp)
{
...
void J9::Node::getSubTreeReferences(TR::SparseBitVector &references, vcount_t visitCount)
{
    // The visit count in the node must be maintained by this method.
    //
    vcount_t oldVisitCount = self()->getVisitCount();
    ...
    if (self()->getOpCode().hasSymbolReference() && self()->getSymbolReference()
...
TR_StorageReference *J9::Node::getStorageReferenceHint()
{
    TR_ASSERT(self()->getOpCode().canHaveStorageReferenceHint(),
        "attempting to access _storageReferenceHint field for node %s %p that does not have it",
...
TR_StorageReference *J9::Node::setStorageReferenceHint(TR_StorageReference *s)
{
    TR_ASSERT(self()->getOpCode().canHaveStorageReferenceHint(),
        "attempting to access _storageReferenceHint field for node %s %p that does not have it",
        self()->getOpCode().getName(), this);
...
bool J9::Node::hasBCDFlags()
{
    // used for BCDCHK only
    return self()->getOpCodeValue() == TR::BCDCHK;
}

J9::Node::UnionPropertyB_Type J9::Node::getUnionPropertyB_Type()
{
    if (self()->hasDecimalInfo())
...
bool J9::Node::hasDecimalPrecision() { return self()->getType().isBCD(); }

bool J9::Node::hasDecimalAdjust()
{
    return !self()->getOpCode().isShift() && !self()->getOpCode().isConversionWithFraction()
        && self()->getType().isBCD();
}

bool J9::Node::hasSetSign() { return self()->getType().isBCD() && self()->getOpCode().isSetSignOnNode(); }

bool J9::Node::hasDecimalFraction() { return self()->getOpCode().isConversionWithFraction(); }

bool J9::Node::hasDecimalRound() { return self()->getType().isBCD() && !self()->getOpCode().isRightShift(); }

void J9::Node::setDecimalPrecision(int32_t p)
{
...
uint8_t J9::Node::getDecimalPrecision()
{
    ...
    TR_ASSERT(self()->hasDecimalInfo(),
        ...
        self()->getOpCode().getName(), self());
    TR_ASSERT(_unionPropertyB._decimalInfo._decimalPrecision > 0
            && _unionPropertyB._decimalInfo._decimalPrecision <= TR_MAX_DECIMAL_PRECISION,
...
void J9::Node::setPDMulPrecision()
{
    ...
    TR_ASSERT(self()->getNumChildren() >= 2, "expecting >= 2 children and not %d children on a packed multiply node\n",
        self()->getNumChildren());
    self()->setDecimalPrecision(
        self()->getFirstChild()->getDecimalPrecision() + self()->getSecondChild()->getDecimalPrecision());
}

void J9::Node::setPDAddSubPrecision()
{
    TR_ASSERT(self()->getOpCode().isPackedAdd() || self()->getOpCode().isPackedSubtract(),
        "setPDAddSubPrecision only valid for pdadd/pdsub nodes\n");
    TR_ASSERT(self()->getNumChildren() >= 2, "expecting >= 2 children and not %d children on a packed add/sub node\n",
        self()->getNumChildren());
    self()->setDecimalPrecision(
        std::max(self()->getFirstChild()->getDecimalPrecision(), self()->getSecondChild()->getDecimalPrecision()) + 1);
}

void J9::Node::setDecimalAdjust(int32_t a)
{
    TR_ASSERT(a >= TR::DataType::getMinDecimalAdjust() && a <= TR::DataType::getMaxDecimalAdjust(),
    ...
    TR_ASSERT(self()->hasDecimalInfo(),
        "attempting to access _decimalAdjustOrFractionOrDivisor field for node %s %p that does not have it",
        self()->getOpCode().getName(), self());
    _unionPropertyB._decimalInfo._decimalAdjustOrFractionOrDivisor = a;
}

int32_t J9::Node::getDecimalAdjust()
{
    // conversions should not have an adjust
    ...
    if (self()->getOpCode().isShift() && self()->getSecondChild()->getOpCode().isLoadConst())
        adjust = self()->getOpCode().isRightShift() ? -self()->getSecondChild()->get64bitIntegralValue()
...
void J9::Node::setDecimalFraction(int32_t f)
{
    TR_ASSERT(self()->hasDecimalInfo(),
        "attempting to access _decimalAdjustOrFractionOrDivisor field for node %s %p that does not have it",
        self()->getOpCode().getName(), self());
    TR_ASSERT(f >= TR::DataType::getMinDecimalFraction() && f <= TR::DataType::getMaxDecimalFraction(),
        "unexpected decimal fraction %d\n", f);
    TR_ASSERT(self()->getOpCode().isConversionWithFraction(),
        "only valid for conversion nodes that have a fraction\n"); // such as f2pd or pd2f
    _unionPropertyB._decimalInfo._decimalAdjustOrFractionOrDivisor = f;
}

int32_t J9::Node::getDecimalFraction()
{
    TR_ASSERT(self()->hasDecimalInfo(),
        "attempting to access _decimalAdjustOrFractionOrDivisor field for node %s %p that does not have it",
        self()->getOpCode().getName(), self());
    TR_ASSERT(_unionPropertyB._decimalInfo._decimalAdjustOrFractionOrDivisor >= TR::DataType::getMinDecimalFraction()
            && _unionPropertyB._decimalInfo._decimalAdjustOrFractionOrDivisor <= TR::DataType::getMaxDecimalFraction(),
        "unexpected decimal fraction %d\n", _unionPropertyB._decimalInfo._decimalAdjustOrFractionOrDivisor);
    TR_ASSERT(self()->getOpCode().isConversionWithFraction(),
        "only valid for conversion nodes that have a fraction\n"); // such as f2pd or pd2f
 ...
 */
bool J9::Node::canHaveSourcePrecision()
{
...
bool J9::Node::hasSourcePrecision()
{
    if (!self()->canHaveSourcePrecision())
        return false;
    TR_ASSERT(self()->hasDecimalInfo(),
        "attempting to access _decimalSourcePrecisionOrDividend field for node %s %p that does not have it",
        self()->getOpCode().getName(), self());
    return (_unionPropertyB._decimalInfo._decimalSourcePrecisionOrDividend > 0);
}

void J9::Node::setSourcePrecision(int32_t prec)
{
    TR_ASSERT(self()->canHaveSourcePrecision(),
...
int32_t J9::Node::getSourcePrecision()
{
    TR_ASSERT(self()->canHaveSourcePrecision(),
...
int32_t J9::Node::getDecimalAdjustOrFractionOrDivisor()
{
...
int32_t J9::Node::getDecimalRoundOrDividend() { return self()->getDecimalRound(); }

bool J9::Node::isSetSignValueOnNode()
{
    if (self()->hasSetSign()) {
        TR_ASSERT(self()->getSetSign() != raw_bcd_sign_unknown,
            "%s (%p) is a setSign node but does not have a value setSign value\n", self()->getOpCode().getName(),
            self());
        return true;
    } else {
        return false;
...
void J9::Node::setSetSign(TR_RawBCDSignCode setSign)
{
    TR_ASSERT(self()->hasSetSign(), "setSetSign only supported for setsign nodes (%s %p)\n",
        self()->getOpCode().getName(), self());
    TR_ASSERT(self()->hasDecimalInfo(),
        "attempting to access _decimalInfo._setSign field for node %s %p that does not have it",
        self()->getOpCode().getName(), self());
    _unionPropertyB._decimalInfo._setSign = setSign;
}

TR_RawBCDSignCode J9::Node::getSetSign()
{
...
bool J9::Node::isDecimalSizeAndShapeEquivalent(TR::Node *other)
{
...
void J9::Node::setDecimalRound(int32_t r)
{
    ...
    TR_ASSERT(self()->hasDecimalInfo(),
        "attempting to access _decimalInfo._round field for node %s %p that does not have it",
        self()->getOpCode().getName(), self());
    _unionPropertyB._decimalInfo._round = (r > 0 ? 1 : 0);
}

uint8_t J9::Node::getDecimalRound()
{
    ...
    TR_ASSERT(round >= 0 && round <= TR::DataType::getMaxDecimalRound() && round <= TR::getMaxUnsigned<TR::Int8>(),
        "unexpected decimal round %d\n", (int32_t)round);
    return (uint8_t)round;
}

bool J9::Node::signStateIsKnown()
{
    TR_ASSERT(self()->hasDecimalInfo(),
        "attempting to access _decimalInfo._signStateIsKnown field for node %s %p that does not have it",
        self()->getOpCode().getName(), self());
    return _unionPropertyB._decimalInfo._signStateIsKnown == 1;
}

void J9::Node::setSignStateIsKnown()
{
...
bool J9::Node::signStateIsAssumed()
{
    TR_ASSERT(self()->hasDecimalInfo(),
        "attempting to access _decimalInfo._signStateIsKnown field for node %s %p that does not have it",
        self()->getOpCode().getName(), self());
...
void J9::Node::setSignStateIsAssumed()
{
...
bool J9::Node::hasKnownCleanSign()
{
    TR_ASSERT(self()->getType().isBCD(), "hasKnownCleanSign only supported for BCD type nodes\n");
    TR_ASSERT(self()->hasDecimalInfo(),
        "attempting to access _decimalInfo._hasCleanSign field for node %s %p that does not have it",
        self()->getOpCode().getName(), self());
...
void J9::Node::setHasKnownCleanSign(bool v)
{
    TR::Compilation *c = TR::comp();
...
bool J9::Node::hasAssumedCleanSign()
{
...
void J9::Node::setHasAssumedCleanSign(bool v)
{
    TR::Compilation *c = TR::comp();
...
bool J9::Node::hasKnownOrAssumedCleanSign()
{
...
void J9::Node::setHasKnownAndAssumedCleanSign(bool v)
{
    TR_ASSERT(self()->hasDecimalInfo(),
        "attempting to access _decimalInfo._hasCleanSign field for node %s %p that does not have it",
        self()->getOpCode().getName(), self());
...
void J9::Node::transferCleanSign(TR::Node *srcNode)
{
    if (srcNode == NULL)
        return;
...
bool J9::Node::hasKnownPreferredSign()
{
    TR_ASSERT(self()->getType().isBCD(), "hasKnownPreferredSign only supported for BCD type nodes\n");
    TR_ASSERT(self()->hasDecimalInfo(),
        "attempting to access _decimalInfo._hasPreferredSign field for node %s %p that does not have it",
        self()->getOpCode().getName(), self());
    return self()->signStateIsKnown() && _unionPropertyB._decimalInfo._hasPreferredSign == 1;
}

void J9::Node::setHasKnownPreferredSign(bool v)
{
    TR::Compilation *c = TR::comp();
...
bool J9::Node::hasAssumedPreferredSign()
{
...
void J9::Node::setHasAssumedPreferredSign(bool v)
{
    TR::Compilation *c = TR::comp();
...
bool J9::Node::hasKnownOrAssumedPreferredSign()
{
...
bool J9::Node::hasAssumedSignCode()
{
...
TR_RawBCDSignCode J9::Node::getAssumedSignCode()
{
...
bool J9::Node::hasKnownOrAssumedSignCode()
{
...
TR_RawBCDSignCode J9::Node::getKnownOrAssumedSignCode()
{
    TR_ASSERT(self()->getType().isBCD(), "getKnownSignCode only supported for BCD type nodes\n");
...
void J9::Node::setKnownOrAssumedSignCode(TR_RawBCDSignCode sign, bool isKnown)
{
    TR::Compilation *c = TR::comp();
    TR_ASSERT(self()->getType().isBCD(), "setKnownSignCode only supported for BCD type nodes\n");
    TR_ASSERT(self()->hasDecimalInfo(), "attempting to access _decimalInfo fields for node %s %p that does not have it",
        self()->getOpCode().getName(), self());
    // TODO: start tracking SeparateOneByte and SeparateTwoByte sign sizes too
    if (self()->getType().isBCD() && TR::Node::typeSupportedForSignCodeTracking(self()->getDataType())) {
        ...
        if ((sign == raw_bcd_sign_0xc || sign == raw_bcd_sign_0xd)
            && (0xc == TR::DataType::getPreferredPlusCode() && 0xd == TR::DataType::getPreferredMinusCode()))
            _unionPropertyB._decimalInfo._hasPreferredSign = 1;
    }
}

void J9::Node::setKnownSignCode(TR_RawBCDSignCode sign)
{
    self()->setKnownOrAssumedSignCode(sign, true); // isKnown=true
}

void J9::Node::transferSignCode(TR::Node *srcNode)
{
    if (srcNode == NULL)
        return;
 ...
 * The final code will be just calling setKnownOrAssumedSignCode(sign, true, c) but now the 'sign' parm will be
 * TR_BCDSignCode of TR_RawBCDSignCode
 */
bool J9::Node::knownSignCodeIs(TR_BCDSignCode sign)
{
    if (self()->hasKnownSignCode() && TR::DataType::getBCDSignFromRawSign(self()->getKnownSignCode()) == sign) {
        return true;
    } else {
        return false;
    }
...
void J9::Node::setKnownSignCode(TR_BCDSignCode sign)
{
    if (TR::Node::typeSupportedForSignCodeTracking(self()->getDataType())) {
...
bool J9::Node::assumedSignCodeIs(TR_BCDSignCode sign)
{
    if (self()->hasAssumedSignCode() && TR::DataType::getBCDSignFromRawSign(self()->getAssumedSignCode()) == sign) {
        return true;
    } else {
        return false;
    }
...
void J9::Node::setAssumedSignCode(TR_RawBCDSignCode sign)
{
    self()->setKnownOrAssumedSignCode(sign, false); // isKnown=false
}

bool J9::Node::knownOrAssumedSignCodeIs(TR_BCDSignCode sign)
{
...
/**
 * The hasSignState property is stored in the reverse sense to make the interface easier to use (at the expense of some
 * confusion here)
 */
bool J9::Node::hasSignStateOnLoad()
{
    if (!self()->getOpCode().isBCDLoad())
        return false;

    TR_ASSERT(self()->hasDecimalInfo(),
        "attempting to access _decimalInfo._hasNoSignStateOnLoad field for node %s %p that does not have it",
        self()->getOpCode().getName(), self());
    if (_unionPropertyB._decimalInfo._hasNoSignStateOnLoad == 0) // !NoSign == HasSignState
        return true; // i.e. sign state of load must be preserved (no ZAP widening, for example, as this could change
                     // 0xf -> 0xc) -- this is the conservative setting
...
void J9::Node::setHasSignStateOnLoad(bool v)
{
    TR::Compilation *c = TR::comp();
    ...
    if (self()->getOpCode().isBCDLoad()
        ...
        TR_ASSERT(self()->hasDecimalInfo(),
            ...
            self()->getOpCode().getName(), self());
        _unionPropertyB._decimalInfo._hasNoSignStateOnLoad
            = v ? 0 : 1; // v=true is the conservative setting, v=false will allow clobbering of the sign code
    }
}

void J9::Node::transferSignState(TR::Node *srcNode, bool digitsLost)
{
    TR_ASSERT(self()->hasDecimalInfo(), "attempting to access _decimalInfo fields for node %s %p that does not have it",
        self()->getOpCode().getName(), self());
    _unionPropertyB._decimalInfo._signStateIsKnown = srcNode->signStateIsKnown();
    _unionPropertyB._decimalInfo._hasCleanSign
        = digitsLost ? 0 : srcNode->hasKnownOrAssumedCleanSign(); //  '10 0d' might be truncated to negative zero '0
                                                                  //  00d' -- so no longer clean
...
bool J9::Node::hasAnyKnownOrAssumedSignState()
{
    if (self()->hasKnownOrAssumedCleanSign() || self()->hasKnownOrAssumedPreferredSign()
...
bool J9::Node::hasAnyDecimalSignState()
{
    TR_ASSERT(self()->hasDecimalInfo(), "attempting to access _decimalInfo fields for node %s %p that does not have it",
        self()->getOpCode().getName(), self());
    if (_unionPropertyB._decimalInfo._hasCleanSign == 1 || _unionPropertyB._decimalInfo._hasPreferredSign == 1
...
void J9::Node::resetDecimalSignFlags()
{
    TR_ASSERT(self()->hasDecimalInfo(), "attempting to access _decimalInfo fields for node %s %p that does not have it",
        self()->getOpCode().getName(), self());
    _unionPropertyB._decimalInfo._signStateIsKnown = 0;
...
bool J9::Node::isSignStateEquivalent(TR::Node *other)
{
...
/**
 * if the 'other' node does not currently have any sign state then an improvement will be any sign state set on 'this'
 * node
 */
bool J9::Node::isSignStateAnImprovementOver(TR::Node *other)
{
    ...
    // having some (but undetermined) sign state for loads can prevent optimization or hinder codegen vs not having to
    // assume anything so return false in this case (but wouldn't be incorrect to return true in this case assuming
    // other conditions below hold)
...
bool J9::Node::chkOpsCastedToBCD()
{
    if (self()->getType().isBCD()) {
        return true;
...
bool J9::Node::castedToBCD()
{
    if (self()->chkOpsCastedToBCD()) {
        TR_ASSERT(self()->hasDecimalInfo(),
            "attempting to access _decimalInfo._castedToBCD field for node %s %p that does not have it",
            self()->getOpCode().getName(), self());
...
void J9::Node::setCastedToBCD(bool v)
{
...
bool J9::Node::isSpineCheckWithArrayElementChild()
{
    TR_ASSERT(self()->getOpCode().isSpineCheck(), "assertion failure");
    return _flags.testAny(spineCHKWithArrayElementChild);
}

void J9::Node::setSpineCheckWithArrayElementChild(bool v, TR::Compilation *comp)
{
    TR_ASSERT(self()->getOpCode().isSpineCheck(), "assertion failure");
    if (performNodeTransformation2(comp,
            "O^O NODE FLAGS: Setting spineCHKWithArrayElementChild flag on node %p to %d\n", self(), v))
        _flags.set(spineCHKWithArrayElementChild, v);
}

bool J9::Node::chkSpineCheckWithArrayElementChild()
{
    return self()->getOpCode().isSpineCheck() && _flags.testAny(spineCHKWithArrayElementChild);
}

bool J9::Node::isUnsafeCopyMemoryIntrinsic()
{
...
bool J9::Node::isUnsafeGetPutCASCallOnNonArray()
{
    ...
    TR_ASSERT(symbol->getMethod()->isUnsafeWithObjectArg() || symbol->getMethod()->isUnsafeCAS(),
        "Attempt to check flag on a method that is not JNI Unsafe that needs special care for arraylets\n");
    return _flags.testAny(unsafeGetPutOnNonArray);
}

void J9::Node::setUnsafeGetPutCASCallOnNonArray() { self()->setUnsafeGetPutCASCallOnNonArray(TR::comp()); }

void J9::Node::setUnsafeGetPutCASCallOnNonArray(TR::Compilation *comp)
{
    TR_ASSERT(self()->getSymbol()->isMethod() && self()->getSymbol()->getMethodSymbol(),
        "setUnsafeGetPutCASCallOnNonArray called on node which is not a call");

    TR_ASSERT(self()->getSymbol()->getMethodSymbol()->getMethod()->isUnsafeWithObjectArg()
            || self()->getSymbol()->getMethodSymbol()->getMethod()->isUnsafeCAS(),
        "Attempt to check flag on a method that is not JNI Unsafe that needs special care for arraylets\n");
...
bool J9::Node::isProcessedByCallCloneConstrain()
{
    return self()->getOpCode().isCall() && self()->getOpCodeValue() != TR::arraycopy
        && _flags.testAny(processedByCallCloneConstrain);
}

void J9::Node::setProcessedByCallCloneConstrain() { _flags.set(processedByCallCloneConstrain, true); }

bool J9::Node::isDAAVariableSlowCall()
{
    TR_ASSERT(self()->getOpCode().isCall(), "Opcode must be a call");
    return _flags.testAny(DAAVariableSlowCall);
}

void J9::Node::setDAAVariableSlowCall(bool v)
{
    TR_ASSERT(self()->getOpCode().isCall(), "Opcode must be a call");
    _flags.set(DAAVariableSlowCall);
}

bool J9::Node::isBCDStoreTemporarilyALoad()
{
    TR_ASSERT(self()->getOpCode().isBCDLoadVar(), "flag only valid for BCD load ops\n");
...
void J9::Node::setBCDStoreIsTemporarilyALoad(bool v)
{
    TR::Compilation *c = TR::comp();
    TR_ASSERT(self()->getOpCode().isBCDLoadVar(), "flag only valid for BCD load ops\n");
    if (self()->getOpCode().isBCDLoadVar()
...
bool J9::Node::cleanSignDuringPackedLeftShift()
{
...
void J9::Node::setCleanSignDuringPackedLeftShift(bool v)
{
    TR::Compilation *c = TR::comp();
    TR_ASSERT(self()->getOpCode().isPackedLeftShift(), "flag only valid for isPackedLeftShift nodes\n");
    if (self()->getOpCode().isPackedLeftShift()
        && performNodeTransformation2(c,
            "O^O NODE FLAGS: Setting CleanSignDuringPackedLeftShift flag on node %p to %d\n", self(), v))
        _flags.set(CleanSignDuringPackedLeftShift, v);
}

bool J9::Node::chkCleanSignDuringPackedLeftShift()
{
    return self()->getOpCode().isPackedLeftShift() && _flags.testAny(CleanSignDuringPackedLeftShift);
}

bool J9::Node::chkOpsSkipCopyOnLoad()
{
    return (self()->getType().isBCD() || self()->getType().isAggregate()) && !self()->getOpCode().isStore()
        && !self()->getOpCode().isCall();
}

bool J9::Node::skipCopyOnLoad()
{
    TR_ASSERT(self()->chkOpsSkipCopyOnLoad(), "flag only valid for BCD or aggregate non-store and non-call ops\n");
    if ((self()->getType().isBCD() || self()->getType().isAggregate()) && !self()->getOpCode().isStore()
        && !self()->getOpCode().isCall())
...
void J9::Node::setSkipCopyOnLoad(bool v)
{
    TR::Compilation *c = TR::comp();
    TR_ASSERT(self()->chkOpsSkipCopyOnLoad(), "flag only valid for BCD or aggregate non-store and non-call ops\n");
    if ((self()->getType().isBCD() || self()->getType().isAggregate()) && !self()->getOpCode().isStore()
...
bool J9::Node::chkSkipCopyOnLoad() { return self()->chkOpsSkipCopyOnLoad() && _flags.testAny(SkipCopyOnLoad); }

bool J9::Node::chkOpsSkipCopyOnStore() { return self()->getOpCode().isBCDStore(); }

bool J9::Node::skipCopyOnStore()
{
...
void J9::Node::setSkipCopyOnStore(bool v)
{
    TR::Compilation *c = TR::comp();
    TR_ASSERT(self()->chkOpsSkipCopyOnStore(), "flag only valid for BCD or aggregate store ops\n");
    if (self()->chkOpsSkipCopyOnStore()
...
bool J9::Node::chkSkipCopyOnStore() { return self()->chkOpsSkipCopyOnStore() && _flags.testAny(SkipCopyOnStore); }

bool J9::Node::chkOpsCleanSignInPDStoreEvaluator()
{
    return self()->getDataType() == TR::PackedDecimal && self()->getOpCode().isStore();
}

bool J9::Node::mustCleanSignInPDStoreEvaluator()
{
...
void J9::Node::setCleanSignInPDStoreEvaluator(bool v)
{
    TR::Compilation *c = TR::comp();
    TR_ASSERT(self()->chkOpsCleanSignInPDStoreEvaluator(), "flag only valid for packed decimal store nodes\n");
    if (self()->chkOpsCleanSignInPDStoreEvaluator()
...
bool J9::Node::chkCleanSignInPDStoreEvaluator()
{
    return self()->chkOpsCleanSignInPDStoreEvaluator() && _flags.testAny(cleanSignInPDStoreEvaluator);
}

bool J9::Node::chkOpsUseStoreAsAnAccumulator() { return self()->getOpCode().canUseStoreAsAnAccumulator(); }

bool J9::Node::useStoreAsAnAccumulator()
{
...
void J9::Node::setUseStoreAsAnAccumulator(bool v)
{
    TR::Compilation *c = TR::comp();
    TR_ASSERT(self()->chkOpsUseStoreAsAnAccumulator(), "flag only valid for BCD or aggregate store ops\n");
    if (self()->chkOpsUseStoreAsAnAccumulator()
...
void J9::Node::setSkipPadByteClearing(bool v)
{
    TR::Compilation *c = TR::comp();
    TR_ASSERT(self()->getType().isAnyPacked() && !self()->getOpCode().isStore(),
        "flag only valid for wcode non-store packed operations\n");
    if (self()->getType().isAnyPacked() && !self()->getOpCode().isStore()
        && (c == NULL
            || performNodeTransformation2(c, "O^O NODE FLAGS: Setting skipPadByteClearing flag on node %p to %d\n",
                self(), v)))
        _flags.set(skipPadByteClearing, v);
}

bool J9::Node::chkSkipPadByteClearing()
{
    return self()->getType().isAnyPacked() && !self()->getOpCode().isStore() && _flags.testAny(skipPadByteClearing);
}

bool J9::Node::chkOpsIsInMemoryCopyProp()
{
    TR::Compilation *c = TR::comp();
    return self()->getOpCode().isStore() && c->cg()->IsInMemoryType(self()->getType());
}

bool J9::Node::isInMemoryCopyProp()
{
...
void J9::Node::setIsInMemoryCopyProp(bool v)
{
    if (self()->chkOpsIsInMemoryCopyProp()) {
...
bool J9::Node::isArrayCopyCall()
{
    if (self()->getOpCode().isCall() && self()->getSymbol()->isMethod()) {
        ...
        if (recognizedMethod == TR::java_lang_System_arraycopy
            ...
            // Method may be unresolved but we would still like to transform arraycopy calls so pattern match the
            // signature as well
            if (method != NULL && (method->nameLength() == 9) && (method->classNameLength() == 16)
...
bool J9::Node::requiresRegisterPair(TR::Compilation *comp) { return OMR::NodeConnector::requiresRegisterPair(comp); }

bool J9::Node::canGCandReturn() { return self()->canGCandReturn(TR::comp()); }

bool J9::Node::canGCandReturn(TR::Compilation *comp)
{
    if (comp->getOption(TR_EnableFieldWatch)) {
        if (self()->getOpCodeValue() == TR::treetop || self()->getOpCode().isNullCheck()
            || self()->getOpCode().isAnchor()) {
...
bool J9::Node::isJitDispatchJ9MethodCall(TR::Compilation *comp)
{
