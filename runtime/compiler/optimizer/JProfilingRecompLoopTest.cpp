 ...
 * list by checking bytecode index and caller index of the bytecode info
 */
bool TR_JProfilingRecompLoopTest::isByteCodeInfoInCurrentTestLocationList(TR_ByteCodeInfo &bci,
...
int32_t TR_JProfilingRecompLoopTest::perform()
{
    if (comp()->getProfilingMode() != JProfiling) {
        logprints(trace(), comp()->log(),
            "JProfiling for profiling compilations has not been enabled, skip JProfilingRecompLoopTest\n");
        return 0;
    }
    ...
    RecompilationTestLocationsInfo testLocations(
        RecompilationTestLocationInfoAllocator(comp()->trMemory()->currentStackRegion()));
    ...
    while (cursor) {
        if (cursor->getNode()->getOpCodeValue() == TR::BBStart) {
            ...
            /**
             * As we are already walking down the tree tops, we can get the enclosing block by tracking TR::BBStart
             * nodes and currentBlock contains this information. We also keep local list of ByteCodeInfo for each test
             * locations (asyncchecknode) in the extended basic blocks. This can avoid adding multiple tests for
             * multiple locations with same byte code info in extended basic block. As soon as we encounter a block
             * which is not extensions of previous block, we clear the list of byte code info.
             */
            ...
            TR_ASSERT_FATAL(currentBlock != NULL,
                "We should have encountered BBStart before and should have the enclosing block");
            if (currentBlock->getStructureOf()->getContainingLoop() != NULL) {
                ...
                // bytecode info of current asynccheck node does not match with any tracked bytecode info, we will add
                // the test otherwise skip it. We can have list for addedLocationBCIList as in extended basic blocks,
                // there will be very less test locations so This list won't be too large.
                if (addedLocationBCIList.empty()
                    || !isByteCodeInfoInCurrentTestLocationList(bci, addedLocationBCIList)) {
...
void TR_JProfilingRecompLoopTest::addRecompilationTests(TR::Compilation *comp,
    RecompilationTestLocationsInfo &testLocations, TR_BlockFrequencyInfo *bfi)
{
    ...
    // Iterating backwards to avoid losing original block associated with the test location tree tops in case we have
    // found multiple recompilation test location in same block.
    for (auto testLocationIter = testLocations.rbegin(), testLocationEnd = testLocations.rend();
         testLocationIter != testLocationEnd; ++testLocationIter) {
        ...
        logprintf(trace(), log, "block_%d, n%dn, depth = %d\n", originalBlock->getNumber(),
            asyncCheckTreeTop->getNode()->getGlobalIndex(), depth);
        ...
        // If we got a bad counters/ bad block frequency info, above API would return NULL, if that is the case we can
        // not generate a recompilation test for that location.
        if (!root) {
        ...
        const char * const name
            = TR::DebugCounter::debugCounterName(comp, "recompilationHelper/(%s)/%d", comp->signature(), depth);
        TR::DebugCounter::prependDebugCounter(comp, name, callTree);

        // Code to calculate the raw frequency of loop from block counters and comparing with the adjusted recompilation
        // threshold. threshold for this particular test location is calculated from the base recompile threshold and
        // the nesting depth of the loop Putting a higher threshold limit to 10K currently to prohibit running profiling
        // body for too long.
        ...
        // To safeguard this scenario, we also check if threshold is negative or zero we set this threshold to
        // maxLoopRecompileThreshold
        TR::Node *cmpNode = TR::Node::createif(TR::ificmple, root,
            TR::Node::iconst(node,
                (threshold > 0 && threshold <= maxLoopRecompilationThreshold) ? threshold
                                                                              : maxLoopRecompilationThreshold),
            remainingCodeBlock->getEntry());
        ...
        if (generateJProfilingRecompQueueTest) {
            TR::Block *calculateLoopRawFreq
                = TR::Block::createEmptyBlock(node, comp, remainingCodeBlock->getFrequency());
            ...
            TR::Node *checkIfQueueForRecompilation = TR::Node::createif(TR::ificmpeq, loadIsQueuedForRecompilation,
                TR::Node::iconst(node, -1), remainingCodeBlock->getEntry());
            TR::TreeTop *checkIfNeededRecompilationTestTT
                = TR::TreeTop::create(comp, originalBlock->getLastRealTreeTop(), checkIfQueueForRecompilation);
            calculateLoopRawFreq->append(cmpFlag);
            cfg->addNode(calculateLoopRawFreq);
            callRecompileBlock->getEntry()->insertTreeTopsBeforeMe(calculateLoopRawFreq->getEntry(),
                calculateLoopRawFreq->getExit());
        ...
        logprintf(trace(), log,
            "\t\t Newly created recompilation Test : Threshold comparison Node n%dn\n\t\tRecompilation Call in "
            "block_%d\n",
            cmpNode->getGlobalIndex(), callRecompileBlock->getNumber());
    }
}

const char *TR_JProfilingRecompLoopTest::optDetailString() const throw() { return "O^O JPROFILER RECOMP TEST: "; }
