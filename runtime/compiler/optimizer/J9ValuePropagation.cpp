 ...
 */
void J9::ValuePropagation::transformCallToNodeWithHCRGuard(TR::TreeTop *callTree, TR::Node *result)
{
    static const char *disableHCRGuards = feGetEnv("TR_DisableHCRGuards");
    TR_ASSERT(!disableHCRGuards && comp()->getHCRMode() != TR::none,
        "foldCallToConstantInHCRMode should be called in HCR mode");
    ...
    if (!comp()->incInlineDepth(calleeSymbol, callNode, !callNode->getOpCode().isCallIndirect(), NULL,
            calleeSymbol->getResolvedMethod()->classOfMethod(), 0)) {
        logprintf(trace(), comp()->log(), "Cannot inline call %p, quit transforming it into a constant\n", callNode);
        return;
    }
    // With the method added to the inlining table, getCurrentInlinedSiteIndex() will return the desired inlined site
    // index
    int16_t calleeIndex = comp()->getCurrentInlinedSiteIndex();
    TR::Node *compareNode = TR_VirtualGuard::createHCRGuard(comp(), calleeIndex, callNode, NULL, calleeSymbol,
        calleeSymbol->getResolvedMethod()->classOfMethod());
    ...
    J9::TransformUtil::createDiamondForCall(this, callTree, compareTree, ifTree, elseTree,
        false /*changeBlockExtensions*/, true /*markCold*/);
    comp()->decInlineDepth();
}
 ...
 */
uintptr_t *J9::ValuePropagation::getObjectLocationFromConstraint(TR::VPConstraint *constraint)
{
    ...
    if (constraint->isConstString()) {
        ...
        objectLocation = (uintptr_t *)constraint->getClassType()
                             ->asConstString()
                             ->getSymRef()
                             ->getSymbol()
                             ->castToStaticSymbol()
 ...
 */
void J9::ValuePropagation::transformCallToIconstInPlaceOrInDelayedTransformations(TR::TreeTop *callTree, int32_t result,
    bool isGlobal, bool inPlace, bool requiresGuard)
{
    ...
    if (inPlace) {
        logprintf(trace(), log, "Fold the call to %s on node %p to %d\n", signature, callNode, result);
        replaceByConstant(callNode, TR::VPIntConst::create(this, result), isGlobal);
    } else {
        logprintf(trace(), log, "The call to %s on node %p will be folded to %d in delayed transformations\n",
            signature, callNode, result);
        _callsToBeFoldedToNode.add(new (trStackMemory()) TreeNodeResultPair(callTree,
            TR::Node::iconst(callTree->getNode()->getFirstChild(), result), requiresGuard));
    }
}

void J9::ValuePropagation::transformCallToNodeDelayedTransformations(TR::TreeTop *callTree, TR::Node *result,
    bool requiresGuard)
{
    TR::Node *callNode = callTree->getNode()->getFirstChild();
    TR::Method *calledMethod = callNode->getSymbol()->castToMethodSymbol()->getMethod();
    const char *signature = calledMethod ? calledMethod->signature(comp()->trMemory(), stackAlloc) : NULL;
    logprintf(trace(), comp()->log(), "The call to %s on node %p will be folded in delayed transformations\n",
 ...
 */
TR_YesNoMaybe J9::ValuePropagation::isStringObject(TR::VPConstraint *constraint)
{
    if (constraint && constraint->getClassType() && constraint->getClass()
 ...
 */
bool J9::ValuePropagation::isKnownStringObject(TR::VPConstraint *constraint)
{
    return isStringObject(constraint) == TR_yes && constraint->isNonNullObject()
        && (constraint->isConstString() || constraint->getKnownObject());
}

bool J9::ValuePropagation::transformIndexOfKnownString(TR::Node *indexOfNode, TR::Node *sourceStringNode,
    TR::Node *targetCharNode, TR::Node *startNode, TR::Node *lengthNode, bool is16Bit)
{
    // Keep track of whether or not all constraints are global.
    bool isGlobal = true;
    bool isGlobalQuery;
    int32_t headerAdjustment
        = TR::Compiler->om.isOffHeapAllocationEnabled() ? 0 : TR::Compiler->om.contiguousArrayHeaderSizeInBytes();
    ...
    TR::VMAccessCriticalSection transformIndexOfCriticalSection(comp(),
    ...
    if (knownObject) {
        ...
        uintptr_t stringStaticAddr
            = (uintptr_t)constString->getSymRef()->getSymbol()->castToStaticSymbol()->getStaticAddress();
    ...
    if (length == 0 || start >= length) {
        if (performTransformation(comp(),
                "%sReplacing indexOf call node [" POINTER_PRINTF_FORMAT
                "] on empty string receiver with constant value of -1\n",
        ...
        for (int32_t i = start; i < length; ++i) {
            ...
            if (knownObject) {
                if (is16Bit) {
                    uintptr_t element
                        = TR::Compiler->om.getAddressOfElement(comp(), string, (2 * i) + headerAdjustment);
                    ch = *((uint16_t *)element);
                } else {
            ...
            if (ch == targetChar) {
                if (performTransformation(comp(),
                        "%sReplacing indexOf call node [" POINTER_PRINTF_FORMAT
                        "] on known string receiver with constant value of %d\n",
        ...
        if (performTransformation(comp(),
                "%sReplacing indexOf call node [" POINTER_PRINTF_FORMAT
                "] on known string receiver with constant value of -1\n",
                OPT_DETAILS, indexOfNode)) {
            replaceByConstant(indexOfNode, TR::VPIntConst::create(this, -1), isGlobal);
            return true;
        }
        ...
        if (knownObject) {
            if (is16Bit) {
                uintptr_t element
                    = TR::Compiler->om.getAddressOfElement(comp(), string, (2 * start) + headerAdjustment);
                ch = *((uint16_t *)element);
            } else {
        ...
        if (!performTransformation(comp(),
                "%sReplacing indexOf call node [" POINTER_PRINTF_FORMAT
                "] on known string receiver with equivalent icmpeq tree\n",
                OPT_DETAILS, indexOfNode))
            return false;
        ...
        if (!is16Bit && (targetCharNode->getOpCodeValue() != TR::b2i)
            && ((targetConstraint == NULL) || (targetConstraint->getLowInt() < -128)
                || (targetConstraint->getHighInt() > 127))) {
            targetCharNode
                = TR::Node::create(indexOfNode, TR::b2i, 1, TR::Node::create(indexOfNode, TR::i2b, 1, targetCharNode));
        }

        transformCallToNodeDelayedTransformations(_curTree,
            TR::Node::create(indexOfNode, TR::isub, 2,
                TR::Node::create(indexOfNode, TR::icmpeq, 2, targetCharNode, TR::Node::iconst(indexOfNode, ch)),
        ...
        if (!performTransformation(comp(),
                "%sReplacing indexOf call node [" POINTER_PRINTF_FORMAT
                "] on known string receiver with equivalent iselect tree\n",
                OPT_DETAILS, indexOfNode))
            return false;
        ...
        for (int32_t i = length - 1; i >= start; --i) {
            ...
            root = TR::Node::create(TR::iselect, 3,
                TR::Node::create(indexOfNode, TR::icmpeq, 2, targetCharNode, TR::Node::iconst(indexOfNode, ch)),
                TR::Node::iconst(indexOfNode, i), root);
        }
...
bool J9::ValuePropagation::transformUnsafeCopyMemoryCall(TR::Node *arraycopyNode)
{
    ...
    if (comp()->canTransformUnsafeCopyToArrayCopy() && arraycopyNode->isUnsafeCopyMemoryIntrinsic()) {
        ...
        if (TR::Compiler->om.isOffHeapAllocationEnabled()
            ...
            _offHeapCopyMemory.add(
                new (comp()->trStackMemory()) J9::ValuePropagation::TR_TreeTopNodePair(tt, arraycopyNode));
            return true;
        }
        ...
        if ((ttNode->getOpCodeValue() == TR::treetop || ttNode->getOpCode().isResolveOrNullCheck())
            ...
            int64_t srcOffHigh
                = srcOffsetConstraint ? srcOffsetConstraint->getHighInt() : TR::getMaxSigned<TR::Int32>();
            int64_t dstOffLow = dstOffsetConstraint ? dstOffsetConstraint->getLowInt() : TR::getMinSigned<TR::Int32>();
            int64_t dstOffHigh
                = dstOffsetConstraint ? dstOffsetConstraint->getHighInt() : TR::getMaxSigned<TR::Int32>();
...
TR_YesNoMaybe J9::ValuePropagation::isValue(TR::VPConstraint *constraint, TR_OpaqueClassBlock *&clazz)
{
...
static TR::Node *getStoreValueBaseNode(TR::Node *storeValueNode, TR::SymbolReferenceTable *symRefTab)
{
    ...
    //
    if (storeValueNode->getOpCode().isLoadVar() && storeValueNode->getOpCode().isIndirect()
        ...
        && (symRefTab->isNonHelper(storeValueNode->getSymbolReference(),
...
void J9::ValuePropagation::processRefinedMethodHandleINLCall(TR::Node *node)
{
    OMR::Logger *log = comp()->log();
    ...
    for (int32_t i = 0; i < node->getNumChildren(); i++) {
        ...
        if (argNode && argNode->getDataType() == TR::Address) {
            ...
            if (argConstraint && argConstraint->getKnownObject() && argConstraint->isNonNullObject()) {
                argInfo->set(i,
                    new (trStackMemory()) TR_PrexArgument(argConstraint->getKnownObject()->getIndex(), comp()));
                logprintf(trace(), log, "PREX.vp:    Child %d [%p] arg is known object obj%d\n", i, argInfo->get(i),
                    argConstraint->getKnownObject()->getIndex());
            }
        }
    }

    logprintf(trace(), log, "PREX.vp: Done populating prex argInfo for %s %p.\n", node->getOpCode().getName(), node);
    ...
    _refinedMethodHandleINLMethodsToInline.add(
        new (trStackMemory()) OMR::ValuePropagation::CallInfo(this, NULL, argInfo));
}
...
void J9::ValuePropagation::constrainRecognizedMethod(TR::Node *node)
{
    ...
    const bool isObjectEqualityCompare = comp()->getSymRefTab()->isNonHelper(node->getSymbolReference(),
        TR::SymbolReferenceTable::objectEqualityComparisonSymbol);

    const bool isObjectInequalityCompare = comp()->getSymRefTab()->isNonHelper(node->getSymbolReference(),
        TR::SymbolReferenceTable::objectInequalityComparisonSymbol);

    if (isObjectEqualityCompare || isObjectInequalityCompare) {
        // Only constrain the call in the last run of vp to avoid handling the candidate twice if the call is inside a
        // loop
        if (lastTimeThrough()) {
        ...
        if (trace()) {
            log->printf("%s: callNode n%dn: lhsNode n%dn (lhsClass 0x%p)(isLhsValue %s)(lhs %p ValueNumber %d) ",
                __FUNCTION__, node->getGlobalIndex(), lhsNode->getGlobalIndex(), lhsClass,
                comp()->getDebug()->getName(isLhsValue), lhs, getValueNumber(lhsNode));
            log->printf("rhsNode n%dn (rhsClass 0x%p)(isRhsValue %s)(rhs %p ValueNumber %d), areSameRef %d\n",
                rhsNode->getGlobalIndex(), rhsClass, comp()->getDebug()->getName(isRhsValue), rhs,
                getValueNumber(rhsNode), areSameRef);
        }
        ...
        if (isLhsValue == TR_no || isRhsValue == TR_no || areSameRef) {
            ...
            if (performTransformation(comp(), "%sChanging n%un from %s to %s\n", OPT_DETAILS, node->getGlobalIndex(),
                    comp()->getSymRefTab()->getNonHelperSymbolName(isObjectEqualityCompare
                            ? TR::SymbolReferenceTable::objectEqualityComparisonSymbol
                            : TR::SymbolReferenceTable::objectInequalityComparisonSymbol),
                    acmpOp.getName())) {
                ...
                _valueTypesHelperCallsToBeFolded.add(new (trStackMemory()) ObjectComparisonHelperCallTransform(_curTree,
                    node, ValueTypesHelperCallTransform::InsertDebugCounter));
                ...
                TR_ASSERT_FATAL_WITH_NODE(node, replacement == node, "can't replace n%un here", node->getGlobalIndex());
            ...
            if (fieldCount == 0) {
                ...
                if (dataType.isIntegral()) {
                    ...
                    TR_OpaqueClassBlock *fieldClazz = fe()->getClassFromSignature(fieldEntry._typeSignature,
                        (int32_t)strlen(fieldEntry._typeSignature), comp()->getCurrentMethod());
            ...
            if (transformObjectCmp) {
                ...
                _valueTypesHelperCallsToBeFolded.add(new (trStackMemory())
                        ObjectComparisonHelperCallTransform(_curTree, node, flagsForTransform, lhsClass));

                logprintf(trace(), log,
                    "%s: Add delayed transformation for callNode n%dn: lhsNode n%dn rhsNode n%dn fieldCount %d "
                    "lhsClass %p\n",
                    __FUNCTION__, node->getGlobalIndex(), lhsNode->getGlobalIndex(), rhsNode->getGlobalIndex(),
                    fieldCount, lhsClass);
            }
        }
        ...
        if (!transformObjectCmp) {
            ...
            const char *counterName = TR::DebugCounter::debugCounterName(comp(),
                "vt-helper/vp-failed/acmp/(%s)/%s/block_%d/%s", comp()->signature(),
                comp()->getHotnessName(comp()->getMethodHotness()), _curTree->getEnclosingBlock()->getNumber(), reason);
    ...
    TR::SymbolReference *loadFlattenableElementHelperSymRef
        = symRefTab->findOrCreateLoadFlattenableArrayElementSymbolRef();
    TR::SymbolReference *storeFlattenableElementHelperSymRef
        = symRefTab->findOrCreateStoreFlattenableArrayElementSymbolRef();
    bool isLoadFlattenableArrayElement = false;
    bool isStoreFlattenableArrayElement = false;

    isLoadFlattenableArrayElement = node->getOpCode().isCall()
        && symRefTab->isNonHelper(node->getSymbolReference(),
            TR::SymbolReferenceTable::loadFlattenableArrayElementNonHelperSymbol);

    isStoreFlattenableArrayElement = node->getOpCode().isCall()
        && symRefTab->isNonHelper(node->getSymbolReference(),
    ...
    if (isLoadFlattenableArrayElement || isStoreFlattenableArrayElement) {
        ...
        logprintf(trace(), log,
            "%s: isLoadFlattenableArrayElement %d isStoreFlattenableArrayElement %d n%dn symRef #%d\n", __FUNCTION__,
            isLoadFlattenableArrayElement, isStoreFlattenableArrayElement, node->getGlobalIndex(),
         ...
         *               (Transformed in ILGen. VP is still capable of transforming the helper call to regular aaload if
         * encountering one)
         *
         *    - storeFlattenableArrayElementNonHelper
         *         - Transform the helper call to regular aastore (Transformed in VP:
         * canTransformUnflattenedArrayElementLoadStore)
         *
         * Array flattening is enabled
         *     - Identity Type
         *         - Transform the helper call to regular aaload/aastore (Transformed in VP:
         ...
         * canTransformUnflattenedArrayElementLoadStore)
         *         - Flattened
         *             - Transform the helper call to sym ref access (Transformed in VP:
         * canTransformFlattenedArrayElementLoadStore)
         *
         *     - Unknown Type
         *         - If the array constraint has a type hint class
         *             - And the type hint class is VT and it is flattened, transform the helper call with a guard to
         * sym ref access (Transformed in VP: canTransformFlattenedArrayElementLoadStoreUseTypeHint)
         *             - And the type hint class is VT and it is NOT flattened, transform the helper call with a guard
         * to regular aaload/aastore (Transformed in VP: canTransformUnflattenedArrayElementLoadStoreUseTypeHint)
         *             - And the type hint class is identity class, transform the helper call with a guard to regular
         * aaload/aastore (Transformed in VP: canTransformIdentityArrayElementLoadStoreUseTypeHint)
         *         - Other
        ...
        if ((isLoadFlattenableArrayElement || (isStoreFlattenableArrayElement && storeValueConstraint))
            && arrayConstraint && isArrayElementFlattened(arrayConstraint) == TR_yes) {
            TR_OpaqueClassBlock *arrayComponentClass
                = fe()->getComponentClassFromArrayClass(arrayConstraint->getClass());
            ...
            if (arrayComponentClass) {
                ...
                } else // isStoreFlattenableArrayElement
                {
                    ...
                    // storeValueConstraint might not be a resolved class constraint. Therefore valueClass might not
                    // exist
                    if (valueClass) {
                        TR_YesNoMaybe isInstanceOfComponentType = comp()->fej9()->isInstanceOf(valueClass,
                            arrayComponentClass, storeValueConstraint->isFixedClass(), arrayConstraint->isFixedClass());

                        // If the value that's being stored is not the same as the array component type, use helper
                        // call.
        ...
        bool canTransformUnflattenedArrayElementLoadStore
            = TR::Compiler->om.isValueTypeArrayFlatteningEnabled() ? false : true;
        if (!canTransformUnflattenedArrayElementLoadStore && arrayConstraint && (isNullRestrictedArray == TR_yes)
        ...
        static const char *enableFlattenedArrayElementTypeHintXForm
            = feGetEnv("TR_EnableFlattenedArrayElementTypeHintXForm");
        static const char *enableUnflattenedArrayElementTypeHintXForm
            = feGetEnv("TR_EnableUnflattenedArrayElementTypeHintXForm");
        TR_OpaqueClassBlock *typeHintClass = arrayConstraint ? arrayConstraint->getTypeHintClass() : NULL;

        if (typeHintClass && !canTransformFlattenedArrayElementLoadStore
            && !canTransformUnflattenedArrayElementLoadStore && !canTransformIdentityArrayElementLoadStore) {
            TR_OpaqueClassBlock *hintComponentClass = comp()->fej9()->getComponentClassFromArrayClass(typeHintClass);

            if (enableUnflattenedArrayElementTypeHintXForm && TR::Compiler->cls.classHasIdentity(hintComponentClass)) {
                canTransformIdentityArrayElementLoadStoreUseTypeHint = true;
            }

            if (!canTransformIdentityArrayElementLoadStoreUseTypeHint
                && TR::Compiler->cls.isArrayNullRestricted(comp(),
                    typeHintClass)) // TODO-VALUETYPE: typeHintClass eventually should be the null-restricted array
                                    // class
            {
                if (TR::Compiler->cls.isValueTypeClassFlattened(hintComponentClass)) {
                    if (enableFlattenedArrayElementTypeHintXForm) {
                        ...
                        {
                            TR_OpaqueClassBlock *valueClass
                                = storeValueConstraint ? storeValueConstraint->getClass() : NULL;
                            if (valueClass) {
                                TR_YesNoMaybe isInstanceOfHintComponentType
                                    = comp()->fej9()->isInstanceOf(valueClass, hintComponentClass,
                                        storeValueConstraint->isFixedClass(), arrayConstraint->isFixedClass());

                                // If the value that's being stored is not the same as the array hint component type,
                                // use helper call.
                                canTransformFlattenedArrayElementLoadStoreUseTypeHint
                                    = (isInstanceOfHintComponentType == TR_yes);
                            } else {
                                logprintf(trace(), log,
                                    "%s: n%dn %s valueClass is NULL storeValueConstraint 0x%p storeValueNode n%dn "
                                    "0x%p\n",
                                    __FUNCTION__, node->getGlobalIndex(), node->getOpCode().getName(),
                                    storeValueConstraint, storeValueNode->getGlobalIndex(), storeValueNode);
                            }
                        }
                    }
        ...
        logprintf(trace(), log,
            "%s: n%dn %s canTransformFlattenedArrayElementLoadStore %d canTransformUnflattenedArrayElementLoadStore %d "
            "canTransformIdentityArrayElementLoadStore %d canTransformFlattenedArrayElementLoadStoreUseTypeHint %d "
            "canTransformUnflattenedArrayElementLoadStoreUseTypeHint %d "
            "canTransformIdentityArrayElementLoadStoreUseTypeHint %d typeHintClass %p\n",
            __FUNCTION__, node->getGlobalIndex(), node->getOpCode().getName(),
            canTransformFlattenedArrayElementLoadStore, canTransformUnflattenedArrayElementLoadStore,
            canTransformIdentityArrayElementLoadStore, canTransformFlattenedArrayElementLoadStoreUseTypeHint,
            canTransformUnflattenedArrayElementLoadStoreUseTypeHint,
            canTransformIdentityArrayElementLoadStoreUseTypeHint, typeHintClass);

        if (canTransformFlattenedArrayElementLoadStore || canTransformUnflattenedArrayElementLoadStore
            || canTransformIdentityArrayElementLoadStore || canTransformFlattenedArrayElementLoadStoreUseTypeHint
            || canTransformUnflattenedArrayElementLoadStoreUseTypeHint
            ...
            if (canTransformFlattenedArrayElementLoadStore) {
                ...
                if (isStoreFlattenableArrayElement) {
                    ...
                    callToTransform = new (trStackMemory()) ArrayElementStoreHelperCallTransform(_curTree, node,
                        flagsForTransform, arrayLength, arrayConstraint->getClass());
                } else {
                    callToTransform = new (trStackMemory()) ArrayElementLoadHelperCallTransform(_curTree, node,
                        flagsForTransform, arrayLength, arrayConstraint->getClass());
                }
                ...
                if (isStoreFlattenableArrayElement) {
                    ...
                    callToTransform = new (trStackMemory()) ArrayElementStoreHelperCallTransform(_curTree, node,
                        flagsForTransform, arrayLength, typeHintClass);
                } else {
                    callToTransform = new (trStackMemory()) ArrayElementLoadHelperCallTransform(_curTree, node,
                        flagsForTransform, arrayLength, typeHintClass);
                }
                ...
                if (isStoreFlattenableArrayElement) {
                    ...
                    //
                    if (storeValueBaseNode == NULL
                        || getValueNumber(storeValueBaseNode) != getValueNumber(arrayRefNode)) {
                        ...
                        if (!owningMethodDoesNotContainStoreChecks(this, node)
                            && isArrayStoreCheckNeeded(arrayRefNode, storeValueNode, mustFail, storeClassForCheck,
                        ...
                        if ((isNullRestrictedArray != TR_no)
                            ...
                            const char *counterName = TR::DebugCounter::debugCounterName(comp(),
                                "vt-helper/vp-nullvaluechk/aastore/(%s)/%s/block_%d", comp()->signature(),
                    ...
                    callToTransform = new (trStackMemory()) ArrayElementStoreHelperCallTransform(_curTree, node,
                        flagsForTransform, arrayLength, NULL, storeClassForCheck, componentClassForCheck);
                } else {
                    callToTransform = new (trStackMemory())
                        ArrayElementLoadHelperCallTransform(_curTree, node, flagsForTransform, arrayLength);
                }
            } else // canTransformUnflattenedArrayElementLoadStoreUseTypeHint or
                   // canTransformIdentityArrayElementLoadStoreUseTypeHint
            {
                ...
                if (isStoreFlattenableArrayElement) {
                    ...
                    if (storeValueBaseNode == NULL
                        || getValueNumber(storeValueBaseNode) != getValueNumber(arrayRefNode)) {
                        // If storing to an array that is or might be null restricted and the value that's being
                        // assigned is or might be null, a call to <nonNullableArrayNullStoreCheck> is required to check
                        // whether a null reference is being stored to a null-restricted array
                        bool mustFail = false;
                        if (!owningMethodDoesNotContainStoreChecks(this, node)
                            && isArrayStoreCheckNeeded(arrayRefNode, storeValueNode, mustFail, storeClassForCheck,
                                componentClassForCheck)) {
                        ...
                        // TODO: Require the <nonNullableArrayNullStoreCheck> non-helper if
                        // !canSkipNonNullableArrayNullValueChecks(...)
                    ...
                    callToTransform = new (trStackMemory()) ArrayElementStoreHelperCallTransform(_curTree, node,
                        flagsForTransform, arrayLength, typeHintClass, storeClassForCheck, componentClassForCheck);
                } else {
                    callToTransform = new (trStackMemory()) ArrayElementLoadHelperCallTransform(_curTree, node,
                        flagsForTransform, arrayLength, typeHintClass);
                }
            ...
            const char *counterName = TR::DebugCounter::debugCounterName(comp(),
                "vt-helper/vp-failed/%s/(%s)/%s/block_%d/%s", operationName, comp()->signature(),
                comp()->getHotnessName(comp()->getMethodHotness()), _curTree->getEnclosingBlock()->getNumber(), reason);
            TR::DebugCounter::incStaticDebugCounter(comp(), counterName);
        }
        ...
        if (isLoadFlattenableArrayElement && arrayConstraint && arrayConstraint->getClass()
            ...
            if (arrayComponentClass && arrayConstraint->asResolvedClass()) {
                addBlockOrGlobalConstraint(node, TR::VPResolvedClass::create(this, arrayComponentClass),
                    arrayRefGlobal);
            }
        }
    ...
    switch (rm) {
        case TR::java_lang_Class_isInterface: {
            // Only constrain the call in the last run of vp to avoid adding the candidate twice if the call is inside a
            // loop
            if (!lastTimeThrough())
                return;
            ...
            if (receiverChildConstraint && receiverChildConstraint->isJavaLangClassObject() == TR_yes
                && receiverChildConstraint->isNonNullObject() && receiverChildConstraint->getClassType()
                ...
                transformCallToIconstInPlaceOrInDelayedTransformations(_curTree, isInterface, receiverChildGlobal,
                    transformNonnativeMethodInPlace, !transformNonnativeMethodInPlace);
        ...
        case TR::java_lang_String_hashCode: {
            ...
            if (isKnownStringObject(receiverChildConstraint)) {
                ...
                TR_ASSERT(stringLocation,
                    "Expecting non null pointer to String object for constString or known String object");
                int32_t hashCode;
                bool success = comp()->fej9()->getStringHashCode(comp(), stringLocation, hashCode);
                if (!success) {
                    logprints(trace(), log,
                        "Cannot get access to the String object, quit transforming String.hashCode\n");
                    break;
                }
                transformCallToIconstInPlaceOrInDelayedTransformations(_curTree, hashCode, receiverChildGlobal,
                    transformNonnativeMethodInPlace, !transformNonnativeMethodInPlace);
                TR::DebugCounter::incStaticDebugCounter(comp(),
                    TR::DebugCounter::debugCounterName(comp(), "constrainCall/(%s)", signature));
                return;
        ...
        case TR::java_lang_String_equals: {
            ...
            if (isStringObject(receiverChildConstraint) == TR_yes && receiverChildConstraint->isNonNullObject()
                ...
                if (objectChildConstraint->isNullObject()) {
                    transformCallToIconstInPlaceOrInDelayedTransformations(_curTree, 0,
                        receiverChildGlobal && objectChildGlobal, transformNonnativeMethodInPlace,
                        !transformNonnativeMethodInPlace);
                    TR::DebugCounter::incStaticDebugCounter(comp(),
                        TR::DebugCounter::debugCounterName(comp(), "constrainCall/(%s)", signature));
                    return;
                }
                ...
                if (isObjectString == TR_maybe) {
                    logprintf(trace(), log, "Argument type is unknown, quit transforming String.equals on node %p\n",
                        node);
                    break;
                ...
                } else // isObjectString == TR_yes
                {
                    ...
                    if (isKnownStringObject(receiverChildConstraint) && isKnownStringObject(objectChildConstraint)) {
                        ...
                        TR_ASSERT(receiverLocation && objectLocation,
                            "Expecting non null pointer to String object for constString or known String object");
                        ...
                        if (!success) {
                            logprintf(trace(), log,
                                "Does not have VM access, cannot tell whether %p and %p are equal\n", receiverChild,
                                objectChild);
                            break;
                        }
                        transformCallToIconstInPlaceOrInDelayedTransformations(_curTree, result,
                            receiverChildGlobal && objectChildGlobal, transformNonnativeMethodInPlace,
                            !transformNonnativeMethodInPlace);
                        TR::DebugCounter::incStaticDebugCounter(comp(),
                            TR::DebugCounter::debugCounterName(comp(), "constrainCall/(%s)", signature));
            ...
            if (isKnownStringObject(receiverChildConstraint)) {
                ...
                int32_t len;
                {
                    ...
                    if (!getStringlength.hasVMAccess())
                        break;
                    uintptr_t stringObject
                        = comp()->fej9()->getStaticReferenceFieldAtAddress((uintptr_t)stringLocation);
                ...
                transformCallToIconstInPlaceOrInDelayedTransformations(_curTree, len, receiverChildGlobal,
                    transformNonnativeMethodInPlace || rm == TR::java_lang_String_lengthInternal,
                    !(transformNonnativeMethodInPlace || rm == TR::java_lang_String_lengthInternal));
                TR::DebugCounter::incStaticDebugCounter(comp(),
                    TR::DebugCounter::debugCounterName(comp(), "constrainCall/(%s)", signature));
                return;
        ...
        case TR::java_lang_Class_getModifiersImpl: {
            ...
            if (classChildConstraint && classChildConstraint->isJavaLangClassObject() == TR_yes
                && classChildConstraint->isNonNullObject() && classChildConstraint->getClassType()
                && classChildConstraint->getClassType()->asFixedClass()) {
                int32_t modifiersForClass = 0;
                bool success = comp()->fej9()->javaLangClassGetModifiersImpl(classChildConstraint->getClass(),
                    modifiersForClass);
                if (!success)
                    break;
                transformCallToIconstInPlaceOrInDelayedTransformations(_curTree, modifiersForClass, classChildGlobal,
                    true, false);
                TR::DebugCounter::incStaticDebugCounter(comp(),
                    TR::DebugCounter::debugCounterName(comp(), "constrainCall/(%s)", signature));
                return;
        ...
        case TR::java_lang_Class_isAssignableFrom: {
            ...
            if (firstClassChildConstraint && firstClassChildConstraint->isJavaLangClassObject() == TR_yes
                && firstClassChildConstraint->isNonNullObject() && firstClassChildConstraint->getClassType()
                ...
                if (secondClassChildConstraint && secondClassChildConstraint->isJavaLangClassObject() == TR_yes
                    && secondClassChildConstraint
                           ->isNonNullObject() // NullPointerException is expected if the class is null
                    ...
                    if (secondClassChildConstraint->getClassType()->asFixedClass()) {
                        TR_YesNoMaybe isInstanceOfResult
                            = comp()->fej9()->isInstanceOf(secondClass, firstClass, true, true);
                        TR_ASSERT(isInstanceOfResult != TR_maybe,
                            "Expecting TR_yes or TR_no to call isInstanceOf when two types are fixed");
                        if (isInstanceOfResult == TR_yes)
                            assignable = 1;
                    } else if (secondClassChildConstraint->getClassType()->asResolvedClass()) {
                        TR_ASSERT_FATAL(firstClass != NULL && secondClass != NULL,
                            "isAssignableFrom replacement at compile-time requires two class pointers - got %p and %p",
                            firstClass, secondClass);
                        TR_YesNoMaybe isInstanceOfResult
                            = comp()->fej9()->isInstanceOf(secondClass, firstClass, false, true);
                        if (isInstanceOfResult == TR_maybe) {
                            logprintf(trace(), log,
                                "Relationship between %p and %p is unknown, quit transforming Class.isAssignableFrom\n",
                        ...
                        logprints(trace(), log,
                            "The second child class type is not resolved at compile-time, quit transforming "
                            "Class.isAssignableFrom\n");
                        return;
                    }
                    transformCallToIconstInPlaceOrInDelayedTransformations(_curTree, assignable,
                        firstClassChildGlobal && secondClassChildGlobal, true, false);
                    TR::DebugCounter::incStaticDebugCounter(comp(),
                        TR::DebugCounter::debugCounterName(comp(), "constrainCall/(%s)", signature));
                    return;
                }
        ...
        case TR::java_lang_J9VMInternals_fastIdentityHashCode: {
            // Get the last child because sometimes the first child can be a pointer to java/lang/Class field in J9Class
            // for a direct native call
            TR::Node *classChild = node->getLastChild();
            bool classChildGlobal;
            TR::VPConstraint *classChildConstraint = getConstraint(classChild, classChildGlobal);
            if (classChildConstraint && classChildConstraint->isJavaLangClassObject() == TR_yes
                ...
                int32_t hashCodeForClass = comp()->fej9()->getJavaLangClassHashCode(comp(),
                    classChildConstraint->getClass(), hashCodeWasComputed);
                if (hashCodeWasComputed) {
                    transformCallToIconstInPlaceOrInDelayedTransformations(_curTree, hashCodeForClass, classChildGlobal,
                        true, false);
                    TR::DebugCounter::incStaticDebugCounter(comp(),
                        TR::DebugCounter::debugCounterName(comp(), "constrainCall/(%s)", signature));
                    return;
        ...
        case TR::java_lang_Class_isValue: {
            ...
            if (classChildConstraint && classChildConstraint->isJavaLangClassObject() == TR_yes
                ...
                TR::DebugCounter::incStaticDebugCounter(comp(),
                    TR::DebugCounter::debugCounterName(comp(), "constrainCall/(%s)", signature));
            } else if (classChildConstraint && classChildConstraint->isJavaLangClassObject() == TR_yes
                ...
                //
                TR::Node *inlineFlagsTest
                    = TR::Node::create(node, TR::icmpne, 2, testFlagsNode, TR::Node::iconst(node, 0));
        ...
        case TR::java_lang_Class_getComponentType: {
            // Constrain the call in the last run of vp to avoid adding the transformation twice if the call is inside a
            // loop.
            if (!lastTimeThrough())
                return;
            ...
            bool isNonNullJavaLangClass = classChildConstraint
                && classChildConstraint->isJavaLangClassObject() == TR_yes && classChildConstraint->isNonNullObject();
            ...
            //
            if (isNonNullJavaLangClass && classChildConstraint->getClassType()
                && classChildConstraint->getClassType()->asFixedClass()) {
                TR_OpaqueClassBlock *thisClass = classChildConstraint->getClass();
                if (comp()->fej9()->isClassArray(thisClass)) {
                    TR_OpaqueClassBlock *arrayComponentClass
                        = comp()->fej9()->getComponentClassFromArrayClass(thisClass);
                    // J9Class pointer introduced by the opt has to be remembered under AOT
                    if (!arrayComponentClass) {
                        logprintf(trace(), log,
                            "Array component class cannot be remembered, quit transforming Class.getComponentType on "
                            "node %p\n",
                            node);
                        break;
                    }
                    ...
                    node = TR::Node::recreateWithoutProperties(node, TR::aloadi, 1, arrayComponentClassPointer,
                        comp()->getSymRefTab()->findOrCreateJavaLangClassFromClassSymbolRef());

                    TR::KnownObjectTable *knot = comp()->getOrCreateKnownObjectTable();
                    if (knot) {
                        TR::KnownObjectTable::Index knownObjectIndex
                            = knot->getOrCreateIndexAt((uintptr_t *)(arrayComponentClass
                                + comp()->fej9()->getOffsetOfJavaLangClassFromClassField()));
                        ...
                        addBlockOrGlobalConstraint(node,
                            TR::VPClass::create(this, TR::VPKnownObject::createForJavaLangClass(this, knownObjectIndex),
                    ...
                    logprintf(trace(), log, "Class is not an array class, transforming %s on node %p to null\n",
                        signature, node);
                ...
                TR::SymbolReference *symRef
                    = classChild->getOpCode().hasSymbolReference() ? classChild->getSymbolReference() : NULL;
                TR::SymbolReference *jlcFromClassSymRef
                    = comp()->getSymRefTab()->findOrCreateJavaLangClassFromClassSymbolRef();
                TR::Node *classOperand = NULL;
                ...
                if ((classChildConstraint == NULL) || (classChildConstraint->getClassType() == NULL)
                    ...
                    //
                    static_assert(sizeof(J9Class)
                            >= offsetof(J9ArrayClass, componentType) + sizeof(J9ArrayClass::componentType),
                        "The J9ArrayClass.componentType field must be within the size of J9Class");

                    TR::Node *testIsArrayClassNode = TR::Node::create(node, TR::icmpne, 2,
                        comp()->fej9()->testIsClassArrayType(classOperand), TR::Node::iconst(node, 0));
                    loadComponentTypeNode
                        = TR::Node::create(TR::aselect, 3, testIsArrayClassNode, loadComponentTypeNode, classOperand);
                    loadComponentTypeNode
                        = TR::Node::createWithSymRef(node, TR::aloadi, 1, loadComponentTypeNode, jlcFromClassSymRef);
                    jlcOfComponentTypeNode = TR::Node::create(TR::aselect, 3, testIsArrayClassNode,
                        loadComponentTypeNode, TR::Node::aconst(node, 0));
                } else {
        ...
        case TR::com_ibm_jit_JITHelpers_getSuperclass: {
            ...
            if (classChildConstraint && classChildConstraint->isJavaLangClassObject() == TR_yes
                ...
                if (!superClass || TR::Compiler->cls.isInterfaceClass(comp(), thisClass)
                    || TR::Compiler->cls.isPrimitiveClass(comp(), thisClass)) {
                    if (trace()) {
                        if (TR::Compiler->cls.isInterfaceClass(comp(), thisClass))
                            log->printf("Node %p is an interface class and does not have a super class\n", classChild);
                        else if (TR::Compiler->cls.isPrimitiveClass(comp(), thisClass))
                            log->printf("Node %p is a primitive class and does not have a super class\n", classChild);
                        else if (!superClass)
                            log->printf("Node %p is java/lang/Object class and does not have a super class\n",
                                classChild);
                        log->printf("Transform call to %s on node %p to null\n", signature, node);
                    }
                    ...
                    node = TR::Node::recreateWithoutProperties(node, TR::aloadi, 1, superClassPointer,
                        comp()->getSymRefTab()->findOrCreateJavaLangClassFromClassSymbolRef());

                    TR::KnownObjectTable *knot = comp()->getOrCreateKnownObjectTable();
                    if (knot) {
                        TR::KnownObjectTable::Index knownObjectIndex = knot->getOrCreateIndexAt(
                            (uintptr_t *)(superClass + comp()->fej9()->getOffsetOfJavaLangClassFromClassField()));
        ...
        case TR::java_lang_String_indexOf_native: {
            ...
            if (transformIndexOfKnownString(node, sourceStringNode, targetCharNode, startNode, NULL, true))
                return;
            break;
        }
        ...
        case TR::com_ibm_jit_JITHelpers_intrinsicIndexOfUTF16: {
            ...
            if (transformIndexOfKnownString(node, sourceStringNode, targetCharNode, startNode, lengthNode, is16Bit))
                return;
            break;
        }
    ...
    if (!comp()->compileRelocatableCode()) {
        switch (rm) {
            case TR::java_lang_invoke_MethodHandle_asType: {
                ...
                TR_OpaqueClassBlock *methodHandleClass
                    = J9JitMemory::convertClassPtrToClassOffset(J9VMJAVALANGINVOKEMETHODHANDLE(vmThread->javaVM));
                TR_OpaqueClassBlock *methodTypeClass
                    = J9JitMemory::convertClassPtrToClassOffset(J9VMJAVALANGINVOKEMETHODTYPE(vmThread->javaVM));
                if (mhConstraint && mtConstraint && mhConstraint->getKnownObject() && mhConstraint->isNonNullObject()
                    && mtConstraint->getKnownObject() && mtConstraint->isNonNullObject() && mhConstraint->getClass()
                    ...
                    TR::VMAccessCriticalSection asType(comp(), TR::VMAccessCriticalSection::tryToAcquireVMAccess);
                    ...
                    logprintf(trace(), log,
                        "mhLocation %p mtLocation %p mhObject %p mtOffset %d mtInMh %p mtObject %p\n", mhLocation,
                        mtLocation, mhObject, mtOffset, mtInMh, mtObject);

                    // AOT doesn't relocate/validate objects, which is why this transformation is disabled under AOT
                    if (mtInMh == mtObject) {
                        if (!performTransformation(comp(),
                                "%sTransforming %s on node %p to a PassThrough with child %p\n", OPT_DETAILS, signature,
                                node, mh))
                            break;
            ...
            case TR::java_lang_invoke_PrimitiveHandle_initializeClassIfRequired: {
                ...
                TR::VPConstraint *mhConstraint = getConstraint(mh, mhConstraintGlobal);
                J9VMThread *vmThread = comp()->fej9()->vmThread();
                TR_OpaqueClassBlock *primitiveHandleClass
                    = J9JitMemory::convertClassPtrToClassOffset(J9VMJAVALANGINVOKEPRIMITIVEHANDLE(vmThread->javaVM));
                bool removeCall = false;
                if (mhConstraint && mhConstraint->getKnownObject() && mhConstraint->isNonNullObject()
                    && mhConstraint->getClass()
                    && comp()->fej9()->isInstanceOf(mhConstraint->getClass(), primitiveHandleClass, true, true)
                        == TR_yes) {
                    ...
                    if (comp()->fej9()->classInitIsFinished((TR_OpaqueClassBlock *)defcClazz)) {
                        removeCall = true;
                        logprintf(trace(), log, "defc in MethodHandle %p has been initialized\n", mh);
                    }
                }
            ...
            case TR::java_lang_invoke_DirectHandle_nullCheckIfRequired: {
                ...
                if (!mhConstraint || !mhConstraint->getClass()
                    || comp()->fej9()->isInstanceOf(mhConstraint->getClass(), directHandleClass, true, true) != TR_yes)
                    break;

                bool removeCall = false;
                if (receiverConstraint && receiverConstraint->isNonNullObject()) {
                    removeCall = true;
                    logprintf(trace(), log, "receiver child %p for DirectHandle.nullCheckIfRequired is non-null\n",
                        receiver);
                } else if (mhConstraint->getKnownObject() && mhConstraint->isNonNullObject()) {
                    ...
                    if (isStatic) {
                        removeCall = true;
                        logprintf(trace(), log, "DirectHandle %p refers to a static method\n", mh);
                    } else {
                        ...
                        TR::Node *nullchk = TR::Node::createWithSymRef(node, TR::NULLCHK, 1, passthrough,
                            comp()->getSymRefTab()->findOrCreateNullCheckSymbolRef(owningMethodSymbol));
            ...
            // Transform java/lang/Object.newInstancePrototype into new and a call to default constructor of the given
            // class AOT class pointer relocation is only supported on aconst nodes. However aconst node cannot be a
            // child work of a TR::New node because it does not have a symref indicating the size of the instance of the
            // class.
            //
            ...
            case TR::sun_misc_Unsafe_allocateInstance: {
                const bool isNewInstProto = rm == TR::java_lang_Object_newInstancePrototype;
                ...
                if (!classChildConstraint || !classChildConstraint->isNonNullObject()
                    || classChildConstraint->isJavaLangClassObject() != TR_yes || !classChildConstraint->getClassType()
                    || !classChildConstraint->getClassType()->asFixedClass()
                    || !TR::Compiler->cls.isClassInitialized(comp(), classChildConstraint->getClass())) {
                    logprintf(trace(), log,
                        "Class child %p is not a non-null java/lang/Class object with fixed class constraint, quit "
                        "transforming Object.newInstancePrototype on node %p\n",
                        classChild, node);
                    break;
                }
                ...
                if (isNewInstProto) {
                    ...
                    TR::VPConstraint *callerClassChildConstraint
                        = getConstraint(callerClassChild, callerClassChildGlobal);
                    if (!callerClassChildConstraint || !callerClassChildConstraint->isNonNullObject()
                        ...
                        logprintf(trace(), log,
                            "Caller class %p is not a non-null java/lang/Class object with fixed class constraint, "
                            "quit transforming Object.newInstancePrototype on node %p\n",
                ...
                // InstantiationException will be thrown when calling java/lang/Class.newInstance on the following
                // classes
                if (!comp()->fej9()->isConcreteClass(newClass) || comp()->fej9()->isPrimitiveClass(newClass)
                    || comp()->fej9()->isClassArray(newClass)) {
                    logprintf(trace(), log,
                        "Class is not instantiatable via bytecode new, quit transforming Object.newInstancePrototype "
                        "on node %p\n",
                        node);
                    break;
                }
                ...
                if (isNewInstProto) {
                    if (!comp()->fej9()->isClassVisible(callerClass, newClass)) {
                        logprintf(trace(), log,
                            "Class is not visialbe to caller class, quit transforming Object.newInstancePrototype on "
                            "node %p\n",
                            node);
                        break;
                    }
                    ...
                    if (!constructor) {
                        logprintf(trace(), log,
                            "Cannot find the default constructor, quit transforming Object.newInstancePrototype on "
                            "node %p\n",
                            node);
                        break;
                    }
                ...
                // Transform the call
                if (performTransformation(comp(), "%sTransforming %s to new%s on node %p\n", OPT_DETAILS, signature,
                        isNewInstProto ? " and a call to constructor" : "", node)) {
                    if (_curTree->getNode()->getOpCode().isNullCheck()) {
                        ...
                        TR_ASSERT_FATAL(_curTree->getNode()->getFirstChild() == node,
                            "expected call n%un [%p] to be the child of the null check n%un [%p]",
                            node->getGlobalIndex(), node, _curTree->getNode()->getGlobalIndex(), _curTree->getNode());

                        // This had better be a plain null check (not ResolveAndNULLCHK)
                        TR_ASSERT_FATAL(_curTree->getNode()->getOpCodeValue() == TR::NULLCHK,
                            "in n%un [%p] expected opcode NULLCHK, but found %s (%d)",
                            _curTree->getNode()->getGlobalIndex(), _curTree->getNode(),
                            _curTree->getNode()->getOpCode().getName(), (int)_curTree->getNode()->getOpCodeValue());
                    ...
                    TR::Node *classPointerNode = TR::Node::createWithSymRef(node, TR::loadaddr, 0,
                        comp()->getSymRefTab()->findOrCreateClassSymbol(
                            node->getSymbolReference()->getOwningMethodSymbol(comp()), -1, newClass));

                    TR::Node::recreateWithoutProperties(node, TR::New, 1,
                        comp()->getSymRefTab()->findOrCreateNewObjectSymbolRef(
                            node->getSymbolReference()->getOwningMethodSymbol(comp())));
                    node->setAndIncChild(0, classPointerNode);

                    if (isNewInstProto) {
                        TR::SymbolReference *constructorSymRef = comp()->getSymRefTab()->findOrCreateMethodSymbol(
                            JITTED_METHOD_INDEX, -1, comp()->fej9()->createResolvedMethod(trMemory(), constructor),
                            TR::MethodSymbol::Special);

                        TR::TreeTop *constructorCall = TR::TreeTop::create(comp(), _curTree,
                            TR::Node::create(node, TR::treetop, 1,
                                TR::Node::createWithSymRef(node, TR::call, 1, node, constructorSymRef)));
                    }
            ...
            case TR::java_lang_invoke_MethodHandle_invokeBasic: {
                ...
                if (mhConstraint && mhConstraint->getKnownObject() && mhConstraint->isNonNullObject()) {
                    if (!J9::TransformUtil::refineMethodHandleInvokeBasic(comp(), _curTree, node,
                            mhConstraint->getKnownObject()->getIndex(), trace()))
                        return;
            ...
            case TR::java_lang_invoke_MethodHandle_linkToStatic: {
                ...
                if (mnConstraint && mnConstraint->getKnownObject() && mnConstraint->isNonNullObject()) {
                    if (!J9::TransformUtil::refineMethodHandleLinkTo(comp(), _curTree, node,
                            mnConstraint->getKnownObject()->getIndex(), trace()))
                        return;
                    processRefinedMethodHandleINLCall(node);
                }
                break;
...
void J9::ValuePropagation::transformIntoRegularArrayElementLoad(TR::TreeTop *callTree, TR::Node *callNode)
{
    TR::Node *indexNode = callNode->getChild(0);
    TR::Node *arrayRefNode = callNode->getChild(1);

    TR::Node *elementAddressNode
        = J9::TransformUtil::calculateElementAddress(comp(), arrayRefNode, indexNode, TR::Address);
    const TR::ILOpCodes loadOp = comp()->il.opCodeForIndirectArrayLoad(TR::Address);

    TR::SymbolReference *elementSymRef
        = comp()->getSymRefTab()->findOrCreateArrayShadowSymbolRef(TR::Address, arrayRefNode);
    TR::Node *elementLoadNode
        = TR::Node::recreateWithoutProperties(callNode, loadOp, 1, elementAddressNode, elementSymRef);
...
void J9::ValuePropagation::transformIntoRegularArrayElementStore(TR::TreeTop *callTree, TR::Node *callNode,
    bool needsNullValueCheck, bool needsStoreCheck, TR_OpaqueClassBlock *storeClassForArrayStoreCHK,
    TR_OpaqueClassBlock *componentClassForArrayStoreCHK)
{
    ...
    TR_ASSERT_FATAL_WITH_NODE(oldAnchorNode,
        (oldAnchorNode->getNumChildren() == 1) && oldAnchorNode->getFirstChild() == callNode,
        "Expected call node n%un for jitStoreFlattenableArrayElement was anchored under node n%un\n",
    ...
    if (needsStoreCheck) {
        ...
        TR::SymbolReference *storeCheckSymRef
            = comp()->getSymRefTab()->findOrCreateTypeCheckArrayStoreSymbolRef(methodSym);
        TR::Node *storeCheckNode
            = TR::Node::createWithRoomForThree(TR::ArrayStoreCHK, elementStoreNode, 0, storeCheckSymRef);
    ...
    if (needsNullValueCheck) {
        TR::SymbolReference *nonNullableArrayNullStoreCheckSymRef
            = comp()->getSymRefTab()->findOrCreateNonNullableArrayNullStoreCheckSymbolRef();
        TR::Node *nullCheckNode
            = TR::Node::createWithSymRef(TR::call, 2, 2, valueNode, arrayRefNode, nonNullableArrayNullStoreCheckSymRef);
...
void J9::ValuePropagation::transformFlattenedArrayElementLoad(TR_OpaqueClassBlock *arrayClass, TR::Node *callNode)
{
    OMR::Logger *log = comp()->log();
    ...
    // Create newvalue
    TR::Node *newValueNode = TR::Node::recreateWithoutProperties(callNode, TR::newvalue, fieldCount + 1, classNode,
        comp()->getSymRefTab()->findOrCreateNewValueSymbolRef(method));
    ...
    if (fieldCount == 0) {
        logprintf(trace(), log, "%s fieldCount 0: The call node is recreated to newValueNode n%dn\n", __FUNCTION__,
            newValueNode->getGlobalIndex());
        return;
    }
    ...
    TR::Node *elementAddressNode
        = J9::TransformUtil::calculateElementAddressWithElementStride(comp(), arrayRefNode, indexNode, elementStride);
    ...
    for (size_t idx = 0; idx < fieldCount; idx++) {
        ...
        auto *fieldSymRef = comp()->getSymRefTab()->findOrFabricateFlattenedArrayElementFieldShadowSymbol(
            arrayComponentClass, fieldEntry._datatype, fieldEntry._offset - offsetOfFirstField, fieldEntry._isPrivate,
            fieldEntry._fieldname, fieldEntry._typeSignature);
        logprintf(trace(), log, "%s %s fieldSymRef: %s fieldEntry[%d] fieldName %s fieldSig %s type %d offset %d\n",
            __FUNCTION__, comp()->getDebug()->getName(loadOpCode), comp()->getDebug()->getName(fieldSymRef), (int)idx,
...
bool J9::ValuePropagation::transformFlattenedArrayElementStore(TR_OpaqueClassBlock *arrayClass, TR::TreeTop *callTree,
    TR::Node *callNode, bool needsNullValueCheck)
{
    /*
     * primitive public class MultipleFieldsPrimitive {
     *    public final int x;
    ...
    if (needsNullValueCheck) {
        ...
        logprintf(trace(), log, "%s inserted NULLCHK n%dn on valueNode n%dn\n", __FUNCTION__,
            nullCheck->getGlobalIndex(), valueNode->getGlobalIndex());
    }
    ...
    if (fieldCount == 0) {
        ...
        logprintf(trace(), log, "%s fieldCount 0 callTree n%dn is removed\n", __FUNCTION__,
            callTree->getNode()->getGlobalIndex());
        return true;
    }
    ...
    for (size_t idx = 0; idx < fieldCount; idx++) {
        ...
        auto *storeFieldSymRef = comp()->getSymRefTab()->findOrFabricateFlattenedArrayElementFieldShadowSymbol(
            arrayComponentClass, fieldEntry._datatype, fieldEntry._offset - offsetOfFirstField, fieldEntry._isPrivate,
            fieldEntry._fieldname, fieldEntry._typeSignature);

        TR::DataType dataType = fieldEntry._datatype;
        bool useReadBarrier = comp()->getOption(TR_EnableFieldWatch);
        bool useWriteBarrier
            = ((dataType == TR::Address) && (TR::Compiler->om.writeBarrierType() != gc_modron_wrtbar_none))
            || comp()->getOption(TR_EnableFieldWatch);

        TR::ILOpCodes loadOpCode = useReadBarrier ? comp()->il.opCodeForIndirectReadBarrier(dataType)
                                                  : comp()->il.opCodeForIndirectLoad(dataType);
        TR::ILOpCodes storeOpCode = useWriteBarrier ? comp()->il.opCodeForIndirectWriteBarrier(dataType)
                                                    : comp()->il.opCodeForIndirectStore(dataType);

        if (trace()) {
            log->printf("%s fieldEntry[%d] fieldName %s fieldSig %s type %d offset %d\n", __FUNCTION__, (int)idx,
                fieldEntry._fieldname, fieldEntry._typeSignature, dataType.getDataType(), fieldEntry._offset);
            log->printf("    %s loadFieldSymRef %p %s . %s storeFieldSymRef %p %s\n",
                comp()->getDebug()->getName(loadOpCode), loadFieldSymRef, comp()->getDebug()->getName(loadFieldSymRef),
                comp()->getDebug()->getName(storeOpCode), storeFieldSymRef,
                comp()->getDebug()->getName(storeFieldSymRef));
        }

        // There's no need to apply a NULLCHK on the load because the value is known to be a value type, which is never
        // null.
        ...
        if (idx == 0) {
            if (useWriteBarrier)
                storeNode = TR::Node::recreateWithoutProperties(callNode, storeOpCode, 3, elementAddressNode, loadNode,
                    arrayRefNode, storeFieldSymRef);
            else
                storeNode = TR::Node::recreateWithoutProperties(callNode, storeOpCode, 2, elementAddressNode, loadNode,
                    storeFieldSymRef);
        } else {
            if (useWriteBarrier)
                storeNode = TR::Node::createWithSymRef(storeOpCode, 3, 3, elementAddressNode, loadNode, arrayRefNode,
                    storeFieldSymRef);
            else
                storeNode
                    = TR::Node::createWithSymRef(storeOpCode, 2, 2, elementAddressNode, loadNode, storeFieldSymRef);
...
void J9::ValuePropagation::transformFlattenedArrayElementLoadStoreUseTypeHint(TR_OpaqueClassBlock *typeHintClass,
    TR::Node *callNode, TR::TreeTop *callTree, bool isLoad, bool needsNullValueCheck)
{
    OMR::Logger *log = comp()->log();
    ...
    logprintf(trace(), log, "%s: callTree n%dn callNode n%dn isLoad %d needsNullValueCheck %d\n", __FUNCTION__,
        callTree->getNode()->getGlobalIndex(), callNode->getGlobalIndex(), isLoad, needsNullValueCheck);
    ...
    logprintf(trace(), log,
        "%s: ifTree n%dn (%p). slow path helper call treetop n%dn (%p). fast path newvalue treetop n%dn (%p)\n",
        __FUNCTION__, ifTree->getNode()->getGlobalIndex(), ifTree->getNode(), thenTree->getNode()->getGlobalIndex(),
        thenTree->getNode(), elseTree->getNode()->getGlobalIndex(), elseTree->getNode());

    J9::TransformUtil::createDiamondForCall(this, callTree, ifTree /* The tree containing the if node */,
        thenTree /* The tree to be taken when comparison succeeds */,
        elseTree /* The tree to be taken when comparison fails */, false /* changeBlockExtensions */,
        false /* markCold */);
    ...
    if (isLoad) {
        ...
        bool isCallTreeRemoved
            = transformFlattenedArrayElementStore(typeHintClass, elseTree, newCallNode, needsNullValueCheck);
...
void J9::ValuePropagation::transformUnflattenedArrayElementLoadStoreUseTypeHint(TR_OpaqueClassBlock *typeHintClass,
    TR::Node *callNode, TR::TreeTop *callTree, bool isLoad, bool needsNullValueCheck, bool needsStoreCheck,
    TR_OpaqueClassBlock *storeClassForArrayStoreCHK, TR_OpaqueClassBlock *componentClassForArrayStoreCHK)
{
    OMR::Logger *log = comp()->log();

    logprintf(trace(), log,
        "%s: callTree n%dn callNode n%dn isLoad %d needsNullValueCheck %d needsStoreCheck %d "
        "storeClassForArrayStoreCHK %p componentClassForArrayStoreCHK %p\n",
        __FUNCTION__, callTree->getNode()->getGlobalIndex(), callNode->getGlobalIndex(), isLoad, needsNullValueCheck,
        needsStoreCheck, storeClassForArrayStoreCHK, componentClassForArrayStoreCHK);
    ...
    logprintf(trace(), log,
        "%s: ifTree n%dn (%p). slow path helper call treetop n%dn (%p). fast path treetop n%dn (%p)\n", __FUNCTION__,
        ifTree->getNode()->getGlobalIndex(), ifTree->getNode(), thenTree->getNode()->getGlobalIndex(),
        thenTree->getNode(), elseTree->getNode()->getGlobalIndex(), elseTree->getNode());
    ...
    if (isLoad) {
        ...
        transformIntoRegularArrayElementStore(elseTree, newCallNode, needsNullValueCheck, needsStoreCheck,
            storeClassForArrayStoreCHK, componentClassForArrayStoreCHK);
...
TR_YesNoMaybe J9::ValuePropagation::isArrayElementFlattened(TR::VPConstraint *arrayConstraint)
{
    if (!TR::Compiler->om.areValueTypesEnabled()
        || !TR::Compiler->om.isValueTypeArrayFlatteningEnabled()) // isValueTypeArrayFlatteningEnabled() checks
...
TR_YesNoMaybe J9::ValuePropagation::isArrayNullRestricted(TR::VPConstraint *arrayConstraint)
{
    OMR::Logger *log = comp()->log();
    ...
    //
    if (!(arrayConstraint && arrayConstraint->getClass() && arrayConstraint->getClassType()->isArray() == TR_yes)) {
        logprintf(trace(), log, "%s: return TR_maybe. arrayConstraint %p\n", __FUNCTION__, arrayConstraint);
        return TR_maybe;
    }
    ...
    if (TR::Compiler->cls.isArrayNullRestricted(comp(), arrayClass)) {
        logprintf(trace(), log, "%s: return TR_yes. arrayClass %p\n", __FUNCTION__, arrayClass);
        return TR_yes;
    }
    ...
    if (!arrayComponentClass) {
        logprintf(trace(), log, "%s: return TR_maybe. arrayComponentClass NULL\n", __FUNCTION__);
        return TR_maybe;
    }
    ...
    if (!TR::Compiler->cls.isConcreteClass(comp(), arrayComponentClass)) {
        ...
        if (TR::Compiler->cls.classHasIdentity(arrayComponentClass)) {
            logprintf(trace(), log, "%s: return TR_no. abstract classHasIdentity\n", __FUNCTION__);
            return TR_no;
        } else {
            logprintf(trace(), log, "%s: return TR_maybe. Not concrete class\n", __FUNCTION__);
            return TR_maybe;
        }
    }
    ...
    //
    if (sig && sig[0] == '[' && len == 19 && !strncmp(sig, "[Ljava/lang/Object;", 19)) {
        ret = (arrayConstraint->isFixedClass()) ? TR_no : TR_maybe;
        logprintf(trace(), log, "%s: return %s. java.lang.Object\n", __FUNCTION__, comp()->getDebug()->getName(ret));
        return ret;
    }
    ...
    logprintf(trace(), log, "%s: return %s. Concrete class\n", __FUNCTION__, comp()->getDebug()->getName(ret));
    return ret;
}
    ...
    TR::Node *callNode, flags16_t flags)
{
    ...
    // Was the node already transformed?
    ...
    static const bool shortCircuitVTEqualityComparison = (feGetEnv("TR_ShortCircuitVTEqualityComparison") != NULL);

    // Decide whether references should be compared before actually performing the inline comparison of fields.
    // Currently, that's only done if either operand could be a null reference, but short-circuiting the comparison
    // might be worthwhile, so allow for performance experiments under control of the environment variable
    // TR_ShortCircuitVTEqualityComparison.
    //
    ...
    //
    const bool requiresReferenceComparisonTest
        = (fieldCount > 0 && shortCircuitVTEqualityComparison) || requiresLeftOpNullTest || requiresRightOpNullTest;

    const bool requiresControlFlow
        = requiresLeftOpNullTest || requiresRightOpNullTest || requiresReferenceComparisonTest;
    ...
    if (requiresControlFlow) {
        // The IL for the generation starts off looking like this:
        //
        // n10n  treetop
        // n9n     icall  <objectInequalityComparison>
        // n7n       aload  <lhs>
        // n8n       aload  <rhs>
        //   ...
        //
        ...
        // n84n  BBEnd </block_7>        |  |
        //                               |  |
        // n83n  BBStart <block_6>       |  |
        // n82n  istore <result>         |  |
        // n81n    inline-field-comparisons (see below)
        // n50n  goto --------------->+  |  |
        // n49n  BBEnd </block_6>     |  |  |
        //                            |  |  |
        ...
        // origBlock is the block that originally contained the call tree.
        // All generated IL will be inserted into this block and then split from it.
        //
        TR::Block *origBlock = NULL;

        // Find the block that contains the callTree
        //
        ...
        TR::SymbolReference *comparisonResultSymRef
            = comp()->getSymRefTab()->createTemporary(comp()->getMethodSymbol(), TR::Int32);

        TR::TreeTop *storeResultInsertionPointTT = callTree;

        // Copy the original call and anchor it with a store to the temporary that holds the result
        //
        callNodeToTransform = callNode->duplicateTree(false);
        TR::Node *storeCallResult = TR::Node::createWithSymRef(callNodeToTransform,
            comp()->il.opCodeForDirectStore(TR::Int32), 1, callNodeToTransform, comparisonResultSymRef);
        ...
        //
        TR::Node::recreateWithoutProperties(callNode, comp()->il.opCodeForDirectLoad(TR::Int32), 0,
            comparisonResultSymRef);

        // Address comparisons will be inserted before the IL that actually performs the
        // the field comparisons - testInsertionPointTT
        //
        TR::TreeTop *testInsertionPointTT = storeCallResultTT;

        TR::TreeTop *referenceCompareTestTT = NULL;
        TR::TreeTop *leftOpNullTestTT = NULL;
        TR::TreeTop *rightOpNullTestTT = NULL;
        TR::TreeTop *equalResultTT = NULL;
        TR::TreeTop *unequalResultTT = NULL;

        // Keep track of which is the first address comparison TreeTop;
        // we won't need to split a block at that point.
        //
        TR::TreeTop *firstTestTT = NULL;

        // Compare addresses - if they are equal, the instance comparison is equal
        //
        if (requiresReferenceComparisonTest) {
            ...
            firstTestTT = referenceCompareTestTT;

            // Generate store of equality result
            // Don't want any preceding result store to fall through to this result store, so insert a TR::Goto first
            //
            storeResultInsertionPointTT->insertBefore(
                TR::TreeTop::create(comp(), TR::Node::create(callNode, TR::Goto, 0)));

            // istore result <- equality
            TR::Node *equalResultNode = TR::Node::createStore(callNode, comparisonResultSymRef,
                TR::Node::iconst(callNode, isObjectEqualityCompare ? 1 : 0));

            equalResultTT = TR::TreeTop::create(comp(), equalResultNode);
            storeResultInsertionPointTT->insertBefore(equalResultTT);
        }

        // Compare either or both operands with null - if either equals null, the instance
        // comparison is false, because we know the reference addresses are not equal at this point.
        //
        if (requiresRightOpNullTest || requiresLeftOpNullTest) {
            ...
            if (requiresRightOpNullTest) {
                ...
                if (firstTestTT == NULL) {
                    firstTestTT = rightOpNullTestTT;
                }
            }

            // Generate store of inequality result
            // Don't want any preceding result store to fall through to this block, so insert a TR::Goto first
            //
            storeResultInsertionPointTT->insertBefore(
                TR::TreeTop::create(comp(), TR::Node::create(callNode, TR::Goto, 0)));

            TR::Node *unequalResultNode = TR::Node::createStore(callNode, comparisonResultSymRef,
                TR::Node::iconst(callNode, isObjectEqualityCompare ? 0 : 1));

            unequalResultTT = TR::TreeTop::create(comp(), unequalResultNode);
            storeResultInsertionPointTT->insertBefore(unequalResultTT);
        }

        // Now work backwards through newly added TR::TreeTops, splitting blocks at branch points.  Note that
        // the only blocks that might throw exceptions are the block that holds the load of the final result,
        // as it might have successor TreeTops that throw exceptions and the initial block leading up to
        // comparison, including the operands, which will be anchored there.  All other blocks perform
        // comparisons of local variables or constants, or stores to local variables, so they do not need
        // exception successors copied to them in the process of splitting.
        //
        ...
        TR::Block *equalResultBlock = NULL;

        // Keep track of the store blocks in order to remove edges between them.  The edge
        // removal needs to be delayed to avoid the possibility of prematurely removing all
        // edges into a block as that could result in the block's being removed from the CFG
        //
        TR::Block *storeBlocks[] = { NULL, NULL, NULL };
        int storeBlockCnt = 0;

        if (requiresRightOpNullTest || requiresLeftOpNullTest) {
            unequalResultBlock = origBlock->split(unequalResultTT, cfg, false /* fixupCommoning */,
                false /* copyExceptionSuccessors */);
            storeBlocks[storeBlockCnt++] = unequalResultBlock;
        }

        // Split out the block that stores the result if the comparison is equal
        //
        if (requiresReferenceComparisonTest) {
            equalResultBlock
                = origBlock->split(equalResultTT, cfg, false /* fixupCommoning */, false /* copyExceptionSuccessors */);
            ...
            // joinBlock, so update that branch and add an edge in the CFG.
            //
            if (storeBlockCnt > 0) {
                equalResultTT->getNextTreeTop()->getNode()->setBranchDestination(joinBlock->getEntry());
                cfg->addEdge(equalResultBlock, joinBlock);
            }

            storeBlocks[storeBlockCnt++] = equalResultBlock;
        }
        ...
        //
        if (storeBlockCnt > 0) {
            storeCallResultTT->getNextTreeTop()->getNode()->setBranchDestination(joinBlock->getEntry());
            cfg->addEdge(inlineFieldCompareBlock, joinBlock);
        }

        storeBlocks[storeBlockCnt++] = inlineFieldCompareBlock;

        if (requiresRightOpNullTest) {
            // If this is not the first address test, it needs to be in its own block.  Regardless,
            // the test conditionally branches to the unequalResultBlock, so add an edge for that.
            //
            if (firstTestTT != rightOpNullTestTT) {
            ...
            rightOpNullTestTT->getNode()->setBranchDestination(unequalResultBlock->getEntry());
        }
        ...
        if (requiresLeftOpNullTest) {
            ...
            // the test conditionally branches to the unequalResultBlock, so add an edge for that.
            //
            if (firstTestTT != leftOpNullTestTT) {
            ...
            leftOpNullTestTT->getNode()->setBranchDestination(unequalResultBlock->getEntry());
        }
        ...
        if (requiresReferenceComparisonTest) {
            ...
            TR_ASSERT_FATAL(firstTestTT == referenceCompareTestTT,
                ...
                "object{Inequality|Equality}Comparison\n");
            referenceCompareTestTT->getNode()->setBranchDestination(equalResultBlock->getEntry());
            cfg->addEdge(origBlock, equalResultBlock);
        }

        // Store blocks do not fallthrough to one another, so remove edges from between them
        //
        for (int i = storeBlockCnt - 1; i > 0; i--) {
            cfg->removeEdge(storeBlocks[i], storeBlocks[i - 1]);
        }
    } else {
    ...
    if (fieldCount == 0) {
        ...
        TR::ILOpCodes cmpOpCode = isObjectEqualityCompare ? comp()->il.opCodeForCompareEquals(dataType)
                                                          : comp()->il.opCodeForCompareNotEquals(dataType);
        TR::ILOpCodes loadOpCode = comp()->il.opCodeForIndirectLoad(dataType);

        auto *loadFieldSymRef = comp()->getSymRefTab()->findOrFabricateShadowSymbol(containingClass, dataType,
        ...
        if (trace()) {
            ...
            log->printf("    %s loadFieldSymRef %p %s \n", comp()->getDebug()->getName(loadOpCode), loadFieldSymRef,
                comp()->getDebug()->getName(loadFieldSymRef));
        }
        ...
        if (comp()->target().is64Bit()) {
            ...
            offsetNode
                = TR::Node::create(callNodeToTransform, TR::iconst, 0, TR::Compiler->om.objectHeaderSizeInBytes());
...
void J9::ValuePropagation::getArrayLengthLimits(TR::VPConstraint *constraint, int32_t &lowerBoundLimit,
...
void J9::ValuePropagation::doDelayedTransformations()
{
    ListIterator<TreeNodeResultPair> callsToBeFoldedToNode(&_callsToBeFoldedToNode);
    for (TreeNodeResultPair *it = callsToBeFoldedToNode.getFirst(); it; it = callsToBeFoldedToNode.getNext()) {
        TR::TreeTop *callTree = it->_tree;
        TR::Node *result = it->_result;
        TR::Node *callNode = callTree->getNode()->getFirstChild();
        logprintf(trace(), comp()->log(), "Doing delayed call transformation on call node n%dn\n",
    ...
    if (TR::Compiler->om.isOffHeapAllocationEnabled()) {
        ...
        for (copyMemoryPair = copyMemoryIt.getFirst(); copyMemoryPair; copyMemoryPair = copyMemoryIt.getNext()) {
            if (performTransformation(comp(), "O^O Call arraycopy instead of Unsafe.copyMemory: %p\n",
                    copyMemoryPair->_node))
                TR::TransformUtil::transformUnsafeCopyMemorytoArrayCopyForOffHeap(self()->comp(),
                    copyMemoryPair->_treetop, copyMemoryPair->_node);
        }
    ...
    for (ValueTypesHelperCallTransform *callToTransform = valueTypesHelperCallsToBeFolded.getFirst();
         callToTransform != NULL; callToTransform = valueTypesHelperCallsToBeFolded.getNext()) {
        ...
        const bool needsNullValueCheck
            = callToTransform->_flags.testAny(ValueTypesHelperCallTransform::RequiresNullValueCheck);
        const bool needsBoundCheck = callToTransform->_flags.testAny(ValueTypesHelperCallTransform::RequiresBoundCheck);
        const bool isFlattenedElement
            = callToTransform->_flags.testAny(ValueTypesHelperCallTransform::IsFlattenedElement);
        const bool isFlattenedElementUseTypeHint
            = callToTransform->_flags.testAny(ValueTypesHelperCallTransform::IsFlattenedElementUseTypeHint);
        const bool isUnflattenedElementUseTypeHint
            = callToTransform->_flags.testAny(ValueTypesHelperCallTransform::IsUnflattenedElementUseTypeHint);
        ...
        if (!hasCheckedNonVTComparePerformTransformation
            && !performTransformation(comp(), "%sReplacing n%dn %s\n", OPT_DETAILS, callNode->getGlobalIndex(),
                isInlineVTCompare ? "icall of <object{Inequality|Equality}Comparison>"
                                  : (isLoad ? "acall of <loadFlattenableArrayElementNonHelper>"
                                            : "call of <storeFlattenableArrayElementNonHelper>"))) {
            continue;
        }
        ...
        TR_ASSERT_FATAL_WITH_NODE(callNode, !comp()->requiresSpineChecks(),
            "Cannot handle VP yet for jit{Load|Store}FlattenableArrayElement if SpineCHKs are required\n");

        ArrayOperationHelperCallTransform *arrayOpCallToTransform
            = callToTransform->castToArrayOperationHelperCallTransform();
        ...
        if (isLoad) {
            if (isFlattenedElement) {
                ...
                transformFlattenedArrayElementLoadStoreUseTypeHint(arrayOpCallToTransform->_arrayClass, callNode,
                    callTree, true /*isLoad*/, false);
                isCallTreeRemoved = true;
            } else if (isUnflattenedElementUseTypeHint) {
                transformUnflattenedArrayElementLoadStoreUseTypeHint(arrayOpCallToTransform->_arrayClass, callNode,
                    callTree, true /*isLoad*/, false, false, NULL, NULL);
                isCallTreeRemoved = true;
            } else {
                transformIntoRegularArrayElementLoad(callTree, callNode);
            }
        } else if (isStore) {
            ArrayElementStoreHelperCallTransform *arrayStoreCallToTransform
                = callToTransform->castToArrayElementStoreHelperCallTransform();

            if (isFlattenedElement) {
                isCallTreeRemoved = transformFlattenedArrayElementStore(arrayOpCallToTransform->_arrayClass, callTree,
                    callNode, needsNullValueCheck);
            } else if (isFlattenedElementUseTypeHint) {
                transformFlattenedArrayElementLoadStoreUseTypeHint(arrayOpCallToTransform->_arrayClass, callNode,
                    callTree, false /*isLoad*/, needsNullValueCheck);
                isCallTreeRemoved = true;
            } else if (isUnflattenedElementUseTypeHint) {
                transformUnflattenedArrayElementLoadStoreUseTypeHint(arrayOpCallToTransform->_arrayClass, callNode,
                    callTree, false /*isLoad*/, needsNullValueCheck, needsStoreCheck,
                    arrayStoreCallToTransform->_storeClassForArrayStoreCHK,
                    arrayStoreCallToTransform->_componentClassForArrayStoreCHK);
                isCallTreeRemoved = true;
            } else {
                transformIntoRegularArrayElementStore(callTree, callNode, needsNullValueCheck, needsStoreCheck,
                    arrayStoreCallToTransform->_storeClassForArrayStoreCHK,
                    arrayStoreCallToTransform->_componentClassForArrayStoreCHK);
            }
        }
    ...
    for (CallInfo *ci = _refinedMethodHandleINLMethodsToInline.getFirst(); ci; ci = ci->getNext()) {
        ...
        if (!newInlineCall.inlineCall(ci->_tt, ci->_thisType, true, ci->_argInfo, /* initialMaxSize */ 0)) {
            if (trace())
                comp()->log()->prints("Failed to inline refined MH INL call\n");
        }
    }
...
void J9::ValuePropagation::getParmValues()
{
        ...
        if (p && p->getOffset() == 0) {
            ...
            if (usePreexistence()) {
                ...
                if (TR::Compiler->cls.isAbstractClass(comp(), classObject))
                    classObject = comp()->getPersistentInfo()->getPersistentCHTable()->findSingleConcreteSubClass(
                        classObject, comp());
                ...
                if (!classObject) {
                    ...
                    TR_PersistentClassInfo *cl
                        = comp()->getPersistentInfo()->getPersistentCHTable()->findClassInfoAfterLocking(classObject,
                            comp());
                    if (!cl)
                        prexClass = NULL;
                    else {
            ...
            if (0 && constraint) // cannot do this if 'this' is changed in the method...allow the non null property on
                                 // the TR::Node set by IL gen to dictate the creation of non null constraint
            {
    ...
    for (; p; p = parms.getNext()) {
        ...
        if ((dataType == TR::Int8 || dataType == TR::Int16)
            ...
            // If we are inlining this method and the caller has more specific type
            // info for this parm slot (e.g. a subtype of the declared signature
            ...
            if (opaqueClass) {
                ...
                if (usePreexistence()) {
                    ...
                    if (!TR::Compiler->cls.isConcreteClass(comp(), opaqueClass))
                        opaqueClass = comp()->getPersistentInfo()->getPersistentCHTable()->findSingleConcreteSubClass(
                            opaqueClass, comp());
...
static bool isStaticFinalFieldWorthFolding(TR::Compilation *comp, TR_OpaqueClassBlock *declaringClass,
    char *fieldSignature, int32_t fieldSigLength)
{
...
bool J9::ValuePropagation::isUnreliableSignatureType(TR_OpaqueClassBlock *klass, TR_OpaqueClassBlock *&erased)
{
    erased = klass;
    if (klass == NULL)
        return false;
    ...
    TR_ASSERT_FATAL(!comp()->compileRelocatableCode() || comp()->getOption(TR_UseSymbolValidationManager),
        "unexpected unreliable signature check in non-SVM AOT, klass=%p", klass);
    ...
    if (!TR::Compiler->cls.isInterfaceClass(comp(), klass)) {
        ...
        if (TR::Compiler->om.areFlattenableValueTypesEnabled() && (numDims > 0)
    ...
    erased = objectClass;
    while (numDims > 0) {
        TR_OpaqueClassBlock *arrayClass = fe()->getArrayClassFromComponentClass(erased);
        if (arrayClass == NULL) {
...
bool J9::ValuePropagation::canArrayClassBeTrustedAsFixedClass(TR_OpaqueClassBlock *arrayClass,
    TR_OpaqueClassBlock *componentClass)
{
    if (TR::Compiler->om.areFlattenableValueTypesEnabled()
        && !TR::Compiler->cls.isArrayNullRestricted(comp(), arrayClass)
        && // If the array is null-restricted array, we know it is a fixed class
...
bool J9::ValuePropagation::canClassBeTrustedAsFixedClass(TR::SymbolReference *symRef, TR_OpaqueClassBlock *classObject)
{
    ...
    if (classObject) {
        ...
        if ((numDims > 0) && !TR::Compiler->cls.isArrayNullRestricted(comp(), classObject)
            && // If the array is null-restricted array, we know it is a fixed class
            TR::Compiler->cls.isValueTypeClass(klass))
            return false;
    }
...
static TR::Node *setCloneClassInNode(OMR::ValuePropagation *vp, TR::Node *node, TR::VPConstraint *constraint,
    bool isGlobal)
{
    ...
    if (constraint && constraint->getClass()) {
        ...
        if (node->getCloneClassInNode() && clazz && (node->getCloneClassInNode() != clazz)) {
...
TR::Node *J9::ValuePropagation::innerConstrainAcall(TR::Node *node)
{
    OMR::Logger *log = comp()->log();
    ...
    if (method) {
        if (!node->getOpCode().isIndirect()) {
            ...
            bool isVectorSupportLoad = method->getRecognizedMethod() == TR::jdk_internal_vm_vector_VectorSupport_load;
            ...
            bool isVectorSupportBlend = method->getRecognizedMethod() == TR::jdk_internal_vm_vector_VectorSupport_blend;
            bool isVectorSupportBroadcastInt
                = method->getRecognizedMethod() == TR::jdk_internal_vm_vector_VectorSupport_broadcastInt;

            if (isVectorSupportLoad || isVectorSupportBinaryOp || isVectorSupportFromBitsCoerced
                || isVectorSupportUnaryOp || isVectorSupportTernaryOp || isVectorSupportCompare
                || isVectorSupportCompressExpandOp || isVectorSupportConvert || isVectorSupportBlend
                || isVectorSupportBroadcastInt) {
                bool isGlobal; // dummy
                int typeChildIndex;

                if (isVectorSupportLoad || isVectorSupportFromBitsCoerced || isVectorSupportBlend)
                    typeChildIndex = 0;
                ...
                if (isVectorSupportCompressExpandOp && node->getFirstChild()->getOpCode().isLoadConst()
            ...
            if (method->getRecognizedMethod() == TR::java_lang_Object_clone
                ...
                if (constraint) {
                    ...
                    if (constraint->getClass() && !comp()->fej9()->isCloneable(constraint->getClass())) {
                        logprintf(trace(), log, "Object Clone: Class of node %p is not cloneable, quit\n", node);

                        TR::DebugCounter::incStaticDebugCounter(comp(),
                            TR::DebugCounter::debugCounterName(comp(), "inlineClone/unsuitable/(%s)/%s/block_%d",
                    ...
                    if (constraint->isFixedClass()) {
                        ...
                        if (!comp()->compileRelocatableCode() && comp()->getOSRMode() != TR::involuntaryOSR) {
                            if (constraint->getClassType() && constraint->getClassType()->isArray() == TR_no
                                && !_objectCloneCalls.find(_curTree)) {
                                _objectCloneCalls.add(_curTree);
                                _objectCloneTypes.add(new (trStackMemory())
                                        OMR::ValuePropagation::ObjCloneInfo(constraint->getClass(), true));
                            } else if (constraint->getClassType() && constraint->getClassType()->isArray() == TR_yes
                                && !_arrayCloneCalls.find(_curTree)) {
                                _arrayCloneCalls.add(_curTree);
                                _arrayCloneTypes.add(new (trStackMemory())
                                        OMR::ValuePropagation::ArrayCloneInfo(constraint->getClass(), true));
                            }
                        }
                    }
                    ...
                    // Even though all existing supported build config have this flag set, this ifdef serves as a safety
                    // precaution.
#if defined(J9VM_INTERP_FLAGS_IN_CLASS_SLOT)
                    else if (constraint->getClassType() && constraint->getClassType()->asResolvedClass()) {
                        newTypeConstraint = TR::VPResolvedClass::create(this, constraint->getClass());
                        if (!comp()->compileRelocatableCode() && comp()->getOSRMode() != TR::involuntaryOSR) {
                            logprintf(trace(), log, "Object Clone: Resolved Class of node %p \n", node);
                            if (enableDynamicObjectClone && constraint->getClassType()->isArray() == TR_no
                                && !_objectCloneCalls.find(_curTree)) {
                                logprintf(trace(), log, "Object Clone: Resolved Class of node %p object clone\n", node);
                                _objectCloneCalls.add(_curTree);
                                _objectCloneTypes.add(new (trStackMemory())
                                        OMR::ValuePropagation::ObjCloneInfo(constraint->getClass(), false));
                            }
                            ...
                            // Remove the condition as other platforms receive support.
                                ...
                                && constraint->getClassType()->isArray() == TR_yes && !_arrayCloneCalls.find(_curTree)
                                && !comp()->generateArraylets()) {
                                logprintf(trace(), log, "Object Clone: Resolved Class of node %p array clone\n", node);
                                _arrayCloneCalls.add(_curTree);
                                _arrayCloneTypes.add(new (trStackMemory())
                                        OMR::ValuePropagation::ArrayCloneInfo(constraint->getClass(), false));
                            }
                        }
                    }
#endif
                ...
                if (!constraint
                    || (!constraint->isFixedClass()
                        && (enableDynamicObjectClone
                            && !(constraint->getClassType() && constraint->getClassType()->asResolvedClass()
                                && constraint->getClassType()->isArray() == TR_no))))
                    TR::DebugCounter::incStaticDebugCounter(comp(),
                        TR::DebugCounter::debugCounterName(comp(), "inlineClone/miss/(%s)/%s/block_%d",
                            comp()->signature(), comp()->getHotnessName(comp()->getMethodHotness()),
                            _curTree->getEnclosingBlock()->getNumber()));
                else
                    TR::DebugCounter::incStaticDebugCounter(comp(),
                        TR::DebugCounter::debugCounterName(comp(), "inlineClone/hit/(%s)/%s/block_%d",
                            comp()->signature(), comp()->getHotnessName(comp()->getMethodHotness()),
                            _curTree->getEnclosingBlock()->getNumber()));

                TR::VPClassPresence *cloneResultNonNull = TR::VPNonNullObject::create(this);
                TR::VPObjectLocation *cloneResultOnHeap
                    = TR::VPObjectLocation::create(this, TR::VPObjectLocation::HeapObject);
                TR::VPArrayInfo *cloneResultArrayInfo = NULL;
                if (constraint)
                    cloneResultArrayInfo = constraint->getArrayInfo();
                TR::VPConstraint *newConstraint = TR::VPClass::create(this, newTypeConstraint, cloneResultNonNull, NULL,
                    cloneResultArrayInfo, cloneResultOnHeap);
                ...
                TR_OpaqueClassBlock *classObject
                    = fe()->getClassFromSignature("java/math/BigDecimal", 20, owningMethod);
                if (classObject) {
                    constraint = TR::VPFixedClass::create(this, classObject);
        ...
        } else // if (!node->getOpCode().isIndirect())
        {
            if ((method->getRecognizedMethod() == TR::java_math_BigDecimal_add)
                ...
                TR_ResolvedMethod *owningMethod = symRef->getOwningMethod(comp());
                TR_OpaqueClassBlock *bigDecimalClass
                    = fe()->getClassFromSignature("java/math/BigDecimal", 20, owningMethod);

                if (constraint && bigDecimalClass && constraint->isFixedClass()
...
TR_OpaqueClassBlock *J9::ValuePropagation::findLikelySubtype(TR_OpaqueClassBlock *klass)
{
    ...
    if (TR::Compiler->cls.isInterfaceClass(comp(), klass) || TR::Compiler->cls.isAbstractClass(comp(), klass)) {
    ...
    else if (fe()->classHasBeenExtended(klass) || TR::Compiler->vm.isVMInStartupPhase(comp())) {
        return NULL;
    }
...
TR_OpaqueClassBlock *J9::ValuePropagation::findLikelySubtype(const char *sig, int32_t len,
...
TR::VPConstraint *J9::ValuePropagation::createTypeHintConstraint(TR_ResolvedMethod *owningMethod, const char *sig,
    int32_t len)
{
    if (!sig)
        return NULL;
    ...
    if (likelySubtype) {
        ...
        if (trace()) {
            ...
            const char *signature
                = TR::Compiler->cls.classSignature_DEPRECATED(comp(), likelySubtype, length, comp()->trMemory());
            log->printf("%s: %.*s constraint %s: ", __FUNCTION__, length, signature,
                constraint ? constraint->name() : "NULL");
            if (constraint)
                constraint->print(log, comp());
            log->println();
        }
    }
