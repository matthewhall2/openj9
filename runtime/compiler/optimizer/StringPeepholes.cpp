...
TR_StringPeepholes::TR_StringPeepholes(TR::OptimizationManager *manager)
    ...
    , _transformedCalls(manager->trMemory())
{}
...
{
    TR_ASSERT(m != START_STRING_METHODS && m != END_STRINGPEEPHOLES_METHODS, "wrong constant!!");

    static const char *classNames[] = { "java/math/BigDecimal", "java/math/BigDecimal", "java/math/BigDecimal",
        "java/math/BigDecimal", NULL, "java/lang/String", "java/lang/String", "java/lang/String", "java/lang/String",
        "java/lang/String", "java/lang/String" };

    static const char *methodNames[]
        = { "SMAAMSS", "SMSS", "AAMSS", "MSS", NULL, "<init>", "<init>", "<init>", "<init>", "<init>", "<init>" };

    static const char *signatures[] = { "(Ljava/math/BigDecimal;Ljava/math/BigDecimal;Ljava/math/BigDecimal;Ljava/math/"
                                        "BigDecimal;Ljava/math/BigDecimal;IIII)Ljava/math/BigDecimal;",
        "(Ljava/math/BigDecimal;Ljava/math/BigDecimal;Ljava/math/BigDecimal;II)Ljava/math/BigDecimal;",
        "(Ljava/math/BigDecimal;Ljava/math/BigDecimal;Ljava/math/BigDecimal;Ljava/math/BigDecimal;III)Ljava/math/"
        "BigDecimal;",
        "(Ljava/math/BigDecimal;Ljava/math/BigDecimal;I)Ljava/math/BigDecimal;", NULL, "(Ljava/lang/String;C)V",
        "(Ljava/lang/String;Ljava/lang/String;)V", "(Ljava/lang/String;Ljava/lang/String;Ljava/lang/String;)V",
        "(Ljava/lang/String;I)V", "([BIIZ)V", "(ILjava/lang/String;ILjava/lang/String;Ljava/lang/String;)V" };

    // TODO: This is a workaround as we switched to using a byte[] backing array in String*. Remove this workaround once
    // obsolete.
    if (m == SPH_String_init_AIIZ) {
        if (!fe()->getMethodFromName(classNames[m], methodNames[m], signatures[m])) {
            return comp()->getSymRefTab()->methodSymRefFromName(comp()->getMethodSymbol(), classNames[m],
                methodNames[m], "([CIIZ)V", TR::MethodSymbol::Special);
        }
    }

    if (strlen(methodNames[m]) == 6 && !strncmp(methodNames[m], "<init>", 6))
        return comp()->getSymRefTab()->methodSymRefFromName(comp()->getMethodSymbol(), classNames[m], methodNames[m],
            signatures[m], TR::MethodSymbol::Special);

    return comp()->getSymRefTab()->methodSymRefFromName(comp()->getMethodSymbol(), classNames[m], methodNames[m],
        signatures[m], TR::MethodSymbol::Static);
}
...
int32_t TR_StringPeepholes::perform()
{
    ...
    if (comp()->getOption(TR_DisableStringPeepholes)
        ...
        || (skipitAtWarm && comp()->getMethodHotness() == warm) || comp()->isFearPointPlacementUnrestricted())
        return 1;
...
void TR_StringPeepholes::prePerformOnBlocks()
{
    ...
    optSymRefs = (TR::SymbolReference **)comp()->trMemory()->allocateStackMemory(
        sizeof(TR::SymbolReference *) * END_STRINGPEEPHOLES_METHODS);
...
const char *TR_StringPeepholes::optDetailString() const throw() { return "O^O STRING PEEPHOLES: "; }
    ...
    for (TR::TreeTop *tt = block->getEntry(); tt != exit; tt = tt->getNextRealTreeTop()) {
        ...
        if (!_stringClassesRedefined && (node->getOpCodeValue() == TR::New)) {
            ...
            if (len == 22 && !strncmp(className, "java/lang/StringBuffer", 22)) {
                logprintf(trace(), log, "--stringbuffer-- in %s\n", comp()->signature());
                ...
                logprintf(trace(), log, "--stringbuilder-- in %s\n", comp()->signature());
            ...
            if (comp()->cg()->getSupportsBigDecimalLongLookasideVersioning()
                && callNode->getOpCode().hasSymbolReference() && !callNode->getSymbolReference()->isUnresolved()
                ...
                if (((owningMethod->getRecognizedMethod() == TR::java_math_BigDecimal_longString1)
                    ...
                    // Change the constructor call only if String compression is disabled. The reason we are changing
                    // the constructor call to the package protected String constructor is so that we can share the
                    // char[] argument passed in without making a copy internally in the String constructor (we would
                    // have to do this if we call the public API to ensure the String remains immutable). However the
                    // char[] constructed in BigDecimal is always decompressed, so if String compression is enabled
                    // passing such a char[] would force the package protected constructor to create a decompressed
                    // String because it does not check explicitly whether all the characters in the char[] are
                    // compressible or not. The public constructor would make such a check hence why we do not want to
                    // carry out this transformation if String compression is enabled.

                    // TODO: Calls to this constructor in the above BigDecimal methods do not exist. We need to refactor
                    // these classes to call the new String constructors using byte arrays instead.
                    if (strncmp(m->signatureChars(), "([BII)", 6) == 0
                        && !IS_STRING_COMPRESSION_ENABLED_VM(
                        ...
                        if (performTransformation(comp(),
                                "%s Changing the string constructor call node %p to invoke a private constructor\n",
                            ...
                            // Recreate the call node since we need to add an extra child to the constructor which
                            // determines whether the byte[] we are passing in is compressed
                            TR::Node::recreateWithoutProperties(callNode, TR::call, 5,
                                findSymRefForOptMethod(SPH_String_init_AIIZ));
            ...
            if (callNode->getOpCode().hasSymbolReference() && !callNode->getSymbolReference()->isUnresolved()
                && (callNode->getOpCodeValue() == TR::call)
                && !callNode->getSymbolReference()->getSymbol()->castToMethodSymbol()->isHelper()
                && (comp()->getMethodHotness() > warm) && comp()->isOutermostMethod() && !optimizer()->isIlGenOpt()
                && checkMethodSignature(callNode->getSymbolReference(), "java/lang/String.<init>(")) {
                TR::ResolvedMethodSymbol *method
                    = callNode->getSymbolReference()->getSymbol()->castToResolvedMethodSymbol();
                TR_ResolvedMethod *m = method->getResolvedMethod();
                if ((strncmp(m->signatureChars(), "([CII)", 6) == 0)) {
                    if (comp()->getRecompilationInfo()
                        && performTransformation(comp(),
                            "%smight have simplified string pattern at node [%p] if profiling info was available. "
                            "Switching to profiling.\n",
                            optDetailString(), callNode)) {
                        if (comp()->getMethodHotness() == hot) {
                            logprintf(trace(), log, "switching method %s to profiling\n", comp()->signature());
            ...
            if (comp()->cg()->getSupportsBigDecimalLongLookasideVersioning() && comp()->isOutermostMethod()
            ...
            if (comp()->isOptServer() && comp()->cg()->getSupportsBigDecimalLongLookasideVersioning()
                && comp()->isOutermostMethod() && !optimizer()->isIlGenOpt()) {
                TR::TreeTop *prevTree = tt->getPrevTreeTop();
...
struct TR_BDChain {
    ...
    TR_BDChain(TR::RecognizedMethod recognizedMethod, TR::Node *node, TR::TreeTop *tt, int32_t childNum,
        TR_BDChain *prev = NULL, TR_BDChain *next = NULL, int32_t scale1 = -1, int32_t scale2 = -1,
        int32_t resultScale = -1)
        : _recognizedMethod(recognizedMethod)
        , _node(node)
        , _tt(tt)
        , _childNum(childNum)
        , _prev(prev)
...
TR_BDChain *TR_StringPeepholes::detectChain(TR::RecognizedMethod recognizedMethod, TR::TreeTop *cursorTree,
    TR::Node *cursorNode, TR_BDChain *prevInChain)
{
    ...
    if (prevNodeInChain) {
        ...
        if (recognizedMethod == TR::unknownMethod) {
            ...
            if (!cursorNode->getSymbolReference()->isUnresolved()
                ...
                if (cursorNode->getSymbolReference()->getSymbol()->getResolvedMethodSymbol()->getRecognizedMethod()
                    == recognizedMethod) {
                    if (recognizedMethod == TR::java_math_BigDecimal_valueOf)
                        chainNode = cursorNode;
                    else {
                        if ((cursorNode->getNumChildren() > 0) && (cursorNode->getFirstChild() == prevNodeInChain)) {
                            chainNode = cursorNode;
                            childNum = 0;
                            ...
                            chainNode = cursorNode;
                            childNum = 1;
                        } else if ((cursorNode->getNumChildren() > 2) && (cursorNode->getChild(2) == prevNodeInChain)) {
                            chainNode = cursorNode;
                            childNum = 2;
                        }
                    }
    ...
    if (chainNode) {
        TR_BDChain *nextInChain
            = new (trStackMemory()) TR_BDChain(recognizedMethod, chainNode, cursorTree, childNum, prevInChain);
        return nextInChain;
    }
...
static TR::RecognizedMethod bdPattern1[] = { TR::java_math_BigDecimal_valueOf, TR::java_math_BigDecimal_subtract,
    TR::java_math_BigDecimal_multiply, TR::java_math_BigDecimal_setScale, TR::unknownMethod,
    TR::java_math_BigDecimal_valueOf, TR::java_math_BigDecimal_add, TR::java_math_BigDecimal_add,
    TR::java_math_BigDecimal_multiply, TR::java_math_BigDecimal_setScale };

static TR::RecognizedMethod bdPattern2[] = { TR::java_math_BigDecimal_valueOf, TR::java_math_BigDecimal_subtract,
    TR::java_math_BigDecimal_multiply, TR::java_math_BigDecimal_setScale };

static TR::RecognizedMethod bdPattern3[] = { TR::java_math_BigDecimal_valueOf, TR::java_math_BigDecimal_add,
    TR::java_math_BigDecimal_add, TR::java_math_BigDecimal_multiply, TR::java_math_BigDecimal_setScale };

static TR::RecognizedMethod bdPattern4[]
    = { TR::java_math_BigDecimal_valueOf_J, TR::java_math_BigDecimal_multiply, TR::java_math_BigDecimal_setScale };

static TR::RecognizedMethod *bdPatterns[TR_StringPeepholes::numBDPatterns]
    = { bdPattern1, bdPattern2, bdPattern3, bdPattern4 };
static int32_t bdPatternLengths[TR_StringPeepholes::numBDPatterns] = { 10, 4, 5, 3 };

static TR_BDChain *matchBDPattern(TR_BDChain *chain, TR::RecognizedMethod *pattern, int32_t len,
    TR_ValueProfileInfoManager *profileManager, TR::Compilation *comp, bool trace, bool &obtainProfilingInfo)
{
    ...
    while (cursorChain) {
        bool matched = false;
        logprintf(trace, log, "BigDecimal (binary op) node %p method %d pattern %d in a chain\n", cursorChain->_node,
            cursorChain->_recognizedMethod, pattern[curLen]);
        ...
        if (cursorChain->_recognizedMethod == pattern[curLen]) {
            ...
            if ((cursorChain->_recognizedMethod == TR::java_math_BigDecimal_add)
                ...
                if (comp->getMethodHotness() != hot) {
                    ...
                    TR_BigDecimalValueInfo *valueInfo1 = static_cast<TR_BigDecimalValueInfo *>(
                        profileManager ? profileManager->getValueInfo(callNodeInfo, comp, BigDecimalInfo) : 0);
                    callNodeInfo.setByteCodeIndex(callNodeInfo.getByteCodeIndex() + 1);
                    TR_BigDecimalValueInfo *valueInfo2 = static_cast<TR_BigDecimalValueInfo *>(
                        profileManager ? profileManager->getValueInfo(callNodeInfo, comp, BigDecimalInfo) : 0);

                    TR_BigDecimalValueInfo *resultValueInfo = static_cast<TR_BigDecimalValueInfo *>(
                        profileManager ? profileManager->getValueInfo(callNode, comp, BigDecimalInfo) : 0);

                    if (valueInfo1 && (valueInfo1->getTopProbability() == 1.0f) && valueInfo2
                        && (valueInfo2->getTopProbability() == 1.0f) && resultValueInfo
                        && (resultValueInfo->getTopProbability() == 1.0f)) {
                        int32_t flag1;
                        ...
                        if ((curScale1 >= 0) && (flag1 == 1) && (curScale2 >= 0) && (flag2 == 1) && (resultScale >= 0)
                            && (resultFlag == 1)) {
                            foundScaleInfo = true;
                            ...
                            matched = true;
                            if (trace) {
                                log->printf("Matched a BigDecimal (binary op) method with profile info in a chain with "
                                            "scale1 = %d scale2 = %d result scale = %d\n",
                                    curScale1, curScale2, resultScale);
                                log->printf("Matched a BigDecimal (binary op) method with profile info in a chain in "
                                            "method %s with scales = %d and %d result scale = %d\n",
                                    comp->signature(), curScale1, curScale2, resultScale);
                                fflush(stdout);
                            }
                        } else
                            logprintf(trace, log,
                                "0Failed on profile info from %p for a BigDecimal (binary op) method with profile info "
                                "in a chain with scale1 = %d scale2 = %d\n",
                                TR_PersistentProfileInfo::get(comp), curScale1, curScale2);
                    } else
                        logprintf(trace, log,
                            "1Failed on profile info from %p for a BigDecimal (binary op) method with profile info in "
                            "a chain \n",
        ...
        logprintf(trace, log, "1 len %d cur %d next %p\n", len, curLen, cursorChain->_next);
        if (matched) {
            if (curLen == 0)
                firstInChain = cursorChain;

            logprintf(trace, log, "2 len %d cur %d\n", len, curLen);
            ...
            logprintf(trace, log, "Failed to match on a big decimal method in a chain at node %p\n",
                cursorChain->_node);
            curLen = 0;
        }
    ...
    logprintf(trace, log, "first in chain %p last in chain %p\n", firstInChain, lastInChain);
...
static TR_BDChain *matchBDPatterns(TR_BDChain *chain, TR_ValueProfileInfoManager *profileManager, TR::Compilation *comp,
    int32_t &matchedPattern, bool trace, bool &obtainProfilingInfo)
{
    int32_t i = 0;
    while (i < TR_StringPeepholes::numBDPatterns) {
        TR_BDChain *matchedChain = matchBDPattern(chain, bdPatterns[i], bdPatternLengths[i], profileManager, comp,
            trace, obtainProfilingInfo);
        if (matchedChain) {
            matchedPattern = i;
            return matchedChain;
        }
...
static bool isRecognizedMethod(TR::Node *node, TR::RecognizedMethod rm)
{
    ...
    if (result) {
        ...
        result = (sym != NULL) && (sym->getMethodSymbol() != NULL)
...
TR::TreeTop *TR_StringPeepholes::detectSubMulSetScalePattern(TR::TreeTop *tt, TR::TreeTop *exit, TR::Node *node)
{
    ...
    logprintf(trace(), log,
        "Looking for subtract multiply setscale pattern in block %d, starting at tt: %p, node: %p\n",
    ...
    for (; !foundPattern && tt != exit; tt = tt->getNextRealTreeTop()) {
        ...
        if (cursorNode->getOpCodeValue() == TR::acalli) {
            if (!isRecognizedMethod(cursorNode, TR::java_math_BigDecimal_subtract)
                ...
                logprintf(trace(), log, "callNode %p is not subtract or its refcount is not 2\n", cursorNode);
                continue;
            }
            ...
            if (cursorNode->getOpCodeValue() == TR::acalli) {
                if (!isRecognizedMethod(cursorNode, TR::java_math_BigDecimal_multiply)
                    ...
                    logprintf(trace(), log, "callNode %p is not multiply or its refcount is not 3\n", cursorNode);
                    continue;
                }
                ...
                if (subNode != cursorNode->getChild(cursorNode->getFirstArgumentIndex() + 1)) {
                    logprintf(trace(), log, "multiplyNode %p does not have subtractNode %p as the second child\n",
                        cursorNode, subNode);
                    continue;
                }
                ...
                if (cursorNode->getOpCodeValue() == TR::acalli) {
                    if (!isRecognizedMethod(cursorNode, TR::java_math_BigDecimal_setScale)
                        || cursorNode->getReferenceCount() != 2) {
                        logprintf(trace(), log, "callNode %p is not setScale or its refcount is not 2\n", cursorNode);
                        continue;
                    }

                    // check if setScale nodes's receiver is multiply node
                    if (mulNode != cursorNode->getChild(cursorNode->getFirstArgumentIndex())) {
                        logprintf(trace(), log, "setScaleNode's %p receiver is not multiplyNode %p\n", cursorNode,
                            mulNode);
                        continue;
                    }
                    ...
                    if (cursorNode->getChild(cursorNode->getNumChildren() - 2)->getInt() != 2) {
                        logprintf(trace(), log, "setScaleNode's %p scale is not 2: %d\n", cursorNode,
                            cursorNode->getChild(cursorNode->getNumChildren() - 2)->getInt());
                        continue;
                    }

                    // check if round is ROUND_DOWN=1
                    if (cursorNode->getChild(cursorNode->getNumChildren() - 1)->getInt() != 1) {
                        logprintf(trace(), log, "setScaleNode's %p rounding is not 1: %d\n", cursorNode,
                            cursorNode->getChild(cursorNode->getNumChildren() - 1)->getInt());
                        continue;
                    }
                    ...
                    logprintf(trace(), log, "callNode %p should be setScale, but is not an iacall\n", cursorNode);
            } else
                logprintf(trace(), log, "callNode %p should be multiply, but is not an iacall\n", cursorNode);
        } else
            logprintf(trace(), log, "callNode %p should be subtract, but is not an iacall\n", cursorNode);
    }
    ...
    if (foundPattern) {
        ...
        if (comp()->getMethodHotness() > warm && comp()->getMethodHotness() != hot) {
            logprintf(trace(), log, "Method hotness: %d\n", comp()->getMethodHotness());

            // get mulscale
            TR_ByteCodeInfo nodeInfo = mulNode->getByteCodeInfo();
            nodeInfo.setByteCodeIndex(nodeInfo.getByteCodeIndex() + 1);
            TR_BigDecimalValueInfo *valueInfo1 = static_cast<TR_BigDecimalValueInfo *>(
                profileManager ? profileManager->getValueInfo(nodeInfo, comp(), BigDecimalInfo) : 0);
            // restore the index
            nodeInfo.setByteCodeIndex(nodeInfo.getByteCodeIndex() - 1);
            ...
            if (valueInfo1 && (valueInfo1->getTopProbability() == 1.0f) && valueInfo2
                && (valueInfo2->getTopProbability() == 1.0f)) {
                int32_t flag1, flag2;
                mulScale = valueInfo1->getTopValue(flag1);
                subScale = valueInfo2->getTopValue(flag2);

                if ((mulScale != 2) || (flag2 != 1) || (subScale < 0) || (subScale > 6) || (flag2 != 1)) {
                    logprintf(trace(), log,
                        "Failed on profile info from %p for subtract multiply and setscale pattern with sub scale %d, "
                        "and mul scale %d\n",
            ...
            if (comp()->getMethodHotness() == hot && comp()->getRecompilationInfo()
                && performTransformation(comp(),
                    "%smight have simplified big decimal subtract multiply setscale pattern from node [%p] to [%p] if "
                    "profiling info was available. Switching to profiling.\n",
        ...
        logprintf(trace(), log,
            "Successfully obtained profile info from %p for subtract multiply and setscale pattern with sub scale %d, "
            "and mul scale %d\n",
            TR_PersistentProfileInfo::get(comp()), subScale, mulScale);

        TR::SymbolReference *callSymRef = comp()->fej9()->findOrCreateMethodSymRef(comp(), comp()->getMethodSymbol(),
            "java/math/BigDecimal",
            "java/math/BigDecimal.SMSetScale(Ljava/math/BigDecimal;Ljava/math/BigDecimal;II)Ljava/math/BigDecimal;");
        if (callSymRef
            && performTransformation(comp(),
                "%ssimplified big decimal subtract multiply setscale pattern from node [%p] to [%p]\n",
                optDetailString(), firstTree->getNode(), lastTree->getNode())) {
            // do the transformation
            TR::Node *methodCall = TR::Node::createWithSymRef(TR::acall, 4, 4, subChild, mulChild,
                TR::Node::create(node, TR::iconst, 0, subScale), // 4
                TR::Node::create(node, TR::iconst, 0, mulScale), // 2
                callSymRef);

            logprintf(trace(), log, "method call sym ref: %p, method call: %p\n", callSymRef, methodCall);
            ...
            logprintf(trace(), log, "Replaced big decimal sub mul setscale in method %s with call %p\n",
                comp()->signature(), methodCall);
        }
    } else
        logprintf(trace(), log, "Failed to find subtract multiply setscale pattern in block %d\n",
...
TR::TreeTop *TR_StringPeepholes::detectBDPattern(TR::TreeTop *tt, TR::TreeTop *exit, TR::Node *node)
{
    ...
    TR_BDChain *firstInChain = NULL;
    if ((node->getOpCodeValue() == TR::acall) && node->getSymbolReference()->getSymbol()->isResolvedMethod()
        && ((node->getSymbolReference()->getSymbol()->getResolvedMethodSymbol()->getRecognizedMethod()
                == TR::java_math_BigDecimal_valueOf)
            || (node->getSymbolReference()->getSymbol()->getResolvedMethodSymbol()->getRecognizedMethod()
                == TR::java_math_BigDecimal_valueOf_J))) {
        prevInChain = new (trStackMemory()) TR_BDChain(
            node->getSymbolReference()->getSymbol()->getResolvedMethodSymbol()->getRecognizedMethod(), node, tt, -1);
        if (!firstInChain)
            firstInChain = prevInChain;

        tt = tt->getNextTreeTop();
        while (tt != exit) {
            if (tt->getNode()->getOpCode().isAnchor() || skipNodeUnderOSR(tt->getNode())) {
                tt = tt->getNextTreeTop();
                continue;
            }
    ...
    TR_BDChain *matchedChain
        = matchBDPatterns(firstInChain, profileManager, comp(), matchedPattern, trace(), obtainProfilingInfo);

    logprintf(trace(), log, "For sequence starting at %p matched chain = %p\n", node, matchedChain);
    ...
    if (matchedChain) {
        ...
        for (int chainPos = 0; cursorChain; chainPos++) {
                ...
                if (matchedPattern <= 2) {
                    if ((valueOfNode->getFirstChild()->getOpCodeValue() == TR::lconst)
                        ...
                        if (valueOfNode->getFirstChild()->getLongInt() != 1) {
                            logprints(trace(), log, "Failed at 0\n");
                            treesAsExpected = false;
                        }
                        if (valueOfNode->getSecondChild()->getInt() != 0) {
                            logprints(trace(), log, "Failed at 1\n");
                            treesAsExpected = false;
                        }
                    } else {
                        logprints(trace(), log, "Failed at 2\n");
                        treesAsExpected = false;
                    }
                }
            }
                ...
                if (!add1Node) {
                    ...
                    if (matchedPattern == 0 || matchedPattern == 2) {
                        ...
                        if ((add1Node->getChild(add1Node->getNumChildren() - 2) != valueOfNode)
                            || ((add1Node->getOpCode().isCallIndirect() && (valueOfNode->getReferenceCount() != 3))
                                || (!add1Node->getOpCode().isCallIndirect()
                                    && (valueOfNode->getReferenceCount() != 2)))) {
                            logprintf(trace(), log, "Failed at 3 at add node %p valueOfNode %p\n", add1Node,
                                valueOfNode);
                            treesAsExpected = false;
                        } else {
                    ...
                    if (matchedPattern == 0 || matchedPattern == 2) {
                        if ((add2Node->getChild(add2Node->getNumChildren() - 2) != add1Node)
                            ...
                            logprints(trace(), log, "Failed at 4\n");
                            treesAsExpected = false;
                        } else {
                ...
                if (matchedPattern <= 1) {
                    if ((subtractNode->getChild(subtractNode->getNumChildren() - 2) != valueOf1Node)
                        || ((subtractNode->getOpCode().isCallIndirect() && (valueOf1Node->getReferenceCount() != 3))
                            || (!subtractNode->getOpCode().isCallIndirect()
                                && (valueOf1Node->getReferenceCount() != 2)))) {
                        logprintf(trace(), log, "Failed at 5 sub = %p valueOf = %p\n", subtractNode, valueOf1Node);
                        treesAsExpected = false;
                    } else {
            ...
            if (cursorChain->_recognizedMethod == TR::java_math_BigDecimal_multiply) {
                ...
                if (matchedPattern <= 1 && chainPos == 2) {
                    if ((mulNode->getChild(mulNode->getNumChildren() - 1) != subtractNode)
                        ...
                        logprints(trace(), log, "Failed at 6\n");
                        treesAsExpected = false;
                    } else {
                ...
                if ((matchedPattern == 0 && chainPos == 8) || (matchedPattern == 2 && chainPos == 3)) {
                    if ((mulNode->getChild(mulNode->getNumChildren() - 1) != add2Node)
                        ...
                        logprints(trace(), log, "Failed at 7\n");
                        treesAsExpected = false;
                    } else {
                        mulScale = cursorChain->_scale1;
                        mulChild = mulNode->getChild(mulNode->getNumChildren() - 2);
                ...
                if ((matchedPattern == 0) && (chainPos == 8) && astoreSym
                    && (astoreSym
                        != mulNode->getChild(mulNode->getNumChildren() - 2)->getSymbolReference()->getSymbol())) {
                    logprints(trace(), log, "Failed at 8\n");
                    treesAsExpected = false;
                }
                ...
                if (matchedPattern == 3) {
                    if ((mulNode->getChild(mulNode->getNumChildren() - 2) != valueOf1Node)
                        ...
                        logprints(trace(), log, "Failed at 9\n");
                        treesAsExpected = false;
                    } else {
            ...
            if (cursorChain->_recognizedMethod == TR::unknownMethod) {
                ...
                if (!astoreSym || !astoreSym->isAuto() || !astoreSym->getAutoSymbol() || !astoreNode
                    || (astoreNode->getNumChildren() != 1) || (astoreNode->getOpCodeValue() != TR::astore)) {
                    logprints(trace(), log, "Failed at 13\n");
                    treesAsExpected = false;
                }
            }
        ...
        if ((minScale < 0) || (maxScale > 6)) {
            logprintf(trace(), log, "Failed at 14 with minScale %d and maxScale %d\n", minScale, maxScale);
            if (comp()->getMethodHotness() != hot)
                treesAsExpected = false;
            else
                obtainProfilingInfo = true;
        ...
        if (!obtainProfilingInfo && (add1Scale != subScale) && (matchedPattern == 3)) {
            logprints(trace(), log, "Failed at 18 with (add1Scale != subScale) && (matchedPattern == 3)\n");
            treesAsExpected = false;
        }
        ...
        if (treesAsExpected && obtainProfilingInfo) {
            if (comp()->getMethodHotness() == hot && comp()->getRecompilationInfo()
                && performTransformation(comp(),
                    "%smight have simplified big decimal pattern from node [%p] to [%p] pattern=%d if profiling info "
                    "was available. Switching to profiling.\n",
            ...
            callSymRef = callSymRef
                ? getSymRefTab()->findOrCreateMethodSymbol(valueOf1Node->getSymbolReference()->getOwningMethodIndex(),
                      -1, callSymRef->getSymbol()->getResolvedMethodSymbol()->getResolvedMethod(),
                      TR::MethodSymbol::Static)
                : 0;
            if (matchedPattern == 0) {
                methodCall = TR::Node::createWithSymRef(TR::acall, 9, 4, valueOf1Node, subChild, mul1Child, add1Child,
                    callSymRef);
                ...
                methodCall = TR::Node::createWithSymRef(TR::acall, 5, 5, valueOf1Node, subChild, mul1Child,
                    TR::Node::create(node, TR::iconst, 0, subScale), TR::Node::create(node, TR::iconst, 0, mul1Scale),
                    callSymRef);
            } else if (matchedPattern == 2) {
                methodCall = TR::Node::createWithSymRef(TR::acall, 7, 4, valueOf1Node, add1Child, add2Child, mul1Child,
                    callSymRef);
                ...
                methodCall = TR::Node::createWithSymRef(TR::acall, 3, 3, valueOf1Node, mul1Child,
                    TR::Node::create(node, TR::iconst, 0, mul1Scale), callSymRef);
                if (comp()->target().cpu.isPower()) {
                    ...
                    if (!disableBDPrefetch && sourceNode && comp()->getMethodHotness() >= scorching
                        ...
                        for (TR::TreeTop *tt = comp()->getStartTree(); tt; tt = tt->getNextTreeTop()) {
                            ...
                            if (targetNode->getOpCode().hasSymbolReference()
                                ...
                                && (targetNode->getOpCodeValue() == TR::astorei
                                    || targetNode->getOpCodeValue() == TR::awrtbari)) {
                                int32_t len;
                                TR::SymbolReference *symRef = sourceNode->getSymbolReference();
                                const char *fieldName = symRef->getOwningMethod(comp())->fieldName(symRef->getCPIndex(),
                                    len, comp()->trMemory());
                                TR_OpaqueClassBlock *clazz = comp()->fej9()->getSystemClassFromClassName(
                                    "java/math/BigDecimal", strlen("java/math/BigDecimal"));
                                int32_t scaleOffset
                                    = comp()->fej9()->getInstanceFieldOffset(clazz, "scale", strlen("scale"), "I", 1);
                                if (strstr(fieldName, "Ljava/math/BigDecimal") && scaleOffset > 0) {
                                    if (comp()->findPrefetchInfo(targetNode) < 0) {
                                        ...
                                        TR_Pair<TR::Node, uint32_t> *prefetchInfo = new (comp()->trHeapMemory())
                                            TR_Pair<TR::Node, uint32_t>(targetNode, (uint32_t *)(intptr_t)scaleOffset);
...
TR::TreeTop *TR_StringPeepholes::detectPattern(TR::Block *block, TR::TreeTop *tt, bool useStringBuffer)
{
    ...
    enum {
        MAX_STRINGS = 5
    }; // maximum number strings concatenation allowed
    ...
    while (1) {
        ...
        if (useStringBuffer)
            tt = searchForStringAppend("java/lang/StringBuffer.append(", startTree, exitTree, TR::acall, child,
                visitCount, appendedString + stringCount, primToStringTree + stringCount);
        else
            tt = searchForStringAppend("java/lang/StringBuilder.append(", startTree, exitTree, TR::acall, child,
                visitCount, appendedString + stringCount, primToStringTree + stringCount);
    ...
    if ((stringCount > 3)) {
        if ((stringCount == 5) && (pattern[0] == 'I') && (pattern[1] == 'S') && (pattern[2] == 'I')
            && (pattern[3] == 'S') && (pattern[4] == 'S') && primToStringTree[0] && primToStringTree[2]
    ...
    tt = searchForToStringCall(lastAppendTree->getNextRealTreeTop(), exitTree,
        lastAppendTree->getNode()->getFirstChild(), visitCount, &toStringTree, useStringBuffer);
    if (!toStringTree)
        return 0;
    ...
    bool specialCase = (stringCount == 2 && pattern[0] == 'S'
        && ((pattern[1] == 'C' && findSymRefForOptMethod(SPH_String_init_SC))
            || (pattern[1] == 'I' && findSymRefForOptMethod(SPH_String_init_SI))));

    if (!specialCase) {
        if ((stringCount == 5) && (pattern[0] == 'I') && (pattern[1] == 'S') && (pattern[2] == 'I')
            && (pattern[3] == 'S') && (pattern[4] == 'S') && findSymRefForOptMethod(SPH_String_init_ISISS))
            specialCase = true;
    }
    ...
    if (!specialCase)
        for (int j = 0; j < stringCount; j++)
            if (valueOfSymRef[j]) {
                ...
                //                                                      nodeToConvert
                TR::SymbolReference *symRef = getSymRefTab()->findOrCreateMethodSymbol(
                    (appendedString[j] && appendedString[j]->getOpCode().hasSymbolReference())
                        ? appendedString[j]->getSymbolReference()->getOwningMethodIndex()
                        : JITTED_METHOD_INDEX,
                    -1, valueOfSymRef[j]->getSymbol()->getResolvedMethodSymbol()->getResolvedMethod(),
                    TR::MethodSymbol::Static);
                TR::Node *valueOf = TR::Node::createWithSymRef(stringNode, TR::acall, 1, appendedString[j], symRef);
                TR::TreeTop::create(comp(), appendTree[j], TR::Node::create(stringNode, TR::treetop, 1, valueOf));
    ...
    if (stringCount == 2) {
        if (specialCase) {
            if (pattern[1] == 'C') {
                TR::SymbolReference *initSymRef = findSymRefForOptMethod(SPH_String_init_SC)
                    ? getSymRefTab()->findOrCreateMethodSymbol(stringNode->getSymbolReference()->getOwningMethodIndex(),
                          -1,
                          findSymRefForOptMethod(SPH_String_init_SC)
                              ->getSymbol()
                              ->getResolvedMethodSymbol()
                              ->getResolvedMethod(),
                          TR::MethodSymbol::Special)
                    : 0;
                initCall = TR::Node::createWithSymRef(TR::call, 3, 3, stringNode, appendedString[0], appendedString[1],
                    initSymRef);
            } else {
                TR::SymbolReference *initSymRef3 = findSymRefForOptMethod(SPH_String_init_SI)
                    ? getSymRefTab()->findOrCreateMethodSymbol(stringNode->getSymbolReference()->getOwningMethodIndex(),
                          -1,
                          findSymRefForOptMethod(SPH_String_init_SI)
                              ->getSymbol()
                              ->getResolvedMethodSymbol()
                              ->getResolvedMethod(),
                          TR::MethodSymbol::Special)
        ...
        {
            TR::SymbolReference *initSymRef1 = findSymRefForOptMethod(SPH_String_init_SS)
                ? getSymRefTab()->findOrCreateMethodSymbol(stringNode->getSymbolReference()->getOwningMethodIndex(), -1,
                      findSymRefForOptMethod(SPH_String_init_SS)
                          ->getSymbol()
                          ->getResolvedMethodSymbol()
                          ->getResolvedMethod(),
                      TR::MethodSymbol::Special)
        ...
        TR::SymbolReference *initSymRef2 = findSymRefForOptMethod(SPH_String_init_SSS)
            ? getSymRefTab()->findOrCreateMethodSymbol(stringNode->getSymbolReference()->getOwningMethodIndex(), -1,
                  findSymRefForOptMethod(SPH_String_init_SSS)
                      ->getSymbol()
                      ->getResolvedMethodSymbol()
                      ->getResolvedMethod(),
                  TR::MethodSymbol::Special)
            : 0;
        initCall = TR::Node::createWithSymRef(TR::call, 4, 4, stringNode, appendedString[0], appendedString[1],
            appendedString[2], initSymRef2);
    } else if (stringCount == 5) {
        TR::SymbolReference *initSymRef7 = findSymRefForOptMethod(SPH_String_init_ISISS)
            ? getSymRefTab()->findOrCreateMethodSymbol(stringNode->getSymbolReference()->getOwningMethodIndex(), -1,
                  findSymRefForOptMethod(SPH_String_init_ISISS)
                      ->getSymbol()
                      ->getResolvedMethodSymbol()
                  ...
                  TR::MethodSymbol::Special)
            : 0;
        initCall = TR::Node::createWithSymRef(TR::call, 6, 6, stringNode, appendedString[0], appendedString[1],
            appendedString[2], appendedString[3], appendedString[4], initSymRef7);
    }
...
TR::SymbolReference *TR_StringPeepholes::findSymRefForValueOf(const char *sig)
{
    ...
    TR_ResolvedMethod *method
        = comp()->fej9()->getResolvedMethodForNameAndSignature(trMemory(), stringClass, "valueOf", sig);
    return method ? getSymRefTab()->findOrCreateMethodSymbol(JITTED_METHOD_INDEX, -1, method, TR::MethodSymbol::Static)
                  : NULL;
}
...
{
    bool result = false;
    if (comp()->getOption(TR_EnableOSR) && comp()->isOSRTransitionTarget(TR::postExecutionOSR)) {
        if (comp()->getMethodSymbol()->isOSRRelatedNode(node)) {
            result = true;
        }
    }

    if (!result && node->getOpCodeValue() == TR::treetop && node->getFirstChild()->isPotentialOSRPointHelperCall()) {
        result = true;
    }

    if (result)
        logprintf(trace(), comp()->log(), "Skipping OSR node [%p] n%dn\n", node, node->getGlobalIndex());
...
void TR_StringPeepholes::removeAllocationFenceOfNew(TR::TreeTop *newTreeTop)
{
    ...
    if (cursor && cursor->getNode()->getOpCodeValue() == TR::allocationFence
...
bool TR_StringPeepholes::classesRedefined()
{
    ...
    TR_OpaqueClassBlock *clazz
        = comp()->fej9()->getSystemClassFromClassName("java/lang/StringBuffer", strlen("java/lang/StringBuffer"));
...
bool TR_StringPeepholes::classRedefined(TR_OpaqueClassBlock *clazz)
{
    ...
    TR_PersistentClassInfo *clazzInfo
        = comp()->getPersistentInfo()->getPersistentCHTable()->findClassInfoAfterLocking(clazz, fe());
    ...
    TR::ILOpCodes opCode, TR::Node *newBuffer, vcount_t visitCount, TR::Node **string, TR::TreeTop **prim)
{
    ...
    for (; tt != exitTree; tt = tt->getNextRealTreeTop()) {
        ...
        if (skipNodeUnderOSR(node)) {
            logprintf(trace(), log, "Skipping OSR node [%p] when searching for append\n", node);
            continue;
        }

        if (node->getNumChildren() == 1 && node->getFirstChild()->getOpCodeValue() == opCode) {
            if (checkMethodSignature(node->getFirstChild()->getSymbolReference(), sig)) {
                ...
                if ((node->getFirstChild()->getReferenceCount() == refCount)
                    ...
                    TR_ASSERT(node->getFirstChild()->getNumChildren() == 1,
                        "The number of children of java/lang/Integer.toString(I) should be equal to 1");

                    TR::TreeTop *cursor = tt->getNextRealTreeTop();
                    TR::Node *prevCall = node->getFirstChild();
                    while (skipNodeUnderOSR(cursor->getNode())) {
                        logprintf(trace(), log, "Skipping OSR node [%p] when searching for append with integer\n",
                            node);

                        // Check the additional reference to Integer.toString is seen
                        if (cursor->getNode()->getOpCode().isStoreDirect()
                            && cursor->getNode()->getFirstChild() == prevCall)
                            seenRef = true;
                    ...
                    node = cursor->getNode();
                    if (seenRef && node->getNumChildren() == 1 && node->getFirstChild()->getOpCodeValue() == opCode) {
                        if (checkMethodSignature(node->getFirstChild()->getSymbolReference(), sig)) {
                            TR::Node *call = node->getFirstChild();
                            if (call->getFirstChild() == newBuffer) {
        ...
        if (countNodeOccurrencesInSubTree(node, newBuffer, visitCount) > 0)
            return tt;
    }
...
TR::TreeTop *TR_StringPeepholes::searchForInitCall(const char *sig, TR::TreeTop *tt, TR::TreeTop *exitTree,
    TR::Node *newBuffer, vcount_t visitCount, TR::TreeTop **initTree)
{
    for (; tt != exitTree; tt = tt->getNextRealTreeTop()) {
        ...
        if (skipNodeUnderOSR(node)) {
            logprintf(trace(), comp()->log(), "Skipping OSR node [%p] when searching for init\n", node);
            continue;
        }

        if (node->getNumChildren() == 1 && node->getFirstChild()->getOpCodeValue() == TR::call) {
            if (checkMethodSignature(node->getFirstChild()->getSymbolReference(), sig)) {
                TR::Node *call = node->getFirstChild();
                if (call->getFirstChild() == newBuffer)
                    *initTree = tt;
...
TR::TreeTop *TR_StringPeepholes::searchForToStringCall(TR::TreeTop *tt, TR::TreeTop *exitTree, TR::Node *newBuffer,
    vcount_t visitCount, TR::TreeTop **toStringTree, bool useStringBuffer)
{
    for (; tt != exitTree; tt = tt->getNextRealTreeTop()) {
        TR::Node *node = tt->getNode();

        if (skipNodeUnderOSR(node)) {
            logprintf(trace(), comp()->log(), "Skipping OSR node [%p] when searching for toString\n", node);
            continue;
        }

        if (node->getNumChildren() == 1 && node->getFirstChild()->getOpCodeValue() == TR::acall) {
            if (checkMethodSignature(node->getFirstChild()->getSymbolReference(),
                    (useStringBuffer ? "java/lang/StringBuffer.toString()Ljava/lang/String;"
...
{
    if (comp()->supportsInduceOSR() && comp()->isOSRTransitionTarget(TR::postExecutionOSR)
        ...
        if (trace()) {
            ...
            comp()->log()->printf("Post process Trees from %p n%dn to %p n%dn for OSR\n", startNode,
                startNode->getGlobalIndex(), endNode, endNode->getGlobalIndex());
        }
