...
static bool hasOSRFearPoint(TR::Compilation *comp)
{
    for (TR::TreeTop *treeTop = comp->getStartTree(); treeTop; treeTop = treeTop->getNextTreeTop()) {
        ...
        if (ttNode->getNumChildren() == 1 && ttNode->getFirstChild()->isOSRFearPointHelperCall()) {
            return true;
        }
    }
...
static bool hasUnsupportedPotentialOSRPoint(TR::Compilation *comp, bool trace)
{
    for (TR::TreeTop *treeTop = comp->getStartTree(); treeTop; treeTop = treeTop->getNextTreeTop()) {
        ...
        if (comp->isPotentialOSRPoint(ttNode) && !comp->isPotentialOSRPointWithSupport(treeTop)) {
...
void TR_OSRGuardInsertion::removeRedundantPotentialOSRPointHelperCalls(TR_HCRGuardAnalysis *guardAnalysis)
{
    ...
    for (TR::TreeTop *treeTop = comp()->getStartTree(); treeTop; treeTop = treeTop->getNextRealTreeTop()) {
        TR::Node *ttNode = treeTop->getNode();
        ...
        if (comp()->isPotentialOSRPoint(ttNode, &osrNode)) {
            ...
            if (protectedByOSRPoints && osrNode->isPotentialOSRPointHelperCall()) {
    ...
    if (trace()) {
        comp()->dumpMethodTrees(log, "Trees after redundant potentialOSRPointHelper call removal",
    ...
    if (canInsertOSRGuards && (needHCRGuardRemoval || hasFearPoint)) {
        ...
        if (requiresAnalysis)
            TR::DebugCounter::incStaticDebugCounter(comp(),
                TR::DebugCounter::debugCounterName(comp(), "requiresAnalysis/(%s %s)", comp()->signature(),
        ...
        TR_HCRGuardAnalysis *guardAnalysis
            = requiresAnalysis ? new (comp()->allocator()) TR_HCRGuardAnalysis(comp(), optimizer(), structure) : NULL;
...
const char *TR_OSRGuardInsertion::optDetailString() const throw() { return "O^O OSR GUARD INS: "; }
...
TR_Structure *fakeRegion(TR::Compilation *comp)
{
    ...
    TR_Array<TR_StructureSubGraphNode *> *blocks = new (comp->trStackMemory())
        TR_Array<TR_StructureSubGraphNode *>(comp->trMemory(), cfg->getNumberOfNodes(), false, stackAlloc);
    ...
    for (cfgNode = cfg->getFirstNode(); cfgNode; cfgNode = cfgNode->getNext()) {
        ...
        (*blocks)[block->getNumber()] = new (structureMemoryRegion)
            TR_StructureSubGraphNode(new (structureMemoryRegion) TR_BlockStructure(comp, block->getNumber(), block));
...
void TR_OSRGuardInsertion::removeHCRGuards(TR_BitVector &fearGeneratingNodes, TR_HCRGuardAnalysis *guardAnalysis)
{
    for (TR::Block *cursor = comp()->getStartBlock(); cursor != NULL; cursor = cursor->getNextBlock()) {
        ...
        TR_ASSERT(!guardInfo->getByteCodeInfo().isInvalidByteCodeIndex(),
            "we expect to get valid bytecode info for a virtual guard!");
        ...
        if (!guardAnalysis || guardAnalysis->_blockAnalysisInfo[cursor->getNextBlock()->getNumber()]->isEmpty()) {
            if (guardInfo->getKind() == TR_HCRGuard
                ...
                if (cursor->getPredecessors().size() == 1
                    ...
                    potentialGuard
                        = cursor->getPredecessors().front()->getFrom()->asBlock()->getLastRealTreeTop()->getNode();
                ...
                if (additionalVirtualGuard && TR_FearPointAnalysis::virtualGuardKillsFear(comp(), potentialGuard)) {
                    ...
                    // With the HCR guard removed, the fall through node, which should be the first node of the inlined
                    // call, now generates fear
                ...
                TR::DebugCounter::prependDebugCounter(comp(),
                    TR::DebugCounter::debugCounterName(comp(), "hcrGuardRemoval/success"), cursor->getExit());
            } else if (guardInfo->mergedWithHCRGuard()
                && performTransformation(comp(), "O^O HCR GUARD REMOVAL: removing HCR guard merged into node n%un\n",
                    node->getGlobalIndex())) {
                comp()->addClassForOSRRedefinition(guardInfo->getThisClass());
            ...
            if (guardInfo->getKind() == TR_HCRGuard) {
                TR::DebugCounter::prependDebugCounter(comp(),
                    TR::DebugCounter::debugCounterName(comp(), "hcrGuardRemoval/notsuppoted"),
                    cursor->getLastRealTreeTop());
            }
        }
    }
}
...
int32_t TR_OSRGuardInsertion::insertOSRGuards(TR_BitVector &fearGeneratingNodes)
{
    ...
    TR_FearPointAnalysis fearAnalysis(comp(), optimizer(), comp()->getFlowGraph()->getStructure(), fearGeneratingNodes,
        true, trace());
    ...
    for (TR::TreeTop *cursor = comp()->findLastTree(); cursor; cursor = cursor->getPrevTreeTop()) {
        ...
        if (cursor->getNode()->isTheVirtualGuardForAGuardedInlinedCall()
            ...
            TR::DebugCounter::prependDebugCounter(comp(),
                TR::DebugCounter::debugCounterName(comp(), "osrGuardSummary/patch/vguard/%s/=%d",
                    comp()->getHotnessName(comp()->getMethodHotness()), block->getFrequency()),
                cursor);
            TR::DebugCounter::prependDebugCounter(comp(),
                TR::DebugCounter::debugCounterName(comp(), "osrGuardVGLocation/%s/(%s)/block_%d@%d",
                    comp()->getHotnessName(comp()->getMethodHotness()), comp()->signature(), block->getNumber(),
                    block->getFrequency()),
                cursor);
            ...
            bool induceOSR = comp()->allowRecompilation()
                ? comp()->getMethodSymbol()->induceOSRAfterAndRecompile(cursor, nodeBCI, guard, false, 0, &cfgEnd)
            ...
            if (!fear.isEmpty() || forceOSRInsertion) {
                ...
                while (nextTree && nextTree->getNode()->getOpCode().isStoreDirect()
                    && nextTree->getNode()->getOpCode().hasSymbolReference()
                    && nextTree->getNode()->getSymbol()->isAutoOrParm()
                    && !nextTree->getNode()->getSymbol()->isPendingPush()
                    && (nextTree->getNode()->getFirstChild()->getOpCode().isCall()
                        || nextTree->getNode()->getFirstChild()->getOpCode().isNew())) {
                    inductionPoint = nextTree;
                    nextTree = nextTree->getNextTreeTop();
                }
                ...
                if (cursor->getNode()->getOpCodeValue() != TR::asynccheck
                    && cursor->getNode()->getOpCodeValue() != TR::monent
                    ...
                    if (disableNormalCallOSRInduction != NULL) {
                        ...
                        shouldInduce
                            = !(osrCallExcludeLoopsOnly == NULL || block->getStructureOf()->getContainingLoop());
                    }
                }

                if (shouldInduce) {
                    TR::DebugCounter::prependDebugCounter(comp(),
                        TR::DebugCounter::debugCounterName(comp(), "osrGuardSummary/patch/%s/%s/=%d", label,
                            comp()->getHotnessName(comp()->getMethodHotness()), block->getFrequency()),
                        inductionPoint);
                    TR::DebugCounter::prependDebugCounter(comp(),
                        TR::DebugCounter::debugCounterName(comp(), "osrGuardPatchLocation/%s/(%s)/block_%d@%d",
                            comp()->getHotnessName(comp()->getMethodHotness()), comp()->signature(), block->getNumber(),
                            block->getFrequency()),
                        inductionPoint);
                    if (cursor->getNode()->getOpCodeValue() == TR::asynccheck)
                        TR::DebugCounter::prependDebugCounter(comp(),
                            TR::DebugCounter::debugCounterName(comp(), "osrAsynccheckPatch/%s/(%s)/(%s)/block_%d@%d",
                                comp()->getHotnessName(comp()->getMethodHotness()),
                                nodeBCI.getCallerIndex() > -1
                                    ? comp()->getInlinedResolvedMethod(nodeBCI.getCallerIndex())->signature(trMemory())
                                    : comp()->signature(),
                                comp()->signature(), block->getNumber(), block->getFrequency()),
                            inductionPoint);
                    else if (cursor->getNode()->getOpCodeValue() == TR::monent)
                        TR::DebugCounter::prependDebugCounter(comp(),
                            TR::DebugCounter::debugCounterName(comp(), "osrMonentPatch/%s/(%s)/(%s)/block_%d@%d",
                                comp()->getHotnessName(comp()->getMethodHotness()),
                                nodeBCI.getCallerIndex() > -1
                                    ? comp()->getInlinedResolvedMethod(nodeBCI.getCallerIndex())->signature(trMemory())
                                    : comp()->signature(),
                        ...
                        TR::DebugCounter::prependDebugCounter(comp(),
                            TR::DebugCounter::debugCounterName(comp(), "osrCallPatch/%s/(%s)/(%s)/block_%d@%d",
                                comp()->getHotnessName(comp()->getMethodHotness()),
                                nodeBCI.getCallerIndex() > -1
                                    ? comp()->getInlinedResolvedMethod(nodeBCI.getCallerIndex())->signature(trMemory())
                                    : comp()->signature(),
                    ...
                    bool induceOSR = comp()->allowRecompilation()
                        ? targetMethod->induceOSRAfterAndRecompile(inductionPoint, nodeBCI, guard, false,
                              comp()->getOSRInductionOffset(cursor->getNode()), &cfgEnd)
                        : targetMethod->induceOSRAfter(inductionPoint, nodeBCI, guard, false,
                              comp()->getOSRInductionOffset(cursor->getNode()), &cfgEnd);
                    ...
                    TR::DebugCounter::prependDebugCounter(comp(),
                        TR::DebugCounter::debugCounterName(comp(), "osrGuardSummary/exclude/%s/%s/=%d", label,
                            comp()->getHotnessName(comp()->getMethodHotness()), block->getFrequency()),
                        inductionPoint);
                ...
                TR::DebugCounter::prependDebugCounter(comp(),
                    TR::DebugCounter::debugCounterName(comp(), "osrGuardSummary/nofear/%s/%s/=%d", label,
                        comp()->getHotnessName(comp()->getMethodHotness()), block->getFrequency()),
                    cursor);
            ...
            TR::DebugCounter::prependDebugCounter(comp(),
                TR::DebugCounter::debugCounterName(comp(), "osrGuardSummary/allowlist/asynccheck/%s/=%d",
                    comp()->getHotnessName(comp()->getMethodHotness()), block->getFrequency()),
                cursor);
        } else {
            if (cursor->getNode()->getGlobalIndex() < initialNodeCount)
...
void TR_OSRGuardInsertion::generateTriggeringRecompilationTrees(TR::TreeTop *osrGuard,
    TR_PersistentMethodInfo::InfoBits reason)
{
    if (comp()->isRecompilationEnabled() && !comp()->getOption(TR_DisableRecompDueToInlinedMethodRedefinition)) {
        ...
        TR::TreeTop *callTree
            = TR::TransformUtil::generateRetranslateCallerWithPrepTrees(osrInduceBlockStart->getNode(), reason, comp());
 ...
 */
void TR_OSRGuardInsertion::performRemat(TR::TreeTop *osrPoint, TR::TreeTop *osrGuard, TR::TreeTop *rematDest)
{
    ...
    while (realStart->getNode()->getOpCode().isStoreDirect() && !realStart->getNode()->getSymbol()->isPendingPush())
    ...
    for (TR::TreeTop *cursor = realStart; cursor; cursor = cursor->getPrevTreeTop()) {
        ...
        if (node->getOpCode().isStoreDirect() && !seen[node->getSymbolReference()->getReferenceNumber()]) {
            ...
            TR_YesNoMaybe result = RematTools::gatherNodesToCheck(comp(), node, node->getFirstChild(), scanTargets,
                argSymRefsToCheck, trace());
    ...
    if (failedArgs.size() > 0) {
        RematTools::walkTreeTopsCalculatingRematFailureAlternatives(comp(), start, osrGuard, failedArgs, scanTargets,
            safetyInfo, blocksToVisit, trace());
        ...
        for (auto iter = failedArgs.begin(); iter != failedArgs.end(); ++iter) {
            ...
            TR::DebugCounter::prependDebugCounter(comp(),
                TR::DebugCounter::debugCounterName(comp(), "osrGuardRemat/Failed/%s",
                    store->getFirstChild()->getOpCode().getName()),
                storeTree, 1, TR::DebugCounter::Expensive);
            TR::DebugCounter::incStaticDebugCounter(comp(),
                TR::DebugCounter::debugCounterName(comp(), "osrGuardRemat.byMethod/Failed/(%s)/%s", comp()->signature(),
                    store->getFirstChild()->getOpCode().getName()));
            TR::DebugCounter::incStaticDebugCounter(comp(),
                TR::DebugCounter::debugCounterName(comp(), "osrGuardRemat.byReason/%s",
                    store->getFirstChild()->getOpCode().getName()));
        }
    }
    ...
    if (!scanTargets.IsZero())
        RematTools::walkTreesCalculatingRematSafety(comp(), start, osrGuard, scanTargets, unsafeSymRefs, blocksToVisit,
            trace());
    ...
    for (int32_t i = safetyInfo.size() - 1; i >= 0; --i) {
        ...
        if (!unsafeSymRefs.Intersects(safetyInfo.symRefDependencies(i))) {
            ...
            TR::DebugCounter::incStaticDebugCounter(comp(),
                TR::DebugCounter::debugCounterName(comp(), "osrGuardRemat.byMethod/(%s)/Succeeded",
                    comp()->signature()));
            TR::DebugCounter::incStaticDebugCounter(comp(),
                TR::DebugCounter::debugCounterName(comp(), "osrGuardRemat.byReason/Success"));
            TR::DebugCounter::prependDebugCounter(comp(),
                TR::DebugCounter::debugCounterName(comp(), "osrGuradRemat/Succeeded/(%s)",
                    store->getFirstChild()->getOpCode().getName()),
                storeTree, 1, TR::DebugCounter::Expensive);
            ...
            if (rematTree == storeTree) {
                ...
                TR::Node *duplicateStore = TR::Node::createStore(store->getSymbolReference(),
                    TR::Node::createLoad(store, rematTree->getNode()->getSymbolReference()));
                duplicateStore->setByteCodeInfo(store->getByteCodeInfo());
                if (performTransformation(comp(),
                        "O^O GUARDED CALL REMAT: Partial rematerialize of [%p] as [%p] - load of [%d]\n", store,
            ...
            TR::DebugCounter::incStaticDebugCounter(comp(),
                TR::DebugCounter::debugCounterName(comp(), "osrGuardRemat.byMethod/unsafeSymRef/Failed/(%s)/%s",
                    comp()->signature(), store->getFirstChild()->getOpCode().getName()));
            TR::DebugCounter::incStaticDebugCounter(comp(),
                TR::DebugCounter::debugCounterName(comp(), "osrGuardRemat.byReason/unsafeSymRef/%s",
                    store->getFirstChild()->getOpCode().getName()));
            TR::DebugCounter::prependDebugCounter(comp(),
                TR::DebugCounter::debugCounterName(comp(), "osrGuardRemat/Failed/(%s)",
                    store->getFirstChild()->getOpCode().getName()),
...
void TR_OSRGuardInsertion::collectFearFromOSRFearPointHelperCalls(TR_BitVector &fearGeneratingNodes,
    TR_HCRGuardAnalysis *guardAnalysis)
{
    bool protectedByOSRPoints = false;
    TR::NodeChecklist visited(comp());
    ...
    for (TR::TreeTop *treeTop = comp()->getStartTree(); treeTop; treeTop = treeTop->getNextRealTreeTop()) {
        ...
        TR::Node *node = ttNode->getFirstChild();
        if (node && node->isOSRFearPointHelperCall()) {
            ...
            if (assertOnFearPointWithoutProtection) {
                TR_ASSERT_FATAL(protectedByOSRPoints,
                    "A fear point node %p n%dn [%d,%d] is reached by unsupported potential OSR point\n", node,
                    node->getGlobalIndex(), node->getByteCodeInfo().getCallerIndex(), node->getByteCodeIndex());
            }
