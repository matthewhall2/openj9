...
public:
    ...
    bool adjustedMonentBlock(int32_t blockNum)
    {
        if (_monentBlocks.get(blockNum))
            return true;
        return false;
    }

    bool adjustedMonexitBlock(int32_t blockNum)
    {
        if (_monexitBlocks.get(blockNum))
            return true;
        return false;
    }
    ...
    void addMonentEdge(TR::CFGEdge *edge)
    {
        if (!_monentEdges.find(edge))
            _monentEdges.add(edge);
    }

    List<TR::CFGEdge> &getMonexitEdges() { return _monexitEdges; }

    void addMonexitEdge(TR::CFGEdge *edge)
    {
        if (!_monexitEdges.find(edge))
...
public:
    ...
    static void collectPredsAndSuccs(TR::CFGNode *, TR_BitVector *, TR_BitVector **, TR_BitVector **,
        List<TR::CFGEdge> *, TR_BitVector *, TR::Compilation *, bool trace);
...
private:
    ...
    bool searchDownForOtherMonitorsInSuccessors(TR::TreeTop *tt, int32_t &size, TR::list<TR::TreeTop *> &closeMonitors,
        int32_t maxDepth, int32_t minNumberOfNodes);
    TR_ActiveMonitor *findActiveMonitor(TR::TreeTop *tt);
    bool hasMultipleEntriesWithSameExit(TR_ActiveMonitor *monitor);

    TR_Array<TR::Block *> *createFailHandlerBlocks(TR_ActiveMonitor *monitor, TR::SymbolReference *tempSymRef,
        TR::Block *monitorBlock, TR::Block *tstartblock);
    ...
    TR::Block *adjustBlockToCreateReadMonitor(TR::TreeTop *, TR::Node *, int32_t, TR::Block *, TR::Block *,
        TR::Block *);
    ...
    // bool checkIfSuccsInList(TR_SuccessorIterator *, TR_BitVector *, bool checkIfControlFlowInCatch = false, TR::Block
    // *block = NULL); bool checkIfPredsInList(TR_PredecessorIterator *, TR_BitVector *, TR::Block *block = NULL);
    bool checkIfSuccsInList(TR::CFGEdgeList &, TR_BitVector *, bool checkIfControlFlowInCatch = false,
        TR::Block *block = NULL);
    ...
    bool addClassThatShouldNotBeNewlyExtended(TR_OpaqueClassBlock *clazz, TR_LinkHead<TR_ClassExtendCheck> *,
        bool stackAllocation);
    ...
    TR_BitVector *_intersection, *_subtraction, *_temp, *_inclusiveIntersection, *_closureIntersection, *_scratch,
        *_monexitWorkList, *_loopEntryBlocks;
    TR_BitVector *_guardedVirtualCallBlocks, *_adjustedMonexitBlocks, *_adjustedMonentBlocks, *_matchingMonentBlocks,
        *_matchingMonexitBlocks, *_matchingSpecialBlocks, *_matchingNormalMonentBlocks, *_matchingNormalMonexitBlocks;
    TR_BitVector *_containsExceptions, *_containsCalls, *_containsAsyncCheck, *_containsMonents, *_containsMonexits,
        *_lockedObjects, *_multiplyLockedObjects, *_coarsenedMonexits, *_coarsenedMonitors;
