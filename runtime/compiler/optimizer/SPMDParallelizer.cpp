...
{
    if (node->getOpCodeValue() == TR::iload && node->getSymbolReference() == piv)
        return true;
...
int32_t TR_SPMDKernelParallelizer::getUnrollCount(TR::DataType dt)
{
    ...
    switch (dt) {
        case TR::Int8:
            unrollCount = VECTOR_SIZE;
            break;
        case TR::Int16:
            unrollCount = VECTOR_SIZE / 2;
            break;
        case TR::Int32:
        case TR::Float:
            unrollCount = VECTOR_SIZE / 4;
            break;
        case TR::Int64:
        case TR::Double:
            unrollCount = VECTOR_SIZE / 8;
            break;
    }
...
// SIMD_TODO: use visitCount or some caching?
bool TR_SPMDKernelParallelizer::isAffineAccess(TR::Compilation *comp, TR::Node *node, TR_RegionStructure *loop,
    TR::SymbolReference *piv, int32_t &pivStride)
{
    ...
    if (node->getOpCodeValue() == TR::i2l) {
        ...
        if (isFirstChildInvar && isSecondChildInvar) {
            ...
            if (isAffineAccess(comp, node->getFirstChild(), loop, piv, child1Stride)) {
                if (child1Stride == 0)
                    return true;
                else if (child1Stride != INVALID_STRIDE && node->getSecondChild()->getOpCode().isLoadConst())
            ...
            if (isAffineAccess(comp, node->getSecondChild(), loop, piv, child2Stride)) {
                if (child2Stride == 0)
                    return true;
                else if (child2Stride != INVALID_STRIDE && node->getFirstChild()->getOpCode().isLoadConst())
...
{
    // we need to duplicate tree because this node could be commoned with any other expression inside the loop, e.g.
    // address expression. we don't want to vectorize all the common-ed nodes
    TR::Node *splatsNode = TR::Node::create(
        TR::ILOpCode::createVectorOpCode(TR::vsplats, node->getDataType().scalarToVector(VECTOR_LENGTH)), 1,
        node->duplicateTree());
...
static void collectUses(TR::Node *defNode, TR::Compilation *comp, TR_UseDefInfo *info,
    CS2::ArrayOf<TR::Node *, TR::Allocator> &useNodesOfDefsInLoop)
{
...
bool TR_SPMDKernelParallelizer::visitTreeTopToSIMDize(TR::TreeTop *tt, TR_SPMDKernelInfo *pSPMDInfo, bool isCheckMode,
    TR_RegionStructure *loop, CS2::ArrayOf<TR::Node *, TR::Allocator> &useNodesOfDefsInLoop, TR::Compilation *comp,
    TR_UseDefInfo *useDefInfo, SharedSparseBitVector &defsInLoop, SharedSparseBitVector *usesInLoop,
    TR_HashTab *reductionHashTab)
{
    ...
    logprintf(trace, log, "   Visiting Tree Top [%p] during %s mode \n", node,
        isCheckMode ? "detection" : "transformation");
    ...
    if (scalarOp.isStore()) {
        ...
        if (scalarOp.isIndirect()) {
            ...
            if (!isCheckMode) {
                ...
                vecSymRef = comp->getSymRefTab()->findOrCreateArrayShadowSymbolRef(
                    node->getDataType().scalarToVector(TR::VectorLength128), NULL);
            ...
            return visitNodeToSIMDize(node, 1, node->getSecondChild(), pSPMDInfo, isCheckMode, loop, comp, usesInLoop,
                useNodesOfDefsInLoop, useDefInfo, defsInLoop, reductionHashTab, /*storeSymRef*/ 0);
        } else {
            // Direct store
            if (node->getOpCodeValue() == TR::istore) {
                ...
                if (isPIVDefNode) {
                    ...
                    // if the induction variable is not incremented after the soon to be vectorized instructions, then
                    // we bail out (see: openj9/issues/9331)
                    if (isCheckMode) {
                        if (!(tt->getNextTreeTop()->getNode()->getOpCode().isBooleanCompare()
                            ...
                            logprints(trace, log,
                                "   Induction variable may be incremented before vectorized operations, "
                                "this may cause functionally incorrect result. Vectorization not performed. see: "
                                "openj9/issues/9331.\n");
                            return false;
                        }
                    }
                    ...
                    if (!isCheckMode && pSPMDInfo->getIsPIVVectorized() && isAffine) {
                        ...
                        TR::SymbolReference *vecSymRef = pSPMDInfo->getVectorSymRef(symRef);
                        if (vecSymRef == NULL) {
                            vecSymRef = comp->cg()->allocateLocalTemp(
                                node->getDataType().scalarToVector(TR::VectorLength128)); // need to handle alignment?
                        ...
                        TR::ILOpCode scalarOp = node->getOpCode();
                        TR::ILOpCodes vectorOpCode
                            = TR::ILOpCode::convertScalarToVector(scalarOp.getOpCodeValue(), VECTOR_LENGTH);
                        TR_ASSERT(vectorOpCode != TR::BadILOp, "BAD IL Opcode to be assigned during transformation");
                        ...
                        logprintf(trace, log,
                            "   Duplicating trees [%p] into [%p] and vectorizing for Vectorized PIV increment\n", node,
                            dupNode);

                        return visitNodeToSIMDize(dupNode, 0, dupNode->getFirstChild(), pSPMDInfo, false, loop, comp,
                            usesInLoop, useNodesOfDefsInLoop, useDefInfo, defsInLoop, reductionHashTab,
                            /*storeSymRef*/ 0);
                    }
            ...
            if (isCheckMode) {
                ...
                // If the reductionOp field is Reduction_Invalid we already know it is not a valid or supported
                // reduction
                if (reductionInfo->reductionOp != Reduction_Invalid) {
                    ...
                    if (isReduction(comp, loop, node->getFirstChild(), reductionInfo, reductionInfo->reductionOp)) {
                        ...
                        // as a result the nodes that store to it are no longer considered stores to a reduction
                        // variable this adds the recorded nodes back into defsInLoop
                        ...
                        for (TR::Node *nextNode = nodeIt.getCurrent(); nextNode; nextNode = nodeIt.getNext()) {
                            logprintf(trace, log,
                                "   visitTreeTopToSIMDize: Adding recorded invalid reduction node to defsInLoop. node: "
                                "%p\n",
                                nextNode);
            ...
            return visitNodeToSIMDize(node, 0, node->getFirstChild(), pSPMDInfo, isCheckMode, loop, comp, usesInLoop,
                useNodesOfDefsInLoop, useDefInfo, defsInLoop, reductionHashTab, node->getSymbolReference());
        }
    } else if (node->getOpCodeValue() == TR::asynccheck) {
        /*
         * By default, loops with asynccheck nodes are not supported for AutoSIMD.
         * If TR_enableAutoSIMDSkipAsynccheck is set, the asynccheck nodes will be removed from the loop while
         * vectorizing it.
         */
...
bool TR_SPMDKernelParallelizer::visitNodeToSIMDize(TR::Node *parent, int32_t childIndex, TR::Node *node,
    TR_SPMDKernelInfo *pSPMDInfo, bool isCheckMode, TR_RegionStructure *loop, TR::Compilation *comp,
    SharedSparseBitVector *usesInLoop, CS2::ArrayOf<TR::Node *, TR::Allocator> &useNodesOfDefsInLoop,
    TR_UseDefInfo *useDefInfo, SharedSparseBitVector &defsInLoop, TR_HashTab *reductionHashTab,
    TR::SymbolReference *storeSymRef)
{
    ...
    logprintf(trace, log, "   Visiting Node [%p] during %s mode - %s\n", node,
        isCheckMode ? "detection" : "transformation", scalarOp.getName());
    ...
    if (loop->isExprInvariant(node)) {
        ...
        if (isCheckMode
            && !comp->cg()->getSupportsOpCodeForAutoSIMD(TR::ILOpCode::createVectorOpCode(TR::vsplats, vt))) {
    ...
    // If we get BadIlOP during transformation, we can either return half transformed trees or set BadIlOp. Half
    // transformed trees are harder to debug in codegen and can happen due to multiple reasons. So for easy of
    // debugging, we go ahead and set BadIlOp.
    if (isCheckMode && vectorOpCode == TR::BadILOp) {
    ...
    if (scalarOp.isLoadVar()) {
        if (isCheckMode) {
            if (!scalarOp.isLoadIndirect()) {
                ...
                if (loadInductionVar) {
                    ...
                    bool platformSupport = comp->cg()->getSupportsOpCodeForAutoSIMD(
                                               TR::ILOpCode::createVectorOpCode(TR::vstore, vectorType))
                        && comp->cg()->getSupportsOpCodeForAutoSIMD(
                            TR::ILOpCode::createVectorOpCode(TR::vsetelem, vectorType))
                        && comp->cg()->getSupportsOpCodeForAutoSIMD(
                            TR::ILOpCode::createVectorOpCode(TR::vadd, vectorType))
                        && comp->cg()->getSupportsOpCodeForAutoSIMD(
                            TR::ILOpCode::createVectorOpCode(TR::vsplats, vectorType));

                    if (trace) {
                        if (platformSupport) {
                            log->printf("   Found use of induction variable at node [%p]\n", node);
                            log->printf(
                                "   Found use of induction variable at node [%p] - vectorization disabled for now\n",
                                node);
                        } else {
                            log->printf("   Found use of induction variable at node [%p] - platform does not support "
                                        "this vectorization\n",
                                node);
                        }
                    }
        ...
        if (!isCheckMode) {
            if (scalarOp.isLoadIndirect()
                && !hasPIV(node, piv)) // SIMD_TODO: strictly speaking should check for stride == 0
            {
                genVectorAccessForScalar(parent, childIndex, node);
                return true;
            }
            ...
            if (!vecSymRef) {
                ...
                vecSymRef = scalarOp.isLoadIndirect()
                    ? comp->getSymRefTab()->findOrCreateArrayShadowSymbolRef(
                          node->getDataType().scalarToVector(TR::VectorLength128), NULL)
                    : vecSymRef = comp->cg()->allocateLocalTemp(
                          node->getDataType().scalarToVector(TR::VectorLength128)); // need to handle alignment?
                pSPMDInfo->addVectorSymRef(symRef, vecSymRef);
            ...
            if (loadInductionVar && createdNewVecSym) {
                ...
                TR::ILOpCodes splatsOpCode
                    = TR::ILOpCode::createVectorOpCode(TR::vsplats, node->getDataType().scalarToVector(VECTOR_LENGTH));
                TR::ILOpCodes setelemOpCode
                    = TR::ILOpCode::createVectorOpCode(TR::vsetelem, node->getDataType().scalarToVector(VECTOR_LENGTH));
                ...
                TR::ILOpCodes vectorAddOpCode = TR::ILOpCode::createVectorOpCode(TR::vadd,
                    TR::DataType::createVectorType(TR::Int32, VECTOR_LENGTH));
                ...
                TR::Node *vstoreNode = TR::Node::createWithSymRef(
                    TR::ILOpCode::createVectorOpCode(TR::vstore, vecSymRef->getSymbol()->getDataType()), 1, 1, vaddNode,
                    vecSymRef);
            ...
            if (loadInductionVar) {
                ...
                if (trace) {
                    log->printf("   Vectorizing PIV use at node [%p]\n", vloadNode);
        ...
        if (scalarOp.isLoadIndirect()) {
            ...
            if (reductionHashTab->locate(node->getSymbolReference(), id)) {
                TR_SPMDReductionInfo *reductionInfo = (TR_SPMDReductionInfo *)reductionHashTab->getData(id);
                // reduction symRefs can only be read from a statement that also stores back to the same symRef (eg. sum
                // = sum + a[i]) this code checks that this condition is being met. If not, the symRef is not a
                // supported reduction symRef
                if (reductionInfo->reductionOp != Reduction_Invalid && node->getSymbolReference() != storeSymRef) {
                    logprintf(trace, log,
                        "   visitNodeToSIMDize: Load symRef does not match store symref at node: %p\n", node);
                    reductionInfo->reductionOp = Reduction_Invalid;
                    ListIterator<TR::Node> nodeIt(&reductionInfo->storeNodes);
                    for (TR::Node *nextNode = nodeIt.getCurrent(); nextNode; nextNode = nodeIt.getNext()) {
                        logprintf(trace, log,
                            "   visitNodeToSIMDize: Adding recorded invalid reduction node to defsInLoop. node: %p\n",
                            nextNode);
                        defsInLoop[nextNode->getGlobalIndex()] = true;
                        collectUses(nextNode, comp, useDefInfo, useNodesOfDefsInLoop);
                    }
                ...
                logprintf(trace, log,
                    "   visitNodeToSIMDize: Never before seen symRef being recorded as not a reduction at node: %p\n",
                    node);
        ...
        for (int32_t i = 0; i < node->getNumChildren(); i++) {
            if (!visitNodeToSIMDize(node, i, node->getChild(i), pSPMDInfo, isCheckMode, loop, comp, usesInLoop,
                    useNodesOfDefsInLoop, useDefInfo, defsInLoop, reductionHashTab, storeSymRef))
                return false;
        }

        if (scalarOp.isAdd() || scalarOp.isSub() || scalarOp.isMul() || scalarOp.isDiv() || scalarOp.isRem()
            || scalarOp.isLeftShift() || scalarOp.isRightShift() || scalarOp.isShiftLogical() || scalarOp.isAnd()
            || scalarOp.isXor() || scalarOp.isOr()) {
            if (isCheckMode)
                return true;
            ...
            while (firstChild->getOpCodeValue() == TR::PassThrough)
                firstChild = firstChild->getFirstChild();
            while (secondChild->getOpCodeValue() == TR::PassThrough)
                secondChild = secondChild->getFirstChild();

            TR_ASSERT(firstChild->getDataType().isVector() && firstChild->getDataType() == secondChild->getDataType(),
                " Both children of the node to be transformed must be vector types %s %s",
                firstChild->getOpCode().getName(), secondChild->getOpCode().getName());
            TR::Node::recreate(node, vectorOpCode);
            return true;
        }

        if (scalarOp.isNeg() || scalarOp.getOpCodeValue() == TR::l2d) {
            if (isCheckMode)
                return true;
            ...
            TR_ASSERT(firstChild->getDataType().isVector(),
                " Child of the node to be transformed must be a vector type");
            TR::Node::recreate(node, vectorOpCode);
            return true;
        }
    }
...
bool TR_SPMDKernelParallelizer::autoSIMDReductionSupported(TR::Compilation *comp, TR::Node *node)
{
    ...
    if (!enableFPAutoSIMDReduction && !_fpreductionAnnotation
        && (node->getDataType() == TR::Float || node->getDataType() == TR::Double)) {
        logprintf(trace, log,
            "   autoSIMDReductionSupported: float and double reduction are not supported right now. node: %p\n", node);
        return false;
    }
...
//-only the reduction operation is used between the store node and the reduction variable load
bool TR_SPMDKernelParallelizer::isReduction(TR::Compilation *comp, TR_RegionStructure *loop, TR::Node *node,
    TR_SPMDReductionInfo *reductionInfo, TR_SPMDReductionOp pathOp)
{
    bool trace = comp->trace(OMR::SPMDKernelParallelization);
    ...
    {
        if (opCode.isAdd()
            || opCode.isSub()) // sub is a special case of add. It only works if the reduction var is on the left
        {
            switch (pathOp) {
        ...
        if (isReduction(comp, loop, firstChild, reductionInfo, pathOp)) {
            if (noReductionVar(comp, loop, secondChild, reductionInfo))
                return (reductionInfo->reductionOp != Reduction_Invalid); // if noReductionVar sets Reduction_Invalid we
                                                                          // want to still return false here
            else
                return false;
        } else if (!opCode.isSub()
            && isReduction(comp, loop, secondChild, reductionInfo,
                pathOp)) // sub is a special case of add. It only works if the reduction var is on the left
        {
...
bool TR_SPMDKernelParallelizer::noReductionVar(TR::Compilation *comp, TR_RegionStructure *loop, TR::Node *node,
    TR_SPMDReductionInfo *reductionInfo)
{
    bool trace = comp->trace(OMR::SPMDKernelParallelization);
    ...
    if (opCode.isLoadVar()) {
        if (opCode.isLoadDirect() && node->getSymbolReference() == reductionInfo->reductionSymRef) {
            ...
            return true;
    } else if (opCode.isAdd() || opCode.isSub() || opCode.isMul() || opCode.isDiv()
        || opCode.isRem()) // TODO: add max, min and bitwise operations here
    {
        TR::Node *firstChild = node->getFirstChild();
        TR::Node *secondChild = node->getSecondChild();
...
bool TR_SPMDKernelParallelizer::reductionLoopEntranceProcessing(TR::Compilation *comp, TR_RegionStructure *loop,
    TR::SymbolReference *symRef, TR::SymbolReference *vecSymRef, TR_SPMDReductionOp reductionOp)
{
    bool trace = comp->trace(OMR::SPMDKernelParallelization);
    OMR::Logger *log = comp->log();
    ...
    if (!(reductionOp == Reduction_Add || reductionOp == Reduction_Mul)) {
        logprints(trace, log,
            "   reductionLoopEntranceProcessing: Invalid or unknown reductionOp during transformation phase.\n");
    ...
    TR::ILOpCodes splatsOpCode
        = TR::ILOpCode::createVectorOpCode(TR::vsplats, scalarDataType.scalarToVector(VECTOR_LENGTH));
    ...
    switch (scalarDataType) {
        ...
        default:
            logprints(trace, log,
                "   reductionLoopEntranceProcessing: Unknown vector data type during transformation phase.\n");
    ...
    TR::Node *vstoreNode = TR::Node::create(insertionPoint->getNode(),
        TR::ILOpCode::createVectorOpCode(TR::vstore, vecSymRef->getSymbol()->getDataType()), 1);
...
bool TR_SPMDKernelParallelizer::reductionLoopExitProcessing(TR::Compilation *comp, TR_RegionStructure *loop,
    TR::SymbolReference *symRef, TR::SymbolReference *vecSymRef, TR_SPMDReductionOp reductionOp)
{
    bool trace = comp->trace(OMR::SPMDKernelParallelization);
    OMR::Logger *log = comp->log();
    ...
    if (!(reductionOp == Reduction_Add || reductionOp == Reduction_Mul)) {
        logprints(trace, log,
            "   reductionLoopExitProcessing: Invalid or unknown reductionOp during transformation phase.\n");
        TR_ASSERT(0, "Invalid or unknown reductionOp during transformation phase");
        return false;
    }
    ...
    switch (reductionOp) {
        ...
        default:
            logprints(trace, log,
                "   reductionLoopExitProcessing: Invalid or unknown reductionOp during transformation phase (2).\n");
            TR_ASSERT(0, "Invalid or unknown reductionOp during transformation phase (2)");
            return false;
    }
    ...
    switch (scalarDataType) {
        ...
        default:
            logprints(trace, log,
                "   reductionLoopExitProcessing: Unknown vector data type during transformation phase.\n");
            TR_ASSERT(0, "Unknown vector data type during transformation phase.");
            return false;
            break;
    }
    ...
    TR_BitVector *blocksInsideLoop
        = new (comp->trStackMemory()) TR_BitVector(cfg->getNextNodeNumber(), comp->trMemory(), stackAlloc);
    ...
    for (edge = ei.getCurrent(); edge; edge = ei.getNext()) {
        ...
        TR::Node *loadVectorNode
            = TR::Node::create(insertionPoint->getNode(), TR::ILOpCode::createVectorOpCode(TR::vload, vectorType), 0);
...
void TR_SPMDKernelParallelizer::replaceAndAnchorOldNode(TR::Compilation *comp, TR::TreeTop *treeTop, TR::Node *parent,
    TR::Node *oldNode, TR::Node *newNode, int index)
{
...
inline bool TR_SPMDKernelParallelizer::matchIVIncrementPattern(TR::Node *node, TR::SymbolReference *pivSymRef)
{
    return (node->getOpCode().isAdd() || node->getOpCode().isSub()) && node->getFirstChild()->getOpCode().isLoad()
        && node->getFirstChild()->getSymbolReference() == pivSymRef
...
bool TR_SPMDKernelParallelizer::processSPMDKernelLoopForSIMDize(TR::Compilation *comp, TR::Optimizer *optimizer,
    TR_RegionStructure *loop, TR_PrimaryInductionVariable *piv, TR_HashTab *reductionHashTab, int32_t peelCount,
    TR::Block *invariantBlock)
{
    OMR::Logger *log = comp->log();
    ...
    TR_LoopUnroller unroller(comp, optimizer, loop, piv, TR_LoopUnroller::SPMDKernel, unrollCount - 1, peelCount,
        invariantBlock, vectorSize);
    ...
    if (!performTransformation(comp, "%s Simdizing loop %d  branch block = %d inc = %d piv = %d\n", OPT_SIMD_DETAILS,
            loop->getNumber(), branchBlock->getNumber(), unroller._piv->getDeltaOnBackEdge(),
            unroller._piv->getSymRef()->getReferenceNumber())) // method->nameChars())
        return false;
    ...
    logprintf(trace(), log,
        "   Checking Loop ==== (iter = %d, unroll = %d, vectorSize = %d) and unroller._spillLoopRequired %d \n", iters,
        unrollCount, vectorSize, unroller._spillLoopRequired);
   ...
   unroller._optimizer->setEnableOptimization(inductionVariableAnalysis, true, NULL);
        ...
        for (TR::Block *nextBlock = blocksIt1.getCurrent(); nextBlock; nextBlock = blocksIt1.getNext()) {
            for (TR::TreeTop *tt = nextBlock->getEntry(); tt != nextBlock->getExit(); tt = tt->getNextTreeTop()) {
                ...
                if (curNode->getOpCode().isBooleanCompare() && curNode->getOpCode().isBranch()) {
                    TR_HashId hashOne = 0;
                    TR_HashId hashTwo = 0;
                    if (entries->locate(curNode->getFirstChild(), hashOne)
                        || entries->locate(curNode->getSecondChild(), hashTwo)) {
                        int nodeIndex = entries->locate(curNode->getFirstChild(), hashOne) ? 0 : 1;
                        TR::Node *newNode = nodeIndex == 0 ? reinterpret_cast<TR::Node *>(entries->getData(hashOne))
                                                           : reinterpret_cast<TR::Node *>(entries->getData(hashTwo));
                    ...
                    } else if (curNode->getFirstChild()->getReferenceCount() > 1
                        || curNode->getSecondChild()->getReferenceCount() > 1) {
                        TR::Node *oldNode = NULL;
                        TR::Node *newNode = NULL;
                        int32_t childIndex = 0;
                        if (curNode->getFirstChild()->getReferenceCount() > 1
                            && matchIVIncrementPattern(curNode->getFirstChild(), unroller._piv->getSymRef())) {
                            oldNode = curNode->getFirstChild();
                            childIndex = 0;
                        } else if (curNode->getSecondChild()->getReferenceCount() > 1
                            && matchIVIncrementPattern(curNode->getSecondChild(), unroller._piv->getSymRef())) {
                        ...
                        if (oldNode) {
                            ...
                            // So, we need to adjust the stride by vectorSize and store the mapping {oldNode, newNode}
                            // in the hashTable.
                ...
                if (curNode->getOpCodeValue() == TR::istore
                    && curNode->getSymbolReference() == unroller._piv->getSymRef()) {
                    // increment of PIV
                    logprintf(trace(), log,
                        "Reducing the number of iterations of the loop %d at storeNode [%p] by vector length %d \n",
                        loop->getNumber(), curNode, unrollCount);
                    TR_ASSERT_FATAL(curNode->getFirstChild()->getOpCode().isAdd()
                            || curNode->getFirstChild()->getOpCode().isSub(),
                        "PIV increment should be simple (either by add or by sub");
                    TR_ASSERT_FATAL(curNode->getFirstChild()->getFirstChild()->getOpCode().isLoad(),
                        "PIV increment should have load");
                    TR_ASSERT_FATAL(curNode->getFirstChild()->getSecondChild()->getOpCode().isLoadConst(),
                        "PIV increment should have const increment value");
                    TR::Node *newNode = NULL;
                    TR::Node *oldNode = NULL;
                    if (curNode->getFirstChild()->getReferenceCount() > 1) {
                        // We need to uncommon the parent node only (isub/iadd), but keep the commoned children (if it
                        // has any such child). Uncommoning of parent node prevents the propagation of loop stride to
                        // other parts in the block. For example, if the parent commoned node was used in address
                        // calculation, then changing the stride will introduce bug as the address calculation will be
                        // affected. Keeping the commoned children prevents unwanted side effect. For example, commoned
                        // iload should stay the same, even if isub/iadd is uncommoned. That way we know it will use the
                        // old iload value, not load the value again. Const node will be uncommoned before changing the
                        // stride to match with vector operations, so we don't need to uncommon it here.
                        oldNode = curNode->getFirstChild();
                        TR_HashId hashOne = 0;
    ...
    for (TR::Block *nextBlock = blocksIt.getCurrent(); nextBlock; nextBlock = blocksIt.getNext()) {
        for (TR::TreeTop *tt = nextBlock->getEntry(); tt != nextBlock->getExit(); tt = tt->getNextTreeTop()) {
            visitTreeTopToSIMDize(tt, pSPMDInfo, false, loop, useNodesOfDefsInLoop, comp, NULL, defsInLoop, NULL,
                reductionHashTab);
        }
    }
...
int32_t getArrayElementSize(TR::Compilation *comp, TR::SymbolReference *symRef)
{
    ...
    char *signature
        = symRef->getOwningMethod(comp)->classSignatureOfFieldOrStatic(symRef->getCPIndex(), signatureLength);

    if (signature && signature[0] == '[') {
        if (comp->trace(OMR::SPMDKernelParallelization))
            comp->log()->printf("signature %.*s\n", signatureLength, signature);
...
void TR_SPMDKernelParallelizer::convertIntoParm(TR::Node *node, int32_t elementSize,
...
void TR_SPMDKernelParallelizer::reportRejected(const char *msg1, const char *msg2, int32_t lineNumber, TR::Node *node)
{
    ...
    log->printf(msg1, node);

    if (msg2) {
        log->printf(msg2, comp()->signature(), lineNumber, comp()->getLineNumber(node));
        log->println();
    }
...
bool TR_SPMDKernelParallelizer::visitNodeToMapSymbols(TR::Node *node, ListAppender<TR::ParameterSymbol> &parms,
    ListAppender<TR::AutomaticSymbol> &autos, TR_RegionStructure *loop, TR_PrimaryInductionVariable *piv,
    int32_t lineNumber, int32_t visitCount)
{
    ...
    if (opcode.isCall() && node->getSymbolReference()->isUnresolved()) {
        if (trace())
            reportRejected("Stop processing since node %p is unresolved call\n",
                "Rejected forEach in %s at line %d: contains unresolved call", lineNumber, node);
        return false;
    }
    ...
    if (opcode.isLoadVarOrStore()) {
        ...
        if (hostSymRef->getSymbol()->isAuto() && hostSymRef != piv->getSymRef()) {
            if (loop->isExprInvariant(node)) {
                    ...
                    if (elementSize < 0) {
                        if (trace())
                            reportRejected("Stop processing since auto symref node %p is not a supported array\n",
                                "Rejected forEach in %s at line %d: could not transform", lineNumber, node);
                        return false;
                    }
                }
...
bool TR_SPMDKernelParallelizer::visitNodeToDetectArrayAccesses(TR::Node *node, TR_RegionStructure *loop,
    TR_PrimaryInductionVariable *piv, int32_t visitCount, int32_t lineNumber, bool &arrayNotFound, int trace,
    bool postDominates)
{
    if (node->getVisitCount() == visitCount)
        return true;

    bool doTrace = this->trace();
    OMR::Logger *log = comp()->log();
    ...
    if ((opcode.isLoadVarOrStore() && opcode.isIndirect()
        ...
        || node->getOpCodeValue() == TR::arraylength || node->getOpCodeValue() == TR::arraycopy) {
        ...
        for (int childNum = childrenNodeOffset; childNum < numberAddrChildren + childrenNodeOffset; ++childNum) {
            ...
            if ((addrNode->getOpCodeValue() == TR::aload) || (addrNode->getOpCodeValue() == TR::aloadi)) {
                ...
                if (!hostSymRef || parmSlot == -1) {
                    ...
                    if (tempNode && (tempNode->getOpCodeValue() == TR::astore)
                        && (tempNode->getFirstChild()->getOpCodeValue() == TR::aloadi)) {
                    ...
                    if (opcode.isLoadVar() || opcode.getOpCodeValue() == TR::arraylength
                        ...
                        if (!disableDataTransferElimination) {
                                ...
                                && gpuSymbolMap[nc]._rhsAddrExpr != INVALID_ADDR) // single read from that array
                            {
                                ...
                                if (doTrace) {
                                    log->printf("RHS node %p has stride %d with regards to #%d, isAffine=%s\n",
                                        addrExpr, pivStride, piv->getSymRef()->getReferenceNumber(),
                                        affine ? "T" : "F");
                                    log->printf("gpuSymbolMap[%d]._elementSize=%d\n", nc,
                                        gpuSymbolMap[nc]._elementSize);
                                }
                        ...
                        if (!disableDataTransferElimination || !comp()->getOptions()->getEnableGPU(TR_EnableSafeMT)) {
                            if (gpuSymbolMap[nc]._lhsAddrExpr == NULL && gpuSymbolMap[nc]._lhsAddrExpr != INVALID_ADDR
                                && postDominates) // single unconditional write to that array
                            {
                                ...
                                bool affine = isAffineAccess(comp(), addrExpr, loop, piv->getSymRef(), pivStride);
                                if (doTrace) {
                                    log->printf("LHS node %p has stride %d with regards to #%d, isAffine=%s\n",
                                        addrExpr, pivStride, piv->getSymRef()->getReferenceNumber(),
                                        affine ? "T" : "F");
                                    log->printf("gpuSymbolMap[%d]._elementSize=%d\n", nc,
                                        gpuSymbolMap[nc]._elementSize);
                                ...
                                if (pivStride == gpuSymbolMap[nc]._elementSize) {
                                    if (comp()->getOptions()->getEnableGPU(TR_EnableSafeMT)) {
                                        if (trace > 1)
                                            TR_VerboseLog::writeLine(TR_Vlog_GPU,
                                                "Detected contiguous store in %s at line %d", comp()->signature(),
                                                comp()->getLineNumber(node));
                                    }
                                    gpuSymbolMap[nc]._lhsAddrExpr = addrExpr;

                                    logprintf(doTrace, log,
                                        "Detected contiguous store %p in LHS for gpuSymbolMap[%d]\n", addrExpr, nc);
                                } else {
                                    gpuSymbolMap[nc]._lhsAddrExpr = INVALID_ADDR;
                                    if (!comp()->getOptions()->getEnableGPU(TR_EnableSafeMT)) {
                                        if (trace > 0)
                                            TR_VerboseLog::writeLine(TR_Vlog_GPU,
                                                "Rejected forEach in %s at line %d: non-contiguous store to array at "
                                                "line %d",
                                                comp()->signature(), lineNumber, comp()->getLineNumber(node));
                                        return false;
                                    }
                                }
                            } else {
                                ...
                                if (!comp()->getOptions()->getEnableGPU(TR_EnableSafeMT)) {
                                    if (trace > 0)
                                        TR_VerboseLog::writeLine(TR_Vlog_GPU,
                                            "Rejected forEach in %s at line %d: multiple or conditional stores to "
                                            "array at line %d",
                                            comp()->signature(), lineNumber, comp()->getLineNumber(node));
            ...
            if (!foundArray) {
                ...
                if (opcode.isStore()) {
                    if (trace > 0)
                        TR_VerboseLog::writeLine(TR_Vlog_GPU,
                            "Rejected forEach in %s at line %d: ambiguous store to array at line %d",
                            comp()->signature(), lineNumber, comp()->getLineNumber(node));
    ...
    for (int32_t i = 0; i < node->getNumChildren(); i++) {
        if (!visitNodeToDetectArrayAccesses(node->getChild(i), loop, piv, visitCount, lineNumber, arrayNotFound, trace,
                postDominates))
            return false;
    }
...
{
    TR::Block *newBlock
        = TR::Block::createEmptyBlock(prevBlock->getEntry()->getNode(), comp, prevBlock->getFrequency(), prevBlock);
...
void TR_SPMDKernelParallelizer::generateGPUParmsBlock(TR::SymbolReference *allocSymRef, TR::Block *populateParmsBlock,
    TR::Node *firstNode)
{
    CS2::ArrayOf<gpuMapElement, TR::Allocator> &gpuSymbolMap = comp()->cg()->_gpuSymbolMap;
    ...
    for (nc.SetToFirst(); nc.Valid(); nc.SetToNext()) {
        ...
        if (!hostSymRef->getSymbol()->getType().isAddress()) {
            ...
            parmLocationNode->setAndIncChild(0,
                TR::Node::createWithSymRef(firstNode, addressLoadOpCode, 0,
                    allocSymRef)); // base address of array to store GPU parameters in
            parmLocationNode->setAndIncChild(1,
                TR::Node::create(firstNode, TR::lconst, 0, parmSlot * sizeof(void *))); // offset into the array
            ...
            populateParmsNode->setAndIncChild(1,
                TR::Node::createWithSymRef(firstNode, TR::loadaddr, 0, hostSymRef)); // GPU parameter to store
        ...
        populateParmsNode->setAndIncChild(0, parmLocationNode); // address to store GPU parameter in
        populateParmsNode->setAndIncChild(1,
            TR::Node::createWithSymRef(firstNode, TR::loadaddr, 0, devSymRef)); // GPU parameter to store
        populateParmsBlock->append(TR::TreeTop::create(comp(), populateParmsNode));
    }
}

void TR_SPMDKernelParallelizer::insertGPUEstimate(TR::Node *firstNode, TR::Block *estimateGPUBlock,
    TR::SymbolReference *lambdaCost, TR::SymbolReference *dataCost, TR_PrimaryInductionVariable *piv,
    TR::TreeTop *loopTestTree, TR::Block *origLoopBlock, TR::Block *loopInvariantBlock,
    TR::SymbolReference *scopeSymRef)
{
    ...
    for (nc.SetToFirst(); nc.Valid(); nc.SetToNext()) {
        ...
        TR::Node *cmpNode = TR::Node::createif(TR::ifacmpeq, TR::Node::createLoad(hostSymRef),
            TR::Node::createAddressNode(firstNode, TR::aconst, 0), prevNullCheckBlock->getEntry());
    ...
    // startPC
    estimateGPUNode->setAndIncChild(2,
        TR::Node::createWithSymRef(estimateGPUNode, TR::loadaddr, 0,
            comp()->getSymRefTab()->findOrCreateStartPCSymbolRef()));
    ...
    TR::Node *cmpNode = TR::Node::createif(TR::ificmpne, estimateGPUNode, TR::Node::create(firstNode, TR::iconst, 0, 0),
        origLoopBlock->getEntry());

    estimateGPUBlock->append(TR::TreeTop::create(comp(), cmpNode));
}

void TR_SPMDKernelParallelizer::insertGPUParmsAllocate(TR::Node *firstNode, TR::Block *allocateParmsBlock,
    TR::SymbolReference *allocSymRef, TR::SymbolReference *scopeSymRef)
{
    TR::SymbolReference *helper;
    ...
    TR::TreeTop *allocateParmsTreeTop
        = TR::TreeTop::create(comp(), TR::Node::create(TR::treetop, 1, allocateParmsNode));
...
void TR_SPMDKernelParallelizer::insertGPUInvalidateSequence(TR::Node *firstNode, TR::Block *invalidateGPUBlock,
    TR::SymbolReference *scopeSymRef)
{
    CS2::ArrayOf<gpuMapElement, TR::Allocator> &gpuSymbolMap = comp()->cg()->_gpuSymbolMap;
    CS2::ArrayOf<gpuMapElement, TR::Allocator>::Cursor nc(gpuSymbolMap);
...
void TR_SPMDKernelParallelizer::insertGPUErrorHandler(TR::Node *firstNode, TR::Block *errorHandleBlock,
    TR::SymbolReference *scopeSymRef, TR::Block *recoveryBlock)
{
    TR::SymbolReference *helper;
    TR::ILOpCodes addressLoadOpCode = TR::lload;
    TR::ILOpCodes addressCallOpCode = TR::lcall;
    ...
    // startPC
    getStateGPUNode->setAndIncChild(1,
        TR::Node::createWithSymRef(firstNode, TR::loadaddr, 0, comp()->getSymRefTab()->findOrCreateStartPCSymbolRef()));
    ...
    TR::Node *cmpNode = TR::Node::createif(TR::ificmpne, getStateGPUNode,
...
void TR_SPMDKernelParallelizer::insertGPUCopyFromSequence(TR::Node *firstNode, TR::Block *copyFromGPUBlock,
    TR::SymbolReference *scopeSymRef, TR::SymbolReference *launchSymRef, TR_PrimaryInductionVariable *piv)
{
    CS2::ArrayOf<gpuMapElement, TR::Allocator> &gpuSymbolMap = comp()->cg()->_gpuSymbolMap;
    CS2::ArrayOf<gpuMapElement, TR::Allocator>::Cursor nc(gpuSymbolMap);
    TR::ILOpCodes addressLoadOpCode = TR::lload;
    ...
    for (nc.SetToFirst(); nc.Valid(); nc.SetToNext()) {
        ...
        TR::TreeTop *copyFromGPUTreeTop
            = TR::TreeTop::create(comp(), TR::Node::create(TR::treetop, 1, copyFromGPUNode));
...
void TR_SPMDKernelParallelizer::insertGPUCopyToSequence(TR::Node *firstNode, TR::Block *copyToGPUBlock,
    TR::SymbolReference *scopeSymRef, TR_PrimaryInductionVariable *piv)
{
    CS2::ArrayOf<gpuMapElement, TR::Allocator> &gpuSymbolMap = comp()->cg()->_gpuSymbolMap;
    CS2::ArrayOf<gpuMapElement, TR::Allocator>::Cursor nc(gpuSymbolMap);
    TR::ILOpCodes addressCallOpCode = TR::lcall;
    ...
    TR::SymbolReference *startAddressHtoD
        = comp()->getSymRefTab()->createTemporary(comp()->getMethodSymbol(), TR::Address);
    TR::SymbolReference *endAddressHtoD
        = comp()->getSymRefTab()->createTemporary(comp()->getMethodSymbol(), TR::Address);

    TR::SymbolReference *startAddressDtoH
        = comp()->getSymRefTab()->createTemporary(comp()->getMethodSymbol(), TR::Address);
    TR::SymbolReference *endAddressDtoH
        = comp()->getSymRefTab()->createTemporary(comp()->getMethodSymbol(), TR::Address);
    ...
    for (nc.SetToFirst(); nc.Valid(); nc.SetToNext()) {
        ...
        if (useLoopBoundsHtoD || useLoopBoundsDtoH) {
            savePiv = comp()->getSymRefTab()->createTemporary(comp()->getMethodSymbol(),
                piv->getSymRef()->getSymbol()->getDataType());
            copyToGPUBlock->append(
                TR::TreeTop::create(comp(), TR::Node::createStore(savePiv, TR::Node::createLoad(piv->getSymRef()))));
        }
        ...
        if (useLoopBoundsHtoD || useLoopBoundsDtoH) {
            copyToGPUBlock->append(
                TR::TreeTop::create(comp(), TR::Node::createStore(piv->getSymRef(), TR::Node::createLoad(savePiv))));
        }
        ...
        // accessMode
        copyToGPUNode->setAndIncChild(3,
            TR::Node::create(firstNode, TR::iconst, 0,
                gpuSymbolMap[nc]._accessKind & TR::CodeGenerator::ReadWriteAccesses));

        // isNoCopyHtoD
        int isNoCopyHtoD
            = ((gpuSymbolMap[nc]._accessKind & TR::CodeGenerator::ReadAccess) == 0
...
void TR_SPMDKernelParallelizer::insertGPUKernelLaunch(TR::SymbolReference *allocSymRef,
    TR::SymbolReference *scopeSymRef, TR::Block *launchBlock, TR::Node *firstNode, TR_PrimaryInductionVariable *piv,
    TR::TreeTop *loopTestTree, int kernelID, bool hasExceptionChecks)
{
    TR::ILOpCodes addressLoadOpCode = TR::lload;
    TR::SymbolReference *helper;
    ...
    // startPC
    launchGPUKernelNode->setAndIncChild(6,
        TR::Node::createWithSymRef(firstNode, TR::loadaddr, 0, comp()->getSymRefTab()->findOrCreateStartPCSymbolRef()));

    // hasExceptionChecks
    launchGPUKernelNode->setAndIncChild(7, TR::Node::create(firstNode, TR::iconst, 0, hasExceptionChecks));

    TR::TreeTop *launchGPUKernelTreeTop
        = TR::TreeTop::create(comp(), TR::Node::create(TR::treetop, 1, launchGPUKernelNode));

    launchBlock->append(launchGPUKernelTreeTop);
}

void TR_SPMDKernelParallelizer::insertGPURegionEntry(TR::Block *loopInvariantBlock, TR::SymbolReference *scopeSymRef,
    int gpuPtxCount, TR_SPMDScopeInfoType scopeType)
{
    TR::CFG *cfg = comp()->getFlowGraph();
    ...
    // startPC
    regionEntryGPUNode->setAndIncChild(2,
        TR::Node::createWithSymRef(insertionPoint->getNode(), TR::loadaddr, 0,
            comp()->getSymRefTab()->findOrCreateStartPCSymbolRef()));

    switch (scopeType) {
        case scopeSingleKernel:
            regionEntryGPUNode->setAndIncChild(3,
                TR::Node::create(insertionPoint->getNode(), TR::iconst, 0, TR::CodeGenerator::singleKernelScope));
            break;
        case scopeNaturalLoop:
            regionEntryGPUNode->setAndIncChild(3,
                TR::Node::create(insertionPoint->getNode(), TR::iconst, 0, TR::CodeGenerator::naturalLoopScope));
            break;
    }

    regionEntryGPUNode->setAndIncChild(4,
        TR::Node::create(insertionPoint->getNode(), TR::iconst, 0,
            comp()->getOptions()->getEnableGPU(TR_EnableGPUForce)));
...
void TR_SPMDKernelParallelizer::insertFlushGPU(TR_BitVector *flushGPUBlocks, TR::Block **cfgBlocks,
    TR::SymbolReference *scopeSymRef)
{
...
TR::Node *TR_SPMDKernelParallelizer::insertFlushGPU(TR::Block *flushGPUBlock, TR::SymbolReference *scopeSymRef)
{
    ...
    // *cudaInfo
    flushGPUNode->setAndIncChild(0,
        TR::Node::createWithSymRef(insertionPoint->getNode(), addressLoadOpCode, 0, scopeSymRef));
    // blockId
    flushGPUNode->setAndIncChild(1,
        TR::Node::create(insertionPoint->getNode(), TR::iconst, 0, flushGPUBlock->getNumber()));
...
////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
void TR_SPMDKernelParallelizer::insertGPUTemporariesLivenessCode(List<TR::TreeTop> *exitPointsList,
    TR::SymbolReference *liveSymRef, bool firstKernel)
{
    CS2::ArrayOf<gpuMapElement, TR::Allocator> &gpuSymbolMap = comp()->cg()->_gpuSymbolMap;
    CS2::ArrayOf<gpuMapElement, TR::Allocator>::Cursor nc(gpuSymbolMap);
    ...
    for (TR::TreeTop *insertionPoint = lit.getFirst(); insertionPoint; insertionPoint = lit.getNext()) {
        ...
        if (firstKernel) {
            ...
            currentNode = TR::Node::create(TR::a2l, 1,
                TR::Node::createWithSymRef(insertionPoint->getNode(), TR::loadaddr, 0, liveSymRef));
        }
        ...
        for (nc.SetToFirst(); nc.Valid(); nc.SetToNext()) {
            ...
            currentNode = TR::Node::create(TR::ladd, 2,
                TR::Node::create(TR::a2l, 1,
                    TR::Node::createWithSymRef(insertionPoint->getNode(), TR::loadaddr, 0, hostSymRef)),
                currentNode);
            ...
            currentNode = TR::Node::create(TR::ladd, 2,
                TR::Node::create(TR::a2l, 1,
                    TR::Node::createWithSymRef(insertionPoint->getNode(), TR::loadaddr, 0, devSymRef)),
                currentNode);
        }
...
void TR_SPMDKernelParallelizer::insertGPURegionExits(List<TR::Block> *exitBlocks, TR::SymbolReference *scopeSymRef,
    int gpuPtxCount, TR::SymbolReference *liveSymRef, List<TR::TreeTop> *exitPointsList)
{
    TR::CFG *cfg = comp()->getFlowGraph();
    ...
    for (exitBlock = si.getCurrent(); exitBlock; exitBlock = si.getNext()) {
        ...
        // *cudaInfo
        regionExitGPUNode->setAndIncChild(0,
            TR::Node::createWithSymRef(insertionPoint->getNode(), addressLoadOpCode, 0, scopeSymRef));

        // startPC
        regionExitGPUNode->setAndIncChild(1,
            TR::Node::createWithSymRef(insertionPoint->getNode(), TR::loadaddr, 0,
                comp()->getSymRefTab()->findOrCreateStartPCSymbolRef()));
        ...
        // **liveSymRef
        regionExitGPUNode->setAndIncChild(3,
            TR::Node::createWithSymRef(insertionPoint->getNode(), TR::loadaddr, 0, liveSymRef));
...
void TR_SPMDKernelParallelizer::insertGPURegionExitInRegionExits(List<TR::Block> *exitBlocks,
    List<TR::Block> *blocksInRegion, TR::SymbolReference *scopeSymRef, TR::SymbolReference *liveSymRef,
    List<TR::TreeTop> *exitPointsList)
{
    List<TR::CFGEdge> exitEdges(comp()->trMemory());
    ...
    int gpuPtxId = comp()->getGPUPtxCount()
        - 1; // gpuPtxId starts at 0 so we need to subtract 1 to get the id from the current count
...
bool TR_SPMDKernelParallelizer::addRegionCost(TR_RegionStructure *region, TR_RegionStructure *loop,
    TR::Block *estimateGPUBlock, TR::SymbolReference *lambdaCost)
{
    ...
    if (region != loop && region->isNaturalLoop() && piv && piv->getIncrement() == 1 && piv->getEntryValue()
        && piv->getExitBound() &&
        // since these nodes are not used in the loop, we need to recalculate their invariance
        loop->isExprInvariant(piv->getEntryValue(), false) && loop->isExprInvariant(piv->getExitBound(), false)) {
        TR::Node *addNode;

        logprintf(trace(), comp()->log(), "adding cost of loop %d with piv %p entry %p exit %p %d %d\n",
            region->getNumber(), piv, piv->getEntryValue(), piv->getExitBound(),
            loop->isExprInvariant(piv->getEntryValue(), false), loop->isExprInvariant(piv->getExitBound(), false));
...
bool TR_SPMDKernelParallelizer::estimateGPUCost(TR_RegionStructure *loop, TR::Block *estimateBlock,
    TR::SymbolReference *lambdaCost)
{
...
bool TR_SPMDKernelParallelizer::processGPULoop(TR_RegionStructure *loop, TR_SPMDScopeInfo *gpuScope)
{
    OMR::Logger *log = comp()->log();
    _verboseTrace = 0;
    if (comp()->getOptions()->getEnableGPU(TR_EnableGPUVerbose))
        _verboseTrace = 1;
    ...
    if (gpuScope->getScopeType() == scopeNaturalLoop) {
        ...
        if (!findLoopInvariantBlock(comp(), parentLoop))
            return false; // TODO: create preheader
    }
    ...
    if (trace()) {
        ...
        if (gpuScope->getScopeType() == scopeNaturalLoop) {
            log->prints("GPU Scope: natural loop\n");
        } else {
            log->prints("GPU Scope: single kernel\n");
        }
    }
    ...
    while (predBlock->getPredecessors().size() == 1) {
        ...
        if ((node->getOpCodeValue() == TR::ificmpeq || node->getOpCodeValue() == TR::ificmpne)
            && node->getFirstChild()->getOpCode().isLoadDirect()
            && node->getFirstChild()->getSymbolReference()->getSymbol()->isStatic()
            && node->getSecondChild()->getOpCode().isLoadConst() && node->getSecondChild()->getInt() == 0) {
            ...
            if (strcmp("java/util/stream/IntPipeline.ibmTryGPU Z", sig) != 0)
                continue;

            ibmTryGPUBlock = predBlock;
            lambdaCPUBlock
                = ((node->getOpCodeValue() == TR::ificmpeq) != _reversedBranchNodes->isSet(node->getGlobalIndex()))
                ? node->getBranchDestination()->getNode()->getBlock()
                : predBlock->getNextBlock();
            break;
        }
    }
    ...
    if (trace()) {
        log->printf("ibmTryGPU block %d\n", ibmTryGPUBlock->getNumber());
        log->printf("lambdaCPU block %d\n", lambdaCPUBlock->getNumber());
        log->printf("loopInvariantBlock %d\n", loopInvariantBlock->getNumber());
    }
   ...
   while (predBlock->getPredecessors().size() == 1)
    ...
    for (TR::TreeTop *tree = comp()->getStartTree(); tree != comp()->findLastTree(); tree = tree->getNextTreeTop()) {
        ...
        if (!visitNodeToDetectArrayAccesses(tree->getNode(), loop, piv, visitCount, lineNumber, arrayNotFound,
                _verboseTrace, postDominates)
    ...
    if (!comp()->getOptions()->getEnableGPU(TR_EnableGPUForce)) {
        ...
        if (trace()) {
            log->printf("lambdaCost #%d\n", lambdaCost->getReferenceNumber());
            log->printf("dataCost #%d\n", dataCost->getReferenceNumber());
        }

        if (!estimateGPUCost(loop, estimateGPUBlock, lambdaCost)) {
            logprints(trace(), log, "lambda is too small\n");
            if (_verboseTrace > 1)
                TR_VerboseLog::writeLine(TR_Vlog_GPU, "\tforEach in %s at line %d does not have any loops",
                    comp()->signature(), lineNumber);
    ...
    TR::CodeGenerator::GPUResult gpuResult = comp()->cg()->dumpNVVMIR(comp()->getStartTree(), comp()->findLastTree(),
        loop, &blocksInLoop, &autos, &parms, true, programSource, errorNode, gpuPtxCount, &hasExceptionChecks);

    if (gpuResult != TR::CodeGenerator::GPUSuccess)
        return false;

    char *generatePTX(int tracing, const char *programSource, int deviceId, TR::PersistentInfo *persistentInfo,
        TR_Memory *m, bool enableMath);

    bool enableMath = comp()->getOptions()->getEnableGPU(TR_EnableGPUEnableMath);
    int deviceId = 0;
    ptxSource = generatePTX(_verboseTrace, programSource, deviceId, comp()->getPersistentInfo(), comp()->trMemory(),
        enableMath);
    ...
    *kernelLineNumber = lineNumber;
    ...
    switch (gpuScope->getScopeType()) {
        case scopeSingleKernel:
            ...
            insertGPURegionEntry(hoistRegionInvariantBlock, gpuScope->_scopeSymRef, gpuPtxCount,
                gpuScope->getScopeType());
            insertGPURegionExits(&exitBlocks, gpuScope->_scopeSymRef, gpuPtxCount, liveSymRef,
                gpuScope->getExitLocationsList());
        ...
        case scopeNaturalLoop:
            if (!gpuScope->_envelopeLoopTransformed) {
                ...
                gpuScope->_scopeSymRef
                    = comp()->getSymRefTab()->createTemporary(comp()->getMethodSymbol(), addressType);
                scopeSymRef = gpuScope->_scopeSymRef;
                ...
                insertGPURegionExitInRegionExits(&envelopeExitBlocks, &blocksInRegion, scopeSymRef, liveSymRef,
                    gpuScope->getExitLocationsList());
                insertGPUTemporariesLivenessCode(gpuScope->getExitLocationsList(), liveSymRef, true);
    ...
    insertGPUKernelLaunch(allocSymRef, scopeSymRef, launchBlock, firstNode, piv, loopTestTree, gpuPtxCount,
        hasExceptionChecks);
    ...
    if (!comp()->getOptions()->getEnableGPU(TR_EnableGPUForce)) {
        insertGPUEstimate(firstNode, estimateBlock, lambdaCost, dataCost, piv, loopTestTree, origLoopBlock,
            loopInvariantBlock, scopeSymRef);
    }
...
TR_SPMDKernelParallelizer::TR_SPMDKernelParallelizer(TR::OptimizationManager *manager)
    ...
    , _pivList(comp()->allocator(), NULL)
    , _visitedNodes(comp()->getNodeCount(), comp()->trMemory(), heapAlloc, growable)
{
    _loopDataType
        = new (trStackMemory()) TR_HashTab(comp()->trMemory(), stackAlloc, comp()->getFlowGraph()->getNextNodeNumber());
...
int32_t TR_SPMDKernelParallelizer::perform()
{
    ...
    _fpreductionAnnotation
        = comp()->getOption(TR_EnableFpreductionAnnotation) && currentMethodHasFpreductionAnnotation(comp(), trace());
    ...
    if ((!comp()->getOption(TR_DisableAutoSIMD) && comp()->cg()->getSupportsAutoSIMD())
...
const char *TR_SPMDKernelParallelizer::optDetailString() const throw() { return "O^O SPMD KERNEL PARALLELIZER: "; }

void TR_SPMDKernelParallelizer::collectGPUKernels(TR_RegionStructure *region, List<TR_RegionStructure> &gpuKernels)
{
...
bool TR_SPMDKernelParallelizer::visitCPUNode(TR::Node *node, int32_t visitCount, TR::Block *block,
    TR_BitVector *flushGPUBlocks)
{
    if (node->getVisitCount() == visitCount)
        return true;
    ...
    if ((opcode.isLoadVarOrStore() && opcode.isIndirect()
            && node->getSymbolReference()->getSymbol()->isArrayShadowSymbol())
        || node->getOpCodeValue() == TR::arraycopy || opcode.isCall()) {
        logprintf(trace(), log, "Found %s in non-cold CPU node %p\n", opcode.isCall() ? "a call" : "array access",
            node);
        ...
        while (method) {
            if (method->getRecognizedMethod() == TR::java_util_stream_IntPipeline_forEach
                || method->getRecognizedMethod() == TR::java_util_stream_IntPipelineHead_forEach)
                break;
...
void TR_SPMDKernelParallelizer::calculateNonColdCPUBlocks(TR_RegionStructure *region,
    TR_ScratchList<TR::Block> *kernelBlocks, TR_ScratchList<TR::Block> *coldLoopBlocks,
...
void TR_SPMDKernelParallelizer::collectColdLoops(TR_RegionStructure *region, List<TR_RegionStructure> &coldLoops)
{
    if (isParallelForEachLoop(region, comp()))
        return;

    if (region->isNaturalLoop() && region->getEntryBlock()->isCold()) {
...
bool TR_SPMDKernelParallelizer::analyzeGPUScope(TR_SPMDScopeInfo *pScopeInfo)
{
    OMR::Logger *log = comp()->log();
...
void TR_SPMDKernelParallelizer::collectGPUScopes(TR_RegionStructure *region, List<TR_RegionStructure> &gpuKernels,
    List<TR_SPMDScopeInfo> &gpuScopes)
{
    ...
    if (getGpuDeviceCount(comp()->getPersistentInfo(), traceLevel) == 0)
        return;
#endif
    ...
    if (region->isNaturalLoop() && !comp()->getOptions()->getEnableGPU(TR_EnableGPUDisableTransferHoist)) {
        ...
        if (gpuKernels.getSize() > 0) {
            TR_SPMDScopeInfo *pScopeInfo
                = new (comp()->trStackMemory()) TR_SPMDScopeInfo(comp(), region, scopeNaturalLoop);
    ...
    if (!isRegionGPUScope)
        if (isParallelForEachLoop(region, comp())) {
            TR_SPMDScopeInfo *pScopeInfo
                = new (comp()->trStackMemory()) TR_SPMDScopeInfo(comp(), region, scopeSingleKernel);
            gpuScopes.add(pScopeInfo);
            if (traceLevel == 2)
                TR_VerboseLog::writeLine(TR_Vlog_GPU, "Found GPU scope %d in %s (single kernel type)",
                    region->getNumber(), comp()->signature());
        }
...
{
    if (region->isNaturalLoop() && region->getPrimaryInductionVariable()) {
        ...
        if (method->getRecognizedMethod() == com_ibm_simt_SPMDKernel_execute) {
            logprintf(trace(), comp->log(),
                "identified SPMD kernel execution loop in %s block = %d stride = %d symbol = %d\n", comp->signature(),
                branchBlock->getNumber(), piv->getDeltaOnBackEdge(), piv->getSymRef()->getReferenceNumber());
            return true;
        }
#endif
    }
...
bool TR_SPMDKernelParallelizer::checkDataLocality(TR_RegionStructure *loop,
    CS2::ArrayOf<TR::Node *, TR::Allocator> &useNodesOfDefsInLoop, SharedSparseBitVector &defsInLoop,
    TR::Compilation *comp, TR_UseDefInfo *useDefInfo, TR_HashTab *reductionHashTab)
{
    OMR::Logger *log = comp->log();

    logprintf(trace(), log, "Checking data locality in loop %d piv = %d\n", loop->getNumber(),
        loop->getPrimaryInductionVariable()->getSymRef()->getReferenceNumber());
    ...
    TR_SPMDKernelInfo *pSPMDInfo
        = new (comp->trStackMemory()) TR_SPMDKernelInfo(comp, loop->getPrimaryInductionVariable());
    ...
    for (TR::Block *nextBlock = blocksIt.getCurrent(); nextBlock; nextBlock = blocksIt.getNext()) {
        for (TR::TreeTop *tt = nextBlock->getEntry(); tt != nextBlock->getExit(); tt = tt->getNextTreeTop()) {
            if (!visitTreeTopToSIMDize(tt, pSPMDInfo, true, loop, useNodesOfDefsInLoop, comp, useDefInfo, defsInLoop,
                    &usesInLoop, reductionHashTab))
                return false;
        }
    }
...
bool TR_SPMDKernelParallelizer::checkLoopIteration(TR_RegionStructure *loop, TR::Compilation *comp)
{
    ...
    for (TR::TreeTop *tt = branchBlock->getEntry(); tt != branchBlock->getExit(); tt = tt->getNextTreeTop()) {
        ...
        if (storeNode->getOpCodeValue() == TR::istore && storeNode->getSymbolReference() == piv->getSymRef()
            && piv->getIncrement() == 1 && storeNode->getFirstChild()->getOpCodeValue() == TR::isub
            ...
            if (indexChild->getOpCode().isLoadVarDirect() && indexChild->getSymbolReference() == piv->getSymRef()
                && incrementNode->getOpCode().isLoadConst() && incrementNode->getInt() == -1)
                goodLoopBounds = true;
        }
    }
...
void TR_SPMDKernelParallelizer::collectDefsAndUses(TR::Node *node, vcount_t visitCount,
    CS2::ArrayOf<TR::Node *, TR::Allocator> &defs, CS2::ArrayOf<TR::Node *, TR::Allocator> &uses, TR::Compilation *comp)
{
    if (node->getVisitCount() >= visitCount)
        return;
    node->setVisitCount(visitCount);

    if (node->getOpCode().isLikeDef() && !node->getOpCode().isStoreDirect() && node->getOpCodeValue() != TR::asynccheck)
        defs[defs.NumberOfElements()] = node;
    else if (node->getOpCode().isLikeUse() && !node->getOpCode().isLoadDirect())
...
int TR_SPMDKernelParallelizer::symbolicEvaluateTree(TR::Node *node)
{
    ...
    switch (node->getNumChildren()) {
        case 0:
            if (node->getOpCodeValue() == TR::lconst || node->getOpCodeValue() == TR::iconst)
                return node->getInt();
            return 0; // else
        case 1:
...
TR::Node *TR_SPMDKernelParallelizer::findSingleLoopVariant(TR::Node *node, TR_RegionStructure *loop, int *sign,
    int *constantsOnly)
{
    TR::Node *node1 = NULL;
    TR::Node *node2 = NULL;
    if (node->getNumChildren() >= 1)
        node1 = findSingleLoopVariant(node->getFirstChild(), loop, sign, constantsOnly);
    ...
    if (*constantsOnly == 1
        || (node->getNumChildren() == 0 && node->getOpCodeValue() != TR::lconst && node->getOpCodeValue() != TR::iconst
            && loop->isExprInvariant(node))) {
        *constantsOnly = 1;
    }

    if (node->getNumChildren() == 0 && !loop->isExprInvariant(node))
        return node;
    if (node->getNumChildren() == 0 && loop->isExprInvariant(node))
        return NULL;
...
bool TR_SPMDKernelParallelizer::checkConstantDistanceDependence(TR_RegionStructure *loop, TR::Node *node1,
    TR::Node *node2, TR::Compilation *comp, int type)
{
    ...
    if (!(loop->isExprInvariant(node1->getFirstChild()->getFirstChild())
            ...
            && areNodesEquivalent(comp, node1->getFirstChild()->getFirstChild(),
                node2->getFirstChild()->getFirstChild()))) {
        return false;
    }
    ...
    if (!areNodesEquivalent(comp, invariantNodedefs, invariantNodeuses) && sign1 == sign2) {
        return false;
    }
    ...
    if (constantsOnly1 != 0 | constantsOnly2 != 0) {
        ...
        int distance = (s1 - s2);
        logprintf(trace(), comp->log(),
            "SPMD DEPENDENCE ANALYSIS: def %p, %s %p. Constant distance dependence of %d bytes\n", node1,
            type == 0 ? "use" : "def", node2, distance);

        if (type == 0 && (distance >= VECTOR_SIZE || distance <= 0)) { // permissable flow dependence
            return true;
        }
        if (type == 1 && (distance >= 0 || distance <= -VECTOR_SIZE)) { // permissable output dependence
            return true;
        }
        return false;
    }
...
bool TR_SPMDKernelParallelizer::checkIndependence(TR_RegionStructure *loop, TR_UseDefInfo *useDefInfo,
    CS2::ArrayOf<TR::Node *, TR::Allocator> &useNodesOfDefsInLoop, SharedSparseBitVector &defsInLoop,
    TR::Compilation *comp)
{
    OMR::Logger *log = comp->log();
    ...
    for (dc.SetToFirst(); dc.Valid(); dc.SetToNext()) {
        ...
        for (dc2.SetTo(dc + 1); dc2.Valid(); dc2.SetToNext()) {
            if (defs[dc2]->getOpCode().hasSymbolReference()
                ...
                if (trace()) {
                    log->printf("SPMD DEPENDENCE ANALYSIS: def %p and def %p are dependent\n", defs[dc], defs[dc2]);
                    log->prints("SPMD DEPENDENCE ANALYSIS: will not vectorize\n");
                }
        ...
        for (uc.SetToFirst(); uc.Valid(); uc.SetToNext()) {
            if (uses[uc]->getOpCode().hasSymbolReference()
                ...
                if (trace()) {
                    log->printf("SPMD DEPENDENCE ANALYSIS: def %p and use %p are dependent\n", defs[dc], uses[uc]);
                    log->prints("SPMD DEPENDENCE ANALYSIS: will not vectorize\n");
                }
                return false;
            }
    ...
    if (!defsOfDefsInLoop.IsZero()) {
        logprints(trace(), log,
            "Both a definition from inside the loop and outside the loop reaches a temp used inside the loop.\n");
        return false;
    }
...
void TR_SPMDKernelParallelizer::collectParallelLoops(TR_RegionStructure *region, List<TR_RegionStructure> &simdLoops,
    TR_HashTab *reductionOperationsHashTab, TR_UseDefInfo *useDefInfo)
{
    if (region->getEntryBlock()->isCold())
        return;
    ...
    if (isSPMDKernelLoop(region, comp())
        || (!comp()->getOption(TR_DisableAutoSIMD) && comp()->cg()->getSupportsAutoSIMD()
...
bool TR_SPMDKernelParallelizer::vectorize(TR::Compilation *comp, TR_RegionStructure *loop,
    TR_PrimaryInductionVariable *piv, TR_HashTab *reductionHashTab, int32_t peelCount, TR::Optimizer *optimizer)
{
    OMR::Logger *log = comp->log();
    ...
    bool vl
        = processSPMDKernelLoopForSIMDize(comp, optimizer, loop, piv, reductionHashTab, peelCount, loopInvariantBlock);
    return vl;
}
