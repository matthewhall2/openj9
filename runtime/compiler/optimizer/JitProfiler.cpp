...
int32_t TR_JitProfiler::perform()
{
    if (!comp()->getOptions()->isAnySamplingJProfilingOptionSet()) {
        logprints(trace(), comp()->log(), "JIT Profiling disabled, returning\n");
        return 0;
    }
        printf("performing jit prof\n");
    logprintf(trace(), comp()->log(), "Processing method: %s\n", comp()->signature());
...
const char *TR_JitProfiler::optDetailString() const throw() { return "O^O SAMPLING JPROFILER: "; }
...
TR::Block *TR_JitProfiler::appendBranchTree(TR::Node *profilingNode, TR::Block *currentBlock)
{
    ...
    TR::Node *branchNode = TR::Node::createif(TR::ificmpne, jitProfile, offNode, ifBlock->getEntry());
    ...
    logprintf(trace(), comp()->log(), "Inserted Profiling Bypass Branch %p for node %p\n", branchNode, profilingNode);
    ...
    int32_t bufferSizeRequired)
{
    ...
    TR::Node *cursorNode = TR::Node::createWithSymRef(profilingNode, TR::aload, 0,
        getSymRefTab()->findOrCreateProfilingBufferCursorSymbolRef());
    ...
    TR::Node *vmThread = TR::Node::createWithSymRef(profilingNode, TR::loadaddr, 0,
    ...
    logprintf(trace(), comp()->log(),
        "Added buffer condition to block_%d, added call block_%d, and added empty profiling block_%d\n",
...
void TR_JitProfiler::addBranchProfiling(TR::Node *branchNode, TR::TreeTop *tt, TR::Block *currentBlock,
    ProfilingBlocksBypassBranchFlag addBypassBranch)
{
    TR_ASSERT(branchNode->getOpCode().isIf(),
        "Attempting to add branch profiling trees to something other than a branch.\n");
    ...
    TR::Block *profilingBlock
        = createProfilingBlocks(branchNode, ifBlock, TR::Compiler->om.sizeofReferenceAddress() + sizeof(U_8));
    ...
    TR_JitProfiler::ProfileBlockPair blockPair
        = blockCreator.addConditionTree(branchNode->getOpCodeValue(), firstChildLoadNode, secondChildLoadNode);

    // Store one on truth side...
    ProfileBlockCreator trueCreator(this, blockPair.trueBlock, branchNode->getBranchDestination()->getEnclosingBlock(),
        branchNode, TR::Compiler->om.sizeofReferenceAddress());
    ...
    ProfileBlockCreator falseCreator(this, blockPair.falseBlock, fallThrough, branchNode,
    ...
    logprintf(trace(), comp()->log(), "Populated block_%d to profile branch node [%p]\n", profilingBlock->getNumber(),
        branchNode);
...
void TR_JitProfiler::addInstanceProfiling(TR::Node *instanceNode, TR::TreeTop *tt, TR::Block *currentBlock,
    ProfilingBlocksBypassBranchFlag addBypassBranch)
{
...
void TR_JitProfiler::addCallProfiling(TR::Node *callNode, TR::TreeTop *tt, TR::Block *currentBlock,
    ProfilingBlocksBypassBranchFlag addBypassBranch)
{
    TR_ASSERT(callNode->getOpCode().isCall(),
        "Attempting to add indirect call profiling trees to something other than an indirect call.\n");
    ...
    TR::Block *profilingBlock = createProfilingBlocks(callNode, ifBlock,
        isInvokeVirtualOrInterface(*byteCode) ? 4 * TR::Compiler->om.sizeofReferenceAddress()
    ...
    if (isInvokeVirtualOrInterface(*byteCode)) {
        ...
        // Record callee
        TR::Node *calleeNode = TR::Node::aconst(callNode, 0x0);
        /* calleeSymbol->getResolvedMethod()->classOfMethod()*/ // TODO fix it for CSI
    ...
    logprintf(trace(), comp()->log(), "Populated block_%d to profile call [%p]\n", profilingBlock->getNumber(),
        callNode);

    return;
}

TR_JitProfiler::ProfileBlockCreator::ProfileBlockCreator(TR_JitProfiler *jp, TR::Block *profilingBlock,
    TR::Block *successorBlock, TR::Node *profilingNode, int32_t currentOffset)
{
    _jitProfiler = jp;
    _profilingBlock = profilingBlock;
    _successorBlock = successorBlock;
    _profilingNode = profilingNode;
    _cursor = TR::Node::createWithSymRef(profilingNode, TR::aload, 0,
        _jitProfiler->getSymRefTab()->findOrCreateProfilingBufferCursorSymbolRef());
...
TR_JitProfiler::ProfileBlockCreator::~ProfileBlockCreator()
{
    if (!_createdConditional) {
        if (_currentOffset != 0) {
            ...
            TR::Node *storeNode = TR::Node::createWithSymRef(TR::astore, 1, 1, addNode,
                _jitProfiler->getSymRefTab()->findOrCreateProfilingBufferCursorSymbolRef());
...
void TR_JitProfiler::ProfileBlockCreator::addProfilingTree(TR::ILOpCodes storeType, TR::Node *storeValue,
    int32_t storeWidth)
{
    ...
    TR::Node *storeNode = TR::Node::createWithSymRef(storeType, 2, 2, storeOffset, storeValue,
        _jitProfiler->getSymRefTab()->findOrCreateProfilingBufferSymbolRef());
    _profilingBlock->append(TR::TreeTop::create(_jitProfiler->comp(), storeNode));

    return;
}

TR_JitProfiler::ProfileBlockPair TR_JitProfiler::ProfileBlockCreator::addConditionTree(TR::ILOpCodes conditionType,
    TR::Node *firstChild, TR::Node *secondChild)
{
    // Create two new blocks
    TR::Block *trueBlock
        = TR::Block::createEmptyBlock(_profilingNode, _jitProfiler->comp(), _profilingBlock->getFrequency() / 2);
    TR::Block *falseBlock
        = TR::Block::createEmptyBlock(_profilingNode, _jitProfiler->comp(), _profilingBlock->getFrequency() / 2);
