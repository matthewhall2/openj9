...
TR::TreeTop *seqLoadSearchAndCombine(TR::Compilation *comp, bool trace, TR_BitVector *visitedNodes,
    TR::TreeTop *currentTree, TR::Node *currentNode, NodeForwardList *combineNodeList);
bool isValidSeqLoadCombine(TR::Compilation *comp, bool trace, TR::Node *combineNode, NodeForwardList *combineNodeList,
    int32_t &combineNodeCount);
...
TR::TreeTop *generateArraycopyFromSequentialLoads(TR::Compilation *comp, bool trace, TR::TreeTop *currentTreeTop,
    TR::Node *rootNode, NodeForwardList *combineNodeList);

class TR_ShiftedValueTree {
public:
    TR_ALLOC(TR_Memory::LoopTransformer)

    TR_ShiftedValueTree(TR::Compilation *c)
        : _comp(c)
        , _rootNode(NULL)
        , _valNode(NULL)
        , _shiftValue(0)
...
public:
    ...
    TR_arraysetSequentialStores(TR::Compilation *comp, TR_AllocationKind heapOrStack = heapAlloc)
        : TR_SequentialStores(comp)
        , _comp(comp)
        , _processedRefs(false)
        , _activeOffset(-1)
        , _baseOffset(-1)
        , _lastOffset(-1)
        , _indexBase(0)
...
TR::Node *TR_arraysetSequentialStores::checkArrayStore(TR::Node *storeNode, bool supportsArraySet)
{
    ...
    if (offset) {
        if (offset->getOpCode().isLoadConst()) {
            ...
            if ((offset->getOpCodeValue() == TR::iadd) || (offset->getOpCodeValue() == TR::isub)
                || (offset->getOpCodeValue() == TR::ladd) || (offset->getOpCodeValue() == TR::lsub)) {
...
int32_t TR_arraysetSequentialStores::getNumBytes() { return _lastOffset - _baseOffset; }

int32_t TR_arraycopySequentialStores::getNumBytes() { return _numBytes; }

int32_t TR_arraysetSequentialStores::getBaseOffset() { return _baseOffset; }

void TR_arraysetSequentialStores::setLastOffset(int offset) { _lastOffset = offset; }

int64_t TR_arraysetSequentialStores::getConstant() { return _initValue; }

TR::Node *TR_SequentialStores::getALoad() { return _activeALoad; }
...
TR::ILOpCodes TR_SequentialStores::getLoadOpCode() { return _loadOpCode; }

TR_SequentialStores::TR_SequentialStores(TR::Compilation *comp, TR_AllocationKind heapOrStack)
    : _comp(comp)
{}

bool TR_arraysetSequentialStores::getProcessedRefs() { return _processedRefs; }

void TR_arraysetSequentialStores::setProcessedRefs() { _processedRefs = true; }

int32_t TR_arraysetSequentialStores::getActiveOffset() { return _activeOffset; }

TR_arraycopySequentialStores::TR_arraycopySequentialStores(TR::Compilation *comp)
    : TR_SequentialStores(comp)
    , _comp(comp)
    , _alternateDir(false)
{
...
int32_t getOffsetForSeqLoadDEPRECATED(TR::Compilation *comp, TR::Node *rootNode, int32_t totalBytes, int32_t byteNumber)
{
    ...
    if (byteNumber == 1) {
        ...
        if (dummyNode->getSecondChild()->getOpCodeValue() == TR::imul) {
            if (comp->target().is64Bit()) {
                return dummyNode->getSecondChild()
                           ->getFirstChild()
                           ->getFirstChild()
                           ->getFirstChild()
                           ->getSecondChild()
...
TR::Node *getBasePointerReferenceForSeqLoadDEPRECATED(TR::Compilation *comp, TR::Node *rootNode, int32_t totalBytes,
    int32_t byteNumber)
{
    TR::Node *dummyNode = rootNode;
    if (byteNumber == 1) {
...
int32_t getMultValueForSeqLoadDEPRECATED(TR::Compilation *comp, TR::Node *rootNode, int32_t totalBytes,
    int32_t byteNumber)
{
    TR::Node *dummyNode = rootNode;
    if (byteNumber == 1) {
    ...
    TR::TreeTop *currentTree, TR::Node *currentNode, NodeForwardList *combineNodeList)
{
     ...
     * combineNodeList keeps tracks of the nodes used to construct one particular value. It is populated by
     * isValidSeqLoadCombine. combineNodeList needs to be cleared each time before it is repopulated.
    ...
    if (isValidSeqLoadCombine(comp, trace, currentNode, combineNodeList, combineNodeCount)) {
        ...
        for (int i = 0; i < currentNode->getNumChildren(); i++) {
            currentTree = seqLoadSearchAndCombine(comp, trace, visitedNodes, currentTree, currentNode->getChild(i),
                combineNodeList);
        }
    }
    ...
    int32_t &combineNodeCount)
{
    ...
    /*
     * If int width combine nodes are being used, no more than 4 bytes can be combined which means no more than 3
     * combine nodes. If long width combine nodes are being used, no more than 8 bytes can be combined which means no
     * more than 7 combine nodes.
     */
    if ((combineNodeCount > 3)
        && ((combineNode->getOpCodeValue() == TR::iadd) || (combineNode->getOpCodeValue() == TR::ior))) {
        return false;
    } else if ((combineNodeCount > 7)
        && ((combineNode->getOpCodeValue() == TR::ladd) || (combineNode->getOpCodeValue() == TR::lor))) {
        return false;
    }
...
bool isValidSeqLoadAnd(TR::Compilation *comp, bool trace, TR::Node *andNode)
{
     ...
     * This node is supposed to act as a mask so that only 8 bits are affected when this is combined via add/or with the
     * other loaded bytes.
     */
...
bool isValidSeqLoadByteConversion(TR::Compilation *comp, bool trace, TR::Node *conversionNode)
{
    ...
    /*
     * conversionNode's child must be a bloadi with a refcount of 1. After the transformation the intermediate value
     * generated by this node won't be calculated. If the value is needed for something else, the transformation needs
     * to bail out here.
     */
    ...
    if (comp->target().is64Bit()) {
        ...
        if (secondChild->getReferenceCount() != 1) {
            ...
            if ((secondChild->getOpCodeValue() != TR::lconst) && (secondChild->getOpCodeValue() != TR::ladd)
        ...
        if (secondChild->getReferenceCount() != 1) {
            ...
            if ((secondChild->getOpCodeValue() != TR::iconst) && (secondChild->getOpCodeValue() != TR::iadd)
...
int64_t getOffsetForSeqLoad(TR::Compilation *comp, TR::Node *byteConversionNode)
{
    ...
    if ((byteConversionNode->getOpCodeValue() != TR::b2i) && (byteConversionNode->getOpCodeValue() != TR::bu2i)
        ...
        TR_ASSERT_FATAL_WITH_NODE(byteConversionNode, 0,
            "Unsupported opCode. This should have been caught earlier. byteConversionNode: %p.", byteConversionNode);
    }
...
TR::Node *getBaseOffsetForSeqLoad(TR::Node *inputNode)
{
    ...
    switch (inputNode->getOpCodeValue()) {
        ...
        case TR::bu2l:
            ...
            if ((displacementNode->getOpCodeValue() == TR::iconst)
                || (displacementNode->getOpCodeValue() == TR::lconst)) {
                baseOffsetNode = NULL;
                break;
            }
            ...
            break;
        default:
            TR_ASSERT_FATAL_WITH_NODE(inputNode, 0,
                "Unsupported opCode. This should have been caught earlier. inputNode: %p.", inputNode);
            break;
    }
...
bool TR_ShiftedValueTree::process(TR::Node *loadNode)
{
    ...
    if (shift) {
        if (loadNode->getFirstChild()->getOpCodeValue() != shiftCode
            && loadNode->getFirstChild()->getOpCodeValue() != altShiftCode) {
            ...
            switch (constCode) {
                case TR::sconst:
                    _shiftValue = shiftVal->getShortInt();
                    break;
                case TR::iconst:
                    _shiftValue = shiftVal->getInt();
                    break;
                case TR::lconst:
                    _shiftValue = shiftVal->getLongInt();
                    break;
                default:
                    break;
            }
...
bool TR_arraycopySequentialStores::insertConsistentTree()
{
    ...
    TR::Node *activeBaseNode = _activeAddrTree->getBaseVarNode()->isNull()
        ? NULL
        : _activeAddrTree->getBaseVarNode()->getChild()->skipConversions();
    ...
    TR::Node *baseNode = _addrTree[0]->getBaseVarNode()->isNull()
        ? NULL
        : _addrTree[0]->getBaseVarNode()->getChild()->skipConversions();
    TR::SymbolReference *baseRef = _addrTree[0]->getBaseVarNode()->isNull()
        ? NULL
        : _addrTree[0]->getBaseVarNode()->getChild()->skipConversions()->getSymbolReference();
    if (baseNode != activeBaseNode) {
        dumpOptDetails(comp(), " insertTree: base variable is different than previous tree\n");
        return false;
    }

    TR::SymbolReference *indRef = _addrTree[0]->getIndVarNode()->isNull()
        ? NULL
        : _addrTree[0]->getIndVarNode()->getChild()->skipConversions()->getSymbolReference();
    TR::SymbolReference *activeIndRef = _activeAddrTree->getIndVarNode()->isNull()
        ? NULL
        : _activeAddrTree->getIndVarNode()->getChild()->skipConversions()->getSymbolReference();
...
void TR_arraycopySequentialStores::removeTrees(TR::SymbolReference *symRef)
{
    ...
    for (int32_t i = 0; i < _numBytes; ++i) {
        dumpOptDetails(_comp, " Remove trees %p to %p\n", _treeTops[i]->getNode(),
            _treeTops[i]->getNextTreeTop()->getNode());
...
int64_t TR_arraycopySequentialStores::constVal()
{
    ...
    for (int entry = 0; entry < _numBytes; ++entry) {
        ...
        switch (_val[entry]->getValNode()->getOpCodeValue()) {
            case TR::bconst:
                curVal = (uint64_t)(_val[entry]->getValNode()->getByte() & 0xFF);
                break;
            case TR::sconst:
                curVal = (uint64_t)(_val[entry]->getValNode()->getShortInt() & 0xFF);
                break;
            case TR::iconst:
                curVal = (uint64_t)(_val[entry]->getValNode()->getInt() & 0xFF);
                break;
            case TR::lconst:
                curVal = (uint64_t)(_val[entry]->getValNode()->getLongInt() & 0xFF);
                break;
            default:
                break;
        }
...
TR::Node *TR_arraycopySequentialStores::constValNode()
{
    ...
    switch (_numBytes) {
        case 1:
            node = TR::Node::bconst(rootNode, (int8_t)constVal());
            break;
        case 2:
            node = TR::Node::sconst(rootNode, (int16_t)constVal());
            break;
        case 4:
            node = TR::Node::create(rootNode, TR::iconst);
            node->setInt((int32_t)constVal());
            break;
        case 8:
            node = TR::Node::create(rootNode, TR::lconst);
            node->setLongInt(constVal());
            break;
        default:
            TR_ASSERT(false, "const val node has invalid number of bytes");
            break;
    }
    return node;
}
...
static TR::TreeTop *generateArraycopyFromSequentialStores(TR::Compilation *comp, TR::TreeTop *prevTreeTop,
    TR::TreeTop *istoreTreeTop, TR::Node *istoreNode)
{
    ...
    while (arraycopy.numTrees() < arraycopy.maxNumTrees() && arraycopy.checkIStore(curNode)
    ...
    if (codeGen->getSupportsAlignedAccessOnly() && ((arraycopy.getAddrTree()->getOffset() % numBytes) != 0)) {
    ...
    switch (numBytes) {
        case 2:
            opcode = TR::sstorei;
            byteswapOpcode = TR::sbyteswap;
            break;
        case 4:
            opcode = TR::istorei;
            byteswapOpcode = TR::ibyteswap;
            break;
        case 8:
            opcode = TR::lstorei;
            byteswapOpcode = TR::lbyteswap;
            break;
        default:
            TR_ASSERT(0, " number of bytes unexpected\n");
            break;
    }
...
static TR::TreeTop *generateArraycopyFromSequentialLoadsDEPRECATED(TR::Compilation *comp, TR::TreeTop *currentTreeTop,
    TR::Node *bloadiNode)
{
    ...
    if (comp->requiresSpineChecks() && bloadiNode->getReferenceCount() > 1) {
        ...
        TR::TreeTop *lastTreeTop
            = currentTreeTop->getEnclosingBlock()->startOfExtendedBlock()->getFirstRealTreeTop()->getPrevTreeTop();

        // Search backwards to the top of the EBB and look at all spine checks.
        while (tt != lastTreeTop) {
            TR::Node *node = tt->getNode();
            if (node->getOpCodeValue() == TR::BNDCHKwithSpineCHK || node->getOpCodeValue() == TR::SpineCHK) {
    ...
    for (int32_t i = 0; i < numBytes; i++) {
        ...
        if (i == 0) {
            ...
            if (!(isValidSeqLoadIMul(comp, currentNode)))
                return currentTreeTop;
            if (!(shiftVar * multiplier
                    == getMultValueForSeqLoadDEPRECATED(comp, rootNode, numBytes,
                        numBytes - i))) // This check is broken for numBytes == 2 since multiplier is 0 instead of 256.
                return currentTreeTop;
            if (!(baseOffset == getOffsetForSeqLoadDEPRECATED(comp, rootNode, numBytes, numBytes - i) + i)
                || !(aloadNode == getALoadReferenceForSeqLoadDEPRECATED(rootNode, numBytes, numBytes - i))
                || !(basePointerNode
                    == getBasePointerReferenceForSeqLoadDEPRECATED(comp, rootNode, numBytes, numBytes - i)))
                return currentTreeTop;
            baseOffset = getOffsetForSeqLoadDEPRECATED(comp, rootNode, numBytes, numBytes - i);
    ...
    TR::Node *rootNode, NodeForwardList *combineNodeList)
{
     ...
     * Big Endian has support as well but the bytes in the original array need to be in the opposite order or a byteswap
     * is inserted.
     *
     * Original trees:
    ...
    /* byteConversionNodes are the conversion node parent for each loaded byte. Sorted such that the least significant
     * byte is in position 0. */
    TR::Node *byteConversionNodes[8];
    /* byteOffsets are the constants that act as relative offsets for each loaded byte. Sorted such that the least
     * significant byte is in position 0. */
    ...
    logprintf(trace, log, "Sequential Load Simplification Candidate - rootNode: %p\n", rootNode);
    ...
    logprintf(trace, log, "Sequential Load Simplification Candidate - rootNode: %p, byteCount: %d\n", rootNode,
        byteCount);

    if (!is64bitResult && (byteCount > 4)) {
        logprintf(trace, log, "byteCount is too high for constructing an int value. rootNode: %p, byteCount: %d\n",
            rootNode, byteCount);
        return currentTreeTop;
    }
    ...
    if ((byteCount < 2) || (byteCount > 8)) {
        logprintf(trace, log,
            "Sequential Load of size other than 2 to 8 is not supported. rootNode: %p, byteCount: %d\n", rootNode,
            byteCount);
        return currentTreeTop;
    }
    ...
    for (int i = 0; i < byteCount; i++) {
        ...
        logprintf(trace, log, "Sequential Load Simplification Candidate - rootNode: %p, processedByteNodes[%d]: %p\n",
            rootNode, i, processedByteNodes[i]);

        if (0 == i) {
            aloadNode = getALoadReferenceForSeqLoad(processedByteNodes[i]);
            logprintf(trace, log, "Sequential Load Simplification Candidate - rootNode: %p, aloadNode: %p\n", rootNode,
                aloadNode);
            baseOffsetNode = getBaseOffsetForSeqLoad(processedByteNodes[i]);
            logprintf(trace, log, "Sequential Load Simplification Candidate - rootNode: %p, baseOffsetNode: %p\n",
            ...
            if (aloadNode != getALoadReferenceForSeqLoad(processedByteNodes[i])) {
                logprintf(trace, log,
                    "Sequential Load Simplification aload mismatch. rootNode: %p, aloadNode(0): %p, aloadNode(%d): "
                    "%p\n",
            ...
            if (baseOffsetNode != getBaseOffsetForSeqLoad(processedByteNodes[i])) {
                logprintf(trace, log,
                    "Sequential Load Simplification baseOffsetNode mismatch. rootNode: %p, baseOffsetNode(0): %p, "
                    "baseOffsetNode(%d): %p\n",
        ...
        logprintf(trace, log, "Sequential Load Simplification Candidate - rootNode: %p, byteConversionNode: %p\n",
            rootNode, byteConversionNode);
        shiftValue = getShiftValueForSeqLoad(processedByteNodes[i]);
        logprintf(trace, log, "Sequential Load Simplification Candidate - rootNode: %p, shiftValue: %d\n", rootNode,
            shiftValue);

        if ((shiftValue > ((byteCount - 1) * 8)) || (shiftValue < 0)) {
            logprintf(trace, log,
                "shiftValue is out of range for the given byteCount. rootNode: %p, byteCount: %d, shiftValue: %d\n",
        ...
        if (((byteCount - 1) * 8) == shiftValue) {
            ...
            logprintf(trace, log, "Sequential Load Simplification Candidate - rootNode: %p, signExtendResult: %d\n",
                rootNode, signExtendResult);
        } else {
            /* Other bytes can not be sign extended. */
            if (checkForSeqLoadSignExtendedByte(processedByteNodes[i])) {
                logprintf(trace, log,
                    "Bytes other than the most significant byte should not be sign extended. rootNode: %p, "
                    "processedByteNodes[%d]: %p\n",
                    rootNode, i, processedByteNodes[i]);
                return currentTreeTop;
            }
        }
        ...
        if (byteConversionNodes[byteNumber] != NULL) {
            logprintf(trace, log,
                "Duplicate shiftValue. rootNode: %p, shiftValue: %d, byteConversionNodes[%d]: %p, "
                "byteConversionNode(%d): %p\n",
    ...
    if (trace) {
        log->printf("Sequential Load Simplification Candidate - rootNode: %p, byteOffsets: %ld", rootNode,
            byteOffsets[0]);
        for (int i = 1; i < byteCount; i++) {
            log->printf(", %ld", byteOffsets[i]);
        }
        log->println();
    }
     ...
     * At this point it is known what the significance of each loaded byte is so check if they can be loaded with an LE
     * or BE load. If both are incompatible, the opt bails out.
     */
    if (matchLittleEndianSeqLoadPattern(byteOffsets, byteCount)) {
        ...
        logprintf(trace, log, "byteOffsets array does not match a big or little endian pattern. rootNode: %p\n",
            rootNode);
        return currentTreeTop;
    }

    logprintf(trace, log, "Sequential Load Simplification Candidate - rootNode: %p, %s\n", rootNode,
        littleEndianLoad ? "Little Endian Load" : "Big Endian Load");
    logprintf(trace, log,
        "Sequential Load Simplification Candidate - rootNode: %p, newConvertChildNode: %p, newLoadChildNode: %p\n",
    ...
    if (littleEndianLoad && !comp->target().cpu.isLittleEndian()) {
        if (!comp->cg()->supportsByteswap()) {
            logprintf(trace, log,
                "Little endian load on big endian target without byteswap is not supported. rootNode: %p\n", rootNode);
            return currentTreeTop;
        }
        swapBytes = true;
        ...
        if (!comp->cg()->supportsByteswap()) {
            logprintf(trace, log,
                "Big endian load on little endian target without byteswap is not supported. rootNode: %p\n", rootNode);
            return currentTreeTop;
        }
        swapBytes = true;
    }
    ...
    logprintf(trace, log, "Sequential Load reduced at rootNode: %p\n", rootNode);
    ...
    if (((4 == byteCount) && !swapBytes && !is64bitResult) || ((8 == byteCount) && !swapBytes)) {
        if (4 == byteCount) {
            /* byteCount is 4, final result is an int, no byteswaps */
            logprintf(trace, log, "Recreating rootNode (%p) as iloadi.\n", rootNode);
    ...
    } else if (8 == byteCount) /* swapBytes must be true here. */
    {
        ...
        logprintf(trace, log, "Recreating rootNode (%p) as lbyteswap.\n", rootNode);
        ...
        logprintf(trace, log, "Recreating newLoadChildNode (%p) as lloadi.\n", newLoadChildNode);
        ...
        if (is64bitResult) {
            ...
            if (signExtendResult) {
                logprintf(trace, log, "Recreating rootNode (%p) as i2l.\n", rootNode);
                TR::Node::recreate(rootNode, TR::i2l);
            } else {
                logprintf(trace, log, "Recreating rootNode (%p) as iu2l.\n", rootNode);
            ...
            if (!swapBytes) {
                ...
                logprintf(trace, log, "Creating ibyteswapNode (%p) with child %p.\n", ibyteswapNode,
                    ibyteswapNode->getFirstChild());
        ...
        {
            /* Inserts byteswap above the load. */
            logprintf(trace, log, "Recreating rootNode (%p) as ibyteswap.\n", rootNode);
        ...
        logprintf(trace, log, "Recreating newLoadChildNode (%p) as iloadi.\n", newLoadChildNode);
        TR::Node::recreateWithSymRef(newLoadChildNode, TR::iloadi, symRef);
    } else if (2 == byteCount) {
        /* The conversion used is based on whether the final result is an int or a long and if there is a sign extend or
         * not. */
        if (signExtendResult) {
            if (is64bitResult) {
                logprintf(trace, log, "Recreating rootNode (%p) as s2l.\n", rootNode);
                TR::Node::recreate(rootNode, TR::s2l);
            } else {
                logprintf(trace, log, "Recreating rootNode (%p) as s2i.\n", rootNode);
                TR::Node::recreate(rootNode, TR::s2i);
            }
        } else {
            if (is64bitResult) {
                logprintf(trace, log, "Recreating rootNode (%p) as su2l.\n", rootNode);
                TR::Node::recreate(rootNode, TR::su2l);
            } else {
                logprintf(trace, log, "Recreating rootNode (%p) as su2i.\n", rootNode);
        ...
        if (!swapBytes) {
            ...
            logprintf(trace, log, "Creating sbyteswapNode (%p) with child %p.\n", sbyteswapNode,
                sbyteswapNode->getFirstChild());

            logprintf(trace, log, "Setting rootNode (%p) child to %p.\n", rootNode, sbyteswapNode);
            rootNode->setAndIncChild(0, sbyteswapNode);
        }

        logprintf(trace, log, "Recreating newLoadChildNode (%p) as sloadi.\n", newLoadChildNode);
        ...
        logprintf(trace, log, "Creating mulNode (%p) with children %p and %p.\n", mulNode, mulNode->getFirstChild(),
            mulNode->getSecondChild());

        logprintf(trace, log, "Setting rootNode (%p) children to %p and %p.\n", rootNode, mulNode,
            byteConversionNodes[0]);
        rootNode->setAndIncChild(0, mulNode);
        rootNode->setAndIncChild(1, byteConversionNodes[0]);

        /* The conversion used is based on whether the final result is an int or a long. The byte is never sign
         * extended.*/
        if (is64bitResult) {
            logprintf(trace, log, "Recreating byteConversionNodes[0] (%p) as bu2l.\n", byteConversionNodes[0]);
            TR::Node::recreate(byteConversionNodes[0], TR::bu2l);
        } else {
            logprintf(trace, log, "Recreating byteConversionNodes[0] (%p) as bu2i.\n", byteConversionNodes[0]);
        ...
        if (signExtendResult) {
            if (is64bitResult) {
                logprintf(trace, log, "Recreating newConvertChildNode (%p) as s2l.\n", newConvertChildNode);
                TR::Node::recreate(newConvertChildNode, TR::s2l);
            } else {
                logprintf(trace, log, "Recreating newConvertChildNode (%p) as s2i.\n", newConvertChildNode);
                TR::Node::recreate(newConvertChildNode, TR::s2i);
            }
        } else {
            if (is64bitResult) {
                logprintf(trace, log, "Recreating newConvertChildNode (%p) as su2l.\n", newConvertChildNode);
                TR::Node::recreate(newConvertChildNode, TR::su2l);
            } else {
                logprintf(trace, log, "Recreating newConvertChildNode (%p) as su2i.\n", newConvertChildNode);
                TR::Node::recreate(newConvertChildNode, TR::su2i);
            }
        }
        ...
        if (swapBytes) {
            ...
            logprintf(trace, log, "Setting newConvertChildNode (%p) child to %p.\n", newConvertChildNode,
                sbyteswapNode);
        ...
        /* The conversion used is based on whether the final result is sign extended or not. */
        if (signExtendResult) {
            logprintf(trace, log, "Recreating newConvertChildNode (%p) as i2l.\n", newConvertChildNode);
            TR::Node::recreate(newConvertChildNode, TR::i2l);
        } else {
            logprintf(trace, log, "Recreating newConvertChildNode (%p) as iu2l.\n", newConvertChildNode);
         ...
         */
        TR::Node *mulNode
            = TR::Node::create(TR::lmul, 2, newConvertChildNode, TR::Node::create(TR::lconst, 0, 256 * 256));
        logprintf(trace, log, "Creating mulNode (%p) with children %p and %p.\n", mulNode, mulNode->getFirstChild(),
            mulNode->getSecondChild());
        ...
        logprintf(trace, log, "Recreating shortLoadNode (%p) as sloadi.\n", shortLoadNode);
        TR::Node::recreateWithSymRef(shortLoadNode, TR::sloadi, shortSymRef);

        /* The short is never sign extended. */
        logprintf(trace, log, "Recreating shortConversionNode (%p) as su2l.\n", shortConversionNode);
        ...
        if (swapBytes) {
            ...
            logprintf(trace, log, "Setting shortConversionNode (%p) child to %p.\n", shortConversionNode,
                sbyteswapNode);
         ...
         */
        TR::Node *mulNode
            = TR::Node::create(TR::lmul, 2, newConvertChildNode, TR::Node::create(TR::lconst, 0, 256 * 256 * 256));
        logprintf(trace, log, "Creating mulNode (%p) with children %p and %p.\n", mulNode, mulNode->getFirstChild(),
            mulNode->getSecondChild());
        ...
        TR::Node *andNode
            = TR::Node::create(TR::land, 2, lowerIntConversionNode, TR::Node::create(TR::lconst, 0, 0xFFFFFF));
        logprintf(trace, log, "Creating andNode (%p) with children %p and %p.\n", andNode, andNode->getFirstChild(),
            andNode->getSecondChild());
        ...
        logprintf(trace, log, "Recreating lowerIntLoadNode (%p) as iloadi.\n", lowerIntLoadNode);
        ...
        logprintf(trace, log, "Recreating lowerIntConversionNode (%p) as i2l.\n", lowerIntConversionNode);
        ...
        if (swapBytes) {
            ...
            logprintf(trace, log, "Creating ibyteswapNode (%p) with child %p.\n", ibyteswapNode1,
                ibyteswapNode1->getFirstChild());

            logprintf(trace, log, "Setting lowerIntConversionNode (%p) child to %p.\n", lowerIntConversionNode,
                ibyteswapNode1);
        ...
        TR_ASSERT_FATAL_WITH_NODE(rootNode, 0,
            "Unexpected code path during generateArraycopyFromSequentialLoads transformation. rootNode: %p.", rootNode);
    }
    ...
    TR::DebugCounter::prependDebugCounter(comp,
        TR::DebugCounter::debugCounterName(comp,
            "sequentialStoreSimplification/generateArraycopyFromSequentialLoads/%s", comp->signature()),
        currentTreeTop);
...
bool TR_arraysetSequentialStores::checkConstant(TR::Node *constExpr)
{
    ...
    switch (constType) {
        ...
        case TR::Int16: {
            ...
            bv = (value & 0xFF);
            if (bv == ((value & 0xFF00) >> 8)) { // most likely case is 0 or -1
                isValidConst = true;
            }
            break;
        }
        ...
        case TR::Float: {
            ...
            if (bv == ((value & 0xFF00) >> 8) && bv == ((value & 0xFF0000) >> 16)
                && bv == ((value & 0xFF000000) >> 24)) { // most likely case is 0 or -1
                isValidConst = true;
            }
            break;
        }
        ...
        case TR::Double: {
            ...
            if (bv == ((value & 0xFF00) >> 8) && bv == ((value & 0xFF0000) >> 16)
                ...
                if (bv == bv2
                    && (bv == ((value & 0xFF00) >> 8) && bv == ((value & 0xFF0000) >> 16)
                        && bv == ((value & 0xFF000000) >> 24))) {
...
static TR::TreeTop *generateArraysetFromSequentialStores(TR::Compilation *comp, TR::TreeTop *prevTreeTop,
    TR::TreeTop *istoreTreeTop, TR::Node *istoreNode, bool *newTempsCreated)
{
    TR::CodeGenerator *codeGen = comp->cg();
    ...
    while (arrayset.checkIStore(curNode) && arrayset.checkStore(curNode)
        && arrayset.checkALoad(curNode->getFirstChild()) && arrayset.checkConstant(curNode->getSecondChild())) {
    ...
    if (!useArraySet(numBytes, codeGen)) {
        ...
        while (arrayset.checkIStore(curNode)
            ...
            && arrayset.checkALoad(baseNode) && arrayset.checkArrayStoreConstant(curNode->getSecondChild())) {
    ...
    if (areSequentialArrayElements && (totalNumOfStores >= 1)) {
        ...
        if (!codeGen->getSupportsArraySet()) {
            if ((curNode->getSize() == 8) || (numBytes == curNode->getSize()))
                return istoreTreeTop;

            TR_ASSERT((numBytes <= 8),
                "number of bytes is greater than 8 and we cannot use a single store to achieve this\n");
            usingArrayset = false;
        } else if (comp->target().cpu.isPower() && numBytes < 32) // tm - The threshold needs to be adjusted for PPC
        {
            ...
            || ((numBytes < 12)
                && comp->target().is64Bit())) // msf - change to codeGen->arrayInitMinimumNumberOfBytes())
        {
            return istoreTreeTop;
        }
        ...
        if (usingArrayset) {
            ...
            // in each byte of the 'value' (arrayset has a byte child). This is not checked by the sequential array
            // store code but it could be improved to allow arrayset to be used in this case as well.
            //
            return istoreTreeTop;
        }
    } else {
    ...
    if (usingArrayset) {
        ...
        while (numBytesLeft > 0) {
            ...
            switch (numBytesToBeStored) {
                ...
                case 8: {
                    ...
                    if (istoreNode->getOpCodeValue() == TR::bstorei) {
                        ...
                        longConstValue = ((longConstValue << 56) | (longConstValue << 48) | (longConstValue << 40)
                            | (longConstValue << 32) | (longConstValue << 24) | (longConstValue << 16)
                            | (longConstValue << 8) | longConstValue);
                    } else if (istoreNode->getOpCodeValue() == TR::sstorei) {
                        uint16_t shortConstValue = (uint16_t)longConstValue;
                        longConstValue = (int64_t)shortConstValue;
                        longConstValue = ((longConstValue << 48) | (longConstValue << 32) | (longConstValue << 16)
                            | longConstValue);
            ...
            if (!disableHoist && comp->getJittedMethodSymbol() && // avoid NULL pointer on non-Wcode builds
                ...
                if (block && block->getStructureOf() && comp->getFlowGraph()->getStructure()) {
                    ...
                    if (containingLoop && (containingLoop->getNumber() == block->getNumber())) {
                        prevBlock = block->getPrevBlock();
                        if (prevBlock) {
                            if (prevBlock->getStructureOf() && prevBlock->getStructureOf()->isLoopInvariantBlock()
                                && (prevBlock->getSuccessors().size() == 1)
                                && (prevBlock->getSuccessors().front()->getTo() == block)) {
                                canHoistConstant = true;
                                for (auto edge = block->getPredecessors().begin();
                                     edge != block->getPredecessors().end(); ++edge) {
            ...
            if (canHoistConstant) {
                TR::SymbolReference *constSymbolReference
                    = comp->getSymRefTab()->createTemporary(comp->getMethodSymbol(), constValueNode->getDataType());
...
TR_ArrayShiftTree::TR_ArrayShiftTree(TR::Compilation *c, TR::TreeTop *tt)
    : _comp(c)
    , _treeTop(tt)
    , _rootNode(tt->getNode())
{
...
bool TR_ArrayShiftTree::process()
{
     ...
     *     offset
     */
    if (_rootNode->getNumChildren() == 2 && _rootNode->getOpCode().isStoreIndirect()
        && _rootNode->getSecondChild()->getOpCode().isLoadIndirect()) {
        return _targetAddress->process(_rootNode->getFirstChild())
            && _sourceAddress->process(_rootNode->getSecondChild()->getFirstChild());
    }
    return false;
}
...
public:
    ...
    TR_ArrayShiftTreeCollection(TR::Compilation *c)
        : _useAliases(false)
        , _comp(c)
        , _numTrees(0)
    {
        memset(_storeTrees, 0, sizeof(_storeTrees));
    }
...
bool TR_ArrayShiftTreeCollection::aliasCheck()
{
    ...
    if (targetArraySymRef && sourceArraySymRef) {
        ...
        if (targetArraySymRef->getSymbol() == _storeTrees[0]->getTargetArrayNode()->getSymbolReference()->getSymbol()
            && sourceArraySymRef->getSymbol()
                == _storeTrees[0]->getSourceArrayNode()->getSymbolReference()->getSymbol()) {
            int32_t loadSymRefNum
                = _storeTrees[_numTrees]->getRootNode()->getSecondChild()->getSymbolReference()->getReferenceNumber();
            TR::SymbolReference *loadDefSymRef
                = _storeTrees[_numTrees]->getRootNode()->getSecondChild()->getSymbolReference();
...
bool TR_ArrayShiftTreeCollection::insertTree(TR::TreeTop *currTree)
{
    ...
    if (_storeTrees[_numTrees]->process()) {
        if (_storeTrees[_numTrees]->getTargetIndexVarNode() == NULL
            || _storeTrees[_numTrees]->getTargetArrayNode() == NULL) {
        ...
        for (int32_t i = 0; i < _numTrees; ++i) {
            if (_storeTrees[i]->getTargetAddress()->getOffset()
                == _storeTrees[_numTrees]->getSourceAddress()->getOffset()) {
        ...
        if (_storeTrees[_numTrees]->getRootNode()->getOpCode().getSize() * (_numTrees + 1) > _maxStoreSize) {
            dumpOptDetails(comp(), "Max store size of %d exceeeded the max [%p]\n",
                (_storeTrees[_numTrees]->getRootNode()->getOpCode().getSize() * (_numTrees + 1)),
                _storeTrees[_numTrees]->getRootNode());
            return false;
        }
        ...
        if (_storeTrees[_numTrees]->getSourceLoadNode()
            ...
            while (!((tmpTT->getNode()->getOpCode().getOpCodeValue() == TR::BBStart)
                ...
                TR::Node *sideEffectNode = (tmpTT->getNode()->getOpCodeValue() == TR::treetop)
                    ? tmpTT->getNode()->getFirstChild()
                    : tmpTT->getNode();
                // Check if killed.
                if (!firstEncounteredKillNode && sideEffectNode->mayKill().containsAny(loadAlias, comp())) {
                ...
                if (firstEncounteredKillNode && tmpTT->getNode()->containsNode(sourceLoad, comp()->incVisitCount())) {
                    dumpOptDetails(comp(), "Store tree[%p]: multi-ref load[%p] was killed by[%p]\n",
                        _storeTrees[_numTrees]->getRootNode(), _storeTrees[_numTrees]->getSourceLoadNode(),
                        firstEncounteredKillNode);
                    return false;
                }
...
bool mayKillInterferenceBetweenNodes(TR::Compilation *comp, TR::Node *argNode1, TR::Node *argNode2,
    vcount_t thisIterationVisitCount, vcount_t newVisitCount, bool trace);

static TR::TreeTop *reduceArrayLoad(TR_ArrayShiftTreeCollection *storeTrees, TR::Compilation *comp,
    TR::TreeTop *oldTreeTop)
{
    ...
    for (size_t i = storeTrees->getNumTrees() - 1; i >= 1; i--) {
        ...
        while (true) {
            ...
            if (ttTo == storeTrees->getTree(j)->getTreeTop()) {
                ...
                if (mayKillInterferenceBetweenNodes(comp, ttMoveMe->getNode(), ttFrom->getNode(), vc1, vc2,
    ...
    if (storeTrees->getNumTrees() > 1) {
        ...
        TR::Node *newStoreNode = TR::Node::createWithSymRef(storeOpCode, 2, 2,
            storeTrees->getTree(0)->getTargetAddress()->getRootNode(), sourceNode, symRef);
...
static bool isViolatingDirectStore(TR::Node *n, TR_ArrayShiftTreeCollection *st)
{
    ...
    return n->getOpCode().isStore()
        && n->getSymbolReference() == st->getTree(0)->getTargetIndexVarNode()->getSymbolReference();
}
...
static bool isViolatingIndirectStore(TR::Node *n, TR_ArrayShiftTreeCollection *st, TR::Compilation *c)
{
    if (n->getOpCode().isStoreIndirect()) {
        ...
        if ((validIndirectStore
                && tempAddrTree.getBaseVarNode()->getParent()->getFirstChild() == st->getTree(0)->getTargetArrayNode())
            || !validIndirectStore) {
            // if we can't parse the iXstore target tree or if the store is to the same array as the one we are trying
            // to reduce treat it as a violating store
            return true;
        }
    }
    return false;
...
static TR::TreeTop *generateArrayshiftFromSequentialStores(TR::Compilation *comp, TR::TreeTop *prevTreeTop,
    TR::TreeTop *istoreTreeTop)
{
...
int32_t TR_SequentialStoreSimplifier::perform()
{
    ...
    /*
     * SupportsAlignedAccessOnly is currently only set for Power 7 and earlier and also LE Power 8. It is not set for
     * Power 9 and onward and also not set for BE Power 8 and non-Power platforms.
     */
    if (!forceSequentialStoreSimplifier && comp()->cg()->getSupportsAlignedAccessOnly()
        && comp()->cg()->supportsInternalPointers())
        return 1;
...

    {
        ...
        /*
         * Combine nodes are the "add" and "or" nodes that construct the longer value out of several loaded bytes.
         * combineNodeList keeps tracks of the nodes used to construct one particular value. It is populated inside the
         * call to seqLoadSearchAndCombine.
         */
        NodeForwardList *combineNodeList
            = new (stackMemoryRegion) NodeForwardList(NodeForwardListAllocator(stackMemoryRegion));
        TR_BitVector *visitedNodes
            = new (stackMemoryRegion) TR_BitVector(comp()->getNodeCount(), trMemory(), stackAlloc, growable);

        while (currentTree) {
            TR::Node *firstNodeInTree = currentTree->getNode();

            if (firstNodeInTree->getOpCode().isStore() && firstNodeInTree->getOpCode().isIndirect()) {
                currentTree = generateArraysetFromSequentialStores(comp(), prevTree, currentTree, firstNodeInTree,
                    &newTempsCreated);
            ...
            /* Set TR_UseOldSeqLoadOpt to use the original version of the opt that combines byte loads into a wider
             * load. */
            static bool useOldSeqLoadOpt = (feGetEnv("TR_UseOldSeqLoadOpt") != NULL);
            if (!useOldSeqLoadOpt) {
                /*
                 * seqLoadSearchAndCombine goes through the trees are tries to find a matching pattern for sequential
                 * byte loads that can be replaced by wider loads.
                 */
                currentTree
                    = seqLoadSearchAndCombine(comp(), trace, visitedNodes, currentTree, currentNode, combineNodeList);
            } else {
                while ((currentNode->getNumChildren() >= 1) && (currentNode->getFirstChild()->getNumChildren() >= 1)) {
                    currentNode = currentNode->getFirstChild();
                    if (currentNode->getOpCodeValue() == TR::bu2i && currentNode->getFirstChild()->getOpCode().isLoad()
                        && currentNode->getFirstChild()->getOpCode().isIndirect()) {
                        currentTree = generateArraycopyFromSequentialLoadsDEPRECATED(comp(), currentTree,
...
const char *TR_SequentialStoreSimplifier::optDetailString() const throw()
{
 ...
 * [0x2A0A5AEC]     pdconst "+00000" @ offset 240 <prec=5 (len=3)> sign=known(clean/preferred/0xc)    <flags:"0x800302"
 * (X==0 X>=0 X<=0 skipCopyOnLoad )/> [0x2A0A5B74]       aiadd   <flags:"0x8000" (internalPtr )/> [0x2A0A5B34] loadaddr
 * #35[0x292009B8]  Static[$CONSTANT_AREA] [0x2A0A5BB4]         iconst 240 <intPrec=3> [0x2A0A5E84]   astore
 * #200[id=306:"Subscr-AddrTemp"][0x2A0854D0]  Auto[<auto slot 83>]
 *                  ==>aiadd at [0x2A0A561C]
 ...
 *                  ==>aiadd at [0x2A0A561C]
 * [0x2A0A6494]   ipdstore #186[id=49:"WS-ATM-ACCT-OVERDRAW-AMOUNT"][0x292017B8]  Shadow[<refined-array-shadow>]
 * <prec=11 (len=6)> [0x2A0A6414]     aiadd   <flags:"0x8000" (internalPtr )/>
 *                    ==>aiadd at [0x2A0A561C]
 * [0x2A0A6454]       iconst 10 <intPrec=2>    <flags:"0x104" (X!=0 X>=0 )/>
 * [0x2A0A62CC]     pdconst "+00000000000" @ offset 808 <prec=11 (len=6)> sign=known(clean/preferred/0xc)
 * <flags:"0x800302" (X==0 X>=0 X<=0 skipCopyOnLoad )/> [0x2A0A635C]       aiadd   <flags:"0x8000" (internalPtr )/>
...
{
    if (!n1->getOpCode().isStore() || !n1->getOpCode().isIndirect() || !n2->getOpCode().isStore()
        || !n2->getOpCode().isIndirect()) {
        // Return true for everything other than indirect stores.
        return true;
    }

    if (n1->getOpCode().hasSymbolReference() && n1->getSymbolReference() && n2->getOpCode().hasSymbolReference()
    ...
    vcount_t thisIterationVisitCount, vcount_t newVisitCount, bool trace)
{
    ...
    if (n1->getOpCodeValue() == TR::treetop)
        n1 = n1->getFirstChild();
    if (n2->getOpCodeValue() == TR::treetop)
        n2 = n2->getFirstChild();
