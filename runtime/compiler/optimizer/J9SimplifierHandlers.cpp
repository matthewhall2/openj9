...
static TR::Node *foldSetSignIntoNode(TR::Node *setSign, bool setSignIsTheChild, TR::Node *other, bool removeSetSign,
    TR::Block *block, TR::Simplifier *s);
static TR::Node *foldAndReplaceDominatedSetSign(TR::Node *setSign, bool setSignIsTheChild, TR::Node *other,
    TR::Block *block, TR::Simplifier *s);
...
{
    if (!node->isNonNegative() && node->getFirstChild()->isNonNegative() && node->getSecondChild()->isNonNegative()
        && performTransformation(s->comp(),
            "%sSet X>=0 flag on %s [" POINTER_PRINTF_FORMAT "] due to both children X>=0\n", s->optDetailString(),
...
{
    // set sign operations are typically cheaper then the corresponding general sign moving operation (eg. zdshrSetSign
    // vs zdshr).

    TR::Node *child = node->getFirstChild();
    TR::ILOpCodes newSetSignOp = TR::ILOpCode::setSignVersionOfOpCode(node->getOpCodeValue());
    if (node->getReferenceCount() == 1 && newSetSignOp != TR::BadILOp && child->getReferenceCount() == 1
        && (child->getOpCodeValue() == TR::zd2pd || child->getOpCodeValue() == TR::pd2zd)
        && child->getFirstChild()->getReferenceCount() == 1 && child->getFirstChild()->getOpCode().isSetSign()) {
        ...
        if (grandChildSetSign->getOpCode().isLoadConst()) {
            ...
            convertedSetSign
                = TR::DataType::convertSignEncoding(grandChild->getDataType(), node->getDataType(), originalSetSign);
            TR_ASSERT(convertedSetSign != TR::DataType::getInvalidSignCode(),
                "could not convert sign encoding of 0x%x from type %s to type %s\n", originalSetSign,
                grandChild->getDataType().toString(), node->getDataType().toString());
        }
        bool removeSetSignGrandChild = grandChild->getOpCodeValue() == TR::pdSetSign;
        if (convertedSetSign != TR::DataType::getInvalidSignCode()
            && performTransformation(s->comp(),
                "%sFold%s %s [" POINTER_PRINTF_FORMAT "] above parent pd2zd [" POINTER_PRINTF_FORMAT
                "] and into grandparent %s [" POINTER_PRINTF_FORMAT "] and create new ",
                s->optDetailString(), removeSetSignGrandChild ? " and remove" : "",
                child->getFirstChild()->getOpCode().getName(), child->getFirstChild(), child,
                node->getOpCode().getName(), node)) {
            ...
            if (newNode) {
                ...
                if (removeSetSignGrandChild) {
                    TR_ASSERT(grandChild->getNumChildren() == 2,
                        "can only remove setSign grandChild node %p with two children and not %d children\n",
...
{
    // if (!conversion->alwaysGeneratesAKnownCleanSign()) return child; // Allow for pd2udsx nodes too even though they
    // could generate a negative unicode zero?

    // Look for a truncating child or grandchild shift under a pd2x. The truncation will likely be cheaper to perform as
    // part of the pd2x so increase the precision of the shift (and an intermediate clean if present) so it no longer
    // truncates.
    TR::Node *child = conversion->getFirstChild();
    if (child->getReferenceCount() == 1) {
        TR::Node *pdclean = NULL;
        TR::Node *shift = NULL;
        if (child->getOpCodeValue() == TR::pdclean && child->getFirstChild()->getReferenceCount() == 1
        ...
        if (shift && conversion->getDecimalPrecision() == shift->getDecimalPrecision()
            && // final precision matches truncated pdshl precision
            (!pdclean
                || pdclean->getDecimalPrecision()
                    >= shift->getDecimalPrecision())) // and there is no intermediate truncation by the pdclean (if
                                                      // there is a pdclean)
        {
            TR::Node *shiftSource = shift->getFirstChild();
            int32_t shiftedPrecision = shiftSource->getDecimalPrecision() + shift->getDecimalAdjust();
            if (shiftedPrecision <= TR::DataType::getMaxPackedDecimalPrecision()
                && // do not create large pdcleans or pdshrs that may not be supported by the code generator
                shiftedPrecision > conversion->getDecimalPrecision()
                && performTransformation(s->comp(),
                    "%sDelaying truncation until %s [" POINTER_PRINTF_FORMAT
                    "] by increasing %s [" POINTER_PRINTF_FORMAT "] precision %d->%d",
                    s->optDetailString(), conversion->getOpCode().getName(), conversion, shift->getOpCode().getName(),
                    shift, shift->getDecimalPrecision(), shiftedPrecision)) {
                if (pdclean) {
                    dumpOptDetails(s->comp(), " and intermediate pdclean [" POINTER_PRINTF_FORMAT "] precision %d->%d",
                        pdclean, pdclean->getDecimalPrecision(), shiftedPrecision);
...
static TR::Node *flipBinaryChildAndUnaryParent(TR::Node *parent, TR::Node *child, TR::ILOpCodes newParentOp,
    TR::Block *block, TR::Simplifier *s)
{
    // Flip the parent/child order and change the new parent's (the old child's) opcode to newParentOp. Return the new
    ...
    //       x
    //    y
    TR_ASSERT(parent->getNumChildren() == 1 || parent->getNumChildren() == 2,
        "parent node %p must have one or two children and not %d children\n", parent, parent->getNumChildren());
    ...
    if (parent->getType().isBCD()) {
        TR_ASSERT(parentPrec != TR::DataType::getInvalidDecimalPrecision(),
            "expecting original parent node to be a BCD type\n"); // otherwise how do we get the precision value?
    ...
    if (parent->getDecimalPrecision() < newChild->getDecimalPrecision()) {
        ...
        dumpOptDetails(s->comp(),
            "%sparent %s [" POINTER_PRINTF_FORMAT "] and newChild %s [" POINTER_PRINTF_FORMAT
            "] precision mismatch (%d < %d) so create a new %s [" POINTER_PRINTF_FORMAT "]\n",
            s->optDetailString(), parent->getOpCode().getName(), parent, origNewChild->getOpCode().getName(),
            origNewChild, parent->getDecimalPrecision(), origNewChild->getDecimalPrecision(),
            newChild->getOpCode().getName(), newChild);
...
{
    if ((node->getOpCodeValue() == TR::pdclean) && (node->getFirstChild()->getOpCodeValue() == TR::pdshl)
        && (!node->getFirstChild()->getFirstChild()->getOpCode().isConversion())
        && // a conversion will likely have to initialize on its own so no benefit to trying to get the clean to do it
        ...
        if (((shiftAmount & 0x1) == 0) && (pdshl->getDecimalPrecision() >= maxShiftedPrecision)
            && performTransformation(s->comp(),
                "%sCreate a new parent pdshl on pdclean [" POINTER_PRINTF_FORMAT
                "] and remove grandchild pdshl [" POINTER_PRINTF_FORMAT "]\n",
                s->optDetailString(), pdclean, pdshl)) {
            // A pdclean can likely initialize and clean in one step so move it lower in the tree to increase the
            // chances that it will perform the initialization Note that this transformation is not done in the
            // pdcleanSimplifier as we do not want to move the pdclean lower when the parent is pdstore because in this
...
TR::Node *zdsleStoreSimplifier(TR::Node *node, TR::Block *block, TR::Simplifier *s)
{
    ...
    // It is not good to remove all widenings for zdsle children because then this store node will have to move the sign
    // code from the the narrowed child and clear (0xF0) the intermediate bytes. It is better to just let the child do
    // the widening so the sign code does not have to be moved.
    //
    ...
    if (valueChild->isSimpleWidening()) {
        valueChild = node->setValueChild(s->replaceNodeWithChild(valueChild, valueChild->getFirstChild(), s->_curTree,
            block, false)); // correctBCDPrecision=false because node is a widening
    }
...
TR::Node *zd2zdsleSimplifier(TR::Node *node, TR::Block *block, TR::Simplifier *s)
{
    ...
    if (child->getOpCode().isSetSign()) {
        TR::Node *newNode
            = foldSetSignIntoNode(child, true /* setSignIsTheChild */, node, true /* removeSetSign */, block, s);
        if (newNode != node)
            return newNode;
    }
...
TR::Node *pd2udSimplifier(TR::Node *node, TR::Block *block, TR::Simplifier *s)
{
    ...
    if (node->getDecimalPrecision() == child->getDecimalPrecision() && child->isSimpleTruncation()
        && performTransformation(s->comp(),
            "%sRemove simple truncating %s [" POINTER_PRINTF_FORMAT "] under pd2ud node %s [" POINTER_PRINTF_FORMAT
            "]\n",
            s->optDetailString(), child->getOpCode().getName(), child, node->getOpCode().getName(), node)) {
        child = node->setChild(0,
            s->replaceNodeWithChild(child, child->getFirstChild(), s->_curTree, block,
                false)); // correctBCDPrecision=false because nodePrec==childPrec
    }
    ...
    if (node->getFirstChild()->getOpCodeValue() == TR::pdclean
        || node->getFirstChild()->getOpCodeValue() == TR::pdSetSign) {
        node->setChild(0,
            s->replaceNodeWithChild(node->getFirstChild(), node->getFirstChild()->getFirstChild(), s->_curTree, block));
    ...
    if (child->getReferenceCount() == 1
        && (child->getOpCode().isSetSign()
            || TR::ILOpCode::setSignVersionOfOpCode(child->getOpCodeValue()) != TR::BadILOp)) {
        ...
        if (child->getOpCode().isSetSign()) {
            ...
            if (oldSetSignValue->getOpCode().isLoadConst()
                && performTransformation(s->comp(),
                    "%sReplace %s [" POINTER_PRINTF_FORMAT "] dominated %s [" POINTER_PRINTF_FORMAT
                    "] with setSignValueNode [" POINTER_PRINTF_FORMAT "] and value (0x%x) with new ",
                    s->optDetailString(), node->getOpCode().getName(), node, child->getOpCode().getName(), child,
                    oldSetSignValue, oldSetSignValue->get32bitIntegralValue())) {
        ...
        } else if (performTransformation(s->comp(),
                       "%sReplace %s [" POINTER_PRINTF_FORMAT "] dominated %s [" POINTER_PRINTF_FORMAT "] with new ",
                       s->optDetailString(), node->getOpCode().getName(), node, child->getOpCode().getName(), child))

        {
            ...
            {
                if (child->getOpCode().isConversion() && child->getFirstChild()->getType().isUnicodeSeparateSign()
                    ...
                    newNode = TR::Node::create(setSignOp, 2, child->getFirstChild(), newSetSignValue);
                    ...
                    TR_ASSERT(false,
                        "unexpected parent/child combination in pd2udSimplifier for newSetSignIndex == 1\n");
                }
            } else if (newSetSignIndex == 2) // child 3
            {
                ...
                //        iconst TR::DataType::getIgnoredSignCode()
                //
                TR_ASSERT(child->getNumChildren() == 2,
                    "expecting two children on the child node (%p) and not %d children\n", child,
                    child->getNumChildren());
                newNode = TR::Node::create(setSignOp, 3, child->getFirstChild(),
            ...
            } else if (newSetSignIndex == 3) // child 4
            {
                ...
                //        iconst TR::DataType::getIgnoredSignCode()
                //
                TR_ASSERT(child->getNumChildren() == 3,
                    "expecting three children on the child node (%p) and not %d children\n", child,
                    child->getNumChildren());
                newNode = TR::Node::create(setSignOp, 4, child->getFirstChild(),
                    child->getSecondChild(), // shiftAmount
            ...
            if (newNode) {
                dumpOptDetails(s->comp(),
                    " %s [" POINTER_PRINTF_FORMAT "] to ignored value (%d) [" POINTER_PRINTF_FORMAT "]\n",
                    newNode->getOpCode().getName(), newNode, TR::DataType::getIgnoredSignCode());
                newNode->incReferenceCount();
                newNode->setDecimalPrecision(child->getDecimalPrecision());
                // All the children up to but not including the setSignValue on newNode were existing nodes and were
                // therefore incremented one extra time each when creating newNode above
...
TR::Node *ud2pdSimplifier(TR::Node *node, TR::Block *block, TR::Simplifier *s)
{
    ...
    if (!node->hasKnownOrAssumedSignCode()) {
        ...
        if (sign != raw_bcd_sign_unknown
            && performTransformation(s->comp(),
                "%sSetting known sign 0x%x on %s [" POINTER_PRINTF_FORMAT "] from alwaysGeneratedSign\n",
...
TR::Node *udsx2pdSimplifier(TR::Node *node, TR::Block *block, TR::Simplifier *s)
{
    ...
    if (decodeConversionOpcode(node->getOpCode(), node->getDataType(), sourceDataType, targetDataType)) {
        TR::ILOpCodes inverseOp = TR::ILOpCode::getProperConversion(targetDataType, sourceDataType,
            false /* !wantZeroExtension */); // inverse conversion to what we are given
...
TR::Node *pd2udslSimplifier(TR::Node *node, TR::Block *block, TR::Simplifier *s)
{
    ...
    if (node->getFirstChild()->getOpCodeValue() == TR::pdSetSign) {
        TR::Node *newNode = foldSetSignIntoNode(node->getFirstChild(), true /* setSignIsTheChild */, node,
            true /* removeSetSign */, block, s);
        if (newNode != node)
            return newNode;
    }
    ...
    if (node->getFirstChild()->getOpCode().isSetSign()) {
        TR::Node *newNode
            = foldAndReplaceDominatedSetSign(node->getFirstChild(), true /* setSignIsTheChild */, node, block, s);
        if (newNode != node)
            return newNode;
    }
...
{
    TR_ASSERT(node->getOpCodeValue() == TR::pd2zdsls || node->getOpCodeValue() == TR::pd2zdsts,
        "pd2zdslsSimplifier only valid for TR::pd2zdsls/TR::pd2zdsts nodes\n");
    s->simplifyChildren(node, block);
    ...
    if (node->getFirstChild()->getOpCodeValue() == TR::zd2pd) {
        TR::Node *grandChild = s->unaryCancelOutWithChild(node, node->getFirstChild(), s->_curTree,
            TR::zd2pd); // unaryCancelOutWithChild takes care of the details of removing the pd2zdsls/zd2pd
        if (grandChild) {
            TR::Node *newNode = TR::Node::create(
                node->getDataType() == TR::ZonedDecimalSignTrailingSeparate ? TR::zd2zdsts : TR::zd2zdsls, 1,
                grandChild);
...
TR::Node *pd2zdslsSetSignSimplifier(TR::Node *node, TR::Block *block, TR::Simplifier *s) { return node; }
...
{
    TR_ASSERT(node->getOpCodeValue() == TR::zd2zdsls || node->getOpCodeValue() == TR::zd2zdsts,
        "zd2zdslsSimplifier only valid for TR::zd2zdsls/TR::zd2zdsts nodes\n");
    s->simplifyChildren(node, block);
...
TR::Node *zd2pdSimplifier(TR::Node *node, TR::Block *block, TR::Simplifier *s)
{
    ...
    if (firstChild->getOpCodeValue() == TR::zdsle2zd && firstChild->getReferenceCount() == 1
        && node->getDecimalPrecision() < firstChild->getDecimalPrecision()
        && performTransformation(s->comp(),
            "%sReduce zdsle2zd child [" POINTER_PRINTF_FORMAT
            "] precision to %d due to truncating zd2pd [" POINTER_PRINTF_FORMAT "]\n",
...
TR::Node *pd2zdSimplifier(TR::Node *node, TR::Block *block, TR::Simplifier *s)
{
    ...
    if (node->getDecimalPrecision() == firstChild->getDecimalPrecision() && firstChild->isSimpleTruncation()
        && performTransformation(s->comp(),
            "%sRemove simple truncating %s [" POINTER_PRINTF_FORMAT "] under pd2zd node %s [" POINTER_PRINTF_FORMAT
            "]\n",
            s->optDetailString(), firstChild->getOpCode().getName(), firstChild, node->getOpCode().getName(), node)) {
        firstChild = node->setChild(0,
            s->replaceNodeWithChild(firstChild, firstChild->getFirstChild(), s->_curTree, block,
                false)); // correctBCDPrecision=false because nodePrec==childPrec
    }
    ...
    if ((child->getReferenceCount() == 1 && node->getReferenceCount() == 1)
        && (child->getOpCodeValue() == TR::zdsle2pd || child->getOpCodeValue() == TR::zdsls2pd
            || child->getOpCodeValue() == TR::zdsts2pd)) {
        if ((node->getDecimalPrecision() == child->getDecimalPrecision())
            && performTransformation(s->comp(),
                "%sFold %s [" POINTER_PRINTF_FORMAT "] into child %s [" POINTER_PRINTF_FORMAT "] and create new\n",
                s->optDetailString(), node->getOpCode().getName(), node, child->getOpCode().getName(), child)) {
            // Transform:
            // pd2zd
            //   zdsls2pdSetSign
            ...
            TR::ILOpCodes childOpValueForLookup = childIsSetSign
                ? TR::ILOpCode::reverseSetSignOpCode(child->getOpCodeValue())
                : child->getOpCodeValue();
            TR::ILOpCode childOpForLookup;
            childOpForLookup.setOpCodeValue(childOpValueForLookup);
            TR::DataType sourceDataType = TR::NoType;
            TR::DataType targetDataType = TR::NoType;
            if (childOpValueForLookup != TR::BadILOp
                && decodeConversionOpcode(childOpForLookup, child->getDataType(), sourceDataType, targetDataType)) {
                TR::ILOpCodes newConvOp = TR::ILOpCode::getProperConversion(sourceDataType, TR::ZonedDecimal,
                ...
                if (newConvOp != TR::BadILOp) {
                    ...
                    if (childIsSetSign) {
                        if (newSetSignOp != TR::BadILOp) {
                            newNode
                                = TR::Node::create(newSetSignOp, 2, child->getFirstChild(), child->getSecondChild());
                            child->getSecondChild()->decReferenceCount();
                        }
                    } else {
...
{
    if (node->getOpCode().isPackedRightShift() && node->getFirstChild()->getOpCode().isPackedLeftShift()
        ...
        if (performTransformation(s->comp(),
                "%sFold rightShift-over-leftShift : %s by %d [0x%p] over %s by %d [0x%p] by setting parent adjust to "
                "%d and removing child\n",
                s->optDetailString(), node->getOpCode().getName(), parentAdjust, node, child->getOpCode().getName(),
                childAdjust, child, combinedAdjust)) {
            ...
            if (!hasAdjust) {
                newOp = resultIsSetSign ? TR::ILOpCode::setSignOpCode(node->getDataType())
            ...
            node = TR::Node::recreate(node,
                newOp); // If necessary, this will resize the node extension to have enough room for all the children.
            node->setChild(0,
                s->replaceNodeWithChild(child, child->getFirstChild(), s->_curTree, block,
                    false)); // correctBCDPrecision=false because truncChecks already done above

            if (hasAdjust) {
                node->setChild(1,
                    s->replaceNode(node->getChild(1), TR::Node::iconst(node, abs(combinedAdjust)), s->_curTree));
            }
            if (resultIsSetSign) {
                ...
                if (shouldSkipReplacingSignChild)
                    ...
                    node->setChild(setSignValueIndex,
                        s->replaceNode(node->getChild(setSignValueIndex), setSignValueNode, s->_curTree));
            }
...
static TR::Node *reduceShiftRightOverShiftRight(TR::Node *node, TR::Block *block, TR::Simplifier *s)
{
    ...
    if (!(firstChild->getOpCodeValue() == TR::pdshr && node->getSecondChild()->getOpCode().isLoadConst()
    ...
    TR_ASSERT(node->getOpCodeValue() == TR::pdshr || node->getOpCodeValue() == TR::pdshrSetSign,
        "reduceShiftRightOverShiftRight should only be called on packed shift right node");
    ...
    bool childIsTruncation
        = firstChild->getDecimalPrecision() < (firstChild->getFirstChild()->getDecimalPrecision() - childShift);
    bool nodeIsWidening = node->getDecimalPrecision() > (firstChild->getDecimalPrecision() - nodeShift);
    bool sourceIsTooLarge
        = firstChild->getFirstChild()->getDecimalPrecision() > TR::DataType::getMaxPackedDecimalPrecision()
        && nodeRound > 0;
    /*
      dumpOptDetails(s->comp(), "\tchildPrec %d, childSrcPrec %d, childShift %d isTrunc=%s (childPrec %s
      (childSrcPrec-childShift))\n",
      ...
      dumpOptDetails(s->comp(), "\tnodePrec %d, nodeSrcPrec %d, nodeShift %d isWiden=%s (nodePrec %s
      (nodeSrcPrec-nodeShift))\n",
                     node->getDecimalPrecision(),firstChild->getDecimalPrecision(),nodeShift,nodeIsWidening?"yes":"no",nodeIsWidening?">":"<=");
      dumpOptDetails(s->comp(),"\tgrandChild %p prec %d
    ...
    */
    ...
    //        pdload p=10     // x
    //        iconst 6        // shift
    // childIsTruncation=true because the shifted precision of x is 10-6 = 4 but the child precision is 3 so 1 digit is
    // being truncated. nodeIsWidening=true because the shifted precision of child is 3-2 = 1 and this less than the
    // node precision of 2 It is illegal to fold these two pdshr nodes into a single pdshr node because the truncated
    // digit must be removed before it is examined by the widening parent node. If node precision was p=1 then it
    // nodeIsWidening=false and it would be legal to fold because the truncated digit would not be examined by the
    // parent node. If the parent node is rounding, we will be using an SRP, which is restricted to 32 bytes. If the
    // parent node is not rounding, there is no length restriction.

    if (performTransformation(s->comp(),
            "%sFold non-truncating child pdshr [" POINTER_PRINTF_FORMAT "] into parent pdshr [" POINTER_PRINTF_FORMAT
            "] by setting nodeShift %d->%d and nodeRound %d->%d\n",
            s->optDetailString(), firstChild, node, nodeShift, nodeShift + childShift, nodeRound, nodeRound)) {
        node->setChild(0,
            s->replaceNodeWithChild(node->getChild(0), node->getChild(0)->getFirstChild(), s->_curTree, block,
                false)); // correctBCDPrecision=false because truncChecks already done above
        node->setChild(1,
            s->replaceNode(node->getChild(1), TR::Node::iconst(node, nodeShift + childShift), s->_curTree));
        // Don't have to replace rounding node. It will always be the same as the parent, because we do not merge if
        // childRound != 0
    }
...
{
    // lastRun note : doing on lastRun to give a better chance for setSigns to be folded into shifts so we can gen a
    // pdclearSetSign here
    //                once the foldSetSign code is updated to handle setSignOnNode opcodes this can be removed
    //                Also, there are no pdclear/pdclearSetSign simplifiers yet so only transform once the tree is
    //                otherwise optimized
    if (node->getOpCode().isPackedLeftShift() && s->getLastRun() && // see note above
        node->getFirstChild()->getOpCode().isPackedRightShift() && node->getFirstChild()->getDecimalRound() == 0
        && !node->hasIntermediateTruncation()) {
        TR::Node *child = node->getFirstChild();
        bool parentIsSetSign = node->getOpCode().isSetSign();
        bool childIsSetSign = child->getOpCode().isSetSign();
        ...
        if (combinedAdjust == 0 && (!parentIsSetSign || node->getSetSignValueNode()->getOpCode().isLoadConst())
            && (!childIsSetSign || child->getSetSignValueNode()->getOpCode().isLoadConst())
            && performTransformation(s->comp(),
                "%sFold leftShift-over-rightShift : %s by %d [0x%p] over %s by %d [0x%p]\n", s->optDetailString(),
                node->getOpCode().getName(), parentAdjust, node, child->getOpCode().getName(), childAdjust, child)) {
            ...
            if (parentIsSetSign || childIsSetSign) {
                ...
                if (rawSign != raw_bcd_sign_unknown) {
                    ...
                    for (int32_t i = 1; i < node->getFirstChild()->getNumChildren();
                         i++) // node->getFirstChild()->getFirstChild() will remain anchored in the new clearNode
                    {
                        s->anchorNode(node->getFirstChild()->getChild(i), s->_curTree);
                    }
                    if (maskAddress) {
                        clearNode = TR::Node::create(TR::pdclearSetSign, 4, node->getFirstChild()->getFirstChild(),
                            TR::Node::iconst(node, leftMostNibbleToClear), TR::Node::iconst(node, digitsToClear),
                            maskAddress);
                    } else {
                        clearNode = TR::Node::create(TR::pdclearSetSign, 3, node->getFirstChild()->getFirstChild(),
                            TR::Node::iconst(node, leftMostNibbleToClear), TR::Node::iconst(node, digitsToClear));
                    }
                    clearNode->setSetSign(rawSign);
                    clearNode->setDecimalPrecision(node->getDecimalPrecision());
                    dumpOptDetails(s->comp(),
                        "\tcreate new %s [0x%p] :  leftDigit %d, digitsToClear %d, sign 0x%x, maskAddress 0x%p\n",
                        clearNode->getOpCode().getName(), clearNode, leftMostNibbleToClear, digitsToClear,
                        (int32_t)sign, maskAddress);
                }
            } else {
                for (int32_t i = 1; i < node->getFirstChild()->getNumChildren();
                     i++) // node->getFirstChild()->getFirstChild() will remain anchored in the new clearNode
                ...
                clearNode = TR::Node::create(TR::pdclear, 3, node->getFirstChild()->getFirstChild(),
                    TR::Node::iconst(node, leftMostNibbleToClear), TR::Node::iconst(node, digitsToClear));
                clearNode->setDecimalPrecision(node->getDecimalPrecision());
...
// If a grandparent (node) doesn't require a cleaned sign and its child (that would pass through a sign -- like a shift
// or modPrec) is not used in any other context (firstChild) then this cleaning grandChild operation can be removed.
static TR::Node *removeGrandChildClean(TR::Node *node, TR::Block *block, TR::Simplifier *s)
{
    ...
    if (firstChild->getReferenceCount() == 1
        && (firstChild->getOpCode().isPackedShift() || firstChild->getOpCode().isPackedModifyPrecision())
        && // TODO: extend to other packed ops that pass-through the sign
        firstChild->getFirstChild()->getOpCodeValue() == TR::pdclean
        && performTransformation(s->comp(),
            "%sRemove unneeded pdclean [" POINTER_PRINTF_FORMAT "] under parent %s [" POINTER_PRINTF_FORMAT
            "] and child %s [" POINTER_PRINTF_FORMAT "]\n",
            s->optDetailString(), firstChild->getFirstChild(), node->getOpCode().getName(), node,
            firstChild->getOpCode().getName(), firstChild)) {
        grandChild = firstChild->getFirstChild();
        grandChild = firstChild->setChild(0,
            s->replaceNodeWithChild(grandChild, grandChild->getFirstChild(), s->_curTree, block));
    }
    return grandChild;
}

static TR::Node *cancelPackedToIntegralConversion(TR::Node *node, TR::ILOpCodes reverseOp, TR::Block *block,
    TR::Simplifier *s)
{
    ...
    if (firstChild->getOpCodeValue() == reverseOp) {
        ...
        const int32_t maxPrecision
            = isLong ? TR::getMaxSignedPrecision<TR::Int64>() : TR::getMaxSignedPrecision<TR::Int32>();
        newNode = s->unaryCancelOutWithChild(node, firstChild, s->_curTree, reverseOp);
        if (newNode && precision < maxPrecision) {
            // In addition to the (redundant) action of the pd2x/x2pd conversion if the x2pd precision is < 10/19 then
            // there is also a truncation side-effect This side-effect must be maintained by introducing a remainder
            // operation to correct to the final # of result digits
            uint64_t powOfTen = computePositivePowerOfTen(precision);
            newNode = TR::Node::create(isLong ? TR::lrem : TR::irem, 2, newNode,
                isLong ? TR::Node::lconst(node, (int64_t)powOfTen) : TR::Node::iconst(node, (int32_t)powOfTen));
...
static void trackSetSignValue(TR::Node *node, TR::Block *block, TR::Simplifier *s)
{
    ...
    if (node->getOpCode().isSetSign()) {
        ...
        if (performTransformation(s->comp(),
                "%sSet known sign value 0x%x on setSignOnNode %s [" POINTER_PRINTF_FORMAT "]\n", s->optDetailString(),
                TR::DataType::getValue(rawSign), node->getOpCode().getName(), node)) {
            node->resetSignState();
...
static TR::Node *reducePackedArithmeticPrecision(TR::Node *node, int32_t maxComputedResultPrecision, TR::Simplifier *s)
{
    ...
    if (maxComputedResultPrecision < nodePrecision
        && performTransformation(s->comp(),
            "%sReduce %s [" POINTER_PRINTF_FORMAT "] precision from %d to the maxComputedResultPrecision %d\n",
            s->optDetailString(), node->getOpCode().getName(), node, nodePrecision, maxComputedResultPrecision)) {
        // it is not legal to reduce the precision directly as the parent (if it is a call for example) may require the
        // larger node precision the parent can optimize away the new pdModifyPrecision if allowed
        ...
        node->setFlags(0);
        dumpOptDetails(s->comp(),
            "%screated new %s [" POINTER_PRINTF_FORMAT
            "] with maxComputedResultPrecision %d and modify old %s [" POINTER_PRINTF_FORMAT "] to %s\n",
            s->optDetailString(), newArith->getOpCode().getName(), newArith, maxComputedResultPrecision,
            newArith->getOpCode().getName(), node, node->getOpCode().getName());
        return node;
    }
...
static TR_PackedCompareCondition packedCompareCommon(TR::Node *parent, TR::Node *op1, TR::Node *op2,
    bool ignoreConstSign, TR::Simplifier *s)
{
    OMR::Logger *log = s->comp()->log();
    bool trace = s->trace();

    logprintf(trace, log,
        "packedCompare: (parent  %s (%p) castedToBCD=%s, op1 %s (%p), op2 %s (%p), absVal=%s) %s vs %s",
        parent->getOpCode().getName(), parent, parent->castedToBCD() ? "yes" : "no", op1->getOpCode().getName(), op1,
        op2->getOpCode().getName(), op2, ignoreConstSign ? "true" : "false", op1->getOpCode().getName(),
...
static TR::Node *simplifyPackedArithmeticOperand(TR::Node *node, TR::Node *parent, TR::Block *block, TR::Simplifier *s)
{
    ...
    if (node->getType().isAnyPacked() && node->canRemoveArithmeticOperand()) {
        bool parentAllowsRemoval = true;
        // operation (e.g. an pdcmpxx) may have to be done as a logical operation so do not remove sign changing
        // children as the comparison may then fail
        if (parent->castedToBCD()) {
            logprintf(s->trace(), s->comp()->log(),
                "parent %s (%p) castedToBCD=true for child %s (%p) so do not allow removal of child\n",
        ...
        if (parentAllowsRemoval
            && performTransformation(s->comp(),
                "%sRemove unnecessary arithmetic operand %s [" POINTER_PRINTF_FORMAT "]\n", s->optDetailString(),
                node->getOpCode().getName(), node)) {
...
static TR::Node *propagateTruncationToConversionChild(TR::Node *node, TR::Simplifier *s, TR::Block *block)
{
    ...
    TR_ASSERT(node->getOpCode().isShift() || node->getOpCode().isModifyPrecision(),
        "propagateTruncationToConversionChild only valid for shifts or modifyPrecision ops\n");
    ...
    if (child->getReferenceCount() == 1 && child->getOpCode().isConversion()
        && (child->getFirstChild()->getType().isAnyZoned() || child->getFirstChild()->getType().isAnyUnicode())
        ...
        if (survivingDigits > 0
            && performTransformation(s->comp(),
                "%sReduce %s child [" POINTER_PRINTF_FORMAT
                "] precision to %d due to truncating %s parent [" POINTER_PRINTF_FORMAT "]\n",
                s->optDetailString(), child->getOpCode().getName(), child, survivingDigits, node->getOpCode().getName(),
                node)) {
...
{
    // set sign operations are typically cheaper then the corresponding general sign moving operation (eg. zdshrSetSign
    // vs zdshr). Although any iconst setSign value would be valid in newSetSign node created below (as the pdsetsign
    // operation dominates it) choose one that is the zone value (0xf) as this is most likely to end up being an
    // effective nop during code generation. In the case where the pdSetSign is itself to the zone value then this
    // pdSetSign node can be removed completely.
    ...
    if (setSign->getReferenceCount() == 1 && child->getReferenceCount() == 1
        && (child->getOpCodeValue() == TR::zd2pd || child->getOpCodeValue() == TR::pd2zd)
        && child->getFirstChild()->getReferenceCount() == 1
        && TR::ILOpCode::setSignVersionOfOpCode(child->getFirstChild()->getOpCodeValue()) != TR::BadILOp
        && performTransformation(s->comp(),
            "%sFold %s [" POINTER_PRINTF_FORMAT "] below child %s [" POINTER_PRINTF_FORMAT
            "] and into grandchild %s [" POINTER_PRINTF_FORMAT "] and create new ",
            s->optDetailString(), setSign->getOpCode().getName(), setSign, child->getOpCode().getName(), child,
            child->getFirstChild()->getOpCode().getName(), child->getFirstChild())) {
        ...
        if ((setSign->getOpCodeValue() == TR::pdSetSign) && setSign->getSecondChild()->getOpCode().isLoadConst()
        ...
        if (newNode) {
            dumpOptDetails(s->comp(), "%s [" POINTER_PRINTF_FORMAT "] with paddingAddress [" POINTER_PRINTF_FORMAT "]",
                newNode->getOpCode().getName(), newNode, paddingAddress);
...
static bool propagateSignStateLeftShift(TR::Node *node, int32_t shiftAmount, TR::Block *block, TR::Simplifier *s)
{
    ...
    bool validateShiftOp = !validateModPrecOp && node->getOpCode().isLeftShift()
...
TR::Node *pd2iSimplifier(TR::Node *node, TR::Block *block, TR::Simplifier *s)
{
    ...
    if (firstChild->getOpCodeValue() == TR::pdclean)
        firstChild
            = node->setChild(0, s->replaceNodeWithChild(firstChild, firstChild->getFirstChild(), s->_curTree, block));

    removeGrandChildClean(node, block, s);

    firstChild = node->setChild(0, removeOperandWidening(node->getFirstChild(), node, block, s));

    if (!node->isNonNegative() && node->getFirstChild()->isNonNegative()
...
{
    TR_ASSERT(node->getOpCodeValue() == TR::pd2l || node->getOpCodeValue() == TR::pd2lu,
        "expecting nodeOp to be pd2l or pd2lu\n");

    s->simplifyChildren(node, block);

    TR::Node *firstChild = node->setChild(0, simplifyPackedArithmeticOperand(node->getFirstChild(), node, block, s));

    // lu2pd and l2pd are the exact same operation (and lu2pd is not even currently generated) so use l2pd as the
    // reversed op for both pd2l/pd2lu pd2l and pd2lu are only kept as separate ops as different runtime routines need
    // to be called on C for these operations
...
TR::Node *i2pdSimplifier(TR::Node *node, TR::Block *block, TR::Simplifier *s)
{
    ...
    if (!node->isNonNegative()) {
        if (node->getFirstChild()->isNonNegative()
            && performTransformation(s->comp(),
                "%sPropagate x >= 0 flag from %s [" POINTER_PRINTF_FORMAT "] to %s [" POINTER_PRINTF_FORMAT "]\n",
                s->optDetailString(), node->getFirstChild()->getOpCode().getName(), node->getFirstChild(),
                node->getOpCode().getName(), node)) {
            node->setIsNonNegative(true);
            ...
            && performTransformation(s->comp(),
                "%sSet x >= 0 flag due on %s [" POINTER_PRINTF_FORMAT "] due to child %s [" POINTER_PRINTF_FORMAT "]\n",
                s->optDetailString(), node->getOpCode().getName(), node, node->getFirstChild()->getOpCode().getName(),
                node->getFirstChild())) {
            node->setIsNonNegative(true);
        }
    }
...
TR::Node *pdaddSimplifier(TR::Node *node, TR::Block *block, TR::Simplifier *s)
{
    ...
    if (!node->isNonNegative() && node->getFirstChild()->isNonNegative() && node->getSecondChild()->isNonNegative()
        && performTransformation(s->comp(),
            "%sSet x >= 0 flag on %s [" POINTER_PRINTF_FORMAT "] with x >= 0 children\n", s->optDetailString(),
            node->getOpCode().getName(), node)) {
        node->setIsNonNegative(true);
        return node; // try to clean up some upstream setsigns before proceeding with binary reduction
    }

    int32_t maxComputedResultPrecision
        = std::max(node->getFirstChild()->getDecimalPrecision(), node->getSecondChild()->getDecimalPrecision()) + 1;
...
TR::Node *pdsubSimplifier(TR::Node *node, TR::Block *block, TR::Simplifier *s)
{
    ...
    if (firstChild->isZero()
        && performTransformation(s->comp(),
            "%sStrength reduce %s [" POINTER_PRINTF_FORMAT "]  0 - %s [" POINTER_PRINTF_FORMAT "] to pdneg\n",
            s->optDetailString(), node->getOpCode().getName(), node, secondChild->getOpCode().getName(), secondChild)) {
...
TR::Node *pdmulSimplifier(TR::Node *node, TR::Block *block, TR::Simplifier *s)
{
    ...
    if (node->getSecondChild()->getSize() > node->getFirstChild()->getSize()) {
        ...
        && node->getFirstChild()->getOpCode().isLoad() && !node->getSecondChild()->getOpCode().isLoad()) {
        // Try to get the more complex child as the first child so the result can naturally accumulate to the final
        // receiver
        TR::Node *firstChild = node->getFirstChild();
        TR::Node *secondChild = node->getSecondChild();
        swapChildren(node, firstChild, secondChild, s);
    ...
    int32_t maxComputedResultPrecision
        = node->getFirstChild()->getDecimalPrecision() + node->getSecondChild()->getDecimalPrecision();
    TR::Node *newNode = reducePackedArithmeticPrecision(node, maxComputedResultPrecision, s);
    if (newNode != NULL)
        return newNode;
...
TR::Node *pddivSimplifier(TR::Node *node, TR::Block *block, TR::Simplifier *s)
{
    ...
    if (firstChild->getOpCode().isLoadConst() && secondChild->getOpCode().isLoadConst() && secondChild->isNonZero()) {
        if (!node->getOpCode().isPackedDivide())
            return s->simplify(node, block);
    }

    if (!node->isNonNegative() && firstChild->isNonNegative() && secondChild->isNonNegative()
        && performTransformation(s->comp(),
            "%sSet x >= 0 flag on %s [" POINTER_PRINTF_FORMAT "] with x >= 0 children\n", s->optDetailString(),
            node->getOpCode().getName(), node)) {
        node->setIsNonNegative(true);
...
TR::Node *pdshrSimplifier(TR::Node *node, TR::Block *block, TR::Simplifier *s)
{
    ...
    if (!node->isNonNegative() && node->getOpCodeValue() == TR::pdshr && node->getFirstChild()->isNonNegative()
        && performTransformation(s->comp(),
            "%sSet x >= 0 flag on %s [" POINTER_PRINTF_FORMAT "] with x >= 0 children\n", s->optDetailString(),
            node->getOpCode().getName(), node)) {
        node->setIsNonNegative(true);
...
{
    TR_ASSERT(node->getOpCodeValue() == TR::pdSetSign, "pdSetSignSimplifier only valid for pdSetSign nodes\n");
    s->simplifyChildren(node, block);
    node->setChild(0, removeOperandWidening(node->getFirstChild(), node, block, s));
    TR::Node *child = node->getFirstChild();
    ...
    if (child->hasKnownOrAssumedSignCode()) {
        ...
        if (TR::DataType::getValue(childSign) == sign
            && performTransformation(s->comp(),
                "%sA1Fold %s [" POINTER_PRINTF_FORMAT "] and child %s [" POINTER_PRINTF_FORMAT
                "] with %s sign that matches setSign (both are 0x%x)\n",
                s->optDetailString(), node->getOpCode().getName(), node, child->getOpCode().getName(), child,
                child->hasKnownSignCode() ? "known" : "assumed", sign)) {
            return s->replaceNodeWithChild(node, child, s->_curTree, block);
        }
    }

    TR_ASSERT(!isSetSignConstant
            || (sign == TR::DataType::getIgnoredSignCode()
    ...
    if (node->getDecimalPrecision() == child->getDecimalPrecision() && child->isSimpleTruncation()
        && performTransformation(s->comp(),
            "%sRemove simple truncating %s [" POINTER_PRINTF_FORMAT "] under setsign node %s [" POINTER_PRINTF_FORMAT
            "]\n",
            s->optDetailString(), child->getOpCode().getName(), child, node->getOpCode().getName(), node)) {
        child = node->setChild(0,
            s->replaceNodeWithChild(child, child->getFirstChild(), s->_curTree, block,
                false)); // correctBCDPrecision=false // correctBCDPrecision=false because nodePrec==childPrec
    }

    if (child->getOpCode().isNeg()
        && performTransformation(s->comp(),
            "%sRemove dominated %s [" POINTER_PRINTF_FORMAT "] under %s [" POINTER_PRINTF_FORMAT "]\n",
            s->optDetailString(), child->getOpCode().getName(), child, node->getOpCode().getName(), node)) {
    ...
    if (node->getOpCodeValue() == child->getOpCodeValue()
        && performTransformation(s->comp(),
            "%s%s [" POINTER_PRINTF_FORMAT "] dominates setsign child %s [" POINTER_PRINTF_FORMAT
            "] -- remove child [" POINTER_PRINTF_FORMAT "]\n",
    ...
    if (child->getOpCodeValue() == TR::pdclean
        && performTransformation(s->comp(),
            "%s%s [" POINTER_PRINTF_FORMAT "] dominates cleaning child %s [" POINTER_PRINTF_FORMAT
            "] -- remove child [" POINTER_PRINTF_FORMAT "]\n",
            s->optDetailString(), node->getOpCode().getName(), node, child->getOpCode().getName(), child, child)) {
        // Transform:
        // pdSetSign
    ...
    if (child->getReferenceCount() == 1 && child->getOpCode().isSetSign()
        && performTransformation(s->comp(),
            "%sFold %s [" POINTER_PRINTF_FORMAT "] value [" POINTER_PRINTF_FORMAT
            "] into dominated %s child [" POINTER_PRINTF_FORMAT "]\n",
            s->optDetailString(), node->getOpCode().getName(), node, node->getSecondChild(),
...

    {
        ...
        return s->simplify(node, block);
    } else if (child->getReferenceCount() == 1 && child->getOpCode().isSetSignOnNode() && isSetSignConstant
        && TR::DataType::isSupportedRawSign(sign)
        && performTransformation(s->comp(),
            "%sFold %s [" POINTER_PRINTF_FORMAT
            "] sign=0x%x into dominated setSignOnNode %s child [" POINTER_PRINTF_FORMAT "]\n",
...

    {
        ...
        //       sign
        TR::Node *newNode
            = foldSetSignIntoNode(node, false /* !setSignIsTheChild */, child, true /* removeSetSign */, block, s);
        if (newNode != node)
            return newNode;
    }
...
TR::Node *pdcleanSimplifier(TR::Node *node, TR::Block *block, TR::Simplifier *s)
{
    ...
    if (child->getReferenceCount() == 1 && child->getOpCode().isSetSign()
        ...
        if (TR::DataType::rawSignIsPositive(child->getDataType(), rawSign)
            && performTransformation(s->comp(),
                "%sCleaning constant sign on child %s [" POINTER_PRINTF_FORMAT "] : 0x%x->0x%x by ",
    ...
    if (child->hasKnownOrAssumedSignCode()) {
        ...
        TR_ASSERT(TR::DataType::getPreferredPlusCode() == 0x0c,
            "expecting the preferred plus sign code 0x%x to be 0xc (==raw_bcd_sign_0xc)\n",
        ...
        switch (sign) {
            ...
            case raw_bcd_sign_0xf: {
                ...
                if (isLoadVarChild && parent && parent->getOpCode().isPackedStore()
                    ...
                    //          iconst 0xc
                    //
                    // as this will usually result in better codegen (e.g. move and clean can be done by one
                    // instruction) so prevent reversing this optimization here (otherwise both opts keep happening and
                    // flipping the IL back and forth)
                    logprintf(s->trace(), s->comp()->log(),
                        "do not replace %s (%p) with setsign 0xf because parent %s (%p) is a store and child %s (%p) "
                        "is a loadVar\n",
                ...
                if (removeClean
                    && performTransformation(s->comp(),
                        "%sFold pdclean [" POINTER_PRINTF_FORMAT "] of child [" POINTER_PRINTF_FORMAT
                        "] with known sign 0xf to new pdSetSign ",
                        s->optDetailString(), node, child)) {
                    TR::Node *setSignNode = TR::Node::create(TR::pdSetSign, 2, child,
                        TR::Node::iconst(child, TR::DataType::getPreferredPlusCode()));
                    ...
                    return s->replaceNode(node, setSignNode, s->_curTree,
                        false); // needAnchor=false as child is anchored under setSignNode
                }
                break;
            }
            default:
    ...
    if (node->getOpCodeValue() == TR::pdclean && child->getReferenceCount() == 1 && child->isSimpleTruncation()
        && child->getFirstChild()->getOpCodeValue() == TR::pdclean
        && performTransformation(s->comp(),
            "%sRemove pdclean [" POINTER_PRINTF_FORMAT "] under simple truncating %s [" POINTER_PRINTF_FORMAT
            "] as pdclean [" POINTER_PRINTF_FORMAT "] will clean\n",
...
TR::Node *pdclearSimplifier(TR::Node *node, TR::Block *block, TR::Simplifier *s)
{
    ...
    if (firstChild->getOpCodeValue() == TR::pdSetSign && firstChild->hasKnownOrAssumedSignCode()
        && performTransformation(s->comp(),
            "%sFold child %s [" POINTER_PRINTF_FORMAT "] into parent %s [" POINTER_PRINTF_FORMAT "] with sign 0x%x\n",
            s->optDetailString(), firstChild->getOpCode().getName(), firstChild, node->getOpCode().getName(), node,
            TR::DataType::getValue(firstChild->getKnownOrAssumedSignCode()))) {
...
{
    return node->getReferenceCount() == 1 || (node->getReferenceCount() - 1 == node->getFutureUseCount());
}
...
TR::Node *pdstoreSimplifier(TR::Node *node, TR::Block *block, TR::Simplifier *s)
{
    ...
    if (!s->comp()->getOption(TR_KeepBCDWidening) && valueChild->getOpCodeValue() == TR::pdclean
        && node->getDecimalPrecision() <= TR::DataType::getMaxPackedDecimalPrecision()
        && isFirstOrOnlyReference(valueChild)
        && performTransformation(s->comp(),
            "%sFold pdclean [" POINTER_PRINTF_FORMAT
            "] into pdstore by setting CleanSignInPDStoreEvaluator flag on pdstore [" POINTER_PRINTF_FORMAT "]\n",
            s->optDetailString(), valueChild, node)) {
        // this transformation is only done for first/only references to avoid a sub-optimality where an already clean
        // sign is cleaned again on a subsequent reference pdx
        //    pdclean  // firstRef
        //...
        // pdstore
        //    =>pdclean // if this clean is folded into its parent store then the already cleaned pdclean will be
        //    cleaned again
        //
        node->setCleanSignInPDStoreEvaluator(true);
        valueChild
            = node->setValueChild(s->replaceNodeWithChild(valueChild, valueChild->getFirstChild(), s->_curTree, block));
    } else if (valueChild->getOpCodeValue() == TR::pdSetSign && valueChild->getSecondChild()->getOpCode().isLoadConst()
        ...
        && performTransformation(s->comp(),
            "%sFold pdsetsign [" POINTER_PRINTF_FORMAT "] by 0xc of child %s [" POINTER_PRINTF_FORMAT "]",
            s->optDetailString(), valueChild, valueChild->getFirstChild()->getOpCode().getName(),
            valueChild->getFirstChild())) {
        // pdstore
        //    pdsetsign
        //       pdload sign=0xf
        //       iconst 0xc
        ...
        //
        dumpOptDetails(s->comp(),
            " by setting CleanSignInPDStoreEvaluator flag on pdstore [" POINTER_PRINTF_FORMAT "]\n", node);
        node->setCleanSignInPDStoreEvaluator(true);
        valueChild
            = node->setValueChild(s->replaceNodeWithChild(valueChild, valueChild->getFirstChild(), s->_curTree, block));
    }
    ...
    if (node->getType().isAnyPacked() && node->mustCleanSignInPDStoreEvaluator() && valueChild->getReferenceCount() == 1
        && valueChild->isSimpleTruncation() && valueChild->getFirstChild()->getOpCodeValue() == TR::pdclean
        && performTransformation(s->comp(),
            "%sRemove pdclean [" POINTER_PRINTF_FORMAT "] under simple truncating %s [" POINTER_PRINTF_FORMAT
            "] as pdstore [" POINTER_PRINTF_FORMAT "] will clean\n",
    ...
    bool removeSimpleTruncation
        = (node->getDecimalPrecision() == valueChild->getDecimalPrecision()) && valueChild->isSimpleTruncation();
    ...
    if (removeSimpleTruncation
        && performTransformation(s->comp(),
            "%sRemove simple truncating %s [" POINTER_PRINTF_FORMAT "] under store node %s [" POINTER_PRINTF_FORMAT
            "]\n",
            s->optDetailString(), valueChild->getOpCode().getName(), valueChild, node->getOpCode().getName(), node)) {
        valueChild = node->setValueChild(s->replaceNodeWithChild(valueChild, valueChild->getFirstChild(), s->_curTree,
            block, false)); // correctBCDPrecision=false because nodePrec==valueChildPrec
    }
...
TR::Node *pdshlSimplifier(TR::Node *node, TR::Block *block, TR::Simplifier *s)
{
    ...
    if (isShiftByZero) {
        ...
        if (node->getDecimalPrecision() >= firstChild->getDecimalPrecision()
            ...
            return s->replaceNodeWithChild(node, firstChild, s->_curTree, block,
                false); // correctBCDPrecision=false because this is the special case of same byte widening
        } else if (node->getDecimalPrecision() <= firstChild->getDecimalPrecision()
            && node->getFirstChild()->isSimpleTruncation()
            && performTransformation(s->comp(),
                "%sRemove simple truncating firstChild %s [" POINTER_PRINTF_FORMAT
                "] of simple truncating node %s [" POINTER_PRINTF_FORMAT "]\n",
                s->optDetailString(), firstChild->getOpCode().getName(), firstChild, node->getOpCode().getName(), node))

        {
            firstChild = node->setChild(0,
                s->replaceNodeWithChild(node->getFirstChild(), node->getFirstChild()->getFirstChild(), s->_curTree,
                    block, false)); // correctBCDPrecision=false this is a trunc of a trunc
            return s->simplify(node, block);
        } else if ((firstChild->getOpCodeValue() == TR::zd2pd || firstChild->getOpCodeValue() == TR::zdsle2pd
                       || firstChild->getOpCodeValue() == TR::zdsls2pd || firstChild->getOpCodeValue() == TR::zdsts2pd
                       || firstChild->getOpCodeValue() == TR::ud2pd || firstChild->getOpCodeValue() == TR::udsl2pd
                       || firstChild->getOpCodeValue() == TR::udst2pd || firstChild->getOpCodeValue() == TR::pdSetSign
                       || firstChild->getOpCodeValue() == TR::pdclear
                       || firstChild->getOpCodeValue() == TR::pdclearSetSign
                       || firstChild->getOpCodeValue() == TR::pdneg)
            && (firstChild->getReferenceCount() == 1) && isTruncation
            && performTransformation(s->comp(),
                "%sRemove simple truncating %s [" POINTER_PRINTF_FORMAT "] of %s child [" POINTER_PRINTF_FORMAT
                "] by 0 and set child precision to %d\n",
    ...
    if (!isShiftByZero && firstChild->getOpCodeValue() == TR::pdSetSign) {
        TR::Node *newNode
            = foldSetSignIntoNode(firstChild, true, node, true, block, s); // setSignIsTheChild=true, removeSetSign=true
        if (newNode != node)
            return newNode;
    }
    ...
    if (shiftAmount >= 0) {
        ...
        // return now if some sign state change was made to see if a parent pdclean/pdsetsign operation can be removed
        // before possibly converting pdshl to zdshl below
        if (changedSignState)
            return node;
    }
...
TR::Node *pdshlSetSignSimplifier(TR::Node *node, TR::Block *block, TR::Simplifier *s)
{
    ...
    //
    if (firstChild->getOpCodeValue() == TR::pdSetSign && firstChild->getReferenceCount() == 1
        && // not required for correctness but the uncommoning of the pdSetSign in rc>1 cases can lead to worse code --
           // it is sure win in the rc=1 case
        ...
        && !node->hasIntermediateTruncation()
        && performTransformation(s->comp(),
            "%sRemove dominated setSign %s [" POINTER_PRINTF_FORMAT "] under %s [" POINTER_PRINTF_FORMAT
            "] (both signs are 0x%x)\n",
            s->optDetailString(), firstChild->getOpCode().getName(), firstChild, node->getOpCode().getName(), node,
            (int32_t)node->getThirdChild()->get64bitIntegralValue())) {
        firstChild
            = node->setChild(0, s->replaceNodeWithChild(firstChild, firstChild->getFirstChild(), s->_curTree, block));
    }

    if (node->getSecondChild()->getOpCode().isLoadConst() && node->getSecondChild()->get64bitIntegralValue() == 0
        && node->getThirdChild()->getOpCode().isLoadConst()) {
        if (firstChild->hasKnownOrAssumedSignCode()) {
            ...
            if (TR::DataType::getValue(childSign) == sign
                && performTransformation(s->comp(),
                    "%sFold %s [" POINTER_PRINTF_FORMAT "] and child %s [" POINTER_PRINTF_FORMAT
                    "] with %s sign that matches setSign (both are 0x%x)\n",
                    s->optDetailString(), node->getOpCode().getName(), node, firstChild->getOpCode().getName(),
                    firstChild, firstChild->hasKnownSignCode() ? "known" : "assumed", sign)) {
                return s->replaceNodeWithChild(node, firstChild, s->_curTree, block);
            }
        }
    }
    ...
    if (firstChild->getOpCodeValue() == TR::pdSetSign && firstChild->hasKnownOrAssumedSignCode()
        ...
        if (TR::DataType::getValue(childSign) == sign
            && performTransformation(s->comp(),
                ...
                "] with %s sign that matches setSign (both are 0x%x)\n",
                s->optDetailString(), node->getOpCode().getName(), node, firstChild->getOpCode().getName(), firstChild,
                firstChild->hasKnownSignCode() ? "known" : "assumed", sign)) {
            firstChild = node->setChild(0,
                s->replaceNodeWithChild(firstChild, firstChild->getFirstChild(), s->_curTree, block));
        }
    }
...
TR::Node *pdnegSimplifier(TR::Node *node, TR::Block *block, TR::Simplifier *s)
{
    ...
    TR::Node *result = NULL;
    if ((result = s->unaryCancelOutWithChild(node, child, s->_curTree, TR::ILOpCode::negateOpCode(node->getDataType()),
             false))) // anchorChildren=false
        return result;

    TR::ILOpCodes setSignOp = TR::ILOpCode::setSignOpCode(node->getDataType());
    if (setSignOp != TR::BadILOp && NUM_DEFAULT_CHILDREN >= 2 && child->hasKnownOrAssumedSignCode()) {
        TR_RawBCDSignCode sign = child->getKnownOrAssumedSignCode();
        ...
        if (negatedSign != TR::DataType::getInvalidSignCode()
            && performTransformation(s->comp(),
                "%sStrength reducing %s [" POINTER_PRINTF_FORMAT
                "] with known/assumed sign 0x%x child %s [" POINTER_PRINTF_FORMAT "] to ",
                s->optDetailString(), node->getOpCode().getName(), node, TR::DataType::getValue(sign),
                child->getOpCode().getName(), child)) {
            TR::Node::recreate(node, setSignOp);
            dumpOptDetails(s->comp(), "%s 0x%x\n", node->getOpCode().getName(), negatedSign);
            node->setFlags(0);
            if (setSignOp == child->getOpCodeValue()) // if child is just a basic setSign operation too (pdSetSign,...)
    ...
    TR::Block *block, TR::Simplifier *s)
{
    ...
    // if the child is truncating and the parent widens and the setSign operation will be removed then any intermediate
    // truncation will be lost so the transformation would be invalid.
    if (removeSetSign && parent->hasIntermediateTruncation()) {
        logprintf(s->trace(), s->comp()->log(),
            "disallow foldSetSignIntoNode of setSign -- %s (%p) with other node -- %s (%p) because child %p (prec %d) "
            "truncates and parent %p (prec %d) widens\n",
            setSign->getOpCode().getName(), setSign, other->getOpCode().getName(), other, child,
            child->getDecimalPrecision(), parent, parent->getDecimalPrecision());
        return parent;
    }
    ...
    if (setSign->getChild(originalSetSignIndex)->getOpCode().isLoadConst()) {
        ...
        convertedSetSign
            = TR::DataType::convertSignEncoding(setSign->getDataType(), parent->getDataType(), originalSetSign);
        TR_ASSERT(convertedSetSign != TR::DataType::getInvalidSignCode(),
            "could not convert sign encoding of 0x%x from type %s to type %s\n", originalSetSign,
            setSign->getDataType().toString(), parent->getDataType().toString());
    }
    if (convertedSetSign != TR::DataType::getInvalidSignCode() && setSign->getReferenceCount() == 1
        && other->getReferenceCount() == 1 && setSignOp != TR::BadILOp
        && performTransformation(s->comp(),
            "%sFold %s [" POINTER_PRINTF_FORMAT "] into setsign %s %s [" POINTER_PRINTF_FORMAT "] and create new ",
            s->optDetailString(), other->getOpCode().getName(), other, setSignIsTheChild ? "child" : "parent",
            setSign->getOpCode().getName(), setSign)) {
        int32_t newSetSignIndex = TR::ILOpCode::getSetSignValueIndex(setSignOp);
        TR::Node *newNode = NULL;
        TR::Node *src = NULL;
        ...
        newSetSign->incReferenceCount(); // all the other nodes added to newNode are existing nodes so inc'ing here
                                         // allows all children refCounts to be corrected later
        if (newSetSignIndex == 1) // child 2
        {
            if (other->getOpCode().isConversion() && other->getFirstChild()->getType().isUnicodeSeparateSign()
                ...
                if (paddingAddress)
                    newNode = TR::Node::create(setSignOp, 3, src, newSetSign, paddingAddress);
                else
                    newNode = TR::Node::create(setSignOp, 2, src, newSetSign);
                ...
                TR_ASSERT(false,
                    "unexpected parent/child combination in foldSetSignIntoNode for newSetSignIndex == 1\n");
            }
        } else if (newSetSignIndex == 2) // child 3
        {
            // Transform:                      |  Transform:                                             | Transform (a
            // removeSetSign=false case):
            //                                 |                                                         |
            //    pdshl - node                 |      pd2udsl - node                                     |       pd2udsl
            //    - node
            //       pdSetSign - child         |         pdSetSign - child                               | pdshlSetSign
            //       - child
            //          x                      |            x                                            |             x
            ...
            // signAddrForCompareAndSet (could be '-' or '+')
            //                                 |                                                         |      to:
            //    pdshlSetSign - node          |      pd2udslSetSign - node                              |
            //    pd2udslSetSign - node
            //          x                      |            x                                            | pdshlSetSign
            ...
            //                                 |                                                         | iconst sign
            //                                 |                                                         |
            //                                 newSignAddrForSet (from the convertedSign) | |          iconst
            //                                 convertedSign |                                                         |
            //
            ...
            if (isShiftToShiftFold) {
                ...
                TR_ASSERT(false,
                    "unexpected parent/child combination in foldSetSignIntoNode for newSetSignIndex == 2\n");
            }
            if (newSecondChild) {
                if (paddingAddress)
                    newNode = TR::Node::create(setSignOp, 4, src, newSecondChild, newSetSign, paddingAddress);
                else
                    newNode = TR::Node::create(setSignOp, 3, src, newSecondChild, newSetSign);
            }
        } else if (newSetSignIndex == 3) // child 4
        {
            // Transform:
            ...
            if (isShiftToShiftFold) {
                ...
                TR_ASSERT(false,
                    "unexpected parent/child combination in foldSetSignIntoNode for newSetSignIndex == 3\n");
            }
            if (newSecondChild && newThirdChild) {
                if (paddingAddress)
                    newNode = TR::Node::create(setSignOp, 5, src,
                        newSecondChild, // shiftAmount
                        newThirdChild, // roundAmount
                        newSetSign, paddingAddress);
                else
                    newNode = TR::Node::create(setSignOp, 4, src,
                        newSecondChild, // shiftAmount
                        newThirdChild, // roundAmount
                        newSetSign);
            }
        ...
        if (newNode) {
            ...
            dumpOptDetails(s->comp(), "%s [" POINTER_PRINTF_FORMAT "] with decimalPrecision of ",
                newNode->getOpCode().getName(), newNode);
            newNode->incReferenceCount();
            // All the children on newNode were existing nodes except for newSetSign and were therefore incremented one
            // extra time each when creating newNode above and newSetSign was explicitly inc'ed
            ...
            if (removeSetSign) {
                ...
                dumpOptDetails(s->comp(),
                    "%d from parent node %s [" POINTER_PRINTF_FORMAT "] and paddingAddress [" POINTER_PRINTF_FORMAT
                    "]\n",
                ...
                stopUsingSingleNode(setSign, false,
                    s); // removePadding=false, paddingAddress is still attached to the newNode
                return s->simplify(newNode, block);
            } else {
                dumpOptDetails(s->comp(),
                    "%d from other node %s [" POINTER_PRINTF_FORMAT "] and paddingAddress [" POINTER_PRINTF_FORMAT
                    "]\n",
...
static TR::Node *foldAndReplaceDominatedSetSign(TR::Node *setSign, bool setSignIsTheChild, TR::Node *other,
    TR::Block *block, TR::Simplifier *s)
{
    // Attempt to fold a child (or parent) setSign operation into a new setSign version of the parent (or child) and
    // leave the original setSign operation in place (because for example the child setSign also performs some other
    // operation such as a shift that cannot be folded into the parent too). If this is folding is successful then
    // change the original child (or parent) setSign value to a new value that will be less expensive to evaluate.
    //
    ...
    //    RightShift        : MVO
    if (parent->getOpCode().isSetSign() && parent->getOpCode().isRightShift()
        ...
        child->getOpCode().isLeftShift() && child->getSecondChild()->getOpCode().isLoadConst()
        && child->getSecondChild()->get64bitIntegralValue()
            == 1) // left shift by 1 - could be expanded for larger shift amounts when a padding address is used for
                  // left shifts
    {
    ...
    if (setSignIsTheChild || TR::ILOpCode::setSignVersionOfOpCode(child->getOpCodeValue()) != TR::BadILOp) {
        ...
        if (parent->getOpCode().isSetSign() && child->getOpCode().isConversion() && child->getOpCode().isSetSign()
            ...
            //
            logprintf(s->trace(), s->comp()->log(),
                "disallow setting ignored setsign value on dominated %s [" POINTER_PRINTF_FORMAT
                "] so sign verification can be skipped\n",
                child->getOpCode().getName(), child);
            return parent;
        } else if (parent->getOpCode().isSetSign() && parent->getFirstChild()->getOpCode().isSetSign()) {
            int32_t childSetSignIndex = TR::ILOpCode::getSetSignValueIndex(child->getOpCodeValue());
            if (child->getReferenceCount() == 1 && child->getChild(childSetSignIndex)->getOpCode().isLoadConst()
                && performTransformation(s->comp(),
                    "%sReplace %s [" POINTER_PRINTF_FORMAT "] dominated %s [" POINTER_PRINTF_FORMAT
                    "] setSignValue node [" POINTER_PRINTF_FORMAT "] with new ignored setSignValue %d ",
                    s->optDetailString(), parent->getOpCode().getName(), parent, child->getOpCode().getName(), child,
                    child->getChild(childSetSignIndex), newChildSetSignValue)) {
...
static TR::Node *createSetSignForKnownSignChild(TR::Node *node, TR::Block *block, TR::Simplifier *s)
{
    TR::Node *newNode = NULL;
    TR::Node *child = node->getFirstChild();
    if (node->getReferenceCount() == 1 && child->getReferenceCount() == 1
        ...
        if (setSignOp != TR::BadILOp
            && performTransformation(s->comp(),
                "%sFold alwaysGeneratesAKnownPositiveCleanSign child %s [" POINTER_PRINTF_FORMAT
                "] into %s [" POINTER_PRINTF_FORMAT "] and create new ",
                s->optDetailString(), child->getOpCode().getName(), child, node->getOpCode().getName(), node)) {
            int32_t newSetSignIndex = TR::ILOpCode::getSetSignValueIndex(setSignOp);
            int32_t convertedSetSign = TR::DataType::convertSignEncoding(child->getDataType(), node->getDataType(),
                TR::DataType::getPreferredPlusCode());
            TR::Node *newSetSign = TR::Node::iconst(node, convertedSetSign);
            if (newSetSignIndex == 2) {
                ...
                if (node->getOpCode().isShift()) {
                    ...
                    TR_ASSERT(false,
                        "unexpected parent/child combination in pd2udslSimplifier for newSetSignIndex == 2\n");
                }
                if (newSecondChild) {
                    newNode = TR::Node::create(setSignOp, 3, child, newSecondChild, newSetSign);
                }
                ...
                TR_ASSERT(node->getNumChildren() == 3,
                    "expecting three children on the shift node (%p) and not %d children\n", node,
                    node->getNumChildren());
                newNode = TR::Node::create(setSignOp, 4, child,
