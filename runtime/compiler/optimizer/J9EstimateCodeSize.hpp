...
public:
    ...
    static TR::Block *getBlock(TR::Compilation *comp, TR::Block **blocks, TR_ResolvedMethod *feMethod, int32_t i,
        TR::CFG &cfg);

    static void setupNode(TR::Node *node, uint32_t bcIndex, TR_ResolvedMethod *feMethod, TR::Compilation *comp);
    static void setupLastTreeTop(TR::Block *currentBlock, TR_J9ByteCode bc, uint32_t bcIndex,
...
protected:
     ...
     *     Reference to cfg
     */
    TR::CFG &processBytecodeAndGenerateCFG(TR_CallTarget *calltarget, TR::Region &cfgRegion, TR_J9ByteCodeIterator &bci,
        NeedsPeekingHeuristic &nph, TR::Block **blocks, flags8_t *flags);
    bool realEstimateCodeSize(TR_CallTarget *calltarget, TR_CallStack *prevCallStack, bool recurseDown,
        TR::Region &cfgRegion, int32_t callerAnalyzedSizeThreshold = 0);
