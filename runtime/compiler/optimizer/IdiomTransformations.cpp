...
static bool avoidTransformingStringLoops(TR::Compilation *comp)
{
    ...
    if (!cacheInitialized) {
        // TODO: This is a workaround for Java 829 functionality as we switched to using a byte[] backing array in
        // String*. Remove this workaround once obsolete. Idiom recognition currently does not handle idioms involving
        // char[] in a compressed string. String compression is technically a Java 9 feature, but for the sake of
        // evaluating performance we need to be able to run our standard set of benchmarks, most of which do not work
        // under Java 9 at the moment. This leaves us with the only option to run the respective benchmarks on Java 8
        // SR5, however in Java 8 SR5 the java/lang/String.value is of type char[] which will cause functional problems.
        // To avoid these issues we will disable idiom recognition on Java 8 SR5 if String compression is enabled.
        TR_OpaqueClassBlock *stringClass
            = comp->cg()->fej9()->getSystemClassFromClassName("java/lang/String", strlen("java/lang/String"), true);
        ...
        if (stringClass != NULL) {
            ...
            if (comp->cg()->fej9()->getInstanceFieldOffset(stringClass, "value", "[C") != ~0) {
                cacheValue = IS_STRING_COMPRESSION_ENABLED_VM(
                    static_cast<TR_J9VMBase *>(comp->fe())->getJ9JITConfig()->javaVM);
            }
        }
    }
...
//*****************************************************************************************
bool ChangeAlignmentOfRegion(TR_CISCTransformer *trans)
{
    ...
    for (t = T->getEntryNode(); t->isOutsideOfLoop();) {
        if (trans->analyzeT2P(t, pTop) & _T2P_MatchMask) {
            ...
            for (chk = t->getSucc(0); chk->isOutsideOfLoop(); chk = chk->getSucc(0)) {
                if (!chk->isNegligible() && trans->analyzeT2P(chk) == _T2P_NULL)
                    break; // t is still invalid.
            }
            if (!chk->isOutsideOfLoop()) {
                logprintf(disptrace, log, "ChangeAlignmentOfRegion : (t:%d p:%d) no need to change alignment\n",
                    t->getID(), pTop->getID());
                return changed; // Find pTop! Already aligned correctly
            }
        }
        if (t->getNumSuccs() < 1) {
            logprintf(disptrace, log, "ChangeAlignmentOfRegion : #succs of tID:%d is 0\n", t->getID());
        ...
        switch (t->getOpcode()) {
            ...
            case TR::table:
                ...
                for (i = t->getNumSuccs(); --i >= 0;) {
                    ...
                    if (next_t->getOpcode() == TR::Case && next_t->getSucc(0) != T->getExitNode()) {
                        t = next_t->getSucc(0);
                        goto exit_switch;
                    }
                }
...
exit_switch:;
    ...
    if (t->getOpcode() != TR::BBStart)
        return changed; // already aligned by this transformation before
    t = t->getSucc(0); // Skip BBStart

    int condT2P = trans->analyzeT2P(t, pTop);
    if (condT2P & _T2P_MatchMask)
        return changed; // no need to change alignment

    logprintf(disptrace, log, "ChangeAlignmentOfRegion : tTop %d, pTop %d\n", t->getID(), pTop->getID());
    ...
    int branchCount = 0;
    for (;;) {
        if (condT2P != _T2P_NULL || !t->isNegligible())
            lastNode = t;
        t = t->getSucc(0);
        if (t->getOpcode() == TR::BBEnd || t->getOpcode() == TR_exitnode || t == firstNode)
            return changed; // current limitation. peeling can be performed within the first BB of the body
        if (t->getIlOpCode().isBranch())
            if (++branchCount >= 2)
                return changed; // allow a single branch
        condT2P = trans->analyzeT2P(t, pTop);
        if (condT2P & _T2P_MatchMask)
            break; // the target node corresponding to pTop is found
    }
    if (!lastNode)
        return changed; // the last node of the peeling region
    ...
    logprintf(disptrace, log, "ChangeAlignmentOfRegion : foundNode %d, lastNode %d\n", foundNode->getID(),
        lastNode->getID());
    if (branchCount > 0 && !lastNode->getIlOpCode().isBranch()) {
        logprints(disptrace, log, "Fail: there is a branch in the region. lastNode must be a branch node.\n");
        return changed;
    }
    for (t = firstNode;; t = t->getSucc(0)) {
        r.append(t);
    ...
    for (t = ri.getFirst(); t; t = ri.getNext()) // each node in the region r
    {
        if (t->getOpcode() == TR::aload || t->getOpcode() == TR::iload) {
            ...
            if (noDefInR)
                continue; // If there is no def in r, it ignores this load node.
        }
        ...
        for (pn = pi.getFirst(); pn; pn = pi.getNext()) // each parent of t
        {
            if (!r.isIncluded(pn)) {
                logprintf(disptrace, log,
                    "ChangeAlignmentOfRegion : There is a parent(%d) of %d in the outside of the region\n", pn->getID(),
                    t->getID());
                return changed; // fail
            }
        }
    ...
    if (disptrace) {
        log->printf("ChangeAlignmentOfRegion: Succ[0] of %d will be changed from %d to %d.\n", beforeLoop->getID(),
            beforeLoop->getSucc(0)->getID(), foundNode->getID());
    ...
    for (t = ri.getFirst(); t; t = ri.getNext()) {
        if (t->getIlOpCode().isTreeTop()) {
            ...
            logprintf(disptrace, log, "add TR::Node 0x%p (tid:%d) to BeforeInsertionList.\n", rep, t->getID());

            rep = duplicator.duplicateTree(rep);
            if (t->getIlOpCode().isIf()) {
                if (t->getOpcode() != rep->getOpCodeValue()) {
                    TR::TreeTop *ret;
                    for (ret = t->getHeadOfTreeTop()->getNextTreeTop(); ret->getNode()->getOpCodeValue() != TR::BBStart;
...
//*****************************************************************************************
TR_CISCNode *analyzeMoveNodeForward(TR_CISCTransformer *trans, List<TR_CISCNode> *l, TR_CISCNode *n,
    List<TR_CISCNode> *tgt)
{
    const bool disptrace = DISPTRACE(trans);
    ...
    if (tgt->find(t))
        return NULL; // already moved

    bool go = false;
    if (n->isStoreDirect()) {
        go = true;
    } else if (n->getNumChildren() == 2) {
        if (n->getIlOpCode().isAdd() || n->getIlOpCode().isSub() || n->getIlOpCode().isMul()
            || n->getIlOpCode().isLeftShift() || n->getIlOpCode().isRightShift() || n->getIlOpCode().isShiftLogical()
            || n->getIlOpCode().isAnd() || n->getIlOpCode().isOr() || n->getIlOpCode().isXor()) // Safe expressions
        {
            go = true;
            if (n->getChild(0)->getOpcode() == TR_variable || n->getChild(1)->getOpcode() == TR_variable)
                go = false; // not implemented yet.
        }
    } else if (n->getNumChildren() == 1) {
        if (n->getIlOpCode().isConversion() || n->getIlOpCode().isNeg()) // Safe expressions
        {
            go = true;
    ...
    if (go) {
        ...
        while (true) {
            if (chains->find(t))
                break; // it cannot be moved beyond its use/def.
            if (parents->find(t))
                break; // it cannot be moved beyond its parent.

            if (t->getOpcode() == TR::BBStart) {
                TR::Block *block = t->getHeadOfTrNode()->getBlock();
                if (block->getPredecessors().size() > 1)
                    return NULL; // It currently analyzes within this BB.
            }
            if (t->getNumSuccs() >= 2 && specialCareIf) {
                bool fail = true;
                TR_CISCNode *p = trans->getT2Phead(t);
                if (p && p == specialCareIf && t->getSucc(1) == trans->getT()->getExitNode()) {
                    ...
                    if (trNode->getOpCode().isTreeTop()) {
                        if (trNode->getOpCode().isStoreDirect()) {
                            if (!generateCompensation0) {
                                ...
                                logprintf(disptrace, comp->log(),
                                    "analyzeMoveNodeForward: append the tree of 0x%p into AfterInsertionIdiomList\n",
                                    trNode);
                ...
                if (fail)
                    break; // It currently analyzes within this BB.
            }
            t = ti.getNext();
            if (t == NULL)
                break; // cannot find any node in tgt in the list l.
            ret = t;
            if (tgt->find(t))
                break; // find goal!
        }
...
//*****************************************************************************************
bool reorderTargetNodesInBB(TR_CISCTransformer *trans)
{
    TR_CISCGraph *P = trans->getP();
    TR_CISCGraph *T = trans->getT();
            ...
            if (maxPid >= 0) {
                if (maxPid <= currentPID) {
                    ...
                    if (t->isOutsideOfLoop())
                        break; // reordering is currently supported only inside of the loop
                    ...
                    if (tgt) {
                        ...
                        logprintf(disptrace, log, "We can move the node %d to %p(%d)\n", tID, tgt, tgt->getID());
...
//*****************************************************************************************
bool moveStoreOutOfLoopForward(TR_CISCTransformer *trans)
{
    TR_CISCGraph *P = trans->getP();
    ...
    for (t = ti.getFirst(); t; t = ti.getNext()) // for each target node corresponding to p
    {
        ...
        for (tParent = tParentIter.getFirst(); tParent; tParent = tParentIter.getNext()) {
            ...
            if (tParent->isStoreDirect() && !tParent->isNegligible()) {
                // checking whether all variables of stores are same.
                if (!storedVariable)
                    storedVariable = tParent->getChild(1);
                else if (storedVariable != tParent->getChild(1)) {
                    logprints(disptrace, log,
                        "moveStoreOutOfLoopForward failed because all variables of stores are not same.\n");
                    success0 = false;
                ...
                if (checkSuccsSet(trans, tParent, &findBV)) {
                    ...
                    logprints(disptrace, log,
                        "moveStoreOutOfLoopForward failed because tParent will not reach either boolTable or "
                        "optionalCmp.\n");
                    success0 = false;
                    goto endSpecial0; // FAIL!
                }
        ...
        if (tParent)
            targetList.add(tParent); // add a store instruction
    }
endSpecial0:

    if (targetList.isEmpty()) {
        logprints(disptrace, log, "moveStoreOutOfLoopForward failed because targetList is empty.\n");
        success0 = false;
    }
    // check if descendants of p include an array load
    if (!getThreeNodesForArray(p, &ixload, &aload, &iload, true)) {
        logprintf(disptrace, log,
            "moveStoreOutOfLoopForward failed because decendents of pid:%d don't include an array load.\n", p->getID());
        success0 = false;
    }
    ...
    if (success0) {
        ...
        logprintf(disptrace, log, "moveStoreOutOfLoopForward: Target nodes ixload=%d, aload=%d, iload=%d\n",
        ...
        if (ixload && aload && iload && (iload->isLoadVarDirect() || iload->getOpcode() == TR_variable)) {
            ...
            TR::Node *iloadm1Node
                = createOP2(comp, TR::isub, TR::Node::createLoad(iloadNode, iloadNode->getSymbolReference()),
                    TR::Node::create(iloadNode, TR::iconst, 0, 1));

            // prepare base[index]
            TR::Node *arrayLoad0 = createArrayLoad(comp, trans->isGenerateI2L(), ixloadNode,
                aload->getHeadOfTrNodeInfo()->_node, iloadNode, ixloadNode->getSize());

            // prepare base[index-1]  (it may not be used.)
            TR::Node *arrayLoad1 = createArrayLoad(comp, trans->isGenerateI2L(), ixloadNode,
                aload->getHeadOfTrNodeInfo()->_node, iloadm1Node, ixloadNode->getSize());
            ti.set(&targetList);
            t = ti.getFirst();
            ...
            if (VERBOSE(trans))
                printf("%s moveStoreOutOfLoopForward\n", trans->getT()->getTitle());
            logprintf(disptrace, log, "moveStoreOutOfLoopForward adds %d into compensation code [0] and [1]\n",
                t->getID());
            for (; t; t = ti.getNext())
                t->setIsNegligible(); // set negligible to all stores
        } else
            success0 = false;
    }
...
//*****************************************************************************************
bool IANDSpecialNodeTransformer(TR_CISCTransformer *trans)
{
    TR_CISCGraph *P = trans->getP();
    List<TR_CISCNode> *P2T = trans->getP2T();
    TR::Compilation *comp = trans->comp();
    ...
    for (idx = 0; idx < MAX_SPECIALCARE_NODES; idx++) {
        ...
        TR_CISCNode *t;
        for (t = ti.getFirst(); t; t = ti.getNext()) {
            if (t->getOpcode() != TR::i2b)
                continue; // not implemented yet for other OPs
            ...
            switch (ch->getOpcode()) {
                case TR::iand:
                    if (!ch->getParents()->isSingleton() || !testIConst(ch, 1, 0xFF))
                        return false; // child(1) is "iconst 0xff"
                    ch->setIsNegligible(); // this IAND can be negligible!
                    ret = true;

                    ch = ch->getChild(0);
                    if (ch->getOpcode() != TR::ishr && ch->getOpcode() != TR::iushr)
                        break;
                    // fall through if TR::ishr
                case TR::ishr:
                case TR::iushr:
                    if (!testIConst(ch, 1, 0x8))
                        break; // child(1) is "iconst 0x8"

                    ch = ch->getChild(0);
                    if (ch->getOpcode() != TR::iand)
                        break;
                    if (!ch->getParents()->isSingleton() || !testIConst(ch, 1, 0xFF00))
                        return false; // child(1) is "iconst 0xFF00"
...
static void findIndexLoad(TR::Node *aiaddNode, TR::Node *&index1, TR::Node *&index2, TR::Node *&topLevelIndex)
{
    // iloadi
    //      aiadd              <-- aiaddNode
    //           aload
...
//
static bool usedInLoopTest(TR::Compilation *comp, TR::Node *loopTestNode, TR::SymbolReference *srcSymRef)
{
    ...
    if (ivNode->getOpCode().hasSymbolReference()) {
        ...
        dumpOptDetails(comp, "iv %p in the loop test %p has no symRef?\n", ivNode, loopTestNode);
    return false;
}

static bool indexContainsArray(TR::Compilation *comp, TR::Node *index, vcount_t visitCount)
{
    bool trace = comp->trace(OMR::idiomRecognition);
    ...
    logprintf(trace, log, "analyzing node %p\n", index);

    if (index->getOpCode().hasSymbolReference() && index->getSymbolReference()->getSymbol()->isArrayShadowSymbol()) {
        logprintf(trace, log, "found array node %p\n", index);
        return true;
    }
...
static bool indexContainsArrayAccess(TR::Compilation *comp, TR::Node *aXaddNode)
{
    bool trace = comp->trace(OMR::idiomRecognition);
    OMR::Logger *log = comp->log();
    logprintf(trace, log, "axaddnode %p\n", aXaddNode);
    ...
    //
    logprintf(trace, log, "aXaddNode %p topLevelIndex %p\n", aXaddNode, topLevelIndex);
...
//                other element.
static bool isIndexVariableInList(TR::Node *node, List<TR::Node> *nodeList)
{
    TR::Symbol *indexSymbol1 = NULL, *indexSymbol2 = NULL;
    TR::Node *loadNode1, *loadNode2, *topLevelIndex;

    findIndexLoad(node->getOpCode().isAdd() ? node : node->getFirstChild(), loadNode1, loadNode2, topLevelIndex);
    if (loadNode1)
...
//
static bool indicesAndStoresAreConsistent(TR::Compilation *comp, TR::Node *lhsSrcNode, TR::Node *rhsSrcNode,
    TR_CISCNode *lhsNode, TR_CISCNode *rhsNode)
{
    ...
    return (isIndexVariableInList(lhsSrcNode, &variableList) && isIndexVariableInList(rhsSrcNode, &variableList));
}
...
static bool areArraysInvariant(TR::Compilation *comp, TR::Node *inputNode, TR::Node *outputNode, TR_CISCGraph *T)
{
    bool trace = comp->trace(OMR::idiomRecognition);
    OMR::Logger *log = comp->log();
    if (T) {
        ...
        logprintf(trace, log, "aNode = %p bNode = %p\n", aNode, bNode);
        if (aNode && aNode->getOpCode().isLoadDirect() && bNode && bNode->getOpCode().isLoadDirect()) {
            TR_CISCNode *aCNode = T->getCISCNode(aNode);
            TR_CISCNode *bCNode = T->getCISCNode(bNode);

            logprintf(trace, log, "aC = %p %d bC = %p %d\n", aCNode, aCNode->getID(), bCNode, bCNode->getID());
            if (aCNode && bCNode) {
                ...
                for (ch = aDefI.getFirst(); ch; ch = aDefI.getNext()) {
                    if (ch->getDagID() == aCNode->getDagID()) {
                        logprintf(trace, log, "def %d found inside loop for %d\n", ch->getID(), aCNode->getID());
                        return false;
                    }
                }
...
public:
    AutoLoopInvarianceInfo(TR::Compilation *comp, TR_UseDefInfo *ud, TR_RegionStructure *loop);
...
AutoLoopInvarianceInfo::AutoLoopInvarianceInfo(TR::Compilation *comp, TR_UseDefInfo *ud, TR_RegionStructure *loop)
    : _comp(comp)
...
void AutoLoopInvarianceInfo::findAutoStoresAndLoads(TR_RegionStructure *region, TR::NodeChecklist &visited)
{
...
{
    if (node->getOpCode().isLoadVarDirect() && node->getSymbol()->isAutoOrParm()) {
        TR_ASSERT_FATAL_WITH_NODE(node, _autoLoads.contains(node), "expected auto load to be in the loop");
        ...
        if (_storedAutos.isSet(node->getSymbolReference()->getReferenceNumber())) {
            ...
            TR_ASSERT_FATAL_WITH_NODE(node, cursor.Valid(), "single def missing from cursor");

            int32_t defIndex = cursor;
            TR_ASSERT_FATAL_WITH_NODE(node, defIndex >= _useDefInfo->getFirstRealDefIndex(),
                "despite in-loop definition, param reaches this use");

            return invariantExprFromDef(_useDefInfo->getNode(defIndex));
        }
    } else if (node->getOpCode().hasSymbolReference() && node->getOpCodeValue() != TR::loadaddr) {
    ...
    auto memoInsertResult = _invariantExprMemo.insert(std::make_pair(node, (TR::Node *)NULL));
...
{
    TR_ASSERT_FATAL_WITH_NODE(defNode, _autoStores.contains(defNode), "expected an auto store in the loop");

    TR_ASSERT_FATAL_WITH_NODE(defNode, !_defsOnStack.contains(defNode), "circular single-definition dependency");
...
//
static TR::Node *areDefsOnlyInsideLoop(TR::Compilation *comp, TR_CISCTransformer *trans, TR::Node *outputNode)
{
    bool extraTrace = DISPTRACE(trans);
    OMR::Logger *log = trans->comp()->log();

    logprintf(extraTrace, log, "finding defs for index used in tree %p\n", outputNode);
    ...
    if (info) {
        ...
        if (loadNode1 && loadNode2)
            return NULL; // Try to keep the original semantics, but it may be too strict.
        ...
        if (loadNode) {
            ...
            if (!defs.IsZero()) {
                ...
                if (numDefs == 1) {
                    ...
                    logprintf(extraTrace, log, "found single def %p for load %p\n", defTT->getNode(), loadNode);
...
static void findIndVarLoads(TR::Node *node, TR::Node *indVarStoreNode, bool &storeFound, List<TR::Node> *ivLoads,
    TR::Symbol *ivSym, vcount_t visitCount)
{
    ...
    if (node->getOpCodeValue() == TR::iload && node->getSymbolReference()->getSymbol() == ivSym) {
...
static int32_t checkForPostIncrement(TR::Compilation *comp, TR::Block *loopHeader, TR::Node *loopCmpNode,
    TR::Symbol *ivSym)
{
    ...
    logprintf(comp->trace(OMR::idiomRecognition), comp->log(), "found storeIvload %p cmpFirstChild %p\n", storeIvLoad,
        cmpFirstChild);
...
static bool checkByteToChar(TR::Compilation *comp, TR::Node *iorNode, TR::Node *&inputNode, bool bigEndian)
{
    ...
    if ((imulNode->getOpCodeValue() != TR::imul) && (imulNode->getOpCodeValue() != TR::ishl))
    ...
    if ((imulNode->getOpCodeValue() == TR::imul) || (imulNode->getOpCodeValue() == TR::ishl)) {
        ...
        if (bloadiNode->getOpCodeValue() == TR::bloadi) {
            ...
            if (subNode->getOpCode().isSub() && subNode->getSecondChild()->getOpCode().isLoadConst()) {
                int32_t constVal;
...
static bool ivIncrementedBeforeBoolTableExit(TR::Compilation *comp, TR::Node *boolTableExit, TR::Block *entryBlock,
    TR::SymbolReference *ivSymRef)
{
    ...
    for (TR::TreeTop *tt = startTree; tt != entryBlock->getExit(); tt = tt->getNextTreeTop()) {
        ...
        if (n->getOpCode().isStoreDirect() && (n->getSymbolReference()->getSymbol() == ivSymRef->getSymbol()))
            ivStore = n;
    }
...
//*****************************************************************************************
bool defaultSpecialNodeTransformer(TR_CISCTransformer *trans)
{
...
//*****************************************************************************************
bool MEMCPYSpecialNodeTransformer(TR_CISCTransformer *trans)
{
...
//*****************************************************************************************
bool TRTSpecialNodeTransformer(TR_CISCTransformer *trans)
{
...
bool CISCTransform2FindBytes(TR_CISCTransformer *trans)
{
    ...
    if (isLoopPreheaderLastBlockInMethod(comp, block, disptrace)) {
        logprints(disptrace, log,
            "Bailing CISCTransform2FindBytes due to null TT - might be a preheader in last block of method\n");
        return false;
    }
    ...
    if (exitIfRep) {
        if (exitIfRep != trans->getP2TInLoopIfSingle(P->getImportantNode(1))) {
            logprints(disptrace, log, "Give up because of multiple candidates of ificmpge.\n");
            return false;
        }
        bool isDecrement;
        if (!testExitIF(exitIfRep->getOpcode(), &isDecrement, &modLength))
            return false;
        if (isDecrement)
            return false;
    }
    ...
    if (!target) // multiple successors
    {
        ...
        if (listT->isEmpty() || trans->getNumOfBBlistSucc() != 2) {
            logprints(disptrace, log,
                "Currently, CISCTransform2FindBytes allows only the case where there is an ificmpge node and successor "
                "is 2.\n");
            return false;
        }
    }
    ...
    TR_ASSERT(trans->getP2TRepInLoop(P->getImportantNode(3))->getIlOpCode().isByte()
            || trans->getP2TRepInLoop(P->getImportantNode(3))->getIlOpCode().isShort()
                && trans->getP2TRepInLoop(P->getImportantNode(3))->getIlOpCode().isUnsigned(),
        "Error");
    isTRT2Char = trans->getP2TRepInLoop(P->getImportantNode(3))->getIlOpCode().isShort()
        && trans->getP2TRepInLoop(P->getImportantNode(3))->getIlOpCode().isUnsigned();
    ...
    if (!isTRT2Char) {
        if ((count = trans->analyzeByteBoolTable(pBoolTable, tmpTable, P->getImportantNode(1), &retSameExit)) <= 0) {
            ...
            if (!go) {
                logprints(disptrace, log, "analyzeByteBoolTable failed.\n");
        ...
        if ((count = trans->analyzeCharBoolTable(pBoolTable, tmpTable, P->getImportantNode(1), &retSameExit)) <= 0) {
            ...
            if (supportsSRSTU && // Confirm that the processor has the SRSTU instruction.
                ...
                logprints(disptrace, log, "analyzeCharBoolTable failed.\n");
                return false; // fail to analyze
            }
        } else {
            if (!supportsSRSTU || // If we don't have SRSTU support, we can use SRST/TRT for single byte searches if
                                  // delimiters are within 1-255.
                count != 1) // If we only have 1 constant delimiter and have SRSTU support, we can search for any 2-byte
                            // delimiter.
            {
                if (disptrace && count > 1)
                    log->prints("Multiple exit conditions for a char array. We can implement this case using the TRTE "
                                "instruction on z6.\n");

                if (tmpTable[0]) {
                    logprints(disptrace, log,
                        "Char array has '0' as an exit condition, loop will not be reduced TRT/SRST (single-byte) "
                        "instruction.\n");
                ...
                for (int32_t i = 256; i < 65536; i++) {
                    if (tmpTable[i]) {
                        logprintf(disptrace, log,
                            "Char array has one of 256 through 65535 (%d) as an exit condition, loop cannot be reduced "
                            "to TRT/SRST (single-byte) instruction.\n",
                            i);
    ...
    {
        logprints(disptrace, log, "Multiple targets for different delimiter checks detected.  Abandoning reduction.\n");
        return false;
    }
    ...
    if (retSameExit != NULL && !isNeedGenIcmpge && retSameExit->getEnclosingBlock() != target) {
        logprintf(disptrace, log,
            "Target for delimiter check (Treetop: %p / Block %d: %p) is different than loop exit block_%d: %p.  "
    ...
    if (ahConstNode && ahConstNode->getOpCode().isLoadConst()) {
        ahValue = (ahConstNode->getType().isInt64() ? (int32_t)ahConstNode->getLongInt() : ahConstNode->getInt());
        if (ahValue < 0)
            ahValue = -ahValue;
    ...
    if (indexRequiresAdjustment) {
        logprintf(disptrace, log,
            "headerConst node value doesn't equal contiguous array header size %p. Abandoning reduction.\n",
            ahConstNode);
        return false;
    }
    ...
    if (avoidTransformingStringLoops(comp)) {
        logprints(disptrace, log,
            "Abandoning reduction because of functional problems when String compression is enabled in Java 8 SR5\n");
        return false;
    }

    if (count == -1) // single delimiter which is not constant value
    {
        AutoLoopInvarianceInfo inv(trans->comp(), trans->optimizer()->getUseDefInfo(), trans->getCurrentLoop());
        ...
        if (tableNode == NULL) {
            logprintf(disptrace, log,
                ...
                origComparand->getGlobalIndex(), origComparand);
            return false;
        }

        logprintf(disptrace, log, "Single non-constant delimiter found.  Setting %p as tableNode.\n",
    ...
    } else if (count == 1) // single delimiter
    {
        ...
        logprintf(disptrace, log, "Single delimiter found.  Setting 'iconst %d' [%p] as tableNode.\n", i,
            comp->getDebug()->getName(tableNode));
    } else {
        // the static table currently cannot be relocated
        if (comp->compileRelocatableCode()) {
            logprints(disptrace, log, "Abandoning reduction since we can't relocate the static table\n");
            return false;
        }
    ...
    if (indexRequiresAdjustment) {
        ...
        if (ahConstCISCNode) {
            ...
            if (trans->searchNodeInTrees(ahConstCISCNode->getHeadOfTrNode(), indexNode, &indexParentNode, &childNum)) {
                ...
                if (indexLoadNode->getOpCode().isLoadVar() && indexLoadNode->getReferenceCount() > 1)
                    indexNode = indexLoadNode;
            }
        }

        int32_t width = isTRT2Char ? 2 : 1;
        indexNode = TR::Node::create(TR::isub, 2, indexNode,
    ...
    if (listT->isEmpty()) {
        ...
        logprints(disptrace, log, "Loop has TR::ificmpge for comparing the index.\n");
        TR_CISCNode *lenNode;
        ...
        if (modLength)
            lenRepNode
                = createOP2(comp, TR::isub, lenRepNode, TR::Node::create(baseRepNode, TR::iconst, 0, -modLength));
    ...
    if (isNeedGenIcmpge) {
        logprints(disptrace, log,
            "Now assuming that all exits of booltable are identical and the exit of icmpge points different.\n");
        TR_ASSERT(icmpgeRepInfo, "Not implemented yet"); // current restriction
        okDest = retSameExit;
        failDest = icmpgeCISCnode->getDestination();
        // create two empty blocks for inserting compensation code (base[index] and base[index-1]) prepared by
        // moveStoreOutOfLoopForward()
        if (isCompensateCode) {
        ...
        logprintf(disptrace, log, "Block: okDest=%d failDest=%d\n", okDest->getEnclosingBlock()->getNumber(),
        ...
        icmpgeNode = TR::Node::createif(TR::ificmplt,
            TR::Node::createWithSymRef(indexRepNode, TR::iload, 0, indexVarSymRef), lenRepNode, okDest);
    }
    ...
    if (listT->isEmpty()) // no NULLCHK
    {
        ...
        logprints(disptrace, log, "NULLCHK is found!\n");
    ...
    if (!isNeedGenIcmpge) {
        ...
        trans->setSuccessorEdges(block, failDest->getEnclosingBlock(), okDest->getEnclosingBlock());
    }
...
TR_PCISCGraph *makeTRTGraph(TR::Compilation *c, int32_t ctrl)
{
    ...
    TR_PCISCNode *byteArray
        = new (PERSISTENT_NEW) TR_PCISCNode(c->trMemory(), TR_arraybase, TR::NoType, tgt->incNumNodes(), 8, 0, 0, 0);
    tgt->addNode(byteArray); // array base
    TR_PCISCNode *iv
        = new (PERSISTENT_NEW) TR_PCISCNode(c->trMemory(), TR_variable, TR::NoType, tgt->incNumNodes(), 7, 0, 0, 0);
    tgt->addNode(iv); // array index
    TR_PCISCNode *end
        = new (PERSISTENT_NEW) TR_PCISCNode(c->trMemory(), TR_quasiConst2, TR::NoType, tgt->incNumNodes(), 6, 0, 0);
    tgt->addNode(end); // length (optional)
    TR_PCISCNode *aHeader
        = new (PERSISTENT_NEW) TR_PCISCNode(c->trMemory(), TR_ahconst, TR::NoType, tgt->incNumNodes(), 5, 0, 0, 0);
    tgt->addNode(aHeader); // array header
    TR_PCISCNode *increment
        = new (PERSISTENT_NEW) TR_PCISCNode(c->trMemory(), TR::iconst, TR::Int32, tgt->incNumNodes(), 4, 0, 0, -1);
    tgt->addNode(increment);
    TR_PCISCNode *mulFactor
        = new (PERSISTENT_NEW) TR_PCISCNode(c->trMemory(), TR_allconst, TR::NoType, tgt->incNumNodes(), 3, 0, 0);
    tgt->addNode(mulFactor); // Multiply Factor
    TR_PCISCNode *entry
        = new (PERSISTENT_NEW) TR_PCISCNode(c->trMemory(), TR_entrynode, TR::NoType, tgt->incNumNodes(), 2, 1, 0);
    tgt->addNode(entry);
    TR_PCISCNode *nullChk = new (PERSISTENT_NEW)
        TR_PCISCNode(c->trMemory(), TR::NULLCHK, TR::NoType, tgt->incNumNodes(), 1, 1, 1, entry, byteArray);
    tgt->addNode(nullChk); // optional
    TR_PCISCNode *arrayLen = new (PERSISTENT_NEW)
        TR_PCISCNode(c->trMemory(), TR::arraylength, TR::NoType, tgt->incNumNodes(), 1, 1, 1, nullChk, byteArray);
    tgt->addNode(arrayLen);
    TR_PCISCNode *bndChk = new (PERSISTENT_NEW)
        TR_PCISCNode(c->trMemory(), TR::BNDCHK, TR::NoType, tgt->incNumNodes(), 1, 1, 2, arrayLen, arrayLen, iv);
    tgt->addNode(bndChk);
    TR_PCISCNode *arrayLoad
        = createIdiomArrayLoadInLoop(tgt, ctrl, 1, bndChk, TR_ibcload, TR::NoType, byteArray, iv, aHeader, mulFactor);
    TR_PCISCNode *b2iNode = new (PERSISTENT_NEW)
        TR_PCISCNode(c->trMemory(), TR_conversion, TR::NoType, tgt->incNumNodes(), 1, 1, 1, arrayLoad, arrayLoad);
    tgt->addNode(b2iNode);
    TR_PCISCNode *boolTable = new (PERSISTENT_NEW)
        TR_PCISCNode(c->trMemory(), TR_booltable, TR::NoType, tgt->incNumNodes(), 1, 2, 1, b2iNode, b2iNode);
    tgt->addNode(boolTable);
    TR_PCISCNode *ivStore = createIdiomDecVarInLoop(tgt, ctrl, 1, boolTable, iv, increment);
    TR_PCISCNode *loopTest = new (PERSISTENT_NEW)
        TR_PCISCNode(c->trMemory(), TR_ifcmpall, TR::NoType, tgt->incNumNodes(), 1, 2, 2, ivStore, iv, end);
    tgt->addNode(loopTest); // optional
    TR_PCISCNode *exit
        = new (PERSISTENT_NEW) TR_PCISCNode(c->trMemory(), TR_exitnode, TR::NoType, tgt->incNumNodes(), 0, 0, 0);
    tgt->addNode(exit);
...
TR_PCISCGraph *makeTRTGraph2(TR::Compilation *c, int32_t ctrl)
{
    ...
    /*******************************************************************     opc               id        dagId #cfg
     * #child other/pred/children */
    TR_PCISCNode *v0
        = new (PERSISTENT_NEW) TR_PCISCNode(c->trMemory(), TR_arraybase, TR::NoType, tgt->incNumNodes(), 9, 0, 0, 0);
    tgt->addNode(v0); // array base
    TR_PCISCNode *v1
        = new (PERSISTENT_NEW) TR_PCISCNode(c->trMemory(), TR_variable, TR::NoType, tgt->incNumNodes(), 8, 0, 0, 0);
    tgt->addNode(v1); // array index
    TR_PCISCNode *corv
        = new (PERSISTENT_NEW) TR_PCISCNode(c->trMemory(), TR_quasiConst2, TR::NoType, tgt->incNumNodes(), 7, 0, 0);
    tgt->addNode(corv); // length (optional)
    TR_PCISCNode *alen
        = new (PERSISTENT_NEW) TR_PCISCNode(c->trMemory(), TR_quasiConst2, TR::NoType, tgt->incNumNodes(), 6, 0, 0);
    tgt->addNode(alen); // arraylength (optional)
    TR_PCISCNode *cmah
        = new (PERSISTENT_NEW) TR_PCISCNode(c->trMemory(), TR_ahconst, TR::NoType, tgt->incNumNodes(), 5, 0, 0, 0);
    tgt->addNode(cmah); // array header
    TR_PCISCNode *cm1
        = new (PERSISTENT_NEW) TR_PCISCNode(c->trMemory(), TR::iconst, TR::Int32, tgt->incNumNodes(), 4, 0, 0, -1);
    tgt->addNode(cm1);
    TR_PCISCNode *mulFactor
    ...
    TR_PCISCNode *ent
        = new (PERSISTENT_NEW) TR_PCISCNode(c->trMemory(), TR_entrynode, TR::NoType, tgt->incNumNodes(), 2, 1, 0);
    tgt->addNode(ent);
    TR_PCISCNode *nchk = new (PERSISTENT_NEW)
        TR_PCISCNode(c->trMemory(), TR::NULLCHK, TR::NoType, tgt->incNumNodes(), 1, 1, 1, ent, v0);
    tgt->addNode(nchk); // optional
    TR_PCISCNode *bck = new (PERSISTENT_NEW)
        TR_PCISCNode(c->trMemory(), TR::BNDCHK, TR::NoType, tgt->incNumNodes(), 1, 1, 2, nchk, alen, v1);
    tgt->addNode(bck); // optional
    TR_PCISCNode *n2 = createIdiomArrayLoadInLoop(tgt, ctrl, 1, bck, TR_ibcload, TR::NoType, v0, v1, cmah, mulFactor);
    TR_PCISCNode *n3 = new (PERSISTENT_NEW)
        TR_PCISCNode(c->trMemory(), TR_conversion, TR::NoType, tgt->incNumNodes(), 1, 1, 1, n2, n2);
    tgt->addNode(n3);
    TR_PCISCNode *n4 = new (PERSISTENT_NEW)
        TR_PCISCNode(c->trMemory(), TR_booltable, TR::NoType, tgt->incNumNodes(), 1, 2, 1, n3, n3);
    tgt->addNode(n4);
    TR_PCISCNode *n6 = createIdiomDecVarInLoop(tgt, ctrl, 1, n4, v1, cm1);
    TR_PCISCNode *n7 = new (PERSISTENT_NEW)
        TR_PCISCNode(c->trMemory(), TR_ifcmpall, TR::NoType, tgt->incNumNodes(), 1, 2, 2, n6, v1, corv);
    tgt->addNode(n7); // optional
    TR_PCISCNode *n8
        = new (PERSISTENT_NEW) TR_PCISCNode(c->trMemory(), TR_exitnode, TR::NoType, tgt->incNumNodes(), 0, 0, 0);
    tgt->addNode(n8);
...
****************************************************************************************/
TR_PCISCGraph *makeTRT2ByteGraph(TR::Compilation *c, int32_t ctrl)
{
    TR_PCISCGraph *tgt = new (PERSISTENT_NEW) TR_PCISCGraph(c->trMemory(), "TRT2Byte", 0, 16);
    /****************************************************************************    opc               id        dagId
     * #cfg #child other/pred/children */
    TR_PCISCNode *charArray
        = new (PERSISTENT_NEW) TR_PCISCNode(c->trMemory(), TR_arraybase, TR::NoType, tgt->incNumNodes(), 9, 0, 0, 0);
    tgt->addNode(charArray); // array base
    TR_PCISCNode *iv
        = new (PERSISTENT_NEW) TR_PCISCNode(c->trMemory(), TR_variable, TR::NoType, tgt->incNumNodes(), 8, 0, 0, 1);
    tgt->addNode(iv); // array index
    TR_PCISCNode *end
        = new (PERSISTENT_NEW) TR_PCISCNode(c->trMemory(), TR_quasiConst2, TR::NoType, tgt->incNumNodes(), 7, 0, 0);
    tgt->addNode(end); // length (optional)
    TR_PCISCNode *arrayLen
        = new (PERSISTENT_NEW) TR_PCISCNode(c->trMemory(), TR_quasiConst2, TR::NoType, tgt->incNumNodes(), 6, 0, 0);
    ...
    tgt->addNode(entry);
    TR_PCISCNode *nullChk = new (PERSISTENT_NEW)
        TR_PCISCNode(c->trMemory(), TR::NULLCHK, TR::NoType, tgt->incNumNodes(), 1, 1, 1, entry, charArray);
    tgt->addNode(nullChk); // optional
    TR_PCISCNode *bndChk = new (PERSISTENT_NEW)
        TR_PCISCNode(c->trMemory(), TR::BNDCHK, TR::NoType, tgt->incNumNodes(), 1, 1, 2, nullChk, arrayLen, iv);
    tgt->addNode(bndChk); // optional
    TR_PCISCNode *arrayLoad
        = createIdiomArrayLoadInLoop(tgt, ctrl, 1, bndChk, TR::sloadi, TR::Int16, charArray, iv, aHeader, mulFactor);
    TR_PCISCNode *c2iNode = new (PERSISTENT_NEW)
        TR_PCISCNode(c->trMemory(), TR::su2i, TR::Int32, tgt->incNumNodes(), 1, 1, 1, arrayLoad, arrayLoad);
    tgt->addNode(c2iNode);
    TR_PCISCNode *boolTable = new (PERSISTENT_NEW)
        TR_PCISCNode(c->trMemory(), TR_booltable, TR::NoType, tgt->incNumNodes(), 1, 2, 1, c2iNode, c2iNode);
    tgt->addNode(boolTable);
    TR_PCISCNode *ivStore = createIdiomDecVarInLoop(tgt, ctrl, 1, boolTable, iv, increment);
    TR_PCISCNode *loopTest = new (PERSISTENT_NEW)
        TR_PCISCNode(c->trMemory(), TR_ifcmpall, TR::NoType, tgt->incNumNodes(), 1, 2, 2, ivStore, iv, end);
...
//*****************************************************************************************
bool CISCTransform2NestedArrayFindBytes(TR_CISCTransformer *trans)
{
    TR_ASSERT(trans->getOffsetOperand1() == 0 && trans->getOffsetOperand2() == 0, "Not implemented yet");
    ...
    if (isLoopPreheaderLastBlockInMethod(comp, block, disptrace)) {
        logprints(disptrace, log,
            "Bailing CISCTransform2NestedArrayFindBytes due to null TT - might be a preheader in last block of "
            "method\n");
        return false;
    }
    ...
    if (!isMapDirectlyUsed) {
        // TODO: To make this work on non-Java environments, the table should be in the code cache, not persistent
        // memory
    ...
    if (listT->isEmpty()) {
        ...
        logprints(disptrace, log, "TR::ificmpge for comaring the index is found!\n");
        TR_CISCNode *lenNode;
        TR::Node *lenRepNode;
        if (listT->isSingleton()) {
...
TR_PCISCGraph *makeTRT4NestedArrayGraph(TR::Compilation *c, int32_t ctrl)
{
    ...
    TR_PCISCNode *iv
        = new (PERSISTENT_NEW) TR_PCISCNode(c->trMemory(), TR_variable, TR::NoType, tgt->incNumNodes(), 8, 0, 0, 0);
    tgt->addNode(iv); // array index
    TR_PCISCNode *mapArray
        = new (PERSISTENT_NEW) TR_PCISCNode(c->trMemory(), TR_variable, TR::NoType, tgt->incNumNodes(), 7, 0, 0, 1);
    tgt->addNode(mapArray); // outer array base
    TR_PCISCNode *end
        = new (PERSISTENT_NEW) TR_PCISCNode(c->trMemory(), TR_quasiConst2, TR::NoType, tgt->incNumNodes(), 6, 0, 0);
    tgt->addNode(end); // length (optional)
    TR_PCISCNode *aHeader
    ...
    TR_PCISCNode *bALoad
        = createIdiomArrayLoadInLoop(tgt, ctrl, 1, bndChk, TR_ibcload, TR::NoType, byteArray, iv, aHeader, mulFactor);
    TR_PCISCNode *bu2iNode = new (PERSISTENT_NEW)
        TR_PCISCNode(c->trMemory(), TR_conversion, TR::NoType, tgt->incNumNodes(), 1, 1, 1, bALoad, bALoad);
    tgt->addNode(bu2iNode);
    TR_PCISCNode *mapAload = createIdiomArrayLoadInLoop(tgt, ctrl, 1, bu2iNode, TR_ibcload, TR::NoType, mapArray,
        bu2iNode, aHeader, mulFactor);
    TR_PCISCNode *b2iNode = new (PERSISTENT_NEW)
        TR_PCISCNode(c->trMemory(), TR_conversion, TR::NoType, tgt->incNumNodes(), 1, 1, 1, mapAload, mapAload);
    tgt->addNode(b2iNode);
    TR_PCISCNode *boolTable = new (PERSISTENT_NEW)
        TR_PCISCNode(c->trMemory(), TR_booltable, TR::NoType, tgt->incNumNodes(), 1, 2, 1, b2iNode, b2iNode);
    tgt->addNode(boolTable);
    TR_PCISCNode *ivStore = createIdiomDecVarInLoop(tgt, ctrl, 1, boolTable, iv, increment);
    TR_PCISCNode *loopTest = new (PERSISTENT_NEW)
        TR_PCISCNode(c->trMemory(), TR::ificmpge, TR::NoType, tgt->incNumNodes(), 1, 2, 2, ivStore, iv, end);
    tgt->addNode(loopTest); // optional
    TR_PCISCNode *exit
        = new (PERSISTENT_NEW) TR_PCISCNode(c->trMemory(), TR_exitnode, TR::NoType, tgt->incNumNodes(), 0, 0, 0);
    tgt->addNode(exit);
...
bool CISCTransform2NestedArrayIfFindBytes(TR_CISCTransformer *trans)
{
...
TR_PCISCGraph *makeTRT4NestedArrayIfGraph(TR::Compilation *c, int32_t ctrl)
{
     ...
     * #child other/pred/children */
    TR_PCISCNode *v0
        = new (PERSISTENT_NEW) TR_PCISCNode(c->trMemory(), TR_arraybase, TR::NoType, tgt->incNumNodes(), 10, 0, 0, 0);
    tgt->addNode(v0); // array base
    TR_PCISCNode *v1
        = new (PERSISTENT_NEW) TR_PCISCNode(c->trMemory(), TR_variable, TR::NoType, tgt->incNumNodes(), 9, 0, 0, 0);
    tgt->addNode(v1); // array index
    TR_PCISCNode *v2
        = new (PERSISTENT_NEW) TR_PCISCNode(c->trMemory(), TR_variable, TR::NoType, tgt->incNumNodes(), 8, 0, 0, 1);
    tgt->addNode(v2); // outer array base
    TR_PCISCNode *corv
        = new (PERSISTENT_NEW) TR_PCISCNode(c->trMemory(), TR_quasiConst2, TR::NoType, tgt->incNumNodes(), 7, 0, 0);
    tgt->addNode(corv); // length (optional)
    TR_PCISCNode *cmah
        = new (PERSISTENT_NEW) TR_PCISCNode(c->trMemory(), TR_ahconst, TR::NoType, tgt->incNumNodes(), 6, 0, 0, 0);
    tgt->addNode(cmah); // array header
    TR_PCISCNode *cm1
        = new (PERSISTENT_NEW) TR_PCISCNode(c->trMemory(), TR::iconst, TR::Int32, tgt->incNumNodes(), 5, 0, 0, -1);
    tgt->addNode(cm1);
    TR_PCISCNode *cm0
        = new (PERSISTENT_NEW) TR_PCISCNode(c->trMemory(), TR::iconst, TR::Int32, tgt->incNumNodes(), 4, 0, 0, 0);
    tgt->addNode(cm0);
    TR_PCISCNode *mulFactor
        = new (PERSISTENT_NEW) TR_PCISCNode(c->trMemory(), TR_allconst, TR::NoType, tgt->incNumNodes(), 3, 0, 0);
    tgt->addNode(mulFactor); // Multiply Factor
    ...
    TR_PCISCNode *nchk = new (PERSISTENT_NEW)
        TR_PCISCNode(c->trMemory(), TR::NULLCHK, TR::NoType, tgt->incNumNodes(), 1, 1, 1, ent, v0);
    tgt->addNode(nchk); // optional
    TR_PCISCNode *alen = new (PERSISTENT_NEW)
        TR_PCISCNode(c->trMemory(), TR::arraylength, TR::NoType, tgt->incNumNodes(), 1, 1, 1, nchk, v0);
    tgt->addNode(alen);
    TR_PCISCNode *bck = new (PERSISTENT_NEW)
        TR_PCISCNode(c->trMemory(), TR::BNDCHK, TR::NoType, tgt->incNumNodes(), 1, 1, 2, alen, alen, v1);
    tgt->addNode(bck);
    TR_PCISCNode *n2 = createIdiomArrayLoadInLoop(tgt, ctrl, 1, bck, TR_ibcload, TR::NoType, v0, v1, cmah, mulFactor);
    TR_PCISCNode *n3
        = new (PERSISTENT_NEW) TR_PCISCNode(c->trMemory(), TR::b2i, TR::Int32, tgt->incNumNodes(), 1, 1, 1, n2, n2);
    tgt->addNode(n3);
    TR_PCISCNode *nif0 = new (PERSISTENT_NEW)
        TR_PCISCNode(c->trMemory(), TR::ificmplt, TR::NoType, tgt->incNumNodes(), 1, 2, 2, n3, n3, cm0);
    tgt->addNode(nif0);
    TR_PCISCNode *nn2 = createIdiomArrayLoadInLoop(tgt, ctrl, 1, nif0, TR_ibcload, TR::NoType, v2, n3, cmah, mulFactor);
    TR_PCISCNode *nn3
        = new (PERSISTENT_NEW) TR_PCISCNode(c->trMemory(), TR::b2i, TR::Int32, tgt->incNumNodes(), 1, 1, 1, nn2, nn2);
    tgt->addNode(nn3);
    TR_PCISCNode *n4 = new (PERSISTENT_NEW)
        TR_PCISCNode(c->trMemory(), TR_booltable, TR::NoType, tgt->incNumNodes(), 1, 2, 1, nn3, nn3);
    tgt->addNode(n4);
    TR_PCISCNode *n6 = createIdiomDecVarInLoop(tgt, ctrl, 1, n4, v1, cm1);
    TR_PCISCNode *n7 = new (PERSISTENT_NEW)
        TR_PCISCNode(c->trMemory(), TR::ificmpge, TR::NoType, tgt->incNumNodes(), 1, 2, 2, n6, v1, corv);
    tgt->addNode(n7); // optional
    TR_PCISCNode *n8
        = new (PERSISTENT_NEW) TR_PCISCNode(c->trMemory(), TR_exitnode, TR::NoType, tgt->incNumNodes(), 0, 0, 0);
    tgt->addNode(n8);
...
bool CISCTransform2CopyingTROx(TR_CISCTransformer *trans)
{
    const bool disptrace = DISPTRACE(trans);
    TR::Node *trNode;
    TR::TreeTop *trTreeTop;
    ...
    TR::Compilation *comp = trans->comp();
    OMR::Logger *log = comp->log();
    bool isOutputChar = trans->getP2TRepInLoop(P->getImportantNode(3))->getIlOpCode().isShort()
        && trans->getP2TRepInLoop(P->getImportantNode(3))->getIlOpCode().isUnsigned();
    ...
    if (!isOutputChar && genSIMD && !genTRxx) {
        logprints(disptrace, log, "Bailing CISCTransform2CopyingTROx : b2b - no proper evaluator available\n");
        return false;
    }
    ...
    if (isOutputChar) {
        ...
        if (sx == TR_maybe) {
            logprints(disptrace, log, "Bailing CISCTransform2CopyingTROx : unknown integer conversion\n");
            return false;
        }
    ...
    if (additionHigh) {
        ...
        if (additionHigh->getChild(0) != loadResult && additionHigh->getChild(1) != loadResult) {
            logprints(disptrace, log, "Bailing CISCTransform2CopyingTROx : inscrutable iadd\n");
            return false;
        }
    }
    ...
    */
    logprintf(disptrace, log, "Found graph pattern as %d\n", pattern);
    ...
    if (isLoopPreheaderLastBlockInMethod(comp, block, disptrace)) {
        logprints(disptrace, log,
            "Bailing CISCTransform2CopyingTROx due to null TT - might be a preheader in last block of method\n");
        return false;
    }
    ...
    if (trans->countGoodArrayIndex(indexVarSymRef) == 0) {
        logprintf(disptrace, log, "countGoodArrayIndex failed for %p\n", indexRepNode);
        return false;
    }
    ...
    if (dstIndexRepNode)
        variableList.add(dstIndexRepNode);
    if (!isIndexVariableInList(inputNode, &variableList) || !isIndexVariableInList(outputNode, &variableList)) {
        dumpOptDetails(comp,
            "indices used in array loads %p and %p are not consistent with the induction varaible updates\n", inputNode,
            outputNode);
        return false;
    }
    TR::SymbolReference *indexDiffVarSymRef
        = (indexDiffRepNode->getOpCode().isLoadVarOrStore() && !indexDiffRepNode->getOpCode().isIndirect())
        ? indexDiffRepNode->getSymbolReference()
        : NULL;
    TR::Node *ignoreTree = dstIndexVarSymRef && indexDiffVarSymRef && indexVarSymRef
        ? createStoreOP2(comp, dstIndexVarSymRef, TR::iadd, indexVarSymRef, indexDiffVarSymRef, trNode)
        : NULL;
    ...
    if (!target) // multiple successors
    {
        ...
        if (trans->getNumOfBBlistSucc() != 2) {
            logprintf(disptrace, log, "current restrictions. The number of successors is %d\n",
    ...
    int32_t numBoolTableTestChars
        = trans->analyzeByteBoolTable(P->getImportantNode(0), tmpTable, P->getImportantNode(1), &retSameExit);
    ...
    if (0 && boolTableExit) {
        ...
        logprintf(disptrace, log, "boolTableNode : %p of loop %d\n", boolTableNode, block->getNumber());
    ...
    if (comp->cg()->getSupportsArrayTranslateTROTNoBreak() || comp->cg()->getSupportsArrayTranslateTROT()) {
        ...
        if (!isOutputChar) {
            logprints(disptrace, log, "failed because of reason 1 %\n");
            return false;
        }
        if (comp->cg()->getSupportsArrayTranslateTROTNoBreak()) {
            ...
            if (foundLoopToReduce)
                termchar = TERMCHAR; // It needs to be greater than zero, dummy termination char otherwise, i.e., it's
                                     // not gonna be used,
        }
        ...
        //
        if (!foundLoopToReduce) {
            logprints(disptrace, log, "failed because of reason 2\n");
            return false;
        }
        ...
        if (termchar != 0) {
            ...
            if (isSignExtending) {
                logprints(disptrace, log, "Bailing CISCTransform2CopyingTROx due to sign-extension\n");
                return false;
            }
        }
    } else {
        ...
        if (isOutputChar) {
            ...
            if (isSIMDPossible) {
                ...
                logprints(disptrace, log, "Bailing CISCTransform2CopyingTROx: b2c - no proper evaluator available\n");
                return false;
            } else {
                for (int i = 0; i < 256; i++) {
            ...
            if (termchar < 0) {
                logprints(disptrace, log, "No terminating character found. Abandoning reduction.\n");
                return false;
            }
            tableNode = createTableLoad(comp, baseRepNode, 8, 8, tmpTable, disptrace);
        }
        ...
        for (n = li.getFirst(); n; n = li.getNext()) {
            if (trans->getCandidateRegion()->isIncluded(n)) {
                if (icmpgeCISCnode != NULL) {
                    logprintf(disptrace, log, "Bailing CISCTransform2CopyingTROx: multiple loop tests: %d and %d\n",
    ...
    bool isDecrement;
    int32_t modLength;
    if (!testExitIF(icmpgeCISCnode->getOpcode(), &isDecrement, &modLength))
        return false;
    if (isDecrement)
        return false;
    ...
    if (trans->getOffsetOperand1()) {
        ...
        logprintf(disptrace, log, "The array header const of inputNode %p is modified. (offset=%d)\n", inputNode,
            trans->getOffsetOperand1());
    }
    if (trans->getOffsetOperand2()) {
        ...
        TR_ASSERT(constLoad, "Not implemented yet");
        logprintf(disptrace, log, "The array header const of outputNode %p is modified. (offset=%d)\n", outputNode,
            offset);
    }
    ...
    if (modLength)
        lenTmpNode = createOP2(comp, TR::isub, lenTmpNode, TR::Node::create(indexRepNode, TR::iconst, 0, -modLength));
    ...
    translateNode->setSourceCellIsTermChar(false);
    translateNode->setTableBackedByRawStorage(true);
    TR::SymbolReference *translateTemp = comp->getSymRefTab()->createTemporary(comp->getMethodSymbol(), TR::Int32);
    ...
    if (dstIndexRepNode) {
        ...
        for (TR_CISCNode *n = ni.getFirst(); n; n = ni.getNext()) {
            if (n->getNumSuccs() >= 1 && n->getSucc(0) && (n->getSucc(0)->getID() == loopTest->getID())) {
                jstore = n;
                break;
            }
        }
        ...
        if (dstIVStore && dstIVStore->getOpCode().hasSymbolReference()
            ...
            if (pattern == 1) {
                TR::Node *dstAddCountNode = createOP2(comp, TR::iadd,
                    TR::Node::createLoad(dstIndexRepNode, dstIndexVarSymRef), translateNode);
                if (dstIvNeedsUpdate)
                    dstAddCountNode
                        = TR::Node::create(TR::iadd, 2, dstAddCountNode, TR::Node::iconst(dstAddCountNode, 1));
                ...
                if (firstChild->getOpCode().isAdd() || firstChild->getOpCode().isSub()) {
                    ...
                    if (!ivLoad->getOpCode().hasSymbolReference() || (ivLoad->getSymbolReference() != indexVarSymRef)) {
    ...
    if (additionHigh) {
        ...
        highCISCNode
            = (additionHigh->getChild(0) == loadResult) ? additionHigh->getChild(1) : additionHigh->getChild(0);
        List<TR::Node> guardList(comp->trMemory());
        guardList.add(
            TR::Node::createif(TR::ificmpne, convertStoreToLoad(comp, highCISCNode->getHeadOfTrNodeInfo()->_node),
    ...
    if (dstIndVarUpdateTreeTop)
        block->append(dstIndVarUpdateTreeTop);
    block = trans->insertAfterNodes(block);
...
static TR_YesNoMaybe isSignExtendingCopyingTROx(TR_CISCTransformer *trans)
{
    ...
    TR_ASSERT(patStoreConv->getOpcode() == TR_conversion || patStoreConv->getIlOpCode().isConversion(),
    ...
    TR_ASSERT(patLoadConv->getOpcode() == TR_conversion || patLoadConv->getIlOpCode().isConversion(),
    ...
    TR_ASSERT(tgtStoreConv != NULL || tgtLoadConv != NULL,
    ...
    if (storeConv == NULL || loadConv == NULL) // only one conversion
    {
        ...
        TR_ASSERT(op.isZeroExtension() || op.isSignExtension(),
    ...
    if (!firstOp.isInteger() && !firstOp.isUnsigned()) {
        logprints(disptrace, comp->log(), "isSignExtendingCopyingTROx: conversion through non-integer type\n");
        return TR_maybe;
    }

    // The first conversion has to be a (zero- or sign-) extension, because Int8
    // is the smallest available integer type.
    TR_ASSERT(firstOp.isZeroExtension() || firstOp.isSignExtension(),
    ...
    TR_ASSERT(!firstOp.isShort(), "isSignExtendingCopyingTROx: first conversion directly to short\n");
...
bool CISCTransform2CopyingTROxAddDest1(TR_CISCTransformer *trans)
{
...
TR_PCISCGraph *makeCopyingTROOSpecialGraph(TR::Compilation *c, int32_t ctrl)
{
     ...
     * #child other/pred/children */
    TR_PCISCNode *v0
        = new (PERSISTENT_NEW) TR_PCISCNode(c->trMemory(), TR_arraybase, TR::NoType, tgt->incNumNodes(), 13, 0, 0, 0);
    tgt->addNode(v0); // src array base
    TR_PCISCNode *v1
        = new (PERSISTENT_NEW) TR_PCISCNode(c->trMemory(), TR_variable, TR::NoType, tgt->incNumNodes(), 12, 0, 0, 0);
    tgt->addNode(v1); // src array index
    TR_PCISCNode *v2
        = new (PERSISTENT_NEW) TR_PCISCNode(c->trMemory(), TR_arraybase, TR::NoType, tgt->incNumNodes(), 11, 0, 0, 1);
    tgt->addNode(v2); // dst array base
    TR_PCISCNode *v3
        = new (PERSISTENT_NEW) TR_PCISCNode(c->trMemory(), TR_variable, TR::NoType, tgt->incNumNodes(), 10, 0, 0, 1);
    tgt->addNode(v3); // dst array index
    TR_PCISCNode *idx0
        = new (PERSISTENT_NEW) TR_PCISCNode(c->trMemory(), TR_arrayindex, TR::NoType, tgt->incNumNodes(), 9, 0, 0, 0);
    tgt->addNode(idx0);
    TR_PCISCNode *idx1
        = new (PERSISTENT_NEW) TR_PCISCNode(c->trMemory(), TR_arrayindex, TR::NoType, tgt->incNumNodes(), 8, 0, 0, 1);
    tgt->addNode(idx1);
    TR_PCISCNode *vorc
        = new (PERSISTENT_NEW) TR_PCISCNode(c->trMemory(), TR_quasiConst2, TR::NoType, tgt->incNumNodes(), 7, 0, 0);
    tgt->addNode(vorc); // length
    TR_PCISCNode *cmah
        = new (PERSISTENT_NEW) TR_PCISCNode(c->trMemory(), TR_ahconst, TR::NoType, tgt->incNumNodes(), 6, 0, 0, 0);
    tgt->addNode(cmah); // array header
    TR_PCISCNode *cm1
    ...
    TR_PCISCNode *n4 = new (PERSISTENT_NEW)
        TR_PCISCNode(c->trMemory(), TR_booltable, TR::NoType, tgt->incNumNodes(), 1, 2, 1, n3, n3);
    tgt->addNode(n4); // optional
    TR_PCISCNode *n5 = createIdiomArrayLoadInLoop(tgt, ctrl, 1, n4, TR::bloadi, TR::Int8, v0, idx0, cmah, mulFactor);
    TR_PCISCNode *nn0 = createIdiomArrayStoreInLoop(tgt, ctrl | CISCUtilCtl_NoConversion, 1, n5, TR::bstorei, TR::Int8,
        v2, idx1, cmah, mulFactor, n5);
    TR_PCISCNode *n6 = createIdiomDecVarInLoop(tgt, ctrl, 1, nn0, v1, cm1);
    TR_PCISCNode *nn1 = new (PERSISTENT_NEW)
        TR_PCISCNode(c->trMemory(), TR::isub, TR::Int32, tgt->incNumNodes(), 1, 1, 2, n6, v3, cm1);
    tgt->addNode(nn1);
    TR_PCISCNode *nn2
        = new (PERSISTENT_NEW) TR_PCISCNode(c->trMemory(), TR::i2s, TR::Int16, tgt->incNumNodes(), 1, 1, 1, nn1, nn1);
    tgt->addNode(nn2); // optional
    TR_PCISCNode *nn3
        = new (PERSISTENT_NEW) TR_PCISCNode(c->trMemory(), TR::s2i, TR::Int32, tgt->incNumNodes(), 1, 1, 1, nn2, nn2);
    tgt->addNode(nn3); // optional
    TR_PCISCNode *nn6 = new (PERSISTENT_NEW)
        TR_PCISCNode(c->trMemory(), TR::istore, TR::Int32, tgt->incNumNodes(), 1, 1, 2, nn3, nn3, v3);
    tgt->addNode(nn6);
    TR_PCISCNode *n7 = new (PERSISTENT_NEW)
        TR_PCISCNode(c->trMemory(), TR_ifcmpall, TR::NoType, tgt->incNumNodes(), 1, 2, 2, nn6, v1, vorc);
    tgt->addNode(n7);
    TR_PCISCNode *n8
        = new (PERSISTENT_NEW) TR_PCISCNode(c->trMemory(), TR_exitnode, TR::NoType, tgt->incNumNodes(), 0, 0, 0);
    tgt->addNode(n8);
...
TR_PCISCGraph *makeCopyingTROxGraph(TR::Compilation *c, int32_t ctrl, int pattern)
{
     ...
     * #cfg #child other/pred/children */
    TR_PCISCNode *byteArray
        = new (PERSISTENT_NEW) TR_PCISCNode(c->trMemory(), TR_arraybase, TR::NoType, tgt->incNumNodes(), 16, 0, 0, 0);
    tgt->addNode(byteArray); // src array base
    TR_PCISCNode *i
        = new (PERSISTENT_NEW) TR_PCISCNode(c->trMemory(), TR_variable, TR::NoType, tgt->incNumNodes(), 15, 0, 0, 0);
    tgt->addNode(i); // src array index
    TR_PCISCNode *charArray
        = new (PERSISTENT_NEW) TR_PCISCNode(c->trMemory(), TR_arraybase, TR::NoType, tgt->incNumNodes(), 14, 0, 0, 1);
    tgt->addNode(charArray); // dst array base
    TR_PCISCNode *j
        = new (PERSISTENT_NEW) TR_PCISCNode(c->trMemory(), TR_variable, TR::NoType, tgt->incNumNodes(), 13, 0, 0, 1);
    tgt->addNode(j); // dst array index
    TR_PCISCNode *idx0
        = new (PERSISTENT_NEW) TR_PCISCNode(c->trMemory(), TR_arrayindex, TR::NoType, tgt->incNumNodes(), 12, 0, 0, 0);
    tgt->addNode(idx0);
    TR_PCISCNode *idx1
        = new (PERSISTENT_NEW) TR_PCISCNode(c->trMemory(), TR_arrayindex, TR::NoType, tgt->incNumNodes(), 11, 0, 0, 1);
    tgt->addNode(idx1);
    TR_PCISCNode *end
        = new (PERSISTENT_NEW) TR_PCISCNode(c->trMemory(), TR_quasiConst2, TR::NoType, tgt->incNumNodes(), 10, 0, 0);
    tgt->addNode(end); // length
    TR_PCISCNode *high
        = new (PERSISTENT_NEW) TR_PCISCNode(c->trMemory(), TR_quasiConst2, TR::NoType, tgt->incNumNodes(), 9, 0, 0);
    tgt->addNode(high); // optional
    TR_PCISCNode *aHeader0
        = new (PERSISTENT_NEW) TR_PCISCNode(c->trMemory(), TR_ahconst, TR::NoType, tgt->incNumNodes(), 8, 0, 0, 0);
    tgt->addNode(aHeader0); // array header
    TR_PCISCNode *aHeader1
        = new (PERSISTENT_NEW) TR_PCISCNode(c->trMemory(), TR_ahconst, TR::NoType, tgt->incNumNodes(), 7, 0, 0, 1);
    tgt->addNode(aHeader1); // array header
    TR_PCISCNode *increment
        = new (PERSISTENT_NEW) TR_PCISCNode(c->trMemory(), TR::iconst, TR::Int32, tgt->incNumNodes(), 6, 0, 0, -1);
    tgt->addNode(increment);
    TR_PCISCNode *lc1 = createIdiomArrayRelatedConst(tgt, ctrl, tgt->incNumNodes(), 5, 1); // element size for input
    TR_PCISCNode *elemSize
        = new (PERSISTENT_NEW) TR_PCISCNode(c->trMemory(), TR_allconst, TR::NoType, tgt->incNumNodes(), 4, 0, 0);
    tgt->addNode(elemSize); // Multiply Factor
    TR_PCISCNode *offset = NULL;
    if (pattern == 0) {
        offset
            = new (PERSISTENT_NEW) TR_PCISCNode(c->trMemory(), TR_quasiConst2, TR::NoType, tgt->incNumNodes(), 3, 0, 0);
        tgt->addNode(offset); // optional
    }
    TR_PCISCNode *entry
        = new (PERSISTENT_NEW) TR_PCISCNode(c->trMemory(), TR_entrynode, TR::NoType, tgt->incNumNodes(), 2, 1, 0);
    tgt->addNode(entry);
    TR_PCISCNode *byteAddr
        = createIdiomArrayLoadInLoop(tgt, ctrl, 1, entry, TR::bloadi, TR::Int8, byteArray, idx0, aHeader0, lc1);
    TR_PCISCNode *b2iNode = new (PERSISTENT_NEW)
        TR_PCISCNode(c->trMemory(), TR_conversion, TR::NoType, tgt->incNumNodes(), 1, 1, 1, byteAddr, byteAddr);
    tgt->addNode(b2iNode);
    TR_PCISCNode *exitTest = new (PERSISTENT_NEW)
        TR_PCISCNode(c->trMemory(), TR_booltable, TR::NoType, tgt->incNumNodes(), 1, 2, 1, b2iNode, b2iNode);
    tgt->addNode(exitTest); // optional
    TR_PCISCNode *add = new (PERSISTENT_NEW)
        TR_PCISCNode(c->trMemory(), TR::iadd, TR::Int32, tgt->incNumNodes(), 1, 1, 2, exitTest, b2iNode, high);
    tgt->addNode(add); // optional
    TR_PCISCNode *charAddr = createIdiomArrayStoreInLoop(tgt, ctrl, 1, add, TR_ibcstore, TR::NoType, charArray, idx1,
        aHeader1, elemSize, add);
    ...
    TR_PCISCNode *loopTest = new (PERSISTENT_NEW)
        TR_PCISCNode(c->trMemory(), TR_ifcmpall, TR::NoType, tgt->incNumNodes(), 1, 2, 2, jStore, i, end);
...
TR_PCISCGraph *makeCopyingTROTInduction1Graph(TR::Compilation *c, int32_t ctrl, int32_t pattern)
{
    ...
    TR_PCISCNode *v3
        = new (PERSISTENT_NEW) TR_PCISCNode(c->trMemory(), TR_variable, TR::NoType, tgt->incNumNodes(), 10, 0, 0, 1);
    tgt->addNode(v3); // actual dst array index (optional)
    TR_PCISCNode *v4
        = new (PERSISTENT_NEW) TR_PCISCNode(c->trMemory(), TR_variable, TR::NoType, tgt->incNumNodes(), 9, 0, 0, 2);
    tgt->addNode(v4); // difference of dst array index from src array index
    TR_PCISCNode *vorc
        = new (PERSISTENT_NEW) TR_PCISCNode(c->trMemory(), TR_quasiConst2, TR::NoType, tgt->incNumNodes(), 8, 0, 0);
    tgt->addNode(vorc); // length
    TR_PCISCNode *cmah0
        = new (PERSISTENT_NEW) TR_PCISCNode(c->trMemory(), TR_ahconst, TR::NoType, tgt->incNumNodes(), 7, 0, 0, 0);
    tgt->addNode(cmah0); // array header
    TR_PCISCNode *cmah1
        = new (PERSISTENT_NEW) TR_PCISCNode(c->trMemory(), TR_ahconst, TR::NoType, tgt->incNumNodes(), 6, 0, 0, 1);
    tgt->addNode(cmah1); // array header
    TR_PCISCNode *cm1
        = new (PERSISTENT_NEW) TR_PCISCNode(c->trMemory(), TR::iconst, TR::Int32, tgt->incNumNodes(), 5, 0, 0, -1);
    tgt->addNode(cm1);
    ...
    TR_PCISCNode *n5 = new (PERSISTENT_NEW)
        TR_PCISCNode(c->trMemory(), TR::iadd, TR::Int32, tgt->incNumNodes(), 1, 1, 2, n45, v1, v4);
    tgt->addNode(n5);
    ...
    TR_PCISCNode *op0 = new (PERSISTENT_NEW)
        TR_PCISCNode(c->trMemory(), TR::isub, TR::Int32, tgt->incNumNodes(), 1, 1, 2, n6, n5, cm1);
    tgt->addNode(op0); // (optional)
    TR_PCISCNode *op1 = new (PERSISTENT_NEW)
        TR_PCISCNode(c->trMemory(), TR::istore, TR::Int32, tgt->incNumNodes(), 1, 1, 2, op0, op0, v3);
    tgt->addNode(op1); // (optional)
    TR_PCISCNode *n7 = new (PERSISTENT_NEW)
        TR_PCISCNode(c->trMemory(), TR_ifcmpall, TR::NoType, tgt->incNumNodes(), 1, 2, 2, op1, v1, vorc);
    tgt->addNode(n7);
    TR_PCISCNode *n8
        = new (PERSISTENT_NEW) TR_PCISCNode(c->trMemory(), TR_exitnode, TR::NoType, tgt->incNumNodes(), 0, 0, 0);
    tgt->addNode(n8);
...
bool CISCTransform2TROTArray(TR_CISCTransformer *trans)
{
    TR_ASSERT(trans->getOffsetOperand1() == 0 && trans->getOffsetOperand2() == 0, "Not implemented yet");
    const bool disptrace = DISPTRACE(trans);
    TR::Node *trNode;
    ...
    if (isLoopPreheaderLastBlockInMethod(comp, block, disptrace)) {
        logprints(disptrace, comp->log(),
            "Bailing CISCTransform2TROTArray due to null TT - might be a preheader in last block of method\n");
        return false;
    }
    ...
    if (!inputCISCNode || !outputCISCNode)
        return false;
    TR::Node *inputNode = inputCISCNode->getHeadOfTrNodeInfo()->_node->duplicateTree();
    TR::Node *outputNode = outputCISCNode->getHeadOfTrNodeInfo()->_node->duplicateTree();
    ...
    if (target) {
        ...
        lengthTRxx = TR::Node::create(TR::isub, 2, translateNode,
            TR::Node::create(TR::icmpeq, 2, translateNode,
                lengthNode->getOpCodeValue() == TR::i2l ? lengthNode->getChild(0) : lengthNode));
    }
    ...
    TR::TreeTop *dstIndVarUpdateTreeTop = NULL;
    if (dstIndexRepNode) {
        dstIndVarUpdateTreeTop = TR::TreeTop::create(comp,
            createStoreOP2(comp, dstIndexVarSymRef, TR::iadd, dstIndexVarSymRef, lengthTRxx, dstIndexRepNode));
    }
    ...
    if (target) {
        ...
        // Fix up the IV value by adding 1 if translateNode == lengthNode (where no test char was found).  See comment
        // above.
        TR::Node *incIndex
        ...
        if (dstIndVarUpdateTreeTop) {
            TR::Node *incDstIndex
                = createStoreOP2(comp, dstIndexVarSymRef, TR::isub, dstIndexVarSymRef, -1, dstIndexRepNode);
...
TR_PCISCGraph *makeTROTArrayGraph(TR::Compilation *c, int32_t ctrl)
{
    ...
    tgt->addNode(j); // dst array index
    TR_PCISCNode *map
        = new (PERSISTENT_NEW) TR_PCISCNode(c->trMemory(), TR_variable, TR::NoType, tgt->incNumNodes(), 12, 0, 0, 2);
    tgt->addNode(map); // map array base
    TR_PCISCNode *idx0
        = new (PERSISTENT_NEW) TR_PCISCNode(c->trMemory(), TR_arrayindex, TR::NoType, tgt->incNumNodes(), 11, 0, 0, 0);
    tgt->addNode(idx0);
    TR_PCISCNode *idx1
        = new (PERSISTENT_NEW) TR_PCISCNode(c->trMemory(), TR_arrayindex, TR::NoType, tgt->incNumNodes(), 10, 0, 0, 1);
    tgt->addNode(idx1);
    TR_PCISCNode *end
        = new (PERSISTENT_NEW) TR_PCISCNode(c->trMemory(), TR_quasiConst2, TR::NoType, tgt->incNumNodes(), 9, 0, 0);
    tgt->addNode(end); // length
    TR_PCISCNode *exitValue
        = new (PERSISTENT_NEW) TR_PCISCNode(c->trMemory(), TR_quasiConst2, TR::NoType, tgt->incNumNodes(), 8, 0, 0);
    tgt->addNode(exitValue); // exitvalue (optional)
    TR_PCISCNode *aHeader
        = new (PERSISTENT_NEW) TR_PCISCNode(c->trMemory(), TR_ahconst, TR::NoType, tgt->incNumNodes(), 7, 0, 0, 0);
    ...
    tgt->addNode(entry);
    TR_PCISCNode *byteAddr
        = createIdiomArrayLoadInLoop(tgt, ctrl, 1, entry, TR::bloadi, TR::Int8, byteArray, idx0, aHeader, c1);
    TR_PCISCNode *convNode, *mapAddr;
    if (ctrl & CISCUtilCtl_64Bit) {
        convNode = new (PERSISTENT_NEW)
            TR_PCISCNode(c->trMemory(), TR::bu2l, TR::Int64, tgt->incNumNodes(), 1, 1, 1, byteAddr, byteAddr);
        tgt->addNode(convNode);
        mapAddr = createIdiomCharArrayLoadInLoop(tgt, ctrl | CISCUtilCtl_NoI2L, 1, convNode, map, convNode, aHeader,
            elemSize);
    } else {
        convNode = new (PERSISTENT_NEW)
            TR_PCISCNode(c->trMemory(), TR::bu2i, TR::Int32, tgt->incNumNodes(), 1, 1, 1, byteAddr, byteAddr);
        tgt->addNode(convNode);
        mapAddr = createIdiomCharArrayLoadInLoop(tgt, ctrl, 1, convNode, map, convNode, aHeader, elemSize);
    }
    TR_PCISCNode *c2iNode = new (PERSISTENT_NEW)
        TR_PCISCNode(c->trMemory(), TR::su2i, TR::Int32, tgt->incNumNodes(), 1, 1, 1, mapAddr, mapAddr);
    tgt->addNode(c2iNode); // optional
    TR_PCISCNode *exitTest = new (PERSISTENT_NEW) TR_PCISCNode(c->trMemory(), TR::ificmpeq, TR::NoType,
        tgt->incNumNodes(), 1, 2, 2, c2iNode, c2iNode, exitValue); // optional
    tgt->addNode(exitTest);
    TR_PCISCNode *charAddr
        = createIdiomCharArrayStoreInLoop(tgt, ctrl, 1, exitTest, charArray, idx1, aHeader, elemSize, c2iNode);
    TR_PCISCNode *iStore = createIdiomDecVarInLoop(tgt, ctrl, 1, charAddr, i, increment);
    TR_PCISCNode *jStore = createIdiomIncVarInLoop(tgt, ctrl, 1, iStore, j, i, offset); // optional
    TR_PCISCNode *loopTest = new (PERSISTENT_NEW)
        TR_PCISCNode(c->trMemory(), TR::ificmpge, TR::NoType, tgt->incNumNodes(), 1, 2, 2, jStore, i, end);
    tgt->addNode(loopTest);
    TR_PCISCNode *exit
        = new (PERSISTENT_NEW) TR_PCISCNode(c->trMemory(), TR_exitnode, TR::NoType, tgt->incNumNodes(), 0, 0, 0);
    tgt->addNode(exit);
...
//*****************************************************************************************
bool CISCTransform2CopyingTRTx(TR_CISCTransformer *trans)
{
    const bool disptrace = DISPTRACE(trans);
    TR::Node *trNode;
    TR::TreeTop *trTreeTop;
    ...
    if (isOutputChar && genSIMD && !genTRxx) {
        logprints(disptrace, log, "Bailing CISCTransform2CopyingTRTx : c2c - no proper evaluator available\n");
        return false;
    }
    ...
    if (isLoopPreheaderLastBlockInMethod(comp, block, disptrace)) {
        logprints(disptrace, log,
            "Bailing CISCTransform2CopyingTRTx due to null TT - might be a preheader in last block of method\n");
        return false;
    }
    ...
    if (trans->countGoodArrayIndex(indexVarSymRef) == 0
        ...
        logprintf(disptrace, log, "countGoodArrayIndex failed for %p, %p\n", indexRepNode, dstIndexRepNode);
        return false;
    }
    TR_ScratchList<TR::Node> variableList(comp->trMemory());
    variableList.add(indexRepNode);
    ...
    if (!isIndexVariableInList(inputNode, &variableList) || !isIndexVariableInList(outputNode, &variableList)) {
        dumpOptDetails(comp,
            "indices used in array loads %p and %p are not consistent with the induction variable updates\n", inputNode,
            outputNode);
        return false;
    }
    ...
    if (!target) // multiple successors
    {
        ...
        if (trans->getNumOfBBlistSucc() > 3) {
            logprintf(disptrace, log, "trans->getNumOfBBlistSucc() is %d.", trans->getNumOfBBlistSucc());
            return false;
        }
    }
    ...
    int count;
    TR::TreeTop *retSameExit = NULL;
    if ((count = trans->analyzeCharBoolTable(P->getImportantNode(0), tmpTable, P->getImportantNode(1), &retSameExit))
        <= 0) {
        logprints(disptrace, log, "trans->analyzeCharBoolTable failed\n");
        return false;
    }
    ...
    if (comp->cg()->getSupportsArrayTranslateTRTO255() || comp->cg()->getSupportsArrayTranslateTRTO()) {
        ...
        // termchar = TERMBYTE; //It needs to be greater than zero, dummy termination char otherwise, i.e., it's not
        // gonna be used,
        tableNode = TR::Node::create(baseRepNode, TR::iconst, 0, 0); // dummy table node, it's not gonna be used
    } else // Z
    {
        if (!isOutputChar) {
            ...
            if (isSIMDPossible) {
                ...
                logprints(disptrace, log, "Bailing CISCTransform2CopyingTRTx : c2b - no proper evaluator available\n");
                return false;
            } else {
                // TRxx
                for (i = 256; --i >= 0;) {
                ...
                if (termchar < 0) // no room of termchar
                {
                    ...
                    termchar = TERMBYTE;
                    logprints(disptrace, log, "setAllowSourceCellIsTermChar: ");
                }
                logprintf(disptrace, log, "termchar is 0x%02x\n", termchar);

                uint8_t *table = (uint8_t *)comp->trMemory()->allocateMemory(65536, stackAlloc);
                // Only check up to 256 because we already
                for (i = 0; i < 65536; i++) {
                    uint8_t u8 = tmpTable[i];
                    // Not sure I understand the reasning behind discarding those: chars larger than 256 which map to
                    // byte ... possible we have the table to hold all chars. Value needs to represent i & ff for now I
                    // moved the check up - so bail out earlier. Reach here only if chars that need mapping are <256. if
                    // (!u8 && i >= 256)
                    //  return false;
    ...
    // find icmpge in the candidate region
    sortList(P2T + P->getImportantNode(1)->getID(), &necessaryCmp, trans->getCandidateRegion());
    ...
    if (necessaryCmp.getSize() >= 3) {
        logprintf(disptrace, log,
            "Too many (%d) loop exit tests to transform correctly.  Transformation only supports up to 2.  Abandoning "
            "reduction.\n",
    ...
    if (!testExitIF(icmpgeCISCnode1->getOpcode(), &isDecrement, &modLength)) {
        logprints(disptrace, log, "testExitIF for icmpgeCISCnode1 failed\n");
        return false;
    }
    if (isDecrement) {
        logprints(disptrace, log, "Not support a decrement loop. (icmpgeCISCnode1)\n");
        return false;
    }
    TR_ASSERT(modLength == 0 || modLength == 1, "error");
    ...
    if (cmpVarSymRef == NULL) {
        logprints(disptrace, log, "Unable to determine the sym ref of induction variable in loop termination node.\n");
        return false;
    }

    lenRepNode1 = createLoad(icmpgeCISCnode1->getChild(1)->getHeadOfTrNode());
    if (modLength)
        lenRepNode1 = createOP2(comp, TR::isub, lenRepNode1, TR::Node::create(baseRepNode, TR::iconst, 0, -modLength));
    ...
    if (P->getImportantNode(4)) {
        if (icmpgeCISCnode2) {
            logprints(disptrace, log, "Not support yet more than three if-statements. (1)\n");
            return false;
        }
        icmpgeCISCnode2 = trans->getP2TInLoopIfSingle(P->getImportantNode(4));
        if (!icmpgeCISCnode2) {
            logprints(disptrace, log, "Not support yet more than three if-statements. (2)\n");
            return false;
        }
    }
    ...
    if (icmpgeCISCnode2) {
        if (!testExitIF(icmpgeCISCnode2->getOpcode(), &isDecrement, &modLength)) {
            logprints(disptrace, log, "testExitIF for icmpgeCISCnode2 failed\n");
            return false;
        }
        if (isDecrement) {
            logprints(disptrace, log, "Not support a decrement loop. (icmpgeCISCnode2)\n");
            return false;
        }
        TR_ASSERT(modLength == 0 || modLength == 1, "error");
        ...
        if (modLength)
            lenRepNode2
                = createOP2(comp, TR::isub, lenRepNode2, TR::Node::create(baseRepNode, TR::iconst, 0, -modLength));
        ...
        if (!icmpgeNode2->getChild(0)->getOpCode().isLoadVarDirect())
            return false;
        icmpgeSymRef2 = icmpgeNode2->getChild(0)->getSymbolReference();

        TR::Node *lenTmpNode2 = createOP2(comp, TR::isub, lenRepNode2,
    ...
    if (isAllowSourceCellTermChar) {
        ...
        // translateNode->setAllowSourceCellIsTermChar(true); // Generated code will check whether the character is a
        // delimiter.
    ...
    TR::Node *addCountNode = createOP2(comp, TR::iadd,
        TR::Node::createWithSymRef(indexRepNode, TR::iload, 0, indexVarSymRef), translateNode);
    if (ivNeedsUpdate)
    ...
    if (dstIndexRepNode) {
        ...
        if (dstIndexVarSymRef->getSymbol()->getDataType() == TR::Int32) {
            dstAddCountNode = createOP2(comp, TR::iadd,
                TR::Node::createWithSymRef(dstIndexRepNode, TR::iload, 0, dstIndexVarSymRef), translateNode);
            if (dstIvNeedsUpdate)
    ...
    if (icmpgeCISCnode2) {
        ...
        logprints(disptrace, log, "Now assuming that all exits of booltable are identical.\n");
        ...
        if (disptrace) {
            if (okDest == NULL)
                log->prints("error, okDest == NULL!\n");
            if (failDest == NULL)
                log->prints("error, failDest == NULL!\n");
            if (failDest2 == NULL)
                log->prints("error, failDest2 == NULL!\n");
        }
        TR_ASSERT(okDest != NULL && failDest != NULL && failDest2 != NULL, "error!");

        // It generates "ificmpge".
        icmpgeNode = TR::Node::createif(TR::ificmpge, cmpIndexNode->duplicateTree(), lenRepNode1, failDest);
        icmpgeNode2
            = TR::Node::createif(TR::ificmpge, TR::Node::createWithSymRef(indexRepNode, TR::iload, 0, icmpgeSymRef2),
                lenRepNode2->duplicateTree(), failDest2);
        ...
        trans->setSuccessorEdges(block, newBlockForIf2, failDest->getEnclosingBlock());
        trans->setSuccessorEdges(newBlockForIf2, okDest->getEnclosingBlock(), failDest2->getEnclosingBlock());
    } else {
        if (isNeedGenIcmpge) {
            logprints(disptrace, log,
                "Now assuming that all exits of booltable are identical and the exit of icmpge points different.\n");
...
bool CISCTransform2CopyingTRTxAddDest1(TR_CISCTransformer *trans)
{
    trans->setOffsetOperand2(1); // add offset of destination with 1
...
TR_PCISCGraph *makeCopyingTRTxGraph(TR::Compilation *c, int32_t ctrl, int pattern)
{
     ...
     * #cfg #child other/pred/children */
    TR_PCISCNode *charArray
        = new (PERSISTENT_NEW) TR_PCISCNode(c->trMemory(), TR_arraybase, TR::NoType, tgt->incNumNodes(), 15, 0, 0, 0);
    tgt->addNode(charArray); // src array base
    TR_PCISCNode *i
        = new (PERSISTENT_NEW) TR_PCISCNode(c->trMemory(), TR_variable, TR::NoType, tgt->incNumNodes(), 14, 0, 0, 0);
    tgt->addNode(i); // src array index
    TR_PCISCNode *byteArray
        = new (PERSISTENT_NEW) TR_PCISCNode(c->trMemory(), TR_arraybase, TR::NoType, tgt->incNumNodes(), 13, 0, 0, 1);
    tgt->addNode(byteArray); // dst array base
    TR_PCISCNode *j
        = new (PERSISTENT_NEW) TR_PCISCNode(c->trMemory(), TR_variable, TR::NoType, tgt->incNumNodes(), 12, 0, 0, 1);
    tgt->addNode(j); // dst array index
    TR_PCISCNode *idx0
        = new (PERSISTENT_NEW) TR_PCISCNode(c->trMemory(), TR_arrayindex, TR::NoType, tgt->incNumNodes(), 11, 0, 0, 0);
    tgt->addNode(idx0);
    ...
    tgt->addNode(increment);
    TR_PCISCNode *mulFactor
        = new (PERSISTENT_NEW) TR_PCISCNode(c->trMemory(), TR_allconst, TR::NoType, tgt->incNumNodes(), 5, 0, 0);
    tgt->addNode(mulFactor); // Multiply Factor
    TR_PCISCNode *elemSize = createIdiomArrayRelatedConst(tgt, ctrl, tgt->incNumNodes(), 4, 2); // element size
    TR_PCISCNode *offset
        = new (PERSISTENT_NEW) TR_PCISCNode(c->trMemory(), TR_variable, TR::NoType, tgt->incNumNodes(), 3, 0, 0, 2);
    tgt->addNode(offset); // optional
    TR_PCISCNode *entry
        = new (PERSISTENT_NEW) TR_PCISCNode(c->trMemory(), TR_entrynode, TR::NoType, tgt->incNumNodes(), 2, 1, 0);
    tgt->addNode(entry);
    ...
    TR_PCISCNode *c2iNode = new (PERSISTENT_NEW)
        TR_PCISCNode(c->trMemory(), TR::su2i, TR::Int32, tgt->incNumNodes(), 1, 1, 1, charAddr, charAddr);
    tgt->addNode(c2iNode);
    TR_PCISCNode *exitTest = new (PERSISTENT_NEW)
        TR_PCISCNode(c->trMemory(), TR_booltable, TR::NoType, tgt->incNumNodes(), 1, 2, 1, c2iNode, c2iNode);
    tgt->addNode(exitTest);
    TR_PCISCNode *byteAddr = createIdiomArrayStoreInLoop(tgt, ctrl, 1, exitTest, TR_ibcstore, TR::NoType, byteArray,
        idx1, aHeader1, mulFactor, c2iNode);
    ...
    TR_PCISCNode *loopTest = new (PERSISTENT_NEW)
        TR_PCISCNode(c->trMemory(), TR_ifcmpall, TR::NoType, tgt->incNumNodes(), 1, 2, 2, store2, j, end);
    tgt->addNode(loopTest);
    TR_PCISCNode *exit
        = new (PERSISTENT_NEW) TR_PCISCNode(c->trMemory(), TR_exitnode, TR::NoType, tgt->incNumNodes(), 0, 0, 0);
    tgt->addNode(exit);
...
TR_PCISCGraph *makeCopyingTRTxThreeIfsGraph(TR::Compilation *c, int32_t ctrl)
{
    ...
    tgt->addNode(exitTest);
    TR_PCISCNode *loopTest = new (PERSISTENT_NEW)
        TR_PCISCNode(c->trMemory(), TR_ifcmpall, TR::NoType, tgt->incNumNodes(), 1, 2, 2, exitTest, j, end);
    tgt->addNode(loopTest);
    TR_PCISCNode *byteAddr = createIdiomArrayStoreInLoop(tgt, ctrl, 1, loopTest, TR_ibcstore, TR::NoType, byteArray, j,
        aHeader1, mulFactor, c2iNode);
    ...
    TR_PCISCNode *loopTest2 = new (PERSISTENT_NEW)
        TR_PCISCNode(c->trMemory(), TR_ifcmpall, TR::NoType, tgt->incNumNodes(), 1, 2, 2, store2, i, end2);
    tgt->addNode(loopTest2);
    TR_PCISCNode *exit
        = new (PERSISTENT_NEW) TR_PCISCNode(c->trMemory(), TR_exitnode, TR::NoType, tgt->incNumNodes(), 0, 0, 0);
    tgt->addNode(exit);
...
TR_PCISCGraph *makeCopyingTRTOGraphSpecial(TR::Compilation *c, int32_t ctrl)
{
    ...
    /************************************    opc               id        dagId #cfg #child other/pred/children */
    TR_PCISCNode *charArray
        = new (PERSISTENT_NEW) TR_PCISCNode(c->trMemory(), TR_arraybase, TR::NoType, tgt->incNumNodes(), 12, 0, 0, 0);
    tgt->addNode(charArray); // src array base
    TR_PCISCNode *i
        = new (PERSISTENT_NEW) TR_PCISCNode(c->trMemory(), TR_variable, TR::NoType, tgt->incNumNodes(), 11, 0, 0, 0);
    tgt->addNode(i); // src array index
    TR_PCISCNode *byteArray
        = new (PERSISTENT_NEW) TR_PCISCNode(c->trMemory(), TR_arraybase, TR::NoType, tgt->incNumNodes(), 10, 0, 0, 1);
    tgt->addNode(byteArray); // dst array base
    TR_PCISCNode *j
        = new (PERSISTENT_NEW) TR_PCISCNode(c->trMemory(), TR_variable, TR::NoType, tgt->incNumNodes(), 9, 0, 0, 1);
    tgt->addNode(j); // dst array index
    TR_PCISCNode *end
        = new (PERSISTENT_NEW) TR_PCISCNode(c->trMemory(), TR_quasiConst2, TR::NoType, tgt->incNumNodes(), 8, 0, 0);
    tgt->addNode(end); // length
    ...
    TR_PCISCNode *c2iNode = new (PERSISTENT_NEW)
        TR_PCISCNode(c->trMemory(), TR::su2i, TR::Int32, tgt->incNumNodes(), 1, 1, 1, charAddr, charAddr);
    tgt->addNode(c2iNode);
    TR_PCISCNode *lStore = new (PERSISTENT_NEW)
        TR_PCISCNode(c->trMemory(), TR::lstore, TR::Int64, tgt->incNumNodes(), 1, 1, 2, c2iNode, i2lNode, j);
    tgt->addNode(lStore);
    TR_PCISCNode *exitTest = new (PERSISTENT_NEW)
        TR_PCISCNode(c->trMemory(), TR_booltable, TR::NoType, tgt->incNumNodes(), 1, 2, 1, lStore, c2iNode);
    tgt->addNode(exitTest);
    TR_PCISCNode *byteAddr = createIdiomArrayStoreInLoop(tgt, ctrl | CISCUtilCtl_NoI2L, 1, exitTest, TR_ibcstore,
        TR::NoType, byteArray, j, aHeader1, mulFactor, c2iNode);
    TR_PCISCNode *iStore = createIdiomDecVarInLoop(tgt, ctrl, 1, byteAddr, i, increment);
    TR_PCISCNode *loopTest = new (PERSISTENT_NEW)
        TR_PCISCNode(c->trMemory(), TR_ifcmpall, TR::NoType, tgt->incNumNodes(), 1, 2, 2, iStore, i, end);
    tgt->addNode(loopTest);
    TR_PCISCNode *exit
        = new (PERSISTENT_NEW) TR_PCISCNode(c->trMemory(), TR_exitnode, TR::NoType, tgt->incNumNodes(), 0, 0, 0);
    tgt->addNode(exit);
...
TR_PCISCGraph *makeCopyingTRTOInduction1Graph(TR::Compilation *c, int32_t ctrl, int32_t pattern)
{
    ...
    switch (pattern) {
        case 0: {
            ...
            // v1++;
            TR_PCISCNode *op0;
            n5 = new (PERSISTENT_NEW)
                TR_PCISCNode(c->trMemory(), TR::iadd, TR::Int32, tgt->incNumNodes(), 1, 1, 2, n4, v1, v4);
            tgt->addNode(n5);
            nn0 = createIdiomArrayStoreInLoop(tgt, ctrl, 1, n5, TR::bstorei, TR::Int8, v2, n5, cmah1, c1, n3);
            op0 = new (PERSISTENT_NEW)
                TR_PCISCNode(c->trMemory(), TR::isub, TR::Int32, tgt->incNumNodes(), 1, 1, 2, nn0, n5, cm1);
            tgt->addNode(op0); // (optional)
            op1 = new (PERSISTENT_NEW)
                TR_PCISCNode(c->trMemory(), TR::istore, TR::Int32, tgt->incNumNodes(), 1, 1, 2, op0, op0, v3);
            tgt->addNode(op1); // (optional)
        ...
        case 2: {
            ...
            nn0 = createIdiomArrayStoreInLoop(tgt, ctrl, 1, n5, TR::bstorei, TR::Int8, v2, n5, cmah1, c1, n3);
            op1 = new (PERSISTENT_NEW)
                TR_PCISCNode(c->trMemory(), TR::istore, TR::Int32, tgt->incNumNodes(), 1, 1, 2, nn0, n5, v3);
            tgt->addNode(op1);
            n6 = op1;
            op1->setIsChildDirectlyConnected();
            break;
    ...
    TR_PCISCNode *n7 = new (PERSISTENT_NEW)
        TR_PCISCNode(c->trMemory(), TR_ifcmpall, TR::NoType, tgt->incNumNodes(), 1, 2, 2, n6, v1, vorc);
    tgt->addNode(n7);
    TR_PCISCNode *n8
        = new (PERSISTENT_NEW) TR_PCISCNode(c->trMemory(), TR_exitnode, TR::NoType, tgt->incNumNodes(), 0, 0, 0);
    tgt->addNode(n8);
...
TR_PCISCGraph *makeCopyingTRTTSpecialGraph(TR::Compilation *c, int32_t ctrl)
{
    ...
    tgt->addNode(v3); // dst array index
    TR_PCISCNode *idx0
        = new (PERSISTENT_NEW) TR_PCISCNode(c->trMemory(), TR_arrayindex, TR::NoType, tgt->incNumNodes(), 8, 0, 0, 0);
    tgt->addNode(idx0);
    TR_PCISCNode *idx1
        = new (PERSISTENT_NEW) TR_PCISCNode(c->trMemory(), TR_arrayindex, TR::NoType, tgt->incNumNodes(), 7, 0, 0, 1);
    tgt->addNode(idx1);
    TR_PCISCNode *vorc
        = new (PERSISTENT_NEW) TR_PCISCNode(c->trMemory(), TR_quasiConst2, TR::NoType, tgt->incNumNodes(), 6, 0, 0);
    tgt->addNode(vorc); // length
    ...
    TR_PCISCNode *n3
        = new (PERSISTENT_NEW) TR_PCISCNode(c->trMemory(), TR::su2i, TR::Int32, tgt->incNumNodes(), 1, 1, 1, n2, n2);
    tgt->addNode(n3);
    TR_PCISCNode *n4 = new (PERSISTENT_NEW)
        TR_PCISCNode(c->trMemory(), TR_booltable, TR::NoType, tgt->incNumNodes(), 1, 2, 1, n3, n3);
    tgt->addNode(n4);
    ...
    TR_PCISCNode *nn0
        = createIdiomCharArrayStoreInLoop(tgt, ctrl | CISCUtilCtl_NoConversion, 1, n5, v2, idx1, cmah, c2, n5);
    TR_PCISCNode *n6 = createIdiomDecVarInLoop(tgt, ctrl, 1, nn0, v1, cm1);
    TR_PCISCNode *nn6 = createIdiomDecVarInLoop(tgt, ctrl, 1, n6, v3, cm1);
    TR_PCISCNode *n7 = new (PERSISTENT_NEW)
        TR_PCISCNode(c->trMemory(), TR_ifcmpall, TR::NoType, tgt->incNumNodes(), 1, 2, 2, nn6, v3, vorc);
    tgt->addNode(n7);
    TR_PCISCNode *n8
        = new (PERSISTENT_NEW) TR_PCISCNode(c->trMemory(), TR_exitnode, TR::NoType, tgt->incNumNodes(), 0, 0, 0);
    tgt->addNode(n8);
...
//*****************************************************************************************
bool CISCTransform2TRTOArray(TR_CISCTransformer *trans)
{
    TR_ASSERT(trans->getOffsetOperand1() == 0 && trans->getOffsetOperand2() == 0, "Not implemented yet");
    const bool disptrace = DISPTRACE(trans);
    TR::Node *trNode;
    ...
    if (isLoopPreheaderLastBlockInMethod(comp, block, disptrace)) {
        logprints(disptrace, comp->log(),
            "Bailing CISCTransform2TRTOArray due to null TT - might be a preheader in last block of method\n");
        return false;
    }
    ...
    TR::Node *lengthNode = createI2LIfNecessary(comp, trans->isGenerateI2L(),
        createOP2(comp, TR::isub, cmpRepNode->getChild(1)->duplicateTree(), indexNode));
    TR::Node *termCharNode
        = createLoad(trans->getP2TRep(P->getImportantNode(0)->getChild(1))->getHeadOfTrNodeInfo()->_node);
    TR::Node *stoppingNode = TR::Node::create(baseRepNode, TR::iconst, 0, 0xffffffff);
    ...
    if (target) // single successor block
    {
        ...
        /*lengthTRxx = createOP2(comp, TR::isub,
                                     translateNode,
                                     TR::Node::create(translateNode, TR::iconst, 0, -1)); */
    } else { // multiple successor blocks
        ...
        if (getThreeNodesForArray(b2iCiscNode, &ixload, &aload, &iload)) {
            ...
            if (iload->getOpcode() == TR::iload && iload->getChains()->isSingleton()
                && iload->getParents()->isSingleton()) { // simple copy propagation
    ...
    if (P->getImportantNode(4))
        optionalIficmpge = trans->getP2TRepInLoop(P->getImportantNode(4));
    ...
    TR::Node *alignmentCheck
        = createTableAlignmentCheck(comp, tableNode, false, true, trans->isTableBackedByRawStorage());
    if (alignmentCheck)
    ...
    if (target) {
        ...
        if (optionalIficmpge)
            optionalDest = optionalIficmpge->getDestination()->getEnclosingBlock();
...
bool CISCTransform2TRTOArrayTableRaw(TR_CISCTransformer *trans)
{
...
TR_PCISCGraph *makeTRTOArrayGraph(TR::Compilation *c, int32_t ctrl)
{
    ...
    TR_PCISCNode *exitValue
        = new (PERSISTENT_NEW) TR_PCISCNode(c->trMemory(), TR_quasiConst2, TR::NoType, tgt->incNumNodes(), 8, 0, 0);
    tgt->addNode(exitValue); // exitvalue
    TR_PCISCNode *aHeader
        = new (PERSISTENT_NEW) TR_PCISCNode(c->trMemory(), TR_ahconst, TR::NoType, tgt->incNumNodes(), 7, 0, 0, 0);
    ...
    TR_PCISCNode *convNode
        = new (PERSISTENT_NEW) TR_PCISCNode(c->trMemory(), (ctrl & CISCUtilCtl_64Bit) ? TR::su2l : TR::su2i,
            (ctrl & CISCUtilCtl_64Bit) ? TR::Int64 : TR::Int32, tgt->incNumNodes(), 1, 1, 1, charAddr, charAddr);
    tgt->addNode(convNode);
    TR_PCISCNode *mapAddr = createIdiomArrayLoadInLoop(tgt, ctrl | CISCUtilCtl_NoI2L, 1, convNode, TR::bloadi, TR::Int8,
        map, convNode, aHeader, mulFactor);
    TR_PCISCNode *b = new (PERSISTENT_NEW)
        TR_PCISCNode(c->trMemory(), TR::b2i, TR::Int32, tgt->incNumNodes(), 1, 1, 1, mapAddr, mapAddr);
    tgt->addNode(b);
    TR_PCISCNode *exitTest = new (PERSISTENT_NEW)
        TR_PCISCNode(c->trMemory(), TR::ificmpeq, TR::NoType, tgt->incNumNodes(), 1, 2, 2, b, b, exitValue);
    tgt->addNode(exitTest);
    TR_PCISCNode *byteAddr = createIdiomArrayStoreInLoop(tgt, ctrl, 1, exitTest, TR::bstorei, TR::Int8, byteArray, idx1,
        aHeader, mulFactor, b);
    TR_PCISCNode *iStore = createIdiomDecVarInLoop(tgt, ctrl, 1, byteAddr, i, increment);
    TR_PCISCNode *jStore = createIdiomIncVarInLoop(tgt, ctrl, 1, iStore, j, i, offset); // optional
    TR_PCISCNode *loopTest = new (PERSISTENT_NEW)
...
TR_PCISCGraph *makeTRTOArrayGraphSpecial(TR::Compilation *c, int32_t ctrl)
{
    ...
    TR_PCISCNode *c2iNode = new (PERSISTENT_NEW)
        TR_PCISCNode(c->trMemory(), TR::su2i, TR::Int32, tgt->incNumNodes(), 1, 1, 1, charAddr, charAddr);
    tgt->addNode(c2iNode);
    TR_PCISCNode *limitChk = new (PERSISTENT_NEW)
        TR_PCISCNode(c->trMemory(), TR::ificmpge, TR::NoType, tgt->incNumNodes(), 1, 2, 2, c2iNode, c2iNode, limit);
    ...
    tgt->addNode(b2iNode);
    TR_PCISCNode *exitTest = new (PERSISTENT_NEW)
        TR_PCISCNode(c->trMemory(), TR::ificmpeq, TR::NoType, tgt->incNumNodes(), 1, 2, 2, b2iNode, b2iNode, exitValue);
    tgt->addNode(exitTest);
    TR_PCISCNode *byteAddr = createIdiomArrayStoreInLoop(tgt, ctrl, 1, exitTest, TR::bstorei, TR::Int8, byteArray, idx1,
        aHeader, mulFactor, b2iNode);
    TR_PCISCNode *iStore = createIdiomDecVarInLoop(tgt, ctrl, 1, byteAddr, i, increment);
...
static StatusArrayStore checkArrayStore(TR::Compilation *comp, TR::Node *inputNode, TR::Node *outputNode,
    int elementSize, bool isForward)
{
    ...
    if (comp->target().cpu.isZ() || comp->target().cpu.isARM64())
        return NO_NEED_TO_CHECK; // On 390, MVC (which performs byte copies) is generated. ARM64 arraycopy can handle
                                 // byte copies.

    if (inputNode->getFirstChild()->getSymbol()->getRegisterMappedSymbol()
        == outputNode->getFirstChild()->getSymbol()->getRegisterMappedSymbol()) {
        static bool disableArraycopyOverlapTest = feGetEnv("TR_disableArraycopyOverlapTest") != NULL;
        if (!disableArraycopyOverlapTest && comp->getOptLevel() >= hot
            && performTransformation(comp,
                "%sNot abandoning reduction due to src == dest, generating element range overlap test\n",
                OPT_DETAILS)) {
            dumpOptDetails(comp,
                "src and dest are the same, generating guard code 'if (src/dest subranges nonoverlapping)'\n");
...
public:
    SubrangeOverlapTestGenerator(TR::Compilation *comp, TR::Node *arraycopy, TR::Node *byteLength, bool is64Bit,
        int elementSize);
    TR::Node *generate();
...
SubrangeOverlapTestGenerator::SubrangeOverlapTestGenerator(TR::Compilation *comp, TR::Node *arraycopy,
    TR::Node *byteLength, bool is64Bit, int elementSize)
    : _comp(comp)
    , _is64Bit(is64Bit)
...
void SubrangeOverlapTestGenerator::simplifyConstSub()
{
    ...
    if (!performTransformation(_comp,
            "%sSimplifying arraycopy element range overlap test by looking through sub const\n", OPT_DETAILS))
        return;
...
void SubrangeOverlapTestGenerator::simplifyConstMul()
{
    ...
    // Ask permission to transform.
    if (!performTransformation(_comp,
            "%sSimplifying arraycopy element range overlap test by looking through mul const\n", OPT_DETAILS))
        return;
...
TR::Node *SubrangeOverlapTestGenerator::byteOffset(TR::Node *addr)
{
    ...
    TR_ASSERT(op == TR::aladd || op == TR::aiadd, "unexpected arraycopy child opcode %s", addr->getOpCode().getName());
...
void SubrangeOverlapTestGenerator::checkType(const char *desc, TR::Node *node)
{
    ...
    TR_ASSERT(actualType == expectedType, "expected %s node to have type %s, but found %s (%d)", desc,
        TR::DataType::getName(expectedType), TR::DataType::getName(actualType), (int)actualType);
}
} // namespace
...
//*****************************************************************************************
static bool CISCTransform2ArrayCopySub(TR_CISCTransformer *trans, TR::Node *indexRepNode, TR::Node *dstIndexRepNode,
    TR::Node *exitVarRepNode, TR::Node *variableORconstRepNode)
{
    ...
    if (isLoopPreheaderLastBlockInMethod(comp, block, disptrace)) {
        logprints(disptrace, log,
            "Bailing CISCTransform2ARrayCopySub due to null TT - might be a preheader in last block of method\n");
        return false;
    }
    ...
    if (!inLoadCISCNode || !inStoreCISCNode) {
        logprintf(disptrace, log, "CISCTransform2ArrayCopy failed. inLoadCISCNode %x inStoreCISCNode %x\n",
    ...
    if (T && T->getAspects()->getIfCount() > 1) {
        logprintf(disptrace, log, "CISCTransform2ArrayCopySub detected %d if-stmts in loop (> 1).  Not transforming.\n",
    ...
    if (!getMultiplier(trans, P->getImportantNode(2), &mulFactorNode, &elementSize, inLoadNode->getType())) {
        logprints(disptrace, log, "CISCTransform2ArrayCopy getMultiplier failed.\n");
        return false;
    }
    if (elementSize != inLoadNode->getSize() || elementSize != inStoreNode->getSize()) {
        logprintf(disptrace, log,
            "CISCTransform2ArrayCopy failed - Size Mismatch.  Element Size: %d InLoadSize: %d inStoreSize: %d\n",
    ...
    if ((statusArrayStore = checkArrayStore(comp, inputNode, outputNode, elementSize, !isDecrement))
        == ABANDONING_REDUCTION)
        return false;

    if (indexContainsArrayAccess(comp, inLoadNode->getFirstChild())
        || indexContainsArrayAccess(comp, inStoreNode->getFirstChild())) {
        logprintf(disptrace, log, "inputNode %p or outputnode %p contains another arrayaccess, no reduction\n",
    ...
    if (!testExitIF(cmpIfAllCISCNode->getOpcode(), &isDecrementRet, &modLength, &modStartIdx)) {
        logprints(disptrace, log, "CISCTransform2ArrayCopy testExitIF failed.\n");
        return false;
    }
    if (isDecrement != isDecrementRet)
        return false;

    TR::SymbolReference *indexVarSymRef = indexRepNode->getSymbolReference();
    if (!trans->analyzeArrayIndex(indexVarSymRef)) {
        logprintf(disptrace, log, "analyzeArrayIndex failed. %x\n", indexRepNode);
        return false;
    }
    TR::SymbolReference *dstIndexVarSymRef = dstIndexRepNode ? dstIndexRepNode->getSymbolReference() : NULL;
    if (indexVarSymRef == dstIndexVarSymRef)
    ...
    if (!trans->searchNodeInTrees(inputNode, indexRepNode)) {
        logprints(disptrace, log, "searchNodeInTrees for inputNode failed.\n");
        return false;
    }
    if (!trans->searchNodeInTrees(outputNode,
            dstIndexVarSymRef ? convertStoreToLoad(comp, dstIndexRepNode) : indexRepNode)) {
        logprints(disptrace, log, "searchNodeInTrees for outputNode failed.\n");
        return false;
    }
    ...
    if (indexVarSymRef != exitVarSymRef && dstIndexVarSymRef != exitVarSymRef) {
        logprintf(disptrace, log, "CISCTransform2ArrayCopy cannot find exitVarSymRef correctly %x.\n", exitVarRepNode);
        return false;
    }
    ...
    int32_t postIncrement = checkForPostIncrement(comp, block, cmpIfAllCISCNode->getHeadOfTrNodeInfo()->_node,
        exitVarSymRef->getSymbol());

    logprintf(disptrace, log, "detected postIncrement %d modLength %d modStartIdx %d\n", postIncrement, modLength,
        modStartIdx);
    ...
    if (isDecrement) {
        ...
        if (postIncrement && (modStartIdx > 0))
            modStartIdx = 0;
        TR::Node *startIdx = modStartIdx
            ? createOP2(comp, TR::isub, variableORconstRepNode, TR::Node::create(trNode, TR::iconst, 0, -modStartIdx))
            : variableORconstRepNode;
        if (!getThreeNodesForArray(inLoadCISCNode, &ixloadORstore, &aload, &iload))
            return false;
        if ((inputNode = replaceIndexInAddressTree(comp,
                 ixloadORstore->getChild(0)->getHeadOfTrNodeInfo()->_node->duplicateTree(), indexVarSymRef, startIdx))
            == NULL)
            return false;
        if (!getThreeNodesForArray(inStoreCISCNode, &ixloadORstore, &aload, &iload))
            return false;
        if ((outputNode = replaceIndexInAddressTree(comp,
                 ixloadORstore->getChild(0)->getHeadOfTrNodeInfo()->_node->duplicateTree(),
                 dstIndexVarSymRef ? dstIndexVarSymRef : indexVarSymRef, startIdx))
            == NULL)
            return false;
    ...
    if (modLength)
        lengthNode = createOP2(comp, TR::isub, lengthNode, TR::Node::create(trNode, TR::iconst, 0, -modLength));
    ...
    if (!comp->cg()->getSupportsReferenceArrayCopy() && needWriteBarrier) {
        logprints(disptrace, log,
            "CISCTransform2ArrayCopy: needWriteBarrier but not getSupportsReferenceArrayCopy().\n");
        return false;
    }
    ...
    if (!needWriteBarrier) {
        ...
        arraycopy = TR::Node::createArraycopy(inputNode->getFirstChild(), outputNode->getFirstChild(), inputNode,
            outputNode, lengthNode);
    }
    ...
    TR::TreeTop *exitVarUpdateTreeTop = TR::TreeTop::create(comp, TR::Node::createStore(exitVarSymRef, finalValue));
    ...
    if (dstIndexVarSymRef != NULL) {
        ...
        TR::Node *result
            = createOP2(comp, isDecrement ? TR::isub : TR::iadd, TR::Node::createLoad(trNode, theOtherSymRef), diff);
...
bool CISCTransform2ArrayCopy(TR_CISCTransformer *trans)
{
...
bool CISCTransform2ArrayCopySpecial(TR_CISCTransformer *trans)
{
...
bool CISCTransform2ArrayCopyDec(TR_CISCTransformer *trans)
{
    trans->setMEMCPYDec();
...
TR_PCISCGraph *makeMemCpySpecialGraph(TR::Compilation *c, int32_t ctrl)
{
    ...
    tgt->addNode(v3); // dst array index
    TR_PCISCNode *vorc
        = new (PERSISTENT_NEW) TR_PCISCNode(c->trMemory(), TR_quasiConst2, TR::NoType, tgt->incNumNodes(), 12, 0, 0);
    tgt->addNode(vorc); // length

    TR_PCISCNode *v5
        = new (PERSISTENT_NEW) TR_PCISCNode(c->trMemory(), TR_variable, TR::NoType, tgt->incNumNodes(), 11, 0, 0, 2);
    tgt->addNode(v5); // dst array index
    TR_PCISCNode *idx0
        = new (PERSISTENT_NEW) TR_PCISCNode(c->trMemory(), TR_arrayindex, TR::NoType, tgt->incNumNodes(), 10, 0, 0, 0);
    tgt->addNode(idx0);
    TR_PCISCNode *idx1
        = new (PERSISTENT_NEW) TR_PCISCNode(c->trMemory(), TR_arrayindex, TR::NoType, tgt->incNumNodes(), 9, 0, 0, 1);
    tgt->addNode(idx1);
    TR_PCISCNode *v0
        = new (PERSISTENT_NEW) TR_PCISCNode(c->trMemory(), TR_arraybase, TR::NoType, tgt->incNumNodes(), 8, 0, 0, 0);
    tgt->addNode(v0); // src array base
    TR_PCISCNode *v2
        = new (PERSISTENT_NEW) TR_PCISCNode(c->trMemory(), TR_arraybase, TR::NoType, tgt->incNumNodes(), 7, 0, 0, 1);
    tgt->addNode(v2); // dst array base
    TR_PCISCNode *cmah0
        = new (PERSISTENT_NEW) TR_PCISCNode(c->trMemory(), TR_ahconst, TR::NoType, tgt->incNumNodes(), 6, 0, 0, 0);
    tgt->addNode(cmah0); // array header
    TR_PCISCNode *cmah1
        = new (PERSISTENT_NEW) TR_PCISCNode(c->trMemory(), TR_ahconst, TR::NoType, tgt->incNumNodes(), 5, 0, 0, 1);
    tgt->addNode(cmah1); // array header
    TR_PCISCNode *cm1
        = new (PERSISTENT_NEW) TR_PCISCNode(c->trMemory(), TR::iconst, TR::Int32, tgt->incNumNodes(), 4, 0, 0, -1);
    tgt->addNode(cm1);
    ...
    TR_PCISCNode *iis = new (PERSISTENT_NEW)
        TR_PCISCNode(c->trMemory(), TR::istorei, TR::Int32, tgt->incNumNodes(), 1, 1, 2, n6, v5, n6->getChild(0));
    tgt->addNode(iis);
    ...
    TR_PCISCNode *n4
        = new (PERSISTENT_NEW) TR_PCISCNode(c->trMemory(), TR_indload, TR::NoType, tgt->incNumNodes(), 1, 1, 1, n3, n3);
    tgt->addNode(n4);
    TR_PCISCNode *nn1 = new (PERSISTENT_NEW)
        TR_PCISCNode(c->trMemory(), TR_indstore, TR::NoType, tgt->incNumNodes(), 1, 1, 2, n4, n1, n4);
    tgt->addNode(nn1);
    TR_PCISCNode *n7 = createIdiomDecVarInLoop(tgt, ctrl, 1, nn1, v1, cm1);
    TR_PCISCNode *n8 = new (PERSISTENT_NEW)
        TR_PCISCNode(c->trMemory(), TR_ifcmpall, TR::NoType, tgt->incNumNodes(), 1, 2, 2, n7, v1, vorc);
    tgt->addNode(n8);
    TR_PCISCNode *n9
        = new (PERSISTENT_NEW) TR_PCISCNode(c->trMemory(), TR_exitnode, TR::NoType, tgt->incNumNodes(), 0, 0, 0);
    tgt->addNode(n9);
...
TR_PCISCGraph *makeMemCpyGraph(TR::Compilation *c, int32_t ctrl)
{
    ...
    TR_PCISCNode *v4
        = new (PERSISTENT_NEW) TR_PCISCNode(c->trMemory(), TR_variable, TR::NoType, tgt->incNumNodes(), 12, 0, 0, 2);
    tgt->addNode(v4); // exit checking
    TR_PCISCNode *vorc
        = new (PERSISTENT_NEW) TR_PCISCNode(c->trMemory(), TR_quasiConst2, TR::NoType, tgt->incNumNodes(), 11, 0, 0);
    tgt->addNode(vorc); // length
    ...
    tgt->addNode(v2); // dst array base
    TR_PCISCNode *iall
        = new (PERSISTENT_NEW) TR_PCISCNode(c->trMemory(), TR_allconst, TR::NoType, tgt->incNumNodes(), 6, 0, 0);
    tgt->addNode(iall); // Multiply Factor
    TR_PCISCNode *cmah0
        = new (PERSISTENT_NEW) TR_PCISCNode(c->trMemory(), TR_ahconst, TR::NoType, tgt->incNumNodes(), 5, 0, 0, 0);
    tgt->addNode(cmah0); // array header
    TR_PCISCNode *cmah1
        = new (PERSISTENT_NEW) TR_PCISCNode(c->trMemory(), TR_ahconst, TR::NoType, tgt->incNumNodes(), 4, 0, 0, 1);
    tgt->addNode(cmah1); // array header
    TR_PCISCNode *cm1
        = new (PERSISTENT_NEW) TR_PCISCNode(c->trMemory(), TR::iconst, TR::Int32, tgt->incNumNodes(), 3, 0, 0, -1);
    tgt->addNode(cm1);
    TR_PCISCNode *ent
        = new (PERSISTENT_NEW) TR_PCISCNode(c->trMemory(), TR_entrynode, TR::NoType, tgt->incNumNodes(), 2, 1, 0);
    tgt->addNode(ent);
    ...
    TR_PCISCNode *n8 = new (PERSISTENT_NEW)
        TR_PCISCNode(c->trMemory(), TR_ifcmpall, TR::NoType, tgt->incNumNodes(), 1, 2, 2, n7, v4, vorc);
    tgt->addNode(n8);
    TR_PCISCNode *n9
        = new (PERSISTENT_NEW) TR_PCISCNode(c->trMemory(), TR_exitnode, TR::NoType, tgt->incNumNodes(), 0, 0, 0);
    tgt->addNode(n9);
...
//*****************************************************************************************
bool CISCTransform2ArrayCopyB2CorC2B(TR_CISCTransformer *trans)
{
    TR_ASSERT(trans->getOffsetOperand1() == 0 && trans->getOffsetOperand2() == 0, "Not implemented yet");
    TR::Node *trNode;
    TR::TreeTop *trTreeTop;
    ...
    if (isLoopPreheaderLastBlockInMethod(comp, block, disptrace)) {
        logprints(disptrace, comp->log(),
            "Bailing CISCTransform2ArrayCopyB2CorC2B due to null TT - might be a preheader in last block of method\n");
        return false;
    }
    ...
    TR::Node *inputMemNode
        = trans->getP2TRepInLoop(P->getImportantNode(0))->getHeadOfTrNodeInfo()->_node->duplicateTree();
    TR::Node *outputMemNode
        = trans->getP2TRepInLoop(P->getImportantNode(1))->getHeadOfTrNodeInfo()->_node->duplicateTree();
    TR::Node *inputNode = trans->getP2TRepInLoop(P->getImportantNode(0)->getChild(0))->getHeadOfTrNodeInfo()->_node;
    TR::Node *outputNode
        = trans->getP2TRepInLoop(P->getImportantNode(1)->getChild(0))->getHeadOfTrNodeInfo()->_node->duplicateTree();
    ...
    TR::Node *exitVarNode = createLoad(exitVarRepNode);
    variableORconstRepNode = convertStoreToLoad(comp, variableORconstRepNode);
    TR::Node *lengthNode = createOP2(comp, TR::isub, variableORconstRepNode, exitVarNode);
    TR::Node *updateTree1, *updateTree2;
    TR::Node *c2 = TR::Node::create(exitVarRepNode, TR::iconst, 0, 2);
    bool isExitVarChar;
    isExitVarChar = (dstIndexVarSymRef == exitVarSymRef) ? outputMemNode->getSize() == 2 : inputMemNode->getSize() == 2;
    // there are 2 scenarios
    //  dstIndexVarSymRef is a char (ie. outputMemNode size == 2, consequently inputMemNode == 1 and indexVarSymRef is a
    //  byte) or indexVarSymRef is a char (ie. inputMemNode size == 2, consequently outputMemNode == 1 and
    //  dstIndexVarSymRef is a byte) in each of these cases, its possible that each induction variable could be the loop
    //  controlling variable (ie. exitVarSymRef) ; thereby creating 4 possible conditions
    //
    ...
    } else // type is a charToByte copy
    {
        ...
        if (dstIndexVarSymRef == exitVarSymRef) {
            ...
            updateTree1 = createStoreOP2(comp, indexVarSymRef, TR::iadd, indexVarSymRef,
                TR::Node::create(TR::idiv, 2, lengthNode, c2), trNode);
...
TR_PCISCGraph *makeMemCpyByteToCharGraph(TR::Compilation *c, int32_t ctrl)
{
    ...
    tgt->addNode(v1); // src array index
    TR_PCISCNode *v3
        = new (PERSISTENT_NEW) TR_PCISCNode(c->trMemory(), TR_variable, TR::NoType, tgt->incNumNodes(), 14, 0, 0, 1);
    tgt->addNode(v3); // dst array index
    TR_PCISCNode *v4
        = new (PERSISTENT_NEW) TR_PCISCNode(c->trMemory(), TR_variable, TR::NoType, tgt->incNumNodes(), 13, 0, 0, 2);
    tgt->addNode(v4); // exit checking
    TR_PCISCNode *vorc
        = new (PERSISTENT_NEW) TR_PCISCNode(c->trMemory(), TR_quasiConst2, TR::NoType, tgt->incNumNodes(), 12, 0, 0);
    tgt->addNode(vorc); // length
    TR_PCISCNode *v0
        = new (PERSISTENT_NEW) TR_PCISCNode(c->trMemory(), TR_arraybase, TR::NoType, tgt->incNumNodes(), 11, 0, 0, 0);
    tgt->addNode(v0); // src array base
    TR_PCISCNode *v2
        = new (PERSISTENT_NEW) TR_PCISCNode(c->trMemory(), TR_arraybase, TR::NoType, tgt->incNumNodes(), 10, 0, 0, 1);
    tgt->addNode(v2); // dst array base
    TR_PCISCNode *cmah
        = new (PERSISTENT_NEW) TR_PCISCNode(c->trMemory(), TR_ahconst, TR::NoType, tgt->incNumNodes(), 9, 0, 0, 0);
    tgt->addNode(cmah); // array header
    TR_PCISCNode *cmah1 = createIdiomArrayRelatedConst(tgt, ctrl, tgt->incNumNodes(), 8,
        -(int32_t)(TR::Compiler->om.contiguousArrayHeaderSizeInBytes() + 1)); // array header+1
    TR_PCISCNode *ah1 = isBigEndian ? cmah : cmah1;
    TR_PCISCNode *ah2 = isBigEndian ? cmah1 : cmah;
    TR_PCISCNode *cm1
        = new (PERSISTENT_NEW) TR_PCISCNode(c->trMemory(), TR::iconst, TR::Int32, tgt->incNumNodes(), 7, 0, 0, -1);
    tgt->addNode(cm1);
    TR_PCISCNode *cm2
        = new (PERSISTENT_NEW) TR_PCISCNode(c->trMemory(), TR::iconst, TR::Int32, tgt->incNumNodes(), 6, 0, 0, -2);
    tgt->addNode(cm2);
    TR_PCISCNode *c2 = createIdiomArrayRelatedConst(tgt, ctrl, tgt->incNumNodes(), 5, 2); // element size
    TR_PCISCNode *c256
        = new (PERSISTENT_NEW) TR_PCISCNode(c->trMemory(), TR::iconst, TR::Int32, tgt->incNumNodes(), 4, 0, 0, 256);
    tgt->addNode(c256);
    ...
    TR_PCISCNode *nl10;
    if (ctrl & CISCUtilCtl_64Bit) {
        nl00 = new (PERSISTENT_NEW)
            TR_PCISCNode(c->trMemory(), TR::i2l, TR::Int64, tgt->incNumNodes(), 1, 1, 1, ns1, v1);
        tgt->addNode(nl00);
    ...
    TR_PCISCNode *nl12 = new (PERSISTENT_NEW)
        TR_PCISCNode(c->trMemory(), TR::bloadi, TR::Int8, tgt->incNumNodes(), 1, 1, 1, nl11, nl11);
    tgt->addNode(nl12);
    TR_PCISCNode *nl13 = new (PERSISTENT_NEW)
        TR_PCISCNode(c->trMemory(), TR::bu2i, TR::Int32, tgt->incNumNodes(), 1, 1, 1, nl12, nl12);
    tgt->addNode(nl13);
    TR_PCISCNode *nl14 = new (PERSISTENT_NEW)
        TR_PCISCNode(c->trMemory(), TR::imul, TR::Int32, tgt->incNumNodes(), 1, 1, 2, nl13, nl13, c256);
    tgt->addNode(nl14);
    ...
    TR_PCISCNode *nl22 = new (PERSISTENT_NEW)
        TR_PCISCNode(c->trMemory(), TR::bloadi, TR::Int8, tgt->incNumNodes(), 1, 1, 1, nl21, nl21);
    tgt->addNode(nl22);
    TR_PCISCNode *nl23 = new (PERSISTENT_NEW)
        TR_PCISCNode(c->trMemory(), TR::bu2i, TR::Int32, tgt->incNumNodes(), 1, 1, 1, nl22, nl22);
    tgt->addNode(nl23);
    TR_PCISCNode *ns2 = new (PERSISTENT_NEW)
        TR_PCISCNode(c->trMemory(), TR::ior, TR::Int32, tgt->incNumNodes(), 1, 1, 2, nl23, nl14, nl23);
    tgt->addNode(ns2);
    TR_PCISCNode *ns3
        = new (PERSISTENT_NEW) TR_PCISCNode(c->trMemory(), TR::i2s, TR::Int16, tgt->incNumNodes(), 1, 1, 1, ns2, ns2);
    tgt->addNode(ns3);
    TR_PCISCNode *ns4 = new (PERSISTENT_NEW)
        TR_PCISCNode(c->trMemory(), TR::sstorei, TR::Int16, tgt->incNumNodes(), 1, 1, 2, ns3, ns1, ns3);
    tgt->addNode(ns4);
    ...
    TR_PCISCNode *n8 = new (PERSISTENT_NEW)
        TR_PCISCNode(c->trMemory(), TR::ificmpge, TR::NoType, tgt->incNumNodes(), 1, 2, 2, n7, v4, vorc);
    tgt->addNode(n8);
    TR_PCISCNode *n9
        = new (PERSISTENT_NEW) TR_PCISCNode(c->trMemory(), TR_exitnode, TR::NoType, tgt->incNumNodes(), 0, 0, 0);
    tgt->addNode(n9);
...
TR_PCISCGraph *makeMemCpyCharToByteGraph(TR::Compilation *c, int32_t ctrl)
{
    ...
    TR_PCISCNode *c2 = createIdiomArrayRelatedConst(tgt, ctrl, tgt->incNumNodes(), 5, 2); // element size
    TR_PCISCNode *c8
        = new (PERSISTENT_NEW) TR_PCISCNode(c->trMemory(), TR::iconst, TR::Int32, tgt->incNumNodes(), 4, 0, 0, 8);
    tgt->addNode(c8);
    TR_PCISCNode *c1 = createIdiomArrayRelatedConst(tgt, ctrl, tgt->incNumNodes(), 3, 1); // element size
    TR_PCISCNode *ent
        = new (PERSISTENT_NEW) TR_PCISCNode(c->trMemory(), TR_entrynode, TR::NoType, tgt->incNumNodes(), 2, 1, 0);
    ...
    TR_PCISCNode *nl2 = new (PERSISTENT_NEW)
        TR_PCISCNode(c->trMemory(), TR::sloadi, TR::Int16, tgt->incNumNodes(), 1, 1, 1, nl1, nl1);
    tgt->addNode(nl2);
    TR_PCISCNode *cvt0, *cvt1;
    if ((ctrl & CISCUtilCtl_BigEndian)) {
        TR_PCISCNode *nc2i = new (PERSISTENT_NEW)
            TR_PCISCNode(c->trMemory(), TR::su2i, TR::Int32, tgt->incNumNodes(), 1, 1, 1, nl2, nl2);
        tgt->addNode(nc2i);
        TR_PCISCNode *ns22 = new (PERSISTENT_NEW)
            TR_PCISCNode(c->trMemory(), TR_ishrall, TR::NoType, tgt->incNumNodes(), 1, 1, 2, nc2i, nc2i, c8);
        tgt->addNode(ns22);
        cvt0 = new (PERSISTENT_NEW)
            TR_PCISCNode(c->trMemory(), TR::i2b, TR::Int8, tgt->incNumNodes(), 1, 1, 1, ns22, ns22);
        tgt->addNode(cvt0);
    } else {
        cvt0 = new (PERSISTENT_NEW)
            TR_PCISCNode(c->trMemory(), TR::s2b, TR::Int8, tgt->incNumNodes(), 1, 1, 1, nl2, nl2);
        tgt->addNode(cvt0);
    }
    TR_PCISCNode *ns14 = new (PERSISTENT_NEW)
        TR_PCISCNode(c->trMemory(), TR::bstorei, TR::Int8, tgt->incNumNodes(), 1, 1, 2, cvt0, ns11, cvt0);
    tgt->addNode(ns14);
    TR_PCISCNode *ns20 = createIdiomArrayAddressIndexTreeInLoop(tgt, ctrl | CISCUtilCtl_NoI2L, 1, ns14,
        ns10->getChild(0)->getChild(0), cmah1, c1);
    TR_PCISCNode *ns21 = createIdiomArrayAddressInLoop(tgt, ctrl, 1, ns20, v2, ns20);
    if ((ctrl & CISCUtilCtl_BigEndian)) {
        cvt1 = new (PERSISTENT_NEW)
            TR_PCISCNode(c->trMemory(), TR::s2b, TR::Int8, tgt->incNumNodes(), 1, 1, 1, ns21, nl2);
        tgt->addNode(cvt1);
    } else {
        TR_PCISCNode *nc2i = new (PERSISTENT_NEW)
            TR_PCISCNode(c->trMemory(), TR::su2i, TR::Int32, tgt->incNumNodes(), 1, 1, 1, ns21, nl2);
        tgt->addNode(nc2i);
        TR_PCISCNode *ns22 = new (PERSISTENT_NEW)
            TR_PCISCNode(c->trMemory(), TR_ishrall, TR::NoType, tgt->incNumNodes(), 1, 1, 2, nc2i, nc2i, c8);
        tgt->addNode(ns22);
    ...
    TR_PCISCNode *ns24 = new (PERSISTENT_NEW)
        TR_PCISCNode(c->trMemory(), TR::bstorei, TR::Int8, tgt->incNumNodes(), 1, 1, 2, cvt1, ns21, cvt1);
    tgt->addNode(ns24);
...
//*****************************************************************************************
bool CISCTransform2ArrayCopyB2CBndchk(TR_CISCTransformer *trans)
{
    TR_ASSERT(trans->getOffsetOperand1() == 0 && trans->getOffsetOperand2() == 0, "Not implemented yet");
    TR::Node *trNode;
    TR::TreeTop *trTreeTop;
    ...
    if (isLoopPreheaderLastBlockInMethod(comp, block, disptrace)) {
        logprints(disptrace, log,
            "Bailing CISCTransform2ArrayCopyB2CBndchk due to null TT - might be a preheader in last block of method\n");
        return false;
    }
    ...
    if (isExitVarChar) // The variable that checks the exit condition is for a 2-byte array.
    {
        ...
        updateTree1 = TR::Node::createWithSymRef(TR::istorei, 2, 2, indexLoadNode->getChild(0), endIndex,
            indexLoadNode->getSymbolReference());
...
TR_PCISCGraph *makeMemCpyByteToCharBndchkGraph(TR::Compilation *c, int32_t ctrl)
{
     ...
     * #child other/pred/children */
    TR_PCISCNode *v3
        = new (PERSISTENT_NEW) TR_PCISCNode(c->trMemory(), TR_variable, TR::NoType, tgt->incNumNodes(), 17, 0, 0, 0);
    tgt->addNode(v3); // dst array index
    TR_PCISCNode *v4
        = new (PERSISTENT_NEW) TR_PCISCNode(c->trMemory(), TR_variable, TR::NoType, tgt->incNumNodes(), 16, 0, 0, 1);
    tgt->addNode(v4); // exit checking
    TR_PCISCNode *vorc
        = new (PERSISTENT_NEW) TR_PCISCNode(c->trMemory(), TR_quasiConst2, TR::NoType, tgt->incNumNodes(), 15, 0, 0);
    tgt->addNode(vorc); // length
    TR_PCISCNode *alen
        = new (PERSISTENT_NEW) TR_PCISCNode(c->trMemory(), TR_quasiConst2, TR::NoType, tgt->incNumNodes(), 14, 0, 0);
    tgt->addNode(alen); // arraylength
    TR_PCISCNode *v0
        = new (PERSISTENT_NEW) TR_PCISCNode(c->trMemory(), TR_arraybase, TR::NoType, tgt->incNumNodes(), 13, 0, 0, 0);
    tgt->addNode(v0); // src array base
    TR_PCISCNode *v2
        = new (PERSISTENT_NEW) TR_PCISCNode(c->trMemory(), TR_arraybase, TR::NoType, tgt->incNumNodes(), 12, 0, 0, 1);
    tgt->addNode(v2); // dst array base
    TR_PCISCNode *ths
        = new (PERSISTENT_NEW) TR_PCISCNode(c->trMemory(), TR_arraybase, TR::NoType, tgt->incNumNodes(), 11, 0, 0, 2);
    tgt->addNode(ths); // this object
    TR_PCISCNode *aidx
        = new (PERSISTENT_NEW) TR_PCISCNode(c->trMemory(), TR_arrayindex, TR::NoType, tgt->incNumNodes(), 10, 0, 0, 0);
    tgt->addNode(aidx);
    ...
    TR_PCISCNode *ent
        = new (PERSISTENT_NEW) TR_PCISCNode(c->trMemory(), TR_entrynode, TR::NoType, tgt->incNumNodes(), 2, 1, 0);
    tgt->addNode(ent);
    TR_PCISCNode *idx0 = new (PERSISTENT_NEW)
        TR_PCISCNode(c->trMemory(), TR::iloadi, TR::Int32, tgt->incNumNodes(), 1, 1, 1, ent, ths);
    tgt->addNode(idx0);
    TR_PCISCNode *idx1 = new (PERSISTENT_NEW)
        TR_PCISCNode(c->trMemory(), TR::isub, TR::Int32, tgt->incNumNodes(), 1, 1, 2, idx0, idx0, cm1);
    tgt->addNode(idx1);
    TR_PCISCNode *idx2 = new (PERSISTENT_NEW)
        TR_PCISCNode(c->trMemory(), TR::istorei, TR::Int32, tgt->incNumNodes(), 1, 1, 2, idx1, ths, idx1);
    tgt->addNode(idx2);
    TR_PCISCNode *idx3 = new (PERSISTENT_NEW)
        TR_PCISCNode(c->trMemory(), TR::BNDCHK, TR::NoType, tgt->incNumNodes(), 1, 1, 2, idx2, alen, idx0);
    tgt->addNode(idx3);
    TR_PCISCNode *idx4 = new (PERSISTENT_NEW)
        TR_PCISCNode(c->trMemory(), TR::isub, TR::Int32, tgt->incNumNodes(), 1, 1, 2, idx3, idx0, cm2);
    tgt->addNode(idx4);
    TR_PCISCNode *idx5 = new (PERSISTENT_NEW)
        TR_PCISCNode(c->trMemory(), TR::istorei, TR::Int32, tgt->incNumNodes(), 1, 1, 2, idx4, ths, idx4);
    tgt->addNode(idx5);
    TR_PCISCNode *idx6 = new (PERSISTENT_NEW)
        TR_PCISCNode(c->trMemory(), TR::BNDCHK, TR::NoType, tgt->incNumNodes(), 1, 1, 2, idx5, alen, idx1);
    tgt->addNode(idx6);
    ...
    if (ctrl & CISCUtilCtl_64Bit) {
        nl00 = new (PERSISTENT_NEW)
            TR_PCISCNode(c->trMemory(), TR::i2l, TR::Int64, tgt->incNumNodes(), 1, 1, 1, ns1, idx0);
        tgt->addNode(nl00);
        nl10 = createIdiomArrayAddressIndexTreeInLoop(tgt, ctrl | CISCUtilCtl_NoI2L, 1, nl00, nl00, ah1, c1);
    } else {
        nl00 = idx0;
    ...
    tgt->addNode(nl23);
    TR_PCISCNode *ns2 = new (PERSISTENT_NEW)
        TR_PCISCNode(c->trMemory(), TR::iadd, TR::Int32, tgt->incNumNodes(), 1, 1, 2, nl23, nl14, nl23);
    tgt->addNode(ns2);
    TR_PCISCNode *ns3
        = new (PERSISTENT_NEW) TR_PCISCNode(c->trMemory(), TR::i2s, TR::Int16, tgt->incNumNodes(), 1, 1, 1, ns2, ns2);
    tgt->addNode(ns3);
    ...
    tgt->setAspects(isub | iadd | mul | bndchk | sameTypeLoadStore, ILTypeProp::Size_1 | ILTypeProp::Size_4,
        ILTypeProp::Size_2 | ILTypeProp::Size_4);
...
//*****************************************************************************************
bool CISCTransform2ArrayCopyC2BMixed(TR_CISCTransformer *trans)
{
    TR_ASSERT(trans->getOffsetOperand1() == 0 && trans->getOffsetOperand2() == 0, "Not implemented yet");
    TR::Node *trNode;
    TR::TreeTop *trTreeTop;
    ...
    if (isLoopPreheaderLastBlockInMethod(comp, block, disptrace)) {
        logprints(disptrace, log,
            "Bailing CISCTransform2ArrayCopyC2BMixed due to null TT - might be a preheader in last block of method\n");
        return false;
    }
    ...
    logprintf(disptrace, log, "All parameters: %x %x %x %x %x %x\n", LEloadMem, LEstoreMem, BEloadMem, BEstoreMem,
        flagIf, backIf);
    if (!LEloadMem || !LEstoreMem || !BEloadMem || !BEstoreMem || !flagIf || !backIf)
        return false;
    if (flagIf->getOpcode() != TR::ificmpeq && flagIf->getOpcode() != TR::ificmpne)
        return false;

    TR_ASSERT(searchNodeInBlock(flagIf->getSucc(1), LEloadMem) || searchNodeInBlock(flagIf->getSucc(1), BEloadMem),
        "error");
    TR_ASSERT(!searchNodeInBlock(flagIf->getSucc(1), LEloadMem) || !searchNodeInBlock(flagIf->getSucc(1), BEloadMem),
        "error");
    ...
    if (!isBig)
        LEalongJumpPath = !LEalongJumpPath;
    logprintf(disptrace, log, "LEalongJumpPath = %d\n", LEalongJumpPath);
    ...
    TR::Node *BEMemCpy = TR::Node::createArraycopy(BELoadAddrTree, BEStoreAddrTree,
        createI2LIfNecessary(comp, trans->isGenerateI2L(), lengthNode));
    ...
    if (comp->cg()->supportsByteswap()) {
        TR::Node *LEReverseStore = TR::Node::createWithSymRef(TR::sstorei, 2, 2, LEStoreAddrTree,
        ...
        ret = trans->searchNodeInTrees(isBig ? LEStoreAddrTree2 : LEStoreAddrTree, arrayHeaderConst, &replaceParent,
            &childNum);
        TR_ASSERT(ret, "error");
    ...
    blockLE->append(TR::TreeTop::create(comp,
        createStoreOP2(comp, dstIndexVarSymRef, TR::iadd, dstIndexVarSymRef, c2->duplicateTree(), trNode)));
    TR::Node *backIfLE = TR::Node::createif(TR::ificmplt, indexUpdateLE->getChild(0),
        variableORconstRepNode->duplicateTree(), blockLE->getEntry());
    ...
    List<TR::Node> guardList(comp->trMemory());
    guardList.add(
        TR::Node::createif(TR::ifiucmpgt, updateTree2->getChild(0)->duplicateTree(), createLoad(arrayLenRepNode)));
    ...
    if (!LEalongJumpPath)
        TR::Node::recreate(flagIfNode, flagIfNode->getOpCode().getOpCodeForReverseBranch());
...
TR_PCISCGraph *makeMEMCPYChar2ByteMixedGraph(TR::Compilation *c, int32_t ctrl)
{
     ...
     * #child other/pred/children */
    TR_PCISCNode *v1
        = new (PERSISTENT_NEW) TR_PCISCNode(c->trMemory(), TR_variable, TR::NoType, tgt->incNumNodes(), 18, 0, 0, 0);
    tgt->addNode(v1); // src array index
    TR_PCISCNode *v3
        = new (PERSISTENT_NEW) TR_PCISCNode(c->trMemory(), TR_variable, TR::NoType, tgt->incNumNodes(), 17, 0, 0, 1);
    tgt->addNode(v3); // dst array index
    TR_PCISCNode *alen
        = new (PERSISTENT_NEW) TR_PCISCNode(c->trMemory(), TR_quasiConst2, TR::NoType, tgt->incNumNodes(), 16, 0, 0);
    tgt->addNode(alen); // arraylength
    TR_PCISCNode *vorc
        = new (PERSISTENT_NEW) TR_PCISCNode(c->trMemory(), TR_quasiConst2, TR::NoType, tgt->incNumNodes(), 15, 0, 0);
    tgt->addNode(vorc); // length
    TR_PCISCNode *flag
        = new (PERSISTENT_NEW) TR_PCISCNode(c->trMemory(), TR_variable, TR::NoType, tgt->incNumNodes(), 14, 0, 0, 2);
    tgt->addNode(flag); // flag
    TR_PCISCNode *v0
        = new (PERSISTENT_NEW) TR_PCISCNode(c->trMemory(), TR_arraybase, TR::NoType, tgt->incNumNodes(), 13, 0, 0, 0);
    tgt->addNode(v0); // src array base
    TR_PCISCNode *v2
    ...
    TR_PCISCNode *cmah1 = createIdiomArrayRelatedConst(tgt, ctrl, tgt->incNumNodes(), 9,
        -(int32_t)(TR::Compiler->om.contiguousArrayHeaderSizeInBytes() + 1)); // array header+1
    TR_PCISCNode *cm1
        = new (PERSISTENT_NEW) TR_PCISCNode(c->trMemory(), TR::iconst, TR::Int32, tgt->incNumNodes(), 8, 0, 0, -1);
    tgt->addNode(cm1);
    TR_PCISCNode *cm2
        = new (PERSISTENT_NEW) TR_PCISCNode(c->trMemory(), TR::iconst, TR::Int32, tgt->incNumNodes(), 7, 0, 0, -2);
    tgt->addNode(cm2);
    TR_PCISCNode *c0
        = new (PERSISTENT_NEW) TR_PCISCNode(c->trMemory(), TR::iconst, TR::Int32, tgt->incNumNodes(), 6, 0, 0, 0);
    tgt->addNode(c0);
    TR_PCISCNode *c2 = createIdiomArrayRelatedConst(tgt, ctrl, tgt->incNumNodes(), 5, 2); // element size
    TR_PCISCNode *c8
        = new (PERSISTENT_NEW) TR_PCISCNode(c->trMemory(), TR::iconst, TR::Int32, tgt->incNumNodes(), 4, 0, 0, 8);
    ...
    TR_PCISCNode *ent
        = new (PERSISTENT_NEW) TR_PCISCNode(c->trMemory(), TR_entrynode, TR::NoType, tgt->incNumNodes(), 2, 1, 0);
    tgt->addNode(ent);
    TR_PCISCNode *fchk = new (PERSISTENT_NEW)
        TR_PCISCNode(c->trMemory(), TR_ifcmpall, TR::NoType, tgt->incNumNodes(), 1, 2, 2, ent, flag, c0);
    tgt->addNode(fchk);

    // big endian path
    TR_PCISCNode *bbck0 = new (PERSISTENT_NEW)
        TR_PCISCNode(c->trMemory(), TR::BNDCHK, TR::NoType, tgt->incNumNodes(), 1, 1, 2, fchk, alen, v3);
    tgt->addNode(bbck0);
    TR_PCISCNode *bld0
        = createIdiomCharArrayLoadInLoop(tgt, ctrl | CISCUtilCtl_ChildDirectConnected, 1, bbck0, v0, aidx0, cmah, c2);
    TR_PCISCNode *bnc2i = new (PERSISTENT_NEW)
        TR_PCISCNode(c->trMemory(), TR::su2i, TR::Int32, tgt->incNumNodes(), 1, 1, 1, bld0, bld0);
    tgt->addNode(bnc2i);
    TR_PCISCNode *bns22 = new (PERSISTENT_NEW)
        TR_PCISCNode(c->trMemory(), TR::iushr, TR::Int32, tgt->incNumNodes(), 1, 1, 2, bnc2i, bnc2i, c8);
    tgt->addNode(bns22);
    // TR_PCISCNode *bcvt0= new (PERSISTENT_NEW) TR_PCISCNode(c->trMemory(), TR::iu2i, TR::Int32, tgt->incNumNodes(), 1,
    // 1,   1,   bns22, bns22); tgt->addNode(bcvt0);
    TR_PCISCNode *bns0 = createIdiomArrayStoreInLoop(tgt, ctrl | CISCUtilCtl_ChildDirectConnected, 1, bns22,
        TR::bstorei, TR::Int8, v2, v3, cmah, c1, bns22);
    TR_PCISCNode *ba1 = new (PERSISTENT_NEW)
        TR_PCISCNode(c->trMemory(), TR::isub, TR::Int32, tgt->incNumNodes(), 1, 1, 2, bns0, v3, cm1);
    tgt->addNode(ba1);
    TR_PCISCNode *bbck1 = new (PERSISTENT_NEW)
        TR_PCISCNode(c->trMemory(), TR::BNDCHK, TR::NoType, tgt->incNumNodes(), 1, 1, 2, ba1, alen, ba1);
    tgt->addNode(bbck1);
    TR_PCISCNode *bns10
        = createIdiomArrayAddressInLoop(tgt, ctrl | CISCUtilCtl_ChildDirectConnected, 1, bbck1, v2, v3, cmah1, c1);
    TR_PCISCNode *bcvt1
        = new (PERSISTENT_NEW) TR_PCISCNode(c->trMemory(), TR::s2b, TR::Int8, tgt->incNumNodes(), 1, 1, 1, bns10, bld0);
    tgt->addNode(bcvt1);
    TR_PCISCNode *bns11 = new (PERSISTENT_NEW)
        TR_PCISCNode(c->trMemory(), TR::bstorei, TR::Int8, tgt->incNumNodes(), 1, 1, 2, bcvt1, bns10, bcvt1);
    tgt->addNode(bns11);
    TR_PCISCNode *bn6 = createIdiomDecVarInLoop(tgt, ctrl, 1, bns11, v3, cm2);

    // little endian path
    TR_PCISCNode *lbck0 = new (PERSISTENT_NEW)
        TR_PCISCNode(c->trMemory(), TR::BNDCHK, TR::NoType, tgt->incNumNodes(), 1, 1, 2, fchk, alen, v3);
    tgt->addNode(lbck0);
    TR_PCISCNode *lld0
        = createIdiomCharArrayLoadInLoop(tgt, ctrl | CISCUtilCtl_ChildDirectConnected, 1, lbck0, v0, aidx0, cmah, c2);
    TR_PCISCNode *lns10
        = createIdiomArrayAddressInLoop(tgt, ctrl | CISCUtilCtl_ChildDirectConnected, 1, lld0, v2, v3, cmah, c1);
    TR_PCISCNode *lcvt1
        = new (PERSISTENT_NEW) TR_PCISCNode(c->trMemory(), TR::s2b, TR::Int8, tgt->incNumNodes(), 1, 1, 1, lns10, lld0);
    tgt->addNode(lcvt1);
    TR_PCISCNode *lns11 = new (PERSISTENT_NEW)
        TR_PCISCNode(c->trMemory(), TR::bstorei, TR::Int8, tgt->incNumNodes(), 1, 1, 2, lcvt1, lns10, lcvt1);
    tgt->addNode(lns11);
    TR_PCISCNode *la1 = new (PERSISTENT_NEW)
        TR_PCISCNode(c->trMemory(), TR::isub, TR::Int32, tgt->incNumNodes(), 1, 1, 2, lns11, v3, cm1);
    tgt->addNode(la1);
    TR_PCISCNode *lbck1 = new (PERSISTENT_NEW)
        TR_PCISCNode(c->trMemory(), TR::BNDCHK, TR::NoType, tgt->incNumNodes(), 1, 1, 2, la1, alen, la1);
    tgt->addNode(lbck1);
    TR_PCISCNode *lnc2i = new (PERSISTENT_NEW)
        TR_PCISCNode(c->trMemory(), TR::su2i, TR::Int32, tgt->incNumNodes(), 1, 1, 1, lbck1, lld0);
    tgt->addNode(lnc2i);
    TR_PCISCNode *lns22 = new (PERSISTENT_NEW)
        TR_PCISCNode(c->trMemory(), TR::iushr, TR::Int32, tgt->incNumNodes(), 1, 1, 2, lnc2i, lnc2i, c8);
    tgt->addNode(lns22);
    // TR_PCISCNode *lcvt0= new (PERSISTENT_NEW) TR_PCISCNode(c->trMemory(), TR::iu2i, TR::Int32, tgt->incNumNodes(), 1,
    // 1,   1,   lns22, lns22); tgt->addNode(lcvt0);
    TR_PCISCNode *lns0 = createIdiomArrayStoreInLoop(tgt, ctrl | CISCUtilCtl_ChildDirectConnected, 1, lns22,
        TR::bstorei, TR::Int8, v2, v3, cmah1, c1, lns22);
    ...
    TR_PCISCNode *back = new (PERSISTENT_NEW)
        TR_PCISCNode(c->trMemory(), TR::ificmpge, TR::NoType, tgt->incNumNodes(), 1, 2, 2, addv1, v1, vorc);
    tgt->addNode(back);
    TR_PCISCNode *ext
        = new (PERSISTENT_NEW) TR_PCISCNode(c->trMemory(), TR_exitnode, TR::NoType, tgt->incNumNodes(), 0, 0, 0);
    tgt->addNode(ext);
...
//*****************************************************************************************
bool CISCTransform2ArrayCopyC2BIf2(TR_CISCTransformer *trans)
{
    TR_ASSERT(trans->getOffsetOperand1() == 0 && trans->getOffsetOperand2() == 0, "Not implemented yet");
    TR::Node *trNode;
    TR::TreeTop *trTreeTop;
    ...
    if (isLoopPreheaderLastBlockInMethod(comp, block, disptrace)) {
        logprints(disptrace, comp->log(),
            "Bailing CISCTransform2ArrayCopyC2BIf2 due to null TT - might be a preheader in last block of method\n");
        return false;
    }
    ...
    TR::Node *inputNode
        = trans->getP2TRepInLoop(P->getImportantNode(0)->getChild(0))->getHeadOfTrNodeInfo()->_node->duplicateTree();
    TR::Node *outputNode
        = trans->getP2TRepInLoop(P->getImportantNode(1)->getChild(0))->getHeadOfTrNodeInfo()->_node->duplicateTree();
    ...
    TR::Block *orgNextBlock = orgNextTreeTop->getNode()->getBlock();
    // chkLen1
    TR::SymbolReference *lengthCharTemp = comp->getSymRefTab()->createTemporary(comp->getMethodSymbol(), TR::Int32);
    TR::SymbolReference *lengthByteTemp = comp->getSymRefTab()->createTemporary(comp->getMethodSymbol(), TR::Int32);
    TR::SymbolReference *selectLen2 = comp->getSymRefTab()->createTemporary(comp->getMethodSymbol(), TR::Int32);
    ...
    //
    //
    TR::Node *lengthSrcNode = createOP2(comp, TR::isub, variableORconstRepNode, indexRepNode);
    TR::Node *storeSrcCharLen = TR::Node::createStore(lengthCharTemp, lengthSrcNode);
    TR::Node *storeSrcByteLen = TR::Node::createStore(lengthByteTemp, TR::Node::create(TR::imul, 2, lengthSrcNode, c2));
    TR::Node *zeroConst = TR::Node::create(indexRepNode, TR::iconst, 0, 0);
    TR::Node *storeSelectLen = TR::Node::createStore(selectLen2, zeroConst);
    TR::Node *lengthDstNode = createOP2(comp, TR::isub, variableORconstRepNode2, dstIndexRepNode);

    TR::Node *c1 = TR::Node::create(indexRepNode, TR::iconst, 0, 1);

    TR::Node *incr = c1->duplicateTree();
    lengthDstNode = TR::Node::create(TR::ishr, 2, lengthDstNode, incr);
    TR::Node *lengthDstDiv2Node
        = TR::Node::create(TR::isub, 2, lengthDstNode, TR::Node::create(indexRepNode, TR::iconst, 0, -1));
    ...
    TR::Node *storeSrcByteLen2
        = TR::Node::createStore(lengthByteTemp, TR::Node::create(TR::ishl, 2, lengthDstDiv2Node, c1->duplicateTree()));
    ...
    TR::Node *lenNode = createI2LIfNecessary(comp, trans->isGenerateI2L(),
        TR::Node::createWithSymRef(indexRepNode, TR::iload, 0, lengthByteTemp));
    ...
    if (!target) // multiple successor blocks
    {
        ...
        cmpExit = TR::Node::createif(TR::ificmpeq, TR::Node::createWithSymRef(indexRepNode, TR::iload, 0, selectLen2),
            TR::Node::create(indexRepNode, TR::iconst, 0, 0), okDest);
    }
    ...
    if (target) // single successor block
    {
        ...
        trans->setSuccessorEdges(bodyBlock, failDest->getEnclosingBlock(), okDest->getEnclosingBlock());
    }
...
TR_PCISCGraph *makeMEMCPYChar2ByteGraph2(TR::Compilation *c, int32_t ctrl)
{
    ...
    tgt->addNode(v3); // dst array index
    TR_PCISCNode *vorc
        = new (PERSISTENT_NEW) TR_PCISCNode(c->trMemory(), TR_quasiConst2, TR::NoType, tgt->incNumNodes(), 13, 0, 0);
    tgt->addNode(vorc); // length
    TR_PCISCNode *vorc2
        = new (PERSISTENT_NEW) TR_PCISCNode(c->trMemory(), TR_quasiConst2, TR::NoType, tgt->incNumNodes(), 12, 0, 0);
    tgt->addNode(vorc2); // length2
    TR_PCISCNode *v0
        = new (PERSISTENT_NEW) TR_PCISCNode(c->trMemory(), TR_arraybase, TR::NoType, tgt->incNumNodes(), 11, 0, 0, 0);
    tgt->addNode(v0); // src array base
    TR_PCISCNode *v2
    ...
    tgt->addNode(ent);
    TR_PCISCNode *lv1
        = new (PERSISTENT_NEW) TR_PCISCNode(c->trMemory(), TR::iload, TR::Int32, tgt->incNumNodes(), 1, 1, 1, ent, v1);
    tgt->addNode(lv1);
    ...
    TR_PCISCNode *ns20 = createIdiomArrayAddressIndexTreeInLoop(tgt, ctrl | CISCUtilCtl_NoI2L, 1, n6,
        ns10->getChild(0)->getChild(0), cmah1, c1);
    TR_PCISCNode *ns21 = createIdiomArrayAddressInLoop(tgt, ctrl, 1, ns20, v2, ns20);
    if ((ctrl & CISCUtilCtl_BigEndian)) {
        cvt1 = new (PERSISTENT_NEW)
    ...
    tgt->addNode(ns24);
    TR_PCISCNode *n8 = new (PERSISTENT_NEW)
        TR_PCISCNode(c->trMemory(), TR::ificmpge, TR::NoType, tgt->incNumNodes(), 1, 2, 2, ns24, v1, vorc);
    tgt->addNode(n8);
    TR_PCISCNode *n9 = new (PERSISTENT_NEW)
        TR_PCISCNode(c->trMemory(), TR::ificmpgt, TR::NoType, tgt->incNumNodes(), 1, 2, 2, n8, v3, vorc2);
    tgt->addNode(n9);
    TR_PCISCNode *n10
        = new (PERSISTENT_NEW) TR_PCISCNode(c->trMemory(), TR_exitnode, TR::NoType, tgt->incNumNodes(), 0, 0, 0);
    tgt->addNode(n10);
...
//        ImportantNodes(1) - array store
//*****************************************************************************************
bool CISCTransform2ArrayCopyB2I(TR_CISCTransformer *trans)
{
    TR_ASSERT(trans->getOffsetOperand1() == 0 && trans->getOffsetOperand2() == 0, "Not implemented yet");
    TR::Node *trNode;
    TR::TreeTop *trTreeTop;
    ...
    if (isLoopPreheaderLastBlockInMethod(comp, block, disptrace)) {
        logprints(disptrace, comp->log(),
            "Bailing CISCTransform2ArrayCopyB2I due to null TT - might be a preheader in last block of method\n");
        return false;
    }
...
TR_PCISCGraph *makeMEMCPYByte2IntGraph(TR::Compilation *c, int32_t ctrl)
{
    ...
    /************************************    opc               id        dagId #cfg #child other/pred/children */
    TR_PCISCNode *v1
        = new (PERSISTENT_NEW) TR_PCISCNode(c->trMemory(), TR_variable, TR::NoType, tgt->incNumNodes(), 16, 0, 0, 0);
    tgt->addNode(v1); // array index of src and dst
    TR_PCISCNode *vorc
        = new (PERSISTENT_NEW) TR_PCISCNode(c->trMemory(), TR_quasiConst2, TR::NoType, tgt->incNumNodes(), 15, 0, 0);
    tgt->addNode(vorc); // length
    TR_PCISCNode *v0
        = new (PERSISTENT_NEW) TR_PCISCNode(c->trMemory(), TR_arraybase, TR::NoType, tgt->incNumNodes(), 14, 0, 0, 0);
    tgt->addNode(v0); // src array base
    TR_PCISCNode *v2
        = new (PERSISTENT_NEW) TR_PCISCNode(c->trMemory(), TR_arraybase, TR::NoType, tgt->incNumNodes(), 13, 0, 0, 1);
    tgt->addNode(v2); // dst array base
    TR_PCISCNode *cmah
        = new (PERSISTENT_NEW) TR_PCISCNode(c->trMemory(), TR_ahconst, TR::NoType, tgt->incNumNodes(), 12, 0, 0, 0);
    tgt->addNode(cmah); // array header
    TR_PCISCNode *cmah1 = createIdiomArrayRelatedConst(tgt, ctrl, tgt->incNumNodes(), 12,
        -(int32_t)(TR::Compiler->om.contiguousArrayHeaderSizeInBytes() + 1)); // array header+1
    TR_PCISCNode *cmah2 = createIdiomArrayRelatedConst(tgt, ctrl, tgt->incNumNodes(), 11,
        -(int32_t)(TR::Compiler->om.contiguousArrayHeaderSizeInBytes() + 2)); // array header+2
    TR_PCISCNode *cmah3 = createIdiomArrayRelatedConst(tgt, ctrl, tgt->incNumNodes(), 10,
        -(int32_t)(TR::Compiler->om.contiguousArrayHeaderSizeInBytes() + 3)); // array header+3
    ...
    TR_PCISCNode *cm1
        = new (PERSISTENT_NEW) TR_PCISCNode(c->trMemory(), TR::iconst, TR::Int32, tgt->incNumNodes(), 9, 0, 0, -1);
    tgt->addNode(cm1);
    TR_PCISCNode *c4 = createIdiomArrayRelatedConst(tgt, ctrl, tgt->incNumNodes(), 8, 4); // element size
    TR_PCISCNode *ci4 = c4;
    if (ctrl & CISCUtilCtl_64Bit) {
        ci4 = new (PERSISTENT_NEW) TR_PCISCNode(c->trMemory(), TR::iconst, TR::Int32, tgt->incNumNodes(), 7, 0, 0, 4);
        tgt->addNode(ci4);
    }
    TR_PCISCNode *cs8
        = new (PERSISTENT_NEW) TR_PCISCNode(c->trMemory(), TR::iconst, TR::Int32, tgt->incNumNodes(), 6, 0, 0, 0x100);
    tgt->addNode(cs8);
    TR_PCISCNode *cs16
        = new (PERSISTENT_NEW) TR_PCISCNode(c->trMemory(), TR::iconst, TR::Int32, tgt->incNumNodes(), 5, 0, 0, 0x10000);
    tgt->addNode(cs16);
    TR_PCISCNode *cs24 = new (PERSISTENT_NEW)
        TR_PCISCNode(c->trMemory(), TR::iconst, TR::Int32, tgt->incNumNodes(), 4, 0, 0, 0x1000000);
    tgt->addNode(cs24);
    TR_PCISCNode *c1 = createIdiomArrayRelatedConst(tgt, ctrl, tgt->incNumNodes(), 3, 1); // element size
    TR_PCISCNode *ent
        = new (PERSISTENT_NEW) TR_PCISCNode(c->trMemory(), TR_entrynode, TR::NoType, tgt->incNumNodes(), 2, 1, 0);
    ...
    TR_PCISCNode *ns1 = createIdiomArrayAddressInLoop(tgt, ctrl, 1, ns0, v2, ns0);
    TR_PCISCNode *nmul = new (PERSISTENT_NEW)
        TR_PCISCNode(c->trMemory(), TR::imul, TR::Int32, tgt->incNumNodes(), 1, 1, 2, ns1, v1, ci4);
    tgt->addNode(nmul);
    ...
    tgt->addNode(nl13);
    TR_PCISCNode *nl14 = new (PERSISTENT_NEW)
        TR_PCISCNode(c->trMemory(), TR::imul, TR::Int32, tgt->incNumNodes(), 1, 1, 2, nl13, nl13, cs24);
    tgt->addNode(nl14);
    TR_PCISCNode *nl22 = createIdiomArrayLoadInLoop(tgt, ctrl, 1, nl14, TR::bloadi, TR::Int8, v0, nmul, ah2, c1);
    TR_PCISCNode *nl23 = new (PERSISTENT_NEW)
        TR_PCISCNode(c->trMemory(), TR::bu2i, TR::Int32, tgt->incNumNodes(), 1, 1, 1, nl22, nl22);
    tgt->addNode(nl23);
    TR_PCISCNode *nl24 = new (PERSISTENT_NEW)
        TR_PCISCNode(c->trMemory(), TR::imul, TR::Int32, tgt->incNumNodes(), 1, 1, 2, nl23, nl23, cs16);
    tgt->addNode(nl24);
    TR_PCISCNode *nl25 = new (PERSISTENT_NEW)
        TR_PCISCNode(c->trMemory(), TR::ior, TR::Int32, tgt->incNumNodes(), 1, 1, 2, nl24, nl14, nl24);
    tgt->addNode(nl25);
    TR_PCISCNode *nl32 = createIdiomArrayLoadInLoop(tgt, ctrl, 1, nl25, TR::bloadi, TR::Int8, v0, nmul, ah3, c1);
    TR_PCISCNode *nl33 = new (PERSISTENT_NEW)
        TR_PCISCNode(c->trMemory(), TR::bu2i, TR::Int32, tgt->incNumNodes(), 1, 1, 1, nl32, nl32);
    tgt->addNode(nl33);
    TR_PCISCNode *nl34 = new (PERSISTENT_NEW)
        TR_PCISCNode(c->trMemory(), TR::imul, TR::Int32, tgt->incNumNodes(), 1, 1, 2, nl33, nl33, cs8);
    tgt->addNode(nl34);
    TR_PCISCNode *nl35 = new (PERSISTENT_NEW)
        TR_PCISCNode(c->trMemory(), TR::ior, TR::Int32, tgt->incNumNodes(), 1, 1, 2, nl34, nl25, nl34);
    tgt->addNode(nl35);
    TR_PCISCNode *nl42 = createIdiomArrayLoadInLoop(tgt, ctrl, 1, nl35, TR::bloadi, TR::Int8, v0, nmul, ah4, c1);
    TR_PCISCNode *nl43 = new (PERSISTENT_NEW)
        TR_PCISCNode(c->trMemory(), TR::bu2i, TR::Int32, tgt->incNumNodes(), 1, 1, 1, nl42, nl42);
    tgt->addNode(nl43);
    TR_PCISCNode *nl45 = new (PERSISTENT_NEW)
        TR_PCISCNode(c->trMemory(), TR::ior, TR::Int32, tgt->incNumNodes(), 1, 1, 2, nl43, nl35, nl43);
    tgt->addNode(nl45);
    TR_PCISCNode *ns4 = new (PERSISTENT_NEW)
        TR_PCISCNode(c->trMemory(), TR::istorei, TR::Int32, tgt->incNumNodes(), 1, 1, 2, nl45, ns1, nl45);
    tgt->addNode(ns4);
    TR_PCISCNode *n6 = createIdiomDecVarInLoop(tgt, ctrl, 1, ns4, v1, cm1);
    TR_PCISCNode *n8 = new (PERSISTENT_NEW)
        TR_PCISCNode(c->trMemory(), TR::ificmpge, TR::NoType, tgt->incNumNodes(), 1, 2, 2, n6, v1, vorc);
    tgt->addNode(n8);
    TR_PCISCNode *n9
        = new (PERSISTENT_NEW) TR_PCISCNode(c->trMemory(), TR_exitnode, TR::NoType, tgt->incNumNodes(), 0, 0, 0);
    tgt->addNode(n9);
...
TR_PCISCGraph *makeMEMCPYInt2ByteGraph(TR::Compilation *c, int32_t ctrl)
{
    ...
    tgt->addNode(v2); // dst array base
    TR_PCISCNode *cmah
        = new (PERSISTENT_NEW) TR_PCISCNode(c->trMemory(), TR_ahconst, TR::NoType, tgt->incNumNodes(), 13, 0, 0, 0);
    tgt->addNode(cmah); // array header
    TR_PCISCNode *cmah1 = createIdiomArrayRelatedConst(tgt, ctrl, tgt->incNumNodes(), 12,
        -(int32_t)(TR::Compiler->om.contiguousArrayHeaderSizeInBytes() + 1)); // array header+1
    TR_PCISCNode *cmah2 = createIdiomArrayRelatedConst(tgt, ctrl, tgt->incNumNodes(), 11,
    ...
    tgt->addNode(cm1);
    TR_PCISCNode *cs4
        = new (PERSISTENT_NEW) TR_PCISCNode(c->trMemory(), TR::iconst, TR::Int32, tgt->incNumNodes(), 8, 0, 0, 4);
    tgt->addNode(cs4); // element size
    TR_PCISCNode *cl4 = cs4;
    if (ctrl & CISCUtilCtl_64Bit) {
        cl4 = new (PERSISTENT_NEW) TR_PCISCNode(c->trMemory(), TR::lconst, TR::Int64, tgt->incNumNodes(), 7, 0, 0, 4);
        tgt->addNode(cl4); // element size for 64-bit
    }
    TR_PCISCNode *cs8
        = new (PERSISTENT_NEW) TR_PCISCNode(c->trMemory(), TR::iconst, TR::Int32, tgt->incNumNodes(), 6, 0, 0, 8);
    tgt->addNode(cs8);
    TR_PCISCNode *cs16
        = new (PERSISTENT_NEW) TR_PCISCNode(c->trMemory(), TR::iconst, TR::Int32, tgt->incNumNodes(), 5, 0, 0, 16);
    tgt->addNode(cs16);
    TR_PCISCNode *cs24
        = new (PERSISTENT_NEW) TR_PCISCNode(c->trMemory(), TR::iconst, TR::Int32, tgt->incNumNodes(), 4, 0, 0, 24);
    tgt->addNode(cs24);
    TR_PCISCNode *c1 = createIdiomArrayRelatedConst(tgt, ctrl, tgt->incNumNodes(), 3, 1); // element size
    TR_PCISCNode *ent
        = new (PERSISTENT_NEW) TR_PCISCNode(c->trMemory(), TR_entrynode, TR::NoType, tgt->incNumNodes(), 2, 1, 0);
    tgt->addNode(ent);
    TR_PCISCNode *nmul = new (PERSISTENT_NEW)
        TR_PCISCNode(c->trMemory(), TR::imul, TR::Int32, tgt->incNumNodes(), 1, 1, 2, ent, v1, cs4);
    tgt->addNode(nmul);
    ...
    TR_PCISCNode *nl01 = new (PERSISTENT_NEW)
        TR_PCISCNode(c->trMemory(), TR::iloadi, TR::Int32, tgt->incNumNodes(), 1, 1, 1, nl00, nl00);
    tgt->addNode(nl01);
    TR_PCISCNode *ns01 = new (PERSISTENT_NEW)
        TR_PCISCNode(c->trMemory(), TR_ishrall, TR::NoType, tgt->incNumNodes(), 1, 1, 2, nl01, nl01, cs24);
    tgt->addNode(ns01);
    TR_PCISCNode *ns02
        = new (PERSISTENT_NEW) TR_PCISCNode(c->trMemory(), TR::i2b, TR::Int8, tgt->incNumNodes(), 1, 1, 1, ns01, ns01);
    tgt->addNode(ns02);
    TR_PCISCNode *ns03 = new (PERSISTENT_NEW)
        TR_PCISCNode(c->trMemory(), TR::bstorei, TR::Int8, tgt->incNumNodes(), 1, 1, 2, ns02, ns00, ns02);
    tgt->addNode(ns03);
    TR_PCISCNode *ns10 = createIdiomArrayAddressInLoop(tgt, ctrl, 1, ns03, v2, nmul, ah2, c1);
    TR_PCISCNode *ns11 = new (PERSISTENT_NEW)
        TR_PCISCNode(c->trMemory(), TR_ishrall, TR::NoType, tgt->incNumNodes(), 1, 1, 2, ns10, nl01, cs16);
    tgt->addNode(ns11);
    TR_PCISCNode *ns12
        = new (PERSISTENT_NEW) TR_PCISCNode(c->trMemory(), TR::i2b, TR::Int8, tgt->incNumNodes(), 1, 1, 1, ns11, ns11);
    tgt->addNode(ns12);
    TR_PCISCNode *ns13 = new (PERSISTENT_NEW)
        TR_PCISCNode(c->trMemory(), TR::bstorei, TR::Int8, tgt->incNumNodes(), 1, 1, 2, ns12, ns10, ns12);
    tgt->addNode(ns13);
    TR_PCISCNode *ns20 = createIdiomArrayAddressInLoop(tgt, ctrl, 1, ns13, v2, nmul, ah3, c1);
    TR_PCISCNode *ns21 = new (PERSISTENT_NEW)
        TR_PCISCNode(c->trMemory(), TR_ishrall, TR::NoType, tgt->incNumNodes(), 1, 1, 2, ns20, nl01, cs8);
    tgt->addNode(ns21);
    TR_PCISCNode *ns22
        = new (PERSISTENT_NEW) TR_PCISCNode(c->trMemory(), TR::i2b, TR::Int8, tgt->incNumNodes(), 1, 1, 1, ns21, ns21);
    tgt->addNode(ns22);
    TR_PCISCNode *ns23 = new (PERSISTENT_NEW)
        TR_PCISCNode(c->trMemory(), TR::bstorei, TR::Int8, tgt->incNumNodes(), 1, 1, 2, ns22, ns20, ns22);
    tgt->addNode(ns23);
    TR_PCISCNode *ns30 = createIdiomArrayAddressInLoop(tgt, ctrl, 1, ns23, v2, nmul, ah4, c1);
    TR_PCISCNode *ns32
        = new (PERSISTENT_NEW) TR_PCISCNode(c->trMemory(), TR::i2b, TR::Int8, tgt->incNumNodes(), 1, 1, 1, ns30, nl01);
    tgt->addNode(ns32);
    TR_PCISCNode *ns33 = new (PERSISTENT_NEW)
        TR_PCISCNode(c->trMemory(), TR::bstorei, TR::Int8, tgt->incNumNodes(), 1, 1, 2, ns32, ns30, ns32);
    tgt->addNode(ns33);
...
//*****************************************************************************************
static int32_t getAbs(int32_t val) { return val < 0 ? -val : val; }

bool CISCTransform2PtrArraySet(TR_CISCTransformer *trans)
{
    ...
    if (trans->getNumOfBBlistBody() > 1) {
        logprints(trace, log, "Need exactly 1 basic block\n");
        return false;
    }

    // Should have 3 treetops in body. See makePtrArraySetGraph
    if (block->getNumberOfRealTreeTops() != 3) {
        logprints(trace, log, "Need exactly 3 real treetops\n");
        return false;
    }
    ...
    if (!astore) {
        logprints(trace, log, "astore missing\n");
        return false;
    }
    if (!Store) {
        logprints(trace, log, "array element store missing\n");
        return false;
    }
    if (!ifcmp) {
        logprints(trace, log, "if compare missing\n");
        return false;
    }
    ...
    if (!(astoreNode->getChild(0)->getChild(0) == StoreNode->getChild(0)
        ...
        logprints(trace, log, "node trees not in required form\n");
        return false;
    }

    if (!ifcmpNode->getChild(0)->getOpCode().isLoadVar() && !ifcmpNode->getChild(1)->getOpCode().isLoadVar()) {
        logprints(trace, log, "neither comparands are loadvar\n");
        return false;
    }

    if (ifcmpNode->getChild(0)->getOpCode().isLoadVar() ^ ifcmpNode->getChild(1)->getOpCode().isLoadVar()) {
        auto nonLoadChild
            = (ifcmpNode->getChild(0)->getOpCode().isLoadVar()) ? ifcmpNode->getChild(1) : ifcmpNode->getChild(0);
        if (astoreNode->getChild(0) != nonLoadChild) {
            logprints(trace, log, "iv is not a commoned child in if comparand\n");
            return false;
        }
    }
    ...
    if (!ifcmpNode->getOpCode().isCompareForOrder()
        ...
        logprints(trace, log, "invalid compare condition\n");
        return false;
    }
    ...
    if (!StoreNode->getOpCode().isStoreIndirect()
        ...
        logprints(trace, log, "array element store node is neither indirect store nor matched with addr iv\n");
        return false;
    }
    ...
    switch (StoreNode->getSize()) {
        ...
        case 8:
            break;
        default:
            logprints(trace, log, "element size is not power-of-2 <= 8\n");
            return false;
    }

    if (StoreNode->getDataType() == TR::Aggregate) {
        logprints(trace, log, "arrayset can't handle aggregate elem type\n");
        return false;
    }

    auto increment = astoreNode->getChild(0)->getChild(1)->getConst<int32_t>();
    if (StoreNode->getSize() != getAbs(increment)) {
        logprints(trace, log, "increment size does not match element size\n");
        return false;
    }
    ...
    if (!endPtr) {
        logprints(trace, log, "Could not get end pointer\n");
        return false;
    }
...
bool CISCTransform2ArraySet(TR_CISCTransformer *trans)
{
    ...
    if (!cmpIfAllCISCNode) {
        logprints(disptrace, log, "Not implemented yet for multiple-if\n");
        return false;
    }
    ...
    for (inStoreCISCNode = ni.getFirst(); inStoreCISCNode; inStoreCISCNode = ni.getNext()) {
        if (!inStoreCISCNode->isOutsideOfLoop()) {
            ...
            if (!isIndexVariableInList(inStoreNode, &storeList)) {
                dumpOptDetails(comp,
                    "an index used in an array store %p is not consistent with the induction varaible updates\n",
                    inStoreNode);
                return false;
            }
            ...
            if (!trans->comp()->cg()->getSupportsArraySet()
                && !(inStoreNode->getType().isIntegral()
                    && inStoreNode->getSecondChild()->get64bitIntegralValueAsUnsigned() == 0)
                && !(inStoreNode->getType().isAddress() && inStoreNode->getSecondChild()->getAddress() == 0)) {
                dumpOptDetails(comp,
                    "the cg only supports arrayset to zero, but found a non-zero or non-constant value\n");
                return false;
            }
    ...
    if (trans->countGoodArrayIndex(indexVarSymRef) == 0 && trans->countGoodArrayIndex(indexVar1SymRef) == 0)
        return false;
    if (indexVarSymRef != indexVar1SymRef) {
        // there are two induction variables
        if (!listStores.isSingleton()) {
            dumpOptDetails(comp,
                "Multiple induction variables with multiple stores not supported for arrayset transformation.\n");
            return false;
        }
        if (!isIncrement1) {
    ...
    for (inStoreNode = iteratorStores.getFirst(); inStoreNode; inStoreNode = iteratorStores.getNext()) {
        ...
        if (valueNode->getOpCode().isLoadDirect() && valueNode->getOpCode().hasSymbolReference()) {
            if (valueNode->getSymbolReference()->getReferenceNumber()
    ...
    for (inStoreNode = iteratorStores.getFirst(); inStoreNode; inStoreNode = iteratorStores.getNext()) {
        ...
        if (findAndOrReplaceNodesWithMatchingSymRefNumber(outputNode->getSecondChild(), NULL,
                indexVarSymRef->getReferenceNumber())) {
            loopIsIncrement = isIncrement0;
        } else {
            TR_ASSERT(findAndOrReplaceNodesWithMatchingSymRefNumber(outputNode->getSecondChild(), NULL,
                          indexVar1SymRef->getReferenceNumber()),
                "Unable to find matching array access induction variable.\n");
        ...
        if (!loopIsIncrement) // count-down loop
        {
            ...
            bool isIndexVarUpdateBeforeArrayset
                = (trans->findStoreToSymRefInInsertBeforeNodes(indexVarSymRef->getReferenceNumber()) != NULL);
            ...
            if (isIndexVarUpdateBeforeArrayset) {
                ...
                TR::Node *origArrayIndVarLoad = findLoadWithMatchingSymRefNumber(
                    inStoreNode->getChild(0)->getSecondChild(), indexVarSymRef->getReferenceNumber());

                // If they match, we have case (B), so we need to readjust by +1.
                if (origIndVarLoad == origArrayIndVarLoad) {
                    logprintf(disptrace, log,
                        "Identified array index to have been referencing original induction variable value: %p\n",
                        origIndVarLoad);
            ...
            bool isFound = findAndOrReplaceNodesWithMatchingSymRefNumber(outputNode->getSecondChild(), lastLegalValue,
                indexVarSymRef->getReferenceNumber());
            if (!isFound && (indexVarSymRef != indexVar1SymRef))
                isFound = findAndOrReplaceNodesWithMatchingSymRefNumber(outputNode->getSecondChild(), lastLegalValue,
                    indexVar1SymRef->getReferenceNumber());
            ...
            // In the case of the induction variable update is before the array store, the indexNode value has already
            // been decremented by 1 once already (since i--; is inserted before the final arrayset.  We need to
            // readjust that.
            ...
            lengthNode
                = createOP2(comp, TR::iadd, lengthNode, TR::Node::create(indexNode, TR::iconst, 0, lengthAdjustment));
            ...
            computeIndex = createOP2(comp, TR::iadd, lastLegalValue,
                TR::Node::create(indexRepNode, TR::iconst, 0,
                    ((isIndexVarUpdateBeforeArrayset) ? 0 : -1) - arrayStoreCommoningAdjustment));
        ...
        if (elementSize > 1) {
            ...
            lengthByteNode = TR::Node::create(longOffsets ? TR::lmul : TR::imul, 2, lengthByteNode, elementSizeNode);
        }
...
bool CISCTransform2Strlen16(TR_CISCTransformer *trans)
{
    ...
    if (block->getNumberOfRealTreeTops() != 2) {
        logprints(trace, log, "Need exactly 2 real treetops\n");
        return false;
    }
    ...
    logprints(trace, log, "Failed one of the requirements\n");
    return false;
}
...
TR_PCISCGraph *makeStrlen16Graph(TR::Compilation *c, int32_t ctrl)
{
    ...
    auto entry
        = new (PERSISTENT_NEW) TR_PCISCNode(c->trMemory(), TR_entrynode, TR::NoType, tgt->incNumNodes(), 9, 1, 0);
    tgt->addNode(entry);

    auto ptr
        = new (PERSISTENT_NEW) TR_PCISCNode(c->trMemory(), TR_variable, TR::Address, tgt->incNumNodes(), 8, 0, 0, 0);
    tgt->addNode(ptr);
    auto increment
        = new (PERSISTENT_NEW) TR_PCISCNode(c->trMemory(), TR::lconst, TR::Int64, tgt->incNumNodes(), 7, 0, 0, 2);
    tgt->addNode(increment);
    auto addrAdd = new (PERSISTENT_NEW)
        TR_PCISCNode(c->trMemory(), TR::aladd, TR::Address, tgt->incNumNodes(), 6, 1, 2, entry, ptr, increment);
    tgt->addNode(addrAdd);
    auto addrStore = new (PERSISTENT_NEW)
        TR_PCISCNode(c->trMemory(), TR::astore, TR::Address, tgt->incNumNodes(), 5, 1, 2, addrAdd, addrAdd, ptr);
    tgt->addNode(addrStore);

    auto Load = new (PERSISTENT_NEW)
        TR_PCISCNode(c->trMemory(), TR_indload, TR::Int16, tgt->incNumNodes(), 4, 1, 1, addrStore, ptr);
    Load->addHint(addrAdd);
    tgt->addNode(Load);
    auto conversion = new (PERSISTENT_NEW)
        TR_PCISCNode(c->trMemory(), TR_conversion, TR::Int32, tgt->incNumNodes(), 3, 1, 1, Load, Load);
    tgt->addNode(conversion);
    auto nullChar
        = new (PERSISTENT_NEW) TR_PCISCNode(c->trMemory(), TR::iconst, TR::Int32, tgt->incNumNodes(), 2, 0, 0, 0);
    tgt->addNode(nullChar);
    auto loopTest = new (PERSISTENT_NEW) TR_PCISCNode(c->trMemory(), TR_ifcmpall, TR::Int32, tgt->incNumNodes(), 1, 2,
        2, conversion, conversion, nullChar);
    tgt->addNode(loopTest);
    ...
    // tgt->setAspects(storeMasks);  // not sure which to set, but do want astore aload for ptr incr and any size ptr
    // deref store
    tgt->setNoAspects(call | bndchk | bitop1, 0, 0);
    tgt->setMinCounts(1, 1, 0); // minimum ifCount, indirectLoadCount, indirectStoreCount
    tgt->setHotness(warm, false);
...
TR_PCISCGraph *makePtrArraySetGraph(TR::Compilation *c, int32_t ctrl)
{
    ...
    tgt->addNode(ptr);
    auto increment
        = new (PERSISTENT_NEW) TR_PCISCNode(c->trMemory(), TR::lconst, TR::Int64, tgt->incNumNodes(), 7, 0, 0);
    tgt->addNode(increment);
    auto addrAdd = new (PERSISTENT_NEW) TR_PCISCNode(c->trMemory(), is64bit ? TR::aladd : TR::aiadd, TR::Address,
        tgt->incNumNodes(), 6, 1, 2, entry, ptr, increment);
    tgt->addNode(addrAdd);
    auto addrStore = new (PERSISTENT_NEW)
        TR_PCISCNode(c->trMemory(), TR::astore, TR::Address, tgt->incNumNodes(), 5, 1, 2, addrAdd, addrAdd, ptr);
    tgt->addNode(addrStore);
    auto value
        = new (PERSISTENT_NEW) TR_PCISCNode(c->trMemory(), TR_variableORconst, TR::NoType, tgt->incNumNodes(), 4, 0, 0);
    tgt->addNode(value); // set value
    auto Store = new (PERSISTENT_NEW)
        TR_PCISCNode(c->trMemory(), TR_indstore, TR::NoType, tgt->incNumNodes(), 3, 1, 2, addrStore, ptr, value);
    tgt->addNode(Store);
    auto endPtr = new (PERSISTENT_NEW)
        TR_PCISCNode(c->trMemory(), TR_variableORconst, TR::Address, tgt->incNumNodes(), 2, 0, 0, 0);
    tgt->addNode(endPtr);
    auto loopTest = new (PERSISTENT_NEW)
        TR_PCISCNode(c->trMemory(), TR_ifcmpall, TR::Address, tgt->incNumNodes(), 1, 2, 2, Store, ptr, endPtr);
    tgt->addNode(loopTest);
    auto exit = new (PERSISTENT_NEW) TR_PCISCNode(c->trMemory(), TR_exitnode, TR::NoType, tgt->incNumNodes(), 0, 0, 0);
    tgt->addNode(exit);
...
TR_PCISCGraph *makeMemSetGraph(TR::Compilation *c, int32_t ctrl)
{
    ...
    TR_PCISCNode *iv1
        = new (PERSISTENT_NEW) TR_PCISCNode(c->trMemory(), TR_variable, TR::NoType, tgt->incNumNodes(), 10, 0, 0, 1);
    tgt->addNode(iv1); // array index
    TR_PCISCNode *Array = new (PERSISTENT_NEW)
        TR_PCISCNode(c->trMemory(), TR_arraybase, TR::NoType, tgt->incNumNodes(), 9, 0, 0, 0); // array base
    tgt->addNode(Array);
    TR_PCISCNode *end
        = new (PERSISTENT_NEW) TR_PCISCNode(c->trMemory(), TR_quasiConst2, TR::NoType, tgt->incNumNodes(), 8, 0, 0);
    tgt->addNode(end); // length
    // if cg only supports arrayset to zero only match constant nodes
    TR_PCISCNode *value = new (PERSISTENT_NEW) TR_PCISCNode(c->trMemory(),
        c->cg()->getSupportsArraySet() ? TR_variableORconst : TR_allconst, TR::NoType, tgt->incNumNodes(), 7, 0, 0);
    tgt->addNode(value); // set value
    TR_PCISCNode *mulConst
        = new (PERSISTENT_NEW) TR_PCISCNode(c->trMemory(), TR_allconst, TR::NoType, tgt->incNumNodes(), 6, 0, 0);
    tgt->addNode(mulConst); // Multiplicative factor for index into non-byte arrays
    TR_PCISCNode *idx0
        = new (PERSISTENT_NEW) TR_PCISCNode(c->trMemory(), TR_arrayindex, TR::NoType, tgt->incNumNodes(), 5, 0, 0, 0);
    tgt->addNode(idx0);
    TR_PCISCNode *aHeader
        = new (PERSISTENT_NEW) TR_PCISCNode(c->trMemory(), TR_ahconst, TR::NoType, tgt->incNumNodes(), 4, 0, 0, 0);
    tgt->addNode(aHeader); // array header
    TR_PCISCNode *increment
        = new (PERSISTENT_NEW) TR_PCISCNode(c->trMemory(), TR::iconst, TR::Int32, tgt->incNumNodes(), 3, 0, 0, -1);
    tgt->addNode(increment);
    ...
    TR_PCISCNode *i2x = new (PERSISTENT_NEW)
        TR_PCISCNode(c->trMemory(), TR_conversion, TR::NoType, tgt->incNumNodes(), 1, 1, 1, Addr, value);
    tgt->addNode(i2x);
    TR_PCISCNode *Store = new (PERSISTENT_NEW)
        TR_PCISCNode(c->trMemory(), TR_indstore, TR::NoType, tgt->incNumNodes(), 1, 1, 2, i2x, Addr, i2x);
    tgt->addNode(Store);
    ...
    TR_PCISCNode *loopTest = new (PERSISTENT_NEW)
        TR_PCISCNode(c->trMemory(), TR_ifcmpall, TR::NoType, tgt->incNumNodes(), 1, 2, 2, iv1Store, iv, end);
    tgt->addNode(loopTest);
    TR_PCISCNode *exit
        = new (PERSISTENT_NEW) TR_PCISCNode(c->trMemory(), TR_exitnode, TR::NoType, tgt->incNumNodes(), 0, 0, 0);
    tgt->addNode(exit);
...
//*****************************************************************************************
bool CISCTransform2MixedArraySet(TR_CISCTransformer *trans)
{
    const bool disptrace = DISPTRACE(trans);
    TR_ASSERT(trans->getOffsetOperand1() == 0 && trans->getOffsetOperand2() == 0, "Not implemented yet");
    TR::Node *trNode;
    ...
    if (isLoopPreheaderLastBlockInMethod(comp, block, disptrace)) {
        logprints(disptrace, comp->log(),
            "Bailing CISCTransform2MixedArraySet due to null TT - might be a preheader in last block of method\n");
        return false;
    }
    ...
    for (inStoreNode = iteratorStores.getFirst(); inStoreNode; inStoreNode = iteratorStores.getNext()) {
        ...
        if (!isIncrement) // count-down loop
        {
            ...
            bool isInitOffset0 = (variableORconstRepNode1->getOpCodeValue() == TR::iconst
                && (variableORconstRepNode1->getInt() - lengthMod) == 0);
            bool done = false;
            ...
            if (isInitOffset0) {
                ...
                if (lengthMod) {
                    lastValue
                        = createOP2(comp, TR::isub, lastValue, TR::Node::create(indexNode, TR::iconst, 0, lengthMod));
                }
                lengthNode = createOP2(comp, TR::isub, indexNode, lastValue);
                computeIndex = createOP2(comp, TR::iadd, lastValue, constm1);
            }
            lengthNode
                = createOP2(comp, TR::isub, lengthNode, TR::Node::create(indexNode, TR::iconst, 0, -(lengthMod + 1)));
        ...
        } else // count-up loop
        {
            TR::Node *lastValue = convertStoreToLoad(comp, variableORconstRepNode1);
            if (lengthMod) {
                lastValue = createOP2(comp, TR::iadd, lastValue, TR::Node::create(indexNode, TR::iconst, 0, lengthMod));
            }
            lengthNode = createOP2(comp, TR::isub, lastValue, indexNode);
            computeIndex = lastValue;
        }
        ...
        if (elementSize > 1)
            lengthNode
                = TR::Node::create(TR::imul, 2, lengthNode, TR::Node::create(inStoreNode, TR::iconst, 0, elementSize));
...
TR_PCISCGraph *makeMixedMemSetGraph(TR::Compilation *c, int32_t ctrl)
{
    ...
    tgt->addNode(end); // length
    TR_PCISCNode *Array = new (PERSISTENT_NEW)
        TR_PCISCNode(c->trMemory(), TR_arraybase, TR::NoType, tgt->incNumNodes(), 10, 0, 0, 0); // array base
    tgt->addNode(Array);
    TR_PCISCNode *byteArray = new (PERSISTENT_NEW)
        TR_PCISCNode(c->trMemory(), TR_arraybase, TR::NoType, tgt->incNumNodes(), 9, 0, 0, 1); // array base
    tgt->addNode(byteArray);
    TR_PCISCNode *value1
        = new (PERSISTENT_NEW) TR_PCISCNode(c->trMemory(), TR_variableORconst, TR::NoType, tgt->incNumNodes(), 8, 0, 0);
    tgt->addNode(value1); // set value
    TR_PCISCNode *value2
        = new (PERSISTENT_NEW) TR_PCISCNode(c->trMemory(), TR_variableORconst, TR::NoType, tgt->incNumNodes(), 7, 0, 0);
    tgt->addNode(value2); // set value
    TR_PCISCNode *mulConst
        = new (PERSISTENT_NEW) TR_PCISCNode(c->trMemory(), TR_allconst, TR::NoType, tgt->incNumNodes(), 6, 0, 0);
    tgt->addNode(mulConst); // Multiplicative factor for index into non-byte arrays
    TR_PCISCNode *idx0
    ...
    TR_PCISCNode *Addr = createIdiomArrayAddressInLoop(tgt, ctrl, 1, entry, Array, idx0, aHeader, mulConst);
    TR_PCISCNode *i2x = new (PERSISTENT_NEW)
        TR_PCISCNode(c->trMemory(), TR_conversion, TR::NoType, tgt->incNumNodes(), 1, 1, 1, Addr, value1);
    tgt->addNode(i2x);
    TR_PCISCNode *Store = new (PERSISTENT_NEW)
        TR_PCISCNode(c->trMemory(), TR_inbstore, TR::NoType, tgt->incNumNodes(), 1, 1, 2, i2x, Addr, i2x);
    tgt->addNode(Store);
    TR_PCISCNode *byteAddr = createIdiomArrayAddressInLoop(tgt, ctrl, 1, Store, byteArray, idx0, aHeader, c1);
    TR_PCISCNode *i2b = new (PERSISTENT_NEW)
        TR_PCISCNode(c->trMemory(), TR::i2b, TR::Int8, tgt->incNumNodes(), 1, 1, 1, byteAddr, value2);
    tgt->addNode(i2b);
    TR_PCISCNode *byteStore = new (PERSISTENT_NEW)
        TR_PCISCNode(c->trMemory(), TR::bstorei, TR::Int8, tgt->incNumNodes(), 1, 1, 2, i2b, byteAddr, i2b);
...
//*****************************************************************************************
bool CISCTransform2ArrayCmp2Ifs(TR_CISCTransformer *trans)
{
    const bool disptrace = DISPTRACE(trans);
    TR_ASSERT(trans->getOffsetOperand1() == 0 && trans->getOffsetOperand2() == 0, "Not implemented yet");
    TR::Node *trNode;
    ...
    if (isLoopPreheaderLastBlockInMethod(comp, block, disptrace)) {
        logprints(disptrace, comp->log(),
            "Bailing CISCTransform2ArrayCmp2Ifs due to null TT - might be a preheader in last block of method\n");
        return false;
    }
    ...
    // Get the size of elements
    if (!getMultiplier(trans, P->getImportantNode(6), &mulFactorNode, &elementSize, inSrc1Node->getType()))
        return false;
    if (elementSize != inSrc1Node->getSize() || elementSize != inSrc2Node->getSize()) {
        logprintf(disptrace, comp->log(),
            "CISCTransform2ArrayCmp2Ifs failed - Size Mismatch.  Element Size: %d InSrc1Size: %d inSrc2Size: %d\n",
    ...
    getP2TTrRepNodes(trans, &src1IdxRepNode, &src2IdxRepNode, &src1BaseRepNode, &src2BaseRepNode,
        &variableORconstRepNode);
    if (!src2IdxRepNode)
        src2IdxRepNode = src1IdxRepNode;
    TR::SymbolReference *src1IdxSymRef = src1IdxRepNode->getSymbolReference();
    if (!trans->analyzeArrayIndex(src1IdxSymRef))
        return false;
    ...
    if (arrayindex0)
        start1Idx = trans->getP2TRep(arrayindex0)->getHeadOfTrNodeInfo()->_node;
    if (arrayindex1)
        start2Idx = trans->getP2TRep(arrayindex1)->getHeadOfTrNodeInfo()->_node;
    ...
    if (shrCount != 0) {
        equalLen = TR::Node::create(TR::lshr, 2, equalLen, TR::Node::iconst(shrCount));
    }
    ...
    TR::SymbolReference *diffSymRef = comp->getSymRefTab()->createTemporary(comp->getMethodSymbol(), dataType);
    tmpNode = TR::Node::createStore(diffSymRef,
        TR::Node::create(dataType == TR::Int32 ? TR::isub : TR::lsub, 2, ifChild[0]->getHeadOfTrNodeInfo()->_node,
    ...
    tmpNode = TR::Node::createif((TR::ILOpCodes)cmpneIfAllCISCNode[0]->getOpcode(), loadNode, constNode, failDest[0]);
    tmpTreeTop = TR::TreeTop::create(comp, tmpNode);
    newLastTreeTop[0]->join(tmpTreeTop);
    newLastTreeTop[0] = tmpTreeTop;

    //  Second One
    tmpNode = TR::Node::createif((TR::ILOpCodes)cmpneIfAllCISCNode[1]->getOpcode(), loadNode->duplicateTree(),
        constNode->duplicateTree(), failDest[1]);
    tmpTreeTop = TR::TreeTop::create(comp, tmpNode);
    ...
    trans->setSuccessorEdges(if1Block, okDest->getEnclosingBlock(), failDest[1]->getEnclosingBlock());

    trans->setSuccessorEdges(block, if1Block, failDest[0]->getEnclosingBlock());
...
//*****************************************************************************************
bool CISCTransform2ArrayCmp(TR_CISCTransformer *trans)
{
    TR_ASSERT(trans->getOffsetOperand1() == 0 && trans->getOffsetOperand2() == 0, "Not implemented yet");
    const bool disptrace = DISPTRACE(trans);
    TR::Node *trNode;
    ...
    if (isLoopPreheaderLastBlockInMethod(comp, block, disptrace, &preHeader)) {
        logprints(disptrace, log,
            "Bailing CISCTransform2ArrayCmp due to null TT - might be a preheader in last block of method\n");
        return false;
    }
    ...
    if (preHeader) {
        logprintf(disptrace, log, "found preheader to be %d\n", preHeader->getNumber());
        //
        // obtain a CISCNode of each store for incrementing induction variables

        // check if any of the loop indices are defined between the preheader first tree and the first node found to
        // match idiom
        ...
        logprintf(disptrace, log,
            "searching for stores to loop indices between preheader first tree %p and first matching tree %p, looking "
            "for symrefnum %d %d\n",
        ...
        for (TR::TreeTop *tt = preHeader->getFirstRealTreeTop(); tt && tt != trTreeTop; tt = tt->getNextRealTreeTop()) {
            ...
            if (tempNode->getOpCode().isStore() && tempNode->getOpCode().hasSymbolReference()
                ...
                logprintf(disptrace, log,
                    "Bailing CISCTransform2ArrayCmp due to unexpected store (%p) of one of the indices prior to the "
                    "idiom\n",
                    tempNode);
                return false;
            }
    ...
    if (T && T->getAspects()->getIfCount() > 2) {
        logprintf(disptrace, log, "CISCTransform2ArrayCmp detected %d if-stmts in loop (> 2).  Not transforming.\n",
            T->getAspects()->getIfCount());
        return false;
    }
    ...
    if (trans->isBlockInLoopBody(failDest->getNode()->getBlock())) {
        logprints(disptrace, log,
            "CISCTransform2ArrayCmp failing transformer, ifcmpall test branch does not exit the loop.\n");
        return false;
    }
    ...
    if (true == (compareTo = trans->isCompareTo())) {
        if (!generateArraycmplen) {
            ...
            if (trans->canConvertArrayCmpSign(trans->getP2TRep(P->getImportantNode(7))->getHeadOfTrNode(), &compareIfs,
                    &canConvertToArrayCmp)) {
    ...
    TR_CISCNode *src1CISCNode = trans->getP2TInLoopIfSingle(P->getImportantNode(0));
    TR_CISCNode *src2CISCNode = trans->getP2TInLoopIfSingle(P->getImportantNode(1));
    if (!src1CISCNode || !src2CISCNode || src1CISCNode == src2CISCNode)
        return false;
    TR::Node *inSrc1Node = src1CISCNode->getHeadOfTrNodeInfo()->_node;
    TR::Node *inSrc2Node = src2CISCNode->getHeadOfTrNodeInfo()->_node;
    ...
    if (generateArraycmpsign) {
        if (!comp->cg()->getSupportsArrayCmpSign()
            || !((inSrc1Node->getType().isIntegral() && src1CISCNode->getIlOpCode().isUnsigned())
                || inSrc1Node->getType().isAddress())) {
            // arrayCmpLen can be reduced to arrayCmpSign, but either codegen does not support it
            // or we can't guarantee it works with byte-by-byte comparisons (only allow addresses and unsigned
            // integrals)
            generateArraycmpsign = false;
            generateArraycmplen = true;
        }
    ...
    if (generateArraycmplen) {
        if (!comp->cg()->getSupportsArrayCmpLen()) {
            dumpOptDetails(comp,
                "Bailing CISCTransform2ArrayCmp. Arraycmplen is needed to continue this transformation, but codgen "
                "does not support arraycmplen.\n");
            return false;
        }
    }
    ...
    if (!areArraysInvariant(comp, inSrc1Node, inSrc2Node, T)) {
        logprintf(disptrace, log, "input array bases %p and %p are not invariant, no reduction\n", inSrc1Node,
            inSrc2Node);
        return false;
    }
    ...
    if (inSrc1Node->getType() != inSrc2Node->getType()) {
        logprintf(disptrace, log, "CISCTransform2ArrayCmp failed - Array access types differ. inSrc1: %s, inSrc2: %s\n",
            TR::DataType::getName(inSrc1Node->getType()), TR::DataType::getName(inSrc2Node->getType()));
        return false; // Size is mismatch!
    }

    const uint32_t expectedSize
        = inSrc1Node->getType().isAddress() ? TR::Compiler->om.sizeofReferenceField() : inSrc1Node->getSize();
    ...
    if (isDecrement) // count-down loop
    {
        ...
        logprintf(disptrace, log, "isDecrement start1Idx %p start2Idx %p end1Idx %p end2Idx %p\n", start1Idx, start2Idx,
            end1Idx, end2Idx);
        startNode = start2Idx->duplicateTree();
        endNode = useSrc1 ? end1Idx->duplicateTree() : end2Idx->duplicateTree();

        if (arrayindex0)
            end1Idx = trans->getP2TRep(arrayindex0)->getHeadOfTrNodeInfo()->_node;
        if (arrayindex1)
            end2Idx = trans->getP2TRep(arrayindex1)->getHeadOfTrNodeInfo()->_node;
        ...
        logprintf(disptrace, log, "start1Idx %p start2Idx %p end1Idx %p end2Idx %p\n", start1Idx, start2Idx, end1Idx,
            end2Idx);
    ...
    TR::Node *lengthNode;
    if (addLength1) {
        lengthNode = createOP2(comp, TR::isub, diff2, TR::Node::create(src1BaseRepNode, TR::iconst, 0, -1));
    } else {
        lengthNode = diff2;
    }
    ...
    if (compareTo) {
        if (generateArraycmplen) {
            ...
            tmpNode = TR::Node::createStore(tmpNode->getSymbolReference(), TR::Node::create(tmpNode, TR::iconst, 0, 0));
    ...
    if (generateArraycmplen) {
        ...
        if (shrCount != 0) {
            equalLen = TR::Node::create(TR::lshr, 2, equalLen, TR::Node::create(equalLen, TR::iconst, 0, shrCount));
        }
        equalLen = TR::Node::create(TR::l2i, 1, equalLen);
        ...
        if (compareTo) {
            ...
            if (generateArraycmpsign) {
                ...
                cmpIfNode = TR::Node::createif(TR::ificmpeq, arraycmp,
                    TR::Node::create(src1BaseRepNode, TR::iconst, 0, 0), okDest);
            } else {
                logprints(disptrace, log, "ArrayCmp: Convert compareTo into equals!\n");
                topArraycmp = TR::Node::createStore(storeCompareToResult->getSymbolReference(),
                    createOP2(comp, TR::isub, TR::Node::create(src1BaseRepNode, TR::iconst, 0, 1),
                        TR::Node::create(TR::iand, 2,
                            TR::Node::create(TR::ixor, 2, arraycmp, TR::Node::iconst(arraycmp, 1)),
                            TR::Node::create(TR::ishr, 2,
                                TR::Node::create(TR::ixor, 2, arraycmp, TR::Node::iconst(arraycmp, 2)),
                                TR::Node::iconst(arraycmp, 1)))));

                cmpIfNode = TR::Node::createif(TR::ificmpeq, arraycmp,
                    TR::Node::create(src1BaseRepNode, TR::iconst, 0, 0), okDest);
            }
        } else {
            topArraycmp = TR::Node::createStore(src1IdxSymRef,
                TR::Node::create(TR::iand, 2, TR::Node::create(TR::ixor, 2, arraycmp, TR::Node::iconst(arraycmp, 1)),
                    TR::Node::create(TR::ishr, 2,
                        TR::Node::create(TR::ixor, 2, arraycmp, TR::Node::iconst(arraycmp, 2)),
                        TR::Node::iconst(arraycmp, 1))));
            cmpIfNode = TR::Node::createif(TR::ificmpeq, arraycmp, TR::Node::create(src1BaseRepNode, TR::iconst, 0, 0),
                okDest);
        }
    ...
    if (needVersioned) // Need to version the loop to eliminate array bounds checking
    {
        ...
        len1 = TR::Node::create(TR::arraylength, 1, convertStoreToLoad(comp, src1BaseRepNode));
        ...
        len2 = TR::Node::create(TR::arraylength, 1, convertStoreToLoad(comp, src2BaseRepNode));
        ...
        trans->setSuccessorEdges(fastpath, gotoBlock, okDest->getEnclosingBlock());
    ...
    if (0 && isCompensateCode) {
        ...
        //      arraycmp
        //      ...
        logprintf(disptrace, log, "cmpifallciscnode %d ifcmpge %d\n", cmpIfAllCISCNode->getOpcode(), TR::ificmpge);
        TR::Node *compareNode = TR::Node::createif((TR::ILOpCodes)cmpIfAllCISCNode->getOpcode(), startNode, endNode,
            compensateBlock0->getEntry());
...
bool CISCTransform2ArrayCmpCompareTo(TR_CISCTransformer *trans)
{
    trans->setCompareTo();
    return CISCTransform2ArrayCmp(trans);
}

bool CISCTransform2ArrayCmpIndexOf(TR_CISCTransformer *trans)
{
    trans->setIndexOf();
    return CISCTransform2ArrayCmp(trans);
}
...
TR_PCISCGraph *makeMemCmpGraph(TR::Compilation *c, int32_t ctrl)
{
    ...
    /************************************    opc               id        dagId #cfg #child other/pred/children */
    TR_PCISCNode *v1
        = new (PERSISTENT_NEW) TR_PCISCNode(c->trMemory(), TR_variable, TR::NoType, tgt->incNumNodes(), 13, 0, 0, 0);
    tgt->addNode(v1); // array index for src1
    TR_PCISCNode *v2
        = new (PERSISTENT_NEW) TR_PCISCNode(c->trMemory(), TR_variable, TR::NoType, tgt->incNumNodes(), 12, 0, 0, 1);
    tgt->addNode(v2); // array index for src2
    TR_PCISCNode *v3
        = new (PERSISTENT_NEW) TR_PCISCNode(c->trMemory(), TR_arraybase, TR::NoType, tgt->incNumNodes(), 11, 0, 0, 0);
    tgt->addNode(v3); // src1 array base
    TR_PCISCNode *v4
        = new (PERSISTENT_NEW) TR_PCISCNode(c->trMemory(), TR_arraybase, TR::NoType, tgt->incNumNodes(), 10, 0, 0, 1);
    tgt->addNode(v4); // src2 array base
    TR_PCISCNode *vorc1
        = new (PERSISTENT_NEW) TR_PCISCNode(c->trMemory(), TR_quasiConst2, TR::NoType, tgt->incNumNodes(), 9, 0, 0);
    tgt->addNode(vorc1); // length
    TR_PCISCNode *idx0
        = new (PERSISTENT_NEW) TR_PCISCNode(c->trMemory(), TR_arrayindex, TR::NoType, tgt->incNumNodes(), 8, 0, 0, 0);
    tgt->addNode(idx0);
    TR_PCISCNode *idx1
    ...
    TR_PCISCNode *n1 = new (PERSISTENT_NEW)
        TR_PCISCNode(c->trMemory(), TR_conversion, TR::NoType, tgt->incNumNodes(), 1, 1, 1, n0, n0);
    tgt->addNode(n1);
    TR_PCISCNode *n2 = createIdiomArrayLoadInLoop(tgt, ctrl, 1, n1, TR_indload, TR::NoType, v4, idx1, cmah1, iall);
    TR_PCISCNode *n3 = new (PERSISTENT_NEW)
        TR_PCISCNode(c->trMemory(), TR_conversion, TR::NoType, tgt->incNumNodes(), 1, 1, 1, n2, n2);
    tgt->addNode(n3);
    TR_PCISCNode *ncmp = new (PERSISTENT_NEW)
        TR_PCISCNode(c->trMemory(), TR_ifcmpall, TR::NoType, tgt->incNumNodes(), 1, 2, 2, n3, n1, n3);
    tgt->addNode(ncmp);
    ...
    TR_PCISCNode *ncmpge = new (PERSISTENT_NEW)
        TR_PCISCNode(c->trMemory(), TR_ifcmpall, TR::NoType, tgt->incNumNodes(), 1, 2, 2, n7, v1, vorc1);
    tgt->addNode(ncmpge);
    TR_PCISCNode *n9
        = new (PERSISTENT_NEW) TR_PCISCNode(c->trMemory(), TR_exitnode, TR::NoType, tgt->incNumNodes(), 0, 0, 0);
    tgt->addNode(n9);
...
TR_PCISCGraph *makeMemCmpIndexOfGraph(TR::Compilation *c, int32_t ctrl)
{
    ...
    tgt->addNode(v1); // array index for src1
    TR_PCISCNode *v2
        = new (PERSISTENT_NEW) TR_PCISCNode(c->trMemory(), TR_variable, TR::NoType, tgt->incNumNodes(), 11, 0, 0, 1);
    tgt->addNode(v2); // array index for src2
    TR_PCISCNode *v3
        = new (PERSISTENT_NEW) TR_PCISCNode(c->trMemory(), TR_arraybase, TR::NoType, tgt->incNumNodes(), 10, 0, 0, 0);
    tgt->addNode(v3); // src1 array base
    TR_PCISCNode *v4
        = new (PERSISTENT_NEW) TR_PCISCNode(c->trMemory(), TR_arraybase, TR::NoType, tgt->incNumNodes(), 9, 0, 0, 1);
    tgt->addNode(v4); // src2 array base
    TR_PCISCNode *vorc1
        = new (PERSISTENT_NEW) TR_PCISCNode(c->trMemory(), TR_quasiConst2, TR::NoType, tgt->incNumNodes(), 8, 0, 0);
    tgt->addNode(vorc1); // length
    TR_PCISCNode *idx0
        = new (PERSISTENT_NEW) TR_PCISCNode(c->trMemory(), TR_arrayindex, TR::NoType, tgt->incNumNodes(), 7, 0, 0, 0);
    tgt->addNode(idx0);
    TR_PCISCNode *idx1
        = new (PERSISTENT_NEW) TR_PCISCNode(c->trMemory(), TR_arrayindex, TR::NoType, tgt->incNumNodes(), 6, 0, 0, 1);
    tgt->addNode(idx1);
    TR_PCISCNode *cmah
        = new (PERSISTENT_NEW) TR_PCISCNode(c->trMemory(), TR_ahconst, TR::NoType, tgt->incNumNodes(), 5, 0, 0, 0);
    tgt->addNode(cmah); // array header
    ...
    TR_PCISCNode *ent
        = new (PERSISTENT_NEW) TR_PCISCNode(c->trMemory(), TR_entrynode, TR::NoType, tgt->incNumNodes(), 2, 1, 0);
    tgt->addNode(ent);
    TR_PCISCNode *a1 = new (PERSISTENT_NEW)
        TR_PCISCNode(c->trMemory(), TR::isub, TR::Int32, tgt->incNumNodes(), 1, 1, 2, ent, idx0, cm1);
    tgt->addNode(a1);
...
TR_PCISCGraph *makeMemCmpSpecialGraph(TR::Compilation *c, int32_t ctrl)
{
    ...
    tgt->addNode(vorc1); // length
    TR_PCISCNode *v5
        = new (PERSISTENT_NEW) TR_PCISCNode(c->trMemory(), TR_variable, TR::NoType, tgt->incNumNodes(), 8, 0, 0, 2);
    tgt->addNode(v5); // result
    TR_PCISCNode *idx0
        = new (PERSISTENT_NEW) TR_PCISCNode(c->trMemory(), TR_arrayindex, TR::NoType, tgt->incNumNodes(), 7, 0, 0, 0);
    tgt->addNode(idx0);
    TR_PCISCNode *cmah
    ...
    tgt->addNode(cm1);
    TR_PCISCNode *c0
        = new (PERSISTENT_NEW) TR_PCISCNode(c->trMemory(), TR::iconst, TR::Int32, tgt->incNumNodes(), 3, 0, 0, 0);
    tgt->addNode(c0);
    TR_PCISCNode *ent
        = new (PERSISTENT_NEW) TR_PCISCNode(c->trMemory(), TR_entrynode, TR::NoType, tgt->incNumNodes(), 2, 1, 0);
    tgt->addNode(ent);
    ...
    tgt->addNode(n3);
    TR_PCISCNode *n4 = new (PERSISTENT_NEW)
        TR_PCISCNode(c->trMemory(), TR::isub, TR::Int32, tgt->incNumNodes(), 1, 1, 2, n3, n1, n3);
    tgt->addNode(n4);
    TR_PCISCNode *n5 = new (PERSISTENT_NEW)
        TR_PCISCNode(c->trMemory(), TR::istore, TR::Int32, tgt->incNumNodes(), 1, 1, 2, n4, n4, v5);
    tgt->addNode(n5);
    TR_PCISCNode *ncmp = new (PERSISTENT_NEW)
        TR_PCISCNode(c->trMemory(), TR::ificmpne, TR::NoType, tgt->incNumNodes(), 1, 2, 2, n5, v5, c0);
    tgt->addNode(ncmp);
    TR_PCISCNode *n6 = createIdiomDecVarInLoop(tgt, ctrl, 1, ncmp, v1, cm1);
    TR_PCISCNode *n7 = createIdiomDecVarInLoop(tgt, ctrl, 1, n6, v2, cm1);
    TR_PCISCNode *ncmpge = new (PERSISTENT_NEW)
        TR_PCISCNode(c->trMemory(), TR::ificmpge, TR::NoType, tgt->incNumNodes(), 1, 2, 2, n7, v1, vorc1);
    tgt->addNode(ncmpge);
    TR_PCISCNode *n9
        = new (PERSISTENT_NEW) TR_PCISCNode(c->trMemory(), TR_exitnode, TR::NoType, tgt->incNumNodes(), 0, 0, 0);
    tgt->addNode(n9);
...
static void setSubopBitOpMem(TR::Compilation *comp, TR::Node *bitOpMem, TR_CISCNode *opCISCNode)
{
...
static TR::AutomaticSymbol *setPinningArray(TR::Compilation *comp, TR::Node *internalPtrStore, TR::Node *base,
    TR::Block *appendBlock)
{
    TR::AutomaticSymbol *pinningArray = NULL;
    if (base->getOpCode().isLoadVarDirect() && base->getSymbolReference()->getSymbol()->isAuto()) {
    ...
    internalPtrStore->getSymbolReference()->getSymbol()->castToInternalPointerAutoSymbol()->setPinningArrayPointer(
        pinningArray);
    if (internalPtrStore->isInternalPointer())
        internalPtrStore->setPinningArrayPointer(pinningArray);
    return pinningArray;
}
...
//    }
bool CISCTransform2BitOpMem(TR_CISCTransformer *trans)
{
    TR_ASSERT(trans->getOffsetOperand1() == 0 && trans->getOffsetOperand2() == 0, "Not implemented yet");
    const bool disptrace = DISPTRACE(trans);
    TR::Node *trNode;
    ...
    if (isLoopPreheaderLastBlockInMethod(comp, block, disptrace)) {
        logprints(disptrace, log,
            "Bailing CISCTransform2BitOpMem due to null TT - might be a preheader in last block of method\n");
        return false;
    }
    ...
    // Get the size of elements
    if (!getMultiplier(trans, P->getImportantNode(7), &mulFactorNode, &elementSize, inSrc1Node->getType()))
        return false;
    if (elementSize != inSrc1Node->getSize() || elementSize != inSrc2Node->getSize()) {
        logprintf(disptrace, log,
            "CISCTransform2BitOpMem failed - Size Mismatch.  Element Size: %d InSrc1Size: %d inSrc2Size: %d\n",
            elementSize, inSrc1Node->getSize(), inSrc2Node->getSize());
        return false; // Size is mismatch!
    }
    ...
    if (storeSrc2 && storeSrc2 != storeSrc1)
        storeList.add(storeSrc2->getHeadOfTrNode());
    if (storeDest && storeDest != storeSrc1)
        storeList.add(storeDest->getHeadOfTrNode());
    if (!isIndexVariableInList(inSrc1Node, &storeList) || !isIndexVariableInList(inSrc2Node, &storeList)
        || !isIndexVariableInList(inDestNode, &storeList)) {
        dumpOptDetails(comp,
            "indices used in array loads %p, %p, and %p are not consistent with the induction varaible updates\n",
            inSrc1Node, inSrc2Node, inDestNode);
        return false;
    }

    TR::Node *src1IdxRepNode, *src2IdxRepNode, *destIdxRepNode, *src1BaseRepNode, *src2BaseRepNode, *destBaseRepNode,
        *variableORconstRepNode;
    getP2TTrRepNodes(trans, &src1IdxRepNode, &src2IdxRepNode, &destIdxRepNode, &src1BaseRepNode, &src2BaseRepNode,
        &destBaseRepNode, &variableORconstRepNode);
    ...
    if (src1IdxSymRef == destIdxSymRef)
        destIdxSymRef = 0;
    if (src1IdxSymRef == src2IdxSymRef)
        src2IdxSymRef = 0;
    if (trans->countGoodArrayIndex(src1IdxSymRef) == 0)
        return false;
    if (src2IdxSymRef && (trans->countGoodArrayIndex(src2IdxSymRef) == 0))
        return false;
    if (destIdxSymRef && (trans->countGoodArrayIndex(destIdxSymRef) == 0))
        return false;
    ...
    if (elementSize > 1) {
        ...
        lengthNode = TR::Node::create(mulOp, 2, lengthNode, elementSizeNode);
    }
    ...
    if (destIdxSymRef != NULL && src1IdxSymRef != destIdxSymRef) {
        // If there are two induction variables, we need to maintain the other one.
        TR::Node *result
            = createOP2(comp, TR::iadd, TR::Node::createLoad(trNode, destIdxSymRef), diff2->duplicateTree());
    ...
    if (src2IdxSymRef != NULL && src2IdxSymRef != destIdxSymRef && src2IdxSymRef != src1IdxSymRef) {
        ...
        TR::Node *result
            = createOP2(comp, TR::iadd, TR::Node::createLoad(trNode, src2IdxSymRef), diff2->duplicateTree());
        src2Update = TR::Node::createStore(src2IdxSymRef, result);
    }

    TR::Node *bitOpMem = NULL;
    if (outputNode == inputNode1 || outputNode == inputNode2) {
        bitOpMem = TR::Node::create(TR::bitOpMem, 3, outputNode->duplicateTree(),
            (outputNode == inputNode1 ? inputNode2 : inputNode1)->duplicateTree(), lengthNode);
    ...
    // Modify actual code
    //********************
    if (bitOpMem) { // src1 or src2 is equal to dest
        block = trans->modifyBlockByVersioningCheck(block, trTreeTop, lengthNode->duplicateTree());
        block = trans->insertBeforeNodes(block);
        ...
        TR::SymbolReference *destAddrSymRef
            = comp->getSymRefTab()->createTemporary(comp->getMethodSymbol(), TR::Address, true);
        TR::SymbolReference *src1AddrSymRef
            = comp->getSymRefTab()->createTemporary(comp->getMethodSymbol(), TR::Address, true);
        TR::SymbolReference *src2AddrSymRef
            = comp->getSymRefTab()->createTemporary(comp->getMethodSymbol(), TR::Address, true);
        ...
        checkSrc1->append(TR::TreeTop::create(comp,
            TR::Node::createif(TR::ifacmpne, TR::Node::createLoad(trNode, destAddrSymRef),
                TR::Node::createLoad(trNode, src1AddrSymRef), checkSrc2->getEntry())));

        // fastpath1: bitOpMem(dest, src2, length); goto lastpath;
        TR::Node *bitOpMem1 = TR::Node::create(TR::bitOpMem, 3, TR::Node::createLoad(trNode, destAddrSymRef),
            TR::Node::createLoad(trNode, src2AddrSymRef), lengthNode->duplicateTree());
        ...
        checkSrc2->append(TR::TreeTop::create(comp,
            TR::Node::createif(TR::ifacmpne, TR::Node::createLoad(trNode, destAddrSymRef),
                TR::Node::createLoad(trNode, src2AddrSymRef), checkSrc3->getEntry())));

        // fastpath2: bitOpMem(dest, src1, length); goto lastpath;
        TR::Node *bitOpMem2 = TR::Node::create(TR::bitOpMem, 3, TR::Node::createLoad(trNode, destAddrSymRef),
            TR::Node::createLoad(trNode, src1AddrSymRef), lengthNode->duplicateTree());
        ...
        checkSrc3->append(TR::TreeTop::create(comp,
            TR::Node::createif(TR::ifacmpeq, createLoad(destBaseRepNode), createLoad(src1BaseRepNode),
                slowpad->getEntry())));

        // checkSrc4: if (dest.obj == src2.obj) goto slowpad
        checkSrc4->append(TR::TreeTop::create(comp,
            TR::Node::createif(TR::ifacmpeq, createLoad(destBaseRepNode), createLoad(src2BaseRepNode),
                slowpad->getEntry())));

        // fastpath3: bitOpMem(dest, src1, src2, length);
        // We can guarantee the destination NEVER overlaps to src1 or src2.
        bitOpMem = TR::Node::create(TR::bitOpMem, 4, TR::Node::createLoad(trNode, destAddrSymRef),
            TR::Node::createLoad(trNode, src1AddrSymRef), TR::Node::createLoad(trNode, src2AddrSymRef),
        ...
        logprintf(disptrace, log,
            "CISCTransform2BitOpMem: orgPrevBlock=%d checkSrc1=%d lastpath=%d slowpad=%d orgNextTreeTop=%x\n",
            orgPrevBlock->getNumber(), checkSrc1->getNumber(), lastpath->getNumber(), slowpad->getNumber(),
            orgNextTreeTop);
    ...
    if (src2Update)
        block->append(TR::TreeTop::create(comp, src2Update));
    if (destUpdate)
        block->append(TR::TreeTop::create(comp, destUpdate));
...
TR_PCISCGraph *makeBitOpMemGraph(TR::Compilation *c, int32_t ctrl)
{
    ...
    tgt->addNode(v1); // array index for src1
    TR_PCISCNode *v2
        = new (PERSISTENT_NEW) TR_PCISCNode(c->trMemory(), TR_variable, TR::NoType, tgt->incNumNodes(), 15, 0, 0, 1);
    tgt->addNode(v2); // array index for src2
    TR_PCISCNode *v3
        = new (PERSISTENT_NEW) TR_PCISCNode(c->trMemory(), TR_variable, TR::NoType, tgt->incNumNodes(), 14, 0, 0, 2);
    tgt->addNode(v3); // array index for dest
    TR_PCISCNode *v4
        = new (PERSISTENT_NEW) TR_PCISCNode(c->trMemory(), TR_arraybase, TR::NoType, tgt->incNumNodes(), 13, 0, 0, 0);
    tgt->addNode(v4); // src1 array base
    TR_PCISCNode *v5
        = new (PERSISTENT_NEW) TR_PCISCNode(c->trMemory(), TR_arraybase, TR::NoType, tgt->incNumNodes(), 12, 0, 0, 1);
    tgt->addNode(v5); // src2 array base
    TR_PCISCNode *v6
        = new (PERSISTENT_NEW) TR_PCISCNode(c->trMemory(), TR_arraybase, TR::NoType, tgt->incNumNodes(), 11, 0, 0, 2);
    tgt->addNode(v6); // dest array base
    TR_PCISCNode *vorc1
        = new (PERSISTENT_NEW) TR_PCISCNode(c->trMemory(), TR_quasiConst2, TR::NoType, tgt->incNumNodes(), 10, 0, 0);
    tgt->addNode(vorc1); // length
    TR_PCISCNode *iall
        = new (PERSISTENT_NEW) TR_PCISCNode(c->trMemory(), TR_allconst, TR::NoType, tgt->incNumNodes(), 9, 0, 0);
    tgt->addNode(iall); // Multiply Factor
    TR_PCISCNode *idx0
        = new (PERSISTENT_NEW) TR_PCISCNode(c->trMemory(), TR_arrayindex, TR::NoType, tgt->incNumNodes(), 8, 0, 0, 0);
    tgt->addNode(idx0);
    ...
    tgt->addNode(idx1);
    TR_PCISCNode *idx2
        = new (PERSISTENT_NEW) TR_PCISCNode(c->trMemory(), TR_arrayindex, TR::NoType, tgt->incNumNodes(), 6, 0, 0, 2);
    tgt->addNode(idx2);
    TR_PCISCNode *cmah0
        = new (PERSISTENT_NEW) TR_PCISCNode(c->trMemory(), TR_ahconst, TR::NoType, tgt->incNumNodes(), 5, 0, 0, 0);
    tgt->addNode(cmah0); // array header
    ...
    TR_PCISCNode *cv0 = new (PERSISTENT_NEW)
        TR_PCISCNode(c->trMemory(), TR_conversion, TR::NoType, tgt->incNumNodes(), 1, 1, 1, n0, n0);
    tgt->addNode(cv0); // optional
    TR_PCISCNode *n2 = createIdiomArrayLoadInLoop(tgt, ctrl, 1, cv0, TR_indload, TR::NoType, v5, idx2, cmah0, iall);
    TR_PCISCNode *cv1 = new (PERSISTENT_NEW)
        TR_PCISCNode(c->trMemory(), TR_conversion, TR::NoType, tgt->incNumNodes(), 1, 1, 1, n2, n2);
    tgt->addNode(cv1); // optional
    TR_PCISCNode *n4 = new (PERSISTENT_NEW)
        TR_PCISCNode(c->trMemory(), TR_bitop1, TR::NoType, tgt->incNumNodes(), 1, 1, 2, cv1, cv0, cv1);
    tgt->addNode(n4);
    TR_PCISCNode *cv2 = new (PERSISTENT_NEW)
        TR_PCISCNode(c->trMemory(), TR_conversion, TR::NoType, tgt->incNumNodes(), 1, 1, 1, n4, n4);
    tgt->addNode(cv2); // optional
    TR_PCISCNode *sn1 = new (PERSISTENT_NEW)
        TR_PCISCNode(c->trMemory(), TR_indstore, TR::NoType, tgt->incNumNodes(), 1, 1, 2, cv2, sn0, cv2);
    tgt->addNode(sn1);
    ...
    TR_PCISCNode *ncmpge = new (PERSISTENT_NEW)
        TR_PCISCNode(c->trMemory(), TR::ificmpge, TR::NoType, tgt->incNumNodes(), 1, 2, 2, n8, v1, vorc1);
    tgt->addNode(ncmpge);
    TR_PCISCNode *n9
        = new (PERSISTENT_NEW) TR_PCISCNode(c->trMemory(), TR_exitnode, TR::NoType, tgt->incNumNodes(), 0, 0, 0);
    tgt->addNode(n9);
...
static TR::SymbolReference *getSymrefDigit10(TR::Compilation *comp, TR::Node *trNode)
{
...
static TR::Node *createNodeLoadDigit10Table(TR::Compilation *comp, TR::Node *trNode)
{
    TR_ASSERT(trNode->getDataType() == TR::Int32 || trNode->getDataType() == TR::Int64,
        "Unexpected datatype for trNode for CountDigits10.");
...
//*****************************************************************************************
bool CISCTransform2CountDecimalDigit(TR_CISCTransformer *trans)
{
    TR_ASSERT(trans->getOffsetOperand1() == 0 && trans->getOffsetOperand2() == 0, "Not implemented yet");
    const bool disptrace = DISPTRACE(trans);
    TR::Node *trNode;
    ...
    if (comp->compileRelocatableCode()) {
        logprints(disptrace, log, "Bailing CISCTransform2CountDecimalDigit - not supported for AOT compilations.");
        return false;
    }

    if (isLoopPreheaderLastBlockInMethod(comp, block, disptrace)) {
        logprints(disptrace, log,
            "Bailing CISCTransform2CountDecimalDigit due to null TT - might be a preheader in last block of method\n");
        return false;
    }
    ...
    if (!constNode->getIlOpCode().isLoadConst()) {
        logprintf(disptrace, log, "%p is not isLoadConst().\n", constNode);
        return false;
    }
    ...
    switch (ifcmp->getOpcode()) {
        ...
        case TR::iflcmpeq:
            if (constNode->getOtherInfo() != 0) {
                logprintf(disptrace, log, "The exit-if is TR::if*cmpeq but the constant value is %d.\n",
        ...
        case TR::iflcmplt:
            if (constNode->getOtherInfo() != 10) {
                logprintf(disptrace, log, "The exit-if is TR::if*cmplt but the constant value is %d.\n",
                    constNode->getOtherInfo());
                return false;
            }
            ...
            break;
        default:
            logprintf(disptrace, log, "The exit-if %p is not as expected. We may be able to implement this case.\n",
                ifcmp);
            return false;
    }
    ...
    if (modificationResult != 0) {
        digitNode
            = createOP2(comp, TR::isub, digitNode, TR::Node::create(digitNode, TR::iconst, 0, -modificationResult));
    }

    TR::Node *top = TR::Node::createStore(countVarSymRef, createOP2(comp, TR::iadd, countVar, digitNode));
...
TR_PCISCGraph *makeCountDecimalDigitLongGraph(TR::Compilation *c, int32_t ctrl, bool isDiv2Mul)
{
    ...
    if (isDiv2Mul) {
        ...
        TR_PCISCNode *v1
            = new (PERSISTENT_NEW) TR_PCISCNode(c->trMemory(), TR_variable, TR::NoType, tgt->incNumNodes(), 8, 0, 0, 0);
        tgt->addNode(v1); // count
        v2 = new (PERSISTENT_NEW) TR_PCISCNode(c->trMemory(), TR_variable, TR::NoType, tgt->incNumNodes(), 7, 0, 0, 1);
        tgt->addNode(v2); // long var
        cexit = new (PERSISTENT_NEW) TR_PCISCNode(c->trMemory(), TR_allconst, TR::NoType, tgt->incNumNodes(), 6, 0, 0);
        tgt->addNode(cexit); // all constant
        TR_PCISCNode *c2
            = new (PERSISTENT_NEW) TR_PCISCNode(c->trMemory(), TR::iconst, TR::Int32, tgt->incNumNodes(), 5, 0, 0, 2);
        tgt->addNode(c2); // iconst 2
        TR_PCISCNode *c63
            = new (PERSISTENT_NEW) TR_PCISCNode(c->trMemory(), TR::iconst, TR::Int32, tgt->incNumNodes(), 4, 0, 0, 63);
        tgt->addNode(c63); // iconst 63 (optional)
        TR_PCISCNode *cm1
            = new (PERSISTENT_NEW) TR_PCISCNode(c->trMemory(), TR::iconst, TR::Int32, tgt->incNumNodes(), 3, 0, 0, -1);
        tgt->addNode(cm1);
        ent = new (PERSISTENT_NEW) TR_PCISCNode(c->trMemory(), TR_entrynode, TR::NoType, tgt->incNumNodes(), 2, 1, 0);
        tgt->addNode(ent);
        TR_PCISCNode *n1 = createIdiomDecVarInLoop(tgt, ctrl, 1, ent, v1, cm1);
        TR_PCISCNode *mag
            = new (PERSISTENT_NEW) TR_PCISCNode(c->trMemory(), TR::lconst, TR::Int64, tgt->incNumNodes(), 1, 1, 0, n1);
        tgt->addNode(mag); // lconst 7378697629483820647
        TR_PCISCNode *nmul = new (PERSISTENT_NEW)
            TR_PCISCNode(c->trMemory(), TR::lmulh, TR::Int64, tgt->incNumNodes(), 1, 1, 2, mag, v2, mag);
        tgt->addNode(nmul);
        TR_PCISCNode *nshr = new (PERSISTENT_NEW)
            TR_PCISCNode(c->trMemory(), TR::lshr, TR::Int64, tgt->incNumNodes(), 1, 1, 2, nmul, nmul, c2);
        tgt->addNode(nshr);
        TR_PCISCNode *ushr = new (PERSISTENT_NEW)
            TR_PCISCNode(c->trMemory(), TR::lushr, TR::Int64, tgt->incNumNodes(), 1, 1, 2, nshr, v2, c63);
        tgt->addNode(ushr); // optional
        ndiv = new (PERSISTENT_NEW)
            TR_PCISCNode(c->trMemory(), TR::ladd, TR::Int64, tgt->incNumNodes(), 1, 1, 2, ushr, nshr, ushr);
        tgt->addNode(ndiv); // optional
        ...
        TR_PCISCNode *v1
            = new (PERSISTENT_NEW) TR_PCISCNode(c->trMemory(), TR_variable, TR::NoType, tgt->incNumNodes(), 7, 0, 0, 0);
        tgt->addNode(v1); // count
        v2 = new (PERSISTENT_NEW) TR_PCISCNode(c->trMemory(), TR_variable, TR::NoType, tgt->incNumNodes(), 6, 0, 0, 1);
        tgt->addNode(v2); // long var
        cexit = new (PERSISTENT_NEW) TR_PCISCNode(c->trMemory(), TR_allconst, TR::NoType, tgt->incNumNodes(), 5, 0, 0);
        tgt->addNode(cexit); // all constant
        TR_PCISCNode *c10
            = new (PERSISTENT_NEW) TR_PCISCNode(c->trMemory(), TR::lconst, TR::Int64, tgt->incNumNodes(), 4, 0, 0, 10);
        tgt->addNode(c10); // lconst 10
        TR_PCISCNode *cm1
            = new (PERSISTENT_NEW) TR_PCISCNode(c->trMemory(), TR::iconst, TR::Int32, tgt->incNumNodes(), 3, 0, 0, -1);
        tgt->addNode(cm1);
        ent = new (PERSISTENT_NEW) TR_PCISCNode(c->trMemory(), TR_entrynode, TR::NoType, tgt->incNumNodes(), 2, 1, 0);
        ...
        TR_PCISCNode *n1 = createIdiomDecVarInLoop(tgt, ctrl, 1, ent, v1, cm1);
        ndiv = new (PERSISTENT_NEW)
            TR_PCISCNode(c->trMemory(), TR::ldiv, TR::Int64, tgt->incNumNodes(), 1, 1, 2, n1, v2, c10);
        tgt->addNode(ndiv);
        tgt->setNumDagIds(8);
        tgt->setAspects(isub | division);
    }
    TR_PCISCNode *nst = new (PERSISTENT_NEW)
        TR_PCISCNode(c->trMemory(), TR::lstore, TR::Int64, tgt->incNumNodes(), 1, 1, 2, ndiv, ndiv, v2);
    tgt->addNode(nst);
    ncmp = new (PERSISTENT_NEW)
        TR_PCISCNode(c->trMemory(), TR_ifcmpall, TR::NoType, tgt->incNumNodes(), 1, 2, 2, nst, v2, cexit);
    tgt->addNode(ncmp);
    n9 = new (PERSISTENT_NEW) TR_PCISCNode(c->trMemory(), TR_exitnode, TR::NoType, tgt->incNumNodes(), 0, 0, 0);
    tgt->addNode(n9);
...
TR_PCISCGraph *makeCountDecimalDigitIntGraph(TR::Compilation *c, int32_t ctrl, bool isDiv2Mul)
{
    ...
    if (isDiv2Mul) {
        ...
        TR_PCISCNode *v1
            = new (PERSISTENT_NEW) TR_PCISCNode(c->trMemory(), TR_variable, TR::NoType, tgt->incNumNodes(), 9, 0, 0, 0);
        tgt->addNode(v1); // count
        v2 = new (PERSISTENT_NEW) TR_PCISCNode(c->trMemory(), TR_variable, TR::NoType, tgt->incNumNodes(), 8, 0, 0, 1);
        tgt->addNode(v2); // int var
        cexit = new (PERSISTENT_NEW) TR_PCISCNode(c->trMemory(), TR_allconst, TR::NoType, tgt->incNumNodes(), 7, 0, 0);
        tgt->addNode(cexit); // all constant
        TR_PCISCNode *c2
            = new (PERSISTENT_NEW) TR_PCISCNode(c->trMemory(), TR::iconst, TR::Int32, tgt->incNumNodes(), 6, 0, 0, 2);
        tgt->addNode(c2); // iconst 2
        TR_PCISCNode *c31
            = new (PERSISTENT_NEW) TR_PCISCNode(c->trMemory(), TR::iconst, TR::Int32, tgt->incNumNodes(), 5, 0, 0, 31);
        tgt->addNode(c31); // iconst 31 (optional)
        TR_PCISCNode *mag = new (PERSISTENT_NEW)
            TR_PCISCNode(c->trMemory(), TR::iconst, TR::Int32, tgt->incNumNodes(), 4, 0, 0, 1717986919);
        tgt->addNode(mag); // iconst 1717986919
        TR_PCISCNode *cm1
            = new (PERSISTENT_NEW) TR_PCISCNode(c->trMemory(), TR::iconst, TR::Int32, tgt->incNumNodes(), 3, 0, 0, -1);
        tgt->addNode(cm1);
        ent = new (PERSISTENT_NEW) TR_PCISCNode(c->trMemory(), TR_entrynode, TR::NoType, tgt->incNumNodes(), 2, 1, 0);
        ...
        tgt->addNode(v2); // int var
        cexit = new (PERSISTENT_NEW) TR_PCISCNode(c->trMemory(), TR_allconst, TR::NoType, tgt->incNumNodes(), 5, 0, 0);
        tgt->addNode(cexit); // iconst 0
        TR_PCISCNode *mag
            = new (PERSISTENT_NEW) TR_PCISCNode(c->trMemory(), TR::iconst, TR::Int32, tgt->incNumNodes(), 4, 0, 0, 10);
        tgt->addNode(mag); // iconst 10
        TR_PCISCNode *cm1
            = new (PERSISTENT_NEW) TR_PCISCNode(c->trMemory(), TR::iconst, TR::Int32, tgt->incNumNodes(), 3, 0, 0, -1);
        tgt->addNode(cm1);
        ent = new (PERSISTENT_NEW) TR_PCISCNode(c->trMemory(), TR_entrynode, TR::NoType, tgt->incNumNodes(), 2, 1, 0);
    ...
    TR_PCISCNode *nst = new (PERSISTENT_NEW)
        TR_PCISCNode(c->trMemory(), TR::istore, TR::Int32, tgt->incNumNodes(), 1, 1, 2, ndiv, ndiv, v2);
    tgt->addNode(nst);
    ncmp = new (PERSISTENT_NEW)
        TR_PCISCNode(c->trMemory(), TR_ifcmpall, TR::NoType, tgt->incNumNodes(), 1, 2, 2, nst, v2, cexit);
    tgt->addNode(ncmp);
...
static TR::SymbolReference *getSymrefLocalArray(TR::Compilation *comp, int size)
{
    if (comp->target().cpu.isZ()) {
        TR::SymbolReference *workSymRef
            = comp->getSymRefTab()->createLocalPrimArray(size, comp->getMethodSymbol(), 8); // work area for CVD(G)
...
static TR::Node *createNodeLoadLocalArray(TR::Compilation *comp, TR::Node *trNode, int size)
{
...
//*****************************************************************************************
bool CISCTransform2LongToStringDigit(TR_CISCTransformer *trans)
{
    TR_ASSERT(trans->getOffsetOperand1() == 0 && trans->getOffsetOperand2() == 0, "Not implemented yet");
    TR::Node *trNode;
    TR::TreeTop *trTreeTop;
    ...
    if (comp->compileRelocatableCode()) {
        logprints(disptrace, log, "Bailing CISCTransform2LongToStringDigit - not supported for AOT compilations.");
        return false;
    }

    if (isLoopPreheaderLastBlockInMethod(comp, block, disptrace)) {
        logprints(disptrace, log,
            "Bailing CISCTransform2LongToStringDigit due to null TT - might be a preheader in last block of method\n");
        return false;
    }
    ...
    if (P->getImportantNode(3)) {
        ...
        if (nullchkCISC)
            nullchk = nullchkCISC->getHeadOfTrNode()->duplicateTree();
    }
...
#if 1
    TR::Node *digit = TR::Node::create(TR::countDigits, 2, inputVar, createNodeLoadDigit10Table(comp, inputVarRepNode));
#else
    TR::Node *digit = TR::Node::create(TR::countDigits, 2, inputVar, createNodeLoadDigit10Table(comp, trNode));
#endif
    TR::Node *resultV2 = createOP2(comp, TR::isub, countVar, digit);
    replaceParent->setAndIncChild(childNum,
        createOP2(comp, TR::isub, resultV2, TR::Node::create(trNode, TR::iconst, 0, -1)));
...
TR_PCISCGraph *makeLongToStringGraph(TR::Compilation *c, int32_t ctrl)
{
    ...
    TR_PCISCNode *v1
        = new (PERSISTENT_NEW) TR_PCISCNode(c->trMemory(), TR_arraybase, TR::NoType, tgt->incNumNodes(), 16, 0, 0, 0);
    tgt->addNode(v1); // array base
    TR_PCISCNode *v2
        = new (PERSISTENT_NEW) TR_PCISCNode(c->trMemory(), TR_variable, TR::NoType, tgt->incNumNodes(), 15, 0, 0, 0);
    tgt->addNode(v2); // count
    TR_PCISCNode *v3
        = new (PERSISTENT_NEW) TR_PCISCNode(c->trMemory(), TR_variable, TR::NoType, tgt->incNumNodes(), 14, 0, 0, 1);
    tgt->addNode(v3); // long var
    TR_PCISCNode *v4
        = new (PERSISTENT_NEW) TR_PCISCNode(c->trMemory(), TR_variable, TR::NoType, tgt->incNumNodes(), 13, 0, 0, 2);
    tgt->addNode(v4); // stored value
    TR_PCISCNode *vorc
        = new (PERSISTENT_NEW) TR_PCISCNode(c->trMemory(), TR_quasiConst2, TR::NoType, tgt->incNumNodes(), 12, 0, 0);
    tgt->addNode(vorc); // length
    TR_PCISCNode *cmah
        = new (PERSISTENT_NEW) TR_PCISCNode(c->trMemory(), TR_ahconst, TR::NoType, tgt->incNumNodes(), 11, 0, 0, 0);
    tgt->addNode(cmah); // array header
    TR_PCISCNode *cl0
        = new (PERSISTENT_NEW) TR_PCISCNode(c->trMemory(), TR::lconst, TR::Int64, tgt->incNumNodes(), 10, 0, 0, 0);
    tgt->addNode(cl0); // lconst 0
    TR_PCISCNode *cl10
        = new (PERSISTENT_NEW) TR_PCISCNode(c->trMemory(), TR::lconst, TR::Int64, tgt->incNumNodes(), 9, 0, 0, 10);
    tgt->addNode(cl10); // lconst 10
    TR_PCISCNode *c0
        = new (PERSISTENT_NEW) TR_PCISCNode(c->trMemory(), TR::iconst, TR::Int32, tgt->incNumNodes(), 8, 0, 0, 0);
    tgt->addNode(c0); // iconst 0
    TR_PCISCNode *c2 = createIdiomArrayRelatedConst(tgt, ctrl, tgt->incNumNodes(), 7, 2); // element size
    TR_PCISCNode *c9
        = new (PERSISTENT_NEW) TR_PCISCNode(c->trMemory(), TR::iconst, TR::Int32, tgt->incNumNodes(), 6, 0, 0, 9);
    tgt->addNode(c9); // iconst 9
    TR_PCISCNode *cm87
        = new (PERSISTENT_NEW) TR_PCISCNode(c->trMemory(), TR::iconst, TR::Int32, tgt->incNumNodes(), 5, 0, 0, -87);
    tgt->addNode(cm87); // iconst -87
    TR_PCISCNode *cm48
        = new (PERSISTENT_NEW) TR_PCISCNode(c->trMemory(), TR::iconst, TR::Int32, tgt->incNumNodes(), 4, 0, 0, -48);
    tgt->addNode(cm48); // iconst -48
    TR_PCISCNode *cm1
        = new (PERSISTENT_NEW) TR_PCISCNode(c->trMemory(), TR::iconst, TR::Int32, tgt->incNumNodes(), 3, 0, 0, -1);
    tgt->addNode(cm1);
    TR_PCISCNode *ent
        = new (PERSISTENT_NEW) TR_PCISCNode(c->trMemory(), TR_entrynode, TR::NoType, tgt->incNumNodes(), 2, 1, 0);
    tgt->addNode(ent);
    TR_PCISCNode *nrem = new (PERSISTENT_NEW)
        TR_PCISCNode(c->trMemory(), TR::lrem, TR::Int64, tgt->incNumNodes(), 1, 1, 2, ent, v3, cl10);
    tgt->addNode(nrem);
    TR_PCISCNode *nl2i
        = new (PERSISTENT_NEW) TR_PCISCNode(c->trMemory(), TR::l2i, TR::Int32, tgt->incNumNodes(), 1, 1, 1, nrem, nrem);
    tgt->addNode(nl2i);
    TR_PCISCNode *nneg = new (PERSISTENT_NEW)
        TR_PCISCNode(c->trMemory(), TR::isub, TR::Int32, tgt->incNumNodes(), 1, 1, 2, nl2i, c0, nl2i);
    tgt->addNode(nneg);
    TR_PCISCNode *nst4 = new (PERSISTENT_NEW)
        TR_PCISCNode(c->trMemory(), TR::istore, TR::Int32, tgt->incNumNodes(), 1, 1, 2, nneg, nneg, v4);
    tgt->addNode(nst4);
    TR_PCISCNode *ifge = new (PERSISTENT_NEW)
        TR_PCISCNode(c->trMemory(), TR::ificmpgt, TR::NoType, tgt->incNumNodes(), 1, 2, 2, nst4, v4, c9);
    tgt->addNode(ifge);
    ...
    TR_PCISCNode *nck = new (PERSISTENT_NEW)
        TR_PCISCNode(c->trMemory(), TR::NULLCHK, TR::NoType, tgt->incNumNodes(), 1, 1, 1, adm1, v1);
    tgt->addNode(nck); // optional
    TR_PCISCNode *bck = new (PERSISTENT_NEW)
        TR_PCISCNode(c->trMemory(), TR::BNDCHK, TR::NoType, tgt->incNumNodes(), 1, 1, 2, nck, vorc, v2);
    tgt->addNode(bck);
    TR_PCISCNode *ncst = createIdiomCharArrayStoreInLoop(tgt, ctrl, 1, bck, v1, v2, cmah, c2, v4);
    TR_PCISCNode *ndiv = new (PERSISTENT_NEW)
        TR_PCISCNode(c->trMemory(), TR::ldiv, TR::Int64, tgt->incNumNodes(), 1, 1, 2, ncst, v3, cl10);
    tgt->addNode(ndiv);
    TR_PCISCNode *nst3 = new (PERSISTENT_NEW)
        TR_PCISCNode(c->trMemory(), TR::lstore, TR::Int64, tgt->incNumNodes(), 1, 1, 2, ndiv, ndiv, v3);
    tgt->addNode(nst3);
    TR_PCISCNode *ifeq = new (PERSISTENT_NEW)
        TR_PCISCNode(c->trMemory(), TR::iflcmpeq, TR::NoType, tgt->incNumNodes(), 1, 2, 2, nst3, v3, cl0);
    tgt->addNode(ifeq);
    TR_PCISCNode *n9
        = new (PERSISTENT_NEW) TR_PCISCNode(c->trMemory(), TR_exitnode, TR::NoType, tgt->incNumNodes(), 0, 0, 0);
    tgt->addNode(n9);
...
TR_PCISCGraph *makeIntToStringGraph(TR::Compilation *c, int32_t ctrl, bool isDiv2Mul)
{
     ...
     * other/pred/children */
    v1 = new (PERSISTENT_NEW) TR_PCISCNode(c->trMemory(), TR_arraybase, TR::NoType, tgt->incNumNodes(), 15, 0, 0, 0);
    tgt->addNode(v1); // array base
    v2 = new (PERSISTENT_NEW) TR_PCISCNode(c->trMemory(), TR_variable, TR::NoType, tgt->incNumNodes(), 14, 0, 0, 0);
    tgt->addNode(v2); // count
    v3 = new (PERSISTENT_NEW) TR_PCISCNode(c->trMemory(), TR_variable, TR::NoType, tgt->incNumNodes(), 13, 0, 0, 1);
    tgt->addNode(v3); // long var
    c2 = createIdiomArrayRelatedConst(tgt, ctrl, tgt->incNumNodes(), 12, 2); // element size
    c10 = new (PERSISTENT_NEW) TR_PCISCNode(c->trMemory(), TR::iconst, TR::Int32, tgt->incNumNodes(), 11, 0, 0, 10);
    tgt->addNode(c10); // iconst 10
    if (isDiv2Mul) {
        c31 = new (PERSISTENT_NEW) TR_PCISCNode(c->trMemory(), TR::iconst, TR::Int32, tgt->incNumNodes(), 10, 0, 0, 31);
        tgt->addNode(c31); // iconst 31
        if (ctrl & CISCUtilCtl_64Bit) {
            ci2 = new (PERSISTENT_NEW)
                TR_PCISCNode(c->trMemory(), TR::iconst, TR::Int32, tgt->incNumNodes(), 9, 0, 0, 2);
            tgt->addNode(ci2); // iconst 2
        } else
            ci2 = c2;
        mag = new (PERSISTENT_NEW)
            TR_PCISCNode(c->trMemory(), TR::iconst, TR::Int32, tgt->incNumNodes(), 8, 0, 0, 1717986919);
        tgt->addNode(mag); // iconst 1717986919
        otherMask = shr;
    } else {
        ci2 = c31 = NULL;
    ...
    tgt->addNode(cmah); // array header
    TR_PCISCNode *c0
        = new (PERSISTENT_NEW) TR_PCISCNode(c->trMemory(), TR::iconst, TR::Int32, tgt->incNumNodes(), 5, 0, 0, 0);
    tgt->addNode(c0); // iconst 0
    TR_PCISCNode *c48
        = new (PERSISTENT_NEW) TR_PCISCNode(c->trMemory(), TR::iconst, TR::Int32, tgt->incNumNodes(), 4, 0, 0, 48);
    tgt->addNode(c48); // iconst 48
    TR_PCISCNode *cm1
        = new (PERSISTENT_NEW) TR_PCISCNode(c->trMemory(), TR::iconst, TR::Int32, tgt->incNumNodes(), 3, 0, 0, -1);
    tgt->addNode(cm1);
    TR_PCISCNode *ent
    ...
    tgt->addNode(ent);
    TR_PCISCNode *nck = new (PERSISTENT_NEW)
        TR_PCISCNode(c->trMemory(), TR::NULLCHK, TR::NoType, tgt->incNumNodes(), 1, 1, 1, ent, v1);
    tgt->addNode(nck); // optional
    TR_PCISCNode *bck = new (PERSISTENT_NEW)
        TR_PCISCNode(c->trMemory(), TR::BNDCHK, TR::NoType, tgt->incNumNodes(), 1, 1, 2, nck, vorc, v2);
    tgt->addNode(bck);
    ...
    TR_PCISCNode *nmul = new (PERSISTENT_NEW)
        TR_PCISCNode(c->trMemory(), TR::imul, TR::Int32, tgt->incNumNodes(), 1, 1, 2, ndiv, ndiv, c10);
    tgt->addNode(nmul);
    TR_PCISCNode *nrem = new (PERSISTENT_NEW)
        TR_PCISCNode(c->trMemory(), TR::isub, TR::Int32, tgt->incNumNodes(), 1, 1, 2, nmul, v3, nmul);
    tgt->addNode(nrem);
    TR_PCISCNode *nch = new (PERSISTENT_NEW)
        TR_PCISCNode(c->trMemory(), TR::isub, TR::Int32, tgt->incNumNodes(), 1, 1, 2, nrem, c48, nrem);
    tgt->addNode(nch);
    TR_PCISCNode *ncst = createIdiomCharArrayStoreBodyInLoop(tgt, ctrl, 1, nch, addr, nch);
    TR_PCISCNode *nst3 = new (PERSISTENT_NEW)
        TR_PCISCNode(c->trMemory(), TR::istore, TR::Int32, tgt->incNumNodes(), 1, 1, 2, ncst, ndiv, v3);
    tgt->addNode(nst3);
    TR_PCISCNode *adm1 = createIdiomIncVarInLoop(tgt, ctrl, 1, nst3, v2, cm1);
    TR_PCISCNode *ifeq = new (PERSISTENT_NEW)
        TR_PCISCNode(c->trMemory(), TR::ificmpeq, TR::NoType, tgt->incNumNodes(), 1, 2, 2, adm1, v3, c0);
    tgt->addNode(ifeq);
    TR_PCISCNode *n9
        = new (PERSISTENT_NEW) TR_PCISCNode(c->trMemory(), TR_exitnode, TR::NoType, tgt->incNumNodes(), 0, 0, 0);
    tgt->addNode(n9);
