...
class AdjacentBlockIterator {
    ...
    void advance()
    {
        ...
        if (cfg->getStart()->asBlock() == _block && !_visitedBlocks.contains(cfg->getEnd()->asBlock())) {
        ...
        // to make sure the edge is included in the computed MST
        if (cfg->getEnd()->asBlock() == _block && !_visitedBlocks.contains(cfg->getStart()->asBlock())) {
...
public:
    CFGNodeIterator(TR::CFG *cfg, TR::Optimization *opt)
        : _cfg(cfg)
        , _itr(cfg, opt->comp())
        , _start(false)
        , _end(false)
...
public:
    CFGSuccessorIterator(TR::CFG *cfg, TR::CFGEdge *loopBack, TR::Block *block)
        ...
        , _loopBack(loopBack)
        , _itr(block)
        , _done(block != cfg->getEnd()->asBlock())
        , _block(block)
    {}
...
public:
    CFGPredecessorIterator(TR::CFG *cfg, TR::CFGEdge *loopBack, TR::Block *block)
        ...
        , _loopBack(loopBack)
        , _itr(block)
        , _done(block != cfg->getStart()->asBlock())
        , _block(block)
    {}
...
class EdgeFrequencyInfo {
    ...
    void printEdge(OMR::Logger *log, TR_BitVector *toAdd, TR_BitVector *toSub)
    {
        if (!toAdd->isEmpty()) {
...
public:
    EdgeFrequencyInfo(TR::Compilation *comp, TR::CFGEdge *loopBack, int32_t numEdges, TR::Region &region,
        bool trace = false)
        : comp(comp)
        , loopBack(loopBack)
        , region(region)
    ...
    bool hasEdge(TR::CFGEdge *edge)
    {
        TR_ASSERT(edge->getId() > -1, "Edge id must have been set - from block_%d to block_%d!\n",
            edge->getFrom()->getNumber(), edge->getTo()->getNumber());
        return edgeMap[edge->getId() * 2] != NULL;
    }
    ...
    void addAbsoluteEdge(TR::CFGEdge *edge, TR::Block *block)
    {
        ...
        if (trace) {
            comp->log()->printf("abs edge %d-->%d:\n", edge->getFrom()->asBlock()->getNumber(),
                edge->getTo()->asBlock()->getNumber());
    ...
    bool computeBlockFrequencyFromPredecessors(TR_BitVector **blockCounters, TR_BlockFrequencyInfo *bfi,
        TR::Block *block)
    {
...
private:
    void computeBlockFrequency(int32_t blockNum, TR_BitVector **blockCounters, TR_BlockFrequencyInfo *bfi,
        TR_BitVector &additive, TR_BitVector &subtractive)
    {
        int32_t additiveElementCount = additive.elementCount();
        int32_t subtractiveElementCount = subtractive.elementCount();
        TR::DebugCounter::incStaticDebugCounter(comp,
            TR::DebugCounter::debugCounterName(comp, "jprofiling.counterSize/additive/%d", additiveElementCount));
        if (additiveElementCount > 1) {
            blockCounters[blockNum * 2] = new (comp->trPersistentMemory())
                TR_BitVector(additive.elementCount(), comp->trMemory(), persistentAlloc);
        ...
        if (subtractiveElementCount > 0) {
            TR::DebugCounter::incStaticDebugCounter(comp,
                TR::DebugCounter::debugCounterName(comp, "jprofiling.counterSize/subtractive/%d",
                    subtractiveElementCount));
            if (subtractiveElementCount > 1) {
                blockCounters[blockNum * 2 + 1] = new (comp->trPersistentMemory())
                    TR_BitVector(subtractive.elementCount(), comp->trMemory(), persistentAlloc);
 ...
 * \param parent The parent map populated by Prim's algorithm which is the computed spanning tree stored as child-parent
 * key-value pairs \param Q The priority queue to be used to run the algorithm \param stackMemoryRegion The stack memory
 * from which to allocate temporary data structures
 */
void TR_JProfilingBlock::computeMinimumSpanningTree(BlockParents &parent, BlockPriorityQueue &Q,
 ...
 *               and enter blocks
 */
int32_t TR_JProfilingBlock::processCFGForCounting(BlockParents &parent, TR::BlockChecklist &countedBlocks,
    TR::CFGEdge &loopBack)
{
    ...
    for (CFGNodeIterator iter(cfg, this); iter.currentBlock() != NULL; ++iter) {
        ...
        for (TR::CFGEdge *edge = sit.getFirst(); edge; edge = sit.getNext()) {
            ...
            if (block != to) {
                if (parent[block] == to) {
                    logprintf(trace(), log, "skipping edge block_%d to block_%d\n", block->getNumber(),
                        to->getNumber());
                    continue;
                }

                if (parent[to] == block && !(to->hasSuccessor(block) && block->hasSuccessor(to))) {
                    logprintf(trace(), log, "skipping edge block_%d to block_%d\n", block->getNumber(),
                        to->getNumber());
                    continue;
                }
            ...
            if (to->isOSRCatchBlock()) {
                ...
                if (!countedBlocks.contains(codeBlock)) {
                    ...
                    logprintf(trace(), log, "count osr block_%d (predecessor of block_%d)\n", codeBlock->getNumber(),
                        to->getNumber());
                }
            }
            // if the source of the edge has a single source we can count the source
                ...
                if (!countedBlocks.contains(block)) {
                    ...
                    logprintf(trace(), log, "count block_%d (single predecessor of block_%d)\n", block->getNumber(),
                        to->getNumber());
                }
            }
            ...
            // if the destination is a catch block we also just count the destination
                ...
                if (!countedBlocks.contains(to)) {
                    ...
                    insertionBlock = to;
                    logprintf(trace(), log, "count block_%d (single successor of block_%d)\n", to->getNumber(),
                        block->getNumber());
                }
            }
            ...
            // the only exception being for exception edges where we want to just count the catch for performance
                ...
                logprintf(trace(), log, "split edge %d to %d -- new block_%d\n", block->getNumber(), to->getNumber(),
                    insertionBlock->getNumber());
                ...
                for (TR::CFGEdgeList::iterator itr = insertionBlock->getExceptionSuccessors().begin(),
                                               end = insertionBlock->getExceptionSuccessors().end();
                     itr != end; ++itr)
                    (*itr)->setId(edgeId++);
            }
        }
 ...
 */
void TR_JProfilingBlock::dumpCounterDependencies(OMR::Logger *log, TR_BitVector **componentCounters)
{
    TR::CFG *cfg = comp()->getFlowGraph();
    for (CFGNodeIterator iter(cfg, this); iter.currentBlock() != NULL; ++iter) {
        TR::Block *block = iter.currentBlock();
        if (cfg->getStart() == block || cfg->getEnd() == block)
            ...
            log->printf("block_%d [%d,%d]:\n", block->getNumber(), block->getEntry()->getNode()->getInlinedSiteIndex(),
                block->getEntry()->getNode()->getByteCodeIndex());
...
void TR_JProfilingBlock::addRecompilationTests(TR_BlockFrequencyInfo *blockFrequencyInfo)
{
    ...
    if (root != NULL) {
        ...
        TR::Block *guardBlock1 = TR::Block::createEmptyBlock(node, comp(), originalFirstBlock->getFrequency());
        {
            ...
            TR::Node *enableTest = TR::Node::createif(TR::ificmpeq, enableLoad, TR::Node::iconst(node, -1),
        ...
        const char * const dc1 = TR::DebugCounter::debugCounterName(comp(), "methodRecomp/(%s)", comp()->signature());
        ...
        TR::TreeTop *callTree = TR::TransformUtil::generateRetranslateCallerWithPrepTrees(node,
            TR_PersistentMethodInfo::RecompDueToJProfiling, comp());
        ...
        TR::DebugCounter::incStaticDebugCounter(comp(),
            TR::DebugCounter::debugCounterName(comp(), "jprofiling.instrument/badcounters/(%s)", comp()->signature()));
    }
}

/**
 * JProfiling consists of two components: JProfilingValue & JProfilingBlock
 * JProfilingBlock will run early in the compilation, such that the CFG closely resembles the original program
 * structure, whilst the JProfilingValue pass runs later, as values may be introduced, eliminated or simplified.
 *
 * For non-profiling compilations, this can be enabled with the option TR_EnableJProfiling. Other control infrastructure
 * may limit which compilations this is actually applied to. See the env option TR_DisableFilterOnJProfiling for more
 * details.
 *
 * For profiling compilations, JProfiling can replace the existing JitProfiling instrumentation when either
 * TR_EnableJProfiling or TR_EnableJProfilingInProfilingCompilations are set. However, this introduces some complexity
 * as a compilation may switch to profiling part way though, potentially after JProfilingBlock should have run. In such
 * a scenario, the compilation will be restarted. See switchToProfiling() for the restart logic.
 */
int32_t TR_JProfilingBlock::perform()
{
    ...
    TR::DebugCounter::incStaticDebugCounter(comp(),
        TR::DebugCounter::debugCounterName(comp(), "jprofiling.instrument/success/(%s)", comp()->signature()));
    ...
    TR_BitVector **componentCounters = (TR_BitVector **)new (comp()->trMemory(), persistentAlloc,
        TR_Memory::BlockFrequencyInfo) void **[comp()->getFlowGraph()->getNextNodeNumber() * 2]();
    ...
    for (CFGNodeIterator iter(cfg, this); iter.currentBlock() != NULL; ++iter) {
        ...
        if (!countedBlocks.contains(block)) {
            TR::DebugCounter::incStaticDebugCounter(comp(),
                TR::DebugCounter::debugCounterName(comp(), "jprofiling/uncounted/%s",
        ...
        TR::SymbolReference *symRef = comp()->getSymRefTab()->createKnownStaticDataSymbolRef(
            blockFrequencyInfo->getFrequencyForBlock(block->getNumber()), TR::Int32);
        ...
        block->prepend(tree);
        TR::DebugCounter::incStaticDebugCounter(comp(),
            TR::DebugCounter::debugCounterName(comp(), "jprofiling/globalCounted/%s",
                comp()->getHotnessName(comp()->getMethodHotness())));
        ...
        if (block->getSuccessors().size() == 1
            && (block->getExceptionSuccessors().size() == 0 || block->isOSRInduceBlock())) {
        ...
        if (block->getPredecessors().size() == 1 && block->getExceptionPredecessors().size() == 0) {
            ...
            for (TR::CFGEdgeList::iterator itr = block->getExceptionPredecessors().begin(),
                                           end = block->getExceptionPredecessors().end();
                 itr != end; ++itr) {
            ...
            for (TR::CFGEdgeList::iterator itr = block->getPredecessors().begin(), end = block->getPredecessors().end();
                 itr != end; ++itr) {
                TR::Block *from = (*itr)->getFrom()->asBlock();
                edgeInfo.addAbsoluteEdge(*itr, block);
                Q.push(std::make_pair(0, from));
                if (from->isOSRCatchBlock()) {
                    for (TR::CFGEdgeList::iterator itr2 = from->getPredecessors().begin(),
                                                   end = from->getPredecessors().end();
                         itr2 != end; ++itr2) {
    ...
    while (!Q.empty()) {
        ...
        if (edgeInfo.computeBlockFrequencyFromPredecessors(componentCounters, blockFrequencyInfo, block)) {
            ...
            logprintf(trace(), log, "Can compute block_%d from successors\n", block->getNumber());
            bool canComputeParent = true;
            TR::CFGEdge *parentEdge = NULL;
...
const char *TR_JProfilingBlock::optDetailString() const throw() { return "O^O JPROFILING BLOCK: "; }
