...
bool J9::Simplifier::isLegalToUnaryCancel(TR::Node *node, TR::Node *firstChild, TR::ILOpCodes opcode)
{
    ...
    if (node->getOpCode().isConversionWithFraction() && firstChild->getOpCode().isConversionWithFraction()
        ...
        logprintf(trace(), log,
            "disallow unaryCancel of node %p and firstChild %p due to mismatch of decimal fractions (%d != %d)\n", node,
            firstChild, node->getDecimalFraction(), firstChild->getDecimalFraction());
        return false;
    }

    if (firstChild->getOpCodeValue() == opcode && node->getType().isBCD() && firstChild->getType().isBCD()
        && firstChild->getFirstChild()->getType().isBCD() && node->hasIntermediateTruncation()) {
        ...
        logprintf(trace(), log,
            "disallow unaryCancel of node %p and firstChild %p due to intermediate truncation of node\n", node,
            firstChild);
        return false;
...
TR::Node *J9::Simplifier::unaryCancelOutWithChild(TR::Node *node, TR::Node *firstChild, TR::TreeTop *anchorTree,
    TR::ILOpCodes opcode, bool anchorChildren)
{
    TR::Node *grandChild
        = OMR::Simplifier::unaryCancelOutWithChild(node, firstChild, anchorTree, opcode, anchorChildren);
    if (grandChild == NULL)
        return NULL;

    TR_RawBCDSignCode alwaysGeneratedSign = comp()->cg()->alwaysGeneratedSign(node);
    if (node->getType().isBCD() && grandChild->getType().isBCD()
        && (node->getDecimalPrecision() != grandChild->getDecimalPrecision()
            || alwaysGeneratedSign != raw_bcd_sign_unknown)) {
        ...
        if (node->getDecimalPrecision() != grandChild->getDecimalPrecision()) {
            ...
            grandChild
                = TR::Node::create(TR::ILOpCode::modifyPrecisionOpCode(grandChild->getDataType()), 1, origGrandChild);
            ...
            dumpOptDetails(comp(),
                "%sCreate %s [" POINTER_PRINTF_FORMAT
                "] to reconcile precision mismatch between node %s [" POINTER_PRINTF_FORMAT
                "] grandChild %s [" POINTER_PRINTF_FORMAT "] (%d != %d)\n",
                optDetailString(), grandChild->getOpCode().getName(), grandChild, node->getOpCode().getName(), node,
                origOrigGrandChild->getOpCode().getName(), origOrigGrandChild, node->getDecimalPrecision(),
                origOrigGrandChild->getDecimalPrecision());
        }
        // if the top level was always setting a particular sign code (e.g. ud2pd) then must maintain this side-effect
        // here when cancelling
        if (alwaysGeneratedSign != raw_bcd_sign_unknown) {
            ...
            grandChild = TR::Node::create(setSignOp, 2, origGrandChild,
            ...
            dumpOptDetails(comp(),
                "%sCreate %s [" POINTER_PRINTF_FORMAT
                "] to preserve setsign side-effect between node %s [" POINTER_PRINTF_FORMAT
                "] grandChild %s [" POINTER_PRINTF_FORMAT "] (sign=0x%x)\n",
                optDetailString(), grandChild->getOpCode().getName(), grandChild, node->getOpCode().getName(), node,
                origOrigGrandChild->getOpCode().getName(), origOrigGrandChild,
...
bool J9::Simplifier::isRecognizedPowMethod(TR::Node *node)
{
...
bool J9::Simplifier::isRecognizedAbsMethod(TR::Node *node)
{
    TR::Symbol *symbol = node->getSymbol();
    TR::MethodSymbol *methodSymbol = symbol ? symbol->castToMethodSymbol() : NULL;
    return (methodSymbol
...
bool J9::Simplifier::isRecognizedObjectComparisonNonHelper(TR::Node *node,
...
TR::Node *J9::Simplifier::foldAbs(TR::Node *node)
{
    ...
    if (node->getNumChildren() == 1)
        ...
        TR_ASSERT(node->getFirstChild()->getOpCodeValue() == TR::loadaddr,
            "The first child of abs is either the value or loadaddr");
        childNode = node->getSecondChild();
    }

    if (childNode && (childNode->isNonNegative() || (node->getReferenceCount() == 1))
        && performTransformation(comp(), "%sFolded abs for postive argument on node [%p]\n", optDetailString(), node)) {
        TR::TreeTop::create(comp(), _curTree->getPrevTreeTop(), TR::Node::create(TR::treetop, 1, childNode));
...
TR::Node *J9::Simplifier::simplifyiCallMethods(TR::Node *node, TR::Block *block)
{
    if (isRecognizedAbsMethod(node)) {
        ...
        if (isRecognizedObjectComparisonNonHelper(node, nonHelperSymbol)) {
            ...
            const bool lhsNull = lhs->getOpCodeValue() == TR::aconst && lhs->getConstValue() == 0;

            TR::Node *rhs = node->getChild(1);
            const bool rhsNull = rhs->getOpCodeValue() == TR::aconst && rhs->getConstValue() == 0;
            ...
            if (lhsNull || rhsNull || lhs == rhs) {
                ...
                if (performTransformation(comp(), "%sChanging n%un from %s to %s\n", optDetailString(),
                        node->getGlobalIndex(), comp()->getSymRefTab()->getNonHelperSymbolName(nonHelperSymbol),
                        isEqualityComparison ? "acmpeq" : "acmpne")) {
                    const char *counterName = TR::DebugCounter::debugCounterName(comp(),
                        "vt-helper/simplifier-xformed/acmp/(%s)/bc=%d", comp()->signature(), node->getByteCodeIndex());
 ...
 */
TR::Node *J9::Simplifier::simplifylCallMethods(TR::Node *node, TR::Block *block)
{
    ...
    if (methodSymbol) {
        ...
        if (symbol && (symbol->getRecognizedMethod() == TR::java_lang_Math_abs_L)) {
            node = foldAbs(node);
        }
    }
...
TR::Node *J9::Simplifier::convertCurrentTimeMillis(TR::Node *node, TR::Block *block)
{
    if (performTransformation(comp(),
            "%sConvert currentTimeMillis to currentTimeMaxPrecision with divide of" INT64_PRINTF_FORMAT
            " on node [%p]\n",
...
{
    if (performTransformation(comp(),
            "%sConvert nanoTime to currentTimeMaxPrecision with multiply of %d/%d on node [%p]\n", optDetailString(),
            OMRPORT_TIME_HIRES_NANOTIME_NUMERATOR, OMRPORT_TIME_HIRES_NANOTIME_DENOMINATOR, node)) {
        TR::Node *lcallNode = TR::Node::createWithSymRef(node, TR::lcall, 0,
            comp()->getSymRefTab()->findOrCreateCurrentTimeMaxPrecisionSymbol());
        TR::TreeTop *callTreeTop = findTreeTop(node, block);
        TR_ASSERT(callTreeTop != NULL, "should have been able to find this tree top");
...
TR::Node *J9::Simplifier::simplifyaCallMethods(TR::Node *node, TR::Block *block)
{
    if ((node->getOpCode().isCallDirect()) && !node->getSymbolReference()->isUnresolved()
        ...
        if (isSimplifiableMethod && (node->getReferenceCount() == 1)
            && performTransformation(comp(),
                requiresArgNULLCHK ? ("%sReplaced dead BigDecimal/BigInteger call node [" POINTER_PRINTF_FORMAT
                                      "] with NULLCHK of argument\n")
            ...
            if (requiresArgNULLCHK) {
                ...
                TR::TreeTop::create(comp(), _curTree,
                    TR::Node::createWithSymRef(node, TR::NULLCHK, 1,
                        TR::Node::create(node, TR::PassThrough, 1, secondChild), nullCheckSR));
                _alteredBlock = true;
            }
...
bool J9::Simplifier::isLegalToFold(TR::Node *node, TR::Node *firstChild)
{
...
TR::Node *J9::Simplifier::getUnsafeIorByteChild(TR::Node *child, TR::ILOpCodes b2iOpCode, int32_t mulConst)
{
    if (child->getOpCodeValue() == TR::imul && child->getSecondChild()->getOpCodeValue() == TR::iconst
        && child->getSecondChild()->getInt() == mulConst && child->getFirstChild()->getOpCodeValue() == b2iOpCode
        ...
        && child->getFirstChild()->getFirstChild()->getSymbolReference()
            == getSymRefTab()->findOrCreateUnsafeSymbolRef(TR::Int8)) {
...
TR::Node *J9::Simplifier::getLastUnsafeIorByteChild(TR::Node *child)
{
    if (child->getOpCodeValue() == TR::bu2i && child->getReferenceCount() == 1
        && child->getFirstChild()->getOpCodeValue() == TR::bloadi && child->getFirstChild()->getReferenceCount() == 1
...
/** Get the first child of an aiadd/aladd which will be a node representing the underlying array.
  ...
  @return the first child or 0 if we do not pass in an aiadd/aladd node since that would break the pattern we are
  matching.
*/
...
TR::Node *J9::Simplifier::getArrayOffset(TR::Node *node, int32_t isubConst)
{
    ...
    // an ibyteswap.
    if (node->getOpCode().isArrayRef() && node->getReferenceCount() == 1 && node->getSecondChild()->getOpCode().isSub()
        && node->getSecondChild()->getReferenceCount() == 1
        && ((node->getSecondChild()->getSecondChild()->getOpCodeValue() == TR::iconst
                && node->getSecondChild()->getSecondChild()->getInt() == isubConst)
            || (node->getSecondChild()->getSecondChild()->getOpCodeValue() == TR::lconst
                && node->getSecondChild()->getSecondChild()->getLongInt() == (int64_t)isubConst))) {
        return node->getSecondChild()->getFirstChild();
    }
    return NULL;
}

TR::Node *J9::Simplifier::simplifyiOrPatterns(TR::Node *node)
{
    TR::Node *firstChild = node->getFirstChild();
    TR::Node *secondChild = node->getSecondChild();
    ...
    if (firstChild->getReferenceCount() == 1 && firstChildOp == TR::ior
        ...
        if ((addr = getUnsafeBaseAddr(byte2, -1)) && addr == byte1 && (addr = getUnsafeBaseAddr(byte3, -2))
            && addr == byte1 && (addr = getUnsafeBaseAddr(byte4, -3)) && addr == byte1
    ...
    if (!disableIORByteSwap && !comp()->target().cpu.isBigEndian() && // the bigEndian case needs more thought
        ...
        if (secondChild->getSecondChild()->getOpCodeValue() == TR::ior && secondChild->getReferenceCount() == 1
            ...
            && (byte3
                = getArrayByteChildWithMultiplier(secondChild->getSecondChild()->getFirstChild(), TR::bu2i, SHIFT8MUL))
            ...
            offset1 = NULL;
            if (addr1 && byte1->getSecondChild()->getOpCode().isSub()
                && byte1->getSecondChild()->getReferenceCount() == 1)
            ...
            if (offset1) {
                if ((byte1->getSecondChild()->getSecondChild()->getOpCodeValue() == TR::lconst
                        && byte1->getSecondChild()->getSecondChild()->getLongInt()
                            <= ((int64_t)TR::getMaxSigned<TR::Int32>())
                        && byte1->getSecondChild()->getSecondChild()->getLongInt()
                            >= ((int64_t)TR::getMinSigned<TR::Int32>()))) {
            ...
            if (expectedConstantOffset1 && (getArrayBaseAddr(byte2) == addr1)
                && (getArrayOffset(byte2, initialConstantOffset + 1) == offset1) && (getArrayBaseAddr(byte3) == addr1)
                && (getArrayOffset(byte3, initialConstantOffset + 2) == offset1) && (getArrayBaseAddr(byte4) == addr1)
                && (getArrayOffset(byte4, initialConstantOffset + 3) == offset1)
    ...
    //
    //
    if (firstChild->getOpCodeValue() == TR::l2i && secondChild->getOpCodeValue() == TR::l2i
...
TR::Node *J9::Simplifier::getOrOfTwoConsecutiveBytes(TR::Node *ior)
{
    // Big Endian
    // ior
    //   imul
...
TR::Node *J9::Simplifier::simplifyi2sPatterns(TR::Node *node)
{
    TR::Node *firstChild = node->getFirstChild();
    TR::Node *address;
...
TR::Node *J9::Simplifier::simplifyd2fPatterns(TR::Node *node)
{
    ...
    if (!performTransformation(comp(),
            "%sTransforming [" POINTER_PRINTF_FORMAT "] (double)sqrt(f2d(x))->(float)sqrt(x)\n", optDetailString(),
            sqrtCall)) {
        return NULL;
    }
    ...
    TR_ASSERT(callTreeTop != NULL, "should have been able to find this tree top (call %p) in block %d", sqrtCall,
...
bool J9::Simplifier::isBoundDefinitelyGELength(TR::Node *boundChild, TR::Node *lengthChild)
{
    ...
    if (boundChild->getOpCode().isArrayLength()) {
        ...
        if (first->getOpCodeValue() == TR::aloadi && lengthChild->getOpCodeValue() == TR::iloadi
...
static bool symRefPairMatches(TR::SymbolReference *actual1, TR::SymbolReference *actual2, TR::SymbolReference *desired1,
    TR::SymbolReference *desired2)
{
...
TR::Node *J9::Simplifier::simplifyIndirectLoadPatterns(TR::Node *node)
{
    TR::Node *firstChild = node->getFirstChild();
        ...
        if (childOp == TR::aloadi || childOp == TR::iloadi || childOp == TR::lloadi) {
         ...
         if (symRefPairMatches(nodeSymref, childSymref, getSymRefTab()->findClassFromJavaLangClassSymbolRef(), getSymRefTab()->findJavaLangClassFromClassSymbolRef()))
            ...
            if (symRefPairMatches(nodeSymref, childSymref,
                    getSymRefTab()->findClassFromJavaLangClassAsPrimitiveSymbolRef(),
                    getSymRefTab()->findJavaLangClassFromClassSymbolRef()))
                fieldsAreComplementary = true;
            ...
            if (fieldsAreComplementary
                && performTransformation(comp(), "%sFolded complementary field load [%p]->%s->%s\n", optDetailString(),
                    grandchild, nodeSymref->getName(getDebug()), childSymref->getName(getDebug()))) {
                TR::Node *replacement = grandchild;
                if (replacement->getDataType() != node->getDataType()) {
                    TR::ILOpCodes convOpCode
                        = TR::ILOpCode::getProperConversion(replacement->getDataType(), node->getDataType(), false);
                    TR_ASSERT(convOpCode != TR::BadILOp,
                        "Conversion between two different data types requires an opcode");
...
void J9::Simplifier::setNodePrecisionIfNeeded(TR::Node *baseNode, TR::Node *node, TR::NodeChecklist &visited)
{
