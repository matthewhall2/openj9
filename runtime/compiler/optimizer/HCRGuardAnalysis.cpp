...
void TR_HCRGuardAnalysis::analyzeNode(TR::Node *node, vcount_t visitCount, TR_BlockStructure *block,
...
TR_HCRGuardAnalysis::TR_HCRGuardAnalysis(TR::Compilation *comp, TR::Optimizer *optimizer, TR_Structure *rootStructure)
    : TR_UnionSingleBitContainerAnalysis(comp, comp->getFlowGraph(), optimizer,
          false /*comp->getOption(TR_TraceFearPointAnalysis)*/)
{
...
void TR_HCRGuardAnalysis::initializeGenAndKillSetInfo()
{
    ...
    for (TR::TreeTop *treeTop = comp()->getStartTree(); treeTop; treeTop = treeTop->getNextTreeTop()) {
        ...
        if (ttNode->getOpCodeValue() == TR::BBStart) {
            ...
            } else if (hasExceptionKill && !hasExceptionNotKill) /* All exception points have to be kill in order to
                                                                    kill along the exception path */
            {
                _exceptionKillSetInfo[blockId]->setAll(numBits);
                _exceptionGenSetInfo[blockId]->empty();
            }
        ...
        if (comp()->isPotentialOSRPoint(ttNode, &osrNode) && !checklist.contains(osrNode)) {
            ...
            if (supportsOSR) {
                ...
                TR_ASSERT_FATAL(!comp()->isFearPointPlacementUnrestricted(), "unexpected uninducible OSR yield point");
            ...
            if (guardInfo->getKind() != TR_HCRGuard && TR_FearPointAnalysis::virtualGuardKillsFear(comp(), ttNode)) {
                ...
                // fact that there is few optimization opportunities on the taken side and optimizing it has little
                // benefit, we require optimizations that can generate fear stay away from taken side such that
                // considering a guard to be a kill for the taken side is safe.
                //
                isKill = true;
                isGen = false;
            }
...
bool TR_HCRGuardAnalysis::postInitializationProcessing() { return true; }
