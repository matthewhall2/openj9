...
const char *TR::TreeLowering::optDetailString() const throw() { return "O^O TREE LOWERING: "; }

int32_t TR::TreeLowering::perform()
{
...
void TR::TreeLowering::Transformer::moveNodeToEndOfBlock(TR::Block * const block, TR::TreeTop * const tt,
    TR::Node * const node, bool isAddress)
{
    ...
    if (iterTT != blockExit) {
        logprintf(trace(), log,
            "Moving treetop containing node n%dn [%p] for helper call to end of prevBlock in preparation of final "
            "block split\n",
        ...
        while (iterTT != blockExit) {
            ...
            if (isStoreOp && iterTT->getNode()->getFirstChild() == node) {
                logprintf(trace(), log,
                    "Moving treetop containing node n%dn [%p] for store of helper call result to end of prevBlock in "
                    "preparation of final block split\n",
...
TR::Block *TR::TreeLowering::Transformer::splitForFastpath(TR::Block * const block, TR::TreeTop * const splitPoint,
    TR::Block * const targetBlock)
{
 ...
 * @param substituteNode is the reg dep node to substitute if a matching register is found in `sourceNode` (NULL if
 * none)
 */
static void copyExitRegDepsAndSubstitute(TR::Node * const targetNode, TR::Node * const sourceNode,
    TR::Node * const substituteNode)
{
    for (int i = 0; i < sourceNode->getNumChildren(); ++i) {
        TR::Node *child = sourceNode->getChild(i);
        if (substituteNode && child->getLowGlobalRegisterNumber() == substituteNode->getLowGlobalRegisterNumber()
 ...
 */
static TR::Node *copyBranchGlRegDepsAndSubstitute(TR::Node * const branchNode, TR::Node * const sourceGlRegDepsNode,
    TR::Node * const substituteNode)
{
 ...
 *
 */
void AcmpTransformer::lower(TR::Node * const node, TR::TreeTop * const tt)
{
    TR::Compilation *comp = this->comp();
    OMR::Logger *log = comp->log();
    ...
    auto * const anchoredCallArg1TT
        = TR::TreeTop::create(comp, tt->getPrevTreeTop(), TR::Node::create(TR::treetop, 1, node->getFirstChild()));
    auto * const anchoredCallArg2TT
        = TR::TreeTop::create(comp, tt->getPrevTreeTop(), TR::Node::create(TR::treetop, 1, node->getSecondChild()));
    ...
    const bool isObjectEqualityTest
        = (node->getSymbolReference() == comp->getSymRefTab()->findOrCreateAcmpeqHelperSymbolRef());
    ...
    logprintf(trace(), log, "Call node n%un is in block %d, targetBlock is %d\n", node->getGlobalIndex(),
        callBlock->getNumber(), targetBlock->getNumber());
    ...
    if (anchoredNode->getOpCodeValue() == TR::iRegLoad) {
        ...
        logprintf(trace(), log, "Storing constant %d to symref %d (%s)\n", cmpResultForEquality,
            anchoredNode->getSymbolReference()->getReferenceNumber(),
    ...
    auto *ifacmpeqNode = TR::Node::createif(TR::ifacmpeq, anchoredCallArg1TT->getNode()->getFirstChild(),
        anchoredCallArg2TT->getNode()->getFirstChild(), targetBlock->getEntry());
    ...
    if (!anchoredCallArg1TT->getNode()->getFirstChild()->isNonNull()) {
        auto * const checkLhsNull = TR::Node::createif(TR::ifacmpeq, anchoredCallArg1TT->getNode()->getFirstChild(),
            nullConst, targetBlock->getEntry());
        exitGlRegDeps = copyBranchGlRegDepsAndSubstitute(checkLhsNull, exitGlRegDeps, regDepForStoreNode);
        // Set regDepForStoreNode to NULL so that the subsequent calls to copyBranchGlRegDepsAndSubstitute do not need
        // to substitute it.
        regDepForStoreNode = NULL;
        tt->insertBefore(TR::TreeTop::create(comp, checkLhsNull));
        callBlock = splitForFastpath(callBlock, tt, targetBlock);
        logprintf(trace(), log, "Added check node n%un; call node is now in block_%d\n", checkLhsNull->getGlobalIndex(),
            callBlock->getNumber());
    } else {
    ...
    if (!anchoredCallArg2TT->getNode()->getFirstChild()->isNonNull()) {
        auto * const checkRhsNull = TR::Node::createif(TR::ifacmpeq, anchoredCallArg2TT->getNode()->getFirstChild(),
            nullConst, targetBlock->getEntry());
    ...
    auto * const checkLhsIsVT
        = TR::Node::createif(TR::ificmpeq, isLhsValueType, TR::Node::iconst(0), targetBlock->getEntry());
    ...
    // Set regDepForStoreNode to NULL so that the subsequent calls to copyBranchGlRegDepsAndSubstitute do not need to
    // substitute it.
    regDepForStoreNode = NULL;
    tt->insertBefore(TR::TreeTop::create(comp, checkLhsIsVT));
    callBlock = splitForFastpath(callBlock, tt, targetBlock);
    logprintf(trace(), log, "Added check node n%un; call node is now in block_%d\n", checkLhsIsVT->getGlobalIndex(),
        callBlock->getNumber());
    ...
    auto * const checkRhsIsNotVT
        = TR::Node::createif(TR::ificmpne, isRhsValueType, TR::Node::iconst(0), callBlock->getEntry());
...
static void copyRegisterDependency(TR::Node *fromNode, TR::Node *toNode)
{
 ...
 * @param tt is the treetop at the root of the tree ancoring the current node
 */
void NonNullableArrayNullStoreCheckTransformer::lower(TR::Node * const node, TR::TreeTop * const tt)
{
    // Transform
    //   +-----------------------------------------+
    //   | ttprev                                  |
    ...
    if (!sourceChild->isNonNull()) {
        ...
        TR::TreeTop *anchoredArrayTT
            = TR::TreeTop::create(comp(), tt->getPrevTreeTop(), TR::Node::create(TR::treetop, 1, destChild));
        TR::TreeTop *anchoredSourceTT
            = TR::TreeTop::create(comp(), anchoredArrayTT, TR::Node::create(TR::treetop, 1, sourceChild));
        ...
        TR::Node *testIsNotNullRestrictedArray
            = TR::Node::create(TR::icmpeq, 2, testIsNullRestrictedArray, TR::Node::iconst(0));
        ...
        logprintf(enableTrace, comp()->log(),
            "checkNotNullRestrictedArray n%dn is isolated in checkNotNullRestrictedBlock %d\n",
...
static TR::Node *cloneAndTweakGlRegDepsFromBBExit(TR::Node *bbExitNode, TR::Compilation *comp, bool enableTrace,
    TR_GlobalRegisterNumber registerToSkip)
{
    OMR::Logger *log = comp->log();
    ...
    if (bbExitNode->getNumChildren() > 0) {
        ...
        for (int32_t i = 0; i < origRegDeps->getNumChildren(); ++i) {
            ...
            if ((registerToSkip != -1) && regDep->getOpCodeValue() == TR::PassThrough
...
static TR::Node *createStoreNodeForAnchoredNode(TR::Node *anchoredNode, TR::Node *nodeToBeStored, TR::Compilation *comp,
    bool enableTrace, const char *msg)
{
    OMR::Logger *log = comp->log();
    TR::Node *storeNode = NULL;
    ...
    if (anchoredNode->getOpCodeValue() == TR::aRegLoad) {
        ...
        logprintf(enableTrace, log, "Storing %s n%dn to symref %d (%s) storeNode n%dn anchoredNode n%dn\n", msg,
            nodeToBeStored->getGlobalIndex(), anchoredNode->getSymbolReference()->getReferenceNumber(),
            anchoredNode->getSymbolReference()->getName(comp->getDebug()), storeNode->getGlobalIndex(),
            anchoredNode->getGlobalIndex());
    } else {
        TR_ASSERT_FATAL_WITH_NODE(anchoredNode, false,
            "Anchored call node n%dn has been turned into unexpected opcode\n", anchoredNode->getGlobalIndex());
    }
...
/*
 * LoadArrayElementTransformer transforms the block that contains the jitLoadFlattenableArrayElement helper call into
 three blocks:
 *   1. The merge block (blockAfterHelperCall) that contains the tree tops after the helper call
 *   2. The helper call block (helperCallBlock) that contains the helper call and is moved to the end of the tree top
 list
 ...
 *
 */
void LoadArrayElementTransformer::lower(TR::Node * const node, TR::TreeTop * const tt)
{
    bool enableTrace = trace();
    TR::Compilation *comp = this->comp();
    OMR::Logger *log = comp->log();
    ...
    if (!performTransformation(comp,
            "%sTransforming loadArrayElement treetop n%dn node n%dn [%p] in block_%d: elementIndexNode n%dn "
            "arrayBaseAddressNode n%dn ttAfterHelperCall n%dn\n",
            optDetailString(), tt->getNode()->getGlobalIndex(), node->getGlobalIndex(), node,
            originalBlock->getNumber(), elementIndexNode->getGlobalIndex(), arrayBaseAddressNode->getGlobalIndex(),
            tt->getNextTreeTop()->getNode()->getGlobalIndex()))
        return;
    ...
    // Anchoring the helper call ensures that the return value from the helper call or from the nullable array element
    // load will be saved to a register or a temp.
    TR::TreeTop *anchoredCallTT = TR::TreeTop::create(comp, tt, TR::Node::create(TR::treetop, 1, node));
    TR::TreeTop *anchoredElementIndexTT
        = TR::TreeTop::create(comp, tt->getPrevTreeTop(), TR::Node::create(TR::treetop, 1, elementIndexNode));
    TR::TreeTop *anchoredArrayBaseAddressTT
        = TR::TreeTop::create(comp, anchoredElementIndexTT, TR::Node::create(TR::treetop, 1, arrayBaseAddressNode));

    logprintf(enableTrace, log,
        "Anchored call node under treetop n%un (0x%p), elementIndex under treetop n%un (0x%p), arrayBaseAddress under "
        "treetop n%un (0x%p)\n",
        anchoredCallTT->getNode()->getGlobalIndex(), anchoredCallTT->getNode(),
    ...
    // the BBExit of arrayElementLoadBlock. It has to be saved because it might get changed after the next split at the
    // helper call.
    //
    ...
    TR_GlobalRegisterNumber registerToSkip
        = (anchoredNode->getOpCodeValue() == TR::aRegLoad) ? anchoredNode->getGlobalRegisterNumber() : -1;
    TR::Node *tmpGlRegDeps
        = cloneAndTweakGlRegDepsFromBBExit(originalBlock->getExit()->getNode(), comp, enableTrace, registerToSkip);
    ...
    TR::Node *elementAddress = J9::TransformUtil::calculateElementAddress(comp, anchoredArrayBaseAddressNode,
        anchoredElementIndexNode, TR::Address);
    TR::SymbolReference *elementSymRef
        = comp->getSymRefTab()->findOrCreateArrayShadowSymbolRef(TR::Address, anchoredArrayBaseAddressNode);
    TR::Node *elementLoadNode = TR::Node::createWithSymRef(comp->il.opCodeForIndirectArrayLoad(TR::Address), 1, 1,
        elementAddress, elementSymRef);
    elementLoadNode->copyByteCodeInfo(node);

    TR::TreeTop *elementLoadTT = NULL;
    if (comp->useCompressedPointers())
        elementLoadTT
            = originalBlock->append(TR::TreeTop::create(comp, TR::Node::createCompressedRefsAnchor(elementLoadNode)));
    else
        elementLoadTT
            = originalBlock->append(TR::TreeTop::create(comp, TR::Node::create(node, TR::treetop, 1, elementLoadNode)));
    ...
    TR::Node *storeArrayElementNode
        = createStoreNodeForAnchoredNode(anchoredNode, elementLoadNode, comp, enableTrace, "array element load");
    ...
    logprintf(enableTrace, log, "Store array element load node n%dn to n%dn %s\n", elementLoadNode->getGlobalIndex(),
        storeArrayElementNode->getGlobalIndex(), storeArrayElementNode->getOpCode().getName());
    ...
    logprintf(enableTrace, log, "Isolated array element load treetop n%dn node n%dn in block_%d\n",
        elementLoadTT->getNode()->getGlobalIndex(), elementLoadNode->getGlobalIndex(),
        arrayElementLoadBlock->getNumber());
    ...
    if (!skipBoundChecks(comp, node)) {
        ...
        elementLoadTT->insertBefore(TR::TreeTop::create(comp,
            TR::Node::createWithSymRef(TR::BNDCHK, 2, 2, arraylengthNode, anchoredElementIndexNode,
    ...
    // Add storeArrayElementNode to GlRegDeps to pass the return value from the array element load to
    // blockAfterHelperCall
    ...
    TR::Node *gotoAfterHelperCallNode
        = TR::Node::create(helperCallBlock->getExit()->getNode(), TR::Goto, 0, blockAfterHelperCall->getEntry());
...
/*
 * StoreArrayElementTransformer transforms the block that contains the jitStoreFlattenableArrayElement helper call into
 three blocks:
 ...
 *
 */
void StoreArrayElementTransformer::lower(TR::Node * const node, TR::TreeTop * const tt)
{
    bool enableTrace = trace();
    TR::Compilation *comp = this->comp();
    OMR::Logger *log = comp->log();
    ...
    if (!performTransformation(comp,
            "%sTransforming storeArrayElement treetop n%dn node n%dn [%p] in block_%d: children (n%dn, n%dn, n%dn) "
            "ttAfterHelperCall n%dn\n",
            optDetailString(), tt->getNode()->getGlobalIndex(), node->getGlobalIndex(), node,
            originalBlock->getNumber(), valueNode->getGlobalIndex(), elementIndexNode->getGlobalIndex(),
            arrayBaseAddressNode->getGlobalIndex(), tt->getNextTreeTop()->getNode()->getGlobalIndex()))
        return;
    ...
    TR::TreeTop *anchoredArrayBaseAddressTT
        = TR::TreeTop::create(comp, tt->getPrevTreeTop(), TR::Node::create(TR::treetop, 1, arrayBaseAddressNode));
    TR::TreeTop *anchoredElementIndexTT
        = TR::TreeTop::create(comp, anchoredArrayBaseAddressTT, TR::Node::create(TR::treetop, 1, elementIndexNode));
    TR::TreeTop *anchoredValueTT
        = TR::TreeTop::create(comp, anchoredElementIndexTT, TR::Node::create(TR::treetop, 1, valueNode));

    logprintf(enableTrace, log,
        "Anchored elementIndex under treetop n%un (0x%p), arrayBaseAddress under treetop n%un (0x%p), value under "
        "treetop n%un (0x%p), \n",
        anchoredElementIndexTT->getNode()->getGlobalIndex(), anchoredElementIndexTT->getNode(),
        anchoredArrayBaseAddressTT->getNode()->getGlobalIndex(), anchoredArrayBaseAddressTT->getNode(),
    ...
    TR::Node *tmpGlRegDeps = cloneAndTweakGlRegDepsFromBBExit(originalBlock->getExit()->getNode(), comp, enableTrace,
        -1 /* registerToSkip */);
    ...
    TR::SymbolReference *elementSymRef
        = comp->getSymRefTab()->findOrCreateArrayShadowSymbolRef(TR::Address, anchoredArrayBaseAddressNode);
    TR::Node *elementStoreNode = TR::Node::createWithSymRef(TR::awrtbari, 3, 3, elementAddress, anchoredValueNode,
        anchoredArrayBaseAddressNode, elementSymRef);
    ...
    if (!skipArrayStoreChecks(comp, node)) {
        TR::SymbolReference *arrayStoreCHKSymRef
            = comp->getSymRefTab()->findOrCreateTypeCheckArrayStoreSymbolRef(comp->getMethodSymbol());
        arrayStoreCHKNode
            = TR::Node::createWithRoomForThree(TR::ArrayStoreCHK, elementStoreNode, 0, arrayStoreCHKSymRef);
        ...
        arrayStoreTT
            = originalBlock->append(TR::TreeTop::create(comp, TR::Node::create(TR::treetop, 1, elementStoreNode)));
    ...
    logprintf(enableTrace, log, "Isolated array element store treetop n%dn node n%dn in block_%d\n",
        ...
        arrayStoreCHKNode ? arrayStoreCHKNode->getGlobalIndex() : elementStoreNode->getGlobalIndex(),
    ...
    const bool needNullCHK
        = !anchoredArrayBaseAddressNode->isNonNull() && (tt->getNode()->getOpCodeValue() == TR::NULLCHK);
    ...
    if (needNullCHK || needBNDCHK) {
        ...
        // If the helper call node is anchored under NULLCHK due to compactNullChecks, the NULLCHK is split into the
        // helper call block. The NULLCHK is required for regular array block.
        if (needNullCHK) {
            arrayStoreTT->insertBefore(TR::TreeTop::create(comp,
                TR::Node::createWithSymRef(TR::NULLCHK, 1, 1, arraylengthNode,
 ...
 * @endverbatim
 */
void IsIdentityObjectTransformer::lower(TR::Node * const node, TR::TreeTop * const tt)
{
 ...
 *
 */
void TR::TreeLowering::lowerValueTypeOperations(TransformationManager &transformations, TR::Node *node, TR::TreeTop *tt)
{
    TR::SymbolReferenceTable *symRefTab = comp()->getSymRefTab();
    static char *disableInliningCheckAastore = feGetEnv("TR_DisableVT_AASTORE_Inlining");

    if (node->getOpCode().isCall()) {
        if (symRefTab->isNonHelper(node->getSymbolReference(),
        ...
        const bool isObjectEqualityTest = symRefTab->isNonHelper(node->getSymbolReference(),
            TR::SymbolReferenceTable::objectEqualityComparisonSymbol);
        const bool isObjectInequalityTest = symRefTab->isNonHelper(node->getSymbolReference(),
        ...
        if (isObjectEqualityTest || isObjectInequalityTest) {
        ...
        } else if (symRefTab->isNonHelper(node->getSymbolReference(),
                       TR::SymbolReferenceTable::nonNullableArrayNullStoreCheckSymbol)) {
            ...
            if (!disableInliningCheckAaload) {
                TR_ASSERT_FATAL_WITH_NODE(tt->getNode(),
                    (tt->getNode()->getOpCodeValue() == TR::treetop)
                        || (tt->getNode()->getOpCodeValue() == TR::NULLCHK),
                    "LoadArrayElementTransformer cannot process the treetop node that is neither a treetop nor a "
                    "NULLCHK\n");
            ...
            if (!disableInliningCheckAastore) {
                TR_ASSERT_FATAL_WITH_NODE(tt->getNode(),
                    (tt->getNode()->getOpCodeValue() == TR::treetop)
                        || (tt->getNode()->getOpCodeValue() == TR::NULLCHK),
                    "StoreArrayElementTransformer cannot process the treetop node that is neither a treetop nor a "
                    "NULLCHK\n");
                transformations.addTransformation(getTransformer<StoreArrayElementTransformer>(), node, tt);
            }
        } else if (symRefTab->isNonHelper(node->getSymbolReference(),
                       TR::SymbolReferenceTable::isIdentityObjectNonHelperSymbol)) {
