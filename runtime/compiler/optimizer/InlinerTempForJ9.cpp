...
static bool isWarm(TR::Compilation *comp) { return comp->getMethodHotness() >= warm; }
...
static int32_t getJ9InitialBytecodeSize(TR_ResolvedMethod *feMethod, TR::ResolvedMethodSymbol *methodSymbol,
    TR::Compilation *comp)
{
    ...
    if (feMethod->getRecognizedMethod() == TR::java_lang_String_indexOf_String_int
        ...
        if (methodSymbol && !methodSymbol->mayHaveInlineableCall()
            && size <= 5) // favor the inlining of methods that are very small
            size = 0;
    }

    TR_J9EstimateCodeSize::adjustEstimateForStringCompression(feMethod, size,
        TR_J9EstimateCodeSize::STRING_COMPRESSION_ADJUSTMENT_FACTOR);
...
static bool insideIntPipelineForEach(TR_ResolvedMethod *method, TR::Compilation *comp)
{
    ...
    // If the first method passed into this function is IntPipeline$Head.forEach or IntPipeline.forEach, true is
    // returned since IntPipeline$Head.forEach and IntPipeline.forEach needs to be inlined for JIT GPU. If not the
    // method name is checked to see if it is called accept. If the method in the chain just before IntPipeline.forEach
    // is accept, true is also returned This tries to inline accept and the methods inside accept
...
bool TR_J9InlinerPolicy::inlineRecognizedMethod(TR::RecognizedMethod method)
{
    //    if (method ==
    ...
    if (comp()->isConverterMethod(method) && comp()->canTransformConverterMethod(method))
        return false;

    // Check for memoizing methods
    if (comp()->getOption(TR_DisableDememoization)
        || (comp()->getMethodHotness()
            < hot)) // TODO: This should actually check whether EA will run, but that's not crucial for correctness
    {
        switch (method) {
...
int32_t TR_J9InlinerPolicy::getInitialBytecodeSize(TR_ResolvedMethod *feMethod, TR::ResolvedMethodSymbol *methodSymbol,
    TR::Compilation *comp)
{
    return getJ9InitialBytecodeSize(feMethod, methodSymbol, comp);
}

bool TR_J9InlinerPolicy::aggressivelyInlineInLoops() { return _aggressivelyInlineInLoops; }

void TR_J9InlinerPolicy::determineAggressionInLoops(TR::ResolvedMethodSymbol *callerSymbol)
{
    if (isHot(comp()) && OMR_InlinerPolicy::getInitialBytecodeSize(callerSymbol, comp()) < 100)
        _aggressivelyInlineInLoops = true;
}

void TR_J9InlinerPolicy::determineInliningHeuristic(TR::ResolvedMethodSymbol *callerSymbol)
{
    determineAggressionInLoops(callerSymbol);
    return;
}

void TR_MultipleCallTargetInliner::NodeEstimate::operator()(TR_CallTarget *ct, TR::Compilation *comp)
{
...
bool TR_J9InlinerPolicy::mustBeInlinedEvenInDebug(TR_ResolvedMethod *calleeMethod, TR::TreeTop *callNodeTreeTop)
{
    if (calleeMethod) {
        switch (calleeMethod->convertToMethod()->getMandatoryRecognizedMethod()) {
            ...
            case TR::java_lang_invoke_MethodHandle_invokeExactTargetAddress: {
                ...
                while (scanTT
                    && scanTT->getNode()->getByteCodeInfo().getByteCodeIndex()
                        == callNodeTreeTop->getNode()->getByteCodeInfo().getByteCodeIndex()
                    && scanTT->getNode()->getByteCodeInfo().getCallerIndex()
                        == callNodeTreeTop->getNode()->getByteCodeInfo().getCallerIndex()) {
...
/**  Test for methods that we wish to inline whenever possible.
   ...
   the inliner.
*/
bool TR_J9InlinerPolicy::alwaysWorthInlining(TR_ResolvedMethod *calleeMethod, TR::Node *callNode)
{
    if (!calleeMethod)
        return false;
    ...
    if (!strncmp(calleeMethod->classNameChars(), "java/util/concurrent/atomic/",
            strlen("java/util/concurrent/atomic/"))) {
        return true;
    }
    ...
    if (!comp()->getOption(TR_DisableForceInlineAnnotations) && comp()->fej9()->isForceInline(calleeMethod)) {
        logprintf(comp()->trace(OMR::inlining), comp()->log(),
            "@ForceInline was specified for %s, in alwaysWorthInlining\n", calleeMethod->signature(comp()->trMemory()));
        return true;
    }

    if (calleeMethod->getRecognizedMethod() == TR::unknownMethod && comp()->fej9()->isIntrinsicCandidate(calleeMethod)
        && !comp()->getOption(TR_DisableInliningUnrecognizedIntrinsics)) {
        logprintf(comp()->trace(OMR::inlining), comp()->log(),
            "@IntrinsicCandidate was specified for %s, in alwaysWorthInlining\n",
            calleeMethod->signature(comp()->trMemory()));
        return true;
    }
            ...
            if (node->getOpCode().isCall() && !visited.contains(node)) {
                ...
                if (node->getSymbolReference()->getSymbol()->getResolvedMethodSymbol()) {
                    TR_ResolvedMethod *resolvedMethod
                        = node->getSymbolReference()->getSymbol()->getResolvedMethodSymbol()->getResolvedMethod();
                    if (!node->isTheVirtualCallNodeForAGuardedInlinedCall()
                        && (comp->fej9()->isLambdaFormGeneratedMethod(resolvedMethod)
                            ...
                            || resolvedMethod->getRecognizedMethod()
                                == TR::java_lang_invoke_MethodHandle_invokeExact)) {
                        return true;
                    }
                }
            }
...
void TR_J9InlinerUtil::requestAdditionalOptimizations(TR_CallTarget *calltarget)
{
#if !defined(J9VM_OPT_OPENJDK_METHODHANDLE)
    if (calltarget->_myCallSite->getDepth()
            == -1 // only do this for top level callee to prevent exponential walk of inlined trees
        ...
        if (comp()->trace(OMR::inlining))
            heuristicTrace(tracer(),
                "Requesting one more pass of targeted inlining due to method handle invoke in %s\n",
                tracer()->traceSignature(calltarget->_calleeSymbol));
    }
#endif
}

void TR_J9InlinerUtil::adjustByteCodeSize(TR_ResolvedMethod *calleeResolvedMethod, bool isInLoop, TR::Block *block,
    int &bytecodeSize)
{
    ...
    if (isInLoop) {
        ...
        if (doit && calleeResolvedMethod && !strcmp(tmptmp, "Ljava/math/BigDecimal;")) {
            ...
            if ((isInLoop || (blockNestingDepth > 1)) && (bytecodeSize > 10)) {
                if (trace)
                    heuristicTrace(tracer(),
                        "Exceeds Size Threshold: Scaled down size for call block %d from %d to %d\n",
                        block->getNumber(), bytecodeSize, 10);
                bytecodeSize = 15;
            }
        } else
            heuristicTrace(tracer(),
                "Omitting Big Decimal method from size readjustment, calleeResolvedMethod = %p, tmptmp =%s",
                calleeResolvedMethod, tmptmp);
    }
}

TR::Node *TR_J9InlinerPolicy::genCompressedRefs(TR::Node *address, bool genTT, int32_t isLoad)
{
...
void TR_J9InlinerPolicy::createTempsForUnsafeCall(TR::TreeTop *callNodeTreeTop, TR::Node *unsafeCallNode)
{
    ...
    // Will go through all children of the unsafeCallNode and create a store tree that child, insert it before the call,
    // and then replace the call's child with a load of the symref.
    ...
    for (int32_t i = 0; i < unsafeCallNode->getNumChildren(); ++i) {
        ...
        TR::Node *storeNode
            = TR::Node::createWithSymRef(comp()->il.opCodeForDirectStore(dataType), 1, 1, child, newSymbolReference);
        ...
        TR::Node *value
            = TR::Node::createWithSymRef(child, comp()->il.opCodeForDirectLoad(dataType), 0, newSymbolReference);

        debugTrace(tracer(), "Replacing callnode %p child %p with %p", unsafeCallNode, unsafeCallNode->getChild(i),
            value);
...
void TR_J9InlinerPolicy::createTempsForUnsafePutGet(TR::Node *&unsafeAddress, TR::Node *unsafeCall,
    TR::TreeTop *callNodeTreeTop, TR::Node *&offset, TR::SymbolReference *&newSymbolReferenceForAddress,
    bool isUnsafeGet)
{
    ...
    TR::Node *storeNode = TR::Node::createWithSymRef(comp()->il.opCodeForDirectStore(unsafeAddress->getDataType()), 1,
        1, unsafeAddress, newSymbolReference);
    TR::TreeTop *storeTree = TR::TreeTop::create(comp(), storeNode);
    ...
    // Replace the old child with a load of the new sym ref
    unsafeAddress = TR::Node::createWithSymRef(unsafeAddress,
        comp()->il.opCodeForDirectLoad(unsafeAddress->getDataType()), 0, newSymbolReference);

    debugTrace(tracer(), "\tIn createTempsForUnsafePutGet. replacing unsafeCall ( %p) child %p with %p\n", unsafeCall,
        unsafeCall->getChild(1), unsafeAddress);
    ...
    storeNode = TR::Node::createWithSymRef(comp()->il.opCodeForDirectStore(offset->getDataType()), 1, 1, offset,
        newSymbolReference);
    ...
    // Replace the old child with a load of the new sym ref
    offset = TR::Node::createWithSymRef(offset, comp()->il.opCodeForDirectLoad(offset->getDataType()), 0,
        newSymbolReference);

    debugTrace(tracer(), "\tIn createTempsForUnsafePutGet. replacing unsafeCall ( %p) child %p with %p\n", unsafeCall,
        unsafeCall->getChild(2), offset);
    ...
    if (!isUnsafeGet) {
        ...
        newSymbolReference = comp()->getSymRefTab()->createTemporary(comp()->getMethodSymbol(), dataType);
        storeNode = TR::Node::createWithSymRef(comp()->il.opCodeForDirectStore(value->getDataType()), 1, 1, value,
            newSymbolReference);
        storeTree = TR::TreeTop::create(comp(), storeNode);
        callNodeTreeTop->insertTreeTopsBeforeMe(storeTree);

        // Replace the old child with a load of the new sym ref
        value = TR::Node::createWithSymRef(value, comp()->il.opCodeForDirectLoad(value->getDataType()), 0,
            newSymbolReference);
...
TR::TreeTop *TR_J9InlinerPolicy::genClassCheckForUnsafeGetPut(TR::Node *offset, bool branchIfLowTagged)
{
    ...
    TR::ILOpCodes op
        = branchIfLowTagged ? (isILoad ? TR::ificmpeq : TR::iflcmpeq) : (isILoad ? TR::ificmpne : TR::iflcmpne);
...
TR::TreeTop *TR_J9InlinerPolicy::genDirectAccessCodeForUnsafeGetPut(TR::Node *callNode, bool conversionNeeded,
    bool isUnsafeGet)
{
    ...
    TR_ASSERT(((firstChild->getOpCodeValue() == TR::aiadd) || (firstChild->getOpCodeValue() == TR::aladd)),
        "Unexpected opcode in unsafe access\n");
    ...
    // We need to dec the reference count otherwise a child of the callNode that ends up in the final trees will have an
    // extra refcount and will cause an assert
    if (conversionNeeded) {
...
TR::TreeTop *TR_J9InlinerPolicy::genIndirectAccessCodeForUnsafeGetPut(TR::Node *directAccessOrTempStoreNode,
    TR::Node *unsafeAddress)
{
    ...
    TR::Node *addressFirstChild = TR::Node::createWithSymRef(TR::aloadi, 1, 1, oldAddressFirstChild,
        comp()->getSymRefTab()->findOrCreateClassFromJavaLangClassSymbolRef());
    addressFirstChild = TR::Node::createWithSymRef(TR::aloadi, 1, 1, addressFirstChild,
    ...
    TR_ASSERT_FATAL(directSymbol->isUnsafeShadowSymbol(),
        "Expected to find an unsafe symbol for the get/put operation.");
    ...
    TR::SymbolReference *indirectSymRef = comp()->getSymRefTab()->findOrCreateUnsafeSymbolRef(
        directSymbol->getDataType(), true, true, directSymbol->getMemoryOrdering());
...
TR::Block *TR_J9InlinerPolicy::createUnsafeGetPutCallDiamond(TR::TreeTop *callNodeTreeTop, TR::TreeTop *comparisonTree,
    TR::TreeTop *branchTargetTree, TR::TreeTop *fallThroughTree)
{
    // Connect the trees/add blocks etc. properly and split the original block
    TR::Block *joinBlock = callNodeTreeTop->getEnclosingBlock()->createConditionalBlocksBeforeTree(callNodeTreeTop,
        comparisonTree, branchTargetTree, fallThroughTree, comp()->getFlowGraph(), false, false);
    return joinBlock;
}

void TR_J9InlinerPolicy::createAnchorNodesForUnsafeGetPut(TR::TreeTop *treeTop, TR::DataType type, bool isUnsafeGet)
{
    if (comp()->useCompressedPointers() && (type == TR::Address)) {
        ...
        TR::TreeTop *compRefTT
            = TR::TreeTop::create(comp(), genCompressedRefs(isUnsafeGet ? node->getFirstChild() : node, false));
...
void TR_J9InlinerPolicy::genCodeForUnsafeGetPut(TR::Node *unsafeAddress, TR::Node *unsafeOffset, TR::DataType type,
    TR::TreeTop *callNodeTreeTop, TR::TreeTop *prevTreeTop, TR::SymbolReference *newSymbolReferenceForAddress,
    TR::TreeTop *directAccessTreeTop, TR::TreeTop *arrayDirectAccessTreeTop, TR::TreeTop *indirectAccessTreeTop,
    TR::TreeTop *directAccessWithConversionTreeTop, bool needNullCheck, bool isUnsafeGet, bool conversionNeeded,
    bool arrayBlockNeeded, bool typeTestsNeeded, TR::Node *orderedCallNode = NULL)
{
    ...
    //     - blocks: arrayDirectAccess, directAccess, indirectAccess, directAccessWithConversion (if data element size <
    //     4)
    // 2.) typeTestsNeeded && !arrayBlockNeeded && javaLangClass != NULL
    //     (i.e.: object type is unknown at compile time AND gencon AND data element size >= 4 (int, long) AND able to
    //     get java/lang/Class)
    ...
    //     (i.e.: object type is unknown at compile time AND gencon AND data element size >= 4 (int, long) AND unable to
    //     get java/lang/Class)
    //     - tests: NULL test, Array test, Lowtag test
    //     - blocks: directAccess, indirectAccess
    ...
    //    *NOTE: Special Case*
    //     When conversionNeeded is true and offheap is not enabled, both the NULL and non-NULL cases need to be handled
    //     by direct access with conversion (which in the non-offheap case, is the same as array access). Thus, in this
    //     case:
    //     - test: none
    ...
    if (typeTestsNeeded || // CASES (1), (2), (3)
        (arrayBlockNeeded && (conversionNeeded == (bool)directAccessWithConversionTreeTop))
        || // CASE (4) excluding special case
        (!arrayBlockNeeded && conversionNeeded)) // CASE (5)
    {
        // Generate the tree for the null test
        TR::Node *addrLoad = TR::Node::createWithSymRef(unsafeAddress,
            comp()->il.opCodeForDirectLoad(unsafeAddress->getDataType()), 0, newSymbolReferenceForAddress);
        nullComparisonNode
            = TR::Node::createif(TR::ifacmpeq, addrLoad, TR::Node::create(addrLoad, TR::aconst, 0, 0), NULL);
    ...
    TR::TreeTop *lowTagCmpTree = typeTestsNeeded ? // CASES (1), (2), (3)
        genClassCheckForUnsafeGetPut(unsafeOffset, true)
                                                 : NULL;
    ...
    if (typeTestsNeeded && arrayBlockNeeded) // CASE (1)
    {
        ...
        branchTargetTree
            = directAccessWithConversionTreeTop ? directAccessWithConversionTreeTop : arrayDirectAccessTreeTop;
    ...
    } else if (!typeTestsNeeded && arrayBlockNeeded) // CASE (4)
    {
        ...
        if (!directAccessWithConversionTreeTop && conversionNeeded) {
            ...
            firstComparisonTree = nullComparisonTree;
            branchTargetTree
                = directAccessWithConversionTreeTop ? directAccessWithConversionTreeTop : directAccessTreeTop;
            fallThroughTree = arrayDirectAccessTreeTop;
        }
    ...
    {
        joinBlock
            = createUnsafeGetPutCallDiamond(callNodeTreeTop, firstComparisonTree, branchTargetTree, fallThroughTree);
        debugTrace(tracer(), "\t In genCodeForUnsafeGetPut, joinBlock is %d\n", joinBlock->getNumber());
    } else // CASE (6) and CASE (4) special case
    {
        joinBlock = callNodeTreeTop->getEnclosingBlock()->split(callNodeTreeTop, cfg, true);

        // remove original tree from joinBlock
    ...
    TR::Block *directAccessWithConversionBlock = NULL;
    ...
    if (typeTestsNeeded && arrayBlockNeeded) // CASE (1)
    {
        ...
        if (directAccessWithConversionTreeTop) {
            // get directAccessWithConversionBlock
            directAccessWithConversionBlock
                = firstComparisonTree->getNode()->getBranchDestination()->getNode()->getBlock();

            // generate arrayDirectAccessBlock
            arrayDirectAccessBlock
                = TR::Block::createEmptyBlock(lowTagCmpTree->getNode(), comp(), directAccessBlock->getFrequency());
            arrayDirectAccessBlock->append(arrayDirectAccessTreeTop);
            arrayDirectAccessBlock->append(TR::TreeTop::create(comp(),
                TR::Node::create(arrayDirectAccessTreeTop->getNode(), TR::Goto, 0, joinBlock->getEntry())));

            directAccessWithConversionBlock->getExit()->insertTreeTopsAfterMe(arrayDirectAccessBlock->getEntry(),
                arrayDirectAccessBlock->getExit());

            cfg->addNode(arrayDirectAccessBlock);
            cfg->addEdge(TR::CFGEdge::createEdge(arrayDirectAccessBlock, joinBlock, trMemory()));
        } else {
            // get arrayDirectAccessBlock
            ...
            nullComparisonTree->getNode()->setBranchDestination(
                conversionNeeded ? arrayDirectAccessBlock->getEntry() : directAccessBlock->getEntry());
        }
        ...
        indirectAccessBlock->append(TR::TreeTop::create(comp(),
            TR::Node::create(indirectAccessTreeTop->getNode(), TR::Goto, 0, joinBlock->getEntry())));
    ...
    } else if (!typeTestsNeeded && arrayBlockNeeded) // CASE (4)
    {
        // SPECIAL CASE
        if (!directAccessWithConversionTreeTop && conversionNeeded) {
            // Since no runtime tests are being performed, we can simply append the arrayDirectAccessTreeTop to
            // beforeCallBlock
    ...
    {
        directAccessWithConversionBlock = firstComparisonTree->getNode()->getBranchDestination()->getNode()->getBlock();
        directAccessBlock = beforeCallBlock->getNextBlock();
    } else // CASE (6)
    {
    ...
    if (directAccessWithConversionBlock)
        debugTrace(tracer(), "\t In genCodeForUnsafeGetPut, directAccessWithConversion is %d\n",
            directAccessWithConversionBlock->getNumber());
    ...
    if (directAccessWithConversionTreeTop)
        createAnchorNodesForUnsafeGetPut(directAccessWithConversionTreeTop, type, isUnsafeGet);

    TR::Node *objLoad = TR::Node::createWithSymRef(unsafeAddress,
        comp()->il.opCodeForDirectLoad(unsafeAddress->getDataType()), 0, newSymbolReferenceForAddress);
    TR::Node *vftLoad
        = TR::Node::createWithSymRef(TR::aloadi, 1, 1, objLoad, comp()->getSymRefTab()->findOrCreateVftSymbolRef());
    ...
    if (typeTestsNeeded && (arrayBlockNeeded || javaLangClass == NULL)) // CASES (1) and (3)
    {
        ...
        TR::Block *lowTagCmpBlock = TR::Block::createEmptyBlock(unsafeAddress, comp(),
            arrayBlockNeeded ? indirectAccessBlock->getFrequency() : directAccessBlock->getFrequency());
        ...
        TR::Node *isArrayNode
            = TR::Node::createif(TR::ificmpne, testIsArrayFlag, TR::Node::create(TR::iconst, 0), NULL);
        ...
        isArrayNode->setBranchDestination(
            arrayBlockNeeded ? arrayDirectAccessBlock->getEntry() : directAccessBlock->getEntry());

        lowTagCmpBlock->getEntry()->insertTreeTopsBeforeMe(isArrayBlock->getEntry(), isArrayBlock->getExit());
        cfg->addEdge(TR::CFGEdge::createEdge(isArrayBlock, lowTagCmpBlock, trMemory()));
        cfg->addEdge(TR::CFGEdge::createEdge(isArrayBlock,
            arrayBlockNeeded ? arrayDirectAccessBlock : directAccessBlock, trMemory()));
    ...
    {
        TR::Block *nullComparisonBlock
            = TR::Block::createEmptyBlock(objLoad, comp(), indirectAccessBlock->getFrequency());
        ...
        indirectAccessBlock->getEntry()->insertTreeTopsBeforeMe(nullComparisonBlock->getEntry(),
            nullComparisonBlock->getExit());
        ...
        // AOT Relocation relies on guards to locate correct method for a call site, while inlined Unsafe calls do not
        // have a guard. Therefore J9Class of java/lang/Class cannot be relocated correctly. Inserting any guard for an
        // inlined Unsafe call is potentially expensive, especially for applications that intensively use Unsafe, such
        // as Apache Spark. As a compromise, the node is given such BCI as if it is generated from the out-most call, so
        // that J9Class can be correctly relocated without any guard.
    ...
    if (typeTestsNeeded && arrayBlockNeeded)
        cfg->removeEdge(nullComparisonBlock, conversionNeeded ? directAccessBlock : arrayDirectAccessBlock);

    if (needNullCheck) {
        TR::TreeTop *treeBeforeCmp = firstComparisonTree ? firstComparisonTree->getPrevTreeTop()
                                                         : beforeCallBlock->getExit()->getPrevTreeTop();
        TR::TreeTop *nullchkTree = TR::TreeTop::create(comp(), treeBeforeCmp,
...
/*
When converting Unsafe.get/put* routines into inlined code there are three factors that
determine what the resulting IL trees will look like:
1) The size of the element to put/get
2) The GC policy being used
3) The type of the object that the data is being read from/written to

This means that when the type of the object is UNKNOWN at compile time, there are four
cases that need to be handled:
1) if offheap/balanced GC policy is enabled AND element size is 4 bytes or more (int, long)
2) if offheap/balanced GC policy is enabled AND element size is less than 4 bytes (boolean, byte, char, short)
3) if offheap/balanced GC policy is NOT enabled AND element size is 4 bytes or more (int, long)
4) if offheap/balanced GC policy is NOT enabled AND element size is less than 4 bytes (boolean, byte, char, short)

As well, in general, there are four alternatives on how to read from/write to the object: direct read/write,
indirect read/write, array read/write, and direct read/write with conversion. The selection of alternatives
is done by looking at three conditions:
...
Thus, the pseudocode for each of the cases outlined above is as follows:

1) offheap AND element size >= 4 bytes
...
2) offheap AND element size < 4 bytes
...
3) non-offheap AND element size >= 4 bytes

   if (offset is low-tagged)
     use indirect access
   else if (object == NULL)
     use direct access
   else if (offset is Class type)
     use indirect access
   else
     use direct access

4) non-offheap AND element size < 4 bytes
...
When the type of the object IS known at compile time, however, the only condition that needs to be
tested for is whether the object is NULL. In this scenario, there are two situations that need to
be handled:
a) The object is an array type
b) The object is some non-array, non-Class type

The resulting IL trees for each case in this situation are outlined below:

1) offheap AND element size >= 4 bytes
   a) object is array                        b) object is non-array

      if (object == NULL)                       use direct access (no runtime tests needed, since both NULL
        use direct access                                          and non-NULL cases use direct access)
      else
        use array access
...
2) offheap AND element size < 4 bytes
   a) object is array                        b) object is non-array
      ...
      if (object == NULL)                       if (object == NULL)
        use direct access with conversion         use direct access with conversion
      else                                      else
        use array access                          use direct access

3) non-offheap AND element size >= 4 bytes

   use direct access for both (a) and (b) (no runtime tests needed, since all possible cases use direct access)

4) non-offheap AND element size < 4 bytes
   a) object is array                        b) object is non-array

      use direct access with conversion         if (object == NULL)
      (no runtime tests needed, since both        use direct access with conversion
       NULL and non-NULL cases use direct       else
       access with conversion)                    use direct access


The functions used to construct the trees for all the cases outlined above are as follows:
...
bool TR_J9InlinerPolicy::createUnsafePutWithOffset(TR::ResolvedMethodSymbol *calleeSymbol,
    TR::ResolvedMethodSymbol *callerSymbol, TR::TreeTop *callNodeTreeTop, TR::Node *unsafeCall, TR::DataType type,
    TR::Symbol::MemoryOrdering ordering, bool needNullCheck, bool isUnaligned)
{
    if (ordering != TR::Symbol::MemoryOrdering::Transparent && type == TR::Int64 && comp()->target().is32Bit()
        && !comp()->cg()->getSupportsInlinedAtomicLongVolatiles())
        return false;
    ...
    debugTrace(tracer(),
        "\tcreateUnsafePutWithOffset.  call tree %p offset(datatype) %d ordering %s needNullCheck %d\n",
    ...
    const char *objTypeSig = unsafeCall->getChild(1)->getOpCode().hasSymbolReference()
        ? unsafeCall->getChild(1)->getSymbolReference()->getTypeSignature(length)
        : NULL;
    ...
    if (objTypeSig == NULL) {
        ...
        objClass = comp()->fe()->getClassFromSignature(objTypeSig, length,
            unsafeCall->getChild(1)->getSymbolReference()->getOwningMethod(comp()));
        bool isParameter = unsafeCall->getChild(1)->getSymbolReference()->getSymbol()->isParm();
    ...
    bool arrayBlockNeeded
        = (conversionNeeded || TR::Compiler->om.isOffHeapAllocationEnabled() && comp()->target().is64Bit())
    ...
    if (!disableIllegalWriteReport && !comp()->getOption(TR_DisableGuardedStaticFinalFieldFolding)) {
        reportFinalFieldModification = TR::TransformUtil::generateReportFinalFieldModificationCallTree(comp(),
            unsafeCall->getArgument(1)->duplicateTree());
    }
    ...
    if (conversionNeeded) {
        ...
        TR::Node *conversionNode = TR::Node::create(conversionOpCode, 1, valueWithoutConversion);
        ...
        unsafeNodeWithConversion = type == TR::Address && (TR::Compiler->om.writeBarrierType() != gc_modron_wrtbar_none)
            ? TR::Node::createWithSymRef(TR::awrtbari, 3, 3, unsafeAddress, valueWithConversion,
                  unsafeCall->getChild(1), symRef)
            : TR::Node::createWithSymRef(comp()->il.opCodeForIndirectArrayStore(type), 2, 2, unsafeAddress,
                  valueWithConversion, symRef);

        debugTrace(tracer(),
            "\tConversion is Needed, conversionNode = %p unsafeNodeWithConversion = %p valueWithConversion = %p\n",
    ...
    TR::Node *unsafeNode = type == TR::Address && (TR::Compiler->om.writeBarrierType() != gc_modron_wrtbar_none)
        ? TR::Node::createWithSymRef(TR::awrtbari, 3, 3, unsafeAddress, valueWithoutConversion, unsafeCall->getChild(1),
              symRef)
        : TR::Node::createWithSymRef(comp()->il.opCodeForIndirectStore(type), 2, 2, unsafeAddress,
              valueWithoutConversion, symRef);
    ...
    if (tracer()->debugLevel()) {
        ...
        debugTrace(tracer(),
            "\t After callNodeTreeTop setNode oldCallNodeTreeTop dump oldCallNodeTreeTop->getNode->getChild = %p:\n",
    ...
    // We need to generate a separate directAccessWithConversionBlock in the following cases:
    // 1.) arrayBlockNeeded && offheap enabled
    // 2.) !typeTestsNeeded && !arrayBlockNeeded
    // We can simply use the already existing arrayDirectAccessBlock as the directAccessWithConversionBlock in the
    // following cases: 3.) arrayBlockNeeded && gencon Otherwise, the directAccessWithConversionBlock is not needed
    TR::TreeTop *directAccessWithConversionTreeTop = NULL;

    if (conversionNeeded) {
        if ((arrayBlockNeeded && TR::Compiler->om.isOffHeapAllocationEnabled() && comp()->target().is64Bit())
            || (!typeTestsNeeded && !arrayBlockNeeded))
            directAccessWithConversionTreeTop
                = genDirectAccessCodeForUnsafeGetPut(unsafeNodeWithConversion, conversionNeeded, false);
    }
    ...
    if (indirectAccessTreeTop && indirectAccessTreeTop->getNode()
        && indirectAccessTreeTop->getNode()->getOpCode().isWrtBar()) {
    ...
    genCodeForUnsafeGetPut(unsafeAddress, offset, type, callNodeTreeTop, prevTreeTop, newSymbolReferenceForAddress,
        directAccessTreeTop, arrayDirectAccessTreeTop, indirectAccessTreeTop, directAccessWithConversionTreeTop,
        needNullCheck, false, conversionNeeded, arrayBlockNeeded, typeTestsNeeded, orderedCallNode);

    // Test for static final field
    if (reportFinalFieldModification && typeTestsNeeded) {
        ...
        auto isFinalStaticNode = TR::Node::createif(TR::iflcmpeq,
            ...
            TR::Node::lconst(J9_SUN_FINAL_FIELD_OFFSET_TAG), NULL /*branchTarget*/);
        auto isFinalStaticTreeTop = TR::TreeTop::create(comp(), isFinalStaticNode);

        TR::TransformUtil::createConditionalAlternatePath(comp(), isFinalStaticTreeTop, reportFinalFieldModification,
            storeToStaticFieldBlock, storeToStaticFieldBlock, comp()->getMethodSymbol()->getFlowGraph(),
            true /*markCold*/);

        debugTrace(tracer(),
            "Created isFinal test node n%dn whose branch target is Block_%d to report illegal write to static final "
            "field\n",
            isFinalStaticNode->getGlobalIndex(), reportFinalFieldModification->getEnclosingBlock()->getNumber());

        TR::DebugCounter::prependDebugCounter(comp(),
            TR::DebugCounter::debugCounterName(comp(), "illegalWriteReport/put/(%s %s)", comp()->signature(),
...
TR::Node *TR_J9InlinerPolicy::createUnsafeMonitorOp(TR::ResolvedMethodSymbol *calleeSymbol,
    TR::ResolvedMethodSymbol *callerSymbol, TR::TreeTop *callNodeTreeTop, TR::Node *unsafeCall, bool isEnter)
{
    ...
    if (isDirectJNI && unsafeCall->getChild(1)->getOpCodeValue() != TR::loadaddr) {
        logprintf(comp()->trace(OMR::inlining), comp()->log(),
            "Unsafe Inlining: The Unsafe.monitorEnter/Exit() children are not loadaddr's as expected. Not inlining.\n");
        return unsafeCall;
    }

    TR::Node::recreate(callNodeTreeTop->getNode(), TR::NULLCHK);
    callNodeTreeTop->getNode()->setSymbolReference(
        comp()->getSymRefTab()->findOrCreateNullCheckSymbolRef(comp()->getMethodSymbol()));

    if (isEnter) {
        TR::Node::recreate(unsafeCall, TR::monent);
        TR::SymbolReference *monitorEnterSymbolRef
            = comp()->getSymRefTab()->findOrCreateMonitorEntrySymbolRef(comp()->getMethodSymbol());
        ...
        TR::SymbolReference *monitorExitSymbolRef
            = comp()->getSymRefTab()->findOrCreateMonitorExitSymbolRef(comp()->getMethodSymbol());
...
bool TR_J9InlinerPolicy::createUnsafeCASCallDiamond(TR::TreeTop *callNodeTreeTop, TR::Node *callNode)
{
    // This method is used to create an if diamond around a call to any of the unsafe compare and swap methods
    // Codegens have a fast path for the compare and swaps, but cannot deal with the case where the offset value passed
    // in to a the CAS is low tagged (A low tagged offset value means the object being passed in is a java/lang/Class
    // object, and we want a static field)
    ...
    //     balanced GC policy with offheap allocation enabled if the object being operated on is known NOT to be an
    //     array at compile time.
    // 2.) No type checks are generated. This will occur under balanced GC policy with offheap allocation enabled if the
    // object
    //     being operated on is known to be an array at compile time (since if the object is an array, it can't also be
    //     a java/lang/Class object).
    // 3.) Both the array and low tagged checks are generated. This will occur under balanced GC policy with offheap
    // allocation
    //     enabled if the type of the object being operated on is unknown at compile time.
    //
    // In addition to type checks, a NULL check on the object address is needed to ensure that we do not try to load
    // dataAddr from a NULL reference. This is only a concern when offheap is enabled AND there is a possibility that
    // the object is an array. so the NULL check will only be generated in cases (2) and (3).
    ...
    int length;
    const char *objTypeSig = callNode->getChild(1)->getOpCode().hasSymbolReference()
        ? callNode->getChild(1)->getSymbolReference()->getTypeSignature(length)
        : NULL;
    ...
    if (objTypeSig == NULL) {
        ...
        TR_OpaqueClassBlock *objClass = comp()->fe()->getClassFromSignature(objTypeSig, length,
            callNode->getChild(1)->getSymbolReference()->getOwningMethod(comp()));
    ...
    bool arrayTestNeeded
        = TR::Compiler->om.isOffHeapAllocationEnabled() && comp()->target().is64Bit() && objTypeUnknown;
    ...
    if (arrayTestNeeded) {
        ...
        TR::Node *vftLoad = TR::Node::createWithSymRef(TR::aloadi, 1, 1,
            TR::Node::createWithSymRef(unsafeAddress, comp()->il.opCodeForDirectLoad(unsafeAddress->getDataType()), 0,
                newSymbolReferenceForAddress),
            comp()->getSymRefTab()->findOrCreateVftSymbolRef());

        TR::Node *andNode = comp()->fej9()->testIsClassArrayType(vftLoad);
    ...
    if (arrayBlockNeeded) {
        ...
        TR::Node *isNullNode
            = TR::Node::createif(TR::ifacmpeq, objAddr, TR::Node::create(objAddr, TR::aconst, 0, 0), NULL);
    ...
    if (callNode->getReferenceCount() > 1) {
        ...
        debugTrace(tracer(), "Unsafe call has refcount > 1.  Replacing callnode with a load of symref %d",
            newSymbolReference->getReferenceNumber());
    }

    TR::Block *callBlock = callNodeTreeTop->getEnclosingBlock();

    if (arrayTestNeeded) // in case (3), we generate the null test diamond, then the array test diamond, and then the
                         // low tagged test diamond
    {
        TR::TreeTop *nonNullAccessTreeTop = TR::TreeTop::create(comp(), callNodeTreeTop->getNode()->duplicateTree());

        // add null test and array test
        callBlock->createConditionalBlocksBeforeTree(callNodeTreeTop, isNullTreeTop, defaultAccessTreeTop,
            nonNullAccessTreeTop, comp()->getFlowGraph(), false, false);
        TR::Block *joinBlock
            = nonNullAccessTreeTop->getEnclosingBlock()->createConditionalBlocksBeforeTree(nonNullAccessTreeTop,
                isArrayTreeTop, arrayAccessTreeTop, compareTree, comp()->getFlowGraph(), false, false);
        ...
        TR::Block *lowTagAccessBlock = TR::Block::createEmptyBlock(compareTree->getNode(), comp(),
            isLowTaggedBlock->getFrequency(), isLowTaggedBlock);
    ...
    {
        callBlock->createConditionalBlocksBeforeTree(callNodeTreeTop, isNullTreeTop, defaultAccessTreeTop,
            arrayAccessTreeTop, comp()->getFlowGraph(), false, false);
    } else if (compareTree != NULL) // in case (1), we only generate the low tagged test diamond
    {
        callBlock->createConditionalBlocksBeforeTree(callNodeTreeTop, compareTree, defaultAccessTreeTop,
            lowTagAccessTreeTop, comp()->getFlowGraph(), false, false);
    }
    ...
    if (newSymbolReference) {
        TR::Node *defaultAccessStoreNode = TR::Node::createWithSymRef(comp()->il.opCodeForDirectStore(dataType), 1, 1,
            defaultAccessTreeTop->getNode()->getFirstChild(), newSymbolReference);
        ...
        debugTrace(tracer(), "Inserted store tree %p for branch target (taken) side of the diamond",
            defaultAccessStoreNode);

        if (compareTree != NULL) // cases (1) and (3) only
        {
            TR::Node *lowTagAccessStoreNode = TR::Node::createWithSymRef(comp()->il.opCodeForDirectStore(dataType), 1,
                1, lowTagAccessTreeTop->getNode()->getFirstChild(), newSymbolReference);
        ...
        {
            TR::Node *arrayAccessStoreNode = TR::Node::createWithSymRef(comp()->il.opCodeForDirectStore(dataType), 1, 1,
                arrayAccessTreeTop->getNode()->getFirstChild(), newSymbolReference);
...
bool TR_J9InlinerPolicy::createUnsafeGetWithOffset(TR::ResolvedMethodSymbol *calleeSymbol,
    TR::ResolvedMethodSymbol *callerSymbol, TR::TreeTop *callNodeTreeTop, TR::Node *unsafeCall, TR::DataType type,
    TR::Symbol::MemoryOrdering ordering, bool needNullCheck, bool isUnaligned)
{
    if (ordering != TR::Symbol::MemoryOrdering::Transparent && type == TR::Int64 && comp()->target().is32Bit()
    ...
    int length;
    const char *objTypeSig = unsafeAddress->getOpCode().hasSymbolReference()
        ? unsafeAddress->getSymbolReference()->getTypeSignature(length)
        : NULL;
    ...
    if (objTypeSig == NULL) {
        ...
        objClass = comp()->fe()->getClassFromSignature(objTypeSig, length,
            unsafeAddress->getSymbolReference()->getOwningMethod(comp()));
        bool isParameter = unsafeAddress->getSymbolReference()->getSymbol()->isParm();
    ...
    //
    createTempsForUnsafePutGet(unsafeAddress, unsafeCall, callNodeTreeTop, offset, newSymbolReferenceForAddress, true);
    ...
    if (conversionNeeded) {
        TR::Node *loadNode
            = TR::Node::createWithSymRef(comp()->il.opCodeForIndirectArrayLoad(type), 1, 1, unsafeAddress, symRef);
        ...
        switch (calleeSymbol->getRecognizedMethod()) {
            ...
            default:
                TR_ASSERT_FATAL(false,
                    "all TR::sun_misc_Unsafe_get* and  TR::jdk_internal_misc_Unsafe_get* methods must be handled.");
        }

        TR::ILOpCodes conversionOpCode
            = TR::ILOpCode::getProperConversion(type, comp()->fe()->dataTypeForLoadOrStore(type), unsignedType);
    ...
    if (conversionNeeded) {
        callNodeWithConversion = TR::Node::createWithSymRef(storeOpCode, 1, 1, unsafeCallWithConversion, newTemp);
    }

    TR::TreeTop *directAccessTreeTop = genDirectAccessCodeForUnsafeGetPut(callNodeTreeTop->getNode(), false, true);
    ...
    genCodeForUnsafeGetPut(unsafeAddress, offset, type, callNodeTreeTop, prevTreeTop, newSymbolReferenceForAddress,
        directAccessTreeTop, arrayDirectAccessTreeTop, indirectAccessTreeTop, directAccessWithConversionTreeTop,
        needNullCheck, true, conversionNeeded, arrayBlockNeeded, typeTestsNeeded);
...
TR::Node *TR_J9InlinerPolicy::createUnsafeAddress(TR::Node *unsafeCall)
{
    if (comp()->target().is64Bit())
...
bool TR_J9InlinerPolicy::createUnsafePut(TR::ResolvedMethodSymbol *calleeSymbol, TR::ResolvedMethodSymbol *callerSymbol,
    TR::TreeTop *callNodeTreeTop, TR::Node *unsafeCall, TR::DataType type, bool compress)
{
    // Preserve null check on the unsafe object
    TR::TransformUtil::separateNullCheck(comp(), callNodeTreeTop, tracer()->debugLevel());
    ...
    if (type == TR::Address) {
        if (comp()->target().is64Bit()) {
            unsafeNode = TR::Node::createWithSymRef(TR::lstorei, 2, 2, address, value,
                comp()->getSymRefTab()->findOrCreateUnsafeSymbolRef(TR::Int64));
        } else {
            unsafeNode = TR::Node::create(TR::l2i, 1, value);
            unsafeNode = TR::Node::createWithSymRef(TR::istorei, 2, 2, address, unsafeNode,
                comp()->getSymRefTab()->findOrCreateUnsafeSymbolRef(TR::Int32));
        }
    } else {
        switch (type) {
            case TR::Int8:
        ...
        unsafeNode = TR::Node::createWithSymRef(comp()->il.opCodeForIndirectArrayStore(type), 2, 2, address, value,
            comp()->getSymRefTab()->findOrCreateUnsafeSymbolRef(type));
    }

    if (compress && comp()->useCompressedPointers() && (type == TR::Address)) {
...
bool TR_J9InlinerPolicy::createUnsafeGet(TR::ResolvedMethodSymbol *calleeSymbol, TR::ResolvedMethodSymbol *callerSymbol,
    TR::TreeTop *callNodeTreeTop, TR::Node *unsafeCall, TR::DataType type, bool compress)
{
    // Preserve null check on the unsafe object
    TR::TransformUtil::separateNullCheck(comp(), callNodeTreeTop, tracer()->debugLevel());
    ...
    if (type == TR::Address) {
        if (comp()->target().is64Bit()) {
            ...
            unsafeNode = TR::Node::createWithSymRef(TR::iloadi, 1, 1, unsafeAddress,
                comp()->getSymRefTab()->findOrCreateUnsafeSymbolRef(TR::Int32));
        ...
        if (needConversion)
            unsafeNode = TR::Node::createWithSymRef(comp()->il.opCodeForIndirectArrayLoad(type), 1, 1, unsafeAddress,
                comp()->getSymRefTab()->findOrCreateUnsafeSymbolRef(type));
        else
...
TR::Node *TR_J9InlinerPolicy::inlineGetClassAccessFlags(TR::ResolvedMethodSymbol *calleeSymbol,
    TR::ResolvedMethodSymbol *callerSymbol, TR::TreeTop *callNodeTreeTop, TR::Node *callNode)
{
    ...
    TR::SymbolReference *modifiersSymRef
        = comp()->getSymRefTab()->createTemporary(callerSymbol, callNode->getDataType());
    ...
    j9cNode = TR::Node::createWithSymRef(TR::aloadi, 1, 1, j9cNode,
        comp()->getSymRefTab()->findOrCreateClassFromJavaLangClassSymbolRef());

    TR::Node *nullCheckNode = TR::Node::createWithSymRef(TR::NULLCHK, 1, 1, j9cNode,
        comp()->getSymRefTab()->findOrCreateNullCheckSymbolRef(callerSymbol));
    TR::TreeTop *nullCheckTree = TR::TreeTop::create(comp(), nullCheckNode);
    TR::Node *romclassNode = TR::Node::createWithSymRef(TR::aloadi, 1, 1, j9cNode,
        comp()->getSymRefTab()->findOrCreateClassRomPtrSymbolRef());
    TR::Node *classAccessFlagsNode = TR::Node::createWithSymRef(TR::iloadi, 1, 1, romclassNode,
    ...
    TR::Node *iAndNode = TR::Node::create(TR::iand, 2, TR::Node::createLoad(callNode, modifiersSymRef),
        TR::Node::iconst(callNode, (int32_t)comp()->fej9()->constClassFlagsPrimitive()));
    TR::Node *compareNode = TR::Node::createif(TR::ificmpne, iAndNode, TR::Node::iconst(callNode, 0), 0);
    ...
    TR::Node *modifiersBranchTargStrNode = TR::Node::createStore(modifiersSymRef,
        TR::Node::iconst(callNode,
            (int32_t)(comp()->fej9()->constClassFlagsAbstract() | comp()->fej9()->constClassFlagsFinal()
    ...
    TR::Node *modifiersIAndNode = TR::Node::create(TR::iand, 2, TR::Node::createLoad(callNode, modifiersSymRef),
    ...
    block->createConditionalBlocksBeforeTree(callNodeTreeTop, compareTree, branchTargetTree, fallThroughTree,
        callerSymbol->getFlowGraph(), false);

    return resultNode;
}

bool TR_J9InlinerPolicy::inlineUnsafeCall(TR::ResolvedMethodSymbol *calleeSymbol,
    TR::ResolvedMethodSymbol *callerSymbol, TR::TreeTop *callNodeTreeTop, TR::Node *callNode)
{
    ...
    if ((TR::Compiler->vm.canAnyMethodEventsBeHooked(comp())
            && !comp()->fej9()->methodsCanBeInlinedEvenIfEventHooksEnabled(comp()))
    ...
    // I am not sure if having the same type between C/S and B/Z matters here.. ie. if the type is being used as the
    ...
    switch (callNode->getSymbol()->castToResolvedMethodSymbol()->getRecognizedMethod()) {
        ...
        case TR::sun_misc_Unsafe_putObject_jlObjectJjlObject_V:
            return createUnsafePutWithOffset(calleeSymbol, callerSymbol, callNodeTreeTop, callNode, TR::Address,
                TR::Symbol::MemoryOrdering::Transparent, true);
        ...
        case TR::sun_misc_Unsafe_getObject_jlObjectJ_jlObject:
            return createUnsafeGetWithOffset(calleeSymbol, callerSymbol, callNodeTreeTop, callNode, TR::Address,
                TR::Symbol::MemoryOrdering::Transparent, true);

        case TR::sun_misc_Unsafe_putByteVolatile_jlObjectJB_V:
            return createUnsafePutWithOffset(calleeSymbol, callerSymbol, callNodeTreeTop, callNode, TR::Int8,
                TR::Symbol::MemoryOrdering::Volatile);
        ...
        case TR::sun_misc_Unsafe_putObjectVolatile_jlObjectJjlObject_V:
            return createUnsafePutWithOffset(calleeSymbol, callerSymbol, callNodeTreeTop, callNode, TR::Address,
                TR::Symbol::MemoryOrdering::Volatile, true);
        ...
        case TR::sun_misc_Unsafe_putByteOrdered_jlObjectJB_V:
            return createUnsafePutWithOffset(calleeSymbol, callerSymbol, callNodeTreeTop, callNode, TR::Int8,
                TR::Symbol::MemoryOrdering::AcquireRelease);
        ...
        case TR::sun_misc_Unsafe_putObjectOrdered_jlObjectJjlObject_V:
            return createUnsafePutWithOffset(calleeSymbol, callerSymbol, callNodeTreeTop, callNode, TR::Address,
                TR::Symbol::MemoryOrdering::AcquireRelease, true);

        // FIXME: Update createUnsafePutWithOffset signature to have isVolatile, isOrdered, isUnaligned as enum
        case TR::jdk_internal_misc_Unsafe_getCharUnaligned:
            return createUnsafeGetWithOffset(calleeSymbol, callerSymbol, callNodeTreeTop, callNode, TR::Int16,
                TR::Symbol::MemoryOrdering::Transparent, /*needsNullCheck*/ false, /*isUnaligned*/ true);
        case TR::jdk_internal_misc_Unsafe_getShortUnaligned:
            return createUnsafeGetWithOffset(calleeSymbol, callerSymbol, callNodeTreeTop, callNode, TR::Int16,
                TR::Symbol::MemoryOrdering::Transparent, /*needsNullCheck*/ false, /*isUnaligned*/ true);
        ...
        case TR::jdk_internal_misc_Unsafe_getBooleanOpaque_jlObjectJ_Z:
            return createUnsafeGetWithOffset(calleeSymbol, callerSymbol, callNodeTreeTop, callNode, TR::Int8,
                TR::Symbol::MemoryOrdering::Opaque);
        case TR::jdk_internal_misc_Unsafe_getByteOpaque_jlObjectJ_B:
            return createUnsafeGetWithOffset(calleeSymbol, callerSymbol, callNodeTreeTop, callNode, TR::Int8,
                TR::Symbol::MemoryOrdering::Opaque);
        case TR::jdk_internal_misc_Unsafe_getCharOpaque_jlObjectJ_C:
        ...
        case TR::jdk_internal_misc_Unsafe_getReferenceOpaque_jlObjectJ_jlObject:
            return createUnsafeGetWithOffset(calleeSymbol, callerSymbol, callNodeTreeTop, callNode, TR::Address,
                TR::Symbol::MemoryOrdering::Opaque, true);
...
bool TR_J9InlinerPolicy::isInlineableJNI(TR_ResolvedMethod *method, TR::Node *callNode)
{
...
bool TR_J9InlinerPolicy::tryToInline(TR_CallTarget *calltarget, TR_CallStack *callStack, bool toInline)
{
    bool trace = comp()->trace(OMR::inlining);
    OMR::Logger *log = comp()->log();
    ...
    if (toInline) {
        if (!comp()->getOption(TR_DisableForceInlineAnnotations) && comp()->fej9()->isForceInline(method)) {
            logprintf(trace, log, "@ForceInline was specified for %s, in tryToInline\n",
                method->signature(comp()->trMemory()));
            return true;
        }

        if (method->getRecognizedMethod() == TR::unknownMethod && comp()->fej9()->isIntrinsicCandidate(method)
            && !comp()->getOption(TR_DisableInliningUnrecognizedIntrinsics)) {
        ...
        static const bool disable = feGetEnv("TR_disableDontInlineAnnotations") != NULL;
...
bool TR_J9InlinerPolicy::inlineMethodEvenForColdBlocks(TR_ResolvedMethod *method)
{
    bool insideForEach = insideIntPipelineForEach(method, comp());
    return insideForEach;
}

void TR_J9InlinerPolicy::adjustFanInSizeInWeighCallSite(int32_t &weight, int32_t size, TR_ResolvedMethod *callee,
    TR_ResolvedMethod *caller, int32_t bcIndex)
{
    /*
    Our goal is to use the ratio of the weight of a particular caller to the total weight to penalize the callers whose
    weights are relatively small. To reach that goal, we have to introduce two magic numbers: defaultWeight and
    TR::Options::INLINE_fanInCallGraphFactor. *defaultWeight is used when our caller belongs in the other bucket, so we
    don't have a meaningful weight to represent it. *INLINE_fanInCallGraphFactor is simply hand-tuned number by which we
    multiply our ratio.

    INLINE_fanInCallGraphFactor is an integer number divided by 100. This allows us to avoid using float numbers for
    ...
    */
    ...
    uint32_t thresholdSize
        = (!comp()->getOption(TR_InlinerFanInUseCalculatedSize)) ? getJ9InitialBytecodeSize(callee, 0, comp()) : size;
    if (thresholdSize <= TR::Options::_iprofilerFaninMethodMinSize) // if we are less than min_fan_in size, we don't
                                                                    // want to apply fan-in heuristic
    {
        return;
    }
    ...
    if (numCallers < MIN_NUM_CALLERS
        || (totalWeight > 0 && otherBucketWeight * 1.0 / totalWeight < otherBucketThreshold))
        return;
    ...
    if (size >= 0 && totalWeight && fanInWeight) {
        ...
        float dynamicFanInRatio = hasCaller ? ((float)totalWeight - (float)fanInWeight) / (float)totalWeight
                                            : (float)fanInWeight / (float)totalWeight;
        ...
        heuristicTrace(tracer(), "FANIN: callee %s in caller %s @ %d oldWeight %d weight %d",
            callee->signature(comp()->trMemory()), caller->signature(comp()->trMemory()), bcIndex, oldWeight, weight);
    }
}

bool TR_J9InlinerPolicy::_tryToGenerateILForMethod(TR::ResolvedMethodSymbol *calleeSymbol,
    TR::ResolvedMethodSymbol *callerSymbol, TR_CallTarget *calltarget)
{
    OMR::Logger *log = comp()->log();
    bool success = false;
    ...
    TR::IlGeneratorMethodDetails &ilGenMethodDetails
        = TR::IlGeneratorMethodDetails::create(storage, calleeSymbol->getResolvedMethod());
...
bool TR_J9InlinerPolicy::tryToInlineTrivialMethod(TR_CallStack *callStack, TR_CallTarget *calltarget)
{
    ...
    if (!performTransformation(comp(), "%sInlining jni %s into %s\n", OPT_DETAILS,
            calleeSymbol->signature(comp()->trMemory()), callerSymbol->signature(comp()->trMemory()))) {
        return false;
    }
...
bool TR_J9InlinerPolicy::trivialInliningOnly(TR_CallStack *callStack, TR_CallTarget *callTarget)
{
    TR::Node *callNode = callTarget->_myCallSite->_callNode;
    TR::ResolvedMethodSymbol *calleeSymbol = callTarget->_calleeSymbol;
...
bool TR_J9InlinerPolicy::adjustFanInSizeInExceedsSizeThreshold(int bytecodeSize, uint32_t &calculatedSize,
    TR_ResolvedMethod *callee, TR_ResolvedMethod *caller, int32_t bcIndex)
{
    ...
    uint32_t thresholdSize = (!comp()->getOption(TR_InlinerFanInUseCalculatedSize))
        ? getJ9InitialBytecodeSize(callee, 0, comp())
        : calculatedSize;
    if (thresholdSize <= TR::Options::_iprofilerFaninMethodMinSize) // if we are less than min_fan_in size, we don't
                                                                    // want to apply fan-in heuristic
    {
    ...
    {
        heuristicTrace(tracer(),
            "FANIN: callee %s in caller %s @ %d exceeds thresholds due to the caller being in the other bucket",
            callee->signature(comp()->trMemory()), caller->signature(comp()->trMemory()), bcIndex);
    ...
    heuristicTrace(tracer(), "FANIN: callee %s in caller %s @ %d oldCalculatedSize %d calculatedSize %d",
        callee->signature(comp()->trMemory()), caller->signature(comp()->trMemory()), bcIndex, oldCalculatedSize,
        calculatedSize);

    return false;
}

bool TR_J9InlinerPolicy::callMustBeInlinedInCold(TR_ResolvedMethod *method)
{
    bool trace = comp()->trace(OMR::inlining);
    OMR::Logger *log = comp()->log();
...
bool TR_J9InlinerPolicy::callMustBeInlined(TR_CallTarget *calltarget)
{
    TR_ResolvedMethod *method = calltarget->_calleeMethod;
...
void TR_J9InlinerUtil::adjustCallerWeightLimit(TR::ResolvedMethodSymbol *callerSymbol, int &callerWeightLimit)
{
    if (inliner()->getPolicy()->aggressiveSmallAppOpts()
        && (callerSymbol->getRecognizedMethod() == TR::java_util_GregorianCalendar_computeFields) && isHot(comp()))
        callerWeightLimit = 2600;
}

void TR_J9InlinerUtil::adjustMethodByteCodeSizeThreshold(TR::ResolvedMethodSymbol *callerSymbol,
    int &methodByteCodeSizeThreshold)
{
    if (inliner()->getPolicy()->aggressiveSmallAppOpts()
        && (callerSymbol->getRecognizedMethod() == TR::java_util_GregorianCalendar_computeFields))
        methodByteCodeSizeThreshold = 400;
}

bool TR_J9InlinerPolicy::willBeInlinedInCodeGen(TR::RecognizedMethod method)
{
...
bool TR_J9InlinerPolicy::skipHCRGuardForCallee(TR_ResolvedMethod *callee)
{
    ...
    if (rm >= TR::FirstVectorMethod && rm <= TR::LastVectorIntrinsicMethod)
        return true;
...
const char *TR_Inliner::optDetailString() const throw() { return "O^O INLINER: "; }

void TR_MultipleCallTargetInliner::recursivelyWalkCallTargetAndGenerateNodeEstimate(TR_CallTarget *ct,
    NodeEstimate &estimate)
{
    debugTrace(tracer(),
        "recursivelyWalkCallTargetAndGenerateNodeEstimate: Considering Target %p. node estimate before = %d maxbcindex "
        "= %d",
        ct, estimate.getNodeEstimate(), getPolicy()->getInitialBytecodeSize(ct->_calleeMethod, 0, comp()));
...
int32_t TR_MultipleCallTargetInliner::applyArgumentHeuristics(TR_LinkHead<TR_ParameterMapping> &map,
    int32_t originalWeight, TR_CallTarget *target)
{
    ...
    for (TR_ParameterMapping *parm = map.getFirst(); parm; parm = parm->getNext()) {
        if (parm->_parameterNode->getOpCode().isLoadConst()) {
        ...
        } else if (parm->_parameterNode->getOpCodeValue() == TR::aload
            && parm->_parameterNode->getSymbolReference()->getSymbol()->isConstObjectRef()) {
            weight = weight * (fraction - 1) / fraction;
            heuristicTrace(tracer(), "Setting weight to %d because arg is const object reference.", weight);
        } else if (!disableCCI && (parm->_parameterNode->getOpCodeValue() == TR::aloadi)
            && (parm->_parameterNode->getSymbolReference()
                == comp()->getSymRefTab()->findJavaLangClassFromClassSymbolRef())) {
            ...
            weight = comp()->fej9()->adjustedInliningWeightBasedOnArgument(weight, parm->_parameterNode,
                parm->_parmSymbol, comp());
...
bool TR_MultipleCallTargetInliner::eliminateTailRecursion(TR::ResolvedMethodSymbol *calleeSymbol,
    ...
    TR_VirtualGuardSelection *guard)
{
    ...
    if (nextTT->getNode()->getOpCodeValue() != TR::Return && nextTT->getNode()->getFirstChild() != callNode) {
        ...
        TR::Block *otherBranch = toBlock(conditionBlock->getSuccessors().front()->getTo() == block
                ? (*(++conditionBlock->getSuccessors().begin()))->getTo()
                : conditionBlock->getSuccessors().front()->getTo());
            ...
            TR::TreeTop::create(comp(), branchTargetTreeTop->getPrevTreeTop(),
                TR::Node::create(TR::ireturn, 1,
                    TR::Node::create(TR::idiv, 2,
                        TR::Node::create(TR::imul, 2, arithmeticNode,
                            TR::Node::create(TR::iadd, 2, arithmeticNode,
        ...
        TR::TreeTop::create(comp(), callNodeTreeTop->getPrevTreeTop(),
            TR::Node::createStore(temp,
                TR::Node::create(TR::iadd, 2, TR::Node::createLoad(returnNode, temp), arithmeticNode)));
    }
    ...
    if (!comp()->incInlineDepth(calleeSymbol, callNode, !callNode->getOpCode().isCallIndirect(), guard,
            calleeResolvedMethod->classOfMethod(), 0)) {
        return false;
    }
    ...
    if (guard->_kind != TR_NoGuard) {
        ...
        //       block->append(TR::TreeTop::create(comp(), createVirtualGuard(callNode,
        //       callNode->getSymbol()->castToResolvedMethodSymbol(), branchDestination->getEntry(), false, (void
        //       *)calleeResolvedMethod->classOfMethod(), false)));
        ...
        TR::Node *vguardNode = createVirtualGuard(callNode, callNode->getSymbol()->castToResolvedMethodSymbol(),
            block2->getEntry(),
        ...
        TR_ASSERT((block->getSuccessors().size() == 1),
            "eliminateTailRecursion, block with call does not have exactly 1 successor");
        ...
        TR_ASSERT(block->getLastRealTreeTop() == callNodeTreeTop,
            "eliminateTailRecursion call isn't last or second last tree in block");
    }

    if (comp()->getProfilingMode() == JitProfiling) {
        TR::Node *asyncNode = TR::Node::createWithSymRef(callNode, TR::asynccheck, 0,
    ...
    TR::TreeTop *prevTreeTop, TR::Node *callNode)
{
    ...
    for (TR::ParameterSymbol *p = parms.getFirst(); p; ++i, p = parms.getNext()) {
        TR::SymbolReference *sr
            = comp()->getSymRefTab()->findOrCreateAutoSymbol(calleeSymbol, p->getSlot(), p->getDataType(), true);
        TR::Node *arg = callNode->getChild(i);
        if (arg->getReferenceCount() != 1 || !arg->getOpCode().hasSymbolReference()
            || arg->getSymbolReference() != sr) {
...
bool TR_MultipleCallTargetInliner::inlineCallTargets(TR::ResolvedMethodSymbol *callerSymbol,
    TR_CallStack *prevCallStack, TR_InnerPreexistenceInfo *innerPrexInfo)
{
    OMR::Logger *log = comp()->log();
            ...
            if (parent->getOpCodeValue() == TR::BBStart) {
                ...
                if (node->getOpCode().isFunctionCall() && node->getVisitCount() != _visitCount) {
                    ...
                    if (sym->isResolvedMethod()) {
                        ...
                        forceInlineInCold = j9inlinerPolicy->callMustBeInlinedInCold(
                            sym->getResolvedMethodSymbol()->getResolvedMethod());
                    }

                    if ((!isCold || forceInlineInCold) && !node->isTheVirtualCallNodeForAGuardedInlinedCall()) {
                        TR::SymbolReference *symRef = node->getSymbolReference();
                        TR::MethodSymbol *calleeSymbol = symRef->getSymbol()->castToMethodSymbol();

                        TR_CallSite *callsite = TR_CallSite::create(tt, parent, node, 0, symRef, (TR_ResolvedMethod *)0,
                            comp(), trMemory(), stackAlloc);

                        if (prevCallStack == 0) {
                            heuristicTrace(tracer(), "\n");
                            heuristicTrace(tracer(),
                                "^^^ Top Level: Analysing Call at call node %p . Creating callsite %p to encapsulate "
                                "call.",
                                node, callsite);
                        }
                        ...
                        if (!prevCallStack && callsite->numTargets() > 0) {
                            ...
                            TR::ResolvedMethodSymbol *thisCallSiteCallerSymbol
                                = node->getSymbolReference()->getOwningMethodSymbol(comp());
                            TR_PrexArgInfo *compArgInfo
                                = TR_PrexArgInfo::buildPrexArgInfoForMethodSymbol(thisCallSiteCallerSymbol, tracer());
                            ...
                            TR_PrexArgInfo::propagateArgsFromCaller(thisCallSiteCallerSymbol, callsite, compArgInfo,
                                tracer());
                            if (tracer()->heuristicLevel()) {
                        ...
                        heuristicTrace(tracer(), "Searching for Targets returned %d targets for call at node %p. ",
                            callsite->numTargets(), node);

                        if (callsite->numTargets()) {
                            bool flag = false;
                            for (int32_t i = 0; i < callsite->numTargets() && flag == false; i++) {
                                TR_CallStack *cs
                                    = callStack.isCurrentlyOnTheStack(callsite->getTarget(i)->_calleeMethod, 1);
                                if (cs && callsite->getTarget(i)->_calleeSymbol == node->getSymbol()
                                    && eliminateTailRecursion(cs->_methodSymbol, &callStack, callsite->_callNodeTreeTop,
                                        callsite->_parent, callsite->_callNode, callsite->getTarget(i)->_guard)) {
                                    anySuccess2 = true;
                                    flag = true;
                                }
                            }
                            ...
                            if (!flag) {
                                if (prevCallStack == 0) // we only weigh base level calls.. All other calls we proceed
                                                        // right to inlining
                                {
                        ...
                        TR::SymbolReference *symRef = node->getSymbolReference();
                        TR::MethodSymbol *calleeSymbol = symRef->getSymbol()->castToMethodSymbol();
                        heuristicTrace(tracer(), "Block containing call node %p %s. Skipping call.", node,
                            node->isTheVirtualCallNodeForAGuardedInlinedCall() ? "is on the cold side of a guard"
                                                                               : "is cold");
                    ...
                    debugTrace(tracer(),
                        "Failing for an unknown reason. TreeTop = %p, node = %p nodevisitCount = %d _visitCount = %d "
                        "getInlinedSiteIndex() = %d thisCallSite = %d. ",
    ...
    if (prevCallStack == 0) {
        ...
        if (comp()->getOption(TR_TraceAll)) {
            ...
            for (calltarget = _callTargets.getFirst(); calltarget; calltarget = calltarget->getNext()) {
                ...
                log->printf("Calltarget %p callnode %p %s\n", calltarget, &calltarget->_myCallSite->_callNode,
                    tracer()->traceSignature(calltarget->_calleeSymbol));
        ...
        TR_CallTarget *callTargetToChop = NULL;
        {
            ...
            for (calltarget = _callTargets.getFirst(); calltarget;
                 prev = calltarget, calltarget = calltarget->getNext()) {
                totalWeight += calltarget->_weight;
                if (doneInlining)
        ...
        for (calltarget = _callTargets.getFirst(); calltarget != callTargetToChop;
             prev = calltarget, calltarget = calltarget->getNext()) {
            ...
            float factor = 1.1F; // this factor was chosen based on a study of a large WAS app that showed that
                                 // getMaxBytecodeindex was 92% accurate compared to nodes generated

            if ((uint32_t)estimatedNumberOfNodes > ((uint32_t)_nodeCountThreshold * factor)) {
                callTargetToChop = calltarget;
                debugTrace(tracer(),
                    "estimate nodes exceeds _nodeCountThreshold, chopped off targets starting from %p, "
                    "lastTargetToInline %p\n",
        ...
        for (TR_CallTarget *calltarget = _callTargets.getFirst(); calltarget; calltarget = calltarget->getNext()) {
            ...
            for (int i = 0; i < clSite->numTargets(); i++) {
                if (clSite->getTarget(i)->_failureReason != MT_Marked) {
                    debugTrace(tracer(), "removing calltarget %p of %p as it isn't in _callTargets",
                        clSite->getTarget(i), calltarget->_myCallSite);
    ...
    if (prevCallStack == 0) {
        ...
        // there are two types of temporaries inliner generates - availablebasicblocktemps, which are used when inlining
        // breaks a block, and commoning must be broken and availableTemps, which are generally used for when a
        // parameter needs a temporary created for it. all the methods that deal with temps (parametertoargumentmapper,
        // handleinjectedbasicblock,transforminlinedfunction) will consult these lists usually, it will search a list,
        // and if it doesn't find a temp, search the second list. the problem is when inlining out of order and with the
        // fact that both temp lists can be consulted, it is possible that a temp will get misused. an example will be a
        // call lower down was inlined first and created a temp t1, for a parameter (the block doesn't get split).  It
        // gets added to availableTemps after inlining. After, higher up (in the same block) another call now gets
        // inlined, and splits the block.  handleinjectedbasicblock now goes and breaks commoning around this higher up
        // call. when this happens, it can grab the temp t1 from the availableTemps list and reuse it for breaking
        // commoning.  Now there are two stores to t1 in the same block.  If there was any commoning that existed after
        // the second store to t1 that was supposed to get broken, it will now load a bad value of t1.
        ...
        for (TR::TreeTop *tt = callerSymbol->getFirstTreeTop(); tt; tt = tt->getNextTreeTop()) {
            ...
            if (tt->getNode()->getNumChildren() && tt->getNode()->getChild(0)->getOpCode().isCall()) {
                ...
                for (calltarget = _callTargets.getFirst(); calltarget; calltarget = calltarget->getNext()) {
                    if (tracer()->debugLevel())
                        debugTrace(tracer(),
                            " (Second Iteration) Considering call target %p  ByteCodeIndex = %d "
                            "calltarget->_myCallSite->_callNodeTreeTop = %p"
                            " alreadyInlined = %d signature = %s",
                            calltarget, calltarget->_myCallSite->_callNode->getByteCodeIndex(),
                            calltarget->_myCallSite->_callNodeTreeTop, calltarget->_alreadyInlined,
                            tracer()->traceSignature(calltarget->_calleeSymbol));
                    ...
                    if (calltarget->_myCallSite->_callNodeTreeTop == tt && !calltarget->_alreadyInlined) {
                        ...
                        anySuccess |= success;
                        debugTrace(tracer(), "(Second Iteration) call target %p node %p.  success = %d anySuccess = %d",
                            calltarget, oldTt->getNode(), success, anySuccess);
                    }
                }
            }
            if (parent->getOpCodeValue() == TR::BBStart && !parent->getBlock()->isExtensionOfPreviousBlock())
                callStack.makeBasicBlockTempsAvailable(_availableBasicBlockTemps);
        }
    }
...
bool TR_MultipleCallTargetInliner::canSkipCountingNodes(TR_CallTarget *callTarget)
{
    TR::RecognizedMethod rm = callTarget->_calleeMethod->getRecognizedMethod();
    switch (rm) {
        case TR::java_lang_Object_hashCode: {
            if (callTarget->_myCallSite && callTarget->_myCallSite->_ecsPrexArgInfo) {
                ...
                if (arg && arg->getClass() && arg->classIsFixed() && arg->hasKnownObjectIndex()) {
                    logprints(comp()->trace(OMR::inlining), comp()->log(),
                        "Skipping node counting for sub call graph of java/lang/Object.hashCode()I\n");
                    return true;
                }
            }
        } break;
    ...
    bool currentBlockHasExceptionSuccessors, bool dontAddCalls)
{
    ...
    for (int32_t k = 0; k < callsite->numTargets(); k++) {
        ...
        if ((calltarget->_calleeMethod->convertToMethod()->isArchetypeSpecimen()
                && calltarget->_calleeMethod->getMethodHandleLocation())
            || isLambdaFormGeneratedMethod) {
            static char *methodHandleThunkWeightFactorStr = feGetEnv("TR_methodHandleThunkWeightFactor");
            static int32_t methodHandleThunkWeightFactor
                = methodHandleThunkWeightFactorStr ? atoi(methodHandleThunkWeightFactorStr) : 10;
        ...
        //              2) reflection
        //  We only WeighCallSite() for the original method.  So we will only have a list of call sites of top level
        //  methods.
...
bool TR_MultipleCallTargetInliner::inlineSubCallGraph(TR_CallTarget *calltarget)
{
     ...
     * 1. It's a JSR292 related method. This condition allows inlining method handle thunk chain without inlining the
     * leaf java method.
    ...
    if (j9inlinerPolicy->isJSR292Method(calltarget->_calleeMethod) || forceInline(calltarget)
...
void TR_MultipleCallTargetInliner::processChoppedOffCallTargets(TR_CallTarget *lastTargetToInline,
    TR_CallTarget *firstChoppedOffcalltarget, int estimatedNumberOfNodes)
{
    if (firstChoppedOffcalltarget) {
        ...
        for (; calltarget; calltarget = calltarget->getNext()) {
            if (inlineSubCallGraph(calltarget)) {
                 ...
                 */
                static bool dontAbortCompilationEvenWithLargeInliningNodesEstimation
                    = feGetEnv("TR_DontAbortCompilationEvenWithLargeInliningNodesEstimation") ? true : false;
                if (!dontAbortCompilationEvenWithLargeInliningNodesEstimation && estimatedNumberOfNodes > 50000
                    && comp()->getMethodHotness() >= hot)
                    comp()->failCompilation<TR::ExcessiveComplexity>(
                        "too many nodes if forced inlining targets are included");
    ...
    if (lastTargetToInline)
        ...
        _callTargets.setFirst(NULL);
}

// Note, this function is shared by all FE's.  If you are changing the heuristic for your FE only, you need to push this
// method into the various FE's FEInliner.cpp file.
int32_t TR_MultipleCallTargetInliner::scaleSizeBasedOnBlockFrequency(int32_t bytecodeSize, int32_t frequency,
    int32_t borderFrequency, TR_ResolvedMethod *calleeResolvedMethod, TR::Node *callNode, int32_t coldBorderFrequency)
{
    int32_t maxFrequency = MAX_BLOCK_COUNT + MAX_COLD_BLOCK_COUNT;
    ...
    if (largeCompiledCallee) {
        ...
        heuristicTrace(tracer(), "exceedsSizeThreshold (mct): Scaled down size for call from %d to %d", oldSize,
            bytecodeSize);
        ...
        // to avoid division by zero crash. Semantically  freqs of 0 and 1 should be pretty close given maxFrequency of
        // 10K
    ...
    int32_t veryLargeCompiledMethodFaninThreshold)
{
    ...
    if (!calleeResolvedMethod->isInterpreted()) {
        TR_PersistentJittedBodyInfo *bodyInfo
            = ((TR_ResolvedJ9Method *)calleeResolvedMethod)->getExistingJittedBodyInfo();
        if ((comp()->getMethodHotness() <= warm)) {
            ...
            if (bodyInfo && (bodyInfo->getHotness() >= scorching)
...
bool TR_MultipleCallTargetInliner::exceedsSizeThreshold(TR_CallSite *callSite, int bytecodeSize, TR::Block *block,
    TR_ByteCodeInfo &bcInfo, int32_t numLocals, TR_ResolvedMethod *callerResolvedMethod,
    TR_ResolvedMethod *calleeResolvedMethod, TR::Node *callNode, bool allConsts)
{
    OMR::Logger *log = comp()->log();
    bool trace = comp()->trace(OMR::inlining);
    ...
    int polymorphicCalleeSizeThreshold
        = polymorphicCalleeSizeThresholdStr ? atoi(polymorphicCalleeSizeThresholdStr) : 10;
    ...
    static TR::SimpleRegex *trustedInterfaceRegex
        = trustedInterfacePattern ? TR::SimpleRegex::create(trustedInterfacePattern) : NULL;
    ...
    if (comp()->getMethodHotness() > warm && callSite->isInterface() && bytecodeSize > polymorphicCalleeSizeThreshold
        && outterMethodSize > polymorphicRootSizeThreshold && ((bytecodeSize * 100) / outterMethodSize) < 6
        && (!callSite->_ecsPrexArgInfo || !callSite->_ecsPrexArgInfo->get(0)
            || !callSite->_ecsPrexArgInfo->get(0)->getClass())
        && comp()->fej9()->maybeHighlyPolymorphic(comp(), callSite->_callerResolvedMethod, callSite->_cpIndex,
            callSite->_interfaceMethod, callSite->_receiverClass)
        && (!trustedInterfaceRegex
            || !TR::SimpleRegex::match(trustedInterfaceRegex, callSite->_interfaceMethod->signature(trMemory()),
                false))) {
        ...
        if (((!bodyInfo && !calleeResolvedMethod->isInterpretedForHeuristics()
                 && !calleeResolvedMethod->isJITInternalNative()) // jitted method without bodyInfo must be scorching
                || (bodyInfo && bodyInfo->getHotness() == scorching)
                || comp()->fej9()->isQueuedForVeryHotOrScorching(calleeResolvedMethod, comp()))
            && (comp()->getMethodHotness() == scorching))
            debugTrace(tracer(),
                "### inline this callee because the compiling method and callee are both scorching even though it's "
                "potentially highly polymorphic callsite initialCalleeSymbol = %s callerResolvedMethod = %s "
                "calleeResolvedMethod = %s\n",
                tracer()->traceSignature(callSite->_interfaceMethod), tracer()->traceSignature(callerResolvedMethod),
                tracer()->traceSignature(calleeResolvedMethod));
        else {
            debugTrace(tracer(),
                "### exceeding size threshold for potentially highly polymorphic callsite initialCalleeSymbol = %s "
    ...
    heuristicTrace(tracer(),
        ...
        tracer()->traceSignature(calleeResolvedMethod), comp()->isServerInlining(), comp()->getMethodHotness(), warm);
    ...
    if (block) {
        ...
        frequency2 = comp()->convertNonDeterministicInput(block->getFrequency(), MAX_BLOCK_COUNT + MAX_COLD_BLOCK_COUNT,
            randomGenerator(), 0);
        ...
        if (allowBiggerMethods() && !comp()->getMethodSymbol()->doJSR292PerfTweaks() && calleeResolvedMethod
            ...
            if (_isInLoop) {
                ...
                if (calleeResolvedMethod)
                    tmptmp
                        = TR::Compiler->cls.classSignature(comp(), calleeResolvedMethod->containingClass(), trMemory());
                bool doit = false;
                ...
                if (doit && calleeResolvedMethod && !strcmp(tmptmp, "Ljava/math/BigDecimal;")) {
                    ...
                    if ((_isInLoop || (blockNestingDepth > 1)) && (bytecodeSize > 10)) {
                        if (trace)
                            heuristicTrace(tracer(),
                                "Exceeds Size Threshold: Scaled down size for call block %d from %d to %d\n",
                                block->getNumber(), bytecodeSize, 10);
    ...
    if (callNode) {
        ...
        if (callNode->getOpCode().isCall() && !callNode->getSymbolReference()->isUnresolved()
        ...
        for (; i < numArgs; ++i) {
            ...
            } else if (arg->getOpCodeValue() == TR::aload
    ...
    {
        // In JIT, having low caller information is equivalent to lack of information.  We want to exclude only cases
    ...
    if (isCold && (bytecodeSize > _methodInColdBlockByteCodeSizeThreshold)) {
        if (block) {
            TR::DebugCounter::prependDebugCounter(comp(), "inliner.callSites/failed/coldCallee/tooManyBytecodes",
                block->getFirstRealTreeTop());
            TR::DebugCounter::prependDebugCounter(comp(),
                "inliner.callSites/failed/coldCallee/tooManyBytecodes:#bytecodeSize", block->getFirstRealTreeTop(),
                bytecodeSize);
            TR::DebugCounter::prependDebugCounter(comp(),
                "inliner.callSites/failed/coldCallee/tooManyBytecodes:#excess", block->getFirstRealTreeTop(),
    ...
    if (bytecodeSize > _methodInWarmBlockByteCodeSizeThreshold
        ...
        if (block) {
            TR::DebugCounter::prependDebugCounter(comp(), "inliner.callSites/failed/warmCallee/tooManyBytecodes",
                block->getFirstRealTreeTop());
            TR::DebugCounter::prependDebugCounter(comp(),
                "inliner.callSites/failed/warmCallee/tooManyBytecodes:#bytecodeSize", block->getFirstRealTreeTop(),
                bytecodeSize);
            TR::DebugCounter::prependDebugCounter(comp(),
                "inliner.callSites/failed/warmCallee/tooManyBytecodes:#excess", block->getFirstRealTreeTop(),
                bytecodeSize - _methodInWarmBlockByteCodeSizeThreshold);
        }

        if (bytecodeSize <= _methodInWarmBlockByteCodeSizeThreshold) {
            heuristicTrace(tracer(),
                "### Exceeds Size Threshold because calculatedSize %d > "
                "_methodInWarmBlockByteCodeSizeThreshold*multiplier %d (excessive Fan In)",
                calculatedSize, _methodInWarmBlockByteCodeSizeThreshold * multiplier);
            ...
            if (r) {
                ...
                printf(
                    "Method %p %s excluded because it has fan in ratio of %f threshold = %d size = %d original = %d.\n",
                    calleeResolvedMethod,
                    comp()->fej9()->sampleSignature(calleeResolvedMethod->getPersistentIdentifier(), calleeName2, 1024,
                        comp()->trMemory()),
                    (float)calculatedSize / (float)bytecodeSize, _methodInWarmBlockByteCodeSizeThreshold * multiplier,
                    calculatedSize, bytecodeSize);
            }
        } else
            heuristicTrace(tracer(),
    ...
    if (isWarm(comp())) {
        ...
        } else if (comp()->isServerInlining() && !alwaysWorthInlining(calleeResolvedMethod, NULL)
            && callerResolvedMethod->isWarmCallGraphTooBig(bcInfo.getByteCodeIndex(), comp()) && !isHot(comp())) {
            heuristicTrace(tracer(),
...
{
    if ((TR::Compiler->vm.isSelectiveMethodEnterExitEnabled(comp())
            && !comp()->fej9()->methodsCanBeInlinedEvenIfEventHooksEnabled(comp()))
        || (comp()->fej9()->isAnyMethodTracingEnabled(method->getPersistentIdentifier())
            && !comp()->fej9()->traceableMethodsCanBeInlined()))
        return false;
...
bool TR_J9InlinerPolicy::shouldRemoveDifferingTargets(TR::Node *callNode)
{
    ...
    TR::RecognizedMethod rm = callNode->getSymbol()->castToMethodSymbol()->getRecognizedMethod();
            ...
            for (int32_t c = callNode->getNumChildren() - 1; c >= firstArgIndex; c--) {
                ...
                if (p && p->usedProfiledInfo()) {
                    ...
                    if (pc) {
                        ...
                        TR::SymbolReference *newSymbolReference
                            = comp()->getSymRefTab()->createTemporary(callerSymbol, dataType);

                        TR::Node *storeNode
                            = TR::Node::createWithSymRef(comp()->il.opCodeForDirectStore(argument->getDataType()), 1, 1,
                                argument, newSymbolReference);
                        TR::TreeTop *storeTree = TR::TreeTop::create(comp(), storeNode);
                        ...
                        TR::Node *nullcmp = TR::Node::createWithSymRef(argument,
                            comp()->il.opCodeForDirectLoad(argument->getDataType()), 0, newSymbolReference);
                        ...
                        TR::Node *vft = TR::Node::createWithSymRef(TR::aloadi, 1, 1,
                            TR::Node::createWithSymRef(argument,
                                comp()->il.opCodeForDirectLoad(argument->getDataType()), 0, newSymbolReference),
                        ...
                        // callNode->setAndIncChild(c,
                        // TR::Node::create(comp()->il.opCodeForDirectLoad(argument->getDataType()), 0,
                        // newSymbolReference));
...
void TR_J9InlinerUtil::refineInliningThresholds(TR::Compilation *comp, int32_t &callerWeightLimit,
...
{
    comp->fej9()->setInlineThresholds(comp, callerWeightLimit, maxRecursiveCallByteCodeSizeEstimate,
        methodByteCodeSizeThreshold, methodInWarmBlockByteCodeSizeThreshold, methodInColdBlockByteCodeSizeThreshold,
        nodeCountThreshold, size);
}

bool TR_J9InlinerPolicy::doCorrectnessAndSizeChecksForInlineCallTarget(TR_CallStack *callStack,
    TR_CallTarget *calltarget, bool inlinefromgraph, TR_PrexArgInfo *argInfo)
{
    ...
    if (!validateArguments(calltarget, map)) {
        TR_ASSERT(comp()->fej9()->canAllowDifferingNumberOrTypesOfArgsAndParmsInInliner(),
            "Error, call target has a parameter mapping issue.");

        return false;
    }

    debugTrace(tracer(),
        "bool inlinecallTarget: calltarget %p calltarget->mycallsite %p calltarget->alreadyInlined = %d "
        "inlinefromgraph = %d currentNumberOfNodes = %d",
        calltarget, calltarget->_myCallSite, calltarget->_alreadyInlined, inlinefromgraph,
        _inliner->getCurrentNumberOfNodes());

    TR_ASSERT(!(calltarget->_alreadyInlined
                  && calltarget->_myCallSite->_callNode->isTheVirtualCallNodeForAGuardedInlinedCall()),
        "inlineCallTarget: trying to inline the virtual call node for a guarded inline call that's already been "
        "inlined!");
    ...
    bool ignoreThisSmallMethod
        = getInitialBytecodeSize(calltarget->_calleeMethod, calltarget->_calleeSymbol, comp()) <= 20;
    // static int si, sj; ++sj;
    if (((_inliner->getNumAsyncChecks() > HIGH_LOOP_COUNT - 5)
            || ((uint32_t)nodeCount > _inliner->getNodeCountThreshold()))
        && !_inliner->forceInline(calltarget) && !ignoreThisSmallMethod) {
        // getCurrentNumberOfNodes may be unreliable so we must recompute
        if (((uint32_t)(nodeCount = comp()->generateAccurateNodeCount()) > _inliner->getNodeCountThreshold())
            || (_inliner->getNumAsyncChecks() > HIGH_LOOP_COUNT - 5)) {
            ...
            logprintf(trace, log,
                "inliner: failed: Caller has too many nodes %s while considering callee %s  nodeCount = %d "
                "nodeCountThreshold = %d\n",
                comp()->signature(), tracer()->traceSignature(calltarget->_calleeSymbol), nodeCount,
                _inliner->getNodeCountThreshold());
            return false;
        }
    }
...
bool TR_J9InlinerPolicy::validateArguments(TR_CallTarget *calltarget, TR_LinkHead<TR_ParameterMapping> &map)
{
    ...
    if (calltarget->_calleeSymbol->getResolvedMethod()->isJNINative()
        && calltarget->_calleeSymbol->getResolvedMethod()->isStatic()
        && calltarget->_myCallSite->_callNode->isPreparedForDirectJNI())
        numArgs--;

    if (numArgs != numParms) {
        heuristicTrace(tracer(),
            "Number of Parameters %d and Arguments %d Differ.  Removing Call Target for Safety's sake.", numParms,
            numArgs);
        calltarget->_myCallSite->removecalltarget(calltarget, tracer(), Cant_Match_Parms_to_Args);

        TR_ASSERT(comp()->fej9()->canAllowDifferingNumberOrTypesOfArgsAndParmsInInliner(),
            "Can't match args to parms, arg number mismatch");
    ...
    if (argNodeIndex == 0 && calltarget->_calleeSymbol->getResolvedMethod()->isJNINative()
        && calltarget->_calleeSymbol->getResolvedMethod()->isStatic()
        && calltarget->_myCallSite->_callNode->isPreparedForDirectJNI())
        argNodeIndex++;
    ...
    for (; parm; parm = parm->getNext(), ++argNodeIndex) {
        ...
        if (arg->getDataType() != parm->_parmSymbol->getDataType()
            ...
            heuristicTrace(tracer(),
                "For argNodeIndex %d, data type of node %p does not match data type of parameter. Removing Call Target "
                "for Safety's sake.",
                argNodeIndex, arg);
            calltarget->_myCallSite->removecalltarget(calltarget, tracer(), Cant_Match_Parms_to_Args);
...
bool TR_J9InlinerPolicy::suppressInliningRecognizedInitialCallee(TR_CallSite *callsite, TR::Compilation *comp)
{
    ...
    if (callNode != NULL) {
        TR::ResolvedMethodSymbol *callResolvedMethodSym = callNode->getSymbol()->getResolvedMethodSymbol();

        TR_ASSERT_FATAL(callResolvedMethodSym == NULL
                || initialCalleeMethod == callResolvedMethodSym->getResolvedMethod(),
            "call site %p _initialCalleeMethod %p should match %p from _callNode %p", callsite, initialCalleeMethod,
            callResolvedMethodSym->getResolvedMethod(), callNode);
    }
    ...
    switch (rm) {
            ...
            if (callNode != NULL && callNode->getOpCode().isCallDirect()) {
                ...
                if (dontInline
                    && performTransformation(comp,
                        "%sNot inlining dead BigDecimal/BigInteger call node [" POINTER_PRINTF_FORMAT "]\n",
        ...
        case TR::java_lang_J9VMInternals_fastIdentityHashCode: {
            ...
            if (callsite->_callerResolvedMethod->getRecognizedMethod() == TR::java_lang_Object_hashCode && callerArgInfo
                && callerArgInfo->get(0)) {
                if (callerArgInfo->get(0)->hasKnownObjectIndex()) {
                    logprints(comp->trace(OMR::inlining), comp->log(),
                        "Suppressing inlining identityHashCode helper call as it can be evaluated in VP.\n");
                    return true;
                }
            }
            return false;
    ...
    switch (initialCalleeMethod->convertToMethod()->getMandatoryRecognizedMethod()) {
        case TR::java_nio_Bits_keepAlive: // This is an empty method whose only purpose is to serve as an anchored use
                                          // of a given object, so it doesn't get collected
        case TR::java_lang_ref_Reference_reachabilityFence: // This is an empty method whose only purpose is to serve as
                                                            // an anchored use of a given object, so it doesn't get
                                                            // collected
...
static bool isDecimalFormatPattern(TR::Compilation *comp, TR_ResolvedMethod *method)
{
    ...
    if (bc == J9BCgetfield) {
        ...
        resolved = method->fieldAttributes(comp, bci.next2Bytes(), &fieldOffset, &type, &isVolatile, 0, &isPrivate,
            false, &isUnresolvedInCP);
    ...
    if (bc == J9BCinvokevirtual) {
        int32_t cpIndex = bci.next2Bytes();
        TR_ResolvedMethod *resolvedMethod = method->getResolvedVirtualMethod(comp, cpIndex, true, &isUnresolvedInCP);
        if (resolvedMethod && resolvedMethod->getRecognizedMethod() == TR::java_text_NumberFormat_format) {
...
TR_InlinerFailureReason TR_J9JSR292InlinerPolicy::checkIfTargetInlineable(TR_CallTarget *target, TR_CallSite *callsite,
    TR::Compilation *comp)
{
    ...
    TR_ResolvedMethod *resolvedMethod
        = target->_calleeSymbol ? target->_calleeSymbol->getResolvedMethod() : target->_calleeMethod;
...
TR_InlinerFailureReason TR_J9InlinerPolicy::checkIfTargetInlineable(TR_CallTarget *target, TR_CallSite *callsite,
    TR::Compilation *comp)
{
    OMR::Logger *log = comp->log();
    bool trace = comp->trace(OMR::inlining);
    ...
    if (comp->fej9()->isChangesCurrentThread(resolvedMethod)
        ...
        logprintf(trace, log,
            "Preventing inlining of %s as it is a JCL method annotated with @ChangesCurrentThread without its caller "
     ...
     */
    if (rm == TR::java_lang_Thread_runWith || rm == TR::java_lang_VirtualThread_runWith
    ...
    if (rm == TR::com_ibm_jit_JITHelpers_toUpperIntrinsicLatin1
        || rm == TR::com_ibm_jit_JITHelpers_toLowerIntrinsicUTF16
        || rm == TR::com_ibm_jit_JITHelpers_toUpperIntrinsicLatin1
        || rm == TR::com_ibm_jit_JITHelpers_toLowerIntrinsicUTF16 ||

        rm == TR::java_lang_String_compressedArrayCopy_BIBII || rm == TR::java_lang_String_compressedArrayCopy_BICII
        || rm == TR::java_lang_String_compressedArrayCopy_CIBII || rm == TR::java_lang_String_compressedArrayCopy_CICII
        ...
        || rm == TR::java_lang_String_decompressedArrayCopy_CICII || rm == TR::java_lang_Math_max_D
        || rm == TR::java_lang_Math_min_D ||
        // DAA Intrinsic methods will get reduced if intrinsics are on, so don't consider it as a target
        (((TR_ResolvedJ9Method *)resolvedMethod)->isDAAMarshallingIntrinsicMethod()
            && !comp->getOption(TR_DisableMarshallingIntrinsics))
        || (((TR_ResolvedJ9Method *)resolvedMethod)->isDAAPackedDecimalIntrinsicMethod()
...
bool TR_J9InlinerPolicy::isSimpleWrapperForInlineableUnsafeNativeMethod(TR_ResolvedMethod *resolvedMethod)
{
    TR::RecognizedMethod method = resolvedMethod->getRecognizedMethod();
    switch (method) {
        case TR::jdk_internal_misc_Unsafe_getCharUnaligned:
...
void TR_J9InlinerUtil::estimateAndRefineBytecodeSize(TR_CallSite *callsite, TR_CallTarget *calltarget,
    TR_CallStack *callStack, int32_t &bytecodeSize)
{
    if (comp()->getOptLevel() >= warm && bytecodeSize > 100) {
        ...
        calltarget->_originatingBlock = (callsite->_callerBlock != NULL)
            ? callsite->_callerBlock
        ...
        if (calltarget->_originatingBlock && calltarget->_calleeSymbol) {
            ...
            TR_EstimateCodeSize::raiiWrapper ecsWrapper(inliner(), tracer(),
                inliner()->getMaxRecursiveCallByteCodeSizeEstimate());
            TR_EstimateCodeSize *ecs = ecsWrapper.getCodeEstimator();
            vcount_t origVisitCount = comp()->getVisitCount();
            ...
            if (estimateIsFine) {
                logprintf(comp()->trace(OMR::inlining), comp()->log(),
                    "Partial estimate for this target %d, full size %d, real bytecode size %d\n",
...
TR_PrexArgInfo *TR_J9InlinerUtil::computePrexInfo(TR_CallTarget *target, TR_PrexArgInfo *callerArgInfo)
{
    ...
    if (target->_calleeSymbol->hasThisCalls() && target->_receiverClass
        && !TR::Compiler->cls.isAbstractClass(comp(), target->_receiverClass)
        && !fe()->classHasBeenExtended(target->_receiverClass) && thisClassInfo && thisClassInfo->isInitialized()
...
TR_PrexArgInfo *TR_J9InlinerUtil::computePrexInfo(TR_CallTarget *target) { return computePrexInfo(target, NULL); }
 ...
 */
static TR::TreeTop *defToAutoOrParmInEBB(TR::Compilation *comp, TR::TreeTop *treetop, TR::SymbolReference *symRef,
    TR::Node **valueNode)
{
    for (; treetop != NULL; treetop = treetop->getPrevTreeTop()) {
        ...
        if (storeNode && storeNode->getOpCode().isStoreDirect() && storeNode->getSymbolReference() == symRef) {
            auto child = storeNode->getFirstChild();
            // If the child is also an auto, keep walking the trees to find the child's def
            if (child->getOpCode().hasSymbolReference() && child->getSymbolReference()->getSymbol()->isAuto()
 ...
 */
static TR::TreeTop *getFirstOccurrenceOfLoad(TR::Compilation *comp, TR::TreeTop *treetop, TR::Node *loadNode)
{
        ...
        if (c == callNode->getFirstArgumentIndex() && callee && callee->convertToMethod()->isArchetypeSpecimen()
            ...
            if (parmSymbol && !prexArg) {
                if (parmSymbol->getFixedType()) {
                    prexArg = new (inliner->trHeapMemory()) TR_PrexArgument(TR_PrexArgument::ClassIsFixed,
                        (TR_OpaqueClassBlock *)parmSymbol->getFixedType());
                    if (tracePrex) {
                        char *sig = TR::Compiler->cls.classSignature(comp,
                            (TR_OpaqueClassBlock *)parmSymbol->getFixedType(), inliner->trMemory());
                ...
                TR_ASSERT_FATAL(ttForFirstOccurrence, "Could not get a treetop for the first occurence of %p",
                    argument);
                ...
                if (valueNode && valueNode->getOpCode().hasSymbolReference()
                    ...
                    prexArg = new (inliner->trHeapMemory())
                        TR_PrexArgument(valueNode->getSymbolReference()->getKnownObjectIndex(), comp);
                    logprintf(tracePrex, log,
            ...
            if (argFirstChild->getOpCodeValue() == TR::loadaddr && argFirstChild->getSymbol()->isStatic()
                && !argFirstChild->getSymbolReference()->isUnresolved() && argFirstChild->getSymbol()->isClassObject()
                && argFirstChild->getSymbol()->castToStaticSymbol()->getStaticAddress()) {
                auto j9c = (J9Class *)argFirstChild->getSymbolReference()
                               ->getSymbol()
                               ->castToStaticSymbol()
                               ->getStaticAddress();
                ...
                if (knot) {
                    ...
                    auto knownObjectIndex = knot->getOrCreateIndexAt(
                        (uintptr_t *)((uint8_t *)j9c + fej9->getOffsetOfJavaLangClassFromClassField()));
...
bool TR_J9InlinerUtil::needTargetedInlining(TR::ResolvedMethodSymbol *callee)
{
    ...
    //
#if !defined(J9VM_OPT_OPENJDK_METHODHANDLE)
    if (callee->getMethod()->isArchetypeSpecimen() || callee->hasMethodHandleInvokes())
        return true;
#endif
    return false;
}
    ...
    for (int32_t c = callNode->getNumChildren() - 1; c >= firstArgIndex; c--) {
        ...
        if (argOrdinal >= ecsArgInfo->getNumArgs()) {
            logprintf(tracePrex, log, "checkForConstClass skipping c=%d because argOrdinal(%d) >= numArgs(%d)\n", c,
                argOrdinal, ecsArgInfo->getNumArgs());
            continue;
        }
        ...
        TR::Node *argument = callNode->getChild(c);
        logprintf(tracePrex, log, "checkForConstClass: Child %d [%p] arg %p %s%s %s\n", c, argument, prexArgument,
            TR_PrexArgument::priorKnowledgeStrings[priorKnowledge], argument->getOpCode().getName(),
            argument->getOpCode().hasSymbolReference() ? argument->getSymbolReference()->getName(comp->getDebug())
                                                       : "");
        ...
        if (argument->getOpCode().hasSymbolReference()
            ...
            TR::Node *argFirstChild = argument->getFirstChild();
            if (argFirstChild->getOpCode().hasSymbolReference() && argFirstChild->getSymbol()->isStatic()
                ...
                if (j9c != NULL) {
                    ...
                    if (knot) {
                        TR_J9VMBase *fej9 = (TR_J9VMBase *)(comp->fe());
                        knownObjectIndex = knot->getOrCreateIndexAt(
                            (uintptr_t *)((uint8_t *)j9c + fej9->getOffsetOfJavaLangClassFromClassField()));
        ...
        if ((argument->getOpCode().hasSymbolReference()
                && (knownObjectClass || argument->getSymbolReference()->hasKnownObjectIndex()))
#ifdef TR_ALLOW_NON_CONST_KNOWN_OBJECTS
...
TR_PrexArgInfo *TR_PrexArgInfo::buildPrexArgInfoForMethodSymbol(TR::ResolvedMethodSymbol *methodSymbol,
    TR_LogTracer *tracer)
{
    ...
    int index = 0;
    /**
     *  For non-static method, first slot of the paramters is populated with the class owning the method. Most of the
     * time, we should be able to get the class pointer using class signature but in case of hidden classes which
     * according to JEP 371, cannot be used directly by bytecode instructions in other classes also is not possible to
     * refer to them in paramters except for the method from the same hidden class. In any case, for non-static methods
     * instead of making VM query to get the owning class, extract that information from resolvedMethod.
     */
    for (TR::ParameterSymbol *p = parms.getFirst(); p != NULL; index++, p = parms.getNext()) {
        ...
        if (*sig == 'L' || *sig == '[') {
            TR_OpaqueClassBlock *clazz = (index == 0 && !methodSymbol->isStatic())
                ? feMethod->containingClass()
                : comp->fe()->getClassFromSignature(sig, len, feMethod);
            if (clazz) {
                argInfo->set(index,
                    new (comp->trHeapMemory()) TR_PrexArgument(TR_PrexArgument::ClassIsPreexistent, clazz));
                heuristicTrace(tracer, "PREX-CSI:  Parm %d class %p in %p is %.*s\n", index,
                    argInfo->get(index)->getClass(), argInfo->get(index), len, sig);
            }
        }
    }
    return argInfo;
...
static void populateClassNameSignature(TR::Method *m, TR_ResolvedMethod *caller, TR_OpaqueClassBlock *&c, char *&nc,
    int32_t &nl, char *&sc, int32_t &sl)
{
...
{
    int32_t len = m->classNameLength();
    return TR::Compiler->cls.classNameToSignature(m->classNameChars(),
        len /*don't care, cos this gives us a null terminated string*/, comp);
}

static bool treeMatchesCallSite(TR::TreeTop *tt, TR::ResolvedMethodSymbol *callerSymbol, TR_CallSite *callsite,
    TR_LogTracer *tracer)
{
    if (tt->getNode()->getNumChildren() > 0 && tt->getNode()->getFirstChild()->getOpCode().isCall()
        ...
        char *callSiteNameChars, *callNodeNameChars, *callSiteSignatureChars, *callNodeSignatureChars;

        int32_t callSiteNameLength, callNodeNameLength, callSiteSignatureLength, callNodeSignatureLength;

        populateClassNameSignature(callsite->_initialCalleeMethod ? callsite->_initialCalleeMethod->convertToMethod()
                                                                  : // TR_ResolvedMethod doesn't extend TR::Method
                callsite->_interfaceMethod,
            callerSymbol->getResolvedMethod(), callSiteClass, callSiteNameChars, callSiteNameLength,
            callSiteSignatureChars, callSiteSignatureLength);

        populateClassNameSignature(callNodeMS->getMethod(), callerSymbol->getResolvedMethod(), callNodeClass,
            callNodeNameChars, callNodeNameLength, callNodeSignatureChars, callNodeSignatureLength);
        ...
        if (!isLFMethod
            && (!callNodeClass || !callSiteClass
                || callerSymbol->getResolvedMethod()->fe()->isInstanceOf(callNodeClass, callSiteClass, true, true, true)
                    != TR_yes)) {
            if (tracer->heuristicLevel()) {
                TR::Compilation *comp = TR::comp(); // won't be evaluated unless tracing is on
                heuristicTrace(tracer,
                    "ARGS PROPAGATION: Incompatible classes: callSiteClass %p (%s) callNodeClass %p (%s)",
                    callSiteClass,
                    classSignature(callsite->_initialCalleeMethod ? callsite->_initialCalleeMethod->convertToMethod()
                                                                  : callsite->_interfaceMethod,
                        comp),
                    callNodeClass, classSignature(callNodeMS->getMethod(), comp));
            }
            return false;
        }
...
TR::TreeTop *TR_PrexArgInfo::getCallTree(TR::ResolvedMethodSymbol *methodSymbol, TR_CallSite *callsite,
    TR_LogTracer *tracer)
{
    ...
    heuristicTrace(tracer, "ARGS PROPAGATION: Couldn't find a matching node for callsite %p bci %d", callsite,
        callsite->_bcInfo.getByteCodeIndex());
    return NULL;
}

TR::Node *TR_PrexArgInfo::getCallNode(TR::ResolvedMethodSymbol *methodSymbol, TR_CallSite *callsite,
    TR_LogTracer *tracer)
{
...
{
    if (child->getOpCode().hasSymbolReference() && child->getSymbolReference()->getSymbol()->isParm()
...
{
    TR_ASSERT(child->getOpCode().hasSymbolReference() && child->getSymbolReference()->getSymbol()->isParm()
            && child->getSymbolReference()->getSymbol()->getParmSymbol()->getOrdinal() < argInfo->getNumArgs()
            && argInfo->get(child->getSymbolReference()->getSymbol()->getParmSymbol()->getOrdinal()),
    ...
    TR_PrexArgInfo *argInfo, TR_LogTracer *tracer)
{
    ...
    TR::Compilation *comp = tracer->comp();
    heuristicTrace(tracer, "ARGS PROPAGATION: trying to propagate receiver's info for callsite %p at %p", callsite,
        callNode);
...
bool TR_PrexArgInfo::validateAndPropagateArgsFromCalleeSymbol(TR_PrexArgInfo *argsFromSymbol,
    TR_PrexArgInfo *argsFromTarget, TR_LogTracer *tracer)
{
    ...
    for (int32_t i = 0; i < numArgsToEnhance; i++) {
        ...
        */
        if (fe->isInstanceOf(argsFromSymbol->get(i)->getClass(), argsFromTarget->get(i)->getClass(), true, true, true)
                != TR_yes
            && fe->isInstanceOf(argsFromTarget->get(i)->getClass(), argsFromSymbol->get(i)->getClass(), true, true,
                   true)
                != TR_yes) {
            return false;
        }
    ...
    TR_PrexArgInfo *argInfo, TR_LogTracer *tracer)
{
    ...
    TR_ASSERT(argInfo, "otherwise we shouldn't even peek");
    TR::Node *callNode = TR_PrexArgInfo::getCallNode(methodSymbol, callsite, tracer);
    heuristicTrace(tracer, "ARGS PROPAGATION: trying to propagate arg info from caller symbol to callsite %p at %p",
        callsite, callNode);
    ...
    heuristicTrace(tracer, "Caller args:");
    argInfo->dumpTrace();
    ...
    for (int i = callNode->getFirstArgumentIndex(); i < callNode->getNumChildren(); i++) {
        ...
        if (TR_PrexArgInfo::hasArgInfoForChild(child, argInfo)) {
            heuristicTrace(tracer, "ARGS PROPAGATION: arg %d at callsite %p matches caller's arg %d", i, callsite,
                child->getSymbolReference()->getSymbol()->getParmSymbol()->getOrdinal());
...
void TR_J9InlinerUtil::refineColdness(TR::Node *node, bool &isCold)
{
    ...
    if (symRef->getSymbol()->isResolvedMethod()
        ...
        inlineableJNI
            = static_cast<TR_J9InlinerPolicy *>(inliner()->getPolicy())
                  ->isInlineableJNI(symRef->getSymbol()->castToResolvedMethodSymbol()->getResolvedMethod(), node);

    isCold = isCold && !inlineableJNI;
}

void TR_J9InlinerUtil::computeMethodBranchProfileInfo(TR::Block *cfgBlock, TR_CallTarget *calltarget,
    TR::ResolvedMethodSymbol *callerSymbol)
{
    if (cfgBlock) // isn't this equal to genILSucceeded??
    {
        ...
        TR_MethodBranchProfileInfo *mbpInfo = TR_MethodBranchProfileInfo::getMethodBranchProfileInfo(
            cfgBlock->getEntry()->getNode()->getInlinedSiteIndex(), comp());
        if (!mbpInfo) {
            TR::Block *block = callNodeTreeTop->getEnclosingBlock();

            mbpInfo = TR_MethodBranchProfileInfo::addMethodBranchProfileInfo(
                cfgBlock->getEntry()->getNode()->getInlinedSiteIndex(), comp());
            ...
            if (callerSymbol->getFirstTreeTop()->getNode()->getBlock()->getFrequency() > 0) {
                freqScaleFactor
                    = (float)(blockFreq) / callerSymbol->getFirstTreeTop()->getNode()->getBlock()->getFrequency();
                if (callerSymbol->getFlowGraph()->getInitialBlockFrequency() > 0)
                    freqScaleFactor
                        *= (float)(callerSymbol->getFlowGraph()->getInitialBlockFrequency()) / (float)firstBlockFreq;
            }
            ...
            if (comp()->getOption(TR_TraceBFGeneration)) {
                comp()->log()->printf("Setting initial block count for a call with index %d to be %d, call factor %f "
                                      "where block %d (%p) and blockFreq = %d\n",
                    cfgBlock->getEntry()->getNode()->getInlinedSiteIndex(), firstBlockFreq, freqScaleFactor,
                    block->getNumber(), block, blockFreq);
...
TR_TransformInlinedFunction *TR_J9InlinerUtil::getTransformInlinedFunction(TR::ResolvedMethodSymbol *callerSymbol,
    TR::ResolvedMethodSymbol *calleeSymbol, TR::Block *blockContainingTheCall, TR::TreeTop *callNodeTreeTop,
    TR::Node *callNode, TR_ParameterToArgumentMapper &pam, TR_VirtualGuardSelection *guard,
    List<TR::SymbolReference> &tempList, List<TR::SymbolReference> &availableTemps,
    List<TR::SymbolReference> &availableBasicBlockTemps)
{
    return new (comp()->trStackMemory())
        TR_J9TransformInlinedFunction(comp(), tracer(), callerSymbol, calleeSymbol, blockContainingTheCall,
            callNodeTreeTop, callNode, pam, guard, tempList, availableTemps, availableBasicBlockTemps);
}

TR_J9TransformInlinedFunction::TR_J9TransformInlinedFunction(TR::Compilation *c, TR_InlinerTracer *tracer,
    TR::ResolvedMethodSymbol *callerSymbol, TR::ResolvedMethodSymbol *calleeSymbol, TR::Block *callNodeBlock,
    TR::TreeTop *callNodeTreeTop, TR::Node *callNode, TR_ParameterToArgumentMapper &mapper,
    TR_VirtualGuardSelection *guard, List<TR::SymbolReference> &temps, List<TR::SymbolReference> &availableTemps,
    List<TR::SymbolReference> &availableTemps2)
    : TR_TransformInlinedFunction(c, tracer, callerSymbol, calleeSymbol, callNodeBlock, callNodeTreeTop, callNode,
          mapper, guard, temps, availableTemps, availableTemps2)
{}

void TR_J9TransformInlinedFunction::transform()
{
...
void TR_J9TransformInlinedFunction::transformSynchronizedMethod(TR_ResolvedMethod *calleeMethod)
{
...
void TR_J9TransformInlinedFunction::wrapCalleeInTryRegion(bool isSynchronized, bool putCatchInCaller,
    TR_ResolvedMethod *calleeMethod)
{
    ...
    if (doCreateExplicitCatchBlock) {
        ...
        for (TR::TreeTop *tt = _calleeSymbol->getFirstTreeTop(); tt != originalLastTreeTop; tt = tt->getNextTreeTop()) {
            ...
            if (node->getOpCodeValue() == TR::BBStart)
                block = node->getBlock();
            else if (node->getNumChildren() > 0 && (node = node->getFirstChild())->getOpCodeValue() == TR::athrow
                ...
                prevTreeTop = createThrowCatchBlock(isSynchronized, putCatchInCaller, calleeCFG, block, prevTreeTop,
                    symRef, catchBlockHandler, newCatchBlocks);
            }
        }
    }
    ...
    if (isSynchronized)
        ...
        catchBlock
            = appendCatchBlockToRethrowException(calleeMethod, prevTreeTop, putCatchInCaller, 0, handlerIndex, true);
    ...
    for (TR::CFGNode *n = calleeCFG->getFirstNode(); n; n = n->getNext())
        if (!catchBlock->hasSuccessor(n)
            ...
            && !toBlock(n)->isOSRCodeBlock() && !toBlock(n)->isOSRCatchBlock())
...
TR::TreeTop *TR_J9TransformInlinedFunction::createThrowCatchBlock(bool isSynchronized, bool putCatchInCaller,
    TR::CFG *calleeCFG, TR::Block *block, TR::TreeTop *prevTreeTop, TR::SymbolReference *symRef, int32_t handlerIndex,
    TR_ScratchList<TR::Block> &newCatchBlocks)
{
    TR_InlinerDelimiter delimiter(tracer(), "tif.createThrowCatchBlock");
    TR::Block *catchBlock;
    if (isSynchronized) {
        catchBlock = appendCatchBlockForInlinedSyncMethod(symRef->getOwningMethod(comp()), prevTreeTop,
            symRef->getCPIndex(), handlerIndex, false);
        catchBlock->setSpecializedDesyncCatchBlock();
        catchBlock->setIsSynchronizedHandler();
    } else
        catchBlock = appendCatchBlockToRethrowException(symRef->getOwningMethod(comp()), prevTreeTop, putCatchInCaller,
            symRef->getCPIndex(), handlerIndex, false);
...
TR::Block *TR_J9TransformInlinedFunction::appendCatchBlockToRethrowException(TR_ResolvedMethod *calleeMethod,
    TR::TreeTop *prevTreeTop, bool putCatchInCaller, int32_t catchType, int32_t handlerIndex, bool addBlocks)
{
    ...
    if (comp()->getOption(TR_EnableThisLiveRangeExtension)) {
        if (!_calleeSymbol->isStatic()
            ...
            TR::Node *anchoredThis = TR::Node::createWithSymRef(modelNode, TR::aload, 0,
                symRefTab->findOrCreateAutoSymbol(_calleeSymbol, 0, TR::Address));
            TR::SymbolReference *tempSymRef
                = comp()->getSymRefTab()->findOrCreateThisRangeExtensionSymRef(_calleeSymbol);
    ...
    TR::Node *loadExcpSymbol
        = TR::Node::createWithSymRef(modelNode, TR::aload, 0, symRefTab->findOrCreateExcpSymbolRef());
    catchBlock->append(TR::TreeTop::create(comp(),
        TR::Node::createWithSymRef(TR::athrow, 1, 1, loadExcpSymbol,
            symRefTab->findOrCreateAThrowSymbolRef(_calleeSymbol))));
...
TR::Block *TR_J9TransformInlinedFunction::appendCatchBlockForInlinedSyncMethod(TR_ResolvedMethod *calleeResolvedMethod,
    TR::TreeTop *prevTreeTop, int32_t catchType, int32_t handlerIndex, bool addBlocks)
{
    ...
    catchBlock->setHandlerInfo(catchType, (uint8_t)comp()->getInlineDepth(), handlerIndex, calleeResolvedMethod,
        comp());
    catchBlock->setIsSynchronizedHandler();
    ...
    OMR_InlinerUtil::storeValueInATemp(comp(), excpSymbol, tempSymRef, catchBlock->getEntry(), _callerSymbol, _tempList,
        _availableTemps, &_availableTemps2);
    ...
    if (_calleeSymbol->isStatic()) {
        monitorArgHandle = TR::Node::createWithSymRef(lastNode, TR::loadaddr, 0,
            symRefTab->findOrCreateClassSymbol(_calleeSymbol, 0,
                _calleeSymbol->getResolvedMethod()->containingClass()));
        monitorArgHandle = TR::Node::createWithSymRef(TR::aloadi, 1, 1, monitorArgHandle,
            symRefTab->findOrCreateJavaLangClassFromClassSymbolRef());
    } else
        monitorArgHandle = TR::Node::createWithSymRef(lastNode, TR::aload, 0,
            symRefTab->findOrCreateAutoSymbol(_calleeSymbol, 0, TR::Address));
    ...
    if (!_calleeSymbol->isStatic()) {
        ...
        if (!comp()->getOption(TR_DisableLiveMonitorMetadata) && _calleeSymbol->isSynchronised()
        ...
        TR::Node *ifNode = TR::Node::createif(TR::ifacmpeq, monitorArg->duplicateTree(),
            TR::Node::aconst(monitorArg, 0), rethrowBlock->getEntry());
    ...
    //
    if (!comp()->getOption(TR_DisableLiveMonitorMetadata) && !createdStoreForMonitorExit
        && _calleeSymbol->isSynchronised() && _calleeSymbol->getSyncObjectTemp()) {
        TR::Node *addrNode = TR::Node::create(monitorArg, TR::iconst, 0, 0);
        TR::TreeTop *storeTT = TR::TreeTop::create(comp(), (TR::Node::create(lastNode, TR::monexitfence, 0)));
        monexitBlock->append(storeTT);
    }

    TR::Node *monexitNode = TR::Node::createWithSymRef(TR::monexit, 1, 1, monitorArg,
        symRefTab->findOrCreateMonitorExitSymbolRef(_calleeSymbol));
    ...
    if (comp()->getOption(TR_EnableThisLiveRangeExtension)) {
        if (!_calleeSymbol->isStatic()
            && (!comp()->fej9()->isClassFinal(_calleeSymbol->getResolvedMethod()->containingClass())
                || comp()->fej9()->hasFinalizer(_calleeSymbol->getResolvedMethod()->containingClass()))) {
            TR::Node *anchoredThis = TR::Node::createWithSymRef(lastNode, TR::aload, 0,
                symRefTab->findOrCreateAutoSymbol(_calleeSymbol, 0, TR::Address));
            TR::SymbolReference *tempSymRef
                = comp()->getSymRefTab()->findOrCreateThisRangeExtensionSymRef(_calleeSymbol);
            TR::TreeTop *storeTT = TR::TreeTop::create(comp(), TR::Node::createStore(tempSymRef, anchoredThis));
    ...
    rethrowBlock->append(TR::TreeTop::create(comp(),
        TR::Node::createWithSymRef(TR::athrow, 1, 1, temp,
            symRefTab->findOrCreateThrowUnreportedExceptionSymbolRef(_calleeSymbol))));
...
bool TR_J9TransformInlinedFunction::isSyncReturnBlock(TR::Compilation *comp, TR::Block *b)
{
...
bool TR_J9InlinerUtil::addTargetIfMethodIsNotOverridenInReceiversHierarchy(TR_IndirectCallSite *callsite)
{
    ...
    if (!chTable->isOverriddenInThisHierarchy(callsite->_initialCalleeMethod, callsite->_receiverClass,
            callsite->_vftSlot, comp())
        && !comp()->getOption(TR_DisableHierarchyInlining)) {
        if (comp()->trace(OMR::inlining)) {
            int32_t len;
            bool isClassObsolete
                = comp()->getPersistentInfo()->isObsoleteClass((void *)callsite->_receiverClass, comp()->fe());
            if (!isClassObsolete) {
                ...
                heuristicTrace(tracer(),
                    "Virtual call to %s is not overridden in the hierarchy of thisClass <obsolete class>\n",
...
TR_ResolvedMethod *TR_J9InlinerUtil::findSingleJittedImplementer(TR_IndirectCallSite *callsite)
{
    return comp()->getPersistentInfo()->getPersistentCHTable()->findSingleJittedImplementer(callsite->_receiverClass,
        callsite->_vftSlot, callsite->_callerResolvedMethod, comp(), callsite->_initialCalleeSymbol);
}
...
bool TR_J9InlinerUtil::addTargetIfThereIsSingleImplementer(TR_IndirectCallSite *callsite)
{
    ...
    if (!disableSingleJittedImplementerInlining && comp()->getMethodHotness() >= hot
        ...
        logprintf(comp()->trace(OMR::inlining), comp()->log(),
            "inliner: Abstract method %s currently has a single jitted implementation %s\n",
            inliner()->tracer()->traceSignature(callsite->_initialCalleeMethod),
        ...
        if (callsite->_receiverClass && !fe()->classHasBeenExtended(callsite->_receiverClass))
            guard = new (comp()->trHeapMemory())
                TR_VirtualGuardSelection(TR_ProfiledGuard, TR_VftTest, implementer->classOfMethod());
        else
...
TR_PrexArgInfo *TR_J9InlinerUtil::createPrexArgInfoForCallTarget(TR_VirtualGuardSelection *guard,
    TR_ResolvedMethod *implementer)
{
    TR_PrexArgInfo *myPrexArgInfo = NULL;
    // if CSI (context sensitive inlining + args propagation) enabled we still want to create an argInfo for args
    // propagation
    if (!comp()->getOption(TR_DisableInlinerArgsPropagation) && comp()->fej9()->supportsContextSensitiveInlining()) {
        // rather than sticking in a not-null check in TR_J9EstimateCodeSize::realEstimateCodeSize and duplicating the
        // line below we might as well put a supportsContextSensitiveInlining check in here
        myPrexArgInfo
            = new (comp()->trHeapMemory()) TR_PrexArgInfo(implementer->numberOfParameters(), comp()->trMemory());
        if (guard->_type == TR_VftTest) {
            TR_ASSERT(implementer, "no implementer!\n");
            TR_ASSERT(!implementer->isStatic(), "method is static\n");

            myPrexArgInfo->set(0,
                new (comp()->trHeapMemory()) TR_PrexArgument(TR_PrexArgument::ClassIsFixed, guard->_thisClass));
        ...
        bool isArchetypeSpecimen
            = implementer->convertToMethod()->isArchetypeSpecimen() && implementer->getMethodHandleLocation() != NULL;

        bool isMCS = guard->_kind == TR_MutableCallSiteTargetGuard;

        bool isLambdaFormMCS = isMCS && comp()->fej9()->isLambdaFormGeneratedMethod(implementer);
...
TR_InnerPreexistenceInfo *TR_J9InlinerUtil::createInnerPrexInfo(TR::Compilation *c,
    TR::ResolvedMethodSymbol *methodSymbol, TR_CallStack *callStack, TR::TreeTop *callTree, TR::Node *callNode,
    TR_VirtualGuardKind guardKind)
{
    return new (comp()->trStackMemory())
        TR_J9InnerPreexistenceInfo(c, methodSymbol, callStack, callTree, callNode, guardKind);
}
...
TR_J9InnerPreexistenceInfo::ParmInfo::ParmInfo(TR::ParameterSymbol *innerParm, TR::ParameterSymbol *outerParm)
    : _innerParm(innerParm)
    , _outerParm(outerParm)
    , _isInvariant(true)
{}

bool TR_J9InnerPreexistenceInfo::perform(TR::Compilation *comp, TR::Node *guardNode, bool &disableTailRecursion)
{
    static char *disable = feGetEnv("TR_DisableIPREX");
    if (disable || !comp->getOptimizer()->isEnabled(OMR::innerPreexistence) || comp->getOption(TR_FullSpeedDebug)
        || comp->getHCRMode() != TR::none || guardNode->isHCRGuard() || guardNode->isBreakpointGuard()
    ...
    if (hasInnerAssumptions()) {
        ...
        if (point
            && performTransformation(comp,
                "%sIPREX: remove virtual guard for inlined call %p to %s because it inner preexists parm ordinal %d of "
                "%s\n",
                OPT_DETAILS, _callNode, _methodSymbol->getResolvedMethod()->signature(trMemory()), point->_ordinal,
                point->_callStack->_methodSymbol->getResolvedMethod()->signature(trMemory()))) {
            ...
            //
            TR_ASSERT(guardNode->getOpCodeValue() == TR::ificmpne || guardNode->getOpCodeValue() == TR::iflcmpne
...
TR_J9InnerPreexistenceInfo::TR_J9InnerPreexistenceInfo(TR::Compilation *c, TR::ResolvedMethodSymbol *methodSymbol,
    TR_CallStack *callStack, TR::TreeTop *treeTop, TR::Node *callNode, TR_VirtualGuardKind guardKind)
    : TR_InnerPreexistenceInfo(c, methodSymbol, callStack, treeTop, callNode, guardKind)
{
    static char *disable = feGetEnv("TR_DisableIPREX");
    if (!c->getOptimizer()->isEnabled(OMR::innerPreexistence) || c->compileRelocatableCode() || disable
        || !_methodSymbol || c->getHCRMode() == TR::traditional)
        return;
    ...
    //
    for (TR::TreeTop *tt = methodSymbol->getFirstTreeTop(); tt; tt = tt->getNextRealTreeTop()) {
        TR::Node *node = tt->getNode();
        ...
        for (int32_t c = node->getNumChildren() - 1; c >= firstArgIndex; --c) {
            ...
            if (argument->getOpCodeValue() == TR::aload) {
                ...
                if (parmSymbol && c - firstArgIndex < ordinal) {
                    ...
                    if (parmInfo)
                        parmInfo->setOuterSymbol(parmSymbol);
                }
            }
        }
    }
...
TR_J9InnerPreexistenceInfo::PreexistencePoint *TR_J9InnerPreexistenceInfo::getPreexistencePoint(int32_t ordinal)
{
    if (hasInnerAssumptions())
        return 0;
    ...
    return ((TR_J9InnerPreexistenceInfo *)_callStack->_innerPrexInfo)
        ->getPreexistencePointImpl(parmInfo->_outerParm->getOrdinal(), _callStack);
}

TR_J9InnerPreexistenceInfo::PreexistencePoint *TR_J9InnerPreexistenceInfo::getPreexistencePointImpl(int32_t ordinal,
    TR_CallStack *prevCallStack)
{
    ParmInfo *parmInfo = getParmInfo(ordinal);
    ...
    if (parmInfo->_outerParm)
        point = ((TR_J9InnerPreexistenceInfo *)_callStack->_innerPrexInfo)
                    ->getPreexistencePointImpl(parmInfo->_outerParm->getOrdinal(), _callStack);

    if (!point) {
        if (_guardKind != TR_ProfiledGuard
            && (_guardKind != TR_NoGuard || !comp()->hasIntStreamForEach())) // FIXME: this limitation can be removed by
                                                                             // doing the tree transformation
...
bool TR_J9InlinerPolicy::replaceSoftwareCheckWithHardwareCheck(TR_ResolvedMethod *calleeMethod)
{
    if (calleeMethod && comp()->cg()->getSupportsBDLLHardwareOverflowCheck()
        && ((strncmp(calleeMethod->signature(comp()->trMemory()), "java/math/BigDecimal.noLLOverflowAdd(JJJ)Z", 42)
                == 0)
            || (strncmp(calleeMethod->signature(comp()->trMemory()), "java/math/BigDecimal.noLLOverflowMul(JJJ)Z", 42)
                == 0)))
        return true;
    else
        return false;
...
bool TR_J9InlinerPolicy::suitableForRemat(TR::Compilation *comp, TR::Node *callNode, TR_VirtualGuardSelection *guard)
{
    ...
    TR_AddressInfo *valueInfo
        = static_cast<TR_AddressInfo *>(TR_ValueProfileInfoManager::getProfiledValueInfo(callNode, comp, AddressInfo));
    if (guard->_forceTakenSideCold) {
        ...
        if (comp->getMethodHotness() <= warm && comp->getPersistentInfo()->getJitState() == STARTUP_STATE) {
            ...
            TR::DebugCounter::incStaticDebugCounter(comp,
                TR::DebugCounter::debugCounterName(comp, "profiledPrivArgRemat/unsuitableForRemat/warmHighProb"));
        } else {
            TR::DebugCounter::incStaticDebugCounter(comp,
                TR::DebugCounter::debugCounterName(comp, "profiledPrivArgRemat/suitableForRemat/highProb"));
        }
    } else if (valueInfo) {
        if (valueInfo->getTopProbability() >= profiledGuardProbabilityThreshold) {
            TR::DebugCounter::incStaticDebugCounter(comp,
                TR::DebugCounter::debugCounterName(comp, "profiledPrivArgRemat/suitableForRemat/probability=%d",
                    ((int32_t)(valueInfo->getTopProbability() * 100))));
        } else {
            TR::DebugCounter::incStaticDebugCounter(comp,
                TR::DebugCounter::debugCounterName(comp, "profiledPrivArgRemat/unsuitableForRemat/probability=%d",
                    ((int32_t)(valueInfo->getTopProbability() * 100))));
            suitableForRemat = false;
        }
    } else {
        TR::DebugCounter::incStaticDebugCounter(comp,
            TR::DebugCounter::debugCounterName(comp, "profiledPrivArgRemat/unsuitableForRemat/noinfo"));
        suitableForRemat = false;
    }
...
TR_InlinerTracer *TR_J9InlinerUtil::getInlinerTracer(TR::Optimization *optimization)
{
