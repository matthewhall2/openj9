...
static TR::SymbolReferenceTable::CommonNonhelperSymbol equivalentAtomicIntrinsic(TR::Compilation *comp,
    TR::RecognizedMethod rm)
{
    switch (rm) {
        ...
        case TR::sun_misc_Unsafe_getAndSetLong:
            return comp->target().is64Bit() ? TR::SymbolReferenceTable::atomicSwapSymbol
                                            : TR::SymbolReferenceTable::lastCommonNonhelperSymbol;
        ...
        case TR::sun_misc_Unsafe_getAndAddLong:
            return comp->target().is64Bit() ? TR::SymbolReferenceTable::atomicFetchAndAddSymbol
                                            : TR::SymbolReferenceTable::lastCommonNonhelperSymbol;
        default:
            break;
    }
 ...
 *
 */
bool TR_UnsafeFastPath::tryTransformUnsafeAtomicCallInVarHandleAccessMethod(TR::TreeTop *callTree,
    TR::RecognizedMethod callerMethod, TR::RecognizedMethod calleeMethod)
{
    ...
    // Give up on arraylet
    //
    if (isVarHandleOperationMethodOnArray(callerMethod) && TR::Compiler->om.canGenerateArraylets()) {
        logprintf(trace(), log, "Call %p n%dn is accessing an element from an array that might be arraylet, quit\n",
    ...
    if (symbol->getMethod()->isUnsafeCAS()) {
        ...
        if (isVarHandleOperationMethodOnNonStaticField(callerMethod)
            && performTransformation(comp(),
                "%s transforming Unsafe.CAS [" POINTER_PRINTF_FORMAT "] into codegen inlineable\n", optDetailString(),
                node)) {
#if defined(J9VM_GC_SPARSE_HEAP_ALLOCATION)
            if (isUnsafeCallerAccessingArrayElement(callerMethod) && TR::Compiler->om.isOffHeapAllocationEnabled()
                && comp()->target().is64Bit()) {
            ...
            logprintf(trace(), log, "Found Unsafe CAS node %p n%dn on non-static field, set the flag\n", node,
                node->getGlobalIndex());
            return true;
        }
    ...
    if (!comp()->cg()->supportsNonHelper(helper)) {
        logprints(trace(), log, "Equivalent atomic intrinsic is not supported on current platform, quit\n");
        return false;
    }
    ...
    if (isUnsafeCallerAccessingStaticField(callerMethod)) {
        ...
        TR::Node *j9Class = TR::Node::createWithSymRef(node, TR::aloadi, 1, jlClass,
            comp()->getSymRefTab()->findOrCreateClassFromJavaLangClassSymbolRef());
        TR::Node *ramStatics = TR::Node::createWithSymRef(node, TR::aloadi, 1, j9Class,
        ...
        //
        offset = TR::Node::create(node, TR::land, 2, offset, TR::Node::lconst(node, ~J9_SUN_FIELD_OFFSET_MASK));
        unsafeAddress = comp()->target().is32Bit()
            ? TR::Node::create(node, TR::aiadd, 2, ramStatics, TR::Node::create(node, TR::l2i, 1, offset))
        ...
        unsafeAddress = comp()->target().is32Bit()
            ? TR::Node::create(node, TR::aiadd, 2, baseAddr, TR::Node::create(node, TR::l2i, 1, offset))
    ...
    if (callTree->getNode()->getOpCode().isNullCheck()) {
        ...
        TR::Node *checkNode
            = TR::Node::createWithSymRef(nullChkNode, TR::NULLCHK, 1, passthrough, nullChkNode->getSymbolReference());
        callTree->insertBefore(TR::TreeTop::create(comp(), checkNode));
        TR::Node::recreate(nullChkNode, TR::treetop);

        logprintf(trace(), log, "Created node %p n%dn to preserve null check on call %p n%dn\n", checkNode,
            checkNode->getGlobalIndex(), node, node->getGlobalIndex());
    }
    ...
    logprintf(trace(), log, "Transformed the call %p n%dn to codegen inlineable intrinsic\n", node,
        node->getGlobalIndex());
...
int32_t TR_UnsafeFastPath::perform()
{
    ...
    for (TR::TreeTop *tt = methodSymbol->getFirstTreeTop(); tt != NULL; tt = tt->getNextTreeTop()) {
        ...
        TR::Node *node
            = ttNode->getNumChildren() > 0 ? ttNode->getFirstChild() : NULL; // Get the first child of the tree
        ...
        if (node && node->getOpCode().isCall() && !node->getSymbol()->castToMethodSymbol()->isHelper()) {
            ...
            if (!transformed.contains(node)) {
                ...
                if (isVarHandleOperationMethod(caller)
                    && (isTransformableUnsafeAtomic(comp(), callee) || symbol->getMethod()->isUnsafeCAS())) {
            ...
            if (!symRef->isUnresolved() && !comp()->generateArraylets()
                && (symbol->getRecognizedMethod() == TR::java_math_BigDecimal_storeTwoCharsFromInt)
                && performTransformation(comp(),
                    "%s turning the call [" POINTER_PRINTF_FORMAT "] into an indirect store\n", optDetailString(),
                    node)) {
                ...
                prepareToReplaceNode(
                    node); // This will remove the usedef info, valueNumber info and all children of the node
                TR::Node *offsetNode
                    = TR::TransformUtil::generateConvertArrayElementIndexToOffsetTrees(comp(), index, NULL, 4, false);
                TR::Node *addrCalc = TR::TransformUtil::generateArrayElementAddressTrees(comp(), charArray, offsetNode);
                TR::SymbolReference *unsafeSymRef
                    = comp()->getSymRefTab()->findOrCreateUnsafeSymbolRef(TR::Int32, true, false);
                node = TR::Node::recreateWithoutProperties(node, TR::istorei, 2, addrCalc, value, unsafeSymRef);

                TR::TreeTop *newTree
                    = tt->insertAfter(TR::TreeTop::create(comp(), TR::Node::create(TR::treetop, 1, node)));
                tt->getNode()->removeAllChildren();
                TR::TransformUtil::removeTree(comp(), tt);
                tt = newTree;

                logprintf(trace(), log,
                    "Created node [" POINTER_PRINTF_FORMAT "] to store the value [" POINTER_PRINTF_FORMAT
                    "] to target location [" POINTER_PRINTF_FORMAT "]\n",
                    node, value, addrCalc);
                continue;
            }
            ...
            if (symbol->getRecognizedMethod() == TR::com_ibm_jit_JITHelpers_byteToCharUnsigned) {
                if (performTransformation(comp(),
                        "%s Found unsafe/JITHelpers calls, turning node [" POINTER_PRINTF_FORMAT
                        "] into a unsigned widening\n",
                        optDetailString(), node)) {
                    ...
                    if (value->getDataType() != TR::Int8) {
                        TR::ILOpCodes conversionOpCode
                            = TR::ILOpCode::getProperConversion(value->getDataType(), TR::Int8, true);

                        // Sanity check for future modifications
                        TR_ASSERT(conversionOpCode != TR::BadILOp,
                            "Could not get proper conversion for a JITHelpers byIndex operation.\n");
                    ...
                    logprintf(trace(), log,
                        "Created node [" POINTER_PRINTF_FORMAT "] to unsigned widen value [" POINTER_PRINTF_FORMAT
                        "] \n",
                        node, value);
            ...
            if (symbol->getRecognizedMethod() == TR::com_ibm_jit_JITHelpers_acmplt) {
                ...
                anchorAllChildren(node, tt);
                prepareToReplaceNode(node);
                node = TR::Node::recreateWithoutProperties(node, TR::acmplt, 2, obj0, obj1);

                TR::TreeTop *newTree = TR::TreeTop::create(comp(), TR::Node::create(TR::treetop, 1, node));
            ...
            // Set other properties(data type, isVolatile and new value for its field or array element) of the object to
            // be operated
            switch (symbol->getRecognizedMethod()) {
                case TR::sun_misc_Unsafe_putObjectVolatile_jlObjectJjlObject_V:
                ...
                case TR::sun_misc_Unsafe_putLong_jlObjectJJ_V:
                    if (!strncmp(methodSymbol->getResolvedMethod()->classNameChars(),
                            "java/util/concurrent/ThreadLocalRandom", 38))
                        value = node->getChild(3);
            ...
            if (isKnownUnsafeCaller(callerMethod) && TR_J9MethodBase::isUnsafeGetPutWithObjectArg(calleeMethod)) {
                ...
                logprintf(trace(), log,
                    "VarHandle operation: isArrayOperation %d type %s value %p access mode %s on node %p\n",
                    isArrayOperation, J9::DataType::getName(type), value, TR::Symbol::getMemoryOrderingName(ordering),
                    node);
            }
            ...
            if (mightBeArraylets && disableUnsafeForArraylets) {
                logprintf(trace(), log,
                    "unsafeForArraylets is disabled, skip unsafeFastPath for node [" POINTER_PRINTF_FORMAT "]\n", node);
                continue;
            }
            ...
            if (type != TR::NoType
                ...
                TR::SymbolReference *unsafeSymRef
                    = comp()->getSymRefTab()->findOrCreateUnsafeSymbolRef(type, true, isStatic, ordering);
                ...
                if (isStatic) {
                    ...
                    TR::Node *j9Class = TR::Node::createWithSymRef(TR::aloadi, 1, 1, jlClass,
                        comp()->getSymRefTab()->findOrCreateClassFromJavaLangClassSymbolRef());
                    TR::Node *ramStatics = TR::Node::createWithSymRef(TR::aloadi, 1, 1, j9Class,
                        comp()->getSymRefTab()->findOrCreateRamStaticsFromClassSymbolRef());
                    ...
                    TR::Node *newOffset = TR::Node::create(offset, TR::land, 2, offset,
                ...
                if (mightBeArraylets) {
                    logprintf(trace(), log,
                        "This is an array operation in arraylets mode with array [" POINTER_PRINTF_FORMAT
                        "] and offset [ " POINTER_PRINTF_FORMAT "], creating a load/store and a spineCHK\n",
                        object, offset);
                    ...
                    TR::SymbolReference *bndCHKSymRef
                        = comp()->getSymRefTab()->findOrCreateArrayBoundsCheckSymbolRef(methodSymbol);
                    ...
                    if (value) {
                        ...
                        if (type == TR::Address && (TR::Compiler->om.writeBarrierType() != gc_modron_wrtbar_none)) {
                            node = TR::Node::recreateWithoutProperties(node, TR::awrtbari, 3, addrCalc, value, object,
                                unsafeSymRef);
                            spineCHK->setAndIncChild(0, addrCalc);
                        } else {
                            if (value->getDataType() != type) {
                                TR::ILOpCodes conversionOpCode = TR::ILOpCode::getProperConversion(value->getDataType(),
                                    type, needUnsignedConversion(symbol->getRecognizedMethod()));

                                // Sanity check for future modifications
                                TR_ASSERT(conversionOpCode != TR::BadILOp,
                                    "Could not get proper conversion on the value node %p n%dn for call %p n%dn.\n",
                                    value, value->getGlobalIndex(), node, node->getGlobalIndex());

                                value = TR::Node::create(conversionOpCode, 1, value);
                            }

                            node = TR::Node::recreateWithoutProperties(node,
                                comp()->il.opCodeForIndirectArrayStore(type), 2, addrCalc, value, unsafeSymRef);
                        ...
                        logprintf(trace(), log,
                            "Created node [" POINTER_PRINTF_FORMAT "] to store the value [" POINTER_PRINTF_FORMAT
                            "] to target location [" POINTER_PRINTF_FORMAT "] with spineCHK [" POINTER_PRINTF_FORMAT
                            "]\n",
                            node, value, addrCalc, spineCHK);
                    } else {
                        // This is a load
                        if (node->getDataType() != type) {
                            TR::ILOpCodes conversionOpCode = TR::ILOpCode::getProperConversion(type,
                                node->getDataType(), needUnsignedConversion(symbol->getRecognizedMethod()));

                            // Sanity check for future modifications
                            TR_ASSERT(conversionOpCode != TR::BadILOp,
                                "Could not get proper conversion on the result of call %p n%dn.\n", node,
                                node->getGlobalIndex());

                            TR::Node *load = TR::Node::createWithSymRef(node,
                                comp()->il.opCodeForIndirectArrayLoad(type), 1, unsafeSymRef);
                            ...
                            node = TR::Node::recreateWithoutProperties(node,
                                comp()->il.opCodeForIndirectArrayLoad(type), 1, addrCalc, unsafeSymRef);
                        ...
                        logprintf(trace(), log,
                            "Created node [" POINTER_PRINTF_FORMAT "] to load from location [" POINTER_PRINTF_FORMAT
                            "] with spineCHK [" POINTER_PRINTF_FORMAT "]\n",
                    ...
                    if (value) {
                        ...
                        if (type == TR::Address && (TR::Compiler->om.writeBarrierType() != gc_modron_wrtbar_none)) {
                            ...
                            TR::ILOpCodes opCodeForIndirectStore = isArrayOperation
                                ? comp()->il.opCodeForIndirectArrayStore(type)
                                : comp()->il.opCodeForIndirectStore(type);

                            node = TR::Node::recreateWithoutProperties(node, opCodeForIndirectStore, 2, addrCalc, value,
                                unsafeSymRef);
                        }

                        if (trace())
                            log->printf("Created node [" POINTER_PRINTF_FORMAT
                                        "] to store the value [" POINTER_PRINTF_FORMAT
                                        "] to target location [" POINTER_PRINTF_FORMAT "]\n",
                                node, value, addrCalc);
                        ...
                        TR::ILOpCodes opCodeForIndirectLoad = isArrayOperation
                            ? comp()->il.opCodeForIndirectArrayLoad(type)
                        ...
                        if (node->getDataType() != type) {
                            ...
                            node = TR::Node::recreateWithoutProperties(node, opCodeForIndirectLoad, 1, addrCalc,
                                unsafeSymRef);
                        }
...
const char *TR_UnsafeFastPath::optDetailString() const throw() { return "O^O UNSAFE FAST PATH: "; }
