...
int32_t TR_DataAccessAccelerator::perform()
{
     ...
     */
    if (!comp()->getOption(TR_DisableIntrinsics) && !comp()->getOption(TR_MimicInterpreterFrameShape) &&
...
int32_t TR_DataAccessAccelerator::processVariableCalls(TreeTopContainer *variableCallTreeTops)
{
    int32_t result = 0;
...
const char *TR_DataAccessAccelerator::optDetailString() const throw() { return "O^O DATA ACCESS ACCELERATOR: "; }
    ...
    for (TR::TreeTopIterator iter(block->getEntry(), comp()); iter != block->getExit(); ++iter) {
        ...
        if (currentNode != NULL && currentNode->getOpCode().isCall()) {
            ...
            if (callSymbol != NULL) {
                ...
                if (!matched && (isZOS || isZLinux) && !comp()->getOption(TR_DisablePackedDecimalIntrinsics)) {
                    matched = true;
                    switch (callSymbol->getRecognizedMethod()) {
                        ...
                        case TR::com_ibm_dataaccess_ExternalDecimal_checkExternalDecimal_:
                            if (comp()->target().cpu.supportsFeature(
                ...
                if (!matched && (isZOS || isZLinux) && !block->isCold()
                ...
                if (matched && result && !requestOSRGuardRemoval
...
TR::Node *TR_DataAccessAccelerator::insertDecimalGetIntrinsic(TR::TreeTop *callTreeTop, TR::Node *callNode,
    int32_t sourceNumBytes, int32_t targetNumBytes)
{
    ...
    if (requiresByteSwap && !comp()->cg()->supportsByteswap()) {
        printInliningStatus(false, callNode,
            "Unmarshalling is not supported because ByteSwap IL evaluators are not implemented.");
        return NULL;
    }
    ...
    if (performTransformation(comp(), "O^O TR_DataAccessAccelerator: insertDecimalGetIntrinsic on callNode %p\n",
        ...
        switch (sourceNumBytes) {
            case 4:
                sourceDataType = TR::Float;
                break;
            case 8:
                sourceDataType = TR::Double;
                break;
        }
        ...
        switch (sourceNumBytes) {
            case 4:
                op = requiresByteSwap ? TR::iloadi : TR::floadi;
                break;
            case 8:
                op = requiresByteSwap ? TR::lloadi : TR::dloadi;
                break;
        }
        ...
        switch (targetNumBytes) {
            case 4:
                targetDataType = TR::Float;
                break;
            case 8:
                targetDataType = TR::Double;
                break;
        }

        TR::Node *valueNode
            = TR::Node::createWithSymRef(op, 1, 1, constructAddressNode(callNode, byteArrayNode, offsetNode),
        ...
        if (requiresByteSwap) {
            ...
            switch (sourceNumBytes) {
                case 4:
                    valueNode = TR::Node::create(TR::ibits2f, 1, TR::Node::create(TR::ibyteswap, 1, valueNode));
                    break;
                case 8:
                    valueNode = TR::Node::create(TR::lbits2d, 1, TR::Node::create(TR::lbyteswap, 1, valueNode));
                    break;
            }
        }

        if (sourceNumBytes != targetNumBytes) {
            valueNode = TR::Node::create(TR::ILOpCode::getProperConversion(sourceDataType, targetDataType, false), 1,
                valueNode);
        }
...
TR::Node *TR_DataAccessAccelerator::insertDecimalSetIntrinsic(TR::TreeTop *callTreeTop, TR::Node *callNode,
    int32_t sourceNumBytes, int32_t targetNumBytes)
{
    if (sourceNumBytes != 4 && sourceNumBytes != 8) {
        printInliningStatus(false, callNode, "sourceNumBytes is invalid. Valid sourceNumBytes values are 4 or 8.");
    ...
    if (performTransformation(comp(), "O^O TR_DataAccessAccelerator: insertDecimalSetIntrinsic on callNode %p\n",
        ...
        switch (targetNumBytes) {
            case 4:
                op = requiresByteSwap ? TR::istorei : TR::fstorei;
                break;
            case 8:
                op = requiresByteSwap ? TR::lstorei : TR::dstorei;
                break;
        }
        ...
        if (requiresByteSwap) {
            ...
            switch (targetNumBytes) {
                case 4:
                    valueNode = TR::Node::create(TR::ibyteswap, 1, TR::Node::create(TR::fbits2i, 1, valueNode));
                    break;
                case 8:
                    valueNode = TR::Node::create(TR::lbyteswap, 1, TR::Node::create(TR::dbits2l, 1, valueNode));
                    break;
            }
        }

        return TR::Node::createWithSymRef(op, 2, 2, constructAddressNode(callNode, byteArrayNode, offsetNode),
            valueNode, comp()->getSymRefTab()->findOrCreateGenericIntShadowSymbolReference(0));
    }
...
bool TR_DataAccessAccelerator::inlineCheckPackedDecimal(TR::TreeTop *callTreeTop, TR::Node *callNode)
{
    ...
    if (failMsg) {
        TR::DebugCounter::incStaticDebugCounter(comp(),
            TR::DebugCounter::debugCounterName(comp(), "DAA/rejected/chkPacked"));
    ...
    if (performTransformation(comp(), "O^O TR_DataAccessAccelerator: inlineCheckPackedDecimal on callNode %p\n",
            callNode)) {
        TR::DebugCounter::incStaticDebugCounter(comp(),
            TR::DebugCounter::debugCounterName(comp(), "DAA/inlined/chkPacked"));
        ...
        TR::SymbolReference *packedDecimalSymbolReference = comp()->getSymRefTab()->findOrCreateArrayShadowSymbolRef(
            TR::PackedDecimal, NULL, precisionSizeInNumberOfBytes, fe());

        TR::Node *pdchkChild0Node = TR::Node::createWithSymRef(TR::pdloadi, 1, 1,
            constructAddressNode(callNode, byteArrayNode, offsetNode), packedDecimalSymbolReference);

        // The size argument passed to create an array shadow symbol reference is the size in number of bytes that this
        // PackedDecimal represents. Unfortunately when a Node is constructed with this symbol reference we extract the
        // size from the symbol reference and convert it to a precision via a helper function. Because this conversion
        // is not injective we may not get back the original precision we calculated above. This is why we must
        // explicitly set the precision on the Node after creation.

        pdchkChild0Node->setDecimalPrecision(precision);

        if (precision % 2 == 0) {
            const bool ignoreHighNibbleForEvenPrecision
                = static_cast<bool>(ignoreHighNibbleForEvenPrecisionNode->getInt());
            const bool canOverwriteHighNibbleForEvenPrecision
                = static_cast<bool>(canOverwriteHighNibbleForEvenPrecisionNode->getInt());

            if (ignoreHighNibbleForEvenPrecision || canOverwriteHighNibbleForEvenPrecision) {
                // Increase the precision of the pdload by 1 to pretend that we have an extra digit, then create a new
                // parent on top of the pdload which will truncate Packed Decimal by modifying its precision to the
                // desired value. This has the effect of creating a new temporary Packed Decimal value which properly
                // ignores the high nibble if the precision is even, and more over it has a value of 0 in the high
                // nibble.
                ...
                // If we are allowed to overwrite the high nibble if the precision is even then we need to store
                // temporary Packed Decimal we just created back into the original byte array. We once again pretend
                // that we have an extra digit when doing this store because we also want to store out the extra 0 digit
                // which is guaranteed to be present due to the above computation.

                if (canOverwriteHighNibbleForEvenPrecision) {
                    int32_t precisionSizeInNumberOfBytes
                        = TR::DataType::getSizeFromBCDPrecision(TR::PackedDecimal, precision + 1);

                    TR::SymbolReference *packedDecimalSymbolReference
                        = comp()->getSymRefTab()->findOrCreateArrayShadowSymbolRef(TR::PackedDecimal, NULL,
                            precisionSizeInNumberOfBytes, fe());

                    // this node should be inserted after callNode
                    TR::Node *pdstoreNode = TR::Node::createWithSymRef(TR::pdstorei, 2, 2,
                        constructAddressNode(callNode, byteArrayNode, offsetNode), pdchkChild0Node,
                        packedDecimalSymbolReference);
...
bool TR_DataAccessAccelerator::inlineCheckExternalDecimal(TR::TreeTop *callTreeTop, TR::Node *callNode)
{
    ...
    if (failMsg) {
        TR::DebugCounter::incStaticDebugCounter(comp(),
            TR::DebugCounter::debugCounterName(comp(), "DAA/rejected/chkZonedDecimal"));
    ...
    if (performTransformation(comp(), "O^O TR_DataAccessAccelerator: inlineCheckZonedDecimal on callNode %p\n",
            callNode)) {
        TR::DebugCounter::incStaticDebugCounter(comp(),
            TR::DebugCounter::debugCounterName(comp(), "DAA/inlined/chkZonedDecimal"));
        ...
        TR::SymbolReference *zonedDecimalSymbolReference = comp()->getSymRefTab()->findOrCreateArrayShadowSymbolRef(
            decimalType, NULL, precisionSizeInNumberOfBytes, fe());
        TR::Node *zdchkChild0Node = TR::Node::createWithSymRef(loadOpCode, 1, 1,
            constructAddressNode(callNode, byteArrayNode, offsetNode), zonedDecimalSymbolReference);
...
TR::Node *TR_DataAccessAccelerator::insertIntegerGetIntrinsic(TR::TreeTop *callTreeTop, TR::Node *callNode,
    int32_t sourceNumBytes, int32_t targetNumBytes)
{
    if (targetNumBytes != 1 && targetNumBytes != 2 && targetNumBytes != 4 && targetNumBytes != 8) {
        printInliningStatus(false, callNode,
            "targetNumBytes is invalid. Valid targetNumBytes values are 1, 2, 4, or 8.");
        return NULL;
    }
    ...
    if (sourceNumBytes == 0) {
        ...
        if (sourceNumBytes != 1 && sourceNumBytes != 2 && sourceNumBytes != 4 && sourceNumBytes != 8) {
            printInliningStatus(false, callNode,
                "sourceNumBytes is invalid. Valid targetNumBytes values are 1, 2, 4, or 8.");
            return NULL;
        }
    ...
    bool requiresByteSwap
        = sourceNumBytes != 1 && comp()->target().cpu.isBigEndian() != static_cast<bool>(bigEndianNode->getInt());
    ...
    if (performTransformation(comp(), "O^O TR_DataAccessAccelerator: genSimpleGetBinary call: %p inlined.\n",
        ...
        switch (sourceNumBytes) {
            case 1:
                sourceDataType = TR::Int8;
                break;
            case 2:
                sourceDataType = TR::Int16;
                break;
            case 4:
                sourceDataType = TR::Int32;
                break;
            case 8:
                sourceDataType = TR::Int64;
                break;
        }
        ...
        switch (sourceNumBytes) {
            ...
            case 2:
                op = TR::sloadi;
                byteswapOp = TR::sbyteswap;
                break;
            case 4:
                op = TR::iloadi;
                byteswapOp = TR::ibyteswap;
                break;
            case 8:
                op = TR::lloadi;
                byteswapOp = TR::lbyteswap;
                break;
        }
        ...
        if (sourceDataType != targetDataType) {
            valueNode = TR::Node::create(
                TR::ILOpCode::getProperConversion(sourceDataType, targetDataType, needUnsignedConversion), 1,
                valueNode);
        }
...
TR::Node *TR_DataAccessAccelerator::insertIntegerSetIntrinsic(TR::TreeTop *callTreeTop, TR::Node *callNode,
    int32_t sourceNumBytes, int32_t targetNumBytes)
{
    if (sourceNumBytes != 1 && sourceNumBytes != 2 && sourceNumBytes != 4 && sourceNumBytes != 8) {
        printInliningStatus(false, callNode,
            "sourceNumBytes is invalid. Valid sourceNumBytes values are 1, 2, 4, or 8.");
        return NULL;
    }
    ...
    // Determines whether a TR::ByteSwap needs to be inserted before the store to the byteArray
    bool requiresByteSwap
        = targetNumBytes != 1 && comp()->target().cpu.isBigEndian() != static_cast<bool>(bigEndianNode->getInt());

    if (requiresByteSwap && !comp()->cg()->supportsByteswap()) {
        printInliningStatus(false, callNode,
            "Marshalling is not supported because ByteSwap IL evaluators are not implemented.");
        return NULL;
    }
    ...
    if (performTransformation(comp(), "O^O TR_DataAccessAccelerator: genSimplePutBinary call: %p inlined.\n",
        ...
        switch (targetNumBytes) {
            case 1:
                targetDataType = TR::Int8;
                break;
            case 2:
                targetDataType = TR::Int16;
                break;
            case 4:
                targetDataType = TR::Int32;
                break;
...
TR::Node *TR_DataAccessAccelerator::constructAddressNode(TR::Node *callNode, TR::Node *arrayNode, TR::Node *offsetNode,
    bool isUnicodeOrExternalDecimalInvolved, bool isSrcOrTargetUnicodeDecimal)
{
    ...
    if (callNode->getSymbol()->getResolvedMethodSymbol()) {
        if (callNode->getSymbol()->getResolvedMethodSymbol()->getRecognizedMethod()) {
            ...
            if ((callNode->getSymbol()->getResolvedMethodSymbol()->getRecognizedMethod()
                    == TR::com_ibm_dataaccess_DecimalData_convertIntegerToPackedDecimal_ByteBuffer_)
                || (callNode->getSymbol()->getResolvedMethodSymbol()->getRecognizedMethod()
                    == TR::com_ibm_dataaccess_DecimalData_convertLongToPackedDecimal_ByteBuffer_)) {
                isByteBuffer = true;
                pdBufAddressNode = callNode->getChild(5);
                pdBufPositionNode = callNode->getChild(7);
            } else if ((callNode->getSymbol()->getResolvedMethodSymbol()->getRecognizedMethod()
                           == TR::com_ibm_dataaccess_DecimalData_convertPackedDecimalToInteger_ByteBuffer_)
                || (callNode->getSymbol()->getResolvedMethodSymbol()->getRecognizedMethod()
                    == TR::com_ibm_dataaccess_DecimalData_convertPackedDecimalToLong_ByteBuffer_)) {
                isByteBuffer = true;
            ...
            if (isByteBuffer) {
                TR::Node *offset
                    = TR::Node::create(TR::i2l, 1, TR::Node::create(TR::iadd, 2, pdBufPositionNode, offsetNode));
    ...
    totalOffsetNode = TR::TransformUtil::generateConvertArrayElementIndexToOffsetTrees(comp(), offsetNode, NULL,
        isSrcOrTargetUnicodeDecimal ? 2 : 1, false);
...
bool TR_DataAccessAccelerator::genComparisionIntrinsic(TR::TreeTop *treeTop, TR::Node *callNode, TR::ILOpCodes ops)
{
    ...
    if (!performTransformation(comp(),
            "O^O TR_DataAccessAccelerator: genComparison call: %p, Comparison type: %d inlined.\n", callNode, ops)) {
        return false;
    }
    ...
    TR::SymbolReference *symRef1
        = comp()->getSymRefTab()->findOrCreateArrayShadowSymbolRef(TR::PackedDecimal, arrayAddressNode1, 8, fe());
    symRef1->setOffset(0);
    ...
    TR::SymbolReference *symRef2
        = comp()->getSymRefTab()->findOrCreateArrayShadowSymbolRef(TR::PackedDecimal, arrayAddressNode2, 8, fe());
    symRef2->setOffset(0);
    ...
    TR::Node *bcdchkNode = TR::Node::createWithSymRef(TR::BCDCHK, 7, 7, pdOpNode, op1Node, offset1Node, prec1Node,
        op2Node, offset2Node, prec2Node, bcdChkSymRef);
...
bool TR_DataAccessAccelerator::generateI2PD(TR::TreeTop *treeTop, TR::Node *callNode, bool isI2PD, bool isByteBuffer)
{
    ...
    if (failMsg) {
        TR::DebugCounter::incStaticDebugCounter(comp(),
            TR::DebugCounter::debugCounterName(comp(), "DAA/rejected/%s", isI2PD ? "i2pd" : "l2pd"));
        return printInliningStatus(false, callNode, failMsg);
    }
    ...
    if (performTransformation(comp(), "O^O TR_DataAccessAccelerator: %s call: %p inlined.\n",
        ...
        TR::DebugCounter::incStaticDebugCounter(comp(),
            TR::DebugCounter::debugCounterName(comp(), "DAA/inlined/%s", isI2PD ? "i2pd" : "l2pd"));
        ...
        if (isByteBuffer) {
             ...
             * with true for the option fixupCommoning and so it will break the commoning and add any necessary temps
             * for you.
             */
         ...
         *
         * AddrNode1, the second child of the BCDCHK node, is meant to be rematerialized for OOL post-call data copy
         * back. See BCDCHKEvaluatorImpl() for BCDCHK tree structure and intended use of its children.
         *
         ...
         * AddrNode1 and AddrNode2 were the same node, the LocalCSE would not consider AddrNode1 an alternative
         * replacement of AddrNode3 because the BCDCHK's symbol canGCAndReturn().
         *
        ...
        if (needsBCDCHK) {
            ...
            TR::Node *pdshlNode
                = TR::Node::create(TR::pdshlOverflow, 2, i2pdNode, TR::Node::create(callNode, TR::iconst, 0));
             ...
             * definition of a BCDCHK node. BCDCHK node is already a special type of node, and all optimizations expect
             * the call (i2pd) to be inside the first child of BCDCHK. Attaching another call could cause many things to
             * break as all optimizations such as Value Propagation don't expect it to be there. Attaching the
             * callNode's children to BCDCHK would be safe. We would whip up the call with these attached children
             * during codegen for the fallback of the fastpath.
             */
            TR::SymbolReference *bcdChkSymRef = callNode->getSymbolReference();

            if (isByteBuffer) {
                bcdchkNode = TR::Node::createWithSymRef(TR::BCDCHK, 10, 10, pdshlNode, outOfLineCopyBackAddr,
                    callNode->getChild(0), callNode->getChild(1), callNode->getChild(2), callNode->getChild(3),
                    callNode->getChild(4), callNode->getChild(5), callNode->getChild(6), callNode->getChild(7),
                    bcdChkSymRef);
            } else {
                bcdchkNode = TR::Node::createWithSymRef(TR::BCDCHK, 7, 7, pdshlNode, outOfLineCopyBackAddr,
                    callNode->getChild(0), callNode->getChild(1), callNode->getChild(2), callNode->getChild(3),
                    callNode->getChild(4), bcdChkSymRef);
            }
        ...
        if (isByteBuffer) {
             ...
             * */
            callBlock->createConditionalBlocksBeforeTree(treeTop, isValidAddrTreeTop, slowPathTreeTop, fastPathTreeTop,
                cfg, false, true);
        }

        TR::SymbolReference *symRef
            = comp()->getSymRefTab()->findOrCreateArrayShadowSymbolRef(TR::PackedDecimal, storeAddressNode, 8, fe());
        ...
        if (isByteBuffer) {
            // the original call will be deleted by createConditionalBlocksBeforeTree, but if the refcount was > 1, we
            // need to insert stores.
            if (needsBCDCHK) {
...
void TR_DataAccessAccelerator::createPrecisionDiamond(TR::Compilation *comp, TR::TreeTop *treeTop,
    TR::TreeTop *fastTree, TR::TreeTop *slowTree, bool isPD2I, uint32_t numPrecisionNodes, ...)
{
    ...
    for (uint32_t i = 0; i < numPrecisionNodes; ++i) {
        ...
        TR::Node *node1
            = TR::Node::createif(TR::ificmpgt, precisionNode->duplicateTree(), TR::Node::iconst(precisionMax));
        TR::Node *node2
            = TR::Node::createif(TR::ificmplt, precisionNode->duplicateTree(), TR::Node::iconst(precisionMin));
...
bool TR_DataAccessAccelerator::generatePD2IConstantParameter(TR::TreeTop *treeTop, TR::Node *callNode, bool isPD2i,
    bool isByteBuffer)
{
    ...
    if (failMsg) {
        TR::DebugCounter::incStaticDebugCounter(comp(),
            TR::DebugCounter::debugCounterName(comp(), "DAA/rejected/%s", isPD2i ? "pd2i" : "pd2l"));
        return printInliningStatus(false, callNode, failMsg);
    }
    ...
    if (performTransformation(comp(), "O^O TR_DataAccessAccelerator: %s call: %p inlined.\n",
        ...
        TR::DebugCounter::incStaticDebugCounter(comp(),
            TR::DebugCounter::debugCounterName(comp(), "DAA/inlined/%s", isPD2i ? "pd2i" : "pd2l"));
        if (isByteBuffer) {
            /* We will be creating a precision diamond for the fast / slow path and eliminating the original call.
             * Because we are about to split the CFG we would have to store the original parameters of the call into
             * temp slots as we will be duplicating the call node in the precision diamond but we don't need to since
        ...
        TR::SymbolReference *symRef
            = comp()->getSymRefTab()->findOrCreateArrayShadowSymbolRef(TR::PackedDecimal, arrayAddressNode, 8, fe());
        ...
        if (isByteBuffer) {
            ...
            bcdchk = TR::Node::createWithSymRef(TR::BCDCHK, 8, 8, copiedCallNode, callNode->getChild(0),
                callNode->getChild(1), callNode->getChild(2), callNode->getChild(3), callNode->getChild(4),
                callNode->getChild(5), callNode->getChild(6), bcdChkSymRef);
             ...
             * n2975n    BCDCHK [#959] ()
               n2958n      pd2i ()
               n2949n        pdloadi  <array-shadow>[#497  Shadow] [flags 0x80000618 0x0 ] <prec=10 (len=6) adj=0>
             sign=hasState n2948n          l2a n2947n            ladd n2941n              lload  <temp slot 29>[#949
             Auto] [flags 0x4 0x0 ]     // address of ByteBuffer n2946n              i2l n2945n                iadd
            ...
            bcdchk = TR::Node::createWithSymRef(TR::BCDCHK, 5, 5, callNode, callNode->getChild(0),
                callNode->getChild(1), callNode->getChild(2), callNode->getChild(3), bcdChkSymRef);
            /*
             * BCDCHK would look something like this for byte[]:
             *
             * n2937n    BCDCHK [#990] ()
               n990n       pd2i ()
               n2929n        pdloadi  <array-shadow>[#486  Shadow] [flags 0xffffffff80000612 0x0 ] <prec=10 (len=6)
             adj=0> sign=hasState   vn=- sti=- udi=- nc=1 n2928n          aladd (internalPtr sharedMemory ) n986n
             ==>aload n2927n            aladd n2925n              lconst 8 n2926n              i2l n1001n ==>iconst 0
               n986n       ==>aload                                                       // byte[]
        ...
        // The callNode may have more than 1 reference (treetop and i/lstore), so we need to scan through its list of
        // children.
        ...
        if (isByteBuffer) {
            ...
            if (newSymbolReference) {
                ...
                TR::Node *slowStoreNode = TR::Node::createWithSymRef(comp()->il.opCodeForDirectStore(dataType), 1, 1,
                ...
                TR::Node *fastStoreNode = TR::Node::createWithSymRef(comp()->il.opCodeForDirectStore(dataType), 1, 1,
                    bcdchkTreeTop->getNode()->getFirstChild(), newSymbolReference);
...
TR::Node *TR_DataAccessAccelerator::restructureVariablePrecisionCallNode(TR::TreeTop *treeTop, TR::Node *callNode)
{
...
bool TR_DataAccessAccelerator::generatePD2IVariableParameter(TR::TreeTop *treeTop, TR::Node *callNode, bool isPD2i,
    bool isByteBuffer)
{
    TR::Node *precisionNode = callNode->getChild(2);
    ...
    if (comp()->getOption(TR_DisableVariablePrecisionDAA)
        ...
        TR::DebugCounter::incStaticDebugCounter(comp(),
            TR::DebugCounter::debugCounterName(comp(), "DAA/rejected/%s", isPD2i ? "var-pd2i" : "var-pd2l"));
        return false;
    }

    TR::DebugCounter::incStaticDebugCounter(comp(),
        TR::DebugCounter::debugCounterName(comp(), "DAA/inlined/%s", isPD2i ? "var-pd2i" : "var-pd2l"));
    // We will be creating a precision diamond for the fast / slow path and eliminating the original call.
    ...
    if (callNode->getReferenceCount() > 0) {
        ...
        TR::SymbolReference *temp
            = comp()->getSymRefTab()->createTemporary(comp()->getMethodSymbol(), callNode->getDataType());
...
{
    if (!isChildConst(callNode, 2) || !isChildConst(callNode, 5) || !isChildConst(callNode, 8)
        || !isChildConst(callNode, 9)) {
    ...
    if (precision1 < 1 || precision1 > 15 || precision2 < 1 || precision2 > 15 || precisionResult < 1
        || precisionResult > 15) {
        return printInliningStatus(false, callNode, "Invalid precisions. Valid precisions are in range [1, 15]");
    }

    if (performTransformation(comp(),
            "O^O TR_DataAccessAccelerator: genArithmetics call: %p inlined, with opcode:%d \n", callNode, opCode)) {
        // create loading
        //  loading The first operand
        TR::Node *arrayAddressNodeA = constructAddressNode(callNode, input1Node, offset1Node);
        TR::SymbolReference *symRef1
            = comp()->getSymRefTab()->findOrCreateArrayShadowSymbolRef(TR::PackedDecimal, arrayAddressNodeA, 8, fe());
        symRef1->setOffset(0);
        ...
        TR::SymbolReference *symRef2
            = comp()->getSymRefTab()->findOrCreateArrayShadowSymbolRef(TR::PackedDecimal, arrayAddressNodeB, 8, fe());
        symRef2->setOffset(0);
        ...
        switch (opCode) {
            ...
            case TR::pdsub:
                ...
                break;
            case TR::pdmul:
                operationNode->setDecimalPrecision(
                    precision1 + precision2); // TODO: check +1. +1 because pdshlOverflow will do the overflow check
                break;
            case TR::pddiv:
        ...
        TR::ILOpCodes op = comp()->il.opCodeForIndirectStore(TR::PackedDecimal);
        TR::SymbolReference *symRefPdstore = comp()->getSymRefTab()->findOrCreateArrayShadowSymbolRef(TR::PackedDecimal,
            outOfLineCopyBackAddr, 8, fe());
        TR::Symbol *symStore = TR::Symbol::createShadow(comp()->trHeapMemory(), TR::PackedDecimal,
            TR::DataType::getSizeFromBCDPrecision(TR::PackedDecimal, resPrecNode->getInt()));
        symStore->setArrayShadowSymbol();
        symRefPdstore->setSymbol(symStore);

        TR::Node *pdshlNode
            = TR::Node::create(TR::pdshlOverflow, 2, operationNode, TR::Node::create(callNode, TR::iconst, 0, 0));
        pdshlNode->setDecimalPrecision(resPrecNode->getInt());
        TR::Node *pdstore = TR::Node::create(op, 2, pdStoreAddressNode, pdshlNode);

        TR::SymbolReference *bcdChkSymRef = callNode->getSymbolReference();
        TR::Node *bcdchk = TR::Node::createWithSymRef(TR::BCDCHK, 12, 12, pdshlNode, outOfLineCopyBackAddr,
            callNode->getChild(0), callNode->getChild(1), callNode->getChild(2), callNode->getChild(3),
            callNode->getChild(4), callNode->getChild(5), callNode->getChild(6), callNode->getChild(7),
            callNode->getChild(8), callNode->getChild(9), bcdChkSymRef);
...
bool TR_DataAccessAccelerator::genShiftRightIntrinsic(TR::TreeTop *treeTop, TR::Node *callNode)
{
    ...
    if (!isChildConst(callNode, 2) || !isChildConst(callNode, 5) || !isChildConst(callNode, 7)
        || !isChildConst(callNode, 8))
    ...
    if (failMsg) {
        TR::DebugCounter::incStaticDebugCounter(comp(), TR::DebugCounter::debugCounterName(comp(), "DAA/rejected/shr"));
        return printInliningStatus(false, callNode, failMsg);
    }

    TR::DebugCounter::incStaticDebugCounter(comp(), TR::DebugCounter::debugCounterName(comp(), "DAA/inlined/shr"));
    ...
    TR::SymbolReference *bcdChkSymRef = callNode->getSymbolReference();
    TR::Node *bcdchkNode
        = TR::Node::createWithSymRef(TR::BCDCHK, 11, 11, pdshrNode, outOfLineCopyBackAddr, callNode->getChild(0),
            callNode->getChild(1), callNode->getChild(2), callNode->getChild(3), callNode->getChild(4),
            callNode->getChild(5), callNode->getChild(6), callNode->getChild(7), callNode->getChild(8), bcdChkSymRef);
    ...
    TR::ILOpCodes op = comp()->il.opCodeForIndirectStore(TR::PackedDecimal);
    TR::SymbolReference *symRefPdstore
        = comp()->getSymRefTab()->findOrCreateArrayShadowSymbolRef(TR::PackedDecimal, outOfLineCopyBackAddr, 8, fe());
    TR::Symbol *symStore = TR::Symbol::createShadow(comp()->trHeapMemory(), TR::PackedDecimal,
        TR::DataType::getSizeFromBCDPrecision(TR::PackedDecimal, dstPrec));
    symStore->setArrayShadowSymbol();
    symRefPdstore->setSymbol(symStore);
...
bool TR_DataAccessAccelerator::genShiftLeftIntrinsic(TR::TreeTop *treeTop, TR::Node *callNode)
{
    ...
    if (!isChildConst(callNode, 2) || !isChildConst(callNode, 5) || !isChildConst(callNode, 6)
        || !isChildConst(callNode, 7))
    ...
    if (failMsg) {
        TR::DebugCounter::incStaticDebugCounter(comp(), TR::DebugCounter::debugCounterName(comp(), "DAA/rejected/shl"));
        return printInliningStatus(false, callNode, failMsg);
    }

    TR::DebugCounter::incStaticDebugCounter(comp(), TR::DebugCounter::debugCounterName(comp(), "DAA/inlined/shl"));
    ...
    TR::SymbolReference *symRef
        = comp()->getSymRefTab()->findOrCreateArrayShadowSymbolRef(TR::PackedDecimal, srcAddrNode, 8, fe());
    symRef->setOffset(0);
    pdload->setSymbolReference(symRef);
    pdload->setDecimalPrecision(srcPrec);
    ...
    TR::SymbolReference *bcdChkSymRef = callNode->getSymbolReference();
    TR::Node *bcdchkNode = TR::Node::createWithSymRef(TR::BCDCHK, 10, 10, pdshlNode, outOfLineCopyBackAddr,
        callNode->getChild(0), callNode->getChild(1), callNode->getChild(2), callNode->getChild(3),
        callNode->getChild(4), callNode->getChild(5), callNode->getChild(6), callNode->getChild(7), bcdChkSymRef);
...
bool TR_DataAccessAccelerator::generateUD2PD(TR::TreeTop *treeTop, TR::Node *callNode, bool isSrcUnicodeDecimal)
{
    ...
    else if (isSrcUnicodeDecimal && type != 5 && type != 6 && type != 7)
        failMsg = "Invalid decimal type. Supported types are (5|6|7)";
    else if (!isSrcUnicodeDecimal && (type < 1 || type > 4))
        failMsg = "Invalid decimal type. Supported types are (1|2|3|4)";
    ...
    if (failMsg) {
        TR::DebugCounter::incStaticDebugCounter(comp(),
            TR::DebugCounter::debugCounterName(comp(), "DAA/rejected/ud2pd"));
    ...
    if (performTransformation(comp(), "O^O TR_DataAccessAccelerator: generate UD2PD/ED2PD call: %p inlined.\n",
            callNode)) {
        TR::DebugCounter::incStaticDebugCounter(comp(),
            TR::DebugCounter::debugCounterName(comp(), "DAA/inlined/ud2pd"));

        TR::ILOpCodes loadOp;
        TR::DataType dt = TR::DataTypes::NoType; // unicode data type, as it could be unsigned decimal, sign trailing or
                                                 // sign leading.
        ...
        TR::Node *decimalAddressNode
            = constructAddressNode(callNode, decimalNode, decimalOffsetNode, true, isSrcUnicodeDecimal);

        decimalAddressNode->setIsInternalPointer(true);
        TR::SymbolReference *symRef
            = comp()->getSymRefTab()->findOrCreateArrayShadowSymbolRef(dt, decimalAddressNode, 8, fe());
        symRef->setOffset(0);
        ...
        int elementSize
            = isSrcUnicodeDecimal ? TR::DataType::getUnicodeElementSize() : TR::DataType::getZonedElementSize();
        ...
        TR::Node *pdBndvalue
            = TR::Node::create(TR::iadd, 2, pdOffsetNode, TR::Node::create(callNode, TR::iconst, 0, pdPrecSize));
        TR::Node *decimalBndvalue = TR::Node::create(TR::iadd, 2, decimalOffsetNode,
        ...
        TR::SymbolReference *symRefStore
            = comp()->getSymRefTab()->findOrCreateArrayShadowSymbolRef(TR::PackedDecimal, pdAddressNode, 8, fe());
        TR::Symbol *symStore = TR::Symbol::createShadow(comp()->trHeapMemory(), TR::PackedDecimal,
            TR::DataType::getSizeFromBCDPrecision(TR::PackedDecimal, prec));
        ...
        TR::Node *pdNullChkNode = TR::Node::createWithSymRef(TR::NULLCHK, 1, 1, pdPassThroughNode,
            getSymRefTab()->findOrCreateRuntimeHelper(TR_nullCheck, false, true, true));
        TR::Node *decimalNullChkNode = TR::Node::createWithSymRef(TR::NULLCHK, 1, 1, decimalPassThroughNode,
            getSymRefTab()->findOrCreateRuntimeHelper(TR_nullCheck, false, true, true));

        TR::Node *pdBndChk = TR::Node::createWithSymRef(TR::BNDCHK, 2, 2, pdArrayLengthNode, pdOffsetNode,
            getSymRefTab()->findOrCreateRuntimeHelper(TR_arrayBoundsCheck, false, true, true));
        TR::Node *pdBndChk2 = TR::Node::createWithSymRef(TR::BNDCHK, 2, 2, pdArrayLengthNode, pdBndvalue,
            getSymRefTab()->findOrCreateRuntimeHelper(TR_arrayBoundsCheck, false, true, true));
        TR::Node *decimalBndChk = TR::Node::createWithSymRef(TR::BNDCHK, 2, 2, decimalArrayLengthNode,
            decimalOffsetNode, getSymRefTab()->findOrCreateRuntimeHelper(TR_arrayBoundsCheck, false, true, true));
...
bool TR_DataAccessAccelerator::generatePD2UD(TR::TreeTop *treeTop, TR::Node *callNode, bool isTargetUnicodeDecimal)
{
    ...
    if (!isChildConst(callNode, 4) || !isChildConst(callNode, 5))
        failMsg = "Child (4|5) is not constant";
    else if (isTargetUnicodeDecimal && type != 5 && type != 6 && type != 7)
        failMsg = "Invalid decimal type. Supported types are (5|6|7)";
    else if (!isTargetUnicodeDecimal && (type < 1 || type > 4)) // PD2ED
        failMsg = "Invalid decimal type. Supported types are (1|2|3|4)";
    else if (prec < 1 || prec > 31)
        failMsg = "Invalid precision. Valid precision is in range [1, 31]";
    ...
    if (failMsg) {
        TR::DebugCounter::incStaticDebugCounter(comp(),
            TR::DebugCounter::debugCounterName(comp(), "DAA/rejected/%s", isTargetUnicodeDecimal ? "pd2ud" : "pd2ed"));
    ...
    if (performTransformation(comp(), "O^O TR_DataAccessAccelerator: generate PD2UD/PD2ED call: %p inlined.\n",
            callNode)) {
        TR::DebugCounter::incStaticDebugCounter(comp(),
            TR::DebugCounter::debugCounterName(comp(), "DAA/inlined/%s", isTargetUnicodeDecimal ? "pd2ud" : "pd2ed"));
        ...
        TR::Node *decimalAddressNode
            = constructAddressNode(callNode, decimalNode, decimalOffsetNode, true, isTargetUnicodeDecimal);
        ...
        if (isTargetUnicodeDecimal || type == 1) {
        ...
        TR::SymbolReference *symRefDecimalStore
            = comp()->getSymRefTab()->findOrCreateArrayShadowSymbolRef(dt, decimalAddressNode, 8, fe());
        TR::Symbol *symStore
            = TR::Symbol::createShadow(comp()->trHeapMemory(), dt, TR::DataType::getSizeFromBCDPrecision(dt, prec));
        symStore->setArrayShadowSymbol();
        ...
        TR::Node *decimalPassThroughNode = TR::Node::create(TR::PassThrough, 1, decimalNode);
        int elementSize
            = isTargetUnicodeDecimal ? TR::DataType::getUnicodeElementSize() : TR::DataType::getZonedElementSize();
        int pdPrecSize = TR::DataType::getSizeFromBCDPrecision(TR::PackedDecimal, prec) - 1;
        int decimalPrecSize = (TR::DataType::getSizeFromBCDPrecision(dt, prec) / elementSize) - 1;
        TR::Node *pdBndvalue
            = TR::Node::create(TR::iadd, 2, pdOffsetNode, TR::Node::create(callNode, TR::iconst, 0, pdPrecSize));
...
void TR_DataAccessAccelerator::insertByteArrayNULLCHK(TR::TreeTop *callTreeTop, TR::Node *callNode,
    TR::Node *byteArrayNode)
{
    TR::Compilation *comp = OMR::Optimization::comp();

    callTreeTop->insertBefore(TR::TreeTop::create(comp,
        TR::Node::createWithSymRef(TR::NULLCHK, 1, 1, TR::Node::create(TR::PassThrough, 1, byteArrayNode),
            comp->getSymRefTab()->findOrCreateNullCheckSymbolRef(callNode->getSymbol()->getResolvedMethodSymbol()))));
}

void TR_DataAccessAccelerator::insertByteArrayBNDCHK(TR::TreeTop *callTreeTop, TR::Node *callNode,
    TR::Node *byteArrayNode, TR::Node *offsetNode, int32_t index)
{
    TR::Compilation *comp = OMR::Optimization::comp();
    ...
    callTreeTop->insertBefore(TR::TreeTop::create(comp,
        TR::Node::createWithSymRef(TR::BNDCHK, 2, 2, arraylengthNode, offsetNode,
            comp->getSymRefTab()->findOrCreateArrayBoundsCheckSymbolRef(
