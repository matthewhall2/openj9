...
bool shouldEnableSEL(TR::Compilation *comp)
{
    ...
    return ((enableSEL != NULL) && comp->target().is64Bit());
}
...
static bool isNullCheck(TR::Node *node)
{
    ...
    if (opCode == TR::NULLCHK || opCode == TR::ResolveAndNULLCHK)
        return true;
    return false;
}
...
void TR_SignExtendLoads::InitializeHashTable()
{
    ...
    _sharedNodesHash._buckets
        = (HashTableEntry **)trMemory()->allocateStackMemory(_sharedNodesHash._numBuckets * sizeof(HashTableEntry *));
...
bool TR_SignExtendLoads::gatheri2lNodes(TR::Node *parent, TR::Node *node, TR_ScratchList<TR::Node> &i2lList,
    TR_ScratchList<TR::Node> &useri2lList, bool isIndexSubtree)
{
    ...
    switch (node->getOpCodeValue()) {
        ...
        case TR::isub:
            ...
            if (returnValue && changeNode) {
                ...
                if ((node->getChild(1) == secondChild) && (node->getChild(1)->getOpCodeValue() == secondOpCode))
...
// -------------------------------------------------------------------------------------------
void TR_SignExtendLoads::Propagatei2lNode(TR::Node *i2lNode, TR::Node *parent, int32_t childIndex, bool isIndexSubtree)
{
    /*
...
int32_t TR_SignExtendLoads::perform()
{
    ...
    while (currentTree) {
         ...
         */
        if (nodesGatheredInLastBlock && opCode == TR::BBEnd
            ...
            nodesGatheredInLastBlock
                = gatheri2lNodes(NULL, firstNodeInTree, i2lList, useri2lList, false) || nodesGatheredInLastBlock;
        }
...
const char *TR_SignExtendLoads::optDetailString() const throw() { return "O^O SIGN EXTEND LOADS: "; }
