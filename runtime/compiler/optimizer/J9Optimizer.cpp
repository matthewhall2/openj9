...
static const OptimizationStrategy expensiveObjectAllocationOpts[] = {
    { OMR::eachEscapeAnalysisPassGroup, OMR::IfEAOpportunities },
    { OMR::explicitNewInitialization, OMR::IfNews }, // do before local dead store
    { OMR::endGroup }
};

static const OptimizationStrategy eachEscapeAnalysisPassOpts[] = {
...
static const OptimizationStrategy cheapGlobalValuePropagationOpts[] = {
...
static const OptimizationStrategy expensiveGlobalValuePropagationOpts[] = {
    { OMR::CFGSimplification, OMR::IfOptServer }, // for WAS trace folding
    { OMR::treeSimplification, OMR::IfOptServer }, // for WAS trace folding
    { OMR::localCSE, OMR::IfEnabledAndOptServer }, // for WAS trace folding
...
static const OptimizationStrategy loopAliasRefinerOpts[] = {
...
static const OptimizationStrategy signExtendLoadsOpts[] = { { OMR::signExtendLoads }, { OMR::endGroup } };
...
static const OptimizationStrategy warmStrategyOpts[] = {
    ...
    { OMR::deadTreesElimination, OMR::IfEnabled }, // cleanup at the end
    { OMR::globalCopyPropagation,
     OMR::IfMethodHandleInvokes }, // Does a lot of good after methodHandleInvokeInliningGroup
    { OMR::treesCleansing, OMR::IfEnabled },
    { OMR::deadTreesElimination, OMR::IfEnabled }, // cleanup at the end
    { OMR::localCSE, OMR::IfEnabled }, // common up expressions for sunk stores
    { OMR::treeSimplification, OMR::IfEnabledMarkLastRun }, // cleanup the trees after sunk store and localCSE
    ...
    { OMR::redundantGotoElimination,
     OMR::IfEnabledAndNotJitProfiling }, // dead store and dead tree elimination may have left empty blocks
...
//
// ***************************************************************************
const OptimizationStrategy veryHotStrategyOpts[] = { { OMR::hotStrategy }, { OMR::endOpts } };
...
const OptimizationStrategy scorchingStrategyOpts[] = {
    ...
    { OMR::inductionVariableAnalysis, OMR::IfLoops },
    { OMR::redundantInductionVarElimination, OMR::IfLoops },
    { OMR::loopAliasRefinerGroup,
         OMR::IfLoops }, // version loops to improve aliasing (after versioned to reduce code growth)
    ...
    : TR::FullOptimizer(comp, methodSymbol, isIlGen)
{
    ...
    _opts[OMR::dynamicLiteralPool] = new (comp->allocator())
        TR::OptimizationManager(self(), TR_DynamicLiteralPool::create, OMR::dynamicLiteralPool);
    _opts[OMR::arraycopyTransformation] = new (comp->allocator())
        TR::OptimizationManager(self(), TR::ArraycopyTransformation::create, OMR::arraycopyTransformation);
    _opts[OMR::signExtendLoads]
        = new (comp->allocator()) TR::OptimizationManager(self(), TR_SignExtendLoads::create, OMR::signExtendLoads);
    _opts[OMR::sequentialStoreSimplification] = new (comp->allocator())
        TR::OptimizationManager(self(), TR_SequentialStoreSimplifier::create, OMR::sequentialStoreSimplification);
    _opts[OMR::explicitNewInitialization] = new (comp->allocator())
        TR::OptimizationManager(self(), TR_LocalNewInitialization::create, OMR::explicitNewInitialization);
    _opts[OMR::redundantMonitorElimination] = new (comp->allocator())
        TR::OptimizationManager(self(), TR::MonitorElimination::create, OMR::redundantMonitorElimination);
    ...
    _opts[OMR::postEscapeAnalysis] = new (comp->allocator())
        TR::OptimizationManager(self(), TR_PostEscapeAnalysis::create, OMR::postEscapeAnalysis);
    _opts[OMR::localLiveVariablesForGC] = new (comp->allocator())
        TR::OptimizationManager(self(), TR_LocalLiveVariablesForGC::create, OMR::localLiveVariablesForGC);
    _opts[OMR::globalLiveVariablesForGC] = new (comp->allocator())
        TR::OptimizationManager(self(), TR_GlobalLiveVariablesForGC::create, OMR::globalLiveVariablesForGC);
    _opts[OMR::recompilationModifier] = new (comp->allocator())
        TR::OptimizationManager(self(), TR_RecompilationModifier::create, OMR::recompilationModifier);
    _opts[OMR::profileGenerator]
        = new (comp->allocator()) TR::OptimizationManager(self(), TR_ProfileGenerator::create, OMR::profileGenerator);
    _opts[OMR::dataAccessAccelerator] = new (comp->allocator())
        TR::OptimizationManager(self(), TR_DataAccessAccelerator::create, OMR::dataAccessAccelerator);
    _opts[OMR::stringBuilderTransformer] = new (comp->allocator())
        TR::OptimizationManager(self(), TR_StringBuilderTransformer::create, OMR::stringBuilderTransformer);
    ...
    _opts[OMR::SPMDKernelParallelization] = new (comp->allocator())
        TR::OptimizationManager(self(), TR_SPMDKernelParallelizer::create, OMR::SPMDKernelParallelization);
    _opts[OMR::trivialDeadBlockRemover] = new (comp->allocator())
        TR::OptimizationManager(self(), TR_TrivialDeadBlockRemover::create, OMR::trivialDeadBlockRemover);
    ...
    _opts[OMR::jProfilingRecompLoopTest] = new (comp->allocator())
        TR::OptimizationManager(self(), TR_JProfilingRecompLoopTest::create, OMR::jProfilingRecompLoopTest);
    ...
    _opts[OMR::vectorAPIExpansion] = new (comp->allocator())
        TR::OptimizationManager(self(), TR_VectorAPIExpansion::create, OMR::vectorAPIExpansion);
    _opts[OMR::catchBlockProfiler] = new (comp->allocator())
        TR::OptimizationManager(self(), TR::CatchBlockProfiler::create, OMR::catchBlockProfiler);
    // NOTE: Please add new J9 optimizations here!

    // initialize additional J9 optimization groups

    _opts[OMR::loopAliasRefinerGroup] = new (comp->allocator())
        TR::OptimizationManager(self(), NULL, OMR::loopAliasRefinerGroup, loopAliasRefinerOpts);
    _opts[OMR::cheapObjectAllocationGroup] = new (comp->allocator())
        TR::OptimizationManager(self(), NULL, OMR::cheapObjectAllocationGroup, cheapObjectAllocationOpts);
    _opts[OMR::expensiveObjectAllocationGroup] = new (comp->allocator())
        TR::OptimizationManager(self(), NULL, OMR::expensiveObjectAllocationGroup, expensiveObjectAllocationOpts);
    _opts[OMR::eachEscapeAnalysisPassGroup] = new (comp->allocator())
        TR::OptimizationManager(self(), NULL, OMR::eachEscapeAnalysisPassGroup, eachEscapeAnalysisPassOpts);
    _opts[OMR::cheapGlobalValuePropagationGroup] = new (comp->allocator())
        TR::OptimizationManager(self(), NULL, OMR::cheapGlobalValuePropagationGroup, cheapGlobalValuePropagationOpts);
    _opts[OMR::expensiveGlobalValuePropagationGroup] = new (comp->allocator()) TR::OptimizationManager(self(), NULL,
        OMR::expensiveGlobalValuePropagationGroup, expensiveGlobalValuePropagationOpts);
    ...
    _opts[OMR::sequentialStoreSimplificationGroup] = new (comp->allocator()) TR::OptimizationManager(self(), NULL,
        OMR::sequentialStoreSimplificationGroup, sequentialStoreSimplificationOpts);
    ...
    _opts[OMR::sequentialLoadAndStoreColdGroup] = new (comp->allocator())
        TR::OptimizationManager(self(), NULL, OMR::sequentialLoadAndStoreColdGroup, sequentialLoadAndStoreColdOpts);
    _opts[OMR::sequentialLoadAndStoreWarmGroup] = new (comp->allocator())
        TR::OptimizationManager(self(), NULL, OMR::sequentialLoadAndStoreWarmGroup, sequentialLoadAndStoreWarmOpts);
...
OMR_InlinerPolicy *J9::Optimizer::getInlinerPolicy() { return new (comp()->allocator()) TR_J9InlinerPolicy(comp()); }

OMR_InlinerUtil *J9::Optimizer::getInlinerUtil() { return new (comp()->allocator()) TR_J9InlinerUtil(comp()); }

bool J9::Optimizer::switchToProfiling(uint32_t f, uint32_t c)
{
    TR::Recompilation *recomp = comp()->getRecompilationInfo();
    if (!recomp)
        return false;
    if (!recomp->shouldBeCompiledAgain())
        return false; // do not profile if do not intend to compile again
    if (!recomp->switchToProfiling(f, c))
        return false;
...
bool J9::Optimizer::switchToProfiling()
{
