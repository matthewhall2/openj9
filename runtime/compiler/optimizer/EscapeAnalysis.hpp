...
typedef TR::typed_allocator<std::pair<TR::Node * const, std::pair<TR_BitVector *, TR_BitVector *> >, TR::Region &>
    CallLoadMapAllocator;
typedef std::less<TR::Node *> CallLoadMapComparator;
typedef std::map<TR::Node *, std::pair<TR_BitVector *, TR_BitVector *>, CallLoadMapComparator, CallLoadMapAllocator>
    CallLoadMap;
...
typedef std::map<TR::Node *, int32_t, RemainingUseCountMapComparator, RemainingUseCountMapAllocator>
    RemainingUseCountMap;
...
public:
    Candidate(TR::Node *node, TR::TreeTop *treeTop, TR::Block *block, int32_t size, void *classInfo, TR::Compilation *c)
        : _kind(node->getOpCodeValue())
        , _node(node)
        , _treeTop(treeTop)
        , _origKind(node->getOpCodeValue())
        , _stringCopyNode(NULL)
        , _stringCopyCallTree(NULL)
        , _block(block)
        , _class(classInfo)
        ...
        , _initializedWords(0)
        , _maxInlineDepth(0)
        , _inlineBytecodeSize(0)
        , _seenFieldStore(false)
        , _seenSelfStore(false)
        , _seenStoreToLocalObject(false)
        , _seenArrayCopy(false)
        , _argToCall(false)
        , _usedInNonColdBlock(false)
        , _lockedInNonColdBlock(false)
        , _isImmutable(false)
        , _originalAllocationNode(0)
        , _lockedObject(false)
        , _index(-1)
        , _flushRequired(true)
        , _comp(c)
    ...
    void setArgToCall(int32_t depth, bool v = true)
    {
        _argToCallFlags.set(1 << depth, v);
        if (v == true)
            _argToCall = true;
    }
    ...
    void addColdBlockEscapeInfo(TR::Block *block, TR::Node *node, TR::TreeTop *tree)
    {
        ...
        if (curColdBlockInfo) {
            ...
            TR_ColdBlockEscapeInfo *coldBlockInfo
                = new (trStackMemory()) TR_ColdBlockEscapeInfo(block, node, tree, trMemory());
            _coldBlockEscapeInfo.add(coldBlockInfo);
        }
    }

    FieldInfo &findOrSetFieldInfo(TR::Node *fieldRefNode, TR::SymbolReference *symRef, int32_t fieldOffset,
...
public:
    FlushCandidate(TR::TreeTop *flushNode, TR::Node *node, int32_t blockNum, Candidate *candidate = 0)
        : _flushNode(flushNode)
        , _node(node)
        , _blockNum(blockNum)
        , _candidate(candidate)
        , _isKnownToLackCandidate(false)
        , _optimallyPlaced(false)
    {}
...
public:
    ...
    static bool isInCache(TR_LinkHead<SniffCallCache> *sniffCacheList, TR_ResolvedMethod *vmMethod, bool isCold,
...
protected:
    ...
    bool collectValueNumbersOfIndirectAccessesToObject(TR::Node *node, Candidate *candidate, TR::Node *indirectStore,
        TR::NodeChecklist &visited, int32_t baseChildVN = -1);
    ...
    bool checkOverlappingLoopAllocation(TR::Node *node, TR::Node *useNode, TR::Node *allocNode,
    ...
    bool checkIfUseIsInLoopAndOverlapping(TR::TreeTop *start, TR::TreeTop *end, TR::TreeTop *defTree, TR::Node *useNode,
        TR::NodeChecklist &visited, TR::BlockChecklist &vBlocks, bool &decisionMade);
    ...
    int32_t sniffCall(TR::Node *callNode, TR::ResolvedMethodSymbol *methodSymbol, bool ignoreOpCode, bool isCold,
        bool &ignoreRecursion);
    ...
    bool hasFlushOnEntry(int32_t blockNum)
    {
        if (_blocksWithFlushOnEntry->get(blockNum))
            return true;
        return false;
    }
    ...
    struct TR_CallSitesFixedMapper : public TR_Link<TR_CallSitesFixedMapper> {
        TR_CallSitesFixedMapper(TR::TreeTop *virtualCallSite, TR::TreeTop *directCallSite)
            : _vCallSite(virtualCallSite)
            , _dCallSite(directCallSite)
        {}
