...
int32_t TR_ProfileGenerator::perform()
{
    ...
    if (nodeCount > MAX_NUMBER_OF_ALLOWED_NODES) {
        ...
        if (!comp()->getOption(TR_DisableSwitchAwayFromProfilingForHotAndVeryhot))
            switchAwayFromProfilingForHotAndVeryhot
                = (actualNodeCount + actualNodeCount) > comp()->getOptions()->getProfilingCompNodecountThreshold()
                && (comp()->getMethodHotness() == hot
                    || (comp()->getMethodHotness() == veryHot
                        && comp()->isCpuExpensiveCompilation(
                               (int64_t)1000000 * comp()->getOptions()->getCpuCompTimeExpensiveThreshold())
                            == TR_yes));
        if (!comp()->getOption(TR_ProcessHugeMethods)
            && (((actualNodeCount + actualNodeCount) > USHRT_MAX)

                // we are avoiding cloning in more cases conservatively for profiling hot, only when double the actual
                // node count is larger than a threshold this is even more conservative for profiling very hot, where we
                // also consider the actual CPU time spent in compilation. the conditions below may be simplified
                // further by only consider CPU time is expensive or not.
                || switchAwayFromProfilingForHotAndVeryhot))

        {
            if (comp()->getMethodHotness() != hot && comp()->getMethodHotness() != veryHot
                && !switchAwayFromProfilingForHotAndVeryhot)
            ...
            for (treeTop = comp()->getStartTree(); treeTop; treeTop = treeTop->getNextTreeTop()) {
                ...
                if (node->isProfilingCode()) {
                    if (node->getFirstChild()->getOpCode().isCall()) {
                        // For value profiling call nodes we cannot simply unlink the tree because we hold on to a
                        // reference of the value node we are to profile. As such unlinking this treetop could cause the
                        // potentially commoned value node to swing down to its next use which can be functionally
                        // incorrect. Instead we eliminate the value profiling call and we anchor the value node to a
                        // tree top and let dead tree elimination clean it up.
            ...
            TR_ASSERT(comp()->getRecompilationInfo(),
                "recompilationInfo must exist because profiling compilations are followed by recompilations");
    ...
    if (profileInfo && profileInfo->getProfilingFrequency() == DEFAULT_PROFILING_FREQUENCY
...
const char *TR_ProfileGenerator::optDetailString() const throw() { return "O^O PROFILE GENERATOR: "; }
...
int32_t TR_ProfileGenerator::prepareBlocks()
{
    ...
    treeTop = TR::TreeTop::create(comp(), block->getEntry(),
        TR::Node::createWithSymRef(node, TR::asynccheck, 0,
...
void TR_ProfileGenerator::createProfiledMethod()
{
    ...
    _lastOriginalTree->join(
        cloner.cloneBlocks(_firstOriginalTree->getNode()->getBlock(), _lastOriginalTree->getNode()->getBlock())
            ->getEntry());
    ...
    TR::SymbolReference *freqPtrSymRef
    ...
    TR::SymbolReference *cntrPtrSymRef
        = comp()->getSymRefTab()->createTemporary(comp()->getMethodSymbol(), TR::Address);
    ...
    for (treeTop = _firstOriginalTree; treeTop != _lastOriginalTree; treeTop = nextTreeTop) {
        ...
        if (node->getOpCodeValue() == TR::BBStart) {
            ...
            if (firstSplitPoint) {
                ...
                newNode = TR::Node::createif(TR::ifiucmpge, rcNode,
                    TR::Node::create(node, TR::iconst, 0, PROFILING_INVOCATION_COUNT), blockO1c->getEntry());
                ...
                TR::Node *offsetNode
                    = TR::Node::create(TR::imul, 2, TR::Node::createWithSymRef(node, TR::iload, 0, tempRcSymRef),
                ...
                TR::Node *sumNode = TR::Node::create(opCode, 2,
                    TR::Node::createWithSymRef(node, TR::aload, 0, freqPtrSymRef), offsetNode);
                newNode = TR::Node::createWithSymRef(TR::astore, 1, 1, sumNode, freqPtrSymRef);
                treeTop = TR::TreeTop::create(comp(), treeTop, newNode);

                sumNode = TR::Node::create(opCode, 2, TR::Node::createWithSymRef(node, TR::aload, 0, cntrPtrSymRef),
                    offsetNode);
            ...
            TR::Node *freqNode = TR::Node::createWithSymRef(TR::iloadi, 1, 1, freqPtrNode, tSymRef);
            ...
            newNode = TR::Node::createif(TR::ificmpeq, freqNode, TR::Node::create(node, TR::iconst, 0, INT_MAX),
                blockO1a->getEntry());
            TR::TreeTop::create(comp(), treeTop, newNode);
            if (regDeps) {
                newNode->setNumChildren(3);
            ...
            if (blockO1Cont->isExtensionOfPreviousBlock()) {
                ...
                freqNode = TR::Node::createWithSymRef(TR::iloadi, 1, 1, freqPtrNode, tSymRef);
            }

            // decrement
            TR::Node *decrNode = TR::Node::create(TR::iadd, 2, freqNode, TR::Node::create(node, TR::iconst, 0, -1));
            // copy data back to *freqPtr
            newNode = TR::Node::createWithSymRef(TR::istorei, 2, 2, freqPtrNode, decrNode, tSymRef);
            treeTop = TR::TreeTop::create(comp(), treeTop, newNode);

            // test value that we just decremented
            newNode = TR::Node::createif(TR::ificmple, decrNode, TR::Node::create(node, TR::iconst, 0, 0),
                blockC1b->getEntry());
            TR::TreeTop::create(comp(), treeTop, newNode);
            if (regDeps) {
                newNode->setNumChildren(3);
            ...
            //
            // tempFreq = INT_MAX;
            newNode = TR::Node::createWithSymRef(TR::istore, 1, 1, TR::Node::create(node, TR::iconst, 0, INT_MAX),
                tempFreqSymRef);
            ...
            // tempFreq = VALUE (47)
            newNode = TR::Node::createWithSymRef(TR::istore, 1, 1,
                TR::Node::create(node, TR::iconst, 0, recompilationInfo->getProfilingFrequency()), tempFreqSymRef);
            ...
            TR::Node *cntrNode = TR::Node::createWithSymRef(TR::iloadi, 1, 1, cntrPtrNode, tSymRef);
            // decrement it
            decrNode = TR::Node::create(TR::iadd, 2, cntrNode, TR::Node::create(node, TR::iconst, 0, -1));
            // copy data back to *cntrPtr
            newNode = TR::Node::createWithSymRef(TR::istorei, 2, 2, cntrPtrNode, decrNode, tSymRef);

            treeTop = TR::TreeTop::create(comp(), treeTop, newNode);

            newNode = TR::Node::createif(TR::ificmpgt, decrNode, TR::Node::create(node, TR::iconst, 0, 0),
                blockC1a->getEntry());
            treeTop = TR::TreeTop::create(comp(), treeTop, newNode);
            if (regDeps) {
                newNode->setNumChildren(3);
            ...
            newNode = TR::Node::createWithSymRef(TR::istorei, 2, 2,
                ...
                TR::Node::createWithSymRef(node, TR::iload, 0, tempFreqSymRef), tSymRef);
            ...
            if (recompilationInfo->getProfilingFrequency() == 1) {
                ...
                TR::Node *freqPtrNode = TR::Node::createWithSymRef(node, TR::aload, 0, freqPtrSymRef);
                TR::Node *freqNode = TR::Node::createWithSymRef(TR::iloadi, 1, 1, freqPtrNode, tSymRef);
                newNode = TR::Node::createif(TR::ificmpgt, freqNode,
