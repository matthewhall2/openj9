...
public:
    ...
    TR_CISCNode(TR_Memory *m, uint32_t opc, TR::DataType dt, uint16_t id, int16_t dagId, uint16_t ncfgs,
        uint16_t nchildren, TR_AllocationKind allocKind = heapAlloc)
    ...
    TR_CISCNode(TR_Memory *m, uint32_t opc, TR::DataType dt, uint16_t id, int16_t dagId, uint16_t ncfgs,
        uint16_t nchildren, TR_CISCNode *pred, TR_AllocationKind allocKind = heapAlloc)
        : _allocKind(allocKind)
        , _trMemory(m)
        , _preds(m)
        , _parents(m)
    ...
    TR_CISCNode(TR_Memory *m, uint32_t opc, TR::DataType dt, uint16_t id, int16_t dagId, uint16_t ncfgs,
        uint16_t nchildren, TR_CISCNode *pred, TR_CISCNode *child0, TR_AllocationKind allocKind = heapAlloc)
        : _allocKind(allocKind)
        , _trMemory(m)
        , _preds(m)
        , _parents(m)
    ...
    TR_CISCNode(TR_Memory *m, uint32_t opc, TR::DataType dt, uint16_t id, int16_t dagId, uint16_t ncfgs,
        uint16_t nchildren, TR_CISCNode *pred, TR_CISCNode *child0, TR_CISCNode *child1,
    ...
    TR_CISCNode(TR_Memory *m, uint32_t opc, TR::DataType dt, uint16_t id, int16_t dagId, uint16_t ncfgs,
        uint16_t nchildren, int32_t otherInfo, TR_AllocationKind allocKind = heapAlloc)
        : _allocKind(allocKind)
        , _trMemory(m)
        , _preds(m)
        , _parents(m)
    ...
    TR_CISCNode(TR_Memory *m, uint32_t opc, TR::DataType dt, uint16_t id, int16_t dagId, uint16_t ncfgs,
        uint16_t nchildren, int32_t otherInfo, TR_CISCNode *pred, TR_AllocationKind allocKind = heapAlloc)
        : _allocKind(allocKind)
        , _trMemory(m)
        , _preds(m)
        , _parents(m)
    ...
    void setSucc(TR_CISCNode *ch) { setSucc(0, ch); }
    ...
    void setChild(TR_CISCNode *ch) { setChild(0, ch); }
    ...
    bool isDataConnected() { return _flags.testAll(_isChildSimplyConnected | _isParentSimplyConnected); }

    bool isCFGConnected() { return _flags.testAll(_isSuccSimplyConnected | _isPredSimplyConnected); }

    bool isCFGDataConected()
    {
        return _flags.testAll(
            _isSuccSimplyConnected | _isPredSimplyConnected | _isChildSimplyConnected | _isParentSimplyConnected);
    }
    ...
    void dump(OMR::Logger *log, TR::Compilation *comp);
    void printStdout();
    ...
    virtual void allocArrays(uint16_t ncfgs, uint16_t nchildren)
    {
        ...
        _children
            = (TR_CISCNode **)(nchildren ? trMemory()->allocateMemory(nchildren * sizeof(*_children), _allocKind) : 0);
    }
    ...
    void setDest(TR_CISCNode *v)
    {
        ...
        if (v->_latestDest) {
            TR_ASSERT(v->_latestDest->_dest.find(v),
                "TR_CISCNode::setDest(), input variable is not found in _latestDest");
    ...
    TR_CISCNode *getFirstDest() { return _dest.isEmpty() ? 0 : _dest.getListHead()->getData(); }
    ...
    {
        TR_CISCNode *ret
            = new (trHeapMemory()) TR_CISCNode(_trMemory, _opcode, _dt, _id, _dagId, _numSuccs, _numChildren);
        ...
        for (TrNodeInfo *t = li.getFirst(); t; t = li.getNext())
            appender.add(t);
...
public:
    TR_PCISCNode(TR_Memory *m, uint32_t opc, TR::DataType dt, uint16_t id, int16_t dagId, uint16_t ncfgs,
        uint16_t nchildren)
        : TR_CISCNode(m, opc, dt, id, dagId, ncfgs, nchildren, persistentAlloc) {};

    TR_PCISCNode(TR_Memory *m, uint32_t opc, TR::DataType dt, uint16_t id, int16_t dagId, uint16_t ncfgs,
        uint16_t nchildren, TR_CISCNode *pred)
        : TR_CISCNode(m, opc, dt, id, dagId, ncfgs, nchildren, pred, persistentAlloc)
    {}

    TR_PCISCNode(TR_Memory *m, uint32_t opc, TR::DataType dt, uint16_t id, int16_t dagId, uint16_t ncfgs,
        uint16_t nchildren, TR_CISCNode *pred, TR_CISCNode *child0)
    ...
    TR_PCISCNode(TR_Memory *m, uint32_t opc, TR::DataType dt, uint16_t id, int16_t dagId, uint16_t ncfgs,
        uint16_t nchildren, int32_t otherInfo)
        : TR_CISCNode(m, opc, dt, id, dagId, ncfgs, nchildren, otherInfo, persistentAlloc)
    {}

    TR_PCISCNode(TR_Memory *m, uint32_t opc, TR::DataType dt, uint16_t id, int16_t dagId, uint16_t ncfgs,
        uint16_t nchildren, int32_t otherInfo, TR_CISCNode *pred)
        : TR_CISCNode(m, opc, dt, id, dagId, ncfgs, nchildren, otherInfo, pred, persistentAlloc)
    {}

    TR_PCISCNode *getSucc(uint32_t index) { return (TR_PCISCNode *)TR_CISCNode::getSucc(index); }

    TR_PCISCNode *getChild(uint32_t index) { return (TR_PCISCNode *)TR_CISCNode::getChild(index); }
...
public:
    ...
    {
        _ifCount = ifCount;
        _indirectLoadCount = iloadCount;
        _indirectStoreCount = istoreCount;
    }
    ...
    {
        return (_ifCount >= p->_ifCount && _indirectLoadCount >= p->_indirectLoadCount
...
public:
    ...
    List<T> *getList()
    {
        TR_ASSERT(isRegistered(), "error");
        return _list;
    }

    virtual List<T> *duplicateList(bool ifNecessary = true)
    {
        if (ifNecessary && isDuplicated())
            return _list;
        setDuplicated();
...
public:
    ListOfCISCNodeDuplicator(TR_Memory *m)
        : TR_ListDuplicator<TR_CISCNode>(m)
        , _mapping(m) {};
        ...
        if (isDuplicateElement()) {
            ...
            for (t = li.getFirst(); t; t = li.getNext()) {
                ...
                TR_Pair<TR_CISCNode, TR_CISCNode> *pair
                    = new (trHeapMemory()) TR_Pair<TR_CISCNode, TR_CISCNode>(t, newCISC);
                _mapping.add(pair);
            }
    ...
    void restructureList(List<TR_CISCNode> *oldList, List<TR_CISCNode> *newList)
    {
        ...
        TR_CISCNode *t;
        for (t = li.getFirst(); t; t = li.getNext())
            appender.add(findCorrespondingNode(t));
    }
...
public:
        ...
        , _patternType(-1)
    {
        ...
        _aspects.init();
        if (numHashTrNode > 0)
            _trNode2CISCNode.setNumBuckets(numHashTrNode);
        if (numHashOpc > 0)
            _opc2CISCNode.setNumBuckets(numHashOpc);
        int32_t i;
        for (i = MAX_IMPORTANT_NODES; --i >= 0;)
            _importantNode[i] = 0;
        for (i = MAX_SPECIALCARE_NODES; --i >= 0;)
            _specialCareNode[i] = 0;
    }
    ...
    void setImportantNodes(TR_CISCNode *n1, TR_CISCNode *n2, TR_CISCNode *n3, TR_CISCNode *n4, TR_CISCNode *n5,
        TR_CISCNode *n6, TR_CISCNode *n7, TR_CISCNode *n8)
    {
        setImportantNode(0, n1);
        setImportantNode(1, n2);
        setImportantNode(2, n3);
    ...
    TR_CISCNode *getHeadOfNodes() { return _nodes.isEmpty() ? 0 : _nodes.getListHead()->getData(); }
    ...
    bool isDagIdCycle(uint16_t dagId)
    {
        TR_ASSERT(dagId < _numDagIds, "error");
        return getDagId2Nodes()[dagId].isMultipleEntry();
    }

    bool isDagIdDag(uint16_t dagId)
    {
        TR_ASSERT(dagId < _numDagIds, "error");
        return getDagId2Nodes()[dagId].isSingleton();
    }
    ...
    bool isInhibitAfterVersioning()
    {
        TR_ASSERT(isPatternGraph(), "error");
        return _flags.testAny(_inhibitAfterVersioning);
    }

    void setInhibitAfterVersioning(bool v = true)
    {
        TR_ASSERT(isPatternGraph(), "error");
        _flags.set(_inhibitAfterVersioning, v);
    }

    bool isInhibitBeforeVersioning()
    {
        TR_ASSERT(isPatternGraph(), "error");
        return _flags.testAny(_inhibitBeforeVersioning);
    }

    void setInhibitBeforeVersioning(bool v = true)
    {
        TR_ASSERT(isPatternGraph(), "error");
        _flags.set(_inhibitBeforeVersioning, v);
    }

    bool isInsideOfFastVersioned()
    {
        TR_ASSERT(!isPatternGraph(), "error");
        return _flags.testAny(_insideOfFastVersioned);
    }

    void setInsideOfFastVersioned(bool v = true)
    {
        TR_ASSERT(!isPatternGraph(), "error");
        _flags.set(_insideOfFastVersioned, v);
    }

    bool isRequireAHconst()
    {
        TR_ASSERT(isPatternGraph(), "error");
        return _flags.testAny(_requireAHconst);
    }

    void setRequireAHconst(bool v = true)
    {
        TR_ASSERT(isPatternGraph(), "error");
        _flags.set(_requireAHconst, v);
    }
    ...
    void setHotness(TR_Hotness hotness, bool highFreq)
    {
        setHotness(hotness);
        setHighFrequency(highFreq);
    }
    ...
    void setMinCounts(uint8_t ifCount, uint8_t iloadCount, uint8_t istoreCount)
    {
        _aspects.setMinCounts(ifCount, iloadCount, istoreCount);
    }
    ...
    void duplicateListsDuplicator() { _duplicatorNodes.duplicateList(); }
...
protected:
    ...
    bool addTrNode2CISCNode(TR::Node *n, TR_CISCNode *c) { return _trNode2CISCNode.add(getHashKeyTrNode(n), c, true); }
...
public:
    ...
    {
        if (numHashTrNode > 0)
            _trNode2CISCNode.setNumBuckets(numHashTrNode, persistentAlloc);
        if (numHashOpc > 0)
            _opc2CISCNode.setNumBuckets(numHashOpc, persistentAlloc);
    };
...
public:
    ...
    TR_CISCNodeRegion(int32_t bvnum, TR::Region &region)
        : ListHeadAndTail<TR_CISCNode>(region)
    {
        init(bvnum, region);
    }
    ...
    {
        ListHeadAndTail<TR_CISCNode>::init();
        _flags.clear();
        _bvnum = bvnum;
        _bv.init(bvnum, region);
    }
    ...
    {
        if (p->isEssentialNode())
            setIncludeEssentialNode();
        if (p->isOptionalNode())
            setIsOptionalNode();
        _bv.set(p->getID());
...
public:
    ...
    TR_CFGReversePostOrder(TR_Memory *m)
        : _revPost(m)
    {}
...
public:
    ...
    void add(TR_CISCGraph *idiom, int32_t minBC, int32_t maxBC, int32_t minLN, int32_t maxLN)
    {
        ...
        if (_minBCIndex > minBC)
            _minBCIndex = minBC;
        if (_maxBCIndex < maxBC)
            _maxBCIndex = maxBC;
        if (_minLineNumber > minLN)
            _minLineNumber = minLN;
        if (_maxLineNumber < maxLN)
            _maxLineNumber = maxLN;
    }
...
public:
    ...
    TR_NodeDuplicator(TR::Compilation *comp)
        : _list(comp->trMemory())
        , _comp(comp)
    {
        init();
...
public:
    ...
    static bool searchNodeInTrees(TR::Node *top, TR::Node *target, TR::Node **retParent = NULL,
        int *retChildNum = NULL);
    static bool compareTrNodeTree(TR::Node *a, TR::Node *b);
    static bool compareBlockTrNodeTree(TR::Block *a, TR::Block *b);
    bool getBCIndexMinMax(List<TR_CISCNode> *l, int32_t *minIndex, int32_t *maxIndex, int32_t *_minLN, int32_t *_maxLN,
        bool allowInlined);
    ...
    TR_CISCNode *addAllSubNodes(TR_CISCGraph * const graph, TR::Block * const block, TR::TreeTop * const top,
    ...
    bool makeLists();
    int32_t analyzeConnectionOnePairChild(TR_CISCNode * const p, TR_CISCNode * const t, TR_CISCNode * const pn,
        TR_CISCNode *tn);
    ...
    bool analyzeBoolTable(TR_BitVector **bv, TR::TreeTop **retTreeTop, TR_CISCNode *boolTable, TR_BitVector *defBV,
        TR_CISCNode *defNode, TR_CISCNode *ignoreNode, int32_t bvoffset, int32_t allocBVSize);
    int32_t analyzeByteBoolTable(TR_CISCNode *booltable, uint8_t *table256, TR_CISCNode *ignoreNode = 0,
        TR::TreeTop **retSameExit = 0);
    int32_t analyzeCharBoolTable(TR_CISCNode *booltable, uint8_t *table65536, TR_CISCNode *ignoreNode = 0,
        TR::TreeTop **retSameExit = 0);
    ...
    ListHeadAndTail<TR::Node> *getAfterInsertionIdiomList(int32_t pos)
    {
        TR_ASSERT(pos < 2, "error");
        return _afterInsertionsIdiom + pos;
    }
    ...
    enum {
        ...
        _Embed = (_Desc
            | 0x4) // (1) the labels of p and t are equivalent and (2) descendents of t include all children of p.
    };

    static inline bool isDescOrEmbed(uint8_t status)
    {
        return ((status & _Desc) == _Desc);
    } // Assume _Desc=3 and _Embed=7
    ...
    TR::Block *modifyBlockByVersioningCheck(TR::Block *block, TR::TreeTop *startTop, TR::Node *lengthNode,
        List<TR::Node> *guardList = NULL);
