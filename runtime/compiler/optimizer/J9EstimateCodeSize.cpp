...
public:
    ...
    NeedsPeekingHeuristic(TR_CallTarget *calltarget, TR_J9ByteCodeIterator &bci, TR::ResolvedMethodSymbol *methodSymbol,
        TR::Compilation *comp, int d = default_distance)
        ...
        , _tracer(0)
    {
        ...
        for (TR::ParameterSymbol *p = parmIt.getFirst(); p; p = parmIt.getNext(), i++) {
            ...
            TR_OpaqueClassBlock *clazz
                = comp->fej9()->getClassFromSignature(sig, len, methodSymbol->getResolvedMethod());
            if (!clazz) {
                continue;
            }
            ...
            // should fire
            TR_ASSERT(comp->fej9()->isInstanceOf(argClass, clazz, true, true, true) == TR_yes || i != 0
                    || !calltarget->_myCallSite->_isIndirectCall,
                "Incompatible receiver should have been handled by findCallSiteTarget");
            ...
            if (!keepBogusPeekingCondition
                || clazz != argClass) // if two classes aren't equal it follows that argClass is more specific
    ...
    {
        _tracer = trc;
        heuristicTraceIfTracerIsNotNull(_tracer,
            "NeedsPeekingHeuristic is initialized with the following values: _hasArgumentsInfo = %d, NUM_LOADS = %d, "
            "_distance =%d, _needsPeeking = %d",
            _hasArgumentsInfo, NUM_LOADS, _distance, _needsPeeking);
    }
    ...
    void checkIfThereIsAParmLoadWithinDistance()
    {
        for (int i = 0; i < _size; i++) {
            if (_bci.bcIndex() - _loadIndices[i] <= _distance) {
                heuristicTraceIfTracerIsNotNull(_tracer,
                    "There is a parm load at %d which is within %d of a call at %d. Setting needs peeking to true and "
                    "skipping any further param load check.",
                    _loadIndices[i], _distance, _bci.bcIndex());
    ...
    void setNeedsPeekingToTrue() { _needsPeeking = true; }
    ...
    void processParameterLoad(int slotIndex)
    {
        ...
        if (slotIndex < _numOfArgs && _argInfo->get(slotIndex)) {
            heuristicTraceIfTracerIsNotNull(_tracer, "came across of a load of slot %d at %d", slotIndex,
                _bci.bcIndex());
...
void TR_J9EstimateCodeSize::setupNode(TR::Node *node, uint32_t bcIndex, TR_ResolvedMethod *feMethod,
    TR::Compilation *comp)
{
...
TR::Block *TR_J9EstimateCodeSize::getBlock(TR::Compilation *comp, TR::Block **blocks, TR_ResolvedMethod *feMethod,
    int32_t i, TR::CFG &cfg)
{
    if (!blocks[i]) {
...
void TR_J9EstimateCodeSize::setupLastTreeTop(TR::Block *currentBlock, TR_J9ByteCode bc, uint32_t bcIndex,
    TR::Block *destinationBlock, TR_ResolvedMethod *feMethod, TR::Compilation *comp)
{
...
// Partial Inlining
bool TR_J9EstimateCodeSize::isInExceptionRange(TR_ResolvedMethod *feMethod, int32_t bcIndex)
{
...
bool TR_J9EstimateCodeSize::adjustEstimateForStringCompression(TR_ResolvedMethod *method, int32_t &value, float factor)
{
    ...
    if ((classNameLength == 16 && !strncmp(method->classNameChars(), "java/lang/String", classNameLength))
        ...
        // A statistical analysis of the number of places certain methods got inlined yielded results which suggest that
        // the following recognized methods incurr several percent worth of increase in compile-time at no benefit to
        // throughput. As such we can save additional compile-time by not making adjustments to these methods.
 ...
 */
bool TR_J9EstimateCodeSize::adjustEstimateForMethodInvoke(TR_ResolvedMethod *method, int32_t &value, float factor)
{
...
bool TR_J9EstimateCodeSize::adjustEstimateForConstArgs(TR_CallTarget *target, int32_t &value, float factor)
{
    ...
    if (((TR_J9InlinerPolicy *)_inliner->getPolicy())->validateArguments(target, argMap)) {
        ...
        for (TR_ParameterMapping *parm = argMap.getFirst(); parm; parm = parm->getNext()) {
            ...
            if (prexArgInfo)
                prexArg = prexArgInfo->get(argIndex++);
            ...
            if (parmNode->getOpCode().hasSymbolReference() && parmNode->getSymbolReference()->getSymbol()->isStatic()
                && parmNode->getSymbolReference()->getSymbol()->castToStaticSymbol()->isConstString()) {
                value *= factor;
                heuristicTrace(tracer(), "Setting size from %d to %d because arg is constant string.", interimWeight,
                    value);
            } else if (argClassName && parmClassName
                && strncmp(argClassName, parmClassName, parmClassNameLen)
                    != 0 // arg type needs to be more specific than declared type to benefit from weight adjustment
            ...
            } else if (parmNode->getOpCodeValue() == TR::aload && parmNode->getSymbolReference()
                && parmNode->getSymbolReference()->getSymbol()->isConstObjectRef()) {
                value *= factor;
                heuristicTrace(tracer(), "Setting size from %d to %d because arg is const object ref.", interimWeight,
                    value);
            } else if (parmNode->getOpCodeValue() == TR::aloadi && parmNode->getOpCode().hasSymbolReference()
                && parmNode->getSymbolReference() == comp()->getSymRefTab()->findJavaLangClassFromClassSymbolRef()
                && parmNode->getFirstChild() && parmNode->getFirstChild()->getOpCodeValue() == TR::loadaddr
                ...
                value *= factor;
                heuristicTrace(tracer(), "Setting size from %d to %d because arg is const class ref.", interimWeight,
                    value);
            } else if (argClassName && parmClassName && strncmp(argClassName, parmClassName, parmClassNameLen) != 0
                && strcmp(argClassName, "Ljava/lang/Class;") == 0) {
                value *= lessAggressiveAdjustmentFactor;
                heuristicTrace(tracer(), "Setting size from %d to %d because arg is a class ref.", interimWeight,
                    value);
            } else if (enableLoadConstArgWeightAdjustment && parmNode->getOpCode().isLoadConst()) {
                value *= factor;
                heuristicTrace(tracer(), "Setting size from %d to %d because arg is load const.", interimWeight, value);
            } else if (enableLoadConstArgWeightAdjustment && argClassName && parmClassName
                && strncmp(argClassName, parmClassName, parmClassNameLen) != 0
                && (strcmp(argClassName, "Ljava/lang/Integer;") == 0 || strcmp(argClassName, "Ljava/lang/Long;") == 0
                    || strcmp(argClassName, "Ljava/lang/Byte;") == 0 || strcmp(argClassName, "Ljava/lang/Double;") == 0
                    || strcmp(argClassName, "Ljava/lang/Float;") == 0 || strcmp(argClassName, "Ljava/lang/Short;") == 0
                    || strcmp(argClassName, "Ljava/lang/Boolean;") == 0
                    || strcmp(argClassName, "Ljava/lang/Character;") == 0)) {
                value *= lessAggressiveAdjustmentFactor;
                heuristicTrace(tracer(), "Setting size from %d to %d because arg is boxed primitive type.",
                    interimWeight, value);
            }
            if (prexArg && prexArg->hasKnownObjectIndex()) {
                value = knownObjWeight;
                heuristicTrace(tracer(), "Setting size from %d to %d because arg is known object.", interimWeight,
                    value);
                break;
            }
        }
...
bool TR_J9EstimateCodeSize::estimateCodeSize(TR_CallTarget *calltarget, TR_CallStack *prevCallStack, bool recurseDown,
    int32_t callerAnalyzedSizeThreshold)
{
    if (realEstimateCodeSize(calltarget, prevCallStack, recurseDown, comp()->trMemory()->currentStackRegion(),
            callerAnalyzedSizeThreshold)) {
...
TR::CFG &TR_J9EstimateCodeSize::processBytecodeAndGenerateCFG(TR_CallTarget *calltarget, TR::Region &cfgRegion,
    TR_J9ByteCodeIterator &bci, NeedsPeekingHeuristic &nph, TR::Block **blocks, flags8_t *flags)
{
    char nameBuffer[1024];
    const char *callerName = NULL;
    if (tracer()->heuristicLevel())
        callerName = comp()->fej9()->sampleSignature(calltarget->_calleeMethod->getPersistentIdentifier(), nameBuffer,
            1024, comp()->trMemory());
    ...
    int32_t *bcSizes = (int32_t *)comp()->trMemory()->allocateStackMemory(maxIndex * sizeof(int32_t));
    ...
    for (; bc != J9BCunknown; bc = bci.next()) {
        ...
        if (bc == J9BCgenericReturn || bc == J9BCReturnC || bc == J9BCReturnS || bc == J9BCReturnB
            || bc == J9BCReturnZ) {
        ...
        switch (bc) {
            ...
            case J9BCinvokevirtual: {
                ...
                resolvedMethod
                    = calleeMethod->getResolvedPossiblyPrivateVirtualMethod(comp(), cpIndex, true, &isUnresolvedInCP);

                if (resolvedMethod) {
                    TR::RecognizedMethod rm = resolvedMethod->getRecognizedMethod();
                    if (rm == TR::java_util_HashMap_put || rm == TR::java_util_HashMap_get
                        || rm == TR::java_util_concurrent_ConcurrentHashMap_get || rm == TR::java_lang_Object_hashCode
                        || rm == TR::java_lang_reflect_Method_invoke) {
                        nph.setNeedsPeekingToTrue();
                        heuristicTrace(tracer(),
                            "Depth %d: invokevirtual call at bc index %d has Signature %s, enabled peeking for caller "
                            "to propagate prex arg info from caller.",
                ...
                if ((isUnresolvedInCP && !resolvedMethod) || (resolvedMethod && resolvedMethod->isCold(comp(), true))) {
                    if (tracer()->heuristicLevel()) {
                        if (resolvedMethod) {
                            heuristicTrace(tracer(),
                                "Depth %d: Call at bc index %d is Cold.  Not searching for targets. Signature %s",
                                _recursionDepth, i, tracer()->traceSignature(resolvedMethod));
                        } else {
                            TR::Method *meth = comp()->fej9()->createMethod(comp()->trMemory(),
                                calltarget->_calleeMethod->containingClass(), cpIndex);
                            heuristicTrace(tracer(),
                                "Depth %d: Call at bc index %d is Cold.  Not searching for targets. Signature %s",
                                _recursionDepth, i, tracer()->traceSignature(meth));
                        }
                    }
            ...
            case J9BCinvokespecialsplit: {
                if (thisOnStack)
                    hasThisCalls = true;
                cpIndex = bci.next2Bytes();
                resolvedMethod = calltarget->_calleeMethod->getResolvedSpecialMethod(comp(),
                    (bc == J9BCinvokespecialsplit) ? cpIndex |= J9_SPECIAL_SPLIT_TABLE_INDEX_FLAG : cpIndex,
                    &isUnresolvedInCP);
                ...
                if (sig && (!strncmp(sig, "jdk/internal/reflect/DirectMethodHandleAccessor.invokeImpl", 58))) {
                    nph.setNeedsPeekingToTrue();
                    heuristicTrace(tracer(),
                        "Depth %d: invokespecial call at bc index %d has Signature %s, enabled peeking for caller to "
                        "propagate prex arg info from caller.",
                        _recursionDepth, i, sig);
                }
            ...
            case J9BCinvokestaticsplit: {
                cpIndex = bci.next2Bytes();
                resolvedMethod = calltarget->_calleeMethod->getResolvedStaticMethod(comp(),
                    (bc == J9BCinvokestaticsplit) ? cpIndex |= J9_STATIC_SPLIT_TABLE_INDEX_FLAG : cpIndex,
                    &isUnresolvedInCP);
                bool isIndirectCall = false;
                bool isInterface = false;
                TR::Method *interfaceMethod = 0;
                ...
                if (resolvedMethod && resolvedMethod->getRecognizedMethod() == TR::java_util_HashMap_hash) {
                    nph.setNeedsPeekingToTrue();
                    heuristicTrace(tracer(),
                        "Depth %d: invokestatic call at bc index %d has Signature %s, enabled peeking for caller to "
                        "propagate prex arg info from caller.",
                        _recursionDepth, i, tracer()->traceSignature(resolvedMethod));
                }
                if (!resolvedMethod || isUnresolvedInCP || resolvedMethod->isCold(comp(), false)) {
            ...
            case J9BCinvokeinterface: {
                ...
                if (meth) {
                    ...
                    if (sig && (!strncmp(sig, "java/util/Map.put", 17) || !strncmp(sig, "java/util/Map.get", 17))) {
                        nph.setNeedsPeekingToTrue();
                        heuristicTrace(tracer(),
                            "Depth %d: invokeinterface call at bc index %d has Signature %s, enabled peeking for "
                            "caller to propagate prex arg info from caller.",
                            _recursionDepth, i, sig);
                    }
#if JAVA_SPEC_VERSION >= 21
                    else if (sig
                        && (!strncmp(sig, "java/lang/foreign/MemorySegment.get", 35)
                        ...
                        heuristicTrace(tracer(),
                            "Depth %d: invokeinterface call at bc index %d has Signature %s, enabled peeking for "
                            "caller to fold layout field load necessary for VarHandle operation inlining.",
                            _recursionDepth, i, sig);
                    }
                ...
                break;
            case J9BCgetfield:
                resolved = calltarget->_calleeMethod->fieldAttributes(comp(), bci.next2Bytes(), &fieldOffset, &type,
                    &isVolatile, 0, &isPrivate, false, &isUnresolvedInCP, false);
            ...
            case J9BCputfield:
                resolved = calltarget->_calleeMethod->fieldAttributes(comp(), bci.next2Bytes(), &fieldOffset, &type,
                    &isVolatile, 0, &isPrivate, true, &isUnresolvedInCP, false);
                if (!resolved || isUnresolvedInCP) {
                    if (unresolvedSymbolsAreCold)
                        flags[i].set(InterpreterEmulator::BytecodePropertyFlag::isCold);
                    if (!resolved)
                ...
                break;
            case J9BCgetstatic:
                resolved = calltarget->_calleeMethod->staticAttributes(comp(), bci.next2Bytes(), &staticAddress, &type,
                    &isVolatile, 0, &isPrivate, false, &isUnresolvedInCP, false);
                if (!resolved || isUnresolvedInCP) {
                    if (unresolvedSymbolsAreCold)
                        flags[i].set(InterpreterEmulator::BytecodePropertyFlag::isCold);
            ...
            case J9BCinvokedynamic: {
                ...
                resolvedMethod = calltarget->_calleeMethod->getResolvedDynamicMethod(comp(), callSiteTableEntryIndex,
                    &isUnresolvedInCP);
                if (resolvedMethod && !isUnresolvedInCP) {
                    nph.setNeedsPeekingToTrue();
                    heuristicTrace(tracer(),
                        "Depth %d: Resolved invokedynamic call at bc index %d has Signature %s, enabled peeking for "
                        "caller to propagate prex arg info from caller.",
                        _recursionDepth, i, tracer()->traceSignature(resolvedMethod));
                }
            }
            ...
            case J9BCinvokehandle: {
                ...
                isUnresolvedInCP = false;
                resolvedMethod = calltarget->_calleeMethod->getResolvedHandleMethod(comp(), methodTypeTableEntryIndex,
                    &isUnresolvedInCP);
                if (resolvedMethod && !isUnresolvedInCP) {
                    nph.setNeedsPeekingToTrue();
                    heuristicTrace(tracer(),
                        "Depth %d: Resolved invokehandle call at bc index %d has Signature %s, enabled peeking for "
                        "caller to propagate prex arg info from caller.",
                        _recursionDepth, i, tracer()->traceSignature(resolvedMethod));
                }
            }
    ...
    if (adjustEstimateForStringCompression(calltarget->_calleeMethod, size, STRING_COMPRESSION_ADJUSTMENT_FACTOR)) {
        heuristicTrace(tracer(), "*** Depth %d: Adjusting size for %s because of string compression from %d to %d",
            _recursionDepth, callerName, sizeBeforeAdjustment, size);
    }

    if (adjustEstimateForMethodInvoke(calltarget->_calleeMethod, size, METHOD_INVOKE_ADJUSTMENT_FACTOR)) {
        heuristicTrace(tracer(),
            "*** Depth %d: Adjusting size for %s because of java/lang/reflect/Method.invoke from %d to %d",
            _recursionDepth, callerName, sizeBeforeAdjustment, size);
    }

    sizeBeforeAdjustment = size;
    if (adjustEstimateForConstArgs(calltarget, size, CONST_ARG_IN_CALLEE_ADJUSTMENT_FACTOR)) {
        heuristicTrace(tracer(), "*** Depth %d: Adjusting size for %s because of constants in args from %d to %d",
            _recursionDepth, callerName, sizeBeforeAdjustment, size);
    }
    ...
    if (calltarget->_calleeSymbol) {
        TR::DebugCounter::incStaticDebugCounter(comp(),
            TR::DebugCounter::debugCounterName(comp(), "inliner/%s/estimatedBytecodeSize/%d",
                calltarget->_calleeSymbol->signature(comp()->trMemory()), calltarget->_fullSize));
    }

    /********* PHASE 2: Generate CFG **********/

    heuristicTrace(tracer(),
        "--- Done Iterating over Bytecodes in call to %s.  size = %d _recursionDepth = %d _analyzedSize = %d _realSize "
        "= %d _sizeThreshold = %d",
        callerName, size, _recursionDepth, _analyzedSize, _realSize, _sizeThreshold);

    if (hasThisCalls && calltarget->_calleeSymbol)
        calltarget->_calleeSymbol->setHasThisCalls(true);

    TR_Array<TR_J9ByteCodeIterator::TryCatchInfo> tryCatchInfo(comp()->trMemory(),
        calltarget->_calleeMethod->numberOfExceptionHandlers(), true, stackAlloc);

    int32_t i;
    for (i = calltarget->_calleeMethod->numberOfExceptionHandlers() - 1; i >= 0; --i) {
        int32_t start, end, type;
        int32_t handler = calltarget->_calleeMethod->exceptionData(i, &start, &end, &type);
    ...
    cfg.setStartAndEnd(TR::Block::createBlock(TR::TreeTop::create(comp(), TR::Node::create(NULL, TR::BBStart, 0)),
                           TR::TreeTop::create(comp(), TR::Node::create(NULL, TR::BBEnd, 0)), cfg),
        TR::Block::createBlock(TR::TreeTop::create(comp(), TR::Node::create(NULL, TR::BBStart, 0)),
            TR::TreeTop::create(comp(), TR::Node::create(NULL, TR::BBEnd, 0)), cfg));

    cfg.getStart()->asBlock()->getEntry()->join(cfg.getStart()->asBlock()->getExit());
    cfg.getEnd()->asBlock()->getEntry()->join(cfg.getEnd()->asBlock()->getExit());
    ...
    setupNode(cfg.getStart()->asBlock()->getEntry()->getNode(), 0, calltarget->_calleeMethod, comp());
    setupNode(cfg.getStart()->asBlock()->getExit()->getNode(), 0, calltarget->_calleeMethod, comp());
    setupNode(cfg.getEnd()->asBlock()->getEntry()->getNode(), endNodeIndex, calltarget->_calleeMethod, comp());
    setupNode(cfg.getEnd()->asBlock()->getExit()->getNode(), endNodeIndex, calltarget->_calleeMethod, comp());

    debugTrace(tracer(), "PECS: startblock %p %d endblock %p %d", cfg.getStart()->asBlock(),
        cfg.getStart()->getNumber(), cfg.getEnd()->asBlock(), cfg.getEnd()->getNumber());
    ...
    for (TR_J9ByteCode bc = bci.first(); bc != J9BCunknown; bc = bci.next()) {
        ...
        if (flags[i].testAny(InterpreterEmulator::BytecodePropertyFlag::bbStart)) {
            ...
            TR::Block *newBlock = getBlock(comp(), blocks, calltarget->_calleeMethod, i, cfg);

            if (i != startIndex) {
                currentBlock->setBlockSize(bcSizes[i] - blockStartSize);
                if (cfg.getMethodSymbol())
                    cfg.getMethodSymbol()->addProfilingOffsetInfo(
                        currentBlock->getEntry()->getNode()->getByteCodeIndex(),
                        currentBlock->getEntry()->getNode()->getByteCodeIndex() + currentBlock->getBlockSize());
            }

            if (addFallThruEdge) {
                debugTrace(tracer(), "adding a fallthrough edge between block %p %d and %p %d", currentBlock,
                    currentBlock->getNumber(), newBlock, newBlock->getNumber());
                debugTrace(tracer(), "joining nodes between blocks %p %d and %p %d", currentBlock,
                    currentBlock->getNumber(), newBlock, newBlock->getNumber());
        ...
        if (flags[i].testAny(InterpreterEmulator::BytecodePropertyFlag::isCold)) {
            partialTrace(tracer(),
                "Setting block %p[%d] blocks[%d]=%p as cold because bytecode %d was identified as cold", currentBlock,
                currentBlock->getNumber(), i, blocks[i], i);
            currentBlock->setIsCold();
            currentBlock->setFrequency(0);
        }
        if (flags[i].testAny(InterpreterEmulator::BytecodePropertyFlag::isUnsanitizeable)) {
            partialTrace(tracer(), "Setting unsanitizeable flag on block %p[%d] blocks[%d]=%p", currentBlock,
                currentBlock->getNumber(), i, blocks[i]);
        ...
        if (flags[i].testAny(InterpreterEmulator::BytecodePropertyFlag::isBranch)) {
            if (startIndex != i) {
                ...
                currentBlock->setBlockSize(
                    1); // if there startIndex is the same as the current index then the block consists only of a branch
                if (cfg.getMethodSymbol())
                    cfg.getMethodSymbol()->addProfilingOffsetInfo(
                        currentBlock->getEntry()->getNode()->getByteCodeIndex(),
                        currentBlock->getEntry()->getNode()->getByteCodeIndex() + currentBlock->getBlockSize());
            ...
            switch (bc) {
                ...
                case J9BCifnull:
                case J9BCifnonnull: {
                    debugTrace(tracer(), "if branch.i = %d adding edge between blocks %p %d and %p %d", i, currentBlock,
                        currentBlock->getNumber(),
                        getBlock(comp(), blocks, calltarget->_calleeMethod, i + bci.relativeBranch(), cfg),
                        getBlock(comp(), blocks, calltarget->_calleeMethod, i + bci.relativeBranch(), cfg)
                            ->getNumber());

                    setupLastTreeTop(currentBlock, bc, i,
                        getBlock(comp(), blocks, calltarget->_calleeMethod, i + bci.relativeBranch(), cfg),
                        calltarget->_calleeMethod, comp());
                    cfg.addEdge(currentBlock,
                        getBlock(comp(), blocks, calltarget->_calleeMethod, i + bci.relativeBranch(), cfg));
                    addFallThruEdge = true;
                    break;
                }
                ...
                case J9BCtableswitch: {
                    int32_t index = bci.defaultTargetIndex();
                    TR::Block *defaultBlock
                        = getBlock(comp(), blocks, calltarget->_calleeMethod, i + bci.nextSwitchValue(index), cfg);
                    setupLastTreeTop(currentBlock, bc, i, defaultBlock, calltarget->_calleeMethod, comp());
    ...
    for (i = 0; i < (int32_t)tryCatchInfo.size(); ++i) {
        ...
        blocks[handlerInfo->_handlerIndex]->setHandlerInfoWithOutBCInfo(handlerInfo->_catchType, 0,
            handlerInfo->_handlerIndex, calltarget->_calleeMethod, comp());
...
bool TR_J9EstimateCodeSize::realEstimateCodeSize(TR_CallTarget *calltarget, TR_CallStack *prevCallStack,
    bool recurseDown, TR::Region &cfgRegion, int32_t callerAnalyzedSizeThreshold)
{
    TR_ASSERT(calltarget->_calleeMethod, "assertion failure");

    heuristicTrace(tracer(), "*** Depth %d: ECS CSI -- calltarget = %p , _ecsPrexArgInfo = %p", _recursionDepth,
        calltarget, calltarget->_ecsPrexArgInfo);
    ...
    heuristicTrace(tracer(),
        "*** Depth %d: ECS to begin for target %p signature %s size assuming we can partially inline (analyzed size)  "
        "= %d total real size so far = %d sizeThreshold %d",
    ...
    if (_mayHaveVirtualCallProfileInfo) {
        if (!comp()->incInlineDepth(methodSymbol, calltarget->_myCallSite->_bcInfo, 0, NULL,
                !calltarget->_myCallSite->_isIndirectCall)) {
    ...
    if (comp()->getVisitCount() > HIGH_VISIT_COUNT) {
        heuristicTrace(tracer(), "Depth %d: estimateCodeSize aborting due to high comp()->getVisitCount() of %d",
            _recursionDepth, comp()->getVisitCount());
    ...
    if (_recursionDepth > maxRecDepth) {
        ...
        heuristicTrace(tracer(), "*** Depth %d: ECS end for target %p signature %s. Exceeded Recursion Depth",
            _recursionDepth, calltarget, callerName);
        return returnCleanup(ECS_RECURSION_DEPTH_THRESHOLD_EXCEEDED);
    }

    InterpreterEmulator bci(calltarget, methodSymbol, static_cast<TR_J9VMBase *>(comp()->fej9()), comp(), tracer(),
        this);
    ...
    TR_CallSite **callSites = (TR_CallSite **)comp()->trMemory()->allocateStackMemory(maxIndex * sizeof(TR_CallSite *));
    ...
    if (!TR_PrexArgInfo::validateAndPropagateArgsFromCalleeSymbol(argsFromSymbol, calltarget->_ecsPrexArgInfo,
            tracer())) {
        heuristicTrace(tracer(), "*** Depth %d: ECS end for target %p signature %s. Incompatible arguments",
            _recursionDepth, calltarget, callerName);
    ...
    const static bool disableMethodHandleInliningAfterFirstPass
        = feGetEnv("TR_DisableMethodHandleInliningAfterFirstPass") ? true : false;
    bool isArchetypeSpecimen = calltarget->_calleeMethod->convertToMethod()->isArchetypeSpecimen();
    bool inlineArchetypeSpecimen
        = isArchetypeSpecimen && (!disableMethodHandleInliningAfterFirstPass || _inliner->firstPass());
    ...
    if (!inlineLambdaFormGeneratedMethod
        ...
        heuristicTrace(tracer(), "*** Depth %d: ECS CSI -- needsPeeking is true for calltarget %p", _recursionDepth,
            calltarget);

        bool ilgenSuccess = (NULL
            != methodSymbol->getResolvedMethod()->genMethodILForPeekingEvenUnderMethodRedefinition(methodSymbol, comp(),
                false, calltarget->_ecsPrexArgInfo));
        if (ilgenSuccess) {
            heuristicTrace(tracer(), "*** Depth %d: ECS CSI -- peeking was successfull for calltarget %p",
                _recursionDepth, calltarget);
            _inliner->getUtil()->clearArgInfoForNonInvariantArguments(calltarget->_ecsPrexArgInfo, methodSymbol,
                tracer());
            wasPeekingSuccessful = true;
        }
    } else if (inlineArchetypeSpecimen && !mhInlineWithPeeking && debugMHInlineWithOutPeeking) {
        debugTrace(tracer(),
            "printing out trees and bytecodes through peeking because DebugMHInlineWithOutPeeking is on\n");
        methodSymbol->getResolvedMethod()->genMethodILForPeekingEvenUnderMethodRedefinition(methodSymbol, comp(), false,
            NULL);
    }
    ...
    bool callGraphEnabled
        = !comp()->getOption(TR_DisableCallGraphInlining); // profileManager->isCallGraphProfilingEnabled(comp());
    ...
    if (wasPeekingSuccessful && comp()->getOrCreateKnownObjectTable()
        && calltarget->_calleeMethod->convertToMethod()->isArchetypeSpecimen()) {
        TR::Block *currentInlinedBlock = NULL;
        // call sites in method handle thunks are created from trees so skip bci.findAndCreateCallsitesFromBytecodes
        // below
        ...
        for (TR::TreeTop *tt = methodSymbol->getFirstTreeTop(); tt; tt = tt->getNextTreeTop()) {
            ...
            if (tt->getNode()->getNumChildren() > 0 && tt->getNode()->getFirstChild()->getOpCode().isCall()) {
                ...
                if (!callNode->getSymbolReference()->isUnresolved() && !visited.contains(callNode)
                    && !callSites[callNode->getByteCodeIndex()]) // skip if the callsite has already been created for
                                                                 // this byte code index
                {
                    ...
                    TR_CallSite *callsite = TR_CallSite::create(tt, parent, callNode, resolvedMethod->classOfMethod(),
                        symRef, resolvedMethod, comp(), comp()->trMemory(), heapAlloc, calltarget->_calleeMethod,
                        _recursionDepth, false);
                    ...
                    // _callNode will be filled with node generated by actual ilgen @see
                    // TR_InlinerBase::findAndUpdateCallSiteInGraph
...
// VarHandle object through folding ValueLayouts$AbstractValueLayout.accessHandle() if the VarHandle has been created
        ...
        if (!bci.findAndCreateCallsitesFromBytecodes(wasPeekingSuccessful, iteratorWithState)) {
            heuristicTrace(tracer(),
                "*** Depth %d: ECS end for target %p signature %s. bci.findAndCreateCallsitesFromBytecode failed",
                _recursionDepth, calltarget, callerName);
    ...
    if (comp()->isServerInlining()) {
        ...
        if (executedCode != 0) {
            float ratio = ((float)executedCode) / ((float)(coldCode + executedCode));
            ...
            heuristicTrace(tracer(), "Depth %d: Opt Server is reducing size of call to %d", _recursionDepth,
                calltarget->_fullSize);
        }
        ...
        if (j9inlinerPolicy->aggressivelyInlineInLoops() && calltarget && calltarget->_calleeMethod
            && strncmp(calltarget->_calleeMethod->classNameChars(), "java/math/BigDecimal",
                   calltarget->_calleeMethod->classNameLength())
                != 0) {
            if ((callStack._inALoop) && (calltarget->_fullSize > 10)) {
                calltarget->_fullSize = 10;
                heuristicTrace(tracer(), "Opt Server is reducing size of call to %d", calltarget->_fullSize);
            }
        } else
            heuristicTrace(tracer(),
                "Omitting Big Decimal method from size readjustment, calltarget = %p calleemethod = %p", calltarget,
                calltarget ? calltarget->_calleeMethod : 0);
    }
    ...
    if (comp()->getMethodHotness() == veryHot || comp()->getMethodHotness() == scorching) {
        if (callerAnalyzedSizeThreshold > analyzedSizeThreshold) {
            ...
            if (forceInlineMultiplierStr)
                forceInlineMultiplier = atoi(forceInlineMultiplierStr);
    ...
    if (_analyzedSize > analyzedSizeThreshold) // even optimistically we've blown our budget
    {
        calltarget->_isPartialInliningCandidate = false;
        heuristicTrace(tracer(),
            "*** Depth %d: ECS end for target %p signature %s. analyzedSize exceeds analyzedSize threshold %d",
            _recursionDepth, calltarget, callerName, analyzedSizeThreshold);
        return returnCleanup(ECS_OPTIMISTIC_SIZE_THRESHOLD_EXCEEDED);
    }

    if (!recurseDown) {
        heuristicTrace(tracer(),
            "*** Depth %d: ECS end for target %p signature %s. recurseDown set to false. size = %d _fullSize = %d",
            _recursionDepth, calltarget, callerName, size, calltarget->_fullSize);
    ...
    for (TR_J9ByteCode bc = bci.first(); bc != J9BCunknown && bci._inlineableCallExists; bc = bci.next()) {
        int32_t i = bci.bcIndex();
        // heuristicTrace(tracer(),"--- Depth %d: Checking _real size vs Size Threshold: _realSize %d _sizeThreshold %d
        // sizeThreshold %d ",_recursionDepth, _realSize, _sizeThreshold, sizeThreshold);

        if ((!callTargetIsForceInline && _realSize > sizeThreshold)
            || (callTargetIsForceInline && _realSize > analyzedSizeThreshold)) {
            heuristicTrace(tracer(),
                "*** Depth %d: ECS end for target %p signature %s. real size %d exceeds sizeThreshold %d",
                _recursionDepth, calltarget, callerName, _realSize, sizeThreshold);
        ...
        if (callSites[i]) {
            ...
            for (int32_t j = 0; j < callSites[i]->numTargets(); j++) {
                ...
                if (tracer()->heuristicLevel())
                    calleeName = comp()->fej9()->sampleSignature(targetCallee->_calleeMethod->getPersistentIdentifier(),
                        nameBuffer, 1024, comp()->trMemory());
                ...
                if (callGraphEnabled && !currentBlock->isCold()) {
                    ...
                    if (_inliner->getPolicy()->tryToInline(targetCallee, &callStack, true)) {
                        heuristicTrace(tracer(), "tryToInline filter matched %s",
                            targetCallee->_calleeMethod->signature(comp()->trMemory()));
                    } else {
                        // Now check the invocation count for the callee if interpreted.
                        bool isInterpretedCallWithLowFrequency = false;
                        static bool shouldNotInlineInterpretedMethods
                            = feGetEnv("TR_DisableSkippingInliningOfColdInterpretedMethods") == NULL;
                        if (shouldNotInlineInterpretedMethods) {
                            static bool includeWarmCompilationsAsWell
                                = feGetEnv("TR_EnableSkippingInliningOfColdInterpretedMethodsInWarm") != NULL;
                            if (targetCallee->_calleeMethod->isInterpretedForHeuristics()
                                && !comp()->fej9()->compiledAsDLTBefore(targetCallee->_calleeMethod)) {
                                if (comp()->getMethodHotness() > warm) {
                                    static const char *cutOffHotAndAbove
                                        = feGetEnv("TR_FreqCutOffForInterpretedCalleeInHotAndAbove");
                                    static const int32_t cutOffFreqForHotAndAbove = cutOffHotAndAbove
                                        ? atoi(cutOffHotAndAbove)
                                        : FREQ_CUTOFF_INTERPRETED_HOTANDABOVE;
                                    isInterpretedCallWithLowFrequency
                                        = currentBlock->getFrequency() < cutOffFreqForHotAndAbove;
                                } else if (includeWarmCompilationsAsWell && comp()->getMethodHotness() == warm) {
                                    static const char *cutOffForWarm
                                        = feGetEnv("TR_FreqCutOffForInterpretedCalleeInWarm");
                                    static const int32_t cutOffFreqForWarm
                                        = cutOffForWarm ? atoi(cutOffForWarm) : FREQ_CUTOFF_INTERPRETED_WARM;
                                    isInterpretedCallWithLowFrequency
                                        = currentBlock->getFrequency() < cutOffFreqForWarm;
                                }
                                heuristicTrace(tracer(),
                                    "Depth %d: callee %s is interpreted and isInterpretedCallWithLowFrequency = %s.",
                                    _recursionDepth, calleeName, isInterpretedCallWithLowFrequency ? "true" : "false");
                            }
                        }
                        ...
                        bool isColdCall
                            = (currentBlock->getFrequency() < freqCutoff || isInterpretedCallWithLowFrequency)
                            && !(_inliner->alwaysWorthInlining(targetCallee->_calleeMethod, NULL));

                        if (coldCallInfoIsReliable && isColdCall) {
                            heuristicTrace(tracer(),
                                "Depth %d: Skipping estimate on call %s, with block frequency %d, because it's cold.",
                        ...
                        if (comp()->getMethodHotness() <= warm && comp()->isServerInlining()
                            && calltarget->_calleeMethod->isWarmCallGraphTooBig(i, comp())
                            && !_inliner->alwaysWorthInlining(targetCallee->_calleeMethod, NULL)) {
                            heuristicTrace(tracer(),
                                "Depth %d: Skipping estimate on call %s because its warm call graph is too big.",
                ...
                if (currentBlock->isCold()
                    && !_inliner->alwaysWorthInlining(targetCallee->_calleeMethod, callSites[i]->_callNode)) {
                    heuristicTrace(tracer(), "Depth %d: Skipping estimate on call %s, because it's in a cold block.",
                        _recursionDepth, calleeName);
                ...
                if (_analyzedSize <= analyzedSizeThreshold) // for multiple calltargets, is this the desired behaviour?
                {
                    ...
                    debugTrace(tracer(),
                        "About to call ecs on call target %p at depth %d _analyzedSize = %d _realSize = %d "
                        "_sizeThreshold = %d",
                        targetCallee, _recursionDepth, _analyzedSize, _realSize, _sizeThreshold);
                    heuristicTrace(tracer(),
                        "--- Depth %d: EstimateCodeSize to recursively estimate call from %s to %s", _recursionDepth,
                    ...
                    bool estimateSuccess
                        = estimateCodeSize(targetCallee, &callStack, /* recurseDown */ true, analyzedSizeThreshold);
                    ...
                    if ((comp()->getMethodHotness() >= warm) && comp()->isServerInlining()) {
                        ...
                        bool isColdCall = (currentBlock->getFrequency() <= freqCutoff)
                            && !_inliner->alwaysWorthInlining(targetCallee->_calleeMethod, NULL);
                        ...
                        if ((_analyzedSize - origAnalyzedSize - bigCalleesSizeBelowMe) > bigCalleeThreshold) {
                            ...
                            _bigCalleesSize
                                = _bigCalleesSize + _analyzedSize - origAnalyzedSize - bigCalleesSizeBelowMe;
                            heuristicTrace(tracer(), "set warmcallgraphtoobig for method %s at index %d\n", calleeName,
                                newBCInfo.getByteCodeIndex());
                    ...
                    if (!estimateSuccess && !calltargetSetTooBig) {
                        ...
                        static const int32_t graceInliningThreshold
                            = git ? atoi(git) : DEFAULT_GRACE_INLINING_THRESHOLD;

                        if (estimatedSize < graceInliningThreshold)
                            inlineAnyway = true;

                        // non cold calls are checked here probably since we did not add any call sites from the callee
                        // that failed estimation, and so we are making sure that we did not miss out on anything
                        // important (non cold)
                        //
                        if (inlineAnyway && !calleeHasNonColdCalls) {
                            // This resetting is probably needed on this path since we are inlining despite exceeding
                            // some condition/threshold and so it would be an odd state to carry on with _analyzedSize
                            ...
                            calltarget->_isPartialInliningCandidate = false;
                            callSites[i]->removecalltarget(j, tracer(), Callee_Too_Many_Bytecodes);
                            //_analyzedSize = origAnalyzedSize;
                            _realSize = origRealSize;
                            ...
                            _numOfEstimatedCalls--;
                            heuristicTrace(tracer(),
                                "Depth %d: estimateCodeSize skipping estimated call and resetting _realSize to %d",
                                _recursionDepth, _realSize);
                        }

                        if (comp()->getVisitCount() > HIGH_VISIT_COUNT) {
                            heuristicTrace(tracer(),
                                "Depth %d: estimateCodeSize aborting due to high comp()->getVisitCount() of %d",
                                _recursionDepth, comp()->getVisitCount());
                            return returnCleanup(ECS_VISITED_COUNT_THRESHOLD_EXCEEDED);
                        }
                        ...
                        if (!_inliner->alwaysWorthInlining(targetCallee->_calleeMethod, NULL)) {
                            ...
                            heuristicTrace(tracer(),
                                "Depth %d: estimateCodeSize skipping too big estimated call and resetting _realSize to "
                                "%d",
                                _recursionDepth, _realSize);
                        }
                    ...
                    heuristicTrace(tracer(),
                        "Depth %d: estimateCodeSize aborting due to _analyzedSize: %d > sizeThreshold: %d",
                        _recursionDepth, _analyzedSize, sizeThreshold);
                    break;
                }
            }

            if (callSites[i]
                    ->numTargets()) // only add a callSite once, even though it may have more than one call target.
            {
                calltarget->addCallee(callSites[i]);
                heuristicTrace(tracer(),
                    "Depth %d: Subtracting %d from analyzed and real size to account for eliminating call",
                    _recursionDepth, bci.estimatedCodeSize());
                if (_analyzedSize > bci.estimatedCodeSize())
                    _analyzedSize -= bci.estimatedCodeSize(); // subtract what we added before for the size of the call
                                                              // instruction
    ...
    if (callTargetIsForceInline) {
        heuristicTrace(tracer(),
            "Depth %d: Restoring analyzed size from %d to pre-analysis size %d because the analyzed method %s is "
            "always worth inlining",
            _recursionDepth, _analyzedSize, initialAnalyzedSize, callerName);
    ...
    if (adjustEstimateForStringCompression(calltarget->_calleeMethod, calltarget->_partialSize,
            STRING_COMPRESSION_ADJUSTMENT_FACTOR)) {
        heuristicTrace(tracer(),
            "*** Depth %d: Adjusting partial size for %s because of string compression from %d to %d", _recursionDepth,
            callerName, partialSizeBeforeAdjustment, calltarget->_partialSize);
    }

    if (adjustEstimateForMethodInvoke(calltarget->_calleeMethod, calltarget->_partialSize,
            METHOD_INVOKE_ADJUSTMENT_FACTOR)) {
        heuristicTrace(tracer(),
            "*** Depth %d: Adjusting partial size for %s because of java/lang/reflect/Method.invoke from %d to %d",
            _recursionDepth, callerName, partialSizeBeforeAdjustment, calltarget->_partialSize);
    }

    auto fullSizeBeforeAdjustment = calltarget->_fullSize;

    if (adjustEstimateForStringCompression(calltarget->_calleeMethod, calltarget->_fullSize,
            STRING_COMPRESSION_ADJUSTMENT_FACTOR)) {
        heuristicTrace(tracer(), "*** Depth %d: Adjusting full size for %s because of string compression from %d to %d",
            _recursionDepth, callerName, fullSizeBeforeAdjustment, calltarget->_fullSize);
    }

    if (adjustEstimateForMethodInvoke(calltarget->_calleeMethod, calltarget->_fullSize,
            METHOD_INVOKE_ADJUSTMENT_FACTOR)) {
        heuristicTrace(tracer(),
            "*** Depth %d: Adjusting full size for %s because of java/lang/reflect/Method.invoke from %d to %d",
            _recursionDepth, callerName, fullSizeBeforeAdjustment, calltarget->_fullSize);
    }

    auto realSizeBeforeAdjustment = _realSize;

    if (adjustEstimateForStringCompression(calltarget->_calleeMethod, _realSize,
            STRING_COMPRESSION_ADJUSTMENT_FACTOR)) {
        heuristicTrace(tracer(), "*** Depth %d: Adjusting real size for %s because of string compression from %d to %d",
            _recursionDepth, callerName, realSizeBeforeAdjustment, _realSize);
    }

    if (adjustEstimateForMethodInvoke(calltarget->_calleeMethod, _realSize, METHOD_INVOKE_ADJUSTMENT_FACTOR)) {
        heuristicTrace(tracer(),
            "*** Depth %d: Adjusting real size for %s because of java/lang/reflect/Method.invoke from %d to %d",
            _recursionDepth, callerName, realSizeBeforeAdjustment, _realSize);
    }

    reduceDAAWrapperCodeSize(calltarget);

    /****************** PHASE 5: Figure out if We're really going to do a partial Inline and add whatever we do to the
     * realSize. *******************/
    if (isPartialInliningCandidate(calltarget, &callBlocks)) {
        logprintf(comp()->getOption(TR_TraceBFGeneration), comp()->log(),
            "Call Target %s is a partial inline Candidate with a partial size of %d", callerName,
            calltarget->_partialSize);

        heuristicTrace(tracer(),
            "*** Depth %d: ECS end for target %p signature %s. It is a partial inline Candidate with a partial size of "
            "%d",
            _recursionDepth, calltarget, callerName, calltarget->_partialSize);
        _realSize += calltarget->_partialSize;
    } else {
        heuristicTrace(tracer(),
            "*** Depth %d: ECS end for target %p signature %s. It is a full inline Candidate with a full size of %d",
            _recursionDepth, calltarget, callerName, calltarget->_fullSize);
        _realSize += calltarget->_fullSize;
    }

    heuristicTrace(tracer(),
        "--- Depth %d: Checking _real size vs Size Threshold A second Time: _realSize %d _sizeThreshold %d "
        "sizeThreshold %d ",
        _recursionDepth, _realSize, _sizeThreshold, sizeThreshold);

    if ((!callTargetIsForceInline && _realSize > sizeThreshold)
        || (callTargetIsForceInline && _realSize > analyzedSizeThreshold)) {
        heuristicTrace(tracer(), "*** Depth %d: ECS end for target %p signature %s. real size exceeds Size Threshold",
            _recursionDepth, calltarget, callerName);
        return returnCleanup(ECS_REAL_SIZE_THRESHOLD_EXCEEDED);
    }
...
bool TR_J9EstimateCodeSize::reduceDAAWrapperCodeSize(TR_CallTarget *target)
{
    ...
    if (target->_calleeMethod) {
        ...
        bool reducePackedDecimalWrapper
            = ((TR_ResolvedJ9Method *)target->_calleeMethod)->isDAAPackedDecimalWrapperMethod()
            && !comp()->getOption(TR_DisableMarshallingIntrinsics);
        ...
        if (reduceMarshallingWrapper || reducePackedDecimalWrapper) {
            ...
            heuristicTrace(tracer(),
                "DAA: Reducing target %p fullSize to %d and partialSize to %d to increase likelyhood of successful "
                "inlining\n",
...
/******************
 * A graph searching algorithm.  searchItem is the flag type we're looking for, searchPath is the flag type of the path
 * we can go down
 *
 * ***************/

bool TR_J9EstimateCodeSize::graphSearch(TR::CFG *cfg, TR::Block *startBlock, TR::Block::partialFlags searchItem,
    TR::Block::partialFlags searchPath)
{
    TR_BitVector *blocksVisited
    ...
    do {
        ...
        for (auto e = currentBlock->getExceptionSuccessors().begin(); e != currentBlock->getExceptionSuccessors().end();
             ++e) {
            TR::Block *dest = (*e)->getTo()->asBlock();
            if (dest->getPartialFlags().testAny(searchPath))
                nodesToBeEvaluated.enqueue(dest);
...
int32_t TR_J9EstimateCodeSize::labelGraph(TR::CFG *cfg, TR_Queue<TR::Block> *unsanitizeableBlocks,
    TR_Queue<TR::Block> *callBlocks)
{
    ...
    do {
        ...
        if (currentBlock->getBlockSize() == -1 && (currentBlock != startBlock && currentBlock != endBlock))
            TR_ASSERT(0, "labelGraph:  a block does not have a valid size!\n");

        // Part 1:  Successor Test:  ensure all my successors have been evaluated first and that they are not all
        // restart blocks.
        ...
        if (!allVisited && !currentBlock->isDifficultBlock()) {
            partialTrace(tracer(),
                "Requeueing block into difficult Nodes List %p %d because its successors have not been all visited \n",
            ...
            // which actually means doing nothing here, since I only mark allRestarts = false if I found a  partial
            // inline block.
        ...
        } else if ((currentBlock->getFrequency() < minpartialfreq || currentBlock->isCold())
        ...
        for (auto e = currentBlock->getExceptionPredecessors().begin();
             e != currentBlock->getExceptionPredecessors().end(); ++e) {
            TR::Block *dest = (*e)->getFrom()->asBlock();
            nodesToBeEvaluated.enqueue(dest);
        }

        if (currentBlock->isRestartBlock() && currentBlock->isPartialInlineBlock())
            TR_ASSERT(0, "currentBlock is both a restart block AND a partial inline block!\n");

    } while (!nodesToBeEvaluated.isEmpty() || !difficultNodesToBeEvaluated.isEmpty());
...
bool TR_J9EstimateCodeSize::trimBlocksForPartialInlining(TR_CallTarget *calltarget, TR_Queue<TR::Block> *callBlocks)
{
    TR_ASSERT(calltarget->_originatingBlock,
        "trimBlocksForPartialInlining: call target does not have an _originatingBlock set yet!\n");
    ...
    int32_t size = labelGraph(calltarget->_cfg, &unsanitizeableBlocks, callBlocks);
    ...
    if (size > -1 && size + minpartialsize >= calltarget->_fullSize) {
        partialTrace(tracer(),
            " Candidate partial size of %d is too close to full Size of %d to be of any benefit.  Doing a full inline.",
            size, calltarget->_fullSize);
    } else if (size
        > -1) // a size of -1 means we didn't have any restart blocks - so no sense in doing a 'partial' inline
    {
        bool gs = true;
        while (!unsanitizeableBlocks.isEmpty()) {
            TR::Block *aBlock = unsanitizeableBlocks.dequeue();
            if (!aBlock->isRestartBlock()) // if the unsanitizeable block is also a restart block, I don't care who it
                                           // reaches.
            {
                calltarget->_originatingBlock->setIsUnsanitizeable(); // An unsanitizeable block remains in the inline

                gs = !(graphSearch(calltarget->_cfg, aBlock, TR::Block::_restartBlock,
                    (TR::Block::partialFlags)(TR::Block::_partialInlineBlock | TR::Block::_restartBlock)));
                if (!gs) {
                    partialTrace(tracer(),
                        "TrimBlocksForPartialInlining: Unsanitizeable block %p %d can reach a restart block.", aBlock,
                        aBlock->getNumber());
                    break;
                }
            } else
                partialTrace(tracer(), "TrimBlocksForPartialinlining: Unsanitizeable block %p %d is a restart block.",
                    aBlock, aBlock->getNumber());
        }

        if (gs) {
            gs = graphSearch(calltarget->_cfg, calltarget->_cfg->getStart()->asBlock(), TR::Block::_endBlock,
        ...
        partialTrace(tracer(),
            "TrimBlocksForPartialInlining Found a Candidate.  Setting PartialSize to %d. full size = %d", size,
            calltarget->_fullSize);
        ...
        partialTrace(tracer(),
            "TrimBlocksForPartialInlining: No restart blocks found in candidate. Doing a full inline");
    }
...
void TR_J9EstimateCodeSize::processGraph(TR_CallTarget *calltarget)
{
    TR::CFG *cfg = calltarget->_cfg;
    calltarget->_partialInline = new (comp()->trHeapMemory()) TR_InlineBlocks(_inliner->fe(), _inliner->comp());
    TR_BitVector *blocksVisited
        = new (comp()->trStackMemory()) TR_BitVector(cfg->getNextNodeNumber(), comp()->trMemory(), stackAlloc);
    blocksVisited->empty();
    ...
    do {
        ...
        for (auto e = currentBlock->getExceptionSuccessors().begin(); e != currentBlock->getExceptionSuccessors().end();
            ...
            calltarget->_partialInline->addExceptionBlock(
                dest); // only partial blocks will be processed.  any exception block reachable from a partial block
...
bool TR_J9EstimateCodeSize::isPartialInliningCandidate(TR_CallTarget *calltarget, TR_Queue<TR::Block> *callBlocks)
{
    ...
    while (!callBlocks->isEmpty()) {
        ...
        if (callBlock->isUnsanitizeable() && !callBlock->isRestartBlock()) {
            ...
            bool result = graphSearch(calltarget->_cfg, callBlock, TR::Block::_restartBlock,
                (TR::Block::partialFlags)(TR::Block::_partialInlineBlock | TR::Block::_restartBlock));
