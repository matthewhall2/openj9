...
const char *TR::VP_BCDSign::TR_BCDSignConstraintNames[TR_Sign_Num_Types]
    = { "<unknown_sign_state>", "<clean>", "<preferred>", "<plus>", "<minus>", "<unsigned>", "<minus_clean>" };

TR::VP_BCDValue::VP_BCDValue(char *lit, int32_t litSize, char *str, TR_BCDSignConstraint sign, TR::DataType dt,
    OMR::ValuePropagation *vp)
    : TR::VP_BCDSign(sign, dt, vp)
    , _litLowSize(litSize)
{
    ...
    if (dt == TR::Aggregate || dt.isBCD()) {
        ...
        // BCD/Aggr types already carry around a string representation on the node so it is privatized to the constraint
        // here (in case the node itself is changed)
...
bool TR::VP_BCDValue::compareBCDConstraints(int32_t precisionOne, char *litOne, int32_t litSizeOne,
...
bool TR::VP_BCDValue::isEqualTo(int32_t otherPrecision, char *otherLit, int32_t otherLitSize,
...
TR::VP_BCDSign *TR::VP_BCDSign::create(OMR::ValuePropagation *vp, TR_BCDSignConstraint type, TR::DataType dt)
{
    ...
    if (constraint) {
        logprintf(vp->trace(), log, "return existing BCD sign constraint %p (dt=%s, sign=%s)\n", constraint,
            TR::DataType::getName(dt), constraint->getName());
        return constraint;
    }
...
{
    if (getSign() == otherSign && getDataType() == otherDataType) {
        return true;
    } else {
        return false;
    }
...
{
    if (checkPlus(signOne, signTwo) && checkMinus(signOne, signTwo) && checkUnsigned(signOne, signTwo)
        && checkPlus(signTwo, signOne) && checkMinus(signTwo, signOne) && checkUnsigned(signTwo, signOne)) {
        return true;
    } else {
        return false;
    }
...
TR::VPConstraint *TR::VP_BCDSign::intersect1(TR::VPConstraint *other, OMR::ValuePropagation *vp)
{
    ...
    if (asBCDSign() && other->asBCDSign()) {
        TR_ASSERT(TR::VP_BCDSign::signsAreConsistent(asBCDSign()->getSign(), other->asBCDSign()->getSign()),
            "sign mismatch %s != %s\n", TR::VP_BCDSign::getName(asBCDSign()->getSign()),
            TR::VP_BCDSign::getName(other->asBCDSign()->getSign()));
    }
    return NULL;
}
...
TR::VPConstraint *TR::VP_BCDSign::merge1(TR::VPConstraint *other, OMR::ValuePropagation *vp)
{
    ...
    if (asBCDSign() && other->asBCDSign()) {
        ...
        logprintf(vp->trace(), log, "\tthisSign %s thisType %s, otherSign %s otherType %s\n", asBCDSign()->getName(),
            TR::DataType::getName(asBCDSign()->getDataType()), other->asBCDSign()->getName(),
            TR::DataType::getName(other->asBCDSign()->getDataType()));
        ...
        logprintf(vp->trace(), log, "\tmergedSign = %s from %s x %s\n", TR::VP_BCDSign::getName(mergedSign),
            TR::VP_BCDSign::getName(thisSign), TR::VP_BCDSign::getName(otherSign));
        if (mergedSign != TR_Sign_Unknown) {
            TR::VP_BCDSign *mergedSignConstraint = TR::VP_BCDSign::create(vp, mergedSign, asBCDSign()->getDataType());
            if (vp->trace()) {
                log->printf("\treturn mergedSignConstraint %p of sign %s: ", mergedSignConstraint,
                    mergedSignConstraint->getName());
...
void TR::VP_BCDSign::print(OMR::Logger *log, TR::Compilation *comp)
{
    log->printf("%s (%s)", getName(), TR::DataType::getName(getDataType()));
}
