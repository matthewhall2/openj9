...
TR_BoolArrayStoreTransformer::TR_BoolArrayStoreTransformer(NodeSet *bstoreiUnknownArrayTypeNodes,
    NodeSet *bstoreiBoolArrayTypeNodes)
...
void TR_BoolArrayStoreTransformer::perform()
{
    ...
    if (!_hasVariantArgs) {
        ...
        for (auto it = _bstoreiUnknownArrayTypeNodes->begin(); it != _bstoreiUnknownArrayTypeNodes->end();) {
            ...
            if (arrayBaseNode->getOpCode().hasSymbolReference()
                && arrayBaseNode->getSymbolReference()->getSymbol()->isParm()) {
    ...
    if (!_bstoreiUnknownArrayTypeNodes->empty()) {
        if ((_hasByteArrayAutoOrCheckCast && _hasBoolArrayAutoOrCheckCast) || comp()->isDLT()) {
            ...
            if (_hasBoolArrayAutoOrCheckCast) // if only boolean array exist then all the bstorei nodes are operating on
                                              // boolean array
            {
                logprints(trace, log, "only boolean array exist as auto or checkcast type\n");
                _bstoreiBoolArrayTypeNodes->insert(_bstoreiUnknownArrayTypeNodes->begin(),
                    _bstoreiUnknownArrayTypeNodes->end());
            } else {
        ...
        if (secondType != TR_maybe) {
            if (firstType == TR_maybe) {
                ...
                if (traceIt) {
                    ...
                    log->printf(
                        "local #%2d has conflict types keep the first type for now: firstType %s, secondType %s\n", i,
                        getTypeName(firstType, firstTypeBuffer), getTypeName(secondType, secondTypeBuffer));
                }
            }
        }
    }
...
void TR_BoolArrayStoreTransformer::findBoolArrayStoreNodes()
{
    ...
    if (blockIt.currentBlock() && !comp()->isDLT()) {
        ...
        for (TR::ParameterSymbol *p = parms.getFirst(); p; p = parms.getNext()) {
            if (p->getDataType() == TR::Address) {
                ...
                if (type != TR_maybe) {
                    if (!typeInfo)
                        typeInfo = new (comp()->trMemory()->currentStackRegion())
                            TypeInfo(_numLocals, TR_maybe, comp()->trMemory()->currentStackRegion());
    ...
    while (blockIt.currentBlock() && _NumOfBstoreiNodesToVisit > 0) {
        ...
        TypeInfo *blockStartTypeInfo
            = blockStartTypeInfos.find(blockNum) != blockStartTypeInfos.end() ? blockStartTypeInfos[blockNum] : NULL;
...
int TR_BoolArrayStoreTransformer::getArrayDimension(const char *signature, int length, bool boolType)
{
    ...
    return (signature && length >= 2 && signature[length - 1] == expectedTypeChar && signature[length - 2] == '[')
        ? length - 1
        : -1;
}
...
int TR_BoolArrayStoreTransformer::getArrayDimension(TR::Node *node, bool boolType, bool parmAsAuto)
{
    ...
    if (node->getOpCodeValue() == TR::newarray) {
        ...
        TR_ASSERT(arrayTypeNode->getOpCode().isLoadConst(),
            "expect the second child of TR::newarray to be constant " POINTER_PRINTF_FORMAT, arrayTypeNode);
 ...
 */
void TR_BoolArrayStoreTransformer::findLoadAddressAutoAndFigureOutType(TR::Node *node, TypeInfo *typeInfo,
    TR::NodeChecklist &boolArrayNodes, TR::NodeChecklist &byteArrayNodes, TR::NodeChecklist &visitedNodes)
{
 ...
 */
TR_BoolArrayStoreTransformer::TypeInfo *TR_BoolArrayStoreTransformer::processBlock(TR::Block *block,
    TR_BoolArrayStoreTransformer::TypeInfo *blockStartTypeInfo)
{
    bool trace = comp()->getOption(TR_TraceILGen);
    OMR::Logger *log = comp()->log();
        ...
        if (node->getOpCode().isStoreDirect() && node->getSymbolReference()->getSymbol()->isAutoOrParm()) {
            ...
            if (rhs->getDataType().isAddress()) {
                ...
                if (newType != TR_maybe || currentTypeInfo) {
                    if (!currentTypeInfo)
                        currentTypeInfo = new (comp()->trMemory()->currentStackRegion())
                            TypeInfo(_numLocals, TR_maybe, comp()->trMemory()->currentStackRegion());
                    if (trace) {
                        ...
                        log->printf("Local #%2d %s -> %s at node n%dn\n", local->getLocalIndex(),
                            getTypeName(oldType, oldTypeBuffer), getTypeName(newType, newTypeBuffer),
                            node->getGlobalIndex());
                    }
                    (*currentTypeInfo)[local->getLocalIndex()] = newType;
                }
            }
        } else if (node->getOpCodeValue() == TR::bstorei
            && (_bstoreiUnknownArrayTypeNodes->find(node) != _bstoreiUnknownArrayTypeNodes->end())) {
            _NumOfBstoreiNodesToVisit--;
            TR_ASSERT(node->getFirstChild()->isInternalPointer(),
                "node in _bstoreiUnknownArrayTypeNodes can only be array store");
            ...
            if (isBoolArrayNode(typeNode)) {
                ...
                logprintf(trace, log, "checkcast node n%dn force node n%dn to be [Z\n", node->getGlobalIndex(),
                    checkcastedNode->getGlobalIndex());
...
void TR_BoolArrayStoreTransformer::transformUnknownTypeArrayStore()
{
    ...
    for (auto it = _bstoreiUnknownArrayTypeNodes->begin(); it != _bstoreiUnknownArrayTypeNodes->end(); it++) {
        ...
        TR::Node *vft = TR::Node::createWithSymRef(TR::aloadi, 1, 1, arrayBaseNode,
