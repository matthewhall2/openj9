...
int32_t TR_MethodHandleTransformer::perform()
{
    ...
    _blockEndObjectInfos = new (comp()->trMemory()->currentStackRegion())
        BlockResultMap(std::less<int32_t>(), comp()->trMemory()->currentStackRegion());
...
void TR_MethodHandleTransformer::collectAutosFromTrees(List<TR::SymbolReference> &autoSymRefs)
{
    ...
    for (auto treetop = comp()->getMethodSymbol()->getFirstTreeTop(); treetop != NULL;
        ...
        if (storeNode && storeNode->getOpCode().isStoreDirect() && storeNode->getSymbol()->isAuto()) {
...
TR_MethodHandleTransformer::ObjectInfo *TR_MethodHandleTransformer::getMethodEntryObjectInfo()
{
    TR_PrexArgInfo *argInfo = comp()->getCurrentInlinedCallArgInfo();
    ObjectInfo *objectInfo = new (comp()->trMemory()->currentStackRegion()) ObjectInfo(_numLocals,
        static_cast<int>(TR::KnownObjectTable::UNKNOWN), comp()->trMemory()->currentStackRegion());
    if (argInfo) {
        TR_ResolvedMethod *currentMethod = comp()->getCurrentMethod();
        int32_t numArgs = currentMethod->numberOfParameters();

        TR_ASSERT(argInfo->getNumArgs() == numArgs, "Number of prex arginfo %d doesn't match method parm number %d",
            argInfo->getNumArgs(), numArgs);
...
TR_MethodHandleTransformer::ObjectInfo *TR_MethodHandleTransformer::blockStartObjectInfoFromPredecessors(
    TR::Block *block)
{
    OMR::Logger *log = comp()->log();
    ...
    if (block->isCatchBlock()) {
        ...
        return new (comp()->trMemory()->currentStackRegion()) ObjectInfo(_numLocals,
            static_cast<int>(TR::KnownObjectTable::UNKNOWN), comp()->trMemory()->currentStackRegion());
    }
    ...
    for (TR::CFGEdge *edge = pi.getFirst(); edge != NULL; edge = pi.getNext()) {
        ...
        if (_blockEndObjectInfos->find(fromBlockNum) == _blockEndObjectInfos->end()) {
            logprintf(trace(), log,
                "Predecessor block_%d hasn't been visited yet, no object info is propagated for block_%d\n",
...
void TR_MethodHandleTransformer::computeObjectInfoOfNode(TR::TreeTop *tt, TR::Node *node)
{
    OMR::Logger *log = comp()->log();
    ...
    if (node->getOpCode().isLoadDirect() && symbol->isAutoOrParm()) {
        koi = (*_currentObjectInfo)[symbol->getLocalIndex()];
    } else if (node->getOpCode().isCall() && !symbol->castToMethodSymbol()->isHelper()) {
        ...
        switch (rm) {
            ...
            case TR::java_lang_invoke_DirectMethodHandle_internalMemberNameEnsureInit: {
                ...
                if (isKnownObject(mhIndex) && !knot->isNull(mhIndex)) {
                    auto mnIndex = comp()->fej9()->getMemberNameFieldKnotIndexFromMethodHandleKnotIndex(comp(), mhIndex,
                        "member");
                    logprintf(trace(), log,
                        "Get DirectMethodHandle.member known object %d, update node n%dn known object\n", mnIndex,
                        node->getGlobalIndex());
            ...
            case TR::java_lang_invoke_DirectMethodHandle_constructorMethod: {
                ...
                if (isKnownObject(mhIndex) && !knot->isNull(mhIndex)) {
                    auto mnIndex = comp()->fej9()->getMemberNameFieldKnotIndexFromMethodHandleKnotIndex(comp(), mhIndex,
                        "initMethod");
                    logprintf(trace(), log,
                        "Get DirectMethodHandle.initMethod known object %d, update node n%dn known object\n", mnIndex,
                        node->getGlobalIndex());
            ...
            case TR::java_lang_invoke_DelegatingMethodHandle_getTarget: {
                TR::KnownObjectTable::Index dmhIndex = getObjectInfoOfNode(node->getArgument(0));

                koi = comp()->fej9()->delegatingMethodHandleTarget(comp(), dmhIndex, trace());
                break;
            }
#endif
            case TR::java_lang_invoke_Invokers_checkVarHandleGenericType: {
                ...
                if (knot && isKnownObject(adIndex) && isKnownObject(vhIndex) && !knot->isNull(vhIndex)
                    && !knot->isNull(adIndex)) {
                    auto mhIndex = comp()->fej9()->getMethodHandleTableEntryIndex(comp(), vhIndex, adIndex);
                    logprintf(trace(), log,
                        "Invokers_checkVarHandleGenericType with known VarHandle object %d, updating node n%dn with "
                        "known MH object %d from MH table\n",
            ...
            case TR::java_lang_reflect_Method_acquireMethodAccessor: {
                ...
                if (knot && isKnownObject(methodIndex) && !knot->isNull(methodIndex)) {
                    auto maIndex = comp()->fej9()->getMethodAccessorIndex(comp(), methodIndex);
                    logprintf(trace(), log,
                        "Method.acquireMethodAccessor with known Method object %d, updating node n%dn with known "
                        "MethodAccessor object %d from MA field\n",
    ...
    if (isKnownObject(koi) && !knot->isNull(koi)
        && performTransformation(comp(), "%snode n%un [%p] is obj%d\n", optDetailString(), node->getGlobalIndex(), node,
            koi)) {
...
{
    TR_ASSERT_FATAL_WITH_NODE(node, node->getType() == TR::Address, "Can't have object info on non-address type node");
...
void TR_MethodHandleTransformer::visitStoreToLocalVariable(TR::TreeTop *tt, TR::Node *node)
{
    ...
    if (rhs->getDataType().isAddress()) {
        ...
        if (newObject != oldObject && trace()) {
            comp()->log()->printf("Local #%2d obj%d -> obj%d at node n%dn\n", local->getLocalIndex(), oldObject,
                newObject, node->getGlobalIndex());
        }
        ...
        if (knot && isKnownObject(baseObj) && !knot->isNull(baseObj)) {
            ...
            if (symbol->isArrayShadowSymbol() && knot->isArrayWithConstantElements(baseObj)) {
                TR::SymbolReference *improvedSymRef
                    = comp()->getSymRefTab()->findOrCreateImmutableArrayShadowSymbolRef(symbol->getDataType());
...
void TR_MethodHandleTransformer::visitCall(TR::TreeTop *tt, TR::Node *node)
{
    ...
    switch (rm) {
        ...
        case TR::java_lang_invoke_Invokers_checkGenericType:
            process_java_lang_invoke_Invokers_MethodHandle_checkGenericType(tt, node);
            break;
        default:
            break;
    }
...
//
void TR_MethodHandleTransformer::visitNode(TR::TreeTop *tt, TR::Node *node, TR::NodeChecklist &visitedNodes)
{
...
void TR_MethodHandleTransformer::processBlockAndUpdateObjectInfo(TR::Block *block,
    TR_MethodHandleTransformer::ObjectInfo *blockStartObjectInfo)
{
...
const char *TR_MethodHandleTransformer::optDetailString() const throw() { return "O^O METHODHANDLE TRANSFORMER: "; }
...
void TR_MethodHandleTransformer::process_java_lang_invoke_MethodHandle_invokeBasic(TR::TreeTop *tt, TR::Node *node)
{
    ...
    if (!transformed) {
        TR::DebugCounter::prependDebugCounter(comp(),
            TR::DebugCounter::debugCounterName(comp(), "MHUnknownObj/invokeBasic/(%s %s)", comp()->signature(),
...
void TR_MethodHandleTransformer::process_java_lang_invoke_MethodHandle_linkTo(TR::TreeTop *tt, TR::Node *node)
{
    ...
    if (!transformed) {
        TR::DebugCounter::prependDebugCounter(comp(),
            TR::DebugCounter::debugCounterName(comp(), "MHUnknownObj/linkTo/(%s %s)", comp()->signature(),
                comp()->getHotnessName(comp()->getMethodHotness())),
            tt);
    }
}
...
*/
void TR_MethodHandleTransformer::process_java_lang_invoke_Invokers_checkExactType(TR::TreeTop *tt, TR::Node *node)
{
    ...
    if (isKnownObject(mhIndex) && isKnownObject(expectedTypeIndex)) {
        ...
        if (typesMatch
            && performTransformation(comp(), "%sChanging checkExactType call node n%dn to PassThrough\n",
    ...
    uint32_t typeOffset = fej9->getInstanceFieldOffsetIncludingHeader("Ljava/lang/invoke/MethodHandle;", "type",
        "Ljava/lang/invoke/MethodType;", comp()->getCurrentMethod());
    auto typeSymRef = comp()->getSymRefTab()->findOrFabricateShadowSymbol(comp()->getMethodSymbol(),
        TR::Symbol::Java_lang_invoke_MethodHandle_type, TR::Address, typeOffset, false, true, true,
        "java/lang/invoke/MethodHandle.type Ljava/lang/invoke/MethodType;");
    auto handleTypeNode = TR::Node::createWithSymRef(node, comp()->il.opCodeForIndirectLoad(TR::Address), 1,
        methodHandleNode, typeSymRef);
    ...
    TR::Node *zerochkNode = TR::Node::createWithSymRef(TR::ZEROCHK, 1, 1, cmpEqNode,
...
*/
void TR_MethodHandleTransformer::process_java_lang_invoke_Invokers_checkCustomized(TR::TreeTop *tt, TR::Node *node)
{
    TR::KnownObjectTable::Index objIndex = getObjectInfoOfNode(node->getFirstArgument());
    auto knot = comp()->getKnownObjectTable();
    if (isKnownObject(objIndex) && knot && !knot->isNull(objIndex)) {
        if (!performTransformation(comp(),
                "%sRemoving checkCustomized call node n%dn as it is now redundant as MethodHandle has known object "
                "index\n",
                optDetailString(), node->getGlobalIndex()))
            return;
        TR::TransformUtil::transformCallNodeToPassThrough(this, node, tt, node->getFirstArgument());
...
/*
When the VarHandle object and the AccessDescriptor objects are known, we can perform a compile-time lookup of the
corresponding entry in the VarHandle's MethodHandle table. This function replaces Invokers.checkVarHandleGenericType
call nodes with a aloadi node performing the load of the MethodHandle object that would have been returned by
Invokers.checkVarHandleGenericType. This is achieved through inserting trees that perform loads of the field containing
the MethodHandle table. The various checks that Invokers.checkVarHandleGenericType would have performed are done in
TR_J9VMBase::getMethodHandleTableEntryIndex which returns the object index of the MethodHandle entry in the table if the
checks succeed, following which the Invokers.checkVarHandleGenericType can be folded.
...
treetop
acall
java/lang/invoke/Invokers.checkVarHandleGenericType(Ljava/lang/invoke/VarHandle;Ljava/lang/invoke/VarHandle$AccessDescriptor;)Ljava/lang/invoke/MethodHandle;
   aload  <VarHandle>
    ...
    TR::Node *node)
{
    ...
    if (!performTransformation(comp(),
            ...
            optDetailString(), node->getGlobalIndex(), node, vhIndex, mhIndex)) {
        return;
    }

    // obtain symref(s) for creating load for the MH table base
#if JAVA_SPEC_VERSION <= 17
    uint32_t typesAndInvokersOffset
        = comp()->fej9()->getInstanceFieldOffsetIncludingHeader("Ljava/lang/invoke/VarHandle;", "typesAndInvokers",
            "Ljava/lang/invoke/VarHandle$TypesAndInvokers;", comp()->getCurrentMethod());
    const char *tisFieldNameAndSig
        = "java/lang/invoke/VarHandle.typesAndInvokers Ljava/lang/invoke/VarHandle$TypesAndInvokers;";
    TR::SymbolReference *tisSymRef = comp()->getSymRefTab()->findOrFabricateShadowSymbol(comp()->getMethodSymbol(),
        TR::Symbol::Java_lang_invoke_VarHandle_typesAndInvokers, TR::Address, typesAndInvokersOffset, false, false,
        false, tisFieldNameAndSig);
    ...
    uint32_t mhTableOffset
        = comp()->fej9()->getInstanceFieldOffsetIncludingHeader("Ljava/lang/invoke/VarHandle$TypesAndInvokers;",
            "methodHandle_table", "[Ljava/lang/invoke/MethodHandle;", comp()->getCurrentMethod());
    const char *mhTableFieldNameAndSig
        = "java/lang/invoke/VarHandle$TypesAndInvokers.methodHandle_table [Ljava/lang/invoke/MethodHandle;";
    TR::Node *mhTableHolder = tisNode;
    bool mhTableIsFinal = true;
#else
    uint32_t mhTableOffset = comp()->fej9()->getInstanceFieldOffsetIncludingHeader("Ljava/lang/invoke/VarHandle;",
        "methodHandleTable", "[Ljava/lang/invoke/MethodHandle;", comp()->getCurrentMethod());
    const char *mhTableFieldNameAndSig
        = "java/lang/invoke/VarHandle.methodHandleTable [Ljava/lang/invoke/MethodHandle;";
    ...
    TR::SymbolReference *mhTableSymRef = comp()->getSymRefTab()->findOrFabricateShadowSymbol(comp()->getMethodSymbol(),
        TR::Symbol::Java_lang_invoke_VarHandle_methodHandleTable, TR::Address, mhTableOffset, false, false,
        mhTableIsFinal, mhTableFieldNameAndSig);
    ...
    TR::Node *mhAddressNode
        = J9::TransformUtil::calculateElementAddress(comp(), mhTableNode, arrayIndexNode, TR::Address);
    mhAddressNode->copyByteCodeInfo(node);

    anchorAllChildren(node, tt);
    node->removeAllChildren();
    TR::Node::recreateWithSymRef(node, TR::aloadi,
        comp()->getSymRefTab()->findOrCreateArrayShadowSymbolRef(TR::Address, mhTableNode));
    node->setNumChildren(1);
    node->setAndIncChild(0, mhAddressNode);
    TR::SymbolReference *improvedMHSymRef
        = comp()->getSymRefTab()->findOrCreateSymRefWithKnownObject(node->getSymbolReference(), mhIndex);
    node->setSymbolReference(improvedMHSymRef);

    // Insert spine check for arraylets
    if (TR::Compiler->om.canGenerateArraylets()) {
        TR::SymbolReference *spineCHKSymRef
...
void TR_MethodHandleTransformer::process_java_lang_invoke_Invokers_MethodHandle_checkGenericType(TR::TreeTop *tt, TR::Node *node)
{
    if (!comp()->useConstRefs())
        return;
    ...
    if (knot && isKnownObject(mhIndex) && !knot->isNull(mhIndex) && isKnownObject(desiredMTIndex)
        ...
        TR::KnownObjectTable::Index convertedMHIndex = comp()->fej9()->getConvertedMethodhandle(comp(), mhIndex, desiredMTIndex);
