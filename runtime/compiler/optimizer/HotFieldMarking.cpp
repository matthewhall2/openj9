 ...
 * Member '_count' contains the number of blocks within the compilation that contributes to the frequency score value of
 * the field Member '_score' contains the combined block frequency score of the field for the compilation Member
 * '_clazz' contains the class of the field
 */
struct SymStats {
    int32_t _count;
    int32_t _score;
    ...
    SymStats(int32_t count, int32_t score, int32_t fieldNameLength, char *fieldName, int32_t fieldSigLength,
        char *fieldSig, TR_OpaqueClassBlock *clazz)
        : _count(count)
        , _score(score)
...
int32_t TR_HotFieldMarking::perform()
{
    ...
    for (TR::PostorderNodeIterator it(comp()->getStartTree(), comp()); it != NULL; ++it) {
        ...
        if (node->getOpCodeValue() == TR::BBStart) {
            ...
            && node->getSymbolReference()->getSymbol()->isShadow() && !node->isInternalPointer()
            && !node->getOpCode().isArrayLength() && node->getSymbolReference()->getSymbol()->isCollectedReference()) {
            TR::SymbolReference *symRef = node->getSymbolReference();
            if (symRef->getCPIndex() >= 0 && !symRef->getSymbol()->isArrayShadowSymbol()) {
                auto itr = stats.find(symRef->getSymbol());
                if (itr != stats.end()) {
                    itr->second->_score
                        = getReducedFrequency(itr->second->_score, itr->second->_count, block->getFrequency());
                    itr->second->_count += 1;
                    continue;
                }
                ...
                TR_OpaqueClassBlock *containingClass
                    = static_cast<TR_ResolvedJ9Method *>(method)->definingClassFromCPFieldRef(comp(),
                        symRef->getCPIndex(), isStatic);
                if (isStatic)
                    continue;

                stats[symRef->getSymbol()] = new (trStackMemory()) SymStats(1, block->getFrequency(), fieldNameLength,
                    fieldName, fieldSigLength, fieldSig, containingClass);
            }
        }
    }
    ...
    for (auto itr = stats.begin(), end = stats.end(); itr != end; ++itr) {
        if (itr->second->_score >= TR::Options::_hotFieldThreshold) {
            uint32_t fieldOffset
                = (comp()->fej9()->getInstanceFieldOffset(itr->second->_clazz, itr->second->_fieldName,
                       itr->second->_fieldNameLength, itr->second->_fieldSig, itr->second->_fieldSigLength)
            ...
            if (!comp()->fej9()->isAnonymousClass(itr->second->_clazz)
                && performTransformation(comp(),
                    "%sUpdate hot field info for hot field. fieldSignature: %s; fieldName: %s; frequencyScore = %d\n",
                    optDetailString(), itr->second->_fieldSig, itr->second->_fieldName, itr->second->_score)
                && (fieldOffset < U_8_MAX)) {
                comp()->fej9()->reportHotField(getUtilization(),
                    TR::Compiler->cls.convertClassOffsetToClassPtr(itr->second->_clazz), (uint8_t)fieldOffset,
                    itr->second->_score);
                if (trace) {
                    int32_t classNameLength = 0;
                    char *className = comp()->fej9()->getClassNameChars(itr->second->_clazz, classNameLength);
                    log->printf("<traceMarkingOfHotFields\n"
                                "\tmethodSignature=\"%s\"\n"
                                "\tmethodHotness=\"%s\"\n"
                                "\tclassName=\"%s\"\n"
                                "\tfieldName=\"%s\""
                                ...
                                "\tfrequencyScore=%d"
                                "\tfieldOffset=%d>\n",
                        comp()->signature(), comp()->getHotnessName(comp()->getMethodHotness()), className,
                        itr->second->_fieldName, itr->second->_fieldSig, itr->second->_score, fieldOffset);
                }
            }
        }
    }
...
int32_t TR_HotFieldMarking::getUtilization()
{
    ...
    static int32_t hotFieldMarkingUtilizationWarmAndBelowValue
    ...
    static int32_t hotFieldMarkingUtilizationHotValue
        = (hotFieldMarkingUtilizationHot = feGetEnv("TR_hotFieldMarkingUtilizationHot"))
        ? atoi(hotFieldMarkingUtilizationHot)
        : 10;

    static const char *hotFieldMarkingUtilizationScorching;
    static int32_t hotFieldMarkingUtilizationScorchingValue
        = (hotFieldMarkingUtilizationScorching = feGetEnv("TR_hotFieldMarkingUtilizationScorching"))
        ? atoi(hotFieldMarkingUtilizationScorching)
        : 100;
...
const char *TR_HotFieldMarking::optDetailString() const throw() { return "O^O HOT FIELD MARKING: "; }
