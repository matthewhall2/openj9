...
void TR_CISCNode::initializeMembers(uint32_t opc, uint16_t id, int16_t dagId, uint16_t ncfgs, uint16_t nchildren)
{
    initializeLists();
    _flags.clear();
    setOpcode(opc);
...
//*****************************************************************************************
bool TR_CISCNode::isEqualOpc(TR_CISCNode *t)
{
    static_assert((int)TR::NumAllIlOps == TR_variable, "assumption for reducing compilation time");
    ...
    else if (pOpc > TR::NumAllIlOps) // Please see the above assumption
    {
        switch (pOpc) {
            ...
            case TR_ibcload:
                return (t->_ilOpCode.isLoadIndirect()
                    && (t->_ilOpCode.isByte() || (t->_ilOpCode.isShort() && t->_ilOpCode.isUnsigned())));
            case TR_ibcstore:
                return (t->_ilOpCode.isStoreIndirect()
                    && (t->_ilOpCode.isByte() || (t->_ilOpCode.isShort() && t->_ilOpCode.isUnsigned())));
        }
    }
    return false;
...
//*****************************************************************************************
const char *TR_CISCNode::getName(TR_CISCOps op, TR::Compilation *comp)
{
...
// Debug print to file
//*****************************************************************************************
void TR_CISCNode::dump(OMR::Logger *log, TR::Compilation *comp)
{
    int32_t i;
    char buf[256];
...
//*****************************************************************************************
void TR_CISCNode::replaceSucc(uint32_t index, TR_CISCNode *to)
{
...
//*****************************************************************************************
void TR_CISCNode::replaceChild(uint32_t index, TR_CISCNode *ch)
{
...
struct TR_StackForCheckParents {
    ...
    TR_StackForCheckParents(TR_CISCNode *p, TR_CISCNode *t, ListElement<TR_CISCNode> *ple,
        ListElement<TR_CISCNode> *tle, ListElement<TR_CISCNode> *tcle, TR_CISCNode *pn, TR_CISCNode *tn,
        uint16_t pIndex, int8_t level, int8_t label)
    {
        initialize(p, t, ple, tle, tcle, pn, tn, pIndex, level, label);
    }

    void initialize(TR_CISCNode *p, TR_CISCNode *t, ListElement<TR_CISCNode> *ple, ListElement<TR_CISCNode> *tle,
        ListElement<TR_CISCNode> *tcle, TR_CISCNode *pn, TR_CISCNode *tn, uint16_t pIndex, int8_t level, int8_t label)
    {
        _p = p;
        _t = t;
        _ple = ple;
...
//*****************************************************************************************
bool TR_CISCNode::checkParentsNonRec(TR_CISCNode *p, TR_CISCNode *t, int8_t level, TR::Compilation *comp)
{
...
//*****************************************************************************************
void TR_CISCNode::reverseBranchOpCodes()
{
...
//*****************************************************************************************
TR::TreeTop *TR_CISCNode::getDestination(bool isFallThrough)
{
    TR::TreeTop *ret;
    ...
    if (isFallThrough) {
        for (ret = getHeadOfTreeTop()->getNextTreeTop(); ret->getNode()->getOpCodeValue() != TR::BBStart;
...
bool TR_CISCHash::add(uint64_t key, TR_CISCNode *node, bool checkExist)
{
    TR_ASSERT(_numBuckets > 0, "error: _numBuckets == 0!");
    uint32_t index = key % _numBuckets;
    if (checkExist) {
    ...
    struct HashTableEntry *newEntry
        = (struct HashTableEntry *)trMemory()->allocateMemory(sizeof(*newEntry), _allocationKind);
...
void TR_CISCGraphAspects::setLoadAspects(uint32_t val, bool orExistAccess)
{
    TR_ASSERT(val <= loadMasks, "error!");
    if (orExistAccess && (val & 0xFF))
        val |= existAccess;
    set(val);
}

void TR_CISCGraphAspects::setStoreAspects(uint32_t val, bool orExistAccess)
{
    TR_ASSERT(val <= loadMasks, "error!");
    if (orExistAccess && (val & 0xFF))
        val |= existAccess;
...
void TR_CISCGraphAspects::modifyAspects()
{
    uint32_t load = getLoadAspects();
    uint32_t store = getStoreAspects();
    uint32_t temp = load & store & 0xff;
    if (temp)
        set(sameTypeLoadStore);
}

void TR_CISCGraphAspects::print(TR::Compilation *comp, bool noaspects)
{
    comp->log()->printf("CISCGraph%sAspects is %08x\n", noaspects ? "No" : "", getValue());
}

void TR_CISCGraphAspectsWithCounts::print(TR::Compilation *comp, bool noaspects)
{
    comp->log()->printf("CISCGraph%sAspects is %08x\n", noaspects ? "No" : "", getValue());
    comp->log()->printf("min counts: if=%d, indirectLoad=%d, indirectStore=%d\n", _ifCount, _indirectLoadCount,
        _indirectStoreCount);
}

void TR_CISCGraphAspectsWithCounts::setAspectsByOpcode(int opc)
{
    switch (opc) {
        case TR_inbload:
...
/***********              ************/
/*************************************/
void TR_CISCGraph::setEssentialNodes(TR_CISCGraph *tgt)
{
    ListIterator<TR_CISCNode> ni(tgt->getNodes());
    TR_CISCNode *n;

    for (n = ni.getFirst(); n; n = ni.getNext()) {
        if (n->getIlOpCode().isStore() || n->getIlOpCode().isCall()) {
...
//
void TR_CISCGraph::makePreparedCISCGraphs(TR::Compilation *c)
{
...
void TR_CISCGraph::initializeGraphs(TR::Compilation *c)
{
    int32_t num = 0;
    ...
    if (genTRT) {
        ...
        setEssentialNodes(preparedCISCGraphs[num++]);
        // preparedCISCGraphs[num] =  makeTRT4NestedArrayIfGraph(c, ctrl); setEssentialNodes(preparedCISCGraphs[num]);
        // num++;
    }
    if (genMemset) {
    ...
    if (genMemcpy) {
        ...
        // disabled for now
#if STRESS_TEST
        preparedCISCGraphs[num] = makeMEMCPYByte2IntGraph(c, ctrl);
        setEssentialNodes(preparedCISCGraphs[num]);
        num++;
        preparedCISCGraphs[num] = makeMEMCPYInt2ByteGraph(c, ctrl);
        setEssentialNodes(preparedCISCGraphs[num]);
        num++;
#endif
    }
    ...
    if (genDecimal) {
        ...
        setEssentialNodes(preparedCISCGraphs[num++]);
#if STRESS_TEST
        preparedCISCGraphs[num] = makeLongToStringGraph(c, ctrl);
        setEssentialNodes(preparedCISCGraphs[num]);
        num++;
#endif
    }
...
void TR_CISCGraph::dump(OMR::Logger *log, TR::Compilation *comp)
{
...
//*****************************************************************************************
void TR_CISCGraph::addTrNode(TR_CISCNode *n, TR::Block *block, TR::TreeTop *top, TR::Node *trNode)
{
...
//*****************************************************************************************
void TR_CISCGraph::addOpc2CISCNode(TR_CISCNode *n)
{
...
//*****************************************************************************************
void TR_CISCGraph::addNode(TR_CISCNode *n, TR::Block *block, TR::TreeTop *top, TR::Node *trNode)
{
    TR_ASSERT((block == 0 && top == 0 && trNode == 0) || (block != 0 && top != 0 && trNode != 0),
        "error"); // all 0 or all Non-0
    _nodes.add(n);
    if (isRecordingAspectsByOpcode())
        _aspects.setAspectsByOpcode(n);
    if (trNode != 0) {
...
//*****************************************************************************************
TR_CISCNode *TR_CISCGraph::searchStore(TR_CISCNode *target, TR_CISCNode *to)
{
    TR_CISCNode *v = target;
    if (v->isLoadVarDirect())
        v = v->getChild(0);
    if (v->getOpcode() != TR_variable)
        return 0;
    TR_BitVector visited(getNumNodes(), trMemory());

    TR_CISCNode *t = target;
    while (true) {
        if (t->isStoreDirect() && t->getChild(1) == v)
            return t;
        ...
        t = t->getSucc(0);
        if (t == target || t == to || t == getExitNode() || visited.isSet(t->getID()))
            return 0;
    }
...
/***********                           ************/
/**************************************************/
void TR_PCISCGraph::addNode(TR_CISCNode *n, TR::Block *block, TR::TreeTop *top, TR::Node *trNode)
{
...
void TR_CISCGraph::createDagId2NodesTable()
{
    ...
    for (int i = 0; i < _numDagIds; i++)
        _dagId2Nodes[i].setRegion(trMemory()->heapMemoryRegion());
...
void TR_CISCGraph::createOrderByData()
{
    _orderByData.init();
    ListIterator<TR_CISCNode> nodesLi(&_nodes);
    TR_CISCNode *n;
    for (n = nodesLi.getFirst(); n; n = nodesLi.getNext()) {
        if (n->getNumChildren() > 0 || !n->getParents()->isEmpty()) {
            _orderByData.add(n);
        } else {
            switch (n->getOpcode()) {
                case TR_entrynode:
...
void TR_PCISCGraph::createDagId2NodesTable()
{
...
void TR_PCISCGraph::createOrderByData()
{
...
//*****************************************************************************************
void TR_CISCGraph::importUDchains(TR::Compilation *comp, TR_UseDefInfo *useDefInfo, bool reinitialize)
{
    ListIterator<TR_CISCNode> nodesLi(&_nodes);
    TR_CISCNode *n;
    ...
    for (n = nodesLi.getFirst(); n; n = nodesLi.getNext()) {
        if (n->isLoadVarDirect()) {
            ...
            if (!trNode->getSymbol()->isAutoOrParm()) {
                TR_ASSERT_FATAL_WITH_NODE(trNode,
...
//*****************************************************************************************
int32_t TR_CISCGraph::defragDagId()
{
    ListIterator<TR_CISCNode> nodesLi(&_nodes);
    TR_CISCNode *n;
    int32_t curId, newId;
...
//*****************************************************************************************
void TR_CISCGraph::setOutsideOfLoopFlag(uint16_t loopBodyDagId)
{
    ListIterator<TR_CISCNode> li(&_nodes);
    TR_CISCNode *n;
    for (n = li.getFirst(); n; n = li.getNext()) {
        if (n->getDagID() != loopBodyDagId)
            n->setOutsideOfLoop();
    }
}
...
List<TR::CFGNode> *TR_CFGReversePostOrder::compute(TR::CFG *cfg)
{
    createReversePostOrder(cfg, cfg->getEnd());
    return &_revPost;
}

void TR_CFGReversePostOrder::createReversePostOrder(TR::CFG *cfg, TR::CFGNode *n)
{
    TR_StackForRevPost *stack;
    TR_BitVector *visited = new (cfg->comp()->trStackMemory())
        TR_BitVector(cfg->getNextNodeNumber(), cfg->comp()->trMemory(), stackAlloc);
...
void TR_CFGReversePostOrder::dump(TR::Compilation *comp)
{
    OMR::Logger *log = comp->log();
...
/***********                   ************/
/******************************************/
TR_CISCNodeRegion *TR_CISCNodeRegion::clone()
{
...
/***********                   ************/
/******************************************/
TR::Node *TR_NodeDuplicator::restructureTree(TR::Node *oldTree, TR::Node *newTree)
{
...
TR::Node *TR_NodeDuplicator::duplicateTree(TR::Node *org)
{
...
int32_t TR_UseTreeTopMap::buildAllMap()
{
    if (_buildAllMap)
        return 0;
...
void TR_UseTreeTopMap::buildUseTreeTopMap(TR::TreeTop *treeTop, TR::Node *node)
{
    ...
    if (currCount == node->getVisitCount())
        return;
...
{
    _afterInsertionsIdiom
        = (ListHeadAndTail<TR::Node> *)trMemory()->allocateHeapMemory(sizeof(ListHeadAndTail<TR::Node>) * 2);
...
//*****************************************************************************************
bool TR_CISCTransformer::isInsideOfFastVersionedLoop(TR_RegionStructure *l)
{
...
bool TR_CISCTransformer::createLoopCandidates(List<TR_RegionStructure> *loopCandidates)
{
    ...
    detectWhileLoops(whileLoopsInnerFirst, whileLoops, doWhileLoopsInnerFirst, doWhileLoops, _cfg->getStructure(),
        true);
    ...
    if (!candidate->isEmpty()) {
        logprints(enableTracing, log, "createLoopCandidates: Evaluating list of loop candidates.\n");
        ...
        for (nextWhileLoop = whileLoopsIt.getFirst(); nextWhileLoop != 0; nextWhileLoop = whileLoopsIt.getNext()) {
            ...
            if (!loopBlockStructure) {
                logprintf(enableTracing, log, "\tRejected loop %d - no block structure.\n", naturalLoop->getNumber());
                continue;
            }
            if (!naturalLoop->containsOnlyAcyclicRegions()) {
                logprintf(enableTracing, log, "\tRejected loop %d - not inner most loop.\n", naturalLoop->getNumber());
            ...
            logprintf(enableTracing, log, "\tAccepted loop %d as candidate.\n", naturalLoop->getNumber());
        }
#if SHOW_STATISTICS
        if (showMesssagesStdout() && loopCount)
            if (comp()->getMethodHotness() == warm || isAfterVersioning())
                printf("!! #Loop=%d\n", loopCount);
#endif
    }

    logprintf(enableTracing, log, "createLoopCandidates: %d loop candidates found.\n", loopCount);
...
//
TR::Block *TR_CISCTransformer::addPreHeaderIfNeeded(TR_RegionStructure *region)
{
    OMR::Logger *log = comp()->log();
    ...
    for (auto e = loopEntry->getPredecessors().begin(); e != loopEntry->getPredecessors().end(); ++e) {
        ...
        if (predBlock->getStructureOf() && predBlock->getStructureOf()->isLoopInvariantBlock()) {
            preHeader = predBlock;
            break;
        }
    }
    ...
    if (!preHeader) {
        preHeader = TR::Block::createEmptyBlock(loopEntry->getEntry()->getNode(), comp(), loopEntry->getFrequency(),
            loopEntry);
        ...
        for (auto e = loopEntry->getPredecessors().begin(); e != loopEntry->getPredecessors().end(); ++e) {
            ...
            logprintf(trace(), log, "fixing predecessor %d\n", predBlock->getNumber());
        ...
        logprintf(trace(), log, "added preheader block_%d\n", preHeader->getNumber());
    }
...
//
TR::Block *TR_CISCTransformer::findPredecessorBlockOfLoopEntry(TR_RegionStructure *loop)
{
    ...
    if (true /*pred.isDoubleton()*/) {
        for (auto edge = loopEntry->getPredecessors().begin(); edge != loopEntry->getPredecessors().end(); ++edge) {
            ...
            if ((from->getSuccessors().size() == 1) && from->getParentStructureIfExists(_cfg) != loop)
                return from;
        }
    }
    return 0;
...
//*****************************************************************************************
void TR_CISCTransformer::analyzeHighFrequencyLoop(TR_CISCGraph *graph, TR_RegionStructure *naturalLoop)
{
    OMR::Logger *log = comp()->log();
    logprints(trace(), log, "\tAnalyzing if loop is frequently iterated\n");
    ...
    if (loopEntryFrequency <= 0) {
        ...
        if (!outer || outer->getFrequency() < 0) {
            ...
            if (_bblistSucc.isSingleton()) {
                ...
                if (outer->getFrequency() > loopEntryFrequency)
                    outer = 0;
            }
            if (!outer || outer->getFrequency() < 0) {
        ...
        if (outer) {
            ...
            if (outerFrequency < 1)
                outerFrequency = 1;
            logprintf(trace(), log, "\t\tOuter block %d: Frequency=%d Inner/Outer Ratio:(%f)\n", outer->getNumber(),
                outerFrequency, (double)loopEntryFrequency / (double)outerFrequency);
            if (loopEntryFrequency < outerFrequency * cg()->arrayTranslateAndTestMinimumNumberOfIterations())
                highFrequency = false;
        }
    }
#endif
    logprintf(trace(), log, "\t\thighFrequency=%d\n", highFrequency);
...
ificmpge
  ...
  iload    A
*/
void TR_CISCTransformer::easyTreeSimplification(TR::Node * const node)
{
    bool modified = false;
    if (node->getOpCode().isIf()) {
        TR::Node *iconstC = node->getChild(1);
        if (iconstC->getOpCodeValue() != TR::iconst || iconstC->getReferenceCount() > 1)
            return;
        if (node->getOpCodeValue() == TR::ificmplt && iconstC->getInt() == 1) {
            logprintf(trace(), comp()->log(),
                "\t\teasyTreeSimplification: Node: %p converted from ificmplt with 1 to ifcmple with 0", node);
            TR::Node::recreate(node, TR::ificmple);
            iconstC->setInt(0);
        }

        TR::Node *addOrSub = node->getChild(0);
        if (!addOrSub->getOpCode().isAdd() && !addOrSub->getOpCode().isSub())
            return;
        if (addOrSub->getReferenceCount() > 1)
            return;

        TR::Node *iloadB = addOrSub->getChild(1);
        if (iloadB->getOpCodeValue() != TR::iload)
            return;
        if (iloadB->getReferenceCount() > 1)
            return;
        ...
        if (iconstA->getOpCodeValue() == TR::iconst) {
            ...
            if (iloadA->getReferenceCount() > 1 || !addOrSub->getOpCode().isSub())
                return;
            if (node->getOpCodeValue() == TR::ificmple && iconstC->getInt() == 0) {
    ...
    if (modified)
        logprintf(trace(), comp()->log(), "\t\teasyTreeSimplification: The tree %p is simplified.\n", node);
}

//*****************************************************************************************
// analyze the tree "top" and add each node
//*****************************************************************************************
TR_CISCNode *TR_CISCTransformer::addAllSubNodes(TR_CISCGraph * const graph, TR::Block * const block,
    TR::TreeTop * const top, TR::Node * const parent, TR::Node * const node, const int32_t dagId)
{
    ...
    if (node->getVisitCount() == curVisit) // already visited
    {
        ...
        if (parent
            && ((parent->getOpCodeValue() == TR::aiadd && node->getOpCodeValue() == TR::isub
                    && node->getChild(0)->getOpCodeValue() != TR::imul)
                || (parent->getOpCodeValue() == TR::aladd && node->getOpCodeValue() == TR::lsub
                    && node->getChild(0)->getOpCodeValue() != TR::lmul))) {
            ...
            if (!const1) {
                const1 = new (trHeapMemory())
                    TR_CISCNode(trMemory(), opcodeConst, opcodeDataType, graph->incNumNodes(), 0, 0, 0, 1);
                const1->setNewCISCNode();
                graph->addNode(const1, 0, 0, 0);
            }

            TR_CISCNode *mul = new (trHeapMemory()) TR_CISCNode(trMemory(), is64bit ? TR::lmul : TR::imul,
                is64bit ? TR::Int64 : TR::Int32, graph->incNumNodes(), dagId, 1, 2);
            mul->setNewCISCNode();
        ...
        if (node->getOpCode().isLoadVarDirect()) // direct loads (e.g. iload)
        {
            ...
            if (!variable) {
                variable = new (trHeapMemory())
                    TR_CISCNode(trMemory(), TR_variable, TR::NoType, graph->incNumNodes(), 0, 0, 0, refNum);
                variable->addTrNode(block, top, node);
                graph->addNode(variable);
            }

            numChildren = 1;
            newCisc = new (trHeapMemory())
                TR_CISCNode(trMemory(), opcode, nodeDataType, graph->incNumNodes(), dagId, 1, numChildren);
        ...
        } else if (node->getOpCode().isLoadConst()) // constants (e.g. iconst)
        {
            ...
            if (isConst) {
                ...
                if (newCisc) // if the same node is already added
                    ...
                    newCisc = new (trHeapMemory())
                        TR_CISCNode(trMemory(), opcode, nodeDataType, graph->incNumNodes(), 0, 0, 0, val);
            ...
            if (childList.isSingleton()) // for normal direct store (e.g. istore)
            {
                ...
                if (!child->isInterestingConstant())
                    child->setDest(variable);
            ...
            if (node->getOpCode().isCall() && graph->isRecordingAspectsByOpcode())
                return 0;
            ...
            if (opcode == TR::Case) {
                ...
                numChildren = 1;
                newCisc = new (trHeapMemory())
                    TR_CISCNode(trMemory(), opcode, nodeDataType, graph->incNumNodes(), dagId, nSucc, numChildren);
...
//*****************************************************************************************
bool TR_CISCTransformer::makeCISCGraphForBlock(TR_CISCGraph *graph, TR::Block * const block, int32_t dagId)
{
    OMR::Logger *log = comp()->log();

    logprintf(trace(), log, "\t\tmakeCISCGraphForBlock: Building CISCGraph for block %d.\n", block->getNumber());
    ...
    while (true) {
        if (!addAllSubNodes(graph, block, top, NULL, top->getNode(), dagId)) {
            logprintf(trace(), log, "\t\tFailed to create CISCNode for Node %p in block %d : %p\n", top->getNode(),
...
//*****************************************************************************************
void TR_CISCTransformer::resolveBranchTargets(TR_CISCGraph *graph, TR_CISCNode *exitNode)
{
    ...
    for (p = ci.getFirst(); p != 0; p = ci.getNext()) // each element of _backPatchList
    {
        ...
        if (trNode->getOpCode().isBranch()) {
            ...
            if (!destCisc)
                destCisc = exitNode; // set exitNode if the destination is outside
    ...
    for (p = ci.getFirst(); p != 0; p = ci.getNext()) {
        ...
        if (numSuccs >= 2) // To exclude 0 and 1 quickly (reduce compilation time)
        {
            if (numSuccs == 2) // Typical case in "numSuccs >= 2" (reduce compilation time)
            {
                ...
                if (succ0->getOpcode() == TR::BBEnd)
                    p->setSucc(0, (succ0 = succ0->getSucc(0)));
                if (succ1->getOpcode() == TR::BBEnd)
                    p->setSucc(1, (succ1 = succ1->getSucc(0)));
                ...
                for (idx = 0; idx < numSuccs; idx++) {
                    ...
                    if (s->getOpcode() == TR::BBEnd)
                        p->setSucc(idx, s->getSucc(0));
                }
...
uint16_t TR_CISCTransformer::renumberDagId(TR_CISCGraph *graph, int32_t tempMaxDagId, int32_t bodyDagId)
{
   ...
   for (int32_t dId = tempMaxDagId; dId >= 0; dId--)
    ...
    for (int32_t dId = tempMaxDagId; dId >= 0; dId--) {
        while (true) {
            cur = orgNodes->getListHead();
            if (!cur || cur->getData()->getDagID() != dId)
                break;
...
//
TR_CISCGraph *TR_CISCTransformer::makeCISCGraph(List<TR::Block> *pred, List<TR::Block> *body, List<TR::Block> *succ)
{
    ...
    // make entry node
    TR_CISCNode *newCisc
        = new (trHeapMemory()) TR_CISCNode(trMemory(), TR_entrynode, TR::NoType, graph->incNumNodes(), dagId, 1, 0);
    ...
    if (includePreds) {
        logprints(trace(), comp()->log(), "\tmakeCISCGraph: Building CISCGraph for Predecessor Blocks.\n");
        for (block = bi.getFirst(); block != 0; block = bi.getNext()) {
            if (!makeCISCGraphForBlock(graph, block, dagId))
                return 0;
        }
    }
    ...
    bodyDagId = dagId;
    bi.set(body);
    logprints(trace(), comp()->log(), "\tmakeCISCGraph: Building CISCGraph for Loop Body Blocks.\n");
   ...
   for (block = bi.getFirst(); block != 0; block = bi.getNext())
    ...
    TR_CISCNode *exitNode
        = new (trHeapMemory()) TR_CISCNode(trMemory(), TR_exitnode, TR::NoType, graph->incNumNodes(), dagId, 0, 0);
    ...
    if (!graph->getCISCNode(opcode, true, ahsize)) {
        graph->addNode(
            new (trHeapMemory()) TR_CISCNode(trMemory(), opcode, nodeDataType, graph->incNumNodes(), 0, 0, 0, ahsize));
    }
...
int32_t TR_CISCTransformer::perform()
{
    ...
    if (!comp()->mayHaveLoops() || methodHotness < minimumHotnessPrepared || comp()->getProfilingMode() == JitProfiling
        || // if this method is now profiled
        !enable)
        return 0;
        ...
        if (createLoopCandidates(&loopCandidates)) {
            ...
            for (nextLoop = loopIt.getFirst(); nextLoop != 0; nextLoop = loopIt.getNext()) {
                    ...
                    if (region != NULL) {
                        ...
                        while (predBlock != cfgStart) {
                            ...
                            if (region != predBlockRegion) {
                                logprintf(trace(), log,
                                    "Skip the predecessor block_%d, because it is within another region/loop.\n",
                                    predBlock->getNumber());
                                break;
                            }
                        }
                ...
                for (edge = ei.getFirst(); edge != 0; edge = ei.getNext()) {
                    ...
                    if (!toStructureSubGraphNode(from)->getStructure()->asBlock())
                        continue;
                    ...
                    if (edgeFrom != from->getSuccessors().end()) {
                        ...
                        if (block->getEntry() && !bblistSucc.find(block) && !bblistPred.find(block))
                ...
                // Iterate through the loop body blocks to remove Bits.keepAlive() or Reference.reachabilityFence()
                // calls. This is a NOP function inserted into NIO libraries to keep the NIO object and its native ptr
                // alive until after the native pointer accesses.
                ...
                if (!graph) {
                    logprintf(trace(), log, "Loop %d.  Failed to make CISC Graph.\n", nextLoop->getNumber());
            ...
            if (showMesssagesStdout())
                printf("Exiting CISCTransformer\n");
            cost = 1;
        }
...
const char *TR_CISCTransformer::optDetailString() const throw() { return "O^O IDIOM RECOGNITION: "; }

// check if the block is really in the loop body
//
bool TR_CISCTransformer::isBlockInLoopBody(TR::Block *block)
{
    ListIterator<TR::Block> bi(&_bblistBody);
...
void TR_CISCTransformer::showEmbeddedData(const char *title, uint8_t *data)
{
    OMR::Logger *log = comp()->log();
    int32_t i, j;
    log->printf("%s\n    ", title);
...
//***************************************************************************************
bool TR_CISCTransformer::checkParents(TR_CISCNode *p, TR_CISCNode *t, uint8_t *result, bool *inLoop,
    bool *optionalParents)
{
    ...
    for (pn = pi.getFirst(); pn; pn = pi.getNext()) // for each parent of p
    {
        ...
        for (tn = ti.getFirst(); tn; tn = ti.getNext()) // for each parent of t
        {
            if (isPnInsideOfLoop && tn->isOutsideOfLoop())
                continue;
            if (pn->isEqualOpc(tn)) {
                if (result[tmpIdx + tn->getID()] == _Embed && (commutative || tn->getChild(pIndex) == t))
                    break;
            } else {
                if (tn->getIlOpCode().isLoadVarDirect()) /* search one more depth */
                {
                    ...
                    for (tcn = tci.getFirst(); tcn; tcn = tci.getNext()) {
                        if (pn->isEqualOpc(tcn) && result[tmpIdx + tcn->getID()] == _Embed
                            && (commutative || tcn->getChild(pIndex) == tn))
                            break;
                    }
                    if (tcn)
                        break;
            ...
            if (!pn->isOptionalNode()) {
                ...
                if (!pn->getParents()->isEmpty() && !pn->isSkipParentsCheck()) {
                    ...
                    if (checkParents(pn, t, result, &nextInLoop, &nextOptionalParents)) {
                        if (!nextOptionalParents)
                            allOptionalParents = false;
                        if (nextInLoop)
                            isTargetInsideOfLoop = true;
                    } else {
                        return false;
                    }
                }
...
//***************************************************************************************
bool TR_CISCTransformer::computeEmbeddedForData()
{
    OMR::Logger *log = comp()->log();
    ...
    while (true) // This loop is for narrowing down candidates of a leaf.
    {
        ...
        for (ple = plistHead; ple; ple = ple->getNextElement()) {
            ...
            const bool checkLoopInvariant
                = pOpc == TR_variableORconst || pOpc == TR_quasiConst || pOpc == TR_quasiConst2 || pOpc == TR_arraybase;
            ...
            while (true) {
                ...
                if (numPChi != numTChi && numPChi != 0) {
                    if ((!isBoolTable || numTChi < 1) && (!isAllowWrtbar || numTChi < 2))
                        isEmbed = false;
                }

                if (isEmbed) {
                    if (skipScreening && isNecessaryScreening) {
                        existEmbed = true;
                        isEmbed = (result[index] == _Embed);
                    } else if ((isEmbed = p->isEqualOpc(t)) == true) {
                        // if its a constant, check if values match
                        if (isCheckOtherInfo && pOtherInfo != t->getOtherInfo()) {
                            isEmbed = false;
                            ...
                            if (checkLoopInvariant) {
                                if (tOpc == TR_variable) {
                                    ...
                                    for (parent = parenti.getFirst(); parent; parent = parenti.getNext()) {
                                        if (!parent->isOutsideOfLoop() && parent->getIlOpCode().isStoreDirect()) {
                                            logprintf(trace(), log, "pID%d: tID%d isn't loop invariant because of %d\n",
                            ...
                            //
                            if (isEmbed)
                                isEmbed = TR_CISCNode::checkParentsNonRec(p, t, DEPTH_CHECKPARENTS, comp());
                        }
                    }
                                    ...
                                    if (chiData == _Embed
                                        || ((chiData == _Desc)
                                            && (!isChildDirectlyConnected
                                                || (tch->isLoadVarDirect()
                                                    && _Embed
                                                        == result[idx(pch->getID(), tch->getChild(0)->getID())])))) {
                                        // OK!
                                    } else {
                                        isEmbed = false;
                                        break;
                ...
                if (isEmbed) {
                    ...
                    {
                        if (tOpc != TR::arraylength) // The type of the child (ref) is different from the type of
                                                     // destination (int).
                        {
        ...
        for (ple = plistHead; ple; ple = ple->getNextElement()) {
            ...
            if (trace() && count == 0)
                log->printf("screening1: count=%d for p:%d\n", count, p->getID());
            count = 0;
            for (tle = tlistHead; tle; tle = tle->getNextElement()) {
                t = tle->getData();
                if (result[tmpIdx + t->getID()] == _Embed) {
                    bool inLoop = false;
                    bool allOptionalParents = false;
                    if (!checkParents(p, t, result, &inLoop, &allOptionalParents)
                        || !(inLoop || lightScreening || (checkOptionalParents && allOptionalParents))) {
                        modifyEmbeddedResult = true;
                        result[tmpIdx + t->getID()] = _NotEmbed;
                        logprintf(trace(), log, "screening1: set _NotEmbed to (%d, %d)\n", p->getID(), t->getID());
                    } else {
                        count++;
                    }
                }
                ...
                for (s = si.getFirst(); s; s = si.getNext()) {
                    if (mOpcode == s->getOpcode()) {
                        ...
                        if (result[tmpMIdx + tID] == _Embed) {
                            ...
                            logprintf(trace(), log, "screening2, sameOpcode: set _NotEmbed to (%d, %d)\n", m->getID(),
                                tID);
                        }
                    }
                }
                    ...
                    for (s = si.getFirst(); s; s = si.getNext()) {
                        if (mOpcode != s->getOpcode()) {
                            ...
                            if (result[tmpMIdx + tID] == _Embed) {
                                ...
                                thisScreening = true;
                                result[tmpMIdx + tID] = _NotEmbed;
                                logprintf(trace(), log, "screening2, others: set _NotEmbed to (%d, %d)\n", m->getID(),
                                    tID);
                            }
                        }
                    }
    ...
    for (ple = plistHead; ple; ple = ple->getNextElement()) {
        ...
        for (tle = tlistHead; tle; tle = tle->getNextElement()) {
            ...
            if (result[tmpIdx + t->getID()] == _Embed) {
                ...
                if (!checkParents(p, t, result, &inLoop, &allOptionalParents) || !(inLoop || lightScreening)) {
                    result[tmpIdx + t->getID()] = _NotEmbed;
                    logprintf(trace(), log, "screening3: set _NotEmbed to (%d, %d)\n", p->getID(), t->getID());
                }
            }
        }
    }
...
//***************************************************************************************
bool TR_CISCTransformer::dagEmbed(TR_CISCNode *np, TR_CISCNode *nt)
{
    ...
    if (_embeddedForData[index] == _Embed && ((numPSucc == numTSucc) || (numPSucc == 0))) {
...
// I relaxed degree checking for TR_booltable to find switch-case statements.
//***************************************************************************************
bool TR_CISCTransformer::cycleEmbed(uint16_t dagP, uint16_t dagT)
{
...
//***************************************************************************************
bool TR_CISCTransformer::computeEmbeddedForCFG()
{
...
//***************************************************************************************
bool TR_CISCTransformer::makeLists()
{
    OMR::Logger *log = comp()->log();
    TR_CISCNode *p, *t;
    ...
    int i;
    for (i = 0; i < _numPNodes; i++)
        _P2T[i].setRegion(trMemory()->heapMemoryRegion());
    for (i = 0; i < _numTNodes; i++)
        _T2P[i].setRegion(trMemory()->heapMemoryRegion());
    ...
    for (p = pi.getFirst(); p; p = pi.getNext()) {
        ...
        if (pList->isMultipleEntry() && p->getOpcode() == TR_variable) {
            if (!p->isOptionalNode()) {
                logprintf(trace(), log, "makeLists: pid:%d a variable corresponds to multiple nodes\n", p->getID());
...
//*****************************************************************************
int32_t TR_CISCTransformer::analyzeConnectionOnePairChild(TR_CISCNode * const p, TR_CISCNode * const t,
            ...
            if (tnBefore->isLoadVarDirect()) {
                ...
                if (!success) {
                    ...
                    while (preds->isSingleton()) {
                        ...
                        if (n->isStoreDirect() && n->getChild(1) == tnBefore
...
//*****************************************************************************
void TR_CISCTransformer::analyzeConnectionOnePair(TR_CISCNode * const p, TR_CISCNode * const t)
{
    int32_t i, /*j,*/ num;
    uint8_t *result;
    TR_CISCNode *tn, *pn;
    ...
    TR_ASSERT(t->getNumChildren() == num || p->getOpcode() == TR_arrayindex || p->getOpcode() == TR_arraybase
            || p->getOpcode() == TR_quasiConst || p->getOpcode() == TR_quasiConst2 || p->getOpcode() == TR_booltable
            || p->getOpcode() == TR_inbstore || p->getOpcode() == TR_indstore,
        "error");
    if (p->getParents()->isEmpty() || t->getParents()->isEmpty() || t->getOpcode() == TR::Case
        || t->getOpcode() == TR::awrtbari)
        t->setIsParentSimplyConnected();
    ...
    if (num == 0) {
        ...
        for (i = 0; i < num; i++) // for each child
        {
            const bool commutative = p->isCommutative();
            pn = p->getChild(i);
            while (pn->isOptionalNode() && _P2T[pn->getID()].isEmpty() && pn->getNumChildren() > 0) {
                pn = pn->getChild(0);
            }
            const uint32_t tmpIdx = idx(pn->getID(), 0);
            ...
            while (true) {
                ...
                if (pn->isOptionalNode() && pn->getNumChildren() > 0) {
    ...
    num = t->getNumSuccs();
    if (t->getPreds()->isEmpty() || p->getPreds()->isEmpty())
        t->setIsPredSimplyConnected();
    ...
    } else if (p->getNumSuccs() == num) // typical case
    {
        ...
        {
            pn = p->getSucc(i);
            while (pn->isOptionalNode() && _P2T[pn->getID()].isEmpty() && pn->getNumSuccs() > 0) {
            ...
            while (true) {
                ...
                if (!tn->isPredSimplyConnected() && pn->isOptionalNode()) {
                    while (pn->isOptionalNode() && pn->getNumSuccs() > 0) {
                        pn = pn->getSucc(0); // skip optional nodes
                    }
                    continue; // retry!
                }
...
void TR_CISCTransformer::showT2P()
{
    OMR::Logger *log = comp()->log();
    if (trace()) {
        TR_CISCNode *p, *t;
            ...
            for (t = ti.getFirst(); t; t = ti.getNext()) {
                ...
                if (!_T2P[tID].isEmpty()) {
                    ...
                    log->printf(" %c%c%c%c", t->isSuccSimplyConnected() ? 'S' : 'x',
                        t->isPredSimplyConnected() ? 'P' : 'x', t->isParentSimplyConnected() ? 'B' : 'x',
...
//*****************************************************************************
void TR_CISCTransformer::analyzeConnection()
{
    TR_CISCNode *p, *t;
    ListIterator<TR_CISCNode> pi(_P->getNodes());
    ...
    while (true) {
        ...
        if (!specialTransformer || !specialTransformer(this))
            break;
        if (++count > 10)
            break;
    }
...
//*****************************************************************************
void TR_CISCTransformer::analyzeArrayHeaderConst()
{
    OMR::Logger *log = comp()->log();
    int32_t i = 0;
    while (true) // check all TR_ahconst
    {
        ...
        for (t = p2ti.getFirst(); t; t = p2ti.getNext()) // for each target node
        {
                        ...
                        if (loadNode->getOpcode() == TR_variable) {
                            ...
                            for (subNode = loadPi.getFirst(); subNode; subNode = loadPi.getNext()) {
                                if (parent != subNode && subNode->getIlOpCode().isSub()) {
                                    ...
                                    if (constNode->isValidOtherInfo() && constNode->getIlOpCode().isLoadConst()
            ...
            if (invalidate) {
                ...
                logprintf(trace(), log,
                    "tid:%d (pid:%d) is invalidated because of failure of analyzeArrayHeaderConst\n", tid, pid);
...
void TR_CISCTransformer::showCISCNodeRegion(TR_CISCNodeRegion *r, TR::Compilation *comp)
{
    OMR::Logger *log = comp->log();
...
void TR_CISCTransformer::showCISCNodeRegions(List<TR_CISCNodeRegion> *regions, TR::Compilation *comp)
{
...
//*****************************************************************************
bool TR_CISCTransformer::alignTopOfRegion(TR_CISCNodeRegion *r)
{
    OMR::Logger *log = comp()->log();
    ListElement<TR_CISCNode> *le;
    ...
    while (true) {
        ...
        if (t == 0) {
            if (!pTop->isOptionalNode()) {
                logprintf(trace(), log,
                    "alignTopOfRegion failed. There is no target node corresponding to %d.  Check for nodes in broken "
                    "region listings above and x in SPBC listing.\n",
                    pTop->getID());
                return false;
            }
        } else {
            if (!pTop->isOptionalNode() || r->isIncluded(t))
                break;
    ...
    logprintf(trace(), log, "alignTopOfRegion: (pTop, t) is (%d, %d)\n", pTop->getID(), t->getID());
    ...
    for (le = r->getListHead(); le; le = le->getNextElement()) {
        ...
        if (t->isNegligible() || !pHasCFG) {
            if (!firstNegligible && t->getOpcode() != TR::BBEnd)
                firstNegligible = le;
        } else {
            firstNegligible = 0;
        }
    }
    logprintf(trace(), log, "alignTopOfRegion failed. Cannot find pTop:%d in the region.\n", pTop->getID());
    return false;
}
...
//*****************************************************************************
bool TR_CISCTransformer::areAllNodesIncluded(TR_CISCNodeRegion *r)
{
    ListIterator<TR_CISCNode> ni;
    TR_CISCNode *t;
...
//*****************************************************************************
void TR_CISCTransformer::moveCISCNodesInList(List<TR_CISCNode> *l, TR_CISCNode *from, TR_CISCNode *to,
    TR_CISCNode *moveTo)
{
    logprintf(trace(), comp()->log(), "moveCISCNodesInList: r_from:%p(%d) r_to:%p(%d) moveTo:%p(%d)\n", from,
        from->getID(), to, to->getID(), moveTo, moveTo->getID());

    ListElement<TR_CISCNode> *before = 0, *beforeFrom = 0, *beforeMoveTo = 0, *fromLe = 0, *toLe = 0, *moveToLe = 0,
                             *le;
    ...
    if (moveTo == 0) {
        ...
        if (moveToLe == 0)
            return; // the case if the assertion failed
    }
    TR_ASSERT(fromLe != 0 && toLe != 0, "error");
    if (fromLe == 0 || toLe == 0)
        return; // the case if the assertion failed
    if (toLe == beforeMoveTo)
...
// * _T->_orderByData
//*****************************************************************************
void TR_CISCTransformer::moveCISCNodes(TR_CISCNode *from, TR_CISCNode *to, TR_CISCNode *moveTo, const char *debugStr)
{
    if (showMesssagesStdout()) {
    ...
    if (!moveTo) {
        ...
        while (lastDagIdElement) {
            if (lastDagIdElement->getData() == moveTo)
                break;
            beforeLastDagIdElement = lastDagIdElement;
            lastDagIdElement = lastDagIdElement->getNextElement();
        }
    ...
    if (to->getNumChildren() != 0 || !to->getParents()->isEmpty()) // if "to" has a child or a parent.
    {
        ...
        while (fromData->getNumChildren() == 0 && fromData->getParents()->isEmpty())
            fromData = fromData->getSucc(0);
        while (nextDstData->getNumChildren() == 0 && nextDstData->getParents()->isEmpty()
            && nextDstData->getOpcode() != TR_exitnode)
            nextDstData = nextDstData->getSucc(0);
...
//*****************************************************************************
TR_CISCNodeRegion *TR_CISCTransformer::extractMatchingRegion()
{
    OMR::Logger *log = comp()->log();
    TR_CISCNodeRegion *lists
        = new (trHeapMemory()) TR_CISCNodeRegion(_numTNodes, comp()->trMemory()->heapMemoryRegion());
            ...
            if (!_T2P[tID].isEmpty() && (t->isDataConnected() || t->isNegligible())) {
                ...
                if (t->getIlOpCode().isIf() && !t->isOutsideOfLoop())
                    if (!t->isPredSimplyConnected() && !isSingleLoopBody) {
                        isEmbed = false;
                        if (showMesssagesStdout()) {
                            printf("!!!!!!!!!!!!!! Predecessor of tID %" OMR_PRIu32
                                   " is different from that of idiom.\n",
                                tID);
                        }
                        logprintf(trace(), log, "Predecessor of tID %" OMR_PRIu32 " is different from that of idiom.\n",
                            tID);
                    }
            }
            if (isEmbed) {
                ...
                if (empty && firstNegligible) {
                    ...
                    for (tmp_ele = firstNegligible; true; tmp_ele = tmp_ele->getNextElement()) {
                        neg = tmp_ele->getData();
                        if (!neg->isNegligible() || !_T2P[neg->getID()].isEmpty())
                            break;
                        lists->append(neg);
                    }
                ...
                if (!empty) {
                    if (!t->isNegligible() || !_T2P[t->getID()].isEmpty()) {
                        ...
                        firstNegligible = 0;
                        regions.add(lists);
                        lists = new (trHeapMemory())
                            TR_CISCNodeRegion(_numTNodes, comp()->trMemory()->heapMemoryRegion());
                    } else {
                        // It can be added
                        lists->append(t);
                    }
    ...
    for (r = ri.getFirst(); r; r = ri.getNext()) {
        if (r->isIncludeEssentialNode()) {
            if (showingCandidates)
                _candidatesForRegister.add(r->clone());
...
//*****************************************************************************
bool TR_CISCTransformer::verifyCandidate()
{
    ...
    for (b = bi.getFirst(); b; b = bi.getNext()) {
        while (true) {
            if (!le) {
                logprintf(trace(), comp()->log(), "Cannot find TR::BBStart of block_%d in the region\n",
                    b->getNumber());
...
//*****************************************************************************************
int TR_CISCTransformer::countP2T(TR_CISCNode *p, bool inLoop)
{
    uint32_t pID = p->getID();
...
// 0 for no-existence
//*****************************************************************************************
TR_CISCNode *TR_CISCTransformer::getP2TRep(TR_CISCNode *p)
{
    uint32_t pID = p->getID();
    List<TR_CISCNode> *list = _P2T + pID;
    if (list->isEmpty()) {
...
// 0 for no-existence
//*****************************************************************************************
TR_CISCNode *TR_CISCTransformer::getP2TRepInLoop(TR_CISCNode *p, TR_CISCNode *exclude)
{
    uint32_t pID = p->getID();
    List<TR_CISCNode> *list = _P2T + pID;
    if (list->isEmpty()) {
        ...
        TR_CISCNode *t;
        for (t = ni.getFirst(); t; t = ni.getNext()) {
            if (!t->isOutsideOfLoop() && t != exclude)
                return t;
        }
        return 0;
    }
...
// 0 for others
//*****************************************************************************************
TR_CISCNode *TR_CISCTransformer::getP2TInLoopIfSingle(TR_CISCNode *p)
{
    uint32_t pID = p->getID();
    List<TR_CISCNode> *list = _P2T + pID;
    if (list->isEmpty()) {
...
//*****************************************************************************************
TR_CISCNode *TR_CISCTransformer::getP2TInLoopAllowOptionalIfSingle(TR_CISCNode *p)
{
    TR_CISCNode *t;
    while (true) {
...
//*****************************************************************************************
bool TR_CISCTransformer::findFirstNode(TR::TreeTop **retTree, TR::Node **retNode, TR::Block **retBlock)
{
    OMR::Logger *log = comp()->log();
    ListIterator<TR_CISCNode> ci(_candidateRegion);
    TR_CISCNode *cn = NULL;
    ...
    for (cn = ci.getFirst(); cn; cn = ci.getNext()) {
        if (cn->getOpcode() == TR_entrynode)
            continue;
        if (cn->isNewCISCNode())
            continue;
        if (trace() && !cn->getTrNodeInfo()->isSingleton())
            log->printf("!cn->getTrNodeInfo()->isSingleton(): %d\n", cn->getID());
    ...
    *retNode = trNode;
    *retBlock = block;
    logprintf(trace(), log, "First node in candidate region - node: %p block_%d: %p\n", trNode, block->getNumber(),
        block);
    return true;
}
...
//*****************************************************************************************
void TR_CISCTransformer::addEdge(TR::CFGEdgeList *succList, TR::Block *srcBlock, TR::Block *destBlock)
{
...
//*****************************************************************************************
void TR_CISCTransformer::removeEdge(List<TR::CFGEdge> *succList, TR::Block *srcBlock, TR::Block *destBlock)
{
...
//*****************************************************************************************
void TR_CISCTransformer::removeEdgesExceptFor(TR::CFGEdgeList *succList, TR::Block *srcBlock,
    TR::Block *exceptDestBlock)
{
...
//*****************************************************************************************
void TR_CISCTransformer::setEdge(TR::CFGEdgeList *succList, TR::Block *srcBlock, TR::Block *destBlock)
{
...
//*****************************************************************************************
void TR_CISCTransformer::setEdges(TR::CFGEdgeList *succList, TR::Block *srcBlock, TR::Block *destBlock0,
    TR::Block *destBlock1)
{
    ...
    if (!existEdge1)
        addEdge(succList, srcBlock, destBlock1);
    if (!existEdge0)
        addEdge(succList, srcBlock, destBlock0);
...
//*****************************************************************************
TR::Block *TR_CISCTransformer::analyzeSuccessorBlock(TR::Node *ignoreTree)
{
    TR::Block *target = 0;
    if (_bblistSucc.isSingleton()) {
            ...
            for (b2 = bbi2.getFirst(); b2; b2 = bbi2.getNext()) {
                if (b1 != b2) {
                    ...
                    if (gotonode->getOpCodeValue() == TR::Goto
                        ...
                        target = b1;
                    } else if (gotonode->getOpCodeValue() == TR::BBEnd && b2->getNextBlock() == b1) {
                        target = b2;
                    } else {
                        target = 0;
                        break;
...
//*****************************************************************************************
void TR_CISCTransformer::setSuccessorEdge(TR::Block *block, TR::Block *target)
{
    if (target == 0)
    ...
    if (gotonode->getOpCodeValue() != TR::Goto) {
        TR::TreeTop *branchAroundTreeTop
            = TR::TreeTop::create(comp(), TR::Node::create(gotonode, TR::Goto, 0, target->getEntry()));
...
//*****************************************************************************************
TR::Block *TR_CISCTransformer::searchOtherBlockInSuccBlocks(TR::Block *target0, TR::Block *target1)
{
...
//*****************************************************************************************
TR::Block *TR_CISCTransformer::searchOtherBlockInSuccBlocks(TR::Block *target0)
{
...
//*****************************************************************************************
TR::Block *TR_CISCTransformer::setSuccessorEdges(TR::Block *block, TR::Block *target0, TR::Block *target1)
{
    ...
    logprintf(trace(), comp()->log(), "setSuccessorEdges for block_%d [%p]: tgt0=%d tgt1=%d\n", block->getNumber(),
        block, target0->getNumber(), target1->getNumber());

    if (!oldNext || oldNext->getNode()->getBlock() != target0) {
        ...
        TR::TreeTop *branchTreeTop
            = TR::TreeTop::create(comp(), TR::Node::create(lastnode, TR::Goto, 0, target0->getEntry()));
...
//*****************************************************************************************
TR::Block *TR_CISCTransformer::getSuccBlockIfSingle(TR::Block *block)
{
...
//*****************************************************************************************
TR::Block *TR_CISCTransformer::searchPredecessorOfBlock(TR::Block *block)
{
...
//*****************************************************************************************
TR::Block *TR_CISCTransformer::modifyBlockByVersioningCheck(TR::Block *block, TR::TreeTop *startTop,
    TR::Node *lengthNode, List<TR::Node> *guardList)
{
    ...
    {
        if (guardList == 0)
            guardList = &guardListLocal;
        ListAppender<TR::Node> appender(guardList);
        if (lengthNode->getOpCodeValue() == TR::i2l)
            lengthNode = lengthNode->getAndDecChild(0);
    ...
    List<TR::Node> *guardList)
{
    ...
    if (guardList && !guardList->isEmpty()) {
        ...
        logprintf(trace(), comp()->log(),
            "modifyBlockByVersioningCheck: orgPrevBlock=%d firstBlock=%d lastBlock=%d fastpath=%d slowpad=%d "
            "orgNextTreeTop=%x\n",
            orgPrevBlock->getNumber(), firstBlock->getNumber(), lastBlock->getNumber(), fastpath->getNumber(),
            slowpad->getNumber(), orgNextTreeTop);
...
//*****************************************************************************************
TR::Block *TR_CISCTransformer::cloneLoopBodyForPeel(TR::Block **firstBlock, TR::Block **lastBlock,
    TR_CISCNode *cmpifCISCNode)
{
    TR::CFG *cfg = comp()->getFlowGraph();
    cfg->setStructure(0);
...
//*****************************************************************************************
TR::Block *TR_CISCTransformer::appendBlocks(TR::Block *block, TR::Block *firstBlock, TR::Block *lastBlock)
{
    TR::CFG *cfg = comp()->getFlowGraph();
    cfg->setStructure(0);
    TR::Block *ret;
...
//*****************************************************************************************
bool TR_CISCTransformer::isDeadStore(TR::Node *node)
{
    if (node->getOpCode().isStoreDirect()) {
        ...
        // TR_ASSERT(useDefInfo->isDefIndex(useDefIndex), "error!");
        if (!useDefInfo->isDefIndex(useDefIndex))
            return false;
        if (useDefInfo->getUsesFromDefIsZero(useDefIndex))
            return true;
    }
    return false;
}
...
//*****************************************************************************************
TR::Block *TR_CISCTransformer::skipGoto(TR::Block *block, TR::Node *ignoreTree)
{
    while (true) {
        ...
        while (true) {
            ...
            if (!isDeadStore(gotonode) && (ignoreTree == 0 || !compareTrNodeTree(gotonode, ignoreTree)))
                break;
...
//*****************************************************************************************
bool TR_CISCTransformer::searchNodeInTrees(TR::Node *top, TR::Node *target, TR::Node **retParent, int *retChildNum)
{
    int i;
    ...
    for (i = top->getNumChildren(); --i >= 0;) {
        if (searchNodeInTrees(top->getChild(i), target, retParent, retChildNum))
            return true;
    }
    return false;
}
...
//*****************************************************************************************
bool TR_CISCTransformer::compareTrNodeTree(TR::Node *a, TR::Node *b)
{
    if (a == b)
        return true;
    if (a->getOpCodeValue() != b->getOpCodeValue())
        return false;
    ...
    if (a->getOpCode().isLoadConst()) {
        switch (a->getOpCodeValue()) {
            case TR::iconst:
                if (a->getUnsignedInt() != b->getUnsignedInt())
                    return false;
                break;
            case TR::lconst:
                if (a->getUnsignedLongInt() != b->getUnsignedLongInt())
                    return false;
                break;
            case TR::aconst:
            ...
            case TR::dconst:
                ...
                break;
            case TR::bconst:
                if (a->getUnsignedByte() != b->getUnsignedByte())
                    return false;
                break;
            case TR::sconst:
                if (a->getShortInt() != b->getShortInt())
                    return false;
                break;
            default:
                return false;
    ...
    if (numChild != b->getNumChildren())
        return false;
    if (numChild == 2 && a->getOpCode().isCommutative()) {
        if ((!compareTrNodeTree(a->getChild(0), b->getChild(0)) || !compareTrNodeTree(a->getChild(1), b->getChild(1)))
        ...
        for (i = 0; i < numChild; i++) {
            if (!compareTrNodeTree(a->getChild(i), b->getChild(i)))
                return false;
        }
    }
    return true;
...
//*****************************************************************************************
bool TR_CISCTransformer::compareBlockTrNodeTree(TR::Block *a, TR::Block *b)
{
    if (a == b)
        return true;
    ...
    while (true) {
        if (!compareTrNodeTree(ttA->getNode(), ttB->getNode()))
            return false;
        if (ttA == lastA)
            break;
        ttA = ttA->getNextRealTreeTop();
        if (ttA->getNode()->getOpCodeValue() == TR::Goto)
            break;
        ttB = ttB->getNextRealTreeTop();
        if (ttB->getNode()->getOpCodeValue() == TR::Goto)
            break;
    }
    return true;
}
...
//*****************************************************************************************
TR::Block *TR_CISCTransformer::insertBeforeNodes(TR::Block *block)
{
    OMR::Logger *log = comp()->log();
    ...
    logprintf(trace(), log, "insertBeforeNodes added %d node(s) to block_%d [%p]\n", count, block->getNumber(), block);
    if (last && last->getOpCode().isBranch()) {
        ...
        for (auto edge = block->getSuccessors().begin(); edge != block->getSuccessors().end(); ++edge) {
            ...
            if (to != branchDestinationBlock && to != orgNextBlock) {
                logprintf(trace(), log, "insertBeforeNodes added the edge (%d, %d).\n", newBlock->getNumber(),
                    to->getNumber());
        ...
        logprintf(trace(), log, "insertBeforeNodes created block_%d [%p]\n", newBlock->getNumber(), newBlock);
        block = newBlock;
    }
    return block;
}
...
//*****************************************************************************************
TR::Node *TR_CISCTransformer::findStoreToSymRefInInsertBeforeNodes(int32_t symRefNumberToBeMatched)
{
    ListIterator<TR::Node> ni(&_beforeInsertions);
    TR::Node *n = NULL;

    for (n = ni.getFirst(); n; n = ni.getNext()) {
        if (n->getOpCode().isStore() && n->getOpCode().hasSymbolReference()
            && n->getSymbolReference()->getReferenceNumber() == symRefNumberToBeMatched)
            return n;
    }
...
//*****************************************************************************************
TR::Block *TR_CISCTransformer::insertAfterNodes(TR::Block *block, List<TR::Node> *l, bool prepend)
{
    ...
    logprintf(trace(), comp()->log(), "insertAfterNodes adds %d node(s)\n", count);
    return block;
}
...
//*****************************************************************************************
TR::Block *TR_CISCTransformer::insertAfterNodes(TR::Block *block, bool prepend)
{
    return insertAfterNodes(block, &_afterInsertions, prepend);
}

// Add nodes for idiom specific transformations
TR::Block *TR_CISCTransformer::insertAfterNodesIdiom(TR::Block *block, int32_t pos, bool prepend)
{
    return insertAfterNodes(block, _afterInsertionsIdiom + pos, prepend);
}

// Remove all nodes from TreeTops from "start" to "end"
TR::TreeTop *TR_CISCTransformer::removeAllNodes(TR::TreeTop *start, TR::TreeTop *end)
{
...
//*****************************************************************************
bool TR_CISCTransformer::analyzeBoolTable(TR_BitVector **bv, TR::TreeTop **retSameExit, TR_CISCNode *boolTable,
    TR_BitVector *defBV, TR_CISCNode *defNode, TR_CISCNode *ignoreNode, int32_t bvoffset, int32_t allocBVSize)
{
    OMR::Logger *log = comp()->log();
    ...
    TR_CISCNode *n;
    int32_t i;
    TR_BitVector takenBV(allocBVSize, trMemory(), stackAlloc), ntakenBV(allocBVSize, trMemory(), stackAlloc),
        tmpBV(allocBVSize, trMemory(), stackAlloc);
            ...
            if (analyzeT2P(n, defNode) & _T2P_MatchMask) {
                ...
                if (n->getOpcode() == TR::Case) {
                    ...
                    while (!child1->isInterestingConstant()) {
                        ...
                        if (!child1) {
                            logprintf(trace(), log, "analyzeBoolTable failed for %p. (no single chain)\n",
                                n->getChild(1));
                            return false;
                        }
                        if (!child1->isStoreDirect()) {
                            logprintf(trace(), log, "analyzeBoolTable failed for %p. (%p is not store)\n",
                                n->getChild(1), child1);
                            return false;
                        }
                    ...
                    if (!n->isOutsideOfLoop()) {
                        ...
                        if (!initExitTreeTop) {
                            logprintf(trace(), log,
                                "analyzeBoolTable - Delimiter checking node %d targets treetop: %p block_%d: %p\n",
                                n->getID(), thisDestination, thisDestination->getEnclosingBlock()->getNumber(),
                                thisDestination->getEnclosingBlock());
                            initExitTreeTop = true;
                            exitTreeTop = thisDestination;
                        }
                        if (exitTreeTop != thisDestination) {
                            if (trace() && exitTreeTop)
                                log->printf("analyzeBoolTable - found conflicting successors.  Delimiter checking node "
                                            "%d targets treetop: %p (!= %p) block_%d: %p\n",
                                    n->getID(), thisDestination, exitTreeTop,
                                    thisDestination->getEnclosingBlock()->getNumber(),
                                    thisDestination->getEnclosingBlock());
...
int32_t TR_CISCTransformer::analyzeByteBoolTable(TR_CISCNode *boolTable, uint8_t *table256, TR_CISCNode *ignoreNode,
    TR::TreeTop **retSameExit)
{
    OMR::Logger *log = comp()->log();
    TR::StackMemoryRegion stackMemoryRegion(*trMemory());
    ...
    if (!boolTable || !getP2TRepInLoop(boolTable))
        return 0; // # of delimiter is zero
    ...
    switch (opcode) {
        case TR::b2i:
            if (defNode->isOptionalNode())
                defNode = defNode->getChild(0);
            // fall through
        case TR::bloadi:
    ...
    if (!analyzeBoolTable(bv, retSameExit, boolTable, &defBV, defNode, ignoreNode, BYTEBVOFFSET, ALLOCBYTEBVSIZE))
        return -1; // error
...
int32_t TR_CISCTransformer::analyzeCharBoolTable(TR_CISCNode *boolTable, uint8_t *table65536, TR_CISCNode *ignoreNode,
    TR::TreeTop **retSameExit)
{
    OMR::Logger *log = comp()->log();
    TR::StackMemoryRegion stackMemoryRegion(*trMemory());
    ...
    if (!analyzeBoolTable(bv, retSameExit, boolTable, &defBV, defNode, ignoreNode, CHARBVOFFSET, ALLOCCHARBVSIZE))
        return -1; // error
...
//*****************************************************************************
void TR_CISCTransformer::setColdLoopBody()
{
    ListIterator<TR::Block> bi(&_bblistBody);
    TR::Block *b;
    for (b = bi.getFirst(); b; b = bi.getNext()) {
...
//*****************************************************************************
bool TR_CISCTransformer::getBCIndexMinMax(List<TR_CISCNode> *l, int32_t *_minIndex, int32_t *_maxIndex, int32_t *_minLN,
    int32_t *_maxLN, bool allowInlined)
{
...
//*****************************************************************************
void TR_CISCTransformer::showCandidates()
{
    ...
    if (minIndex <= maxIndex) {
        ...
        fprintf(fp, " (%s) in %s", comp()->getHotnessName(comp()->getMethodHotness()), _T->getTitle());
#if SHOW_BCINDICES
...
//*****************************************************************************
void TR_CISCTransformer::registerCandidates()
{
    if (!isShowingCandidates())
        return;
    ...
    if (minIndex <= maxIndex)
        _candidatesForShowing.add(_P, minIndex, maxIndex, minLN, maxLN);
}
...
//*****************************************************************************
CISCT2PCond TR_CISCTransformer::analyzeT2P(TR_CISCNode *t, TR_CISCNode *p)
{
    // CISCT2PCond ret;
    int32_t tid = t->getID();
...
//*****************************************************************************
bool TR_CISCTransformer::analyzeOneArrayIndex(TR_CISCNode *arrayindex, TR::SymbolReference *inductionVariableSymRef)
{
    List<TR_CISCNode> *l = getP2T() + arrayindex->getID();
    if (l->isEmpty()) {
        if (arrayindex->isOptionalNode())
            return true;
        else
            return false;
    } else if (!l->isSingleton())
...
bool TR_CISCTransformer::analyzeArrayIndex(TR::SymbolReference *inductionVariableSymRef)
{
    ...
    for (int32_t i = 0;; i++) {
        ...
        if (!analyzeOneArrayIndex(arrayindex, inductionVariableSymRef))
            return false;
    }
    return true;
}
...
int32_t TR_CISCTransformer::countGoodArrayIndex(TR::SymbolReference *inductionVariableSymRef)
{
    int32_t ret = 0;
    // check each array index
    for (int32_t i = 0;; i++) {
        ...
        if (!arrayindex) {
            if (i == 0)
                ret = -1; // no TR_arrayindex in the pattern
            break; // end
        }
...
//*****************************************************************************
bool TR_CISCTransformer::simpleOptimization()
{
        ...
        if (!n->isNegligible()) {
            switch (n->getOpcode()) {
                        ...
                        if (ch->getOtherInfo() >= 256) {
                            ...
                            switch (ch->getOpcode()) {
                                case TR::iload:
                                    ...
                                    if (def) {
                                        if ((def->getNumChildren() > 0)
                                            && def->getChild(0)) // There is a bug where def is a TR_entrynode, so
                                                                 // getChild(0) is null.
                                        {
                ...
                default:
                    ...
                    {
                        if (quasiConst2.isEqualOpc(n) && n->getParents()->isSingleton()) {
                            TR_CISCNode *parent = n->getHeadOfParents();
                            if (parent->getOpcode() == TR::iadd) {
                                l = _T2P + parent->getID();
                                if (l->isSingleton() && l->getListHead()->getData()->getOpcode() == TR_arrayindex) {
                                    n->setIsNegligible();
                                }
                            }
                        }
...
//*****************************************************************************
uint64_t TR_CISCTransformer::getHashValue(TR_CISCNodeRegion *r)
{
    uint64_t ret = 0;
    int count = 0;
...
//*****************************************************************************
bool TR_CISCTransformer::canConvertArrayCmpSign(TR::Node *storeNode, List<TR::TreeTop> *compareIfs,
    bool *canConvertToArrayCmp)
{
    OMR::Logger *log = comp()->log();
    ...
    if (!info.IsZero()) {
        ...
        for (cursor.SetToFirstOne(); cursor.Valid(); cursor.SetToNextOne()) {
            ...
            if (useNode->getReferenceCount() > 1) {
                logprintf(trace(), log, "canConvertArrayCmpSign failed because ReferenceCount > 1. %p\n", useNode);
                return false;
            }
            ...
            if (useCISCNode) {
                ...
                if (NULL == foundTT || !searchNodeInTrees(foundTT->getNode(), useNode, &parentNode, &retChildNum)) {
                    logprintf(trace(), log,
                        "canConvertArrayCmpSign failed because searchNodeInTrees failed. UseNode: %p with "
                        "corresponding TreeTop: %p\n",
                        useNode, foundTT);
                    return false;
                }
            }
            if (!parentNode) {
                logprintf(trace(), log, "canConvertArrayCmpSign failed because parentNode is NULL. %p\n", useNode);
                return false;
            }
            ...
            if (parentNode->getOpCode().isStoreDirect()) {
                if (!canConvertArrayCmpSign(parentNode, compareIfs, &convertToArrayCmp)) {
                    logprintf(trace(), log,
                        "canConvertArrayCmpSign failed because canConvertArrayCmpSign(p) failed. %p\n", useNode);
                    return false;
                }
                ...
                if (theOtherChild->getInt() != 0 || theOtherChild->getOpCodeValue() != TR::iconst) {
                    logprintf(trace(), log, "canConvertArrayCmpSign failed because theOtherChild is not iconst 0. %p\n",
                        useNode);
                    return false;
                }
                if (compareIfs)
                    compareIfs->add(foundTT);
                switch (parentNode->getOpCodeValue()) {
                    ...
                    case TR::ificmpne:
                        break; // OK!
                    default:
                        logprintf(trace(), log, "convertArrayCmp failed because parentNode is %s. %x\n",
                ...
                logprintf(trace(), log, "canConvertArrayCmpSign failed because unhandled opcode %s. %x %x\n",
                    parentNode->getOpCode().getName(), useNode, parentNode);
                return false;
            }
        }
        if (canConvertToArrayCmp)
            *canConvertToArrayCmp = convertToArrayCmp;
    }
    if (disableCMP)
        *canConvertToArrayCmp = false;
#if VERBOSE
    if (*canConvertToArrayCmp)
...
bool TR_CISCTransformer::computeTopologicalEmbedding(TR_CISCGraph *P, TR_CISCGraph *T)
{
    OMR::Logger *log = comp()->log();
    ...
    if (disabledPatterns && TR::SimpleRegex::match(disabledPatterns, P->getTitle())) {
        logprintf(trace(), log, "%s is disabled by disabledIdiomPatterns={}\n", P->getTitle());
        return false;
    }

    // FIXME: improve this
    if (!T->testAllAspects(P)) {
        logprintf(trace(), log, "%s is skipped since graph properties do not match (%08x)\n", P->getTitle(),
            P->getAspectsValue());
        return false; // No need to analyze
    }
    if (T->testAnyNoAspects(P)) {
        logprintf(trace(), log, "%s is skipped due to existence of testAnyNoAspects (%08x)\n", P->getTitle(),
            P->getNoAspectsValue());
        return false; // No need to analyze
    }
    if (!T->meetMinCounts(P)) {
        logprintf(trace(), log, "%s is skipped due to failure of meetMinCounts (%d %d %d)\n", P->getTitle(),
            P->getAspects()->getIfCount(), P->getAspects()->getIndirectLoadCount(),
            P->getAspects()->getIndirectStoreCount());
        return false; // No need to analyze
    }

    // avoid analyzing very large graphs
    if (T->getNumNodes() >= IDIOM_SIZE_FACTOR * P->getNumNodes()) {
        logprintf(trace(), log, "%s is skipped due to loop being very large\n", P->getTitle());
        return false; // No need to analyze
    }
    ...
    if (1) //!_compilation->getOption(TR_EnableIdiomRecognitionWarm))
    {
        if (T->getHotness() < P->getHotness()) {
            logprintf(trace(), log, "%s is skipped due to hotness\n", P->getTitle());
            return false; // No need to analyze
        }
    }
    ...
    if (!incorrectBBFreqLevel) {
        if (P->isHighFrequency() && !T->isHighFrequency()) {
            logprintf(trace(), log, "%s is skipped due to the rarely iterated loop (!isHighFrequency)\n",
                P->getTitle());
            return false; // No need to analyze
        }
    }
    ...
    {
        if (isAfterVersioning() ? P->isInhibitAfterVersioning() : P->isInhibitBeforeVersioning()) {
            logprintf(trace(), log, "%s is skipped due to loop versioning check\n", P->getTitle());
            return false; // No need to analyze
        }
    }
#endif
    ...
    //
    logprintf(trace(), log, "Computing embedding info for idiom %s in loop %d\n", P->getTitle(),
        _bblistBody.getListHead()->getData()->getNumber());
    if (showMesssagesStdout())
        printf("Idiom: loop %d, %s\n", _bblistBody.getListHead()->getData()->getNumber(), P->getTitle());
    ...
    if (!computeEmbeddedForData())
        return false; // It cannot find all of the idiom nodes.
    if (showMesssagesStdout())
        printf("find1 %s\n", P->getTitle());
    logprintf(trace(), log, "Detected IL nodes in loop for idiom %s\n", P->getTitle());
    ...
    if (!computeEmbeddedForCFG())
        return false; // It cannot find all of the idiom nodes.
    if (showMesssagesStdout())
        printf("find2 %s\n", P->getTitle());
    logprintf(trace(), log, "finished topological embedding for idiom %s\n", P->getTitle());
    ...
    // If the analysis fails, it'll invalidate that node.
    //
    if (P->isRequireAHconst())
        analyzeArrayHeaderConst();
    ...
    if (!_candidateRegion || !verifyCandidate()
        || // all blocks of the loop body are not included in the _candidateRegion
        embeddingHasConflictingBranches()) {
        logprints(trace(), log, "computeTopologicalEmbedding: Graph transformations failed. (step 3)\n\n");
        registerCandidates();
    ...
    if (trace() || showMesssagesStdout()) {
        ...
        if (minIndex <= maxIndex) {
            snprintf(tmpbuf, sizeof(tmpbuf),
                ", bcindex %" OMR_PRIu32 " - %" OMR_PRIu32 " linenumber %" OMR_PRIu32 " - %" OMR_PRIu32 "%s.", minIndex,
                maxIndex, minLN, maxLN, inlined ? " (inlined)" : "");
            bcinfo = tmpbuf;
        }
#endif
#if SHOW_STATISTICS
        ...
        logprintf(trace(), log, "***** Transformed *****, %s, %s, %s, loop:%d%s\n",
            comp()->getHotnessName(comp()->getMethodHotness()), P->getTitle(), T->getTitle(),
            _bblistBody.getListHead()->getData()->getNumber(), bcinfo);
        if (showMesssagesStdout())
            printf("== Transformed == %s, %s, %s, loop:%d%s\n", comp()->getHotnessName(comp()->getMethodHotness()),
                P->getTitle(), T->getTitle(), _bblistBody.getListHead()->getData()->getNumber(), bcinfo);
    }

    TR::DebugCounter::incStaticDebugCounter(comp(),
        TR::DebugCounter::debugCounterName(comp(), "idiomRecognition.matched/%s/(%s)/%s/loop=%d", P->getTitle(),
...
static void traceConflictingBranches(TR_CISCTransformer *opt, TR_CISCNode *pn, List<TR_CISCNode> *matches)
{
    if (!opt->trace())
        return;
    ...
    OMR::Logger *log = comp->log();
    log->printf("Pattern node %d (%s) has conflicting branches:", pn->getID(),
    ...
    for (TR_CISCNode *tn = ti.getFirst(); tn != NULL; tn = ti.getNext()) {
        log->printf("%s %d (%s)", first ? "" : ",", tn->getID(),
 ...
 */
bool TR_CISCTransformer::embeddingHasConflictingBranches()
{
    static const char * const disableEnv = feGetEnv("TR_disableIdiomRecognitionConflictingBranchTest");
    ...
    for (int32_t dag = 0; dag < dagCount; dag++) {
        ...
        for (TR_CISCNode *pn = pi.getFirst(); pn != NULL; pn = pi.getNext()) {
            ...
            bool isIf = op == (uint32_t)TR_ifcmpall || (op < (uint32_t)TR::NumAllIlOps && pn->getIlOpCode().isIf());
                    ...
                    if (inLoopMatch != NULL) {
                        ...
                        TR::DebugCounter::incStaticDebugCounter(comp(),
                            TR::DebugCounter::debugCounterName(comp(),
                                "idiomRecognition.rejected/branchConflict/%s/(%s)/%s/loop=%d", _P->getTitle(),
                                comp()->signature(), comp()->getHotnessName(comp()->getMethodHotness()),
                                _bblistBody.getListHead()->getData()->getNumber()));
                        return true;
                    }
...
// Reference.reachabilityFence is a newly introduced Java 9 public API, we will remove this call only if the caller
// comes from the java.nio package, i.e. only for Reference.reachabilityFence calls replacing existing Bits.keepAlive
// calls in the java.nio package. For all other non-nio callers, we will conservatively not remove the call to not break
// existing Idiom Recognition code.

bool TR_CISCTransformer::removeBitsKeepAliveCalls(List<TR::Block> *body)
{
    OMR::Logger *log = comp()->log();
    logprints(trace(), log, "\tScanning for java/nio/Bits.keepAlive(Ljava/lang/Object;)V calls.\n");
        ...
        for (TR::TreeTop *tt = block->getEntry(); tt != block->getExit(); tt = tt->getNextTreeTop()) {
            ...
            if (node->getOpCodeValue() == TR::treetop) {
                ...
                if (node->getOpCode().isCall()) {
                    ...
                    // If the call is Bits.keepAlive() which is package private hence can only be called from the nio
                    // package, or the call is Reference.reachabilityFence() which is public _and_ provided the caller
                    // is from the nio package, only then we will remove the call node.
                    if (methodSym->getRecognizedMethod() == TR::java_nio_Bits_keepAlive
                        || ((methodSym->getRecognizedMethod() == TR::java_lang_ref_Reference_reachabilityFence)
                            && (!strncmp(
                                comp()->fe()->sampleSignature(node->getOwningMethod(), 0, 0, comp()->trMemory()),
                                "java/nio/", 9)))) {
                        logprintf(trace(), log, "\t\tRemoving KeepAlive call found in block %d [%p] @ Node: %p\n",
                            block->getNumber(), block, node);
                        foundCall = true;

                        TR_BitsKeepAliveInfo *info = new (comp()->trStackMemory())
                            TR_CISCTransformer::TR_BitsKeepAliveInfo(block, tt, tt->getPrevTreeTop());
...
void TR_CISCTransformer::insertBitsKeepAliveCalls(TR::Block *block)
{
    OMR::Logger *log = comp()->log();
    logprints(trace(), log, "\tInserting java/nio/Bits.keepAlive(Ljava/lang/Object;)V calls into reduced loop.\n");
...
void TR_CISCTransformer::restoreBitsKeepAliveCalls()
{
    OMR::Logger *log = comp()->log();
    logprints(trace(), log, "\tRestoring for java/nio/Bits.keepAlive(Ljava/lang/Object;)V calls.\n");
    ListIterator<TR_BitsKeepAliveInfo> bi(&_BitsKeepAliveList);
    ...
    for (TR_BitsKeepAliveInfo *info = bi.getFirst(); info != NULL; info = bi.getNext()) {
        ...
        logprintf(trace(), log, "\t\tInserting KeepAlive call found in block %d [%p] @ Node: %p\n", block->getNumber(),
            block, tt->getNode());
        prev->insertAfter(tt);
    }
}

void TR_CISCTransformer::countFail(const char *fmt, ...)
{
    _countFailBuf.clear();
    ...
    logprintf(trace(), comp()->log(), "failed: %s\n", _countFailBuf.text());

    TR::DebugCounter::incStaticDebugCounter(comp(),
        TR::DebugCounter::debugCounterName(comp(), "idiomRecognition.failed/%s/%s/(%s)/%s/loop=%d",
            _countFailBuf.text(), _P->getTitle(), comp()->signature(),
            comp()->getHotnessName(comp()->getMethodHotness()), _bblistBody.getListHead()->getData()->getNumber()));
}

void TR_CISCTransformer::countUnhandledOpcode(const char *where, uint32_t opcode)
{
