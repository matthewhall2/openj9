...
bool TR_EscapeAnalysis::isImmutableObject(TR::Node *node)
{
    ...
    if (NULL != className && !strncmp("java/lang/", className, 10)
        && (!strcmp("Integer", &className[10]) || !strcmp("Long", &className[10]) || !strcmp("Short", &className[10])
            || !strcmp("Byte", &className[10]) || !strcmp("Boolean", &className[10])
            || !strcmp("Character", &className[10]) || !strcmp("Double", &className[10])
...
int32_t TR_EscapeAnalysis::perform()
{
    ...
    TR_ASSERT_FATAL(_maxSniffDepth < 16,
        "The argToCall and nonThisArgToCall flags are 16 bits - a depth limit greater than 16 will not fit in these "
        "flags");

    if (getLastRun())
        _maxPassNumber = 0; // Notwithstanding our heuristics, if this is the last run, our max "pass number" is zero
...

    {
        ...
        _callsToProtect
            = new (trStackMemory()) CallLoadMap(CallLoadMapComparator(), comp()->trMemory()->currentStackRegion());
        ...
        if (inspector.checkMonitorStructure(comp()->getFlowGraph())) {
            ...
            logprints(trace(), log, "Disallowing monitor-removal because of strange monitor structure\n");
        }
#endif
        ...
        if (!_callsToProtect->empty() && manager()->numPassesCompleted() < _maxPassNumber) {
            ...
            RemainingUseCountMap *remainingUseCount = new (comp()->trStackMemory())
                RemainingUseCountMap(RemainingUseCountMapComparator(), comp()->trMemory()->currentStackRegion());
            for (TR::TreeTop *tt = comp()->findLastTree(); tt != NULL; tt = tt->getPrevTreeTop()) {
                ...
                else if (node->getOpCode().isCheck() || node->getOpCode().isAnchor()
                    || node->getOpCodeValue() == TR::treetop)
                    node = node->getFirstChild();

                auto nodeLookup = _callsToProtect->find(node);
                if (nodeLookup == _callsToProtect->end() || TR_EscapeAnalysisTools::isFakeEscape(node)
                    || node->getSymbol() == NULL || node->getSymbol()->getResolvedMethodSymbol() == NULL
                ...
                if (!performTransformation(comp(),
                        "%sHCR CALL PEEKING: Protecting call [%p] n%dn with an HCR guard and escape helper\n",
                        OPT_DETAILS, node, node->getGlobalIndex()))
                    continue;

                ((TR_EscapeAnalysis::PersistentData *)manager()->getOptData())
                    ->_processedCalls->set(node->getGlobalIndex());
                ...
                for (TR::TreeTop *itr = block->getExit(); itr != prevTT; itr = itr->getPrevTreeTop()) {
                    ...
                    if (storeNode && storeNode->getOpCodeValue() == TR::astore
                ...
                TR::Node *guard = TR_VirtualGuard::createHCRGuard(comp(), node->getByteCodeInfo().getCallerIndex(),
                    node, NULL, node->getSymbol()->getResolvedMethodSymbol(),
                ...
                heapificationBlock->getExit()->insertBefore(
                    TR::TreeTop::create(comp(), TR::Node::create(node, TR::Goto, 0, callBlock->getEntry())));
                tools.insertFakeEscapeForLoads(heapificationBlock, node, *nodeLookup->second.second);

                logprintf(trace(), log, "Created heapification block_%d\n", heapificationBlock->getNumber());

                ((TR_EscapeAnalysis::PersistentData *)manager()->getOptData())
                    ->_peekableCalls->set(node->getGlobalIndex());
...
const char *TR_EscapeAnalysis::optDetailString() const throw() { return "O^O ESCAPE ANALYSIS: "; }
...
void TR_EscapeAnalysis::rememoize(Candidate *candidate, bool mayDememoizeNextTime)
{
    ...
    TR_ASSERT(candidate->_treeTop->getEnclosingBlock() == candidate->_dememoizedConstructorCall->getEnclosingBlock(),
        "Dememoized constructor call %p must be in the same block as allocation %p", candidate->_treeTop->getNode(),
        candidate->_dememoizedConstructorCall->getNode());
    logprintf(trace(), comp()->log(), "   Rememoizing%s [%p] using constructor call [%p]\n",
        mayDememoizeNextTime ? "" : " and inlining", candidate->_node,
        candidate->_dememoizedConstructorCall->getNode()->getFirstChild());

    // Change trees back
    //
    candidate->_node->getFirstChild()->recursivelyDecReferenceCount(); // remove loadaddr of class
    candidate->_node->setAndIncChild(0,
        candidate->_dememoizedConstructorCall->getNode()->getFirstChild()->getSecondChild()); // original call argument
...
static TR_YesNoMaybe candidateHasField(Candidate *candidate, TR::Node *fieldNode, int32_t fieldOffset,
    TR_EscapeAnalysis *ea)
{
    ...
    if (candidate->_origKind == TR::New || candidate->_origKind == TR::newvalue) {
        ...
        minHeaderSize = std::min(TR::Compiler->om.contiguousArrayHeaderSizeInBytes(),
            TR::Compiler->om.discontiguousArrayHeaderSizeInBytes());
        maxHeaderSize = std::max(TR::Compiler->om.contiguousArrayHeaderSizeInBytes(),
            TR::Compiler->om.discontiguousArrayHeaderSizeInBytes());
    }
    ...
    if (withinObjectHeader == TR_yes) {
        ...
        TR_OpaqueClassBlock *fieldClassInCP
            = fieldSymRef->getOwningMethod(comp)->getClassFromFieldOrStatic(comp, fieldSymRef->getCPIndex());
        if (fieldClassInCP
            ...
            if (!debugEAFieldValidityCheck
                || performTransformation(comp,
                    "%sQuick Using candidateHasField=yes (withinObjectBound=%s) for candidate [%p] field access [%p]\n",
    ...
    if (result == TR_maybe) {
        ...
        if (candidateHasFieldCriticalSection.hasVMAccess()) {
            TR_OpaqueClassBlock *fieldDeclaringClass
                = fieldSymRef->getOwningMethod(comp)->getDeclaringClassFromFieldOrStatic(comp,
                    fieldSymRef->getCPIndex());
            if (fieldDeclaringClass)
                belongsToAllocatedClass
                    = comp->fej9()->isInstanceOf((TR_OpaqueClassBlock *)candidate->_class, fieldDeclaringClass, true);

            result = ynmOr(withinObjectHeader, belongsToAllocatedClass);

            if (debugEAFieldValidityCheck) {
                if (!performTransformation(comp,
                        "%sUsing candidateHasField=%s (withinObjectBound=%s) for candidate [%p] field access [%p]\n",
            ...
            logprintf(ea->trace(), comp->log(),
                "   Unable to acquire vm access; conservatively assume field [%p] does not belong to candidate [%p]\n",
    ...
    if (debugEAFieldValidityCheck) {
        if ((withinObjectBound != result)
            && !performTransformation(comp,
                "%sSubstituting candidateHasField=%s (withinObjectBound=%s) for candidate [%p] field access [%p]\n",
                OPT_DETAILS, ynmString(result), ynmString(withinObjectBound), candidate->_node, fieldNode)) {
    ...
    logprintf(ea->trace(), comp->log(),
        "   Candidate [%p] field access [%p] candidateHasField=%s (withinObjectBound=%s withinObjectHeader=%s "
        "belongsToAllocatedClass=%s)\n",
        candidate->_node, fieldNode, ynmString(result), ynmString(withinObjectBound), ynmString(withinObjectHeader),
...
int32_t TR_EscapeAnalysis::performAnalysisOnce()
{
    ...
    if (!_candidates.isEmpty()) {
        ...
        _blocksWithFlushOnEntry
            = new (trStackMemory()) TR_BitVector(comp()->getFlowGraph()->getNextNodeNumber(), trMemory(), stackAlloc);
        ...
        if (!_useDefInfo) {
            logprints(trace(), log, "Can't do Escape Analysis, no use/def information\n");
            _candidates.setFirst(NULL);
        }

        _valueNumberInfo = optimizer()->getValueNumberInfo();
        if (!_valueNumberInfo) {
            logprints(trace(), log, "Can't do Escape Analysis, no value number information\n");
            _candidates.setFirst(NULL);
        } else {
            _ignorableUses = new (trStackMemory()) TR_BitVector(0, trMemory(), stackAlloc);
            _nonColdLocalObjectsValueNumbers
                = new (trStackMemory()) TR_BitVector(_valueNumberInfo->getNumberOfValues(), trMemory(), stackAlloc);
            _allLocalObjectsValueNumbers
                = new (trStackMemory()) TR_BitVector(_valueNumberInfo->getNumberOfValues(), trMemory(), stackAlloc);
            _notOptimizableLocalObjectsValueNumbers
                = new (trStackMemory()) TR_BitVector(_valueNumberInfo->getNumberOfValues(), trMemory(), stackAlloc);
            _notOptimizableLocalStringObjectsValueNumbers
                = new (trStackMemory()) TR_BitVector(_valueNumberInfo->getNumberOfValues(), trMemory(), stackAlloc);
        }
    }
    ...
    if (!_candidates.isEmpty()) {
        ...
        if (cg()->getEnforceStoreOrder()) {
            TR_FlowSensitiveEscapeAnalysis flowSensitiveAnalysis(comp(), optimizer(),
                comp()->getFlowGraph()->getStructure(), this);
        }
    ...
    for (candidate = _candidates.getFirst(); candidate; candidate = next) {
        ...
        for (callSite = it.getFirst(); callSite; callSite = callSiteNext) {
            ...
            if (!found) {
                ...
                _fixedVirtualCallSites.add(
                    new (trStackMemory()) TR_EscapeAnalysis::TR_CallSitesFixedMapper(callSite, NULL));
        ...
        logprintf(trace(), log, "   Make [%p] non-local because we'll try it again in another pass\n",
            candidate->_node);
    }
    ...
    for (candidate = _candidates.getFirst(); candidate; candidate = next) {
        ...
        for (callSite = it.getFirst(); callSite; callSite = callSiteNext) {
            ...
            bool found = findCallSiteFixed(callSite);
            if (found) {
                logprintf(trace(), log, "replacing callsite %p for candidate %p with it's direct call\n", callSite,
                    candidate->_node);
                candidate->getCallSites()->remove(callSite);
                candidate->setLocalAllocation(false);
                // candidate->getCallSites()->add(directCallTree);
        ...
        for (TR_ColdBlockEscapeInfo *info = coldBlockInfoIt.getFirst(); info != NULL;
             info = coldBlockInfoIt.getNext()) {
    ...
    for (candidate = _candidates.getFirst(); candidate; candidate = next) {
        ...
        if (candidate->_dememoizedConstructorCall) {
            if (candidate->isLocalAllocation() && !candidate->mustBeContiguousAllocation()
                ...
                logprintf(trace(), log, "2 setting local alloc %p to false\n", candidate->_node);
                ...
                if (trace()) {
                    ...
                    log->printf("   Fail [%p] because dememoization failed; will%s attempt dememoization again on next "
                                "EA pass\n",
                        candidate->_node, mayDememoizeNextTime ? "" : " NOT");
                    log->printf(" 4 booleans are %d %d %d %d\n", candidate->isLocalAllocation(),
                        candidate->mustBeContiguousAllocation(), candidate->getCallSites()->isSingleton(),
                        candidate->getCallSites()->find(candidate->_dememoizedConstructorCall));
                }
                ...
                logprintf(trace(), log, "8 removing cand %p to false\n", candidate->_node);
            }
    ...
    if (manager()->numPassesCompleted() < _maxPassNumber) {
        for (candidate = _candidates.getFirst(); candidate; candidate = next) {
            ...
            logprintf(trace(), log, "   0 Look at [%p] must be %d\n", candidate->_node,
            ...
            if (candidate->mustBeContiguousAllocation() || !candidate->hasCallSites()
                || (candidate->_stringCopyNode && (candidate->_stringCopyNode != candidate->_node)
                    && !candidate->escapesInColdBlocks() && !candidate->isLockedObject() && !candidate->_seenSelfStore
                    && !candidate->_seenStoreToLocalObject))
                continue;

            logprintf(trace(), log, "   0.5 Look at [%p]\n", candidate->_node);
            ...
            logprintf(trace(), log, "   1 Look at [%p]\n", candidate->_node);
            ...
            //
            if (seenGuardedCall || candidate->_maxInlineDepth >= (_maxPassNumber - manager()->numPassesCompleted())
                || candidate->_inlineBytecodeSize
                    > (_maxInlinedBytecodeSize - getOptData()->_totalInlinedBytecodeSize)) {
                candidate->setMustBeContiguousAllocation();
                logprintf(trace(), log, "   Make [%p] contiguous because we can't inline enough\n", candidate->_node);
                continue;
            }
                ...
                if (!_inlineCallSites.find(callSite)) {
                    if (comp()->getMethodHotness() <= warm) {
                        ...
                        switch (node->getSymbol()->castToMethodSymbol()->getRecognizedMethod()) {
                            ...
                            default:
                                candidate->setMustBeContiguousAllocation();
                                logprintf(trace(), log, "   Make [%p] contiguous because we can't inline it at %s\n",
    ...
    for (candidate = _candidates.getFirst(); candidate; candidate = next) {
        ...
        if (candidate->_kind == TR::New || candidate->_kind == TR::newvalue) {
            ...
            if (!doEAOpt && comp()->useCompressedPointers()) {
                if (candidate->_seenSelfStore || candidate->_seenStoreToLocalObject) {
                    candidate->setLocalAllocation(false);
                    logprintf(trace(), log,
                        "   Make [%p] non-local because self store seen in compressed pointers mode\n",
                        candidate->_node);
                }
            }

            if (CANT_CREATE_BCD_TEMPS && candidate->isLocalAllocation() && !candidate->isContiguousAllocation()
                && candidate->_fields) {
                for (int32_t i = candidate->_fields->size() - 1; i >= 0; i--) {
                    ...
                    if (field && field->getSymbol()->getDataType().isBCD()) {
                        candidate->setMustBeContiguousAllocation();
                        logprintf(trace(), log,
                            "   Make [%p] contiguous because we can't create temp for BCD field #%d\n",
            ...
            if (candidate->isLocalAllocation() && candidate->isContiguousAllocation()) {
                if (!_createLocalObjects) {
                    candidate->setLocalAllocation(false);
                    logprintf(trace(), log, "   Make [%p] non-local because we can't create local objects\n",
                        candidate->_node);
                }
            }

            if (candidate->isLocalAllocation() && candidate->escapesInColdBlocks()
                && (candidate->isLockedObject() || candidate->_seenSelfStore || candidate->_seenStoreToLocalObject)) {
                candidate->setLocalAllocation(false);
                logprintf(trace(), log,
            ...
            switch (candidate->_kind) {
                case TR::New:
                case TR::newvalue:
                    if (comp()->fej9()->getReferenceSlotsInClass(comp(),
                            (TR_OpaqueClassBlock *)candidate->_node->getFirstChild()
                                ->getSymbol()
                                ->getStaticSymbol()
                                ->getStaticAddress())) {
            ...
            if (candidate->isLocalAllocation()
                && (candidate->isInAColdBlock()
                    || (objectHasReferenceFields && candidate->isContiguousAllocation()
            ...
            {
                candidate->setLocalAllocation(false);
                logprintf(trace(), log, "   Make [%p] non-local because the uses are not in hot enough blocks\n",
                    candidate->_node);
            }
            ...
            if (candidate->_valueNumbers->size() > 1 && candidate->_fields) {
                for (int32_t i = candidate->_fields->size() - 1; i >= 0; i--) {
                    if (candidate->_fields->element(i).hasBadFieldSymRef()) {
                        logprintf(trace(), log,
                            "   Fail [%p] because candidate is dereferenced via a field that does not belong to "
                            "allocated class\n",
                            candidate->_node);
                        rememoize(candidate);
                        _candidates.remove(candidate);
            ...
            if (candidate->_stringCopyNode && (candidate->_stringCopyNode != candidate->_node)) {
                if (!candidate->escapesInColdBlocks() && !candidate->isLockedObject() && !candidate->_seenSelfStore
                    && !candidate->_seenStoreToLocalObject)
                    candidate->setMustBeContiguousAllocation();
                else
            ...
            if (candidate->_dememoizedMethodSymRef && candidate->isContiguousAllocation()) {
                logprintf(trace(), log, "   Fail [%p] because dememoized allocations must be non-contiguous\n",
                    candidate->_node);
                rememoize(candidate);
                _candidates.remove(candidate);
            }
        ...
        if (candidate->_kind == TR::anewarray) {
            ...
            if (candidate->isContiguousAllocation()) {
                ...
                if (classNode->getOpCodeValue() != TR::loadaddr || classNode->getSymbolReference()->isUnresolved()) {
                    logprintf(trace(), log, "   Fail [%p] because base class is unresolved\n", candidate->_node);
                    rememoize(candidate);
                    _candidates.remove(candidate);
                }
            }
        ...
        if (candidate->isProfileOnly() && candidate->isLocalAllocation()) {
            ...
            logprintf(trace(), log, "3 setting local alloc %p to false\n", candidate->_node);

            if (performTransformation(comp(), "%sInitiate value profiling for length of %s [%p]\n", OPT_DETAILS,
                    candidate->_node->getOpCode().getName(), candidate->_node)) {
                if (optimizer()->switchToProfiling()) {
                    ...
                    TR_ByteCodeInfo originalBcInfo
                        = TR_ProfiledNodeVersioning::temporarilySetProfilingBcInfoOnNewArrayLengthChild(
                            candidate->_node, comp());
                    valueProfiler->addProfilingTrees(numElementsNode, candidate->_treeTop, 5);
                    numElementsNode->setByteCodeInfo(originalBcInfo);
                } else {
                    logprintf(trace(), log, "  Unable to switch to profiling mode; no profiling trees added for [%p]\n",
                        candidate->_node);
                }
            }
        }
        ...
        //
        if (!candidate->isLocalAllocation()) {
            logprintf(trace(), log, "   Fail [%p] array candidate is not locally allocatable\n", candidate->_node);
            rememoize(candidate);
            _candidates.remove(candidate);
            continue;
        }
    ...
    if (suppressAtRegex != NULL && !_candidates.isEmpty()) {
        for (candidate = _candidates.getFirst(); candidate; candidate = next) {
            ...
            if (TR::SimpleRegex::match(suppressAtRegex, bcInfo)) {
                candidate->setLocalAllocation(false);
                logprintf(trace(), log,
                    "  Suppressing stack allocation of candidate node [%p] - matched suppressEA option\n",
                    candidate->_node);
            }
        }
    }
    ...
    if (comp()->getStartBlock() && !_candidates.isEmpty()) {
        ...
        if (blockIsInLoop(block)) {
            ...
            TR::Block *firstBlockReplacement = toBlock(comp()->getFlowGraph()->addNode(TR::Block::createEmptyBlock(
                block->getEntry()->getNode(), comp(), std::max(MAX_COLD_BLOCK_COUNT + 1, block->getFrequency() / 10))));
            ...
            comp()->getFlowGraph()->addEdge(TR::CFGEdge::createEdge(comp()->getFlowGraph()->getStart()->asBlock(),
                firstBlockReplacement, trMemory()));
            ...
            if (performTransformation(comp(), "%sStack allocating candidate [%p]\n", OPT_DETAILS, candidate->_node)) {
                ...
                if (candidate->escapesInColdBlocks()) {
                    ...
                    if (_initializedHeapifiedTemps == NULL) {
                        _initializedHeapifiedTemps
                            = new (trStackMemory()) TR_BitVector(0, trMemory(), stackAlloc, growable);
                        ...
                        for (TR::TreeTop *tt = comp()->getStartTree();
                             tt != NULL && tt->getNode()->getOpCodeValue() != TR::BBEnd; tt = tt->getNextTreeTop()) {
                            TR::Node *node = tt->getNode();
    ...
    //
    if ((nonContiguousAllocations > 0) || (tempsCreatedForColdEscapePoints > 0)) {
        bool containsCatchBlocks = false;
        for (TR::CFGNode *node = comp()->getFlowGraph()->getFirstNode(); !containsCatchBlocks && node;
             node = node->getNext())
...
void TR_EscapeAnalysis::findIgnorableUses()
{
    ...
    for (TR::TreeTop *treeTop = comp()->getStartTree(); treeTop; treeTop = treeTop->getNextTreeTop()) {
        ...
        else if (inOSRCodeBlock && treeTop->getNode()->getNumChildren() > 0
...
void TR_EscapeAnalysis::markUsesAsIgnorable(TR::Node *node, TR::NodeChecklist &visited)
{
    if (visited.contains(node))
        return;
    visited.add(node);
    logprintf(trace(), comp()->log(), "Marking n%dn as an ignorable use\n", node->getGlobalIndex());
...
void TR_EscapeAnalysis::findLocalObjectsValueNumbers(TR::Node *node, TR::NodeChecklist &visited)
{
    ...
    if (node->getOpCode().hasSymbolReference() && node->getSymbolReference()->getSymbol()->isLocalObject()) {
        ...
        if (!node->escapesInColdBlock()) {
            ...
            if (node->cannotTrackLocalUses()) {
                if (!_notOptimizableLocalObjectsValueNumbers->get(_valueNumberInfo->getValueNumber(node))) {
                    ...
                    // _valueNumberInfo->getValueNumber(node));
...
void TR_EscapeAnalysis::findCandidates()
{
    ...
    for (_curTree = comp()->getStartTree(); _curTree; _curTree = _curTree->getNextTreeTop()) {
        ...
        if (node->getOpCode().isNew() && node->isHeapificationAlloc()) {
            logprintf(trace(), log, "Reject candidate %s n%dn [%p] because it is for heapification\n",
        ...
        if (!comp()->fej9()->callTargetsNeedRelocations() && node->getOpCodeValue() == TR::acall
            ...
            if (constructor && performTransformation(comp(), "%sTry dememoizing %p\n", OPT_DETAILS, node)) {
                ...
                TR::SymbolReference *constructorSymRef
                    = comp()->getSymRefTab()->findOrCreateMethodSymbol(JITTED_METHOD_INDEX, -1,
                ...
                dememoizedConstructorCall = TR::TreeTop::create(comp(), _curTree,
                    TR::Node::create(TR::treetop, 1,
                        TR::Node::createWithSymRef(TR::call, 2, 2, node, node->getFirstChild(), constructorSymRef)));
                ...
                node->setSymbolReference(
                    comp()->getSymRefTab()->findOrCreateNewObjectSymbolRef(comp()->getMethodSymbol()));
            }
        }
        ...
        if (_disableValueTypeStackAllocation && (node->getOpCodeValue() == TR::newvalue)) {
            logprintf(trace(), log, "Reject candidate %s n%dn [%p] because value type stack allocation is disabled\n",
                node->getOpCode().getName(), node->getGlobalIndex(), node);
        ...
        if (_curBlock->isCold() || _curBlock->isCatchBlock()
        ...
        logprintf(trace(), log, "4 setting local alloc %p to %s\n", candidate->_node,
            candidate->isLocalAllocation() ? "true" : "false");

        if (foundUserAnnotation) {
            candidate->setForceLocalAllocation(true);
            candidate->setObjectIsReferenced();
            logprintf(trace(), log, "   Force [%p] to be locally allocated due to annotation of %s\n", node, className);
        }
...
Candidate *TR_EscapeAnalysis::createCandidateIfValid(TR::Node *node, TR_OpaqueClassBlock *&classInfo,
    bool foundUserAnnotation)
{
    OMR::Logger *log = comp()->log();
    ...
    if (!foundUserAnnotation) {
        ...
        if (node->getOpCodeValue() == TR::New || node->getOpCodeValue() == TR::newvalue) {
            TR::Node *classNode = node->getFirstChild();
            if (classNode->getOpCodeValue() != TR::loadaddr) {
                logprintf(trace(), log, "   Node [%p] failed: child is not TR::loadaddr\n", node);
                return NULL;
            }

            if (classNode->getSymbolReference()->isUnresolved()) {
                logprintf(trace(), log, "   Node [%p] failed: class is unresolved\n", node);
                return NULL;
            }
            ...
            if (comp()->getRunnableClassPointer()
                && comp()->fej9()->isInstanceOf((TR_OpaqueClassBlock *)classSym->getStaticAddress(),
                       comp()->getRunnableClassPointer(), true)
                    == TR_yes) {
                if (trace()) {
                    const char *className = getClassName(classNode);
                    log->printf("secs Class %s implements Runnable in %s\n",
                        className ? className : "<Missing class name>", comp()->signature());
        ...
        else if (!comp()->cg()->getHasDoubleWordAlignedStack() && node->getOpCodeValue() == TR::newarray
            && !comp()->getOption(TR_EnableSIMDLibrary)) {
            TR::Node *typeNode = node->getSecondChild();
            if (typeNode->getInt() == 7 || typeNode->getInt() == 11) {
                logprintf(trace(), log, "   Node [%p] failed: double-size array\n", node);
                return NULL;
            }
        }
    }
    ...
    if (comp()->cg()->getSupportsStackAllocationOfArraylets()) {
        if (node->getOpCodeValue() != TR::New && node->getOpCodeValue() != TR::newvalue) {
            logprintf(trace(), log, "   Node [%p] failed: arraylet\n", node);
            return NULL;
        }
    }
    ...
    if (classInfo
        && !TR::Compiler->cls.sameClassLoaders(comp(), classInfo,
            comp()->getJittedMethodSymbol()->getResolvedMethod()->containingClass())
        && !comp()->fej9()->isClassLoadedBySystemClassLoader(classInfo))
        return NULL;

    if (size <= 0) {
        logprintf(trace(), log, "   Node [%p] failed: VM can't skip allocation (code %d, class %p)\n", node, size,
            classInfo);

        if (size == 0 && classInfo && (manager()->numPassesCompleted() == 0)
            && optimizer()->isEnabled(OMR::profiledNodeVersioning) && !_curBlock->isCold()) {
            ...
            if (numElementsNode && valueProfiler
                && performTransformation(comp(), "%sContinue analyzing %s node %s for size-profiling opportunity\n",
                    OPT_DETAILS, node->getOpCode().getName(), comp()->getDebug()->getName(node))) {
                profileOnly = true;
                size = TR::Compiler->om.contiguousArrayHeaderSizeInBytes(); // Must be at least this big
            } else {
                return NULL;
            }
        } else if ((node->getOpCodeValue() == TR::New || node->getOpCodeValue() == TR::newvalue) && classInfo)
            size = 0;
        else
            return NULL;
    } else {
        ...
        while (currentClass && currentClass != jlObject) {
            if (currentClass == jlReference) {
                logprintf(trace(), log, "   Node [%p] failed: class %p is subclass of j/l/r/Reference\n", node,
                    classInfo);
                return NULL;
            } else {
...
bool TR_EscapeAnalysis::isEscapePointCold(Candidate *candidate, TR::Node *node)
{
    ...
    if (!disableColdEsc
        && (_inColdBlock
            || (candidate->isInsideALoop() && (candidate->_block->getFrequency() > 4 * _curBlock->getFrequency())))
...
void TR_EscapeAnalysis::checkDefsAndUses()
{
    ...
    for (candidate = _candidates.getFirst(); candidate; candidate = next) {
        ...
        if (candidate->isInsideALoop()) {
            if (_otherDefsForLoopAllocation)
                ...
                _otherDefsForLoopAllocation
                    = new (trStackMemory()) TR_BitVector(_useDefInfo->getNumDefNodes(), trMemory(), stackAlloc);
        }

        if (comp()->getOptions()->realTimeGC() && comp()->compilationShouldBeInterrupted(ESC_CHECK_DEFSUSES_CONTEXT)) {
        ...
        if (checkDefsAndUses(node, candidate)) {
            if (candidate->_valueNumbers->size() > 1) {
                candidate->setMustBeContiguousAllocation();
                logprintf(trace(), log, "   Make [%p] contiguous because its uses can be reached from other defs\n",
                    candidate->_node);
            }
        } else {
            candidate->setLocalAllocation(false);
            logprintf(trace(), log, "5 setting local alloc %p to false\n", candidate->_node);
        }
    }

    _vnTemp = new (trStackMemory())
        TR_BitVector(optimizer()->getValueNumberInfo()->getNumberOfNodes(), trMemory(), stackAlloc, notGrowable);
    ...
    for (; tt; tt = tt->getNextTreeTop()) {
        ...
        if (node->getOpCode().isStoreIndirect() || (node->getOpCodeValue() == TR::arraycopy)) {
            TR::Node *baseObject = node;

            if (node->getSymbol()->isArrayShadowSymbol() && node->getFirstChild()->getOpCode().isArrayRef())
            ...
            if (node->getOpCode().isStoreIndirect() && (baseObject->getFirstChild() == node->getSecondChild())) {
                ...
                if ((baseChild && (baseChild->getOpCodeValue() == TR::loadaddr)
                    ...
                    if (node->getOpCodeValue() == TR::arraycopy) {
                        ...
                        logprintf(trace(), log, "Reached 0 with baseChild %p VN %d\n", baseChild, baseChildVN);
                    } else {
                        if (!baseChild->cannotTrackLocalUses()) {
                            storeOfObjectIntoField = true;
                            storeIntoOtherLocalObject = true;
                            logprintf(trace(), log, "Reached 1 with baseChild %p VN %d\n", baseChild, baseChildVN);
                        } else {
                            _notOptimizableLocalObjectsValueNumbers->set(baseChildVN);
                            _notOptimizableLocalStringObjectsValueNumbers->set(baseChildVN);
                            storeOfObjectIntoField = false;
                                ...
                                if (defNode && (defNode->getOpCodeValue() == TR::astore)
                                    ...
                                    if (defChild && (defChild->getOpCodeValue() == TR::loadaddr)
                                        ...
                                        if (node->getOpCode().isStoreIndirect()
                                            && (_valueNumberInfo->getValueNumber(defChild)
                                                == _valueNumberInfo->getValueNumber(baseChild))) {
                                            baseChildVN = _valueNumberInfo->getValueNumber(baseChild);
                                            storeOfObjectIntoField = true;
                                            storeIntoOtherLocalObject = true;
                                        } else {
                                            _notOptimizableLocalObjectsValueNumbers->set(
                                                _valueNumberInfo->getValueNumber(baseChild));
                                            _notOptimizableLocalObjectsValueNumbers->set(
                                                _valueNumberInfo->getValueNumber(defChild));
                                            _notOptimizableLocalStringObjectsValueNumbers->set(
                                                _valueNumberInfo->getValueNumber(baseChild));
                                            _notOptimizableLocalStringObjectsValueNumbers->set(
                                                _valueNumberInfo->getValueNumber(defChild));
                                            storeOfObjectIntoField = false;
        ...
        if (node->getOpCode().isCall() && (node->getSymbol()->getResolvedMethodSymbol())
            && (node->getReferenceCount() > 1) && (node->getNumChildren() > 0)) {
                ...
                case TR::java_math_BigDecimal_possibleClone: {
                    ...
                    for (Candidate *candidate = _candidates.getFirst(); candidate; candidate = candidate->getNext()) {
                        if (usesValueNumber(candidate, nodeVN)) {
                            ...
                            //
                            if (methodSymbol
                                && (!methodSymbol->getResolvedMethod()->virtualMethodIsOverridden()
        ...
        if (_nodeUsesThroughAselect) {
            ...
            for (auto mi = _nodeUsesThroughAselect->begin(); mi != _nodeUsesThroughAselect->end(); mi++) {
                ...
                for (auto di = mi->second->begin(), end = mi->second->end(); di != end; di++) {
                    ...
                    log->printf("%s[%p] n%dn", (first ? "" : ", "), aselectNode, aselectNode->getGlobalIndex());
                    first = false;
                }
...
void TR_EscapeAnalysis::associateAselectWithChild(TR::Node *aselectNode, int32_t idx)
{
    ...
    if (NULL == _nodeUsesThroughAselect) {
        _nodeUsesThroughAselect = new (trStackMemory())
            NodeToNodeDequeMap((NodeComparator()), NodeToNodeDequeMapAllocator(stackMemoryRegion));
    }
    ...
    if (_nodeUsesThroughAselect->end() != search) {
        ...
        nodeAlreadyMapsToAselect
            = (std::find(search->second->begin(), search->second->end(), aselectNode) != search->second->end());
    } else {
...
bool TR_EscapeAnalysis::collectValueNumbersOfIndirectAccessesToObject(TR::Node *node, Candidate *candidate,
    TR::Node *indirectStore, TR::NodeChecklist &visited, int32_t baseChildVN)
{
    OMR::Logger *log = comp()->log();
    ...
    if (node->getOpCode().isLoadIndirect()) {
        ...
        if (node->getSymbolReference()->getReferenceNumber()
            == indirectStore->getSymbolReference()->getReferenceNumber())
            sameSymbol = true;
        ...
        logprintf(trace(), log, "store node %p load node %p candidate %p baseChildVN %d\n", indirectStore, node,
            candidate->_node, baseChildVN);
                                    ...
                                    for (i = _useDefInfo->getNumDefOnlyNodes() - 1; i >= 0; --i) {
                                        ...
                                        if (defNode && defNode->getOpCode().isStore()) {
                                            if (_vnTemp->isSet(_valueNumberInfo->getValueNumber(defNode))) {
                                                ...
                                                _useDefInfo->getUsesFromDef(usesOfThisDef,
                                                    defNode->getUseDefIndex() + _useDefInfo->getFirstDefIndex());
                                                if (!usesOfThisDef.IsZero()) {
                                                    TR_UseDefInfo::BitVector::Cursor cursor(usesOfThisDef);
                                                    for (cursor.SetToFirstOne(); cursor.Valid();
                                                         cursor.SetToNextOne()) {
                                                        int32_t useIndex = cursor;
                                                        TR::Node *useNode = _useDefInfo->getNode(
                                                            useIndex + _useDefInfo->getFirstUseIndex());
    ...
    int32_t i;
    for (i = 0; i < node->getNumChildren(); i++) {
        if (collectValueNumbersOfIndirectAccessesToObject(node->getChild(i), candidate, indirectStore, visited,
                baseChildVN))
            foundAccess = true;
    }
    return foundAccess;
            ...
            for (auto di = search->second->begin(), end = search->second->end(); di != end; di++) {
                ...
                if (i < 0) {
                    ...
                    logprintf(trace(), comp()->log(),
                        "   Checking uses of node %p through aselect operation %p for candidate %p\n", node,
                        aselectNode, candidate->_node);
                        ...
                        if (i < 0) {
                            ...
                            if (candidate->isInsideALoop()) {
                                ...
                                if (!p) {
                                    ...
                                    //
                                    logprintf(trace(), log, "   Look at other defs for use node %p of candidate %p\n",
                                        useNode, candidate->_node);
                                    ////_otherDefsForLoopAllocation->set(udIndex);

                                    if (!checkOverlappingLoopAllocation(useNode, candidate)) {
                                        logprintf(trace(), log,
                                            "   Make [%p] non-local because it overlaps with use [%p]\n",
                                            candidate->_node, useNode);
                                        returnValue = false;
                                    }
                                    if (!checkOtherDefsOfLoopAllocation(useNode, candidate,
                                            (next->getFirstChild() == candidate->_node))) {
                                        logprintf(trace(), log,
                                            "   Make [%p] non-local because multiple defs to node [%p]\n",
                                            candidate->_node, useNode);
                                        returnValue = false;
                                    }
                                } else
...
bool TR_EscapeAnalysis::checkOtherDefsOfLoopAllocation(TR::Node *useNode, Candidate *candidate, bool isImmediateUse)
{
    ...
    // object from a prior pass of escape analysis, it obviously cannot be a candidate for stack allocation in this
    // pass.
    //
    OMR::Logger *log = comp()->log();
    ...
    for (cursor.SetToFirstOne(); cursor.Valid(); cursor.SetToNextOne()) {
        ...
        //
        TR::Node *defNode = _useDefInfo->getNode(defIndex);
        if (!seenOtherDef && isImmediateUse
        ...
        {
            logprintf(trace(), log, "      Ignoring def node [%p] for use node [%p]\n", defNode, useNode);
            continue;
        }

        _otherDefsForLoopAllocation->set(defIndex);

        logprintf(trace(), log, "      Look at def node [%p] for use node [%p]\n", defNode, useNode);
        ...
        if (!allnewsonrhs
            && !(defNode->getOpCode().isStoreDirect()
                && ((defNode->getFirstChild()->getOpCodeValue() == TR::aconst)
                    || (defNode->getFirstChild()->getOpCode().isLoadVar()
                        && // load from a static or array shadow or shadow (field) based off a non local object are fine
                           // since such memory locations would imply that the object being pointed at escaped already
                           // (meaning there would be another escape point anyway)
                        (defNode->getFirstChild()->getSymbol()->isStatic()
                            || (defNode->getFirstChild()->getSymbol()->isShadow()
                                && (defNode->getFirstChild()->getSymbol()->isArrayShadowSymbol()
                                    || !_nonColdLocalObjectsValueNumbers->get(_valueNumberInfo->getValueNumber(
                                        defNode->getFirstChild()->getFirstChild()))))))))) {
            if (_valueNumberInfo->getValueNumber(defNode) != _valueNumberInfo->getValueNumber(useNode)) {
                ...
                if (checkIfUseIsInSameLoopAsDef(_useDefInfo->getTreeTop(defIndex), useNode)
                    ...
                    logprintf(trace(), log, "         Def node [%p] same as candidate [%p]\n", defNode,
                        candidate->_node);
                    return false;
                }
            }
        ...
        logprintf(trace(), log, "         Def node [%p] not the same as candidate [%p]\n", defNode, candidate->_node);
    }
    return true;
}
...
bool TR_EscapeAnalysis::checkAllNewsOnRHSInLoopWithAliasing(int32_t defIndex, TR::Node *useNode, Candidate *candidate)
{
    ...
    if ((defNode->getFirstChild() == candidate->_node)
        && (_valueNumberInfo->getValueNumber(defNode) == _valueNumberInfo->getValueNumber(useNode))) {
        logprintf(trace(), log, "      Value numbers match for def node [%p] with use node [%p]\n", defNode, useNode);
        ...
        logprintf(trace(), log,
            "      Value numbers match for def node [%p] with candidate node [%p], and def node's symref is alias of "
        ...
        for (cursor2.SetToFirstOne(); cursor2.Valid(); cursor2.SetToNextOne()) {
            ...
            for (Candidate *otherAllocNode = _candidates.getFirst(); otherAllocNode;
                 otherAllocNode = otherAllocNode->getNext()) {
                ...
                logprintf(trace(), log, "         Look at defNode2 [%p] with otherAllocNode [%p]\n", defNode2,
                    otherAllocNode->_node);

                if (!rhsIsHarmless
                    && (_valueNumberInfo->getValueNumber(defNode2)
                        == _valueNumberInfo->getValueNumber(otherAllocNode->_node))) {
                    TR::TreeTop *treeTop;
                    bool collectAliases = false;
                    _aliasesOfOtherAllocNode->empty();
                    _visitedNodes->empty();
                    for (treeTop = otherAllocNode->_treeTop->getEnclosingBlock()->getEntry(); treeTop;
                    ...
                    if ((_useDefInfo->getTreeTop(defIndex2)->getEnclosingBlock() == otherAllocNode->_block)
                        ...
                        logprintf(trace(), log, "      rhs is harmless for defNode2 [%p] with otherAllocNode [%p]\n",
            ...
            if (!rhsIsHarmless) {
                logprintf(trace(), log, "   defNode2 vn=%d is local %d\n", _valueNumberInfo->getValueNumber(defNode2),
                    _allLocalObjectsValueNumbers->get(_valueNumberInfo->getValueNumber(defNode2)));
            ...
            if (!rhsIsHarmless) {
                // Another special case when it is certain that the rhs of the def is not a candidate allocation from a
                // prior iteration In this case we are loading a value from an Integer cache anyway and that should be
                // an allocation that has already escaped that has nothing to do with the candidate allocation
                //
                if (firstChild->getOpCode().hasSymbolReference() && firstChild->getSymbol()->isArrayShadowSymbol()) {
                    ...
                    if (addr->getOpCode().isArrayRef()) {
                        ...
                        if (underlyingArray && underlyingArray->getOpCode().hasSymbolReference()
                            ...
                            fieldName = underlyingArray->getSymbolReference()->getOwningMethod(comp())->staticName(
                                underlyingArray->getSymbolReference()->getCPIndex(), fieldNameLen, comp()->trMemory());
                        }

                        if (fieldName && (fieldNameLen > 10) && !strncmp("java/lang/", fieldName, 10)
                        ...
                        {
                            logprintf(trace(), log,
                                "         rhs is harmless for defNode2 [%p] access of Integer cache\n", defNode2);
                ...
                // Another special case when it is certain that the rhs of the def is not a candidate allocation from a
                // prior iteration In this case we are loading a value through a call to jitLoadFlattenableArrayElement.
                // Such a value must be something that has already escaped and has nothing to do with the candidate
                // allocation
                //
                    ...
                    logprintf(trace(), log,
                        "         rhs is harmless for defNode2 [%p] - call to jitLoadFlattenableArrayElement\n",
                        defNode2);
            ...
            if (!rhsIsHarmless) {
                logprintf(trace(), log, "      rhs not harmless for defNode2 [%p]\n", defNode2);
...
bool TR_EscapeAnalysis::checkOverlappingLoopAllocation(TR::Node *node, TR::Node *useNode, TR::Node *allocNode,
    rcount_t &numReferences)
{
    ...
    if ((node != allocNode) && (_valueNumberInfo->getValueNumber(node) == _valueNumberInfo->getValueNumber(useNode))) {
...
bool TR_EscapeAnalysis::checkIfUseIsInSameLoopAsDef(TR::TreeTop *defTree, TR::Node *useNode)
{
    ...
    if (highestCyclicStructure) {
        ...
        for (nextBlock = blocksIt.getCurrent(); nextBlock; nextBlock = blocksIt.getNext()) {
            ...
            while (currentTree && (currentTree != exitTree)) {
...
bool TR_EscapeAnalysis::checkIfUseIsInLoopAndOverlapping(Candidate *candidate, TR::TreeTop *defTree, TR::Node *useNode)
{
    ...
    logprintf(trace(), log, "Started checking for candidate %p\n", candidate->_node);
    bool decisionMade = false;
    bool b = checkIfUseIsInLoopAndOverlapping(allocTree->getNextTreeTop(), candidate->_block->getExit(), defTree,
        useNode, visited, vBlocks, decisionMade);
    logprintf(trace(), log, "Finished checking for candidate %p\n", candidate->_node);
    return b;
}

bool TR_EscapeAnalysis::checkIfUseIsInLoopAndOverlapping(TR::TreeTop *start, TR::TreeTop *end, TR::TreeTop *defTree,
    TR::Node *useNode, TR::NodeChecklist &visited, TR::BlockChecklist &vBlocks, bool &decisionMade)
{
    OMR::Logger *log = comp()->log();
    ...
    while (currentTree && (currentTree != end)) {
        if (checkUse(currentTree->getNode(), useNode, visited)) {
            decisionMade = true;
            logprintf(trace(), log, "Returning TRUE at %p\n", currentTree->getNode());
            return true;
        }

        if (currentTree == defTree) {
            logprintf(trace(), log, "Returning FALSE at %p\n", currentTree->getNode());
            decisionMade = true;
            return false;
        }
    ...
    for (auto nextEdge = block->getSuccessors().begin(); nextEdge != block->getSuccessors().end(); ++nextEdge) {
        ...
        if (!vBlocks.contains(next) && (next != cfg->getEnd())) {
            logprintf(trace(), log, "Looking at block_%d\n", next->getNumber());
            bool b = checkIfUseIsInLoopAndOverlapping(next->getEntry(), next->getExit(), defTree, useNode, visited,
                vBlocks, decisionMade);
            if (decisionMade) {
                if (b)
                    return true;
            }
    ...
    for (auto nextEdge = block->getExceptionSuccessors().begin(); nextEdge != block->getExceptionSuccessors().end();
         ++nextEdge) {
        TR::Block *next = toBlock((*nextEdge)->getTo());
        decisionMade = false;
        if (!vBlocks.contains(next) && (next != cfg->getEnd())) {
    ...
    logprintf(trace(), log, "Returning FALSE at block_%d\n", block->getNumber());
    return false;
}
        ...
        if (usesValueNumber(candidate, valueNumber)) {
            if (!forceFail && checkIfEscapePointIsCold(candidate, reason)) {
                if (!isImmutableObject(candidate)) {
                    logprintf(trace(), log, "   Make [%p] contiguous because of node [%p]\n", candidate->_node, reason);
                    candidate->setMustBeContiguousAllocation();
                    // candidate->setLocalAllocation(false);
                } else {
                    logprintf(trace(), log,
                        "  Marking immutable candidate [%p] as referenced in forceEscape to allow for non-contiguous "
                        "allocation, but compensating for escape at [%p]\n",
                        candidate->_node, reason);
                    candidate->setObjectIsReferenced();
                }
            } else {
                if (candidate->forceLocalAllocation()) {
                    logprintf(trace(), log,
                        "   Normally would fail [%p] because it escapes via node [%p] (cold %d), but user forces it to "
                        "be local\n",
                        candidate->_node, reason, _inColdBlock);
                    continue;
                }

                logprintf(trace(), log, "   Fail [%p] because it escapes via node [%p] (cold %d)\n", candidate->_node,
                    reason, _inColdBlock);
...
void TR_EscapeAnalysis::markCandidatesUsedInNonColdBlock(TR::Node *node)
{
    ...
    for (candidate = _candidates.getFirst(); candidate; candidate = next) {
        ...
        if (!candidate->usedInNonColdBlock() && usesValueNumber(candidate, valueNumber)) {
            ...
            logprintf(trace(), comp()->log(), "   Mark [%p] used in non-cold block because of node [%p]\n",
...
bool TR_EscapeAnalysis::detectStringCopy(TR::Node *node)
{
    ...
    for (candidate = _candidates.getFirst(); candidate; candidate = next) {
        next = candidate->getNext();
        if ((baseNode == candidate->_node) && (baseNode->getOpCodeValue() == TR::New) && node->getOpCode().isIndirect()
            && (baseNode->getFirstChild()->getSymbolReference()->getSymbol()->getStaticSymbol()->getStaticAddress()
                == comp()->getStringClassPointer())) {
            if (valueNode->getOpCode().isIndirect() && (valueNode->getSymbolReference() == node->getSymbolReference())
                && (valueNode->getFirstChild()->getOpCode().isLoadVar()
                    && !valueNode->getFirstChild()->getSymbolReference()->isUnresolved())
...
bool TR_EscapeAnalysis::restrictCandidates(TR::Node *node, TR::Node *reason, restrictionType type)
{
    ...
    bool locked = false;
    if (reason && ((reason->getOpCodeValue() == TR::monent) || (reason->getOpCodeValue() == TR::monexit)))
        locked = true;
        ...
        if (candidate->isLocalAllocation() && usesValueNumber(candidate, valueNumber)) {
            ...
            if (locked) {
                if (!_inColdBlock) {
                    ...
                    logprintf(trace(), log, "   Mark [%p] used and locked in non-cold block because of node [%p]\n",
                        candidate->_node, node);
                }
            ...
            if (type == MakeNonLocal) {
                if (checkIfEscapePointIsCold(candidate, reason)) {
                    // candidate->setObjectIsReferenced();
                    logprintf(trace(), log, "   Do not make [%p] non-local because of cold node [%p]\n",
                        candidate->_node, reason);
                } else {
                    logprintf(trace(), log, "   Make [%p] non-local because of node [%p]\n", candidate->_node, reason);
                    candidate->setLocalAllocation(false);
                ...
                if (type == MakeContiguous) {
                    ...
                    if (checkIfEscapePointIsCold(candidate, reason)) {
                        // candidate->setObjectIsReferenced();
                        logprintf(trace(), log, "   Do not make [%p] contiguous because of cold node [%p]\n",
                            candidate->_node, reason);
                    } else {
                        logprintf(trace(), log, "   Make [%p] contiguous because of node [%p]\n", candidate->_node,
                            reason);
                    ...
                    if (!isImmutableObject(candidate))
                        wasRestricted = true;
                } else if (!candidate->objectIsReferenced() && !candidate->mustBeContiguousAllocation()) {
                    logprintf(trace(), log, "   Make [%p] object-referenced because of node [%p]\n", candidate->_node,
                        reason);
                    candidate->setObjectIsReferenced();
                    wasRestricted = true;
                }
...
{
    if (_curBlock->isOSRCodeBlock() || _curBlock->isOSRCatchBlock())
        return false;
    ...
    if (isEscapePointCold(candidate, node)) {
        ...
        for (j = 0; j < node->getNumChildren(); j++) {
            ...
            if (usesValueNumber(candidate, _valueNumberInfo->getValueNumber(resolvedChildAtTopLevel))) {
                if (resolvedChildAtTopLevel->getOpCode().isLoadVarDirect() && (_curBlock != candidate->_block)
                    ...
                    if (_curBlock->isCatchBlock()) {
                        ...
                        if (!firstTree->getOpCode().isStoreDirect() || !firstTree->getSymbol()->isAuto()
                            || !firstTree->getFirstChild()->getOpCode().hasSymbolReference()
                            || (firstTree->getFirstChild()->getSymbolReference()
                                != comp()->getSymRefTab()->findOrCreateExcpSymbolRef()))
                            recognizedCatch = false;
                    }
                    if (recognizedCatch) {
                        logprintf(trace(), log, "Adding cold block info for child %p value number %d candidate %p\n",
                        ...
                        logprintf(trace(), log,
                            "   For candidate [%p], seen an unexpected opcode in child [%p] of call [%p]\n",
        ...
        if (canStoreToHeap) {
            candidate->setObjectIsReferenced();
            logprintf(trace(), log,
                "  Marking candidate [%p] as referenced in checkIfEscapePointIsCold - escape point [%p]\n",
                candidate->_node, node);

            if (!isImmutableObject(candidate) && (_parms || !node->getOpCode().isReturn())) {
                // candidate->setObjectIsReferenced();
                logprintf(trace(), log, "   Make candidate [%p] contiguous to allow heapification\n", candidate->_node);
                candidate->setMustBeContiguousAllocation();
            }
...
static void checkForDifferentSymRefs(Candidate *candidate, int32_t i, TR::SymbolReference *symRef,
    TR_EscapeAnalysis *ea, bool peeking)
{
    ...
    if (memorizedSymRef && (memorizedSymRef != symRef)
        && (symRef->isUnresolved() || memorizedSymRef->isUnresolved()
        ...
        if (memorizedSymRef->getCPIndex() == -1 || symRef->getCPIndex() == -1
            || !TR::Compiler->cls.jitFieldsAreSame(comp, memorizedSymRef->getOwningMethod(comp),
                memorizedSymRef->getCPIndex(), symRef->getOwningMethod(comp), symRef->getCPIndex(),
            ...
            if (!aliasingIsSafe) {
                logprintf(ea->trace(), comp->log(),
                    "candidate n%dn %p excluded because of ambiguous field symrefs #%d [%s] and [%s]\n",
                    candidate->_node->getGlobalIndex(), candidate->_node, memorizedSymRef->getReferenceNumber(),
                    memorizedSymRef->getName(comp->getDebug()), symRef->getName(comp->getDebug()));
...
TR::SymbolReference *FieldInfo::fieldSymRef() { return _goodFieldSymrefs->getHeadData(); }
...
bool FieldInfo::symRefIsForFieldInAllocatedClass(TR::SymbolReference *symRef)
{
    ...
    TR_ASSERT(0, "symRefIsForFieldInAllocatedClass expects symref #%d to have been remembered",
        symRef->getReferenceNumber());
    return true;
}

bool FieldInfo::hasBadFieldSymRef() { return !_badFieldSymrefs->isEmpty(); }
...
//
void TR_EscapeAnalysis::referencedField(TR::Node *base, TR::Node *field, bool isStore, bool seenSelfStore,
    bool seenStoreToLocalObject)
{
    ...
    for (candidate = _candidates.getFirst(); candidate; candidate = candidate->getNext()) {
        if (seenStoreToLocalObject) {
            if (candidate->isLocalAllocation() && usesValueNumber(candidate, storedValueNumber)) {
                if (candidate->isInsideALoop()) {
                    candidate->setLocalAllocation(false);
                    logprintf(trace(), log, "7 setting local alloc %p to false\n", candidate->_node);
                } else
        ...
        if (candidate->isLocalAllocation() && usesValueNumber(candidate, valueNumber)) {
            if (usesStackTrace) {
                ...
                candidate->setMustBeContiguousAllocation();
                logprintf(trace(), log, "   Make [%p] contiguous because of setUsesStackTrace\n", candidate->_node);
            }
...
TR::Node *TR_EscapeAnalysis::resolveSniffedNode(TR::Node *node)
{
    if (_parms == NULL)
        return node;
    if (!node->getOpCode().isLoadVarOrStore() && (node->getOpCodeValue() != TR::loadaddr))
        return NULL;
...
void TR_EscapeAnalysis::checkEscape(TR::TreeTop *firstTree, bool isCold, bool &ignoreRecursion)
{
    ...
    for (treeTop = firstTree; treeTop && !_candidates.isEmpty(); treeTop = treeTop->getNextTreeTop()) {
        ...
        if (node->getOpCode().isStoreIndirect() && detectStringCopy(node)) {
            ...
            logprintf(trace(), log, "Base candidate: [%p], base node VN: %d, copy node VN: %d\n", baseCandidate->_node,
                baseNodeVN, copyNodeVN);
        ...
        } else if (0 && (node->getOpCodeValue() != TR::call) && (node->getNumChildren() > 0)) {
            node = node->getFirstChild();
            if ((node->getOpCodeValue() == TR::call) && !node->getSymbolReference()->isUnresolved()
                && !node->getSymbol()->castToMethodSymbol()->isHelper()
                && (node->getSymbol()->castToResolvedMethodSymbol()->getResolvedMethod()->getRecognizedMethod()
                    == TR::java_lang_String_init_String)
                ...
                for (candidate = _candidates.getFirst(); candidate; candidate = next) {
                    ...
                    if ((candidate->_stringCopyNode == NULL)
                        ...
                        if ((baseNode == candidate->_node) && (baseNode->getOpCodeValue() == TR::New)
    ...
    for (treeTop = firstTree; treeTop && !_candidates.isEmpty(); treeTop = treeTop->getNextTreeTop()) {
        ...
        if (node->getOpCode().isCall() && !vnsCall.contains(node)) {
            if (node->getSymbolReference()->getReferenceNumber() != TR_prepareForOSR
                || comp()->getOSRMode() != TR::voluntaryOSR || _curBlock->isOSRInduceBlock())
...
static bool isConstantClass(TR::Node *classNode, TR_EscapeAnalysis *ea)
{
    ...
    //
    if (classNode->getOpCodeValue() == TR::loadaddr && classNode->getSymbol()->isStatic()
        && !classNode->getSymbolReference()->isUnresolved()) {
        result = true;
    }

    logprintf(ea->trace(), comp->log(), "   isConstantClass(%p)=%s (supportsInliningOfIsInstance=%s)\n", classNode,
        result ? "true" : "false", comp->cg()->supportsInliningOfIsInstance() ? "true" : "false");
...
{
    TR_ASSERT(vftLoad->getOpCode().isLoadIndirect() && vftLoad->getFirstChild() == candidate
            && vftLoad->getSymbolReference()->getSymbol()->isClassObject() && candidate->getOpCode().isRef(),
    ...
    */
    ...
    return root->getOpCodeValue() == ifOp && r1->getOpCodeValue() == andOp && r2->getOpCodeValue() == constOp
        && (usesLongOps ? r2->getLongInt() : r2->getInt()) == 0 && loadNode->getOpCodeValue() == loadOp
        && r12->getOpCodeValue() == constOp
        && (usesLongOps ? r12->getLongInt() : r12->getInt()) == fej9->getFlagValueForFinalizerCheck()
...
void TR_EscapeAnalysis::checkEscapeViaNonCall(TR::Node *node, TR::NodeChecklist &visited)
{
    ...
    //
    if (node->getOpCode().isLoadVarOrStore() && node->getSymbol()->isArrayShadowSymbol()
        && node->getOpCode().isIndirect()) {
        ...
        if (node->getSecondChild()->isNull()) {
            ...
            for (i = node->getNumChildren() - 1; i >= 0; i--) {
                ...
                if (candidate && candidate->_dememoizedConstructorCall) {
                    logprintf(trace(), log, "Rememoize [%p] due to [%p] under address compare [%p]\n", candidate->_node,
                        child, node);
                    rememoize(candidate);
                    _candidates.remove(candidate);
                }
    ...
    for (i = node->getNumChildren() - 1; i >= 0; i--) {
        ...
        if (!visited.contains(child)) {
            if (node->getOpCode().isCallIndirect() && (i < node->getFirstArgumentIndex()))
    ...
    if (node->getOpCodeValue() == TR::areturn) {
        ...
        if (_parms) {
            ...
            bool doit = true;
            if (!_methodSymbol || !_inBigDecimalAdd
                || (_methodSymbol->getRecognizedMethod() != TR::java_math_BigDecimal_possibleClone))
                doit = false;
            else {
                logprintf(trace(), log, "seen bd possible clone at node %p\n", callNode);
            }

            if (argument && doit
                && (callNode->getOpCodeValue() == TR::acall || callNode->getOpCodeValue() == TR::acalli)
                && _sniffDepth == 1 // Note: this only goes one level deep.  If a returns it to b which returns it to
                                    // the caller, we'll call that an escape.
                && performTransformation(comp(),
                    "%sPeeked method call [%p] returning argument [%p] NOT considered an escape\n", OPT_DETAILS,
                    callNode, argument)) {
                // Bingo.  Add the call node's value number to the bag for every
                // candidate for which argument is already in the bag.
                //
                logprintf(trace(), log, "call node is %p (depth %d) for which return opt was done\n", callNode,
                    _sniffDepth);
#if 0
            //
    ...
    if (node->getOpCode().hasSymbolReference()
        && (node->getSymbolReference()->getSymbol() == comp()->getSymRefTab()->findGenericIntShadowSymbol()
            || node->getSymbol()->isUnsafeShadowSymbol() || node->getDataType().isBCD()
    ...
    if (node->getOpCode().isLoadVarOrStore()) {
        ...
        if (node->getOpCode().isIndirect()) {
            ...
            if (node->getOpCode().isStore()) {
                ...
                if (node->getSymbol()->isArrayShadowSymbol() && baseObject->getOpCode().isArrayRef())
                ...
                if ((node->getSecondChild() != baseObject)
                    && (!baseObject->getOpCode().hasSymbolReference()
                        ...
                        || ((baseObject->getReferenceCount() != 1)
                            && (!((baseObject->getReferenceCount() == 2) && (node->getOpCodeValue() == TR::awrtbari)
                                && (node->getChild(2) == baseObject))))
                    ...
                    if ((!_nonColdLocalObjectsValueNumbers || !_notOptimizableLocalObjectsValueNumbers
                            || !resolvedBaseObject
                            || (comp()->useCompressedPointers() && (TR::Compiler->om.compressedReferenceShift() > 3)
                                && !cg()->supportsStackAllocations())
                            || !resolvedBaseObject->getOpCode().hasSymbolReference()
                            || !_nonColdLocalObjectsValueNumbers->get(
                                _valueNumberInfo->getValueNumber(resolvedBaseObject))
                            || (((node->getSymbolReference()->getSymbol()->getRecognizedField()
                                     != TR::Symbol::Java_lang_String_value)
                                    || stringCopyOwningMethod
                                    || _notOptimizableLocalStringObjectsValueNumbers->get(
                                        _valueNumberInfo->getValueNumber(resolvedBaseObject)))
                                && _notOptimizableLocalObjectsValueNumbers->get(
                                    _valueNumberInfo->getValueNumber(resolvedBaseObject))))) {
            ...
            child = node->getFirstChild();
            if (node->getSymbol()->isArrayShadowSymbol() && child->getOpCode().isArrayRef())
            ...
            //
            else if (node->getSymbolReference()->getOffset() < (int32_t)comp()->fej9()->getObjectHeaderSizeInBytes()
                && !node->getSymbol()->isArrayShadowSymbol()) {
                if (!node->getSymbolReference()->getSymbol()->isClassObject() || node->getOpCode().isStore()
                    || (!classObjectLoadForVirtualCall
                        && !isFinalizableInlineTest(comp(), child, _curTree->getNode(), node))) {
                    restrictCandidates(child, node, MakeContiguous);
                }
            }
        }
    ...
    //
    if (node->getOpCode().isCheckCast() || node->getOpCodeValue() == TR:: instanceof) {
        ...
        if (candidate) {
            ...
            if (!isConstantClass(classNode, this)) {
                ...
                && comp()->fej9()->isInstanceOf((TR_OpaqueClassBlock *)candidate->_class,
                       (TR_OpaqueClassBlock *)classNode->getSymbol()->castToStaticSymbol()->getStaticAddress(), true)
                    != TR_yes) {
    ...
    else if (node->getOpCodeValue() != TR::treetop && !node->getOpCode().isArrayRef()
                 && node->getOpCodeValue() != TR::PassThrough && !node->getOpCode().isArrayLength()
                 && node->getOpCodeValue() != TR::NULLCHK && node->getOpCodeValue() != TR::ResolveCHK
                 && node->getOpCodeValue() != TR::ResolveAndNULLCHK && node->getOpCodeValue() != TR::
                 instanceof &&
#if CHECK_MONITORS
                            ((node->getOpCodeValue() != TR::monent && node->getOpCodeValue() != TR::monexit)
                                || !_removeMonitors)
                 &&
#endif
    ...
    if (wasRestricted) {
        logprintf(trace(), log, "Object referenced via %s\n", node->getOpCode().getName());
    }
}

bool SniffCallCache::isInCache(TR_LinkHead<SniffCallCache> *sniffCacheList, TR_ResolvedMethod *vmMethod, bool isCold,
    int32_t &bytecodeSize)
{
    ...
    for (sniffCallCache = sniffCacheList->getFirst(); sniffCallCache; sniffCallCache = sniffCallCache->getNext()) {
        if (vmMethod->isSameMethod(sniffCallCache->_vmMethod) && isCold == sniffCallCache->_isCold) {
...
TR::SymbolReference *SymRefCache::findSymRef(TR_LinkHead<SymRefCache> *symRefList, TR_ResolvedMethod *resolvedMethod)
{
...
void TR_EscapeAnalysis::checkEscapeViaCall(TR::Node *node, TR::NodeChecklist &visited, bool &ignoreRecursion)
{
    ...
    if (methodSymbol && _methodSymbol && (_methodSymbol->getRecognizedMethod() == TR::java_math_BigDecimal_longString1C)
        && (methodSymbol->getRecognizedMethod() == TR::sun_misc_Unsafe_putInt_jlObjectII_V)
        && (methodSymbol->isNative())) {
        logprintf(trace(), log, "Ignoring escapes via call [%p] \n", node);
        return;
    }
    ...
    for (int32_t arg = firstArgIndex; arg < node->getNumChildren() && !_candidates.isEmpty(); arg++) {
        ...
        // be conservative in checking for method exit hook: if an argument is returned by the callee exit hook would
        // make it escape
        //
        if (TR::Compiler->vm.canMethodEnterEventBeHooked(comp())
        ...
        for (candidate = _candidates.getFirst(); candidate; candidate = candidate->getNext()) {
            if (usesValueNumber(candidate, nodeVN)) {
                ...
                //
                if (arg == firstArgIndex && methodSymbol
                    && (!node->getOpCode().isIndirect()
                        || !methodSymbol->getResolvedMethod()->virtualMethodIsOverridden())) {
                ...
                //
                if (1) {
                    TR::SymbolReference *finalizeSymRef = comp()->getSymRefTab()->findOrCreateRuntimeHelper(
                        TR_jitCheckIfFinalizeObject, true, true, true);
                    if (node->getSymbolReference() == finalizeSymRef) {
                        logprintf(trace(), log, "Candidate [%p] cannot escape because of node [%p]\n", candidate->_node,
                            node);
                        continue;
                    }
                }
                ...
                // this call
                //
                if (!node->getOpCode().isIndirect() && methodSymbol
                    && methodSymbol->getResolvedMethod()->isJNINative()) {
                    if (methodSymbol
                        && !comp()->fej9()->argumentCanEscapeMethodCall(methodSymbol, arg - firstArgIndex)) {
    ...
    if (needToSniff) {
        ...
        for (candidate = _candidates.getFirst(); candidate; candidate = next) {
            ...
            if (candidate->isArgToCall(_sniffDepth)) {
                if (!candidate->isNonThisArgToCall(_sniffDepth) && node->getOpCode().isIndirect()
                    && ((candidate->_node->getOpCodeValue() == TR::New)
                        || (candidate->_node->getOpCodeValue() == TR::newvalue))
                    && (thisVN > -1) && usesValueNumber(candidate, thisVN)) {
                    ...
                    TR_OpaqueClassBlock *originalMethodClass
                        = comp()->fej9()->getClassFromSignature(s, len, owningMethod);
                    TR_OpaqueClassBlock *thisType = (TR_OpaqueClassBlock *)candidate->_node->getFirstChild()
                                                        ->getSymbol()
                                                        ->castToStaticSymbol()
                                                        ->getStaticAddress();
                    int32_t offset = -1;
                    ...
                    if (resolvedMethodMustExist
                        ...
                        if (sym->isInterface() && originalMethodClass) {
                        ...
                        } else if (sym->isVirtual() && !symRef->isUnresolved()
                            && sym->getRecognizedMethod()
                                != TR::java_lang_Object_newInstancePrototype) // 166813: Temporary fix
                        {
                    ...
                    if (resolvedMethod) {
                        ...
                        sniffCallInCache
                            = SniffCallCache::isInCache(&sniffCacheList, resolvedMethod, _inColdBlock, bytecodeSize);
                        dumpOptDetails(comp(), "Refined sniff call %p called %s, cached=%s\n", node,
                            resolvedMethod->signature(trMemory()), sniffCallInCache ? "true" : "false");
                        if (!sniffCallInCache) {
                            ...
                            if (!newSymRef) {
                                newSymRef = getSymRefTab()->findOrCreateMethodSymbol(symRef->getOwningMethodIndex(), -1,
                                    resolvedMethod, TR::MethodSymbol::Virtual);
                            ...
                            ignoreRecursion = oldIgnoreRecursion;
                            bytecodeSize = sniffCall(node, newMethodSymbol->getResolvedMethodSymbol(), true,
                                _inColdBlock, ignoreRecursion);
                            SniffCallCache *sniffCallCache = new (trStackMemory())
                                SniffCallCache(newMethodSymbol->getResolvedMethodSymbol()->getResolvedMethod(),
                                    _inColdBlock, bytecodeSize);
                ...
                if ((bytecodeSize > 0)
                    && (!node->getOpCode().isIndirect() || !node->isTheVirtualCallNodeForAGuardedInlinedCall())
                    && (!symRef->isUnresolved() || !refinedSniff)) {
                    ...
                    if (!symRef->isUnresolved() || !refinedSniff) {
                        // The sniff was successful.
                        // If this is the top-level call site, remember that it
                        // references the candidate. If we cannot inline the call site
                        // then the candidate will have to be a contiguous allocation
                        ...
                        if (_sniffDepth == 0) {
                            ...
                            if (!refinedSniff && node->getOpCode().isIndirect()
                                && (methodSymbol && !methodSymbol->getResolvedMethod()->virtualMethodIsOverridden()))
                    ...
                    logprintf(trace(), log, "   Make [%p] contiguous because of call node [%p]\n", candidate->_node,
                        node);
                } else {
                    ...
                    if (candidate->forceLocalAllocation()) {
                        logprintf(trace(), log,
                            "   Normally [%p] would fail because child of call [%p] to %s, but user wants it locally "
                            "allocated\n",
                            candidate->_node, node,
                    ...
                    //
                    logprintf(trace(), log, "   Fail [%p] because child of call [%p]\n", candidate->_node, node);
...
int32_t TR_EscapeAnalysis::sniffCall(TR::Node *callNode, TR::ResolvedMethodSymbol *methodSymbol, bool ignoreOpCode,
    bool isCold, bool &ignoreRecursion)
{
    ...
    if (!ignoreOpCode
        && (callNode->getOpCode().isIndirect()
            && (methodSymbol->getResolvedMethod()->virtualMethodIsOverridden() || isCold || (_sniffDepth != 0)
                || (manager()->numPassesCompleted() == _maxPassNumber))))
        return 0;
    ...
    if (owningMethod->isSameMethod(calleeMethod) && (comp()->getJittedMethodSymbol() != owningMethodSymbol)) {
        if (ignoreRecursion)
            return bytecodeSize;
        else
    ...
    if (!methodSymbol->getFirstTreeTop()) {
        ...
        bool isPeekableCall
            = ((PersistentData *)manager()->getOptData())->_peekableCalls->get(callNode->getGlobalIndex());
        if (isPeekableCall)
            ilgenFailed = (NULL
                == methodSymbol->getResolvedMethod()->genMethodILForPeekingEvenUnderMethodRedefinition(methodSymbol,
                    comp()));
        else
         ...
         */
        if (ilgenFailed) {
            logprints(trace(), log, "   (IL generation failed)\n");
            static char *disableHCRCallPeeking = feGetEnv("TR_disableEAHCRCallPeeking");
            if (!isPeekableCall && disableHCRCallPeeking == NULL && comp()->getOSRMode() == TR::voluntaryOSR
                && comp()->getHCRMode() == TR::osr && !_candidates.isEmpty()
                && !((TR_EscapeAnalysis::PersistentData *)manager()->getOptData())
                        ->_processedCalls->get(callNode->getGlobalIndex())) {
                dumpOptDetails(comp(),
                    "%sAdding call [%p] n%dn to list of calls to protect for peeking to increase opportunities for "
                    "stack allocation\n",
        ...
        logprints(trace(), log, "   (trees already dumped)\n");
    }

    int32_t firstArgIndex = callNode->getFirstArgumentIndex();
    TR_Array<TR::Node *> *newParms = new (trStackMemory())
        TR_Array<TR::Node *>(trMemory(), callNode->getNumChildren() - firstArgIndex, false, stackAlloc);
    ...
    if (_removeMonitors) {
        ...
        if (inspector.checkMonitorStructure(methodSymbol->getFlowGraph())) {
            _removeMonitors = false;
            logprintf(trace(), log,
                "Disallowing monitor-removal because of strange monitor structure in sniffed method %s\n",
...
void TR_EscapeAnalysis::checkObjectSizes()
{
    ...
    for (candidate = _candidates.getFirst(); candidate; candidate = next) {
        ...
        if (candidate->isContiguousAllocation()) {
            if (candidate->_size > MAX_SIZE_FOR_ONE_CONTIGUOUS_OBJECT) {
                logprintf(trace(), log, "   Fail [%p] because object size %d is too big\n", candidate->_node,
                    candidate->_size);
    ...
    while (totalSize > MAX_SIZE_FOR_ALL_OBJECTS) {
        ...
        logprintf(trace(), log, "   Fail [%p] because total object size is too big\n", candidate->_node);
...
void TR_EscapeAnalysis::anchorCandidateReference(Candidate *candidate, TR::Node *reference)
{
    ...
    // the current treetop.
    //
    if (reference->getReferenceCount() > 1 && _curTree->getNextTreeTop()->getNode()->getOpCodeValue() != TR::BBEnd
...
bool TR_EscapeAnalysis::fixupNode(TR::Node *node, TR::Node *parent, TR::NodeChecklist &visited)
{
    ...
    if (node->getOpCode().isIndirect() && node->getOpCode().isLoadVarOrStore()
        ...
        for (candidate = _candidates.getFirst(); candidate; candidate = candidate->getNext()) {
            ...
            if (usesValueNum
                ...
                && node->getSymbolReference()->getSymbol()->isClassObject() && node->getOpCode().isLoadIndirect()
                && isFinalizableInlineTest(comp(), node->getFirstChild(), _curTree->getNode(), node)) {
                // This transformation is optional for candidates that can't be stack-allocated and contiguous
                // allocations, but required for discontiguous allocations because the class field will no longer exist
                if (!candidate->isLocalAllocation() || candidate->isContiguousAllocation()) {
                    ...
                    logprintf(trace(), log, "Removing inline finalizable test [%p] for discontiguous candidate [%p]\n",
            ...
            if (candidate->isLocalAllocation() && usesValueNum) {
                ...
                for (j = candidate->_fields->size() - 1; j >= 0; j--) {
                    if ((candidate->_fields->element(j)._offset == fieldOffset)
                        && (!candidate->_fields->element(j).symRefIsForFieldInAllocatedClass(
                        ...
                        if (mustRemoveDereferences) {
                            ...
                            if (trace()) {
                                logprintf(trace(), log,
                                    "Change illegal deref %s [%p] to use dummy base %s [%p] in place of %s [%p]\n",
                            ...
                            if (node->getOpCode().isWrtBar() && !disableWrtbarFixing) {
                                logprintf(trace(), log, " -> Change %s [%p] into astorei, removing child %s [%p]\n",
                                    node->getOpCode().getName(), node, node->getChild(2)->getOpCode().getName(),
                                    node->getChild(2));
                                ...
                                if (parent->getOpCode().isCheck() || parent->getOpCodeValue() == TR::compressedRefs) {
                                    logprintf(trace(), log, " -> Eliminate %s [%p]\n", parent->getOpCode().getName(),
                                        parent);
                ...
                if (fieldIsPresentInObject) {
                    ...
                    if (candidate->escapesInColdBlock(_curBlock)
                        ...
                        if (TR_yes == candidateHasField(candidate, node, fieldOffset, this)) {
                            ...
                            if (fieldOffset + nodeSize <= candidate->_size) // cmvc 200318: redundant but harmless --
                                                                            // leave in place to minimize disruption
                            {
                                for (i = candidate->_fields->size() - 1; i >= 0; i--) {
    ...
    if (node->getOpCode().isCall()) {
        ...
        if (calledMethod
            && (!calledMethod->getResolvedMethod()->virtualMethodIsOverridden() || !node->getOpCode().isIndirect())) {
            if (calledMethod->getRecognizedMethod() == TR::java_lang_Throwable_fillInStackTrace) {
                ...
                if (candidate && candidate->fillsInStackTrace() && !candidate->usesStackTrace()
                    && candidate->isLocalAllocation() && !candidate->escapesInColdBlocks()
                    && performTransformation(comp(), "%sRemoving call node [%p] to fillInStackTrace\n", OPT_DETAILS,
                        node)) {
    ...
    //
        ...
        if (candidate
            && performTransformation(comp(), "%sReplacing arraylength [%p] by constant %d\n", OPT_DETAILS, node,
                candidate->_node->getFirstChild()->getInt())) {
            // TR_ASSERT((candidate->_node->getOpCodeValue() != TR::New) && (candidate->_node->getOpCodeValue() !=
            // TR::newStructRef), "assertion failure");
            anchorCandidateReference(candidate, child);
            if (optimizer()->prepareForNodeRemoval(node, /* deferInvalidatingUseDefInfo = */ true))
                _invalidateUseDefInfo = true;
            node->removeAllChildren();
            TR::Node::recreate(node, TR::iconst);
            if (candidate->_node->getOpCodeValue() == TR::New || candidate->_node->getOpCodeValue() == TR::newvalue) {
    ...
    else if (node->getOpCode().isCheckCast() || node->getOpCodeValue() == TR:: instanceof) {
        ...
        if (candidate && isConstantClass(classNode, this)) {
            ...
            if (optimizer()->prepareForNodeRemoval(node, /* deferInvalidatingUseDefInfo = */ true))
                _invalidateUseDefInfo = true;
            if (comp()->fej9()->isInstanceOf((TR_OpaqueClassBlock *)candidate->_class,
                    (TR_OpaqueClassBlock *)classNode->getSymbol()->castToStaticSymbol()->getStaticAddress(), true)
                == TR_yes) {
                ...
                if (node->getOpCodeValue() == TR:: instanceof) {
                    ...
                    //
                    TR_ASSERT(!candidate->isLocalAllocation() || candidate->isContiguousAllocation()
        ...
        if (firstValue == secondValue) {
            ...
            candidate = findCandidate(firstValue);
            if (candidate
                && ((!candidate->_seenSelfStore && !candidate->_seenStoreToLocalObject
                        && !candidate->escapesInColdBlocks() && !usesValueNumber(candidate, secondValue))
                ...
                if (candidate
                    && ((!candidate->_seenSelfStore && !candidate->_seenStoreToLocalObject
                            && !candidate->escapesInColdBlocks() && !usesValueNumber(candidate, firstValue))
                        || (node->getSecondChild()->getOpCodeValue() == TR::aconst)))
                    notEqual = true;
            }
            if (notEqual)
        ...
        if (compareValue >= 0
            && performTransformation(comp(),
                "%sChanging compare node [%p] so that constant propagation can predict it\n", OPT_DETAILS, node)) {
    ...
    else if (comp()->useCompressedPointers() && node->getOpCodeValue() == TR::compressedRefs) {
        TR::Node *loadOrStore = node->getFirstChild();
        bool treeAsExpected = false;
        if (loadOrStore && (loadOrStore->getOpCode().isLoadVarOrStore() || loadOrStore->getOpCode().isLoadConst()))
            treeAsExpected = true;

        if (!treeAsExpected
            || (loadOrStore->getOpCode().hasSymbolReference()
                && loadOrStore->getSymbolReference()->getSymbol()->isAuto())) {
            TR::Node::recreate(node, TR::treetop);
            ...
            if (loadOrStore->getOpCode().isStore()) {
                ...
                logprintf(trace(), log,
                    "Changing orphaned compressed ref anchor [%p] to auto to a treetop and removing store [%p].\n",
                    node, loadOrStore);
            } else {
                logprintf(trace(), log, "Changing orphaned compressed ref anchor [%p] to auto to a treetop.\n", node);
            }
            return false;
        }
    }
...
#if CHECK_MONITORS
    if (_removeMonitors && (node->getOpCodeValue() == TR::monent || node->getOpCodeValue() == TR::monexit))
    ...
    if (synchronizedObject) {
        ...
        candidate = findCandidate(valueNumber);
        if (candidate && !candidate->escapesInColdBlocks()) {
            if (calledMethod) {
                if (_desynchronizeCalls) {
                    logprintf(trace(), log, "Mark call node [%p] as desynchronized\n", node);
...
#if CHECK_MONITORS
                logprintf(trace(), log, "Remove redundant monitor node [%p]\n", node);
                removeThisNode = true;
#else
                if (node->getOpCodeValue() == TR::allocationFence) {
                    if (candidate->isLocalAllocation()) {
                        logprintf(trace(), log,
                            "Redundant flush node [%p] found for candidate [%p]! Set omitSync flag on redundant flush "
                            "node.\n",
                    ...
                    logprintf(trace(), log, "Mark monitor node [%p] for candidate [%p] as local object monitor\n", node,
                        candidate->_node);
                }
#endif
            }
        ...
        else if (candidate && candidate->escapesInColdBlocks() && node->getOpCodeValue() == TR::allocationFence
            && candidate->isLocalAllocation()) {
            ListIterator<TR_ColdBlockEscapeInfo> coldBlkInfoIt(candidate->getColdBlockEscapeInfo());
            TR_ColdBlockEscapeInfo *info;
            for (info = coldBlkInfoIt.getFirst(); info != NULL; info = coldBlkInfoIt.getNext()) {
                ...
                if (!hasFlushOnEntry(coldBlk->getNumber())) {
                    ...
                    logprintf(trace(), log, "Adding flush node %p for candidate %p to cold block_%d\n", flush,
                        candidate->_node, coldBlk->getNumber());
                    setHasFlushOnEntry(coldBlk->getNumber());
                }
            }

            logprintf(trace(), log, "Remove redundant flush node [%p] for candidate [%p]\n", node, candidate->_node);
            removeThisNode = true;
        }
    }
    ...
    if (candidate && candidate->isLocalAllocation() && !candidate->isContiguousAllocation() &&
        // shouldRemoveTree(candidate, _curBlock) &&
        !candidate->objectIsReferenced() && !comp()->suppressAllocationInlining()) {
        ...
        if (_curTree != candidate->_treeTop) {
            ...
            if (_curTree->getNode()->getOpCode().isStore()
                && (_curTree->getNode()->getSymbol()->holdsMonitoredObject()
                    || (_curTree->getNode()->getSymbolReference()
                ...
                logprintf(trace(), log, "%sFixed up liveMonitor store [%p] for candidate [%p]\n", OPT_DETAILS,
                    _curTree->getNode(), candidate->_node);
                removeThisNode = false;
            } else {
                removeThisNode = true;

                logprintf(trace(), log, "Remove tree [%p] with direct reference to candidate [%p]\n",
                    _curTree->getNode(), candidate->_node);
            }
        }
...
bool TR_EscapeAnalysis::fixupFieldAccessForContiguousAllocation(TR::Node *node, Candidate *candidate)
{
    ...
    // later if we can).
    //
    if (candidate->isExplicitlyInitialized() && node->getSymbol() == getSymRefTab()->findGenericIntShadowSymbol()) {
        return false;
    }
    ...
    //
    if (node->getOpCode().isWrtBar() && !candidate->escapesInColdBlocks()
        && _valueNumberInfo->getValueNumber(node->getFirstChild())
            == _valueNumberInfo->getValueNumber(candidate->_node)) {
        if (candidate->_origKind == TR::New) {
            ...
            node->setNumChildren(2);
            _repeatAnalysis = true;
            logprintf(trace(), comp()->log(), "Change node [%p] from write barrier to regular store\n", node);
        } else {
    ...
    if (TR_yes == candidateHasField(candidate, node, fieldOffset, this)) {
        ...
        if (fieldOffset + nodeSize
            <= candidate->_size) // cmvc 200318: redundant but harmless -- leave in place to minimize disruption
        {
            for (i = candidate->_fields->size() - 1; i >= 0; i--) {
                if (candidate->_fields->element(i)._offset == fieldOffset) {
                    candidate->_fields->element(i).rememberFieldSymRef(node, candidate, this);
            ...
            TR_ASSERT(i >= 0,
                "candidate [%p] must have field (%s) at offset %d due to %p.  Scan for escapes missed a field "
                "reference!\n",
...
static TR::DataType convertArrayTypeToDataType[] = { TR::NoType, // 0
    TR::NoType, // 1
    TR::NoType, // 2
    TR::NoType, // 3
    TR::Int8, // 4
...
bool TR_EscapeAnalysis::fixupFieldAccessForNonContiguousAllocation(TR::Node *node, Candidate *candidate,
    TR::Node *parent)
{
    OMR::Logger *log = comp()->log();
    int32_t i;
    ...
    //
    if (candidate->isExplicitlyInitialized() && node->getOpCode().isStore()
        ...
        logprintf(trace(), log, "Remove explicit new initialization node [%p]\n", node);
        return true;
    }
    ...
    if (i >= 0) {
        ...
        if (node->getOpCode().isLoadVar()) {
            ...
            if (autoSymRefDataType.isVector() && !node->getDataType().isVector()) {
                TR::Node::recreate(node, TR::ILOpCode::createVectorOpCode(TR::vgetelem, autoSymRefDataType));
                node->setAndIncChild(0,
                    TR::Node::create(node, TR::ILOpCode::createVectorOpCode(TR::vload, autoSymRefDataType), 0));
                node->setNumChildren(2);
            ...
            // completely
            if (candidate->escapesInColdBlock(_curBlock) && isImmutableObject(candidate)
                && !candidate->isContiguousAllocation()) {
                ...
                logprintf(trace(), log,
                    "Preserve old node [%p] for store to non-contiguous immutable candidate [%p] that escapes in cold "
                    "block; create new tree for for direct store [%p]\n",
            ...
            if (autoSymRefDataType.isVector() && !node->getDataType().isVector()) {
                ...
                TR::Node *newValue
                    = TR::Node::create(node, TR::ILOpCode::createVectorOpCode(TR::vsetelem, autoSymRefDataType), 3);
                newValue->setAndIncChild(0,
                    TR::Node::create(node, TR::ILOpCode::createVectorOpCode(TR::vload, autoSymRefDataType), 0));
        ...
        logprintf(trace(), log, "Change node [%p] into a direct load or store of #%d (%d bytes) field %d cand %p\n",
        ...
        if (parent) {
            if (parent->getOpCode().isNullCheck())
                ...
                while (i < parent->getNumChildren()) {
                    TR::TreeTop *tt
                        = TR::TreeTop::create(comp(), TR::Node::create(TR::treetop, 1, parent->getChild(i)));
                ...
                TR::Node *checkNode = TR::Node::createWithSymRef(TR::checkcast, 2, 2, source, typeNode,
        ...
        if (node->getOpCode().isStore())
            ...
            logprintf(trace(), log, "Change node [%p] into a constant\n", node);
        }
    }
...
void TR_EscapeAnalysis::makeLocalObject(Candidate *candidate)
{
    ...
    if (candidate->_kind == TR::New || candidate->_kind == TR::newvalue) {
        symRef = getSymRefTab()->createLocalObject(candidate->_size, comp()->getMethodSymbol(),
            allocationNode->getFirstChild()->getSymbolReference());

#if LOCAL_OBJECTS_COLLECTABLE
        if (candidate->isContiguousAllocation())
            referenceSlots = comp()->fej9()->getReferenceSlotsInClass(comp(),
                (TR_OpaqueClassBlock *)candidate->_node->getFirstChild()
                    ->getSymbol()
                    ->getStaticSymbol()
                    ->getStaticAddress());
        ...
        symRef = getSymRefTab()->createLocalAddrArray(candidate->_size, comp()->getMethodSymbol(),
            allocationNode->getSecondChild()->getSymbolReference());
        ...
        symRef = getSymRefTab()->createLocalPrimArray(candidate->_size, comp()->getMethodSymbol(),
            allocationNode->getSecondChild()->getInt());
        symRef->setStackAllocatedArrayAccess();
    }
    ...
    const char *dbgCntName
        = TR::DebugCounter::debugCounterName(comp(), "escapeAnalysis/contiguous-allocation/%s/%s/(%s)/(%d,%d)",
    ...
    if (candidate->_seenArrayCopy || candidate->_argToCall || candidate->_seenSelfStore
        || candidate->_seenStoreToLocalObject) {
    ...
    if (nodeToUseInInit != allocationNode) {
        ...
        if (candidate->escapesInColdBlocks() || candidate->_seenArrayCopy || candidate->_argToCall
            || candidate->_seenSelfStore || candidate->_seenStoreToLocalObject) {
            if (candidate->escapesInColdBlocks())
...
void TR_EscapeAnalysis::avoidStringCopyAllocation(Candidate *candidate)
{
    ...
    logprintf(trace(), comp()->log(), "secs (%d) String (copy) allocation of size %d found in %s\n",
    ...
    TR::Node *initNode
        = TR::Node::createWithSymRef(comp()->il.opCodeForDirectStore(candidate->_stringCopyNode->getDataType()), 1, 1,
            candidate->_stringCopyNode, newSymbolReference);
...
bool TR_EscapeAnalysis::tryToZeroInitializeUsingArrayset(Candidate *candidate, TR::TreeTop *precedingTreeTop)
{
    ...
    if (cg()->getSupportsArraySet() && candidate->_kind != TR::newvalue) {
        int32_t candidateHeaderSizeInBytes = candidate->_origKind == TR::New
            ? comp()->fej9()->getObjectHeaderSizeInBytes()
            : TR::Compiler->om.contiguousArrayHeaderSizeInBytes();
        ...
        if (candidateObjectSizeInBytes > 0) {
            ...
            if (performTransformation(comp(), "%sUse arrayset to initialize [%p]\n", OPT_DETAILS, allocationNode)) {
                ...
                TR::Node *arrayset = TR::Node::createWithSymRef(TR::arrayset, 3, 3,
                    TR::Node::createWithSymRef(allocationNode, TR::loadaddr, 0,
                        new (trHeapMemory()) TR::SymbolReference(comp()->getSymRefTab(), allocationSymRef->getSymbol(),
                            allocationSymRef->getOffset() + candidateHeaderSizeInBytes)),
                    TR::Node::bconst(allocationNode, 0), TR::Node::iconst(allocationNode, candidateObjectSizeInBytes),
                    comp()->getSymRefTab()->findOrCreateArraySetSymbol());

                TR::TreeTop *arraysetTreeTop
                    = TR::TreeTop::create(comp(), precedingTreeTop, TR::Node::create(TR::treetop, 1, arrayset));

                TR::DebugCounter::prependDebugCounter(comp(),
                    TR::DebugCounter::debugCounterName(comp(), "escapeAnalysis/zeroInitializeArrayset/%s",
                        comp()->signature()),
                    arraysetTreeTop);
...
void TR_EscapeAnalysis::makeNewValueLocalAllocation(Candidate *candidate)
{
    ...
    //
    TR_OpaqueClassBlock *valueClass = static_cast<TR_OpaqueClassBlock *>(
        candidate->_node->getFirstChild()->getSymbol()->getStaticSymbol()->getStaticAddress());
    ...
    for (int i = 1; i < candidate->_node->getNumChildren(); i++) {
        ...
        TR::SymbolReference *symref = comp()->getSymRefTab()->findOrFabricateShadowSymbol(valueClass,
            fieldEntry._datatype, fieldEntry._offset, fieldEntry._isVolatile, fieldEntry._isPrivate,
            fieldEntry._isFinal, fieldEntry._fieldname, fieldEntry._typeSignature);
        ...
        FieldInfo &field
            = candidate->findOrSetFieldInfo(NULL, symref, fieldEntry._offset, fieldSize, fieldEntry._datatype, this);
        ...
        if (candidate->isContiguousAllocation()) {
            ...
            const bool useWriteBarrier
                = ((fieldDataType == TR::Address) && (TR::Compiler->om.writeBarrierType() != gc_modron_wrtbar_none))
                || comp()->getOption(TR_EnableFieldWatch);
            TR::Node *storeNode;

            if (useWriteBarrier) {
                const TR::ILOpCodes storeOpCode
                    = comp()->il.opCodeForIndirectWriteBarrier(fieldValueNode->getDataType());
                storeNode = TR::Node::createWithSymRef(storeOpCode, 3, 3, candidate->_node, fieldValueNode,
                    candidate->_node, symref);
            } else {
            ...
            completeFieldStoreNode
                = TR::Node::createWithSymRef(comp()->il.opCodeForDirectStore(type), 1, 1, fieldValueNode, autoSymRef);
        }
...
void TR_EscapeAnalysis::makeContiguousLocalAllocation(Candidate *candidate)
{
    ...
    dumpOptDetails(comp(), "%sMaking %s node [%p] into a local object of size %d\n", OPT_DETAILS,
        candidate->_node->getOpCode().getName(), candidate->_node, candidate->_size);

    logprintf(trace(), comp()->log(), "secs (%d) Contiguous allocation of size %d found in %s\n",
        manager()->numPassesCompleted(), candidate->_size, comp()->signature());
    ...
    if (candidate->isExplicitlyInitialized()) {
            ...
            for (initTree = candidate->_treeTop->getNextTreeTop(); initTree; initTree = next) {
                ...
                if (candidate->_fields && (candidate->_origKind == TR::New || candidate->_origKind == TR::newvalue)) {
                    for (i = candidate->_fields->size() - 1; i >= 0; i--) {
                        ...
                        if (field._symRef && offset == node->getSymbolReference()->getOffset()) {
        ...
        if (candidate->_kind == TR::newarray) {
            // TODO (Task 118458): We need to investigate the effect of using arrayset on small number of elements. This
            // involves verifying the instruction sequences each individual codegen will generate and only if the
            // codegens can handle arraysets of small number of elements in a performant manner can we enable this
            // optimization.
            ...
            if (enableNewArrayArraySetPendingInvestigation && tryToZeroInitializeUsingArrayset(candidate, initTree)) {
                TR::DebugCounter::prependDebugCounter(comp(),
                    TR::DebugCounter::debugCounterName(comp(), "escapeAnalysis/zeroInitializeArrayset/%s/primitive",
                        comp()->signature()),
                    initTree->getNextTreeTop());
        ...
        while (i < candidate->_size) {
            if (!candidate->isInsideALoop() && referenceSlots
                && i == referenceSlots[refSlotIndex] * TR::Compiler->om.sizeofReferenceField()) {
                refSlotIndex++;
                i += refIncrVal;
                continue;
            }
            ...
            if (candidate->_fields && (candidate->_origKind == TR::New || candidate->_origKind == TR::newvalue)) {
                for (j = candidate->_fields->size() - 1; j >= 0; j--) {
                    ...
                    if (field._offset == i && field._symRef) {
                        ...
                        node = TR::Node::createWithSymRef(comp()->il.opCodeForIndirectStore(type), 2, 2, allocationNode,
                            node, field._symRef);
    ...
    if (referenceSlots) {
        ...
        if (tryToZeroInitializeUsingArrayset(candidate, initTree)) {
            TR::DebugCounter::prependDebugCounter(comp(),
                TR::DebugCounter::debugCounterName(comp(), "escapeAnalysis/zeroInitializeArrayset/%s/reference",
                    comp()->signature()),
                initTree->getNextTreeTop());
            ...
            if (candidate->_fields && (candidate->_origKind == TR::New || candidate->_origKind == TR::newvalue)) {
                for (j = candidate->_fields->size() - 1; j >= 0; j--) {
                    ...
                    if (field._offset == offset && field._symRef) {
                        TR::DataType type = field._symRef->getSymbol()->getDataType();
                        node = createConst(comp(), allocationNode, type, 0);
                        node = TR::Node::createWithSymRef(comp()->il.opCodeForIndirectStore(type), 2, 2, baseNode, node,
                            field._symRef);
                        if (comp()->useCompressedPointers())
                            initTree
                                = TR::TreeTop::create(comp(), initTree, TR::Node::createCompressedRefsAnchor(node));
                        else
                            initTree = TR::TreeTop::create(comp(), initTree, node);
                        break;
                    }
...
void TR_EscapeAnalysis::makeNonContiguousLocalAllocation(Candidate *candidate)
{
    ...
    logprintf(trace(), comp()->log(), "Pass: (%d) Non-contiguous allocation found in %s\n",
    ...
    if (candidate->objectIsReferenced()) {
        if ((candidate->_kind == TR::New) || (candidate->_kind == TR::newvalue)) {
            ...
            TR::ResolvedMethodSymbol *owningMethodSymbol
                = candidate->_node->getSymbolReference()->getOwningMethodSymbol(comp());
            ...
            TR::SymbolReference *classSymRef
                = getSymRefTab()->findOrCreateClassSymbol(owningMethodSymbol, -1, classObject, false);
            ...
            candidate->_size
                = comp()->fej9()->getObjectHeaderSizeInBytes() + TR::Compiler->cls.classInstanceSize(classObject);
            ...
            TR_ASSERT(sizeChild->getOpCodeValue() == TR::iconst,
                "The size of non-contiguous stack allocated array object should be constant\n");
        ...
        const char *dbgCntName
            = TR::DebugCounter::debugCounterName(comp(), "escapeAnalysis/noncontiguous-allocation/%s/%s/(%s)/(%d,%d)",
                comp()->getHotnessName(comp()->getMethodHotness()), comp()->signature(),
                (bcInfo.getCallerIndex() > -1)
                    ? comp()->getInlinedResolvedMethod(bcInfo.getCallerIndex())->signature(trMemory())
                    : comp()->signature(),
...
void TR_EscapeAnalysis::heapifyForColdBlocks(Candidate *candidate)
{
    ...
    logprintf(trace(), log, "Found candidate allocated with cold block compensation in %s numBlocks compensated = %d\n",
    ...
    //
    TR::Node *initStoreNode
        = TR::Node::createWithSymRef(TR::astore, 1, 1, TR::Node::aconst(candidate->_node, 0), heapSymRef);
    ...
    for (info = coldBlockInfoIt.getFirst(); info != NULL; info = coldBlockInfoIt.getNext()) {
        ...
        TR::Node *heapAllocation
            = TR::Node::create(TR::treetop, 1, candidate->_originalAllocationNode->duplicateTree());
        heapAllocation->getFirstChild()->setHeapificationAlloc(true);

        logprintf(trace(), log,
            "heapifying %p:  isContiguousAllocation %d; _dememoizedMethodSymRef %d; new heapAllocation treetop %p\n",
        ...
        if (!candidate->isContiguousAllocation() && _dememoizedAllocs.find(candidate->_node)) {
            heapAllocation->getFirstChild()
                ->getFirstChild()
                ->recursivelyDecReferenceCount(); // remove loadaddr of class
            ...
            if (autoSymRefForValue)
                stackFieldLoad = TR::Node::createWithSymRef(heapAllocation,
                    comp()->il.opCodeForDirectLoad(autoSymRefForValue->getSymbol()->getDataType()), 0,
                    autoSymRefForValue);
            else
        ...
        TR::Block *heapAllocationBlock
            = toBlock(cfg->addNode(TR::Block::createEmptyBlock(heapAllocation, comp(), coldBlock->getFrequency())));
        ...
        //
        TR::Node *tempLoad
            = TR::Node::createWithSymRef(heapAllocation, comp()->il.opCodeForDirectLoad(TR::Address), 0, heapSymRef);
        TR::Node *heapComparisonNode
            = TR::Node::createif(TR::ifacmpne, tempLoad, candidate->_node->duplicateTree(), NULL);
        TR::TreeTop *heapComparisonTree = TR::TreeTop::create(comp(), heapComparisonNode, NULL, NULL);
        TR::Block *heapComparisonBlock
            = toBlock(cfg->addNode(TR::Block::createEmptyBlock(heapComparisonNode, comp(), coldBlock->getFrequency())));
        ...
        const char *dbgCntName
            = TR::DebugCounter::debugCounterName(comp(), "escapeAnalysis/heapification/%s/%s/(%s)/(%d,%d)",
                comp()->getHotnessName(comp()->getMethodHotness()), comp()->signature(),
                (bcInfo.getCallerIndex() > -1)
                    ? comp()->getInlinedResolvedMethod(bcInfo.getCallerIndex())->signature(trMemory())
                    : comp()->signature(),
        ...
        if (candidate->_origKind != TR::newvalue) {
            ...
            while (i < size) {
                ...
                if (candidate->_fields) {
                    ...
                    for (j = candidate->_fields->size() - 1; j >= 0; j--) {
                        ...
                        if (field._offset == i && field._symRef
                            ...
                            if (!candidate->isContiguousAllocation())
                                stackFieldLoad = TR::Node::createWithSymRef(heapAllocation,
                                    comp()->il.opCodeForDirectLoad(type), 0, field._symRef);
                            else
                                stackFieldLoad = TR::Node::createWithSymRef(comp()->il.opCodeForIndirectLoad(type), 1,
                                    1, stackAllocation, field._symRef);

                            TR::Node *heapFieldStore = NULL;
                            TR::TreeTop *translateTT = NULL;
                            if (stackFieldLoad->getDataType() == TR::Address) {
                                heapFieldStore
                                    = TR::Node::createWithSymRef(TR::awrtbari, 3, 3, heapAllocation->getFirstChild(),
                                        stackFieldLoad, heapAllocation->getFirstChild(), field.fieldSymRef());
                                if (comp()->useCompressedPointers()) {
                                    translateTT = TR::TreeTop::create(comp(),
                                        TR::Node::createCompressedRefsAnchor(heapFieldStore), NULL, NULL);
                                }
                            } else
                                heapFieldStore = TR::Node::createWithSymRef(comp()->il.opCodeForIndirectStore(type), 2,
                                    2, heapAllocation->getFirstChild(), stackFieldLoad, field.fieldSymRef());
                ...
                //                getSymRefTab()->findOrCreateGenericIntShadowSymbolReference(i);
                ...
                //                TR::Node *heapFieldStore = TR::Node::create(TR::astorei, 2,
                //                heapAllocation->getFirstChild(), stackFieldLoad, intShadow); TR::TreeTop
                //                *heapFieldStoreTree = TR::TreeTop::create(comp(), heapFieldStore, NULL, NULL);
                //                treeBeforeInsertionPoint->join(heapFieldStoreTree);
                //                heapFieldStoreTree->join(heapAllocationExitTree);
                //                treeBeforeInsertionPoint = heapFieldStoreTree;
                ...
                TR::Node *heapFieldStore = TR::Node::createWithSymRef(TR::istorei, 2, 2,
                    heapAllocation->getFirstChild(), stackFieldLoad, intShadow);
        ...
        } else // if (candidate->_origKind == TR::newvalue)
        {
            ...
            TR_ASSERT_FATAL(candidate->_fields != NULL || fieldCount == 0,
                "candidate %p of kind newvalue is expected to have non-NULL candidate->_fields if it has a non-zero "
                "fieldCount %d\n",
            ...
            logprintf(trace(), log, "Updating heap allocation node %p with fieldCount == %d\n",
            ...
            for (size_t idx = 0; idx < fieldCount; idx++) {
                ...
                for (j = candidate->_fields->size() - 1; j >= 0; j--) {
                    ...
                    if (field._offset == fieldEntry._offset && field._symRef
                        && (candidate->isContiguousAllocation() || field._symRef->getSymbol()->isAuto())) {
                        TR::DataType type = field._symRef->getSymbol()->getDataType();

                        if (!candidate->isContiguousAllocation()) {
                            stackFieldLoad = TR::Node::createWithSymRef(newValueHeapificationNode,
                                comp()->il.opCodeForDirectLoad(type), 0, field._symRef);
                        } else {
                            stackFieldLoad = TR::Node::createWithSymRef(comp()->il.opCodeForIndirectLoad(type), 1, 1,
                                stackAllocation, field._symRef);
                        }
                        break;
                    }
                }

                TR_ASSERT_FATAL(stackFieldLoad != NULL,
                    "newvalue candidate %p was missing an entry in candidate->_fields for its field number %d at "
                    "offset %d",
                    candidate->_node, idx, fieldEntry._offset);
                newValueHeapificationNode->setAndIncChild(idx + 1, stackFieldLoad);
                logprintf(trace(), log, "Updating heap allocation node %p child %d with field load node %p\n",
        ...
        for (escapeNode = nodesIt.getFirst(); escapeNode != NULL;
             escapeNode = nodesIt.getNext(), escapeTree = treesIt.getNext()) {
                ...
                while (i < size) {
                    ...
                    if (candidate->_fields) {
                         ...
                         field._size == TR::Symbol::convertTypeToSize(TR_SInt32)*/)
                        {
                                ...
                                TR::Node *heapFieldLoad = TR::Node::createWithSymRef(
                                    comp()->il.opCodeForIndirectLoad(type), 1, 1, escapeNode, field.fieldSymRef());
                                TR::TreeTop *translateTT = NULL;
                                if (comp()->useCompressedPointers() && (type == TR::Address)) {
                                    translateTT = TR::TreeTop::create(comp(),
                                        TR::Node::createCompressedRefsAnchor(heapFieldLoad), NULL, NULL);
                                }
                                TR::Node *stackStore = TR::Node::createWithSymRef(comp()->il.opCodeForDirectStore(type),
                                    1, 1, heapFieldLoad, field._symRef);
                                TR::TreeTop *stackStoreTree = TR::TreeTop::create(comp(), stackStore, NULL, NULL);
                                logprintf(trace(), log, "Emitting stack store back %p cold %p next %p\n", stackStore,
                                    coldBlockTree->getNode(), nextTreeInColdBlock->getNode());
        ...
        for (symRef = symRefsIt.getFirst(); symRef; symRef = symRefsIt.getNext()) {
            ...
            if (useSelectOp) {
                ...
                TR::Node *addrCompareNode
                    = TR::Node::create(candidate->_node, TR::acmpeq, 2, symLoad, candidateStackAddrLoad);
                ...
                TR::Node *comparisonNode = TR::Node::createif(TR::ifacmpne,
                    TR::Node::createWithSymRef(candidate->_node, TR::aload, 0, symRef),
                    candidate->_node->duplicateTree(), targetBlock->getEntry());
                TR::TreeTop *comparisonTree = TR::TreeTop::create(comp(), comparisonNode, NULL, NULL);
                TR::Block *comparisonBlock = toBlock(
                    cfg->addNode(TR::Block::createEmptyBlock(comparisonNode, comp(), coldBlock->getFrequency())));
                ...
                TR::Block *storeBlock = toBlock(
                    cfg->addNode(TR::Block::createEmptyBlock(storeTree->getNode(), comp(), coldBlock->getFrequency())));
        ...
        for (auto pred = coldBlock->getPredecessors().begin(); pred != coldBlock->getPredecessors().end();) {
            ...
            pred++;
            if ((useSelectOp && (predNode != heapComparisonBlock) && (predNode != heapAllocationBlock))
                || (!useSelectOp && (predNode != lastComparisonBlock) && (predNode != lastStoreBlock))
                ...
                if (!coldBlock->isCatchBlock()
                    ...
                    if (lastNode->getOpCode().isBranch() && (lastNode->getBranchDestination() == coldBlock->getEntry()))
                        predBlock->changeBranchDestination(heapComparisonBlock->getEntry(), cfg);
                    else {
                        if (lastNode->getOpCode().isSwitch())
                            lastTree->adjustBranchOrSwitchTreeTop(comp(), coldBlock->getEntry(),
                                heapComparisonBlock->getEntry());
        ...
        if (coldBlock->isCatchBlock()) {
            ...
            for (auto succ = coldBlock->getExceptionSuccessors().begin();
                 succ != coldBlock->getExceptionSuccessors().end(); ++succ)
                coldExceptionSuccessors.add(*succ);

            for (auto succ = heapComparisonBlock->getSuccessors().begin();
                 succ != heapComparisonBlock->getSuccessors().end();) {
                if (!coldBlock->hasSuccessor((*succ)->getTo()))
                    cfg->addEdge(coldBlock, (*succ)->getTo());
                cfg->removeEdge(heapComparisonBlock, (*(succ++))->getTo());
            }

            for (auto succ = heapComparisonBlock->getExceptionSuccessors().begin();
                 succ != heapComparisonBlock->getExceptionSuccessors().end();) {
...
TR::TreeTop *TR_EscapeAnalysis::storeHeapifiedToTemp(Candidate *candidate, TR::Node *value, TR::SymbolReference *symRef)
{
    ...
    if (!symRef->getSymbol()->isParm() && !_initializedHeapifiedTemps->get(symRef->getReferenceNumber())) {
        TR::Node *initStoreNode
            = TR::Node::createWithSymRef(TR::astore, 1, 1, TR::Node::aconst(candidate->_node, 0), symRef);
        if (symRef->getSymbol()->holdsMonitoredObject())
...
bool TR_EscapeAnalysis::devirtualizeCallSites()
{
    ...
    while (!_devirtualizedCallSites.isEmpty()) {
        ...
        if (calledMethod
            && (!(calledMethod->getResolvedMethod()->virtualMethodIsOverridden())
                && callNode->getOpCode().isIndirect())) {
            ...
            TR::Node *guardNode = TR_VirtualGuard::createNonoverriddenGuard(TR_NonoverriddenGuard, comp(),
                callNode->getByteCodeInfo().getCallerIndex(), callNode, NULL,
            ...
            TR::Block *remainder = block->createConditionalBlocksBeforeTree(callSite, compareTree, coldTree,
                directCallTree, comp()->getFlowGraph(), false);
            ...
            if (callSite->getNode()->getOpCode().hasSymbolReference())
                directCallTreeNode = TR::Node::createWithSymRef(callSite->getNode()->getOpCodeValue(), 1, 1, directCall,
                    callSite->getNode()->getSymbolReference());
            else
            ...
            if (callSite->getNode()->getOpCode().hasSymbolReference())
                coldTreeNode = TR::Node::createWithSymRef(callSite->getNode()->getOpCodeValue(), 1, 1, coldCall,
                    callSite->getNode()->getSymbolReference());
            else
...
bool TR_EscapeAnalysis::inlineCallSites()
{
    ...
    while (!_inlineCallSites.isEmpty()) {
        ...
        // The inliner might remove unreachable regions/blocks - check if the remaining calls to inline exist in the
        // remaining trees.
        ...
        if (tt == exitTree) {
            logprintf(trace(), log, "attempt to inline call %p failed because the block was removed\n",
...
void TR_EscapeAnalysis::scanForExtraCallsToInline()
{
    if (!_repeatAnalysis) {
        ...
        for (TR::TreeTop *tt = comp()->getStartTree(); tt; tt = tt->getNextTreeTop()) {
            if (tt->getNode()->getOpCodeValue() == TR::BBStart && tt->getNode()->getBlock()->isCold()) {
            ...
            const char *reason = "??";
            if (tt->getNode()->getNumChildren() >= 1 && tt->getNode()->getFirstChild()->getOpCode().isCall()
            ...
            if (callTreeToInline && !alreadyGoingToInline(callNode, _inlineCallSites)) {
                ...
                logprintf(trace(), comp()->log(), "Consider inlining %s n%dn [%p] of %s because %s\n",
                    callNode->getOpCode().getName(), callNode->getGlobalIndex(), callNode,
                    callNode->getSymbolReference()->getName(comp()->getDebug()), reason);
            }
        }
    }
}
...
FieldInfo &Candidate::findOrSetFieldInfo(TR::Node *fieldRefNode, TR::SymbolReference *symRef, int32_t fieldOffset,
    int32_t fieldSize, TR::DataType fieldStoreType, TR_EscapeAnalysis *ea)
{
    int32_t resultIdx = -1;
    ...
    for (int j = 0; j < N; j++) {
        ...
        if (i < 0) {
            ...
            (*this->_fields)[i]._goodFieldSymrefs
                = new (trStackMemory()) TR_ScratchList<TR::SymbolReference>(trMemory());
            (*this->_fields)[i]._badFieldSymrefs
                = new (trStackMemory()) TR_ScratchList<TR::SymbolReference>(trMemory());
        }
        if (resultIdx < 0) {
            resultIdx = i;
...
static void printSymRefList(TR_ScratchList<TR::SymbolReference> *list, TR::Compilation *comp)
{
    ...
    for (TR::SymbolReference *symRef = iter.getFirst(); symRef; symRef = iter.getNext()) {
        log->printf("%s#%d", sep, symRef->getReferenceNumber());
        sep = ",";
    }
}
...
#define PRINT_FLAG_IF(comp, cond, text) \
    do {                                \
        if (cond)                       \
            log->prints(text " ");      \
    } while (false)
...
#undef PRINT_FLAG
    ...
    if (_fields) {
        ...
        for (int32_t i = 0; i < _fields->size(); i++) {
            ...
            log->printf("     %2d: offset=%-3d size=%-2d vectorElem=%-2d ", i, field._offset, field._size,
                field._vectorElem);
            if (field._symRef)
...
void TR_MonitorStructureChecker::processBlock(TR::Block *block)
{
    ...
    for (TR::TreeTop *tt = block->getEntry(); tt != exitTT && !_foundIllegalStructure; tt = tt->getNextTreeTop()) {
        TR::Node *node = tt->getNode();
...
void TR_MonitorStructureChecker::propagateInfoTo(TR::Block *block, int32_t inInfo)
{
    if (inInfo < 0)
        ...
        else if (inInfo != thisInfo)
            _foundIllegalStructure = true;
    }
}
#endif

static TR_DependentAllocations *getDependentAllocationsFor(Candidate *c,
    List<TR_DependentAllocations> *dependentAllocations)
{
...
TR_DataFlowAnalysis::Kind TR_FlowSensitiveEscapeAnalysis::getKind() { return FlowSensitiveEscapeAnalysis; }

TR_FlowSensitiveEscapeAnalysis *TR_FlowSensitiveEscapeAnalysis::asFlowSensitiveEscapeAnalysis() { return this; }

bool TR_FlowSensitiveEscapeAnalysis::supportsGenAndKillSets() { return false; }

int32_t TR_FlowSensitiveEscapeAnalysis::getNumberOfBits() { return _numAllocations; }
...
TR_FlowSensitiveEscapeAnalysis::TR_FlowSensitiveEscapeAnalysis(TR::Compilation *comp, TR::Optimizer *optimizer,
    TR_Structure *rootStructure, TR_EscapeAnalysis *escapeAnalysis)
...
{
    OMR::Logger *log = comp->log();
    logprints(trace(), log, "Starting FlowSensitiveEscapeAnalysis analysis\n");
    ...
    for (candidate = _candidates->getFirst(); candidate; candidate = next) {
        next = candidate->getNext();
        logprintf(trace(), log, "Allocation node %p is represented by bit position %d\n", candidate->_node,
            _numAllocations);
        candidate->_index = _numAllocations++;
    }

    logprintf(trace(), log, "_numAllocations = %d\n", _numAllocations);
    ...
    for (treeTop = comp->getStartTree(); treeTop; treeTop = treeTop->getNextTreeTop()) {
        ...
        if (node->getOpCodeValue() == TR::BBStart) {
            ...
            //     !blockInfo->isEmpty())
            //    {
            //    printf("%d allocation(s) reached allocation in block_%d in method %s\n", blockInfo->elementCount(),
            //    blockNum, comp->signature(); blockInfo->print(comp); fflush(stdout);
            //    }

            continue;
        }

        if (node->getOpCode().isNullCheck() || node->getOpCode().isResolveCheck()
            || (node->getOpCodeValue() == TR::treetop))
            node = node->getFirstChild();
        ...
        if ((node->getOpCodeValue() == TR::monent) || (node->getOpCodeValue() == TR::monexit)) {
            Candidate *candidate;
            for (candidate = _candidates->getFirst(); candidate; candidate = candidate->getNext()) {
                if (escapeAnalysis->_valueNumberInfo->getValueNumber(candidate->_node)
                    == escapeAnalysis->_valueNumberInfo->getValueNumber(node->getFirstChild())) {
                    if (blockInfo && blockInfo->get(candidate->_index)
                        && performTransformation(comp,
                            "%sMark monitor node [%p] as a local object monitor (flow sensitive escape analysis)\n",
                            OPT_DETAILS, node)) {
    ...
    for (TR::CFGNode *node = comp->getFlowGraph()->getFirstNode(); node; node = node->getNext()) {
        _successorInfo[node->getNumber()]
            = new (trStackMemory()) TR_BitVector(numBlocks, trMemory(), stackAlloc, notGrowable);
        _predecessorInfo[node->getNumber()]
            = new (trStackMemory()) TR_BitVector(numBlocks, trMemory(), stackAlloc, notGrowable);
    }
    ...
    TR::MonitorElimination::collectPredsAndSuccs(comp->getFlowGraph()->getStart(), visitedNodes, _predecessorInfo,
        _successorInfo, &_cfgBackEdges, _loopEntryBlocks, comp, trace());
    ...
    *_scratch2 |= *_catchBlocks;
    ...
    for (flushCandidate = _flushCandidates->getFirst(); flushCandidate; flushCandidate = flushCandidate->getNext()) {
        ...
        while (succIt.hasMoreElements()) {
            ...
            *_scratch2 |= *_catchBlocks;
            ...
            logprintf(trace(), log, "Successor %d being examined\n", nextSucc);

            if ((_blocksWithFlushes->get(nextSucc) || _blocksWithSyncs->get(nextSucc)) && succInfo
                && succInfo->get(candidate->_index)) {
                logprintf(trace(), log, "Current allocation %d reaches successor %d\n", candidate->_index, nextSucc);
                ...
                *_scratch2 &= *_scratch;
                ...
                for (nextNode = cfg->getFirstNode(); nextNode; nextNode = nextNode->getNext()) {
                    if (_scratch->get(nextNode->getNumber())) {
                        for (auto succ = nextNode->getSuccessors().begin(); succ != nextNode->getSuccessors().end();
                             ++succ) {
                            logprintf(trace(), log, "Checking succ edge from %d to %d\n", nextNode->getNumber(),
                                (*succ)->getTo()->getNumber());
                            ...
                            if (!_scratch->get((*succ)->getTo()->getNumber())
                                ...
                                logprintf(trace(), log, "Adding flush edge from %d to %d\n", nextNode->getNumber(),
                                    (*succ)->getTo()->getNumber());
                            }
                        }
                    }
                ...
                // if (postDominated)
                {
                    logprintf(trace(), log, "Current allocation %d is post dominated by allocation in successor %d\n",
                        candidate->_index, nextSucc);

                    Candidate *succCandidate = NULL;
                    FlushCandidate *succFlushCandidate = NULL;
                    if (!_blocksWithSyncs->get(nextSucc)) {
                        for (succFlushCandidate = _flushCandidates->getFirst(); succFlushCandidate;
                             succFlushCandidate = succFlushCandidate->getNext())
                        // for (succCandidate = _candidates->getFirst(); succCandidate; succCandidate =
                        // succCandidate->getNext())
                        {
                            ...
                            if ((succBlockNum == nextSucc) && succCandidate->_flushRequired) {
                                bool nextAllocCanReach = true;

                                ListIterator<Candidate> candIt(&candidate->_flushMovedFrom);
                                for (Candidate *dependentCandidate = candIt.getFirst(); dependentCandidate;
                                     dependentCandidate = candIt.getNext()) {
                    ...
                    // If we have a sync node where the flush is not already optimally placed or we have a successor
                    // flush candidate
                    if ((_blocksWithSyncs->get(nextSucc)
                            && flushCandidate->getFlush()->getNode() != tt->getPrevTreeTop()->getNode())
                        || succCandidate) {
                        ...
                        if ((prevTree->getNextTreeTop() == flushTree) && (nextTree->getPrevTreeTop() == flushTree)) {
                            if (flushTree->getNode()->getOpCodeValue() == TR::allocationFence) {
                                TR_ASSERT_FATAL(flushTree->getNode()->getAllocation() != NULL
                                        || flushTree->getNode()->canOmitSync() == true,
                                    "EA: Attempt to omit sync on a fence (%p) with allocation already set to ALL.\n",
                                    flushTree->getNode());
                                flushTree->getNode()->setOmitSync(true);
                                logprintf(trace(), log, "Setting AF node %p for allocation %p to omitsync\n",
                        ...
                        if (!succCandidate) {
                            ...
                            if (afNode->getOpCodeValue() == TR::allocationFence) {
                                if (afNode->canOmitSync() == true) {
                                    ...
                                    logprintf(trace(), log, "Restoring AF node %p for allocation %p above node %p.\n",
                                        afNode, candidate->_node, tt->getNode());
                                } else {
                                    afNode->setAllocation(
                                        NULL); // Existing AllocationFence is now needed by more then one allocation
                                    logprintf(trace(), log, "Setting AF node %p allocation to ALL above %p.\n", afNode,
                                        tt->getNode());
                                }
                            } else {
                                // Insert an AllocationFence above the monExit/volatile-access. CodeGen can optimize the
                                // fence & monExit/volatile-access sequence so that only one flush instruction is
                                // executed. With lock-reservation or lock-reentry it's possible for a monExit to skip
                                // the flush so this AllocationFence will ensure correct behaviour. In addition, if the
                                // monExit/volatile-access happens to be moved/removed later in the compile, this
                                // AllocationFence will ensure a flush instruction is generated.
                                ...
                                logprintf(trace(), log, "Inserted AF node %p for candidate %p above %p (%s).\n", afNode,
                                    candidate->_node, tt->getNode(), tt->getNode()->getOpCode().getName());
                            }
                        }

                        if (trace()) {
                            if (succCandidate) {
                                log->printf("Flush for current allocation %d is post dominated by (and moved to) Flush "
                                            "for allocation %d\n",
                                    candidate->_index, succCandidate->_index);
                            } else {
                                log->printf("Flush for current allocation %d is post dominated by (and moved to) real "
                                            "sync for in succ %d\n",
                                    candidate->_index, nextSucc);
                            }
                        }
        ...
        for (listElem = _flushEdges.getListHead(); listElem != NULL;) {
            ...
            if (trace()) {
                log->printf("Processing flush edge from %d to %d\n", pair->getEdge()->getFrom()->getNumber(),
                    pair->getEdge()->getTo()->getNumber());
    ...
    for (TR_CFGEdgeAllocationPair *pair = pairIt.getFirst(); pair; pair = pairIt.getNext()) {
        ...
        if (to == comp->getFlowGraph()->getEnd()) {
            ...
            logprintf(trace(), log, "For edge %d->%d adding Flush in block_%d for candidate %p index %d\n",
                pair->getEdge()->getFrom()->getNumber(), pair->getEdge()->getTo()->getNumber(), splitBlock->getNumber(),
                pair->getAllocation(), pair->getAllocation()->_index);
        } else {
            splitBlock = findOrSplitEdge(toBlock(pair->getEdge()->getFrom()), toBlock(to));
            logprintf(trace(), log, "Splitting edge %d->%d with block_%d for candidate %p index %d\n",
                pair->getEdge()->getFrom()->getNumber(), pair->getEdge()->getTo()->getNumber(), splitBlock->getNumber(),
                pair->getAllocation(), pair->getAllocation()->_index);
        }
        ...
        if (firstNode->getOpCodeValue() == TR::allocationFence)
            ...
            splitBlock->prepend(TR::TreeTop::create(comp,
                TR::Node::createAllocationFence(pair->getAllocation()->_node, pair->getAllocation()->_node), NULL,
                NULL));
    }
...
bool TR_FlowSensitiveEscapeAnalysis::postInitializationProcessing()
{
    ...
    for (treeTop = comp()->getStartTree(); treeTop; treeTop = treeTop->getNextTreeTop()) {
        ...
        if ((node->getOpCodeValue() == TR::allocationFence) && node->getAllocation()) {
        ...
        if (node->getOpCodeValue() == TR::monent) {
            Candidate *candidate;
            for (candidate = _candidates->getFirst(); candidate; candidate = candidate->getNext()) {
                if (_escapeAnalysis->_valueNumberInfo->getValueNumber(node->getFirstChild())
                    == _escapeAnalysis->_valueNumberInfo->getValueNumber(candidate->_node))
        ...
        // Due to lock-reservation and lock reentry it's not guaranteed that a synchronized method will issue a memory
        // flush
    ...
    if (!_newlyAllocatedObjectWasLocked && _flushCandidates->isEmpty()) {
        return false;
    }
...
TR::Block *TR_FlowSensitiveEscapeAnalysis::findOrSplitEdge(TR::Block *from, TR::Block *to)
{
    ...
    if (!from->hasSuccessor(to)) {
        for (auto edge2 = to->getPredecessors().begin(); edge2 != to->getPredecessors().end(); ++edge2) {
            if (_splitBlocks.find((*edge2)->getFrom()) && from->hasSuccessor((*edge2)->getFrom())) {
...
void TR_FlowSensitiveEscapeAnalysis::analyzeTreeTopsInBlockStructure(TR_BlockStructure *blockStructure)
{
    ...
    if ((block == comp()->getFlowGraph()->getStart()) || (block == comp()->getFlowGraph()->getEnd()))
        return;
...
void TR_FlowSensitiveEscapeAnalysis::analyzeNode(TR::Node *node, TR::TreeTop *treeTop, bool seenException,
    int32_t blockNum, TR::Node *parent)
{
    ...
    if (opCode.hasSymbolReference()) {
        ...
        if (!symReference->getSymbol()->isTransparent()) {
            _blocksWithSyncs->set(blockNum);
            // Keep track of this volatile load/store node tt, prefer volatile load/store over monexits since monexits
            // can skip sync in some cases
            _syncNodeTTForBlock[blockNum] = treeTop;
        }
    }
    ...
    if (!node->getOpCode().isCall()) {
        ...
        if ((node->getOpCodeValue() == TR::areturn) || (node->getOpCodeValue() == TR::athrow))
            child = node->getFirstChild();
        ...
        } else if (node->getOpCode().isStore() && node->getSymbolReference()->getSymbol()->isStatic())
            child = node->getFirstChild();
        ...
        for (candidate = _candidates->getFirst(); candidate; candidate = candidate->getNext()) {
            if (child && escapeAnalysis->usesValueNumber(candidate, valueNumber)) {
                if (!dependencies) {
                    _regularInfo->reset(candidate->_index);
                    if (seenException)
                        _exceptionInfo->reset(candidate->_index);

                    TR_DependentAllocations *deps
                        = getDependentAllocationsFor(candidate, &(escapeAnalysis->_dependentAllocations));
                    if (deps) {
...
int32_t TR_LocalFlushElimination::perform()
{
    ...
    for (treeTop = comp()->getStartTree(); treeTop; treeTop = treeTop->getNextRealTreeTop()) {
        ...
        if ((node->getOpCodeValue() == TR::allocationFence) && node->getAllocation()) {
            FlushCandidate *candidate
                = new (trStackMemory()) FlushCandidate(treeTop, node->getAllocation(), block->getNumber());
            _flushCandidates->add(candidate);
        }
    }
...
bool TR_LocalFlushElimination::examineNode(TR::Node *node, TR::TreeTop *tt, TR::Block *currBlock,
    TR::NodeChecklist &visited)
{
    ...
    if (!opCode.isCall()) {
        ...
        if (node->getOpCodeValue() == TR::monexit) {
            ...
            // Due to lock-reservation and lock reentry it's not guaranteed that a synchronised method will issue a
            // flush
            // if (opCode.isCall() &&
        ...
        // for (candidate = _candidates->getFirst(); candidate; candidate = candidate->getNext())
        {
            ...
            if (child
                && ((_escapeAnalysis && _escapeAnalysis->usesValueNumber(candidate, valueNumber))
                    || !_escapeAnalysis)) {
                if (!dependencies) {
            ...
            if (((node == flushCandidate->getFlush()->getNode()) && candidate->_flushRequired)
                || (nodeHasSync && notAnalyzedSync)) {
                ...
                while (allocIt.hasMoreElements()) {
                    ...
                    for (reachingFlushCandidate = _flushCandidates->getFirst(); reachingFlushCandidate;
                         reachingFlushCandidate = reachingFlushCandidate->getNext())
                    // for (reachingCandidate = _candidates->getFirst(); reachingCandidate; reachingCandidate =
                    // reachingCandidate->getNext())
                    {
                        ...
                        if ((reachingCandidate->_index == nextAlloc) && (reachingFlushCandidate != flushCandidate)
                    ...
                    // If reachingFlushCandidate is already placed in the best possible location (above a volatile
                    // access) make sure we don't attempt to moved it
                    if (nodeHasVolatile && reachingFlushCandidate
                        && reachingFlushCandidate->getFlush()->getNode() == tt->getPrevTreeTop()->getNode()) {
                        reachingFlushCandidate->setOptimallyPlaced(true);
                        logprintf(trace(), log, "AF %p is now marked as Optimally Placed!\n",
                            reachingFlushCandidate->getFlush()->getNode());
                    }

                    // Skip to next allocation if we don't have a candidate, or the reachingFlushCandidate is right were
                    // we want it already
                    ...
                    if (nextAllocCanReach) {
                        ...
                        if ((prevTree->getNextTreeTop() == flushTree) && (nextTree->getPrevTreeTop() == flushTree)) {
                            if (flushTree->getNode()->getOpCodeValue() == TR::allocationFence) {
                                TR_ASSERT_FATAL(flushTree->getNode()->getAllocation() != NULL,
                                    "localEA: Attempt to omit sync on a fence (%p) with allocation already set to "
                                    "ALL.\n",
                                    flushTree->getNode());
                                flushTree->getNode()->setOmitSync(true);
                                logprintf(trace(), log, "(local) Setting AF node %p for allocation %p to omitsync\n",
                                    flushTree->getNode(), flushTree->getNode()->getAllocation());
                                flushTree->getNode()->setAllocation(NULL);
                            }
                            // prevTree->join(nextTree);
                        ...
                        if (!nodeHasSync) {
                            ...
                            if (tt->getPrevTreeTop()->getNode()->getOpCodeValue() == TR::allocationFence) {
                                ...
                                if (afNode->canOmitSync()) {
                                    ...
                                    logprintf(trace(), log,
                                        "(local) Restoring AF node %p for allocation %p above node %p.\n", afNode,
                                        candidate->_node, tt->getNode());
                                } else {
                                    afNode->setAllocation(
                                        NULL); // Existing AllocationFence is now needed by more then one allocation
                                    logprintf(trace(), log, "(local) Setting AF node %p allocation to ALL above %p.\n",
                                        afNode, tt->getNode());
                                }
                            } else {
                                // Insert an AllocationFence above the monExit/volatile-access. CodeGen can optimize the
                                ...
                                tt->insertBefore(TR::TreeTop::create(comp(), afNode, NULL, NULL));
                                logprintf(trace(), log,
                                    "(local) Inserted AF node %p for candidate %p above node %p (%s).\n", afNode,
                                    candidate->_node, node, node->getOpCode().getName());
                            }
                        }
                        ...
                        if (trace()) {
                            if (!nodeHasSync) {
                                ...
                                log->printf("Flush for current allocation %d is post dominated by (and moved to) sync "
                                            "in the same block\n",
        ...
        // if (_escapeAnalysis)
        {
            ...
            for (int32_t arg = firstArgIndex; arg < node->getNumChildren(); arg++) {
                ...
                Candidate *candidate;
                for (candidate = _candidates->getFirst(); candidate; candidate = candidate->getNext()) {
                    if ((_escapeAnalysis && _escapeAnalysis->usesValueNumber(candidate, valueNumber))
                        || !_escapeAnalysis) {
                        _allocationInfo->reset(candidate->_index);
