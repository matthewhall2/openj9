...
bool TR_DynamicLiteralPool::visitTreeTop(TR::TreeTop *tt, TR::Node *grandParent, TR::Node *parent, TR::Node *node,
    vcount_t visitCount)
{
    ...
    if (cg()->supportsOnDemandLiteralPool()) {
        ...
        } else if (opCode.hasSymbolReference() && node->getSymbol()->isStatic()
...
bool TR_DynamicLiteralPool::transformNeeded(TR::Node *grandParent, TR::Node *parent, TR::Node *child)
{
    ...
    if (parentOpCode.isMul() || parentOpCode.isDiv())
        return false;
    ...
    if (cg()->getSupportsTestUnderMask() && parentOpCode.isIf() && parent->getNumChildren() == 2
        && parent->getSecondChild()->getOpCode().isLoadConst()
        && isPowerOf2(parent->getSecondChild()->get64bitIntegralValue())) {
        return false;
    }
    if (cg()->getSupportsTestUnderMask() && grandParent && grandParent->getOpCode().isIf() && parentOpCode.isAnd()
        && parent->getNumChildren() == 2 && parent->getSecondChild()->getOpCode().isLoadConst()
        && isPowerOf2(parent->getSecondChild()->get64bitIntegralValue())) {
        return false;
    }
    ...
    if (parentOpCodeValue == TR::arrayset)
        return false;

    if (child->isClassUnloadingConst())
        return false;

    // TODO: This function can be simplified because the context in which it is used the following call will always
    // result yield false. As such many paths through this function that return false are effectively dead and can be
    // removed. There is only one path that does not return false, and this is the only path that needs to exist, though
    // my intuition is that this path too should be verified as it may no longer be applicable.
...
bool TR_DynamicLiteralPool::transformConstToIndirectLoad(TR::Node *parent, TR::Node *child)
{
    ...
    {
        indirectLoad = TR::Node::createWithSymRef(comp()->il.opCodeForIndirectLoad(child->getDataType()), 1, 1,
            addrNode, shadow);
...
bool TR_DynamicLiteralPool::transformStaticSymRefToIndirectLoad(TR::TreeTop *tt, TR::Node *parent, TR::Node *&child)
{
    ...
    if (childOpcodeValue == TR::loadaddr) {
        ...
        TR::Node *loadLiteralFromThePool
            = TR::Node::createWithSymRef(TR::aloadi, 1, 1, getAloadFromCurrentBlock(child), childSymRef);
        ...
        if (childOpcodeValue == TR::awrtbar) {
            ...
            child = TR::Node::create(TR::awrtbari, 3, loadLiteralFromThePool, child->getFirstChild(),
...
bool TR_DynamicLiteralPool::handleNodeUsingVMThread(TR::TreeTop *tt, TR::Node *parent, TR::Node *node,
    vcount_t visitCount)
{
    /*
    if (node->getOpCode().isLoadVarOrStore())
       ...
       if (symRef->isUnresolved())
          ...
          if (symRef->getSymbol()->isMethodMetaData())
             ...
             TR::SymbolReference *metaDataShadow =
    getSymRefTab()->findOrCreateGenericIntShadowSymbolReference(symRef->getOffset()); TR::Node *
    loadVMThreadFromTheStack = getVMThreadAloadFromCurrentBlock(node); TR::DataType type = node->getDataType(); if
...
bool TR_DynamicLiteralPool::handleNodeUsingSystemStack(TR::TreeTop *tt, TR::Node *parent, TR::Node *node,
    vcount_t visitCount)
{
    return true;
}

TR::SymbolReference *getVMThreadSym() { return NULL; }
...
const char *TR_DynamicLiteralPool::optDetailString() const throw() { return "O^O DYNAMIC LITERAL POOL: "; }
