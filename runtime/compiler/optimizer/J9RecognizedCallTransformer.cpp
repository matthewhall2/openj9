...
void J9::RecognizedCallTransformer::processConvertingUnaryIntrinsicFunction(TR::TreeTop *treetop, TR::Node *node,
    TR::ILOpCodes argConvertOpcode, TR::ILOpCodes opcode, TR::ILOpCodes resultConvertOpcode)
{
...
void J9::RecognizedCallTransformer::process_java_lang_Class_IsAssignableFrom(TR::TreeTop *treetop, TR::Node *node)
{
    ...
    treetop->insertBefore(TR::TreeTop::create(comp(),
        TR::Node::createWithSymRef(TR::NULLCHK, 1, 1, TR::Node::create(node, TR::PassThrough, 1, toClass), nullchk)));
    treetop->insertBefore(TR::TreeTop::create(comp(),
...
static void substituteNode(TR::NodeChecklist &visited, TR::Node *subOld, TR::Node *subNew, TR::Node *node)
{
    ...
    for (int32_t i = 0; i < node->getNumChildren(); i++) {
        ...
        if (child == subOld) {
            ...
            TR_ASSERT_FATAL_WITH_NODE(subOld, subOld->getReferenceCount() >= 2,
...
void J9::RecognizedCallTransformer::process_java_lang_Class_cast(TR::TreeTop *treetop, TR::Node *node)
{
    ...
    TR_ASSERT_FATAL_WITH_NODE(node, comp()->getOSRMode() != TR::involuntaryOSR,
    ...
    TR::SymbolReference *classFromJavaLangClassSR = srTab->findOrCreateClassFromJavaLangClassSymbolRef();

    TR::SymbolReference *checkcastSR = srTab->findOrCreateCheckCastSymbolRef(comp()->getMethodSymbol());

    TR::Node *j9class = TR::Node::createWithSymRef(TR::aloadi, 1, 1, jlClass, classFromJavaLangClassSR);

    TR::Node *cast = TR::Node::createWithSymRef(node, TR::checkcast, 2, checkcastSR);
    ...
    TR_ASSERT_FATAL_WITH_NODE(node, node->getReferenceCount() == 1, "expected exactly one occurrence to remain");
...
// if-diamond inlines the call using a compiler intrinsic and the fallback path reverts back to calling the method
// traditionally.
void J9::RecognizedCallTransformer::process_java_lang_StringCoding_encodeASCII(TR::TreeTop *treetop, TR::Node *node)
{
    ...
    uint32_t *latin1FieldAddress
        = (uint32_t *)fej9->getStaticFieldAddress(stringClass, (unsigned char *)"LATIN1", 6, (unsigned char *)"B", 1);
    ...
    // Note that *latin1FieldAddress will be correct for AOT compilations as well because String.LATIN1 is a final
    // static field with a constant initializer. This means its value is determined by the ROM class of String, and
    // therefore when the String class chain validation succeeds on AOT load, the value is guaranteed to be the same as
    // the one we see during AOT compilation.
    TR::Node *constNode = TR::Node::iconst(node,
        ((TR_J9VM *)fej9)->dereferenceStaticFinalAddress(latin1FieldAddress, TR::Int32).dataInt32Bit);
    ...
    TR::Block *fallbackPathBlock
        = ifCmpBlock->split(treetop, cfg, true /* fixUpCommoning */, true /* copyExceptionSuccessors */);
    // Then split again to create the tail block
    TR::Block *tailBlock = fallbackPathBlock->split(treetop->getNextTreeTop(), cfg, true /* fixUpCommoning */,
        true /* copyExceptionSuccessors */);

    // Now we have the originalBlock, the fallback block and the merge block (tailBlock). The call result will be an
    // astore. If the nodes in the above blocks were uncommoned as expected, then the treetop after the call node should
    // be an astore to a temp slot. So assert that it is, and then grab a reference to the temp slot so we can store to
    // it ourselves in the forward path. Note that we can rely on the call result having been used in a later tree since
    // encodeASCII represents a private method whose call sites we can inspect and because this transformation happens
    // very early as part of ilgen opts
    ...
    TR_ASSERT_FATAL(resultStoreNode->getOpCode().getOpCodeValue() == TR::astore,
        "Treetop after call must be an astore to a temp!");
    ...
    TR::Node *destinationArrayNode = TR::Node::createWithSymRef(node, TR::newarray, 2,
        comp()->getSymRefTab()->findOrCreateNewArraySymbolRef(
    ...
    TR::TreeTop *destinationArrayTreeTop
        = TR::TreeTop::create(comp(), TR::Node::create(node, TR::treetop, 1, destinationArrayNode));
    ...
    // We now have the length node, and also the destination array. Now we create an encodeASCIISymbol node to do the
    // encoding operation.
    /*
    ...
    */
    ...
    TR::TreeTop *encodeASCIITreeTop
        = TR::TreeTop::create(comp(), TR::Node::create(node, TR::treetop, 1, encodeASCIINode));
    ...
    TR::Block *fallthroughBlock = destinationArrayTreeTop->getEnclosingBlock()->split(destinationArrayTreeTop, cfg,
        true /* fixUpCommoning */, true /* copyExceptionSuccessors */);
    ...
    // Now we have fallthrough block, fallback block and tail/merge block. Let's set the ifcmp's destination to the
    // fallback block, and update the CFG as well.
...
void J9::RecognizedCallTransformer::process_java_lang_StringLatin1_compareTo_BBII(TR::TreeTop *treetop, TR::Node *node)
{
    ...
    TR::Node *ifCmpNode
        = TR::Node::createif(TR::iflcmpeq, lim->duplicateTree(), TR::Node::createLoad(node, arraycmplenSymRef));
    TR::TreeTop *ifCmpTreeTop = TR::TreeTop::create(comp(), ifCmpNode);

    TR::DataType resultDataType = node->getDataType();
    TR::SymbolReference *resultSymRef
        = comp()->getSymRefTab()->createTemporary(comp()->getMethodSymbol(), resultDataType);

    // result = arrayLen1 - arrayLen2;
    TR::Node *isubNode1 = TR::Node::create(node, TR::isub, 2, arrayLen1->duplicateTree(), arrayLen2->duplicateTree());
    TR::Node *storeResult1 = TR::Node::createStore(node, resultSymRef, isubNode1);
    TR::TreeTop *storeTreeTop1 = TR::TreeTop::create(comp(), storeResult1);
    ...
    TR::Block *remainderBlock = callBlock->createConditionalBlocksBeforeTree(treetop, ifCmpTreeTop, storeTreeTop1,
...
void J9::RecognizedCallTransformer::process_java_lang_StringLatin1_inflate_BIBII(TR::TreeTop *treetop, TR::Node *node)
{
    ...
    TR_ASSERT_FATAL(comp()->cg()->getSupportsArrayTranslateTROTNoBreak(),
        "Support for arraytranslateTROTNoBreak is required");
    ...
    TR::TreeTop *callCopyTT
        = TR::TreeTop::create(comp(), TR::Node::create(node, TR::treetop, 1, node->duplicateTree(false)));
    ...
    TR::Node *tmpNode
        = TR::TransformUtil::generateConvertArrayElementIndexToOffsetTrees(comp(), srcOff, NULL, 1, false);
    ...
    TR::Block *ifCmpBlock3
        = ifCmpBlock2->split(ifCmpTreeTop3, cfg, true /* fixUpCommoning */, true /* copyExceptionSuccessors */);
    TR::Block *ifCmpBlock4
        = ifCmpBlock3->split(ifCmpTreeTop4, cfg, true /* fixUpCommoning */, true /* copyExceptionSuccessors */);
    TR::Block *ifCmpBlock5
        = ifCmpBlock4->split(ifCmpTreeTop5, cfg, true /* fixUpCommoning */, true /* copyExceptionSuccessors */);
    TR::Block *fallThroughPathBlock
        = ifCmpBlock5->split(arrayTranslateTreeTop, cfg, true /* fixUpCommoning */, true /* copyExceptionSuccessors */);
    // This block contains the original call node
    TR::Block *fallbackPathBlock
        = fallThroughPathBlock->split(treetop, cfg, true /* fixUpCommoning */, true /* copyExceptionSuccessors */);
    TR::Block *tailBlock = fallbackPathBlock->split(treetop->getNextTreeTop(), cfg, true /* fixUpCommoning */,
        true /* copyExceptionSuccessors */);
...
void J9::RecognizedCallTransformer::process_java_lang_StringUTF16_toBytes(TR::TreeTop *treetop, TR::Node *node)
{
    ...
    TR::Node *newByteArrayNode = TR::Node::createWithSymRef(TR::newarray, 2, 2,
        TR::Node::create(TR::ishl, 2, lenNode, TR::Node::iconst(1)), TR::Node::iconst(byteArrayType),
    ...
    TR::Node *newCallNode = TR::Node::createWithSymRef(node, TR::call, 5,
        getSymRefTab()->methodSymRefFromName(comp()->getMethodSymbol(), "java/lang/String", "decompressedArrayCopy",
            "([CI[BII)V", TR::MethodSymbol::Static));
    ...
    TR::TreeTop *lastFallThroughTreeTop
        = TR::TreeTop::create(comp(), newByteArraryTreeTop, TR::Node::create(node, TR::treetop, 1, newCallNode));
    ...
    TR::Block *fallThroughPathBlock
        = ifCmpBlock->split(newByteArraryTreeTop, cfg, true /* fixUpCommoning */, true /* copyExceptionSuccessors */);

    // Then split the fallback, out-of-line call into its own block
    TR::Block *fallbackPathBlock = fallThroughPathBlock->split(fallbackTreeTop, cfg, true /* fixUpCommoning */,
        true /* copyExceptionSuccessors */);

    // Then split again at the original call TreeTop to create the tail block
    TR::Block *tailBlock
        = fallbackPathBlock->split(treetop, cfg, true /* fixUpCommoning */, true /* copyExceptionSuccessors */);
    ...
    TR::CFG *cfg)
{
    ...
    TR::Node *nullCheckNode
        = TR::Node::createif(TR::ifacmpeq, node->duplicateTree(), TR::Node::aconst(node, 0), newCallBlock->getEntry());
    ...
    if (insertArrayCheck) {
        ...
        TR::Node *arrayCheckNode = TR::Node::createif(TR::ificmpeq, maskedIsArrayClassNode, TR::Node::iconst(node, 0),
    ...
    TR::Node *node)
{
    ...
    if (TR::Compiler->om.isOffHeapAllocationEnabled()) {
        int aLen, bLen;
        const char *aObjTypeSig = a->getSymbolReference() ? a->getSymbolReference()->getTypeSignature(aLen) : NULL;
        const char *bObjTypeSig = b->getSymbolReference() ? b->getSymbolReference()->getTypeSignature(bLen) : NULL;
        ...
        {
            TR_OpaqueClassBlock *aClass = comp()->fej9()->getClassFromSignature(aObjTypeSig, aLen,
                a->getSymbolReference()->getOwningMethod(comp()));
            aCheckNeeded = aClass == NULL || aClass == comp()->getObjectClassPointer()
                || TR::Compiler->cls.isInterfaceClass(comp(), aClass);
        }

        if (!bCheckNeeded) {
            TR_OpaqueClassBlock *bClass = comp()->fej9()->getClassFromSignature(bObjTypeSig, bLen,
                b->getSymbolReference()->getOwningMethod(comp()));
            bCheckNeeded = bClass == NULL || bClass == comp()->getObjectClassPointer()
        ...
        if (aAdjustmentNeeded || bAdjustmentNeeded) {
            ...
            if (aAdjustmentNeeded) {
                ...
                a = insertVectorizedMisMatchArgumentChecksAndAdjustForOffHeap(this, a, currentBlock, callBlock,
                    aCheckNeeded, cfg);
            } else {
                a = a->duplicateTree();
            }
            ...
            if (bAdjustmentNeeded) {
                // create and arrange nullCheckBlock, arrayCheckBlock, adjustBlock, and newCallBlock
                // also create storeTree in currentBlock
                b = insertVectorizedMisMatchArgumentChecksAndAdjustForOffHeap(this, b, currentBlock, callBlock,
                    bCheckNeeded, cfg);
            } else {
                b = b->duplicateTree();
            }
...
*/
void J9::RecognizedCallTransformer::processUnsafeAtomicCall(TR::TreeTop *treetop,
    TR::SymbolReferenceTable::CommonNonhelperSymbol helper, bool needsNullCheck)
{
    bool enableTrace = trace();
    bool isNotStaticField = !strncmp(comp()->getCurrentMethod()->classNameChars(), "java/util/concurrent/atomic/",
    ...
    if (treetop->getNode()->getOpCode().isNullCheck()) {
        ...
        TR::Node *checkNode = TR::Node::createWithSymRef(treetop->getNode(), TR::NULLCHK, 1, passthrough,
    ...
    if (isNotStaticField) {
        ...
        if (TR::Compiler->om.isOffHeapAllocationEnabled()) {
            ...
            logprintf(enableTrace, comp()->log(),
                "Created isObjectNull test node n%dn, non-null object will fall through to Block_%d\n",
                isObjectNullNode->getGlobalIndex(), treetop->getEnclosingBlock()->getNumber());

            // generate array check treetop
            TR::Node *vftLoad = TR::Node::createWithSymRef(TR::aloadi, 1, 1, objectNode->duplicateTree(),
                comp()->getSymRefTab()->findOrCreateVftSymbolRef());

            isObjectArrayNode = TR::Node::createif(TR::ificmpne, comp()->fej9()->testIsClassArrayType(vftLoad),
                TR::Node::create(TR::iconst, 0), NULL);
            isObjectArrayTreeTop = TR::TreeTop::create(comp(), isObjectArrayNode, NULL, NULL);
            treetop->insertBefore(isObjectArrayTreeTop);
            treetop->getEnclosingBlock()->split(treetop, cfg, fixupCommoning);

            logprintf(enableTrace, log,
                "Created isObjectArray test node n%dn, array will branch to array access block\n",
                isObjectArrayNode->getGlobalIndex());

            address = comp()->target().is32Bit()
                ? TR::Node::create(TR::aiadd, 2, objectNode->duplicateTree(),
                      TR::Node::create(TR::l2i, 1, offsetNode->duplicateTree()))
                : TR::Node::create(TR::aladd, 2, objectNode->duplicateTree(), offsetNode->duplicateTree());
        } else
#endif /* J9VM_GC_SPARSE_HEAP_ALLOCATION */
        {
            address = comp()->target().is32Bit()
                ? TR::Node::create(TR::aiadd, 2, objectNode, TR::Node::create(TR::l2i, 1, offsetNode))
        ...
        isNotLowTaggedNode = TR::Node::createif(TR::iflcmpne,
            TR::Node::create(TR::land, 2, offsetNode->duplicateTree(),
                TR::Node::lconst(J9_SUN_STATIC_FIELD_OFFSET_TAG)),
            TR::Node::lconst(J9_SUN_STATIC_FIELD_OFFSET_TAG), NULL);
        isNotLowTaggedTreeTop = TR::TreeTop::create(comp(), isNotLowTaggedNode);
        treetop->insertBefore(isNotLowTaggedTreeTop);
        treetop->getEnclosingBlock()->split(treetop, cfg, fixupCommoning);

        logprintf(enableTrace, log,
            "Created isNotLowTagged test node n%dn, static field will fall through to Block_%d\n",
        ...
        if (!disableIllegalWriteReport && !comp()->getOption(TR_DisableGuardedStaticFinalFieldFolding)
            && TR_J9MethodBase::isUnsafePut(
                unsafeCall->getSymbol()->castToMethodSymbol()->getMandatoryRecognizedMethod())) {
            ...
            auto reportFinalFieldModification
                = TR::TransformUtil::generateReportFinalFieldModificationCallTree(comp(), objectNode->duplicateTree());
            auto elseBlock = treetop->getEnclosingBlock();
            TR::TransformUtil::createConditionalAlternatePath(comp(), isFinalTreeTop, reportFinalFieldModification,
                elseBlock, elseBlock, comp()->getMethodSymbol()->getFlowGraph(), true /*markCold*/);
            logprintf(enableTrace, log,
                "Created isFinal test node n%dn, non-final-static field will fall through to Block_%d, final field "
                "goes to Block_%d\n",
                isFinalNode->getGlobalIndex(), treetop->getEnclosingBlock()->getNumber(),
                reportFinalFieldModification->getEnclosingBlock()->getNumber());

            TR::DebugCounter::prependDebugCounter(comp(),
                TR::DebugCounter::debugCounterName(comp(), "illegalWriteReport/atomic/(%s %s)", comp()->signature(),
                    comp()->getHotnessName(comp()->getMethodHotness())),
        ...
        auto objectAdjustmentNode = TR::Node::createWithSymRef(TR::astore, 1, 1,
            TR::Node::createWithSymRef(TR::aloadi, 1, 1,
                TR::Node::createWithSymRef(TR::aloadi, 1, 1, objectNode->duplicateTree(),
                    comp()->getSymRefTab()->findOrCreateClassFromJavaLangClassSymbolRef()),
        ...
        auto offsetAdjustmentNode = TR::Node::createWithSymRef(TR::lstore, 1, 1,
            TR::Node::create(TR::land, 2, offsetNode->duplicateTree(), TR::Node::lconst(~J9_SUN_FIELD_OFFSET_MASK)),
            offsetNode->getSymbolReference());

        logprintf(enableTrace, log,
            "Created node objectAdjustmentNode n%dn #%d and offsetAdjustmentNode #%d n%dn to adjust object and offset "
            "for static field\n",
            objectAdjustmentNode->getGlobalIndex(),
            objectAdjustmentNode->getOpCode().hasSymbolReference()
                ? objectAdjustmentNode->getSymbolReference()->getReferenceNumber()
                : -1,
            offsetAdjustmentNode->getGlobalIndex(),
            offsetAdjustmentNode->getOpCode().hasSymbolReference()
                ? offsetAdjustmentNode->getSymbolReference()->getReferenceNumber()
                : -1);
    ...
    if (isObjectNullTreeTop) {
        ...
        if (isNotStaticField && isObjectArrayTreeTop) {
            ...
            treetop->getEnclosingBlock()->getExit()->insertTreeTopsAfterMe(arrayAccessBlock->getEntry(),
                arrayAccessBlock->getExit());
            isObjectArrayNode->setBranchDestination(arrayAccessTreeTop->getEnclosingBlock()->getEntry());

            treetop->getEnclosingBlock()->append(TR::TreeTop::create(comp(),
                TR::Node::create(arrayAccessTreeTop->getNode(), TR::Goto, 0, returnBlock->getEntry())));
            cfg->addNode(arrayAccessBlock);
            cfg->addEdge(TR::CFGEdge::createEdge(isObjectArrayTreeTop->getEnclosingBlock(), arrayAccessBlock,
                comp()->trMemory()));
            ...
            if (storeReturnNode) {
                TR::Node *storeNode = TR::Node::createStore(unsafeCall, storeReturnNode->getSymbolReference(),
                    arrayAccessTreeTop->getNode()->getFirstChild());
            ...
            cfg->addEdge(TR::CFGEdge::createEdge(isObjectNullTreeTop->getEnclosingBlock(), treetop->getEnclosingBlock(),
                comp()->trMemory()));

            logprintf(enableTrace, log,
                "Created array access helper block_%d that loads dataAddr pointer from array object address\n",
            ...
            if (!isNotStaticField) {
                ...
                logprintf(enableTrace, log,
                    "Created atomic method helper block_%d that loads from ramStatics treetop n%dn. returnBlock "
                    "block_%d\n",
                    unsafeCallRamStaticsTT->getEnclosingBlock()->getNumber(),
                    unsafeCallRamStaticsTT->getNode()->getGlobalIndex(), returnBlock->getNumber());
                ...
                if (isObjectArrayTreeTop != NULL) {
                    ...
                    TR::Block *arrayAccessBlock = TR::Block::createEmptyBlock(arrayAccessTreeTop->getNode(), comp(),
                        treetop->getEnclosingBlock()->getFrequency());
                    arrayAccessBlock->append(arrayAccessTreeTop);
                    arrayAccessBlock->append(TR::TreeTop::create(comp(),
                        TR::Node::create(arrayAccessTreeTop->getNode(), TR::Goto, 0, returnBlock->getEntry())));
                    ...
                    unsafeCallRamStaticsTT->getEnclosingBlock()->getExit()->insertTreeTopsAfterMe(
                        arrayAccessBlock->getEntry(), arrayAccessBlock->getExit());
                    isObjectArrayNode->setBranchDestination(arrayAccessTreeTop->getEnclosingBlock()->getEntry());
                ...
                logprintf(enableTrace, log,
                    "Block_%d contains call to atomic method helper, and is the target of isObjectNull and "
                    "isNotLowTagged tests\n",
                    treetop->getEnclosingBlock()->getNumber());

                isObjectNullNode->setBranchDestination(treetop->getEnclosingBlock()->getEntry());
                cfg->addEdge(TR::CFGEdge::createEdge(isObjectNullTreeTop->getEnclosingBlock(),
                    treetop->getEnclosingBlock(), comp()->trMemory()));
                isNotLowTaggedNode->setBranchDestination(treetop->getEnclosingBlock()->getEntry());
                cfg->addEdge(TR::CFGEdge::createEdge(isNotLowTaggedTreeTop->getEnclosingBlock(),
                    treetop->getEnclosingBlock(), comp()->trMemory()));
...
static char *getSignatureForComputedCall(const char * const extraParamsBefore, const char * const extraParamsAfter,
    TR::Compilation *comp, const char * const origSignature, int32_t &signatureLength)
{
    ...
    char *computedCallSignature = (char *)comp->trMemory()->allocateMemory(signatureAllocSize, heapAlloc);
    ...
    if (!returnType)
        returnType = paramsEnd + 1;
    const char * const returnTypeEnd = nextSignatureArgument(returnType);

    // Put together the new signature.
    TR::snprintfNoTrunc(computedCallSignature, signatureAllocSize, "(%s%.*s%s)%.*s", extraParamsBefore,
        (int)(paramsEnd - paramsStart), paramsStart, extraParamsAfter, (int)(returnTypeEnd - returnType), returnType);
    ...
    TR::Node *node)
{
    ...
    uint32_t offset = fej9->getInstanceFieldOffsetIncludingHeader("Ljava/lang/invoke/MethodHandle;", "form",
        "Ljava/lang/invoke/LambdaForm;", comp()->getCurrentMethod());

    TR::SymbolReference *lambdaFormSymRef = comp()->getSymRefTab()->findOrFabricateShadowSymbol(
        comp()->getMethodSymbol(), TR::Symbol::Java_lang_invoke_MethodHandle_form, TR::Address, offset, isVolatile,
        isPrivate, isFinal, "java/lang/invoke/MethodHandle.form Ljava/lang/invoke/LambdaForm;");

    offset = fej9->getInstanceFieldOffsetIncludingHeader("Ljava/lang/invoke/LambdaForm;", "vmentry",
        "Ljava/lang/invoke/MemberName;", comp()->getCurrentMethod());

    TR::SymbolReference *memberNameSymRef = comp()->getSymRefTab()->findOrFabricateShadowSymbol(
        comp()->getMethodSymbol(), TR::Symbol::Java_lang_invoke_LambdaForm_vmentry, TR::Address, offset, isVolatile,
        isPrivate, isFinal, "java/lang/invoke/LambdaForm.vmentry Ljava/lang/invoke/MemberName;");
    ...
    for (int i = 0; i < node->getNumChildren(); i++) {
        ...
        TR::SymbolReference *newSymbolReference
            = comp()->getSymRefTab()->createTemporary(comp()->getMethodSymbol(), currentChild->getDataType());
    ...
    TR::Node *mhNode = TR::Node::createLoad(node,
        argsList->front()); // the first arg of invokeBasic call is the receiver MethodHandle object
    TR::Node *lambdaFormNode
        = TR::Node::createWithSymRef(node, comp()->il.opCodeForIndirectLoad(TR::Address), 1, mhNode, lambdaFormSymRef);
    lambdaFormNode->setIsNonNull(true);
    TR::Node *memberNameNode = TR::Node::createWithSymRef(node, comp()->il.opCodeForIndirectLoad(TR::Address), 1,
        lambdaFormNode, memberNameSymRef);
    ...
    TR::Node *node)
{
    ...
    TR::DebugCounter::prependDebugCounter(comp(),
        TR::DebugCounter::debugCounterName(comp(), "mh.unrefined/linkTo%s/(%s)/%s",
            isLinkToSpecial ? "Special" : "Static", comp()->signature(), comp()->getHotnessName()),
        treetop);
    ...
    if (comp()->cg()->enableJitDispatchJ9Method()) {
        if (isLinkToSpecial) {
            ...
            TR::Node *nullCheck = TR::Node::createWithSymRef(node, TR::NULLCHK, 1, passThrough, nullCheckSymRef);
        ...
        TR::Node *target = TR::Node::createWithSymRef(node, TR::aloadi, 1, memberName, vmTargetSymRef);
    ...
    TR::Node *vmTargetNode, TR::list<TR::SymbolReference *> *argsList, TR::Node *inlCallNode)
{
    ...
    TR::Node *extra = TR::Node::createWithSymRef(node,
        comp()->il.opCodeForIndirectLoad(extraField->getSymbol()->getDataType()), 1, vmTargetNode, extraField);
    TR::SymbolReference *extraTempSlotSymRef
        = comp()->getSymRefTab()->createTemporary(comp()->getMethodSymbol(), extraField->getSymbol()->getDataType());
    ...
    TR::Node *mask = comp()->target().is64Bit() ? TR::Node::lconst(node, J9_STARTPC_NOT_TRANSLATED)
                                                : TR::Node::iconst(node, J9_STARTPC_NOT_TRANSLATED);

    TR::Node *isCompiledNode = TR::Node::createif(xcmpne,
        TR::Node::create(xand, 2, TR::Node::createLoad(node, extraTempSlotSymRef), mask), zero, NULL);
    ...
    TR_ResolvedMethod *dummyMethod = fej9->createResolvedMethodWithSignature(comp()->trMemory(), dummyInvoke, NULL,
        signature, signatureLength, owningMethodSymbol->getResolvedMethod());
    TR::SymbolReference *computedCallSymbol = comp()->getSymRefTab()->findOrCreateMethodSymbol(
        owningMethodSymbol->getResolvedMethodIndex(), -1, dummyMethod, TR::MethodSymbol::ComputedStatic);

    uint32_t numComputedCallArgs = argsList->size() + 1;
    TR::Node *computedCallNode
        = TR::Node::createWithSymRef(node, node->getSymbol()->castToMethodSymbol()->getMethod()->indirectCallOpCode(),
            numComputedCallArgs, computedCallSymbol);
    computedCallNode->setAndIncChild(0,
        jitAddress); // first arguments of computed calls is the method address to jump to
    int32_t child_i = 1;
    ...
    TR::TreeTop *computedCallTreeTop
        = TR::TreeTop::create(comp(), TR::Node::create(node, TR::treetop, 1, computedCallNode));
    TR::RecognizedMethod rm = node->getSymbol()->castToMethodSymbol()->getMandatoryRecognizedMethod();
    ...
    TR::TreeTop *inlCallTreeTop = TR::TreeTop::create(comp(),
        TR::Node::create(node, TR::treetop, 1, inlCallNode)); // original treetop gets deleted by createDiamondForCall
    TR::TransformUtil::createDiamondForCall(this, treetop, cmpCheckTreeTop, inlCallTreeTop, computedCallTreeTop, false,
        false);
    if (nullChkNode)
        computedCallTreeTop->insertBefore(TR::TreeTop::create(comp(), nullChkNode));
    ...
    TR::Node *node)
{
    ...
    TR::Node *vftNode = TR::Node::createWithSymRef(node, TR::aloadi, 1, receiver, vftSymRef);
    ...
    TR::Node *nullCheck = TR::Node::createWithSymRef(node, TR::NULLCHK, 1, vftNode, nullCheckSymRef);
    ...
    TR::SymbolReference *vmIndexSymRef = comp()->getSymRefTab()->findOrFabricateShadowSymbol(comp()->getMethodSymbol(),
        TR::Symbol::Java_lang_invoke_MemberName_vmindex, TR::Int64, comp()->fej9()->getVMIndexOffset(), false, false,
        true, "java/lang/invoke/MemberName.vmindex J");

    TR::Node *vftOffset = TR::Node::createWithSymRef(node, TR::lloadi, 1, memberNameNode, vmIndexSymRef);
    ...
    TR::Node *memberNameNode)
{
    ...
    TR_OpaqueMethodBlock *dispatchVirtual = fej9->getMethodFromName("com/ibm/jit/JITHelpers", "dispatchVirtual", "()V");

    int signatureLength;
    char *signature = getSignatureForComputedCall("JJ", "", comp(),
        node->getSymbol()->castToMethodSymbol()->getMethod()->signatureChars(), signatureLength);

    TR::ResolvedMethodSymbol *owningMethodSymbol = node->getSymbolReference()->getOwningMethodSymbol(comp());

    TR_ResolvedMethod *dispatchVirtualResolvedMethod = fej9->createResolvedMethodWithSignature(comp()->trMemory(),
        dispatchVirtual, NULL, signature, signatureLength, owningMethodSymbol->getResolvedMethod());

    TR::SymbolReference *dispatchVirtualSymRef
        = comp()->getSymRefTab()->findOrCreateMethodSymbol(owningMethodSymbol->getResolvedMethodIndex(), -1,
            dispatchVirtualResolvedMethod, TR::MethodSymbol::ComputedStatic);

    TR::ILOpCodes indirectCallOp = node->getSymbol()->castToMethodSymbol()->getMethod()->indirectCallOpCode();
    ...
    TR::Node *xconstSizeofJ9Class = comp()->target().is64Bit() ? TR::Node::lconst(node, sizeof(J9Class))
    ...
    TR::SymbolReference *genericIntShadow = comp()->getSymRefTab()->createGenericIntShadowSymbolReference(0);
...
void J9::RecognizedCallTransformer::process_java_lang_invoke_MethodHandle_linkToInterface(TR::TreeTop *treetop,
    TR::Node *node)
{
    ...
    TR::DebugCounter::prependDebugCounter(comp(),
        TR::DebugCounter::debugCounterName(comp(), "mh.unrefined/linkToInterface/(%s)/%s", comp()->signature(),
            comp()->getHotnessName()),
        treetop);
    ...
    TR::Node *vftOffset = TR::Node::createWithSymRef(node, comp()->target().is64Bit() ? TR::lcall : TR::icall, 2,
    ...
    TR::SymbolReference *vftSymRef = comp()->getSymRefTab()->findOrCreateVftSymbolRef();
    TR::Node *vftNode = TR::Node::createWithSymRef(node, TR::aloadi, 1, receiverNode, vftSymRef);

    vftOffset->setAndIncChild(1, vftNode);

    treetop->insertBefore(TR::TreeTop::create(comp(), TR::Node::create(node, TR::treetop, 1, vftOffset)));
    ...
    if (isILGenPass) {
        switch (rm) {
            case TR::sun_misc_Unsafe_getAndAddInt:
                return !comp()->getOption(TR_DisableUnsafe) && !comp()->compileRelocatableCode()
            ...
            case TR::sun_misc_Unsafe_getAndAddLong:
                return !comp()->getOption(TR_DisableUnsafe) && !comp()->compileRelocatableCode()
                    && !TR::Compiler->om.canGenerateArraylets() && comp()->target().is64Bit()
                    && cg()->supportsNonHelper(TR::SymbolReferenceTable::atomicFetchAndAddSymbol);
            ...
            case TR::java_lang_Class_cast: {
                static const bool disable = feGetEnv("TR_disableClassCastToCheckcast") != NULL;
