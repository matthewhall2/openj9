...
static X VarHandleMethods[] = {
    // We only get the original method name when the call is unresolved, but recognized method only works for resolved
    // method
    // The list for resolved method is in j9method.cpp, any changes in this list must be reflected in the other
    // Notice this list has a tableIndex field to indicate the index of MethodHandle/MethodType of VarHandle access
    // operation
        ...
        if (node && node->getOpCode().isCall() && !node->getSymbol()->castToMethodSymbol()->isHelper()) {
            ...
            if (varHandleAccessMethod != TR::unknownMethod) {
                if (comp()->compileRelocatableCode()) {
                    logprintf(doTrace, comp()->log(),
                        "A call to VarHandle access method %s is not supported in AOT. Aborting compile.\n",
                        comp()->getDebug()->getName(symRef));
                    comp()->failCompilation<J9::AOTHasInvokeVarHandle>(
                        "A call to a VarHandle access method is not supported in AOT. Aborting compile.");
                }

                if (comp()->getOption(TR_FullSpeedDebug) && !comp()->isPeekingMethod()) {
                    logprintf(doTrace, comp()->log(),
                        "A call to VarHandle access method %s is not supported in FSD. Failing ilgen.\n",
                        comp()->getDebug()->getName(symRef));
                    comp()->failCompilation<J9::FSDHasInvokeHandle>(
                        "A call to a VarHandle access method is not supported in FSD. Failing ilgen.");
                }
                ...
                if (callTree->getNode()->getOpCode().isCheck()) {
                    // Remove resolve check and preserve other checks, null check should be preserved with a separate
                    // tree
                    ...
                    if (hasNULLCHK) {
                        ...
                        TR::Node *checkNode = TR::Node::createWithSymRef(TR::NULLCHK, 1, 1, passthrough,
                ...
                TR::Node *index = TR::Node::iconst(
                    VarHandleMethods[varHandleAccessMethod - TR::java_lang_invoke_VarHandle_get].tableIndex);
                ...
                TR::SymbolReference *handleTableSymRef = comp()->getSymRefTab()->findOrFabricateShadowSymbol(
                    methodSymbol, TR::Symbol::Java_lang_invoke_VarHandle_handleTable, TR::Address, handleTableOffset,
                    false, false, true, "java/lang/invoke/VarHandle.handleTable [Ljava/lang/invoke/MethodHandle;");
                TR::Node *handleTable = TR::Node::createWithSymRef(comp()->il.opCodeForIndirectLoad(TR::Address), 1, 1,
                    varHandle, handleTableSymRef);
                handleTable->copyByteCodeInfo(node);

                if (comp()->useCompressedPointers())
                    callTree->insertBefore(
                        TR::TreeTop::create(comp(), TR::Node::createCompressedRefsAnchor(handleTable)));
                else
                    callTree->insertBefore(
                        TR::TreeTop::create(comp(), TR::Node::create(node, TR::treetop, 1, handleTable)));

                TR::Node *methodHandleAddr
                    = J9::TransformUtil::calculateElementAddress(comp(), handleTable, index, TR::Address);
                TR::Node *methodHandle = TR::Node::createWithSymRef(comp()->il.opCodeForIndirectArrayLoad(TR::Address),
                    1, 1, methodHandleAddr,
                    comp()->getSymRefTab()->findOrCreateArrayShadowSymbolRef(TR::Address, handleTable));
                ...
                if (comp()->useCompressedPointers())
                    callTree->insertBefore(
                        TR::TreeTop::create(comp(), TR::Node::createCompressedRefsAnchor(methodHandle)));
                else
                    callTree->insertBefore(
                        TR::TreeTop::create(comp(), TR::Node::create(node, TR::treetop, 1, methodHandle)));
                ...
                // Danger: using the compilation method as the owning method only because VarHandleTransformer is an
                // ilgen opt
                // TODO: check if optimizer::getMethodSymbol() works to avoid confusion
                TR::SymbolReference *callSiteMethodTypeSymRef
                    = comp()->getSymRefTab()->findOrCreateVarHandleMethodTypeTableEntrySymbol(methodSymbol, cpIndex);
                ...
                if (callSiteMethodTypeSymRef->isUnresolved()) {
                    TR::Node *resolveChkOnMethodType = TR::Node::createWithSymRef(TR::ResolveCHK, 1, 1,
                        callSiteMethodType, comp()->getSymRefTab()->findOrCreateResolveCheckSymbolRef(methodSymbol));
                    callTree->insertBefore(TR::TreeTop::create(comp(), resolveChkOnMethodType));
                }
                // Convert MH
                TR::SymbolReference *typeConversionSymRef = comp()->getSymRefTab()->methodSymRefFromName(methodSymbol,
                    JSR292_MethodHandle, JSR292_asType, JSR292_asTypeSig, OMR::MethodSymbol::Static);
                TR::Node *convertedMethodHandle = TR::Node::createWithSymRef(TR::acall, 2, 2, methodHandle,
                    callSiteMethodType, typeConversionSymRef);
                convertedMethodHandle->copyByteCodeInfo(node);
                callTree->insertBefore(
                    TR::TreeTop::create(comp(), TR::Node::create(node, TR::treetop, 1, convertedMethodHandle)));

                // Get the method address
                TR::SymbolReference *invokeExactTargetAddrSymRef = comp()->getSymRefTab()->methodSymRefFromName(
                    methodSymbol, JSR292_MethodHandle, JSR292_invokeExactTargetAddress,
                    JSR292_invokeExactTargetAddressSig, OMR::MethodSymbol::Special);
                TR::Node *invokeExactTargetAddr
                    = TR::Node::createWithSymRef(TR::lcall, 1, 1, convertedMethodHandle, invokeExactTargetAddrSymRef);
                invokeExactTargetAddr->copyByteCodeInfo(node);
                callTree->insertBefore(
                    TR::TreeTop::create(comp(), TR::Node::create(node, TR::treetop, 1, invokeExactTargetAddr)));
                ...
                TR::SymbolReference *invokeExactOriginal = comp()->getSymRefTab()->methodSymRefFromName(methodSymbol,
                    JSR292_MethodHandle, JSR292_invokeExact, JSR292_invokeExactSig, OMR::MethodSymbol::ComputedVirtual);
                TR::SymbolReference *invokeExactSymRef = comp()->getSymRefTab()->methodSymRefWithSignature(
                    invokeExactOriginal, mhSig, sigLen + VarHandleParmLength);
                ...
                TR::list<TR::Node *> *nodeList = new (comp()->trStackMemory())
                ...
                int32_t numChildren = method->numberOfExplicitParameters()
                    + 3; // The extra 3 arguments are: the target method address, the MH receiver, the VarHandle object

                prepareToReplaceNode(
                    node); // This will remove the usedef info, valueNumber info and all children of the node
                TR::Node *invokeCall = TR::Node::recreateWithoutProperties(node, method->indirectCallOpCode(),
                    numChildren, invokeExactSymRef);
...
const char *TR_VarHandleTransformer::optDetailString() const throw() { return "O^O VARHANDLE TRANSFORMER: "; }
