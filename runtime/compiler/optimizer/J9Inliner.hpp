...
protected:
    virtual int32_t scaleSizeBasedOnBlockFrequency(int32_t bytecodeSize, int32_t frequency, int32_t borderFrequency,
        TR_ResolvedMethod *calleeResolvedMethod, TR::Node *callNode, int32_t coldBorderFrequency = 0);
...
private:
     ...
     *
     * \notes
     *    Call targets are chopped off from \ref _callTargets list because of global budget limit like node counts and
     * total weight. This function chooses to keep some chopped of targets if they meet certain conditions.
     */
    void processChoppedOffCallTargets(TR_CallTarget *lastTargetToInline, TR_CallTarget *firstChoppedOffcalltarget,
     ...
     *   function provides a mechanism for examining call targets and evaluating whether it is safe to skip counting
     * nodes.
     *
     * \param callTarget
...
public:
    ...
    virtual void adjustByteCodeSize(TR_ResolvedMethod *calleeResolvedMethod, bool isInLoop, TR::Block *block,
        int &bytecodeSize);
    virtual void adjustCallerWeightLimit(TR::ResolvedMethodSymbol *callSymbol, int &callerWeightLimit);
    virtual void adjustMethodByteCodeSizeThreshold(TR::ResolvedMethodSymbol *callSymbol,
    ...
    virtual TR_PrexArgInfo *createPrexArgInfoForCallTarget(TR_VirtualGuardSelection *guard,
    ...
    static TR_PrexArgInfo *computePrexInfo(TR_InlinerBase *inliner, TR_CallSite *site,
        TR_PrexArgInfo *callerArgInfo = NULL);
    virtual void refineInlineGuard(TR::Node *callNode, TR::Block *&block1, TR::Block *&block2, bool &appendTestToBlock1,
        TR::ResolvedMethodSymbol *callerSymbol, TR::TreeTop *cursorTree, TR::TreeTop *&virtualGuard, TR::Block *block4);
    virtual void refineInliningThresholds(TR::Compilation *comp, int32_t &callerWeightLimit,
...
protected:
    ...
    virtual void computeMethodBranchProfileInfo(TR::Block *cfgBlock, TR_CallTarget *calltarget,
        TR::ResolvedMethodSymbol *callerSymbol);
    virtual TR_InnerPreexistenceInfo *createInnerPrexInfo(TR::Compilation *c, TR::ResolvedMethodSymbol *methodSymbol,
        TR_CallStack *callStack, TR::TreeTop *callTree, TR::Node *callNode, TR_VirtualGuardKind guardKind);
    virtual void estimateAndRefineBytecodeSize(TR_CallSite *callsite, TR_CallTarget *target, TR_CallStack *callStack,
        int32_t &bytecodeSize);
    virtual TR_TransformInlinedFunction *getTransformInlinedFunction(TR::ResolvedMethodSymbol *,
        TR::ResolvedMethodSymbol *, TR::Block *, TR::TreeTop *, TR::Node *, TR_ParameterToArgumentMapper &,
        TR_VirtualGuardSelection *, List<TR::SymbolReference> &, List<TR::SymbolReference> &,
...
public:
    ...
    bool adjustFanInSizeInExceedsSizeThreshold(int bytecodeSize, uint32_t &calculatedSize, TR_ResolvedMethod *callee,
        TR_ResolvedMethod *caller, int32_t bcIndex);
    void adjustFanInSizeInWeighCallSite(int32_t &weight, int32_t size, TR_ResolvedMethod *callee,
        TR_ResolvedMethod *caller, int32_t bcIndex);
    ...
    virtual int32_t getInitialBytecodeSize(TR_ResolvedMethod *feMethod, TR::ResolvedMethodSymbol *methodSymbol,
        TR::Compilation *comp);
...
protected:
    ...
    TR::Node *inlineGetClassAccessFlags(TR::ResolvedMethodSymbol *, TR::ResolvedMethodSymbol *, TR::TreeTop *,
        TR::Node *);
     ...
     */
    TR::Block *createUnsafeGetPutCallDiamond(TR::TreeTop *callNodeTreeTop, TR::TreeTop *comparisonTree,
        TR::TreeTop *branchTargetTree, TR::TreeTop *fallThroughTree);
    bool createUnsafePutWithOffset(TR::ResolvedMethodSymbol *, TR::ResolvedMethodSymbol *, TR::TreeTop *, TR::Node *,
    ...
    void createTempsForUnsafePutGet(TR::Node *&unsafeAddress, TR::Node *unsafeCall, TR::TreeTop *callNodeTreeTop,
        TR::Node *&offset, TR::SymbolReference *&newSymbolReferenceForAddress, bool isUnsafeGet);
    bool createUnsafeGet(TR::ResolvedMethodSymbol *, TR::ResolvedMethodSymbol *, TR::TreeTop *, TR::Node *,
        TR::DataType, bool compress = true);
    bool createUnsafePut(TR::ResolvedMethodSymbol *, TR::ResolvedMethodSymbol *, TR::TreeTop *, TR::Node *,
        TR::DataType, bool compress = true);
    TR::Node *createUnsafeAddress(TR::Node *);
    bool createUnsafeGetWithOffset(TR::ResolvedMethodSymbol *, TR::ResolvedMethodSymbol *, TR::TreeTop *, TR::Node *,
        TR::DataType, TR::Symbol::MemoryOrdering ordering = TR::Symbol::MemoryOrdering::Transparent,
        bool needNullCheck = false, bool isUnaligned = false);
     ...
     *           ordered method
     */
    void genCodeForUnsafeGetPut(TR::Node *unsafeAddress, TR::Node *unsafeOffset, TR::DataType type,
    ...
    bool _tryToGenerateILForMethod(TR::ResolvedMethodSymbol *calleeSymbol, TR::ResolvedMethodSymbol *callerSymbol,
        TR_CallTarget *calltarget);
    bool doCorrectnessAndSizeChecksForInlineCallTarget(TR_CallStack *callStack, TR_CallTarget *calltarget,
        bool inlinefromgraph, TR_PrexArgInfo *argInfo);
    ...
    virtual TR_InlinerFailureReason checkIfTargetInlineable(TR_CallTarget *target, TR_CallSite *callsite,
        TR::Compilation *comp);
    /** \brief
...
public:
    TR_J9TransformInlinedFunction(TR::Compilation *c, TR_InlinerTracer *tracer, TR::ResolvedMethodSymbol *callerSymbol,
        TR::ResolvedMethodSymbol *calleeSymbol, TR::Block *callNodeBlock, TR::TreeTop *callNodeTreeTop,
        TR::Node *callNode, TR_ParameterToArgumentMapper &mapper, TR_VirtualGuardSelection *guard,
...
private:
    ...
    TR::Block *appendCatchBlockForInlinedSyncMethod(TR_ResolvedMethod *, TR::TreeTop *, int32_t, int32_t,
        bool addBlocks = true);
    bool isSyncReturnBlock(TR::Compilation *comp, TR::Block *b);
    // { RTSJ Support begins
    void wrapCalleeInTryRegion(bool, bool, TR_ResolvedMethod *);
    TR::TreeTop *createThrowCatchBlock(bool, bool, TR::CFG *, TR::Block *, TR::TreeTop *, TR::SymbolReference *,
        int32_t, TR_ScratchList<TR::Block> &newCatchBlocks);
    TR::Block *appendCatchBlockToRethrowException(TR_ResolvedMethod *, TR::TreeTop *, bool, int32_t, int32_t,
        bool addBlocks = true);
    // } RTSJ Support ends
};
...
public:
    TR_J9InnerPreexistenceInfo(TR::Compilation *c, TR::ResolvedMethodSymbol *methodSymbol, TR_CallStack *callStack,
        TR::TreeTop *callTree, TR::Node *callNode, TR_VirtualGuardKind _guardKind);
