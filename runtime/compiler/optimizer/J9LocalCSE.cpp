...
bool J9::LocalCSE::shouldTransformBlock(TR::Block *block)
{
    if (!OMR::LocalCSE::shouldTransformBlock(block))
        return false;

    if (self()->comp()->getMethodHotness() < warm && block->getFrequency() < TR::Options::_localCSEFrequencyThreshold
...
bool J9::LocalCSE::shouldCommonNode(TR::Node *parent, TR::Node *node)
{
    ...
    if (parent != NULL) {
        ...
        // true then the evaluation of BCDCHK may or may not raise a Java exception. The issue with this is the
        ...
        // n6n     iconst 1
        //
        // Assuming n2n and n5n represent the same operation local CSE will attempt to common n2n with n5n. However
        // given the above this can result in unwanted behavior. Commoning n5n with n2n is incorrect because the BCDCHK
        // at n4n had checkOverflow argument as true (n6n) and thus the original DAA Java calls were in fact different.
        // Assuming the BCDCHKs at n1n and n4n both raised hardware interrupts the fallback OOL path in case of n1n
        // would have not raised a Java exception and the execution would resume as normal. However in case on n6n the
        // fallback OOL path may or may not raise a Java exception so execution could in fact diverge to a different
        // path. Hence it is not correct to common n5n and n2n in such cases.
        //
        ...
        if (parent->getOpCodeValue() == TR::BCDCHK && parent->getFirstChild() == node) {
            ...
            TR_ASSERT(bcdchkMethodSymbol != NULL,
                "BCDCHK should always have a resolved method symbol since it was reduced in DataAccessAccelerator "
                "optimization");
                ...
                case TR::com_ibm_dataaccess_DecimalData_convertLongToPackedDecimal_ByteBuffer_: {
                    ...
                    if (!(checkOverflowNode->getOpCode().isLoadConst() && checkOverflowNode->getConstValue() == 0)) {
                        logprintf(trace(), comp()->log(),
                            "Skipping propagation of %s [%p] into the first child of %s [%p] because of potential "
                            "overflow checking\n",
...
bool J9::LocalCSE::shouldCopyPropagateNode(TR::Node *parent, TR::Node *node, int32_t childNum, TR::Node *storeNode)
{
    ...
    // However due to late arrayOp scalarization there is a chance that CSE may create incorrect types during
    // propagation so prevent these transformations from happening. The cost of adding a conversion to make things type
    // correct is too high here (a runtime call) so disallow these.
    ...
    if (parent && parent->getChild(childNum)) {
        ...
        if (oldType.isBCD() != newType.isBCD() || oldType.isFloatingPoint() != newType.isFloatingPoint())
        ...
        if (!propagationIsTypeCorrect && (comp()->cg()->traceBCDCodeGen() || trace())) {
            ...
            comp()->log()->printf("z^z : skipping type invalid propagation : parent %s (%p), rhsOfStoreDefNode %s (%p) "
                                  "line_no=%d (offset %06X)\n",
