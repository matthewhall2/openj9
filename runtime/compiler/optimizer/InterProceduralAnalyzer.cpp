    ...
    , _globalsWrittenInCurrentPeek(trMemory())
{
    ...
    _classesThatShouldNotBeNewlyExtendedInCurrentPeekHT
        = (TR_ScratchList<TR_ClassExtendCheck> *)trMemory()->allocateHeapMemory(
            sizeof(TR_ScratchList<TR_ClassExtendCheck>) * (CLASSHASHTABLE_SIZE + 1));

    memset(_classesThatShouldNotBeNewlyExtendedInCurrentPeekHT, 0,
        sizeof(TR_ScratchList<TR_ClassExtendCheck>) * (CLASSHASHTABLE_SIZE + 1));
    ...
    _classesThatShouldNotBeNewlyExtendedHT = (TR_LinkHead<TR_ClassExtendCheck> *)trMemory()->allocateHeapMemory(
        sizeof(TR_LinkHead<TR_ClassExtendCheck>) * (CLASSHASHTABLE_SIZE + 1));
    memset(_classesThatShouldNotBeNewlyExtendedHT, 0,
        sizeof(TR_LinkHead<TR_ClassExtendCheck>) * (CLASSHASHTABLE_SIZE + 1));
}

int32_t TR::InterProceduralAnalyzer::perform() { return 2; }

bool TR::InterProceduralAnalyzer::capableOfPeekingVirtualCalls()
{
    if (comp()->performVirtualGuardNOPing() && !comp()->getOption(TR_DisableIPA))
        return true;
    return false;
}
...
{
    if (comp()->isProfilingCompilation() || !capableOfPeekingVirtualCalls())
        return 0;
    ...
    while (currCec) {
        TR_PersistentClassInfo *cl = comp()->getPersistentInfo()->getPersistentCHTable()->findClassInfoAfterLocking(
            currCec->getData()->_clazz, comp());
...
List<OMR::RuntimeAssumption> *TR::InterProceduralAnalyzer::analyzeCallGraph(TR::Node *callNode, bool *success)
{
    ...
    if (_sniffDepth >= _maxSniffDepth) {
        ...
        *success = false;
        ...
        logprints(trace(), log, "High sniff depth made peek unsuccessful\n");
    ...
    if (!resolvedMethodSymbol && !methodSymbol->isInterface()) {
        *success = false;
        logprintf(trace(), log, "Unresolved non-interface call node %p made peek unsuccessful\n", callNode);

        return 0;
    }

    if ((*success) && callNode->getOpCode().isIndirect() && !capableOfPeekingVirtualCalls()) {
        *success = false;
        return 0;
    }
    ...
    if (!resolvedMethodSymbol) // as per current logic, it is an interface method
    {
        ...
        if (!clazz) {
            ...
            if (!s) {
                *success = false;
                logprintf(trace(), log,
                    "Found unresolved method call node %p while peeking whose class is unresolved and unable to add "
                    "assumption -- peek unsuccessful\n",
                    callNode);
            } else {
                addClassThatShouldNotBeLoaded(s, len);
                logprintf(trace(), log, "Found unresolved method call node %p while peeking -- add assumption\n",
                    callNode);
            }
            return 0;
        }
    ...
    if ((*success) && callNode->getOpCode().isIndirect()) {
        ...
        if (clazz) {
            if (!addClassThatShouldNotBeNewlyExtended(clazz)) {
                logprints(trace(), log, "Could not add Class That should not be newly extended to assumptions list.\n");
            ...
            logprints(trace(), log,
                "Found class for this object -- add assumption that the class should not be newly extended\n");
        }
        ...
        if (classInfo) {
            ...
            logprintf(trace(), log, "Number of subclasses = %d\n", subClasses.getSize());
...
List<OMR::RuntimeAssumption> *TR::InterProceduralAnalyzer::analyzeMethod(TR::Node *callNode, TR_ResolvedMethod *method,
    bool *success)
{
    OMR::Logger *log = comp()->log();

    logprintf(trace(), log, "Consider method %s for peek\n", method->signature(trMemory()));
    ...
    if (0 && alreadyPeekedMethod(method, &successfulPriorPeek, &priorPeek)) {
        if (!successfulPriorPeek) {
            *success = false;
            ...
            logprintf(trace(), log, "Prior peek failure for method %s caused current peek to be unsuccessful\n",
                method->signature(trMemory()));
        } else {
            logprintf(trace(), log,
                "Prior peek success for method %s caused current peek to be successful -- prior assumptions added\n",
                method->signature(trMemory()));

            for (TR_ClassLoadCheck *clc = priorPeek->_classesThatShouldNotBeLoaded.getFirst(); clc;
                 clc = clc->getNext())
                addClassThatShouldNotBeLoaded(clc->_name, clc->_length);

            for (TR_ClassExtendCheck *cec = priorPeek->_classesThatShouldNotBeNewlyExtended.getFirst(); cec;
                 cec = cec->getNext())
    ...
    if (bytecodeSize > MAX_SNIFF_BYTECODE_SIZE) {
        *success = false;
        logprintf(trace(), log, "Large bytecode size %d made peek unsuccessful\n", bytecodeSize);
    ...
    if (method->isStatic())
        newSymRef = comp()->getSymRefTab()->findOrCreateMethodSymbol(symRef->getOwningMethodIndex(), -1, method,
    ...
    if (!resolvedMethodSymbol->getFirstTreeTop()) {
        ...
        const char **argInfo = (const char **)trMemory()->allocateHeapMemory(
            numRealArgs * sizeof(char *)); // can stack allocate in some cases
        memset(argInfo, 0, numRealArgs * sizeof(char *));
        int32_t *lenInfo = (int32_t *)trMemory()->allocateHeapMemory(
            numRealArgs * sizeof(int32_t)); // can stack allocate in some cases
        ...
        for (int32_t c = callNode->getNumChildren() - 1; c >= firstArgIndex; --c) {
            ...
            if (argument->getDataType() == TR::Address) {
                ...
                if (s && (c == firstArgIndex)) {
                    TR_OpaqueClassBlock *argClazz
                        = fe()->getClassFromSignature(s, len, symRef->getOwningMethod(comp()));
                    TR_OpaqueClassBlock *thisClazz = method->containingClass();

                    if (!argClazz || !thisClazz) {
                        *success = false;
                        logprints(trace(), log,
                            "The call argument class is NULL, bailing out. (probably because of different class "
                            "loaders)\n");
        ...
        if (!performTransformation(comp(),
                "O^O INTERPROCEDURAL ANALYZER: Peeking into the IL for doing a limited form of interprocedural "
                "analysis  \n"))
            return 0;

        TR_PeekingArgInfo *peekInfo = (TR_PeekingArgInfo *)trMemory()->allocateStackMemory(
            sizeof(TR_PeekingArgInfo)); // can stack allocate in some cases
    ...
    for (treeTop = resolvedMethodSymbol->getFirstTreeTop(); treeTop; treeTop = treeTop->getNextTreeTop()) {
        ...
        if (!*success) {
            logprintf(trace(), log, "Node %p made peek unsuccessful\n", node);
            break;
        }
    ...
    if (0 && *success) {
        ...
        while (currClc != prevClc) {
            priorPeek->_classesThatShouldNotBeLoaded.add(
                new (trHeapMemory()) TR_ClassLoadCheck(currClc->getData()->_name, currClc->getData()->_length));
            currClc = currClc->getNextElement();
        }

        while (currCec != prevCec) {
            priorPeek->_classesThatShouldNotBeNewlyExtended.add(
                new (trHeapMemory()) TR_ClassExtendCheck(currCec->getData()->_clazz));
            currCec = currCec->getNextElement();
        }
        ...
        logprintf(trace(), log, "Method %s is successfully peeked\n",
            resolvedMethodSymbol->getResolvedMethod()->signature(trMemory()));
    } else {
        if (_sniffDepth == 0) {
            logprintf(trace(), log, "1Method %s is unsuccessfully peeked\n",
                resolvedMethodSymbol->getResolvedMethod()->signature(trMemory()));
            ...
            if (!_maxSniffDepthExceeded) {
                logprintf(trace(), log, "2Method %s is unsuccessfully peeked\n",
                    resolvedMethodSymbol->getResolvedMethod()->signature(trMemory()));
    ...
    for (i = 0; i <= topOfStack; i++) {
        ...
        if (peekInfo) {
            ...
            /*
              ...
              TR_PersistentClassInfo *classInfo =
              comp()->getPersistentInfo()->getPersistentCHTable()->findClassInfoAfterLocking(clazz, comp()); if
              (classInfo)
              {
              ...
              ListIterator<TR_PersistentClassInfo> subClassesIt(&subClasses);
              for (TR_PersistentClassInfo *subClassInfo = subClassesIt.getFirst(); subClassInfo; subClassInfo =
              subClassesIt.getNext())
              {
...
bool TR::InterProceduralAnalyzer::alreadyPeekedMethod(TR_ResolvedMethod *method, bool *success,
    TR::PriorPeekInfo **priorPeek)
{
    ListIterator<TR::PriorPeekInfo> priorPeekedMethodsIt(&_successfullyPeekedMethods);
    TR::PriorPeekInfo *priorPeekedMethod = NULL;
    for (priorPeekedMethod = priorPeekedMethodsIt.getFirst(); priorPeekedMethod;
         priorPeekedMethod = priorPeekedMethodsIt.getNext()) {
...
bool TR::InterProceduralAnalyzer::addClassThatShouldNotBeLoaded(char *name, int32_t len)
{
    bool found = false;

    for (ListElement<TR_ClassLoadCheck> *currClc = _classesThatShouldNotBeLoadedInCurrentPeek.getListHead();
         currClc != _prevClc; currClc = currClc->getNextElement()) {
...
bool TR::InterProceduralAnalyzer::addClassThatShouldNotBeNewlyExtended(TR_OpaqueClassBlock *clazz)
{
...
bool TR::InterProceduralAnalyzer::addSingleClassThatShouldNotBeNewlyExtended(TR_OpaqueClassBlock *clazz)
{
    ...
    for (ListElement<TR_ClassExtendCheck> *currCec
         = _classesThatShouldNotBeNewlyExtendedInCurrentPeekHT[hashNum].getListHead();
         currCec != _prevCec; currCec = currCec->getNextElement()) {
    ...
    if (!found) {
        ...
        _classesThatShouldNotBeNewlyExtendedInCurrentPeekHT[hashNum].add(
            new (trStackMemory()) TR_ClassExtendCheck(clazz));
    }

    found = false;
    for (TR_ClassExtendCheck *clc = _classesThatShouldNotBeNewlyExtendedHT[hashNum].getFirst(); clc;
         clc = clc->getNext())
        if (clc->_clazz == clazz) {
            found = true;
            break;
...
bool TR::InterProceduralAnalyzer::addWrittenGlobal(TR::SymbolReference *symRef)
{
    ...
    for (ListElement<TR::GlobalSymbol> *currSymRef = _globalsWrittenInCurrentPeek.getListHead();
        ...
        if (currSymReference->getSymbol()->isStaticField())
            ...
            currSig = currSymReference->getOwningMethod(comp())->fieldName(currSymReference->getCPIndex(), currLength,
                trMemory());
...
bool TR::InterProceduralAnalyzer::addMethodThatShouldNotBeNewlyOverridden(TR_OpaqueMethodBlock *method)
{
    //
