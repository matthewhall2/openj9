...
TR_PCISCNode *createIdiomIOP2VarInLoop(TR_PCISCGraph *tgt, int32_t ctrl, int dagId, TR_PCISCNode *pred, int opcode,
    TR_PCISCNode *v, TR_PCISCNode *src1, TR_PCISCNode *val)
{
    TR_PCISCNode *n0, *n1;
    TR_ASSERT(v->getOpcode() == TR_variable || v->getOpcode() == TR::iload, "Error!");
    n0 = new (PERSISTENT_NEW) TR_PCISCNode(tgt->trMemory(), opcode,
        v->getOpcode() == TR_variable ? TR::NoType : TR::Int32, tgt->incNumNodes(), dagId, 1, 2, pred);
    tgt->addNode(n0);
    n1 = new (PERSISTENT_NEW) TR_PCISCNode(tgt->trMemory(), TR::istore, TR::Int32, tgt->incNumNodes(), dagId, 1, 2, n0);
    tgt->addNode(n1);
...
//*****************************************************************************************
TR_PCISCNode *createIdiomIOP2VarInLoop(TR_PCISCGraph *tgt, int32_t ctrl, int dagId, TR_PCISCNode *pred, int opcode,
    TR_PCISCNode *v, TR_PCISCNode *val)
{
...
TR_PCISCNode *createIdiomIncVarInLoop(TR_PCISCGraph *tgt, int32_t ctrl, int dagId, TR_PCISCNode *pred, TR_PCISCNode *v,
    TR_PCISCNode *src1, TR_PCISCNode *addval)
{
...
TR_PCISCNode *createIdiomArrayRelatedConst(TR_PCISCGraph *tgt, int32_t ctrl, uint16_t id, int dagId, int32_t val)
{
    ...
    ret = new (PERSISTENT_NEW)
        TR_PCISCNode(tgt->trMemory(), opcode, opcode == TR::lconst ? TR::Int64 : TR::Int32, id, dagId, 0, 0, val);
    tgt->addNode(ret);
    return ret;
}
...
{
    return createIdiomArrayRelatedConst(tgt, ctrl, id, dagId,
    ...
    TR_PCISCNode *node)
{
    ...
    if ((ctrl & (CISCUtilCtl_64Bit | CISCUtilCtl_NoI2L)) == (CISCUtilCtl_64Bit | 0)) {
        ret = new (PERSISTENT_NEW)
            TR_PCISCNode(tgt->trMemory(), TR::i2l, TR::Int64, tgt->incNumNodes(), dagId, 1, 1, *pred, node);
        tgt->addNode(ret);
        *pred = ret;
    }
    return ret;
...
TR_PCISCNode *createIdiomArrayAddressIndexTreeInLoop(TR_PCISCGraph *tgt, int32_t ctrl, int dagId, TR_PCISCNode *pred,
    TR_PCISCNode *index, TR_PCISCNode *cmah, TR_PCISCNode *constForMul)
{
    TR_PCISCNode *n0, *mul;
    if (ctrl & CISCUtilCtl_64Bit) {
        TR_PCISCNode *i2l;
        if (ctrl & CISCUtilCtl_NoI2L) {
            mul = new (PERSISTENT_NEW)
                TR_PCISCNode(tgt->trMemory(), TR::lmul, TR::Int64, tgt->incNumNodes(), dagId, 1, 2, pred);
            tgt->addNode(mul);
            i2l = mul;
        } else {
            i2l = new (PERSISTENT_NEW)
                TR_PCISCNode(tgt->trMemory(), TR::i2l, TR::Int64, tgt->incNumNodes(), dagId, 1, 1, pred);
            tgt->addNode(i2l);
            mul = new (PERSISTENT_NEW)
                TR_PCISCNode(tgt->trMemory(), TR::lmul, TR::Int64, tgt->incNumNodes(), dagId, 1, 2, i2l);
            tgt->addNode(mul);
            mul->setIsChildDirectlyConnected();
            mul->setChild(i2l);
        }
        n0 = new (PERSISTENT_NEW)
            TR_PCISCNode(tgt->trMemory(), TR::lsub, TR::Int64, tgt->incNumNodes(), dagId, 1, 2, mul);
        tgt->addNode(n0);
        i2l->setChild(index);
        ...
        mul = new (PERSISTENT_NEW)
            TR_PCISCNode(tgt->trMemory(), TR::imul, TR::Int32, tgt->incNumNodes(), dagId, 1, 2, pred);
        tgt->addNode(mul);
        n0 = new (PERSISTENT_NEW)
            TR_PCISCNode(tgt->trMemory(), TR::isub, TR::Int32, tgt->incNumNodes(), dagId, 1, 2, mul);
        tgt->addNode(n0);
        mul->setChild(index);
        n0->setIsChildDirectlyConnected();
        switch (index->getOpcode()) {
...
TR_PCISCNode *createIdiomArrayAddressInLoop(TR_PCISCGraph *tgt, int32_t ctrl, int dagId, TR_PCISCNode *pred,
    TR_PCISCNode *base, TR_PCISCNode *index, TR_PCISCNode *cmah, TR_PCISCNode *constForMul)
{
    TR_PCISCNode *n0, *n1;
    n0 = createIdiomArrayAddressIndexTreeInLoop(tgt, ctrl, dagId, pred, index, cmah, constForMul);
    n1 = new (PERSISTENT_NEW) TR_PCISCNode(tgt->trMemory(), (ctrl & CISCUtilCtl_64Bit) ? TR::aladd : TR::aiadd,
        TR::Address, tgt->incNumNodes(), dagId, 1, 2, n0);
    tgt->addNode(n1);
    n1->setChildren(base, n0);
    if (base->getOpcode() == TR_variable || base->getOpcode() == TR_arraybase)
        n1->setIsChildDirectlyConnected();
    ...
    TR_PCISCNode *base, TR_PCISCNode *indexTree)
{
    ...
    n1 = new (PERSISTENT_NEW) TR_PCISCNode(tgt->trMemory(), (ctrl & CISCUtilCtl_64Bit) ? TR::aladd : TR::aiadd,
        TR::Address, tgt->incNumNodes(), dagId, 1, 2, pred);
    tgt->addNode(n1);
...
TR_PCISCNode *createIdiomArrayLoadInLoop(TR_PCISCGraph *tgt, int32_t ctrl, int dagId, TR_PCISCNode *pred, int opcode,
    TR::DataType dataType, TR_PCISCNode *base, TR_PCISCNode *index, TR_PCISCNode *cmah, TR_PCISCNode *mulconst)
{
    TR_PCISCNode *n1, *n2;
    n1 = createIdiomArrayAddressInLoop(tgt, ctrl, dagId, pred, base, index, cmah, mulconst);
    n2 = new (PERSISTENT_NEW) TR_PCISCNode(tgt->trMemory(), opcode, dataType, tgt->incNumNodes(), dagId, 1, 1, n1);
    tgt->addNode(n2);
...
TR_PCISCNode *createIdiomArrayStoreBodyInLoop(TR_PCISCGraph *tgt, int32_t ctrl, int dagId, TR_PCISCNode *pred,
    int opcode, TR::DataType dataType, TR_PCISCNode *addr, TR_PCISCNode *storeval)
{
    TR_PCISCNode *n2, *i2c;
    if (ctrl & CISCUtilCtl_NoConversion) {
        i2c = storeval;
    } else {
        i2c = new (PERSISTENT_NEW)
            TR_PCISCNode(tgt->trMemory(), (opcode == TR::sstorei) ? (TR_CISCOps)TR::i2s : TR_conversion,
                (opcode == TR::sstorei) ? TR::Int16 : TR::NoType, tgt->incNumNodes(), dagId, 1, 1, pred);
        tgt->addNode(i2c);
        i2c->setIsOptionalNode();
        pred = i2c;
        i2c->setChild(storeval);
    }
    n2 = new (PERSISTENT_NEW) TR_PCISCNode(tgt->trMemory(), opcode, dataType, tgt->incNumNodes(), dagId, 1, 2, pred);
    tgt->addNode(n2);
...
TR_PCISCNode *createIdiomArrayStoreInLoop(TR_PCISCGraph *tgt, int32_t ctrl, int dagId, TR_PCISCNode *pred, int opcode,
    TR::DataType dataType, TR_PCISCNode *base, TR_PCISCNode *index, TR_PCISCNode *cmah, TR_PCISCNode *const2,
...
{
    return createIdiomArrayStoreInLoop(tgt, ctrl, dagId, pred, TR::sstorei, TR::Int16, base, index, cmah, const2,
        storeval);
}
    ...
    TR_PCISCNode *base, TR_PCISCNode *index)
{
    ...
    if (ctrl & CISCUtilCtl_64Bit) {
        if (ctrl & CISCUtilCtl_NoI2L) {
            n0 = new (PERSISTENT_NEW)
                TR_PCISCNode(tgt->trMemory(), TR::ladd, TR::Int64, tgt->incNumNodes(), dagId, 1, 2, pred);
            tgt->addNode(n0);
            parentIndex = n0;
        } else {
            parentIndex = new (PERSISTENT_NEW)
                TR_PCISCNode(tgt->trMemory(), TR::i2l, TR::Int64, tgt->incNumNodes(), dagId, 1, 1, pred);
            tgt->addNode(parentIndex);
            n0 = new (PERSISTENT_NEW)
                TR_PCISCNode(tgt->trMemory(), TR::ladd, TR::Int64, tgt->incNumNodes(), dagId, 1, 2, parentIndex);
            tgt->addNode(n0);
            n0->setChild(parentIndex);
            n0->setIsChildDirectlyConnected();
        }
        parentIndex->setChild(index);
        if ((ctrl & CISCUtilCtl_ChildDirectConnected) || index->getOpcode() == TR_variable
            || index->getOpcode() == TR_arrayindex)
            parentIndex->setIsChildDirectlyConnected();
    } else {
        parentIndex = new (PERSISTENT_NEW)
            TR_PCISCNode(tgt->trMemory(), TR::l2i, TR::Int32, tgt->incNumNodes(), dagId, 1, 1, pred);
        tgt->addNode(parentIndex);
        parentIndex->setChild(base);
        parentIndex->setIsChildDirectlyConnected();
        base = parentIndex;
        n0 = new (PERSISTENT_NEW)
            TR_PCISCNode(tgt->trMemory(), TR::iadd, TR::Int32, tgt->incNumNodes(), dagId, 1, 2, parentIndex);
        tgt->addNode(n0);
        n0->setChild(index);
        if ((ctrl & CISCUtilCtl_ChildDirectConnected) || index->getOpcode() == TR_variable
            || index->getOpcode() == TR_arrayindex)
    ...
    n0->setChild(1, base);
    n2 = new (PERSISTENT_NEW) TR_PCISCNode(tgt->trMemory(), TR::bloadi, TR::Int8, tgt->incNumNodes(), dagId, 1, 1, n0);
    tgt->addNode(n2);
    ...
    TR_PCISCNode *src1, TR_PCISCNode *src2, TR_PCISCNode *c2, TR_PCISCNode *c31)
{
    ...
    if (isDiv2Mul) {
        TR_PCISCNode *nmul = new (PERSISTENT_NEW)
            TR_PCISCNode(tgt->trMemory(), TR::imulh, TR::Int32, tgt->incNumNodes(), 1, 1, 2, pred, src1, src2);
        tgt->addNode(nmul);
        TR_PCISCNode *nshr = new (PERSISTENT_NEW)
            TR_PCISCNode(tgt->trMemory(), TR::ishr, TR::Int32, tgt->incNumNodes(), 1, 1, 2, nmul, nmul, c2);
        tgt->addNode(nshr);
        TR_PCISCNode *ushr = new (PERSISTENT_NEW)
            TR_PCISCNode(tgt->trMemory(), TR::iushr, TR::Int32, tgt->incNumNodes(), 1, 1, 2, nshr, src1, c31);
        tgt->addNode(ushr); // optional
        ret = new (PERSISTENT_NEW)
            TR_PCISCNode(tgt->trMemory(), TR::iadd, TR::Int32, tgt->incNumNodes(), 1, 1, 2, ushr, nshr, ushr);
        tgt->addNode(ret); // optional
        ushr->setIsOptionalNode();
        ushr->setSkipParentsCheck();
        ret->setIsOptionalNode();
    } else {
        ret = new (PERSISTENT_NEW)
            TR_PCISCNode(tgt->trMemory(), TR::idiv, TR::Int32, tgt->incNumNodes(), 1, 1, 2, pred, src1, src2);
        tgt->addNode(ret);
    }
    return ret;
}
...
bool searchNodeInBlock(TR_CISCNode *start, TR_CISCNode *target)
{
    ...
    while (t->getNumSuccs() == 1 && t->getPreds()->isSingleton()) {
        ...
        t = t->getSucc(0);
        if (t == start)
            break; // maybe, this condition never happen.
    }
...
bool checkSuccsSet(TR_CISCTransformer * const trans, TR_CISCNode *t, TR_BitVector * const pBV)
{
    ...
    while (t->getNumSuccs() == 1) {
        ...
        if (!t->isNegligible()) {
            ...
            for (p = li.getFirst(); p; p = li.getNext())
                if (pBV->isSet(p->getID()))
                    return true; // found the pattern node in pBV
    ...
    for (int i = t->getNumSuccs(); --i >= 0;) {
        ...
        if (!tn->isNegligible()) {
            ...
            for (p = li.getFirst(); p; p = li.getNext())
                if (pBV->isSet(p->getID()))
                    break; // found the pattern node in pBV
            if (!p)
                return false; // cannot find the pattern node.
        } else {
            if (!checkSuccsSet(trans, tn, pBV))
                return false;
        }
    }
    return true;
...
//*****************************************************************************************
static TR_CISCNode *searchIload(TR_CISCNode *q, bool allowArrayIndex)
{
    while (true) {
        bool fail = false;
        ...
        if (q->getOpcode() == TR::iload || q->getOpcode() == TR_variable)
            return q;
        if (allowArrayIndex && q->getOpcode() == TR_arrayindex)
            return q;
        if (fail || q->getOpcode() == TR::lload || q->getNumChildren() < 1)
            return 0;
        q = q->getChild(0);
    }
    return 0;
...
//*****************************************************************************************
bool getThreeNodesForArray(TR_CISCNode *top, TR_CISCNode **ixloadORstore, TR_CISCNode **aload, TR_CISCNode **iload,
    bool allowArrayIndex)
{
    TR_CISCNode *p;
    ...
    while (true) {
        if (p->getNumChildren() == 0)
            return false;
        if (p->getIlOpCode().isLoadIndirect() || p->getIlOpCode().isStoreIndirect() || p->getOpcode() == TR_inbload
            || p->getOpcode() == TR_inbstore || p->getOpcode() == TR_indload || p->getOpcode() == TR_indstore
    ...
    switch (p->getOpcode()) {
        ...
        case TR::aiadd:
            ...
            while (true) {
                ...
                if (q->getNumChildren() != 1)
                    return false;
                q = q->getChild(0);
            }

            // search iload
            if ((q = searchIload(p->getChild(1), allowArrayIndex)) == 0)
                return false;
            *iload = q;
            break;
        ...
        case TR::iadd:
            ...
            while (true) {
                ...
                if (q->getOpcode() == TR::iload || q->getNumChildren() != 1)
                    return false;
                q = q->getChild(0);
            }
            break;
...
//*****************************************************************************************
bool testExitIF(int opcode, bool *isDecrement, int32_t *modLength, int32_t *modStartIdx)
{
    switch (opcode) {
        case TR::ificmplt:
            if (isDecrement)
...
TR::Node *convertStoreToLoadWithI2LIfNecessary(TR::Compilation *comp, bool is64bit, TR::Node *indexNode)
{
    return (indexNode->getOpCode().isStoreDirect())
        ...
        : createI2LIfNecessary(comp, is64bit, indexNode->getReferenceCount() ? indexNode->duplicateTree() : indexNode);
}
...
{
    return (indexNode->getOpCode().isStoreDirect()) ? TR::Node::createLoad(indexNode, indexNode->getSymbolReference())
        : indexNode->getReferenceCount()            ? indexNode->duplicateTree()
                                                    : indexNode;
}
...
TR::Node *createArrayLoad(TR::Compilation *comp, bool is64bit, TR::Node *ixload, TR::Node *baseNode,
    TR::Node *indexNode, int multiply)
{
    TR::Node *top, *c1;

    if (comp->useCompressedPointers() && (ixload->getDataType() == TR::Address))
        multiply = multiply >> 1;
...
TR::Node *createOP2(TR::Compilation *comp, TR::ILOpCodes op2, TR::Node *ch1, TR::Node *ch2)
{
        ...
        switch (op2) {
            ...
            case TR::isub:
                if (value == 0) {
                    ...
                    newVal = (op2 == TR::iadd) ? (ch1->getInt() + ch2->getInt()) : (ch1->getInt() - ch2->getInt());
            ...
            case TR::idiv:
                if (value == 1) {
                    ...
                    if (ch2->getInt() == 0 && op2 == TR::idiv)
                        break; // divide by 0
                    newVal = (op2 == TR::imul) ? (ch1->getInt() * ch2->getInt()) : (ch1->getInt() / ch2->getInt());
                    op2Node = TR::Node::create(ch1, TR::iconst, 0, newVal);
                    done = true;
                }
                break;
    ...
    if (!done)
        op2Node = TR::Node::create(op2, 2, ch1, ch2);
    return op2Node;
}
    ...
    TR::Node *ch2, TR::Node *rep)
{
    return createStoreOP2(comp,
        ...
        op2, TR::Node::createWithSymRef(rep, comp->il.opCodeForDirectLoad(ch1->getSymbol()->getDataType()), 0, ch1),
        ch2);
}
    ...
    TR::SymbolReference *ch2, TR::Node *rep)
{
    return createStoreOP2(comp, store, op2, ch1,
        TR::Node::createWithSymRef(rep, comp->il.opCodeForDirectLoad(ch2->getSymbol()->getDataType()), 0, ch2), rep);
}
...
{
    return createStoreOP2(comp, store, op2, ch1, TR::Node::create(rep, TR::iconst, 0, ch2Const), rep);
}
...
{
    if (x->getOpCodeValue() == TR::iconst && y->getOpCodeValue() == TR::iconst) {
        return TR::Node::create(x, TR::iconst, 0, std::min(x->getInt(), y->getInt()));
    } else {
        TR::Node *y_x = TR::Node::create(TR::isub, 2, y, x); // y-x
        TR::Node *shift = TR::Node::create(TR::ishr, 2, y_x, TR::Node::create(y_x, TR::iconst, 0, 31)); // y_x >> 31
...
TR::Node *createMax(TR::Compilation *comp, TR::Node *x, TR::Node *y)
{
    if (x->getOpCodeValue() == TR::iconst && y->getOpCodeValue() == TR::iconst) {
        ...
        TR::Node *shift = TR::Node::create(TR::ishr, 2, x_y, TR::Node::create(x_y, TR::iconst, 0, 31)); // x_y >> 31
        ...
        if (tn) {
            ...
            if (candidate->getOpCode().isStoreDirect()) {
                ...
                for (p = pi.getFirst(); p; p = pi.getNext()) {
                    if (p->isLoadVarDirect())
                        noLoad = false;
                }
                if (noLoad) {
                    for (p = pi.getFirst(); p; p = pi.getNext()) {
                        if (!p->isOutsideOfLoop() && p->isStoreDirect() && p->isNegligible()) {
    ...
    bool tableBackedByRawStorage)
{
    ...
    if (mask != 0 && mask != 7) {
        if (comp->target().is64Bit()) {
            ...
            if (!tableBackedByRawStorage) {
                TR::Node *hdrSizeNode
                    = TR::Node::create(tableNode, TR::iconst, 0, TR::Compiler->om.contiguousArrayHeaderSizeInBytes());
...
//*****************************************************************************************
List<TR_CISCNode> *sortList(List<TR_CISCNode> *input, List<TR_CISCNode> *output, List<TR_CISCNode> *order, bool reverse)
{
...
bool isLoopPreheaderLastBlockInMethod(TR::Compilation *comp, TR::Block *block, bool trace, TR::Block **predBlock)
{
    ...
    if (block->getStructureOf() && block->getStructureOf()->isLoopInvariantBlock()) {
        ...
        for (auto edge = block->getPredecessors().begin(); edge != block->getPredecessors().end(); ++edge) {
            ...
            if (source->getStructureOf() && source->getStructureOf()->isLoopInvariantBlock()) {
                if (predBlock)
                    *predBlock = source;
...
TR::Node *findLoadWithMatchingSymRefNumber(TR::Node *curNode, int32_t symRefNumberToBeMatched)
{
    ...
    for (int32_t i = 0; i < curNode->getNumChildren(); i++) {
        ...
        if (childNode->getOpCode().isLoad() && childNode->getOpCode().hasSymbolReference()
            && (childNode->getSymbolReference()->getReferenceNumber() == symRefNumberToBeMatched)) {
            node = childNode;
            break;
        } else {
    ...
    int32_t symRefNumberToBeMatched)
{
    ...
    for (int32_t i = 0; i < curNode->getNumChildren(); i++) {
        ...
        if (childNode->getOpCode().hasSymbolReference()
            ...
            isFound = findAndOrReplaceNodesWithMatchingSymRefNumber(childNode, targetNode, symRefNumberToBeMatched)
                || isFound;
        }
    }
    return isFound;
