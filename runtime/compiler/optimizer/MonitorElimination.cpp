...
public:
...

    {
        ...
        if (trace() && treeTop)
            c->log()->printf("Adding new monitor [%p]\n", getMonitorNode());
    }
    ...
    TR::Node *getMonitorNode()
    {
        ...
        if (node->getOpCodeValue() == TR::NULLCHK || node->getOpCodeValue() == TR::treetop)
    ...
    int32_t isBlockSeen(int32_t blockNum)
    {
        return (
            _currentBlocksSeen.get(blockNum) || _innerBlocksSeen.get(blockNum) || _partialExitBlocksSeen.get(blockNum));
    }
    ...
    void addPath(TR_MonitorPath *path)
    {
        ...
        logprintf(trace(), comp()->log(), "Adding path [%p] in block_%d to monitor [%p]\n", path->getNode(),
            path->getBlock()->getNumber(), getMonitorNode());
    }
...
private:
    ...
    // 1) Transactions too close to each other (dynamically of course).  Ie you exit one transaction and a tbegin is too
    // close by. 2) Some threads are locking, and some are using TLE.  This is caused by some compiles not doing TLE on
    // the same monitor
    //
    // Further info:
    // 1) Do we profile monitor object types?
    // 2) I think we want high false contention scenarios.  This allows TLE to work nicely
    // 3) No contention (ie single thread using a lock, or there's only one thread ever trying to acquire the lock) is
    // bad.  There's a 3-5 cycle penalty for Tbegin and compareandswap is 1 cycle
public:
...
int32_t TR::MonitorElimination::perform()
{
    ...
    if (trace()) {
        log->printf(
            ...
            comp()->signature());
        comp()->dumpMethodTrees(log, "Trees before Monitor Elimination");
    }
    ...
    if (!firstPass()) {
        ...
        _alreadyClonedRhs.init(), _specialBlockInfo.init(), _coarsenedMonitorsInfo.init(), _monentEdges.init();
        _monexitEdges.init();
        _cfgBackEdges.init();
        _splitBlocks.init();
...

    {
        ...
        if (comp()->getOSRMode() == TR::voluntaryOSR && comp()->getOption(TR_DisableLiveMonitorMetadata)) {
            logprints(trace(), log,
                "Cannot remove redundant monitors: voluntary OSR requires the monitor object stores from live monitor "
                "metadata\n");
            attemptRedundantMonitors = false;
        }
        ...
        if (attemptRedundantMonitors && findRedundantMonitors()) {
            ...
            if (!comp()->getOption(TR_DisableTLE) && comp()->cg()->getSupportsTLE()) {
                ...
                if (evaluateMonitorsForTMCandidates()) {
                    logprintf(trace(), log,
                        "evaluateMonitorsForTMCandidates returned true. firstPass = %d numPassesCompleted = %d\n",
    ...
    if (trace()) {
        comp()->dumpMethodTrees(log, "Trees after Monitor Elimination");
        log->prints("Ending Monitor Elimination\n");

        if (firstPass() && !_hasTMOpportunities)
            log->prints("Not doing second monitor elimination run because there are no TM opportunities\n");
    }
...
const char *TR::MonitorElimination::optDetailString() const throw() { return "O^O MONITOR ELIMINATION: "; }
...
bool TR::MonitorElimination::findRedundantMonitors()
{
    ...
    while (!_monitorStack->isEmpty()) {
        ...
        logprintf(trace(), log, "Examining path [%p] in block_%d for monitor [%p]\n", path->getNode(),
            path->getBlock()->getNumber(), monitor->getMonitorNode());
        ...
        for (treeTop = path->getTreeTop(); treeTop != exitTree; treeTop = treeTop->getNextTreeTop()) {
            ...
            // this section is to deal with the fact that there's a bug in the s390 linux kernel which prevent traps
            // from working correctly in TM
            if (!comp()->cg()->supportsTrapsInTMRegion()) {
                if (node->getOpCode().canRaiseException()) {
                    // only certain exceptions are implicit on s390 linux.
                    if (node->getOpCodeValue() == TR::ArrayCopyBNDCHK
                        || node->getOpCodeValue() == TR::BNDCHKwithSpineCHK || node->getOpCodeValue() == TR::BNDCHK
                        || node->getOpCodeValue() == TR::NULLCHK || node->getOpCodeValue() == TR::DIVCHK) {
            ...
            if (node->getOpCode().isCall()
                || (node->getNumChildren() > 0 && node->getFirstChild()->getOpCode().isCall())) {
                ...
                if ((!block->isCold() && block->getFrequency() >= minBlockCount)
                    || (monitor->getMonitorTree()
                        && (monitor->getMonitorTree()->getEnclosingBlock()->getNextBlock() == block))) {
            ...
            // For live monitor meta data, the temp holding the monitored object must either be between the monent and
            // the monexit or just before the monent. It is necessary to find this store so that the monent can be
            // repeated later if it is removed. The temp store will only occur before the monent in the event that it is
            // for a synchronized method.
            //
            if (node->getOpCode().isStoreDirect() && node->getOpCode().hasSymbolReference()
                && node->getNumChildren() > 0) {
                if (node->getSymbolReference()->holdsMonitoredObjectForSyncMethod()) {
                    if (multipleTempsForMonitor) {
                        syncMethodTemp = NULL;
                        logprintf(trace(), log,
                            "Found another temp #%d holding monitored object for sync method, giving up\n",
                ...
                } else if (node->getSymbolReference()->getSymbol()->holdsMonitoredObject()
                    && _monitorStack->topIndex() > 0) {
                    ...
                    if (monitor->getMonitorNode()->getNumChildren() > 0)
                        TR_ASSERT(optimizer()->areNodesEquivalent(node->getFirstChild(),
                                      monitor->getMonitorNode()->getFirstChild()),
                            "object stored in temp and locked by monitor should be equivalent");
            ...
            if (node->getOpCodeValue() == TR::monent) {
                ...
                //
                TR_ActiveMonitor *newMonitor
                    = new (trStackMemory()) TR_ActiveMonitor(comp(), treeTop, _numBlocks, monitor, trace());
                ...
                if (node->isSyncMethodMonitor() && syncMethodTemp) {
                    if (node->getNumChildren() > 0)
                        TR_ASSERT(optimizer()->areNodesEquivalent(syncMethodTemp->getNode()->getFirstChild(),
                                      node->getFirstChild()),
                            "object stored in temp and locked by sync monitor should be equivalent");
            ...
            if (node->getOpCodeValue() == TR::monexit) {
                ...
                // Add any exception paths for the current monitor
                //
                if (exceptionsRaisedTillMonexit
                    && !addExceptionPaths(monitor, block->getExceptionSuccessors(), exceptionsRaisedTillMonexit)) {
                    resetReadMonitors(_monitorStack->topIndex());
                    return false;
                }
            ...
            if ((node->getOpCodeValue() != TR::monexit) && (node->getOpCodeValue() != TR::monent)) {
                if ((node->exceptionsRaised() != 0) || node->getOpCode().isStoreIndirect()
        ...
        /////   {
        /////   if (trace())
        /////      log->printf("No monitor exit for monitor enter at [%p]\n", monitor->getMonitorNode());
        /////   return false;
        /////   }
    }
    ...
    if (block->getEntry()) {
        ...
        if (monitor->isBlockSeenInContainers(block->getNumber())) {
            logprintf(trace(), comp()->log(),
                "Monitor enter [%p] loops back to containing monitor scope via block_%d\n", monitor->getMonitorNode(),
                block->getNumber());
            return false;
        }
...
bool TR::MonitorElimination::addExceptionPaths(TR_ActiveMonitor *monitor, TR::CFGEdgeList &paths,
    ...
    for (monitor = it.getFirst(); monitor; monitor = it.getNext()) {
        ...
        if (!comp()->cg()->supportsTrapsInTMRegion() && monitor->_containsTraps) {
            heuristicTrace(tracer(),
                "TM: monitor at node %p contains trap instructions and platform does not support it. Not doing TM",
                monitor->getMonitorNode());
            continue;
        }
        ...
        if (hasMultipleEntriesWithSameExit(monitor)) {
            logprintf(trace(), log,
                "TM: monitor at node %p has multiple exits for a given entry (not supported yet. Not doing TM",
                monitor->getMonitorNode());
            continue;
        }
...
TR::SymbolReference *TR::MonitorElimination::createAndInsertTMRetryCounter(TR_ActiveMonitor *monitor)
{
    ...
    TR::SymbolReference *tempSymRef
        = comp()->getSymRefTab()->createTemporary(comp()->getMethodSymbol(), TR::Int32); // our temporary 't' variable
...
TR_Array<TR::Block *> *TR::MonitorElimination::createFailHandlerBlocks(TR_ActiveMonitor *monitor,
    TR::SymbolReference *tempSymRef, TR::Block *monitorblock, TR::Block *tstartblock)
{
    // This array is used to store the first block and the last block of the fail handler. (In order)  THe rest of the
    // handler should be self contained This array is used to store important fail handler blocks.  element 0 ->
    // persistent fail start, element 1 -> transient failure start, element 2 -> last block
    TR_Array<TR::Block *> *fhBlocksArray
        = new (comp()->trStackMemory()) TR_Array<TR::Block *>(comp()->trMemory(), 3, true, stackAlloc);

    // creating persistent failure block

    TR::Node *resetTempNode = TR::Node::createWithSymRef(TR::istore, 1, 1,
        TR::Node::create(monitor->getMonitorNode(), TR::iconst, 0, 0), tempSymRef);
    ...
    // New logic. I'm adding a goto at the end of the persistent fail block to point directly to the monitor block. This
    // should allow induction variable analysis to exclude the setting of the temp=0 from the loop, and make it a
    // counted loop.

    persistfhBlock->append(TR::TreeTop::create(comp(),
        TR::Node::create(monitor->getMonitorNode(), TR::Goto, 0, monitorblock->getEntry())));

    // create transient failure block -- sub first then do compare

    TR::Node *monentif = TR::Node::createif(TR::ificmple,
        TR::Node::createWithSymRef(monitor->getMonitorNode(), TR::iload, 0, tempSymRef),
        TR::Node::create(monitor->getMonitorNode(), TR::iconst, 0, 0), NULL);
    ...
    TR::Block *fhBlock = TR::Block::createEmptyBlock(monitor->getMonitorNode(), comp(),
        6); // eww a magic number.  the node passed in is used to populate the node with the right bc info, etc.
    ...
    TR::Node *subtree = TR::Node::createWithSymRef(TR::istore, 1, 1,
        TR::Node::create(TR::isub, 2, TR::Node::createWithSymRef(monitor->getMonitorNode(), TR::iload, 0, tempSymRef),
    ...
    if (trace()) {
        comp()->log()->printf("Created fhBlock %d(%p)\n", fhBlock->getNumber(), fhBlock);
        comp()->log()->printf("Created check1Block %d(%p)\n", check1->getNumber(), check1);
    }
...
bool TR::MonitorElimination::searchDownForOtherMonitorsInCurrentBlock(TR::TreeTop *tt, int32_t &size,
    TR::TreeTop *&monitorTT)
{
...
void addSuccessorsToBeProcessed(TR::Block *block, TR_Stack<TR::Block *> &nodesToBeEvaluated, int32_t depth,
    CS2::ArrayOf<int32_t, TR::Allocator> &nodeDepth)
{
    ...
    for (auto succEdge = block->getExceptionSuccessors().begin(); succEdge != block->getExceptionSuccessors().end();
         ++succEdge) {
        TR::Block *succ = (*succEdge)->getTo()->asBlock();
        nodeDepth[succ->getNumber()] = depth;
        nodesToBeEvaluated.push(succ);
...
bool TR::MonitorElimination::searchDownForOtherMonitorsInSuccessors(TR::TreeTop *tt, int32_t &size,
    TR::list<TR::TreeTop *> &closeMonitors, int32_t maxDepth, int32_t minNumberOfNodes)
{
    OMR::Logger *log = comp()->log();
    ...
    logprintf(trace(), log,
        "Begun search down for monitors in successors  monexit at %p.\n Setting numberOfNodesAtDepth[%d] to %d\n",
...
TR_ActiveMonitor *TR::MonitorElimination::findActiveMonitor(TR::TreeTop *tt)
{
    TR_ActiveMonitor *monitor = NULL;
    ListIterator<TR_ActiveMonitor> it(&_monitors);
    for (monitor = it.getFirst(); monitor; monitor = it.getNext()) {
...
void TR::MonitorElimination::searchAndLabelNearbyMonitors(TR_ActiveMonitor *currentMonitor)
{
    ...
    static const int32_t maxDepth = 4; // this is the maxDepth of our CFG search.  Ie, this will limit how many blocks
                                       // down we go visit for the current monitor
    static const int32_t minNumberOfNodes
        = comp()->cg()->getMinimumNumberOfNodesBetweenMonitorsForTLE(); // the number of nodes we want between a monexit
                                                                        // and the next monent

    TR::CFG *cfg = comp()->getFlowGraph();

    logprintf(trace(), log, "Begun search for other nearby Monitors.  Active Monitor %p with Node %p\n", currentMonitor,
        currentMonitor->getMonitorNode());
    ...
    for (et = treeIT.getCurrent(); et; et = treeIT.getNext()) {
        ...
        if (searchDownForOtherMonitorsInCurrentBlock(et, numberOfTTs,
                monitorTT)) // found the closest monent in the current block
        {
            if (numberOfTTs < minNumberOfNodes) {
                TR_ActiveMonitor *monitor = findActiveMonitor(monitorTT);
                if (monitor) {
                    logprintf(trace(), log,
                        "Setting Active monitor with monitorNode %p to NOT a TM Candidate because it's too close"
            ...
            for (auto iterator = closeMonitors.begin(); iterator != closeMonitors.end(); ++iterator) {
                ...
                if (monitor) {
                    logprintf(trace(), log,
                        "Setting Active monitor with monitorNode %p to NOT a TM Candidate because it's too close (in "
                        "another block)"
                        " to previous TM Candidate with monexit %p\n",
                        monitor->getMonitorNode(), et->getNode());
                    monitor->setNotTMCandidate();
...
{
    // this goes and checks to see if the monexit is on another monenter path.  We will only perform tm if all monenter
    // paths are tm candidates
    ...
    if (!isCandidate) {
        logprintf(trace(), comp()->log(),
            "TM:monitor %p at node %p is NOT a TM Candidate because some other monitor sharing the exit is not a TM "
            "Candidate\n",
...
void TR::MonitorElimination::transformMonitorsIntoTMRegions()
{
    ...
    for (monitor = it.getFirst(); monitor; monitor = it.getNext()) {
        ...
        // Part 2: split the monitor block at the spot of the monitor. in the first half of the block, append a goto to
        // the second half of the block.  insert the fail handler block in between these two.
        //  We rely on block splitting to properly create temporaries for us.  We do this by adding a treetop with the
        //  monitor child above where we do the split.  It works out nicely :)

        TR::TreeTop *tempTree
            = TR::TreeTop::create(comp(), TR::Node::create(TR::treetop, 1, monitor->getMonitorNode()->getFirstChild()));
        monitor->getMonitorTree()->insertBefore(tempTree);

        TR::Block *monitorblock
            = monitor->getMonitorTree()->getEnclosingBlock()->split(monitor->getMonitorTree(), cfg, true, true);
        TR::Block *tophalfmonitorblock = monitorblock->getEntry()->getPrevTreeTop()->getEnclosingBlock();
        TR::Block *critSectStart = 0;

        debugTrace(tracer(),
            "After monitor split.getmonitornode = %p  monitor->getMonitorTree() = %p node = %p nexttt = %p nextt node "
            "= %p\n",
            monitor->getMonitorNode(), monitor->getMonitorTree(), monitor->getMonitorTree()->getNode(),
            monitor->getMonitorTree()->getNextTreeTop(), monitor->getMonitorTree()->getNextTreeTop()->getNode());
        debugTrace(tracer(), "tophalfmonitorblock = %p %d , exit tt = %p node %p\n", tophalfmonitorblock,
            tophalfmonitorblock->getNumber(), tophalfmonitorblock->getExit(),
            tophalfmonitorblock->getExit()->getNode());
        ...
        if (monitor->getMonitorTree()->getNextTreeTop()->getNode()->getOpCodeValue() == TR::BBEnd) {
            ...
            critSectStart
                = monitor->getMonitorTree()->getEnclosingBlock()->getExit()->getNextTreeTop()->getEnclosingBlock();
        } else {
            critSectStart = monitor->getMonitorTree()->getEnclosingBlock()->split(
                monitor->getMonitorTree()->getNextTreeTop(), cfg, true, true);
        ...
        debugTrace(tracer(), "monitorblock = %d(%p) critSectStart = %d(%p)\n", monitorblock->getNumber(), monitorblock,
            critSectStart->getNumber(), critSectStart);

        // clone the monitor block to create the tstartblock.  put this newly created tstart block in the spot of the
        // monitor move the monitor block to the end of the il
        ...
        tophalfmonitorblock->append(TR::TreeTop::create(comp(),
            TR::Node::create(monitor->getMonitorNode(), TR::Goto, 0, tstartblock->getEntry())));
        ...
        TR::TreeTop *monitorgoto = TR::TreeTop::create(comp(),
            TR::Node::create(monitor->getMonitorNode(), TR::Goto, 0, critSectStart->getEntry()));
        ...
        TR::Node *persistentFailNode
            = TR::Node::create(monitor->getMonitorNode(), TR::branch, 0, (*fhBlocks)[0]->getEntry());
        TR::Node *transientFailNode
            = TR::Node::create(monitor->getMonitorNode(), TR::branch, 0, (*fhBlocks)[1]->getEntry());
        TR::Node *fallThroughNode
            = TR::Node::create(monitor->getMonitorNode(), TR::branch, 0, critSectStart->getEntry());
        TR::Node *monitorObject = TR::Node::copy(monitor->getMonitorNode()->getFirstChild());
        monitorObject->setReferenceCount(0);
        TR::Node *tstartNode = TR::Node::createWithRoomForFive(TR::tstart, persistentFailNode, transientFailNode,
            fallThroughNode, monitorObject);
        ...
        tstartNode->setSymbolReference(
            comp()->getSymRefTab()->findOrCreateTransactionEntrySymbolRef(comp()->getMethodSymbol()));
        ...
        // CFG Modifications -- recall that removeEdge hides complex code under its seemingly simple name, and must be
        // done last
        ...
        for (TR::TreeTop *exitTree = exitTrees.getFirst(); exitTree; exitTree = exitTrees.getNext()) {
            ...
            if (exitTree->getNode()->getOpCodeValue() != TR::monexit
                && (exitTree->getNode()->getFirstChild()
                    && exitTree->getNode()->getFirstChild()->getOpCodeValue() != TR::monexit)) {
                debugTrace(tracer(), "\nTM: Already transformed exit tree %p with node %p. Not doing it again.",
            ...
            else if (exitTree->getNode()->getFirstChild()
                && exitTree->getNode()->getFirstChild()->getOpCodeValue() == TR::monexit)
            ...
            TR::Node *exitif = TR::Node::createif(TR::ificmple,
                TR::Node::createWithSymRef(exitTree->getNode(), TR::iload, 0, tempSymRef),
                TR::Node::create(exitTree->getNode(), TR::iconst, 0, 0), NULL);
            ...
            if (!clonedExit) {
                ...
                monexitblock->append(TR::TreeTop::create(comp(),
                    TR::Node::create(exitTree->getNode(), TR::Goto, 0, remainderBlock->getEntry())));

                // cfg->addEdge(monexitblock,remainderBlock);

                debugTrace(tracer(), "\nTM: cloned block_%d(%p) to be used as monexit block.monexitblock = %d(%p)",
                    tEndBlock->getNumber(), tEndBlock, monexitblock->getNumber(), monexitblock);
                // There is a problem with sharing the same monexit when you have an exception block that unlocks the
                // monitor as well. I suspect the problem is that the original monexit had an exception successor which
                // could unlock the monitor. this results in some funky cfg afterwards where the monexit block has the
                // exception block as a predecessor and an exception successor
                // clonedExit = true;
            }
            ...
            */
            ...
            tfinishNode->setSymbolReference(
                comp()->getSymRefTab()->findOrCreateTransactionExitSymbolRef(comp()->getMethodSymbol()));
        ...
        for (TR::TreeTop *osrInduceTT = osrInduceBlocks.getFirst(); osrInduceTT;
             osrInduceTT = osrInduceBlocks.getNext()) {
...
{
    if ((node->getOpCodeValue() == TR::monexit) || (node->getOpCodeValue() == TR::monent)
        || (node->exceptionsRaised() != 0) || node->getOpCode().isStoreIndirect()
...
bool TR::MonitorElimination::transformIntoReadMonitor()
{
    ...
    for (; treeTop; treeTop = treeTop->getNextTreeTop()) {
        ...
        if (node->getOpCodeValue() == TR::BBStart) {
            ...
            if (prevBlock && (prevBlock->getSuccessors().size() == 1) && (block->getPredecessors().size() == 1)
...
void TR::MonitorElimination::recognizeIfThenReadRegion(TR::TreeTop *lastMonitor, TR::Node *lastMonitorNode,
    int32_t lastMonitorTreeNum, TR::Block *lastMonitorBlock, TR::Block *block)
{
    TR::CFG *cfg = comp()->getFlowGraph();
    ...
    if (edges.size() == 2) {
        ...
        if (mergeBlock1
            && (((block2->getSuccessors().size() == 1) && block2->hasSuccessor(mergeBlock1))
                || (mergeBlock1 == block2))) {
            ...
            if (mergeBlock2
                && (((block1->getSuccessors().size() == 1) && block1->hasSuccessor(mergeBlock2))
                    || (mergeBlock2 == block1))) {
        ...
        if (mergeBlock1 && mergeBlock1 == mergeBlock2 && mergeBlock1 != cfg->getEnd()) {
            TR::Node *monexitNode = NULL;
            if (preservesReadRegion(lastMonitorNode, mergeBlock1, &monexitNode)
                && performTransformation(comp(),
                    "%s Found a locked region that was almost read-only in %s and num intermediate blocks = %d\n",
                ...
                if (canBeAdjusted && lastMonitorNode && monexitNode) {
                    ...
                    for (TR::Block *nextBlock = interIt.getFirst(); nextBlock; nextBlock = interIt.getNext()) {
                        ...
                        TR::Block *clonedBlockWithIf = adjustBlockToCreateReadMonitor(lastMonitor, lastMonitorNode,
                            lastMonitorTreeNum, lastMonitorBlock, nextBlock, block);
                        // dumpOptDetails(comp(), "lastTree %p %s blockNum %d\n",
                        // clonedBlockWithIf->getLastRealTreeTop()->getNode(),
                        // clonedBlockWithIf->getLastRealTreeTop()->getNode()->getOpCode().getName(),
                        // clonedBlockWithIf->getNumber());
                        TR::Block *targetBlock = clonedBlockWithIf->getLastRealTreeTop()
                                                     ->getNode()
                                                     ->getBranchDestination()
                                                     ->getNode()
                                                     ->getBlock();
                        ...
                        if (targetBlock != mergeBlock1) {
                            ...
                            if (nextBlock && (nextBlock != mergeBlock1) && targetBlock->hasSuccessor(nextBlock)) {
                                TR::CFGEdge *e = targetBlock->getEdge(nextBlock);
                                TR::Block *gotoBlock = TR::Block::createEmptyBlock(targetBlock->getEntry()->getNode(),
                                    comp(), e->getFrequency(), targetBlock);
                                TR::TreeTop *gotoTreeTop = TR::TreeTop::create(comp(),
                                    TR::Node::create(targetBlock->getEntry()->getNode(), TR::Goto, 0,
                                        nextBlock->getEntry()));
                                gotoBlock->append(gotoTreeTop);
                                lastTree->join(gotoBlock->getEntry());
                                gotoBlock->getExit()->setNextTreeTop(NULL);
                                clonedTargetBlock->getLastRealTreeTop()->adjustBranchOrSwitchTreeTop(comp(),
                                    nextBlock->getEntry(), gotoBlock->getEntry());
                            ...
                            for (auto nextEdge = targetBlock->getExceptionSuccessors().begin();
                                 nextEdge != targetBlock->getExceptionSuccessors().end(); ++nextEdge) {
                            ...
                            for (auto nextEdge = targetBlock->getSuccessors().begin();
                                 nextEdge != targetBlock->getSuccessors().end(); ++nextEdge) {
                                TR::Block *succ = (*nextEdge)->getTo()->asBlock();
                                if (gotoForFallThrough && (succ == nextBlock)) {
                                    // Already added above
                                    //
                                } else
                        ...
                        // clonedMergeBlock1->getEntry()->getNode(), clonedMergeBlock1->getNumber());
                        ...
                        if (nextBlock && mergeBlock1->hasSuccessor(nextBlock)) {
                            TR::CFGEdge *e = mergeBlock1->getEdge(nextBlock);
                            gotoBlock = TR::Block::createEmptyBlock(mergeBlock1->getEntry()->getNode(), comp(),
                                e->getFrequency(), mergeBlock1);
                            TR::TreeTop *gotoTreeTop = TR::TreeTop::create(comp(),
                                TR::Node::create(mergeBlock1->getEntry()->getNode(), TR::Goto, 0,
                                    nextBlock->getEntry()));
                            gotoBlock->append(gotoTreeTop);
                            lastTree->join(gotoBlock->getEntry());
                            gotoBlock->getExit()->setNextTreeTop(NULL);
                            clonedMergeBlock1->getLastRealTreeTop()->adjustBranchOrSwitchTreeTop(comp(),
                                nextBlock->getEntry(), gotoBlock->getEntry());
                            lastTree = gotoBlock->getExit();
                            TR::CFGEdge *newEdge = NULL;
                            cfg->addNode(gotoBlock);
                        ...
                        if (dupNode->getOpCodeValue() == TR::monexit)
                            dupNode = TR::Node::createWithSymRef(TR::NULLCHK, 1, 1, dupNode,
                                comp()->getSymRefTab()->findOrCreateNullCheckSymbolRef(
                                    dupNode->getSymbolReference()->getOwningMethodSymbol(comp())));
                        ...
                        for (auto nextEdge = mergeBlock1->getExceptionSuccessors().begin();
                             nextEdge != mergeBlock1->getExceptionSuccessors().end(); ++nextEdge) {
                            TR::Block *succ = (*nextEdge)->getTo()->asBlock();
                            cfg->addExceptionEdge(clonedMergeBlock1, succ);
                            if (gotoBlock)
                                cfg->addExceptionEdge(gotoBlock, succ);
                        }
                        for (auto nextEdge = mergeBlock1->getSuccessors().begin();
                             nextEdge != mergeBlock1->getSuccessors().end(); ++nextEdge) {
                            TR::Block *succ = (*nextEdge)->getTo()->asBlock();
                            if (gotoBlock && (succ == nextBlock)) {
                                // Already added above
                                //
                            } else
                        ...
                        if (clonedTargetBlock) {
                            ...
                            clonedBlockWithIf->getLastRealTreeTop()->getNode()->setBranchDestination(
                                clonedTargetBlock->getEntry());
                            if (!gotoForFallThrough) {
...
TR::Block *TR::MonitorElimination::adjustBlockToCreateReadMonitor(TR::TreeTop *lastMonitor, TR::Node *lastMonitorNode,
    int32_t lastMonitorTreeNum, TR::Block *lastMonitorBlock, TR::Block *blockToBeAdjusted, TR::Block *blockWithIf)
{
    TR::CFG *cfg = comp()->getFlowGraph();
    ...
    while (cursorTree) {
        ...
        // cursorNode->getOpCode().getName(), treeNum, lastMonitorTreeNum);
    ...
    cfg->addEdge(clonedBlockWithIf,
        blockWithIf->getLastRealTreeTop()->getNode()->getBranchDestination()->getNode()->getBlock());
...
bool TR::MonitorElimination::preservesReadRegion(TR::Node *lastMonitorNode, TR::Block *block, TR::Node **monexitNode)
{
    ...
    for (; treeTop != exitTree; treeTop = treeTop->getNextTreeTop()) {
        ...
        if (node->getOpCodeValue() == TR::monexit) {
            ...
            if ((valueNumber < 0) || (valueNumber != lastNodeValueNumber))
                return false;
        ...
        if (isNullCheck || killsReadMonitorProperty(node))
            return false;
    }
...
void TR::MonitorElimination::removeMonitorNode(TR::Node *node)
{
    ...
    if (node->getOpCodeValue() == TR::NULLCHK) {
        ...
        TR_ASSERT(node->getOpCodeValue() == TR::monent || node->getOpCodeValue() == TR::monexit,
            "Expected monitor node");
...
bool TR::MonitorElimination::markBlocksAtSameNestingLevel(TR_Structure *structure,
    TR_BitVector *blocksAtCurrentNestingLevel)
{
    if (structure->asBlock())
        ...
        if (!isLoop && !region->isAcyclic())
            return true;

        TR_BitVector *blocksInRegion = blocksAtCurrentNestingLevel;
        if (isLoop) {
            blocksInRegion = new (trStackMemory())
                TR_BitVector(comp()->getFlowGraph()->getNextNodeNumber(), trMemory(), stackAlloc, notGrowable);
        ...
        if (isLoop || comp()->getFlowGraph()->getStructure() == region)
...
//
void TR::MonitorElimination::collectPredsAndSuccs(TR::CFGNode *startNode, TR_BitVector *visitedNodes,
    TR_BitVector **predecessorInfo, TR_BitVector **successorInfo, List<TR::CFGEdge> *cfgBackEdges,
    TR_BitVector *loopEntryBlocks, TR::Compilation *comp, bool trace)
{
    ...
    bool isFirst = true;
    TR_BitVector *subtraction = new (comp->trStackMemory())
        TR_BitVector(comp->getFlowGraph()->getNextNodeNumber(), comp->trMemory(), stackAlloc, notGrowable);
    ...
    while (!analysisQueue.isEmpty()) {
        ...
        if ((recState->getState() == 0) || (recState->getState() == 2)) {
            ...
            for (; edge; edge = pit.getNext()) {
                ...
                if (!visitedNodes->get(pred->getNumber())
                    && !(loopEntryBlocks->get(node->getNumber()) && cfgBackEdges->find(edge))) {
                    ...
                    if (!(loopEntryBlocks->get(node->getNumber()) && cfgBackEdges->find(edge))) {
                        ...
                        *subtraction -= *predecessorInfo[node->getNumber()];
                        ...
                        if (!loopEntryBlocks->get(node->getNumber())
                            ...
                            TR_ASSERT(0,
                                "CFG node cannot have itself as a predecessor in a CFG walk ignoring back edges\n");
                        }
                    } else {
        ...
        if (somethingChanged ||
          ...
          (recState->getState() == 2) */)
         {
            ...
            for (; edge; edge = sit.getNext()) {
                TR::CFGNode *succ = edge->getTo();
                if (!(loopEntryBlocks->get(succ->getNumber()) && cfgBackEdges->find(edge))) {
...
void TR::MonitorElimination::coarsenMonitorRanges()
{
    ...
    _storedSymRefsInSimpleLockedRegion
        = new (trStackMemory()) TR_BitVector(symRefCount, trMemory(), stackAlloc, growable);
    ...
    for (; treeTop; treeTop = treeTop->getNextTreeTop()) {
        ...
        if (node->getOpCode().isStore() || node->mightHaveNonTransparentSymbolReference()) {
            ...
            node->getFirstChild()->getSymbolReference()->getUseDefAliases().getAliasesAndUnionWith(
                *_symbolsWritten[blockNum]);
        }

        TR::Block *guard = NULL;
        bool virtCall = false;
        if (node->getOpCode().isCall() && node->isTheVirtualCallNodeForAGuardedInlinedCall()
            && (block->getPredecessors().size() == 1) && (guard = block->findVirtualGuardBlock(cfg))
        ...
        if ((treetopNode->exceptionsRaised() || (treetopNode->getOpCodeValue() == TR::asynccheck)
                || node->getOpCode().isCall())
            && (node->getOpCodeValue() != TR::monent && node->getOpCodeValue() != TR::monexit)) {
            if ((treetopNode->getOpCodeValue() != TR::asynccheck)
                && (!treetopNode->getOpCode().isCheck() || treetopNode->getOpCode().isResolveCheck()
        ...
        if (node->getOpCodeValue() == TR::monent) {
            ...
            if (node->getFirstChild()->getOpCode().hasSymbolReference()) {
                if (_safeValueNumbers[lockedObjectValueNumber] > 0) {
                    ...
                    if (prevNode->getSymbolReference()->getReferenceNumber()
                        != node->getFirstChild()->getSymbolReference()->getReferenceNumber())
                        // if (_safeValueNumbers[lockedObjectValueNumber] !=
                        // node->getFirstChild()->getSymbolReference()->getReferenceNumber())
                ...
                //_safeValueNumbers[lockedObjectValueNumber] =
                // node->getFirstChild()->getSymbolReference()->getReferenceNumber();
            }
            ...
            if (_monexitBlockInfo[blockNum] > -1) {
                if (_monexitBlockInfo[blockNum] == lockedObjectValueNumber) {
                    if ((!containsCall)
                        && performTransformation(comp(), "%s Success: Coarsening monexit %p locally in block_%d\n",
            ...
            if ((_monentBlockInfo[blockNum] == -2) && (_monexitBlockInfo[blockNum] == -2)
    ...
    if (trace()) {
        log->prints("Guarded virtual call blocks : \n");
        _guardedVirtualCallBlocks->print(log, comp());
        log->println();
    }
    ...
    TR_BitVector *blocksAtCurrentNestingLevel
        = new (trStackMemory()) TR_BitVector(numBlocks, trMemory(), stackAlloc, notGrowable);
    bool containsImproperRegions
        = markBlocksAtSameNestingLevel(comp()->getFlowGraph()->getStructure(), blocksAtCurrentNestingLevel);

    if (containsImproperRegions) {
        return;
    }

    collectPredsAndSuccs(cfg->getStart(), visitedNodes, _predecessorInfo, _successorInfo, &_cfgBackEdges,
        _loopEntryBlocks, comp(), trace());
    ...
    //_matchingSpecialMonexitBlocks = new (trStackMemory()) TR_BitVector(numBlocks, trMemory(), stackAlloc,
    // notGrowable);
    ...
    for (; treeTop; treeTop = treeTop->getNextTreeTop()) {
        ...
        if (node->getOpCodeValue() == TR::BBEnd) {
            logprintf(trace(), log, "Coarsening: Considering node %p blockNum %d prevLockedObject = %d\n", node,
                blockNum, prevLockedObject);

            if (prevLockedObject > -1) {
                logprintf(trace(), log,
                    "Coarsening: _multiplyLockedObjects->get(prevLockedObject) = %d "
                    "_safeValueNumbers[prevLockedObject] = %d _coarsendMonexits->get(blockNum) = %d\n",
                    _multiplyLockedObjects->get(prevLockedObject), _safeValueNumbers[prevLockedObject],
                    _coarsenedMonexits->get(blockNum));

                if (_multiplyLockedObjects->get(prevLockedObject) && (_safeValueNumbers[prevLockedObject] > 0)
                    && !_coarsenedMonexits->get(blockNum)) {
        ...
        if (node->getOpCodeValue() == TR::monexit && !_coarsenedMonexits->get(blockNum)) {
            ...
            // _monexitBlockInfo[blockNum], prevLockedObject);
...
void TR::MonitorElimination::coarsenMonitor(int32_t origBlockNum, int32_t prevLockedObject, TR::Node *prevMonitorNode)
{
    ...
    while (!_monexitWorkList->isEmpty()) {
        ...
        *_temp = *successors;
        ...
        if (trace()) {
            log->printf("Successors of block_%d\n", blockNum);
        ...
        *_temp &= *_containsMonents;
        ...
        if (trace()) {
            log->prints("Blocks At Same Nesting Level\n");
        ...
        if (_specialBlockInfo.getSize() > numSpecialBlocks) {
            ...
            *_inclusiveIntersection |= *_matchingMonentBlocks;
            ...
            while (listIt.hasMoreElements() && nodesInList) {
                int32_t nextListElement = listIt.getNextElement();
                block = _blockInfo[nextListElement];

                bool controlFlowInCatch = false;

                if (!prevMonitorNode->isSyncMethodMonitor() && !block->getExceptionSuccessors().empty()) {
                    nodesInList = false;
                ...
                if (!nodesInList) {
                    logprintf(trace(), log,
                        "Cannot coarsen monexit in block_%d because of unknown catch block structure\n",
                        nextListElement);
                    return;
                } else {
                    // TR_SuccessorIterator succIt(block);
                    nodesInList
                        = checkIfSuccsInList(block->getSuccessors(), _inclusiveIntersection, controlFlowInCatch, block);
                    if (nodesInList) {
                        nodesInList = checkIfSuccsInList(block->getExceptionSuccessors(), _inclusiveIntersection,
                            controlFlowInCatch, block);
                    }
                }

                if (!nodesInList) {
                    logprintf(trace(), log,
                        "Cannot coarsen monexit in block_%d because succs of block_%d are NOT in list\n", origBlockNum,
                        nextListElement);
                    return;
                }
            }
            ...
            if (trace()) {
                log->prints("Closure of common preds and succs : \n");
                _closureIntersection->print(log, comp());
                log->println();
            }
            ...
            while (listIt.hasMoreElements()) {
                ...
                if (nodesInList) {
                    ...
                    if (!nodesInList) {
                        logprintf(trace(), log,
                            "Cannot coarsen monexit in block_%d because succ test failed for block_%d\n", origBlockNum,
                            nextListElement);
                        break;
                    }
                } else {
                    logprintf(trace(), log,
                        "Cannot coarsen monexit in block_%d because pred test failed for block_%d\n", origBlockNum,
                        nextListElement);
                    break;
                }
            }
            ...
            while (succs.hasMoreElements() && nodesInList) {
                int32_t nextSucc = succs.getNextElement();

                if (trace()) {
                    log->printf("0Block number %d monent status %d prevLockedObject %d\n", nextSucc,
                        _monentBlockInfo[nextSucc], prevLockedObject);
                    log->printf("_nodesInList %p\n", nodesInList);
                }
                ...
                if (nodesInList) {
                    ...
                    logprintf(trace(), log,
                        "Cannot coarsen monexit in block_%d because preds of succ %d are NOT in list\n", origBlockNum,
                        nextSucc);
                }

                if (!nodesInList) {
                    logprintf(trace(), log,
                        "Cannot coarsen monexit in block_%d because of matching monent in block_%d\n", origBlockNum,
                        nextSucc);
                }
            }
            ...
            if (nodesInList) {
                ...
                *_scratch &= *_guardedVirtualCallBlocks;
                ...
                while (scratchIt.hasMoreElements()) {
                    ...
                    for (auto succEdge = nextBlock->getSuccessors().begin();
                         succEdge != nextBlock->getSuccessors().end(); ++succEdge) {
                ...
                if (trace()) {
                    log->prints("Guarded virtual call blocks to be adjusted : \n");
                    _guardedVirtualCallBlocks->print(log, comp());
                    log->println();
                }

                TR::CoarsenedMonitorInfo *monitorInfo
                    = findOrCreateCoarsenedMonitorInfo(prevLockedObject, prevMonitorNode);
...
static bool sideEntranceOrExitExists(TR_BitVector *intersection, TR::Block **blockInfo, int32_t predNum,
    int32_t succNum)
{
    TR::Block *first = NULL;
    TR::CFGEdge *edge;
    ...
    for (auto edge = first->getPredecessors().begin(); edge != first->getPredecessors().end(); ++edge) {
        if (!intersection->get((*edge)->getFrom()->getNumber()) && ((*edge)->getFrom()->getNumber() != predNum)) {
            return true;
        }
    }
    ...
    while (intersectIt.hasMoreElements()) {
        ...
        for (auto edge = nextBlock->getSuccessors().begin(); edge != nextBlock->getSuccessors().end(); ++edge) {
            if (!intersection->get((*edge)->getTo()->getNumber()) && ((*edge)->getTo()->getNumber() != succNum)) {
                return true;
            }
        }
...
void TR::MonitorElimination::buildClosure(int32_t blockNum, TR_BitVector *monentBlocks, TR_BitVector *successors,
    int32_t lockedObject)
{
    collectSuccessors(blockNum, monentBlocks, successors, lockedObject);
}

void TR::MonitorElimination::collectSuccessors(int32_t blockNum, TR_BitVector *monentBlocks, TR_BitVector *successors,
    int32_t lockedObject)
{
    OMR::Logger *log = comp()->log();
    ...
    if (_containsCalls->get(blockNum)) {
        ...
        recoveryPossible = treesAllowCoarsening(_monexitTrees[blockNum]->getNextTreeTop(), block->getExit(),
            &peekedCall, &blockContainsCheck);
        if (recoveryPossible) {
        } else
            return;
    }
    ...
    while (succs.hasMoreElements()) {
        ...
        if (trace()) {
            log->printf("1Block number %d monent status %d prevLockedObject %d\n", nextSucc, _monentBlockInfo[nextSucc],
                lockedObject);
            log->printf("_monentBlockInfo %d _monentBlockInfo %d\n", _monentBlockInfo[nextSucc], lockedObject);
        }

        if (_monentBlockInfo[nextSucc] != -1 && _monentBlockInfo[nextSucc] == lockedObject) {
            //
            // Found a possible monent to coarsen with
            //
            TR_BitVector *predecessors = _predecessorInfo[nextSucc];
            if (trace()) {
                log->printf("Predecessors for block_%d\n", nextSucc);
                predecessors->print(log, comp());
                log->println();
            }
            ...
            *_subtraction = *_intersection;
            ...
            if (trace()) {
                log->printf("Intersection for block_%d\n", nextSucc);
                _intersection->print(log, comp());
                log->println();
                log->printf("Subtraction for block_%d\n", nextSucc);
                _subtraction->print(log, comp());
                log->println();
                log->printf("ContainsCalls for block_%d\n", nextSucc);
            ...
            *_subtraction -= *_loopEntryBlocks;
            ...
            if (*_subtraction == *_intersection) {
                *_subtraction -= *_containsCalls;
                ...
                if (_containsCalls->get(nextSucc))
                    recoveryPossibleInSucc = treesAllowCoarsening(_blockInfo[nextSucc]->getEntry(),
                        _monentTrees[nextSucc]->getPrevTreeTop(), &peekedCallInSucc, &succContainsCheck);

                if (!interveningBlocksContainCalls && !blockContainsCheck && !succContainsCheck
                    && recoveryPossibleInSucc && !peekedCallInSucc && recoveryPossible && !peekedCall) {
                    ...
                    *_subtraction -= *_scratch;
                    if (!(*_subtraction == *_intersection)) {
                        ...
                        //
                        if (succPostDominatesPred && isSimpleLockedRegion(_monentTrees[nextSucc])) {
                            if (symbolsAreNotWritten(_intersection)) {
                                if (symbolsAreNotWrittenInTrees(_monexitTrees[blockNum]->getNextTreeTop(),
                                        _blockInfo[blockNum]->getExit())
                                    && symbolsAreNotWrittenInTrees(_blockInfo[nextSucc]->getEntry(),
                                        _monentTrees[nextSucc]->getPrevTreeTop())) {
                                    canRecover = true;
                    ...
                    if (succPostDominatesPred && isSimpleLockedRegion(_monentTrees[nextSucc])) {
                        ...
                        if (recoveryPossibleInInterveningBlocks && peekedCall) {
                            bool peekingDone = false;
                            TR::Block *block = _blockInfo[blockNum];
                            recoveryPossibleInInterveningBlocks = treesAllowCoarsening(
                                _monexitTrees[blockNum]->getNextTreeTop(), block->getExit(), &peekingDone);
                        }

                        if (recoveryPossibleInInterveningBlocks && peekedCallInSucc) {
                            bool peekingDone = false;
                            recoveryPossibleInInterveningBlocks = treesAllowCoarsening(_blockInfo[nextSucc]->getEntry(),
                                _monentTrees[nextSucc]->getPrevTreeTop(), &peekingDone);
                        }

                        if (recoveryPossibleInInterveningBlocks) {
                            if (symbolsAreNotWritten(_intersection)
                                && symbolsAreNotWrittenInTrees(_monexitTrees[blockNum]->getNextTreeTop(),
                                    _blockInfo[blockNum]->getExit())
                                && symbolsAreNotWrittenInTrees(_blockInfo[nextSucc]->getEntry(),
                                    _monentTrees[nextSucc]->getPrevTreeTop())) {
                                canRecover = true;
                                logprints(trace(), log,
                                    "Found a coarsening opportunity across call (peek done successfully)\n");
                            }
                        }
                    }
                }
            ...
            if ((*_subtraction == *_intersection) && !blockContainsCheck && !succContainsCheck && !peekedCall
                && !peekedCallInSucc)
                simpleCoarsening = true;

            if ((simpleCoarsening && !_matchingSpecialBlocks->get(blockNum) && !_matchingSpecialBlocks->get(nextSucc))
                || (canRecover && !_matchingNormalMonentBlocks->get(nextSucc)
                    && !_matchingNormalMonexitBlocks->get(blockNum))) {
                if (!simpleCoarsening) {
                    _matchingSpecialBlocks->set(nextSucc);

                    SpecialBlockInfo *specialBlockInfo
                        = (SpecialBlockInfo *)trMemory()->allocateStackMemory(sizeof(SpecialBlockInfo));
                    ...
                    for (TR_ClassLoadCheck *clc = _classesThatShouldNotBeLoaded.getFirst(); clc; clc = clc->getNext())
                        addClassThatShouldNotBeLoaded(clc->_name, clc->_length,
                            comp()->getClassesThatShouldNotBeLoaded(), false);

                    for (TR_ClassExtendCheck *cec = _classesThatShouldNotBeNewlyExtended.getFirst(); cec;
                         cec = cec->getNext())
                        addClassThatShouldNotBeNewlyExtended(cec->_clazz,
                            comp()->getClassesThatShouldNotBeNewlyExtended(), false);
                } else {
                    ...
                    *_closureIntersection |= *_intersection;
                    ...
                    if (!_matchingMonentBlocks->get(nextSucc)) {
                        ...
                        TR_BitVector *predsContainingMonexits = new (trStackMemory()) TR_BitVector(
                            comp()->getFlowGraph()->getNextNodeNumber(), trMemory(), stackAlloc, notGrowable);
...
void TR::MonitorElimination::collectPredecessors(int32_t blockNum, TR_BitVector *monexitBlocks,
    TR_BitVector *predecessors, int32_t lockedObject)
{
    OMR::Logger *log = comp()->log();
    bool peekedCall = false;
    bool recoveryPossible = true;
    ...
    if (_containsCalls->get(blockNum)) {
        TR::Block *block = _blockInfo[blockNum];
        recoveryPossible = treesAllowCoarsening(block->getEntry(), _monentTrees[blockNum]->getPrevTreeTop(),
            &peekedCall, &blockContainsCheck);
        if (recoveryPossible) {
        } else
            return;
    ...
    while (preds.hasMoreElements()) {
        ...
        if (_monexitBlockInfo[nextPred] != -1 && _monexitBlockInfo[nextPred] == lockedObject) {
            //
            // Found a possible monent to coarsen with
            //
            TR_BitVector *successors = _successorInfo[nextPred];
            if (trace()) {
                log->printf("Successors for block_%d\n", nextPred);
            ...
            *_subtraction -= *_loopEntryBlocks;
            ...
            if (*_subtraction == *_intersection) {
                *_subtraction -= *_containsCalls;
                ...
                if (_containsCalls->get(nextPred))
                    recoveryPossibleInPred = treesAllowCoarsening(_monexitTrees[nextPred]->getNextTreeTop(),
                        _blockInfo[nextPred]->getExit(), &peekedCallInPred, &predContainsCheck);

                if (!interveningBlocksContainCalls && !blockContainsCheck && !predContainsCheck
                    && recoveryPossibleInPred && !peekedCallInPred && recoveryPossible && !peekedCall) {
                    *_scratch = *_containsAsyncCheck;
                    //*_scratch -= *blocksAtSameNestingLevel;
                    *_subtraction -= *_scratch;
                    if (!(*_subtraction == *_intersection)) {
                        ...
                        // into previous locked region of code.
                        //
                        if (succPostDominatesPred && isSimpleLockedRegion(_monexitTrees[nextPred])) {
                            if (symbolsAreNotWritten(_intersection)
                                && symbolsAreNotWrittenInTrees(_monexitTrees[nextPred]->getNextTreeTop(),
                                    _blockInfo[nextPred]->getExit())
                                && symbolsAreNotWrittenInTrees(_blockInfo[blockNum]->getEntry(),
                                    _monentTrees[blockNum]->getPrevTreeTop())) {
                                canRecover = true;
                                logprints(trace(), log, "Found a coarsening opportunity across loop\n");
                            }
                        }
                    ...
                    if (succPostDominatesPred && isSimpleLockedRegion(_monexitTrees[nextPred])) {
                        ...
                        if (recoveryPossibleInInterveningBlocks && peekedCall) {
                            bool peekingDone = false;
                            TR::Block *block = _blockInfo[blockNum];
                            recoveryPossibleInInterveningBlocks = treesAllowCoarsening(block->getEntry(),
                                _monentTrees[blockNum]->getPrevTreeTop(), &peekingDone);
                        }

                        if (recoveryPossibleInInterveningBlocks && peekedCallInPred) {
                            bool peekingDone = false;
                            recoveryPossibleInInterveningBlocks
                                = treesAllowCoarsening(_monexitTrees[nextPred]->getNextTreeTop(),
                                    _blockInfo[nextPred]->getExit(), &peekingDone);
                        }
            ...
            bool simpleCoarsening = false;
            if ((*_subtraction == *_intersection) && !blockContainsCheck && !predContainsCheck && !peekedCall
                && !peekedCallInPred)
                simpleCoarsening = true;

            if ((simpleCoarsening && !_matchingSpecialBlocks->get(blockNum) && !_matchingSpecialBlocks->get(nextPred))
                || (canRecover && !_matchingNormalMonentBlocks->get(blockNum)
                    && !_matchingNormalMonexitBlocks->get(nextPred))) {
                if (!simpleCoarsening) {
                    ...
                    *_closureIntersection |= *_intersection;
                    ...
                    if (!_matchingMonexitBlocks->get(nextPred)) {
                        ...
                        TR_BitVector *succsContainingMonents = new (trStackMemory()) TR_BitVector(
                            comp()->getFlowGraph()->getNextNodeNumber(), trMemory(), stackAlloc, notGrowable);
...
TR_BitVector *TR::MonitorElimination::getBlocksAtSameNestingLevel(TR::Block *block)
{
    ...
    while (structure && !structure->asRegion()->isNaturalLoop() && (structure != rootStructure))
        structure = structure->getParent()->asRegion();

    return structure->getBlocksAtSameNestingLevel();
}

void TR::MonitorElimination::adjustMonentAndMonexitBlocks(TR::Node *prevMonitorNode, TR_BitVector *blocks,
    int32_t lockedObject)
{
...
void TR::MonitorElimination::prependMonexitAndAppendMonentInBlock(TR::Node *prevMonitorNode, TR::Block *block,
    int32_t lockedObject)
{
...
void TR::MonitorElimination::prependMonexitInBlock(TR::Node *prevMonitorNode, TR::Block *nextBlock,
    int32_t lockedObject, bool insertNullTest)
{
    TR::CoarsenedMonitorInfo *monitorInfo = findOrCreateCoarsenedMonitorInfo(lockedObject, prevMonitorNode);
...
void TR::MonitorElimination::prependMonexitInBlock(TR::Node *prevMonitorNode, TR::Block *nextBlock, bool insertNullTest)
{
    ...
    TR::Node *monexitNode
        = TR::Node::createWithSymRef(TR::monexit, 1, 1, prevMonitorNode->getFirstChild()->duplicateTree(),
            symRefTab->findOrCreateMonitorExitSymbolRef(owningMethodSymbol));
    TR::Node *treetopNode = TR::Node::createWithSymRef(TR::NULLCHK, 1, 1, monexitNode,
...
void TR::MonitorElimination::appendMonentInBlock(TR::Node *prevMonitorNode, TR::Block *nextBlock, int32_t lockedObject,
    bool insertNullTest)
{
    TR::CoarsenedMonitorInfo *monitorInfo = findOrCreateCoarsenedMonitorInfo(lockedObject, prevMonitorNode);
...
void TR::MonitorElimination::appendMonentInBlock(TR::Node *prevMonitorNode, TR::Block *nextBlock, bool insertNullTest)
{
    ...
    TR::Node *monentNode
        = TR::Node::createWithSymRef(TR::monent, 1, 1, prevMonitorNode->getFirstChild()->duplicateTree(),
            symRefTab->findOrCreateMonitorEntrySymbolRef(owningMethodSymbol));
    monentNode->setSyncMethodMonitor(true);
    TR::Node *treetopNode = TR::Node::createWithSymRef(TR::NULLCHK, 1, 1, monentNode,
        symRefTab->findOrCreateNullCheckSymbolRef(owningMethodSymbol));
    if (treetopNode->getNullCheckReference()->getOpCodeValue() != TR::loadaddr) {
        treetopNode->getNullCheckReference()->setIsNonNull(false);
        treetopNode->getNullCheckReference()->setIsNull(false);
...
void TR::MonitorElimination::insertNullTestBeforeBlock(TR::Node *prevMonitorNode, TR::Block *nextBlock)
{
    ...
    TR::Node *ifNode = TR::Node::createif(TR::ifacmpeq, prevMonitorNode->getFirstChild()->duplicateTree(),
        TR::Node::aconst(prevMonitorNode, 0), destBlock->getEntry());
    ...
    TR::Block *ifBlock
        = TR::Block::createEmptyBlock(nextBlock->getEntry()->getNode(), comp(), nextBlock->getFrequency(), nextBlock);
    ...
    for (auto edge = nextBlock->getPredecessors().begin(); edge != nextBlock->getPredecessors().end(); ++edge)
        (*edge)->getFrom()->asBlock()->getLastRealTreeTop()->adjustBranchOrSwitchTreeTop(comp(), nextBlock->getEntry(),
            ifBlock->getEntry());
...
void TR::MonitorElimination::removeLastMonexitInBlock(TR::Block *block)
{
    ...
    while (monitorTree != block->getEntry()) {
        ...
        if (monitorNode->getOpCode().isNullCheck() || monitorNode->getOpCodeValue() == TR::treetop)
        ...
        if (monitorNode->getOpCodeValue() == TR::monexit) {
            ...
            TR::Node *treetopNode = TR::Node::createWithSymRef(TR::NULLCHK, 1, 1, passThroughNode,
                symRefTab->findOrCreateNullCheckSymbolRef(
                    monitorNode->getSymbolReference()->getOwningMethodSymbol(comp())));
        ...
        for (auto edge2 = to->getPredecessors().begin(); edge2 != to->getPredecessors().end(); ++edge2) {
            if (_splitBlocks.find((*edge2)->getFrom())) {
                ...
                if (!splitBlock) {
                    ...
                    while (fromBlock && (fromBlock->getPredecessors().size() == 1)) {
                        ...
                        if (isNullTestBlock && from->hasSuccessor(nullTestBlock)) {
...
void TR::MonitorElimination::addCatchBlocks()
{
    ...
    while (listElem) {
        ...
        TR::Node *excpNode
            = TR::Node::createWithSymRef(monitorNode, TR::aload, 0, symRefTab->findOrCreateExcpSymbolRef());
        ...
        TR::Node *monexitNode
            = TR::Node::createWithSymRef(TR::monexit, 1, 1, monitorNode->getFirstChild()->duplicateTree(),
                symRefTab->findOrCreateMonitorExitSymbolRef(owningMethodSymbol));
        monexitNode->setSyncMethodMonitor(true);
...
bool TR::MonitorElimination::addClassThatShouldNotBeLoaded(char *name, int32_t len,
    TR_LinkHead<TR_ClassLoadCheck> *classesThatShouldNotBeLoaded, bool stackAllocation)
{
...
bool TR::MonitorElimination::addClassThatShouldNotBeNewlyExtended(TR_OpaqueClassBlock *clazz,
    TR_LinkHead<TR_ClassExtendCheck> *classesThatShouldNotBeNewlyExtended, bool stackAllocation)
{
...
void TR::MonitorElimination::coarsenSpecialBlock(SpecialBlockInfo *blockInfo)
{
    ...
    if (!comp()->getClassesThatShouldNotBeLoaded()->isEmpty()
        ...
        TR::CFG *cfg = comp()->getFlowGraph();
        TR::Block *gotoBlock = NULL;
        if (block->getNextBlock() && block->hasSuccessor(block->getNextBlock())) {
        ...
        if (gotoBlock) {
            TR::TreeTop *gotoTreeTop = TR::TreeTop::create(comp(),
                TR::Node::create(block->getEntry()->getNode(), TR::Goto, 0, block->getNextBlock()->getEntry()));
            gotoBlock->append(gotoTreeTop);
            newBlock->getExit()->join(gotoBlock->getEntry());
            gotoBlock->getExit()->setNextTreeTop(NULL);
            gotoBlock->setIsCold();
            gotoBlock->setFrequency(VERSIONED_COLD_BLOCK_COUNT);
            newBlock->getLastRealTreeTop()->adjustBranchOrSwitchTreeTop(comp(), block->getNextBlock()->getEntry(),
                gotoBlock->getEntry());
        ...
        TR::Block *guardBlock
            = TR::Block::createEmptyBlock(block->getEntry()->getNode(), comp(), block->getFrequency(), block);
        ...
        for (auto nextEdge = block->getPredecessors().begin(); nextEdge != block->getPredecessors().end(); ++nextEdge) {
            ...
            if (!pred->hasSuccessor(guardBlock)) {
                ...
                pred->getLastRealTreeTop()->adjustBranchOrSwitchTreeTop(comp(), block->getEntry(),
                    guardBlock->getEntry());
        ...
        for (auto nextEdge = block->getExceptionPredecessors().begin();
             nextEdge != block->getExceptionPredecessors().end(); ++nextEdge) {
            TR::Block *pred = (*nextEdge)->getFrom()->asBlock();
        ...
        for (auto nextEdge = block->getSuccessors().begin(); nextEdge != block->getSuccessors().end(); ++nextEdge) {
            TR::Block *succ = (*nextEdge)->getTo()->asBlock();
            if (gotoBlock && (succ == block->getNextBlock())) {
                // Already added above
        ...
        TR::Node *guard
            = comp()->createSideEffectGuard(comp(), guardBlock->getEntry()->getNode(), newBlock->getEntry());
    ...
    while (currTree) {
        ...
        if ((currNode->getOpCodeValue() == TR::monexit)
            || ((currNode->getNumChildren() > 0) && (currNode->getFirstChild()->getOpCodeValue() == TR::monexit))) {
            monexitTree = currTree;
            break;
        }

        if ((currNode->getOpCodeValue() == TR::treetop) && currNode->getFirstChild()->getOpCode().isStore())
        ...
        // if (!_alreadyClonedNodes.find(currNode))
        {
            ...
            if (currNode->getOpCode().isStore()) {
                ...
                if (newNode && !_alreadyClonedNodes.find(currNode)) {
                    TR::SymbolReference *symRef = currNode->getSymbolReference();
                    TR::SymbolReference *newSymRef = comp()->getSymRefTab()->createTemporary(comp()->getMethodSymbol(),
...
bool TR::MonitorElimination::checkIfSuccsInList(TR::CFGEdgeList &edgesList, TR_BitVector *list,
    bool checkIfControlFlowInCatch, TR::Block *block)
{
    for (auto edge = edgesList.begin(); edge != edgesList.end(); ++edge) {
        ...
        if (!list->get(toNum)) {
            ...
            // _adjustedMonexitBlocks->set(toNum);
            {
                ...
                if (block) {
                    bool result = true;
                    if (std::find(block->getExceptionSuccessors().begin(), block->getExceptionSuccessors().end(), *edge)
                        == block->getExceptionSuccessors().end()) {
                        logprintf(trace(), comp()->log(), "0Tripped on succ %d(%d)\n", (*edge)->getTo()->getNumber(),
                            checkIfControlFlowInCatch);
                        result = false;
                    }

                    if (!result && checkIfControlFlowInCatch) {
                        if (block->getExceptionSuccessors().size() == 1) {
                            TR::CFGNode *catchBlock = block->getExceptionSuccessors().front()->getTo();
                            if (std::find(catchBlock->getSuccessors().begin(), catchBlock->getSuccessors().end(), *edge)
                                != catchBlock->getSuccessors().end())
                                result = true;
                        }
                    }
...
// bool TR::MonitorElimination::checkIfPredsInList(TR_PredecessorIterator *edgesIt, TR_BitVector *list, TR::Block
// *block)
...
TR::CoarsenedMonitorInfo *TR::MonitorElimination::findOrCreateCoarsenedMonitorInfo(int32_t lockedObject,
    TR::Node *lockedNode)
{
    TR::CoarsenedMonitorInfo *monitorInfo = findCoarsenedMonitorInfo(lockedObject);
    if (!monitorInfo) {
        monitorInfo = new (trStackMemory())
            TR::CoarsenedMonitorInfo(trMemory(), lockedObject, comp()->getFlowGraph()->getNextNodeNumber(), lockedNode);
...
bool TR::MonitorElimination::isSimpleLockedRegion(TR::TreeTop *monentTree)
{
    ...
    if ((monNode->getOpCodeValue() == TR::monexit)
        || ((monNode->getNumChildren() > 0) && (monNode->getFirstChild()->getOpCodeValue() == TR::monexit))) {
        ...
        while (cursorTree) {
            ...
            if ((cursorNode->exceptionsRaised() != 0) || (cursorNode->getOpCode().isStoreIndirect())
                || (cursorNode->getOpCode().isStore() && cursorNode->getSymbolReference()->getSymbol()->isStatic())
    ...
    while (currTree) {
        ...
        if ((currNode->getOpCodeValue() == TR::monent) || (currNode->exceptionsRaised() != 0)
            || (currNode->getOpCode().isStoreIndirect()) || (currNode->getOpCodeValue() == TR::BBEnd)) {
            return false;
        }
...
bool TR::MonitorElimination::treesAllowCoarsening(TR::TreeTop *startTree, TR::TreeTop *endTree, bool *peekedCall,
    bool *seenCheck)
{
    ...
    while (cursorTree != endTree) {
        ...
        if (node->getOpCode().isResolveCheck()
            || ((node->getOpCodeValue() == TR::monexit) || (node->getOpCodeValue() == TR::monent))
            || ((node->getNumChildren() > 0)
        ...
        if (node->getOpCode().isCall() &&
            ...
            TR_ResolvedMethod *resolvedMethod = node->getSymbolReference()
                                                    ->getSymbol()
                                                    ->castToMethodSymbol()
            ...
            if (!ipaAnalyzer.analyzeCall(node)) {
                ...
                *peekedCall = true;
                for (TR::GlobalSymbol *currSym = ipaAnalyzer._globalsWritten.getFirst(); currSym;
                     currSym = currSym->getNext()) {
                        ...
                        if (symRef->getSymbol()->isStatic()
                            ...
                            if (symRef->getSymbol()->isStatic()) {
                                if (symRef->getSymbol()->isConstObjectRef())
                                    ...
                                    sig = symRef->getOwningMethod(comp())->staticName(symRef->getCPIndex(), length,
                                        trMemory());
                            } else if (symRef->getSymbol()->isShadow()) {
                                sig = symRef->getOwningMethod(comp())->fieldName(symRef->getCPIndex(), length,
                                    trMemory());
                            }
                            ...
                            if (currSymReference->getSymbol()->isStatic()) {
                                if (currSymReference->getSymbol()->isConstObjectRef())
                                    ...
                                    currSig = currSymReference->getOwningMethod(comp())->staticName(
                                        currSymReference->getCPIndex(), currLength, trMemory());
                            } else if (currSymReference->getSymbol()->isShadow()) {
                                currSig = currSymReference->getOwningMethod(comp())->fieldName(
                                    currSymReference->getCPIndex(), currLength, trMemory());
                            }

                            if ((length == currLength) && (memcmp(sig, currSig, length) == 0)) {
                                logprintf(trace(), comp()->log(),
                                    "Recovery is NOT possible from call %p to method %s due to written symbols\n", node,
                                    resolvedMethod->signature(trMemory()));
                                return false;
                            }
                        }
                ...
                for (TR_ClassLoadCheck *clc = ipaAnalyzer._classesThatShouldNotBeLoaded.getFirst(); clc;
                     clc = clc->getNext())
                    addClassThatShouldNotBeLoaded(clc->_name, clc->_length, &_classesThatShouldNotBeLoaded, true);

                for (TR_ClassExtendCheck *cec = ipaAnalyzer._classesThatShouldNotBeNewlyExtended.getFirst(); cec;
                     cec = cec->getNext())
...
bool TR::MonitorElimination::symbolsAreNotWrittenInTrees(TR::TreeTop *startTree, TR::TreeTop *endTree)
{
    ...
    while (cursorTree != endTree) {
        ...
        if (cursorNode->getOpCode().isNullCheck() || cursorNode->getOpCode().isResolveCheck()
            || (cursorNode->getOpCodeValue() == TR::treetop))
            cursorNode = cursorNode->getFirstChild();

        if (cursorNode->getOpCode().isStore() || cursorNode->mightHaveNonTransparentSymbolReference()) {
            ...
            || (cursorNode->getOpCode().hasSymbolReference() && cursorNode->getSymbolReference()->isUnresolved())) {
            bool isCallDirect = false;
            if (cursorNode->getOpCode().isCallDirect())
                isCallDirect = true;

            if (cursorNode->getSymbolReference()
                    ->getUseDefAliases(isCallDirect)
                    .containsAny(*_symRefsInSimpleLockedRegion, comp()))
                symbolsNotWritten = false;
        }
...
bool TR::CoarseningInterProceduralAnalyzer::analyzeNode(TR::Node *node, vcount_t visitCount, bool *success)
{
    ...
    if (symRef && symRef->isUnresolved()) {
        if (!(symRef->getSymbol()->isStatic() && symRef->getSymbol()->castToStaticSymbol()->isConstObjectRef())) {
            if ((node->getOpCodeValue() == TR::loadaddr)
                && (node->getSymbolReference()->getSymbol()->isClassObject())) {
                uint32_t classNameLength;
                char *className = symRef->getOwningMethod(comp())->getClassNameFromConstantPool(symRef->getCPIndex(),
                    classNameLength);

                if (!className) {
                    *success = false;
                    logprintf(trace(), log,
                        "Found unresolved class object load %p while peeking and unable to add assumption -- peek "
                        "unsuccessful\n",
                        node);
                } else {
                    addClassThatShouldNotBeLoaded(className, classNameLength);
                    logprintf(trace(), log,
                        "Found unresolved class object node %p while peeking -- add assumption -- skip peeking in rest "
                        "of block\n",
                        node);
                ...
                if (symRef->getSymbol()->isShadow() || symRef->getSymbol()->isStatic()) {
                    className = symRef->getOwningMethod(comp())->classNameOfFieldOrStatic(symRef->getCPIndex(),
                        classnameLength);
                    ...
                    if (originalMethod) {
                        ...
                        className = TR::Compiler->cls.classNameToSignature(originalMethod->classNameChars(),
                            classnameLength, comp());
                    }
                }

                if (className) {
                    TR_OpaqueClassBlock *declaringClazz = comp()->fej9()->getClassFromSignature(className,
                        classnameLength, symRef->getOwningMethod(comp()));
                    if (!declaringClazz) {
                        addClassThatShouldNotBeLoaded(className, classnameLength);

                        logprintf(trace(), log,
                            "Found unresolved class object node %p while peeking -- skip peeking in rest of block\n",
                            node);
                    ...
                    /*
                      ...
                      //
                       ...
                       {
                       *success = false;
                       logprintf(trace(), log, "Found unresolved node %p while peeking whose class in resolved -- peek
                    unsuccessful\n", node);
                       }
                    */
                } else {
                    *success = false;
                    logprintf(trace(), log,
                        "Found unresolved node %p while peeking whose class is unresolved and unable to add assumption "
                        "-- peek unsuccessful\n",
                        node);
                }
            }
