...
const char *TR_LoopAliasRefiner::optDetailString() const throw() { return "O^O LOOP ALIAS REFINER: "; }
...
void TR_LoopAliasRefiner::collectArrayAliasCandidates(TR::Node *parentArrayNode, TR::Node *node, vcount_t visitCount,
    bool isStore)
{
    OMR::Logger *log = comp()->log();

    if (node->getOpCodeValue() == TR::aiadd || node->getOpCodeValue() == TR::aladd) {
        logprintf(trace(), log, "LAR: Inspecting aiadd %p\n", node);
        ...
        if (comp()->getSymRefTab()->isRefinedArrayShadow(parentArrayNode->getSymbolReference())) {
            logprintf(trace(), log, "FAIL: Shadow #%d in [%p] already refined\n",
        ...
        if (!parentArrayNode->getSymbol()->isArrayShadowSymbol()) {
            logprintf(trace(), log, "FAIL: Shadow #%d in [%p] is not an array shadow\n",
                parentArrayNode->getSymbolReference()->getReferenceNumber(), parentArrayNode);
            return;
        }
        ...
        if (parentArrayNode->getSymbol()->isUnsafeShadowSymbol()) {
            logprintf(trace(), log, "FAIL: Shadow #%d in [%p] is an unsafe shadow\n",
                parentArrayNode->getSymbolReference()->getReferenceNumber(), parentArrayNode);
            return;
        }
        ...
        if (!_containsCall
            ...
            && _currentNaturalLoop->isExprInvariant(arrayAddress) && arrayAddress->getSymbol()->isCollectedReference()
            && !arrayAddress->getSymbol()->isInternalPointerAuto()) {
            logprintf(trace(), log, "\tA) Adding candidate node %p parent %p for block_%d\n", node, parentArrayNode,
                _currentBlock->getNumber());
            _arrayMemberLoadCandidates->add(
                new (trStackMemory()) TR_NodeParentBlockTuple(node, parentArrayNode, _currentBlock));
        }
    }
}
...
bool TR_LoopAliasRefiner::processArrayAliasCandidates()
{
    ...
    if (isAlreadyProcessed(_currentNaturalLoop->getNumber())) {
        logprintf(trace(), log, "Already processed loop %d\n", _currentNaturalLoop->getNumber());
        return false;
    }

    logprintf(trace(), log, "LAR: Processing loop %d\n", _currentNaturalLoop->getNumber());

    markAsProcessed(_currentNaturalLoop->getNumber());

    if (!SPMDPreCheck::isSPMDCandidate(comp(), _currentNaturalLoop)) {
        logprintf(trace(), log, "LAR: SPMDPreCheck failed - skipping consideration of loop %d\n",
            _currentNaturalLoop->getNumber());
        return false;
    }
    ...
    logprintf(trace(), log, "LAR: Finished loop processing\n\t%s\n",
        (haveGoodMemberCandidates) && foundSecondArray ? "Candidates exist" : "No Candidates");

    if (!foundSecondArray || !haveGoodMemberCandidates)
        return false;
    ...
    while ((curTuple = _arrayMemberLoadCandidates->popHead())) {
        ...
        TR_ScratchList<TR_NodeParentBlockTuple> *copyOfCandidateRefs
            = new (trStackMemory()) TR_ScratchList<TR_NodeParentBlockTuple>(trMemory());
        ...
        for (; curTuple; curTuple = memberCand.getNext()) {
            ...
            if (curTupleBaseSymRef == currentBaseSymRef && curTupleMemberSymRef == currentMemberSymRef) {
        ...
        if (goodCandidateDetected) {
            logprintf(trace(), log, "\tAdding entry for base #%d member #%d offset %d with %d refs\n",
                currentBaseSymRef->getReferenceNumber(),
                currentMemberSymRef ? currentMemberSymRef->getReferenceNumber() : 0,
                currentMemberSymRef ? currentMemberSymRef->getOffset() : 0, refCount);
...
void TR_LoopAliasRefiner::buildAliasRefinementComparisonTrees(List<TR::TreeTop> *nullCheckTrees,
    List<TR::TreeTop> *divCheckTrees, List<TR::TreeTop> *checkCastTrees, List<TR::TreeTop> *arrayStoreCheckTrees,
    TR_ScratchList<TR::Node> *comparisonTrees, TR::Block *exitGotoBlock)
{
    if (!_arrayRanges) {
        logprintf(trace(), comp()->log(), "array ranges is null for %s\n", comp()->signature());
        return;
    }
    ...
    for (arrayPtr = arangeIterator.getFirst(); arrayPtr; arrayPtr = arangeIterator.getNext()) {
        ...
        collectAllExpressionsToBeChecked(nullCheckTrees, divCheckTrees, checkCastTrees, arrayStoreCheckTrees,
            nodeToBeNullChkd, comparisonTrees, exitGotoBlock, visitCount);
    }
    ...
    while (ArrayRangeLimits *arlAPtr = _arrayRanges->popHead()) {
        ...
        for (arlBPtr = arIterator.getFirst(); arlBPtr; arlBPtr = arIterator.getNext()) {
            ...
            if (testExpr
                && performTransformation(comp(), "%sAdding test [%p] to refine aliases for loop %d\n",
                    optDetailString(), testExpr, _currentNaturalLoop->getNumber())) {
...
void TR_LoopAliasRefiner::refineArrayAliases(TR_RegionStructure *whileLoop)
{
    if (_arrayRanges
    ...
    for (ArrayRangeLimits *arlPtr = arIterator.getFirst(); arlPtr; arlPtr = arIterator.getNext()) {
        ...
        for (npbt = candIterator.getFirst(); npbt; npbt = candIterator.getNext()) {
            ...
            if (targetNode->getVisitCount() == visitCount)
                continue;
            targetNode->setVisitCount(visitCount);

            TR_ASSERT(targetNode->getOpCode().isLoad() || targetNode->getOpCode().isStore(),
                "Unexpected opcode on %p\n", targetNode);
            oldShadow = targetNode->getSymbolReference();

            if (!newShadow) {
                newShadow
                    = comp()->getSymRefTab()->createRefinedArrayShadowSymbolRef(oldShadow->getSymbol()->getDataType());

                dumpOptDetails(comp(), "Replacing1 shadow #%d with #%d in [%p] %d %d\n",
                    oldShadow->getReferenceNumber(), newShadow->getReferenceNumber(), targetNode,
                    oldShadow->getSymbol()->getDataType().getDataType(),
                    newShadow->getSymbol()->getDataType().getDataType());

                ListIterator<TR::SymbolReference> symRefIterator(&newShadowList);

                for (TR::SymbolReference *symRef = symRefIterator.getFirst(); symRef;
                     symRef = symRefIterator.getNext()) {
            ...
            TR_ASSERT(newShadow->getSymbol()->getDataType() == oldShadow->getSymbol()->getDataType(),
                "shadow's types don't match\n");
 ...
 */
TR::Node *TR_LoopAliasRefiner::ArrayRangeLimits::createRangeTestExpr(TR::Compilation *comp, ArrayRangeLimits *other,
    TR::Block *targetBlock, bool trace)
{
    ...
    dumpOptDetails(comp, "#%d(%d) (member #%d(%d) vs. #%d(%d) (member #%d(%d))\n",
        getBaseSymRef()->getReferenceNumber(), getBaseSymRef()->getOffset(),
        getMemberSymRef() ? getMemberSymRef()->getReferenceNumber() : 0,
        getMemberSymRef() ? getMemberSymRef()->getOffset() : 0, other->getBaseSymRef()->getReferenceNumber(),
