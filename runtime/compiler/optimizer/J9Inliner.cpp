...
const float MIN_PROFILED_CALL_FREQUENCY
    = (.65f); // lowered this from .80f since opportunities were being missed in WAS; in those cases getting rid of the
              // call even in 65% of the cases was beneficial probably due to the improved icache impact
...
static TR::KnownObjectTable::Index failMCS(const char *reason, TR_CallSite *callSite, TR_InlinerBase *inliner)
{
    debugTrace(inliner->tracer(), "  Fail isMutableCallSiteTargetInvokeExact(%p): %s", callSite, reason);
    return TR::KnownObjectTable::UNKNOWN;
}

static TR::KnownObjectTable::Index isMutableCallSiteTargetInvokeExact(TR_CallSite *callSite, TR_InlinerBase *inliner)
{
...
TR_CallSite *TR_CallSite::create(TR::TreeTop *callNodeTreeTop, TR::Node *parent, TR::Node *callNode,
    TR_OpaqueClassBlock *receiverClass, TR::SymbolReference *symRef, TR_ResolvedMethod *resolvedMethod,
    TR::Compilation *comp, TR_Memory *trMemory, TR_AllocationKind kind, TR_ResolvedMethod *caller, int32_t depth,
    bool allConsts)

{
    ...
    if (callNode->getOpCode().isCallIndirect()) {
        if (calleeSymbol->isInterface()) {
            return new (trMemory, kind) TR_J9InterfaceCallSite(lCaller, callNodeTreeTop, parent, callNode,
                calleeSymbol->getMethod(), receiverClass, (int32_t)symRef->getOffset(), symRef->getCPIndex(),
                resolvedMethod, calleeSymbol->getResolvedMethodSymbol(), callNode->getOpCode().isCallIndirect(),
                calleeSymbol->isInterface(), callNode->getByteCodeInfo(), comp, depth, allConsts,
                comp->retainedMethods(), wasRefinedFromKnownObject);
        } else {
            if (calleeSymbol->getResolvedMethodSymbol()
                ...
                return new (trMemory, kind) TR_J9MethodHandleCallSite(lCaller, callNodeTreeTop, parent, callNode,
                    calleeSymbol->getMethod(), receiverClass, (int32_t)symRef->getOffset(), symRef->getCPIndex(),
                    resolvedMethod, calleeSymbol->getResolvedMethodSymbol(), callNode->getOpCode().isCallIndirect(),
                    calleeSymbol->isInterface(), callNode->getByteCodeInfo(), comp, depth, allConsts,
                    comp->retainedMethods(), wasRefinedFromKnownObject);
            ...
            if (calleeSymbol->getResolvedMethodSymbol()
                && calleeSymbol->getResolvedMethodSymbol()->getRecognizedMethod()
                    == TR::java_lang_invoke_MethodHandle_invokeExact) {
                return new (trMemory, kind) TR_J9MutableCallSite(lCaller, callNodeTreeTop, parent, callNode,
                    calleeSymbol->getMethod(), receiverClass, (int32_t)symRef->getOffset(), symRef->getCPIndex(),
                    resolvedMethod, calleeSymbol->getResolvedMethodSymbol(), callNode->getOpCode().isCallIndirect(),
                    calleeSymbol->isInterface(), callNode->getByteCodeInfo(), comp, depth, allConsts,
                    comp->retainedMethods(), wasRefinedFromKnownObject);
            ...
            return new (trMemory, kind) TR_J9VirtualCallSite(lCaller, callNodeTreeTop, parent, callNode,
                calleeSymbol->getMethod(), receiverClass, (int32_t)symRef->getOffset(), symRef->getCPIndex(),
                resolvedMethod, calleeSymbol->getResolvedMethodSymbol(), callNode->getOpCode().isCallIndirect(),
                calleeSymbol->isInterface(), callNode->getByteCodeInfo(), comp, depth, allConsts,
                comp->retainedMethods(), wasRefinedFromKnownObject);
    ...
    return new (trMemory, kind) TR_DirectCallSite(lCaller, callNodeTreeTop, parent, callNode, calleeSymbol->getMethod(),
        resolvedMethod && !resolvedMethod->isStatic() ? receiverClass : NULL, (int32_t)symRef->getOffset(),
        symRef->getCPIndex(), resolvedMethod, calleeSymbol->getResolvedMethodSymbol(),
        callNode->getOpCode().isCallIndirect(), calleeSymbol->isInterface(), callNode->getByteCodeInfo(), comp, depth,
        allConsts, comp->retainedMethods(), wasRefinedFromKnownObject);
}

static void computeNumLivePendingSlotsAndNestingDepth(TR::Optimizer *optimizer, TR_CallTarget *calltarget,
    TR_CallStack *callStack, int32_t &numLivePendingPushSlots, int32_t &nestingDepth)
{
    ...
    if (comp->getOption(TR_EnableOSR)) {
        ...
        TR::ResolvedMethodSymbol *caller
            = (callerIndex == -1) ? comp->getMethodSymbol() : comp->getInlinedResolvedMethodSymbol(callerIndex);
        ...
        TR_BitVector *deadSymRefs
            = osrMethodData->getLiveRangeInfo(calltarget->_myCallSite->_callNode->getByteCodeIndex());

        for (int32_t i = 0; i < numPendingSlots; i++) {
            List<TR::SymbolReference> symRefsAtThisSlot = (*pendingPushSymRefs)[i];

            if (symRefsAtThisSlot.isEmpty())
                continue;
 ...
 */
static void populateOSRCallSiteRematTable(TR::Optimizer *optimizer, TR_CallTarget *calltarget, TR_CallStack *callStack)
{
    ...
    //
    for (TR::TreeTop *cursor = call->getPrevTreeTop(); cursor && method->isOSRRelatedNode(cursor->getNode(), bci);
    ...
    //
    if (failedPP.size() > 0)
        RematTools::walkTreeTopsCalculatingRematFailureAlternatives(comp, blockStart, call, failedPP, scanTargets,
            safetyInfo, verboseCallSiteRemat != NULL);
    ...
    if (!scanTargets.IsZero())
        RematTools::walkTreesCalculatingRematSafety(comp, blockStart, call, scanTargets, unsafeSymRefs,
            verboseCallSiteRemat != NULL);
        ...
        if (!unsafeSymRefs.Intersects(safetyInfo.symRefDependencies(i))) {
            if (storeTree == rematTree) {
                ...
                comp->setOSRCallSiteRemat(comp->getCurrentInlinedSiteIndex(),
                    storeTree->getNode()->getSymbolReference(), child->getSymbolReference());
            } else {
                int32_t callerIndex = node->getByteCodeInfo().getCallerIndex();
                if (node->getSymbol()->isParm() || node->getSymbol()->isPendingPush()
                    ...
                    comp->setOSRCallSiteRemat(comp->getCurrentInlinedSiteIndex(),
                        storeTree->getNode()->getSymbolReference(), node->getSymbolReference());
                }
            }
        }
    }
...
bool TR_InlinerBase::inlineCallTarget(TR_CallStack *callStack, TR_CallTarget *calltarget, bool inlinefromgraph,
    TR_PrexArgInfo *argInfo, TR::TreeTop **cursorTreeTop)
{
    ...
    if (!((TR_J9InlinerPolicy *)getPolicy())
             ->doCorrectnessAndSizeChecksForInlineCallTarget(callStack, calltarget, inlinefromgraph, argInfo)) {
    ...
    if (!comp()->incInlineDepth(calltarget->_calleeSymbol, calltarget->_myCallSite->_callNode,
            !calltarget->_myCallSite->_isIndirectCall, calltarget->_guard, calltarget->_receiverClass, argInfo)) {
        return false;
    }
    ...
    if (comp()->getOption(TR_EnableOSR)) {
        computeNumLivePendingSlotsAndNestingDepth(_optimizer, calltarget, callStack, numLivePendingPushSlots,
            nestingDepth);
    }

    // Add the pending pushes above this call to the OSRCallSiteRematTable
    if (comp()->getOption(TR_EnableOSR) && !comp()->getOption(TR_DisableOSRCallSiteRemat)
        && comp()->getOSRMode() == TR::voluntaryOSR && comp()->isOSRTransitionTarget(TR::postExecutionOSR)
    ...
    if (NumInlinedMethods != NULL) {
        ...
        InlinedSizes[comp()->getMethodHotness()] += TR::Compiler->mtd.bytecodeSize(
            calltarget->_calleeSymbol->getResolvedMethod()->getPersistentIdentifier());
    }
    return successful;
}
...
{
    return comp()->getPersistentInfo()->getPersistentCHTable()->findSingleJittedImplementer(_receiverClass,
        TR::Compiler->cls.isInterfaceClass(comp(), _receiverClass) ? _cpIndex : _vftSlot, _callerResolvedMethod, comp(),
...
bool TR_J9VirtualCallSite::findCallSiteForAbstractClass(TR_InlinerBase *inliner)
{
    ...
    if (canInline && TR::Compiler->cls.isAbstractClass(comp(), _receiverClass)
        ...
        TR_VirtualGuardSelection *guard
            = new (comp()->trHeapMemory()) TR_VirtualGuardSelection(TR_AbstractGuard, TR_MethodTest);
...
TR_OpaqueClassBlock *TR_J9VirtualCallSite::getClassFromMethod() { return _initialCalleeMethod->classOfMethod(); }
...
bool TR_J9VirtualCallSite::isBasicInvokeVirtual()
{
    TR_OpaqueMethodBlock *method
        = ((TR_ResolvedJ9Method *)_initialCalleeMethod->owningMethod())->getPersistentIdentifier();
    int32_t methodSize = TR::Compiler->mtd.bytecodeSize(method);
    uintptr_t methodStart = TR::Compiler->mtd.bytecodeStart(method);

    TR_ASSERT_FATAL(_bcInfo.getByteCodeIndex() >= 0 && _bcInfo.getByteCodeIndex() + 2 < methodSize,
        "Bytecode index can't be less than zero or higher than the methodSize");
...
bool TR_J9VirtualCallSite::findCallSiteTarget(TR_CallStack *callStack, TR_InlinerBase *inliner)
{
    ...
    if (_cpIndex != -1 && _receiverClass && TR::Compiler->cls.isInterfaceClass(comp(), _receiverClass)
        ...
        if (nameLen && nameLen == _initialCalleeMethod->nameLength() && sigLen
            && sigLen == _initialCalleeMethod->signatureLength() && strncmp(cpMethodName, methodName, nameLen) == 0
            && strncmp(cpMethodSig, methodSig, sigLen) == 0) {
            ...
            if (callSiteClass && callSiteClass != _receiverClass) {
                if (comp()->fej9()->isJavaLangObject(callSiteClass))
                    ...
                    TR_YesNoMaybe callSiteClassOk = fe()->isInstanceOf(callSiteClass, defInterface, true, true, true);

                    TR_ASSERT_FATAL(callSiteClassOk == TR_yes,
                        "class %p inherits a method from interface %p without implementing it", callSiteClass,
                        defInterface);

                    _isCallingObjectMethod = TR_no;
                    if (comp()->trace(OMR::inlining)) {
                        char *oldClassSig
                            = TR::Compiler->cls.classSignature(comp(), _receiverClass, comp()->trMemory());
                        char *callSiteClassSig
                            = TR::Compiler->cls.classSignature(comp(), callSiteClass, comp()->trMemory());
                        comp()->log()->printf("Receiver type %p sig %s is class of an interface method for "
    ...
    if (addTargetIfMethodIsNotOverriden(inliner) || addTargetIfMethodIsNotOverridenInReceiversHierarchy(inliner)
        || findCallSiteForAbstractClass(inliner) || addTargetIfThereIsSingleImplementer(inliner)) {
        return true;
    }
...
static TR_ResolvedMethod * findSingleImplementer(
   TR_OpaqueClassBlock * thisClass, int32_t cpIndexOrVftSlot, TR_ResolvedMethod * callerMethod, TR::Compilation * comp,
bool locked, TR_YesNoMaybe useGetResolvedInterfaceMethod)
   {
...
comp->getPersistentInfo()->getPersistentCHTable()->findClassInfoAfterLocking(thisClass, comp, true); if (!classInfo)
      {
      return 0;
      }

   TR_ResolvedMethod *implArray[2]; // collect maximum 2 implementers if you can
   int32_t implCount = TR_ClassQueries::collectImplementorsCapped(classInfo, implArray, 2, cpIndexOrVftSlot,
callerMethod, comp, locked, useGetResolvedInterfaceMethod); return (implCount == 1 ? implArray[0] : 0);
   }
*/
...
bool TR_J9InterfaceCallSite::findCallSiteTarget(TR_CallStack *callStack, TR_InlinerBase *inliner)
{
    ...
    //
    if (_receiverClass != NULL && !TR::Compiler->cls.isInterfaceClass(comp(), _receiverClass)) {
        TR_ASSERT_FATAL(fe()->isInstanceOf(_receiverClass, iface, true, true, true) == TR_yes,
            "interface call site %p receiver type %p "
            "does not implement the expected interface %p",
            this, _receiverClass, iface);

        heuristicTrace(inliner->tracer(), "Interface call site %p has receiver class bound %p; nop guards ok", this,
            _receiverClass);
    } else {
        ...
        for (int32_t i = 0; i < numTargets(); i++) {
            ...
            TR_ASSERT_FATAL(kind == TR_ProfiledGuard,
                "interface call site %p requires profiled guard (kind %d), "
                "but target %d [%p] uses %s (kind %d)",
                this, (int)TR_ProfiledGuard, i, tgt,
                debug == NULL ? "<unknown name>" : debug->getVirtualGuardKindName(kind), (int)kind);
            ...
            TR_ASSERT_FATAL(fe()->isInstanceOf(passClass, iface, true, true, true) == TR_yes,
                "interface call site %p target %d [%p] (J9Method %p) "
                "accepts receivers of type %p, "
                "which does not implement the expected interface %p",
                this, i, tgt, callee->getPersistentIdentifier(), passClass, iface);
        }
    }
    ...
    TR_OpaqueClassBlock *iface)
{
    ...
    // TR_ResolvedMethod* calleeResolvedMethod = inliner->findInterfaceImplementationToInline(_interfaceMethod,
    // _cpIndex, _callerResolvedMethod, _receiverClass);
    TR_ResolvedMethod *calleeResolvedMethod
        = comp()->getPersistentInfo()->getPersistentCHTable()->findSingleImplementer(_receiverClass, _cpIndex,
            _callerResolvedMethod, inliner->comp(), false, TR_yes);

    if (!comp()->performVirtualGuardNOPing()
        || (comp()->compileRelocatableCode() && !TR::Options::getCmdLineOptions()->allowRecompilation())) {
        calleeResolvedMethod = NULL;
    }

    heuristicTrace(inliner->tracer(), "Found a Single Interface Implementer with Resolved Method %p for callsite %p",
        calleeResolvedMethod, this);
    ...
    if (calleeResolvedMethod && !calleeResolvedMethod->virtualMethodIsOverridden()) {
        ...
        if (_receiverClass != NULL && !TR::Compiler->cls.isInterfaceClass(comp(), _receiverClass)) {
            ...
            // body to be run for more receivers.
                ...
                TR_ValueProfileInfoManager *profMgr = TR_ValueProfileInfoManager::get(comp());

                TR_AddressInfo *valueInfo = NULL;
                if (profMgr != NULL) {
                    valueInfo = static_cast<TR_AddressInfo *>(profMgr->getValueInfo(_bcInfo, comp(), AddressInfo));
                }

                if (useVftTestHeuristics && valueInfo != NULL && !comp()->getOption(TR_DisableProfiledInlining)) {
                    TR_ASSERT_FATAL(!comp()->compileRelocatableCode()
...

                    {
                        ...
                        for (; cur != NULL; cur = it.getNext()) {
                            auto *curClass = reinterpret_cast<TR_OpaqueClassBlock *>(cur->_value);
                    ...
                    if (topProfiledClass != NULL && remainingTotalFreq >= 32 && topProfiledFreq == remainingTotalFreq) {
                        bool valid = true;
        ...
        if (calleeResolvedMethod != NULL) {
            ...
            TR_VirtualGuardSelection *guard
                = new (comp()->trHeapMemory()) TR_VirtualGuardSelection(kind, testType, thisClass);
            ...
            addTarget(comp()->trMemory(), inliner, guard, calleeResolvedMethod, thisClass, heapAlloc);
...
void TR_J9InterfaceCallSite::findSingleProfiledMethod(ListIterator<TR_ExtraAddressInfo> &sortedValuesIt,
    TR_AddressInfo *valueInfo, TR_InlinerBase *inliner)
{
    return;
}
...
bool TR_J9MethodHandleCallSite::findCallSiteTarget(TR_CallStack *callStack, TR_InlinerBase *inliner)
{
    ...
    addTarget(comp()->trMemory(), inliner, new (comp()->trHeapMemory()) TR_VirtualGuardSelection(TR_NoGuard),
        _initialCalleeMethod, _receiverClass, heapAlloc);
...
bool TR_J9MutableCallSite::findCallSiteTarget(TR_CallStack *callStack, TR_InlinerBase *inliner)
{
    ...
    if (_mcs != TR::KnownObjectTable::UNKNOWN) {
        ...
        TR_VirtualGuardSelection *vgs
            = new (comp()->trHeapMemory()) TR_VirtualGuardSelection(TR_MutableCallSiteTargetGuard, TR_DummyTest);
        ...
        if (vgs->_mutableCallSiteEpoch != TR::KnownObjectTable::UNKNOWN) {
            ...
            TR_ASSERT_FATAL(targetJ9Method != NULL, "failed to find MCS target (obj%d) LambdaForm method",
                (int)vgs->_mutableCallSiteEpoch);

            TR_ResolvedMethod *targetMethod
                = comp()->fej9()->createResolvedMethod(comp()->trMemory(), targetJ9Method, callStack->_method);

            heuristicTrace(inliner->tracer(), "Refine callee of MCS target invokeBasic to %s\n",
                targetMethod->signature(comp()->trMemory(), stackAlloc));
#else
            TR::MethodSymbol::Kinds methodKind = TR::MethodSymbol::ComputedVirtual;
            TR_ResolvedMethod *targetMethod = comp()->fej9()->createMethodHandleArchetypeSpecimen(comp()->trMemory(),
                knot->getPointerLocation(vgs->_mutableCallSiteEpoch), _callerResolvedMethod);
#endif
            TR_CallTarget *target
                = addTarget(comp()->trMemory(), inliner, vgs, targetMethod, _receiverClass, heapAlloc);
            TR_ASSERT(target, "There should be only one target for TR_MutableCallSite");
            target->_calleeMethodKind = methodKind;

            heuristicTrace(inliner->tracer(), "  addTarget: MutableCallSite %p epoch is obj%d",
...
bool TR_InlinerBase::tryToGenerateILForMethod(TR::ResolvedMethodSymbol *calleeSymbol,
...
void TR_InlinerBase::getBorderFrequencies(int32_t &hotBorderFrequency, int32_t &coldBorderFrequency,
    TR_ResolvedMethod *calleeResolvedMethod, TR::Node *callNode)
{
...
int32_t TR_InlinerBase::scaleSizeBasedOnBlockFrequency(int32_t bytecodeSize, int32_t frequency, int32_t borderFrequency,
    TR_ResolvedMethod *calleeResolvedMethod, TR::Node *callNode, int32_t coldBorderFrequency)
{
    ...
    if (frequency > borderFrequency) {
        ...
        if (bytecodeSize < 10)
            bytecodeSize = 10;
    } else if (frequency < coldBorderFrequency && !alwaysWorthInlining(calleeResolvedMethod, callNode)) {
...
float TR_MultipleCallTargetInliner::getScalingFactor(float factor) { return std::max(factor, 0.7f); }

void TR_ProfileableCallSite::findSingleProfiledReceiver(ListIterator<TR_ExtraAddressInfo> &sortedValuesIt,
    TR_AddressInfo *valueInfo, TR_InlinerBase *inliner)
{
    OMR::Logger *log = comp()->log();
    bool trace = comp()->trace(OMR::inlining);
    ...
    for (TR_ExtraAddressInfo *profiledInfo = sortedValuesIt.getFirst(); profiledInfo != NULL;
        ...
        float val = (float)freq
            / (float)valueInfo
                  ->getTotalFrequency(); // x87 hardware rounds differently if you leave this division in compare
        ...
        if (!isClassObsolete) {
            ...
            // high opt level compiles during JIT STARTUP could be affected by classes being loaded - maximize the
            // chances of success by using method tests
        ...
        static const float minProfiledCallFrequency = userMinProfiledCallFreq ? atof(userMinProfiledCallFreq)
            : comp()->getOption(TR_DisableMultiTargetInlining)                ? MIN_PROFILED_CALL_FREQUENCY
                                                                              : .10f;

        if ((val >= minProfiledCallFrequency || (firstInstanceOfCheckFailed && val >= SECOND_BEST_MIN_CALL_FREQUENCY))
            ...
            if (callSiteClass && !isInterface() && TR::Compiler->cls.isInterfaceClass(comp(), callSiteClass)
                && isCallingObjectMethod() != TR_yes) {
                // TR_J9VirtualCallSite::findCallSiteTarget() should have refined the _receiverClass, but it must have
                // failed, we need to abort this inlining
                logprintf(trace, log,
                    "inliner: callSiteClass [%p] is an interface making it impossible to confirm correct context of "
                    "the profiled class [%p]\n",
            ...
            if (comp()->compileRelocatableCode()) {
                ...
                if (!fej9->canRememberClass(tempreceiverClass) || !fej9->canRememberClass(callSiteClass)) {
                    logprintf(trace, log,
                        "inliner: profiled class [%p] or callSiteClass [%p] cannot be rememberd in shared cache\n",
                        tempreceiverClass, callSiteClass);
                    continue;
                }
            }
            ...
            if (preferMethodTest)
                guard = new (comp()->trHeapMemory())
                    TR_VirtualGuardSelection(TR_ProfiledGuard, TR_MethodTest, tempreceiverClass);
            else
                guard = new (comp()->trHeapMemory())
                    TR_VirtualGuardSelection(TR_ProfiledGuard, TR_VftTest, tempreceiverClass);
            ...
            heuristicTrace(inliner->tracer(), "Creating a profiled call. callee Symbol %p frequencyadjustment %f",
                _initialCalleeSymbol, val);
...
void TR_ProfileableCallSite::findSingleProfiledMethod(ListIterator<TR_ExtraAddressInfo> &sortedValuesIt,
    TR_AddressInfo *valueInfo, TR_InlinerBase *inliner)
{
    OMR::Logger *log = comp()->log();
    bool trace = comp()->trace(OMR::inlining);
    ...
    if (!callSiteClass
        || (TR::Compiler->cls.isInterfaceClass(comp(), callSiteClass) && isCallingObjectMethod() != TR_yes)) {
        // TR_J9VirtualCallSite::findCallSiteTarget() should refine the _receiverClass, but if it failed, we need to
        // abort this inlining
        if (callSiteClass && trace)
            log->printf("callSiteClass [%p] is an interface making it impossible to confirm correct context for any "
                        "profiled class\n",
                callSiteClass);
        return;
    }
    ...
    logprints(trace, log,
        "No decisive class profiling info for the virtual method, we'll try to see if more than one class uses the "
    ...
    for (TR_ExtraAddressInfo *profiledInfo = sortedValuesIt.getFirst(); profiledInfo != NULL;
        ...
        if (comp()->compileRelocatableCode()) {
            ...
            if (!fej9->canRememberClass(clazz) || !fej9->canRememberClass(callSiteClass)) {
                classValuesAreSane = false;
                break;
            }
        }
    ...
    if (bestMethodInfo) {
        for (profiledMethodInfo = methodValuesIt.getNext(); profiledMethodInfo != NULL;
             profiledMethodInfo = methodValuesIt.getNext()) {
        ...
        static const char *userMinProfiledCallFreq = feGetEnv("TR_MinProfiledCallFrequency");
        static const float minProfiledCallFrequency
            = userMinProfiledCallFreq ? atof(userMinProfiledCallFreq) : MIN_PROFILED_CALL_FREQUENCY;
        ...
        if (methodProbability >= minProfiledCallFrequency) {
            ...
            if (targetMethod && targetClass) {
                TR_VirtualGuardSelection *guard = new (comp()->trHeapMemory())
                    TR_VirtualGuardSelection(TR_ProfiledGuard, TR_MethodTest, targetClass);
...
bool TR_ProfileableCallSite::findProfiledCallTargets(TR_CallStack *callStack, TR_InlinerBase *inliner)
{
    ...
    TR_AddressInfo *valueInfo
        = static_cast<TR_AddressInfo *>(profileManager->getValueInfo(_bcInfo, comp(), AddressInfo));

    if (!valueInfo || comp()->getOption(TR_DisableProfiledInlining)) {
        heuristicTrace(inliner->tracer(),
            " no valueInfo or valueInfo is not of AddressInfo type or TR_DisableProfiledInlining specified for %p\n",
            this);
        return false;
    }
