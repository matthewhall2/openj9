...
private:
    ...
    void processConvertingUnaryIntrinsicFunction(TR::TreeTop *treetop, TR::Node *node, TR::ILOpCodes argConvertOpcode,
        TR::ILOpCodes opcode, TR::ILOpCodes resultConvertOpcode);
#if defined(J9VM_OPT_OPENJDK_METHODHANDLE)
    /** \brief
     *     Helper function that constructs the alternate faster path for bypassing VM INL calls, used by linkToStatic,
     * linkToSpecial, and invokeBasic call tranformers
     *
     *  \param treetop
     ...
     *
     */
    void processVMInternalNativeFunction(TR::TreeTop *treetop, TR::Node *node, TR::Node *vmTargetNode,
        TR::list<TR::SymbolReference *> *argsList, TR::Node *inlCallNode);
#endif
    /** \brief
     *     Transforms java/lang/Class.IsAssignableFrom(Ljava/lang/Class;)Z into a JIT helper call TR_checkAssignable
     ...
     *
     *  \param node
     *     The call node representing a call to java/lang/Class.IsAssignableFrom(Ljava/lang/Class;)Z which has the
     * following shape:
     *
     *     \code
    ...
    /** \brief
     *     Transforms java/lang/StringCoding.encodeASCII(B[B)[B into a compiler intrinsic for codegen acceleration (i.e
     * encodeASCII symbol).
     *
     *  \param treetop
     *     The treetop which anchors the call node.
     ...
     *
     *  \param node
     *     The call node representing a call to
     * jdk/internal/util/ArraysSupport.vectorizedMismatch(Ljava/lang/Object;JLjava/lang/Object;JII)I which has the
     * following shape:
     *
     *     \code
     ...
     *
     *         // the following mask calculation is equivalent to 'mask = (log2ArrayIndexScale<2) ? 3 : 7', assuming
     * log2ArrayIndexScale <= 3
     *         // the original java implementation checks multiple of 8B at a time, but for elements smaller than 4B it
     * also checks another 4B at the end
     *         // this mask serves to round down to nearest multiple of 8B (or 4B if the element is smaller than 4B) and
     * get remainder mask = (log2ArrayIndexScale<<1) | 3
     *
     *         lengthToCompare = lengthInBytes & ~(mask)                          // round down to nearest multiple of 8
     * (or 4) mismatchIndex = arrayCmpLen(a+aOffset, b+bOffset, lengthToCompare) if (mismatchIndex == lengthToCompare)
     * // no mismatch found return ~((lengthInBytes & mask) >> log2ArrayIndexScale)        // inverted remainder,
     ...
     */
    void processUnsafeAtomicCall(TR::TreeTop *treetop, TR::SymbolReferenceTable::CommonNonhelperSymbol helper,
        bool needsNullCheck = false);
#if defined(J9VM_OPT_OPENJDK_METHODHANDLE)
    /** \brief
     *     Transforms java/lang/MethodHandle.invokeBasic calls when receiver MethodHandle (first arg) is not a known
     * object. invokeBasic is a VM INL call that would construct the call frame for the method handle invocation. This
    ...
    /** \brief
     *     Transforms java/lang/MethodHandle.linkToStatic and java/lang/MethodHandle.linkToSpecial calls when the
     * memberName (last arg) is not a known object. linkToStatic and linkToSpecial are VM INL calls that would construct
     * the call frame for the target method invocation. This would be the case even if the method to be invoked is
     * compiled, resulting in j2i and i2j transitions. This transformation creates an alternate conditional path for
     * such invocations to directly call the compiled method if the method is compiled. The transformation is skipped if
     * the linkToStatic call was created as a result of unresolved invokedynamic and invokehandle, as the VM needs to
     * check if the appendix object pushed as the second last argument is NULL, which cannot be determined at compile
     * time.
     *
     *  \param treetop
     *     the TreeTop anchoring the call node
    ...
    /** \brief
     *     Transforms java/lang/MethodHandle.linkToInterface when the MemberName object (last arg) is not a known
     * object.
     *
