...
public:
    ...
    static int32_t _smallMethodBytecodeSizeThresholdForJITServerAOTCache; // for determining which methods to convert to
                                                                          // AOT done remotely
     ...
     */
    static int32_t getExternalOptionIndex(ExternalOptions option) { return _externalOptionsMetadata[option]._argIndex; }

    static int8_t getExternalOptionMatch(ExternalOptions option) { return _externalOptionsMetadata[option]._match; }
    ...
    static void JITServerParseLocalSyncCompiles(J9VMInitArgs *vmArgsArray, J9JavaVM *vm, TR::CompilationInfo *compInfo,
        bool isFSDEnabled, bool postRestore = false);
#endif /* defined(J9VM_OPT_JITSERVER) */
...
private:
#if defined(J9VM_OPT_JITSERVER)
    static const char *JITServerAOTCacheLimitOption(const char *option, void *base, TR::OptionTable *entry,
