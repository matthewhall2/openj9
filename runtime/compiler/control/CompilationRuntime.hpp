...
public:
    ...
    TR_MethodToBeCompiled *findAndDequeueFromLPQ(TR::IlGeneratorMethodDetails &details, uint8_t reason, TR_J9VMBase *fe,
        bool &dequeued);
    ...
    struct Entry {
        ...
        void initialize(J9Method *m, uint32_t c)
        {
            _j9method = (uintptr_t)m;
            _count = c;
            _queuedForCompilation = false;
        }
    };
...
public:
    ...
    class TR_MethodCnt {
        ...
        TR_MethodCnt(J9Method *m, int32_t c)
            : _next(NULL)
            , _method(m)
            , _skippedCount(c)
        {}
...
public:
    DLTTracking(TR::PersistentInfo *persistentInfo)
        : J9Method_HT(persistentInfo)
    {}
...
public:
    ...
    TR_MethodToBeCompiled *addMethodToBeCompiled(TR::IlGeneratorMethodDetails &details, void *pc,
        CompilationPriority priority, bool async, TR_OptimizationPlan *optPlan, bool *queued,
    ...
    TR_MethodToBeCompiled *getNextMethodToBeCompiled(TR::CompilationInfoPerThread *compInfoPT,
        bool compThreadCameOutOfSleep, TR_CompThreadActions *);
    TR_MethodToBeCompiled *peekNextMethodToBeCompiled();

    TR_MethodToBeCompiled *getMethodQueue() { return _methodQueue; }

    int32_t getOverallCompCpuUtilization() const
    {
        return _overallCompCpuUtilization;
    } // -1 in case of error. 0 if feature is not enabled
    ...
    static int32_t getCompThreadSuspensionThreshold(int32_t threadID)
    {
        return _compThreadSuspensionThresholds[threadID];
    }

    // updateNumUsableCompThreads() is called before startCompilationThread() to update
    // TR::Options::_numUsableCompilationThreads. It makes sure the number of usable compilation threads is within
    // allowed bounds. If not, set it to the upper bound based on the mode: JITClient/non-JITServer or JITServer.
    ...
    void *compileOnSeparateThread(J9VMThread *context, TR::IlGeneratorMethodDetails &details, void *oldStartPC,
        TR_YesNoMaybe async, TR_CompilationErrorCode *, bool *queued, TR_OptimizationPlan *optPlan);
    void invalidateRequestsForUnloadedMethods(TR_OpaqueClassBlock *unloadedClass, J9VMThread *vmThread,
    ...
    void setTransientClassLoadersSet(PersistentUnorderedSet<J9ClassLoader *> *transientClassLoaders)
    {
        _transientClassLoaders = transientClassLoaders;
    }
    ...
    int32_t decreaseQueueWeightBy(uint8_t w)
    {
        TR_ASSERT((int32_t)w <= _queueWeight, "assertion failure");
        return _queueWeight -= (int32_t)w;
    }

    int32_t getOverallQueueWeight() const
    {
        return _queueWeight; /*+ (_LPQWeight >> 1);*/
    } // make secondary queue count only half as much
    ...
    void decNumCompThreadsCompilingHotterMethods()
    {
        _numCompThreadsCompilingHotterMethods--;
        TR_ASSERT(_numCompThreadsCompilingHotterMethods >= 0, "assertion failure");
    }
    ...
    void setNumAppThreadsActive(int32_t n, uint64_t t)
    {
        _numAppThreadsActive = n;
        _elapsedTimeNumAppThreadsActiveWasSet = t;
    }
    ...
    TR::CompilationInfoPerThread *getCompilationInfoForDiagnosticThread() const
    {
        return _compInfoForDiagnosticCompilationThread;
    }

    TR::CompilationInfoPerThread * const *getArrayOfCompilationInfoPerThread() const
    {
        return _arrayOfCompilationInfoPerThread;
    }
    ...
    void setSamplerState(TR_SamplerStates s)
    {
        _prevSamplerState = _samplerState;
        _samplerState = s;
    }
    ...
    int32_t getSamplingThreadWaitTimeInDeepIdleToNotifyVM() const
    {
        return _samplingThreadWaitTimeInDeepIdleToNotifyVM;
    }

    void setSamplingThreadWaitTimeInDeepIdleToNotifyVM()
    {
        int32_t minIdleWaitTimeToNotifyVM
            = _jitConfig->javaVM->internalVMFunctions->getVMMinIdleWaitTime(_jitConfig->javaVM);
        ...
        if (minIdleWaitTimeToNotifyVM == 0)
            ...
            if (minIdleWaitTimeToNotifyVM <= TR::Options::_waitTimeToEnterDeepIdleMode)
                ...
                _samplingThreadWaitTimeInDeepIdleToNotifyVM
                    = minIdleWaitTimeToNotifyVM - TR::Options::_waitTimeToEnterDeepIdleMode;
        }
    }

    TR::CompilationInfoPerThread *findFirstLowPriorityCompilationInProgress(
    ...
    */
    ...
    uint64_t computeFreePhysicalLimitAndAbortCompilationIfLow(TR::Compilation *comp, bool &incompleteInfo,
    ...
    int32_t getTotalCompThreadCpuUtilWhenStarvationComputed() const
    {
        return _totalCompThreadCpuUtilWhenStarvationComputed;
    }

    int32_t getNumActiveCompThreadsWhenStarvationComputed() const
    {
        return _numActiveCompThreadsWhenStarvationComputed;
    }
    ...
    void enterLowCompDensityMode()
    {
        _lowCompDensityMode = true;
        _hasEnteredLowCompDensityModeInThePast = true;
    }
    ...
    PersistentVector<TR_OpaqueClassBlock *> *getIllegalFinalFieldModificationList() const
    {
        return _illegalFinalFieldModificationList;
    }

    void setIllegalFinalFieldModificationList(PersistentVector<TR_OpaqueClassBlock *> *it)
    {
        _illegalFinalFieldModificationList = it;
    }

    PersistentUnorderedMap<TR_OpaqueClassBlock *, uint8_t> *getNewlyExtendedClasses() const
    {
    ...
    void setNewlyExtendedClasses(PersistentUnorderedMap<TR_OpaqueClassBlock *, uint8_t> *it)
    {
    ...
    bool useSSL() const
    {
        return (_sslKeys.size() || _sslCerts.size() || _sslRootCerts.size() || _metricsSslKeys.size()
            || _metricsSslCerts.size());
    }

    void setCompThreadActivationPolicy(JITServer::CompThreadActivationPolicy newPolicy)
    {
        _activationPolicy = newPolicy;
    }
    ...
    void setJITServerAOTDeserializer(JITServerAOTDeserializer *deserializer)
    {
        _JITServerAOTDeserializer = deserializer;
    }
    ...
    static void addJ9HookVMDynamicCodeLoadForAOT(J9VMThread *vmThread, J9Method *method, J9JITConfig *jitConfig,
        TR_MethodMetaData *relocatedMetaData);

#if defined(J9VM_INTERP_AOT_COMPILE_SUPPORT) && defined(J9VM_OPT_SHARED_CLASSES)                        \
    && (defined(TR_HOST_X86) || defined(TR_HOST_POWER) || defined(TR_HOST_S390) || defined(TR_HOST_ARM) \
        || defined(TR_HOST_ARM64))
    static void storeAOTInSharedCache(J9VMThread *vmThread, J9ROMMethod *romMethod, const U_8 *dataStart,
...
private:
    ...
    TR::MonitorTable *_j9MonitorTable; // used only for RAS (debuggerExtensions); no accessor; use
                                       // TR_J9MonitorTable::get() everywhere else
    ...
    int32_t _overallCompCpuUtilization; // In percentage points. Valid only if TR::Options::_compThreadCPUEntitlement
    ...
    volatile TR_SamplingThreadLifetimeStates
        _samplingThreadLifetimeState; // access guarded by jitConfig->samplerMonitor (most of the time)
    ...
    int32_t
        _numAllocatedCompThreads; // Number of allocated compilation threads that does not include the diagnostic thread
    ...
    bool _lowCompDensityMode; // set to true when compilations occur infrequently and are unlikely to contribute to JVM
                              // performance
    ...
    PersistentVector<TR_OpaqueClassBlock *>
        *_illegalFinalFieldModificationList; // JITServer list of classes that have
                                             // J9ClassHasIllegalFinalFieldModifications is set
    ...
    uint32_t _lastCriticalCompReqSeqNo; // seqNo for last request that carried information that needs to be processed in
                                        // order
