...
TR_OptimizationPlan *J9::CompilationStrategy::processEvent(TR_MethodEvent *event, bool *newPlanCreated)
{
    ...
    switch (event->_eventType) {
        ...
        case TR_MethodEvent::JitCompilationInducedByDLT:
            ...
            *newPlanCreated = true;
            break;
        case TR_MethodEvent::OtherRecompilationTrigger: // sync recompilation through fixMethodCode or recomp triggered
                                                        // from jitted code (like counting recompilation)
            ...
            if (methodInfo->getReasonForRecompilation() == TR_PersistentMethodInfo::RecompDueToInlinedMethodRedefinition
                || (methodInfo->getReasonForRecompilation() == TR_PersistentMethodInfo::RecompDueToJProfiling
                    && !bodyInfo->getIsProfilingBody())) // if the recompilation is triggered from a JProfiling block
                                                         // but not in a profiled compilation keep the current
                                                         // compilation level unchanged
            {
        ...
        case TR_MethodEvent::CustomMethodHandleThunk:
            ...
            if (hotnessLevel < warm && event->_eventType == TR_MethodEvent::CustomMethodHandleThunk)
                hotnessLevel
                    = warm; // Custom thunks benefit a LOT from warm opts like preexistence and repeated inlining passes
            plan = TR_OptimizationPlan::alloc(hotnessLevel);
...
TR_OptimizationPlan *J9::CompilationStrategy::processInterpreterSample(TR_MethodEvent *event)
{
    ...
    if (!TR::CompilationInfo::isCompiled(j9method)) {
        ...
        if (!cmdLineOptions->getOption(TR_DisableInterpreterSampling)) {
            ...
            if (count > 0) {
                ...
                bool isJSR292Method = _J9ROMMETHOD_J9MODIFIER_IS_SET((J9_ROM_METHOD_FROM_RAM_METHOD(j9method)),
                    J9AccMethodHasMethodHandleInvokes);
                if (jitConfig->javaVM->phase != J9VM_PHASE_NOT_STARTUP) {
                    threshold = isJSR292Method ? TR::Options::_interpreterSamplingThresholdInJSR292
                                               : TR::Options::_interpreterSamplingThresholdInStartupMode;
                    divisor = TR::Options::_interpreterSamplingDivisorInStartupMode;
                } else {
                    threshold = isJSR292Method ? TR::Options::_interpreterSamplingThresholdInJSR292
                                               : TR::Options::_interpreterSamplingThreshold;
                ...
                if (count <= threshold && count > activeThreadsThreshold) {
                    ...
                    if (newCount == 0 && fe->isAsyncCompilation()) {
                        if (TR::Options::_compilationDelayTime <= 0
                            || compInfo->getPersistentInfo()->getElapsedTime()
                                >= 1000 * TR::Options::_compilationDelayTime)
            ...
            } else // count==-1
            {
                if (TR::CompilationInfo::getJ9MethodVMExtra(j9method) == J9_JIT_QUEUED_FOR_COMPILATION) {
                    ...
                    if (compInfo && (compInfo->compBudgetSupport() || compInfo->dynamicThreadPriority())) {
                    ...
                    if (logSampling) {
                        int msgLen = snprintf(curMsg, maxMsgLen, " cannot be compiled, extra field is %" OMR_PRIdPTR,
                            TR::CompilationInfo::getJ9MethodExtra(j9method));
                        if ((0 < msgLen) && ((size_t)msgLen <= maxMsgLen)) {
                            curMsg += msgLen;
                            maxMsgLen -= msgLen;
                        } else {
...
J9::CompilationStrategy::ProcessJittedSample::ProcessJittedSample(J9JITConfig *jitConfig, J9VMThread *vmThread,
    TR::CompilationInfo *compInfo, TR_J9VMBase *fe, TR::Options *cmdLineOptions, J9Method *j9method,
...
void J9::CompilationStrategy::ProcessJittedSample::initializeRecompRelatedFields()
{
    _recompile = false;
    _useProfiling = false;
    ...
    if (_logSampling) {
        ...
        int msgLen = snprintf(_curMsg, maxMsgLen,
            " cnt=%d ncl=%d glblSmplCnt=%d startCnt=%d[-%u,+%u] samples=[%d %d] windows=[%d %u] crtSmplIntrvlCnt=%u",
            _count, _methodInfo->getNextCompileLevel(), _totalSampleCount, _startSampleCount,
            _bodyInfo->getOldStartCountDelta(), _bodyInfo->getHotStartCountDelta(), _globalSamples,
            _globalSamplesInHotWindow, _scorchingSampleInterval, _hotSampleInterval, _crtSampleIntervalCount);
        _curMsg += ((0 < msgLen) && ((size_t)msgLen <= maxMsgLen)) ? (size_t)msgLen : maxMsgLen;
    }
}

void J9::CompilationStrategy::ProcessJittedSample::logSampleInfoToBuffer()
{
...
#define SIG_SZ 150
        ...
        if (_logSampling) {
            size_t maxMsgLen = sizeof(_msg) - (_curMsg - _msg);
            int msgLen = snprintf(_curMsg, maxMsgLen, "(%d)\tCompiled %s\tPC=" POINTER_PRINTF_FORMAT "\t%+d\t",
                _totalSampleCount, sig, _startPC, pcOffset);
            _curMsg += ((0 < msgLen) && ((size_t)msgLen <= maxMsgLen)) ? (size_t)msgLen : maxMsgLen;
        }
...
void J9::CompilationStrategy::ProcessJittedSample::printBufferToVLog()
{
    if (_logSampling) {
...
void J9::CompilationStrategy::ProcessJittedSample::yieldToAppThread()
{
...
void J9::CompilationStrategy::ProcessJittedSample::findAndSetBodyAndMethodInfo()
{
...
bool J9::CompilationStrategy::ProcessJittedSample::shouldProcessSample()
{
    bool shouldProcess = true;
    void *currentStartPC
        = TR::CompilationInfo::getPCIfCompiled(reinterpret_cast<J9Method *>(_methodInfo->getMethodInfo()));
    ...
    if (currentStartPC != _startPC) // rare case
    {
        ...
        || TR::Options::getAOTCmdLineOptions()->getFixedOptLevel()
            != -1) // prevent recompilation when opt level is specified
    {
        ...
        if (_bodyInfo->getSamplingRecomp() && // flag needs to be tested after getting compilationMonitor
            ...
            if (_logSampling) {
                size_t maxMsgLen = sizeof(_msg) - (_curMsg - _msg);
                int msgLen
                    = snprintf(_curMsg, maxMsgLen, " uses sampling but a recomp decision has already been taken");
                _curMsg += ((0 < msgLen) && ((size_t)msgLen <= maxMsgLen)) ? (size_t)msgLen : maxMsgLen;
            }
            shouldProcess = false;
        }
    ...
    if (!_isAlreadyBeingCompiled) {
        ...
        } else // allow transition to HOT through exhaustion of count
        {
            ...
            if (_count < 0 && !_methodInfo->disableMiscSamplingCounterDecrementation()) {
                ...
                _methodInfo->setReasonForRecompilation(_bodyInfo->getIsPushedForRecompilation()
...
void J9::CompilationStrategy::ProcessJittedSample::determineWhetherRecompileIsHotOrScorching(float scalingFactor,
    bool conservativeCase, bool useAggressiveRecompilations, bool isBigAppStartup)
{
...
void J9::CompilationStrategy::ProcessJittedSample::determineWhetherToRecompileBasedOnThreshold()
{
    ...
    int32_t avgCodeSize = (TR::Compiler->target.cpu.isI386() || TR::Compiler->target.cpu.isPower())
        ? 1500
        : 3000; // experimentally determined

    TR_ASSERT(codeSize != 0, "startPC!=0 ==> codeSize!=0");

    float scalingFactor = 0.01
        * ((100 - TR::Options::_sampleThresholdVariationAllowance)
            + (avgCodeSize << 1) * TR::Options::_sampleThresholdVariationAllowance / (float)(avgCodeSize + codeSize));
    size_t maxMsgLen = sizeof(_msg) - (_curMsg - _msg);
    ...
    if (_jitConfig->javaVM->phase != J9VM_PHASE_NOT_STARTUP && TR::Compiler->target.numberOfProcessors() <= 2)
    ...
    bool isBigAppStartup = (_jitConfig->javaVM->phase != J9VM_PHASE_NOT_STARTUP && TR::Options::sharedClassCache()
        && _compInfo->getPersistentInfo()->getNumLoadedClasses() >= TR::Options::_bigAppThreshold
    ...
    if (_globalSamplesInHotWindow <= _scaledHotThreshold || secondCriteriaHot) {
        determineWhetherRecompileIsHotOrScorching(scalingFactor, conservativeCase, useAggressiveRecompilations,
            isBigAppStartup);
    }
    ...
    if (_recompile) {
        ...
        } else // the method is already being compiled; maybe we need to update the opt level
        {
            ...
            if ((int32_t)_nextOptLevel > (int32_t)_methodInfo->getNextCompileLevel()) {
                ...
                TR_MethodToBeCompiled *entry = _compInfo->adjustCompilationEntryAndRequeue(details, _methodInfo,
                    _nextOptLevel, _useProfiling, CP_ASYNC_NORMAL, _fe);
                if (entry) {
                    if (_logSampling) {
                        size_t maxMsgLen = sizeof(_msg) - (_curMsg - _msg);
                        int msgLen = snprintf(_curMsg, maxMsgLen, " adj opt lvl to %d",
                            (int32_t)(entry->_optimizationPlan->getOptLevel()));
                        _curMsg += ((0 < msgLen) && ((size_t)msgLen <= maxMsgLen)) ? (size_t)msgLen : maxMsgLen;
                    }
                    int32_t measuredCpuUtil = _crtSampleIntervalCount == 0
                        ? // scorching interval done?
...
void J9::CompilationStrategy::ProcessJittedSample::determineWhetherToRecompileLessOptimizedMethods()
{
    if (_bodyInfo->getFastRecompilation() && !_isAlreadyBeingCompiled) {
        ...
        if (_bodyInfo->getFastScorchingRecompilation()) {
            if (!TR::Options::getCmdLineOptions()->getOption(TR_DisableProfiling)
                && TR::Recompilation::countingSupported() && !(_methodInfo->profilingDisabled())) {
                _nextOptLevel = veryHot;
                _useProfiling = true;
            } else {
                _nextOptLevel = scorching;
    ...
    } else if (!_postponeDecision && !TR::Options::getCmdLineOptions()->getOption(TR_DisableUpgrades) &&
        // case (1) methods downgraded to cold
        ((_bodyInfo->getHotness() < warm
             && (_methodInfo->isOptLevelDowngraded()
                 || _cmdLineOptions->getOption(TR_EnableUpgradingAllColdCompilations)))
            ||
        ...
        if ((uint32_t)_crtSampleIntervalCount >= threshold
            ...
            } else // AOT bodies
            {
                ...
                } else // -Xquickstart (and AOT)
                {
                    ...
                    if (_cmdLineOptions->getOption(TR_UpgradeBootstrapAtWarm)
                        && _fe->isClassLibraryMethod((TR_OpaqueMethodBlock *)_j9method)) {
                        TR_J9SharedCache *sc
                            = TR_J9VMBase::get(_jitConfig, _event->_vmThread, TR_J9VMBase::AOT_VM)->sharedCache();
...
TR_OptimizationPlan *J9::CompilationStrategy::ProcessJittedSample::triggerRecompIfNeeded()
{
    ...
    if (_recompile) {
        ...
        if (plan) {
            ...
            if (_logSampling) {
                ...
                int msgLen = 0;
                if (_useProfiling)
                    msgLen = snprintf(_curMsg, maxMsgLen, " --> recompile at level %d, profiled CPU=%.1f%%",
                        _nextOptLevel, cpu);
                else
                ...
                if (_methodInfo->getReasonForRecompilation() == TR_PersistentMethodInfo::RecompDueToThreshold) {
                    maxMsgLen = sizeof(_msg) - (_curMsg - _msg);
                    msgLen = snprintf(_curMsg, maxMsgLen, " scaledThresholds=[%d %d]", _scaledScorchingThreshold,
                        _scaledHotThreshold);
                    _curMsg += ((0 < msgLen) && ((size_t)msgLen <= maxMsgLen)) ? (size_t)msgLen : maxMsgLen;
                }
            }
        ...
        if (_isAlreadyBeingCompiled)
            ...
            msgLen = snprintf(_curMsg, maxMsgLen, " not recompiled, CPU=%.1f%% %s scaledThresholds=[%d %d]",
                measuredCpuUtil, _postponeDecision ? " postpone decision" : "", _scaledScorchingThreshold,
                _scaledHotThreshold);
        }
        _curMsg += ((0 < msgLen) && ((size_t)msgLen <= maxMsgLen)) ? (size_t)msgLen : maxMsgLen;
    }
...
TR_OptimizationPlan *J9::CompilationStrategy::ProcessJittedSample::process()
{
    TR_OptimizationPlan *plan = NULL;
...
TR_OptimizationPlan *J9::CompilationStrategy::processJittedSample(TR_MethodEvent *event)
{
    TR::Options *cmdLineOptions = TR::Options::getCmdLineOptions();
    J9Method *j9method = event->_j9method;
...
TR_OptimizationPlan *J9::CompilationStrategy::processHWPSample(TR_MethodEvent *event)
{
    ...
    if (nextOptLevel > hotnessLevel
        || (bodyInfo->getIsAotedBody()
            && !TR::Options::getCmdLineOptions()->getOption(TR_DontRIUpgradeAOTWarmMethods))) {
        J9JITConfig *jitConfig = event->_vmThread->javaVM->jitConfig;
        TR_J9VMBase *fe = TR_J9VMBase::get(jitConfig, event->_vmThread);
        fe->acquireCompilationLock();
        bool isAlreadyBeingCompiled = TR::Recompilation::isAlreadyBeingCompiled(
            (TR_OpaqueMethodBlock *)event->_j9method, event->_oldStartPC, fe);
        fe->releaseCompilationLock();
        if (!isAlreadyBeingCompiled) {
            if (nextOptLevel == scorching && !TR::Options::getCmdLineOptions()->getOption(TR_DisableProfiling)
                && TR::Recompilation::countingSupported() && !bodyInfo->_methodInfo->profilingDisabled()) {
...
void J9::CompilationStrategy::beforeCodeGen(TR_OptimizationPlan *plan, TR::Recompilation *recomp)
{
    ...
    if (!recomp->_doNotCompileAgain) {
        ...
        if ((countValue > 0) || (recomp->isProfilingCompilation() && current < scorching)
            || plan->isOptLevelDowngraded() || recomp->_bodyInfo->getUsesGCR()) {
