...
// purposes.
//
static UDATA findField(J9VMThread *vmStruct, J9ConstantPool *constantPool, UDATA index, BOOLEAN isStatic,
    J9Class **declaringClass)
{
    ...
    *declaringClass = NULL;
    ...
    if (NULL != clazz) {
        ...
        if (!isStatic) {
            UDATA instanceField;
            IDATA offset = javaVM->internalVMFunctions->instanceFieldOffset(vmStruct, clazz, J9UTF8_DATA(name),
                J9UTF8_LENGTH(name), J9UTF8_DATA(signature), J9UTF8_LENGTH(signature), declaringClass, &instanceField,
                J9_LOOK_NO_JAVA);
            ...
            UDATA staticField;
            void *addr = javaVM->internalVMFunctions->staticFieldAddress(vmStruct, clazz, J9UTF8_DATA(name),
                J9UTF8_LENGTH(name), J9UTF8_DATA(signature), J9UTF8_LENGTH(signature), declaringClass, &staticField,
...
static UDATA jitFindField(TR::Compilation *comp, J9VMThread *vmStruct, J9ConstantPool *constantPool, UDATA index,
    BOOLEAN isStatic, J9Class **declaringClass)
{
...
static void handler_IProfiler_profilingSample(JITServer::ClientStream *client, TR_J9VM *fe, TR::Compilation *comp)
{
    ...
    if (entry && !entry->isInvalid()) {
        ...
        if (canPersist == IPBC_ENTRY_CAN_PERSIST) {
            ...
            client->write(JITServer::MessageType::IProfiler_profilingSample, entryBytes, totalSamples, (size_t)1,
                /*wholeMethod=*/false, usePersistentCache, isCompiled, uncachedClasses, classInfoTuples);
        } else {
...
static bool handleResponse(JITServer::MessageType response, JITServer::ClientStream *client,
    TR::CompilationInfoPerThread *compInfoPT, TR::Compilation *comp, TR_J9VM *fe, J9VMThread *vmThread)
{
    ...
    switch (response) {
        ...
        case MessageType::getUnloadedClassRangesAndCHTable: {
            ...
            if ((previousUID != serverUID) && TR::Options::getVerboseOption(TR_VerboseJITServerConns)) {
                TR_VerboseLog::writeLineLocked(TR_Vlog_JITServer, "t=%6u Connected to a server (serverUID=%llu)",
                    (uint32_t)compInfo->getPersistentInfo()->getElapsedTime(), (unsigned long long)serverUID);
            }
        } break;
        case MessageType::ClientSessionData_getPermanentLoaders: {
            ...
            newPermanentLoaders.insert(newPermanentLoaders.end(), permanentLoaders.begin() + serverCount,
        ...
        case MessageType::VM_getClassFromSignature: {
            ...
            J9ClassLoader *cl
                = clazz ? reinterpret_cast<J9ClassLoader *>(fej9->getClassLoader((TR_OpaqueClassBlock *)clazz)) : NULL;
        ...
        case MessageType::VM_getResolvedMethodsAndMirror: {
            ...
            for (int i = 0; i < numMethods; ++i) {
                ...
                TR_ResolvedJ9JITServerMethod::createResolvedMethodMirror(methodInfo,
                    (TR_OpaqueMethodBlock *)&(methods[i]), 0, 0, fe, trMemory);
        ...
        case MessageType::VM_getVMInfo: {
            ...
            vmInfo._elgibleForPersistIprofileInfo
                = vmInfo._isIProfilerEnabled ? fe->getIProfiler()->elgibleForPersistIprofileInfo(comp) : false;
            ...
            vmInfo._noTypeInvokeExactThunkHelper
                = comp->getSymRefTab()->findOrCreateRuntimeHelper(TR_icallVMprJavaSendInvokeExact0)->getMethodAddress();
            vmInfo._int64InvokeExactThunkHelper
                = comp->getSymRefTab()->findOrCreateRuntimeHelper(TR_icallVMprJavaSendInvokeExactJ)->getMethodAddress();
            vmInfo._int32InvokeExactThunkHelper
                = comp->getSymRefTab()->findOrCreateRuntimeHelper(TR_icallVMprJavaSendInvokeExact1)->getMethodAddress();
            vmInfo._addressInvokeExactThunkHelper
                = comp->getSymRefTab()->findOrCreateRuntimeHelper(TR_icallVMprJavaSendInvokeExactL)->getMethodAddress();
            vmInfo._floatInvokeExactThunkHelper
                = comp->getSymRefTab()->findOrCreateRuntimeHelper(TR_icallVMprJavaSendInvokeExactF)->getMethodAddress();
            vmInfo._doubleInvokeExactThunkHelper
                = comp->getSymRefTab()->findOrCreateRuntimeHelper(TR_icallVMprJavaSendInvokeExactD)->getMethodAddress();
...

            {
                ...
                if (javaVM->srMethodAccessor != NULL)
                    vmInfo._srMethodAccessorClass
                        = (TR_OpaqueClassBlock *)J9VM_J9CLASS_FROM_JCLASS(vmThread, javaVM->srMethodAccessor);
                else
                    vmInfo._srMethodAccessorClass = NULL;
                if (javaVM->srConstructorAccessor != NULL)
                    vmInfo._srConstructorAccessorClass
                        = (TR_OpaqueClassBlock *)J9VM_J9CLASS_FROM_JCLASS(vmThread, javaVM->srConstructorAccessor);
                else
                ...
                if (sharedCacheConfig)
                    vmInfo._shareLambdaForm
                        = J9_ARE_ALL_BITS_SET(sharedCacheConfig->runtimeFlags2, J9SHR_RUNTIMEFLAG2_SHARE_LAMBDAFORM);
                else
            ...
            if (fe->sharedCache() && fe->sharedCache()->getCacheDescriptorList()) {
                ...
                do {
                    ClientSessionData::CacheDescriptor cacheDesc
                        = { (uintptr_t)curCache->cacheStartAddress, curCache->cacheSizeBytes,
                              (uintptr_t)curCache->romclassStartAddress, (uintptr_t)curCache->metadataStartAddress };
            ...
            vmInfo._needsMethodTrampolines
            ...
            int32_t enableProfileCacheArgIndex
                = J9::Options::getExternalOptionIndex(J9::ExternalOptions::XXplusJITServerUseProfileCache);
            int32_t disableProfileCacheArgIndex
                = J9::Options::getExternalOptionIndex(J9::ExternalOptions::XXminusJITServerUseProfileCache);
            // The profile cache feature needs to be enabled explicitly by the client. The presence of the AOT cache is
            // a pre-requisite.
            ...
            client->write(response, vmInfo, listOfCacheDescriptors,
                comp->getPersistentInfo()->getJITServerAOTCacheName());
        } break;
        ...
        case MessageType::VM_setJ2IThunk: {
            ...
            if (!comp->compileRelocatableCode() || comp->ignoringLocalSCC()) {
                ...
                uint8_t *thunkStart = TR_JITServerRelocationRuntime::copyDataToCodeCache(serializedThunk.data(),
                    serializedThunk.size(), fe, comp->codeCacheKind());
                if (!thunkStart)
                    compInfoPT->getCompilation()->failCompilation<TR::CodeCacheError>(
                        "Failed to allocate space in the code cache");

                thunkAddress = thunkStart + 8;
                void *vmHelper
                    = j9ThunkVMHelperFromSignature(fe->_jitConfig, signature.size(), (char *)signature.data());
                compInfoPT->reloRuntime()->reloTarget()->performThunkRelocation(
                    reinterpret_cast<uint8_t *>(thunkAddress), (uintptr_t)vmHelper);
            } else {
        ...
        case MessageType::VM_getInstanceFieldOffset: {
            ...
            client->write(response,
                fe->getInstanceFieldOffset(clazz, field.data(), field.length(), sig.data(), sig.length(), options));
        } break;
        ...
        case MessageType::VM_getMethodFromName: {
            ...
            client->write(response, fe->getMethodFromName(className.data(), methodName.data(), signature.data()));
        } break;
        case MessageType::VM_getMethodFromClass: {
            ...
            client->write(response,
                fe->getMethodFromClass(methodClass, methodName.data(), signature.data(), callingClass));
        } break;
        case MessageType::VM_createMethodHandleArchetypeSpecimen: {
            ...
            char *thunkableSignature;
            {
                ...
                uintptr_t signatureString = fe->getReferenceField(
                    fe->getReferenceField(*methodHandleLocation, "thunks", "Ljava/lang/invoke/ThunkTuple;"),
                ...
                TR_ResolvedMethod *resolvedMethod = fe->createResolvedMethodWithSignature(trMemory, archetype, NULL,
                    thunkableSignature, length, owningMethod);
                resolvedMethod->convertToMethod()->setArchetypeSpecimen();
                resolvedMethod->setMethodHandleLocation(methodHandleLocation);

                TR_ResolvedJ9JITServerMethodInfo methodInfo;
                TR_ResolvedJ9JITServerMethod::packMethodInfo(methodInfo,
                    static_cast<TR_ResolvedJ9Method *>(resolvedMethod), fe);
        ...
        case MessageType::VM_getStaticFieldAddress: {
            ...
            client->write(response,
                fe->getStaticFieldAddress(clazz,
        ...
        case MessageType::VM_methodOfDirectOrVirtualHandle: {
            ...
            TR_J9VMBase::MethodOfHandle moh = fe->methodOfDirectOrVirtualHandle(mh, isVirtual);
        ...
        case MessageType::VM_getKnotIndexOfInvokeCacheArrayAppendixElement: {
            ...
            TR::KnownObjectTable::Index idx
                = fe->getKnotIndexOfInvokeCacheArrayAppendixElement(comp, std::get<0>(recv));
        ...
        case MessageType::VM_targetMethodFromInvokeCacheArrayMemberNameObj: {
            ...
            auto *targetMethod = static_cast<TR_ResolvedJ9Method *>(
                fe->targetMethodFromInvokeCacheArrayMemberNameObj(comp, std::get<0>(recv), std::get<1>(recv)));
            TR_ResolvedJ9JITServerMethodInfo methodInfo;
        ...
        case MessageType::VM_delegatingMethodHandleTarget: {
            ...
            TR::KnownObjectTable::Index idx
                = fe->delegatingMethodHandleTargetHelper(comp, std::get<0>(recv), std::get<1>(recv));
            client->write(response, idx, knot->getPointerLocation(idx));
        } break;
        ...
        case MessageType::VM_getMemberNameFieldKnotIndexFromMethodHandleKnotIndex: {
            ...
            TR::KnownObjectTable::Index fieldKnotIndex
                = fe->getMemberNameFieldKnotIndexFromMethodHandleKnotIndex(comp, std::get<0>(recv), &memberNameStr[0]);
        ...
        case MessageType::VM_getConvertedMethodhandle: {
            ...
            TR::KnownObjectTable::Index convertedMHIndex = fe->getConvertedMethodhandle(comp, mhIndex, desiredTypeIndex);
        ...
        case MessageType::VM_getMethodHandleTableEntryIndex: {
            ...
            TR::KnownObjectTable::Index mhIndex
                = fe->getMethodHandleTableEntryIndex(comp, std::get<0>(recv), std::get<1>(recv));
        ...
        case MessageType::mirrorResolvedJ9Method: {
            // allocate a new TR_ResolvedJ9Method on the heap, to be used as a mirror for performing actions which are
            // only easily done on the client side.
        ...
        case MessageType::ResolvedMethod_staticAttributes: {
            ...
            bool unresolvedInCP;
            bool result = method->staticAttributes(comp, cpIndex, &address, &type, &volatileP, &isFinal, &isPrivate,
                isStore, &unresolvedInCP, needAOTValidation);
            TR_J9MethodFieldAttributes attrs(reinterpret_cast<uintptr_t>(address), type.getDataType(), volatileP,
                isFinal, isPrivate, unresolvedInCP, result);
        ...
        case MessageType::ResolvedMethod_fieldAttributes: {
            ...
            bool isPrivate = false;
            bool unresolvedInCP;
            bool result = method->fieldAttributes(comp, cpIndex, &fieldOffset, &type, &volatileP, &isFinal, &isPrivate,
                isStore, &unresolvedInCP, needAOTValidation);
            TR_J9MethodFieldAttributes attrs(static_cast<uintptr_t>(fieldOffset), type.getDataType(), volatileP,
                isFinal, isPrivate, unresolvedInCP, result);
            client->write(response, attrs);
        } break;
        case MessageType::ResolvedMethod_getResolvedStaticMethodAndMirror: {
...

            {
                ...
                ramMethod = jitResolveStaticMethodRef(fe->vmThread(), method->cp(), cpIndex,
            ...
            if (ramMethod)
                TR_ResolvedJ9JITServerMethod::createResolvedMethodFromJ9MethodMirror(methodInfo,
                    (TR_OpaqueMethodBlock *)ramMethod, 0, method, fe, trMemory);
        ...
        case MessageType::ResolvedMethod_getResolvedSpecialMethodAndMirror: {
            ...
            if (!((fe->_jitConfig->runtimeFlags & J9JIT_RUNTIME_RESOLVE)
                ...
                ramMethod = jitResolveSpecialMethodRef(fe->vmThread(), method->cp(), cpIndex,
                    J9_RESOLVE_FLAG_JIT_COMPILE_TIME);
        ...
        case MessageType::ResolvedMethod_getResolvedVirtualMethod: {
            ...
            TR_ResolvedJ9JITServerMethodInfo methodInfo;
            if (ramMethod)
                TR_ResolvedJ9JITServerMethod::createResolvedMethodMirror(methodInfo, ramMethod, 0, owningMethod, fe,
                    trMemory);
            client->write(response, ramMethod, methodInfo);
        } break;
        ...
        case MessageType::ResolvedMethod_getResolvedInterfaceMethodAndMirror_3: {
            auto recv
                = client->getRecvData<TR_OpaqueMethodBlock *, TR_OpaqueClassBlock *, I_32, TR_ResolvedJ9Method *>();
            auto method = std::get<0>(recv);
            auto clazz = std::get<1>(recv);
            ...
            if (resolved)
                TR_ResolvedJ9JITServerMethod::createResolvedMethodFromJ9MethodMirror(methodInfo,
                    (TR_OpaqueMethodBlock *)ramMethod, 0, owningMethod, fe, trMemory);
        ...
        case MessageType::ResolvedMethod_getResolvedImproperInterfaceMethodAndMirror: {
            ...
            if (j9method)
                TR_ResolvedJ9JITServerMethod::createResolvedMethodFromJ9MethodMirror(methodInfo,
                    (TR_OpaqueMethodBlock *)j9method, (uint32_t)vtableOffset, mirror, fe, trMemory);
        ...
        case MessageType::ResolvedMethod_getResolvedHandleMethod: {
            ...
            auto *handleMethod = static_cast<TR_ResolvedJ9Method *>(
                owningMethod->getResolvedHandleMethod(comp, cpIndex, &isUnresolvedInCP, &isInvokeCacheAppendixNull));
            TR_ResolvedJ9JITServerMethodInfo methodInfo;
            ...
            client->write(response, handleMethod->getPersistentIdentifier(), methodInfo, signature, isUnresolvedInCP,
        ...
        case MessageType::ResolvedMethod_getResolvedDynamicMethod: {
            ...
            bool isUnresolvedInCP = false;
            bool isInvokeCacheAppendixNull = false;
            auto *dynamicMethod = static_cast<TR_ResolvedJ9Method *>(owningMethod->getResolvedDynamicMethod(comp,
                callSiteIndex, &isUnresolvedInCP, &isInvokeCacheAppendixNull));
            TR_ResolvedJ9JITServerMethodInfo methodInfo;
            TR_ResolvedJ9JITServerMethod::packMethodInfo(methodInfo, (TR_ResolvedJ9Method *)dynamicMethod, fe);

            client->write(response, dynamicMethod->getPersistentIdentifier(), methodInfo,
                std::string(utf8Data(dynamicMethod->_signature), J9UTF8_LENGTH(dynamicMethod->_signature)),
                isUnresolvedInCP, isInvokeCacheAppendixNull);
        } break;
        ...
        case MessageType::ResolvedMethod_getMultipleResolvedMethods: {
            auto recv
                = client
                      ->getRecvData<TR_ResolvedJ9Method *, std::vector<TR_ResolvedMethodType>, std::vector<int32_t> >();
            auto owningMethod = std::get<0>(recv);
            ...
            for (int32_t i = 0; i < numMethods; ++i) {
                ...
                switch (type) {
                    case TR_ResolvedMethodType::VirtualFromCP: {
                        resolvedMethod = static_cast<TR_ResolvedJ9Method *>(
                            owningMethod->getResolvedPossiblyPrivateVirtualMethod(comp, cpIndex, true,
                                &unresolvedInCP));
                    ...
                    case TR_ResolvedMethodType::Static: {
                        resolvedMethod = static_cast<TR_ResolvedJ9Method *>(
                            owningMethod->getResolvedStaticMethod(comp, cpIndex, &unresolvedInCP));
                        break;
                    }
                    case TR_ResolvedMethodType::Special: {
                        resolvedMethod = static_cast<TR_ResolvedJ9Method *>(
                            owningMethod->getResolvedSpecialMethod(comp, cpIndex, &unresolvedInCP));
                        break;
                    }
        ...
        case MessageType::ResolvedMethod_getResolvedImplementorMethods: {
            auto recv
                = client->getRecvData<TR_OpaqueClassBlock *, int32_t, int32_t, TR_ResolvedJ9Method *, TR_YesNoMaybe>();
            TR_OpaqueClassBlock *clazz = std::get<0>(recv);
            ...
            int32_t implCount = TR_ClassQueries::collectImplementorsCapped(classInfo, implArray.data(), maxCount,
                slotOrIndex, callerMethod, comp, false, useGetResolvedInterfaceMethod);
        ...
        case MessageType::ResolvedMethod_getTargetMethodFromMemberName: {
            ...
            TR_OpaqueMethodBlock *targetMethod
                = owningMethod->getTargetMethodFromMemberName(invokeCacheArray, &isInvokeCacheAppendixNull);
        ...
        case MessageType::ResolvedRelocatableMethod_createResolvedRelocatableJ9Method: {
            ...
            TR_ResolvedJ9JITServerMethodInfo methodInfo;
            TR_ResolvedJ9JITServerMethod::createResolvedMethodFromJ9MethodMirror(methodInfo,
                (TR_OpaqueMethodBlock *)j9method, vTableSlot, mirror, fe, trMemory);
            ...
            TR_OpaqueClassBlock *clazzOfCompiledMethod
                = fe->convertClassPtrToClassOffset(J9_CLASS_FROM_METHOD(mirror->ramMethod()));
            sameLoaders = fe->sameClassLoaders(clazzOfInlinedMethod, clazzOfCompiledMethod);
            if (resolvedMethod)
                sameClass = fe->convertClassPtrToClassOffset(J9_CLASS_FROM_METHOD(mirror->ramMethod()))
                    == fe->convertClassPtrToClassOffset(J9_CLASS_FROM_METHOD(j9method));
        ...
        case MessageType::ResolvedRelocatableMethod_fieldAttributes: {
            ...
            TR_J9MethodFieldAttributes attrs(static_cast<uintptr_t>(fieldOffset), type.getDataType(), volatileP,
                isFinal, isPrivate, unresolvedInCP, result, definingClass);
        ...
        case MessageType::SharedCache_getClassChainOffsetIdentifyingLoader: {
            ...
            uintptr_t offset = sharedCache ? sharedCache->getClassChainOffsetIdentifyingLoader(j9class, &chain)
            ...
            if (getName) {
                if (chain) {
                    ...
                    auto name = fe->getPersistentInfo()
                                    ->getPersistentClassLoaderTable()
                                    ->lookupClassNameAssociatedWithClassLoader(fe->getClassLoader(j9class));
                    if (name)
                        nameStr = std::string((const char *)J9UTF8_DATA(name), J9UTF8_LENGTH(name));
                }
            }
        ...
        case MessageType::SharedCache_rememberClass: {
            ...
            uintptr_t classChainOffset = needClientOffset ? fe->sharedCache()->rememberClass(clazz, NULL, create)
                                                          : TR_SharedCache::INVALID_CLASS_CHAIN_OFFSET;
            ...
            if (create && getClasses) {
                // The first word of the class chain data stores the size of the whole record in bytes, so the number of
                // classes is 1 less than the necessary class chain length.
        ...
        case MessageType::runFEMacro_invokeILGenMacrosInvokeExactAndFixup: {
            ...
            uintptr_t methodHandle = listOfOffsets.size() == 0
                ? receiverHandle
                : JITServerHelpers::walkReferenceChainWithOffsets(fe, listOfOffsets, receiverHandle);
            uintptr_t methodDescriptorRef
                = fe->getReferenceField(fe->getReferenceField(methodHandle, "type", "Ljava/lang/invoke/MethodType;"),
        ...
        case MessageType::runFEMacro_invokeGuardWithTestHandleNumGuardArgs: {
            ...
            uintptr_t methodHandle = *std::get<0>(recv);
            uintptr_t guardArgs = fe->getReferenceField(
                fe->methodHandle_type(fe->getReferenceField(methodHandle, "guard", "Ljava/lang/invoke/MethodHandle;")),
                "ptypes", "[Ljava/lang/Class;");
        ...
        case MessageType::runFEMacro_invokeExplicitCastHandleConvertArgs: {
            ...
            uintptr_t methodHandle = *std::get<0>(recv);
            uintptr_t methodDescriptorRef = fe->getReferenceField(
                fe->getReferenceField(fe->getReferenceField(methodHandle, "next", "Ljava/lang/invoke/MethodHandle;"),
        ...
        case MessageType::runFEMacro_targetTypeL: {
            ...
            uintptr_t targetArguments = fe->getReferenceField(
                fe->getReferenceField(fe->getReferenceField(methodHandle, "next", "Ljava/lang/invoke/MethodHandle;"),
                    "type", "Ljava/lang/invoke/MethodType;"),
                "ptypes", "[Ljava/lang/Class;");
            TR_OpaqueClassBlock *targetParmClass
                = (TR_OpaqueClassBlock *)(intptr_t)fe->getInt64Field(fe->getReferenceElement(targetArguments, argIndex),
            ...
            TR_OpaqueClassBlock *sourceParmClass
                = (TR_OpaqueClassBlock *)(intptr_t)fe->getInt64Field(fe->getReferenceElement(sourceArguments, argIndex),
                    "vmRef" /* should use fej9->getOffsetOfClassFromJavaLangClassField() */);
        ...
        case MessageType::runFEMacro_invokeSpreadHandleArrayArg: {
            ...
            uint32_t spreadPositionOffset
                = fe->getInstanceFieldOffset(fe->getObjectClass(methodHandle), "spreadPosition", "I");
            ...
            client->write(response, arrayJ9Class, spreadPosition, arity, leafClass,
                std::string(leafClassNameChars, leafClassNameLength), isPrimitive);
        } break;
        case MessageType::runFEMacro_invokeSpreadHandle: {
            ...
            uintptr_t arguments
                = fe->getReferenceField(fe->methodHandle_type(methodHandle), "ptypes", "[Ljava/lang/Class;");
            int32_t numArguments = (int32_t)fe->getArrayLengthInElements(arguments);
            uintptr_t next = fe->getReferenceField(methodHandle, "next", "Ljava/lang/invoke/MethodHandle;");
            uintptr_t nextArguments
                = fe->getReferenceField(fe->methodHandle_type(next), "ptypes", "[Ljava/lang/Class;");
        ...
        case MessageType::runFEMacro_invokeFoldHandle: {
            ...
            if (arrayLength != 0) {
                ...
                uintptr_t combinerArguments
                    = fe->getReferenceField(fe->methodHandle_type(combiner), "ptypes", "[Ljava/lang/Class;");
        ...
        case MessageType::runFEMacro_invokeFinallyHandle: {
            ...
            uintptr_t finallyTarget
                = fe->getReferenceField(methodHandle, "finallyTarget", "Ljava/lang/invoke/MethodHandle;");
            ...
            uintptr_t methodDescriptorRef
                = fe->getReferenceField(finallyType, "methodDescriptor", "Ljava/lang/String;");
            intptr_t methodDescriptorLength = fe->getStringUTF8Length(methodDescriptorRef);
            char *methodDescriptor = (char *)alloca(methodDescriptorLength + 1);
            fe->getStringUTF8(methodDescriptorRef, methodDescriptor, methodDescriptorLength + 1);
        ...
        case MessageType::runFEMacro_invokeFilterArgumentsHandle: {
...

            {
                ...
                uintptr_t methodDescriptorRef
                    = fe->getReferenceField(fe->getReferenceField(fe->getReferenceField(methodHandle, "next",
            ...
            client->write(response, startPos, nextSignatureString, haveFilterList, filterIndexList,
                filterObjectReferenceLocationList);
        } break;
        ...
        case MessageType::runFEMacro_invokeCatchHandle: {
            ...
            uintptr_t catchTarget
                = fe->getReferenceField(methodHandle, "catchTarget", "Ljava/lang/invoke/MethodHandle;");
            uintptr_t catchArguments
                = fe->getReferenceField(fe->getReferenceField(catchTarget, "type", "Ljava/lang/invoke/MethodType;"),
                    "ptypes", "[Ljava/lang/Class;");
...

            {
                ...
                uintptr_t arguments
                    = fe->getReferenceField(fe->getReferenceField(fe->getReferenceField(methodHandle, "next",
                                                ...
                                                "type", "Ljava/lang/invoke/MethodType;"),
                        "ptypes", "[Ljava/lang/Class;");
                componentClazz = fe->getComponentClassFromArrayClass(
                    fe->getClassFromJavaLangClass(fe->getReferenceElement(arguments, collectPosition)));
            }
        ...
        case MessageType::KnownObjectTable_dereferenceKnownObjectField: {
            ...
            TR::Symbol *fieldSymbol = TR::Symbol::createPossiblyRecognizedShadowFromCP(comp, trMemory->trStackMemory(),
                calleeMethod, cpIndex, &type, &fieldOffset, false);
        ...
        case MessageType::KnownObjectTable_dereferenceKnownObjectField2: {
...

            {
                ...
                int32_t targetFieldOffset
                    = fe->getInstanceFieldOffset(mutableCallsiteClass, "target", "Ljava/lang/invoke/MethodHandle;");
                ...
                TR_ASSERT_FATAL(fe->isInstanceOf(receiverClass, mutableCallsiteClass, true) == TR_yes,
                    "receiver of mutableCallsite_getTarget must be instance of MutableCallSite (*%p)",
                    knot->getPointerLocation(receiverIndex));
        ...
        case MessageType::KnownObjectTable_getVectorBitSize: {
            ...
            int32_t vectorBitSize = 0;
            {
                ...
                uintptr_t vectorShapeLocation
                    = fe->getReferenceField(vectorSpeciesLocation, "vectorShape", "Ljdk/incubator/vector/VectorShape;");
        ...
        case MessageType::KnownObjectTable_addFieldAddressFromBaseIndex: {
            ...
            uintptr_t *resultPointer
                = (resultIndex == TR::KnownObjectTable::UNKNOWN) ? NULL : knot->getPointerLocation(resultIndex);
        ...
        case MessageType::AOTCache_getROMClassBatch: {
            ...
            for (J9Class *ramClass : ramClasses) {
                classInfos.push_back(
                    JITServerHelpers::packRemoteROMClassInfo(ramClass, vmThread, trMemory, true /*serializeClass*/));

                // If this is an array class, remember its base component class for later
                int32_t numDimensions = 0;
                J9Class *baseComponent = (J9Class *)TR_J9VMBase::staticGetBaseComponentClass(
                    (TR_OpaqueClassBlock *)ramClass, numDimensions);
                if (numDimensions)
            ...
            if (!baseComponentClasses.empty()) {
                ...
                for (J9Class *baseComponent : baseComponentClasses) {
                    if (classesCachedAtServer.find(baseComponent)
                        == classesCachedAtServer.end()) // server doesn't have it
                    {
            ...
            for (J9Class *baseComponent : uncachedBaseComponentClasses) {
                classInfos.push_back(JITServerHelpers::packRemoteROMClassInfo(baseComponent, vmThread, trMemory,
                    true /*serializeClass*/));
            ...
            // Finally, update client's view of classes cached by the server.
            {
                ...
                compInfo->getclassesCachedAtServer().insert(uncachedBaseComponentClasses.begin(),
                    uncachedBaseComponentClasses.end());
        ...
        case MessageType::CodeGenerator_assignKeepaliveConstRefLabels: {
...
#else
            auto recv = client->getRecvData<std::vector<TR::KnownObjectTable::Index>,
                std::vector<TR_OpaqueClassBlock *>, std::vector<J9::ResolvedInlinedCallSite>,
                std::vector<J9::RepeatRetainedMethodsAnalysis::InlinedSiteInfo>, std::vector<TR_ResolvedMethod *>,
            ...
            // memory region.
            auto trInliningTable
                = new (comp->region()) TR::vector<J9::ResolvedInlinedCallSite, TR::Region &>(comp->region());
            ...
            J9::RepeatRetainedMethodsAnalysis::analyzeOnClient(comp, *trInliningTable, inlinedSiteInfo,
            ...
            TR_ResolvedJ9JITServerMethod::getClientBondMethodsToSendToServer(comp, clientBondMethods);

            client->write(response, newKnownObjects, oldKnobKeepalives, provenanceEdges, clientBondMethods);
#endif
        } break;
        case MessageType::CodeGenerator_getConstProvenanceEdges: {
            ...
            TR_ASSERT_FATAL(false,
                ...
                "OpenJDK MethodHandles only");
#else
            auto recv = client->getRecvData<std::vector<J9::ResolvedInlinedCallSite>,
                std::vector<J9::RepeatRetainedMethodsAnalysis::InlinedSiteInfo>, std::vector<TR_ResolvedMethod *>,
                std::vector<TR_ResolvedMethod *> >();
...
static bool handleServerMessage(JITServer::ClientStream *client, TR_J9VM *fe, JITServer::MessageType &response)
{
    ...
    TR_ASSERT(TR::MonitorTable::get()->getClassUnloadMonitorHoldCount(compInfoPT->getCompThreadId()) == 0,
        "Must not hold classUnloadMonitor");
    TR::MonitorTable *table = TR::MonitorTable::get();
    TR_ASSERT(table && table->isThreadInSafeMonitorState(vmThread),
        "Must not hold any monitors when waiting for server");
    ...
    if (interruptReason && response != MessageType::jitDumpPrintIL) {
        ...
        if ((response != MessageType::compilationCode) && (response != MessageType::compilationFailure)
            && (response != MessageType::AOTCache_serializedAOTMethod)
            && (response != MessageType::AOTCache_storedAOTMethod) && (response != MessageType::AOTCache_failure))
        ...
        if (TR::Options::isAnyVerboseOptionSet(TR_VerboseJITServer, TR_VerboseCompilationDispatch))
            TR_VerboseLog::writeLineLocked(TR_Vlog_FAILURE,
                "Interrupting remote compilation (interruptReason %u) in handleServerMessage(%s) for %s @ %s",
                interruptReason, JITServer::messageNames[response], comp->signature(), comp->getHotnessName());

        Trc_JITServerInterruptRemoteCompile(vmThread, interruptReason, JITServer::messageNames[response],
    ...
    try {
        ...
        if (TR::Options::isAnyVerboseOptionSet(TR_VerboseJITServer, TR_VerboseCompilationDispatch))
            TR_VerboseLog::writeLineLocked(TR_Vlog_FAILURE,
                "Interrupting remote compilation (%s) in handleServerMessage(%s) for %s @ %s", e.what(),
                JITServer::messageNames[response], comp->signature(), comp->getHotnessName());
...
static TR_MethodMetaData *remoteCompilationEnd(J9VMThread *vmThread, TR::Compilation *comp, TR_ResolvedMethod *compilee,
    J9Method *method, TR::CompilationInfoPerThreadBase *compInfoPT, const std::string &codeCacheStr,
    const std::string &dataCacheStr)
{
    ...
    if (!fe->isAOT_DEPRECATED_DO_NOT_USE()
        && !comp->isDeserializedAOTMethod()) // For relocating received JIT compilations
    {
        compInfoPT->reloRuntime()->setReloStartTime(compInfoPT->getTimeWhenCompStarted());

        metaData = compInfoPT->reloRuntime()->prepareRelocateAOTCodeAndData(vmThread, fe, comp->cg()->getCodeCache(),
            (J9JITDataCacheHeader *)dataCacheStr.data(), method, false, comp->getOptions(), comp, compilee,
            (uint8_t *)codeCacheStr.data());

        if (!metaData) {
            if (TR::Options::getVerboseOption(TR_VerboseJITServer)) {
                TR_VerboseLog::writeLineLocked(TR_Vlog_JITServer, "JITClient: Relocation failure: %d",
    ...
    && (defined(TR_HOST_X86) || defined(TR_HOST_POWER) || defined(TR_HOST_S390) || defined(TR_HOST_ARM64))
    else // For relocating received AOT compilations
    {
        ...
        if (canRelocateMethod) {
            ...
            if (TR::Options::getVerboseOption(TR_VerboseJITServer))
                TR_VerboseLog::writeLineLocked(TR_Vlog_JITServer,
                    "JITClient: Applying remote AOT relocations to %s AOT body for %s @ %s", compilationType,
                    comp->signature(), comp->getHotnessName());
            ...
            try {
                ...
                if (comp->isDeserializedAOTMethod()
                    && compInfo->getPersistentInfo()->getJITServerAOTCacheIgnoreLocalSCC()) {
                    auto deserializerSCC = fe->deserializerSharedCache();
                    // This should be extremely rare
                    if (!deserializerSCC)
                        comp->failCompilation<TR::CompilationException>(
                            "Deserializer was not allocated in compilation thread");
                    cacheOverride = deserializerSCC;
                }
                metaData = entry->_compInfoPT->reloRuntime()->prepareRelocateAOTCodeAndData(vmThread, fe,
                    comp->cg()->getCodeCache(), (J9JITDataCacheHeader *)dataCacheStr.data(), method, false,
                    comp->getOptions(), comp, compilee, (uint8_t *)codeCacheStr.data(), cacheOverride);
            ...
            if (metaData) {
                if (TR::Options::getVerboseOption(TR_VerboseJITServer)) {
                    TR_VerboseLog::writeLineLocked(TR_Vlog_JITServer,
                        "JITClient successfully relocated metadata for %s", comp->signature());
                }
                ...
                if (TR::Options::getVerboseOption(TR_VerboseJITServer)) {
                    ...
                    TR_VerboseLog::writeLineLocked(TR_Vlog_JITServer, "JITClient: AOT Relocation failure: %d %d (%s)",
                        compInfoPT->reloRuntime()->returnCode(), errorCode,
                        compInfoPT->reloRuntime()->getReloErrorCodeName(errorCode));
                }
        ...
        // If we're not using the AOT cache, we still store by default. This avoids certain test failures in the short
        // term. Also, if we've explicitly been requested by the user to delay method relocations then we need to store
        // methods in the SCC to support that option.
        auto persistentInfo = compInfo->getPersistentInfo();
        if ((persistentInfo->getJITServerAOTCacheDelayMethodRelocation()
            ...
            TR::CompilationInfo::storeAOTInSharedCache(vmThread, romMethod, (uint8_t *)dataCacheStr.data(),
                dataCacheStr.size(), (uint8_t *)codeCacheStr.data(), codeCacheStr.size(), comp, jitConfig, entry);
        }
    }
#endif // defined(J9VM_INTERP_AOT_COMPILE_SUPPORT) && defined(J9VM_OPT_SHARED_CLASSES) && (defined(TR_HOST_X86) ||
       // defined(TR_HOST_POWER) || defined(TR_HOST_S390) || defined(TR_HOST_ARM64))
    return metaData;
}

void updateCompThreadActivationPolicy(TR::CompilationInfoPerThreadBase *compInfoPT,
    JITServer::ServerMemoryState nextMemoryState, JITServer::ServerActiveThreadsState nextActiveThreadState)
{
    // When the server starts running low on memory or uses high number of compilation threads, clients need to reduce
    // load on the server. This is achieved by suspending compilation threads once low server memory or high number of
    // compilation threads are detected.
    //
    ...
    //
    // Once the server reaches LOW threshold for memory or HIGH threshold for active threads, the client enters MAINTAIN
    // activation policy, it keeps the current threads running but is not allowed to start new ones.
    //
    // If the situation deteriorates further and the server reaches VERY_LOW threshold for memory or very HIGH threshold
    // for active threads, client begins SUSPEND activation policy, disabling all but one compilation thread.
    //
    ...
    if (nextMemoryState == JITServer::ServerMemoryState::VERY_LOW
        || nextActiveThreadState == JITServer::ServerActiveThreadsState::VERY_HIGH_THREAD) {
        ...
        || nextActiveThreadState == JITServer::ServerActiveThreadsState::HIGH_THREAD) {
...
TR_MethodMetaData *remoteCompile(J9VMThread *vmThread, TR::Compilation *compiler, TR_ResolvedMethod *compilee,
    J9Method *method, TR::IlGeneratorMethodDetails &details, TR::CompilationInfoPerThreadBase *compInfoPT)
{
    ...
    if (persistentInfo->getJITServerAOTCacheIgnoreLocalSCC()) {
        aotCacheStore = entry->_useAOTCacheCompilation
            && compInfo->methodCanBeJITServerAOTCacheStored(compiler->signature(),
                compilee->convertToMethod()->methodType());
        aotCacheLoad = persistentInfo->getJITServerUseAOTCache() && !persistentInfo->doNotRequestJITServerAOTCacheLoad()
            && !TR::CompilationInfo::isCompiled(method) && !entry->_doNotLoadFromJITServerAOTCache
            && compInfo->methodCanBeJITServerAOTCacheLoaded(compiler->signature(),
                compilee->convertToMethod()->methodType());
    } else {
    ...
    if (!client) {
        try {
            if (JITServerHelpers::isServerAvailable()) {
            ...
            } else if (JITServerHelpers::shouldRetryConnection(
                           OMRPORT_FROM_J9PORT(compInfoPT->getJitConfig()->javaVM->portLibrary))) {
                client = new (PERSISTENT_NEW) JITServer::ClientStream(persistentInfo);
                if (!enableJITServerPerCompConn)
                    compInfoPT->setClientStream(client);
                ...
                if (TR::Options::isAnyVerboseOptionSet(TR_VerboseJITServer, TR_VerboseCompilationDispatch))
                    TR_VerboseLog::writeLineLocked(TR_Vlog_FAILURE,
                        "Server is not available. Retry with local compilation for %s @ %s", compiler->signature(),
                        compiler->getHotnessName());
                Trc_JITServerRetryLocalCompile(vmThread, compiler->signature(), compiler->getHotnessName());
                compiler->failCompilation<JITServer::StreamFailure>(
                    "Server is not available, should retry with local compilation.");
            }
        } catch (const JITServer::StreamFailure &e) {
            if (TR::Options::isAnyVerboseOptionSet(TR_VerboseJITServer, TR_VerboseCompilationDispatch))
                TR_VerboseLog::writeLineLocked(TR_Vlog_FAILURE, "JITServer::StreamFailure: %s for %s @ %s", e.what(),
            ...
            Trc_JITServerStreamFailure(vmThread, compInfoPT->getCompThreadId(), __FUNCTION__, compiler->signature(),
                compiler->getHotnessName(), e.what());

            compiler->failCompilation<JITServer::StreamFailure>(e.what());
        } catch (const std::bad_alloc &e) {
            if (TR::Options::isAnyVerboseOptionSet(TR_VerboseJITServer, TR_VerboseCompilationDispatch))
                TR_VerboseLog::writeLineLocked(TR_Vlog_FAILURE, "std::bad_alloc: %s for %s @ %s", e.what(),
                    compiler->signature(), compiler->getHotnessName());
    ...
    if (aotCacheStore || aotCacheLoad) {
        if (!compiler->getPersistentInfo()->getJITServerAOTCacheIgnoreLocalSCC()) {
            ...
            if (TR_SharedCache::INVALID_CLASS_CHAIN_OFFSET == classChainOffset) {
                if (TR::Options::getVerboseOption(TR_VerboseJITServer))
                    TR_VerboseLog::writeLineLocked(TR_Vlog_JITServer,
                        "ERROR: Failed to get defining class chain offset for method %s", compiler->signature());
        ...
        // The first entry of the class chain data stores the size of the whole record in bytes, so the number of
        // classes is 1 less than the necessary class chain length.
        uintptr_t numClasses = compiler->fej9vm()->necessaryClassChainLength(clazz) - 1;
        ramClassChain = JITServerHelpers::getRAMClassChain(clazz, numClasses, vmThread, compiler->trMemory(), compInfo,
            uncachedRAMClasses, uncachedClassInfos);
    }
    ...
    std::vector<uintptr_t> newKnownIds
        = deserializer ? deserializer->getNewKnownIds(context) : std::vector<uintptr_t>();
    ...
    // sequencing monitor critical section scope
    {
        ...
        if (!chtableUpdates.first.empty() || !chtableUpdates.second.empty() || !illegalModificationList.empty()
            || !unloadedClasses.empty() || details.isJitDumpMethod()) {
    ...
    uint32_t methodIndex
        = (uint32_t)(method - clazz->ramMethods); // Index in the array of methods of the defining class
    try {
        ...
        if (TR::Options::getVerboseOption(TR_VerboseJITServer)) {
            TR_VerboseLog::writeLineLocked(TR_Vlog_JITServer,
                "Client sending compReq seqNo=%u to server for method %s @ %s.", seqNo, compiler->signature(),
                compiler->getHotnessName());
        }

        Trc_JITServerRemoteCompileRequest(vmThread, seqNo, compiler->signature(), compiler->getHotnessName());

        client->buildCompileRequest(persistentInfo->getClientUID(), seqNo, lastCriticalSeqNo, method, clazz,
            *entry->_optimizationPlan, detailsStr, details.getType(), unloadedClasses, illegalModificationList,
            classInfoTuple, optionsStr, recompMethodInfoStr, chtableUpdates.first, chtableUpdates.second,
            useAotCompilation, TR::Compiler->vm.isVMInStartupPhase(compInfoPT->getJitConfig()), aotCacheStore,
            aotCacheLoad, methodIndex, classChainOffset, ramClassChain, uncachedRAMClasses, uncachedClassInfos,
            newKnownIds, numPermanentLoaders);
        ...
        if (JITServer::MessageType::compilationCode == response) {
            auto recv = client->getRecvData<std::string, std::string, CHTableCommitData,
                std::vector<TR_OpaqueClassBlock *>, std::string, std::vector<TR_ResolvedJ9Method *>,
            ...
            auto recv = client->getRecvData<std::string, std::vector<std::string>, CHTableCommitData,
                std::vector<TR_OpaqueClassBlock *>, std::string, std::vector<TR_ResolvedJ9Method *>,
                TR_OptimizationPlan, std::vector<SerializedRuntimeAssumption>, JITServer::ServerMemoryState,
                JITServer::ServerActiveThreadsState, std::vector<TR_OpaqueMethodBlock *> >();
            ...
            bool writeVerboseLog = TR::Options::isAnyVerboseOptionSet(TR_VerboseJITServer,
                TR_VerboseCompilationDispatch, TR_VerbosePerformance, TR_VerboseCompFailure);
            if (writeVerboseLog)
                TR_VerboseLog::writeLineLocked(TR_Vlog_FAILURE,
                    "Server compilation thread crashed while compiling %s (%s)", compiler->signature(),
                    compiler->getHotnessName());
            Trc_JITServerCompThreadCrashed(vmThread, compInfoPT->getCompThreadId(), compiler->signature(),
                compiler->getHotnessName());
            ...
            TR_ASSERT_FATAL((response == JITServer::MessageType::compilationThreadCrashed)
            ...
            if (response == JITServer::MessageType::compilationThreadCrashed) {
                ...
                if (writeVerboseLog)
                    TR_VerboseLog::writeLineLocked(TR_Vlog_JITServer, "Requested JitDump recompilation of %s (%s)",
                        compiler->signature(), compiler->getHotnessName());
                if (enableJITServerPerCompConn)
                    compInfoPT->setClientStream(client);
                try {
                    ...
                    if (writeVerboseLog)
                        TR_VerboseLog::writeLineLocked(TR_Vlog_FAILURE,
                            "Server diagnostic thread crashed while compiling %s (%s)", compiler->signature(),
                            compiler->getHotnessName());
                }
            } else {
                if (writeVerboseLog)
                    TR_VerboseLog::writeLineLocked(TR_Vlog_FAILURE,
                        "Failed to generate IL of the crashing method, aborting diagnostic recompilation");
            }

            // Since server has crashed, all compilations will switch to local
            JITServerHelpers::postStreamFailure(OMRPORT_FROM_J9PORT(compInfoPT->getJitConfig()->javaVM->portLibrary),
                compInfo, false, false);
            ...
            if (aotCacheUnavailable) {
                ...
                if (TR::Options::getVerboseOption(TR_VerboseJITServer))
                    TR_VerboseLog::writeLineLocked(TR_Vlog_JITServer,
                        "Server AOT cache unavailable (serverUID=%llu). Disabling future AOT cache requests for this "
                        "server.",
                ...
                if (TR::Options::getVerboseOption(TR_VerboseJITServer))
                    TR_VerboseLog::writeLineLocked(TR_Vlog_JITServer,
                        "Cannot store methods in server AOT cache (serverUID=%llu). Disabling future AOT cache store "
            ...
            TR_ASSERT(JITServer::MessageType::compilationFailure == response,
                "Received %u %s but expected JITServer::MessageType::compilationFailure message type", response,
                JITServer::messageNames[response]);
            auto recv = client->getRecvData<uint32_t, uint64_t>();
            statusCode = std::get<0>(recv);
            uint64_t otherData = std::get<1>(recv);
            if (statusCode == compilationLowPhysicalMemory
                && otherData != -1) // if failed due to low memory, should've received an updated memory state
        ...
        if (statusCode >= compilationMaxError)
            throw JITServer::StreamTypeMismatch(
                "Did not receive a valid TR_CompilationErrorCode as the final message on the stream.");
        ...
        JITServerHelpers::postStreamFailure(OMRPORT_FROM_J9PORT(compInfoPT->getJitConfig()->javaVM->portLibrary),
            compInfo, e.retryConnectionImmediately(), false);
        ...
        if (!details.isJitDumpMethod()) {
            ...
            TR_Memory::jitPersistentFree(client);
            compInfoPT->setClientStream(NULL);
            JITServer::ClientStream::incrementIncompatibilityCount(
                OMRPORT_FROM_J9PORT(compInfoPT->getJitConfig()->javaVM->portLibrary));
        }

        if (TR::Options::isAnyVerboseOptionSet(TR_VerboseJITServer, TR_VerboseCompilationDispatch))
            TR_VerboseLog::writeLineLocked(TR_Vlog_FAILURE, "JITServer::StreamVersionIncompatible: %s for %s @ %s",
                e.what(), compiler->signature(), compiler->getHotnessName());
        ...
        if (TR::Options::isAnyVerboseOptionSet(TR_VerboseJITServer, TR_VerboseCompilationDispatch))
            TR_VerboseLog::writeLineLocked(TR_Vlog_FAILURE, "JITServer::StreamMessageTypeMismatch: %s for %s @ %s",
                e.what(), compiler->signature(), compiler->getHotnessName());
    ...
    if (statusCode == compilationOK || statusCode == compilationNotNeeded) {
        try {
            ...
            metaData
                = remoteCompilationEnd(vmThread, compiler, compilee, method, compInfoPT, codeCacheStr, dataCacheStr);
            if (metaData) {
                if (TR::Options::getVerboseOption(TR_VerboseVectorAPI)
                    && (metaData->flags & JIT_METADATA_VECTORIZED_CODE)) {
                ...
                for (auto &it : serializedRuntimeAssumptions) {
                    uint8_t *basePtr
                        = it.isOffsetFromMetaDataBase() ? (uint8_t *)metaData : (uint8_t *)(metaData->codeCacheAlloc);
                    uint8_t *addrToPatch = (uint8_t *)(basePtr + it.getOffset());
                    switch (it.getKind()) {
                        case RuntimeAssumptionOnRegisterNative: {
                            TR_PatchJNICallSite::make(compiler->fej9vm(), compiler->trPersistentMemory(), it.getKey(),
                                addrToPatch, compiler->getMetadataAssumptionList());
                            break;
                        }
                        case RuntimeAssumptionOnClassRedefinitionPIC: {
                            createClassRedefinitionPicSite((void *)it.getKey(), addrToPatch, it.getSize(), false,
                                compiler->getMetadataAssumptionList());
                            compiler->setHasClassRedefinitionAssumptions();
                            break;
                        }
                        case RuntimeAssumptionOnClassRedefinitionUPIC: {
                            createClassRedefinitionPicSite((void *)it.getKey(), addrToPatch, it.getSize(), true,
                                compiler->getMetadataAssumptionList());
                            compiler->setHasClassRedefinitionAssumptions();
                            break;
                        }
                        case RuntimeAssumptionOnClassUnload: {
                            createClassUnloadPicSite((void *)it.getKey(), addrToPatch, it.getSize(),
                                compiler->getMetadataAssumptionList());
                ...
                for (auto &it : methodsRequiringTrampolines) {
                    if (compInfoPT->reloRuntime()->codeCache()->reserveResolvedTrampoline(it, true)
                        != OMR::CodeCacheErrorCode::ERRORCODE_SUCCESS)
                        compiler->failCompilation<TR::RecoverableTrampolineError>(
                            "Failed to allocate trampoline in the code cache");
                }
            }

            if (!useAotCompilation
                && (compiler->isDeserializedAOTMethodStore() || !compiler->isDeserializedAOTMethod())) {
#if defined(J9VM_OPT_OPENJDK_METHODHANDLE)
                ...
                for (TR_OpaqueClassBlock *clazz : classesThatShouldNotBeNewlyExtended) {
                    ...
                    if (it != newlyExtendedClasses->end() && (it->second & (1 << compInfoPT->getCompThreadId()))) {
                        if (TR::Options::isAnyVerboseOptionSet(TR_VerboseJITServer, TR_VerboseCompileEnd,
                                TR_VerbosePerformance, TR_VerboseCompFailure))
                            TR_VerboseLog::writeLineLocked(TR_Vlog_FAILURE,
                                "Class that should not be newly extended was extended when compiling %s",
                                compiler->signature());
                        compiler->failCompilation<J9::CHTableCommitFailure>(
                            "Class that should not be newly extended was extended");
                    }
                }
                ...
                if (!JITClientCHTableCommit(compiler, metaData, chTableData)) {
                    ...
                    if (TR::Options::isAnyVerboseOptionSet(TR_VerboseJITServer, TR_VerboseCompileEnd,
                            TR_VerbosePerformance, TR_VerboseCompFailure)) {
                        TR_VerboseLog::writeLineLocked(TR_Vlog_FAILURE,
                            "JITClient: Failure while committing chtable for %s", compiler->signature());
                    }
            ...
            if (compiler->getOption(TR_JITServerFollowRemoteCompileWithLocalCompile) && compilationSequenceNumber) {
                ...
                if (TR::Options::getVerboseOption(TR_VerboseJITServer))
                    TR_VerboseLog::writeLineLocked(TR_Vlog_JITServer,
                        "Client compiling method %s @ %s [metaData=%p, startPC=%p] locally following remote "
                        "compilation",
                        compiler->signature(), compiler->getHotnessName(), metaData,
                        (metaData) ? (void *)metaData->startPC : NULL);
                intptr_t rtn = 0;
                try {
                ...
                if (TR::Options::getVerboseOption(TR_VerboseJITServer))
                    TR_VerboseLog::writeLineLocked(TR_Vlog_JITServer,
                        "Client successfully recompiled method %s @ %s [metaData=%p, startPC=%p] locally following "
                        "remote compilation",
                        compiler->signature(), compiler->getHotnessName(), metaData,
                        (metaData) ? (void *)metaData->startPC : NULL);
                if (compileWithoutVMAccess)
            ...
            if (TR::Options::getVerboseOption(TR_VerboseJITServer)) {
                TR_VerboseLog::writeLineLocked(TR_Vlog_JITServer,
                    "Client successfully loaded method %s @ %s following compilation request. [metaData=%p, "
                    "startPC=%p]",
                    compiler->signature(), compiler->getHotnessName(), metaData,
                    (metaData) ? (void *)metaData->startPC : NULL);
            }
            Trc_JITServerMethodSuccessfullyLoaded(vmThread, compiler->signature(), compiler->getHotnessName(), metaData,
                (metaData) ? (void *)metaData->startPC : NULL);
        } catch (const std::exception &e) {
            // Log for JITClient mode and re-throw
            if (TR::Options::getVerboseOption(TR_VerboseJITServer)) {
                TR_VerboseLog::writeLineLocked(TR_Vlog_JITServer,
                    "Client failed to load method %s @ %s following compilation request.", compiler->signature(),
                    compiler->getHotnessName());
            }
        ...
        if (TR::Options::isAnyVerboseOptionSet(TR_VerboseJITServer, TR_VerboseCompilationDispatch))
            TR_VerboseLog::writeLineLocked(TR_Vlog_FAILURE,
                "JITServer::ServerCompilationFailure: errCode %u for %s @ %s", statusCode, compiler->signature(),
                compiler->getHotnessName());
