 ...
 */
JITServer::ServerMemoryState computeServerMemoryState(TR::CompilationInfo *compInfo)
{
    ...
    uint64_t lowMemoryThreshold = TR::Options::getSafeReservePhysicalMemoryValue()
        + (numClients + 4) * TR::Options::getScratchSpaceLowerBound();
    uint64_t veryLowMemoryThreshold
    ...
    if (freePhysicalMemorySizeB != OMRPORT_MEMINFO_NOT_AVAILABLE) {
        memoryState = freePhysicalMemorySizeB <= veryLowMemoryThreshold
            ...
            : (freePhysicalMemorySizeB <= lowMemoryThreshold ? JITServer::ServerMemoryState::LOW
 ...
 */
JITServer::ServerActiveThreadsState computeServerActiveThreadsState(TR::CompilationInfo *compInfo)
{
    ...
    JITServer::ServerActiveThreadsState activeThreadState = numOfActiveThreads > veryHighActiveThreadThreshold
        ...
        : (numOfActiveThreads > highActiveThreadThreshold ? JITServer::ServerActiveThreadsState::HIGH_THREAD
...
TR::CompilationInfoPerThreadRemote::CompilationInfoPerThreadRemote(TR::CompilationInfo &compInfo,
    J9JITConfig *jitConfig, int32_t id, bool isDiagnosticThread)
 ...
 */
void TR::CompilationInfoPerThreadRemote::notifyAndDetachWaitingRequests(ClientSessionData *clientSession)
{
    ...
    while (nextEntry) {
        ...
        if (expectedSeqNo <= clientSession->getLastProcessedCriticalSeqNo()) {
            ...
            if (TR::Options::getVerboseOption(TR_VerboseJITServer))
                TR_VerboseLog::writeLineLocked(TR_Vlog_JITServer,
                    "compThreadID=%d notifying out-of-sequence thread %d for clientUID=%llu seqNo=%u (entry=%p)",
                    getCompThreadId(), nextEntry->_compInfoPT->getCompThreadId(),
                    (unsigned long long)clientSession->getClientUID(), nextWaitingSeqNo, nextEntry);
 ...
 */
void TR::CompilationInfoPerThreadRemote::waitForMyTurn(ClientSessionData *clientSession, TR_MethodToBeCompiled &entry)
{
    ...
    do // Do a timed wait until the missing seqNo arrives
    {
        ...
        if (TR::Options::getVerboseOption(TR_VerboseJITServer))
            TR_VerboseLog::writeLineLocked(TR_Vlog_JITServer,
                "compThreadID=%d (entry=%p) doing a timed wait for %d ms (waiting for seqNo=%u)", getCompThreadId(),
                &entry, (int32_t)waitTimeMillis, criticalSeqNo);

        Trc_JITServerTimedWait(getCompilationThread(), getCompThreadId(), clientSession,
            (unsigned long long)clientSession->getClientUID(), &entry, seqNo, criticalSeqNo,
            clientSession->getNumActiveThreads(), (int32_t)waitTimeMillis);
        ...
        if (monitorStatus == 0) // Thread was notified
        {
            ...
            if (TR::Options::getVerboseOption(TR_VerboseJITServer))
                TR_VerboseLog::writeLineLocked(TR_Vlog_JITServer,
                    "compThreadID=%d (entry=%p) is parked. seqNo=%u was notified", getCompThreadId(), &entry, seqNo);
            ...
            Trc_JITServerParkThread(getCompilationThread(), getCompThreadId(), clientSession,
                (unsigned long long)clientSession->getClientUID(), &entry, seqNo, criticalSeqNo,
                clientSession->getNumActiveThreads(), seqNo);
        } else // Timeout
        {
            entry.getMonitor()->exit();
            ...
            if (TR::Options::isAnyVerboseOptionSet(TR_VerboseCompFailure, TR_VerboseJITServer, TR_VerbosePerformance))
                TR_VerboseLog::writeLineLocked(TR_Vlog_JITServer,
                    "compThreadID=%d (entry=%p) timed-out while waiting for seqNo=%u ", getCompThreadId(), &entry,
                    criticalSeqNo);

            Trc_JITServerTimedOut(getCompilationThread(), getCompThreadId(), clientSession,
                (unsigned long long)clientSession->getClientUID(), &entry, seqNo, criticalSeqNo,
                clientSession->getNumActiveThreads(), criticalSeqNo);
            ...
            if (criticalSeqNo <= clientSession->getLastProcessedCriticalSeqNo()) {
                ...
                if (headEntry) {
                    ...
                    TR_ASSERT_FATAL(seqNo < headSeqNo,
                        "Next in line method cannot be in the waiting list: seqNo=%u >= headSeqNo=%u entry=%p "
                        "headEntry=%p",
                ...
                !getWaitToBeNotified()) // Avoid a cohort of threads clearing the caches
            {
                ...
                if (TR::Options::getVerboseOption(TR_VerboseJITServer))
                    TR_VerboseLog::writeLineLocked(TR_Vlog_JITServer,
                        "compThreadID=%d has cleared the session caches for clientUID=%llu criticalSeqNo=%u seqNo=%u "
                        "firstEntry=%p",
                ...
                Trc_JITServerClearedSessionCaches(getCompilationThread(), getCompThreadId(), clientSession,
                    (unsigned long long)clientSession->getClientUID(), seqNo, criticalSeqNo,
                    clientSession->getNumActiveThreads(), &entry, clientSession->getLastProcessedCriticalSeqNo(),
                    seqNo);
                ...
                if (TR::Options::getVerboseOption(TR_VerboseJITServer))
                    TR_VerboseLog::writeLineLocked(TR_Vlog_JITServer,
                        "compThreadID=%d which previously timed-out will go to sleep again. Possible reasons "
                        "numActiveThreads=%d waitToBeNotified=%d",
                ...
                Trc_JITServerThreadGoSleep(getCompilationThread(), getCompThreadId(), clientSession,
                    (unsigned long long)clientSession->getClientUID(), seqNo, criticalSeqNo,
                    clientSession->getNumActiveThreads(), getWaitToBeNotified());
            }
        }
    } while (criticalSeqNo > clientSession->getLastProcessedCriticalSeqNo());
}

bool TR::CompilationInfoPerThreadRemote::serveCachedAOTMethod(TR_MethodToBeCompiled &entry, J9Method *method,
    J9Class *definingClass, TR_OptimizationPlan *optPlan, ClientSessionData *clientData,
    J9::J9SegmentProvider &scratchSegmentProvider)
{
    ...
    auto aotCache = clientData->getAOTCache();
    auto serializedMethod = aotCache->findMethod(_definingClassChainRecord, _methodIndex, optPlan->getOptLevel(),
    ...
    if (TR::Options::isAnyVerboseOptionSet(TR_VerbosePerformance, TR_VerboseCompileEnd)) {
        ...
        TR_VerboseLog::write(TR_Vlog_COMP, "(AOT cache %s) %.*s.%.*s%.*s %s Q_SZ=%d Q_SZI=%d QW=%d j9m=%p bcsz=%u",
            ...
            J9UTF8_LENGTH(J9ROMMETHOD_SIGNATURE(romMethod)),
            (const char *)J9UTF8_DATA(J9ROMMETHOD_SIGNATURE(romMethod)), entry.getMethodDetails().name(),
            _compInfo.getMethodQueueSize(), _compInfo.getNumQueuedFirstTimeCompilations(), _compInfo.getQueueWeight(),
            method, _compInfo.getMethodBytecodeSize(romMethod));
    ...
    entry._stream->write(JITServer::MessageType::AOTCache_serializedAOTMethod,
        std::string((const char *)&serializedMethod->data(), serializedMethod->data().size()), serializedRecords,
        *optPlan, computeServerMemoryState(getCompilationInfo()),
 ...
 */
void TR::CompilationInfoPerThreadRemote::processCompilationRequest(CompilationRequest &req,
    JITServer::ServerStream *stream, TR::CompilationInfo *compInfo, J9VMThread *compThread,
    ClientSessionData *&clientSession, TR_MethodToBeCompiled &entry, TR_OptimizationPlan *&optPlan,
    J9::J9SegmentProvider &scratchSegmentProvider, uint64_t &clientId, uint32_t &seqNo, bool &hasUpdatedSeqNo,
    bool &useAotCompilation, bool &isCriticalRequest, bool &hasIncNumActiveThreads, bool &aotCacheHit,
    bool &abortCompilation)
{
    ...
    *(uintptr_t *)clientDetails = 0; // smash remote vtable pointer to catch bugs early
    ...
    TR::IlGeneratorMethodDetails *serverDetails
        = TR::IlGeneratorMethodDetails::clone(serverDetailsStorage, *clientDetails, detailsType);

    isCriticalRequest = (!chtableMods.empty() || !chtableUnloads.empty() || !illegalModificationList.empty()
    ...
    bool sessionDataWasEmpty = false;
    {
        ...
        if (!(clientSession = compInfo->getClientSessionHT()->findOrCreateClientSession(clientId, criticalSeqNo,
                  &sessionDataWasEmpty, _jitConfig)))
            throw std::bad_alloc();
        ...
        TR_ASSERT(!clientSession->usesPerClientMemory()
                || (TR::Compiler->persistentMemory() != compInfo->persistentMemory()),
    ...
    if (TR::Options::getVerboseOption(TR_VerboseJITServer))
        TR_VerboseLog::writeLineLocked(TR_Vlog_JITServer,
            "compThreadID=%d %s clientSessionData=%p for clientUID=%llu seqNo=%u (isCritical=%d) (criticalSeqNo=%u "
            "lastProcessedCriticalReq=%u)",
            getCompThreadId(), sessionDataWasEmpty ? "created" : "found", clientSession, (unsigned long long)clientId,
            seqNo, isCriticalRequest, criticalSeqNo, clientSession->getLastProcessedCriticalSeqNo());

    Trc_JITServerClientSessionData1(compThread, getCompThreadId(), sessionDataWasEmpty ? "created" : "found",
        clientSession, (unsigned long long)clientId, seqNo, isCriticalRequest, criticalSeqNo,
    ...
    if (criticalSeqNo > clientSession->getLastProcessedCriticalSeqNo()) {
        ...
        if (TR::Options::getVerboseOption(TR_VerboseJITServer))
            TR_VerboseLog::writeLineLocked(TR_Vlog_JITServer,
                "compThreadID=%d out-of-sequence msg=%u detected for clientUID=%llu criticalSeqNo=%u > "
                "lastCriticalSeqNo=%u. Parking this thread (entry=%p)",
                getCompThreadId(), seqNo, (unsigned long long)clientId, criticalSeqNo,
                clientSession->getLastProcessedCriticalSeqNo(), &entry);

        Trc_JITServerOutOfSequenceMsg1(compThread, getCompThreadId(), clientSession, (unsigned long long)clientId,
            &entry, seqNo, clientSession->getLastProcessedCriticalSeqNo(), clientSession->getNumActiveThreads(),
            clientSession->getLastProcessedCriticalSeqNo());
    ...
    TR_ASSERT_FATAL(criticalSeqNo <= clientSession->getLastProcessedCriticalSeqNo(),
        "Critical requests must be processed in order: compThreadID=%d seqNo=%u criticalSeqNo=%u > "
        "lastProcessedCriticalSeqNo=%u clientUID=%llu",
        getCompThreadId(), seqNo, criticalSeqNo, clientSession->getLastProcessedCriticalSeqNo(),
    ...
    if (criticalSeqNo < clientSession->getLastProcessedCriticalSeqNo()) {
        ...
        if (isCriticalRequest) {
            if (TR::Options::isAnyVerboseOptionSet(TR_VerboseCompFailure, TR_VerboseJITServer, TR_VerbosePerformance))
                TR_VerboseLog::writeLineLocked(TR_Vlog_JITServer,
                    "compThreadID=%d discarding older msg for clientUID=%llu seqNo=%u criticalSeqNo=%u < "
                    "lastProcessedCriticalSeqNo=%u",
                    getCompThreadId(), (unsigned long long)clientId, seqNo, criticalSeqNo,
                    clientSession->getLastProcessedCriticalSeqNo());

            Trc_JITServerDiscardMessage(compThread, getCompThreadId(), clientSession, (unsigned long long)clientId,
                seqNo, criticalSeqNo, clientSession->getNumActiveThreads());
    ...
    bool mustClearCaches
        = std::find(unloadedClasses.begin(), unloadedClasses.end(), ClientSessionData::mustClearCachesFlag)
        != unloadedClasses.end();
    if (mustClearCaches) {
        if (TR::Options::isAnyVerboseOptionSet(TR_VerboseCompFailure, TR_VerboseJITServer, TR_VerbosePerformance))
            TR_VerboseLog::writeLineLocked(TR_Vlog_JITServer,
    ...
    if (clientSession->cachesAreCleared()) {
        ...
        if (clientSession->cachesAreCleared()) {
            if (TR::Options::getVerboseOption(TR_VerboseJITServer))
                TR_VerboseLog::writeLineLocked(TR_Vlog_JITServer,
                    "compThreadID=%d will ask for address ranges of unloaded classes and CHTable for clientUID %llu",
                    getCompThreadId(), (unsigned long long)clientId);

            stream->write(JITServer::MessageType::getUnloadedClassRangesAndCHTable,
                compInfo->getPersistentInfo()->getServerUID());
            auto response
                = stream->read<std::vector<TR_AddressRange>, int32_t, std::string, std::vector<J9Method *> >();
            ...
            // Need CHTable mutex
            TR_ASSERT_FATAL(!chTable->isInitialized(), "CHTable must be empty for clientUID=%llu",
                (unsigned long long)clientId);
            if (TR::Options::getVerboseOption(TR_VerboseJITServer))
                TR_VerboseLog::writeLineLocked(TR_Vlog_JITServer,
                    "compThreadID=%d will initialize CHTable for clientUID %llu size=%zu", getCompThreadId(),
                    (unsigned long long)clientId, serializedCHTable.size());
    ...
    TR_ASSERT_FATAL(!clientSession->cachesAreCleared(),
        "Client Session caches should not be cleared at this point for clientUID=%llu", (unsigned long long)clientId);
    if (!initializedCHTable) {
        ...
        if (!illegalModificationList.empty()) {
            clientSession->processIllegalFinalFieldModificationList(
                illegalModificationList); // this locks getROMMapMonitor()
        }

        // Process the CHTable updates in order
        // Note that applying the updates will acquire the CHTable monitor and VMAccess
        if ((!chtableUnloads.empty() || !chtableMods.empty()) && !serverDetails->isJitDumpMethod()) {
            auto chTable
                = (JITServerPersistentCHTable *)clientSession->getCHTable(); // Will create CHTable if it doesn't exist
            TR_ASSERT_FATAL(chTable->isInitialized(), "CHTable must have been initialized for clientUID=%llu",
                (unsigned long long)clientId);
    ...
    if (isCriticalRequest) {
        clientSession->getSequencingMonitor()->enter();
        TR_ASSERT_FATAL(criticalSeqNo <= clientSession->getLastProcessedCriticalSeqNo(),
            "compThreadID=%d clientSessionData=%p clientUID=%llu seqNo=%u, criticalSeqNo=%u != "
            "lastProcessedCriticalSeqNo=%u, numActiveThreads=%d",
        ...
        Trc_JITServerUpdateSeqNo(getCompilationThread(), getCompThreadId(), clientSession,
            (unsigned long long)clientSession->getClientUID(), getSeqNo(), criticalSeqNo,
            clientSession->getNumActiveThreads(), clientSession->getLastProcessedCriticalSeqNo(), seqNo);
    ...
    if (!(romClass = JITServerHelpers::getRemoteROMClassIfCached(clientSession, clazz))) {
        ...
        if (!(std::get<0>(classInfoTuple).empty())) {
            romClass = JITServerHelpers::romClassFromString(std::get<0>(classInfoTuple), std::get<24>(classInfoTuple),
                clientSession->persistentMemory());
        } else {
            ...
            romClass
                = JITServerHelpers::getRemoteROMClass(clazz, stream, clientSession->persistentMemory(), classInfoTuple);
        }
        ...
        if (!_definingClassChainRecord) {
            if (TR::Options::getVerboseOption(TR_VerboseJITServer))
                TR_VerboseLog::writeLineLocked(TR_Vlog_JITServer,
                    ...
                    (unsigned long long)clientId, clazz,
    ...
    if (aotCacheLoad)
        aotCacheHit
            = serveCachedAOTMethod(entry, ramMethod, clazz, &clientOptPlan, clientSession, scratchSegmentProvider);
    ...
    if (clientSession->useServerOffsets(stream) && requestedAOTCacheStore && !aotCacheHit && !_aotCacheStore) {
 ...
 * @brief Private method specific to processing a compilation request.
 */
void TR::CompilationInfoPerThreadRemote::processAOTCacheMapRequest(const std::string &aotCacheName,
    TR::CompilationInfo *compInfo, JITServer::ServerStream *stream)
{
    if (TR::Options::getVerboseOption(TR_VerboseJITServer)) {
        TR_VerboseLog::writeLineLocked(TR_Vlog_JITServer,
            "compThreadID=%d handling request for AOT cache %s method list", getCompThreadId(), aotCacheName.c_str());
    }
        ...
        try {
            {
                ...
                for (; cachedAOTMethod != NULL; cachedAOTMethod = cachedAOTMethod->getNextRecord()) {
            ...
            if (TR::Options::isAnyVerboseOptionSet(TR_VerboseJITServer))
                TR_VerboseLog::writeLineLocked(TR_Vlog_FAILURE, "std::bad_alloc: %s", e.what());
        }

        if (TR::Options::getVerboseOption(TR_VerboseJITServer)) {
            TR_VerboseLog::writeLineLocked(TR_Vlog_JITServer, "Sending the list of AOT methods size %d",
 ...
 */
void TR::CompilationInfoPerThreadRemote::processEntry(TR_MethodToBeCompiled &entry,
    J9::J9SegmentProvider &scratchSegmentProvider)
{
    ...
    try {
        ...
        if (messageType == JITServer::MessageType::compilationRequest) {
            processCompilationRequest(req, stream, compInfo, compThread, clientSession, entry, optPlan,
                scratchSegmentProvider, clientId, seqNo, hasUpdatedSeqNo, useAotCompilation, isCriticalRequest,
                hasIncNumActiveThreads, aotCacheHit, abortCompilation);
        ...
        if (TR::Options::getVerboseOption(TR_VerboseJITServer))
            TR_VerboseLog::writeLineLocked(TR_Vlog_JITServer,
                "compThreadID=%d stream failed while reading the compilation request: %s", getCompThreadId(), e.what());
        ...
        if (TR::Options::getVerboseOption(TR_VerboseJITServer)
            ...
            TR_VerboseLog::writeLineLocked(TR_Vlog_JITServer, "compThreadID=%d stream version incompatible: %s",
        ...
        if (TR::Options::getVerboseOption(TR_VerboseJITServer))
            TR_VerboseLog::writeLineLocked(TR_Vlog_JITServer, "compThreadID=%d stream message type mismatch: %s",
                getCompThreadId(), e.what());
        ...
        if (TR::Options::getVerboseOption(TR_VerboseJITServer))
            TR_VerboseLog::writeLineLocked(TR_Vlog_JITServer,
                "compThreadID=%d stream connection terminated by JITClient on stream %p while reading the compilation "
                "request: %s",
        ...
        if (TR::Options::getVerboseOption(TR_VerboseJITServer))
            TR_VerboseLog::writeLineLocked(TR_Vlog_JITServer,
                "compThreadID=%d stream interrupted by JITClient while reading the compilation request: %s",
                getCompThreadId(), e.what());
        ...
        if (isCriticalRequest && !hasUpdatedSeqNo) {
            ...
            if (seqNo > clientSession->getLastProcessedCriticalSeqNo()) {
                Trc_JITServerUpdateSeqNo(getCompilationThread(), getCompThreadId(), clientSession,
                    (unsigned long long)clientSession->getClientUID(), seqNo, getExpectedSeqNo(),
                    clientSession->getNumActiveThreads(), clientSession->getLastProcessedCriticalSeqNo(), seqNo);
    ...
    if (abortCompilation) {
        ...
        if (TR::Options::getVerboseOption(TR_VerboseJITServer)) {
            if (getClientData()) {
                TR_VerboseLog::writeLineLocked(TR_Vlog_JITServer,
                    "compThreadID=%d did an early abort for clientUID=%llu seqNo=%u", getCompThreadId(),
                    getClientData()->getClientUID(), getSeqNo());
                Trc_JITServerEarlyAbortClientData(compThread, getCompThreadId(),
                    (unsigned long long)getClientData()->getClientUID(), getSeqNo());
            } else {
        ...
        if (!compInfo->getPersistentInfo()->getDisableFurtherCompilation() && !deleteStream
        ...
        if (getClientData()) {
            ...
            if (getClientData()->getInUse() == 0) {
                ...
                if (result) {
                    if (TR::Options::getVerboseOption(TR_VerboseJITServer))
                        TR_VerboseLog::writeLineLocked(TR_Vlog_JITServer, "client (%llu) deleted",
                            (unsigned long long)clientId);
                }
            }
            setClientData(NULL);
    ...
    //   {
    //   J9::NewInstanceThunkDetails &newInstanceDetails = static_cast<J9::NewInstanceThunkDetails
    //   &>(entry.getMethodDetails()); J9Class  *classForNewInstance = newInstanceDetails.classNeedingThunk();
    ...
    if (!compInfo->getPersistentInfo()->getDisableFurtherCompilation() && !enableJITServerPerCompConn
 ...
 */
bool TR::CompilationInfoPerThreadRemote::cacheIProfilerInfo(TR_OpaqueMethodBlock *method, uint32_t byteCodeIndex,
 ...
 * @return always return true at the moment
 */
bool TR::CompilationInfoPerThreadRemote::cacheIProfilerInfo(TR_OpaqueMethodBlock *method,
    const Vector<TR_IPBytecodeHashTableEntry *> &entries)
{
 ...
 */
TR_IPBytecodeHashTableEntry *TR::CompilationInfoPerThreadRemote::getCachedIProfilerInfo(TR_OpaqueMethodBlock *method,
 ...
 * @return returns void
 */
void TR::CompilationInfoPerThreadRemote::cacheResolvedMethod(TR_ResolvedMethodKey key, TR_OpaqueMethodBlock *method,
    uint32_t vTableSlot, const TR_ResolvedJ9JITServerMethodInfo &methodInfo, bool isUnresolvedInCP,
    int32_t ttlForUnresolved)
{
    ...
    if (!std::get<1>(methodInfo).empty()) {
        bodyInfo = (TR_PersistentJittedBodyInfo *)trMemory->allocateHeapMemory(sizeof(TR_PersistentJittedBodyInfo),
            TR_MemoryBase::Recompilation);
    ...
    if (!std::get<2>(methodInfo).empty()) {
        pMethodInfo = (TR_PersistentMethodInfo *)trMemory->allocateHeapMemory(sizeof(TR_PersistentMethodInfo),
            TR_MemoryBase::Recompilation);
    ...
    if (!std::get<3>(methodInfo).empty()) {
        entry = (TR_ContiguousIPMethodHashTableEntry *)trMemory->allocateHeapMemory(
            sizeof(TR_ContiguousIPMethodHashTableEntry), TR_MemoryBase::Recompilation);
 ...
 */
bool TR::CompilationInfoPerThreadRemote::getCachedResolvedMethod(TR_ResolvedMethodKey key,
    TR_ResolvedJ9JITServerMethod *owningMethod, TR_ResolvedMethod **resolvedMethod, bool *unresolvedInCP)
{
    ...
    *resolvedMethod = NULL;
    ...
    if (getCachedValueFromPerCompilationMap(_resolvedMethodInfoMap, key, methodCacheEntry)) {
        ...
        TR_ResolvedJ9JITServerMethodInfo methodInfo = make_tuple(methodInfoStruct,
            methodCacheEntry.persistentBodyInfo
                ? std::string((const char *)methodCacheEntry.persistentBodyInfo, sizeof(TR_PersistentJittedBodyInfo))
                : std::string(),
            methodCacheEntry.persistentMethodInfo
                ? std::string((const char *)methodCacheEntry.persistentMethodInfo, sizeof(TR_PersistentMethodInfo))
                : std::string(),
            methodCacheEntry.IPMethodInfo
                ? std::string((const char *)methodCacheEntry.IPMethodInfo, sizeof(TR_ContiguousIPMethodHashTableEntry))
                : std::string());
        // Re-add validation record
        if (comp->compileRelocatableCode() && comp->getOption(TR_UseSymbolValidationManager)
            && !comp->getSymbolValidationManager()->inHeuristicRegion()) {
        ...
        if (key.type != TR_ResolvedMethodType::VirtualFromOffset) {
            *resolvedMethod = owningMethod->createResolvedMethodFromJ9Method(comp, key.cpIndex, vTableSlot,
                (J9Method *)method, NULL, methodInfo);
        } else {
            if (_vm->isAOT_DEPRECATED_DO_NOT_USE())
                *resolvedMethod = method ? new (comp->trHeapMemory()) TR_ResolvedRelocatableJ9JITServerMethod(method,
                                               _vm, comp->trMemory(), methodInfo, owningMethod)
                                         : 0;
            else
                *resolvedMethod = method
                    ? new (comp->trHeapMemory())
                          TR_ResolvedJ9JITServerMethod(method, _vm, comp->trMemory(), methodInfo, owningMethod)
                    : 0;
        }
 ...
 *
 * @param type Resolved method type: VirtualFromCP, VirtualFromOffset, Interface, Static, Special, ImproperInterface,
 * NoType
 ...
 */
TR_ResolvedMethodKey TR::CompilationInfoPerThreadRemote::getResolvedMethodKey(TR_ResolvedMethodType type,
    TR_OpaqueClassBlock *ramClass, int32_t cpIndex, TR_OpaqueClassBlock *classObject)
{
 ...
 * @return void
 */
void TR::CompilationInfoPerThreadRemote::cacheResolvedMirrorMethodsPersistIPInfo(TR_ResolvedJ9Method *resolvedMethod)
{
 ...
 * @return void
 */
void TR::CompilationInfoPerThreadRemote::cacheNullClassOfStatic(TR_OpaqueClassBlock *ramClass, int32_t cpIndex)
{
 ...
 */
bool TR::CompilationInfoPerThreadRemote::getCachedNullClassOfStatic(TR_OpaqueClassBlock *ramClass, int32_t cpIndex)
{
 ...
 * @return void
 */
void TR::CompilationInfoPerThreadRemote::cacheFieldOrStaticAttributes(TR_OpaqueClassBlock *ramClass, int32_t cpIndex,
 ...
 */
bool TR::CompilationInfoPerThreadRemote::getCachedFieldOrStaticAttributes(TR_OpaqueClassBlock *ramClass,
    int32_t cpIndex, TR_J9MethodFieldAttributes &attrs, bool isStatic)
{
    if (isStatic)
...
void TR::CompilationInfoPerThreadRemote::addClassToNullClassSignatureCache(const ClassLoaderStringPair &clsp)
{
...
bool TR::CompilationInfoPerThreadRemote::classIsInNullClassSignatureCache(const ClassLoaderStringPair &clsp)
{
    if (!_nullClassSignatureCache) {
        return false;
    }
 ...
 */
void TR::CompilationInfoPerThreadRemote::clearPerCompilationCaches()
{
 ...
 */
void TR::CompilationInfoPerThreadRemote::deleteClientSessionData(uint64_t clientId, TR::CompilationInfo *compInfo,
    J9VMThread *compThread)
{
    // Need to acquire compilation monitor for both deleting the client data and the setting the thread state to
    ...
    if (TR::Options::getVerboseOption(TR_VerboseJITServer) || TR::Options::getVerboseOption(TR_VerboseJITServerConns)) {
        ...
        if (!result) {
            TR_VerboseLog::writeLineLocked(TR_Vlog_JITServer,
                "t=%6u Client (clientUID=%llu) disconnected. Client session not deleted", timestamp,
                (unsigned long long)clientId);
        } else {
            TR_VerboseLog::writeLineLocked(TR_Vlog_JITServer,
                "t=%6u Client (clientUID=%llu) disconnected. Client session deleted", timestamp,
                (unsigned long long)clientId);
        }
    }
...
void TR::CompilationInfoPerThreadRemote::freeAllResources()
{
    if (_recompilationMethodInfo) {
