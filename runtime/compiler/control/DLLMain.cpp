...
static IDATA initializeCompilerArgs(J9JavaVM *vm, J9VMDllLoadInfo *loadInfo, J9VMInitArgs *j9vmArgs, IDATA argIndex,
    char **xCommandLineOptionsPtr, bool isXjit, bool mergeCompilerOptions)
{
    ...
    if (mergeCompilerOptions) {
        ...
        if (sizeOfOption) {
            ...
            TR_ASSERT_FATAL(cursor == &xCommandLineOptions[sizeOfOption - 1],
                "%s cursor=%p, xCommandLineOptions=%p, sizeOfOption=%d\n", VMOPT_WITH_COLON, cursor,
                xCommandLineOptions, sizeOfOption);
...
uintptr_t initializeCompilerArgsPostRestore(J9JavaVM *vm, intptr_t argIndex, char **xCommandLineOptionsPtr, bool isXjit,
    bool mergeCompilerOptions)
{
    J9VMDllLoadInfo *loadInfo = FIND_DLL_TABLE_ENTRY(THIS_DLL_NAME);
    return initializeCompilerArgs(vm, loadInfo, vm->checkpointState.restoreArgsList, argIndex, xCommandLineOptionsPtr,
        isXjit, mergeCompilerOptions);
}
#endif
...
#define AOT_INIT_STAGE SYSTEM_CLASSLOADER_SET /* defined separately to ensure dependencies below */
    ...
    switch (stage) {
        case DLL_LOAD_TABLE_FINALIZED: {
            ...
            argIndexRIDisabled
                = J9::Options::getExternalOptionIndex(J9::ExternalOptions::XXminusRuntimeInstrumentation);
            ...
            TR::Options::_doNotProcessEnvVars
                = (J9::Options::getExternalOptionIndex(J9::ExternalOptions::XXdoNotProcessJitEnvVars) >= 0);
...
/* If debuginfoserver has been loaded and no FSD support, unload this library and aot */
            ...
            /* Platform detection seems difficult at this point.                              */
            {
                ...
                bool disableZeroedTLHPages = disableDualTLH
                    && (((notlhPrefetch >= 0)
                        || (!TR::Compiler->target.cpu.is(OMR_PROCESSOR_PPC_P6)
                            && !TR::Compiler->target.cpu.is(OMR_PROCESSOR_PPC_P7))));
#endif // TR_HOST_POWER
#endif // TR_HOST_X86
                ...
                if (
                    ...
                    if (!IS_STAGE_COMPLETED(gcLoadInfo->completedBits,
                            JCL_INITIALIZED)) //&& vm->memoryManagerFunctions)
                    {
        ...
        case AOT_INIT_STAGE:
            if (isAOT) {
                ...
                // TODO: This now acts as a general "Is the jit shutting down" flag, should it be renamed / changed for
                // something different?
                if (vm->jitConfig)
        ...
        case JIT_INITIALIZED:
            if (isJIT || isAOT) {
                ...
                /* We need to initialize the following if we allow JIT compilation, AOT compilation or AOT relocation to
                 * be done */
                try {
                    argIndexMergeOptionsEnabled
                        = J9::Options::getExternalOptionIndex(J9::ExternalOptions::XXplusMergeCompilerOptions);
                    argIndexMergeOptionsDisabled
                        = J9::Options::getExternalOptionIndex(J9::ExternalOptions::XXminusMergeCompilerOptions);
                    ...
                    if (isJIT && argIndexXjit >= 0) {
                        IDATA rc = initializeCompilerArgs(vm, loadInfo, vm->vmArgsArray, argIndexXjit,
                            &xjitCommandLineOptions, true, mergeCompilerOptions);
                        if (rc)
                            return rc;
                    }
                    ...
                    if (isAOT && argIndexXaot >= 0) {
                        IDATA rc = initializeCompilerArgs(vm, loadInfo, vm->vmArgsArray, argIndexXaot,
                            &xaotCommandLineOptions, false, mergeCompilerOptions);
                        if (rc)
                            return rc;
                    }
                    ...
                    if (!jitConfig) {
                        vm->internalVMFunctions->setErrorJ9dll(PORTLIB, loadInfo, "cannot initialize JIT: no jitconfig",
                            FALSE);
                        return J9VMDLLMAIN_FAILED;
                    }
                    ...
                    if (jitConfig->runtimeFlags & J9JIT_JIT_ATTACHED)
                        goto _abort;
                    if (onLoadInternal(vm, jitConfig, xjitCommandLineOptions, xaotCommandLineOptions, initialFlags,
                            reserved, isJIT ? 0 : 1))
                        goto _abort;
...
_abort:
        ...
        case ABOUT_TO_BOOTSTRAP: {
            ...
            if (vm->sharedClassConfig && ((vm->sharedClassConfig->runtimeFlags & aotFlags) == aotFlags)
                && TR::Options::getAggressivityLevel() != OMR::Options::AGGRESSIVE_THROUGHPUT) {
                ...
                if (sharedCache != NULL) {
                    ...
                    if (persistentInfo->getTrackAOTDependencies()
                        && !TR::Options::getCmdLineOptions()->getOption(TR_DisableCHOpts)) {
                        TR_AOTDependencyTable *dependencyTable
                            = new (PERSISTENT_NEW) TR_AOTDependencyTable(sharedCache);
            ...
            if (isJIT || isAOT) {
                /* Although we've initialized the compiler, we have to tell the compiler whether it can perform JIT
                 * compiles This is because the compiler can be configured to only AOT compile. */
                ...
                } else if (!TR::Options::getCmdLineOptions()->allowRecompilation()
                ...
                if (isAOT && TR::Options::getAOTCmdLineOptions()->getOption(TR_EnableClassChainValidationCaching)
                    && (TR::Options::getCmdLineOptions()->getOption(TR_DisableCHOpts))) {
        ...
        case VM_INITIALIZATION_COMPLETE: {
            ...
            if (isJIT || isAOT) {
                ...
                TR::CompilationInfoPerThread * const *arrayOfCompInfoPT
                    = compInfo->getArrayOfCompilationInfoPerThread();
                ...
                for (int32_t i = 0; i < compInfo->getNumTotalAllocatedCompilationThreads(); i++) {
                    ...
                    // snprintf(threadName, sizeof(threadName), "JIT Compilation Thread-%d",
                    // curCompThreadInfoPT->getCompThreadId());

                    char *threadName = (curCompThreadInfoPT->compilationThreadIsActive()
                            ? curCompThreadInfoPT->getActiveThreadName()
                            : curCompThreadInfoPT->getSuspendedThreadName());

                    vm->internalVMFunctions->initializeAttachedThread(curThread, threadName, vm->systemThreadGroupRef,
                        ((compThread->privateFlags & J9_PRIVATE_FLAGS_DAEMON_THREAD) != 0), compThread);

                    if ((curThread->currentException != NULL) || (curThread->threadObject == NULL)) {
                        if ((NULL == loadInfo->fatalErrorStr) || ('\0' == loadInfo->fatalErrorStr[0]))
                            vm->internalVMFunctions->setErrorJ9dll(PORTLIB, loadInfo,
                                "cannot create the jit Thread object", FALSE);
                        return J9VMDLLMAIN_FAILED;
                    }
                ...
                if (compInfo->getSamplerThread()) {
                    ...
                    vm->internalVMFunctions->initializeAttachedThread(curThread, "JIT-SamplerThread",
                    ...
                    if ((curThread->currentException != NULL) || (curThread->threadObject == NULL)) {
                        if ((NULL == loadInfo->fatalErrorStr) || ('\0' == loadInfo->fatalErrorStr[0]))
                            vm->internalVMFunctions->setErrorJ9dll(PORTLIB, loadInfo,
                                "cannot create the jit Sampler Thread object", FALSE);
                        return J9VMDLLMAIN_FAILED;
                    }
                ...
                if (iProfiler) {
                    ...
                    if (iProfilerThread) {
                        vm->internalVMFunctions->initializeAttachedThread(curThread, "IProfiler",
                            vm->systemThreadGroupRef,
                            ((iProfilerThread->privateFlags & J9_PRIVATE_FLAGS_DAEMON_THREAD) != 0), iProfilerThread);
                        if ((curThread->currentException != NULL) || (curThread->threadObject == NULL)) {
                            if ((NULL == loadInfo->fatalErrorStr) || ('\0' == loadInfo->fatalErrorStr[0]))
                                vm->internalVMFunctions->setErrorJ9dll(PORTLIB, loadInfo,
                                    "cannot create the iProfiler Thread object", FALSE);
                            return J9VMDLLMAIN_FAILED;
                        }
                ...
                if (jProfiler) {
                    ...
                    if (jProfilerThread) {
                        vm->internalVMFunctions->initializeAttachedThread(curThread, "JProfiler",
                            vm->systemThreadGroupRef,
                            ((jProfilerThread->privateFlags & J9_PRIVATE_FLAGS_DAEMON_THREAD) != 0), jProfilerThread);
                        if ((curThread->currentException != NULL) || (curThread->threadObject == NULL)) {
                            if ((NULL == loadInfo->fatalErrorStr) || ('\0' == loadInfo->fatalErrorStr[0]))
                                vm->internalVMFunctions->setErrorJ9dll(PORTLIB, loadInfo,
                                    "cannot create the jProfiler Thread object", FALSE);
                            return J9VMDLLMAIN_FAILED;
                        }
                    ...
                    if (!trvm->isAOT_DEPRECATED_DO_NOT_USE() && trvm->_compInfo) {
                        ...
                        if (trvm->_compInfo->getPersistentInfo()->getRemoteCompilationMode() == JITServer::SERVER) {
                            ...
                            MetricsServer *metricsServer
                                = ((TR_JitPrivateConfig *)(vm->jitConfig->privateConfig))->metricsServer;
                            if (metricsServer) {
                                metricsServer->stop();
                            }
                        }
