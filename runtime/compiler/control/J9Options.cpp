...
int32_t J9::Options::_relaxedCompilationLimitsSampleThreshold
    = 120; // normally should be lower than the scorchingSampleThreshold
int32_t J9::Options::_sampleThresholdVariationAllowance = 30;

int32_t J9::Options::_maxCheckcastProfiledClassTests = 3;
int32_t J9::Options::_maxOnsiteCacheSlotForInstanceOf
    = 0; // Setting this value to zero will disable onsite cache in instanceof.
...
int32_t J9::Options::_sampleDontSwitchToProfilingThreshold
    = 3000; // default=1% use large value to disable// To be tuned
...
int32_t J9::Options::_smallMethodBytecodeSizeThresholdForJITServerAOTCache
    = 0; // 0 means disabled; good values to try 0-32
...
int32_t J9::Options::_lowCompDensityModeEnterThreshold
    = 4; // Maximum number of compilations per 10 min of CPU required to enter low compilation density mode. Use 0 to
         // disable feature
int32_t J9::Options::_lowCompDensityModeExitThreshold
    = 15; // Minimum number of compilations per 10 min of CPU required to exit low compilation density mode
int32_t J9::Options::_lowCompDensityModeExitLPQSize
    = 120; // Minimum number of compilations in LPQ to take us out of low compilation density mode
...
int32_t J9::Options::_interpreterSamplingThresholdInJSR292
    = TR_DEFAULT_INITIAL_COUNT - 2; // Run stuff twice before getting too excited about interpreter ticks
...
int32_t J9::Options::_disableIProfilerClassUnloadThreshold
    = 20000; // The usefulness of IProfiling is questionable at this point
...
#endif
int32_t J9::Options::_iProfilerBcHashTableSize
    = 131049; // prime number; Note: 131049 * 8 fits in 1 segment of persistent memory
...
int32_t J9::Options::_numQueuedInvReqToDowngradeOptLevel
    = 20; // If more than 20 inv req are queued we compiled them at cold
...
uint32_t J9::Options::_hwprofilerReducedWarmOptLevelThreshold
    = 0; // 0 ==> upgrade methods with just 1 tick in any given interval
...
int32_t J9::Options::_hwProfilerRecompDecisionWindow
    = 5000; // Should be at least as big as _hwProfilerRecompFrequencyThreshold
...
int32_t J9::Options::_seriousCompFailureThreshold
    = 10; // above this threshold we generate a trace point in the Snap file
...
size_t J9::Options::_scratchSpaceLimitKBWhenLowVirtualMemory
    = 64 * 1024; // 64MB; currently, only used on 32 bit Windows
...
int32_t J9::Options::_numDLTBufferMatchesToEagerlyIssueCompReq
    = 8; // a value of 1 or less disables the DLT tracking mechanism
...
const char *getLargePageTypeString(UDATA pageFlags)
{
...
bool J9::Options::useCompressedPointers() { return TR::Compiler->om.compressObjectReferences(); }
...
const char *J9::Options::versionOption(const char *option, void *base, TR::OptionTable *entry)
{
...
const char *J9::Options::limitOption(const char *option, void *base, TR::OptionTable *entry)
{
...
const char *J9::Options::limitfileOption(const char *option, void *base, TR::OptionTable *entry)
{
    ...
    if (J9::Options::getJITCmdLineOptions() == NULL) {
        // if JIT options are NULL, means we're processing AOT options now
        return J9::Options::getDebug()->limitfileOption(option, base, entry, TR::Options::getAOTCmdLineOptions(), false,
            pseudoRandomNumbersListPtr);
    } else {
        // otherwise, we're processing JIT options
        return J9::Options::getDebug()->limitfileOption(option, base, entry, TR::Options::getJITCmdLineOptions(), false,
            pseudoRandomNumbersListPtr);
    }
}

const char *J9::Options::inlinefileOption(const char *option, void *base, TR::OptionTable *entry)
{
    if (!J9::Options::getDebug() && !J9::Options::createDebug())
        return 0;
...
const char *Options::gcOnResolveOption(const char *option, void *base, TR::OptionTable *entry)
{
    J9JITConfig *jitConfig = (J9JITConfig *)base;
...
const char *Options::vmStateOption(const char *option, void *base, TR::OptionTable *entry)
{
    J9JITConfig *jitConfig = (J9JITConfig *)base;
    PORT_ACCESS_FROM_JAVAVM(jitConfig->javaVM);
    char *p = (char *)option;
                ...
                case ((J9VMSTATE_SHAREDCLASS_FIND >> 16) & 0xF):
                    ...
                    if ((state & 0xFFFF0) == (origState & 0xFFFF0)) {
                        ...
                        if (lowState >= 0x1 && lowState <= vmStateArray[index]._xsize) {
                            invalidState = false;
                            j9tty_printf(PORTLIB, "vmState [0x%x]: {%s}\n", state,
                                vmSharedStateArray[--lowState]._xname);
                        }
                    }
                    break;
...
const char *Options::loadLimitOption(const char *option, void *base, TR::OptionTable *entry)
{
...
const char *Options::loadLimitfileOption(const char *option, void *base, TR::OptionTable *entry)
{
    if (!TR::Options::getDebug() && !TR::Options::createDebug())
        return 0;
    ...
    if (TR::Options::getJITCmdLineOptions() == NULL) {
        // if JIT options are NULL, means we're processing AOT options now
        return TR::Options::getDebug()->limitfileOption(option, base, entry, TR::Options::getAOTCmdLineOptions(),
            true /* new param */, pseudoRandomNumbersListPtr);
    } else {
        J9JITConfig *jitConfig = (J9JITConfig *)base;
        PORT_ACCESS_FROM_JAVAVM(jitConfig->javaVM);
        // otherwise, we're processing JIT options
...
#if defined(J9VM_OPT_JITSERVER)
const char *Options::JITServerAOTCacheLimitOption(const char *option, void *base, TR::OptionTable *entry,
    TR::CompilationFilters *&filters, const char *optName)
{
    if (!TR::Options::getDebug() && !TR::Options::createDebug())
        return NULL;
    if (TR::Options::getJITCmdLineOptions() == NULL) {
...
const char *Options::JITServerAOTCacheStoreLimitOption(const char *option, void *base, TR::OptionTable *entry)
{
    return JITServerAOTCacheLimitOption(option, base, entry, _JITServerAOTCacheStoreFilters,
        "jitserverAOTCacheStoreExclude");
}

const char *Options::JITServerAOTCacheLoadLimitOption(const char *option, void *base, TR::OptionTable *entry)
{
    return JITServerAOTCacheLimitOption(option, base, entry, _JITServerAOTCacheLoadFilters,
        "jitserverAOTCacheLoadExclude");
}

const char *Options::JITServerRemoteExclude(const char *option, void *base, TR::OptionTable *entry)
{
    if (!TR::Options::getDebug() && !TR::Options::createDebug())
        return 0;
    if (TR::Options::getJITCmdLineOptions() != NULL) {
        // this should be specified as a JIT option
        return TR::Options::getDebug()->limitOption(option, base, entry, TR::Options::getJITCmdLineOptions(),
            _JITServerRemoteExcludeFilters);
    } else {
...
const char *Options::tprofOption(const char *option, void *base, TR::OptionTable *entry)
{
    J9JITConfig *jitConfig = (J9JITConfig *)base;
    PORT_ACCESS_FROM_JAVAVM(jitConfig->javaVM);
...
const char *Options::setJitConfigRuntimeFlag(const char *option, void *base, TR::OptionTable *entry)
{
...
const char *Options::resetJitConfigRuntimeFlag(const char *option, void *base, TR::OptionTable *entry)
{
    J9JITConfig *jitConfig = (J9JITConfig *)_feBase;
    jitConfig->runtimeFlags &= ~(entry->parm2);
    return option;
...
const char *Options::setJitConfigNumericValue(const char *option, void *base, TR::OptionTable *entry)
{
...
TR::OptionTable OMR::Options::_feOptions[] = {
    ...
    { "activeThreadsThresholdForInterpreterSampling=",
     "M<nnn>\tSampling does not affect invocation count beyond this threshold", TR::Options::setStaticNumeric,
     (intptr_t)&TR::Options::_activeThreadsThreshold, 0, "F%d", NOT_IN_SUBSET },
#if defined(J9VM_OPT_JITSERVER)
    { "aotCacheDisableGeneratedClassSupport",
     " \tDisable support for generated classes such as lambdas in JITServer AOT cache", TR::Options::setStaticBool,
     (intptr_t)&TR::Options::_aotCacheDisableGeneratedClassSupport, 1, "F%d", NOT_IN_SUBSET },
    { "aotCachePersistenceMinDeltaMethods=",
     "M<nnn>\tnumber of extra AOT methods that need to be added to the JITServer AOT cache before considering a "
        "save operation", TR::Options::setStaticNumeric, (intptr_t)&TR::Options::_aotCachePersistenceMinDeltaMethods, 0, "F%d",
     NOT_IN_SUBSET },
    { "aotCachePersistenceMinPeriodMs=",
     "M<nnn>\tmiminum time between two consecutive JITServer AOT cache save operations (ms)", TR::Options::setStaticNumeric, (intptr_t)&TR::Options::_aotCachePersistenceMinPeriodMs, 0, "F%d",
     NOT_IN_SUBSET },
#endif  /* defined(J9VM_OPT_JITSERVER) */
    { "aotMethodCompilesThreshold=",
     "R<nnn>\tIf this many AOT methods are compiled before exceeding aotMethodThreshold, don't stop AOT compiling", TR::Options::setStaticNumeric, (intptr_t)&TR::Options::_aotMethodCompilesThreshold, 0, "F%d", NOT_IN_SUBSET },
    ...
    { "bcLimit=", "C<nnn>\tbytecode size limit", TR::Options::setJitConfigNumericValue,
     offsetof(J9JITConfig, bcSizeLimit), 0, "P%d" },
    { "bcountForBootstrapMethods=",
     "M<nnn>\tcount for loopy methods belonging to bootstrap classes. "
        "Used in no AOT cases", TR::Options::setStaticNumeric, (intptr_t)&TR::Options::_countForLoopyBootstrapMethods, 250, "F%d",
     NOT_IN_SUBSET },
    ...
    { "classLoadPhaseInterval=",
     "O<nnn>\tnumber of sampling ticks before we run "
        "again the code for a class loading phase detection", TR::Options::setStaticNumeric, (intptr_t)&TR::Options::_classLoadingPhaseInterval, 0, "P%d", NOT_IN_SUBSET },
    { "classLoadPhaseQuiesceIntervals=", "O<nnn>\tnumber of intervals we remain in classLoadPhase after it ended",
     TR::Options::setStaticNumeric, (intptr_t)&TR::Options::_numClassLoadPhaseQuiesceIntervals, 0, "F%d",
     NOT_IN_SUBSET },
    { "classLoadPhaseThreshold=",
     "O<nnn>\tnumber of classes loaded per sampling tick that "
        "needs to be attained to enter the class loading phase. "
        "Specify a very large value to disable this optimization", TR::Options::setStaticNumeric, (intptr_t)&TR::Options::_classLoadingPhaseThreshold, 0, "P%d", NOT_IN_SUBSET },
    { "classLoadPhaseVariance=",
     "O<nnn>\tHow much the classLoadPhaseThreshold can deviate from "
        "its average value (as a percentage). Specify an integer 0-99", TR::Options::setStaticNumeric, (intptr_t)&TR::Options::_classLoadingPhaseVariance, 0, "F%d", NOT_IN_SUBSET },
    ...
    { "code=", "C<nnn>\tcode cache size, in KB", TR::Options::setJitConfigNumericValue,
     offsetof(J9JITConfig, codeCacheKB), 0, "F%d (KB)" },
    { "codepad=", "C<nnn>\ttotal code cache pad size, in KB", TR::Options::setJitConfigNumericValue,
    ...
    { "compilationPriorityQSZThreshold=",
     "M<nnn>\tCompilation queue size threshold when priority of post-profiling"
        "compilation requests is increased", TR::Options::setStaticNumeric, (intptr_t)&TR::Options::_compPriorityQSZThreshold, 0, "F%d", NOT_IN_SUBSET },
    { "compilationQueueSizeLimit=",
     "R<nnn>\tWhen limit is reached, first-time compilations are postponed by replenishing the invocation count", TR::Options::setStaticNumeric, (intptr_t)&TR::Options::_qszLimit, 0, "F%d", NOT_IN_SUBSET },
    ...
    { "compilationYieldStatsHeartbeatPeriod=",
     "M<nnn>\tperiodically print stats about compilation yield points "
        "Period is in ms. Default is 0 which means don't do it. "
        "Values between 1 and 99 ms will be upgraded to 100 ms.", TR::Options::setStaticNumeric, (intptr_t)&TR::Options::_compYieldStatsHeartbeatPeriod, 0, "F%d",
     NOT_IN_SUBSET },
    { "compilationYieldStatsThreshold=",
     "M<nnn>\tprint stats about compilation yield points if the "
        "threshold is exceeded. Default 1000 usec. ", TR::Options::setStaticNumeric, (intptr_t)&TR::Options::_compYieldStatsThreshold, 0, "F%d", NOT_IN_SUBSET },
    { "compThreadPriority=",
     "M<nnn>\tThe priority of the compilation thread. "
        "Use an integer between 0 and 4. Default is 4 (highest priority)", TR::Options::setStaticNumeric, (intptr_t)&TR::Options::_compilationThreadPriorityCode, 0, "F%d",
     NOT_IN_SUBSET },
    { "conservativeScorchingSampleThreshold=",
     "R<nnn>\tLower bound for scorchingSamplingThreshold when scaling based on numProc", TR::Options::setStaticNumeric, (intptr_t)&TR::Options::_conservativeScorchingSampleThreshold, 0, "F%d",
     NOT_IN_SUBSET },
    { "countForBootstrapMethods=",
     "M<nnn>\tcount for loopless methods belonging to bootstrap classes. "
        "Used in no AOT cases", TR::Options::setStaticNumeric, (intptr_t)&TR::Options::_countForLooplessBootstrapMethods, 1000, "F%d",
     NOT_IN_SUBSET },
    { "cpuCompTimeExpensiveThreshold=",
     "M<nnn>\tthreshold for when hot & very-hot compilations occupied enough cpu time to be considered expensive in "
        "millisecond", TR::Options::setStaticNumeric, (intptr_t)&TR::Options::_cpuCompTimeExpensiveThreshold, 0, "F%d",
     NOT_IN_SUBSET },
    ...
    { "cpuUtilThresholdForStarvation=", "M<nnn>\tThreshold for deciding that a comp thread is not starved",
     ...
     NOT_IN_SUBSET },
    { "data=", "C<nnn>\tdata cache size, in KB", TR::Options::setJitConfigNumericValue,
     offsetof(J9JITConfig, dataCacheKB), 0, "F%d (KB)" },
    ...
    { "dataCacheQuantumSize=",
     "I<nnn>\tLargest guaranteed common byte multiple of data cache allocations.  This value will be rounded up for "
        "pointer alignment.", TR::Options::setStaticNumeric, (intptr_t)&TR::Options::_dataCacheQuantumSize, 0, "F%d", NOT_IN_SUBSET },
    { "datatotal=", "C<nnn>\ttotal data memory limit, in KB", TR::Options::setJitConfigNumericValue,
     offsetof(J9JITConfig, dataCacheTotalKB), 0, "F%d (KB)" },
    ...
    { "disableIProfilerClassUnloadThreshold=",
     "R<nnn>\tNumber of classes that can be unloaded before we disable the IProfiler", TR::Options::setStaticNumeric,
     (intptr_t)&TR::Options::_disableIProfilerClassUnloadThreshold, 0, "F%d", NOT_IN_SUBSET },
    ...
    { "experimentalClassLoadPhaseInterval=", "O<nnn>\tnumber of sampling ticks to stay in a class load phase",
     TR::Options::setStaticNumeric, (intptr_t)&TR::Options::_experimentalClassLoadPhaseInterval, 0, "P%d",
     NOT_IN_SUBSET },
    { "gcNotify", "L\tlog scavenge/ggc notifications to stdout", SET_JITCONFIG_RUNTIME_FLAG(J9JIT_GC_NOTIFY) },
    { "gcOnResolve", "D[=<nnn>]\tscavenge on every resolve, or every resolve after nnn", TR::Options::gcOnResolveOption,
     0, 0, "F=%d" },
    { "GCRQueuedThresholdForCounting=",
     "M<nnn>\tDisable GCR counting if number of queued GCR requests exceeds this threshold", TR::Options::setStaticNumeric, (intptr_t)&TR::Options::_GCRQueuedThresholdForCounting, 0, "F%d",
     NOT_IN_SUBSET },
#ifdef DEBUG
    { "gcTrace=", "D<nnn>\ttrace gc stack walks after gc number nnn", TR::Options::setJitConfigNumericValue,
     offsetof(J9JITConfig, gcTraceThreshold), 0, "F%d" },
#endif
#if defined(J9VM_OPT_JITSERVER)
...
#endif  /* defined(J9VM_OPT_JITSERVER) */
    { "HWProfilerAOTWarmOptLevelThreshold=", "O<nnn>\tAOT Warm Opt Level Threshold", TR::Options::setStaticNumeric,
     (intptr_t)&TR::Options::_hwprofilerAOTWarmOptLevelThreshold, 0, "F%d", NOT_IN_SUBSET },
    { "HWProfilerBufferMaxPercentageToDiscard=",
     "O<nnn>\tpercentage of HW profiling buffers "
        "that JIT is allowed to discard instead of processing", TR::Options::setStaticNumeric, (intptr_t)&TR::Options::_hwProfilerBufferMaxPercentageToDiscard, 0, "F%d",
     NOT_IN_SUBSET },
    { "HWProfilerDisableAOT", "O<nnn>\tDisable RI AOT", SET_OPTION_BIT(TR_HWProfilerDisableAOT), "F", NOT_IN_SUBSET },
    ...
    { "HWProfilerExpirationTime=", "R<nnn>\t", TR::Options::setStaticNumeric,
     (intptr_t)&TR::Options::_hwProfilerExpirationTime, 0, "F%d", NOT_IN_SUBSET },
    { "HWProfilerHotOptLevelThreshold=", "O<nnn>\tHot Opt Level Threshold", TR::Options::setStaticNumeric,
     (intptr_t)&TR::Options::_hwprofilerHotOptLevelThreshold, 0, "F%d", NOT_IN_SUBSET },
    { "HWProfilerLastOptLevel=", "O<nnn>\tLast Opt level", TR::Options::setStaticNumeric,
     (intptr_t)&TR::Options::_hwprofilerLastOptLevel, 0, "F%d", NOT_IN_SUBSET },
    { "HWProfilerNumDowngradesToTurnRION=", "R<nnn>\t", TR::Options::setStaticNumeric,
     (intptr_t)&TR::Options::_numDowngradesToTurnRION, 0, "F%d", NOT_IN_SUBSET },
    { "HWProfilerNumOutstandingBuffers=",
     "O<nnn>\tnumber of outstanding hardware profiling buffers "
        "allowed in the system. Specify 0 to disable this optimization", TR::Options::setStaticNumeric, (intptr_t)&TR::Options::_hwprofilerNumOutstandingBuffers, 0, "F%d",
     NOT_IN_SUBSET },
    { "HWProfilerPRISamplingRate=", "O<nnn>\tP RI Scaling Factor", TR::Options::setStaticNumeric,
     (intptr_t)&TR::Options::_hwprofilerPRISamplingRate, 0, "F%d", NOT_IN_SUBSET },
    { "HWProfilerQSZMaxThresholdToRIDowngrade=", "R<nnn>\t", TR::Options::setStaticNumeric,
     (intptr_t)&TR::Options::_qszMaxThresholdToRIDowngrade, 0, "F%d", NOT_IN_SUBSET },
    { "HWProfilerQSZMinThresholdToRIDowngrade=", "R<nnn>\t", TR::Options::setStaticNumeric,
     (intptr_t)&TR::Options::_qszMinThresholdToRIDowngrade, 0, "F%d", NOT_IN_SUBSET },
    { "HWProfilerQSZToTurnRION=", "R<nnn>\t", TR::Options::setStaticNumeric,
     (intptr_t)&TR::Options::_qszThresholdToTurnRION, 0, "F%d", NOT_IN_SUBSET },
    { "HWProfilerRecompilationDecisionWindow=",
     "R<nnn>\tNumber of decisions to wait for before looking at stats decision outcome", TR::Options::setStaticNumeric, (intptr_t)&TR::Options::_hwProfilerRecompDecisionWindow, 0, "F%d",
     NOT_IN_SUBSET },
    { "HWProfilerRecompilationFrequencyThreshold=", "R<nnn>\tLess than 1 in N decisions to recompile, turns RI off",
     TR::Options::setStaticNumeric, (intptr_t)&TR::Options::_hwProfilerRecompFrequencyThreshold, 0, "F%d",
     NOT_IN_SUBSET },
    { "HWProfilerRecompilationInterval=", "O<nnn>\tRecompilation Interval", TR::Options::setStaticNumeric,
     (intptr_t)&TR::Options::_hwprofilerRecompilationInterval, 0, "F%d", NOT_IN_SUBSET },
    { "HWProfilerReducedWarmOptLevelThreshold=", "O<nnn>\tReduced Warm Opt Level Threshold",
     TR::Options::setStaticNumeric, (intptr_t)&TR::Options::_hwprofilerReducedWarmOptLevelThreshold, 0, "F%d",
     NOT_IN_SUBSET },
    { "HWProfilerRIBufferPoolSize=", "O<nnn>\tRI Buffer Pool Size", TR::Options::setStaticNumeric,
     (intptr_t)&TR::Options::_hwprofilerRIBufferPoolSize, 0, "F%d", NOT_IN_SUBSET },
    { "HWProfilerRIBufferProcessingFrequency=", "O<nnn>\tRI Buffer Processing Frequency", TR::Options::setStaticNumeric,
     (intptr_t)&TR::Options::_hwProfilerRIBufferProcessingFrequency, 0, "F%d", NOT_IN_SUBSET },
    { "HWProfilerRIBufferThreshold=", "O<nnn>\tRI Buffer Threshold", TR::Options::setStaticNumeric,
     (intptr_t)&TR::Options::_hwprofilerRIBufferThreshold, 0, "F%d", NOT_IN_SUBSET },
    { "HWProfilerScorchingOptLevelThreshold=", "O<nnn>\tScorching Opt Level Threshold", TR::Options::setStaticNumeric,
     (intptr_t)&TR::Options::_hwprofilerScorchingOptLevelThreshold, 0, "F%d", NOT_IN_SUBSET },
    { "HWProfilerWarmOptLevelThreshold=", "O<nnn>\tWarm Opt Level Threshold", TR::Options::setStaticNumeric,
     (intptr_t)&TR::Options::_hwprofilerWarmOptLevelThreshold, 0, "F%d", NOT_IN_SUBSET },
    { "HWProfilerZRIBufferSize=", "O<nnn>\tZ RI Buffer Size", TR::Options::setStaticNumeric,
     (intptr_t)&TR::Options::_hwprofilerZRIBufferSize, 0, "F%d", NOT_IN_SUBSET },
    { "HWProfilerZRIMode=", "O<nnn>\tZ RI Mode", TR::Options::setStaticNumeric,
     (intptr_t)&TR::Options::_hwprofilerZRIMode, 0, "F%d", NOT_IN_SUBSET },
    { "HWProfilerZRIRGS=", "O<nnn>\tZ RI Reporting Group Size", TR::Options::setStaticNumeric,
     (intptr_t)&TR::Options::_hwprofilerZRIRGS, 0, "F%d", NOT_IN_SUBSET },
    { "HWProfilerZRISF=", "O<nnn>\tZ RI Scaling Factor", TR::Options::setStaticNumeric,
     (intptr_t)&TR::Options::_hwprofilerZRISF, 0, "F%d", NOT_IN_SUBSET },
    { "inlinefile=",
     "D<filename>\tinline filter defined in filename.  "
        "Use inlinefile=filename", TR::Options::inlinefileOption, 0, 0, "F%s" },
    { "interpreterSamplingDivisor=",
     "R<nnn>\tThe divisor used to decrease the invocation count when an interpreted method is sampled", TR::Options::setStaticNumeric, (intptr_t)&TR::Options::_interpreterSamplingDivisor, 0, "F%d", NOT_IN_SUBSET },
    { "interpreterSamplingThreshold=",
     "R<nnn>\tThe maximum invocation count at which a sampling hit will result in the count being divided by the "
        "value of interpreterSamplingDivisor", TR::Options::setStaticNumeric, (intptr_t)&TR::Options::_interpreterSamplingThreshold, 0, "F%d", NOT_IN_SUBSET },
    { "interpreterSamplingThresholdInJSR292=",
     "R<nnn>\tThe maximum invocation count at which a sampling hit will result in the count being divided by the "
        "value of interpreterSamplingDivisor on a MethodHandle-oriented workload", TR::Options::setStaticNumeric, (intptr_t)&TR::Options::_interpreterSamplingThresholdInJSR292, 0, "F%d",
     NOT_IN_SUBSET },
    { "interpreterSamplingThresholdInStartupMode=",
     "R<nnn>\tThe maximum invocation count at which a sampling hit will result in the count being divided by the "
        "value of interpreterSamplingDivisor", TR::Options::setStaticNumeric, (intptr_t)&TR::Options::_interpreterSamplingThresholdInStartupMode, 0, "F%d",
     NOT_IN_SUBSET },
    { "invocationThresholdToTriggerLowPriComp=",
     "M<nnn>\tNumber of times a loopy method must be invoked to be eligible for LPQ", TR::Options::setStaticNumeric,
     (intptr_t)&TR::Options::_invocationThresholdToTriggerLowPriComp, 0, "F%d", NOT_IN_SUBSET },
    ...
    { "iprofilerBufferInterarrivalTimeToExitDeepIdle=",
     "M<nnn>\tIn ms. If 4 IP buffers arrive back-to-back more frequently than this value, JIT exits DEEP_IDLE", TR::Options::setStaticNumeric, (intptr_t)&TR::Options::_iProfilerBufferInterarrivalTimeToExitDeepIdle, 0, "F%d",
     NOT_IN_SUBSET },
    { "iprofilerBufferMaxPercentageToDiscard=",
     "O<nnn>\tpercentage of interpreter profiling buffers "
        "that JIT is allowed to discard instead of processing", TR::Options::setStaticNumeric, (intptr_t)&TR::Options::_iprofilerBufferMaxPercentageToDiscard, 0, "F%d",
     NOT_IN_SUBSET },
    { "iprofilerBufferSize=", "I<nnn>\t set the size of each iprofiler buffer", TR::Options::setStaticNumeric,
     (intptr_t)&TR::Options::_iprofilerBufferSize, 0, "F%d", NOT_IN_SUBSET },
    ...
    { "iprofilerIntToTotalSampleRatio=", "O<nnn>\tRatio of Interpreter samples to Total samples",
     TR::Options::setStaticNumeric, (intptr_t)&TR::Options::_iprofilerIntToTotalSampleRatio, 0, "F%d",
     NOT_IN_SUBSET },
    { "iprofilerMaxCount=", "O<nnn>\tmax invocation count for IProfiler to be active", TR::Options::setStaticNumeric,
     (intptr_t)&TR::Options::_maxIprofilingCount, 0, "F%d", NOT_IN_SUBSET },
    { "iprofilerMaxCountInStartupMode=", "O<nnn>\tmax invocation count for IProfiler to be active in STARTUP phase",
     TR::Options::setStaticNumeric, (intptr_t)&TR::Options::_maxIprofilingCountInStartupMode, 0, "F%d",
     NOT_IN_SUBSET },
    { "iprofilerMemoryConsumptionLimit=", "O<nnn>\tlimit on memory consumption for interpreter profiling data",
     TR::Options::setStaticNumeric, (intptr_t)&TR::Options::_iProfilerMemoryConsumptionLimit, 0, "P%d",
     NOT_IN_SUBSET },
    ...
    { "iprofilerNumOutstandingBuffers=",
     "O<nnn>\tnumber of outstanding interpreter profiling buffers "
        "allowed in the system. Specify 0 to disable this optimization", TR::Options::setStaticNumeric, (intptr_t)&TR::Options::_iprofilerNumOutstandingBuffers, 0, "F%d",
     NOT_IN_SUBSET },
    ...
    { "iprofilerOffSubtractionFactor=", "O<nnn>\tCounts Subtraction factor when IProfiler is Off",
     TR::Options::setStaticNumeric, (intptr_t)&TR::Options::_IprofilerOffSubtractionFactor, 0, "F%d",
     NOT_IN_SUBSET },
    { "iprofilerPreCheckpointDropRate=", "O<nnn>\tPercent*10 of buffers to drop precheckpoint",
     TR::Options::setStaticNumeric, (intptr_t)&TR::Options::_IprofilerPreCheckpointDropRate, 0, "F%d",
     NOT_IN_SUBSET },
    { "iprofilerSamplesBeforeTurningOff=",
     "O<nnn>\tnumber of interpreter profiling samples "
        "needs to be taken after the profiling starts going off to completely turn it off. "
        "Specify a very large value to disable this optimization", TR::Options::setStaticNumeric, (intptr_t)&TR::Options::_iprofilerSamplesBeforeTurningOff, 0, "P%d",
     NOT_IN_SUBSET },
    { "itFileNamePrefix=", "L<filename>\tprefix for itrace filename", TR::Options::setStringForPrivateBase,
     offsetof(TR_JitPrivateConfig, itraceFileNamePrefix), 0, "P%s" },
#if defined(J9VM_OPT_JITSERVER)
    ...
    { "jitserverMallocTrimInterval=",
     "M<nnn>\tmiminum time between two consecutive JITServer client malloc_trim invocations (ms)", TR::Options::setStaticNumeric, (intptr_t)&TR::Options::_jitserverMallocTrimInterval, 0, "F%d", NOT_IN_SUBSET },
#endif  /* defined(J9VM_OPT_JITSERVER) */
    { "jProfilingEnablementSampleThreshold=",
     "M<nnn>\tNumber of global samples to allow generation of JProfiling bodies", TR::Options::setStaticNumeric,
     (intptr_t)&TR::Options::_jProfilingEnablementSampleThreshold, 0, "F%d", NOT_IN_SUBSET },
    ...
    { "largeTranslationTime=",
     "D<nnn>\tprint IL trees for methods that take more than this value (usec)"
        "to compile. Need to have a log file defined on command line", TR::Options::setStaticNumeric, (intptr_t)&TR::Options::_largeTranslationTime, 0, "F%d", NOT_IN_SUBSET },
    { "limit=", "D<xxx>\tonly compile methods beginning with xxx", TR::Options::limitOption, 0, 0, "P%s" },
    { "limitfile=",
     "D<filename>\tfilter method compilation as defined in filename.  "
        "Use limitfile=(filename,firstLine,lastLine) to limit lines considered from firstLine to lastLine", TR::Options::limitfileOption, 0, 0, "F%s" },
    { "loadExclude=", "D<xxx>\tdo not relocate AOT methods beginning with xxx", TR::Options::loadLimitOption, 1, 0,
     "P%s" },
    { "loadLimit=", "D<xxx>\tonly relocate AOT methods beginning with xxx", TR::Options::loadLimitOption, 0, 0, "P%s" },
    { "loadLimitFile=",
     "D<filename>\tfilter AOT method relocation as defined in filename.  "
        "Use loadLimitfile=(filename,firstLine,lastLine) to limit lines considered from firstLine to lastLine", TR::Options::loadLimitfileOption, 0, 0, "P%s" },
    { "localCSEFrequencyThreshold=",
     "O<nnn>\tBlocks with frequency lower than the threshold will not be considered by localCSE", TR::Options::setStaticNumeric, (intptr_t)&TR::Options::_localCSEFrequencyThreshold, 0, "F%d", NOT_IN_SUBSET },
    { "loopyMethodDivisionFactor=", "O<nnn>\tCounts Division factor for Loopy methods", TR::Options::setStaticNumeric,
     (intptr_t)&TR::Options::_LoopyMethodDivisionFactor, 0, "F%d", NOT_IN_SUBSET },
...
#if defined(J9VM_OPT_JITSERVER)
    { "lowCompDensityModeEnterThreshold=",
     "M<nnn>\tMaximum number of compilations per 10 min of CPU required to enter low compilation density mode. Use "
        "0 to disable feature.", TR::Options::setStaticNumeric, (intptr_t)&TR::Options::_lowCompDensityModeEnterThreshold, 0, "F%d",
     NOT_IN_SUBSET },
    { "lowCompDensityModeExitLPQSize=",
     "M<nnn>\tMinimum number of compilations in LPQ to take us out of low compilation density mode", TR::Options::setStaticNumeric, (intptr_t)&TR::Options::_lowCompDensityModeExitLPQSize, 0, "F%d",
     NOT_IN_SUBSET },
    { "lowCompDensityModeExitThreshold=",
     "M<nnn>\tMinimum number of compilations per 10 min of CPU required to exit low compilation density mode", TR::Options::setStaticNumeric, (intptr_t)&TR::Options::_lowCompDensityModeExitThreshold, 0, "F%d",
     NOT_IN_SUBSET },
#endif  /* defined(J9VM_OPT_JITSERVER) */
    ...
    { "lowVirtualMemoryMBThreshold=",
     "M<nnn>\tThreshold when we declare we are running low on virtual memory. Use 0 to disable the feature", TR::Options::setStaticNumeric, (intptr_t)&TR::Options::_lowVirtualMemoryMBThreshold, 0, "F%d", NOT_IN_SUBSET },
    ...
    { "numDLTBufferMatchesToEagerlyIssueCompReq=", "R<nnn>\t", TR::Options::setStaticNumeric,
     (intptr_t)&TR::Options::_numDLTBufferMatchesToEagerlyIssueCompReq, 0, "F%d", NOT_IN_SUBSET },
    ...
    { "queuedInvReqThresholdToDowngradeOptLevel=", "M<nnn>\tDowngrade opt level if too many inv req",
     TR::Options::setStaticNumeric, (intptr_t)&TR::Options::_numQueuedInvReqToDowngradeOptLevel, 0, "F%d",
     NOT_IN_SUBSET },
    { "queueSizeThresholdToDowngradeDuringCLP=",
     "M<nnn>\tCompilation queue size threshold (interpreted methods) when opt level is downgraded during class load "
        "phase", TR::Options::setStaticNumeric, (intptr_t)&TR::Options::_qsziThresholdToDowngradeDuringCLP, 0, "F%d",
     NOT_IN_SUBSET },
    { "queueSizeThresholdToDowngradeOptLevel=", "M<nnn>\tCompilation queue size threshold when opt level is downgraded",
     TR::Options::setStaticNumeric, (intptr_t)&TR::Options::_qszThresholdToDowngradeOptLevel, 0, "F%d",
     NOT_IN_SUBSET },
    { "queueSizeThresholdToDowngradeOptLevelDuringStartup=",
     "M<nnn>\tCompilation queue size threshold when opt level is downgraded during startup phase", TR::Options::setStaticNumeric, (intptr_t)&TR::Options::_qszThresholdToDowngradeOptLevelDuringStartup, 0, "F%d",
     NOT_IN_SUBSET },
#if defined(J9VM_OPT_JITSERVER)
    ...
    { "relaxedCompilationLimitsSampleThreshold=",
     ...
     NOT_IN_SUBSET },
#if defined(J9VM_OPT_JITSERVER)
    { "remoteCompileExclude=",
     "D{regex}\tdo not send remote compilation request for methods matching regex to the JITServer", TR::Options::JITServerRemoteExclude, 1, 0, "P%s" },
#endif  /* defined(J9VM_OPT_JITSERVER) */
    { "resetCountThreshold=",
     "R<nnn>\tThe number of global samples which if exceed during a method's sampling interval will cause the "
        "method's sampling counter to be incremented by the number of samples in a sampling interval", TR::Options::setStaticNumeric, (intptr_t)&TR::Options::_resetCountThreshold, 0, "F%d", NOT_IN_SUBSET },
    { "rtlog=", "L<filename>\twrite verbose run-time output to filename", TR::Options::setStringForPrivateBase,
     offsetof(TR_JitPrivateConfig, rtLogFileName), 0, "P%s" },
    { "rtResolve", "D\ttreat all data references as unresolved", SET_JITCONFIG_RUNTIME_FLAG(J9JIT_RUNTIME_RESOLVE) },
    { "safeReservePhysicalMemoryValue=",
     "C<nnn>\tsafe buffer value before we risk running out of physical memory, in KB", TR::Options::setStaticNumericKBAdjusted, (intptr_t)&TR::Options::_safeReservePhysicalMemoryValue, 0,
     "F%d (bytes)" },
    { "sampleDontSwitchToProfilingThreshold=",
     "R<nnn>\tThe maximum number of global samples taken during a sample interval for which the method is denied "
        "swithing to profiling", TR::Options::setStaticNumeric, (intptr_t)&TR::Options::_sampleDontSwitchToProfilingThreshold, 0, "F%d",
     NOT_IN_SUBSET },
    { "sampleThresholdVariationAllowance=",
     "R<nnn>\tThe percentage that we add or subtract from"
        " the original threshold to adjust for method code size."
        " Must be 0--100. Make it 0 to disable this optimization.", TR::Options::setStaticNumeric, (intptr_t)&TR::Options::_sampleThresholdVariationAllowance, 0, "P%d",
     NOT_IN_SUBSET },
    { "samplingFrequencyInDeepIdleMode=",
     "R<nnn>\tnumber of milliseconds between samples for hotness - in deep idle mode", TR::Options::setStaticNumeric,
     (intptr_t)&TR::Options::_samplingFrequencyInDeepIdleMode, 0, "F%d", NOT_IN_SUBSET },
    ...
    { "scorchingSampleThreshold=",
     "R<nnn>\tThe maximum number of global samples taken during a sample interval for which the method will be "
        "recompiled as scorching", TR::Options::setStaticNumeric, (intptr_t)&TR::Options::_scorchingSampleThreshold, 0, "F%d", NOT_IN_SUBSET },
#if defined(J9VM_OPT_JITSERVER)
    { "scratchSpaceFactorWhenJITServerWorkload=", "M<nnn>\tMultiplier for scratch space limit at JITServer",
     TR::Options::setStaticNumeric, (intptr_t)&TR::Options::_scratchSpaceFactorWhenJITServerWorkload, 0, "F%d",
     NOT_IN_SUBSET },
#endif  /* defined(J9VM_OPT_JITSERVER) */
    { "scratchSpaceFactorWhenJSR292Workload=",
     "M<nnn>\tMultiplier for scratch space limit when MethodHandles are in use", TR::Options::setStaticNumeric,
     (intptr_t)&TR::Options::_scratchSpaceFactorWhenJSR292Workload, 0, "F%d", NOT_IN_SUBSET },
    { "scratchSpaceLimitKBForHotCompilations=",
     "M<nnn>\tLimit for memory used by JIT when compiling at hot and above (in KB)", TR::Options::setStaticNumericKBAdjusted, (intptr_t)&TR::Options::_scratchSpaceLimitForHotCompilations, 0,
     "F%d (bytes)", NOT_IN_SUBSET },
    { "scratchSpaceLimitKBWhenLowVirtualMemory=",
     "M<nnn>\tLimit for memory used by JIT when running on low virtual memory", TR::Options::setStaticNumeric,
     (intptr_t)&TR::Options::_scratchSpaceLimitKBWhenLowVirtualMemory, 0, "F%d", NOT_IN_SUBSET },
    { "secondaryClassLoadPhaseThreshold=",
     "O<nnn>\tWhen class load rate just dropped under the CLP threshold  "
        "we use this secondary threshold to determine class load phase", TR::Options::setStaticNumeric, (intptr_t)&TR::Options::_secondaryClassLoadingPhaseThreshold, 0, "F%d",
     NOT_IN_SUBSET },
    { "seriousCompFailureThreshold=",
     "M<nnn>\tnumber of srious compilation failures after which we write a trace point in the snap file", TR::Options::setStaticNumeric, (intptr_t)&TR::Options::_seriousCompFailureThreshold, 0, "F%d", NOT_IN_SUBSET },
#if defined(J9VM_OPT_JITSERVER)
    { "sharedROMClassCacheNumPartitions=",
     " \tnumber of JITServer ROMClass cache partitions (each has its own monitor)", TR::Options::setStaticNumeric,
     (intptr_t)&TR::Options::_sharedROMClassCacheNumPartitions, 0, "F%d", NOT_IN_SUBSET },
#endif  /* defined(J9VM_OPT_JITSERVER) */
    { "singleCache", "C\tallow only one code cache and one data cache to be allocated",
     RESET_JITCONFIG_RUNTIME_FLAG(J9JIT_GROW_CACHES) },
#if defined(J9VM_OPT_CRIU_SUPPORT)
...
#endif
    { "smallMethodBytecodeSizeThreshold=",
     "O<nnn> \tThreshold for determining small methods "
        "(measured in number of bytecodes)", TR::Options::setStaticNumeric, (intptr_t)&TR::Options::_smallMethodBytecodeSizeThreshold, 0, "F%d",
     NOT_IN_SUBSET },
    { "smallMethodBytecodeSizeThresholdForCold=",
     "O<nnn>\tThreshold for determining small methods at cold "
        "(measured in number of bytecodes)", TR::Options::setStaticNumeric, (intptr_t)&TR::Options::_smallMethodBytecodeSizeThresholdForCold, 0, "F%d",
     NOT_IN_SUBSET },
    { "smallMethodBytecodeSizeThresholdForJITServerAOTCache=",
     "O<nnn>\tThreshold for determining small methods that should "
        "not be converted to AOT, but rather be jitted remotely", TR::Options::setStaticNumeric, (intptr_t)&TR::Options::_smallMethodBytecodeSizeThresholdForJITServerAOTCache, 0,
     "F%d", NOT_IN_SUBSET },
    { "stack=", "C<nnn>\tcompilation thread stack size in KB", TR::Options::setStaticNumeric,
     (intptr_t)&TR::Options::_stackSize, 0, "F%d", NOT_IN_SUBSET },
#if defined(J9VM_OPT_JITSERVER)
    { "statisticsFrequency=", "R<nnn>\tnumber of milliseconds between statistics print", TR::Options::setStaticNumeric,
     (intptr_t)&TR::Options::_statisticsFrequency, 0, "F%d", NOT_IN_SUBSET },
#endif  /* defined(J9VM_OPT_JITSERVER) */
    ...
    { "tlhPrefetchSize=", "O<nnn>\tallocation prefetch size for allocation prefetch", TR::Options::setStaticNumeric,
     (intptr_t)&TR::Options::_TLHPrefetchSize, 0, "P%d", NOT_IN_SUBSET },
    { "tlhPrefetchStaggeredLineCount=", "O<nnn>\tallocation prefetch staggered line for allocation prefetch",
     TR::Options::setStaticNumeric, (intptr_t)&TR::Options::_TLHPrefetchStaggeredLineCount, 0, "P%d",
     NOT_IN_SUBSET },
    ...
    { "tprof", "D\tgenerate time profiles with SWTRACE (requires -Xrunjprof12x:jita2n)", TR::Options::tprofOption, 0, 0,
     "F" },
    { "updateFreeMemoryMinPeriod=",
     "R<nnn>\tnumber of milliseconds after which point we will update the free physical memory available", TR::Options::setStaticNumeric, (intptr_t)&TR::Options::_updateFreeMemoryMinPeriod, 0, "F%d", NOT_IN_SUBSET },
    { "upperBoundNumProcForScaling=",
     "M<nnn>\tHigher than this numProc we'll use the conservativeScorchingSampleThreshold", TR::Options::setStaticNumeric, (intptr_t)&TR::Options::_upperBoundNumProcForScaling, 0, "F%d", NOT_IN_SUBSET },
    { "userClassLoadPhaseThreshold=",
     "O<nnn>\tnumber of user classes loaded per sampling tick that "
        "needs to be attained to enter the class loading phase. "
        "Specify a very large value to disable this optimization", TR::Options::setStaticNumeric, (intptr_t)&TR::Options::_userClassLoadingPhaseThreshold, 0, "P%d",
     NOT_IN_SUBSET },
    ...
    { "version", "L\tdisplay the jit build version", TR::Options::versionOption, 0, 0, "F" },
#if defined(J9VM_OPT_JITSERVER)
...
#endif  /* defined(J9VM_OPT_JITSERVER) */
    { "veryHotSampleThreshold=",
     "R<nnn>\tThe maximum number of global samples taken during a sample interval for which the method will be "
        "recompiled at hot with normal priority", TR::Options::setStaticNumeric, (intptr_t)&TR::Options::_veryHotSampleThreshold, 0, "F%d", NOT_IN_SUBSET },
    { "vlog=", "L<filename>\twrite verbose output to filename", TR::Options::setString,
     offsetof(J9JITConfig, vLogFileName), 0, "F%s" },
    { "vmState=", "L<vmState>\tdecode a given vmState", TR::Options::vmStateOption, 0, 0, "F" },
    { "waitTimeToEnterDeepIdleMode=",
     "M<nnn>\tTime spent in idle mode (ms) after which we enter deep idle mode sampling", TR::Options::setStaticNumeric, (intptr_t)&TR::Options::_waitTimeToEnterDeepIdleMode, 0, "F%d", NOT_IN_SUBSET },
    ...
    { "weightOfAOTLoad=", "M<nnn>\tWeight of an AOT load. 0 by default", TR::Options::setStaticNumeric,
     (intptr_t)&TR::Options::_weightOfAOTLoad, 0, "F%d", NOT_IN_SUBSET },
...
{
    if (this == TR::Options::getAOTCmdLineOptions() && self()->getOption(TR_NoLoadAOT)
        && self()->getOption(TR_NoStoreAOT))
        return false;
    else
...
static std::string readFileToString(char *fileName)
{
    ...
    if (iter <= MAX_FILE_SIZE_IN_PAGES) {
        ...
        j9tty_printf(PORTLIB, "Fatal Error: File (%s) is too large, max allowed size is %dKB\n", fileName,
...
{
    return postRestore ? FIND_ARG_IN_ARGS(vmArgsArray, J9::Options::getExternalOptionMatch(option),
                             J9::Options::getExternalOptionString(option), 0)
                       : J9::Options::getExternalOptionIndex(option);
}

bool J9::Options::JITServerParseCommonOptions(J9VMInitArgs *vmArgsArray, J9JavaVM *vm, TR::CompilationInfo *compInfo,
    bool postRestore)
{
    int32_t xxJITServerPortArgIndex
        = getArgIndex(vm, J9::ExternalOptions::XXJITServerPortOption, vmArgsArray, postRestore);
    int32_t xxJITServerTimeoutArgIndex
        = getArgIndex(vm, J9::ExternalOptions::XXJITServerTimeoutOption, vmArgsArray, postRestore);
    int32_t xxJITServerSSLKeyArgIndex
        = getArgIndex(vm, J9::ExternalOptions::XXJITServerSSLKeyOption, vmArgsArray, postRestore);
    int32_t xxJITServerSSLCertArgIndex
        = getArgIndex(vm, J9::ExternalOptions::XXJITServerSSLCertOption, vmArgsArray, postRestore);
    int32_t xxJITServerSSLRootCertsArgIndex
        = getArgIndex(vm, J9::ExternalOptions::XXJITServerSSLRootCertsOption, vmArgsArray, postRestore);
    int32_t xxJITServerUseAOTCacheArgIndex
        = getArgIndex(vm, J9::ExternalOptions::XXplusJITServerUseAOTCacheOption, vmArgsArray, postRestore);
    int32_t xxDisableJITServerUseAOTCacheArgIndex
        = getArgIndex(vm, J9::ExternalOptions::XXminusJITServerUseAOTCacheOption, vmArgsArray, postRestore);
    int32_t xxRequireJITServerArgIndex
        = getArgIndex(vm, J9::ExternalOptions::XXplusRequireJITServerOption, vmArgsArray, postRestore);
    int32_t xxDisableRequireJITServerArgIndex
        = getArgIndex(vm, J9::ExternalOptions::XXminusRequireJITServerOption, vmArgsArray, postRestore);
    int32_t xxJITServerLogConnectionsArgIndex
        = getArgIndex(vm, J9::ExternalOptions::XXplusJITServerLogConnections, vmArgsArray, postRestore);
    int32_t xxDisableJITServerLogConnectionsArgIndex
        = getArgIndex(vm, J9::ExternalOptions::XXminusJITServerLogConnections, vmArgsArray, postRestore);
    int32_t xxJITServerAOTmxArgIndex
        = getArgIndex(vm, J9::ExternalOptions::XXJITServerAOTmxOption, vmArgsArray, postRestore);

    if (xxJITServerPortArgIndex >= 0) {
        UDATA port = 0;
        const char *xxJITServerPortOption
            = J9::Options::getExternalOptionString(J9::ExternalOptions::XXJITServerPortOption);
    ...
    if (xxJITServerTimeoutArgIndex >= 0) {
        UDATA timeoutMs = 0;
        const char *xxJITServerTimeoutOption
            = J9::Options::getExternalOptionString(J9::ExternalOptions::XXJITServerTimeoutOption);
        IDATA ret
            = GET_INTEGER_VALUE_ARGS(vmArgsArray, xxJITServerTimeoutArgIndex, xxJITServerTimeoutOption, timeoutMs);
        if (ret == OPTION_OK)
    ...
    if (xxJITServerAOTmxArgIndex >= 0) {
        ...
        const char *xxJITServerAOTmxOption
            = J9::Options::getExternalOptionString(J9::ExternalOptions::XXJITServerAOTmxOption);
        if (GET_MEMORY_VALUE_ARGS(vmArgsArray, xxJITServerAOTmxArgIndex, xxJITServerAOTmxOption, aotMaxBytes)
            == OPTION_OK) {
...
void J9::Options::JITServerParseLocalSyncCompiles(J9VMInitArgs *vmArgsArray, J9JavaVM *vm,
    TR::CompilationInfo *compInfo, bool isFSDEnabled, bool postRestore)
{
    int32_t xxJITServerLocalSyncCompilesArgIndex
        = getArgIndex(vm, J9::ExternalOptions::XXplusJITServerLocalSyncCompilesOption, vmArgsArray, postRestore);
    int32_t xxDisableJITServerLocalSyncCompilesArgIndex
        = getArgIndex(vm, J9::ExternalOptions::XXminusJITServerLocalSyncCompilesOption, vmArgsArray, postRestore);
    ...
    if ((xxDisableJITServerLocalSyncCompilesArgIndex > xxJITServerLocalSyncCompilesArgIndex)
        || ((xxJITServerLocalSyncCompilesArgIndex < 0) && (xxDisableJITServerLocalSyncCompilesArgIndex < 0)
...
void J9::Options::preProcessMmf(J9JavaVM *vm, J9JITConfig *jitConfig)
{
    ...
    value = mmf->j9gc_modron_getConfigurationValueForKey(vm,
                j9gc_modron_configuration_heapBaseForBarrierRange0_isVariable, &value)
        ? value
        : 0;
    self()->setIsVariableHeapBaseForBarrierRange0(value);

    value = mmf->j9gc_modron_getConfigurationValueForKey(vm,
                j9gc_modron_configuration_heapSizeForBarrierRange0_isVariable, &value)
        ? value
        : 0;
    self()->setIsVariableHeapSizeForBarrierRange0(value);

    value = mmf->j9gc_modron_getConfigurationValueForKey(vm, j9gc_modron_configuration_activeCardTableBase_isVariable,
                &value)
        ? value
        : 0;
    self()->setIsVariableActiveCardTableBase(value);

    value = mmf->j9gc_modron_getConfigurationValueForKey(vm, j9gc_modron_configuration_heapAddressToCardAddressShift,
                &value)
        ? value
        : 0;
    ...
    // { RTSJ Support Begin
    value = mmf->j9gc_modron_getConfigurationValueForKey(vm, j9gc_modron_configuration_allocationType, &value) ? value
                                                                                                               : 0;
    ...
    if (_aggressivenessLevel == -1) // not yet set
    {
        ...
        if (J9_ARE_ANY_BITS_SET(jitConfig->runtimeFlags, J9JIT_QUICKSTART)) {
            ...
            if (argIndex >= 0) {
                ...
                const char *aggressiveOption
                    = J9::Options::getExternalOptionString(J9::ExternalOptions::XaggressivenessLevel);
...
double getCodeCacheMaxPercentageOfAvailableMemory(J9JavaVM *vm)
{
    ...
    int32_t XXcodeCacheTotalPercentArg
        = J9::Options::getExternalOptionIndex(J9::ExternalOptions::XXcodecachetotalMaxRAMPercentage);
    if (XXcodeCacheTotalPercentArg >= 0) {
        const char *xxccPercentOption
            = J9::Options::getExternalOptionString(J9::ExternalOptions::XXcodecachetotalMaxRAMPercentage);
        ...
        if (OPTION_OK == returnCode) {
            if (!(codeCacheTotalPercentage >= 1.0 && codeCacheTotalPercentage <= 100.0)) {
                j9nls_printf(PORTLIB, J9NLS_WARNING, J9NLS_JIT_OPTIONS_PERCENT_OUT_OF_RANGE, xxccPercentOption,
...
void J9::Options::preProcessCodeCacheIncreaseTotalSize(J9JavaVM *vm, J9JITConfig *jitConfig)
{
    ...
    if (!codecachetotalAlreadyParsed) // avoid processing twice for AOT and JIT and produce duplicate messages
    {
        ...
        if (freePhysicalMemoryB != 0) {
            ...
            uint64_t proposedCodeCacheTotalKB
                = ((uint64_t)(((double)freePhysicalMemoryB / 100.0) * getCodeCacheMaxPercentageOfAvailableMemory(vm)))
                >> 10;
        ...
        UDATA dcTotalSize
            = (double)ccTotalSize / (double)(jitConfig->codeCacheTotalKB) * (double)(jitConfig->dataCacheTotalKB);
...
void J9::Options::preProcessCodeCachePrintCodeCache(J9JavaVM *vm)
{
    ...
    int32_t xxDisablePrintCodeCacheArgIndex
        = J9::Options::getExternalOptionIndex(J9::ExternalOptions::XXminusPrintCodeCache);
...
bool J9::Options::preProcessCodeCacheXlpCodeCache(J9JavaVM *vm, J9JITConfig *jitConfig)
{
    ...
    // PPC supports -Xlp:codecache option.. since it's set via environment variables.  JVM should always request 4k
    // pages. fePreProcess is called twice - for AOT and JIT options parsing, which is redundant in terms of processing
    // the -Xlp:codecache options. We should parse the -Xlp:codecache options only once though to avoid duplicate NLS
    // messages.
    ...
    if (!parsedXlpCodeCacheOptions) {
        ...
        if (xlpCodeCacheIndex > xlpIndex) {
            ...
            while (optionsString < scanLimit) {
                if (try_scan(&optionsString, ",")) {
                    ...
                    switch (parsingState) {
                        ...
                        case XLPCC_PARSING_OPTION:
                            ...
                            break;
                        case XLPCC_PARSING_COMMA:
                            // can not find comma after option - so this is something unrecognizable at the end of known
                            // option
                ...
                if (XLPCC_PARSING_ERROR == parsingState) {
                    ...
                    j9nls_printf(PORTLIB, J9NLS_ERROR, J9NLS_JIT_OPTIONS_XLP_UNRECOGNIZED_OPTION,
                        xlpOptionErrorStringSize, xlpOptionErrorString);
                    return false;
                }
                ...
                if (try_scan(&optionsString, "pagesize=")) {
                    ...
                    if (0 != scanResult) {
                        ...
                        j9nls_printf(PORTLIB, J9NLS_ERROR, J9NLS_JIT_OPTIONS_INCORRECT_MEMORY_SIZE,
            ...
            if (0 == pageSizeHowMany) {
                j9nls_printf(PORTLIB, J9NLS_ERROR, J9NLS_JIT_OPTIONS_XLP_INCOMPLETE_OPTION,
            ...
            if ((0 == pageableHowMany) && (0 == nonPageableHowMany)) {
                ...
                j9nls_printf(PORTLIB, J9NLS_ERROR, J9NLS_JIT_OPTIONS_XLP_INCOMPLETE_OPTION, xlpOptionErrorString,
                    xlpMissingOptionString);
                return false;
            }
        ...
        // Parse Size -Xlp<size>
            ...
            // GET_MEMORY_VALUE macro casts it's second parameter to (char**)&, so a pointer to the option string is
            // passed rather than the string literal.
        ...
        if (requestedLargeCodePageSize != 0) {
            ...
            j9vmem_find_valid_page_size(J9PORT_VMEM_MEMORY_MODE_EXECUTE, &largePageSize, &largePageFlags,
            ...
            if (!isRequestedSizeSupported) {
                ...
                if (NULL == oldPageType) {
                    if (NULL == newPageType)
                        j9nls_printf(PORTLIB, J9NLS_INFO, J9NLS_JIT_OPTIONS_LARGE_PAGE_SIZE_NOT_SUPPORTED, oldSize,
                            oldQualifier, newSize, newQualifier);
                    else
                        j9nls_printf(PORTLIB, J9NLS_INFO,
                            J9NLS_JIT_OPTIONS_LARGE_PAGE_SIZE_NOT_SUPPORTED_WITH_NEW_PAGETYPE, oldSize, oldQualifier,
                            newSize, newQualifier, newPageType);
                } else {
                    if (NULL == newPageType)
                        j9nls_printf(PORTLIB, J9NLS_INFO,
                            J9NLS_JIT_OPTIONS_LARGE_PAGE_SIZE_NOT_SUPPORTED_WITH_REQUESTED_PAGETYPE, oldSize,
                            oldQualifier, oldPageType, newSize, newQualifier);
                    else
                        j9nls_printf(PORTLIB, J9NLS_INFO, J9NLS_JIT_OPTIONS_LARGE_PAGE_SIZE_NOT_SUPPORTED_WITH_PAGETYPE,
                            oldSize, oldQualifier, oldPageType, newSize, newQualifier, newPageType);
                }
            }
        }
...
void J9::Options::preProcessCompilationThreads(J9JavaVM *vm, J9JITConfig *jitConfig)
{
    ...
    if (notYetParsed) {
        ...
        if (argIndex >= 0) {
            ...
            const char *compThreadsOption
                = J9::Options::getExternalOptionString(J9::ExternalOptions::XcompilationThreads);
...
void J9::Options::preProcessTLHPrefetch(J9JavaVM *vm)
{
    ...
    preferTLHPrefetch = TR::Compiler->target.cpu.isAtLeast(OMR_PROCESSOR_PPC_P6)
        && TR::Compiler->target.cpu.isAtMost(OMR_PROCESSOR_PPC_P7);
...
void J9::Options::preProcessDeterministicMode(J9JavaVM *vm)
{
    ...
    if (TR::Options::_deterministicMode == -1) // not yet set
    {
        ...
        if (argIndex >= 0) {
            ...
            const char *deterministicOption
                = J9::Options::getExternalOptionString(J9::ExternalOptions::XXdeterministic);
...
bool J9::Options::preProcessJitServer(J9JavaVM *vm, J9JITConfig *jitConfig)
{
    ...
    if (!JITServerAlreadyParsed) // Avoid processing twice for AOT and JIT and produce duplicate messages
    {
        ...
        if (vm->internalVMFunctions->isJITServerEnabled(vm)) {
            ...
            int32_t xxEnableProbesArgIndex
                = J9::Options::getExternalOptionIndex(J9::ExternalOptions::XXplusHealthProbes);
            int32_t xxDisableProbesArgIndex
                = J9::Options::getExternalOptionIndex(J9::ExternalOptions::XXminusHealthProbes);
            if (xxEnableProbesArgIndex >= xxDisableProbesArgIndex) // probes are enabled by default
            {
                ...
                int32_t xxJITServerHealthPortArgIndex
                    = J9::Options::getExternalOptionIndex(J9::ExternalOptions::XXJITServerHealthProbePortOption);
                if (xxJITServerHealthPortArgIndex >= 0) {
                    UDATA port = 0;
                    const char *xxJITServerHealthPortOption
                        = J9::Options::getExternalOptionString(J9::ExternalOptions::XXJITServerHealthProbePortOption);
            ...
            int32_t xxEnableMetricsServerArgIndex
                = J9::Options::getExternalOptionIndex(J9::ExternalOptions::XXplusMetricsServer);
            int32_t xxDisableMetricsServerArgIndex
                = J9::Options::getExternalOptionIndex(J9::ExternalOptions::XXminusMetricsServer);
            if (xxEnableMetricsServerArgIndex > xxDisableMetricsServerArgIndex) {
                ...
                int32_t xxJITServerMetricsPortArgIndex
                    = J9::Options::getExternalOptionIndex(J9::ExternalOptions::XXJITServerMetricsPortOption);
                if (xxJITServerMetricsPortArgIndex >= 0) {
                    UDATA port = 0;
                    const char *xxJITServerMetricsPortOption
                        = J9::Options::getExternalOptionString(J9::ExternalOptions::XXJITServerMetricsPortOption);
                ...
                int32_t xxJITServerMetricsSSLKeyArgIndex
                    = J9::Options::getExternalOptionIndex(J9::ExternalOptions::XXJITServerMetricsSSLKeyOption);
                int32_t xxJITServerMetricsSSLCertArgIndex
                    = J9::Options::getExternalOptionIndex(J9::ExternalOptions::XXJITServerMetricsSSLCertOption);
            ...
            int32_t xxJITServerShareROMClassesArgIndex
                = J9::Options::getExternalOptionIndex(J9::ExternalOptions::XXplusJITServerShareROMClassesOption);
            int32_t xxDisableJITServerShareROMClassesArgIndex
                = J9::Options::getExternalOptionIndex(J9::ExternalOptions::XXminusJITServerShareROMClassesOption);
            ...
            int32_t xxJITServerAOTCachePersistenceArgIndex
                = J9::Options::getExternalOptionIndex(J9::ExternalOptions::XXplusJITServerAOTCachePersistenceOption);
            int32_t xxDisableJITServerAOTCachePersistenceArgIndex
                = J9::Options::getExternalOptionIndex(J9::ExternalOptions::XXminusJITServerAOTCachePersistenceOption);
            if (xxJITServerAOTCachePersistenceArgIndex > xxDisableJITServerAOTCachePersistenceArgIndex) {
                ...
                int32_t xxJITServerAOTCacheDirArgIndex
                    = J9::Options::getExternalOptionIndex(J9::ExternalOptions::XXJITServerAOTCacheDirOption);
        ...
        } else // Client mode (possibly)
        {
            ...
            int32_t xxUseJITServerArgIndex
                = J9::Options::getExternalOptionIndex(J9::ExternalOptions::XXplusUseJITServerOption);
            int32_t xxDisableUseJITServerArgIndex
                = J9::Options::getExternalOptionIndex(J9::ExternalOptions::XXminusUseJITServerOption);
            ...
            if (useJitServerExplicitlySpecified
                ...
                int32_t xxJITServerTechPreviewMessageArgIndex
                    = J9::Options::getExternalOptionIndex(J9::ExternalOptions::XXplusJITServerTechPreviewMessageOption);
                int32_t xxDisableJITServerTechPreviewMessageArgIndex = J9::Options::getExternalOptionIndex(
                    J9::ExternalOptions::XXminusJITServerTechPreviewMessageOption);
                ...
                int32_t xxJITServerAddressArgIndex
                    = J9::Options::getExternalOptionIndex(J9::ExternalOptions::XXJITServerAddressOption);
                ...
                int32_t xxJITServerAOTCacheNameArgIndex
                    = J9::Options::getExternalOptionIndex(J9::ExternalOptions::XXJITServerAOTCacheNameOption);
                ...
                int32_t xxJITServerAOTCacheDelayMethodRelocationArgIndex = J9::Options::getExternalOptionIndex(
                    J9::ExternalOptions::XXplusJITServerAOTCacheDelayMethodRelocation);
                int32_t xxDisableJITServerAOTCacheDelayMethodRelocationArgIndex = J9::Options::getExternalOptionIndex(
                    J9::ExternalOptions::XXminusJITServerAOTCacheDelayMethodRelocation);

                if (xxJITServerAOTCacheDelayMethodRelocationArgIndex
                    > xxDisableJITServerAOTCacheDelayMethodRelocationArgIndex) {
...
bool J9::Options::fePreProcess(void *base)
{
    J9JITConfig *jitConfig = (J9JITConfig *)base;
    ...
    if (xxLateSCCDisclaimTime >= 0) {
        UDATA disclaimMs = 0;
        const char *xxLateSCCDisclaimTimeOption
            = J9::Options::getExternalOptionString(J9::ExternalOptions::XXLateSCCDisclaimTimeOption);
    ...
    int32_t xxEnableTrackAOTDependenciesArgIndex
        = J9::Options::getExternalOptionIndex(J9::ExternalOptions::XXplusTrackAOTDependencies);
    int32_t xxDisableTrackAOTDependenciesArgIndex
        = J9::Options::getExternalOptionIndex(J9::ExternalOptions::XXminusTrackAOTDependencies);
...
void J9::Options::openLogFiles(J9JITConfig *jitConfig)
{
    char *vLogFileName = ((TR_JitPrivateConfig *)jitConfig->privateConfig)->vLogFileName;
    if (vLogFileName)
        ((TR_JitPrivateConfig *)jitConfig->privateConfig)->vLogFile
            = fileOpen(self(), jitConfig, vLogFileName, (char *)"wb", true);

    char *rtLogFileName = ((TR_JitPrivateConfig *)jitConfig->privateConfig)->rtLogFileName;
    if (rtLogFileName)
        ((TR_JitPrivateConfig *)jitConfig->privateConfig)->rtLogFile
            = fileOpen(self(), jitConfig, rtLogFileName, (char *)"wb", true);
}

#if defined(J9VM_OPT_JITSERVER)
void J9::Options::setupJITServerOptions()
{
    TR::CompilationInfo *compInfo = getCompilationInfo(jitConfig);
    ...
    if (compInfo->getPersistentInfo()->getRemoteCompilationMode() == JITServer::SERVER
        ...
        self()->setOption(
            TR_DisableProfiling); // JITServer limitation, JIT profiling data is not available to remote compiles yet
        ...
        // In the JITServer world, expensive compilations are performed remotely so there is no risk of blowing the
        // footprint limit on the JVM Setting _expensiveCompWeight to a large value so that JSR292/hot/scorching
        // compilation are allowed to be executed concurrently
    ...
    if (self()->getVerboseOption(TR_VerboseJITServer)) {
        ...
        if (persistentInfo->getRemoteCompilationMode() == JITServer::SERVER) {
            ...
            TR_VerboseLog::writeLineLocked(TR_Vlog_JITServer,
                "JITServer Server Mode. Port: %d. Connection Timeout %ums", persistentInfo->getJITServerPort(),
                persistentInfo->getSocketTimeout());
        } else if (persistentInfo->getRemoteCompilationMode() == JITServer::CLIENT) {
            JITServer::CommunicationStream::printJITServerVersion();
            TR_VerboseLog::writeLineLocked(TR_Vlog_JITServer,
                "JITServer Client Mode. Server address: %s port: %d. Connection Timeout %ums",
...
bool J9::Options::fePostProcessJIT(void *base)
{
    ...
    if (_numUsableCompilationThreads <= 0) {
        ...
        if (_numUsableCompilationThreads <= 0) {
            ...
            compInfo->updateNumUsableCompThreads(_numUsableCompilationThreads);
            _numUsableCompilationThreads = numOnlineCPUs > 1
                ? std::min((numOnlineCPUs - 1), static_cast<uint32_t>(_numUsableCompilationThreads))
                : 1;
        }
    }
...
bool J9::Options::disableMemoryDisclaimIfNeeded(J9JITConfig *jitConfig)
{
#if defined(LINUX)
    J9JavaVM *javaVM = jitConfig->javaVM;
    ...
    if (!omrsysinfo_os_kernel_info(&kernelInfo) || kernelInfo.kernelVersion < 5
        ...
        if (TR::Options::getVerboseOption(TR_VerbosePerformance)) {
            TR_VerboseLog::writeLineLocked(TR_Vlog_PERF,
                "WARNING: Disclaim feature disabled because either uname() failed or kernel version is not 5.4 or "
                "later");
        }
    }
    if (!shouldDisableMemoryDisclaim) {
        ...
        UDATA *pageSizes
            = j9vmem_supported_page_sizes(); // // Default page size is always the first element of this array
        if (pageSizes[0] > 4096) {
            shouldDisableMemoryDisclaim = true;
            if (TR::Options::getVerboseOption(TR_VerbosePerformance)) {
                TR_VerboseLog::writeLineLocked(TR_Vlog_PERF,
                    "WARNING: Disclaim feature disabled because default page size is larger than 4K");
            }
        }
    }
    if (!shouldDisableMemoryDisclaim) {
        ...
        if (!TR::Options::getCmdLineOptions()->getOption(TR_DontDisclaimMemoryOnSwap)
            ...
            J9MemoryInfo memInfo;
            if ((omrsysinfo_get_memory_info(&memInfo) == 0)
                && (memInfo.availSwap >= ((uint64_t)J9::Options::_minDiskSpaceForDisclaimMB << 20))) {
        ...
        if (retVal == 0 && statfsbuf.f_type != TMPFS_MAGIC && statfsbuf.f_type != RAMFS_MAGIC
            && statfsbuf.f_type != NFS_SUPER_MAGIC
            && ((uint64_t)statfsbuf.f_bavail * statfsbuf.f_bsize)
                >= ((uint64_t)J9::Options::_minDiskSpaceForDisclaimMB << 20)) {
            compInfo->setCanDisclaimOnFile(true);
            if (!compInfo->canDisclaimOnSwap() && TR::Options::getVerboseOption(TR_VerbosePerformance)) {
                TR_VerboseLog::writeLineLocked(TR_Vlog_PERF,
                    "Memory disclaim will be done on /tmp because swap is not suitable");
            }
        } else {
            if (TR::Options::getVerboseOption(TR_VerbosePerformance)) {
                TR_VerboseLog::writeLineLocked(TR_Vlog_PERF,
                    "WARNING: Disclaim feature disabled because swap and /tmp are not suitable");
            }
        }
    }
...
bool J9::Options::isFSDNeeded(J9JavaVM *javaVM, J9HookInterface **vmHooks)
{
#if defined(J9VM_OPT_CRIU_SUPPORT)
...
J9::Options::FSDInitStatus J9::Options::initializeFSDIfNeeded(J9JavaVM *javaVM, J9HookInterface **vmHooks, bool &doAOT)
{
...
bool J9::Options::feLatePostProcess(void *base, TR::OptionSet *optionSet)
{
   ...
   if ((TR::Options::getCmdLineOptions()->getOption(TR_ReportMethodEnter) ||
    ...
    // If the thresholds are bigger than the usableCompilationThreads, Then set them to be equal to the number of
    // compilation threads.
...
#define J9VM_DEBUG_ATTRIBUTE_MAINTAIN_FULL_INLINE_MAP 0
    ...
    if ((javaVM->requiredDebugAttributes & J9VM_DEBUG_ATTRIBUTE_CAN_REDEFINE_CLASSES)
        && !self()->getOption(TR_FullSpeedDebug))
    ...
    // First, there is confusion between the VM and the JIT as to whether a call to jitRetranslateCallerWithPreparation
    // is a decompilation point. Having the JIT agree with the VM could not be done by simply marking the symbol
    // canGCandReturn, as this would affect other parts of the JIT Second, in a sync method this call will occur before
    // the temporary sync object has been stored to its temp This results in the sync object not being available when
    // the VM calls the buffer filling code
    //
    if (self()->getOption(TR_FullSpeedDebug)) {
        ...
        // the following 2 opts might insert async checks at new bytecode index where the OSR infrastructures doesn't
        // exist
    ...
    if (TR::Options::sharedClassCache()) {
...
#endif /* defined(J9VM_OPT_CRIU_SUPPORT) */
        {
            ...
            if (!self()->getOption(TR_DisablePersistIProfile)
                && J9_ARE_ALL_BITS_SET(javaVM->sharedClassConfig->runtimeFlags,
    ...
    // during the startup phase
    {
        ...
        int32_t xxDisableIProfileDuringStartupPhaseArgIndex
            = J9::Options::getExternalOptionIndex(J9::ExternalOptions::XXminusIProfileDuringStartupPhase);
    ...
    //
    if ((TR::Options::getAOTCmdLineOptions()->getFixedOptLevel() != -1)
        && (TR::Options::getJITCmdLineOptions()->getFixedOptLevel() == -1))
        TR::Options::getJITCmdLineOptions()->setFixedOptLevel(TR::Options::getAOTCmdLineOptions()->getFixedOptLevel());
    if ((TR::Options::getJITCmdLineOptions()->getFixedOptLevel() != -1)
        && (TR::Options::getAOTCmdLineOptions()->getFixedOptLevel() == -1))
...
static void unpackRegex(TR::SimpleRegex *&regexPtr)
{
    if (!regexPtr)
        return;
...
static void addRegexStringSize(TR::SimpleRegex *regexPtr, size_t &len)
{
    if (regexPtr)
...
TR::Options *J9::Options::unpackOptions(char *clientOptions, size_t clientOptionsSize,
...
#if defined(J9VM_OPT_CRIU_SUPPORT)
void J9::Options::setFSDOptions(bool flag)
{
...
void J9::Options::setFSDOptionsForAll(bool flag)
{
    setFSDOptions(flag);
...
J9::Options::FSDInitStatus J9::Options::resetFSD(J9JavaVM *vm, J9VMThread *vmThread, bool &doAOT)
{
    ...
    if (fsdStatusJIT == TR::Options::FSDInitStatus::FSDInit_NotInitialized
        && !vm->internalVMFunctions->isCheckpointAllowed(vm) && vm->internalVMFunctions->isDebugOnRestoreEnabled(vm)) {
