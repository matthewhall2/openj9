...
static size_t getPaddedUTF8Size(size_t length) { return OMR::alignNoCheck(sizeof(J9UTF8) + length, sizeof(J9UTF8)); }

static size_t getPaddedUTF8Size(const J9UTF8 *str) { return getPaddedUTF8Size(J9UTF8_LENGTH(str)); }
...
static size_t copyUTF8(J9UTF8 *dst, const J9UTF8 *src, size_t generatedPrefixLength = 0)
{
...
public:
    ...
    ROMSegmentMap(TR_Memory *trMemory, const J9ROMClass *romClass)
        ...
        , _segments(
              ...
              decltype(_segments)::allocator_type(trMemory->currentStackRegion()))
    {}
    ...
    void registerDebugInfoSegment(const J9ROMClass *romClass, const uint8_t *debugInfoSegmentStart,
...
void ROMSegmentMap::registerDebugInfoSegment(const J9ROMClass *romClass, const uint8_t *debugInfoSegmentStart,
    size_t debugInfoSegmentSize)
{
    ...
    TR_ASSERT(_segments.empty() || (_segments.back()._boundaryOffset < debugOffset),
        "Debug info addresses must be traversed in memory order");
...
size_t ROMSegmentMap::newOffsetFromOld(size_t oldOffset) const
{
    ...
    auto it = std::lower_bound(_segments.begin(), _segments.end(), find,
        [](Segment x, Segment y) { return x._boundaryOffset <= y._boundaryOffset; });
    ...
    if (it == _segments.end()) {
        ...
        // Because {0, 0} is the first element of _segments, if we found an upper boundary then it is necessarily not
        // the first element of _segments
...
struct ROMClassPackContext {
    ROMClassPackContext(TR_Memory *trMemory, const J9ROMClass *romClass, size_t origSize, bool noDebugInfoStripping,
        const J9UTF8 *className, size_t generatedPrefixLength)
        : _segmentMap(trMemory, romClass)
        , _origRomClassStart((const uint8_t *)romClass)
        , _noDebugInfoStripping(noDebugInfoStripping)
        , _origSize(origSize)
        , _className(className)
        , _generatedPrefixLength(generatedPrefixLength)
        , _callback(NULL)
        , _preStringSize(0)
        , _stringsSize(0)
        , _origUtf8SectionStart((const uint8_t *)-1)
        , _origUtf8SectionEnd(NULL)
        , _utf8SectionSize(0)
        , _strToOffsetMap(decltype(_strToOffsetMap)::allocator_type(trMemory->currentStackRegion()))
        , _srpCallback(NULL)
        , _wsrpCallback(NULL)
        , _newUtf8SectionStart((const uint8_t *)-1)
        , _packedRomClass(NULL)
        , _cursor(NULL)
    {}
...
const uint8_t *ROMClassPackContext::newAddressFromOld(const uint8_t *oldAddr) const
{
    TR_ASSERT(isInline(oldAddr, (const J9ROMClass *)_origRomClassStart),
        "Address must be within the original ROM class");
...
static bool shouldSkipSlot(const char *slotName)
{
...
static void sizeInfoCallback(const J9ROMClass *romClass, const J9SRP *origSrp, const char *slotName,
    ROMClassPackContext &ctx)
{
    ...
    auto result = ctx._strToOffsetMap.insert({
        str, { skip ? (size_t)-1 : ctx._stringsSize, truncate }
    });
    if (!result.second) {
...
static void packCallback(const J9ROMClass *romClass, const J9SRP *origSrp, const char *slotName,
    ROMClassPackContext &ctx)
{
    ...
    if (shouldSkipSlot(slotName)) {
        TR_ASSERT(ctx._strToOffsetMap.find(str) != ctx._strToOffsetMap.end(), "UTF8 slot %s not visited in 1st pass",
            slotName);
        *srp = 0;
        return;
    }
    ...
    if (dst == ctx._cursor)
        ...
        TR_ASSERT((dst < ctx._cursor)
                && ((it->second.second
                        && (memcmp(utf8Data((J9UTF8 *)dst), utf8Data((J9UTF8 *)str), ctx._generatedPrefixLength) == 0))
                    || (!it->second.second && (memcmp(dst, str, J9UTF8_TOTAL_SIZE(str)) == 0))),
            "Must be already copied");
}

static void utf8SlotCallback(const J9ROMClass *romClass, const J9SRP *srp, const char *slotName, void *userData)
{
...
static void adjustSRPCallback(const J9ROMClass *romClass, const J9SRP *origSrp, const char *slotName,
    ROMClassPackContext &ctx)
{
    if (!ctx.isInline(origSrp, romClass))
        return;
...
static void adjustWSRPCallback(const J9ROMClass *romClass, const J9WSRP *origSrp, const char *slotName,
    ROMClassPackContext &ctx)
{
    if (!ctx.isInline(origSrp, romClass))
        return;
...
static void srpSlotCallback(const J9ROMClass *romClass, const J9SRP *srp, const char *slotName, void *userData)
{
    auto &ctx = *(ROMClassPackContext *)userData;
...
static void wsrpSlotCallback(const J9ROMClass *romClass, const J9WSRP *srp, const char *slotName, void *userData)
{
    auto &ctx = *(ROMClassPackContext *)userData;
...
static void slotCallback(J9ROMClass *romClass, uint32_t slotType, void *slotPtr, const char *slotName, void *userData)
{
    switch (slotType) {
        case J9ROM_SRP:
...
static bool isArrayROMClass(const J9ROMClass *romClass)
{
    if (!J9ROMCLASS_IS_ARRAY(romClass))
        return false;

    auto name = J9ROMCLASS_CLASSNAME(romClass);
    TR_ASSERT((name->length == 2) && (name->data[0] == '['), "Unexpected array ROMClass name: %.*s", name->length,
        name->data);
    return true;
}
...
static size_t getArrayROMClassPackedSize(const J9ROMClass *romClass)
{
...
static void packUTF8(const J9UTF8 *str, J9SRP &srp, ROMClassPackContext &ctx)
{
...
static void packArrayROMClassData(const J9ROMClass *romClass, ROMClassPackContext &ctx)
{
...
static void sectionEndCallback(J9ROMClass *romClass, void *sectionPtr, uintptr_t sectionSize, const char *sectionName,
    void *userData)
{
    auto ctx = (ROMClassPackContext *)userData;
    // Important ROM class walker properties that allow us to calculate the _preStringSize in parallel with the UTF8
    // string section start:
    //
    // 1. The ROM class walker only visits sections that are entirely within the pre-string section or entirely outside
    // the ROM class
    // 2. The sections entirely cover the pre-string section, up to but not including any padding that was added during
    // writing
    //
    // That the _preStringSize ends up being where we expect it to be (equal to the starting offset of the UTF8 string
    // section up to alignment padding) is checked in a fatal assert when we calculate the packedNonStringSize in
    // packROMClass.
...
//
J9ROMClass *JITServerHelpers::packROMClass(const J9ROMClass *romClass, TR_Memory *trMemory, TR_J9VMBase *fej9,
    size_t &packedSize, size_t expectedSize, size_t generatedPrefixLength)
{
    ...
    bool isSharedROMClass
        = !alreadyPacked && fej9->sharedCache() && fej9->sharedCache()->isROMClassInSharedCache((J9ROMClass *)romClass);
    ...
    if (!alreadyPacked) {
        ...
        if (JITServerHelpers::isAddressInROMClass(icData, romClass) && (icData != (const uint8_t *)romClass)) {
            TR_ASSERT_FATAL(icData + romClass->intermediateClassDataLength
                    == (const uint8_t *)romClass + romClass->romSize,
    ...
    if (isArrayROMClass(romClass)) {
        ...
        allSlotsInROMClassDo((J9ROMClass *)romClass, slotCallback, needsDebugInfoStripping ? sectionEndCallback : NULL,
            NULL, &ctx);
        ...
        TR_ASSERT_FATAL(ctx._utf8SectionSize == end - ctx._origUtf8SectionStart,
            "Missed strings in ROMClass %.*s UTF8 section: %zu != %zu", name->length, name->data, ctx._utf8SectionSize,
            end - ctx._origUtf8SectionStart);
        end = (const uint8_t *)OMR::alignNoCheck((uintptr_t)end, sizeof(uint64_t));
        TR_ASSERT_FATAL(end == classEnd, "UTF8 section not stored at the end of ROMClass %.*s: %p != %p", name->length,
            name->data, end, classEnd);
        ...
        if (needsDebugInfoStripping) {
            ...
            TR_ASSERT_FATAL(OMR::alignNoCheck(ctx._preStringSize, sizeof(uint64_t))
            ...
            packedNonStringSize
                = OMR::alignNoCheck(ctx._preStringSize - ctx._segmentMap.removedDebugInfoSize(), sizeof(uint64_t));
    ...
    if (needsDebugInfoStripping) {
        ...
        // We will have recorded k+1 starts of non-debug info segments, interspersed with the starts of k debug info
        // segments. (k can be zero).
        ...
        memcpy((uint8_t *)ctx._packedRomClass + finalSegment.newBoundaryOffset(),
            (uint8_t *)romClass + finalSegment._boundaryOffset, finalCopySize);
    ...
    if (!alreadyPacked && !needsDebugInfoStripping) {
        ...
        for (size_t i = 0; i < ctx._packedRomClass->romMethodCount; ++i) {
            if (J9ROMMETHOD_HAS_DEBUG_INFO(romMethod)) {
                auto debugInfo = methodDebugInfoFromROMMethod(romMethod);
                TR_ASSERT_FATAL(!(debugInfo->srpToVarInfo & 1),
                    "ROM class %.*s has inline debug info that should have been stripped", name->length, name->data);
    ...
    TR_ASSERT_FATAL(end == (uint8_t *)ctx._packedRomClass + packedSize, "Invalid final cursor position: %p != %p", end,
        (uint8_t *)ctx._packedRomClass + packedSize);
...
void JITServerHelpers::insertIntoOOSequenceEntryList(ClientSessionData *clientData, TR_MethodToBeCompiled *entry)
{
...
void JITServerHelpers::printJITServerMsgStats(J9JITConfig *jitConfig, TR::CompilationInfo *compInfo)
{
    ...
    for (int i = 0; i < JITServer::MessageType_MAXTYPE; ++i) {
        if (JITServer::CommunicationStream::_msgTypeCount[i]) {
            ...
            j9tty_printf(PORTLIB, "\t%f\t%f\t%f\t%f\t%f", stat.maxVal(), stat.minVal(), stat.mean(), stat.stddev(),
                stat.sum());
#endif /* defined(MESSAGE_SIZE_STATS) */
...
void JITServerHelpers::printJITServerCHTableStats(J9JITConfig *jitConfig, TR::CompilationInfo *compInfo)
{
#ifdef COLLECT_CHTABLE_STATS
    PORT_ACCESS_FROM_JITCONFIG(jitConfig);
    j9tty_printf(PORTLIB, "JITServer CHTable Statistics:\n");
    if (compInfo->getPersistentInfo()->getRemoteCompilationMode() == JITServer::CLIENT) {
        JITClientPersistentCHTable *table
            = (JITClientPersistentCHTable *)compInfo->getPersistentInfo()->getPersistentCHTable();
        j9tty_printf(PORTLIB, "Num updates sent: %d (1 per compilation)\n", table->_numUpdates);
        if (table->_numUpdates) {
            j9tty_printf(PORTLIB, "Num commit failures: %d. Average per compilation: %f\n", table->_numCommitFailures,
                table->_numCommitFailures / float(table->_numUpdates));
            j9tty_printf(PORTLIB, "Num classes updated: %d. Average per compilation: %f\n", table->_numClassesUpdated,
                table->_numClassesUpdated / float(table->_numUpdates));
            j9tty_printf(PORTLIB, "Num classes removed: %d. Average per compilation: %f\n", table->_numClassesRemoved,
                table->_numClassesRemoved / float(table->_numUpdates));
        ...
        JITServerPersistentCHTable *table
            = (JITServerPersistentCHTable *)compInfo->getPersistentInfo()->getPersistentCHTable();
        ...
        if (table->_numUpdates) {
            ...
            j9tty_printf(PORTLIB, "Num class info queries: %d. Average per compilation: %f\n", table->_numQueries,
                table->_numQueries / float(table->_numUpdates));
            j9tty_printf(PORTLIB, "Total update bytes: %d. Compilation max: %d. Average per compilation: %f\n",
                table->_updateBytes, table->_maxUpdateBytes, table->_updateBytes / float(table->_numUpdates));
        }
    }
...
void JITServerHelpers::printJITServerCacheStats(J9JITConfig *jitConfig, TR::CompilationInfo *compInfo)
{
    PORT_ACCESS_FROM_JITCONFIG(jitConfig);
 ...
 */
void JITServerHelpers::freeRemoteROMClass(J9ROMClass *romClass, TR_PersistentMemory *persistentMemory)
{
 ...
 */
J9ROMClass *JITServerHelpers::cacheRemoteROMClassOrFreeIt(ClientSessionData *clientSessionData, J9Class *clazz,
...
ClientSessionData::ClassInfo &JITServerHelpers::cacheRemoteROMClass(ClientSessionData *clientSessionData,
    J9Class *clazz, J9ROMClass *romClass, const ClassInfoTuple &classInfoTuple)
{
    ...
    classInfoStruct._interfaces
        = new (persistentAllocator) PersistentVector<TR_OpaqueClassBlock *>(tmpInterfaces.begin(), tmpInterfaces.end(),
...
J9ROMClass *JITServerHelpers::getRemoteROMClassIfCached(ClientSessionData *clientSessionData, J9Class *clazz)
{
...
JITServerHelpers::ClassInfoTuple JITServerHelpers::packRemoteROMClassInfo(J9Class *clazz, J9VMThread *vmThread,
    TR_Memory *trMemory, bool serializeClass)
{
    ...
    for (uint32_t i = 0; i < numMethods; ++i) {
        methodTracingInfo.push_back(
            static_cast<uint8_t>(fe->isMethodTracingEnabled((TR_OpaqueMethodBlock *)&methodsOfClass[i])));
    ...
    uintptr_t classChainOffsetIdentifyingLoader = sharedCache
        ? sharedCache->getClassChainOffsetIdentifyingLoaderNoFail((TR_OpaqueClassBlock *)clazz,
    ...
    if (fe->getPersistentInfo()->getJITServerUseAOTCache()
        ...
        auto nameInfo
            = fe->getPersistentInfo()->getPersistentClassLoaderTable()->lookupClassNameAssociatedWithClassLoader(
                loader);
        if (nameInfo) {
    ...
    return std::make_tuple(packedROMClassStr, methodsOfClass, baseClass, numDims, parentClass,
        TR::Compiler->cls.getITable((TR_OpaqueClassBlock *)clazz), methodTracingInfo, classHasFinalFields,
...
J9ROMClass *JITServerHelpers::getRemoteROMClass(J9Class *clazz, JITServer::ServerStream *stream,
    TR_PersistentMemory *persistentMemory, ClassInfoTuple &classInfoTuple)
{
    ...
    return romClassFromString(std::get<0>(classInfoTuple), std::get<24>(classInfoTuple), persistentMemory);
}

// Return true if able to get data from cache, return false otherwise.
bool JITServerHelpers::getAndCacheRAMClassInfo(J9Class *clazz, ClientSessionData *clientSessionData,
    JITServer::ServerStream *stream, ClassInfoDataType dataType, void *data)
{
    ...
    auto it = clientSessionData->getROMClassMap().find(clazz);
    if (it == clientSessionData->getROMClassMap().end()) {
        auto romClass = romClassFromString(std::get<0>(classInfoTuple), std::get<24>(classInfoTuple),
            clientSessionData->persistentMemory());
        auto &classInfoStruct
            = JITServerHelpers::cacheRemoteROMClass(clientSessionData, clazz, romClass, classInfoTuple);
...
// Return true if able to get data from cache, return false otherwise.
bool JITServerHelpers::getAndCacheRAMClassInfo(J9Class *clazz, ClientSessionData *clientSessionData,
    JITServer::ServerStream *stream, ClassInfoDataType dataType1, void *data1, ClassInfoDataType dataType2, void *data2)
{
    if (!clazz)
        return false;
...
void JITServerHelpers::getROMClassData(const ClientSessionData::ClassInfo &classInfo, ClassInfoDataType dataType,
    void *data)
{
    switch (dataType) {
...
ClientSessionData::ClassInfo &JITServerHelpers::getJ9ClassInfo(TR::CompilationInfoPerThread *threadCompInfo,
    J9Class *clazz)
{
    ...
    TR_ASSERT_FATAL(it != classMap.end(),
        "compThreadID %d, ClientData %p, clazz %p: ClassInfo is not in the class map %p!!\n",
        threadCompInfo->getCompThreadId(), threadCompInfo->getClientData(), clazz, &classMap);
    return it->second;
}

J9ROMMethod *JITServerHelpers::romMethodOfRamMethod(J9Method *method)
{
    // JITServer
...
void JITServerHelpers::postStreamFailure(OMRPortLibrary *portLibrary, TR::CompilationInfo *compInfo,
    bool retryConnectionImmediately, bool connectionFailure)
{
        ...
        if (TR::Options::getCmdLineOptions()->getVerboseOption(TR_VerboseJITServerConns)) {
            if (compInfo->getPersistentInfo()->getServerUID() != 0) {
                TR_VerboseLog::writeLineLocked(TR_Vlog_JITServer,
                    ...
                    (unsigned long long)compInfo->getPersistentInfo()->getServerUID(), _waitTimeMs);
            } else // Was not connected to a server
            {
                TR_VerboseLog::writeLineLocked(TR_Vlog_JITServer,
                    "t=%6u Could not connect to a server. Next attempt in %llu ms.",
                    (uint32_t)compInfo->getPersistentInfo()->getElapsedTime(), _waitTimeMs);
            }
        }
...
void JITServerHelpers::postStreamConnectionSuccess()
{
    _serverAvailable = true;
    _waitTimeMs = TR::Options::_reconnectWaitTimeMs;
}

bool JITServerHelpers::shouldRetryConnection(OMRPortLibrary *portLibrary)
{
    OMRPORT_ACCESS_FROM_OMRPORT(portLibrary);
    return omrtime_current_time_millis() >= _nextConnectionRetryTime;
}

bool JITServerHelpers::isAddressInROMClass(const void *address, const J9ROMClass *romClass)
{
    return ((address >= romClass) && (address < (((uint8_t *)romClass) + romClass->romSize)));
}

uintptr_t JITServerHelpers::walkReferenceChainWithOffsets(TR_J9VM *fe, const std::vector<uintptr_t> &listOfOffsets,
    uintptr_t receiver)
{
...
uintptr_t JITServerHelpers::getRemoteClassDepthAndFlagsWhenROMClassNotCached(J9Class *clazz,
    ClientSessionData *clientSessionData, JITServer::ServerStream *stream)
{
    stream->write(JITServer::MessageType::ResolvedMethod_getRemoteROMClassAndMethods, clazz);
    ...
    auto it = clientSessionData->getROMClassMap().find(clazz);
    if (it == clientSessionData->getROMClassMap().end()) {
        auto romClass = JITServerHelpers::romClassFromString(std::get<0>(classInfoTuple), std::get<24>(classInfoTuple),
            clientSessionData->persistentMemory());
        auto &classInfoStruct
            = JITServerHelpers::cacheRemoteROMClass(clientSessionData, clazz, romClass, classInfoTuple);
...
static void addRAMClassToChain(std::vector<J9Class *> &chain, J9Class *clazz, std::vector<J9Class *> &uncached,
...
std::vector<J9Class *> JITServerHelpers::getRAMClassChain(J9Class *clazz, size_t numClasses, J9VMThread *vmThread,
    TR_Memory *trMemory, TR::CompilationInfo *compInfo, std::vector<J9Class *> &uncachedRAMClasses,
...
void JITServerHelpers::cacheRemoteROMClassBatch(ClientSessionData *clientData, const std::vector<J9Class *> &ramClasses,
    const std::vector<ClassInfoTuple> &classInfoTuples)
{
    ...
    for (size_t i = 0; i < ramClasses.size(); ++i) {
        auto romClass = romClassFromString(std::get<0>(classInfoTuples[i]), std::get<24>(classInfoTuples[i]),
            clientData->persistentMemory());
        cacheRemoteROMClassOrFreeIt(clientData, ramClasses[i], romClass, classInfoTuples[i]);
    }
}

uint64_t JITServerHelpers::generateUID()
{
...
uint32_t JITServerHelpers::getFullClassNameLength(const J9ROMClass *romClass, const J9ROMClass *baseComponent,
...
void JITServerHelpers::getFullClassName(uint8_t *name, uint32_t length, const J9ROMClass *romClass,
    const J9ROMClass *baseComponent, uint32_t numDimensions, bool checkGenerated)
{
    TR_ASSERT(length == getFullClassNameLength(romClass, baseComponent, numDimensions, checkGenerated),
        "Invalid length");
...
size_t JITServerHelpers::getGeneratedClassNamePrefixLength(const J9UTF8 *name)
{
    ...
    if ((nameLen > accessorPrefixStartLen + accessorPrefixEndLen)
        ...
        const char *kindEnd = std::search(nameStr + accessorPrefixStartLen + 1, nameStr + nameLen, accessorPrefixEnd,
            accessorPrefixEnd + accessorPrefixEndLen);
