...
#define FIND_AND_CONSUME_RESTORE_ARG(match, optionName, optionValue) \
    FIND_AND_CONSUME_ARG(vm->checkpointState.restoreArgsList, match, optionName, optionValue)
#define FIND_ARG_IN_RESTORE_ARGS(match, optionName, optionValue) \
    FIND_ARG_IN_ARGS(vm->checkpointState.restoreArgsList, match, optionName, optionValue)
#define GET_INTEGER_VALUE_RESTORE_ARGS(element, optname, result) \
    GET_INTEGER_VALUE_ARGS(vm->checkpointState.restoreArgsList, element, optname, result)
#define GET_OPTION_VALUE_RESTORE_ARGS(element, delimChar, resultPtr) \
    GET_OPTION_VALUE_ARGS(vm->checkpointState.restoreArgsList, element, delimChar, resultPtr)

J9::OptionsPostRestore::OptionsPostRestore(J9VMThread *vmThread, J9JITConfig *jitConfig, TR::CompilationInfo *compInfo,
    TR::Region &region)
...
void J9::OptionsPostRestore::processJitServerOptions()
{
#if defined(J9VM_OPT_JITSERVER)
    bool jitserverEnabled = ((_argIndexUseJITServer > _argIndexDisableUseJITServer)
    ...
    if (jitserverEnabled) {
        ...
        TR::Options::JITServerParseLocalSyncCompiles(vm->checkpointState.restoreArgsList, vm, _compInfo,
            TR::Options::getCmdLineOptions()->getOption(TR_FullSpeedDebug), true);
...
void J9::OptionsPostRestore::processInternalCompilerOptions(bool isAOT)
{
    ...
    int32_t argIndex;
    if (isAOT)
        argIndex = FIND_ARG_IN_RESTORE_ARGS(STARTSWITH_MATCH,
            J9::Options::getExternalOptionString(J9::ExternalOptions::Xaotcolon), 0);
    else
        argIndex = FIND_ARG_IN_RESTORE_ARGS(STARTSWITH_MATCH,
            J9::Options::getExternalOptionString(J9::ExternalOptions::Xjitcolon), 0);

    if (argIndex >= 0) {
        intptr_t rc = initializeCompilerArgsPostRestore(_jitConfig->javaVM, argIndex, &commandLineOptions, !isAOT,
    ...
    if (commandLineOptions) {
        const char *remainder
            = TR::Options::processOptionSetPostRestore(_jitConfig, commandLineOptions, options, isAOT);
        if (*remainder) {
            // TODO: Error condition
        }
    }
...
void J9::OptionsPostRestore::invalidateCompiledMethod(J9Method *method, TR_J9VMBase *fej9)
{
    ...
    if (bodyInfo) {
        ...
        if (TR::Options::getCmdLineOptions()->getVerboseOption(TR_VerboseCheckpointRestoreDetails)) {
            TR_VerboseLog::CriticalSection();
            TR_VerboseLog::write(TR_Vlog_CHECKPOINT_RESTORE, "Unable to invalidate %smethod ",
                isNative ? "native " : "");
            _compInfo->printMethodNameToVlog(method);
            TR_VerboseLog::writeLine(" (%p)", method);
        }
...
bool J9::OptionsPostRestore::shouldInvalidateCompiledMethod(J9Method *method, TR_J9VMBase *fej9,
    bool compilationFiltersExist)
{
...
void J9::OptionsPostRestore::invalidateCompiledMethodsIfNeeded(bool invalidateAll)
{
        ...
        while (j9clazz) {
            ...
            for (uint32_t index = 0; index < numMethods; index++) {
                ...
                if (TR::CompilationInfo::isCompiled(method)) {
                    if (invalidateAll || shouldInvalidateCompiledMethod(method, fej9, compilationFiltersExist)) {
...
void J9::OptionsPostRestore::disableAOTCompilation(bool disabledPreCheckpoint)
{
...
void J9::OptionsPostRestore::openNewVlog(char *vLogFileName)
{
    TR_VerboseLog::vlogAcquire();

    if (_oldVLogFileName) {
        TR_ASSERT_FATAL(vLogFileName, "vlogFileName cannot be NULL if _oldVLogFileName (%s) is not NULL\n",
            _oldVLogFileName);
...
void J9::OptionsPostRestore::openNewRTLog(char *rtLogFileName)
{
    ...
    if (closeOldFile) {
        TR_ASSERT_FATAL(rtLogFileName, "rtLogFileName cannot be NULL if _oldRtLogFileName (%s) is not NULL\n",
            _oldRtLogFileName);
...
void J9::OptionsPostRestore::openLogFilesIfNeeded()
{
...
void J9::OptionsPostRestore::preProcessInternalCompilerOptions()
{
    // Needed for FIND_AND_CONSUME_RESTORE_ARG
    J9JavaVM *vm = _jitConfig->javaVM;
    ...
    _argIndexMergeOptionsEnabled = FIND_AND_CONSUME_RESTORE_ARG(EXACT_MATCH,
...
void J9::OptionsPostRestore::postProcessInternalCompilerOptions()
{
    ...
    bool exceptionCatchEventHooked = J9_EVENT_IS_HOOKED(vm->hookInterface, J9HOOK_VM_EXCEPTION_CATCH)
        || J9_EVENT_IS_RESERVED(vm->hookInterface, J9HOOK_VM_EXCEPTION_CATCH);
    bool exceptionThrowEventHooked = J9_EVENT_IS_HOOKED(vm->hookInterface, J9HOOK_VM_EXCEPTION_THROW)
...
J9:
        Options::_xrsSync = J9_ARE_ALL_BITS_SET(vm->sigFlags, J9_SIG_XRS_SYNC);
        invalidateAll = true;
        disableAOT = true;
    ...
    if (!_compInfo->getCRRuntime()->isFSDEnabled() && fsdStatus == TR::Options::FSDInitStatus::FSDInit_Initialized) {
    ...
    if ((_argIndexIProfileDuringStartupPhase >= 0) || (_argIndexDisableIProfileDuringStartupPhase >= 0)) {
        bool IProfileDuringStartupPhase
        ...
        if (!TR::Options::getCmdLineOptions()->getOption(TR_DisablePersistIProfile)
            && J9_ARE_ALL_BITS_SET(vm->sharedClassConfig->runtimeFlags,
...
void J9::OptionsPostRestore::processCompilerOptions()
{
    // Needed for FIND_AND_CONSUME_RESTORE_ARG
    J9JavaVM *vm = _jitConfig->javaVM;
    PORT_ACCESS_FROM_JAVAVM(vm);

    bool aotEnabledPreCheckpoint
        = TR_J9SharedCache::aotHeaderValidationDelayed() ? true : TR::Options::sharedClassCache();

    bool jitEnabled = TR::Options::canJITCompile();
    bool aotEnabled = aotEnabledPreCheckpoint;

    _argIndexXjit = FIND_AND_CONSUME_RESTORE_ARG(OPTIONAL_LIST_MATCH,
        J9::Options::getExternalOptionString(J9::ExternalOptions::Xjit), 0);
    _argIndexXnojit = FIND_AND_CONSUME_RESTORE_ARG(OPTIONAL_LIST_MATCH,
        J9::Options::getExternalOptionString(J9::ExternalOptions::Xnojit), 0);
    _argIndexXaot = FIND_AND_CONSUME_RESTORE_ARG(OPTIONAL_LIST_MATCH,
        J9::Options::getExternalOptionString(J9::ExternalOptions::Xaot), 0);
    _argIndexXnoaot = FIND_AND_CONSUME_RESTORE_ARG(OPTIONAL_LIST_MATCH,
        J9::Options::getExternalOptionString(J9::ExternalOptions::Xnoaot), 0);
...
void J9::OptionsPostRestore::processOptionsPostRestore(J9VMThread *vmThread, J9JITConfig *jitConfig,
    TR::CompilationInfo *compInfo)
{
    try {
        ...
        J9::OptionsPostRestore optsPostRestore
            = J9::OptionsPostRestore(vmThread, jitConfig, compInfo, postRestoreOptionsRegion);
