...
struct CompileParameters {
    CompileParameters(TR::CompilationInfoPerThreadBase *compilationInfo, TR_J9VMBase *vm, J9VMThread *vmThread,
        ...
        TR::SegmentAllocator &scratchSegmentProvider, TR::Region &dispatchRegion, TR_Memory &trMemory,
        const TR::CompileIlGenRequest &ilGenRequest, bool checkpointInProgress
#if defined(J9VM_OPT_JITSERVER)
        ,
...
public:
    CompilationInfoPerThreadBase(TR::CompilationInfo &compInfo, J9JITConfig *jitConfig, int32_t id,
        bool onSeparateThread);
    ...
    TR_MethodMetaData *compile(J9VMThread *context, TR::Compilation *, TR_ResolvedMethod *compilee, TR_J9VMBase &,
        TR_OptimizationPlan *, TR::SegmentAllocator const &scratchSegmentProvider);
    TR_MethodMetaData *performAOTLoad(J9VMThread *context, TR::Compilation *, TR_ResolvedMethod *compilee,
        TR_J9VMBase *vm, J9Method *method);
    ...
    TR_MethodMetaData *installAotCachedMethod(J9VMThread *vmThread, const void *aotCachedMethod, J9Method *method,
        TR_FrontEnd *fe, TR::Options *options, TR_ResolvedMethod *compilee, TR_MethodToBeCompiled *entry,
        TR::Compilation *compiler);
#endif

    /*
     * LdTM: This should, pedantically speaking, be an 'extern "C"' friend function rather than a static member function
     * (with C++ linkage). However a brief search reveals that such an approach is fraught with its own set of
     * compiler-bug-related risk.
     */
     ...
     * \details
     *
     * Interruptible (\see InterruptibleOperation) and Uninterruptible (\see UninterruptibleOperation) operations
     * related classes expressing whether the current compilation thread can terminate the compilation at the next yield
     * point. The two classes can be intertwined and the logic of what happens when scopes mix is as follows:
     *
    ...
    public:
        UninterruptibleOperation(TR::CompilationInfoPerThreadBase &compThread)
            : _compThread(compThread)
            , _originalValue(_compThread._compilationCanBeInterrupted)
        {
            _compThread._compilationCanBeInterrupted = false;
...
private:
    void logCompilationSuccess(J9VMThread *vmThread, TR_J9VMBase &vm, J9Method *method,
        const TR::SegmentAllocator &scratchSegmentProvider, TR_ResolvedMethod *compilee, TR::Compilation *compiler,
        TR_MethodMetaData *metadata, TR_OptimizationPlan *optimizationPlan);

    void processException(J9VMThread *vmThread, const TR::SegmentAllocator &scratchSegmentProvider,
        TR::Compilation *compiler, volatile bool &haveLockedClassUnloadMonitor, const char *exceptionName,
        J9JITExceptionTable *metaData) throw();
    ...
    void outputVerboseMMapEntries(TR_ResolvedMethod *compilee, TR_MethodMetaData *metaData, const char *profiledString,
...
public:
    CompilationInfoPerThread(TR::CompilationInfo &compInfo, J9JITConfig *jitConfig, int32_t id,
    ...
    PersistentUnorderedSet<TR_OpaqueClassBlock *> *getClassesThatShouldNotBeNewlyExtended() const
    {
