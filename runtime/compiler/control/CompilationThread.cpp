...
IDATA J9THREAD_PROC protectedCompilationThreadProc(J9PortLibrary *portLib,
    TR::CompilationInfoPerThread *compInfoPT /*void *vmthread*/);
...
TR_RelocationRuntime *TR::CompilationInfoPerThreadBase::reloRuntime()
{
#if defined(J9VM_OPT_JITSERVER)
    if (_methodBeingCompiled->isAotLoad()
        ...
        || (_compInfo.getPersistentInfo()->getRemoteCompilationMode() == JITServer::CLIENT
            && TR::Options::sharedClassCache())) // Enable local AOT compilations at client
...
void TR::CompilationInfoPerThreadBase::setCompilation(TR::Compilation *compiler)
{
#if defined(DEBUG) || defined(PROD_WITH_ASSUMES)
    TR::Compilation *tlsCompiler = TR::comp();
    // Exclude PPC LE from the following assert due to xlc compiler bug
    TR_ASSERT(tlsCompiler == compiler
            || (TR::Compiler->target.cpu.isPower() && TR::Compiler->target.cpu.isLittleEndian()),
        "TLS compilation object @ " POINTER_PRINTF_FORMAT
        " does not match provided compilation object @ " POINTER_PRINTF_FORMAT,
        tlsCompiler, compiler);
#endif
    _compiler = compiler;
}
...
static uintptr_t jitSignalHandler(struct J9PortLibrary *portLibrary, uint32_t gpType, void *gpInfo, void *handler_arg)
{
    ...
    if (!table || !comp || (table && !table->isThreadInSafeMonitorState(vmThread)))
        recoverable = false;
 ...
 *
 */
static int inflateBuffer(U_8 *buffer, int numberOfBytes, U_8 *outBuffer, int unCompressedSize)
{
 ...
 *
 */
static int deflateBuffer(const U_8 *buffer, int numberOfBytes, U_8 *outBuffer, int level)
{
    z_stream _stream;
    _stream.zalloc = Z_NULL;
    _stream.zfree = Z_NULL;
...
#endif
#endif
inline void TR::CompilationInfo::incrementMethodQueueSize()
{
    _numQueuedMethods++;
...
TR_MethodToBeCompiled *TR::CompilationInfo::getCompilationQueueEntry()
{
...
TR::CompilationInfoPerThread *TR::CompilationInfo::findFirstLowPriorityCompilationInProgress(
    CompilationPriority priority) // needs compilationQueueMonitor in hand
{
...
TR_YesNoMaybe TR::CompilationInfo::shouldActivateNewCompThread()
{
    ...
    if (getNumCompThreadsActive() > 0 && javaVM->internalVMFunctions->isDebugOnRestoreEnabled(javaVM)
    ...
    if (freePhysicalMemorySizeB != OMRPORT_MEMINFO_NOT_AVAILABLE
        && freePhysicalMemorySizeB
            <= (uint64_t)TR::Options::getSafeReservePhysicalMemoryValue() + TR::Options::getScratchSpaceLowerBound())
        return TR_no;
    ...
    if (TR::Options::sharedClassCache() && !TR::Options::getCmdLineOptions()->isQuickstartDetected() && !isSecondAOTRun
...
bool TR::CompilationInfo::isMethodIneligibleForAot(J9Method *method, TR_Memory *trMemory, TR_J9VMBase *fe)
{
    ...
    if (J9UTF8_LENGTH(className) == 36
        ...
        if (J9UTF8_LENGTH(methodName) == 21 && 0 == memcmp(J9UTF8_DATA(methodName), "isVMDeepCopySupported", 21))
            return true;
    }
    return false;
}
...
#ifdef J9VM_OPT_JITSERVER
    TR_ASSERT(getPersistentInfo()->getRemoteCompilationMode() != JITServer::SERVER,
        "shouldDowngradeCompReq should not be used by JITServer");
#endif
    if (!isCompiled(method) && entry->_optimizationPlan->getOptLevel() == warm
        && // only warm compilations are subject to downgrades
        !methodDetails.isMethodInProgress() && !methodDetails.isJitDumpMethod()
        ...
        if (((_J9ROMMETHOD_J9MODIFIER_IS_SET(romMethod, J9AccMethodHasMethodHandleInvokes))
            ...
            && (_jitConfig->javaVM->phase == J9VM_PHASE_NOT_STARTUP || !TR::Options::sharedClassCache()
        ...
        // perform JNI at cold
            ...
            (entry->_methodIsInSharedCache == TR_yes && entry->_doNotAOTCompile
        ...
        // downgrade during IDLE
            ...
            !(TR::Options::getCmdLineOptions()->getOption(TR_UseIdleTime) && getCpuUtil()
                && getCpuUtil()->hasValidData() && getCpuUtil()->getCpuUsage() < 10
        ...
        // Downgrade if the JVM is starved of CPU
            ...
            if (TR::Options::getCmdLineOptions()->getOption(TR_DontDowgradeToColdDuringGracePeriod)
                ...
                if ((persistentInfo->isClassLoadingPhase()
                        && getNumQueuedFirstTimeCompilations() > TR::Options::_qsziThresholdToDowngradeDuringCLP)
                    ||
                ...
                // Downgrade if RI based recompilation is enabled
                else if (persistentInfo->isRuntimeInstrumentationRecompilationEnabled()
                    && // RI and RI Recompilation is functional
...
#endif
                    {
                        if (TR::Options::getCmdLineOptions()->getOption(TR_UseRIOnlyForLargeQSZ)) {
                            ...
                            // If we entered this mode, keep downgrading until Q_SZ drops under
                            // _qszMinThresholdToRIDowngrade
                            ...
                            if (qsz > TR::Options::_qszMaxThresholdToRIDowngrade) {
                                ...
                                if (hwProfiler->getQSzThresholdToDowngrade()
                                    != TR::Options::_qszMinThresholdToRIDowngrade)
                                ...
                                if (hwProfiler->getQSzThresholdToDowngrade()
                                    != TR::Options::_qszMaxThresholdToRIDowngrade)
...
bool TR::CompilationInfo::createCompilationInfo(J9JITConfig *jitConfig)
{
...
void TR::CompilationInfo::freeAllCompilationThreads()
{
...
void TR::CompilationInfoPerThread::closeRTLogFile()
{
    if (_rtLogFile) {
...
void TR::CompilationInfoPerThread::openRTLogFile()
{
...
#if defined(J9VM_OPT_JITSERVER)
J9ROMClass *TR::CompilationInfoPerThread::getRemoteROMClassIfCached(J9Class *clazz)
{
    return JITServerHelpers::getRemoteROMClassIfCached(getClientData(), clazz);
}

J9ROMClass *TR::CompilationInfoPerThread::getAndCacheRemoteROMClass(J9Class *clazz)
{
    auto romClass = getRemoteROMClassIfCached(clazz);
    if (romClass == NULL) {
        JITServerHelpers::ClassInfoTuple classInfoTuple;
        romClass = JITServerHelpers::getRemoteROMClass(clazz, getStream(), getClientData()->persistentMemory(),
            classInfoTuple);
...
void TR::CompilationInfoPerThread::waitForGCCycleMonitor(bool threadHasVMAccess)
{
    ...
    while (vm->omrVM->_gcCycleOn) {
        ...
        if (TR::Options::getCmdLineOptions()->getVerboseOption(TR_VerboseGCcycle)) {
            ...
            TR_VerboseLog::writeLineLocked(TR_Vlog_GCCYCLE,
                "CompilationThread woke up (GC cycle finished); Waiting time = %u msec", (uint32_t)waitTime);
        }
...
void TR::CompilationInfoPerThreadBase::setCompilationThreadState(CompilationThreadState v)
{
    TR_ASSERT(_compInfo.getCompilationMonitor()->owned_by_self(),
...
void TR::CompilationInfoPerThread::setCompilationThreadState(CompilationThreadState v)
{
    TR::CompilationInfoPerThreadBase::setCompilationThreadState(v);
}

bool TR::CompilationInfoPerThreadBase::compilationThreadIsActive()
{
    bool compThreadIsActive = _compilationThreadState == COMPTHREAD_ACTIVE
        || _compilationThreadState == COMPTHREAD_SIGNAL_WAIT || _compilationThreadState == COMPTHREAD_WAITING;
...
TR::CompilationInfoPerThreadBase::CompilationInfoPerThreadBase(TR::CompilationInfo &compInfo, J9JITConfig *jitConfig,
    int32_t id, bool onSeparateThread)
    : _compInfo(compInfo)
    ...
    _addToJProfilingQueue(false)
{
    ...
    TR_ASSERT_FATAL(_compThreadId
            < (TR::Options::_numAllocatedCompilationThreads + TR::CompilationInfo::MAX_DIAGNOSTIC_COMP_THREADS),
        "Cannot have a compId %d greater than %u", _compThreadId,
        (TR::Options::_numAllocatedCompilationThreads + TR::CompilationInfo::MAX_DIAGNOSTIC_COMP_THREADS));
}

TR::CompilationInfoPerThread::CompilationInfoPerThread(TR::CompilationInfo &compInfo, J9JITConfig *jitConfig,
    int32_t id, bool isDiagnosticThread)
    ...
    , _compThreadCPU(_compInfo.persistentMemory()->getPersistentInfo(), jitConfig, 490000000, id)
{
    ...
    if (_activeThreadName && _suspendedThreadName) {
        ...
        snprintf(_suspendedThreadName, suspendedThreadNameLength, (char *)selectedSuspendedThreadName,
            getCompThreadId());
    ...
    , _lastAllocatedCompThreadID(0)
{
    ...
    _classUnloadMonitor
        = TR::MonitorTable::get()->getClassUnloadMonitor(); // by this time this variable is initialized
...
void TR::CompilationInfo::initCPUEntitlement() { _cpuEntitlement.init(_jitConfig); }

#if defined(J9VM_OPT_CRIU_SUPPORT)
void TR::CompilationInfo::setNumUsableCompilationThreadsPostRestore(int32_t &numUsableCompThreads)
{
#if defined(J9VM_OPT_JITSERVER)
...
#endif // #if defined(J9VM_OPT_JITSERVER)
    {
        ...
        if (numUsableCompThreads <= 0) {
            numUsableCompThreads = (DEFAULT_CLIENT_USABLE_COMP_THREADS > numAllocatedThreads)
                ? numAllocatedThreads
                : DEFAULT_CLIENT_USABLE_COMP_THREADS;
...
TR::CompilationInfoPerThread *TR::CompilationInfo::getFirstSuspendedCompilationThread() // MCT
{
    for (int32_t i = getFirstCompThreadID(); i <= getLastCompThreadID(); i++) {
        ...
        if (currentState == COMPTHREAD_SUSPENDED || currentState == COMPTHREAD_SIGNAL_SUSPEND)
            return curCompThreadInfoPT;
    }
...
TR_YesNoMaybe TR::CompilationInfo::detectCompThreadStarvation()
{
    ...
    if (getCpuUtil()->hasValidData() && getCpuUtil()->getAvgCpuIdle() > 5 && // This is for the entire machine
...
void TR::CompilationInfo::waitOnCompMonitor(J9VMThread *vmThread) { getCompilationMonitor()->wait(); }
...
uint32_t TR::CompilationInfo::getMethodBytecodeSize(const J9ROMMethod *romMethod)
{
    return (romMethod->bytecodeSizeHigh << 16) + romMethod->bytecodeSizeLow;
}

#if defined(J9VM_OPT_JITSERVER)
JITServer::ServerStream *TR::CompilationInfo::getStream()
{
    return (TR::compInfoPT) ? TR::compInfoPT->getStream() : NULL;
}
#endif /* defined(J9VM_OPT_JITSERVER) */

uint32_t TR::CompilationInfo::getMethodBytecodeSize(J9Method *method)
{
#if defined(J9VM_OPT_JITSERVER)
...
bool TR::CompilationInfo::isJSR292(const J9ROMMethod *romMethod)
{
    return (_J9ROMMETHOD_J9MODIFIER_IS_SET(romMethod, J9AccMethodHasMethodHandleInvokes));
}

bool TR::CompilationInfo::isJSR292(J9Method *j9method)
{
#if defined(J9VM_OPT_JITSERVER)
    if (auto stream = TR::CompilationInfo::getStream()) {
        {
...
void TR::CompilationInfo::disableAOTCompilations()
{
...
void TR::CompilationInfo::printMethodNameToVlog(J9Method *method)
{
    ...
    TR_VerboseLog::write("%.*s.%.*s%.*s", J9UTF8_LENGTH(className), (char *)J9UTF8_DATA(className), J9UTF8_LENGTH(name),
...
const char *eventNames[]
    = { "SyncReq", "SmallLag", "LargeLag", "Medium-NoBudget", "Medium-highBudget", "Medium-LowBudget", "Medium-Idle" };
...
bool TR_LowPriorityCompQueue::createLowPriorityCompReqAndQueueIt(TR::IlGeneratorMethodDetails &details, void *startPC,
    uint8_t reason)
{
    ...
    compReq->_weight = (J9ROMMETHOD_HAS_BACKWARDS_BRANCHES(romMethod)) ? TR::CompilationInfo::WARM_LOOPY_WEIGHT
                                                                       : TR::CompilationInfo::WARM_LOOPLESS_WEIGHT;
    // add at the end of queue
...
{
    TR_ASSERT(TR::CompilationInfo::isCompiled(j9method) && startPC,
        "j9method %p must be compiled because it is an upgrade", j9method);
    TR::IlGeneratorMethodDetails details(j9method);
...
bool TR::CompilationInfo::canProcessLowPriorityRequest()
{
    ...
    if (getLowPriorityCompQueue().getFirstLPQRequest()->_reqFromSecondaryQueue
        == TR_MethodToBeCompiled::REASON_SERVER_UNAVAILABLE)
    ...
    {
        if (getCpuUtil() && getCpuUtil()->hasValidData() && getCpuUtil()->getAvgCpuIdle() > idleThreshold()
            && // enough idle time
    ...
    return (getCpuUtil() && getCpuUtil()->hasValidData() && getCpuUtil()->getAvgCpuIdle() > idleThreshold()
        && // enough idle time
        getJvmCpuEntitlement() - getCpuUtil()->getVmCpuUsage() > 50); // at least half a processor should be empty
}

int64_t TR::CompilationInfo::getCpuTimeSpentInCompilation()
{
    I_64 totalTime = 0;
    for (int32_t i = getFirstCompThreadID(); i <= getLastCompThreadID(); i++) {
        TR::CompilationInfoPerThread *curCompThreadInfoPT = _arrayOfCompilationInfoPerThread[i];
...
int32_t TR::CompilationInfo::changeCompThreadPriority(int32_t priority, int32_t locationCode)
{
...
//----------------------------------------------------------
bool TR::CompilationInfo::SmoothCompilation(TR_MethodToBeCompiled *entry, int32_t *optLevelAdjustment)
{
...
void TR::CompilationInfo::decNumGCRReqestsQueued(TR_MethodToBeCompiled *entry)
{
    if (entry->_GCRrequest)
        _numGCRQueued--;
}

void TR::CompilationInfo::incNumGCRRequestsQueued(TR_MethodToBeCompiled *entry)
{
    entry->_GCRrequest = true;
    _numGCRQueued++;
}

void TR::CompilationInfo::decNumInvReqestsQueued(TR_MethodToBeCompiled *entry)
{
    if (entry->_entryIsCountedAsInvRequest) {
        _numInvRequestsInCompQueue--;
        TR_ASSERT(_numInvRequestsInCompQueue >= 0, "_numInvRequestsInCompQueue is negative : %d",
            _numInvRequestsInCompQueue);
    }
}

void TR::CompilationInfo::incNumInvRequestsQueued(TR_MethodToBeCompiled *entry)
{
    entry->_entryIsCountedAsInvRequest = true;
    _numInvRequestsInCompQueue++;
}

void TR::CompilationInfo::updateCompQueueAccountingOnDequeue(TR_MethodToBeCompiled *entry)
{
    ...
    if (entry->getMethodDetails().isOrdinaryMethod() && entry->_oldStartPC == 0) {
        ...
        TR_ASSERT(_numQueuedFirstTimeCompilations >= 0, "_numQueuedFirstTimeCompilations is negative : %d",
            _numQueuedFirstTimeCompilations);
    }
...
bool TR::CompilationInfo::useOptLevelAdjustment()
{
    static bool answer = TR::Options::getCmdLineOptions()->getOption(TR_UseOptLevelAdjustment)
        && asynchronousCompilation() && TR::Options::getCmdLineOptions()->allowRecompilation();
    return answer;
}
...
void TR::CompilationInfo::invalidateRequestsForNativeMethods(J9Class *clazz, J9VMThread *vmThread)
{
    ...
    if (verbose)
        TR_VerboseLog::writeLineLocked(TR_Vlog_HK, "invalidateRequestsForNativeMethods class=%p vmThread=%p", clazz,
            vmThread);
        ...
        if (methodBeingCompiled && !methodBeingCompiled->_unloadedMethod) {
            ...
            if (method) {
                ...
                if (classOfMethod == clazz && TR::CompilationInfo::isJNINative(method)) {
                    ...
                    {
                        TR_ASSERT(methodBeingCompiled->_numThreadsWaiting == 0,
                            "Async compilations should not have threads waiting");
                    ...
                    if (verbose)
                        TR_VerboseLog::writeLineLocked(TR_Vlog_HK,
                            "Have marked as unloaded the JNI thunk compilation for method %p", method);
                }
            }
        }
    } // end for
    ...
    while (cur) {
        ...
        if (method && J9_CLASS_FROM_METHOD(method) == clazz && TR::CompilationInfo::isJNINative(method)) {
            if (verbose)
                TR_VerboseLog::writeLineLocked(TR_Vlog_HK,
                    "Invalidating JNI thunk compile request for method %p class %p", method,
                    J9_CLASS_FROM_METHOD(method));
    ...
    bool hotCodeReplacement)
{
    ...
    if (verbose)
        TR_VerboseLog::writeLineLocked(TR_Vlog_HK,
            "invalidateRequestsForUnloadedMethods class=%p vmThread=%p hotCodeReplacement=%d", clazz, vmThread,
            hotCodeReplacement);
        ...
        if (methodBeingCompiled && !methodBeingCompiled->_unloadedMethod) {
            ...
            if ((!unloadedClass && hotCodeReplacement) || // replacement in FSD mode
                ...
                if (!hotCodeReplacement) {
                    ...
                    TR_ASSERT(methodBeingCompiled->_numThreadsWaiting == 0
                            || methodBeingCompiled->_priority >= CP_SYNC_MIN,
    ...
    while (cur) {
        ...
        if (method) {
            ...
            if (verboseDetails)
                TR_VerboseLog::writeLineLocked(TR_Vlog_HD, "Looking at compile request for method %p class %p", method,
                    methodClass);
            if ((!unloadedClass && hotCodeReplacement) || // replacement in FSD mode
                ...
                || (details.isNewInstanceThunk()
                    && static_cast<J9::NewInstanceThunkDetails &>(details).isThunkFor(unloadedClass))) {
                if (verbose)
                    TR_VerboseLog::writeLineLocked(TR_Vlog_HK, "Invalidating compile request for method %p class %p",
                        method, methodClass);
                if (!hotCodeReplacement) {
                    TR_ASSERT(cur->_priority < CP_SYNC_MIN, "Unloading cannot happen for SYNC requests");
                } else {
...
//
bool TR::CompilationInfo::shouldRetryCompilation(J9VMThread *vmThread, TR_MethodToBeCompiled *entry,
    TR::Compilation *comp)
{
        ...
        if (entry->_compilationAttemptsLeft > 0) {
            ...
            switch (entry->_compErrCode) {
                ...
                case compilationStreamFailure:
                    ...
                    if (compInfo->getPersistentInfo()->getRequireJITServer()) {
                        TR_ASSERT_FATAL(false,
                            "Option -XX:+JITServerRequireServer is used, terminate the JITClient due to unavailable "
                            "JITServer.");
                    }
                ...
                case compilationVirtualAddressExhaustion:
                    ...
                    bodyInfo = 0;
                    if (comp->allowRecompilation() && entry->_optimizationPlan
                        && entry->_optimizationPlan->getOptLevel() > minHotness) {
                    ...
                    if (tryCompilingAgain) {
                        ...
                        entry->_optimizationPlan->setDisableEDO(
                            true); // disable EDO to prevent recompilation triggered from native code
                        entry->_optimizationPlan->setDisableGCR(
                            true); // disable GCR to prevent recompilation triggered from native code
                    }
                    break;
                ...
                case compilationLambdaEnforceScorching:
                    // This must be a warm compilation. Retry the same compilation at scorching even if it's a first
                    // time compilation Watch out for the following scenario: (1) warm comp req. (2) Ilgen fails comp
                    // because it wants a hot comp (3) hot comp fails due to some compiler limit and we lower opt level
                    // at warm; (4) Ilgen fails again because it wants a hot compilation Also we should not fail the
                    // compilation for fixed opt level.
                    if (entry->_compilationAttemptsLeft
                        == MAX_COMPILE_ATTEMPTS) // Raise opt level only for the first attempt
                    {
                        TR_Hotness hotness = entry->_optimizationPlan->getOptLevel();
                        if (hotness <= veryHot) {
            ...
            if (tryCompilingAgain && comp) {
                ...
                if (methodInfo)
                    methodInfo->setNextCompileLevel(entry->_optimizationPlan->getOptLevel(),
                        entry->_optimizationPlan->insertInstrumentation());
            }
        } else if (0) // later add test for multiple aot relocation failure to catch that scenario, which shouldn't
                      // happen
        {
    ...
    if (tryCompilingAgain) {
        ...
        if (javaVM->internalVMFunctions->isDebugOnRestoreEnabled(javaVM)
            && javaVM->internalVMFunctions->isCheckpointAllowed(javaVM)
            && !compInfo->getCRRuntime()->isCheckpointInProgress() && !compInfo->isInShutdownMode()) {
            if (TR::Options::getCmdLineOptions()->getVerboseOption(TR_VerboseCheckpointRestoreDetails))
                TR_VerboseLog::writeLineLocked(TR_Vlog_CHECKPOINT_RESTORE, "Pushing failed compilation %p",
            ...
            if (comp
                && entry->_compInfoPT->methodCanBeCompiled(comp->trMemory(), comp->fej9(),
                    comp->getMethodBeingCompiled(), filter))
...
void TR::CompilationInfo::suspendCompilationThread(bool purgeCompQueue)
{
    ...
    for (int32_t i = getFirstCompThreadID(); i <= getLastCompThreadID(); i++) {
        ...
        if (curCompThreadInfoPT->compilationThreadIsActive()) {
            ...
            if (TR::Options::getCmdLineOptions()->getVerboseOption(TR_VerboseCompilationThreads)) {
                TR_VerboseLog::writeLineLocked(TR_Vlog_PERF, "t=%6u Suspension request for compThread %d sleeping=%s",
                    (uint32_t)getPersistentInfo()->getElapsedTime(), curCompThreadInfoPT->getCompThreadId(),
                    curCompThreadInfoPT->getMethodBeingCompiled() ? "NO" : "YES");
            }
        }
    }
...
void TR::CompilationInfo::resumeCompilationThread()
{
    ...
    for (int32_t i = getFirstCompThreadID(); i <= getLastCompThreadID(); i++) {
        ...
        if (currentThreadState == COMPTHREAD_ACTIVE || currentThreadState == COMPTHREAD_SIGNAL_WAIT
            || currentThreadState == COMPTHREAD_WAITING || currentThreadState == COMPTHREAD_SIGNAL_SUSPEND) {
            if (curCompThreadInfoPT->compilationThreadIsActive())
    ...
    if (numActiveCompThreads != getNumCompThreadsActive()) {
        TR_ASSERT(false,
            "Discrepancy regarding active compilation threads: numActiveCompThreads=%d getNumCompThreadsActive()=%d\n",
    ...
    if (compInfoPTHot) {
        ...
        if (TR::Options::getCmdLineOptions()->getVerboseOption(TR_VerboseCompilationThreads)) {
            TR_VerboseLog::writeLineLocked(TR_Vlog_INFO, "t=%6u Resume compThread %d Qweight=%d active=%d",
                (uint32_t)getPersistentInfo()->getElapsedTime(), compInfoPTHot->getCompThreadId(), getQueueWeight(),
    ...
    if (getNumCompThreadsActive() == 0) {
        TR_VerboseLog::writeLineLocked(TR_Vlog_INFO,
            "No threads were activated following a resume all compilation threads call");
    }
...
int TR::CompilationInfo::computeCompilationThreadPriority(J9JavaVM *vm)
{
    ...
    static UDATA priorityConversionTable[] = { J9THREAD_PRIORITY_MIN, J9THREAD_PRIORITY_USER_MIN,
        J9THREAD_PRIORITY_NORMAL, J9THREAD_PRIORITY_USER_MAX, J9THREAD_PRIORITY_MAX };
    if (TR::Options::_compilationThreadPriorityCode >= 0 && TR::Options::_compilationThreadPriorityCode <= 4)
...
bool TR::CompilationInfo::allocateCompilationThreads(int32_t numCompThreads)
{
    if (_compThreadActivationThresholds || _compThreadSuspensionThresholds
        || _compThreadActivationThresholdsonStarvation || _arrayOfCompilationInfoPerThread) {
    ...
    TR_ASSERT_FATAL((numCompThreads == TR::Options::_numAllocatedCompilationThreads),
        "numCompThreads %d is not equal to the Option value %d", numCompThreads,
    ...
    TR::MonitorTable *table = TR::MonitorTable::get();
    if ((!table) || (!table->allocInitClassUnloadMonitorHolders(numTotalCompThreads))) {
        return false;
    }

    _compThreadActivationThresholds
        = static_cast<int32_t *>(jitPersistentAlloc((numTotalCompThreads + 1) * sizeof(int32_t)));
    _compThreadSuspensionThresholds
        = static_cast<int32_t *>(jitPersistentAlloc((numTotalCompThreads + 1) * sizeof(int32_t)));
    _compThreadActivationThresholdsonStarvation
        = static_cast<int32_t *>(jitPersistentAlloc((numTotalCompThreads + 1) * sizeof(int32_t)));

    _arrayOfCompilationInfoPerThread = static_cast<TR::CompilationInfoPerThread **>(
        jitPersistentAlloc(numTotalCompThreads * sizeof(TR::CompilationInfoPerThread *)));
    ...
    if (_compThreadActivationThresholds && _compThreadSuspensionThresholds
        ...
        // compThreadActivationThresholdsonStarvation[MAX_TOTAL_COMP_THREADS+1] = {-1, 800, 1600, 3200, 6400, 12800,
        // 19200, 25600, 32000};
        ...
        for (int32_t i = 2; i < (numTotalCompThreads + 1); ++i) {
            if (_compThreadActivationThresholdsonStarvation[i - 1] < 12800) {
                ...
                _compThreadActivationThresholdsonStarvation[i]
                    = _compThreadActivationThresholdsonStarvation[i - 1] + 6400;
            }
        }
...
//----------------------------------------------------------------------------
uintptr_t TR::CompilationInfo::startCompilationThread(int32_t priority, int32_t threadId, bool isDiagnosticThread)
{
    if (!_compilationMonitor)
        return 1;
    ...
    if (vm->internalVMFunctions->createThreadWithCategory(&compInfoPT->_osThread,
            ...
            (j9thread_entrypoint_t)compilationThreadProc, compInfoPT, J9THREAD_CATEGORY_SYSTEM_JIT_THREAD)) {
...
void TR::CompilationInfo::cleanDLTRecordOnUnload()
{
    for (int32_t i = 0; i < DLT_HASHSIZE; i++) {
        ...
        while (curr != NULL) {
            ...
            // Non-Anon classes will be unloaded with their classloaders, hence the class's classloader will be marked
            // as dead. Anon Classes can be independently unloaded without their classloaders, however their classes are
            // marked as dying.
                 ...
                 fprintf(stderr, "%.*s", J9UTF8_LENGTH(utf8), (char *) J9UTF8_DATA(utf8));
                    */
                    ...
                    if (count < 0) {
                        ...
                        intptr_t initialCount = J9ROMMETHOD_HAS_BACKWARDS_BRANCHES(romMethod)
...
void TR::CompilationInfo::stopCompilationThreads()
{
    ...
    if (TR::Options::getCmdLineOptions()->getOption(TR_PrintCodeCacheUsage) || printCompMem || printCCUsage) {
        ...
        fprintf(stderr,
            "\nCodeCache: size=%" OMR_PRIuPTR "kB used=%" OMR_PRIuSIZE "kB max_used=%" OMR_PRIuSIZE
            "kB free=%" OMR_PRIuSIZE "kB\n\n",
            _jitConfig->codeCacheTotalKB, currTotalUsedKB, maxUsedKB, (_jitConfig->codeCacheTotalKB - currTotalUsedKB));
    }

    if (printCompMem) {
        int32_t codeCacheAllocated
            = TR::CodeCacheManager::instance()->getCurrentNumberOfCodeCaches() * _jitConfig->codeCacheKB;
        ...
        fprintf(stderr, "Allocated memory for data cache = %d KB\tLimit = %" OMR_PRIuPTR " KB\n",
            TR_DataCacheManager::getManager()->getTotalSegmentMemoryAllocated() / 1024, _jitConfig->dataCacheTotalKB);
    ...
    if (aotStatsEnabled) {
        ...
        for (uint32_t i = 0; i < TR_NumExternalRelocationKinds; i++) {
            fprintf(stderr, "%s: %d\n", TR::ExternalRelocation::getName((TR_ExternalRelocationTargetKind)i),
                aotStats->numRelocationsFailedByType[i]);
        }
        fprintf(stderr, "-------------------------\n");
        ...
        try {
            ...
            if (TR::Options::getVerboseOption(TR_VerboseJITServer))
                TR_VerboseLog::writeLineLocked(TR_Vlog_JITServer,
                    "JITServer StreamFailure (server unreachable before the termination message was sent): %s",
                    e.what());
        }
    }
#endif /* defined(J9VM_OPT_JITSERVER) */
...
IDATA J9THREAD_PROC compilationThreadProc(void *entryarg)
{
    ...
    int rc = vm->internalVMFunctions->internalAttachCurrentThread(vm, &compThread, NULL,
    ...
    if (!TR_NoStructuredHandler) {
        ...
        I_32 sig_result = j9sig_protect((j9sig_protected_fn)protectedCompilationThreadProc, compInfoPT,
            vm->internalVMFunctions->structuredSignalHandler, compThread, flags, (UDATA *)&result);
        if (sig_result != 0) {
            result = JNI_ERR;
        }
    } else
...
IDATA J9THREAD_PROC protectedCompilationThreadProc(J9PortLibrary *,
    TR::CompilationInfoPerThread *compInfoPT /*void *vmthread*/)
{
    ...
    compInfo->setCompBudgetSupport(TR::Options::_compilationBudget > 0 && compInfo->asynchronousCompilation()
    ...
    if (TR::Options::isAnyVerboseOptionSet()) {
        TR_VerboseLog::writeLineLocked(TR_Vlog_INFO, "Stopping compilation thread, vmThread pointer %p, thread ID %d",
            compThread, compInfoPT->getCompThreadId());
    }
...
#if defined(J9VM_OPT_JITSERVER)
JITServer::ServerStream *TR::CompilationInfoPerThread::getStream()
{
    return (_methodBeingCompiled) ? _methodBeingCompiled->_stream : NULL;
}
#endif /* defined(J9VM_OPT_JITSERVER) */

void TR::CompilationInfoPerThread::run()
{
#if defined(J9VM_OPT_JITSERVER)
    TR::compInfoPT = this; // set the thread_local pointer to this object on first run
#endif
    for (CompilationThreadState threadState = getCompilationThreadState(); threadState != COMPTHREAD_SIGNAL_TERMINATE;
...
void TR::CompilationInfoPerThread::waitForWork()
{
...
void TR::CompilationInfoPerThread::doSuspend()
{
...
J9::J9SegmentCache TR::CompilationInfoPerThread::initializeSegmentCache(J9::J9SegmentProvider &segmentProvider)
{
#if defined(J9VM_OPT_JITSERVER)
...
void TR::CompilationInfoPerThread::processEntries()
{
    if (TR::Options::getVerboseOption(TR_VerboseCompilationDispatch)) {
        TR_VerboseLog::writeLineLocked(TR_Vlog_DISPATCH,
            "Starting to process queue entries. compThreadID=%d state=%d Q_SZ=%d Q_SZI=%d QW=%d", getCompThreadId(),
            getCompilationThreadState(), _compInfo.getMethodQueueSize(), _compInfo.getNumQueuedFirstTimeCompilations(),
            _compInfo.getQueueWeight());
    }
    // Possible scenarios here
    // 1. We take a method from queue and compile it
    // 2. We refrain to take a method from queue because that means two simultaneous compilations --> wait to be
    // notified
    // 3. We refrain to take a method from queue because compilation consumes too much CPU.
    // 3.1 If there is another active comp thread, then suspend this one. The activation should be avoided in this
    // situation 3.2 If this is the last active comp thread, wait on the comp monitor timed. Note: the diagnostic thread
    // must not wait.
    TR::CompilationInfo *compInfo = getCompilationInfo();
    J9VMThread *compThread = getCompilationThread();
    try {
        J9::SegmentAllocator scratchSegmentAllocator(MEMORY_TYPE_JIT_SCRATCH_SPACE | MEMORY_TYPE_VIRTUAL,
            *_jitConfig->javaVM);
        ...
        while (getCompilationThreadState() == COMPTHREAD_ACTIVE) {
            ...
            TR_MethodToBeCompiled *entry = compInfo->getNextMethodToBeCompiled(this,
                _previousCompilationThreadState == COMPTHREAD_WAITING, &compThreadAction);
            switch (compThreadAction) {
                ...
                case TR::CompilationInfo::GO_TO_SLEEP_EMPTY_QUEUE: {
                    ...
                    if (getCompilationThreadState() == COMPTHREAD_WAITING) {
                         ...
                         */
                        {
                            ...
                            if (TR::Options::getVerboseOption(TR_VerboseCompilationDispatch)) {
                                TR_VerboseLog::writeLineLocked(TR_Vlog_DISPATCH,
                                    "compThreadID=%d woke up after timeout on compMonitor", getCompThreadId());
                            }
                        }
                    }
                    break;
                ...
                case TR::CompilationInfo::SUSPEND_COMP_THREAD_EMPTY_QUEUE:
                    TR_ASSERT(compInfo->getNumCompThreadsActive() > 1,
                        "Should not suspend the last active compilation thread: %d\n",
                        compInfo->getNumCompThreadsActive());
                    setCompilationThreadState(COMPTHREAD_SIGNAL_SUSPEND);
                    compInfo->decNumCompThreadsActive();
                    if (TR::Options::getCmdLineOptions()->getVerboseOption(TR_VerboseCompilationThreads)) {
                        TR_VerboseLog::writeLineLocked(TR_Vlog_INFO,
                            "t=%6u Suspending compThread %d due to %s Qweight=%d active=%d overallCompCpuUtil=%d",
                            (uint32_t)compInfo->getPersistentInfo()->getElapsedTime(), getCompThreadId(),
                            compThreadAction == TR::CompilationInfo::SUSPEND_COMP_THREAD_EXCEED_CPU_ENTITLEMENT
                                ? "exceeding CPU entitlement"
                                : "empty queue",
                            compInfo->getQueueWeight(), compInfo->getNumCompThreadsActive(),
                ...
                case TR::CompilationInfo::THROTTLE_COMP_THREAD_EXCEED_CPU_ENTITLEMENT: {
                    TR_ASSERT(compInfo->getNumCompThreadsActive() <= 1,
                        "Throttling when we have several comp threads active: %d\n",
                        compInfo->getNumCompThreadsActive());
                    int32_t sleepTimeMs = compInfo->computeCompThreadSleepTime(getLastCompilationDuration());
                    if (TR::Options::getCmdLineOptions()->getVerboseOption(TR_VerboseCompilationThreads)) {
                        TR_VerboseLog::writeLineLocked(TR_Vlog_INFO,
                            "t=%6u compThread %d sleeping %d ms due to throttling Qweight=%d active=%d "
                            "overallCompCpuUtil=%d",
                            (uint32_t)compInfo->getPersistentInfo()->getElapsedTime(), getCompThreadId(), sleepTimeMs,
                            compInfo->getQueueWeight(), compInfo->getNumCompThreadsActive(),
                            compInfo->getOverallCompCpuUtilization());
                    }
        ...
        if (compInfo->getNumCompThreadsActive() > 1) {
            if (compilationThreadIsActive()) {
                ...
                if (TR::Options::isAnyVerboseOptionSet(TR_VerboseCompilationThreads, TR_VerbosePerformance)) {
                    TR_VerboseLog::writeLineLocked(TR_Vlog_PERF,
                        "t=%6u Suspending compilation thread %d due to insufficient resources",
                        (uint32_t)compInfo->getPersistentInfo()->getElapsedTime(), getCompThreadId());
                }
            }
        } else {
            ...
            if (TR::Options::isAnyVerboseOptionSet(TR_VerboseCompilationThreads, TR_VerbosePerformance,
                ...
                TR_VerboseLog::writeLineLocked(TR_Vlog_PERF,
                    "t=%6u Disable further compilation due to OOM while processing compile entries",
...
void TR::CompilationInfoPerThread::processEntry(TR_MethodToBeCompiled &entry,
    J9::J9SegmentProvider &scratchSegmentProvider)
{
    ...
    //
    // This conversion is safe. The macro J9VM_J9CLASS_TO_HEAPCLASS will not make a conversion if Classes on Heap is not
    // enabled.
    jobject classObject = NULL;
    if (!entry.isOutOfProcessCompReq())
        classObject = compThread->javaVM->internalVMFunctions->j9jni_createLocalRef((JNIEnv *)compThread,
            J9VM_J9CLASS_TO_HEAPCLASS(details.getClass()));
    ...
    if ((compThread->publicFlags & J9_PUBLIC_FLAGS_VM_ACCESS) == 0) {
        TR_ASSERT_FATAL(isDiagnosticThread(),
            "A compilation thread has finished a compilation but does not hold VM access");
        ...
        if (entry._compErrCode == compilationOK) {
            if (entry._optimizationPlan->shouldAddToUpgradeQueue()) {
                ...
                compInfo->getLowPriorityCompQueue().addUpgradeReqToLPQ(getMethodBeingCompiled(),
                    TR_MethodToBeCompiled::REASON_UPGRADE);
#ifdef STATS
                ...
                if (TR::Options::isAnyVerboseOptionSet(TR_VerboseJITServer, TR_VerboseCompilationDispatch,
                        TR_VerbosePerformance, TR_VerboseCompFailure))
                    TR_VerboseLog::writeLineLocked(TR_Vlog_JITServer,
                        "t=%6u Buffering an upgrade request into the LPQ: j9method=%p",
                        (uint32_t)compInfo->getPersistentInfo()->getElapsedTime(),
                        entry.getMethodDetails().getMethod());
                compInfo->getLowPriorityCompQueue().addUpgradeReqToLPQ(getMethodBeingCompiled(),
                    TR_MethodToBeCompiled::REASON_SERVER_UNAVAILABLE);
    ...
    if (entry._tryCompilingAgain) {
        ...
        if (oldPriority <= CP_ASYNC_MAX) {
            ...
            if (reservedDataCache() && compInfo->getMethodQueue()
                && compInfo->getMethodQueue()->_priority >= CP_ASYNC_MAX) {
        ...
        entry._reqFromSecondaryQueue
            = TR_MethodToBeCompiled::REASON_NONE; // we are going to put this into the main queue, so entry is not
                                                  // coming from LPQ anymore
    ...
    if (!isDiagnosticThread() // must not be reserved for log
        ...
        && (compInfo->getRampDownMCT() // force to have only one thread active
            ...
            || (!tryCompilingAgain
                ...
                && compInfo->getQueueWeight()
                    < TR::CompilationInfo::getCompThreadSuspensionThreshold(compInfo->getNumCompThreadsActive()))
#if defined(J9VM_OPT_JITSERVER)
            || (compInfo->getPersistentInfo()->getRemoteCompilationMode() == JITServer::CLIENT
                && compInfo->getCompThreadActivationPolicy()
                    == JITServer::CompThreadActivationPolicy::SUSPEND) // keep suspending threads until server space
                                                                       // frees up
#endif
            )) {
        // Suspend this thread
...
#if defined(J9VM_OPT_JITSERVER)
                compInfo->getCompThreadActivationPolicy() == JITServer::CompThreadActivationPolicy::SUSPEND
                    ? "ServerLowPhysicalMemOrHighThreadCount"
                    :
#endif
                    "");
        }
        ...
        if (compInfo->getNumCompThreadsJobless() > 0) {
            ...
            if (TR::Options::getCmdLineOptions()->getVerboseOption(TR_VerboseCompilationThreads)) {
                TR_VerboseLog::writeLineLocked(TR_Vlog_INFO,
                    "t=%6u compThread %d notifying other sleeping comp threads. Jobless=%d",
                    (uint32_t)compInfo->getPersistentInfo()->getElapsedTime(), getCompThreadId(),
    ...
    } else // We will not suspend this thread
    {
        ...
        if (compInfo->getSuspendThreadDueToLowPhysicalMemory() && compInfo->getNumCompThreadsActive() < 2)
            compInfo->setSuspendThreadDueToLowPhysicalMemory(false);
    }
}

bool TR::CompilationInfoPerThread::shouldPerformCompilation(TR_MethodToBeCompiled &entry)
{
    if (entry.isOutOfProcessCompReq())
        return true;
    TR::CompilationInfo *compInfo = getCompilationInfo();
    ...
    if (entry._reqFromSecondaryQueue) {
        ...
        if (!entry._oldStartPC) {
            ...
            if (!(J9_ROM_METHOD_FROM_RAM_METHOD(method)->modifiers
                    & J9AccNative)) // never change the extra field of a native method
            {
        ...
        {
            TR_ASSERT(entry._reqFromSecondaryQueue == TR_MethodToBeCompiled::REASON_UPGRADE,
                "wrong reason for upgrade");
...
TR_MethodToBeCompiled *TR::CompilationInfo::addMethodToBeCompiled(TR::IlGeneratorMethodDetails &details, void *pc,
    CompilationPriority priority, bool async, TR_OptimizationPlan *optimizationPlan, bool *queued,
    TR_YesNoMaybe methodIsInSharedCache)
{
    ...
    if (!disableSkipSearching && pc == 0 && // Interpreted methods
        details.isOrdinaryMethod() && !isJNINative(method)
    ...
    if (cur) {
        if (TR::Options::getCmdLineOptions()->getVerboseOption(TR_VerboseCompileRequest))
            TR_VerboseLog::writeLineLocked(TR_Vlog_CR,
                "%p     Already present in compilation queue. OldPriority=%x NewPriority=%x entry=%p",
                _jitConfig->javaVM->internalVMFunctions->currentVMThread(_jitConfig->javaVM), cur->_priority, priority,
                cur);
        ...
        if (cur->_optimizationPlan->getOptLevel() != optimizationPlan->getOptLevel()) {
            ...
            if (pc) {
                ...
                if (methodInfo && methodInfo->getNextCompileLevel() != cur->_optimizationPlan->getOptLevel())
                    methodInfo->setNextCompileLevel(cur->_optimizationPlan->getOptLevel(),
                        cur->_optimizationPlan->insertInstrumentation());
            }
        }
    ...
    //
        ...
        if (!skipSearchingForDuplicates) {
            if (queueWeight != _queueWeight) // QW
            {
                if (TR::Options::isAnyVerboseOptionSet())
                    TR_VerboseLog::writeLineLocked(TR_Vlog_INFO,
                        "Discrepancy for queue weight while adding to queue: computed=%u recorded=%u", queueWeight,
                        _queueWeight);
                // correction
            ...
            if (numEntries != _numQueuedMethods) {
                if (TR::Options::isAnyVerboseOptionSet())
                    TR_VerboseLog::writeLineLocked(TR_Vlog_INFO,
                        "Discrepancy for queue size while adding to queue: Before adding numEntries=%d  "
                        "_numQueuedMethods=%d",
        ...
        if (pc) {
            ...
            TR_PersistentMethodInfo *methodInfo
                = bodyInfo->getMethodInfo(); // TR::Recompilation::getMethodInfoFromPC(pc);
        ...
        } else if (details.isOrdinaryMethod()) // ordinary interpreted methods
        {
            ...
            if (async && (getInvocationCount(method) == 0) // this will filter out JNI natives
                && !(J9_ROM_METHOD_FROM_RAM_METHOD(method)->modifiers
                    & J9AccNative)) // Never change the extra field of a native method
            {
...
#if defined(J9VM_INTERP_AOT_RUNTIME_SUPPORT) && defined(J9VM_OPT_SHARED_CLASSES)                        \
    && (defined(TR_HOST_X86) || defined(TR_HOST_POWER) || defined(TR_HOST_S390) || defined(TR_HOST_ARM) \
        || defined(TR_HOST_ARM64))
        ...
        *queued = true; // set the flag that we added a new request to the queue
        ...
        Trc_JIT_CompRequest(vmThread, method, pc, !async, optimizationPlan->getOptLevel(), (int)priority,
            _numQueuedMethods);
        ...
        if (TR::Options::getCmdLineOptions()->getVerboseOption(TR_VerboseCompileRequest))
            TR_VerboseLog::writeLineLocked(TR_Vlog_CR,
                "%p   Added entry %p of weight %d to comp queue. Now Q_SZ=%d weight=%d",
                _jitConfig->javaVM->internalVMFunctions->currentVMThread(_jitConfig->javaVM), cur, entryWeight,
                getMethodQueueSize(), getQueueWeight());

        // Examine if we need to activate a new thread
        TR_YesNoMaybe activate = shouldActivateNewCompThread();
        if (activate == TR_maybe
            && TR::Options::getCmdLineOptions()->getOption(TR_ActivateCompThreadWhenHighPriReqIsBlocked)
            && getNumCompThreadsActive() == 1 && priority >= CP_ASYNC_BELOW_MAX) // AOT loads and JNI
        {
            ...
            if (lowPriCompThread) {
                ...
                uint8_t targetWeight
                    = _compThreadSuspensionThresholds[2] <= 0xff ? _compThreadSuspensionThresholds[2] : 0xff;
                ...
                if (TR::Options::getCmdLineOptions()->getVerboseOption(TR_VerboseCompilationThreads))
                    TR_VerboseLog::writeLineLocked(TR_Vlog_INFO,
                        "t=%6u High priority req (0x%x) blocked by priority 0x%x",
                        (uint32_t)getPersistentInfo()->getElapsedTime(), priority, lowPriCompReq->_priority);
            }
        }

        if (activate == TR_yes) {
            // Must find one that is SUSPENDED/SUSPENDING otherwise the logic in shouldActivateNewCompThread is
            // incorrect
            ...
            if (TR::Options::getCmdLineOptions()->getVerboseOption(TR_VerboseCompilationThreads)) {
                TR_VerboseLog::writeLineLocked(TR_Vlog_INFO, "t=%6u Activate compThread %d Qweight=%d active=%d",
                    (uint32_t)getPersistentInfo()->getElapsedTime(), compInfoPT->getCompThreadId(), getQueueWeight(),
                    getNumCompThreadsActive());
            }
        }
    }
...
//----------------------------------------------------------------------------
TR_MethodToBeCompiled *TR::CompilationInfo::adjustCompilationEntryAndRequeue(TR::IlGeneratorMethodDetails &details,
    TR_PersistentMethodInfo *methodInfo, TR_Hotness newOptLevel, bool useProfiling, CompilationPriority priority,
    TR_J9VMBase *fe)
{
    for (int32_t i = getFirstCompThreadID(); i <= getLastCompThreadID(); i++) {
        TR::CompilationInfoPerThread *curCompThreadInfoPT = _arrayOfCompilationInfoPerThread[i];
...
TR_MethodToBeCompiled *TR::CompilationInfo::requestExistsInCompilationQueue(TR::IlGeneratorMethodDetails &details,
    TR_FrontEnd *fe)
{
    ...
    for (int32_t i = getFirstCompThreadID(); i <= getLastCompThreadID(); i++) {
        ...
        if (curCompThreadInfoPT->getMethodBeingCompiled()
            ...
            && !curCompThreadInfoPT->getMethodBeingCompiled()
                    ->_unloadedMethod) // Redefinition; see cmvc 192606 and RTC 36898
...
TR_MethodToBeCompiled *TR::CompilationInfo::getNextMethodToBeCompiled(TR::CompilationInfoPerThread *compInfoPT,
    bool compThreadCameOutOfSleep, TR_CompThreadActions *compThreadAction)
{
    ...
    // compilation request (see eclipse-openj9/openj9#11772 for details). We must ensure that this does not happen and
    // simply skip the request if we are a non-diagnostic thread attempting to process a JitDump compilation.
    if (compInfoPT->isDiagnosticThread()) {
        ...
        *compThreadAction = GO_TO_SLEEP_EMPTY_QUEUE;
        ...
        if (_methodQueue) {
            ...
            TR_ASSERT_FATAL(nextMethodToBeCompiled->getMethodDetails().isJitDumpMethod(),
                "Diagnostic thread attempting to process non-JitDump compilation");
        ...
        *compThreadAction = PROCESS_ENTRY;
        ...
        if (_methodQueue != NULL && !_methodQueue->getMethodDetails().isJitDumpMethod()) {
...
#if defined(J9VM_OPT_JITSERVER)
                || getPersistentInfo()->getRemoteCompilationMode()
                    == JITServer::SERVER // compile right away in server mode
#endif
            ) {
                nextMethodToBeCompiled = _methodQueue;
                _methodQueue = _methodQueue->_next;
            ...
            else if (exceedsCompCpuEntitlement() == TR_yes && !compThreadCameOutOfSleep
                && // Don't throttle a comp thread that has just slept its share of time
                (TR::Options::_compThreadCPUEntitlement < 100
                    || getNumCompThreadsActive() * 100 > (TR::Options::_compThreadCPUEntitlement + 50))) {
            ...
            } else // scan for a cold/warm method
            {
                ...
                for (nextMethodToBeCompiled = _methodQueue->_next; nextMethodToBeCompiled;
                     prev = nextMethodToBeCompiled, nextMethodToBeCompiled = nextMethodToBeCompiled->_next) {
                    ...
                    *compThreadAction = GO_TO_SLEEP_CONCURRENT_EXPENSIVE_REQUESTS;
                    ...
                    for (int32_t i = getFirstCompThreadID(); i <= getLastCompThreadID(); i++) {
                        ...
                        if (currentThreadState == COMPTHREAD_ACTIVE || currentThreadState == COMPTHREAD_SIGNAL_WAIT
                            ...
                            if (curCompThreadInfoPT->getMethodBeingCompiled()
                                && curCompThreadInfoPT->getMethodBeingCompiled()
                                       ->_hasIncrementedNumCompThreadsCompilingHotterMethods)
                                numHot++;
                        }
                    }
                    ...
                    TR_ASSERT(numHot >= 1 || numLowPriority >= 1,
                        "We must have a comp thread working on a hot or low priority method %d %d", numHot,
                        numLowPriority);
        ...
        // When no request is in the main queue we can look in the low priority queue
            ...
            // Check if we need to throttle
            if (exceedsCompCpuEntitlement() == TR_yes && !compThreadCameOutOfSleep
                && // Don't throttle a comp thread that has just slept its share of time
                (TR::Options::_compThreadCPUEntitlement < 100
                    || getNumCompThreadsActive() * 100 > (TR::Options::_compThreadCPUEntitlement + 50))
#if defined(J9VM_OPT_JITSERVER)
                && !getLowPriorityCompQueue()
                        .getFirstLPQRequest()
                        ->shouldUpgradeOutOfProcessCompilation() // Don't throttle if compilation will be done remotely
#endif
            ) {
                // If at all possible suspend the compilation thread
                // Otherwise, perform a timed wait
        ...
        if (nextMethodToBeCompiled != NULL) {
            // See explanation at the start of this function of why it is important to ensure this
            TR_ASSERT_FATAL(!nextMethodToBeCompiled->getMethodDetails().isJitDumpMethod(),
                "Non-diagnostic thread attempting to process JitDump compilation");
        }
    }
...
int32_t TR::CompilationInfo::computeCompThreadSleepTime(int32_t compilationTimeMs)
{
    ...
    if (sleepTimeMs < TR::Options::_minSleepTimeMsForCompThrottling) // too small values are not useful because of high
                                                                     // overhead of calling wait()
...
void *TR::CompilationInfo::startPCIfAlreadyCompiled(J9VMThread *vmThread, TR::IlGeneratorMethodDetails &details,
    void *oldStartPC)
{
    if (details.isNewInstanceThunk()) {
        return jitNewInstanceMethodStartAddress(vmThread,
...

        {
            ...
            if (thunkAddress != initialInvokeExactThunkGlueAddress) {
                ...
                // avoid this case.
                //
                J9JITHashTable *hashTable
                    = (J9JITHashTable *)avl_search(jitConfig->translationArtifacts, (UDATA)thunkAddress);
                if (hashTable) {
                ...
                if (verboseDetails) {
                    if (thunkStartPC)
                        TR_VerboseLog::writeLineLocked(TR_Vlog_MHD,
                            "%p   Metadata lookup: handle %p thunk body at %p has startPC %p", vmThread,
                            (void *)methodHandle, thunkAddress, thunkStartPC);
                    else
                        TR_VerboseLog::writeLineLocked(TR_Vlog_MHD,
                            "%p   Metadata lookup FAILED for: handle %p thunk body at %p -- jit will probably create a "
                            "redundant body",
                            vmThread, (void *)methodHandle, thunkAddress);
                }
            }
        }
...
{
    TR_ASSERT_FATAL((entry->_freeTag & (ENTRY_INITIALIZED))
            || (entry->_freeTag & (ENTRY_IN_POOL_NOT_FREE | ENTRY_IN_POOL_FREE | ENTRY_DEALLOCATED)),
...
void *TR::CompilationInfo::compileMethod(J9VMThread *vmThread, TR::IlGeneratorMethodDetails &details, void *oldStartPC,
    TR_YesNoMaybe requireAsyncCompile, TR_CompilationErrorCode *compErrCode, bool *queued,
    TR_OptimizationPlan *optimizationPlan)
{
    ...
    //
    if ((_jitConfig->runtimeFlags & J9JIT_SCAVENGE_ON_RESOLVE) && !(_jitConfig->runtimeFlags & J9JIT_TOSS_CODE)
        && !requireAsyncCompile && (oldStartPC || newInstanceThunkDetails))
...
#if defined(J9VM_OPT_SHARED_CLASSES)
extern bool validateSharedClassAOTHeader(J9JavaVM *javaVM, J9VMThread *curThread, TR::CompilationInfo *compInfo,
    TR_FrontEnd *fe);
#endif
...
void *TR::CompilationInfo::compileOnSeparateThread(J9VMThread *vmThread, TR::IlGeneratorMethodDetails &details,
    void *oldStartPC, TR_YesNoMaybe requireAsyncCompile, TR_CompilationErrorCode *compErrCode, bool *queued,
    TR_OptimizationPlan *optimizationPlan)
{
    void *startPC = NULL;
    ...
    if (startPC && !details.isJitDumpMethod() && !optimizationPlan->isGPUCompilation()) {
    ...
    if ((getNumCompThreadsActive() == 0
        ...
        } else // Similar request already exists in the queue. We cannot fail this compilation
        {
            ...
            if (TR::Options::isAnyVerboseOptionSet(TR_VerboseCompFailure, TR_VerboseCompileRequest)) {
                TR_VerboseLog::writeLineLocked(TR_Vlog_FAILURE,
                    "t=%u <WARNING: JIT Compilations are suspended. Current request will be coalesced with existing "
                    "one in the queue>",
        ...
        if (shouldReturn) {
            ...
            if (TR::Options::isAnyVerboseOptionSet(TR_VerboseCompFailure, TR_VerboseCompileRequest)) {
                TR_VerboseLog::writeLineLocked(TR_Vlog_FAILURE, "t=%u <WARNING: JIT Compilations are suspended>",
                    (uint32_t)getPersistentInfo()->getElapsedTime());
            }
    ...
    TR_ASSERT(asynchronousCompilation() || requireAsyncCompile != TR_yes,
        "We cannot require async compilation when it is turned off");
    ...
    if (async) {
        J9::PrivateLinkage::LinkageInfo *linkageInfo
            = oldStartPC ? J9::PrivateLinkage::LinkageInfo::get(oldStartPC) : 0;
        if (requireAsyncCompile != TR_yes && !oldStartPC && !details.isMethodInProgress()) {
            ...
            if (async) {
                ...
                if (J9UTF8_LENGTH(className) == 36
                    ...
                    if (J9UTF8_LENGTH(utf8) == 21 && 0 == memcmp(J9UTF8_DATA(utf8), "isVMDeepCopySupported", 21))
                        isORB = true;
                }
        ...
        if (!async) {
            ...
            if ((linkageInfo && linkageInfo->isBeingCompiled()) || // for jitted methods
                (oldStartPC == 0 && // for interpreted methods
                    ...
                    && getJ9MethodVMExtra(details.getMethod())
                        == J9_JIT_QUEUED_FOR_COMPILATION)) // this will also filter out JNI natives
            {
                // Release the compilation lock and return
            ...
            if (getMethodQueueSize() >= TR::Options::_qszLimit && oldStartPC == 0 && // Only look at interpreted methods
                details.isOrdinaryMethod()) // Do not apply this optimization to DLT
            {
                J9ROMMethod *romMethod = J9_ROM_METHOD_FROM_RAM_METHOD(method);
                if (!(romMethod->modifiers
                        & J9AccNative)) // We are not allowed to change the invocation count for native methods
                {
                    int32_t newCount = J9ROMMETHOD_HAS_BACKWARDS_BRANCHES(romMethod) ? TR_DEFAULT_INITIAL_BCOUNT
                                                                                     : TR_DEFAULT_INITIAL_COUNT;
                    bool success = TR::CompilationInfo::replenishInvocationCountIfExpired(method, newCount);
                    if (success) {
                        if (TR::Options::getCmdLineOptions()->getVerboseOption(TR_VerbosePerformance)) {
                            TR_VerboseLog::writeLineLocked(TR_Vlog_PERF,
                                "Reencoding count=%d for j9m=%p because Q_SZ is too large", newCount, method);
                        }
                        // Release the compilation lock and return
                        debugPrint(vmThread,
                            "\tapplication thread releasing compilation monitor - comp.req. in progress\n");
                        debugPrint(vmThread, "-CM\n");
                        releaseCompMonitor(vmThread);
                        if (compErrCode)
                            *compErrCode = compilationNotNeeded;
    ...
    if (details.isOrdinaryMethod() || details.isMethodInProgress()) {
        ...
        if (_J9ROMMETHOD_J9MODIFIER_IS_SET(romMethod, J9AccMethodFrameIteratorSkip)) {
            if (details.isMethodHandleThunk()) {
                ...
                debugPrint(vmThread,
                    "\tapplication thread releasing compilation monitor - trying to compile thunk archetype\n");
                debugPrint(vmThread, "-CM\n");
                releaseCompMonitor(vmThread);
                return 0; // In prod, it's safe just to fail to queue the method
            } else {
                TR_ASSERT(0,
                    "Should never attempt a JavaOrdinaryMethod compilation on a method with the "
                    "J9AccMethodFrameIteratorSkip flag");
    ...
    if (TR::Options::sharedClassCache() && !TR::Options::getAOTCmdLineOptions()->getOption(TR_NoLoadAOT)
        && details.isOrdinaryMethod()) {
        if (!isJNINative(method) && !isCompiled(method)) {
            ...
            //
            if (vmThread->javaVM->sharedClassConfig->existsCachedCodeForROMMethod(vmThread,
                    fe->getROMMethodFromRAMMethod(method))) {
                if (static_cast<TR_JitPrivateConfig *>(jitConfig->privateConfig)->aotValidHeader == TR_yes) {
                    ...
                    TR_ASSERT_FATAL(static_cast<TR_JitPrivateConfig *>(jitConfig->privateConfig)->aotValidHeader
                            != TR_maybe,
                        "Should not be possible for aotValidHeader to be TR_maybe at this point\n");
                }
            }
        }
    }
#endif // defined(J9VM_INTERP_AOT_RUNTIME_SUPPORT) && defined(J9VM_OPT_SHARED_CLASSES) && (defined(TR_HOST_X86) ||
    ...
    if (getLowCompDensityMode() && async && // do not postpone synchronous requests
        oldStartPC
        && // address only recompilations; first time compilations may be downgraded before placing an upgrade in LPQ
        ...
        if (TR::Options::isAnyVerboseOptionSet(TR_VerbosePerformance, TR_VerboseJITServer, TR_VerboseCompileRequest)) {
            TR_VerboseLog::writeLineLocked(TR_Vlog_CR,
                "t=%u Postponing recompilation for j9method=%p due to low compilation density mode",
    ...
    if (async) {
        if (details.isMethodInProgress())
            ...
            } else if (!TR::Options::getCmdLineOptions()->getOption(
                           TR_DisableReducedPriorityForCustomMethodHandleThunks)) {
            ...
            if (oldStartPC) // compiled method
            {
                ...
                // than later
                if (bodyInfo->getIsProfilingBody()
                    && !TR::Options::getCmdLineOptions()->getOption(TR_DisablePostProfileCompPriorityBoost)) {
                ...
                else if (optimizationPlan->isUpgradeRecompilation() || getPersistentInfo()->isClassLoadingPhase()
                ...
                // hot methods below 6% will be given lower priority
                    ...
                    && optimizationPlan->getPerceivedCPUUtil()
                        <= (TR::Options::_sampleInterval * 1000 / TR::Options::_veryHotSampleThreshold)
    ...
    && (defined(TR_HOST_X86) || defined(TR_HOST_POWER) || defined(TR_HOST_S390) || defined(TR_HOST_ARM) \
        ...
        if (TR::Options::getCmdLineOptions()->getOption(TR_EnableEarlyCompilationDuringIdleCpu) && oldStartPC == 0
            && // interpreted method
            compPriority <= CP_ASYNC_NORMAL && details.isOrdinaryMethod() && // exclude DLT and thunks
    ...
    TR_MethodToBeCompiled *entry = addMethodToBeCompiled(details, oldStartPC, compPriority, async, optimizationPlan,
        queued, methodIsInSharedCache);
...
#if defined(J9VM_JIT_DYNAMIC_LOOP_TRANSFER)
    if (details.isMethodInProgress() && !isCompiled(method) && *queued
    ...
    if (async) {
        ...
        if (getMethodQueueSize() <= 1
            || getNumCompThreadsJobless()
                > 0) // send notification if any thread is sleeping on comp monitor waiting for suitable work
        {
    ...
    } else // synchronous request
    {
        ...
        if (entry->hasChangedToLocalSyncComp() && (entry->_numThreadsWaiting <= 1) && !forcedSync
            ...
            if (!currOldStartPC) {
                ...
                if (plan) {
                    TR_MethodToBeCompiled *entryRemoteCompReq = addMethodToBeCompiled(details, startPC, compPriority,
                        isAsync, plan, &queuedForRemote, mthInSharedCache);

                    if (entryRemoteCompReq) {
                        entryRemoteCompReq->_async = isAsync;

                        if (getMethodQueueSize() <= 1
                            || getNumCompThreadsJobless() > 0) // send notification if any thread is sleeping on comp
                                                               // monitor waiting for suitable work
                        {
                            getCompilationMonitor()->notifyAll();
                        }
                        if (TR::Options::getJITCmdLineOptions()->getVerboseOption(TR_VerboseCompileRequest))
                            TR_VerboseLog::writeLineLocked(TR_Vlog_CR,
                                "%p   Queued a remote async compilation: entry=%p, j9method=%p", vmThread,
                                entryRemoteCompReq, entryRemoteCompReq->getMethodDetails().getMethod());
                    }
                    if (!queuedForRemote)
                        TR_OptimizationPlan::freeOptimizationPlan(plan);
                }
...
#ifdef TR_HOST_S390
void TR::CompilationInfoPerThreadBase::outputVerboseMMapEntry(TR_ResolvedMethod *compilee, const struct tm &date,
    const struct timeval &time, void *startPC, void *endPC, const char *fmt, const char *profiledString,
    const char *compileString)
{
    TR_VerboseLog::writeLine(TR_Vlog_MMAP, fmt, date.tm_hour, date.tm_min, date.tm_sec, time.tv_usec,
        compilee->classNameLength(), compilee->classNameChars(), compilee->nameLength(), compilee->nameChars(),
        compilee->signatureLength(), compilee->signatureChars(), "[", profiledString, compileString, "]", startPC,
        endPC);
}

void TR::CompilationInfoPerThreadBase::outputVerboseMMapEntries(TR_ResolvedMethod *compilee,
    TR_MethodMetaData *metaData, const char *profiledString, const char *compileString)
{
    ...
    gettimeofday(&time, 0);
    struct tm date;
    if (!localtime_r(&time.tv_sec, &date))
        return;

    TR_VerboseLog::CriticalSection vlogLock;
    outputVerboseMMapEntry(compilee, date, time, reinterpret_cast<void *>(metaData->startPC),
        reinterpret_cast<void *>(metaData->endWarmPC), fmtNoEOL, profiledString, compileString);
    if (metaData->startColdPC)
        outputVerboseMMapEntry(compilee, date, time, reinterpret_cast<void *>(metaData->startColdPC),
            reinterpret_cast<void *>(metaData->endPC), fmtEOL, profiledString, compileString);
}
...
#pragma option_override(TR::CompilationInfo::compile(J9VMThread *, TR_MethodToBeCompiled *, bool), "OPT(SPILL,256)")
    ...
    && (defined(TR_HOST_X86) || defined(TR_HOST_POWER) || defined(TR_HOST_S390) || defined(TR_HOST_ARM) \
        || defined(TR_HOST_ARM64))
TR_MethodMetaData *TR::CompilationInfoPerThreadBase::installAotCachedMethod(J9VMThread *vmThread,
    const void *aotCachedMethod, J9Method *method, TR_FrontEnd *fe, TR::Options *options, TR_ResolvedMethod *compilee,
    TR_MethodToBeCompiled *entry, TR::Compilation *compiler)
{
    ...
    metaData = reloRuntime()->prepareRelocateAOTCodeAndData(vmThread, fe, aotMCCRuntimeCodeCache, cacheEntry, method,
        false, options, compiler, compilee);
    ...
    if (metaData) {
        ...
        if (TrcEnabled_Trc_JIT_AotLoadEnd) {
            ...
            Trc_JIT_AotLoadEnd(vmThread, compiler->signature(), metaData->startPC, metaData->endWarmPC,
                metaData->startColdPC, metaData->endPC, reloTime, method, metaData, _compInfo.getMethodQueueSize(),
                TR::CompilationInfo::getMethodBytecodeSize(method), 0);
        }
        ...
        if (TR::Options::isAnyVerboseOptionSet(TR_VerbosePerformance, TR_VerboseCompileEnd)) {
            ...
            TR_VerboseLog::write(" @ " POINTER_PRINTF_FORMAT "-" POINTER_PRINTF_FORMAT, metaData->startPC,
                metaData->endWarmPC);
            TR_VerboseLog::write(" Q_SZ=%d Q_SZI=%d QW=%d j9m=%p bcsz=%u", _compInfo.getMethodQueueSize(),
                _compInfo.getNumQueuedFirstTimeCompilations(), _compInfo.getQueueWeight(), method,
                _compInfo.getMethodBytecodeSize(method));
        ...
        if (TR::Options::getVerboseOption(TR_VerboseMMap)) {
            ...
            TR_ResolvedMethod *compilee
                = vm->createResolvedMethod(compiler->trMemory(), (TR_OpaqueMethodBlock *)method);
        ...
        if (J9_EVENT_IS_HOOKED(_jitConfig->javaVM->hookInterface, J9HOOK_VM_DYNAMIC_CODE_LOAD)) {
            ...
            ALWAYS_TRIGGER_J9HOOK_VM_DYNAMIC_CODE_LOAD(_jitConfig->javaVM->hookInterface, vmThread, method,
                (U_8 *)metaData->startPC, metaData->endWarmPC - metaData->startPC, "JIT warm body", metaData);
            if (metaData->startColdPC) {
                // Register the cold code section too
                //
                ALWAYS_TRIGGER_J9HOOK_VM_DYNAMIC_CODE_LOAD(_jitConfig->javaVM->hookInterface, vmThread, method,
                    (U_8 *)metaData->startColdPC, metaData->endPC - metaData->startColdPC, "JIT cold body", metaData);
            }
            ccMethodHeader = getCodeCacheMethodHeader((char *)metaData->startPC, 32, metaData);
            if (ccMethodHeader && (metaData->bodyInfo != NULL)) {
                J9::PrivateLinkage::LinkageInfo *linkageInfo
                    = J9::PrivateLinkage::LinkageInfo::get((void *)metaData->startPC);
                if (linkageInfo->isRecompMethodBody()) {
                    ALWAYS_TRIGGER_J9HOOK_VM_DYNAMIC_CODE_LOAD(_jitConfig->javaVM->hookInterface, vmThread, method,
                        (U_8 *)((char *)ccMethodHeader->_eyeCatcher + 4),
                        metaData->startPC - (UDATA)((char *)ccMethodHeader->_eyeCatcher + 4), "JIT method header",
                        metaData);
                }
            }
        }
    ...
    } else // relocation failed
    {
        if (entry) {
            ...
            entry->_tryCompilingAgain = _compInfo.shouldRetryCompilation(vmThread, entry,
                compiler); // this will set entry->_doNotAOTCompile = true;

            // Feature [Defect 172216/180384]: Implement hints for failed AOT validations.  The idea is that the failed
            // validations are due to the fact that AOT methods are loaded at a lower count than when they were compiled
            // so the JVM is given less opportunity to resolve things. The new hint tells us that a previous relocation
            // failed a validation so specify a higher scount for the next run to give it more chance to resolve things.
...
{
#if defined(J9VM_INTERP_AOT_RUNTIME_SUPPORT) && defined(J9VM_OPT_SHARED_CLASSES) \
    && (defined(TR_HOST_X86) || defined(TR_HOST_POWER) || defined(TR_HOST_S390))

    TR_ASSERT(!originalEntry->isDLTCompile(), "No DLTs allowed in queueForcedAOTUpgrade"); // no DLT here
    TR_ASSERT(originalEntry->_compInfoPT && originalEntry->_compInfoPT->_methodBeingCompiled == originalEntry,
        "assertion failure");
    ...
    if (hints & TR_HintScorching) {
        hotness = veryHot;
        doProfiling
            = !TR::Options::getCmdLineOptions()->getOption(TR_DisableProfiling) && !methodInfo->profilingDisabled();
    ...
    } else // See if we should upgrade at warm
    {
        ...
        if (_jitConfig->javaVM->phase == J9VM_PHASE_NOT_STARTUP
            || !(hints & (TR_HintLargeMemoryMethodW | TR_HintLargeCompCPUW))) {
            if (!TR::Options::isQuickstartDetected())
                hotness = warm;
            else if (TR::Options::getCmdLineOptions()->getOption(
                         TR_UpgradeBootstrapAtWarm)) // This is a JIT option because it pertains to JIT compilations
            {
                // To reduce affect on short applications like tomcat
                // upgrades to warm should be performed only outside the grace period
                // if (getPersistentInfo()->getElapsedTime() >=
                // (uint64_t)getPersistentInfo()->getClassLoadingPhaseGracePeriod())
                {
                    TR_OpaqueMethodBlock *method
                        = (TR_OpaqueMethodBlock *)originalEntry->getMethodDetails().getMethod();
                    TR_J9VMBase *fej9 = (TR_J9VMBase *)fe;
...
#endif // #if defined(J9VM_INTERP_AOT_RUNTIME_SUPPORT) && defined(J9VM_OPT_SHARED_CLASSES) && (defined(TR_HOST_X86) ||
       // defined(TR_HOST_POWER) || defined(TR_HOST_S390))
}
...
void TR::CompilationInfoPerThreadBase::generatePerfToolEntry()
{
#if defined(LINUX)
    TR_ASSERT(getCompilation() && getMetadata(),
        "generatePerfToolEntry() must be executed only for successful compilations");
        ...
        if (!getPerfFile()) // couldn't open the file
        {
            if (TR::Options::getJITCmdLineOptions()->getVerboseOption(TR_VerboseCompFailure))
                TR_VerboseLog::writeLineLocked(TR_Vlog_FAILURE, "t=%u WARNING: Cannot open perf tool file: %s",
                    ...
                    perfFilename); // even if snprintf failed above we should still be ok.
            // Do we want a message without verbose log being specified?
        }
    }
    if (getPerfFile()) {
        j9jit_fprintf(getPerfFile(), "%p %lX %s_%s%s\n", getMetadata()->startPC,
            getMetadata()->endWarmPC - getMetadata()->startPC, getCompilation()->signature(),
            getCompilation()->getHotnessName(getCompilation()->getMethodHotness()),
        ...
        if (getMetadata()->startColdPC) {
            j9jit_fprintf(getPerfFile(), "%p %lX %s_%s%s\n", getMetadata()->startColdPC,
                getMetadata()->endPC - getMetadata()->startColdPC, getCompilation()->signature(),
                getCompilation()->getHotnessName(getCompilation()->getMethodHotness()),
                getMetadata()->flags & JIT_METADATA_IS_FSD_COMP ? "_fsd"
                                                                : ""); // should we change the name of the method?
        }
...
const void *TR::CompilationInfoPerThreadBase::findAotBodyInSCC(J9VMThread *vmThread, const J9ROMMethod *romMethod)
{
#if defined(J9VM_INTERP_AOT_RUNTIME_SUPPORT) && defined(J9VM_OPT_SHARED_CLASSES)                        \
    && (defined(TR_HOST_X86) || defined(TR_HOST_POWER) || defined(TR_HOST_S390) || defined(TR_HOST_ARM) \
        || defined(TR_HOST_ARM64))
#if defined(J9VM_OPT_JITSERVER)
    TR_ASSERT(!TR::CompilationInfo::getStream(),
        "This function should not be called at the server because SCC does not exist at the server.");
#endif /* defined(J9VM_OPT_JITSERVER) */
    UDATA flags = 0;
    const void *aotCachedMethod
        = vmThread->javaVM->sharedClassConfig->findCompiledMethodEx1(vmThread, romMethod, &flags);
...
bool TR::CompilationInfoPerThreadBase::isMemoryCheapCompilation(uint32_t bcsz, TR_Hotness optLevel)
{
    if (optLevel > warm) {
        return false;
    }
    ...
    {
        // All methods at noOpt, very small methods at cold/warm and medium sized methods at cold (expected to
        // take 2.5MB max)
...
bool TR::CompilationInfoPerThreadBase::isCPUCheapCompilation(uint32_t bcsz, TR_Hotness optLevel)
{
    ...
    if (cpuEntitlement < 150.0) // [100, 150)
    {
        ...
        if (cpuUtil->hasValidData() && cpuUtil->getCpuIdle() >= 15 && // There is idle CPU to use
...
bool TR::CompilationInfoPerThreadBase::cannotPerformRemoteComp(
#if defined(J9VM_OPT_CRIU_SUPPORT)
    J9VMThread *vmThread
#endif /* defined(J9VM_OPT_CRIU_SUPPORT) */
)
{
    ...
    return
        ...
        (_jitConfig->javaVM->internalVMFunctions->isCheckpointAllowed(_jitConfig->javaVM)
            && !_compInfo.getCRRuntime()->canPerformRemoteCompilationInCRIUMode())
        ||
#endif /* defined(J9VM_OPT_CRIU_SUPPORT) */
        suppressAllRemoteCompilations
        || !JITServer::ClientStream::isServerCompatible(OMRPORT_FROM_J9PORT(_jitConfig->javaVM->portLibrary))
        || (!JITServerHelpers::isServerAvailable()
            && !JITServerHelpers::shouldRetryConnection(OMRPORT_FROM_J9PORT(_jitConfig->javaVM->portLibrary)))
        || (!JITServer::CommunicationStream::shouldReadRetry()
            && !JITServerHelpers::shouldRetryConnection(OMRPORT_FROM_J9PORT(_jitConfig->javaVM->portLibrary)))
        || (TR::Compiler->target.cpu.isPower()
            && _jitConfig->inlineFieldWatches); // Power codegen is missing some FieldWatch relocation support
}

bool TR::CompilationInfoPerThreadBase::preferLocalComp(const TR_MethodToBeCompiled *entry)
{
    if (_compInfo.getPersistentInfo()->isLocalSyncCompiles() && (entry->_optimizationPlan->getOptLevel() <= cold)
        && !entry->_async) {
        return true;
    }
...
void TR::CompilationInfoPerThreadBase::enterPerClientAllocationRegion()
{
...
void TR::CompilationInfoPerThreadBase::exitPerClientAllocationRegion()
{
    if (_compiler)
...
void TR::CompilationInfoPerThreadBase::downgradeLocalCompilationIfLowPhysicalMemory(TR_MethodToBeCompiled *entry)
{
    ...
    if (!TR::Options::getCmdLineOptions()->getOption(TR_DisableJITServerBufferedExpensiveCompilations)
        && TR::Options::getCmdLineOptions()->allowRecompilation() && !TR::CompilationInfo::isCompiled(method)
        && // do not downgrade recompilations
        ...
        if (freePhysicalMemorySizeB != OMRPORT_MEMINFO_NOT_AVAILABLE
            && freePhysicalMemorySizeB <= (uint64_t)TR::Options::getSafeReservePhysicalMemoryValue()
                    + (numActiveThreads + 4) * TR::Options::getScratchSpaceLowerBound()) {
            if (TR::Options::isAnyVerboseOptionSet(TR_VerboseJITServer, TR_VerboseCompilationDispatch,
                    TR_VerbosePerformance, TR_VerboseCompFailure))
                TR_VerboseLog::writeLineLocked(TR_Vlog_JITServer,
                    "t=%6u Downgraded a forced local compilation to cold due to low memory: j9method=%p",
 ...
 * @param eligibleForRelocatableCompile Reference to a bool; Indicates if the current compilation is eligible for AOT
 * (output)
 ...
 */
void TR::CompilationInfoPerThreadBase::preCompilationTasks(J9VMThread *vmThread, TR_MethodToBeCompiled *entry,
    J9Method *method, const void **aotCachedMethod, TR_Memory &trMemory, bool &canDoRelocatableCompile,
    bool &eligibleForRelocatableCompile, TR_RelocationRuntime *reloRuntime)
{
    ...
    //
    entry->setAotCodeToBeRelocated(
        NULL); // make sure decision to load AOT comes from below and not previous compilation/relocation pass
    ...
    if (entry->_methodIsInSharedCache == TR_yes // possible AOT load
        && !TR::CompilationInfo::isCompiled(method) && !entry->_doNotAOTCompile
        && !TR::Options::getAOTCmdLineOptions()->getOption(TR_NoLoadAOT)
        && !(_jitConfig->runtimeFlags & J9JIT_TOSS_CODE) && !_jitConfig->inlineFieldWatches) {
        ...
        if (debug && !entry->isOutOfProcessCompReq()) {
            setCompilationShouldBeInterrupted(
                0); // zero the flag because createResolvedMethod calls
        ...
        if (canRelocateMethod && !entry->_oldStartPC) {
            ...
            *aotCachedMethod = findAotBodyInSCC(vmThread, entry->getMethodDetails().getRomMethod(fe));
            ...
            if (*aotCachedMethod && !fe->sharedCache()->classMatchesCachedVersion(J9_CLASS_FROM_METHOD(method))) {
                if (TR::Options::getCmdLineOptions()->getVerboseOption(TR_VerbosePerformance)) {
                    TR_VerboseLog::writeLineLocked(TR_Vlog_PERF,
                        "Failed to validate the class chain of the class of method %p", method);
                }
            ...
            if (*aotCachedMethod) {
#ifdef COMPRESS_AOT_DATA
                TR_AOTMethodHeader *aotMethodHeader
                    = (TR_AOTMethodHeader *)(((J9JITDataCacheHeader *)*aotCachedMethod) + 1);
                if (aotMethodHeader->flags & TR_AOTMethodHeader_CompressedMethodInCache) {
                     ...
                     * ---------------------------------------------------------------------------------------
                     * When we compile and store a method, it stores J9RomMethod for the method in the
                     * CompiledMethodWrapper along side size of metadata and codedata in the cache. Now when we request
                     * a method from shared class cache it returns address that points to J9JITDataCacheHeader. As while
                     * decompressing the data, we need information about the size of original data (We get that
                     * information from AOTMethodHeader) and size of deflated data in the cache (Which is stored in
                    ...
                    int originalDataSize
                        = aotMethodHeader->compileMethodDataSize + aotMethodHeader->compileMethodCodeSize;
                    ...
                    if (inflateBuffer((U_8 *)(*aotCachedMethod) + aotMethodHeaderSize,
                            sizeOfCompressedDataInCache - aotMethodHeaderSize,
                            (U_8 *)(originalData) + aotMethodHeaderSize, originalDataSize - aotMethodHeaderSize)
                        ...
                        if (TR::Options::getVerboseOption(TR_VerboseAOTCompression)) {
                            ...
                            J9UTF8 *signature;
                            getClassNameSignatureFromMethod(method, className, name, signature);
                            TR_VerboseLog::writeLineLocked(TR_Vlog_AOTCOMPRESSION,
                                "!%.*s.%.*s%.*s : Decompression of method data failed - Compressed Method Size = %d "
                                "bytes, Stored original method size = %d bytes",
                        ...
                        *aotCachedMethod = NULL;
                        ...
                        if (TR::Options::getVerboseOption(TR_VerboseAOTCompression)) {
                            ...
                            getClassNameSignatureFromMethod(method, className, name, signature);
                            TR_VerboseLog::writeLineLocked(TR_Vlog_AOTCOMPRESSION,
                                "%.*s.%.*s%.*s : Decompression of method data Successful - Compressed Method Size = %d "
                                "bytes, Stored original method size = %d bytes",
                                J9UTF8_LENGTH(className), (char *)J9UTF8_DATA(className), J9UTF8_LENGTH(name),
                                (char *)J9UTF8_DATA(name), J9UTF8_LENGTH(signature), (char *)J9UTF8_DATA(signature),
                                sizeOfCompressedDataInCache, originalDataSize);
      ...
      // All conditions are met for doing an AOT load. Mark this fact on the entry.
            ...
            J9JITExceptionTable *oldMetaData
                = jitConfig->jitGetExceptionTableFromPC(vmThread, (UDATA)entry->_oldStartPC);
            if (oldMetaData) {
                TR_PersistentJittedBodyInfo *oldBodyInfo = (TR_PersistentJittedBodyInfo *)oldMetaData->bodyInfo;
                TR_ASSERT(oldBodyInfo && oldBodyInfo->getIsInvalidated(),
                    "entry is interpreted and has a non-NULL _oldStartPC but the old body has not been invalidated.");
            }
        }
    }
#if defined(J9VM_OPT_JITSERVER)
    ...
    if (TR::Options::getDebug() && !cannotDoRemoteCompilation
        && _compInfo.getPersistentInfo()->getRemoteCompilationMode() == JITServer::CLIENT) {
        setCompilationShouldBeInterrupted(
            0); // zero the flag because createResolvedMethod can
                // acquire/releaseVMaccessIfNeeded and may see the flag set by previous compilation
        TR_ResolvedMethod *resolvedMethod = fe->createResolvedMethod(&trMemory, (TR_OpaqueMethodBlock *)method);
        cannotDoRemoteCompilation = !_compInfo.methodCanBeRemotelyCompiled(resolvedMethod->signature(&trMemory),
            resolvedMethod->convertToMethod()->methodType());
    }
#endif /* defined(J9VM_OPT_JITSERVER) */
    ...
    if (!entry->isAotLoad()) // We don't/can't relocate this method
    {
        ...
        if (entry->isOutOfProcessCompReq()) {
            ...
            bool withoutSCCEligibleForRelocatableCompile =
                ...
                !entry->isJNINative() && !details.isNewInstanceThunk() && !details.isMethodHandleThunk()
                && !entry->isDLTCompile()
                ...
                && (!TR::Options::getCmdLineOptions()->getOption(TR_FullSpeedDebug) || !entry->_oldStartPC)
            ...
            eligibleForRelocatableCompile = withoutSCCEligibleForRelocatableCompile
                ...
                && (TR_SharedCache::INVALID_CLASS_CHAIN_OFFSET
                    != fe->sharedCache()->rememberClass(J9_CLASS_FROM_METHOD(method)));

#if defined(J9VM_OPT_JITSERVER)
            eligibleForRemoteAOTNoSCCCompile = withoutSCCEligibleForRelocatableCompile
                && persistentInfo->getRemoteCompilationMode() == JITServer::CLIENT
                && persistentInfo->getJITServerUseAOTCache() // Ideally we would check if the options allow us to use
                                                             // the AOT cache for this method
                && persistentInfo->getJITServerAOTCacheIgnoreLocalSCC() // Need to be using the new implementation
                && !entry->_doNotLoadFromJITServerAOTCache && !persistentInfo->doNotRequestJITServerAOTCacheStore()
                && !_compInfo.getLowCompDensityMode() // AOT req is sent to JITServer and we don't want to send
                                                      // JITServer any messages in this mode
        ...
        if (sharedClassTest) {
            ...
            } else // Use AOT heuristics
            {
                ...
                if ((!isSecondAOTRun && entry->_optimizationPlan->isOptLevelDowngraded()
                        && !_compInfo.getLowCompDensityMode())
                    ...
                    if (persistentInfo->getRemoteCompilationMode() == JITServer::CLIENT
                        ...
                        && // Ideally we would check if the options allow us to use the AOT cache for this method
                        !_compInfo.getLowCompDensityMode()
                        && // AOT req is sent to JITServer and we don't want to send JITServer any messages in this mode
                        !cannotDoRemoteCompilation) // Make sure remote compilations are possible (no strong guarantees)
                    {
                        if (!preferLocalComp(entry)) {
                            ...
                            if (J9ROMMETHOD_HAS_BACKWARDS_BRANCHES(romMethod)
                                || TR::CompilationInfo::getMethodBytecodeSize(method)
                                    >= TR::Options::_smallMethodBytecodeSizeThresholdForJITServerAOTCache) {
                                canDoRelocatableCompile = true;
                            }
                        }
                    }
    ...
    if (canDoRelocatableCompile) {
        ...
        TR_ASSERT(!entry->_useAotCompilation,
            "Client requests AOT compilation but server cannot do remote AOT compilation\n");
    } else {
        ...
        if ((persistentInfo->getRemoteCompilationMode() == JITServer::CLIENT) && TR::Options::canJITCompile()) {
            bool doLocalCompilation
                = entry->isAotLoad() || cannotDoRemoteCompilation || preferLocalComp(entry) || entry->isJNINative();
            ...
            TR::IlGeneratorMethodDetails &details = entry->getMethodDetails();
            if (!doLocalCompilation && !entry->_async && !entry->isJNINative()
                && entry->_optimizationPlan->getOptLevel() > cold && !details.isNewInstanceThunk()
                ...
                && TR::Options::getCmdLineOptions()->allowRecompilation() && !details.isMethodInProgress()
                ...
                if (TR::Options::isAnyVerboseOptionSet(TR_VerbosePerformance, TR_VerboseJITServer,
                    ...
                    TR_VerboseLog::writeLineLocked(TR_Vlog_DISPATCH,
                        "t=%u Changed the remote sync compilation to a local sync cold compilation: j9method=%p",
            ...
            if (!doLocalCompilation
                && (_compInfo.getLowCompDensityMode()
                    || _compInfo.hasEnteredLowCompDensityModeInThePast()
                        && _jitConfig->javaVM->internalVMFunctions->getVMRuntimeState(_jitConfig->javaVM)
                            == J9VM_RUNTIME_STATE_IDLE)
                && entry->_optimizationPlan->getOptLevel() <= warm) {
                ...
                } else // warm compilations here
                {
                    ...
                    if (!TR::CompilationInfo::isCompiled(method)
                        ...
                        if (TR::Options::isAnyVerboseOptionSet(TR_VerbosePerformance, TR_VerboseJITServer,
                                TR_VerboseCompilationDispatch)) {
                            TR_VerboseLog::writeLineLocked(TR_Vlog_DISPATCH,
                                "t=%u Changed remote warm compilation into local cold for j9method=%p due to low "
                                "compilation density mode",
                                (uint32_t)persistentInfo->getElapsedTime(), method);
                        }
                        // Add an upgrade recomp if possible
                        if (!entry->isJNINative() && !details.isNewInstanceThunk() && !details.isMethodHandleThunk()
                            && !details.isMethodInProgress()
                            && !TR::Options::getCmdLineOptions()->getOption(TR_MimicInterpreterFrameShape)) {
                            static char *dontUpgrade = feGetEnv("TR_DontUpgradeDuringLowCompDensityMode");
                            if (!dontUpgrade) {
                                entry->_optimizationPlan->setOptLevelDowngraded(true);
                                // Mark the optimization plan to place an upgrade request in LPQ at the end of this
                                // compilation
                                entry->_optimizationPlan->setAddToUpgradeQueue();
                                if (TR::Options::isAnyVerboseOptionSet(TR_VerbosePerformance, TR_VerboseJITServer,
                                        TR_VerboseCompilationDispatch)) {
                                    TR_VerboseLog::writeLineLocked(TR_Vlog_DISPATCH,
                                        "t=%u Will enqueue an upgrade compilation into LPQ for j9method=%p",
                                        (uint32_t)persistentInfo->getElapsedTime(), method);
                                }
                            }
                        }
 ...
 */
void *TR::CompilationInfoPerThreadBase::postCompilationTasks(J9VMThread *vmThread, TR_MethodToBeCompiled *entry,
    J9Method *method, const void *aotCachedMethod, TR_MethodMetaData *metaData, bool canDoRelocatableCompile,
    bool eligibleForRelocatableCompile, TR_RelocationRuntime *reloRuntime)
{
#if defined(J9VM_OPT_JITSERVER)
    ...
    if (_compiler != NULL && _compiler->getKnownObjectTable() != NULL) {
        ...
        TR_ASSERT_FATAL(_compiler->isOutOfProcessCompilation(),
    ...
    if (TR::Options::getCmdLineOptions()->getOption(TR_PerfTool) && _compiler && startPC != 0
        && startPC != entry->_oldStartPC) {
    ...
    if (_compiler) {
        ...
        if (dataCache) {
            ...
            if (entry->_tryCompilingAgain && entry->_compErrCode == compilationAotCacheFullReloFailure) {
                TR_ASSERT(canDoRelocatableCompile,
                    "Only AOT compilations can fail with compilationAotCacheFullReloFailure. entry=%p\n", entry);
 ...
 */
void *TR::CompilationInfoPerThreadBase::compile(J9VMThread *vmThread, TR_MethodToBeCompiled *entry,
    J9::J9SegmentProvider &scratchSegmentProvider)
{
    ...
    try {
        ...
        if (TR::Options::getVerboseOption(TR_VerboseCompilationDispatch))
            TR_VerboseLog::writeLineLocked(TR_Vlog_DISPATCH,
                "Compilation thread executing compile(): j9method=%p isAotLoad=%d canDoRelocatableCompile=%d "
                "eligibleForRelocatableCompile=%d isRemoteCompReq=%d _doNotAOTCompile=%d AOTfe=%d isDLT=%d",
                method, entry->isAotLoad(), canDoRelocatableCompile, eligibleForRelocatableCompile,
                entry->isRemoteCompReq(), entry->_doNotAOTCompile, _vm->isAOT_DEPRECATED_DO_NOT_USE(),
                entry->isDLTCompile());

        if ((TR::Options::canJITCompile() || canDoRelocatableCompile || entry->isAotLoad() || entry->isRemoteCompReq())
            ...
            U_32 flags = J9PORT_SIG_FLAG_MAY_RETURN | J9PORT_SIG_FLAG_SIGSEGV | J9PORT_SIG_FLAG_SIGFPE
            ...
            if (entry->getMethodDetails().isJitDumpMethod()) {
                ...
                protectedResult = j9sig_protect(wrappedCompile, static_cast<void *>(&compParam), jitDumpSignalHandler,
                    vmThread, flags, &result);
            } else {
                protectedResult = j9sig_protect(wrappedCompile, static_cast<void *>(&compParam), jitSignalHandler,
                    vmThread, flags, &result);
            }
...
// static method
TR_MethodMetaData *TR::CompilationInfoPerThreadBase::wrappedCompile(J9PortLibrary *portLib, void *opaqueParameters)
{
    ...
    try {
        ...
        if (details.isMethodHandleThunk()) {
            ...
            compilee = vm->createResolvedMethod(p->trMemory(), method, NULL,
                (TR_OpaqueClassBlock *)niDetails.classNeedingThunk());
        } else {
        ...
        if (!that->_methodBeingCompiled->isOutOfProcessCompReq()
            && !that->methodCanBeCompiled(p->trMemory(), vm, compilee, filterInfo)) {
            ...
            if (that->_methodBeingCompiled->isOutOfProcessCompReq()) {
                ...
                TR_ASSERT_FATAL(compInfoPTRemote->getClientOptions(),
...
#endif /* defined(J9VM_OPT_JITSERVER) */
            {
                ...
                TR_ASSERT(!that->_methodBeingCompiled->isOutOfProcessCompReq(),
                    "JITServer should not change options passed by client");
#endif /* defined(J9VM_OPT_JITSERVER) */
                ...
                if (that->_methodBeingCompiled->_useAotCompilation) {
                    ...
                    if ((TR::Options::getCmdLineOptions()->getAggressivityLevel() == TR::Options::AGGRESSIVE_AOT
                            ...
                            || (!TR::Compiler->target.cpu.isPower()
                                && // Temporary change until we figure out the AOT bug on PPC
                        ...
                        && (TR::Options::getCmdLineOptions()->getOption(
                ...
                //
                options = new (p->trMemory(), heapAlloc) TR::Options(p->trMemory(), optionSetIndex, lineNumber,
                ...
                if (that->_methodBeingCompiled->isRemoteCompReq()) {
                    ...
                    // Infrastructure to support the TOC is currently not available for Remote Compilations. We disable
                    // the feature here so that the codegen doesn't generate TOC enabled code as it won't be valid on
                    // the client JVM.
                    options->setOption(TR_DisableTOC);
                }
                // Determine if known annotations exist and if so, keep annotations enabled
                if (!that->_methodBeingCompiled->isAotLoad() && !vm->isAOT_DEPRECATED_DO_NOT_USE()
                    && options->getOption(TR_EnableAnnotations)) {
                    if (!TR_AnnotationBase::scanForKnownAnnotationsAndRecord(&that->_compInfo, details.getMethod(),
                            vmThread->javaVM, vm))
                        options->setOption(TR_EnableAnnotations, false);
                }
                ...
                if (vm->isAOT_DEPRECATED_DO_NOT_USE()) {
                    // Disable dynamic literal pool for AOT because of an unresolved data snippet patching issue in
                    // which the "Address Of Ref. Instruction" in the unresolved data snippet points to the wrong load
                    // instruction
                    ...
                    // Do not delay relocations for JITServer client when server side AOT caching is used (gives better
                    // performance) Testing the presence of the deserializer is sufficient, because the deserializer is
                    // only created at the client and only if server side AOT caching is enabled
#if defined(J9VM_OPT_JITSERVER)
                ...
                if (options->getOption(TR_DisablePrexistenceDuringGracePeriod)) {
                    if (that->getCompilationInfo()->getPersistentInfo()->getElapsedTime()
                        < that->getCompilationInfo()->getPersistentInfo()->getClassLoadingPhaseGracePeriod())
                        options->setDisabled(OMR::invariantArgumentPreexistence, true);
                }
                ...
                {
                    J9Method *method = details.getMethod();
                    // See if we need to profile first level compilations and if we can do it, change the optimization
                    // plan
                    //
                    if (options->getOption(TR_FirstLevelProfiling) && !that->_methodBeingCompiled->isDLTCompile()
                        && // filter out DLTs
                        ...
                        && options->canJITCompile() && !options->getOption(TR_DisableProfiling)
                        && !options->getOption(TR_NoRecompile) && options->allowRecompilation()
                        && // don't do it for fixed opt level
                        ...
                        // TODO: should we prevent SamplingJProfiling for AOT bodies?
                        {
                            ...
                            if (options->getOption(TR_UseSamplingJProfilingForInterpSampledMethods)) {
                                int32_t skippedCount
                                    = that->getCompilationInfo()->getInterpSamplTrackingInfo()->findAndDelete(method);
                                if (skippedCount > 0) {
                                    // Enable SamplingJProfiling
                                    options->setDisabled(OMR::samplingJProfiling, false);
                                    // Determine the count
                            ...
                            if (!options->isDisabled(OMR::samplingJProfiling) &&
                                ...
                                // TODO: should we disable sampling to prevent upgrades before the method collects
                                // enough profiling info?
                            }
                        }
                    }
                    ...
                    if (that->_methodBeingCompiled->_reqFromJProfilingQueue) {
                        doJProfile = true;
                        if (TR::Options::getCmdLineOptions()->getVerboseOption(TR_VerbosePerformance))
                            TR_VerboseLog::writeLineLocked(TR_Vlog_INFO, "t=%6u Processing req from JPQ",
                                (uint32_t)that->getCompilationInfo()->getPersistentInfo()->getElapsedTime());
                    } else {
                    ...
                    // JProfiling may be enabled if TR_EnableJProfilingInProfilingCompilations is set and its a
                    // profiling compilation. See optimizer/JProfilingBlock.cpp
                    if (!doJProfile) {
                    ...
                    if (that->_methodBeingCompiled->_oldStartPC != 0) {
                        TR_PersistentJittedBodyInfo *bodyInfo
                            = TR::Recompilation::getJittedBodyInfoFromPC(that->_methodBeingCompiled->_oldStartPC);
                        if (bodyInfo) {
                            TR_PersistentMethodInfo *methodInfo = bodyInfo->getMethodInfo();
                            if (methodInfo->getReasonForRecompilation()
                                == TR_PersistentMethodInfo::RecompDueToInlinedMethodRedefinition)
                    ...
                    if (jitConfig->javaVM->phase != J9VM_PHASE_NOT_STARTUP
                        ...
                        if (options->getOptLevel() < warm) {
                            if (!vm->isAOT_DEPRECATED_DO_NOT_USE()) {
                                ...
                                // first run
                                options->setDumbInlinerBytecodeSizeCutoff(
                                    that->getCompilationInfo()->computeDynamicDumbInlinerBytecodeSizeCutoff(options));
                        ...
                        if (!options->getOption(TR_NoOptServer) && !options->getOption(TR_DisableSelectiveNoOptServer)
                            ...
                            } else // recompilation requests
                            {
                                ...
                                } else // recompilations triggered through GCR need to be cheaper
                                {
                                    ...
                                    //
                                    TR_PersistentJittedBodyInfo *bodyInfo = TR::Recompilation::getJittedBodyInfoFromPC(
                                        that->_methodBeingCompiled->_oldStartPC);
                                    if (bodyInfo->getMethodInfo()->getReasonForRecompilation()
                                        == TR_PersistentMethodInfo::RecompDueToGCR) {
                                        if (bodyInfo->getHotness()
                                            < options->getOptLevel()) // prevent warm+NoOptServer --> warm+NoOptServer
                                                                      // transitions
                                        {
                                            options->setOption(TR_NoOptServer);
                                        }
                        ...
                        if (options->getOption(TR_VaryInlinerAggressivenessWithTime)) {
                            int32_t inlAggr
                                = that->getCompilationInfo()->getPersistentInfo()->getInliningAggressiveness();
                            if (inlAggr != 100 && options->getOptLevel() == warm && !vm->isAOT_DEPRECATED_DO_NOT_USE()
                                && TR::Options::getCmdLineOptions()->getAggressivityLevel()
                                    == TR::Options::AGGRESSIVE_AOT) {
                    ...
                    // However, we should perform such expensive opts for AOT compilations or precheckpoint or under
                    // -Xtune:throughput
                    if (options->getOptLevel() <= warm) {
                        ...
                        bool enableExpensiveOptsAtWarm = forceExpensiveOptsAtWarm || // override
                            (vm->isAOT_DEPRECATED_DO_NOT_USE() && !that->_compInfo.isJVMStarved())
                            || // AOT compilations, but not under starvation conditions
#if defined(J9VM_OPT_CRIU_SUPPORT)
                            ...
                            ||
#endif /* defined(J9VM_OPT_CRIU_SUPPORT) */
                            TR::Options::getAggressivityLevel()
                                == TR::Options::TR_AggresivenessLevel::AGGRESSIVE_THROUGHPUT;
                ...
                TR_ASSERT(TR::comp() == NULL,
                    "there seems to be a current TLS TR::Compilation object %p for this thread. At this point there "
                    "should be no current TR::Compilation object",
                    TR::comp());
                ...
                if (TR::Options::getAggressivityLevel() == TR::Options::TR_AggresivenessLevel::AGGRESSIVE_THROUGHPUT
                    && options->getOptLevel() > warm && !options->getOption(TR_ProcessHugeMethods)) {
            ...
            if (details.isJitDumpMethod() && options->getDebug()) {
                ...
                if (jitDumpDetails.getOptionsFromOriginalCompile() != NULL) {
                    ...
                    //
                    // Ideally we would just copy construct the current options from the original crashed compile,
                    // however this is currently not possible. Although a copy constructor exists, it is not a good idea
                    // to use it here due to a number of problems:
                    //
                    // 1. The non-copy constructor takes care of initializing tracing, among other things, which may
                    // have
                    //    not been active on the original compilation. There is currently no easy way for us to
                    //    reinitialize such logic.
                    //
                    // 2. Options can be set at any point during the compilation, and decisions to set options can be
                    // based
                    //    off of whether other options are set or not. This is very problematic. For example during the
                    //    original compilation we may have set option A at some point X during the compilation thus
                    //    changing the value of that option from that point onward. This means that if we were to copy
                    //    construct the set of options from the original compile right here for the JitDump compilation
                    //    then option A would yield a different value from the start of the compilation until point X.
                    //
                    // This should eventually be improved if the options framework is ever simplified to contain only
                    // option values, not things like optimization plans, start PCs, compile thread IDs, etc.
                    //
                    // Because of this limitation we do our best to only copy options which are known to be timing
                    // sensitive and that could change between the JitDump compilation and the original compilation.
                    // This is not a silver bullet, and should be updated as we encounter more sources of
                    ...
                    options->setOption(TR_UseSymbolValidationManager,
                        optionsFromOriginalCompile->getOption(TR_UseSymbolValidationManager));
                    options->setOption(TR_DisableGuardedCountingRecompilations,
                        optionsFromOriginalCompile->getOption(TR_DisableGuardedCountingRecompilations));
                    options->setOption(TR_DisableNextGenHCR,
                        optionsFromOriginalCompile->getOption(TR_DisableNextGenHCR));
                }
            ...
            // The following code effectively replaces the cpu with client's cpu through the getProcessorDescription()
            // that has JITServer support
            TR::Environment target = TR::Compiler->target;
#if defined(J9VM_OPT_JITSERVER)
            if (that->_methodBeingCompiled->isOutOfProcessCompReq()) {
                // Customize target.cpu based on the processor description fetched from the client
                OMRProcessorDesc JITClientProcessorDesc = that->getClientData()
                                                              ->getOrCacheVMInfo(that->_methodBeingCompiled->_stream)
            ...
            compiler = new (p->trMemory(), heapAlloc)
                TR::Compilation(that->getCompThreadId(), vmThread, vm, compilee, p->_ilGenRequest, *options,
                    p->_dispatchRegion, p->trMemory(), p->_optimizationPlan, reloRuntime, &target
#if defined(J9VM_OPT_JITSERVER)
                    ,
                    p->_numPermanentLoaders, that->_methodBeingCompiled->isRemoteCompReq(),
            ...
            TR_ASSERT(TR::comp() == compiler,
                "the TLS TR::Compilation object %p for this thread does not match the one %p just created.", TR::comp(),
                compiler);
            ...
            if (compiler) {
                ...
                else if (TR::Options::getAggressivityLevel()
                        == TR::Options::TR_AggresivenessLevel::AGGRESSIVE_THROUGHPUT
                    && compiler->getOptions()->getOptLevel() > warm
                    && TR::Options::getScratchSpaceLimitForHotCompilations()
                        > proposedScratchMemoryLimit) // Make sure we don't decrease the value proposed so far
                {
                ...
                if (compiler->getOption(TR_EnableSelfTuningScratchMemoryUsageBeforeCompile)) {
                    ...
                    uint64_t physicalLimit = that->_compInfo.computeFreePhysicalLimitAndAbortCompilationIfLow(compiler,
                        incompleteInfo, TR::Options::getScratchSpaceLowerBound());
                    ...
                    if (physicalLimit != OMRPORT_MEMINFO_NOT_AVAILABLE) {
                        ...
                        if (compiler->isOutOfProcessCompilation())
                            physicalLimit = std::max(physicalLimit >> 1,
                                static_cast<uint64_t>(TR::Options::getScratchSpaceLowerBound()));
#endif
                        if (proposedScratchMemoryLimit > physicalLimit) {
                            if (incompleteInfo) {
                                ...
                                if (isJSR292) {
                                    proposedScratchMemoryLimit
                                        = (physicalLimit >= scratchSegmentProvider.allocationLimit()
                                                ? physicalLimit
                ...
                size_t proposedCapped
                    = proposedScratchMemoryLimit > UINT_MAX ? UINT_MAX : (size_t)proposedScratchMemoryLimit;
        ...
        try {
            ...
            if (TR::Options::getVerboseOption(TR_VerboseJITServer))
                TR_VerboseLog::writeLineLocked(TR_Vlog_JITServer,
                    "<EARLY TRANSLATION FAILURE: JITServer stream failure>");
            ...
            if (TR::Options::getVerboseOption(TR_VerboseJITServer))
                TR_VerboseLog::writeLineLocked(TR_Vlog_JITServer,
                    "<EARLY TRANSLATION FAILURE: compilation interrupted by JITClient>");
    ...
    if (compiler) {
        if (compiler->getRecompilationInfo()) {
            // if the compiler is created successfully and the method supports recompilation, we can set this field on
            // the bodyInfo
            ...
            if (bodyInfo) {
                ...
                isEDOCompilation
                    = bodyInfo->getMethodInfo()->getReasonForRecompilation() == TR_PersistentMethodInfo::RecompDueToEdo;
            }
        }
    ...
    try {
        ...
        TR_J9SharedCache *sc = (TR_J9SharedCache *)(vm->sharedCache());
        if (metaData && !that->_methodBeingCompiled->isDLTCompile() && !that->_methodBeingCompiled->isAotLoad()
            && sc) // skip AOT loads
        {
            ...
            if (TR::Compiler->vm.isVMInStartupPhase(jitConfig)) {
                ...
                if (that->getCompilationInfo()->getMethodQueueSize() - that->getQszWhenCompStarted()
                    > TR::Options::_cpuExpensiveCompThreshold) {
                    TR_Hotness hotness = that->_methodBeingCompiled->_optimizationPlan->getOptLevel();
                    if (hotness <= cold) {
        ...
        if (TR::Options::getVerboseOption(TR_VerboseJITServer))
            TR_VerboseLog::writeLineLocked(TR_Vlog_JITServer, "compThreadID=%d JITServer StreamFailure: %s",
                that->getCompThreadId(), e.what());
        Trc_JITServerStreamFailure(vmThread, that->getCompThreadId(), __FUNCTION__, compiler->signature(),
...
static bool foundJ9SharedDataForMethod(TR_OpaqueMethodBlock *method, TR::Compilation *comp, J9JITConfig *jitConfig)
{
...
TR_MethodMetaData *TR::CompilationInfoPerThreadBase::performAOTLoad(J9VMThread *vmThread, TR::Compilation *compiler,
    TR_ResolvedMethod *compilee, TR_J9VMBase *vm, J9Method *method)
{
    TR_FilterBST *filterInfo = NULL;
    ...
    if (TR::Options::getVerboseOption(TR_VerboseCompilationDispatch)) {
        TR_VerboseLog::writeLineLocked(TR_Vlog_DISPATCH, "Loading previously AOT compiled body for %s @ %s",
            compiler->signature(), compiler->getHotnessName());
    }

    omrthread_jit_write_protect_disable();
    TR_MethodMetaData *metaData = installAotCachedMethod(vmThread, _methodBeingCompiled->_aotCodeToBeRelocated, method,
        loadvm, compiler->getOptions(), compilee, _methodBeingCompiled, compiler);
    omrthread_jit_write_protect_enable();
    ...
    {
        if (TR::Options::getVerboseOption(TR_VerboseCompilationDispatch)) {
            TR_VerboseLog::writeLineLocked(TR_Vlog_DISPATCH, "Failed to load previously AOT compiled body for %s @ %s",
                compiler->signature(), compiler->getHotnessName());
        }
        ...
        if (TR::Options::getVerboseOption(TR_VerboseCompilationDispatch)) {
            TR_VerboseLog::writeLineLocked(TR_Vlog_DISPATCH,
                "Successfully loaded previously AOT compiled body for %s @ %s", compiler->signature(),
            ...
            if (sc && !foundJ9SharedDataForMethod(compilee->getPersistentIdentifier(), compiler, _jitConfig)) {
                ...
                for (int32_t i = 0; i < numCallSites; i++) {
                    ...
                    if (!isPatchedValue((J9Method *)j9method)) {
                        TR_ResolvedJ9Method resolvedj9method
                            = TR_ResolvedJ9Method(j9method, compiler->fej9(), compiler->trMemory());
...
TR_MethodMetaData *TR::CompilationInfoPerThreadBase::compile(J9VMThread *vmThread, TR::Compilation *compiler,
    TR_ResolvedMethod *compilee, TR_J9VMBase &vm, TR_OptimizationPlan *optimizationPlan,
    TR::SegmentAllocator const &scratchSegmentProvider)
{
    ...
    if (_methodBeingCompiled->isDLTCompile())
        compiler->setDltBcIndex(
            static_cast<J9::MethodInProgressDetails &>(_methodBeingCompiled->getMethodDetails()).getByteCodeIndex());
    ...
    try {
        ...
        if (_jitConfig->tracingHook)
            compiler->setDebug(((TR_CreateDebug_t)_jitConfig->tracingHook)(compiler));
        if (!_methodBeingCompiled->isRemoteCompReq()) // remote compilations are performed elsewhere
        {
            ...
            struct CompilationTrace {
                CompilationTrace(TR::Compilation &compiler)
                    : _compiler(compiler)
                {
                    TR_ASSERT(_compiler.getHotnessName(_compiler.getMethodHotness()),
                        "expected to have a hotness string");
...
#if defined(J9VM_OPT_JITSERVER)
                        if (_compiler.isOutOfProcessCompilation()
                            && TR::compInfoPT->getClientData()) // using jitserver && client JVM
                        {
                            log->println();
        ...
        if (TR::Options::isAnyVerboseOptionSet(TR_VerbosePerformance, TR_VerboseCompileStart)) {
            ...
            TR::snprintfNoTrunc(compilationTypeString, sizeof(compilationTypeString), "%s%s",
                vm.isAOT_DEPRECATED_DO_NOT_USE() ? "AOT " : "", compiler->isProfilingCompilation() ? "profiled " : "");
            bool incomplete;
            uint64_t freePhysicalMemorySizeB = _compInfo.computeAndCacheFreePhysicalMemory(incomplete);

            if (freePhysicalMemorySizeB != OMRPORT_MEMINFO_NOT_AVAILABLE) {
                TR_VerboseLog::writeLineLocked(TR_Vlog_COMPSTART,
                    "(%s%s) Compiling %s %s %s j9m=%p %s t=%llu compThreadID=%d memLimit=%zu KB "
                    "freePhysicalMemory=%llu MB",
                    compilationTypeString, compiler->getHotnessName(compiler->getMethodHotness()),
                    compiler->signature(), compiler->isDLT() ? "DLT" : "", details.name(), method,
                    _methodBeingCompiled->isRemoteCompReq() ? "remote" : "",
                    _compInfo.getPersistentInfo()->getElapsedTime(), compiler->getCompThreadID(),
                    scratchSegmentProvider.allocationLimit() >> 10, freePhysicalMemorySizeB >> 20);
            } else {
                TR_VerboseLog::writeLineLocked(TR_Vlog_COMPSTART,
                    "(%s%s) Compiling %s %s %s j9m=%p t=%llu compThreadID=%d memLimit=%zu KB", compilationTypeString,
                    compiler->getHotnessName(compiler->getMethodHotness()), compiler->signature(),
                    compiler->isDLT() ? "DLT" : "", details.name(), method,
                    _compInfo.getPersistentInfo()->getElapsedTime(), compiler->getCompThreadID(),
        ...
        if (_methodBeingCompiled->_oldStartPC && compiler->getOption(TR_FailPreXRecompile)
...
#else
            bool doAcquireClassUnloadMonitor
                = compiler->getOption(TR_EnableHCR) || compiler->getOption(TR_FullSpeedDebug);
#endif
        ...
        //
        if ((_jitConfig->runtimeFlags & J9JIT_TOSS_CODE) || _methodBeingCompiled->isOutOfProcessCompReq()
            ...
            if (_reservedDataCache) {
                TR_ASSERT(_methodBeingCompiled->_compilationAttemptsLeft < MAX_COMPILE_ATTEMPTS,
                    "reserved dataCache for a brand new compilation\n");
                ...
                TR_DataCache *dataCache
                    = TR_DataCacheManager::getManager()->reserveAvailableDataCache(vmThread, 4 * 1024);
        ...
        } else // non AOT, non TOSS_CODE
        {
            ...
            if (_reservedDataCache) {
                TR_ASSERT(_methodBeingCompiled->_compilationAttemptsLeft < MAX_COMPILE_ATTEMPTS,
                    "Reserved dcache must be saved only for a comp retrial");
                TR_DataCacheManager::getManager()->makeDataCacheAvailable(_reservedDataCache);
                _reservedDataCache = NULL;
            }
        }
        ...
        if ((vm.isAOT_DEPRECATED_DO_NOT_USE() && !_methodBeingCompiled->isRemoteCompReq())
            ...
            //
            TR_AOTMethodHeader *cacheEntryAOTMethodHeader = pointer_cast<TR_AOTMethodHeader *>(
                vm.allocateRelocationData(compiler, sizeof(struct TR_AOTMethodHeader)));
            ...
            cacheEntryAOTMethodHeader->majorVersion
                = TR_AOTMethodHeader_MajorVersion; // AOT code and data major version
            cacheEntryAOTMethodHeader->minorVersion
                = TR_AOTMethodHeader_MinorVersion; // AOT code and data minor version
        }
        ...
        {
            if (TR::Options::getVerboseOption(TR_VerboseCompilationDispatch)) {
                TR_VerboseLog::writeLineLocked(TR_Vlog_DISPATCH, "Compiling %s @ %s", compiler->signature(),
                    compiler->getHotnessName());
            }

            if (compiler->getOption(TR_AlwaysSafeFatal)) {
                TR_ASSERT_SAFE_FATAL(false, "alwaysSafeFatal set");
                TR_VerboseLog::writeLineLocked(TR_Vlog_INFO, "Bypassed TR_ASSERT_SAFE_FATAL %s @ %s",
                    compiler->signature(), compiler->getHotnessName());
            }
            ...
            if (TR::Options::getVerboseOption(TR_VerboseCompilationDispatch) && !rtn) {
                TR_VerboseLog::writeLineLocked(TR_Vlog_DISPATCH,
                    ...
                    "] for %s @ %s",
                    compiler->cg()->getCodeStart(), compiler->cg()->getCodeEnd(), compiler->signature(),
                    compiler->getHotnessName());
            }
        }
        ...
        if (!compiler->getOption(TR_DisableNoVMAccess)) {
...
#endif
            {
                if (haveLockedClassUnloadMonitor) {
                    ...
                    haveLockedClassUnloadMonitor
                        = false; // Need this because an exception might happen during createMethodMetadata
            ...
            // The GC could have unloaded some classes when we released the classUnloadMonitor, or HCR could have kicked
            // in
            //
            if (compilationShouldBeInterrupted()) {
                if (TR::Options::getVerboseOption(TR_VerboseCompilationDispatch)) {
                    TR_VerboseLog::writeLineLocked(TR_Vlog_DISPATCH, "Interrupting compilation of %s @ %s",
                        compiler->signature(), compiler->getHotnessName());
                }
        ...
        if (!_methodBeingCompiled->isAotLoad()
            ...
            if (TR::Options::getVerboseOption(TR_VerboseCompilationDispatch)) {
                TR_VerboseLog::writeLineLocked(TR_Vlog_DISPATCH, "Creating metadata for %s @ %s", compiler->signature(),
                    compiler->getHotnessName());
            }
            ...
            if (!metaData) {
                if (TR::Options::getVerboseOption(TR_VerboseCompilationDispatch)) {
                    TR_VerboseLog::writeLineLocked(TR_Vlog_DISPATCH, "Failed to create metadata for %s @ %s",
                        compiler->signature(), compiler->getHotnessName());
                }
            ...
            if (TR::Options::getVerboseOption(TR_VerboseCompilationDispatch)) {
                TR_VerboseLog::writeLineLocked(TR_Vlog_DISPATCH,
                    "Successfully created metadata [" POINTER_PRINTF_FORMAT "] for %s @ %s", metaData,
                    compiler->signature(), compiler->getHotnessName());
            }
            ...
            if (metaData->startColdPC) {
                uint8_t *coldMethodHeader
                    = reinterpret_cast<uint8_t *>(metaData->startColdPC) - sizeof(OMR::CodeCacheMethodHeader);
        ...
        if (metaData && compiler->getPersistentInfo()->isRuntimeInstrumentationEnabled()) {
            // This must be called after createMethodMetaData because TR_PersistentJittedBodyInfo can change in that
            // function.
        ...
        if (chTable && !chTable->canSkipCommit(compiler)) {
            ...
            if (chTable->commit(compiler) == false) {
                ...
                if (metaData->startPC) {
                    TR_PersistentJittedBodyInfo *bodyInfo
                        = TR::Recompilation::getJittedBodyInfoFromPC(reinterpret_cast<void *>(metaData->startPC));
                    if (bodyInfo) {
                ...
                if (TR::Options::isAnyVerboseOptionSet(TR_VerboseCompileEnd, TR_VerbosePerformance,
                        TR_VerboseCompFailure)) {
                    TR_VerboseLog::writeLineLocked(TR_Vlog_FAILURE, "Failure while committing chtable for %s",
                        compiler->signature());
                }
        ...
        logCompilationSuccess(vmThread, vm, method, scratchSegmentProvider, compilee, compiler, metaData,
            optimizationPlan);
...
extern J9_CFUNC void jitMethodHandleTranslated(J9VMThread *currentThread, j9object_t methodHandle, j9object_t arg,
    void *jitEntryPoint, void *intrpEntryPoint);
#endif
...
*/
void *TR::CompilationInfo::compilationEnd(J9VMThread *vmThread, TR::IlGeneratorMethodDetails &details,
    J9JITConfig *jitConfig, void *startPC, void *oldStartPC, bool preventFutureMethodCountingOnFailure, TR_FrontEnd *fe,
    TR_MethodToBeCompiled *entry, TR::Compilation *comp)
{
    ...
    if (details.isNewInstanceThunk()) {
        if (isJITServerMode) {
            ...
            if (startPC) // compilation succeeded
            {
                ...
                if (TR::Options::getVerboseOption(TR_VerboseJITServer)) {
                    TR_VerboseLog::writeLineLocked(TR_Vlog_JITServer,
                        "compThreadID=%d has failed to compile a new instance thunk",
                        entry->_compInfoPT->getCompThreadId());
                }
    ...
    if (details.isMethodHandleThunk()) {
        ...
        if (isJITServerMode) {
            ...
            } else if (entry) // failure
            {
                if (TR::Options::getVerboseOption(TR_VerboseJITServer)) {
                    TR_VerboseLog::writeLineLocked(TR_Vlog_JITServer,
                        "compThreadID=%d has failed to compile a methodHandleThunk method",
                        entry->_compInfoPT->getCompThreadId());
                }
...
#endif /* defined(J9VM_OPT_JITSERVER) */
        {
            if (startPC) {
                J9::MethodHandleThunkDetails &mhDetails = static_cast<J9::MethodHandleThunkDetails &>(details);
                uintptr_t thunks
                    = trvm->getReferenceField(*mhDetails.getHandleRef(), "thunks", "Ljava/lang/invoke/ThunkTuple;");
                int64_t jitEntryPoint
                    = (int64_t)(intptr_t)startPC + J9::PrivateLinkage::LinkageInfo::get(startPC)->getJitEntryOffset();

#if defined(JIT_METHODHANDLE_TRANSLATED)
                jitMethodHandleTranslated(vmThread, *mhDetails.getHandleRef(),
                    mhDetails.getArgRef() ? *mhDetails.getArgRef() : NULL, jitEntryPoint, startPC);
#else
    ...
    {
        TR_ASSERT_FATAL(comp && entry,
            "comp object and entry must always exist when calling compilationEnd for a successful compilation");
    ...
    && (defined(TR_HOST_X86) || defined(TR_HOST_POWER) || defined(TR_HOST_S390) || defined(TR_HOST_ARM) \
        || defined(TR_HOST_ARM64))
                    if (TR::Options::sharedClassCache()) {
                        ...
                        aotMethodHeaderEntry->unused
                            = TR::Compiler->host.is64Bit() ? 0xDEADC0DEDEADC0DEULL : 0xDEADC0DE;

                        J9ROMMethod *romMethod = comp->fej9()->getROMMethodFromRAMMethod(method);

                        TR::CompilationInfo::storeAOTInSharedCache(vmThread, romMethod, dataStart, dataSize, codeStart,
                            codeSize, comp, jitConfig, entry);
                        ...
                        if (canRelocateMethod) {
                            ...
                            // If compilation and/or slot monitors are held, they must now be released in order to call
                            // the AOT relocation.
                            //
                            if (entry->getMonitor()) {
                            ...
                            if (TR::Options::getVerboseOption(TR_VerboseCompilationDispatch)) {
                                TR_VerboseLog::writeLineLocked(TR_Vlog_DISPATCH,
                                    "Applying relocations to newly AOT compiled body for %s @ %s", comp->signature(),
                                    comp->getHotnessName());
                            }
                            try {
                                TR::CompilationInfoPerThreadBase::InterruptibleOperation relocatingAOTBody(
                                    *entry->_compInfoPT);
                                // need to get a non-shared cache VM to relocate
                                TR_J9VMBase *fe = TR_J9VMBase::get(jitConfig, vmThread);
                                TR_ResolvedMethod *compilee
                                    = fe->createResolvedMethod(comp->trMemory(), (TR_OpaqueMethodBlock *)method);
                                relocatedMetaData = entry->_compInfoPT->reloRuntime()->prepareRelocateAOTCodeAndData(
                                    vmThread, fe, aotMCCRuntimeCodeCache, cacheEntry, method, true, comp->getOptions(),
                                    comp, compilee);
                            ...
                            if (relocatedMetaData) {
                                if (TR::Options::getVerboseOption(TR_VerboseCompilationDispatch)) {
                                    TR_VerboseLog::writeLineLocked(TR_Vlog_DISPATCH,
                                        "Successfully relocated metadata for %s", comp->signature());
                                }
                                ...
                                if (J9_EVENT_IS_HOOKED(jitConfig->javaVM->hookInterface, J9HOOK_VM_DYNAMIC_CODE_LOAD)) {
                                    TR::CompilationInfo::addJ9HookVMDynamicCodeLoadForAOT(vmThread, method, jitConfig,
                                        relocatedMetaData);
                                }
                                ...
                                if (TR::Options::isAnyVerboseOptionSet(TR_VerboseCompileEnd, TR_VerbosePerformance,
                                    ...
                                    TR_VerboseLog::writeLineLocked(TR_Vlog_FAILURE,
                                        ...
                                        comp->signature(), returnCode,
                                        (returnCode >= 0) && (returnCode < compilationMaxError)
                                            ? compilationErrorNames[returnCode]
                                            : "unknown error",
                                        comp->reloRuntime()->getReloErrorCodeName(
                                            comp->reloRuntime()->getReloErrorCode()));
                                }
                            }
                        }
                        ...
                        if (!canRelocateMethod || !relocatedMetaData) {
                            ...
                            TR_AOTMethodHeader *aotMethodHeaderEntry = comp->getAotMethodHeaderEntry();
                            J9JITDataCacheHeader *cacheEntry
                                = (J9JITDataCacheHeader *)aotMethodHeaderEntry->compileMethodDataStartPC;
                            J9JITDataCacheHeader *exceptionTableCacheEntry = (J9JITDataCacheHeader *)((U_8 *)cacheEntry
                                + aotMethodHeaderEntry->offsetToExceptionTable);
                            ...
                            // Since we didn't performed the relocation, we don't have to
                            // artifactManager->removeArtifact(metaData);

                            // Delete any assumptions that might still exist in persistent memory
                            // The metadata parameter is NULL meaning that we want to delete ALL assumptions, including
                            // those for JBI
                            compInfo->getPersistentInfo()->getRuntimeAssumptionTable()->reclaimAssumptions(
                                comp->getMetadataAssumptionList(), NULL);

                            // reclaim code memory so we can use it for something else
                            TR::CodeCacheManager::instance()->addFreeBlock(static_cast<void *>(metaData),
                                reinterpret_cast<uint8_t *>(metaData->startPC));
...
#endif // defined(J9VM_INTERP_AOT_COMPILE_SUPPORT) && defined(J9VM_OPT_SHARED_CLASSES) && (defined(TR_HOST_X86) ||
       // defined(TR_HOST_POWER) || defined(TR_HOST_S390) || defined(TR_HOST_ARM))
                    {
#ifdef AOT_DEBUG
                        fprintf(stderr,
                            "ERROR: AOT compiling but shared class is not available. Method will run interpreted\n");
#endif
                        TR_ASSERT(0,
                            "shared classes flag not enabled yet we got an AOT compilation"); // This is a branch that
                        ...
                        if (TR::Options::isAnyVerboseOptionSet(TR_VerboseCompileEnd, TR_VerbosePerformance,
                                TR_VerboseCompFailure)) {
                            TR_VerboseLog::writeLineLocked(TR_Vlog_INFO,
                                "Shared Class not available for AOT compile for %s", comp->signature());
                        }
                    }
                    ...
                    if ((trvm->isAOT_DEPRECATED_DO_NOT_USE()
                        ...
                        if (jitConfig->javaVM->internalVMFunctions->isCheckpointAllowed(jitConfig->javaVM)
                            ...
                            if (comp->getRecompilationInfo() && comp->getRecompilationInfo()->getJittedBodyInfo()) {
                                if (TR::Options::getCmdLineOptions()->getVerboseOption(
                                        TR_VerboseCheckpointRestoreDetails))
                                    TR_VerboseLog::writeLineLocked(TR_Vlog_CHECKPOINT_RESTORE,
                                        "Will force %p to be recompiled post-restore", method);
                                ...
                                if (TR::Options::getCmdLineOptions()->getVerboseOption(
                                        TR_VerboseCheckpointRestoreDetails))
                                    TR_VerboseLog::writeLineLocked(TR_Vlog_CHECKPOINT_RESTORE,
                                        "Cannot force %p to be recompiled post-restore because the bodyInfo does not "
                                        "exist",
                                        method);
                            }
                        }
        ...
        if (oldStartPC) {
            if (TR::Options::getVerboseOption(TR_VerboseCodeCache)) {
                ...
                OMR::CodeCacheMethodHeader *recompiledMethodHeader
                    = getCodeCacheMethodHeader((char *)startPC, 32, NULL);
                ...
                TR_VerboseLog::writeLineLocked(TR_Vlog_INFO,
                    "Recompile %s @ " POINTER_PRINTF_FORMAT "-" POINTER_PRINTF_FORMAT
                    "(%d bytes) -> " POINTER_PRINTF_FORMAT "-" POINTER_PRINTF_FORMAT "(%d bytes)",
                    comp->signature(), oldStartPC, oldEndPC, oldMethodSize, startPC, endPC, methodSize);
            }
        ...
        if (entry && isJITServerMode) // failure at the JITServer
        {
            ...
            if (TR::Options::getVerboseOption(TR_VerboseJITServer)) {
                TR_VerboseLog::writeLineLocked(TR_Vlog_JITServer,
                    "compThreadID=%d has failed to compile: compErrCode %u %s", entry->_compInfoPT->getCompThreadId(),
                    entry->_compErrCode, comp ? comp->signature() : "");
            }
            if (vmThread)
    ...
    } else // recompilation failure
    {
        ...
        if (entry && isJITServerMode) {
            if (TR::Options::getVerboseOption(TR_VerboseJITServer)) {
                TR_VerboseLog::writeLineLocked(TR_Vlog_JITServer,
                    "compThreadID=%d has failed to recompile: compErrCode %u %s", entry->_compInfoPT->getCompThreadId(),
                    entry->_compErrCode, comp ? comp->signature() : "");
            }
            if (vmThread)
...
//
bool TR::CompilationInfoPerThreadBase::methodCanBeCompiled(TR_Memory *trMemory, TR_FrontEnd *vm,
    TR_ResolvedMethod *method, TR_FilterBST *&filter)
{
    filter = NULL;
...
void TR::CompilationInfoPerThreadBase::logCompilationSuccess(J9VMThread *vmThread, TR_J9VMBase &vm, J9Method *method,
    TR::SegmentAllocator const &scratchSegmentProvider, TR_ResolvedMethod *compilee, TR::Compilation *compiler,
    TR_MethodMetaData *metaData, TR_OptimizationPlan *optimizationPlan)
{
    if (!_methodBeingCompiled->isAotLoad()) {
        J9JavaVM *javaVM = _jitConfig->javaVM;
        ...
        if (!vm.isAOT_DEPRECATED_DO_NOT_USE()) {
            if (J9_EVENT_IS_HOOKED(javaVM->hookInterface, J9HOOK_VM_DYNAMIC_CODE_LOAD)) {
                OMR::CodeCacheMethodHeader *ccMethodHeader;
                ALWAYS_TRIGGER_J9HOOK_VM_DYNAMIC_CODE_LOAD(javaVM->hookInterface, vmThread, method,
                    (U_8 *)metaData->startPC, metaData->endWarmPC - metaData->startPC, "JIT warm body", metaData);
                ...
                if (metaData->startColdPC) {
                    ...
                    //
                    ALWAYS_TRIGGER_J9HOOK_VM_DYNAMIC_CODE_LOAD(javaVM->hookInterface, vmThread, method,
                        (U_8 *)metaData->startColdPC, metaData->endPC - metaData->startColdPC, "JIT cold body",
                        metaData);
                }
                ccMethodHeader = getCodeCacheMethodHeader((char *)metaData->startPC, 32, metaData);
                if (ccMethodHeader && (metaData->bodyInfo != NULL)) {
                    ...
                    if (linkageInfo->isRecompMethodBody()) {
                        ALWAYS_TRIGGER_J9HOOK_VM_DYNAMIC_CODE_LOAD(javaVM->hookInterface, vmThread, method,
                            (void *)((char *)ccMethodHeader->_eyeCatcher + 4),
                            metaData->startPC - (UDATA)((char *)ccMethodHeader->_eyeCatcher + 4), "JIT method header",
                            metaData);
                    }
                }
        ...
        {
            TR_PersistentJittedBodyInfo *oldBodyInfo
                = TR::Recompilation::getJittedBodyInfoFromPC(_methodBeingCompiled->_oldStartPC);
        ...
        if (_jitConfig->runtimeFlags & J9JIT_TOSS_CODE) {
            ...
            if (TR::Options::isAnyVerboseOptionSet(TR_VerboseCompileEnd, TR_VerboseGc, TR_VerboseRecompile,
                ...
                TR_VerboseLog::write(TR_Vlog_COMP, "(%s%s) %s @ " POINTER_PRINTF_FORMAT "-" POINTER_PRINTF_FORMAT,
                    compilationTypeString, hotnessString, compiler->signature(), startPC,
                ...
                if (compiler->getRecompilationInfo()
                    && compiler->getRecompilationInfo()->getJittedBodyInfo()->getUsesGCR())
                ...
                if (TR::Options::getVerboseOption(TR_VerbosePerformance)) {
                    TR_VerboseLog::write(" mem=[region=%llu system=%llu]KB",
                ...
                if (TR::Options::getVerboseOption(TR_VerboseRecompile)) {
                    TR_VerboseLog::write("%s [profiling c(%d), f(%d), ivc(%d)]", prexString, profilingCount,
                        profilingFrequencey, counter);
                }

                if (compiler->getOption(TR_CountOptTransformations)
                    && compiler->getOption(TR_VerboseOptTransformations)) {
                ...
                if (TR::Options::getVerboseOption(TR_VerboseOptimizer)) {
                    TR_VerboseLog::write(" opts=%d.%d", compiler->getLastPerformedOptIndex(),
                        compiler->getLastPerformedOptSubIndex());
                }
                ...
                if (TR::Options::getVerboseOption(TR_VerboseJitMemory)) {
                    ...
                    TR_VerboseLog::writeLine(TR_Vlog_METHOD_STATS,
                        "%s j9m=%p "
                        ...
                        compiler->signature(), method, methodStats.codeSize, methodStats.warmBlocks,
                        methodStats.coldBlocks, methodStats.prologue, methodStats.snippets, methodStats.outOfLine,
                        methodStats.unaccounted, methodStats.blocksInColdCache, methodStats.overestimateInColdCache);
                }
            }

            char compilationAttributes[40] = { 0 };
            TR::snprintfNoTrunc(compilationAttributes, sizeof(compilationAttributes), "%s %s %s %s %s %s %s",
                prexString, !_methodBeingCompiled->_async ? "sync" : "", compilee->isJNINative() ? "JNI" : "",
                compiler->getOption(TR_EnableOSR) ? "OSR" : "",
                (compiler->getRecompilationInfo()
                    && compiler->getRecompilationInfo()->getJittedBodyInfo()->getUsesGCR())
                    ? "GCR"
                    : "",
                compiler->isDLT() ? "DLT" : "", TR::CompilationInfo::isJSR292(method) ? "JSR292" : "");

            Trc_JIT_compileEnd15(vmThread, compilationTypeString, hotnessString, compiler->signature(), startPC,
                endWarmPC, startColdPC, endPC, translationTime, method, metaData, recompReason,
                _compInfo.getMethodQueueSize(), TR::CompilationInfo::getMethodBytecodeSize(method),
...
void TR::CompilationInfoPerThreadBase::processException(J9VMThread *vmThread,
    TR::SegmentAllocator const &scratchSegmentProvider, TR::Compilation *compiler,
    volatile bool &haveLockedClassUnloadMonitor, const char *exceptionName, J9JITExceptionTable *metaData) throw()
{
    if (TR::Options::getVerboseOption(TR_VerboseCompilationDispatch)) {
        TR_VerboseLog::writeLineLocked(TR_Vlog_DISPATCH, "Failed to finalize compiled body for %s @ %s",
            compiler->signature(), compiler->getHotnessName());
    }
    ...
    /* Excessive Complexity Exceptions End */
        ...
        if (TR::Options::isAnyVerboseOptionSet(TR_VerbosePerformance, TR_VerboseCompileEnd, TR_VerboseCompFailure)) {
            ...
            TR_VerboseLog::writeLineLocked(TR_Vlog_COMPFAIL,
                "(%s%s) %s Q_SZ=%d Q_SZI=%d QW=%d j9m=%p time=%dus %s compThreadID=%d", compilationTypeString,
                compiler->getHotnessName(compiler->getMethodHotness()), compiler->signature(),
                _compInfo.getMethodQueueSize(), _compInfo.getNumQueuedFirstTimeCompilations(),
                _compInfo.getQueueWeight(), _methodBeingCompiled->getMethodDetails().getMethod(), translationTime,
                "Class chain persistence failure", getCompThreadId());
        }
        ...
        if (TR::Options::isAnyVerboseOptionSet(TR_VerbosePerformance, TR_VerboseCompileEnd, TR_VerboseCompFailure)) {
            ...
            TR_VerboseLog::writeLineLocked(TR_Vlog_COMPFAIL,
                ...
                compilationTypeString, compiler->getHotnessName(compiler->getMethodHotness()), compiler->signature(),
                _compInfo.getMethodQueueSize(), _compInfo.getNumQueuedFirstTimeCompilations(),
                _compInfo.getQueueWeight(), _methodBeingCompiled->getMethodDetails().getMethod(), translationTime,
                getCompThreadId());
        }
        ...
        Trc_JITServerStreamFailure(vmThread, getCompThreadId(), __FUNCTION__, compiler->signature(),
            compiler->getHotnessName(), e.what());
        _methodBeingCompiled->_compErrCode = compilationStreamFailure;
    } catch (const JITServer::StreamInterrupted &e) {
        if (TR::Options::getVerboseOption(TR_VerboseJITServer))
            TR_VerboseLog::writeLineLocked(TR_Vlog_JITServer, "compThreadID=%d JITServer StreamInterrupted: %s",
                getCompThreadId(), e.what());
        Trc_JITServerStreamInterrupted(vmThread, getCompThreadId(), __FUNCTION__, compiler->signature(),
            compiler->getHotnessName(), e.what());
        ...
        if (TR::Options::getVerboseOption(TR_VerboseJITServer)
            ...
            TR_VerboseLog::writeLineLocked(TR_Vlog_JITServer, "compThreadID=%d JITServer StreamVersionIncompatible: %s",
                getCompThreadId(), e.what());
        Trc_JITServerStreamVersionIncompatible(vmThread, getCompThreadId(), __FUNCTION__, compiler->signature(),
            compiler->getHotnessName(), e.what());
        _methodBeingCompiled->_compErrCode = compilationStreamVersionIncompatible;
    } catch (const JITServer::StreamMessageTypeMismatch &e) {
        if (TR::Options::getVerboseOption(TR_VerboseJITServer))
            TR_VerboseLog::writeLineLocked(TR_Vlog_JITServer, "compThreadID=%d JITServer StreamMessageTypeMismatch: %s",
                getCompThreadId(), e.what());
        Trc_JITServerStreamMessageTypeMismatch(vmThread, getCompThreadId(), __FUNCTION__, compiler->signature(),
            compiler->getHotnessName(), e.what());
...
void TR::CompilationInfoPerThreadBase::processExceptionCommonTasks(J9VMThread *vmThread,
    TR::SegmentAllocator const &scratchSegmentProvider, TR::Compilation *compiler, const char *exceptionName,
    TR_MethodToBeCompiled *entry)
{
    ...
    if (TR::Options::isAnyVerboseOptionSet(TR_VerbosePerformance, TR_VerboseCompileEnd, TR_VerboseCompFailure)) {
        ...
        TR::snprintfNoTrunc(compilationTypeString, sizeof(compilationTypeString), "%s%s", isAOTComp ? "AOT " : "",
            isProfiledComp ? "profiled " : "");

        const char *hotnessString
            = entry->isAotLoad() ? "load" : compiler->getHotnessName(compiler->getMethodHotness());
        ...
        if (_methodBeingCompiled->_compErrCode != compilationFailure) {
            if ((_jitConfig->runtimeFlags & J9JIT_TOSS_CODE)
                ...
                TR_VerboseLog::write(TR_Vlog_FAILURE, "Translating %s j9m=%p time=%dus-- Interrupted because of %s",
                    compiler->signature(), _methodBeingCompiled->getMethodDetails().getMethod(), translationTime,
                    exceptionName);
            } else {
                TR_VerboseLog::write(TR_Vlog_COMPFAIL, "(%s%s) %s Q_SZ=%d Q_SZI=%d QW=%d j9m=%p time=%dus %s",
                    compilationTypeString, hotnessString, compiler->signature(), _compInfo.getMethodQueueSize(),
                    _compInfo.getNumQueuedFirstTimeCompilations(), _compInfo.getQueueWeight(),
                    _methodBeingCompiled->getMethodDetails().getMethod(), translationTime,
                ...
                if (entry->isAotLoad()) {
                    ...
                    if (reloRuntime)
                        TR_VerboseLog::write(" (%s)",
                            reloRuntime->getReloErrorCodeName(reloRuntime->getReloErrorCode()));
                }
            ...
            TR_VerboseLog::write(TR_Vlog_COMPFAIL,
                "(%s%s) %s Q_SZ=%d Q_SZI=%d QW=%d j9m=%p time=%dus <TRANSLATION FAILURE: %s>", compilationTypeString,
                hotnessString, compiler->signature(), _compInfo.getMethodQueueSize(),
                _compInfo.getNumQueuedFirstTimeCompilations(), _compInfo.getQueueWeight(),
                _methodBeingCompiled->getMethodDetails().getMethod(), translationTime, exceptionName);
        }
...
#if DEBUG
void TR::CompilationInfo::debugPrint(const char *debugString)
{
    if (!_traceCompiling)
        return;
...
void TR::CompilationInfo::debugPrint(J9VMThread *vmThread, const char *debugString)
{
    if (!_traceCompiling)
        return;
...
void TR::CompilationInfo::debugPrint(const char *debugString, intptr_t val)
{
    if (!_traceCompiling)
        return;
...
void TR::CompilationInfo::debugPrint(J9VMThread *vmThread, const char *debugString, IDATA val)
{
    if (!_traceCompiling)
        return;
    fprintf(stderr, "%8" OMR_PRIxPTR ":", (uintptr_t)vmThread);
...
void TR::CompilationInfo::debugPrint(J9Method *method)
{
    if (!_traceCompiling)
        return;
    if (!method) {
    ...
    J9UTF8 *signature;
    getClassNameSignatureFromMethod(method, className, name, signature);
    fprintf(stderr, "%.*s.%.*s%.*s", J9UTF8_LENGTH(className), (char *)J9UTF8_DATA(className), J9UTF8_LENGTH(name),
        (char *)J9UTF8_DATA(name), J9UTF8_LENGTH(signature), (char *)J9UTF8_DATA(signature));
}

void TR::CompilationInfo::debugPrint(const char *debugString, J9Method *method)
{
    if (!_traceCompiling)
        return;
...
void TR::CompilationInfo::debugPrint(const char *debugString, TR::IlGeneratorMethodDetails &details,
    J9VMThread *vmThread)
{
    if (!_traceCompiling)
        return;
...
void TR::CompilationInfo::debugPrint(J9VMThread *vmThread, const char *msg, TR_MethodToBeCompiled *entry)
{
    if (!_traceCompiling)
        return;
...
//
void TR::CompilationInfo::printQueue()
{
    if (!_traceCompiling)
        return;
...
TR_HWProfiler *TR::CompilationInfo::getHWProfiler() const
{
    return ((TR_JitPrivateConfig *)_jitConfig->privateConfig)->hwProfiler;
}

TR_JProfilerThread *TR::CompilationInfo::getJProfilerThread() const
{
    return ((TR_JitPrivateConfig *)_jitConfig->privateConfig)->jProfiler;
}

int32_t TR::CompilationInfo::calculateCodeSize(TR_MethodMetaData *metaData)
{
    int32_t codeSize = 0;
...
void TR::CompilationInfo::increaseUnstoredBytes(U_32 aotBytes, U_32 jitBytes)
{
#if defined(J9VM_OPT_SHARED_CLASSES)                                                                    \
    && (defined(TR_HOST_X86) || defined(TR_HOST_POWER) || defined(TR_HOST_S390) || defined(TR_HOST_ARM) \
        || defined(TR_HOST_ARM64))
...
uint64_t TR::CompilationInfo::computeFreePhysicalMemory(bool &incompleteInfo)
{
    ...
    J9MemoryInfo memInfo;
    if (0 == j9sysinfo_get_memory_info(&memInfo) && memInfo.availPhysical != OMRPORT_MEMINFO_NOT_AVAILABLE
...
uint64_t TR::CompilationInfo::computeAndCacheFreePhysicalMemory(bool &incompleteInfo, int64_t updatePeriodMs)
{
    if (updatePeriodMs < 0)
    ...
    if (_cachedFreePhysicalMemoryB != OMRPORT_MEMINFO_NOT_AVAILABLE) {
        ...
        if (lastUpdateTime == 0 || (crtElapsedTime - lastUpdateTime) >= updatePeriodMs) {
...
uint64_t TR::CompilationInfo::computeFreePhysicalLimitAndAbortCompilationIfLow(TR::Compilation *comp,
    bool &incompleteInfo, size_t sizeToAllocate)
{
    uint64_t freePhysicalMemorySizeB = computeAndCacheFreePhysicalMemory(incompleteInfo);
    ...
    if (!incompleteInfo && freePhysicalMemorySizeB < desiredMemory) {
        ...
        if (!incompleteInfo && freePhysicalMemorySizeB < desiredMemory) {
            if (TR::Options::isAnyVerboseOptionSet(TR_VerbosePerformance, TR_VerboseCompileEnd,
                    TR_VerboseCompFailure)) {
                TR_VerboseLog::writeLineLocked(TR_Vlog_FAILURE,
                    "Aborting Compilation: Low On Physical Memory %zu B, sizeToAllocate %zu safeMemReserve %zu",
...
void TR::CompilationInfo::replenishInvocationCount(J9Method *method, TR::Compilation *comp)
{
    ...
    if ((methodVMExtra == 1) || (methodVMExtra == J9_JIT_QUEUED_FOR_COMPILATION)) {
        ...
        if (TR::Options::getCountsAreProvidedByUser() || (TR::Options::startupTimeMatters() == TR_yes))
            ...
            count
                = J9ROMMETHOD_HAS_BACKWARDS_BRANCHES(romMethod) ? TR_DEFAULT_INITIAL_BCOUNT : TR_DEFAULT_INITIAL_COUNT;

        TR::CompilationInfo::setInvocationCount(method, count);
        if (TR::Options::getCmdLineOptions()->getVerboseOption(TR_VerbosePerformance)) {
            // compiler must exist because startPC != NULL
            TR_VerboseLog::writeLineLocked(TR_Vlog_PERF, "Reencoding count=%d for %s j9m=%p ", count, comp->signature(),
                method);
        }
    } else {
        TR_ASSERT(comp->getOptimizationPlan()->isExplicitCompilation(),
            "Unexpected value for method->extra = %p (method=%p)\n", TR::CompilationInfo::getJ9MethodExtra(method),
            method);
    }
}

void TR::CompilationInfo::addJ9HookVMDynamicCodeLoadForAOT(J9VMThread *vmThread, J9Method *method,
    J9JITConfig *jitConfig, TR_MethodMetaData *relocatedMetaData)
{
    OMR::CodeCacheMethodHeader *ccMethodHeader;
    ALWAYS_TRIGGER_J9HOOK_VM_DYNAMIC_CODE_LOAD(jitConfig->javaVM->hookInterface, vmThread, method,
        reinterpret_cast<void *>(relocatedMetaData->startPC), relocatedMetaData->endWarmPC - relocatedMetaData->startPC,
        "JIT warm body", relocatedMetaData);
    if (relocatedMetaData->startColdPC) {
        // Register the cold code section too
        //
        ALWAYS_TRIGGER_J9HOOK_VM_DYNAMIC_CODE_LOAD(jitConfig->javaVM->hookInterface, vmThread, method,
            reinterpret_cast<void *>(relocatedMetaData->startColdPC),
            relocatedMetaData->endPC - relocatedMetaData->startColdPC, "JIT cold body", relocatedMetaData);
    }
    ccMethodHeader
        = getCodeCacheMethodHeader(reinterpret_cast<char *>(relocatedMetaData->startPC), 32, relocatedMetaData);
    if (ccMethodHeader && (relocatedMetaData->bodyInfo != NULL)) {
        J9::PrivateLinkage::LinkageInfo *linkageInfo
            = J9::PrivateLinkage::LinkageInfo::get(reinterpret_cast<void *>(relocatedMetaData->startPC));
        if (linkageInfo->isRecompMethodBody()) {
            ALWAYS_TRIGGER_J9HOOK_VM_DYNAMIC_CODE_LOAD(jitConfig->javaVM->hookInterface, vmThread, method,
                ccMethodHeader->_eyeCatcher + 4,
                relocatedMetaData->startPC - reinterpret_cast<uintptr_t>(ccMethodHeader->_eyeCatcher + 4),
                "JIT method header", relocatedMetaData);
        }
    }
}
    ...
    && (defined(TR_HOST_X86) || defined(TR_HOST_POWER) || defined(TR_HOST_S390) || defined(TR_HOST_ARM) \
        || defined(TR_HOST_ARM64))
void TR::CompilationInfo::storeAOTInSharedCache(J9VMThread *vmThread, J9ROMMethod *romMethod, const U_8 *dataStart,
    UDATA dataSize, const U_8 *codeStart, UDATA codeSize, TR::Compilation *comp, J9JITConfig *jitConfig,
    TR_MethodToBeCompiled *entry)
{
    bool safeToStore;
    ...
    if (static_cast<TR_JitPrivateConfig *>(jitConfig->privateConfig)->aotValidHeader == TR_yes) {
        ...
        TR_ASSERT_FATAL(static_cast<TR_JitPrivateConfig *>(jitConfig->privateConfig)->aotValidHeader != TR_maybe,
            "Should not be possible for aotValidHeader to be TR_maybe at this point\n");
    } else {
        safeToStore = false;
    }
    ...
    if (safeToStore) {
        ...
        try {
            if (!comp->getOption(TR_DisableAOTBytesCompression)) {
                 ...
                 * For each method Shared Class Cache API adds CompiledMethodWrapper header which holds the size of data
                 * in cache, as well as J9ROMMethod of compiled method. Size of the original data is extracted from the
                 * TR_AOTMethodHeader while size of compressed data is extracted from CompiledMethodWrapper. That is why
                 * We do not compress header as we can extract the original data size which is useful information while
                 * inflating the method data while loading compiled method.
                 *
                 * Compressed data stored in the cache looks like following
                 * ------------------------------------------------------------------------------------------------------------------------------------------------
                 * | CompiledMethodWrapper | J9JITDataCacheHeader (UnCompressed)| TR_AOTMethodHeader (UnCompressed)|
                 * (Metadata + Compiled Code Data) (Compressed) |
                 * ------------------------------------------------------------------------------------------------------------------------------------------------
                 *
                 */
                ...
                int aotMethodHeaderSize = sizeof(J9JITDataCacheHeader) + sizeof(TR_AOTMethodHeader);
                int compressedDataSize = deflateBuffer((U_8 *)(originalData) + aotMethodHeaderSize,
                    dataSize + codeSize - aotMethodHeaderSize, (U_8 *)(compressedData) + aotMethodHeaderSize,
                ...
                if (compressedDataSize != COMPRESSION_FAILED) {
                     ...
                     * Because of this reason we copy both metadata and compiled code in one buffer and deflate it
                     * together.
                     * TODO: We will always query the shared class cache to get full data for stored AOT compiled method
                     * that is combined meta data and code data. Even if compression of AOT bytes is disabled, we do not
                     * need to send two different buffers to store in cache and also as no one queries either code data
                     * / metadata for method, clean up the share classs cache API.
                     */
                    const J9JITDataCacheHeader *aotMethodHeader
                        = reinterpret_cast<const J9JITDataCacheHeader *>(dataStart);
                    TR_AOTMethodHeader *aotMethodHeaderEntry = const_cast<TR_AOTMethodHeader *>(
                        reinterpret_cast<const TR_AOTMethodHeader *>(aotMethodHeader + 1));
                    ...
                    if (TR::Options::getVerboseOption(TR_VerboseAOTCompression)) {
                        TR_VerboseLog::writeLineLocked(TR_Vlog_AOTCOMPRESSION,
                            "%s : Compression of method data Successful - Original method size = %d bytes, Compressed "
                            "Method Size = %d bytes",
                            comp->signature(), dataSize + codeSize, metadataToStoreSize);
                    }
                } else {
                    if (TR::Options::getVerboseOption(TR_VerboseAOTCompression)) {
                        TR_VerboseLog::writeLineLocked(TR_Vlog_AOTCOMPRESSION,
                            "!%s : Compression of method data Failed - Original method size = %d bytes",
                            comp->signature(), dataSize + codeSize);
                    }
                }
            }
        } catch (const std::bad_alloc &allocationFailure) {
            // In case we can not allocate extra memory to hold temp data for compressing, we still the uncompressed
            // data into the cache
            if (TR::Options::getVerboseOption(TR_VerboseAOTCompression)) {
                TR_VerboseLog::writeLineLocked(TR_Vlog_AOTCOMPRESSION,
                    "!%s : Method will not be compressed as necessary memory can not be allocated, Method Size = %d "
                    "bytes",
                    comp->signature(), dataSize + codeSize);
            }
        }
        ...
        storedCompiledMethod = reinterpret_cast<const J9JITDataCacheHeader *>(
            jitConfig->javaVM->sharedClassConfig->storeCompiledMethod(vmThread, romMethod, (const U_8 *)metadataToStore,
                metadataToStoreSize, (const U_8 *)codedataToStore, codedataToStoreSize, 0));
...
TR_LowPriorityCompQueue::TR_LowPriorityCompQueue()
    ...
    , _STAT_conflict(0)
    , _STAT_staleScrubbed(0)
    , _STAT_bypass(0)
    , _STAT_compReqQueuedByJIT(0)
    , _STAT_LPQcompFromIprofiler(0)
    , _STAT_LPQcompFromInterpreter(0)
    , _STAT_LPQcompUpgrade(0)
    ,
#if defined(J9VM_OPT_JITSERVER)
    _STAT_compReqQueuedByJITServer(0)
    , _STAT_LPQcompServerUnavailable(0)
    ,
#endif /* defined(J9VM_OPT_JITSERVER) */
    _STAT_compReqQueuedByInterpreter(0)
...
{
    if (threshold > 0 && _compInfo->getCpuUtil()
        && _compInfo->getCpuUtil()->isFunctional()) // Need to have support for CPU utilization
    {
        ...
        if (_spine) {
            ...
            if (TR::Options::getCmdLineOptions()->getVerboseOption(TR_VerbosePerformance)) {
                TR_VerboseLog::writeLineLocked(TR_Vlog_INFO, "t=%6u Allocated the LPQ tracking hashtable",
                    (uint32_t)_compInfo->getPersistentInfo()->getElapsedTime());
            }
            FLUSH_MEMORY(TR::Compiler->target.isSMP());
...
inline bool TR_LowPriorityCompQueue::isTrackableMethod(J9Method *j9method) const
{
        ...
        if (++entry->_count > _threshold) // hot entry; possibly queue compilation
        {
            if (!entry->_queuedForCompilation) {
                ...
                if (TR::Options::sharedClassCache() && // must have AOT enabled
                    ...
                    if (vmThread->javaVM->sharedClassConfig->existsCachedCodeForROMMethod(vmThread,
                            fe->getROMMethodFromRAMMethod(j9method)))
                        entry->setInvalid();
                } else {
                    ...
                    if (isLoopy) {
                        ...
                        // For bootstrap romMethods present in SCC (but no AOT body) and loaded during startup, use low
                        // counts For everybody else use high counts
                        ...
                        enoughIPInfo = (crtInvocationCount + TR::Options::_invocationThresholdToTriggerLowPriComp
                            < guessedInitialCount);
                    } else {
                        enoughIPInfo = true;
                    }
                    if (enoughIPInfo) {
                        ...
                        bool enqueued
                            = addFirstTimeCompReqToLPQ(j9method, TR_MethodToBeCompiled::REASON_IPROFILER_CALLS);
                        ...
                        if (enqueued && _compInfo->canProcessLowPriorityRequest()) {
                            if (_compInfo->getNumCompThreadsJobless() > 0) {
                                ...
                                if (TR::Options::getCmdLineOptions()->getVerboseOption(TR_VerboseCompilationThreads)) {
                                    TR_VerboseLog::writeLineLocked(TR_Vlog_INFO,
                                        "t=%6u LPQ logic waking up a sleeping comp thread. Jobless=%d",
                                        (uint32_t)_compInfo->getPersistentInfo()->getElapsedTime(),
                            ...
                            } else // At least one thread is active.
                            {
                                ...
                                int32_t numCompThreadsSuspended = _compInfo->getNumUsableCompilationThreads()
                                ...
                                if (numCompThreadsSuspended > 1) {
                                    ...
                                    if (activate == TR_yes
                                        || (activate == TR_maybe
                                            ...
                                            && // ConcurrentLPQ is too damaging to startup
                                            _compInfo->getNumCompThreadsActive() + 2 < _compInfo->getNumTargetCPUs())) {
                                        TR::CompilationInfoPerThread *compInfoPT
                                            = _compInfo->getFirstSuspendedCompilationThread();
                                        compInfoPT->resumeCompilationThread();
                                        if (TR::Options::getCmdLineOptions()->getVerboseOption(
                                                TR_VerboseCompilationThreads)) {
                                            TR_VerboseLog::writeLineLocked(TR_Vlog_INFO,
                                                "t=%6u Activate compThread %d to handle LPQ request. Qweight=%d "
                                                "active=%d",
                                                (uint32_t)_compInfo->getPersistentInfo()->getElapsedTime(),
                                                compInfoPT->getCompThreadId(), _compInfo->getQueueWeight(),
                                                _compInfo->getNumCompThreadsActive());
                                        }
                                    }
                                }
                        ...
                        if (enqueued) {
                            if (TR::Options::getCmdLineOptions()->getVerboseOption(TR_VerboseCompileRequest)) {
                                TR_VerboseLog::writeLineLocked(TR_Vlog_CR,
                                    "t=%u Compile request to LPQ for j9m=%p loopy=%d smpl=%u cnt=%d Q_SZ=%d Q_SZI=%d "
                                    "LPQ_SZ=%d CPU=%d%% JVM_CPU=%d%%",
                                    (uint32_t)_compInfo->getPersistentInfo()->getElapsedTime(), j9method, isLoopy,
                                    entry->_count, (int32_t)TR::CompilationInfo::getInvocationCount(j9method),
                                    _compInfo->getMethodQueueSize(), _compInfo->getNumQueuedFirstTimeCompilations(),
                                    getLowPriorityQueueSize(), _compInfo->getCpuUtil()->getCpuUsage(),
    ...
    } else // no match
    {
        if (entry->isInvalid()) // entry is invalid, so we can reuse it
        {
            ...
            if (!isTrackableMethod((J9Method *)entry->_j9method)) {
                ...
                // TR_LowPriorityCompQueue::hash(j9method));
            }
        }
    }
}
...
void TR_LowPriorityCompQueue::incStatsReqQueuedToLPQ(uint8_t reason)
{
    switch (reason) {
        case TR_MethodToBeCompiled::REASON_IPROFILER_CALLS:
            _STAT_compReqQueuedByIProfiler++;
            break;
        case TR_MethodToBeCompiled::REASON_LOW_COUNT_EXPIRED:
            _STAT_compReqQueuedByInterpreter++;
            break;
        case TR_MethodToBeCompiled::REASON_UPGRADE:
            _STAT_compReqQueuedByJIT++;
            break;
#if defined(J9VM_OPT_JITSERVER)
        case TR_MethodToBeCompiled::REASON_SERVER_UNAVAILABLE:
            _STAT_compReqQueuedByJITServer++;
            break;
#endif /* defined (J9VM_OPT_JITSERVER) */
        default:
            TR_ASSERT(false, "No other known reason for LPQ compilations\n");
...
void TR_LowPriorityCompQueue::printStats() const
{
    ...
    fprintf(stderr, "   Comps.  from LPQ = %4u (Sources: IProfiler=%3u Interpreter=%3u JIT=%3u)\n",
        _STAT_LPQcompFromIprofiler + _STAT_LPQcompFromInterpreter + _STAT_LPQcompUpgrade, _STAT_LPQcompFromIprofiler,
        _STAT_LPQcompFromInterpreter, _STAT_LPQcompUpgrade);
#endif /* defined(J9VM_OPT_JITSERVER) */
    ...
    while (cur) {
        ...
        if (method) {
            if (J9_CLASS_FROM_METHOD(method) == unloadedClass
                ...
                if (verbose)
                    TR_VerboseLog::writeLineLocked(TR_Vlog_HK,
                        "Invalidating compile request from LPQ for method=%p class=%p", method, unloadedClass);
...
TR_MethodToBeCompiled *TR_LowPriorityCompQueue::findAndDequeueFromLPQ(TR::IlGeneratorMethodDetails &details,
...
TR_CompilationErrorCode TR::CompilationInfo::scheduleLPQAndBumpCount(TR::IlGeneratorMethodDetails &details,
    TR_J9VMBase *fe)
{
    J9Method *method = details.getMethod();
    // Search the LPQ and if not found, add method to LPQ and bump invocation count
    ...
    if (count == 0) {
        bool dequeued = false;
        TR_MethodToBeCompiled *entry = getLowPriorityCompQueue().findAndDequeueFromLPQ(details,
            TR_MethodToBeCompiled::REASON_LOW_COUNT_EXPIRED, fe, dequeued);
        if (!dequeued) // Early compilation request
        {
            ...
            {
                queued = getLowPriorityCompQueue().addFirstTimeCompReqToLPQ(method,
                    TR_MethodToBeCompiled::REASON_LOW_COUNT_EXPIRED);
            }
            if (queued) {
                ...
                intptr_t newCount
                    = getCount(romMethod, TR::Options::getCmdLineOptions(), TR::Options::getAOTCmdLineOptions());
                newCount = newCount * (100 - TR::Options::_countPercentageForEarlyCompilation)
                    / 100; // TODO: verify we don't have negative numbers
                if (TR::CompilationInfo::setInvocationCount(method, count, newCount)) {
                    ...
                    bool removed;
                    TR_MethodToBeCompiled *req = getLowPriorityCompQueue().findAndDequeueFromLPQ(details,
                        TR_MethodToBeCompiled::REASON_LOW_COUNT_EXPIRED, fe, removed);
                    if (req && removed) {
    ...
    if (crtTime > _timestampOfLastInterval) // avoid races
    {
        ...
        if (_samplesPerSecondDuringLastInterval > _maxSamplesPerSecond) // update maximum
        {
            ...
            if (_maxSamplesPerSecond >= TR::Options::_sampleDensityBaseThreshold) {
                increaseFactor = 2
                    + ((_maxSamplesPerSecond - TR::Options::_sampleDensityBaseThreshold)
                        / TR::Options::_sampleDensityIncrementThreshold);
            }
        ...
        if (TR::Options::getCmdLineOptions()->getVerboseOption(TR_VerboseSampleDensity))
            TR_VerboseLog::writeLineLocked(TR_Vlog_INFO,
                "t=%6u globalSamplesDensity: %u/%u=%u samples/sec  max=%u samples/sec increaseFactor=%u", crtTime,
                diffSamples, _sizeOfLastInterval, _samplesPerSecondDuringLastInterval, _maxSamplesPerSecond,
                _increaseFactor);
    }
}
...
J9Method_HT::HT_Entry::HT_Entry(J9Method *j9method, uint64_t timestamp)
    : _next(NULL)
    , _j9method(j9method)
    , _count(TR::CompilationInfo::getInvocationCount(j9method))
    , _seqID(0)
...
bool J9Method_HT::addNewEntry(J9Method *j9method, uint64_t timestamp)
{
    ...
    if (TR::Options::getVerboseOption(TR_VerboseSampling))
        TR_VerboseLog::writeLineLocked(TR_Vlog_SAMPLING,
            "t=%6u J9MethodTracking: j9m=%p Adding new entry. compiled:%d success=%d totalEntries=%u",
...
bool DLTTracking::shouldIssueDLTCompilation(J9Method *j9method, int32_t numHitsInDLTBuffer)
{
    ...
    if (methodCount <= 0 || numHitsInDLTBuffer >= TR::Options::_numDLTBufferMatchesToEagerlyIssueCompReq) {
        if (TR::Options::getVerboseOption(TR_VerboseSampling))
            TR_VerboseLog::writeLineLocked(TR_Vlog_SAMPLING,
                "t=%6u DLTTracking: j9m=%p Issue DLT because methodCount=%d numHitsInDLTBuffer=%d",
    ...
    if (entry) // if method already exists in DLT_HT
    {
        ...
        if (entryCount == methodCount) {
            ...
            if (methodCount < entryCount) {
                ...
                do {
                    ...
                    != VM_AtomicSupport::lockCompareExchangeU32((uint32_t *)&entry->_count, oldValue, newValue));
        ...
        if (TR::Options::getVerboseOption(TR_VerboseSampling))
            TR_VerboseLog::writeLineLocked(TR_Vlog_SAMPLING,
                "t=%6u DLTTracking: j9m=%p issueDLT=%d entry=%p entryCount=%d methodCount=%d seqID=%d",
                (unsigned)getPersistentInfo()->getElapsedTime(), j9method, shouldIssueDLT, entry, entryCount,
                methodCount, entry->_seqID);
...
void DLTTracking::adjustStoredCounterForMethod(J9Method *j9method, int32_t countDiff)
{
    ...
    if (entry) {
        ...
        if (TR::Options::getVerboseOption(TR_VerboseSampling))
            TR_VerboseLog::writeLineLocked(TR_Vlog_SAMPLING,
                "t=%6u DLTTracking: j9m=%p entry=%p adjusting entry count to %d",
...
{
    if (!options->getOption(TR_EnableJProfiling) || entry->isJNINative() || entry->_oldStartPC != 0
        || // reject non first time compilations
        !entry->getMethodDetails().isOrdinaryMethod() || entry->_optimizationPlan->insertInstrumentation()
        || !options->canJITCompile() || options->getOption(TR_NoRecompile) || !options->allowRecompilation())
        return false;
    ...
    while (cur) {
        ...
        if (method) {
            if (J9_CLASS_FROM_METHOD(method) == unloadedClass
                ...
                if (verbose)
                    TR_VerboseLog::writeLineLocked(TR_Vlog_HK,
                        "Invalidating compile request from JPQ for method=%p class=%p", method, unloadedClass);
...
bool TR::CompilationInfo::canProcessJProfilingRequest()
{
    ...
    if (getJProfilingCompQueue().getAllowProcessing()) {
        ...
        if (_jitConfig->javaVM->phase != J9VM_PHASE_NOT_STARTUP || getPersistentInfo()->getJitState() == STARTUP_STATE
...
bool TR::CompilationInfo::canRelocateMethod(TR::Compilation *comp)
{
#if defined(J9VM_OPT_JITSERVER)
...
TR_MethodToBeCompiled *TR::CompilationInfo::addOutOfProcessMethodToBeCompiled(JITServer::ServerStream *stream)
{
    ...
    if (entry) {
        ...
        if (activate == TR_yes) {
            ...
            if (compInfoPT) {
                ...
                if (TR::Options::getCmdLineOptions()->getVerboseOption(TR_VerboseCompilationThreads)) {
                    TR_VerboseLog::writeLineLocked(TR_Vlog_INFO, "t=%6u Activate compThread %d Qweight=%d active=%d",
                        (uint32_t)getPersistentInfo()->getElapsedTime(), compInfoPT->getCompThreadId(),
                        getQueueWeight(), getNumCompThreadsActive());
                }
            }
        }
    }
...
void TR::CompilationInfo::requeueOutOfProcessEntry(TR_MethodToBeCompiled *entry)
{
    TR_ASSERT(getPersistentInfo()->getRemoteCompilationMode() == JITServer::SERVER,
        "Should be called in JITServer server mode only");
...
static bool queryJITServerFilter(const char *methodSig, TR::Method::Type ty, TR::CompilationFilters *filters)
{
    if (!filters)
        return true;
...
bool TR::CompilationInfo::methodCanBeJITServerAOTCacheStored(const char *methodSig, TR::Method::Type ty)
{
    return queryJITServerFilter(methodSig, ty, TR::Options::_JITServerAOTCacheStoreFilters);
}

bool TR::CompilationInfo::methodCanBeJITServerAOTCacheLoaded(const char *methodSig, TR::Method::Type ty)
{
    return queryJITServerFilter(methodSig, ty, TR::Options::_JITServerAOTCacheLoadFilters);
}

bool TR::CompilationInfo::methodCanBeRemotelyCompiled(const char *methodSig, TR::Method::Type ty)
{
    return queryJITServerFilter(methodSig, ty, TR::Options::_JITServerRemoteExcludeFilters);
}

void TR::CompilationInfo::notifyCompilationThreadsOfDeserializerReset()
{
    for (int32_t i = getFirstCompThreadID(); i <= getLastCompThreadID(); i++) {
        TR::CompilationInfoPerThread *curCompThreadInfoPT = _arrayOfCompilationInfoPerThread[i];
        TR_ASSERT(curCompThreadInfoPT, "a thread's compinfo is missing\n");
