...
public:
    ...
    using ClassInfoTuple = std::tuple<std::string, // 0:  _packedROMClass
    ...
    static J9ROMClass *romClassFromString(const std::string &romClassStr, const std::string &romClassHashStr,
    ...
    static bool getAndCacheRAMClassInfo(J9Class *clazz, ClientSessionData *clientSessionData,
        JITServer::ServerStream *stream, ClassInfoDataType dataType1, void *data1, ClassInfoDataType dataType2,
        void *data2);
    ...
    static uintptr_t getRemoteClassDepthAndFlagsWhenROMClassNotCached(J9Class *clazz,
    ...
    static uintptr_t walkReferenceChainWithOffsets(TR_J9VM *fe, const std::vector<uintptr_t> &listOfOffsets,
        uintptr_t receiver);

    // Called by the client as part of preparing the compilation request and handling the SharedCache_rememberClass
    // message when AOT cache is enabled. Returns the list of RAMClasses in the class chain for clazz and populates
    // uncachedRAMClasses and uncachedClassInfos with the classes (and their data) that the client has not yet sent to
    // the server.
    static std::vector<J9Class *> getRAMClassChain(J9Class *clazz, size_t numClasses, J9VMThread *vmThread,
