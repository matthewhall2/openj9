...
public:
    ...
    bool isExcludedPostRestore() { return _invalidationReasons.contains(TR_JitBodyInvalidations::PostRestoreExclude); }
    ...
    enum InfoBits {
        ...
        HasBeenReplaced
            = 0x00100000, // HCR: this struct is for the old version of a replaced method
    ...
    void setNextCompileLevel(TR_Hotness level, bool profile)
    {
        _nextHotness = level;
        if (profile)
            TR_ASSERT(!profilingDisabled(), "assertion failure");
...
private:
    ...
    uint8_t _numberOfInlinedMethodRedefinition; // how many times this method triggers recompilation because of its
                                                // inlined callees being redefined
...
public:
    ...
    void setHasEdoSnippet()
    {
        _flags.set(HasEdoSnippet, true);
    } // set by codegen when the recompilation snippet is created
    ...
    void setIsInvalidated(TR_JitBodyInvalidations::Reason reason) { _invalidationReasons.add(reason); }
    ...
    uint8_t decAggressiveRecompilationChances()
    {
        return _aggressiveRecompilationChances > 0 ? --_aggressiveRecompilationChances : 0;
    }

    uint8_t getNumScorchingIntervals() const { return _numScorchingIntervals; }

    void incNumScorchingIntervals()
    {
        if (_numScorchingIntervals < 255)
            ++_numScorchingIntervals;
    }
...
private:
    ...
    TR_PersistentJittedBodyInfo(TR_PersistentMethodInfo *methodInfo, TR_Hotness hotness, bool profile,
