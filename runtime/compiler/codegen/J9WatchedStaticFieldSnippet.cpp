...
TR::J9WatchedStaticFieldSnippet::J9WatchedStaticFieldSnippet(TR::CodeGenerator *cg, TR::Node *node, J9Method *m,
...
uint8_t *TR::J9WatchedStaticFieldSnippet::emitSnippetBody()
{
    ...
    // We emit the dataSnippet based on the assumption that the J9JITWatchedStaticFieldData structure is laid out as
    // below:
    /*
    ...
    */
    ...
    if (cg()->comp()->getOption(TR_UseSymbolValidationManager)) {
        cg()->addExternalRelocation(
            TR::ExternalRelocation::create(cursor + offsetof(J9JITWatchedStaticFieldData, method),
        ...
        cg()->addExternalRelocation(
            TR::ExternalRelocation::create(cursor + offsetof(J9JITWatchedStaticFieldData, method), NULL, TR_RamMethod,
                cg()),
            __FILE__, __LINE__, node);
    }

    bool isResolved = !node->getSymbolReference()->isUnresolved();
    // If the field is unresolved then we populate these snippet fields with the correct value at runtime (via the
    // instructions generated by generateFillInDataBlockSequenceForUnresolvedField) and hence don't need to add
    // relocation records here.
    if (isResolved) {
        if (cg()->needRelocationsForStatics()) {
            cg()->addExternalRelocation(
                TR::ExternalRelocation::create(cursor + offsetof(J9JITWatchedStaticFieldData, fieldAddress),
                    reinterpret_cast<uint8_t *>(node->getSymbolReference()),
                    reinterpret_cast<uint8_t *>(node->getInlinedSiteIndex()), TR_DataAddress, cg()),
                __FILE__, __LINE__, node);
        }

        if (cg()->comp()->getOption(TR_UseSymbolValidationManager)) {
            cg()->addExternalRelocation(
                TR::ExternalRelocation::create(cursor + offsetof(J9JITWatchedStaticFieldData, fieldClass),
                    reinterpret_cast<uint8_t *>(staticFieldData.fieldClass),
                    reinterpret_cast<uint8_t *>(TR::SymbolType::typeClass), TR_SymbolFromManager, cg()),
                __FILE__, __LINE__, node);
        }
        // relocations for TR_ClassAddress are needed for AOT/AOTaaS compiles and not needed for regular JIT and
        // JITServer compiles. cg->needClassAndMethodPointerRelocations() tells us whether a relocation is needed
        // depending on the type of compile being performed.
        else if (cg()->needClassAndMethodPointerRelocations()) {
            // As things currently stand, this will not work on Power because TR_ClassAddress is used to a generate a 5
            // instruction sequence that materializes the address into a register. Meanwhile we are using
            // TR_ClassAddress here to represent a contiguous word. A short-term solution would be to use
            // TR_ClassPointer. However this is hacky because TR_ClassPointer expects an aconst node (so we would have
            // to create a dummy node). The proper solution would be to implement the functionality in the power
            // codegenerator to be able to patch TR_ClassAddress contiguous word.
            cg()->addExternalRelocation(
                TR::ExternalRelocation::create(cursor + offsetof(J9JITWatchedStaticFieldData, fieldClass),
                    reinterpret_cast<uint8_t *>(node->getSymbolReference()),
                    reinterpret_cast<uint8_t *>(node->getInlinedSiteIndex()), TR_ClassAddress, cg()),
                __FILE__, __LINE__, node);
        }
    }
...
void TR::J9WatchedStaticFieldSnippet::print(OMR::Logger *log, TR_Debug *debug)
{
