...
int32_t J9::SetMonitorStateOnBlockEntry::addSuccessors(TR::CFGNode *cfgNode,
    TR_Stack<TR::SymbolReference *> *monitorStack, bool traceIt, bool dontPropagateMonitor, MonitorInBlock monitorType,
    int32_t callerIndex, bool walkOnlyExceptionSuccs)
{
    OMR::Logger *log = comp()->log();

    logprintf(traceIt, log,
        "\tIn SMSOBE::addSuccessors for cfgNode %d, monitorStack %p dontPropagateMonitor %d monitorType = %d "
    ...
    for (TR::CFGEdge *edge = succs.getFirst(); edge; edge = succs.getNext()) {
        ...
        if (succBlock->getEntry()) {
            ...
            if (monitorType == MonitorEnter) {
                logprintf(traceIt, log,
                    "\tIn J9::SetMonitorStateOnBlockEntry::addSuccessors monitorType = MonitorEnter  block %d\n",
                    succBlock->getNumber());
            ...
            logprintf(traceIt, log,
                "process succBlock %d propagate (t/f: %d) isCatchBlock=%d monitorType=%d callerIndex=%d "
                "entryCallerIndex=%d\n",
                succBlock->getNumber(), addInfo, succBlock->isCatchBlock(), monitorType, callerIndex,
            ...
            if (monitorStack) {
                ...
                //
                if (!addInfo && !monitorStack->isEmpty()) {
                    popMonitor = true;
                }

                if (succBlock->getVisitCount() != _visitCount) {
                    TR_Stack<TR::SymbolReference *> *newMonitorStack
                        = new (trHeapMemory()) TR_Stack<TR::SymbolReference *>(*monitorStack);

                    logprintf(traceIt, log,
                        "\tIn SMSOnBE::addSuccesors  created newMonitorStack %p and monitorStack %p\n", newMonitorStack,
                        monitorStack);

                    if (popMonitor) {
                        logprintf(traceIt, log, "popping monitor symRef=%d before propagation\n",
                    ...
                    logprintf(traceIt, log, "adding monitorstack to successor %d (%p size %d)\n",
                    ...
                    if (!succBlock->isOSRCatchBlock() && !succBlock->isOSRCodeBlock()) {
                        if (!isMonitorStateConsistentForBlock(succBlock, monitorStack, popMonitor))
                            ...
                            logprintf(traceIt, log, "verified block_%d monitorState is consistent\n",
                                succBlock->getNumber());
                        }
                    }
                    continue;
            ...
            logprintf(traceIt, log, "\tIn SMSOnBE::addSuccessors adding block %d to blocksToVisit\n",
                succBlock->getNumber());
...
bool J9::SetMonitorStateOnBlockEntry::isMonitorStateConsistentForBlock(TR::Block *block,
    TR_Stack<TR::SymbolReference *> *newMonitorStack, bool popMonitor)
{
    TR_Stack<TR::SymbolReference *> *oldMonitorStack
    ...
    logprintf(traceIt, log, "MonitorState block_%d: oldMonitorStack %p newMonitorStack %p popMonitor %d\n",
    ...
    if (!newMonitorStack || newMonitorStack->isEmpty() || (newMonitorStack->size() == 1 && popMonitor))
        newMonitorStackEmpty = true;
    if (oldMonitorStackEmpty != newMonitorStackEmpty) {
        logprintf(traceIt, log,
            "MonitorState inconsistent for block_%d: oldMonitorStack isEmpty %d, newMonitorStack isEmpty %d\n",
    ...
    if (newSize != oldSize) {
        logprintf(traceIt, log,
            "MonitorState inconsistent for block_%d: oldMonitorStack size %d, newMonitorStack size %d\n",
    ...
    for (int i = oldMonitorStack->topIndex(); i >= 0; i--) {
        if (newMonitorStack->element(i)->getReferenceNumber() != oldMonitorStack->element(i)->getReferenceNumber()) {
            logprintf(traceIt, log,
                "MonitorState inconsistent for block_%d: oldMonitorStack(%d) symRef=%d, newMonitorStack(%d) "
                "symRef=%d\n",
                block->getNumber(), i, oldMonitorStack->element(i)->getReferenceNumber(), i,
                newMonitorStack->element(i)->getReferenceNumber());
            return false;
        }
    }
    return true;
...
//
//
static bool canPopMonitorStack(TR::Compilation *comp, TR_Stack<TR::SymbolReference *> *monitorStack, TR::Node *node,
    bool blockExitsMethod, bool traceIt)
{
    OMR::Logger *log = comp->log();
    ...
    if (comp->isDLT()) {
        if (callerIndex == -1) {
            if (comp->getJittedMethodSymbol()->isSynchronised()) {
                ...
                //
                if (monitorStack->size() == 1 && !comp->getJittedMethodSymbol()->isStatic() && !blockExitsMethod) {
                    logprintf(traceIt, log,
                        "monitorStack is empty (except for special DLT sync object slot) for DLT compile at monexit "
                        "%p\n",
                        node);
                    return false;
                } else if (monitorStack->isEmpty()) {
                    logprintf(traceIt, log, "monitorStack is empty for DLT compile at monexit %p\n", node);
                    return false;
                }
            } else {
                if (monitorStack->isEmpty()) {
                    logprintf(traceIt, log, "monitorStack is empty for non-synchronized DLT compile at monexit %p\n",
                        node);
                    return false;
                }
            }
            ...
            monitorStack->top()->getOwningMethodIndex(), monitorStack->top()->getReferenceNumber());
               TR_ASSERT(monitorStack->top()->getOwningMethodIndex() == callerIndex, "unbalanced monitorStack, trying to
...
static bool needToPushMonitor(TR::Compilation *comp, TR::Block *block, bool traceIt)
{
    ...
    for (auto e = block->getSuccessors().begin(); e != block->getSuccessors().end(); ++e) {
        ...
        if (comp->getFlowGraph()->compareExceptionSuccessors(block, succ) == 0) {
            logprintf(traceIt, log, "found identical exception successors for block %d and succ %d\n",
            ...
            for (TR::TreeTop *tt = succ->getEntry(); tt != succ->getExit(); tt = tt->getNextTreeTop()) {
                ...
                if (aNode
                    && (aNode->getOpCodeValue() == TR::tfinish || aNode->getOpCodeValue() == TR::monexit
                        || ((aNode->getOpCodeValue() == TR::treetop || aNode->getOpCodeValue() == TR::NULLCHK)
                            && aNode->getFirstChild()->getOpCodeValue() == TR::monexit))) {
                    logprintf(traceIt, log,
                        "overriding identical exception decision because node %p in block %d is either monexit or "
                        "tfinish",
                        aNode, succ->getNumber());
                    retval = false;
                    break;
                }
            }
...
void J9::SetMonitorStateOnBlockEntry::set(bool &lmmdFailed, bool traceIt)
{
    ...
    static bool traceInitMonitorsForExceptionAfterMonexit
        = feGetEnv("TR_traceInitMonitorsForExceptionAfterMonexit") ? true : false;
    ...
    while (!_blocksToVisit.isEmpty()) {
        ...
        logprintf(traceIt, log, "block to process: %d\n", block->getNumber());
        ...
        if (traceIt && monitorStack && !monitorStack->isEmpty())
            log->printf("top of the stack symRef=%d, and size=%d\n", monitorStack->top()->getReferenceNumber(),
                monitorStack->size());
        else
            logprints(traceIt, log, "monitor stack is empty\n");
        ...
        for (TR::TreeTop *tt = block->getEntry();; tt = tt->getNextTreeTop()) {
            ...
            if ((node->getOpCode().isStore() && // only monents are represented by this store now
                ...
                if (monitorStack) {
                    ...
                    logprintf(traceIt, log, "adding monitor to stack symbol=%p symRef=%d (size=%d) (node %p)\n",
                        node->getSymbol(), node->getSymbolReference()->getReferenceNumber(), monitorStack->size() + 1,
                        node);
                } else {
                    monitorStack = new (trHeapMemory()) TR_Stack<TR::SymbolReference *>(trMemory());
                    logprintf(traceIt, log, "adding monitor to fresh stack symbol=%p symRef=%d (size=%d) (node %p)\n",
                        node->getSymbol(), node->getSymbolReference()->getReferenceNumber(), monitorStack->size() + 1,
                        node);
                }
                ...
                //
                if (node->getSymbolReference()->holdsMonitoredObjectForSyncMethod() && (callerIndex == -1))
                    isSyncMethodMonent = true;
            } else if ((node->getOpCode().getOpCodeValue() == TR::monexitfence) && monitorStack
                ...
                //
                monitorPoppedForExceptionSucc = addSuccessors(block, monitorStack, traceIt, false /*not used*/,
                    MonitorExit, callerIndex, true /*walkOnlyExceptionSuccs*/);
                // monexit
                if (monitorStack->topIndex() == 0) {
                    monitorStack = new (trHeapMemory()) TR_Stack<TR::SymbolReference *>(*monitorStack);
                    logprintf(traceIt, log, "popping monitor off stack symRef=%d, BEFORE pop size=%d, ",
                        monitorStack->top()->getReferenceNumber(), monitorStack->size());
                    monitorStack->pop();
                    logprintf(traceIt, log, "AFTER size=%d\n", monitorStack->size());
                }
                ...
                if (monitorExitFence > 0) {
                    ...
                    for (; edge != block->getExceptionSuccessors().end(); ++edge) {
                        ...
                        if (node->getByteCodeInfo().getCallerIndex()
                            == succBlock->getEntry()->getNode()->getByteCodeInfo().getCallerIndex()) {
                            logprintf(traceInitMonitorsForExceptionAfterMonexit, log,
                                "block_%d has exceptions after monexit with catch block in the same method %s\n",
            ...
            if (tt == block->getExit()) {
                ...
                if ((monitorExitFence + monitorEnterStore) >= 2) {
                    logprintf(traceIt, log, "block_%d has monitorEnterStore=%d monitorExitFence=%d\n",
                ...
                if (blockHasMonexit) {
                    ...
                    if ((monitorPoppedForExceptionSucc > 0) && monitorStackTop
                        && needToPushMonitor(comp(), block, traceIt)) {
                        logprintf(traceIt, log, "pushing monexit symRef=%d back temporarily\n",
                ...
                logprintf(traceIt, log,
                    "blockHasMonent=%d blockHasMonexit=%d dontPropagateMonitor=%d callerIndex=%d "
                    "monitorPoppedForExceptionSucc=%d\n",
            ...
            if (autos) {
                ...
                for (TR::RegisterMappedSymbol *a = iterator.getFirst(); a; a = iterator.getNext()) {
                    TR::DebugCounter::incStaticDebugCounter(comp(),
                        TR::DebugCounter::debugCounterName(comp(), "lmmdFailed/(%s)", comp()->signature()));
