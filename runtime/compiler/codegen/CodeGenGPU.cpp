...
static void getAutoOrParmName(TR::Symbol *sym, char *s, bool addr = true)
{
    ...
    if (sym->isParm())
        TR::snprintfNoTrunc(s, MAX_NAME, "%%p%" OMR_PRId32 "%s", sym->castToParmSymbol()->getSlot(),
            addr ? ".addr" : "");
    else
        TR::snprintfNoTrunc(s, MAX_NAME, "%%a%" OMR_PRId32 "%s", sym->castToAutoSymbol()->getLiveLocalIndex(),
            addr ? ".addr" : "");
}
...
bool J9::CodeGenerator::handleRecognizedMethod(TR::Node *node, NVVMIRBuffer &ir, TR::Compilation *comp)
{
    ...
    switch (((TR::MethodSymbol *)node->getSymbolReference()->getSymbol())->getRecognizedMethod()) {
        ...
        case TR::java_lang_Math_abs_D:
            ...
            ir.print("  %%%d = call double @__nv_%s(double %s)\n", node->getLocalIndex(), getNVVMMathFunctionName(node),
                name0);
            break;
        case TR::java_lang_StrictMath_sin:
        case TR::java_lang_StrictMath_cos:
...
void J9::CodeGenerator::printArrayCopyNVVMIR(TR::Node *node, NVVMIRBuffer &ir, TR::Compilation *comp)
{
    ...
    */
    ...
    if (isBackwardsCopy) {
        if (!is64bitCopyLength) {
            ir.print("  %%%d = sext %s %s to i64\n", node->getLocalIndex(),
                getTypeName(node->getChild(2 + childrenNodeOffset)->getDataType()), name2);
            node->setLocalIndex(_gpuNodeCount++);
        }

        ir.print("  %%%d = ptrtoint %s %s to i64\n", node->getLocalIndex(),
            getTypeName(node->getChild(0 + childrenNodeOffset)->getDataType()), name0);
        node->setLocalIndex(_gpuNodeCount++);

        ir.print("  %%%d = ptrtoint %s %s to i64\n", node->getLocalIndex(),
            getTypeName(node->getChild(1 + childrenNodeOffset)->getDataType()), name1);
        node->setLocalIndex(_gpuNodeCount++);

        if (is64bitCopyLength) {
            ir.print("  %%%d = add i64 %%%d, %s\n", node->getLocalIndex(), node->getLocalIndex() - 2, name2);
            node->setLocalIndex(_gpuNodeCount++);

            ir.print("  %%%d = add i64 %%%d, %s\n", node->getLocalIndex(), node->getLocalIndex() - 2, name2);
            node->setLocalIndex(_gpuNodeCount++);
        } else {
            ir.print("  %%%d = add i64 %%%d, %%%d\n", node->getLocalIndex(), node->getLocalIndex() - 2,
    ...
    ir.print("  %%%d = phi %s [ %s, %%ArrayCopy%d ], [ %%%d, %%ArrayCopyBody%d ]\n", node->getLocalIndex(),
        getTypeName(node->getChild(2 + childrenNodeOffset)->getDataType()), name2, arrayCopyID,
        unknownCopy ? node->getLocalIndex() + 11 : node->getLocalIndex() + 13, arrayCopyID);
    node->setLocalIndex(_gpuNodeCount++);

    if (!isBackwardsCopy) {
        // source address
        ir.print("  %%%d = phi %s [ %s, %%ArrayCopy%d ], [ %%%d, %%ArrayCopyBody%d ]\n", node->getLocalIndex(),
            getTypeName(node->getChild(0 + childrenNodeOffset)->getDataType()), name0, arrayCopyID,
            unknownCopy ? node->getLocalIndex() + 8 : node->getLocalIndex() + 10, arrayCopyID);
        node->setLocalIndex(_gpuNodeCount++);

        // destination address
        ir.print("  %%%d = phi %s [ %s, %%ArrayCopy%d ], [ %%%d, %%ArrayCopyBody%d ]\n", node->getLocalIndex(),
            getTypeName(node->getChild(1 + childrenNodeOffset)->getDataType()), name1, arrayCopyID,
            unknownCopy ? node->getLocalIndex() + 8 : node->getLocalIndex() + 10, arrayCopyID);
        node->setLocalIndex(_gpuNodeCount++);
    } else {
        // source address
        ir.print("  %%%d = phi %s [ %%%d, %%ArrayCopy%d ], [ %%%d, %%ArrayCopyBody%d ]\n", node->getLocalIndex(),
            getTypeName(node->getChild(0 + childrenNodeOffset)->getDataType()), node->getLocalIndex() - 3, arrayCopyID,
            unknownCopy ? node->getLocalIndex() + 8 : node->getLocalIndex() + 10, arrayCopyID);
        node->setLocalIndex(_gpuNodeCount++);

        // destination address
        ir.print("  %%%d = phi %s [ %%%d, %%ArrayCopy%d ], [ %%%d, %%ArrayCopyBody%d ]\n", node->getLocalIndex(),
            getTypeName(node->getChild(1 + childrenNodeOffset)->getDataType()), node->getLocalIndex() - 3, arrayCopyID,
            unknownCopy ? node->getLocalIndex() + 8 : node->getLocalIndex() + 10, arrayCopyID);
        node->setLocalIndex(_gpuNodeCount++);
    }
    ...
    if (isWordCopy || isHalfwordCopy) {
        ir.print("  %%%d = bitcast %s %%%d to %s\n", node->getLocalIndex(),
            getTypeName(node->getChild(0 + childrenNodeOffset)->getDataType()), node->getLocalIndex() - 2,
            isWordCopy ? "i32*" : "i16*");
        node->setLocalIndex(_gpuNodeCount++);

        ir.print("  %%%d = bitcast %s %%%d to %s\n", node->getLocalIndex(),
            getTypeName(node->getChild(1 + childrenNodeOffset)->getDataType()), node->getLocalIndex() - 2,
            isWordCopy ? "i32*" : "i16*");
        node->setLocalIndex(_gpuNodeCount++);
    }
    ...
    // move source pointer
    ir.print("  %%%d = %s i64 %%%d, %d\n", node->getLocalIndex(), isBackwardsCopy ? "sub" : "add",
...
{
    return symRef->getSymbol()->isParm() && ((TR::ParameterSymbol *)symRef->getSymbol())->getSlot() == 0;
}
...
TR::CodeGenerator::GPUResult J9::CodeGenerator::printNVVMIR(NVVMIRBuffer &ir, TR::Node *node, TR_RegionStructure *loop,
    TR_BitVector *targetBlocks, vcount_t visitCount, TR_SharedMemoryAnnotations *sharedMemory, int32_t &nextParmNum,
    TR::Node *&errorNode)
{
    GPUResult result;
    ...
    if (self()->comp()->isGPUCompilation() && opcode.isLoadVarDirect() && isThisPointer(node->getSymbolReference()))
        return GPUSuccess; // will handle in the parent

    if (opcode.isLoadConst()) {
        if ((node->getDataType() == TR::Address) && (node->getAddress() != 0)) {
            logprintf(trace, log, "Load Const with a non-zero address in node %p\n", node);
    ...
    if (!enableExceptionChecks && (opcode.isNullCheck() || opcode.isBndCheck() || node->getOpCodeValue() == TR::DIVCHK))
        return GPUSuccess;
    ...
    if (opcode.isNullCheck()) {
        ...
        if (_gpuPostDominators && _gpuPostDominators->dominates(_gpuCurrentBlock, _gpuStartBlock)) {
            ...
            if (symRef->getSymbol()->isParm()) {
                ...
                ir.print("; DELETE NULLCHK [%p] (ref[%p] is parm %d) since BB[%d] postdominates BB[%d]\n", node,
                    refNode, argpos, _gpuCurrentBlock->getNumber(), _gpuStartBlock->getNumber());
        ...
        if (node->getChild(0)->getOpCodeValue() == TR::arraylength) {
            ...
            if (isSharedMemory(refNode, sharedMemory, self()->comp())) {
                ...
                isSMReference = true;
                ir.print("; USE CONSTANT LENGTH %d for NULLCHK [%p] since this reference [%p] is allocated in shared "
                         "memory\n",
                    smsize, node, refNode);
            }
        }
        ...
        if (!isSMReference) {
            result = self()->printNVVMIR(ir, node->getChild(0), loop, targetBlocks, visitCount, sharedMemory,
                nextParmNum, errorNode);
            if (result != GPUSuccess)
                return result;
        }
        result = self()->printNVVMIR(ir, node->getChild(1), loop, targetBlocks, visitCount, sharedMemory, nextParmNum,
            errorNode);
        if (result != GPUSuccess)
            return result;
        ...
        result = self()->printNVVMIR(ir, idivNode->getChild(0), loop, targetBlocks, visitCount, sharedMemory,
            nextParmNum, errorNode);
        if (result != GPUSuccess)
            return result;

        result = self()->printNVVMIR(ir, idivNode->getChild(1), loop, targetBlocks, visitCount, sharedMemory,
            nextParmNum, errorNode);
        if (result != GPUSuccess)
            return result;
    ...
    if (node->getOpCode().isLoadVar()
        ...
        node->setLocalIndex(_gpuNodeCount++);
        logprintf(trace, log, "node %p assigned index %d\n", node, node->getLocalIndex());

        ir.print("  %%%d = %s %s* %s, align %d\n", node->getLocalIndex(), getOpCodeName(node->getOpCodeValue()),
            getTypeName(node->getDataType()), name0, node->getSize());
        return GPUSuccess;
    }
    ...
    for (int32_t i = 0; i < node->getNumChildren(); ++i) {
        ...
        if ((child->getReferenceCount() >= 2 && !child->getOpCode().isLoadConst()) || printChildrenWithRefCount1) {
            result
                = self()->printNVVMIR(ir, child, loop, targetBlocks, visitCount, sharedMemory, nextParmNum, errorNode);
            if (result != GPUSuccess)
                return result;
        }
    }
    ...
    // if block has a label previous block has to end with a branch
    else if (node->getOpCodeValue() == TR::BBEnd && !_gpuCurrentBlock->endsInBranch()
        ...
        && _gpuCurrentBlock->getNextBlock() && targetBlocks->get(_gpuCurrentBlock->getNextBlock()->getNumber())) {
        node->setLocalIndex(_gpuNodeCount--);
        ...
        if (node->getNumChildren() == 0) {
            ...
            getNodeName(node->getChild(0), name0, self()->comp());
            ir.print("  %s %s %s\n", getOpCodeName(node->getOpCodeValue()), getTypeName(node->getDataType()), name0);
        }
    } else if (node->getOpCode().isStoreIndirect()) // TODO: handle statics
    {
        ...
        {
            _gpuNodeCount--;
            ir.print("  %%%d = bitcast %s %s to %s %s*\n", _gpuNodeCount, getTypeName(firstChild->getDataType()), name0,
                getTypeName(secondChild->getDataType()), firstChild->chkSharedMemory() ? "addrspace(3)" : "");

            ir.print("  %s %s %s, %s %s * %%%d, align %d\n", getOpCodeName(node->getOpCodeValue()),
                getTypeName(secondChild->getDataType()), name1, getTypeName(secondChild->getDataType()),
                firstChild->chkSharedMemory() ? "addrspace(3)" : "", _gpuNodeCount, secondChild->getSize());
            _gpuNodeCount++;
        }
    } else if (node->getOpCode().isLoadIndirect()) // TODO: handle statics
    {
        ...
        if (node->getSymbolReference()->getCPIndex() != -1) // field of some object
        {
            ...
            TR_ASSERT(firstChild->getOpCode().isLoadDirect() && isThisPointer(firstChild->getSymbolReference()),
            ...
            if (field.getSize() > 0) {
                ir.print("  %%%d = bitcast [%d x %s] addrspace(3)* @%.*s to i8*\n", node->getLocalIndex(),
                    field.getSize(), getTypeNameFromSignature(field.getFieldSig(), field.getFieldSigLength()),
                ...
                ir.print("  %%%d = %s %s* %%p%d.addr, align %d\n", node->getLocalIndex(),
                    getOpCodeName(node->getOpCodeValue()), getTypeName(node->getDataType()), parmNum, node->getSize());
            }
        } else {
            // assume SM35 or more
            ...
            if (node->getSymbolReference()->getSymbol()->isArrayShadowSymbol() &&
                ...
                node->getDataType() != TR::Address && !disableReadOnlyCacheArray && _gpuCanUseReadOnlyCache) {
                ...
                if ((addrNode->getOpCodeValue() == TR::aload) || (addrNode->getOpCodeValue() == TR::aloadi)) {
                    ...
                    if (hostSymRef && (parmSlot != -1)
                        && (gpuSymbolMap[nc]._accessKind & TR::CodeGenerator::ReadWriteAccesses)
                            == TR::CodeGenerator::ReadAccess) {
                        isReadOnlyArray = true;
                    }
                }
            }

            ir.print("  %%%d = bitcast %s %s to %s %s*\n", _gpuNodeCount - 1, getTypeName(firstChild->getDataType()),
                name0, getTypeName(node->getDataType()),
                firstChild->chkSharedMemory() ? "addrspace(3)"
                    : isReadOnlyArray         ? "addrspace(1)"
                                              : "");
            ...
            if (isReadOnlyArray) {
                if (_gpuUseOldLdgCalls) {
                    ir.print(
                        "  %%%d = tail call %s @llvm.nvvm.ldg.global.%s.%s.p1%s(%s addrspace(1)* %%%d), !align !1%d\n",
                        node->getLocalIndex(), getTypeName(node->getDataType()),
                        (node->getDataType() >= TR::Float) ? "f" : "i", getVarTypeName(node->getDataType()),
                        getVarTypeName(node->getDataType()), getTypeName(node->getDataType()), _gpuNodeCount - 2,
                        node->getSize());
                } else {
                ...
                ir.print("  %%%d = %s %s %s * %%%d, align %d\n", node->getLocalIndex(),
                    getOpCodeName(node->getOpCodeValue()), getTypeName(node->getDataType()),
                    firstChild->chkSharedMemory() ? "addrspace(3)" : "", _gpuNodeCount - 2, node->getSize());
        }
        ...
        logprintf(trace, log, "unrecognized call %p\n", node);
        return GPUInvalidProgram;
        ...
        if (!node->getSymbol()->isAutoOrParm()) {
            logprintf(trace, log, "unexpected symbol in node %p\n", node);
            return GPUInvalidProgram;
        }
        ...
        ir.print("  %s %s %s, %s* %s, align %d\n", getOpCodeName(node->getOpCodeValue()),
            getTypeName(node->getChild(0)->getDataType()), name0, getTypeName(node->getDataType()), name1,
        ...
        ir.print("  %%%d = %s inbounds %s %s, %s %s\n", node->getLocalIndex(), getOpCodeName(node->getOpCodeValue()),
            getTypeName(node->getChild(0)->getDataType()), name0, getTypeName(node->getChild(1)->getDataType()), name1);
        ...
        if (_gpuCanUseReadOnlyCache && !disableReadOnlyCacheObjHdr) {
            if (_gpuUseOldLdgCalls) {
                ir.print(
                    "  %%%d = tail call i32 @llvm.nvvm.ldg.global.i.i32.p1i32(i32 addrspace(1)* %%%d), !align !14\n",
                    node->getLocalIndex(), node->getLocalIndex() - 1);
            } else {
    ...
    // Binary Operations
    else if ((node->getOpCodeValue() == TR::lshl || node->getOpCodeValue() == TR::lshr)
        && node->getChild(1)->getDataType() == TR::Int32) {
        getNodeName(node->getChild(0), name0, self()->comp());
        getNodeName(node->getChild(1), name1, self()->comp());

        ir.print("  %%%d = sext i32 %s to i64\n", node->getLocalIndex(), name1);
        node->setLocalIndex(_gpuNodeCount++);

        ir.print("  %%%d = %s %s %s, %%%d\n", node->getLocalIndex(), getOpCodeName(node->getOpCodeValue()),
            getTypeName(node->getDataType()), name0, _gpuNodeCount - 2);
        ...
        if (extendChild0) {
            ir.print("  %%%d = %s %s %s to %s\n", node->getLocalIndex(), isSignedMul ? "sext" : "zext",
                getTypeName(node->getChild(0)->getDataType()), name0, isLongMul ? "i128" : "i64");
            node->setLocalIndex(_gpuNodeCount++);
        } else {
        ...
        if (extendChild1) {
            ir.print("  %%%d = %s %s %s to %s\n", node->getLocalIndex(), isSignedMul ? "sext" : "zext",
                getTypeName(node->getChild(1)->getDataType()), name1, isLongMul ? "i128" : "i64");
            node->setLocalIndex(_gpuNodeCount++);
        } else {
            ir.print("  %%%d = lshr i64 %s, 0\n", node->getLocalIndex(), name1);
            node->setLocalIndex(_gpuNodeCount++);
        ...
        ir.print("  %%%d = mul %s %%%d, %%%d\n", node->getLocalIndex(), isLongMul ? "i128" : "i64",
            node->getLocalIndex() - 2, node->getLocalIndex() - 1);
        node->setLocalIndex(_gpuNodeCount++);

        ir.print("  %%%d = lshr %s %%%d, %s\n", node->getLocalIndex(), isLongMul ? "i128" : "i64",
            node->getLocalIndex() - 1, isLongMul ? "64" : "32");
        node->setLocalIndex(_gpuNodeCount++);

        ir.print("  %%%d = trunc %s %%%d to %s\n", node->getLocalIndex(), isLongMul ? "i128" : "i64",
            node->getLocalIndex() - 1, isLongMul ? "i64" : "i32");
        ...
        ir.print("  %%%d = sub %s %%%d, %%%d\n", node->getLocalIndex(), getTypeName(node->getDataType()),
            node->getLocalIndex() - 1, node->getLocalIndex() - 2);
        ...
        ir.print("  %%%d = shl %s %s, %s\n", node->getLocalIndex(), getTypeName(node->getDataType()), name0, name1);
        node->setLocalIndex(_gpuNodeCount++);

        ir.print("  %%%d = sub %s %s, %s\n", node->getLocalIndex(), getTypeName(node->getChild(1)->getDataType()),
            isInt ? "32" : "64", name1);
        node->setLocalIndex(_gpuNodeCount++);

        ir.print("  %%%d = and %s %%%d, %s\n", node->getLocalIndex(), getTypeName(node->getChild(1)->getDataType()),
            node->getLocalIndex() - 1, isInt ? "31" : "63");
        node->setLocalIndex(_gpuNodeCount++);
        ...
        ir.print("  %%%d = or %s %%%d, %%%d\n", node->getLocalIndex(), getTypeName(node->getDataType()),
            node->getLocalIndex() - 4, node->getLocalIndex() - 1);
        ...
        ir.print("  %%%d = %s %s %s to %s\n", node->getLocalIndex(), getOpCodeName(node->getOpCodeValue()),
            getTypeName(node->getChild(0)->getDataType()), name0, getTypeName(node->getDataType()));
    } else if (node->getOpCode().isArithmetic() && node->getNumChildren() == 2
        && getOpCodeName(node->getOpCodeValue())) // supported binary opcode
    {
        ...
        ir.print("  %%%d = %s %s %s, %s\n", node->getLocalIndex(), getOpCodeName(node->getOpCodeValue()),
            getTypeName(node->getDataType()), name0, name1);
    } else if (node->getOpCode().isConversion() && getOpCodeName(node->getOpCodeValue())) {
        getNodeName(node->getChild(0), name0, self()->comp());
        ...
        ir.print("  %s %s %s, label %%block_%d [ ", getOpCodeName(node->getOpCodeValue()),
            getTypeName(node->getChild(0)->getDataType()), name0,
            node->getChild(1)->getBranchDestination()->getNode()->getBlock()->getNumber());
        for (int i = 2; i < node->getNumChildren(); ++i) {
            ir.print("%s %d, label %%block_%d ", getTypeName(node->getChild(0)->getDataType()),
        ...
        ir.print("  %s %s %s, label %%block_%d [ ", getOpCodeName(node->getOpCodeValue()),
            ...
            node->getChild(1)->getBranchDestination()->getNode()->getBlock()->getNumber());
        for (int i = 2; i < node->getNumChildren(); ++i) {
            ir.print("%s %d, label %%block_%d ", getTypeName(node->getChild(0)->getDataType()), i - 2,
                node->getChild(i)->getBranchDestination()->getNode()->getBlock()->getNumber());
        }
        ir.print("]\n");
        node->setLocalIndex(_gpuNodeCount--);
    } else if (node->getOpCode().isBooleanCompare()) // Needs to be after "isIf()" check
    {
        ...
        node->setLocalIndex(_gpuNodeCount--);
    } else if (getOpCodeName(node->getOpCodeValue()) && strcmp(getOpCodeName(node->getOpCodeValue()), "INVALID") == 0) {
        node->setLocalIndex(_gpuNodeCount--);
        logprintf(trace, log, "INVALID operation required by node %p\n", node);
        return GPUInvalidProgram;
    } else {
        node->setLocalIndex(_gpuNodeCount--);
        if (trace) {
            ...
            log->printf("unsupported opcode (%s) on line %d %p\n", node->getOpCode().getName(),
                self()->comp()->getLineNumber(node), node);
        }
...
void traceNVVMIR(TR::Compilation *comp, char *buffer)
{
    ...
    while (*cs != '\0') {
        ...
        memcpy(msg, cs, len);
        msg[len] = '\0';
        logprintf(trace, log, "%6d: %s", line++, msg);
        if (*(ce - 1) == '\0') {
            ce--;
        }
        cs = ce;
...
void J9::CodeGenerator::findExtraParms(TR::Node *node, int32_t &numExtraParms, TR_SharedMemoryAnnotations *sharedMemory,
    vcount_t visitCount)
{
    if (node->getVisitCount() == visitCount)
        return;
...
void J9::CodeGenerator::dumpInvariant(CS2::ArrayOf<gpuParameter, TR::Allocator>::Cursor pit, NVVMIRBuffer &ir,
    bool isbufferalign)
{
    return;
    ...
    for (pit.SetToFirst(); pit.Valid(); pit.SetToNext()) {
        ...
        if (sym->getDataType() == TR::Address) {
            ...
            ir.print("  call void @llvm.invariant.end({}* %%inv_%s_header, i64 %d, i8* %s)\n", &parmName[1],
                self()->objectHeaderInvariant(), parmName);
        }
    }
}
...
TR::CodeGenerator::GPUResult J9::CodeGenerator::dumpNVVMIR(TR::TreeTop *firstTreeTop, TR::TreeTop *lastTreeTop,
    TR_RegionStructure *loop, SharedSparseBitVector *blocksInLoop, ListBase<TR::AutomaticSymbol> *autos,
    ListBase<TR::ParameterSymbol> *parms, bool staticMethod, char *&nvvmIR, TR::Node *&errorNode, int gpuPtxCount,
    bool *hasExceptionChecks)
{
    ...
    if (!calculateComputeCapability(/*tracing*/ 0, &computeMajor, &computeMinor, /*deviceId*/ 0)) {
        logprints(trace, log, "calculateComputeCapability was unsuccessful.\n");
    ...
    if (!getNvvmVersion(/*tracing*/ 0, &nvvmMajorVersion, &nvvmMinorVersion)) {
        logprints(trace, log, "getNvvmVersion was unsuccessful.\n");
        return GPUHelperError;
    }
    ...
    for (TR::TreeTop *tree = firstTreeTop; tree != lastTreeTop; tree = tree->getNextTreeTop()) {
        ...
        if (node->getOpCode().isBranch()) {
            TR_ASSERT(node->getBranchDestination()->getNode()->getOpCodeValue() == TR::BBStart,
                "Attempted to get Block number of a non-BBStart node.");
            targetBlocks.set(node->getBranchDestination()->getNode()->getBlock()->getNumber());

            if (tree->getNextTreeTop() && tree->getNextTreeTop()->getNextTreeTop()) {
                node = tree->getNextTreeTop()->getNextTreeTop()->getNode();
                TR_ASSERT(node->getOpCodeValue() == TR::BBStart,
                    "Attempted to get Block number of a non-BBStart node.");
            ...
            for (int childIndex = 0; childIndex < node->getNumChildren(); ++childIndex) {
                if (node->getChild(childIndex)->getOpCode().isBranch()) {
                    TR_ASSERT(node->getChild(childIndex)->getBranchDestination()->getNode()->getOpCodeValue()
                            == TR::BBStart,
                        "Attempted to get Block number of a non-BBStart node.");
                    targetBlocks.set(
                        node->getChild(childIndex)->getBranchDestination()->getNode()->getBlock()->getNumber());
                }
            }
        } else if (node->getOpCode().isReturn())
    ...
    logprintf(trace, log, "extra parameters = %d\n", numExtraParms);
    ir.print("target triple = \"nvptx64-unknown-cuda\"\n");
    ir.print("target datalayout = "
             "\"e-p:64:64:64-i1:8:8-i8:8:8-i16:16:16-i32:32:32-i64:64:64-f32:32:32-f64:64:64-v16:16:16-v32:32:32-v64:"
             "64:64-v128:128:128-n16:32:64\"\n\n"); // TODO: 32-bit

    // TODO: alignment, arraylength !!!
    for (auto lit = sharedMemory.getSharedMemoryFields().begin(); lit != sharedMemory.getSharedMemoryFields().end();
         ++lit) {
        if ((*lit).getSize() > 0)
    ...
    for (ait.SetToFirst(); ait.Valid(); ait.SetToNext()) {
        if (!ait->_hostSymRef)
            continue;
        logprintf(trace, log, "hostSymRef #%d parmSlot %d\n", (int)ait, ait->_parmSlot);
    ...
    ir.print("%s%s%s %%ExceptionKind", numParms > 0 ? ", " : "",
        self()->comp()->isGPUCompilation() ? "" : "i32 %startInclusive, i32 %endExclusive, ", getTypeName(TR::Address));
    ...
    for (; parm; parm = pi.getNext()) {
        ...
        ir.print("  %s = alloca %s, align %d\n", name, getTypeName(parm->getDataType()), parm->getSize());

        char origName[MAX_NAME];
        getAutoOrParmName(parm, origName, false);
        ir.print("  store %s %s, %s* %s, align %d\n", getTypeName(parm->getDataType()), origName,
            getTypeName(parm->getDataType()), name, parm->getSize());
    }
    ...
    for (pit.SetToFirst(); pit.Valid(); pit.SetToNext()) {
        ...
        ir.print("  %s = alloca %s, align %d\n", addrName, getTypeName(sym->getDataType()), sym->getSize());
        ...
        ir.print("  store %s %s, %s* %s, align %d\n", getTypeName(sym->getDataType()), parmName,
            getTypeName(sym->getDataType()), addrName, sym->getSize());
    }
    ...
    for (TR::AutomaticSymbol *a = ai.getFirst(); a != NULL; a = ai.getNext()) {
        ir.print("  %%a%d.addr = alloca %s, align %d\n", liveLocalIndex, getTypeName(a->getDataType()), a->getSize());
    ...
    for (TR::TreeTop *tree = firstTreeTop; tree != lastTreeTop; tree = tree->getNextTreeTop()) {
        ...
        result = self()->printNVVMIR(ir, tree->getNode(), loop, &targetBlocks, visitCount, &sharedMemory, nextParmNum,
            errorNode);
        if (result != GPUSuccess) {
            return result;
        }
...
void J9::CodeGenerator::generateGPU()
{
    if (self()->comp()->isGPUCompilation()) {
        char *programSource;
        TR::Node *errorNode;
...

        {
            ...
            result = self()->dumpNVVMIR(self()->comp()->getStartTree(), self()->comp()->findLastTree(), NULL, NULL,
                &method->getAutomaticList(), &method->getParameterList(),
...
uintptr_t J9::CodeGenerator::objectLengthOffset() { return self()->fe()->getOffsetOfContiguousArraySizeField(); }

uintptr_t J9::CodeGenerator::objectHeaderInvariant() { return self()->objectLengthOffset() + 4 /*length*/; }
