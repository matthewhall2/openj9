...
TR::Register *J9::ARM::TreeEvaluator::NULLCHKEvaluator(TR::Node *node, TR::CodeGenerator *cg)
{
    return TR::TreeEvaluator::evaluateNULLCHKWithPossibleResolve(node, false, cg);
}

TR::Register *J9::ARM::TreeEvaluator::resolveAndNULLCHKEvaluator(TR::Node *node, TR::CodeGenerator *cg)
{
    return TR::TreeEvaluator::evaluateNULLCHKWithPossibleResolve(node, true, cg);
}

TR::Register *J9::ARM::TreeEvaluator::evaluateNULLCHKWithPossibleResolve(TR::Node *node, bool needsResolve,
    TR::CodeGenerator *cg)
{
    // NOTE:
    ...
    if (needCode) {
        ...
        TR::Instruction *instr1 = generateImmSymInstruction(cg, TR::InstOpCode::bl, node,
            (uintptr_t)NULLCHKException->getMethodAddress(), NULL, NULLCHKException, NULL, NULL, ARMConditionCodeEQ);
    ...
    //
    if (needCode && opCode.isLoad() && firstChild->getReferenceCount() == 1 && firstChild->getSymbolReference()
    ...
    TR::Register *temp3Reg, TR::RegisterDependencyConditions *deps, TR::CodeGenerator *cg)
{
    ...
    if (!definitelyNonHeapObject) {
        ...
        generateMemSrc1Instruction(cg, TR::InstOpCode::strb, node,
            new (cg->trHeapMemory()) TR::MemoryReference(temp1Reg, temp3Reg, 1, cg), temp2Reg);

        generateLabelInstruction(cg, TR::InstOpCode::label, node, noChkLabel, deps);
    }
}

void J9::ARM::TreeEvaluator::generateTestAndReportFieldWatchInstructions(TR::CodeGenerator *cg, TR::Node *node,
    ...
    TR::Register *dataSnippetRegister)
{
    TR_ASSERT_FATAL(false,
        "This helper implements platform specific code for Fieldwatch, which is currently not supported on ARM "
        "platforms.\n");
}

void J9::ARM::TreeEvaluator::generateFillInDataBlockSequenceForUnresolvedField(TR::CodeGenerator *cg, TR::Node *node,
 ...
 *     Register for destination object
 */
void J9::ARM::TreeEvaluator::genWrtbarForArrayCopy(TR::Node *node, TR::Register *srcObjReg, TR::Register *dstObjReg,
    TR::CodeGenerator *cg)
{
    TR::Compilation *comp = cg->comp();
    ...
    if (ageCheckIsNeeded) {
        ...
        TR::Instruction *gcPoint = generateImmSymInstruction(cg, TR::InstOpCode::bl, node,
            (uintptr_t)wbRef->getSymbol()->castToMethodSymbol()->getMethodAddress(), NULL, wbRef);
