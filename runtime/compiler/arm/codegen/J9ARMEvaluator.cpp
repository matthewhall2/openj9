...
static inline TR::Instruction *genNullTest(TR::CodeGenerator *cg, TR::Node *node, TR::Register *objectReg,
    TR::Register *scratchReg, TR::Instruction *cursor)
{
...
static TR::Register *nonFixedDependency(TR::CodeGenerator *cg, TR::RegisterDependencyConditions *conditions,
    TR::Register *nonFixedReg, TR_RegisterKinds kind)
{
    if (nonFixedReg == NULL)
...
TR::Instruction *OMR::ARM::TreeEvaluator::generateVFTMaskInstruction(TR::CodeGenerator *cg, TR::Node *node,
    TR::Register *dstReg, TR::Register *srcReg, TR::Instruction *prev)
{
...
TR::Instruction *OMR::ARM::TreeEvaluator::generateVFTMaskInstruction(TR::CodeGenerator *cg, TR::Node *node,
    TR::Register *reg, TR::Instruction *prev)
{
    return generateVFTMaskInstruction(cg, node, reg, reg, prev);
}

static TR::Instruction *genTestIsSuper(TR::CodeGenerator *cg, TR::Node *node, TR::Register *objClassReg,
    TR::Register *castClassReg, TR::Register *scratch1Reg, TR::Register *scratch2Reg, int32_t castClassDepth,
    TR::LabelSymbol *failLabel, TR::Instruction *cursor, TR::LabelSymbol *doneLabel)
{
    uint32_t base, rotate;
    ...
    TR::MemoryReference *tempMR
        = new (cg->trHeapMemory()) TR::MemoryReference(objClassReg, offsetof(J9Class, classDepthAndFlags), cg);
    ...
    if (failLabel) {
        ...
        cursor = generateImmSymInstruction(cg, TR::InstOpCode::bl, node, (uintptr_t)symRef->getMethodAddress(), NULL,
            symRef, NULL, NULL, ARMConditionCodeLE);
...
TR::Register *OMR::ARM::TreeEvaluator::VMcheckcastEvaluator(TR::Node *node, TR::CodeGenerator *cg)
{
    ...
    if (!testEqualClass && !testCastClassIsSuper) {
        ...
        TR::Instruction *gcPoint = generateImmSymInstruction(cg, TR::InstOpCode::bl, node,
            (uint32_t)symRef->getMethodAddress(), deps, symRef);
    ...
    if (testEqualClass) {
        ...
        if (testCastClassIsSuper)
            ...
            gcPoint = generateImmSymInstruction(cg, TR::InstOpCode::bl, node, (uintptr_t)symRef->getMethodAddress(),
                NULL, symRef, NULL, NULL, ARMConditionCodeNE);
            gcPoint->ARMNeedsGCMap(0xFFFFFFFF);
        }
    }
    ...
    if (testCastClassIsSuper) {
        ...
        genTestIsSuper(cg, node, objClassReg, castClassReg, scratch1Reg, scratch2Reg, castClassDepth, NULL, NULL,
            doneLabel);
        gcPoint = generateImmSymInstruction(cg, TR::InstOpCode::bl, node, (uintptr_t)symRef->getMethodAddress(), NULL,
            symRef, NULL, NULL, ARMConditionCodeNE);
        gcPoint->ARMNeedsGCMap(0xFFFFFFFF);
    }

    if (isCheckCastOrNullCheck && implicitExceptionPointInstr && !objNode->isNonNull()) {
...
TR::Register *OMR::ARM::TreeEvaluator::VMifInstanceOfEvaluator(TR::Node *node, TR::CodeGenerator *cg)
{
    ...
    if (depNode != NULL) {
        if (!needsHelperCall
            && numberOfRegisterCandidate(depNode, TR_GPR) + 6 > cg->getMaximumNumberOfGPRsAllowedAcrossEdge(node))
        ...
        for (i = 0; i < depNode->getNumChildren(); i++) {
            ...
            if (needsHelperCall) {
                ...
                if (!properties.getPreserved((TR::RealRegister::RegNum)regIndex)
                    || (highIndex > -1 && !properties.getPreserved((TR::RealRegister::RegNum)highIndex)))
                    return (TR::Register *)1;
            } else {
    ...
    if (testEqualClass || testCastClassIsSuper) {
        TR::MemoryReference *tempMR = new (cg->trHeapMemory())
            TR::MemoryReference(objectReg, (int32_t)TR::Compiler->om.offsetOfObjectVftField(), cg);
    ...
    if (testCastClassIsSuper) {
        ...
        iCursor = genTestIsSuper(cg, node, objClassReg, castClassReg, scratch1Reg, scratch2Reg, castClassDepth,
            res0Label, iCursor, NULL);
        iCursor = generateConditionalBranchInstruction(cg, node, ARMConditionCodeNE, res0Label, iCursor);
        if (trueLabel == NULL) {
            if (needResult) {
                iCursor = generateTrg1ImmInstruction(cg, TR::InstOpCode::mov, node, resultReg, 1, 0, iCursor);
...
#if 0 // TODO:PPC has this fixup.  It maybe a fix to something so keeping here as note.  See J9PPCEvaluator.cpp for
      // reviveResultRegister() imple. We have to revive the resultReg here. Should revisit to see how to use registers
      // better (e.g., reducing the chance of moving around).
...
TR::Register *OMR::ARM::TreeEvaluator::VMmonexitEvaluator(TR::Node *node, TR::CodeGenerator *cg)
{
    ...
    if ((comp->getOption(TR_FullSpeedDebug) /*&& !comp->getOption(TR_EnableLiveMonitorMetadata)*/)
        || comp->getOption(TR_DisableInlineMonExit) || lwOffset <= 0) {
        TR::ILOpCodes opCode = node->getOpCodeValue();
    ...
    if (cg->comp()->target().isSMP() && cg->comp()->target().cpu.id() != TR_DefaultARMProcessor) {
        // instr = generateInstruction(cg, (cg->comp()->target().cpu.id() == TR_ARMv6) ? TR::InstOpCode::dmb_v6 :
        // TR::InstOpCode::dmb, node);
    ...
    TR::LabelSymbol *doneLabel = TR::LabelSymbol::create(cg->trHeapMemory(), cg);
    TR::Snippet *snippet
        = new (cg->trHeapMemory()) TR::ARMMonitorExitSnippet(cg, node, lwOffset, decLabel, callLabel, doneLabel);
...
static void genHeapAlloc(TR::CodeGenerator *cg, TR::Node *node, TR::Instruction *&iCursor, bool isVariableLen,
    TR::Register *enumReg, TR::Register *resReg, TR::Register *zeroReg, TR::Register *dataSizeReg,
    TR::Register *heapTopReg, TR::Register *sizeReg, TR::LabelSymbol *callLabel, int32_t allocSize)
{
    ...
    if (isVariableLen) {
        ...
        iCursor = generateTrg1Src1ImmInstruction(cg, TR::InstOpCode::and_r, node, zeroReg, enumReg, 0x000000FF, 24,
            iCursor);
    ...
    if (sizeInReg) {
        if (isVariableLen) {
            ...
            int32_t round
                = (elementSize >= OBJECT_ALIGNMENT) ? 0 : OBJECT_ALIGNMENT; // zero indicates no rounding is necessary
            bool headerAligned = allocSize % OBJECT_ALIGNMENT ? 0 : 1;

            if (elementSize >= 4) {
                TR_ARMOperand2 *operand2
                    = new (cg->trHeapMemory()) TR_ARMOperand2(ARMOp2RegLSLImmed, enumReg, trailingZeroes(elementSize));
                ...
                iCursor = generateTrg1Src1ImmInstruction(cg, TR::InstOpCode::add, node, dataSizeReg, enumReg, 3, 0,
                    iCursor);
                if (elementSize == 2) {
                    iCursor = generateTrg1Src2Instruction(cg, TR::InstOpCode::add, node, dataSizeReg, dataSizeReg,
                        enumReg, iCursor);
                }
                iCursor = generateTrg1Src1ImmInstruction(cg, TR::InstOpCode::bic, node, dataSizeReg, dataSizeReg, 3, 0,
                    iCursor);
            }
            if ((round != 0 && round != 4) || !headerAligned) {
                iCursor = generateTrg1Src1ImmInstruction(cg, TR::InstOpCode::add, node, sizeReg, dataSizeReg,
                    allocSize + round - 1, 0, iCursor);
                iCursor = generateTrg1Src1ImmInstruction(cg, TR::InstOpCode::bic, node, sizeReg, sizeReg, round - 1, 0,
                    iCursor);
            } else
                iCursor = generateTrg1Src1ImmInstruction(cg, TR::InstOpCode::add, node, sizeReg, dataSizeReg, allocSize,
                    0, iCursor);
        } else
    ...
    if (allocSize > cg->getMaxObjectSizeGuaranteedNotToOverflow()) {
        if (sizeInReg) {
            ...
            iCursor = generateTrg1Src1ImmInstruction(cg, TR::InstOpCode::add_r, node, sizeReg, resReg, base, rotate,
                iCursor);
        }
        ...
        if (sizeInReg) {
            ...
            iCursor
                = generateTrg1Src1ImmInstruction(cg, TR::InstOpCode::add, node, sizeReg, resReg, base, rotate, iCursor);
        }
    }
...
static void genInitObjectHeader(TR::CodeGenerator *cg, TR::Node *node, TR::Instruction *&iCursor, J9Class *clazz,
    TR::Register *classReg, TR::Register *resReg, TR::Register *zeroReg, TR::Register *temp1Reg, TR::Register *temp2Reg)
{
    TR_J9VMBase *fej9 = (TR_J9VMBase *)(cg->fe());
    ...
    if (classReg == NULL) {
        ...
        tempMR = new (cg->trHeapMemory())
            TR::MemoryReference(resReg, (int32_t)TR::Compiler->om.offsetOfObjectVftField(), cg);
    ...
    if (isStaticFlag != 0) {
        ...
        if (constantIsImmed8r(staticFlag, &val1, &val2)) {
            iCursor = generateTrg1Src1ImmInstruction(cg, TR::InstOpCode::orr, node, temp1Reg, temp1Reg, val1, val2,
                iCursor);
        } else {
...
static void genAlignDoubleArray(TR::CodeGenerator *cg, TR::Node *node, TR::Instruction *&iCursor, bool isVariableLen,
    TR::Register *resReg, int32_t objectSize, int32_t dataBegin, TR::Register *dataSizeReg, TR::Register *temp1Reg,
...
static void genInitArrayHeader(TR::CodeGenerator *cg, TR::Node *node, TR::Instruction *&iCursor, bool isVariableLen,
    J9Class *clazz, TR::Register *resReg, TR::Register *zeroReg, TR::Register *eNumReg, TR::Register *dataSizeReg,
    TR::Register *temp1Reg, TR::Register *temp2Reg)
{
    TR_J9VMBase *fej9 = (TR_J9VMBase *)(cg->fe());
    int32_t elementSize = TR::Compiler->om.getSizeOfArrayElement(node);
    ...
    TR::MemoryReference *tempMR = new (cg->trHeapMemory())
        TR::MemoryReference(resReg, (int32_t)(fej9->getOffsetOfContiguousArraySizeField()), cg);
...
TR::Register *OMR::ARM::TreeEvaluator::VMnewEvaluator(TR::Node *node, TR::CodeGenerator *cg)
{
    ...
    genHeapAlloc(cg, node, iCursor, isVariableLen, enumReg, resReg, zeroReg, dataSizeReg, temp1Reg, temp2Reg, callLabel,
        allocateSize);

    if (isArray) {
        // Align the array if necessary.
        if (isDoubleArray)
            genAlignDoubleArray(cg, node, iCursor, isVariableLen, resReg, objectSize, dataBegin, dataSizeReg, temp1Reg,
                temp2Reg);
        genInitArrayHeader(cg, node, iCursor, isVariableLen, clazz, resReg, zeroReg, enumReg, dataSizeReg, temp1Reg,
            temp2Reg);
    } else {
    ...
    if (!node->canSkipZeroInitialization() && (initInfo == NULL || initInfo->numZeroInitSlots > 0)) {
        if (!isVariableLen) {
            if (initInfo != NULL && initInfo->zeroInitSlots != NULL && initInfo->numZeroInitSlots <= 9
                && objectSize <= UPPER_IMMED12) {
                TR_BitVectorIterator bvi(*initInfo->zeroInitSlots);

                while (bvi.hasMoreElements()) {
                    tempMR = new (cg->trHeapMemory())
                        TR::MemoryReference(resReg, bvi.getNextElement() * 4 + dataBegin, cg);
                ...
                iCursor = generateTrg1Src1ImmInstruction(cg, TR::InstOpCode::add, node, temp2Reg, resReg, dataBegin, 0,
                    iCursor);
                ...
                iCursor
                    = generateTrg1Src1ImmInstruction(cg, TR::InstOpCode::add, node, temp2Reg, temp2Reg, 16, 0, iCursor);

                iCursor = generateTrg1Src1ImmInstruction(cg, TR::InstOpCode::sub_r, node, temp1Reg, temp1Reg, 1, 0,
                    iCursor);
        ...
        } else // Init variable length array
        {
            ...
            iCursor = generateTrg1Src1Instruction(cg, TR::InstOpCode::mov_r, node, temp1Reg,
                new (cg->trHeapMemory()) TR_ARMOperand2(ARMOp2RegLSRImmed, dataSizeReg, 2), iCursor);
            iCursor = generateConditionalBranchInstruction(cg, node, ARMConditionCodeEQ, doneLabel, iCursor);
            iCursor = generateTrg1Src1ImmInstruction(cg, TR::InstOpCode::add, node, temp2Reg, resReg, dataBegin - 4, 0,
                iCursor);
            ...
            iCursor = generateMemSrc1Instruction(cg, TR::InstOpCode::str, node, tempMR, zeroReg, iCursor);
            iCursor
                = generateTrg1Src1ImmInstruction(cg, TR::InstOpCode::sub_r, node, temp1Reg, temp1Reg, 1, 0, iCursor);
            iCursor = generateConditionalBranchInstruction(cg, node, ARMConditionCodeNE, initLoop, iCursor);
        }
    }
    ...
    iCursor = generateImmSymInstruction(cg, TR::InstOpCode::bl, node,
        (uint32_t)node->getSymbolReference()->getMethodAddress(), deps, node->getSymbolReference());
...
TR::Register *OMR::ARM::TreeEvaluator::VMmonentEvaluator(TR::Node *node, TR::CodeGenerator *cg)
{
    TR::Compilation *comp = TR::comp();
    TR_J9VMBase *fej9 = (TR_J9VMBase *)(cg->fe());
    int32_t lwOffset = fej9->getByteOffsetToLockword(cg->getMonClass(node));
    ...
    if ((comp->getOption(TR_FullSpeedDebug) /*&& !comp->getOption(TR_EnableLiveMonitorMetadata)*/)
        || comp->getOption(TR_DisableInlineMonEnt) || lwOffset <= 0) {
        TR::ILOpCodes opCode = node->getOpCodeValue();
        TR::Node::recreate(node, TR::call);
        TR::Register *targetRegister = directCallEvaluator(node, cg);
        TR::Node::recreate(node, opCode);
    ...
    generateTrg1MemInstruction(cg, TR::InstOpCode::ldrex, node, dataReg,
        new (cg->trHeapMemory()) TR::MemoryReference(addrReg, (int32_t)0, cg));
    ...
    generateTrg1MemSrc1Instruction(cg, TR::InstOpCode::strex, node, tempReg,
        new (cg->trHeapMemory()) TR::MemoryReference(addrReg, (int32_t)0, cg), metaReg);
    ...
    if (cg->comp()->target().isSMP() && cg->comp()->target().cpu.id() != TR_DefaultARMProcessor) {
        generateInstruction(cg,
            (cg->comp()->target().cpu.id() == TR_ARMv6) ? TR::InstOpCode::dmb_v6 : TR::InstOpCode::dmb, node);
    }

    generateLabelInstruction(cg, TR::InstOpCode::label, node, doneLabel, deps);

    TR::Snippet *snippet
        = new (cg->trHeapMemory()) TR::ARMMonitorEnterSnippet(cg, node, lwOffset, incLabel, callLabel, doneLabel);
    cg->addSnippet(snippet);
    doneLabel->setEndInternalControlFlow();
...
void OMR::Power::TreeEvaluator::VMarrayStoreCHKEvaluator(TR::Node *node, TR::Register *src, TR::Register *dst,
TR::Register *t1Reg, TR::Register *t2Reg, TR::Register *cndReg, TR::LabelSymbol *toWB, TR_Instruction *iPtr)
   {
   ...
   iPtr = generateTrg1MemInstruction(TR::InstOpCode::lwz, node, t1Reg,
             new (cg->trHeapMemory()) TR::MemoryReference(dst, (int32_t)TR::Compiler->om.offsetOfObjectVftField(), 4),
iPtr); iPtr = generateTrg1MemInstruction(TR::InstOpCode::lwz, node, t2Reg, new (cg->trHeapMemory())
TR::MemoryReference(src, (int32_t)TR::Compiler->om.offsetOfObjectVftField(), 4), iPtr); iPtr =
generateTrg1MemInstruction(TR::InstOpCode::lwz, node, t1Reg, new (cg->trHeapMemory()) TR::MemoryReference(t1Reg,
(int32_t)offsetof(J9ArrayClass, componentType), 4), iPtr); iPtr = generateTrg1Src2Instruction(TR::InstOpCode::cmpl4,
node, cndReg, t1Reg, t2Reg, iPtr); iPtr = generateConditionalBranchInstruction(TR::InstOpCode::beq, node, toWB, cndReg,
iPtr); if (upper == 0)
      {
...
void OMR::ARM::TreeEvaluator::VMwrtbarEvaluator(TR::Node *node, TR::Register *srcReg, TR::Register *dstReg,
    TR::Register *flagReg, bool needDeps, TR::CodeGenerator *cg)
{
    TR::Compilation *comp = TR::comp();
    TR::LabelSymbol *doneLabel = TR::LabelSymbol::create(cg->trHeapMemory(), cg);
    ...
    if (gcMode != gc_modron_wrtbar_always) {
        ...
        // @@      if (flagReg == NULL)
        // @@         generateTrg1MemInstruction(cg, TR::InstOpCode::ldr, node, tempReg, new (cg->trHeapMemory())
        // TR::MemoryReference(dstReg, fej9->getWordOffsetToGCFlags(), cg));
        // @@      generateTrg1Src2Instruction(cg, TR::InstOpCode::and_r, node, tempReg, tempReg, new
        // (cg->trHeapMemory()) TR_ARMOperand2(fej9->getWriteBarrierGCFlagMaskAsByte(), 8));
    }
