...
static TR::RealRegister::RegNum _singleArgumentRegisters[] = { TR::RealRegister::fs0, TR::RealRegister::fs1,
    TR::RealRegister::fs2, TR::RealRegister::fs3, TR::RealRegister::fs4, TR::RealRegister::fs5, TR::RealRegister::fs6,
    TR::RealRegister::fs7, TR::RealRegister::fs8, TR::RealRegister::fs9, TR::RealRegister::fs10, TR::RealRegister::fs11,
    TR::RealRegister::fs12, TR::RealRegister::fs13, TR::RealRegister::fs14, TR::RealRegister::fs15 };
...
int32_t J9::ARM::JNILinkage::buildArgs(TR::Node *callNode, TR::RegisterDependencyConditions *dependencies,
    TR::Register *&vftReg, bool isVirtual)
{
...
void J9::ARM::JNILinkage::buildVirtualDispatch(TR::Node *callNode, TR::RegisterDependencyConditions *dependencies,
...
TR::ARMLinkageProperties &J9::ARM::JNILinkage::getProperties() { return _properties; }
...
TR::Register *J9::ARM::JNILinkage::pushDoubleArgForJNI(TR::Node *child)
{
    ...
    if (pushRegister->getKind() == TR_GPR) {
        ...
        generateTrg1Src2Instruction(cg(), TR::InstOpCode::fmdrr, child, trgReg, pair->getLowOrder(),
            pair->getHighOrder());
        return trgReg;
    }
    return pushRegister;
...
TR::MemoryReference *J9::ARM::JNILinkage::getOutgoingArgumentMemRef(int32_t totalSize, int32_t offset,
...
int32_t J9::ARM::JNILinkage::buildJNIArgs(TR::Node *callNode, TR::RegisterDependencyConditions *dependencies,
    TR::Register *&vftReg, bool passReceiver, bool passEnvArg)
{
        ...
        switch (callNode->getChild(i)->getDataType()) {
            ...
            case TR::Double:
                ...
                if (isEABI) {
                    ...
                    if (numIntegerArgRegIndex + 1 >= numIntArgRegs) {
                        /* if numIntegerArgRegIndex >= numIntArgReg - 1, we do not need to care about the register slot
                         * padding. */
            ...
            case TR::Float:
                if ((numFloatArgRegIndex < numFloatArgRegs)
                    || (numFloatArgRegIndex == numFloatArgRegs && numSingleArgRegIndex != 0)) {
            ...
            case TR::Float:
                ...
                if (numIntegerArgRegIndex < numIntArgRegs) {
                    if ((childType != TR::Address) && !cg()->canClobberNodesRegister(child, 0)) {
                        /* If the reg is shared by another node, we copy it to tempReg, so that the reg is not reused
                         * after returning the call. */
                    ...
                    if (numIntegerArgRegIndex == 0 && (resType.isAddress() || resType.isInt32() || resType.isInt64())) {
                        ...
                        TR::addDependency(dependencies, reg,
                            jniLinkageProperties.getIntegerArgumentRegister(numIntegerArgRegIndex), TR_GPR, codeGen);
                    }
                } else {
#ifdef DEBUG_ARM_LINKAGE
...
#endif
                    tempMR
                        = getOutgoingArgumentMemRef(0, stackOffset, reg, TR::InstOpCode::str, pushToMemory[memArg++]);
                    stackOffset += 4;
                }
                numIntegerArgRegIndex++;
                break;
            ...
            case TR::Double:
                ...
                if (numIntegerArgRegIndex < numIntArgRegs) {
                    if (!cg()->canClobberNodesRegister(child, 0)) {
                        ...
                        if (bigEndian) {
                            generateTrg1Src1Instruction(codeGen, TR::InstOpCode::mov, child, tempReg,
                    ...
                    dependencies->addPreCondition(bigEndian ? reg->getRegisterPair()->getHighOrder()
                                                            : reg->getRegisterPair()->getLowOrder(),
                        jniLinkageProperties.getIntegerArgumentRegister(numIntegerArgRegIndex));
                    if ((numIntegerArgRegIndex == 0) && resType.isAddress())
                        dependencies->addPostCondition(codeGen->allocateCollectedReferenceRegister(),
                            TR::RealRegister::gr0);
                    else
                        dependencies->addPostCondition(codeGen->allocateRegister(),
                            jniLinkageProperties.getIntegerArgumentRegister(numIntegerArgRegIndex));
                    ...
                    if (numIntegerArgRegIndex + 1 < numIntArgRegs) {
                        ...
                        dependencies->addPreCondition(bigEndian ? reg->getRegisterPair()->getLowOrder()
                                                                : reg->getRegisterPair()->getHighOrder(),
                            jniLinkageProperties.getIntegerArgumentRegister(numIntegerArgRegIndex + 1));
                        dependencies->addPostCondition(codeGen->allocateRegister(),
                            jniLinkageProperties.getIntegerArgumentRegister(numIntegerArgRegIndex + 1));
                    } else {
#ifdef DEBUG_ARM_LINKAGE
                        ...
                        fflush(stdout);
#endif
                        tempMR = getOutgoingArgumentMemRef(0, stackOffset,
                    ...
                    tempMR = getOutgoingArgumentMemRef(0, stackOffset,
                        ...
                        TR::InstOpCode::str, pushToMemory[memArg++]);
                    tempMR = getOutgoingArgumentMemRef(0, stackOffset + 4,
                        bigEndian ? reg->getRegisterPair()->getLowOrder() : reg->getRegisterPair()->getHighOrder(),
                        TR::InstOpCode::str, pushToMemory[memArg++]);
                    stackOffset += 8;
                }
            ...
            case TR::Float:
                ...
                if ((numFloatArgRegIndex < numFloatArgRegs)
                    ...
                    if ((numFloatArgRegIndex == 0) && resType.isFloatingPoint()) {
                        ...
                        if (numSingleArgRegIndex == 0) {
                            dependencies->addPostCondition(reg,
                                jniLinkageProperties.getFloatArgumentRegister(numFloatArgRegIndex));
                        }
                    }
                    ...
                    tempMR
                        = getOutgoingArgumentMemRef(0, stackOffset, reg, TR::InstOpCode::fsts, pushToMemory[memArg++]);
                    stackOffset += 4;
            ...
            case TR::Double:
                ...
                if (numFloatArgRegIndex < numFloatArgRegs) {
                    ...
                    if ((numFloatArgRegIndex == 0) && resType.isFloatingPoint()) {
                        ...
                        TR::addDependency(dependencies, reg,
                            jniLinkageProperties.getFloatArgumentRegister(numFloatArgRegIndex), TR_FPR, codeGen);
                } else {
                    if (isEABI)
                        stackOffset = (stackOffset + 4) & (~7);
                    tempMR
                        = getOutgoingArgumentMemRef(0, stackOffset, reg, TR::InstOpCode::fstd, pushToMemory[memArg++]);
                    stackOffset += 8;
                }
                numFloatArgRegIndex++;
                break;
    ...
    for (i = 0; i < jniLinkageProperties.getNumFloatArgRegs(); i++) {
        ...
        if (!dependencies->searchPreConditionRegister(realReg)) {
            ...
            if (!dependencies->searchPreConditionRegister(singleReg1)) {
                TR_ASSERT(!dependencies->searchPreConditionRegister(singleReg2),
                    "Wrong dependency for single precision register.\n");
                if (realReg == jniLinkageProperties.getFloatArgumentRegister(0)
                    && (resType.getDataType() == TR::Float)) {
    ...
    for (i = 0; i < numMemArgs; i++) {
        ...
        generateMemSrc1Instruction(codeGen, pushToMemory[i].opCode, callNode, pushToMemory[i].argMemory,
...
TR::Register *J9::ARM::JNILinkage::buildDirectDispatch(TR::Node *callNode)
{
    ...
    TR::RegisterDependencyConditions *deps = new (trHeapMemory()) TR::RegisterDependencyConditions(
        jniLinkageProperties.getNumberOfDependencyGPRegisters() + jniLinkageProperties.getNumFloatArgRegs() * 2,
        jniLinkageProperties.getNumberOfDependencyGPRegisters() + jniLinkageProperties.getNumFloatArgRegs() * 2,
        trMemory());
    ...
    TR::RegisterDependencyConditions *spillDeps = new (trHeapMemory())
        TR::RegisterDependencyConditions(0, jniLinkageProperties.getNumFloatArgRegs() * 2, codeGen->trMemory());
    for (i = 0; i < jniLinkageProperties.getNumFloatArgRegs() * 2; i++) {
        spillDeps->addPostCondition(codeGen->allocateRegister(TR_FPR),
            (TR::RealRegister::RegNum)((uint32_t)TR::RealRegister::fs0 + i));
    }
    ...
    generateTrg1Src1Instruction(codeGen, TR::InstOpCode::mov, callNode, gr4Reg,
        new (trHeapMemory()) TR_ARMOperand2(flags.getByte3(), 24));
    generateTrg1Src2Instruction(codeGen, TR::InstOpCode::add, callNode, gr4Reg, gr4Reg,
        new (trHeapMemory()) TR_ARMOperand2(flags.getByte2(), 16));
    tempMR = new (trHeapMemory())
        TR::MemoryReference(stackPtr, -((int)TR::Compiler->om.sizeofReferenceAddress()), codeGen);
    tempMR->setImmediatePreIndexed();
    generateMemSrc1Instruction(codeGen, TR::InstOpCode::str, callNode, tempMR, gr4Reg);
    ...
    generateTrg1Src1Instruction(codeGen, TR::InstOpCode::mov, callNode, gr4Reg,
        new (trHeapMemory()) TR_ARMOperand2(ramMethod.getByte3(), 24));
    generateTrg1Src2Instruction(codeGen, TR::InstOpCode::add, callNode, gr4Reg, gr4Reg,
        new (trHeapMemory()) TR_ARMOperand2(ramMethod.getByte2(), 16));
    generateTrg1Src2Instruction(codeGen, TR::InstOpCode::add, callNode, gr4Reg, gr4Reg,
        new (trHeapMemory()) TR_ARMOperand2(ramMethod.getByte1(), 8));
    generateTrg1Src2Instruction(codeGen, TR::InstOpCode::add, callNode, gr4Reg, gr4Reg,
        new (trHeapMemory()) TR_ARMOperand2(ramMethod.getByte0(), 0));
    tempMR = new (trHeapMemory())
        TR::MemoryReference(stackPtr, -((int)TR::Compiler->om.sizeofReferenceAddress()), codeGen);
    tempMR->setImmediatePreIndexed();
    generateMemSrc1Instruction(codeGen, TR::InstOpCode::str, callNode, tempMR, gr4Reg);
    ...
    generateTrg1Src1Instruction(codeGen, TR::InstOpCode::mov, callNode, gr4Reg,
        new (trHeapMemory()) TR_ARMOperand2(frameType.getByte1(), 8));
    generateTrg1Src2Instruction(codeGen, TR::InstOpCode::add, callNode, gr4Reg, gr4Reg,
        new (trHeapMemory()) TR_ARMOperand2(frameType.getByte0(), 0));
    ...
    instr
        = new (trHeapMemory()) TR::ARMMultipleMoveInstruction(TR::InstOpCode::stmdb, callNode, gr13Reg, 0x0f, codeGen);
    instr->setWriteBack();

    // AOT relocation is handled in TR::ARMImmSymInstruction::generateBinaryEncoding()
    TR::Instruction *gcPoint = generateImmSymInstruction(codeGen, TR::InstOpCode::bl, callNode,
        (uint32_t)helperSymRef->getMethodAddress(), NULL, helperSymRef);
    gcPoint->ARMNeedsGCMap(~(jniLinkageProperties.getPreservedRegisterMapForGC()));
    instr
        = new (trHeapMemory()) TR::ARMMultipleMoveInstruction(TR::InstOpCode::ldmia, callNode, gr13Reg, 0x0f, codeGen);
    instr->setWriteBack();
    ...
    codeGen->getJNICallSites().push_front(
        new (trHeapMemory()) TR_Pair<TR_ResolvedMethod, TR::Instruction>(calleeSym->getResolvedMethod(), gcPoint));
    gcPoint->ARMNeedsGCMap(jniLinkageProperties.getPreservedRegisterMapForGC());
    ...
    switch (resolvedMethod->returnType()) {
        case TR::Int8:
            if (resolvedMethod->returnTypeIsUnsigned())
                generateTrg1Src1ImmInstruction(codeGen, TR::InstOpCode::and_, callNode, returnRegister, returnRegister,
                    0xFF, 0);
            else {
    ...
    if (codeGen->hasHardFloatReturn()) {
        switch (resolvedMethod->returnType()) {
            case TR::Float: {
                ...
                tempMR
                    = new (trHeapMemory()) TR::MemoryReference(metaReg, fej9->thisThreadGetFloatTemp1Offset(), codeGen);
            ...
            case TR::Double: {
                ...
                TR_ASSERT(fej9->thisThreadGetFloatTemp2Offset() - fej9->thisThreadGetFloatTemp1Offset() == 4,
                    "floatTemp1 and floatTemp2 not contiguous");
                tempMR
                    = new (trHeapMemory()) TR::MemoryReference(metaReg, fej9->thisThreadGetFloatTemp1Offset(), codeGen);
                ...
                generateTrg1MemInstruction(codeGen, TR::InstOpCode::ldr, callNode,
                    bigEndian ? returnRegister->getHighOrder() : returnRegister->getLowOrder(), tempMR);
                tempMR
                    = new (trHeapMemory()) TR::MemoryReference(metaReg, fej9->thisThreadGetFloatTemp2Offset(), codeGen);
                generateTrg1MemInstruction(codeGen, TR::InstOpCode::ldr, callNode,
                    bigEndian ? returnRegister->getLowOrder() : returnRegister->getHighOrder(), tempMR);
                break;
            }
        }
    }
    ...
    // AOT relocation is handled in TR::ARMImmSymInstruction::generateBinaryEncoding()
    gcPoint = generateImmSymInstruction(codeGen, TR::InstOpCode::bl, callNode,
        (uint32_t)helperSymRef->getMethodAddress(), NULL, helperSymRef);
    ...
    // AOT relocation is handled in TR::ARMImmSymInstruction::generateBinaryEncoding()
    generateImmSymInstruction(codeGen, TR::InstOpCode::bl, callNode, (uint32_t)helperSymRef->getMethodAddress(), NULL,
        helperSymRef, NULL, NULL, ARMConditionCodeEQ);
    ...
    // AOT relocation is handled in TR::ARMImmSymInstruction::generateBinaryEncoding()
    gcPoint = generateImmSymInstruction(codeGen, TR::InstOpCode::bl, callNode,
        (uint32_t)helperSymRef->getMethodAddress(), NULL, helperSymRef, NULL, NULL, ARMConditionCodeNE);
    gcPoint->ARMNeedsGCMap(1 << (jniLinkageProperties.getIntegerReturnRegister() - TR::RealRegister::FirstGPR));
