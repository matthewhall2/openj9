        ...
        switch (child->getDataType()) {
            ...
            case TR::Float:
                ...
                if (intArgNum < linkageProperties.getNumIntArgRegs()) {
                    buffer = storeArgumentItem(TR::InstOpCode::str, buffer,
...
static int32_t instructionCountForArguments(TR::Node *callNode, TR::CodeGenerator *cg)
{
    ...
    const TR::ARMLinkageProperties &linkage
...
TR_RuntimeHelper TR::ARMCallSnippet::getHelper()
{
    ...
    *(int32_t *)cursor = (intptr_t)getCallRA();
    ...
    if (methodSymRef->isUnresolved() || comp->compileRelocatableCode()) {
        ...
        *(int32_t *)cursor = (uintptr_t)methodSymbol->getMethodAddress();
        ...
        cg()->addExternalRelocation(TR::ExternalRelocation::create(cursor, (uint8_t *)methodSymRef,
                                        getNode() ? (uint8_t *)getNode()->getInlinedSiteIndex() : (uint8_t *)-1,
                                        TR_MethodObject, cg()),
            __FILE__, __LINE__, callNode);
        /*
        TR_RelocationRecordInformation *recordInfo = ( TR_RelocationRecordInformation
        *)comp->trMemory()->allocateMemory(sizeof( TR_RelocationRecordInformation), heapAlloc); recordInfo->data1 =
        (uintptr_t)methodSymRef; recordInfo->data2 = (uintptr_t)(getNode() ? (uint8_t *)getNode()->getInlinedSiteIndex()
        : (uint8_t *)-1); recordInfo->data3 = (uintptr_t)fixedSequence1;
...
uint8_t *TR::ARMVirtualUnresolvedSnippet::emitSnippetBody()
{
    ...
    *(int32_t *)cursor = (intptr_t)methodSymRef->getOwningMethod(comp)->constantPool();
    ...
    cg()->addExternalRelocation(TR::ExternalRelocation::create(cursor, *(uint8_t **)cursor,
                                    getNode() ? (uint8_t *)getNode()->getInlinedSiteIndex() : (uint8_t *)-1, TR_Thunks,
                                    cg()),
        __FILE__, __LINE__, getNode());
...
uint32_t TR::ARMVirtualUnresolvedSnippet::getLength(int32_t estimatedSnippetStart) { return 16; }
...
uint32_t TR::ARMInterfaceCallSnippet::getLength(int32_t estimatedSnippetStart) { return 24; }

uint8_t *TR::ARMCallSnippet::generateVIThunk(TR::Node *callNode, int32_t argSize, TR::CodeGenerator *cg)
{
    TR_J9VMBase *fej9 = (TR_J9VMBase *)(cg->fe());
    int32_t codeSize
        = 4 * (instructionCountForArguments(callNode, cg) + 2) + 8; // Additional 4 bytes to hold size of thunk
...
TR_MHJ2IThunk *TR::ARMCallSnippet::generateInvokeExactJ2IThunk(TR::Node *callNode, int32_t argSize,
