...
void TR::ARMMonitorEnterSnippet::print(OMR::Logger *log, TR_Debug *debug)
{
    ...
    //    mvn     tempReg, (OBJECT_HEADER_LOCK_BITS_MASK - OBJECT_HEADER_LOCK_LAST_RECURSION_BIT) -> 0x7f
    log->printf("mvn \t%s, #0x%08x; (OBJECT_HEADER_LOCK_BITS_MASK - OBJECT_HEADER_LOCK_LAST_RECURSION_BIT)",
    ...
    debug->printPrefix(log, NULL, bufferPos, 4);
    //    addeq   dataReg, dataReg, LOCK_INC_DEC_VALUE
    log->printf("addeq \t%s, %s, #0x%08x; Increment the count", debug->getName(dataReg), debug->getName(dataReg),
        LOCK_INC_DEC_VALUE);
    bufferPos += 4;
    ...
    debug->printPrefix(log, NULL, bufferPos, 4);
    //    beq     restartLabel
    log->printf("beq\t" POINTER_PRINTF_FORMAT "\t\t; Return to %s", (intptr_t)(restartLabel->getCodeLocation()),
        debug->getName(restartLabel));
...
void TR::ARMMonitorExitSnippet::print(OMR::Logger *log, TR_Debug *debug)
{
    ...
    debug->printPrefix(log, NULL, bufferPos, 4);
    //    mvn     threadReg, OBJECT_HEADER_LOCK_RECURSION_MASK  // mask out count field
    log->printf("mvn\t%s, #0x%08x; OBJECT_HEADER_LOCK_RECURSION_MASK", debug->getName(threadReg),
    ...
    debug->printPrefix(log, NULL, bufferPos, 4);
    //    subeq   monitorReg, monitorReg, LOCK_INC_DEC_VALUE
    log->printf("subeq\t%s, %s, #0x%08x; Decrement the count", debug->getName(monitorReg), debug->getName(monitorReg),
        LOCK_INC_DEC_VALUE);
    bufferPos += 4;
