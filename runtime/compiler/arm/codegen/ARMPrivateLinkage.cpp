...
TR::ARMLinkageProperties J9::ARM::PrivateLinkage::properties = {
    ...
    { // preserved registers
        //       TR::RealRegister::gr4,
        //       TR::RealRegister::gr5,
        TR::RealRegister::gr6, TR::RealRegister::gr7, TR::RealRegister::gr8, TR::RealRegister::gr9,
...
#if !defined(__VFP_FP__) || defined(__SOFTFP__)
                            TR::RealRegister::fp4, TR::RealRegister::fp5, TR::RealRegister::fp6, TR::RealRegister::fp7
#endif
    },
    {
                            // argument registers
...
TR::ARMLinkageProperties &J9::ARM::PrivateLinkage::getProperties() { return properties; }
...
uint32_t J9::ARM::PrivateLinkage::getRightToLeft() { return getProperties().getRightToLeft(); }
    ...
    if (linkage.getRightToLeft()) {
        ...
        while (parmCursor != NULL) {
            parmCursor->setParameterOffset(
                sizeOfParameterArea - parmCursor->getParameterOffset() - parmCursor->getSize() + offsetToFirstParm);
            parmCursor = parameterIterator.getNext();
        }
    }
...
uint16_t getAssignedRegisterCount(TR::Machine *machine, const TR::ARMLinkageProperties *linkage)
{
    ...
    for (int i = TR::RealRegister::LastGPR; i; i--) {
        if (linkage->getPreserved((TR::RealRegister::RegNum)i)) {
            assignedRegisters
                += machine->getRealRegister((TR::RealRegister::RegNum)i)->getHasBeenAssignedInMethod() ? 1 : 0;
        }
    }
...
void J9::ARM::PrivateLinkage::createPrologue(TR::Instruction *cursor)
{
    ...
    // overflow.
    //
    if (!codeGen->getSnippetList().empty() || bodySymbol->isEHAware() || machine->getLinkRegisterKilled()) {
    ...
    if (constantIsImmed8r(totalFrameSize, &base, &rotate)) {
        cursor = generateTrg1Src1ImmInstruction(codeGen, TR::InstOpCode::sub, firstNode, stackPtr, stackPtr, base,
            rotate, cursor);
    } else {
    ...
    if (!comp()->isDLT()) {
        ...
        TR::Snippet *snippet = new (trHeapMemory())
            TR::ARMStackCheckFailureSnippet(codeGen, cursor->getNode(), restartLabel, snippetLabel);
    ...
    // outgoingArgSize <= 1020 because the JVM spec limits the max number of method param to 255, so constantIsImmed8r
    // can handle outgoingArgSize.
    if (intRegistersSaved) {
        if (1 == intRegistersSaved) {
            ...
            cursor = generateTrg1Src1ImmInstruction(codeGen, TR::InstOpCode::add, firstNode, gr4, stackPtr, base,
                rotate, cursor);
            // FIXME Need generateMultiMoveInstruction().
            cursor = new (trHeapMemory()) TR::ARMMultipleMoveInstruction(cursor, TR::InstOpCode::stm, firstNode, gr4,
                getAssignedRegisterList(machine, &linkage), codeGen);
        }
    }

    // for saving preserved VFP registers, the offset can be larger than 1024, so constantIsImmed10 and
    // constantIsImmed8r might not be able to handle the offset.
    ...
    if (fpRegistersSaved > 0) {
        if (1 == fpRegistersSaved) {
            ...
            if (constantIsImmed10(outgoingArgSize)) {
                ...
                cursor
                    = generateTrg1Src2Instruction(codeGen, TR::InstOpCode::add, firstNode, gr4, stackPtr, gr4, cursor);
            ...
            cursor = new (trHeapMemory())
                TR::ARMMultipleMoveInstruction(cursor, TR::InstOpCode::fstmd, firstNode, gr4, offset, codeGen);
        ...
        if (numLocalsToBeInitialized > 0) {
            ...
            if (numLocalsToBeInitialized > 0) {
                ...
                if (numLocalsToBeInitialized > 8) {
                    ...
                    cursor = generateTrg1Src2Instruction(codeGen, TR::InstOpCode::add, firstNode, gr4, gr4, stackPtr,
                        cursor);
                    ...
                    cursor = generateTrg1Src1ImmInstruction(codeGen, TR::InstOpCode::sub_r, firstNode, gr5, gr5, 4, 0,
                        cursor);
                    cursor = generateConditionalBranchInstruction(codeGen, firstNode, ARMConditionCodeGE, loopLabel,
                        cursor);
                } else {
                    for (uint32_t i = 0; i < numLocalsToBeInitialized; i++, offset += 4) {
                        tempMR = new (trHeapMemory()) TR::MemoryReference(stackPtr, offset, codeGen);
                        cursor
                            = generateMemSrc1Instruction(codeGen, TR::InstOpCode::str, firstNode, tempMR, gr11, cursor);
                    }
                }
            }
        }
...
void J9::ARM::PrivateLinkage::createEpilogue(TR::Instruction *cursor)
{
    ...
    if (intRegistersSaved) {
        if (1 == intRegistersSaved) {
            ...
            TR::RealRegister *gr4 = machine->getRealRegister(TR::RealRegister::gr4);
            constantIsImmed8r(outgoingArgSize, &base, &rotate);
            cursor = generateTrg1Src1ImmInstruction(codeGen, TR::InstOpCode::add, lastNode, gr4, stackPtr, base, rotate,
                cursor);
            cursor = new (trHeapMemory()) TR::ARMMultipleMoveInstruction(cursor, TR::InstOpCode::ldm, lastNode, gr4,
                getAssignedRegisterList(machine, &linkage), codeGen);
        }
    }
    ...
    if (fpRegistersSaved > 0) {
        ...
        if (1 == fpRegistersSaved) {
            ...
            if (constantIsImmed10(outgoingArgSize)) {
                ...
                cursor
                    = generateTrg1Src2Instruction(codeGen, TR::InstOpCode::add, lastNode, gr4, stackPtr, gr4, cursor);
                tempMR = new (trHeapMemory()) TR::MemoryReference(gr4, 0, codeGen);
            }
            cursor = generateMemSrc1Instruction(codeGen, TR::InstOpCode::fldd, lastNode, tempMR, reg, cursor);
        } else {
            if (constantIsImmed8r(outgoingArgSize, &base, &rotate)) {
                cursor = generateTrg1Src1ImmInstruction(codeGen, TR::InstOpCode::add, lastNode, gr4, stackPtr, base,
                    rotate, cursor);
            } else {
                cursor = armLoadConstant(lastNode, outgoingArgSize, gr4, codeGen, cursor);
                cursor
            ...
            uint16_t offset = ((TR::RealRegister::fp8 - TR::RealRegister::FirstFPR) << 12) | (fpRegistersSaved << 1);
            cursor = new (trHeapMemory())
                TR::ARMMultipleMoveInstruction(cursor, TR::InstOpCode::fldmd, lastNode, gr4, offset, codeGen);
            ((TR::ARMMultipleMoveInstruction *)cursor)->setIncrement();
        }
    }
    ...
    if (codeGen->getSnippetList().size() > 1 || (comp()->isDLT() && !codeGen->getSnippetList().empty())
        || bodySymbol->isEHAware() || machine->getLinkRegisterKilled()) {
        tempMR = new (trHeapMemory())
            TR::MemoryReference(stackPtr, totalFrameSize + properties.getOffsetToFirstLocal(), codeGen);
        cursor = generateTrg1MemInstruction(codeGen, TR::InstOpCode::ldr, lastNode, gr14, tempMR, cursor);
    }

    // Collapse stack frame and return.
    if (constantIsImmed8r(totalFrameSize, &base, &rotate)) {
        cursor = generateTrg1Src1ImmInstruction(codeGen, TR::InstOpCode::add, lastNode, stackPtr, stackPtr, base,
            rotate, cursor);
    } else {
        TR::RealRegister *gr4 = machine->getRealRegister(TR::RealRegister::gr4);
...
TR::MemoryReference *J9::ARM::PrivateLinkage::getOutgoingArgumentMemRef(int32_t totalSize, int32_t offset,
...
int32_t J9::ARM::PrivateLinkage::buildArgs(TR::Node *callNode, TR::RegisterDependencyConditions *dependencies,
    TR::Register *&vftReg, bool isVirtual)
{
    return buildARMLinkageArgs(callNode, dependencies, vftReg, TR_Private, isVirtual);
}

void J9::ARM::PrivateLinkage::buildVirtualDispatch(TR::Node *callNode, TR::RegisterDependencyConditions *dependencies,
    TR::RegisterDependencyConditions *postDeps, TR::Register *vftReg, uint32_t sizeOfArguments)
{
            ...
            case TR::java_lang_invoke_ComputedCalls_dispatchVirtual: {
                ...
                if (!thunk) {
                    thunk = fej9->setJ2IThunk(j2iSignature, signatureLen,
                        TR::ARMCallSnippet::generateVIThunk(
            ...
            default:
                if (fej9->needsInvokeExactJ2IThunk(callNode, comp())) {
                    comp()->getPersistentInfo()->getInvokeExactJ2IThunkTable()->addThunk(
                        TR::ARMCallSnippet::generateInvokeExactJ2IThunk(callNode, sizeOfArguments, codeGen,
    ...
    uint8_t *thunk = (uint8_t *)fej9->getJ2IThunk(methodSymbol->getMethod()->signatureChars(),
        methodSymbol->getMethod()->signatureLength(), comp());
    if (!thunk)
        thunk = (uint8_t *)fej9->setJ2IThunk(methodSymbol->getMethod()->signatureChars(),
            methodSymbol->getMethod()->signatureLength(),
            TR::ARMCallSnippet::generateVIThunk(callNode, sizeOfArguments, codeGen), comp());
    ...
    if (methodSymbol->isVirtual()) {
        ...
        if (methodSymRef
            == comp()
                   ->getSymRefTab()
                   ->findObjectNewInstanceImplSymbol()) { // In this case, methodSymRef is resolved and VTable index is
                                                          // small enough to fit in 12bit, so we can safely use gr11 for
                                                          // classReg.
            classReg = gr11;
            generateTrg1MemInstruction(codeGen, TR::InstOpCode::ldr, callNode, gr11,
                new (trHeapMemory()) TR::MemoryReference(gr0, fej9->getOffsetOfClassFromJavaLangClassField(), codeGen));
        } else {
            classReg = vftReg;
        }
        TR::MemoryReference *tempMR;
        if (methodSymRef->isUnresolved() || comp()->compileRelocatableCode()) {
            ...
            TR::LabelSymbol *snippetLabel = generateLabelSymbol(codeGen);
            codeGen->addSnippet(new (trHeapMemory()) TR::ARMVirtualUnresolvedSnippet(codeGen, callNode, snippetLabel,
                sizeOfArguments, doneLabel, thunk));
            ...
            if (constantIsImmed12(offset)) {
                TR::Instruction *instr
                    = generateTrg1Src1Instruction(codeGen, TR::InstOpCode::mov, callNode, gr14, gr15);
                instr->setDependencyConditions(dependencies);
                dependencies->incRegisterTotalUseCounts(codeGen);
        ...
        codeGen->addSnippet(new (trHeapMemory())
                TR::ARMInterfaceCallSnippet(codeGen, callNode, snippetLabel, sizeOfArguments, doneLabel, thunk));
...
TR::Register *J9::ARM::PrivateLinkage::buildDirectDispatch(TR::Node *callNode)
{
    ...
    if (callSym->isJNI() && callNode->isPreparedForDirectJNI()) {
...
TR::Register *J9::ARM::PrivateLinkage::buildIndirectDispatch(TR::Node *callNode)
{
    ...
    TR::RegisterDependencyConditions *deps = new (trHeapMemory())
    ...
    switch (callNode->getOpCodeValue()) {
        ...
        case TR::fcalli:
            ...
            if (resType.isFloatingPoint()) {
                ...
                TR::Instruction *cursor
                    = generateTrg1Src1Instruction(codeGen, TR::InstOpCode::fmsr, callNode, tempReg, returnRegister);
...
#endif
        {
            ...
            if (resType.isDouble()) {
                ...
                TR::Instruction *cursor
                    = generateTrg1Src2Instruction(codeGen, TR::InstOpCode::fmdrr, callNode, tempReg, lowReg, highReg);
                returnRegister = tempReg;
            }
...
int32_t J9::ARM::HelperLinkage::buildArgs(TR::Node *callNode, TR::RegisterDependencyConditions *dependencies,
    TR::Register *&vftReg, bool isVirtual)
{
