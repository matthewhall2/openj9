...
static uint32_t getJitEntryOffset(J9::PrivateLinkage::LinkageInfo *linkageInfo)
{
...
TR_PersistentJittedBodyInfo *J9::Recompilation::getJittedBodyInfoFromPC(void *startPC)
{
    ...
    TR_PersistentJittedBodyInfo *info = linkageInfo->isRecompMethodBody()
        ? *(TR_PersistentJittedBodyInfo **)((uint8_t *)startPC + OFFSET_METHODINFO_FROM_STARTPC)
        : 0;
    return info;
}
...
bool J9::Recompilation::isAlreadyPreparedForRecompile(void *startPC)
{
    ...
    if (DEBUG_ARM_RECOMP) {
        printf("\nmethodPC 0x%x is %salready prepared for recompile\n", startPC,
            ((*jitEntry & 0x0e000000) == 0x0A000000) ? "" : "not ");
        fflush(stdout);
    }
...
void J9::Recompilation::fixUpMethodCode(void *startPC)
{
    ...
    if (linkageInfo->isCountingMethodBody()) {
        ...
        if (DEBUG_ARM_RECOMP) {
            printf(
                "fixUpMethodCode for sampling, jumpBackInst encoding is 0x%x from jitEntry 0x%x and encoding is 0x%x\n",
                jumpBackInstruction, jitEntry, *jitEntry);
            fflush(stdout);
        }
                ...
                if (_compareAndSwapSMP(jitEntry, preserved, jumpBackInstruction)) {
                    if (DEBUG_ARM_RECOMP) {
                        printf("\tCmpAndExchangeSMP passes jitEntry address = 0x%x oldInst = 0%x, newInst = 0x%x\n",
                            jitEntry, preserved, jumpBackInstruction);
                        fflush(stdout);
                    }
...
void J9::Recompilation::methodHasBeenRecompiled(void *oldStartPC, void *newStartPC, TR_FrontEnd *fe)
{
    ...
    if (linkageInfo->isCountingMethodBody()) {
        ...
        patchAddr = (int32_t *)((uint8_t *)oldStartPC + getJitEntryOffset(linkageInfo)
            + OFFSET_COUNTING_BRANCH_FROM_JITENTRY - 4);
        newInstr = encodeRuntimeHelperBranchAndLink(TR_ARMcountingPatchCallSite, (uint8_t *)patchAddr, fe);
        if (DEBUG_ARM_RECOMP) {
            printf("\tcounting recomp, change instruction location (0x%x) above blt to branch encoding 0x%x (to "
                   "TR_ARMcountingPatchCallSite)\n",
                patchAddr, newInstr);
            fflush(stdout);
        }
        *patchAddr = newInstr;
        ...
        if (DEBUG_ARM_RECOMP) {
            printf("\tsampling recomp, change instruction location (0x%x) of sampling branch to branch encoding 0x%x "
                   "(to TR_ARMsamplingPatchCallSite)\n",
                patchAddr, newInstr);
            fflush(stdout);
        }
        *patchAddr = newInstr;
...
void J9::Recompilation::methodCannotBeRecompiled(void *oldStartPC, TR_FrontEnd *fe)
{
        ...
        || bodyInfo->getIsInvalidated()) // TODO: reconsider whether this is a race cond for info
    {
        ...
        *patchAddr = newInstr;
        ...
        int32_t *branchLocation = (int32_t *)((uint8_t *)oldStartPC + getJitEntryOffset(linkageInfo)
            + OFFSET_COUNTING_BRANCH_FROM_JITENTRY);
        patchAddr = (int32_t *)((uint8_t *)oldStartPC + getJitEntryOffset(linkageInfo));
        ...
        *patchAddr = newInstr;
        armCodeSync((uint8_t *)patchAddr, 4);
        if (DEBUG_ARM_RECOMP) {
            printf("MethodCannotBeRecompiled counting recomp, branch over counting prologue by patching inst 0x%x at "
                   "location 0x%x\n",
                newInstr, patchAddr);
            fflush(stdout);
        }
        ...
        if (!fej9->isAsyncCompilation()) {
            ...
            if (DEBUG_ARM_RECOMP) {
                printf("MethodCannotBeRecompiled sampling recomp sync compilation restoring preserved jitEntry of 0x%x "
                       "at location 0x%x\n",
                    *((uint32_t *)((uint8_t *)oldStartPC + OFFSET_SAMPLING_PRESERVED_FROM_STARTPC)), startByte);
                fflush(stdout);
            }
...
extern bool armCodePatching(void *method, void *callSite, void *currentPC, void *currentTramp, void *newPC,
    void *extra);
extern "C" {
