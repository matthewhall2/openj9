...
uint8_t *TR_ARMRelocationTarget::eipBaseForCallOffset(uint8_t *reloLocation)
{
    // reloLocation points at the start of the call offset
    return reloLocation;
}

void TR_ARMRelocationTarget::storeCallTarget(uintptr_t callTarget, uint8_t *reloLocation)
{
...
uint32_t TR_ARMRelocationTarget::loadCPIndex(uint8_t *reloLocation)
{
...
uintptr_t TR_ARMRelocationTarget::loadThunkCPIndex(uint8_t *reloLocation)
{
    // reloLocation points at the cpAddress in snippet, need to find cpIndex location
...
void TR_ARMRelocationTarget::performThunkRelocation(uint8_t *thunkBase, uintptr_t vmHelper)
{
...
uint8_t *TR_ARMRelocationTarget::loadAddressSequence(uint8_t *reloLocation)
{
...
void TR_ARMRelocationTarget::storeAddressSequence(uint8_t *address, uint8_t *reloLocation, uint32_t seqNumber)
{
...
void TR_ARMRelocationTarget::storeRelativeTarget(uintptr_t callTarget, uint8_t *reloLocation)
{
    // reloLocation points at the start of the call offset, so just store the uint8_t * at reloLocation
...
{
    return !(((IDATA)(helperAddress - baseLocation) <= BRANCH_FORWARD_LIMIT)
        && ((IDATA)(helperAddress - baseLocation) >= BRANCH_BACKWARD_LIMIT));
}

uint8_t *TR_ARMRelocationTarget::arrayCopyHelperAddress(J9JavaVM *javaVM)
{
...
void TR_ARMRelocationTarget::flushCache(uint8_t *codeStart, unsigned long size)
{
