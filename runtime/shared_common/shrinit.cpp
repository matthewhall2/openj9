...
j9shr_init(J9JavaVM *vm, UDATA loadFlags, UDATA* nonfatal)
{
	...
	if (rcStartup != 0) {
		...
		config->storedGCHints = j9shr_storedGCHints;
...
j9shr_guaranteed_exit(J9JavaVM *vm, BOOLEAN exitForDebug)
{
	...
	if (vm && vm->sharedClassConfig && vm->sharedClassConfig->sharedClassCache) {
		...
		/* If j9shr_storeGCHints() is not invoked by the GC (via j9gc_storeGCHints)
		 * then storeStartupHintsToSharedCache does nothing.
		 */
		if (J9_IS_GCCONTAINERHEURISTICS(vm)
			...
			&& J9_ARE_NO_BITS_SET(vm->sharedClassConfig->localStartupHints.localStartupHintFlags, J9SHR_LOCAL_STARTUPHINTS_FLAG_HINTS_STORED)
		) {
			storeStartupHintsToSharedCache(vm->mainThread);
		}
...
generateStartupHintsKey(J9JavaVM* vm)
{
	...
	if (J9_IS_GCCONTAINERHEURISTICS(vm)
		...
		/* In portable mode, use the JVM version string as the key */
		keyLength = strlen(J9VM_VERSION_STRING) + 1; /* +1 for null terminator */
	...
	if (J9_ARE_ANY_BITS_SET(vm->sharedClassConfig->localStartupHints.localStartupHintFlags, J9SHR_LOCAL_STARTUPHINTS_FLAG_WRITE_HINTS)) {
		...
		if (NULL != key) {
			...
			if (NULL == ret) {
				...
				vm->sharedClassConfig->localStartupHints.localStartupHintFlags |= J9SHR_LOCAL_STARTUPHINTS_FLAG_HINTS_STORED;
...
BOOLEAN
j9shr_storedGCHints(J9VMThread *currentThread)
{
	BOOLEAN returnVal = FALSE;
	J9JavaVM *vm = currentThread->javaVM;

	if (J9_ARE_ANY_BITS_SET(vm->sharedClassConfig->localStartupHints.localStartupHintFlags, J9SHR_LOCAL_STARTUPHINTS_FLAG_HINTS_STORED)) {
		returnVal = TRUE;
	}
